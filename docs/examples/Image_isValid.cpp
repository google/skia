...
void draw(SkCanvas* canvas) {
    auto drawImage = [=](sk_sp<SkImage> image, const char* label) -> void {
        ...
        if (canvas->recordingContext()) {
            canvas->drawString(image->isValid(canvas->recordingContext()) ? "is valid on GPU" :
                    "not valid on GPU", 20, image->height() * 5 / 8, font, paint);
        }
        canvas->drawString(image->isValid(nullptr) ? "is valid on CPU" :
                "not valid on CPU", 20, image->height() * 7 / 8, font, paint);
    };
