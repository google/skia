...
SkPath MakeCubicSplineInterpolation(const SkPoint* pts, size_t N) {
    ...
    struct Scratch { SkPoint a, b, c, r, p; };
    // Can I do this will less allocation?
    std::unique_ptr<Scratch[]> s(new Scratch[n]);
    s[0].a = { 0, 0 };
    s[0].b = { 2, 2 };
    s[0].c = { 1, 1 };
    s[0].r = { pts[0].x() + 2 * pts[1].x(),
               pts[0].y() + 2 * pts[1].y() };
    for (size_t i = 1; i < n - 1; ++i) {
        s[i].a = { 1, 1 };
        s[i].b = { 4, 4 };
        s[i].c = { 1, 1 };
        s[i].r = { 4 * pts[i].x() + 2 * pts[i + 1].x(),
                   4 * pts[i].y() + 2 * pts[i + 1].y() };
    }
    s[n - 1].a = { 2, 2 };
    s[n - 1].b = { 7, 7 };
    s[n - 1].c = { 0, 0 };
    s[n - 1].r = { 8 * pts[n - 1].x() + pts[N - 1].x(),
                   8 * pts[n - 1].y() + pts[N - 1].y() };
    ...
    for (size_t i = 0; i < n - 1; i++) {
        SkPoint q = { 2 * pts[i + 1].x() - s[i + 1].p.fX,
                      2 * pts[i + 1].y() - s[i + 1].p.fY };
