...
void paintOctopus(int x, int y, int size_base, SkColor color, SkCanvas* canvas) {
    ...
    paint.setColor(SkColorSetARGB(
            SK_AlphaOPAQUE, SkColorGetR(color), SkColorGetG(color), SkColorGetB(color)));
    ...
    paint.setColor(SkColorSetRGB(
            SkColorGetR(color) + 20, SkColorGetG(color) + 20, SkColorGetB(color) + 20));
...
void draw(SkCanvas* canvas) {
    ...
    for (int i = 0; i < 400; ++i) {
        paintOctopus(rand.nextRangeScalar(0, 256), rand.nextRangeScalar(0, 256),
                     rand.nextRangeScalar(6, 12), rand.nextU(), canvas);
    }
}
}  // END FIDDLE
