...
void draw(SkCanvas* canvas) {
    const char* colors[] = {"Unknown",      "Alpha_8",  "RGB_565",   "ARGB_4444",
                            "RGBA_8888",    "RGB_888x", "BGRA_8888", "RGBA_1010102",
                            "RGBA_101010x", "Gray_8",   "RGBA_F16"};
    ...
    SkDebugf(
            "%16s Canonical    Unknown           Opaque            Premul            "
            "Unpremul\n",
            " ");
    for (SkColorType colorType :
        ...
        for (SkAlphaType canonicalAlphaType : alphaTypes) {
            ...
            SkDebugf("%15s %10s ", colors[(int)colorType], alphas[(int)canonicalAlphaType]);
