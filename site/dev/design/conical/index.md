...
We present a fast shader algorithm (compared to bruteforcely solving
the quadratic equation) for computing the two-point conical gradient (i.e., createRadialGradient in
[spec](https://html.spec.whatwg.org/multipage/canvas.html#dom-context-2d-createradialgradient)).

This document has 3 major parts: 1. Problem Statement and Setup; 2. Solving $x_t$; 3. Acceleration.
Part 1 is self-explanatory. Part 2 demonstrates the key theorem and formula. Part 3 shows how we
use the formula from part 2 to make our shader algorithm run faster.
...
it's valide (i.e., $x_t \geq 0$) iff $x \geq 0$.
