...
is just drawing it on top of itself. Subpixel anti-aliased glyph masks drawn with this guess
will be drawn without any adjustment at all, leaving them color fringy. On macOS Skia tweaks
the destination guess to reduce the correction when using bright sources. This helps reducing
...
Every step of this process is now automated in some way. However, there is a lot of hidden
knowledge in this process that wasn't really written down, so didn't really make it into the
design specification when automating. In this case consider the art of optical sizing and
managing thin features.

In general smaller type should be relatively heavier in weight than would be expected if taking
a larger size and just linearly scaling it down. The type designer will draw out the faces with
this in mind, potentially with an eye toward how it would vary with size and with potential need
...
One way the type designer can influence this without 'opsz' is through hinting. Any manual
...
Another thing the type designer may do is provide multiple font files with different optical sizes
and expect the user to select the right one (like Display and Text variants). In theory the right
one might also be selected automatically based on some criteria. Switching font files because of
nominal size changes may seem drastic, but this is how the system font on macOS worked for a while.
...
Another automatic way to do optical sizing is to just over-cover all the edges when doing
...
of the display, the visual acuity of the user, the distance of the user from the display, and
the user's sensitivity to the potential variations in stem darkness which may result. This can
all be short circuited by just giving the user a slider and having them look at various settings
and just picking one that looks the least bad to them.
...
Much like the gamma hack can be modified to reduce the correction for light on dark to fight
blooming, the contrast hack can also be reduced when the color being dawn is known to be
