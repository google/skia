...
If you call a method on a parent type that must stand out as being the parent's
version of that method, we usually typedef that parent type to `INHERITED`
privately within the class.  That lets calls like `INHERITED::onFoo()` stand
