...
CanvasKit is a wasm module that uses Skia to draw to canvas elements with better performance and more features than the standard API.

Minimal application
-------------------

This is a minimal Canvaskit application that draws a rounded rect for one frame.
...
<script type="text/javascript" src="https://unpkg.com/canvaskit-wasm@0.17.2/bin/canvaskit.js"></script>
<script type="text/javascript">
  const ckLoaded = CanvasKitInit({locateFile: (file) => 'https://unpkg.com/canvaskit-wasm@0.17.2/bin/'+file});
...
`<canvas id=foo width=300 height=300></canvas>` Creates the canvas which CanvasKit will draw to.
...
Hardware acceleration is the default behavior, but this can be overridden by calling
`MakeSWCanvasSurface` instead. This is also where alternative color spaces or gl 
...
```
Defines a function that will draw our frame. the function is provided an SkCanvas object which we
make draw calls on. One to clear the entire canvas, and one to draw the rounded rect with the
paint from above. Nothing happens until we call
...
which causes skia to batch and send WebGL commands, making visible changes appear onscreen. 
We also delete the paint object. CanvasKit objects created with `new` must be deleted for the
wasm memory to be released. Javascript's GC will not take care of it automatically. `rr` is just
an array, wasn't created with `new` and doesn't point to any WASM memory, so we don't have to call
delete on it. This example draws once and disposes of the surface. As promised, it is is a minimal
application.
...
(the browser will typicall target 60fps), our function is called, we clear the canvas with white, redraw the round rect,
...
used in all the same ways. If your application would only make visible changes as a result of mouse events,
...
```
Specifies the style of the text. The font's name, Roboto, will be used to fetch it from the font manager.
you can specify either (color) or (foregroundColor and backgroundColor) in order to have a highlight.
TODO(nifong): link to the full reference for ParagraphStyle / TextStyle
