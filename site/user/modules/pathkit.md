...
Example:
    ...
    let path = PathKit.FromSVGStr('M150 0 L75 200 L225 200 Z');
    // path represents a triangle
    // don't forget to do path.delete() when it goes out of scope.

#### `FromCmds(cmds)` ####
**cmds** - a 2D array of commands, where a command is a verb followed by its arguments.

Returns: an SkPath with the same verbs and arguments  or `null` on a failure.
