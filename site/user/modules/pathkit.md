...
#### `transform(matr)` ####
**matr** - `SkMatrix`, i.e. an `Array<Number>` of the 9 numbers of an Affine Transform Matrix.
...
**scaleX, skewX, transX, skewY, scaleY, transY, pers0, pers1, pers2** -
        `Number`, the 9 numbers of an Affine Transform Matrix.
...
### SkOpBuilder (object)  ###
This object allows to chain multiple PathOps together
Create with `let builder = new PathKit.SkOpBuilder();`
...
It basically takes a 9 element 1D Array and turns it into a
...
This are used in `PathKit.MakeFromOp()` and `path.op()`.
...
These are used by `path.getFillType()` and `path.setFillType()`, but
generally clients will want `path.getFillTypeString()`.
...
See See [SkPaint reference](https://skia.org/user/api/SkPaint_Reference#Stroke_Cap) for more details.
