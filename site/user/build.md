...
If you don't have authorization to use those assets, then see
infra/bots/assets/armhf_sysroot/README.md,
infra/bots/assets/chromebook_arm_gles/README.md and
infra/bots/assets/chromebook_x86_64_gles/README.md for instructions on creating those assets.
...
Once those files are in place, generate the GN args that resemble the following:
    ...
    cxx = "/opt/clang/bin/clang++"
    extra_cflags=[
        "-I/usr/local/google/home/kjlubick/skia/skia/out/__clang5/include/c++/v1/",
    ...
    extra_ldflags=[
        "-stdlib=libc++",
        "-fuse-ld=lld",
        "-L/opt/chromebook_x86_64_gles/include",
    ]
    target_cpu="x64"
    skia_use_fontconfig = false
    skia_use_system_freetype2 = false
...
Compile with ninja, as per usual.

Push the binary to a chromebook via ssh and run a command like the following:

    dm --undefok --resourcePath [dir] --images [dir] --nocpu  --config gles

Most chromebooks by default have their home directory parition marked as noexec.
