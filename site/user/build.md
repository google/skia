...
Alternatively you can generate an Xcode project by passing `--ide=xcode` to `bin/gn gen`. If you are using Xcode
version 10 or later, you'll need to go to `Workplace Settings...` and change `Build System:` to `Legacy Build System`.
You may find you still have issues with building or signing -- in that case you'll need to go to the `Signing and Capabilities`
settings for the target, uncheck `Automatically manage signing`, and fill in the bundle identifier and provisioning
