...
are matched 1 to 1 with OpenGL contexts or Vulkan devices. You should
should use the same GrContext for all SkCanvases that will be drawn using
the same OpenGL context. Skia does not create the OpenGL context for you.
It also assumes that the correct OpenGL context is bound to the current
thread when Skia calls are made.
...
<!--?prettify lang=cc?-->
    ...
    void gpu_example(int width, int height, void(*draw)(SkCanvas*), const char* path) {
