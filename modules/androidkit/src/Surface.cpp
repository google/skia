...
public:
    NativeSurface(ANativeWindow* win, std::unique_ptr<sk_app::WindowContext> wctx)
        : fWindow(win)
...
static jlong Surface_CreateVK(JNIEnv* env, jobject, jobject jsurface) {
    __android_log_print(ANDROID_LOG_ERROR, "*** AK", "Surface_CreateVK");
    ...
    return reinterpret_cast<jlong>(new NativeSurface(win, std::move(winctx)));
}
