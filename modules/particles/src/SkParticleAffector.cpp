...
constexpr SkParticleValue kRandomParticleValue =
    { SkParticleValue::kRandom_Source,
      kWorld_ParticleFrame,
      1.0f, 0.0f,
      SkParticleValue::kClamp_TileMode };

class SkPositionOnPathAffector : public SkParticleAffector {
public:
    SkPositionOnPathAffector(const char* path = "", bool setHeading = true,
                             SkParticleValue input = kRandomParticleValue)
            : fPath(path)
            , fInput(input)
            , fSetHeading(setHeading) {
        this->rebuild();
