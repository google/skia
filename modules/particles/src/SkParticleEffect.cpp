...
public:
    ...
    // TODO: Figure out how to handle the external values. Do we need to cache two per binding?
    // Can we share the external value (if the compiler is shared?) Ick.
    // Need an API to abstract re-binding the random seeds?

    void bind(SkSL::Compiler* compiler) override {
        class Value : public SkSL::ExternalValue {
        public:
            Value(const char* name, SkSL::Compiler& compiler, const SkCurve& curve)
                : INHERITED(name, *compiler.context().fFloat_Type)
                , fCompiler(compiler)
                , fCurve(curve)
                , fRandom(nullptr) {
        ...
        auto value = skstd::make_unique<Value>(fName.c_str(), *compiler, fCurve);
