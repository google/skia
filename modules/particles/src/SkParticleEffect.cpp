...
void SkParticleEffect::start(double now, bool looping) {
    ...
    // Every effect should define its own rate of emission, so leave that as zero initially.
    fState.fLifetime = 1.0f;
    fState.fRate = 0.0f;

    // Run optional effectSpawn to set initial spawn rate and other emitter properties
    // TODO: If we add burst, need to handle that here (defer burst to first update)
    if (const auto& byteCode = fParams->fEffectProgram.fByteCode) {
        if (auto fun = byteCode->getFunction("effectSpawn")) {
            ...
            SkAssertResult(byteCode->run(fun, &fState.fLifetime, nullptr, 1,
                                         &fState.fDeltaTime, 2));
        }
    }
}
...
void SkParticleEffect::update(double now) {
    ...
    // TODO: Sub-frame spawning. Tricky with script driven position. Supply variable effectAge?
    // Could be done if effectAge were an external value that offset by particle lane, perhaps.
    ...
    auto runProgram = [this](const SkParticleEffectParams* params, const char* entry,
        ...
        if (const auto& byteCode = params->fParticleProgram.fByteCode) {
            ...
            SkAssertResult(byteCode->runStriped(byteCode->getFunction(entry),
                                                ...
                                                &fState.fDeltaTime, 4, nullptr, 0));
        }
    };

    // Spawn new particles
    // TODO: Include most particle params in effect - use those as template for new particles.
    ...
    for (int i = 0; i < fCount; ++i) {
        ...
        SkScalar s = SkScalarSin(fParticles.fData[SkParticles::kVelocityAngular][i] * fState.fDeltaTime),
                 c = SkScalarCos(fParticles.fData[SkParticles::kVelocityAngular][i] * fState.fDeltaTime);
