...
SkParticleEffect::SkParticleEffect(sk_sp<SkParticleEffectParams> params, const SkRandom& random)
        ...
        , fEffectUniformCount(0)
        , fParticleUniformCount(0) {
    fState.fAge = -1.0f;
    ...
    if (const auto& byteCode = fParams->fEffectProgram.fByteCode) {
        if (auto fun = byteCode->getFunction(entry)) {
            ...
            SkAssertResult(byteCode->run(fun, &fState.fAge, sizeof(EffectState) / sizeof(float),
                                         nullptr, 0,
                                         fEffectUniforms.get(), fEffectUniformCount));
    ...
    if (const auto& byteCode = fParams->fParticleProgram.fByteCode) {
        if (auto fun = byteCode->getFunction(entry)) {
            ...
            SkAssertResult(byteCode->runStriped(fun, count, args, SkParticles::kNumChannels,
                                                nullptr, 0,
                                                fParticleUniforms.get(), fParticleUniformCount));
...
void SkParticleEffect::advanceTime(double now) {
    ...
    auto maybeRealloc = [](SkAutoTMalloc<float>* block, int* size, const SkSL::ByteCode* byteCode) {
        int newSize = byteCode ? byteCode->getUniformBlockSize() : *size;
        if (byteCode && *size != newSize) {
            block->realloc(newSize);
    ...
    maybeRealloc(&fEffectUniforms, &fEffectUniformCount, this->effectCode());
    maybeRealloc(&fParticleUniforms, &fParticleUniformCount, this->particleCode());
    ...
    // Spawn new particles
    float desired = fState.fRate * deltaTime + fSpawnRemainder;
