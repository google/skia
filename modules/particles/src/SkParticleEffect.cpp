...
// It's fast (integer mul + add). The modulus is 2^32, so we can just rely on uint overflow. All
// output bits are used, so again - it simplifies the usage here (although we shift down so that
// our float divisor retains precision).
...
void SkParticleEffect::start(double now, bool looping, SkPoint position, SkVector heading,
                             ...
                             float frame, uint32_t flags, uint32_t random) {
    fCount = 0;
    fLastTime = now;
    fSpawnRemainder = 0.0f;
    fLooping = looping;
