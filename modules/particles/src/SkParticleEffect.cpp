...
struct SkParticleProgram {
    ...
    // External values created by each SkParticleBinding
    std::vector<std::unique_ptr<SkSL::ExternalFunction>> fExternalValues;

    // Storage for uniforms generated by external values
    skvm::Uniforms fExternalValueUniforms;
...
void SkParticleEffectParams::prepare(const skresources::ResourceProvider* resourceProvider) {
    ...
    auto buildProgram = [this](const SkSL::String& code,
        ...
        // uniforms generated by any external values. These are managed with a Uniforms instance,
        ...
        skvm::Uniforms evUniforms(skvm::Ptr{0}, 0);
        ...
        for (const auto& binding : fBindings) {
            if (binding) {
                externalValues.push_back(binding->toValue(compiler, &evUniforms, alloc.get()));
            }
        }
        ...
        auto buildFunction = [&](const char* name){
            ...
            skvm::Builder b;
            skvm::Ptr evUniformPtr   = b.uniform(),  // aka evUniforms.base
        ...
        return std::unique_ptr<SkParticleProgram>(new SkParticleProgram{std::move(spawnProgram),
...
void SkParticleEffect::runEffectScript(EntryPoint entryPoint) {
    ...
    void* args[kNumEffectArgs
               + 1    // external value uniforms
               + 1];  // SkSL uniforms

    args[0] = fParams->fEffectProgram->fExternalValueUniforms.buf.data();
...
void SkParticleEffect::runParticleScript(EntryPoint entryPoint, int start, int count) {
    ...
    void* args[SkParticles::kNumChannels
               + 1    // external value uniforms
               + 1];  // SkSL uniforms
    args[0] = fParams->fParticleProgram->fExternalValueUniforms.buf.data();
