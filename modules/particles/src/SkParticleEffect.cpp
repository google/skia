...
void SkParticleEffectParams::prepare(const skresources::ResourceProvider* resourceProvider) {
    ...
    fEffectProgram.fInterpreter.reset(new SkSL::Interpreter<1>(std::move(effectProgram.first)));
...
void SkParticleEffect::runEffectScript(double now, const char* entry) {
    SkSL::Interpreter<1>* interpreter = fParams->fEffectProgram.fInterpreter.get();
    if (interpreter) {
        ...
        if (auto fun = byteCode.getFunction(entry)) {
            ...
            SkAssertResult(interpreter->run(fun, (SkSL::Interpreter<1>::Vector*) &fState.fAge,
                                            nullptr));
...
void SkParticleEffect::runParticleScript(double now, const char* entry, int start, int count) {
    ...
    if (interpreter) {
        ...
        if (auto fun = byteCode.getFunction(entry)) {
            ...
            SkAssertResult(interpreter->runStriped(fun, count, (float**) args, nullptr));
