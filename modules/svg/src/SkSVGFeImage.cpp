...
sk_sp<SkImageFilter> SkSVGFeImage::onMakeImageFilter(const SkSVGRenderContext& ctx,
    ...
    // Load image and map viewbox (image bounds) to viewport (filter effect subregion).
    ...
    SkRect srcRect = SkRect::Make(imgInfo.fImage->bounds());
    SkRect dstRect = imgInfo.fDst;
    auto imgfilt = SkImageFilters::Image(
            imgInfo.fImage, srcRect, dstRect, SkSamplingOptions(SkFilterMode::kLinear));
