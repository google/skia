...
static SkColorChannel GetColorChannel(SkSVGFeDisplacementMap::ChannelSelector channel) {
    switch (channel) {
        case SkSVGFeDisplacementMap::ChannelSelector::kR:
            return SkColorChannel::kR;
        case SkSVGFeDisplacementMap::ChannelSelector::kG:
            return SkColorChannel::kG;
        case SkSVGFeDisplacementMap::ChannelSelector::kB:
            return SkColorChannel::kB;
        case SkSVGFeDisplacementMap::ChannelSelector::kA:
...
sk_sp<SkImageFilter> SkSVGFeDisplacementMap::onMakeImageFilter(const SkSVGRenderContext& ctx,
    ...
    const SkColorChannel xChannel = GetColorChannel(fXChannelSelector);
    const SkColorChannel yChannel = GetColorChannel(fYChannelSelector);
    ...
    if (fctx.primitiveUnits().type() == SkSVGObjectBoundingBoxUnits::Type::kObjectBoundingBox) {
        ...
        // TODO: unclear if this is the right length scale factor
        const SkScalar lengthSize = SkScalarSqrt(objBounds.width() * objBounds.height());
        scale *= lengthSize;
    }

    return SkImageFilters::DisplacementMap(xChannel, yChannel, scale, in2, in, cropRect);
}
