...
sk_sp<SkImageFilter> SkSVGFeSpecularLighting::makePointLight(const SkSVGRenderContext& ctx,
    ...
    const SkColor color = this->resolveLightingColor(ctx);
    const SkRect cropRect = this->resolveFilterSubregion(ctx, fctx);
    const SkPoint3 loc = this->resolveXYZ(ctx, fctx, light->getX(), light->getY(), light->getZ());
    sk_sp<SkImageFilter> input =
            fctx.resolveInput(ctx, this->getIn(), this->resolveColorspace(ctx, fctx));

    return SkImageFilters::PointLitSpecular(loc, color, this->getSurfaceScale(), fSpecularConstant,
                                            fSpecularExponent, std::move(input), cropRect);
}
