...
SkSVGTextContext::PosAttrs SkSVGTextContext::ScopedPosResolver::resolve(size_t charIndex) const {
    ...
    if (charIndex < fLastPosIndex) {
        ...
        // Rotation semantics are interestingly different:
        //
        ...
        //
        // This means we need to discriminate between "strong" rotation (ancestor has an explicit
        // value for the given char) and "weak" rotation (ancestor has some values - but not for
        // the requested char - so we *may* use the last one)
        if (!fRotate.empty()) {
            if (localCharIndex < fRotate.size()) {
                // Strong rotation value overrides anything in the ancestor chain.
                attrs[PosAttrs::kRotate] = fRotate[localCharIndex];
                attrs.setWeakRotate(false);
            } else if (!attrs.has(PosAttrs::kRotate) || attrs.hasWeakRotate()){
                // Local weak rotation (last specified value) overrides ancestor weak rotation.
                attrs[PosAttrs::kRotate] = fRotate.back();
                attrs.setWeakRotate(true);
            }
        }
...
void SkSVGTextContext::flushChunk(const SkSVGRenderContext& ctx) {
    ...
    for (const auto& run : fRuns) {
        ...
        for (size_t i = 0; i < run.glyphCount; ++i) {
            ...
            const auto& rot = run.glyphRot[i];
            buf.xforms()[i] = SkRSXform::MakeFromRadians(/*scale=*/ 1,
                                                         SkDegreesToRadians(rot),
                                                         pos.fX, pos.fY, 0, 0);
        }
