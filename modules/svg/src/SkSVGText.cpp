...
public:
    ...
    public:
        ScopedPosResolver(const SkSVGTextContainer& txt, const SkSVGLengthContext& lctx,
            ...
            , fXPos(ResolveLengths(lctx, txt.getX(), SkSVGLengthContext::LengthType::kHorizontal))
            , fYPos(ResolveLengths(lctx, txt.getY(), SkSVGLengthContext::LengthType::kVertical))
        {
        ...
        PosAttrs resolve(size_t charIndex) const {
            ...
            if (charIndex < fLastPosIndex) {
                ...
                const auto hasAllLocal = localCharIndex < fXPos.size() &&
                                         localCharIndex < fYPos.size();
                if (!hasAllLocal && fParent) {
                    attrs = fParent->resolve(charIndex);
                }

                if (localCharIndex < fXPos.size()) {
                    attrs[PosAttrs::kX] = fXPos[localCharIndex];
                }
                if (localCharIndex < fYPos.size()) {
                    attrs[PosAttrs::kY] = fYPos[localCharIndex];
                }
