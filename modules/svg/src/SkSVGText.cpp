...
SkSVGTextContext::SkSVGTextContext(const SkSVGRenderContext& ctx, const BlobHandler& handler,
                                   const SkSVGTextPath* tpath)
    : fRenderContext(ctx)
    , fBlobHandler(handler)
...
void SkSVGTextContext::flushChunk(const SkSVGRenderContext& ctx) {
    ...
    for (const auto& run : fRuns) {
        ...
        fBlobHandler(ctx, blobBuilder.make(), run.fillPaint.get(), run.strokePaint.get());
    }
...
void SkSVGText::onRender(const SkSVGRenderContext& ctx) const {
    class BlobRenderer final : public SkSVGTextContext::BlobHandler {
        void operator()(const SkSVGRenderContext& ctx, const sk_sp<SkTextBlob>& blob,
                        const SkPaint* fill, const SkPaint* stroke) const override {
            if (fill) {
...
void SkSVGTextPath::onShapeText(const SkSVGRenderContext& ctx, SkSVGTextContext* parent_tctx,
    ...
    SkSVGTextContext tctx(ctx, parent_tctx->handler(), this);
