...
const SkSVGFilterContext::Result* SkSVGFilterContext::findResultById(const SkSVGStringType& id) const {
...
sk_sp<SkImageFilter> SkSVGFilterContext::ConvertFilterColorspace(sk_sp<SkImageFilter>&& input,
                                                                 SkSVGColorspace src,
                                                                 SkSVGColorspace dst) {
    if (src == SkSVGColorspace::kSRGB && dst == SkSVGColorspace::kLinearRGB) {
        return SkImageFilters::ColorFilter(SkColorFilters::SRGBToLinearGamma(), input);
    } else if (src == SkSVGColorspace::kLinearRGB && dst == SkSVGColorspace::kSRGB) {
        return SkImageFilters::ColorFilter(SkColorFilters::LinearToSRGBGamma(), input);
    } else {
        SkDebugf("Unhandled SVG colorspace conversion %d to %d\n", src, dst);
