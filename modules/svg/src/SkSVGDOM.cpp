...
sk_sp<SkSVGDOM> SkSVGDOM::Builder::make(SkStream& str) const {
    ...
    return sk_sp<SkSVGDOM>(new SkSVGDOM(std::move(root), std::move(fFontMgr), std::move(mapper)));
}

SkSVGDOM::SkSVGDOM(sk_sp<SkSVGNode> root, sk_sp<SkFontMgr> fmgr, SkSVGIDMapper&& mapper)
    : fRoot(std::move(root))
    , fFontMgr(std::move(fmgr))
    ...
    SkASSERT(fRoot && fRoot->tag() == SkSVGTag::kSvg);
    const SkSVGLengthContext lctx(SkSize::Make(0, 0));
    fContainerSize = static_cast<const SkSVGSVG*>(fRoot.get())->intrinsicSize(lctx);
}
