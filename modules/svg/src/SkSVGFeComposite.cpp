...
bool SkSVGFeComposite::parseAndSetAttribute(const char* name, const char* value) {
    return INHERITED::parseAndSetAttribute(name, value) ||
           // Defined in SkSVGFe.cpp:
           this->setInput2(SkSVGAttributeParser::parse<SkSVGFeInputType>("in2", name, value)) ||
...
sk_sp<SkImageFilter> SkSVGFeComposite::onMakeImageFilter(const SkSVGRenderContext& ctx,
    ...
    const sk_sp<SkImageFilter> background = fctx.resolveInput(ctx, fInput2);
    const sk_sp<SkImageFilter> foreground = fctx.resolveInput(ctx, this->getInput());
