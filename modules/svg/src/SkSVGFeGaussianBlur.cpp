...
bool SkSVGAttributeParser::parse<SkSVGFeGaussianBlur::StdDeviation>(
    ...
    SkSVGNumberType stdevX;
    if (!this->parse(&stdevX)) {
        return false;
    }

    SkSVGNumberType stdevY;
    this->parseCommaWspToken();
    if (this->parse(&stdevY)) {
        *stdDeviation = SkSVGFeGaussianBlur::StdDeviation({stdevX, stdevY});
    } else {
        *stdDeviation = SkSVGFeGaussianBlur::StdDeviation({stdevX, stdevX});
    }
