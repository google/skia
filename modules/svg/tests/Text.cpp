...
#include "modules/svg/src/SkSVGText.cpp"

DEF_TEST(Svg_Text_PosProvider, r) {
    #define L(x) SkSVGLength(x)
    const float N = PosAttrs()[PosAttrs::kX];
    ...
    auto test = [&](const PosTestDesc& tst) {
        ...
        SkSVGTextContext::ScopedPosResolver pa(*a.get(), lctx, &tctx, tst.offseta);
        SkSVGTextContext::ScopedPosResolver pb(*b.get(), lctx, &tctx, tst.offsetb);

        for (size_t i = 0; i < tst.expected.size(); ++i) {
            const auto& exp = tst.expected[i];
            auto pos = i >= tst.offsetb ? pb.resolve(i) : pa.resolve(i);

            REPORTER_ASSERT(r, pos[PosAttrs::kX] == exp.fX);
            REPORTER_ASSERT(r, pos[PosAttrs::kY] == exp.fY);
        }
    };
