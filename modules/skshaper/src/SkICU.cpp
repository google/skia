/*
 * Copyright 2021 Google Inc.
 *
 * Use of this source code is governed by a BSD-style license that can be
 * found in the LICENSE file.
 */

#include "modules/skshaper/include/SkICU.h"
#include "src/utils/SkUTF.h"

#include <unicode/ubidi.h>
#include <unicode/ubrk.h>
#include <unicode/uscript.h>
#include <unicode/ustring.h>
#include <unicode/utext.h>
#include <unicode/utypes.h>

#if defined(SK_USING_THIRD_PARTY_ICU)
#include "SkLoadICU.h"
#endif

static inline uint32_t bool2bit(bool pred, uint32_t bit) {
    return pred ? bit : 0;
}

static void handle_br_grapheme(unsigned request, int32_t status, uint32_t* outProp) {
    *outProp |= SkICU::kGraphemeBreak;
}

static void handle_br_word(unsigned request, int32_t status, uint32_t* outProp) {
    *outProp |= SkICU::kWordBreak;
}

static void handle_br_line(unsigned request, int32_t status, uint32_t* outProp) {
    *outProp |= (status == UBRK_LINE_HARD) ? SkICU::kHardLineBreak
                                           | SkICU::kSoftLineBreak;
}

static void handle_br_sentence(unsigned request, int32_t status, uint32_t* outProp) {
    *outProp |= SkICU::kSentenceBreak;
}

bool SkICU::ComputeProperties(SkSpan<const uint16_t> text16,
                              uint32_t requests,
                              SkSpan<uint32_t> properties) {
    sk_bzero(properties.data(), properties.size() * sizeof(uint32_t));

    if (requests & (kIsControl_Request | kIsSpace_Request | kIsWhiteSpace_Request)) {
        const uint16_t* u16Ptr = text16.data();
        const uint16_t* ut16End = u16Ptr + text.size();

        for (int i = 0; i < unichars.size(); ++i) {
            const int32_t uni = SkUTF::NextUTF16(&u16Ptr, u16End);
            uint32_t outProp = 0;

            if (requests & kIsControl) {
                outProp |= bool2bit(u_iscntrl(uni), kIsControl);
            }
            if (requests & kIsSpace) {
                outProp |= bool2bit(u_isspace(uni), kIsSpace);
            }
            if (requests & kWhiteSpaceBreak) {
                outProp |= bool2bit(u_isWhitespace(uni), kWhiteSpaceBreak);
            }

            outProperties[i] = outProp;
        }
    }

    const struct {
        uint32_t request,   // our request bit(s)
        unsigned icbr,      // ICU's enum
        void (*handler)(unsigned request, int32_t status, uint32_t* outProp);
    } breakRecs [] = {
        { kGraphemeBreak_Request, UBRK_CHARACTER, handle_br_grapheme },
        { kWordBreak_Request,     UBRK_WORD     , handle_br_word     },
        { kSentenceBreak_Request, UBRK_SENTENCE , handle_br_sentence },
        { kLineBreak_Request,     UBRK_LINE     , handle_br_line     },
    };
    for (auto rec : breakRecs) {
        if (requests & rec.request) {
            UErrorCode status = U_ZERO_ERROR;
            auto iter = ubrk_open(rec.icbr, locale, text16.data(), text16.data().size(), &status);
            if (U_FAILURE(status)) {
                return false;
            }

            ubrk_first(iter);
            int32_t pos = ubrk_first(iter);
            while (pos != UBRK_DONE) {
                rec.handler(rec.request, ubrk_getRuleStatus(iter), &outProperties[pos]);
                pos = ubrk_next(iter);
            }
            ubrk_close(iter);
        }
    }

    return true;
}

