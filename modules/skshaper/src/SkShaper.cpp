...
#ifdef SK_SHAPER_HARFBUZZ_AVAILABLE
    std::unique_ptr<SkShaper::BiDiRunIterator> bidi = SkShaper::MakeIcuBiDiRunIterator(utf8, utf8Bytes, bidiLevel);
    if (bidi) {
        return bidi;
    }
#endif
    ...
    void consume() override {
        ...
        // If not, try to find a fallback typeface
            ...
            if (candidate) {
                ...
                fCurrentFont = &fFallbackFont;
                SkString familyName;
                fFallbackFont.getTypeface()->getFamilyName(&familyName);
            } else {
                fCurrentFont = &fFont;
            }
        }
