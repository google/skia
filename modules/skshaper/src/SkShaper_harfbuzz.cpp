    ...
    while (runSegmenter.advanceRuns()) {  // For each item
        ...
        while (utf8Start < utf8End) {  // While there are still code points left in this item
            if (modelNeedsRegenerated) {
                ...
                SkVector advance = {0, 0};
                modelText.reset(new TextProps[utf8End - utf8Start + 1]());
                ...
                modelText[utf8End - utf8Start].glyphLen = model.fNumGlyphs;
                modelText[utf8End - utf8Start].advance = model.fAdvance;
                modelTextOffset = 0;
