...
static void handle_line(SkShaper::LineHandler* handler, const ShapedRun& run, int start, int end,
                        SkPoint* p) {
    unsigned len = end - start;

    static constexpr size_t kInlineStorage = 256;
    SkSTArray<kInlineStorage, SkGlyphID, true> glyphs(len);
    SkSTArray<kInlineStorage, SkPoint  , true> pos(len);
    SkSTArray<kInlineStorage, uint32_t , true> clusters(len);
    ...
    for (unsigned i = 0; i < len; i++) {
        ...
        glyphs.push_back(glyph.fID);
        pos.push_back(SkPoint::Make(p->fX + glyph.fOffset.fX, p->fY - glyph.fOffset.fY));
        clusters.push_back(glyph.fCluster);

        p->fX += glyph.fAdvance.fX;
        p->fY += glyph.fAdvance.fY;
    }

    (*handler)(glyphs.data(), pos.data(), clusters.data(), len);
}
    ...
    SkPoint currentPoint = point;
{
    ...
    while (glyphIterator.current()) {
        ...
        for (int i = 0; i < numRuns; ++i) {
            ...
            handle_line(handler, runs[logicalIndex], startGlyphIndex, endGlyphIndex, &currentPoint);
        }
