...
public:
    ShaperHarfBuzz(std::unique_ptr<SkUnicode>, SkUnicodeBreak, SkUnicodeBreak, HBBuffer, sk_sp<SkFontMgr>);
...
std::unique_ptr<SkShaper> SkShaper::MakeShapeDontWrapOrReorder(sk_sp<SkFontMgr> fontmgr) {
    ...
    return std::make_unique<ShapeDontWrapOrReorder>(std::move(unicode), nullptr, nullptr, std::move(buffer), std::move(fontmgr));
}
