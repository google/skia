...
// Iterate over the glyphs in logical order to mark line endings.
{
    ...
    /** The position of the candidate break. */
    ShapedRunGlyphIterator candidateBreak(runs);
    SkScalar candidateBreakWidth = 0;
    ...
    while (ShapedGlyph* glyph = current.current()) {
        if (current != beginning) {
            bool anyBreakOk = candidateBreak == beginning ||
                              (!candidateBreak.current()->fGraphemeBreakBefore &&
                               !candidateBreak.current()->fMayLineBreakBefore);
            bool graphemeBreakOk = candidateBreak == beginning ||
                                   !candidateBreak.current()->fMayLineBreakBefore;
            if ( glyph->fMayLineBreakBefore ||
                (graphemeBreakOk && glyph->fGraphemeBreakBefore) ||
                anyBreakOk)
            {
                candidateBreak = current;
                candidateBreakWidth = currentWidth;
            }
        }
        SkScalar glyphWidth = glyph->fAdvance.fX;
        if (currentWidth + glyphWidth <= width || !glyph->fHasVisual || currentWidth == 0) {
