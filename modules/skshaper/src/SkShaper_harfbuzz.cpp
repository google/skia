...
static std::unique_ptr<SkShaper> MakeHarfBuzz(sk_sp<SkFontMgr> fontmgr, bool correct) {
    ...
    auto lineIter = unicode->makeBreakIterator("th", nullptr, 0,SkUnicode::BreakType::kLines);
    if (!lineIter) {
        return nullptr;
    }
    auto graphIter = unicode->makeBreakIterator("th", nullptr, 0,SkUnicode::BreakType::kGraphemes);
    if (!graphIter) {
        return nullptr;
    }
                               ...
                               RunHandler* handler) const
{
    ...
    while (runSegmenter.advanceRuns()) {  // For each item
        ...
        while (utf8Start < utf8End) {  // While there are still code points left in this item
            ...
            {
                auto text = fUnicode->makeText(utf8Start, utf8runLength);
                if (!text || !fLineBreakIterator->setText(text.get())) {
                    return;
                }
            }
...
{
    {
        auto text = fUnicode->makeText(utf8, utf8Bytes);
        if (!text) {
            return;
        }
        if (!fLineBreakIterator->setText(text.get())) {
            return;
        }
        if (!fGraphemeBreakIterator->setText(text.get())) {
            return;
        }
    }
