...
public:
    static SkTLazy<BiDiRunIterator> Make(const char* utf8, size_t utf8Bytes, UBiDiLevel level) {
        ...
        ret.init(utf8, utf8Bytes, std::move(bidi));
        return ret;
    }
    BiDiRunIterator(const char* utf8, size_t utf8Bytes, ICUBiDi bidi)
        : fBidi(std::move(bidi))
        , fEndOfCurrentRun(utf8)
        , fEndOfAllRuns(utf8 + utf8Bytes)
    ...
    SkTArray<ShapedRun> runs;
{
    ...
    while (runSegmenter.advanceRuns()) {
        ...
        while (utf8Current < utf8End) {
            ...
            hb_codepoint_t u = SkUTF8_NextUnichar(&utf8Current, end);
