                                  ...
                                  Feature const * const features, size_t const featuresSize) const
{
    ...
    static SkLRUCache<SkFontID, HBFace> gCache(100);
    static SkMutex gCacheMutex;
    HBFace* hbFaceCached;
    {
        SkAutoMutexExclusive lock(gCacheMutex);
        SkFontID dataId = font.currentFont().getTypeface()->uniqueID();
        hbFaceCached = gCache.find(dataId);
        if (!hbFaceCached) {
            HBFace hbFace(create_hb_face(*font.currentFont().getTypeface()));
            hbFaceCached = gCache.insert(dataId, std::move(hbFace));
        }
    }
