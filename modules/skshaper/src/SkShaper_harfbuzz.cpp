...
ShaperHarfBuzz::ShaperHarfBuzz(HBBuffer buffer, sk_sp<SkFontMgr> fontmgr)
    : fLineBreakIterator(std::move(fUnicode->makeBreakIterator("th", nullptr, 0, UBreakType::kLines)))
    , fGraphemeBreakIterator(std::move(fUnicode->makeBreakIterator("th", nullptr, 0, UBreakType::kGraphemes)))
                               ...
                               RunHandler* handler) const
{
    ...
    while (runSegmenter.advanceRuns()) {  // For each item
        ...
        while (utf8Start < utf8End) {  // While there are still code points left in this item
            ...
            if (!breakIterator.setUtf8Text(utf8Start, utf8runLength, {})) {
                return;
            }
    ...
    SkTArray<ShapedRun> runs;
{
    ...
    if (!SkBreakIterator::setUtf8Text(utf8, utf8Bytes, { &lineBreakIterator, &graphemeBreakIterator })) {
        return;
    }
