...
public:
    ShaperHarfBuzz(HBBuffer, sk_sp<SkFontMgr>);
...
static std::unique_ptr<SkShaper> MakeHarfBuzz(sk_sp<SkFontMgr> fontmgr, bool correct) {
    ...
    if (correct) {
        return std::make_unique<ShaperDrivenWrapper>(std::move(buffer), std::move(fontmgr));
    } else {
        return std::make_unique<ShapeThenWrap>(std::move(buffer), std::move(fontmgr));
    }
}

ShaperHarfBuzz::ShaperHarfBuzz(HBBuffer buffer, sk_sp<SkFontMgr> fontmgr)
    : fUnicode(SkUnicode::Make())
    , fLineBreakIterator(std::move(fUnicode->makeBreakIterator("th", nullptr, 0, SkUnicode::UBreakType::kLines)))
    , fGraphemeBreakIterator(std::move(fUnicode->makeBreakIterator("th", nullptr, 0, SkUnicode::UBreakType::kGraphemes)))
    ...
    while (runSegmenter.advanceRuns()) {  // For each item
        ...
        while (utf8Start < utf8End) {  // While there are still code points left in this item
            ...
            for (int32_t breakIteratorCurrent = breakIterator.next();
                 breakIteratorCurrent != breakIterator.isDone();
                         ...
                         RunHandler* handler) const
{
    SkTArray<ShapedRun> runs;
{
    if (!fLineBreakIterator->setUtf8Text(utf8, utf8Bytes, { fGraphemeBreakIterator.get() })) {
        return;
    }
    ...
    while (runSegmenter.advanceRuns()) {
        ...
        for (size_t i = 0; i < run.fNumGlyphs; ++i) {
            ...
            while (lineBreakIteratorCurrent != lineBreakIterator.isDone() &&
            ...
            while (graphemeBreakIteratorCurrent != graphemeBreakIterator.isDone() &&
...
std::unique_ptr<SkShaper> SkShaper::MakeShapeDontWrapOrReorder(sk_sp<SkFontMgr> fontmgr) {
    ...
    return std::make_unique<ShapeDontWrapOrReorder>(std::move(buffer), std::move(fontmgr));
}
