...
// Reorder the runs and glyphs per line and write them out.
{
    ...
    while (glyphIterator.current()) {
        ...
        struct SubRun { const ShapedRun& run; size_t start; size_t end; };
        ...
        auto makeRunInfo = [](const ShapedRun& run, size_t startGlyphIndex, size_t endGlyphIndex) {
            uint32_t startUtf8 = run.fGlyphs[startGlyphIndex].fCluster;
            uint32_t endUtf8 = (endGlyphIndex < run.fNumGlyphs)
                             ? run.fGlyphs[endGlyphIndex].fCluster
                             : run.fUtf8Range.end();

            SkVector advance = SkVector::Make(0, 0);
            for (size_t i = startGlyphIndex; i < endGlyphIndex; ++i) {
                advance += run.fGlyphs[i].fAdvance;
            }
        ...
        for (int i = 0; i < numRuns; ++i) {
            ...
            handler->runInfo(makeRunInfo(sub.run, sub.start, sub.end));
        }
        handler->commitRunInfo();
        for (int i = 0; i < numRuns; ++i) {
            SubRun sub = makeSubRun(i);
            append(handler, makeRunInfo(sub.run, sub.start, sub.end), sub.run, sub.start, sub.end);
        }
