                           ...
                           RunHandler* handler) const
{
    ...
    this->wrap(utf8, utf8Bytes, bidi, language, script, font, runSegmenter, features, featuresSize, width, handler);
}
                                  ...
                                  Feature const * const features, size_t const featuresSize) const
{
    ...
    for (const auto& feature : SkMakeSpan(features, featuresSize)) {
        if (feature.end < SkTo<size_t>(utf8Start - utf8) || SkTo<size_t>(utf8End - utf8)  <= feature.start) {
            continue;
        }
        if (feature.start <= SkTo<size_t>(utf8Start - utf8) && SkTo<size_t>(utf8End - utf8) <= feature.end) {
            hbFeatures.push_back({ (hb_tag_t)feature.tag, feature.value, HB_FEATURE_GLOBAL_START, HB_FEATURE_GLOBAL_END});
        } else {
            hbFeatures.push_back({ (hb_tag_t)feature.tag, feature.value, SkTo<unsigned>(feature.start), SkTo<unsigned>(feature.end)});
        }
    }
