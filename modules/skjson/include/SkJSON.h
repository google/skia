...
public:
    ...
    /**
     * Unguarded coversion to facade types.
     *
    ...
    /**
     * Guarded coversion to facade types.
     *
     * @return    The record cast as facade type T*.
     */
    template <typename T>
...
private:
    ...
    static constexpr size_t kTagOffset = sizeof(fData8) - 1;
...
public:
    ...
    static Value Make() { return NullValue(); }
...
public:
    ...
    static Value Make(bool b) { return BoolValue(b); }
...
public:
    ...
    static Value Make(int32_t i) { return NumberValue(i); }
    static Value Make(float   f) { return NumberValue(f); }
...
protected:
    VectorValue(Tag t, void *p) : INHERITED(t, p) {}
...
public:
    static ArrayValue Make(const Value* src, size_t size, SkArenaAlloc& alloc) {
        return ArrayValue(src, size, alloc);
    }
...
public:
    ...
    static StringValue MakeEmpty();
    static StringValue Make(const char* src, size_t size, SkArenaAlloc& alloc);
...
private:
    ...
    StringValue(const char* src, size_t size);
...
public:
    static ObjectValue Make(const Member* src, size_t size, SkArenaAlloc& alloc) {
        return ObjectValue(src, size, alloc);
    }
