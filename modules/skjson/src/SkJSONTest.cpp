...
DEF_TEST(SkJSON_DOM_build, reporter) {
    ...
    const auto v0  = NullValue::Make();
    check_value<NullValue>(reporter, v0, "null");

    const auto v1  = BoolValue::Make(true);
    check_value<BoolValue>(reporter, v1, "true");

    const auto v2  = BoolValue::Make(false);
    check_value<BoolValue>(reporter, v2, "false");

    const auto v3  = NumberValue::Make(0);
    check_value<NumberValue>(reporter, v3, "0");

    const auto v4  = NumberValue::Make(42);
    check_value<NumberValue>(reporter, v4, "42");

    const auto v5  = NumberValue::Make(42.75f);
    check_value<NumberValue>(reporter, v5, "42.75");

    const auto v6  = StringValue::Make(nullptr, 0, alloc);
    check_value<StringValue>(reporter, v6, "\"\"");

    const auto v7  = StringValue::Make(" foo ", 5, alloc);
    check_value<StringValue>(reporter, v7, "\" foo \"");

    const auto v8  = StringValue::Make(" foo bar baz ", 13, alloc);
    check_value<StringValue>(reporter, v8, "\" foo bar baz \"");

    const auto v9  = ArrayValue::Make(nullptr, 0, alloc);
    check_value<ArrayValue>(reporter, v9, "[]");

    const Value values0[] = { v0, v3, v9 };
    const auto v10 = ArrayValue::Make(values0, SK_ARRAY_COUNT(values0), alloc);
    check_value<ArrayValue>(reporter, v10, "[null,0,[]]");

    const auto v11 = ObjectValue::Make(nullptr, 0, alloc);
    check_value<ObjectValue>(reporter, v11, "{}");

    const Member members0[] = {
        { StringValue::Make("key_0", 5, alloc), v1  },
        { StringValue::Make("key_1", 5, alloc), v4  },
        { StringValue::Make("key_2", 5, alloc), v11 },
    };
    const auto v12 = ObjectValue::Make(members0, SK_ARRAY_COUNT(members0), alloc);
    ...
    const auto v13 = ArrayValue::Make(values1, SK_ARRAY_COUNT(values1), alloc);
    ...
    const Member members1[] = {
        { StringValue::Make("key_00", 6, alloc), v5  },
        { StringValue::Make("key_01", 6, alloc), v7  },
        { StringValue::Make("key_02", 6, alloc), v13 },
    };
    const auto v14 = ObjectValue::Make(members1, SK_ARRAY_COUNT(members1), alloc);
