   ...
   it('can readPixels from an SkImage', (done) => {
    ...
    describe('ColorSpace Support', () => {
        ...
        it('combine draws from several color spaces', () => {
            ...
            canvas.drawPaint(paint);
            paint.setColor(CanvasKit.RED, displayP3); // 93.7
            canvas.drawRect(CanvasKit.LTRBRect(200, 0, 400, 600), paint);
            paint.setColor(CanvasKit.RED, sRGB); // 85.9
            ...
            // reportSurface cannot handle f16 buffer yet, check correctness here.
            // readPixels gives a Uint8Array. For this surface, for one pixel, it's
            // length should be 16 bytes or 4 32-bit floats. Skia is opting to use 32 bits even though
            // we asked to use fewer, and if we don't parrot that implementation detail back to it,
            // we can't copy out the pixels.
            const pixel = surface.getCanvas().readPixels(100, 300, 1, 1,
                CanvasKit.AlphaType.Unpremul, CanvasKit.ColorType.RGBA_F16, adobeRGB);
            expect(pixel.length).toEqual(8);
