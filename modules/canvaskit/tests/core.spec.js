    ...
    it('can use various image filters', function(done) {
        ...
        Promise.all([imgPromise, LoadCanvasKit]).then((values) => {
            ...
            catchException(done, () => {
                ...
                const m = CanvasKit.SkMatrix.rotated(-Math.PI/6, 200, 200);
                const imgIF = CanvasKit.SkImageFilter.MakeImage(img);
                const rotateIF = CanvasKit.SkImageFilter.MakeMatrixTransform(m, CanvasKit.FilterQuality.Medium, imgIF);

                const blurIF = CanvasKit.SkImageFilter.MakeBlur(8, 0.2, CanvasKit.TileMode.Decal, null);

                const combined = CanvasKit.SkImageFilter.MakeComposed(blurIF, rotateIF);
