describe('Core canvas behavior', () => {
   ...
   it('can readPixels from an SkImage', (done) => {
    ...
    gm('image_decoding_methods', async (canvas) => {
        ...
        let row = 1;
        // Test 4 different methods of decoding an image for each of the three images in IMAGE_FILE_PATHS.
        ...
        for (const imageFilePath of IMAGE_FILE_PATHS) {
            ...
            //   Testing showed that transferring an ImageBitmap to a canvas using the 'bitmaprenderer' context
            //   and passing that canvas to CanvasKit.MakeImageFromCanvasImageSource() is marginally faster
            //   than passing ImageBitmap to CanvasKit.MakeImageFromCanvasImageSource() directly.
            ...
            const skImage2 = await CanvasKit.MakeImageFromCanvasImageSource(canvasBitmapElement);
            ...
            await promise1;
            const skImage3 = await CanvasKit.MakeImageFromCanvasImageSource(image);

            // Method 4 (roundabout, but works if all you have is a Blob) decode from Blob using HTMLImageElement:
            const imageObjectUrl = URL.createObjectURL( blob );
            const image2 = new Image();
            const promise2 = new Promise((resolve) => image2.addEventListener('load', resolve));
            image2.src = imageObjectUrl;
            await promise2;
            const skImage4 = await CanvasKit.MakeImageFromCanvasImageSource(image2);

            // Draw decoded images
            canvas.drawImageRect(skImage1, CanvasKit.XYWHRect(0,0, 150, 150), CanvasKit.XYWHRect(0,row * 100, 90, 90), null, false);
            canvas.drawImageRect(skImage2, CanvasKit.XYWHRect(0,0, 150, 150), CanvasKit.XYWHRect(100,row * 100, 90, 90), null, false);
            canvas.drawImageRect(skImage3, CanvasKit.XYWHRect(0,0, 150, 150), CanvasKit.XYWHRect(200,row * 100, 90, 90), null, false);
            canvas.drawImageRect(skImage4, CanvasKit.XYWHRect(0,0, 150, 150), CanvasKit.XYWHRect(300,row * 100, 90, 90), null, false);
