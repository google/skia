...
public:
    WasmWebGlTestContext(GrGLStandard forcedGpuAPI, GLTestContext* shareContext) {}
    ...
    std::unique_ptr<GLTestContext> makeNew() const override {
        std::unique_ptr<sk_gpu_test::GLTestContext> ctx(
            new WasmWebGlTestContext(this->gl()->fStandard, nullptr));
        if (ctx) {
            ctx->makeCurrent();
        }
        return ctx;
    ...
    // These are no-ops and that seems to work fine.
    // I didn't know what they're supposed to do, but it's possible that more tests will
    // pass if they are correctly implemented.
