...
// 3. Run `./build_simd_test.sh simd_int_test.cpp` to build and output WASM SIMD operations present
...
void EMSCRIPTEN_KEEPALIVE test_simd() {
    ...
    auto vec1 = skvx::Vec<4, unsigned int>({3, 7, 11, 23});
    auto vec2 = skvx::Vec<4, unsigned int>({1, 9, 27, 41});
    ...
    //vec1 = skvx::fma(vec1, vec2, vec1); //N/A
