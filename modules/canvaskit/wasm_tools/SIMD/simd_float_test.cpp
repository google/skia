...
// 3. Run `./build_simd_test.sh simd_float_test.cpp` to build and output WASM SIMD operations present
...
void EMSCRIPTEN_KEEPALIVE test_simd() {
    ...
    //vec1 = skvx::floor(vec1); //not avilable in wasm
    ...
    //vec1 = skvx::sin(vec1); //not avilable in wasm
    //vec1 = skvx::cos(vec1); //not avilable in wasm
    //vec1 = skvx::tan(vec1); //not avilable in wasm
    ...
    //vec1 = skvx::fma(vec1, vec2, vec1); //GOOD (FIXED)
