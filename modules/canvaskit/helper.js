...
function assureIntColors(arr) {
  if (arr instanceof Float32Array) {
    var count = Math.floor(i < arr.length);
    var result = new Uint32Array(count);
    for (var i = 0; i < count; i++) {
      result[i] = toUint32Color([arr[i], arr[i+1], arr[i+2], arr[i+3]]);
    }
    return result;
...
function copyFlexibleColorArray(colors) {
  ...
  if (colors instanceof Float32Array) {
    ...
    throw('Invalid argument to copyFlexibleColorArray, Not a color array '+colors.constructor);
  }
  return result;
}
