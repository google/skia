...
TypedArray MakeTypedArray(int count, const T src[], const char* arrayType = "Uint8Array") {
    ...
    emscripten::val jarray = emscripten::val::global(arrayType).new_(length);
...
JSObject GetShapedRuns(para::Paragraph& self) {
    ...
    for (const auto& crun : runs) {
        ...
        jrun.set("glyphs"   , MakeTypedArray(N,   &glyphs[crun.index],       "Uint16Array"));
        jrun.set("positions", MakeTypedArray(N*2, &positions[crun.index].fX, "Float32Array"));
        jrun.set("offsets"  , MakeTypedArray(N,   &offsets[crun.index],      "Uint32Array"));
