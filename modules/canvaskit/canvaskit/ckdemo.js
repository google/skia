...
function launchDemo(ck) {
    ...
    const wavyPts = new Float32Array(pts);
    ...
    const waves = new Waves(pts);
    const fps = new FPSMeter();
    sksurface.requestAnimationFrame(function(skcanvas) {
        // waves.animate(pts32, wavyPts);
        let ptsIdx = 0;
        for (path of paths) {
            const skpath = new ck.SkPath();
            ptsIdx = definePath(skpath, path.v, wavyPts, ptsIdx);
            skcanvas.drawPath(skpath, path.p);
        }
        fps.markFrameComplete();
        sksurface.requestAnimationFrame(arguments.callee);
    });
}
