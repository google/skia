...
function parsePaths(ck, node, paths, pts) {
    ...
    if (node.nodeName == "path") {
        ...
        paths.push({
            verbs:verbs, ckVerbs:mVerbs, numPts:numPts, paint:skpaint, ckpath: new ck.SkPath(),
        });
    }
    for (child of node.childNodes) {
...
function launchDemo(ck) {
    ...
    const drawFrame = (skcanvas) => {
        ...
        for (const path of paths) {
            ...
            path.ckpath.rewind();
            path.ckpath.setVerbsAndPoints(path.ckVerbs,
              mWavyPts.subarray(ptsIdx, ptsIdx + path.numPts*2));

            ptsIdx += path.numPts*2;
            skcanvas.drawPath(path.ckpath, path.paint);
        }
