  ...
  CanvasKit._extraInitializations.push(function() {
    ...
    CanvasKit.TextStyle = function(s) {
      ...
      if (s['shadows']) {
        ...
        const shadowColors = shadows.map(s => s['color'] || CanvasKit.BLACK);
        const shadowOffsets = shadows.map(s => s['offset'] || [0, 0]);
        const shadowBlurRadii = shadows.map(s => s['blurRadius'] || 0.0);
      ...
      if (s['fontFeatures']) {
        const fontFeatures = s['fontFeatures'];
        const fontFeatureNames = fontFeatures.map(s => s['name']);
        const fontFeatureValues = fontFeatures.map(s => s['value']);
        s['_fontFeatureLen'] = fontFeatures.length;
        s['_fontFeatureNamesPtr'] = naiveCopyStrArray(fontFeatureNames);
        s['_fontFeatureValuesPtr'] = copy1dArray(fontFeatureValues, "HEAPF32");
      } else {
        s['_fontFeatureLen'] = 0;
    ...
    CanvasKit.ParagraphBuilder.prototype.addPlaceholder = function(style) {
      this._addPlaceholder(placeholderStyle(style));
    }
});
