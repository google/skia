...
if [[ $@ == *no_font* ]]; then
  ...
  FONT_JS=""
  GN_FONT+="skia_enable_fontmgr_custom_embedded=false skia_enable_fontmgr_custom_empty=false skia_fontmgr_factory=\"empty\""
elif [[ $@ == *no_embedded_font* ]]; then
  echo "Omitting the built-in font(s)"
  GN_FONT+="skia_enable_fontmgr_custom_embedded=false skia_enable_fontmgr_custom_empty=true skia_fontmgr_factory=\"empty-freetype\""
else
  ...
  GN_FONT+="skia_enable_fontmgr_custom_embedded=true skia_enable_fontmgr_custom_empty=false skia_fontmgr_factory=\"embedded-freetype\""
fi
