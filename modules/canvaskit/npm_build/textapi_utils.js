...
function MakeStyle(length) {
    return {
        ...
        mergeFrom: function(src) {
            ...
            if (src.paint)    { this.paint  = src.paint; }
...
function styles_find_first_index(styles, start) {
...
function MakeEditor(text, style, cursor, width) {
        ...
        _validateStyles: function() {
            ...
            for (const s of this._styles) {
                ...
                if (len > this._text.length) {
                    console.log('exceed style lengths at', s, 'by', len - this._text.length);
                    throw "";
                }
            }
        },
    ...
    //              this._font.setTypeface(r.typeface); // r.typeface is always null (for now)
        ...
        applyStyleToRange: function(style, start, end) {
            ...
            if (end > 0) {
                ...
                ns._length = end;
                s._length -= end;                   // trim the (unchanged) 2nd half
                console.log('merging tail at', i, 'for', end);
                layoutChanged |= ns.mergeFrom(style);
                console.log('final splice at', i, ns);
    ...
    console.log(ed);
    console.log(ed._text.length);

    const s = MakeStyle(ed._text.length);
    s.mergeFrom(style);
    console.log('style0', s);
    let tmp = [ s ];
    console.log(tmp.length);
    console.log('initial styless', tmp);
    console.log(tmp.length);
    ed._styles = tmp;
    ed._buildLines();
    return ed;
