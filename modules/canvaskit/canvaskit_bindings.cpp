...
EMSCRIPTEN_BINDINGS(Skia) {
    ...
    class_<SkImage>("Image")
        ...
        .function("_makeShaderCubic", optional_override([](sk_sp<SkImage> self,
            ...
            SkCubicResampler cr {B, C};
            SkSamplingOptions so(cr);
            return self->makeShader(tx, ty, so, &localMatrix);
        }), allow_raw_pointers())
        .function("_makeShaderOptions", optional_override([](sk_sp<SkImage> self,
                                 SkTileMode tx, SkTileMode ty,
                                 SkFilterMode sample, SkMipmapMode mipmap,
                                 uintptr_t /* SkScalar*  */ mPtr)->sk_sp<SkShader> {
            OptionalMatrix localMatrix(mPtr);
            SkSamplingOptions so(sample, mipmap);
            // TODO(kjlubick, reed) Do we need to do something with Mipmap builder?
            return self->makeShader(tx, ty, so, &localMatrix);
        }), allow_raw_pointers())
