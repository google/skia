...
// TODO(nifong): consider folding this function into drawshadow to eliminate all the transformations.
// Accepts 8 floats, first four are ambient color, next four are spot color
Float32Array _computeTonalColors(uintptr_t cPtrAmbi /* float * */, uintptr_t cPtrSpot /* float * */) {
    ...
    // returning colors is done via Float32Array, but we can't put that in a struct, so return 8 floats
    const float outArray[] = {
        ambi4f.fR, ambi4f.fG, ambi4f.fB, ambi4f.fA,
        spot4f.fR, spot4f.fG, spot4f.fB, spot4f.fA,
    };
    return Float32Array(typed_memory_view(8, outArray));
}
...
EMSCRIPTEN_BINDINGS(Skia) {
    ...
    function("_computeTonalColors", &_computeTonalColors);
