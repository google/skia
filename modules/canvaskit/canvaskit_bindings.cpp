...
EMSCRIPTEN_BINDINGS(Skia) {
    ...
    class_<SkRuntimeEffect>("SkRuntimeEffect")
        ...
        .function("_makeShader", optional_override([](SkRuntimeEffect& self,
            uintptr_t fptr, size_t len, bool isOpaque)->sk_sp<SkShader> {
            // See comment above for uintptr_t explanation
        ...
        .function("_makeShader", optional_override([](SkRuntimeEffect& self,
            uintptr_t fptr, size_t len, bool isOpaque, SimpleMatrix sm)->sk_sp<SkShader> {
            // See comment above for uintptr_t explanation
            void* inputData = reinterpret_cast<void*>(fptr);
            sk_sp<SkData> inputs = SkData::MakeFromMalloc(inputData, len);
            auto m = toSkMatrix(sm);
        ...
        .function("_makeShaderWithChildren", optional_override([](SkRuntimeEffect& self,
            uintptr_t fptr, size_t len, uintptr_t chilptr, size_t numChildren, bool isOpaque)->sk_sp<SkShader> {
            // See comment above for uintptr_t explanation
            void* inputData = reinterpret_cast<void*>(fptr);
            sk_sp<SkData> inputs = SkData::MakeFromMalloc(inputData, len);
            sk_sp<SkShader>* children = reinterpret_cast<sk_sp<SkShader>*>(chilptr);
            return self.makeShader(inputs, children, numChildren, nullptr, isOpaque);
        }))
        .function("_makeShaderWithChildren", optional_override([](SkRuntimeEffect& self,
            uintptr_t fptr, size_t len, uintptr_t chilptr, size_t numChildren, bool isOpaque, SimpleMatrix sm)->sk_sp<SkShader> {
            // See comment above for uintptr_t explanation
            void* inputData = reinterpret_cast<void*>(fptr);
            sk_sp<SkData> inputs = SkData::MakeFromMalloc(inputData, len);
            sk_sp<SkShader>* children = reinterpret_cast<sk_sp<SkShader>*>(chilptr);
            auto m = toSkMatrix(sm);
            return self.makeShader(inputs, children, numChildren, &m, isOpaque);
        }));
#endif
