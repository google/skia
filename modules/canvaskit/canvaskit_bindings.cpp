...
sk_sp<GrContext> MakeGrContext(EMSCRIPTEN_WEBGL_CONTEXT_HANDLE context)
{
    ...
    if (grContext) {
        ...
        constexpr static int kResourceCacheBytes = 256 * 1024 * 1024;
        grContext->setResourceCacheLimit(kResourceCacheBytes);
    }
    return grContext;
}
...
sk_sp<SkSurface> MakeOffscreenFramebuffer(sk_sp<GrContext> grContext, int width, int height,
    ...
    glRenderbufferStorageMultisample(GL_RENDERBUFFER, 8, GL_STENCIL_INDEX8, width, height);
...
void BlitOffscreenFramebuffer(sk_sp<SkSurface> surface, int srcX0, int srcY0, int srcX1, int srcY1,
  ...
  GrGLFramebufferInfo glInfo;
  auto backendRT = surface->getBackendRenderTarget(SkSurface::kFlushWrite_BackendHandleAccess);
