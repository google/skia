...
The ImageDecoder API handles decoding of both still and animated images. Similar to the larger [web codecs](https://github.com/WICG/web-codecs/blob/master/explainer.md) proposal which is focused more on video and audio. The ImageDecoder API could be used with `CanvasKit.MakeImageFromCanvasImageSource` for creating CanvasKit compatible `SkImage`s. For still images, the `createImageBitmap(blob)` API achieves the same result.
