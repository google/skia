...
protected:
    ...
    void parseKeyFrames(const skjson::ArrayValue& jframes) {
        for (const skjson::ObjectValue* jframe : jframes) {
            ...
            float t0;
            if (!json::Parse<float>((*jframe)["t"], &t0))
                continue;
            ...
            if (!json::ParseDefault<bool>((*jframe)["h"], false)) {
                ...
                static constexpr SkPoint kDefaultC0 = { 0, 0 },
                                         kDefaultC1 = { 1, 1 };
                const auto c0 = json::ParseDefault<SkPoint>((*jframe)["i"], kDefaultC0),
                           c1 = json::ParseDefault<SkPoint>((*jframe)["o"], kDefaultC1);
...
private:
    ...
    int parseValue(const skjson::Value& jv) override {
        T val;
        if (!json::Parse<T>(jv, &val) || (!fVs.empty() &&
...
static inline bool BindPropertyImpl(const skjson::ObjectValue* jprop,
    ...
    if (!json::ParseDefault<bool>(jpropA, false)) {
        T val;
        if (json::Parse<T>(jpropK, &val)) {
            // Static property.
            if (noop && val == *noop)
                return false;
...
bool BindProperty(const skjson::Value& jv,
    ...
    return json::ParseDefault<bool>(jv.as<skjson::ObjectValue>()["s"], false)
