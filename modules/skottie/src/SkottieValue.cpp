...
static constexpr size_t kFloatsPerVertex = 6;
...
bool PathValue::ParseEncodingLen(const skjson::Value& jv, size_t* len) {
...
bool PathValue::ParseEncodingData(const skjson::Value& jv, size_t data_len, float data[]) {
    ...
    auto parse_point = [](const skjson::ArrayValue* ja, size_t i, float dst[]) {
        SkASSERT(ja);
        const skjson::ArrayValue* jpts = (*ja)[i];

        if (!jpts || jpts->size() != 2ul) {
            return false;
        }

        return Parse((*jpts)[0], dst + 0)
            && Parse((*jpts)[1], dst + 1);
    };

    auto parse_optional_point = [parse_point](const skjson::ArrayValue* ja, size_t i, float dst[]) {
        if (!ja || i >= ja->size()) {
        ...
        return parse_point(*ja, i, dst);
    };

    for (size_t i = 0; i < jvs->size(); ++i) {
        float* dst = data + i * 6;
        SkASSERT(dst + 6 <= data + data_len);

        if (!parse_point         (jvs, i, dst + 0) ||
            !parse_optional_point(jis, i, dst + 2) ||
            !parse_optional_point(jos, i, dst + 4)) {
            return false;
        }
    }
