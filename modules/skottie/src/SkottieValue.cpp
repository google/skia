...
VecValue::VecValue(std::initializer_list<float> l) {
    *this = l;
}

VecValue& VecValue::operator=(std::initializer_list<float> l) {
    this->realloc(l.size());
...
void VecValue::do_realloc(size_t sz) {
    SkASSERT(sz != fSize);
    fSize = sz;
...
template <>
bool ValueTraits<VecValue>::FromJSON(const skjson::Value& jv, const internal::AnimationBuilder*,
                                     VecValue* v) {
    if (const skjson::ArrayValue* ja = jv) {
        const auto size = ja->size();
        return ParseArray(jv, v->realloc(size), size);
    }
