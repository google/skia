...
bool AnimationBuilder::AttachLayerContext::hasMotionBlur(const skjson::ObjectValue& jlayer) const {
    if (fMotionBlurSamples < 2 || fMotionBlurAngle <= 0) {
        return false;
    }

    return ParseDefault(jlayer["mb"], false);
}
...
sk_sp<sksg::RenderNode> AnimationBuilder::attachLayer(const skjson::ObjectValue* jlayer,
    ...
    // Motion blur is a no-op when the layer is not animated.
    if (layerCtx->hasMotionBlur(*jlayer) && has_animators) {
