...
bool VectorKeyframeAnimatorBuilder::parseValue(const AnimationBuilder& abuilder,
    ...
    v->resize(vec_len);
    return fParseData(jv, v->data(), vec_len);
}

bool VectorKeyframeAnimatorBuilder::parseKFValue(const AnimationBuilder& abuilder,
    ...
    if (!fParseData(jv, fStorage.data() + offset, fVecLen)) {
        return false;
    }
...
bool AnimatablePropertyContainer::bind<VectorValue>(const AnimationBuilder& abuilder,
    ...
    if (!ParseDefault<bool>((*jprop)["s"], false)) {
        ...
        VectorKeyframeAnimatorBuilder builder(
                    ...
                    // Data parser.
                    [](const skjson::Value& jv, float* data, size_t count) {
                        return parse_array(jv, data, count);
                    });
