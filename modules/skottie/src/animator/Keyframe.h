...
class KeyframeAnimatorBase::Builder : public SkNoncopyable {
public:
    virtual ~Builder();
...
protected:
    virtual bool parseKFValue(const AnimationBuilder&,
                              ...
                              const skjson::Value&,
                              KeyframeAnimatorBase::Keyframe::Value*) = 0;
...
template <typename T>
static inline T LERP(const T& a, const T& b, float t) { return a + (b - a) * t; }

bool BindPropertyImpl(const AnimationBuilder&,
                      const skjson::ObjectValue*,
                      KeyframeAnimatorBase::Builder&,
                      sksg::AnimatorList*,
                      void* target_value);
