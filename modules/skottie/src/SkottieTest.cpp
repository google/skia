...
DEF_TEST(Skottie_Properties, reporter) {
    ...
    class TestPropertyObserver final : public skottie::PropertyObserver {
    public:
        struct ColorInfo {
        ...
        skottie::PropertyObserver::PropertyMask accept(const char node_name[]) override {
            return skottie::PropertyObserver::kColor_PropertyType   |
                   skottie::PropertyObserver::kOpacity_PropertyType |
                   skottie::PropertyObserver::kTransform_PropertyType;
        }

        void onColorProperty(const char node_name[],
                             std::unique_ptr<skottie::ColorPropertyHandle> proxy) override {
            fColors.push_back({node_name, proxy->getColor()});
        }

        void onOpacityProperty(const char node_name[],
                               std::unique_ptr<skottie::OpacityPropertyHandle> proxy) override {
            fOpacities.push_back({node_name, proxy->getOpacity()});
        }

        void onTransformProperty(const char node_name[],
                                 std::unique_ptr<skottie::TransformPropertyHandle> proxy) override {
            fTransforms.push_back({node_name, proxy->getTotalMatrix()});
        }
