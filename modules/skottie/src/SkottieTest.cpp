...
DEF_TEST(Skottie_Shaper, reporter) {
    static constexpr char kFontResource[] = "fonts/Roboto-Regular.ttf";
    auto font_stream = GetResourceAsStream(kFontResource);
    REPORTER_ASSERT(reporter, font_stream);

    auto font_data = SkData::MakeFromStream(font_stream.get(), font_stream->getLength());
    REPORTER_ASSERT(reporter, font_data);

    auto typeface = SkTypeface::MakeFromData(font_data);
