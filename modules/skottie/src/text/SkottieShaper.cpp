    ...
    for (size_t i = 0; i < kMaxIter; ++i) {
        ...
        if (res_height > box.height()) {
            ...
            try_scale = (in_scale == 0)
                    ? try_scale * 0.5f // initial in_size not found yet - search exponentially
                    : (in_scale + out_scale) * 0.5f; // in_size found - binary search
        } else {
            ...
            try_scale = (out_scale == std::numeric_limits<float>::max())
                    ? try_scale * 2 // initial out_size not found yet - search exponentially
                    : (in_scale + out_scale) * 0.5f; // out_size found - binary search
        }
    }
