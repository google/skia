    ...
    for (size_t i = 0; i < kMaxIter; ++i) {
        ...
        if (res_height > box.height()) {
            ...
            try_size = (in_size == 0)
                    ...
                    : (in_size + out_size) / 2; // in_size found - binary search
        } else {
            ...
            try_size = (out_size == std::numeric_limits<float>::max())
                    ...
                    : (in_size + out_size) / 2; // out_size found - binary search
        }
    }
...
Shaper::Result Shaper::Shape(const SkString& txt, const TextDesc& desc, const SkPoint& point) {
    SkTCopyOnFirstWrite<TextDesc> adjusted_desc(desc);

    if (desc.fVAlign == VAlign::kResizeToFit) {
        // makes no sense in point mode
        return { nullptr, {0, 0} };
    }
