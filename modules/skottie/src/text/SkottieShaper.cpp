...
SkRect VisualBlobBounds(const sk_sp<SkTextBlob>& blob) {
...
SkRect VisualResultBounds(const Shaper::Result& result) {
    auto bounds = SkRect::MakeEmpty();
    for (const auto& fragment : result.fFragments) {
        bounds.join(VisualBlobBounds(fragment.fBlob).makeOffset(fragment.fPos.x(),
                                                                fragment.fPos.y()));
    }
    return bounds;
}
...
public:
    ...
    Shaper::Result finalize(float* shaped_height) {
        ...
        auto extent_box = [&]() {
            auto box = VisualResultBounds(fResult);
