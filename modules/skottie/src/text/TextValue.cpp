...
bool ValueTraits<TextValue>::FromJSON(const skjson::Value& jv,
    ...
    static constexpr Shaper::ResizePolicy gResizeMap[] = {
        ...
        Shaper::ResizePolicy::kVisualResizeToFit,    // 'sk_rs': 1
        Shaper::ResizePolicy::kVisualDownscaleToFit, // 'sk_rs': 2
    };
    ...
    if (Parse((*jtxt)["sk_vj"], &sk_vj)) {
        if (sk_vj < SK_ARRAY_COUNT(gVAlignMap)) {
            ...
            switch (sk_vj) {
            case 3:
                // 'sk_vj': 3 -> kVisualCenter/kVisualResizeToFit
                v->fVAlign = Shaper::VAlign::kVisualCenter;
                v->fResize = Shaper::ResizePolicy::kVisualResizeToFit;
                break;
            case 4:
                // 'sk_vj': 4 -> kVisualCenter/kVisualDownscaleToFit
                v->fVAlign = Shaper::VAlign::kVisualCenter;
                v->fResize = Shaper::ResizePolicy::kVisualDownscaleToFit;
                break;
            default:
