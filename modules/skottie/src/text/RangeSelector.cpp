...
void AddCoverageProc(float amount, TextAnimator::AnimatedPropsModulator* dst, size_t count) {
...
static constexpr CoverageProcT gCoverageProcs[] = {
    AddCoverageProc, // Mode::kAdd
};
...
static const ShapeGenerator gShapeGenerators[] = {
    ...
    // Shape::kSmooth
    { 0, 0, [](float t)->float { return (std::cos(M_PI * (1 + 2 * t)) + 1) * 0.5f; }},
};
...
void RangeSelector::modulateCoverage(TextAnimator::ModulatorBuffer& buf) const {
    ...
    if (i0 == i1) {
        ...
        const auto coverage_0 = 1 - (f0 - i0),
                     amount_0 = Lerp(generator.lo, amount, coverage_0);
        generator(coverage_proc, SkTPin(t, 0.0f, 1.0f), amount_0, buf.data() + i0);
        t += dt;
        ...
        const auto coverage_1 = (f1 - i1),
                     amount_1 = Lerp(generator.hi, amount, coverage_1);
        generator(coverage_proc, SkTPin(t, 0.0f, 1.0f), amount_1, buf.data() + i1);
    }
}
