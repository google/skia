...
std::tuple<float, float> RangeSelector::resolve(size_t len) const {
    ...
    const auto resolver = [this]() {
        switch (fUnits) {
        case Units::kPercentage: return UnitTraits<Units::kPercentage>::Resolve;
        case Units::kIndex     : return UnitTraits<Units::kIndex     >::Resolve;
        }
    }();
...
void RangeSelector::modulateCoverage(std::vector<float>& buf) const {
    SkASSERT(!buf.empty());
    ...
    if (i0 == i1) {
        ...
        modulate(buf.data() + i0, buf.data() + i0 + 1, fract * amount);
    } else {
