...
int main(int argc, char** argv) {
    ...
    auto executor = SkExecutor::MakeFIFOThreadPool(SkTPin<int>(FLAGS_threads, 0, 128));
    SkTaskGroup tgroup(*executor);

    tgroup.batch(static_cast<int>((t1 - t0) / dt), [&](int i) {
        thread_local std::unique_ptr<Sink> tl_sink(MakeSink(FLAGS_format[0]));
        thread_local sk_sp<skottie::Animation> tl_anim(
                    i == 0 ? anim : skottie::Animation::Builder()
                                        .setResourceProvider(rp)
                                        .make(static_cast<const char*>(data->data()),
                                              data->size()));
