...
struct MP4Sink final : public Sink {
    explicit MP4Sink(const SkMatrix& scale_matrix) : fScaleMatrix(scale_matrix) {}

    SkCanvas* beginFrame(size_t i) override {
        fSurface = SkSurface::MakeRasterN32Premul(FLAGS_width, FLAGS_height);
        fSurface->getCanvas()->concat(fScaleMatrix);
    ...
    bool endFrame(size_t i) override {
        ...
        fSurface->readPixels(gMP4Frames[i].pixmap(), 0,0);
        return true;
    }
