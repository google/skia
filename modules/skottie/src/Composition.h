...
public:
    explicit CompositionBuilder(const skjson::ObjectValue& jcomp);
    ~CompositionBuilder();

    sk_sp<sksg::RenderNode> build(const AnimationBuilder&);

    const LayerBuilder* findBuilder(int layer_index) const;

    const sk_sp<sksg::Transform>& getCameraTransform() const { return fCameraTransform; }

    void setMatte(sk_sp<sksg::RenderNode>);
    sk_sp<sksg::RenderNode> releaseMatte();
