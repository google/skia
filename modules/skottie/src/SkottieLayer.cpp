...
sk_sp<sksg::RenderNode> AttachMask(const skjson::ArrayValue* jmask,
    ...
    for (const skjson::ObjectValue* m : *jmask) {
        ...
        if (!mask_info) {
            abuilder->log(Logger::Level::kWarning, nullptr, "Unsupported mask mode: '%c'.\n", mode);
            continue;
        }
...
sk_sp<sksg::RenderNode> AnimationBuilder::attachNestedAnimation(const char* name,
    ...
    if (!data) {
        this->log(Logger::Level::kError, nullptr, "Could not load: %s.\n", name);
        return nullptr;
    }
    ...
    if (!animation) {
        this->log(Logger::Level::kError, nullptr, "Could not parse nested animation: %s.\n", name);
        return nullptr;
    }
...
sk_sp<sksg::RenderNode> AnimationBuilder::attachAssetRef(
    ...
    if (!asset_info) {
        this->log(Logger::Level::kError, nullptr, "Asset not found: '%s'.\n", refId.c_str());
        return nullptr;
    }

    if (asset_info->fIsAttaching) {
        this->log(Logger::Level::kError, nullptr,
                  "Asset cycle detected for: '%s'\n", refId.c_str());
        return nullptr;
    }
...
sk_sp<sksg::RenderNode> AnimationBuilder::attachImageAsset(const skjson::ObjectValue& jimage,
    ...
    if (!data) {
        this->log(Logger::Level::kError, nullptr,
                  "Could not load image resource: %s/%s.\n", path_cstr, name_cstr);
        return nullptr;
    }
