...
sk_sp<sksg::RenderNode> AnimationBuilder::attachNestedAnimation(const char* name,
    ...
    const auto data = fRec.fProvider->load("", name);
    if (!data) {
        LOG("!! Could not load: %s\n", name);
        return nullptr;
    }

    auto animation = Animation::Make(static_cast<const char*>(data->data()), data->size(), fRec);
    if (!animation) {
...
sk_sp<sksg::RenderNode> AnimationBuilder::attachImageAsset(const skjson::ObjectValue& jimage,
    ...
    const auto data = fRec.fProvider->load(path_cstr, name_cstr);
    if (!data) {
