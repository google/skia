...
AnimationBuilder::loadImageAsset(const skjson::ObjectValue& jimage) const {
    ...
    if (!asset) {
        ...
        public:
            ...
            sk_sp<SkImage> getImage(const LayerInfo&, float) override { return fImage; }
...
sk_sp<sksg::RenderNode> AnimationBuilder::attachImageAsset(const skjson::ObjectValue& jimage,
    ...
    const ImageAsset::LayerInfo image_layer_info = {
        layer_info.fInPoint,
        layer_info.fOutPoint,
        fFrameRate
    };

    auto image = asset_info->fAsset->getImage(image_layer_info, layer_info.fInPoint);
    if (!image) {
