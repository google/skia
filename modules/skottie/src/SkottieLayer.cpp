...
AnimationBuilder::loadImageAsset(const skjson::ObjectValue& jimage) const {
    ...
    if (!asset) {
        ...
        public:
            ...
            sk_sp<SkImage> getImage(uint32_t, float) override { return fImage; }
...
sk_sp<sksg::RenderNode> AnimationBuilder::attachImageAsset(const skjson::ObjectValue& jimage,
    ...
    auto image = asset_info->fAsset->getImage(0, 0);
    if (!image) {
    ...
    if (asset_info->fAsset->isMultiFrame()) {
        ...
        public:
            MultiFrameAnimator(sk_sp<ImageAsset> asset, sk_sp<sksg::Image> image_node, float fps)
            ...
            void onTick(float t) override {
                fImageNode->setImage(
                    fAsset->getImage(static_cast<uint32_t>(t * 1000 / fAnimationFPS), t));
            }
