...
Animation::Animation(std::unique_ptr<sksg::Scene> scene, SkString version, const SkSize& size,
    ...
    , fInfo({std::move(version), size, fps, duration})
    , fInPoint(inPoint)
    , fOutPoint(outPoint)
    , fFlags(flags) {
...
void Animation::render(SkCanvas* canvas, const SkRect* dstR, RenderFlags renderFlags) const {
    ...
    const SkRect srcR = SkRect::MakeSize(fInfo.fSize);
    if (dstR) {
...
void Animation::seekFrame(SkScalar t, sksg::InvalidationController* ic) {
    TRACE_EVENT0("skottie", TRACE_FUNC);
