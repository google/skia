...
sk_sp<sksg::GeometryNode> AttachPolystarGeometry(const skjson::ObjectValue& jstar,
    ...
    const auto type = json::ParseDefault<int>(jstar["sy"], 0) - 1;
...
sk_sp<sksg::Gradient> AttachGradient(const skjson::ObjectValue& jgrad, AttachContext* ctx) {
    ...
    const auto stopCount = json::ParseDefault<int>((*stops)["p"], -1);
    if (stopCount < 0)
        return nullptr;
    ...
    if (json::ParseDefault<int>(jgrad["t"], 1) == 1) {
...
sk_sp<sksg::PaintNode> AttachStroke(const skjson::ObjectValue& jstroke, AttachContext* ctx,
    ...
    stroke_node->setStrokeMiter(json::ParseDefault<SkScalar>(jstroke["ml"], 4.0f));
    ...
    stroke_node->setStrokeJoin(gJoins[SkTPin<int>(json::ParseDefault<int>(jstroke["lj"], 1) - 1,
    ...
    stroke_node->setStrokeCap(gCaps[SkTPin<int>(json::ParseDefault(jstroke["lc"], 1) - 1,
...
std::vector<sk_sp<sksg::GeometryNode>> AttachMergeGeometryEffect(
    ...
    const auto mode = gModes[SkTPin<int>(json::ParseDefault<int>(jmerge["mm"], 1) - 1,
...
std::vector<sk_sp<sksg::GeometryNode>> AttachTrimGeometryEffect(
    ...
    const auto mode = gModes[SkTPin<int>(json::ParseDefault<int>(jtrim["m"], 1) - 1,
                                         0, SK_ARRAY_COUNT(gModes) - 1)];
...
const ShapeInfo* FindShapeInfo(const skjson::ObjectValue& jshape) {
    ...
    SkString type;
    if (!json::Parse<SkString>(jshape["ty"], &type) || type.isEmpty())
        return nullptr;
...
sk_sp<sksg::RenderNode> AttachAssetRef(const skjson::ObjectValue& jlayer, AttachContext* ctx,
    ...
    const auto refId = json::ParseDefault<SkString>(jlayer["refId"], SkString());
    if (refId.isEmpty()) {
...
sk_sp<sksg::RenderNode> AttachCompLayer(const skjson::ObjectValue& jlayer, AttachContext* ctx,
    ...
    const auto start_time = json::ParseDefault<float>(jlayer["st"], 0.0f),
             stretch_time = json::ParseDefault<float>(jlayer["sr"], 1.0f);
...
sk_sp<sksg::RenderNode> AttachSolidLayer(const skjson::ObjectValue& jlayer, AttachContext*,
                                         float*, float*) {
    const auto size = SkSize::Make(json::ParseDefault<float>(jlayer["sw"], 0.0f),
                                   json::ParseDefault<float>(jlayer["sh"], 0.0f));
    const auto hex = json::ParseDefault<SkString>(jlayer["sc"], SkString());
    uint32_t c;
    if (size.isEmpty() ||
        !hex.startsWith("#") ||
...
sk_sp<sksg::RenderNode> AttachImageAsset(const skjson::ObjectValue& jimage, AttachContext* ctx) {
    const auto name = json::ParseDefault<SkString>(jimage["p"], SkString()),
               path = json::ParseDefault<SkString>(jimage["u"], SkString());
    if (name.isEmpty())
        return nullptr;
...
struct AttachLayerContext {
    ...
    sk_sp<sksg::Matrix> AttachLayerMatrix(const skjson::ObjectValue& jlayer) {
        const auto layer_index = json::ParseDefault<int>(jlayer["ind"], -1);
        if (layer_index < 0)
            return nullptr;
...
private:
    sk_sp<sksg::Matrix> AttachParentLayerMatrix(const skjson::ObjectValue& jlayer,
                                                int layer_index) {
        const auto parent_index = json::ParseDefault<int>(jlayer["parent"], -1);
        ...
        for (const skjson::ObjectValue* l : fLayerList) {
            ...
            if (json::ParseDefault<int>((*l)["ind"], -1) == parent_index) {
...
sk_sp<sksg::RenderNode> AttachMask(const skjson::ArrayValue* jmask,
    ...
    for (const skjson::ObjectValue* m : *jmask) {
        ...
        mask_path->setFillType(json::ParseDefault<bool>((*m)["inv"], false)
            ? SkPath::kInverseWinding_FillType
            : SkPath::kWinding_FillType);

        SkString mode;
        if (!json::Parse<SkString>((*m)["mode"], &mode) ||
            mode.size() != 1 ||
...
sk_sp<sksg::RenderNode> AttachLayer(const skjson::ObjectValue* jlayer,
    ...
    int type = json::ParseDefault<int>((*jlayer)["ty"], -1);
    ...
    float w = 0, h = 0;
    if (json::Parse<float>((*jlayer)["w"], &w) && json::Parse<float>((*jlayer)["h"], &h)) {
    ...
    const auto        in = json::ParseDefault<float>((*jlayer)["ip"], 0.0f),
                     out = json::ParseDefault<float>((*jlayer)["op"], in);
    ...
    if (json::ParseDefault<bool>((*jlayer)["td"], false)) {
    ...
    if (layerCtx->fCurrentMatte) {
        ...
        const auto matteType = json::ParseDefault<int>((*jlayer)["tt"], 1) - 1;
...
sk_sp<Animation> Animation::Make(SkStream* stream, const ResourceProvider* provider, Stats* stats) {
    ...
    const auto version = json::ParseDefault<SkString>(json["v"], SkString());
    const auto size    = SkSize::Make(json::ParseDefault<float>(json["w"], 0.0f),
                                      json::ParseDefault<float>(json["h"], 0.0f));
    const auto fps     = json::ParseDefault<float>(json["fr"], -1.0f);
...
Animation::Animation(const ResourceProvider& resources,
    ...
    , fSize(size)
    , fFrameRate(fps)
    , fInPoint(json::ParseDefault<float>(json["ip"], 0.0f))
    , fOutPoint(SkTMax(json::ParseDefault<float>(json["op"], SK_ScalarMax), fInPoint)) {

    AssetMap assets;
    if (const skjson::ArrayValue* jassets = json["assets"]) {
        for (const skjson::ObjectValue* asset : *jassets) {
            if (asset) {
                assets.set(json::ParseDefault<SkString>((*asset)["id"], SkString()),
                           { asset, false });
            }
        }
    }
