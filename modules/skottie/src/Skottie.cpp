...
sk_sp<sksg::GeometryNode> AttachPolystarGeometry(const skjson::ObjectValue& jstar,
    ...
    const auto type = ParseDefault<unsigned>(jstar["sy"], 0) - 1;
    if (type >= SkTo<int>(SK_ARRAY_COUNT(gTypes))) {
...
sk_sp<sksg::PaintNode> AttachStroke(const skjson::ObjectValue& jstroke, AttachContext* ctx,
    ...
    stroke_node->setStrokeJoin(gJoins[SkTPin<unsigned>(ParseDefault<unsigned>(jstroke["lj"], 1) - 1,
    ...
    stroke_node->setStrokeCap(gCaps[SkTPin<unsigned>(ParseDefault<unsigned>(jstroke["lc"], 1) - 1,
...
std::vector<sk_sp<sksg::GeometryNode>> AttachMergeGeometryEffect(
    ...
    const auto mode = gModes[SkTPin<unsigned>(ParseDefault<unsigned>(jmerge["mm"], 1) - 1,
...
std::vector<sk_sp<sksg::GeometryNode>> AttachTrimGeometryEffect(
    ...
    const auto mode = gModes[SkTPin<unsigned>(ParseDefault<unsigned>(jtrim["m"], 1) - 1,
                                              0, SK_ARRAY_COUNT(gModes) - 1)];
...
sk_sp<sksg::RenderNode> AttachLayer(const skjson::ObjectValue* jlayer,
    ...
    if (layerCtx->fCurrentMatte) {
        ...
        const auto matteType = ParseDefault<unsigned>((*jlayer)["tt"], 1) - 1;
