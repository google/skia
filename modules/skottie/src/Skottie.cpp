...
sk_sp<sksg::PaintNode> AttachStroke(const skjson::ObjectValue& jstroke, AttachContext* ctx,
    ...
    stroke_node->setStrokeJoin(gJoins[SkTPin<size_t>(ParseDefault<size_t>(jstroke["lj"], 1) - 1,
    ...
    stroke_node->setStrokeCap(gCaps[SkTPin<size_t>(ParseDefault<size_t>(jstroke["lc"], 1) - 1,
...
std::vector<sk_sp<sksg::GeometryNode>> AttachMergeGeometryEffect(
    ...
    const auto mode = gModes[SkTPin<size_t>(ParseDefault<size_t>(jmerge["mm"], 1) - 1,
...
std::vector<sk_sp<sksg::GeometryNode>> AttachTrimGeometryEffect(
    ...
    const auto mode = gModes[SkTPin<size_t>(ParseDefault<size_t>(jtrim["m"], 1) - 1,
                                            0, SK_ARRAY_COUNT(gModes) - 1)];
