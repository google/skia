...
AnimationBuilder::AnimationBuilder(const ResourceProvider& rp, const SkFontMgr& fontmgr,
...
Animation::Builder& Animation::Builder::setFontManager(const SkFontMgr* fmgr) {
    fFontMgr = fmgr;
    return *this;
}
...
class AutoReset {
public:
    explicit AutoReset(T& target)
        : fTarget(target), fSaved(target) {}
    ~AutoReset() { fTarget = fSaved; }
...
sk_sp<Animation> Animation::Builder::make(const char* data, size_t data_len) {
    ...
    NullResourceProvider localProvider;
    const AutoReset<const ResourceProvider*> rpSaver(fResourceProvider);
    if (!fResourceProvider) {
        fResourceProvider = &localProvider;
    }

    sk_sp<SkFontMgr> localFontMgr;
    const AutoReset<const SkFontMgr*> fmgrSaver(fFontMgr);
    if (!fFontMgr) {
        localFontMgr = SkFontMgr::RefDefault();
        fFontMgr = localFontMgr.get();
    }
    ...
    internal::AnimationBuilder builder(*fResourceProvider, *fFontMgr, &fStats, duration, fps);
