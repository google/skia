...
void GradientAdapter::apply() {
    ...
    while (c_rec || o_rec) {
        ...
        const auto& cr_ = c_rec
                ? *c_rec
                : ColorRec{ o_rec->t,
                            current_stop.fColor.fR,
                            current_stop.fColor.fG,
                            current_stop.fColor.fB };
        const auto& or_ = o_rec
                ? *o_rec
                : OpacityRec{ c_rec->t, current_stop.fColor.fA };

        // Compute component scales based on the relative position of the stops being considered.
        // The idea is to select the smaller-pos stop, use its own properties as specified
        // (lerp with t == 1), and lerp (with t < 1) the properties from the larger-pos stop
        // against the previously computed gradient stop values.
        const auto     c_pos = std::max(cr_.t, current_stop.fPosition),
                       o_pos = std::max(or_.t, current_stop.fPosition),
        ...
        current_stop = {
                std::min(c_pos, o_pos),
                {
                    lerp(current_stop.fColor.fR, cr_.r, t_c ),
                    lerp(current_stop.fColor.fG, cr_.g, t_c ),
                    lerp(current_stop.fColor.fB, cr_.b, t_c ),
                    lerp(current_stop.fColor.fA, or_.a, t_o)
                }
        };
