...
public:
    ...
    const sk_sp<T>& node() const { return fRenderNode; }

protected:
    DiscardableAdapterBase()
        : fRenderNode(T::Make()) {}

    explicit DiscardableAdapterBase(sk_sp<T> render_node)
        : fRenderNode(std::move(render_node)) {}

private:
    const sk_sp<T> fRenderNode;
};
