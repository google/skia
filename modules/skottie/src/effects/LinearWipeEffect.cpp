...
public:
    explicit LinearWipeAdapter(const SkSize& ls)
    ...
    const sk_sp<sksg::MaskFilter>& maskNode() const { return fMaskNode; }
...
private:
    void apply() const {
        ...
        if (fCompletion >= 1) {
            // TODO: short circuit for the fully-transitioned case.
            // return;
        }
...
sk_sp<sksg::RenderNode> EffectBuilder::attachLinearWipeEffect(const skjson::ArrayValue& jprops,
    ...
    auto adapter = sk_make_sp<LinearWipeAdapter>(fLayerSize);
    ...
    return sksg::MaskFilterEffect::Make(std::move(layer), adapter->maskNode());
}
