...
static constexpr char gSphereSkSL[] = R"(
    ...
    half4 main(float2 xy) {
        ...
        half3 ro = half3(0, 0, 10),
              rd = normalize(half3(xy, -ro.z));
        half   t = dot(-ro, rd);
        half3  p = ro + rd*t;
        half  dt = sqrt(1 - dot(p,p));
        half3 p0 = ro + rd*(t + sel*dt);
...
private:
    ...
    SkM44 computeRotation() const {
        ...
        switch (SkScalarRoundToInt(fRotOrder)) {
            case 1:  return rz * ry * rx;
            case 2:  return ry * rz * rx;
            case 3:  return rz * rx * ry;
            case 4:  return rx * rz * ry;
            case 5:  return ry * rx * rz;
            case 6:
