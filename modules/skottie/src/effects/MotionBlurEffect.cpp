...
sk_sp<MotionBlurEffect> MotionBlurEffect::Make(std::unique_ptr<sksg::Animator> animator,
    ...
    if (samples_per_frame <= 1 || shutter_angle <= 0) {
        return nullptr;
    }

    // shutter_angle is in [   0..360]
    // shutter_phase is in [-360,.360]
    const auto samples_duration = shutter_angle / 360,
                          phase = samples_duration * (shutter_phase / 360 + 0.5f),
...
SkRect MotionBlurEffect::onRevalidate(sksg::InvalidationController* ic, const SkMatrix& ctm) {
