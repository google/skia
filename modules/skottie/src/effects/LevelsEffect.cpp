...
sk_sp<sksg::RenderNode> EffectBuilder::attachLevelsEffect(const skjson::ArrayValue& jprops,
    ...
    fBuilder->bindProperty<ScalarValue>((*channel_prop)["v"], fScope,
        [adapter](const ScalarValue& channel) {
            adapter->setChannel(channel);
        });
    fBuilder->bindProperty<ScalarValue>((*iblack_prop)["v"], fScope,
        [adapter](const ScalarValue& ib) {
            adapter->setInBlack(ib);
        });
    fBuilder->bindProperty<ScalarValue>((*iwhite_prop)["v"], fScope,
        [adapter](const ScalarValue& iw) {
            adapter->setInWhite(iw);
        });
    fBuilder->bindProperty<ScalarValue>((*oblack_prop)["v"], fScope,
        [adapter](const ScalarValue& ob) {
            adapter->setOutBlack(ob);
        });
    fBuilder->bindProperty<ScalarValue>((*owhite_prop)["v"], fScope,
        [adapter](const ScalarValue& ow) {
            adapter->setOutWhite(ow);
        });
    fBuilder->bindProperty<ScalarValue>((*gamma_prop)["v"], fScope,
        [adapter](const ScalarValue& g) {
            adapter->setGamma(g);
        });

    fBuilder->bindProperty<ScalarValue>((*clip_black_prop)["v"], fScope,
        [adapter](const ScalarValue& cb) {
            adapter->setClipBlack(cb);
        });
    fBuilder->bindProperty<ScalarValue>((*clip_white_prop)["v"], fScope,
