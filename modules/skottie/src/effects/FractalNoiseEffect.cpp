...
static constexpr char gNoiseSkSL[] = R"(
    ...
    half4 main(float2 xy) {
        ...
        for (half z = 0; z < %u; z += 1) {
            half scale = amp*saturate(o);
            n += scale*compute_noise(xy);
...
private:
    ...
    void onSync() override {
        ...
        n->setEvolution(SkTPin(fRandomSeed, 0.0f, 100000.0f) + SkDegreesToRadians(fEvolution)/4);
