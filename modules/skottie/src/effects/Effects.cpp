...
EffectBuilder::EffectBuilder(const AnimationBuilder* abuilder,
                             const skjson::ArrayValue& jeffects,
...
sk_sp<sksg::RenderNode> EffectBuilder::attachEffects(sk_sp<sksg::RenderNode> layer) const {
    if (!layer) {
        return nullptr;
    }

    for (const skjson::ObjectValue* jeffect : fEffects) {
        if (!jeffect) {
            continue;
        }
