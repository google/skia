...
static constexpr char gDisplacementSkSL[] = R"(
    ...
    half4 main(float2 xy) {
        ...
        // We need unpremul inputs.
        d.rgb /= d.a + 0.0001;

        // Run the displacemnt input through the selector transform.
        d = d*selector_matrix + selector_offset;

        // At this point, our channels hold:
        //
        //   R,G -> x/y displacement
...
private:
    ...
    struct SelectorCoeffs {
        float r, g, b, a, displ_offset, alpha_scale, alpha_offset;
    };
    ...
    static SelectorCoeffs Coeffs(Selector sel) {
        ...
        static constexpr SelectorCoeffs gCoeffs[] = {
            ...
            { 1,0,0,0,0,   1,0 },   // kH: D = h, C = a   (HSLA)
            { 0,1,0,0,0,   1,0 },   // kL: D = l, C = a   (HSLA)
            { 0,0,1,0,0,   1,0 },   // kS: D = s, C = a   (HSLA)
    ...
    sk_sp<SkShader> buildEffectShader(sksg::InvalidationController* ic, const SkMatrix& ctm) {
        ...
        if (this->children()[0]->hasInval()) {
            fChildContent = get_content_picture(this->children()[0], ic, ctm);
        }

        if (fDisplSource->hasInval()) {
            fDisplContent = get_content_picture(fDisplSource, ic, ctm);
        }
        ...
        auto displ_shader = fDisplContent->makeShader(displ_mode,
                                                      displ_mode,
                                                      &displ_matrix,
                                                      &displ_tile);
        ...
        const float selector_m[] = {
            xc.r*s.x, xc.g*s.x, xc.b*s.x,       xc.a*s.x,   // R -> horizontal displacement
            yc.r*s.y, yc.g*s.y, yc.b*s.y,       yc.a*s.y,   // G -> vertical displacement
                   0,        0,        0, xc.alpha_scale,   // B -> horizontal modulation
                   0,        0,        0, yc.alpha_scale,   // A -> vertical modulation
        };
        const float selector_o[] = {
            (xc.displ_offset - .5f) * s.x,
            (yc.displ_offset - .5f) * s.y,
            xc.alpha_offset,
            yc.alpha_offset,
        };
