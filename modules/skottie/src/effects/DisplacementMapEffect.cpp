...
private:
    ...
    sk_sp<SkShader> selectChannels(sk_sp<SkShader> input) const {
        ...
        return input->makeWithColorFilter(SkColorFilters::Matrix({
            xc.dr, xc.dg, xc.db,      xc.da, xc.d_offset,   // X displacement
            yc.dr, yc.dg, yc.db,      yc.da, yc.d_offset,   // Y displacement
                0,     0,     0, xc.c_scale, xc.c_offset,   // X displacement coverage
                0,     0,     0, yc.c_scale, yc.c_offset,   // Y displacement coverage
        }));
    }
    ...
    sk_sp<SkShader> buildEffectShader(sksg::InvalidationController* ic, const SkMatrix& ctm) {
        ...
        builder.child("displ") = this->selectChannels(std::move(displ_shader));
