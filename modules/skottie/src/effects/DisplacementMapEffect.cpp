...
static constexpr char gDisplacementSkSL[] = R"(
    ...
    half4 main(float2 xy) {
        ...
        d = unpremul(d)*selector_matrix + selector_offset;
...
private:
    ...
    sk_sp<SkShader> buildEffectShader(sksg::InvalidationController* ic, const SkMatrix& ctm) {
        ...
        const float selector_m[] = {
            xc.dr*s.x, xc.dg*s.x, xc.db*s.x,  xc.da*s.x,   // R -> horizontal displacement
            yc.dr*s.y, yc.dg*s.y, yc.db*s.y,  yc.da*s.y,   // G -> vertical displacement
                    0,         0,         0, xc.c_scale,   // B -> horizontal modulation
                    0,         0,         0, yc.c_scale,   // A -> vertical modulation
        };
