...
TransformAdapter2D::TransformAdapter2D(const AnimationBuilder& abuilder,
    ...
    if (auto_orient) {
        ...
        this->bind(abuilder, jposition, fPosition.fVec);
    }
}
...
SkMatrix TransformAdapter2D::totalMatrix() const {
    ...
    t.postRotate(fRotation + fPosition.fRot);
    t.postTranslate(fPosition.fVec.x, fPosition.fVec.y);
    // TODO: skew
...
SkPoint TransformAdapter2D::getPosition() const {
    return { fPosition.fVec.x, fPosition.fVec.y };
}

void TransformAdapter2D::setPosition(const SkPoint& p) {
    fPosition.fVec = { p.x(), p.y() };
    this->onSync();
}
