...
const SkFontStyle FontStyle(const char* style) {
    ...
    for (const auto& w : gWeightMap) {
        if (const auto* found = strstr(style, w.fName)) {
            if (found == style) {
                weight = w.fWeight;
                style = found + strlen(w.fName);
                break;
            }
        }
    }
    ...
    if (*style != '\0') {
        for (const auto& s : gSlantMap) {
            if (const auto* found = strstr(style, s.fName)) {
                if (found == style) {
                    slant = s.fSlant;
                    style = found + strlen(s.fName);
                    break;
                }
            }
        }
...
FontMap ParseFonts(const skjson::ObjectValue* jfonts, const skjson::ArrayValue* jglyphs,
    ...
    if (jglyphs) {
        ...
        for (const skjson::ObjectValue* jglyph : *jglyphs) {
            ...
            const skjson::StringValue* jstyle  = (*jglyph)["style"]; // "style", not "fStyle"...
