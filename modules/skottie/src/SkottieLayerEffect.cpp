...
sk_sp<sksg::RenderNode> AttachDropShadowLayerEffect(const skjson::ArrayValue& jprops,
    ...
    const skjson::ObjectValue* shadow_color_prop = jprops[kShadowColor_Index];
    const skjson::ObjectValue*      opacity_prop = jprops[    kOpacity_Index];
    ...
    sk_sp<sksg::Color> color_node = abuilder->attachColor(*shadow_color_prop, ascope, "v");
    if (!color_node) {
        return nullptr;
    }
    ...
    abuilder->bindProperty<ScalarValue>((*opacity_prop)["v"], ascope,
        [color_node](const ScalarValue& o) {
            const auto c = color_node->getColor();
            const auto a = sk_float_round2int_no_saturate(SkTPin(o, 0.0f, 255.0f));
            color_node->setColor(SkColorSetA(c, a));
        });

    auto shadow_effect = sksg::DropShadowEffect::Make(std::move(layer), std::move(color_node));
