...
using GlyphMap = SkTHashMap<SkUnichar, GlyphInfo>;
...
struct FontInfo {
    ...
    std::unique_ptr<GlyphMap> fGlyphs;

    bool matches(const char family[], const SkFontStyle&) const;
};
...
FontMap ParseFonts(const skjson::ObjectValue* jfonts,
                   const skjson::ArrayValue* jglyphs,
                   const SkFontMgr*);
