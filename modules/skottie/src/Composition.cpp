...
bool CompositionBuilder::hasMotionBlur(const skjson::ObjectValue& jlayer) const {
...
sk_sp<sksg::RenderNode> CompositionBuilder::build(const AnimationBuilder& abuilder) {
    // First thing we do is attach a camera transform if needed, as it is required to build
    // all other 3D transform chains.
    if (fCameraBuilderIndex >= 0) {
        // Explicit camera.
        fCameraTransform = fLayerBuilders[fCameraBuilderIndex].buildTransform(abuilder, this);
    } else if (fHas3D) {
