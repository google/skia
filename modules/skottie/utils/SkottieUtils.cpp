...
public:
    ...
    void onColorProperty(const char node_name[],
        ...
        const auto markedKey = skottie_utils::CustomPropertyManager::acceptKey(node_name);
    ...
    void onTextProperty(const char node_name[],
        ...
        const auto key = skottie_utils::CustomPropertyManager::acceptKey(node_name);
        if (!key.empty()) {
...
public:
    ...
    void onMarker(const char name[], float t0, float t1) override {
        const auto key = skottie_utils::CustomPropertyManager::acceptKey(name);
        if (!key.empty()) {
