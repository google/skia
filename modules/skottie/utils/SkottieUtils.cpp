...
class NestedAnimationLayer final : public skottie::ExternalLayer {
public:
    NestedAnimationLayer(sk_sp<skottie::Animation> anim, const SkSize& size)
...
CustomPrecompInterceptor::CustomPrecompInterceptor(sk_sp<skresources::ResourceProvider> rprovider,
                                                   const char prefixp[])
    : fResourceProvider(std::move(rprovider))
    , fPrefix(prefixp) {}

CustomPrecompInterceptor::~CustomPrecompInterceptor() = default;

sk_sp<skottie::ExternalLayer> CustomPrecompInterceptor::onLoadPrecomp(const char[],
                                                                      const char name[],
                                                                      const SkSize& size) {
    ...
    return anim ? sk_make_sp<NestedAnimationLayer>(std::move(anim), size)
                : nullptr;
}
