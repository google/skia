...
public:
    ...
    std::vector<PropInfo<skottie::ColorPropertyValue>> getAllColorProps() const;
    bool setColorProp(const PropKey&, skottie::ColorPropertyValue);

    std::vector<PropInfo<skottie::OpacityPropertyValue>> getAllOpacityProps() const;
    bool setOpacityProp(const PropKey&, skottie::OpacityPropertyValue);

    std::vector<PropInfo<skottie::TransformPropertyValue>> getAllTransformProps() const;
    bool setTransformProp(const PropKey&, const skottie::TransformPropertyValue&);
...
private:
    ...
    std::vector<PropInfo<V>> getAllProps(const PropMap<T>& container) const;

    template <typename V, typename T>
    bool setProp(const PropKey&, const V&, const PropMap<T>& container);
