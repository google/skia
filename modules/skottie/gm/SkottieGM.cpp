...
protected:
    ...
    void onOnceBeforeDraw() override {
        if (auto stream = GetResourceAsStream("skottie/skottie_sample_search.json")) {
            ...
            fAllColors   = fPropManager->getAllColorProps();
        }
    }
    ...
    bool onHandleKey(SkUnichar uni) override {
        ...
        if (uni == 'c') {
            ...
            for (const auto& prop : fAllColors) {
                fPropManager->setColorProp(prop.fKey, kColors[fColorIndex]);
            }
            return true;
        }
...
private:
    ...
    std::vector<CustomPropertyManager::PropInfo<SkColor>> fAllColors;
