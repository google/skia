...
private:
    ...
    public:
        ...
        sk_sp<SkImage> getImage(uint32_t frame_ms, float frame_number) override {
            if (!fPlayer) {
                return nullptr;
            }

            const auto player_time = fPlayer->duration() ? frame_ms % fPlayer->duration() : 0;
            fPlayer->seek(player_time);
