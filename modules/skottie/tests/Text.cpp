...
public:
    const SkFontStyle* style(const char* family) const {
        auto s = fStyles.find(family);
        return s != fStyles.end() ? &s->second : nullptr;
    }
...
private:
    ...
    mutable std::unordered_map<std::string, SkFontStyle> fStyles;
};
...
DEF_TEST(Skottie_Text_Style, r) {
    ...
    auto fmgr = sk_make_sp<MockFontMgr>();
    ...
    for (const auto& exp : expected) {
        const auto* style = fmgr->style(exp.family);
