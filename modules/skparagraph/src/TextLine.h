...
public:
    ...
    inline TextStyle style() const { return fTextStyle; }
...
public:
    ...
    TextLine(SkVector offset, SkVector advance, SkSpan<const TextBlock> blocks, SkSpan<const char> text,
           SkSpan<const Cluster> clusters, size_t start, size_t end, LineMetrics sizes);

    inline SkSpan<const char> text() const { return fText; }
    inline SkSpan<const Cluster> clusters() const { return fClusters; }
...
private:
    Run* shapeEllipsis(const std::string& ellipsis, Run* run);
