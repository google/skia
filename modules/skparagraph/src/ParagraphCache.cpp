...
public:
    ParagraphCacheValue(const ParagraphImpl* paragraph)
        ...
        fRuns.reserve(paragraph->fRuns.size());
...
void ParagraphCache::updateTo(ParagraphImpl* paragraph, const Entry* entry) {
    ...
    paragraph->fRuns.reserve(entry->fValue->fRuns.size());
    for (auto& run : entry->fValue->fRuns) {
        auto& newRun = paragraph->fRuns.emplace_back(run);
        newRun.setMaster(paragraph);
    }
