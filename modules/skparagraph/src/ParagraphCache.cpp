...
public:
    ...
    void print(const char* prefix) {
        SkDebugf("%s='%s' %d %d ", prefix, fText.c_str(), fPlaceholders.count(), fTextStyles.count());
        if (fPlaceholders.count() > 1) {
          auto& ph = fPlaceholders[0];
            SkDebugf("[%d:%d): %f %f %f %f %f\n", ph.fRange.start, ph.fRange.end,
                    ph.fStyle.fWidth, ph.fStyle.fHeight, ph.fStyle.fBaselineOffset,
                    (int)ph.fStyle.fBaseline, (int)ph.fStyle.fAlignment);
        }
    }
...
uint32_t ParagraphCache::KeyHash::operator()(const ParagraphCacheKey& key) const {
    ...
    for (auto& ph : key.fPlaceholders) {
        ...
        if (ph.fStyle.fAlignment == PlaceholderAlignment::kAboveBaseline ||
            ph.fStyle.fAlignment == PlaceholderAlignment::kBaseline ||
            ph.fStyle.fAlignment == PlaceholderAlignment::kBelowBaseline) {
            hash = mix(hash, SkGoodHash()(relax(ph.fStyle.fBaselineOffset)));
        }
