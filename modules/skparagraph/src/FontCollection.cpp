...
bool FontCollection::findFirst(const char* codepoint, SkFont* font, SkScalar* height) {
...
bool FontCollection::findNext(const char* codepoint, SkFont* font, SkScalar* height) {
    auto found = fFontMapping.find(codepoint);
    if (found == nullptr) {
        return false;
    }
...
void FontCollection::findAllFontsForStyledBlock(const TextStyle& style, SkSpan<const char> text) {
    fCodepoints.reset();
    fCharacters.reset();
    ...
    if (fUnresolved > 0) {
        auto typeface = this->matchDefaultTypeface(style.getFontStyle());
        if (typeface.get() != nullptr) {
            // Resolve all unresolved characters
            auto font = makeFont(typeface, style.getFontSize(), style.getHeight());
            resolveAllCharactersByFont(font);
...
size_t FontCollection::resolveAllCharactersByFont(std::pair<SkFont, SkScalar> font) {
...
void FontCollection::addResolvedWhitespacesToMapping() {
...
std::pair<SkFont, SkScalar> FontCollection::makeFont(sk_sp<SkTypeface> typeface,
                                                   SkScalar size,
                                                   SkScalar height) {
...
SkUnichar FontCollection::firstUnresolved() {
