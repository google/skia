...
FontCollection::FamilyKey::FamilyKey(const std::vector<SkString>& familyNames, SkFontStyle style)
        : fFontStyle(style) {
    for (const SkString& familyName : familyNames) {
        fFamilyNames += familyName;
        fFamilyNames += ',';
    }
}
...
size_t FontCollection::FamilyKey::Hasher::operator()(const FontCollection::FamilyKey& key) const {
    return std::hash<std::string>()(key.fFamilyNames.c_str()) ^
