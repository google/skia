...
void FontResolver::findAllFontsForStyledBlock(const TextStyle& style, TextRange textRange) {
    ...
    const char* end = fMaster->text().begin() + textRange.end;
    const char* current = fMaster->text().begin() + textRange.start;
    while (current != end) {
    ...
    if (fUnresolved > 0 && fMaster->fontCollection()->fontFallbackEnabled()) {
        while (fUnresolved > 0) {
            ...
            auto typeface = fMaster->fontCollection()->defaultFallback(unicode, style.getFontStyle(), style.getLocale().c_str());
            if (typeface == nullptr) {
                break;
            }
            auto font = makeFont(typeface, style.getFontSize(), style.getHeight());
