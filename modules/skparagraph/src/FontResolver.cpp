...
SkFont FontResolver::firstResolvedFont() {
    if (fFirstResolvedFont.getTypeface() == nullptr) {
        // Time for last resort font
        auto font = makeFont(fFontCollection->defaultFallback(0, SkFontStyle()), 14, 1);
        fFirstResolvedFont = font.first;
    }
    return fFirstResolvedFont;
}
