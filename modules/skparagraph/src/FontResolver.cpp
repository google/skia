...
void FontResolver::findAllFontsForStyledBlock(const TextStyle& style, TextRange textRange) {
    ...
    if (fUnresolved > 0 && fFontCollection->fontFallbackEnabled()) {
        while (fUnresolved > 0) {
            ...
            auto typeface = fFontCollection->defaultFallback(unicode, style.getFontStyle(), style.getLocale().c_str());
            if (typeface == nullptr) {
                break;
            }
            auto font = makeFont(typeface, style.getFontSize(), style.getHeight());
    ...
    if (fResolvedFonts.count() == 0) {
        auto result = fFontCollection->defaultFallback(firstUnresolved(), style.getFontStyle(), style.getLocale().c_str());
        if (result == nullptr) {
