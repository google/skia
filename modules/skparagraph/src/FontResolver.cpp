...
size_t FontResolver::resolveAllCharactersByFont(const FontDescr& font) {
    ...
    auto processRuns = [&]() {
        ...
        if (resolved.width() == whitespaces.width()) {
            ...
            SkDebugf("Resolved: @%d: %s, %f, %d\n",
                    fCharacters[resolved.start] - fText.begin(),
                    name.c_str(),
                    font.fFont.getSize(),
                    font.fFont.getTypeface()->fontStyle());
        }
    };
    ...
    SkString name;
    font.fFont.getTypeface()->getFamilyName(&name);
    SkDebugf("See what is resolved by %s, %d\n", name.c_str(), font.fFont.getTypeface()->fontStyle());
