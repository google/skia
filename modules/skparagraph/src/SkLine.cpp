...
SkScalar SkLine::paintText(SkCanvas* canvas, SkSpan<const char> text, const SkTextStyle& style,
    ...
    return this->iterateThroughRuns(
            text, offsetX, false,
            [paint, canvas, shiftDown](SkRun* run, int32_t pos, size_t size, SkRect clip,
...
SkScalar SkLine::paintShadow(SkCanvas* canvas, SkSpan<const char> text, const SkTextStyle& style,
    ...
    for (SkTextShadow shadow : style.getShadows()) {
        ...
        result = this->iterateThroughRuns(
                text, offsetX, false,
                [canvas, shadow, paint, shiftDown](SkRun* run, size_t pos, size_t size, SkRect clip,
                                                   SkScalar shift, bool clippingNeeded) {
                    SkTextBlobBuilder builder;
...
void SkLine::computeDecorationPaint(SkPaint& paint,
                                    SkRect clip,
...
void SkLine::justify(SkScalar maxWidth) {
    ...
    this->iterateThroughClustersInGlyphsOrder(
            false, [&whitespacePatches, &textLen, &whitespacePatch](const SkCluster* cluster) {
    ...
    whitespacePatch = false;
    this->iterateThroughClustersInGlyphsOrder(false, [&](const SkCluster* cluster) {
        if (cluster->isWhitespaces()) {
            if (!whitespacePatch) {
                shift += step;
                whitespacePatch = true;
...
void SkLine::createEllipsis(SkScalar maxWidth, const std::string& ellipsis, bool) {
    ...
    iterateThroughClustersInGlyphsOrder(
            true, [this, &width, ellipsis, maxWidth](const SkCluster* cluster) {
                if (cluster->isWhitespaces()) {
                    width -= cluster->width();
                    return true;
                }
...
SkRun* SkLine::shapeEllipsis(const std::string& ellipsis, SkRun* run) {
    ...
    private:
        ...
        Buffer runBuffer(const RunInfo& info) override {
            fRun = fEllipsisCache.set(info.fFont,
                                      SkRun(SkSpan<const char>(), info, fLineHeight, 0, 0));
    ...
    shaper->shape(ellipsis.data(), ellipsis.size(), run->font(), true,
