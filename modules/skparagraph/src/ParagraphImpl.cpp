...
bool ParagraphImpl::shapeTextIntoEndlessLine() {
    ...
    if (!fFontCollection->getParagraphCache()->findParagraph(this)) {
        ...
        // Iterate through all the placeholders as shaping regions
        // (there is one empty placeholer in the end to shape the rest of the text)
        SkScalar advanceX = 0;
        for (auto& placeholder : fPlaceholders) {
            ...
            if (placeholder.fTextBefore.width() > 0) {
                ...
                ShapeHandler handler(*this, placeholder.fTextBefore.start, &font, advanceX);
