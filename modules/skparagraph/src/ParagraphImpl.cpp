...
std::vector<TextBox> ParagraphImpl::getRectsForRange(unsigned start,
    ...
    for (auto& line : fLines) {
        ...
        auto lineResults = line.getRectsForRange(intersect, rectHeightStyle, rectWidthStyle);
        ...
        if (rectWidthStyle == RectWidthStyle::kMax && !lastLine) {
            ...
            auto left = lineResults.front();
            auto right = lineResults.back();
            if (left.rect.fLeft > lineStart && left.direction == TextDirection::kRtl) {
                left.rect.fRight = left.rect.fLeft;
                left.rect.fLeft = 0;
                lineResults.insert(lineResults.begin() + 1, left);
            }
            if (right.direction == TextDirection::kLtr &&
                ...
                lineResults.emplace_back(right);
            }
        }

        for (auto& r : lineResults) {
