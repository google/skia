...
std::vector<TextBox> ParagraphImpl::getRectsForRange(unsigned start,
    ...
    if (start < fCodePoints.size()) {
        ...
        if (start > grapheme.fCodepointRange.start) {
            if (end == grapheme.fCodepointRange.end &&
                start == grapheme.fCodepointRange.end - 1) {
    ...
    if (end <= fCodePoints.size()) {
        ...
        auto grapheme = fGraphemes16[endCodepoint.fGrapheme];
        if (end >= grapheme.fCodepointRange.end) {
            text.end = grapheme.fTextRange.end;
        } else {
