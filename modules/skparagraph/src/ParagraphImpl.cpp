...
std::vector<TextBox> ParagraphImpl::getRectsForRange(unsigned start,
    ...
    if (end < fCodePoints.size()) {
        auto endGrapheme = fGraphemes16[fCodePoints[end - 1].fGrapheme];
        if (endGrapheme.fCodepointRange.end <= end) {
        ...
        line.iterateThroughVisualRuns(true,
            ...
            (const Run* run, SkScalar runOffset, TextRange textRange, SkScalar* width) {
                ...
                auto findCodepointByTextIndex = [this](ClusterIndex clusterIndex8) {
                    return
                        clusterIndex8 < fUtf8Text.size()
                            ? fUtf8Text[clusterIndex8].fCodepointRange
                            : CodepointRange(fUtf8Text.size(), fUtf8Text.size());
                };
