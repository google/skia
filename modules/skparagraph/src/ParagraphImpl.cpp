...
std::vector<TextBox> ParagraphImpl::getRectsForRange(unsigned start,
    ...
    for (auto& line : fLines) {
        ...
        line.iterateThroughVisualRuns(true,
            [&](TextLine::ClipContext context, SkScalar runOffset, TextRange textRange) {
                ...
                context = line.measureTextInsideOneRun(intersect, context.run, runOffset, true);
...
std::vector<TextBox> ParagraphImpl::GetRectsForPlaceholders() {
  ...
  for (auto& line : fLines) {
      ...
      (TextLine::ClipContext context, SkScalar runOffset, TextRange textRange) {
          ...
          if (context.run->textRange().width() == 0) {
              return false;
          }
          SkRect clip = context.clip;
          clip.offset(line.offset());
          boxes.emplace_back(clip, context.run->leftToRight() ? TextDirection::kLtr : TextDirection::kRtl);
          return true;
      });
  }
        ...
        line.iterateThroughVisualRuns(true,
            ...
            (TextLine::ClipContext context, SkScalar runOffset, TextRange textRange) {
                ...
                for (size_t i = context.pos; i < context.pos + context.size; ++i) {
                    if (context.run->positionX(i) + context.shift > dx) {
                        break;
                    }
                    found = i;
                }
                ...
                if (graphemeSize > 1) {
                    ...
                    center = (codepointStart + codepointEnd) / 2 + context.shift;
                } else {
