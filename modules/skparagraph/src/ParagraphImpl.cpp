...
SkUnichar utf16_next(const uint16_t** ptr, const uint16_t* end) {
    return SkUTF::NextUTF16(ptr, end);
    return 0;
}
*/
...
bool operator < (Codepoint lhs, Codepoint rhs) { return lhs.fTextIndex < rhs.fTextIndex; }
...
void ParagraphImpl::markGraphemes() {
    ...
    while (!breaker.eof()) {
        ...
        // Collect all the clusters that belong to the grapheme
        while (clusters.end < fClusters.size() && fClusters[clusters.end].textRange().end <= endPos) {
            ++clusters.end;
        }

        // Update all the clusters that intersect with this grapheme
        for (auto i = clusters.start; i < clusters.end; ++i) {
            auto& cluster = fClusters[i];
            if (cluster.fGraphemeRange.start == EMPTY_INDEX) {
                cluster.fGraphemeRange.start = fGraphemes.size();
            }
            cluster.fGraphemeRange.end = fGraphemes.size();
        }
        ...
        fGraphemes.emplace_back(codepoints, clusters, TextRange(startPos, endPos));
    ...
    for (auto& line : fLines) {
        ...
        line.iterateThroughRuns(
            ...
            (Run* run, size_t pos, size_t size, TextRange, SkRect clip, SkScalar shift, bool clippingNeeded) {
                ...
                auto codepoint = std::lower_bound(fCodePoints.begin(), fCodePoints.end(),
                        Codepoint(EMPTY_INDEX, clusterIndex8));
                ...
                // TODO: Assume for now that one grapheme is one glyph
                SkScalar center;
                if (graphemeSize > 1) {
