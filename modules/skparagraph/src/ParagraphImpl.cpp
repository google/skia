...
void ParagraphImpl::layout(SkScalar rawWidth) {
    ...
    if (fState < kShaped) {
        ...
        this->fTextIndexByCodepoint.reset();
        this->fCodepointIndexByText.reset();
        this->fRuns.reset();
...
std::vector<TextBox> ParagraphImpl::getRectsForRange(unsigned start,
    ...
    if (start >= end || start > fTextIndexByCodepoint.size() || end == 0) {
        return results;
    }
    ...
    if (start < fTextIndexByCodepoint.size()) {
        text.start = findGraphemeStart(fTextIndexByCodepoint[start]);
    }
    if (end < fTextIndexByCodepoint.size()) {
        text.end = findGraphemeStart(fTextIndexByCodepoint[end]);
    }
...
void ParagraphImpl::setState(InternalState state) {
    ...
    switch (fState) {
        case kUnknown:
            ...
            fWords.clear();
            fBidiRegions.reset();
            fTextIndexByCodepoint.reset();
            fCodepointIndexByText.reset();
            [[fallthrough]];
...
void ParagraphImpl::getCodepoints() {
    if (!fCodepointIndexByText.empty()) {
        return;
    }
    ...
    while (ptr < end) {
        ...
        for (auto i = index; i < next; ++i) {
            fCodepointIndexByText.emplace_back(fTextIndexByCodepoint.size());
        }
        SkASSERT(fCodepointIndexByText.size() == next);

        // One or two codepoints refer to the same text index
        uint16_t buffer[2];
        size_t count = SkUTF::ToUTF16(u, buffer);
        fTextIndexByCodepoint.emplace_back(index);
        if (count > 1) {
            fTextIndexByCodepoint.emplace_back(index);
        }
    }
    fCodepointIndexByText.emplace_back(fTextIndexByCodepoint.size());
    fTextIndexByCodepoint.emplace_back(fText.size());
}
