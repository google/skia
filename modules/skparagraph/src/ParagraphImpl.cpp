...
std::vector<TextBox> ParagraphImpl::getRectsForRange(unsigned start,
    ...
    if (end < fCodePoints.size()) {
        ...
        auto cluster = fClusters[fTextToClusters[codepoint.fTextIndex]];
        text.end = cluster.fTextRange.start;

        auto endGrapheme = fGraphemes16[codepoint.fGrapheme];
        if (text.end > endGrapheme.fTextRange.start) {
    ...
    if (start < fCodePoints.size()) {
        ...
        auto cluster = fClusters[fTextToClusters[codepoint.fTextIndex]];
        text.start = cluster.fTextRange.start;
        ...
        if (end < startGrapheme.fCodepointRange.end) {
            ...
            text.end = startGrapheme.fTextRange.start;
        } else if (end == startGrapheme.fCodepointRange.end) {
            text.start = startGrapheme.fTextRange.start;
            text.end = startGrapheme.fTextRange.end;
        }
