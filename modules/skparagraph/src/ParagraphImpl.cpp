...
bool ParagraphImpl::computeCodeUnitProperties() {
    ...
    // Get all spaces
    fUnicode->iterateThroughSpaces(fText.c_str(), fText.size(),
    [this](size_t start, size_t end, SkUnichar unichar) {

              if (fUnicode->isWhitespace(unichar)) {
                  for (size_t i = start; i < end; ++i) {
                      fCodeUnitProperties[i] |=  CodeUnitFlags::kPartOfWhiteSpaceBreak;
                  }
              } else if (fUnicode->isSpace(unichar)) {
                  for (size_t i = start; i < end; ++i) {
...
std::pair<bool, bool> ParagraphImpl::isSpaces(TextRange textRange) {
    ...
    for (auto i = textRange.start; i < textRange.end; ++i) {
        ...
        if (properties & CodeUnitFlags::kPartOfWhiteSpaceBreak) {
            ++whiteSpacesBreakLen;
        } else if (properties & CodeUnitFlags::kPartOfIntraWordBreak) {
            ++intraWordBreakLen;
        }
    }
