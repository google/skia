...
void ParagraphImpl::layout(SkScalar rawWidth) {
    ...
    if (fState < kMarked) {
        ...
        this->resetShifts();
        this->fClusterIndexes.reset();
        this->fClusterIndexes.push_back_n(fText.size() + 1, EMPTY_INDEX);
...
void ParagraphImpl::buildClusterTable() {
    ...
    for (auto& run : fRuns) {
        ...
        if (run.isPlaceholder()) {
            ...
            for (auto i = run.textRange().start; i < run.textRange().end; ++i) {
                fClusterIndexes[i] = fClusters.size();
            }
    ...
    fClusterIndexes[fText.size()] = fClusters.size();
