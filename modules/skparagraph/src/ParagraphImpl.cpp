...
void ParagraphImpl::layout(SkScalar rawWidth) {
    ...
    if (fState < kShaped) {
        ...
        this->fCodeUnitProperties.push_back_n(fText.size() + 1, CodeUnitFlags::kNone);
        this->fWords.clear();
...
bool ParagraphImpl::extractICUInformation() {
...
bool ParagraphImpl::extractICUWords() {
...
void ParagraphImpl::iterateThroughIcuProperties(CodeUnitFlags filter, IcuVisitor visitor) {

    size_t first = 0;
    for (size_t i = 1; i < fText.size(); ++i) {
        auto properties = fCodeUnitProperties[i];
        if (properties & filter) {
            visitor(first, i, properties);
            first = i;
        }

    }
    visitor(first, fText.size(), fCodeUnitProperties[first]);
}
...
void ParagraphImpl::setState(InternalState state) {
    ...
    switch (fState) {
        case kUnknown:
            ...
            fCodeUnitProperties.push_back_n(fText.size() + 1, CodeUnitFlags::kNone);
            fWords.clear();
            fBidiRegions.reset();
            fGraphemes16.reset();
            fCodepoints.reset();
