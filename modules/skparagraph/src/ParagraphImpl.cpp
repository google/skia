...
void ParagraphImpl::resolveStrut() {
    ...
    for (auto& fontFamily : strutStyle.fFontFamilies) {
        typeface = fFontCollection->matchTypeface(fontFamily.c_str(), strutStyle.fFontStyle);
    ...
    fStrutMetrics =
            LineMetrics(metrics.fAscent * strutStyle.fHeight,
...
void ParagraphImpl::shapeTextIntoEndlessLine() {
    TRACE_EVENT0("skia", TRACE_FUNC);
    ...
    FontIterator font(fUtf8, styles, fFontCollection, fParagraphStyle.hintingIsOn());
...
void ParagraphImpl::breakShapedTextIntoLines(SkScalar maxWidth) {
    ...
    textWrapper.breakTextIntoLines(
            ...
            [&](Cluster* start,
                ...
                auto& line =
                        this->addLine(offset, advance, text, clusters, startPos, endPos, metrics);
                if (addEllipsis) {
...
PositionWithAffinity ParagraphImpl::getGlyphPositionAtCoordinate(SkScalar dx, SkScalar dy) {
    ...
    for (auto& line : fLines) {
        ...
        line.iterateThroughRuns(
                ...
                0,
                true,
                [dx, dy, &result](Run* run, size_t pos, size_t size, SkRect clip, SkScalar shift,
                                  bool clippingNeeded) {
                    if (dx < clip.fLeft) {
