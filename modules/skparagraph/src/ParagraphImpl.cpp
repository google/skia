...
void ParagraphImpl::layout(SkScalar rawWidth) {
    ...
    std::chrono::milliseconds endTime = std::chrono::duration_cast<std::chrono::milliseconds>(
            std::chrono::system_clock::now().time_since_epoch());
    SkString shortText(fText.c_str(), std::min(100ul, fText.size()));
    SkDebugf("layout('%s'\n, %f): %d %f %f\n", shortText.c_str(),
             endTime.count() - startTime.count(), rawWidth, fMinIntrinsicWidth, fMaxIntrinsicWidth);
}
...
std::vector<TextBox> ParagraphImpl::getRectsForRange(unsigned start,
    ...
    std::chrono::milliseconds endTime =
            std::chrono::duration_cast<std::chrono::milliseconds>(
                    std::chrono::system_clock::now().time_since_epoch());
    SkDebugf("getRectsForRange(%d, %d): %d\n", start, end, endTime.count() - startTime.count());
...
PositionWithAffinity ParagraphImpl::getGlyphPositionAtCoordinate(SkScalar dx, SkScalar dy) {
    ...
    SkDebugf("getGlyphPositionAtCoordinate(%f,%f) = %d %d\n", dx, dy, result.position, endTime.count() - startTime.count());
    return result;
}
