...
void ParagraphImpl::resolveStrut() {
    ...
    if (strutStyle.getHeight() != 0 && strutStyle.getHeight() != 1) {
        auto strutHeight = metrics.fDescent - metrics.fAscent + metrics.fLeading;
        fStrutMetrics = LineMetrics(
                metrics.fAscent / strutHeight * strutStyle.getHeight() * strutStyle.getFontSize(),
                metrics.fDescent / strutHeight * strutStyle.getHeight() * strutStyle.getFontSize(),
...
PositionWithAffinity ParagraphImpl::getGlyphPositionAtCoordinate(SkScalar dx, SkScalar dy) {
    ...
    for (auto& line : fLines) {
        ...
        auto lastLine = &fLines.back() == &line;
        line.iterateThroughRuns(
                line.textWithSpaces(),
                0,
                true,
                [dx, &result, lastLine](Run* run, size_t pos, size_t size, SkRect clip, SkScalar shift,
                              bool clippingNeeded) {
                    if (dx < clip.fLeft) {
