...
void OneLineShaper::sortOutGlyphs(std::function<void(GlyphRange)>&& sortOutUnresolvedBLock) {
    ...
    for (size_t i = 0; i < fCurrentRun->size(); ++i) {
        ...
        bool isControl = u_iscntrl(codepoint);
        bool isWhitespace = u_isWhitespace(codepoint);

        // Inspect the glyph
        auto glyph = fCurrentRun->fGlyphs[i];
        if (glyph == 0 && !isControl) { // Unresolved glyph and not control codepoint
            ++unresolvedGlyphs;
            if (block.start == EMPTY_INDEX) {
                // Start new unresolved block
                block.start = whitespacesStart == EMPTY_INDEX ? i : whitespacesStart;
