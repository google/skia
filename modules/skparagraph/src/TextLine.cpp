...
TextLine::TextLine(SkVector offset, SkVector advance, SkSpan<const TextBlock> blocks,
               SkSpan<const char> text, SkSpan<const Cluster> clusters, size_t startPos,
               size_t endPos, LineMetrics sizes)
        : fBlocks(blocks)
        , fText(text)
        , fClusters(clusters)
        , fStartPos(startPos)
...
void TextLine::scanStyles(SkStyleType style, const StyleVisitor& visitor) {
    if (this->empty()) {
        return;
    }
...
void TextLine::createEllipsis(SkScalar maxWidth, const std::string& ellipsis, bool) {
    TRACE_EVENT0("skia", TRACE_FUNC);
...
Run* TextLine::shapeEllipsis(const std::string& ellipsis, Run* run) {
    TRACE_EVENT0("skia", TRACE_FUNC);
...
void TextLine::iterateThroughStylesInTextOrder(SkStyleType styleType,
