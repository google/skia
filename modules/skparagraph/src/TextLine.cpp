...
TextLine::ClipContext TextLine::measureTextInsideOneRun(TextRange textRange,
    ...
    if (result.clip.fRight > fAdvance.fX && !includeGhostSpaces) {
        result.clippingNeeded =
                !SkScalarNearlyEqual(result.clip.fRight, fAdvance.fX, SK_ScalarNearlyZero * fClusterRange.width()/100.0);
        if (result.clippingNeeded) {
...
void TextLine::iterateThroughVisualRuns(bool includingGhostSpaces, const RunVisitor& visitor) const {
    ...
    if (!includingGhostSpaces &&
        !SkScalarNearlyEqual(runOffset, this->width(), SK_ScalarNearlyZero * fClusterRange.width()/100.0)) {
