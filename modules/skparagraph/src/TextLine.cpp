...
TextLine::TextLine(ParagraphImpl* master,
        ...
        , fDisableAscent(false)
        , fDisableDescent(false) {
    // Reorder visual runs
...
TextLine::ClipContext TextLine::measureTextInsideOneRun(TextRange textRange,
    ...
    if (run->fEllipsis) {
        ...
        result.clip = SkRect::MakeXYWH(runOffsetInLine,
                                       sizes().runTop(run),
                                       run->advance().fX,
                                       run->calculateHeight(this->fDisableAscent, this->fDisableDescent));
        return result;
        ...
        if (SkScalarIsFinite(run->fFontMetrics.fAscent)) {
          result.clip = SkRect::MakeXYWH(runOffsetInLine,
                                         sizes().runTop(run),
                                         run->advance().fX,
                                         // TODO: do we ignore TextHeightBehavior for placeholders? Need a confirmation
                                         run->calculateHeight(false, false));
        } else {
...
*/
    ...
    result.clip =
            SkRect::MakeXYWH(0,
                             sizes().runTop(run, this->fDisableAscent),
        ...
        (TextRange textRange, const TextStyle& style, const TextLine::ClipContext& lineContext) {
            ...
            switch (rectHeightStyle) {
                ...
                case RectHeightStyle::kIncludeLineSpacingTop: {
                    if (isFirstLine()) {
                        auto verticalShift =  this->sizes().runTop(context.run, true);
