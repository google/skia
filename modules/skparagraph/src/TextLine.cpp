...
TextLine::ClipContext TextLine::measureTextInsideOneRun(TextRange textRange,
    ...
    auto round1 = littleRound(result.clip.fRight);
    auto round2 = littleRound(fAdvance.fX);
    if (round1 > round2 && !includeGhostSpaces) {
...
void TextLine::iterateThroughVisualRuns(bool includingGhostSpaces, const RunVisitor& visitor) const {
    ...
    auto round1 = littleRound(runOffset);
    auto round2 = littleRound(this->width());
    if (!includingGhostSpaces && round1 != round2) {
