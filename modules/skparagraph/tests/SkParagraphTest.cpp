...
DEF_TEST(SkParagraph_NoCache1, reporter) {
    ...
    auto test = [&](const char* test, const char* text, bool editing, bool wasted) {
        ParagraphBuilderImpl builder(paragraph_style, fontCollection);
        //SkDebugf("test %s:\n", test);
        builder.pushStyle(text_style);
        builder.addText(text);
        ...
        auto impl = static_cast<ParagraphImpl*>(paragraph.get());
        REPORTER_ASSERT(reporter, (countBefore == countAfter) == (wasted || editing));
        REPORTER_ASSERT(reporter, cache->isTooMuchMemoryWasted(impl) == wasted);
    };

    // Long arabic text
    str.append(text);
    test("Long arabic text", str.c_str(), false, false);

    // Add one character
    str.append("عل");
    test("+1 character", str.c_str(), true, false);

    // Remove 2 characters
    auto len = strlen("عل");
    str.resize(str.size() - len * 2);
    test("-2 characters", str.c_str(), true, false);

    // Too long arabic text
    str.append(" ");
    str.append(text);
    test("Double the text1", str.c_str(), false, false);
    ...
    test("Toooooo long text2", str.c_str(), false, false);


    str.append("عل");
    test("Toooooo long text3", str.c_str(), true, false);
