...
public:
        ...
        : fData(std::move(data))
    {
      ...
      fMemInputStream = new parselib::MemInputStream(const_cast<unsigned char *>(
					    static_cast<const unsigned char *>(fData->data())),
                                            static_cast<int32_t>(fData->size()));
      fReader = std::make_unique<parselib::WavStreamReader>(
            parselib::WavStreamReader{fMemInputStream});
      fReader->parse();
