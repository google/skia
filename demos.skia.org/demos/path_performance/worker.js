...
addEventListener('message', async ({ data: {svgData, offscreenCanvas, type, switchMethod} }) => {
    ...
    // The worker receives a "switchMethod" message whenever the user clicks a different rendering
    // method button
    if (switchMethod) {
        //pause other renderers, start correct renderer
        //report fps using postMessage
