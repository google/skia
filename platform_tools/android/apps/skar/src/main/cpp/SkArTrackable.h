...
public:
    ...
    /**
    * Used to check the current SkArTrackingState of the SkArTrackable
    * @param session           current SkArSession
    * @param trackingState     out variable containing state
    */
    void getTrackingState(SkArTrackingState* outTrackingState);

    /**
     * @param trackableType out variable in which to store this SkArTrackable's type
     */
    void getTrackableType(SkArTrackableType* outTrackableType);

    /**
     * @return vector of raw pointers to SkArAnchors contained within this SkArTrackable
     */
    std::vector<SkArAnchor*> getAnchors();

private:
    SkArTrackable();

    SkArTrackable(sk_sp<SkArSession> session, ArTrackable* trackable);
    ...
    friend class SkArHit;
    friend class SkArSession;
};
