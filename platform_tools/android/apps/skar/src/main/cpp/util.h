    ...
    namespace util {
        ...
        struct MatrixComputationInfo {
            ...
            MatrixComputationInfo(std::vector<glm::vec3> skiaAxes, glm::mat4 initRotation,
                                  glm::mat4 backToOrigin, glm::mat4 backToPlane,
                                  glm::mat4 planeModel, glm::mat4 hitModel)
                : skiaAxes(skiaAxes), initRotation(initRotation), backToOrigin(backToOrigin),
                  backToPlane(backToPlane), planeModel(planeModel), hitModel(hitModel) {};
        };
        ...
        /* ARCore utils */
        void GetTransformMatrixFromPose(ArSession *ar_session, const ArPose *ar_pose,
        ...
        // Performs a hit test at a screen location. Returns a list
        // of hit test results + size of list
        ...
        // Returns true if and only if the hit pose is inside the plane
        // polygon & is from above the plane.
...
/*        // Releases memory allocated by a hitResult traversal: hitRes, list, & corresponding pose
        ...
        // Computes an initial rotation matrix to be applied to all Skia objects
        // (moves from XY to XZ plane)
        ...
        void SetCameraAlignedHorizontal(glm::mat4& caMat, ArPlaneType planeType,
                                        const glm::vec3 hitLook, const MatrixComputationInfo* info);
    }  // namespace util
}  // namespace hello_ar
