...
sk_sp<SkArSession> SkArSession::Make(void* env, void* context) {
    ...
    if (outSession->fArSession == nullptr) {
...
std::vector<std::unique_ptr<SkArTrackable>> SkArSession::getAllTrackables(SkArTrackableType filter) {
    // Initialize structs
    ...
    for (int i = 0; i < listSize; i++) {
        ...
        unique_ptr<SkArTrackable> currSkArHit(new SkArTrackable(this, currTrackable));
