...
namespace hello_ar {
    ...
    HelloArApplication::~HelloArApplication() {
        if (ar_session_ != nullptr) {
            ...
            //Arsession is a shared ptr, handles itself
            //ArFrame_destroy(ar_frame_);
            //Arframe is a shared_ptr, handles itself
        }
    }
...
/*
        ...
        ArCamera_getProjectionMatrix(ar_session_, ar_camera, 0.1f, 100.f,
...
*/
    ...
    void HelloArApplication::GetSkModelMatrices(std::vector<SkMatrix44>& outSkModels) {
        for (const auto &obj_iter : tracked_obj_set_) {
            ...
            ArAnchor_getTrackingState(ar_session_->getArSession(), obj_iter->GetArAnchor(), &tracking_state);
    ...
    void HelloArApplication::OnDrawFrame() {
        ...
        ar_session_->setCameraTextureName(background_renderer_.GetTextureId());

        // Update session to get current frame and render camera background.
       /* if (ArSession_update(ar_session_, ar_frame_) != AR_SUCCESS) {
            LOGE("HelloArApplication::OnDrawFrame ArSession_update error");
        }*/
        if (!ar_session_->update(ar_frame_)) {
            LOGE("HelloArApplication::OnDrawFrame ArSession_update error");
        }
        ...
        ArFrame_getLightEstimate(ar_session_->getArSession(), ar_frame_->getArFrame(), ar_light_estimate);
        ...
        plane_renderer_.Draw(ar_session_->getArSession(), ar_frame_->getArFrame(), grContext.get(), skSurface.get(), vpv, plane_count_);
        point_cloud_renderer_.Draw(ar_session_->getArSession(), ar_frame_->getArFrame(), skSurface.get(), vpv);
    ...
    bool HelloArApplication::OnTouchedFirst(float x, float y, int drawMode) {
        ...
        if (ar_frame_ != nullptr && ar_session_ != nullptr) {
            ...
            util::GetHitTestInfo(ar_session_->getArSession(), ar_frame_->getArFrame(), x, y, hitResultListSize, &hitResultList);
            ...
            if (hitResult) {
                ...
                if (ArSession_acquireNewAnchor(ar_session_->getArSession(), hitPose, &newAnchor) == AR_SUCCESS) {
    ...
    void HelloArApplication::AddAnchorWrapper(AnchorWrapper* anchorW, ArPlane* containingPlane) {
        ...
        ArTrackingState tracking_state = AR_TRACKING_STATE_STOPPED;
        ArAnchor_getTrackingState(ar_session_->getArSession(), anchorW->GetArAnchor(), &tracking_state);
        if (tracking_state != AR_TRACKING_STATE_TRACKING) {
    ...
    void HelloArApplication::SetCameraAlignedMatrix(glm::mat4& caMat, glm::vec3 hitPos,
        ...
        util::GetCameraInfo(ar_session_->getArSession(), ar_frame_->getArFrame(), cameraPos, cameraRotationMatrix);

        //Set matrix depending on type of surface
        ArPlaneType planeType = AR_PLANE_VERTICAL;
        ArPlane_getType(ar_session_->getArSession(), pendingAnchor->GetContainingPlane(), &planeType);
    ...
    bool HelloArApplication::CheckNeighborAnchors(ArPlane* containingPlane, const glm::vec4& hitPosition) {
        ...
        if (planeAnchors != plane_anchors_map_.end()) {
            ...
            for(AnchorWrapper* a: anchorWrappers) {
                ...
                ArPose_create(ar_session_->getArSession(), nullptr, &anchorPose);
                ArAnchor_getPose(ar_session_->getArSession(), a->GetArAnchor(), anchorPose);
                float anchorPoseRaw[] = {0, 0, 0, 0, 0, 0, 0};
                ArPose_getPoseRaw(ar_session_->getArSession(), anchorPose, anchorPoseRaw);
    ...
    bool HelloArApplication::TraverseHitResultList(ArHitResultList* hitResultList, int index,
        ...
        if (!util::GetTrackableInfo(ar_session_->getArSession(), hitResultList, index, currHitResult, currTrackable)) {
        ...
        if (currTrackableType == AR_TRACKABLE_PLANE) {
            ...
            int32_t inPolygon = 0;
            ArPlane_isPoseInPolygon(ar_session_->getArSession(), currPlane, currHitPose, &inPolygon);
            float currHitPoseRaw[] = {0, 0, 0, 0, 0, 0, 0};

            if (!util::CheckHitLocation(ar_session_->getArSession(), ar_frame_->getArFrame(), currHitPose, inPolygon, currHitPoseRaw)) {
            ...
            *outHitPlane = currPlane;
            ...
            ArPose_create(ar_session_->getArSession(), hitPoseRaw, outHitPose);
            return false; //break traversal
        }
