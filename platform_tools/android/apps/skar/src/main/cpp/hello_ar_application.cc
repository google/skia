...
namespace hello_ar {
    ...
    bool GetCameraPerspectiveMatrices(sk_sp<SkArSession> ar_session_, sk_sp<SkArFrame> ar_frame_,
        ...
        std::unique_ptr<SkArCamera> arCamera = SkArCamera::Make(ar_session_.get(), ar_frame_.get());

        arCamera->getViewMatrix(ar_session_.get(), glm::value_ptr(outViewMat));
        arCamera->getProjectionMatrix(ar_session_.get(), 0.1f, 100.f, glm::value_ptr(outProjMat));

        SkArTrackingState* cameraTrackingState = nullptr;
        arCamera->getTrackingState(ar_session_.get(), cameraTrackingState);
...
/*            //Perform hit test & get # of hit objects
            ...
            //Perform hit test & get # of hit objects
            std::vector<std::unique_ptr<SkArHit>> hitResults = SkArHit::HitTest(ar_session_,
                                                                                ar_frame_.get(),
                                                                                x, y);
            ...
            unique_ptr<SkArHit> hitResult = nullptr;

            //Traverse the list of hit results
            for (auto i = 0; i < hitResults.size(); ++i) {
                /*bool traversalResult = TraverseHitResultList(hitResultList, i, &hitPose, &hitResult,
                                                             &hitPlane);*/
                bool traversalResult = TraverseHitResultList(hitResults, i, &hitPose, &hitResult, &hitPlane);
                if (traversalResult) {
    ...
    bool HelloArApplication::CheckNeighborAnchors(ArPlane* containingPlane,
        ...
        if (planeAnchors != plane_anchors_map_.end()) {
            ...
            for(AnchorWrapper* a: anchorWrappers) {
                ...
                a->getArAnchor()->getPose()->getPoseRaw(anchorPoseRaw);
    ...
    bool HelloArApplication::TraverseHitResultList(/*ArHitResultList* hitResultList,*/
                                                    std::vector<std::unique_ptr<SkArHit>>&
                                                    hitResultList, int index,
...
/*        ArHitResult* currHitResult = nullptr;
        ...
        std::unique_ptr<SkArHit> currHitResult(nullptr);
        std::unique_ptr<SkArTrackable> currTrackable(nullptr);

        if(!util::GetTrackableInfo(ar_session_, hitResultList, index, currHitResult,
                                   currTrackable)) {
            return true; //continue traversal

        }
        SkArTrackableType currTrackableType;
        currTrackable->getTrackableType(&currTrackableType);
        if (currTrackableType == SkArTrackableType::kPlane) {
            // Get Plane of hit location

            // Get Pose of hit location
            SkArPose* currHitPose = currHitResult->getPose();
        }
        ...
        if (currTrackableType == AR_TRACKABLE_PLANE) {
            ...
            (*outHitResult).reset(currHitResult);
            *outHitResult = std::move(currHitResult);
