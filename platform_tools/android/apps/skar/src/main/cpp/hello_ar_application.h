...
// HelloArApplication handles all application logics.
    ...
    private:
        ...
        void DrawBackground(SkCanvas* canvas, sk_sp<GrContext> context);

        // Fills a vector of SkMat44 with the model matrices to be drawn this frame
        void GetSkModelMatrices(std::vector<SkMatrix44>& outSkModels);

        /* Model matrix computations */
        // Updates model matrix maps with new anchor-matrix key-value pairs
        void AddModelMatrices(AnchorWrapper* anchorWrapperKey, glm::mat4 aaMat, glm::mat4 caMat,
                              glm::mat4 snapMat);

        // Updates a pre-existing anchor wrapper key in the model matrix maps
        void UpdateModelMatrices(AnchorWrapper* anchorWrapperKey, glm::mat4 aaMat, glm::mat4 caMat,
                                 glm::mat4 snapMat);

        // Handles creating all model matrices for a given anchor
        void SetModelMatrices(glm::mat4& aaMat, glm::mat4& caMat, glm::mat4& snapMat,
                              util::MatrixComputationInfo* info);

        // Called by SetModelMatrices to set the camera-aligned matrix
        void SetCameraAlignedMatrix(glm::mat4& caMat, glm::vec3 hitPos,
                                    const util::MatrixComputationInfo* info);

        // Creates and returns a unique_ptr to a MatrixComputationInfo given the
        // finalized pending anchor
        std::unique_ptr<util::MatrixComputationInfo>
        CreateMatrixComputationInfo(ArPlane* containingPlane);
        ...
        // If traversal must end/break, then pendingAnchor edit mode has been updated to true
        // or false
        bool TraverseHitResultList(/*ArHitResultList* hitResultList,*/
                                   std::vector<std::unique_ptr<SkArHit>>& hitResultList, int index,
                                   unique_ptr<SkArPose>* outHitPose, /*ArHitResult** outHitResult,*/
                                   unique_ptr<SkArHit>* outHitResult,
                                   ArPlane** outHitPlane);

        bool TraverseHitResultListOnTranslate(ArHitResultList* hitResultList, int index,
                                                unique_ptr<SkArPose>*, ArHitResult** outHitResult,
                                                ArPlane** outHitPlane, glm::mat4& outTranslate);

        /* Anchor management */
        // Handles all operations associated with deleting an anchor wrapper
        // (memory & state management)
        void RemoveAnchorWrapper(AnchorWrapper* anchorW);

        // Handles all operations associated with adding an anchor wrapper
        // (memory & state management)
        ...
        void ComputeAnchorWrapperPostTranslate(AnchorWrapper* oldWrapper,
                                               AnchorWrapper* newWrapper);

        // Checks if the pending anchor is valid (not null & is stored in anchor map)
        // on any edit call
        ...
        // ARCore state
        sk_sp<SkArSession> ar_session_ = nullptr;
        sk_sp<SkArFrame> ar_frame_ = nullptr;
