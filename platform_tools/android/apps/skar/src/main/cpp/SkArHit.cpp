...
std::unique_ptr<SkArTrackable> SkArHit::makeTrackable() {
    ArTrackable* trackable = nullptr;
    ArHitResult_acquireTrackable(fSkArSession->getArSession(), fArHitResult, &trackable);
    return std::unique_ptr<SkArTrackable>(new SkArTrackable(std::move(fSkArSession), trackable));
}
