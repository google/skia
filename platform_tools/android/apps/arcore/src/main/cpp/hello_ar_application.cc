...
namespace hello_ar {
	...
	void HelloArApplication::OnDrawFrame() {
		...
		if (surface != nullptr) {
			...
			DrawAxes(canvas, m);
			//DrawVector(canvas, vpv, begins[0], ends[0], SK_ColorYELLOW); //zDir at origin
			SkMatrix postScale;
			//postScale.setScale(0.01, 0.01);
			postScale.postConcat(m);
			canvas->concat(m);
			...
			shaper.shape(&builder, paint, text, byteLength, true,
			             p , 10);
		...
		if (ar_frame_ != nullptr && ar_session_ != nullptr) {
			...
			for (int32_t i = 0; i < hit_result_list_size; ++i) {
				...
				if (AR_TRACKABLE_PLANE == ar_trackable_type) {
					...
					backToOrigin = glm::translate(backToOrigin, -glm::vec3(objPos));
					glm::mat4 backToPlane(1);
					backToPlane = glm::translate(backToPlane, glm::vec3(objPos));

					//Axes: start with XYZ, totate to get X(-Z)Y, paste on plane, go back to origin --> plane orientation but on origin
					glm::vec3 xDir = glm::normalize(glm::vec3(backToOrigin * planeModel * initRotation * glm::vec4(1,0,0,1))); //X still X
					glm::vec3 yDir = glm::normalize(glm::vec3(backToOrigin * planeModel * initRotation * glm::vec4(0,1,0,1))); //Y is now Z
					glm::vec3 zDir = glm::normalize(glm::vec3(backToOrigin * planeModel * initRotation * glm::vec4(0,0,1,1))); //Z is now Y
					...
					begins.push_back(cameraPos);
					ends.push_back(hitPos);
					glm::vec3 localX = glm::normalize(glm::cross(zDir, hitLook));
					glm::vec3 localY = glm::normalize(glm::cross(zDir, localX));
					/*float multiple = util::dot(hitLook, -zDir) / (glm::length(zDir) * glm::length(zDir));
					glm::vec3 hitLookProj = -(hitLook - multiple * (-zDir));*/

					glm::vec3 hitLookProj = localY;

					float angleRad = acos(util::dot(hitLookProj, yDir) / (glm::length(hitLookProj) * glm::length(yDir)));
					LOGI("ANGLE IS: %.6f", glm::degrees(angleRad));
					glm::mat4 rotateTowardsCamera(1);
					rotateTowardsCamera = glm::rotate(rotateTowardsCamera, angleRad, zDir);
					matrixCameraAligned =   backToPlane * rotateTowardsCamera * backToOrigin * planeModel * initRotation;
			...
			if (ar_hit_result) {
				...
				anchor_skmat4_snap_aligned_map_.insert({anchor, util::glmMatToSkMat(matrixSnapAligned)});
