...
namespace hello_ar {
	...
	SkMatrix HelloArApplication::SkiaRenderer(const glm::mat4 &proj, const glm::mat4 &view, const glm::mat4 &model) {
		SkMatrix44 skProj = glmMatToSkMat(proj);
		SkMatrix44 skView = glmMatToSkMat(view);
		SkMatrix44 skModel = glmMatToSkMat(model);
		...
		SkMatrix44 m = skProj * skView * skModel;
		viewport.preConcat(m);
		return viewport;
	...
	void HelloArApplication::OnDrawFrame() {
		...
		for (const auto &obj_iter : tracked_obj_set_) {
			...
			if (tracking_state == AR_TRACKING_STATE_TRACKING) {
				...
				if (false) {
					...
					glm::mat4 andyMatrix = projection_mat * view_mat * model_mat;
				}
			}
		}
		SkString string;
		...
		SkMatrix44 mTest4 = SkMatrix44::kIdentity_Constructor;
		mTest4.postScale(2, 2, 1);
		SkMatrix id(mTest4);


		skView.preConcat(skModel);
		skProj.preConcat(skView);

		canvas->concat(skProj);
