...
/*			DrawVector(canvas, vpv, begins[0], ends[0], SK_ColorMAGENTA);
	...
	glm::mat4 GetCameraRotationMatrix(float cameraOutRaw[]) {
		...
		glm::quat cameraQuat = glm::quat(cameraOutRaw[0], cameraOutRaw[1], cameraOutRaw[2], cameraOutRaw[3]);
		...
		if (ar_frame_ != nullptr && ar_session_ != nullptr) {
			...
			for (int32_t i = 0; i < hit_result_list_size; ++i) {
				...
				if (AR_TRACKABLE_PLANE == ar_trackable_type) {
					...
					glm::vec3 objX = glm::normalize(glm::vec3(backToOrigin * planeModel * initRotation * glm::vec4(1,0,0,1))); //X still X
					glm::vec3 objY = glm::normalize(glm::vec3(backToOrigin * planeModel * initRotation * glm::vec4(0,1,0,1))); //Y is now Z
					glm::vec3 objZ = glm::normalize(glm::vec3(backToOrigin * planeModel * initRotation * glm::vec4(0,0,1,1))); //Z is now Y
					...
					if (planeType == ArPlaneType::AR_PLANE_VERTICAL) {
						...
						matrixCameraAligned =   backToPlane * flip *  rotateTowardsCamera * backToOrigin * planeModel * initRotation;
					} else {
