...
// @return true if obj is loaded correctly, otherwise false.
		...
		bool CreateAxes( std::vector<GLfloat> *out_vertices,
		                 std::vector<GLfloat> *out_colors,
		                 std::vector<GLushort> *out_indices);
...
// pose or hit test pose.
		...
		void printGlmMat(glm::mat4 m, char* type);

		void printSkMat44(SkMatrix44 m, char* type);

		void printSkMat(SkMatrix m, char* type);
