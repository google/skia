	...
	void HelloArApplication::OnDrawFrame() {
		...
		for (const auto &obj_iter : tracked_obj_set_) {
			...
			if (tracking_state == AR_TRACKING_STATE_TRACKING) {
				...
				skModel = util::glmMatToSkMat(model_mat);
				SkMatrix44 initRotation;
				initRotation.setRotateAboutUnit(1, 0, 0, SK_ScalarPI/2);
		...
		if (surface != nullptr) {
			SkMatrix44 m = skViewport * skProj * skView  * skModel;
			...
			SkMatrix postScale;
			postScale.setScale(0.01, 0.01);
			canvas->concat(postScale);

			/*printSkMat(m, "SkMatrix: ");
			SkMatrix matrix;
		...
		if (ar_frame_ != nullptr && ar_session_ != nullptr) {
			...
			for (int32_t i = 0; i < hit_result_list_size; ++i) {
				...
				if (AR_TRACKABLE_PLANE == ar_trackable_type) {
					...
					//Position of anchor
					glm::vec4 objPos(out_hit_raw[4], out_hit_raw[5], out_hit_raw[6], 1);

					//Plane model matrix
					glm::mat4 planeModel(1);
					util::GetTransformMatrixFromPose(ar_session_, plane_pose, &planeModel);
					planeModel[3] = objPos;
					matrixAxisAligned = planeModel;

					//STEPS:
					// 1) skia XZ to arcore plane XY
					// 2) rotate on plane
					// 3) tilt
					// 4) go to location

					//obj model, for skia starts in XY plane
					glm::mat4 objModel(1);
					objModel = glm::rotate(objModel, currentValue, glm::vec3(0, 0, 1));
/*

					//step 1: go to XZ plane
					glm::mat4 skiaToArcore(1);
					skiaToArcore = glm::rotate(skiaToArcore, 90.0f, glm::vec3(1, 0, 0));
					objModel = skiaToArcore;

					//step 2: rotate to look at camera
					//Vector: camera to plane
					...
					//OpenGL orientation: (1, 0, 0) || (0, 1, 0) || (0, 0, -1)
					//Plane up vector
					glm::vec4 planeUp4 = planeModel[1];
					glm::vec3 planeUp(planeUp4.x, planeUp4.y, planeUp4.z);
					//Project look vector on plane
					glm::vec3 localX(glm::cross(planeUp, hitLook));
					glm::vec3 localZ(glm::cross(planeUp, localX)); //projection of hitlook
					float cosine = util::dot(localZ, glm::vec3(planeModel[2])) / (glm::length(localZ) * glm::length(glm::vec3(planeModel[2])));
					float angle = glm::degrees(acos(cosine));
					glm::mat4 planeRotate(1);
					planeRotate = glm::rotate(planeRotate, angle, glm::vec3(0, 1, 0));
					objModel = planeRotate * objModel;
...
*/
				...
				/*	//step 3
					...
					glm::mat4 planeTilt(1);
					planeTilt = glm::rotate(planeTilt, angle, glm::vec3(planeUp));
					objModel = planeTilt * objModel;*/

					//step 4

					matrixCameraAligned = objModel;
/*



					//Wanted orientation of object
					glm::mat3 modelMatrix(glm::normalize(glm::vec3(localX)), planeUp, glm::vec3(localZ));
					glm::mat4 modelMatrix4(modelMatrix);
					modelMatrix4[3] = objPos;
					matrixCameraAligned = modelMatrix4;*/

					float wanted_raw_pose[] = {out_plane_raw[0], out_plane_raw[1], out_plane_raw[2], out_plane_raw[3],
			...
			if (ar_hit_result) {
				...
				/*if (ArHitResult_acquireNewAnchor(ar_session_, ar_hit_result, &anchor) !=
				    AR_SUCCESS) */
				if (ArSession_acquireNewAnchor(ar_session_, out_pose, &anchor) !=
				                       AR_SUCCESS) {
					LOGE("HelloArApplication::OnTouched ArHitResult_acquireNewAnchor error");
					return;
				}

				anchor_skmat4_axis_aligned_map_.insert({anchor, util::glmMatToSkMat(matrixAxisAligned)});
				anchor_skmat4_camera_aligned_map_.insert({anchor, util::glmMatToSkMat(matrixCameraAligned)});
