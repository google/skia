...
public class SkottieView extends FrameLayout {
    ...
    public SkottieView(Context context, AttributeSet attrs) {
        ...
        try {
            // set mLooping
            mLooping = a.getBoolean(R.styleable.SkottieView_looping, true);
    ...
    private SkottieAnimation setSourceHelper(InputStream inputStream) {
        if (mBackingView instanceof TextureView) {
            return SkottieRunner.getInstance()
                .createAnimation(((TextureView) mBackingView), inputStream, mBackgroundColor, mLooping);
        } else {
            return SkottieRunner.getInstance()
                .createAnimation(((SurfaceView) mBackingView), inputStream, mBackgroundColor, mLooping);
        }
    }
