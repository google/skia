            ...
            try {
                runOnGLThread(() -> {
                    ...
                    if (mEglSurface != null) {
                        ...
                        mEgl.eglMakeCurrent(mEglDisplay, mPBufferSurface, mPBufferSurface, mEglContext);
            ...
            try {
                ...
                if (mEglSurface != null) {
                    if (!mEgl.eglMakeCurrent(mEglDisplay, mEglSurface, mEglSurface, mEglContext)) {
                        ...
                        Log.w(LOG_TAG, "eglMakeCurrent failed " + GLUtils.getEGLErrorString(mEgl.eglGetError()));
                        mNewSurface = true;
                        return;
                    }

                    nDrawFrame(mNativeProxy, mSurfaceWidth, mSurfaceHeight, false, frameTimeNanos);
                    if (!mEgl.eglSwapBuffers(mEglDisplay, mEglSurface)) {
                        int error = mEgl.eglGetError();
                        if (error == EGL10.EGL_BAD_SURFACE || error == EGL10.EGL_BAD_NATIVE_WINDOW) {
                            // For some reason our surface was destroyed. Recreate EGL surface on next frame.
                            mNewSurface = true;
                            // This really shouldn't happen, but if it does we can recover easily
                            // by just not trying to use the surface anymore
                            Log.w(LOG_TAG, "swapBuffers failed "+ GLUtils.getEGLErrorString(error));
                            return;
                        }

                        // Some other fatal EGL error happened, log an error and stop the animation.
                        throw new RuntimeException("Cannot swap buffers " + GLUtils.getEGLErrorString(error));
                    }
                }
            } catch (Throwable t) {
