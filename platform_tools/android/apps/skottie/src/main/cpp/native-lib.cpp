...
struct SkottieAnimation {
    ...
    long                      mTimeBase;
    float                     mDuraiton; //in milliseconds
};

extern "C" JNIEXPORT jlong
JNICALL
Java_org_skia_skottie_SkottieRunner_00024SkottieAnimation_nCreateProxy(JNIEnv *env, jobject clazz, jlong runner, jobject is,
                                                                       jbyteArray storage) {
    ...
    skottieAnimation->mAnimation = skottie::Animation::Make(skottieAnimation->mStream.get(), &RP,
                                                            &skottieAnimation->mAnimationStats);
    skottieAnimation->mTimeBase  = 0.0f; // force a time reset
    skottieAnimation->mDuraiton = 1000 * skottieAnimation->mAnimation->duration();

    if (skottieAnimation->mAnimation) {
        skottieAnimation->mAnimation->setShowInval(false);
    } else {
...
extern "C" JNIEXPORT void
JNICALL
Java_org_skia_skottie_SkottieRunner_00024SkottieAnimation_nDeleteProxy(JNIEnv *env, jclass clazz, jlong nativeProxy) {
    if (!nativeProxy) {
        return;
    }
...
extern "C" JNIEXPORT void
JNICALL
Java_org_skia_skottie_SkottieRunner_00024SkottieAnimation_nDrawFrame(JNIEnv *env, jclass clazz, jlong nativeProxy, jint width,
                                             jint height, jboolean wideColorGamut, jlong frameTimeNanos) {
    ...
    if (skottieAnimation->mAnimation) {
        ...
        float intpart;
        float animState = modff(t / skottieAnimation->mDuraiton, &intpart);
