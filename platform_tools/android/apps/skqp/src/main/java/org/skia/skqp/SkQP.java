...
public class SkQP {
    ...
    protected void runTests(Context context, String outputDirPath) {
        ...
        Log.w(LOG_PREFIX, "Let's do this: " + outputDirPath);
        if (outputDir.exists()) {
          try {
        ...
        if (!outputDir.mkdirs()) {
          Log.w(LOG_PREFIX, "Failure to create directory " + outputDirPath);
        } else {
          Log.w(LOG_PREFIX, "Success creating directory " + outputDirPath);
        }
