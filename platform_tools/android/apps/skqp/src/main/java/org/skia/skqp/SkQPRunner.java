...
public class SkQPRunner extends Runner {
    ...
    public SkQPRunner(Class testClass) {
        impl = new SkQPRunnerImpl();
        Context context = InstrumentationRegistry.getTargetContext();
        File filesDir = context.getFilesDir();
        try {
            SkQPRunnerImpl.deleteDirectoryContents(filesDir);
        } catch (IOException e) {
        ...
        for (int backend = 0; backend < impl.mBackends.length; backend++) {
            String classname = SkQPRunnerImpl.kSkiaGM + impl.mBackends[backend];
        ...
        for (int unitTest = 0; unitTest < impl.mUnitTests.length; unitTest++) {
            mDescription.addChild(Description.createTestDescription(SkQPRunnerImpl.kSkiaUnitTests,
    ...
    public void run(RunNotifier notifier) {
        ...
        for (int unitTest = 0; unitTest < impl.mUnitTests.length; unitTest++) {
            Description desc = Description.createTestDescription(
                          SkQPRunnerImpl.kSkiaUnitTests, impl.mUnitTests[unitTest], annots);
            notifier.fireTestStarted(desc);
