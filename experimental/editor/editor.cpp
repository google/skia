...
void Editor::setFont(SkFont font) {
    if (font != fFont) {
        ...
        Editor::Shape(&textLine, shaper.get(), FLT_MAX, fFont);
...
static inline bool operator<(Editor::TextPosition u, Editor::TextPosition v) {
...
void Editor::paint(SkCanvas* c, PaintOpts options) {
    ...
    for (TextPosition pos = options.fSelectionBegin;
         pos < options.fSelectionEnd;
...
void Editor::reshapeAll() {
    if (fNeedsReshape) {
        ...
        for (TextLine& line : fLines) {
            if (!line.textBlob()) {
                executor->add([&]() {
                    Editor::Shape(&line, SkShaper::Make().get(), shape_width, fFont);
                    semaphore.signal();
                }
                ++jobCount;
            });
