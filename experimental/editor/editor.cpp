...
void Editor::shape(TextLine* line, SkShaper* shaper) {
    SkASSERT(line);
    SkASSERT(shaper);
    ...
    shaper->shape(line->fText.c_str(), line->fText.size(), fFont, true, width, &runHandler);
    float h = std::max(runHandler.endPoint().y(), fFont.getSpacing());
