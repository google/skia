...
Editor::TextPosition Editor::insert(TextPosition pos, const char* utf8Text, size_t byteLen) {
    ...
    if (pos.fLine < fLines.size()) {
        fLines[pos.fLine].fText.insert(pos.fColumn, utf8Text, byteLen);
        fLines[pos.fLine].fBlob = nullptr;
        fNeedsReshape = true;
        return Editor::TextPosition{pos.fColumn + SkToUInt(byteLen), pos.fLine};
    } else {
        // append new line
    }
    return pos;
...
Editor::TextPosition Editor::remove(TextPosition pos1, TextPosition pos2) {
    ...
    if (start == end || start.fLine == fLines.size()) {
        return start;
    }
    if (start.fLine == end.fLine) {
        SkASSERT(end.fColumn > start.fColumn);
        fLines[start.fLine].fText.remove(start.fColumn, end.fColumn - start.fColumn);
        fLines[start.fLine].fBlob = nullptr;
        fNeedsReshape = true;
    } else {
        // delete across lines
    }
...
static unsigned align_column(const StringSlice& str, unsigned p) {
    if (p >= str.size()) {
        return str.size();
    }
...
void Editor::paint(SkCanvas* c, PaintOpts options) {
    ...
    if (fLines.size() > 0) {
        ...
        if (options.fCursor.fColumn < cLine.fCursorPos.size()) {
