...
public:
    ...
    void addImage(const SkString& name, sk_sp<SkData> img) {
        sk_sp<skottie_utils::MultiFrameImageAsset> asset =
                                skottie_utils::MultiFrameImageAsset::Make(img);
...
public:
    static sk_sp<ManagedAnimation> Make(const std::string& json, SkottieAssetProvider* ap) {
        auto mgr = skstd::make_unique<skottie_utils::CustomPropertyManager>();
        auto provider = sk_sp<SkottieAssetProvider>(ap);
...
EMSCRIPTEN_BINDINGS(Skottie) {
    ...
    function("_MakeManagedAnimation", optional_override([](std::string json,
        ...
        SkottieAssetProvider* assetProvider = new SkottieAssetProvider();
        ...
        for (int i = 0; i < imageCount; i++) {
            ...
            sk_sp<SkData> bytes = SkData::MakeWithoutCopy(imgDatas[i],
                                                          imgSizes[i]);
            assetProvider->addImage(name, bytes);
        }

        return ManagedAnimation::Make(json, assetProvider);
    }));
