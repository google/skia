...
public:
    ...
    sk_sp<skottie::ImageAsset> loadImageAsset(const char path[],
        ...
        auto image_data = this->findAsset(combined.c_str());

        return image_data ? skottie_utils::MultiFrameImageAsset::Make(std::move(image_data))
                          : nullptr;
    }
...
EMSCRIPTEN_BINDINGS(Skottie) {
    ...
    function("_MakeManagedAnimation", optional_override([](std::string json,
        ...
        char**    assetNames = reinterpret_cast<char**>(nptr);
        uint8_t** assetDatas = reinterpret_cast<uint8_t**>(dptr);
        size_t*   assetSizes = reinterpret_cast<size_t*>(sptr);
        for (size_t i = 0; i < assetCount; i++) {
            SkString name(assetNames[i]);
