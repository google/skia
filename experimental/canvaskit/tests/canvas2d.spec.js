...
describe('CanvasKit\'s Canvas 2d Behavior', function() {
    ...
    describe('Path drawing API', function() {
        ...
        xit('can get and put pixels', function(done) {
            LoadCanvasKit.then(catchException(done, () => {
                multipleCanvasTest('get_put_imagedata', done, (canvas) => {
                    ...
                    let blackBox = ctx.createImageData(20, 40);
                    ctx.putImageData(blackBox, 10, 300);
                    let biggerBlackBox = ctx.createImageData(iData);
                    ctx.putImageData(biggerBlackBox, 10, 350);
                    expect(biggerBlackBox.width).toBe(iData.width);
                    expect(biggerBlackBox.height).toBe(iData.height);
                });
        }));
        });
