...
static int test6(std::vector<const Cmd*>* test, std::vector<int>* expectedOrder) {
    ...
    SkIRect r{0, 0, 100, 100};
    test->push_back(new RectCmd(1, kInvalidZ, r.makeOffset(8, 8),   FakePaint(SK_ColorRED),   state));
    test->push_back(new RectCmd(2, kInvalidZ, r.makeOffset(48, 48), FakePaint(SK_ColorGREEN), state));
...
static int test7(std::vector<const Cmd*>* test, std::vector<int>* expectedOrder) {
    ...
    test->push_back(new RectCmd(4, kInvalidZ, r.makeOffset(98, 98),   FakePaint(SK_ColorBLUE),    state));
    test->push_back(new RectCmd(5, kInvalidZ, r.makeOffset(148, 148), FakePaint(SK_ColorCYAN),    state));

    s.pop();
    state = s.snapState();

    test->push_back(new RectCmd(6, kInvalidZ, r.makeOffset(148, 8),   FakePaint(SK_ColorMAGENTA), state));
    test->push_back(new RectCmd(7, kInvalidZ, r.makeOffset(8, 148),   FakePaint(SK_ColorYELLOW),  state));
