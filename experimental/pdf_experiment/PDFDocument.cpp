...
static IRef emit_pages(PDFDocument* doc, const std::vector<std::pair<IRef, SkISize>>& pages) {
...
void PDFDocument::onClose(SkWStream*) {
    IRef rootRef = make_root(this, emit_pages(this, fPages));
...
IRef EmitStream(PDFDocument* doc, std::unique_ptr<SkStreamAsset> src, const PDFDict* dict) {
    return EmitStream(doc, std::move(src), doc->reserve(), dict);
}
