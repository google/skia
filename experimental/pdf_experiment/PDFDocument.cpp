...
void PDFDocument::onClose(SkWStream*) {
    IRef rootRef = make_root(this, this->emitPages());
...
IRef PDFDocument::emitPages() {
    IRef reservedRef = this->reserve();
    SkSTArenaAlloc<512> arena;
    PDFList kids(&arena, fPages.size());
    for (PageRecord pageRecord : fPages) {
        PDFDictST<3> page;
        PDFIndirectReference parent(reservedRef), content(pageRecord.fContent);
        page.add("Parent", &parent);
        page.add("Contents", &content);
        PDFInt zero(0), w(pageRecord.fSize.width()), h(pageRecord.fSize.width());
        ...
        kids.add(arena.make<PDFIndirectReference>(EmitObject(this, page)));
    }
    PDFInt pageCountObject((int)fPages.size());
    PDFDictST<2>().add("Count", &pageCountObject)
                  .add("Kids", &kids)
                  .emit(this->beginObj(reservedRef));
    this->endObj();
    return reservedRef;
}
