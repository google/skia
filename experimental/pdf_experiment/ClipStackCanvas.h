...
struct ClipStackCanvas final : public SkCanvas {
    ...
    void onDrawText(const void* text, size_t byteLength, SkScalar x,
        ...
        for (const SkPaint& p : SkAutoDrawLooper(this, &paint)) {
            ...
            grb.drawText(p, text, byteLength, SkPoint{x, y});
            this->drawGlyphs(grb.useGlyphRunList(), p);
        }
    }
    void onDrawPosText(const void* text, size_t byteLength,
        ...
        for (const SkPaint& p : SkAutoDrawLooper(this, &paint)) {
            SkGlyphRunBuilder grb;
            grb.drawPosText(p, text, byteLength, pos);
            this->drawGlyphs(grb.useGlyphRunList(), p);
        }
    }
    void onDrawPosTextH(const void* text, size_t byteLength,
        ...
        for (const SkPaint& p : SkAutoDrawLooper(this, &paint)) {
            SkGlyphRunBuilder grb;
            grb.drawPosTextH(p, text, byteLength, xpos, constY);
            this->drawGlyphs(grb.useGlyphRunList(), p);
        }
    }
    void onDrawTextRSXform(const void* text, size_t byteLength, const SkRSXform xform[],
        ...
        for (const SkPaint& p : SkAutoDrawLooper(this, &paint)) {
            SkGlyphRunBuilder grb;
            grb.drawTextAtOrigin(p, text, byteLength);
    ...
    void onDrawTextBlob(const SkTextBlob* blob, SkScalar x, SkScalar y,
        ...
        for (const SkPaint& p : SkAutoDrawLooper(this, &paint)) {
            SkGlyphRunBuilder grb;
            grb.drawTextBlob(p, *blob, SkPoint{x, y});
            this->drawGlyphs(grb.useGlyphRunList(), p);
        }
    }
    ...
    void drawGlyphs(const SkGlyphRunList& glyphRuns, const SkPaint& p) {
        fPDFPage.drawGlyphs(fClipStack, this->getTotalMatrix(), glyphRuns, p);
    }
