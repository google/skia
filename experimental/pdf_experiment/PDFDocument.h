...
private:
    ...
    std::vector<std::pair<IRef, SkISize>> fPages;
    IRef emitPages();
};

IRef EmitObject(PDFDocument*, const PDFObject&, IRef);

IRef EmitObject(PDFDocument*, const PDFObject&);

IRef EmitStream(PDFDocument*, std::unique_ptr<SkStreamAsset>, IRef, const PDFDict* dict = nullptr);
