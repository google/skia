...
function diffPaths(expected, actual) {
    // First, check to see if the SVG strings are exactly equal:
    if (expected.toSVGString() === actual.toSVGString()) {
        return null;
    }

    // Then look through commands and see if they are within tolerance.
...
describe('PathKit\'s PathOps Behavior', function() {
    ...
    xit('matches the behavior we see from PathKit in C++ (-m PathOpsOp$)', function(done){
        LoadPathKit.then(() => {
                        ...
                        if (test.expectSuccess !== 'yes') {
                            ...
                            if (test.expectMatch === 'yes'){
                                if (diff) {
                                    ...
                                    //expect(test.out).toBe(combinedStr, testName);
                                    addSVG(testName, expected, combined, diff);
                                } else {
                                    success++;
                                }
                            } else {
                                if (!diff) {
                                    ...
                                    //expect(test.out).not.toBe(combinedStr, testName);
                                } else {
                                    success++;
                                }
                            }
