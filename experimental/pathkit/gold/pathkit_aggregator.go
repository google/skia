...
const JSON_FILENAME = "dm.json"    // to fit in with other Upload tasks
const LOG_FILENAME = "verbose.log" // to fit in with other Upload tasks
...
func reporter(w http.ResponseWriter, r *http.Request) {
	...
	if outputType == "png" {
		...
		// disable svgs for now.  Will need to find a way to make these PNGs, since
		// gold doesn't quite handle them
		fmt.Printf("Skipping %s %s since only PNG are ingestible by Gold\n", testOutput.Extension, testOutput.TestName)
		return
	}
