...
const UNINTERESTING_HASHES_URL = "https://storage.googleapis.com/skia-infra-gm/hash_files/gold-prod-hashes.txt"
...
func writeFile(data, extension string) (string, error) {
	if extension == "png" { // created on a canvas, this is a base64 encoded string
		...
		outputFile, err := os.Create(p)
		if err != nil {
			return "", fmt.Errorf("Could not create file %s: %s", p, err)
		}
