...
type ImageInfo struct {
	...
	// shadow value for the C code.
	skVal C.sk_imageinfo_t
}

func (i *ImageInfo) copy() {
	i.skVal.width = C.int32_t(i.Width)
	i.skVal.height = C.int32_t(i.Height)
	i.skVal.colorType = C.sk_colortype_t(i.ColorType)
	i.skVal.alphaType = C.sk_alphatype_t(i.AlphaType)
}
...
type SurfaceProps struct {
	...
	skVal C.sk_surfaceprops_t
}

func (s *SurfaceProps) copy() {
	s.skVal.pixelGeometry = C.sk_pixelgeometry_t(s.PixelGeometry)
}
...
func NewRasterSurface(imgInfo *ImageInfo, surfaceProps *SurfaceProps) *Surface {
	imgInfo.copy()
	surfaceProps.copy()
	ret := &Surface{
		skSurfaceRef: C.sk_surface_new_raster(&imgInfo.skVal, &surfaceProps.skVal),
	}
