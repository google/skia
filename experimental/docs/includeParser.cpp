...
void InterfaceParser::enumHeaderOut(const RootDefinition* root,
    ...
    if (nullptr != nameDef.fContentEnd) {
        ...
        string fullName = root->fName + "::" + enumName;
...
void InterfaceParser::enumSizeItems(const Definition& child) {
    ...
    for (auto& enumItem : fEnumDef->fChildren) {
        ...
        ptrdiff_t lineLen = enumLine.lineLength();
        enumLine.skipTo(enumLine.fChar + lineLen);
        if (!enumLine.eof()) {
            enumLine.skipWhiteSpace();
        }
        enumItem->fShort = enumLine.eof() && longest + lineLen < 100;
        SkDebugf("");
    }
}
...
void InterfaceParser::methodOut(const Definition* method) {
    ...
    if (hasParmReturn) {
        this->lf(2);
        this->lfcr();
        column += fIndent + sizeof("@return ") - 1;
...
bool InterfaceParser::populate(Definition* def, RootDefinition* root) {
    ...
    for (auto& child : def->fTokens) {
        ...
        if (MarkType::kMethod == child.fMarkType) {
            const char* bodyEnd = fDeferComment ? fDeferComment->fContentStart - 1 :
                    child.fContentStart;
            if ("getHash" == child.fName || fLineCount == 145) {
                SkDebugf("");
            }
            // FIXME: roll end-trimming into writeBlockTrim call
            while (fStart < bodyEnd && ' ' >= bodyEnd[-1]) {
                ...
                for (auto& token : child.fTokens) {
                    if (State::kInit == state) {
                        ...
                        for (auto& enumItem : fEnumDef->fChildren) {
                            ...
                            const char* commentStart = enumItem->fContentStart;
                            const char* commentEnd;
                            if (enumItem->fChildren.size() > 0) {
                                commentEnd = enumItem->fChildren[0]->fStart;
                            } else {
                                commentEnd = enumItem->fContentEnd;
                            }
                            TextParser enumComment(fFileName, commentStart, commentEnd,
                                    enumItem->fLineCount);
                            SkAssertResult(enumComment.skipToLineStart()); // skip const value
                            int commentLen = (int) (commentEnd - enumComment.fChar);
                            SkASSERT(commentLen > 0 && commentLen < 1000);
                            if (!enumItem->fShort) {
                                this->writeString("/** ");
                                fIndent += 4;
                                this->rewriteBlock(commentLen, enumComment.fChar);
                                this->lfcr();
                                fIndent -= 4;
                                this->indentToEdge();
                                this->writeString("*/");
                            ...
                            const char* bodyEnd = token.fContentEnd;
                            while (fStart < bodyEnd && ' ' >= fStart[0]) {
                                ++fStart;
                            }
                            this->writeBlock((int) (bodyEnd - fStart), fStart);
                            start here;
                            // need to write enum value on next token iteration
                            this->indentToColumn(fEnumItemValueTab);
                            this->writeBlock(); // write const value if any
                            this->writeString(",");
                            if (!enumItem->fShort) {
...
void InterfaceParser::rewriteBlock(int size, const char* data) {
    ...
    while (run < size) {
        ...
        // will also need to not lower case sentence starts
        // what to do about param fragments? start lower? punctuated?
        if (data[run] == '_') {
