...
void InterfaceParser::methodOut(const Definition* method) {
    ...
    // start  here  compute indention column
    size_t column = fIndent;
    ...
    for (auto methodPart : method->fChildren) {
        ...
        if (MarkType::kParam == methodPart->fMarkType) {
            ...
            int paramLen = (int) (partEnd - partStart);
            SkASSERT(paramLen > 0 && paramLen < 200);
            this->rewriteBlock(paramLen, partStart);
            this->lfAlways(1);
...
//            partEnd -= 2;
            ...
            int returnLen = (int) (partEnd - partStart);
            SkASSERT(returnLen > 0 && returnLen < 200);
            this->rewriteBlock(returnLen, partStart);
            this->lfAlways(1);
        }
    }
    fIndent -= 3;
...
bool InterfaceParser::populate(Definition* def, RootDefinition* root) {
    ...
    for (auto& child : def->fTokens) {
        ...
        if (MarkType::kMethod == child.fMarkType) {
            ...
            methodName = root->fName + "::";
            if (def->fParent->fName != root->fName) {
                methodName += def->fParent->fName + "::";
            }
                        ...
                        if (child.fName == root->fName) {
                            if (Definition* parent = root->fParent) {
                                if (MarkType::kTopic == parent->fMarkType ||
                                    ...
                                    this->writeString("/** \\");
                                    this->writeString(KeyWord::kClass == child.fKeyWord ?
                                        "class " : "struct ");
                                    this->writeString(root->fName.c_str());
                                    this->lfAlways(1);
                                    this->rewriteBlock((int) (commentEnd - commentStart),
                                        commentStart);
                                    this->lfAlways(1);
                                    this->writeString("*/");
                                    this->lfAlways(1);
