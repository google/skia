...
//start here;
// need to populate parent class in fIClassMap with child structs
// (currently also in fIClassMap) and then change cross check
// to iterate for each child class/struct in the root class
    ...
    for (const auto& token : tokens) {
        ...
        switch (token.fMarkType) {
            ...
            case MarkType::kComment:
                break;
            case MarkType::kEnum: {
                if (!def && 0 == token.fName.find("_anonymous")) {
                    def = root->find(className + token.fName);
                }
                if (!def) {
