...
bool InterfaceParser::populate(Definition* def, RootDefinition* root) {
    ...
    for (auto& child : def->fTokens) {
        ...
        } else if (MarkType::kMethod == child.fMarkType) {
            const char* bodyEnd = fDeferComment ? fDeferComment->fContentStart - 1 :
                    child.fContentStart;
            fprintf(fOut, "%.*s", (int) (bodyEnd - fStart), fStart);
            fStart = child.fContentStart;
            string methodName = root->fName + "::" + child.fName;
            if (const Definition* method = root->find(methodName)) {
                // start here
                // walk children and output complete method doxygen description
                const char* commentStart = method->fContentStart;
                // FIXME: hack around leading linefeeds on comment start
                while ('\n' == commentStart[0]) {
                    commentStart++;
                }
                ...
                fprintf(fOut, "/**\n");
                fprintf(fOut, "%.*s", (int) (commentEnd - commentStart),
                    commentStart);
                for (auto methodPart : method->fChildren) {
                    ...
                    if (MarkType::kParam == methodPart->fMarkType) {
                        fprintf(fOut, "@param %s  ", methodPart->fName.c_str());
                        fprintf(fOut, "%.*s\n", (int) (partEnd - partStart), partStart);
                    } else if (MarkType::kReturn == methodPart->fMarkType) {
                        fprintf(fOut, "@return  ");
                        // FIXME: hack around incorrect content end
                        partEnd -= 2;
                        while ('\n' == partEnd[-1]) {
                            --partEnd;
                        }
                ...
                return this->reportError<bool>("can't find method");
            }
        } else if (Definition::Type::kKeyWord == child.fType) {
            switch (child.fKeyWord) {
                case KeyWord::kClass:
                case KeyWord::kStruct:
                        ...
                        if (child.fName == root->fName) {
                            if (Definition* parent = root->fParent) {
                                    if (MarkType::kTopic == parent->fMarkType ||
                                        ...
                                        fprintf(fOut, "/** \\");
                                        fprintf(fOut, KeyWord::kClass == child.fKeyWord ?
                                            "class " : "struct ");
                                        fprintf(fOut, "%s\n", root->fName.c_str());
                                        fprintf(fOut, "%.*s", (int) (commentEnd - commentStart),
                                            commentStart);
