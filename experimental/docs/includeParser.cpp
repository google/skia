...
bool Parser::parseEnum(Definition* child, Definition* parent) {
    ...
    if (child->fTokens.size() > 0) {
        ...
        if (Definition::Type::kWord == firstToken.fType) {
            nameStr += string(firstToken.fStart, firstToken.fEnd - firstToken.fStart);
        } else {
            nameStr += "anon";
        }
    }
...
printf(""                                                                                 "\n");
printf("##Method %.*s #%s"                                                                "\n",
                      (int) (token.fEnd - token.fStart), token.fStart, token.fName.c_str());
printf(""                                                                                 "\n");
