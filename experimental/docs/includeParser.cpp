...
bool Parser::parseClass(Definition* includeDef) {
    ...
    auto commentIter = parent->fTokens.begin();
    std::advance(commentIter, index);
    SkASSERT(&*commentIter == includeDef);
    ...
    while (&*commentIter != includeDef) {
        if (!this->parseComment(*commentIter, asMarkupDef)) {
            return false;
        }
    }
...
bool Parser::parseComment(const Definition& includeDef, Definition* markupDef) {
    ...
    markupDef->fTokens.emplace_back(MarkType::kComment, fChar, markupDef);
