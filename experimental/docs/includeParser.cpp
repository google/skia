...
void InterfaceParser::methodOut(const Definition* method) {
    ...
    start here;
    // need to rethink linefeeds
    // the end of a thing (e.g. enum) needs to say put a line feed after me
    // if the next thing doesn't normally require a linefeed (e.g. a struct member)
    // currently we either double up on linefeeds or have none at all
    this->lf(2);
    this->lfcrIndent();
    this->writeString("/** ");
    fIndent += 4;
...
void InterfaceParser::structSizeMembers(Definition& child) {
    int longestType = 0;
...
string InterfaceParser::resolveRef(const char* start, const char* end, bool first) {
        // look up Xxx_Xxx 
...
bool InterfaceParser::rewriteBlock(int size, const char* data) {
    ...
    Punctuation punctuation = Punctuation::kStart;
    int start = 0;
    int lastWrite = 0;
    int lineFeeds = 0;
    int lastPrintable = 0;
    ...
    while (run < size) {
        ...
        if (lineFeeds && ' ' < c) {
            ...
            this->lfcrIndent(); // defer the indent until non-whitespace is seen
            lastWrite = run;
            lineFeeds = 0;
        }
        if (' ' < c) {
        ...
        switch (c) {
            ...
            case ' ':
                switch (word) {
                    ...
                    case Word::kUnderline: {
                        const int end = Punctuation::kDelimiter == punctuation ||
                                Punctuation::kPeriod == punctuation ? run - 1 : run;
                        string temp = this->resolveRef(&data[start], &data[end],
                                Word::kFirst == word);
                        if (!temp.length()) {
                        ...
                        if (temp.length()) {
                            if (start > lastWrite) {
                                ...
                                this->writeBlock(start - lastWrite, &data[lastWrite]);
                            }
                ...
                punctuation = Punctuation::kStart;
                word = Word::kStart;
                break;
            case '.':
                switch (word) {
                    case Word::kStart:
                        punctuation = Punctuation::kDelimiter;
                    case Word::kCap:
                    case Word::kFirst:
                    case Word::kUnderline:
                    case Word::kMixed:
                        if (Punctuation::kDelimiter == punctuation ||
                                Punctuation::kPeriod == punctuation) {
                            word = Word::kMixed;
                        }
                        punctuation = Punctuation::kPeriod;
                        break;
                    default:
                        SkASSERT(0);
                }
            ...
            case 'Z':
                switch (word) {
                    case Word::kStart:
                        word = Punctuation::kStart == punctuation ? Word::kFirst : Word::kCap;
                        start = run;
                        break;
                    case Word::kCap:
                    case Word::kFirst:
                        ...
                        break;
                    case Word::kUnderline:
                        word = '_' == last ? Word::kUnderline : Word::kMixed;
                        break;
                    case Word::kMixed:
                        break;
                    default:
                ...
                if (Punctuation::kPeriod == punctuation || Punctuation::kDelimiter == punctuation) {
                    word = Word::kMixed;
                } else {
                    punctuation = Punctuation::kStart;
                }
