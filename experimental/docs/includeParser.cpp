...
bool Parser::parseClass(Definition* includeDef) {
    ...
    ClassDefinition* markupDef = this->findClass(*includeDef, nameStr);
    if (!markupDef) {
        return false;
    }
    ...
    while (iter != includeDef->fTokens.end()
            && publicLen == iter->fContentEnd - iter->fStart
    ...
    while (iter != includeDef->fTokens.end()
            && (protectedLen != iter->fContentEnd - iter->fStart
            || strncmp(iter->fStart, protectedName, protectedLen))
            && (privateLen != iter->fContentEnd - iter->fStart
...
bool Parser::parseEnum(Definition* child, Definition* parent) {
    ...
    Definition* markupDef = this->findIncludeObject(*child, MarkType::kEnum, nameStr);
    if (!markupDef) {
        return false;
    }
...
bool Parser::parseInclude(const string& name) {
    ...
    string className = name.substr(name.find_last_of("/"));
    SkASSERT(className[0] == '/');
    className = className.substr(1, className.size() - 3);
    const ClassDefinition& classMap = fIClassMap[className];
    const list<Definition>& tokens = classMap.fTokens;
    for (const auto token : tokens) {

printf("%s", "--------------------------------------------------------------------------------" "\n");
printf(""                                                                                 "\n");
printf("##Method %.*s #%s"                                                                "\n",
                      (int) (token.fContentEnd - token.fStart), token.fStart, token.fName.c_str());
printf(""                                                                                 "\n");
...
bool Parser::parseMethod(Definition* child, Definition* markupDef) {
    ...
    markupDef->fTokens.emplace_back(MarkType::kMethod, tokenIter->fStart, end, markupDef);
    Definition* markupChild = &markupDef->fTokens.back();
    ClassDefinition& classDef = fIClassMap[markupDef->fName];
...
bool Parser::parseObjects(Definition* parent) {
	for (auto& child : parent->fChildren) {
        if (!this->parseObject(child, nullptr)) {
            return false;
        }
    }
    return true;
...
bool Parser::parseObject(Definition* child, Definition* markupDef) {
    bool success = true;
