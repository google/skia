...
bool InterfaceParser::parseClass(Definition* includeDef) {
    ...
    if (string::npos != nameStr.find('{')) {
        return INHERITED::reportError<bool>("malformed class definition, unexpected open brace");
    } 
    iter = std::next(iter);
    this->skipWhiteSpace();
    if ('{' != iter->fStart[0]) {
        return INHERITED::reportError<bool>("malformed class definition, missing open brace");
    }
...
bool InterfaceParser::parseEnum(Definition* child, Definition* parent) {
...
bool InterfaceParser::parseInclude(const string& name) {
    ...
    const IClassDefinition& classMap = fIClassMap[className];
...
bool InterfaceParser::parseObjects(Definition* parent) {
