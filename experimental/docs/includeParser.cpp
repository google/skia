...
printf(""                                                                                 "\n");
        ...
        for (const auto& child : token.fTokens) {
            if (Definition::Type::kMark == child.fType && MarkType::kComment == child.fMarkType) {
                const char* s = child.fContentStart;
                int len = (int) (child.fContentEnd - s);
                printf("%.*s\n", len, s);
                foundComment = true;
            }
        }
...
bool InterfaceParser::parseComment(const Definition& includeDef, Definition* markupDef) {
    TextParser parser(includeDef.fContentStart, includeDef.fContentEnd);
    parser.fLineCount = includeDef.fLineCount;
...
bool InterfaceParser::parseEnum(Definition* child, Definition* markupDef) {
    ...
    TextParser parser(markupDef->fContentStart, markupDef->fContentEnd);
    parser.fLineCount = markupDef->fLineCount;
    parser.skipToEndBracket('{');
    const char* dataEnd;
    do {
        parser.next();
        ...
        markupDef->fTokens.emplace_back(MarkType::kMember, dataStart, dataEnd, parser.fLineCount,
                markupDef);
        Definition* member = &markupDef->fTokens.back();
        member->fName = memberName;
        markupDef->fChildren.push_back(member);
