        ...
        switch (token.fMarkType) {
            case MarkType::kEnum:
                ...
                for (auto child : token.fChildren) {
                    ...
                    printf("#Const %s %.*s"                                               "\n",
                            child->fName.c_str(), (int) (valEnd - val.fStart), val.fStart);
                    this->dumpComment(*child);
                    printf("##"                                                           "\n");
                }
                printf(""                                                                 "\n");
            break;
...
bool InterfaceParser::parseEnum(Definition* child, Definition* markupDef) {
    ...
    do {
        ...
        if (parser.startsWith("/*") || parser.startsWith("//")) {
            ...
            if (!this->parseComment(start, end, parser.fLineCount, markupChild)) {
                return false;
            }
