    ...
    for (const auto& token : tokens) {
        ...
        switch (token.fMarkType) {
            case MarkType::kMethod: {
                string fullName = className + "::" + token.fName;
                if (const Definition* def = root->find(fullName)) {
                    const char* defStart = def->fStart;
                    SkASSERT(bmhParser.fMC == defStart[0]);  // FIXME: needs to be per definition
                    ++defStart;
                    const char* methodID = bmhParser.fMaps[(int) MarkType::kMethod].fName;
                    SkASSERT(!strncmp(defStart, methodID, strlen(methodID)));
                    defStart += strlen(methodID);
                    ptrdiff_t bmhLen = def->fContentStart - defStart;
                    ptrdiff_t incLen = token.fContentEnd - token.fContentStart;
                    if (bmhLen != incLen || strncmp(defStart, token.fContentStart, bmhLen)) {
                        fprintf(fIOut, "differs from bmh: %s\n", fullName.c_str());
                        def->fVisited = true;
                    }
                } else {
                    fprintf(fIOut, "missing from bmh: %s\n", fullName.c_str());
                }
            } break;
...
bool InterfaceParser::parseClass(Definition* includeDef, enum class IsStruct isStruct) {
    ...
    switch (child->fType) {
        case Definition::Type::kKeyWord:
            switch (child->fKeyWord) {
                case KeyWord::kClass: 
                    success = this->parseClass(child, IsStruct::kNo);
                    break;
                case KeyWord::kEnum:
                    success = this->parseEnum(child, markupDef);
                    break;
                case KeyWord::kStruct:
                    success = this->parseClass(child, IsStruct::kYes);
                    break;
