...
bool Parser::parseClass(Definition* def) {
    SkASSERT(def->fTokens.size() > 0);
    ...
    fChar = fLine = iter->fStart;  // set up for error reporting
    fEnd = iter->fEnd;
    if (memchr(fChar, '{', fEnd - fChar)) {
        return this->reportError<bool>("malformed class definition, unexpected open brace");
    } 
    std::string nameStr(fChar, fEnd - fChar);
    iter = std::next(iter);
    fChar = fLine = iter->fStart;  // set up for error reporting
    fEnd = iter->fEnd;
    this->skipWhiteSpace();
    ...
    SkASSERT(1 == def->fChildren.size());
    def = def->fChildren.front();
    iter = def->fTokens.begin();
	// skip until public
    int publicIndex = 0;
    while (iter != def->fTokens.end()
            && strncmp(iter->fStart, kKeyWords[(int) KeyWord::kPublic].fName,
            iter->fEnd - iter->fStart)) {
        iter = std::next(iter);
        ++publicIndex;
    }
    auto childIter = def->fChildren.begin();
    while (childIter != def->fChildren.end() && (*childIter)->fParentIndex < publicIndex) {
        childIter = std::next(childIter);
    }
    int lastPublic = publicIndex;
    while (iter != def->fTokens.end()
            && strncmp(iter->fStart, kKeyWords[(int) KeyWord::kProtected].fName,
                       iter->fEnd - iter->fStart)
            && strncmp(iter->fStart, kKeyWords[(int) KeyWord::kPrivate].fName,
                       iter->fEnd - iter->fStart)) {
        iter = std::next(iter);
        ++lastPublic;
    }
    while (childIter != def->fChildren.end() && (*childIter)->fParentIndex < lastPublic) {
        // start here:
