...
bool InterfaceParser::checkForMissingParams(const vector<string>& methodParams, 
        const vector<string>& foundParams) const {
    bool result = false;
        ...
        for (const auto& token : tokens) {
            ...
            switch (token.fMarkType) {
                case MarkType::kMethod: {
                    ...
                    if (!def) {
                        bool allLower = true;
                        for (int index = 0; index < token.fName.length(); ++index) {
...
void InterfaceParser::dumpComment(Definition* token) {
            ...
            if ('@' == child.fContentStart[0]) {
                ...
                do {
                    ...
                    if (parser.startsWith("param ")) {
                        ...
                        foundParams.
                                emplace_back(string(parmStart, (int) (parser.fChar - parmStart)));
                        fprintf(fIOut, "#Param %.*s  ", (int) (parser.fChar - parmStart), parmStart);
                        parser.skipWhiteSpace();
                        fprintf(fIOut, "%.*s", (int) (parser.fEnd - parser.fChar), parser.fChar);
                        parser.skipTo(parser.fEnd);
                        sawParam = true;
                    } else if (parser.startsWith("return ")) {
                        parser.skipWord("return");
                        missingParamCheck = true;
                        foundReturn = true;
                        if (sawParam) {
                            fprintf(fIOut, " ##\n");
                        } else if (prevComment) {
                        ...
                        sawParam |= this->checkForMissingParams(methodParams, foundParams);
                        if (sawParam) {
                            fprintf(fIOut, "\n");
                            sawParam = false;
                        }
                        fprintf(fIOut, "#Return  %.*s ##", (int) (parser.fEnd - parser.fChar),
                                parser.fChar);
                        parser.skipTo(parser.fEnd);
                    } else {
                ...
                if (sawParam || foundComment) {
                    fprintf(fIOut, "\n");
                }
                fprintf(fIOut, "%.*s", child.length(), child.fContentStart);
                if (!sawParam) {
                    prevComment = true;
                }
            }
    ...
    if (!missingParamCheck) {
        if (this->checkForMissingParams(methodParams, foundParams)) {
            fprintf(fIOut, "\n");
        }
    }
    if (methodHasReturn != foundReturn) {
        if (!methodHasReturn) {
            this->reportError("unexpected doxygen return");
        } else {
            fprintf(fIOut, "#Return  ##");
        }
    }
}
...
bool InterfaceParser::parseEnum(Definition* child, Definition* markupDef) {
    ...
    for (int index = 1; index < child->fChildren.size(); ++index) {
...
bool InterfaceParser::parseMethod(Definition* child, Definition* markupDef) {
    ...
    markupDef->fTokens.emplace_back(MarkType::kMethod, tokenIter->fStart, end, tokenIter->fLineCount,
            markupDef);
    Definition* markupChild = &markupDef->fTokens.back();
...
bool InterfaceParser::populate(Definition* def, RootDefinition* root) {
    ...
    for (auto child : def->fChildren) {
        ...
        } else if (KeyWord::kClass == child->fKeyWord || KeyWord::kStruct == child->fKeyWord) {
            if (child->fChildren.size() > 0) {
                if (fDeferComment) {
                    fprintf(fIOut, "%.*s", (int) (fDeferComment->fContentStart - fStart), fStart);
                    fStart = child->fContentStart;
                    // start here: find corresponding object in bmh
                        // (suppress comments on consecutive methods with same name)
                        // write its comments before object
                    fDeferComment = nullptr;
                }
                SkDebugf("");
            }
        } else if (Definition::Type::kBracket == child->fType) {
            fDeferComment = nullptr;
        } else {
                SkDebugf("");
        }
        allPassed &= this->populate(child, root);
    }
    return allPassed;
}
...
bool InterfaceParser::populate(BmhParser& bmhParser) {
    ...
    for (auto& includeMapper : fIncludeMap) {
        ...
        string skClassName = includeMapper.first.substr(lastSlash + 1);
        string fileName = skClassName + ".h";
        fIOut = fopen(fileName.c_str(), "w");
