...
bool InterfaceParser::parseEnum(Definition* child, Definition* markupDef) {
    ...
    do {
        ...
        std::unique_ptr<Definition>commentHolder;
        ...
        if (parser.startsWith("/*") || parser.startsWith("//")) {
            ...
            commentHolder = std::unique_ptr<Definition>(new Definition());
            comment = &*commentHolder;
            comment->fStart = comment->fContentStart = start;
            comment->fTerminator = comment->fContentEnd = end;
            comment->fLineCount = parser.fLineCount;
            comment->fParent = markupChild;
            comment->fMarkType = MarkType::kComment;
            comment->fType = Definition::Type::kMark;
        ...
        if (comment) {
            member->fTokens.emplace_back(std::move(*comment));
