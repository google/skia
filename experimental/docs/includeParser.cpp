    ...
    for (auto& child : def->fTokens) {
                ...
                case KeyWord::kEnum: {
                    ...
                    if (nullptr != nameDef.fContentEnd) {
                       ...
                       // child[0] should be #Code comment starts at child[0].fTerminator
                        ...
                        enumName += "::_anonymous";
                        if (anonymousEnumCount > 1) {
                            enumName += '_' + std::to_string(anonymousEnumCount);
                        }
                        enumDef = root->find(enumName);
                        SkASSERT(enumDef);
                        ++anonymousEnumCount;
        ...
        if (Definition::Type::kBracket == child.fType) {
            if (KeyWord::kEnum == child.fParent->fKeyWord) {
                // start here;
                // iterate through children and find how much remains for 1 line comments
