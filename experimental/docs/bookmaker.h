...
public:
    ...
    void rewriteBlock(int size, const char* data) {
        while (size > 0 && data[0] <= ' ') {
            --size;
            ++data;
        }
        do {
            int run = 0;
            while (run < size && data[run] != '_' && data[run] != '\n') {
                ++run;
            }
            // omit underscores between capitalized words
            if (run + 1 < size && data[run] == '_' && isupper(data[run + 1])) {
                int check = run;
                while (check > 0 && islower(data[--check]))
                    ;
                if (check > 0 && isupper(data[check]) && !islower(data[check - 1])) {
                    check = run + 1;
                    while (++check < size && islower(data[check]))
                        ;
                    char c = data[check];
                    if (c <= ' ' || '.' == c || ',' == c || ';' == c || ':' == c) {
                        // start here: need to look up substitute
                        // can't tell from capitalization alone whether topic has skia equivalent
            ...
            // set indent and linefeed to end of written data
            int index = size - 1;
            while (index > 0 && ' ' == data[index]) {
                --index;
            }
            fSpaces = size - index - 1;
            int lfStart = index;
            while (index > 0 && '\n' == data[index]) {
                --index;
            }
            fLinefeeds = lfStart - index;
            fColumn = -1; // don't rely on column being set
            break;
        } while (size > 0);
    }
    ...
    void writeBlock(int size, const char* data) {
        SkASSERT(size > 0 && size < 8000);
#if 0
        string check(data, size);
        if (string::npos != check.find("getHash")) {
            SkDebugf("");
        }
#endif
...
, { "Link",        nullptr,      MarkType::kLink,        R_Y, E_N, 0 }
...
, { "Private",     nullptr,      MarkType::kPrivate,     R_Y, E_N, 0 }
