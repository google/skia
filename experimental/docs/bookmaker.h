...
struct Definition {
    ...
    void setParentIndex() {
        fParentIndex = fParent ? (int) fParent->fChildren.size() : -1;
    }
...
struct Parser {
    ...
    void popBracket() {
        ...
        SkASSERT(Definition::Type::kBracket == lastBracket->fType);
        SkASSERT(!lastBracket->fEnd);
        lastBracket->fEnd = fChar;
        SkASSERT(fBracketStack.size() > 0);
        fBracketStack.pop();
        Bracket bracket = fBracketStack.size() > 0 ? fBracketStack.top()->fBracket : Bracket::kNone;
    ...
    Bracket topBracket() const {
        SkASSERT(!fBracketStack.size()
                || Definition::Type::kBracket == fBracketStack.top()->fType);
        return fBracketStack.size() ? fBracketStack.top()->fBracket : Bracket::kNone;
    }
