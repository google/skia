...
struct ClassDefinition : public Definition {
    map<string, Definition*> fMethods;
};
...
struct Parser {
    ...
    ClassDefinition* findClass(const Definition& includeDef, const string& className) {
        map<string, ClassDefinition>& map = fIClassMap;
    ...
    template <typename T>
    string uniqueName(const map<string, T>& m, const string& typeName) {
        string name(typeName.size() > 0 ? typeName : "anon");
        int anonCount = 1;
        do {
            map<string, T>::const_iterator iter = m.find(name);
            if (iter == m.end()) {
                return name;
            }
            name = typeName + '_';
    ...
    map<string, ClassDefinition> fIClassMap;
