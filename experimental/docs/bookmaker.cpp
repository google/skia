...
string Parser::addReferences(const string& text) {
    string result;
    TextParser t(&text.front(), &text.back());
    do {
        const char* base = t.fChar;
        t.skipToAlpha();
        const char* start = t.fChar;
        t.skipToNonAlphaNum();
        const char* end = t.fChar;
        if (base == end) {
            break;
        }
        if (start < end) {
            ...
            if (iter != fRoot->fChildDefinitions.end()) {
                ...
                // see if this should have been a findable reference
                 ...
                 // look for Sk / sk / SK ..
                if (!ref.compare(0, 2, "Sk") || !ref.compare(0, 2, "SK")
                        || !ref.compare(0, 3, "sk_")) {
                    // try with a prefix
                    string prefixed = fRoot->fName + ref;
                    iter = fRoot->fChildDefinitions.find(prefixed);
                    if (iter == fRoot->fChildDefinitions.end()) {
                        SkDebugf("%s\n", ref.c_str());
                    }
                } else if (isupper(start[0]) && start != "Set" && start != "If") {
                    while (t.startsWith(" ")) {
                        SkAssertResult(t.skipSpace());
                        if (!t.eof() && isupper(t.fChar[0])) {
                            t.skipToNonAlphaNum();
                        }
                    }
                    string prefixed = fRoot->fName + ref;
               ...
               } else if (t.startsWith("k") || t.startsWith("g") || t.startsWith("f")) {
                    t.next();
                    if (!t.eof() && isupper(t.fChar[0])) {
                        t.skipToNonAlphaNum();
                        SkDebugf("%.*s\n", t.fChar - start, start);
			...
			if (this->peek() == fMC) {  // definition
				...
				switch (markType) {
                    ...
                    case MarkType::kStruct: {
                        ...
                        if (nullptr == fRoot) {
                            if (nullptr != fParent) {
					            return this->reportError<bool>("expected no parent");
                            }
						    fRoot = this->findBmhObject(markType, typeNameBuilder[0]);
                            definition = fRoot;
                        } else {
                            if (nullptr == fParent) {
					            return this->reportError<bool>("expected parent");
                            }
                            if (fParent == fRoot && this->hasEndToken()) {
                                definition = fParent;
                            } else {
                                auto dup = fRoot->fChildDefinitions.find(name);
                                if (!this->hasEndToken() && fRoot->fChildDefinitions.end() != dup) {
