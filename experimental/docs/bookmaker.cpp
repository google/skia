...
const Definition* BmhParser::isDefined(const string& ref) const {
...
string BmhParser::addReferences(const char* refStart, const char* refEnd) {
    ...
    do {
        ...
        if (start < t.fChar) {
            ...
            const Definition* def = this->isDefined(ref);
            if (def) {
                add_link(ref, &result);
                continue;
            }
    ...
    // methods may start with upper (static) or lower (most)
            ...
            if (isupper(start[0])) {
                ...
                if (isupper(t.fChar[0]) && startsSentence) {
                    ...
                    const Definition* def = this->isDefined(nextWord);
                    if (def) {
                        add_ref(ref, &result);
                        continue;
                    }
	...
	while (!this->eof()) {
		if (this->peek() == fMC) {
			...
			if (this->peek() == fMC) {  // definition
                ...
                vector<string> typeNameBuilder = this->typeName(markType, &isConstructor);
...
string BmhParser::optionalName(MarkType markType) {
    string builder;
    const char* end = this->lineEnd();
    const char* mc = this->strnchr(fMC, end);
    if (mc) {
        ...
        const Definition* parent = this->parentClass();
        if (parent) {
            builder += parent->fName;
        }
        builder = this->word(builder);
...
string BmhParser::memberName() {
    ...
    // fixme: change markup.bmh to require # if duplicate name before parent
    // or first paren is not beginning of function parameter list
string BmhParser::methodName(bool* isConstructor) {
    if (this->hasEndToken()) {
...
const Definition* BmhParser::parentClass() const {
    const Definition* parent = fParent;
...
vector<string> BmhParser::typeName(MarkType markType, bool* isConstructor) {
    vector<string> result;
    string builder;
    if (fParent) {
    ...
    switch (markType) {
        ...
        case MarkType::kExample: // todo: break out example to check if there is more than one
            ...
            builder = this->uniqueName(markType);
            break;
        case MarkType::kDeprecated:
        case MarkType::kDescription:
        ...
        case MarkType::kMember:
            ...
            break;
        case MarkType::kMethod:
            builder = this->methodName(isConstructor);
            break;
        case MarkType::kParam:
...
string BmhParser::uniqueName(MarkType markType) {
    string builder(this->optionalName(markType));
    if (!builder.length()) {
        builder = fParent->fName;
    }
    int number = 2;
