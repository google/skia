...
// methods may start with upper (static) or lower (most)
        ...
        auto topicIter = fTopicMap.find(ref);
        if (topicIter != fTopicMap.end()) {
            result += linkRef(ref, topicIter->second);
            continue;
        }
...
string BmhParser::linkRef(const string& ref, const Definition* def) const {
    ...
    if (string::npos != under) {
        string classPart = str->substr(0, under);
        string namePart = str->substr(under + 1, str->length());
        SkASSERT(fRoot);
        ...
        if (classMatch) {
            ...
            Definition* defRoot = def->fParent;
            classMatch = defRoot->fName == classPart;
            while (!defRoot->fParent) {
                defRoot = defRoot->fParent;
                classMatch = defRoot->fName == classPart;
            }
            if (curRoot != defRoot) {
                buildup = fRoot->asRoot()->fFileName;
                buildup += "?cl=9919#";
                buildup += namePart;
            }
            str = &buildup;
