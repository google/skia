...
void BmhParser::addLink(const string& leadingSpaces, const string& ref, const string& def,
        string* result) {
    SkASSERT(ref.length() > 0);
    SkASSERT(def.length() > 0);
    const string* defPtr = &def;
    string buildup;
    if (ref != def && def.substr(0, 2) == "Sk") {
        size_t under = def.find('_');
        string classPart = string::npos != under ? def.substr(0, under) : def;
        string namePart = string::npos != under ? def.substr(under + 1, def.length()) : def;
        const string& rootName = fRoot->fName;
        if (!def.compare(0, rootName.size(), rootName)) {
            buildup = namePart;
        } else {
            buildup = "bmh_";
            buildup += classPart;
    ...
    *result += leadingSpaces + "<a href=\"" + *defPtr + "\">" + ref + "</a>";
}
...
string BmhParser::addReferences(const char* refStart, const char* refEnd) {
    ...
    do {
        ...
        if (this->isDefined(t, ref, &def)) {
            addLink(leadingSpaces, ref, def, &result);
            continue;
        }
...
// methods may start with upper (static) or lower (most)
        ...
        if (!ref.compare(0, 2, "Sk") && ref != "Skew") {
            SkDebugf("%s\n", ref.c_str());
            addLink(leadingSpaces, ref, ref, &result);
            continue;
        } 
        ...
        auto topicIter = fTopicMap.find(ref);
        if (topicIter != fTopicMap.end()) {
            addLink(leadingSpaces, ref, topicIter->second->fFiddle, &result);
            continue;
        }
        ...
        do {
            ...
            if ((def = root->find(prefixed))) {
                addLink(leadingSpaces, ref, def->fFiddle, &result);
                break;
            }
        } while ((root = root->rootParent()));
        if (!root) {
