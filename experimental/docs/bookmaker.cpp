            ...
            if (string::npos == comma) {
                ...
                while (!params.eof()) {
                    ...
                    if (params.startsWith("const") || params.startsWith("int")
                        ...
                        result += '_' + string(wordStart, params.fChar - wordStart);
                    } else {
                        params.skipToNonAlphaNum();
                    }
                    if ('*' == params.peek()) {
...
static void add_link(const string& leadingSpaces, const string& ref, const string& def,
        string* result) {
    *result += leadingSpaces + "[" + ref + "](#" + def + ")";
}
...
string BmhParser::addReferences(const char* refStart, const char* refEnd) {
    ...
    do {
        ...
        if (this->isDefined(ref, &def)) {
            add_link(leadingSpaces, ref, def, &result);
            continue;
        }
...
// methods may start with upper (static) or lower (most)
        ...
        if (!ref.compare(0, 2, "Sk")) {
            SkDebugf("%s\n", ref.c_str());
            add_link(leadingSpaces, ref, ref, &result);
            continue;
        } 
        if (!ref.compare(0, 2, "SK")) {
            SkDebugf("%s\n", ref.c_str());
        ...
        auto topicIter = fTopicMap.find(ref);
        if (topicIter != fTopicMap.end()) {
            add_link(leadingSpaces, ref, topicIter->second->fFiddle, &result);
            continue;
        }
        ...
        do {
            ...
            if ((def = root->find(prefixed))) {
                add_link(leadingSpaces, ref, def->fFiddle, &result);
                break;
            }
...
// -b "C:/skia/experimental/docs/paint.bmh"  -r -f "C:/skia/experimental/docs/example.htm"
