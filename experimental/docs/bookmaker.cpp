    ...
    for (auto const& iter : fChildren) {
        switch (iter->fMarkType) {
            ...
            case MarkType::kImage:
                imageStr = string(iter->fContentStart, iter->fContentEnd - iter->fContentStart - 1);
                break;
            case MarkType::kToDo:
                break;
...
bool BmhParser::isDefined(const TextParser& parser, const string& ref, string* def) const {
    ...
    if (fRoot) {
        ...
        do {
            ...
            if ((definition = root->find(prefixed))) {
                *def = definition->fFiddle;
                return true;
            }
...
static void add_link(const string& leadingSpaces, const string& ref, const string& def,
        string* result) {
    *result += leadingSpaces + "<a href=\"" + def + "\">" + ref + "</a>";
}
        ...
        case MarkType::kTrack:
            if (hasEnd) {
                ...
                definition->fFiddle = string::npos != typeNameBuilder[0].find('(', 0) 
                        ? fParent->fFiddle : normalized_name(typeNameBuilder[0]);
                    this->setAsParent(definition);
            }
            break;
...
// methods may start with upper (static) or lower (most)
        ...
        // see if this should have been a findable reference
            ...
            // look for Sk / sk / SK ..
        if (!ref.compare(0, 2, "Sk") && ref != "Skew") {
            SkDebugf("%s\n", ref.c_str());
        ...
        if (isupper(t.fChar[1]) && startsSentence) {
            ...
            string dummy;
            if (this->isDefined(t, nextWord, &dummy)) {
                add_ref(leadingSpaces, ref, &result);
                continue;
            }
        }
...
void BmhParser::markTypeOut(Definition* def) {
    ...
    switch (def->fMarkType) {
        ...
        case MarkType::kMethod: {
            ...
            string methodStr(funcParser.fChar, (size_t) (funcParser.trimmedLineEnd() - funcParser.fChar));
        //    start here;
            // format method to fit in some # of chars
                // rewrap method so that it fits within arbitrary # of chars,
                // break as we do in skia headers, pad with spaces to line things up
