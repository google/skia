...
bool Definition::checkMethod() const {
    ...
    methodParser.fLineCount = fLineCount;
    ...
    for (auto& child : fChildren) {
        ...
        if (!expectReturn) {
            methodParser.reportError("no #Return expected");
            return false;
        }
        if (foundReturn) {
            methodParser.reportError("multiple #Return markers");
            return false;
        }
        foundReturn = true;
    }
    if (expectReturn && !foundReturn) {
        methodParser.reportError("missing #Return marker");
        return false;
    }
    const char* paren = methodParser.strnchr('(', methodParser.fEnd);
    if (!paren) {
        methodParser.reportError("missing #Method function defintion");
        return false;
    }
    const char* nextStart = paren + 1;
    const char* nextEnd;
    do {
        nextEnd = methodParser.anyOf(",)");
        if (!nextEnd) {
            methodParser.reportError("#Method function missing close paren");
            return false;
        }
        ...
        while (paramEnd > nextStart && ' ' == paramEnd[-1]) {
            --paramEnd;
        }
        const char* paramStart = paramEnd;
        while (paramStart > nextStart && isalnum(paramStart[-1])) {
            --paramStart;
        }
        if (paramStart >= paramEnd) {
            methodParser.reportError("#Method missing param name");
            return false;
        }
        ...
        for (auto& child : fChildren) {
            ...
            if (child->fVisited) {
                methodParser.reportError("multiple #Method param with same name");
                return false;
            }
            child->fVisited = true;
            if (foundParam) {
                methodParser.fLineCount = child->fLineCount;
                methodParser.reportError("multiple #Param with same name");
                return false;
            }
            foundParam = true;
        }
        if (!foundParam) {
            methodParser.reportError("no #Param found");
            return false;
        }
        if (')' == nextEnd[0]) {
            break;
    ...
    for (auto& child : fChildren) {
        ...
        if (child->fVisited) {
            methodParser.fLineCount = child->fLineCount;
            methodParser.reportError("#Param without param in #Method");
            return false;
        }
    }
    return true;
