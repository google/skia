				...
				switch (markType) {
                   case MarkType::kAnchor: 
                    ...
                    case MarkType::kTopic: // may define multiple keys
				        ...
				        if (this->hasEndToken()) {
					        if (!this->popParentStack(definition ? definition : fParent)) {
                                return false;
                            }
                        }
                        break;
...
string Parser::methodName(string prefix) {
...
bool Parser::reportError(const Definition& def, const char* errorStr) const {
    ReportError(errorStr, -1 /* todo: add line # to def */, def.fEnd - def.fStart,
...
vector<string> Parser::typeName(MarkType markType) {
    ...
    switch (markType) {
        ...
        case MarkType::kStdOut:
            return result;  // unnamed
        case MarkType::kMethod:
            builder = this->methodName(builder);
           break;
        case MarkType::kParam:
    ...
    for (auto topic : parser.fTopics) {
        for (auto child : topic.fChildren ) {
            if (MarkType::kExample == child->fMarkType) {
                ...
                /*  
                parse the example
                    check for  SkDebugf and / or StdOut children
                    check to see if it is bracketed with
