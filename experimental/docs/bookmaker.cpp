...
string Parser::addReferences(const string& text) {
    ...
    do {
        ...
        if (start < t.fChar) {
            ...
            auto externalIter = fExternalMap.find(ref);
            if (externalIter != fExternalMap.end()) {
                continue;
            }
            ...
            if (!ref.compare(0, 2, "SK") || !ref.compare(0, 3, "sk_")
                    || (('k' == start[0] || 'g' == start[0] || 'f' == start[0]) &&
                        ref.length() > 1 && isupper(start[1]))) {
                // try with a prefix
