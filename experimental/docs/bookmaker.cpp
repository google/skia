...
void BmhParser::markTypeOut(Definition* def) {
    ...
    if (MarkType::kParam != def->fMarkType && TableState::kNone != fTableState) {
        fprintf(fMdOut, 
                "</table>\n"
                "\n"
        );
    ...
    switch (def->fMarkType) {
        ...
        case MarkType::kMethod: {
            ...
            fprintf(fMdOut, 
                ...
                "\n", 
                def->fFiddle.c_str(),  // anchor name
                (int) (funcParser.trimmedLineEnd() - funcParser.fChar), funcParser.fChar  // method
            );
            fTableState = TableState::kNone;
            } break;
        case MarkType::kParam: {
...
void BmhParser::resolveOut(const char* start, const char* end) {
    ...
    if (resolved.length()) {
        fprintf(fMdOut, "%s", resolved.c_str());
