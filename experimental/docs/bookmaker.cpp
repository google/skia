...
bool BmhParser::buildReferences(const char* fileOrPath) {
    if (!sk_isdir(fileOrPath)) {
...
bool BmhParser::buildRefFromFile(const char* name) {
...
string BmhParser::uniqueName(const string& base, MarkType markType) const {
    string builder(base);
    if (!builder.length()) {
        builder = fParent->fName;
    }
...
string BmhParser::uniqueRootName(const string& base, MarkType markType) const {
...
//        parser.setParseType(Parser::ParseType::kInclude);
    ...
    if (FLAGS_ref && !FLAGS_examples) {
        bmhParser.buildReferences(FLAGS_bmh[0]);
        SkDebugf("\n");
    }
    int examples = 0;
    int methods = 0;
