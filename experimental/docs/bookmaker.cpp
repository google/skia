...
void RootDefinition::dumpUnVisited() {
 ...
 Definition* RootDefinition::find(const string& ref) {
    auto leafIter = fLeaves.find(ref);
    if (leafIter != fLeaves.end()) {
        return &leafIter->second;
    }
    auto branchIter = fBranches.find(ref);
    if (branchIter != fBranches.end()) {
        RootDefinition* rootDef = &branchIter->second;
        return rootDef;
    }
    for (auto iter : fBranches) {
        Definition* branchDef = iter.second.find(ref);
        if (branchDef) {
            return branchDef;
        }
    }
...
bool BmhParser::isDefined(const string& ref, string* def) const {
    ...
    if (fRoot) {
        if ((definition = fRoot->find(ref))) {
            *def = definition->fFiddle;
            return true;
        }
        string prefixed = fRoot->fName + "_" + ref;
        if ((definition = fRoot->find(prefixed))) {
            *def = definition->fFiddle;
            return true;
        }
    }
        ...
        if ('k' == ref[0]) {
            for (auto const& iter : fEnumMap) {
                if (iter.second.find(ref)) {
                    *def = iter.second.fFiddle;
                    return true;
                }
            }
        }
        return this->reportError<bool>("SK undefined");
    }
    if (isupper(ref[0])) {
        auto topicIter = fTopicMap.find(ref);
        if (topicIter != fTopicMap.end()) {
            *def = topicIter->second->fFiddle;
            return true;
        }
        ...
        if (string::npos != pos) {
            ...
            if (classIter != fClassMap.end()) {
                if ((definition = classIter->second.find(ref))) {
                    *def = definition->fFiddle;
                    return true;
                }
            }
            ...
            if (enumIter != fEnumMap.end()) {
                if ((definition = enumIter->second.find(ref))) {
                    *def = definition->fFiddle;
                    return true;
                }
            }
            return this->reportError<bool>("_ undefined");
        }
    }
    return false;
}
...
static void add_link(const string& ref, const string& def, string* result) {
...
static void add_ref(const string& ref, string* result) {
    if (result->length() > 0 && ' ' < result->back() && isalnum(ref[0])) {
        *result += ' ';
    }
    *result += ref;
}
...
string BmhParser::addReferences(const char* refStart, const char* refEnd) {
    ...
    do {
        ...
        if (start < t.fChar) {
            ...
            string def;
            if (this->isDefined(ref, &def)) {
                add_link(ref, def, &result);
                continue;
            }
    ...
    // methods may start with upper (static) or lower (most)
            ...
            if (isupper(start[0])) {
                auto topicIter = fTopicMap.find(ref);
                if (topicIter != fTopicMap.end()) {
                    add_link(ref, topicIter->second->fFiddle, &result);
                    continue;
                }
                ...
                if (!t.eof() && !isupper(t.fChar[0]) && startsSentence) {
                    add_ref(ref, &result);
                    continue;
                }
                if (!t.eof() && isupper(t.fChar[0]) && startsSentence) {
                    ...
                    string dummy;
                    if (this->isDefined(nextWord, &dummy)) {
                        add_ref(ref, &result);
                        continue;
                    }
                }
                RootDefinition* root = fRoot;
                do {
                    string prefixed = root->fName + "_" + ref;
                    Definition* def;
                    if ((def = root->find(prefixed))) {
                        add_link(ref, def->fFiddle, &result);
                        break;
                    }
                } while ((root = root->rootParent()));
                if (!root) {
                    SkDebugf("%s\n", ref.c_str());
                }
            }
...
if (MarkType::kExample != def->fMarkType && MarkType::kFile != def->fMarkType && MarkType::kError != def->fMarkType
             && MarkType::kFunction != def->fMarkType && MarkType::kToDo != def->fMarkType && MarkType::kDefinedBy != def->fMarkType
             && MarkType::kStdOut != def->fMarkType && MarkType::kComment != def->fMarkType) {
    ...
    SkDebugf("");
				...
				switch (markType) {
                        ...
                        if (nullptr == fRoot) {
                            ...
                            if (fParent == fRoot && hasEnd) {
                                ...
                                if (MarkType::kStruct == markType || MarkType::kClass == markType) {
                                    ...
                                    SkASSERT(!hasEnd);
                                    RootDefinition* childRoot = &fRoot->fBranches[name];
                    ...
                    case MarkType::kDefinedBy: {
                        if (!fParent || MarkType::kMethod != fParent->fMarkType) {
					        return this->reportError<bool>("expect method parent");
                        }
                        ...
                        definition->fName = definition->fFiddle = name;
...
void BmhParser::markTypeOut(Definition* def) {
    ...
    switch (def->fMarkType) {
        ...
        case MarkType::kConst: {
            this->mdHeaderOut();
            fprintf(fMdOut, "<a name=\"%s\"></a> Const %s", def->fName.c_str(), colonform.c_str());
            ...
            fprintf(fMdOut, " %.*s", (int) (lineEnd - textStart), textStart);
            textStart = lineEnd;
            } break;
        ...
        case MarkType::kEnumClass:
            this->mdHeaderOut();
            fprintf(fMdOut, "<a name=\"%s\"></a> Enum %s\n", def->fName.c_str(), colonform.c_str());
            break;
        case MarkType::kError:
            break;
        case MarkType::kExample:
            fprintf(fMdOut, "\n<fiddle-embed hash=\"%s\"></fiddle-embed>\n", def->fHash.c_str());
            break;
        case MarkType::kExternal:
            break;
        case MarkType::kFile:
        ...
        case MarkType::kMethod: {
            this->mdHeaderOut();
            fprintf(fMdOut, "<a name=\"%s\"></a>", def->fFiddle.c_str());
            TextParser funcParser(def->fStart, def->fContentStart);
            funcParser.skipWhiteSpace();
            SkASSERT(funcParser.startsWith("#Method"));
            funcParser.next();
            funcParser.skipToNonAlphaNum();
            funcParser.skipSpace();
            fprintf(fMdOut, "\n\n<!--?prettify lang=cc?-->\n\n    %.*s\n",
                    (int) (funcParser.trimmedLineEnd() - funcParser.fChar), funcParser.fChar);
            } break;
        case MarkType::kParam: {
            ...
            paramParser.next();
            paramParser.skipToNonAlphaNum();
            paramParser.skipSpace();
            fprintf(fMdOut, "\n**`%.*s`**  ", (int) (paramParser.trimmedLineEnd() - paramParser.fChar),
                    paramParser.fChar);
        } break;
        case MarkType::kPlatform:
            break;
        ...
        case MarkType::kSubtopic:
            this->mdHeaderOut();
            fprintf(fMdOut, "<a name=\"Subtopic_%s\"></a> %s\n", def->fName.c_str(), printable.c_str());
            break;
        case MarkType::kTable:
            fprintf(fMdOut, "\n");
            break;
        ...
        case MarkType::kTopic:
            this->mdHeaderOut();
            fprintf(fMdOut, "<a name=\"Topic_%s\"></a> %s\n", def->fName.c_str(), printable.c_str());
            break;
        case MarkType::kTrack:
            break;
        case MarkType::kTypedef:
...
void BmhParser::resolveOut(const char* start, const char* end) {
    ...
    trim_end(resolved);
    if (resolved.length()) {
        fprintf(fMdOut, "%s", resolved.c_str());  // dump to console while debugging
    }
}
...
DEFINE_bool2(tokens, t, false, "Output include tokens.");
DEFINE_bool2(crosscheck, x, false, "Check bmh against includes.");
