...
bool Definition::exampleToScript(string* result) const {
    ...
    string example = "var " + fName + "_code = \n";
    size_t pos = 0;
    ...
    if (!hasFunc && !preprocessor) {
        ...
        outIndent = 4;
        textIndent = count_indent(text, pos, end);
    }
    bool firstTime = true;
    do {
        if (!firstTime) {
        ...
        if (textIndent != nextIndent) {
            SkASSERT(textIndent < nextIndent || outIndent >= nextIndent + 1);
            outIndent += textIndent < nextIndent ? 1 : -1;
            textIndent = nextIndent;
        }
        ...
        example += '"';
        for (int index = 0; index < outIndent; ++index) {
            example += "    ";
        }
        pos += nextIndent;
        while (pos < len) {
            example += '"' == text[pos] ? "\\\"" : text.substr(pos, 1);
            ++pos;
        }
        example += "\\n\"";
        if ('\r' == text[pos]) {
    ...
    example += "}\n\n";
    example += "postAjax(\"https://fiddle.skia.org/_/run\", JSON.stringify(" + fName + "_json), ";
    example +=            "testResult);\n";
    *result = example;
    return true;
}
...
static bool dump_examples(const Definition& def) {
    for (auto child : def.fChildren ) {
        if (MarkType::kExample == child->fMarkType) {
            string result;
            if (!child->exampleToScript(&result)) {
                return false;
            }
            SkDebugf("%s\n", result.c_str());
        } else {
...
int main(int argc, char** const argv) {
    ...
    for (auto oneclass : parser.fClassMap) {
        if (!dump_examples(oneclass.second)) {
            SkDebugf("malformed example\n");
            return -1;
        }
