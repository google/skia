...
string BmhParser::addLink(const string& leadingSpaces, const string& ref, const string& def) const {
    string result = leadingSpaces;
    if (fOutputMarkDown) {
        result += "[" + ref + "](#" + def + ")";
    } else {
        result += "<a href=\"#" + def + "\">" + ref + "</a>";
    }
    return result;
}
...
string BmhParser::addReferences(const char* refStart, const char* refEnd) {
    ...
    do {
        ...
        if (this->isDefined(ref, &def)) {
            result += addLink(leadingSpaces, ref, def);
            continue;
        }
...
// methods may start with upper (static) or lower (most)
        ...
        if (!ref.compare(0, 2, "Sk")) {
            SkDebugf("%s\n", ref.c_str());
            result += addLink(leadingSpaces, ref, ref);
            continue;
        } 
        if (!ref.compare(0, 2, "SK")) {
            SkDebugf("%s\n", ref.c_str());
        ...
        auto topicIter = fTopicMap.find(ref);
        if (topicIter != fTopicMap.end()) {
            result += addLink(leadingSpaces, ref, topicIter->second->fFiddle);
            continue;
        }
        ...
        do {
            ...
            if ((def = root->find(prefixed))) {
                result += addLink(leadingSpaces, ref, def->fFiddle);
                break;
            }
        } while ((root = root->rootParent()));
        if (!root) {
            this->reportError<string>("undefined reference");
        }
    } while (!t.eof());
    return result;
}
...
void BmhParser::markTypeOut(Definition* def) {
    ...
    switch (def->fMarkType) {
        ...
        case MarkType::kCode:
            // fCodeIndent = true;
            fprintf(fMdOut, "<div style=\"display: block; font-family: monospace; white-space: pre;"
                "margin: 1em 0px 1em; background-color: #ddd;\">\n");
            fOutputCode = true;
            fOutputMarkDown = true;
            break;
