...
string BmhParser::addReferences(const string& text) {
    string result;
...
void BmhParser::buildReferences() {
    for (auto oneclass : fClassMap) {
        ...
        string ref = this->addReferences(text);
        if (ref.length()) {
            SkDebugf("%s\n", ref.c_str());
        }
...
bool BmhParser::childOf(MarkType markType) const {
    auto childError = [this](MarkType markType) -> bool
    {
        ...
        errStr += " parent";
	    return INHERITED::reportError<bool>(errStr.c_str());
    };

    if (this->hasEndToken()) {
        if (!fParent->fParent) {
			return INHERITED::reportError<bool>("expected grandparent");
        }
...
bool BmhParser::findDefinitions() {
		...
		if (this->peek() == fMC) {
			...
			if (this->peek() == fMC) {  // definition
				if (!lineStart) {
					this->next();
					if (' ' < this->peek()) {
						return INHERITED::reportError<bool>("expected definition");
					}
					continue;
				}
				this->next();
                if (this->peek() == fMC) {
                    ...
                    if (' ' >= fMC) {
                        return INHERITED::reportError<bool>("illegal markup character");
                    }
                    continue;
                }
                ...
                if (!this->skipSpace()) {
                    return INHERITED::reportError<bool>("unexpected end");
                }
				...
				switch (markType) {
                    ...
                    case MarkType::kTypedef: {
				        if (!typeNameBuilder.size()) {
					        return INHERITED::reportError<bool>("unnamed markup");
				        }
                        if (typeNameBuilder.size() > 1) {
					        return INHERITED::reportError<bool>("expected one name only");
                        }
                        ...
                        if (nullptr == fRoot) {
                            ...
                            if (nullptr == fParent) {
					            return INHERITED::reportError<bool>("expected parent");
                            }
                            if (fParent == fRoot && hasEnd) {
                                ...
                                if (!hasEnd && fRoot->fChildDefinitions.end() != dup) {
    					            return INHERITED::reportError<bool>("duplicate symbol");
                                }
                    ...
                    case MarkType::kSubtopic:
				        ...
				        if (!hasEnd) {
				            if (!typeNameBuilder.size()) {
					            return INHERITED::reportError<bool>("unnamed topic");
				            }
                        ...
                        for (auto topic : typeNameBuilder) {
                            ...
                            if (hasEnd) {
                                if (!definition) {
					                ...
					                return INHERITED::reportError<bool>("mismatched topic");
                                }
                            } else {
                                if (nullptr != defPtr) {
					                return INHERITED::reportError<bool>("already declared topic");
                                }
                    ...
                    case MarkType::kTrack:
				        if (hasEnd) {
                            definition = fParent;
                            if (markType != fParent->fMarkType) {
    					        return INHERITED::reportError<bool>("end element mismatch");
                    ...
                    case MarkType::kTime:
				        if (hasEnd) {
    					    return INHERITED::reportError<bool>("one liners omit end element");
                        }
                        fMarkup.emplace_front(markType, defStart, fParent);
                        definition = &fMarkup.front();
                        definition->fName = typeNameBuilder[0];
				...
				if (endHashes <= 1) {  // one line comment
					...
					if (2 != endHashes) {
                        ...
                        errorStr += fMC;
                        errorStr += fMC;
						return INHERITED::reportError<bool>(errorStr.c_str());
					}
					if (!fParent || MarkType::kTable != fParent->fMarkType) {
						return INHERITED::reportError<bool>("missing table");
					}
				}
			}
		}
    ...
    if (fParent) {
        return INHERITED::reportError<bool>("mismatched end");
    }
	return true;
}
...
fail:
    if (MarkLookup::kRequire == lookup) {
        return INHERITED::reportError<MarkType>("unknown mark type");
    }
    ...
    if (mc) {
        ...
        while (nameStart > fChar && ' ' != (ch = *--nameStart)) {
		    if (!isalnum(ch) && '_' != ch && ':' != ch && '-' != ch) {
			    return INHERITED::reportError<string>("unexpected method name char");
		    }
        }
        if (' ' != nameStart[0]) {
            return INHERITED::reportError<string>("missing method name");
        }
        ++nameStart;
        if (nameStart >= paren) {
            return INHERITED::reportError<string>("missing method name length");
        }
        ...
        return INHERITED::reportError<string>("missing method name and reference");
    }
    if (fChar < --end) {
        fChar = end;
    }
...
bool BmhParser::popParentStack(Definition* definition) {
	if (!fParent) {
		return INHERITED::reportError<bool>("missing parent");
	}
	if (definition != fParent) {
		return INHERITED::reportError<bool>("definition end is not parent");
	}
	if (!definition->fStart) {
		return INHERITED::reportError<bool>("definition missing start");
	}
	if (definition->fContentEnd) {
		return INHERITED::reportError<bool>("definition already ended");
	}
...
bool BmhParser::skipToDefinitionEnd(MarkType markType) {
    if (this->eof()) {
        return INHERITED::reportError<bool>("missing end");
    }
...
vector<string> BmhParser::typeName(MarkType markType) {
    ...
    switch (markType) {
        ...
        case MarkType::kTable:
            ...
            if ('\n' != this->peek()) {
                INHERITED::reportError<vector<string>>("unexpected table content");
            }
            break;  // unnamed
        case MarkType::kSubtopic:
...
string BmhParser::word(const string& prefix) {
	...
	while (fChar < lineEnd) {
		...
		if (!isalnum(ch) && '_' != ch && ':' != ch && '-' != ch) {
			return INHERITED::reportError<string>("unexpected char");
		}
        if (':' == ch) {
            // expect pair, and expect word to start with Sk
            if (nameStart[0] != 'S' || nameStart[1] != 'k') {
                return INHERITED::reportError<string>("expected Sk");
            }
            if (':' != this->peek()) {
                return INHERITED::reportError<string>("expected ::");
            }
            this->next();
        } else if ('-' == ch) {
            // expect word not to start with Sk or kX where X is A-Z
            if (nameStart[0] == 'k' && nameStart[1] >= 'A' && nameStart[1] <= 'Z') {
                return INHERITED::reportError<string>("didn't expected kX");
            }
            if (nameStart[0] == 'S' && nameStart[1] == 'k') {
                return INHERITED::reportError<string>("expected Sk");
            }
...
int main(int argc, char** const argv) {
    ...
    FiddleParser fparser;
    if (!FLAGS_fiddle.isEmpty()) {
        if (!fparser.parseFile(FLAGS_fiddle[0])) {
			return -1;
		}
    }
