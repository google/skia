...
DEFINE_string2(examples, e, "fiddle.json", "File of fiddlecli input, by default fiddle.json (For now, disables -r -f -s)");
DEFINE_string2(include, i, "", "A path to a *.h file or a directory.");
DEFINE_string2(fiddle, f, "fiddleout.json", "File of fiddlecli output, by default fiddleout.json");
...
int main(int argc, char** const argv) {
   ...
   SkCommandLineFlags::SetUsage(
    ...
    if (!FLAGS_ref.isEmpty() && FLAGS_examples.isEmpty()) {
        MdOut mdOut(bmhParser);
        mdOut.buildReferences(FLAGS_bmh[0], FLAGS_ref[0]);
        SkDebugf("\n");
    }
    if (FLAGS_spellcheck && FLAGS_examples.isEmpty()) {
    ...
    if (!FLAGS_examples.isEmpty()) {
        fiddleOut = fopen(FLAGS_examples[0], "w");
        fprintf(fiddleOut, "{\n");
