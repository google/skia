...
bool BmhParser::isDefined(const string& ref) const {
    ...
    auto prefixIter = fRoot->fChildDefinitions.find(prefixed);
    if (prefixIter != fRoot->fChildDefinitions.end()) {
        return true;
    }
    ...
    } else if (isupper(ref[0]) || '~' == ref[0] || starts_with(ref, "operator")) {
			...
			if (this->peek() == fMC) {
				...
				switch (markType) {
                    ...
                    case MarkType::kTime:
                        ...
                        if (MarkType::kAnchor == markType) {
                            this->skipToEndBracket(fMC);
                            fMarkup.emplace_front(MarkType::kLink, fChar, definition);
                            SkAssertResult(fMC == this->next());
                            this->skipWhiteSpace();
                ...
                if (!fParent || (MarkType::kTable != fParent->fMarkType
				    ...
				    if (endHashes <= 1) {  // one line comment
                        if (fParent) {
                            fMarkup.emplace_front(MarkType::kComment, fChar - 1, fParent);
                    ...
                    fMarkup.emplace_front(MarkType::kRow, colStart, fParent);
                    ...
                    do {
                        fMarkup.emplace_front(MarkType::kColumn, colStart, fParent);
