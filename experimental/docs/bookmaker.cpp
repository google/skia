...
bool Definition::checkMethod() const {
    ...
    if (4 != returnEnd - lastStart || !strncmp("void", lastStart, 4)) {
        bool foundReturn = false;
        for (auto& returnInfo : fChildren) {
            if (MarkType::kReturn != returnInfo->fMarkType) {
                continue;
            }
            if (foundReturn) {
                return false;  // todo: report error "multiple #Return markers"
            }
            foundReturn = true;
        }
        if (!foundReturn) {
            return false;  // todo: report error "missing #Return marker"
        }
    }
    const char* paren = methodParser.strnchr('(', methodParser.fEnd);
    if (!paren) {
        return false;  // todo: report error "missing #Method function defintion"
    }
    ...
    do {
        ...
        if (!nextEnd) {
            return false;  // todo: report error "#Method function missing close paren"
        }
        ...
        for (auto& paramInfo : fChildren) {
            if (MarkType::kParam != paramInfo->fMarkType) {
                continue;
            }
            if (foundReturn) {
                return false;  // todo: report error "multiple #Return markers"
