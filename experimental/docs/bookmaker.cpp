...
static string canonical_fiddle_name(const string& name, Definition::MethodType* methodType) {
    *methodType = Definition::MethodType::kNone;
    ...
    if (string::npos != name.find('~', doubleColons)) {
        *methodType = Definition::MethodType::kDestructor;
        result += "destructor";
    } else {
        ...
        if (string::npos != opPos) {
            *methodType = Definition::MethodType::kOperator;
            ...
            if (isupper(name[doubleColons + 2])) {
                *methodType = Definition::MethodType::kConstructor;
                result += "empty_constructor"; 
            } else {
                result += name.substr(doubleColons, name.length() - doubleColons - 2);
            }
        } else {
            ...
            if (string::npos == comma && string::npos != start) {
                ...
                *methodType = Definition::MethodType::kConstructor;
                ...
                TextParser params("", &name[start] + 1, &*name.end(), 0);
...
bool Definition::checkMethod() const {
    ...
    if (!paren) {
        return methodParser.reportError<bool>("missing #Method function defintion");
    }
    methodParser.fChar = paren + 1;
    methodParser.skipSpace();
    const char* nextEnd;
    do {
        nextEnd = methodParser.anyOf(",)");
        if (!nextEnd) {
        ...
        string paramName(paramStart, paramEnd - paramStart);
        if (!paramName.length()) {
            if (')' != nextEnd[0]) {
...
//                        fRoot->fChildren.push_back(definition);
                ...
                if (MarkType::kMethod == markType) {
                    if (string::npos != name.find(':', 0)) {
                        definition->fFiddle = canonical_fiddle_name(name, &definition->fMethodType);
                    } else {
...
// -b "C:/skia/experimental/docs" -r -f "C:/skia/experimental/docs/fiddleout.json"
// cp out/skia/obj/bmh_Sk*.md sites/user/api
// or
...
DEFINE_bool2(ref, r, false, "Build references. (Requires -b)");
