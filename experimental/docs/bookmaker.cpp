...
more space between name and def for Parameters
...
two spaces before 'const' in getFontMetrics()
inconsistent use of capitalization in #Param

fiddle runtime error in Paint_Dither: Failed to run, possibly violated security container.
fiddle runtime error in SkPaint_getTextBlobIntercepts: Failed to run, possibly violated security container.
fiddle runtime error in SkPaint_setImageFilter: Failed to run, possibly violated security container.
fiddle runtime error in SkPaint_setRasterizer: Failed to run, possibly violated security container.
 */
...
string BmhParser::uniqueRootName(const string& base, MarkType markType) {
    ...
    do {
        if (fRoot) {
            for (const auto& iter : fRoot->fBranches) {
                if (checkName(*iter.second, numBuilder)) {
                    goto tryNext;
                }
            }
            for (const auto& iter : fRoot->fLeaves) {
                if (checkName(iter.second, numBuilder)) {
                    goto tryNext;
                }
            }
...
// ~/go/bin/fiddlecli --input out/skia/obj/fiddle.json --output experimental/docs/fiddleout.json
// -b "C:/skia/experimental/docs" -r "C:/skia/site/user/api" -f "C:/skia/experimental/docs/fiddleout.json"
// or
// -b "C:/skia/experimental/docs" -x "C:/skia/includes"

DEFINE_string2(bmh, b, "", "A path to a *.bmh file or a directory.");
DEFINE_string2(include, i, "", "A path to a *.h file or a directory.");
DEFINE_string2(fiddle, f, "", "A path to go/bin/fiddlecli output, usually fiddleout.json");
...
DEFINE_bool2(examples, e, false, "Output examples. (For now, disables -r -f -s)");
...
int main(int argc, char** const argv) {
   ...
   SkCommandLineFlags::SetUsage(
    ...
    if (FLAGS_bmh.isEmpty() && FLAGS_examples) {
    ...
    if (!FLAGS_include.isEmpty()) {
        ...
        if (FLAGS_populate) {
            if (!interfaceParser.populate(bmhParser)) {
                return -1;
            }
        }
    }
    ...
    if (!FLAGS_ref.isEmpty() && !FLAGS_examples) {
        MdOut mdOut(bmhParser);
        mdOut.buildReferences(FLAGS_bmh[0], FLAGS_ref[0]);
        SkDebugf("\n");
    }
    if (FLAGS_spellcheck && !FLAGS_examples) {
    ...
    FILE* fiddleOut;
    if (FLAGS_examples) {
        fiddleOut = fopen("fiddle.json", "w");
        fprintf(fiddleOut, "{\n");
        bool continuation = false;
        for (auto& oneclass : bmhParser.fClassMap) {
            dump_examples(fiddleOut, oneclass.second, &continuation);
        }
        fprintf(fiddleOut, "\n}\n");
        fclose(fiddleOut);
    }
