...
static string canonical_fiddle_name(const string& name) {
    ...
    string result = name.substr(0, doubleColons + 1);
...
//                        fRoot->fChildren.push_back(definition);
        ...
        case MarkType::kSubtopic:
            ...
            for (auto& topic : typeNameBuilder) {
                ...
                Definition* root = fRoot;
                while (root) {
                    if (MarkType::kTopic == root->fMarkType) {
                        fullTopic = root->fName + '_' + fullTopic;
                    }
                    root = root->fParent;
                }
...
string BmhParser::addReferences(const char* refStart, const char* refEnd) {
    ...
    do {
        ...
        if (const Definition* def = this->isDefined(t, ref)) {
            ...
            result += linkRef(leadingSpaces, def);
            continue;
        }
...
// methods may start with upper (static) or lower (most)
        ...
        auto topicIter = fTopicMap.find(ref);
        if (topicIter != fTopicMap.end()) {
            result += linkRef(leadingSpaces, topicIter->second);
            continue;
        }
...
string BmhParser::linkRef(const string& leadingSpaces, const Definition* def) const {
    string buildup;
    ...
    if (classMatch) {
        ...
        str = &buildup;
        defStr = def->printableName();
    }
    return leadingSpaces + "<a href=\"" + *str + "\">" + def->printableName() + "</a>";
}
