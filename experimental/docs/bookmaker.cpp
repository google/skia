...
TextParser::TextParser(Definition* definition) :
...
void TextParser::reportError(const char* errorStr) const {
    TextParser err(fFileName, fStart, fEnd, fLineCount);
    ...
    SkDebugf("%s:%zd:%td: error: %s\n", fFileName.c_str(), err.fLineCount, spaces, errorStr);
    if (0 == lineLen) {
        SkDebugf("[blank line]\n");
    } else {
        SkDebugf("%.*s\n", (int) (lineLen - 1), err.fLine);
        SkDebugf("%*s^\n", (int) spaces, "");
    }
