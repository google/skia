    ...
    if (isupper(ref[0])) {
        ...
        if (string::npos != pos) {
            ...
            if (classIter != fClassMap.end()) {
                if (classIter->second.find(ref)) {
                    return true;
                }
            }
            auto enumIter = fEnumMap.find(className);
            if (enumIter != fEnumMap.end()) {
                if (enumIter->second.find(ref)) {
                    return true;
                }
            }
...
//                    if ("Some" == ref) {  // todo: temporary skip until one pass is done
//                        continue;
//                    }
                }
...
void BmhParser::markTypeOut(Definition* def) {
    ...
    switch (def->fMarkType) {
        ...
        case MarkType::kMethod: {
            this->mdHeaderOut();
            printf("<a name=\"%s\">%s</a>\n", def->fFiddle.c_str(), colonform.c_str());
            } break;
        case MarkType::kParam:
