...
void IncludeWriter::enumMembersOut(const RootDefinition* root, const Definition& child) {
    ...
    for (int onePast = 0; onePast < 2; onePast += tokenIter == child.fTokens.end()) {
        ...
        if (!currentEnumItem->fShort) {
            this->writeCommentHeader();
            fIndent += 4;
            bool wroteLineFeed = this->rewriteBlock(commentLen, commentStart);
            fIndent -= 4;
...
void IncludeWriter::methodOut(const Definition* method) {
    ...
    for (auto methodProp : method->fChildren) {
        if (MarkType::kDeprecated == methodProp->fMarkType) {
            deprecated = true;
        } else if (MarkType::kPrivate == methodProp->fMarkType) {
            privateMethod = true;
        } else if (MarkType::kDefinedBy == methodProp->fMarkType) {
            definedBy = true;
        }
    }
    if (0 == fIndent) {
    ...
    bool altComment = deprecated || privateMethod || definedBy;
    const char* commentStart = method->fContentStart;
    if (altComment) {
        commentStart = method->fChildren[0]->fContentEnd;
    }
    const char* commentEnd;
    ...
    fIndent += 4;
    if (deprecated) {
        this->writeString("(to be deprecated)");
        this->lfcr();
    } else if (privateMethod || definedBy) {
        commentStart = method->fChildren[0]->fTerminator;
        commentEnd = method->fChildren[1]->fStart;
        if (privateMethod) {
            this->writeString("(to be made private)");
        }
        this->lfcr();
    }
    int commentLen = (int) (commentEnd - commentStart);
    SkASSERT((commentLen > 0 && commentLen < 1000) || altComment);
    if (!deprecated) {
        this->rewriteBlock(commentLen, commentStart);
    }
...
static string ConvertRef(const char* start, const char* end) {
    string substitute;
    while (start < end) {
        char c = *start++;
        if ('_' == c) {
...
string IncludeWriter::resolveMethod(const char* start, const char* end, bool first) {
    ...
    string substitute;
    const RootDefinition* rootDef = nullptr;
    ...
    const Definition* rootDef = nullptr;
    {
        ...
        if (fBmhParser->fTopicMap.end() != rootDefIter) {
            ...
            if (fBmhParser->fTopicMap.end() != rootDefIter) {
                ...
                string originalName(start, end - start);
                auto aliasIter = fBmhParser->fAliasMap.find(originalName);
                if (fBmhParser->fAliasMap.end() != aliasIter) {
                    rootDef = aliasIter->second->fParent;
                } else if (!first) {
                    for (const auto& external : fBmhParser->fExternals) {
                        if (external.fName == originalName) {
        ...
        if (!substitute.length()) {
            ...
            if (parent) {
                if (MarkType::kClass == parent->fMarkType ||
                    ...
                    substitute += ConvertRef(&undername[under], &undername.back() + 1);
                }
            }
        }
    }
...
int IncludeWriter::lookupReference(const PunctuationState punctuation, const Word word,
    ...
    if (!temp.length()) {
        if (Word::kFirst != word && '_' != last) {
            temp = ConvertRef(&data[start], &data[end]);
        }
    }                     
    if (temp.length()) {
        if (start > lastWrite) {
...
bool IncludeWriter::rewriteBlock(int size, const char* data) {
