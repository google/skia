...
static Definition* find_name(Definition* def, const string& name) {
    if (MarkType::kExample == def->fMarkType && name == def->fName) {
        return def;
    }
    for (auto child : def->fChildren) {
        Definition* result = find_name(child, name);
        if (result) {
            return result;
        }
    }
...
Definition* FiddleParser::findExample(const string& name) const {
    for (auto oneclass : fBmhParser.fClassMap) {
        Definition* def = find_name(&oneclass.second, name);
        if (def) {
            return def;
        }
    }
...
bool FiddleParser::parseFiddles() {
	while (!this->eof()) {
        if (this->startsWith("example.htm")) {
            break;
        }
    ...
    while (!this->eof()) {
        ...
        if (this->contains(nameStr, end, &nameLoc)
            ...
            } while (!this->startsWith("example.htm:"));
