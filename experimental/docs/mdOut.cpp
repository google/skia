...
// methods may start with upper (static) or lower (most)
        ...
        if (!isupper(start[0])) {
            // start here
            // check to see if ref is more than a word, e.g. dstInfo.width()
            // also look for all lowercase w/o trailing parens as mistaken method matches
            // will also need to see if Example Description matches var in example
            const Definition* def;
            if (fMethod && (def = fMethod->hasParam(ref))) {
                result += linkRef(leadingSpaces, def, ref);
                continue;
            } else if (string::npos != ref.find_first_of("ABCDEFGHIJKLMNOPQRSTUVWXYZ")) {
...
bool MdOut::buildReferences(const char* fileOrPath) {
    if (!sk_isdir(fileOrPath)) {
        if (!this->buildRefFromFile(fileOrPath)) {
...
bool MdOut::buildRefFromFile(const char* name) {
...
const Definition* MdOut::isDefined(const TextParser& parser, const string& ref) const {
...
void MdOut::lf(int count) {
    while (fLinefeeds < count) {
        fprintf(fMdOut, "\n");
        ++fLinefeeds;
    }
}

void MdOut::lfAlways(int count) {
    fLinefeeds = 0;
    this->lf(count);
}
...
void MdOut::markTypeOut(Definition* def) {
    ...
    switch (def->fMarkType) {
        ...
        case MarkType::kClass:
            this->mdHeaderOut(1);
            fprintf(fMdOut, "<a name=\"%s\"></a> Class %s", this->linkName(def).c_str(),
                    colonform.c_str());
            this->lfAlways(1);
            break;
        case MarkType::kCode:
            this->lf(2);
            fprintf(fMdOut, "<pre style=\"padding: 1em 1em 1em 1em;"
        ...
        case MarkType::kEnumClass:
            this->mdHeaderOut(2);
            fprintf(fMdOut, "<a name=\"%s\"></a> Enum %s", def->fName.c_str(), colonform.c_str());
            this->lfAlways(2);
            break;
        ...
        case MarkType::kExample:
            this->mdHeaderOut(3);
            fprintf(fMdOut, "Example\n"
                            "\n"
                            "<div><fiddle-embed name=\"%s\">", def->fHash.c_str());
            break;
        ...
        case MarkType::kMethod: {
            ...
            if (!def->isClone()) {
                this->lf(2);
                fprintf(fMdOut, "<a name=\"%s\"></a>", def->fiddleName().c_str());
                this->lfAlways(1);
                this->mdHeaderOut(2);
                fprintf(fMdOut, "%s", method_name.c_str());
                this->lfAlways(2);
            }
        ...
        case MarkType::kStruct:
            fRoot = def->asRoot();
            this->mdHeaderOut(1);
            fprintf(fMdOut, "<a name=\"%s\"></a> Struct %s", def->fName.c_str(), colonform.c_str());
            this->lfAlways(1);
            break;
        case MarkType::kSubtopic:
            this->mdHeaderOut(2);
            fprintf(fMdOut, "<a name=\"%s\"></a> %s", def->fName.c_str(), printable.c_str());
            this->lfAlways(2);
            break;
        ...
        case MarkType::kTopic:
            this->mdHeaderOut(1);
            fprintf(fMdOut, "<a name=\"%s\"></a> %s", this->linkName(def).c_str(),
                    printable.c_str());
            this->lfAlways(1);
            break;
    ...
    switch (def->fMarkType) {  // post child work, at least for tables
        ...
        case MarkType::kEnumClass:
            ...
            break;
        case MarkType::kExample:
            fprintf(fMdOut, "</fiddle-embed></div>");
            this->lfAlways(2);
        break;
        case MarkType::kLegend: {
...
void MdOut::mdHeaderOut(int depth) {
    this->lf(2);
...
void MdOut::resolveOut(const char* start, const char* end) {
