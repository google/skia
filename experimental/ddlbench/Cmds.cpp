...
void RectCmd::execute(FakeCanvas* c) const {
    ...
    c->drawRect(fID, fRect, p, fMCState);
}

static void apply_diff(SkCanvas* c, const FakeMCBlob& current, const FakeMCBlob* prior) {
    // Find the common prefix between the two states
    int prefix = 0;

    if (prior) {
        for ( ;
             prefix < current.count() && prefix < prior->count() && current[prefix] == (*prior)[prefix];
             ++prefix) {
            ;
        }
    ...
    for (int j = prefix; j < current.count(); ++j) {
        current[j].apply(c);
    }
}
...
void RectCmd::execute(SkCanvas* c, const FakeMCBlob* priorState) const {
    ...
    apply_diff(c, *fMCState.get(), priorState);
