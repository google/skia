 ...
 * preserves the RectCmd order is ultimately used for its rendering and this can be compared
...
static void check_state(FakeMCBlob* state,
                        SkIPoint expectedCTM,
                        const std::vector<SkIRect>& expectedClips) {
    SkASSERT(state->ctm() == expectedCTM);

    int i = 0;
    auto states = state->clipRects();
    for (auto& s : states) {
        for (auto r : s.rects()) {
...
static void mcstack_test() {
    ...
    const std::vector<SkIRect> s0Clips;
    const std::vector<SkIRect> s1Clips { r.makeOffset(s1Trans) };
    const std::vector<SkIRect> s2aClips {
        r.makeOffset(s1Trans),
        r.makeOffset(s2TransA)
    };
    const std::vector<SkIRect> s2bClips {
        r.makeOffset(s1Trans),
        r.makeOffset(s2TransA),
    ...
    check_state(state0.get(), { 0, 0 }, s0Clips);
    ...
    check_state(state1.get(), s1Trans, s1Clips);
    ...
    check_state(state2a.get(), s1Trans + s2TransA, s2aClips);

    s.translate(s2TransB);
    s.clipRect(r);

    auto state2b = s.snapState();
    check_state(state2b.get(), s1Trans + s2TransA + s2TransB, s2bClips);
    SkASSERT(state2a != state2b);

    //----------------
    s.pop();
    auto state3 = s.snapState();
    check_state(state3.get(), s1Trans, s1Clips);
    SkASSERT(state1 == state3);

    //----------------
    s.pop();
    auto state4 = s.snapState();
    check_state(state4.get(), { 0, 0 }, s0Clips);
