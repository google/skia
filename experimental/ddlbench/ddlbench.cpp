...
struct ThreadInfo {
    ...
    double seconds() const { return std::chrono::duration<double>(fElapsed).count(); }
    double ms() const { return std::chrono::duration<double, std::milli>(fElapsed).count(); }

    void dump() const {
        duration totalElapsed = fThreadStop - fThreadStart;
        double totalElapsedSeconds = std::chrono::duration<double>(totalElapsed).count();
        ...
        printf("%s: num work units %d work: %.2gs total: %.2gs utilization %.2g%%\n",
               ...
               this->seconds(),
               totalElapsedSeconds,
               100.0f * this->seconds() / totalElapsedSeconds);
    }
    ...
    int                fWorkUnit = 0;
    duration           fElapsed {0};
    clock2::time_point fThreadStart;
    clock2::time_point fThreadStop;
};
...
static void set_up_context_on_thread(ThreadInfo* threadInfo) {
    ...
    threadInfo->fThreadStart = clock2::now();
...
private:
    ...
    bool do_work(ThreadInfo* threadInfo) {
        ...
        if (!work) {
            threadInfo->fThreadStop = clock2::now();
            return false;  // This is Loop()'s signal to shut down.
        }

        clock2::time_point start = clock2::now();
        work();
        threadInfo->fElapsed = clock2::now() - start;
        threadInfo->fWorkUnit++;

        return true;
    }

    static void Loop(void* ctx, ThreadInfo* threadInfoIn) {
        set_up_context_on_thread(threadInfoIn);

        ThreadInfo* threadInfo = get_thread_local_info();
...
static bool create_contexts(GrContextFactory* factory,
    ...
    gpuThread[0] = { SkString("g0"), mainInfo.directContext(), mainInfo.testContext() };
