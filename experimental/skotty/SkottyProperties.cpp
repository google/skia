...
static SkScalar lerp_scalar(SkScalar v0, SkScalar v1, float t) {
    SkASSERT(t >= 0 && t <= 1);
    return v0 * (1 - t) + v1 * t;
}

static SkPoint lerp_point(const SkPoint& v0, const SkPoint& v1, float t) {
    SkASSERT(t >= 0 && t <= 1);
...
template <>
void Keyframed<ScalarValue>::Keyframe::lerp(float t, ScalarValue* v) const {
    *v = lerp_scalar(fV0, fV1, t);
}

template <>
void Keyframed<VectorValue>::Keyframe::lerp(float t, VectorValue* v) const {
...
template <>
void Keyframed<ShapeValue>::Keyframe::lerp(float t, ShapeValue* v) const {
    SkASSERT(fV0.cardinality() == fV1.cardinality());
    SkASSERT(v->cardinality() == 0);
