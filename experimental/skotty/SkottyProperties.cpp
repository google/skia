...
ScalarValue Keyframed<ScalarValue>::Keyframe::lerp(float t) const {
    return ScalarValue({ fV0.as<SkScalar>() * (1 - t) + fV1.as<SkScalar>() * t });
}
...
VectorValue Keyframed<VectorValue>::Keyframe::lerp(float t) const {
    ...
    VectorValue result;
    result.fVals.reserve(fV0.cardinality());

    for (int i = 0; i < fV0.fVals.count(); ++i) {
        const auto v0 = fV0.fVals[i].as<SkScalar>(),
                   v1 = fV1.fVals[i].as<SkScalar>();
