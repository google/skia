...
bool AttachProperty(const Json::Value& jprop, AttachContext* ctx, const sk_sp<NodeT>& node,
    ...
    if (!ParseBool(jprop["a"], false)) {
        ...
        // Keyframe property.
        auto keyframe_prop = Keyframed<ValueT>::Make(jprop);
        if (!keyframe_prop) {
            return LogFail(jprop, "Could not parse keyframe property");
        }

        using AnimatorT = Animator<ValueT, AttrT, NodeT>;
        ctx->fAnimators.emplace_back(
            skstd::make_unique<AnimatorT>(node, std::move(keyframe_prop), std::move(apply)));
    }
...
sk_sp<sksg::GeometryNode> AttachPathGeometry(const Json::Value& jpath, AttachContext* ctx) {
    ...
    auto path_attached = AttachProperty<ShapeValue, SkPath>(jpath["ks"], ctx, path_node,
        [](const sk_sp<sksg::Path>& node, SkPath p) { node->setPath(p); });
...
sk_sp<sksg::PaintNode> AttachStrokePaint(const Json::Value& jstroke, AttachContext* ctx) {
    ...
    auto color = AttachColorPaint(jstroke, ctx);
    if (color) {
        LOG("** Attached color stroke: 0x%x\n", color->getColor());
    }

    auto stroke_node = sksg::Stroke::Make(std::move(color));
    if (!stroke_node)
        return nullptr;

    auto width_attached = AttachProperty<ScalarValue, SkScalar>(jstroke["w"], ctx, stroke_node,
        [](const sk_sp<sksg::Stroke>& node, SkScalar width) { node->setStrokeWidth(width); });
    if (!width_attached)
        return nullptr;
...
const ShapeInfo* FindShapeInfo(const Json::Value& shape) {
    ...
    // TODO: binary search?
    for (const auto& info : gShapeInfo) {
        if (!strcmp(type.asCString(), info.fTypeString)) {
            return &info;
        }
    }

    LOG("!! unknown shape type '%s'\n", type.asCString());
...
sk_sp<sksg::RenderNode> AttachLayer(const Json::Value& layer, AttachContext* ctx) {
    ...
    return gLayerAttachers[type](layer, ctx);
}
