...
sk_sp<sksg::PaintNode> AttachFillColor(const Json::Value& jfill, AttachContext* ctx) {
    SkASSERT(jfill.isObject());

    return AttachPaint(jfill, ctx, AttachColor(jfill, ctx));
}

sk_sp<sksg::PaintNode> AttachFillGradient(const Json::Value& jfill, AttachContext* ctx) {
    SkASSERT(jfill.isObject());

    return AttachPaint(jfill, ctx, AttachGradient(jfill, ctx));
}

sk_sp<sksg::PaintNode> AttachStrokeColor(const Json::Value& jstroke, AttachContext* ctx) {
    SkASSERT(jstroke.isObject());

    return AttachStroke(jstroke, ctx, AttachPaint(jstroke, ctx, AttachColor(jstroke, ctx)));
}

sk_sp<sksg::PaintNode> AttachStrokeGradient(const Json::Value& jstroke, AttachContext* ctx) {
    SkASSERT(jstroke.isObject());
...
const ShapeInfo* FindShapeInfo(const Json::Value& shape) {
    static constexpr ShapeInfo gShapeInfo[] = {
        ...
        { "gf", ShapeType::kPaint         , 2 }, // gfill     -> AttachFillGradient
        { "gr", ShapeType::kGroup         , 0 }, // group     -> AttachShapeGroup
        { "gs", ShapeType::kPaint         , 3 }, // gstroke   -> AttachStrokeGradient
