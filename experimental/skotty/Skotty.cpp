...
sk_sp<sksg::GeometryNode> AttachRectGeometry(const Json::Value& jrect, AttachContext* ctx) {
    ...
    auto r_attached = AttachProperty<VectorValue, std::vector<SkScalar>>(jrect["r"], ctx, composite,
            [](const sk_sp<CompositeRRect>& node, const std::vector<SkScalar>& radii) {
                node->setRadii(radii);
            });
...
const ShapeInfo* FindShapeInfo(const Json::Value& shape) {
    static constexpr ShapeInfo gShapeInfo[] = {
        ...
        { "rc", ShapeType::kGeometry , 1 }, // shape     -> AttachPathGeometry
