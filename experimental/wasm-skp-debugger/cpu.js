...
(function(DebuggerView){
    ...
    DebuggerView.MakeSWCanvasSurface = function(canvas) {
      ...
      surface = DebuggerView.MakeSurface(canvas.width, canvas.height);
      if (surface) {
    ...
    DebuggerView.onRuntimeInitialized = function() {
      ...
      DebuggerView.SkSurface.prototype.flush = function() {
        ...
        if (this._canvas) {
          pixels = new Uint8ClampedArray(DebuggerView.buffer, this._pixelPtr, this._bufferLen);
          imageData = new ImageData(pixels, this._width, this._height);
