...
(function(DebuggerView){
    ...
    DebuggerView.MakeSurface = function(width, height) {
      bufferLen = width * height * 4; // 4 bytes per pixel
      // Allocate the buffer of pixels to be drawn into.
      pixelPtr = DebuggerView._malloc(bufferLen);
      imageInfo = {
        'width':  width,
        'height': height,
        'colorType': DebuggerView.ColorType.RGBA_8888, // RGBA 8888 is the only pixel format we can show on an HTML canvas.
        'alphaType': DebuggerView.AlphaType.Unpremul, // HTML canvas cannot accept premul alpha AFAIK
      }
      surface = this._getRasterDirectSurface(imageInfo, pixelPtr, width * 4);
      if (surface) {
