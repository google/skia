...
(function(DebuggerView){
    function makeWebGLContext(canvas, attrs) {
      ...
      contextAttributes = {
        ...
        stencil: 0,
        antialias: 1,
        premultipliedAlpha: 1, // why?? shouldn't this be off?
        preserveDrawingBuffer: 1, // for the zoom to be able to access the pixels. Incurs performance penalty
      ...
      context = GL.createContext(canvas, contextAttributes);
      if (!context) {
    ...
    DebuggerView.MakeWebGLCanvasSurface = function(canvas) {
      ...
      ctx = DebuggerView.GetWebGLContext(canvas);

      if (!ctx || ctx < 0) {
        throw 'failed to create webgl context: err ' + ctx;
      }

      grcontext = this.MakeGrContext(ctx);
      if (!grcontext) {
        throw (
      ...
      surface = this.MakeOnScreenGLSurface(grcontext, canvas.width, canvas.height);
      if (!surface) {
