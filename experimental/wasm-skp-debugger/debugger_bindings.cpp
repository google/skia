...
class SkpDebugPlayer {
  ...
  private:
      ...
      void loadMultiFrame(SkMemoryStream* stream) {
          ...
          std::unique_ptr<SkSharingDeserialContext> deserialContext =
            std::make_unique<SkSharingDeserialContext>();
          SkDeserialProcs procs = SkSharingDeserializationProcs(deserialContext.get());
