...
class SkpDebugPlayer {
  public:
    ...
    /* loadSkp deserializes a skp file that has been copied into this WASMs memory.
     ...
     * This is because parameters of functions exposed by emscripten may only be primitive data types.
     */
    void loadSkp(uintptr_t cptr, int length) {
      const auto* data = reinterpret_cast<const void*>(cptr);
      const char* chars = reinterpret_cast<const char*>(cptr);
      // todo: pass in bounds
      ...
      SkDebugf("SkPicture::MakeFromData(%d, %d).\n", data, length);
