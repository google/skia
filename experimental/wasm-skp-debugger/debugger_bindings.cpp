...
class SkpDebugPlayer {
  public:
    ...
    const SkMatrix& getCurrentMatrix() { return fDebugCanvas->getCurrentMatrix(); }
    const SkIRect& getCurrentClip() {return fDebugCanvas->getCurrentClip(); }
    ...
    // Return the command list in JSON representation
    // Specifically a poiner to it in M.buffer
    uintptr_t jsonCommandList(SkSurface* surface) {
      SkDynamicMemoryWStream stream;
      SkJSONWriter writer(&stream, SkJSONWriter::Mode::kFast);
      UrlDataManager udm(SkString("/")); // wrong
      ...
      writer.flush();
      return reinterpret_cast<const uintptr_t>(stream.detachAsData()->data());
    }
...
EMSCRIPTEN_BINDINGS(my_module) {
  ...
  class_<SkpDebugPlayer>("SkpDebugPlayer")
    ...
    //.function("getCurrentMatrix",     &SkpDebugPlayer::getCurrentMatrix)
    .function("getCurrentClip",       &SkpDebugPlayer::getCurrentClip)
