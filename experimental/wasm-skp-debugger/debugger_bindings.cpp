...
class SkpDebugPlayer {
  public:
     ...
     *
     * Returns an error string which is populated in the case that the fule cannot be read.
     */
  ...
  private:
      ...
      std::string loadMultiFrame(SkMemoryStream* stream) {
        ...
        if (!page_count) {
          // MSKP's have a version seperate from the SKP subpictures they contain.
