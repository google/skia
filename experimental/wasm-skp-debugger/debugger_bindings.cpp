...
class SkpDebugPlayer {
  public:
    ...
    void drawTo(SkSurface* surface, int32_t index) {
      ...
      if (index >= cmdlen) {
        ...
        index = cmdlen-1;
        // TODO(nifong): inform UI of new command selection?
      }
  ...
  private:
      ...
      void loadMultiFrame(SkMemoryStream* stream) {
          ...
          std::unique_ptr<sharingproc::SharingDeserialContext> deserialContext =
            std::make_unique<sharingproc::SharingDeserialContext>();
          SkDeserialProcs procs = sharingproc::SharingDeserializationProcs(deserialContext.get());
