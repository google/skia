...
double SkVideoEncoder::computeTimeStamp(const AVFrame* frame) const {
...
bool SkVideoEncoder::init(const SkImageInfo& info) {
    this->reset();

    AVPixelFormat fmt = colortype_to_format(info.colorType());
    if (fmt == AV_PIX_FMT_NONE) {
        return false;
    }

    //AVCodec* codec = avcodec_find_encoder_by_name("mpeg4");
    AVCodec* codec = avcodec_find_encoder_by_name("mp42");
    if (!codec) {
        return false;
    }
    ...
    /* frames per second */
    fEncoderCtx->time_base = (AVRational){1, 25};
    fEncoderCtx->framerate = (AVRational){25, 1};
    fEncoderCtx->sample_aspect_ratio = (AVRational){1, 1};
     ...
     */
    fEncoderCtx->gop_size = 10;
    fEncoderCtx->max_b_frames = 1;
    fEncoderCtx->pix_fmt = fmt;
    ...
    fFrame->format = fEncoderCtx->pix_fmt;
...
bool SkVideoEncoder::beginRecording(const SkImageInfo& info, SkWStream* stream) {
    if (!stream) {
        return false;
    }
    if (!this->init(info)) {
...
bool SkVideoEncoder::sendFrame(AVFrame* frame) {
    ...
    for (;;) {
        int ret = avcodec_receive_packet(fEncoderCtx, fPacket);
...
void SkVideoEncoder::endRecording() {
    (void)this->sendFrame(nullptr);

    /* add sequence end code to have a real MPEG file */
    const uint8_t endcode[] = { 0, 0, 1, 0xb7 };
    fWStream->write(endcode, sizeof(endcode));
