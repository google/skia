...
std::unique_ptr<SkBmpRustCodec> SkBmpRustCodec::MakeFromStream(std::unique_ptr<SkStream> stream,
    ...
    // Read metadata explicitly. In streaming mode it might not be available yet
    // if the stream lacks enough data (mapped to kIncompleteInput so the caller
    // can retry). ICO streams are fully buffered, so this resolves immediately.
    // ICO uses ICO-aware parsing: no file header, halved height, alpha channel.
    if (!reader->metadata_loaded()) {
        rust_bmp::DecodingResult metadataResult = (type == StreamType::kICO)
        ...
        // If metadata reading returned Success, metadata must be loaded.
        SkASSERT_RELEASE(reader->metadata_loaded());
    }

    auto codec = MakeFromReader(std::move(stream), std::move(reader), type, result);
    return codec;
}
