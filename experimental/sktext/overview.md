...
Native (desktop, mobile) frameworks (e.g. CoreGraphics, Direct2D, Skia) these all share a common foundation
- transforms and clipping
- colors, gradients, patterns
- blending and colorspaces

However, one primitive in particular stands out: **Text**. It is often drawn with the same foundational elements (transforms, colors, etc.) but it has another component unrelated to actual drawing, a 'processing' step that is surprisingly complex and (potentially) expensive: **Shaping**. The native frameworks mentioned above recognized this, and offer a suite of APIs just around this processing:
- CoreGraphics --> CoreText
- Direct2D/X --> DirectWrite
- Skia --> SkShaper

This proposal embraces this pattern, and proposes to expose this functionality to the Web.
...
Finding what glyphs, in what order and positions, are needed to represent a set of letters, is the heart of **Shaping**, and it is this process and resulting typefaces + positioned glyphs that we propose exposing to Web Apps.

## Summary

We posit that drawing *internationally correct* Text is critical to most Web Apps, and that it is both complex to get correct, and can be computationally expensive. We propose exposing this process to frameworks and text-heavy apps, providing them with correct results that can be efficiently drawn / animated.
- correct : the processing (shaping) logic to be performed by the Browser
- efficiently drawn : the returned results (typefaces, glyphs) can be drawn very fast

The core [Shaping APIs](shaping.md) are detailed here.

Assocated [Canvas2D extensions](canvas2d.md) are detailed here.
