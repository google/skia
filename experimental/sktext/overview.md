...
- rectangles, lines
- complex "paths" containing (multiple) contours, each containing lines and curves
- images (often scaled, transformed, filtered)
- text (also scalable and transformable)
- ...

In most native frameworks (e.g. CoreGraphics, Direct2D, Skia, Cairo) these all share a common foundation
- transforms (matrices)
- clipping
- colors, gradients
- blending
...
However, one primitive in particular stands out: **Text**. It is often drawn with the same foundational elements (transforms, colors, etc.) but it has another component unrelated to actual drawing, a 'processing' step that is surprisingly complex and (potentially) expensive: **Shaping**.
