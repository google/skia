...
public:
    TextFormatStyle(TextAlign textAlign)
...
public:
    ...
    Processor(const SkString& text)
        : fText(text)
        , fTextFormatStyle(TextAlign::kLeft)
        , fTextFontStyle(TextDirection::kLtr, SkFontMgr::RefDefault())
        , fUnicode(nullptr) {}
    ...
    using DecorBlockVisitor = std::function<SkScalar(const TextRun* run, Range glyphRange, SkSize offset, const SkPaint* foreground)>;
...
private:
    ...
    SkTArray<DecorBlock, true> fDecorBlocks;
