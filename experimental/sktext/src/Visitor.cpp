...
void Visitor::visit(Processor* processor) {

    SkPoint offset = SkPoint::Make(0 , 0);
    for (auto& line : processor->fLines) {
        offset.fX = 0;
        ...
        for (auto& runIndex : line.fRunsInVisualOrder) {
            auto& run = processor->fRuns[runIndex];
            ...
            for (size_t i = startGlyph; i <= endGlyph; ++i) {
                positions[i] = run.fPositions[i] + offset + SkPoint::Make(0, line.fTextMetrics.baseline());
            }
...
void Visitor::visit(Processor* processor, SkSpan<DecorBlock> decorBlocks) {
...
void SkiaVisitor::onBeginLine(const LineInfo&) { }
void SkiaVisitor::onGlyphRun(const RunInfo& runInfo, const DecorBlock& decorBlock) {
    SkTextBlobBuilder builder;
    const auto& blobBuffer = builder.allocRunPos(SkFont(runInfo.typeface, runInfo.size) , SkToInt(runInfo.glyphs.size()));
    sk_careful_memcpy(blobBuffer.glyphs, runInfo.glyphs.data(), runInfo.glyphs.size() * sizeof(SkGlyphID));
    sk_careful_memcpy(blobBuffer.points(), runInfo.positions.data(),runInfo.positions.size() * sizeof(SkPoint));
    fCanvas->drawTextBlob(builder.make(), fXY.fX, fXY.fY, *decorBlock.fForegroundColor);
}

void SkiaVisitor::onPlaceholderRun(const PlaceholderInfo&) { }
void SkiaVisitor::onEndLine(const LineInfo&) { }
