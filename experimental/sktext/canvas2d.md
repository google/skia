...
=============
[Shaped Text](shaper.md) is a proposal for exposing the Browser's text shaping engine. It takes in
...
results are represented in the glyphs, positions, and [Font](shaper.md) parameters.

[Font](shaper.md) is far more specific in this extension that the existing context.font attribute. In
...
[Shaped Text](shaper.md) introduced the Font interface, but for shaping, it only needed to specify
...
```WebIDL
interface Font {
    ...     // see [Shaped Text](shaper.md) for descriptive attributes
