...
Many frames, each with
  A frame header with
    ...
    A number of bytes after the frame header to read containging the hardware buffer skia was asked to draw into for this frame. (currently unused and always zero)
...
`SkMultiSkpWriter` is meant to be instantiated at the beginning of an animation capture (in Android) and provided with a filename to write to.
