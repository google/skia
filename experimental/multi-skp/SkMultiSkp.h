...
private:
    // Constructor is private and only used in factory method.
    SkMultiSkpWriter(int frames, std::unique_ptr<SkFILEWStream> stream);

    // The main function of the file writing thread
    void writingFn();

    // Pop one item from the queue, waiting if the queue is empty.
    std::unique_ptr<SkPictureRecorder> tryPop();
...
// A class to read a multi skp file
class SkMultiSkpReader : SkNoncopyable {
public:
    SkMultiSkpReader();
    ~SkMultiSkpReader();

    static std::unique_ptr<SkMultiSkpReader> Make();
