...
sk_sp<sksg::RenderNode> AttachOpacity(const json::ValueRef& jtransform, AttachContext* ctx,
    ...
    if (opacity.isObject() &&
        !opacity["a"].asDefault(true) &&
        opacity["k"].asDefault<int>(-1) == 100) {
...
sk_sp<sksg::GeometryNode> AttachPolystarGeometry(const json::ValueRef& jstar, AttachContext* ctx) {
    ...
    const auto type = jstar["sy"].asDefault<int>(0) - 1;
...
sk_sp<sksg::Gradient> AttachGradient(const json::ValueRef& obj, AttachContext* ctx) {
    ...
    const auto stopCount = stops["p"].asDefault<int>(-1);
    if (stopCount < 0)
        return nullptr;
...
sk_sp<sksg::PaintNode> AttachStroke(const json::ValueRef& jstroke, AttachContext* ctx,
    ...
    stroke_node->setStrokeMiter(jstroke["ml"].asDefault(4.0f));
    ...
    stroke_node->setStrokeJoin(gJoins[SkTPin<int>(jstroke["lj"].asDefault<int>(1) - 1,
    ...
    stroke_node->setStrokeCap(gCaps[SkTPin<int>(jstroke["lc"].asDefault<int>(1) - 1,
...
std::vector<sk_sp<sksg::GeometryNode>> AttachMergeGeometryEffect(
    ...
    const auto mode = gModes[SkTPin<int>(jmerge["mm"].asDefault(1) - 1,
...
std::vector<sk_sp<sksg::GeometryNode>> AttachTrimGeometryEffect(
    ...
    const auto mode = gModes[SkTPin<int>(jtrim["m"].asDefault(1) - 1,
                                         0, SK_ARRAY_COUNT(gModes) - 1)];
...
const ShapeInfo* FindShapeInfo(const json::ValueRef& shape) {
    ...
    const auto type = shape["ty"].asDefault(SkString());
    if (type.isEmpty())
        return nullptr;
...
sk_sp<sksg::RenderNode> AttachCompLayer(const json::ValueRef& jlayer, AttachContext* ctx,
    ...
    SkString refId;
    if (!jlayer["refId"].as(&refId) || refId.isEmpty()) {
        LOG("!! Comp layer missing refId\n");
        return nullptr;
    }

    const auto start_time = jlayer["st"].asDefault(0.0f),
             stretch_time = jlayer["sr"].asDefault(1.0f);
...
sk_sp<sksg::RenderNode> AttachSolidLayer(const json::ValueRef& jlayer, AttachContext*,
    ...
    const auto size = SkSize::Make(jlayer["sw"].asDefault(0.0f),
                                   jlayer["sh"].asDefault(0.0f));
    const auto hex = jlayer["sc"].asDefault(SkString());
    uint32_t c;
    if (size.isEmpty() ||
        !hex.startsWith("#") ||
...
sk_sp<sksg::RenderNode> AttachImageAsset(const json::ValueRef& jimage, AttachContext* ctx) {
    ...
    const auto name = jimage["p"].asDefault(SkString()),
               path = jimage["u"].asDefault(SkString());
    if (name.isEmpty())
        return nullptr;
...
sk_sp<sksg::RenderNode> AttachImageLayer(const json::ValueRef& layer, AttachContext* ctx,
    ...
    SkString refId;
    if (!layer["refId"].as(&refId) || refId.isEmpty()) {
...
struct AttachLayerContext {
    ...
    sk_sp<sksg::Matrix> AttachParentLayerMatrix(const json::ValueRef& jlayer) {
        ...
        const auto parent_index = jlayer["parent"].asDefault<int>(-1);
        if (parent_index < 0)
            return nullptr;
        ...
        fLayerList.forEach([&](const json::ValueRef& l) {
            if (!l.isObject()) {
                return true;
            }

            if (l["ind"].asDefault<int>(-1) == parent_index) {
    ...
    sk_sp<sksg::Matrix> AttachLayerMatrix(const json::ValueRef& jlayer) {
        ...
        const auto layer_index = jlayer["ind"].asDefault<int>(-1);
        if (layer_index < 0)
            return nullptr;
...
sk_sp<sksg::RenderNode> AttachMask(const json::ValueRef& jmask,
    ...
    jmask.forEach([&](const json::ValueRef& m) {
        if (!m.isObject())
            return true;
        ...
        mask_path->setFillType(m["inv"].asDefault(false)
...
sk_sp<sksg::RenderNode> AttachLayer(const json::ValueRef& jlayer, AttachLayerContext* layerCtx) {
    ...
    int type = jlayer["ty"].asDefault<int>(-1);
    ...
    float w = 0, h = 0;
    if (jlayer["w"].as(&w) && jlayer["h"].as(&h)) {
    ...
    const auto        in = jlayer["ip"].asDefault(0.0f),
                     out = jlayer["op"].asDefault(in);
    ...
    if (layerCtx->fCurrentMatte) {
        ...
        const auto matteType = jlayer["tt"].asDefault<int>(1) - 1;
...
sk_sp<sksg::RenderNode> AttachComposition(const json::ValueRef& comp, AttachContext* ctx) {
    ...
    jlayers.forEach([&](const json::ValueRef& l) {
...
sk_sp<Animation> Animation::Make(SkStream* stream, const ResourceProvider& res, Stats* stats) {
    ...
    Json::Value jdocument = json::ParseStream(stream);
    ...
    const json::ValueRef json(jdocument);

    const auto version = json["v"].asDefault(SkString());
    const auto size    = SkSize::Make(json["w"].asDefault(0.0f),
                                      json["h"].asDefault(0.0f));
    const auto fps     = json["fr"].asDefault(-1.0f);
...
Animation::Animation(const ResourceProvider& resources,
    ...
    , fSize(size)
    , fFrameRate(fps)
    , fInPoint(json["ip"].asDefault(0.0f))
    , fOutPoint(SkTMax(json["op"].asDefault(SK_ScalarMax), fInPoint)) {

    AssetMap assets;
    json["assets"].forEach([&](const json::ValueRef& asset) {
        if (asset.isObject()) {
            assets.set(asset["id"].asDefault(SkString()), asset);
        }
        return true;
    });
