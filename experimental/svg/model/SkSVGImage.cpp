...
sk_sp<SkImage> ImageFromBase64Encoded(const char* str) {
  ...
  if (!strncmp(str, kDataURIImagePrefix, SK_ARRAY_COUNT(kDataURIImagePrefix) - 1)) {
      const char* encoding_start = strstr(str + SK_ARRAY_COUNT(kDataURIImagePrefix) - 1,
                                          kDataURIEncodingStr);
      if (encoding_start) {
          ...
          if (result == SkBase64::kNoError) {
            ...
            auto data = SkData::MakeWithProc(b64.getData(), b64.getDataSize(),
                        [](const void* ptr, void*) {
...
SkRect SkSVGImage::resolve(const SkSVGLengthContext& lctx) const {
...
void SkSVGImage::onRender(const SkSVGRenderContext& ctx) const {
  if (fImage) {
    ...
    ctx.canvas()->drawImageRect(fImage, this->resolve(ctx.lengthContext()), nullptr);
  }
}

SkPath SkSVGImage::onAsPath(const SkSVGRenderContext& ctx) const {
  SkPath path;
  path.addRect(this->resolve(ctx.lengthContext()));
