...
void SkSVGImage::onRender(const SkSVGRenderContext& ctx) const {
  if (fImage) {
    const SkRect rect = SkRect::MakeXYWH(fX.value(), fY.value(), fWidth.value(),
                                         fHeight.value());
    ctx.canvas()->drawImageRect(fImage, rect, nullptr);
  }
}

SkPath SkSVGImage::onAsPath(const SkSVGRenderContext& ctx) const {
  const SkRect rect =
      SkRect::MakeXYWH(fX.value(), fY.value(), fWidth.value(), fHeight.value());
  SkPath path;
  path.addRect(rect);
  return path;
}
