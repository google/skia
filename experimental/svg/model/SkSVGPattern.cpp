...
void SkSVGPattern::setWidth(const SkSVGLength& w) {
    fAttributes.width.set(w);
}

void SkSVGPattern::setHeight(const SkSVGLength& h) {
    fAttributes.height.set(h);
}
...
void SkSVGPattern::setPatternTransform(const SkSVGTransformType& patternTransform) {
    fAttributes.patternTransform.set(patternTransform);
}
...
template <typename T>
static inline void inheritIfNeeded(const SkTLazy<T>& src, SkTLazy<T>& dst) {
    if (!dst.isValid()) {
        dst = src;
    }
}
...
const SkSVGPattern* SkSVGPattern::resolveHref(const SkSVGRenderContext& ctx,
    ...
    const SkSVGPattern *currentNode = this,
                       *contentNode = this;
    do {
        inheritIfNeeded(currentNode->fAttributes.x               , attrs->x);
        inheritIfNeeded(currentNode->fAttributes.y               , attrs->y);
        inheritIfNeeded(currentNode->fAttributes.width           , attrs->width);
        inheritIfNeeded(currentNode->fAttributes.height          , attrs->height);
        inheritIfNeeded(currentNode->fAttributes.patternTransform, attrs->patternTransform);
...
bool SkSVGPattern::onAsPaint(const SkSVGRenderContext& ctx, SkPaint* paint) const {
    ...
    const auto tile = ctx.lengthContext().resolveRect(
            attrs.x.isValid()      ? *attrs.x.get()      : SkSVGLength(0),
            attrs.y.isValid()      ? *attrs.y.get()      : SkSVGLength(0),
            attrs.width.isValid()  ? *attrs.width.get()  : SkSVGLength(0),
            attrs.height.isValid() ? *attrs.height.get() : SkSVGLength(0));

    if (tile.isEmpty()) {
        return false;
    }

    const SkMatrix* patternTransform = attrs.patternTransform.isValid()
            ? &attrs.patternTransform.get()->value()
            : nullptr;
    ...
    sk_sp<SkShader> patternShader = SkShader::MakePictureShader(recorder.finishRecordingAsPicture(),
