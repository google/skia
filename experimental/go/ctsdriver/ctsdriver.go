...
func RecordTestResult(testName string, img *Image) (string, error) {
	...
	if err := fileutil.EnsureDirPathExists(fName); err != nil {
		return fmt.Sprintf("Unable to create output dir: %s", err), err
	}

	outFile, err := os.Create(fName)
	if err != nil {
		return fmt.Sprintf("Unable to create output file: %s", err), err
	}
	...
	if err != nil {
		return fmt.Sprintf("Unable to write png: %s", err), err
	}

	_, err = upload.UploadFile(fName, "http://104.154.66.138/upload")
	if err != nil {
		return fmt.Sprintf("Unable to upload file %s. Got error: %s", fName, err), nil
	}

	return "", nil
}

func oldRecordTestResult(testName string, img *Image) (string, error) {
	// Convert the image to an NRGBA images.
	// var resultImg *image.NRGBA = nil
