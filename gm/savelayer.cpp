...
static void draw_cell(SkCanvas* canvas, sk_sp<SkTextBlob> blob, SkColor c, SkScalar w, SkScalar h) {
    ...
    int sc0 = canvas->getSaveCount();
    int sc1 = canvas->saveBehind(&r);
    int sc2 = canvas->getSaveCount();
    SkASSERT(sc0 == sc1);
    SkASSERT(sc0 + 1 == sc2);

    // draw the forground (including over the 'behind' section
