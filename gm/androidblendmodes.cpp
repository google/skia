...
protected:
    ...
    sk_sp<SkCustomBlend> getRuntimeBlend(SkBlendMode mode) {
        if (sk_sp<SkCustomBlend> effect = fBlendMap[mode]) {
            return effect;
        }
...
half4 blend_src_in(half4 src, half4 dst) {
    return true /*sk_Caps.inBlendModesFailRandomlyForAllZeroVec*/
            ? (src == half4(0) ? half4(0) : src*dst.a)
            : src*dst.a;
}
...
half4 main(half4 src, half4 dst) {
    return %s(src, dst);
        ...
        SkRuntimeEffect::Result result = SkRuntimeEffect::MakeForBlend(std::move(code));
        SkASSERT(result.effect);
        return (fBlendMap[mode] = result.effect->makeBlend(/*uniforms=*/nullptr));
    }
    ...
    void drawTile(SkCanvas* canvas, int xOffset, int yOffset, SkBlendMode mode) {
        ...
        p.setCustomBlend(this->getRuntimeBlend(mode));
...
private:
    std::unordered_map<SkBlendMode, sk_sp<SkCustomBlend>> fBlendMap;
