...
DEF_SIMPLE_GM(bicubic, canvas, 300, 320) {
    ...
    for (auto we : {SkImage::BicubicWeights::kCentripetalCatmulRom,
                    SkImage::BicubicWeights::kMitchellNetravali}) {
        paint.setShader(img->makeBicubicShader(SkTileMode::kClamp, SkTileMode::kClamp,
                                               SkImage::GetBicubicWeights(we)));
