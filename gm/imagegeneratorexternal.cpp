...
protected:
    ...
    onAccessScaledPixels(const SkRect& src, const SkMatrix& matrix, SkFilterQuality) override {
        // Not strictly needed for this immutable class.
        SkAutoExclusive lock(fMutex);

        SkSize scaleSize;
        if (!matrix.decomposeScale(&scaleSize, nullptr)) {
            return std::make_tuple(nullptr, SkRect::MakeEmpty(), kNone_SkFilterQuality);
        }
        ...
        sk_sp<SkImage> img = fMips[SkTPin(lvl, 0, fMips.count())];
        ...
        SkRect adjustedSrc;
        srcMap.mapRect(&adjustedSrc, SkRect::MakeWH(src.width(), src.height()));

        return std::make_tuple(std::move(img), adjustedSrc, kLow_SkFilterQuality);
    }
