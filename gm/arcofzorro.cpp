...
protected:
    ...
    void check(uint32_t val, uint32_t floorPow2, uint32_t ceilPow2, uint32_t expectedApprox,
               bool valIsPow2) {
        SkASSERT(GrIsPow2(val) == valIsPow2);

        uint32_t low = GrPrevPow2(val);
        SkASSERT(GrIsPow2(low));

        uint32_t high = GrNextPow2(val);
        SkASSERT(GrIsPow2(high));

        uint32_t approx = make_approx(val);
        SkASSERT(0 == (approx % 2));
