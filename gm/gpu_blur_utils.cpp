...
DEF_SIMPLE_GPU_GM(gpu_blur_utils, ctx, rtc, canvas, 620, 155) {
    ...
    if (surf) {
        ...
        paint.setColor(/*orange*/0xFFfc9d03);
        //surf->getCanvas()->drawLine({1.f*w/8.f, 0.f}, {1.f*h/8.f, h}, paint);
        paint.setColor(/*sea green*/0xff71EEB8);
    ...
    auto blur = [&](SkIRect dstB, SkIRect srcB, SkTileMode mode) -> GrSurfaceProxyView {
        const SkScalar sigmaX = 8.f;
        const SkScalar sigmaY = 8.f;
        auto resultRTC =
    ...
    for (const auto& dstRect : dstRects) {
        for (int t = 0; t < kSkTileModeCount; ++t) {
            ...
            if (mode != SkTileMode::kRepeat) continue;
            ...
            // Draw the image in the tile mode faded as a reference before drawing the blur on top
            {
                auto fp = GrTextureEffect::Make(src, kPremul_SkAlphaType, SkMatrix::I(), sampler, caps);
                GrPaint paint;
            ...
            auto blurView = blur(dstRect, srcRect, mode);
            {
                auto fp = GrTextureEffect::Make(blurView, kPremul_SkAlphaType, SkMatrix::I(), sampler, caps);
                GrPaint paint;
                ...
                rtc->fillRectToRect(GrNoClip(), std::move(paint), GrAA::kNo, m, SkRect::Make(dstRect),
