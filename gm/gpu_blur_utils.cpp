...
static void run(GrContext* ctx, GrRenderTargetContext* rtc, bool subsetSrc, bool ref) {
    ...
    const std::vector<SkIRect> dstRectSets[] = {
            {
                    SkIRect::MakeSize(srcRect.size()).makeOutset(srcW/5, srcH/5)
            },

            {
                    SkIRect::MakeXYWH(0,  3*srcH/4, srcW, srcH),
                    SkIRect::MakeXYWH(0, -3*srcH/4, srcW, srcH)
            },

            {
                    SkIRect::MakeXYWH(   0,  srcH, srcW, srcH),
                    SkIRect::MakeXYWH(srcW,     0, srcW, srcH),
                    SkIRect::MakeXYWH(   0, -srcH, srcW, srcH),
                    SkIRect::MakeXYWH(-srcW,    0, srcW, srcH)
            },

            {
                    SkIRect::MakeXYWH(-6.f*srcW/8.f, -7.f*srcH/8.f,  4.f*srcW/8.f,  5.f*srcH/2.f),
    ...
    for (const auto& dstRects : dstRectSets) {
        for (int t = 0; t < kSkTileModeCount; ++t) {
            ...
            for (const auto& dstRect : dstRects) {
                ...
                if (ref) {
                    ...
                    blurDstRect = dstRect.makeOffset(-refRect.topLeft() /*+ srcDstOffset*/);
