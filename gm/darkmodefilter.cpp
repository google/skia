...
static void draw_label(SkCanvas* canvas, const SkDarkModeConfig& config) {
    char labelBuffer[256];
    const char* invertStr =
            (config.fInvertStyle == InvertStyle::kInvertBrightness
                     ? "InvBright"
                     : (config.fInvertStyle == InvertStyle::kInvertLightness ? "InvLight"
                                                                             : "NoInvert"));
...
static void draw_scene(SkCanvas* canvas, const SkDarkModeConfig& config) {
    SkRect bounds = SkRect::MakeLTRB(0.0f, 0.0f, 1.0f, 1.0f);
    SkPaint xferPaint;
    xferPaint.setColorFilter(SkDarkModeFilter::Make(config));
...
class DarkModeFilterGM : public skiagm::GM {
public:
    DarkModeFilterGM() {
...
protected:
    SkString onShortName() override { return SkString("darkmodefilter"); }

    SkISize onISize() override { return SkISize::Make(800, 420); }

    void onDraw(SkCanvas* canvas) override {
        SkDarkModeConfig configs[] = {
...
DEF_GM(return new DarkModeFilterGM;)
