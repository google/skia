...
static bool draw_diff(SkCanvas* canvas, SkImage* imgA, SkImage* imgB) {
    ...
    if (maxDiff == 0) {
        // Green outline to make it obvious that things are correct when triaging
        outline.setColor(0xFF007F00);
    } else {
        ...
        *highlight.getAddr32(dX, SkTMin(dY + 1, h - 1)) = yellow;
        ...
        // Use red outlines to call attention to failing tests
        outline.setColor(0xFF7F0000);
...
DEF_SIMPLE_GM_BG(shapes_as_paths, canvas, 275, 310, SK_ColorBLACK) {
    ...
    SkMatrix mScale = SkMatrix::MakeScale(0.75f);
    SkMatrix mFlipX;
    ...
    const SkMatrix* kMatrices[] = { &mI, &mRot, &mScale, &mFlipX, &mFlipY, &mPersp, };
