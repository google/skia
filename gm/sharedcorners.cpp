...
protected:
    ...
    SkISize onISize() override {
        return SkISize::Make(8 * (kBoxSize + kPadSize) + kPadSize,
                             6 * (kBoxSize + kPadSize) + kPadSize);
    }
    ...
    void drawTriangleBoxes(SkCanvas* canvas, const std::vector<SkPoint>& points,
        ...
        rot.setRotate(-45 - 66.67, path.getBounds().centerX(), path.getBounds().centerY());
        path.transform(rot);
        this->drawRow(canvas, path);
        canvas->translate(0, kBoxSize + kPadSize);
    }
    ...
    void drawRow(SkCanvas* canvas, const SkPath& path) {
        ...
        for (SkPoint jitter : {SkPoint{0, 0}, {.5, .5}, {2/3.f, 1/3.f}}) {
            {
                SkAutoCanvasRestore acr(canvas, true);
