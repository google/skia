...
// A GpuGM replaces the onDraw method with one that also accepts GPU objects alongside the SkCanvas.
// Its onDraw is only invoked on GPU configs; on non-GPU configs it will automatically draw a
// GPU-only message and abort.
...
// onDraw method (via DEF_SIMPLE_.*_GM.* macros), and don't store state or otherwise require more
// GM functionality. It can inherit from GM and GpuGM interchangeably.
