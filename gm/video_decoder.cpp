...
static void split_into_yuv(const SkImage* img, const SkPixmap dst[3]) {
    float m[20];
    SkMakeRGB2YUV(m, kRec709_SkYUVColorSpace);
...
protected:
    ...
    void onDraw(SkCanvas* canvas) override {
        ...
        auto img = SkImage::MakeFromYUVAPixmaps(gr, kRec709_SkYUVColorSpace, fPM, indices,
                                                fPM[0].info().dimensions(), kTopLeft_GrSurfaceOrigin,
                                                false, false, nullptr);
        canvas->drawImage(fOrig, 0, 0, nullptr);
        if (img) {
            canvas->drawImage(img, fOrig->width(), 0, nullptr);
        }

        canvas->translate(0, fOrig->height());

        canvas->drawImage(SkImage::MakeRasterCopy(fPM[0]), 0, 0, nullptr);
        canvas->drawImage(SkImage::MakeRasterCopy(fPM[1]), fPM[0].width(), 0, nullptr);
        canvas->drawImage(SkImage::MakeRasterCopy(fPM[2]), fPM[0].width() + fPM[1].width(), 0, nullptr);
