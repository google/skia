...
static sk_sp<SkImage> make_image(GrRecordingContext* context) {
    ...
    return imageHelper->refImage(context);
}
...
DEF_SIMPLE_GM_CAN_FAIL(yuv420_odd_dim, canvas, errMsg,
    ...
    if (!image) {
        return context->abandoned() ? skiagm::DrawResult::kOk : skiagm::DrawResult::kFail;
    }
