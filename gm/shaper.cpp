...
static void draw(SkCanvas* canvas, const char* name, const char* face) {
    sk_sp<SkData> data = GetResourceAsData(SkStringPrintf("text/%s.txt", name).c_str());
    if (!data) { return; }
    constexpr float kWidth = 504;
    sk_sp<SkFontMgr> fontManager = SkFontMgr::RefDefault();
    if (!fontManager) { SkDebugf("%s: no default font mugger?", name); return; }
    //SkFont font(nullptr, 18);
    //if (face) {
    //    if (0 == strcmp(face, "Noto Color Emoji")) {
    //         font = SkFont(fontManager->makeFromFile(
    //                     "/usr/share/fonts/truetype/noto/NotoColorEmoji.ttf"), 18);
    //    } else {
    //        font = SkFont(fontManager->legacyMakeTypeface(face, SkFontStyle()), 18);
    //    }
    //}
    SkFont font(face ? fontManager->legacyMakeTypeface(face, SkFontStyle()) : nullptr, 18);
    SkPaint paint;
    float x = 4;
    float y = 4 + font.getSpacing();
    auto title = SkStringPrintf("%s - %s", name, face ? face : "{}");
    y += draw_shaped(canvas, title.c_str(), title.size(), x, y, font, paint, kWidth) +
         font.getSpacing();
    readlines(data->data(), data->size(), [&](const char* text, size_t len) {
        while (len > 0 && text[len-1] == '\n') { --len; }
        y += draw_shaped(canvas, text, len, x, y, font, paint, kWidth) + font.getSpacing();
    });
}

DEF_SIMPLE_GM(shaper_emoji_color, c, 512, 512) { draw(c, "emoji", "Noto Color Emoji"); }

#define M(X) DEF_SIMPLE_GM(shaper_ ## X, c, 512, 512) { draw(c, #X, "Noto Sans"); }
M(arabic)
M(armenian)
M(balinese)
M(bengali)
