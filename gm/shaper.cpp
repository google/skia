...
static void draw(SkCanvas* canvas, const char str[], size_t len, SkScalar width,
                 SkFont srcFont, SkScalar x, SkScalar y, SkShaper* shaper) {
    ...
    shaper->shape(str, len, *font, *bidi, *script, *language, width, &builder);

    canvas->drawTextBlob(builder.makeBlob(), x, y, SkPaint());
}

DEF_SIMPLE_GM(shaper_0, canvas, 700, 500) {
    const char text[] =
    ...
    draw(canvas, text, strlen(text), 400, font, 20, 20, shaper.get());
}
