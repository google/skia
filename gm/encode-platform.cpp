...
protected:
    ...
    void onDraw(SkCanvas* canvas) override {
        ...
        if (!GetResourceAsBitmap("images/mandrill_256.png", &opaqueBm)) {
            skiagm::GM::DrawFailureMessage(canvas, "Could not load images/mandrill_256.png.png. "
                                                   "Did you forget to set the resourcePath?");
            return;
        }
        SkBitmap tmp;
        if (!GetResourceAsBitmap("images/yellow_rose.png", &tmp)) {
            skiagm::GM::DrawFailureMessage(canvas, "Could not load images/yellow_rose.png. "
                                                   "Did you forget to set the resourcePath?");
            return;
        }
