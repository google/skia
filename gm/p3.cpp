...
DEF_SIMPLE_GM(p3, canvas, 320, 240) {
    ...
    if (!canvas->readPixels(bm, 10,10)) {
        canvas->drawString("can't readPixels() on this canvas :(", 100,20, SkPaint{});
        mark_good(canvas);
        return;
    }
    ...
    for (auto l : lines) {
        canvas->drawString(l.label,               80,20, SkPaint{});
        canvas->drawString(fmt(l.color).c_str(), 140,20, SkPaint{});
