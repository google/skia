...
const ShapeTypeRec gShapeTypeRecs[] = {
    ...
    { kFill_ShapeType,      "??", []() -> Shape* { return new FillShape; }},
    ...
    { kTrim_ShapeType,      "tm", []() -> Shape* { return new TrimShape; }},
...
Shape* Shape::Make(const Json::Value& v) {
    Shape* shape = nullptr;
    if (const char* type = v["ty"].asCString()) {
...
DEF_SIMPLE_GM(json, canvas, 500, 600) {
    auto data = GetResourceAsData("menu-home.json");
//    data = SkData::MakeFromFileName("/skia/tmp.json");
