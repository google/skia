...
DEF_SIMPLE_GM(combinemaskfilter, canvas, 512, 512) {
    const SkRect r = { 0, 0, 100, 100 };

    SkPath pathA;
    pathA.moveTo(r.fLeft, r.fTop);
    pathA.lineTo(r.fRight, r.fBottom);
    pathA.lineTo(r.fLeft, r.fBottom);
    auto mfA = make_path_mf(pathA, 1 * 0xFF / 3);

    SkPath pathB;
    pathB.moveTo(r.fLeft, r.fBottom);
    pathB.lineTo(r.fRight, r.fTop);
