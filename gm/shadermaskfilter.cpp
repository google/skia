...
#include "Resources.h"
DEF_SIMPLE_GM(shadermaskfilter_image, canvas, 560, 355) {
    ...
    sk_sp<SkMaskFilter> array[] = { nullptr , blurmf, gradmf };
    for (sk_sp<SkMaskFilter> mf : array) {
        draw_masked_image(canvas, image.get(), 10, 10 + image->height() + 10.f, mask.get(), mf);
