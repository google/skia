...
protected:
    ...
    void drawShape(SkCanvas* canvas, SkPaint* paint, ShapeType type) {
        const SkRect kRect = SkRect::MakeXYWH(SkIntToScalar(0), SkIntToScalar(0),
                                              SkIntToScalar(75), SkIntToScalar(85));
        switch (type) {
            ...
            case kConcavePath_ShapeType:
                if (fConcavePath.isEmpty()) {
                    SkPoint points[5] = {{50, 0}};
                    SkMatrix rot;
                    rot.setRotate(SkIntToScalar(360) / 5, 50, 70);
                    for (int i = 1; i < 5; ++i) {
            ...
            case kText_ShapeType: {
                ...
                canvas->drawText(text, strlen(text), 0, 100, *paint);
            }
            default:
                break;
        }
    ...
    void onDraw(SkCanvas* canvas) override {
        ...
        for (int i = 0; i < kNumShapeTypes; i++) {
            ...
            for (int r = 0; r <= 15; r++) {
                SkPaint p;
                p.setAntiAlias(true);
                p.setColor(GetColor(&colorRandom, 0x80));
