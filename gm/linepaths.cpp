...
static void draw(SkCanvas* canvas, bool doClose) {
        ...
        skiagm::DrawShapedString(canvas, title, 20.0f, 20.0f, font, titlePaint);
        ...
        for (size_t cap = 0; cap < SK_ARRAY_COUNT(gCaps); ++cap) {
            ...
            for (size_t fill = 0; fill < SK_ARRAY_COUNT(gFills); ++fill) {
                ...
                for (size_t style = 0; style < SK_ARRAY_COUNT(gStyles); ++style) {
                    ...
                    font.setSize(10);
                    skiagm::DrawShapedString(canvas, gStyles[style].fName, 0, rect.height() + 12.0f,
                                             font, labelPaint);
                    skiagm::DrawShapedString(canvas, gFills[fill].fName, 0, rect.height() + 24.0f,
                                             font, labelPaint);
                    skiagm::DrawShapedString(canvas, gCaps[cap].fName, 0, rect.height() + 36.0f,
                                             font, labelPaint);
                }
                canvas->restore();
            }
