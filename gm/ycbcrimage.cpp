...
public:
    static void Release(void* releaseContext) {
        ...
        ycbcrContext->fYCbCrHelper.destroyBackendTexture(ycbcrContext->fContext);
        delete ycbcrContext;
    }

    YCbCrCallbackContext(GrContext* context) : fContext(context) {}

    skiagm::DrawResult allocVkMemory(int width, int height, SkString* errorMsg) {
        if (!VkYcbcrSamplerHelper::IsYCbCrSupported(fContext)) {
            *errorMsg = "YCbCr sampling not supported.";
            return skiagm::DrawResult::kSkip;
        }

        if (!fYCbCrHelper.createBackendTexture(fContext, width, height)) {
...
protected:
    ...
    DrawResult onHailMary(GrContext* context, SkString* errorMsg) override {
