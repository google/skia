...
protected:
    ...
    DrawResult onDraw(GrContext* context, GrRenderTargetContext*, SkCanvas* canvas,
        ...
        if (context->backend() != GrBackendApi::kVulkan) {
            *errorMsg = "This GM requires an Vulkan context.";
            return DrawResult::kSkip;
        }

        VkYcbcrSamplerHelper helper;
        if (!helper.init(sk_ref_sp<GrContext>(context))) {
            *errorMsg = "Danger, Will Robinson!";
            return DrawResult::kSkip;
        }

        sk_sp<SkImage> ycbcrImage = helper.createI420Image(kImageSize, kImageSize);
        if (!ycbcrImage) {
            *errorMsg = "Could not create YCbCr image.";
