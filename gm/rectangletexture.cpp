...
protected:
    ...
    sk_sp<SkImage> createRectangleTextureImg(GrContext *context, GrSurfaceOrigin origin,
        ...
        GR_GL_CALL(gl, TexParameteri(kTarget, GR_GL_TEXTURE_MAG_FILTER,
                                     GR_GL_NEAREST));
        GR_GL_CALL(gl, TexParameteri(kTarget, GR_GL_TEXTURE_MIN_FILTER,
                                     GR_GL_NEAREST));
        GR_GL_CALL(gl, TexParameteri(kTarget, GR_GL_TEXTURE_WRAP_S,
                                     GR_GL_CLAMP_TO_EDGE));
        GR_GL_CALL(gl, TexParameteri(kTarget, GR_GL_TEXTURE_WRAP_T,
                                     GR_GL_CLAMP_TO_EDGE));
        ...
        GR_GL_CALL(gl, TexImage2D(kTarget, 0, GR_GL_RGBA, width, height, 0,
    ...
    void onDraw(SkCanvas* canvas) override {
        ...
        sk_sp<SkImage> rectImgs[] = {
                ...
                this->createRectangleTextureImg(context, kBottomLeft_GrSurfaceOrigin, kWidth, kHeight, pixels),
        };
        ...
        for (size_t i = 1; i < SK_ARRAY_COUNT(rectImgs); ++i) {
            for (auto s : kScales) {
                canvas->save();
                canvas->scale(s, s);
