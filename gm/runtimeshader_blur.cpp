...
public:
    ...
    static constexpr float kMaxCrossFadeRadius = 50.0f;
    ...
    KawaseBlurFilter() {
        SkString blurString(R"(
            ...
            void main(float2 xy, inout half4 color) {
                ...
                color = half4(c.rgb * 0.2, 1.0);
            }
        )");

        SkString mixString(R"(
            in shader in_blur;
            in shader in_original;
            uniform float in_inverseScale;
            ...
            void main(float2 xy, inout half4 color) {
                ...
                float4 blurred = sample(in_blur, scaled_xy);
                float4 composition = sample(in_original, xy);
                color = half4(mix(composition, blurred, in_mix));
            }
        )");
        ...
        auto [mixEffect, error2] = SkRuntimeEffect::Make(mixString);
        if (!mixEffect) {
            SkDebugf("RuntimeShader error: %s\n", error2.c_str());
        }
    ...
    void draw(SkCanvas* canvas, sk_sp<SkImage> input, int blurRadius) {
        ...
        SkImageInfo info = SkImageInfo::MakeN32Premul((float)input->width() * kInputScale,
                                                      (float)input->height() * kInputScale);
        auto drawSurface = canvas->makeSurface(info);
        ...
        {
        SkRuntimeShaderBuilder builder(fBlurEffect);
        builder.child("input") = input->makeShader();
        builder.uniform("in_inverseScale") = kInverseInputScale;
        builder.uniform("in_blurOffset") = SkV2{stepX * kInverseInputScale, stepY * kInverseInputScale};

        SkPaint paint;
        paint.setShader(builder.makeShader(nullptr, false));
        drawSurface->getCanvas()->drawIRect(info.bounds(), paint);

        // DEBUG draw each of the stages
        canvas->save();
        canvas->drawImage(drawSurface->makeImageSnapshot(), input->width()/4, 0, nullptr);
        ...
        if (numberOfPasses > 1) {
            ...
            drawSurface = canvas->makeSurface(info);

            for (auto i = 1; i < numberOfPasses; i++) {
                const float stepScale = (float)i * kInputScale;

                SkRuntimeShaderBuilder builder(fBlurEffect);
                builder.child("input") = readSurface->makeImageSnapshot()->makeShader();
                builder.uniform("in_inverseScale") = 1.0f;
                builder.uniform("in_blurOffset") = SkV2{stepX * stepScale , stepY * stepScale};
                ...
                // DEBUG draw each of the stages
                canvas->drawImage(drawSurface->makeImageSnapshot(), 0, 0, nullptr);
        ...
        SkRuntimeShaderBuilder builder(fMixEffect);
        builder.child("in_blur") = lastDrawTarget->makeImageSnapshot()->makeShader();
        builder.child("in_original") = input->makeShader();
        builder.uniform("in_inverseScale") = kInputScale / 1.0f;
        builder.uniform("in_mix") = std::min(1.0f, (float)blurRadius / kMaxCrossFadeRadius);
...
public:
    KawaseBlurRT() {}
    SkString onShortName() override { return SkString("blur_kawase_rt"); }
