...
public:
    ...
    KawaseBlurFilter() {
        ...
        SkString mixString(R"(
            ...
            half4 main(float2 xy) {
                ...
                return half4(mix(composition, blurred, in_mix));
            }
        )");
    ...
    void draw(SkCanvas* canvas, sk_sp<SkImage> input, int blurRadius) {
        ...
        SkRuntimeShaderBuilder downsampleBuilder(fBlurEffect);
        downsampleBuilder.child("input") = input->makeShader();
        downsampleBuilder.uniform("in_inverseScale") = kInverseInputScale;
        downsampleBuilder.uniform("in_blurOffset") = SkV2{stepX * kInverseInputScale, stepY * kInverseInputScale};

        SkPaint paint;
        paint.setShader(downsampleBuilder.makeShader(nullptr, false));
        ...
        mixBuilder.child("in_original") = input->makeShader();
        mixBuilder.uniform("in_inverseScale") = kInputScale / 1.0f;
        mixBuilder.uniform("in_mix") = std::min(1.0f, (float)blurRadius / kMaxCrossFadeRadius);

        paint.setShader(mixBuilder.makeShader(nullptr, false));
