...
static void make_images() {
    for (int i = 1; i <= 8; ++i) {
        ...
        // We apply the inverse transformation indicated by origin to the PNG we generate. Our
        // bash script above will convert the PNG to a JPEG using magick, then modify the JPEG's tag
        // using exiftool (without modifying the stored JPEG data). When we load the resulting JPEG
        // and make an SkImage from it in the GM we should use the tag to undo this transformation.
