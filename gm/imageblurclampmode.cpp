...
void draw_image(SkCanvas* canvas, const sk_sp<SkImage> image, sk_sp<SkImageFilter> filter) {
...
protected:
    ...
    void onDraw(SkCanvas* canvas) override {
        ...
        // blur.
        const float sigma[] = { 0.6f, 3.0f, 8.0f, 20.0f };
        for (size_t i = 0; i < SK_ARRAY_COUNT(sigma); ++i) {
            canvas->save();
            sk_sp<SkImageFilter> filter(SkBlurImageFilter::Make(sigma[i], 0.0f, nullptr));
            draw_image(canvas, image, filter);
            canvas->translate(image->width() + 20, 0);

            filter = SkBlurImageFilter::Make(0.0f, sigma[i], nullptr);
            draw_image(canvas, image, filter);
            canvas->translate(image->width() + 20, 0);

            filter = SkBlurImageFilter::Make(sigma[i], sigma[i], nullptr);
            draw_image(canvas, image, filter);
            canvas->translate(image->width() + 20, 0);
