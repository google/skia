...
class DFTextPerspGM : public skiagm::GM {
public:
    DFTextPerspGM() { this->setBGColor(0xFFFFFFFF); }

protected:

    SkString onShortName() override {
        SkString name("dftext_persp");
    ...
    virtual void onDraw(SkCanvas* inputCanvas) override {
        ...
        for (auto twm : {TranslateWithMatrix::kNo, TranslateWithMatrix::kYes}) {
            for (auto pm : {PerspMode::kNone, PerspMode::kX, PerspMode::kY, PerspMode::kXY}) {
                for (auto scale : { 1.f, 1.75f }) {
                    for (int i = 0; i < fBlobs.count(); ++i) {
                        SkScalar h = fBlobs[i]->bounds().height() * scale;
                        this->drawBlob(canvas, fBlobs[i].get(), SK_ColorBLACK, x, y + h, scale, pm, twm);
                        x += fBlobs[i]->bounds().width() * scale + 20.f;
                        maxH = SkTMax(h, maxH);
                    }
                    x = 0;
                    y += maxH + 20.f;
...
private:
    ...
    void drawBlob(SkCanvas* canvas, SkTextBlob* blob, SkColor color, SkScalar x, SkScalar y,
                  SkScalar scale, PerspMode perspMode, TranslateWithMatrix translateWithMatrix) {
        x /= scale;
        y /= scale;
        canvas->save();
...
DEF_GM(return new DFTextPerspGM;)
