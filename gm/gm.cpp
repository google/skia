...
SimpleGM* SimpleGM::Create(
    ...
    auto proc = [drawProc](SkCanvas* canvas) -> const char* {
        drawProc(canvas);
        return kDrawComplete;
    };
    return new SimpleGM(name, std::move(proc), size, backgroundColor);
}
...
SimpleGM* SimpleGM::CreateForGPU(
    ...
    auto drawProc = [gpuDrawProc](SkCanvas* canvas) -> const char* {
