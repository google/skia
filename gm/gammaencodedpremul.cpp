...
protected:
    ...
    void onDraw(SkCanvas* canvas) override {
        for (int i = 0; i < kNumColors; i++) {
            ...
            uint32_t dstPremul[kAlphaMax];
            fXform->apply(SkColorSpaceXform::kRGBA_8888_ColorFormat, dstPremul,
                          SkColorSpaceXform::kBGRA_8888_ColorFormat, row, kAlphaMax,
                          kUnpremul_SkAlphaType);
            SkOpts::RGBA_to_rgbA(dstPremul, dstPremul, kAlphaMax);

            // Write the dst space premultiplied row to the canvas.
            SkImageInfo writeInfo = canvas->imageInfo().makeColorType(kRGBA_8888_SkColorType)
                                                       .makeWH(kAlphaMax, kStripeHeight);
            size_t rowBytes = kAlphaMax * sizeof(uint32_t);
            for (int j = 0; j < kStripeHeight; j++) {
                canvas->writePixels(writeInfo, dstPremul, rowBytes, 0, 2 * i * kStripeHeight + j);
            }

            // Premultiply, then transform the row to dst.
            uint32_t srcPremul[kAlphaMax];
            SkOpts::RGBA_to_rgbA(srcPremul, row, kAlphaMax);
            fXform->apply(SkColorSpaceXform::kRGBA_8888_ColorFormat, srcPremul,
                          SkColorSpaceXform::kBGRA_8888_ColorFormat, srcPremul, kAlphaMax,
                          kUnpremul_SkAlphaType);
            clamp_to_alpha(srcPremul, kAlphaMax);

            // Write the src space premultiplied row to the canvas.
            for (int j = 0; j < kStripeHeight; j++) {
                canvas->writePixels(writeInfo, srcPremul, rowBytes, 0,
                                    (2 * i + 1) * kStripeHeight + j);
            }
        }
    }
...
DEF_GM(return new GammaEncodedPremulGM(SkColorSpace::MakeSRGB(),
        SkColorSpace::MakeRGB(SkColorSpace::kSRGB_RenderTargetGamma, SkColorSpace::kSRGB_Gamut),
        "toLinear");)
