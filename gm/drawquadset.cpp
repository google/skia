...
static void draw_gradient_tiles(SkCanvas* canvas, bool alignGradients) {
    ...
    for (int i = 0; i < kRowCount; ++i) {
        for (int j = 0; j < kColCount; ++j) {
            ...
            canvas->experimental_DrawEdgeAARectV1(tile,
                    static_cast<SkCanvas::QuadAAFlags>(aa), paint);
...
static void draw_color_tiles(SkCanvas* canvas, bool multicolor) {
    ...
    for (int i = 0; i < kRowCount; ++i) {
        for (int j = 0; j < kColCount; ++j) {
            ...
            if (multicolor) {
                paint.setColor4f({(i + 1.f) / kRowCount, (j + 1.f) / kColCount, .4f, 1.f}, nullptr);
            } else {
                paint.setColor4f({.2f, .8f, .3f, 1.f}, nullptr);
            }
            ...
            canvas->experimental_DrawEdgeAARectV1(
                    tile, static_cast<SkCanvas::QuadAAFlags>(aa), paint);
        }
    }
}
