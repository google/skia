...
static sk_sp<SkColorFilter> MakeTintColorFilter(SkColor lo, SkColor hi) {
    ...
    return SkColorFilterMaker::MatrixRowMajor255(tint_matrix)
        ...
        canvas->translate(0, fTileSize.height() * 0.1f);
        {
            ...
            for (size_t i = 0; i < fTileCount; ++i) {
                paint.setColorFilter(
                    SkColorFilterMaker::Lerp(static_cast<float>(i) / (fTileCount - 1), cf0, cf1));
...
DEF_SIMPLE_GM(mixercolorfilter, canvas, 768, 512) {
    ...
    p.setColorFilter(SkColorFilterMaker::Mixer(mx, cf0, cf1));
    draw_rect(canvas, r, p,   0, 256);
    p.setColorFilter(SkColorFilterMaker::Mixer(mx, cf0, nullptr));
    draw_rect(canvas, r, p, 256, 256);
    p.setColorFilter(SkColorFilterMaker::Mixer(mx, nullptr, cf1));
