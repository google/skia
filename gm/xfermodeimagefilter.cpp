...
protected:
    ...
    void onDraw(SkCanvas* canvas) override {
        ...
        paint.setImageFilter(SkImageFilters::Xfermode(mode, cropped, background, nullptr));
        DrawClippedPaint(canvas, clipRect, paint, x, y);
        x += fBitmap.width() + MARGIN;
        if (x + fBitmap.width() > WIDTH) {
            x = 0;
        ...
        paint.setImageFilter(SkImageFilters::Xfermode(mode, background, cropped, nullptr));
        DrawClippedPaint(canvas, clipRect, paint, x, y);
        x += fBitmap.width() + MARGIN;
        if (x + fBitmap.width() > WIDTH) {
            x = 0;
        ...
        paint.setImageFilter(SkImageFilters::Xfermode(mode, background, cropped, &cropRectFull));
        DrawClippedPaint(canvas, clipRect, paint, x, y);
        x += fBitmap.width() + MARGIN;
        if (x + fBitmap.width() > WIDTH) {
            x = 0;
