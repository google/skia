...
static void draw_atlas(SkCanvas* canvas, const SkRect& r, sk_sp<SkImageFilter> imf) {
    // First, create an atlas image.
    SkImageInfo atlasInfo = SkImageInfo::MakeN32Premul(r.width(), r.height());
    sk_sp<SkSurface> atlasSurface(ToolUtils::makeSurface(canvas, atlasInfo));
    SkCanvas* atlasCanvas = atlasSurface->getCanvas();

    SkPaint atlasPaint;
    atlasPaint.setColor(SK_ColorBLUE);
    SkFont font(ToolUtils::create_portable_typeface(), r.height() * 0.4);
    SkTextUtils::DrawString(atlasCanvas, "Atlas", r.centerX(), r.centerY(), font, atlasPaint,
                            SkTextUtils::kCenter_Align);
    sk_sp<SkImage> atlas = atlasSurface->makeImageSnapshot();

    SkRSXform xform;
    const SkScalar rad = SkDegreesToRadians(15.0);
    xform.fSCos = SkScalarCos(rad);
    xform.fSSin = SkScalarSin(rad);
    xform.fTx = r.width() * 0.15;
