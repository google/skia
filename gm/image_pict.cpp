...
protected:
    GrTexture* onGenerateTexture(GrContext* ctx, const SkIRect* subset) override {
        ...
        if (!subset) {
            return SkSafeRef(fProxy->instantiate(ctx->textureProvider())->asTexture());
        }
        ...
        sk_sp<GrSurfaceContext> dstContext(ctx->contextPriv().makeDeferredSurfaceContext(
                                                                                desc,
                                                                                SkBudgeted::kNo));
        if (!dstContext) {
            return nullptr;
