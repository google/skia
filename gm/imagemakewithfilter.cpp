...
protected:
    ...
    void onDraw(SkCanvas* canvas) override {
        ...
        SkScalar DY = fMainImage->height() + MARGIN;
        ...
        for (auto clipBound : clipBounds) {
            ...
            for (size_t i = 0; i < SK_ARRAY_COUNT(filters); ++i) {
                ...
                canvas->drawImage(fMainImage, 0, 0, &alpha);

                this->drawImageWithFilter(canvas, filters[i], clipBound, subset, &outSubset);
...
private:
    ...
    void drawImageWithFilter(SkCanvas* canvas, FilterFactory filterFactory,
                             const SkIRect& clip, const SkIRect& subset,
                             SkIRect* dstRect) {
        ...
        sk_sp<SkImageFilter> filter = filterFactory(fAuxImage,
        ...
        if (fStrategy == Strategy::kSaveLayer) {
            ...
            canvas->drawImageRect(fMainImage, subset, SkRect::Make(subset), nullptr);
            ...
            *dstRect = subset;
            ...
            result = fMainImage->makeWithFilter(filter.get(), subset, clip, &outSubset, &offset);
