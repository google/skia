...
protected:
    ...
    SkISize onISize() override { return SkISize::Make(1560, 720); }
    ...
    void onOnceBeforeDraw() override {
        ...
        fMainImage = sk_gpu_test::AutoGPUImage::Make(surface->makeImageSnapshot());

        ToolUtils::draw_checkerboard(surface->getCanvas());
        fAuxImage = sk_gpu_test::AutoGPUImage::Make(surface->makeImageSnapshot());
    }
    ...
    void onDraw(SkCanvas* canvas) override {
        ...
        sk_sp<SkImage> mainImage = fMainImage->refImage(canvas->getGrContext());
        sk_sp<SkImage> auxImage = fAuxImage->refImage(canvas->getGrContext());
        SkASSERT(mainImage->isTextureBacked() || !canvas->getGrContext());
        SkASSERT(auxImage->isTextureBacked() || !canvas->getGrContext());
...
private:
    ...
    std::unique_ptr<sk_gpu_test::AutoGPUImage> fMainImage;
    std::unique_ptr<sk_gpu_test::AutoGPUImage> fAuxImage;
    ...
    void drawImageWithFilter(SkCanvas* canvas, sk_sp<SkImage> mainImage, sk_sp<SkImage> auxImage,
        ...
        // When creating the filter with a crop rect equal to the clip, we should expect to see
        // no difference with a filter with no crop rect. However, if the CTM isn't managed properly
