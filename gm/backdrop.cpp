 ...
 *  - row:0     no-hint-rect    no-clip-rect        expect big blur (sans inner circle)
 *  - row:1     no-hint-rect    clip-rect           expect small blur (sans inner circle)
 *  - row:2     hint-rect       no-clip-rect        expect big blur (sans inner circle)
 *  - row:3     hint-rect       clip-rect           expect small blur (sans inner circle)
 *
 *  The test is that backdrop effets should be independent of the hint-rect, but should
 *  respect the clip-rect.
 */
DEF_SIMPLE_GM(backdrop_picture, canvas, 768, 512) {
    for (bool useHintRect : {false, true}) {
        for (bool useClip : {false, true}) {
            SkAutoCanvasRestore acr(canvas, true);

            doDraw(canvas, useClip, useHintRect);

            SkPictureRecorder rec;
            doDraw(rec.beginRecording(256, 256), useClip, useHintRect);
            ...
            doDraw(&lite, useClip, useHintRect);
            canvas->translate(256, 0);
            dl.draw(canvas);
