...
public:
    ...
    constexpr explicit YUVAPlanarConfig(const std::initializer_list<YUVALocation>& yuvaLocations);
    ...
    /**
     * Given a mask of SkColorChannelFlags choose a channel by index. Legal 'channelMask' values are:
     * kAlpha, kGray, (kAlpha | kGray), kRed, kRG, kRGB, kRGBA.
     * The channel index must be less than the number of bits set in the mask. The index order
     * is the order listed above (e.g. if 'channelMask' is kRGB and 'channelIdx' is 1 then
     * SkColorChannel::kG is returned as 'channel').
     * The function fails if 'channelMask' is not one of the listed allowed values or 'channelIdx'
     * is invalid for the mask.
     */
    static bool ChannelIndexToChannel(uint32_t channelMask, int channelIdx, SkColorChannel* channel);
     ...
     */
    bool getYUVAIndices(const GrBackendTexture textures[], int numTextures, bool externalAlphaPlane, SkYUVAIndex indices[4]) const;

    /** Bridge function for GrYUVProvider. */
    void asYUVAIndices(SkYUVAIndex indices[4]) const;
...
constexpr YUVAPlanarConfig::YUVAPlanarConfig(const std::initializer_list<YUVALocation>& yuvaLocations) {
    ...
    // Check that planes are used channel usage in each plane is tightly packed and convert to a
    // count
...
bool YUVAPlanarConfig::ChannelIndexToChannel(uint32_t channelFlags, int channelIdx, SkColorChannel* channel) {
    switch (channelFlags) {
        ...
        case kAlpha_SkColorChannelFlag:
            ...
            return false;
        case kGray_SkColorChannelFlag | kAlpha_SkColorChannelFlag:
            if (channelIdx == 0) {
                *channel = SkColorChannel::kR;
                return true;
...
bool YUVAPlanarConfig::getYUVAIndices(const GrBackendTexture textures[], int numTextures, bool externalAlphaPlane, SkYUVAIndex indices[4]) const {
    if (this->hasAlpha() && externalAlphaPlane) {
        return false;
    }
    int numPlanes = fNumPlanes + SkToInt(externalAlphaPlane);
    if (numTextures != numPlanes) {
        return false;
    }
    for (int i = 0; i < 4; ++i) {
        ...
        if (plane < 0) {
            ...
            SkASSERT(plane < numTextures);
            auto channelMask = textures[plane].getBackendFormat().channelMask();
            if (!ChannelIndexToChannel(channelMask, fLocations[i].fChannelIdx, &indices[i].fChannel)) {
                return false;
            }
        }
    }
    if (externalAlphaPlane) {
        auto channelMask = textures[numPlanes - 1].getBackendFormat().channelMask();
        if (!ChannelIndexToChannel(channelMask, 0, &indices[3].fChannel)) {
            return false;
        }
...
void YUVAPlanarConfig::asYUVAIndices(SkYUVAIndex indices[4]) const {
    for (int i = 0; i < 4; ++i) {
        indices[i].fIndex = fLocations[i].fPlaneIdx;
        // This is a bit lossy because it assumes an RGBA color type.
        indices[i].fChannel = static_cast<SkColorChannel>(fLocations->fChannelIdx);
    }
}
...
static const YUVAPlanarConfig& YUVAFormatPlanarConfig(YUVFormat format) {
    switch (format) {
        case kP016_YUVFormat: {
            static constexpr YUVAPlanarConfig kConfig({{0, 0}, {1, 0}, {1, 1}});
            return kConfig;
        }

    }
    static constexpr YUVAPlanarConfig kFormatInfos[] = {
            // kP016_YUVFormat
            YUVAPlanarConfig({{0, 0}, {1, 0}, {1, 1}}),
            // kP010_YUVFormat
            YUVAPlanarConfig({{0, 0}, {1, 0}, {1, 1}}),
            // kP016F_YUVFormat
            YUVAPlanarConfig({{0, 0}, {1, 0}, {1, 1}}),
            // kY416_YUVFormat
            YUVAPlanarConfig({{0, 1}, {0, 0}, {0, 2}, {0, 3}}),
            // kAYUV_YUVFormat
            YUVAPlanarConfig({{0, 0}, {0, 1}, {0, 2}, {0, 3}}),
            // kY410_YUVFormat
            YUVAPlanarConfig({{0, 1}, {0, 0}, {0, 2}, {0, 3}}),
            // kNV12_YUVFormat
            YUVAPlanarConfig({{0, 0}, {1, 0}, {1, 1}}),
            // kNV21_YUVFormat
            YUVAPlanarConfig({{0, 0}, {1, 1}, {1, 0}}),
            // kI420_YUVFormat
            YUVAPlanarConfig({{0, 0}, {1, 0}, {2, 0}}),
            // kYV12_YUVFormat
            YUVAPlanarConfig({{0, 0}, {2, 0}, {1, 0}}),
    };
}
...
static int create_YUV(const PlaneData& planes, YUVFormat yuvFormat, SkBitmap resultBMs[], bool opaque) {
    ...
    if (!kFormatInfos[(int)yuvFormat].hasAlpha() && !opaque) {
...
public:
    YUVGenerator(const SkImageInfo& ii,
            ...
            , fAllA8(true) {
        SkASSERT(!externalAlphaPlane || !kFormatInfos[fYUVFormat].hasAlpha());
...
protected:
    bool onGetPixels(const SkImageInfo& info, void* pixels, size_t rowBytes,
        ...
        if (kUnknown_SkColorType == fFlattened.colorType()) {
            ...
            int f = static_cast<int>(fYUVFormat);
            for (int y = 0; y < info.height(); ++y) {
                for (int x = 0; x < info.width(); ++x) {
                    ...
                    for (auto c : {YUVAChannel::kY, YUVAChannel::kU, YUVAChannel::kV}) {
                        const auto& bmp = fYUVBitmaps[kFormatInfos[f].planeIndex(c)];
                        int channelIdx = kFormatInfos[f].channelIndex(c);
                        yuva[static_cast<int>(c)] = look_up(x1, y1, bmp, channelIdx);
                    }
                    if (kFormatInfos[f].hasAlpha()) {
                        const auto& bmp = fYUVBitmaps[kFormatInfos[f].planeIndex(YUVAChannel::kA)];
                        int channelIdx = kFormatInfos[f].channelIndex(YUVAChannel::kA);
    ...
    bool onQueryYUVA8(SkYUVASizeInfo* size,
        ...
        kFormatInfos[static_cast<int>(fYUVFormat)].asYUVAIndices(yuvaIndices);
...
private:
    int numPlanes() const { return kFormatInfos[fYUVFormat].numPlanes() + SkToInt(fExternalAlphaPlane); }
...
static sk_sp<SkImage> make_yuv_gen_image(const SkImageInfo& ii,
                                         ...
                                         bool opaque,
                                         SkBitmap bitmaps[]) {
    bool externalAlphaPlane = !opaque && !kFormatInfos[yuvFormat].hasAlpha();
    std::unique_ptr<SkImageGenerator> gen(new YUVGenerator(ii, yuvColorSpace,
                                                           yuvFormat, externalAlphaPlane, bitmaps));
...
protected:
    ...
    sk_sp<SkImage> resizeOnGpu(GrContext* context,
        ...
        for (int i = 0; i < numTextures; ++i) {
            if (!context->priv().caps()->isFormatRenderable(yuvaTextures[i].getBackendFormat(), 1)) {
                return nullptr;
            }
            auto srcProxy = context->priv().proxyProvider()->wrapBackendTexture(
                    yuvaTextures[i], kBorrow_GrWrapOwnership, GrWrapCacheable::kNo,
                    GrIOType::kRead_GrIOType);
            if (!srcProxy) {
                return nullptr;
            }
            GrSurfaceProxyView srcView(std::move(srcProxy), kTopLeft_GrSurfaceOrigin,
                                       GrSwizzle("rgba"));
            GrPaint paint;
            paint.addColorFragmentProcessor(
                    GrTextureEffect::Make(std::move(srcView), kUnknown_SkAlphaType));
            ...
            shrunkTextures[i] = context->createBackendTexture(shrunkPlaneSize.width(),
                                                              ...
                                                              GrMipMapped::kNo,
                                                              GrRenderable::kYes);
            if (!shrunkTextures->isValid()) {
                return nullptr;
            }
            // Store this away so it will be cleaned up at the end.
            fBackendTextures.push_back(shrunkTextures[i]);

            auto dstProxy = context->priv().proxyProvider()->wrapRenderableBackendTexture(
                    shrunkTextures[i], 1, kBorrow_GrWrapOwnership, GrWrapCacheable::kNo);
            if (!dstProxy) {
                return nullptr;
            }

            auto rtc = GrRenderTargetContext::Make(
                    context->priv().asRecordingContext(), GrColorType::kUnknown, nullptr,
                    std::move(dstProxy), kTopLeft_GrSurfaceOrigin, nullptr);
            if (!rtc) {
                return nullptr;
            }

            rtc->fillRectToRect(GrNoClip(), std::move(paint), GrAA::kNo, SkMatrix::I(),
                                SkRect::Make(shrunkTextures[i].dimensions()),
                                SkRect::Make(yuvaTextures[i].dimensions()));
        ...
        for (bool opaque : { false, true }) {
            for (int cs = kJPEG_SkYUVColorSpace; cs <= kLastEnum_SkYUVColorSpace; ++cs) {
                ...
                for (int format = kP016_YUVFormat; format <= kLast_YUVFormat; ++format) {
                    ...
                    int numTextures = create_YUV(planes, (YUVFormat) format, resultBMs, opaque);
                    ...
                    if (context) {
                        ...
                        bool externalAlphaPlane = !opaque && !kFormatInfos[format].hasAlpha();
                        if (!kFormatInfos[format].getYUVAIndices(yuvaTextures, numTextures, externalAlphaPlane, yuvaIndices)) {
                            continue;
                        }
...
protected:
    ...
    void createImages(GrContext* context) {
        for (bool opaque : { false, true }) {
            ...
            int numPlanes = kFormatInfos[kAYUV_YUVFormat].numPlanes();
            ...
            SkYUVAIndex yuvaIndices[4];
            kFormatInfos[kAYUV_YUVFormat].getYUVAIndices(yuvaTextures, numPlanes, false, yuvaIndices);
