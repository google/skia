...
static const float kGoldenRatio = 1.6180339887f;
...
static void add_arc(SkPath* path,
    ...
    SkScalar t = ((o2.fX - o1.fX) * v4.fY - (o2.fY - o1.fY) * v4.fX) / (v3.fX * v4.fY - v3.fY * v4.fX);
...
protected:
    ...
    bool onQueryYUV8(SkYUVSizeInfo* size, SkYUVColorSpace* yuvColorSpace) const override {
        if (AYUV_YUVFormat != fYUVFormat &&
            kI420_YUVFormat != fYUVFormat &&
            kYV12_YUVFormat != fYUVFormat) {
            return false; // currently this API only supports planar formats
        }

        *yuvColorSpace = fYUVColorSpace;
        size->fSizes[0].fWidth = fBitmaps[fYUVAIndices[0].fIndex].width();
        size->fSizes[0].fHeight = fBitmaps[fYUVAIndices[0].fIndex].height();
        size->fWidthBytes[0] = fBitmaps[fYUVAIndices[0].fIndex].rowBytes();

        size->fSizes[1].fWidth = fBitmaps[fYUVAIndices[1].fIndex].width();
        size->fSizes[1].fHeight = fBitmaps[fYUVAIndices[1].fIndex].height();
        size->fWidthBytes[1] = fBitmaps[fYUVAIndices[1].fIndex].rowBytes();

        size->fSizes[2].fWidth = fBitmaps[fYUVAIndices[2].fIndex].width();
        size->fSizes[2].fHeight = fBitmaps[fYUVAIndices[2].fIndex].height();
        size->fWidthBytes[2] = fBitmaps[fYUVAIndices[2].fIndex].rowBytes();
        return true;
    }

    bool onGetYUV8Planes(const SkYUVSizeInfo&, void* planes[3]) override {
        planes[0] = fBitmaps[fYUVAIndices[0].fIndex].getAddr(0, 0);
        planes[1] = fBitmaps[fYUVAIndices[1].fIndex].getAddr(0, 0);
        planes[2] = fBitmaps[fYUVAIndices[2].fIndex].getAddr(0, 0);
        return true;
    }
...
static sk_sp<SkImage> make_yuv_generator_backed_image(const SkImageInfo& ii,
                                                      YUVFormat yuvFormat,
                                                      SkYUVColorSpace yuvColorSpace,
                                                      SkYUVAIndex yuvaIndices[4],
                                                      SkBitmap bitmaps[4]) {
...
protected:
    ...
    SkISize onISize() override {
        int numCols = 2; // opaque & transparent
        int numRows = 1 + (kLast_YUVFormat + 1) * (kLastEnum_SkYUVColorSpace + 1);
        return SkISize::Make(numCols * kTileWidthHeight + (numCols+1) * kPad,
                             numRows * kTileWidthHeight + (numRows+1) * kPad);
    }
        ...
        for (bool opaque : { false, true }) {
            for (int cs = kJPEG_SkYUVColorSpace; cs <= kLastEnum_SkYUVColorSpace; ++cs) {
                ...
                for (int format = AYUV_YUVFormat; format <= kLast_YUVFormat; ++format) {
                    ...
                    fImages[opaque][cs][format] = make_yuv_generator_backed_image(fOriginalBMs[opaque].info(),
                                                                                  (YUVFormat) format,
                                                                                  (SkYUVColorSpace) cs,
                                                                                  yuvaIndices,
                                                                                  resultBMs);
