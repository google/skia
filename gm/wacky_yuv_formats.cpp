...
public:
    ...
    /** Bridge function for GrYUVProvider. This is lossy. */
...
bool YUVAPlanarConfig::getYUVAIndices(const GrBackendTexture textures[],
    ...
    for (int i = 0; i < 4; ++i) {
        ...
        if (plane < 0) {
            ...
            if (!ChannelIndexToChannel(channelMask, fLocations[i].fChannelIdx,
                                       &indices[i].fChannel)) {
                return false;
            }
        }
...
static const YUVAPlanarConfig& YUVAFormatPlanarConfig(YUVFormat format) {
    switch (format) {
        ...
        case kY416_YUVFormat: {
            static constexpr YUVAPlanarConfig kConfig({{0, 1}, {0, 0}, {0, 2}, {0, 3}});
            return kConfig;
        }
        case kAYUV_YUVFormat: {
            static constexpr YUVAPlanarConfig kConfig({{0, 0}, {0, 1}, {0, 2}, {0, 3}});
            return kConfig;
        }
        ...
        case kNV21_YUVFormat: {
            static constexpr YUVAPlanarConfig kConfig({{0, 0}, {1, 1}, {1, 0}});
            return kConfig;
        }
        case kI420_YUVFormat: {
            static constexpr YUVAPlanarConfig kConfig({{0, 0}, {1, 0}, {2, 0}});
            return kConfig;
        }
        case kYV12_YUVFormat: {
            static constexpr YUVAPlanarConfig kConfig({{0, 0}, {2, 0}, {1, 0}});
            return kConfig;
        }
    }
    SkUNREACHABLE;
...
protected:
    ...
    bool onQueryYUVA8(SkYUVASizeInfo* size,
        ...
        YUVAFormatPlanarConfig(fYUVFormat).asYUVAIndices(yuvaIndices);
            ...
            for (int cs = kJPEG_SkYUVColorSpace; cs <= kLastEnum_SkYUVColorSpace; ++cs) {
                ...
                for (int f = kP016_YUVFormat; f <= kLast_YUVFormat; ++f) {
                    ...
                    if (context) {
                        ...
                        bool externalAlphaPlane =
                                !opaque && !YUVAFormatPlanarConfig(format).hasAlpha();
                        if (!YUVAFormatPlanarConfig(format).getYUVAIndices(
                                    yuvaTextures, numTextures, externalAlphaPlane, yuvaIndices)) {
                            continue;
                        }
