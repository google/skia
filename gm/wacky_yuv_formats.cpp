...
static SkPMColor convert_yuva_to_rgba_jpeg(uint8_t y, uint8_t u, uint8_t v, uint8_t a) {
    ...
    uint8_t r = SkScalarPin(SkScalarRoundToInt( 1.0f * c                   +  1.402f    * e ), 0, 255);
    uint8_t g = SkScalarPin(SkScalarRoundToInt( 1.0f * c - (0.344136f * d) - (0.714136f * e)), 0, 255);
    uint8_t b = SkScalarPin(SkScalarRoundToInt( 1.0f * c +  1.773f    * d                   ), 0, 255);
        ...
        case AYUV_YUVFormat: {
            ...
            for (int y = 0; y < planes.fYFull.height(); ++y) {
                for (int x = 0; x < planes.fYFull.width(); ++x) {
                    ...
                    //SkDebugf("%d %d %d %d\n", SK_R32_SHIFT, SK_G32_SHIFT, SK_B32_SHIFT, SK_A32_SHIFT);

                    // NOT premul!
                    SkColor c1 = SkColorSetARGB(A, Y, U, V);
                    *yuvaFull.getAddr32(x, y) = c1;

                    static bool done = false;
                    if (A && !done) {
                        SkColor col = yuvaFull.getColor(x, y);
                      //  SkDebugf("%x %x\n", c1, col);

                        uint8_t r2 = SkColorGetR(col);
                        uint8_t g2 = SkColorGetG(col);
                        uint8_t b2 = SkColorGetB(col);
                        uint8_t a2 = SkColorGetA(col);

                        SkDebugf("in %d %d %d %d - out %d %d %d %d\n",
                                 Y, U, V, A, r2, g2, b2, a2);
                        done = true;
                    }
                }
            ...
            yuvaIndices[0].fChannel = kR_SkImageSourceChannel;
            yuvaIndices[1].fIndex = 0;
            yuvaIndices[1].fChannel = kG_SkImageSourceChannel;
            yuvaIndices[2].fIndex = 0;
            yuvaIndices[2].fChannel = kB_SkImageSourceChannel;
...
static uint8_t look_up(float x1, float y1, const SkBitmap& bm, SkImageSourceChannel channel) {
    uint8_t result;
