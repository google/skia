            ...
            for (int y = 0; y < info.height(); ++y) {
                for (int x = 0; x < info.width(); ++x) {
                    ...
                    if (fYUVAIndices[3].fIndex >= 0) {
                        if (kAlpha_8_SkColorType == fYUVBitmaps[fYUVAIndices[3].fIndex].colorType()) {
                            alpha = *fYUVBitmaps[fYUVAIndices[3].fIndex].getAddr8(x, y);
                        } else {
                            alpha = SkColorGetA(fYUVBitmaps[fYUVAIndices[0].fIndex].getColor(x, y));
                        }
                    }
                    ...
                    // Making premul here.
                    *fFlattened.getAddr32(x, y) = SkColorSetARGB(alpha, g, g, g);
                }
            }
        }
