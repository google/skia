...
static void extract_planes(const SkBitmap& bm, SkYUVColorSpace yuvColorSpace, PlaneData* planes) {
    ...
    planes->fUQuarter.allocPixels(SkImageInfo::Make(bm.width()/2, bm.height()/2, kGray_8_SkColorType,
                                  kUnpremul_SkAlphaType));
    planes->fVQuarter.allocPixels(SkImageInfo::Make(bm.width()/2, bm.height()/2, kGray_8_SkColorType,
                                  kUnpremul_SkAlphaType));
