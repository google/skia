...
// the lower 6 bits to 0 for the 10bpp formats.
...
// Create a 16bit 2x2 downsampled SkBitmap. It is stored in an RG texture.
static SkBitmap make_quarter_2_channel_16(const SkBitmap& quarter, bool tenBitsPP) {
    SkASSERT(quarter.colorType() == kRGBA_F32_SkColorType);

    SkBitmap result;

    result.allocPixels(SkImageInfo::Make(quarter.width(), quarter.height(),
                                         kRG_1616_SkColorType,
                                         kUnpremul_SkAlphaType));

    float* src = (float*) quarter.getAddr(0, 0);
    for (int y = 0; y < quarter.height(); ++y) {
        for (int x = 0; x < quarter.width(); ++x) {
            uint32_t u16 = SkScalarRoundToInt(src[1] * 65535.0f);
            uint32_t v16 = SkScalarRoundToInt(src[2] * 65535.0f);
...
static void create_YUV(const PlaneData& planes, YUVFormat yuvFormat,
    ...
    switch (yuvFormat) {
        ...
        case kP010_YUVFormat: {
            ...
            resultBMs[nextLayer++] = make_quarter_2_channel_16(planes.fQuarter,
                                                               yuvFormat == kP010_YUVFormat);
