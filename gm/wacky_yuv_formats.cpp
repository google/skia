    ...
    YUVGenerator(const SkImageInfo& ii,
        ...
        for (int i = 0; i < 4; ++i) {
            if (yuvaIndices[i].fIndex >= 0) {
                fLastUsedBitmap = SkTMax(fLastUsedBitmap, i);
...
protected:
    ...
    bool onQueryYUVA8(SkYUVASizeInfo* size,
        ...
        *yuvColorSpace = fYUVColorSpace;
        ...
        for ( ; i < 4; ++i) {
            size->fSizes[1].fWidth = 0;
            size->fSizes[1].fHeight = 0;
            size->fWidthBytes[1] = 0;
        }
    ...
    bool onGetYUVA8Planes(const SkYUVASizeInfo&,
        ...
        for (int i = 0; i <= fLastUsedBitmap; ++i) {
            planes[i] = fYUVBitmaps[fYUVAIndices[i].fIndex].getPixels();
        }
        return true;
    }
