...
protected:
    ...
    sk_sp<SkImage> resizeOnGpu(GrContext* context,
        ...
        BackendReleaseContext* releaseContexts[4] = {};
        ...
        for (int i = 0; i < numTextures; ++i) {
            ...
            if (ct == kUnknown_SkColorType || !context->colorTypeSupportedAsSurface(ct)) {
                BackendReleaseContext::unwind(context, releaseContexts);
                return nullptr;
            }
    ...
    DrawResult onGpuSetup(GrContext* context, SkString* errorMsg) override {
        ...
        if (!this->createImages(context)) {    // This isn't a GpuGM so a null 'context' is okay
...
protected:
    ...
    bool createImages(GrContext* context) {
        for (bool opaque : { false, true }) {
            ...
            GrBackendTexture srgbYUVATextures[4];
            BackendReleaseContext* srgbReleaseContexts[4] = {};

            for (int i = 0; i < numPlanes; ++i) {
                yuvaTextures[i] = create_yuva_texture(context, resultBMs[i]);
                if (!yuvaTextures[i].isValid()) {
                    BackendReleaseContext::unwind(context, releaseContexts);
                    BackendReleaseContext::unwind(context, srgbReleaseContexts);
                    return false;
                }

                releaseContexts[i] = new BackendReleaseContext(context, yuvaTextures[i]);

                srgbYUVATextures[i] = create_yuva_texture(context, resultBMs[i]);
                if (!srgbYUVATextures[i].isValid()) {
                    BackendReleaseContext::unwind(context, releaseContexts);
                    BackendReleaseContext::unwind(context, srgbReleaseContexts);
                    return false;
                }

                srgbReleaseContexts[i] = new BackendReleaseContext(context, srgbYUVATextures[i]);
            }
            ...
            fImages[opaque][0] = SkImage::MakeFromYUVATextures(
                    ...
                    BackendReleaseContext::release, (void**) releaseContexts);
            fImages[opaque][1] = SkImage::MakeFromYUVATextures(
                    ...
                    BackendReleaseContext::release, (void**) srgbReleaseContexts);
        }

        // Some backends require all work be completed for backend textures before they are
        // deleted. Since we don't know when we'll next have access to a direct context, flush
        // all the work now.
        GrFlushInfo flushInfoSyncCpu;
        flushInfoSyncCpu.fFlags = kSyncCpu_GrFlushFlag;
        context->flush(flushInfoSyncCpu);
