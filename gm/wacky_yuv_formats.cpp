...
static void add_arc(SkCanvas* c, SkPath* path,
...
static SkPath create_splat(SkCanvas* canvas, const SkPoint& o, SkScalar innerRadius, SkScalar outerRadius,
    ...
    for (int i = 0; i < numDivisions; ++i) {
        ...
        if (0 == (i % 2)) {
            ...
            p.lineTo(top);
            add_arc(canvas, &p, top, curV, nextTop, nextV, circles, true);
        } else {
            ...
            p.lineTo(bot);
            add_arc(canvas, &p, bot, curV, nextBot, nextV, nullptr, false);
        }
...
protected:
...
#else
        return SkISize::Make(kTileWidthHeight + 2 * kPad,
                             kTileWidthHeight + 2 * kPad);
#endif
    }
    ...
    void onOnceBeforeDraw() override {
...
#if 0
        {
            ...
            SkPath path = create_splat({ 128.0f, 128.0f }, 20.0f, 54.0f, 4.0f, 5, &circles);
            fBMs[0] = make_bitmap(path, circles, false);
        }

        {
            SkTDArray<SkRect> circles;
            SkPath path = create_splat({ 128.0f, 128.0f }, 20.0f, 54.0f, 4.0f, 3, &circles);
    ...
    void onDraw(SkCanvas* canvas) override {
        ...
        {
            SkTDArray<SkRect> circles;
            SkPath path = create_splat(canvas, { 128.0f, 128.0f }, 20.0f, 54.0f, 1.0f, 5, &circles);
