        ...
        case kNV12_YUVFormat: {
            ...
            for (int y = 0; y < planes.fYFull.height()/2; ++y) {
                for (int x = 0; x < planes.fYFull.width()/2; ++x) {
                    ...
                    // NOT premul!
                    *uvQuarter.getAddr32(x, y) = SkColorSetARGB(0xFF, U, V, 0);
                }
            }
        ...
        case kNV21_YUVFormat: {
            ...
            for (int y = 0; y < planes.fYFull.height()/2; ++y) {
                for (int x = 0; x < planes.fYFull.width()/2; ++x) {
                    ...
                    // NOT premul!
                    *vuQuarter.getAddr32(x, y) = SkColorSetARGB(0xFF, V, U, 0);
                }
            }
    ...
    if (kAlpha_8_SkColorType == bm.colorType()) {
        ...
        switch (channel) {
            ...
            case SkColorChannel::kG:
                ...
                break;
            case SkColorChannel::kB:
                result = ((c >> 00) & 0x3ff) * (255.0f/1023.0f);
                break;
            case SkColorChannel::kA:
