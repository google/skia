...
static void make_RG_88(const GrCaps* caps,
    ...
    pixels->reset(2 * bm.width() * bm.height());
...
static void make_RG_1616(const GrCaps* caps,
    ...
    uint16_t u16, v16;
    pixels->reset(4 * bm.width() * bm.height());
...
static GrBackendTexture create_yuva_texture(GrGpu* gpu, const SkBitmap& bm,
    ...
    if (format_uses_16_bpp(yuvFormat) || 2 == channelCount) {
        ...
        if (2 == channelCount) {
            SkASSERT(yuvFormat == kP016_YUVFormat || yuvFormat == kP016_YUVFormat ||
                     yuvFormat == kNV12_YUVFormat || yuvFormat == kNV21_YUVFormat);

            if (format_uses_16_bpp(yuvFormat)) {
                make_RG_1616(caps, bm, yuvFormat, &pixels, &format);
            } else {
                make_RG_88(caps, bm, &pixels, &format);
            }
        } else {
            SkASSERT(yuvFormat == kP016_YUVFormat || yuvFormat == kP016_YUVFormat ||
                     yuvFormat == kY416_YUVFormat);
            SkASSERT(kRGBA_8888_SkColorType == bm.colorType() ||
                     kAlpha_8_SkColorType == bm.colorType());
