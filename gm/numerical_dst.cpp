...
// The three gradients should all look roughly the same.

DEF_SIMPLE_GM(numerical_dst, canvas, 128*3, 160) {
    auto draw = [canvas](const char* label, SkImageInfo info) {
        SkPaint gradient;
        ...
        if (auto surface = canvas->makeSurface(info)) {
    ...
    draw("native",    canvas->imageInfo());
    draw("gamma=2.9", canvas->imageInfo().makeColorSpace(gamma_2_9));
    draw("numerical", canvas->imageInfo().makeColorSpace(numerical));
}
