...
static sk_sp<SkTypeface> make_tf() {
    ...
    const SkMatrix scale = SkMatrix::Scale(1.0f/kUPEM, 1.0f/kUPEM);

    {
        SkFontMetrics metrics;
        font.getMetrics(&metrics);
        builder.setMetrics(metrics, 1.0f/kUPEM);
    }
    ...
    for (SkGlyphID index = 0; index <= 127; ++index) {
        ...
        builder.setGlyph(index, width/kUPEM, path.makeTransform(scale));
    }
