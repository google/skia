...
void WindowRectanglesMaskGM::onCoverClipStack(const SkClipStack& stack, SkCanvas* canvas) {
    ...
    GrRenderTargetContext* dc = canvas->internal_private_accessTopLayerRenderTargetContext();
...
void WindowRectanglesMaskGM::visualizeAlphaMask(GrContext* ctx, GrRenderTargetContext* dc,
...
void WindowRectanglesMaskGM::visualizeStencilMask(GrContext* ctx, GrRenderTargetContext* dc,
                                                  const GrReducedClip& reducedClip,
                                                  const GrPaint& paint) {
...
void WindowRectanglesMaskGM::stencilCheckerboard(GrRenderTargetContext* dc, bool flip) {
    ...
    dc->renderTargetContextPriv().clearStencilClip(GrFixedClip::Disabled(), false);
    ...
    for (int y = 0; y < kLayerRect.height(); y += kMaskCheckerSize) {
        for (int x = (y & 1) == flip ? 0 : kMaskCheckerSize;
            ...
            dc->renderTargetContextPriv().stencilRect(GrNoClip(), &kSetClip, false, SkMatrix::I(),
                                                      SkRect::Make(checker));
        }
    }
}
