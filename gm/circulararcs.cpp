...
DEF_SIMPLE_GM(circular_arc_stroke_matrix, canvas, 370, 1090) {
    ...
    matrices.push_back().setAll( 0,  1,  0,
                                 1,  0,  0,
                                 0,  0,  1);
    int cnt = matrices.count();
    SkMatrix m90;
    m90.setRotate(90, kRadius, kRadius);
    SkMatrix m45;
    m45.setRotate(45, kRadius, kRadius);
    SkMatrix m180;
    m180.setRotate(180, kRadius, kRadius);
    for (int i = 0; i < cnt; ++i) {
        matrices.push_back().setConcat(matrices[i], m90);
    }
    for (int i = 0; i < cnt; ++i) {
        matrices.push_back().setConcat(matrices[i], m45);
    }
    for (int i = 0; i < cnt; ++i) {
        matrices.push_back().setConcat(matrices[i], m180);
    }
