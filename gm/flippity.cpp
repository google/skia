...
static void UVMatToGeomMat1(SkMatrix* geomMat, const SkMatrix& uvMat, bool isBottomLeft) {
    ...
    SkMatrix foo;
    foo.setConcat(tmp, *geomMat);

    for (int y = 0; y < 3; ++y) {
        for (int x = 0; x < 3; ++x) {
            if (x == y) {
                SkASSERT(SkScalarNearlyEqual(foo[y*3+x], 1.0f));
            } else {
                SkASSERT(SkScalarNearlyEqual(foo[y*3+x], 0.0f));
            }
        }
    }
}

static void UVMatToGeomMat2(SkMatrix* geomMat, const SkMatrix& uvMat) {
...
protected:
    ...
    void drawImageWithMatrixAndLabels(SkCanvas* canvas, SkImage* image, int matIndex, bool isBottomLeft) {
        SkPaint paint;

        canvas->save();

        SkMatrix geomMat1;
        UVMatToGeomMat1(&geomMat1, kUVMatrices[matIndex], isBottomLeft);


#if MUNGE_UVS
            image->mTextureMatrix = kUVMatrices[matIndex];
            canvas->drawImage(image, 0, 0, &paint);
#else
            SkMatrix geomMat2;
            UVMatToGeomMat2(&geomMat2, kUVMatrices[matIndex]);

            canvas->save();
                canvas->concat(geomMat2);
                canvas->drawImage(image, 0, 0, &paint);
            canvas->restore();
#endif

            canvas->save();
            canvas->concat(geomMat1);
