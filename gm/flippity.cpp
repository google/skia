...
constexpr int kNumMatrices = 6;
constexpr int kImageSize = 128;
constexpr int kLabelSize = 32;
constexpr int kNumLabels = 4;
constexpr int kInset = 16;

constexpr int kCellSize = kImageSize+2*kLabelSize;
constexpr int kGMWidth  = kNumMatrices*kCellSize;
constexpr int kGMHeight = 4*kCellSize;
...
// image.
static bool UVMatToGeomMatForImage(SkMatrix* geomMat, const SkMatrix& uvMat, bool imageIsBottomLeft) {
...
protected:
    ...
    void drawImageWithMatrixAndLabels(SkCanvas* canvas, SkImage* image, int matIndex,
                                      bool imageIsBottomLeft, bool drawSubset, bool drawScaled) {
        ...
        SkAssertResult(UVMatToGeomMatForImage(&imageGeomMat,
                                              kUVMatrices[matIndex],
                                              imageIsBottomLeft));
        ...
        canvas->save();
            ...
            for (int i = 0; i < kNumMatrices; ++i) {
                this->drawImageWithMatrixAndLabels(canvas, referenceImage.get(), i,
                                                   bottomLeftImage, drawSubset, drawScaled);
