    ...
    for (int y = 0; y < info.height(); y++) {
        for (int x = 0; x < info.width(); x++) {
            ...
            if (kUnpremul_SkAlphaType == info.alphaType()) {
                ...
                rgba = Sk4f::Max(rgba, 0.0f);
            } else {
                SkASSERT(kPremul_SkAlphaType == info.alphaType());
                rgba = Sk4f::Min(rgba, rgba[3]);
                rgba = Sk4f::Max(rgba, 0.0f);
            }
...
static void draw_image(SkCanvas* canvas, SkColorType dstColorType, SkAlphaType dstAlphaType,
                       sk_sp<SkColorSpace> dstColorSpace, SkColorType srcColorType,
...
protected:
    ...
    SkISize onISize() override {
        return SkISize::Make(256 * 6, 256 * 18);
    }
                ...
                for (SkAlphaType srcAlphaType : alphaTypes) {
                    ...
                    for (SkColorType dstColorType : colorTypes) {
                        for (SkAlphaType dstAlphaType : alphaTypes) {
                            draw_image(canvas, dstColorType, dstAlphaType, dstColorSpace,
                                       srcColorType, srcAlphaType);
