...
DEF_SIMPLE_GM(shadow_utils, canvas, kW, kH) {
    ...
    for (auto flags : {kNone_ShadowFlag, kTransparentOccluder_ShadowFlag}) {
        ...
        for (const auto& path : paths) {
            ...
            if (i++ == 0 && flags == kTransparentOccluder_ShadowFlag) {
                // draw_shadow(canvas, path, kHeight, SK_ColorRED, lightPos, lightR, true, flags);
    ...
    // Show where the light is (device space)
