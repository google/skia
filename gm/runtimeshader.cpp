...
public:
    ClipSquircle() : RuntimeShaderGM("clip_squircle", {512, 256}, R"(
        uniform float2x2 viewMatrixSkew;
        uniform float2 exponents;
        half4 main(float2 xy) {
            xy = abs(xy);
            float2 expMinus1 = pow(xy, exponents - 1);
            float f = dot(expMinus1, xy) - 1;
            float2 grad = (exponents * expMinus1) * viewMatrixSkew;
    ...
    void onDraw(SkCanvas* canvas) override {
        ...
        if (localMatrix.invert(&squircleMatrix)) {
            SkRuntimeShaderBuilder builder(fEffect);
            builder.uniform("viewMatrixSkew") = SkV4{squircleMatrix.getScaleX(),
