...
public:
    ...
    void onDraw(SkCanvas* canvas) override {
        ...
        if (localMatrix.invert(&inverse)) {
            SkRuntimeShaderBuilder builder(fEffect);
            builder.uniform("dxdy") = SkV4{inverse.getScaleX(), inverse.getSkewY(),
                                           inverse.getSkewX(), inverse.getScaleY()};
