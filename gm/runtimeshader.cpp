...
public:
    ClipSuperRRect(const char* name, float power) : RuntimeShaderGM(name, {500, 500}, R"(
        ...
        uniform float2 one_minus_1_over_inset;
        uniform float2x2 derivatives;
        half4 main(float2 xy) {
            xy = max(abs(xy) + one_minus_1_over_inset, 0);
    ...
    void drawSuperRRect(SkCanvas* canvas, const SkRect& superRRect, float radX, float radY,
        ...
        // Inset such that the "apex" of our "super" rounded corner is in the same location that the
        // apex of a circular rounded corner would be with the given radii. We define the apex as
        // the point on the rounded corner that is 45 degrees between the horizontal and vertical
        // edges.
        float scale = (1 - SK_ScalarRoot2Over2) / (1 - exp2f(-1/fPower));
        float insetX = radX * scale;
        float insetY = radY * scale;
        insetX = std::min(insetX, superRRect.width() * .5f);
        insetY = std::min(insetY, superRRect.height() * .5f);
        builder.uniform("one_minus_1_over_inset") = SkV2{1 - superRRect.width()*.5f / insetX,
                                                         1 - superRRect.height()*.5f / insetY};
        ...
        //                                     |0  n|   |dt/dx  dt/dy|
        //
        //              = [s^(n-1)  t^(n-1)] * |2m/squircleWidth   0| * mat2x2(canvasMatrix)^-1
        //                                     |0  2n/squricleHeight|
        //
        ...
        float dx = fPower / (insetX * determinant);
        float dy = fPower / (insetY * determinant);
        ...
        // corner's space, the shader will have to take the absolute value, subtract
        // one_minus_1_over_inset, then clamp above zero.
        SkMatrix cornerToLocal;
        cornerToLocal.setScaleTranslate(insetX, insetY, superRRect.centerX(), superRRect.centerY());
