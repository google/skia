...
public:
    ...
    ConstantMatrixEffect(std::unique_ptr<GrFragmentProcessor> child)
        ...
        this->registerChildProcessor(
                std::move(child), SkSL::SampleMatrix::MakeConstUniform(
    ...
    GrGLSLFragmentProcessor* onCreateGLSLInstance() const override {
        class Impl : public GrGLSLFragmentProcessor {
            void emitCode(EmitArgs& args) override {
                ...
                fb->codeAppendf("float2 coord = %s / 64.0;", args.fLocalCoord);
...
DEF_SIMPLE_GPU_GM(fp_sample_chaining, ctx, rtCtx, canvas, 1024, 256) {
    ...
    // FIXME stroke_rect_shader doesn't get varying properly, same with strokedlines
    // Maybe something going on with yuv_splitter?


    // First row: no transform, then each one independently applied
    draw({});
    draw({ kConstant });  // Correct
    draw({ kUniform  });  // Correct now, Bug #1: No transform applied (see comment in UniformMatrixEffect)
    ...
    draw({ kUniform,  kUniform  });  // Correct now, Bug #2 (#1B): Only one transform applied
    draw({ kVariable, kVariable });  // Correct now, Bug #3: Only one transform applied (unknown issue)
    draw({ kExplicit, kExplicit });  // Correct
    nextRow();

    // Remember, these are applied inside out:
    draw({ kConstant, kUniform  });  // Correct (I think)... Bug #4 (#1C?): Shader error (unknown identifier 'matrix')
    draw({ kConstant, kVariable });  // Correct (I think).  Bug #5 (#3B?): Shader error (unknown identifier 'uCoordTransformMatrix_0_Stage0)
    draw({ kConstant, kExplicit });  // Correct
    nextRow();

   draw({ kExplicit, kConstant });  // correct? Bug #6: Shader error (unknown identifier 'uCoordTransformMatrix_0_Stage0)
   draw({ kExplicit, kUniform  });  // correct? Bug #7 (#6B?): Shader error (unknown identifier 'uCoordTransformMatrix_0_Stage0)
   draw({ kExplicit, kVariable });  // correct? Bug #8: SkASSERT(!hasVariableMatrix) in writeProcessorFunction

   nextRow();
   draw({ kConstant, kExplicit });
   draw({ kUniform, kExplicit });
   draw({ kVariable, kExplicit });
   draw({ kUniform, kVariable });
}
