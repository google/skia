/*
 * Copyright 2016 Google Inc.
 *
 * Use of this source code is governed by a BSD-style license that can be
 * found in the LICENSE file.
 */
#include "gm.h"
#include "SkCanvas.h"
#include "SkPath.h"

namespace skiagm {

//this test exercise SkCanvas::setDeviceClipRestriction behavior
class ComplexClip4GM : public GM {
public:
  ComplexClip4GM(bool aaclip)
    : fDoAAClip(aaclip) {
        this->setBGColor(0xFFDEDFDE);
    }

protected:


    SkString onShortName() {
        SkString str;
        str.printf("complexclip4_%s",
                   fDoAAClip ? "aa" : "bw");
        return str;
    }

    SkISize onISize() { return SkISize::Make(970, 780); }

    virtual void onDraw(SkCanvas* canvas) {
        SkPaint p;
        p.setAntiAlias(fDoAAClip);
        p.setColor(SK_ColorYELLOW);

        canvas->save();
            // draw a yellow rect through a rect clip
            canvas->setDeviceClipRestriction(SkIRect::MakeLTRB(100, 100, 300, 300));
            canvas->clipRect(SkRect::MakeLTRB(100, 100, 300, 300),
                             SkCanvas::kReplace_Op, fDoAAClip);
            canvas->drawColor(SK_ColorGREEN);
            canvas->save();
                canvas->clipRect(SkRect::MakeLTRB(100, 200, 400, 500),
                                 SkCanvas::kReplace_Op, fDoAAClip);
                canvas->drawRect(SkRect::MakeLTRB(100, 200, 400, 500), p);
            canvas->restore();

            // draw a yellow rect through a diamond clip
            canvas->setDeviceClipRestriction(SkIRect::MakeLTRB(500, 100, 800, 300));
            canvas->clipRect(SkRect::MakeLTRB(500, 100, 800, 300),
                             SkCanvas::kReplace_Op, fDoAAClip);
            canvas->drawColor(SK_ColorGREEN);
            canvas->save();
                SkPath pathClip;
                pathClip.moveTo(SkIntToScalar(650),  SkIntToScalar(200));
                pathClip.lineTo(SkIntToScalar(900), SkIntToScalar(300));
                pathClip.lineTo(SkIntToScalar(650), SkIntToScalar(400));
                pathClip.lineTo(SkIntToScalar(650), SkIntToScalar(300));
                pathClip.close();
                canvas->clipPath(pathClip, SkCanvas::kReplace_Op, fDoAAClip);
                canvas->drawRect(SkRect::MakeLTRB(500, 200, 900, 500), p);
            canvas->restore();

            // draw a yellow rect through a round rect clip
            canvas->setDeviceClipRestriction(SkIRect::MakeLTRB(500, 500, 800, 700));
            canvas->clipRect(SkRect::MakeLTRB(500, 500, 800, 700),
                             SkCanvas::kReplace_Op, fDoAAClip);
            canvas->drawColor(SK_ColorGREEN);
            canvas->save();
                canvas->clipRRect(SkRRect::MakeOval(SkRect::MakeLTRB(500, 600, 900, 750)),
                                 SkCanvas::kReplace_Op, fDoAAClip);
                canvas->drawRect(SkRect::MakeLTRB(500, 600, 900, 750), p);
            canvas->restore();

            // draw a yellow rect through a rect clip showing that intersect op
            //is not affected by clip restriction
            canvas->setDeviceClipRestriction(SkIRect::MakeLTRB(100, 400, 300, 750));
            canvas->clipRect(SkRect::MakeLTRB(100, 400, 300, 750),
                             SkCanvas::kReplace_Op, fDoAAClip);
            canvas->drawColor(SK_ColorGREEN);
            canvas->setDeviceClipRestriction(SkIRect::MakeLTRB(150, 450, 250, 700));
            canvas->clipRect(SkRect::MakeLTRB(100, 400, 300, 750),
                             SkCanvas::kIntersect_Op, fDoAAClip); //a replace op
            //would have been affected by clip restriction, but not intersect
            canvas->drawColor(SK_ColorYELLOW);

        canvas->restore();
    }
private:

    bool fDoAAClip;

    typedef GM INHERITED;
};

//////////////////////////////////////////////////////////////////////////////

DEF_GM(return new ComplexClip4GM(false);)
DEF_GM(return new ComplexClip4GM(true);)
}
