...
namespace skiagm {
class ImageFromYUVTexturesWithExternalTexture : public GM {
public:
    ImageFromYUVTexturesWithExternalTexture() { this->setBGColor(0xFFFFFFFF); }

protected:
    SkString onShortName() override {
        return SkString("image_from_yuv_textures_with_external_backend");
    }

    SkISize onISize() override { return SkISize::Make(50, 175); }
    ...
    void createYUVTextures(GrContext* context, GrBackendTexture yuvTextures[3]) {
        ...
        for (int i = 0; i < 3; ++i) {
            ...
            yuvTextures[i] = gpu->createTestingOnlyBackendTexture(
                    ...
                    kAlpha_8_GrPixelConfig, false, GrMipMapped::kNo);
        }
    ...
    void createExternalBackendTexture(GrContext* context, int width, int height,
        ...
        externalBackendTexture = gpu->createTestingOnlyBackendTexture(
                nullptr, width, height, kAlpha_8_GrPixelConfig, true, GrMipMapped::kNo);

        context->resetContext();
    }

    void deleteTextures(GrContext* context, GrBackendTexture yuvTextures[3],
                        GrBackendTexture externalBackendTexture) {
        if (context->abandoned()) {
            return;
        }
    ...
    void onDraw(SkCanvas* canvas) override {
        ...
        canvas->drawImage(fRGBImage.get(), kPad, kPad);
        sk_sp<SkImage> image;
        for (int space = kJPEG_SkYUVColorSpace, i = 1; space <= kLastEnum_SkYUVColorSpace;
             ++space, ++i) {
            ...
            image = SkImage::MakeFromTexture(context, externalBackendTexture,
                                             kTopLeft_GrSurfaceOrigin, kAlpha_8_SkColorType,
            ...
            this->deleteTextures(context, yuvTextures, externalBackendTexture);
        }
    }
...
DEF_GM(return new ImageFromYUVTexturesWithExternalTexture;)
}  // namespace skiagm
