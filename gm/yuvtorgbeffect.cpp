...
protected:
    ...
    DrawResult onDraw(GrContext* context, GrRenderTargetContext* renderTargetContext,
        ...
        static const GrSamplerState::Filter kFilters[] = { GrSamplerState::Filter::kNearest, GrSamplerState::Filter::kBilerp };
        static const SkRect kGreenRect = SkRect::MakeLTRB(0.25f * YSIZE, 0.25f * YSIZE, 0.75f * YSIZE, 0.75f * YSIZE);
        ...
        for (uint i = 0; i < SK_ARRAY_COUNT(kFilters); ++i) {
            SkScalar x = kDrawPad + kTestPad;
            ...
            for (uint j = 0; j < 2; ++j) {
                ...
                const SkRect* domain = j > 0 ? &kGreenRect : nullptr;
                std::unique_ptr<GrFragmentProcessor> fp(GrYUVtoRGBEffect::Make(proxies, yuvaIndices, kJPEG_SkYUVColorSpace, kFilters[i], domain));
                if (fp) {
                    GrPaint grPaint;
                    grPaint.addColorFragmentProcessor(std::move(fp));
