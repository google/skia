...
typedef std::function< sk_sp<SkShader>(sk_sp<SkShader>, sk_sp<SkShader>, SkBlendMode, float) > ShaderMaker;

static void do_mixershader(SkCanvas* canvas, const ShaderMaker& maker) {
...
DEF_SIMPLE_GM(mixershader2, canvas, 500, 500) {
    do_mixershader(canvas, [](sk_sp<SkShader> a, sk_sp<SkShader> b, SkBlendMode mode, float t) {
        ...
        auto mx = SkMixer::MakeLerp(t);
        return SkShader::MakeMixer(a, b, mx->makeSplit(SkMixer::MakeFirst(),
                                                       blender->makeReverse()));
    });
}
