...
public:
    ...
    static void show_bounds(SkCanvas* canvas, const SkPaint& paint, const SkFont& font,
                            SkScalar x, SkScalar y, SkColor boundsColor)
    {
        SkRect fontBounds = SkFontPriv::GetFontBounds(font);
        fontBounds.offset(x, y);
        SkPaint boundsPaint(paint);
        ...
        SkGlyphID left = 0, right = 0, top = 0, bottom = 0;
        {
            ...
            for (int i = 0; i < numGlyphs; ++i) {
                SkGlyphID glyphId = i;
                SkRect cur;
                font.measureText(&glyphId, sizeof(glyphId), kGlyphID_SkTextEncoding, &cur);
        ...
        for (size_t i = 0; i < SK_ARRAY_COUNT(str); ++i) {
            ...
            SkPaint p(paint);
            p.setStyle(style);
            canvas->drawSimpleText(&str[i], sizeof(str[0]), kGlyphID_SkTextEncoding, x, y, font, p);
        ...
        for (int i = 0; i < count; ++i) {
            ...
            for (int j = 0; j < set->count() && j < 3; ++j) {
                ...
                if (font.getTypeface() && font.getTypeface()->countGlyphs() < 1000) {
                    SkRect fontBounds = SkFontPriv::GetFontBounds(font);
                    x -= fontBounds.fLeft;
                    show_bounds(canvas, paint, font, x, y, boundsColors[index & 1]);
