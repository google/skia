...
static SkScalar draw_string(SkAtlasTextTarget* target, const SkString& text, SkScalar x,
                           SkScalar y, uint32_t color, sk_sp<SkTypeface> typeface, float size) {
...
protected:
    ...
    void onDraw(SkCanvas* canvas) override {
        /** Don't run on GPU configs to avoid issues with multiple GL contexts. */
        if (!fRenderer) {
...
constexpr SkScalar AtlasTextGM::kSize;
