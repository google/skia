...
#define SKSL_MATRIX_SELECTORS                                       \
    ...
    " return step(   float2(0.0, 0.5), float2(x))"                  \
    "      * step(x, float2(0.5, 1.0));"                            \
    "}"                                                             \
    "inline float3 sel3(float x) {"                                 \
    " return step(   float3(0.0,  0.33, 0.66), float3(x))"          \
    "      * step(x, float3(0.33, 0.66, 1.0 ));"                    \
    "}"                                                             \
    "inline float4 sel4(float x) {"                                 \
    "    return step(   float4(0.0,  0.25, 0.5,  0.75), float4(x))" \
    "         * step(x, float4(0.25, 0.5,  0.75, 1.0 ));"           \
    "}"
