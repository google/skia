...
DEF_SIMPLE_GM_BG(runtime_intrinsics_relational,
    ...
    plot_bvec(canvas, "equal(lessThanEqual(p, v1), greaterThanEqual(p, v1))", "equal(bvec)"); col(canvas);
    plot_bvec(canvas, "notEqual(lessThanEqual(p, v1), greaterThanEqual(p, v1))", "notequal(bvec)"); row(canvas);
