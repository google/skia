...
static void plot(SkCanvas* canvas,
    ...
    auto [effect, error] = SkRuntimeShaderEffect::Make(make_unary_sksl_1d(fn));
    if (!effect) {
        SkDebugf("Error: %s\n", error.c_str());
        return;
    }
...
static void plot_matrix_comp_mult(SkCanvas* canvas,
    ...
    auto [effect, error] = SkRuntimeShaderEffect::Make(make_matrix_comp_mult_sksl(N));
    if (!effect) {
        SkDebugf("Error: %s\n", error.c_str());
        return;
    }
...
static void plot_matrix_inverse(SkCanvas* canvas, std::array<float, N*N> mtx, const char* label) {
    ...
    auto [effect, error] = SkRuntimeShaderEffect::Make(make_matrix_inverse_sksl(N));
    if (!effect) {
        SkDebugf("Error: %s\n", error.c_str());
        return;
    }
...
static void plot_bvec(SkCanvas* canvas, const char* fn, const char* label) {
    ...
    auto [effect, error] = SkRuntimeShaderEffect::Make(make_bvec_sksl(type, fn));
    if (!effect) {
        SkDebugf("Error: %s\n", error.c_str());
        return;
    }
