...
DEF_SIMPLE_GM_BG(runtime_intrinsics_common,
    ...
    do_unary(canvas, "fract(x)", -3.0f, 3.0f,  0.0f, 1.0f); row(canvas);
...
//    do_unary(canvas, "mod(p, -2).x", -4.0f, 4.0f, -2.0f, 2.0f); col(canvas);
    ...
    do_unary(canvas, "min(p, v1).x", 0.0f, 2.0f, 0.0f, 2.0f); row(canvas);
    ...
    do_unary(canvas, "clamp(p, v1, v2).x", 0.0f, 3.0f, 0.0f, 3.0f); row(canvas);
    ...
    do_unary(canvas, "step(v1, p).x", 0.0f, 2.0f, -0.5f, 1.5f); row(canvas);
