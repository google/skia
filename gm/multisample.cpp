...
DEF_SIMPLE_GM(multisample, canvas, 512,512) {
    ...
    // Sample `gradient` at given N relative points with optional weights, 1.0f/N by default.
    auto multisample = [&](std::initializer_list<SkPoint>  samples,
    ...
    // One sample at the center, the same as drawing `gradient` directly.
    multisample({{0.0f,0.0f}}, {});

    canvas->translate(256,0);

    // 2 samples
    multisample({{-0.25f,-0.25f},
                 {+0.25f,+0.25f}}, {});

    canvas->translate(-256,+256);

    // 4 samples in somewhat common arrangement
    multisample({{ 4/9.0f - 0.5f, 2/9.0f - 0.5f },
