...
CheckPixelColorNearby::CheckPixelColorNearby(int pixelRadius, float colorDistanceThreshold,
...
SkString CheckPixelColorNearby::name() const { return SkString("CheckPixelColorNearby"); }

VerifierResult CheckPixelColorNearby::verifyWithGold(const SkIRect& region, const SkBitmap& gold,
    ...
    return numFailedPixels <= failedPixelThreshold
                   ? VerifierResult::Ok()
                   : makeError(SkStringPrintf("%d failed pixels is above threshold of %d",
                                              numFailedPixels, failedPixelThreshold));
}
