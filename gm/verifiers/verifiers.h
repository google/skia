...
public:
    explicit CheckPixelColorNearby(int pixelRadius = 1, float colorDistanceThreshold = 0.07f,
                                   float percentPixelDifferenceThreshold = 0.01f);

    SkString name() const override;

protected:
    VerifierResult verifyWithGold(const SkIRect& region, const SkBitmap& gold,
                                  const SkBitmap& actual) override;
