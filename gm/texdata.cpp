...
DEF_SIMPLE_GM_BG(texdata, canvas, 2 * S, 2 * S, SK_ColorBLACK) {
    ...
    const SkImageInfo ii = SkImageInfo::Make(S, S, kRGBA_8888_SkColorType, kPremul_SkAlphaType);
    ...
    for (int i = 0; i < 2; ++i) {
        ...
        paint.addColorTextureProcessor(sk_ref_sp(tContext->asDeferredTexture()), nullptr, vm);
