 ...
 * images on gold.
 */
class TessellationGM : public skiagm::GpuGM {
...
public:
    TessellationTestTriShader() : GrGeometryProcessor(kTessellationTestTriShader_ClassID) {
...
private:
    ...
    class Impl : public GrGLSLGeometryProcessor {
        ...
        void onEmitCode(EmitArgs& args, GrGPArgs*) override {
            ...
            args.fVertBuilder->codeAppend("P_ = position;");
    ...
    void writeTessControlShaderGLSL(SkString* code) const override;
    void writeTessEvaluationShaderGLSL(SkString* code) const override;
};

void TessellationTestTriShader::writeTessControlShaderGLSL(SkString* code) const {
    code->append(R"(
...
void TessellationTestTriShader::writeTessEvaluationShaderGLSL(SkString* code) const {
    code->append(R"(
            precision highp float;
            ...
            i = abs(mod(i, 2) - 1);
            barycentric_coord = vec3(i, 0, 1 - i);
        })");
}
...
public:
    TessellationTestRectShader() : GrGeometryProcessor(kTessellationTestRectShader_ClassID) {
        this->setWillUseTessellationShaders();
    }
...
void TessellationTestRectShader::writeTessControlShaderGLSL(SkString* code) const {
    code->append(R"(
            precision highp float;
...
void TessellationTestRectShader::writeTessEvaluationShaderGLSL(SkString* code) const {
    code->appendf(R"(
            precision highp float;
            ...
            void main() {
                vec4 R = vec4(%f, %f, %f, %f);
                vec2 devcoord = mix(R.xy, R.zw, gl_TessCoord.xy);
                // devcoord = round(devcoord - .5) + .5;  // Make horz and vert lines on px bounds.
                gl_Position = vec4(devcoord.xy * sk_RTAdjust.xz + sk_RTAdjust.yw, 0.0, 1.0);

                float i = gl_TessCoord.x * 8;
                i = abs(mod(i, 2) - 1);
                if (gl_TessCoord.y == 0 || gl_TessCoord.y == 1) {
                    barycentric_coord = vec4(i, 1 - i, 0, 0);
                } else {
                    barycentric_coord = vec4(0, 0, i, 1 - i);
                }
...
public:
    TessellationTestOp(const std::array<float, 3>* triPositions)
            : GrDrawOp(ClassID()), fTriPositions(triPositions) {
...
private:
    ...
    void onExecute(GrOpFlushState* state, const SkRect& chainBounds) override {
        ...
        if (fTriPositions) {
            ...
            shader = std::make_unique<TessellationTestTriShader>();
        } else {
            ...
            shader = std::make_unique<TessellationTestRectShader>();
        }
...
void TessellationGM::onDraw(GrContext* ctx, GrRenderTargetContext* rtc, SkCanvas* canvas) {
    ...
    rtc->priv().testingOnly_addDrawOp(pool->allocate<TessellationTestOp>(kTri1));
    rtc->priv().testingOnly_addDrawOp(pool->allocate<TessellationTestOp>(kTri2));
    rtc->priv().testingOnly_addDrawOp(pool->allocate<TessellationTestOp>(nullptr));
}
