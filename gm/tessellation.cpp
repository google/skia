...
private:
    ...
    void createProgramInfo(const GrCaps* caps,
        ...
        GrGeometryProcessor* shader;
        if (fTriPositions) {
            if (!fVertexBuffer) {
                return;
            }
            tessellationPatchVertexCount = 3;
            shader = TessellationTestTriShader::Make(arena, fViewMatrix);
        } else {
            ...
            shader = TessellationTestRectShader::Make(arena, fViewMatrix);
        }

        fProgramInfo = GrSimpleMeshDrawOpHelper::CreateProgramInfo(caps, arena, outputView,
                                                                   std::move(appliedClip),
                                                                   dstProxyView, shader,
                                                                   GrProcessorSet(SkBlendMode::kSrc),
