...
static sk_sp<SkData> load_dds(const char* filename, ImageInfo* imageInfo) {
    ...
    if (header.dwFlags & kDDSD_MIPMAPCOUNT) {
        if (header.dwMipMapCount == 1) {
            ...
            if (header.dwMipMapCount != numRequiredLevels) {
                return nullptr;
            }
            imageInfo->fMipMapped = GrMipMapped::kYes;
...
protected:
    ...
    void loadImages(GrContext *context) {
        ...
        if (!fETC1Image) {
            const char* filename = GetResourcePath("images/flower-etc1.ktx").c_str();

            ImageInfo info;
            sk_sp<SkData> data = load_ktx(filename, &info);
            if (data) {
        ...
        if (!fBC1Image) {
            const char* filename = GetResourcePath("images/flower-bc1.dds").c_str();

            ImageInfo info;
            sk_sp<SkData> data = load_dds(filename, &info);
            if (data) {
                SkASSERT(info.fDim.equals(kImgWidthHeight, kImgWidthHeight));
                SkASSERT(info.fMipMapped == GrMipMapped::kNo);
