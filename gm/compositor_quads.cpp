...
public:
    ...
    int drawTile(SkCanvas* canvas, const SkRect& rect, const SkPoint clip[4], const bool edgeAA[4],
                  int tileID, int quadID) override {
        canvas->experimental_DrawEdgeAAQuad(rect, clip, clip ? 4 : 0, this->maskToFlags(edgeAA),
...
public:
    ...
    int drawTile(SkCanvas* canvas, const SkRect& rect, const SkPoint clip[4], const bool edgeAA[4],
        ...
        fSetEntries.push_back(
                {fImage, localRect, rect, clipCount, matrixIdx, 1.f, this->maskToFlags(edgeAA)});
...
private:
    ...
    int drawAndReset(SkCanvas* canvas) {
        ...
        for (int i = 0; i < fSetEntries.count(); ++i) {
            expectedDstClipCount += fSetEntries[i].fDstClipCount;
