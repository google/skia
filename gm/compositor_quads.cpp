    ...
    void drawTiles(SkCanvas* canvas, GrContext* context, GrRenderTargetContext* rtc) {
        ...
        for (int i = 0; i < kRowCount; ++i) {
            for (int j = 0; j < kColCount; ++j) {
                ...
                tile.toQuad(quad);
                int quadCount = 0;
                if (!this->clipTile(canvas, tileID, tile, quad, edgeAA, lines, 3, &quadCount)) {
                    // Wasn't clipped so draw it directly as a rectangle
                    this->drawTile(canvas, tile, nullptr, edgeAA, tileID, quadCount);
                    quadCount++;
                }
                tileID++;
            }
...
protected:
    ...
    bool clipTile(SkCanvas* canvas, int tileID, const SkRect& baseRect, const SkPoint tile[4],
        ...
        if (intersectionCount == 2) {
            ...
            for (int i = 0; i < subtiles.count(); ++i) {
                ...
                bool split = this->clipTile(
                        canvas, tileID, baseRect, sub, subAA, lines + 2, lineCount - 1, quadCount);
                if (!split) {
                    // The quad was not further split, which means we're responsible for sending it
                    // to the renderer.
                    this->drawTile(canvas, baseRect, sub, subAA, tileID, *quadCount);
            ...
            return this->clipTile(
                    canvas, tileID, baseRect, tile, edgeAA, lines + 2, lineCount - 1, quadCount);
        }
    }
};
...
protected:
    ...
    void onDraw(GrContext* ctx, GrRenderTargetContext* rtc, SkCanvas* canvas) override {
        ...
        static constexpr SkScalar kBannerWidth = 110.f;
