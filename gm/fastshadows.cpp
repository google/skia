...
static void draw_row(SkCanvas* canvas, int row, int width) {
    ...
    canvas->readPixels(SkIRect::MakeXYWH(0, row, width, 1), &readback);
...
// This GM exercises the SkGaussianEdgeShader. It draws four columns showing filled,
// stroked, and stroke and filled rendering plus a visualization of the rendered curves.
// It draws three rows showing a blur radius smaller than, equal to
// and, finally, larger than the RRect's corner radius
class FastShadowsGM : public GM {
public:
    FastShadowsGM() {
...
protected:
    ...
    SkString onShortName() override {
        return SkString("fastshadows");
    }

    SkISize onISize() override {
        return SkISize::Make(kPad+(kNumBaseCols+1)*(kCellWidth+kPad),
...
DEF_GM(return new FastShadowsGM;)
}
