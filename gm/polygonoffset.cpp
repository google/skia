...
const SkPoint* gSimplePoints[] = {
    ...
    gPoints8, gPoints13, gPoints11, gPoints12, gPoints14, gPoints15,
    gPoints16,
};
...
protected:
    ...
    static void GetConvexPath(int index, SkPath::Direction dir,
        ...
        if (index < (int)SK_ARRAY_COUNT(PolygonOffsetData::gConvexPoints)) {
            ...
            *numPts = (int)PolygonOffsetData::gConvexSizes[index];
            ...
            int arrayIndex = index - SK_ARRAY_COUNT(PolygonOffsetData::gConvexPoints);
            SkASSERT(arrayIndex >= 0 && arrayIndex < SK_ARRAY_COUNT(numPtsArray));
    ...
    static void GetSimplePath(int index, SkPath::Direction dir,
                              std::unique_ptr<SkPoint[]>* data, int* numPts) {
        if (index < (int)SK_ARRAY_COUNT(PolygonOffsetData::gSimplePoints)) {
            ...
            *numPts = (int)PolygonOffsetData::gSimpleSizes[index];
            ...
            SkScalar width = kMaxPathHeight / 2;
            SkScalar height = kMaxPathHeight / 2;
            int numPtsArray[] = { 3, 4, 5, 7, 8, 20, 100 };
            ...
            SkASSERT(arrayIndex >= 0 && arrayIndex < SK_ARRAY_COUNT(numPtsArray));
            *numPts = numPtsArray[arrayIndex];
            if (arrayIndex >= 0 && arrayIndex <= 2 || arrayIndex == 4) {
                // squashed diamond, pentagon, and octagon
                width = kMaxPathHeight / 5;
            }
    ...
    // Draw a single path with insets and potentially outsets
    void drawPath(SkCanvas* canvas, int index, SkPoint* offset) {
        ...
        SkPoint center;
        {
            ...
            int numPts;
            if (fConvexOnly) {
                GetConvexPath(index, SkPath::kCW_Direction, &data, &numPts);
            } else {
                GetSimplePath(index, SkPath::kCW_Direction, &data, &numPts);
            }
            SkRect bounds;
        ...
        const float offsets[] = { 5, 10, 15, 20, 25, 30, 35, 40, -5, -10, -15 };
        ...
        int numPts;
        if (fConvexOnly) {
            GetConvexPath(index, dirs[index % 2], &data, &numPts);
        } else {
            GetSimplePath(index, dirs[index % 2], &data, &numPts);
        }
