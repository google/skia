...
static float lum(float r, float g, float b) { return r*0.30f + g*0.59f + b*0.11; }
...
// The two SetSat() routines in the specs look different, but they're actually equivalent.
...
static void hue(float  dr, float  dg, float  db,
                ...
                void (*clip_color)(float*,float*,float*)) {
    // Hue of Src, Saturation and Luminance of Dst.
    float R = *sr, G = *sg, B = *sb;
    set_sat(&R,&G,&B, sat(dr,dg,db));
    set_lum(&R,&G,&B, lum(dr,dg,db), clip_color);
    *sr = R; *sg = G; *sb = B;
}
...
static void saturation(float  dr, float  dg, float  db,
    ...
    set_lum(&R,&G,&B, lum( dr, dg, db), clip_color);  // This may seem redundant, but it's not.
    *sr = R; *sg = G; *sb = B;
}
...
static void luminosity(float  dr, float  dg, float  db,
    ...
    float R = dr, G = dg, B = db;
    set_lum(&R,&G,&B, lum(*sr,*sg,*sb), clip_color);
    *sr = R; *sg = G; *sb = B;
}
...
static SkColor blend(SkColor dst, SkColor src,
    ...
    SkASSERT(SkColorGetA(dst) == 0xff
          && SkColorGetA(src) == 0xff);   // Not fundamental, just simplifying here.
    ...
    mode(dr,dg,db, &sr,&sg,&sb, clip_color);

    return SkColorSetRGB(sr * 255.0f + 0.5f,
                         sg * 255.0f + 0.5f,
                         sb * 255.0f + 0.5f);
}
...
DEF_SIMPLE_GM(hsl, canvas, 600, 100) {
    ...
    struct {
        SkBlendMode mode;
        void (*reference)(float,float,float, float*,float*,float*, void(*)(float*,float*,float*));
    } tests[] = {
    ...
    for (auto test : tests) {
        ...
        if (test.reference) {
            ...
            canvas->drawArc({30,30,70,70},   0,180, true, KHR);
            canvas->drawArc({30,30,70,70}, 180,360, true, web);
        }
