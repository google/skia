...
protected:
    ...
    void onOnceBeforeDraw() override {
        ...
        if (fUseObject) {
            ...
            std::copy(fPts, fPts + kMeshVertexCnt, points.get());
            std::copy(fTexs, fTexs + kMeshVertexCnt, texs.get());
            std::copy(fColors, fColors + kMeshVertexCnt, colors.get());
            std::copy(kMeshFan, kMeshFan + kMeshIndexCnt, indices.get());
            fVertices = SkVertices::Make(SkCanvas::kTriangleFan_VertexMode, std::move(points),
            ...
            for (uint8_t alpha : {0xFF, 0x80}) {
                for (const auto& cf : {sk_sp<SkColorFilter>(nullptr), fColorFilter}) {
                    for (const auto& shader : {fShader1, fShader2}) {
                        // This part is a bit odd in order to not change the image rendered by an
                        // older revision.
                        ...
                        for (auto attrs : kAttrs) {
                            ...
                            if (fUseObject) {
                                uint32_t flags = 0;
                                flags |= attrs.fHasColors ? 0 : SkCanvas::kIgnoreColors_VertexFlag;
                                flags |= attrs.fHasTexs ? 0 : SkCanvas::kIgnoreTexCoords_VertexFlag;
...
static void batching(SkCanvas* canvas, bool useObject) {
    ...
    if (useObject) {
        vertices =
                SkVertices::Make(SkCanvas::kTriangles_VertexMode, std::move(pts), std::move(colors),
                                 std::move(texs), kMeshVertexCnt, std::move(indices), 3 * kNumTris);
    }
    canvas->save();
    ...
    for (bool useShader : {false, true}) {
        for (bool useTex : {false, true}) {
            for (const auto& m : matrices) {
                ...
                if (useObject) {
                    uint32_t flags = useTex ? 0 : SkCanvas::kIgnoreTexCoords_VertexFlag;
...
DEF_SIMPLE_GM(vertices_batching, canvas, 100, 500) {
    batching(canvas, false);
    canvas->translate(50, 0);
    batching(canvas, true);
}
