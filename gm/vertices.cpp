    ...
    void onOnceBeforeDraw() override {
        ...
        if (fUseObject) {
            ...
            fVertices = SkVertices::MakeIndexed(
                    ...
                    std::unique_ptr<const uint16_t[]>((const uint16_t*)indices.release()), kMeshIndexCnt);
        }
    }
...
static void draw_batching(SkCanvas* canvas, bool useObject) {
    ...
    if (useObject) {
        ...
        vertices = SkVertices::MakeIndexed(
                SkCanvas::kTriangles_VertexMode, std::unique_ptr<const SkPoint[]>((const SkPoint*)pts.release()),
                std::unique_ptr<const SkColor[]>((const SkColor*)colors.release()),
                std::unique_ptr<const SkPoint[]>((const SkPoint*)texs.release()), kMeshVertexCnt,
                std::unique_ptr<const uint16_t[]>((const uint16_t*)indices.release()), 3 * kNumTris);
    }
    canvas->save();
