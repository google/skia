    ...
    void onOnceBeforeDraw() override {
        ...
        if (fUseObject) {
            fVertices = SkVertices::MakeCopy(SkCanvas::kTriangleFan_VertexMode, kMeshVertexCnt,
                                             fPts, fColors, fTexs, kMeshFan, nullptr);
            SkASSERT(fVertices->vertexCount() == kMeshVertexCnt);
            SkASSERT(fVertices->indexCount() == kMeshIndexCnt);
        }
    }
...
static void draw_batching(SkCanvas* canvas, bool useObject) {
    SkVertices::Builder builder(SkCanvas::kTriangles_VertexMode, kMeshVertexCnt,
                                SkVertices::kHasColors_Flag | SkVertices::kHasTexs_Flag
                                                            | SkVertices::kHasIndices_Flag,
                                nullptr);
    ...
    SkASSERT(builder.indexCount() == 3 * kNumTris);
