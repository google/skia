...
protected:
    ...
    virtual void onDraw(SkCanvas* canvas) {
        ...
        skiagm::DrawShapedString(canvas, "I", 0, 100, font, paint);
    }
private:
...
protected:
    ...
    virtual void onDraw(SkCanvas* canvas) {
        ...
        skiagm::DrawShapedString(canvas, "Normal Fill Text", 0, 50, font, paint);
        paint.setStyle(SkPaint::kStroke_Style);
        skiagm::DrawShapedString(canvas, "Normal Stroke Text", 0, 100, font, paint);

        // Minimal repro doesn't require AA, LCD, or a nondefault typeface
        paint.setShader(make_chrome_solid());

        paint.setStyle(SkPaint::kFill_Style);
        skiagm::DrawShapedString(canvas, "Gradient Fill Text", 0, 150, font, paint);
        paint.setStyle(SkPaint::kStroke_Style);
        skiagm::DrawShapedString(canvas, "Gradient Stroke Text", 0, 200, font, paint);
    }
private:
    typedef GM INHERITED;
};
    ...
    for (int i = 0; i < 2; ++i) {
        for (const SkPaint& paint : paints) {
            for (SkFont::Edging edging : edgings) {
                ...
                skiagm::DrawShapedString(canvas, "When in the course of human events", 0, 0, font,
                                         paint);
