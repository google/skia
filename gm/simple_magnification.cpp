...
protected:
    ...
    void draw(SkCanvas* canvas, sk_sp<SkImage> image, const SkIPoint& offset, int inset) {
        ...
        SkPaint paint;
        paint.setImageFilter(magFilter);
    ...
    void onDraw(SkCanvas* canvas) override {
        ...
        //canvas->drawImage(bottomLImg, kPad, kPad);
        //canvas->drawImage(topLImg, 2.0f * kPad + kRectSize, kPad);

        int bgOffset = 2 * kPad + kImgSize;

        this->draw(canvas, bottomLImg, SkIPoint::Make(kPad, kPad), 1);
        this->draw(canvas, topLImg, SkIPoint::Make(bgOffset, kPad), 1);
        this->draw(canvas, bottomLImg, SkIPoint::Make(kPad, bgOffset), 7);
        this->draw(canvas, topLImg, SkIPoint::Make(bgOffset, bgOffset), 7);
    }
