...
DEF_SIMPLE_GM(image_to_yuv_planes, canvas, 120, 525) {
    ...
    for (size_t s = 0; s < SK_ARRAY_COUNT(kSizes); ++s) {
        ...
        sk_up<uint8_t[]> yPlane(new uint8_t[realRowBytes[0] * sizes[0].fHeight]);
        sk_up<uint8_t[]> uPlane(new uint8_t[realRowBytes[1] * sizes[1].fHeight]);
        sk_up<uint8_t[]> vPlane(new uint8_t[realRowBytes[2] * sizes[2].fHeight]);
