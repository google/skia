...
private:
    ...
    void onOnceBeforeDraw() override {
        ...
        auto surf = SkSurface::MakeRaster(SkImageInfo::Make(kW, kH, kRGBA_8888_SkColorType, kPremul_SkAlphaType));
        ...
        auto grad = SkGradientShader::MakeLinear(pts1, kColors1, nullptr, 2, SkShader::kClamp_TileMode);
        ...
        for (int y = 0; y < kN; ++y) {
            for (int x = 0; x < kM; ++x) {
                fImage[x][y] = img->makeSubset(SkIRect::MakeXYWH(x * kTileW, y * kTileH, kTileW, kTileH));
            }
        }
    }
    ...
    void onDraw(SkCanvas* canvas) override {
        ...
        matrices[0].postTranslate(200, 0);
        // perespective
        SkPoint src[4];
        SkRect::MakeWH(kM * kTileW, kN * kTileH).toQuad(src);
        SkPoint dst[4] = {{0, 0}, {kM * kTileW + 10.f, -5.f},
                          {kM * kTileW - 28.f, kN * kTileH + 40.f}, {45.f, kN * kTileH - 25.f}};
        matrices[1].setPolyToPoly(src, dst, 4);
        matrices[1].postTranslate(d, 10.f);
        matrices[2].setRotate(-60.f);
        matrices[2].postSkew(0.5f, -1.35f);
        matrices[2].postScale(0.6, 1.05);
        ...
        for (size_t m = 0; m < SK_ARRAY_COUNT(matrices); ++m) {
            ...
            for (int x = 1; x < kM; ++x) {
                ...
                SkRect lineRect = SkRect::MakeLTRB(x * kTileW - kLineHalfW, -kLineOutset,
                                                   x * kTileW + kLineHalfW,
                                                   kN * kTileH + kLineOutset);
                rtc->drawRect(GrNoClip(), std::move(paint), GrAA::kYes, matrices[m], lineRect);
            }
            for (int y = 1; y < kN; ++y) {
                GrPaint paint;
                paint.setColor4f(color);
                SkRect lineRect = SkRect::MakeLTRB(-kLineOutset, y * kTileH - kLineHalfW,
                                                   kTileW * kM + kLineOutset,
                                                   y * kTileH + kLineHalfW);
                rtc->drawRect(GrNoClip(), std::move(paint), GrAA::kYes, matrices[m], lineRect);
            }
            //static constexpr GrColor kColor = 0xB0B0B0B0;
            static constexpr GrColor kColor = GrColor_WHITE;
            for (int x = 0; x < kM; ++x) {
                for (int y = 0; y < kN; ++y) {
                    auto proxy = as_IB(fImage[x][y])->asTextureProxyRef(ctx,
