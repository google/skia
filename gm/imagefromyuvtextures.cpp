...
protected:
    ...
    void deleteAllTextures(GrContext* context, GrBackendTexture yuvTextures[3],
        ...
        deleteYUVTextures(context, yuvTextures);
    ...
    void onDraw(SkCanvas* canvas) override {
        ...
        for (int space = kJPEG_SkYUVColorSpace, i = images.count();
            ...
            SkImage::MakeFromYUVTexturesCopyWithExternalBackend(context,
                                                                static_cast<SkYUVColorSpace>(space),
                                                                yuvTextures,
                                                                kTopLeft_GrSurfaceOrigin,
                                                                externalBackendTexture[0]);

            // Check whether yuv->rgb pixels fill into external backend texture.
            image = SkImage::MakeFromTexture(context, externalBackendTexture[0],
