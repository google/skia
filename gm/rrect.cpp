...
DEF_SIMPLE_GPU_GM_BG(
    ...
    // 1. GrOvalOpFactory
    GrPaint p1;
    p1.setColor4f({1.f, 0.f, 0.f, 1.f});
    p1.setPorterDuffXPFactory(SkBlendMode::kPlus);
    rtc->priv().testingOnly_addDrawOp(GrOvalOpFactory::MakeRRectOp(context, std::move(p1), SkMatrix::I(), giant, SkStrokeRec(SkStrokeRec::kFill_InitStyle), context->priv().caps()->shaderCaps()));

    // 2. GrFillRRectOp
    GrPaint p2;
    p2.setColor4f({0.f, 1.f, 0.f, 1.f});
    p2.setPorterDuffXPFactory(SkBlendMode::kPlus);
    rtc->priv().testingOnly_addDrawOp(GrFillRRectOp::Make(context, std::move(p2), SkMatrix::I(), giant, GrAAType::kCoverage));

    // 3. GrRRectEffect
    GrPaint p3;
    p3.setColor4f({0.f, 0.f, 1.f, 1.f});
    ...
    rtc->priv().testingOnly_addDrawOp(GrFillRectOp::MakeNonAARect(context, std::move(p3), SkMatrix::I(), giant.getBounds()));
}
