...
static sk_sp<SkImage> do_read_and_scale(Src* src,
    ...
    auto rescaling = SkRescalingMode(quality);
    src->asyncRescaleAndReadPixels(ii, srcRect, rescaleGamma, rescaling, async_callback,
                                   asyncContext);
    if (direct) {
        direct->submit();
    }
...
static sk_sp<SkImage> do_read_and_scale_yuv(Src* src,
    ...
    src->asyncRescaleAndReadPixelsYUV420(yuvCS, SkColorSpace::MakeSRGB(), srcRect, size,
                                         rescaleGamma, rescaling, async_callback, &asyncContext);
    if (direct) {
        direct->submit();
    }
    while (!asyncContext.fCalled) {
