...
static sk_sp<SkImage> do_read_and_scale_yuv(SkSurface* surface, SkYUVColorSpace yuvCS,
    ...
    auto callback = [](void* c, const void* data[3], size_t rowBytes[3]) {
        ...
        context->fSucceeded = true;
        SkRectMemcpy(context->fYData.writable_addr(), context->fYData.rowBytes(), data[0], rowBytes[0],
                     context->fYData.width(), context->fYData.height());
        SkRectMemcpy(context->fUData.writable_addr(), context->fUData.rowBytes(), data[1], rowBytes[1],
                     context->fUData.width(), context->fUData.height());
        SkRectMemcpy(context->fVData.writable_addr(), context->fVData.rowBytes(), data[2], rowBytes[2],
