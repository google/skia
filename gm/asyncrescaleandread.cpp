...
static sk_sp<SkImage> do_read_and_scale(
        ...
        SkSurface::RescaleLinear rescaleLiner, SkFilterQuality quality,
    ...
    surf->asyncRescaleAndReadPixels(ii, srcRect, rescaleLiner, quality, callback, &context);
...
static void do_rescale_grid(SkCanvas* canvas, const char* imageFile, const SkIRect& srcRect,
                            int newW, int newH) {
    ...
    canvas->save();
    for (auto linear : {SkSurface::RescaleLinear::kNo, SkSurface::RescaleLinear::kYes}) {
        canvas->save();
...
#define DEF_RESCALE_AND_READ_GM(IMAGE_FILE, TAG, SRC_RECT, W, H)                  \
    DEF_SIMPLE_GM(async_rescale_and_read_##TAG, canvas, 3 * W, 2 * H) {           \
        ToolUtils::draw_checkerboard(canvas, SK_ColorDKGRAY, SK_ColorLTGRAY, 25); \
        do_rescale_grid(canvas, #IMAGE_FILE, SRC_RECT, W, H);                     \
    }
