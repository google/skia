...
static sk_sp<SkImage> do_read_and_scale_yuv(SkSurface* surface, SkYUVColorSpace yuvCS,
    ...
    surface->asyncRescaleAndReadPixelsYUV420(yuvCS, SkColorSpace::MakeSRGB(), srcRect, dstW, dstH, rescaleGamma, quality, callback, &context);
    while (!context.fCalled) {
        // Only GPU should actually be asynchronous.
        SkASSERT(surface->getCanvas()->getGrContext());
        surface->getCanvas()->getGrContext()->checkAsyncWorkCompletion();
    ...
    backendTextures[0] = gr->priv().getGpu()->createTestingOnlyBackendTexture(dstW, dstH, kAlpha_8_SkColorType, GrMipMapped::kNo, GrRenderable::kNo, yData.get());
    backendTextures[1] = gr->priv().getGpu()->createTestingOnlyBackendTexture(dstW / 2, dstH / 2, kRGBA_8888_SkColorType, GrMipMapped::kNo, GrRenderable::kNo, uvxxData.get());
    bool yIsRed = backendTextures[0].pixelConfig() == kAlpha_8_as_Red_GrPixelConfig;
    ...
    *cleanup = {[gr, backendTextures] {
    ...
    return SkImage::MakeFromYUVATextures(gr, yuvCS, backendTextures, indices, {dstW, dstH}, kTopLeft_GrSurfaceOrigin, SkColorSpace::MakeSRGB());
}
...
static skiagm::DrawResult do_rescale_grid(SkCanvas* canvas, SkSurface* surface,
    ...
    for (auto gamma : {SkSurface::RescaleGamma::kSrc, SkSurface::RescaleGamma::kLinear}) {
        ...
        for (auto quality : {kNone_SkFilterQuality, kLow_SkFilterQuality, kHigh_SkFilterQuality}) {
            ...
            if (doYUV420) {
                result = do_read_and_scale_yuv(surface, yuvColorSpace, srcRect, newW, newH, gamma, quality, &cleanup);
...
#define DEF_RESCALE_AND_READ_YUV_GM(IMAGE_FILE, TAG, SRC_RECT, W, H)                     \
    DEF_SIMPLE_GM_CAN_FAIL(async_rescale_and_read__yuv420_##TAG, canvas, errorMsg, 3 * W, 2 * H) {   \
        ToolUtils::draw_checkerboard(canvas, SK_ColorDKGRAY, SK_ColorLTGRAY, 25);            \
