...
static sk_sp<SkImage> do_read_and_scale(Src* src,
    ...
    src->asyncRescaleAndReadPixels(ii, srcRect, rescaleGamma, quality, async_callback, context);
    if (grCtx) {
        grCtx->submit();
    }
    while (!context->fCalled) {
        ...
        SkASSERT(grCtx);
        grCtx->checkAsyncWorkCompletion();
    }
    if (!context->fResult) {
        return nullptr;
    }
...
static sk_sp<SkImage> do_read_and_scale_yuv(Src* src,
    ...
    backendTextures[0] = grCtx->createBackendTexture(yPM, GrRenderable::kNo, GrProtected::kNo);
    backendTextures[1] = grCtx->createBackendTexture(uPM, GrRenderable::kNo, GrProtected::kNo);
    backendTextures[2] = grCtx->createBackendTexture(vPM, GrRenderable::kNo, GrProtected::kNo);
    ...
    *cleanup = {[grCtx, backendTextures] {
        GrFlushInfo flushInfo;
        flushInfo.fFlags = kSyncCpu_GrFlushFlag;
        grCtx->flush(flushInfo);
        grCtx->submit(true);
        grCtx->deleteBackendTexture(backendTextures[0]);
        grCtx->deleteBackendTexture(backendTextures[1]);
        grCtx->deleteBackendTexture(backendTextures[2]);
    }};

    return SkImage::MakeFromYUVATextures(grCtx, yuvCS, backendTextures, indices, size,
...
DEF_SIMPLE_GM_CAN_FAIL(async_yuv_no_scale, canvas, errorMsg, 400, 300) {
    ...
    auto yuvImage = do_read_and_scale_yuv(
            ...
            {400, 300}, SkSurface::RescaleGamma::kSrc, kNone_SkFilterQuality, &scopeExit);
