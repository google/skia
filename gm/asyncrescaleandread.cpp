...
static sk_sp<SkImage> do_read_and_scale_yuv(SkSurface* surface, SkYUVColorSpace yuvCS,
    ...
    backendTextures[1] = gr->priv().getGpu()->createTestingOnlyBackendTexture(
            dstW / 2, dstH / 2, kAlpha_8_SkColorType, GrMipMapped::kNo, GrRenderable::kNo,
            uData.get());
    backendTextures[2] = gr->priv().getGpu()->createTestingOnlyBackendTexture(
            dstW / 2, dstH / 2, kAlpha_8_SkColorType, GrMipMapped::kNo, GrRenderable::kNo,
            vData.get());
    auto channel = backendTextures[0].pixelConfig() == kAlpha_8_as_Red_GrPixelConfig
                           ? SkColorChannel::kR
                           : SkColorChannel::kA;
