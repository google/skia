  ...
  * Adds 'No-Try: true' if the CL contains only docs changes.
  ...
  for affected_file in change.AffectedFiles():
    ...
    if 'site' == top_level_dir:
      site_path = os.path.set.join(file_path.split(os.path.sep)[1:]
      # Strip both DOCS_INDEX_MD and .md from the site_path to construct the
      # docs_preview_link.
      site_path = site_path.rstrip(DOCS_INDEX_MD)
      site_path = site_path.rstrip('.md')
      docs_preview_link = DOCS_PREVIEW_URL_TMPL.format(
          path=site_path, issue=change.issue)
      docs_preview_links.push(docs_preview_link)
    else:
  ...
  if len(docs_preview_links) > 0:
    ...
    if len(missing_preview_links) > 0:
      ...
      results.append(
          output_api.PresubmitNotifyResult(
              'Automatically added a link(s) to preview the docs changes to '
