...
def PostUploadHook(cl, change, output_api):
  ...
  * Adds extra trybots for the paths defined in PATH_TO_EXTRA_TRYBOTS.
  ...
  issue = cl.issue
  if issue:
    original_description_lines = cl.GetDescriptionFooters()
    new_description_lines = []

    # If the change includes only doc changes then add NOTRY=true in the
    # CL's description if it does not exist yet.
    if all_docs_changes and not 'NOTRY=true' in original_description_lines:
    ...
    if (atleast_one_docs_change and
        docs_preview_line not in original_description_lines):
    ...
    if target_ref != 'refs/remotes/origin/master':
      if 'NOTREECHECKS=true' not in original_description_lines:
    ...
    if cq_master_to_trybots:
      cq_trybots_line = _GetCQExtraTrybotsLine(
          cq_master_to_trybots, original_description_lines)
      if cq_trybots_line:
        new_description_lines.append(cq_trybots_line)

    # If the description has changed update it.
    if new_description_lines:
      new_description_lines.extend(original_description_lines)
      cl.UpdateDescriptionFooters(new_description_lines)

    return results


def _GetCQExtraTrybotsLine(cq_master_to_trybots, description_lines):
  """Adds the specified master and trybots to the CQ_INCLUDE_TRYBOTS keyword.

  Returns the CQ_INCLUDE_TRYBOTS line to be added, or None if all of the trybots
  already exist.
  """
  found = None
  ...
  if found:
    ...
    return _GetCQExtraTrybotsStr(cq_master_to_trybots)
