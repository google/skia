...
class CodeReview(object):
  ...
  def GetApprovers(self):
    ...
    if self._gerrit:
      code_review_label = (
          self._gerrit.GetChangeInfo(self._issue)['labels']['Code-Review'])
      if code_review_label.get('all'):
        for m in code_review_label['all']:
          if m.get("value") == 1:
...
def _CheckLGTMsForPublicAPI(input_api, output_api):
  ...
  if input_api.change.issue:
    ...
    if input_api.gerrit:
      reviewers = cr.GetReviewers()
      for r in reviewers:
        if r in PUBLIC_API_OWNERS:
          # If an owner is specified as an reviewer then ignore the public
          # api owners check.
          return results
    else:
