...
class CodeReview(object):
  ...
  def GetMessages(self):
    class Message(object):
      def __init__(self, sender_email, text):
        self.sender_email = sender_email
        self.text = text

    messages = []  # Hold list of Message objects.
    if self._gerrit:
      for m in self._gerrit.GetChangeInfo(self._issue).get('messages', []):
        messages.append(Message(m['author']['email'], m['message']))
    else:
      for m in self._rietveld_properties.get('messages', []):
        messages.append(Message(m['sender'], m['text']))
    return messages
...
def _CheckLGTMsForPublicAPI(input_api, output_api):
  ...
  if input_api.change.issue:
    ...
    for m in cr.GetMessages():
      if (m.sender_email in PUBLIC_API_OWNERS and 'lgtm' in m.text.lower()):
  ...
  if input_api.gerrit:
    results.append(
         output_api.PresubmitNotifyResult(
             'Post upload hooks are not supported yet for Gerrit CLs'))
    return results
