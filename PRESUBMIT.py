...
class CodeReview(object):
  ...
  def GetApprovers(self):
    approvers = []
    if self._gerrit:
      for m in self._gerrit.GetChangeInfo(self._issue)['labels']['Code-Review']['all']:
        if m.get("value") == 1:
...
def _CheckLGTMsForPublicAPI(input_api, output_api):
  ...
  if input_api.change.issue:
    ...
    for approver in cr.GetApprovers():
      if (approver in ('rmistry@google.com', )):
      # if (m.sender_email in (PUBLIC_API_OWNERS + 'rmistry@google.com') and 'lgtm' in m.text.lower()):
