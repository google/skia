...
std::string gmrunner_run_test(const char* testName, ImageData* outputImg) {
  ...
  if (!skImg->asLegacyBitmap(&bm, SkImage::kRO_LegacyBitmapMode) ||
      !bm.peekPixels(&pm)) {
      return "Unable to extract image data.";
  }

  auto dstInfo = SkImageInfo::Make(size.width(), size.height(), colorType, kUnpremul_SkAlphaType);

  auto ptr = new uint8_t[pm.getSafeSize()];
  if (!ptr) {
    return "Unable to allocate memory to hold pixels.";
  }

  outputImg->pix = std::shared_ptr<uint8_t>(ptr);
  if (!pm.readPixels(info, reinterpret_cast<void*>(outputImg->pix.get()), pm.rowBytes())) {
  ...
  outputImg->byteOrder = pm.getSafeSize();
