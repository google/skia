...
func defaultSwarmDimensions(parts map[string]string) []string {
	...
	if os, ok := parts["os"]; ok {
		...
		// unresolved issues with building iOS and MoltenVK. MacMini7.1 are stuck on
		// 10.13.3 because they run *SAN, which (currently) needs to have an XCode
		// installed that matches the XCode of the builder.
