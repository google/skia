...
func kitchenTask(name, recipe, isolate, serviceAccount string, dimensions []string, extraProps map[string]string, outputDir string) *specs.TaskSpec {
	...
	cmd := []string{"/bin/bash", "skia/infra/bots/run_recipe.sh"}
	if strings.Contains(name, "Win") {
		cmd = []string{"skia/infra/bots/run_recipe.cmd"}
	}
	task := &specs.TaskSpec{
		Caches: []*specs.Cache{
			&specs.Cache{
		...
		CipdPackages: cipd,
		Command:      append(cmd, "${ISOLATED_OUTDIR}", recipe, props(properties), project()),
