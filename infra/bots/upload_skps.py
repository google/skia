...
def main(target_dir, gitcookies):
  with git_utils.NewGitCheckout(repository=CHROMIUM_SKIA) as checkout:
    import time
    print 'SLEEPING for 2 mins'
    print checkout.root

  # Try changing current working dir to see what happens :/
    os.chdir(checkout.root)

    print 'HERE HERE'
    print os.getcwd()
    # import time
    # time.sleep(5*60)  # sleep for 10 mins
    ...
    # TODO(rmistry): Add a new env variable here??
    ...
    with git_utils.GitLocalConfig(config_dict):
      with git_utils.GitBranch(branch_name='update_skp_version',
                               ...
                               commit_queue=False):  # Just for testing.
