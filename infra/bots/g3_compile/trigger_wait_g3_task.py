...
MISSING_APPROVAL_ERROR_MSG = (
    '\n\n'
    'To run the G3 tryjob changes must be owned and authored by Googlers\n'
    'or approved (Code-Review+1) by Googlers.\n'
)
MERGE_CONFLICT_ERROR_MSG = (
    '\n\n'
    'Tryjob failed because this change is causing a merge conflict when \n'
...
def trigger_and_wait(options):
  ...
  while True:
    ...
    if ret.get('status'):
      ...
      else if ret['status'] == 'missing_approval':
          raise G3CompileException(MISSING_APPROVAL_ERROR_MSG)
      else if ret['status'] == 'merge_conflict':
          raise G3CompileException(MERGE_CONFLICT_ERROR_MSG)
      else if ret['status'] == 'failure':
        raise G3CompileException('Run failed G3 TAP: %s', ret['cl'])
      else if ret['status'] == 'success':
        print 'Run passed G3 TAP: %s' % ret['cl']
        return 0
