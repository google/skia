...
def _add_cl_comment(issue, comment):
  # Need a checkout to use "git cl" even though we are just adding a
  # comment to change unrealted to the checkout.
...
def trigger_and_wait(options):
  ...
  #### TESTING HERE
  _add_cl_comment(task['issue'], 'test comment')
  #### TESTING HERE
  ...
  while True:
    ...
    if ret.get('done'):
      ...
      msg = ('FYI: Both with patch and no patch builds of the Android '
             'framework compile bot failed. This means that the Android tree '
             'might be broken and infra is investigating. Marking this bot '
             'as successful to not block the CQ.\n'
             'With patch logs are here: %s\n'
             'No patch logs are here: %s\n') % (
                 ret['withpatch_log'], ret['nopatch_log'])
      print msg
