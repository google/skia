...
func botUpdate(ctx context.Context, checkoutRoot, gitCacheDir, skiaRev, patchRef, depotToolsDir string) error {
...
func main() {
	...
	if err := botUpdate(ctx, checkoutRoot, gitCacheDir, *skiaRev, *patchRef, depotToolsDir); err != nil {
		td.Fatal(ctx, err)
	}
