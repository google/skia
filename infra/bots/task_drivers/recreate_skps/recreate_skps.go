...
`
		...
		// These are required for some reason, despite our not needing them.
...
func main() {
	...
	if err := td.Do(ctx, td.Props("Build Chrome"), func(ctx context.Context) error {
		installSysroot := filepath.Join("build", "linux", "sysroot_scripts", "install-sysroot.py")
		if _, err := exec.RunCwd(ctx, chromiumDir, installSysroot, "--arch=amd64"); err != nil {
			return err
		}
