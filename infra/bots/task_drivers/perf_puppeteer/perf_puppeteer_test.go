...
func TestBenchSkottieFrames_CPUHasNoUseGPUFlag(t *testing.T) {
	...
	res := td.RunTestSteps(t, false, func(ctx context.Context) error {
		...
		assert.Equal(t, []string{"perf-with-puppeteer",
			...
			"--input", filepath.Join(lotties, "animation_1", "data.json"),
...
func TestProcessSkottieFramesData_CPUTwoInputsGetSummarizedAndCombined(t *testing.T) {
	...
	// The output perf file name will be named randomly. We seed the rng with 0 to control this
	// randomness.
	rand.Seed(0)
	res := td.RunTestSteps(t, false, func(ctx context.Context) error {
		return processSkottieFramesData(ctx, perfObj, input, output)
	})
	require.Empty(t, res.Errors)

	// Re-seed the RNG, so we can get the filename the code wrote to.
	rand.Seed(0)
	b, err := ioutil.ReadFile(filepath.Join(output, getRandomOutputName()))
	require.NoError(t, err)
...
func TestProcessSkottieFramesData_GPUTwoInputsGetSummarizedAndCombined(t *testing.T) {
	const taskName = "Perf-Ubuntu18-EMCC-Golo-GPU-QuadroP400-wasm-Release-All-Puppeteer_SkottieFrames_TaskDriver"
