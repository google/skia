...
func main() {
	...
	// Put our node on the PATH. This does not necessarily mean we can call an unprefixed npm and have
	// it use the right one (for who knows why), but it allows the correct node to be used in the
	// event node is not installed or the installed one is too old.
	ctx = td.WithEnv(ctx, []string{
		fmt.Sprintf("PATH=%s:%s", nodeBinAbsPath, td.PATH_PLACEHOLDER),
	})
...
func setup(ctx context.Context, benchmarkPath, nodeBinPath string) error {
	...
	// You may be tempted to add nodeBinPath to the PATH environment variable and then just call
	// npm ci here. That does not seem to work on the waterfall for an unknown reason, as it will
	// call the wrong version of npm (maybe the one installed globablly). Current working
	// theory is that npm-cli has some logic which tries to be smart about which npm you meant and
	// that logic is working against us.
...
func benchSkottieFrames(ctx context.Context, perf perfJSONFormat, benchmarkPath, canvaskitBinPath, lottiesPath, nodeBinPath string) error {
	...
	fmt.Printf("Identified %d lottie folders to benchmark\n", len(lottieFolders))
