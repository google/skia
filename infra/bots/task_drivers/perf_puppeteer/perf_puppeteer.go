...
func main() {
	var (
		...
		nodeBinPath   = flag.String("node_bin_path", "", "Path to the node bin directory (should have npm also)")
		// TODO(kjlubick) Get this from swarming env?
	...
	if *nodeBinPath == "" {
		td.Fatalf(ctx, "--node_bin_path must be specified")
	}

	nodeBinAbsPath, err := filepath.Abs(*nodeBinPath)
	if err != nil {
		td.Fatal(ctx, err)
	}

	// FIXME(kjlubick) Remove these when done.
	if _, err := exec.RunCwd(ctx, ".", "ls", "-ahl"); err != nil {
		td.Fatal(ctx, err)
	}

	if _, err := exec.RunCwd(ctx, ".", filepath.Join(nodeBinAbsPath, "npm"), "version"); err != nil {
		td.Fatal(ctx, err)
	}

	if _, err := exec.RunCwd(ctx, ".", "ls", "-ahl", "node/node/bin"); err != nil {
		td.Fatal(ctx, err)
	}
	...
	// Run the infra tests.
	if err := setup(ctx, *benchmarkPath, nodeBinAbsPath); err != nil {
		td.Fatal(ctx, err)
	}
	...
	case "Puppeteer_SkottieFrames_TaskDriver":
		if err := benchSkottieFrames(ctx, outputWithoutResults, *benchmarkPath, *canvaskitBin, *lottiesPath, nodeBinAbsPath); err != nil {
			td.Fatal(ctx, err)
		}
...
func setup(ctx context.Context, benchmarkPath, nodeBinAbsPath string) error {
	ctx = td.StartStep(ctx, td.Props("setup").Infra())
	defer td.EndStep(ctx)

	if _, err := exec.RunCwd(ctx, benchmarkPath, filepath.Join(nodeBinAbsPath, "npm"), "ci"); err != nil {
...
func benchSkottieFrames(ctx context.Context, perf perfJSONFormat, benchmarkPath, canvasKitBin, lottiesPath, nodeBinAbsPath string) error {
	...
	for _, lottie := range lottieFolders {
		...
		err = td.Do(ctx, td.Props("Benchmark "+name), func(ctx context.Context) error {
			args := []string{filepath.Join(nodeBinAbsPath, "node"),
				"perf-with-puppeteer",
