...
func main() {
	...
	if err := runTests(ctx, builtAbsPath, nodeBinAbsPath, skiaAbsPath, testsWorkPath, *webGLVersion); err != nil {
		td.Fatal(ctx, err)
	}
...
func runTests(ctx context.Context, builtPath, nodeBinPath, skiaPath, workPath string, webglVersion int) error {
