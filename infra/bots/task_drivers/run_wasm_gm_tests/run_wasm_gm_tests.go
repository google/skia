...
// Test with go run ./infra/bots/task_drivers/run_wasm_gm_tests/ --logtostderr --local --service_account_path $HOME/service_accounts/gold-external-uploader.json --work_path /tmp/gold2 --gold_ctl_path=`which goldctl` --built_path $SKIA_ROOT/out/wasm_gm_tests --skia_path $SKIA_ROOT --git_commit "154e656f6cdadb1eba23a0b24a8142630cfe9f54" --gold_key "browser:Chrome" --gold_key "alpha_type:Premul" --gold_key "arch:wasm" --gold_key "color_depth:8888" --node_bin_path /opt/node-v12.16.1-linux-x64/bin/
	...
	err = td.Do(ctx, td.Props(fmt.Sprintf("Call goldtl on %d results", len(results))), func(ctx context.Context) error {
		for _, result := range results {
			...
			if err == os.ErrNotExist {
				...
				return skerr.Wrapf(err, "Looking for %s", potentialPNGFile)
			}
