...
func main() {
	...
	// ts, err := auth_steps.Init(ctx, *local, datastore.ScopeDatastore)
	// ts, err := auth_steps.Init(ctx, *local)
	if err != nil {
		td.Fatal(ctx, skerr.Wrap(err))
	}

	// TEST TEST
	// dsClient, err := datastore.NewClient(ctx, "skia-public", option.WithTokenSource(ts))
	// if err != nil {
	// 	sklog.Fatalf("Could not init datastore: %s", err)
	// }
	// q := datastore.NewQuery("Status").Namespace("tree-status-staging").Order("-date").Limit(1)
	...
	if err := waitForCanaryRoll(ctx, manualRollDB, req.Id); err != nil {
		td.FailStep(ctx, skerr.Wrap(err)) // Should this be fail or fatal? test by abandoning
		return
		// td.Fatal(ctx, skerr.Wrap(err))
	}
	// // Step 3: for waiting for manual roll
	// for {
	...
	// 			sklog.Infof("Canary roll [ %s ] has successful", cl)
	// 			return
...
func waitForCanaryRoll(ctx context.Context, manualRollDB manual.DB, rollId string) error {
	...
	for {
		...
		if err != nil {
			return fmt.Errorf("Coudl not find canary roll with ID: %s", rollId)
		}
		cl := roll.Url
		// TODO(rmistry): Figure out how to display the CL number in task driver.
		...
		if roll.Status == manual.STATUS_COMPLETE {
			if roll.Result == manual.RESULT_SUCCESS {
				...
				return td.FailStep(ctx, fmt.Errorf("%s failed", cl))
			} else if roll.Result == manual.RESULT_UNKNOWN {
				return td.FailStep(ctx, fmt.Errorf("%s completed with an unknown result", cl))
			}
		}
		if cl == "" {
			sklog.Infof("Canary roll has status %s", roll.Status)
