...
func waitForCanaryRoll(ctx context.Context, manualRollDB manual.DB, rollId string) error {
	...
	// For writing to step logs
	stdout := td.NewLogStream(ctx, "stdout", td.Info)
	stderr := td.NewLogStream(ctx, "stderr", td.Error)

	if _, err := stdout.Write([]byte("TESTING TESTING TESTING")); err != nil {
		return td.FailStep(ctx, fmt.Errorf("Could not write to stdout: %s", err))
	}
	for {
		roll, err := manualRollDB.Get(ctx, rollId)
		if err != nil {
			if _, writeErr := stderr.Write([]byte("TESTING TESTING TESTING")); err != nil {
				return td.FailStep(ctx, skerr.Wrapf(err, "Could not write to stderr: %s", writeErr))
			}
		...
		if _, err := stdout.Write([]byte(fmt.Sprintf("Canary roll [ %s ] has status %s", roll.Url, roll.Status))); err != nil {
			return td.FailStep(ctx, fmt.Errorf("Could not write to stdout: %s", err))
		}
