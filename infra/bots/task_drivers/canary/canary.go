...
func main() {
	...
	for retry := 0; ; retry++ {
		...
		if err := td.Do(ctx, td.Props(fmt.Sprintf("Trigger canary roll%s", retryText)).Infra(), func(ctx context.Context) error {
			...
			// Should this be Fail step instead? TEST THIS
			td.Fatal(ctx, td.FailStep(ctx, skerr.Wrap(err)))
		} else {
			if retry == 2 {
				td.Fatal(ctx, td.FailStep(ctx, skerr.Wrap(errors.New("Jsut testing"))))
			}
		}

		if err := waitForCanaryRoll(ctx, manualRollDB, req.Id, fmt.Sprintf("Wait for canary roll%s", retryText), retry); err != nil {
			// Retry these errors.
...
func waitForCanaryRoll(parentCtx context.Context, manualRollDB manual.DB, rollId, stepName string, retryNumTesting int) error {
	ctx := td.StartStep(parentCtx, td.Props(stepName))
	defer td.EndStep(ctx)
	startTime := time.Now()

	// TESTING TESTING
	if retryNumTesting < 1 {
