		...
		if roll.Status == manual.STATUS_COMPLETE {
			if roll.Result == manual.RESULT_SUCCESS {
				...
				if time.Now().Before(startTime.Add(MIN_WAIT_DURATION)) {
					return td.FailStep(ctx, fmt.Errorf("Canary roll [ %s ] returned success in less than %s. Failing canary due to skbug.com/10563. Please retry the Canary.", cl, MIN_WAIT_DURATION))
				}
				return nil
