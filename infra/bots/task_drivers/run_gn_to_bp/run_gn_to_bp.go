...
func main() {
	...
	skiaCheckout, err := filepath.Abs("skia")
	if err != nil {
		td.Fatal(ctx, skerr.Wrap(err))
	}
	gnToBpAbsPath := filepath.Join(skiaCheckout, *gnToBpPath)
	binAbsPath := filepath.Join(skiaCheckout, "bin")

	if _, err := exec.RunCwd(ctx, ".", filepath.Join(binAbsPath, "fetch-gn")); err != nil {
		td.Fatal(ctx, skerr.Wrap(err))
	}
	gnEnv := []string{fmt.Sprintf("PATH=%s/:%s", binAbsPath, os.Getenv("PATH"))}
	if _, gnToBpErr := exec.RunCommand(ctx, &exec.Command{
		Env:  gnEnv,
		Dir:  skiaCheckout,
		Name: "python", // Will not work?
	...
	err = td.Do(ctx, td.Props(fmt.Sprintf("Run %s", gnToBpAbsPath)), func(ctx context.Context) error {
		if _, err := exec.RunCwd(ctx, "", gnToBpAbsPath); err != nil {
