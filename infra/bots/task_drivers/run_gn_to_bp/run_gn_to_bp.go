...
func main() {
	...
	err = td.Do(ctx, td.Props("Run ./bin/sync"), func(ctx context.Context) error {
		if _, err := exec.RunCwd(ctx, co.Dir(), "./bin/sync"); err != nil {
	...
	err = td.Do(ctx, td.Props(fmt.Sprintf("Run %s", *gnToBpPath)), func(ctx context.Context) error {
		if _, err := exec.RunCwd(ctx, co.Dir(), fmt.Sprintf("vpython %s", *gnToBpPath)); err != nil {
			return fmt.Errorf("Error in running %s: %s", *gnToBpPath, err)
		}
		return nil
	})
	if err != nil {
