...
func main() {
	...
	run := td.StartRun(projectId, taskId, taskName, output, local)
	defer td.EndRun(run)

	// Run fm --listTests to find the names of all linked GMs.
	stdout, err := exec.RunCwd(run, *workdir, *fm, "--listGMs", "-i", *resources)
	if err != nil {
		td.Fatal(run, err)
	}
	...
	for i := 0; i < len(gms); i += batchSize {
		...
		wg.Add(1)
		go func(cmd []string) {
			step := td.StartStep(run, &td.StepProperties{})
			defer td.EndStep(step)
			_, err := exec.RunCwd(step, *workdir, cmd...)
			if err != nil {
				td.FailStep(step, err)
			}
			wg.Done()
		}(cmd)
	}
