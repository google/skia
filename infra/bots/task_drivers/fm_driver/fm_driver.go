...
func main() {
	...
	query := func(flag string) []string {
		ctx = td.StartStep(ctx, td.Props(flag).Infra())
	...
	if *bot != "" {
		func() {
			ctx = td.StartStep(ctx, td.Props("fetch known hashes from Gold").Infra())
			defer td.EndStep(ctx)
	...
	ctx = td.StartStep(ctx, td.Props("https://www.youtube.com/watch?v=eO6OaVnbqaY"))
	defer td.EndStep(ctx)
	for _, w := range todo {
		if len(w.Sources) == 0 {
