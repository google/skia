...
func main() {
	var (
		...
		images    = flag.String("images", "", "Glob to shorthand as 'images'.")
		skps      = flag.String("skps", "", "Glob to shorthand as 'skps'.")
		svgs      = flag.String("svgs", "", "Glob to shorthand as 'svgs'.")
	...
	mustMatch := func(matches []string, err error) []string {
		if err != nil {
			fatal(err)
		}
		return matches
	...
	shorthands := map[string][]string{
		...
		"image": mustMatch(filepath.Glob(*images)),
		"skp":   mustMatch(filepath.Glob(*skps)),
		"svg":   mustMatch(filepath.Glob(*svgs)),
	}
	...
	if *bot != "" {
		...
		if CPU_or_GPU == "CPU" {
			...
			run(all("gms", "images", "skps", "svgs", "tests"), "")
			...
			if model == "GCE" {
				...
				run(all("images"), "--decodeToDst --ct f16 --gamut rec2020 --tf rec2020")
			}
