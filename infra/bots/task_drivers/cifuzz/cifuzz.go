...
func main() {
	...
	if err := buildAndRunCIFuzz(ctx, workDir); err != nil {
		td.Fatal(ctx, skerr.Wrap(err))
	}
...
func prepareSkiaCheckout(ctx context.Context, skiaAbsPath, workDir, gitAbsPath string) error {
	...
	skiaCopyDir := filepath.Join(workDir, "skia_copy")

	if _, err := exec.RunCwd(ctx, workDir, gitAbsPath, "clone", skiaAbsPath, skiaCopyDir); err != nil {
		return td.FailStep(ctx, skerr.Wrap(err))
	}

	// We also need the third_party/externals
	if _, err := exec.RunCwd(ctx, workDir, "cp", "-r", filepath.Join(skiaAbsPath, "third_party", "externals"),
		filepath.Join(skiaCopyDir, "third_party")); err != nil {
		return td.FailStep(ctx, skerr.Wrap(err))
	}
...
func buildAndRunCIFuzz(ctx context.Context, workDir string) error {
	...
	if _, err := exec.RunCwd(ctx, workDir, dockerExe, "run",
		...
		"--env", "DRY_RUN=false",
		"--env", "CI=true",
		"--env", "FUZZ_TIME=600", // This is split up between all affected fuzzers.
		"--env", "SANITIZER=address",
		"--env", "GITHUB_SHA=TODO(kjlubick)",
		"--volume", "/var/run/docker.sock:/var/run/docker.sock",
		"--mount", fmt.Sprintf("type=bind,source=%s,destination=%s", workDir, workDir),
