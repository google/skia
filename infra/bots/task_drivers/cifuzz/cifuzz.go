...
func main() {
	...
	// FIXME(kjlubick) remove this debugging
	if _, err := exec.RunCwd(ctx, workAbsPath, "ls", skiaAbsPath); err != nil {
		fatalOrSleep(ctx, skerr.Wrap(err))
	}

	if _, err := exec.RunCwd(ctx, workAbsPath, "ls", workAbsPath); err != nil {
		fatalOrSleep(ctx, skerr.Wrap(err))
	}
...
func extractOutput(ctx context.Context, workDir, outAbsPath string) error {
	...
	// FIXME(kjlubick) remove
	if _, err := exec.RunCwd(ctx, workDir, "ls", workDir); err != nil {
		return td.FailStep(ctx, skerr.Wrap(err))
	}

	if _, err := exec.RunCwd(ctx, workDir, "ls", filepath.Join(workDir, "out")); err != nil {
		return td.FailStep(ctx, skerr.Wrap(err))
	}
	...
	if _, err := exec.RunCwd(ctx, workDir, "ls", outAbsPath); err != nil {
		return td.FailStep(ctx, skerr.Wrap(err))
	}
