...
func main() {
	...
	if err := setupCIFuzzRepoAndDocker(ctx, gitAbsPath); err != nil {
		td.Fatal(ctx, skerr.Wrap(err))
	}

	// build and run fuzzers
	if err := buildAndRunCIFuzz(ctx, skiaAbsPath); err != nil {
		td.Fatal(ctx, skerr.Wrap(err))
	}
}
...
func setupCIFuzzRepoAndDocker(ctx context.Context, gitAbsPath string) error {
...
func buildAndRunCIFuzz(ctx context.Context, skiaAbsPath string) error {
	return nil
}
