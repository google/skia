...
func TestBenchSKPs_CPUHasNoUseGPUFlag(t *testing.T) {
	...
	res := td.RunTestSteps(t, false, func(ctx context.Context) error {
		...
		require.Len(t, mock.Commands(), skpBenchRepetitions)
		cmd := mock.Commands()[0]
		...
		assert.Equal(t, []string{"perf-canvaskit-with-puppeteer",
			...
			"--output", "/fake/path/to/perf-puppeteer/out/first_skp_001.json"}, cmd.Args)
		return nil
	})
	require.Empty(t, res.Errors)
	require.Empty(t, res.Exceptions)
...
func TestBenchSKPs_GPUHasFlag(t *testing.T) {
	...
	res := td.RunTestSteps(t, false, func(ctx context.Context) error {
		...
		assert.Equal(t, []string{"perf-canvaskit-with-puppeteer",
			...
			"--canvaskit_wasm", "/fake/path/to/canvaskit/canvaskit.wasm",
			"--input_skp", filepath.Join(skps, "first_skp"),
			"--output", "/fake/path/to/perf-puppeteer/out/first_skp_001.json",
			"--use_gpu"}, cmd.Args)
		return nil
	})
	require.Empty(t, res.Errors)
...
func TestProcessSkottieFramesData_GPUTwoInputsGetSummarizedAndCombined(t *testing.T) {
	...
	err = writeFilesToDisk(filepath.Join(input, "out"), map[string]string{
		"first_skp_001.json":  firstSKPSampleOne,
		"first_skp_002.json":  firstSKPSampleTwo,
		"first_skp_003.json":  firstSKPSampleThree,
		"second_skp_001.json": secondSKPSampleOne,
		"second_skp_002.json": secondSKPSampleTwo,
		"second_skp_003.json": secondSKPSampleThree,
	})
	require.NoError(t, err)
  ...
  "results": {
    "first_skp": {
      "webgl2": {
        "avg_json_load_ms": 4.9583335,
        "avg_render_with_flush_ms": 133.91167,
        "avg_render_without_flush_ms": 45.398335,
        "median_json_load_ms": 4.915,
        "median_render_with_flush_mss": 125.185,
        "median_render_without_flush_ms": 37.445,
        "render_json_load_ms_samples": [
          1.715,
          8.245,
          4.915
        ],
        "render_with_flush_ms_samples": [
          125.185,
          120.895,
          155.655
        ],
        "render_without_flush_ms_samples": [
          23.71,
          37.445,
          75.04
        ],
        "stddev_json_load_ms": 2.6660373,
    ...
    "second_skp": {
      "webgl2": {
        "avg_json_load_ms": 3.625,
        "avg_render_with_flush_ms": 233.91167,
        "avg_render_without_flush_ms": 85.39834,
        "median_json_load_ms": 3.715,
        "median_render_with_flush_mss": 225.185,
...
const firstSKPSampleOne = `{"without_flush_ms":23.71,"with_flush_ms":125.185,"json_load_ms":1.715}`
const firstSKPSampleTwo = `{"without_flush_ms":37.445,"with_flush_ms":120.895,"json_load_ms":8.245}`
const firstSKPSampleThree = `{"without_flush_ms":75.04,"with_flush_ms":155.655,"json_load_ms":4.915}`

const secondSKPSampleOne = `{"without_flush_ms":43.71,"with_flush_ms":225.185,"json_load_ms":3.715}`
const secondSKPSampleTwo = `{"without_flush_ms":67.445,"with_flush_ms":220.895,"json_load_ms":3.245}`
const secondSKPSampleThree = `{"without_flush_ms":145.04,"with_flush_ms":255.655,"json_load_ms":3.915}`
