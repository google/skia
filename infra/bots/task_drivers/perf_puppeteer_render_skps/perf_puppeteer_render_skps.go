...
const (
	// skpBenchRepetitions is how many times we benchmark a single SKP. We can't do these benchmarks
	// all in the same Chrome window because stuff is cached and we don't get very reliable data.
	// For example, the first time to render a given skp might be 100ms and then the second time and
	// beyond are few millisconds. This number was chosen arbitrarily to make sure we have sufficient
	// samples, such that we don't take too long, and is odd so our naive median calculation works
	// without needing to average the middle two points.
	skpBenchRepetitions = 11
)
...
func benchSKPs(ctx context.Context, perf perfJSONFormat, benchmarkPath, canvaskitBinPath, skpsPath, nodeBinPath string) error {
	...
	for _, skp := range skpFiles {
		...
		for i := 1; i <= skpBenchRepetitions; i++ {
