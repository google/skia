...
func nanobenchFlags(bot string, parts map[string]string) ([]string, map[string]string) {
	...
	if parts["cpu_or_gpu"] == "CPU" {
		...
		if sampleCount != "" {
			configs = append(configs, glPrefix+"msaa"+sampleCount)
		}
		...
		if has("ANGLE") {
			...
			if sampleCount != "" {
				configs = append(configs, "angle_d3d11_es2_msaa"+sampleCount)
			}
			if has("QuadroP400") {
				// See skia:7823 and chromium:693090.
				configs = append(configs, "angle_gl_es2")
				if sampleCount != "" {
					configs = append(configs, "angle_gl_es2_msaa"+sampleCount)
				}
			}
		}
		if has("ChromeOS") {
