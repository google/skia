...
func (b *taskBuilder) nanobenchFlags(doUpload bool) {
	...
	if (b.model("Pixel5")) {
		// skia:11152
		match = append(match, "~^thinconcavepaths$")
		match = append(match, "~^smallpaths$")
		match = append(match, "~^simpleaaclip_path$")
		match = append(match, "~^quadclosepath$")
		match = append(match, "~^quadpath$")
		match = append(match, "~^path-reverse$")
		match = append(match, "~^pathops_skbug_10155$")
		match = append(match, "~^pathopsinverse$")
		match = append(match, "~^path_mask_cache$")
		match = append(match, "~^pathinterior$")
		match = append(match, "~^path_arcto_skbug_9077$")
		match = append(match, "~^path_stroke_clip_crbug1070835$")
		match = append(match, "~^pathinvfill$")
		match = append(match, "~^pathfill$")
		match = append(match, "~^rotatedcubicpath$")
		match = append(match, "~^stroke_and_fill_patheffect$")
		match = append(match, "~^combo-patheffects$")
		match = append(match, "~^patheffect$")
		match = append(match, "~^shallow_angle_path_arcto$")
		match = append(match, "~^zero_length_paths_dbl_bw$")
		match = append(match, "~^zero_length_paths_dbl_aa$")
		match = append(match, "~^zero_length_paths_bw$")
		match = append(match, "~^zero_length_paths_aa$")
		match = append(match, "~^nonclosedpaths$")
		match = append(match, "~^lineclosepath$")
		match = append(match, "~^linepath$")
		match = append(match, "~^largeclippedpath_evenodd$")
		match = append(match, "~^largeclippedpath_winding$")
		match = append(match, "~^inverse_paths$")
		match = append(match, "~^path_huge_aa$")
		match = append(match, "~^path_huge_crbug_800804$")
		match = append(match, "~^hittestpath$")
		match = append(match, "~^getpostextpath$")
		match = append(match, "~^fatpathfill$")
		match = append(match, "~^emptypath$")
		match = append(match, "~^path_effect_empty_result$")
		match = append(match, "~^longpathdash$")
		match = append(match, "~^trimpatheffect$")
