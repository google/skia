...
func (b *builder) defaultSwarmDimensions(parts map[string]string) []string {
	...
	if parts["role"] == "Test" || parts["role"] == "Perf" {
		if strings.Contains(parts["os"], "Android") {
			...
			// This could run on SMALL machines, but these tasks can take a very long time, which clogs
			// things up for Housekeeper tasks, which in turn prevents other tasks from running.
