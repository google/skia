...
func (b *builder) maybeAddIosDevImage(t *specs.TaskSpec) {
	for _, dim := range t.Dimensions {
		if strings.HasPrefix(dim, "os:iOS") {
			if strings.HasPrefix(dim, "iOS-13.2") {
				t.CipdPackages = append(t.CipdPackages, b.MustGetCipdPackageFromAsset("ios-dev-image-13.2"))
			}
			// TODO: error if unknown version
			break
		}
	}
}
