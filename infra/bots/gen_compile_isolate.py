...
# If a parent path contains more than this many child paths, we will include
# the parent in the isolate file instead of the children. This results in a
# simpler isolate file.
COMBINE_PATHS_THRESHOLD = 3

# These parent paths will never have their children merged into them, even if
# COMBINE_PATHS_THRESHOLD is exceeded.
NO_COMBINE_PATHS = [
  'assets',
  'infra',
]
    ...
    def entries(self):
      ...
      Will not return children in the following cases:
        - This Node is a leaf, ie. it represents an entry which was explicitly
          ...
          entries.
        - This Node has children exceeding COMBINE_PATHS_THRESHOLD and thus has
          been upgraded to a leaf node.
      '''
      if self._is_leaf:
        return [self._name]
      if (not self.is_root and \
          len(self._children) >= COMBINE_PATHS_THRESHOLD and \
          not self._name in NO_COMBINE_PATHS):
        return [self._name]
      rv = []
