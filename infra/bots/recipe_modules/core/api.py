...
class SkiaApi(recipe_api.RecipeApi):
  ...
  def __init__(self, *args, **kwargs):
    # These are set on first use, or overridden by a caller.
    self._checkout_root = None
    self._gclient_cache = None
    super(SkiaApi, self).__init__(*args, **kwargs)
  ...
  def checkout_root(self):
    if not self._checkout_root:
      self._checkout_root = self.m.vars.cache_dir.join('work')
      if 'Flutter' in self.m.vars.builder_name:
        self._checkout_root = self._checkout_root.join('flutter')
    return self._checkout_root

  @property
  def gclient_cache(self):
    if not self._gclient_cache:
      self._gclient_cache = self.m.vars.cache_dir.join('git')
    return self._gclient_cache

  def set_checkout_root(self, cr):
    """Override the default checkout root."""
    self._checkout_root = cr

  def set_gclient_cache(self, gc):
    """Override the default gclient cache."""
    self._gclient_cache = gc

  def checkout_git(self):
    """Run the steps to perform a pure-git checkout without DEPS."""
    skia_dir = self.checkout_root.join('skia')
    self.m.git.checkout(
