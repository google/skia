...
def test_steps(api):
  ...
  if 'Ubuntu' in api.vars.builder_name and 'Vulkan' in api.vars.builder_name:
    ...
    lib_path = api.vars.slave_dir.join('linux_vulkan_intel_driver_release')
    if 'Debug' in api.vars.builder_name:
      lib_path = api.vars.slave_dir.join('linux_vulkan_intel_driver_debug')


    env.update({
      'PATH':'%%(PATH)s:%s' % sdk_path,
      'LIBGL_DRIVERS_PATH':'%s' % lib_path,
      'VK_ICD_FILENAMES':'%s' % lib_path.join('intel_icd.json'),
      })
