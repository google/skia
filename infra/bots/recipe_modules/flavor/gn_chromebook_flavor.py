...
class GNChromebookFlavorUtils(gn_flavor.GNFlavorUtils):
  ...
  def install(self):
    # TODO(kjlubick) are mkdir calls necessary?
    pass
  ...
  def copy_file_to_device(self, host_path, device_path):
    ...
    # Recipe
    self.m.python.inline(str('scp %s %s' % (device_path, host_path)),
    """
    import subprocess
    import sys
    host = sys.argv[1]
    device   = sys.argv[2]
    subprocess.check_call(['scp', host, device])
  ...
  def _copy_dir(self, src, dest):
    ...
    src = sys.argv[1]
    dest   = sys.argv[2]
    subprocess.check_call(['scp', '-r', src, dest])
  ...
  def step(self, name, cmd, **kwargs):
    ...
    self.copy_file_to_device(app, self._bin_dir)
