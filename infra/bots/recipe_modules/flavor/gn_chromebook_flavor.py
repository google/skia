...
class GNChromebookFlavorUtils(gn_flavor.GNFlavorUtils):
  ...
  def install(self):
    self._ssh('mkdir %s' % self.m.vars.chromeos_homedir, 'mkdir', '-p',
              self.m.vars.chromeos_homedir)
  ...
  def create_clean_device_dir(self, path):
    ...
    # scp will create the directory if it doesn't exist. If it does exist,
    # it copies into that file, leaving us with an undesireable structure
    # like [home]/skps/skp
    # Thus, to create device_dirs successfully, we just delete them here
    # and assume they will be created via scp, or manually if needed.
  ...
  def _copy_dir(self, src, dest):
    ...
    src = sys.argv[1]
    dest   = sys.argv[2]
    print subprocess.check_output(['scp', '-r', src, dest])
  ...
  def step(self, name, cmd, **kwargs):
    ...
    # First, ensure the home dir is marked executable
    self._ssh('remount %s as exec' % self.m.vars.chromeos_homedir,
              'sudo', 'mount', '-i', '-o', 'remount,exec', '/home/chronos')
    app = self.m.vars.skia_out.join(self.m.vars.configuration, cmd[0])

    self._ssh('rm %s' % self._bin_dir, 'rm', '-rf', self._bin_dir)
    self._ssh('mkdir %s' % self._bin_dir, 'mkdir', '-p', self._bin_dir)
    self._ssh('mkdir %s' % self.device_dirs.dm_dir, 'mkdir', '-p',
              self.device_dirs.dm_dir)

    cmd[0] = '%s/%s' % (self._bin_dir, cmd[0])
    self.copy_file_to_device(app, cmd[0])

    self._ssh('chmod dm', 'chmod', '+x', cmd[0])
    self._ssh('dm', *cmd)
