  ...
  def compile(self, unused_target):
    ...
    with self.m.context(cwd=self.m.vars.skia_dir):
      ...
      with self.m.env(env):
        ...
        self.m.file.listdir('list include/android', self.m.vars.skia_dir.join('include', 'android'))
        self._run('ninja', [ninja] + ninja_args)

    if 'Goma' in extra_config:
      with self.m.context(cwd=goma_dir, env=env):
        self._py('print goma stats', 'goma_ctl.py', args=['stat'])
        self._py('stop goma', 'goma_ctl.py', args=['stop'])
