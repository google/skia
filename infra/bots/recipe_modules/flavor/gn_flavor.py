...
""" % json_key,
  ...
  def _get_goma(self):
    goma_dir = self.m.vars.tmp_dir.join('goma')
    self.m.file.ensure_directory('makedirs goma', goma_dir)
    goma_ctl_file = goma_dir.join('goma_ctl.py')
    self.m.python.inline(
...
""",
  ...
  def compile(self, unused_target):
    ...
    if 'Goma' in extra_config:
      with self.m.context(cwd=goma_dir, env=env):
        self._py('goma stat', 'goma_ctl.py', args=['stat'])
