...
class GNChromecastFlavorUtils(gn_android_flavor.GNAndroidFlavorUtils):
  ...
  def compile(self, unused_target):
    ...
    extra_cflags = target + [
      ...
      '-I%s' % toolchain.join('armv7a/usr/armv7a-cros-linux-gnueabi' +
                              '/usr/include'),
      '-I%s' % gles_dir.join('include'),
      '-DMESA_EGL_NO_X11_HEADERS',
      '-DSK_NO_COMMAND_BUFFER',
      # Avoid unused warning with yyunput
      '-Wno-error=unused-function',  # TODO: follow up?
    ...
    extra_ldflags = target + [
      ...
      '-static-libstdc++', '-static-libgcc',  # TODO: follow up?
      '-L%s' % toolchain.join('armv7a', 'lib'),  # TODO: follow up?
    ]
