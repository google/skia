...
if model == 'Nexus10':
  ...
  freq = maxfreq
  for f in reversed(available_freqs[:-1]):
    if f <= target:
      freq = f
      break
  ...
  set_freq = subprocess.check_output([ADB, 'shell', 'cat '
      '/sys/devices/system/cpu/cpu0/cpufreq/scaling_setspeed']).trim()
  if set_freq != str(freq):
    raise Exception('set frequency what was expected (%s, %d)'
                    % (set_freq, freq))
        """,
    ...
    #   # ADB root failed
    #   if self.m.vars.builder_cfg.get('model') not in self.rootable_blacklist:
    #     self.m.run(self.m.step, 'Unexpected adb root failure', cmd=['false'])
    #   return
    ...
    #     # The list of available frequencies is hidden, but if we select one that
    ...
    #   else:
    #     # We should only get here if things have gone wrong and we can't
    #     # set the frequency.
    #     self.m.run(self.m.step, 'Could not fetch frequency range for scaling',
    #                cmd=['false'])
    #     return
    ...
    # set governor to userspace first, so frequency "takes"
    # self._adb('Set cpu governor to userspace', 'shell', 'echo "userspace" > '
    #           '/sys/devices/system/cpu/cpu0/cpufreq/scaling_governor')
