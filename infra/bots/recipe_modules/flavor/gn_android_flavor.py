...
""",
  ...
  def compile(self, unused_target):
    ...
    if 'SKQP' in extra_tokens:
      # If there is no model hash this becomes a no-op.
      if not self.skqp_model_exists():
        return
      ...
      pkg_dir = self.m.vars.skia_dir.join('infra', 'cts', 'run_testlab.go')
      build_cmd = ['go', 'build', '-o', output_binary, pkg_dir]
