...
""",
  ...
  def copy_directory_contents_to_device(self, host, device):
    ...
    assert not str(host).endswith('/'), 'expected host dir name without slash'
    assert not device.endswith('/'), 'expected device dir name without slash'
    host_str = str(host) + '/'
    with self.m.step.nest('push %s/* %s' % (host, device)):
      contents = self.m.file.listdir('list %s' % host, host, recursive=True,
                                     test_data=['file1', 'subdir/file2',
                                                '.file3', '.ignore/file4'])
      for path in contents:
        path_str = str(path)
        assert(path_str.startswith(host_str),
               'expected %s to have %s as a prefix' % (path, host))
        ...
        if self.m.path.dirname(relpath).startswith('.'):
          continue
        device_path = device + '/' + relpath
        self._adb('push %s' % path, 'push', path, device_path)
