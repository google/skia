...
""",
  ...
  def copy_directory_contents_to_device(self, host, device):
    # We'd like to just adb push the directory to device directly,
    # but we need to resolve symlinks (cipd packages) first via a local copy.
    tmp = self.m.path.mkdtemp('copy_directory_contents_to_device')
    self.m.file.copytree('cp -r %s %s' % (host, tmp), host, tmp)

    self.remove_file_on_device(device)
    self._adb('push %s %s' % (tmp, device), 'push', '-z', 'any', tmp, device)
