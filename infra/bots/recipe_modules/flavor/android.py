...
""",
  ...
  def step(self, name, cmd, **kwargs):
    ...
    """ % (self.ADB_BINARY, self.ADB_BINARY),
      ...
      app_stderr = None
      if self.m.step.active_result and self.m.step.active_result.stderr:
        app_stderr = self.m.step.active_result.stderr.strip()
      if app_stderr:
        with self.m.step.nest(
            'symbolize backtraces in %s stderr' % cmd[0]) as presentation:
          symbolized_lines = []
          # Assume all hex numbers are addresses.
          hex_re = re.compile(r'(0x\w+)')
          for line in app_stderr.split('\n'):
            for match in hex_re.finditer(line):
              addr = match.group(1)
              a2l_out = self.m.run(self.m.step, 'addr2line %s' % addr,
                                   cmd=['addr2line', '--demangle',
          ...
          if symbolized_lines:
            ...
            presentation.logs['symbolized stderr'] = '\n'.join(symbolized_lines)
