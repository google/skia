...
""",
  ...
  def copy_directory_contents_to_device(self, host, device):
    if self.m.path.exists(host.join('.cipd')):
      # adb push can't handle CIPD's symlinks, so copy locally to resolve them.
      tmp = self.m.path.mkdtemp('resolve_cipd_symlinks')
      self.m.file.copytree('cp -r %s %s' % (host, tmp), host, tmp)
      self.m.file.rmtree('rm -r %s/.cipd' % tmp, tmp.join('.cipd'))
      host = tmp
    self.remove_file_on_device(device)
    self._adb('push %s %s' % (host, device), 'push', '-z', 'any', host, device)
