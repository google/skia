...
""")  # pragma: nocover
  ...
  def install(self):
    ...
    except self.m.step.StepFailure as e:  # pragma: nocover
    ...
    if ('ImagePresent: true' not in image_info_out and
      ...
      if len(image_pkgs) != 1:
        raise Exception('glob for ios-dev-image* returned %s' % image_pkgs)        # pragma: nocover
    ...
    if not kwargs.get('skip_binary_push', False):
      ...
      def uninstall_app(attempt):
        ...
        self.m.run(self.m.step,
                   ...
                   cmd=['ideviceinstaller', '-U', bundle_id],
                   infra_step=True,
      ...
      self.m.run.with_retry(self.m.step, 'install %s' % app_name, num_attempts,
                            cmd=['ideviceinstaller', '-i', '%s.app' % app_name],
