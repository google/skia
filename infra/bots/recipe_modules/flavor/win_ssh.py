...
class WinSSHFlavor(default.DefaultFlavor):
  ...
  def copy_directory_contents_to_device(self, host_path, device_path):
    ...
    with self.m.step.nest('copy %s to device' % host_path):
      self.create_clean_device_dir(self.device_dirs.tmp_dir)
      tmpdir = self.device_path_join(self.device_dirs.tmp_dir,
                                     self.m.path.basename(host_path))
      self._copy_dir(host_path, self._prefix_device_path(tmpdir))
  ...
  def copy_directory_contents_to_host(self, device_path, host_path):
    ...
    src = self._prefix_device_path(self.device_path_join(device_path, '*'))
    self._copy_dir(src, host_path)

  def step(self, name, cmd, infra_step=False, **kwargs):
    # There may be DLLs in the same dir as the executable that are supposed to
    # be loaded (yes, Windows allows overriding system DLLs with files in the
    # local directory). For simplicity, just copy the entire dir to the device.
