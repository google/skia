...
class ValgrindFlavorUtils(gn_flavor.GNFlavorUtils):
  def __init__(self, m):
    ...
    self._suppressions_file = self.m.vars.skia_dir.join(
        'tools', 'valgrind.supp')
    self._valgrind_dir = self.m.vars.slave_dir.join('valgrind')
    self._valgrind = self._valgrind_dir.join('bin', 'valgrind')
    self._lib_dir = self._valgrind_dir.join('lib', 'valgrind')
