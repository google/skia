...
def compile_fn(api, build_dir, out_dir):
  ...
  extra_cflags.append('-DDUMMY_ndk_version=%s' %
                      api.run.asset_version(ndk_asset, build_dir))
  ...
  with api.context(cwd=build_dir):
    ...
    if 'SKQP' in extra_tokens:
      ...
      build_target = build_dir.join('infra', 'cts', 'run_testlab.go')
      ...
      mk_universal = build_dir.join('tools', 'skqp', 'make_universal_apk')
...
def copy_extra_build_products(api, build_dir, src, dst):
  if 'SKQP' in api.vars.extra_tokens:
    wlist = build_dir.join('infra','cts', 'whitelist_devices.json')
