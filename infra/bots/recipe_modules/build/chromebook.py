...
def compile_fn(api, checkout_root, out_dir):
  ...
  if 'arm' == target_arch:
    ...
    gl_dir = os.path.join('/SRC','chromebook_x86_64_gles')
    env = {}
    args['extra_asmflags'] = []
    args['extra_cflags'] = [
      '-DMESA_EGL_NO_X11_HEADERS',
  ...
  api.run(api.step, 'printf debug', cmd=['echo', '%s, %s' % (os_name, builder_name)])
  if os_name == 'Debian9' and 'Docker' in builder_name:
    ...
    args['cc'] = os.path.join('/SRC', 'clang_linux', 'bin', 'clang')
    args['cxx'] = os.path.join('/SRC', 'clang_linux', 'bin', 'clang++')
    args['debuggging some shit'] = os_name + ", " + builder_name
