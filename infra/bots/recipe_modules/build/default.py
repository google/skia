...
def compile_fn(api, build_dir, out_dir):
  ...
  if compiler == 'Clang' and api.vars.is_linux:
    ...
    if target_arch in ['mips64el', 'loongson3a']:
      ...
      extra_cflags.extend([
          ...
          ('-DDUMMY_mips64el_toolchain_linux_version=%s' %
           api.run.asset_version('mips64el_toolchain_linux', build_dir))
      ])
    ...
    extra_cflags.append('-DDUMMY_emscripten_sdk_version=%s' %
                        api.run.asset_version('emscripten_sdk', build_dir))
  ...
  if 'SwiftShader' in extra_tokens:
    swiftshader_root = build_dir.join('third_party', 'externals', 'swiftshader')
  ...
  if 'CommandBuffer' in extra_tokens:
    # TODO(borenet): This should be passed in somehow; we shouldn't be making
    # assumptions about where the checkout is located in this module.
    chrome_dir = api.vars.cache_dir.join('work')
    api.run.run_once(build_command_buffer, api, chrome_dir, build_dir, out_dir)
  if 'MSAN' in extra_tokens:
  ...
  if compiler == 'Clang' and 'Win' in os:
    ...
    extra_cflags.append('-DDUMMY_clang_win_version=%s' %
                        api.run.asset_version('clang_win', build_dir))
  if target_arch == 'wasm':
    args.update({
      'skia_use_freetype':   'false',
      'skia_use_fontconfig': 'false',
  ...
  with api.context(cwd=build_dir):
    api.run(api.python,
            ...
            infra_step=True)
    if 'CheckGeneratedFiles' in extra_tokens:
      env['PATH'] = '%s:%%(PATH)s' % build_dir.join('bin')
      api.run(api.python,
              'fetch-clang-format',
              script=build_dir.join('bin', 'fetch-clang-format'),
              infra_step=True)
    if target_arch == 'wasm':
