...
def compile_fn(api, checkout_root, out_dir):
  ...
  if os == 'Mac':
    ...
    # DO NOT SUBMIT
    api.file.rmtree('clean xcode', xcode_app_path)
    # Copied from
    ...
    if 'iOS' in extra_tokens:
      ...
      extra_cflags.append('-mmacosx-version-min=10.10.0')
      ...
      try:
        ...
        if goma_ctl:
          if not build_successful and api.properties.get('gs_bucket', ''):
            ...
            api.run(api.step, 'upload goma report',
                    cmd=['gsutil', 'cp', src, dst], infra_step=True,
                    abort_on_failure=False, fail_build_on_failure=False)
