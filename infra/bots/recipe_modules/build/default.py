  ...
  if compiler == 'Clang' and 'Win' in os:
    ...
    api.run(api.step, 'clang version',
            cmd=[api.vars.slave_dir.join('clang_win', 'bin', 'clang-cl.exe'),
                 '--version'])
    api.run(api.step, 'cmd clang version',
            cmd=['cmd', '/c', api.vars.slave_dir.join('clang_win', 'bin', 'clang-cl.exe'),
                 '--version'])
  ...
  with api.context(cwd=skia_dir):
    ...
    with api.env(env):
      ...
      try:
        ...
        api.run(api.step, 'icacls grant',
                cmd=['icacls', api.vars.slave_dir, '/t', '/grant',
                     'NT AUTHORITY\Authenticated Users:M'])
        api.run(api.step, 'icacls display',
                cmd=['icacls', api.vars.slave_dir, '/t'])
