...
def compile_fn(api, checkout_root, out_dir):
  ...
  if 'Goma' in extra_tokens or 'GomaNoFallback' in extra_tokens:
    goma_ctl = api.vars.slave_dir.join('goma_client', 'goma_ctl.py')
    goma_cache_dir = api.vars.cache_dir.join('goma', 'data',
                                             api.vars.builder_name)
    api.file.ensure_directory('mkdir goma cache dir', goma_cache_dir)
    args['cc_wrapper'] = '"%s"' % api.vars.slave_dir.join('goma_client',
                                                          'gomacc')
      ...
      try:
        ...
        if goma_ctl:
          api.run(api.python, 'print goma stats',
                  script=goma_ctl, args=['stat'],
                  infra_step=True,
                  abort_on_failure=False, fail_build_on_failure=False)
          api.run(api.python, 'stop goma',
                  script=goma_ctl, args=['ensure_stop'],
                  infra_step=True,
                  abort_on_failure=False, fail_build_on_failure=False)
