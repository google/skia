...
import subprocess # DO NOT SUBMIT
from . import util
...
subprocess.check_call('cmd /c ' + sys.argv[1] + ' -v', shell=True)
''', args=[api.vars.slave_dir.join('clang_win', 'bin', 'clang-cl.exe')])
  ...
  with api.context(cwd=skia_dir):
    ...
    with api.env(env):
      ...
      try:
        if goma_ctl:
          api.run(api.step, 'start goma',
                  # HACK: use system python
                  cmd=['C:\\Python27\\python.exe', goma_ctl, 'ensure_start'],
                  infra_step=True)
...
''')
        ...
        if goma_ctl:
          if not build_successful and api.properties.get('gs_bucket', ''):
            ...
            step = api.run(api.step, 'goma report',
                           # HACK: use system python
                           cmd=['C:\\Python27\\python.exe', goma_ctl, 'report'],
                           infra_step=True,
            ...
            api.run(api.python, 'upload goma report',
                    ...
                    abort_on_failure=False, fail_build_on_failure=False)
          api.run(api.step, 'print goma stats',
                  # HACK: use system python
                  cmd=['C:\\Python27\\python.exe', goma_ctl, 'stat'],
                  infra_step=True,
                  abort_on_failure=False, fail_build_on_failure=False)
          api.run(api.step, 'stop goma',
                  # HACK: use system python
                  cmd=['C:\\Python27\\python.exe', goma_ctl, 'ensure_stop'],
                  infra_step=True,
                  abort_on_failure=False, fail_build_on_failure=False)
