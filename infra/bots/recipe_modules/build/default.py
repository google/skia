  ...
  with api.context(cwd=skia_dir):
    ...
    with api.env(env):
      try:
        ...
        api.run(api.python, 'create goma report',
                script=goma_dir.join('goma_ctl.py'), args=['report'],
                infra_step=True,
                abort_on_failure=False, fail_build_on_failure=False)
        api.file.copy('copy goma report',
                      api.vars.tmp_dir.join('t', 'goma-report.tgz'),
