...
def compile_swiftshader(api, extra_tokens, swiftshader_root, cc, cxx, out):
  ...
  # Extra work for MSAN, if necessary.
  if 'MSAN' in extra_tokens:
    # TODO(borenet): Do this in a persistent dir.
    with api.context(cwd=api.vars.tmp_dir):
      api.run(api.step, 'sync llvm', cmd=['svn', 'co', 'http://llvm.org/svn/llvm-project/llvm/trunk', 'llvm'])
      with api.context(cwd=api.vars.tmp_dir.join('llvm', 'projects')):
        api.run(api.step, 'sync libcxx', cmd=['svn', 'co', 'http://llvm.org/svn/llvm-project/libcxx/trunk', 'libcxx'])
        api.run(api.step, 'sync libxxabi', cmd=['svn', 'co', 'http://llvm.org/svn/llvm-project/libcxxabi/trunk', 'libcxxabi'])
    libcxx_msan = api.vars.tmp_dir.join('libcxx_msan')
    api.file.ensure_directory('makedirs libcxx_msan', libcxx_msan)
    with api.context(cwd=libcxx_msan, env=env):
      api.run(api.step, 'cmake libcxx_msan', cmd=[
        'cmake', '../llvm',
      ...
      api.run(api.step, 'ninja -t targets all', cmd=['ninja', '-t', 'targets', 'all'])
      api.run(api.step, 'ninja cxx', cmd=['ninja', 'cxx'])
    msan_cflags = ' '.join([
      '-fsanitize=memory',
      '-stdlib=libc++',
      '-L%s/lib' % libcxx_msan,
  ...
  with api.context(cwd=out, env=env):
    ...
    api.run(api.step, 'cat CMakeCache.txt', cmd=['cat', 'CMakeCache.txt'])
    api.run(api.step, 'swiftshader ninja',
            cmd=['ninja', '-C', out, '-v', 'libEGL.so', 'libGLESv2.so'])
