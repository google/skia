...
def compile_fn(api, checkout_root, out_dir):
  ...
  if compiler == 'Clang' and 'Win' in os:
    ...
    api.run(api.step, 'clang version',
            cmd=[api.vars.slave_dir.join('clang_win', 'bin', 'clang-cl.exe'),
                 '--version'])
...
for curdir, _, files in os.walk(sys.argv[1], onerror=raise_error):
  print curdir
  subprocess.check_call(['icacls', curdir])
  for f in files:
    p = os.path.join(curdir, f)
    print p
    subprocess.check_call(['icacls', p])
'''
        api.python.inline(name='icacls start_dir',
                          program=ricacls,
                          args=[api.vars.slave_dir])
        api.python.inline(name='icacls out_dir',
                          program=ricacls,
                          args=[out_dir])
