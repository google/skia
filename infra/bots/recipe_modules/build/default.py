...
def compile_fn(api, checkout_root, out_dir):
  ...
  if compiler == 'Clang':
    if os == 'Mac':
      ...
      if not any('SAN' in t for t in extra_tokens) and 'Mac' in os:
        ...
        # We need to use Chromium's dsymutil for compatibility.
        env['PATH'] = '%s:%%(PATH)s' % (
            str(api.vars.slave_dir.join('dsymutil')) + '/bin')
      ...
      cc, cxx = 'clang', 'clang++'
      raise Exception(
          'if you intend to use clang on PATH, remove this exception and '
          'pragma: nocover')
...
def copy_build_products(api, src, dst):
  ...
  if os == 'Mac' and any('SAN' in t for t in extra_tokens):
    ...
    # /clang/10.0.1/lib/darwin, where 10.0.1 could change in future versions.
