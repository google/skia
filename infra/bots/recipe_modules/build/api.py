...
class BuildApi(recipe_api.RecipeApi):
  ...
  def __call__(self, build_dir, out_dir):
    """Compile the code."""
    self.compile_fn(self.m, build_dir, out_dir)

  def copy_build_products(self, build_dir, out_dir, dst):
    """Copy whitelisted build products to dst."""
    util.copy_whitelisted_build_products(self.m, out_dir, dst)
    self.copy_fn(self.m, build_dir, out_dir, dst)
