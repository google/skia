...
def compile_fn(api, checkout_root, out_dir):
    ...
    if compiler == 'GCC':
      if not extra_tokens:
        image = (
            ...
            '6155d6cfa02e976201b8b3934ad234e801e096f809dd97890aa62ac256060036')
        args['cc'] = '"gcc"'
        args['cxx'] = '"g++"'
  if not image:
  ...
  # Run the compile script inside the docker container. It expects three mounts:
  # the Skia checkout at /SRC, the output directory at /OUT, and the Swarming
  # start_dir at /start_dir.
  src_mnt = 'type=bind,source=%s,target=/SRC,readonly' % checkout_root
  out_mnt = 'type=bind,source=%s,target=/OUT' % out_dir
  start_mnt = 'type=bind,source=%s,target=/start_dir,readonly' % (
      api.path['start_dir'])
  inner_script_path = ('/start_dir/recipe_bundle/skia/infra/bots/recipe_modules'
                       '/build/resources/docker-compile.sh')
  cmd = ['docker', 'run', '--rm', '--mount', src_mnt, '--mount', out_mnt,
         '--mount', start_mnt, image, inner_script_path, gn_args]
