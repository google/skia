...
def compile_fn(api, checkout_root, out_dir):
  ...
  script_mnt = 'type=bind,source=%s,target=/docker-compile.sh' % (
      api.resource('docker-compile.sh'))
  cmd = ['docker', 'run', '--rm', '--mount', src_mnt, '--mount', out_mnt,
         '--mount', depot_tools_mnt, '--mount', script_mnt, image,
         '/docker-compile.sh', gn_args]
