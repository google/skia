...
ACTUAL_OUT_DIR = '/tmp/WASM'

def compile_fn(api, checkout_root, _out_dir):
  INNER_BUILD_SCRIPT = '/SRC/skia/experimental/pathkit/docker/build_pathkit.sh'

  api.file.ensure_directory('mkdirs out_dir', ACTUAL_OUT_DIR)

  api.run(
    api.step,
    'Build PathKit with Docker',
    cmd=['docker', 'run', '--rm', '-v', '%s:/SRC' % checkout_root,
         '-v', '%s:/OUT' % ACTUAL_OUT_DIR,
         DOCKER_IMAGE, INNER_BUILD_SCRIPT]
    )


def copy_extra_build_products(api, _out_dir, dst):
  api.python.inline(
