...
def test_steps(api):
  ...
  if 'Vulkan' in bot and 'IntelIris540' in bot and 'Win' in bot:
    for t in ['aarectmodes', 'aaxfermodes', 'dont_clip_to_layer', 'dftext', 'dftext_blob_persp', 'drawregionmodes', 'filterfastbounds', 'fontmgr_iter', 'fontmgr_match', 'fontscaler', 'fontscalerdistortable', 'gammagradienttext', 'gammatext', 'gradtext', 'hairmodes', 'imagefilters_xfermodes', 'imagefiltersclipped', 'imagefiltersscaled', 'imagefiltersstroked', 'imagefilterstransformed', 'imageresizetiled', 'lcdblendmodes', 'lcdoverlap', 'lcdtext', 'lcdtextsize', 'matriximagefilter', 'mixedtextblobs', 'resizeimagefilter', 'rotate_imagefilter', 'savelayer_lcdtext', 'shadermaskfilter_image', 'srcmode', 'surfaceprops', 'textblobgeometrychange', 'textbloblooper', 'textblobmixedsizes', 'textblobrandomfont', 'textfilter_color', 'textfilter_image', 'varied_text_clipped_lcd', 'varied_text_ignorable_clip_lcd']:
      api.run(api.flavor.step, 'dm %s' % t, cmd=args + ['--match', '^%s$' % t], abort_on_failure=False)
  else:
