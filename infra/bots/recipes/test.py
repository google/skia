...
def test_steps(api):
  ...
  if 'Vulkan' in bot and 'Win10' in bot and 'IntelIris655' in bot:
    ...
    for i in range(shards):
      api.run(api.flavor.step, 'dm %d dr' % i, cmd=args + ['--shard', str(i), '--shards', str(shards), '--dryRun'], abort_on_failure=False)
      api.run(api.flavor.step, 'dm %d' % i, cmd=args + ['--shard', str(i), '--shards', str(shards)], abort_on_failure=False)
  else:
