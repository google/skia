...
def test_steps(api):
  ...
  with api.step.context({'env': env}):
    api.run(api.flavor.step, 'dm0', cmd=args+['--shard', '0', '--shards' '5'], abort_on_failure=False)
    api.run(api.flavor.step, 'dm1', cmd=args+['--shard', '1', '--shards' '5'], abort_on_failure=False)
    api.run(api.flavor.step, 'dm2', cmd=args+['--shard', '2', '--shards' '5'], abort_on_failure=False)
    api.run(api.flavor.step, 'dm3', cmd=args+['--shard', '3', '--shards' '5'], abort_on_failure=False)
    api.run(api.flavor.step, 'dm4', cmd=args+['--shard', '4', '--shards' '5'], abort_on_failure=False)
