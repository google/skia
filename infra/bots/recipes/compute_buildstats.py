...
def RunSteps(api):
  api.vars.setup()
  out_dir = api.vars.swarming_out_dir.join('perf')
  ...
  with api.context(cwd=bin_dir):
    ...
    if len(files):
      analyze_wasm(api, out_dir, files)


def analyze_wasm(api, out_dir, files):
  keys = []
  keys_blacklist = ['role']
