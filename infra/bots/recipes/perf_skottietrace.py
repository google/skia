...
lottie_samples_to_skip = [
  # These samples generate trace files which are too large to load into
  # memory.
  # Ask for cateogry????
  # Skip because of consecutive seeks. fmalita@ is investigating.
  # 'gears.json',
  # 'map_animation_edit.json',
  # 'machine.json',
]
...
def perf_steps(api):
  ...
  api.flavor.create_clean_host_dir(
  ...
  for idx, lottie_file in enumerate(lottie_files):
    ...
    if ' ' not in trace_match:
      ...
      # there is a space because ... automatically adds quotes.
      for sp_char in ['(', ')', '\'', '!', '&']:
  ...
  for idx, lottie_file in enumerate(lottie_files):
    ...
    # trace_content = api.file.read_text(
    #     'Read %s.json' % (idx + 1),
    #     api.flavor.host_dirs.dm_dir.join('%s.json' % (idx + 1)),
    #     test_data=test_data)
    trace_content = test_data
    if not trace_content:
      trace_file = api.flavor.host_dirs.dm_dir.join('%s.json' % (idx + 1))
      with open(str(trace_file)) as f:
        trace_content = f.read()
    out = api.json.loads(trace_content)

    print 'trace_content is'
    print trace_content
    print out
    ...
    # mem output
    os.system("free -m")
    # del some things to see what happens
    del trace_content
    del out
    # Force gc to see what happens
    import gc
    gc.collect()
    # mem output
    os.system("free -m")
