...
def perf_steps(api):
  ...
  for lottie_file in api.file.listdir(
      ...
      test_data=['lottie1.json', 'lottie2.json'])[:2]:
    lottie_filename = os.path.basename(api.path.abspath(lottie_file))
    trace_file = api.flavor.device_dirs.dm_dir + '/' + (
        'trace-%s' % lottie_filename)
  ...
  # api.flavor.remove_file_on_device(api.flavor.device_dirs.dm_dir + '/gles')
  # api.flavor.copy_directory_contents_to_host(api.flavor.device_dirs.dm_dir,
  #                                            api.flavor.host_dirs.dm_dir)
  ...
  for lottie_file in api.file.listdir(
    ...
    lottie_file_results['frame_avg_us'] = "%.2f" % (
        frame_cummulative/total_frames)

  # Temperory below just for testing!
  assert perf_results['lottie1.json']['frame_max_us'] == 218.25
  assert perf_results['lottie1.json']['frame_min_us'] == 141.17
  # assert perf_results['lottie1.json']['frame_avg_us'] == 179.71

  # How do I keep these in a perf.json file?
  # Read DM.json and get everything out of it!
  dm_json_device = api.flavor.device_dirs.dm_dir.join('dm.json')
  dm_json_test_data = api.properties.get('dm_json_test_data', '{}')
  dm_content = api.flavor.read_file_on_device(dm_json_device)
  # dm_content = api.file.read_text('Read dm.json', dm_json, test_data=dm_json_test_data)
  if not dm_content and dm_json_test_data:
    dm_content = dm_json_test_data
  ...
  # Add it to perf directory for isolate out?
...
with open('%s', 'w') as outfile:
  ...
  # Do I need to clean up anything? what about the dm_dir on host?
