...
def perf_steps(api):
  ...
  with api.tempfile.temp_dir('dm_trace_outputs') as tmp:
    for lottie_file in api.file.listdir(
      ...
      args.extend(['--writePath', tmp])

      api.run(api.flavor.step, 'dm', cmd=args,
              abort_on_failure=False)

      # This will not work.. need to copy the directory out of the device!
