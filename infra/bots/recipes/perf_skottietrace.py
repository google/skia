...
def perf_steps(api):
  ...
  for lottie_file in api.file.listdir(
      ...
      test_data=['lottie1.json', 'lottie2.json', 'lottie 3.json'])[:10]:
    ...
    trace_file = trace_file.replace(' ', '_')
  ...
  for lottie_file in api.file.listdir(
      'list lottie files', api.flavor.host_dirs.lotties_dir,
      test_data=['lottie1.json', 'lottie2.json', 'lottie_3.json'])[:10]:
    # Read the trace file from the device.
    lottie_filename = os.path.basename(api.path.abspath(lottie_file))
    trace_file = api.flavor.device_dirs.dm_dir + '/trace-%s' % lottie_filename
