...
def perf_steps(api):
  ...
  for idx, lottie_file in enumerate(lottie_files):
    ...
    perf_results[lottie_filename] = parse_trace(trace_json, lottie_filename,
                                                api)
...
def parse_trace(trace_json, lottie_filename, api):
  ...
  for trace in trace_json:
    if SEEK_TRACE_NAME in trace['name']:
      ...
      if frame_start:
        api.run(
            api.python.inline,
            'got consecutive seeks',
            program="""
  raise Exception('We got consecutive Animation::seek without a render for %s. Something is wrong')
  """ % lottie_filename)
  ...
  perf_results['frame_avg_us'] = float("%.2f" % (
      frame_cumulative/total_frames))
  return perf_results
...
"""
  ...
  yield (
      ...
      api.properties(buildername=buildername,
                     ...
                     path_config='kitchen',
                     swarm_out_dir='[SWARM_OUT_DIR]') +
      api.step_data('got consecutive seeks', retcode=1)
  )
  yield (
