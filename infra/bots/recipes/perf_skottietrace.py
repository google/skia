...
def perf_steps(api):
  ...
  for idx, lottie_file in enumerate(lottie_files):
    ...
    # args.extend([
    #     '--match',
    #     '^%s$' % lottie_filename.replace('(', '\(').replace(')', '\)')])
    ...
    # trace_content = api.flavor.read_file_on_device(trace_output)
    # HACKHACK HACK
    api.flavor.copy_directory_contents_to_host(api.flavor.device_dirs.dm_dir,
                                               api.flavor.host_dirs.dm_dir)
    # DELETE HERE?
    trace_content = api.file.read_text('Read /%s.json' % (idx + 1),
                                       api.flavor.host_dirs.dm_dir.join('%s.json' % (idx + 1)), test_data=test_data)
    # trace_content = {}
    # if not trace_content and test_data:
    ...
    # Delete. needed?
    api.flavor.remove_file_on_device(trace_output)
