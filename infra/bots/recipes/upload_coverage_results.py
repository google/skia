...
def RunSteps(api):
  ...
  api.step('Merge and index', cmd=[clang_dir.join('llvm-profdata'),
                                   'merge',
                                   '-sparse',
                                   raw_data,
                                   '-o',
  ...
  cp(api, 'parsed data', indexed_data,
     'gs://%s/%s%s' % (bucket, path, gcs_file))

  # Create output
  output_data = api.path['start_dir'].join('coverage_output')
  api.step('Create Summary', cmd=[clang_dir.join('llvm-cov'),
                                  'show',
                                  executable,
  ...
  cp(api, 'coverage summary', output_data.join('index.txt'),
     'gs://%s/%s%s' % (bucket, path, gcs_file), ['-Z'])

  tar_file = indexed_data = api.path['start_dir'].join('coverage.tar')

  api.step('Create Coverage Archive', cmd=['tar', '-cvf',
                                           tar_file, output_data])

  gcs_file = COVERAGE_TAR_FILE % builder_name
  cp(api, 'coverage data', tar_file,
     'gs://%s/%s%s' % (bucket, path, gcs_file), ['-Z'])
