...
def RunSteps(api):
  ...
  api.infra.cp_gcs('raw data', raw_data,
  ...
  api.infra.cp_gcs('parsed data', indexed_data,
                   'gs://%s/%s%s' % (bucket, path, gcs_file), ['-Z'])
  ...
  api.infra.cp_gcs('coverage summary', output_data.join('index.txt'),
                   'gs://%s/%s%s' % (bucket, path, gcs_file), ['-Z'])
  ...
  api.infra.cp_gcs('text coverage data', tar_file,
                   'gs://%s/%s%s' % (bucket, path, gcs_file), ['-Z'])
  ...
  api.infra.cp_gcs('html coverage data', tar_file,
                   'gs://%s/%s%s' % (bucket, path, gcs_file), ['-Z'])
