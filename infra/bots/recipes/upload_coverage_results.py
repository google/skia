...
def RunSteps(api):
  ...
  cp(api, 'coverage summary', output_data.join('index.txt'), 'gs://%s/%s%s' % (bucket, path, gcs_file), ['-Z'])

  tar_file = COVERAGE_TAR_FILE % builder_name

  api.step('Tar Summary', cmd=['tar', '-cvf', tar_file, output_data])

  cp(api, 'coverage data', tar_file, 'gs://%s/%s%s' % (bucket, path, tar_file), ['-Z'])
