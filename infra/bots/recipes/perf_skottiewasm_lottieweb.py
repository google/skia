...
# Revisit this since we are back on SVG.
...
def RunSteps(api):
  ...
  with api.tempfile.temp_dir('g3_try') as output_dir:
    ...
    for _, lottie_file in enumerate(lottie_files):
      ...
      trace_file_content = api.flavor.read_file_on_device(output_file)
      if not trace_file_content and trace_test_data:
        trace_file_content = trace_test_data

      perf_results[lottie_filename] = {
          'gl': parse_trace(trace_file_content, lottie_filename, api, renderer),
      }
