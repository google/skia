...
subprocess.check_call(['chmod', '-R', 'a+r', os.path.join(checkout_root, 'skia')])
''',
      args=[copy_dest, base_dir, bundle_name, out_dir, checkout_root, checkout_root.join(INNER_KARMA_SCRIPT)],
      infra_step=True)
  ...
  cmd.extend([
      DOCKER_IMAGE,             '/SRC/' + INNER_KARMA_SCRIPT,
