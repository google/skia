...
def RunSteps(api):
  ...
  node_path = api.path['start_dir'].join('node', 'bin', 'node')
  ...
  for idx, lottie_file in enumerate(lottie_files):
    ...
    with api.context(cwd=skottie_wasm_perf_dir, env=env):
      print 'RUNNING THIS'
      print '%s %s --input %s' % (node_path, skottie_wasm_js_path, lottie_file)
      api.step('Run skottie-wasm.js', cmd=[
          node_path, skottie_wasm_js_path, '--input', lottie_file])
