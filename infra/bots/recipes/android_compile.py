...
def RunSteps(api):
  ...
  try:
    ...
    env = {'AWS_CREDENTIAL_FILE': None, 'BOTO_CONFIG': None}  # NEEDED??
    with api.context(env=env):
      gs_file = 'gs://android-compile-tasks/%d-%d.json' % (issue, patchset)
      step_result = api.step('Read task JSON file',
                             ['gsutil', 'cat', gs_file],
...
def GenTests(api):
  ...
  yield(
    ...
    api.step_data('Trigger and wait for task on android-compile.skia.org',
                  retcode=1) +
    api.step_data('Read task JSON file',
