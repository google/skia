  ...
  if 'Vulkan' in api.vars.builder_name and 'IntelIris540' in api.vars.builder_name and 'Win' in api.vars.builder_name:
    ...
    'desk_googlespreadsheet']:
      for repeat in range(5):
        api.run(api.flavor.step, "%s %s %d" % (target, test, repeat), cmd=args + ['--match', test],
                abort_on_failure=False)
  elif 'IntelIris540' in bot and 'ANGLE' in bot:
    for repeat in range(5):
      api.run(api.flavor.step, target, cmd=args + ['--match', 'tile_image_filter_tiled_64'],
                abort_on_failure=False)
  else:
