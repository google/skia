...
def RunSteps(api):
  ...
  with api.context(cwd=api.vars.skia_dir, env=api.infra.go_env):
    ...
    if 'PerCommit' in buildername:  # pragma: nocover
      ...
      try:
        ...
        cmd = [bookmaker_binary,
               '-a', 'docs/status.json',  # File containing status of docs.
               '-p',  # Populate include from bmh.
               ]
        api.run(api.step, 'Display what changed', cmd=cmd, infra_step=True,
                abort_on_failure=False, fail_build_on_failure=False)
        e.reason += ('\n\nRun `bookmaker -a docs/status.json -p` to recreate '
                     'includes from bmh.')
        print dir(api.step.active_result)
        print dir(api.step.active_result.presentation)
        print api.step.active_result.presentation.step_summary_text
        print api.step.active_result.presentation.step_text
        print api.step.active_result.presentation.logs
        raise e
      # rmistry:
      # If this fails then output "out/Release/bookmaker -a docs/status.json -p"
