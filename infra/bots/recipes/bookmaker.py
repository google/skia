  ...
  with api.context(cwd=api.vars.skia_dir, env=api.infra.go_env):
    ...
    if 'PerCommit' in buildername:  # pragma: nocover
      ...
      try:
        ...
        api.run(api.step, 'What needs to be fixed', cmd=cmd, infra_step=True,
                abort_on_failure=False, fail_build_on_failure=False)
        e.reason += (
            '\n\nView the output of the "What needs to be fixed" step to see '
            'how to get this bot green.'
            '\n\nhttps://skia.org/user/api/usingBookmaker details how to build '
            'and run the bookmaker utility locally.')
        raise e
