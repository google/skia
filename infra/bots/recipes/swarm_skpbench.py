...
def skpbench_steps(api):
  ...
  cpu_table = api.path.join(api.vars.swarming_out_dir, 'cpu_table')
  gpu_table = api.path.join(api.vars.swarming_out_dir, 'gpu_table')

  api.run(api.python, 'skpbench (cpu)',
      ...
      args=[
        ...
        '--adb',
        '--resultsfile', cpu_table,
        '--config', 'gpu,esinst4'])

  api.run(api.python, 'skpbench (gpu)',
      script=skpbench_dir.join('skpbench.py'),
      args=[
        api.path.join(api.vars.android_bin_dir, 'skpbench'),
        api.path.join(api.vars.android_data_dir, 'skps'),
        '--adb',
        '--resultsfile', gpu_table,
        '--config', 'gpu,esinst4',
        '--gpu'])
