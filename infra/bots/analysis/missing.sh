...
# against columns in /tmp/allbots.csv, such as '$vulkan == "true"'.
set -e
...
mlr --csv filter "${FILTER}" /tmp/allbots.csv  | mlr --csv cut -f cpu_or_gpu_value,model | mlr --csv sort -f cpu_or_gpu_value | mlr --csv uniq -f cpu_or_gpu_value | mlr --csv join -f /tmp/cpu_or_gpu_value,model.csv -j cpu_or_gpu_value --ul --np