...
# These are derived from above constants.
URL_PREFIX = CDS_URL + '/Mac/'
CDS_FILE = "clang-%s.tgz" % PACKAGE_VERSION
CDS_FULL_URL = URL_PREFIX + CDS_FILE


def create_asset(target_dir):
  """Create the asset."""
  with utils.tmp_dir():
    subprocess.check_call(["curl", CDS_FULL_URL, "-o", "clang_mac.tgz"])
    subprocess.check_call(['tar', '--extract', '--gunzip', '--file',
                           'clang_mac.tgz', '--directory', target_dir])
