Creating the Mesa Intel driver asset (which supports Vulkan)
============================================================

Using the automated asset python scripts requires that Docker be installed.

    mesa_intel_driver$ python create_and_upload.py
...
This is known to work on Ubuntu 18.04.
...
The image only needs to be re-built when the dependencies change or the build_mesa.sh is updated.
    ...
    docker tag mesa-driver-builder gcr.io/skia-public/mesa-driver-builder:latest
    docker push gcr.io/skia-public/mesa-driver-builder:latest