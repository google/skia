...
def create_asset(target_dir, gl_path):
  ...
  # The libdrm on the bots does not necessarily have drmGetDevices2.
  for f in glob.glob(os.path.join(gl_path,'libdrm*')):
    shutil.copy(f, lib_dir)

  for f in glob.glob(os.path.join(gl_path,'libstdc++*')):
    shutil.copy(f, lib_dir)

  shutil.copy(os.path.join(gl_path,'libc.so'), lib_dir)
