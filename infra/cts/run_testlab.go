...
`
	...
	RESULT_DIR_TMPL      = "testruns/%s/testrun-%d"
	CMD_AVAILABE_DEVICES = "gcloud firebase test android models list --format json"
)

var BLACKLIST_DEV_IDS = []string{
	"lt02wifi", // ignore Galaxy Tab 3
}
...
func getAvailableDevices(ignoreIDs []string) ([]*tsuite.FirebaseDevice, []*tsuite.FirebaseDevice, error) {
	...
	cmd := exec.ParseCommand(CMD_AVAILABE_DEVICES)
	cmd.Stdout = &buf
	err := cmd.Run()
...
func runTests(apk_path string, devices, ignoredDevices []*tsuite.FirebaseDevice, client *http.Client) error {
	...
	resultsDir := fmt.Sprintf(RESULT_DIR_TMPL, now.Format("2006/01/02"), nowMs)
	...
	cmd := exec.ParseCommand(cmdStr)
	for _, arg := range cmd.Args {
		sklog.Infof("ARG: %s", arg)
	}

	// return nil
	cmd.CombinedOutput = os.Stdout
	exitCode := 0
	if err := cmd.Run(); err != nil {
		...
		sklog.Infof("Exit code: %d", exitCode)
	...
	writeMetaData(meta, RESULT_BUCKET, resultsDir+"/meta.json", client)
	return nil
}
