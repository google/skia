...
func uploadAPK(apkPath, gcsPath, propStr string, client *http.Client) error {
	...
	if err := writeToGCS(gcsPath, apkFile, client, "application/vnd.android.package-archive", properties, true); err != nil {
		return err
	}
...
func (t *TestRunMeta) writeToGCS(gcsPath string, client *http.Client) error {
	...
	return writeToGCS(gcsPath, bytes.NewBuffer(jsonBytes), client, "", nil, false)
}

// writeToGCS writes the meta data as JSON to the given bucket and path in
// GCS. It assumes that the provided client as permissions to write to the
// specified location in GCS.
func writeToGCS(gcsPath string, reader io.Reader, client *http.Client, contentType string, metaData map[string]string, public bool) error {
