...
// of ignored devices.
func getAvailableDevices() ([]*tsuite.DeviceVersions, DeviceList, error) {
	...
	foundDevices := []*tsuite.DeviceVersions{}
	bufBytes := buf.Bytes()
...
func filterDevices(foundDevices []*tsuite.DeviceVersions, whiteList DeviceList, minAPIVersion, maxAPIVersion int) ([]*tsuite.DeviceVersions, []*tsuite.DeviceVersions) {
	...
	for _, dev := range foundDevices {
		...
		if targetDev := whiteList.find(dev.ID); targetDev != nil && (len(targetDev.RunVersions) > 0) {
			...
			if len(whiteListVersions) > 0 {
				ret = append(ret, &tsuite.DeviceVersions{FirebaseDevice: dev.FirebaseDevice, RunVersions: whiteListVersions})
			}
			ignored = append(ignored, &tsuite.DeviceVersions{FirebaseDevice: dev.FirebaseDevice, RunVersions: ignoredVersions})
		} else {
			ignored = append(ignored, &tsuite.DeviceVersions{FirebaseDevice: dev.FirebaseDevice, RunVersions: dev.VersionIDs})
		}
		allDevices = append(allDevices, &tsuite.DeviceVersions{FirebaseDevice: dev.FirebaseDevice, RunVersions: dev.VersionIDs})
	}
