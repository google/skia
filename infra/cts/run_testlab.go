...
func main() {
	...
	if (*uploadGCSPath != "") && !*dryRun {
		if err := uploadAPK(apkPath, *uploadGCSPath, client); err != nil {
...
func runTests(apk_path string, devices, ignoredDevices []*DeviceVersions, client *http.Client, dryRun bool) error {
	...
	targetPath := fmt.Sprintf("%s/%s", RESULT_BUCKET, resultsDir+"/"+META_DATA_FILENAME)
	return meta.writeToGCS(targetPath, client)
}

func uploadAPK(apkPath, gcsPath string, client *http.Client) error {
	apkFile, err := os.Open(apkPath)
	if err != nil {
		return err
	}
	return writeToGCS(gcsPath, apkFile, client, "application/vnd.android.package-archive")
}
...
func (t *TestRunMeta) writeToGCS(gcsPath string, client *http.Client) error {
	...
	return writeToGCS(gcsPath, ioutil.NopCloser(bytes.NewBuffer(jsonBytes)), client, "")
}

// writeToGCS writes the meta data as JSON to the given bucket and path in
// GCS. It assumes that the provided client as permissions to write to the
// specified location in GCS.
func writeToGCS(gcsPath string, reader io.ReadCloser, client *http.Client, contentType string) error {
