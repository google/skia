...
func uploadAPK(apkPath, gcsPath, propStr string, client *http.Client) error {
	...
	if err := copyReaderToGCS(gcsPath, apkFile, client, "application/vnd.android.package-archive", properties, true); err != nil {
		return err
	}
...
func (t *TestRunMeta) writeToGCS(gcsPath string, client *http.Client) error {
	...
	return copyReaderToGCS(gcsPath, bytes.NewReader(jsonBytes), client, "", nil, false)
}

// copyReaderToGCS reads all available content from the given reader and writes
// it to the given path in GCS.
func copyReaderToGCS(gcsPath string, reader io.Reader, client *http.Client, contentType string, metaData map[string]string, public bool) error {
