...
# For example:
# docker run -v $SKIA_ROOT:/SRC -v /tmp/dockerout:/OUT gcr.io/skia-public/gold-lottie-web-puppeteer:68.0.3440.106_v1 /SRC/infra/lottiecap/docker/test_pathkit.sh
...
cd /SRC/tools/lottiecap/

find /LOTTIE_FILES -not -path /LOTTIE_FILES -exec \
    node /SRC/tools/lottiecap/lottiecap.js --port 8082 \
