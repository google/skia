...
String HCodeGenerator::ParameterType(const Type& type) {
    if (type.fName == "vec2") {
        ...
        return "SkMatrix44";
    } else if (type.fName == "sampler2D") {
        return "sk_sp<GrTextureProxy>";
    } else if (type.fName == "GrColorSpaceXform") {
...
String HCodeGenerator::FieldType(const Type& type) {
    if (type.fName == "sampler2D") {
        return "TextureSampler";
    }
    return ParameterType(type);
}

bool HCodeGenerator::IsSupportedSection(const char* name) {
    return !strcmp(name, BODY_SECTION) ||
           !strcmp(name, CLASS_SECTION) ||
...
bool HCodeGenerator::SectionAcceptsArgument(const char* name) {
...
bool HCodeGenerator::writeSection(const char* name, const char* prefix) {
    ...
    if (found != fSections.end()) {
        this->writef("%s%s", prefix, found->second.c_str());
        return true;
    }
    return false;
}
...
void HCodeGenerator::writeExtraConstructorParams(const char* separator) {
    ...
    const auto found = fSections.find(String(CONSTRUCTOR_PARAMS_SECTION));
    if (found != fSections.end()) {
        const char* s = found->second.c_str();
...
void HCodeGenerator::writeConstructor() {
    ...
    for (const auto& param : fParameters) {
        ...
        if (param->fType.fName == "sampler2D") {
            this->writef("\n    , f%c%s(resourceProvider, std::move(%s))",
                         toupper(name[0]),
                         name + 1,
                         name);
        } else {
            this->writef("\n    , f%c%s(%s)", toupper(name[0]), name + 1, name);
        }
    }
    this->writef(" {\n");
    if (fProgram.fInputs.fDistanceVector) {
        this->writef("        this->setWillUseDistanceVectorField();\n");
    }
        ...
        if (ProgramElement::kVar_Kind == p->fKind) {
            ...
            for (const auto& decl : decls->fVars) {
                if (decl->fVar->fType.kind() == Type::kSampler_Kind) {
                    ...
                    this->writef("        this->addTextureSampler(&f%c%s);\n",
                                 toupper(name[0]),
                                 name + 1);
                }
            }
...
void HCodeGenerator::writeFields() {
    ...
    for (const auto& param : fParameters) {
        const char* name = param->fName.c_str();
        this->writef("    %s f%c%s;\n", FieldType(param->fType).c_str(), toupper(name[0]),
                     name + 1);
    }    
}
...
bool HCodeGenerator::generateCode() {
    ...
    for (const auto& param : fParameters) {
        ...
        const char* name = param->fName.c_str();
        this->writef("%s %s() const { return f%c%s; }\n",
                     FieldType(param->fType).c_str(), name, toupper(name[0]), name + 1);
    }
    this->writeMake();
