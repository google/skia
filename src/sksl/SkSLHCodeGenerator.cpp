...
void HCodeGenerator::writeConstructor() {
    ...
    int usesLocalCoordsDirectly = fProgram.fSource->find("sk_TransformedCoords", 0);
    if (usesLocalCoordsDirectly >= 0) {
        this->writef("        this->setUsesLocalCoordsDirectly();\n");
    }
    ...
    for (const Variable* param : fSectionAndParameterHelper.getParameters()) {
        if (param->fType.kind() == Type::kSampler_Kind) {
            ...
            if (explicitCoords && matrix.isNoOp()) {
                registerFunc = "registerExplicitlySampledChildProcessor";
            } else {
                registerFunc = "registerChildProcessor";
                if (explicitCoords) {
                    explicitArg = ", true";
                }
                switch(matrix.fKind) {
