...
String SymbolWriter::runInFunction(const FunctionDeclaration& f,
    ...
    return String::printf("((void) symbols.reset(symbol_table_for(*%s, symbols)), "
                          "pop_symbols(&symbols, %s))", symbolCode(f).c_str(), code.c_str());
}
...
String SymbolWriter::symbolCode(const Symbol& symbol) {
    ...
    switch (symbol.fKind) {
        case Symbol::kFunctionDeclaration_Kind: {
            ...
            return String::printf("get_function(*symbols, \"%s\", \"%s\")",
                                  String(f.fName).c_str(), f.declaration().c_str());
        }
        case Symbol::kType_Kind: {
        ...
        default:
            return String::printf("(%s*) symbols->get(\"%s\")", typeName,
