...
const UniformCTypeMapper* UniformCTypeMapper::arrayMapper(int count) const {
    static std::map<std::pair<const UniformCTypeMapper*, int>, UniformCTypeMapper> registered;
    std::pair<const UniformCTypeMapper*, int> key(this, count);
    auto found = registered.find(key);
    if (found == registered.end()) {
        registered.insert({key, *this});
        UniformCTypeMapper* created = &registered.find(key)->second;
        created->fArrayCount = count;
        return created;
    }
    return &found->second;
}
