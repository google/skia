...
String evalTemplate(const String& format, const std::vector<String>& tokens,
    ...
    for (size_t i = 0; i < format.size(); i++) {
        if (tokenNameStart >= 0) {
            ...
            if (format[i] == '}') {
                // Skip 2 extra characters at the beginning for the $ and {,
                // which must exist since otherwise tokenNameStart < 0
                String token(format.c_str() + tokenNameStart + 2,
                             i - tokenNameStart - 2);
                // Search for the token in supported list
                bool found = false;
                for (size_t j = 0; j < tokens.size(); j++) {
                    if (token == tokens[j]) {
                        // Found a match so append the value corresponding to j
                        // to the output
                ...
                if (!found) {
                    // Write out original characters as if we didn't consider
                    // it to be a token name
                    stream.writeText("${");
            ...
            // Outside of a token name, so check if this character starts
            // a name: i == $ and i+1 == {
            if (i < format.size() - 1
                && format[i] == '$' && format[i + 1] == '{') {
...
bool determineInlineFromTemplate(const String& uniformTemplate) {
    // True if there is at most one instance of the ${var} template matcher
    // in fUniformTemplate.
    ...
    int secondMatch = uniformTemplate.find("${var}", firstMatch + 6);
...
String UniformCTypeMapper::dirtyExpression(const String& newVar,
                                       const String& oldVar) const {
    if (fSupportsTracking) {
        std::vector<String> tokens = { "newVar", "oldVar" };
        std::vector<const String*> values = { &newVar, &oldVar };
        return evalTemplate(fDirtyExpressionTemplate, tokens, values);
    } else {
        return "";
    }
}

String UniformCTypeMapper::saveState(const String& newVar,
                                 const String& oldVar) const {
    if (fSupportsTracking) {
        std::vector<String> tokens = { "newVar", "oldVar" };
        std::vector<const String*> values = { &newVar, &oldVar };
        return evalTemplate(fSaveStateTemplate, tokens, values);
    } else {
        return "";
    }
}

String UniformCTypeMapper::setUniform(
        const String& pdman, const String& uniform, const String& var) const {
    std::vector<String> tokens = { "pdman", "uniform", "var" };
    std::vector<const String*> values = { &pdman, &uniform, &var };
    return evalTemplate(fUniformTemplate, tokens, values);
}
...
UniformCTypeMapper::UniformCTypeMapper(
    ...
    , fInlineValue(determineInlineFromTemplate(setUniformFormat))
    , fSupportsTracking(enableTracking)
    , fDefaultValue(defaultValue)
    , fDirtyExpressionTemplate(dirtyExpressionFormat)
    , fSaveStateTemplate(saveStateFormat) { }

// NOTE: These would be macros, but C++ initialization lists for the sksl
// type names do not play well with macro parsing.

UniformCTypeMapper REGISTER(
        const char* ctype, const std::vector<String>& skslTypes,
        const char* uniformFormat, const char* defaultValue, const char* dirtyExpression) {
...
// registeredMappers is a static collection of known handlers
std::vector<UniformCTypeMapper> registeredMappers = {
    REGISTER("SkRect", { "half4", "float4", "double4" },
        "${pdman}.set4fv(${uniform}, 1, "                   // to gpu
                "reinterpret_cast<const float*>(&${var}))",
        "SkRect::MakeEmpty()",                              // default value
        "${oldVar}.isEmpty() || ${oldVar} != ${newVar}"),   // dirty check

    REGISTER("SkIRect", { "half4", "float4", "double4" },
        "${pdman}.set4f(${uniform}, 1, "                    // to gpu
                "reinterpret_cast<const int*>(&${var}))",
...
const UniformCTypeMapper* UniformCTypeMapper::Get(const Context& context,
                                                  const Type& type,
                                                  const Layout& layout) {
    String ctype = layout.fCType;
    // If there's no custom ctype declared in the layout, use the
    ...
    for (size_t i = 0; i < registeredMappers.size(); i++) {
        if (registeredMappers[i].ctype() == ctype) {
            // Check for sksl support, since some c types (e.g. SkMatrix) can
            // be used in multiple uniform types and send data to the gpu
            // differently in those conditions
            ...
            for (size_t j = 0; j < supportedSKSL.size(); j++) {
                if (supportedSKSL[j] == skslType) {
                    // Found a match, so return it or an explicitly untracked
                    // version if tracking is disabled in the layout
