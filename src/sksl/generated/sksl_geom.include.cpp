...
symbols->add("EndPrimitive", std::unique_ptr<Symbol>(new FunctionDeclaration(-1, Modifiers(l, 4096), "EndPrimitive", {  }, *(Type*) symbols->get("void"), 1)));
...
symbols->add("EmitStreamVertex", std::unique_ptr<Symbol>(new FunctionDeclaration(-1, Modifiers(l, 4096), "EmitStreamVertex", { simple_parameter("stream", "int", symbols) }, *(Type*) symbols->get("void"), 1)));
symbols->add("sk_InvocationID", std::unique_ptr<Symbol>(new Variable(-1, Modifiers(Layout::builtin(8), 2), "sk_InvocationID", *(Type*) symbols->get("int"), (Variable::Storage) 0, nullptr)));
symbols->add("EndStreamPrimitive", std::unique_ptr<Symbol>(new FunctionDeclaration(-1, Modifiers(l, 4096), "EndStreamPrimitive", { simple_parameter("stream", "int", symbols) }, *(Type*) symbols->get("void"), 1)));
symbols->add("sk_PointSize", std::unique_ptr<Symbol>(new Field(-1, *(Variable*) (*symbols)["sk_PerVertex_$var"], 1)));
symbols->add("EmitVertex", std::unique_ptr<Symbol>(new FunctionDeclaration(-1, Modifiers(l, 4096), "EmitVertex", {  }, *(Type*) symbols->get("void"), 1)));
symbols->add("sk_in", std::unique_ptr<Symbol>(new Variable(-1, Modifiers(Layout::builtin(10002), 2), "sk_in", *new Type("sk_PerVertex[1]", Type::kArray_Kind, *new Type(94, "sk_PerVertex", { Type::Field(Modifiers(Layout::builtin(0), 0), "sk_Position", (Type*) symbols->get("float4")), Type::Field(Modifiers(Layout::builtin(1), 0), "sk_PointSize", (Type*) symbols->get("float")), Type::Field(Modifiers(Layout::builtin(3), 0), "sk_ClipDistance", new Type("float[1]", Type::kArray_Kind, *(Type*) symbols->get("float"), 1)) }), 1), (Variable::Storage) 0, nullptr)));
...
elements.emplace_back(new VarDeclarations(-1, (Type*) symbols->get("int"), make_vector<VarDeclaration>(1, new VarDeclaration((Variable*) symbols->get("sk_InvocationID"), make_vector<Expression>(0), std::unique_ptr<Expression>(nullptr)))));
}
}
