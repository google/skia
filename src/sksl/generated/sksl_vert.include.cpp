...
symbols->add("sk_VertexID", std::unique_ptr<Symbol>(new Variable(-1, Modifiers(Layout::builtin(42), 2), "sk_VertexID", *(Type*) (*symbols)["int"], (Variable::Storage) 0, nullptr)));
symbols->add("sk_ClipDistance", std::unique_ptr<Symbol>(new Field(-1, *(Variable*) (*symbols)["sk_PerVertex_$var"], 2)));
symbols->add("sk_InstanceID", std::unique_ptr<Symbol>(new Variable(-1, Modifiers(Layout::builtin(43), 2), "sk_InstanceID", *(Type*) (*symbols)["int"], (Variable::Storage) 0, nullptr)));
...
elements.emplace_back(new VarDeclarations(-1, (Type*) (*symbols)["int"], make_vector<VarDeclaration>(1, new VarDeclaration((Variable*) (*symbols)["sk_VertexID"], make_vector<Expression>(0), std::unique_ptr<Expression>(nullptr)))));
elements.emplace_back(new VarDeclarations(-1, (Type*) (*symbols)["int"], make_vector<VarDeclaration>(1, new VarDeclaration((Variable*) (*symbols)["sk_InstanceID"], make_vector<Expression>(0), std::unique_ptr<Expression>(nullptr)))));
}
}
