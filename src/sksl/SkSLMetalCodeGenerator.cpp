...
void MetalCodeGenerator::writeIntrinsicCall(const FunctionCall& c) {
    auto i = fIntrinsicMap.find(c.function().fName);
...
void MetalCodeGenerator::writeFunctionCall(const FunctionCall& c) {
    ...
    const auto& entry = fIntrinsicMap.find(function.fName);
    if (entry != fIntrinsicMap.end()) {
        this->writeIntrinsicCall(c);
        return;
    }
    if (function.fBuiltin && function.fName == "atan" && arguments.size() == 2) {
        this->write("atan2");
    } else if (function.fBuiltin && function.fName == "inversesqrt") {
        this->write("rsqrt");
    } else if (function.fBuiltin && function.fName == "inverse") {
        SkASSERT(arguments.size() == 1);
        this->writeInverseHack(*arguments[0]);
    } else if (function.fBuiltin && function.fName == "dFdx") {
        this->write("dfdx");
    } else if (function.fBuiltin && function.fName == "dFdy") {
        // Flipping Y also negates the Y derivatives.
        this->write((fProgram.fSettings.fFlipY) ? "-dfdy" : "dfdy");
    } else {
        this->writeName(function.fName);
    }
    this->write("(");
