...
// Flags an error if an array type is found. Meant to be used in places where GLSL has support for
// array types but Metal doesn't--e.g., constructors of the form `int[2](0, 1)`, array return types
// such as `float[8] myFunc() {}`, etc.
    ...
    if ("main" == f.name()) {
        ...
        for (const ProgramElement* e : fProgram.elements()) {
            if (e->is<GlobalVarDeclaration>()) {
                ...
                this->writeArrayDimensions(intf.variable().type());
                this->write(" [[buffer(");
