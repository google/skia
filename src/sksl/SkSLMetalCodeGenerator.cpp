...
void MetalCodeGenerator::writeFunctionCall(const FunctionCall& c) {
    if (c.fFunction.fBuiltin && "texture" == c.fFunction.fName) {
        const char* dim = "";
        bool proj = false;
    ...
    } else if (c.fFunction.fBuiltin && "atan" == c.fFunction.fName && 2 == c.fArguments.size()) {
        this->write("atan2");
    } else {
...
bool MetalCodeGenerator::requiresInputs(const Expression& e) {
    switch (e.fKind) {
        ...
        case Expression::kVariableReference_Kind: {
            ...
            return v.fVariable.fModifiers.fLayout.fBuiltin == SK_FRAGCOORD_BUILTIN ||
                   ((v.fVariable.fModifiers.fFlags & Modifiers::kIn_Flag) &&
                    Variable::kGlobal_Storage == v.fVariable.fStorage);
        }
        default:
            return false;
    }
