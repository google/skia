...
void MetalCodeGenerator::assembleMatrixFromMatrix(const Type& sourceMatrix, int rows, int columns) {
    ...
    for (int c = 0; c < columns; ++c) {
        ...
        // Determine how many values to take from the source matrix for this row. This could be
        // calculated without a loop but this is O(4), easy to understand, and the optimizer should
        // be able to flatten it.
        int swizzleLength = 0;
        for (int r = 0; r < rows; ++r) {
            if (r < sourceMatrix.rows() && c < sourceMatrix.columns()) {
                ++swizzleLength;
            }
        }
