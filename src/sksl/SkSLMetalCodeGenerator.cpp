...
String MetalCodeGenerator::getOutParamHelper(const FunctionCall& call,
    ...
    //
    // float _skOutParamHelper0_originalFuncName(float var0, float var1, float& outParam) {
    ...
    ++fIndentation;
    for (size_t index = 0; index < outVars.size(); ++index) {
        if (!outVars[index]) {
            continue;
        }
        // float3 var2 = outParam.zyx;
    ...
    // [int _skResult = ] myFunction(inputs, outputs, globals, var0, var1, var2, var3);
