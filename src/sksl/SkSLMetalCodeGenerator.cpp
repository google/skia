...
bool MetalCodeGenerator::matrixConstructHelperIsNeeded(const Constructor& c) {
    ...
    // A matrix construct helper is required if any input argument is a matrix.
    // Helpers are also necessary when any argument would span multiple rows. For instance:
    //
    // float2 x = (1, 2);
    // float3x2(x, 3, 4, 5, 6) = | 1 3 5 | = no helper needed
    //                           | 2 4 6 |
    //
    // float2 x = (2, 3);
    // float3x2(1, x, 4, 5, 6) = | 1 3 5 | = x spans multiple rows; a helper is needed
    //                           | 2 4 6 |
    //
    // float4 x = (1, 2, 3, 4);
    // float2x2(x) = | 1 3 | = x spans multiple rows; a helper is needed
    //               | 2 4 |
    //
