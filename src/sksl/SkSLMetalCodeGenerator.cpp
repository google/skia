...
void MetalCodeGenerator::writeConstructor(const Constructor& c, Precedence parentPrecedence) {
    ...
    for (const std::unique_ptr<Expression>& arg : c.fArguments) {
        this->write(separator);
        separator = ", ";
        if (Type::TypeKind::kMatrix == c.fType.typeKind() &&
...
void MetalCodeGenerator::writeBinaryExpression(const Expression& b,
    ...
    switch (op) {
        case Token::Kind::TK_EQEQ:
            if (b.expressionChild(IRNode::Index::kBinaryLeft).fType.typeKind() ==
                Type::TypeKind::kVector) {
                this->write("all");
                needParens = true;
            }
            break;
    ...
    if (op == Token::Kind::TK_STAREQ && left.fType.typeKind() == Type::TypeKind::kMatrix &&
        right.fType.typeKind() == Type::TypeKind::kMatrix) {
        this->writeMatrixTimesEqualHelper(left.fType, right.fType, b.fType);
    }
...
MetalCodeGenerator::Requirements MetalCodeGenerator::requirements(const Expression* e) {
    ...
    switch (e->kind()) {
        ...
        case Expression::Kind::kBinary:
            return this->requirements(&e->expressionChild(IRNode::Index::kBinaryLeft)) |
                   this->requirements(&e->expressionChild(IRNode::Index::kBinaryRight));
        case Expression::Kind::kIndex: {
