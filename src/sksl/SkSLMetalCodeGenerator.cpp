...
void MetalCodeGenerator::writeInterfaceBlock(const InterfaceBlock& intf) {
    ...
    int currentOffset = 0;
    // Pack types and add char arrays for padding when needed to maintain alignment since MoltenVK
    // wants half types converted to floats.
    for (unsigned int i = 0; i < fields.size(); i++) {
        const Type::Field* curField = &fields[i];
        const Type::Field* nextField = nullptr;
        if (i + 1 < fields.size()) {
            nextField = &fields[i + 1];
        }
        int curFieldOffset = curField->fModifiers.fLayout.fOffset;
        int nextFieldOffset = -1;
        if (nextField) {
            nextFieldOffset = nextField->fModifiers.fLayout.fOffset;
        }
        if (curFieldOffset != -1) {
            if (currentOffset > curFieldOffset) {
                ABORT("Original interface offsets are too close for MoltenVK");
            } else if (currentOffset < curFieldOffset) {
                this->write("char pad");
                this->write(to_string(fPaddingCount++));
                this->write("[");
                this->write(to_string(curFieldOffset - currentOffset));
                this->writeLine("];");
                currentOffset = curFieldOffset;
            }
        }
        const Type* curType = curField->fType;
        std::vector<int> sizes;
        while (curType->kind() == Type::kArray_Kind) {
            sizes.push_back(curType->columns());
            curType = &curType->componentType();
        }
        bool isPacked = false;
        if (curType->kind() != Type::kMatrix_Kind && curFieldOffset != -1) {
            if (nextFieldOffset != -1) {
                if (size(curType, isPacked) + currentOffset > nextFieldOffset) {
                    // pack if the next field's offset requires it
                    this->write(PACKED_PREFIX);
                    isPacked = true;
                }
            }
            if (!isPacked && currentOffset % alignment(curType, isPacked) != 0) {
                // pack if the current field's offset requires it
                this->write(PACKED_PREFIX);
                isPacked = true;
            }
        }
        if (fProgram.fKind == Program::kFragment_Kind && nextField == nullptr &&
            curType->kind() == Type::kVector_Kind && curType->columns() == 3 &&
            curFieldOffset != -1) {
             // FIXME - figure out if its possible/required to generalize this special case
            this->write(PACKED_PREFIX);
        }
        this->writeType(*curType);
        this->write(" " + curField->fName);
        for (int s : sizes) {
            if (s <= 0) {
                this->write("[]");
            } else {
        ...
        this->writeLine(";");
        fInterfaceBlockMap[curField] = &intf;
        currentOffset += size(curType, isPacked);
    }
    if (fProgram.fKind == Program::kFragment_Kind) {
