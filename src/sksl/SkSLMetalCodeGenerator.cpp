...
void MetalCodeGenerator::writeFunctionCall(const FunctionCall& c) {
    ...
    } else if (c.fFunction.fBuiltin && c.fFunction.fName == "atan" && c.fArguments.size() == 2) {
        this->write("atan2");
    } else {
...
void MetalCodeGenerator::writeConstructor(const Constructor& c) {
    ...
    for (const auto& arg : c.fArguments) {
        this->write(separator);
        separator = ", ";
        if (c.fType.kind() == Type::kMatrix_Kind && arg->fType.kind() == Type::kScalar_Kind) {
...
void MetalCodeGenerator::writeVariableReference(const VariableReference& ref) {
    switch (ref.fVariable.fModifiers.fLayout.fBuiltin) {
        ...
        case SK_INVOCATIONID_BUILTIN:
            ...
            break;
        default:
            if (ref.fVariable.fStorage == Variable::kGlobal_Storage &&
...
void MetalCodeGenerator::writeBinaryExpression(const BinaryExpression& b,
    ...
    if (Compiler::IsAssignment(b.fOperator) &&
        b.fLeft->fKind == Expression::kVariableReference_Kind &&
        ((VariableReference&) *b.fLeft).fVariable.fStorage == Variable::kParameter_Storage &&
    ...
    if (b.fOperator != Token::EQ && Compiler::IsAssignment(b.fOperator) &&
        b.fLeft->fKind == Expression::kSwizzle_Kind && !b.fLeft->hasSideEffects()) {
        ...
        case Expression::kVariableReference_Kind: {
            ...
            return SK_FRAGCOORD_BUILTIN == v.fVariable.fModifiers.fLayout.fBuiltin ||
                   ((v.fVariable.fModifiers.fFlags & Modifiers::kIn_Flag) &&
                    v.fVariable.fStorage == Variable::kGlobal_Storage);
        }
        default:
            return false;
    }
