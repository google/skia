...
void GLSLCodeGenerator::writeForStatement(const ForStatement& f) {
    ...
    const bool hasNonemptyInitializer = f.initializer() && !f.initializer()->isEmpty();
    if (hasNonemptyInitializer && f.initializer()->is<Block>()) {
        ...
        // splits these apart into separate statements. We synthesize a scope, emit the
        // vardecls, and then emit the for statement.
        this->writeLine("{");
        ++fIndentation;
        this->writeStatement(*f.initializer());
        this->write("for (; ");
        closeForLoopScope = true;
    } else if (hasNonemptyInitializer) {
        // Our initializer-statement can be emitted normally.
        this->write("for (");
        this->writeStatement(*f.initializer());
    } else {
        // Our for loop has an empty initializer-statement.
        this->write("for (; ");
    }
