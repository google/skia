...
void DSLCPPCodeGenerator::addUniform(const Variable& var) {
    ...
    // Uniforms can't have initial values.
    SkASSERT(!var.initialValue());
    ...
    this->writef("%.*sVar = VarUniformHandle(%s);\n",
                 (int)var.name().size(), var.name().data(), varCppName);

    if (var.modifiers().fLayout.fWhen.fLength) {
        this->writef("    DeclareGlobal(%s);\n", varCppName);

        // If the `when` clause is false, we declare the Var as a const global with a default value.
        this->writef("} else {\n"
                     "    Var(kConst_Modifier, %s, \"%.*s\").swap(%s);\n"
                     "    DeclareGlobal(%s, %s);\n"
                     "}\n",
