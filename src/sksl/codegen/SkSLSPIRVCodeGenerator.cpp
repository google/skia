...
SpvId SPIRVCodeGenerator::writeInstruction(SpvOp_ opCode,
    ...
    if (key.fResultKind == Word::Kind::kNone) {
        // The instruction doesn't return a SpvId, but we can still deduplicate it via the op cache.
...
SpvId SPIRVCodeGenerator::writeStruct(const Type& type, const MemoryLayout& memoryLayout) {
    ...
    // If this struct (or one of identical shape) is already in our cache, return it as-is.
    if (SpvId* cachedOp = fOpCache.find(BuildInstructionKey(SpvOpTypeStruct, words))) {
        return *cachedOp;
    }
