...
SpvId SPIRVCodeGenerator::writeSpecialIntrinsic(const FunctionCall& c, SpecialIntrinsic kind,
    ...
    switch (kind) {
        ...
        case kDFdy_SpecialIntrinsic: {
            ...
            using namespace dsl;
            DSLExpression rtFlip(DSLWriter::IRGenerator().convertIdentifier(/*offset=*/-1, SKSL_RTFLIP_NAME));
            SpvId rtFlipY = this->vectorize(*rtFlip.y().release(), callType.columns(), out);
            SpvId flipped = this->nextId(&callType);
            this->writeInstruction(SpvOpFMul, this->getType(callType), flipped, result, rtFlipY, out);
            result = flipped;
            break;
        }
...
SpvId SPIRVCodeGenerator::writeInterfaceBlock(const InterfaceBlock& intf, bool appendRTFlip) {
    ...
    if (fProgram.fInputs.fUseFlipRTUniform && appendRTFlip) {
        ...
        fields.emplace_back(Modifiers(), SKSL_RTFLIP_NAME, fContext.fTypes.fFloat2.get());
        if (fProgram.fPool) {
            fProgram.fPool->attachToThread();
        }
        const Type* rtFlipStructType =
                fProgram.fSymbols->takeOwnershipOfSymbol(Type::MakeStructType(type.fOffset,
                                                                              String(type.name()),
                                                                              std::move(fields)));
        const Variable* modifiedVar = fProgram.fSymbols->takeOwnershipOfSymbol(
                std::make_unique<Variable>(intfVar.fOffset, &intfVar.modifiers(), intfVar.name(),
                                           rtFlipStructType, intfVar.isBuiltin(), intfVar.storage()));
        InterfaceBlock modifiedCopy(intf.fOffset, modifiedVar, intf.typeName(), intf.instanceName(),
                                    intf.arraySize(), intf.typeOwner());
        SpvId result = this->writeInterfaceBlock(modifiedCopy, false);
        fProgram.fSymbols->add(std::make_unique<Field>(/*offset=*/-1, modifiedVar,
...
void SPIRVCodeGenerator::addRTFlipUniform(int offset) {
    ...
    const Type* intfStruct = fSynthetics.takeOwnershipOfSymbol(Type::MakeStructType(/*offset=*/-1,
                                                                                    name,
                                                                                    fields));
