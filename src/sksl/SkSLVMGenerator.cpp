...
// It's awkward to use % for multiply, but an infix operator keeps things terse,
// and % is both unused for floats and has the exact precedence we need.
static skvm::F32 operator%(skvm::F32 x, skvm::F32 y) { return fast_mul(x,y); }
static skvm::F32 operator%(float     x, skvm::F32 y) { return fast_mul(x,y); }
