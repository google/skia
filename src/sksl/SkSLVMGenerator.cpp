...
private:
     ...
     * Every Variable is mapped to a Location -- a contiguous set of slots in our vector of
     ...
     * (and eliminate the Location indirection), but it would add overhead for each Variable,
     ...
     *
     * For a VariableReference, producing a Value is straightforward - we get the Location of the
     * Variable, use that to look up the current skvm::Vals holding the variable's contents, and
     * construct a Value with those ids.
     */

    /**
     * Returns the Location holding v's Val(s). Allocates storage if this is first time 'v' is
     * referenced. Compound variables (e.g. vectors) will consume more than one slot, with the
     * getLocation return value indicating the start of the contiguous chunk of slots.
     */
    Location getLocation(const Variable& v);

    /**
     * As above, but computes the Location of an expression involving indexing & field access.
     * The expression doesn't have separate storage - this returns the Location of the underlying
...
Value SkVMGenerator::writeBinaryExpression(const BinaryExpression& b) {
    ...
    switch (op) {
        ...
        case Token::Kind::TK_SHR: {
            // TODO: Support variable shifts, once these operators are allowed in public SkSL
            SkASSERT(nk == Type::NumberKind::kSigned || nk == Type::NumberKind::kUnsigned);
            SkASSERT(right.isCompileTimeConstant());
            int64_t shift = right.getConstantInt();
            SkASSERT(shift >= 0 && shift <= 31);
    ...
    if (op == Token::Kind::TK_STAR && lVecOrMtx && rVecOrMtx &&
        ...
        for (int r = 0; r < lRows; ++r) {
            ...
            result[resultIdx++] = sum.id;
        }
        SkASSERT(resultIdx == result.size());
        if (isAssignment) {
    ...
    auto binary = [&](auto&& f_fn, auto&& i_fn) {
        for (size_t i = 0; i < count; ++i) {
            ...
            result[i] = nk == Type::NumberKind::kFloat ? f_fn(f32(l_id), f32(r_id)).id
            ...
            if (srcType.isMatrix()) {
                ...
                for (int r = 0; r < dstType.rows(); ++r) {
                    if (c < srcType.columns() && r < srcType.rows()) {
                        ...
                        dst[dstIndex++] = fBuilder->splat(c == r ? 1.0f : 0.0f).id;
                    }
                }
                ...
                for (int c = 0; c < dstType.columns(); ++c)
                for (int r = 0; r < dstType.rows(); ++r) {
                    dst[dstIndex++] = (c == r ? f32(src) : fBuilder->splat(0.0f)).id;
                }
            } else {
                SkDEBUGFAIL("Invalid matrix constructor");
            }
            SkASSERT(dstIndex == dst.size());
            return dst;
        }
    ...
    if (int builtin = expression_as_builtin(e); builtin >= 0) {
        switch (builtin) {
            case SK_FRAGCOORD_BUILTIN: {
                Value result(4);
                result[0] = fDeviceCoord.x.id;
                result[1] = fDeviceCoord.y.id;
                result[2] = fBuilder->splat(0.0f).id;
                result[3] = fBuilder->splat(1.0f).id;
                return result;
            }
            default:
...
Value SkVMGenerator::writeMatrixInverse3x3(const Value& m) {
    ...
    Value result(m.size());
    result[0] = ((a22 * a33 - a23 * a32) * idet).id;
    result[1] = ((a23 * a31 - a21 * a33) * idet).id;
    result[2] = ((a21 * a32 - a22 * a31) * idet).id;
    result[3] = ((a13 * a32 - a12 * a33) * idet).id;
    result[4] = ((a11 * a33 - a13 * a31) * idet).id;
    result[5] = ((a12 * a31 - a11 * a32) * idet).id;
    result[6] = ((a12 * a23 - a13 * a22) * idet).id;
    result[7] = ((a13 * a21 - a11 * a23) * idet).id;
    result[8] = ((a11 * a22 - a12 * a21) * idet).id;
    return result;
}
...
Value SkVMGenerator::writeMatrixInverse4x4(const Value& m) {
    ...
    Value result(m.size());
    result[ 0] = (a11 * b11 - a12 * b10 + a13 * b09).id;
    result[ 1] = (a02 * b10 - a01 * b11 - a03 * b09).id;
    result[ 2] = (a31 * b05 - a32 * b04 + a33 * b03).id;
    result[ 3] = (a22 * b04 - a21 * b05 - a23 * b03).id;
    result[ 4] = (a12 * b08 - a10 * b11 - a13 * b07).id;
    result[ 5] = (a00 * b11 - a02 * b08 + a03 * b07).id;
    result[ 6] = (a32 * b02 - a30 * b05 - a33 * b01).id;
    result[ 7] = (a20 * b05 - a22 * b02 + a23 * b01).id;
    result[ 8] = (a10 * b10 - a11 * b08 + a13 * b06).id;
    result[ 9] = (a01 * b08 - a00 * b10 - a03 * b06).id;
    result[10] = (a30 * b04 - a31 * b02 + a33 * b00).id;
    result[11] = (a21 * b02 - a20 * b04 - a23 * b00).id;
    result[12] = (a11 * b07 - a10 * b09 - a12 * b06).id;
    result[13] = (a00 * b09 - a01 * b07 + a02 * b06).id;
    result[14] = (a31 * b01 - a30 * b03 - a32 * b00).id;
    result[15] = (a20 * b03 - a21 * b01 + a22 * b00).id;
    return result;
}
...
Value SkVMGenerator::writeIntrinsicCall(const FunctionCall& c) {
    ...
    auto binary = [&](auto&& fn) {
        ...
        for (size_t i = 0; i < count; ++i) {
            result[i] = fn({fBuilder, args[0][std::min(i, args[0].size() - 1)]},
                           {fBuilder, args[1][std::min(i, args[1].size() - 1)]}).id;
        }
        return result;
    };
    ...
    auto ternary = [&](auto&& fn) {
        ...
        for (size_t i = 0; i < count; ++i) {
            result[i] = fn({fBuilder, args[0][std::min(i, args[0].size() - 1)]},
                           ...
                           {fBuilder, args[2][std::min(i, args[2].size() - 1)]}).id;
        }
        return result;
    };
...
Value SkVMGenerator::writeTernaryExpression(const TernaryExpression& t) {
    ...
    Value result(count);
    for (size_t i = 0; i < count; ++i) {
        result[i] = skvm::select(i32(test), i32(ifTrue[i]), i32(ifFalse[i])).id;
    }
    return result;
}
...
skvm::Color ProgramToSkVM(const Program* program,
    ...
    for (const SkSL::Variable* param : function->declaration().parameters()) {
        numParams += SlotCount(param->type());
    }
    SkASSERT(numParams <= SK_ARRAY_COUNT(params));

    SkVMGenerator generator(program, function, builder, uniforms, {params, numParams},
