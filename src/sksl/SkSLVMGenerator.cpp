...
private:
    ...
    skvm::I32 mask() { return fMask & ~topFunction().fReturned; }
    ...
    Function& topFunction() { return fFunctionStack.back(); }
...
Value SkVMGenerator::writeFunctionCall(const FunctionCall& f) {
    ...
    auto argIter = f.arguments().begin();
    size_t valIdx = 0;
    for (const Variable* p : decl.parameters()) {
        size_t nslots = slot_count(p->type());
        if (p->modifiers().fFlags & Modifiers::kOut_Flag) {
...
void SkVMGenerator::writeReturnStatement(const ReturnStatement& r) {
    ...
    if (r.expression()) {
        ...
        auto result = topFunction().fReturnValue.begin();
        for (size_t i = 0; i < val.slots(); ++i) {
            result[i] = select(returnsHere, f32(val[i]), f32(result[i])).id;
        }
    }

    topFunction().fReturned |= returnsHere;
}
