...
Value SkVMGenerator::writeBinaryExpression(const BinaryExpression& b) {
    ...
    // All of the other ops always evaluate both sides of the expression, and don't require any
    ...
    auto binary = [&](auto&& f_fn, auto&& i_fn) {
        ...
        for (size_t i = 0; i < nslots; ++i) {
            ...
            skvm::Val left  = lVal.slots() == 1 ? lVal[0] : lVal[i],
                      right = rVal.slots() == 1 ? rVal[0] : rVal[i];
            if (nk == Type::NumberKind::kFloat) {
                result[i] = f_fn(f32(left), f32(right));
            } else {
                result[i] = i_fn(i32(left), i32(right));
            }
        }
        return isAssignment ? this->writeStore(left, result) : result;
    };
    ...
    auto binary = [&](auto&& fn) {
        ...
        for (size_t i = 0; i < nslots; ++i) {
            result[i] = fn({fBuilder, args[0][args[0].slots() == 1 ? 0 : i]},
                           {fBuilder, args[1][args[1].slots() == 1 ? 0 : i]}).id;
        }
        return result;
    };
    ...
    auto ternary = [&](auto&& fn) {
        ...
        for (size_t i = 0; i < nslots; ++i) {
            result[i] = fn({fBuilder, args[0][args[0].slots() == 1 ? 0 : i]},
                           ...
                           {fBuilder, args[2][args[2].slots() == 1 ? 0 : i]}).id;
        }
        return result;
    };
...
Value SkVMGenerator::writeTernaryExpression(const TernaryExpression& t) {
    ...
    Value result(nslots);
    for (size_t i = 0; i < nslots; ++i) {
        result[i] = skvm::select(test, i32(ifTrue[i]), i32(ifFalse[i])).id;
    }
    return result;
}
