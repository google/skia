...
Value SkVMGenerator::writeIntrinsicCall(const FunctionCall& c) {
    ...
    switch (found->second) {
        ...
        case Intrinsic::kFaceforward: {
            ...
            skvm::F32 NrefdotI = dot(Nref, I);
            return unary(N, [&](skvm::F32 N_x) { return select(NrefdotI<0, N_x, -N_x); });
        }
        case Intrinsic::kReflect: {
            const Value &I = args[0],
                        &N = args[1];

            skvm::F32 NdotI = dot(N, I);
            return binary([&](skvm::F32 I_x, skvm::F32 N_x) { return I_x - 2*NdotI*N_x; });
        }
        case Intrinsic::kRefract: {
            ...
            return binary([&](skvm::F32 I_x, skvm::F32 N_x) {
                return select(k<0, 0.0f, eta*I_x - (eta*NdotI + sqrt(k))*N_x);
            });
        }
