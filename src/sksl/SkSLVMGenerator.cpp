...
private:
    ...
    int getClampedIndex(const IndexExpression& expr);
...
SkVMGenerator::Slot SkVMGenerator::getSlot(const Expression& e) {
    switch (e.kind()) {
        ...
        case Expression::Kind::kIndex: {
            ...
            return baseSlot + this->getClampedIndex(i) * stride;
        }
...
Value SkVMGenerator::writeFieldAccess(const FieldAccess& expr) {
    ...
    for (int i = 0; i < expr.fieldIndex(); ++i) {
        start += slot_count(*expr.base()->type().fields()[i].fType);
    }
    Value field(slot_count(expr.type()));
    for (size_t i = 0; i < field.slots(); ++i) {
        field[i] = base[start + i];
    }
    return field;
}

int SkVMGenerator::getClampedIndex(const IndexExpression& expr) {
    ...
    return SkTPin(indexValue, 0, expr.base()->type().columns() - 1);
}

Value SkVMGenerator::writeIndexExpression(const IndexExpression& expr) {
    Value base = this->writeExpression(*expr.base());
    int indexValue = this->getClampedIndex(expr);
    size_t elementSlotCount = slot_count(expr.type());
    Value element(elementSlotCount);
    for (size_t i = 0; i < elementSlotCount; ++i) {
        element[i] = base[indexValue*elementSlotCount + i];
    }
    return element;
}
