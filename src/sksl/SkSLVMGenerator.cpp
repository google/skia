        ...
        switch (dstKind) {
            case Type::NumberKind::kFloat:
                if (srcKind == Type::NumberKind::kSigned) {
                    ...
                    // bool -> float
                    for (size_t i = 0; i < src.slots(); ++i) {
                        dst[i] = skvm::select(i32(src[i]), f32(1), f32(0));
                    }
                    return dst;
                }
                break;
            ...
            case Type::NumberKind::kBoolean:
                if (srcKind == Type::NumberKind::kFloat || srcKind == Type::NumberKind::kSigned) {
                    // int|float -> bool
                    for (size_t i = 0; i < src.slots(); ++i) {
                        dst[i] = skvm::select(i32(src[i]), i32(0xFFFF'FFFF), i32(0));
                    }
                    return dst;
                }
                break;
