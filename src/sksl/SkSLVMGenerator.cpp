...
SkVMGenerator::SkVMGenerator(const Program* program,
    ...
    for (const ProgramElement* e : fProgram.elements()) {
        if (e->is<GlobalVarDeclaration>()) {
            ...
            if (e->as<FunctionDefinition>().declaration().name() == functionName) {
    ...
    if (fFunction) {
        ...
        if (fReturnValue.size() != returnCount) {
            fErrors.error(-1, "Expected " + to_string(returnCount) +
                                      " return slots, but received " +
                                      to_string(fReturnValue.size()));
            fFunction = nullptr;
        }
        ...
        fErrors.error(-1, "Function '" + String(functionName) + "' not found");
    }
}
...
Value SkVMGenerator::writeIntrinsicCall(const FunctionCall& c) {
    ...
    switch (found->second) {
        ...
        case Intrinsic::kInverse: return {}; // TODO
...
bool ProgramToSkVM(const Program* program,
                   ...
                   SampleChildFn sampleChild,
                   SkSpan<skvm::Val> outReturn) {
    TrivialErrorReporter errors;
    SkVMGenerator generator(program, functionName, builder, uniforms, params, deviceCoord,
