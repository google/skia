        ...
        if (e.fKind == Expression::kFunctionCall_Kind) {
            ...
            if (is_sample_call_to_fp(fc, fFP)) {
                ...
                if (lastArg->fType == *context.fFloat2_Type) {
                    fUsage.merge(SampleUsage::MakeExplicitCoords());
                    ...
                    if (lastArg->isConstantOrUniform()) {
                        if (lastArg->fKind == Expression::Kind::kVariableReference_Kind ||
                            ...
                            fUsage.merge(SampleUsage::MakeConstUniform(lastArg->description()));
                        } else {
                            ...
                            // names. Once they are tracked separately, any constant/uniform
                            // expression can work, but right now this avoids issues from '0.5 *
                            // matrix' that is both a constant AND a uniform.
                            fUsage.merge(SampleUsage::MakeVariable());
                        }
                    } else {
                        fUsage.merge(SampleUsage::MakeVariable());
                    }
                    ...
                    fUsage.merge(SampleUsage::MakePassThrough());
                }
