...
bool TProgramVisitor<PROG, EXPR, STMT, ELEM>::visitStatement(STMT s) {
    switch (s.kind()) {
        ...
        case Statement::Kind::kExpression:
            return this->visitExpression(*s.template as<ExpressionStatement>().fExpression);
...
bool TProgramVisitor<PROG, EXPR, STMT, ELEM>::visitProgramElement(ELEM pe) {
    switch (pe.kind()) {
        ...
        case ProgramElement::Kind::kInterfaceBlock:
            for (const auto& e : pe.template as<InterfaceBlock>().fSizes) {
        ...
        case ProgramElement::Kind::kVar:
            for (const auto& v : pe.template as<VarDeclarations>().fVars) {
