...
bool Analysis::ForLoopIsValidForES2(const ForStatement& loop,
    ...
    //
    if (!loop.initializer()) {
        return invalid("missing init declaration");
    }
    if (!loop.initializer()->is<VarDeclaration>()) {
        return invalid("invalid init declaration");
    }
    const VarDeclaration& initDecl = loop.initializer()->as<VarDeclaration>();
    if (!initDecl.baseType().isInteger() && !initDecl.baseType().isFloat()) {
        return invalid("invalid type for loop index");
    }
    if (initDecl.arraySize() != 0) {
        return invalid("invalid type for loop index");
    }
    if (!initDecl.value()) {
        return invalid("missing loop index initializer");
    }
    if (!getConstant(initDecl.value(), &loopInfo->fStart)) {
        return invalid("loop index initializer must be a constant expression");
    }
    ...
    //
    if (!loop.test()) {
        return invalid("missing condition");
    }
    ...
    if (!is_loop_index(cond.left())) {
        return invalid("expected loop index on left hand side of condition");
    }
    ...
    switch (cond.getOperator()) {
        ...
        case Token::Kind::TK_NEQ:
            break;
        default:
            return invalid("invalid relational operator");
    }
    double loopEnd = 0;
    if (!getConstant(cond.right(), &loopEnd)) {
        return invalid("loop index must be compared with a constant expression");
    }
    ...
    switch (loop.next()->kind()) {
        ...
        default:
            return invalid("invalid loop expression");
    }
    ...
    if (Analysis::StatementWritesToVariable(*loop.statement(), initDecl.var())) {
        return invalid("loop index must not be modified within body of the loop");
    }
    ...
    if (loopInfo->fCount > kMaxUnrollableLoopLength) {
        return invalid("loop must guarantee termination in fewer iterations");
    }
