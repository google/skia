...
static bool innerRun(const ByteCode* byteCode, const ByteCodeFunction* f, VValue* stack,
                     ...
                     int baseIndex) {
#ifdef THREADED
    // If you're not familiar with the &&label syntax, it's part of the GCC / Clang "labels as
    // values" extension. We use this to implement threaded code, where each opcode jumps directly
    // to the next opcode to execute rather than having a for loop around a switch statement.
    ...
    LABEL(kDup3)         PUSH(sp[(int)ByteCodeInstruction::kDup - (int)inst]);
    LABEL(kDup2)         PUSH(sp[(int)ByteCodeInstruction::kDup - (int)inst]);
    LABEL(kDup)          PUSH(sp[(int)ByteCodeInstruction::kDup - (int)inst]);
                         NEXT();
    ...
    LABEL(kLoad3)         sp[3] = stack[*ip + 2];
    LABEL(kLoad2)         sp[2] = stack[*ip + 1];
    LABEL(kLoad)          sp[1] = stack[*ip + 0];
                          ++ip;
                          sp += COUNT(kLoad);
                          NEXT();

    CHECKED_LABEL(kLoadGlobal4) sp[4] = globals[*ip + 3];
    LABEL(kLoadGlobal3)         sp[3] = globals[*ip + 2];
    LABEL(kLoadGlobal2)         sp[2] = globals[*ip + 1];
    LABEL(kLoadGlobal)          sp[1] = globals[*ip + 0];
                                ++ip;
