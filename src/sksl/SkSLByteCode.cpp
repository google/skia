...
// A label which asserts that it matches the instruction used to jump to it. Use for labels to which
// there is no fallthrough.
#define CHECKED_LABEL(name) name: SkASSERT(inst == ByteCodeInstruction::name);
...
#define NEXT()                                        \
    ...
    printf(" (stack: %d)\n", (int) (sp - stack) + 1); \
    inst = (ByteCodeInstruction) READ16();            \
    goto *labels[(int) inst]
#else
#define NEXT()                             \
    inst = (ByteCodeInstruction) READ16(); \
    goto *labels[(int) inst]
...
// computes the count (1 - 4) of the current instruction, given the name of the base instruction
#define COUNT(base) (4 - (int)inst + (int)ByteCodeInstruction::base ## 4)

#define VECTOR_BINARY_OP(base, field, op)             \
    CHECKED_LABEL(base ## 4)                          \
        sp[-4] = sp[-4].field op sp[0].field;         \
        POP();                                        \
        /* fall through */                            \
    base ## 3: {                                      \
        int target = -COUNT(base);                    \
        sp[target] = sp[target].field op sp[0].field; \
        POP();                                        \
    }   /* fall through */                            \
    base ## 2: {                                      \
        int target = -COUNT(base);                    \
...
#define VECTOR_UNARY_FN(base, fn, field)                 \
    CHECKED_LABEL(base ## 4)  sp[-3] = fn(sp[-3].field); \
    base ## 3:                sp[-2] = fn(sp[-2].field); \
    base ## 2:                sp[-1] = fn(sp[-1].field); \
    base:                     sp[ 0] = fn(sp[ 0].field); \
                              NEXT();
...
static bool innerRun(const ByteCode* byteCode, const ByteCodeFunction* f, VValue* stack,
    ...
    CHECKED_LABEL(kConvertFtoI4) sp[-3] = skvx::cast<int>(sp[-3].fFloat);
    kConvertFtoI3:               sp[-2] = skvx::cast<int>(sp[-2].fFloat);
    kConvertFtoI2:               sp[-1] = skvx::cast<int>(sp[-1].fFloat);
    kConvertFtoI:                sp[ 0] = skvx::cast<int>(sp[ 0].fFloat);
                                 NEXT();

    CHECKED_LABEL(kConvertStoF4) sp[-3] = skvx::cast<float>(sp[-3].fSigned);
    kConvertStoF3:               sp[-2] = skvx::cast<float>(sp[-2].fSigned);
    kConvertStoF2:               sp[-1] = skvx::cast<float>(sp[-1].fSigned);
    kConvertStoF:                sp[ 0] = skvx::cast<float>(sp[ 0].fSigned);
                                 NEXT();

    CHECKED_LABEL(kConvertUtoF4) sp[-3] = skvx::cast<float>(sp[-3].fUnsigned);
    kConvertUtoF3:               sp[-2] = skvx::cast<float>(sp[-2].fUnsigned);
    kConvertUtoF2:               sp[-1] = skvx::cast<float>(sp[-1].fUnsigned);
    kConvertUtoF:                sp[ 0] = skvx::cast<float>(sp[ 0].fUnsigned);
                                 NEXT();
    ...
    CHECKED_LABEL(kDup4) PUSH(sp[(int)ByteCodeInstruction::kDup - (int)inst]);
    kDup3:               PUSH(sp[(int)ByteCodeInstruction::kDup - (int)inst]);
    kDup2:               PUSH(sp[(int)ByteCodeInstruction::kDup - (int)inst]);
    kDup:                PUSH(sp[(int)ByteCodeInstruction::kDup - (int)inst]);
                         NEXT();
    ...
    CHECKED_LABEL(kLoad4) sp[4] = stack[*ip + 3];
    kLoad3:               sp[3] = stack[*ip + 2];
    kLoad2:               sp[2] = stack[*ip + 1];
    kLoad:                sp[1] = stack[*ip + 0];
                          ++ip;
                          sp += COUNT(kLoad);
                          NEXT();

    CHECKED_LABEL(kLoadGlobal4) sp[4] = globals[*ip + 3];
    kLoadGlobal3:               sp[3] = globals[*ip + 2];
    kLoadGlobal2:               sp[2] = globals[*ip + 1];
    kLoadGlobal:                sp[1] = globals[*ip + 0];
                                ++ip;
                                sp += COUNT(kLoadGlobal);
                                NEXT();

    CHECKED_LABEL(kLoadExtended) {
        int count = READ8();
        I32 src = POP().fSigned;
        I32 m = mask();
        for (int i = 0; i < count; ++i) {
    ...
    CHECKED_LABEL(kLoadExtendedGlobal) {
        int count = READ8();
        I32 src = POP().fSigned;
        I32 m = mask();
        for (int i = 0; i < count; ++i) {
    ...
    CHECKED_LABEL(kLoadSwizzleGlobal) {
        int src = READ8();
        int count = READ8();
        for (int i = 0; i < count; ++i) {
    ...
    CHECKED_LABEL(kNegateF4) sp[-3] = -sp[-3].fFloat;
    kNegateF3:               sp[-2] = -sp[-2].fFloat;
    kNegateF2:               sp[-1] = -sp[-1].fFloat;
    kNegateF:                sp[ 0] = -sp[ 0].fFloat;
                             NEXT();

    CHECKED_LABEL(kNegateFN) {
        int count = READ8();
    ...
    CHECKED_LABEL(kNegateI4) sp[-3] = -sp[-3].fSigned;
    kNegateI3:               sp[-2] = -sp[-2].fSigned;
    kNegateI2:               sp[-1] = -sp[-1].fSigned;
    kNegateI:                sp[ 0] = -sp[ 0].fSigned;
                             NEXT();

    CHECKED_LABEL(kPop4) POP();
    kPop3:               POP();
    kPop2:               POP();
    kPop:                POP();
                         NEXT();
    ...
    CHECKED_LABEL(kPushImmediate)
        PUSH(U32(READ32()));
        NEXT();

    CHECKED_LABEL(kReadExternal)
    kReadExternal2:
    kReadExternal3:
    kReadExternal4: {
        int count = COUNT(kReadExternal);
        int src = READ8();
        float tmp[4];
        I32 m = mask();
        for (int i = 0; i < VecWidth; ++i) {
    ...
    CHECKED_LABEL(kReserve)
        sp += READ8();
        NEXT();

    CHECKED_LABEL(kReturn) {
        int count = READ8();
        if (frames.empty()) {
            if (outReturn) {
    ...
    CHECKED_LABEL(kStoreExtendedGlobal) {
        int count = READ8();
        I32 target = POP().fSigned;
        VValue* src = sp - count + 1;
        I32 m = mask();
    ...
    CHECKED_LABEL(kStoreSwizzleGlobal) {
        int target = READ8();
        int count = READ8();
        for (int i = count - 1; i >= 0; --i) {
    ...
    CHECKED_LABEL(kStoreSwizzleIndirect) {
        int count = READ8();
        I32 target = POP().fSigned;
        I32 m = mask();
        for (int i = count - 1; i >= 0; --i) {
    ...
    CHECKED_LABEL(kStoreSwizzleIndirectGlobal) {
        int count = READ8();
        I32 target = POP().fSigned;
        I32 m = mask();
        for (int i = count - 1; i >= 0; --i) {
    ...
    CHECKED_LABEL(kWriteExternal4)
    kWriteExternal3:
    kWriteExternal2:
    kWriteExternal: {
        int count = COUNT(kWriteExternal);
        int target = READ8();
        float tmp[4];
        I32 m = mask();
        sp -= count;
    ...
    CHECKED_LABEL(kMaskPush)
        condPtr[1] = POP().fSigned;
        maskPtr[1] = maskPtr[0] & condPtr[1];
        ++condPtr; ++maskPtr;
        NEXT();
    CHECKED_LABEL(kMaskPop)
        --condPtr; --maskPtr;
        NEXT();
    CHECKED_LABEL(kMaskNegate)
        maskPtr[0] = maskPtr[-1] & ~condPtr[0];
        NEXT();
    CHECKED_LABEL(kMaskBlend) {
        int count = READ8();
        I32 m = condPtr[0];
        --condPtr; --maskPtr;
        for (int i = 0; i < count; ++i) {
    ...
    CHECKED_LABEL(kBranchIfAllFalse) {
        int target = READ16();
        if (!skvx::any(mask())) {
            ip = code + target;
        }
    ...
    CHECKED_LABEL(kLoopBegin)
        contPtr[1] = 0;
        loopPtr[1] = loopPtr[0];
        ++contPtr; ++loopPtr;
        NEXT();
    CHECKED_LABEL(kLoopNext)
        *loopPtr |= *contPtr;
        *contPtr = 0;
        NEXT();
    CHECKED_LABEL(kLoopMask)
        *loopPtr &= POP().fSigned;
        NEXT();
    CHECKED_LABEL(kLoopEnd)
        --contPtr; --loopPtr;
        NEXT();
    CHECKED_LABEL(kLoopBreak)
        *loopPtr &= ~mask();
        NEXT();
    CHECKED_LABEL(kLoopContinue) {
        I32 m = mask();
        *contPtr |=  m;
        *loopPtr &= ~m;
        NEXT();
