...
static const uint8_t* disassemble_instruction(const uint8_t* ip) {
    ...
    #else
        #define NEXT() goto *((char*) base + READ16())
    #endif
#else
...
static bool innerRun(const ByteCode* byteCode, const ByteCodeFunction* f, VValue* stack,
    ...
    static SkOnce once;
    once([] {
        // Find the smallest label. Due to optimization, we can't just assume labels[0] comes first.
        base = (void*) ~0;
        for (size_t i = 0; i < SK_ARRAY_COUNT(labels); ++i) {
            base = (void*) std::min((uintptr_t) base, (uintptr_t) labels[i]);
        }
    });
    if (!f->fPreprocessed) {
        ((ByteCodeFunction*) f)->preprocess(labels, base);
    }
#endif
    ...
    LABEL(kCall) {
        ...
        if (!fun->fPreprocessed) {
            ((ByteCodeFunction*) fun)->preprocess(labels, base);
        }
#endif
        if (skvx::any(mask())) {
...
void ByteCodeFunction::preprocess(const void* labels[], const void* base) {
#if defined(SK_ENABLE_SKSL_INTERPRETER)
#ifdef TRACE
    this->disassemble();
#endif
    ...
    while (ip < fCode.data() + fCode.size()) {
        ...
        intptr_t offset = (char*) labels[(int) inst] - (char*) base;
        // if you trip this assert, the code in innerRun has gotten too big for 64K offsets.
        SkASSERT(offset >= 0 && offset <= 65535);
        *(ip - 2) = offset & 0xFF;
        *(ip - 1) = offset >> 8;
        switch (inst) {
