...
#define VECTOR_DISASSEMBLE(op, text)                          \
    case ByteCodeInstruction::op: s.printf(text); break;        \
    case ByteCodeInstruction::op##2: s.printf(text "2"); break; \
    case ByteCodeInstruction::op##3: s.printf(text "3"); break; \
    case ByteCodeInstruction::op##4: s.printf(text "4"); break;
...
#define VECTOR_MATRIX_DISASSEMBLE(op, text)                   \
    ...
    case ByteCodeInstruction::op##3: s.printf(text "3"); break; \
    case ByteCodeInstruction::op##4: s.printf(text "4"); break; \
    case ByteCodeInstruction::op##N: s.printf(text "N %d", READ8()); break;

static const uint8_t* disassemble_instruction(const uint8_t* ip, SkString& s) {
    switch ((ByteCodeInstruction) READ16()) {
        VECTOR_MATRIX_DISASSEMBLE(kAddF, "addf")
        VECTOR_DISASSEMBLE(kAddI, "addi")
        case ByteCodeInstruction::kAndB: s.printf("andb"); break;
        case ByteCodeInstruction::kBranch: s.printf("branch %d", READ16()); break;
        case ByteCodeInstruction::kCall: s.printf("call %d", READ8()); break;
        case ByteCodeInstruction::kCallExternal: {
            int argumentCount = READ8();
            int returnCount = READ8();
            int externalValue = READ8();
            s.printf("callexternal %d, %d, %d", argumentCount, returnCount, externalValue);
            break;
        }
        case ByteCodeInstruction::kClampIndex: s.printf("clampindex %d", READ8()); break;
        ...
        case ByteCodeInstruction::kCross: s.printf("cross"); break;
        ...
        case ByteCodeInstruction::kLoad: s.printf("load %d", READ8()); break;
        case ByteCodeInstruction::kLoad2: s.printf("load2 %d", READ8()); break;
        case ByteCodeInstruction::kLoad3: s.printf("load3 %d", READ8()); break;
        case ByteCodeInstruction::kLoad4: s.printf("load4 %d", READ8()); break;
        case ByteCodeInstruction::kLoadGlobal: s.printf("loadglobal %d", READ8()); break;
        case ByteCodeInstruction::kLoadGlobal2: s.printf("loadglobal2 %d", READ8()); break;
        case ByteCodeInstruction::kLoadGlobal3: s.printf("loadglobal3 %d", READ8()); break;
        case ByteCodeInstruction::kLoadGlobal4: s.printf("loadglobal4 %d", READ8()); break;
        case ByteCodeInstruction::kLoadSwizzle: {
            int target = READ8();
            int count = READ8();
            s.printf("loadswizzle %d %d", target, count);
        ...
        case ByteCodeInstruction::kLoadSwizzleGlobal: {
            int target = READ8();
            int count = READ8();
            s.printf("loadswizzleglobal %d %d", target, count);
            for (int i = 0; i < count; ++i) {
                s.appendf(", %d", READ8());
            }
            break;
        }
        case ByteCodeInstruction::kLoadExtended: s.printf("loadextended %d", READ8()); break;
        case ByteCodeInstruction::kLoadExtendedGlobal: s.printf("loadextendedglobal %d", READ8());
            break;
        case ByteCodeInstruction::kMatrixToMatrix: {
            ...
            int dstCols = READ8();
            int dstRows = READ8();
            s.printf("matrixtomatrix %dx%d %dx%d", srcCols, srcRows, dstCols, dstRows);
            break;
        }
        case ByteCodeInstruction::kMatrixMultiply: {
            ...
            int lRows = READ8();
            int rCols = READ8();
            s.printf("matrixmultiply %dx%d %dx%d", lCols, lRows, rCols, lCols);
            break;
        }
        ...
        case ByteCodeInstruction::kNotB: s.printf("notb"); break;
        case ByteCodeInstruction::kOrB: s.printf("orb"); break;
        VECTOR_MATRIX_DISASSEMBLE(kPop, "pop")
        case ByteCodeInstruction::kPushImmediate: {
            uint32_t v = READ32();
            union { uint32_t u; float f; } pun = { v };
            s.printf("pushimmediate %s", (to_string(v) + "(" + to_string(pun.f) + ")").c_str());
            break;
        }
        case ByteCodeInstruction::kReadExternal: s.printf("readexternal %d", READ8()); break;
        case ByteCodeInstruction::kReadExternal2: s.printf("readexternal2 %d", READ8()); break;
        case ByteCodeInstruction::kReadExternal3: s.printf("readexternal3 %d", READ8()); break;
        case ByteCodeInstruction::kReadExternal4: s.printf("readexternal4 %d", READ8()); break;
        ...
        case ByteCodeInstruction::kReserve: s.printf("reserve %d", READ8()); break;
        case ByteCodeInstruction::kReturn: s.printf("return %d", READ8()); break;
        case ByteCodeInstruction::kScalarToMatrix: {
            int cols = READ8();
            int rows = READ8();
            s.printf("scalartomatrix %dx%d", cols, rows);
            break;
        }
        VECTOR_DISASSEMBLE(kSin, "sin")
        VECTOR_DISASSEMBLE(kSqrt, "sqrt")
        case ByteCodeInstruction::kStore: s.printf("store %d", READ8()); break;
        case ByteCodeInstruction::kStore2: s.printf("store2 %d", READ8()); break;
        case ByteCodeInstruction::kStore3: s.printf("store3 %d", READ8()); break;
        case ByteCodeInstruction::kStore4: s.printf("store4 %d", READ8()); break;
        case ByteCodeInstruction::kStoreGlobal: s.printf("storeglobal %d", READ8()); break;
        case ByteCodeInstruction::kStoreGlobal2: s.printf("storeglobal2 %d", READ8()); break;
        case ByteCodeInstruction::kStoreGlobal3: s.printf("storeglobal3 %d", READ8()); break;
        case ByteCodeInstruction::kStoreGlobal4: s.printf("storeglobal4 %d", READ8()); break;
        case ByteCodeInstruction::kStoreSwizzle: {
            int target = READ8();
            int count = READ8();
            s.printf("storeswizzle %d %d", target, count);
            for (int i = 0; i < count; ++i) {
                s.appendf(", %d", READ8());
            }
            break;
        ...
        case ByteCodeInstruction::kStoreSwizzleGlobal: {
            int target = READ8();
            int count = READ8();
            s.printf("storeswizzleglobal %d %d", target, count);
            for (int i = 0; i < count; ++i) {
                s.appendf(", %d", READ8());
            }
            break;
        ...
        case ByteCodeInstruction::kStoreSwizzleIndirect: {
            int count = READ8();
            s.printf("storeswizzleindirect %d", count);
            for (int i = 0; i < count; ++i) {
                s.appendf(", %d", READ8());
            }
            break;
        ...
        case ByteCodeInstruction::kStoreSwizzleIndirectGlobal: {
            int count = READ8();
            s.printf("storeswizzleindirectglobal %d", count);
            for (int i = 0; i < count; ++i) {
                s.appendf(", %d", READ8());
            }
            break;
        }
        case ByteCodeInstruction::kStoreExtended: s.printf("storeextended %d", READ8()); break;
        case ByteCodeInstruction::kStoreExtendedGlobal: s.printf("storeextendedglobal %d", READ8());
            break;
        VECTOR_MATRIX_DISASSEMBLE(kSubtractF, "subtractf")
        VECTOR_DISASSEMBLE(kSubtractI, "subtracti")
        case ByteCodeInstruction::kSwizzle: {
            s.printf("swizzle %d, ", READ8());
            int count = READ8();
            s.appendf("%d", count);
            for (int i = 0; i < count; ++i) {
                s.appendf(", %d", READ8());
        ...
        case ByteCodeInstruction::kWriteExternal: s.printf("writeexternal %d", READ8()); break;
        case ByteCodeInstruction::kWriteExternal2: s.printf("writeexternal2 %d", READ8()); break;
        case ByteCodeInstruction::kWriteExternal3: s.printf("writeexternal3 %d", READ8()); break;
        case ByteCodeInstruction::kWriteExternal4: s.printf("writeexternal4 %d", READ8()); break;
        case ByteCodeInstruction::kXorB: s.printf("xorb"); break;
        case ByteCodeInstruction::kMaskPush: s.printf("maskpush"); break;
        case ByteCodeInstruction::kMaskPop: s.printf("maskpop"); break;
        case ByteCodeInstruction::kMaskNegate: s.printf("masknegate"); break;
        case ByteCodeInstruction::kMaskBlend: s.printf("maskblend %d", READ8()); break;
        case ByteCodeInstruction::kBranchIfAllFalse:
            s.printf("branchifallfalse %d", READ16());
            break;
        case ByteCodeInstruction::kLoopBegin: s.printf("loopbegin"); break;
        case ByteCodeInstruction::kLoopNext: s.printf("loopnext"); break;
        case ByteCodeInstruction::kLoopMask: s.printf("loopmask"); break;
        case ByteCodeInstruction::kLoopEnd: s.printf("loopend"); break;
        case ByteCodeInstruction::kLoopContinue: s.printf("loopcontinue"); break;
        case ByteCodeInstruction::kLoopBreak: s.printf("loopbreak"); break;
        default: s.printf("unknown(%d)\n", *(ip - 1)); SkASSERT(false);
    }
    return ip;
}
...
void ByteCodeFunction::disassemble(std::vector<SkString>& lines, std::map<int, int>& lookup) const {
    lines.clear();
    lookup.clear();
    const uint8_t* ip = fCode.data();
    while (ip < fCode.data() + fCode.size()) {
