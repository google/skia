...
struct SampleMatrix {
    enum class Kind {
        // No sample(child, matrix) call affects the fragment processor.
        kNone,
        // The child is sampled with a matrix whose value is fixed and based only on constants or
        // uniforms, and thus the transform can be hoisted to the vertex shader.
        kConstantOrUniform,
        // The child is sampled with a non-constant/uniform value, or sampled multiple times, and
        // thus the transform cannot be hoisted to the vertex shader.
        kVariable,
        // The child is sampled with a constant or uniform value, *and* also inherits a variable
