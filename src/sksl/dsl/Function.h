...
public:
    ...
    void define(Stmt... stmts) {
        std::unique_ptr<SkSL::Statement> block = Block(std::move(stmts)...);
        this->define(std::move(block->template as<SkSL::Block>().children()));
    }

    void define(SkSL::StatementArray stmts) {
        SkSL::StatementArray body = DSLWriter::Instance().pendingDeclarations();
        body.move_back_n(stmts.count(), stmts.data());
...
protected:
    ...
    void declare() override {
        ...
        SkSL::SymbolTable& symbols = *DSLWriter::Instance().symbolTable();
        // FIXME: need to figure out why this-> is required for these field accesses
