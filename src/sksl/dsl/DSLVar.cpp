...
DSLVar::DSLVar(const char* name)
    ...
    if (!strcmp(name, "sk_SampleCoord")) {
        ...
        // if we attempt to look it up in the symbol table we'll get null. Since all we really need
        // is a correctly-named variable with the right type, we just create a placeholder for it.
