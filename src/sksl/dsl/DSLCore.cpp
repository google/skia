...
public:
    ...
    static DSLPossibleStatement Declare(DSLVar& var, DSLExpression initialValue) {
        if (!var.fDeclaration) {
            DSLWriter::Compiler().error(/*offset=*/-1,
                                        "Declare failed (was the variable already declared?)");
    ...
    static DSLPossibleExpression Swizzle(DSLExpression base, SkSL::SwizzleComponent::Type a) {
        char mask[] = { swizzle_component(a), 0 };
        return DSLWriter::IRGenerator().convertSwizzle(base.release(), mask);
    }

    static DSLPossibleExpression Swizzle(DSLExpression base,
...
DSLExpression Faceforward(DSLExpression n, DSLExpression i, DSLExpression nref, PositionInfo pos) {
    return DSLExpression(DSLCore::Call("faceforward", std::move(n), std::move(i), std::move(nref)), pos);
}
...
DSLExpression Smoothstep(DSLExpression edge1, DSLExpression edge2, DSLExpression x, PositionInfo pos) {
    return DSLExpression(DSLCore::Call("smoothstep", std::move(edge1), std::move(edge2), std::move(x)), pos);
}
...
DSLExpression Swizzle(DSLExpression base, SkSL::SwizzleComponent::Type a,
                      PositionInfo pos) {
    return DSLExpression(DSLCore::Swizzle(std::move(base), a), pos);
}
...
DSLExpression Swizzle(DSLExpression base,
                      ...
                      SkSL::SwizzleComponent::Type b,
                      PositionInfo pos) {
    return DSLExpression(DSLCore::Swizzle(std::move(base), a, b), pos);
}
...
DSLExpression Swizzle(DSLExpression base,
                      ...
                      SkSL::SwizzleComponent::Type c,
                      PositionInfo pos) {
    return DSLExpression(DSLCore::Swizzle(std::move(base), a, b, c), pos);
}
...
DSLExpression Swizzle(DSLExpression base,
                      ...
                      SkSL::SwizzleComponent::Type d,
                      PositionInfo pos) {
    return DSLExpression(DSLCore::Swizzle(std::move(base), a, b, c, d), pos);
}
