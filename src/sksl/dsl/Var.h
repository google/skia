...
public:
    ...
    Var(const char* name, int index)
        : fName(name)
        , fIndex(index) {}

    Var(const char* name)
        : Var(name, DSLWriter::Instance().declare(std::make_unique<SkSL::Variable>(
                                                 /*offset=*/-1,
    ...
    const SkSL::Variable* variable() const {
        return DSLWriter::Instance().var(fIndex);
    }
