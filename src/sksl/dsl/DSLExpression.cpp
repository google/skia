...
DSLExpression::~DSLExpression() {
    SkDEBUGCODE(SkSL::Expression* expr = fExpression.get();)
    SkASSERTF(!expr, "Expression destroyed without being incorporated into output tree");
}
...
std::unique_ptr<SkSL::Expression> DSLExpression::coerceAndRelease(const SkSL::Type& type) {
    ...
    SkASSERTF(!dsl.compiler().errorCount(), "Unexpected SkSL DSL error: %s",
              dsl.compiler().errorText().c_str());
...
DSLExpression Swizzle(DSLExpression base, SwizzleComponent a) {
    return DSLExpression(check(DSLWriter::Instance().irGenerator().convertSwizzle(base.release(),
                                                                         swizzle_component(a))));
}
