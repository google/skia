...
const SkSL::Type& Array(const SkSL::Type& base, int count) {
    SkSL::String name = base.name() + "[" + SkSL::to_string(count) + "]";
    return *DSLWriter::Instance().symbolTable()->takeOwnershipOfSymbol(
                     std::make_unique<SkSL::Type>(name, SkSL::Type::TypeKind::kArray, base, count));
}
