...
DSLExpression dsl_construct(const SkSL::Type& type, std::vector<DSLExpression> rawArgs) {
    ...
    return DSLExpression(dsl.irGenerator().call(/*offset=*/-1,
...
DSLExpression T(DSLExpression a) {                                                                 \
    return construct1(*DSLWriter::Instance().context().f ## T ## _Type, std::move(a));             \
}                                                                                                  \
DSLExpression T ## 2(DSLExpression a) {                                                            \
    return construct1(*DSLWriter::Instance().context().f ## T ## 2_Type, std::move(a));            \
}                                                                                                  \
DSLExpression T ## 2(DSLExpression a, DSLExpression b) {                                           \
    return construct2(*DSLWriter::Instance().context().f ## T ## 2_Type, std::move(a),             \
                      std::move(b));                                                               \
}                                                                                                  \
DSLExpression T ## 3(DSLExpression a) {                                                            \
    return construct1(*DSLWriter::Instance().context().f ## T ## 3_Type, std::move(a));            \
}                                                                                                  \
DSLExpression T ## 3(DSLExpression a, DSLExpression b) {                                           \
    return construct2(*DSLWriter::Instance().context().f ## T ## 3_Type, std::move(a),             \
                      std::move(b));                                                               \
}                                                                                                  \
DSLExpression T ## 3(DSLExpression a, DSLExpression b, DSLExpression c) {                          \
    return construct3(*DSLWriter::Instance().context().f ## T ## 3_Type, std::move(a),             \
                      std::move(b), std::move(c));                                                 \
}                                                                                                  \
DSLExpression T ## 4(DSLExpression a) {                                                            \
    return construct1(*DSLWriter::Instance().context().f ## T ## 4_Type, std::move(a));            \
}                                                                                                  \
DSLExpression T ## 4(DSLExpression a, DSLExpression b) {                                           \
    return construct2(*DSLWriter::Instance().context().f ## T ## 4_Type, std::move(a),             \
                      std::move(b));                                                               \
}                                                                                                  \
DSLExpression T ## 4(DSLExpression a, DSLExpression b, DSLExpression c) {                          \
    return construct3(*DSLWriter::Instance().context().f ## T ## 4_Type, std::move(a),             \
                      std::move(b), std::move(c));                                                 \
}                                                                                                  \
DSLExpression T ## 4(DSLExpression a, DSLExpression b, DSLExpression c, DSLExpression d) {         \
    return construct4(*DSLWriter::Instance().context().f ## T ## 4_Type, std::move(a),             \
...
DSLType Array(const DSLType& base, int count) {
    ...
    return DSLType(DSLWriter::Instance().symbolTable()->takeOwnershipOfSymbol(
