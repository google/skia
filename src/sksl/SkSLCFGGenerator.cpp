...
bool BasicBlock::tryRemoveExpression(std::vector<BasicBlock::Node>::iterator* iter) {
    ...
    switch (expr->kind()) {
        case Expression::Kind::kBinary: {
            if (expr->getBinaryData().fOperator == Token::Kind::TK_EQ) {
                if (!this->tryRemoveLValueBefore(
                                              iter,
                                              &expr->expressionChild(IRNode::Index::kBinaryLeft))) {
                    return false;
                }
            } else if (!this->tryRemoveExpressionBefore(
                                              iter,
                                              &expr->expressionChild(IRNode::Index::kBinaryLeft))) {
                return false;
            }
            if (!this->tryRemoveExpressionBefore(
                                             iter,
                                             &expr->expressionChild(IRNode::Index::kBinaryRight))) {
                return false;
            }
    ...
    switch (expr.kind()) {
        case Expression::Kind::kBinary: {
            if (!this->tryInsertExpression(iter,
                                           &expr.expressionPointer(IRNode::Index::kBinaryRight))) {
                return false;
            }
            ++(*iter);
            if (!this->tryInsertExpression(iter,
                                           &expr.expressionPointer(IRNode::Index::kBinaryLeft))) {
                return false;
            }
            ++(*iter);
            BasicBlock::Node node = { BasicBlock::Node::kExpression_Kind, true, exprPtr, nullptr };
...
void CFGGenerator::addExpression(CFG& cfg, std::unique_ptr<Expression>* exprPtr,
                                 bool constantPropagate) {
    SkASSERT(exprPtr);
    Expression& expr = **exprPtr;
    switch (expr.kind()) {
        case Expression::Kind::kBinary: {
            switch (expr.getBinaryData().fOperator) {
                ...
                case Token::Kind::TK_LOGICALOR: {
                    ...
                    this->addExpression(cfg, &expr.expressionPointer(IRNode::Index::kBinaryLeft),
                                        constantPropagate);
                    BlockId start = cfg.fCurrent;
                    cfg.newBlock();
                    this->addExpression(cfg, &expr.expressionPointer(IRNode::Index::kBinaryRight),
                                        constantPropagate);
                    cfg.newBlock();
                ...
                case Token::Kind::TK_EQ: {
                    this->addExpression(cfg, &expr.expressionPointer(IRNode::Index::kBinaryRight),
                                        constantPropagate);
                    this->addLValue(cfg, &expr.expressionPointer(IRNode::Index::kBinaryLeft));
                    cfg.fBlocks[cfg.fCurrent].fNodes.push_back({
                        BasicBlock::Node::kExpression_Kind,
                        constantPropagate,
                        exprPtr,
                ...
                default:
                    this->addExpression(cfg, &expr.expressionPointer(IRNode::Index::kBinaryLeft),
                                        !Compiler::IsAssignment(expr.getBinaryData().fOperator));
                    this->addExpression(cfg, &expr.expressionPointer(IRNode::Index::kBinaryRight),
                                        constantPropagate);
                    cfg.fBlocks[cfg.fCurrent].fNodes.push_back({
                        BasicBlock::Node::kExpression_Kind,
        ...
        case Expression::Kind::kConstructor: {
            ...
            cfg.fBlocks[cfg.fCurrent].fNodes.push_back({ BasicBlock::Node::kExpression_Kind,
                                                         constantPropagate, exprPtr, nullptr });
            break;
        }
        case Expression::Kind::kExternalFunctionCall: {
            ExternalFunctionCall& c = expr.as<ExternalFunctionCall>();
            for (auto& arg : c.fArguments) {
                this->addExpression(cfg, &arg, constantPropagate);
            }
            cfg.fBlocks[cfg.fCurrent].fNodes.push_back({ BasicBlock::Node::kExpression_Kind,
        ...
        case Expression::Kind::kFunctionCall: {
            FunctionCall& c = expr.as<FunctionCall>();
            for (auto& arg : c.fArguments) {
                this->addExpression(cfg, &arg, constantPropagate);
            }
            cfg.fBlocks[cfg.fCurrent].fNodes.push_back({ BasicBlock::Node::kExpression_Kind,
        ...
        case Expression::Kind::kFieldAccess: {
            ...
            this->addExpression(cfg, &fa.fBase, constantPropagate);
            cfg.fBlocks[cfg.fCurrent].fNodes.push_back({ BasicBlock::Node::kExpression_Kind,
                                                         constantPropagate, exprPtr, nullptr });
            break;
        }
        case Expression::Kind::kIndex: {
            IndexExpression& ie = expr.as<IndexExpression>();
            this->addExpression(cfg, &ie.fBase, constantPropagate);
            this->addExpression(cfg, &ie.fIndex, constantPropagate);
            cfg.fBlocks[cfg.fCurrent].fNodes.push_back({ BasicBlock::Node::kExpression_Kind,
                                                         constantPropagate, exprPtr, nullptr });
            break;
        }
        case Expression::Kind::kPrefix: {
            PrefixExpression& p = expr.as<PrefixExpression>();
        ...
        case Expression::Kind::kPostfix: {
            PostfixExpression& p = expr.as<PostfixExpression>();
            this->addExpression(cfg, &p.fOperand, false);
            cfg.fBlocks[cfg.fCurrent].fNodes.push_back({ BasicBlock::Node::kExpression_Kind,
                                                         constantPropagate, exprPtr, nullptr });
            break;
        }
        case Expression::Kind::kSwizzle: {
            Swizzle& s = expr.as<Swizzle>();
            this->addExpression(cfg, &s.fBase, constantPropagate);
            cfg.fBlocks[cfg.fCurrent].fNodes.push_back({ BasicBlock::Node::kExpression_Kind,
                                                         constantPropagate, exprPtr, nullptr });
            break;
        }
