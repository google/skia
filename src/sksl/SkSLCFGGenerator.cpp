    ...
    for (size_t i = 0; i < fBlocks.size(); i++) {
        ...
        for (auto iter = fBlocks[i].fBefore.begin(); iter != fBlocks[i].fBefore.end(); iter++) {
            printf("%s%s = %s", separator, iter->first.node().description().c_str(),
                   iter->second.id() ? iter->second.node().description().c_str() : "<undefined>");
            separator = ", ";
        }
        printf("\nEntrances: ");
        separator = "";
...
bool BasicBlock::tryRemoveExpressionBefore(std::vector<BasicBlock::Node>::iterator* iter,
                                           IRNode::ID id) {
    Expression& e = (Expression&) id.expression();
...
bool BasicBlock::tryRemoveLValueBefore(std::vector<BasicBlock::Node>::iterator* iter,
                                       IRNode::ID lvalueID) {
    Expression& lvalue = lvalueID.expression();
    switch (lvalue.fKind) {
...
bool BasicBlock::tryInsertExpression(std::vector<BasicBlock::Node>::iterator* iter,
                                     IRNodeIDPtr exprID) {
    Expression& expr = exprID.expression();
    switch (expr.fKind) {
        case Expression::kBinary_Kind: {
            BinaryExpression& b = (BinaryExpression&) expr;
...
void CFGGenerator::addLValue(CFG& cfg, IRNodeIDPtr eID) {
    ...
    switch (e.fKind) {
        ...
        case Expression::kSwizzle_Kind:
            ...
            break;
        case Expression::kVariableReference_Kind:
            ((Variable&) ((VariableReference&) e).fVariable.node()).fWriteCount++;
            break;
    ...
    switch (s.fKind) {
        ...
        case Statement::kVarDeclarations_Kind: {
            ...
            for (size_t i = 0; i < decl.fVars.size(); ++i) {
                Statement& stmt = decl.fVars[i].statement();
        ...
        case Statement::kSwitch_Kind: {
            ...
            if (0 == ss.fCases.size() ||
                ((SwitchCase&) ss.fCases[ss.fCases.size() - 1].node()).fValue) {
