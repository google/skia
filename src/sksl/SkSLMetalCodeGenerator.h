...
protected:
    ...
    bool requiresInputs(const FunctionDeclaration& f);

    bool requiresInputs(const Expression& e);

    bool requiresInputs(const Statement& e);
    ...
    // maps whether functions require access to the _in struct
    std::unordered_map<const FunctionDeclaration*, bool> fNeedsInputs;
