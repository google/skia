...
void GLSLCodeGenerator::writeDeterminantHack(const Expression& mat) {
    String name;
    if (mat.type() == *fContext.fFloat2x2_Type || mat.type() == *fContext.fHalf2x2_Type) {
        name = "_determinant2";
    ...
    else if (mat.type() == *fContext.fFloat3x3_Type || mat.type() == *fContext.fHalf3x3_Type) {
        name = "_determinant3";
        if (fWrittenIntrinsics.find(name) == fWrittenIntrinsics.end()) {
            fWrittenIntrinsics.insert(name);
            fExtraFunctions.writeText((
    ...
    else if (mat.type() == *fContext.fFloat4x4_Type || mat.type() == *fContext.fHalf4x4_Type) {
        name = "_determinant3";
        if (fWrittenIntrinsics.find(name) == fWrittenIntrinsics.end()) {
            fWrittenIntrinsics.insert(name);
            fExtraFunctions.writeText((
...
void GLSLCodeGenerator::writeTransposeHack(const Expression& mat) {
    String name = "transpose" + to_string(mat.type().columns()) + to_string(mat.type().rows());
    if (fWrittenIntrinsics.find(name) == fWrittenIntrinsics.end()) {
        fWrittenIntrinsics.insert(name);
        String type = this->getTypeName(mat.type());
        const Type& base = mat.type().componentType();
        ...
        for (int row = 0; row < mat.type().rows(); ++row) {
            for (int column = 0; column < mat.type().columns(); ++column) {
            ...
            case FunctionClass::kTexture: {
                ...
                switch (c.fArguments[0]->type().dimensions()) {
                    case SpvDim1D:
                        ...
                        if (c.fArguments[1]->type() == *fContext.fFloat_Type) {
                            proj = false;
                        } else {
                    ...
                    case SpvDim2D:
                        dim = "2D";
                        if (c.fArguments[0]->type() != *fContext.fSamplerExternalOES_Type) {
                            isTextureFunctionWithBias = true;
                        }
                    ...
                    case SpvDim3D:
                        ...
                        if (c.fArguments[1]->type() == *fContext.fFloat3_Type) {
                            ...
                            SkASSERT(c.fArguments[1]->type() == *fContext.fFloat4_Type);
                            proj = true;
                        }
                        break;
                    case SpvDimCube:
...
void GLSLCodeGenerator::writeFieldAccess(const FieldAccess& f) {
    ...
    switch (f.fBase->type().fields()[f.fFieldIndex].fModifiers.fLayout.fBuiltin) {
        case SK_CLIPDISTANCE_BUILTIN:
            this->write("gl_ClipDistance");
            break;
        default:
            StringFragment name = f.fBase->type().fields()[f.fFieldIndex].fName;
            if (name == "sk_Position") {
                this->write("gl_Position");
            } else if (name == "sk_PointSize") {
                this->write("gl_PointSize");
            } else {
                this->write(f.fBase->type().fields()[f.fFieldIndex].fName);
            }
    }
}
...
void GLSLCodeGenerator::writeIntLiteral(const IntLiteral& i) {
    ...
    } else if (i.type() == *fContext.fUShort_Type) {
        this->write(to_string(i.fValue & 0xffff) + "u");
    } else if (i.type() == *fContext.fUByte_Type) {
...
void GLSLCodeGenerator::writeForStatement(const ForStatement& f) {
    ...
    if (f.fTest) {
        if (fProgram.fSettings.fCaps->addAndTrueToLoopCondition()) {
            std::unique_ptr<Expression> and_true = std::make_unique<BinaryExpression>(
                                                             -1,
                                                             f.fTest->clone(),
