...
void GLSLCodeGenerator::writeFunctionCall(const FunctionCall& c) {
    ...
    if (found != fFunctionClasses->end()) {
        switch (found->second) {
            case FunctionClass::kAbs: {
                ...
                if (c.fArguments[0].expression().fType != fContext.fInt_Type) {
                    break;
                }
            ...
            case FunctionClass::kAtan:
                if (fProgram.fSettings.fCaps->mustForceNegatedAtanParamToFloat() &&
                    c.fArguments.size() == 2 &&
                    (c.fArguments[1].expression()).fKind ==
                    Expression::kPrefix_Kind) {
                    const PrefixExpression& p =
                                              (PrefixExpression&) c.fArguments[1].node();
                    if (p.fOperator == Token::MINUS) {
                        this->write("atan(");
                        this->writeExpression(c.fArguments[0].expression(), kSequence_Precedence);
                        this->write(", -1.0 * ");
                        this->writeExpression(p.fOperand.expression(), kMultiplicative_Precedence);
                        this->write(")");
                        return;
                    }
                }
            ...
            case FunctionClass::kDeterminant:
                if (fProgram.fSettings.fCaps->generation() < k150_GrGLSLGeneration) {
                    SkASSERT(c.fArguments.size() == 1);
                    this->writeDeterminantHack(c.fArguments[0].expression());
                    return;
                }
                break;
            case FunctionClass::kFMA:
                if (!fProgram.fSettings.fCaps->builtinFMASupport()) {
                    ...
                    this->writeExpression(c.fArguments[0].expression(), kSequence_Precedence);
                    this->write(") * (");
                    this->writeExpression(c.fArguments[1].expression(), kSequence_Precedence);
                    this->write(") + (");
                    this->writeExpression(c.fArguments[2].expression(), kSequence_Precedence);
                    this->write("))");
                    return;
                }
                break;
            ...
            case FunctionClass::kInverse:
                if (fProgram.fSettings.fCaps->generation() < k140_GrGLSLGeneration) {
                    SkASSERT(c.fArguments.size() == 1);
                    this->writeInverseHack(c.fArguments[0].expression());
                    return;
                }
                break;
            case FunctionClass::kInverseSqrt:
                if (fProgram.fSettings.fCaps->generation() < k130_GrGLSLGeneration) {
                    SkASSERT(c.fArguments.size() == 1);
                    this->writeInverseSqrtHack(c.fArguments[0].expression());
                    return;
                }
                break;
            case FunctionClass::kMin:
                if (!fProgram.fSettings.fCaps->canUseMinAndAbsTogether()) {
                    ...
                    Expression& arg0 = c.fArguments[0].expression();
                    Expression& arg1 = c.fArguments[1].expression();
                    if (is_abs(arg0)) {
            ...
            case FunctionClass::kPow:
                ...
                this->write("exp2(");
                this->writeExpression(c.fArguments[1].expression(), kMultiplicative_Precedence);
                this->write(" * log2(");
                this->writeExpression(c.fArguments[0].expression(), kSequence_Precedence);
                this->write("))");
                return;
            ...
            case FunctionClass::kTranspose:
                if (fProgram.fSettings.fCaps->generation() < k130_GrGLSLGeneration) {
                    SkASSERT(c.fArguments.size() == 1);
                    this->writeTransposeHack(c.fArguments[0].expression());
                    return;
                }
                break;
        }
    ...
    for (auto argID : c.fArguments) {
        ...
        separator = ", ";
        this->writeExpression(argID.expression(), kSequence_Precedence);
    }
...
void GLSLCodeGenerator::writeConstructor(const Constructor& c, Precedence parentPrecedence) {
    if (c.fArguments.size() == 1 &&
        (this->getTypeName(c.fType.typeNode()) ==
         this->getTypeName((c.fArguments[0].expression()).fType.typeNode()) ||
        (c.fType.typeNode().kind() == Type::kScalar_Kind &&
         (c.fArguments[0].expression()).fType ==
        ...
        this->writeExpression(c.fArguments[0].expression(), parentPrecedence);
        return;
    }
...
void GLSLCodeGenerator::writeIndexExpression(const IndexExpression& expr) {
    this->writeExpression(expr.fBase.expression(), kPostfix_Precedence);
    this->write("[");
    this->writeExpression(expr.fIndex.expression(), kTopLevel_Precedence);
    this->write("]");
}

bool is_sk_position(const FieldAccess& f) {
    return "sk_Position" == f.fBase.expression().fType.typeNode().fields()[f.fFieldIndex].fName;
}

void GLSLCodeGenerator::writeFieldAccess(const FieldAccess& f) {
    if (f.fOwnerKind == FieldAccess::kDefault_OwnerKind) {
        this->writeExpression(f.fBase.expression(), kPostfix_Precedence);
        this->write(".");
    }
    Expression& base = f.fBase.expression();
...
void GLSLCodeGenerator::writeSwizzle(const Swizzle& swizzle) {
    ...
    this->writeExpression(swizzle.fBase.expression(), kPostfix_Precedence);
    this->write(".");
...
void GLSLCodeGenerator::writeBinaryExpression(const BinaryExpression& b,
    ...
    Expression& left = b.fLeft.expression();
    Expression& right = b.fRight.expression();
...
void GLSLCodeGenerator::writeShortCircuitWorkaroundExpression(const BinaryExpression& b,
    ...
    // a && b  =>   a ? b : false
    // a || b  =>   a ? true : b
    this->writeExpression(b.fLeft.expression(), kTernary_Precedence);
    this->write(" ? ");
    if (b.fOperator == Token::LOGICALAND) {
        this->writeExpression(b.fRight.expression(), kTernary_Precedence);
    } else {
...
void GLSLCodeGenerator::writeTernaryExpression(const TernaryExpression& t,
    ...
    this->writeExpression(t.fTest.expression(), kTernary_Precedence);
    this->write(" ? ");
    this->writeExpression(t.fIfTrue.expression(), kTernary_Precedence);
    this->write(" : ");
    this->writeExpression(t.fIfFalse.expression(), kTernary_Precedence);
    if (kTernary_Precedence >= parentPrecedence) {
        this->write(")");
    }
}
...
void GLSLCodeGenerator::writePrefixExpression(const PrefixExpression& p,
    ...
    this->writeExpression(p.fOperand.expression(), kPrefix_Precedence);
    if (kPrefix_Precedence >= parentPrecedence) {
        this->write(")");
    }
}
...
void GLSLCodeGenerator::writePostfixExpression(const PostfixExpression& p,
    ...
    this->writeExpression(p.fOperand.expression(), kPostfix_Precedence);
    this->write(Compiler::OperatorName(p.fOperator));
    if (kPostfix_Precedence >= parentPrecedence) {
        this->write(")");
    }
...
void GLSLCodeGenerator::writeInterfaceBlock(const InterfaceBlock& intf) {
    ...
    if (intf.fInstanceName.size()) {
        ...
        for (const auto& size : intf.fSizes) {
            this->write("[");
            if (size) {
                this->writeExpression(size.expression(), kTopLevel_Precedence);
            }
            this->write("]");
        }
    }
...
void GLSLCodeGenerator::writeVarDeclarations(const VarDeclarations& decls, bool global) {
    ...
    for (IRNode::ID stmt : decls.fVars) {
        ...
        if (decl.fValue) {
            this->write(" = ");
            this->writeVarInitializer(var, decl.fValue.expression());
        }
...
void GLSLCodeGenerator::writeStatement(const Statement& s) {
    switch (s.fKind) {
        ...
        case Statement::kExpression_Kind:
            this->writeExpression(((ExpressionStatement&) s).fExpression.expression(),
...
void GLSLCodeGenerator::writeIfStatement(const IfStatement& stmt) {
    this->write("if (");
    this->writeExpression(stmt.fTest.expression(), kTopLevel_Precedence);
    this->write(") ");
    this->writeStatement(stmt.fIfTrue.statement());
    if (stmt.fIfFalse) {
        this->write(" else ");
        this->writeStatement(stmt.fIfFalse.statement());
    }
}

void GLSLCodeGenerator::writeForStatement(const ForStatement& f) {
    this->write("for (");
    if (f.fInitializer && !f.fInitializer.statement().isEmpty()) {
    ...
    if (f.fTest) {
        if (fProgram.fSettings.fCaps->addAndTrueToLoopCondition()) {
            ...
            IRNode::ID and_true = fIRGenerator.createNode(new BinaryExpression(
                                                                              ...
                                                                              trueId,
                                                                              fContext.fBool_Type));
            this->writeExpression(and_true.expression(), kTopLevel_Precedence);
        } else {
            this->writeExpression(f.fTest.expression(), kTopLevel_Precedence);
        }
    }
    this->write("; ");
    if (f.fNextExpression) {
        this->writeExpression(f.fNextExpression.expression(),
                              kTopLevel_Precedence);
    }
    this->write(") ");
...
void GLSLCodeGenerator::writeWhileStatement(const WhileStatement& w) {
    this->write("while (");
    this->writeExpression(w.fTest.expression(), kTopLevel_Precedence);
    this->write(") ");
...
void GLSLCodeGenerator::writeDoStatement(const DoStatement& d) {
    if (!fProgram.fSettings.fCaps->rewriteDoWhileLoops()) {
        ...
        this->write(" while (");
        this->writeExpression(d.fTest.expression(), kTopLevel_Precedence);
        this->write(");");
        return;
    }
    ...
    fIndentation++;
    this->write("if (!");
    this->writeExpression(d.fTest.expression(), kPrefix_Precedence);
    this->writeLine(") {");
    fIndentation++;
...
void GLSLCodeGenerator::writeSwitchStatement(const SwitchStatement& s) {
    this->write("switch (");
    this->writeExpression(s.fValue.expression(), kTopLevel_Precedence);
    this->writeLine(") {");
    fIndentation++;
    for (const auto& cID : s.fCases) {
        ...
        if (c.fValue) {
            this->write("case ");
            this->writeExpression(c.fValue.expression(), kTopLevel_Precedence);
            this->writeLine(":");
        } else {
...
void GLSLCodeGenerator::writeReturnStatement(const ReturnStatement& r) {
    ...
    if (r.fExpression) {
        this->write(" ");
        this->writeExpression(r.fExpression.expression(), kTopLevel_Precedence);
    }
    this->write(";");
}
