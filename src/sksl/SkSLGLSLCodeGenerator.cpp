...
String GLSLCodeGenerator::getTypeName(const Type& type) {
    String result = this->getBaseName(type);

    for (const Type* t = &type; t->typeKind() == Type::TypeKind::kArray; t = &t->componentType()) {
        result += "["
                  + ((t->columns() != Type::kUnsizedArray) ? to_string(t->columns()) : "")
                  + "]";
    }
...
String GLSLCodeGenerator::getBaseName(const Type& type) const {
    switch (type.typeKind()) {
        ...
        case Type::TypeKind::kArray: {
            return this->getBaseName(type.componentType());
        }
