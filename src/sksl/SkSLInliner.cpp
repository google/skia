...
std::unique_ptr<Expression> Inliner::inlineExpression(int offset,
    ...
    auto expr = [&](const Expression* e) -> std::unique_ptr<Expression> {
        if (e) {
    ...
    switch (expression.kind()) {
        case Expression::Kind::kBinary: {
            const BinaryExpression& bin = expression.as<BinaryExpression>();
            return std::make_unique<BinaryExpression>(offset, expr(&bin.left()), bin.getOperator(),
                                                      expr(&bin.right()), &bin.type());
        }
        ...
        case Expression::Kind::kFieldAccess: {
            ...
            return std::make_unique<FieldAccess>(expr(f.fBase.get()), f.fFieldIndex, f.fOwnerKind);
        }
        ...
        case Expression::Kind::kIndex: {
            ...
            return std::make_unique<IndexExpression>(*fContext, expr(idx.fBase.get()),
                                                     expr(idx.fIndex.get()));
        }
        case Expression::Kind::kPrefix: {
            const PrefixExpression& p = expression.as<PrefixExpression>();
            return std::make_unique<PrefixExpression>(p.fOperator, expr(p.fOperand.get()));
        }
        case Expression::Kind::kPostfix: {
            const PostfixExpression& p = expression.as<PostfixExpression>();
            return std::make_unique<PostfixExpression>(expr(p.fOperand.get()), p.fOperator);
        }
        case Expression::Kind::kSetting:
            return expression.clone();
        case Expression::Kind::kSwizzle: {
            const Swizzle& s = expression.as<Swizzle>();
            return std::make_unique<Swizzle>(*fContext, expr(s.fBase.get()), s.fComponents);
        }
        case Expression::Kind::kTernary: {
            const TernaryExpression& t = expression.as<TernaryExpression>();
            return std::make_unique<TernaryExpression>(offset, expr(t.fTest.get()),
                                                       expr(t.fIfTrue.get()),
                                                       expr(t.fIfFalse.get()));
        }
...
Inliner::InlinedCall Inliner::inlineCall(std::unique_ptr<FunctionCall> call,
    ...
    for (int i = 0; i < (int) arguments.size(); ++i) {
        ...
        varMap[param] = makeInlineVar(String(param->fName), arguments[i]->type(), param->fModifiers,
                                      &arguments[i]);
    }
