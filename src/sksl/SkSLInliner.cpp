...
bool Inliner::isSafeToInline(const FunctionDefinition* functionDef) {
    ...
    for (const Variable* param : functionDef->declaration().parameters()) {
        if (param->modifiers().fFlags & Modifiers::kOut_Flag) {
            // Refuse to inline functions with `out` parameters.
            return false;
        }
        if (Analysis::StatementWritesToVariable(*functionDef->body(), *param)) {
            // Refuse to inline functions that write into variables.
            return false;
        }
    }
