            ...
            switch (stmt.kind()) {
                case Statement::Kind::kBlock: {
                    ...
                    return block.childCount() &&
                           this->visitStatement(block.child(block.childCount() - 1));
                }
    ...
    if (parentStmt && (parentStmt->is<IfStatement>() || parentStmt->is<ForStatement>() ||
        ...
        for (Block* nestedBlock = inlinedBody;; ) {
            ...
            if (nestedBlock->childCount() != 1) {
                ...
                inlinedBody->setIsScope(true);
                return;
            }
            if (!nestedBlock->child(0).is<Block>()) {
...
std::unique_ptr<Statement> Inliner::inlineStatement(int offset,
    ...
    auto blockStmts = [&](const Block& block) {
        ...
        for (int i = 0; i < block.childCount(); ++i) {
            result.push_back(stmt(block.childPointer(i)));
        }
        return result;
    };
...
Inliner::InlinedCall Inliner::inlineCall(FunctionCall* call,
    ...
    inlinedBody.addChild(std::make_unique<InlineMarker>(call->fFunction));
    ...
    auto makeInlineVar = [&](const String& baseName, const Type* type, Modifiers modifiers,
        ...
        inlinedBody.addChild(std::make_unique<VarDeclarationsStatement>(
    ...
    inlineBlock->reserve(body.childCount());
    for (const Statement& stmt : body) {
        inlineBlock->addChild(this->inlineStatement(
    ...
    if (hasEarlyReturn) {
        ...
        // emulate a goto.
        inlinedBody.addChild(std::make_unique<DoStatement>(
                /*offset=*/-1,
        ...
        inlinedBody.addChild(std::move(inlineBlock));
    }
    ...
    for (size_t i = 0; i < arguments.size(); ++i) {
        ...
        if (p->fModifiers.fFlags & Modifiers::kOut_Flag) {
            ...
            inlinedBody.addChild(std::make_unique<ExpressionStatement>(
...
bool Inliner::analyze(Program& program) {
        ...
        void visitStatement(std::unique_ptr<Statement>* stmt,
            ...
            switch ((*stmt)->kind()) {
                ...
                case Statement::Kind::kBlock: {
                    ...
                    for (int i = 0; i < block.childCount(); ++i) {
                        this->visitStatement(&block.childPointer(i));
                    }
                    break;
                }
                case Statement::Kind::kDo: {
    ...
    for (const InlineCandidate& candidate : analyzer.fInlineCandidates) {
        ...
        if (inlinedCall.fInlinedBody) {
            ...
            inlinedCall.fInlinedBody->addChild(std::move(*candidate.fEnclosingStmt));
