...
std::unique_ptr<Expression> Inliner::inlineExpression(int offset,
    ...
    auto argList = [&](const SkTArray<std::unique_ptr<Expression>>& originalArgs)
            -> SkSTArray<1, std::unique_ptr<Expression>> {
        SkSTArray<1, std::unique_ptr<Expression>> args;
...
Inliner::InlinedCall Inliner::inlineCall(FunctionCall* call,
    ...
    SkTArray<std::unique_ptr<Expression>>& arguments = call->arguments();
