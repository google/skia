...
bool Inliner::isSafeToInline(const FunctionCall& functionCall, int inlineThreshold) {
    ...
    if (functionCall.fFunction.fBuiltin) {
        // Can't inline built-in functions.
        return false;
    }
        ...
        void visitStatement(std::unique_ptr<Statement>* stmt) {
            ...
            switch ((*stmt)->fKind) {
                ...
                case Statement::kDo_Kind: {
                    ...
                    // The test-expr cannot be inlined, because it must be evaluated more than once.
                    break;
                }
                ...
                case Statement::kFor_Kind: {
                    ...
                    // The test- and increment-expressions cannot be inlined, because they must be
                ...
                case Statement::kSwitch_Kind: {
                    ...
                    for (std::unique_ptr<SwitchCase>& switchCase : switchStmt.fCases) {
                        ...
                        for (std::unique_ptr<Statement>& switchStmt : switchCase->fStatements) {
                            this->visitStatement(&switchStmt);
                        }
                    }
                    break;
                }
