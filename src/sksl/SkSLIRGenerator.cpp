...
// Assumes that left has fKind = kBoolLiteral_Kind
...
std::unique_ptr<Expression> IRGenerator::constantFold(const Expression& left,
    ...
    if (left.isConstant() && left.fKind == Expression::kBoolLiteral_Kind && !right.isConstant()) {
        return short_circuit_boolean(fContext, left, op, right);
    } else if (right.isConstant() && right.fKind == Expression::kBoolLiteral_Kind
               && !left.isConstant()) {
