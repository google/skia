    ...
    switch (statement.fKind) {
        ...
        default:
            ...
            if (fRTAdjust && Program::kGeometry_Kind == fKind) {
                SkASSERT(result.statement().fKind == Statement::kExpression_Kind);
                Expression& expr = ((ExpressionStatement&) result.node()).fExpression.expression();
    ...
    for (; iter != decls.end(); ++iter) {
        ...
        for (size_t i = 0; i < varData.fSizeCount; ++i, ++iter) {
            ...
            if (rawSize) {
                ...
                int64_t count;
                Expression& size = sizeID.expression();
...
IRNode::ID IRGenerator::convertIf(const ASTNode& n) {
    ...
    if (test.expression().fKind == Expression::kBoolLiteral_Kind) {
...
IRNode::ID IRGenerator::convertFor(const ASTNode& f) {
    ...
    if (*iter) {
        ...
        this->checkValid(next.expression());
    }
    ++iter;
...
IRNode::ID IRGenerator::convertSwitch(const ASTNode& s) {
    ...
    if (valueID.expression().fType != fContext.fUInt_Type &&
        valueID.expression().fType.typeNode().kind() != Type::kEnum_Kind) {
    ...
    for (; iter != s.end(); ++iter) {
        ...
        if (*childIter) {
            ...
            caseValue = this->coerce(caseValue, valueID.expression().fType);
            if (!caseValue) {
                return IRNode::ID();
            }
            ...
            int64_t v;
            this->getConstantInt(caseValue.expression(), &v);
...
IRNode::ID IRGenerator::convertInterfaceBlock(const ASTNode& intf) {
    ...
    for (size_t i = 0; i < id.fSizeCount; ++i) {
        ...
        if (size) {
            ...
            String name = type.typeNode().fName;
            int64_t count;
            if (converted.expression().fKind == Expression::kIntLiteral_Kind) {
    ...
    switch (value.fKind) {
        ...
        case Expression::kVariableReference_Kind: {
            ...
            if ((var.fModifiers.fFlags & Modifiers::kConst_Flag) &&
                var.fInitialValue) {
                this->getConstantInt(var.fInitialValue.expression(), out);
            }
            break;
        }
        default:
...
void IRGenerator::convertEnum(const ASTNode& e) {
    ...
    for (auto iter = e.begin(); iter != e.end(); ++iter) {
        ...
        if (child.begin() != child.end()) {
            ...
            this->getConstantInt(value.expression(), &currentValue);
        }
...
IRNode::ID IRGenerator::coerce(IRNode::ID exprID, IRNode::ID type) {
    ...
    if (expr.fType.typeNode() == type.typeNode()) {
        return exprID;
    }
...
IRNode::ID IRGenerator::shortCircuitBoolean(IRNode::ID leftID, Token::Kind op, IRNode::ID rightID) {
    Expression& left = leftID.expression();
    Expression& right = rightID.expression();
...
IRNode::ID IRGenerator::convertTernaryExpression(const ASTNode& node) {
    ...
    IRNode::ID falseType;
    IRNode::ID resultType;
    if (!determine_binary_type(fContext, Token::EQEQ, ifTrue.expression().fType,
                               ifFalse.expression().fType, &trueType, &falseType, &resultType,
                               true) || trueType != falseType) {
        fErrors.error(node.fOffset, "ternary operator result mismatch: '" +
                                    ifTrue.expression().fType.node().description() + "', '" +
                                    ifFalse.expression().fType.node().description() + "'");
        return IRNode::ID();
    }
...
IRNode::ID IRGenerator::callFunction(int offset, IRNode::ID functionID,
    ...
    if (!function.determineFinalTypes(arguments, &types, &returnType)) {
        ...
        for (size_t i = 0; i < arguments.size(); i++) {
            msg += separator;
            separator = ", ";
            msg += arguments[i].expression().fType.node().description();
        }
        msg += ")";
        fErrors.error(offset, msg);
        return IRNode::ID();
    ...
    for (size_t i = 0; i < arguments.size(); i++) {
        ...
        if (arguments[i] && (param.fModifiers.fFlags & Modifiers::kOut_Flag)) {
            this->setRefKind(arguments[i].expression(),
...
int IRGenerator::callCost(const FunctionDeclaration& function,
    ...
    for (size_t i = 0; i < arguments.size(); i++) {
        int cost = arguments[i].expression().coercionCost(types[i].typeNode());
        if (cost != INT_MAX) {
            total += cost;
        } else {
            return INT_MAX;
...
IRNode::ID IRGenerator::callExpression(int offset, IRNode::ID functionValue,
                                       std::vector<IRNode::ID> arguments) {
    switch (functionValue.expression().fKind) {
...
IRNode::ID IRGenerator::convertNumberConstructor(int offset, IRNode::ID typeID,
    ...
    if (args[0].expression().fType == fContext.fBool_Type) {
    ...
    if (!args[0].expression().fType.typeNode().isNumber()) {
        fErrors.error(offset, "invalid argument to '" + type.description() +
                              "' constructor (expected a number or bool, but found '" +
                              args[0].expression().fType.node().description() + "')");
        return IRNode::ID();
    }
...
IRNode::ID IRGenerator::convertCompoundConstructor(int offset, IRNode::ID type,
    ...
    if (type.typeNode().kind() == Type::kMatrix_Kind && args.size() == 1 &&
        args[0].expression().fType.typeNode().kind() == Type::kMatrix_Kind) {
    ...
    if (args.size() != 1 ||
        expected != component_count(args[0].expression().fType.typeNode()) ||
        type.typeNode().componentType().typeNode().isNumber() !=
        args[0].expression().fType.typeNode().componentType().typeNode().isNumber()) {
        for (size_t i = 0; i < args.size(); i++) {
            if (args[i].expression().fType.typeNode().kind() == Type::kVector_Kind) {
                if (type.typeNode().componentType().typeNode().isNumber() !=
                    args[i].expression().fType.typeNode().componentType().typeNode().isNumber()) {
                    fErrors.error(offset, "'" +
                                          args[i].expression().fType.node().description() +
                                          "' is not a valid parameter to '" +
                                          type.node().description() +
                                          "' constructor");
                    return IRNode::ID();
                }
                actual += args[i].expression().fType.typeNode().columns();
            } else if (args[i].expression().fType.typeNode().kind() == Type::kScalar_Kind) {
                actual += 1;
                if (type.typeNode().kind() != Type::kScalar_Kind) {
                ...
                fErrors.error(offset, "'" + args[i].expression().fType.node().description() +
                                      "' is not a valid parameter to '" +
...
IRNode::ID IRGenerator::convertConstructor(int offset, IRNode::ID type,
    ...
    if (args.size() == 1 && args[0].expression().fType == type) {
...
IRNode::ID IRGenerator::convertPrefixExpression(const ASTNode& expression) {
    ...
    Expression& base = baseID.expression();
...
IRNode::ID IRGenerator::convertIndex(IRNode::ID baseID, const ASTNode& index) {
    ...
    if (converted.expression().fType != fContext.fUInt_Type) {
...
IRNode::ID IRGenerator::convertAppend(int offset, const std::vector<ASTNode>& args) {
    ...
    if (pipeline.expression().fType != fContext.fSkRasterPipeline_Type) {
    ...
    if ("premul" == name) {
        ...
        if (stageArgs.size() == 1 &&
            stageArgs[0].expression().fType.typeNode().fName != "float[20]") {
...
IRNode::ID IRGenerator::convertIndexExpression(const ASTNode& index) {
    ...
    } else if (base.expression().fKind == Expression::kTypeReference_Kind) {
...
IRNode::ID IRGenerator::convertFieldExpression(const ASTNode& fieldNode) {
    ...
    if (base.expression().fType == fContext.fSkCaps_Type) {
        return this->getCap(fieldNode.fOffset, field);
    }
    if (base.expression().fType == fContext.fSkArgs_Type) {
        return this->getArg(fieldNode.fOffset, field);
    }
    if (base.expression().fKind == Expression::kTypeReference_Kind) {
        return this->convertTypeField(base.node().fOffset, ((TypeReference&) base.node()).fValue,
                                      field);
    }
    if (base.expression().fKind == Expression::kExternalValue_Kind) {
        return this->convertField(base, field);
    }
    switch (base.expression().fType.typeNode().kind()) {
        case Type::kVector_Kind:
        ...
        default:
            fErrors.error(base.node().fOffset, "cannot swizzle value of type '" +
                                               base.expression().fType.node().description() +
                                               "'");
            return IRNode::ID();
    }
}
...
IRNode::ID IRGenerator::convertPostfixExpression(const ASTNode& expression) {
    ...
    if (!base.expression().fType.typeNode().isNumber()) {
        fErrors.error(expression.fOffset,
                      "'" + String(Compiler::OperatorName(expression.getToken().fKind)) +
                      "' cannot operate on '" + base.expression().fType.node().description() +
                      "'");
        return IRNode::ID();
    }
    this->setRefKind(base.expression(), kReadWrite_RefKind);
...
void IRGenerator::setRefKind(const Expression& expr, RefKind kind) {
    switch (expr.fKind) {
        ...
        case Expression::kFieldAccess_Kind:
            this->setRefKind(((FieldAccess&) expr).fBase.expression(), kind);
            break;
        case Expression::kSwizzle_Kind: {
            const Swizzle& swizzle = (Swizzle&) expr;
            this->checkSwizzleWrite(swizzle);
            this->setRefKind(swizzle.fBase.expression(), kind);
            break;
        }
        case Expression::kIndex_Kind:
            this->setRefKind(((IndexExpression&) expr).fBase.expression(), kind);
            break;
        case Expression::kTernary_Kind: {
            TernaryExpression& t = (TernaryExpression&) expr;
            this->setRefKind(t.fIfTrue.expression(), kind);
            this->setRefKind(t.fIfFalse.expression(), kind);
            break;
        }
        case Expression::kExternalValue_Kind: {
