...
void IRGenerator::convertEnum(const ASTNode& e) {
    ...
    fSymbolTable = std::make_shared<SymbolTable>(&fErrors);
...
std::unique_ptr<Expression> IRGenerator::convertTypeField(int offset, const Type& type,
    ...
    auto findEnum = [](const Type& t, std::vector<std::unique_ptr<ProgramElement>>& elements) {
        for (const auto& e : elements) {
            if (e->kind() == ProgramElement::Kind::kEnum && t.name() == e->as<Enum>().fTypeName) {
                return e.get();
            }
        }
        return static_cast<ProgramElement*>(nullptr);
    };
    const ProgramElement* enumElement = findEnum(type, *fProgramElements);
    if (fInherited && !enumElement) {
        enumElement = findEnum(type, *fInherited);
    }
