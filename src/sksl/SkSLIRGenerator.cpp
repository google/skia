...
std::unique_ptr<Expression> IRGenerator::coerce(std::unique_ptr<Expression> expr,
    ...
    if (type.kind() == Type::kScalar_Kind) {
        ...
        if (type == *fContext.fFloatLiteral_Type) {
            ctor = this->convertIdentifier(ASTNode(-1, ASTNode::Kind::kIdentifier, "float"));
        } else if (type == *fContext.fIntLiteral_Type) {
            ctor = this->convertIdentifier(ASTNode(-1, ASTNode::Kind::kIdentifier, "int"));
        } else {
            ctor = this->convertIdentifier(ASTNode(-1, ASTNode::Kind::kIdentifier, type.fName));
        }
        if (!ctor) {
