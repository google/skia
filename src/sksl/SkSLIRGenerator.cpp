...
std::unique_ptr<Block> IRGenerator::convertBlock(const ASTNode& block) {
    ...
    std::vector<std::unique_ptr<Statement>> statements;
    for (const auto& child : block.fChildren) {
...
std::unique_ptr<VarDeclarations> IRGenerator::convertVarDeclarations(const ASTNode& decls,
    ...
    const Modifiers& modifiers = decls.fChildren[0].getModifiers();
    const ASTNode& rawType = decls.fChildren[1];
    ...
    for (size_t i = 2; i < decls.fChildren.size(); ++i) {
        const ASTNode& varDecl = decls.fChildren[i];
        ...
        if (modifiers.fLayout.fLocation == 0 && modifiers.fLayout.fIndex == 0 &&
            ...
            varData.fName != "sk_FragColor") {
            fErrors.error(decls.fOffset,
                          "out location=0, index=0 is reserved for sk_FragColor");
        }
        const Type* type = baseType;
        std::vector<std::unique_ptr<Expression>> sizes;
        for (size_t j = 0; j < varData.fSizeCount; ++j) {
            const ASTNode& rawSize = varDecl.fChildren[j];
            if (rawSize) {
        ...
        if (varDecl.fChildren.size() > varData.fSizeCount) {
            value = this->convertExpression(varDecl.fChildren[varData.fSizeCount]);
            if (!value) {
                return nullptr;
            }
        ...
        if (storage == Variable::kGlobal_Storage && var->fName == "sk_FragColor" &&
            ...
            variables.emplace_back(new VarDeclaration(var.get(), std::move(sizes),
                                                      std::move(value)));
            fSymbolTable->add(var->fName, std::move(var));
        }
    }
...
std::unique_ptr<Statement> IRGenerator::convertIf(const ASTNode& n) {
    ...
    std::unique_ptr<Expression> test = this->coerce(this->convertExpression(n.fChildren[0]),
                                                    *fContext.fBool_Type);
    if (!test) {
        return nullptr;
    }
    std::unique_ptr<Statement> ifTrue = this->convertStatement(n.fChildren[1]);
    if (!ifTrue) {
        return nullptr;
    }
    std::unique_ptr<Statement> ifFalse;
    if (n.fChildren.size() > 2) {
        ifFalse = this->convertStatement(n.fChildren[2]);
        if (!ifFalse) {
            return nullptr;
        }
    }
...
std::unique_ptr<Statement> IRGenerator::convertFor(const ASTNode& f) {
    ...
    if (f.fChildren[0]) {
        initializer = this->convertStatement(f.fChildren[0]);
        if (!initializer) {
            return nullptr;
        }
    }
    ...
    if (f.fChildren[1]) {
        test = this->coerce(this->convertExpression(f.fChildren[1]), *fContext.fBool_Type);
        if (!test) {
            return nullptr;
        }
    }
    ...
    if (f.fChildren[2]) {
        next = this->convertExpression(f.fChildren[2]);
        if (!next) {
            return nullptr;
        }
        this->checkValid(*next);
    }
    std::unique_ptr<Statement> statement = this->convertStatement(f.fChildren[3]);
    if (!statement) {
        return nullptr;
    }
...
std::unique_ptr<Statement> IRGenerator::convertWhile(const ASTNode& w) {
    ...
    AutoLoopLevel level(this);
    std::unique_ptr<Expression> test = this->coerce(this->convertExpression(w.fChildren[0]),
                                                    *fContext.fBool_Type);
    if (!test) {
        return nullptr;
    }
    std::unique_ptr<Statement> statement = this->convertStatement(w.fChildren[1]);
    if (!statement) {
        return nullptr;
    }
...
std::unique_ptr<Statement> IRGenerator::convertDo(const ASTNode& d) {
    ...
    AutoLoopLevel level(this);
    std::unique_ptr<Statement> statement = this->convertStatement(d.fChildren[0]);
    if (!statement) {
        return nullptr;
    }
    std::unique_ptr<Expression> test = this->coerce(this->convertExpression(d.fChildren[1]),
                                                    *fContext.fBool_Type);
    if (!test) {
        return nullptr;
    }
...
std::unique_ptr<Statement> IRGenerator::convertSwitch(const ASTNode& s) {
    ...
    std::unique_ptr<Expression> value = this->convertExpression(s.fChildren[0]);
    if (!value) {
        return nullptr;
    }
    ...
    for (size_t i = 1; i < s.fChildren.size(); ++i) {
        const ASTNode& c = s.fChildren[i];
        SkASSERT(c.fKind == ASTNode::Kind::kSwitchCase);
        std::unique_ptr<Expression> caseValue;
        if (c.fChildren[0]) {
            caseValue = this->convertExpression(c.fChildren[0]);
            if (!caseValue) {
                return nullptr;
            }
        ...
        std::vector<std::unique_ptr<Statement>> statements;
        for (size_t j = 1; j < c.fChildren.size(); ++j) {
            std::unique_ptr<Statement> converted = this->convertStatement(c.fChildren[j]);
            if (!converted) {
                return nullptr;
            }
...
std::unique_ptr<Statement> IRGenerator::convertReturn(const ASTNode& r) {
    ...
    if (r.fChildren.size() > 0) {
        std::unique_ptr<Expression> result = this->convertExpression(r.fChildren[0]);
        if (!result) {
            return nullptr;
        }
...
std::unique_ptr<Block> IRGenerator::applyInvocationIDWorkaround(std::unique_ptr<Block> main) {
    ...
    std::unique_ptr<Expression> next(new PostfixExpression(
                ...
                Token::PLUSPLUS));
    ASTNode endPrimitiveID(-1, ASTNode::Kind::kIdentifier, "EndPrimitive");
...
void IRGenerator::convertFunction(const ASTNode& f) {
    SkASSERT(f.fKind == ASTNode::Kind::kFunction);
    ASTNode::FunctionData fd = f.getFunctionData();
    const Type* returnType = this->convertType(f.fChildren[0]);
    if (!returnType) {
        return;
    }
    std::vector<const Variable*> parameters;
    for (size_t i = 1; i < fd.fParameterCount + 1; ++i) {
        const ASTNode& param = f.fChildren[i];
        SkASSERT(param.fKind == ASTNode::Kind::kParameter);
        ASTNode::ParameterData pd = param.getParameterData();
        const Type* type = this->convertType(param.fChildren[0]);
        if (!type) {
            return;
        }
        for (int j = (int) pd.fSizeCount; j >= 1; j--) {
            int size = param.fChildren[j].getInt();
        ...
        Variable* var = new Variable(param.fOffset, param.getParameterData().fModifiers, name,
                                     *type, Variable::kParameter_Storage);
        fSymbolTable->takeOwnership(var);
        parameters.push_back(var);
    }

    if (f.getFunctionData().fName == "main") {
    ...
    if (f.fChildren.size() > fd.fParameterCount + 1) {
        // compile body
        SkASSERT(!fCurrentFunction);
        ...
        AutoSymbolTable table(this);
        if (f.getFunctionData().fName == "main" && fKind == Program::kPipelineStage_Kind) {
        ...
        std::unique_ptr<Block> body = this->convertBlock(f.fChildren.back());
...
std::unique_ptr<InterfaceBlock> IRGenerator::convertInterfaceBlock(const ASTNode& intf) {
    ...
    this->popSymbolTable();
    Type* type = new Type(intf.fOffset, id.fTypeName, fields);
    old->takeOwnership(type);
    std::vector<std::unique_ptr<Expression>> sizes;
    for (size_t i = 0; i < id.fSizeCount; ++i) {
        const ASTNode& size = intf.fChildren[id.fDeclarationCount + i];
        if (size) {
    ...
    Variable* var = new Variable(intf.fOffset, id.fModifiers,
...
void IRGenerator::convertEnum(const ASTNode& e) {
    ...
    Layout layout;
    ASTNode enumType(e.fOffset, ASTNode::Kind::kType, (ASTNode::TypeData) { e.getString(), false });
    ...
    for (const auto& child : e.fChildren) {
        SkASSERT(child.fKind == ASTNode::Kind::kEnumCase);
        std::unique_ptr<Expression> value;
        if (child.fChildren.size()) {
            value = this->convertExpression(child.fChildren[0]);
            if (!value) {
...
const Type* IRGenerator::convertType(const ASTNode& type) {
    ...
    if (result && result->fKind == Symbol::kType_Kind) {
        for (const auto& size : type.fChildren) {
            ...
            name += "]";
            result = new Type(name, Type::kArray_Kind, (const Type&) *result,
                              size ? size.getInt() : -1);
...
std::unique_ptr<Expression> IRGenerator::coerce(std::unique_ptr<Expression> expr,
    ...
    if (type.kind() == Type::kScalar_Kind) {
        ...
        ASTNode id(-1, ASTNode::Kind::kIdentifier, type.fName);
...
std::unique_ptr<Expression> IRGenerator::convertBinaryExpression(const ASTNode& expression) {
    ...
    std::unique_ptr<Expression> left = this->convertExpression(expression.fChildren[0]);
    if (!left) {
        return nullptr;
    }
    std::unique_ptr<Expression> right = this->convertExpression(expression.fChildren[1]);
    if (!right) {
        return nullptr;
    }
    const Type* leftType;
...
std::unique_ptr<Expression> IRGenerator::convertTernaryExpression(const ASTNode& node) {
    ...
    std::unique_ptr<Expression> test = this->coerce(this->convertExpression(node.fChildren[0]),
                                                    *fContext.fBool_Type);
    if (!test) {
        return nullptr;
    }
    std::unique_ptr<Expression> ifTrue = this->convertExpression(node.fChildren[1]);
    if (!ifTrue) {
        return nullptr;
    }
    std::unique_ptr<Expression> ifFalse = this->convertExpression(node.fChildren[2]);
    if (!ifFalse) {
        return nullptr;
    }
    const Type* trueType;
    ...
    if (!determine_binary_type(fContext, Token::EQEQ, ifTrue->fType, ifFalse->fType, &trueType,
        ...
        fErrors.error(node.fOffset, "ternary operator result mismatch: '" + ifTrue->fType.fName +
                                    "', '" + ifFalse->fType.fName + "'");
        return nullptr;
    }
...
std::unique_ptr<Expression> IRGenerator::convertPrefixExpression(const ASTNode& expression) {
    ...
    std::unique_ptr<Expression> base = this->convertExpression(expression.fChildren[0]);
    if (!base) {
        return nullptr;
    }
...
std::unique_ptr<Expression> IRGenerator::convertTypeField(int offset, const Type& type,
    ...
    for (const auto& e : *fProgramElements) {
        if (e->fKind == ProgramElement::kEnum_Kind && type.name() == ((Enum&) *e).fTypeName) {
            ...
            result = convertIdentifier(ASTNode(offset, ASTNode::Kind::kIdentifier, field));
            fSymbolTable = old;
        }
    }
    if (!result) {
...
std::unique_ptr<Expression> IRGenerator::convertIndexExpression(const ASTNode& index) {
    ...
    std::unique_ptr<Expression> base = this->convertExpression(index.fChildren[0]);
    if (!base) {
        return nullptr;
    }
    if (index.fChildren.size() > 1) {
        return this->convertIndex(std::move(base), index.fChildren[1]);
...
std::unique_ptr<Expression> IRGenerator::convertCallExpression(const ASTNode& callNode) {
    ...
    std::unique_ptr<Expression> base = this->convertExpression(callNode.fChildren[0]);
    if (!base) {
        return nullptr;
    }
    if (Expression::kFunctionReference_Kind == base->fKind &&
        ...
        arguments.insert(arguments.begin(), callNode.fChildren.begin(), callNode.fChildren.end());
        return convertAppend(callNode.fOffset, arguments);
    }
    std::vector<std::unique_ptr<Expression>> arguments;
    for (size_t i = 1; i < callNode.fChildren.size(); i++) {
        std::unique_ptr<Expression> converted =
                this->convertExpression(callNode.fChildren[i]);
        if (!converted) {
            return nullptr;
        }
...
std::unique_ptr<Expression> IRGenerator::convertFieldExpression(const ASTNode& fieldNode) {
    std::unique_ptr<Expression> base = this->convertExpression(fieldNode.fChildren[0]);
    if (!base) {
        return nullptr;
    }
...
void IRGenerator::convertProgram(Program::Kind kind,
    ...
    std::vector<ASTNode> parsed = parser.file();
