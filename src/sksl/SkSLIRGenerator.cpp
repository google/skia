...
std::unique_ptr<StructDefinition> IRGenerator::convertStructDefinition(const ASTNode& node) {
    ...
    std::shared_ptr<SymbolTable> outerSymbolScope = fSymbolTable;
    AutoSymbolTable table(this);

    const Type* innerType = this->convertType(node);
    if (!innerType) {
        return nullptr;
    }
    if (innerType->typeKind() != Type::TypeKind::kStruct) {
        fErrors.error(node.fOffset, "expected a struct here, found '" + innerType->name() + "'");
        return nullptr;
    }

    const Type* type = outerSymbolScope->takeOwnershipOfSymbol(
            std::make_unique<Type>(node.fOffset, innerType->name(), innerType->fields()));
    return std::make_unique<StructDefinition>(node.fOffset, *type, fSymbolTable);
}
