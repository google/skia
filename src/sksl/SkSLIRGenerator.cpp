...
void IRGenerator::cloneBuiltinVariables() {
    ...
    public:
        ...
        bool visitExpression(Expression& e) override {
            ...
            if (e.is<VariableReference>() && e.as<VariableReference>().fVariable->fBuiltin) {
                ...
                if (auto sharedDecls = fGenerator->fIntrinsics->findAndInclude(sharedVar->fName)) {
                    ...
                    fRemap.insert(std::make_pair(sharedVar, clonedVar));

                    // Add the VarDeclarations to this Program
                    fGenerator->fProgramElements->push_back(std::move(clonedDecls));
                }

                // SkASSERT(found), once all pre-includes are converted?
    ...
    if (!fIsBuiltinCode) {
        ...
        // The remapper adds variable declarations to fProgramElements (invalidating iterators),
        // so we can't use range-based for here.
        for (size_t i = 0; i < fProgramElements->size(); ++i) {
            remapper.visitProgramElement(*(*fProgramElements)[i]);
        }
    }
}
