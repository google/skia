...
std::unique_ptr<Expression> IRGenerator::inlineCall(
    ...
    int returnCount = return_count(body);
    if (returnCount == 0 || (returnCount == 1 &&
        body.fStatements.back()->fKind == Statement::kReturn_Kind)) {
        // No early returns, so we can just dump the code in directly
        fExtraStatements.push_back(this->inlineStatement(offset, &varMap, resultVar, false,
                                                       *((Block&) *function.fBody).fStatements[0]));
    } else {
        // Function has an early return. Since we output to backends that don't have a goto
        // statement, we fake it by wrapping the function in a do { } while (false); and then use
        // break statements to jump to the end in order to emulate early returns.
        std::vector<std::unique_ptr<Statement>> inlined;
        for (const auto& s : body.fStatements) {
            inlined.push_back(this->inlineStatement(offset, &varMap, resultVar, true, *s));
        }
