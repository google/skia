...
std::unique_ptr<Statement> IRGenerator::getNormalizeSkPositionCode() {
    ...
    SkTArray<std::unique_ptr<Expression>> children;
    children.reserve(3);
...
std::unique_ptr<Expression> IRGenerator::convertSwizzle(std::unique_ptr<Expression> base,
    ...
    if (baseType.isNumber()) {
        SkTArray<std::unique_ptr<Expression>> scalarConstructorArgs;
    ...
    SkSTArray<3, std::unique_ptr<Expression>> constructorArgs;
            ...
            case '0':
                if (constantZeroIdx == -1) {
                    ...
                    auto zero = std::make_unique<Constructor>(
                            offset, numberType, SkTArray<std::unique_ptr<Expression>>{});
