...
std::unique_ptr<Expression> IRGenerator::convertCompoundConstructor(
    ...
    for (size_t i = 0; i < args.size(); i++) {
        if (actual >= expected) {
            fErrors.error(position, "too many arguments to '" + type.description() +
                                    "' constructor");
            return nullptr;
        }
        if (args[i]->fType.kind() == Type::kVector_Kind) {
            ...
            if (actual + args[i]->fType.columns() > expected) {
                // vector has too many components, swizzle it to get just the number we need
                std::vector<int> components;
                for (int i = 0; i < type.columns() - actual; i++) {
                    components.push_back(i);
                }
                args[i] = std::unique_ptr<Expression>(new Swizzle(fContext, std::move(args[i]),
                                                                  components));
            }
