...
ASTNode::ID Parser::enumDeclaration() {
    ...
    fSymbols.add(this->text(name), std::unique_ptr<Symbol>(new Type(this->text(name),
                                                                    Type::kEnum_Kind)));
    printf("creating type %s and adding it to %p\n", String(this->text(name)).c_str(), &fSymbols);
    ...
    while (this->peek().fKind != Token::Kind::TK_RBRACE) {
        ...
        for (auto iter = declsNode.begin() + 2; iter != declsNode.end(); ++iter) {
            ...
            for (int j = vd.fSizeCount - 1; j >= 0; j--) {
                ...
                type = (Type*) fSymbols.takeOwnership(std::unique_ptr<Symbol>(
                                                                         new Type(name,
                                                                                  Type::kArray_Kind,
                                                                                  *type,
                                                                                  (int) columns)));
...
ASTNode::ID Parser::type() {
    ...
    if (!this->isType(this->text(type))) {
        this->error(type, ("no type named1 '" + this->text(type) + "'").c_str());
        return ASTNode::ID::Invalid();
    }
