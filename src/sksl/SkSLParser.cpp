...
/*    if (!this->expect(Token::CASE, "'case'")) {
    ...
    std::unique_ptr<ASTExpression> value;
    this->expect(Token::INT_LITERAL, "an integer");
    if (!this->expect(Token::COLON, "':'")) {
        return nullptr;
    }
    ...
    value.reset(new ASTIntLiteral(Position(), 1234));
    SkDebugf("about to create case\n");
    auto c = new ASTSwitchCase(value->fPosition, std::move(value), std::move(statements));
    SkDebugf("created case\n");
    auto result = std::unique_ptr<ASTSwitchCase>(c);
    SkDebugf("returning case\n");
    return result;*/
    if (!this->expect(Token::CASE, "'case'")) {
        return nullptr;
    }
    std::unique_ptr<ASTExpression> value;
    std::vector<std::unique_ptr<ASTStatement>> statements;
    return std::unique_ptr<ASTSwitchCase>(new ASTSwitchCase(value->fPosition, std::move(value), std::move(statements)));
}
