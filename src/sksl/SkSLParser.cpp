...
ASTNode::ID Parser::createChild(ASTNode::ID target, ASTNode::ID child) {
...
ASTNode::ID Parser::enumDeclaration() {
    ...
    if (!this->checkNext(Token::Kind::TK_RBRACE)) {
        ...
        if (this->checkNext(Token::Kind::TK_EQ)) {
            ...
            ASTNode::ID child = this->createChild(
...
ASTNode::ID Parser::varDeclarationEnd(Modifiers mods, ASTNode::ID type, StringFragment name) {
    ...
    this->createChild(result, this->createNode(/*offset=*/-1, ASTNode::Kind::kModifiers, mods));
    getNode(result).addChild(type);
    ASTNode::ID currentVar = this->createNode(-1, ASTNode::Kind::kVarDeclaration);
...
ASTNode::ID Parser::parameter() {
    ...
    while (this->checkNext(Token::Kind::TK_LBRACKET)) {
        ...
        this->createChild(result, this->createNode(sizeToken.fOffset, ASTNode::Kind::kInt,
    ...
    while (this->checkNext(Token::Kind::TK_LBRACKET)) {
        if (this->peek().fKind != Token::Kind::TK_RBRACKET) {
            ...
            if (this->intLiteral(&i)) {
                this->createChild(result, this->createNode(/*offset=*/-1, ASTNode::Kind::kInt, i));
            } else {
                return ASTNode::ID::Invalid();
            }
        } else {
...
ASTNode::ID Parser::switchStatement() {
    ...
    if (this->peek().fKind == Token::Kind::TK_DEFAULT) {
        ...
        ASTNode::ID defaultCase = this->createChild(
