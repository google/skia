...
ASTNode::ID Parser::enumDeclaration() {
    ...
    fTypes.add(this->text(name), fTypes.fIRGenerator->createNode(new Type(&fIRGenerator,
                                                                          this->text(name),
                                                                          Type::kEnum_Kind)));
...
ASTNode::ID Parser::structDeclaration() {
    ...
    while (this->peek().fKind != Token::RBRACE) {
        ...
        for (auto iter = declsNode.begin() + 2; iter != declsNode.end(); ++iter) {
            ...
            for (int j = vd.fSizeCount - 1; j >= 0; j--) {
                ...
                typeID = fTypes.fIRGenerator->createNode(new Type(&fIRGenerator, name,
                                                                  Type::kArray_Kind, typeID,
                                                                  (int) columns));
            }
    ...
    fTypes.add(this->text(name), fIRGenerator.createNode(new Type(&fIRGenerator, name.fOffset,
                                                                  this->text(name), fields)));
