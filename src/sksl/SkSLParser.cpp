...
, fTypes(types)
, fErrors(errors) {
    fLexer.start(fText.c_str(), fText.size());
}
...
String Parser::text(Token token) {
    return String(fText.c_str() + token.fOffset, token.fLength);
}
...
std::unique_ptr<ASTDeclaration> Parser::directive() {
    ...
    String text = this->text(start);
    if (text == "#version") {
        this->expect(Token::INT_LITERAL, "a version number");
        Token next = this->peek();
        String nextText = this->text(next);
...
std::unique_ptr<ASTDeclaration> Parser::section() {
    ...
    return std::unique_ptr<ASTDeclaration>(new ASTSection(start.fOffset,
                                                          String(this->text(start).c_str() + 1),
                                                          argument,
                                                          text));
}
