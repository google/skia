    ...
    for (;;) {
        switch (this->peek().fKind) {
            ...
            case Token::GTEQ: {
                ...
                result.reset(new ASTBinaryExpression(std::move(result), t,
                                                     std::move(right)));
                break;
            }
            default:
    ...
    switch (next.fKind) {
        ...
        case Token::COLONCOLON: {
            ...
            if (this->identifier(&text)) {
                return std::unique_ptr<ASTSuffix>(new ASTFieldSuffix(offset, text));
            }
            return nullptr;
        }
        case Token::LPAREN: {
...
std::unique_ptr<ASTExpression> Parser::term() {
    ...
    switch (t.fKind) {
        case Token::IDENTIFIER: {
            StringFragment text;
            if (this->identifier(&text)) {
                result.reset(new ASTIdentifier(t.fOffset, text));
            }
            break;
        }
