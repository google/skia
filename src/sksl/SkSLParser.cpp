...
bool Parser::expectIdentifier(Token* result) {
    ...
    if (this->isType(this->text(*result))) {
        // We don't stop the parse entirely here; we're not necessarily in a broken state.
...
ASTNode::ID Parser::enumDeclaration() {
    ...
    if (!this->checkNext(Token::Kind::TK_RBRACE)) {
        ...
        if (this->isType(this->text(id))) {
            this->error(name, "typename '" + this->text(id) + "' cannot be redeclared");
            return ASTNode::ID::Invalid();
        }
