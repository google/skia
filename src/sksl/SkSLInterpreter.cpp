...
static float4 mix(float4 start, float4 end, float4 t) {
    return start * (1 - t) + end * t;
}
...
void innerRun(const ByteCode* byteCode, const ByteCodeFunction* f, VValue* stack, Value* outReturn,
              int4 initMask, VValue globals[], int globalCount) {
    ...
    int4 condStack[16];  // Independent condition masks
    int4 maskStack[16];  // Combined masks (eg maskStack[0] & maskStack[1] & ...)
    int4 contStack[16];  // Continue flags for loops
    int4 loopStack[16];  // Loop execution masks
    condStack[0] = maskStack[0] = initMask;
    contStack[0] = int4( 0);
    loopStack[0] = int4(~0);
    int4* condPtr = condStack;
    int4* maskPtr = maskStack;
    int4* contPtr = contStack;
    int4* loopPtr = loopStack;
    ...
    for (;;) {
        ...
        switch (inst) {
            ...
            case ByteCodeInstruction::kLoadExtendedGlobal: {
                ...
                int4 m = mask();
                for (int i = 0; i < count; ++i) {
                    if (m[0]) { sp[i + 1].fUnsigned[0] = globals[src[0]].fUnsigned[0]; }
                    if (m[1]) { sp[i + 1].fUnsigned[1] = globals[src[1]].fUnsigned[1]; }
                    if (m[2]) { sp[i + 1].fUnsigned[2] = globals[src[2]].fUnsigned[2]; }
                    if (m[3]) { sp[i + 1].fUnsigned[3] = globals[src[3]].fUnsigned[3]; }
                }
                sp += count;
                break;
            }
            ...
            case ByteCodeInstruction::kStoreExtendedGlobal: {
                ...
                int4 m = mask();
                for (int i = 0; i < count; ++i) {
                    if (m[0]) { globals[target[0]].fSigned[0] = src[i].fSigned[0]; }
                    if (m[1]) { globals[target[1]].fSigned[1] = src[i].fSigned[1]; }
                    if (m[2]) { globals[target[2]].fSigned[2] = src[i].fSigned[2]; }
                    if (m[3]) { globals[target[3]].fSigned[3] = src[i].fSigned[3]; }
                }
                sp -= count;
                break;
            }
            ...
            case ByteCodeInstruction::kStoreSwizzleIndirectGlobal: {
                ...
                for (int i = count - 1; i >= 0; --i) {
                    int4 v = POP().fSigned;
                    if (m[0]) { globals[target[0] + *(ip + i)].fSigned[0] = v[0]; }
                    if (m[1]) { globals[target[1] + *(ip + i)].fSigned[1] = v[1]; }
                    if (m[2]) { globals[target[2] + *(ip + i)].fSigned[2] = v[2]; }
                    if (m[3]) { globals[target[3] + *(ip + i)].fSigned[3] = v[3]; }
                }
                ip += count;
                break;
            }
...
void VecRun(const ByteCode* byteCode, const ByteCodeFunction* f, Value args[], Value* outReturn,
    ...
    while (N) {
        ...
        innerRun(byteCode, f, stack, outReturn, kMasks[w], globals, byteCode->fGlobalCount);
