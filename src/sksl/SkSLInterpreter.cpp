...
void innerRun(const ByteCode* byteCode, const ByteCodeFunction* f, VValue* stack, Value* outReturn,
              VValue globals[], int globalCount) {
    ...
    int4 condStack[16] = { int4(~0) };  // Independent condition masks
    int4 maskStack[16] = { int4(~0) };  // Combined masks (eg maskStack[0] & maskStack[1] & ...)
    int4 contStack[16] = { int4( 0) };  // Continue flags for loops
    int4 loopStack[16] = { int4(~0) };  // Loop execution masks
...
void VecRun(const ByteCode* byteCode, const ByteCodeFunction* f, Value args[], Value* outReturn,
    ...
    std::unique_ptr<VValue[]> largeGlobals;
    VValue* globals = smallGlobals;
    if ((int)SK_ARRAY_COUNT(smallGlobals) < byteCode->fGlobalCount) {
        largeGlobals.reset(new VValue[byteCode->fGlobalCount]);
        globals = largeGlobals.get();
    }
    for (uint8_t slot : byteCode->fInputSlots) {
