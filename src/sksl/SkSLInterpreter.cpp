...
#define BINARY_OP(inst, type, field, op)       \
    case ByteCodeInstruction::inst: {          \
        tmp[0] = POP();                        \
        sp->field = sp->field op tmp[0].field; \
        break;                                 \
    }
    ...
    for (;;) {
        ...
        switch (inst) {
            ...
            case ByteCodeInstruction::kLoad:
                src = READ8();
                SkASSERT(src < STACK_SIZE());
                PUSH(stack[src]);
                break;
            case ByteCodeInstruction::kLoad2:
                src = READ8();
                memcpy(sp + 1, &stack[src], 2 * sizeof(Value));
                sp += 2;
                break;
            case ByteCodeInstruction::kLoad3:
                src = READ8();
                memcpy(sp + 1, &stack[src], 3 * sizeof(Value));
                sp += 3;
                break;
            case ByteCodeInstruction::kLoad4:
                src = READ8();
                memcpy(sp + 1, &stack[src], 4 * sizeof(Value));
                sp += 4;
                break;
            ...
            case ByteCodeInstruction::kLoadGlobal2:
                ...
                memcpy(sp + 1, &fGlobals[tmp[0].fSigned], count * sizeof(Value));
                sp += count;
                break;
            ...
            case ByteCodeInstruction::kReadExternal:
                src = READ8();
                ++sp;
                fByteCode->fExternalValues[src]->read(sp);
                break;
