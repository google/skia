...
Interpreter::Value Interpreter::pop() {
 ...
 Interpreter::StackIndex Interpreter::stackAlloc(int count) {
    ...
    for (int i = 0; i < count; ++i) {
        fStack.push_back({ .fInt = (int) 0xDEADBEEF });
    }
    return result;
}

// returns true if the statement exits normally (i.e. we should advance to the next statement),
// false if the next statement has already been set
void Interpreter::runStatement() {
    ...
    switch (stmt.fKind) {
        case Statement::kBlock_Kind: {
            ...
            ASSERT(index >= 0 && index < b.fStatements.size());
...
static void do_callback(SkJumper_CallbackCtx* raw, int activePixels) {
    ...
    for (int i = 0; i < activePixels; ++i) {
        ctx.fInterpreter->push({ .fFloat = ctx.rgba[i * 4 + 0] });
        ctx.fInterpreter->push({ .fFloat = ctx.rgba[i * 4 + 1] });
        ctx.fInterpreter->push({ .fFloat = ctx.rgba[i * 4 + 2] });
    ...
    switch (a.fStage) {
        ...
        case SkRasterPipeline::callback: {
            ...
            for (const auto& e : fProgram->fElements) {
                if (ProgramElement::kFunction_Kind == e->fKind) {
                    const FunctionDefinition& f = (const FunctionDefinition&) *e;
                    if (&f.fDeclaration == ((const FunctionReference&) *a.fArguments[0]).fFunctions[0]) {
                        ctx->fFunction = &f;
                    }
                }
            }
    ...
    switch (expr.fKind) {
        ...
        case Expression::kBinary_Kind: {
            #define ARITHMETIC(op) {                                      \
                ...
                switch (type_kind(b.fLeft->fType)) {                      \
                    case kFloat_TypeKind:                                 \
                        return { .fFloat = left.fFloat op right.fFloat }; \
                    case kInt_TypeKind:                                   \
                        return { .fInt = left.fInt op right.fInt };       \
                    default:                                              \
                        abort();                                          \
                }                                                         \
            }
            ...
            #define LOGIC(op) {                                          \
                ...
                switch (type_kind(b.fLeft->fType)) {                     \
                    case kFloat_TypeKind:                                \
                        return { .fBool = left.fFloat op right.fFloat }; \
                    case kInt_TypeKind:                                  \
                        return { .fBool = left.fInt op right.fInt };     \
                    default:                                             \
                        abort();                                         \
                }                                                        \
            }
        ...
        case Expression::kBoolLiteral_Kind:
            return { .fBool = ((const BoolLiteral&) expr).fValue };
        case Expression::kConstructor_Kind:
            break;
        case Expression::kIntLiteral_Kind:
            return { .fInt = (int) ((const IntLiteral&) expr).fValue };
        case Expression::kFieldAccess_Kind:
            break;
        case Expression::kFloatLiteral_Kind:
            return { .fFloat = (float) ((const FloatLiteral&) expr).fValue };
            ...
            switch (p.fOperator) {
                case Token::MINUS: {
                    ...
                    switch (type_kind(p.fType)) {
                        case kFloat_TypeKind:
                            return { .fFloat = -base.fFloat };
                        case kInt_TypeKind:
                            return { .fInt = -base.fInt };
                        default:
                            abort();
                    }
                }
