...
void Interpreter::run(const FunctionDefinition& f) {
    ...
    for (int i = f.fDeclaration.fParameters.size() - 1; i >= 0; --i) {
        ...
        fVars.top()[f.fDeclaration.fParameters[i]] = current;
    }
    fCurrentIndex.push({ f.fBody.get(), 0 });
...
Interpreter::Value Interpreter::pop() {
 ...
 Interpreter::StackIndex Interpreter::stackAlloc(int count) {
    ...
    for (int i = 0; i < count; ++i) {
        fStack.push_back(Value{ .fInt = (int) 0xDEADBEEF });
    }
    return result;
}

// returns true if the statement exits normally (i.e. we should advance to the
// next statement), false if the next statement has already been set
void Interpreter::runStatement() {
    const Statement& stmt = *fCurrentIndex.top().fStatement;
    const size_t index = fCurrentIndex.top().fIndex;
    fCurrentIndex.pop();
    switch (stmt.fKind) {
        case Statement::kBlock_Kind: {
            ...
            if (index < b.fStatements.size() - 1) {
                fCurrentIndex.push({ &b, index + 1 });
            }
            fCurrentIndex.push({ b.fStatements[index].get(), 0 });
            break;
        }
        ...
        case Statement::kFor_Kind: {
            ...
            switch (index) {
                case 0:
                    // initializer
                    fCurrentIndex.push({ &f, 1 });
                    if (f.fInitializer) {
                        fCurrentIndex.push({ f.fInitializer.get(), 0 });
                    }
                    break;
                case 1:
                    // test & body
                    if (f.fTest && !evaluate(*f.fTest).fBool) {
                        ...
                        fCurrentIndex.push({ &f, 2 });
                        fCurrentIndex.push({ f.fStatement.get(), 0 });
                    }
                    break;
                case 2:
                    // next
        ...
        case Statement::kIf_Kind: {
            ...
            if (evaluate(*i.fTest).fBool) {
                fCurrentIndex.push({ i.fIfTrue.get(), 0 });
            } else if (i.fIfFalse) {
                fCurrentIndex.push({ i.fIfFalse.get(), 0 });
            }
            break;
        }
...
Interpreter::StackIndex Interpreter::getLValue(const Expression& expr) {
    switch (expr.fKind) {
        ...
        case Expression::kIndex_Kind: {
            ...
            return this->evaluate(*idx.fBase).fIndex + this->evaluate(*idx.fIndex).fInt;
        }
        case Expression::kSwizzle_Kind:
            break;
        case Expression::kVariableReference_Kind:
            ASSERT(fVars.size());
            ASSERT(fVars.top().find(&((VariableReference&) expr).fVariable) !=
                   fVars.top().end());
            return fVars.top()[&((VariableReference&) expr).fVariable];
...
static void do_callback(SkJumper_CallbackCtx* raw, int activePixels) {
    ...
    for (int i = 0; i < activePixels; ++i) {
        ctx.fInterpreter->push(Interpreter::Value{ .fFloat = ctx.rgba[i * 4 + 0] });
        ctx.fInterpreter->push(Interpreter::Value{ .fFloat = ctx.rgba[i * 4 + 1] });
        ctx.fInterpreter->push(Interpreter::Value{ .fFloat = ctx.rgba[i * 4 + 2] });
...
void Interpreter::appendStage(const AppendStage& a) {
    switch (a.fStage) {
        case SkRasterPipeline::matrix_4x5: {
            ...
            StackIndex transpose = evaluate(*a.fArguments[0]).fIndex;
...
Interpreter::Value Interpreter::evaluate(const Expression& expr) {
    switch (expr.fKind) {
        case Expression::kAppendStage_Kind:
            ...
            return Value{ .fInt = (int) 0xDEADBEEF };
        case Expression::kBinary_Kind: {
            #define ARITHMETIC(op) {                                      \
                ...
                switch (type_kind(b.fLeft->fType)) {                      \
                    case kFloat_TypeKind:                                 \
                        return Value{ .fFloat = left.fFloat op right.fFloat }; \
                    case kInt_TypeKind:                                   \
                        return Value{ .fInt = left.fInt op right.fInt };       \
                    default:                                              \
                        abort();                                          \
                }                                                         \
            }
            ...
            #define LOGIC(op) {                                          \
                ...
                switch (type_kind(b.fLeft->fType)) {                     \
                    case kFloat_TypeKind:                                \
                        return Value{ .fBool = left.fFloat op right.fFloat }; \
                    case kInt_TypeKind:                                  \
                        return Value{ .fBool = left.fInt op right.fInt };     \
                    default:                                             \
                        abort();                                         \
                }                                                        \
            }
        ...
        case Expression::kBoolLiteral_Kind:
            return Value{ .fBool = ((const BoolLiteral&) expr).fValue };
        case Expression::kConstructor_Kind:
            break;
        case Expression::kIntLiteral_Kind:
            return Value{ .fInt = (int) ((const IntLiteral&) expr).fValue };
        case Expression::kFieldAccess_Kind:
            break;
        case Expression::kFloatLiteral_Kind:
            return Value{ .fFloat = (float) ((const FloatLiteral&) expr).fValue };
        case Expression::kFunctionCall_Kind:
            return this->call((const FunctionCall&) expr);
        case Expression::kIndex_Kind: {
            const IndexExpression& idx = (const IndexExpression&) expr;
            StackIndex pos = this->evaluate(*idx.fBase).fIndex +
            ...
            switch (p.fOperator) {
                case Token::MINUS: {
                    ...
                    switch (type_kind(p.fType)) {
                        case kFloat_TypeKind:
                            return Value{ .fFloat = -base.fFloat };
                        case kInt_TypeKind:
                            return Value{ .fInt = -base.fInt };
                        default:
                            abort();
                    }
                }
        ...
        case Expression::kVariableReference_Kind:
            ...
            ASSERT(fVars.top().find(&((VariableReference&) expr).fVariable) !=
                   fVars.top().end());
            return fStack[fVars.top()[&((VariableReference&) expr).fVariable]];
        case Expression::kTernary_Kind: {
            const TernaryExpression& t = (const TernaryExpression&) expr;
