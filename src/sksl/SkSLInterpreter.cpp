    ...
    for (;;) {
        ...
        switch (inst) {
            ...
            case ByteCodeInstruction::kStoreGlobal3:
                fGlobals[*ip+2] = skvx::if_then_else(mask(), POP().fFloat, fGlobals[*ip+2].fFloat);
            case ByteCodeInstruction::kStoreGlobal2:
                fGlobals[*ip+1] = skvx::if_then_else(mask(), POP().fFloat, fGlobals[*ip+1].fFloat);
            case ByteCodeInstruction::kStoreGlobal :
                fGlobals[*ip+0] = skvx::if_then_else(mask(), POP().fFloat, fGlobals[*ip+0].fFloat);
                ++ip;
                break;
