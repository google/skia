...
void innerRun(const ByteCode* byteCode, const ByteCodeFunction* f, VValue* stack,
              int32_t* outReturn, I32 initMask, VValue globals[]) {
...
void ByteCode::run(const ByteCodeFunction* f, void* args, void* outReturn, int N,
    ...
    Interpreter::VValue smallGlobals[32];
    Interpreter::VValue* globals = smallGlobals;
    SkASSERT((int)SK_ARRAY_COUNT(smallGlobals) >= fGlobalCount);
    ...
    while (N) {
        ...
        auto mask = w > gLanes;
        innerRun(this, f, stack, intOut, mask, globals);
