    ...
    for (;;) {
        ...
        switch (inst) {
            ...
            case ByteCodeInstruction::kLoadSwizzleGlobal: {
                ...
                int count = READ8();
                for (int i = 0; i < count; ++i) {
                    PUSH(uint4(fGlobals[src + *(ip + i)].fUnsigned));
                }
                ip += count;
                break;
            }
            ...
            case ByteCodeInstruction::kRemainderF4:
                sp[-4] = sp[-4].fFloat - skvx::trunc(sp[-4].fFloat / sp[0].fFloat) * sp[0].fFloat;
                POP();
            case ByteCodeInstruction::kRemainderF3: {
                int tgt = (int) ByteCodeInstruction::kRemainderF - (int) inst - 1;
                sp[tgt] = sp[tgt].fFloat - skvx::trunc(sp[tgt].fFloat/sp[0].fFloat) * sp[0].fFloat;
                POP();
            }
