...
void VecInterpreter::innerRun(const ByteCodeFunction& f, VValue* stack, SValue* outReturn) {
    ...
    int4 condStack[32] = { int4(~0) };  // Independent condition masks
    int4 maskStack[32] = { int4(~0) };  // Combined masks (eg maskStack[0] & maskStack[1] & ...)
    int4* cond = condStack;
    int4* mask = maskStack;
