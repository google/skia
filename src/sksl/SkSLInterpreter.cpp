...
void Interpreter::run(const ByteCodeFunction& f, Interpreter::Value args[],
                      Interpreter::Value inputs[], Interpreter::Value* outReturn) {
    fCurrentFunction = &f;
#ifdef TRACE
    this->disassemble(f);
#endif
    ...
    Value* stack = (Value*) alloca(fCurrentFunction->fStackCount * sizeof(Value));
...
#ifdef TRACE
        if (inst != ByteCodeInstruction::kNop) {
