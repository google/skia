...
, fErrorCount(0) {
    ...
    fRootModule = {fRootSymbolTable, nullptr};

    fGPUModule =
            this->processIncludeFile(Program::kFragment_Kind, PRE_INCLUDE_DATA(gpu), fRootModule);
    fVertexModule =
            this->processIncludeFile(Program::kVertex_Kind, PRE_INCLUDE_DATA(vert), fGPUModule);
    fFragmentModule =
            this->processIncludeFile(Program::kFragment_Kind, PRE_INCLUDE_DATA(frag), fGPUModule);
}

Compiler::~Compiler() {}

void Compiler::loadGeometryIntrinsics() {
    if (!fGeometryModule.first) {
        fGeometryModule = this->processIncludeFile(
                Program::kGeometry_Kind, PRE_INCLUDE_DATA(geom), fGPUModule);
    }
}

void Compiler::loadFPIntrinsics() {
    if (!fFPModule.first) {
        fFPModule = this->processIncludeFile(
                Program::kFragmentProcessor_Kind, PRE_INCLUDE_DATA(fp), fGPUModule);
    }
}

void Compiler::loadPipelineIntrinsics() {
    if (!fPipelineModule.first) {
        fPipelineModule = this->processIncludeFile(
                Program::kPipelineStage_Kind, PRE_INCLUDE_DATA(pipeline), fGPUModule);
    }
}

void Compiler::loadInterpreterIntrinsics() {
    if (!fInterpreterModule.first) {
        fInterpreterModule = this->processIncludeFile(
                Program::kGeneric_Kind, PRE_INCLUDE_DATA(interp), fRootModule);
    }
}

Compiler::LoadedModule Compiler::loadIncludeFile(Program::Kind kind,
                                                 Compiler::PreIncludeData data,
    ...
    fIRGenerator->start(&settings, {base, nullptr}, true);
    fIRGenerator->convertProgram(kind, source->c_str(), source->length(), &module.second);
    fIRGenerator->fCanInline = true;
    if (this->fErrorCount) {
        printf("Unexpected errors: %s\n", this->fErrorText.c_str());
        SkDEBUGFAILF("%s %s\n", data, this->fErrorText.c_str());
    }
    module.first = fIRGenerator->fSymbolTable;
    fSource = nullptr;
    ...
    module.first = rehydrator.symbolTable();
    module.second = rehydrator.elements();
    fModifiers.push_back(fIRGenerator->releaseModifiers());
#endif
    ...
    for (std::unique_ptr<ProgramElement>& element : module.second) {
        if (element->is<FunctionDefinition>()) {
            element->as<FunctionDefinition>().fDeclaration.callCount() = 0;
        }
    }
...
Compiler::ParsedModule Compiler::processIncludeFile(Program::Kind kind,
                                                    PreIncludeData data,
                                                    const ParsedModule& base) {
    auto [symbols, elements] = this->loadIncludeFile(kind, data, base.first);
    auto intrinsics = std::make_unique<IRIntrinsicMap>(base.second.get());
