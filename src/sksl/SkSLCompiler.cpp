...
/**
 * Used in the implementations of vectorize_left and vectorize_right. Given an expression of the
 * form vec<n>(x) and another expression y, deletes the expression pointed to by iter and replaces
 * it with vec4<n>(y).
 */
static void vectorize(BasicBlock* b,
                      std::vector<BasicBlock::Node>::iterator* iter,
                      std::unique_ptr<Expression>* vectorExpression,
                      std::unique_ptr<Expression>* otherExpression,
                      bool* outUpdated,
                      bool* outNeedsRescan) {
    ASSERT((*(*iter)->expression())->fKind == Expression::kBinary_Kind);
    ASSERT((*vectorExpression)->fType.kind() == Type::kVector_Kind);
    ASSERT((*otherExpression)->fType.kind() == Type::kScalar_Kind);
    *outUpdated = true;
    std::unique_ptr<Expression>* target = (*iter)->expression();
    if (!b->tryRemoveExpression(iter)) {
        *target = construct((*vectorExpression)->fType, std::move(*otherExpression));
        *outNeedsRescan = true;
    } else {
        *target = construct((*vectorExpression)->fType, std::move(*otherExpression));
...
static void vectorize_left(BasicBlock* b,
    ...
    vectorize(b, iter, &bin.fRight, &bin.fLeft, outUpdated, outNeedsRescan);
}
...
static void vectorize_right(BasicBlock* b,
                            ...
                            bool* outNeedsRescan) {
    BinaryExpression& bin = (BinaryExpression&) **(*iter)->expression();
    vectorize(b, iter, &bin.fLeft, &bin.fRight, outUpdated, outNeedsRescan);
}
