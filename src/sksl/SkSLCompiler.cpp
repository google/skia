...
static void dumpDefs(DefinitionMap& definitions) {
/*    printf("### definitions(%p) ###\n", &definitions);
    for (const auto& e : definitions) {
        if (e.first && e.second.id())
            printf("    %s: %s(%p)\n", e.first.node().description().c_str(), e.second.node().description().c_str(), e.second.fNode);
    }*/
}

// add the definition created by assigning to the lvalue to the definition set
void Compiler::addDefinition(IRNode::ID lvalueID, IRNodeIDPtr expr,
                             DefinitionMap* definitions) {
    Expression& lvalue = lvalueID.expression();
    IRNodeIDPtr defined{IRNode::ID(), 0, -1};
    switch (lvalue.fKind) {
        case Expression::kVariableReference_Kind: {
            IRNode::ID varID = ((VariableReference&) lvalue).fVariable;
            if (((Variable&) varID.node()).fStorage == Variable::kLocal_Storage) {
                printf("addDefinition: %s = %s\n", lvalue.description().c_str(), expr.node().description().c_str());
        ...
        case Expression::kSwizzle_Kind:
            ...
            this->addDefinition(((Swizzle&) lvalue).fBase, defined, definitions);
            break;
        case Expression::kIndex_Kind:
            // see comments in Swizzle
            this->addDefinition(((IndexExpression&) lvalue).fBase, defined, definitions);
            break;
        case Expression::kFieldAccess_Kind:
            // see comments in Swizzle
            this->addDefinition(((FieldAccess&) lvalue).fBase, defined, definitions);
            break;
        case Expression::kTernary_Kind:
            ...
            this->addDefinition(((TernaryExpression&) lvalue).fIfTrue, defined, definitions);
            this->addDefinition(((TernaryExpression&) lvalue).fIfFalse, defined, definitions);
            break;
            ...
            if (node.fKind == BasicBlock::Node::kStatement_Kind) {
                ...
                if (s.fKind == Statement::kVarDeclarations_Kind) {
                    ...
                    for (IRNode::ID decl : ((VarDeclarations&) vd.fDeclaration.node()).fVars) {
                        if (decl.statement().fKind == Statement::kVarDeclaration_Kind) {
...
static bool is_dead(const Expression& lvalue) {
    switch (lvalue.fKind) {
        ...
        case Expression::kSwizzle_Kind:
            return is_dead(((Swizzle&) lvalue).fBase.expression());
        case Expression::kFieldAccess_Kind:
            return is_dead(((FieldAccess&) lvalue).fBase.expression());
        case Expression::kIndex_Kind: {
            const IndexExpression& idx = (IndexExpression&) lvalue;
            return is_dead(idx.fBase.expression()) &&
                   !idx.fIndex.expression().hasSideEffects();
        }
        case Expression::kTernary_Kind: {
            const TernaryExpression& t = (TernaryExpression&) lvalue;
            return !t.fTest.expression().hasSideEffects() &&
                   is_dead(t.fIfTrue.expression()) &&
                   is_dead(t.fIfFalse.expression());
        }
        case Expression::kExternalValue_Kind:
            return false;
        default:
...
static bool dead_assignment(const BinaryExpression& b) {
    ...
    return is_dead(b.fLeft.expression());
}
...
void delete_left(BasicBlock* b,
    ...
    *outUpdated = true;
    ...
    SkASSERT(!bin.fLeft.expression().hasSideEffects());
    bool result;
...
void delete_right(BasicBlock* b,
    ...
    *outUpdated = true;
    ...
    IRNodeIDPtr target = (*iter)->idPtr();
    BinaryExpression& bin = (BinaryExpression&) (*iter)->expression();
    SkASSERT(!bin.fRight.expression().hasSideEffects());
...
static void vectorize(BasicBlock* b,
    ...
    SkASSERT(otherExpression.expression().fType.typeNode().kind() == Type::kScalar_Kind);
    *outUpdated = true;
    IRNodeIDPtr target = (*iter)->idPtr();
    if (!b->tryRemoveExpression(iter)) {
...
static void vectorize_left(BasicBlock* b,
                           ...
                           bool* outNeedsRescan) {
    BinaryExpression& bin = (BinaryExpression&) (*iter)->expression();
    vectorize(b, iter, bin.fRight.expression().fType, bin.fLeft, outUpdated, outNeedsRescan);
}
...
static void vectorize_right(BasicBlock* b,
                            ...
                            bool* outNeedsRescan) {
    BinaryExpression& bin = (BinaryExpression&) (*iter)->expression();
    vectorize(b, iter, bin.fLeft.expression().fType, bin.fRight, outUpdated, outNeedsRescan);
}
...
void clear_write(const Expression& expr) {
    switch (expr.fKind) {
        ...
        case Expression::kFieldAccess_Kind:
            clear_write(((FieldAccess&) expr).fBase.expression());
            break;
        case Expression::kSwizzle_Kind:
            clear_write(((Swizzle&) expr).fBase.expression());
            break;
        case Expression::kIndex_Kind:
            clear_write(((IndexExpression&) expr).fBase.expression());
            break;
        default:
...
void Compiler::simplifyExpression(DefinitionMap& definitions,
    ...
    if ((*iter)->fConstantPropagation) {
        printf("checking propagation of %s\n", expr->description().c_str());
        printf("with definitions:\n");
        dumpDefs(definitions);
        IRNode::ID optimized = expr->constantPropagate(definitions);
        if (optimized) {
            printf("replacing %s with %s\n", expr->description().c_str(), optimized.node().description().c_str());
            *outUpdated = true;
...
//                this->error(expr->fOffset, "'" + var.fName + "' has not been assigned");
        ...
        case Expression::kTernary_Kind: {
            ...
            if (t->fTest.expression().fKind == Expression::kBoolLiteral_Kind) {
                // ternary has a constant test, replace it with either the true or
                // false branch
                if (((BoolLiteral&) t->fTest.expression()).fValue) {
        ...
        case Expression::kBinary_Kind: {
            ...
            Expression& left = bin->fLeft.expression();
            Expression& right = bin->fRight.expression();
    ...
    switch (s.fKind) {
        case Statement::kBlock_Kind: {
            ...
            for (IRNode::ID sub : b.fStatements) {
                if (contains_conditional_break(sub.statement(), inConditional)) {
                    return true;
                }
            }
            return false;
        ...
        case Statement::kIf_Kind: {
            ...
            return contains_conditional_break(i.fIfTrue.statement(), true) ||
                   (i.fIfFalse && contains_conditional_break(i.fIfFalse.statement(), true));
        }
        default:
            return false;
    }
    ...
    switch (s.fKind) {
        case Statement::kBlock_Kind:
            for (IRNode::ID sub : ((Block&) s).fStatements) {
                if (contains_unconditional_break(sub.statement())) {
                    return true;
                }
            }
            return false;
    ...
    for (IRNode::ID current : switchStatement.fCases) {
        ...
        if (capturing) {
            for (IRNode::ID stmt : ((SwitchCase&) current.node()).fStatements) {
                if (contains_conditional_break(stmt.statement(),
                                               stmt.statement().fKind == Statement::kIf_Kind)) {
                    return IRNode::ID();
                }
                if (contains_unconditional_break(stmt.statement())) {
                    capturing = false;
                    break;
                }
...
void Compiler::simplifyStatement(DefinitionMap& definitions,
    ...
    switch (stmt.fKind) {
        ...
        case Statement::kIf_Kind: {
            ...
            if (i.fTest.expression().fKind == Expression::kBoolLiteral_Kind) {
                // constant if, collapse down to a single branch
                if (((BoolLiteral&) i.fTest.expression()).fValue) {
                    SkASSERT(i.fIfTrue);
            ...
            if (i.fIfFalse && i.fIfFalse.statement().isEmpty()) {
                // else block doesn't do anything, remove it
                i.fIfFalse = IRNode::ID();
                *outUpdated = true;
                *outNeedsRescan = true;
            }
            if (!i.fIfFalse && i.fIfTrue.statement().isEmpty()) {
                // if block doesn't do anything, no else block
                if (i.fTest.expression().hasSideEffects()) {
        ...
        case Statement::kSwitch_Kind: {
            ...
            if (s.fValue.expression().isConstant()) {
                ...
                for (IRNode::ID cID : s.fCases) {
                    ...
                    SkASSERT(c.fValue.expression().fKind == s.fValue.expression().fKind);
                    found = c.fValue.expression().compareConstant(s.fValue.expression());
                    if (found) {
        ...
        case Statement::kExpression_Kind: {
            ...
            if (!e.fExpression.expression().hasSideEffects()) {
    ...
    printf("after simplifyStatement: %s\n", (*iter)->statement().description().c_str());
}
...
void Compiler::scanCFG(IRNode::ID functionDefinition) {
    ...
    printf("after scan:\n");
    printf("%s\n", functionDefinition.node().description().c_str());
                ...
                switch (s.fKind) {
                    ...
                    case Statement::kVarDeclarations_Kind: {
                        ...
                        for (auto varIter = decls.fVars.begin(); varIter != decls.fVars.end();) {
                            if (varIter->statement().fKind == Statement::kNop_Kind) {
...
void Compiler::registerExternalValue(ExternalValue* value) {
    IRNode::ID symbol = fIRGenerator->createNode(new ExternalValueSymbol(fIRGenerator, value));
    fIRGenerator->fSymbolTable->add(value->fName, symbol);
}
