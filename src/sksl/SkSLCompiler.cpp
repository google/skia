...
static std::unique_ptr<Statement> block_for_case(SwitchStatement* switchStatement,
    ...
    int index;
    for (index = 0; index < switchStatement->caseCount(); ++index) {
        if (&switchStatement->getCase(index) == caseToCapture) {
            break;
        }
    }
    ...
    for (; index < switchStatement->caseCount(); ++index) {
        for (std::unique_ptr<Statement>& stmt : switchStatement->getCase(index).statements()) {
    ...
    while (startIndex != index) {
        SwitchCase& c = switchStatement->getCase(startIndex);
        for (std::unique_ptr<Statement>& stmt : c.statements()) {
