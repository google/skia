...
#define MODULE_DATA(name) { "sksl_" #name ".sksl", nullptr, 0 }
...
#define MODULE_DATA(name) { nullptr,  SKSL_INCLUDE_sksl_##name, SKSL_INCLUDE_sksl_##name##_LENGTH }
...
LoadedModule Compiler::loadModule(Program::Kind kind,
    ...
    fIRGenerator->start(&settings, {base, /*intrinsics=*/nullptr}, /*builtin=*/true);
...
ParsedModule Compiler::parseModule(Program::Kind kind, ModuleData data, const ParsedModule& base) {
    ...
    auto intrinsics = std::make_unique<IRIntrinsicMap>(base.fIntrinsics.get());
