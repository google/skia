...
std::unique_ptr<Program> Compiler::convertProgram(Program::Kind kind,
                                                  String text,
                                                  const Program::Settings& settings,
                                                  std::vector<ExternalValue*>* externalValues) {
    ...
    if (externalValues) {
        ...
        for (auto ev : *externalValues) {
            fIRGenerator->fSymbolTable->addWithoutOwnership(ev->fName, ev);
        }
    }
