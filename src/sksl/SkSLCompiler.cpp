                ...
                if (iter->fKind == BasicBlock::Node::kExpression_Kind) {
                    ...
                    switch (expr->fKind) {
                        case Expression::kVariableReference_Kind: {
                            ...
                            if (firstPass && var.fStorage == Variable::kLocal_Storage &&
                                !definitions[&var]) {
                            ...
                            for (auto varIter = vd.fVars.begin(); varIter != vd.fVars.end();
                                ...
                                if (varIter->fVar->dead() && (!varIter->fValue ||
                                    ...
                                    if (varIter->fValue) {
                                        ...
                                        if (!b.tryRemoveExpressionBefore(&iter,
                                                                         varIter->fValue.get())) {
                                            needsRescan = true;
                                        }
                                    }
                                ...
                                if (assignment_to_dead_var(bin)) {
                                    if (bin.fRight->hasSideEffects()) {
                                        ...
                                        if (bin.fOperator == Token::EQ) {
                                            if (!b.tryRemoveLValueBefore(&iter,
                                                                         bin.fLeft.get())) {
                                                needsRescan = true;
                                            }
                                        } else if (!b.tryRemoveExpressionBefore(&iter,
                                                                                bin.fLeft.get())) {
                                            needsRescan = true;
                                        }
                                        ...
                                        // pick up subexpressions. This will leave an extra copy of
