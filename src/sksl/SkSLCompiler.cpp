...
#include "include/private/DSL_priv.h"
...
LoadedModule Compiler::loadModule(ProgramKind kind,
    ...
    dsl::StartBuiltinCode(this, kind, settings, baseModule);
    ...
    dsl::DSLWriter::ProgramElements().clear();
    dsl::End();
    if (this->fErrorCount) {
...
std::unique_ptr<Program> Compiler::convertProgram(
    ...
    auto program = std::make_unique<Program>(std::move(textPtr),
                                             std::move(dsl::DSLWriter::ProgramConfig()),
                                             fContext,
                                             std::move(ir.fElements),
                                             std::move(ir.fSharedElements),
                                             std::move(dsl::DSLWriter::ModifiersPool()),
