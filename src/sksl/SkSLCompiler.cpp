    ...
    switch (expr->kind()) {
        case Expression::Kind::kVariableReference: {
            ...
            if (ref.refKind() != VariableReference::RefKind::kWrite &&
                ...
                optimizationContext->fIgnores.find(var) == optimizationContext->fIgnores.end()) {
                optimizationContext->fIgnores.insert(var);
                    ...
                    if (caseValue == switchValue) {
                        ...
                        if (newBlock) {
                            ...
                            if (s.isStatic() && !(fFlags & kPermitInvalidStaticTests_Flag) &&
                                optimizationContext->fIgnores.find(&s) ==
                                optimizationContext->fIgnores.end()) {
                                this->error(s.fOffset,
                                            "static switch contains non-static conditional break");
                                optimizationContext->fIgnores.insert(&s);
                            }
