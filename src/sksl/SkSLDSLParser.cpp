...
std::unordered_map<StringFragment, DSLParser::LayoutToken>* DSLParser::layoutTokens;
...
/*    if (!parseArrayDimensions(currentVar, &vd)) {
    ...
    result.emplace_back(mods, type, Retain(String(name))->c_str(),
...
skstd::string_view DSLParser::layoutCode() {
    if (!this->expect(Token::Kind::TK_EQ, "'='")) {
        return "";
    }
...
Layout::CType DSLParser::layoutCType() {
    if (this->expect(Token::Kind::TK_EQ, "'='")) {
        ...
        StringFragment text = this->text(t);
