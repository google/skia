...
class DSLParserErrorHandler : public ErrorHandler {
public:
...
std::unique_ptr<Program> DSLParser::program() {
    ...
    fText = *Retain(SkSL::String(fText));
...
bool DSLParser::declaration() {
    ...
    if (lookahead.fKind == Token::Kind::TK_IDENTIFIER && !IsType(this->text(lookahead))) {
        ...
        this->error(lookahead, "interface blocks are not yet supported");
    }
...
bool DSLParser::functionDeclarationEnd(DSLModifiers modifiers, DSLType type, const Token& name) {
    ...
    const char* c_name = Retain(String(this->text(name)))->c_str();
    SkTArray<DSLVar*> parameterPointers;
    for (skstd::optional<DSLWrapper<DSLVar>>& param : parameters) {
        parameterPointers.push_back(&*param.value());
    }
...
SkTArray<dsl::DSLVar> DSLParser::varDeclarationEnd(DSLModifiers mods, DSLType baseType,
    ...
    result.push_back(DSLVar(mods, type, name, std::move(initializer)));
    ...
    while (this->checkNext(Token::Kind::TK_COMMA)) {
        ...
        result.push_back(DSLVar(mods, type, this->text(identifierName), std::move(initializer)));
    }
...
/*        if (isArray || this->isArrayType(type)) {
        ...
        isArray = true;*/
        this->error(name, "arrays are not yet fully supported");
    }
    return {{DSLVar(modifiers, *type, this->text(name))}};
}
...
/*        if (isArray) {
        ...
        this->error(type, "arrays are not yet fully supported");
    }
    return result;
}
...
/*    Token name;
    ...
    this->expect(Token::Kind::TK_SEMICOLON, "';'");
    return result;*/
    abort(); // TODO(ethannicholas): implement this
}
...
/* ternaryExpression ((EQEQ | STAREQ | SLASHEQ | PERCENTEQ | PLUSEQ | MINUSEQ | SHLEQ | SHREQ |
   BITWISEANDEQ | BITWISEXOREQ | BITWISEOREQ | LOGICALANDEQ | LOGICAzLXOREQ | LOGICALOREQ)
...
//        OPERATOR_RIGHT(^^, logicalXorExpression)
        this->nextToken();
        this->error(/*offset=*/-1, "'^^' operator is not yet supported");
    }
    return result;
}
