...
void PipelineStageCodeGenerator::writeFunction(const FunctionDefinition& f) {
    ...
    if (f.declaration().name() == "main") {
    ...
    } else {
        {
            ...
            SkAssertResult(type_to_grsltype(fContext, decl.returnType(), &result.fReturnType));
            result.fName = decl.name();
            for (const Variable* v : decl.parameters()) {
                GrSLType paramSLType;
                SkAssertResult(type_to_grsltype(fContext, v->type(), &paramSLType));
...
void PipelineStageCodeGenerator::writeConstructor(const Constructor& c, Precedence parentPrecedence) {
