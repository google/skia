...
public:
    PipelineStageCodeGenerator(const Program& program,
            ...
            , fDeclareUniform(std::move(declareUniform))
            , fDefineFunction(std::move(defineFunction))
            , fSampleChild(std::move(sampleChild))
            , fSampleChildWithMatrix(std::move(sampleChildWithMatrix)) {}
...
private:
    ...
    DeclareUniformFn     fDeclareUniform;
    DefineFunctionFn     fDefineFunction;
    SampleChildFn        fSampleChild;
    SampleChildFn        fSampleChildWithMatrix;
...
void PipelineStageCodeGenerator::writeFunctionCall(const FunctionCall& c) {
    ...
    if (function.isBuiltin() && function.name() == "sample") {
        ...
        if (matrixCall) {
            this->write(fSampleChildWithMatrix(index, std::move(coordsOrMatrix)));
        } else {
            this->write(fSampleChild(index, std::move(coordsOrMatrix)));
        }
        return;
    }
...
void PipelineStageCodeGenerator::writeFunction(const FunctionDefinition& f) {
    ...
    String fnName = fDefineFunction(&f.declaration(), body.fBuffer.str());
...
void PipelineStageCodeGenerator::writeGlobalVarDeclaration(const GlobalVarDeclaration& g) {
    ...
    if (var.modifiers().fFlags & Modifiers::kUniform_Flag) {
        String uniformName = fDeclareUniform(&decl);
...
void PipelineStageCodeGenerator::generateCode() {
    ...
    // Write the functions last. The Inliner likes to move function bodies around. After inlining,
    // code can inadvertently move upwards, above ProgramElements that the code relies on.
    for (const ProgramElement* e : fProgram.elements()) {
        if (e->is<FunctionDefinition>()) {
            this->writeProgramElement(*e);
        }
...
void ConvertProgram(const Program& program,
                    ...
                    SampleChildFn sampleChild,
                    SampleChildFn sampleChildWithMatrix) {
    PipelineStageCodeGenerator generator(program, sampleCoords, std::move(declareUniform),
                                         std::move(defineFunction), std::move(sampleChild),
                                         std::move(sampleChildWithMatrix));
