    ...
    if (c.fFunction.fBuiltin && c.fFunction.fName == "sample" &&
        ...
        fArgs->fFormatArgs.push_back(
               Compiler::FormatArg(matrixCall ? Compiler::FormatArg::Kind::kChildProcessorWithMatrix
                                   ...
                                   index));
        while ((int) fSampleMatrices->size() <= index) {
            fSampleMatrices->push_back(SampleMatrix(SampleMatrix::Kind::kNone));
        }
        if (matrixCall) {
            SkASSERT((*fSampleMatrices)[index].fKind == SampleMatrix::Kind::kNone);
            (*fSampleMatrices)[index] = SampleMatrix(SampleMatrix::Kind::kVariable);
        }
