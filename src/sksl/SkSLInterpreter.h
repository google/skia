...
public:
    ...
    Vector* run(const ByteCodeFunction* f, Vector args[]) {
        ...
        Vector* result = this->innerRun(f, context);
        int slot = 0;
    ...
    void runStriped(const ByteCodeFunction* f, int count, Vector* args[], Vector* outReturn) {
        SkASSERT(f);
        Vector* stack = fMemory + MEMORY_SIZE;
        int stackCount = f->fStackSlotCount + f->fParameterSlotCount;
        stack -= stackCount;
        ...
        for (int i = 0; i < count; ++i) {
            ...
            for (const auto& p : f->fParameters) {
                if (p.fIsOutParameter) {
                    memcpy(&args[slot][i], &stack[slot], p.fSlotCount * sizeof(Vector));
                }
                slot += p.fSlotCount;
            }
        }
    ...
    void disassemble(const uint8_t** ip) {
        ...
        switch (inst) {
            ...
            case ByteCode::Instruction::kCall: {
                ...
                printf("call %s($%d...)", f->fName.c_str(), read<ByteCode::Register>(ip).fIndex);
                printf("\n");
                break;
            }
    ...
    Vector* innerRun(const ByteCodeFunction* f, Context context) {
        ...
        for (;;) {
            ...
            switch (inst) {
                ...
                LABEL(kCallExternal) {
                    /*uint8_t argumentCount = read<uint8_t>(&ip);
                    uint8_t returnCount = read<uint8_t>(&ip);
                    ...
                    for (int i = 0; i < width; ++i) {
                        if (mask[i]) {
                            for (int j = 0; j < argumentCount; ++j) {
                                tmpArgs[j] = fRegisters[argumentsBase.fIndex + j].fFloat[i];
                            }
                            v->call(baseIndex + i, tmpArgs, tmpReturn);
                            for (int j = 0; j < returnCount; ++j) {
                                fRegisters[returnBase.fIndex + j].fFloat[i] = tmpReturn[j];
                            }
                        }
                    }
                    sp += returnCount - 1;
                ...
                LABEL(kReturnValue) {
                    ByteCode::Register returnValue = read<ByteCode::Register>(&ip);
                    if (context.fCallStack.empty()) {
                        return &fRegisters[returnValue.fIndex];
                    }
                    StackFrame frame = context.fCallStack.top();
                    ip = frame.fIP;
                    context.fStack += frame.fStackSlotCount;
