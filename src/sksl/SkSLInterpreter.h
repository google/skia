...
public:
    ...
    bool runStriped(const ByteCodeFunction* f, int count, float* args[], Vector** outResult) {
        SkASSERT(f);
        Vector* stack = fMemory + MEMORY_SIZE;
        int stackCount = f->fStackSlotCount + f->fParameterSlotCount;
        stack -= stackCount;
        ...
        for (int i = 0; i < count; i += width) {
            ...
            Vector* result;
            if (!this->innerRun(f, context, i, &result)) {
                return false;
            }
            int slot = 0;
            for (const auto& p : f->fParameters) {
            ...
            switch (inst) {
                ...
                LABEL(kPrint) {
                    if (skvx::any(mask())) {
                        ...
                        for (int i = 0; i < width; ++i) {
                            if (mask()[i]) {
                                printf("%s%f", separator, fRegisters[src.fIndex].fFloat[0]);
                            }
                            else {
                                printf("%s-", separator);
                            }
