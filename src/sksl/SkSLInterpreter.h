...
public:
    ...
    Vector* run(const ByteCodeFunction* f, Vector args[]) {
        ...
        stack -= f->fStackCount + f->fParameterCount;
        memcpy(stack, args, f->fParameterCount * sizeof(Vector));
...
private:
    ...
    void handleCall(const uint8_t** ip, Context context) {
        ...
        context.fStack -= f->fStackCount + f->fParameterCount;
        printf("fixme need to copy in parameters\n");
        Vector* v = this->innerRun(f, context);
        printf("fixme need to copy out result\n");
    ...
    Vector* innerRun(const ByteCodeFunction* f, Context context) {
        ...
        for (;;) {
            ...
            switch (inst) {
                ...
                BINARY_OP(kDivideS, fInt, fInt, /)
                BINARY_OP(kDivideU, fUInt, fUInt, /)
//                BINARY_OP(kRemainderS, fInt, fInt, %)
//                BINARY_OP(kRemainderU, fUInt, fUInt, %)
                ...
                case ByteCode::Instruction::kLoopMask: {
                    ...
                    *context.fLoopStack &= fRegisters[value.fIndex].fInt;
                    printf("loopMask %d(%d)\n", value.fIndex, *context.fLoopStack);
                    break;
                }
