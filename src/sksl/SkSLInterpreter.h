...
public:
    ...
    Vector* run(const ByteCodeFunction* f, Vector args[]) {
        ...
        Vector* result = this->innerRun(f, Context{stack, maskStack, loopStack, continueStack});
        int slot = 0;
        ...
        switch (inst) {
            ...
            case ByteCode::Instruction::kCall: {
                ...
                for (int i = 0; i < f->getParameterCount(); ++i) {
                    ...
                    printf("%s$%d", i ? ", " : "", arg.fIndex);
                }
                printf(")");
                if (hasReturn) {
                    printf(" -> $%d", result.fIndex);
                }
                printf("\n");
                break;
            }
