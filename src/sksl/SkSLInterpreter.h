...
public:
    ...
    Interpreter(std::unique_ptr<ByteCode> code)
        ...
        memset(fMemory, 0, sizeof(fRegisters));
    }
     ...
     */
     bool run(const ByteCodeFunction* f, Vector args[], const float* uniforms, Vector** outResult) {
        SkASSERT(f);
        ...
        Context context(fMemory, stack, maskStack, loopStack, continueStack);
        if (this->innerRun(f, context, outResult)) {
            int slot = 0;
    ...
    bool runStriped(const ByteCodeFunction* f, int count, Vector* args[], const float* uniforms,
                    Vector** outResult) {
        SkASSERT(f);
        SkASSERT(count % width == 0);
        Vector* stack = fMemory + MEMORY_SIZE;
        int stackCount = f->fStackSlotCount + f->fParameterSlotCount;
        stack -= stackCount;
        VectorI maskStack[MASK_STACK_SIZE];
        ...
        for (int i = 0; i < count; i += width) {
            ...
            for (int j = 0; j < f->fParameterSlotCount; ++j) {
                memcpy(stack + j, &args[j][i], sizeof(Vector));
            }
            Vector* result;
            if (!this->innerRun(f, context, &result)) {
                return false;
            }
            int slot = 0;
            for (const auto& p : f->fParameters) {
                if (p.fIsOutParameter) {
                    for (int j = 0; j < p.fSlotCount; ++j) {
                        memcpy(&args[slot + j][i], stack + slot + j, sizeof(Vector));
                    }
                }
                slot += p.fSlotCount;
            }
...
private:
    ...
    struct Context {
        Context(Vector* memory, Vector* stack, VectorI* maskStack, VectorI* loopStack,
    ...
    void disassemble(const uint8_t** ip) {
        ...
        switch (inst) {
            ...
            case ByteCode::Instruction::kBoundsCheck: {
                ...
                uint8_t length = read<uint8_t>(ip);
            ...
            case ByteCode::Instruction::kCallExternal: {
                ...
                ByteCodeFunction* f = fCode->fFunctions[idx].get();
                printf("callExternal %s($%d(%d)...) -> $%d(%d)", f->fName.c_str(), args.fIndex,
            ...
            case ByteCode::Instruction::kMatrixToMatrix: {
                ...
                printf("scalarToMatrix $%d, %dx%d to %dx%d -> $%d\n", src.fIndex, srcColumns,
    ...
    void handleCall(const uint8_t** ip, Context context) {
        ...
        bool hasReturn = f->fReturnSlotCount > 0;
        ByteCode::Register result;
        if (hasReturn) {
            result = read<ByteCode::Register>(ip);
        }
        ...
        currentArg = context.fStack;
        for (int i = 0; i < f->getParameterCount(); ++i) {
            const auto& p = f->getParameter(i);
            int count = p.fSlotCount;
            if (p.fIsOutParameter) {
                memcpy(&fRegisters[args[i].fIndex], currentArg, sizeof(Vector) * count);
            }
            currentArg += count;
        }
        if (hasReturn) {
    ...
    bool innerRun(const ByteCodeFunction* f, Context context, Vector** outResult) {
#ifdef SKSL_THREADED_CODE
        static const void* labels[] = {
            ...
            // corresponding CHECK_LABEL() or CHECK_*_LABELS() assert below.
            &&kNop,
            &&kAddF,
            &&kAddI,
            &&kAnd,
        ...
        for (;;) {
            ...
            switch (inst) {
                ...
                LABEL(kCall) {
                    uint8_t idx = read<uint8_t>(&ip);
                    ByteCode::Register parameters = read<ByteCode::Register>(&ip);
                    ...
                    context.fCallStack.push(StackFrame(f, ip, stackSlotCount,
                                                       &fRegisters[parameters.fIndex],
                                                       &fRegisters[returnValue.fIndex]));
                    f = target;
                    code = f->fCode.data();
                    ip = code;
                    context.fStack -= stackSlotCount;
                    memcpy(context.fStack, &fRegisters[parameters.fIndex],
                ...
                LABEL(kInverse2x2) {
                    ByteCode::Register target = read<ByteCode::Register>(&ip);
                    ByteCode::Register src = read<ByteCode::Register>(&ip);
                    memcpy(&fRegisters[target.fIndex], &fRegisters[src.fIndex], sizeof(Vector) * 4);
                ...
                LABEL(kInverse3x3) {
                    ByteCode::Register target = read<ByteCode::Register>(&ip);
                    ByteCode::Register src = read<ByteCode::Register>(&ip);
                    memcpy(&fRegisters[target.fIndex], &fRegisters[src.fIndex], sizeof(Vector) * 9);
                ...
                LABEL(kInverse4x4) {
                    ByteCode::Register target = read<ByteCode::Register>(&ip);
                    ByteCode::Register src = read<ByteCode::Register>(&ip);
                    memcpy(&fRegisters[target.fIndex], &fRegisters[src.fIndex], sizeof(Vector) * 16);
                ...
                LABEL(kReadExternal) {
                    ...
                    for (int i = 0; i < width; ++i) {
                        if (m[i]) {
                            fCode->fExternalValues[index]->read(i, tmp);
                ...
                LABEL(kWriteExternal) {
                    ...
                    for (int i = 0; i < width; ++i) {
                        if (m[i]) {
                            ...
                            fCode->fExternalValues[index]->write(i, tmp);
                        }
                    }
                    NEXT();
                }
