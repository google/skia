...
String CPPCodeGenerator::formatRuntimeValue(const Type& type,
    ...
    if (type == *fContext.fFloat4x4_Type || type == *fContext.fHalf4x4_Type) {
        for (int c=0; c<4; ++c) {
            for (int r=0; r<4; ++r) {
        ...
        return type.name() + "(%f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f)";
    }
...
void CPPCodeGenerator::writeRuntimeValue(const Type& type, const Layout& layout,
                                         const String& cppCode) {
    this->write(formatRuntimeValue(type, layout, cppCode, &fFormatArgs));
}
...
void CPPCodeGenerator::writeDumpInfo() {
    if (!this->writeSection(kDumpInfoSection)) {
        ...
        this->writef("SkString %s::dumpInfo() const {\n"
                     "    return SkStringPrintf(\"%s",
                     fFullName.c_str(), fName.c_str());

        String formatString;
        std::vector<String> argumentList;

        const std::vector<const Variable*>& params = fSectionAndParameterHelper.getParameters();
        if (!params.empty()) {
            for (const Variable* param : params) {
                ...
                String fieldName = HCodeGenerator::FieldName(String(param->fName).c_str());
                String runtimeValue = formatRuntimeValue(param->fType, param->fModifiers.fLayout,
