...
void CPPCodeGenerator::writeBinaryExpression(const BinaryExpression& b,
    ...
    if (op == Token::Kind::TK_PERCENT) {
        ...
        Operators::Precedence precedence = Operators::GetBinaryPrecedence(op);
...
void CPPCodeGenerator::writeVarInitializer(const Variable& var, const Expression& value) {
    if (is_private(var)) {
        ...
        this->writeExpression(value, Operators::kTopLevel_Precedence);
    }
}
...
void CPPCodeGenerator::writeSwizzle(const Swizzle& swizzle) {
    if (fCPPMode) {
        ...
        this->writeExpression(*swizzle.base(), Operators::kPostfix_Precedence);
...
void CPPCodeGenerator::writeFunctionCall(const FunctionCall& c) {
    ...
    if (function.isBuiltin()) {
        ...
        for (const auto& arg : arguments) {
            ...
            separator = ", ";
            this->writeExpression(*arg, Operators::kSequence_Precedence);
        }
        this->write(")");
    }
    if (function.isBuiltin() && function.name() == "sample") {
    ...
    for (const ProgramElement* p : fProgram.elements()) {
        if (p->is<GlobalVarDeclaration>()) {
            ...
            if (is_private(decl.var()) && decl.value()) {
                ...
                fCPPMode = true;
                this->writeExpression(*decl.value(), Operators::kAssignment_Precedence);
                fCPPMode = false;
                this->write(";\n");
            }
        }
...
String CPPCodeGenerator::convertSKSLExpressionToCPP(const Expression& e,
    ...
    this->writeExpression(e, Operators::kTopLevel_Precedence);
