...
void CPPCodeGenerator::writeFunctionCall(const FunctionCall& c) {
    if (c.fFunction.fBuiltin && c.fFunction.fName == "process") {
        ...
        if (c.fArguments.size() > 1) {
            // Second argument must also be a half4, and to keep things simple,
            // it must be a variable reference that's not sk_InColor or sk_OutColor
            SkASSERT("half4" == c.fArguments[1]->fType.name());
            if (c.fArguments[1]->fKind != Expression::kVariableReference_Kind) {
                fErrors.error(c.fArguments[1]->fOffset, "process()'s child input argument must be a variable reference");
                return;
            } else {
                String name = ((VariableReference&) *c.fArguments[1]).fVariable.fName;
                if (name == "sk_InColor" || name == "sk_OutColor") {
                    fErrors.error(c.fArguments[1]->fOffset, "sk_InColor and sk_OutColor are unsupported arguments to process()");
                    return;
                }
            }
        }
        ...
        if (c.fArguments.size() == 1) {
            ...
            String inputName = ((VariableReference&) *c.fArguments[1]).fVariable.fName;
