...
void CPPCodeGenerator::writeBinaryExpression(const Expression& b,
    ...
    if (op == Token::Kind::TK_PERCENT) {
        ...
        this->writeExpression(b.expressionChild(IRNode::Index::kBinaryLeft), precedence);
        this->write(" %% ");
        this->writeExpression(b.expressionChild(IRNode::Index::kBinaryRight), precedence);
        if (precedence >= parentPrecedence) {
            this->write(")");
        }
    } else if (b.expressionChild(IRNode::Index::kBinaryLeft).kind() ==
                                                                   Expression::Kind::kNullLiteral ||
               b.expressionChild(IRNode::Index::kBinaryRight).kind() ==
                                                                   Expression::Kind::kNullLiteral) {
        const Variable* var;
        if (b.expressionChild(IRNode::Index::kBinaryLeft).kind() !=
            Expression::Kind::kNullLiteral) {
            var = &b.expressionChild(IRNode::Index::kBinaryLeft).as<VariableReference>().fVariable;
        } else {
            var = &b.expressionChild(IRNode::Index::kBinaryRight).as<VariableReference>().fVariable;
        }
