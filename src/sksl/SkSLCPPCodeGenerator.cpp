    ...
    for (const auto& p : fProgram) {
        if (ProgramElement::kVar_Kind == p.fKind) {
            ...
            for (const auto& raw : decls.fVars) {
                ...
                } else if (is_uniform_in(*decl.fVar) &&
                           (decl.fVar->fModifiers.fLayout.fFlags & Layout::kTracked_Flag)) {
                    // An auto-tracked uniform in variable, so add a field
                    // to hold onto the prior state
    ...
    for (const auto u : uniforms) {
        if (is_uniform_in(*u)) {
            ...
            if (needsValueDeclaration) {
                ...
                this->writef("%sconst %s& %s = _outer.%s();\n", indent.c_str(),
                             mapper->ctype().c_str(), valueVar.c_str(), name);
            } else {
        ...
        if (ProgramElement::kVar_Kind == p.fKind) {
            ...
            for (const auto& raw : decls.fVars) {
                ...
                if (is_uniform_in(*decl.fVar)) {
                    // Validate the "uniform in" declarations to make sure they
                    // are fully supported, instead of generating surprising C++
                    const UniformCTypeMapper* mapper = UniformCTypeMapper::Get(fContext, *decl.fVar);
                    if (mapper == nullptr) {
