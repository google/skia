...
void CPPCodeGenerator::newExtraEmitCodeBlock() {
    ...
    // Mark its location in the output buffer, uses ${\d} for the token since
    // ${} will not occur in valid sksl and makes detection trivial.
    this->writef("${%lu}", fExtraEmitCodeBlocks.size() - 1);
}
...
void CPPCodeGenerator::flushEmittedCode() {
    ...
    while (i < sksl.size()) {
        if (tokenStart >= 0) {
            ...
            if (sksl[i] == '}') {
                ...
                // so fFormatArgs will be in a valid state for any future sksl
    ...
    // After appending, switch back to the emptied sksl buffer
    fOut = skslBuffer;
}
