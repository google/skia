...
String CPPCodeGenerator::getSamplerVarCode(const Variable& var) {
    int samplerCount = 0;
    for (const auto param : fSectionAndParameterHelper.fParameters) {
        if (&var == param) {
            return "fragBuilder->getProgramBuilder()->samplerVariable(args.fTexSamplers[" +
                   to_string(samplerCount) + "])";
        }
        if (param->fType.kind() == Type::kSampler_Kind) {
            ++samplerCount;
        }
    ...
    switch (ref.fVariable.fModifiers.fLayout.fBuiltin) {
        ...
        default:
            if (ref.fVariable.fType.kind() == Type::kSampler_Kind) {
                this->write("%s");
                fFormatArgs.push_back(this->getSamplerVarCode(ref.fVariable) + ".c_str()");
                return;
            }
