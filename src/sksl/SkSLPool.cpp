...
static thread_local GrMemoryPool* sMemPool = nullptr;

static GrMemoryPool* get_thread_local_memory_pool() {
    return sMemPool;
}

static void set_thread_local_memory_pool(GrMemoryPool* memPool) {
    sMemPool = memPool;
}
...
std::unique_ptr<Pool> Pool::Create() {
    ...
    if (sRecycledPool) {
        ...
        pool->fMemPool = GrMemoryPool::Make(/*preallocSize=*/65536, /*minAllocSize=*/16384);
...
void* Pool::AllocIRNode(size_t size) {
    // Is a pool attached?
    GrMemoryPool* memPool = get_thread_local_memory_pool();
    if (memPool) {
