...
static pthread_key_t get_pthread_key() {
    struct RunOnce {
        RunOnce() {
            int result = pthread_key_create(&fKey, /*destructor=*/nullptr);
            if (result != 0) {
    ...
    static RunOnce sRunOnce;
    return sRunOnce.fKey;
}
...
static thread_local PoolData* s_PoolData = nullptr;
