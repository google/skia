...
struct PoolData {
    ...
    IRNodeData* fFreeListHeadIfEnabled = &fNodes[0];

    // This tracks the enable state of the pool. (This is somewhat implied by fFreeListHead versus
    // fFreeListHeadIfEnabled, but imperfectly, as a fully drained pool will have nulls in both.)
    bool fEnabled = false;
...
void Pool::Enable() {
    ...
    pool.fEnabled = true;
    pool.fFreeListHead = pool.fFreeListHeadIfEnabled;
    pool.fFreeListHeadIfEnabled = nullptr;
...
void Pool::Disable() {
    ...
    pool.fEnabled = false;
    pool.fFreeListHeadIfEnabled = pool.fFreeListHead;
    pool.fFreeListHead = nullptr;
...
void Pool::FreeIRNode(void* node_v) {
    ...
    if (node >= &pool.fNodes[0] && node < &pool.fNodes[kPoolSize]) {
        ...
        PoolData::IRNodeData*& poolFreeList = pool.fEnabled ? pool.fFreeListHead
                                                            : pool.fFreeListHeadIfEnabled;
        node->fFreeListNext = poolFreeList;
        poolFreeList = node;
    } else {
