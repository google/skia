...
static void report_leaks(PoolData* data) {
    ptrdiff_t nodeCount = data->fNodesEnd - data->fNodes;
    std::vector<bool> freed(nodeCount);
    for (IRNodeData* node = data->fFreeListHead; node; node = node->fFreeListNext) {
        ptrdiff_t nodeIndex = node - data->fNodes;
        freed[nodeIndex] = true;
    }
    bool foundLeaks = false;
    for (int index = 0; index < nodeCount; ++index) {
        if (!freed[index]) {
            VLOG("Node %d leaked: %s\n", index,
                 reinterpret_cast<IRNode*>(data->fNodes[index].fBuffer)->description().c_str());
            foundLeaks = true;
        }
    }
    if (foundLeaks) {
        SkDEBUGFAIL("leaking SkSL pool nodes; if they are later freed, this will be fatal");
    }
}
...
Pool::~Pool() {
    ...
    if (fData->fFreeListHead != &fData->fNodes[0]) {
        SkDEBUGCODE(report_leaks(fData));
    }
