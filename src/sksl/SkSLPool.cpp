...
struct PoolData {
    struct IRNodeData {
        union {
            ...
            IRNodeData* fNextFreeNode;
        };
    };

    // This holds the first free node in the pool when pooling is enabled.
    // It will be null when pooling is disabled, or when the pool is exhausted.
    IRNodeData* fFreeList = nullptr;

    // This holds the first free node in the pool when pooling is disabled.
    IRNodeData* fFreeListIfEnabled = &fNodes[0];

    // This tracks the enable state of the pool. (This is somewhat implied by fFreeList versus
    // fFreeListIfEnabled, but imperfectly, as a fully drained pool will have nulls in both.)
    bool fEnabled = false;
    ...
    PoolData() {
        ...
        for (int index = 0; index < kPoolSize - 1; ++index) {
            fNodes[index].fNextFreeNode = &fNodes[index + 1];
        }

        fNodes[kPoolSize - 1].fNextFreeNode = nullptr;
    }
};

static thread_local PoolData sPoolData;
...
void Pool::Enable() {
    ...
    pool.fEnabled = true;
    pool.fFreeList = pool.fFreeListIfEnabled;
    pool.fFreeListIfEnabled = nullptr;
...
void Pool::Disable() {
    ...
    pool.fEnabled = false;
    pool.fFreeListIfEnabled = pool.fFreeList;
    pool.fFreeList = nullptr;
...
void* Pool::AllocIRNode() {
    ...
    PoolData::IRNodeData* node = pool.fFreeList;
    if (node) {
        pool.fFreeList = node->fNextFreeNode;
...
void Pool::FreeIRNode(void* node_v) {
    ...
    if (node >= &pool.fNodes[0] && node < &pool.fNodes[kPoolSize]) {
        ...
        PoolData::IRNodeData*& poolFreeList = pool.fEnabled ? pool.fFreeList
                                                            : pool.fFreeListIfEnabled;
        node->fNextFreeNode = poolFreeList;
        poolFreeList = node;
    } else {
