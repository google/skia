...
private:
    ...
    StringFragment readString() {
        uint8_t length = this->readU8();
        uint16_t offset = this->readU16();
        const char* chars = (const char*) fStart + offset;
    ...
    T* symbolRef(Symbol::Kind kind) {
        ...
        SkASSERT(fSymbolsById.find(result) != fSymbolsById.end());
        SkASSERT(fSymbolsById[result]->fKind == kind);
        return (T*) fSymbolsById[result];
    }
    ...
    std::map<uint16_t, const Symbol*> fSymbolsById;
