        ...
        case kGLSL_STD_450_IntrinsicKind: {
            ...
            for (size_t i = 0; i < arguments.size(); i++) {
                if (function.parameters()[i]->modifiers().fFlags & Modifiers::kOut_Flag) {
                    // TODO(skia:11052): use a temp variable if getPointer() fails
...
SpvId SPIRVCodeGenerator::writeFunctionCall(const FunctionCall& c, OutputStream& out) {
    ...
    // Temp variables are used to support out-parameters that match GLSL semantics.
    ...
    for (size_t i = 0; i < arguments.size(); i++) {
        ...
        if (is_out(*function.parameters()[i])) {
            // To preserve GLSL semantics for out parameters, the lvalue cannot just be read and
            // written via a pointer (and in fact, this isn't even possible for swizzled values).
            // Instead, we load the value into a temp, call the function with the temp, and copy
            // back to the original lvalue at the end.
