...
std::vector<SpvId> SPIRVCodeGenerator::vectorize(
    ...
    for (const auto& a : args) {
        ...
        if (vectorSize && a->type().typeKind() == Type::TypeKind::kScalar) {
            SpvId vector = this->nextId();
            this->writeOpCode(SpvOpCompositeConstruct, 3 + vectorSize, out);
            this->writeWord(this->getType(a->type().toCompound(fContext, vectorSize, 1)), out);
            this->writeWord(vector, out);
            for (int i = 0; i < vectorSize; i++) {
                this->writeWord(raw, out);
            }
            this->writePrecisionModifier(a->type(), vector);
...
SpvId SPIRVCodeGenerator::writeSpecialIntrinsic(const FunctionCall& c, SpecialIntrinsic kind,
    ...
    switch (kind) {
        ...
        case kSubpassLoad_SpecialIntrinsic: {
            ...
            args.emplace_back(new FloatLiteral(fContext, -1, 0.0));
            Constructor ctor(-1, fContext.fFloat2_Type.get(), std::move(args));
        ...
        case kTexture_SpecialIntrinsic: {
            ...
            switch (c.fArguments[0]->type().dimensions()) {
                case SpvDim1D:
                    if (c.fArguments[1]->type() == *fContext.fFloat2_Type) {
                        op = SpvOpImageSampleProjImplicitLod;
                    } else {
                        SkASSERT(c.fArguments[1]->type() == *fContext.fFloat_Type);
                    }
                    break;
                case SpvDim2D:
                    if (c.fArguments[1]->type() == *fContext.fFloat3_Type) {
                        op = SpvOpImageSampleProjImplicitLod;
                    } else {
                        SkASSERT(c.fArguments[1]->type() == *fContext.fFloat2_Type);
                    }
                    break;
                case SpvDim3D:
                    if (c.fArguments[1]->type() == *fContext.fFloat4_Type) {
                        op = SpvOpImageSampleProjImplicitLod;
                    } else {
                        SkASSERT(c.fArguments[1]->type() == *fContext.fFloat3_Type);
                    }
                    break;
            ...
            SpvId type = this->getType(c.type());
        ...
        case kDFdy_SpecialIntrinsic: {
            ...
            if (fProgram.fSettings.fFlipY) {
                ...
                this->writeInstruction(SpvOpFNegate, this->getType(c.type()), flipped, result, out);
                this->writePrecisionModifier(c.type(), flipped);
                return flipped;
            }
            break;
        }
        case kClamp_SpecialIntrinsic: {
            ...
            this->writeGLSLExtendedInstruction(c.type(), result, GLSLstd450FClamp, GLSLstd450SClamp,
        ...
        case kMax_SpecialIntrinsic: {
            std::vector<SpvId> args = this->vectorize(c.fArguments, out);
            SkASSERT(args.size() == 2);
            this->writeGLSLExtendedInstruction(c.type(), result, GLSLstd450FMax, GLSLstd450SMax,
        ...
        case kMin_SpecialIntrinsic: {
            std::vector<SpvId> args = this->vectorize(c.fArguments, out);
            SkASSERT(args.size() == 2);
            this->writeGLSLExtendedInstruction(c.type(), result, GLSLstd450FMin, GLSLstd450SMin,
        ...
        case kMix_SpecialIntrinsic: {
            std::vector<SpvId> args = this->vectorize(c.fArguments, out);
            SkASSERT(args.size() == 3);
            this->writeGLSLExtendedInstruction(c.type(), result, GLSLstd450FMix, SpvOpUndef,
                                               SpvOpUndef, args, out);
            break;
        }
...
SpvId SPIRVCodeGenerator::writeConstantVector(const Constructor& c) {
    SkASSERT(c.type().typeKind() == Type::TypeKind::kVector && c.isCompileTimeConstant());
    SpvId result = this->nextId();
    std::vector<SpvId> arguments;
    for (size_t i = 0; i < c.fArguments.size(); i++) {
    ...
    if (c.fArguments.size() == 1) {
        ...
        this->writeOpCode(SpvOpConstantComposite, 3 + c.type().columns(), fConstantBuffer);
...
SpvId SPIRVCodeGenerator::writeFloatConstructor(const Constructor& c, OutputStream& out) {
    ...
    SkASSERT(c.fArguments.size() == 1);
    SkASSERT(c.fArguments[0]->type().isNumber());
    SpvId result = this->nextId();
    SpvId parameter = this->writeExpression(*c.fArguments[0], out);
    if (c.fArguments[0]->type().isSigned()) {
        this->writeInstruction(SpvOpConvertSToF, this->getType(c.type()), result, parameter,
                               out);
    } else {
        SkASSERT(c.fArguments[0]->type().isUnsigned());
        this->writeInstruction(SpvOpConvertUToF, this->getType(c.type()), result, parameter,
                               out);
    }
    return result;
}
...
SpvId SPIRVCodeGenerator::writeIntConstructor(const Constructor& c, OutputStream& out) {
    ...
    SpvId result = this->nextId();
    SpvId parameter = this->writeExpression(*c.fArguments[0], out);
    if (c.fArguments[0]->type().isFloat()) {
        this->writeInstruction(SpvOpConvertFToS, this->getType(c.type()), result, parameter,
                               out);
    }
    else {
        SkASSERT(c.fArguments[0]->type().isUnsigned());
        this->writeInstruction(SpvOpBitcast, this->getType(c.type()), result, parameter,
                               out);
    }
    return result;
}
...
SpvId SPIRVCodeGenerator::writeUIntConstructor(const Constructor& c, OutputStream& out) {
    ...
    SpvId parameter = this->writeExpression(*c.fArguments[0], out);
    if (c.fArguments[0]->type().isFloat()) {
        this->writeInstruction(SpvOpConvertFToU, this->getType(c.type()), result, parameter,
                               out);
    } else {
        SkASSERT(c.fArguments[0]->type().isSigned());
        this->writeInstruction(SpvOpBitcast, this->getType(c.type()), result, parameter,
                               out);
    }
    return result;
...
SpvId SPIRVCodeGenerator::writeMatrixConstructor(const Constructor& c, OutputStream& out) {
    SkASSERT(c.type().typeKind() == Type::TypeKind::kMatrix);
    ...
    SpvId result = this->nextId();
    int rows = c.type().rows();
    int columns = c.type().columns();
    if (arguments.size() == 1 && c.fArguments[0]->type().typeKind() == Type::TypeKind::kScalar) {
        this->writeUniformScaleMatrix(result, arguments[0], c.type(), out);
    } else if (arguments.size() == 1 &&
               c.fArguments[0]->type().typeKind() == Type::TypeKind::kMatrix) {
        this->writeMatrixCopy(result, arguments[0], c.fArguments[0]->type(), c.type(), out);
    } else if (arguments.size() == 1 &&
               c.fArguments[0]->type().typeKind() == Type::TypeKind::kVector) {
        SkASSERT(c.type().rows() == 2 && c.type().columns() == 2);
        SkASSERT(c.fArguments[0]->type().columns() == 4);
        ...
        SpvId columnType = this->getType(c.type().componentType().toCompound(fContext, 2, 1));
        ...
        this->writeInstruction(SpvOpCompositeConstruct, this->getType(c.type()), result, column1,
                               column2, out);
    } else {
        SpvId columnType = this->getType(c.type().componentType().toCompound(fContext, rows, 1));
        std::vector<SpvId> columnIds;
        ...
        int currentCount = 0;
        Precision precision = c.type().highPrecision() ? Precision::kHigh : Precision::kLow;
        for (size_t i = 0; i < arguments.size(); i++) {
            if (currentCount == 0 &&
                c.fArguments[i]->type().typeKind() == Type::TypeKind::kVector &&
                c.fArguments[i]->type().columns() == c.type().rows()) {
                // this is a complete column by itself
                columnIds.push_back(arguments[i]);
            } else {
                if (c.fArguments[i]->type().columns() == 1) {
                    this->addColumnEntry(columnType, precision, &currentColumn, &columnIds,
                                         &currentCount, rows, arguments[i], out);
                } else {
                    SpvId componentType = this->getType(c.fArguments[i]->type().componentType());
                    for (int j = 0; j < c.fArguments[i]->type().columns(); ++j) {
    ...
    this->writePrecisionModifier(c.type(), result);
    return result;
}

SpvId SPIRVCodeGenerator::writeVectorConstructor(const Constructor& c, OutputStream& out) {
    SkASSERT(c.type().typeKind() == Type::TypeKind::kVector);
    ...
    std::vector<SpvId> arguments;
    for (size_t i = 0; i < c.fArguments.size(); i++) {
        if (c.fArguments[i]->type().typeKind() == Type::TypeKind::kVector) {
            ...
            const Type& src = c.fArguments[i]->type().componentType();
            const Type& dst = c.type().componentType();
...
SpvId SPIRVCodeGenerator::writeArrayConstructor(const Constructor& c, OutputStream& out) {
    SkASSERT(c.type().typeKind() == Type::TypeKind::kArray);
    // go ahead and write the arguments so we don't try to write new instructions in the middle of
    // an instruction
    std::vector<SpvId> arguments;
    for (size_t i = 0; i < c.fArguments.size(); i++) {
...
std::unique_ptr<SPIRVCodeGenerator::LValue> SPIRVCodeGenerator::getLValue(const Expression& expr,
    ...
    Precision precision = exprType.highPrecision() ? Precision::kHigh : Precision::kLow;
    switch (expr.kind()) {
        case Expression::Kind::kVariableReference: {
            SpvId type;
            const Variable& var = ((VariableReference&) expr).fVariable;
            if (var.fModifiers.fLayout.fBuiltin == SK_IN_BUILTIN) {
                type = this->getType(Type("sk_in", Type::TypeKind::kArray, var.fType.componentType(),
                                          fSkInCount));
            } else {
                type = this->getType(exprType);
            }
        ...
        case Expression::Kind::kFieldAccess: {
            ...
            this->writeWord(this->getPointerType(exprType, get_storage_class(expr)), out);
        ...
        default:
            ...
            SpvId result = this->nextId();
            SpvId type = this->getPointerType(exprType, SpvStorageClassFunction);
    ...
    if (count == 1) {
        ...
        for (int component : swizzle.fComponents) {
            if (component == SKSL_SWIZZLE_0) {
                this->writeWord(swizzle.fBase->type().columns(), out);
            } else if (component == SKSL_SWIZZLE_1) {
                this->writeWord(swizzle.fBase->type().columns() + 1, out);
            } else {
...
SpvId SPIRVCodeGenerator::writeBinaryExpression(const BinaryExpression& b, OutputStream& out) {
    ...
    SpvId rhs = this->writeExpression(right, out);
    SpvId result = this->writeBinaryExpression(left.type(), lhs, remove_assignment(op),
...
SpvId SPIRVCodeGenerator::writeTernaryExpression(const TernaryExpression& t, OutputStream& out) {
    ...
    if (t.fIfTrue->type().columns() == 1 &&
        ...
        this->writeInstruction(SpvOpSelect, this->getType(t.type()), result, test, trueId, falseId,
                               out);
        return result;
    }
    ...
    this->writeInstruction(SpvOpVariable, this->getPointerType(t.type(), SpvStorageClassFunction),
    ...
    this->writeLabel(end, out);
    SpvId result = this->nextId();
    this->writeInstruction(SpvOpLoad, this->getType(t.type()), result, var, out);
    this->writePrecisionModifier(t.type(), result);
    return result;
}
...
SpvId SPIRVCodeGenerator::writePostfixExpression(const PostfixExpression& p, OutputStream& out) {
    ...
    switch (p.fOperator) {
        case Token::Kind::TK_PLUSPLUS: {
            SpvId temp = this->writeBinaryOperation(type, type, result, one, SpvOpFAdd, SpvOpIAdd,
                                                    SpvOpIAdd, SpvOpUndef, out);
            lv->store(temp, out);
            return result;
        }
        case Token::Kind::TK_MINUSMINUS: {
            SpvId temp = this->writeBinaryOperation(type, type, result, one, SpvOpFSub, SpvOpISub,
...
SpvId SPIRVCodeGenerator::writeFloatLiteral(const FloatLiteral& f) {
    ...
    if (entry == fNumberConstants.end()) {
        ...
        this->writeInstruction(SpvOpConstant, this->getType(f.type()), result, bits,
                               fConstantBuffer);
        fNumberConstants[key] = result;
        return result;
    }
