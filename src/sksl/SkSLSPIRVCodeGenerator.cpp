...
void SPIRVCodeGenerator::writeMatrixCopy(SpvId id, SpvId src, const Type& srcType,
    ...
    ABORT("matrix copy unimplemented");
}
...
void SPIRVCodeGenerator::writeGlobalVars(Program::Kind kind, const VarDeclarations& decl,
    ...
    for (size_t i = 0; i < decl.fVars.size(); i++) {
        ...
        if (var->fType.kind() == Type::kMatrix_Kind) {
            this->writeInstruction(SpvOpDecorate, id, SpvDecorationColMajor, fDecorationBuffer);
            this->writeInstruction(SpvOpDecorate, id, SpvDecorationMatrixStride,
