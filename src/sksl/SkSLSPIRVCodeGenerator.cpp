...
static bool is_top_level_uniform(const Variable& variable) {
    return variable.storage() == Variable::Storage::kGlobal &&
           !variable.type().componentType().isStruct() &&
...
int SPIRVCodeGenerator::findUniformFieldIndex(const Variable& var) const {
    for (int index = 0; index < (int)fTopLevelUniforms.size(); ++index) {
        if (&fTopLevelUniforms[index]->var() == &var) {
            return index;
        }
    }
    SkDEBUGFAILF("unable to find top-level uniform for '%s'", var.description().c_str());
    return -1;
}
    ...
    switch (expr.kind()) {
        case Expression::Kind::kVariableReference: {
            ...
            if (is_top_level_uniform(var)) {
                ...
                fErrors.error(expr.fOffset, "unrecognized uniform '" + expr.description() + "'");
            }
            SpvId typeId;
...
SpvId SPIRVCodeGenerator::writeVariableReference(const VariableReference& ref, OutputStream& out) {
    ...
    if (is_top_level_uniform(*variable)) {
