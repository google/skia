...
void SPIRVCodeGenerator::writeGlobalVars(Program::Kind kind, const VarDeclarations& decl,
    ...
    for (size_t i = 0; i < decl.fVars.size(); i++) {
        ...
        SkASSERT(!(modifiers.fFlags & (Modifiers::kReadOnly_Flag |
                                       Modifiers::kWriteOnly_Flag |
                                       Modifiers::kCoherent_Flag |
                                       Modifiers::kVolatile_Flag |
                                       Modifiers::kRestrict_Flag)));
        if (modifiers.fLayout.fBuiltin == BUILTIN_IGNORE) {
            continue;
        }
        if (modifiers.fLayout.fBuiltin == SK_FRAGCOLOR_BUILTIN &&
        ...
        fVariableMap[var] = id;
        SpvId typeId;
        if (modifiers.fLayout.fBuiltin == SK_IN_BUILTIN) {
        ...
        this->writeLayout(modifiers.fLayout, id);
