...
SPIRVCodeGenerator::UniformBuffer SPIRVCodeGenerator::writeUniformBuffer(
    ...
    uniformBuffer.fVariable = std::make_unique<Variable>(
            /*offset=*/-1, fProgram.fModifiers->addToPool(uniformBuffer.fModifiers),
            "_UniformBuffer", uniformBuffer.fStruct.get(), /*builtin=*/false,
            Variable::Storage::kGlobal);

    // Create an interface block object for this global variable.
    uniformBuffer.fInterfaceBlock = std::make_unique<InterfaceBlock>(
            /*offset=*/-1, uniformBuffer.fVariable.get(), kUniformBufferName, kUniformBufferName,
            /*arraySize=*/0, topLevelSymbolTable);

    // Emit the new interface block.
    SpvId interfaceBlockId = this->writeInterfaceBlock(*uniformBuffer.fInterfaceBlock);
    (void) interfaceBlockId;
