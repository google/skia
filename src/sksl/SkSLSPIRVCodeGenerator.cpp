...
SpvId SPIRVCodeGenerator::getType(const Type& type, const MemoryLayout& layout) {
    ...
    if (type.kind() == Type::kMatrix_Kind || type.kind() == Type::kVector_Kind) {
        if (type == *fContext.fHalf_Type) {
            return this->getType(fContext.fFloat_Type->toCompound(fContext, type.columns(),
                                                                  type.rows()));
        }
        if (type == *fContext.fShort_Type) {
            return this->getType(fContext.fInt_Type->toCompound(fContext, type.columns(),
                                                                type.rows()));
        }
        if (type == *fContext.fUShort_Type) {
            return this->getType(fContext.fUInt_Type->toCompound(fContext, type.columns(),
                                                                 type.rows()));
        }
    }
