...
SpvId SPIRVCodeGenerator::writeVariableReference(const VariableReference& ref, OutputStream& out) {
    ...
    if (ref.variable()->modifiers().fLayout.fBuiltin == SK_FRAGCOORD_BUILTIN &&
        ...
        this->writeInstruction(SpvOpCompositeExtract, this->getType(*fContext.fTypes.fFloat), rawYId,
                               result, 1, out);
        SpvId flippedYId = 0;
        if (fProgram.fSettings.fFlipY) {
        ...
        this->writeInstruction(SpvOpCompositeExtract, this->getType(*fContext.fTypes.fFloat), rawWId,
                               result, 3, out);
