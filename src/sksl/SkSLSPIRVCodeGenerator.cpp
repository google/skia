...
SpvId SPIRVCodeGenerator::writeIntrinsicCall(const FunctionCall& c, OutputStream& out) {
    ...
    const SkTArray<std::unique_ptr<Expression>>& arguments = c.arguments();
...
std::vector<SpvId> SPIRVCodeGenerator::vectorize(const SkTArray<std::unique_ptr<Expression>>& args,
                                                 OutputStream& out) {
    int vectorSize = 0;
...
SpvId SPIRVCodeGenerator::writeSpecialIntrinsic(const FunctionCall& c, SpecialIntrinsic kind,
    ...
    switch (kind) {
        ...
        case kSaturate_SpecialIntrinsic: {
            ...
            SkTArray<std::unique_ptr<Expression>> finalArgs;
            finalArgs.reserve(3);
