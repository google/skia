...
SpvId SPIRVCodeGenerator::writeExpression(const Expression& expr, OutputStream& out) {
    switch (expr.kind()) {
        case Expression::Kind::kBinary:
            return this->writeBinaryExpression(expr, out);
...
SpvId SPIRVCodeGenerator::writeMatrixConstructor(const Constructor& c, OutputStream& out) {
    ...
    if (arguments.size() == 1 && c.fArguments[0]->fType.typeKind() == Type::TypeKind::kScalar) {
        ...
        for (size_t i = 0; i < arguments.size(); i++) {
            if (currentCount == 0 && c.fArguments[i]->fType.typeKind() == Type::TypeKind::kVector &&
...
SpvId SPIRVCodeGenerator::writeBinaryExpression(const Expression& b, OutputStream& out) {
    ...
    SpvId rhs = this->writeExpression(right, out);
    SpvId result = this->writeBinaryExpression(left.fType, lhs, remove_assignment(op), right.fType,
                                               rhs, b.fType, out);
    if (lvalue) {
...
SpvId SPIRVCodeGenerator::writeLogicalAnd(const Expression& a, OutputStream& out) {
    SkASSERT(a.getBinaryData().fOperator == Token::Kind::TK_LOGICALAND);
    BoolLiteral falseLiteral(fContext, -1, false);
    SpvId falseConstant = this->writeBoolLiteral(falseLiteral);
    SpvId lhs = this->writeExpression(a.expressionChild(IRNode::Index::kBinaryLeft), out);
    ...
    SpvId rhs = this->writeExpression(a.expressionChild(IRNode::Index::kBinaryRight), out);
...
SpvId SPIRVCodeGenerator::writeLogicalOr(const Expression& o, OutputStream& out) {
    SkASSERT(o.getBinaryData().fOperator == Token::Kind::TK_LOGICALOR);
    BoolLiteral trueLiteral(fContext, -1, true);
    SpvId trueConstant = this->writeBoolLiteral(trueLiteral);
    SpvId lhs = this->writeExpression(o.expressionChild(IRNode::Index::kBinaryLeft), out);
    SpvId rhsLabel = this->nextId();
    SpvId end = this->nextId();
    SpvId lhsBlock = fCurrentBlock;
    this->writeInstruction(SpvOpSelectionMerge, end, SpvSelectionControlMaskNone, out);
    ...
    this->writeLabel(rhsLabel, out);
    SpvId rhs = this->writeExpression(o.expressionChild(IRNode::Index::kBinaryRight), out);
    SpvId rhsBlock = fCurrentBlock;
    this->writeInstruction(SpvOpBranch, end, out);
    this->writeLabel(end, out);
    SpvId result = this->nextId();
