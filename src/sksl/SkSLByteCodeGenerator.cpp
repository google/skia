...
static int slot_count(const Type& type, bool allowCompund = false) {
    if (allowCompund) {
        if (type.kind() == Type::kStruct_Kind) {
            ...
            SkASSERT(columns >= 0);
            int slots = columns * slot_count(type.componentType(), true);
            SkASSERT(slots <= 255);
            return slots;
        }
    }
            ...
            case ProgramElement::kVar_Kind: {
                ...
                for (const auto& v : decl.fVars) {
                    ...
                    if (declVar->fModifiers.fFlags & Modifiers::kIn_Flag) {
                        for (int i = slot_count(declVar->fType, true); i > 0; --i) {
                            fOutput->fInputSlots.push_back(fOutput->fGlobalCount++);
                        }
                    } else {
                        fOutput->fGlobalCount += slot_count(declVar->fType, true);
                    }
                }
                break;
            }
...
std::unique_ptr<ByteCodeFunction> ByteCodeGenerator::writeFunction(const FunctionDefinition& f) {
    ...
    if (returnType != *fContext.fVoid_Type) {
        result->fReturnCount = slot_count(returnType, true);
    }
    fLocals.clear();
    fFunction = nullptr;
    return result;
    ...
    switch (var.fStorage) {
        ...
        case Variable::kParameter_Storage: {
            ...
            for (const auto& p : fFunction->fDeclaration.fParameters) {
                ...
                offset += slot_count(p->fType, true);
            }
            SkASSERT(false);
            return 0;
        }
            ...
            for (const auto& e : fProgram) {
                if (e.fKind == ProgramElement::kVar_Kind) {
                    ...
                    for (const auto& v : decl.fVars) {
                        ...
                        offset += slot_count(declVar->fType, true);
                    }
                }
            }
            SkASSERT(false);
...
void ByteCodeGenerator::writeFieldAccess(const FieldAccess& f) {
    ...
    this->write8(slot_count(f.fType, true));
}
...
void ByteCodeGenerator::writeIndexExpression(const IndexExpression& i) {
    ...
    this->write(storage == Variable::kGlobal_Storage ? ByteCodeInstruction::kLoadExtendedGlobal
                                                     : ByteCodeInstruction::kLoadExtended);
    this->write8(slot_count(i.fType, true));
}
...
void ByteCodeGenerator::writeVariableReference(const VariableReference& v) {
    ...
    int count = slot_count(v.fType, true);
    if (count > 4) {
        this->write(ByteCodeInstruction::kPushImmediate);
...
public:
    ...
    void store() override {
        ...
        int count = slot_count(fRef.fType, true);
...
public:
    ...
    void store() override {
        ...
        int count = slot_count(fIndex.fType, true);
        bool isGlobal = storage == Variable::kGlobal_Storage;
        if (count > 4) {
            fGenerator.write(ByteCodeInstruction::kDupN);
            fGenerator.write8(count);
...
void ByteCodeGenerator::writeReturnStatement(const ReturnStatement& r) {
    ...
    this->write(ByteCodeInstruction::kReturn);
    this->write8(slot_count(r.fExpression->fType, true));
}
    ...
    for (const auto& declStatement : v.fVars) {
        ...
        if (decl.fValue) {
            ...
            this->write(vector_instruction(ByteCodeInstruction::kStore,
                                           slot_count(decl.fValue->fType))); // , true ?
...
void ByteCodeGenerator::writeStatement(const Statement& s) {
    switch (s.fKind) {
        ...
        case Statement::kExpression_Kind: {
            ...
            // , true ??? Yes, if all expressions are allowed to return compound types too.
            // Going to need that for assignment of structs?
