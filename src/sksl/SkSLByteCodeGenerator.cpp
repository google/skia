...
bool ByteCodeGenerator::writeBinaryExpression(const Expression& b, bool discard) {
    IRNode::BinaryData binaryData = b.getBinaryData();
    Token::Kind op = binaryData.fOperator;
    if (op == Token::Kind::TK_EQ) {
        std::unique_ptr<LValue> lvalue = this->getLValue(b.expressionChild(
                                                                       IRNode::Index::kBinaryLeft));
        this->writeExpression(b.expressionChild(IRNode::Index::kBinaryRight));
        lvalue->store(discard);
        discard = false;
        return discard;
    }
    const Type& lType = b.expressionChild(IRNode::Index::kBinaryLeft).fType;
    const Type& rType = b.expressionChild(IRNode::Index::kBinaryRight).fType;
    ...
    if (is_assignment(b.getBinaryData().fOperator)) {
        lvalue = this->getLValue(b.expressionChild(IRNode::Index::kBinaryLeft));
        lvalue->load();
        op = remove_assignment(op);
    } else {
        this->writeExpression(b.expressionChild(IRNode::Index::kBinaryLeft));
    ...
    switch (op) {
        ...
        case Token::Kind::TK_SHR: {
            ...
            if (!b.expressionChild(IRNode::Index::kBinaryRight).isCompileTimeConstant()) {
                fErrors.error(b.expressionChild(IRNode::Index::kBinaryRight).fOffset,
                              "Shift amounts must be constant");
                return false;
            }
            int64_t shift = b.expressionChild(IRNode::Index::kBinaryRight).getConstantInt();
            if (shift < 0 || shift > 31) {
                fErrors.error(b.expressionChild(IRNode::Index::kBinaryRight).fOffset,
    ...
    if (op == Token::Kind::TK_STAR && lVecOrMtx && rVecOrMtx &&
        ...
        this->write(ByteCodeInstruction::kMatrixMultiply,
                    SlotCount(*binaryData.fType) - (SlotCount(lType) + SlotCount(rType)));
        ...
        SkASSERT(SlotCount(*binaryData.fType) == lRows * rCols);
        this->write8(lCols);
        this->write8(lRows);
        this->write8(rCols);
    } else {
...
void ByteCodeGenerator::writeExpression(const Expression& e, bool discard) {
    switch (e.kind()) {
        case Expression::Kind::kBinary:
            discard = this->writeBinaryExpression(e, discard);
            break;
