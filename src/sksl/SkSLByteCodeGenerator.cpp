...
ByteCodeGenerator::Location ByteCodeGenerator::getLocation(const Variable& var) {
    ...
    switch (var.fStorage) {
        ...
        case Variable::kGlobal_Storage: {
            ...
            uint16_t offset = isUniform ? fOutput->getGlobalSlotCount() : 0;
...
void ByteCodeGenerator::writeBinaryExpression(const BinaryExpression& b,
    ...
    switch (b.fOperator) {
        ...
        case Token::Kind::LOGICALOREQ:
            ...
            break;
        default:
            left = this->next(SlotCount(b.fLeft->fType));
    }
    ...
    SkDEBUGCODE(int count = std::max(SlotCount(lType), SlotCount(rType)));
    switch (op) {
        case Token::Kind::LOGICALAND: {
    ...
    ByteCode::Register right = this->next(SlotCount(lType));
    this->writeExpression(*b.fRight, right);
    if (lVecOrMtx && !rVecOrMtx) {
        for (int i = 1; i < SlotCount(lType); ++i) {
    ...
    switch (op) {
        case Token::Kind::EQEQ:
            this->writeBinaryInstruction(lType, left, right,
...
void ByteCodeGenerator::incOrDec(Token::Kind op, Expression& operand, bool prefix,
    ...
    int count = SlotCount(operand.fType);
    ByteCode::Register value;
    if (prefix) {
        value = this->next(count);
    } else {
        value = result;
    }
    lvalue->load(value);
    ByteCode::Register one = this->next(count);
    this->write(ByteCode::Instruction::kImmediate);
    this->write(one);
    ...
    if (prefix) {
        ...
        ByteCode::Register temp = this->next(count);
        this->write(temp);
        this->write(value);
        this->write(one);
        lvalue->store(temp);
