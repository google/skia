        ...
        case Variable::kGlobal_Storage: {
            ...
            for (IRNode::ID e : fProgram) {
                if (((ProgramElement&) e.node()).fKind == ProgramElement::kVar_Kind) {
                    VarDeclarations& decl = (VarDeclarations&) e.node();
    ...
    switch (expr.fKind) {
        ...
        case Expression::kIndex_Kind: {
            ...
            if (i.fIndex.node().isConstant()) {
                ...
                if (i.fIndex.expression().hasSideEffects()) {
                ...
                this->writeExpression(i.fIndex.expression());
                if (stride != 1) {
                    this->write(ByteCodeInstruction::kPushImmediate);
                    this->write32(stride);
            ...
            int baseAddr = this->getLocation(i.fBase.expression(), storage);
        ...
        switch (found->second.fValue.fInstruction) {
            ...
            case ByteCodeInstruction::kTan:
                ...
                this->write((ByteCodeInstruction) ((int) found->second.fValue.fInstruction +
                            SlotCount(c.fArguments[0].expression().fType.typeNode()) - 1));
                break;
...
void ByteCodeGenerator::writeFunctionCall(const FunctionCall& f) {
    ...
    if (fn.fBuiltin) {
        for (IRNode::ID arg : f.fArguments) {
            this->writeExpression(arg.expression());
        }
    ...
    for (int i = 0; i < argCount; ++i) {
        ...
        const Expression& arg = f.fArguments[i].expression();
    ...
    for (int i = argCount - 1; i >= 0; --i) {
        ...
        Expression& arg = f.fArguments[i].expression();
        if (param.fModifiers.fFlags & Modifiers::kOut_Flag) {
            pop();
...
void ByteCodeGenerator::writeTernaryExpression(const TernaryExpression& t) {
    this->writeExpression(t.fTest.expression());
    this->write(ByteCodeInstruction::kMaskPush);
    this->writeExpression(t.fIfTrue.expression());
    this->write(ByteCodeInstruction::kMaskNegate);
    this->writeExpression(t.fIfFalse.expression());
...
public:
    ...
    void store(bool discard) override {
        ...
        Variable::Storage storage;
        int location = fGenerator.getLocation(fSwizzle.fBase.expression(), &storage);
        bool isGlobal = storage == Variable::kGlobal_Storage;
        if (location < 0) {
...
void ByteCodeGenerator::writeDoStatement(const DoStatement& d) {
    ...
    this->writeStatement(d.fStatement.statement());
    this->write(ByteCodeInstruction::kLoopNext);
    this->writeExpression(d.fTest.expression());
...
void ByteCodeGenerator::writeForStatement(const ForStatement& f) {
    ...
    size_t start = fCode->size();
    if (f.fTest) {
        this->writeExpression(f.fTest.expression());
        this->write(ByteCodeInstruction::kLoopMask);
    }
    this->write(ByteCodeInstruction::kBranchIfAllFalse);
    DeferredLocation endLocation(this);
    this->writeStatement(f.fStatement.statement());
    this->write(ByteCodeInstruction::kLoopNext);
    if (f.fNextExpression) {
        this->writeExpression(f.fNextExpression.expression(), true);
    }
    this->write(ByteCodeInstruction::kBranch);
    this->write16(start);
    endLocation.set();
...
void ByteCodeGenerator::writeIfStatement(const IfStatement& i) {
    this->writeExpression(i.fTest.expression());
    this->write(ByteCodeInstruction::kMaskPush);
    this->write(ByteCodeInstruction::kBranchIfAllFalse);
    DeferredLocation falseLocation(this);
    this->writeStatement(i.fIfTrue.statement());
    falseLocation.set();
    if (i.fIfFalse) {
        this->write(ByteCodeInstruction::kMaskNegate);
        this->write(ByteCodeInstruction::kBranchIfAllFalse);
        DeferredLocation endLocation(this);
        this->writeStatement(i.fIfFalse.statement());
        endLocation.set();
    }
...
void ByteCodeGenerator::writeReturnStatement(const ReturnStatement& r) {
    this->writeExpression(r.fExpression.expression());
    this->write(ByteCodeInstruction::kReturn);
    this->write8(SlotCount(r.fExpression.expression().fType.typeNode()));
}
...
void ByteCodeGenerator::writeVarDeclarations(const VarDeclarations& v) {
    for (IRNode::ID declStatement : v.fVars) {
        ...
        if (decl.fValue) {
            this->writeExpression(decl.fValue.expression());
            int count = SlotCount(decl.fValue.expression().fType.typeNode());
            if (count > 4) {
                this->write(ByteCodeInstruction::kPushImmediate);
                this->write32(location);
...
void ByteCodeGenerator::writeWhileStatement(const WhileStatement& w) {
    ...
    this->writeExpression(w.fTest.expression());
    this->write(ByteCodeInstruction::kLoopMask);
    this->write(ByteCodeInstruction::kBranchIfAllFalse);
    DeferredLocation endLocation(this);
    this->writeStatement(w.fStatement.statement());
    this->write(ByteCodeInstruction::kLoopNext);
    this->write(ByteCodeInstruction::kBranch);
    this->write16(cond);
    endLocation.set();
...
void ByteCodeGenerator::writeStatement(const Statement& s) {
    switch (s.fKind) {
        ...
        case Statement::kExpression_Kind:
            this->writeExpression(((ExpressionStatement&) s).fExpression.expression(), true);
            break;
