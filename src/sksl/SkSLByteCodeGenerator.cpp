        ...
        switch (e.fKind) {
            ...
            case ProgramElement::kVar_Kind: {
                ...
                for (const auto& v : decl.fVars) {
                    ...
                    fOutput->fGlobalCount += declVar->fType.columns() * declVar->fType.rows();
                }
                break;
            }
            default:
    ...
    if (c.fArguments.size() == 1) {
        ...
        if (inCategory != outCategory) {
            if (inCategory == TypeCategory::kFloat) {
                ...
                SkASSERT(inCategory == TypeCategory::kSigned ||
                         inCategory == TypeCategory::kUnsigned);
                this->write(ByteCodeInstruction::kIntToFloat);
            }
...
void ByteCodeGenerator::writeFloatLiteral(const FloatLiteral& f) {
    this->write(ByteCodeInstruction::kPushImmediate);
    float value = f.fValue;
    this->write32(*(int32_t*) &value);
}
...
class SwizzleLValue : public ByteCodeGenerator::LValue {
public:
    SwizzleLValue(ByteCodeGenerator* generator, const Swizzle& swizzle)
        : INHERITED(*generator)
        , fSwizzle(swizzle) {
...
class VariableLValue : public ByteCodeGenerator::LValue {
public:
    VariableLValue(ByteCodeGenerator* generator, const Variable& var)
        : INHERITED(*generator)
    ...
    void load() override {
        ...
        fGenerator.write(ByteCodeInstruction::kLoad);
    }
    ...
    void store() override {
        ...
        fGenerator.write(ByteCodeInstruction::kStore);
    }
...
std::unique_ptr<ByteCodeGenerator::LValue> ByteCodeGenerator::getLValue(const Expression& e) {
    switch (e.fKind) {
        case Expression::kIndex_Kind:
            abort();
        case Expression::kVariableReference_Kind:
            return std::unique_ptr<LValue>(new VariableLValue(this,
                                                              ((VariableReference&) e).fVariable));
        case Expression::kSwizzle_Kind:
            return std::unique_ptr<LValue>(new SwizzleLValue(this, (Swizzle&) e));
        case Expression::kTernary_Kind:
        default:
