...
static int stack_delta(ByteCodeInstruction inst, int count) {
    switch (inst) {
...
int ByteCodeGenerator::getLocation(const Expression& expr, Variable::Storage* storage) {
    switch (expr.fKind) {
        case Expression::kFieldAccess_Kind: {
            ...
            if (baseAddr < 0) {
                if (offset != 0) {
                    this->write(ByteCodeInstruction::kPushImmediate, 1);
                    this->write32(offset);
...
void ByteCodeGenerator::write(ByteCodeInstruction i, int count) {
    ...
    fStackCount += stack_delta(i, count);
...
void ByteCodeGenerator::writeTypedInstruction(const Type& type, ByteCodeInstruction s,
    ...
    switch (type_category(type)) {
        case TypeCategory::kSigned:
            this->write(vector_instruction(s, count), count);
            break;
        case TypeCategory::kUnsigned:
            this->write(vector_instruction(u, count), count);
            break;
        case TypeCategory::kFloat: {
            if (count > 4) {
                ...
                this->write(vector_instruction(f, count), count);
            }
            break;
        }
        default:
    ...
    if (op == Token::Kind::STAR && lVecOrMtx && rVecOrMtx &&
        ...
        switch (op) {
            ...
            case Token::Kind::LOGICALXOR:
                SkASSERT(type_category(lType) == SkSL::TypeCategory::kBool && count == 1);
                this->write(ByteCodeInstruction::kXorB, 1);
                break;
...
void ByteCodeGenerator::writeExternalValue(const ExternalValueReference& e) {
    ...
    this->write(vector_instruction(ByteCodeInstruction::kReadExternal, count), count);
    int index = fOutput->fExternalValues.size();
...
void ByteCodeGenerator::writeIntrinsicCall(const FunctionCall& c) {
    ...
    if (found->second.fIsSpecial) {
        ...
        this->write((ByteCodeInstruction)((int)ByteCodeInstruction::kMultiplyF + count - 1), count);
        for (int i = count; i > 1; --i) {
        ...
        switch (found->second.fValue.fInstruction) {
            ...
            case ByteCodeInstruction::kCross:
                this->write(found->second.fValue.fInstruction, count);
                break;
            default:
                SkASSERT(false);
        }
...
void ByteCodeGenerator::writeFunctionCall(const FunctionCall& f) {
    ...
    // we continue execution. We adjust our max stack depth below.
    this->write(ByteCodeInstruction::kCall, 0);
    this->write8(idx);
    ...
    auto pop = [&]() {
        if (popCount > 4) {
            ...
            this->write(vector_instruction(ByteCodeInstruction::kPop, popCount), popCount);
        }
        popCount = 0;
    };
...
bool ByteCodeGenerator::writePostfixExpression(const PostfixExpression& p, bool discard) {
    switch (p.fOperator) {
        ...
        case Token::Kind::MINUSMINUS: {
            ...
            lvalue->store(discard);
            this->write(ByteCodeInstruction::kPop, 1);
            discard = false;
            break;
        }
        default:
...
void ByteCodeGenerator::writeTernaryExpression(const TernaryExpression& t) {
    ...
    this->write(ByteCodeInstruction::kMaskPush, 1);
    this->writeExpression(*t.fIfTrue);
    this->write(ByteCodeInstruction::kMaskNegate, 1);
    this->writeExpression(*t.fIfFalse);
    this->write(ByteCodeInstruction::kMaskBlend, 1);
...
void ByteCodeGenerator::writeExpression(const Expression& e, bool discard) {
    ...
    if (discard) {
        ...
        if (count > 4) {
            ...
            this->write(vector_instruction(ByteCodeInstruction::kPop, count), count);
        }
        discard = false;
    }
}
...
public:
    ...
    void load() override {
        fGenerator.write(vector_instruction(ByteCodeInstruction::kReadExternal, fCount), fCount);
        fGenerator.write8(fIndex);
    }

    void store(bool discard) override {
        if (!discard) {
            fGenerator.write(vector_instruction(ByteCodeInstruction::kDup, fCount), fCount);
        }
        fGenerator.write(vector_instruction(ByteCodeInstruction::kWriteExternal, fCount), fCount);
        fGenerator.write8(fIndex);
    }
...
public:
    ...
    void store(bool discard) override {
        ...
        if (!discard) {
            fGenerator.write(vector_instruction(ByteCodeInstruction::kDup, count), count);
        }
        Variable::Storage storage;
...
public:
    ...
    void store(bool discard) override {
        ...
        if (location < 0 || count > 4) {
            if (location >= 0) {
                fGenerator.write(ByteCodeInstruction::kPushImmediate, 1);
...
void ByteCodeGenerator::writeBreakStatement(const BreakStatement& b) {
    ...
    this->write(ByteCodeInstruction::kLoopBreak, 1);
}

void ByteCodeGenerator::writeContinueStatement(const ContinueStatement& c) {
    // TODO: Include BranchIfAllFalse to top-most LoopNext
    this->write(ByteCodeInstruction::kLoopContinue, 1);
}

void ByteCodeGenerator::writeDoStatement(const DoStatement& d) {
    this->write(ByteCodeInstruction::kLoopBegin, 1);
    size_t start = fCode->size();
    this->writeStatement(*d.fStatement);
    this->write(ByteCodeInstruction::kLoopNext, 1);
    this->writeExpression(*d.fTest);
    this->write(ByteCodeInstruction::kLoopMask, 1);
    // TODO: Could shorten this with kBranchIfAnyTrue
    this->write(ByteCodeInstruction::kBranchIfAllFalse, 1);
    DeferredLocation endLocation(this);
    this->write(ByteCodeInstruction::kBranch, 1);
    this->write16(start);
    endLocation.set();
    this->write(ByteCodeInstruction::kLoopEnd, 1);
}
...
void ByteCodeGenerator::writeIfStatement(const IfStatement& i) {
    ...
    this->write(ByteCodeInstruction::kMaskPop, 1);
}
    ...
    for (const auto& declStatement : v.fVars) {
        ...
        if (decl.fValue) {
            ...
            if (count > 4) {
                ...
                this->write(vector_instruction(ByteCodeInstruction::kStore, count), count);
                this->write8(location);
            }
        }
    }
