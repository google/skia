...
ByteCode::Register ByteCodeGenerator::writeFunctionCall(const FunctionCall& c) {
    ...
    for (const auto& a : c.fArguments) {
        args.push_back(this->writeExpression(*a));
    }
    if (c.fFunction.fBuiltin) {
        printf("need to handle intrinsic call for %s\n", c.description().c_str());
        return ByteCode::Register{0};
    }
    ...
    if (c.fType != *fContext.fVoid_Type) {
        ...
        this->write(ByteCode::Instruction::kCallVoid);
        this->write((uint8_t) idx);
        result.fIndex = -1;
    }
...
void ByteCodeGenerator::writeFunction(const FunctionDefinition& f) {
    ...
    result->fReturnCount = SlotCount(f.fDeclaration.fReturnType);
    fParameterCount = 0;
    ...
    result->fStackCount = fLocals.size();
    this->write(ByteCode::Instruction::kReturn);
