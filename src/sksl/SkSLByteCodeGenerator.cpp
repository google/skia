...
bool ByteCodeGenerator::writeBinaryExpression(const BinaryExpression& b, bool discard) {
    ...
    if (op == Token::Kind::TK_EQ) {
        std::unique_ptr<LValue> lvalue = this->getLValue(b.left());
        this->writeExpression(b.right());
        lvalue->store(discard);
        discard = false;
        return discard;
    }
    const Type& lType = b.left().type();
    const Type& rType = b.right().type();
    ...
    if (is_assignment(b.getOperator())) {
        lvalue = this->getLValue(b.left());
        lvalue->load();
        op = remove_assignment(op);
    } else {
        this->writeExpression(b.left());
    ...
    switch (op) {
        ...
        case Token::Kind::TK_SHR: {
            ...
            if (!b.right().isCompileTimeConstant()) {
                fErrors.error(b.right().fOffset,
                              "Shift amounts must be constant");
                return false;
            }
            int64_t shift = b.right().getConstantInt();
            if (shift < 0 || shift > 31) {
                fErrors.error(b.right().fOffset,
