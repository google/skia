...
std::unique_ptr<Expression> PrefixExpression::constantPropagate(const IRGenerator& irGenerator,
    ...
    const Expression& operand = *this->operand();
    if (operand.isCompileTimeConstant() && this->getOperator() == Token::Kind::TK_MINUS) {
        ...
        switch (this->operand()->kind()) {
            case Expression::Kind::kFloatLiteral:
                ...
                return std::make_unique<FloatLiteral>(fOffset,
                                                      -operand.as<FloatLiteral>().value(),
                                                      &operand.type());

            case Expression::Kind::kIntLiteral:
                // Convert -intLiteral(1) to intLiteral(-1).
                return std::make_unique<IntLiteral>(fOffset,
            ...
            case Expression::Kind::kConstructor: {
                ...
                // To optimize this, the outer negation is removed and each argument is
                // negated:
                //     float4(-float3(floatLiteral(1)), -floatLiteral(2))
                ...
                //     float4(float3(floatLiteral(-1)), floatLiteral(-2))
                std::unique_ptr<Expression> result = operand.clone();
