...
bool check_modifiers(const Context& context, int offset, const Modifiers& modifiers) {
...
bool check_return_type(const Context& context, int offset, const Type& returnType, bool isBuiltin) {
...
bool check_parameters(const Context& context, ProgramKind kind, ModifiersPool& modifiersPool,
    ...
    for (auto& param : parameters) {
        IRGenerator::CheckModifiers(context.fErrors, param->fOffset, param->modifiers(),
        ...
        if (isMain && (kind == ProgramKind::kRuntimeColorFilter ||
            ...
            if (m.fLayout.fBuiltin) {
                param->setModifiers(modifiersPool.addToPool(m));
            }
        }
...
static bool check_main_signature(const Context& context, ProgramKind kind, int offset,
                                 const Type& returnType,
                                 std::vector<std::unique_ptr<Variable>>& parameters,
                                 bool isMain, bool isBuiltin) {
    if (!isMain) {
        return true;
    }
    ErrorReporter& errors = context.fErrors;
 ...
 */
bool find_existing_declaration(const Context& context, SymbolTable& symbols, int offset,
                               StringFragment name,
                               std::vector<std::unique_ptr<Variable>>& parameters,
            ...
            if (parameters.size() == other->parameters().size()) {
                ...
                if (match) {
                    if (*returnType != other->returnType()) {
                        ...
                        FunctionDeclaration newDecl(offset,
                                                    &other->modifiers(),
                                                    name,
                                                    std::move(paramPtrs),
                                                    returnType,
                                                    isBuiltin);
                        errors.error(offset,
                                     "functions '" + newDecl.description() + "' and '" +
...
const FunctionDeclaration* FunctionDeclaration::Convert(const Context& context, ProgramKind kind,
    ...
    bool success = check_modifiers(context, offset, *modifiers) &&
                   check_return_type(context, offset, *returnType, isBuiltin) &&
                   check_parameters(context, kind, modifiersPool, parameters, isMain, isBuiltin) &&
                   check_main_signature(context, kind, offset, *returnType, parameters, isMain,
                                        isBuiltin) &&
    ...
    std::unique_ptr<FunctionDeclaration> result = std::make_unique<FunctionDeclaration>(offset,
            modifiers, name, std::move(finalParameters), returnType, isBuiltin);
