...
, fRefKind(refKind) {
    if (refKind != kRead_RefKind) {
        ((Variable&) fVariable.node()).fWriteCount++;
    }
    if (refKind != kWrite_RefKind) {
        ((Variable&) fVariable.node()).fReadCount++;
    }
    printf("NEW REFERENCE TO %s (%d, %d)\n", variable.node().description().c_str(), ((Variable&) fVariable.node()).fReadCount, ((Variable&) fVariable.node()).fWriteCount);
}
