...
protected:
    ...
    struct NodeData {
        ...
        NodeData(BlockData data)
            : fKind(Kind::kBlock) {
            BlockData& dest = *reinterpret_cast<BlockData*>(fBytes);
            new(&dest.fSymbolTable) std::shared_ptr<SymbolTable>(data.fSymbolTable);
            dest.fIsScope = data.fIsScope;
        }
        ...
        ~NodeData() {
            if (fKind == Kind::kBlock) {
                reinterpret_cast<BlockData*>(fBytes)->fSymbolTable.~shared_ptr<SymbolTable>();
            }
        }
    };
