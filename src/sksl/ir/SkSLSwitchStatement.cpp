...
static std::forward_list<const SwitchCase*> find_duplicated_case_values(
...
std::unique_ptr<Statement> SwitchStatement::Make(const Context& context,
    ...
    std::forward_list<const SwitchCase*> duplicatedCases = find_duplicated_case_values(cases);
    if (!duplicatedCases.empty()) {
        for (const SwitchCase* sc : duplicatedCases) {
            if (sc->value() != nullptr) {
...
std::unique_ptr<Statement> SwitchStatement::Make(const Context& context,
    ...
    SkASSERT(find_duplicated_case_values(cases).empty());
