...
static std::unique_ptr<Statement> block_for_case(std::vector<std::unique_ptr<SwitchCase>>* cases,
...
std::unique_ptr<Statement> SwitchStatement::Make(const Context& context,
    ...
    // If the switch value is constant, we might be able to eliminate the switch entirely.
    ...
    if (ConstantFolder::GetConstantInt(*value, &switchValue)) {
        ...
        std::unique_ptr<Statement> newBlock = block_for_case(&cases, matchingCase, symbolTable);
        if (newBlock) {
            return newBlock;
        }

        // Report an error if this was a static switch and block_for_case failed us.
