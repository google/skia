...
struct FunctionDeclaration : public Symbol {
    ...
    bool matches(const FunctionDeclaration& f) const {
        ...
        for (size_t i = 0; i < fParameters.size(); i++) {
            if (fParameters[i].expression().fType != f.fParameters[i].expression().fType) {
                return false;
            }
        }
        return true;
    ...
    bool determineFinalTypes(const std::vector<IRNode::ID>& arguments,
        ...
        for (size_t i = 0; i < arguments.size(); i++) {
            Expression& arg = (Expression&) arguments[i].expression();
            ...
            if (parameter.fType.typeNode().kind() == Type::kGeneric_Kind) {
                const std::vector<IRNode::ID>& types = parameter.fType.typeNode().coercibleTypes();
                if (genericIndex == -1) {
                    for (size_t j = 0; j < types.size(); j++) {
                        if (types[j] && type.canCoerceTo(types[j].typeNode())) {
                            genericIndex = j;
                            break;
                        }
                    }
