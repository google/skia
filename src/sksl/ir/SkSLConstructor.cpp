...
std::unique_ptr<Expression> Constructor::MakeCompoundConstructor(const Context& context,
    ...
    if (argsToOptimize) {
        // We found at least one argument that could be flattened out. Re-walk the constructor args
        // and flatten the candidates we found during our initial pass.
...
//    if (this->strictES2Mode()) {  // TODO(johnstiles): move strictES2Mode out of IRGenerator
    if (context.fConfig->fKind == ProgramKind::kRuntimeEffect ||
        context.fConfig->fKind == ProgramKind::kGeneric) {
