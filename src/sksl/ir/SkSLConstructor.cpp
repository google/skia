...
std::unique_ptr<Expression> Constructor::constantPropagate(const IRGenerator& irGenerator,
    ...
    if (this->arguments().size() == 1) {
        return this->simplifyConversionConstructor(irGenerator.fContext, this->type(),
                                                   *this->arguments().front());
    }
    return nullptr;
}

std::unique_ptr<Expression> Constructor::simplifyConversionConstructor(const Context& context,
