        ...
        switch (arg->kind()) {
            case Kind::kConstructor: {
                ...
                if (current + constructor.type().columns() > index) {
                    ...
                    return getInnerVecComponent(constructor, index - current);
                }
                break;
            }
            case Kind::kPrefix: {
                ...
                if (current + prefix.type().columns() > index) {
                    if (prefix.operand()->type().isVector()) {
                        return getInnerVecComponent(prefix, index - current);
                    } else {
                        return getConstantValue(prefix);
                    }
                }
                break;
            }
