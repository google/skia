...
std::unique_ptr<Expression> ConstructorComposite::Make(const Context& context,
    ...
    if (context.fConfig->fSettings.fOptimize) {
        // Find VectorConstructors embedded inside other VectorConstructors and flatten them out.
        //   -  float4(float2(1, 2), 3, 4)                -->  float4(1, 2, 3, 4)
        //   -  float4(w, float3(sin(x), cos(y), tan(z))) -->  float4(w, sin(x), cos(y), tan(z))

        // See how many fields we would have if vector constructors were flattened out.
        size_t fields = std::accumulate(
                args.begin(), args.end(), /*initial value*/ (size_t)0,
                [](size_t n, const std::unique_ptr<Expression>& arg) {
                    return n + (arg->is<ConstructorComposite>() ? arg->type().slotCount() : 1);
                });

        // If it's greater than the size of the argument list, we found at least one argument that
