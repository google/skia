...
std::unique_ptr<Expression> BinaryExpression::constantPropagate(const IRGenerator& irGenerator,
    ...
    // Non-assignment operators can be constant-folded.
    if (!Compiler::IsAssignment(this->getOperator())) {
    ...
    if (this->left()->is<VariableReference>() &&
        this->getOperator() == Token::Kind::TK_EQ &&
        this->right()->is<VariableReference>()) {
        if (this->left()->as<VariableReference>().variable() ==
