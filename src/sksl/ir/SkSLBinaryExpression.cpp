...
static std::unique_ptr<Expression> make_compound_assignment(const BinaryExpression& expr) {
    ...
    // is a suitable compound operator for us to use. Finally, we need to confirm that `exprA` and
    // `exprB` are actually equivalent expression trees. That is, we need to have the same variable
    // reference, or field access, or swizzle, etc. on both sides of the assignment.
