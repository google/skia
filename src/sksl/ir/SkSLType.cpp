...
std::unique_ptr<Expression> Type::coerceExpression(std::unique_ptr<Expression> expr,
    ...
    const bool allowNarrowingConversions = context.fConfig->fSettings.fAllowNarrowingConversions;

    if (!expr->coercionCost(*this).isPossible(allowNarrowingConversions)) {
