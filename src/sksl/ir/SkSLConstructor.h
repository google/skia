...
struct Constructor : public Expression {
    ...
    IRNode::ID constantPropagate(const DefinitionMap& definitions) override {
        if (fArguments.size() == 1 &&
            fArguments[0].expression().fKind == Expression::kIntLiteral_Kind) {
    ...
    type getVecComponent(int index) const {
        ...
        if (fArguments.size() == 1 &&
            fArguments[0].expression().fType.typeNode().kind() == Type::kScalar_Kind) {
            if (std::is_floating_point<type>::value) {
                return fArguments[0].expression().getConstantFloat();
            } else {
                return fArguments[0].expression().getConstantInt();
            }
        }
        int current = 0;
        for (const auto& argID : fArguments) {
            Expression& arg = argID.expression();
    ...
    SKSL_FLOAT getMatComponent(int col, int row) const override {
        ...
        if (fArguments.size() == 1) {
            Expression& arg = fArguments[0].expression();
            const Type& argType = arg.fType.typeNode();
            if (argType.kind() == Type::kScalar_Kind) {
