...
struct Constructor : public Expression {
    ...
    bool compareConstant(const Context& context, const Expression& other) const override {
        SkASSERT(other.kind() == Expression::Kind::kConstructor && other.type() == this->type());
        Constructor& c = (Constructor&) other;
        if (c.type().typeKind() == Type::TypeKind::kVector) {
            bool isFloat = c.type().columns() > 1 ? c.type().componentType().isFloat()
                                                 : c.type().isFloat();
            for (int i = 0; i < this->type().columns(); i++) {
                if (isFloat) {
        ...
        // literal
        SkASSERT(this->type().typeKind() == Type::TypeKind::kMatrix);
    ...
    type getVecComponent(int index) const {
        ...
        if (fArguments.size() == 1 && fArguments[0]->type().typeKind() == Type::TypeKind::kScalar) {
    ...
    SKSL_FLOAT getMatComponent(int col, int row) const override {
        ...
        SkASSERT(this->type().typeKind() == Type::TypeKind::kMatrix);
        SkASSERT(col < this->type().columns() && row < this->type().rows());
        if (fArguments.size() == 1) {
            if (fArguments[0]->type().typeKind() == Type::TypeKind::kScalar) {
            ...
            if (fArguments[0]->type().typeKind() == Type::TypeKind::kMatrix) {
                ...
                const Type& argType = ((Constructor&) *fArguments[0]).type();
        ...
        for (const auto& arg : fArguments) {
            ...
            SkASSERT(arg->type().rows() == 1);
            if (currentIndex + arg->type().columns() > targetIndex) {
                if (arg->type().columns() == 1) {
                    return arg->getConstantFloat();
                } else {
                    return arg->getFVecComponent(targetIndex - currentIndex);
                }
            }
            currentIndex += arg->type().columns();
        }
