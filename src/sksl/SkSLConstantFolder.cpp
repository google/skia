...
// If `expr` points to a const variable with a literal-value initial value, returns that value.
// Otherwise returns a pointer to `expr` as-is.
const Expression* ConstantFolder::GetConstantValueForVariable(const Expression& expr) {
    ...
    const Expression* initialValue = var.initialValue();
    if (!initialValue ||
        !(var.modifiers().fFlags & Modifiers::kConst_Flag) ||
        !Analysis::IsTrivialExpression(*initialValue)) {
        return &expr;
    }

    return GetConstantValueForVariable(*initialValue);
}
