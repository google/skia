...
const Expression* ConstantFolder::GetConstantValueForVariable(const Expression& inExpr) {
    for (const Expression* expr = &inExpr;;) {
        // We're looking for const variable references.
        ...
        // If the variable defines a compile-time constant, return it.
        const Expression* initialValue = var.initialValue();
        SkASSERT(initialValue);
        if (initialValue->isCompileTimeConstant()) {
            return initialValue;
        }
        // If this variable references another const variable, keep trying; otherwise, stop.
        if (initialValue->is<VariableReference>()) {
            break;
        }
        expr = initialValue;
    }
