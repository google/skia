...
// If `expr` points to a const variable with a compile-time-constant initial value, returns that
// value. Otherwise returns a pointer to `expr` as-is.
const Expression* ConstantFolder::GetConstantValueForVariable(const Expression& expr) {
    ...
    return initialValue && (initialValue->is<VariableReference>() ||
                            initialValue->isCompileTimeConstant())
                   ? GetConstantValueForVariable(*initialValue)
                   : &expr;
}
