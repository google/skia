...
public:
    ...
    struct DoStatement {};
    struct EndDoStatement { ASTNode::ID fTest; };
    ...
    struct Function {
        ...
        Function& operator,(const DoStatement& s) {
            ASTNode::ID id = fOwner->create(ASTNode::Kind::kDo);
            ASTNode::ID block = fOwner->create(ASTNode::Kind::kBlock);
            fOwner->getNode(this->inner()).addChild(id);
            fOwner->getNode(id).addChild(block);
            fScopes.push_back({ScopeType::kDo, id, block});
            return *this;
        }

        Function& operator,(const EndDoStatement& s) {
            SkASSERT(fScopes.back().fType == ScopeType::kDo);
            fOwner->getNode(fScopes.back().fID).addChild(s.fTest);
            fScopes.pop_back();
            return *this;
        }
    ...
    Expression prefix(Token::Kind kind, ASTNode::ID val) {
        ASTNode::ID id = this->create(ASTNode::Kind::kPrefix, Token(kind, -1, -1));
        this->getNode(id).addChild(val);
        return Expression(this, id);
    }

    Expression postfix(Token::Kind kind, ASTNode::ID val) {
        ASTNode::ID id = this->create(ASTNode::Kind::kPostfix, Token(kind, -1, -1));
        this->getNode(id).addChild(val);
        return Expression(this, id);
    }
...
private:
    ...
    ASTNode::ID create(ASTNode::Kind kind, Args&&... args) {
        ...
        fFile->fNodes.emplace_back(&fFile->fNodes, id.fValue, kind, args...);
        return id;
    }
...
#define PREFIX_OP(op, tk)                                              \
    static inline File::Expression operator op (const File::Node& x) { \
        return x.fOwner->prefix(Token::Kind::tk, x.id());              \
    }

PREFIX_OP(+, TK_PLUS)
PREFIX_OP(-, TK_MINUS)
PREFIX_OP(!, TK_LOGICALNOT)
PREFIX_OP(~, TK_BITWISENOT)
PREFIX_OP(++, TK_PLUSPLUS)
PREFIX_OP(--, TK_MINUSMINUS)

static inline File::Expression operator++(const File::Node& x, int) {
    return x.fOwner->postfix(Token::Kind::TK_PLUSPLUS, x.id());
}

static inline File::Expression operator--(const File::Node& x, int) {
    return x.fOwner->postfix(Token::Kind::TK_MINUSMINUS, x.id());
}
...
static const File::DoStatement Do;
static inline File::EndDoStatement EndDo(const File::Node& test) {
    return File::EndDoStatement{ test.id() };
}
