    ...
    if (outputPath.endsWith(".spirv")) {
        ...
        return compileProgram(
                [](SkSL::Compiler&, SkSL::Program& program, SkSL::OutputStream& out) {
                    ...
                    // "_coords" and "_color". At runtime, those variable names are irrelevant when
                    // the new SkSL is emitted inside the FP - references to those variables are
                    // replaced with strings from EmitArgs, and might be varyings or differently
                    // named parameters.
                    Callbacks callbacks;
