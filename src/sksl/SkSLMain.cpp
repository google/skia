...
static bool ends_with(const char* s, const char* suffix) {
    ...
    return slen >= suffixlen && !strcmp(s + slen - suffixlen, suffix);
}
...
int main(int argc, const char** argv) {
    ...
    if (ends_with(argv[1], ".vert")) {
        kind = SkSL::Program::kVertex_Kind;
    } else if (ends_with(argv[1], ".frag")) {
        kind = SkSL::Program::kFragment_Kind;
    } else if (ends_with(argv[1], ".geom")) {
        kind = SkSL::Program::kGeometry_Kind;
    } else if (ends_with(argv[1], ".fp")) {
    ...
    settings.fArgs.insert(std::make_pair("gpImplementsDistanceVector", SkSL::SettingValue(true)));
    SkSL::String name(argv[2]);
    if (name.endsWith(".spirv")) {
        printf("writing spirv because %s ends with .spirv\n", name.c_str());
