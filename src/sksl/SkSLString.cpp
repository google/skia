...
int String::find(const char* substring, int fromPos) const {
    SkASSERT(fromPos >= 0);
#ifdef SKSL_USE_STD_STRING
    // use std::string find() and check it against npos for not found,
    // and find() natively supports searching from a position
    size_t found = INHERITED::find(substring, (size_t) fromPos);
    return found == std::string::npos ? -1 : found;
#else
    // use SkStrFind on the underling c string, and pointer arithmetic
    // to support the searching position
