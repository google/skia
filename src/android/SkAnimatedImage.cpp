...
SkAnimatedImage::Frame::Frame()
    : fIndex(SkCodec::kNoRequiredFrame)
{}
...
void SkAnimatedImage::reset() {
    ...
    if (fDisplayFrame.fIndex != 0) {
        fDisplayFrame.fIndex = SkCodec::kNoRequiredFrame;
        this->decodeNextFrame();
    }
}
...
int SkAnimatedImage::decodeNextFrame() {
    ...
    if (fCodec->codec()->getFrameInfo(frameToDecode, &frameInfo)) {
        ...
        if (0 == frameToDecode) {
            ...
            frameInfo.fRequiredFrame = SkCodec::kNoRequiredFrame;
    ...
    if (frameInfo.fRequiredFrame == SkCodec::kNoRequiredFrame) {
        if (is_restore_previous(frameInfo.fDisposalMethod)) {
            ...
            // future.
            if (fDecodingFrame.fIndex != SkCodec::kNoRequiredFrame &&
        ...
        auto validPriorFrame = [&frameInfo, &frameToDecode](const Frame& frame) {
            if (SkCodec::kNoRequiredFrame == frame.fIndex ||
