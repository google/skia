    ...
    , fRepetitionsCompleted(0)
{
    ...
    // clip), orientation, and scaling outside of fCodec. The matrices are computed indivdually
...
sk_sp<SkImage> SkAnimatedImage::getCurrentFrame() {
    ...
    sk_sp<SkPicture> pic(this->newPictureSnapshot());
    const auto& bitmap = fDisplayFrame.fBitmap;
    const auto bitDepth = bitmap.colorType() == kRGBA_F16_SkColorType ? SkImage::BitDepth::kF16
                                                                      : SkImage::BitDepth::kU8;
    return SkImage::MakeFromPicture(std::move(pic), fCropRect.size(), nullptr, nullptr,
            bitDepth, bitmap.refColorSpace());
}
