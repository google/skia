...
SI T* ptr_at_xy(const SkJumper_MemoryCtx* ctx, int x, int y) {
    ...
    SI V operator-(V x, V y) { return x.vec - y.vec; }
    SI V operator*(V x, V y) {
        // (xy + 127) / 255 == (xy + (xy+128)>>8 +128) >> 8
        auto xy = vmull_u8(x, y);
        return vraddhn_u16(xy, vrshrq_n_u16(xy, 8));
    }
    SI V operator/(V x, int k) { return x.vec / k; }

    SI V inv(V x) { return 1.0f - x; }
    SI V lerp(V from, V to, V t) { return to*t + from*inv(t); }

    template <typename C>
    SI V if_then_else(C c, V t, V e) {
        return (t.vec & c) | (e.vec & ~c);
    }
    SI V max(V a, V b) { return if_then_else(a > b, a, b); }
    SI V min(V a, V b) { return if_then_else(a > b, b, a); }
    ...
    SI U8 load_u8(const uint8_t* ptr, size_t tail) {
        ...
        U8 v = 0;
        switch (__builtin_expect(tail, 0)) {
            case 0: v = vld1_u8(ptr); break;
            case 7: v = vld1_lane_u8(ptr+6, v, 6);
            case 6: v = vld1_lane_u8(ptr+5, v, 5);
            case 5: v = vld1_lane_u8(ptr+4, v, 4);
            case 4: v = vld1_lane_u8(ptr+3, v, 3);
            case 3: v = vld1_lane_u8(ptr+2, v, 2);
            case 2: v = vld1_lane_u8(ptr+1, v, 1);
            case 1: v = vld1_lane_u8(ptr+0, v, 0);
        }
        return v;
    }
    ...
    SI void store_u8(uint8_t* ptr, size_t tail, U8 v) {
        ...
        switch (__builtin_expect(tail, 0)) {
            case 0: vst1_u8(ptr, v); break;
            case 7: vst1_lane_u8(ptr, v, 6);
            case 6: vst1_lane_u8(ptr, v, 5);
            case 5: vst1_lane_u8(ptr, v, 4);
            case 4: vst1_lane_u8(ptr, v, 3);
            case 3: vst1_lane_u8(ptr, v, 2);
            case 2: vst1_lane_u8(ptr, v, 1);
            case 1: vst1_lane_u8(ptr, v, 0);
        }
    }
    ...
    BLEND_MODE(hardlight) {
        return s*inv(da) + d*inv(sa)
             + if_then_else(s <= sa/2, 2*s*d, sa*da - 2*(da - d)*(sa - s));
    }
    BLEND_MODE(overlay) {
        return d*inv(sa) + s*inv(da)
             + if_then_else(d <= da/2, 2*s*d, sa*da - 2*(da - d)*(sa - s));
    }
