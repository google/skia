...
SI T* ptr_at_xy(const SkJumper_MemoryCtx* ctx, int x, int y) {
    ...
    STAGE(store_565) {
        ...
        U16 rr = __builtin_convertvector(r >> 3, U16);
        U16 gg = __builtin_convertvector(g >> 2, U16);
        U16 bb = __builtin_convertvector(b >> 3, U16);
        store_u16(ptr, (rr << 11) | (gg << 5) | bb, tail);
    }
    ...
    STAGE(load_565_dst) {
        ...
        b = __builtin_convertvector(( p           << 3) | ((p & 0x18) >>  3), U8);
        a = 0xFF;
    }
