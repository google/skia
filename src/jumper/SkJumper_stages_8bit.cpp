...
SI T* ptr_at_xy(const SkJumper_MemoryCtx* ctx, int x, int y) {
    ...
    SI V hardlight_or_overlay(V s, V sa, V d, V da) {
        U16 both = mul_wide(s, inv(da))
                 + mul_wide(d, inv(sa));

        U16 isLite = vmovl_s8(sa-s < s);  // Note the _s8: this sign-extends the comparison mask.

        U16 lite = mul_wide(sa,da) - 2*mul_wide(sa-s, da-d),
            dark = 2*mul_wide(s,d);

        return both + ((isLite & lite) | (~isLite & dark));
    }
