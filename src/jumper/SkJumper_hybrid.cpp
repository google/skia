...
// Clang's available function ABIs and not spilling onto the stack.
    ...
    // To keep things simple, we'll only define the minimum in this portable path,
    // handling only one high precision Pixel at a time.
    using F = float;
    struct    XY_H { F x,y; };
    struct Pixel_H { float r,g,b,a; };

    //SI int trunc_(float v) { return (int)v; }
    ...
    #define STAGE_GP_H(name, ...)                                          \
        template <typename F, typename XY, typename Pixel>                 \
        SI void name##_T(__VA_ARGS__);                                     \
        extern "C" void name##_H(int tail, void** program, int x, int y,   \
                                 float f0, float f1, float f2, float f3,   \
                                 float f4, float f5, float f6, float f7) { \
            Pixel_H src,dst;                                               \
            Ctx ctx { program };                                           \
            name##_T<float>(ctx,x,y, XY_H{f0,f1}, &src,&dst);              \
            auto next = (Stage)load_and_inc(program);                      \
            next(tail,program,x,y, src.r,src.g,src.b,src.a,                \
                                   dst.r,dst.g,dst.b,dst.a);               \
        }                                                                  \
        template <typename F, typename XY, typename Pixel>                 \
        SI void name##_T(__VA_ARGS__)
    #define STAGE_GP_L(name, ...) template <typename F, typename XY, typename Pixel> \
                                  SI void name##_L(__VA_ARGS__)
...
STAGE_GP_H(gather_8888, const SkJumper_MemoryCtx* ctx,
           int x, int y, XY xy, Pixel* src, Pixel* dst) {
    //auto ptr = (const uint32_t*)ctx->pixels;
    //auto  ix = trunc_(xy.x) + trunc_(xy.y)*ctx->stride;
}
