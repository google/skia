...
// *program++, in a single instruction on x86-64 when program is in %rsi.
...
#if 0 && (defined(__i386__) || defined(__arm__))
    // 32-bit x86 and ARM are somewhat limited in the number of registers they can pass,
    // both general purpose and vector.  We stick to two general purpose and four vector,
    // with the rest on the stack in Params.
    template <typename R> struct Params { int tail,x,y; R r4,r5,r6,r7; };
    template <typename R> using Stage = void(ABI *)(Params<R>*, void** program, R,R,R,R);

    #define STAGE_GG_COMMON(name, precision)                                          \
        ABI extern "C" void name##_##precision(Params<R>* params, void** program,     \
                                               R r0, R r1, R r2, R r3) {              \
            R &r4 = params->r4, &r5 = params->r5, &r6 = params->r6, &r7 = params->r7; \
            precision::XY xy;                                                         \
            unpack(&xy, r0,r1,r2,r3,r4,r5,r6,r7);                                     \
            name##_T(Ctx{program},params->x,params->y, &xy);                          \
            pack(xy, &r0,&r1,&r2,&r3,&r4,&r5,&r6,&r7);                                \
            auto next = (Stage<R>)load_and_inc(program);                              \
            next(params, program, r0,r1,r2,r3);                                       \
        }

    #define STAGE_PP_COMMON(name, precision)                                          \
        ABI extern "C" void name##_##precision(Params<R>* params, void** program,     \
                                               R r0, R r1, R r2, R r3) {              \
            R &r4 = params->r4, &r5 = params->r5, &r6 = params->r6, &r7 = params->r7; \
            precision::Pixel src,dst;                                                 \
            unpack(&src, &dst, r0,r1,r2,r3, r4,r5,r6,r7);                             \
            name##_##precision##T(Ctx{program},params->x,params->y, &src,&dst);       \
...
#else
    // We can pass many more registers on 64-bit platforms.
    template <typename R> using Stage = void(ABI *)(int, void** program,int,int, R,R,R,R, R,R,R,R);

    #define STAGE_GG_COMMON(name, precision)                                           \
        ABI extern "C" void name##_##precision(int tail, void** program, int x, int y, \
                                           R r0, R r1, R r2, R r3,                     \
                                           R r4, R r5, R r6, R r7) {                   \
            precision::XY xy;                                                          \
            unpack(&xy, r0,r1,r2,r3,r4,r5,r6,r7);                                      \
