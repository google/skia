...
STAGE_PP(dither, const float* rate) {
    ...
    // like 0 and 1 unchanged after rounding.  And then scale everything to our lowp 255 bias.
    F dither = cast<F>(M) * (255.0f * 2/128.0f) - (255.0f * 63/128.0f);

    F R = cast<F>(r) + *rate * dither,
      G = cast<F>(g) + *rate * dither,
      B = cast<F>(b) + *rate * dither;

    R = max(0, min(R, cast<F>(a)));
    G = max(0, min(G, cast<F>(a)));
    B = max(0, min(B, cast<F>(a)));

    r = cast<U16>(trunc_(R));  // TODO: round?
    g = cast<U16>(trunc_(G));
    b = cast<U16>(trunc_(B));
}
