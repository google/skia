...
SI void from_8888(U32 rgba, F* r, F* g, F* b, F* a) {
    ...
    // Get each channel's bytes entirely in one register, in 04 16 26 37 order.
    U16 rg = _mm_unpacklo_epi16(lo, hi),  // r0 g0 r4 g4  r1 g1 r5 g5 ...
        ba = _mm_unpackhi_epi16(lo, hi);

    // Unpack as 16-bit values, in 01 23 45 67 order.
    // Each byte goes into the high half of its 16-bit lane, to get a free *256 we'll need next.
    const int _ = ~0;
    const __m128i shuffle_rb = _mm_setr_epi8(_,0,_,4, _,8,_,12, _,2,_,6, _,10,_,14),
                  shuffle_ga = _mm_setr_epi8(_,1,_,5, _,9,_,13, _,3,_,7, _,11,_,15);
    U16 R = _mm_shuffle_epi8(rg, shuffle_rb),
        G = _mm_shuffle_epi8(rg, shuffle_ga),
        B = _mm_shuffle_epi8(ba, shuffle_rb),
        A = _mm_shuffle_epi8(ba, shuffle_ga);
...
SI U32 to_8888(F r, F g, F b, F a) {
    ...
    // This is only important for inputs which were 32768, which become 256 after >>7.
    U16 rb = _mm_packus_epi16(r,b),       // r0 r1 r2 r3 r4 r5 r6 r7 b0 b1 b2 b3 b4 b5 b6 b7
        ga = _mm_packus_epi16(g,a);
