...
SI D cast(S src) {
    ...
    template <>
    inline U16 cast(U32 src) {
        static_assert(sizeof(U32) == 2*sizeof(__m256i), "");
        __m256i lo,hi;
        memcpy(&lo, (const char*)&src + 0*sizeof(__m256i), sizeof(__m256i));
        memcpy(&hi, (const char*)&src + 1*sizeof(__m256i), sizeof(__m256i));
...
SI void load_8888(const uint32_t* ptr, size_t tail, U16* r, U16* g, U16* b, U16* a) {
    ...
    *r = cast<U16>((rgba >>  0) & 255);
    *g = cast<U16>((rgba >>  8) & 255);
    *b = cast<U16>((rgba >> 16) & 255);
    *a = cast<U16>((rgba >> 24) & 255);
#endif
}
