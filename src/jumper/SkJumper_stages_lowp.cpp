...
SI void store_8888(uint32_t* ptr, size_t tail, U16 r, U16 g, U16 b, U16 a) {
    ...
    // TODO
#else
    store(ptr, tail, __builtin_convertvector(r, U32) <<  0
                   | __builtin_convertvector(g, U32) <<  8
                   | __builtin_convertvector(b, U32) << 16
                   | __builtin_convertvector(a, U32) << 24);
#endif
}
...
SI U16 load_8(const uint8_t* ptr, size_t tail) {
    return __builtin_convertvector(load<U8>(ptr, tail), U16);
}
SI void store_8(uint8_t* ptr, size_t tail, U16 v) {
    store(ptr, tail, __builtin_convertvector(v, U8));
}
