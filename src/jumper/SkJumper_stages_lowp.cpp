...
SI void load_8888(const uint32_t* ptr, size_t tail, U16* r, U16* g, U16* b, U16* a) {
    ...
    *r = __builtin_convertvector(rgba.val[0], U16);
    *g = __builtin_convertvector(rgba.val[1], U16);
    *b = __builtin_convertvector(rgba.val[2], U16);
    *a = __builtin_convertvector(rgba.val[3], U16);
#else
    U32 rgba = 0;
    switch (tail & (N-1)) {
        case  0: memcpy(&rgba, ptr, sizeof(rgba)); break;
        case 15: rgba[14] = ptr[14];
        case 14: rgba[13] = ptr[13];
        case 13: rgba[12] = ptr[12];
        case 12: memcpy(&rgba, ptr, 12*sizeof(uint32_t)); break;
        case 11: rgba[10] = ptr[10];
        case 10: rgba[ 9] = ptr[ 9];
        case  9: rgba[ 8] = ptr[ 8];
        case  8: memcpy(&rgba, ptr,  8*sizeof(uint32_t)); break;
        case  7: rgba[ 6] = ptr[ 6];
        case  6: rgba[ 5] = ptr[ 5];
        case  5: rgba[ 4] = ptr[ 4];
        case  4: memcpy(&rgba, ptr,  4*sizeof(uint32_t)); break;
        case  3: rgba[ 2] = ptr[ 2];
        case  2: memcpy(&rgba, ptr,  2*sizeof(uint32_t)); break;
        case  1: rgba[ 0] = ptr[ 0];
    }
