...
#define STAGE_GG(name, ...)                                                            \
    SI void name##_k(__VA_ARGS__, size_t x, size_t y, size_t tail, F& X, F& Y);        \
    ABI extern "C" void WRAP(name)(size_t tail, void** program, size_t x, size_t y,    \
                                   U16  r, U16  g, U16  b, U16  a,                     \
                                   U16 dr, U16 dg, U16 db, U16 da) {                   \
        auto X = join<F>(r,g),                                                         \
             Y = join<F>(b,a);                                                         \
        name##_k(Ctx{program}, x,y,tail, X,Y);                                         \
        split(X, &r,&g);                                                               \
        split(Y, &b,&a);                                                               \
...
SI F mad(F f, F m, F a) { return f*m+a; }  // TODO: FMA intrinsics?

// ~~~~~~ Basic / misc. stages ~~~~~~ //

STAGE_GG(seed_shader, const float* iota) {
    X = cast<F>(I32(x)) + unaligned_load<F>(iota);
    Y = cast<F>(I32(y)) + 0.5f;
}

STAGE_GG(matrix_2x3, const float* m) {
    auto X_ = mad(X,m[0], mad(Y,m[2], m[4])),
         Y_ = mad(X,m[1], mad(Y,m[3], m[5]));
    X = X_;
    Y = Y_;
}
