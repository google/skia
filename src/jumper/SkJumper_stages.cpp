...
// Not all constants can be generated used here.  Stages read the rest from this struct.
...
STAGE(from_2dot2) {
    r = approx_powf(r, 2.2f);
    g = approx_powf(g, 2.2f);
    b = approx_powf(b, 2.2f);
}
STAGE(to_2dot2) {
    r = approx_powf(r, 1/2.2f);
    g = approx_powf(g, 1/2.2f);
    b = approx_powf(b, 1/2.2f);
}
...
STAGE(hsl_to_rgb) {
    ...
    F q = if_then_else(l < 0.5f, l*(1.0f + s), l + s - l*s),
      p = 2.0f*l - q;

    auto hue_to_rgb = [&](F t) {
        F t2 = if_then_else(t < 0.0f, t + 1.0f,
               if_then_else(t > 1.0f, t - 1.0f,
                                      t));

        return if_then_else(t2 < (1/6.0f),  p + (q-p)*6.0f*t,
               if_then_else(t2 < (3/6.0f),  q,
               if_then_else(t2 < (4/6.0f),  p + (q-p)*6.0f*((4/6.0f) - t2),
                                            p)));
    };
