...
static inline void store(T* dst, V v, size_t tail) {
    ...
    static inline U32 mask(size_t tail) {
        // It's easiest to build the mask as 8 16-bit values, either 0x0000 or 0xffff.
        // Start fully on, then shift away lanes from the top until we've got our mask.
        auto mask = _mm_set1_epi16(0xffff);
        if ((kStride-tail) & 4) { mask = _mm_srli_si128(mask, 8); }
        if ((kStride-tail) & 2) { mask = _mm_srli_si128(mask, 4); }
        if ((kStride-tail) & 1) { mask = _mm_srli_si128(mask, 2); }
        // Sign-extend each mask lane to its full width, 0x00000000 or 0xffffffff.
        return _mm256_cvtepi16_epi32(mask);
    }
