...
// We often have a nominally [0,1] float value we need to scale and convert
// to an integer, whether for a table lookup or to pack back down into bytes
// for storage.
//
// In practice, especially when dealing with interesting color spaces,
// that nominally [0,1] float may be out of [0,1] range.  We clamp in that case.
SI U32 to_unorm(F v, int scale) {
    // TODO: possibly remove the round() function entirely.
    // Any time we use it probably wants to be a to_unorm().
    // TODO: rework this logic to scale, then clamp to [0,scale]?
    return round(clamp_01(v), scale);
}
