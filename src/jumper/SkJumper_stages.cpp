...
// with a little bith of math simplification inspired by
//   https://www.khronos.org/registry/OpenGL/extensions/KHR/KHR_blend_equation_advanced.txt,
// BUT keep in mind the KHR document has a typo in ClipColor().  w3's is correct.
...
SI void set_sat(F* r, F* g, F* b, F s) {
    ...
    // Map min channel to 0, max channel to s, and scales the middle channel proportionally.
    auto scale = [=](F c) {
...
SI void clip_color(F* r, F* g, F* b) {
    ...
    auto clip = [=](F c) {
        ...
        c = max(c,0);  // Sometimes without this we go a little negative.
        return c;
    };
    *r = clip(*r);
    *g = clip(*g);
