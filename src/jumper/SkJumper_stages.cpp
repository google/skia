...
static inline float K(float f) {
    int x = K(unaligned_load<int>(&f));
    return unaligned_load<float>(&x);
}
static inline int   operator "" _i(unsigned long long int i) { return K(  (int)i); }
static inline float operator "" _f(           long double f) { return K((float)f); }
...
static void from_565(U16 _565, F* r, F* g, F* b) {
    ...
    *r = cast(wide & K(31<<11)) * K(1.0f / (31<<11));
    *g = cast(wide & K(63<< 5)) * K(1.0f / (63<< 5));
    *b = cast(wide & K(31<< 0)) * K(1.0f / (31<< 0));
}
...
struct LazyCtx {
    ...
    using Stage = void(size_t x, void** program, size_t tail, F,F,F,F, F,F,F,F);

    #if defined(JUMPER) && defined(WIN)
    __attribute__((ms_abi))
    #endif
    extern "C" size_t WRAP(start_pipeline)(size_t x, void** program, size_t limit) {
        F v{};
        auto start = (Stage*)load_and_inc(program);
        while (x + kStride <= limit) {
            start(x,program,0,    v,v,v,v, v,v,v,v);
            x += kStride;
        }
        if (size_t tail = limit - x) {
            start(x,program,tail, v,v,v,v, v,v,v,v);
        }
        return limit;
    }

    #define STAGE(name)                                                           \
        static void name##_k(size_t x, LazyCtx ctx, size_t tail,                  \
                             F& r, F& g, F& b, F& a, F& dr, F& dg, F& db, F& da); \
        extern "C" void WRAP(name)(size_t x, void** program, size_t tail,         \
                                   F r, F g, F b, F a, F dr, F dg, F db, F da) {  \
            LazyCtx ctx(program);                                                 \
            name##_k(x,ctx,tail, r,g,b,a, dr,dg,db,da);                           \
            auto next = (Stage*)load_and_inc(program);                            \
            next(x,program,tail, r,g,b,a, dr,dg,db,da);                           \
        }                                                                         \
        static void name##_k(size_t x, LazyCtx ctx, size_t tail,                  \
                             F& r, F& g, F& b, F& a, F& dr, F& dg, F& db, F& da)
    ...
    using Stage = void(size_t x, void** program, F,F,F,F, F,F,F,F);
    ...
    extern "C" size_t WRAP(start_pipeline)(size_t x, void** program, size_t limit) {
        ...
        auto start = (Stage*)load_and_inc(program);
        while (x + kStride <= limit) {
            start(x,program, v,v,v,v, v,v,v,v);
            x += kStride;
        }
        return x;
    }
    ...
    #define STAGE(name)                                                           \
        static void name##_k(size_t x, LazyCtx ctx, size_t tail,                  \
                             F& r, F& g, F& b, F& a, F& dr, F& dg, F& db, F& da); \
        extern "C" void WRAP(name)(size_t x, void** program,                      \
                                   F r, F g, F b, F a, F dr, F dg, F db, F da) {  \
            LazyCtx ctx(program);                                                 \
            name##_k(x,ctx,0, r,g,b,a, dr,dg,db,da);                              \
            auto next = (Stage*)load_and_inc(program);                            \
            next(x,program, r,g,b,a, dr,dg,db,da);                                \
        }                                                                         \
        static void name##_k(size_t x, LazyCtx ctx, size_t tail,                  \
                             F& r, F& g, F& b, F& a, F& dr, F& dg, F& db, F& da)
#endif
...
extern "C" void WRAP(just_return)(size_t, void**, F,F,F,F, F,F,F,F) {}
...
STAGE(scale_u8) {
    ...
    auto c = cast(expand(scales)) * K(1/255.0f);
...
STAGE(load_a8) {
    ...
    r = g = b = 0.0f;
    a = cast(expand(load<U8>(ptr, tail))) * K(1/255.0f);
}
STAGE(store_a8) {
...
STAGE(load_8888) {
    ...
    r = cast((px      ) & 0xff_i) * K(1/255.0f);
    g = cast((px >>  8) & 0xff_i) * K(1/255.0f);
    b = cast((px >> 16) & 0xff_i) * K(1/255.0f);
    a = cast((px >> 24)         ) * K(1/255.0f);
}
