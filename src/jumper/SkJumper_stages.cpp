...
SI void set_sat(F* r, F* g, F* b, F new_sat) {
    ...
    *r = if_then_else(*r == mn, 0,
         if_then_else(*r == mx, new_sat, (md - mn) * new_sat / old_sat));
    *g = if_then_else(*g == mn, 0,
         if_then_else(*g == mx, new_sat, (md - mn) * new_sat / old_sat));
    *b = if_then_else(*b == mn, 0,
         if_then_else(*b == mx, new_sat, (md - mn) * new_sat / old_sat));
}
...
SI void clip_color(F* r, F* g, F* b, F a) {
    ...
    F scale = if_then_else((mn < 0.0f) & ((l - mn) != 0.0f), l / (l - mn),
              if_then_else((mx > a   ) & ((mx - l) != 0.0f), (a - l) / (mx - l),
                                                             1.0f));
    *r *= scale;
    *g *= scale;
    *b *= scale;
