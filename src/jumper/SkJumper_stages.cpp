...
STAGE(xy_to_2pt_conical_linear) {
    ...
    g = if_then_else(mad(r, c->fDR, c->fR0) >= 0, F(1), F(0)); // R(t) < 0
    g = g * if_then_else(r != r, F(0), F(1));                  // NaN
    unaligned_store(&c->fMaskContext, g);
}

STAGE(vector_scale) {
    auto* c = (const SkJumper_VectorScaleCtx*)ctx;
    const F scale = unaligned_load<F>(c->scale);
