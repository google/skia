...
extern "C" {
    ...
    void ASM(start_pipeline_bulk,aarch64)(
        void** program, K* k,
    ...
    void ASM(start_pipeline_bulk,vfp4)(
        void** program, K* k,
        int* y, const int* L, const int* R, size_t height, void** dstPtr, void* base, int xshift, size_t rowbytes);
    ...
    void ASM(start_pipeline_bulk,hsw)(
        void** program, K* k,
        int* y, const int* L, const int* R, size_t height, void** dstPtr, void* base, int xshift, size_t rowbytes);
    void ASM(start_pipeline_bulk,avx)(
        void** program, K* k,
        int* y, const int* L, const int* R, size_t height, void** dstPtr, void* base, int xshift, size_t rowbytes);
    void ASM(start_pipeline_bulk,sse41)(
        void** program, K* k,
        int* y, const int* L, const int* R, size_t height, void** dstPtr, void* base, int xshift, size_t rowbytes);
    void ASM(start_pipeline_bulk,sse2)(
        void** program, K* k,
        int* y, const int* L, const int* R, size_t height, void** dstPtr, void* base, int xshift, size_t rowbytes);
...
void SkRasterPipeline::runBulk(
    ...
    auto build_and_run = [&](size_t   min_stride,
                             StageFn* (*lookup)(SkRasterPipeline::StockStage),
                             StageFn* just_return,
                             void     (*start_pipeline_bulk)(
                                     void** program, K* k, int* y, const int* L, const int* R, size_t height,
        ...
        *ip = (void*)just_return;
        ...
        start_pipeline_bulk(program.get(), &kConstants, y, L, R, height, dstPtr, base, xshift, rowbytes);
    };
...
#elif defined(__aarch64__)
    build_and_run(4, lookup_aarch64, ASM(just_return,aarch64), ASM(start_pipeline_bulk,aarch64));

#elif defined(__arm__)
    if (1 && SkCpu::Supports(SkCpu::NEON|SkCpu::NEON_FMA|SkCpu::VFP_FP16)) {
        build_and_run(2, lookup_vfp4, ASM(just_return,vfp4), ASM(start_pipeline_bulk,vfp4));
    }

#elif defined(__x86_64__) || defined(_M_X64)
    if (1 && SkCpu::Supports(SkCpu::HSW)) {
        build_and_run(1, lookup_hsw, ASM(just_return,hsw), ASM(start_pipeline_bulk,hsw));
        return;
    }
    if (1 && SkCpu::Supports(SkCpu::AVX)) {
        build_and_run(1, lookup_avx, ASM(just_return,avx), ASM(start_pipeline_bulk,avx));
        return;
    }
    if (1 && SkCpu::Supports(SkCpu::SSE41)) {
        build_and_run(4, lookup_sse41, ASM(just_return,sse41), ASM(start_pipeline_bulk,sse41));
        return;
    }
    if (1 && SkCpu::Supports(SkCpu::SSE2)) {
        build_and_run(4, lookup_sse2, ASM(just_return,sse2), ASM(start_pipeline_bulk,sse2));
        return;
    }
#endif

    // Finish up any leftover with portable code one pixel at a time.
    build_and_run(1, lookup_portable, sk_just_return, sk_start_pipeline_bulk);
}
