...
SkRasterPipeline::StartPipelineFn SkRasterPipeline::build_pipeline(void** ip) const {
    ...
    // First try to build a lowp pipeline.  If we can't, we'll fall back to a float pipeline.
    void** reset_point = ip;
    ...
    *--ip = (void*)SkOpts::just_return_lowp;
    for (const StageList* st = fStages; st; st = st->prev) {
        ...
        if (auto stage = SkOpts::stages_lowp[st->stage]) {
            if (st->ctx) {
                *--ip = st->ctx;
            }
            *--ip = (void*)stage;
    ...
    *--ip = (void*)SkOpts::just_return;
    ...
    for (const StageList* st = fStages; st; st = st->prev) {
        ...
        *--ip = (void*)SkOpts::stages[st->stage];
    }
    return SkOpts::start_pipeline;
}
