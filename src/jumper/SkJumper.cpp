...
extern "C" {
    ...
    #define M(st) \
        template <> StageFn* hsw_lowp<SkRasterPipeline::st>() { return ASM(st,hsw_lowp); }
        LOWP_STAGES(M)
    #undef M
    ...
    #define M(st) \
        template <> StageFn* ssse3_lowp<SkRasterPipeline::st>() { return ASM(st,ssse3_lowp); }
        LOWP_STAGES(M)
    #undef M
#endif
