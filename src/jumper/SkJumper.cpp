...
extern "C" {
    ...
    #define M(st) template <> constexpr StageFn* hsw_lowp<SkRasterPipeline::st>() { \
    ...
    #define M(st) template <> constexpr StageFn* ssse3_lowp<SkRasterPipeline::st>() { \
...
StartPipelineFn* SkRasterPipeline::build_pipeline(void** ip) const {
    ...
    *--ip = (void*)gLowp.just_return;
    for (const StageList* st = fStages; st; st = st->prev) {
        ...
        if (StageFn* fn = gEngine.stages[st->stage]) {
            if (st->ctx) {
                *--ip = st->ctx;
            }
            *--ip = (void*)fn;
