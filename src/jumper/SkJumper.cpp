...
// Declare the portable, single pixel stages from the SkJumper_stages.o that are linked into Skia.
extern "C" {
...
// Convert from SkRasterPipeline's enum to pointers to our portable, single pixel stages.
...
bool SkRasterPipeline::run_with_jumper(size_t x, size_t n) const {
    ...
    just_return = (void*)aarch64_just_return;
#elif defined(__ARM_NEON__)
    if (SkCpu::Supports(SkCpu::NEON|SkCpu::NEON_FMA|SkCpu::VFP_FP16)) {
        stride      = 2;
        lookup      = armv7_lookup;
        just_return = (void*)armv7_just_return;
    }
    ...
    // Build and run aprogram to run at full vector stride.
    if (stride) {
        void** ip = program.get();
        for (auto&& st : fStages) {
