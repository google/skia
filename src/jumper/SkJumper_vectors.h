...
// Every function in this file should be marked static and inline using SI (see SkJumper_misc.h).
    ...
    SI void load4(const uint16_t* ptr, size_t tail, U16* r, U16* g, U16* b, U16* a) {
        __m128 _01{}, _23{};
        __builtin_assume(tail < 4);
        if (__builtin_expect(tail,0)) {
            switch (tail - 1) {
                case 2: _23 = _mm_load_sd( (double*)ptr + 2); /* fall through */
                case 1: _01 = _mm_loadu_pd((double*)ptr);     break;
                case 0: _01 = _mm_load_sd( (double*)ptr);     break;
            }
        } else {
            _01 = _mm_loadu_si128(((__m128i*)ptr) + 0);
            _23 = _mm_loadu_si128(((__m128i*)ptr) + 1);
    ...
    SI void store4(uint16_t* ptr, size_t tail, U16 r, U16 g, U16 b, U16 a) {
        ...
        auto _01 = _mm_castsi128_pd(_mm_unpacklo_epi32(rg, ba)),
             _23 = _mm_castsi128_pd(_mm_unpackhi_epi32(rg, ba));
        __builtin_assume(tail < 4);
        if (__builtin_expect(tail, 0)) {
            switch (tail - 1) {
                case 2: _mm_store_sd( (double*)ptr + 2, _23); /* fall through */
                case 1: _mm_storeu_pd((double*)ptr, _01);     break;
                case 0: _mm_store_sd( (double*)ptr, _01);     break;
            }
        } else {
            _mm_storeu_pd((double*)ptr + 0, _01);
            _mm_storeu_pd((double*)ptr + 2, _23);
        }
    }
    ...
    SI void load4(const float* ptr, size_t tail, F* r, F* g, F* b, F* a) {
        ...
        if (__builtin_expect(tail, 0)) {
            switch (tail-1) {
                case 2: _2 = _mm_loadu_ps(ptr+ 8);
                case 1: _1 = _mm_loadu_ps(ptr+ 4);
                case 0: _0 = _mm_loadu_ps(ptr+ 0);
            }
        } else {
    ...
    SI void store4(float* ptr, size_t tail, F r, F g, F b, F a) {
        ...
        if (__builtin_expect(tail, 0)) {
            switch (tail - 1) {
                case 2: _mm_storeu_ps(ptr+ 8, b);
                case 1: _mm_storeu_ps(ptr+ 4, g);
                case 0: _mm_storeu_ps(ptr+ 0, r);
            }
        } else {
