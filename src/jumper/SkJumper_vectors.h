...
// Every function in this file should be marked static and inline using SI (see SkJumper_misc.h).
    ...
    SI void load4(const float* ptr, size_t tail, F* r, F* g, F* b, F* a) {
        F _01, _23, _45, _67;
        if (__builtin_expect(tail, 0)) {
            if (tail > 0) { _01 = _mm256_insertf128_ps(_01, _mm_loadu_ps(ptr+ 0), 0); }
            if (tail > 1) { _01 = _mm256_insertf128_ps(_01, _mm_loadu_ps(ptr+ 4), 1); }
            if (tail > 2) { _23 = _mm256_insertf128_ps(_23, _mm_loadu_ps(ptr+ 8), 0); }
            if (tail > 3) { _23 = _mm256_insertf128_ps(_23, _mm_loadu_ps(ptr+12), 1); }
            if (tail > 4) { _45 = _mm256_insertf128_ps(_45, _mm_loadu_ps(ptr+16), 0); }
            if (tail > 5) { _45 = _mm256_insertf128_ps(_45, _mm_loadu_ps(ptr+20), 1); }
            if (tail > 6) { _67 = _mm256_insertf128_ps(_67, _mm_loadu_ps(ptr+24), 0); }
        } else {
            _01 = _mm256_loadu_ps(ptr+ 0);
            _23 = _mm256_loadu_ps(ptr+ 8);
            _45 = _mm256_loadu_ps(ptr+16);
            _67 = _mm256_loadu_ps(ptr+24);
        }

        F rg0213 = _mm256_unpacklo_ps(_01,_23),  // r0 r2 g0 g2 | r1 r3 g1 g3
          rg4657 = _mm256_unpacklo_ps(_45,_67),
          ba0213 = _mm256_unpackhi_ps(_01,_23),  // b0 b2 a0 ...
          ba4657 = _mm256_unpackhi_ps(_45,_67);

        F reo = _mm256_unpacklo_pd(rg0213, rg4657),  // r0 r2 r4 r6 | r1 r3 r5 r7
          geo = _mm256_unpackhi_pd(rg0213, rg4657),  // g0 g2 ...
          beo = _mm256_unpacklo_pd(ba0213, ba4657),
          aeo = _mm256_unpackhi_pd(ba0213, ba4657);
