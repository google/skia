...
def parse_object_file(dot_o, directive, target=None):
  ...
  for line in subprocess.check_output(cmd + disassemble).split('\n'):
    ...
    if m:
      print
      sym = m.group(1)
      if sym.startswith('.literal'):  # .literal4, .literal16
        print sym.replace('.literal', align)
      elif sym.startswith('.const'):  # 32-byte constants end up in .const
        print align + '32'
      elif sym.startswith('LCPI'):    # 16-byte constant re-align marker?
        print align + '16'
