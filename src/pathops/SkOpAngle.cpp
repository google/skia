...
bool SkOpAngle::after(SkOpAngle* test) {
    ...
    this->debugAfter(lh, rh, &bugOut);
#endif
    if (lh->debugID() == 18 && this->debugID() == 11 && rh->debugID() == 10) {
        SkDebugf("");
    }
    ...
    */
    ...
    if (lhHasOffset | rhHasOffset) {
        ...
        if (Turn::kCcw == thStartFromLhEnd && Turn::kCw == thEndFromRhStart) {
...
// start here
// if the pair of angles share an edge (e.g., endCheck == 0 && halfPlane.dot(test) >= 0)
// and one is a curve, use its other sweep to check the turn value
