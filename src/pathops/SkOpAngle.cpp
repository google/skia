...
// start here
// if a pair cross only slightly, ccwOf will return none
// instead, treat the slight crossing as if it wasn't there
// unsure how to detect / fix this
...
static double cross_check(const SkDVector& v1, const SkDVector& v2, bool check) {
    if (check) {
        return v1.crossCheck(v2);
    }
    return v1.cross(v2);
}

SkOpAngle::Turn SkOpAngle::ccwOf(const SkOpAngle* rh, bool rhCW, bool thisCCW)
        const {
    ...
    for (bool check : { true, true } ) {
        double endCheck = cross_check(sweep, *testV, check);
    #if 01 && DEBUG_ANGLE
        ...
        if (0 == endCheck) {
            ...
            if ((fPart.isCurve() || rh->fPart.isCurve())) {
                return this->ccwOf(rh, !rhCW, !thisCCW);
            }
        }
        if (commonPt) {
            return toTurn(endCheck > 0);
        }
        double startCheck = cross_check(sweep, startOffset, check);
        ...
        double reverseStartCheck = cross_check(reverseSweep, rhReverseStartV, check);
        SkDVector rhReverseEndV = rhReverseStartV + rhSweep;
        double reverseEndCheck = cross_check(reverseSweep, rhReverseEndV, check);
        if (same_side(reverseStartCheck, reverseEndCheck)) {
            return toTurn(reverseStartCheck < 0);
        }
        double rhEndCheck = cross_check(rhSweep, rhReverseEndV, check);
        double rhStartCheck = cross_check(rhSweep, *testV, check);
        double endSide = same_side_candidate(rhEndCheck, rhStartCheck);
        SkDVector rhReverseSweep = -rhSweep;
        double rhReverseEndCheck = cross_check(rhReverseSweep, rhReverseStartV, check);
        double rhReverseStartCheck = cross_check(rhReverseSweep, startOffset, check);
