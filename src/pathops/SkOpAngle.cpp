...
bool SkOpAngle::after(SkOpAngle* test) {
    ...
    this->debugAfter(lh, rh, &bugOut);
#endif
    if (lh->debugID() == 38 && this->debugID() == 84 && rh->debugID() == 23) {
        SkDebugf("");
    }
    ...
    */
    ...
    if (lhHasOffset | rhHasOffset) {
        bool lhSweepsCCW = (lhStart <= lhEnd) ^ lhX0;
        bool thSweepsCCW = (thStart <= thEnd) ^ thX0;
        bool rhSweepsCCW = (rhStart <= rhEnd) ^ rhX0;
        bool thStartCcwOfLhEnd = this->ccwOf(lh, lhSweepsCCW, !thSweepsCCW);
        bool thEndCcwOfRhStart = this->ccwOf(rh, !rhSweepsCCW, thSweepsCCW);
        if (thStartCcwOfLhEnd & !thEndCcwOfRhStart) {
            return COMPARE_RESULT(5, true); 
        }
        bool lhStartCCWofRhStart = lh->ccwOf(rh, !rhSweepsCCW, !lhSweepsCCW);
        bool lhEndCCWofRhStart = lh->fPart.isCurve()
                ? lh->ccwOf(rh, !rhSweepsCCW, lhSweepsCCW) : lhStartCCWofRhStart;
        if ((thStartCcwOfLhEnd | !thEndCcwOfRhStart) & lhStartCCWofRhStart & lhEndCCWofRhStart) {
            return COMPARE_RESULT(6, true);
        }
        bool lhOrRhIsCurve = lh->fPart.isCurve() || rh->fPart.isCurve();
        bool lhStartCCWofRhEnd, lhEndCCWofRhEnd;
        if (lhOrRhIsCurve) {
            if (rh->fPart.isCurve()) {
                lhStartCCWofRhEnd = lh->ccwOf(rh, rhSweepsCCW, !lhSweepsCCW);
                lhEndCCWofRhEnd = lh->fPart.isCurve()
                    ? lh->ccwOf(rh, rhSweepsCCW, !lhSweepsCCW) : lhStartCCWofRhEnd;
            } else {
                lhStartCCWofRhEnd = lhStartCCWofRhStart;
                lhEndCCWofRhEnd = lhEndCCWofRhStart;
            }
            lrOverlap = lhStartCCWofRhEnd != lhEndCCWofRhStart;
        } else {
            lrOverlap = false;
        }
        if (lhOrRhIsCurve || fPart.isCurve()) {
            // if lh, this, rh hulls don't overlap, then this is not between lh and rh
            bool thStartCcwOfRhEnd = rh->fPart.isCurve() && fPart.isCurve()
                    ? this->ccwOf(rh, rhSweepsCCW, !thSweepsCCW) : thEndCcwOfRhStart;
            bool thEndCcwOfLhStart = lh->fPart.isCurve() && fPart.isCurve()
                    ? this->ccwOf(lh, !lhSweepsCCW, thSweepsCCW) : thStartCcwOfLhEnd;
            ltrOverlap = thStartCcwOfRhEnd != thEndCcwOfRhStart
                    || thStartCcwOfLhEnd != thEndCcwOfLhStart;
        } else {
            ltrOverlap = false;
        }
        if (!lrOverlap & !ltrOverlap) {
            return COMPARE_RESULT(7, false);
        }
        if (lhHasOffset) {
...
// start here
// seems backwards -- this is asking 'which side of this is L (or R) on
// but shouldn't it be asking 'which side of L (or R) is this on?
bool SkOpAngle::ccwOf(const SkOpAngle* rh, bool rhCW, bool thisCCW) const {
    const SkDVector& halfPlane = fPart.fSweep[(int) thisCCW];
    double hPLenSq = halfPlane.lengthSquared();
    SkDVector rhSweep = rh->fPart.fSweep[(int) rhCW];
    rhSweep.setLengthSquared(hPLenSq);
    SkDPoint rhHullEnd = rh->fPart.fCurve[0];
    rhHullEnd += rhSweep;
    SkDVector test = rhHullEnd - fPart.fCurve[0];
    return halfPlane.crossCheck(test) > 0;
}
