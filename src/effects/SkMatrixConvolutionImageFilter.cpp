...
sk_sp<SkSpecialImage> SkMatrixConvolutionImageFilter::onFilterImage(SkSpecialImage* source,
    ...
    SkIPoint foo = { offset->fX - inputOffset.fX, offset->fY - inputOffset.fY };
    ...
    this->filterBorderPixels(inputBM, &dst, foo, top, srcBounds);
    this->filterBorderPixels(inputBM, &dst, foo, left, srcBounds);
    this->filterInteriorPixels(inputBM, &dst, foo, interior, srcBounds);
    this->filterBorderPixels(inputBM, &dst, foo, right, srcBounds);
    this->filterBorderPixels(inputBM, &dst, foo, bottom, srcBounds);
