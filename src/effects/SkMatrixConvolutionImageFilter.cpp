...
//        print_irect("unMungedSrcBounds", unMungedSrcBounds);
        ...
        SkIRect filterRect = SkIRect::MakeWH(fKernelSize.width(), fKernelSize.height());
        filterRect.offset(-fKernelOffset);

        if (!IntersectWithRepeat(&mungedSrcBounds, filterRect, originalSrcBounds)) {
            return nullptr;
        }
    } else {
