...
static uint8_t* create_circle_profile(float sigma, float circleR,
...
static sk_sp<GrTextureProxy> create_profile_texture(
  ...
  sk_sp<GrTextureProxy> blurProfile =
      resourceProvider->findProxyByUniqueKey(key);
  if (!blurProfile) {
    ...
    blurProfile = GrSurfaceProxy::MakeDeferred(
        resourceProvider, texDesc, SkBudgeted::kYes, profile.get(), 0);
    if (!blurProfile) {
      return nullptr;
    }
...
sk_sp<GrFragmentProcessor> GrCircleBlurFragmentProcessor::Make(
  ...
  sk_sp<GrTextureProxy> profile(create_profile_texture(
      resourceProvider, circle, sigma, &solidRadius, &textureRadius));
  if (!profile) {
    return nullptr;
  }
  return sk_sp<GrFragmentProcessor>(
      new GrCircleBlurFragmentProcessor(circle, textureRadius, solidRadius,
                                        std::move(profile), resourceProvider));
}
 ...
 public:
  ...
  void emitCode(EmitArgs& args) override {
    ...
    fragBuilder->codeAppendf(
        "vec2 vec = vec2((sk_FragCoord.x - %s.x) * %s.w, (sk_FragCoord.y - "
        "%s.y) * %s.w);\nfloat dist = length(vec) + (0.5 - %s.z) * %s.w;\n%s = "
        "%s * texture(%s, vec2(dist, 0.5)).%s.w;\n",
...
sk_sp<GrFragmentProcessor> GrCircleBlurFragmentProcessor::TestCreate(
  ...
  return GrCircleBlurFragmentProcessor::Make(testData->resourceProvider(),
                                             circle, sigma);
}
#endif
#endif
