...
public:
    ...
    void emitCode(EmitArgs& args) override {
        ...
        fragBuilder->codeAppendf(
                "mediump half2 vec = half2(half((sk_FragCoord.x - highfloat(%s.x)) * "
                "highfloat(%s.w)), half((sk_FragCoord.y - highfloat(%s.y)) * "
                "highfloat(%s.w)));\nmediump half dist = highfloat(length(vec)) + (0.5 - "
                "highfloat(%s.z)) * highfloat(%s.w);\n%s = %s * texture(%s, highfloat2(half2(dist, "
                "half(0.5)))).%s.w;\n",
