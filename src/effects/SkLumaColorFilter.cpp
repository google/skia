...
#if SK_SUPPORT_GPU
GrFragmentProcessor::MakeResult SkLumaColorFilter::asFragmentProcessor(
        std::unique_ptr<GrFragmentProcessor> inputFP, GrRecordingContext*,
        const GrColorInfo&) const {
    return GrFragmentProcessor::MakeSuccess(GrLumaColorFilterEffect::Make(std::move(inputFP)));
}
#endif
