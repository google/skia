...
public:
    ...
    void emitCode(EmitArgs& args) override {
        ...
        fragBuilder->codeAppendf(
                "half4 _tmpVar1;mediump half4 color = half4(%stexture(%s, %s).%s%s);\nmediump "
                "half4 mask_color = half4(texture(%s, %s).%s);\nif (highfloat(mask_color.w) < 0.5) "
                "{\n    if (color.w > %s) {\n        mediump half scale = %s / color.w;\n        "
                "color.xyz *= scale;\n        color.w = %s;\n    }\n} else if (color.w < %s) {\n   "
                " mediump half scale = highfloat(%s) / max(0.001, highfloat(color.w));\n    "
                "color.xyz *= scale;\n    color.w = %s;\n}\n%s = highfloat4(color);\n",
