...
using InvertStyle = SkDarkModeConfig::InvertStyle;

SkDarkModeFilter::SkDarkModeFilter(const SkDarkModeConfig& config) {
    fConfig = config;
...
sk_sp<SkColorFilter> SkDarkModeFilter::Make(const SkDarkModeConfig& config) {
    if (!config.isValid()) {
        return nullptr;
    }

    return sk_make_sp<SkDarkModeFilter>(config);
}

bool SkDarkModeFilter::onAppendStages(const SkStageRec& rec, bool shaderIsOpaque) const {
    ...
    // Linearize before applying dark mode filter.
...
skvm::Color SkDarkModeFilter::onProgram(skvm::Builder* p,
                                        skvm::Color c,
                                        SkColorSpace* dstCS,
    ...
    if (fConfig.fGrayscale) {
        skvm::F32 gray = p->mad(
                p->splat(SK_LUM_COEFF_R), c.r,
                p->mad(p->splat(SK_LUM_COEFF_G), c.g, p->mul(p->splat(SK_LUM_COEFF_B), c.b)));
...
void SkDarkModeFilter::flatten(SkWriteBuffer& buffer) const {
...
sk_sp<SkFlattenable> SkDarkModeFilter::CreateProc(SkReadBuffer& buffer) {
    SkDarkModeConfig config;
    config.fGrayscale = buffer.readBool();
    config.fInvertStyle = buffer.read32LE(InvertStyle::kLast);
    config.fContrast = buffer.readScalar();

    return SkDarkModeFilter::Make(config);
}

#if SK_SUPPORT_GPU
class DarkModeFilterEffect : public GrFragmentProcessor {
public:
    static std::unique_ptr<GrFragmentProcessor> Make(const SkDarkModeConfig& config,
                                                     bool linearize) {
        return std::unique_ptr<GrFragmentProcessor>(new DarkModeFilterEffect(config, linearize));
    }

    const char* name() const override { return "DarkModeFilter"; }

    const SkDarkModeConfig& config() const { return fConfig; }
...
private:
    DarkModeFilterEffect(const SkDarkModeConfig& config, bool linearize)
            : INHERITED(kDarkModeFilterEffect_ClassID, kNone_OptimizationFlags)
            , fConfig(config)
    ...
    bool onIsEqual(const GrFragmentProcessor& other) const override {
        const DarkModeFilterEffect& that = other.cast<DarkModeFilterEffect>();
        return fConfig.fGrayscale == that.fConfig.fGrayscale &&
               fConfig.fInvertStyle == that.fConfig.fInvertStyle &&
               fConfig.fContrast == that.fConfig.fContrast && fLinearize == that.fLinearize;
    }
...
class GLDarkModeFilterEffect : public GrGLSLFragmentProcessor {
public:
...
GrGLSLFragmentProcessor* DarkModeFilterEffect::onCreateGLSLInstance() const {
    return new GLDarkModeFilterEffect();
}

void DarkModeFilterEffect::onGetGLSLProcessorKey(const GrShaderCaps& caps,
                                                 GrProcessorKeyBuilder* b) const {
    GLDarkModeFilterEffect::GenKey(*this, caps, b);
}

void GLDarkModeFilterEffect::onSetData(const GrGLSLProgramDataManager& pdm,
                                       const GrFragmentProcessor& proc) {
    const DarkModeFilterEffect& hcfe = proc.cast<DarkModeFilterEffect>();
    pdm.set1f(fContrastUni, hcfe.config().fContrast);
}

void GLDarkModeFilterEffect::GenKey(const GrProcessor& proc,
                                    const GrShaderCaps&,
...
void GLDarkModeFilterEffect::emitCode(EmitArgs& args) {
    const DarkModeFilterEffect& hcfe = args.fFp.cast<DarkModeFilterEffect>();
    const SkDarkModeConfig& config = hcfe.config();
    ...
    if (config.fGrayscale) {
        fragBuilder->codeAppendf("half luma = dot(color, half4(%f, %f, %f, 0));", SK_LUM_COEFF_R,
                                 SK_LUM_COEFF_G, SK_LUM_COEFF_B);
...
std::unique_ptr<GrFragmentProcessor> SkDarkModeFilter::asFragmentProcessor(
        GrRecordingContext*, const GrColorInfo& csi) const {
    bool linearize = !csi.isLinearlyBlended();
    return DarkModeFilterEffect::Make(fConfig, linearize);
}
#endif
