...
static void apply_morphology_pass(GrTextureProvider* provider,
    ...
    if (direction == Gr1DKernelEffect::kX_Direction) {
        bounds[0] = (SkIntToScalar(srcRect.left()) + 0.5f) / textureProxy->width();
        bounds[1] = (SkIntToScalar(srcRect.right()) - 0.5f) / textureProxy->width();
        ...
        bounds[0] = (SkIntToScalar(srcRect.top()) + 0.5f) / textureProxy->height();
        bounds[1] = (SkIntToScalar(srcRect.bottom()) - 0.5f) / textureProxy->height();
