...
void SkAmbientShadowTessellator::lineTo(const SkPoint& p)  {
    ...
    if (fInitPoints.count() < 2) {
        *fInitPoints.push() = p;
        ...
        // add first quad
        SkVector normal;
        normal.fX = fDirection*(fInitPoints[0].fY - fInitPoints[1].fY);
        normal.fY = fDirection*(fInitPoints[1].fX - fInitPoints[0].fX);
        if (!normal.normalize()) {
        ...
        *fPositions.push() = fInitPoints[0] + normal;
        *fColors.push() = 0;
        *fPositions.push() = fInitPoints[1];
        *fColors.push() = fUmbraColor;
        *fPositions.push() = fInitPoints[1] + normal;
        *fColors.push() = 0;
    ...
    // add next quad
    SkVector normal;
    normal.fX = fDirection*(fPositions[fPrevInnerIndex].fY - p.fY);
    normal.fY = fDirection*(p.fX - fPositions[fPrevInnerIndex].fX);
    if (!normal.normalize()) {
        //*** better error recovery?
        return;
    }
