...
static void compute_normal(const SkPoint& p0, const SkPoint& p1, SkScalar radius, SkScalar dir,
                           SkVector* newNormal) {
    SkVector normal;
...
SkAmbientShadowTessellator::SkAmbientShadowTessellator(const SkMatrix& viewMatrix,
    ...
    SkVector normal;
    compute_normal(fPositions[fPrevInnerIndex], fPositions[fFirstVertex], fRadius, fDirection,
                   &normal);
    addArc(normal);
...
void SkAmbientShadowTessellator::lineTo(const SkPoint& p)  {
    ...
    if (fInitPoints.count() < 2) {
        *fInitPoints.push() = p;
        ...
        // add first quad
        compute_normal(fInitPoints[0], fInitPoints[1], fRadius, fDirection,
                       &fFirstNormal);

        fFirstVertex = fPositions.count();
        fPrevNormal = fFirstNormal;
        fPrevInnerIndex = fFirstVertex + 2; 
        ...
        *fIndices.push() = fFirstVertex + 1;
        *fIndices.push() = fFirstVertex + 3;
        *fIndices.push() = fPrevInnerIndex;

        // if transparent, add points to first one in array and add to center fan
        if (fTransparent) {
    ...
    SkVector normal;
    compute_normal(fPositions[fPrevInnerIndex], p, fRadius, fDirection,
                   &normal);
