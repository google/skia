...
skif::IRect<In::kLayer, For::kOutput> SkComposeImageFilterImpl::onFilterOutputBounds(
    ...
    const SkImageFilter_Base* outer = as_IFB(this->getInput(0));
    const SkImageFilter_Base* inner = as_IFB(this->getInput(1));
    SkASSERT(outer && inner); // Sanity check, ctor should have caught this

    // First determine what the inner filter would produce when processing 'contentBounds'
    skif::IRect<In::kLayer, For::kOutput> innerOut = inner->filterOutputBounds(
            contentBounds, layerMatrix);
    // Use the output of the inner filter as the input to the outer filter
    return outer->filterOutputBounds(skif::LayerCast<For::kInput>(innerOut), layerMatrix);
}
...
skif::IRect<In::kLayer, For::kInput> SkComposeImageFilterImpl::onFilterLayerBounds(
    ...
    skif::IRect<In::kLayer, For::kInput> outerInput = outer->filterLayerBounds(
            targetOutputBounds, layerMatrix, &originalInput);
    // Use the required input as the target output for the inner filter
    return inner->filterLayerBounds(skif::LayerCast<For::kOutput>(outerInput), layerMatrix,
                                    &outerInput);
}
