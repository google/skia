...
sk_sp<SkSpecialImage> SkXfermodeImageFilterImpl::filterImageGPU(
    ...
    if (backgroundProxy) {
        ...
        bgFP = GrDomainEffect::Make(std::move(bgFP), bgMatrix, GrTextureDomain::MakeTexelDomain(bgSubset, GrTextureDomain::kDecal_Mode), GrTextureDomain::kDecal_Mode);
    ...
    if (foregroundProxy) {
        ...
        auto foregroundFP = GrSimpleTextureEffect::Make(std::move(foregroundProxy), fgColorType, SkMatrix::I(),
                                                        GrSamplerState::Filter::kNearest);
        foregroundFP = GrDomainEffect::Make(std::move(foregroundFP), fgMatrix, GrTextureDomain::MakeTexelDomain(fgSubset, GrTextureDomain::kDecal_Mode), GrTextureDomain::kDecal_Mode);
