...
sk_sp<SkSpecialImage> SkXfermodeImageFilterImpl::filterImageGPU(
    ...
    if (backgroundProxy) {
        ...
        bgFP = GrSimpleTextureEffect::Make(std::move(backgroundProxy), background->alphaType(), bgMatrix,
    ...
    if (foregroundProxy) {
        ...
        auto foregroundFP =
                GrSimpleTextureEffect::Make(std::move(foregroundProxy), foreground->alphaType(), fgMatrix,
                                            GrSamplerState::Filter::kNearest);
