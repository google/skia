...
sk_sp<SkSpecialImage> ArithmeticImageFilterImpl::filterImageGPU(
    ...
    if (backgroundProxy) {
        ...
        bgFP = GrSimpleTextureEffect::Make(std::move(backgroundProxy), background->alphaType(), backgroundMatrix,
    ...
    if (foregroundProxy) {
        ...
        auto foregroundFP =
                GrSimpleTextureEffect::Make(std::move(foregroundProxy), foreground->alphaType(), foregroundMatrix,
                                            GrSamplerState::Filter::kNearest);
