...
sk_sp<SkSpecialImage> SkImageSourceImpl::onFilterImage(const SkFilterContext& ctx,
    ...
    if (fSrcRect == bounds) {
        ...
        if (dstRect.width() == bounds.width() && dstRect.height() == bounds.height() &&
            ...
            return SkSpecialImage::MakeFromImage(ctx.getGrContext(),
                                                 SkIRect::MakeWH(fImage->width(), fImage->height()),
                                                 fImage, &ctx.sourceImage()->props());
        }
    }
