...
sk_sp<SkSpecialImage> SkMatrixConvolutionImageFilterImpl::onFilterImage(const Context& ctx,
    ...
    if (!fConvolveAlpha && inputBM.alphaType() == kPremul_SkAlphaType) {
        SkBitmap upm;
        upm.allocPixels(inputBM.info().makeAlphaType(kUnpremul_SkAlphaType));
        inputBM.readPixels(upm.pixmap(), 0,0);
        inputBM = upm;
    }
