...
sk_sp<SkShader> make_normal_shader(sk_sp<SkShader> alphaMap,
    ...
    static const SkRuntimeEffect* effect = SkMakeRuntimeEffect(SkRuntimeEffect::MakeForShader,
        ...
        "half get_clamped_alpha(float2 coord) {"
            "return alphaMap.eval(clamp(coord, edgeBounds.LT, edgeBounds.RB)).a;"
        "}"

        "half4 main(float2 coord) {"
            "half3 alphaC0 = half3("
                "get_clamped_alpha(coord + float2(-1,-1)),"
                "get_clamped_alpha(coord + float2(-1, 0)),"
                "get_clamped_alpha(coord + float2(-1, 1)));"
            "half3 alphaC1 = half3("
                "get_clamped_alpha(coord + float2( 0,-1)),"
                "get_clamped_alpha(coord + float2( 0, 0)),"
                "get_clamped_alpha(coord + float2( 0, 1)));"
            "half3 alphaC2 = half3("
                "get_clamped_alpha(coord + float2( 1,-1)),"
                "get_clamped_alpha(coord + float2( 1, 0)),"
