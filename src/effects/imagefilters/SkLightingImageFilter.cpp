...
#if SK_SUPPORT_GPU
    sk_sp<SkSpecialImage> filterImageGPU(const SkFilterContext& ctx,
...
sk_sp<SkSpecialImage> SkLightingImageFilterInternal::filterImageGPU(
    ...
    sk_sp<GrRenderTargetContext> renderTargetContext(
            context->priv().makeDeferredRenderTargetContext(
                    ...
                    offsetBounds.width(),
                    offsetBounds.height(),
                    SkColorTypeToGrColorType(ctx.colorType()),
...
sk_sp<SkSpecialImage> SkDiffuseLightingImageFilter::onFilterImage(const SkFilterContext& ctx,
...
sk_sp<SkSpecialImage> SkSpecularLightingImageFilter::onFilterImage(const SkFilterContext& ctx,
                                                                   SkIPoint* offset) const {
    SkIPoint inputOffset = SkIPoint::Make(0, 0);
    sk_sp<SkSpecialImage> input(this->filterInput(0, ctx, &inputOffset));
    if (!input) {
