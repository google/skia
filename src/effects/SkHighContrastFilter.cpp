...
void GLHighContrastFilterEffect::GenKey(
  ...
  b->add32(static_cast<uint32_t>(hcfe.config().fContrast * 65536));
}
...
void GLHighContrastFilterEffect::emitCode(EmitArgs& args) {
    ...
    if (fConfig.fInvertStyle == HighContrastInvertStyle::kInvertLightness) {
    ...
    // Contrast.
    if (fConfig.fContrast != 0) {
        float m = (1.0f + fConfig.fContrast) / (1.0f - fConfig.fContrast);
        fragBuilder->codeAppendf("float m = %f;", m);
    ...
    fragBuilder->codeAppendf("%s.a = %s.a;", args.fOutputColor, args.fInputColor);
}
