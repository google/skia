...
static skvm::Color rgb_to_hsl(skvm::Builder* p, skvm::Color c) {
...
static skvm::Color hsl_to_rgb(skvm::Builder* p, skvm::Color c) {
    // See GrRGBToHSLFilterEffect.fp

    auto h = c.r,
         s = c.g,
         l = c.b,
         x = p->mul(p->sub(p->splat(1.0f), p->abs(p->sub(p->add(l,l), p->splat(1.0f)))), s);

    auto hue_to_rgb = [&](auto hue) {
        auto q = p->sub(p->abs(p->mad(p->fract(hue),p->splat(6.0f), p->splat(-3.0f))), p->splat(1.0f));
             q = p->clamp(q, p->splat(0.0f), p->splat(1.0f));
        return p->mad(p->sub(q, p->splat(0.5f)), x, l);
    };
...
skvm::Color SkHighContrast_Filter::onProgram(skvm::Builder* p, skvm::Color c, SkColorSpace* dstCS,
    ...
    if (dstCS) {
        ...
        sk_srgb_singleton()->transferFn(&tf);   // legacy value {2,1, 0,0,0,0,0};
    }
    c = sk_program_transfer_fn(p, uniforms, tf, c);

    if (fConfig.fGrayscale) {
        auto gray = p->mad(p->splat(SK_LUM_COEFF_R),c.r,
    ...
    if (fConfig.fInvertStyle == InvertStyle::kInvertBrightness) {
        ...
        c = {inv(c.r), inv(c.g), inv(c.b), c.a};
    } else if (fConfig.fInvertStyle == InvertStyle::kInvertLightness) {
        c = rgb_to_hsl(p, c);
        c.b = p->sub(p->splat(1.0f), c.b);   // c.b is luminance
        c = hsl_to_rgb(p, c);
    }
    ...
    if (fConfig.fContrast != 0.0) {
        ...
        auto M = p->uniformF(uniforms->pushF(m));
        auto B = p->uniformF(uniforms->pushF(b));
        c = {p->mad(M,c.r, B), p->mad(M,c.g, B), p->mad(M,c.b, B), c.a};
    }

    auto clamp = [p](auto x) {
        return p->clamp(x, p->splat(0.0f), p->splat(1.0f));
    };
    c = {clamp(c.r), clamp(c.g), clamp(c.b), c.a};

    // Re-encode back from linear.
    if (dstCS) {
        dstCS->invTransferFn(&tf);
    } else {
        sk_srgb_singleton()->invTransferFn(&tf);    // legacy value {0.5f,1, 0,0,0,0,0};
    }
    c = sk_program_transfer_fn(p, uniforms, tf, c);
