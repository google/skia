...
sk_sp<SkColorFilter> SkHighContrastFilter::Make(const SkHighContrastConfig& config) {
    ...
    // The sksl code above all asssumes linear unpremul inputs and outputs.
    skcms_TransferFunction linear = SkNamedTransferFn::kLinear;
    SkAlphaType unpremul = kUnpremul_SkAlphaType;

    // Not sure treating legacy as gamma=2 is a strictly _good_ idea,
    // but it is important for pixel-perfect compatibility.
    sk_sp<SkColorSpace> dstNullCS = SkColorSpace::MakeRGB({2,1,0,0,0,0,0}, SkNamedGamut::kSRGB);

    sk_sp<SkColorFilter> input = nullptr;
    return SkColorFilters::WithColorXform(
            effect->makeColorFilter(SkData::MakeWithCopy(&uniforms,sizeof(uniforms)), &input, 1),
            &linear, nullptr/*use dst gamut*/, &unpremul, dstNullCS);
#endif
}
