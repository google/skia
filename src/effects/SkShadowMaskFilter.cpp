...
bool SkShadowMaskFilterImpl::directFilterRRectMaskGPU(GrContext*,
    ...
    if (fAmbientAlpha > 0.0f) {
        ...
        if (isRect) {
            ...
            ambientRRect = rrect.makeOffset(ambientPathOutset, ambientPathOutset);
        }
        ...
        paint.setColor4f(GrColor4f(fAmbientAlpha/255.f, (iDevSpaceAmbientRadius >> 8)/255.f,
                                   (iDevSpaceAmbientRadius & 0xff)/255.f, 4.0f * pad/255.f));

        sk_sp<GrFragmentProcessor> fp(GrShadowEdgeEffect::Make(GrShadowEdgeEffect::kGaussian_Type));
        paint.addCoverageFragmentProcessor(std::move(fp));
        drawContext->drawRRect(clip, paint, viewMatrix, ambientRRect,
                               GrStyle(ambientStrokeRec, nullptr));
    }

    if (fSpotAlpha > 0.0f) {
        float zRatio = fOccluderHeight / (fLightPos.fZ - fOccluderHeight);
        // clamp
        if (zRatio < 0.0f) {
            zRatio = 0.0f;
        ...
        paint.setColor4f(GrColor4f(fAmbientAlpha / 255.f, (iDevSpaceSpotRadius >> 8) / 255.f,
                         (iDevSpaceSpotRadius & 0xff) / 255.f, 0));
        spotShadowRRect.offset(spotOffset.fX, spotOffset.fY);

        sk_sp<GrFragmentProcessor> fp(GrShadowEdgeEffect::Make(GrShadowEdgeEffect::kGaussian_Type));
        paint.addCoverageFragmentProcessor(std::move(fp));

        drawContext->drawRRect(clip, paint, viewMatrix, spotShadowRRect,
