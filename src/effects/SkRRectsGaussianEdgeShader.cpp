...
public:
    ...
    public:
        ...
        void emitModeCode(Mode mode,
            ...
            // Map all the cases to the lower left quadrant.
            ...
            case kSimpleCircular_Mode:
                ...
                //    the fractional position from the corner center to the corner's circle
                //    the distance to the bounding rectangle
                ...
                fragBuilder->codeAppendf("float tol = max(%s, %s.%c);",
                                         radName, radiiName, indices[0]);
                fragBuilder->codeAppendf("float insetDist = min(tol, min(%s.%c, %s.%c));",
                                         sizesName, indices[0], sizesName, indices[1]);
                // "maxValue" is a correction term for if the blurRadius is larger than the
                // size of the RRect. We'll never go all the way to black.
                ...
                fragBuilder->codeAppend("float bSq = b*b;"); //max(b*b, fourAC);");

                fragBuilder->codeAppend("float coneDist = max(0.0, "
                                                            "(-b + sqrt(bSq - fourAC)) / 2.0);");
