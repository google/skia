...
static SkScalar compute_rrect_normalized_dist(const SkRRect& rr, const SkPoint& p, SkScalar rad) {
    ...
    if (rr.getType() == SkRRect::kOval_Type && radii.fX < rad) {
        // In case that our circle's radii < rad, we manually set innerRect to the center point
        // (so innerRect has 0 width/height instead negative ones) to match our previous
        // implementation. This is different from other types (rrect, rect) where innerRect
        // could have negative width/height (even in our previous implementation).
        innerRect = rr.rect().makeInset(radii.fX, radii.fY);
        rrr -= rad - radii.fX; // This is equivalent to enlarge the dist by rad - radii
    }
...
public:
    ...
    public:
        ...
        void emitModeCode(Mode mode,
            ...
            // w and h are half width and height of the inner rect
            ...
            // implementation. This is different from other types (rrect, rect) where innerRect
            // could have negative width/height (even in our previous implementation).
            if (mode == kCircle_Mode) {
                fragBuilder->codeAppendf("w = 0;");
                fragBuilder->codeAppendf("h = 0;");

                // This is equivalent to enlarge the dist by rad - radii
                fragBuilder->codeAppendf("rrr -= max(0, %s - %s.%c);",
                                         radName, sizesName, indices[0]);
            }
