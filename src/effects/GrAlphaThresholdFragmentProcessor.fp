...
//    vec4 color = texture(image, sk_TransformedCoords2D[0], colorXform);
    vec4 tmpColor;
    vec4 color = (tmpColor = texture(image, sk_TransformedCoords2D[0], colorXform) , colorXform != mat4(1) ? vec4(clamp((colorXform * vec4(tmpColor.rgb, 1.0)).rgb, 0.0, tmpColor.a), tmpColor.a) : tmpColor);
