...
sk_sp<SkBlender> SkBlenders2::Arithmetic(float k1, float k2, float k3, float k4,
                                        bool enforcePremul) {
    ...
    auto [effect, errorText] = SkRuntimeEffect::MakeForBlender(SkString(prog));
    if (!effect) {
        SkDebugf("%s\n", errorText.c_str());
        return nullptr;
    }
    ...
    return effect->makeBlender(SkData::MakeWithCopy(array, sizeof(array)));
}
