...
bool SkSweepGradient::onAppendStages(
    SkRasterPipeline* pipeline, SkColorSpace* dstCS, SkArenaAlloc* alloc,
    const SkMatrix& ctm, const SkPaint& paint,
    const SkMatrix* localM) const {

    auto adjustMatrix = [&] (SkMatrix* matrix) {
        *matrix = SkMatrix::Concat(SkMatrix::MakeTrans(-fCenter.fX, -fCenter.fY), *matrix);
        return true;
    };

    auto addStage = [&] (SkRasterPipeline* pipeline) {
        pipeline->append(SkRasterPipeline::xy_to_polar_unit);
        return true;
    };
