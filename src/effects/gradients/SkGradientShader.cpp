    ...
    const SkMatrix* localM) const
{
    ...
    if (fColorCount == 2 && fOrigPos == nullptr) {
        ...
        if (fOrigPos == nullptr) {
            ...
            for (int i = 0; i < 4; i++) {
                ctx->fs[i] = alloc->makeArray<float>(n+1);
                ctx->bs[i] = alloc->makeArray<float>(n+1);
            }
            ...
            auto init_stop = [&](int stop, SkPM4f c_l, SkPM4f c_r) {
                auto Fs = SkPM4f::From4f((c_r.to4f() - c_l.to4f()) * n);
                auto Bs = SkPM4f::From4f(c_l.to4f() - (Fs.to4f() * ((float)stop / n)));
                add_stop(stop, Fs, Bs);
            };
            ...
            add_stop(n, SkPM4f::FromPremulRGBA(0,0,0,0), c_l);
