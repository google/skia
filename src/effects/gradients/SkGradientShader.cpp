    ...
    const SkMatrix* localM) const
{
    ...
    if (fColorCount == 2 && fOrigPos == nullptr) {
        ...
        auto add_stop = [&](size_t stop, SkPM4f Fs, SkPM4f Bs) {
            (ctx->fs[0])[stop] = Fs.r();
            (ctx->fs[1])[stop] = Fs.g();
            (ctx->fs[2])[stop] = Fs.b();
            (ctx->fs[3])[stop] = Fs.a();
        ...
        for (int i = 0; i < 4; i++) {
            ctx->fs[i] = alloc->makeArray<float>(std::max(fColorCount, 8));
            ctx->bs[i] = alloc->makeArray<float>(std::max(fColorCount, 8));
        }
        ...
        if (fOrigPos == nullptr) {
            ...
            pipeline->append(SkRasterPipeline::evenly_spaced_linear_gradient, ctx);
        } else {
            // Handle arbitrary stops.

            ctx->ts = alloc->makeArray<float>(fColorCount);
            ...
            add_stop(stopCount, SkPM4f::FromPremulRGBA(0,0,0,0), c_l);
