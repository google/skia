...
bool SkLinearGradient::onAppendStages(SkRasterPipeline* p, SkColorSpace* cs, SkArenaAlloc* alloc,
    ...
    if (dstToUnit.asAffine(ctx->fMatrix)) {
        p->append(SkRasterPipeline::matrix_2x3, ctx->fMatrix);
    } else {
        dstToUnit.get9(ctx->fMatrix);
    ...
    switch (fTileMode) {
        case kClamp_TileMode:
            ctx->fLimit += 0.5f; // why is clamp_x offsetting its limit?
            p->append(SkRasterPipeline::clamp_x,  &ctx->fLimit);
            break;
        case kMirror_TileMode: p->append(SkRasterPipeline::mirror_x, &ctx->fLimit); break;
        case kRepeat_TileMode: p->append(SkRasterPipeline::repeat_x, &ctx->fLimit); break;
    }
    ...
    p->append(SkRasterPipeline::linear_gradient_2stops, ctx);
