...
bool SkOpPE::onComputeFastBounds(SkRect* bounds) const {
    if (!bounds) {
        return (!SkToBool(fOne) || ComputeFastBounds(fOne.get(), nullptr)) &&
               (!SkToBool(fTwo) || ComputeFastBounds(fTwo.get(), nullptr));
    }

    // bounds will hold the result of the fOne while b2 holds the result of fTwo's fast bounds
    SkRect b2 = *bounds;
    if (fOne && !ComputeFastBounds(fOne.get(), bounds)) {
        return false;
    }
    if (fTwo && !ComputeFastBounds(fTwo.get(), &b2)) {
        return false;
    }
...
bool SkStrokePE::onComputeFastBounds(SkRect* bounds) const {
    if (bounds) {
        SkStrokeRec rec(SkStrokeRec::kFill_InitStyle);
        rec.setStrokeStyle(fWidth);
        rec.setStrokeParams(fCap, fJoin, fMiter);
