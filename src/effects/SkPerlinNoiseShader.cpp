    ...
    void init(SkScalar seed)
    {
        ...
        for (int channel = 0; channel < 4; ++channel) {
            for (int i = 0; i < kBlockSize; ++i) {
                ...
                fNoise[channel][i][0] = SkScalarRoundToInt((fGradient[channel][i].fX + 1) * gHalfMax16bits);
                fNoise[channel][i][1] = SkScalarRoundToInt((fGradient[channel][i].fY + 1) * gHalfMax16bits);
            }
        }
    }
