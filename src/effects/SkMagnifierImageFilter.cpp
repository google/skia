...
public:
    ...
    const SkIRect& bounds1() const { return fBounds1; }    // Bounds of source image.
...
void GrGLMagnifierEffect::onSetData(const GrGLSLProgramDataManager& pdman,
...

    {
        ...
        if (tex->origin() != kTopLeft_GrSurfaceOrigin) {
            y = 1.0f - (zoom.srcRect().height() / zoom.bounds1().height()) - y;
        }
    ...
    {
        SkScalar y = zoom.bounds1().y() * invH;
        if (tex->origin() != kTopLeft_GrSurfaceOrigin) {
            y = 1.0f - y;
        }

        pdman.set4f(fBoundsVar,
                    zoom.bounds1().x() * invW, y, tex->width() * invW, tex->height() * invH);
    }
...
bool GrMagnifierEffect::onIsEqual(const GrFragmentProcessor& sBase) const {
    ...
    return (this->fBounds1 == s.fBounds1 &&
