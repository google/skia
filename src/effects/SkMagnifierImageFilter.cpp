...
void GrGLMagnifierEffect::emitCode(EmitArgs& args) {
    ...
    fragBuilder->codeAppendf("\t\thalf2 coord = %s;\n", coords2D.c_str());
    fragBuilder->codeAppendf("\t\thalf2 zoom_coord = %s + %s * %s;\n",
    ...
    fragBuilder->codeAppendf("\t\thalf2 delta = (coord - %s.xy) * %s.zw;\n", bounds, bounds);
    ...
    fragBuilder->codeAppend("\t\t\thalf2 delta_squared = delta * delta;\n");
    fragBuilder->codeAppend("\t\t\tweight = min(min(delta_squared.x, delta_squared.y), 1.0);\n");
    fragBuilder->codeAppend("\t\t}\n");

    fragBuilder->codeAppend("\t\thalf2 mix_coord = mix(coord, zoom_coord, weight);\n");
