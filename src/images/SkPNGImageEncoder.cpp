...
public:
    ...
    bool setICC(SkColorSpace* colorSpace);
    bool writeInfo(const SkPixmap& src);
...
bool SkPngEncoderMgr::setICC(SkColorSpace* colorSpace) {
    if (setjmp(png_jmpbuf(fPngPtr))) {
        return false;
    }
    ...
    if (colorSpace) {
        if (colorSpace->isSRGB()) {
            ...
            if (icc) {
                set_icc(fPngPtr, fInfoPtr, std::move(icc));
            }
        }
    }
...
bool SkPngEncoderMgr::writeInfo(const SkPixmap& src) {
    if (setjmp(png_jmpbuf(fPngPtr))) {
        return false;
    }

    png_write_info(fPngPtr, fInfoPtr);
    if (kRGBA_F16_SkColorType == src.colorType() && kOpaque_SkAlphaType == src.alphaType()) {
...
std::unique_ptr<SkPngEncoder> SkPngEncoder::Make(SkWStream* dst, const SkPixmap& src,
    ...
    if (!encoderMgr->setICC(src.colorSpace())) {
        return nullptr;
    }

    if (!encoderMgr->writeInfo(src)) {
        return nullptr;
    }
