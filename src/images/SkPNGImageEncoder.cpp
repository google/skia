...
static transform_scanline_proc choose_proc(const SkImageInfo& info) {
    const bool isGammaEncoded = info.colorSpace() && info.colorSpace()->gammaCloseToSRGB();
    switch (info.colorType()) {
        case kRGBA_8888_SkColorType:
            switch (info.alphaType()) {
                ...
                case kPremul_SkAlphaType:
                    return isGammaEncoded ? transform_scanline_rgbA :
                                            transform_scanline_legacy_rgbA;
                default:
                    SkASSERT(false);
                    return nullptr;
            }
        case kBGRA_8888_SkColorType:
            switch (info.alphaType()) {
                ...
                case kPremul_SkAlphaType:
                    return isGammaEncoded ? transform_scanline_bgrA :
                                            transform_scanline_legacy_bgrA;
                default:
                    SkASSERT(false);
                    return nullptr;
            }
...
static bool do_encode(SkWStream* stream, const SkPixmap& pixmap,
    ...
    */
    ...
    if (kIndex_8_SkColorType == pixmap.colorType()) {
        ...
        SkASSERT(colorTable);
        int numTrans = pack_palette(colorTable, paletteColors, trans, pixmap.alphaType());
