 ...
 */
bool choose_proc(J_COLOR_SPACE* jpegColorType, int* numComponents, transform_scanline_proc* proc,
    ...
    *proc = nullptr;
    switch (info.colorType()) {
        ...
        case kGray_8_SkColorType:
            SkASSERT(kOpaque_SkAlphaType == info.alphaType());
...
bool SkEncodeImageAsJPEG(SkWStream* stream, const SkPixmap& pixmap, int quality) {
    ...
    // Declare before set call setjmp.
    ...
    int numComponents;
    transform_scanline_proc proc = nullptr;
    if (!choose_proc(&jpegColorSpace, &numComponents, &proc, pixmap.info())) {
        return false;
    }
