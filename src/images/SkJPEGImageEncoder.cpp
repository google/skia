...
class SkQJpegEncoderMgr : SkNoncopyable {
public:
     ...
     */
    static std::unique_ptr<SkQJpegEncoderMgr> Make(SkWStream* stream) {
        return std::unique_ptr<SkQJpegEncoderMgr>(new SkQJpegEncoderMgr(stream));
    }
...
private:
    ...
    SkQJpegEncoderMgr(SkWStream* stream)
        : fDstMgr(stream)
        , fProc(nullptr)
    {
...
bool SkQJpegEncoderMgr::setParams(const SkImageInfo& srcInfo) {
...
public:
    SkJpegEncoder_Base(std::unique_ptr<SkQJpegEncoderMgr> encoderMgr, const SkPixmap& src);

    bool onEncodeRows(int numRows);

private:
    std::unique_ptr<SkQJpegEncoderMgr> fEncoderMgr;
    SkPixmap                          fSrc;
    int                               fCurrRow;
...
std::unique_ptr<SkJpegEncoder> SkJpegEncoder::Make(SkWStream* dst, const SkPixmap& src,
    ...
    std::unique_ptr<SkQJpegEncoderMgr> encoderMgr = SkQJpegEncoderMgr::Make(dst);
...
SkJpegEncoder_Base::SkJpegEncoder_Base(std::unique_ptr<SkQJpegEncoderMgr> encoderMgr,
                                       const SkPixmap& src)
