...
/**
 * Blend 8888 onto black.
 * Note that blending onto black (all zeros) is equivalent to multiplying r, g, and b by alpha.
 */
template <bool kIsPremul>
static inline void transform_scanline_blend_legacy(char* SK_RESTRICT dst,
                                                   const char* SK_RESTRICT src,
                                                   int width, int, const SkPMColor*) {
    SkOpts::RGBA_to_rgbA((uint32_t*)dst, (const uint32_t*)src, width);
    if (!kIsPremul) {
        SkOpts::RGBA_to_rgbA((uint32_t*)dst, (const uint32_t*)dst, width);
    }
}

/**
 * Blend 8888 onto black linearly.
 * Note that blending onto black (all zeros) is equivalent to multiplying r, g, and b by alpha.
 */
template <bool kIsPremul>
static inline void transform_scanline_blend_linear(char* SK_RESTRICT dst,
                                                   const char* SK_RESTRICT src,
                                                   int width, int, const SkPMColor*) {
    SkRasterPipeline p;
...
/**
 * Blend F16 onto black.
 * Note that blending onto black (all zeros) is equivalent to multiplying r, g, and b by alpha.
 */
template <bool kIsPremul>
static inline void transform_scanline_F16_blend(char* SK_RESTRICT dst, const char* SK_RESTRICT src,
                                                int width, int, const SkPMColor*) {
    SkRasterPipeline p;
    p.append(SkRasterPipeline::load_f16, (const void**) &src);
    if (!kIsPremul) {
