...
static inline sk_sp<SkData> icc_from_color_space(SkColorType ct, const SkColorSpace& cs) {
    const SkColorSpace* to_use = &cs;
    sk_sp<SkColorSpace> owned;
    if (ct == kRGBA_F16_SkColorType) {
        // We'll be converting to 8 or 16-bit sRGB, so we'd better tag it that way.
    ...
    if (to_use->isNumericalTransferFn(&fn) && to_use->toXYZD50(&toXYZD50)) {
