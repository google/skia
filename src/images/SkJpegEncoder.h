...
public:
    ...
    /**
     *  Encode |numRows| rows of input.  Returns true on success.  If the caller requests more
     *  rows than are remaining in the src, this will silently encode all of the remaining rows.
     *
     *  Returns false on failure.  This should only fail if numRows <= 0 or if the encode is
     *  already complete.
     */
