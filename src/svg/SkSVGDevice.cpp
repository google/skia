...
#include "../codec/SkJpegCodec.h"
#include "../codec/SkPngCodec.h"
...
sk_sp<SkData> AsDataUri(SkImage* image) {
    ...
    } else if (SkJpegCodec::IsJpeg(src, imageData->size())) {
...
void SkSVGDevice::AutoElement::addImageShaderResources(const SkShader* shader, const SkPaint& paint,
    ...
    if (!image) {
        SkDebugf("error: called addImageShaderResources on a non-image");
        return;
    }
...
void SkSVGDevice::drawRect(const SkRect& r, const SkPaint& paint) {
    // in order to reset the viewport, and ensure that repeating backgrounds
    // start at the rect, we wrap rects in another svg
    AutoElement svg("svg", fWriter, fResourceBucket.get(), MxCp(this), paint);
