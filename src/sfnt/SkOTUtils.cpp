...
SkData* SkOTUtils::RenameFont(SkStreamAsset* fontData, const char* fontName, int fontNameLen) {
    ...
    for (const auto& encoding : encodings) {
        for (const auto& name : names) {
            nameRecords[nrIdx].platformID.value = SkOTTableName::Record::PlatformID::Windows;
            nameRecords[nrIdx].encodingID.windows.value = encoding;
            nameRecords[nrIdx].languageID.windows.value = SkOTTableName::Record::LanguageID::Windows::English_UnitedStates;
            nameRecords[nrIdx].nameID.predefined.value = name;
            nameRecords[nrIdx].offset = SkEndian_SwapBE16(0);
            nameRecords[nrIdx].length = SkEndian_SwapBE16(SkToU16(fontNameLen * sizeof(SK_OT_USHORT)));
            ++nrIdx;
        }
    }
