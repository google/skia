...
static SkColor get_neighbor_avg_color(const SkBitmap& bm, int xOrig, int yOrig) {
    ...
    for (int y = ymin; y <= ymax; ++y) {
        SkColor* scanline = bm.getAddr32(0, y);
...
static void bitmap_to_pdf_pixels(const SkBitmap& bitmap, SkWStream* out) {
    ...
    } else if (kGray_8_SkColorType == bitmap.colorType()) {
        for (int y = 0; y < bitmap.height(); ++y) {
            out->write(bitmap.getAddr8(0, y), bitmap.width());
        }
    } else {
        SkBitmap bgra;
        bgra.allocPixels(bitmap.info().makeColorType(kBGRA_8888_SkColorType)
                                      .makeAlphaType(kUnpremul_SkAlphaType));
        SkAssertResult(bitmap.readPixels(bgra.pixmap()));
...
static void bitmap_alpha_to_a8(const SkBitmap& bitmap, SkWStream* out) {
    ...
    SkAssertResult(bitmap.readPixels(alpha.pixmap()));
