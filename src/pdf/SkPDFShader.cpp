...
static SkPDFIndirectReference make_fallback_shader(SkPDFDocument* doc,
    ...
    SkScalar bitmapArea = (float)std::max(1, surfaceBBox.width()) *
                          (float)std::max(1, surfaceBBox.height());
    ...
    SkISize size = {
        SkTClamp(sk_float_ceil2int(rasterScale * surfaceBBox.width()),  1, kMaxBitmapArea),
        SkTClamp(sk_float_ceil2int(rasterScale * surfaceBBox.height()), 1, kMaxBitmapArea)};
