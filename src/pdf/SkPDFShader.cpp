...
SkPDFShader::State make_state(SkShader* shader, const SkMatrix& canvasTransform,
    ...
    if (SkImage* skimg = shader->isAImage(&state.fShaderTransform, state.fImageTileModes)) {
        // TODO(halcanary): delay converting to bitmap.
        if (skimg->asLegacyBitmap(imageDst, SkImage::kRO_LegacyBitmapMode)) {
            state.fBitmapKey = SkBitmapKey{imageDst->getSubset(), imageDst->getGenerationID()};
            return state;
        }
    }
...
sk_sp<SkPDFObject> SkPDFShader::GetPDFShader(SkPDFDocument* doc,
    ...
    if (SkShader::kNone_GradientType != shader->asAGradient(nullptr)) {
        return SkPDFGradientShader::Make(doc, shader, matrix, surfaceBBox);
    }
...
//    State state(shader, matrix, surfaceBBox, rasterScale, &image);
    State state = make_state(shader, matrix, surfaceBBox, rasterScale, &image);
