...
static sk_sp<SkPDFObject> find_pdf_shader(SkPDFDocument* doc,
                                          SkPDFGradientShader::Key key,
                                          bool hasAlpha) {
    SkASSERT(gradient_has_alpha(key) == hasAlpha);
    SkPDFCanon* canon = doc->canon();
    SkPDFGradientShader::HashMap& map = canon->fGradientPatternMap;
    if (sk_sp<SkPDFObject>* ptr = map.find(key)) {
        return *ptr;
    }
    ...
    map.set(std::move(key), pdfShader);
    return pdfShader;
}
