...
static uint32_t hash(const SkPDFGradientShader::Key& k) {
    SkScalar floatBuffer[] = {
        ...
        k.fInfo.fRadius[0],
        k.fInfo.fRadius[1],
        k.fCanvasTransform[SkMatrix::kMScaleX],
        k.fCanvasTransform[SkMatrix::kMSkewX],
        k.fCanvasTransform[SkMatrix::kMTransX],
        k.fCanvasTransform[SkMatrix::kMSkewY],
        k.fCanvasTransform[SkMatrix::kMScaleY],
        k.fCanvasTransform[SkMatrix::kMTransY],
        k.fCanvasTransform[SkMatrix::kMPersp0],
        k.fCanvasTransform[SkMatrix::kMPersp1],
        k.fCanvasTransform[SkMatrix::kMPersp2],
        k.fShaderTransform[SkMatrix::kMScaleX],
        k.fShaderTransform[SkMatrix::kMSkewX],
        k.fShaderTransform[SkMatrix::kMTransX],
        k.fShaderTransform[SkMatrix::kMSkewY],
        k.fShaderTransform[SkMatrix::kMScaleY],
        k.fShaderTransform[SkMatrix::kMTransY],
        k.fShaderTransform[SkMatrix::kMPersp0],
        k.fShaderTransform[SkMatrix::kMPersp1],
        k.fShaderTransform[SkMatrix::kMPersp2],
    };
    uint32_t intBuffer[] = {
        (uint32_t)k.fType,
        (uint32_t)k.fInfo.fColorCount,
        (uint32_t)k.fInfo.fTileMode,
        k.fInfo.fGradientFlags,
        SkOpts::hash(floatBuffer, sizeof(floatBuffer)),
        SkOpts::hash(k.fColors.get(), k.fInfo.fColorCount * sizeof(SkColor)),
        SkOpts::hash(k.fStops.get(), k.fInfo.fColorCount * sizeof(SkScalar)),
    };
    return SkOpts::hash(intBuffer, sizeof(intBuffer));
}
...
static sk_sp<SkPDFObject> create_smask_graphic_state(SkPDFDocument* doc,
    ...
    SkPDFGradientShader::Key luminosityState = {
        state.fType,
        state.fInfo,  // change pointers later.
        std::unique_ptr<SkColor[]>(new SkColor[state.fInfo.fColorCount]),
        std::unique_ptr<SkScalar[]>(new SkScalar[state.fInfo.fColorCount]),
        state.fCanvasTransform,
        state.fShaderTransform,
        state.fBBox, 0};
    luminosityState.fInfo.fColors = luminosityState.fColors.get();
    luminosityState.fInfo.fColorOffsets = luminosityState.fStops.get();
    for (int i = 0; i < luminosityState.fInfo.fColorCount; i++) {
        luminosityState.fInfo.fColorOffsets[i] = state.fInfo.fColorOffsets[i];
        SkAlpha alpha = SkColorGetA(state.fInfo.fColors[i]);
...
static sk_sp<SkPDFStream> make_alpha_function_shader(SkPDFDocument* doc,
                                                     const SkPDFGradientShader::Key& state) {
    SkASSERT(state.fType != SkShader::kNone_GradientType);
    SkPDFGradientShader::Key opaqueState = {
        state.fType,
        state.fInfo,  // change pointers later.
        std::unique_ptr<SkColor[]>(new SkColor[state.fInfo.fColorCount]),
        std::unique_ptr<SkScalar[]>(new SkScalar[state.fInfo.fColorCount]),
        ...
        state.fShaderTransform,
        state.fBBox, 0};
    opaqueState.fInfo.fColors = opaqueState.fColors.get();
    opaqueState.fInfo.fColorOffsets = opaqueState.fStops.get();
    for (int i = 0; i < opaqueState.fInfo.fColorCount; i++) {
        opaqueState.fInfo.fColorOffsets[i] = state.fInfo.fColorOffsets[i];
        opaqueState.fInfo.fColors[i] = SkColorSetA(state.fInfo.fColors[i], SK_AlphaOPAQUE);
    }
