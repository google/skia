...
static uint32_t hash(const SkGradientInfo& v) {
    uint32_t buffer[] = {
 ...
 */
static void gradient_function_code(const SkGradientInfo& info,
                                 SkDynamicMemoryWStream* result) {
...
static std::unique_ptr<SkPDFDict> gradientStitchCode(const SkGradientInfo& info) {
    auto retval = SkPDFMakeDict();
...
static void linearCode(const SkGradientInfo& info,
...
static void radialCode(const SkGradientInfo& info,
                       const SkMatrix& perspectiveRemover,
                       SkDynamicMemoryWStream* function) {
    function->writeText("{");
 ...
 */
static void twoPointConicalCode(const SkGradientInfo& info,
                                const SkMatrix& perspectiveRemover,
                                SkDynamicMemoryWStream* function) {
...
static void sweepCode(const SkGradientInfo& info,
                          const SkMatrix& perspectiveRemover,
                          SkDynamicMemoryWStream* function) {
...
static SkPDFIndirectReference make_function_shader(SkPDFDocument* doc,
    ...
    const SkGradientInfo& info = state.fInfo;
    SkMatrix finalMatrix = state.fCanvasTransform;
    finalMatrix.preConcat(state.fShaderTransform);

    bool doStitchFunctions = (state.fType == SkGradientType::kLinear ||
                              state.fType == SkGradientType::kRadial ||
                              state.fType == SkGradientType::kConical) &&
    ...
    if (doStitchFunctions) {
        ...
        shadingType = (state.fType == SkGradientType::kLinear) ? 2 : 3;
        ...
        if (state.fType == SkGradientType::kConical) {
            ...
            coords = SkPDFMakeArray(pt1.x(),
                                    ...
                                    pt2.y(),
                                    r2);
        } else if (state.fType == SkGradientType::kRadial) {
        ...
        switch (state.fType) {
            case SkGradientType::kLinear:
                break;
            case SkGradientType::kRadial:
                transformPoints[1] = transformPoints[0];
                transformPoints[1].fX += info.fRadius[0];
                break;
            case SkGradientType::kConical: {
                transformPoints[1] = transformPoints[0];
                transformPoints[1].fX += SK_Scalar1;
                break;
            }
            case SkGradientType::kSweep:
                transformPoints[1] = transformPoints[0];
                transformPoints[1].fX += SK_Scalar1;
                break;
            case SkGradientType::kColor:
            case SkGradientType::kNone:
            default:
...
static bool gradient_has_alpha(const SkPDFGradientShader::Key& key) {
    SkASSERT(key.fType != SkGradientType::kNone);
...
static SkPDFIndirectReference create_smask_graphic_state(SkPDFDocument* doc,
                                                     const SkPDFGradientShader::Key& state) {
    SkASSERT(state.fType != SkGradientType::kNone);
...
static SkPDFGradientShader::Key make_key(const SkShader* shader,
    ...
    SkPDFGradientShader::Key key = {
         ...
         bbox, 0};
    key.fType = as_SB(shader)->asAGradient(&key.fInfo);
    SkASSERT(SkGradientType::kNone != key.fType);
    ...
    (void)as_SB(shader)->asAGradient(&key.fInfo);
    key.fHash = hash(key);
    return key;
}
...
SkPDFIndirectReference SkPDFGradientShader::Make(SkPDFDocument* doc,
                                             ...
                                             const SkIRect& bbox) {
    SkASSERT(shader);
    SkASSERT(SkGradientType::kNone != as_SB(shader)->asAGradient(nullptr));
