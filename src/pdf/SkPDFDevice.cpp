...
static void draw_draw_glyph_run_as_path(SkPDFDevice* device, const SkGlyphRun& glyphRun) {
    SkPath path;
    ...
    glyphRun.paint().getPosTextPath(glyphRun.shuntGlyphsIDs().data(),
                                    ...
                                    &path);
    device->drawPath(path, glyphRun.paint(), nullptr, true);
}
...
void SkPDFDevice::internalDrawGlyphRun(const SkGlyphRun& glyphRun, SkPoint offset) {
    ...
    if (srcPaint.getPathEffect()
        ...
        return draw_draw_glyph_run_as_path(this, glyphRun);
    }
    ...
    //constexpr float alignmentFactor =  0.0f;
