...
void SkPDFDevice::setGraphicState(sk_sp<SkPDFObject> gs, SkDynamicMemoryWStream* content) {
    SkPDFUtils::ApplyGraphicState(add_resource(fGraphicStateResources, fDocument->serialize(gs)),
                                  content);
}
...
SkDynamicMemoryWStream* SkPDFDevice::setUpContentEntry(const SkClipStack* clipStack,
    ...
    *dst = nullptr;
    ...
    if (!treat_as_regular_pdf_blend_mode(blendMode) && blendMode != SkBlendMode::kDstOver) {
        if (!isContentEmpty()) {
            *dst = SkPDFObj::Make(this->makeFormXObjectFromDevice());
...
void SkPDFDevice::finishContentEntry(const SkClipStack* clipStack,
    ...
    if (this->isContentEmpty()) {
        ...
        if (shape == nullptr || blendMode == SkBlendMode::kDstOut ||
            ...
            this->drawFormXObject(fDocument->serialize(dst), content.stream());
            return;
        } else {
            blendMode = SkBlendMode::kClear;
        }
    } else {
        srcFormXObject = SkPDFObj::Make(this->makeFormXObjectFromDevice());
    }
    ...
    if (blendMode == SkBlendMode::kSrcATop) {
        ...
        this->drawFormXObjectWithMask(fDocument->serialize(srcFormXObject),
                                      fDocument->serialize(dst), SkBlendMode::kSrcOver, true);
    } else {
        if (shape != nullptr) {
            ...
            this->drawFormXObjectWithMask(fDocument->serialize(dst),
                                          shapeDev.makeFormXObjectFromDevice(),
                                          SkBlendMode::kSrcOver, true);
        } else {
            this->drawFormXObjectWithMask(fDocument->serialize(dst),
                                          fDocument->serialize(srcFormXObject),
                                          SkBlendMode::kSrcOver, true);
        }
    }
    ...
    if (blendMode == SkBlendMode::kClear) {
        ...
        ScopedContentEntry content(this, nullptr, SkMatrix::I(), stockPaint);
        if (content) {
            this->drawFormXObject(fDocument->serialize(srcFormXObject), content.stream());
        }
    ...
    if (blendMode == SkBlendMode::kSrcIn ||
        ...
        if (blendMode == SkBlendMode::kModulate) {
            this->drawFormXObjectWithMask(fDocument->serialize(srcFormXObject),
                                          fDocument->serialize(dst), SkBlendMode::kSrcOver, false);
            mode = SkBlendMode::kMultiply;
        }
        this->drawFormXObjectWithMask(fDocument->serialize(dst),
                                      fDocument->serialize(srcFormXObject), mode,
                                      blendMode == SkBlendMode::kDstOut);
        return;
    }
}
...
void SkPDFDevice::populateGraphicStateEntryFromPaint(
    ...
    if (shader) {
        if (SkShader::kColor_GradientType == shader->asAGradient(nullptr)) {
            ...
            if (pdfShader) {
                ...
                entry->fShaderIndex =
                    add_resource(fShaderResources, fDocument->serialize(pdfShader));
            }
        }
    }
    ...
    entry->fGraphicStateIndex =
        add_resource(fGraphicStateResources, fDocument->serialize(std::move(newGraphicState)));
