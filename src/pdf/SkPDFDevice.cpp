...
public:
    static void DrawGlyphRunAsPath(SkPDFDevice* dev, const SkGlyphRun& glyphRun, SkPoint offset);
};

void PDFTextHandler::DrawGlyphRunAsPath(
        SkPDFDevice* dev, const SkGlyphRun& glyphRun, SkPoint offset) {
    ...
    if (dev->ctm().hasPerspective()) {
        SkMatrix prevCTM = dev->ctm();
        dev->setCTM(SkMatrix::I());
        dev->internalDrawGlyphRun(tmp, offset);
        dev->setCTM(prevCTM);
    } else {
        dev->internalDrawGlyphRun(tmp, offset);
    }
    ...
    draw_missing_glyphs(dev, paint, tmp.paint().getTypeface(), missingGlyphs);
}
...
void SkPDFDevice::internalDrawGlyphRun(const SkGlyphRun& glyphRun, SkPoint offset) {
    ...
    if (srcPaint.getPathEffect()
        ...
        return PDFTextHandler::DrawGlyphRunAsPath(this, glyphRun, offset);
    }
    SkPaint paint(srcPaint);
    remove_color_filter(&paint);
    replace_srcmode_on_opaque_paint(&paint);
