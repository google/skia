...
void SkPDFDevice::drawSpecial(SkSpecialImage* srcImg, int x, int y, const SkPaint& paint,
    ...
    if (filter) {
        ...
        SkFilterOutputProperties outputProperties(kN32_SkColorType, srcImg->getColorSpace());
        SkFilterContext ctx(matrix, clipBounds, cache.get(), outputProperties);

        sk_sp<SkSpecialImage> resultImg(filter->priv().filterImage(srcImg, ctx, &offset));
        if (resultImg) {
