...
std::unique_ptr<SkStreamAsset> SkPDFDevice::content() {
    ...
    if (fContentEntries.back() && fContentEntries.back() ==  fContentEntries.front()) {
        ...
        for (const SkDynamicMemoryWStream& entry : fContentEntries) {
            buffer.writeText("q\n");
            // TODO: write a non-const iterator for SkSinglyLinkedList.h
            const_cast<SkDynamicMemoryWStream&>(entry).writeToAndReset(&buffer);
            buffer.writeText("Q\n");
        }
    }
    fContentEntries.reset();
...
static bool is_regular_pdf_blend_mode(SkBlendMode blendMode) {
    switch (blendMode) {
...
SkDynamicMemoryWStream* SkPDFDevice::setUpContentEntry(const SkClipStack* clipStack,
    ...
    *dst = nullptr;
    ...
    if (is_regular_pdf_blend_mode(blendMode) || blendMode == SkBlendMode::kXor
