...
void SkPDFDict::emit(SkWStream* stream) const {
    ...
    for (size_t i = 0; i < fRecords.size(); i++) {
        const std::pair<SkPDFObject, SkPDFObject>& record = fRecords[i];
...
void emit(bool v, SkWStream* s) { s->writeText(v ? "true" : "false"); }

void emit(int32_t v, SkWStream* s) { s->writeDecAsText(v); }

void emit(SkScalar v, SkWStream* s) { SkPDFUtils::AppendScalar(v, s); }

void emit(SkPDFColorComponentU v, SkWStream* s) {
    SkPDFUtils::AppendColorComponent(v.fValue, s);
}

void emit(SkPDFColorComponentF v, SkWStream* s) {
    SkPDFUtils::AppendColorComponentF(v.fValue, s);
}

void emit(const SkString& v, SkWStream* s) { write_string(s, v.c_str(), v.size()); }

void emit(const char* v, SkWStream* s) { write_string(s, v, strlen(v)); }

void emit(const SkPDFStringName& v, SkWStream* s) {
    s->writeText("/");
    write_name_escaped(s, v.fValue.c_str());
}

void emit(SkPDFStaticName v, SkWStream* s) {
    s->writeText("/");
    SkASSERT(is_valid_name(v.fValue));
    s->writeText(v.fValue);
}

void emit(const std::unique_ptr<SkPDFArray>& v, SkWStream* s) {
    SkASSERT(v);
    v->emit(s);
}

void emit(const std::unique_ptr<SkPDFDict>& v, SkWStream* s) {
    SkASSERT(v);
    v->emit(s);
}

void emit(SkPDFIndirectReference v, SkWStream* s) {
    SkASSERT(v);
