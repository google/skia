...
public:
    ...
    // Call this before adding pages.
    void setTagRoot(sk_sp<SkPDFTag> tagRoot);

    sk_sp<SkPDFDict> getPage(int pageIndex);

    // Returns -1 if no mark ID.
    int getMarkIdForNodeId(int64_t nodeId);

private:
    void recursiveAddNodeIdToMap(sk_sp<SkPDFTag> tag);
    ...
    SkTHashMap<int64_t, sk_sp<SkPDFTag>> fNodeIdToTag;
