...
public:
    ...
    void appendObject(std::unique_ptr<SkPDFDict> d) {
        this->appendObject(std::unique_ptr<SkPDFObject>((SkPDFObject*)d.release()));
    }
    void appendObject(std::unique_ptr<SkPDFArray> a) {
        this->appendObject(std::unique_ptr<SkPDFObject>((SkPDFObject*)a.release()));
    }
...
inline void SkPDFArray_Append(SkPDFArray* a) {}

template <typename... Args>
inline std::unique_ptr<SkPDFArray> SkPDFMakeArray(Args... args) {
...
public:
    ...
    void insertObject(const char key[], std::unique_ptr<SkPDFArray> v) {
        this->insertObject(std::move(key), std::unique_ptr<SkPDFObject>((SkPDFObject*)v.release()));
    }
    void insertObject(SkString key, std::unique_ptr<SkPDFArray> v) {
        this->insertObject(std::move(key), std::unique_ptr<SkPDFObject>((SkPDFObject*)v.release()));
    }
    void insertObject(const char key[], std::unique_ptr<SkPDFDict> v) {
        this->insertObject(std::move(key), std::unique_ptr<SkPDFObject>((SkPDFObject*)v.release()));
    }
    void insertObject(SkString key, std::unique_ptr<SkPDFDict> v) {
        this->insertObject(std::move(key), std::unique_ptr<SkPDFObject>((SkPDFObject*)v.release()));
    }
