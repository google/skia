...
struct AdvanceMetric {
    ...
    AdvanceMetric(SkGlyphID startId) : fStartId(startId) {}
...
static int16_t findMode(SkSpan<const int16_t> advances) {
    if (advances.empty()) {
        return 0;
    }
...
std::unique_ptr<SkPDFArray> SkPDFMakeCIDGlyphWidthsArray(const SkTypeface& typeface,
    ...
    // the ASCII representation of a glyph id plus a space is 5 characters
    ...
    //  b. Run wins for 2+ repeats " gfid gfid adv.ances"
    //                             " gfid [adv.ances adv.ances]"
    //     rule: 2+ repeats create run as long as possible, else start range
    //
    // When in a range
    // Cost of stop and start range plus run is 28 characters "] gfid gfid adv.ances gfid ["
    //  c. Switching for 3+ repeats wins                      "adv.ances adv.ances adv.ances"
    //     rule: always end range for 3+ repeats
    //  d. Switching for 2+ repeats and 4+ don't cares wins   "adv.ances 0 0 0 0 adv.ances"
    //     rule: always end range for 2+ repeats with 4+ don't cares
    //
    // The cost of stopping and starting a range is 8 characters "] gfid ["
    //  e. Skipping 5+ don't care's is a win                     "0 0 0 0 0"
    //     rule: always end range if 5+ don't cares.
    //  f. Skipping defaults is always a win                     "adv.ances"
    //     rule: always end range if default seen
    ...
    for (size_t i = 0; i < glyphs.size(); ++i) {
...

        {
            ...
            for (; j < glyphs.size(); ++j) {
                ...
                // f. always end range if default seen
                if (advance == modeAdvance) {
                    break;
                }

                int dontCares = glyphs[j]->getGlyphID() - glyphs[j - 1]->getGlyphID() - 1;
                // e. always end range if 5+ don't cares.
                if (dontCares >= 5) {
                    break;
                }

                int16_t next_advance = 0;
                // d. always end range for 2+ repeats with 4+ don't cares
                if (j + 1 < glyphs.size()) {
                    next_advance = (int16_t)glyphs[j+1]->advanceX();
                    int next_dontCares = glyphs[j+1]->getGlyphID() - glyphs[j]->getGlyphID() - 1;
                    if (advance == next_advance && next_dontCares >= 4) {
                        break;
                    }
                }

                // c. always end range for 3+ repeats
