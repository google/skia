...
public:
    GLSLSweepProcessor(const GrProcessor&) {
        fCachedTBiasScale[kTBiasIndex ] = std::numeric_limits<float>::max();
        fCachedTBiasScale[kTScaleIndex] = std::numeric_limits<float>::max();
    }
...
protected:
    void onSetData(const GrGLSLProgramDataManager& pdman,
        ...
        if (fCachedTBiasScale[kTBiasIndex]  != data.fTBias ||
            fCachedTBiasScale[kTScaleIndex] != data.fTScale) {
            fCachedTBiasScale[kTBiasIndex]  = data.fTBias;
            fCachedTBiasScale[kTScaleIndex] = data.fTScale;
            pdman.set1fv(fTBiasScaleUni, 2, fCachedTBiasScale);
        }
    }
...
private:
    ...
    static constexpr size_t kTBiasIndex  = 0;
    static constexpr size_t kTScaleIndex = 1;
    float fCachedTBiasScale[2];
...
void GrSweepGradient::GLSLSweepProcessor::emitCode(EmitArgs& args) {
    ...
    fTBiasScaleUni = uniformHandler->addUniformArray(kFragment_GrShaderFlag, kFloat_GrSLType,
                                                     kDefault_GrSLPrecision, "SweepFSParams", 2);
    GrShaderVar tBiasScaleV = uniformHandler->getUniformVariable(fTBiasScaleUni);
    ...
    const SkString t = SkStringPrintf("((%s * 0.1591549430918 + 0.5 + %s[%lu]) * %s[%lu])",
                                      atan.c_str(),
                                      tBiasScaleV.c_str(), kTBiasIndex,
                                      tBiasScaleV.c_str(), kTScaleIndex);
