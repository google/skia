...
public:
    ...
    bool isWellBehaved() const { return !this->isLinear() && this->r1() > 1; }
...
protected:
    ...
    void onSetData(const GrGLSLProgramDataManager& pdman, const GrFragmentProcessor& p) override {
        ...
        SkScalar r0 = data.r0();
        SkScalar r1 = data.r1();
        pdman.set1f(fParamUni, data.getType() == kRadial_Type ? r0 / (r1 - r0) : r0 * r0);
    }
...
std::unique_ptr<GrFragmentProcessor> Gr2PtConicalGradientEffect::Make(
    ...
    GrGradientEffect::CreateArgs newArgs(args.fContext, args.fShader, &matrix, args.fWrapMode,
        args.fDstColorSpace);
    // Data and matrix has to be prepared before constructing TwoPointConicalEffect its parent class
    // can have the right matrix to work with during construction.
