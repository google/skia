...
public:
    ...
    bool isFocalOnCircle() const { return fData.fFocalData.isFocalOnCircle(); }
    bool isSwapped() const { return fData.fFocalData.isSwapped(); }
...
protected:
    ...
    void onSetData(const GrGLSLProgramDataManager& pdman, const GrFragmentProcessor& p) override {
        ...
        const TwoPointConicalEffect& ge = p.cast<TwoPointConicalEffect>();
        // kRadialType should imply r1 - r0 = 1 (after our transformation) so r0 = r0 / (r1 - r0)
        SkASSERT(ge.getType() == Type::kStrip || SkScalarNearlyZero(ge.diffRadius() - 1));
        pdman.set1f(fParamUni, ge.getType() == Type::kRadial ? ge.r0() : ge.r0() * ge.r0());
    }
...
protected:
    ...
    void onSetData(const GrGLSLProgramDataManager& pdman, const GrFragmentProcessor& p) override {
        INHERITED::onSetData(pdman, p);
        const TwoPointConicalEffect& ge = p.cast<TwoPointConicalEffect>();
        pdman.set2f(fParamUni, 1 / ge.r1(), ge.focalX());
    }
