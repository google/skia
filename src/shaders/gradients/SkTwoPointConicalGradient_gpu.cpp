...
public:
    ...
    bool isWellBehaved() const {
        if (fType == kInsideFocal_Type || fType == kOutsideFocal_Type) {
            return SkScalarAbs(fRadius0 + fDiffRadius) > 1;
        }
...
protected:
    ...
    bool onIsEqual(const GrFragmentProcessor& sBase) const override {
        ...
        return (INHERITED::onIsEqual(sBase) && fDiffRadius == s.fDiffRadius
            && fRadius0 == s.fRadius0 && fType == s.fType);
    }
    ...
    explicit TwoPointConicalEffect(const CreateArgs& args)
        ...
        if (fType != kRadial_Type) {
            ...
            bool isFocal = !this->isLinear() && !SkScalarNearlyZero(fDiffRadius)
                    && !SkScalarNearlyZero(shader.getEndRadius());
...
protected:
    void emitCode(EmitArgs& args) override {
        ...
        SkString p0; // r1^2 for ouside case, r1 for inside case
        ...
        if (ge.getType() == kInsideFocal_Type) {
            ...
            fragBuilder->codeAppendf("\thalf %s_prime = (sqrt(temp) - %s.x);",
                    tName.c_str(), coords2D);
        }
...
std::unique_ptr<GrFragmentProcessor> Gr2PtConicalGradientEffect::Make(
    ...
    if (SkScalarNearlyZero(shader.getCenterX1())) {
        ...
        if (!isLinear && !SkScalarNearlyZero(shader.getDiffRadius())
                && !SkScalarNearlyZero(shader.getEndRadius())) {
