...
void Edge2PtConicalEffect::GLSLEdge2PtConicalProcessor::emitCode(EmitArgs& args) {
    ...
    //fragBuilder->codeAppendf("%s = clamp(%s, 0.0, 1.0);", tName.c_str(), tName.c_str());

#if 1
    // if r(t) > 0, then t will be the x coordinate
    fragBuilder->codeAppendf("half tempMath = %s * %s + %s;", tName.c_str(),
                             p2.c_str(), p0.c_str());
    fragBuilder->codeAppendf("\tif (!isnan(tempMath) && !isinf(tempMath) && tempMath > 0.0) {\n");
