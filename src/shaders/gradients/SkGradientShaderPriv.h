...
public:
    ...
    InterpolationStrategy getStrategy() const { return fStrategy; }
...
protected:
    ...
    static std::unique_ptr<GrFragmentProcessor> AdjustFP(
        ...
        if (gradientFP->getStrategy() == InterpolationStrategy::kTexture) {
...
private:
    void addInterval(const SkGradientShaderBase&, size_t color0, size_t color1, PremulType,
                     SkColorSpace*);

    static OptimizationFlags OptFlags(bool isOpaque);

    // Interpolation intervals, encoded as 4f pairs of (scale, bias)
...
private:
    ...
    enum {
        // First three bits reserved for strategy encoding.
        kStrategyBits           =  3,

        // One bit for premul before/after interpolation
        kPremulBeforeInterpKey  =  8,
        ...
        // Lower six bits for strategy, premul, and tile mode
        kReservedBits           = 6,
    };
