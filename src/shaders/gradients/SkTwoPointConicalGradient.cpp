...
void SkTwoPointConicalGradient::FocalData::set(SkScalar r0, SkScalar r1, SkMatrix& matrix) {
    fIsSwapped = false;
    SkScalar focalX = r0 / (r0 - r1);
    if (SkScalarNearlyZero(focalX - 1)) {
        // swap r0, r1
        matrix.postTranslate(-1, 0);
        matrix.postScale(-1, 1);
        std::swap(r0, r1);
        focalX = 0; // because r0 is now 0
        fIsSwapped = true;
    }
...
void SkTwoPointConicalGradient::appendGradientStages(SkArenaAlloc* alloc, SkRasterPipeline* p,
    ...
    ctx->fP1 = fFocalData.fR0 / (fFocalData.fR0 - fFocalData.fR1);
    ...
    } else if (this->isSwapped() || !this->isRadiusIncreasing()) {
    ...
    if (!this->isNativelyFocal()) {
        p->append(SkRasterPipeline::xy_to_2pt_conical_compensate_focal, ctx);
    }
    if (this->isSwapped()) {
