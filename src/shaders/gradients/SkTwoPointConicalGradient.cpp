...
void SkTwoPointConicalGradient::transformT(skvm::Builder* p, skvm::Uniforms* uniforms,
                                           skvm::F32 x, skvm::F32 y,
                                           skvm::F32* t, skvm::I32* mask) const {
    ...
    if (fType == Type::kRadial) {
        float denom = 1.0f / (fRadius2 - fRadius1),
              scale = SkTMax(fRadius1, fRadius2) * denom,
               bias =                  -fRadius1 * denom;
        *t = p->mad(p->norm(x,y), p->uniformF(uniforms->pushF(scale))
    ...
    if (fType == Type::kStrip) {
        ...
        *mask = p->eq(*t,*t);   // *t != NaN
        *t = p->bit_cast(p->bit_and(*mask, p->bit_cast(*t)));
        return;
    }

    skvm::F32 invR1 = p->uniformF(uniforms->pushF(1 / fFocalData.fR1));
    ...
    skvm::F32 focalX = p->uniformF(uniforms->pushF(fFocalData.fFocalX));
