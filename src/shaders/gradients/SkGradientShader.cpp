...
bool SkGradientShaderBase::onProgram(skvm::Builder* p,
    ...
    if (fColorCount == 2) {
        ...
        *r = p->mad(T, p->uniformF(uniforms->pushF(f[0])), p->uniformF(uniforms->pushF(lo[0])));
        *g = p->mad(T, p->uniformF(uniforms->pushF(f[1])), p->uniformF(uniforms->pushF(lo[1])));
        *b = p->mad(T, p->uniformF(uniforms->pushF(f[2])), p->uniformF(uniforms->pushF(lo[2])));
        *a = p->mad(T, p->uniformF(uniforms->pushF(f[3])), p->uniformF(uniforms->pushF(lo[3])));
    } else {
        // To handle clamps in search we add a conceptual stop at t=-inf,
        // so we may need up to fColorCount+1 FBs and will track up to
        // fColorCount t stops:
        //
        //   FBs:    [color 0] [color 0->1] [color 1->2]  ...
        //   stops:    (-inf)  [    t0    ] [    t1    ]  ...
        //
