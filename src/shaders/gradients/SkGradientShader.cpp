...
bool SkGradientShaderBase::onProgram(skvm::Builder* p,
    ...
    // TODO: onAppendStages() above collapses away any dummy color stops that
    // GradientShaderBase's ctor adds when no explicit stop is set at 0 or 1.
    // Our math here (and onAppendStages's math too) will work out fine whether
    // or not we do that; it's purely to decrease the number of stops, to
    // generate less code, to test fewer stops at runtime.
    ...
    // Logically that should be `stops.push_back(-INFINITY)`, but to seed the
    // lo/hi induction properly and end up with the correct first f scale, we
    // need t_lo to start at the real this->getPos(0).
    //
    // When it comes to finding what stops our runtime t lies between, we'll
    // never actually even read this stops[0] value, instead just acting as if
    // it were -INFINITY, with all t always >= stops[0].  This starts the
    // skvm::I32 ix off sanely at 0, our first color.
    //
    // If we were really careful we could skip that first call to push_back()
    // entirely, but it would make the count of t-stops and color-stops fall
    // out of lock step.  It's just kind of a way to keep our head straight
    // here, and maybe could be refactored away if we find a clearer way to
    // express this setup.

    // Now the non-edge cases, calculating scale and bias between pairs of normal stops.
    for (int i = 1; i < fColorCount; i++) {
        ...
        // TODO: a more careful calculation of f where 0/0 -> 0 would produce the same effect.
        SkASSERT(t_lo <= t_hi);
        if (t_lo < t_hi) {
    ...
    if (fOrigPos == nullptr) {
        ...
        // Of note: we need to clamp t and ignore that conceptual -INFINITY stop we made up.
        ix = p->trunc(p->mad(p->clamp(t, p->splat(0.0f), p->splat(1.0f)),
                             p->uniformF(uniforms->pushF(stops.size() - 2.0f)),
                             p->splat(1.0f)));
    } else {
        // Starting ix at 0 and skipping stops[0] bakes in our conceptual first stop at -INFINITY.
    ...
    // TODO: this general solution leans heavily on gathers.
    // There are cheaper approaches for evenly-spaced gradients (especially 2-stop),
