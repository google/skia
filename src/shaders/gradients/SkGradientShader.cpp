...
void GrGradientEffect::GLSLProcessor::emitAnalyticalColor(GrGLSLFPFragmentBuilder* fragBuilder,
    ...
    switch (ge.fWrapMode) {
        ...
        case GrSamplerState::WrapMode::kMirrorRepeat:
            ...
            fragBuilder->codeAppendf("half tiled_t = t_1 - (2.0 * ((t_1 *0.5) - fract(t_1 * 0.5)));");
            // tiled_t should be between 0 and 2 here;
            fragBuilder->codeAppendf("if (tiled_t >= 2.0) {");
            fragBuilder->codeAppendf("  child = half4(0.75, 0.75, 0.75, 0.75);");
            fragBuilder->codeAppendf("} else if (tiled_t < 0.0) {");
            fragBuilder->codeAppendf("  child = half4(0.75, 0.00, 0.75, 0.75);");
