...
void GrGradientEffect::GLSLProcessor::emitAnalyticalColor(GrGLSLFPFragmentBuilder* fragBuilder,
    ...
    switch (ge.fWrapMode) {
        ...
        case GrSamplerState::WrapMode::kMirrorRepeat:
            ...
            fragBuilder->codeAppendf("half tiled_t = (t_1 - (2.0 * ((t_1 *0.5) - fract(t_1 * 0.5))))-1;");
            // tiled_t should be between 0 and 2 here;
#define USE_MY_IF_THING 0
#if USE_MY_IF_THING
            fragBuilder->codeAppendf("if (tiled_t > 1.0) {");
            fragBuilder->codeAppendf("  child = half4(0.1, 5000.1, 5000.1, 5000.1);");
            fragBuilder->codeAppendf("} else if (tiled_t < -1.0) {");
            fragBuilder->codeAppendf("  child = half4(5000.1, 0.1, 5000.1, 5000.1);");
            fragBuilder->codeAppendf("} else {");
#endif
            fragBuilder->codeAppendf("tiled_t = clamp(tiled_t, -1.0, 1.0);");
            fragBuilder->codeAppendf("tiled_t = abs(tiled_t);");
            //fragBuilder->codeAppendf("if (tiled_t < 0.0) { tiled_t = 0.0; }");
