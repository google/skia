...
bool SkGradientShaderBase::onProgram(skvm::Builder* p,
    ...
    // Most tiling happens here, with kDecal also needing some work at the end.
    switch(fTileMode) {
    ...
    skvm::F32 clamped_t = p->clamp(t, p->splat(0.0f), p->splat(1.0f));

    // Transform our colors as we want them interpolated, in dst color space, possibly premul.
    SkImageInfo base = SkImageInfo::Make(fColorCount,1, kRGBA_F32_SkColorType, kUnpremul_SkAlphaType),
                src  = base.makeColorSpace(fColorSpace),
                dst  = base.makeColorSpace(sk_ref_sp(dstCS));
    if (fGradFlags & SkGradientShader::kInterpolateColorsInPremul_Flag) {
        dst = dst.makeAlphaType(kPremul_SkAlphaType);
    }
    SkSTArray<4*4, float> rgba;
    rgba.resize(4*fColorCount);
    SkConvertPixels(dst,  rgba.begin(), dst.minRowBytes(),
                    src, fOrigColors4f, src.minRowBytes());

    // Transform our colors into a bias B and scale factor F such that for
    // any t between stops i and i+1, the color we want is mad(t, F[i], B[i]).
    float* fb = alloc->makeArrayDefault<float>(8*fColorCount);
    for (int i = 0; i < fColorCount-1; i++) {
        // Scale Factors: (hi - lo) / (t_hi - t_lo)
        for (int j = 0; j < 4; j++) {
            fb[j+0] = (rgba[4*(i+1) + j] - rgba[4*i + j])
                    / (this->getPos(i+1) - this->getPos(i));
        }
        // Biases: hi - F*t_lo
        for (int j = 0; j < 4; j++) {
            fb[j+4] = rgba[4*(i+1) + j] - fb[j+0]*this->getPos(i);
        }
    }
    ...
    for (int i = 1; i < fColorCount; i++) {
        // ix += (clamped_t >= stop[i]) ? +1 : 0 ~~>
        // ix -= (clamped_t >= stop[i]) ? -1 : 0
        ix = p->sub(ix, p->gte(clamped_t, p->uniformF(uniforms->pushF(this->getPos(i)))));
    }

    // 4 scale factors and 4 biases per pixel.
    ix = p->shl(ix, 3);
    skvm::F32 Fr = p->bit_cast(p->gather32(fbs, ix)); ix = p->add(ix, p->splat(1));
    skvm::F32 Fg = p->bit_cast(p->gather32(fbs, ix)); ix = p->add(ix, p->splat(1));
    skvm::F32 Fb = p->bit_cast(p->gather32(fbs, ix)); ix = p->add(ix, p->splat(1));
    skvm::F32 Fa = p->bit_cast(p->gather32(fbs, ix)); ix = p->add(ix, p->splat(1));
    skvm::F32 Br = p->bit_cast(p->gather32(fbs, ix)); ix = p->add(ix, p->splat(1));
    skvm::F32 Bg = p->bit_cast(p->gather32(fbs, ix)); ix = p->add(ix, p->splat(1));
    skvm::F32 Bb = p->bit_cast(p->gather32(fbs, ix)); ix = p->add(ix, p->splat(1));
    skvm::F32 Ba = p->bit_cast(p->gather32(fbs, ix));

    *r = p->mad(clamped_t, Fr, Br);
    *g = p->mad(clamped_t, Fg, Bg);
    *b = p->mad(clamped_t, Fb, Bb);
    *a = p->mad(clamped_t, Fa, Ba);
    ...
    if (fTileMode == SkTileMode::kDecal) {
        skvm::I32 mask = p->eq(t, clamped_t);
