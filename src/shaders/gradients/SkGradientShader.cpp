    ...
    switch (ge.getColorType()) {
        case kSingleHardStop_ColorType: {
            ...
            fragBuilder->codeAppendf("{"
                                     "  float stopT = %s;"
                                     "  float s = step(stopT, clamp_t);"
                                     "  c0_index = int(s * 2.0);"
                                     "  c1_index = c0_index + 1;"
                                     "  lerp = (clamp_t - s * stopT) / (stopT * (1 - s * 2.0) + s);"
                                     "}",
                                     stopT);
        ...
        case kHardStopLeftEdged_ColorType: {
            ...
            if (ge.fTileMode == SkShader::kClamp_TileMode) {
                fragBuilder->codeAppendf("  float s = step(0.0, %s);"
                                         "  c0_index = int(s * 1.0);" // 0 | 1
                                         "  c1_index = int(s * 2.0);" // 0 | 2
                                         , t);
            } else {
        ...
        case kHardStopRightEdged_ColorType: {
            ...
            fragBuilder->codeAppend     ("{");
            if (ge.fTileMode == SkShader::kClamp_TileMode) {
                fragBuilder->codeAppendf("  float s = step(1.0, %s);"
                                         "  c0_index = 0 + int(s * 2.0);" // 0 | 2
                                         "  c1_index = 1 + int(s * 1.0);" // 1 | 2
                                         , t);
            } else {
        ...
        case kThree_ColorType: {
            ...
            fragBuilder->codeAppend("{"
                                    "  float s = step(0.5, clamp_t);"
                                    "  c0_index = 0 + int(s);"   // 0 | 1
                                    "  c1_index = 1 + int(s);"   // 1 | 2
                                    "  lerp = (clamp_t - s * 0.5) * 2.0;"
                                    "}");
            break;
        }
