...
bool SkGradientShaderBase::onProgram(skvm::Builder* p,
    ...
    // Perhaps unexpectedly, clamping for kClamp mode is handled by our search,
    // and not by explicitly clamping to [0,1] here like repeat and mirror.
    auto clamp_01 = [&](skvm::F32 t) {
        return p->clamp(t, p->splat(0.0f), p->splat(1.0f));
    };
    switch(fTileMode) {
        case SkTileMode::kDecal:  break;
        case SkTileMode::kClamp:  break;
        case SkTileMode::kRepeat: t = clamp_01(p->sub(t, p->floor(t))); break;
    ...
    *a = p->mad(t, Fa, Ba);
    ...
    if (fTileMode == SkTileMode::kDecal) {
        skvm::I32 mask = p->eq(t, clamp_01(t));
