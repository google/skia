...
static SkColor3f xform_color(const SkColor3f& color, SkColorSpaceXformer* xformer) {
    ...
    return SkColor3f::Make(SkIntToScalar(SkGetPackedR32(xformedColor)) / 255.0f,
                           SkIntToScalar(SkGetPackedG32(xformedColor)) / 255.0f,
                           SkIntToScalar(SkGetPackedB32(xformedColor)) / 255.0f);
}
