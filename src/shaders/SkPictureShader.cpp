...
public:
        ...
        , fScale(scale)
    {
        ...
        this->init(&gBitmapShaderKeyNamespaceLabel,
...
struct BitmapShaderRec : public SkResourceCache::Rec {
    BitmapShaderRec(const BitmapShaderKey& key, sk_sp<SkImage> image)
        : fKey(key)
    ...
    static CachedImageInfo Make(const SkRect& bounds,
        ...
        return {
            ...
            SkImageInfo::Make(tileSize.width(), tileSize.height(),
                              bitDepth == SkImage::BitDepth::kU8 ? kRGBA_8888_SkColorType
                                                                 : kRGBA_F16Norm_SkColorType,
                              kPremul_SkAlphaType, imgCS),
        };
    }
...
sk_sp<SkShader> SkPictureShader::rasterShader(const SkMatrix& viewMatrix,
    ...
    BitmapShaderKey key(info.imageInfo.colorSpace(), info.imageInfo.colorType(),
                        fPicture->uniqueID(), info.tileScale);

    sk_sp<SkImage> image;
    if (!SkResourceCache::Find(key, BitmapShaderRec::Visitor, &image)) {
        image = info.makeImage(SkSurface::MakeRaster(info.imageInfo), fPicture.get());
        if (!image) {
            return nullptr;
        }

        SkResourceCache::Add(new BitmapShaderRec(key, image));
...
std::unique_ptr<GrFragmentProcessor> SkPictureShader::asFragmentProcessor(
    ...
    const auto ctm = args.fMatrixProvider.localToDevice();
    auto info = CachedImageInfo::Make(fTile, ctm, &lm,
                                      dstColorType, args.fDstColorInfo->colorSpace(),
                                      args.fContext->priv().caps()->maxTextureSize());
    if (!info.success) {
        return nullptr;
    }

    GrSurfaceProxyView view;    // add caching here
    sk_sp<SkImage> image;
    if (!view) {
        const int msaaSampleCount = 0;
        const SkSurfaceProps* props = nullptr;
        const bool createWithMips = false;
             image = info.makeImage(SkSurface::MakeRenderTarget(args.fContext,
                                                                SkBudgeted::kYes,
                                                                info.imageInfo,
                                                                msaaSampleCount,
        ...
        auto [v, ct] = as_IB(image)->asView(args.fContext, GrMipmapped::kNo);
        view = std::move(v);
    }

    // We are defined to only look at filtermode (no mips, no bicubic)
    const auto filterMode = sampling_to_filter(args.fSampling);

    if (true) {
        GrFPArgs newArgs(args.fContext, args.fMatrixProvider, args.fSampling, args.fDstColorInfo);
        newArgs.fPreLocalMatrix = lm.get();

        auto sh = image->makeShader(fTmx, fTmy, SkSamplingOptions(filterMode), nullptr);
        return as_SB(sh)->asFragmentProcessor(newArgs);
    } else {
        const GrSamplerState sampler(static_cast<GrSamplerState::WrapMode>(fTmx),
                                     static_cast<GrSamplerState::WrapMode>(fTmy),
                                     filterMode);
        return GrTextureEffect::Make(std::move(view), kPremul_SkAlphaType, SkMatrix(), sampler,
