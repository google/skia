...
SkPictureShader::LockedTile SkPictureShader::lockBitmapTile(const SkMatrix& ctm,
    ...
    LockedTile locked = { nullptr, tileSize, totalLocalMatrix };
    ...
    const BitmapTileKey key(keyCS, fPicture->uniqueID(), fTile, tileSize, blendBehavior);
    sk_sp<SkCachedData> data;

    if (!SkResourceCache::Find(key, BitmapTileRec::Visitor, &data)) {
        const auto size = info.computeMinByteSize();
        data.reset(SkResourceCache::NewCachedData(size));
        ...
        auto cacheCanvas = SkCanvas::MakeRasterDirect(info,
                                                      data->writable_data(),
                                                      info.minRowBytes(),
                                                      &props);
        if (!cacheCanvas) {
        ...
        auto* canvas = cacheCanvas.get();
        std::unique_ptr<SkCanvas> xformCanvas;
        if (SkTransferFunctionBehavior::kIgnore == blendBehavior && info.colorSpace()) {
    ...
    if (locked.fTile) {
        // Ref ownership was passed to the tile image, we can release.
        (void)data.release();
    }
