...
sk_sp<SkShader> SkPictureShader::lockTile(const TileInfo& tileInfo) const {
    ...
    if (!SkResourceCache::Find(tileInfo.fKey, BitmapTileRec::Visitor, &data)) {
        ...
        SkResourceCache::Add(new BitmapTileRec(tileInfo.fKey, data.get()));
        fAddedToCache.store(true);
    }

    SkASSERT(data && data->data());
    const SkPixmap pm(tileInfo.fInfo, data->data(), tileInfo.fInfo.minRowBytes());
    auto tileImage = SkImage::MakeFromRaster(pm, [](const void*, void* ctx) {
...
sk_sp<SkShader> SkPictureShader::lockTile(const TileInfo& tileInfo, const GrFPArgs& args) const {
    ...
    const auto grKey = tileInfo.fKey.asGrUniqueKey(kPictureShaderTileDomain);
