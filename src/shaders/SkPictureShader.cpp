...
#if SK_SUPPORT_GPU
    if (ctx) {
        ctxID = ctx->priv().contextID();
    }
#endif

    BitmapShaderKey key(imgCS.get(), bitDepth, fPicture->uniqueID(), tileScale, ctxID);

    sk_sp<SkImage> image;
    if (!SkResourceCache::Find(key, BitmapShaderRec::Visitor, &image)) {
        auto make_surface = [&]() {
            auto info = SkImageInfo::Make(tileSize.width(), tileSize.height(),
                                          bitDepth == SkImage::BitDepth::kU8
...
#if SK_SUPPORT_GPU
            if (ctx) {
                return SkSurface::MakeRenderTarget(ctx, SkBudgeted::kYes, info);
            }
#endif
        ...
        canvas->concat(SkMatrix::RectToRect(fTile, SkRect::MakeIWH(tileSize.width(),
                                                                   tileSize.height())));
