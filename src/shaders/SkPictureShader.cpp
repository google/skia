...
struct CachedImageInfo {
    ...
    static CachedImageInfo Make(const SkRect& bounds,
        ...
        const SkSize tileScale = SkSize::Make(SkIntToScalar(tileSize.width()) / bounds.width(),
                                              SkIntToScalar(tileSize.height()) / bounds.height());
...
std::unique_ptr<GrFragmentProcessor> SkPictureShader::asFragmentProcessor(
    ...
    if (!view) {
        auto image = info.makeImage(SkSurface::MakeRenderTarget(args.fContext,
                                                                SkBudgeted::kYes,
                                                                info.imageInfo),
                                    fPicture.get());
        if (!image) {
    ...
    return GrTextureEffect::Make(std::move(view), kPremul_SkAlphaType, *lm, sampler,
