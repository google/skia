...
SkImageShader::SkImageShader(sk_sp<SkImage> img,
    ...
    , fClampBicubicAsIfUnpremul(clampBicubicAsIfUnpremul)
{}

// fClampBicubicAsIfUnpremul is always false when constructed through public APIs,
...
void SkImageShader::flatten(SkWriteBuffer& buffer) const {
    ...
    SkASSERT(fClampBicubicAsIfUnpremul == false);
}
...
sk_sp<SkShader> SkImageShader::Make(sk_sp<SkImage> image,
    ...
    return sk_sp<SkShader>{
        new SkImageShader(image, tx,ty, localMatrix, clampBicubicAsIfUnpremul)
    };
}
...
bool SkImageShader::onAppendStages(const StageRec& rec) const {
    ...
    auto append_misc = [&] {
        ...
        if (quality > kLow_SkFilterQuality) {
            ...
            p->append(fClampBicubicAsIfUnpremul ? SkRasterPipeline::clamp_1
                                                : SkRasterPipeline::clamp_a);
        }
    #endif
        append_gamut_transform(p, alloc, info.colorSpace(), rec.fDstCS,
                               fClampBicubicAsIfUnpremul ? kUnpremul_SkAlphaType
                                                         : kPremul_SkAlphaType);
        return true;
    };
