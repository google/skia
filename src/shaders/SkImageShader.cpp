...
bool SkImageShader::doStages(const SkStageRec& rec, SkImageStageUpdater* updater) const {
    ...
    auto sampling = SkSamplingOptions(state->quality());
    ...
    if (updater) {
        ...
        tweak_sampling_and_inv_matrix(&sampling, &matrix);
        p->append_matrix(alloc, matrix);
    }

    Technique tech = sampling.fUseCubic                        ? Technique::kCubic
                   : sampling.fFilter == SkFilterMode::kLinear ? Technique::kLinear
                                                               : Technique::kNearest;
    ...
    if (tech == Technique::kNearest) {
        ...
        SkASSERT(tech == Technique::kCubic);
        p->append(SkRasterPipeline::save_xy, sampler);
