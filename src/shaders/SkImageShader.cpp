...
SkShaderBase::Context* SkImageShader::onMakeContext(const ContextRec& rec,
    ...
    // Send in a modified paint with different filter-quality if necessary
    SkPaint modifiedPaint;
    ContextRec modifiedRec = rec;
    SkFilterQuality quality = SkPaintPriv::ResolveFilterQuality(*rec.fPaint);
...
std::unique_ptr<GrFragmentProcessor> SkImageShader::asFragmentProcessor(
    ...
    GrSamplerState::Filter textureFilterMode = GrSkFilterQualityToGrFilterMode(
            fImage->width(), fImage->height(), this->resolveFilterQuality(args.fFilterQuality),
...
bool SkImageShader::doStages(const SkStageRec& rec, SkImageStageUpdater* updater) const {
    ...
    if (fFiltering != kInheritFromPaint) {
        quality = (SkFilterQuality)fFiltering;
    }
...
skvm::Color SkImageShader::onProgram(skvm::Builder* p, skvm::F32 x, skvm::F32 y, skvm::Color paint,
    ...
    // TODO: should we move this to the caller, so it can be done up-front?
    quality = this->resolveFilterQuality(quality);
