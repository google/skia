...
SkShaderBase::Context* SkImageShader::onMakeContext(const ContextRec& rec,
    ...
    return SkBitmapProcLegacyShader::MakeContext(*this, fTileModeX, fTileModeY, as_IB(fImage.get()),
                                                 rec, alloc);
}
#endif
...
std::unique_ptr<GrFragmentProcessor> SkImageShader::asFragmentProcessor(
    ...
    if (as_IB(fImage)->isYUVA()) {
    ...
    } else if (GrSurfaceProxyView view = as_IB(fImage)->refPinnedView(args.fContext, &pinnedUniqueID)) {
        ...
        if (args.fContext->priv().caps()->isFormatSRGB(view.proxy()->backendFormat())) {
            ...
            colorInfo = GrColorInfo(
                    GrColorType::kRGBA_8888_SRGB, fImage->alphaType(), fImage->refColorSpace());
        } else {
    ...
    auto fp = producer->createFragmentProcessor(lmInverse, SkRect::Make(fImage->dimensions()), GrTextureProducer::kNo_FilterConstraint, false, wmX, wmY, filterOrNull);
...
sk_sp<SkShader> SkMakeBitmapShader(const SkBitmap& src,
                                   SkTileMode tmx,
                                   SkTileMode tmy,
                                   const SkMatrix* localMatrix,
...
skvm::Color SkImageShader::onProgram(skvm::Builder* p,
    ...
    auto sample = [&](skvm::F32 sx, skvm::F32 sy) -> skvm::Color {
        ...
        switch (fTileModeX) {
            ...
            case SkTileMode::kClamp: /*    we always clamp   */
                break;
            case SkTileMode::kRepeat:
                sx = repeat(sx, pm.width());
                break;
            case SkTileMode::kMirror:
                sx = mirror(sx, pm.width());
                break;
        }
        switch (fTileModeY) {
            case SkTileMode::kDecal: /* handled after gather */
            ...
            case SkTileMode::kClamp: /*    we always clamp   */
                break;
            case SkTileMode::kRepeat:
                sy = repeat(sy, pm.height());
                break;
            case SkTileMode::kMirror:
                sy = mirror(sy, pm.height());
                break;
        }
        ...
        switch (pm.colorType()) {
            ...
            case kRGB_565_SkColorType:
                c = unpack_565(gather16(img, index));
                break;
    ...
    if (quality == kHigh_SkFilterQuality) {
        ...
        skvm::F32 limit =
                (at == kUnpremul_SkAlphaType || fClampAsIfUnpremul) ? p->splat(1.0f) : c.a;
