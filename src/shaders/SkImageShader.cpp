...
skvm::Color SkImageShader::onProgram(skvm::Builder* p,
    ...
    skvm::Coord upperLocal = SkShaderBase::ApplyMatrix(p, upperInv, origLocal, uniforms),
                lowerLocal;
    if (lower) {
        lowerInv = post_scale(lower->dimensions(), baseInv);
        lowerLocal = SkShaderBase::ApplyMatrix(p, lowerInv, origLocal, uniforms);
        lowerWeightUniform = p->uniformF(uniforms->pushF(lowerWeight));
    }
    ...
    // Except in the simpliest case (no mips, no filtering), we reference uniforms
    ...
    auto setup_uniforms = [&](const SkPixmap& pm) {
        uni.w   = p->uniformF(uniforms->pushF(     pm.width()));
        uni.iw  = p->uniformF(uniforms->pushF(1.0f/pm.width()));
        uni.i2w = p->uniformF(uniforms->pushF(0.5f/pm.width()));

        uni.h   = p->uniformF(uniforms->pushF(     pm.height()));
        uni.ih  = p->uniformF(uniforms->pushF(1.0f/pm.height()));
        uni.i2h = p->uniformF(uniforms->pushF(0.5f/pm.height()));

        uni.clamp_w = compute_clamp_limit(pm. width());
        uni.clamp_h = compute_clamp_limit(pm.height());

        uni.addr = uniforms->pushPtr(pm.addr());
        uni.rowBytesAsPixels  = p->uniform32(uniforms->push(pm.rowBytesAsPixels()));

        uni.colorType = pm.colorType();
    };

    auto sample_texel = [&](skvm::F32 sx, skvm::F32 sy) -> skvm::Color {
        ...
        switch (fTileModeX) {
            ...
            case SkTileMode::kRepeat: sx = repeat(sx, uni.w, uni.iw);  break;
            case SkTileMode::kMirror: sx = mirror(sx, uni.w, uni.i2w); break;
        }
        switch (fTileModeY) {
            case SkTileMode::kDecal:  /* handled after gather */  break;
            case SkTileMode::kClamp:  /*    we always clamp   */  break;
            case SkTileMode::kRepeat: sy = repeat(sy, uni.h, uni.ih);  break;
            case SkTileMode::kMirror: sy = mirror(sy, uni.h, uni.i2h); break;
        }
        ...
        skvm::F32 clamped_x = clamp(sx, 0, uni.clamp_w),
                  clamped_y = clamp(sy, 0, uni.clamp_h);

        // Load pixels from pm.addr()[(int)sx + (int)sy*stride].
        skvm::Uniform img = uni.addr;
        skvm::I32 index = trunc(clamped_x) +
                          trunc(clamped_y) * uni.rowBytesAsPixels;
        skvm::Color c;
        switch (uni.colorType) {
    ...
    auto sample_level = [&](const SkPixmap& pm, const SkMatrix& inv, skvm::Coord local) {
        ...
        if (sampling == SamplingEnum::kNearest) {
            return sample_texel(local.x,local.y);
            ...
            return lerp(lerp(sample_texel(left,top   ), sample_texel(right,top   ), fx),
                        lerp(sample_texel(left,bottom), sample_texel(right,bottom), fx), fy);
        } else {
            ...
            for (int j = 0; j < 4; j++, sy += 1.0f) {
                ...
                for (int i = 0; i < 4; i++, sx += 1.0f) {
                    skvm::Color s = sample_texel(sx,sy);
    ...
    if (lower) {
        ...
        c = lerp(sample_level(*lower, lowerInv, lowerLocal), c, lowerWeightUniform);
    }
