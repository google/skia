...
skvm::Color SkImageShader::onProgram(skvm::Builder* p,
    ...
    auto sample_texel = [&](const SkPixmap& pm, skvm::F32 sx, skvm::F32 sy) -> skvm::Color {
    ...
    auto sample_level = [&](const SkPixmap& pm, const SkMatrix& inv, skvm::Coord local) {
        if (sampling == SamplingEnum::kNearest) {
            return sample_texel(pm, local.x,local.y);
            ...
            return lerp(lerp(sample_texel(pm, left,top   ), sample_texel(pm, right,top   ), fx),
                        lerp(sample_texel(pm, left,bottom), sample_texel(pm, right,bottom), fx), fy);
        } else {
            ...
            for (int j = 0; j < 4; j++, sy += 1.0f) {
                ...
                for (int i = 0; i < 4; i++, sx += 1.0f) {
                    skvm::Color s = sample_texel(pm, sx,sy);
