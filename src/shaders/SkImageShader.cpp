...
SkShaderBase::Context* SkImageShader::onMakeContext(const ContextRec& rec,
    ...
    auto quality = (SkFilterQuality)this->resolveFilterEnum(rec.fPaint->getFilterQuality());
...
skvm::Color SkImageShader::onProgram(skvm::Builder* p,
    ...
    if (fFilterEnum == kUseFilterOptions) {
        auto* access = alloc->make<SkMipmapAccessor>(as_IB(fImage.get()), inv,
    ...
    auto sampling_stage = [&](const SkPixmap& pm) {
    ...
    sampling_stage(*upper);
    if (lower) {
        sampling_stage(*lower);
    }
