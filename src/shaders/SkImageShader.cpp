...
sk_sp<SkFlattenable> SkImageShader::CreateProc(SkReadBuffer& buffer) {
    ...
    if (buffer.isVersionLT(SkPicturePriv::kBicubicInImageShader_Version)) {
    ...
    switch (fe) {
        ...
        case kUse44Weights:
            return SkImageShader::Make(std::move(img), tmx, tmy, mx, &localMatrix);
        default:
            break;
    }
...
skvm::Color SkImageShader::onProgram(skvm::Builder* p,
    ...
    auto sample_level = [&](const SkPixmap& pm, const SkMatrix& inv, skvm::Coord local) {
        ...
        if (sampling == SamplingEnum::kNearest) {
            ...
            } else {    // legacy hard-codec weights
