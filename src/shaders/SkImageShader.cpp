...
sk_sp<SkFlattenable> SkImageShader::CreateProc(SkReadBuffer& buffer) {
    ...
    if (!buffer.isVersionLT(SkPicturePriv::kFilterEnumInImageShader_Version)) {
        fe = buffer.read32LE<FilterEnum>(kInheritFromPaint);
    }
    ...
    if (!buffer.isVersionLT(SkPicturePriv::kFilterOptionsInImageShader_Version)) {
        fo.fSampling = buffer.read32LE<SkSamplingMode>(SkSamplingMode::kNearest);
        fo.fMipmap   = buffer.read32LE<SkMipmapMode>(SkMipmapMode::kNone);
    }
