...
bool SkImageShader::onProgram(skvm::Builder* p,
    ...
    skvm::F32 clamp_x = clamp(x, pm. width()),
              clamp_y = clamp(y, pm.height());

    // Load pixels from pm.addr()[(int)x + (int)y*stride].
    skvm::Builder::Uniform img = uniforms->pushPtr(pm.addr());
    skvm::I32 index = p->add(p->trunc(clamp_x),
                      p->mul(p->trunc(clamp_y),
    ...
    skvm::I32 decal = p->splat(~0);
    if (fTileModeX == SkTileMode::kDecal) { decal = p->bit_and(decal, p->eq(x, clamp_x)); }
    if (fTileModeY == SkTileMode::kDecal) { decal = p->bit_and(decal, p->eq(y, clamp_y)); }
    if (fTileModeX == SkTileMode::kDecal || fTileModeY == SkTileMode::kDecal) {
        c.r = p->bit_cast(p->bit_and(decal, p->bit_cast(c.r)));
        c.g = p->bit_cast(p->bit_and(decal, p->bit_cast(c.g)));
        c.b = p->bit_cast(p->bit_and(decal, p->bit_cast(c.b)));
        c.a = p->bit_cast(p->bit_and(decal, p->bit_cast(c.a)));
    }
