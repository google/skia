...
sk_sp<GrTextureContext> GrRecordingContext::makeDeferredTextureContext(
    ...
    if (GrMipMapped::kNo == mipMapped) {
        texture = this->proxyProvider()->createProxy(format, desc, GrRenderable::kNo, origin, fit, budgeted);
    } else {
        texture = this->proxyProvider()->createMipMapProxy(format, desc, GrRenderable::kNo, origin, budgeted);
    }
    if (!texture) {
        return nullptr;
    }
...
sk_sp<GrRenderTargetContext> GrRecordingContext::makeDeferredRenderTargetContext(
    ...
    if (GrMipMapped::kNo == mipMapped) {
        rtp = this->proxyProvider()->createProxy(format, desc, GrRenderable::kYes, origin, fit, budgeted);
    } else {
        rtp = this->proxyProvider()->createMipMapProxy(format, desc, GrRenderable::kYes, origin, budgeted);
    }
    if (!rtp) {
        return nullptr;
    }
