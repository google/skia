...
sk_sp<GrSurfaceContext> GrRecordingContext::makeWrappedSurfaceContext(
    ...
    if (proxy->asRenderTargetProxy()) {
        ...
        SkASSERT(!props);
        return this->drawingManager()->makeTextureContext(std::move(proxy), colorType,
                                                          std::move(colorSpace));
    }
}
...
sk_sp<GrSurfaceContext> GrRecordingContext::makeDeferredSurfaceContext(
    ...
    sk_sp<GrSurfaceContext> sContext = this->makeWrappedSurfaceContext(
...
sk_sp<GrSurfaceContext> GrRecordingContextPriv::makeWrappedSurfaceContext(
        ...
        sk_sp<SkColorSpace> colorSpace,
        const SkSurfaceProps* props) {
    return fContext->makeWrappedSurfaceContext(std::move(proxy), colorType, std::move(colorSpace),
                                               props);
}
