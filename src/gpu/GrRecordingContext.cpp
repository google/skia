...
sk_sp<GrTextureContext> GrRecordingContext::makeDeferredTextureContext(
    ...
    auto format = this->caps()->getBackendFormatFromColorType1(colorType, GrRenderable::kNo);
    if (!format.isValid()) {
        return nullptr;
    }
...
sk_sp<GrRenderTargetContext> GrRecordingContext::makeDeferredRenderTargetContext(
    ...
    auto format = this->caps()->getBackendFormatFromColorType1(colorType, GrRenderable::kYes);
    if (!format.isValid()) {
        return nullptr;
    }
    auto config = this->caps()->getConfigFromBackendFormat(format, colorType);
...
GrBackendFormat GrRecordingContext::defaultBackendFormat(SkColorType skColorType,
    ...
    GrBackendFormat format = caps->getBackendFormatFromColorType1(grColorType, renderable);
    if (!format.isValid()) {
        return GrBackendFormat();
    }
