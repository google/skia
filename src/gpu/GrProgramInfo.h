...
public:
    GrProgramInfo(int numSamples,
        ...
        fRequestedFeatures = fPrimProc.requestedFeatures1();
        for (int i = 0; i < fPipeline.numFragmentProcessors(); ++i) {
            fRequestedFeatures |= fPipeline.getFragmentProcessor(i).requestedFeatures1();
        }
        fRequestedFeatures |= fPipeline.getXferProcessor().requestedFeatures1();
    }
