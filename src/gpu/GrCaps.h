...
public:
     ...
     * succeed as well as the proxy origin. renderable will be set to true if the destination
     * texture should be renderable in order to implement the copy. rectsMustMatch will be set to
     * true if the copy operation must ensure that the src and dest rects are identical.
     * disallowSubrect will be set to true if copy rect must equal src's bounds.
     */
    virtual bool initDescForDstCopy(const GrRenderTargetProxy* src, GrSurfaceDesc* desc, GrRenderable* renderable,
