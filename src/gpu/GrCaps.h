...
public:
    ...
    /**
     * Given a src surface's pixel config and it's backend format as well as a color type the caller
     * would like read into, this provides a legal color type that the caller may pass to
     * GrGpu::readPixels(). The color type may differ thand the passed dstColorType, in which case
     * the caller must convert the pixel data (see GrConvertPixels). When converting from to
     * dstColorType the swizzle in the returned struct should be applied. The caller must check the
     * returned color type for kUnknown.
     */
