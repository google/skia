...
inline bool circle_stays_circle(const SkMatrix& m) { return m.isSimilarity(); }
}
    ...
    public:
        ...
        void onEmitCode(EmitArgs& args, GrGPArgs* gpArgs) override {
            ...
            this->emitTransforms(vertBuilder, varyingHandler, uniformHandler, gpArgs->fPositionVar,
                                 cgp.fInPosition->fName, cgp.fLocalMatrix,
            ...
            if (args.fDistanceVectorName) {
                ...
                fragBuilder->codeAppendf(
                        "    %s = vec4(normalize(circleEdge.xy), distanceToOuterEdge, %s);",
...
private:
    ...
    public:
        ...
        void onEmitCode(EmitArgs& args, GrGPArgs* gpArgs) override {
            ...
            // emit transforms
            this->emitTransforms(vertBuilder, varyingHandler, uniformHandler, gpArgs->fPositionVar,
                                 egp.fInPosition->fName, egp.fLocalMatrix,
                                 args.fFPCoordTransformHandler);
...
private:
    ...
    public:
        ...
        void onEmitCode(EmitArgs& args, GrGPArgs* gpArgs) override {
            ...
            // Setup position
            this->setupPosition(vertBuilder, uniformHandler, gpArgs, diegp.fInPosition->fName,
                                diegp.fViewMatrix, &fViewMatrixUniform);

            // emit transforms
            this->emitTransforms(vertBuilder, varyingHandler, uniformHandler, gpArgs->fPositionVar,
                                 diegp.fInPosition->fName, args.fFPCoordTransformHandler);
...
static int rrect_type_to_vert_count(RRectType type) {
    static const int kTypeToVertCount[] = {
            kVertsPerStandardRRect, kVertsPerStandardRRect, kVertsPerOverstrokeRRect,
...
static int rrect_type_to_index_count(RRectType type) {
    static const int kTypeToIndexCount[] = {
            kIndicesPerFillRRect, kIndicesPerStrokeRRect, kIndicesPerOverstrokeRRect,
...
static const uint16_t* rrect_type_to_indices(RRectType type) {
    static const uint16_t* kTypeToIndices[] = {
            gStandardRRectIndices, gStandardRRectIndices, gOverstrokeRRectIndices,
...
public:
    ...
    RRectCircleRendererBatch(GrColor color, bool needsDistance, const SkMatrix& viewMatrix,
                             const SkRect& devRect, float devRadius, float devStrokeWidth,
                             bool strokeOnly)
...
static GrDrawOp* create_rrect_batch(GrColor color, bool needsDistance, const SkMatrix& viewMatrix,
...
GrDrawOp* GrOvalRenderer::CreateRRectBatch(GrColor color, bool needsDistance,
...
GrDrawOp* GrOvalRenderer::CreateOvalBatch(GrColor color, const SkMatrix& viewMatrix,
...
GrDrawOp* GrOvalRenderer::CreateArcBatch(GrColor color, const SkMatrix& viewMatrix,
                                         const SkRect& oval, SkScalar startAngle,
                                         SkScalar sweepAngle, bool useCenter, const GrStyle& style,
                                         const GrShaderCaps* shaderCaps) {
    ...
    SkPoint center = {oval.centerX(), oval.centerY()};
    CircleBatch::ArcParams arcParams = {SkDegreesToRadians(startAngle),
                                        SkDegreesToRadians(sweepAngle), useCenter};
