    ...
    public:
        ...
        void onEmitCode(EmitArgs& args, GrGPArgs* gpArgs) override{
            ...
            if (args.fDistanceVectorName) {
                ...
                // in the interior of the circle (which for rrects can be a rect)
                fragBuilder->codeAppend ("if ((circleEdge.z * d) <= 0.0) {");
                fragBuilder->codeAppend ("    float rectDist = abs(max(circleEdge.x, circleEdge.y));");
                fragBuilder->codeAppend ("    distanceToOuterEdge = (rectDist + 1.0) * abs(circleEdge.z);");
                fragBuilder->codeAppend ("    edgeAlpha = 1.0;");
                fragBuilder->codeAppendf("    %s = vec4(1.0, 0.0, distanceToOuterEdge, %s);",
    ...
    void onPrepareDraws(Target* target) const override {
        ...
        for (int i = 0; i < instanceCount; i++) {
            ...
            if (kFillWithDist_RRectType == args.fType) {
                SkScalar xDepth, yDepth;
                SkScalar xs[2], ys[2];

                if (SkScalarNearlyEqual(bounds.width(), bounds.height())) {
                    xs[0] = xs[1] = bounds.centerX();
                    ys[0] = ys[1] = bounds.centerY();
                    xDepth = -(bounds.width() / outerRadius - 2);
                    yDepth = -(bounds.height() / outerRadius - 2);
                } else if (bounds.width() > bounds.height()) {
                    xs[0] = bounds.fLeft + bounds.height()/2.0f;
                    xs[1] = bounds.fRight - bounds.height()/2.0f;
                    ys[0] = ys[1] = bounds.centerY();
                    xDepth = yDepth = -(bounds.height() / outerRadius - 2);
                } else {
                    xs[0] = xs[1] = bounds.centerX();
                    ys[0] = bounds.fTop + bounds.width() / 2.0f;
                    ys[1] = bounds.fBottom - bounds.width()/2.0f;
                    xDepth = yDepth = -(bounds.width() / outerRadius - 2);
                }
                ...
                verts->fPos = SkPoint::Make(xs[0], ys[0]);
                verts->fColor = color;
                verts->fOffset = SkPoint::Make(xDepth, yDepth);
                verts->fOuterRadius = -outerRadius;
                verts->fInnerRadius = 0.0f;
                verts++;

                verts->fPos = SkPoint::Make(xs[1], ys[0]);
                verts->fColor = color;
                verts->fOffset = SkPoint::Make(xDepth, yDepth);
                verts->fOuterRadius = -outerRadius;
                verts->fInnerRadius = 0.0f;
                ...
                verts->fPos = SkPoint::Make(xs[0], ys[1]);
                verts->fColor = color;
                verts->fOffset = SkPoint::Make(xDepth, yDepth);
                verts->fOuterRadius = -outerRadius;
                verts->fInnerRadius = 0.0f;
                ...
                verts->fPos = SkPoint::Make(xs[1], ys[1]);
                verts->fColor = color;
                verts->fOffset = SkPoint::Make(xDepth, yDepth);
                verts->fOuterRadius = -outerRadius;
                verts->fInnerRadius = 0.0f;
