 ...
 * the applyPipelineAnalysis call, where the op can use the information to inform decisions about
 * GrPrimitiveProcessor creation.
 */
 ...
 * GrOp which can inform its decisions for GrPrimitiveProcessor setup. These are not properly
 * part of the pipeline because they reflect the specific inputs that the op provided to perform
 * the analysis (e.g. that the GrGeometryProcessor would output an opaque color).
 *
 * The analysis may have allowed GrProcessor's to be elided. However, those eliminations may
 * depend upon changing the color output by the GrGeometryProcessor used by the GrDrawOp. The
 * op must check getOverrideColorIfSet() for this.
 */
class GrPipelineAnalysisResult {
public:
