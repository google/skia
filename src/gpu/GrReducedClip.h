...
public:
    ...
    GrReducedClip(const SkClipStack&, const SkRect& queryBounds, int maxWindowRectangles = 0,
                  int maxAnalyticFPs = 0, GrCoverageCountingPathRenderer* = nullptr);
    ...
    /**
     * Called once the client knows the ID of the opList the clip FPs will operate in. This method
     * finishes any outstanding work that was waiting for the opList ID and returns a list of FPs
     * that complete the clip.
     *
     ...
     */
    std::unique_ptr<GrFragmentProcessor> finishAndDetachAnalyticFPs(uint32_t opListID);
...
private:
    ...
    SkSTArray<4, SkPath> fCCPRClipPaths; // Will be converted to CCPR FPs once we have an opList ID.
};
