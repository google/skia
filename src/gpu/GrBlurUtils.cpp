...
static void draw_shape_with_mask_filter2(GrContext* context,
    ...
    if (shape.style().modifiesGeometry()) {
        ...
        tempShape = shape.applyStyle(GrStyle::Apply::kPathEffectAndStrokeRec, styleScale);
        if (tempShape.isEmpty()) {
            return;
        }
    }

    SkRect devSpaceShapeBounds = shape.bounds();
    viewMatrix.mapRect(&devSpaceShapeBounds);

    SkRect maskRect;
    if (maskFilter->canFilterMaskGPU(shape, //SkRRect::MakeRect(path->getBounds()),
                                     devSpaceShapeBounds,
                                     clipConservativeBounds,
                                     viewMatrix,
                                     &maskRect)) {
        ...
        SkASSERT(!shape.style().modifiesGeometry());
        ...
        if (useCache) {
            ...
            GrUniqueKey::Builder builder(&maskKey, kDomain, 5 + shape.unstyledKeySize(),
                                         "SW Path Mask");
            ...
            uint32_t styleBits = shape.style().isSimpleHairline() ?
                                 ((shape.style().strokeRec().getCap() << 1) | 1) : 0;
            builder[4] = fracX | (fracY >> 8) | (styleBits << 16);
            shape.writeUnstyledKey(&builder[5]);
        }
