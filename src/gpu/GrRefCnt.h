...
// We have to use auto for the function pointers here because if the actual functions line of the
...
public:
    ...
    /**
     * Copies the underlying object pointer from the argument to the gr_sb. It will then call
    ...
    gr_sp<T, Ref, Unref>& operator=(std::nullptr_t) {
        this->reset();
        return *this;
    }
     ...
     */
    gr_sp<T, Ref, Unref>& operator=(const gr_sp<T, Ref, Unref>& that) {
        if (this != &that) {
     ...
     * Ref() on the new object.
     */
    gr_sp<T, Ref, Unref>& operator=(const sk_sp<T>& that) {
        this->reset(SafeRef(that.get()));
        return *this;
    }
     ...
     * be made.
     */
    gr_sp<T, Ref, Unref>& operator=(gr_sp<T, Ref, Unref>&& that) {
     ...
     * Ref() on the new object.
     */
    gr_sp<T, Ref, Unref>& operator=(sk_sp<T>&& that) {
        this->reset(SafeRef(that.get()));
        return *this;
    }
