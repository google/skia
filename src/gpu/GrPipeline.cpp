...
GrPipeline::GrPipeline(const InitArgs& args, GrProcessorSet&& processors,
    ...
    int numTotalProcessors = fNumColorProcessors +
                             ...
                             (appliedClip.hasClipCoverageFragmentProcessor() ? 1 : 0);
    ...
    if (appliedClip.hasClipCoverageFragmentProcessor()) {
        fFragmentProcessors[currFPIdx++] = appliedClip.detachClipCoverageFragmentProcessor();
    }
}
