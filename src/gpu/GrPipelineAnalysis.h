...
/**
 * GrColorFragmentProcessorAnalysis gathers invariant data from a set of color fragmetn processor.
 * It is used to recognize optimizations related to eliminating stages and vertex attributes that
 * aren't necessary for a draw.
 */
class GrColorFragmentProcessorAnalysis {
public:
    GrColorFragmentProcessorAnalysis() = default;

    GrColorFragmentProcessorAnalysis(const GrPipelineAnalysisColor& input) : GrColorFragmentProcessorAnalysis() {
    ...
    void reset(const GrPipelineAnalysisColor& input) { *this = GrColorFragmentProcessorAnalysis(input); }
