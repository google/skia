...
public:
    ...
    // is empty then return false and this is unmodified. If clipped the input x/y, the width/
    ...
    bool clip(int surfaceWidth, int surfaceHeight, int* x, int* y, T** data, size_t rowBytes) {
        ...
        *data = static_cast<T*>(static_cast<C*>(*data) + (rect.fTop - *y) * rowBytes +
                                (rect.fLeft - *x) * this->bpp());
        *x = rect.fLeft;
        *y = rect.fTop;
        fWidth = rect.width();
        fHeight = rect.height();
