...
void GrOpFlushState::doUpload(GrDrawOp::DeferredUploadFn& upload) {
    GrDrawOp::WritePixelsFn wp = [this](GrTextureProxy* proxy,
        ...
        fGpu->getWritePixelsInfo(proxy, width, height, proxy->config(), &drawPreference,
                                 &tempInfo);
        if (GrGpu::kNoDraw_DrawPreference == drawPreference) {
            return this->fGpu->writePixels(surface, proxy->origin(), left, top, width, height, config, buffer,
                                           rowBytes);
        }
        GrSurfaceDesc desc;
        ...
        sk_sp<GrTexture> temp(this->fResourceProvider->createApproxTexture(
                desc, proxy->origin(), GrResourceProvider::kNoPendingIO_Flag));
        if (!temp) {
            return false;
        }
        if (!fGpu->writePixels(temp.get(), proxy->origin(), 0, 0, width, height, desc.fConfig, buffer, rowBytes)) {
            return false;
        }
