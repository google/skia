...
sk_sp<GrTexture> GrDawnGpu::onCreateTexture(const GrSurfaceDesc& desc,
    ...
    tex->upload(texels, mipLevelCount, getCopyEncoder());
    return tex;
}
...
bool GrDawnGpu::onCopySurface(GrSurface* dst,
    ...
    getCopyEncoder().CopyTextureToTexture(&srcTextureView, &dstTextureView, &copySize);
    return true;
}
...
bool GrDawnGpu::onReadPixels(GrSurface* surface, int left, int top, int width, int height,
    ...
    getCopyEncoder().CopyTextureToBuffer(&srcTexture, &dstBuffer, &copySize);
    flush();
