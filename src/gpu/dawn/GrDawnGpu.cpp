...
std::unique_ptr<GrStagingBuffer> GrDawnGpu::onCreateStagingBuffer(size_t size) {
    ...
    GrDawnStagingBuffer* stagingBuffer = new GrDawnStagingBuffer(this, result.buffer, desc.size, result.data);
...
void GrDawnGpu::mapStagingBuffers() {
    // Map all busy buffers, so we get a callback when they're done.
    for (const auto &b : fStagingBuffers) {
        GrStagingBuffer* buffer = b.get();
