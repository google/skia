...
sk_sp<GrTexture> GrDawnGpu::onCreateTexture(const GrSurfaceDesc& desc, SkBudgeted budgeted,
    ...
    tex->upload(texels, mipLevels, fCopyBuilder);
    return tex;
}
...
sk_sp<GrTexture> GrDawnGpu::onWrapRenderableBackendTexture(const GrBackendTexture& tex,
    ...
    GrMipMapsStatus status = GrMipMapsStatus::kNotAllocated;
    return GrDawnTexture::MakeWrapped(this, desc, status, info);
}
...
GrBackendTexture GrDawnGpu::createTestingOnlyBackendTexture(const void* pixels,
    ...
    if (GrMipMapped::kYes == mipMapped && isRenderTarget) {
        return GrBackendTexture();
    }
    ...
    if (GrMipMapped::kYes == mipMapped) {
        desc.levelCount = SkMipMap::ComputeLevelCount(width, height) + 1;
    } else {
        desc.levelCount = 1;
    }
    ...
    for (uint32_t i = 0; i < desc.levelCount; i++) {
        ...
        copyBuilder.CopyBufferToTexture(&srcBuffer, &dstTexture, &copySize);
        w = SkTMax(1, w / 2);
        h = SkTMax(1, h / 2);
    }
    ...
    info.fLevelCount = desc.levelCount;
...
bool GrDawnGpu::onReadPixels(GrSurface* surface,
    ...
    auto commandBuffer = device().CreateCommandBufferBuilder()
        .CopyTextureToBuffer(&srcTexture, &dstBuffer, &copySize)
        .GetResult();
...
sk_sp<GrDawnProgram> GrDawnGpu::getOrCreateRenderPipeline(GrRenderTarget* rt,
    ...
    GrProgramDesc desc;
    GrProgramDesc::Build(&desc, primProc, hasPoints, pipeline, this);
    ...
    sk_sp<GrDawnProgram> program = GrDawnProgramBuilder::Build(
        this, pipeline, primProc, primProcProxies, primitiveType, colorFormat, hasDepthStencil, stencilFormat, &desc);
