...
bool GrDawnGpu::onSubmitToGpu(bool syncCpu) {
    std::vector<sk_sp<GrGpuBuffer>> stagingBuffers;
    if (fStagingBufferManager.hasBuffers()) {
        auto moveStagingBuffersToBusy = [&](sk_sp<GrGpuBuffer> buffer) {
    ...
    this->moveStagingBuffersToBusyAndMap(stagingBuffers);
    if (syncCpu) {
...
void GrDawnGpu::moveStagingBuffersToBusyAndMap(std::vector<sk_sp<GrGpuBuffer>>& stagingBuffers) {
    for (size_t i = 0; i < stagingBuffers.size(); ++i) {
        GrDawnBuffer* buffer = static_cast<GrDawnBuffer*>(stagingBuffers[i].get());
        buffer->mapWriteAsync();
        fBusyStagingBuffers.push_back(std::move(stagingBuffers[i]));
    }
