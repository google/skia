...
bool GrDawnGpu::onRegenerateMipMapLevels(GrTexture* tex) {
    ...
    texDesc.usage = wgpu::TextureUsage::Sampled |
                    wgpu::TextureUsage::CopySrc |
                    wgpu::TextureUsage::OutputAttachment;
    texDesc.size.width = ceilf(tex->width() * 0.5f);
    texDesc.size.height = ceilf(tex->height() * 0.5f);
    ...
    const char* vs =
        ...
        "float2 positions[4] = float2[4](float2(-1.0, 1.0), float2(1.0, 1.0), float2(-1.0, -1.0), float2(1.0, -1.0));\n"
        "float2 texCoords[4] = float2[4](float2(0.0, 0.0), float2(1.0, 0.0), float2(0.0, 1.0), float2(1.0, 1.0));\n"
        "void main() {\n"
    ...
    vertexStateDesc.vertexBufferCount = 0;
    vertexStateDesc.vertexBuffers = nullptr;
    ...
    for (uint32_t mipLevel = 0; mipLevel < texDesc.mipLevelCount; mipLevel++) {
        ...
        const float* c = debugColors[mipLevel % 12];
    ...
    wgpu::CommandBuffer cb = commandEncoder.Finish();
    fQueue.Submit(1, &cb);
    return true;
}
