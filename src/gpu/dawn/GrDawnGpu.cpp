...
sk_sp<GrTexture> GrDawnGpu::onCreateTexture(const GrSurfaceDesc& desc, GrRenderable renderable,
    ...
    sk_sp<GrDawnTexture> tex = GrDawnTexture::Make(this, desc, renderable, budgeted, mipMapsStatus);
    if (!tex) {
        return nullptr;
    }
...
sk_sp<GrTexture> GrDawnGpu::onWrapBackendTexture(const GrBackendTexture& backendTex,
    ...
    sk_sp<GrDawnTexture> tgt = GrDawnTexture::MakeWrapped(this, desc, GrRenderable::kNo, status,
                                                          cacheable, info);
    return tgt;
}
...
sk_sp<GrTexture> GrDawnGpu::onWrapRenderableBackendTexture(const GrBackendTexture& tex,
    ...
    GrMipMapsStatus status = GrMipMapsStatus::kNotAllocated;
    return GrDawnTexture::MakeWrapped(this, desc, GrRenderable::kYes, status, cacheable, info);
}
...
GrDawnStagingBuffer* GrDawnGpu::getStagingBuffer(size_t size) {
    return fStagingManager.findOrCreateStagingBuffer(size, dawn::BufferUsageBit::CopySrc);
}
