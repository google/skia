...
sk_sp<GrGpuBuffer> GrDawnGpu::onCreateBuffer(size_t size, GrGpuBufferType type,
    ...
    if (type == GrGpuBufferType::kXferCpuToGpu || type == GrGpuBufferType::kXferGpuToCpu) {
        ...
        b.reset(new GrDawnBuffer(this, size, type, accessPattern));
    }
    if (data && b) {
...
void GrDawnGpu::checkForCompletedStagingBuffers() {
    while (!fBusyStagingBuffers.empty()) {
        ...
        GrDawnTransferBuffer* buffer = static_cast<GrDawnTransferBuffer*>(front.get());
        if (buffer->mappedState() == GrDawnTransferBuffer::MappedState::kMapped) {
            fBusyStagingBuffers.pop_front();
        } else {
            // We expect all the buffer maps to trigger in order of submission so we bail after the
            // first non finished map since we always push new busy buffers to the back of our list.
            break;
        }
    }
}
...
bool GrDawnGpu::onSubmitToGpu(bool syncCpu) {
    if (fStagingBufferManager.hasBuffers()) {
        auto moveStagingBuffersToBusy = [this](sk_sp<GrGpuBuffer> buffer) {
...
void GrDawnGpu::mapStagingBuffers() {
    ...
    for (auto it = fBusyStagingBuffers.rbegin(); it != fBusyStagingBuffers.rend(); ++it) {
        GrDawnTransferBuffer* buffer = static_cast<GrDawnTransferBuffer*>(it->get());
        // Since we always push back new busy buffers to the back any buffers that are still needing
        // to be mapped will be grouped at the end of the list. Thus we can stop our iterator as
        // soon as we find an already mapped buffer.
        if (buffer->mappedState() != GrDawnTransferBuffer::MappedState::kNotMapped) {
            break;
        }
