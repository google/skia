...
GrDawnGpu::~GrDawnGpu() {
    while (fStagingBufferBusyCount > 0) {
        fDevice.Tick();
    }
}
...
void GrDawnGpu::unmapStagingBuffers() {
    // Unmap all non-empty, non-busy buffers.
    for (const auto &b : fStagingBuffers) {
        GrDawnStagingBuffer* buffer = b.get();
        if (!buffer->empty() && !buffer->busy()) {
            buffer->unmap();
            fStagingBufferBusyCount++;
        }
    }
}

void GrDawnGpu::mapStagingBuffers() {
    // Map all non-empty, non-busy buffers.
    for (const auto &b : fStagingBuffers) {
        GrDawnStagingBuffer* buffer = b.get();
        if (!buffer->empty() && !buffer->busy()) {
            buffer->mapAsync();
...
void GrDawnGpu::markStagingBufferReady() {
    fStagingBufferBusyCount--;
}
...
GrDawnStagingBuffer* GrDawnGpu::findStagingBuffer(size_t size) {
    for (const auto &b : fStagingBuffers) {
        if (!b->busy() && b->remaining() >= size) {
            return b.get();
        }
    }
    return nullptr;
...
GrDawnStagingBuffer::Slice GrDawnGpu::allocateStagingBufferSlice(size_t size) {
    ...
    GrDawnStagingBuffer* stagingBuffer = this->findStagingBuffer(size);
    if (!stagingBuffer) {
        wgpu::BufferDescriptor desc;
