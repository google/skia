...
GrGpuRTCommandBuffer* GrDawnGpu::createCommandBuffer(
...
GrGpuTextureCommandBuffer* GrDawnGpu::createCommandBuffer(GrTexture* texture,
...
void GrDawnGpu::clearStencil(GrRenderTarget* target, int clearValue) {
    // FIXME: this clear could be integrated into the next draw's subpass
    auto attachment = static_cast<GrDawnStencilAttachment*>(target->renderTargetPriv().getStencilAttachment());
    ...
    builder.BeginRenderPass(renderPassDescriptor).EndPass();
    appendCommandBuffer(builder.GetResult());
}
