...
void GrDawnStagingManager::mapBusyList() {
    // Map all non-empty, non-busy buffers for writing and mark them as busy. When they're no
    // longer in flight on the GPU, their callback will be called and they'll be moved to the
    // ready pool.
    for (const auto &b : fBuffers) {
        GrDawnStagingBuffer* buffer = b.get();
        if (buffer->fOffset > 0 && !buffer->fBusy) {
