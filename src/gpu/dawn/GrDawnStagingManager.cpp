...
GrDawnStagingManager::GrDawnStagingManager(dawn::Device device) : fDevice(device.Clone()) {
}
...
static void init_callback(dawnBufferMapAsyncStatus status, void* data,
...
GrDawnStagingBuffer* GrDawnStagingManager::findOrCreateStagingBuffer(size_t size,
    ...
    if (i != fReadyPool.end()) {
        ...
        std::unique_ptr<GrDawnStagingBuffer> b(new GrDawnStagingBuffer(this, buffer.Clone(), sizePow2, usage));
        stagingBuffer = b.get();
        fBuffers.push_back(std::move(b));
        buffer.MapWriteAsync(0, sizePow2, init_callback, reinterpret_cast<dawn::CallbackUserdata>(stagingBuffer));
...
static void callback(dawnBufferMapAsyncStatus status, void* data, dawnCallbackUserdata userData) {
...
void GrDawnStagingManager::mapBusyList() {
    ...
    for (GrDawnStagingBuffer* buffer : fBusyList) {
        buffer->fBuffer.MapWriteAsync(0, buffer->fSize, callback,
                                      reinterpret_cast<dawn::CallbackUserdata>(buffer));
        fWaitingCount++;
    }
    fBusyList.clear();
}
