...
static void callback(WGPUBufferMapAsyncStatus status, void* userData) {
    static_cast<GrDawnBuffer*>(userData)->setMapPtr();
}

void GrDawnBuffer::setMapPtr() {
    fMapPtr = fBuffer.GetMappedRange();
}

void GrDawnBuffer::mapWriteAsync() {
    SkASSERT(!this->isMapped());
    fBuffer.MapAsync(wgpu::MapMode::Write, 0, 0, callback, this);
}
void GrDawnBuffer::mapReadAsync() {
    SkASSERT(!this->isMapped());
    fBuffer.MapAsync(wgpu::MapMode::Read, 0, 0, callback, this);
}
