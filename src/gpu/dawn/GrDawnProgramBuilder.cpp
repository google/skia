...
dawn::ColorStateDescriptor create_color_state(const GrDawnGpu* gpu, const GrPipeline& pipeline,
    ...
    descriptor.colorWriteMask = mask;
...
dawn::DepthStencilStateDescriptor create_depth_stencil_state(const GrStencilSettings& stencilSettings, dawn::TextureFormat depthStencilFormat) {
    ...
    if (stencilSettings.isDisabled()) {
        ...
        state.stencilFront = to_stencil_state_face(stencilSettings.front());
        if (stencilSettings.isTwoSided()) {
            state.stencilBack = to_stencil_state_face(stencilSettings.back());
        } else {
...
sk_sp<GrDawnProgram> GrDawnProgramBuilder::Build(GrDawnGpu* gpu,
    ...
    std::vector<dawn::VertexInputDescriptor> inputs;
    std::vector<dawn::VertexAttributeDescriptor> attributes;
    ...
    if (primProc.numVertexAttributes() > 0) {
        ...
        for (const auto& attrib : primProc.vertexAttributes()) {
            ...
            attribute.inputSlot = vertexBindingSlot;
        ...
        input.inputSlot = vertexBindingSlot;
        input.stride = offset;
    ...
    if (primProc.numInstanceAttributes() > 0) {
        ...
        for (const auto& attrib : primProc.instanceAttributes()) {
            dawn::VertexAttributeDescriptor attribute;
            attribute.shaderLocation = i;
            attribute.inputSlot = instanceBindingSlot;
            attribute.offset = offset;
            attribute.format = to_dawn_vertex_format(attrib.cpuType());
            attributes.push_back(attribute);
            offset += attrib.sizeAlign4();
        ...
        dawn::VertexInputDescriptor input;
        input.inputSlot = instanceBindingSlot;
        input.stride = offset;
        input.stepMode = dawn::InputStepMode::Instance;
        inputs.push_back(input);
    }
    dawn::InputStateDescriptor inputState;
    inputState.numInputs = inputs.size();
    inputState.inputs = &inputs.front();
    inputState.numAttributes = attributes.size();
    inputState.attributes = &attributes.front();
    inputState.indexFormat = dawn::IndexFormat::Uint16;
