...
void set_stencil_function(dawn::DepthStencilStateBuilder& builder, dawn::Face dawnFace, const GrStencilSettings::Face& face) {
     builder.SetStencilFunction(dawnFace,
                                to_dawn_compare_function(face.fTest),
                                to_dawn_stencil_operation(face.fFailOp),
                                to_dawn_stencil_operation(face.fFailOp),
                                to_dawn_stencil_operation(face.fPassOp));
}
...
sk_sp<GrDawnProgram> GrDawnProgramBuilder::Build(GrDawnGpu* gpu,
                                               ...
                                               GrProgramDesc* desc) {
    GrDawnProgramBuilder builder(gpu, primProc, pipeline, desc);
    ...
    if (0 != geometryUniformSize) {
        bindings.push_back({ GrDawnUniformHandler::kGeometryBinding, dawn::ShaderStageBit::Vertex, dawn::BindingType::UniformBuffer});
    }
    if (0 != fragmentUniformSize) {
        bindings.push_back({ GrDawnUniformHandler::kFragBinding, dawn::ShaderStageBit::Fragment, dawn::BindingType::UniformBuffer});
    }
    ...
    if (primProc.numVertexAttributes() > 0) {
        ...
        for (int i = 0; i < primProc.numVertexAttributes(); i++) {
            ...
            inputStateBuilder
                .SetAttribute(i, vertexBindingSlot, to_dawn_vertex_format(attrib.type()), offset);
    ...
    if (primProc.numInstanceAttributes() > 0) {
        ...
        for (int i = 0; i < primProc.numInstanceAttributes(); i++) {
            ...
            inputStateBuilder
                .SetAttribute(i, instanceBindingSlot, to_dawn_vertex_format(attrib.type()), offset);
        }
    ...
    renderPipelineBuilder
        ...
        .SetPrimitiveTopology(to_dawn_primitive_topology(primitiveType));
    if (hasDepthStencil) {
...
static void setTexture(GrDawnGpu* gpu, const GrSamplerState& state, GrTexture* texture, dawn::BindGroupBuilder& builder, int* binding) {
...
void GrDawnProgram::buildKey(BindGroupKey* key, const GrPrimitiveProcessor& primProc, const GrPipeline& pipeline) {
...
dawn::BindGroup GrDawnProgram::setData(GrDawnGpu* gpu, const GrPrimitiveProcessor& primProc, const GrPipeline& pipeline) {
    ...
    if (0 != geometryUniformSize) {
        ...
        auto view = geom.fBuffer.CreateBufferViewBuilder()
            ...
            .GetResult();
        bindGroupBuilder.SetBufferViews(GrDawnUniformHandler::kGeometryBinding, 1, &view);
    ...
    if (0 != fragmentUniformSize) {
        ...
        auto view = frag.fBuffer.CreateBufferViewBuilder()
            ...
            .GetResult();
        bindGroupBuilder.SetBufferViews(GrDawnUniformHandler::kFragBinding, 1, &view);
