...
static dawn::BindGroupBinding MakeBindGroupBinding(uint32_t binding, const dawn::Sampler& sampler, const dawn::TextureView& textureView, const dawn::BufferView& bufferView) {
...
static dawn::BindGroupBinding MakeBindGroupBinding(uint32_t binding, const dawn::Sampler& sampler) {
    return MakeBindGroupBinding(binding, sampler, nullptr, nullptr);
}

static dawn::BindGroupBinding MakeBindGroupBinding(uint32_t binding, const dawn::TextureView& textureView) {
    return MakeBindGroupBinding(binding, nullptr, textureView, nullptr);
}

static dawn::BindGroupBinding MakeBindGroupBinding(uint32_t binding, const dawn::BufferView& bufferView) {
    return MakeBindGroupBinding(binding, nullptr, nullptr, bufferView);
}
...
dawn::BindGroup GrDawnProgram::setData(GrDawnGpu* gpu, const GrPrimitiveProcessor& primProc, const GrPipeline& pipeline, const GrTextureProxy* const primProcTextures[]) {
    ...
    if (0 != geometryUniformSize) {
        ...
        auto view = geom.fBuffer.CreateBufferViewBuilder()
            ...
            .GetResult();
        bindings.push_back(MakeBindGroupBinding(GrDawnUniformHandler::kGeometryBinding, view));
    ...
    if (0 != fragmentUniformSize) {
        ...
        auto view = frag.fBuffer.CreateBufferViewBuilder()
            ...
            .GetResult();
        bindings.push_back(MakeBindGroupBinding(GrDawnUniformHandler::kFragBinding, view));
