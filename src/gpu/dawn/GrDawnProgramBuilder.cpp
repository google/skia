...
dawn::BlendStateDescriptor create_blend_state(const GrDawnGpu* gpu, const GrPipeline& pipeline) {
    ...
    descriptor.blendEnabled = !blendOff;
...
dawn::DepthStencilStateDescriptor create_depth_stencil_state(const GrStencilSettings& stencilSettings) {
    ...
    state.front = to_stencil_state_face(stencilSettings.front());
    if (stencilSettings.isTwoSided()) {
        state.back = to_stencil_state_face(stencilSettings.back());
    } else {
...
sk_sp<GrDawnProgram> GrDawnProgramBuilder::Build(GrDawnGpu* gpu,
    ...
    if (pipeline.isStencilEnabled()) {
        ...
        depthStencilState = create_depth_stencil_state(stencil);
    } else {
