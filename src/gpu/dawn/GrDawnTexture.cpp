...
sk_sp<GrDawnTexture> GrDawnTexture::Make(GrDawnGpu* gpu, const GrSurfaceDesc& desc,
    ...
    if (renderTarget) {
        ...
        result = sk_sp<GrDawnTexture>(new GrDawnTexture(gpu, tex.Clone(), textureView.Clone(),
                                                      desc, info, status));
    }
...
sk_sp<GrDawnTexture> GrDawnTexture::MakeWrapped(GrDawnGpu* gpu, const GrSurfaceDesc& desc,
    ...
    sk_sp<GrDawnTexture> tex(new GrDawnTexture(gpu, texture.Clone(), textureView.Clone(), desc, info, status));
