...
void GrOpsTask::planMerge(const SkTInternalLList<GrRenderTask>& tasks) {
...
void GrOpsTask::mergeForward(const SkTInternalLList<GrRenderTask>& tasks) {
    ...
    // First pass: determine how much memory we need and carry over primitive fields.
    ...
    for (GrRenderTask* task = fNext; task; task = task->fNext) {
        auto opsTask = task->asOpsTask();
        if (!opsTask || !opsTask->isSetFlag(kMerged_Flag)) {
            break;
        }
    ...
    // Second pass: move array contents and skip merged tasks.
    fLastClipStackGenID = SK_InvalidUniqueID;
