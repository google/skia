...
inline GrOpsTask::OpChain::List::List(GrOp::OpOwner op)
...
inline GrOp::OpOwner GrOpsTask::OpChain::List::popHead() {
    SkASSERT(fHead);
...
inline GrOp::OpOwner GrOpsTask::OpChain::List::removeOp(GrOp* op) {
#ifdef SK_DEBUG
    auto head = op;
...
inline void GrOpsTask::OpChain::List::pushHead(GrOp::OpOwner op) {
    SkASSERT(op);
...
inline void GrOpsTask::OpChain::List::pushTail(GrOp::OpOwner op) {
    SkASSERT(op->isChainTail());
...
GrOpsTask::OpChain::OpChain(GrOp::OpOwner op,
        ...
        while (a) {
            ...
            if (merged) {
                ...
                if (canBackwardMerge) {
                    ...
                    GrOp::OpOwner detachedA = chainA.removeOp(a);
                    // The unique_ptr releases the op.
                    chainB.popHead();
...
GrOp::OpOwner GrOpsTask::OpChain::appendOp(
        GrOp::OpOwner op, GrProcessorSet::Analysis processorAnalysis,
...
void GrOpsTask::recordOp(
        GrOp::OpOwner op, GrProcessorSet::Analysis processorAnalysis, GrAppliedClip* clip,
