...
void GrOpsTask::onPrepare(GrOpFlushState* flushState) {
    SkASSERT(fTargetView.asSurfaceProxy()->peekRenderTarget());
    SkASSERT(this->isClosed());
#ifdef SK_BUILD_FOR_ANDROID_FRAMEWORK
    TRACE_EVENT0("skia.gpu", TRACE_FUNC);
#endif
...
void GrOpsTask::setColorLoadOp(GrLoadOp op, const SkPMColor4f& color) {
    ...
    if (GrLoadOp::kClear == fColorLoadOp) {
        GrSurfaceProxy* proxy = fTargetView.asSurfaceProxy();
        SkASSERT(proxy);
        fTotalBounds.setWH(proxy->width(), proxy->height());
    }
}
...
void GrOpsTask::gatherProxyIntervals(GrResourceAllocator* alloc) const {
    ...
    GrSurfaceProxy* targetProxy = fTargetView.asSurfaceProxy();
...
void GrOpsTask::recordOp(
    ...
    GrSurfaceProxy* proxy = fTextureView.asSurfaceProxy();
    SkASSERT(proxy);
...
GrRenderTask::ExpectedOutcome GrOpsTask::onMakeClosed(
    ...
    if (!this->isNoOp()) {
        GrSurfaceProxy* proxy = fTargetView.asSurfaceProxy();
        SkRect clippedContentBounds = SkRect::MakeIWH(proxy->width(), proxy->height());
