...
int GrOpsTask::mergeFrom(SkSpan<const sk_sp<GrRenderTask>> tasks) {
    // Find the index of the last color clear. -1 indicates this or "there are none."
    ...
    for (const sk_sp<GrRenderTask>& task : tasks) {
        ...
        if (opsTask->fColorLoadOp == GrLoadOp::kClear) {
    ...
    if (indexOfLastColorClear >= 0) {
        ...
        fLoadClearColor = opsTasks[indexOfLastColorClear]->fLoadClearColor;
