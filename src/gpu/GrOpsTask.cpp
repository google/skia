    ...
    do {
        ...
        while (a) {
            ...
            if (canForwardMerge || canBackwardMerge) {
                auto result = a->combineIfPossible(chainB.head(), arena, caps);
...
bool GrOpsTask::OpChain::tryConcat(
        ...
        SkArenaAlloc* arena, GrAuditTrail* auditTrail) {
    ...
    do {
        switch (fList.tail()->combineIfPossible(list->head(), arena, caps))
        {
            ...
            case GrOp::CombineResult::kMayChain:
                fList = DoConcat(std::move(fList), std::exchange(*list, List()), caps, arena,
                                 auditTrail);
...
bool GrOpsTask::OpChain::prependChain(OpChain* that, const GrCaps& caps, SkArenaAlloc* arena,
                                      GrAuditTrail* auditTrail) {
...
void GrOpsTask::forwardCombine(const GrCaps& caps) {
    ...
    for (int i = 0; i < fOpChains.count() - 1; ++i) {
        ...
        while (true) {
            ...
            if (candidate.prependChain(&chain, caps, &fArena, fAuditTrail)) {
                break;
            }
