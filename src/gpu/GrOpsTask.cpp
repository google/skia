...
#endif
    // TODO: remove the check for discard here once reduce op splitting is turned on. Currently we
    // can end up with GrOpsTask that only have a discard load op and no ops. For vulkan validation
    // we need to keep that discard and not drop it. Once we have reduce op list splitting enabled
    // we shouldn't end up with GrOpsTasks with only dicard.
