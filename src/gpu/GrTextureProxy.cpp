...
#ifdef SK_DEBUG
    if (kInvalidGpuMemorySize != this->accessRawGpuSize_debugOnly()) {
        SkASSERT(fTarget->gpuMemorySize() <= this->accessRawGpuSize_debugOnly());
    }
#endif
...
size_t GrTextureProxy::onGpuMemorySize() const {
    ...
    return GrTexture::ComputeSize(fDesc, true);
}
