...
public:
    ...
    public:
        ...
        GrGpuBuffer* buffer() { return fBuffer.get(); }
        sk_sp<GrGpuBuffer> refBuffer() { return fBuffer; }

        void setBuffer(sk_sp<GrGpuBuffer> buffer) {
            // TODO: once we add the gpuBuffer we could get rid of the 'fVertices'
            SkASSERT(!fBuffer);
            fBuffer = buffer;
        }
    ...
    static sk_sp<VertexData> Make(const void*, int vertexCount, size_t vertexSize);
...
private:
    ...
    Entry* getEntry(const GrUniqueKey&, sk_sp<VertexData> verts) SK_REQUIRES(fSpinLock);
    ...
    std::tuple<sk_sp<VertexData>, sk_sp<SkData>> GrThreadSafeCache::internalFindVerts(
                                                       const GrUniqueKey&)  SK_REQUIRES(fSpinLock);
