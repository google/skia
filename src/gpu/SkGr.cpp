...
static inline bool skpaint_to_grpaint_impl(GrContext* context,
    ...
    if (SkPaintPriv::ShouldDither(skPaint, ct) && grPaint->numColorFragmentProcessors() > 0 &&
        ...
        int ditherRange = dither_range_type_for_config(colorSpaceInfo.config());
        if (ditherRange >= 0) {
            args.insert(std::make_pair("rangeType",
                SkSL::Program::Settings::Value(ditherRange)));
            auto ditherFP = GrSkSLFP::Make(context, "Dither", SKSL_DITHER_SRC, args);
            if (ditherFP) {
