...
static const SkPixmap* foo(const GrCaps& caps, const SkPixmap& pixmap, 
                           GrSurfaceDesc* desc, SkBitmap* tmpBitmap, SkPixmap* tmpPixmap) {
...
GrTexture* GrUploadPixmapToTexture(GrContext* ctx, const SkPixmap& pixmap, SkBudgeted budgeted) {
    ...
    if (const SkPixmap* pmap = foo(*ctx->caps(), pixmap, &desc, &tmpBitmap, &tmpPixmap)) {
...
sk_sp<GrTextureProxy> GrUploadPixmapToTextureProxy(GrContext* ctx,
                                                   const SkPixmap& pixmap, SkBudgeted budgeted) {
    if (!SkImageInfoIsValid(pixmap.info())) {
        return nullptr;
    }
