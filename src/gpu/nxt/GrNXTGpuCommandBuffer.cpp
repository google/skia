...
GrNXTGpuRTCommandBuffer::GrNXTGpuRTCommandBuffer(GrNXTGpu* gpu,
    ...
    fBuilder.BeginRenderPass(createRenderPassDescriptor());
}
...
nxt::RenderPassDescriptor GrNXTGpuRTCommandBuffer::createRenderPassDescriptor() const {
    ...
    auto renderPassBuilder = fGpu->device().CreateRenderPassDescriptorBuilder()
        .SetColorAttachment(0, colorView, to_nxt_load_op(fColorInfo.fLoadOp))
        .SetColorAttachmentClearColor(0, c[0], c[1], c[2], c[3])
        .Clone();
    if (stencilAttachment) {
        renderPassBuilder
            .SetDepthStencilAttachment(stencilAttachment->view(),
                                       to_nxt_load_op(fStencilInfo.fLoadOp),
                                       to_nxt_load_op(fStencilInfo.fLoadOp));
    }
