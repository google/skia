...
GrNXTGpuTextureCommandBuffer::GrNXTGpuTextureCommandBuffer(GrNXTGpu* gpu,
                                                           GrTexture* texture,
...
void GrNXTGpuTextureCommandBuffer::copy(GrSurface* src, GrSurfaceOrigin srcOrigin,
    ...
    dawn::Texture s = static_cast<GrNXTTexture*>(src->asTexture())->texture();
    dawn::Texture d = static_cast<GrNXTTexture*>(fTexture)->texture();
...
dawn::LoadOp to_nxt_load_op(GrLoadOp loadOp) {
    switch (loadOp) {
        case GrLoadOp::kLoad:
...
GrNXTGpuRTCommandBuffer::GrNXTGpuRTCommandBuffer(GrNXTGpu* gpu,
    ...
    fBuilder = fGpu->device().CreateCommandBufferBuilder();
    dawn::LoadOp colorOp = to_nxt_load_op(colorInfo.fLoadOp);
    dawn::LoadOp stencilOp = to_nxt_load_op(stencilInfo.fLoadOp);
...
dawn::RenderPassDescriptor GrNXTGpuRTCommandBuffer::createRenderPassDescriptor(
        dawn::LoadOp colorOp,
    ...
    dawn::TextureView colorView = texture.CreateTextureViewBuilder().GetResult();
...
void GrNXTGpuRTCommandBuffer::onClearStencilClip(const GrFixedClip& clip, bool insideStencilMask) {
    fBuilder.EndRenderPass();
    fBuilder.BeginRenderPass(createRenderPassDescriptor(dawn::LoadOp::Load, dawn::LoadOp::Clear));
}

void GrNXTGpuRTCommandBuffer::onClear(const GrFixedClip& clip, GrColor color) {
    fBuilder.EndRenderPass();
    fBuilder.BeginRenderPass(createRenderPassDescriptor(dawn::LoadOp::Clear, dawn::LoadOp::Load));
}
...
void GrNXTGpuRTCommandBuffer::copy(GrSurface* src, GrSurfaceOrigin srcOrigin, const SkIRect& srcRect,
    ...
    fBuilder.BeginRenderPass(createRenderPassDescriptor(dawn::LoadOp::Load, dawn::LoadOp::Load));
}
...
void GrNXTGpuRTCommandBuffer::sendInstancedMeshToGpu(GrPrimitiveType,
    ...
    dawn::Buffer vb = static_cast<const GrNXTBuffer*>(vertexBuffer)->get();
...
void GrNXTGpuRTCommandBuffer::sendIndexedInstancedMeshToGpu(GrPrimitiveType,
    ...
    dawn::Buffer vb = static_cast<const GrNXTBuffer*>(vertexBuffer)->get();
    dawn::Buffer ib = static_cast<const GrNXTBuffer*>(indexBuffer)->get();
