...
void GrNXTTexture::upload(const GrMipLevel texels[], int mipLevels, const SkIRect& rect,
    ...
    for (int i = 0; i < mipLevels; i++) {
        if (!fStagingBuffer) {
            size_t totalRowBytes = this->width() * GrBytesPerPixel(this->config());
            totalRowBytes = (totalRowBytes + 0xFF) & ~0xFF;
        ...
        if ((rowBytes & 0xFF) != 0) {
            ...
            fStagingBuffer.SetSubData(0, rowBytes * height,
                static_cast<const uint32_t*>(static_cast<const void*>(buf)));
            delete[] buf;
        } else {
            fStagingBuffer.SetSubData(0, rowBytes * height,
