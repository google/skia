...
#include "nxt/GrNXTCaps.h"
#include "nxt/GrNXTGpu.h"
#include "nxt/GrNXTProgramBuilder.h"
#include "SkSLCompiler.h"

GrNXTUniformHandler::GrNXTUniformHandler(GrGLSLProgramBuilder* program)
    : INHERITED(program)
...
const GrShaderVar& GrNXTUniformHandler::getUniformVariable(UniformHandle u) const {
    return fUniforms[u.toIndex()].fVar;
}

const char* GrNXTUniformHandler::getUniformCStr(UniformHandle u) const {
...
GrGLSLUniformHandler::UniformHandle GrNXTUniformHandler::internalAddUniformArray(
                                                                            uint32_t visibility,
                                                                            GrSLType type,
...
GrGLSLUniformHandler::SamplerHandle GrNXTUniformHandler::addSampler(uint32_t visibility,
                                                                   GrSwizzle swizzle,
                                                                   GrSLType type,
                                                                   GrSLPrecision precision,
                                                                   const char* name) {
    ...
    SkASSERT(type == kTexture2DSampler_GrSLType);
    ...
    info.fVar.setPrecision(precision);
    ...
    texInfo.fVar.setPrecision(precision);
    ...
    texInfo.fVisibility = visibility;
    texInfo.fUBOOffset = 0;

    SkString reference;
    reference.printf("makeSampler2D(%s, %s)", texInfo.fVar.getName().c_str(), info.fVar.getName().c_str());
    fSamplerReferences.push_back() = reference;
    return GrGLSLUniformHandler::SamplerHandle(fSamplers.count() - 1);
}

const char* GrNXTUniformHandler::samplerVariable(GrGLSLUniformHandler::SamplerHandle handle) const {
    return fSamplerReferences[handle.toIndex()].c_str();
}

GrSwizzle GrNXTUniformHandler::samplerSwizzle(GrGLSLUniformHandler::SamplerHandle handle) const {
    return fSamplerSwizzles[handle.toIndex()];
}

GrGLSLUniformHandler::TexelBufferHandle GrNXTUniformHandler::addTexelBuffer(uint32_t visibility,
                                                                           GrSLPrecision precision,
                                                                           const char* name) {
    ...
    return GrGLSLUniformHandler::TexelBufferHandle(0);
}

const GrShaderVar& GrNXTUniformHandler::texelBufferVariable(GrGLSLUniformHandler::TexelBufferHandle handle) const {
    SkASSERT(!"unimplemented");
    return fDummyVar;
}

void GrNXTUniformHandler::appendUniformDecls(GrShaderFlags visibility, SkString* out) const {
