...
void GrNXTProgramDataManager::uploadUniformBuffers(GrNXTGpu* gpu,
                                                   GrNXTStagingBuffer* geometryBuffer,
                                                   GrNXTStagingBuffer* fragmentBuffer,
                                                   nxt::CommandBufferBuilder cmdBuilder) const {

    if (geometryBuffer && fGeometryUniformsDirty) {
        memcpy(geometryBuffer->fData, fGeometryUniformData.get(), fGeometryUniformSize);
        geometryBuffer->fBuffer.Unmap();
        cmdBuilder.TransitionBufferUsage(geometryBuffer->fBuffer, nxt::BufferUsageBit::Uniform);
    }
    if (fragmentBuffer && fFragmentUniformsDirty) {
        memcpy(fragmentBuffer->fData, fFragmentUniformData.get(), fFragmentUniformSize);
        fragmentBuffer->fBuffer.Unmap();
        cmdBuilder.TransitionBufferUsage(fragmentBuffer->fBuffer, nxt::BufferUsageBit::Uniform);
    }
}
