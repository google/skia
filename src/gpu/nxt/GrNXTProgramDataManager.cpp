...
void GrNXTProgramDataManager::uploadUniformBuffers(nxt::Buffer geometryBuffer,
                                                   nxt::Buffer fragmentBuffer) const {
    if (geometryBuffer && fGeometryUniformsDirty) {
        geometryBuffer.TransitionUsage(nxt::BufferUsageBit::TransferDst);
        geometryBuffer.SetSubData(0, fGeometryUniformSize, static_cast<const uint32_t*>(fGeometryUniformData.get()));
        fGeometryUniformsDirty = false;
    }
    if (fragmentBuffer && fFragmentUniformsDirty) {
        fragmentBuffer.TransitionUsage(nxt::BufferUsageBit::TransferDst);
        fragmentBuffer.SetSubData(0, fFragmentUniformSize, static_cast<const uint32_t*>(fFragmentUniformData.get()));
        fFragmentUniformsDirty = false;
    }
}
