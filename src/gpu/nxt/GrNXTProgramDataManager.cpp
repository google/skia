...
void GrNXTProgramDataManager::uploadUniformBuffers(GrNXTRingBuffer::Slice geometryBuffer,
    ...
    if (geom && fGeometryUniformsDirty) {
        geom.TransitionUsage(nxt::BufferUsageBit::TransferDst);
        geom.SetSubData(geometryBuffer.fOffset, fGeometryUniformSize,
                        static_cast<const uint32_t*>(fGeometryUniformData.get()));
    }
    if (frag && fFragmentUniformsDirty) {
        frag.TransitionUsage(nxt::BufferUsageBit::TransferDst);
        frag.SetSubData(fragmentBuffer.fOffset, fFragmentUniformSize,
                        static_cast<const uint32_t*>(fFragmentUniformData.get()));
    }
}
