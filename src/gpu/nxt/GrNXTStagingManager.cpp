...
GrNXTStagingManager::GrNXTStagingManager(dawn::Device device) : fDevice(device.Clone()) {
}
...
GrNXTStagingBuffer* GrNXTStagingManager::findOrCreateStagingBuffer(size_t size,
    ...
    if (i != fReadyPool.end()) {
        ...
        dawn::Buffer buffer = fDevice.CreateBufferBuilder()
            .SetSize(sizePow2)
            .SetAllowedUsage(dawn::BufferUsageBit::MapWrite | usage)
            .GetResult();
