...
static void setTexture(GrNXTGpu* gpu, const GrResourceIOProcessor::TextureSampler& textureSampler, nxt::BindGroupBuilder& builder, int* binding) {
    // FIXME: could probably cache samplers in GrNXTProgram
    nxt::Sampler sampler = gpu->getOrCreateSampler(textureSampler.samplerState());
    builder.SetSamplers((*binding)++, 1, &sampler);
    GrNXTTexture* tex = static_cast<GrNXTTexture*>(textureSampler.peekTexture());
    nxt::TextureView textureView = tex->textureView();
    builder.SetTextureViews((*binding)++, 1, &textureView);
}

static void appendTextureKey(GrNXTProgram::BindGroupKey* key,
                             const GrResourceIOProcessor::TextureSampler& textureSampler) {
    GrNXTTexture* tex = static_cast<GrNXTTexture*>(textureSampler.peekTexture());
...
void GrNXTProgram::buildKey(BindGroupKey* key, const GrPrimitiveProcessor& primProc, const GrPipeline& pipeline) {
    ...
    for (int i = 0; i < primProc.numTextureSamplers(); ++i) {
        appendTextureKey(key, primProc.textureSampler(i));
    }
    ...
    while (fp && glslFP) {
        ...
        for (int i = 0; i < fp->numTextureSamplers(); ++i) {
            appendTextureKey(key, fp->textureSampler(i));
        }
        fp = iter.next();
        glslFP = glslIter.next();
    }
    if (GrTextureProxy* proxy = pipeline.dstTextureProxy()) {
        GrResourceIOProcessor::TextureSampler dstTextureSampler(sk_ref_sp(proxy));
        appendTextureKey(key, dstTextureSampler);
    }
}
...
nxt::BindGroup GrNXTProgram::setData(GrNXTGpu* gpu, const GrPrimitiveProcessor& primProc, const GrPipeline& pipeline, nxt::CommandBufferBuilder cmdBuilder) {
    ...
    if (BindGroupValue* value = fBindGroupCache.find(key)) {
        if (value->fGeometryBuffer) {
            cmdBuilder.TransitionBufferUsage(value->fGeometryBuffer, nxt::BufferUsageBit::Uniform);
        }
        if (value->fFragmentBuffer) {
            cmdBuilder.TransitionBufferUsage(value->fFragmentBuffer, nxt::BufferUsageBit::Uniform);
        }
    ...
    int binding = GrNXTUniformHandler::kSamplerBindingBase;
    for (int i = 0; i < primProc.numTextureSamplers(); ++i) {
        setTexture(gpu, primProc.textureSampler(i), bindGroupBuilder, &binding);
    }
    GrFragmentProcessor::Iter iter(pipeline);
    GrGLSLFragmentProcessor::Iter glslIter(fFragmentProcessors.get(), fFragmentProcessorCnt);
    const GrFragmentProcessor* fp = iter.next();
    ...
    while (fp && glslFP) {
        for (int i = 0; i < fp->numTextureSamplers(); ++i) {
            setTexture(gpu, fp->textureSampler(i), bindGroupBuilder, &binding);
        }
        fp = iter.next();
        glslFP = glslIter.next();
    }
    if (GrTextureProxy* proxy = pipeline.dstTextureProxy()) {
        GrResourceIOProcessor::TextureSampler dstTextureSampler(sk_ref_sp(proxy));
        setTexture(gpu, dstTextureSampler, bindGroupBuilder, &binding);
    }
    fDataManager.uploadUniformBuffers(gpu, geom, frag, cmdBuilder.Clone());
