...
dawn::BlendFactor to_nxt_blend_factor(GrBlendCoeff coeff) {
    switch (coeff) {
...
dawn::BlendFactor to_nxt_blend_factor_for_alpha(GrBlendCoeff coeff) {
    switch (coeff) {
...
dawn::BlendOperation to_nxt_blend_operation(GrBlendEquation equation) {
    switch (equation) {
...
dawn::CompareFunction to_nxt_compare_function(GrStencilTest test) {
    switch (test) {
...
dawn::StencilOperation to_nxt_stencil_operation(GrStencilOp op) {
    switch (op) {
...
static dawn::PrimitiveTopology to_nxt_primitive_topology(GrPrimitiveType primitiveType) {
...
static dawn::VertexFormat to_nxt_vertex_format(GrVertexAttribType type) {
    switch (type) {
...
dawn::BlendState create_blend_state(const GrNXTGpu* gpu, const GrPipeline& pipeline) {
    ...
    dawn::BlendFactor srcFactor = to_nxt_blend_factor(srcCoeff);
    dawn::BlendFactor dstFactor = to_nxt_blend_factor(dstCoeff);
    dawn::BlendFactor srcFactorAlpha = to_nxt_blend_factor_for_alpha(srcCoeff);
    dawn::BlendFactor dstFactorAlpha = to_nxt_blend_factor_for_alpha(dstCoeff);
    dawn::BlendOperation operation = to_nxt_blend_operation(equation);
...
void set_stencil_function(dawn::DepthStencilStateBuilder& builder, dawn::Face nxtFace, const GrStencilSettings::Face& face) {
...
dawn::DepthStencilState create_depth_stencil_state(const GrNXTGpu* gpu, const GrStencilSettings& stencilSettings) {
...
sk_sp<GrNXTProgram> GrNXTProgramBuilder::Build(GrNXTGpu* gpu,
    ...
    if (0 != geometryUniformSize) {
        bindings.push_back({ GrNXTUniformHandler::kGeometryBinding, dawn::ShaderStageBit::Vertex, dawn::BindingType::UniformBuffer});
    }
    if (0 != fragmentUniformSize) {
        bindings.push_back({ GrNXTUniformHandler::kFragBinding, dawn::ShaderStageBit::Fragment, dawn::BindingType::UniformBuffer});
    }
    uint32_t binding = GrNXTUniformHandler::kSamplerBindingBase;
    for (int i = 0; i < builder.fUniformHandler.fSamplers.count(); ++i) {
        bindings.push_back({ binding++, dawn::ShaderStageBit::Fragment, dawn::BindingType::Sampler});
        bindings.push_back({ binding++, dawn::ShaderStageBit::Fragment, dawn::BindingType::SampledTexture});
    }
...
dawn::ShaderModule GrNXTProgramBuilder::CreateShaderModule(dawn::Device device, const GrGLSLShaderBuilder& builder, SkSL::Program::Kind kind, SkSL::Program::Inputs* inputs) {
    SkString source;
...
static void setTexture(GrNXTGpu* gpu, const GrSamplerState& state, GrTexture* texture, dawn::BindGroupBuilder& builder, int* binding) {
...
static void appendTextureSamplerKey(GrNXTProgram::BindGroupKey* key,
                                    GrTexture* texture,
...
dawn::BindGroup GrNXTProgram::setData(GrNXTGpu* gpu, const GrPrimitiveProcessor& primProc, const GrPipeline& pipeline) {
