...
nxt::BindGroup GrNXTProgram::setData(GrNXTGpu* gpu, const GrPrimitiveProcessor& primProc, const GrPipeline& pipeline, nxt::CommandBufferBuilder cmdBuilder) {
    ...
    GrNXTStagingBuffer* geom = nullptr;
    GrNXTStagingBuffer* frag = nullptr;
    ...
    if (0 != geometryUniformSize) {
        geom = gpu->getStagingBuffer(geometryUniformSize, nxt::BufferUsageBit::Uniform);
    ...
    if (0 != fragmentUniformSize) {
        frag = gpu->getStagingBuffer(fragmentUniformSize, nxt::BufferUsageBit::Uniform);
