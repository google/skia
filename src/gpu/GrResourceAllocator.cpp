...
bool GrResourceAllocator::Register::isRecyclable(GrSurfaceProxy* proxy, int useCount) const {
    ...
    if (this->uniqueKey().isValid()) {
        ...
        // If things get really tight, `instantiateSurface` we may pull it back out but there is
