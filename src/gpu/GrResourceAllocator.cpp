...
void GrResourceAllocator::addInterval(GrSurfaceProxy* proxy,
                                      unsigned int start1, unsigned int end) {
    SkASSERT(start1 <= end);
...
public:
    SelectionFilter(bool rejectPendingIO) : fRejectPendingIO(rejectPendingIO) { }

    bool operator()(const GrSurface* s) const {
        return !fRejectPendingIO || !s->surfacePriv().hasPendingIO();
    }
...
sk_sp<GrSurface> GrResourceAllocator::findSurfaceFor(const GrSurfaceProxy* proxy) {
    ...
    sk_sp<GrSurface> surface(fFreePool.findAndRemove(key,
                                            SelectionFilter(proxy->priv().requiresNoPendingIO())));
    if (surface) {
...
void GrResourceAllocator::assign() {
    ...
    for (FreePoolMultiMap::ConstIter iter(&fFreePool); !iter.done(); ++iter) {
        GrSurface* surf = (GrSurface*) *iter;
        surf->unref();
    }
