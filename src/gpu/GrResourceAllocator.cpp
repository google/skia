...
bool GrResourceAllocator::planAssignment() {
    ...
    while (Interval* cur = fIntvlList.popHead()) {
        ...
        // Lazy proxies get instantiated now for the sake of accurate size calculation.
        // For instance, if a non-texturable DDL is played back into a mipped texturable
        // destination, the memory size of the surface will be greater than the proxy's
        // estimate. Besides, lazy proxies are a fixed cost that we'll have to instantiate
        // regardless and won't reuse.
        if (cur->proxy()->isLazy()) {
