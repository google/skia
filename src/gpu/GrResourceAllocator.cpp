...
void GrResourceAllocator::addInterval(GrSurfaceProxy* proxy, unsigned int start, unsigned int end,
    ...
    if (proxy->readOnly()) {
        ...
        SkASSERT(proxy->isInstantiated() || proxy->isLazy());
        return;
    }
...
bool GrResourceAllocator::assign(int* startIndex, int* stopIndex, AssignError* outError) {
    ...
    *stopIndex = fEndOfOpsTaskOpIndices.count();
    ...
    while (Interval* cur = fIntvlList.popHead()) {
        ...
        // We should have handled this in addInterval, and not actually added an interval.
        SkASSERT(!cur->proxy()->isLazy());
        if (sk_sp<GrSurface> surface = this->findSurfaceFor(
