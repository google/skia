...
bool GrResourceAllocator::Register::isRecyclable(GrSurfaceProxy* proxy, int knownUseCount) const {
    ...
    if (this->uniqueKey().isValid()) {
        ...
        // If things get really tight, `instantiateSurface` may pull it back out but there is
    ...
    return !proxy->refCntGreaterThan(useCount);
}
...
void GrResourceAllocator::expire(unsigned int curIndex) {
    while (!fActiveIntvls.empty() && fActiveIntvls.peekHead()->end() < curIndex) {
        ...
        if (r && r->isRecyclable(intvl->proxy(), intvl->uses())) {
