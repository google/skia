...
sk_sp<GrSurface> GrResourceAllocator::findSurfaceFor(const GrSurfaceProxy* proxy) {
    ...
    sk_sp<GrSurface> surface(fFreePool.findAndRemove(key, Baz(proxy->priv().requiresNoPendingIO())));
    if (surface) {
...
void GrResourceAllocator::assign() {
    ...
    for (FreePoolMultiMap::ConstIter iter(&fFreePool); !iter.done(); ++iter) {
        ...
        SkDebugf("---------------- %d refs %d reads %d writes %d\n",
                                             surf->uniqueID().asUInt(),
                                             surf->fRefCnt,
                                             surf->fPendingReads,
                                             surf->fPendingWrites);
