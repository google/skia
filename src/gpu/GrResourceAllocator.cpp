...
// Remove any intervals that end before the current index
void GrResourceAllocator::IntervalList::expire(unsigned int curIndex) {
    while (fHead && fHead->fEnd < curIndex) {
        Interval* temp = fHead;
        fHead = temp->fNext;
...
void GrResourceAllocator::assign() {
    ...
    while (Interval* cur = fIntvlList.popStart()) {
        ...
        active.insertByIncreasingEnd(cur);
    }
}
