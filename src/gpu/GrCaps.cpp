...
void GrCaps::dumpJSON(SkJSONWriter* writer) const {
    ...
    SkASSERT(0 == this->maxSampleCount(kUnknown_GrPixelConfig));
    ...
    for (size_t i = 1; i < kGrPixelConfigCnt; ++i) {
        ...
        writer->appendS32("max sample count", this->maxSampleCount(config));
...
bool GrCaps::validateTextureDesc(const GrSurfaceDesc& desc, GrMipMapped mipped) const {
    ...
    if (SkToBool(desc.fFlags & kRenderTarget_GrSurfaceFlag)) {
        if (0 == this->getSampleCount(desc.fSampleCnt, desc.fConfig)) {
            return false;
        }
