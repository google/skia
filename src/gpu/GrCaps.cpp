...
GrDstSampleFlags GrCaps::getDstSampleFlagsForProxy(const GrRenderTargetProxy* rt,
    ...
    SkASSERT(rt);
    // If the rt has samples we better have set opRequiresMSAA
    SkASSERT((rt->numSamples() <= 1) || opRequiresMSAA);
    if (this->textureBarrierSupport() && (!opRequiresMSAA || this->msaaResolvesAutomatically())) {
