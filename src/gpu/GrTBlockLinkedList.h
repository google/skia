...
#ifndef GrTBlockLinkedList_DEFINED
#define GrTBlockLinkedList_DEFINED

#include "src/gpu/GrBlockAllocator.h"

#include <type_traits>

// Forward declarations for the iterators used by GrTBlockLinkedList
...
/**
 * GrTBlockLinkedList manages dynamic storage for instances of T, reserving fixed blocks such that
 ...
 * invoked when the GrTBlockLinkedList is destructed. The addresses of instances are guaranteed
...
class GrTBlockLinkedList {
public:
    /**
     * Create an allocator that defaults to using StartingItems as heap increment.
     */
    GrTBlockLinkedList() : GrTBlockLinkedList(StartingItems) {}
     ...
     */
    template <int SI>
    void concat(GrTBlockLinkedList<T, SI>&& other);
    ...
    const T& item(int i) const {
        return const_cast<GrTBlockLinkedList*>(this)->item(i);
    }

private:
    // Let other GrTBlockLinkedLists have access (only ever used when T and S are the same but you cannot
    // have partial specializations declared as a friend...)
    template<typename S, int N> friend class GrTBlockLinkedList;
...
template <int SI2>
void GrTBlockLinkedList<T, SI1>::concat(GrTBlockLinkedList<T, SI2>&& other) {
    ...
    for (GrBlockAllocator::Block* block : other.fAllocator->blocks()) {
        ...
        if (blockItemCount <= avail || block == other.fAllocator->headBlock()) {
            if (blockItemCount > avail) {
                ...
                // 'other's heap blocks will be appended after it and any extra space would be waste
                fAllocator->template reserve<alignof(T)>(
                        (blockItemCount - avail) * sizeof(T),
