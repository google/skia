...
sk_sp<GrTexture> GrMockGpu::onCreateTexture(const GrSurfaceDesc& desc,
    ...
    SkASSERT(format.asMockCompressionType() == SkImage::kNone_CompressionType);

    GrColorType ct = format.asMockColorType1();
    SkASSERT(ct != GrColorType::kUnknown);

    GrMipMapsStatus mipMapsStatus =
            mipLevelCount > 1 ? GrMipMapsStatus::kDirty : GrMipMapsStatus::kNotAllocated;
    GrMockTextureInfo texInfo(ct, SkImage::kNone_CompressionType, NextInternalTextureID());
...
sk_sp<GrTexture> GrMockGpu::onCreateCompressedTexture(int width, int height,
    ...
    SkASSERT(compression != SkImage::kNone_CompressionType);
    ...
    GrMockTextureInfo texInfo(GrColorType::kUnknown, format.asMockCompressionType(), NextInternalTextureID());

    return sk_sp<GrTexture>(
        new GrMockTexture(this, budgeted, desc, GrProtected::kNo, mipMapsStatus, texInfo));
}
...
sk_sp<GrTexture> GrMockGpu::onWrapRenderableBackendTexture(const GrBackendTexture& tex,
    ...
    GrMockTextureInfo texInfo;
    SkAssertResult(tex.getMockTextureInfo(&texInfo));
    SkASSERT(texInfo.compressionType() == SkImage::kNone_CompressionType);

    SkASSERT(colorType == texInfo.colorType1());
    GrSurfaceDesc desc;
    desc.fWidth = tex.width();
    desc.fHeight = tex.height();
    desc.fConfig = texInfo.pixelConfig();
    ...
    GrMockRenderTargetInfo rtInfo(texInfo.colorType1(), NextInternalRenderTargetID());
...
sk_sp<GrRenderTarget> GrMockGpu::onWrapBackendRenderTarget(const GrBackendRenderTarget& rt,
    ...
    SkASSERT(colorType == info.colorType2());
    GrSurfaceDesc desc;
...
GrBackendTexture GrMockGpu::onCreateBackendTexture(SkISize dimensions,
    ...
    auto colorType = format.asMockColorType1();
    if (!this->caps()->isFormatTexturable(format)) {
        return GrBackendTexture();  // invalid
    }

    GrMockTextureInfo info(colorType, SkImage::kNone_CompressionType, NextExternalTextureID());
