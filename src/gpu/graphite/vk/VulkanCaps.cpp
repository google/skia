...
std::pair<SkEnumBitMask<TextureUsage>, SkEnumBitMask<SampleCount>> VulkanCaps::getTextureSupport(
    ...
    const SupportedSampleCounts* supportedSampleCounts = nullptr;
    ...
    if (TextureFormatIsDepthOrStencil(format)) {
        ...
        supportedSampleCounts = &formatInfo.fSupportedSampleCounts;
    ...
    if ((featureFlags & renderBits) == renderBits) {
        ...
        if (this->msaaRenderToSingleSampledSupport() &&
            supportedSampleCounts->fSampleCounts > VK_SAMPLE_COUNT_1_BIT) {
    ...
    if (VkFormatNeedsYcbcrSampler(vkFormat)) {
    ...
    // a per-texture basis if that texture ends up being allocated with protected memory.
    ...
    sampleCounts = static_cast<SampleCount>(supportedSampleCounts->fSampleCounts);
