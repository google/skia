...
void GrResourceCache::notifyCntReachedZero(GrGpuResource* resource, uint32_t flags) {
    ...
    resource->cacheAccess().setFrameNumberWhenResourceBecamePurgeable(fFrameNumber);
...
void GrResourceCache::purgeResourceNotUsedInFrames(int n) {
    GrFrameNumber oldFrame = fFrameNumber - n;
    while (fPurgeableQueue.count()) {
        GrFrameNumber resourceFrame =
                fPurgeableQueue.peek()->cacheAccess().frameNumberWhenResourceBecamePurgeable();
        if (resourceFrame > oldFrame) {
