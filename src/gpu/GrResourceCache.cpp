...
void GrResourceCache::notifyCntReachedZero(GrGpuResource* resource, uint32_t flags) {
...

    {
        ...
        if (budgetedType == BudgetedType::kBudgeted) {
            ...
            if (hasUniqueKey && budgetedType == BudgetedType::kUnbudgetedCacheable) {
                return;
            }
    ...
    struct Stats {
        ...
        void update(GrGpuResource* resource) {
            ...
            if (resource->cacheAccess().isScratch()) {
                ...
                SkASSERT(BudgetedType::kBudgeted != resource->resourcePriv().budgetedType() ||
                         uniqueKey.isValid());
