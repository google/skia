...
GrGpuTextureProxyRef::GrGpuTextureProxyRef() {
    fProxy = nullptr;
    fOwnRef = false;
    fPendingIO = false;
}

GrGpuTextureProxyRef::GrGpuTextureProxyRef(sk_sp<GrTextureProxy> proxy, GrIOType ioType) {
    fProxy = nullptr;
    fOwnRef = false;
    fPendingIO = false;
...
void GrGpuTextureProxyRef::reset() {
    SkASSERT(!fPendingIO);
...
void GrGpuTextureProxyRef::setProxy(sk_sp<GrTextureProxy> proxy, GrIOType ioType) {
    SkASSERT(!fPendingIO);
    SkASSERT(SkToBool(fProxy) == fOwnRef);
    SkSafeUnref(fProxy);
    if (!proxy) {
...
void GrGpuTextureProxyRef::markPendingIO() const {
    // This should only be called when the owning GrProgramElement gets its first
    // pendingExecution ref.
    SkASSERT(!fPendingIO);
    SkASSERT(fProxy);
...
void GrGpuTextureProxyRef::pendingIOComplete() const {
    // This should only be called when the owner's pending executions have ocurred but it is still
    // reffed.
    SkASSERT(fOwnRef);
    SkASSERT(fPendingIO);
...
void GrGpuTextureProxyRef::removeRef() const {
    // This should only be called once, when the owners last ref goes away and
    // there is a pending execution.
    SkASSERT(fOwnRef);
    SkASSERT(fPendingIO);
