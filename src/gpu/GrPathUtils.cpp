...
static Sk2f normalize(const Sk2f& n) {
    Sk2f nn = n*n;
...
bool GrPathUtils::chopMonotonicQuads(const SkPoint p[3], SkPoint dst[5]) {
    ...
    // Check if the curve is already monotonic (i.e. sign(tan0 dot v) == sign(tan1 dot v)).
    // This should almost always be this case for well-behaved curves in the real world.
    float result[2];
    Sk2f dot0 = tan0 * v;
    Sk2f dot1 = tan1 * v;
    (dot0 * dot1 + dot0 * SkNx_shuffle<1,0>(dot1)).store(result);
