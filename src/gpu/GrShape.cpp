...
int GrShape::unstyledKeySize() const {
    ...
    switch (fType) {
        ...
        case Type::kArc:
            SkASSERT(!fInheritedKey.count());
            SkASSERT(0 == sizeof(fArcData) % sizeof(uint32_t));
...
void GrShape::attemptToSimplifyRRect() {
    ...
    if (!this->style().hasPathEffect()) {
        ...
        // Possible TODO here: Check whether the dash results in a single arc and convert to arc.
    }
...
void GrShape::attemptToSimplifyArc() {
    ...
    if (fStyle.isSimpleFill() || (!fStyle.pathEffect() &&
            !fArcData.fUseCenter &&
    ...
    // Possible TODO here: Look at whether dash pattern results in a single dash and convert to
    // non-dashed stroke.
}
