...
private:
    GrTriangulator(const SkPath& path, float tolerance, const SkRect& clipBounds, Mode mode)
            : fPath(path)
    ...
    void pathToContours(VertexList* contours);
    ...
    void* polysToTriangles(Poly* polys, void* data);
    ...
    // Additional helpers and drivers.
    void buildEdges(VertexList* contours, int contourCnt, VertexList* mesh, Comparator&);
    Poly* pathToPolys(int contourCnt, VertexList* outerMesh);
