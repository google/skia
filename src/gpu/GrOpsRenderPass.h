...
public:
    ...
    // test is enabled on the current pipeline, then the client must always call setScissorRect()
    // before drawing. The scissor rect may also be updated between draws without having to bind a
    // new pipeline.
    ...
    // followed setScissor() and/or updatePrimProcTextureBindings() if necessary, before using this
    // method.
