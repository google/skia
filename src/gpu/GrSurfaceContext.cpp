...
void GrSurfaceContext::asyncRescaleAndReadPixels(GrDirectContext* dContext,
    ...
    int y = srcRect.fTop;
    if (needsRescale) {
        tempRTC = this->rescale(info, kTopLeft_GrSurfaceOrigin, srcRect, rescaleGamma, sampling);
        if (!tempRTC) {
            callback(callbackContext, nullptr);
            return;
        }
...
bool GrSurfaceContext::rescaleInto(GrSurfaceDrawContext* dst,
    ...
    while (srcRect.size() != finalSize) {
        ...
        if (sampling.useCubic) {
            ...
            // TODO: should we respect sampling.cubic instead of forcing catmullrom?
            ...
            auto filter = sampling == SkSamplingOptions() ? GrSamplerState::Filter::kNearest
                                                          : GrSamplerState::Filter::kLinear;
