...
static std::unique_ptr<GrFragmentProcessor> make_multibilerp_effect(GrRecordingContext* context,
    ...
    for (int j = 0; j < NY; ++j) {
        for (int i = 0; i < NX; ++i) {
            float tx = x0 + i*dx,
                  ty = y0 + j*dy;
            SkMatrix m = SkMatrix::Translate(srcRect.x(), srcRect.y());
            m.preScale(sx, sy);
            m.postTranslate(tx, ty);
            SkRect domain = SkRect::Make(srcRect).makeInset(sx, sy).makeOffset(tx, ty);
    ...
    while (srcRect.size() != info.dimensions()) {
        ...
        switch (stepType) {
            ...
            case StepType::kFourTapBilinear: {
                auto effect = make_multibilerp_effect<2, 2>(fContext, texView, srcAlphaType,
                                                            srcRect, nextDims);
                effect = GrColorSpaceXformEffect::Make(std::move(effect), std::move(xform));
                GrPaint paint;
                paint.addColorFragmentProcessor(std::move(effect));
                paint.setPorterDuffXPFactory(SkBlendMode::kSrc);
