...
void GrSurfaceContext::asyncRescaleAndReadPixels(GrDirectContext* dContext,
    ...
    int y = srcRect.fTop;
    if (needsRescale) {
        tempRTC = this->rescale(info, kTopLeft_GrSurfaceOrigin, srcRect, rescaleGamma, rescaling);
        if (!tempRTC) {
            callback(callbackContext, nullptr);
            return;
        }
...
bool GrSurfaceContext::rescaleInto(GrSurfaceDrawContext* dst,
    ...
    while (srcRect.size() != finalSize) {
        ...
        if (rescaling != SkRescalingMode::kNearest) {
        ...
        if (rescaling == SkRescalingMode::kRepeatedCubic) {
            SkMatrix matrix;
            ...
            auto filter = rescaling == SkRescalingMode::kNearest ? GrSamplerState::Filter::kNearest
                                                                 : GrSamplerState::Filter::kLinear;
