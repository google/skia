...
std::unique_ptr<GrRenderTargetContext> GrSurfaceContext::rescale(
    ...
    if (rescaleGamma == SkSurface::kLinear && this->colorInfo().colorSpace() &&
        ...
        linearRTC->drawTexture(GrNoClip(), texProxy, srcAlphaType, GrSamplerState::Filter::kNearest,
    ...
    while (stepsX || stepsY) {
        ...
        if (rescaleQuality == kHigh_SkFilterQuality) {
            ...
            tempB->drawTexture(GrNoClip(), texProxy, srcAlphaType, filter, SkBlendMode::kSrc,
