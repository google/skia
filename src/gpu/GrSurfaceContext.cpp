...
bool GrSurfaceContext::readPixels(const GrPixelInfo& origDstInfo, void* dst, size_t rowBytes,
    ...
    if (!dstInfo.clip(this->width(), this->height(), &pt.fX, &pt.fY, &dst, rowBytes)) {
        return false;
    }
...
bool GrSurfaceContext::writePixels(const GrPixelInfo& origSrcInfo, const void* src, size_t rowBytes,
    ...
    if (!srcInfo.clip(this->width(), this->height(), &pt.fX, &pt.fY, &src, rowBytes)) {
        return false;
    }
    ...
    if (convert) {
        ...
        rowBytes = tmpRB;
        src = tmpPixels.get();
        pt.fY = flip ? pt.fY = dstSurface->height() - pt.fY - tmpInfo.height() : pt.fY;
    }
