...
sk_sp<GrRenderTargetContext> GrSurfaceContext::rescale(const SkImageInfo& info,
    ...
    if (!texProxy) {
        texProxy = GrSurfaceProxy::Copy(fContext, this->asSurfaceProxy(), GrMipMapped::kNo,
    ...
    if (rescaleQuality > kNone_SkFilterQuality) {
        stepsX = static_cast<int>((sx > 1.f) ? ceil(log2f(sx))
                                             : floor(log2f(sx)));
        stepsY = static_cast<int>((sy > 1.f) ? ceil(log2f(sy))
                                             : floor(log2f(sy)));
    } else {
        stepsX = sx != 1.f;
        stepsY = sy != 1.f;
    ...
    while (stepsX || stepsY) {
        ...
        if (!stepsX && !stepsY) {
            ...
            xform = GrColorSpaceXform::Make(currCtx->colorSpaceInfo().colorSpace(),
                                            currCtx->colorSpaceInfo().alphaType(),
