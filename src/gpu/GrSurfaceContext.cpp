    ...
    if (readFlag == GrCaps::SurfaceReadPixelsSupport::kCopyToTexture2D || canvas2DFastPath) {
        ...
        if (canvas2DFastPath) {
            fp = direct->priv().createPMToUPMEffect(
                    GrSimpleTextureEffect::Make(sk_ref_sp(srcProxy->asTextureProxy()), colorType,
                                                SkMatrix::I()));
            ...
            fp = GrSimpleTextureEffect::Make(sk_ref_sp(srcProxy->asTextureProxy()), colorType,
                                             SkMatrix::I());
        }
        if (!fp) {
            return false;
...
std::unique_ptr<GrRenderTargetContext> GrSurfaceContext::rescale(
    ...
    if (!texProxy) {
        texProxy = GrSurfaceProxy::Copy(fContext, this->asSurfaceProxy(),
                                        this->colorInfo().colorType(), GrMipMapped::kNo, srcRect,
