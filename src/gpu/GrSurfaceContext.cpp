    ...
    if (readFlag == GrCaps::SurfaceReadPixelsSupport::kCopyToTexture2D || canvas2DFastPath) {
        ...
        if (this->asTextureProxy()) {
            ...
            if (canvas2DFastPath) {
                ...
                // The render target context is incorrectly tagged as kPremul even though we're writing unpremul data thanks to the PMToUPM effect. Fake out the dst alpha type so we don't double unpremul.
            ...
            SkBackingFit fit = this->origin() == kTopLeft_GrSurfaceOrigin ? SkBackingFit::kApprox : SkBackingFit::kExact;
            if (restrictions.fMustCopyWholeSrc) {
                copy = GrSurfaceProxy::Copy(fContext, srcProxy, this->origin(), GrMipMapped::kNo, fit, SkBudgeted::kYes);
            } else {
                copy = GrSurfaceProxy::Copy(fContext, srcProxy, this->origin(), GrMipMapped::kNo, SkIRect::MakeXYWH(pt.fX, pt.fY, dstInfo.width(), dstInfo.height()), fit, SkBudgeted::kYes, restrictions.fRectsMustMatch);
                pt = {0, 0};
            }
            if (!copy) {
                return false;
            ...
            tempCtx = GrSurfaceContext::Make(direct, std::move(view), this->colorInfo().colorType(), this->colorInfo().alphaType(), this->colorInfo().refColorSpace());
            SkASSERT(tempCtx);
        }
        return tempCtx->readPixels(dstInfo, dst, rowBytes, pt, direct);
    }
