...
#ifdef SK_DEBUG
    // We should have copied the surface flags directly from 'surf'.
    SkASSERT(fSurfaceFlags == fTarget->surfacePriv().flags());
    // Now assign them the way we would have expected and make sure they still agree.
    this->initSurfaceFlags(*fTarget->getContext()->priv().caps());
    SkASSERT(fSurfaceFlags == fTarget->surfacePriv().flags());
#endif
}
