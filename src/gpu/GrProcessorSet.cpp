...
GrProcessorSet::GrProcessorSet(GrPaint&& paint) {
    ...
    fFlags = 0;
    if (fColorFragmentProcessorCnt <= kMaxColorProcessors) {
        int i = 0;
...
void GrProcessorSet::FragmentProcessorAnalysis::init(const GrPipelineInput& colorInput,
    ...
    this->internalInit(colorInput, coverageInput, processors, usesPLSDstRead, clipFP, caps);
...
GrProcessorSet::FragmentProcessorAnalysis::FragmentProcessorAnalysis(
    ...
    this->internalInit(colorInput, coverageInput, GrProcessorSet(GrPaint()), false, nullptr, caps);
}
