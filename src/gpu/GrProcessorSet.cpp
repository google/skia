...
bool GrProcessorSet::operator==(const GrProcessorSet& that) const {
    ...
    if (this->xferProcessor() || that.xferProcessor()) {
        const GrXferProcessor& thisXP = this->xferProcessor() ? *this->xferProcessor()
                                                        : GrPorterDuffXPFactory::SimpleSrcOverXP();
        const GrXferProcessor& thatXP = that.xferProcessor() ? *that.xferProcessor()
                                                        : GrPorterDuffXPFactory::SimpleSrcOverXP();
...
GrProcessorSet::Analysis GrProcessorSet::finalize(const GrProcessorAnalysisColor& colorInput,
    ...
    auto xp = GrXPFactory::MakeXferProcessor(this->xpFactory(), outputColor, outputCoverage,
                                             isMixedSamples, caps);
