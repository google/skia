...
GrProcessorSet::GrProcessorSet(GrPaint&& paint) : fXP(paint.getXPFactory()) {
    fFlags = 0;
    fHeadColorFragmentProcessor = std::move(paint.detachColorFragmentProcessors());
    fHeadCoverageFragmentProcessor = std::move(paint.detachCoverageFragmentProcessors());
}
