...
void GrProcessorSet::FragmentProcessorAnalysis::internalInit(
    ...
    GrXPFactory::AnalysisProperties props = GrXPFactory::GetAnalysisProperties(processors.fXPFactory, colorInfo.outputColor(), outputCoverage, caps);
    if (!processors.numCoverageFragmentProcessors() && GrPipelineAnalysisCoverage::kNone == coverageInput) {
        fCanCombineOverlappedStencilAndCover = SkToBool(props & GrXPFactory::AnalysisProperties::kCanCombineOverlappedStencilAndCover);
    } else {
    ...
    fCompatibleWithCoverageAsAlpha &= SkToBool(props & GrXPFactory::AnalysisProperties::kCompatibleWithAlphaAsCoverage);
    if (props & GrXPFactory::AnalysisProperties::kIgnoresInputColor) {
        ...
        fInputColor = GrColor_TRANSPARENT_BLACK; // Don't care what value this is.
