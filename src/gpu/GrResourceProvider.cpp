...
sk_sp<GrTexture> GrResourceProvider::getExactScratch(const GrSurfaceDesc& desc,
                                                     ...
                                                     GrProtected isProtected,
                                                     Flags flags) {
    sk_sp<GrTexture> tex(this->refScratchTexture(desc, renderable, renderTargetSampleCnt,
            isProtected, flags));
...
sk_sp<GrTexture> GrResourceProvider::createTexture(const GrSurfaceDesc& desc,
    ...
    sk_sp<GrTexture> tex =
            (SkBackingFit::kApprox == fit)
                    ...
                    : this->createTexture(desc, renderable, renderTargetSampleCnt, budgeted, isProtected, flags);
    if (!tex) {
        return nullptr;
    }
...
sk_sp<GrTexture> GrResourceProvider::createTexture(const GrSurfaceDesc& desc,
    ...
    if (!GrPixelConfigIsCompressed(desc.fConfig)) {
        sk_sp<GrTexture> tex =
                this->getExactScratch(desc, renderable, renderTargetSampleCnt, budgeted, isProtected, flags);
        if (tex) {
            return tex;
        }
    }
    ...
    if (fCaps->createTextureMustSpecifyAllLevels()) {
        ...
        return fGpu->createTexture(desc, renderable, renderTargetSampleCnt, budgeted, isProtected, &level, 1);
    }
...
sk_sp<GrTexture> GrResourceProvider::createApproxTexture(const GrSurfaceDesc& desc,
    ...
    if (auto tex = this->refScratchTexture(desc, renderable, renderTargetSampleCnt, isProtected, flags)) {
        return tex;
    }
    ...
    if (auto tex = this->refScratchTexture(*copyDesc, renderable, renderTargetSampleCnt, isProtected, flags)) {
        return tex;
    }
    ...
    if (this->caps()->createTextureMustSpecifyAllLevels()) {
        ...
        level.fRowBytes = rowBytes;
        level.fPixels = zeros.get();
        return fGpu->createTexture(*copyDesc, renderable, renderTargetSampleCnt, SkBudgeted::kYes, isProtected, &level, 1);
    }
    return fGpu->createTexture(*copyDesc, renderable, renderTargetSampleCnt, SkBudgeted::kYes, isProtected);
}
