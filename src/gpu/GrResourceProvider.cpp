...
#else
        sk_sp<GrTextureProxy> proxy = proxyProvider->createFoo(desc, SkBackingFit::kApprox,
                                                               budgeted, 0);
#endif
        if (proxy) {
...
sk_sp<GrTexture> GrResourceProvider::wrapBackendTexture1(const GrBackendTexture& tex,
