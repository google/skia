...
sk_sp<GrTexture> GrResourceProvider::createTexture(const GrSurfaceDesc& desc,
    ...
    if (scratch) {
        ...
        return this->writePixels(std::move(scratch), colorType, {desc.fWidth, desc.fHeight}, texels, mipLevelCount);
    }
    ...
    if (mipLevelCount) {
        tempColorType = this->prepareLevels(format, colorType, {desc.fWidth, desc.fHeight}, texels, mipLevelCount,
                                            &tmpTexels, &tmpDatas);
...
static bool prepare_level(const GrMipLevel& inLevel, const SkISize& size, bool rowBytesSupport,
...
GrColorType GrResourceProvider::prepareLevels(const GrBackendFormat& format, GrColorType colorType,
...
sk_sp<GrTexture> GrResourceProvider::writePixels(sk_sp<GrTexture> texture, GrColorType colorType, const SkISize& baseSize, const GrMipLevel texels[], int mipLevelCount) const {
    SkASSERT(!this->isAbandoned());
    SkASSERT(texture);
    ...
    SkAutoSTMalloc<14, GrMipLevel> tmpTexels;
    SkAutoSTArray<14, std::unique_ptr<char[]>> tmpDatas;
    auto tempColorType = this->prepareLevels(texture->backendFormat(), colorType, baseSize,
            texels, mipLevelCount, &tmpTexels, &tmpDatas);
    if (tempColorType == GrColorType::kUnknown) {
        return nullptr;
    }
