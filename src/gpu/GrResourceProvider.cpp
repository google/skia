...
sk_sp<GrTexture> GrResourceProvider::createTexture(const GrSurfaceDesc& desc,
                                                   GrRenderable renderable, SkBudgeted budgeted, GrProtected isProtected,
    ...
    return fGpu->createTexture(desc, renderable, budgeted, isProtected, tmpTexels.get(), mipLevelCount);
}
...
sk_sp<GrTexture> GrResourceProvider::createTexture(const GrSurfaceDesc& desc,
    ...
    if (!GrPixelConfigIsCompressed(desc.fConfig)) {
        sk_sp<GrTexture> tex = this->getExactScratch(desc, renderable, budgeted, isProtected, flags);
        if (tex) {
            return tex;
        }
    }
...
sk_sp<GrTexture> GrResourceProvider::createApproxTexture(const GrSurfaceDesc& desc,
                                                         GrRenderable renderable, GrProtected isProtected, Flags flags) {
    ASSERT_SINGLE_OWNER
