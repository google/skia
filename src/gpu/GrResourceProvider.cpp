...
sk_sp<GrGpuResource> GrResourceProvider::findAndRefResourceByUniqueKey(const GrUniqueKey& key) {
    ASSERT_SINGLE_OWNER
...
bool GrResourceProvider::attachStencilAttachment(GrRenderTarget* rt) {
    ...
    if (!rt->wasDestroyed() && rt->canAttemptStencilAttachment()) {
        ...
        auto stencil = this->findAndRefTByUniqueKey<GrStencilAttachment>(sbKey);
        if (!stencil) {
