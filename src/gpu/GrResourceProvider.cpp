...
sk_sp<GrTexture> GrResourceProvider::createTexture(const GrSurfaceDesc& desc, SkBudgeted budgeted,
    ...
    bool mustInitializeAllLevels = this->caps()->shouldInitializeTextures() &&
...
sk_sp<GrTexture> GrResourceProvider::createTexture(const GrSurfaceDesc& desc,
    ...
    bool mustInitialize = this->caps()->shouldInitializeTextures() &&
                          !this->caps()->canClearTextureOnCreation();
    ...
    if (!prepare_level(mipLevel, bpp, desc.fWidth, desc.fHeight,
                       this->caps()->writePixelsRowBytesSupport(), mustInitialize,
                       &tmpLevel, &tmpData)) {
        return nullptr;
    }
...
sk_sp<GrTexture> GrResourceProvider::createApproxTexture(const GrSurfaceDesc& desc,
    ...
    if (this->caps()->shouldInitializeTextures() && !this->caps()->canClearTextureOnCreation()) {
