...
public:
    ...
    GrDrawOpAtlasConfig() : GrDrawOpAtlasConfig(kMaxEffectiveTextureDim, 0) {}
...
private:
    ...
    // We store texture coordinates for the atlas in a 16-bit int, and we steal a bit to
    // pack the texture index for multitexturing, so our effective largest size is 8096
    static constexpr int kMaxEffectiveTextureDim = 8096;
