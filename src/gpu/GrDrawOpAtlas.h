...
private:
    ...
    private:
        Plot(int index, int pageIndex, uint64_t genID, int offX, int offY, int width, int height,
         ...
         */
        Plot* clone() const {
            return new Plot(fIndex, fPageIndex, fGenID + 1, fX, fY, fWidth, fHeight, fConfig);
        }

        static GrDrawOpAtlas::AtlasID CreateId(uint32_t plotIdx, uint32_t pageIdx,
                                               uint64_t generation) {
            SkASSERT(plotIdx < (1 << 8));
            SkASSERT(pageIdx < (1 << 8));
            SkASSERT(pageIdx == 0); // for now, we only support one page
            SkASSERT(generation < ((uint64_t)1 << 48));
            return generation << 16 | pageIdx << 8 | plotIdx;
        }
