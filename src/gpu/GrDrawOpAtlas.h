...
public:
    ...
    bool hasID(PlotLocator plotLocator) {
        if (kInvalidPlotPath == plotLocator) {
            return false;
        }
...
private:
    ...
    public:
        ...
        GrDrawOpAtlas::PlotLocator plotLocator() const {
            SkASSERT(GrDrawOpAtlas::kInvalidPlotPath != fPlotLocator);
            return fPlotLocator;
        }
    ...
    private:
        ...
        static GrDrawOpAtlas::PlotLocator CreateId(uint32_t pageIdx, uint32_t plotIdx,
                                                   uint64_t generation) {
