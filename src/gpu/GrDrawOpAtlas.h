...
public:
    ...
    class AtlasLocator {
    public:
        GrURect16 uvs(bool distanceField) const;
    ...
    bool hasID(const AtlasLocator& atlasLocator) {
        ...
        uint32_t plot = GetPlotIndexFromID(atlasLocator.fPlotLocator); // 8 bits
        uint32_t page = GetPageIndexFromID(atlasLocator.fPlotLocator); // 8 bits
        uint64_t plotGeneration = fPages[page].fPlotArray[plot]->genID();
        uint64_t locatorGeneration = GetGenerationFromID(atlasLocator.fPlotLocator); // 48 bits
