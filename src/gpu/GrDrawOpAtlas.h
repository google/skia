 ...
 * for using GrDrawOpUploadToken to detect data hazards. Op's uploads are perfomed in "asap" mode
 ...
 * draw uses enough subimage space to overflow the atlas texture addToAtlast fails, giving the op
 * the chance to end the draw and begin a new one. Additional uploads will succeed in inline mode.
 */
class GrDrawOpAtlas {
public:
     ...
     * coordinates in the backing texture. nullptr is returned if the subimage cannot fit in the
     * atlas without overwriting texels that have pending reads on the GPU. This indicates that the
     * op should end its current draw and begin another before adding more data. Upon success,
     * an upload of the provided image data will have been added to the GrDrawOp::Target.
     * NOTE: When the GrDrawOp prepares a draw that reads from the atlas, it must should immediately
     * call 'setUseToken' with the currentToken from the GrDrawOp::Target, otherwise the next call
     * to addToAtlas might cause the previous data to be overwritten before it has been read.
     */
    ...
    /*
     * A class which can be handed back to GrDrawOpAtlas for updating last use tokensin bulk.  The
