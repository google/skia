...
/**
 * This class manages an atlas texture on behalf of GrDrawOps. The draw ops that use the texture
 * perform texture uploads when preparing their draws during flush. The class provides facilities
 * for using GrDrawOpUploadToken to detect data hazards. Ops upload data in "asap" mode until a
 * hazard is detected at which point they switch to "inline" uploads that occur between draws of the
 * flush.
 */
class GrDrawOpAtlas {
public:
    ...
    // with the currentToken from the GrDrawOp::Target, otherwise the next call to addToAtlas might
    // cause an eviction.
