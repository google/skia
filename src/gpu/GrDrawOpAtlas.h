 ...
 * for using GrDrawOpUploadToken to detect data hazards. Ops uploads are perfomed in "asap" mode
 * until it is impossible to add data without overwriting draws that have not yet executed. At that
 * point the uploads are performed "inline" between draws. If a single draw would exceed the atlas,
 * the atlas fails giving the op the chance to end the draw and begin a new one, at which point
 * uploads will succeed in inline mode.
 */
class GrDrawOpAtlas {
public:
    ...
    /**
     * Adds a width x height subimage to the atlas. Upon success it returns an ID and the data's
     * coordinates in the backing texture. nullptr is returned if the subimage cannot fit in the
     * atlas. This indicates that the op should end its current draw and begin another before adding
     * more data. Upon succeess, an upload of the provided image data will have been added to the
     * GrDrawOp::Target.
     * NOTE: When the GrDrawOp reads from data in the atlas, it must should immediately call
    ...
    /*
     * A class which can be handed back to GrDrawOpAtlas for updating in bulk last use tokens.  The
