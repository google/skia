...
private:
    DefaultGeoProc(uint32_t gpTypeFlags,
        ...
        if (fFlags & kBonesAttribute_GPFlag) {
            ...
            fInBoneIndices = {"inBoneIndices", kUByte4_GrVertexAttribType};
            ++cnt;
...
sk_sp<GrGeometryProcessor> DefaultGeoProc::TestCreate(GrProcessorTestData* d) {
    ...
    bones.reserve(numBones * kNumFloatsPerSkMatrix);
    for (int i = 0; i < numBones * kNumFloatsPerSkMatrix; i++) {
        bones.push_back(d->fRandom->nextF());
    }
