...
private:
    DefaultGeoProc(uint32_t gpTypeFlags,
        ...
        if (fFlags & kLocalCoordAttribute_GPFlag) {
            fInLocalCoords = {"inLocalCoord", kFloat2_GrVertexAttribType, offset};
            offset += fInLocalCoords.size();
            ++cnt;
        }
        if (fFlags & kCoverageAttribute_GPFlag) {
            fInCoverage = {"inCoverage", kHalf_GrVertexAttribType, offset};
            offset += fInCoverage.size();
            ++cnt;
        }
