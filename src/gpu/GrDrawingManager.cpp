...
void GrDrawingManager::reorderTasks() {
    ...
    auto newEnd = std::stable_partition(fDAG.begin(),
                                        fDAG.end(),
                                        [&](const sk_sp<GrRenderTask>& task) {
        if (countForRemoval > 0) {
            task->disown(this);
            countForRemoval--;
            return false;
        } else if (auto opsTask = task->asOpsTask()) {
            SkSpan<const sk_sp<GrRenderTask>> taskSpan(fDAG);
            countForRemoval = opsTask->mergeFrom(taskSpan.last(fDAG.end() - &task - 1));
        }
        return true;
    });
    fDAG.resize_back(newEnd - fDAG.begin());
}
