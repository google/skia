...
void GrDrawingManager::moveRenderTasksToDDL(SkDeferredDisplayList* ddl) {
    ...
    fDAG.swap(&ddl->fRenderTasks1);
    SkASSERT(!fDAG.numRenderTasks());

    for (auto& renderTask : ddl->fRenderTasks1) {
...
void GrDrawingManager::createDDLTask(sk_sp<const SkDeferredDisplayList> ddl,
    ...
    GrRenderTask* ddlTask = fDAG.add(sk_make_sp<GrDDLTask>(this, sk_ref_sp(newDest), std::move(ddl)));
