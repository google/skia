...
void GrDrawingManager::newWaitRenderTask(sk_sp<GrSurfaceProxy> proxy,
    ...
    if (fReduceOpsTaskSplitting) {
        ...
        if (fActiveOpsTask && (fActiveOpsTask->fTarget == proxy)) {
            ...
            // on the proxy, add the dependency, and then reset the lastRenderTask to fActiveOpsTask.
            // Additionally we make the waitDesk depend on all of fActiveOpsTask dependencies so
            // that we don't unnecessarily reorder the waitTask before them.
            ...
            // dependency isn't strictly needed but it goes keep the DAG from reordering the
