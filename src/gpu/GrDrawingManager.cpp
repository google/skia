...
void GrDrawingManager::createDDLTask(sk_sp<const SkDeferredDisplayList> ddl,
    ...
    if (auto activeOpsTask = this->getActiveOpsTask()) {
        // Tasks before this DDL can't combine with tasks within the DDL.
...
void GrDrawingManager::validate() const {
    if (auto activeOpsTask = this->getActiveOpsTask()) {
        // The resolveTask associated with the active ops task remains open for as long as
        // the task does.
        if (auto resolveTask = activeOpsTask->fTextureResolveTask) {
            SkASSERT(!resolveTask->isClosed());
        }
    }
}
#endif
