...
sk_sp<GrOpsTask> GrDrawingManager::newOpsTask(GrSurfaceProxyView surfaceView,
    ...
    GrSurfaceProxy* proxy = surfaceView.asSurfaceProxy();
...
void GrDrawingManager::newWaitRenderTask(sk_sp<GrSurfaceProxy> proxy,
    ...
    if (fReduceOpsTaskSplitting) {
        ...
        if (fActiveOpsTask && (fActiveOpsTask->fTargetView.asSurfaceProxy() == proxy.get())) {
...
bool GrDrawingManager::newCopyRenderTask(GrSurfaceProxyView srcView,
    ...
    this->closeRenderTasksForNewRenderTask(dstView.asSurfaceProxy());
    const GrCaps& caps = *fContext->priv().caps();

    GrSurfaceProxy* srcProxy = srcView.asSurfaceProxy();
