...
std::unique_ptr<SkDeferredDisplayList> GrDrawingManager::detachDDL(const SkSurfaceCharacterization& c,
    ...
    return std::unique_ptr<SkDeferredDisplayList>(new SkDeferredDisplayList(c, std::move(fOpLists), origDest));
}

bool GrDrawingManager::insertDDL(const SkDeferredDisplayList* ddl, GrRenderTargetProxy* newDest) {
    ddl->remap(newDest);
