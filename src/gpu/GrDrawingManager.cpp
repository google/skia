...
GrDrawingManager::~GrDrawingManager() {
    this->closeAllTasks();
    this->removeRenderTasks(0, fTasks.count());
}
...
bool GrDrawingManager::flush(
    ...
    if (!info.fNumSemaphores && !info.fFinishedProc &&
        ...
        for (int i = 0; i < numProxies && canSkip; ++i) {
            bool used = std::any_of(fTasks.begin(), fTasks.end(), [&](auto& task) {
                return task && task->isUsed(proxies[i]);
            });
            canSkip = !used && !this->isDDLTarget(proxies[i]);
        }
        if (canSkip) {
            if (info.fSubmittedProc) {
    ...
    if (!fOnFlushCBObjects.empty()) {
        fFlushingRenderTaskIDs.reset(fTasks.count());
        for (int i = 0; i < fTasks.count(); i++) {
            if (fTasks[i]) {
                fFlushingRenderTaskIDs[i] = fTasks[i]->uniqueID();
            }
        }
    ...
    {
        GrResourceAllocator alloc(resourceProvider SkDEBUGCODE(, fTasks.count()));
        for (int i = 0; i < fTasks.count(); ++i) {
            if (fTasks[i]) {
                fTasks[i]->gatherProxyIntervals(&alloc);
            }
        ...
        while (alloc.assign(&startIndex, &stopIndex, &error)) {
            if (GrResourceAllocator::AssignError::kFailedProxyInstantiation == error) {
                for (int i = startIndex; i < stopIndex; ++i) {
                    GrRenderTask* renderTask = fTasks[i].get();
                    if (!renderTask) {
                        continue;
                    }
...
bool GrDrawingManager::executeRenderTasks(int startIndex, int stopIndex, GrOpFlushState* flushState,
    ...
    SkASSERT(startIndex <= stopIndex && stopIndex <= fTasks.count());

#if GR_FLUSH_TIME_OP_SPEW
    SkDebugf("Flushing opsTask: %d to %d out of [%d, %d]\n",
                            startIndex, stopIndex, 0, fTasks.count());
    for (const auto& task : fTasks) {
        if (task) {
            task->dump(true);
        }
    ...
    for (const auto& renderTask : fTasks) {
        if (!renderTask || !renderTask->isInstantiated()) {
            continue;
        }
...
void GrDrawingManager::removeRenderTasks(int startIndex, int stopIndex) {
    for (int i = startIndex; i < stopIndex; ++i) {
        GrRenderTask* task = fTasks[i].get();
        if (!task) {
            continue;
        }
        if (!task->unique()) {
...
void GrDrawingManager::sortTasks() {
    if (!SkTTopoSort<GrRenderTask, GrRenderTask::TopoSortTraits>(&fTasks)) {
    ...
    if (!fTasks.empty()) {
        GrOpsTask* prevOpsTask = fTasks[0]->asOpsTask();
        for (int i = 1; i < fTasks.count(); ++i) {
            GrOpsTask* curOpsTask = fTasks[i]->asOpsTask();
...
GrRenderTask* GrDrawingManager::insertTaskBeforeLast(sk_sp<GrRenderTask> task) {
    SkASSERT(!fTasks.empty());
    SkASSERT(task && task->unique());
    // Release 'fTasks.back()' and grab the raw pointer, in case the SkTArray grows
    // and reallocates during emplace_back.
    fTasks.emplace_back(fTasks.back().release());
    return (fTasks[fTasks.count() - 2] = std::move(task)).get();
}

GrRenderTask* GrDrawingManager::appendTask(sk_sp<GrRenderTask> task) {
    SkASSERT(task && task->unique());
    return fTasks.push_back(std::move(task)).get();
}

GrRenderTask* GrDrawingManager::appendTasks(SkSpan<const sk_sp<GrRenderTask>> tasks) {
#ifdef SK_DEBUG
    for (const auto& task : tasks) {
        SkASSERT(task && task->unique());
    }
#endif
    fTasks.push_back_n(tasks.count(), tasks.begin())->get();
    return fTasks.back().get();
}
...
void GrDrawingManager::copyRenderTasksFromDDL(sk_sp<const SkDeferredDisplayList> ddl,
    ...
    this->appendTask(sk_make_sp<GrUnrefDDLTask>(std::move(ddl)));
    fTasks.back()->makeClosed(*fContext->priv().caps());
...
void GrDrawingManager::validate() const {
    if (fReduceOpsTaskSplitting) {
        ...
        if (fActiveOpsTask) {
            ...
            SkASSERT(fActiveOpsTask == fTasks.back().get());
        }

        for (int i = 0; i < fTasks.count(); ++i) {
            if (fActiveOpsTask != fTasks[i].get()) {
                // The resolveTask associated with the activeTask remains open for as long as the
                // activeTask does.
                bool isActiveResolveTask =
                    fActiveOpsTask && fActiveOpsTask->fTextureResolveTask == fTasks[i].get();
                SkASSERT(isActiveResolveTask || fTasks[i]->isClosed());
            }
        }

        if (!fTasks.empty() && !fTasks.back()->isClosed()) {
            SkASSERT(fActiveOpsTask == fTasks.back().get());
        }
    }
}
...
void GrDrawingManager::newTransferFromRenderTask(sk_sp<GrSurfaceProxy> srcProxy,
    ...
    fTasks.push_back(sk_make_sp<GrTransferFromRenderTask>(
            srcProxy, srcRect, surfaceColorType, dstColorType,
            std::move(dstBuffer), dstOffset));
    GrRenderTask* task = fTasks.back().get();
...
bool GrDrawingManager::newCopyRenderTask(GrSurfaceProxyView srcView,
    ...
    fTasks.push_back(GrCopyRenderTask::Make(this, std::move(srcView), srcRect, std::move(dstView),
                                            dstPoint, &caps));
    GrRenderTask* task = fTasks.back().get();
    if (!task) {
        return false;
