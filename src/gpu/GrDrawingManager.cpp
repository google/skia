...
// Copy unskipped tasks from llist to array. Unref and discard skipped tasks.
// Don't ref/unref unskipped tasks.
...
static void copy_unskipped_tasks(const SkTInternalLList<GrRenderTask>& llist,
    ...
    for (GrRenderTask* t : llist) {
        ...
        // Skipped tasks will get unreffed when we resize_back at the end.
...
bool GrDrawingManager::reorderTasks(GrResourceAllocator* resourceAllocator) {
    ...
    // At this point we're committed to the reordered version. Merge.
    for (GrRenderTask* task : llist) {
        if (GrOpsTask* opsTask = task->asOpsTask()) {
    ...
    copy_unskipped_tasks(llist, &fDAG, [this](GrRenderTask* task) {
        task->disown(this);
    });
    return true;
}
