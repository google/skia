...
void GrDrawingManager::RenderTaskDAG::removeRenderTask(int index) {
    if (!fRenderTasks[index]->unique()) {
        ...
        if (GrOpList* opList = fRenderTasks[index]->asOpList()) {
            opList->endFlush();
        }
    }
...
void GrDrawingManager::RenderTaskDAG::cleanup(const GrCaps* caps) {
    for (int i = 0; i < fRenderTasks.count(); ++i) {
        ...
        if (!fRenderTasks[i]->unique()) {
            ...
            if (GrOpList* opList = fRenderTasks[i]->asOpList()) {
                opList->endFlush();
            }
        }
    }
...
GrDrawingManager::GrDrawingManager(GrRecordingContext* context,
                                   ...
                                   bool reduceRenderTaskSplitting)
        : fContext(context)
        ...
        , fFlushing(false)
        , fReduceRenderTaskSplitting(reduceRenderTaskSplitting) {
}
...
GrSemaphoresSubmitted GrDrawingManager::flush(GrSurfaceProxy* proxies[], int numProxies,
    ...
    // Semi-usually the GrRenderTasks are already closed at this point, but sometimes Ganesh
    // needs to flush mid-draw. In that case, the SkGpuDevice's GrRenderTasks won't be closed
    // but need to be flushed anyway. Closing such GrRenderTasks here will mean new
    // GrRenderTasks will be created to replace them if the SkGpuDevice(s) write to them again.
    fDAG.closeAll(fContext->priv().caps());
    fActiveRenderTask = nullptr;
        ...
        while (alloc.assign(&startIndex, &stopIndex, &error)) {
            if (GrResourceAllocator::AssignError::kFailedProxyInstantiation == error) {
                for (int i = startIndex; i < stopIndex; ++i) {
                    ...
                    if (GrOpList* opList = renderTask->asOpList()) {
                        opList->purgeOpsWithUninstantiatedProxies();
                    }
                }
            }
...
bool GrDrawingManager::executeRenderTasks(int startIndex, int stopIndex, GrOpFlushState* flushState,
    ...
    for (int i = startIndex; i < stopIndex; ++i) {
        ...
        GrRenderTask* opList = fDAG.renderTask(i);
    ...
    for (sk_sp<GrRenderTargetOpList>& onFlushOpList : fOnFlushCBOpLists) {
...
void GrDrawingManager::copyRenderTasksFromDDL(const SkDeferredDisplayList* ddl,
    ...
    if (fActiveRenderTask) {
        ...
        // own opList.
        fActiveRenderTask->makeClosed(*fContext->priv().caps());
        fActiveRenderTask = nullptr;
    }
...
void GrDrawingManager::validate() const {
    if (fDAG.sortingRenderTasks() && fReduceRenderTaskSplitting) {
        SkASSERT(!fActiveRenderTask);
    } else {
        if (fActiveRenderTask) {
            SkASSERT(!fDAG.empty());
            SkASSERT(!fActiveRenderTask->isClosed());
            SkASSERT(fActiveRenderTask == fDAG.back());
        }

        for (int i = 0; i < fDAG.numRenderTasks(); ++i) {
            if (fActiveRenderTask != fDAG.renderTask(i)) {
