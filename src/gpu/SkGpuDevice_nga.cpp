...
bool SkGpuDevice_nga::onAccessPixels(SkPixmap* pmap) {
    ASSERT_SINGLE_OWNER
    return false;
}

#if 0
GrSurfaceDrawContext* SkGpuDevice_nga::surfaceDrawContext() {
    ASSERT_SINGLE_OWNER
    return fSurfaceDrawContext.get();
}
#endif

void SkGpuDevice_nga::clearAll() {
    ASSERT_SINGLE_OWNER
...
void SkGpuDevice_nga::replaceSurfaceDrawContext(std::unique_ptr<GrSurfaceDrawContext> sdc,
...
void SkGpuDevice_nga::replaceSurfaceDrawContext(SkSurface::ContentChangeMode mode) {
    ASSERT_SINGLE_OWNER
...
SkGpuDevice_nga::ClipType SkGpuDevice_nga::onGetClipType() const {
...
void SkGpuDevice_nga::drawEdgeAAQuad(const SkRect& rect, const SkPoint clip[4],
...
void SkGpuDevice_nga::drawRegion(const SkRegion& region, const SkPaint& paint) {
...
void SkGpuDevice_nga::drawPath(const SkPath& origSrcPath,
                               const SkPaint& paint,
                               bool pathIsMutable) {
#if GR_TEST_UTILS
...
void SkGpuDevice_nga::drawImageRect(const SkImage* image, const SkRect* src, const SkRect& dst,
...
void SkGpuDevice_nga::drawViewLattice(GrSurfaceProxyView view,
...
void SkGpuDevice_nga::drawAtlas(const SkImage* atlas, const SkRSXform xform[],
...
bool SkGpuDevice_nga::wait(int numSemaphores, const GrBackendSemaphore* waitSemaphores,
...
SkBaseDevice* SkGpuDevice_nga::onCreateDevice(const CreateInfo& cinfo, const SkPaint*) {
    ...
    return SkGpuDevice_nga::Make(fContext.get(), std::move(sdc), init).release();
}

sk_sp<SkSurface> SkGpuDevice_nga::makeSurface(const SkImageInfo& info,
...
bool SkGpuDevice_nga::android_utils_clipWithStencil() {
    SkRegion clipRegion;
