...
bool GrSurfaceDrawContext::setupDstProxyView(const GrDrawOp& op, GrDstProxyView* dstProxyView) {
    ...
    // If we require a barrier to sample the dst it means we are sampling the RT itself either as a
    // texture or input attachment.
    bool canSampleSelf = (dstSampleFlags & GrDstSampleFlags::kRequiresTextureBarrier);

    bool usesAttachmentIfDMSAA = fCanUseDynamicMSAA && !this->caps()->msaaResolvesAutomatically();
    bool opUsesMSAA = op.usesMSAA();
    if (usesAttachmentIfDMSAA && (this->getOpsTask()->usesMSAASurface() || opUsesMSAA)) {
        ...
        if (opUsesMSAA) {
            ...
            // without barriers.
            canSampleSelf = canSampleSelf || this->asTextureProxy();
            dstSampleFlags = GrDstSampleFlags::kNone;
        }
    }
