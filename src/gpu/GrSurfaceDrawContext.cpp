...
void GrSurfaceDrawContext::fillRectToRect(const GrClip* clip,
    ...
    if (fContext->priv().alwaysAntialias() && this->caps()->drawInstancedSupport() &&
        ...
        if (auto op = GrFillRRectOp::Make(fContext, std::move(paint), viewMatrix,
                                          ...
                                          GrAA::kYes)) {
            // Even if we couldn't update the coordinates previously because the quad wasn't a rect,
            // preApply already ruled out clipping, so passing null to addDrawOp will entirely skip
            // calling GrClip::apply().
            const GrClip* optimizedClip = (opt == QuadOptimization::kClipApplied) ? nullptr : clip;
