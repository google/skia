...
void GrSurfaceDrawContext::addDrawOp(const GrClip* clip,
    ...
    SkDebugf("addDrawOp. usesAttachmentIfDMSAA: %d, opRequiresDMSAAAttachment: %d, opTriggers: %d\n",
             usesAttachmentIfDMSAA, opRequiresDMSAAAttachment, opTriggersDMSAAAttachment);
    if (opTriggersDMSAAAttachment) {
        ...
        if (this->getOpsTask()->renderPassXferBarriers() & GrXferBarrierFlags::kTexture) {
            SkDebugf("addDrawOp. breaking up for tex barrier\n");
...
bool GrSurfaceDrawContext::setupDstProxyView(const SkRect& opBounds,
    ...
    SkDebugf("setupDstProxyView. opsTask uses msaa: %d, opRequiresMSAA: %d\n",
    ...
    SkDebugf("setupDstProxyView. first dst sample flags: %d\n", dstSampleFlags);
    ...
    if (!(dstSampleFlags & GrDstSampleFlags::kRequiresTextureBarrier) &&
        ...
        SkDebugf("setupDstProxyView. second dst sample flags: %d\n", newFlags);
        ...
        if (newFlags & GrDstSampleFlags::kRequiresTextureBarrier) {
            ...
            SkASSERT(!this->getOpsTask()->isColorNoOp());
            SkDebugf("setupDstProxyView. breaking up ops task 1\n");
            this->replaceOpsTask()->setCannotMergeBackward();
    ...
    if (dstSampleFlags & GrDstSampleFlags::kRequiresTextureBarrier) {
        ...
        SkDebugf("setupDstProxyView. early return 1\n");
    ...
    if (fCanUseDynamicMSAA && opRequiresMSAA && this->asTextureProxy() &&
        ...
        dstProxyView->setOffset(0, 0);
        dstProxyView->setDstSampleFlags(dstSampleFlags);
        SkDebugf("setupDstProxyView. early return 2\n");
    ...
    SkDebugf("setupDstProxyView. doing copy\n");
    ...
    dstProxyView->setDstSampleFlags(dstSampleFlags);
    SkDebugf("setupDstProxyView. final return\n");
