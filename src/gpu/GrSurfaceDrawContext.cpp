...
void GrSurfaceDrawContext::addDrawOp(const GrClip* clip,
    ...
    bool opUsesDMSAAAttachment = usesAttachmentIfDMSAA && usesMSAA;
    bool opTriggersDMSAAAttachment =
            opUsesDMSAAAttachment && !this->getOpsTask()->usesMSAASurface();
    ...
    if (analysis.requiresDstTexture()) {
        if (!this->setupDstProxyView(*drawOp, usesMSAA, &dstProxyView)) {
            return;
        }
#ifdef SK_DEBUG
...
bool GrSurfaceDrawContext::setupDstProxyView(const GrDrawOp& op, bool drawUsesMSAA,
                                             GrDstProxyView* dstProxyView) {
    ...
    auto dstSampleFlags = this->caps()->getDstSampleFlagsForProxy(
            this->asRenderTargetProxy(), this->getOpsTask()->usesMSAASurface() || drawUsesMSAA);

    // If we don't have barriers for this draw then we will definitely be breaking up the GrOpsTask.
    // However, the new GrOpsTask will not have MSAA already enabled on it and then may allow us to
    // use barriers in it. So we break up the ops task and get the flags again.
    if (!(dstSampleFlags & GrDstSampleFlags::kRequiresTextureBarrier) &&
        this->getOpsTask()->usesMSAASurface() && !drawUsesMSAA) {
        auto newFlags =
                this->caps()->getDstSampleFlagsForProxy(this->asRenderTargetProxy(), drawUsesMSAA);
    ...
    // copy.
    if (fCanUseDynamicMSAA && drawUsesMSAA) {
        if (!this->getOpsTask()->isNoOp()) {
            this->replaceOpsTask()->setCannotMergeBackward();
        }
        dstProxyView->setProxyView(this->readSurfaceView());
        dstProxyView->setOffset(0, 0);
