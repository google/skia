...
void GrSurfaceDrawContext::fillRectToRect(const GrClip* clip,
    ...
    if (fContext->priv().alwaysAntialias() && this->caps()->drawInstancedSupport() &&
        ...
        if (clip && opt == QuadOptimization::kClipApplied && viewMatrix.rectStaysRect() &&
            quad.fDevice.asRect(&croppedRect) && quad.fLocal.asRect(&croppedLocal)) {
            ...
            inverse.mapRect(&croppedRect, croppedRect);
        } else {
            // Even if attemptQuadOptimization modified 'quad', GrFillRRectOp needs a rect in
            // pre-matrix space, so use the original rect.
