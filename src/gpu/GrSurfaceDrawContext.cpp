...
void GrSurfaceDrawContext::addDrawOp(const GrClip* clip,
    ...
    if (analysis.requiresDstTexture()) {
        ...
        bool opTriggersDMSAA = opUsesDMSAAAttachment && !this->getOpsTask()->usesMSAASurface();
        if (opTriggersDMSAA) {
            // This will be the op that actually triggers DMSAA. Texture barriers can't be promoted
            // to a DMSAA attachment, so if there already are any on the current opsTask then we
            // need to split.
...
bool GrSurfaceDrawContext::setupDstProxyView(const GrDrawOp& op, GrDstProxyView* dstProxyView) {
    ...
    if ((dstSampleFlags & GrDstSampleFlags::kRequiresTextureBarrier) ||
        ...
        if (hasDMSAAAttachment && (opUsesMSAA || this->getOpsTask()->usesMSAASurface())) {
            ...
            // attachment.
            // If we _will_ render to the DMSAA attachment, we need to split so that DMSAA resolves
            // the RT's texture before we read it.
