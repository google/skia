...
public:
    ...
    bool flushGLState(GrRenderTarget*, const GrProgramInfo&) SK_WARN_UNUSED_RESULT;
    void flushScissorRect(const SkIRect&, int rtWidth, int rtHeight, GrSurfaceOrigin);

    // Returns the last GrGLProgram set by flushGLState(), or nullptr if a different program has
    // since been put in use via some other method (e.g., resetContext, copySurfaceAsDraw).
