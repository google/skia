...
void GrGLCaps::initConfigTable(const GrContextOptions& contextOptions,
   ...
   // GL_EXT_texture_format_BGRA8888.
    ...
    fConfigTable[kRGB_ETC_GrPixelConfig].fFormats.fBaseInternalFormat = GR_GL_COMPRESSED_ETC1_RGB8;
    fConfigTable[kRGB_ETC_GrPixelConfig].fFormats.fSizedInternalFormat =
        GR_GL_COMPRESSED_ETC1_RGB8;
//*** ??    fConfigTable[kRGB_ETC_GrPixelConfig].fFormats.fExternalFormat[kOther_ExternalFormatUsage] = 0;
    fConfigTable[kRGB_ETC_GrPixelConfig].fFormats.fExternalType = 0;
    fConfigTable[kRGB_ETC_GrPixelConfig].fFormatType = kNormalizedFixedPoint_FormatType;
    if (kGL_GrGLStandard == standard) {
        if (version >= GR_GL_VER(4, 3) || ctxInfo.hasExtension("GL_ARB_ES3_compatibility")) {
            fConfigTable[kRGB_ETC_GrPixelConfig].fFlags = ConfigInfo::kTextureable_Flag;
        }
    } else {
        if (version >= GR_GL_VER(3, 0) ||
    ...
    fConfigTable[kRGB_ETC_GrPixelConfig].fSwizzle = GrSwizzle::RGBA();
...
bool GrGLCaps::canCopyTexSubImage(GrPixelConfig dstConfig, bool dstHasMSAARenderBuffer,
    ...
    if (this->canConfigBeFBOColorAttachment(srcConfig) &&
        ...
        dstIsGLTexture2D &&
        dstOrigin == srcOrigin &&
        //*** seems like this could be handled in canConfigBeFBOColorAttachment
        !GrPixelConfigIsCompressed(srcConfig)) {
        return true;
    } else {
        return false;
    }
