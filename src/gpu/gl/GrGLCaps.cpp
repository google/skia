...
void GrGLCaps::initFormatTable(const GrGLContextInfo& ctxInfo, const GrGLInterface* gli,
    ...
    // RGBA8
    {
        ...
        // kRGBA_8888
        {
            auto& ctInfo = info.fColorTypeInfos.get()[ctIdx++];
            ...
            // kRGBA_8888
            {
                auto& ioFormat = ctInfo.fExternalIOFormats.get()[ioIdx++];
    ...
    for (int i = 0; i < kGrGLFormatCount; ++i) {
        ...
        for (int j = 0; j < formatInfo.fColorTypeInfoCount; ++j) {
            const auto& ctInfo = formatInfo.fColorTypeInfos.get()[j];
            ...
            for (int k = 0; k < ctInfo.fExternalIOFormatCount; ++k) {
                const auto& ioInfo = ctInfo.fExternalIOFormats.get()[k];
...
GrCaps::SupportedRead GrGLCaps::onSupportedReadPixelsColorType(
    ...
    for (int i = 0; !foundSrcCT && i < formatInfo.fColorTypeInfoCount; ++i) {
        if (formatInfo.fColorTypeInfos.get()[i].fColorType == srcColorType) {
            const ColorTypeInfo& ctInfo = formatInfo.fColorTypeInfos.get()[i];
            foundSrcCT = true;
            for (int j = 0; j < ctInfo.fExternalIOFormatCount; ++j) {
                const auto& ioInfo = ctInfo.fExternalIOFormats.get()[j];
...
GrColorType GrGLCaps::supportedWritePixelsColorType(GrPixelConfig config,
    ...
    for (int i = 0; !foundSurfaceCT && i < formatInfo.fColorTypeInfoCount; ++i) {
        if (formatInfo.fColorTypeInfos.get()[i].fColorType == surfaceColorType) {
            const ColorTypeInfo& ctInfo = formatInfo.fColorTypeInfos.get()[i];
            foundSurfaceCT = true;
            for (int j = 0; j < ctInfo.fExternalIOFormatCount; ++j) {
                const auto& ioInfo = ctInfo.fExternalIOFormats.get()[j];
