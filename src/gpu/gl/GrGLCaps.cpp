...
bool GrGLCaps::initDescForDstCopy(const GrRenderTargetProxy* src, GrSurfaceDesc* desc,
    ...
    *disallowSubrect = false;
...

    {
        const GrTexture* srcTexture = src->priv().peekTexture();
        if (srcTexture) {
            const GrRenderTarget* srcRT = srcTexture->asRenderTarget();
            const GrGLRenderTarget* glSrcRT = static_cast<const GrGLRenderTarget*>(srcRT);
            if (glSrcRT->renderFBOID() != glSrcRT->textureFBOID()) {
        ...
        } else if (src->desc().fSampleCnt > 0 && this->usesMSAARenderBuffers()) {
            // This is rather fraught so just fallback for now.
            return false;
        }
    }
