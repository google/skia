...
void GrGLCaps::init(const GrContextOptions& contextOptions,
    ...
    **************************************************************************/
    ...
    } // Not sure if this impacts WebGL
...
//            fTransferBufferType = kChromium_TransferBufferType;
    ...
    } // Explictily not supported in WebGL 2.0
...
bool is_float_fp32(const GrGLContextInfo& ctxInfo, const GrGLInterface* gli, GrGLenum precision) {
    ...
    // Both GL ES and WebGL have glGetShaderPrecisionFormat defined
    SkASSERT(gli->fFunctions.fGetShaderPrecisionFormat);
...
void GrGLCaps::initConfigTable(const GrContextOptions& contextOptions,
   ...
   // GL_EXT_texture_format_BGRA8888.
    ...
    // affects Windows.
    if (kSkia8888_GrPixelConfig == kBGRA_8888_GrPixelConfig && GR_IS_GR_GL_ES(standard)) {
        fSRGBSupport = false;
    }
...
#ifdef SK_DEBUG
    // Make sure we initialized everything by comparing all configs to it
    // and making sure they are not equal.
    ConfigInfo uninitializedEntry;
    for (int i = 0; i < kGrPixelConfigCnt; ++i) {
        ...
        // SkDebugf("baseInternalFormat[%d] %d != %d\n", i, uninitializedEntry.fFormats.fBaseInternalFormat, fConfigTable[i].fFormats.fBaseInternalFormat);
        // SkDebugf("fSizedInternalFormat[%d] %d != %d\n", i, uninitializedEntry.fFormats.fSizedInternalFormat, fConfigTable[i].fFormats.fSizedInternalFormat);

        SkASSERT(uninitializedEntry.fFormats.fBaseInternalFormat !=
                 fConfigTable[i].fFormats.fBaseInternalFormat);
        SkASSERT(uninitializedEntry.fFormats.fSizedInternalFormat !=
                 fConfigTable[i].fFormats.fSizedInternalFormat);
        for (int j = 0; j < kExternalFormatUsageCnt; ++j) {
            SkASSERT(uninitializedEntry.fFormats.fExternalFormat[j] !=
                     fConfigTable[i].fFormats.fExternalFormat[j]);
        }
        SkASSERT(uninitializedEntry.fFormats.fExternalType != fConfigTable[i].fFormats.fExternalType);
    }
#endif
}
