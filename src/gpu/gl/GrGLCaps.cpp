...
void GrGLCaps::initFormatTable(const GrContextOptions& contextOptions,
    ...
    // BGRA8
    {
        ...
        // We currently only use the renderbuffer format when allocating msaa render buffers, so we
        // are making decisions here based on that use case. The GL_EXT_texture_format_BGRA8888
        // extension adds BGRA color render buffer support for ES 2.0, but this does not guarantee
        // support for MSAA renderbuffers. The GL_APPLE_texture_format_BGRA8888 does not add support
        // for BGRA color renderbuffers at all. So instead for both cases will use RGBA[8] for our
        // format here for the MSAA buffer. In the GL_EXT_texture_format_BGRA8888 case we can still
