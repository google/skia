...
void GrGLCaps::init(const GrContextOptions& contextOptions,
    ...
    fTextureUsageSupport = IS_GR_GL_ES(standard) &&
    ...
    if (((IS_GR_GL(standard) && version >= GR_GL_VER(4,3)) ||
         (IS_GR_GL_ES(standard) && version >= GR_GL_VER(3,0)) ||
...
void GrGLCaps::initGLSL(const GrGLContextInfo& ctxInfo, const GrGLInterface* gli) {
    ...
    **************************************************************************/
    ...
    if (IS_GR_GL_ES(standard) && k110_GrGLSLGeneration == shaderCaps->fGLSLGeneration) {
...
bool GrGLCaps::hasPathRenderingSupport(const GrGLContextInfo& ctxInfo, const GrGLInterface* gli) {
    ...
    } else if (IS_GR_GL_ES(ctxInfo.standard())) {
...
void GrGLCaps::initBlendEqationSupport(const GrGLContextInfo& ctxInfo) {
    ...
    if ((IS_GR_GL(fStandard) && shaderCaps->generation() >= k140_GrGLSLGeneration)  ||
        (IS_GR_GL_ES(fStandard) && shaderCaps->generation() >= k330_GrGLSLGeneration)) {
...
void GrGLCaps::initConfigTable(const GrContextOptions& contextOptions,
   ...
   // GL_EXT_texture_format_BGRA8888.
    ...
    // affects Windows.
    if (kSkia8888_GrPixelConfig == kBGRA_8888_GrPixelConfig && IS_GR_GL_ES(standard)) {
        fSRGBSupport = false;
    }
    ...
    // update.
    if (texStorageSupported && IS_GR_GL_ES(standard)) {
    ...
    if (alpha8IsValidForGL || (IS_GR_GL_ES(standard) && version < GR_GL_VER(3, 0))) {
    ...
    bool useSizedRbFormats = IS_GR_GL_ES(standard);
    ...
    // in ES.
    if (useSizedTexFormats && IS_GR_GL_ES(standard) && !textureRedSupport) {
    ...
    if (IS_GR_GL_ES(standard) && version == GR_GL_VER(2,0)) {
...
bool GrGLCaps::canCopyTexSubImage(GrPixelConfig dstConfig, bool dstHasMSAARenderBuffer,
    ...
    if (IS_GR_GL_ES(fStandard) && this->bgraIsInternalFormat() &&
