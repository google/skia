...
void GrGLCaps::init(const GrContextOptions& contextOptions,
    ...
    fTextureUsageSupport = IS_GR_GLES(standard) &&
    ...
    fImagingSupport = IS_GR_GL(standard) &&
                      ctxInfo.hasExtension("GL_ARB_imaging");

    if (((IS_GR_GL(standard) && version >= GR_GL_VER(4,3)) ||
         (IS_GR_GLES(standard) && version >= GR_GL_VER(3,0)) ||
...
void GrGLCaps::initGLSL(const GrGLContextInfo& ctxInfo, const GrGLInterface* gli) {
    ...
    **************************************************************************/
    ...
    if (IS_GR_GLES(standard) && k110_GrGLSLGeneration == shaderCaps->fGLSLGeneration) {
...
bool GrGLCaps::hasPathRenderingSupport(const GrGLContextInfo& ctxInfo, const GrGLInterface* gli) {
    ...
    } else if (IS_GR_GLES(ctxInfo.standard())) {
...
void GrGLCaps::initBlendEqationSupport(const GrGLContextInfo& ctxInfo) {
    ...
    if ((IS_GR_GL(fStandard) && shaderCaps->generation() >= k140_GrGLSLGeneration)  ||
        (IS_GR_GLES(fStandard) && shaderCaps->generation() >= k330_GrGLSLGeneration)) {
...
void GrGLCaps::initConfigTable(const GrContextOptions& contextOptions,
   ...
   // GL_EXT_texture_format_BGRA8888.
    ...
    // affects Windows.
    if (kSkia8888_GrPixelConfig == kBGRA_8888_GrPixelConfig && IS_GR_GLES(standard)) {
        fSRGBSupport = false;
    }
    ...
    if (fSRGBSupport && IS_GR_GL(standard)) {
    ...
    // update.
    if (texStorageSupported && IS_GR_GLES(standard)) {
    ...
    bool alpha8IsValidForGL = IS_GR_GL(standard) &&
    ...
    if (alpha8IsValidForGL || (IS_GR_GLES(standard) && version < GR_GL_VER(3, 0))) {
    ...
    if ((IS_GR_GL(standard) && version <= GR_GL_VER(3, 0)) ||
        (IS_GR_GLES(standard) && version < GR_GL_VER(3, 0))) {
    ...
    if (kStandard_MSFBOType == this->msFBOType() && IS_GR_GL(standard) &&
    ...
    uint32_t fpRenderFlags = (IS_GR_GL(standard)) ? allRenderFlags : nonMSAARenderFlags;
    ...
    if (IS_GR_GL(standard) ||
       (IS_GR_GLES(standard) && ctxInfo.version() >= GR_GL_VER(3, 0))) {
    ...
    // in ES.
    if (useSizedTexFormats && IS_GR_GLES(standard) && !textureRedSupport) {
    ...
    if (IS_GR_GLES(standard) && ctxInfo.version() == GR_GL_VER(2,0)) {
...
bool GrGLCaps::canCopyTexSubImage(GrPixelConfig dstConfig, bool dstHasMSAARenderBuffer,
    ...
    if (IS_GR_GLES(fStandard) && this->bgraIsInternalFormat() &&
