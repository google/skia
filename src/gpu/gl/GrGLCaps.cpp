...
bool GrGLCaps::canCopyTexSubImage(GrPixelConfig dstConfig, bool dstHasMSAARenderBuffer,
    ...
    if (this->canConfigBeFBOColorAttachment(srcConfig) &&
        (!srcIsTextureable || !srcIsGLTextureExternal) &&
        dstIsGLTextureExternal) {
        return true;
    } else {
        return false;
    }
...
bool GrGLCaps::canCopyAsBlit(GrPixelConfig dstConfig, int dstSampleCnt,
    ...
    if (dstIsTextureable && dstIsGLTextureExternal) {
        return false;
    }
    if (srcIsTextureable && srcIsGLTextureExternal) {
        return false;
    }
...
bool GrGLCaps::onCanCopySurface(const GrSurfaceProxy* dst, const GrSurfaceProxy* src,
    ...
    bool dstIsExternal = dstTex ? (dstTex->textureType() == GrTextureType::kExternal) : false;
    bool srcIsExternal = srcTex ? (srcTex->textureType() == GrTextureType::kExternal) : false;

    return this->canCopyTexSubImage(dstConfig, has_msaa_render_buffer(dst, *this),
                                    SkToBool(dstTex), dstIsExternal,
                                    srcConfig, has_msaa_render_buffer(src, *this),
                                    SkToBool(srcTex), srcIsExternal) ||
           this->canCopyAsBlit(dstConfig, dstSampleCnt, SkToBool(dstTex),
                               dstIsExternal, srcConfig, srcSampleCnt, SkToBool(srcTex),
                               srcIsExternal, src->getBoundsRect(), src->priv().isExact(),
                               srcRect, dstPoint);
}
