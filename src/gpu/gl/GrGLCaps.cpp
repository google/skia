...
void GrGLCaps::init(const GrContextOptions& contextOptions,
    ...
    **************************************************************************/
    ...
    // SGX and Mali GPUs have tiled architecture that have trouble with frequently changing VBOs.
    ...
    // The Chrome command buffer blocks the use of client side buffers (but may emulate VBOs with them).
    // Client side buffers are not allowed in core profiles.
    if (ctxInfo.driver() != kChromium_GrGLDriver &&
        !fIsCoreProfile &&
        (ctxInfo.vendor() == kARM_GrGLVendor ||
         ctxInfo.vendor() == kImagination_GrGLVendor ||
