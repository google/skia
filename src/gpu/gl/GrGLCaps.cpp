...
void GrGLCaps::applyDriverCorrectnessWorkarounds(const GrGLContextInfo& ctxInfo,
    ...
    // Also fails with LLVM pipe on GalliumLLVM ES.
    formatWorkarounds->fDisableAlpha8Renderable =
            kOSMesa_GrGLRenderer == ctxInfo.renderer() ||
            (GR_IS_GR_GL_ES(ctxInfo.standard()) && ctxInfo.renderer() == kGalliumLLVM_GrGLRenderer);
