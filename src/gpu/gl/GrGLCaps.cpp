...
void GrGLCaps::init(const GrContextOptions& contextOptions,
    ...
    **************************************************************************/
    ...
    // The Nexus 5 and Nexus 7 with 127.0 had different git hashes in the GL_VERSION string so
    // it seems that either it was fixed sometime around the 127.0 version or it reproduces on a 320
    ...
    if (kAdreno3xx_GrGLRenderer == ctxInfo.renderer() && kQualcomm_GrGLDriver == ctxInfo.driver() &&
        ctxInfo.driver() > GR_GL_DRIVER_VER(53, 0)) {
