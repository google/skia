...
void GrGLCaps::initFormatTable(const GrGLContextInfo& ctxInfo, const GrGLInterface* gli,
    ...
    GrGLenum fp16ExternalType = 0;
    if (GR_IS_GR_GL(standard) ||
        (GR_IS_GR_GL_ES(standard) && version >= GR_GL_VER(3, 0))) {
        fp16ExternalType = GR_GL_HALF_FLOAT;
    } else {
        fp16ExternalType = GR_GL_HALF_FLOAT_OES;
    }
    ...
    // RGBA8
    {
        ...
        // kRGBA_8888
        {
            ...
            auto& ctInfo = info.fColorTypeInfos.emplace_back(GrColorType::kRGBA_8888, flags);
            this->setColorTypeFormat(GrColorType::kRGBA_8888, GrGLFormat::kRGBA8);

            // External IO ColorTypes:
            // kRGBA_8888
            ctInfo.fExternalIOFormats.emplace_back(GrColorType::kRGBA_8888, GR_GL_UNSIGNED_BYTE,
        ...
        // kBGRA_8888
        {
            if (GR_IS_GR_GL(standard)) {
                if (version >= GR_GL_VER(1, 2) || ctxInfo.hasExtension("GL_EXT_bgra")) {
                    ...
                    auto& ctInfo = info.fColorTypeInfos.emplace_back(GrColorType::kBGRA_8888,
                                                                     flags);
                    this->setColorTypeFormat(GrColorType::kBGRA_8888, GrGLFormat::kRGBA8);

                    // External IO ColorTypes:
                    // kBGRA_8888
                    ctInfo.fExternalIOFormats.emplace_back(GrColorType::kBGRA_8888,
                                                           GR_GL_UNSIGNED_BYTE, GR_GL_BGRA, 0);

                    // kRGBA_8888
                    ctInfo.fExternalIOFormats.emplace_back(GrColorType::kRGBA_8888,
                                                           GR_GL_UNSIGNED_BYTE, 0, GR_GL_RGBA);
                }
            }
        }
        ...
        {
            uint32_t flags = ColorTypeInfo::kUploadData_Flag;
            auto& ctInfo = info.fColorTypeInfos.emplace_back(GrColorType::kRGB_888x, flags);

            // External IO ColorTypes:
            // kRGBA_888x
            ctInfo.fExternalIOFormats.emplace_back(GrColorType::kRGB_888x, GR_GL_UNSIGNED_BYTE,
                                                   GR_GL_RGBA, GR_GL_RGBA);
        }
    }
    ...
    // R8
    {
        ...
        if (textureRedSupport) {
            ...
            {
                uint32_t flags = ColorTypeInfo::kUploadData_Flag | ColorTypeInfo::kRenderable_Flag;
                auto& ctInfo = info.fColorTypeInfos.emplace_back(GrColorType::kAlpha_8, flags);
                this->setColorTypeFormat(GrColorType::kAlpha_8, GrGLFormat::kR8);

                // External IO ColorTypes:
                // kAlpha_8
                ctInfo.fExternalIOFormats.emplace_back(GrColorType::kAlpha_8, GR_GL_UNSIGNED_BYTE,
                                                       GR_GL_RED, 0);

                // kRGBA_8888
                ctInfo.fExternalIOFormats.emplace_back(GrColorType::kRGBA_8888, GR_GL_UNSIGNED_BYTE,
                                                       0, GR_GL_RGBA, GrSwizzle("000r"));
            }

            // kGray_8
            {
                uint32_t flags = ColorTypeInfo::kUploadData_Flag;
                auto& ctInfo = info.fColorTypeInfos.emplace_back(GrColorType::kGray_8, flags);
                this->setColorTypeFormat(GrColorType::kGray_8, GrGLFormat::kR8);

                // External IO ColorTypes:
                // kGray_8
                ctInfo.fExternalIOFormats.emplace_back(GrColorType::kGray_8, GR_GL_UNSIGNED_BYTE,
                                                       GR_GL_RED, 0);

                // kRGBA_8888
                ctInfo.fExternalIOFormats.emplace_back(GrColorType::kRGBA_8888, GR_GL_UNSIGNED_BYTE,
                                                       0, GR_GL_RGBA, GrSwizzle("rrr1"));
            }
        }
    }
        ...
        if (alpha8IsValidForGL || alpha8IsValidForGLES || alpha8IsValidForWebGL) {
            // kAlpha_8
            {
                ...
                if (alpha8IsValidForGL || alpha8IsValidForGLES || alpha8IsValidForWebGL) {
                    ...
                    // External IO ColorTypes:
                    // kAlpha_8
                    ctInfo.fExternalIOFormats.emplace_back(GrColorType::kAlpha_8,
                                                           GR_GL_UNSIGNED_BYTE, GR_GL_ALPHA, 0);
    ...
    // BGRA8
    {
        ...
        if (SkToBool(info.fFlags &FormatInfo::kTextureable_Flag)) {
            ...
            {
                uint32_t flags = ColorTypeInfo::kUploadData_Flag | ColorTypeInfo::kRenderable_Flag;
                auto& ctInfo = info.fColorTypeInfos.emplace_back(GrColorType::kBGRA_8888, flags);
                this->setColorTypeFormat(GrColorType::kBGRA_8888, GrGLFormat::kBGRA8);

                // External IO ColorTypes:
                // kBGRA_8888
                ctInfo.fExternalIOFormats.emplace_back(GrColorType::kBGRA_8888, GR_GL_UNSIGNED_BYTE,
                                                       GR_GL_BGRA, 0);
    ...
    // RGB565
    {
        ...
        if (SkToBool(info.fFlags &FormatInfo::kTextureable_Flag)) {
            ...
            {
                uint32_t flags = ColorTypeInfo::kUploadData_Flag | ColorTypeInfo::kRenderable_Flag;
                auto& ctInfo = info.fColorTypeInfos.emplace_back(GrColorType::kBGR_565, flags);
                this->setColorTypeFormat(GrColorType::kBGR_565, GrGLFormat::kRGB565);

                // External IO ColorTypes:
                // kBGR_565
                ctInfo.fExternalIOFormats.emplace_back(GrColorType::kBGR_565,
                                                       GR_GL_UNSIGNED_SHORT_5_6_5, GR_GL_RGB, 0);
    ...
    // RGBA16F
    {
        ...
        if (hasFP16Textures) {
            ...
            // kRGBA_F16
            {
                auto& ctInfo = info.fColorTypeInfos.emplace_back(GrColorType::kRGBA_F16, flags);
                this->setColorTypeFormat(GrColorType::kRGBA_F16, GrGLFormat::kRGBA16F);

                // External IO ColorTypes:
                // kRGBA_F16
                ctInfo.fExternalIOFormats.emplace_back(GrColorType::kRGBA_F16,
                                                       fp16ExternalType, GR_GL_RGBA, 0);

                // kRGBA_F32
                ctInfo.fExternalIOFormats.emplace_back(GrColorType::kRGBA_F32, GR_GL_FLOAT,
                                                       0, GR_GL_RGBA);
            }

            // kRGBA_F16_Clamped
            {
                auto& ctInfo = info.fColorTypeInfos.emplace_back(GrColorType::kRGBA_F16_Clamped,
                                                                flags);
                this->setColorTypeFormat(GrColorType::kRGBA_F16_Clamped, GrGLFormat::kRGBA16F);

                // External IO ColorTypes:
                // kRGBA_F16_Clamped
                ctInfo.fExternalIOFormats.emplace_back(GrColorType::kRGBA_F16_Clamped,
                                                       fp16ExternalType, GR_GL_RGBA, 0);
    ...
    // R16F
    {
        ...
        if (textureRedSupport && hasFP16Textures) {
            ...
            {
                uint32_t flags = ColorTypeInfo::kUploadData_Flag | ColorTypeInfo::kRenderable_Flag;
                auto& ctInfo = info.fColorTypeInfos.emplace_back(GrColorType::kAlpha_F16, flags);
                this->setColorTypeFormat(GrColorType::kAlpha_F16, GrGLFormat::kR16F);

                // External IO ColorTypes:
                // kRGBA_F16
                ctInfo.fExternalIOFormats.emplace_back(GrColorType::kAlpha_F16,
                                                       fp16ExternalType, GR_GL_RED, 0);
    ...
    // RGB8
    {
        ...
        // kRGB_888x
        {
            ...
            // External IO ColorTypes:
            // kRGB_888x
            ctInfo.fExternalIOFormats.emplace_back(GrColorType::kRGB_888x,
                                                   GR_GL_UNSIGNED_BYTE, GR_GL_RGB, 0);
    ...
    // RG8
    {
        ...
        if (textureRedSupport) {
            ...
            {
                uint32_t flags = ColorTypeInfo::kUploadData_Flag | ColorTypeInfo::kRenderable_Flag;
                auto& ctInfo = info.fColorTypeInfos.emplace_back(GrColorType::kRG_88, flags);
                this->setColorTypeFormat(GrColorType::kRG_88, GrGLFormat::kRG8);

                // External IO ColorTypes:
                // kRG_88
                ctInfo.fExternalIOFormats.emplace_back(GrColorType::kRG_88,
                                                       GR_GL_UNSIGNED_BYTE, GR_GL_RG, 0);
    ...
    // RGB10_A2
    {
        ...
        if (SkToBool(info.fFlags &FormatInfo::kTextureable_Flag)) {
            ...
            {
                uint32_t flags = ColorTypeInfo::kUploadData_Flag | ColorTypeInfo::kRenderable_Flag;
                auto& ctInfo = info.fColorTypeInfos.emplace_back(GrColorType::kRGBA_1010102, flags);
                this->setColorTypeFormat(GrColorType::kRGBA_1010102, GrGLFormat::kRGB10_A2);

                // External IO ColorTypes:
                // kRGBA_1010102
                ctInfo.fExternalIOFormats.emplace_back(GrColorType::kRGBA_1010102,
    ...
    // RGBA4
    {
        ...
        {
            uint32_t flags = ColorTypeInfo::kUploadData_Flag | ColorTypeInfo::kRenderable_Flag;
            auto& ctInfo = info.fColorTypeInfos.emplace_back(GrColorType::kABGR_4444, flags);
            this->setColorTypeFormat(GrColorType::kABGR_4444, GrGLFormat::kRGBA4);

            // External IO ColorTypes:
            // kABGR_4444
            ctInfo.fExternalIOFormats.emplace_back(GrColorType::kABGR_4444,
    ...
    // SRGB8_ALPHA8
    {
        ...
        if (fSRGBSupport) {
            ...
            {
                uint32_t flags = ColorTypeInfo::kUploadData_Flag | ColorTypeInfo::kRenderable_Flag;
                auto& ctInfo = info.fColorTypeInfos.emplace_back(GrColorType::kRGBA_8888_SRGB,
                                                                flags);
                ...
                // param to Tex(Sub)Image. ES 2.0 requires the <internalFormat> and <format> params
                // to match. Thus, on ES 2.0 we will use GL_SRGB_ALPHA as the <format> param. On
                // OpenGL and ES 3.0+ GL_SRGB_ALPHA does not work for the <format> param to
                // glTexImage.
                if (GR_IS_GR_GL_ES(standard) && version == GR_GL_VER(2,0)) {
                    texImageFormat = GR_GL_SRGB_ALPHA;
                }
                ctInfo.fExternalIOFormats.emplace_back(GrColorType::kRGBA_8888_SRGB,
                                                       GR_GL_UNSIGNED_BYTE,
                                                       texImageFormat, GR_GL_RGBA);
            }
        }
    }
    ...
    // GR_GL_R16
    {
        ...
        if (r16AndRG1616Supported) {
            ...
            {
                uint32_t flags = ColorTypeInfo::kUploadData_Flag | ColorTypeInfo::kRenderable_Flag;
                auto& ctInfo = info.fColorTypeInfos.emplace_back(GrColorType::kR_16, flags);
                this->setColorTypeFormat(GrColorType::kR_16, GrGLFormat::kR16);

                // External IO ColorTypes:
                // kR_16
                ctInfo.fExternalIOFormats.emplace_back(GrColorType::kR_16, GR_GL_UNSIGNED_SHORT,
                                                       GR_GL_RED, 0);
    ...
    // GR_GL_RG16
    {
        ...
        if (r16AndRG1616Supported) {
            ...
            {
                uint32_t flags = ColorTypeInfo::kUploadData_Flag | ColorTypeInfo::kRenderable_Flag;
                auto& ctInfo = info.fColorTypeInfos.emplace_back(GrColorType::kRG_1616, flags);
                this->setColorTypeFormat(GrColorType::kRG_1616, GrGLFormat::kRG16);

                // External IO ColorTypes:
                // kRG_1616
                ctInfo.fExternalIOFormats.emplace_back(GrColorType::kRG_1616, GR_GL_UNSIGNED_SHORT,
                                                       GR_GL_RG, 0);
    ...
    // GR_GL_RGBA16
    {
        ...
        if (rgba16161616Supported) {
            ...
            {
                uint32_t flags = ColorTypeInfo::kUploadData_Flag | ColorTypeInfo::kRenderable_Flag;
                auto& ctInfo = info.fColorTypeInfos.emplace_back(GrColorType::kRGBA_16161616,
                                                                 flags);
                this->setColorTypeFormat(GrColorType::kRGBA_16161616, GrGLFormat::kRGBA16);

                // External IO ColorTypes:
                // kRGBA_16161616
                ctInfo.fExternalIOFormats.emplace_back(GrColorType::kRGBA_16161616,
                                                       GR_GL_UNSIGNED_SHORT, GR_GL_RGBA, 0);
    ...
    // GR_GL_RG16F
    {
        ...
        if (rg16fTexturesSupported) {
            ...
            {
                uint32_t flags = ColorTypeInfo::kUploadData_Flag | ColorTypeInfo::kRenderable_Flag;
                auto& ctInfo = info.fColorTypeInfos.emplace_back(GrColorType::kRG_F16, flags);
                this->setColorTypeFormat(GrColorType::kRG_F16, GrGLFormat::kRG16F);

                // External IO ColorTypes:
                // kRG_F16
                ctInfo.fExternalIOFormats.emplace_back(GrColorType::kRG_F16,
                                                       fp16ExternalType, GR_GL_RG, 0);
...
GrCaps::SupportedRead GrGLCaps::onSupportedReadPixelsColorType(
    ...
    // dstColorType. If that doesn't exisits we will use any valid read pixels GrColorType.
    GrCaps::SupportedRead fallbackRead = {GrSwizzle{}, GrColorType::kUnknown, 0};
    auto& formatInfo = this->getFormatInfo(srcFormat);
    bool foundSrcCT = false;
    for (int i = 0; !foundSrcCT && i < formatInfo.fColorTypeInfos.count(); ++i) {
        if (formatInfo.fColorTypeInfos[i].fColorType == srcColorType) {
            const ColorTypeInfo& ctInfo = formatInfo.fColorTypeInfos[i];
            foundSrcCT = true;
            for (int j = 0; j < ctInfo.fExternalIOFormats.count(); ++j) {
                auto& ioInfo = ctInfo.fExternalIOFormats[j];
                if (ioInfo.fExternalFormat[kReadPixels_ExternalFormatUsage] != 0) {
...
GrColorType GrGLCaps::supportedWritePixelsColorType(GrPixelConfig config,
    ...
    // srcColorType. If that doesn't exisits we will use any supported GrColorType.
    GrColorType fallbackCT = GrColorType::kUnknown;
    auto& formatInfo = this->getFormatInfo(surfaceFormat);
    bool foundSurfaceCT = false;
    for (int i = 0; !foundSurfaceCT && i < formatInfo.fColorTypeInfos.count(); ++i) {
        if (formatInfo.fColorTypeInfos[i].fColorType == surfaceColorType) {
            const ColorTypeInfo& ctInfo = formatInfo.fColorTypeInfos[i];
            foundSurfaceCT = true;
            for (int j = 0; j < ctInfo.fExternalIOFormats.count(); ++j) {
                auto& ioInfo = ctInfo.fExternalIOFormats[j];
                if (ioInfo.fExternalFormat[kTexImage_ExternalFormatUsage] != 0) {
