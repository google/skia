    ...
    **************************************************************************/
    ...
    if (GR_IS_GR_GL(standard)) {
        ...
        if (ctxInfo.vendor() != GrGLVendor::kARM &&
            ...
            GrGLRenderer::kAdreno4xx_other != ctxInfo.renderer()) {
...
//            fTransferBufferType = TransferBufferType::kChromium;
    ...
    fReuseScratchTextures = GrGLVendor::kARM != ctxInfo.vendor();

#if 0
    fReuseScratchBuffers = kARM != ctxInfo.vendor() &&
                           kQualcomm != ctxInfo.vendor();
#endif
...
void GrGLCaps::applyDriverCorrectnessWorkarounds(const GrGLContextInfo& ctxInfo,
    ...
    if (ctxInfo.renderer() == GrGLRenderer::kPowerVRRogue) {
        // Our ChromebooGrGLRenderer::k with kPowerVRRogue crashes on large instanced draws. The
    ...
    if (GrGLRenderer::kTegra_PreK1 == ctxInfo.renderer()) {
        fDisallowTexSubImageForUnormConfigTexturesEverBoundToFBO = true;
    ...
    if (ctxInfo.renderer() == GrGLRenderer::kAdreno530 ||
        GrGLRenderer::kAdreno5xx_other == ctxInfo.renderer()) {
    ...
    shaderCaps->fCanUseAnyFunctionInShader = GrGLVendor::kImagination != ctxInfo.vendor();
    ...
    formatWorkarounds->fDisableSRGBRenderWithMSAAForMacAMD = kATI == ctxInfo.vendor();
#endif
