...
void GrGLCaps::getTexSubImageFormatAndType(GrGLFormat surfaceFormat, GrColorType surfaceColorType,
...
void GrGLCaps::initFormatTable(const GrGLContextInfo& ctxInfo, const GrGLInterface* gli,
    ...
    // Format: ALPHA8
    {
        ...
        info.fFormatType = FormatType::kNormalizedFixedPoint;
        GrGLenum sizedFormatIfAvailable;
        if (GR_IS_GR_GL_ES(standard)) {
            // ES does not have sized internal format GL_ALPHA8.
            sizedFormatIfAvailable = GR_GL_ALPHA;
        } else {
            sizedFormatIfAvailable = GR_GL_ALPHA8;
        }
        ...
        // TODO: Are these workarounds just a consequence of not having a sized internal format
        // available and glTexStorage2D() requiring one in certain cicrumstances?
        if (texStorageSupported &&
            !formatWorkarounds.fDisablePerFormatTextureStorageForCommandBufferES2 &&
            !formatWorkarounds.fDisableNonRedSingleChannelTexStorageForANGLEGL) {
            info.fFlags |= FormatInfo::kCanUseTexStorage_Flag;
            info.fInternalFormatForTexImageOrStorage = sizedFormatIfAvailable;
        } else {
            info.fInternalFormatForTexImageOrStorage = texImageSupportsSizedInternalFormat ? sizedFormatIfAvailable : GR_GL_ALPHA;
        }
    ...
    // Format: BGRA8
    {
        ...
        if (GR_IS_GR_GL_ES(standard)) {
            if (ctxInfo.hasExtension("GL_EXT_texture_format_BGRA8888")) {
                ...
                // GL_EXT_texture_format_BGRA8888. However, ES3 also adds glTexStorage but
                // without GL_EXT_texture_storage does not add BGRA support.
    ...
    // Format: RG8
    {
        ...
        if (!(info.fFlags & FormatInfo::kCanUseTexStorage_Flag)) {
            info.fInternalFormatForTexImageOrStorage =
