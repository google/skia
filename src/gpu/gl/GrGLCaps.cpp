...
GrCaps::SupportedRead GrGLCaps::supportedReadPixelsColorType(GrPixelConfig config,
    ...
    auto swizzle = fConfigTable[config].fRGBAReadSwizzle;
    switch (fConfigTable[config].fFormatType) {
        case kNormalizedFixedPoint_FormatType:
            if (kRGB_888X_GrPixelConfig == config && *glFormat == GR_GL_RGB8) {
                // This can skip an unecessary conversion.
