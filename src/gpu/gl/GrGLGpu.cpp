...
bool GrGLGpu::onWritePixels(GrSurface* surface, int left, int top, int width, int height,
    ...
    if (GrPixelConfigIsCompressed(glTex->config())) {
        ...
        SkASSERT(srcAsConfig == glTex->config());
        success = this->uploadCompressedTexData(glTex->config(), glTex->width(), glTex->height(),
                                                glTex->target(), kWrite_UploadType,
                                                left, top, width, height, srcAsConfig, texels,
                                                mipLevelCount);
    } else {
...
//*** how different is this? Could it be folded into allocate_and_populate_texture?
...
bool GrGLGpu::uploadTexData(GrPixelConfig texConfig, int texWidth, int texHeight, GrGLenum target,
    ...
    // *** Why? What's so special about compressed data? It's just data.
...
bool GrGLGpu::uploadCompressedTexData(GrPixelConfig texConfig, int texWidth, int texHeight,
    ...
    //*** set up mip status properly (see above)
    if (mipMapsStatus) {
        *mipMapsStatus = GrMipMapsStatus::kValid;
    }
    ...
    if (kNewTexture_UploadType == uploadType) {
        ...
        for (int currentMipLevel = 0; currentMipLevel < mipLevelCount; currentMipLevel++) {
            ...
            // Make sure that the width and height that we pass to OpenGL
            // is a multiple of the block size.
            size_t dataSize = GrCompressedFormatDataSize(texConfig, currentWidth, currentHeight);
...
//**** seems like we could just add one check to onCreateTexture...
sk_sp<GrTexture> GrGLGpu::onCreateCompressedTexture(const GrSurfaceDesc& desc, SkBudgeted budgeted,
                                                    const GrMipLevel texels[],
                                                    int mipLevelCount) {
    ...
    initialTexParams = set_initial_texture_params(this->glInterface(), idDesc.fInfo);

    if (!this->uploadCompressedTexData(desc.fConfig, desc.fWidth, desc.fHeight,
                                       idDesc.fInfo.fTarget, kNewTexture_UploadType, 0, 0,
                                       desc.fWidth, desc.fHeight, desc.fConfig,
                                       texels, mipLevelCount, &mipMapsStatus)) {
        GL_CALL(DeleteTextures(1, &(idDesc.fInfo.fID)));
        return return_null_texture();
    }
    idDesc.fInfo.fFormat = this->glCaps().configSizedInternalFormat(desc.fConfig);

    sk_sp<GrGLTexture> tex = sk_make_sp<GrGLTexture>(this, budgeted, desc, idDesc, mipMapsStatus,
                                                     true);
    tex->setCachedParams(&initialTexParams, tex->getCachedNonSamplerParams(),
                         this->getResetTimestamp());
#ifdef TRACE_TEXTURE_CREATION
...
void GrGLGpu::bindTexture(int unitIdx, GrSamplerState samplerState, GrGLTexture* texture) {
    ...
    if (samplerState.filter() == GrSamplerState::Filter::kMipMap) {
        if (!this->caps()->mipMapSupport() ||
            ...
            GrPixelConfigIsCompressed(texture->config())) {
