...
void GrGLGpu::flushRenderTargetNoColorWrites(GrGLRenderTarget* target, bool useMultisampleFBO) {
    ...
    if (fHWBoundRenderTargetUniqueID != rtID || fHWBoundRenderTargetIsMSAA != useMultisampleFBO) {
        GrGLuint fbo = (useMultisampleFBO) ? target->multisampleFBOID()
                                           : target->singleSampleFBOID();
        this->bindFramebuffer(GR_GL_FRAMEBUFFER, fbo);
#ifdef SK_DEBUG
        ...
        fHWBoundRenderTargetIsMSAA = useMultisampleFBO;
