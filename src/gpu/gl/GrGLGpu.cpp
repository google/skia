...
void GrGLGpu::onFinishFlush(GrSurfaceProxy*[], int, SkSurface::BackendSurfaceAccess access,
    ...
    if (info.fFinishedProc) {
        ...
        if (this->caps()->fenceSyncSupport()) {
            ...
            SkDebugf("has finish proc with fence support. sync is: %d\n", callback.fSync);
        } else {
            SkDebugf("has finish proc but no fence support\n", callback.fSync);
            callback.fSync = 0;
        }
...
bool GrGLGpu::onSubmitToGpu(bool syncCpu) {
    SkDebugf("gl submit to gpu 1. syncCpu: %d\n", syncCpu);
