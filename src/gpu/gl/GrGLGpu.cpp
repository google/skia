...
bool GrGLGpu::waitSync(GrGLsync sync, uint64_t timeout, bool flush) {
    if (this->glCaps().fenceType() == GrGLCaps::FenceType::kNVFence) {
        // Ignore the timeout and flush params. FinishFence always flushes and has no
        // way to timeout. If this really becomes necessary we could poll TestFence() to
        // implement these params.
...
bool GrGLGpu::waitFence(GrFence fence, uint64_t timeout) {
    SkASSERT(this->glCaps().fenceType() == GrGLCaps::FenceType::kSyncObject);
