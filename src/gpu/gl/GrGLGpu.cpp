...
GrBackendTexture GrGLGpu::onCreateBackendTexture(SkISize dimensions,
    ...
    info.fID = this->createTexture2D(dimensions, glFormat, renderable,
                                     &initialState, numMipLevels);
    if (!info.fID) {
        return {};
    }
    ...
    if (data && data->type() == BackendTextureData::Type::kPixmaps) {
        ...
        if (!this->uploadTexData(glFormat, colorType, desc.fWidth, desc.fHeight,
        ...
        GrColorType colorType;
        GrGLenum externalFormat, externalType;
        this->glCaps().getTexSubImageDefaultFormatTypeAndColorType(glFormat, &externalFormat, &externalType, &colorType);
