...
bool GrGLGpu::onGetWritePixelsInfo(GrSurface* dstSurface, GrSurfaceOrigin dstOrigin, int width,
    ...
    auto srcConfigSRGBEncoding = GrPixelConfigIsSRGBEncoded(dstSurface->config());
    if (*drawPreference != kNoDraw_DrawPreference) {
        ...
        srcConfigSRGBEncoding = GrPixelConfigIsSRGBEncoded(tempDrawInfo->fTempSurfaceDesc.fConfig);
    }
    ...
    auto srcAsConfig = GrColorTypeToPixelConfig(srcColorType, srcConfigSRGBEncoding);
...
bool GrGLGpu::onGetReadPixelsInfo(GrSurface* srcSurface, GrSurfaceOrigin srcOrigin, int width,
    ...
    // We don't want to introduce a sRGB conversion if we trigger a draw.
    auto dstConfigSRGBEncoding = GrPixelConfigIsSRGBEncoded(srcSurface->config());
    if (*drawPreference != kNoDraw_DrawPreference) {
