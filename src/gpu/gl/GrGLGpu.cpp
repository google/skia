...
static void fill_in_with_color(GrPixelConfig config, int width, int height,
    ...
    switch (config) {
        ...
        case kGray_8_as_Red_GrPixelConfig: {
            int gray = SkTPin(0.2989f * r + 0.5870f * g + 0.1140f * b, 0.0f, 255.0f);
            memset(dest, gray, width * height);
            break;
        }
        case kRGB_565_GrPixelConfig: {
        ...
        case kRGBA_half_Clamped_GrPixelConfig: {
            ...
            for (int i = 0; i < 4 * width * height; ++i, destHalf += 4) {
                destHalf[0] = rHalf;
                destHalf[1] = gHalf;
                destHalf[2] = bHalf;
                destHalf[3] = aHalf;
...
GrBackendTexture GrGLGpu::createBackendTexture(int w, int h,
    ...
    if (GrPixelConfigIsCompressed(config)) {
        ...
        if (!pixels) {
            fill_in_with_color(config, w, h, colorf, defaultStorage.get());
