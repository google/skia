...
sk_sp<GrTexture> GrGLGpu::onCreateTexture(const GrSurfaceDesc& desc,
    ...
    SkASSERT(GrGLCaps::kNone_MSFBOType == this->glCaps().msFBOType() ||
    ...
    if (renderable == GrRenderable::kYes) {
        ...
        if (!this->createRenderTargetObjects(desc, renderTargetSampleCount, idDesc.fInfo, &rtIDDesc)) {
            GL_CALL(DeleteTextures(1, &idDesc.fInfo.fID));
            return return_null_texture();
        }
        tex = sk_make_sp<GrGLTextureRenderTarget>(this, budgeted, desc, renderTargetSampleCount, idDesc, rtIDDesc,
                                                  mipMapsStatus);
