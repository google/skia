...
GrGLGpu::~GrGLGpu() {
    ...
    if (0 != fClearColorProgram.fProgram) {
...
bool GrGLGpu::createClearColorProgram() {
    ...
    if (!fClearProgramArrayBuffer) {
        static const GrGLfloat vdata[] = {-1, -1, 1, -1, -1, 1, 1, 1};
        fClearProgramArrayBuffer.reset(GrGLBuffer::Create(
                this, sizeof(vdata), kVertex_GrBufferType, kStatic_GrAccessPattern, vdata));
        if (!fClearProgramArrayBuffer) {
            return false;
        }
    ...
    SkSL::Program::Inputs inputs;
    GrGLuint vshader =
            GrGLCompileAndAttachShader(*fGLContext, fClearColorProgram.fProgram, GR_GL_VERTEX_SHADER,
                                       &str, &length, 1, &fStats, settings, &inputs);
    SkASSERT(inputs.isEmpty());
    ...
    length = SkToInt(fshaderTxt.size());
    GrGLuint fshader =
            GrGLCompileAndAttachShader(*fGLContext, fClearColorProgram.fProgram, GR_GL_FRAGMENT_SHADER,
                                       &str, &length, 1, &fStats, settings, &inputs);
    SkASSERT(inputs.isEmpty());
    ...
    GL_CALL_RET(fClearColorProgram.fColorUniform, GetUniformLocation(fClearColorProgram.fProgram, "u_color"));
...
void GrGLGpu::clearColorAsDraw(const GrFixedClip& clip, GrGLfloat r, GrGLfloat g, GrGLfloat b, GrGLfloat a, GrRenderTarget* dst,
                               GrSurfaceOrigin origin) {
    if (!fClearColorProgram.fProgram) {
