...
bool GrGLGpu::onTransferPixelsTo(GrTexture* texture, int left, int top, int width, int height,
    ...
    this->glCaps().getTexSubImageFormatAndType(textureFormat, textureColorType, bufferColorType,
...
bool GrGLGpu::uploadTexData(GrGLFormat textureFormat, GrColorType textureColorType,
    ...
    this->glCaps().getTexSubImageFormatAndType(textureFormat, textureColorType, srcColorType,
                                                &externalFormat, &externalType);
    if (!externalFormat || !externalType) {
        return false;
    }
...
sk_sp<GrTexture> GrGLGpu::onCreateTexture(const GrSurfaceDesc& desc,
    ...
    if (levelClearMask) {
        ...
        size_t bpp;
        this->glCaps().getTexSubImageZeroFormatTypeAndBpp(texDesc.fFormat,
                                                          &externalFormat, &externalType, &bpp);
        if (this->glCaps().clearTextureSupport()) {
            ...
            for (int i = 0; i < mipLevelCount; ++i) {
                if (levelClearMask & (1U << i)) {
                    int levelWidth  = SkTMax(1, texDesc.fSize.width( ) >> (1 << i));
                    int levelHeight = SkTMax(1, texDesc.fSize.height() >> (1 << i));
                    if (!zeros) {
                        zeros.reset(new char[levelWidth*levelHeight*bpp]);
                    }
                    GL_CALL(TexSubImage2D(GR_GL_TEXTURE_2D, i, 0, 0, levelWidth, levelHeight, externalFormat, externalType, zeros.get()));
                }
            }
        }
    }
...
int GrGLGpu::getCompatibleStencilIndex(GrGLFormat format) {
    ...
    if (!this->glCaps().hasStencilFormatBeenDeterminedForFormat(format)) {
        ...
        // TODO: null initial state, leave-bound?
        GrGLTextureParameters::SamplerOverriddenState initialState;
        GrGLuint colorID = this->createTexture2D({kSize, kSize}, format, GrRenderable::kYes, &initialState, 1);
        if (!colorID) {
            return -1;
        }
        // unbind the texture from the texture unit before binding it to the frame buffer
...
GrBackendTexture GrGLGpu::createBackendTexture(int w, int h,
    ...
    if (GrGLFormatToCompressionType(glFormat, &compressionType)) {
        ...
        if (mipLevelCount && ! this->uploadTexData(glFormat, textureColorType,
                 desc.fWidth, desc.fHeight, GR_GL_TEXTURE_2D,
                 0, 0,
