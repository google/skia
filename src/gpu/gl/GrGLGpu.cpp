...
bool GrGLGpu::onTransferPixels(GrTexture* texture,
    ...
    if (glTex->width() != width) {
        // at the moment we only support transfer buffers on desktop so we can assume support
        SkASSERT(this->glCaps().unpackRowLengthSupport());
        GL_CALL(PixelStorei(GR_GL_UNPACK_ROW_LENGTH, glTex->width()));
