...
void GrGLGpu::onFBOChanged() {
    if (this->caps()->workarounds().flush_on_framebuffer_change ||
        ...
        this->glFlushIfRequired(ForceFlush::kYes);
    }
#ifdef SK_DEBUG
...
void GrGLGpu::glFlushIfRequired(ForceFlush forceFlush) {
    if (fNeedsGLFlush || forceFlush == ForceFlush::kYes) {
        GL_CALL(Flush());
        fNeedsGLFlush = false;
    }
}
