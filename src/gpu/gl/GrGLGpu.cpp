...
void GrGLGpu::draw(GrRenderTarget* renderTarget, GrSurfaceOrigin origin,
    ...
    if (!this->flushGLState(renderTarget, origin, primProc, pipeline, fixedDynamicState, dynamicStateArrays, meshCount,
                            hasPoints)) {
        return;
    }
    ...
    for (int m = 0; m < meshCount; ++m) {
        if (GrXferBarrierType barrierType = pipeline.xferBarrierType(renderTarget->asTexture(), *this->caps())) {
