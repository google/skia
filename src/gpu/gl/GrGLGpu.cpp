...
bool GrGLGpu::flushGLState(GrRenderTarget* renderTarget,
    ...
    this->flushBlendAndColorWrite(
        programInfo.pipeline().getXferProcessor().getBlendInfo(), programInfo.pipeline().outputSwizzle());
    ...
    if (programInfo.pipeline().isStencilEnabled()) {
        ...
        stencil.reset(*programInfo.pipeline().getUserStencil(), programInfo.pipeline().hasStencilClip(),
    ...
    if (programInfo.pipeline().isScissorEnabled()) {
        ...
        GrScissorState state(programInfo.fixedDynamicState() ? programInfo.fixedDynamicState()->fScissorRect : kBogusScissor);
...
void GrGLGpu::draw(GrRenderTarget* renderTarget,
    ...
    if (programInfo.dynamicStateArrays()) {
        dynamicScissor = programInfo.pipeline().isScissorEnabled() && programInfo.dynamicStateArrays()->fScissorRects;
        dynamicPrimProcTextures = programInfo.dynamicStateArrays()->fPrimitiveProcessorTextures;
    }
    for (int m = 0; m < meshCount; ++m) {
        ...
        if (dynamicScissor) {
            GrGLRenderTarget* glRT = static_cast<GrGLRenderTarget*>(renderTarget);
            this->flushScissor(GrScissorState(programInfo.dynamicStateArrays()->fScissorRects[m]),
                               glRT->width(), glRT->height(), programInfo.origin());
        }
        if (dynamicPrimProcTextures) {
            auto texProxyArray = programInfo.dynamicStateArrays()->fPrimitiveProcessorTextures +
                                 m * programInfo.primProc().numTextureSamplers();
