    ...
    if (caps.formatSupportsTexStorage(format)) {
        ...
        if (!mipLevelCount) {
            ...
            for (int currentMipLevel = 0; currentMipLevel < mipLevelCount; currentMipLevel++) {
                ...
                // performance on a ES2 GPUs we're calling glTexImage2D and then glTexSubImage2D and
                // hoping we don't get any performance regressions.
                GL_ALLOC_CALL(&interface,
                              TexImage2D(target,
                                         currentMipLevel,
                                         internalFormat,
                                         ...
                                         nullptr));
                if (currentMipData) {
                    GR_GL_CALL(&interface, TexSubImage2D(target, currentMipLevel, 0, 0, currentWidth, currentHeight, externalFormat, externalType, currentMipData));
                }
                GrGLenum error = CHECK_ALLOC_ERROR(&interface);
                if (error != GR_GL_NO_ERROR) {
                    return false;
