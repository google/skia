...
GrGLGpu::GrGLGpu(GrGLContext* ctx, GrContext* context)
    ...
    fStencilClipClearProgram.fProgram = 0;
...
GrGLGpu::~GrGLGpu() {
    ...
    if (0 != fStencilClipClearProgram.fProgram) {
        GL_CALL(DeleteProgram(fStencilClipClearProgram.fProgram));
    }
...
void GrGLGpu::disconnect(DisconnectType type) {
    ...
    if (DisconnectType::kCleanup == type) {
        ...
        if (fStencilClipClearProgram.fProgram) {
            GL_CALL(DeleteProgram(fStencilClipClearProgram.fProgram));
        }
    } else {
        if (fProgramCache) {
...
bool GrGLGpu::createStencilClipClearProgram() {
    if (!fStencilClipClearArrayBuffer) {
        ...
        fStencilClipClearArrayBuffer.reset(GrGLBuffer::Create(this, sizeof(vdata), kVertex_GrBufferType,
                                                      kStatic_GrAccessPattern, vdata));
        if (!fStencilClipClearArrayBuffer) {
            return false;
        }
    ...
    SkASSERT(!fStencilClipClearProgram.fProgram);
    GL_CALL_RET(fStencilClipClearProgram.fProgram, CreateProgram());
    if (!fStencilClipClearProgram.fProgram) {
        return false;
    }
    ...
    SkSL::Program::Inputs inputs;
    GrGLuint vshader = GrGLCompileAndAttachShader(*fGLContext, fStencilClipClearProgram.fProgram,
                                                  GR_GL_VERTEX_SHADER, &str, &length, 1,
                                                  &fStats, settings, &inputs);
    SkASSERT(inputs.isEmpty());
    ...
    str = fshaderTxt.c_str();
    length = SkToInt(fshaderTxt.size());
    GrGLuint fshader = GrGLCompileAndAttachShader(*fGLContext, fStencilClipClearProgram.fProgram,
                                                  GR_GL_FRAGMENT_SHADER, &str, &length, 1,
                                                  &fStats, settings, &inputs);
    SkASSERT(inputs.isEmpty());

    GL_CALL(LinkProgram(fStencilClipClearProgram.fProgram));

    GL_CALL(BindAttribLocation(fStencilClipClearProgram.fProgram, 0, "a_vertex"));
...
void GrGLGpu::clearStencilClipAsDraw(const GrFixedClip& clip, bool insideStencilMask,
    ...
    GL_CALL(UseProgram(fStencilClipClearProgram.fProgram));
    fHWProgramID = fStencilClipClearProgram.fProgram;
    ...
    SkASSERT(sb);
    GrStencilSettings settings = GrStencilSettings(*GrStencilSettings::SetClipBitSettings(insideStencilMask), false, sb->bits());
