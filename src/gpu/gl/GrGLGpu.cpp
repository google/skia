...
sk_sp<GrRenderTarget> GrGLGpu::onWrapBackendRenderTarget(const GrBackendRenderTarget& backendRT) {
    ...
    rtIDs.fNumSamplesOwnedPerPixel = sampleCount;
...
bool GrGLGpu::createRenderTargetObjects(const GrGLTexture::Desc& desc,
    ...
    rtIDs->fNumSamplesOwnedPerPixel = 0;
    ...
    GL_CALL(FramebufferTexture2D(GR_GL_FRAMEBUFFER,
                                 ...
                                 desc.fID,
                                 0));
    ++rtIDs->fNumSamplesOwnedPerPixel;
...
void GrGLGpu::flushWindowRectangles(const GrWindowRectsState& windowState,
    ...
    SkASSERT(!windowState.enabled() || rt->singleSampleFBOID() != 0 || rt->multisampleFBOID() != 0);
...
void GrGLGpu::endCommandBuffer(GrRenderTarget* rt,
    ...
    if (GrGLCaps::kNone_InvalidateFBType != this->glCaps().invalidateFBType()) {
        ...
        if (GrStoreOp::kDiscard == colorLoadStore.fStoreOp) {
            ...
            discardAttachments.push_back((renderFBOID == 0) ? GR_GL_COLOR
                                                            : GR_GL_COLOR_ATTACHMENT0);
        }
        if (GrStoreOp::kDiscard == stencilLoadStore.fStoreOp) {
            GrGLuint renderFBOID = (glRT->numSamples() > 1) ? glRT->multisampleFBOID()
                                                            : glRT->singleSampleFBOID();
            discardAttachments.push_back((renderFBOID == 0) ? GR_GL_STENCIL
                                                            : GR_GL_STENCIL_ATTACHMENT);
        }
...
bool GrGLGpu::readOrTransferPixelsFrom(GrSurface* surface, int left, int top, int width, int height,
    ...
    if (renderTarget) {
        if (renderTarget->numSamples() > 1) {
            ...
            this->flushRenderTargetNoColorWrites(renderTarget, true/*useMultsampleFBO*/);
        } else {
            this->flushRenderTargetNoColorWrites(renderTarget, false/*useMultsampleFBO*/);
        }
    } else {
        // Use a temporary FBO.
...
void GrGLGpu::flushRenderTargetNoColorWrites(GrGLRenderTarget* target, bool useMultisampleFBO) {
    ...
    if (fHWBoundRenderTargetUniqueID != rtID || fHWMultisampleFBOIsBound != useMultisampleFBO) {
        ...
        fHWMultisampleFBOIsBound = useMultisampleFBO;
...
void GrGLGpu::xferBarrier(GrRenderTarget* rt, GrXferBarrierType type) {
    ...
    switch (type) {
        case kTexture_GrXferBarrierType: {
            ...
            SkASSERT(glrt->singleSampleFBOID() != 0 && glrt->multisampleFBOID() != 0);
