...
bool GrGLGpu::onWritePixels(GrSurface* surface, GrSurfaceOrigin origin, int left, int top,
    ...
    // No sRGB transformation occurs in uploadTexData. We choose to match the src config match the
...
bool GrGLGpu::onGetReadPixelsInfo(GrSurface* srcSurface, GrSurfaceOrigin srcOrigin, int width,
    ...
    auto dstConfigSRGBEncoding = GrPixelConfigIsSRGBEncoded(tempDrawInfo->fTempSurfaceDesc.fConfig);
    if (*drawPreference != kNoDraw_DrawPreference) {
    ...
    auto dstAsConfig = GrColorTypeToPixelConfig(dstColorType, dstConfigSRGBEncoding);
