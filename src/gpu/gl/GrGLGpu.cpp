...
GrBackendRenderTarget GrGLGpu::createTestingOnlyBackendRenderTarget(SkISize dimensions,
    ...
    if ((sampleCnt > 1 && !this->glCaps().usesMSAARenderBuffers())) {
        ...
        // No extension allows making render buffers of BGRA8. So we must use a texture, which
        // means we must fail if MSAA. We already checked above for render-to-msaa-texture
        // extensions.
        if (sampleCnt > 1) {
            return {};
        }
