...
bool GrGLGpu::onWritePixels(GrSurface* surface,
    ...
    if (GrPixelConfigIsCompressed(glTex->config())) {
        ...
        success = this->uploadCompressedTexData(glTex, texels, kWrite_UploadType, left, top, width,
                                                height);
    } else {
        success = this->uploadTexData(glTex, kWrite_UploadType, left, top, width, height, config,
                                      texels);
    }
...
bool GrGLGpu::onTransferPixels(GrSurface* surface,
    ...
    success = this->uploadTexData(glTex, kTransfer_UploadType, left, top, width, height, config,
                                  texels);
    return success;
}
    ...
    if (useTexStorage) {
        ...
        if (error != GR_GL_NO_ERROR) {
            ...
            for (int currentMipLevel = 0; currentMipLevel < texels.count(); currentMipLevel++) {
                ...
                size_t dataSize = GrCompressedFormatDataSize(config, currentWidth,
                                                             currentHeight);
...
bool GrGLGpu::uploadTexData(GrPixelConfig texConfig,
    ...
    GrGLenum externalFormat;
    GrGLenum externalType;
    if (!this->glCaps().getTexImageFormats(texConfig, dataConfig, &internalFormat,
                                           &externalFormat, &externalType)) {
        return false;
    }
    ...
    if (!texelsShallowCopy.empty()) {
        ...
        GR_GL_CALL(interface, PixelStorei(GR_GL_UNPACK_ALIGNMENT,
                                          config_alignment(texConfig)));
    }

    bool succeeded = true;
    if (kNewTexture_UploadType == uploadType) {
        succeeded = 0 == left && 0 == top &&
                   texWidth == width && texHeight == height &&
                   allocate_and_populate_uncompressed_texture(texConfig,
                                                             *interface, caps, target,
                                                             internalFormat,
                                                             internalFormatForTexStorage,
                                                             externalFormat, externalType,
...
bool GrGLGpu::uploadCompressedTexData(GrPixelConfig config,
                                      int texWidth, int texHeight,
                                      GrSurfaceOrigin texOrigin,
                                      GrGLenum target,
                                      const SkTArray<GrMipLevel>& texels,
    ...
    if (kNewTexture_UploadType == uploadType) {
        return allocate_and_populate_compressed_texture(config, *interface,
                                                        caps, target, internalFormat, texels, width, height);
    } else {
        for (int currentMipLevel = 0; currentMipLevel < texels.count(); currentMipLevel++) {
