        ...
        for (const GrMesh::SoftRepetition& repetition : mesh) {
            ...
            if (const GrBuffer* indexBuffer = mesh.fIndexBuffer.get()) {
                ...
                if (this->glCaps().drawRangeElementsSupport()) {
                    ...
                    int start = 0;
                    int end = mesh.fVertexCount * repetition.fRepeatCount - 1;
                    GL_CALL(DrawRangeElements(gPrimitiveType2GLMode[mesh.fPrimitiveType],
                                              start, end,
                                              mesh.fIndexCount * repetition.fRepeatCount,
                ...
                GL_CALL(DrawArrays(gPrimitiveType2GLMode[mesh.fPrimitiveType], 0,
                                   mesh.fVertexCount * repetition.fRepeatCount));
            }
            fStats.incNumDraws();
        }
    }
