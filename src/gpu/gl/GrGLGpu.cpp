...
bool GrGLGpu::onGetWritePixelsInfo(GrSurfaceProxy* dstProxy, int width, int height,
                                   GrPixelConfig srcConfig,
                                   DrawPreference* drawPreference,
                                   WritePixelTempDrawInfo* tempDrawInfo) {
    if (SkToBool(dstProxy->asRenderTargetProxy())) {
    ...
    if (!this->glCaps().unpackFlipYSupport() &&
        kBottomLeft_GrSurfaceOrigin == dstProxy->origin()) {
        ElevateDrawPreference(drawPreference, kGpuPrefersDraw_DrawPreference);
    }
...
sk_sp<GrTexture> GrGLGpu::onCreateTexture(const GrSurfaceDesc& desc, GrSurfaceOrigin origin,
                                          SkBudgeted budgeted,
                                          const GrMipLevel texels[],
                                          int mipLevelCount) {
    ...
    idDesc.fOwnership = GrBackendObjectOwnership::kOwned;
    GrGLTexture::TexParams initialTexParams;
    if (!this->createTextureImpl(desc, origin, &idDesc.fInfo, isRenderTarget, &initialTexParams,
...
bool GrGLGpu::createTextureImpl(const GrSurfaceDesc& desc, GrSurfaceOrigin origin, GrGLTextureInfo* info,
    ...
    if (!this->uploadTexData(desc.fConfig, desc.fWidth, desc.fHeight, origin, info->fTarget,
...
bool GrGLGpu::flushGLState(const GrPipeline& pipeline, const GrPrimitiveProcessor& primProc,
    ...
    if (blendInfo.fWriteColor) {
        ...
        const GrSwizzle& swizzle = this->caps()->shaderCaps()->configOutputSwizzle(
            pipeline.getRenderTarget1()->config());
        this->flushBlend(blendInfo, swizzle);
    }

    program->setData(primProc, pipeline);

    GrGLRenderTarget* glRT = static_cast<GrGLRenderTarget*>(pipeline.getRenderTarget1());
    ...
    this->flushScissor(pipeline.getScissorState(), glRT->getViewport(), pipeline.origin());
...
void GrGLGpu::clear(const GrFixedClip& clip, GrColor color, GrRenderTarget* target, GrSurfaceOrigin origin) {
    this->handleDirtyContext();
...
bool GrGLGpu::readPixelsSupported(GrPixelConfig rtConfig, GrPixelConfig readConfig) {
    ...
    auto bindRenderTarget = [this, rtConfig, &temp]() -> bool {
        ...
        if (this->glCaps().isConfigRenderable(rtConfig, false)) {
            desc.fFlags = kRenderTarget_GrSurfaceFlag;
            temp = this->createTexture(desc, kBottomLeft_GrSurfaceOrigin, SkBudgeted::kNo);
            if (!temp) {
                return false;
            }
            ...
            temp = this->createTexture(desc, kTopLeft_GrSurfaceOrigin, SkBudgeted::kNo);
            if (!temp) {
                return false;
            }
            GrGLIRect vp;
...
void GrGLGpu::draw(const GrPipeline& pipeline,
    ...
    for (int i = 0; i < meshCount; ++i) {
        if (GrXferBarrierType barrierType = pipeline.xferBarrierType(*this->caps())) {
            this->xferBarrier(pipeline.getRenderTarget1(), barrierType);
        }
...
static inline bool can_blit_framebuffer_for_copy_surface(const GrSurface* dst, GrSurfaceOrigin dstOrigin,
...
bool GrGLGpu::onCopySurface(GrSurface* dst, GrSurfaceOrigin dstOrigin,
    ...
    if (can_blit_framebuffer_for_copy_surface(dst, dstOrigin, src, srcOrigin, srcRect, dstPoint, this)) {
...
bool GrGLGpu::copySurfaceAsBlitFramebuffer(GrSurface* dst, GrSurfaceOrigin dstOrigin,
                                           ...
                                           const SkIRect& srcRect,
                                           const SkIPoint& dstPoint) {
    SkASSERT(can_blit_framebuffer_for_copy_surface(dst, dstOrigin, src, srcOrigin, srcRect, dstPoint, this));
    SkIRect dstRect = SkIRect::MakeXYWH(dstPoint.fX, dstPoint.fY,
                                        srcRect.width(), srcRect.height());
    if (dst == src) {
...
bool GrGLGpu::generateMipmap(GrGLTexture* texture, GrSurfaceOrigin textureOrigin, bool gammaCorrect) {
