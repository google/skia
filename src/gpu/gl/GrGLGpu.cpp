...
void GrGLGpu::setupGeometry(const GrBuffer* indexBuffer,
    ...
    // Ordering the vertex attributes before the instance attributes must agree with the order
    // in which GrGLProgramBuilder assigns attrib locations (BindAttribLocation).
    if (int vertexStride = fHWProgram->vertexStride()) {
        SkASSERT(vertexBuffer && !vertexBuffer->isMapped());
        auto bufferOffset = vertexBuffer->baseOffset() + baseVertex * vertexStride;
        for (int i = 0; i < fHWProgram->numVertexAttributes(); ++i) {
            ...
            attribState->set(this, i, vertexBuffer, attrib.fType, vertexStride,
    ...
    if (int instanceStride = fHWProgram->instanceStride()) {
        ...
        auto bufferOffset = instanceBuffer->baseOffset() + baseInstance * instanceStride;
        ...
        for (int i = 0; i < fHWProgram->numInstanceAttributes(); ++i, ++attribIdx) {
            ...
            attribState->set(this, attribIdx, instanceBuffer, attrib.fType, instanceStride,
                             bufferOffset + attrib.fOffset, kDivisor);
        }
    }
}
