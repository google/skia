...
void GrGLGpu::onResetContext(uint32_t resetBits) {
    ...
    if (resetBits & kStencil_GrGLBackendState) {
        ...
        for (int i = 0; i < 2; ++i) {
            constexpr static GrGLenum kInvalidStencilEnum = -1;
...
void GrGLGpu::clearStencilClip(const GrScissorState& scissor, bool insideStencilMask,
    ...
    static constexpr uint16_t clipStencilMask  = (1 << (stencilBitCount - 1));
#else
    ...
    static constexpr uint16_t clipStencilMask  = ~0;
#endif
    GrGLint value;
