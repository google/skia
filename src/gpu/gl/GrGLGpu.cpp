...
GrGpuRTCommandBuffer* GrGLGpu::getCommandBuffer(
    ...
    if (!fCachedRTCommandBuffer1) {
        fCachedRTCommandBuffer1.reset(new GrGLGpuRTCommandBuffer(this));
    }

    fCachedRTCommandBuffer1->set(rt, origin, colorInfo, stencilInfo);
    return fCachedRTCommandBuffer1.get();
}
...
void GrGLGpu::submit(GrGpuCommandBuffer* buffer) {
    if (buffer->asRTCommandBuffer()) {
        SkASSERT(fCachedRTCommandBuffer1 == buffer);
        fCachedRTCommandBuffer1->reset();
    } else {
        SkASSERT(fCachedTexCommandBuffer == buffer);
