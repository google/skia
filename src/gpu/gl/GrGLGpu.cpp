...
static inline bool can_blit_framebuffer_for_copy_surface(const GrSurface* dst,
    ...
    bool dstIsGLTextureExt= dstTex ? GR_GL_TEXTURE_EXTERNAL == dstTex->target() : false;
    bool srcIsGLTextureExt = srcTex ? GR_GL_TEXTURE_EXTERNAL == srcTex->target() : false;

    return caps.canCopyAsBlit(dst->config(), dstSampleCnt, SkToBool(dstTex), dstIsGLTextureExt,
                              src->config(), srcSampleCnt, SkToBool(srcTex), srcIsGLTextureExt,
...
static inline bool can_copy_texsubimage(const GrSurface* dst, const GrSurface* src,
    ...
    return caps.canCopyTexSubImage(dst->config(), dstHasMSAARenderBuffer, SkToBool(dstTex),
                                   dstIsGLTextureExt, src->config(), srcHasMSAARenderBuffer,
                                   SkToBool(srcTex), srcIsGLTextureExt);
}
...
#if 0
    SkDebugf("GrGLGpu::onCopySurface. srcRect LTRB: (%d, %d, %d, %d), dstPoint: (%d, %d)\n",
             srcRect.fLeft, srcRect.fTop, srcRect.fRight, srcRect.fBottom, dstPoint.fX, dstPoint.fY);
#endif
...
#if 0
        SkDebugf("doing copy as tex sub image\n");
#endif
...
#if 0
        SkDebugf("doing copy as blit framebuffer\n");
#endif
...
#if 0
    SkDebugf("src width: %d, src height: %d, dst width: %d, dst height: %d\n",
             src->width(), src->height(), dst->width(), dst->height());
    SkDebugf("srcLeft: %d, srcBottom: %d, srcRight: %d, srcTop: %d\n",
             srcRect.fLeft, srcRect.fTop, srcRect.fRight, srcRect.fBottom);
    SkDebugf("dstLeft: %d, dstBottom: %d, dstRight: %d, dstTop: %d\n",
