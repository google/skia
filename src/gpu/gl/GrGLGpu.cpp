        ...
        if (this->glCaps().usesMSAARenderBuffers()) {
            ...
            if (GrGLCaps::kES_Apple_MSFBOType == this->glCaps().msFBOType()) {
                ...
                if (GrGLCaps::kResolveMustBeFull_BlitFrambufferFlag &
                    this->glCaps().blitFramebufferSupport()) {
                    l = 0;
                    b = 0;
                    r = target->width();
                    t = target->height();
...
static inline bool can_blit_framebuffer_for_copy_surface(const GrSurface* dst,
                                                         ...
                                                         const GrGLGpu* gpu) {
    auto blitFramebufferFlags = gpu->glCaps().blitFramebufferSupport();
    ...
    if (GrGLCaps::kNoMSAADst_BlitFramebufferFlag & blitFramebufferFlags) {
        // When copying to a MSAA GrRenderTarget we write into the MSAA render buffer not
        // the texture used for resolving.
