...
sk_sp<GrTexture> GrGLGpu::onWrapBackendTexture(const GrBackendTexture& backendTex,
    ...
    if (renderTarget) {
        ...
        sk_sp<GrGLTextureRenderTarget> texRT(GrGLTextureRenderTarget::MakeWrapped(this, surfDesc, idDesc, rtIDDesc));
        texRT->level0WasBoundToFBO();
    }
...
bool GrGLGpu::onGetWritePixelsInfo(GrSurface* dstSurface, int width, int height,
    ...
    if (texture) {
        ...
        if (texture->hasLevel0BeenBoundToFBO() &&
