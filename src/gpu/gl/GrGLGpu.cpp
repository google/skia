...
static bool allocate_and_populate_compressed_texture(GrPixelConfig config,
                                                     ...
                                                     int baseWidth, int baseHeight) {
    CLEAR_ERROR_BEFORE_ALLOC(&interface);
    SkASSERT(GrGLFormatIsCompressed(target));
    ...
    if (useTexStorage) {
        ...
        if (error != GR_GL_NO_ERROR) {
            ...
            for (int currentMipLevel = 0; currentMipLevel < mipLevelCount; currentMipLevel++) {
                ...
                size_t dataSize = GrGLFormatCompressedDataSize(target, currentWidth, currentHeight);
        ...
        for (int currentMipLevel = 0; currentMipLevel < mipLevelCount; currentMipLevel++) {
            ...
            // Make sure that the width and height that we pass to OpenGL
            // is a multiple of the block size.
            size_t dataSize = GrGLFormatCompressedDataSize(target, baseWidth, baseHeight);
