...
bool GrGLGpu::onGetWritePixelsInfo(GrSurface* dstSurface, GrSurfaceOrigin dstOrigin, int width,
    ...
    auto srcAsConfig = GrColorTypeToPixelConfig(srcColorType,
                                                GrPixelConfigIsSRGBEncoded(dstSurface->config()));
...
bool GrGLGpu::onWritePixels(GrSurface* surface, GrSurfaceOrigin origin, int left, int top,
    ...
    if (!check_write_and_transfer_input(glTex, surface)) {
        return false;
    }
    ...
    auto srcAsConfig = GrColorTypeToPixelConfig(srcColorType, GrSRGBEncoded::kNo);
...
bool GrGLGpu::onTransferPixels(GrTexture* texture, int left, int top, int width, int height,
    ...
    if (!check_write_and_transfer_input(glTex, texture)) {
        return false;
    }
...
bool GrGLGpu::onGetReadPixelsInfo(GrSurface* srcSurface, GrSurfaceOrigin srcOrigin, int width,
    ...
    // If we use a intermediate draw, should the temporary surface be sRGB encoded.
    // If we have to do a conversion it was already setup by our caller so we'll infer from the
    // config chosen for the temp surface. Otherwise, we aren't doing a sRGB conversion so mimic
    // the src config.
    auto dstAsConfigIsSRGBEncoded = GrPixelConfigIsSRGBEncoded(tempDrawInfo->fTempSurfaceDesc.fConfig);
    if (*drawPreference != kNoDraw_DrawPreference) {
        ...
        SkASSERT(tempDrawInfo->fSwizzle == GrSwizzle::RGBA());
        dstAsConfigIsSRGBEncoded =
                GrPixelConfigIsSRGBEncoded(tempDrawInfo->fTempSurfaceDesc.fConfig);
    }
    ...
    auto dstAsConfig = GrColorTypeToPixelConfig(dstColorType, dstAsConfigIsSRGBEncoded);
