...
void GrGLGpu::draw(GrRenderTarget* renderTarget, GrSurfaceOrigin origin,
    ...
    if (!this->flushGLState(renderTarget, origin, primProc, pipeline, fixedDynamicState,
        ...
        // TEMPORARY: We will soon remove msaa and mipmap status from GrRenderTarget and GrTexture,
        // but as we transition, we are asserting that the proxies agree with our own dirty states.
        // The proxies could not have known flushGLStates would fail, and will expect the target to
        // be dirty. Mark it dirty before aborting the draw just to keep the asserts happy.
        renderTarget->flagAsNeedingResolve();
