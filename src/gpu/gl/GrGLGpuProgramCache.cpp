...
sk_sp<GrGLProgram> GrGLGpu::ProgramCache::findOrCreateProgram(GrRenderTarget* renderTarget,
    ...
    if (!tmp) {
        fGpu1->fStats.incNumInlineCompilationFailures();
    } else {
        fGpu1->fStats.incNumInlineProgramCacheResult(stat);
    }
    ...
    *stat = Stats::ProgramCacheResult::kHit;
    ...
    if (entry && !(*entry)->fProgram) {
        ...
        if (!(*entry)->fProgram) {
            ...
            fGpu1->fStats.incNumCompilationFailures();
            return nullptr;
        }
        fGpu1->fStats.incNumPartialCompilationSuccesses();
        *stat = Stats::ProgramCacheResult::kPartial;
        ...
        fGpu1->fStats.incNumCompilationSuccesses();
