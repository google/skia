...
GrGLProgram::GrGLProgram(GrGLGpu* gpu,
    ...
    fProgramDataManager.setImages(images);
}
...
void GrGLProgram::bindTextures(const GrProcessor& processor,
    ...
    for (int i = 0; i < processor.numStorageImages(); ++i) {
        const GrProcessor::StorageImageAccess& access = processor.storageImageAccess(i);
        fGpu->bindImage((*nextSamplerIdx)++, access.ioType(),
