...
private:
    ...
    bool onUpdatePrimProcTextureBindings(const GrPrimitiveProcessor& primProc, const GrPipeline&,
                                         const GrSurfaceProxy* const proxies[]) override {
        fGpu->updatePrimitiveProcessorTextureBindings(primProc, proxies);
        return true;
    }
