...
void GrGLGetDriverInfo(GrGLStandard standard,
    ...
    *outVersion = GR_GL_DRIVER_UNKNOWN_VER;
    ...
    if (vendor == kImagination_GrGLVendor) {
        int revision;
        int n = sscanf(versionString, "OpenGL ES %d.%d build %d.%d@%d", &major, &minor, &driverMajor, &driverMinor, &revision);
        if (n == 5) {
            *outVersion == GR_GL_DRIVER_VER(driverMajor, driverMinor, revision);
        }
        return;
    }
