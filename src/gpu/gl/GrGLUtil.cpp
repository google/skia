...
// If innerString is not null then if this is recognized as an ANGLE renderer string it will contain
// the renderer string with the initial "ANGLE (" and final ")" stripped off.
static GrGLANGLEBackend get_angle_backend(const char* rendererString, SkString* innerString) {
    SkASSERT(rendererString);
    // crbug.com/1203705 ANGLE renderer will be "ANGLE (<gl-vendor>, <gl-renderer>, <gl-version>)"
    // on ANGLE's GL backend with similar substitutions for the inner strings on other backends.
    ...
    if (!strncmp(rendererString, kHeader, kHeaderLength) &&
        ...
        if (strstr(rendererString, "Direct3D11")) {
            return GrGLANGLEBackend::kD3D11;
        } else if (strstr(rendererString, "Direct3D9")) {
            return GrGLANGLEBackend::kD3D9;
        } else if (strstr(rendererString, "OpenGL")) {
            return GrGLANGLEBackend::kOpenGL;
        }
    }
...
get_angle_gl_vendor_and_renderer(
    ...
    GrGLVendor angleVendor = get_vendor(parts[0].c_str());
    auto [angleDriver, angleDriveVersion] = get_driver_and_version(kGLES_GrGLStandard,
                                                                   angleVendor,
                                                                   parts[1].c_str(),
                                                                   parts[2].c_str());
    return {angleVendor,
            get_renderer(parts[1].c_str(), extensions),
            angleDriver,
            angleDriveVersion};
}
...
GrGLDriverInfo GrGLGetDriverInfo(const GrGLInterface* interface) {
    ...
    info.fANGLEBackend = get_angle_backend(renderer, &innerAngleRendererString);
