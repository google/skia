...
SkString list_shaders(const char** skslStrings, int* lengths, int count, const SkSL::String& glsl) {
    SkString sksl = GrGLSLPrettyPrint::PrettyPrintGLSL(skslStrings, lengths, count, false);
...
std::unique_ptr<SkSL::Program> translate_to_glsl(const GrGLContext& context, GrGLenum type,
    ...
    SkString sksl;
#ifdef SK_DEBUG
    sksl = GrGLSLPrettyPrint::PrettyPrintGLSL(skslStrings, lengths, count, false);
#else
...
GrGLuint GrGLCompileAndAttachShader(const GrGLContext& glCtx,
    ...
    // Lazy initialized pretty-printed sksl for dumping.
