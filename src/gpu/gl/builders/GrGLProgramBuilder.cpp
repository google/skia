...
struct GrGLSLCacheEntry {
    GrGLSLCacheEntry(const GrGLSLSet& glsl) {
    ...
    const char* get(int shaderType) const {
        ...
        // Can't directly read fOffset, it's likely misaligned in the cache
        size_t offset;
        memcpy(&offset, &fOffset[shaderType], sizeof(size_t));
        return offset ? SkTAddOffset<const char>(this, offset) : nullptr;
    }
...
void GrGLProgramBuilder::storeShaderInCache(const SkSL::Program::Inputs& inputs, GrGLuint programID,
    ...
    if (fGpu->glCaps().programBinarySupport()) {
        ...
        // source cache
        size_t dataLength = sizeof(inputs) + sizeof(GrGLSLCacheEntry) + glsl.getCacheSize();
        std::unique_ptr<uint8_t[]> data(new uint8_t[dataLength]);
        size_t offset = 0;
        memcpy(data.get() + offset, &inputs, sizeof(inputs));
        offset += sizeof(inputs);
        GrGLSLCacheEntry entry(glsl);
...
GrGLProgram* GrGLProgramBuilder::finalize() {
    ...
    if (cached) {
        ...
        if (fGpu->glCaps().programBinarySupport()) {
            ...
            const GrGLSLCacheEntry* entry = (const GrGLSLCacheEntry*)(bytes + offset);
            for (int i = 0; i < kGrShaderTypeCount; ++i) {
                if (entry->fOffset[i]) {
