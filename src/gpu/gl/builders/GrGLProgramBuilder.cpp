...
GrGLProgram* GrGLProgramBuilder::finalize() {
    ...
    SkSL::String gsGLSL;
    SkSL::String fsGLSL;
    bool cached = fCachedVS.size();
    if (cached) {
        vsGLSL = std::move(fCachedVS);
        gsGLSL = std::move(fCachedGS);
        fsGLSL = std::move(fCachedFS);
    } else {
        GrSkSLtoGLSL(gpu()->glContext(), GR_GL_VERTEX_SHADER, fVS.fCompilerStrings.begin(),
                     fVS.fCompilerStringLengths.begin(), fVS.fCompilerStrings.count(), settings,
                     &vsGLSL);
        if (primProc.willUseGeoShader()) {
            GrSkSLtoGLSL(gpu()->glContext(), GR_GL_GEOMETRY_SHADER, fGS.fCompilerStrings.begin(),
                         fGS.fCompilerStringLengths.begin(), fGS.fCompilerStrings.count(), settings,
                         &gsGLSL);
        }
        ...
        std::unique_ptr<SkSL::Program> fs = GrSkSLtoGLSL(gpu()->glContext(),
                                                         ...
                                                         settings,
                                                         &fsGLSL);
        if (fs->fInputs.fRTHeight) {
            this->addRTHeightUniform(SKSL_RTHEIGHT_NAME);
        }
        if (fCache) {
            fCache->store(*this->desc(), vsGLSL, gsGLSL, fsGLSL);
        }
    }
    ...
    if (fCachedFS.size()) {
        if (!this->compileAndAttachShaders(fCachedFS.c_str(), fCachedFS.size(), programID,
                                           GR_GL_FRAGMENT_SHADER, &shadersToDelete, settings,
                                           fCachedFSInputs)) {
            this->cleanupProgram(programID, shadersToDelete);
            return nullptr;
        }
    } else if (!this->compileAndAttachShaders(fFS, programID, GR_GL_FRAGMENT_SHADER, &shadersToDelete,
                                              settings, &inputs)) {
        this->cleanupProgram(programID, shadersToDelete);
        return nullptr;
    }
