...
GrBackendTexture GrGLTexture::getBackendTexture() const {
    ...
    info.fID = fID;
    info.fFormat = GrGLFormatToSizedEnum(fFormat);
...
GrBackendFormat GrGLTexture::backendFormat() const {
    return GrBackendFormat::MakeGL(GrGLFormatToSizedEnum(fFormat),
                                   target_from_texture_type(this->texturePriv().textureType()));
}

sk_sp<GrGLTexture> GrGLTexture::MakeWrapped(GrGLGpu* gpu, const Desc& desc,
                                            GrMipMapsStatus mipMapsStatus,
                                            sk_sp<GrGLTextureParameters> parameters,
                                            GrWrapCacheable cacheable, GrIOType ioType) {
    return sk_sp<GrGLTexture>(new GrGLTexture(gpu, desc, mipMapsStatus,
