...
static inline GrTextureType TextureTypeFromTarget(GrGLenum target) {
    switch (target) {
...
static inline GrGLenum TargetFromTextureType(GrTextureType type) {
    switch (type) {
...
GrGLenum GrGLTexture::target() const {
    return TargetFromTextureType(this->texturePriv().textureType());
}
...
GrBackendTexture GrGLTexture::getBackendTexture() const {
    ...
    info.fTarget = TargetFromTextureType(this->texturePriv().textureType());
    info.fID = fID;
