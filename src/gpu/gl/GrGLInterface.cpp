...
bool GrGLInterface::validate() const {
    ...
    if (IS_GR_GL(fStandard) ||
       (IS_GR_GLES(fStandard) && glVer >= GR_GL_VER(3,0))) {
    ...
    if (IS_GR_GL(fStandard) ||
       (IS_GR_GLES(fStandard) && glVer >= GR_GL_VER(3,1))) {
    ...
    // which we require.
    if (IS_GR_GL(fStandard) ||
       (IS_GR_GLES(fStandard) && fExtensions.has("GL_OES_mapbuffer"))) {
    ...
    if (IS_GR_GL(fStandard) || IS_GR_GLES(fStandard)) {
    ...
    } else if (IS_GR_GLES(fStandard) && glVer >= GR_GL_VER(3,0)) {
    ...
    if (IS_GR_GLES(fStandard) && fExtensions.has("GL_CHROMIUM_map_sub")) {
    ...
    if ((IS_GR_GL(fStandard) && fExtensions.has("GL_ARB_map_buffer_range")) ||
        (IS_GR_GLES(fStandard) && fExtensions.has("GL_EXT_map_buffer_range")) ||
        // These functions are added to the 3.0 version of both GLES and GL.
        ((IS_GR_GL(fStandard) || IS_GR_GLES(fStandard)) && glVer >= GR_GL_VER(3,0))) {
    ...
    if (IS_GR_GLES(fStandard) || glVer >= GR_GL_VER(4,1) ||
    ...
    if ((IS_GR_GL(fStandard) &&
        ...
        (IS_GR_GLES(fStandard) && fExtensions.has("GL_EXT_multi_draw_indirect"))) {
