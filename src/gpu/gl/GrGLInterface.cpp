...
bool GrGLInterface::validate() const {
    ...
    if (IS_GR_GL(fStandard) ||
       (IS_GR_GL_ES(fStandard) && glVer >= GR_GL_VER(3,0))) {
    ...
    if (IS_GR_GL(fStandard) ||
       (IS_GR_GL_ES(fStandard) && glVer >= GR_GL_VER(3,1))) {
    ...
    if (IS_GR_GL(fStandard) && !fFunctions.fGetQueryObjectiv) {
        RETURN_FALSE_INTERFACE;
    }
    ...
    // which we require.
    if (IS_GR_GL(fStandard) ||
       (IS_GR_GL_ES(fStandard) && fExtensions.has("GL_OES_mapbuffer"))) {
    ...
    if (IS_GR_GL(fStandard) || IS_GR_GL_ES(fStandard)) {
    ...
    if ((IS_GR_GL(fStandard) && glVer >= GR_GL_VER(4,3)) ||
    ...
    } else if (IS_GR_GL_ES(fStandard) && glVer >= GR_GL_VER(3,0)) {
    ...
    if (IS_GR_GL_ES(fStandard) && fExtensions.has("GL_CHROMIUM_map_sub")) {
    ...
    if ((IS_GR_GL(fStandard) && fExtensions.has("GL_ARB_map_buffer_range")) ||
        (IS_GR_GL_ES(fStandard) && fExtensions.has("GL_EXT_map_buffer_range")) ||
        // These functions are added to the 3.0 version of both GLES and GL.
        ((IS_GR_GL(fStandard) || IS_GR_GL_ES(fStandard)) && glVer >= GR_GL_VER(3,0))) {
    ...
    if (IS_GR_GL_ES(fStandard) || glVer >= GR_GL_VER(4,1) ||
    ...
    if ((IS_GR_GL(fStandard) &&
        ...
        (IS_GR_GL_ES(fStandard) && fExtensions.has("GL_EXT_multi_draw_indirect"))) {
    ...
    if ((IS_GR_GL(fStandard) && (glVer >= GR_GL_VER(4,2) ||
        (IS_GR_GL_ES(fStandard) && glVer >= GR_GL_VER(3,0)) ||
    ...
    if ((IS_GR_GL(fStandard) && glVer >= GR_GL_VER(4,1)) ||
        (IS_GR_GL_ES(fStandard) && glVer >= GR_GL_VER(3,0))) {
