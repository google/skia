...
sk_sp<GrGLRenderTarget> GrGLRenderTarget::MakeWrapped(GrGLGpu* gpu,
    ...
    if (stencilBits) {
        ...
        // a format with the same number of bits. We don't even directly use the format or any
        // other properties. Thus it is fine for us to just assign an arbitrary format that matches
        // the bit count.
