...
#define ASSERT_ARRAY_UPLOAD_IN_BOUNDS(UNI, COUNT) \
         SkASSERT((COUNT) <= (UNI).fArrayCount || \
                  (1 == (COUNT) && GrShadeVar::kNonArray == (UNI).fArrayCount))
...
GrGLProgramDataManager::GrGLProgramDataManager(GrGLGpu* gpu, GrGLuint programID,
    ...
    for (int i = 0; i < count; i++) {
        ...
        SkASSERT(GrShadeVar::kNonArray == builderUniform.fVariable.getArrayCount() ||
    ...
    for (int i = 0; i < count; i++) {
        ...
        SkASSERT(GrShadeVar::kNonArray == builderPathProcVarying.fVariable.getArrayCount() ||
