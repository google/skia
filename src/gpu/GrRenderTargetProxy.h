...
public:
    ...
    bool hasDynamicMSAA(const GrRecordingContext* ctx) const {
#if GR_TEST_UTILS
        return ctx->priv().options().fAlwaysAntialias &&
               ctx->priv().caps()->dynamicMSAASupport() &&
               ctx->priv().caps()->internalMultisampleCount(this->backendFormat()) > 1 &&
               this->asTextureProxy();
#else
        return false;
#endif
