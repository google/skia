...
public:
    ...
    bool isMSAADirty() const {
        SkASSERT(this->requiresManualMSAAResolve() || !fIsMSAADirty);
        return fIsMSAADirty;
    }
...
private:
    ...
    // Indicates whether any sub-rectangle of the render target requires MSAA resolve. We currently
    // rely on the GrRenderTarget itself to track the actual dirty rect, and only resolve that area.
