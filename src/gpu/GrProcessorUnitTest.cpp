...
std::unique_ptr<GrFragmentProcessor> GrProcessorUnitTest::MakeChildFP(GrProcessorTestData* data) {
    ...
    // We've gone too deep, but we can't return null at non-optional call sites without risking an
    // assertion. Instead, return a known-simple zero-child FP. This limits the recursion, and the
...
std::unique_ptr<GrFragmentProcessor> GrProcessorUnitTest::MakeInputFP(GrProcessorTestData* data) {
    std::unique_ptr<GrFragmentProcessor> inputFP;
    if (data->fCurrentTreeDepth == 0) {
        // At the top level of the tree, provide the input FP from the test data.
        return data->inputFP();
    } else {
        // At deeper levels of recursion, fill in a random FP.
        return MakeChildFP(data);
    }
}
