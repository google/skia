...
public:
     ...
     * extent.
     */
    GrCoordTransform(GrTextureProxy* proxy) : fProxy(proxy) {}

    /**
     * Create a transformation from a matrix. The proxy origin also implies whether a y-reversal
     * should be performed.
     */
    GrCoordTransform(const SkMatrix& m, GrTextureProxy* proxy) : fProxy(proxy), fMatrix(m) {
        SkASSERT(proxy);
    }
    ...
    bool normalize() const { return fProxy && fProxy->textureType() != GrTextureType::kRectangle; }
...
private:
    const GrTextureProxy*   fProxy = nullptr;
