...
public:
     ...
     *
     * @param dimensions     width/height of the texture to be created.
     ...
     *                       floor(max((log2(dimensions.fWidth), log2(dimensions.fHeight)))). It must
     *                       be the latter if GrCaps::createTextureMustSpecifyAllLevels() is true.
...
private:
    ...
    sk_sp<GrTexture> createTextureCommon(const SkISize&,
                                         const GrBackendFormat&,
                                         GrRenderable,
                                         int renderTargetSampleCnt,
                                         SkBudgeted,
