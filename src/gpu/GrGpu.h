...
public:
    ...
    virtual GrBackendRenderTarget createTestingOnlyBackendRenderTarget(
            SkISize, GrColorType, int sampleCount = 1, const SkPMColor4f* = nullptr) = 0;

    /**
     * Deletes a GrBackendRenderTarget allocated with the above. Should only be called after the
     * release proc on SkSurface::MakeFromBackendRenderTarget has been called.
     */
