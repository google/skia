...
public:
    ...
    // src/dst bounds and non-empty. They must be in exact device space including already being
    // flipped for origin if need be. No origin transformations will be performed. If
    // canDiscardOutsideDstRect is set to true then we don't need to preserve any data on the dst
    // surface outside of the copy.
