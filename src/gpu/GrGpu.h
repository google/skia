...
public:
     ...
     * @param renderTargetSampleCount The number of samples to use for rendering if renderable is
    ...
    sk_sp<GrTexture> createTexture(const GrSurfaceDesc& desc, GrRenderable renderable,
                                   int renderTargetSampleCount, SkBudgeted, GrProtected isProtected,
                                   const GrMipLevel texels[], int mipLevelCount);

    /**
     * Simplified createTexture() interface for when there is no initial texel data to upload.
     */
    sk_sp<GrTexture> createTexture(const GrSurfaceDesc&, GrRenderable, int renderTargetSampleCount,
