...
public:
     ...
     */
    sk_sp<GrTexture> createTexture(const GrSurfaceDesc3&, SkBudgeted,
                                   const GrMipLevel texels[], int mipLevelCount);

    /**
     * Simplified createTexture() interface for when there is no initial texel data to upload.
     */
    sk_sp<GrTexture> createTexture(const GrSurfaceDesc3& desc, SkBudgeted budgeted);
...
private:
    ...
    virtual sk_sp<GrTexture> onCreateTexture(const GrSurfaceDesc3& desc,
                                             SkBudgeted budgeted,
