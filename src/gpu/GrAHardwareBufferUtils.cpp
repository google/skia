...
public:
    GLCleanupHelper(GrGLuint texID, EGLImageKHR image, EGLDisplay display, GrGLuint texTarget)
        : fTexID(texID)
        , fImage(image)
        , fDisplay(display)
...
void GLCleanupHelper::rebind(GrContext* grContext) {
...
static GrBackendTexture make_gl_backend_texture(
    ...
    *imageCtx = new GLCleanupHelper(texID, image, display, target);
