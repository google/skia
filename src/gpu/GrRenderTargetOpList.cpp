...
GrRenderTargetOpList::OpChain::Chain GrRenderTargetOpList::OpChain::DoConcat(
        Chain chainA, Chain chainB, const GrCaps& caps, GrOpMemoryPool* pool,
...
GrRenderTargetOpList::OpChain::TryConcat(Chain chainA, const DstProxy& dstProxyA,
    ...
    // All returns use explicit tuple constructor rather than {a, b} to workaround old GCC bug.
    if (chainA.head()->classID() != chainB.head()->classID()) {
        return std::tuple<Chain, Chain>(std::move(chainA), std::move(chainB));
    }
    ...
    do {
        switch (chainA.tail()->combineIfPossible(chainB.head(), caps)) {
            ...
            case GrOp::CombineResult::kMayChain:
                ...
                return std::tuple<Chain, Chain>(std::move(chainA), Chain());
...
bool GrRenderTargetOpList::OpChain::prependChain(OpChain* that, const GrCaps& caps,
                                                 GrOpMemoryPool* pool, GrAuditTrail* auditTrail) {
    std::tie(that->fChain, fChain) = TryConcat(
            std::move(that->fChain), that->dstProxy(), that->appliedClip(), std::move(fChain),
...
std::unique_ptr<GrOp> GrRenderTargetOpList::OpChain::appendOp(std::unique_ptr<GrOp> op,
    ...
    std::tie(fChain, chain) =
            TryConcat(std::move(fChain), this->dstProxy(), fAppliedClip, std::move(chain),
                      *dstProxy, applidClip, caps, pool, auditTrail);
    fChain.validate();
    if (!chain.isEmpty()) {
