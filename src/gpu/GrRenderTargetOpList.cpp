...
void GrRenderTargetOpList::forwardCombine(const GrCaps& caps) {
    ...
    for (int i = 0; i < fRecordedOps.count() - 1; ++i) {
        ...
        if (op->isChained()) {
            // If this op is in a chain then it will draw when the head of the chain draws. That
            // means we would have to check bounds from the head to the target for merging or
            // chaining of 'op', which the loop below does not do.
            continue;
        }
