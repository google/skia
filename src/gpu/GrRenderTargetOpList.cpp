...
void GrRenderTargetOpList::discard() {
    ...
    if (this->isEmpty()) {
        ...
        this->setStencilLoadOp(GrLoadOp::kDiscard);
    }
}
...
void GrRenderTargetOpList::fullClear(const GrCaps& caps, GrColor color) {
    ...
    // may a prior op that writes to the stencil buffer. Although the clear will ignore the
