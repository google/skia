...
void GrRenderTargetOpList::addDrawOp(GrRenderTargetContext* renderTargetContext,
    ...
    op->pipeline()->addDependenciesTo(fSurface);
#endif
    // We only need to retain the clip if it actually clips something.
...
GrOp* GrRenderTargetOpList::recordOp(std::unique_ptr<GrOp> op,
    ...
    if (maxCandidates && renderTarget && clipIdx < 0) {
        ...
        while (true) {
            ...
            if (candidate.fClipIdx < 0 &&
                    candidate.fOp->combineIfPossible(op.get(), *this->caps(), nullptr)) {
        ...
        while (true) {
            ...
            if (j == i +1) {
                ...
                SkASSERT(!op->combineIfPossible(candidate.fOp.get(), *this->caps(), nullptr));
                    ...
                    op->combineIfPossible(candidate.fOp.get(), *this->caps(), nullptr)) {
                GrOP_INFO("\t\tCombining with (%s, B%u)\n", candidate.fOp->name(),
                          candidate.fOp->uniqueID());
