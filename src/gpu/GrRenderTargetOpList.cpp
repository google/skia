...
inline GrRenderTargetOpList::OpChain::Chain::Chain(std::unique_ptr<GrOp> op)
        : fHead(std::move(op)), fTail(fHead.get()) {
    this->validate();
}

inline GrRenderTargetOpList::OpChain::Chain::Chain(Chain&& that) { *this = std::move(that); }

inline GrRenderTargetOpList::OpChain::Chain& GrRenderTargetOpList::OpChain::Chain::operator=(
        Chain&& that) {
...
inline std::unique_ptr<GrOp> GrRenderTargetOpList::OpChain::Chain::popHead() {
    SkASSERT(fHead);
...
inline std::unique_ptr<GrOp> GrRenderTargetOpList::OpChain::Chain::removeOp(GrOp* op) {
#ifdef SK_DEBUG
    auto head = op;
...
inline void GrRenderTargetOpList::OpChain::Chain::pushHead(std::unique_ptr<GrOp> op) {
    SkASSERT(op);
...
inline void GrRenderTargetOpList::OpChain::Chain::pushTail(std::unique_ptr<GrOp> op) {
    SkASSERT(op->isChainTail());
    fTail->chainConcat(std::move(op));
    fTail = fTail->nextInChain();
}

inline void GrRenderTargetOpList::OpChain::Chain::validate() {
#ifdef SK_DEBUG
    if (fHead) {
        SkASSERT(fTail);
...
GrRenderTargetOpList::OpChain::OpChain(std::unique_ptr<GrOp> op, GrAppliedClip* appliedClip,
        ...
        : fChain{std::move(op)}, fAppliedClip(appliedClip) {
    if (dstProxy) {
        fDstProxy = *dstProxy;
    }
    fBounds = fChain.head()->bounds();
}
...
void GrRenderTargetOpList::OpChain::visitProxies(const GrOp::VisitProxyFunc& func,
    ...
    for (const auto& op : GrOp::ChainRange<>(fChain.head())) {
...
void GrRenderTargetOpList::OpChain::deleteOps(GrOpMemoryPool* pool) {
    while (!fChain.isEmpty()) {
        pool->release(fChain.popHead());
    }
}
...
GrRenderTargetOpList::OpChain::Chain GrRenderTargetOpList::OpChain::DoConcat(Chain chainA,
                                                                             Chain chainB,
                                                                             const GrCaps& caps,
                                                                             GrOpMemoryPool* pool) {
    do {
        bool merged = false;
        bool canBackwardMerge = true;
        SkRect forwardMergeBounds = SkRectPriv::MakeLargestInverted();
        GrOp* a = chainA.tail();
        while (a) {
        ...
        if (!merged) {
            ...
            // There is a slight inefficiency here now that a's tail is pointing is a node that came
            // from b. Presumably, we already tried to merge nodes from b into the new tail and
            // yet our loop starts from a's tail and moves towards the head when attempting to find
            // a merge for bHead. To address this we'd have to keep track of an original a tail
            // which would move forward if it merges back into b's head and also ensure we do bounds
            // checks against the nodes at the end of chain a even if we don't try to merge into
            // them. Keeping it a little simpler for now.
        }
    } while (!chainB.isEmpty());
    return chainA;
}
...
std::tuple<GrRenderTargetOpList::OpChain::Chain, GrRenderTargetOpList::OpChain::Chain>
GrRenderTargetOpList::OpChain::TryConcat(Chain chainA, const DstProxy& dstProxyA,
                                         const GrAppliedClip* appliedClipA, Chain chainB,
                                         const DstProxy& dstProxyB,
                                         const GrAppliedClip* appliedClipB, const GrCaps& caps,
                                         GrOpMemoryPool* pool) {
    SkASSERT(!chainA.isEmpty());
    SkASSERT(!chainB.isEmpty());
    if (chainA.head()->classID() != chainB.head()->classID()) {
        return {std::move(chainA), std::move(chainB)};
    }
    ...
    do {
        switch (chainA.tail()->combineIfPossible(chainB.head(), caps)) {
            ...
            case GrOp::CombineResult::kMayChain:
                chainA = DoConcat(std::move(chainA), std::move(chainB), caps, pool);
                return {std::move(chainA), Chain()};
...
bool GrRenderTargetOpList::OpChain::prependChain(OpChain* that, const GrCaps& caps,
                                                 GrOpMemoryPool* pool) {
    std::tie(that->fChain, fChain) =
            TryConcat(std::move(that->fChain), that->dstProxy(), that->appliedClip(),
                      std::move(fChain), this->dstProxy(), this->appliedClip(), caps, pool);
    if (!fChain.isEmpty()) {
        // append failed
        return false;
    }
    // 'that' owns the combined chain. Move it into this.
    fChain = std::move(that->fChain);
...
std::unique_ptr<GrOp> GrRenderTargetOpList::OpChain::appendOp(std::unique_ptr<GrOp> op,
    ...
    Chain chain(std::move(op));
    std::tie(fChain, chain) = TryConcat(std::move(fChain), this->dstProxy(), fAppliedClip,
                                        std::move(chain), *dstProxy, applidClip, caps, pool);
    fChain.validate();
    if (!chain.isEmpty()) {
