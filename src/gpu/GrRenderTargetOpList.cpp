...
bool GrRenderTargetOpList::OpChain::tryConcat(
        List* list, const SkRect& listBounds, const DstProxy& dstProxy,
        const GrAppliedClip* appliedClip, const GrCaps& caps, GrOpMemoryPool* pool,
        GrAuditTrail* auditTrail) {
    ...
    fBounds.joinPossiblyEmptyRect(listBounds);
    return true;
}
...
bool GrRenderTargetOpList::OpChain::prependChain(OpChain* that, const GrCaps& caps,
    ...
    if (!that->tryConcat(
            &fList, fBounds, this->dstProxy(), this->appliedClip(), caps, pool, auditTrail)) {
        this->validate();
        // append failed
        return false;
    }
...
std::unique_ptr<GrOp> GrRenderTargetOpList::OpChain::appendOp(std::unique_ptr<GrOp> op,
    ...
    if (!this->tryConcat(&chain, opBounds, *dstProxy, appliedClip, caps, pool, auditTrail)) {
