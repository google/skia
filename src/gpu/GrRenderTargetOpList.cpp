...
void GrRenderTargetOpList::fullClear(GrColor color) {
...
GrOp* GrRenderTargetOpList::recordOp(std::unique_ptr<GrOp> op,
    ...
    if (maxCandidates) {
        ...
        while (true) {
            ...
            if (this->combineIfPossible(candidate, op.get(), clip, dstTexture, *fCaps)) {
    ...
    for (int i = 0; i < fRecordedOps.count() - 1; ++i) {
        ...
        while (true) {
            ...
            if (this->combineIfPossible(fRecordedOps[i], candidate.fOp.get(),
                                        candidate.fAppliedClip, &candidate.fDstTexture, *fCaps)) {
