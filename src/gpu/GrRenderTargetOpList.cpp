...
bool GrRenderTargetOpList::executeOps(GrOpFlushState* flushState) {
    ...
    for (int i = 0; i < fRecordedOps.count(); ++i) {
        ...
        const GrAppliedClip* clip = fRecordedOps[i].fClipIdx >= 0 ? &fAppliedClips[i] : nullptr;
        fRecordedOps[i].fOp->execute(flushState, fRecordedOps[i].fClippedBounds, clip);
    }
    if (commandBuffer) {
        commandBuffer->end();
        commandBuffer->submit();
...
GrOp* GrRenderTargetOpList::recordOp(std::unique_ptr<GrOp> op,
    ...
    if (maxCandidates && !renderTargetID.isInvalid() && clipIdx < 0) {
        int i = 0;
        while (true) {
    ...
    GR_AUDIT_TRAIL_OP_RESULT_NEW(fAuditTrail, op);
        fRecordedOps.emplace_back(RecordedOp{std::move(op), clippedBounds, renderTargetID, clipIdx});
    fLastFullClearOp = nullptr;
    fLastFullClearRenderTargetID.makeInvalid();
...
void GrRenderTargetOpList::forwardCombine() {
    ...
    for (int i = 0; i < fRecordedOps.count() - 2; ++i) {
        ...
        // If we don't have a valid destination render target ID then we cannot reorder.
        if (renderTargetID.isInvalid() || fRecordedOps[i].fClipIdx >= 0) {
            continue;
        }
        ...
        while (true) {
            ...
            } else if (candidate.fClipIdx < 0 &&
                       op->combineIfPossible(candidate.fOp.get(), *this->caps())) {
                GrOP_INFO("\t\tCombining with (%s, B%u)\n", candidate.fOp->name(),
                          candidate.fOp->uniqueID());
