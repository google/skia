...
bool GrRenderTargetOpList::OpChain::tryConcat(
        ...
        const SkRect& listBounds, const GrCaps& caps, GrOpMemoryPool* pool,
        GrAuditTrail* auditTrail) {
    ...
    // The new ops were successfully merged and/or chained onto ours.
...
bool GrRenderTargetOpList::OpChain::prependChain(OpChain* that, const GrCaps& caps,
    ...
    if (!that->tryConcat(
            &fList, this->dstProxy(), this->appliedClip(), fBounds, caps, pool, auditTrail)) {
        this->validate();
        // append failed
        return false;
    }
