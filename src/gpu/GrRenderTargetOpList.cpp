...
bool GrRenderTargetOpList::onExecute(GrOpFlushState* flushState) {
    ...
    // discard and then store the data back in memmory so that the load op on future draws doesn't
    ...
    // loading or discarding. In this world we don't need to worry about executing oplists with no
    // ops just to do a discard.
