...
bool GrRenderTargetOpList::copySurface(GrSurface* dst,
    ...
    // force the caller to end the previous command buffer and execute this copy before beginning a
    // new one.
...
GrOp* GrRenderTargetOpList::recordOp(sk_sp<GrOp> op, GrRenderTargetContext* renderTargetContext,
    ...
    GrGpuResource::UniqueID renderTargetID = renderTargetContext ? renderTargetContext->accessRenderTarget()->uniqueID() : GrGpuResource::UniqueID::InvalidID();
