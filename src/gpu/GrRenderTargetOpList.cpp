...
void GrRenderTargetOpList::init(sk_sp<GrRenderTargetProxy> proxy, GrGpu* gpu,
                                GrAuditTrail* auditTrail) {
    INHERITED::init(std::move(proxy), auditTrail);

    SkASSERT(!fLastFullClearOp);
    SkASSERT(GrGpuResource::UniqueID::InvalidID() == fLastFullClearResourceID);
    SkASSERT(GrSurfaceProxy::UniqueID::InvalidID() == fLastFullClearProxyID);
    SkASSERT(SK_InvalidUniqueID == fLastClipStackGenID);
    fNumClips = 0;
...
void GrRenderTargetOpList::reset1() {
