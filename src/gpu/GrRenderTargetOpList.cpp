...
void GrRenderTargetOpList::addDrawOp(const GrPipelineBuilder& pipelineBuilder,
    ...
    if (analysis.usesPLSDstRead()) {
        ...
        SkIRect ibounds;
        ibounds.fLeft = SkTPin(SkScalarFloorToInt(op->boundsX().fLeft), 0, width);
        ibounds.fTop = SkTPin(SkScalarFloorToInt(op->boundsX().fTop), 0, height);
        ibounds.fRight = SkTPin(SkScalarCeilToInt(op->boundsX().fRight), 0, width);
        ibounds.fBottom = SkTPin(SkScalarCeilToInt(op->boundsX().fBottom), 0, height);
