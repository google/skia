...
void GrRenderTargetOpList::OpChain::visitProxies(const GrSurfaceProxy::VisitProxyFunc& func) const {
    if (fList.empty()) {
        return;
    }
...
void GrRenderTargetOpList::gatherProxyIntervals(GrResourceAllocator* alloc) {
    ...
    auto gather = [ alloc, msaaResolvesAutomatically SkDEBUGCODE(, this) ] (
