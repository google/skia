...
GrTextureProducer::DomainMode GrTextureProducer::DetermineDomainMode(
                                ...
                                const GrSamplerParams::FilterMode* filterModeOrNullForBicubic,
                                SkRect* domainRect) {
    const SkRect proxyBounds = SkRect::MakeIWH(proxy->width(), proxy->height());

    SkASSERT(proxyBounds.contains(constraintRect));
    // We only expect a content area rect if there is some non-content area.
    SkASSERT(!contentRect ||
             (!contentRect->contains(SkIRect::MakeWH(proxy->width(), proxy->height())) &&
              proxyBounds.contains(*contentRect) &&
              SkRect::Make(*contentRect).contains(constraintRect)));

    const bool proxyIsExact = GrResourceProvider::IsFunctionallyExact(proxy);

    SkIRect foo;
    if (!contentRect && !proxyIsExact) {
        foo = SkIRect::MakeWH(proxy->width(), proxy->height());
        contentRect = &foo;
    }
    ...
    // don't need a domain.
    if (!restrictFilterToRect && !contentRect && proxyIsExact && coordsLimitedToConstraintRect) {
        return kNoDomain_DomainMode;
    }
    ...
    if (filterModeOrNullForBicubic) {
        switch (*filterModeOrNullForBicubic) {
            ...
            case GrSamplerParams::kBilerp_FilterMode:
                ...
                break;
            case GrSamplerParams::kMipMap_FilterMode:
                if (restrictFilterToRect || contentRect || !proxyIsExact) {
                    // No domain can save us here.
                    return kTightCopy_DomainMode;
                }
                return kNoDomain_DomainMode;
