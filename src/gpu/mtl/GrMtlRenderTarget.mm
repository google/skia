...
GrMtlRenderTarget::MakeWrappedRenderTarget(GrMtlGpu* gpu, const GrSurfaceDesc& desc,
    ...
    SkASSERT(1 == textureInfo.fLevelCount);

    id<MTLTexture> renderTexture = TransferTexture(textureInfo.fTexture,
...
GrBackendRenderTarget GrMtlRenderTarget::getBackendRenderTarget() const {
    ...
    ExtractMTLTextureInfo(fRenderTexture, &textureInfo);
    return GrBackendRenderTarget(textureInfo);
}
