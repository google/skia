...
/**
 * Returns a sk_cfp<id<MTLTexture>> to the MTLTexture pointed at by the const void*.
 * Will retain the MTLTexture.
 */
SK_ALWAYS_INLINE sk_cfp<id<MTLTexture>> GrRetainMTLTexture(const void* mtlTexture) {
    sk_cfp<id<MTLTexture>> texture(sk_ret_cfp((id<MTLTexture>)mtlTexture));
    return texture;
}
 ...
 */
sk_cfp<MTLTextureDescriptor*> GrGetMTLTextureDescriptor(id<MTLTexture> mtlTexture);

/**
 * Returns a compiled MTLLibrary created from MSL code generated by SkSLC
 */
sk_cfp<id<MTLLibrary>> GrGenerateMtlShaderLibrary(const GrMtlGpu* gpu,
...
*/
sk_cfp<id<MTLLibrary>> GrCompileMtlShaderLibrary(const GrMtlGpu* gpu,
