...
std::unique_ptr<GrMtlSemaphore> GrMtlSemaphore::Make(GrMtlGpu* gpu) {
    if (@available(macOS 10.14, iOS 12.0, *)) {
        sk_cf_obj<id<MTLEvent>> event([gpu->device() newEvent]);
...
std::unique_ptr<GrMtlSemaphore> GrMtlSemaphore::MakeWrapped(GrMTLHandle event,
    ...
    if (@available(macOS 10.14, iOS 12.0, *)) {
        sk_cf_obj<id<MTLEvent>> mtlEvent((id<MTLEvent>)event);
...
GrMtlSemaphore::GrMtlSemaphore(sk_cf_obj<id<MTLEvent>> event, uint64_t value)
...
GrBackendSemaphore GrMtlSemaphore::backendSemaphore() const {
    ...
    if (@available(macOS 10.14, iOS 12.0, *)) {
        GrMTLHandle handle = (GrMTLHandle)([fEvent.get() retain]);
