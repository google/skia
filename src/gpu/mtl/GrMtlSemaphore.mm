...
std::unique_ptr<GrMtlSemaphore> GrMtlSemaphore::Make(GrMtlGpu* gpu) {
    if (@available(macOS 10.14, iOS 12.0, *)) {
        sk_cfp<GrMTLHandle> event(static_cast<GrMTLHandle>([gpu->device() newEvent]));
...
std::unique_ptr<GrMtlSemaphore> GrMtlSemaphore::MakeWrapped(GrMTLHandle event,
    ...
    if (@available(macOS 10.14, iOS 12.0, *)) {
        sk_cfp<GrMTLHandle> mtlEvent(event);
...
GrMtlSemaphore::GrMtlSemaphore(sk_cfp<GrMTLHandle> event, uint64_t value)
