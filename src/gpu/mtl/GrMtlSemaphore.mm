...
std::unique_ptr<GrMtlSemaphore> GrMtlSemaphore::Make(GrMtlGpu* gpu) {
    if (@available(macOS 10.14, iOS 12.0, *)) {
        ...
        return std::unique_ptr<GrMtlSemaphore>(new GrMtlSemaphore(gpu, event, value));
    } else {
        return nullptr;
    }
}
...
std::unique_ptr<GrMtlSemaphore> GrMtlSemaphore::MakeWrapped(GrMtlGpu* gpu,
    ...
    if (@available(macOS 10.14, iOS 12.0, *)) {
        ...
        return std::unique_ptr<GrMtlSemaphore>(new GrMtlSemaphore(gpu, mtlEvent, value));
    } else {
        return nullptr;
    }
}

GrMtlSemaphore::GrMtlSemaphore(GrMtlGpu* gpu, id<MTLEvent> event, uint64_t value)
