...
public:
    ...
    id<MTLEvent> event() const SK_MTL_API_AVAILABLE(macos(10.14), ios(12.0)) { return fEvent; }
    uint64_t value() const { return fValue; }

    GrBackendSemaphore backendSemaphore() const override;

private:
    GrMtlSemaphore(id<MTLEvent> event, uint64_t value) SK_MTL_API_AVAILABLE(macos(10.14), ios(12.0));

    void setIsOwned() override {}

    id<MTLEvent> fEvent SK_MTL_API_AVAILABLE(macos(10.14), ios(12.0));
    uint64_t     fValue;
