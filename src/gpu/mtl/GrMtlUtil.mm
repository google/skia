...
// Wrapper to get atomic assignment compatible with __block
@interface AtomicMtlObject : NSObject
@property (atomic, strong) id object;
@end
@implementation AtomicMtlObject
@synthesize object;
@end
...
id<MTLLibrary> GrMtlNewLibraryWithSource(id<MTLDevice> device, NSString* mslCode,
    ...
    __block sk_sp<MtlCompileResult> compileResult(new MtlCompileResult);
    ...
    // Wait 10 seconds for the compiler
    if (dispatch_semaphore_wait(semaphore, dispatch_time(DISPATCH_TIME_NOW, 1000UL))) {
    ...
    std::tie(compiledLibrary, compileError) = compileResult->get();
    if (compileError) {
        SkDebugf("Error compiling MSL shader: %s\n%s\n",
            mslCode,
            [[compileError localizedDescription] cStringUsingEncoding: NSASCIIStringEncoding]);
    }
...
//                                         MTLCompileOptions* options) {
//    dispatch_semaphore_t semaphore = dispatch_semaphore_create(0);
//    __block AtomicMtlObject* compiledLibrary = [AtomicMtlObject new];
//    MTLNewLibraryCompletionHandler completionHandler =
//            ^(id<MTLLibrary> library, NSError* error) {
//                compiledLibrary.object = library;
//                dispatch_semaphore_signal(semaphore);
//                if (error) {
//                    SkDebugf("Error compiling MSL shader: %s\n%s\n",
//                        mslCode,
...
//
//    // Wait 10 seconds for the compiler
//    if (dispatch_semaphore_wait(semaphore, dispatch_time(DISPATCH_TIME_NOW, 10000000UL))) {
//        SkDebugf("Timeout compiling MSL shader\n");
//        return nil;
//    }
//
//    return (id<MTLLibrary>)compiledLibrary.object;
//}
...
id<MTLRenderPipelineState> GrMtlNewRenderPipelineStateWithDescriptor(
    ...
    // Wait 10 seconds for pipeline creation
    if (dispatch_semaphore_wait(semaphore, dispatch_time(DISPATCH_TIME_NOW, 10000000UL))) {
        SkDebugf("Timeout creating pipeline.\n");
        return nil;
    }

    return (id<MTLRenderPipelineState>)pipelineState.object;
}
