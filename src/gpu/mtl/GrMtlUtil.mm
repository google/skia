...
// std::atomic<> doesn't work well with block__, so we wrap this in an object
@interface AtomicBool : NSObject
...
id<MTLLibrary> GrMtlNewLibraryWithSource(id<MTLDevice> device, NSString* mslCode,
    ...
    __block AtomicBool* timedout = [[AtomicBool alloc] initWithValue:false];
