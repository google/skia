...
GrMtlTexture::GrMtlTexture(GrMtlGpu* gpu,
    ...
    SkASSERT((GrMipmapStatus::kNotAllocated == mipmapStatus) ==
             (1 == fTexture->mtlTexture().mipmapLevelCount));
    if (@available(macOS 10.11, iOS 9.0, *)) {
        SkASSERT(SkToBool(fTexture->mtlTexture().usage & MTLTextureUsageShaderRead));
    }
    SkASSERT(!fTexture->mtlTexture().framebufferOnly);
