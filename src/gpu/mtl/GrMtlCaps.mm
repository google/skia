...
void GrMtlCaps::init(const GrContextOptions& contextOptions, const id<MTLDevice> device) {
    this->initFeatureSet(device);
    this->initGrCaps(device);
    this->initShaderCaps();
    this->initConfigTable();
}

void GrMtlCaps::initFeatureSet(const id<MTLDevice> device) {
    // Mac OSX
...
void GrMtlCaps::initGrCaps(const id<MTLDevice> device) {
    ...
    fSRGBWriteControl = false; // No sure what this is for
...
int GrMtlCaps::getSampleCount(int requestedCount, GrPixelConfig config) const {
    ...
    SkASSERT(count > 0); // We always 0 as a valid sample count
...
void GrMtlCaps::initShaderCaps() {
    ...
    // With assumption this cap eventually means we support varying percisions and not just via
    // modifiers.
