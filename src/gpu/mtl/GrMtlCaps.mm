...
bool GrMtlCaps::canCopyAsResolve(GrSurface* dst,
    ...
    return this->canCopyAsResolve(dstFormat, dstSampleCount, srcFormat, srcSampleCount, srcIsRenderTarget, srcSize, srcRect, dstPoint, src == dst);
}
...
bool GrMtlCaps::onCanCopySurface(const GrSurfaceProxy* dst, const GrSurfaceProxy* src,
    ...
    if (this->canCopyAsResolve(GrBackendFormatAsMTLPixelFormat(dst->backendFormat()), dstSampleCnt,
                               GrBackendFormatAsMTLPixelFormat(src->backendFormat()), srcSampleCnt,
                               srcIsRenderTarget,
...
GrCaps::SurfaceReadPixelsSupport GrMtlCaps::surfaceSupportsReadPixels(const GrSurface* surface) const {
...
GrCaps::DstCopyRestrictions GrMtlCaps::getDstCopyRestrictions(const GrRenderTargetProxy* src,
    ...
    // If the src is a RT with only a MSAA buffer then our only supported copy mode is
    // resolve. Currently we only support resolving the entire texture.
    DstCopyRestrictions restrictions = {};
    if (auto mtlRT = static_cast<const GrMtlRenderTarget*>(src->peekRenderTarget())) {
        if (mtlRT->numSamples() > 1 && mtlRT->mtlColorTexture() && !mtlRT->mtlResolveTexture()) {
