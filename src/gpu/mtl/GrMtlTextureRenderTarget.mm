...
sk_cfp<id<MTLTexture>> create_msaa_texture(GrMtlGpu* gpu, SkISize dimensions,
    ...
    return sk_cfp<id<MTLTexture>>([gpu->device() newTextureWithDescriptor:texDesc.get()]);
}
...
sk_sp<GrMtlTextureRenderTarget> GrMtlTextureRenderTarget::MakeNewTextureRenderTarget(
        ...
        GrMipmapStatus mipmapStatus) {
    sk_cfp<id<MTLTexture>> texture([gpu->device() newTextureWithDescriptor:texDesc]);
    if (!texture) {
        return nullptr;
    }
    if (@available(macOS 10.11, iOS 9.0, *)) {
