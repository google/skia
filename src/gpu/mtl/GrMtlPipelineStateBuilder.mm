...
static MTLRenderPipelineColorAttachmentDescriptor* create_color_attachment(
        MTLPixelFormat format1, const GrPipeline& pipeline) {
    auto mtlColorAttachment = [[MTLRenderPipelineColorAttachmentDescriptor alloc] init];

    // pixel format
    mtlColorAttachment.pixelFormat = format1;
...
GrMtlPipelineState* GrMtlPipelineStateBuilder::finalize(GrRenderTarget* renderTarget,
    ...
    GrMTLPixelFormat pixelFormat = renderTarget->backendFormat().asMtlFormat();

    {
        MTLPixelFormat testFormat;
        SkAssertResult(GrPixelConfigToMTLFormat(renderTarget->config(), &testFormat));
        SkAssertResult(pixelFormat == testFormat);
    }
