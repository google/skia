...
void GrMtlPipelineStateBuilder::storeShadersInCache(const SkSL::String shaders[],
    ...
    if (isSkSL) {
        ...
        GrPersistentCacheUtils::PackCachedShaders(kSKSL_Tag, shaders, inputs, kGrShaderTypeCount,
                                                  &meta);
    } else {
        GrPersistentCacheUtils::PackCachedShaders(kMSL_Tag, shaders, inputs, kGrShaderTypeCount);
    }
...
bool GrMtlPipelineStateBuilder::PrecompileShaders(GrMtlGpu* gpu, const SkData& cachedData) {
    ...
    SkSL::Program::Inputs inputs[kGrShaderTypeCount];
    if (!GrPersistentCacheUtils::UnpackCachedShaders(&reader, shaders, inputs, GrShaderTypeCount,
                                                     &meta)) {
        return false;
    }
