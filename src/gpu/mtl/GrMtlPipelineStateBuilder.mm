...
bool GrMtlPipelineStateBuilder::loadShadersFromCache(SkReadBuffer* cached,
                                                     sk_cfp<id<MTLLibrary>> outLibraries[]) {
...
sk_cfp<id<MTLLibrary>> GrMtlPipelineStateBuilder::generateMtlShaderLibrary(
    ...
    sk_cfp<id<MTLLibrary>> shaderLibrary = GrGenerateMtlShaderLibrary(fGpu, shader, kind,
                                                                         settings, msl, inputs);
...
sk_cfp<id<MTLLibrary>> GrMtlPipelineStateBuilder::compileMtlShaderLibrary(
    ...
    sk_cfp<id<MTLLibrary>> shaderLibrary = GrCompileMtlShaderLibrary(fGpu, shader);
    if (shaderLibrary != nil && inputs.fRTHeight) {
        this->addRTHeightUniform(SKSL_RTHEIGHT_NAME);
    }
    return shaderLibrary;
