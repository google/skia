...
void GrMtlPipelineStateBuilder::storeShadersInCache(const SkSL::String shaders[],
    ...
    if (isSkSL) {
        ...
        data = GrPersistentCacheUtils::PackCachedShaders(kSKSL_Tag, shaders, inputs, kGrShaderTypeCount,
                                                  &meta);
    } else {
        data = GrPersistentCacheUtils::PackCachedShaders(kMSL_Tag, shaders, inputs, kGrShaderTypeCount);
    }
