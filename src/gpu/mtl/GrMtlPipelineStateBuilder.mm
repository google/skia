...
static MTLRenderPipelineColorAttachmentDescriptor* create_color_attachment(
        const GrRenderTarget* renderTarget, const GrPipeline& pipeline) {
    auto mtlColorAttachment = [[MTLRenderPipelineColorAttachmentDescriptor alloc] init];

    // pixel format
    MTLPixelFormat format;
    SkAssertResult(GrPixelConfigToMTLFormat(renderTarget->config(), &format));
...
GrMtlPipelineState* GrMtlPipelineStateBuilder::finalize(const GrPrimitiveProcessor& primProc,
    ...
    pipelineDescriptor.colorAttachments[0] = create_color_attachment(this->renderTarget(), pipeline);
