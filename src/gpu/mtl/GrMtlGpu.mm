...
sk_sp<GrTexture> GrMtlGpu::onCreateCompressedTexture(SkISize dimensions,
    ...
    SkImage::CompressionType textureCompressionType = GrMtlFormatToCompressionType(mtlTexture.pixelFormat);
    if (textureCompressionType == SkImage::CompressionType::kNone) {
        return nullptr;
    }
    ...
    id<MTLBlitCommandEncoder> blitCmdEncoder = this->commandBuffer()->getBlitCommandEncoder();
    const size_t rowBytes = GrCompressedRowBytes(textureCompressionType, dimensions.width());
