...
bool GrMtlGpu::uploadToTex(GrMtlTexture* tex, int left, int top, int width, int height,
...
sk_sp<GrTexture> GrMtlGpu::onCreateTexture(const GrSurfaceDesc& desc, SkBudgeted budgeted,
    ...
    if (mipLevelCount) {
        if (!this->uploadToTex(tex.get(), 0, 0, desc.fWidth, desc.fHeight, colorType, texels,
                               mipLevelCount)) {
            tex->unref();
            return nullptr;
        }
...
bool GrMtlGpu::createTestingOnlyMtlTextureInfo(GrPixelConfig config, int w, int h, bool texturable,
    ...
    id<MTLBlitCommandEncoder> blitCmdEncoder = [fCmdBuffer blitCommandEncoder];
    [transferTexture replaceRegion: MTLRegionMake2D(0, 0, w, h)
                       mipmapLevel: 0
                         withBytes: srcData
                       bytesPerRow: rowBytes];
