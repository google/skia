...
void GrMtlGpu::submitCommandBuffer(SyncQueue sync) {
    ...
    [fCurrentCmdBuffer commit];
    if (SyncQueue::kForce_SyncQueue == sync) {
        [fCurrentCmdBuffer waitUntilCompleted];
    }
    fCurrentCmdBuffer = [fDevice commandBuffer];
}
...
bool GrMtlGpu::onReadPixels(GrSurface* surface, int left, int top, int width, int height,
    ...
    SkASSERT(surface);
    if (GrPixelConfigToColorType(surface->confg()) != dstColorType) {
        return false;
    }
