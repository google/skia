...
sk_sp<GrGpu> GrMtlGpu::Make(const GrMtlBackendContext& context, const GrContextOptions& options,
    ...
    //**** need to fix binary archive
    return sk_sp<GrGpu>(new GrMtlGpu(direct, options, std::move(device),
                                     std::move(queue), context.fBinaryArchive.get(),
                                     featureSet));
}
...
GrMtlGpu::GrMtlGpu(GrDirectContext* direct, const GrContextOptions& options,
    ...
    if (@available(macOS 11.0, iOS 14.0, *)) {
        //*** check refcounts here
        fBinaryArchive.reset((id<MTLBinaryArchive>)(binaryArchive));
    }
#endif
}
