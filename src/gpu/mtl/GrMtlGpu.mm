...
bool GrMtlGpu::copySurfaceAsDrawThenBlit(GrSurface* dst, GrSurfaceOrigin dstOrigin,
    ...
    SkIRect transferSrcRect = SkIRect::MakeXYWH(0, 0, srcRect.width(), srcRect.height());
...
void GrMtlGpu::findOrCreateCopyProgramPipelineState(id<MTLTexture> dstTex) {
    // Find or create pipeline state for copy as draw
    ...
    if (fCopyProgramPipelineState == nil) {
        ...
        fCopyProgramPipelineState =
                [fDevice newRenderPipelineStateWithDescriptor: pipelineDescriptor
                                                        error: &error];
        if (error) {
...
void GrMtlGpu::createCopyProgramSamplerState() {
...
void GrMtlGpu::createCopyProgramBuffer() {
...
bool GrMtlGpu::createCopyProgram(id<MTLTexture> dstTex) {
    ...
    this->findOrCreateCopyProgramPipelineState(dstTex);
    if (fCopyProgramPipelineState == nil) {
        return false;
    }

    if (fCopyProgramSamplerState == nil) {
        this->createCopyProgramSamplerState();
    }
