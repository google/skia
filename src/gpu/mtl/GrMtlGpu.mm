...
bool GrMtlGpu::createMtlTextureForBackendSurface(MTLPixelFormat mtlFormat,
    ...
    desc.retain([MTLTextureDescriptor texture2DDescriptorWithPixelFormat: mtlFormat
    ...
    info->fTexture.reset([*fDevice newTextureWithDescriptor:desc.get()]);
    return true;
}
...
GrFence SK_WARN_UNUSED_RESULT GrMtlGpu::insertFence() {
    ...
    const void* cfFence = (const void*)[*semaphore retain];
    return (GrFence) cfFence;
}

bool GrMtlGpu::waitFence(GrFence fence) {
    const void* cfFence = (const void*) fence;
    sk_cf_obj<dispatch_semaphore_t> semaphore;
    semaphore.retain((dispatch_semaphore_t)cfFence);

    long result = dispatch_semaphore_wait(semaphore.get(), 0);
