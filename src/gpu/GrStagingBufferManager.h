...
public:
    ...
    void detachActiveBuffers() {
        this->onDetachActiveBuffers();
#ifdef SK_DEBUG
        for (size_t i = 0; i < fActiveBuffers.size(); ++i) {
            SkASSERT(!fActiveBuffers[i]);
        }
#endif
...
protected:
    ...
    std::vector<std::unique_ptr<GrStagingBuffer>> fActiveBuffers;

private:
    static constexpr size_t kMinStagingBufferSize = 32 * 1024;

    virtual void onDetachActiveBuffers() = 0;
