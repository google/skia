...
void PrimitiveProcessor::emitVertexShader(const GrCCPRCoverageProcessor& proc,
    ...
    v->codeAppend ("highp float2 atlasoffset = float2((packedoffset<<16) >> 16, packedoffset >> 16);");
...
int PrimitiveProcessor::emitHullGeometry(GrGLSLGeometryBuilder* g, const char* emitVertexFn,
    ...
    if (!insetPts) {
        g->codeAppendf("highp float2 centroidpt = %s * vec%i(%f);",
    ...
    if (3 == numSides) {
        ...
        g->codeAppend ("highp float2 dr = float2(qsr.y != 0 ? +qsr.y : 1, "
                                            "qsr.x != 0 ? -qsr.x : 1);");
        g->codeAppend ("highp float2 dl = (qls == float2(0)) ? dr : float2(qls.y != 0 ? +qls.y : 1, "
                                                                    "qls.x != 0 ? -qls.x : 1);");
    }
...
void PrimitiveProcessor::emitEdgeDistanceEquation(GrGLSLGeometryBuilder* g,
    ...
    g->codeAppendf("highp float2 kk = n * mat2(%s + bloat * d1, %s - bloat * d1);", leftPt, leftPt);
