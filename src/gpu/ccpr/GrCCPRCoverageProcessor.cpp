...
const char* GrCCPRCoverageProcessor::GetProcessorName(Mode mode) {
    switch (mode) {
        case Mode::kAnchorHulls:
            return "GrCCPRTriangleProcessor (hulls)";
        case Mode::kAnchorEdges:
            return "GrCCPRTriangleProcessor (edges)";
        case Mode::kCombinedFanHullsAndEdges:
            return "GrCCPRTriangleProcessor (combined hulls & edges)";
...
GrGLSLPrimitiveProcessor* GrCCPRCoverageProcessor::createGLSLInstance(const GrShaderCaps&) const {
    switch (fMode) {
        using CoverageType = PrimitiveProcessor::CoverageType;
        using GeometryType = GrCCPRTriangleProcessor::GeometryType;

        case Mode::kAnchorHulls:
            return new GrCCPRTriangleProcessor(CoverageType::kOne, GeometryType::kHulls);
        case Mode::kAnchorEdges:
            return new GrCCPRTriangleProcessor(CoverageType::kInterpolated, GeometryType::kEdges);
        case Mode::kCombinedFanHullsAndEdges:
            return new GrCCPRTriangleProcessor(CoverageType::kInterpolated,
        ...
        case Mode::kSerpInset:
            return new GrCCPRCubicInsetProcessor(GrCCPRCubicProcessor::Type::kSerpenitne);
        case Mode::kSerpBorder:
            return new GrCCPRCubicBorderProcessor(GrCCPRCubicProcessor::Type::kSerpenitne);
        case Mode::kLoopInset:
...
void PrimitiveProcessor::onEmitCode(EmitArgs& args, GrGPArgs* gpArgs) {
    const GrCCPRCoverageProcessor& cp = args.fGP.cast<GrCCPRCoverageProcessor>();
    ...
    varyingHandler->emitAttributes(cp);

    this->emitVertexShader(cp, args.fVertBuilder, args.fRTAdjustName, args.fTexelBuffers[0],
                           gpArgs);
    this->emitGeometryShader(cp, args.fGeomBuilder, args.fRTAdjustName);
    this->emitCoverage(cp, args.fFragBuilder, args.fOutputColor, args.fOutputCoverage);

    SkASSERT(!args.fFPCoordTransformHandler->nextCoordTransform());
}

void PrimitiveProcessor::emitVertexShader(const GrCCPRCoverageProcessor& cp, GrGLSLVertexBuilder* v,
                                          const char* rtAdjustName,
                                          const TexelBufferHandle& pointsBuffer,
                                          GrGPArgs* gpArgs) const {
    v->codeAppendf("int packedoffset = %s[0];", cp.instanceAttrib());
    v->codeAppend ("highp vec2 atlasoffset = vec2((packedoffset<<16) >> 16, packedoffset >> 16);");

    this->onEmitVertexShader(cp, v, rtAdjustName, pointsBuffer, gpArgs);
}

void PrimitiveProcessor::emitGeometryShader(const GrCCPRCoverageProcessor& cp,
                                            GrGLSLGeometryBuilder* g,
                                            const char* rtAdjustName) const {
    g->declareGlobal(fGeomWind);
    this->emitWind(g, fGeomWind.c_str(), rtAdjustName);
    ...
    g->codeAppendf("highp vec2 bloat = %f * abs(%s.xz);", kAABloatRadius, rtAdjustName);
...
#ifdef SK_DEBUG
    if (cp.debugVisualizations()) {
        g->codeAppendf("bloat *= %f;", GrCCPRCoverageProcessor::kDebugBloat);
    }
#endif

    return this->onEmitGeometryShader(g, emitVertexFn.c_str(), fGeomWind.c_str(), rtAdjustName);
}

void PrimitiveProcessor::emitWind(GrGLSLGeometryBuilder* g, const char* windName,
                                  const char* /*rtAdjustName*/) const {
...
void PrimitiveProcessor::emitCoverage(const GrCCPRCoverageProcessor& cp, GrGLSLFragmentBuilder* f,
...
int PrimitiveProcessor::emitHullGeometry(GrGLSLGeometryBuilder* g, int numPts, const char* wedgeIdx,
                                         const char* emitVertexFn, const char* hullPts,
                                         const char* insetPts) const {
    SkASSERT(numPts >= 3);

    if (!insetPts) {
        g->codeAppendf("highp vec2 centroidpt = %s * vec%i(%f);", hullPts, numPts, 1.0 / numPts);
    }

    g->codeAppendf("int previdx = (%s + %i) %% %i, "
                       "nextidx = (%s + 1) %% %i;", wedgeIdx, numPts - 1, numPts, wedgeIdx, numPts);

    g->codeAppendf("highp vec2 self = %s[%s];"
                   "highp int leftidx = %s > 0 ? previdx : nextidx;"
                   "highp int rightidx = %s > 0 ? nextidx : previdx;",
                   hullPts, wedgeIdx, fGeomWind.c_str(), fGeomWind.c_str());
    ...
    if (3 == numPts) {
        ...
        SkASSERT(4 == numPts);
        g->codeAppendf("highp vec2 diag = %s[(%s + 2) %% 4];", hullPts, wedgeIdx);
...
void PrimitiveProcessor::emitEdgeDistanceEquation(GrGLSLGeometryBuilder* g,
    ...
    g->codeAppendf("%s = vec3(-n, kk[1]) * scale;", distanceEquationName);
}
...
int PrimitiveProcessor::definePseudoSampleLocations(GrGLSLFragmentBuilder* f,
