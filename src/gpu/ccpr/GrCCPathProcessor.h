...
public:
    struct Instance {
        ...
        SkPMColor4f fColor;  // Color always stored as 4 x fp16

        void set(const GrOctoBounds&, const SkIVector& devToAtlasOffset, SkPMColor4f, GrFillRule);
        void set(const GrCCPathCacheEntry&, const SkIVector& shift, SkPMColor4f, GrFillRule);
    };
...
inline void GrCCPathProcessor::Instance::set(
        const GrOctoBounds& octoBounds, const SkIVector& devToAtlasOffset, SkPMColor4f color,
        GrFillRule fillRule) {
