...
public:
    ...
    public:
        ...
        const SingleDraw* head() const { SkASSERT(fDebugInstanceCount >= 1); return &fHeadDraw; }
        SkDEBUGCODE(int debugSkippedInstances() const { return fDebugSkippedInstances; })
    ...
    // These are keyed by SkPath generation ID, and store which device-space paths are accessed
    // where by clip FPs in a given opList. Many clip FPs can reference a single ClipPath. At flush
    // time their coverage count masks are packed into atlas(es) alongside normal DrawPathOps.
    class ClipPath {
    public:
        ClipPath() = default;
        ...
        ~ClipPath() {
            // Ensure no FPs or proxy lambdas exist that could have dangling pointers into CCPR.
...
private:
    ...
    // Map from render target ID to the individual render target's pending paths.
