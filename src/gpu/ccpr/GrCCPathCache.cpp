...
GrCCPathCache::OnFlushEntryRef GrCCPathCache::find(GrOnFlushResourceProvider* onFlushRP,
    ...
    if (0 == entry->fOnFlushRefCnt) {
        ...
        if (entry->fCachedAtlas) {
            if (!entry->fCachedAtlas->onFlushProxy()) {
                SkASSERT(0 == entry->fCachedAtlas->onFlushRefCnt());
    ...
    SkASSERT(!entry->fCachedAtlas || entry->fCachedAtlas->onFlushProxy());
...
GrCCPathCache::OnFlushEntryRef::~OnFlushEntryRef() {
    ...
    if (fEntry->fCachedAtlas) {
        fEntry->fCachedAtlas->onFlushUnref();
    }
    fEntry->unref();
}
...
void GrCCPathCacheEntry::setCoverageCountAtlas(
    ...
    fCachedAtlas->onFlushRef(fOnFlushRefCnt);
...
GrCCPathCacheEntry::ReleaseAtlasResult GrCCPathCacheEntry::releaseCachedAtlas(
    ...
    if (fCachedAtlas) {
        ...
        if (fOnFlushRefCnt) {
            SkASSERT(fOnFlushRefCnt > 0);
            fCachedAtlas->onFlushUnref(fOnFlushRefCnt);
        }
...
void GrCCCachedAtlas::onFlushUnref(int count) const {
    SkASSERT(count > 0);
