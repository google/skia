...
GrCCPathCache::OnFlushEntryRef GrCCPathCache::find(
    ...
    if (HashNode* node = fHashTable.find(*fScratchKey)) {
        ...
        if (!fuzzy_equals(m, entry->fMaskTransform)) {
            ...
            } else if (entry->fTimestamp == quickPerFlushTimestamp() && entry->fCachedAtlas &&
                       entry->fCachedAtlas->coverageType() == GrCCAtlas::CoverageType::kA8_Multisample) {
                // This means the entry was already used in current flush by some previous draw ops.
                // If entry's coverage type is kA8_Multisample, we cannot release or evict it from cache.
                // Because:
                // 1. it would invalidate the entry used in previous draw ops, causing cached paths to be re-rendered
                // 2. GrCCPerFlushResources does not allocate stencil buffer resource for cached paths when coverage
                //    type is kA8_Multisample.
                return OnFlushEntryRef();
            } else {
