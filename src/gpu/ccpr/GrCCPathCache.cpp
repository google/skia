...
sk_sp<GrCCPathCacheEntry> GrCCPathCache::find(const GrShape& shape, const MaskTransform& m,
    ...
    if (writeKey.allocCountU32() > kMaxKeyDataCountU32) {
        return nullptr;
    }

    SkASSERT(fScratchKey->unique());
    fScratchKey->resetDataCountU32(writeKey.allocCountU32());
    writeKey.write(shape, fScratchKey->data());
    ...
    if (!entry) {
        ...
        sk_sp<Key> permanentKey = Key::Make(fInvalidatedKeysInbox.uniqueID(),
                                            writeKey.allocCountU32(), fScratchKey->data());
