...
void GrCCPathCache::purgeEntriesOlderThan(const GrStdSteadyClock::time_point& purgeTime) {
    ...
    SkDEBUGCODE(auto lastTimestamp = fLRU.isEmpty() ? fPerFlushTimestamp : fLRU.tail()->fTimestamp);
    while (!fLRU.isEmpty() && fLRU.tail()->fTimestamp < purgeTime) {
        SkASSERT(fLRU.tail()->fTimestamp >= lastTimestamp);  // fLRU should be sorted by timestamp.
        SkDEBUGCODE(lastTimestamp = fLRU.tail()->fTimestamp);
