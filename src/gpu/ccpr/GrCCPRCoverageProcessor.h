 ...
 *
 * Caller provides the primitives' (x,y) points in an fp32 RG texel buffer.
 *
 * The draw buffers should be laid out with a single, "kVec4i_GrVertexAttribType", per-instance
 * attrib for each primitive. The contents of this int32x4 instance attrib vary based on primitive
 * type:
 *
 * Triangle (GrPrimitiveType::kTriangles):
 *
 *     {packed_atlas_offset,  // (offsetY << 16) | offsetX
 *      P2_idx,               // P2 index in texel buffer. (Out of order to WAR a driver bug.)
 *      P0_idx,
 *      P1_idx}
 *
 * Quadratic (GrPrimitiveType::kTriangles):
 *
 *     {packed_atlas_offset,  // (offsetY << 16) | offsetX
 *      0,                    // Unused.
 *      P1_idx,               // Control point index in texel buffer.
 *      P0_P2_idx}            // The endpoints (P0 and P2) are adjacent in the texel buffer.
 *
 * Cubic (GrPrimitiveType::kLinesAdjacency):
 *
 *     {packed_atlas_offset,  // (offsetY << 16) | offsetX
 *      0,                    // Unused.
 *      P1_P2_TSl_TSm_idx,    // The control points (P1 and P2) are adjacent in the texel buffer,
 *                            // followed immediately by the homogenous KLM roots ({tl,sl}, {tm,sm})
 *      P0_P3_idx}            // The endpoints (P0 and P3) are adjacent in the texel buffer.
 */
