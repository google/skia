...
public:
    ...
    enum class RenderPass {
        ...
        // will be false and this pass must not be used.
        kTriangleEdges,
    ...
    constexpr static bool DoesRenderPass(RenderPass renderPass, const GrShaderCaps& caps) {
        return DoesRenderPass(renderPass, caps.geometryShaderSupport() ?
                                          Impl::kGeometryShader : Impl::kVertexShader);
    }

    constexpr static bool DoesRenderPass(RenderPass renderPass, Impl impl) {
        return RenderPass::kTriangleEdges != renderPass || Impl::kVertexShader != impl;
    }

    GrCCPRCoverageProcessor(GrResourceProvider* rp, RenderPass pass, const GrShaderCaps& caps)
            : GrCCPRCoverageProcessor(rp, pass, caps.geometryShaderSupport() ?
                                                Impl::kGeometryShader : Impl::kVertexShader) {
    }

    GrCCPRCoverageProcessor(GrResourceProvider* rp, RenderPass pass, Impl impl)
            : INHERITED(kGrCCPRCoverageProcessor_ClassID)
            , fRenderPass(pass)
            , fImpl(impl) {
        SkASSERT(DoesRenderPass(pass, impl));
    ...
    protected:
        ...
        // assigned. This is intended to work for both GSImpl and VSImpl.
