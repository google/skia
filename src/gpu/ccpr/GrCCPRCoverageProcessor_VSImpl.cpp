...
static constexpr int32_t kHull3Vertices[] = {
...
GR_DECLARE_STATIC_UNIQUE_KEY(gHull3VertexBufferKey);

static constexpr uint16_t kHull3Indices[] =  {
...
GR_DECLARE_STATIC_UNIQUE_KEY(gHull3IndexBufferKey);
 ...
 */
class VSHullImpl : public GrCCPRCoverageProcessor::VSImpl {
public:
    ...
    const char* emitVertexPosition(const GrCCPRCoverageProcessor& proc, GrGLSLVertexBuilder* v,
        ...
        // to offset each vertex. Triangle edges are also handled here (see kHull3Indices). For more
...
void GrCCPRCoverageProcessor::initVS(GrResourceProvider* rp) {
    ...
    switch (fRenderPass) {
        case RenderPass::kTriangleHulls: {
            GR_DEFINE_STATIC_UNIQUE_KEY(gHull3VertexBufferKey);
            fVertexBuffer = rp->findOrCreateStaticBuffer(sizeof(kHull3Vertices),
                                                        kVertex_GrBufferType, kHull3Vertices,
                                                        gHull3VertexBufferKey);
            GR_DEFINE_STATIC_UNIQUE_KEY(gHull3IndexBufferKey);
            fIndexBuffer = rp->findOrCreateStaticBuffer(sizeof(kHull3Indices),
                                                        kIndex_GrBufferType, kHull3Indices,
                                                        gHull3IndexBufferKey);
        ...
        case RenderPass::kCubicHulls: {
            ...
            fVertexBuffer = rp->findOrCreateStaticBuffer(sizeof(kHull4Vertices),
                                                        kVertex_GrBufferType, kHull4Vertices,
                                                        gHull4VertexBufferKey);
            GR_DEFINE_STATIC_UNIQUE_KEY(gHull4IndexBufferKey);
            fIndexBuffer = rp->findOrCreateStaticBuffer(sizeof(kHull4Indices),
                                                        kIndex_GrBufferType, kHull4Indices,
        ...
        case RenderPass::kCubicCorners: {
            ...
            fIndexBuffer = rp->findOrCreateStaticBuffer(sizeof(kCornerIndices),
                                                        kIndex_GrBufferType, kCornerIndices,
...
static int num_indices_per_instance(GrCCPRCoverageProcessor::RenderPass pass) {
    switch (pass) {
        ...
        case RenderPass::kTriangleHulls:
            return SK_ARRAY_COUNT(kHull3Indices);
        case RenderPass::kQuadraticHulls:
        case RenderPass::kCubicHulls:
...
GrCCPRCoverageProcessor::createVSImpl(std::unique_ptr<Shader> shader) const {
    switch (fRenderPass) {
        case RenderPass::kTriangleHulls:
            return new VSHullImpl(std::move(shader), 3);
        case RenderPass::kQuadraticHulls:
        case RenderPass::kCubicHulls:
            return new VSHullImpl(std::move(shader), 4);
        case RenderPass::kTriangleEdges:
            SK_ABORT("kTriangleEdges RenderPass is not used by VSImpl.");
        case RenderPass::kTriangleCorners:
        case RenderPass::kQuadraticCorners:
