...
void GrCCPRCoverageProcessor::initVS(GrResourceProvider* rp) {
    ...
    switch (fRenderPass) {
        case RenderPass::kTriangleHulls: {
            ...
            fVertexBuffer = rp->findOrCreateStaticBuffer(sizeof(kHull3AndEdgeVertices),
                                                        kVertex_GrBufferType, kHull3AndEdgeVertices,
                                                        gHull3AndEdgeVertexBufferKey);
            GR_DEFINE_STATIC_UNIQUE_KEY(gHull3AndEdgeIndexBufferKey);
            fIndexBuffer = rp->findOrCreateStaticBuffer(sizeof(kHull3AndEdgeIndices),
                                                        kIndex_GrBufferType, kHull3AndEdgeIndices,
