...
void GrCCPRCubicProcessor::onEmitVertexShader(const GrCCPRCoverageProcessor& proc,
    ...
    v->codeAppendf("short4 indices = short4(%s.y, %s.x, %s.x + 1, %s.y + 1);",
...
void GrCCPRCubicProcessor::onEmitGeometryShader(GrGLSLGeometryBuilder* g, const char* emitVertexFn,
    ...
    if (Type::kSerpentine == fType) {
        ...
        g->codeAppend ("highfloat4 dxe = float4(d.s * e.s, d.s * e.t, d.t * e.s, d.t * e.t);");
        g->codeAppend ("K = highfloat4(0, dxe.x, -dxe.y - dxe.z, dxe.w);");
        g->codeAppend ("L = highfloat4(-1,1,-1,1) * d.sstt * (dxe.xyzw + float4(0, 2*dxe.zy, 0));");
        g->codeAppend ("M = highfloat4(-1,1,-1,1) * e.sstt * (dxe.xzyw + float4(0, 2*dxe.yz, 0));");
    }
...
void GrCCPRCubicBorderProcessor::emitShaderCoverage(GrGLSLFragmentBuilder* f,
    ...
    f->codeAppendf("highfloat2x4 grad_klmd = float2x4(%s, %s);", fdKLMDdx.fsIn(),
                   fdKLMDdy.fsIn());
