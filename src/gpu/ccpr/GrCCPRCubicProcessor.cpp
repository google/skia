...
void GrCCPRCubicProcessor::onEmitVertexShader(const GrCCPRCoverageProcessor& cp,
    ...
    v->codeAppendf("ivec4 vertex_indices = ivec4(%s.w, %s.z, %s.z + 1, %s.w + 1);",
                   cp.instanceAttrib(), cp.instanceAttrib(), cp.instanceAttrib(),
                   cp.instanceAttrib());
    v->codeAppend ("highp mat4x2 pts = mat4x2(");
    ...
    // Find the corner of the inset geometry that corresponds to this bezier vertex (pts[0]).
    ...
    v->codeAppendf("%s.xy = K * inverse(N) + atlasoffset;", fInset.vsOut());
    ...
    v->codeAppendf("int dataidx = ((%s.y >> 31) ^ %s.y);",
                   cp.instanceAttrib(), cp.instanceAttrib());
    v->codeAppendf("%s = ", fTS.vsOut());
    v->appendTexelFetch(pointsBuffer,
                        SkStringPrintf("%s.z + 2 + sk_VertexID/2", cp.instanceAttrib()).c_str());
    v->codeAppend ("[sk_VertexID % 2];");

    // Emit the vertex position.
    v->codeAppendf("highp vec2 self = pts[0] + atlasoffset;");
    gpArgs->fPositionVar.set(kVec2f_GrSLType, "self");
}

void GrCCPRCubicProcessor::emitWind(GrGLSLGeometryBuilder* g, const char* windName,
    ...
    g->codeAppend ("highp float area_times_2 = determinant(mat3(1,P[0], 1,P[2], 0,P[3]-P[1]));");
    // Drop curves that are nearly flat. The KLM  math becomes unstable in this case.
    g->codeAppendf("if (2 * abs(area_times_2) < length((P[3] - P[0]) * %s.zx)) {", rtAdjustName);
...
void GrCCPRCubicProcessor::onEmitGeometryShader(GrGLSLGeometryBuilder* g, const char* emitVertexFn,
                                                const char* wind, const char* rtAdjustName) const {
    ...
    g->codeAppendf("%s[0] = %s[0].xy * %s.xz;",
                   fKLMDerivatives.c_str(), fKLMMatrix.c_str(), rtAdjustName);
    g->codeAppendf("%s[1] = %s[1].xy * %s.xz;",
                   fKLMDerivatives.c_str(), fKLMMatrix.c_str(), rtAdjustName);
    g->codeAppendf("%s[2] = %s[2].xy * %s.xz;",
                   fKLMDerivatives.c_str(), fKLMMatrix.c_str(), rtAdjustName);

    this->emitCubicGeometry(g, emitVertexFn, wind, rtAdjustName);
}
...
void GrCCPRCubicBorderProcessor::emitCubicGeometry(GrGLSLGeometryBuilder* g,
    ...
    g->codeAppendf("highp vec2 edgept0 = P[edgeidx0];");
    g->codeAppendf("highp vec2 edgept1 = P[3 - edgeidx0];");
    ...
    g->codeAppendf("%s = %s.xy * %s.xz;",
                   fEdgeDistanceDerivatives.c_str(), fEdgeDistanceEquation.c_str(), rtAdjustName);

    g->declareGlobal(fEdgeSpaceTransform);
    g->codeAppend ("highp vec4 edgebbox = vec4(min(P[0], P[3]) - bloat, max(P[0], P[3]) + bloat);");
    ...
    int maxVertices = this->emitHullGeometry(g, 4, "sk_InvocationID", emitVertexFn, "P", "inset");
...
void GrCCPRCubicBorderProcessor::emitShaderCoverage(GrGLSLFragmentBuilder* f,
                                                    const char* outputCoverage) const {
    // Use pseudo msaa to determine coverage.
    const int sampleCount = this->definePseudoSampleLocations(f, "samples");
