...
void GrCCPRCubicProcessor::onEmitVertexShader(const GrCCPRCoverageProcessor& proc,
    ...
    v->codeAppend ("highp float2 N = float2(bezierpts[3].y - bezierpts[0].y, "
                                           "bezierpts[0].x - bezierpts[3].x, "
                                           "bezierpts[1].y - bezierpts[0].y, "
                                           "bezierpts[0].x - bezierpts[1].x);");
    v->codeAppend ("highp float2 P = float2(bezierpts[3], bezierpts[1]);");
...
void GrCCPRCubicProcessor::onEmitGeometryShader(GrGLSLGeometryBuilder* g, const char* emitVertexFn,
    ...
    g->codeAppend ("highp float2 X = float2(inset[x].y - inset[x+1].y, "
    ...
    g->codeAppend ("highp float3 CI = inverse(float3x3(C[0][0], C[0][middlerow], C[0][3], "
    ...
    g->codeAppendf("%s *= float3(orientation[0] * orientation[1], 0, 0, "
                                "0, orientation[0], 0, "
