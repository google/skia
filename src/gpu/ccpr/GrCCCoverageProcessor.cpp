...
void GrCCCoverageProcessor::getGLSLProcessorKey(const GrShaderCaps&,
    ...
    if (TriangleSubpass::kCorners == fTriangleSubpass) {
        key |= 4;
    }
...
void GrCCCoverageProcessor::draw(GrOpFlushState* flushState, const GrPipeline& pipeline,
    ...
    if (Impl::kGeometryShader == fImpl && RenderPass::kTriangles == fRenderPass) {
        SkASSERT(TriangleSubpass::kHullsAndEdges == fTriangleSubpass);

        GrCCCoverageProcessor cornerProc(flushState->resourceProvider(), fRenderPass, fWindMethod);
        cornerProc.fTriangleSubpass = TriangleSubpass::kCorners;
#ifdef SK_DEBUG
        if (this->debugVisualizationsEnabled()) {
            cornerProc.enableDebugVisualizations(fDebugBloat);
        }
#endif
