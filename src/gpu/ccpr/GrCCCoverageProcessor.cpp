...
void GrCCCoverageProcessor::Shader::CalcCornerAttenuation(GrGLSLVertexGeoBuilder* s,
    ...
    //
    // NOTE: leftDir and rightDir point in the same direction the path was defined with, i.e.,
    // leftDir points into the corner and rightDir points away from the corner.
    s->codeAppendf("half obtuseness = max(dot(%s, %s), 0);", leftDir, rightDir);

    // axis_alignedness = 1  when the corner bisector is aligned with the x- or y-axis
