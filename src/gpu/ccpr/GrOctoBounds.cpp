...
bool GrOctoBounds::clip(const SkIRect& clipRect) {
    ...
    // may be tighter now even than the clipped bounds, depending on the diagonals. The vertex
    // shader that emits octagons expects both bounding boxes to circumcribe the inner octagon, and
