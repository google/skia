...
public:
    ...
    // GrDrawOp interface.
    const char* name() const override { return "Coverage Counting Coverage Op"; }
...
private:
    void drawMaskPrimitives(GrOpFlushState*, const GrPipeline&, const GrCCPRCoverageProcessor::Mode,
                            GrPrimitiveType, int PrimitiveTallies::* instanceType) const;
    ...
    mutable SkTArray<GrMesh>                     fMeshScratch;
    mutable SkTArray<GrPipeline::DynamicState>   fDynamicStateScratch;
...
bool GrCCPRCoverageOpsBuilder::init(GrOnFlushResourceProvider* onFlushRP,
    ...
    fInstanceData = static_cast<std::array<int, 4>*>(fInstanceBuffer->map());
...
void GrCCPRCoverageOpsBuilder::saveParsedPath(int16_t offsetX, int16_t offsetY,
                                              const SkIRect& clipBounds) {
    const PrimitiveTallies& baseIndices = fInstanceIndices[(int)fCurrScissorMode];
    int packedOffset = (offsetX & 0xffff) | (offsetY << 16);
    for (int i = baseIndices.fTriangles; i < fCurrPathIndices.fTriangles; ++i) {
        fInstanceData[i][0] = packedOffset;
    }
    ...
    if (ScissorMode::kScissored == fCurrScissorMode) {
        fScissorBatches.push_back() = {
            fCurrPathIndices - fInstanceIndices[(int)fCurrScissorMode],
            clipBounds.makeOffset(offsetX, offsetY)
        };
    }
...
void GrCCPRCoverageOpsBuilder::quadraticTo(AccumulatingViewMatrix& m, const SkPoint P[3]) {
    ...
    fInstanceData[fCurrPathIndices.fQuadratics++] = {{
        0,
        0,
        fControlPtsIdx - 1,
        fFanPtsIdx - 2
...
void GrCCPRCoverageOpsBuilder::emitCubicSegment(AccumulatingViewMatrix& m,
    ...
    fInstanceData[SkCubicType::kLoop != type ?
                  fCurrPathIndices.fSerpentines++ : --fCurrPathIndices.fLoops] = {{
        0,
        0,
        fControlPtsIdx - 4,
        fFanPtsIdx - 2
...
void GrCCPRCoverageOpsBuilder::emitHierarchicalFan(int indices[], int count) {
    if (count < 3) {
        return;
    }
    ...
    fInstanceData[fCurrPathIndices.fTriangles++] = {{
        0,
        indices[0],
        indices[oneThirdPt],
        indices[twoThirdsPt]
...
GrCCPRCoverageOpsBuilder::CoverageOp::CoverageOp(const SkISize& drawBounds,
    ...
    GR_STATIC_ASSERT(2 == kNumScissorModes);
...
void CoverageOp::onExecute(GrOpFlushState* flushState) {
    ...
    fMeshScratch.reserve(1 + fScissorBatches.count());
    fDynamicStateScratch.reserve(1 + fScissorBatches.count());

    // Triangles.
    this->drawMaskPrimitives(flushState, pipeline, Mode::kCombinedFanHullsAndEdges,
                             GrPrimitiveType::kTriangles, &PrimitiveTallies::fTriangles);
    this->drawMaskPrimitives(flushState, pipeline, Mode::kAnchorCorners,
                             GrPrimitiveType::kTriangles, &PrimitiveTallies::fTriangles);

    // Quadratics.
    this->drawMaskPrimitives(flushState, pipeline, Mode::kQuadraticHulls,
                             GrPrimitiveType::kTriangles, &PrimitiveTallies::fQuadratics);
    this->drawMaskPrimitives(flushState, pipeline, Mode::kQuadraticInnerEdges,
                             GrPrimitiveType::kTriangles, &PrimitiveTallies::fQuadratics);

    // Cubics.
    this->drawMaskPrimitives(flushState, pipeline, Mode::kSerpInset,
                             GrPrimitiveType::kLinesAdjacency, &PrimitiveTallies::fSerpentines);
    this->drawMaskPrimitives(flushState, pipeline, Mode::kLoopInset,
                             GrPrimitiveType::kLinesAdjacency, &PrimitiveTallies::fLoops);
    this->drawMaskPrimitives(flushState, pipeline, Mode::kSerpBorder,
                             GrPrimitiveType::kLinesAdjacency, &PrimitiveTallies::fSerpentines);
    this->drawMaskPrimitives(flushState, pipeline, Mode::kLoopBorder,
                             GrPrimitiveType::kLinesAdjacency, &PrimitiveTallies::fLoops);
}
...
void CoverageOp::drawMaskPrimitives(GrOpFlushState* flushState, const GrPipeline& pipeline,
    ...
    fMeshScratch.reset();
    fDynamicStateScratch.reset();

    const int vertexCount = GrPrimitiveType::kLinesAdjacency == primType ? 4 : 3;
    ...
    if (const int instanceCount = fInstanceCounts[(int)ScissorMode::kNonScissored].*instanceType) {
        ...
        fMeshScratch.emplace_back(primType).setInstanced(fTrianglesBuffer.get(),
                                                         abs(instanceCount),
                                                         baseInstance + SkTMin(instanceCount, 0),
                                                         vertexCount);
        fDynamicStateScratch.push_back().fScissorRect.setXYWH(0, 0, fDrawBounds.width(),
                                                              fDrawBounds.height());
    }
    ...
    if (fInstanceCounts[(int)ScissorMode::kScissored].*instanceType) {
        ...
        for (const ScissorBatch& batch : fScissorBatches) {
            ...
            fMeshScratch.emplace_back(primType).setInstanced(fTrianglesBuffer.get(),
                                                             ...
                                                             baseInstance + SkTMin(instanceCount,0),
                                                             vertexCount);
            fDynamicStateScratch.push_back().fScissorRect = batch.fScissor;
            baseInstance += instanceCount;
        }
    }

    GrCCPRCoverageProcessor proc(mode, fPointsBuffer.get());
    flushState->commandBuffer()->draw(pipeline, proc, fMeshScratch.begin(),
                                      fDynamicStateScratch.begin(), fMeshScratch.count(),
                                      this->bounds());
}
