        ...
        if (3 == fNumSides) { // Only triangles emit corner boxes.
            ...
            v->codeAppendf("if (0 != (%s & %i)) {", // Are we a corner?
                           proc.getAttrib(kAttribIdx_VertexData).fName, kVertexData_IsCornerBit);
                               ...
                               // In corner boxes, all 4 coverage values will not map linearly. It
                               // is important to align the box so its diagonal shared edge points
                               // out of the triangle, in the direction that ramps to zero.
            v->codeAppend (    "bloatdir = float2(leftdir.x > rightdir.x ? +1 : -1, "
                               ...
                               // causes the upcoming code to always rotate, generating all 4
                               // vertices of the corners box.
