...
void GrCCCoverageProcessor::VSImpl::onEmitCode(EmitArgs& args, GrGPArgs* gpArgs) {
    ...
    if (PrimitiveType::kWeightedTriangles == proc.fPrimitiveType) {
        ...
        SkASSERT(kFloat4_GrVertexAttribType == proc.fInstanceAttributes[kInstanceAttribIdx_X].type());
...
void GrCCCoverageProcessor::initVS(GrResourceProvider* rp) {
    ...
    if (4 == this->numInputPoints() || this->hasInputWeight()) {
        ...
        GR_STATIC_ASSERT(sizeof(QuadPointInstance::fX) == GrVertexAttribTypeSize(kFloat4_GrVertexAttribType));
        GR_STATIC_ASSERT(sizeof(QuadPointInstance::fY) == GrVertexAttribTypeSize(kFloat4_GrVertexAttribType));
        xyAttribType = kFloat4_GrVertexAttribType;
    } else {
        GR_STATIC_ASSERT(offsetof(TriPointInstance, fX) == 0);
        GR_STATIC_ASSERT(sizeof(TriPointInstance::fX) == GrVertexAttribTypeSize(kFloat3_GrVertexAttribType));
        GR_STATIC_ASSERT(sizeof(TriPointInstance::fY) == GrVertexAttribTypeSize(kFloat3_GrVertexAttribType));
        xyAttribType = kFloat3_GrVertexAttribType;
    }
    fInstanceAttributes[kInstanceAttribIdx_X] = {"X", xyAttribType};
    fInstanceAttributes[kInstanceAttribIdx_Y] = {"Y", xyAttribType, fInstanceAttributes[kInstanceAttribIdx_X]};
    this->setInstanceAttributeInfo(2, fInstanceAttributes[kInstanceAttribIdx_Y].nextOffsetInRecord());

    fVertexAttribute = {"vertexdata", kInt_GrVertexAttribType, 0};
    this->setVertexAttributeInfo(1, GrVertexAttribTypeSize(kInt_GrVertexAttribType));
