...
static constexpr int kVertexData_InvertCoverageBit = 1 << 5;
...
static constexpr int32_t corner_vertex_data(int32_t cornerID, int32_t bloatIdx, int n) {
    return pack_vertex_data((cornerID + n - 1) % n, (cornerID + 1) % n, bloatIdx, cornerID,
...
static constexpr int32_t kTriangleVertices[] = {
    ...
    edge_vertex_data(1, 0, 0, kVertexData_InvertCoverageBit),
    edge_vertex_data(1, 0, 1, kVertexData_InvertCoverageBit),
    edge_vertex_data(1, 0, 2, kVertexData_InvertCoverageBit),

    edge_vertex_data(1, 2, 0),
    edge_vertex_data(1, 2, 1),
    edge_vertex_data(1, 2, 2),
    edge_vertex_data(2, 1, 0, kVertexData_InvertCoverageBit),
    edge_vertex_data(2, 1, 1, kVertexData_InvertCoverageBit),
    edge_vertex_data(2, 1, 2, kVertexData_InvertCoverageBit),

    edge_vertex_data(2, 0, 0),
    edge_vertex_data(2, 0, 1),
    edge_vertex_data(2, 0, 2),
    edge_vertex_data(0, 2, 0, kVertexData_InvertCoverageBit),
    edge_vertex_data(0, 2, 1, kVertexData_InvertCoverageBit),
    edge_vertex_data(0, 2, 2, kVertexData_InvertCoverageBit),

    corner_vertex_data(0, 0, 3),
    corner_vertex_data(0, 1, 3),
    corner_vertex_data(0, 2, 3),
    corner_vertex_data(0, 3, 3),

    corner_vertex_data(1, 0, 3),
    corner_vertex_data(1, 1, 3),
    corner_vertex_data(1, 2, 3),
    corner_vertex_data(1, 3, 3),

    corner_vertex_data(2, 0, 3),
    corner_vertex_data(2, 1, 3),
    corner_vertex_data(2, 2, 3),
    corner_vertex_data(2, 3, 3),
};
 ...
 * independent conservative rasters around each edge and corner. (See comments for RenderPass)
 */
...
public:
    ...
    const char* emitVertexPosition(const GrCCCoverageProcessor& proc, GrGLSLVertexBuilder* v,
        ...
        //
        // Triangle edges and corners are also handled here using the same concept (see
        // kTriangleIndices).
        ...
        // For corner boxes, we hack 'left_right_notequal' to [true, true]. This causes the below
        // code to always rotate, which is the right thing for corners.
        v->codeAppendf("bool2 left_right_notequal = 0 != (%s & %i) ? " // Are we a corner?
                                                   "bool2(true, true) :"
                                                   "notEqual(leftbloat, rightbloat);",
                                                   proc.getAttrib(kAttribIdx_VertexData).fName,
                                                   kVertexData_IsCornerBit);
        ...
        if (3 == fNumSides) {
            ...
            // All 4 corner coverage values will not map linearly, so it is important to rotate the
            // box so its diagonal shared edge runs in the direction that ramps to zero.
            ...
            // linearly-interpolated values that ramp to zero in one diagonal, and ramp from
...
void GrCCCoverageProcessor::initVS(GrResourceProvider* rp) {
    ...
    switch (fRenderPass) {
        ...
        case RenderPass::kTriangleCorners:
            SK_ABORT("kTriangleEdges and kTriangleCorners RenderPasses are not used by VSImpl.");
            break;
