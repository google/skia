...
public:
    ...
    void draw(GrOpFlushState*, const GrPipeline&, const GrMesh[], const GrPipeline::DynamicState[],
              int meshCount, const SkRect& drawBounds);
    ...
    public:
        ...
        // clockwise-ordered points, 'leftPt' and 'rightPt'. 'bloatDir' is a pair of +/-1 values
        // that point in the direction of bloat, starting from a point on the edge.
        //
...
private:
    ...
    // Used by GSImpl. (Geometry shader backend draws triangles in two subpasses.)
    enum class TriangleSubpass : bool {
        kHullsAndEdges,
        kCorners
    };

    TriangleSubpass fTriangleSubpass = TriangleSubpass::kHullsAndEdges;
