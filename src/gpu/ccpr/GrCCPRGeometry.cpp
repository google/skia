...
void GrCCPRGeometry::beginContour(const SkPoint& devPt) {
    ...
    // Store the current verb count in the triangles field for now. When we close the contour we
    // will use this value to calculate the number of triangles required.
...
GrCCPRGeometry::PrimitiveTallies GrCCPRGeometry::endContour() {
    ...
    // fCurrContourTallies.fTriangles currently contains this contour's starting verb index.
    // We can now subtract it to calculate the number of triangles in the contour.
    fCurrContourTallies.fTriangles = fVerbs.count() - fCurrContourTallies.fTriangles - 2;

    if (fCurrFanPoint == fCurrAnchorPoint) {
        --fCurrContourTallies.fTriangles;
        fVerbs.push_back(Verb::kEndClosedContour);
    } else {
        fVerbs.push_back(Verb::kEndOpenContour);
    }
    fCurrContourTallies.fTriangles = SkTMax(fCurrContourTallies.fTriangles, 0);
