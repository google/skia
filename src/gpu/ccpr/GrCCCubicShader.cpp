...
void GrCCCubicShader::onEmitVaryings(GrGLSLVaryingHandler* varyingHandler,
    ...
    fKLM_Edge.reset(kFloat4_GrSLType, scope);
    varyingHandler->addVarying("klm_edge", &fKLM_Edge);
    ...
    code->appendf("%s.xyz = klm * float3(1, %s, %s);",
                  OutName(fKLM_Edge), coverage, coverage); // coverage == wind on curves.
    code->appendf("%s.w = dot(float3(%s, 1), %s);", // Flat edge opposite the curve.
                  OutName(fKLM_Edge), position, fEdgeDistanceEquation.c_str());
    ...
    if (cornerCoverage) {
        ...
        this->calcHullCoverage(code, OutName(fKLM_Edge), OutName(fGradMatrix), "hull_coverage");
        code->appendf("}");
...
void GrCCCubicShader::onEmitFragmentCode(GrGLSLFPFragmentBuilder* f,
    ...
    this->calcHullCoverage(&AccessCodeString(f), fKLM_Edge.fsIn(), fGradMatrix.fsIn(),
                           outputCoverage);
