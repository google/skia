    ...
    for (SingleDraw& draw : fDraws) {
        ...
        if (auto cacheEntry = draw.fCacheEntry.get()) {
            ...
            if (cacheEntry->atlasKey().isValid()) {
                ...
                cacheEntry->resetAtlasKey();
            }

            // If the draw has a cache entry, we will attempt to stash away its mask for copying
            // into a cached atlas during the next flush.
...
void GrCCDrawPathsOp::setupResources(GrOnFlushResourceProvider* onFlushRP,
    ...
    for (SingleDraw& draw : fDraws) {
        ...
        if (auto cacheEntry = draw.fCacheEntry.get()) {
            ...
            if (auto proxy = draw.fCachedAtlasProxy.get()) {
                ...
                this->recordInstance(proxy, AtlasType::kTextureProxy,
            ...
            // Have we encountered this path previously during the flush? (e.g. was the same SkPath
            // drawn more than once in a single flush, with a compatible matrix.)
            if (auto atlas = cacheEntry->currFlushAtlas()) {
                this->recordInstance(atlas, AtlasType::kCCAtlas, resources->nextPathInstanceIdx());
        ...
        if (auto atlas = resources->renderPathInAtlas(draw.fLooseClippedIBounds, draw.fMatrix, path,
                                                      ...
                                                      &atlasOffset)) {
            this->recordInstance(atlas, AtlasType::kCCAtlas, resources->nextPathInstanceIdx());
            resources->appendDrawPathInstance().set(devBounds, devBounds45, atlasOffset,
                                                    draw.fColor, doEvenOddFill);
            if (draw.fCanStashPathMask && resources->nextAtlasToStash() == atlas) {
                SkASSERT(draw.fCacheEntry);
                const GrUniqueKey& atlasKey =
                        resources->nextAtlasToStash()->getOrAssignUniqueKey(onFlushRP);
                draw.fCacheEntry->initAsStashedAtlas(atlasKey, atlasOffset, devBounds, devBounds45,
    ...
    if (!fInstanceRanges.empty()) {
        fInstanceRanges.back().fEndInstance = resources->nextPathInstanceIdx();
    }
}

inline void GrCCDrawPathsOp::recordInstance(const void* atlas, AtlasType atlasType,
                                            int instanceIdx) {
    SkASSERT(atlas);
    if (fInstanceRanges.empty()) {
        fInstanceRanges.push_back({atlas, atlasType, instanceIdx});
        return;
    }
    if (fInstanceRanges.back().fAtlas != atlas) {
        fInstanceRanges.back().fEndInstance = instanceIdx;
        fInstanceRanges.push_back({atlas, atlasType, instanceIdx});
        return;
    }
    SkASSERT(atlasType == fInstanceRanges.back().fAtlasType);
}
...
void GrCCDrawPathsOp::onExecute(GrOpFlushState* flushState) {
    ...
    for (const InstanceRange& range : fInstanceRanges) {
        SkASSERT(range.fAtlas);
        SkASSERT(range.fEndInstance > baseInstance);

        const GrTextureProxy* proxy;
        if (AtlasType::kTextureProxy == range.fAtlasType) {
            proxy = static_cast<const GrTextureProxy*>(range.fAtlas);
        } else {
            SkASSERT(AtlasType::kCCAtlas == range.fAtlasType);
            proxy = static_cast<const GrCCAtlas*>(range.fAtlas)->textureProxy();
        }

        if (proxy) {
            GrCCPathProcessor pathProc(flushState->resourceProvider(), sk_ref_sp(proxy),
                                       fViewMatrixIfUsingLocalCoords);
            pathProc.drawPaths(flushState, pipeline, *resources, baseInstance, range.fEndInstance,
                               this->bounds());
        }

        baseInstance = range.fEndInstance;
    }
}
