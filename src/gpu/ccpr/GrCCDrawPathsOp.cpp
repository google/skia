...
std::unique_ptr<GrCCDrawPathsOp> GrCCDrawPathsOp::Make(
    ...
    int conservativeSize = SkTMax(conservativeDevBounds.height(), conservativeDevBounds.width());
...
GrDrawOp::RequiresDstTexture GrCCDrawPathsOp::finalize(const GrCaps& caps,
    ...
    // if we can cap their minimum width at 1, and then start reducing opacity to create the
    // illusion of thin-ness. This strategy also helps reduce artifacts from coverage dilation when
    ...
    if (analysis.isCompatibleWithCoverageAsAlpha() &&
        ...
        draw->fShape = GrShape(path, GrStyle::SimpleHairline());
