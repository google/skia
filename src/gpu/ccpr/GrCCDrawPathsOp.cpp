...
void GrCCDrawPathsOp::accountForOwnPaths(GrCCPathCache* pathCache,
    ...
    for (SingleDraw& draw : fDraws) {
        ...
        if (auto cacheEntry = draw.fCacheEntry.get()) {
            ...
            if (Visibility::kMostlyComplete == draw.fMaskVisibility && cacheEntry->hitCount() > 0) {
                // We've seen this path before with a compatible matrix, and it's mostly visible.
                // Just render the whole mask so we can cache it.
...
void GrCCDrawPathsOp::setupResources(GrOnFlushResourceProvider* onFlushRP,
    ...
    for (SingleDraw& draw : fDraws) {
        ...
        if (auto atlas = resources->renderPathInAtlas(draw.fMaskDevIBounds, draw.fMatrix, path,
            ...
            // If we have a spot in the pathe cache, try to make a note of where this mask is so we
            // can reuse it in the future.
            if (auto cacheEntry = draw.fCacheEntry.get()) {
                SkASSERT(!cacheEntry->hasCachedAtlas());

                if (Visibility::kComplete != draw.fMaskVisibility) {
                    // Don't cache a path mask unless it's completely visible.
                    //
                    // NOTE: mostly-visible paths with a positive hit count should have been
                    // promoted to fully visible during accountForOwnPaths().
                    continue;
                }
