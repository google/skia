    ...
    for (SingleDraw& draw : fDraws) {
        ...
        if (auto cacheEntry = draw.fCacheEntry.get()) {
            ...
            if (auto proxy = draw.fCachedAtlasProxy.get()) {
                ...
                this->recordInstance(AtlasPtrType::kGrTextureProxy, proxy,
            ...
            if (auto currFlushAtlas = cacheEntry->currFlushAtlas()) {
                this->recordInstance(AtlasPtrType::kGrCCAtlas, currFlushAtlas,
                                     resources->nextPathInstanceIdx());
            ...
            if (DoCopiesToCache::kYes == doCopies && cacheEntry->atlasKey().isValid()) {
                ...
                this->recordInstance(AtlasPtrType::kGrCCAtlas, atlas,
                                     resources->nextPathInstanceIdx());
                resources->appendDrawPathInstance().set(*cacheEntry, draw.fCachedMaskShift,
                                                        draw.fColor);
        ...
        if (auto atlas = resources->renderPathInAtlas(draw.fLooseClippedIBounds, draw.fMatrix, path,
                                                      ...
                                                      &devToAtlasOffset)) {
            this->recordInstance(AtlasPtrType::kGrCCAtlas, atlas, resources->nextPathInstanceIdx());
...
inline void GrCCDrawPathsOp::recordInstance(AtlasPtrType atlasPtrType, const void* atlasPtr,
                                            int instanceIdx) {
    SkASSERT(atlasPtr);
    if (fInstanceRanges.empty()) {
        fInstanceRanges.push_back({atlasPtrType, atlasPtr, instanceIdx});
        return;
    }
    if (fInstanceRanges.back().fAtlasPtr != atlasPtr) {
        fInstanceRanges.back().fEndInstance = instanceIdx;
        fInstanceRanges.push_back({atlasPtrType, atlasPtr, instanceIdx});
        return;
    }
    SkASSERT(atlasPtrType == fInstanceRanges.back().fAtlasPtrType);
}
...
void GrCCDrawPathsOp::onExecute(GrOpFlushState* flushState) {
    ...
    for (const InstanceRange& range : fInstanceRanges) {
        SkASSERT(range.fAtlasPtr);
        SkASSERT(range.fEndInstance > baseInstance);

        const GrTextureProxy* proxy;
        if (AtlasPtrType::kGrTextureProxy == range.fAtlasPtrType) {
            proxy = static_cast<const GrTextureProxy*>(range.fAtlasPtr);
        } else {
            SkASSERT(AtlasPtrType::kGrCCAtlas == range.fAtlasPtrType);
            proxy = static_cast<const GrCCAtlas*>(range.fAtlasPtr)->textureProxy();
        }
