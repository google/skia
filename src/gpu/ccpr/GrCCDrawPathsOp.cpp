...
GrCCDrawPathsOp::SingleDraw::SingleDraw(const SkIRect& clippedDevIBounds, const SkMatrix& m,
    ...
    if (fShape.hasUnstyledKey()) {
        // Shave off fractional translates for cachable paths on Android. This is done to match HWUI
        // for cache hit ratio, which doesn't consider the matrix at all when caching paths.
        fMatrix.setTranslate(SkScalarRoundToScalar(fMatrix.getTranslateX()),
                             SkScalarRoundToScalar(fMatrix.getTranslateY()));
    }
#endif
}
