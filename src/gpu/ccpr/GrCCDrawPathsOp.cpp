...
void GrCCDrawPathsOp::SingleDraw::setupResources(
    ...
    if (auto cacheEntry = fCacheEntry.get()) {
        // Does the path already exist in a cached atlas texture?
        if (cacheEntry->cachedAtlas()) {
            SkASSERT(cacheEntry->cachedAtlas()->onFlushProxy());
            ...
            op->recordInstance(cacheEntry->cachedAtlas()->onFlushProxy(),
                                 resources->nextPathInstanceIdx());
            // TODO4F: Preserve float colors
            resources->appendDrawPathInstance().set(*cacheEntry, fCachedMaskShift,
                                                    fColor.toBytes_RGBA());
            return;
        }
    }
