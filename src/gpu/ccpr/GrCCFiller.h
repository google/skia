...
class GrCCFiller {
public:
    enum class AAType : bool {
        kCoverageCount,
        kStencilMultisample
    };

    GrCCFiller(AAType, int numPaths, int numSkPoints, int numSkVerbs, int numConicWeights);
    ...
    bool prepareToDraw(GrOnFlushResourceProvider*,
                       GrCCCoverageProcessor::TriPointInstance::Ordering triangleOrdering);
...
private:
    ...
    public:
        ...
        void tessellateFan(
                const SkPath& originalPath, AAType, const GrCCFillGeometry&, int verbsIdx,
