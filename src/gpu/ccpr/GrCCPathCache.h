...
/**
 * This class implements an LRU cache that maps from GrShape to GrCCPathCachEntry objects. Shapes
 * only have one entry in the cache, so any time they are accessed with a different matrix, the old
 * entry gets evicted.
 */
class GrCCPathCache {
public:
    static constexpr int kMaxCount = 1 << 16;
    ...
    // shaved off and returned to the caller. The caller is responsible for these integer shifts.
    ...
    // Finds an entry in the cache. Shapes only have one entry, so any time they are accessed with a
    // different MaskTransform, the old entry gets evicted.
...
private:
    ...
    // moved, which guarantees the hash table references each entry exactly once. When a HashNode
    // goes out of scope, it means the entry has been evicted from the cache.
...
/**
 * This class stores all the data necessary to draw a specific shape from its corresponding cached
 * atlas.
 */
...
public:
    ...
    void resetAtlasKey() { fAtlasKey.reset(); }

    // This a utility for the caller to detect when a path gets drawn more than once during the same
    // flush, with compatible matrices. Before adding a path to an atlas, the caller may check here
    // to see they have already placed the path previously during the same flush. The caller is
    // responsible to reset all currFlushAtlas references back to null before any subsequent flush.
...
inline void GrCCPathProcessor::Instance::set(const GrCCPathCacheEntry& entry,
                                             const SkIVector& shift, uint32_t color,
