...
public:
    ...
    ~GrCCPathCacheEntry() override {
        // HashNode should have cleared our cache pointer and invalidated our atlas.
        SkASSERT(!fCacheWeakPtr);
        SkASSERT(!fAtlasKey.isValid());
        SkASSERT(!fCachedAtlasInfo);
        // The client is required to reset all currFlushAtlas references back to null.
        SkASSERT(!fCurrFlushAtlas);
    }
#endif
