...
bool GrCoverageCountingPathRenderer::IsSupported(const GrCaps& caps, CoverageType* coverageType) {
    ...
    if (!shaderCaps.integerSupport() ||
        ...
        !caps.halfFloatVertexAttributeSupport()) {  // This checks both texturable and renderable
        return false;
    }
...
std::unique_ptr<GrFragmentProcessor> GrCoverageCountingPathRenderer::makeClipProcessor(
    ...
    SkASSERT(!fFlushing);
    SkASSERT(deviceSpacePath.getBounds().height() *
             deviceSpacePath.getBounds().width() <= kMaxClipPathArea);
