...
void GrCoverageCountingPathRenderer::preFlush(GrOnFlushResourceProvider* onFlushRP,
    ...
    GrCCAtlas::Specs atlasSpecs;
    atlasSpecs.fMaxPreferredTextureSize = onFlushRP->caps()->maxPreferredRenderTargetSize();
    atlasSpecs.fMinTextureSize = SkTMin(1024, atlasSpecs.fMaxPreferredTextureSize);
    ...
    for (int i = 0; i < numOpListIDs; ++i) {
        ...
        for (const GrCCDrawPathsOp* op : fFlushingPaths.back()->fDrawOps) {
            numPathDraws += op->countPaths(&flushingPathStats, &atlasSpecs);
        }
        for (const auto& clipsIter : fFlushingPaths.back()->fClipPaths) {
            clipsIter.second.countPath(&flushingPathStats, &atlasSpecs);
        }
