...
std::unique_ptr<GrFragmentProcessor> GrCoverageCountingPathRenderer::makeClipProcessor(
        GrProxyProvider* proxyProvider, uint32_t opListID, const SkPath& deviceSpacePath,
...
void GrCoverageCountingPathRenderer::preFlush(GrOnFlushResourceProvider* onFlushRP,
    ...
    // Dig up the stashed atlas from the previous flush so we can attempt to copy any reusable paths
    // out of it and into cached textures. We also need to clear its unique key.
    // TODO: make sure the texture gets freed here and reused by the next atlas.
    sk_sp<GrTextureProxy> stashedAtlasProxy;
    if (fStashedAtlasKey.isValid()) {
        stashedAtlasProxy =
                onFlushRP->findOrCreateProxyByUniqueKey(fStashedAtlasKey, GrCCAtlas::kOrigin);
