...
void GrCoverageCountingPathRenderer::preFlush(GrOnFlushResourceProvider* onFlushRP,
    ...
    GrCCResourceInitCounts initCounts;
    int maxPreferredRTSize = onFlushRP->caps()->maxPreferredRenderTargetSize();
    initCounts.fAtlasSpecs.fMaxPreferredTextureSize = maxPreferredRTSize;
    initCounts.fAtlasSpecs.fMinTextureSize = SkTMin(1024, maxPreferredRTSize);
    ...
    for (int i = 0; i < numOpListIDs; ++i) {
        ...
        for (const GrCCDrawPathsOp* op : fFlushingPaths.back()->fDrawOps) {
            op->countPaths(&initCounts);
        }
        for (const auto& clipsIter : fFlushingPaths.back()->fClipPaths) {
            clipsIter.second.countPath(&initCounts);
        }
    }

    if (initCounts.isEmpty()) {
        return;  // Nothing to draw.
    }

    auto resources = sk_make_sp<GrCCPerFlushResources>(onFlushRP, initCounts);
    ...
    SkASSERT(resources->nextPathInstanceIdx() == initCounts.fNumRenderedPaths - numSkippedPaths);
