...
sk_sp<GrCoverageCountingPathRenderer> GrCoverageCountingPathRenderer::Create(const GrCaps& caps) {
    const GrShaderCaps& shaderCaps = *caps.shaderCaps();
    if (shaderCaps.geometryShaderSupport() &&
...
GrCoverageCountingPathRenderer::DrawPathsOp::DrawPathsOp(GrCoverageCountingPathRenderer* ccpr,
    ...
    // (This is nontrivial due do the octagon cover geometry.)
...
bool DrawPathsOp::onCombineIfPossible(GrOp* op, const GrCaps& caps) {
    ...
    if (this->getFillType() != that->getFillType() || fProcessors != that->fProcessors) {
        return false;
    }

    if (that->fOwner) {
        SkASSERT(fOwner == that->fOwner);
        fOwner->fOpList.remove(that);
    } else {
        ...
        fOwner->fMaxBufferItems.countPathItems(onlyDraw.fScissorMode, onlyDraw.fPath);
    }

    fTailDraw->fNext = &fOwner->fDrawsAllocator.push_back(that->fHeadDraw);
...
void DrawPathsOp::wasRecorded(GrRenderTargetOpList* opList) {
    SkASSERT(!fOwner);
    SingleDraw& onlyDraw = this->getOnlyPathDraw();
    fOwner = &fCCPR->fRTPendingOpsMap[opList->uniqueID()];
    fOwner->fOpList.addToTail(this);
    fOwner->fMaxBufferItems.countPathItems(onlyDraw.fScissorMode, onlyDraw.fPath);
}
...
void GrCoverageCountingPathRenderer::preFlush(GrOnFlushResourceProvider* onFlushRP,
    ...
    while (DrawPathsOp* op = iter.get()) {
        ...
        for (const DrawPathsOp::SingleDraw* draw = &op->fHeadDraw; draw; draw = draw->fNext) {
            ...
            atlasOpsBuilder.saveParsedPath(offsetX, offsetY, draw->fClipBounds);
        }
...
void DrawPathsOp::onExecute(GrOpFlushState* flushState) {
    ...
    for (int i = 0; i < fAtlasBatches.count(); baseInstance = fAtlasBatches[i++].fEndInstanceIdx) {
...
#ifdef SK_DEBUG
        using Attrib = GrCCPRPathProcessor::Attrib;
        using Instance = GrCCPRPathProcessor::Instance;
        ...
        SkASSERT(offsetof(Instance, fViewMatrix) ==
                 coverProc.getAttrib(Attrib::kViewMatrix).fOffsetInRecord);
        SkASSERT(offsetof(Instance, fViewTranslate) ==
                 coverProc.getAttrib(Attrib::kViewTranslate).fOffsetInRecord);
        SkASSERT(offsetof(Instance, fAtlasOffset) ==
                 coverProc.getAttrib(Attrib::kAtlasOffset).fOffsetInRecord);
        SkASSERT(offsetof(Instance, fColor) ==
                 coverProc.getAttrib(Attrib::kColor).fOffsetInRecord);

        GR_STATIC_ASSERT(6 == GrCCPRPathProcessor::kNumAttribs);
#endif

        GrMesh mesh(GrPrimitiveType::kTriangleStrip);
        mesh.setInstanced(fCCPR->fPerFlushInstanceBuffer.get(),
                          batch.fEndInstanceIdx - baseInstance, baseInstance, 8);
