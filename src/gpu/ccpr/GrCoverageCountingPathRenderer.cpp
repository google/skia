...
CCPR::DrawPathsOp::DrawPathsOp(GrCoverageCountingPathRenderer* ccpr, const DrawPathArgs& args,
    ...
    ++fCCPR->fPendingDrawOpsCount;
    fBaseInstance = -1;
    fInstanceCount = 1;
    fNumSkippedInstances = 0;
...
void GrCoverageCountingPathRenderer::preFlush(GrOnFlushResourceProvider* onFlushRP,
    ...
    for (int i = 0; i < numOpListIDs; ++i) {
        ...
        while (DrawPathsOp* op = drawOpsIter.get()) {
            ...
            drawOpsIter.next();
            skippedTotalPaths += op->numSkippedInstances_debugOnly();
        }
