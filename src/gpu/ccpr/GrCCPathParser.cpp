...
void GrCCPathParser::saveParsedPath(ScissorMode scissorMode, const SkIRect& clippedDevIBounds,
    ...
    int tessellationWork = numVerbs * (32 - SkCLZ(numVerbs)); // Tessellation is N log N on average.
    int fanningWork = clippedDevIBounds.height() * clippedDevIBounds.width();
    if (tessellationWork * (50*50) + (100*100) < fanningWork) { // Don't tessellate under 100 x 100.
        ...
        // Build an SkPath of the triangle fan.
        SkPath fan;
