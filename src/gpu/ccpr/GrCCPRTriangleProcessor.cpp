...
void GrCCPRTriangleCornerProcessor::onEmitVertexShader(const GrCCPRCoverageProcessor& proc,
    ...
    // bloat width.
    v->codeAppend ("float2 n = float2(next.y - self.y, self.x - next.x);");
    v->codeAppendf("float2 d = n * float2x2(self + %f * sign(n), "
                                           "self - %f * sign(n));",
...
void GrCCPRTriangleCornerProcessor::onEmitGeometryShader(GrGLSLGeometryBuilder* g,
    ...
    g->codeAppend ("float2 self = in_vertices[sk_InvocationID];");
...
void GrCCPRTriangleCornerProcessor::emitShaderCoverage(GrGLSLFragmentBuilder* f,
    ...
    f->codeAppendf("float4 widths = max(%s - %s, 0);", rightedge.c_str(), leftedge.c_str());
