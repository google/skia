...
void GrCCPRTriangleProcessor::emitWind(GrGLSLGeometryBuilder* g, const char* /*rtAdjust*/,
    ...
    g->codeAppend("highp mat3x2 vertices = mat3x2(sk_in[0].gl_Position.xy, "
                                                 "sk_in[1].gl_Position.xy, "
                                                 "sk_in[2].gl_Position.xy);");
    g->codeAppendf("%s = sign(determinant(mat2(vertices[1] - vertices[0], "
                                              "vertices[2] - vertices[0])));", outputWind);
}
...
void GrCCPRTriangleHullAndEdgeProcessor::onEmitGeometryShader(GrGLSLGeometryBuilder* g,
    ...
    if (GeometryType::kEdges != fGeometryType) {
        // We defined "vertices" in emitWind.
        maxVertices += this->emitHullGeometry(g, emitVertexFn, "vertices", 3, "sk_InvocationID");
    }

    if (GeometryType::kHulls != fGeometryType) {
        g->codeAppend ("int edgeidx0 = sk_InvocationID, "
                           "edgeidx1 = (edgeidx0 + 1) % 3;");
        g->codeAppendf("highp vec2 edgept0 = vertices[%s > 0 ? edgeidx0 : edgeidx1];", wind);
        g->codeAppendf("highp vec2 edgept1 = vertices[%s > 0 ? edgeidx1 : edgeidx0];", wind);
