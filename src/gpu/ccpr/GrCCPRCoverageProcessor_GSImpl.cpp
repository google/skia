...
void GrCCPRCoverageProcessor::appendGSMesh(GrBuffer* instanceBuffer, int instanceCount,
    ...
    // GSImpl doesn't actually make instanced draw calls. Instead, we feed each primitive shape's
    // transposed x,y point values to the GPU in a regular vertex array as kLines (see initGS). Then, each
    // vertex invocation receives either the x or y values as inputs, which it forwards to the
    // geometry shader.
