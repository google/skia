...
public:
    ...
    void onEmitGeometryShader(GrGLSLGeometryBuilder* g, const GrShaderVar& wind,
        ...
        // attention to wind, we can identify the points as top, (bottom) right, and (bottom) left.
        //
        ...
        //   sk_InvocationID=1: Finishes the remaining two vertices for the third corner.
        g->codeAppendf("bool2 right_down_notequal = notEqual(rightbloat, downbloat);");
        g->codeAppend ("if (any(right_down_notequal) || sk_InvocationID == 0) {");
        g->codeAppendf(    "%s(sk_InvocationID == 0 ? left + leftbloat : right + downbloat, 1);",
                           emitVertexFn);
        g->codeAppend ("}");
        g->codeAppend ("if (all(right_down_notequal) && sk_InvocationID != 0) {");
...
public:
    ...
    void onEmitGeometryShader(GrGLSLGeometryBuilder* g, const GrShaderVar& wind,
        ...
        // Remaining two vertices for the top-right corner.
