...
// have a longer perimeter to resolve.
//
// Returns the next triangle instance after the final one emitted.
static TriangleInstance* emit_hierarchical_fan(int32_t indices[], int count, int packedAtlasOffset,
    ...
    *out++ = {
    ...
    out = emit_hierarchical_fan(indices, oneThirdPt + 1, packedAtlasOffset, out);
    out = emit_hierarchical_fan(&indices[oneThirdPt], twoThirdsPt - oneThirdPt + 1,
                                packedAtlasOffset, out);

    int32_t oldIndex = indices[count];
    indices[count] = indices[0];
    out = emit_hierarchical_fan(&indices[twoThirdsPt], count - twoThirdsPt + 1, packedAtlasOffset,
                                out);
...
bool GrCCPRCoverageOpsBuilder::finalize(GrOnFlushResourceProvider* onFlushRP,
        ...
        switch (verb) {
            ...
            case GrCCPRGeometry::Verb::kEndOpenContour: // endPt != startPt.
                if (currFan.count() >= 3) {
                    ...
                    // Reserve space for emit_hierarchical_fan. Technically this can grow to
                    // fanSize + log3(fanSize), but we approximate with log2.
                    currFan.push_back_n(SkNextLog2(fanSize));
                    SkDEBUGCODE(TriangleInstance* end =)
                    emit_hierarchical_fan(currFan.begin(), fanSize, packedAtlasOffset,
    ...
    SkASSERT(instanceIndices[1].fLoops * sizeof(CurveInstance) == instanceBufferSize);
    return true;
}
