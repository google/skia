...
bool GrCCPRCoverageOpsBuilder::finalize(GrOnFlushResourceProvider* onFlushRP,
    ...
    // CubicInstances. (Cubics index the same instance buffer as a different type of struct.)
    int cubicBaseIdx = (triEndIdx * sizeof(TriangleInstance) + sizeof(CubicInstance) - 1) /
    ...
    for (GrCCPRGeometry::Verb verb : fGeometry.verbs()) {
        switch (verb) {
            case GrCCPRGeometry::Verb::kBeginPath:
                ...
                atlasOffset = Sk2f(atlasOffsetX, atlasOffsetY);
#ifdef SK_DEBUG
