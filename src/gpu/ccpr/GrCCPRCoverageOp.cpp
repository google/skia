...
void GrCCPRCoverageOpsBuilder::parsePath(const SkMatrix& m, const SkPath& path, SkRect* devBounds,
    ...
    SkASSERT(numPts + 1 <= fTransformedPathPoints.count());
    ...
    devPt.store(&fTransformedPathPoints[0]);
    ...
    for (int i = 1; i < numPts; ++i) {
        ...
        devPt.store(&fTransformedPathPoints[i]);
    }
    ...
    this->parsePath(path, fTransformedPathPoints.get());
}
