...
static TriangleInstance* emit_recursive_fan(int32_t indices[], int count, int packedAtlasOffset,
    ...
    *out++ = {
    ...
    out = emit_recursive_fan(indices, oneThirdPt + 1, packedAtlasOffset, out);
    out = emit_recursive_fan(&indices[oneThirdPt], twoThirdsPt - oneThirdPt + 1, packedAtlasOffset,
                             out);

    int32_t oldIndex = indices[count];
    indices[count] = indices[0];
    out = emit_recursive_fan(&indices[twoThirdsPt], count - twoThirdsPt + 1, packedAtlasOffset,
                             out);
        ...
        switch (verb) {
            ...
            case GrCCPRGeometry::Verb::kEndOpenContour: // endPt != startPt.
                if (currFan.count() >= 3) {
                    ...
                    emit_recursive_fan(currFan.begin(), fanSize, packedAtlasOffset,
