...
public:
    ...
    enum class Verb : uint8_t {
        ...
        // half.) Miters have an additional triangle cap on top of the bevel, and round joins have
        // an additional conic on top.
        kBevelJoin,
        kMiterJoin,
        kRoundJoin,
    ...
    void exitContour();  // Add endcaps (if any) and exit the contour.

private:
    void lineTo(SkPaint::Join leftJoin, const SkPoint&);
    ...
    void rotateTo(SkPaint::Join leftJoin, const SkVector& normal);
    ...
    void recordLeftJoinIfNotEmpty(SkPaint::Join, const SkVector& nextNormal);
