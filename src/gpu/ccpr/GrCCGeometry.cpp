...
//                                                       |C0x  C0y|
//
// This function finds the T value whose tangent angle is halfway between tan0 and tan1.
static inline float find_midtangent(const Sk2f& tan0, const Sk2f& tan1,
    ...
    // 'n' is normal to the the tangent line whose angle is halfway between tan0 and tan1
    // (a.k.a. the midtangent):
    //
    //     n dot midtangent = 0
    //
    Sk2f n = normalize(tan0) - normalize(tan1);
...
void GrCCGeometry::conicTo(const SkPoint P[3], float w) {
    ...
    if (!(midT > 0 && midT < 1)) {
        // The conic flat. Otherwise there would be a real midtangent inside T=0..1.
        this->appendLine(p2);
        return;
    }
    ...
    if (are_collinear(p0, midpoint, p2, 1) || // Check if the curve is within one pixel of flat.
        ...
        // gets wrapped too tight, and if we get too close to p1 we will pick up artifacts from the
        // implicit function's reflection.
    ...
    if (!is_convex_curve_monotonic(p0, tan0, p2, tan1)) {
        ...
        Sk2f choppedWs = Sk2f(p3d01[2], p3d12[2]) * Sk2f(p3d012[2]).rsqrt();
        this->appendMonotonicConic(p0, Sk2f(p3d01[0], p3d01[1]) / p3d01[2], midpoint, choppedWs[0]);
        this->appendMonotonicConic(midpoint, Sk2f(p3d12[0], p3d12[1]) / p3d12[2], p2, choppedWs[1]);
        return;
    }
