...
GrCCPRClipProcessor::GrCCPRClipProcessor(const ClipPath* clipPath, bool mustCheckBounds,
                                         SkPath::FillType overrideFillType)
        : INHERITED(kCCPRClipProcessor_ClassID, kCompatibleWithCoverageAsAlpha_OptimizationFlag)
        , fClipPath(clipPath)
        , fMustCheckBounds(mustCheckBounds)
        , fOverrideFillType(overrideFillType) {
    fAtlasAccess.reset(sk_ref_sp(fClipPath->atlasLazyProxy()), GrSamplerState::Filter::kNearest,
                       GrSamplerState::WrapMode::kClamp, kFragment_GrShaderFlag);
    this->addTextureSampler(&fAtlasAccess);
}

std::unique_ptr<GrFragmentProcessor> GrCCPRClipProcessor::clone() const {
    return skstd::make_unique<GrCCPRClipProcessor>(fClipPath, fMustCheckBounds, fOverrideFillType);
}

void GrCCPRClipProcessor::onGetGLSLProcessorKey(const GrShaderCaps&,
                                                GrProcessorKeyBuilder* b) const {
    b->add32((fOverrideFillType << 1) | fMustCheckBounds);
}
...
bool GrCCPRClipProcessor::onIsEqual(const GrFragmentProcessor& fp) const {
    ...
    // Each device space path has its own unique proxy, so FPs with different paths should have been
    // weeded before reaching this point due to unequal texture proxies.
...
public:
    void emitCode(EmitArgs& args) override {
        ...
        if (SkPath::kWinding_FillType == proc.fOverrideFillType ||
            SkPath::kInverseWinding_FillType == proc.fOverrideFillType) {
    ...
    void onSetData(const GrGLSLProgramDataManager& pdman,
        ...
        const ClipPath* clipPath = proc.fClipPath;
        if (proc.fMustCheckBounds) {
            const SkRect pathIBounds = SkRect::Make(clipPath->pathDevIBounds());
            pdman.set4f(fPathIBoundsUniform, pathIBounds.left(), pathIBounds.top(),
                        pathIBounds.right(), pathIBounds.bottom());
        }
        const SkVector& scale = clipPath->atlasScale();
        const SkVector& trans = clipPath->atlasTranslate();
