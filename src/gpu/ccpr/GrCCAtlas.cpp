...
bool GrCCAtlas::addRect(const SkIRect& devIBounds, SkIVector* offset) {
    // This can't be called anymore once makeTextureProxy() has been called.
...
sk_sp<GrRenderTargetContext> GrCCAtlas::makeClearedTextureProxy(
    ...
    sk_sp<GrRenderTargetContext> rtc =
            onFlushRP->makeRenderTargetContext(desc, kOrigin, nullptr, nullptr);
    if (!rtc) {
...
GrCCAtlas* GrCCAtlasStack::addRect(const SkIRect& devIBounds, SkIVector* offset) {
    ...
    if (fAllocator.empty() || !fAllocator.back().addRect(devIBounds, offset)) {
        // The retired atlas is out of room and can't grow any bigger.
        retiredAtlas = !fAllocator.empty() ? &fAllocator.back() : nullptr;
        fAllocator.emplace_back(fSpecs);
        SkASSERT(devIBounds.width() <= fSpecs.fMinWidth);
        SkASSERT(devIBounds.height() <= fSpecs.fMinHeight);
        SkAssertResult(fAllocator.back().addRect(devIBounds, offset));
    }
    return retiredAtlas;
}
