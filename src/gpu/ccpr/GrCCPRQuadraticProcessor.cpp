...
void GrCCPRQuadraticHullProcessor::emitQuadraticGeometry(GrGLSLGeometryBuilder* g,
    ...
    g->codeAppend ("highfloat2 T = midnorm * float2x2(tan0 - tan1, tan0);");
...
void GrCCPRQuadraticCornerProcessor::emitShaderCoverage(GrGLSLFragmentBuilder* f,
    ...
    f->codeAppendf("highfloat2x3 grad_xyd = float2x3(%s, %s);", fdXYDdx.fsIn(), fdXYDdy.fsIn());
    ...
    f->codeAppend ("highfloat2 grad_f = float2(2*x, -1) * highfloat2x2(grad_xyd);");
