...
void GrCCPRQuadraticProcessor::emitPerVertexGeometryCode(SkString* fnBody, const char* position,
    ...
    fnBody->appendf("%s.zw = highfloat2(2 * %s.x * %s[0].x - %s[0].y, "
                                       "2 * %s.x * %s[1].x - %s[1].y);",
...
void GrCCPRQuadraticProcessor::emitShaderCoverage(GrGLSLFragmentBuilder* f,
    ...
    f->codeAppendf("highfloat d = (%s.x * %s.x - %s.y) * inversesqrt(dot(%s.zw, %s.zw));",
...
void GrCCPRQuadraticHullProcessor::emitQuadraticGeometry(GrGLSLGeometryBuilder* g,
    ...
    g->codeAppend ("highfloat2 n = (normalize(bezierpts[0] - bezierpts[1]) + "
                                   "normalize(bezierpts[2] - bezierpts[1]));");
    g->codeAppend ("highfloat t = dot(bezierpts[0] - bezierpts[1], n) / "
                                     "dot(bezierpts[2] - 2 * bezierpts[1] + bezierpts[0], n);");
    g->codeAppend ("highfloat2 pt = (1 - t) * (1 - t) * bezierpts[0] + "
                                   "2 * t * (1 - t) * bezierpts[1] + "
                                   "t * t * bezierpts[2];");

    // Clip the triangle by the tangent line at this halfway point.
    g->codeAppend ("highfloat2x2 v = highfloat2x2(bezierpts[0] - bezierpts[1], "
                                                 "bezierpts[2] - bezierpts[1]);");
    g->codeAppend ("highfloat2 nv = n * v;");
    g->codeAppend ("highfloat2 d = abs(nv[0]) > 0.1 * max(bloat.x, bloat.y) ? "
                                    "(dot(n, pt - bezierpts[1])) / nv : highfloat2(0);");
...
void GrCCPRQuadraticSharedEdgeProcessor::emitQuadraticGeometry(GrGLSLGeometryBuilder* g,
    ...
    g->codeAppendf("highfloat2 left = bezierpts[leftidx];");
    g->codeAppendf("highfloat2 right = bezierpts[2 - leftidx];");
    this->emitEdgeDistanceEquation(g, "left", "right", "highfloat3 edge_distance_equation");
...
void GrCCPRQuadraticSharedEdgeProcessor::emitShaderCoverage(GrGLSLFragmentBuilder* f,
    ...
    f->codeAppendf("highfloat2x3 grad_xyd = highfloat2x3(%s[0],%s.y, %s[1],%s.z);",
                   fFragCanonicalDerivatives.fsIn(), fEdgeDistance.fsIn(),
                   fFragCanonicalDerivatives.fsIn(), fEdgeDistance.fsIn());
    f->codeAppendf("highfloat3 center_xyd = highfloat3(%s.xy, %s.x);",
                   fCanonicalCoord.fsIn(), fEdgeDistance.fsIn());

    f->codeAppendf("for (int i = 0; i < %i; ++i) {", sampleCount);
    f->codeAppend (    "highfloat3 xyd = grad_xyd * samples[i] + center_xyd;");
    f->codeAppend (    "half f = xyd.x * xyd.x - xyd.y;"); // f > 0 -> outside curve.
    f->codeAppend (    "bool2 outside_curve_inside_edge = greaterThan(half2(f, xyd.z), half2(0));");
