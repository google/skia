...
void GrCCPRQuadraticProcessor::onEmitVertexShader(const GrCCPRCoverageProcessor& cp,
                                                  GrGLSLVertexBuilder* v, const char* rtAdjustName,
                                                  const TexelBufferHandle& pointsBuffer,
                                                  GrGPArgs* gpArgs) const {
    v->codeAppendf("ivec3 vertex_indices = ivec3(%s.w, %s.z, %s.w + 1);",
                   cp.instanceAttrib(), cp.instanceAttrib(), cp.instanceAttrib());
...
void GrCCPRQuadraticProcessor::emitWind(GrGLSLGeometryBuilder* g, const char* windName,
    ...
    g->codeAppend ("highp float area_times_2 = determinant(mat2(P[1] - P[0], P[2] - P[0]));");
    // Drop curves that are nearly flat, in favor of the higher quality triangle antialiasing.
    g->codeAppendf("if (2 * abs(area_times_2) < length((P[2] - P[0]) * %s.zx)) {", rtAdjustName);
...
#endif
    g->codeAppend ("}");
    g->codeAppendf("%s = sign(area_times_2);", windName);
}
...
void GrCCPRQuadraticProcessor::onEmitGeometryShader(GrGLSLGeometryBuilder* g,
    ...
    g->codeAppendf("%s = mat3(0.0, 0, 1, "
                             "0.5, 0, 1, "
                             "1.0, 1, 1) * "
                        "inverse(mat3(P[0],1, P[1],1, P[2],1));", fCanonicalMatrix.c_str());

    g->declareGlobal(fCanonicalDerivatives);
    g->codeAppendf("%s = mat2(%s) * mat2(%s.x, 0, 0, %s.z);",
                   fCanonicalDerivatives.c_str(), fCanonicalMatrix.c_str(), rtAdjustName,
                   rtAdjustName);

    this->emitQuadraticGeometry(g, emitVertexFn, wind, rtAdjustName);
}
...
void GrCCPRQuadraticHullProcessor::emitQuadraticGeometry(GrGLSLGeometryBuilder* g,
    ...
    g->codeAppendf("highp vec2 n = (normalize(P[0] - P[1]) + normalize(P[2] - P[1]));");
    g->codeAppendf("highp float t = dot(P[0] - P[1], n) / dot(P[2] - 2*P[1] + P[0], n);");
    g->codeAppend ("highp vec2 pt = (1 - t) * (1 - t) * P[0] + "
                                   "2 * t * (1 - t) * P[1] + "
                                   "t * t * P[2];");
    ...
    int maxVertices = this->emitHullGeometry(g, 4, "sk_InvocationID", emitVertexFn, "hull");
...
void GrCCPRQuadraticSharedEdgeProcessor::emitQuadraticGeometry(GrGLSLGeometryBuilder* g,
    ...
    g->codeAppendf("highp vec2 left = P[leftidx];");
    g->codeAppendf("highp vec2 right = P[2 - leftidx];");
    ...
    g->codeAppendf("%s = edge_distance_equation.xy * %s.xz;",
                   fEdgeDistanceDerivatives.c_str(), rtAdjustName);
...
void GrCCPRQuadraticSharedEdgeProcessor::emitPerVertexGeometryCode(SkString* fnBody,
    ...
    fnBody->appendf("%s.x = coverage + 0.5;", fEdgeDistance.gsOut()); // outer = -.5, inner = +.5.
...
void GrCCPRQuadraticSharedEdgeProcessor::emitShaderCoverage(GrGLSLFragmentBuilder* f,
    ...
    int sampleCount = this->definePseudoSampleLocations(f, "samples");
