...
private:
    ...
    enum class AtlasPtrType {
        kGrCCAtlas,
        kGrTextureProxy
    };

    // FIXME!! doing it this way because GrCCAtlases don't have their texture proxy yet. However we
    // should just give them lazy proxies and atlasPtr can just always be GrTextureProxy.
    // But you need a proxyProvider to create a fully lazy proxy???
    void recordInstance(AtlasPtrType, const void* atlasPtr, int instanceIdx);
    ...
    struct InstanceRange {
        AtlasPtrType fAtlasPtrType;
        const void* fAtlasPtr;
        int fEndInstance;
    };
