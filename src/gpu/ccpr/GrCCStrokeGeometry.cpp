...
void GrCCStrokeGeometry::lineTo(SkPaint::Join leftJoinType, SkPoint pt) {
...
void GrCCStrokeGeometry::rotateTo(SkPaint::Join leftJoinType, SkVector normal) {
    this->recordLeftJoinIfNotEmpty(leftJoinType, normal);
    fNormals.push_back(normal);
}

void GrCCStrokeGeometry::recordLeftJoinIfNotEmpty(SkPaint::Join joinType, SkVector nextNormal) {
    ...
    Sk2f dot = base * n0;
    float numer = SkScalarAbs(dot[0] + dot[1]);
    float denom = SkScalarAbs(cross[0] - cross[1]) * 2;
    static constexpr float kNearlyInfinite = 64;
    float miterCapHeightOverWidth = (numer > denom * kNearlyInfinite)
            ? kNearlyInfinite : numer/denom;
...
void GrCCStrokeGeometry::closeContour() {
    ...
    if (fPoints.back() != fPoints[fCurrContourFirstPtIdx]) {
        ...
        this->lineTo(fCurrStrokeJoinType, fPoints[fCurrContourFirstPtIdx]);
    }
    if (fNormals.count() > fCurrContourFirstNormalIdx) {
        // Join the first and last lines.
        this->rotateTo(fCurrStrokeJoinType,fNormals[fCurrContourFirstNormalIdx]);
    } else {
