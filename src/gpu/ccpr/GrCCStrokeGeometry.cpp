...
void GrCCStrokeGeometry::recordLeftJoinIfNotEmpty(SkPaint::Join joinType, SkVector nextNormal) {
    ...
    Sk2f dot = base * n0;
    float numer = dot[0] + dot[1];
    float denom = (cross[0] - cross[1]) * 2;
    float miterCapHeightOverWidth = (SkScalarAbs(numer) * SK_ScalarNearlyZero > SkScalarAbs(denom))
            ? 1/SK_ScalarNearlyZero : numer/denom;
