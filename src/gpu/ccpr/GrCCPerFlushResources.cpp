...
void GrCCPerFlushResources::upgradeEntryToLiteralCoverageAtlas(
    ...
    SkASSERT(cachedAtlas->onFlushProxy());
    ...
    this->recordCopyPathInstance(*entry, newAtlasOffset, evenOdd,
                                 sk_ref_sp(cachedAtlas->onFlushProxy()));

    sk_sp<GrTexture> previousAtlasTexture = sk_ref_sp(cachedAtlas->onFlushProxy()->peekTexture());
...
static void emplace_at(SkTArray<GrCCPerFlushResources::CopyPathRange>* array, int idx,
                       sk_sp<GrTextureProxy> srcProxy, int rangeCount) {
    using CopyPathRange = GrCCPerFlushResources::CopyPathRange;

    if (int moveCount = array->count() - idx) {
        array->push_back();
        CopyPathRange* location = array->begin() + idx;
        memcpy(location+1, location, moveCount * sizeof(CopyPathRange));
        new (location) CopyPathRange(std::move(srcProxy), 1);
    } else {
        array->emplace_back(std::move(srcProxy), 1);
    }
}
...
void GrCCPerFlushResources::recordCopyPathInstance(const GrCCPathCacheEntry& entry,
    ...
    emplace_at(&fCopyPathRanges, fCurrCopyAtlasRangesIdx, std::move(srcProxy), 1);
}
