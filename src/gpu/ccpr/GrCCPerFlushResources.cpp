...
public:
    ...
    void onExecute(GrOpFlushState* flushState, const SkRect& chainBounds) override {
        ...
        GrCCPathProcessor pathProc(
                drawMode, srcProxy->peekTexture(), srcProxy->textureSwizzle(), srcProxy->origin());
...
GrCCPerFlushResources::GrCCPerFlushResources(
        ...
        , fNextRenderedPathInstanceIdx(
                ...
                specs.fNumCopiedPaths[kFillIdx] + specs.fNumCopiedPaths[kStrokeIdx])
        , fBaseRenderedPathInstanceIdx(fNextRenderedPathInstanceIdx) {
    if (!fIndexBuffer) {
...
void GrCCPerFlushResources::placeRenderedPathInAtlas(
    ...
    if (GrCCAtlas* retiredAtlas =
        ...
        retiredAtlas->setPathEndInstanceIdx(fNextRenderedPathInstanceIdx);
    }
}
...
bool GrCCPerFlushResources::finalize(GrOnFlushResourceProvider* onFlushRP,
    ...
    if (!fRenderedAtlasStack.empty()) {
        ...
        fRenderedAtlasStack.current().setPathEndInstanceIdx(fNextRenderedPathInstanceIdx);
    }
    ...
    int baseRenderedPathInstance = fBaseRenderedPathInstanceIdx;
    for (GrCCAtlasStack::Iter atlas(fRenderedAtlasStack); atlas.next();) {
        ...
        if (auto rtc = atlas->makeRenderTargetContext(onFlushRP, std::move(backingTexture))) {
            ...
            if (CoverageType::kA8_Multisample == fRenderedAtlasStack.coverageType()) {
                op = StencilAtlasOp::Make(
                        ...
                        atlas->getStrokeBatchID(), baseRenderedPathInstance,
                        atlas->getPathEndInstanceIdx(), atlas->drawBounds());
        ...
        SkASSERT(atlas->getPathEndInstanceIdx() >= baseRenderedPathInstance);
        baseRenderedPathInstance = atlas->getPathEndInstanceIdx();
    }
    SkASSERT(baseRenderedPathInstance == fEndRenderedPathInstance);
