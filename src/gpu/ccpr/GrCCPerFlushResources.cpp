...
int GrCCPerFlushResources::appendRenderedPathInstance(
        const GrOctoBounds& octoBounds, const SkIVector& devToAtlasOffset, uint64_t color,
        GrCCPathProcessor::DoEvenOddFill doEvenOddFill) {
    SkASSERT(this->isMapped());
    SkASSERT(fNextPathInstanceIdx < fEndPathInstance);
    fPathInstanceData[fNextPathInstanceIdx].set(
            octoBounds, devToAtlasOffset, color, doEvenOddFill);
    ...
    if (GrCCAtlas::CoverageType::kA8_Multisample == this->renderedPathCoverageType()) {
        ...
        SkRect atlasBounds = octoBounds.bounds().makeOffset(
                devToAtlasOffset.x(), devToAtlasOffset.y());
        SkIRect ibounds;
        atlasBounds.roundOut(&ibounds);
        if (GrCCPathProcessor::DoEvenOddFill::kYes == doEvenOddFill) {
            ...
            std::swap(ibounds.fLeft, ibounds.fRight);
        }
        fStencilResolveInstanceData[fNextStencilResolveInstanceIdx++] = {
            (int16_t)ibounds.left(), (int16_t)ibounds.top(), (int16_t)ibounds.right(),
            (int16_t)ibounds.bottom()};
    }
