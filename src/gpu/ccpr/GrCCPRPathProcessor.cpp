...
void GLSLPathProcessor::onEmitCode(EmitArgs& args, GrGPArgs* gpArgs) {
    ...
    // octagon cover geometry. A vertex is the intersection of two lines: one edge from the path's
    // bounding box and one edge from its 45-degree bounding box.
    v->codeAppendf("highp mat2 N = mat2(%s);", proc.getEdgeNormsAttrib().fName);

    // N[0] is the normal for the edge we are intersecting from the regular bounding box, pointing
    // out of the octagon.
    v->codeAppendf("highp vec2 refpt = mat2(%s)[max(N[0].x, N[0].y) > 0 ? 1 : 0];",
    ...
    // out of the octagon.
    v->codeAppendf("highp vec2 refpt45 = mat2(%s)[N[1].x > 0 ? 1 : 0];",
