...
GrCCPRPathProcessor::GrCCPRPathProcessor(GrResourceProvider* rp, sk_sp<GrTextureProxy> atlas,
    ...
    SkASSERT(offsetof(Instance, fDevBounds) == this->getAttrib(Attrib::kDevBounds).fOffsetInRecord);
    SkASSERT(offsetof(Instance, fDevBounds45) ==
             this->getAttrib(Attrib::kDevBounds45).fOffsetInRecord);
    SkASSERT(offsetof(Instance, fViewMatrix) ==
             this->getAttrib(Attrib::kViewMatrix).fOffsetInRecord);
    SkASSERT(offsetof(Instance, fViewTranslate) ==
             this->getAttrib(Attrib::kViewTranslate).fOffsetInRecord);
    SkASSERT(offsetof(Instance, fAtlasOffset) ==
             this->getAttrib(Attrib::kAtlasOffset).fOffsetInRecord);
    SkASSERT(offsetof(Instance, fColor) == this->getAttrib(Attrib::kColor).fOffsetInRecord);
...
void GLSLPathProcessor::onEmitCode(EmitArgs& args, GrGPArgs* gpArgs) {
    ...
    varyingHandler->addFlatPassThroughAttribute(&proc.getAttrib(Attrib::kColor), args.fOutputColor,
                                                kLow_GrSLPrecision);
    ...
    v->codeAppendf("highp vec2 atlascoord = octocoord + vec2(%s);",
                   proc.getAttrib(Attrib::kAtlasOffset).fName);
    ...
    v->codeAppendf("highp vec2 pathcoord = inverse(mat2(%s)) * (octocoord - %s);",
                   proc.getAttrib(Attrib::kViewMatrix).fName,
                   proc.getAttrib(Attrib::kViewTranslate).fName);
