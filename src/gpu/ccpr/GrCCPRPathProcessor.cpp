...
GrCCPRPathProcessor::GrCCPRPathProcessor(GrResourceProvider* rp, sk_sp<GrTextureProxy> atlas,
    ...
    GR_STATIC_ASSERT(0 == int(Attrib::kDevBounds));
    GR_STATIC_ASSERT(1 == int(Attrib::kDevBounds45));
    GR_STATIC_ASSERT(2 == int(Attrib::kViewMatrix));
    GR_STATIC_ASSERT(3 == int(Attrib::kViewTranslate));
    GR_STATIC_ASSERT(4 == int(Attrib::kAtlasOffset));
    GR_STATIC_ASSERT(5 == int(Attrib::kColor));
    GR_STATIC_ASSERT(6 == kNumAttribs);
...
private:
    void setData(const GrGLSLProgramDataManager& pdman, const GrPrimitiveProcessor& primProc,
        ...
        const GrCCPRPathProcessor& pp = primProc.cast<GrCCPRPathProcessor>();
        pdman.set2f(fAtlasAdjustUniform, 1.0f / pp.atlas()->width(), 1.0f / pp.atlas()->height());
...
void GLSLPathProcessor::onEmitCode(EmitArgs& args, GrGPArgs* gpArgs) {
    ...
    const GrCCPRPathProcessor& pp = args.fGP.cast<GrCCPRPathProcessor>();
    ...
    varyingHandler->emitAttributes(pp);
    ...
    varyingHandler->addFlatPassThroughAttribute(&pp.getAttrib(Attrib::kColor), args.fOutputColor,
                                                kLow_GrSLPrecision);
    ...
    v->codeAppendf("highp vec2 atlascoord = octocoord + vec2(%s);",
                   pp.getAttrib(Attrib::kAtlasOffset).fName);
    if (kTopLeft_GrSurfaceOrigin == pp.atlas()->origin()) {
        v->codeAppendf("%s = atlascoord * %s;", texcoord.vsOut(), atlasAdjust);
    } else {
        SkASSERT(kBottomLeft_GrSurfaceOrigin == pp.atlas()->origin());
    ...
    v->codeAppendf("highp vec2 pathcoord = inverse(mat2(%s)) * (octocoord - %s);",
                   pp.getAttrib(Attrib::kViewMatrix).fName,
                   pp.getAttrib(Attrib::kViewTranslate).fName);
    ...
    if (SkPath::kWinding_FillType == pp.fillType()) {
        f->codeAppendf("%s = vec4(min(abs(coverage_count), 1));", args.fOutputCoverage);
    } else {
        SkASSERT(SkPath::kEvenOdd_FillType == pp.fillType());
