    ...
    */
    ...
    bool usesLocalCoords() const {
        ...
        return SkToBool(fFlags & kHasCoordTranforms_Flag) &&
               SkToBool(fFlags & kApplyCoordTransformsInVertexShader_Flag);
    }

    bool applyCoordTransformsInVertexShader() const {
        return SkToBool(fFlags & kApplyCoordTransformsInVertexShader_Flag);
    }

    void setSampledWithExplicitCoords(bool value) {
        if (value) {
            fFlags &= ~kApplyCoordTransformsInVertexShader_Flag;
        } else {
            fFlags |= kApplyCoordTransformsInVertexShader_Flag;
        }
    ...
    public:
        ...
        const GrFragmentProcessor* currentFP() const { return fCurrFP; }
...
protected:
    ...
    GrFragmentProcessor(ClassID classID, OptimizationFlags optimizationFlags)
            : INHERITED(classID)
            , fFlags(optimizationFlags | kApplyCoordTransformsInVertexShader_Flag) {
...
private:
    ...
    enum PrivateFlags {
        ...
        kApplyCoordTransformsInVertexShader_Flag = kFirstPrivateFlag << 1,
    };

    uint32_t fFlags = kApplyCoordTransformsInVertexShader_Flag;
