...
public:
    ...
    */
    ...
    /**
     *  Returns a fragment processor that returns the passed-in color modulated by the child's
     *  alpha. (Pass a null FP to use the alpha from sk_InColor instead of a child FP.)
     */
    static std::unique_ptr<GrFragmentProcessor> ModulateAlpha(
            const SkPMColor4f& color, std::unique_ptr<GrFragmentProcessor> child);
