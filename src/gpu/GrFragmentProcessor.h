...
public:
    ...
    */
    ...
    // Like CIterRange but non const and only constructable from GrFragmentProcessor.
    ...
    using FPCRange    = CIterRange<const GrFragmentProcessor>;
    using PaintCRange = CIterRange<GrPaint>;

    // Implementation details for iterators that walk an array of things owned by a set of FPs.
    using CountFn = int (GrFragmentProcessor::*)() const;
    template <typename Item, bool IsConst> struct GetT;
    ...
    template <typename Item, bool IsConst> using GetFn = typename GetT<Item, IsConst>::GetFn;
    template <typename Item, CountFn Count, GetFn<Item,std::is_const_v<Item>> Get> class FPItemIter;
    ...
    using NonConstCoordTransformIter = FPItemIter<GrCoordTransform, &GrFragmentProcessor::numCoordTransforms, &GrFragmentProcessor::coordTransform>;
...
protected:
    ...
    /** Protected non-const version of childProcessor(). */
...
public:
    ...
    Iter& operator++() { this->increment(); return *this; }
};
...
public:
    ...
    CIter& operator++() { this->increment(); return *this; }
};
...
template <typename Item, GrFragmentProcessor::CountFn Count, GrFragmentProcessor::GetFn<Item, std::is_const_v<Item>> Get>
...
private:
    typename std::conditional<std::is_const_v<Item>, CIter, Iter>::type fFPIter;
    int fIndex;
};

template <typename Item, GrFragmentProcessor::CountFn Count, GrFragmentProcessor::GetFn<Item, std::is_const_v<Item>> Get>
template <typename Src>
GrFragmentProcessor::FPItemIter<Item, Count, Get>::FPItemIter(Src& s)
        : fFPIter(s), fIndex(-1) {
    if (fFPIter) {
        ++*this;
    }
