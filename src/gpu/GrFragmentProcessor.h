...
public:
    ...
    */
     ...
     * processor. This can return true even if this FP does not refer to local coordinates, but
     * true if a child FP (or grand child) uses them.
     *
     * FPs that are sampled explicitly do not require primitive-generated local coordinates.
     */
    ...
    /**
     * True if this FP refers directly to the local coordinate paremeter of its function
     * (e.g. uses EmitArgs::fLocalCoord in emitCode()). This also returns true if the direct
     * local coordinate reference comes from autogenerated code invoking 'sample(matrix)'
     * expressions.
     *
     * Unlike requiresPrimitiveLocalCoords(), this can return true whether or not the FP is
     * explicitly sampled.
     */
    bool referencesLocalCoords() const {
        if (SkToBool(fFlags & kUsesLocalCoordsDirectly_Flag) || fCoordTransforms.count() > 0) {
            ...
            // then it needs the default local coords to kick it off.
            return !fParent && this->isSampledWithExplicitCoords();
        }
    }
...
protected:
    ...
    // emitCode() function uses the EmitArgs::fLocalCoord variable in generated SkSL.
    void setUsesLocalCoordsDirectly() {
        fFlags |= kUsesLocalCoordsDirectly_Flag;
    }
...
private:
    ...
    void notifyParentHasPerspective();

    enum PrivateFlags {
        kFirstPrivateFlag = kAll_OptimizationFlags + 1,
        kHasCoordTransforms_Flag = kFirstPrivateFlag,
        kSampledWithExplicitCoords = kFirstPrivateFlag << 1,
        kUsesLocalCoordsDirectly_Flag = kFirstPrivateFlag << 2,
        kUsesLocalCoordsIndirectly_Flag = kFirstPrivateFlag << 3
    };
