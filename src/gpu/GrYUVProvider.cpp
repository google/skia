...
sk_sp<GrTextureProxy> GrYUVProvider::refAsTextureProxy(GrContext* ctx, const GrSurfaceDesc& desc,
    ...
    for (int i = 0; i < 3; i++) {
        ...
        // Grabbed a ref to cached yuv data. When the SkImage we create below goes away it will call
        // a call
        dataStoragePtr->ref();
        sk_sp<SkImage> yuvImage = SkImage::MakeFromRaster(pixmap, YUVGen_DataReleaseProc,
                                                          dataStoragePtr);

        yuvTextureProxies[i] = ctx->contextPriv().proxyProvider()->createTextureProxy(yuvImage, kNone_GrSurfaceFlags,
