...
sk_sp<GrTextureProxy> GrYUVProvider::refAsTextureProxy(GrRecordingContext* ctx,
    ...
    for (int i = 0; i < SkYUVASizeInfo::kMaxCount; ++i) {
        ...
        // We grab a ref to cached yuv data. When the SkBitmap we create below goes away it will call
        ...
        GrColorType grCT;
        std::tie(yuvTextureProxies[i], grCT) = maker.refTextureProxy(GrMipMapped::kNo);
