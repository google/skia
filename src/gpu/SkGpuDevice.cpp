...
void SkGpuDevice::drawSpecial(SkSpecialImage* special, int left, int top, const SkPaint& paint,
    ...
    if (clipImage) {
        ...
        auto cfp = GrSimpleTextureEffect::Make(std::move(clipProxy), inverseClipMatrix, sampler);
        ...
        // Else fall through and draw the subset since that is a contained in the clip geometry
    }
