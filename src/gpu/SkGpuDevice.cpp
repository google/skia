...
SkGpuDevice::SkGpuDevice(GrContext* context,
        ...
        , fClip(fRenderTargetContext->dimensions(), &this->cs()) {
...
void SkGpuDevice::drawStrokedLine(const SkPoint points[2],
    ...
    GrPaint grPaint;
    if (!SkPaintToGrPaint(this->context(), fRenderTargetContext->colorInfo(), newPaint,
                          matrixProvider, &grPaint)) {
        return;
    }
    ...
    if (clipImage) {
        ...
        if (cfp) {
            ...
            if (!clipGeometry.contains(inverseClipMatrix.mapRect(dstRect))) {
                ...
                dstRectClip.clipDevRect(SkIRect::MakeXYWH(left, top, subset.width(), subset.height()),
                                 SkClipOp::kIntersect);
