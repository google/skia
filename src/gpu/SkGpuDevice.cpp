...
void SkGpuDevice::drawBitmapTile(const SkBitmap& bitmap,
    ...
    if (needsTextureDomain && (SkCanvas::kStrict_SrcRectConstraint == constraint)) {
        ...
        if (bicubic) {
            ...
            fp = GrSimpleTextureEffect::Make(std::move(proxy), srcAlphaType, SkBlendMode::kModulate,
                                             texMatrix, samplerState);
...
void SkGpuDevice::drawSpecial(SkSpecialImage* special, int left, int top, const SkPaint& paint,
    ...
    auto fp = GrSimpleTextureEffect::Make(std::move(proxy), special->alphaType(),
                                          SkBlendMode::kModulate, SkMatrix::I());
    ...
    if (clipImage) {
        ...
        if (clipProxy && ctm.invert(&inverseClipMatrix)) {
            ...
            cfp = GrSimpleTextureEffect::Make(std::move(clipProxy), clipImage->alphaType(),
                                              SkBlendMode::kModulate, inverseClipMatrix, sampler);
