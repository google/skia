...
void SkGpuDevice::drawRect(const SkRect& rect, const SkPaint& paint) {
    ...
    if (paint.getMaskFilter() || paint.getPathEffect()) {
        ...
        GrUniqueKey maskKey;
        if (paint.getMaskFilter() && !paint.getPathEffect() &&
            as_MFB(paint.getMaskFilter())->asABlur1(nullptr) &&
            this->ctm().preservesAxisAlignment()) {

            SkMaskFilterBase::BlurRec rec;
            SkAssertResult(as_MFB(paint.getMaskFilter())->asABlur1(&rec));
...
//                                 ((args.fShape->style().strokeRec().getCap() << 1) | 1) : 0;
        ...
        GrBlurUtils::drawPathWithMaskFilter1(fContext.get(), fRenderTargetContext.get(),
                                            ...
                                            this->devClipBounds(), true, maskKey);
#else
        GrShape shape(rect, style);

        GrBlurUtils::drawShapeWithMaskFilter3(fContext.get(), fRenderTargetContext.get(),
...
void SkGpuDevice::drawDRRect(const SkRRect& outer,
    ...
    GrBlurUtils::drawPathWithMaskFilter1(fContext.get(), fRenderTargetContext.get(), this->clip(),
...
void SkGpuDevice::drawPath(const SkPath& origSrcPath,
    ...
    GrBlurUtils::drawPathWithMaskFilter1(fContext.get(), fRenderTargetContext.get(), this->clip(),
                                        ...
                                        this->devClipBounds(), pathIsMutable, maskKey);
}
