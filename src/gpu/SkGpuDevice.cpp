...
void SkGpuDevice::replaceRenderTargetContext(SkSurface::ContentChangeMode mode) {
    ...
    auto newRTC = MakeRenderTargetContext(this->context1(),
                                          budgeted,
                                          this->imageInfo(),
...
void SkGpuDevice::drawPoints(SkCanvas::PointMode mode,
    ...
    if (paint.getPathEffect() && 2 == count && SkCanvas::kLines_PointMode == mode) {
        ...
        GrPaint grPaint;
        if (!SkPaintToGrPaint(this->context1(), fRenderTargetContext->colorInfo(), paint,
                              this->asMatrixProvider(), &grPaint)) {
            return;
        }
        SkPath path;
...
void SkGpuDevice::drawStrokedLine(const SkPoint points[2],
    ...
    GrPaint grPaint;
    if (!SkPaintToGrPaint(this->context1(), fRenderTargetContext->colorInfo(), newPaint,
                          matrixProvider, &grPaint)) {
        return;
    }
...
void SkGpuDevice::drawSpecial(SkSpecialImage* special, int left, int top, const SkPaint& paint,
    ...
    GrSurfaceProxyView view = result->view(this->context1());
    if (!view) {
        return;
    }
    ...
    GrPaint grPaint;
    if (!SkPaintToGrPaintReplaceShader(this->context1(), fRenderTargetContext->colorInfo(),
    ...
    if (clipImage) {
        ...
        GrSurfaceProxyView clipView = as_IB(clipImage)->refView(this->context1(), GrMipMapped::kNo);
...
void SkGpuDevice::drawImageNine(const SkImage* image,
    ...
    if (GrSurfaceProxyView view = as_IB(image)->refPinnedView(this->recordingContext(),
                                                              &pinnedUniqueID)) {
        GrTextureAdjuster adjuster(this->context1(), std::move(view),
