...
static bool needs_antialiasing(SkCanvas::PointMode mode, size_t count, const SkPoint pts[],
    ...
    if (count == 2) {
        // We do not antialias horizontal or vertical lines pixel centers, even when the ends of
        // the line do not fully cover the first and last pixel of the line, which is slightly
        // wrong.
