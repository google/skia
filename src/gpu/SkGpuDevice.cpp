...
sk_sp<SkGpuDevice> SkGpuDevice::Make(GrRecordingContext* rContext,
                                     ...
                                     InitContents init) {
    auto sdc = MakeSurfaceDrawContext7(rContext,
                                      budgeted,
                                      ii,
                                      fit,
                                      sampleCount,
...
std::unique_ptr<GrSurfaceDrawContext> SkGpuDevice::MakeSurfaceDrawContext7(
...
bool SkGpuDevice::replaceBackingProxy(SkSurface::ContentChangeMode mode,
    ...
    auto sdc = GrSurfaceDrawContext::Make(rContext, grColorType, std::move(newRTP), std::move(colorSpace),
                                          origin, props);
    if (!sdc) {
        return false;
    }
