...
static const double kNearlyZero = (SK_Scalar1 / (1 << 20));
...
SegSide calculate_side_of_quad(
    ...
    if (RowData::kVerticalLine == rowData.fIntersectionType) {
        ...
        if ((nearly_equal(p1, segment.fP0T.x(), segment.fNearlyZeroScaled, true) &&
            rowData.fQuadXDirection * rowData.fScanlineXDirection == -1) ||
            (nearly_equal(p1, segment.fP2T.x(), segment.fNearlyZeroScaled, true) &&
            rowData.fQuadXDirection * rowData.fScanlineXDirection == 1)) {
            includeP1 = false;
        }
        if ((nearly_equal(p2, segment.fP0T.x(), segment.fNearlyZeroScaled, true) &&
            rowData.fQuadXDirection * rowData.fScanlineXDirection == 1) ||
            (nearly_equal(p2, segment.fP2T.x(), segment.fNearlyZeroScaled, true) &&
            rowData.fQuadXDirection * rowData.fScanlineXDirection == -1)) {
            includeP2 = false;
        }
        ...
        if (rowData.fScanlineXDirection == 1 &&
            ...
            (nearly_equal(p, segment.fP0T.x(), segment.fNearlyZeroScaled, true) ||
             nearly_equal(p, segment.fP2T.x(), segment.fNearlyZeroScaled, true))) {
            side = (SegSide)(signP * rowData.fQuadXDirection);
        }
    }
