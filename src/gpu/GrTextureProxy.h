...
public:
    ...
    SkDEBUGCODE(bool wasCreatedByDDLProvider() const { return fCreatedByDDLProvider; })
...
protected:
    ...
    // Wrapped version
    GrTextureProxy(sk_sp<GrSurface>, UseAllocator, bool createdByDDLProvider);
...
private:
    ...
    // Setting the fCreatedByDDLProvider flag loosens up asserts that the key of an instantiated
    // uniquely-keyed textureProxy is also always set on the backing GrTexture.
    bool             fCreatedByDDLProvider = false;
