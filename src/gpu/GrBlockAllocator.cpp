...
size_t GrBlockAllocator::totalUsableSpace() const {
    ...
    if (size > 0) {
        size -= kDataStart; // scratchBlockSize reports total block size, not usable size of block
    }
...
void GrBlockAllocator::releaseBlock(Block* block) {
    if (block == &fHead) {
        ...
        // Have the released block become the new scratch block (if it's bigger), or delete it
...
void GrBlockAllocator::reset() {
    for (Block* b : this->rblocks()) {
        if (b == &fHead) {
            // Reset metadata and cursor, delete any scratch block, tail points to head block
            fTail = b;
            b->fNext = nullptr;
...
void GrBlockAllocator::addBlock(int minimumSize, int maxSize) {
    ...
    int allocSize;
    void* mem = nullptr;
    if (fHead.fPrev && fHead.fPrev->fSize >= minimumSize) {
