...
GrBlockAllocator::GrBlockAllocator(void* headBlock, int blockSize, GrowthPolicy policy)
        ...
        , fN1(1) {
    SkASSERT(reinterpret_cast<uintptr_t>(headBlock) % alignof(BlockHeader) == 0);
    SkASSERT(reinterpret_cast<uintptr_t>(headBlock) > reinterpret_cast<uintptr_t>(this));
    SkASSERT(blockSize >= (int) sizeof(BlockHeader) &&
             blockSize <= (int) MaxBlockSize(alignof(BlockHeader)));
...
GrBlockAllocator::BlockHeader::BlockHeader(int size)
         ...
         , fMetadata(0) {
    SkASSERT(size >= (int) sizeof(BlockHeader));
...
void GrBlockAllocator::addBlock(int minimumSize, int maxSize) {
    SkASSERT(minimumSize > (int) sizeof(BlockHeader) && minimumSize <= maxSize);
...
void GrBlockAllocator::validate() const {
    ...
    for (Block b : Blocks(this)) {
        ...
        SkASSERT(block->fSize >= (int) sizeof(BlockHeader));
        SkASSERT(block->fCursor >= (int) sizeof(BlockHeader));
