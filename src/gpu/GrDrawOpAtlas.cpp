...
bool GrDrawOpAtlas::Plot::addSubImage(int width, int height, const void* image, SkIPoint16* loc) {
    ...
    if (!fData) {
        fData = reinterpret_cast<unsigned char*>(sk_calloc_throw(fBytesPerPixel *
                                                                 fSize.fWidth * fSize.fHeight));
    }
...
void GrDrawOpAtlas::Plot::resetRects() {
    ...
    // zero out the plot
    if (fData) {
        sk_bzero(fData, fBytesPerPixel * fSize.fWidth * fSize.fHeight);
    }
...
GrDrawOpAtlas::GrDrawOpAtlas(GrContext* context, sk_sp<GrTextureProxy> proxy,
    ...
    for (int y = 0; y < maxPlots.fY; ++y) {
        for (int x = 0; x < maxPlots.fX; ++x) {
            ...
            // TODO: we could defer creating the non-active Plots until they are actually needed
...
void GrDrawOpAtlas::validate() const {
    ...
    for (unsigned int i = 0; i < (unsigned int) fMaxPlots.fX * fMaxPlots.fY; ++i) {
        ...
        if (plot->fActive) {
            SkASSERT(plot->fXYIndices.fX <= fActivePlots.fX &&
                     plot->fXYIndices.fY <= fActivePlots.fY);
