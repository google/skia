...
GrDrawOpAtlas::GrDrawOpAtlas(GrTextureProvider* textureProvider, sk_sp<GrTextureProxy> proxy,
                             int numPlotsX, int numPlotsY)
        : fTextureProvider(textureProvider)
        , fProxy1(std::move(proxy))
        , fAtlasGeneration(kInvalidAtlasGeneration + 1) {
    fPlotWidth = fProxy1->width() / numPlotsX;
    fPlotHeight = fProxy1->height() / numPlotsY;
    SkASSERT(numPlotsX * numPlotsY <= BulkUseTokenUpdater::kMaxPlots);
    SkASSERT(fPlotWidth * numPlotsX == fProxy1->width());
    SkASSERT(fPlotHeight * numPlotsY == fProxy1->height());

    SkDEBUGCODE(fNumPlots = numPlotsX * numPlotsY;)

    // We currently do not support compressed atlases...
    SkASSERT(!GrPixelConfigIsCompressed(fProxy1->desc().fConfig));
    ...
    for (int y = numPlotsY - 1, r = 0; y >= 0; --y, ++r) {
        for (int x = numPlotsX - 1, c = 0; x >= 0; --x, ++c) {
            ...
            currPlot->reset(
                    new Plot(index, 1, x, y, fPlotWidth, fPlotHeight, fProxy1->desc().fConfig));
...
inline void GrDrawOpAtlas::updatePlot(GrDrawOp::Target* target, AtlasID* id, Plot* plot) {
    ...
    if (target->hasDrawBeenFlushed(plot->lastUploadToken())) {
        ...
        GrTexture* texture = fProxy1->instantiate(fTextureProvider);
...
bool GrDrawOpAtlas::addToAtlas(AtlasID* id, GrDrawOp::Target* target, int width, int height,
    ...
    while ((plot = plotIter.get())) {
        SkASSERT(GrBytesPerPixel(fProxy1->desc().fConfig) == plot->bpp());
    ...
    SkASSERT(GrBytesPerPixel(fProxy1->desc().fConfig) == newPlot->bpp());
