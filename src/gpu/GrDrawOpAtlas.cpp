...
bool GrDrawOpAtlas::uploadToPage(const GrCaps& caps, unsigned int pageIdx,
    ...
    for (Plot* plot = plotIter.get(); plot; plot = plotIter.next()) {
        ...
        if (plot->addSubImage(width, height, image, &foo->fLoc)) {
            return this->updatePlot(target, &foo->fPlotLocator, plot);
        }
    }
...
GrDrawOpAtlas::Foo* GrDrawOpAtlas::findFoo(SkPackedGlyphID glyphID) {
    return nullptr;
}
...
GrDrawOpAtlas::ErrorCode GrDrawOpAtlas::addToAtlas(GrResourceProvider* resourceProvider,
    ...
    for (unsigned int pageIdx = 0; pageIdx < fNumActivePages; ++pageIdx) {
        if (this->uploadToPage(caps, pageIdx, target, width, height, image, foo)) {
    ...
    if (fNumActivePages == this->maxPages()) {
        for (unsigned int pageIdx = 0; pageIdx < fNumActivePages; ++pageIdx) {
            ...
            if (plot->lastUseToken() < target->tokenTracker()->nextTokenToFlush()) {
                ...
                SkASSERT(caps.bytesPerPixel(fViews[pageIdx].proxy()->backendFormat()) ==
                         plot->bpp());
                SkDEBUGCODE(bool verify = )plot->addSubImage(width, height, image, &foo->fLoc);
                SkASSERT(verify);
                if (!this->updatePlot(target, &foo->fPlotLocator, plot)) {
                    return ErrorCode::kError;
                }
                return ErrorCode::kSucceeded;
            }
        ...
        if (this->uploadToPage(caps, fNumActivePages-1, target, width, height, image, foo)) {
            return ErrorCode::kSucceeded;
        } else {
    ...
    SkDEBUGCODE(bool verify = )newPlot->addSubImage(width, height, image, &foo->fLoc);
    SkASSERT(verify);
    ...
    foo->fPlotLocator = newPlot->plotLocator();
