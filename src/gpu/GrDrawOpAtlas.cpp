...
GrDrawOpAtlas::Plot::Plot(int pageIndex, int plotIndex, uint64_t genID, int offX, int offY,
        ...
        , fGenID(genID)
        , fPlotPath(CreateId(fPageIndex, fPlotIndex, fGenID))
        , fData(nullptr)
        , fWidth(width)
        , fHeight(height)
        , fX(offX)
...
inline void GrDrawOpAtlas::processEviction(PlotPath plotPath) {
    for (auto evictor : fEvictionCallbacks) {
        evictor->evict(plotPath);
    }
...
inline bool GrDrawOpAtlas::updatePlot(GrDeferredUploadTarget* target,
    ...
    int pageIdx = GetPageIndexFromID(plot->plotPath());
    ...
    *plotPath = plot->plotPath();
    return true;
}

bool GrDrawOpAtlas::uploadToPage(const GrCaps& caps, unsigned int pageIdx, PlotPath* plotPath,
    ...
    for (Plot* plot = plotIter.get(); plot; plot = plotIter.next()) {
        ...
        if (plot->addSubImage(width, height, image, loc)) {
            return this->updatePlot(target, plotPath, plot);
        }
    }
...
GrDrawOpAtlas::ErrorCode GrDrawOpAtlas::addToAtlas(GrResourceProvider* resourceProvider,
    ...
    for (unsigned int pageIdx = 0; pageIdx < fNumActivePages; ++pageIdx) {
        if (this->uploadToPage(caps, pageIdx, plotPath, target, width, height, image, loc)) {
    ...
    if (fNumActivePages == this->maxPages()) {
        for (unsigned int pageIdx = 0; pageIdx < fNumActivePages; ++pageIdx) {
            ...
            if (plot->lastUseToken() < target->tokenTracker()->nextTokenToFlush()) {
                ...
                SkASSERT(verify);
                if (!this->updatePlot(target, plotPath, plot)) {
                    return ErrorCode::kError;
                }
                return ErrorCode::kSucceeded;
            }
        ...
        if (this->uploadToPage(
                caps, fNumActivePages-1, plotPath, target, width, height, image, loc)) {
            return ErrorCode::kSucceeded;
        } else {
