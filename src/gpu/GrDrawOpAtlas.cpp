...
GrDrawOpAtlas::Plot::Plot(int index, int pageIndex, uint64_t genID, int offX, int offY,
        ...
        , fGenID(genID)
        , fID(CreateId(fIndex, fPageIndex, fGenID))
        , fData(nullptr)
        , fWidth(width)
        , fHeight(height)
        , fX(offX)
...
GrDrawOpAtlas::GrDrawOpAtlas(GrContext* context, GrPixelConfig config, int width, int height,
    ...
    for (int y = numPlotsY - 1, r = 0; y >= 0; --y, ++r) {
        for (int x = numPlotsX - 1, c = 0; x >= 0; --x, ++c) {
            ...
            currPlot->reset(
                    new Plot(index, 0, 1, x, y, fPlotWidth, fPlotHeight, fPixelConfig));
