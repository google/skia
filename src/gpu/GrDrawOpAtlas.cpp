...
GrDrawOpAtlasConfig::GrDrawOpAtlasConfig(int maxTextureSize, size_t maxBytes) {
    ...
    SkASSERT(kARGBDimensions[index].width() < kMaxARGBDim);
    SkASSERT(kARGBDimensions[index].height() < kMaxARGBDim);
...
SkISize GrDrawOpAtlasConfig::atlasDimensions(GrMaskFormat type) const {
    if (kA8_GrMaskFormat == type) {
        int maxAllowedDimension = SkTMin<int>(fMaxTextureSize, kMaxA8Dim);
        // A8 is always 2x the ARGB dimensions, clamped by the A8 max and system max
