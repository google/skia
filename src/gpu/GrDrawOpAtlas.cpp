...
std::array<uint16_t, 4> GrDrawOpAtlas::AtlasLocator::uvs(bool distanceField) const {

    uint16_t left, top, right, bottom;
    if (distanceField) {
        left   = fRect.fLeft   + SK_DistanceFieldInset;
        bottom = fRect.fBottom + SK_DistanceFieldInset;
        right  = fRect.fRight  - 2 * SK_DistanceFieldInset;
        top    = fRect.fTop    - 2 * SK_DistanceFieldInset;
    } else {
        left   = fRect.fLeft;
        bottom = fRect.fBottom;
        right  = fRect.fRight;
        top    = fRect.fTop;
    }

    // We pack the 2bit page index in the low bit of the u and v texture coords
    uint32_t pageIndex = GrDrawOpAtlas::GetPageIndexFromID(fPlotLocator);
...
bool GrDrawOpAtlas::Plot::addSubImage(int width, int height, const void* image, GrIRect16* rect) {
    ...
    SkIPoint16 loc2;
    if (!fRectanizer.addRect(width, height, &loc2)) {
        return false;
    }

    *rect = GrIRect16::MakeXYWH(loc2.fX, loc2.fY, width, height);
    ...
    dataPtr += fBytesPerPixel * fWidth * rect->fBottom;
