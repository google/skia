...
public:
    ...
    bool addRect(int w, int h, SkIPoint16* loc) {
        ...
        if (w < fRectanizer.width()) {
            w = SkTMin(w + kPadding, fRectanizer.width());
        }
        if (h < fRectanizer.height()) {
            h = SkTMin(h + kPadding, fRectanizer.height());
        }
...
GrDynamicAtlas::GrDynamicAtlas(GrColorType colorType, InternalMultisample internalMultisample,
        ...
        , fWidth(std::min(SkNextPow2(initialSize.width()), maxAtlasSize))
        , fHeight(std::min(SkNextPow2(initialSize.height()), maxAtlasSize))
        , fTextureProxy(MakeLazyAtlasProxy(
                ...
                fColorType, internalMultisample, caps, GrSurfaceProxy::UseAllocator::kNo)) {
