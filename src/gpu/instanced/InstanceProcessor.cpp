...
InstanceProcessor::InstanceProcessor(OpInfo opInfo, GrBuffer* paramsBuffer) : fOpInfo(opInfo) {
    ...
    if (GrAATypeIsHW(fOpInfo.fAAType)) {
        if (!fOpInfo.isSimpleRects() || GrAAType::kMixedSamples == fOpInfo.fAAType) {
...
private:
    bool isMixedSampled() const { return GrAAType::kMixedSamples == fOpInfo.fAAType; }
