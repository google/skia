...
GrCaps::InstancedSupport InstanceProcessor::CheckSupport(const GrShaderCaps& shaderCaps,
                                                         const GrCaps& caps) {
    if (!shaderCaps.canUseAnyFunctionInShader() || !shaderCaps.flatInterpolationSupport() ||
        !shaderCaps.integerSupport() || 0 == shaderCaps.maxVertexSamplers() ||
        !caps.shaderCaps()->texelBufferSupport() || caps.maxVertexAttributes() < kNumAttribs) {
        return GrCaps::InstancedSupport::kNone;
    }
    if (!caps.sampleLocationsSupport() || !shaderCaps.sampleVariablesSupport() ||
        !shaderCaps.shaderDerivativeSupport()) {
        return GrCaps::InstancedSupport::kBasic;
    }
    if (0 == caps.maxRasterSamples() || !shaderCaps.sampleMaskOverrideCoverageSupport()) {
...
public:
    VertexInputs(const InstanceProcessor& instProc, GrGLSLVertexBuilder* vertexBuilder)
        : fInstProc(instProc), fVertexBuilder(vertexBuilder) {}
    ...
    void initParams(const SamplerHandle paramsBuffer) {
        ...
        fVertexBuilder->codeAppendf("highp int paramsIdx = int(%s & 0x%x);",
                                    this->attr(Attrib::kInstanceInfo), kParamsIdx_InfoMask);
    }
...
protected:
    ...
    virtual void onEmitCode(GrGLSLVertexBuilder*, GrGLSLPPFragmentBuilder*, const char* outCoverage,
                            const char* outColor) = 0;
...
void GLSLInstanceProcessor::onEmitCode(EmitArgs& args, GrGPArgs* gpArgs) {
    ...
    if (!ip.batchInfo().fHasPerspective) {
        v->codeAppendf("mat2x3 shapeMatrix = mat2x3(%s, %s);", inputs.attr(Attrib::kShapeMatrixX),
                       inputs.attr(Attrib::kShapeMatrixY));
    } else {
        ...
        v->codeAppendf("if (0 != (%s & PERSPECTIVE_FLAG)) {", inputs.attr(Attrib::kInstanceInfo));
    ...
    if (ip.batchInfo().fInnerShapeTypes) {
        ...
        if (!hasSingleInnerShapeType) {
            ...
            v->codeAppendf(
                    "uint innerShapeType = ((%s & INNER_SHAPE_TYPE_MASK) >> "
        ...
        v->codeAppend(
                "vec4 outer2Inner = vec4(outer.zw - outer.xy, "
    ...
    if (ip.batchInfo().fHasLocalMatrix && ip.batchInfo().fHasParams) {
        ...
        v->codeAppendf("if (0 != (%s & LOCAL_MATRIX_FLAG)) {", inputs.attr(Attrib::kInstanceInfo));
        if (!ip.batchInfo().fUsesLocalCoords) {
            inputs.skipParams(2);
        } else {
    ...
    v->codeAppendf("%s deviceCoords = vec3(%s, 1) * shapeMatrix;", GrGLSLTypeString(positionType),
                   backend->outShapeCoords());
...
void GLSLInstanceProcessor::Backend::setupRRect(GrGLSLVertexBuilder* v, int* usedShapeDefinitions) {
    ...
    if (fArcCoords.vsOut()) {
        v->codeAppendf("%s = (cornerSign * %s + radii - vec2(1)) / radii;", fArcCoords.vsOut(),
                       fModifiedShapeCoords);
    }
    if (fTriangleIsArc.vsOut()) {
        v->codeAppendf("%s = int(all(equal(vec2(1), abs(%s))));", fTriangleIsArc.vsOut(),
...
void GLSLInstanceProcessor::Backend::setupComplexRadii(GrGLSLVertexBuilder* v) {
    ...
    if (fNeedsNeighborRadii) {
        v->codeAppend(
                "neighborRadii = vec2(p[1u - corner.x][corner.y], "
...
public:
    BackendNonAA(BatchInfo batchInfo, const VertexInputs& inputs) : INHERITED(batchInfo, inputs) {
...
void GLSLInstanceProcessor::BackendNonAA::onEmitCode(GrGLSLVertexBuilder*,
    ...
    if (fTriangleIsArc.fsIn()) {
        ...
        f->codeAppendf("if (%s != 0 && dot(%s, %s) > 1.0) %s;", fTriangleIsArc.fsIn(),
                       fArcCoords.fsIn(), fArcCoords.fsIn(), dropFragment);
    }
    if (fBatchInfo.fInnerShapeTypes) {
        ...
        if (kRect_ShapeFlag == fBatchInfo.fInnerShapeTypes) {
            ...
            f->codeAppendf("if ((dot(%s, %s) <= 1.0)) %s;", fInnerShapeCoords.fsIn(),
                           fInnerShapeCoords.fsIn(), dropFragment);
        } else {
            f->codeAppendf("if (all(lessThan(abs(%s), vec2(1)))) {", fInnerShapeCoords.fsIn());
            f->codeAppendf("vec2 distanceToArcEdge = abs(%s) - %s.xy;", fInnerShapeCoords.fsIn(),
                           fInnerRRect.fsIn());
            f->codeAppend("if (any(lessThan(distanceToArcEdge, vec2(0)))) {");
            f->codeAppendf("%s;", dropFragment);
            f->codeAppend("} else {");
            f->codeAppendf("vec2 rrectCoords = distanceToArcEdge * %s.zw;", fInnerRRect.fsIn());
...
public:
    BackendCoverage(BatchInfo batchInfo, const VertexInputs& inputs)
        ...
        fTweakAlphaForCoverage =
                !fBatchInfo.fCannotTweakAlphaForCoverage && !fBatchInfo.fInnerShapeTypes;
...
void GLSLInstanceProcessor::BackendCoverage::onInit(GrGLSLVaryingHandler* varyingHandler,
    ...
    v->codeAppend(
            "vec2 shapeHalfSize = vec2(length(shapeTransposeMatrix[0]), "
...
void GLSLInstanceProcessor::BackendCoverage::setupRect(GrGLSLVertexBuilder* v) {
    ...
    v->codeAppend(
            "float maxCoverage = 4.0 * min(0.5, shapeHalfSize.x) *"
...
void GLSLInstanceProcessor::BackendCoverage::setupOval(GrGLSLVertexBuilder* v) {
    ...
    if (fTriangleIsArc.vsOut()) {
        v->codeAppendf("%s = int(%s != 0);", fTriangleIsArc.vsOut(),
                       fInputs.attr(Attrib::kVertexAttrs));
    }
...
void GLSLInstanceProcessor::BackendCoverage::adjustRRectVertices(GrGLSLVertexBuilder* v) {
    ...
    v->codeAppendf(
            "if (abs(%s.x) == 0.5)"
            "bloatedShapeCoords.x = borderEdge.x;",
            fInputs.attr(Attrib::kShapeCoords));
    v->codeAppendf(
            "if (abs(%s.y) == 0.5)"
            "bloatedShapeCoords.y = borderEdge.y;",
            fInputs.attr(Attrib::kShapeCoords));
...
void GLSLInstanceProcessor::BackendCoverage::onSetupRRect(GrGLSLVertexBuilder* v) {
    ...
    v->codeAppendf(
            "%s = (cornerSign * bloatedShapeCoords + clampedRadii - vec2(1)) * "
            "shapeHalfSize;",
            fEllipseCoords.vsOut());
...
void GLSLInstanceProcessor::BackendCoverage::onSetupInnerSimpleRRect(GrGLSLVertexBuilder* v) {
    ...
    v->codeAppendf(
            "%s = 1.0 / (innerRadii * innerRadii * innerShapeHalfSize * "
            "innerShapeHalfSize);",
            fInnerEllipseName.vsOut());
...
void GLSLInstanceProcessor::BackendCoverage::onEmitCode(GrGLSLVertexBuilder* v,
    ...
    if (fColorTimesRectCoverage.vsOut()) {
        ...
        v->codeAppendf("%s = %s * rectCoverage;", fColorTimesRectCoverage.vsOut(),
                       fInputs.attr(Attrib::kColor));
    }
...
void GLSLInstanceProcessor::BackendCoverage::emitCircle(GrGLSLPPFragmentBuilder* f,
    ...
    f->codeAppendf("mediump float distanceToEdge = %s - length(%s);", fBloatedRadius.fsIn(),
                   fEllipseCoords.fsIn());
    f->codeAppendf("%s = clamp(distanceToEdge, 0.0, 1.0);", outCoverage);
}

void GLSLInstanceProcessor::BackendCoverage::emitArc(
        GrGLSLPPFragmentBuilder* f, const char* ellipseCoords, const char* ellipseName,
        bool ellipseCoordsNeedClamp, bool ellipseCoordsMayBeNegative, const char* outCoverage) {
...
void GLSLInstanceProcessor::BackendCoverage::emitInnerRect(GrGLSLPPFragmentBuilder* f,
                                                           const char* outCoverage) {
    f->codeAppendf("lowp vec2 c = %s - abs(%s);", fInnerShapeBloatedHalfSize.fsIn(),
                   fDistanceToInnerEdge.fsIn());
...
private:
    bool isMixedSampled() const {
        return AntialiasMode::kMixedSamples == fBatchInfo.fAntialiasMode;
    }
...
void GLSLInstanceProcessor::BackendMultisample::onInit(GrGLSLVaryingHandler* varyingHandler,
    ...
    if (!fBatchInfo.fHasPerspective) {
        ...
        v->codeAppend(
                "vec2 fragShapeSpan = abs(vec4(shapeInverseMatrix).xz) + "
...
void GLSLInstanceProcessor::BackendMultisample::setupOval(GrGLSLVertexBuilder* v) {
    ...
    if (fTriangleIsArc.vsOut()) {
        if (!this->isMixedSampled()) {
            v->codeAppendf("%s = %s & 1;", fTriangleIsArc.vsOut(),
                           fInputs.attr(Attrib::kVertexAttrs));
        } else {
    ...
    if (fEarlyAccept.vsOut()) {
        v->codeAppendf("%s = ~%s & SAMPLE_MASK_ALL;", fEarlyAccept.vsOut(),
                       fInputs.attr(Attrib::kVertexAttrs));
    }
}
...
void GLSLInstanceProcessor::BackendMultisample::adjustRRectVertices(GrGLSLVertexBuilder* v) {
    ...
    if (!fBatchInfo.fHasPerspective) {
        ...
        v->codeAppend(
                "vec2 cornerSize = any(lessThan(radii, fragShapeSpan)) ? "
...
void GLSLInstanceProcessor::BackendMultisample::onInitInnerShape(
        GrGLSLVaryingHandler* varyingHandler, GrGLSLVertexBuilder* v) {
...
void GLSLInstanceProcessor::BackendMultisample::onEmitCode(GrGLSLVertexBuilder*,
                                                           GrGLSLPPFragmentBuilder* f, const char*,
                                                           const char*) {
    ...
    if (!this->isMixedSampled()) {
        ...
        if (arcTest && fBatchInfo.fHasPerspective) {
            ...
            f->codeAppendf("highp vec2 arcTest = %s - 0.5 * fwidth(%s);", fArcTest.fsIn(),
                           fArcTest.fsIn());
            arcTest = "arcTest";
        }
        ...
        if (arcTest) {
            ...
            f->codeAppendf(
                    "if (gl_SampleMaskIn[0] == SAMPLE_MASK_ALL || "
                    "all(greaterThan(%s, vec2(0)))) {",
                    arcTest);
            this->emitArc(f, arcCoords, false, clampArcCoords, opts);
            f->codeAppend("} else {");
...
void GLSLInstanceProcessor::BackendMultisample::emitRect(GrGLSLPPFragmentBuilder* f,
    ...
    if (coords.fFragHalfSpan) {
        f->codeAppendf("if (all(lessThanEqual(abs(%s), 1.0 - %s))) {", coords.fVarying->fsIn(),
                       coords.fFragHalfSpan);
        ...
        if (opts.fIsTightGeometry && !fRectTrianglesMaySplit) {
            f->codeAppendf("if (any(lessThan(abs(%s), 1.0 - %s))) {", coords.fVarying->fsIn(),
                           coords.fFragHalfSpan);
...
void GLSLInstanceProcessor::BackendMultisample::emitArc(GrGLSLPPFragmentBuilder* f,
    ...
    if (coords.fFragHalfSpan) {
        ...
        if (clampCoords) {
            f->codeAppendf("if (%s(max(%s + %s, vec2(0))) < 1.0) {", fSquareFun.c_str(),
                           absArcCoords.c_str(), coords.fFragHalfSpan);
        } else {
            f->codeAppendf("if (%s(%s + %s) < 1.0) {", fSquareFun.c_str(), absArcCoords.c_str(),
                           coords.fFragHalfSpan);
        }
        // The entire pixel is inside the arc.
        this->acceptOrRejectWholeFragment(f, true, opts);
        f->codeAppendf("} else if (%s(max(%s - %s, vec2(0))) >= 1.0) {", fSquareFun.c_str(),
                       absArcCoords.c_str(), coords.fFragHalfSpan);
...
void GLSLInstanceProcessor::BackendMultisample::emitSimpleRRect(GrGLSLPPFragmentBuilder* f,
    ...
    if (coords.fInverseMatrix && coords.fFragHalfSpan) {
        ...
        f->codeAppend(";");
        f->codeAppendf("highp vec2 rrectPt = max(abs(shapePt) - %s.xy, vec2(0)) * %s.zw;", rrect,
                       rrect);
...
void GLSLInstanceProcessor::BackendMultisample::interpolateAtSample(
        GrGLSLPPFragmentBuilder* f, const GrGLSLVarying& varying, const char* sampleIdx,
        const char* interpolationMatrix) {
...
void GLSLInstanceProcessor::BackendMultisample::acceptOrRejectWholeFragment(
        GrGLSLPPFragmentBuilder* f, bool inside, const EmitShapeOpts& opts) {
...
const uint8_t kIndexData[] = {
        ...
        0, 1, 5, 5, 4, 0, 1, 2, 6, 6, 5, 1, 2, 3, 7, 7, 6, 2, 3, 0, 4, 4, 7, 3, 4, 5, 6, 6, 7, 4,

        // Octagon that inscribes the unit circle, cut by an interior unit octagon.
        10, 8, 9, 12, 10, 11, 14, 12, 13, 16, 14, 15, 18, 16, 17, 20, 18, 19, 22, 20, 21, 8, 22, 23,
        8, 10, 12, 12, 14, 16, 16, 18, 20, 20, 22, 8, 8, 12, 16, 16, 20, 8,

        // Same octagons, but with the interior arranged as a fan. Used by mixed samples.
        10, 8, 9, 12, 10, 11, 14, 12, 13, 16, 14, 15, 18, 16, 17, 20, 18, 19, 22, 20, 21, 8, 22, 23,
        24, 8, 10, 12, 24, 10, 24, 12, 14, 16, 24, 14, 24, 16, 18, 20, 24, 18, 24, 20, 22, 8, 24,
        22,

        // Same octagons, but with the inner and outer disjoint. Used by coverage AA.
        8, 22, 23, 9, 8, 23, 10, 8, 9, 11, 10, 9, 12, 10, 11, 13, 12, 11, 14, 12, 13, 15, 14, 13,
        16, 14, 15, 17, 16, 15, 18, 16, 17, 19, 18, 17, 20, 18, 19, 21, 20, 19, 22, 20, 21, 23, 22,
        21, 22, 8, 10, 10, 12, 14, 14, 16, 18, 18, 20, 22, 22, 10, 14, 14, 18, 22,

        // Rectangle with disjoint corner segments.
        27, 25, 26, 30, 28, 29, 33, 31, 32, 36, 34, 35, 25, 27, 28, 28, 30, 31, 31, 33, 34, 34, 36,
        25, 25, 28, 31, 31, 34, 25,

        // Same rectangle with disjoint corners, but with the interior arranged as a fan. Used by
        // mixed samples.
        27, 25, 26, 30, 28, 29, 33, 31, 32, 36, 34, 35, 27, 37, 25, 28, 37, 27, 30, 38, 28, 31, 38,
        30, 33, 39, 31, 34, 39, 33, 36, 40, 34, 25, 40, 36,

        // Same rectangle with disjoint corners, with a border as well. Used by coverage AA.
        41, 25, 26, 42, 41, 26, 27, 42, 26, 43, 28, 29, 44, 43, 29, 30, 44, 29, 45, 31, 32, 46, 45,
        32, 33, 46, 32, 47, 34, 35, 48, 47, 35, 36, 48, 35, 27, 28, 42, 42, 28, 43, 30, 31, 44, 44,
        31, 45, 33, 34, 46, 46, 34, 47, 36, 25, 48, 48, 25, 41, 41, 42, 43, 43, 44, 45, 45, 46, 47,
        47, 48, 41, 41, 43, 45, 45, 47, 41,

        // Same as the disjoint octagons, but with 16-gons instead. Used by coverage AA when the
        // oval is
        // sufficiently large.
        49, 79, 80, 50, 49, 80, 51, 49, 50, 52, 51, 50, 53, 51, 52, 54, 53, 52, 55, 53, 54, 56, 55,
        54, 57, 55, 56, 58, 57, 56, 59, 57, 58, 60, 59, 58, 61, 59, 60, 62, 61, 60, 63, 61, 62, 64,
        63, 62, 65, 63, 64, 66, 65, 64, 67, 65, 66, 68, 67, 66, 69, 67, 68, 70, 69, 68, 71, 69, 70,
        72, 71, 70, 73, 71, 72, 74, 73, 72, 75, 73, 74, 76, 75, 74, 77, 75, 76, 78, 77, 76, 79, 77,
        78, 80, 79, 78, 49, 51, 53, 53, 55, 57, 57, 59, 61, 61, 63, 65, 65, 67, 69, 69, 71, 73, 73,
        75, 77, 77, 79, 49, 49, 53, 57, 57, 61, 65, 65, 69, 73, 73, 77, 49, 49, 57, 65, 65, 73, 49,
};
...
const char* InstanceProcessor::GetNameOfIndexRange(IndexRange range) {
    switch (range.fStart) {
        case kRect_FirstIndex:
            return "basic_rect";
        case kFramedRect_FirstIndex:
            return "coverage_rect";

        case kOctagons_FirstIndex:
            return "basic_oval";
        case kDisjointOctagons_FirstIndex:
            return "coverage_oval";
        case kDisjoint16Gons_FirstIndex:
            return "coverage_large_oval";
        case kOctagonsFanned_FirstIndex:
            return "mixed_samples_oval";

        case kCorneredRect_FirstIndex:
            return "basic_round_rect";
        case kCorneredFramedRect_FirstIndex:
            return "coverage_round_rect";
        case kCorneredRectFanned_FirstIndex:
            return "mixed_samples_round_rect";
