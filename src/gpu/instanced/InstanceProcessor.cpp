...
public:
    ...
    void fetchNextParam(GrSLType type = kHalf4_GrSLType) const {
...
void GLSLInstanceProcessor::onEmitCode(EmitArgs& args, GrGPArgs* gpArgs) {
    ...
    if (!ip.opInfo().fHasPerspective) {
        ...
        inputs.fetchNextParam(kHalf3_GrSLType);
        v->codeAppend (    ";");
        v->codeAppend ("}");
    }
    ...
    GrSLType positionType = ip.opInfo().fHasPerspective ? kHalf3_GrSLType : kHalf2_GrSLType;
    v->codeAppendf("%s deviceCoords = half3(%s, 1) * shapeMatrix;",
    ...
    if (fOpInfo.fInnerShapeTypes) {
        ...
        if (kOval_ShapeFlag == fOpInfo.fInnerShapeTypes) {
            ...
            if (kRect_ShapeFlag == fOpInfo.fInnerShapeTypes) {
                ...
                f->codeAppend ("} else {");
                f->codeAppendf(    "mediump half2 ellipseCoords = distanceToArcEdge * %s.zw;",
                                   fInnerRRect.fsIn());
