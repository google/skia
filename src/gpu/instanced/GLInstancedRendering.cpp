...
public:
    ...
    GLInstancedOp(GLInstancedRenderingAllocator* alloc, GrPaint&& paint)
...
std::unique_ptr<InstancedOp> GLInstancedRenderingAllocator::makeOp(GrPaint&& paint) {
...
void GLInstancedRendering::onBeginFlush(GrResourceProvider* rp) {
    ...
    while (InstancedOp* o = iter.get()) {
        ...
        const InstancedOp::Draw* draw = op->fHeadDraw1;
        SkASSERT(draw);
        do {
            int instanceCount = 0;
...
void GLInstancedRendering::onDraw1(const GrPipeline& pipeline, const InstanceProcessor& instProc,
