...
void InstancedRendering::Op::getFragmentProcessorAnalysisInputs(
    ...
    if (GrAAType::kCoverage == fInfo.fAAType ||
        (GrAAType::kNone == fInfo.fAAType && !fInfo.isSimpleRects() && fInfo.fCannotDiscard)) {
...
void InstancedRendering::Op::applyPipelineOptimizations(
    ...
    if (kRect_ShapeFlag == fInfo.fShapeTypes) {
        draw.fGeometry = InstanceProcessor::GetIndexRangeForRect(fInfo.fAAType);
    } else if (kOval_ShapeFlag == fInfo.fShapeTypes) {
        draw.fGeometry = InstanceProcessor::GetIndexRangeForOval(fInfo.fAAType, this->bounds());
    } else {
        draw.fGeometry = InstanceProcessor::GetIndexRangeForRRect(fInfo.fAAType);
    }
