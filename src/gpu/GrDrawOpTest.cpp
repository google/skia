...
#define DRAW_OP_TEST_EXTERN(Op) \
    extern std::unique_ptr<GrDrawOp> Op##__Test(GrPaint&&, SkRandom*, GrContext*, int, int);
...
void GrDrawRandomOp(SkRandom* random, GrRenderTargetContext* renderTargetContext, GrPaint&& paint) {
    ...
    using MakeDrawOpFn = std::unique_ptr<GrDrawOp>(GrPaint&&, SkRandom*, GrContext*, int, int);
    ...
    if (index < SK_ARRAY_COUNT(gLegacyFactories)) {
        const GrUserStencilSettings* uss = GrGetRandomStencil(random);
        ...
        SkASSERT(op);
        renderTargetContext->priv().testingOnly_addLegacyMeshDrawOp(std::move(paint), aaType,
                                                                    std::move(op), uss,
                                                                    snapToCenters);
    } else  {
        bool stencilSamples = renderTargetContext->isStencilBufferMultisampled();
        bool colorSamples = renderTargetContext->isUnifiedMultisampled();
        auto op = gFactories[index - SK_ARRAY_COUNT(gFactories)](std::move(paint), random, context, stencilSamples, colorSamples);
        SkASSERT(op);
