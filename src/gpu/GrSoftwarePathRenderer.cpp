...
bool GrSoftwarePathRenderer::onDrawPath(const DrawPathArgs& args) {
    ...
    if (!proxy) {
        ...
        int maskWidth = boundsForMask->width();
        int maskHeight = boundsForMask->height();

        SkTaskGroup* taskGroup = args.fContext->contextPriv().getTaskGroup();
        if (taskGroup) {
            proxy = make_deferred_mask_texture_proxy(args.fContext, fit, maskWidth, maskHeight);
            if (!proxy) {
                return false;
            }
            ...
            if (FLAGS_pbo) {
                ...
                size_t size = maskWidth * maskHeight;
            ...
            if (FLAGS_pbo) {
                SkImageInfo info = SkImageInfo::MakeA8(maskWidth, maskHeight);
                uploader->getPixels()->reset(info, data, info.minRowBytes());
            }
            ...
            SkImageInfo info = SkImageInfo::MakeA8(maskWidth, maskHeight);
            pixmap.reset(info, data, info.minRowBytes());
            ...
            {
                TRACE_EVENT0("skia", "Transfer Pixels");
                if (!args.fContext->getGpu()->transferPixels(tex, 0, 0, maskWidth, maskHeight,
            ...
            sk_sp<GrSurfaceContext> sContext =
                    args.fContext->contextPriv().makeWrappedSurfaceContext(proxy, nullptr);
            if (!sContext->writePixels(pixmap.info(), pixmap.addr(), pixmap.rowBytes(), 0, 0)) {
                return false;
            }
        }
