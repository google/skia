...
public:
    GrMaskUploaderPreFlushCallback(sk_sp<GrTextureProxy> proxy, const SkIRect& maskBounds,
...
bool GrSoftwarePathRenderer::onDrawPath(const DrawPathArgs& args) {
    ...
    if (!proxy) {
        ...
        if (args.fContext->caps()->useThreadedPathMaskRendering()) {
            ...
            auto uploader = skstd::make_unique<GrMaskUploaderPreFlushCallback>(
                    proxy, *boundsForMask, *args.fViewMatrix, *args.fShape, aa);
            GrMaskUploaderPreFlushCallback* uploaderRaw = uploader.get();
            ...
            auto drawAndUploadMask = [uploaderRaw] {
                ...
                if (helper.init(uploaderRaw->getMaskBounds(), uploaderRaw->getViewMatrix())) {
                    ...
                    SkDEBUGFAIL("Unable to initialize SW mask helper. Too late to recover.");
                }
                uploaderRaw->getSemaphore()->signal();
            };
            taskGroup->add(std::move(drawAndUploadMask));
            args.fRenderTargetContext->getOpList()->addPreFlushCallback(std::move(uploader));
        } else {
