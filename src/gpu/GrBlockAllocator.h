...
protected:
     ...
     * per-block and per-allocation metadata. It exposes similar APIs to the public interface, but
     * for 'safe' parameters, it accepts them as ints directly instead of size_t. Non-safe
     * parameters are kept as size_t and validated so subclasses don't need to worry about that.
     * As part of this, it also exposes the locations of the blocks it manages.
     */
     ...
     * to the header constructor. Subclasses are responsible for calling ~Meta the headers if needed
     */
    ...
    /**
     * Attempt to resize the allocation described by ('block', 'offset', 'size') to the
     * 'newSize'. It is assumed that the 3-tuple was previously provided from a call to
     * allocate(). Returns true if the allocation was updated to the new size.
     */
...
private:
    ...
    struct BlockHeader {
        ...
        void* ptr(int offset) const {
            SkASSERT(offset >= sizeof(BlockHeader) && offset < fSize);
...
int GrBlockAllocator::allocateInternal(int size) {
    ...
    static constexpr size_t kMaxInputSize =
            kMaxAllocationSize + GrAlignTo(kMaxAllocMetadataSize, kAlign);
    static constexpr size_t kMaxAlignedCursor = GrAlignTo(MaxBlockSize(kAlign), kAlign);
    ...
    SkASSERT(fTail->Size - alignedCursor >= size);
