...
public:
    ...
    size_t preallocSize() const {
        return sizeof(GrBlockAllocator) + fHead.fSize - sizeof(Block); // Don't double count fHead
    }
    /**
...
GrBlockAllocator::ByteRange GrBlockAllocator::allocate(size_t size) {
    ...
    SkASSERT(alloc.fEnd - alloc.fOffset == paddedSize);
    SkASSERT(alloc.fOffset - alloc.fStart >= Padding &&
             alloc.fOffset - alloc.fStart <= Padding + Align - 1);
    SkASSERT(reinterpret_cast<uintptr_t>(fTail->ptr(alloc.fOffset)) % Align == 0);
