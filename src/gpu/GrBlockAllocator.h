...
public:
    ...
    public:
        ...
        bool isOwner(void* p) const { 
            intptr_t pdiff = reinterpret_cast<intptr_t>(p) - reinterpret_cast<intptr_t>(this);
            return pdiff >= (intptr_t) kDataStart && pdiff < (intptr_t) fSize;
        }
    private:
        ...
        template <size_t Align, size_t Padding>
        ByteRange alignedRange(int size) {
            int offset = this->cursor<Align, Padding>();
            return {this, fCursor, offset, offset + size};
        }
...
private:
    ...
    static constexpr int kDataStart = sizeof(Block); //offsetof(Block, fMetadata) + sizeof(int);
...
GrBlockAllocator::ByteRange GrBlockAllocator::allocate(size_t size) {
    ...
    int iSize = (int) size;
    ByteRange alloc = fTail->alignedRange<Align, Padding>(iSize);
    if (alloc.fEnd > fTail->fSize) {
        this->addBlock(iSize + kBlockOverhead, MaxBlockSize(Align, Padding));
        alloc = fTail->alignedRange<Align, Padding>(iSize);
    }

    // Check invariants
    SkASSERT(alloc.fEnd <= fTail->fSize);
    SkASSERT(alloc.fEnd - alloc.fAlignedOffset == iSize);
    SkASSERT(alloc.fAlignedOffset - alloc.fStart >= (int) Padding &&
             alloc.fAlignedOffset - alloc.fStart <= (int) (Padding + Align - 1));
    SkASSERT(reinterpret_cast<uintptr_t>(fTail->ptr(alloc.fAlignedOffset)) % Align == 0);

    fTail->fCursor = alloc.fEnd;
    return alloc;
}
...
bool GrBlockAllocator::Block::release(int start, int end) {
    ...
    if (fCursor == end) {
        fCursor = start;
        // SkDebugf("releasing, reset back to %d, kDataStart == %d, block size = %d\n",
                // start, kDataStart, sizeof(Block));
        return true;
    } else {
        return false;
    }
