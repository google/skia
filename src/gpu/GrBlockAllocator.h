 ...
 *
 * // FIXME could we provide an operator new overload that was just
 *   new (totalSize) GrBlockAllocator(policy, heapAllocSize);
 *   and then also have an operator delete, so it can be easily wrapped in a smart pointer?
 * 2. In-place new'd
 ...
 *    template <size_t N>
 *    class Foo {
 *        GrBlockAllocator fAllocator{policy, heapAllocSize, N};
 *        char             fStorage[N];
 *    };
 */
final class GrBlockAllocator : SkNoncopyable {
public:
    ...
    public:
         ...
         * it were released instead). If it would be negative, false is returned and the byte
         * reservation is unmodified.
         *
         ...
         * accomodate the increase in size, or another allocation is blocking the increase in size,
    ...
    // FIXME maybe this should be this->blocks() and this->rblocks() or iter() and riter()?
    // FIXME do we also want non-const versions of the block iteration?
    /**
