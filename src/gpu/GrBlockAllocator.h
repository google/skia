...
public:
     ...
     */
    static std::unique_ptr<GrBlockAllocator> Make(size_t preallocSize,
    ...
    void* allocate(size_t size) {
        int offset = this->allocateInternal<Align>(CheckSize(size));
     ...
     */
    template <size_t Align>
    size_t avail() { return this->avail<Align>(this->currentBlock()); }
...
protected:
    ...
    /**
     * Like the public allocate<Align>(size) except that it also automatically inlines a header of
     ...
     */
    template <size_t Align, typename Meta, typename... MetaArgs>
    ...
    int avail(Block block) const {
        ...
        return blockPtr->fSize - blockPtr->cursor<Align>();
    }

    /**
     * Attempt to resize the allocation described by ('block', 'offset', 'size') to the 'newSize'.
     * It is assumed that the 3-tuple was previously returned from a call to allocate(). Returns
     * true if the allocation was updated to the new size.
     */
    inline bool resize(Block block, int offset, int size, size_t newSize);

    /**
     * Attempt to release the allocation described by ('block', 'offset', 'size'). It is assumed the
     * 3-tuple was previously returned from a call to allocate(). Returns true if the memory was
     * recovered by the allocator.
     */
    inline bool release(Block block, int offset, int size);
     ...
     * call to allocate<Align, Meta>(...).
     */
    template <size_t Align, typename Meta>
    ...
    static constexpr int kFreedMarker    = 0xCAFEDAAB;
...
private:
    ...
    struct BlockHeader {
        ...
        // Get fCursor, but aligned to Align
        template <size_t Align>
        int cursor() const;
...
std::unique_ptr<T> GrBlockAllocator::Prealloc(size_t preallocSize, CtorArgs... ctorArgs) {
    ...
    preallocSize = std::max(kMinSize, std::min(preallocSize, MaxBlockSize(alignof(BlockHeader))));
...
Meta* GrBlockAllocator::Metadata(void* p) {
    ...
    static constexpr int kPadding = GrAlignTo(sizeof(Meta), Align);
...
void*  GrBlockAllocator::allocate(size_t size, Meta** metadata, MetaArgs... metaArgs) {
    static constexpr int kMaxAlign = std::max(Align, alignof(Meta));
    // Will pad sizeof(Meta) up if Align > alignof(Meta), but if kkMaxAlign is alignof(Meta)
    // then this will be sizeof(Meta) since that will still produce a Align'ed user pointer.
    static constexpr int kPadding = GrAlignTo(sizeof(Meta), Align);
...
int GrBlockAllocator::allocateInternal(int size) {
    ...
    // math will overflow
    static_assert(MaxBlockSize(Align) <= std::numeric_limits<int32_t>::max());
    static constexpr int kMaxInputSize =
            kMaxAllocationSize + GrAlignTo(kMaxAllocMetadataSize, Align);
    static constexpr int kMaxAlignedCursor = GrAlignTo(MaxBlockSize(Align), Align);
    ...
    int alignedCursor = fTail->cursor<Align>();
    if (alignedCursor + size > fTail->fSize) {
        static constexpr int kPadding = GrAlignTo(sizeof(BlockHeader), Align);
        this->addBlock(kPadding + size, MaxBlockSize(Align));
        alignedCursor = fTail->cursor<Align>();
    }
...
int GrBlockAllocator::BlockHeader::cursor() const {
    static_assert(SkIsPow2(Align) && Align > 0);
    // Aligning adds Align - 1 as an intermediate step, so ensure that can't overflow
    static_assert(MaxBlockSize(Align) + Align - 1 <= std::numeric_limits<int32_t>::max());
    if /* constexpr */ (Align <= alignof(std::max_align_t)) {
        // Same as GrAlignTo, but operates on ints instead of size_t
        return (fCursor + Align - 1) & ~(Align - 1);
    } else {
        ...
        uintptr_t alignedPtr = (blockPtr + fCursor + Align - 1) & ~(Align - 1);
...
bool GrBlockAllocator::resize(Block block, int offset, int size, size_t newSize) {
    ...
    SkASSERT(offset >= (int) sizeof(BlockHeader) && offset + size <= bp->fSize);
