...
void GrD3DPipelineState::bindBuffers(GrD3DGpu* gpu, sk_sp<const GrBuffer> indexBuffer,
    ...
    if (vertexBuffer) {
        ...
        const_cast<GrD3DBuffer*>(d3dVertexBuffer)
                ->setResourceState(gpu, D3D12_RESOURCE_STATE_VERTEX_AND_CONSTANT_BUFFER);
    }
    if (instanceBuffer) {
        ...
        const_cast<GrD3DBuffer*>(d3dInstanceBuffer)
                ->setResourceState(gpu, D3D12_RESOURCE_STATE_VERTEX_AND_CONSTANT_BUFFER);
    }
    ...
    if (auto* d3dIndexBuffer = static_cast<const GrD3DBuffer*>(indexBuffer.get())) {
        ...
        const_cast<GrD3DBuffer*>(d3dIndexBuffer)
                ->setResourceState(gpu, D3D12_RESOURCE_STATE_INDEX_BUFFER);
