...
void GrD3DDescriptorTableManager::bindHeapsIfNecessary(GrD3DGpu* gpu) {
    ...
    if (currentCBVSRVHeap.get() != fCurrentCBVSRVDescriptorHeap ||
        currentSamplerHeap.get() != fCurrentSamplerDescriptorHeap) {
        ...
        commandList->addRecycledResource(currentCBVSRVHeap);
        commandList->addRecycledResource(currentSamplerHeap);
        fCurrentCBVSRVDescriptorHeap = currentCBVSRVHeap.get();
        fCurrentSamplerDescriptorHeap = currentSamplerHeap.get();
    }
}

void GrD3DDescriptorTableManager::prepForSubmit(GrD3DGpu* gpu) {
    // Don't necessarily need to reset these as the pools should reset their internal heaps
    // -- but just in case.
    this->resetCurrentHeaps();
