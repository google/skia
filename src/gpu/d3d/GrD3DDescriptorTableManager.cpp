...
void GrD3DDescriptorTableManager::bindHeaps(GrD3DDirectCommandList* commandList) {
    //if (fCBVSRVDescriptorPool.bindingNeedsUpdate() || fSamplerDescriptorPool.bindingNeedsUpdate()) {
        ID3D12DescriptorHeap* heaps[2] = {
            fCBVSRVDescriptorPool.currentD3DDescriptorHeap(),
            fSamplerDescriptorPool.currentD3DDescriptorHeap()
        };
...
ID3D12DescriptorHeap* GrD3DDescriptorTableManager::HeapPool::currentD3DDescriptorHeap() const {
    if (fDescriptorHeaps.size() > 0) {
        return fDescriptorHeaps[fDescriptorHeaps.size() - 1]->d3dDescriptorHeap();
    }
    return nullptr;
}
