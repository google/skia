...
public:
    GrD3DTextureBase(const GrD3DGpu* gpu, const GrD3DTextureInfo& info,
            ...
            , fResource(new Resource(gpu, fInfo.fTexture)) {
        if (fIsBorrowed) {
...
private:
    ...
    public:
        explicit Resource(const GrD3DGpu* gpu)
            : fGpu(gpu)
            , fTexture(nullptr) {
        }

        Resource(const GrD3DGpu* gpu, ID3D12Resource* texture)
            : fGpu(gpu)
            , fTexture(texture) {
            fTexture->AddRef();
        }
