...
std::unique_ptr<GrD3DDescriptorTable> GrD3DResourceProvider::createShaderOrConstantResourceTable(
        unsigned int size) {
    return std::move(fDescriptorTableManager.createShaderOrConstantResourceTable(fGpu, size));
}

std::unique_ptr<GrD3DDescriptorTable> GrD3DResourceProvider::createSamplerTable(unsigned int size) {
    return std::move(fDescriptorTableManager.createSamplerTable(fGpu, size));
}
...
void GrD3DResourceProvider::prepForSubmit() {
    ...
    fDescriptorTableManager.prepForSubmit(fGpu->currentCommandList());
}
