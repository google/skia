...
void GrD3DCaps::init(const GrContextOptions& contextOptions, IDXGIAdapter1* adapter,
    ...
    fMaxPerStageShaderResourceViews =
        (D3D12_RESOURCE_BINDING_TIER_1 == optionsDesc.ResourceBindingTier) ? 128 : INT_MAX;
    fMaxPerStageUnorderedAccessViews =
        (D3D12_RESOURCE_BINDING_TIER_1 == optionsDesc.ResourceBindingTier &&
         D3D_FEATURE_LEVEL_11_0 == flDesc.MaxSupportedFeatureLevel) ? 8 : 64;

    this->initGrCaps(optionsDesc, options2Desc);
    this->initShaderCaps(optionsDesc);

    // TODO: set up formats and stencil

    if (!contextOptions.fDisableDriverCorrectnessWorkarounds) {
        this->applyDriverCorrectnessWorkarounds(adapter);
    }
...
void GrD3DCaps::initShaderCaps(const D3D12_FEATURE_DATA_D3D12_OPTIONS& optionsDesc) {
    ...
    shaderCaps->fMaxFragmentSamplers =
        (D3D12_RESOURCE_BINDING_TIER_1 == optionsDesc.ResourceBindingTier) ? 16 : 2048;
}

void GrD3DCaps::applyDriverCorrectnessWorkarounds(IDXGIAdapter1* adapter) {
    DXGI_ADAPTER_DESC adapterDesc;
    HRESULT hr = adapter->GetDesc(&adapterDesc);

    // Flat interpolation appears to be slow on Qualcomm GPUs. This was tested in GL and is assumed
    // to be true with Vulkan as well, so for now will assume the same for D3D.
    if (kQualcomm_D3DVendor == adapterDesc.VendorId) {
        fShaderCaps->fFlatInterpolationSupport = false;
    }
}
