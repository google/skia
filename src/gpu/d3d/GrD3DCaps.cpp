...
// For many checks in caps, we need to know whether the GrBackendFormat is external or not.
static bool backend_format_is_external(const GrBackendFormat& format) {
// TODO: Add YCbCr support
...
uint64_t GrD3DCaps::computeFormatKey(const GrBackendFormat& format) const {
    ...
    SkASSERT(!backend_format_is_external(format));
