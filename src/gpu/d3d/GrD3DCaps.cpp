...
void GrD3DCaps::initGrCaps(const D3D12_FEATURE_DATA_D3D12_OPTIONS& optionsDesc,
    ...
    // D3D12 doesn't define this, but it's probably safe to assume standard locations
...
void GrD3DCaps::initShaderCaps(const D3D12_FEATURE_DATA_D3D12_OPTIONS& optionsDesc) {
    ...
    // We're not running on Qualcomm so this shouldn't be an issue
    ...
    shaderCaps->fMaxFragmentSamplers =
        // Equal to max SRVs (shader resource views) per stage
        (D3D12_RESOURCE_BINDING_TIER_1 == optionsDesc.ResourceBindingTier) ? 128 : 2048;
}
