...
bool copy_compressed_data(char* mapPtr, DXGI_FORMAT dxgiFormat,
...
sk_sp<GrTexture> GrD3DGpu::onCreateCompressedTexture(SkISize dimensions,
    ...
    sk_sp<GrD3DTexture> tex = GrD3DTexture::MakeNewTexture(this, budgeted, dimensions, resourceDesc,
                                                           isProtected, mipMapsStatus);
    if (!tex) {
        return nullptr;
    }

    GrD3DTexture* d3dTex = (GrD3DTexture*)(tex.get());
    ...
    (void)copy_compressed_data(bufferData, desc.Format, placedFootprints.get(),
                               numRows.get(), rowSizeInBytes.get(), data,
                               mipLevelCount);

    GrD3DBuffer* d3dBuffer = static_cast<GrD3DBuffer*>(transferBuffer.get());
    fCurrentDirectCommandList->copyBufferToTexture(d3dBuffer, d3dTex, mipLevelCount,
...
bool GrD3DGpu::onUpdateBackendTexture(const GrBackendTexture& backendTexture,
    ...
    if (data->type() == BackendTextureData::Type::kPixmaps) {
        ...
        result = copy_compressed_data(bufferData, info.fFormat, placedFootprints.get(),
                                      numRows.get(), rowSizeInBytes.get(), data->compressedData(),
                                      info.fLevelCount);
    } else {
        ...
        if (SkImage::CompressionType::kNone == compression) {
            ...
            result = copy_compressed_data(bufferData, info.fFormat, placedFootprints.get(),
                                          numRows.get(), rowSizeInBytes.get(), tempData.get(),
                                          info.fLevelCount);
        }
    }
    transferBuffer->unmap();
