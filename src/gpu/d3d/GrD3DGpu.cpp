...
bool GrD3DGpu::submitDirectCommandList(SyncQueue sync) {
    ...
    for (int i = 0; i < fTemporaryCPUDescriptors.count(); ++i) {
        fResourceProvider.recycleShaderView(fTemporaryCPUDescriptors[i]);
    }
    fTemporaryCPUDescriptors.reset();
...
bool GrD3DGpu::onRegenerateMipMapLevels(GrTexture * tex) {
    ...
    //*** TODO: check for typed format support?
    ...
    DXGI_FORMAT originalFormat = d3dTex->d3dResource()->GetDesc().Format;
    bool isBGR = SkToBool(originalFormat == DXGI_FORMAT_B8G8R8A8_UNORM);

    // if the format is not unordered accessible and not bgra, copy to resource with
    // R8G8B8A8 format and flag set. If SRGB adjust shader to sample in linear.

    // if the format is bgra
    // * use R8G8B8A8 copy to generate mips
    // * insert aliasing barrier R8G8B8A8->alias
    // * copy alias back to original

    sk_sp<GrD3DTexture> bgrAlias;
    ...
    if (caps.isFormatUnorderedAccessible(d3dTex->dxgiFormat()) &&
        ...
        // if the format is unordered accessible, copy to resource
        // with same format (or typeless?) and flag set
        if (caps.isFormatUnorderedAccessible(d3dTex->dxgiFormat())) {
            //*** set typeless format?
        } else {
        ...
        uavTexture = GrD3DTexture::MakeNewTexture(this, SkBudgeted::kNo, tex->dimensions(),
                                                  uavDesc, GrProtected::kNo /*for now*/,
                                                  tex->mipmapStatus());

        d3dTex->setResourceState(this, D3D12_RESOURCE_STATE_COPY_SOURCE);
        if (isBGR) {
            // create bgrAlias and copy to that
            bgrAlias = GrD3DTexture::MakeAliasingTexture(this, uavTexture, originalFormat);
            bgrAlias->setResourceState(this, D3D12_RESOURCE_STATE_COPY_DEST);
            this->currentCommandList()->copyTextureToTexture(bgrAlias.get(), d3dTex, 0);
            // transition alias to uavTexture
            this->currentCommandList()->aliasingBarrier(bgrAlias->resource(),
                                                        bgrAlias->d3dResource(),
                                                        uavTexture->resource(),
                                                        uavTexture->d3dResource());
        } else {
            // otherwise just copy directly to uavTexture
    ...
    //*** TODO: use NPOT shader if necessary
    //*** use &?
    ...
    for (unsigned int dstMip = 1; dstMip < levelCount; ++dstMip) {
        ...
        fTemporaryCPUDescriptors.push_back(srvHandle);
        // create UAV
        ...
        fTemporaryCPUDescriptors.push_back(uavHandle);
        ...
        // change resource state of dstMip subresource so we can write to it
        ...
        this->currentCommandList()->dispatch(std::max(width/8, 1), std::max(height/8, 1));

        // guarantee UAV writes have completed
        this->currentCommandList()->uavBarrier(uavTexture->resource(), uavTexture->d3dResource());

        // change resource state of dstMip subresource so we can read in the next stage
    ...
    if (uavTexture.get() != d3dTex) {
        d3dTex->setResourceState(this, D3D12_RESOURCE_STATE_COPY_DEST);
        if (bgrAlias) {
            // transition alias to bgrAlias
            this->currentCommandList()->aliasingBarrier(uavTexture->resource(),
                                                        uavTexture->d3dResource(),
                                                        bgrAlias->resource(),
                                                        bgrAlias->d3dResource());
            // copy to bgrAlias
            bgrAlias->setResourceState(this, D3D12_RESOURCE_STATE_COPY_SOURCE);
            this->currentCommandList()->copyTextureToTexture(d3dTex, bgrAlias.get(), (UINT)-1);
        } else {
            ...
            this->addResourceBarriers(uavTexture->resource(), 1, &barrier);
            this->currentCommandList()->copyTextureToTexture(d3dTex, uavTexture.get(), (UINT)-1);
        }
    } else {
        // patch up resource state back to original
        barrier.Subresource = D3D12_RESOURCE_BARRIER_ALL_SUBRESOURCES;
        barrier.StateBefore = D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE;
