...
static bool check_image_info(const GrD3DCaps& caps,
...
static bool check_tex_image_info(const GrD3DCaps& caps, const GrD3DTextureResourceInfo& info) {
...
static bool check_rt_image_info(const GrD3DCaps& caps, const GrD3DTextureResourceInfo& info,
                                int sampleCnt) {
...
sk_sp<GrTexture> GrD3DGpu::onWrapBackendTexture(const GrBackendTexture& tex, GrColorType colorType,
                                                GrWrapOwnership ownership, GrWrapCacheable wrapType,
                                                GrIOType ioType) {
    ...
    if (!check_image_info(this->d3dCaps(), textureInfo, colorType)) {
        return nullptr;
    }

    if (!check_tex_image_info(this->d3dCaps(), textureInfo)) {
        return nullptr;
    }
    ...
    SkASSERT(state);
    return GrD3DTexture::MakeWrappedTexture(this, tex.dimensions(), ownership, wrapType, ioType,
...
sk_sp<GrTexture> GrD3DGpu::onWrapCompressedBackendTexture(const GrBackendTexture& tex,
    ...
    if (!check_image_info(this->d3dCaps(), textureInfo, GrColorType::kUnknown)) {
        return nullptr;
    }
    ...
    sk_sp<GrD3DResourceState> state = tex.getGrD3DResourceState();
    SkASSERT(state);
    return GrD3DTexture::MakeWrappedTexture(this, tex.dimensions(), ownership, wrapType,
                                            kRead_GrIOType, textureInfo, std::move(state));
}
...
sk_sp<GrTexture> GrD3DGpu::onWrapRenderableBackendTexture(const GrBackendTexture& tex,
    ...
    if (!check_rt_image_info(this->d3dCaps(), textureInfo, sampleCnt)) {
        return nullptr;
    }
...
sk_sp<GrRenderTarget> GrD3DGpu::onWrapBackendRenderTarget(const GrBackendRenderTarget& rt,
    ...
    if (!check_image_info(this->d3dCaps(), info, colorType)) {
        return nullptr;
    }

    if (!check_rt_image_info(this->d3dCaps(), info, rt.sampleCnt())) {
        return nullptr;
    }
