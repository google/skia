...
void GrD3DGpu::submitDirectCommandList() {
    ...
    if (!fCurrentDirectCommandList->hasWork()) {
        return;
    }
...
bool GrD3DGpu::onReadPixels(GrSurface* surface, int left, int top, int width, int height,
    ...
    D3D12_PLACED_SUBRESOURCE_FOOTPRINT transferLayout;
    UINT transferNumRows;
    UINT64 transferRowBytes;
    UINT64 transferTotalBytes;
    const UINT64 baseOffset = 0;
    fDevice->GetCopyableFootprints(&desc, 0, 1, baseOffset, &transferLayout, &transferNumRows,
                                   &transferRowBytes, &transferTotalBytes);
    ...
    if (!this->readOrTransferPixels(surface, left, top, width, height, dstColorType,
                                    d3dResource, 0, transferTotalBytes, transferRowBytes)) {
        return false;
    }
    this->submitDirectCommandList();

    const void* mappedMemory = transferBuffer->map();

    SkRectMemcpy(buffer, rowBytes, mappedMemory, transferRowBytes, transferRowBytes,
                 transferNumRows);
...
bool GrD3DGpu::readOrTransferPixels(GrSurface* surface, int left, int top, int width, int height,
                                    GrColorType dstColorType, ID3D12Resource* transferBuffer,
                                    size_t offset,
                                    const D3D12_PLACED_SUBRESOURCE_FOOTPRINT& transferLayout) {
    ID3D12Resource* dst = static_cast<GrD3DTexture*>(surface->asTexture())->d3dResource();
    ...
    srcLocation.pResource = transferBuffer;
    dstLocation.Type = D3D12_TEXTURE_COPY_TYPE_PLACED_FOOTPRINT;
    dstLocation.PlacedFootprint = transferLayout;
    D3D12_BOX srcBox = {};
    srcBox.left = left;
    srcBox.top = top;
    ...
    srcBox.front = 0;
    srcBox.back = 1;
    fCurrentDirectCommandList->copyTextureRegion(dst, &dstLocation, 0, 0, 0, transferBuffer,
                                                 &srcLocation, &srcBox);
...
bool GrD3DGpu::onSubmitToGpu(bool syncCpu) {
    // TODO: implement sync and return value
    this->submitDirectCommandList();
    return true;
}
