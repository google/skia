...
// This constant determines how many OustandingCommandLists are allocated together as a block in
...
static const int kDefaultOustandingAllocCnt = 8;
...
GrD3DGpu::GrD3DGpu(GrContext* context, const GrContextOptions& contextOptions,
        ...
        , fOutstandingCommandLists(sizeof(OutstandingCommandList), kDefaultOustandingAllocCnt) {
    ...
    SkDEBUGCODE(HRESULT hr = ) fDevice->CreateFence(0, D3D12_FENCE_FLAG_NONE,
                                                    IID_PPV_ARGS(&fFence));
...
void GrD3DGpu::submitDirectCommandList() {
    ...
    // heled by a finished command list causes us send a new command to the gpu (like changing the
    // resource state.
