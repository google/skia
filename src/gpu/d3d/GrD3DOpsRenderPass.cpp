...
bool GrD3DOpsRenderPass::set(GrRenderTarget* rt, GrSurfaceOrigin origin, const SkIRect& bounds,
    ...
    for (int i = 0; i < sampledProxies.count(); ++i) {
        if (sampledProxies[i]->isInstantiated()) {
            ...
            GrD3DTexture* d3dTex = static_cast<GrD3DTexture*>(sampledProxies[i]->peekTexture());
            SkASSERT(d3dTex);
            d3dTex->setResourceState(fGpu, D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE);
        }
    }
...
#ifdef SK_DEBUG
void check_sampled_texture(GrTexture* tex, GrRenderTarget* rt, GrD3DGpu* gpu) {
    SkASSERT(!tex->isProtected() || (rt->isProtected() && gpu->protectedContext()));
    GrD3DTexture* d3dTex = static_cast<GrD3DTexture*>(tex);
    SkASSERT(SkToBool(d3dTex->currentState() & D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE));
}
#endif
