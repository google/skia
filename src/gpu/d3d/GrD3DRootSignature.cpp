...
sk_sp<GrD3DRootSignature> GrD3DRootSignature::Make(GrD3DGpu* gpu, int numTextureSamplers) {
    // Just allocate enough space for 3 incase we need it.
    ...
    if (numTextureSamplers) {
        // Now handle the textures and samplers. We need a range for each sampler cause of the
        // interaction between how we set bindings and spirv-cross. Each binding value is used for
        // the register value in the HLSL shader. So setting a bining of i for a texture will give
        ...
        // a continguous range of HLSL registers so we must define a different range for each.
    ...
    // We always assume we at least have a parameter slot for uniforms. Otherwise we need a
    // descriptor table for the textures and another for the samplers.
