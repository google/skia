...
GrD3DAttachmentViewManager::Heap::Heap(GrD3DGpu* gpu, D3D12_DESCRIPTOR_HEAP_TYPE type,
                                       unsigned int numDescriptors)
    : fFreeBlocks(numDescriptors)
    , fFreeCount(numDescriptors) {
    fHeap = GrD3DDescriptorHeap::Make(gpu, type, numDescriptors, D3D12_DESCRIPTOR_HEAP_FLAG_NONE);
...
GrD3DAttachmentViewManager::HeapPool::HeapPool(GrD3DGpu* gpu, D3D12_DESCRIPTOR_HEAP_TYPE heapType)
    ...
    std::unique_ptr<GrD3DAttachmentViewManager::Heap> heap(
            new GrD3DAttachmentViewManager::Heap(gpu, fHeapType, fMaxAvailableDescriptors));
