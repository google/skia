...
GrD3DStencilAttachment* GrD3DStencilAttachment::Make(GrD3DGpu* gpu,
    ...
    //*** it'd be better if we initialized the resource in the correct state instead of COMMON
    if (!GrD3DTextureResource::InitTextureResourceInfo(gpu, resourceDesc, GrProtected::kNo, &info)) {
        return nullptr;
    }

    sk_sp<GrD3DResourceState> state(new GrD3DResourceState(D3D12_RESOURCE_STATE_DEPTH_WRITE));
