...
sk_sp<GrD3DSampler> GrD3DSampler::Make(GrD3DGpu* gpu, const GrSamplerState& samplerState) {
    ...
    return sk_sp<GrD3DSampler>(new GrD3DSampler(gpu, samplerDescriptor));
}

GrD3DSampler::~GrD3DSampler() {
    fGpu->resourceProvider().recycleSampler(&fSampler);
}
