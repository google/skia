...
sk_sp<GrGpuBuffer> GrD3DConstantRingBuffer::createBuffer(size_t size) {
    ...
    return GrD3DBuffer::Make(fGpu, size, GrGpuBufferType::kVertex, kStatic_GrAccessPattern);
}

void GrD3DConstantRingBuffer::prepForSubmit() {
    GrD3DBuffer* d3dBuffer = static_cast<GrD3DBuffer*>(this->buffer());
    //*** handle this somewhere else?
    if (d3dBuffer->isMapped()) {
        d3dBuffer->unmap();
    }
    fGpu->currentCommandList()->addResource(d3dBuffer->resource());

    // *** manage the completion handler
    //*** how do I reset the tail?
}
