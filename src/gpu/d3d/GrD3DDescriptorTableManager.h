...
public:
    ...
    GrD3DDescriptorTable createShaderOrConstantResourceTable(GrD3DGpu*, unsigned int count);
    GrD3DDescriptorTable createSamplerTable(GrD3DGpu*, unsigned int count);

    class Heap : public GrRecycledResource {
    public:
        static Heap* Make(GrD3DGpu* gpu, D3D12_DESCRIPTOR_HEAP_TYPE type,
                          unsigned int numDescriptors);

        GrD3DDescriptorTable allocateTable(unsigned int count);
...
private:
    ...
    private:
        std::vector<Heap*> fDescriptorHeaps;
