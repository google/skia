...
GrD3DRenderTarget::GrD3DRenderTarget(GrD3DGpu* gpu,
        ...
        , GrD3DSurfaceResource(info, std::move(state), GrBackendObjectOwnership::kBorrowed)
        // for the moment we only support 1:1 color to stencil
        , GrRenderTarget(gpu, dimensions, sampleCnt, info.fProtected)
        , fMSAATexture(new GrD3DSurfaceResource(msaaInfo, std::move(msaaState),
...
sk_sp<GrD3DRenderTarget> GrD3DRenderTarget::MakeWrappedRenderTarget(
            GrD3DGpu* gpu, SkISize dimensions, int sampleCnt, const GrD3DTextureInfo& info,
            sk_sp<GrD3DResourceState> state) {
    ...
    if (sampleCnt > 1) {
        ...
        if (!GrD3DSurfaceResource::InitTextureInfo(gpu, msTextureDesc, info.fProtected, &msInfo)) {
            return nullptr;
        }

        // Set color attachment image
        colorTexture = msInfo.fTexture;

        msState.reset(new GrD3DResourceState(
                                  static_cast<D3D12_RESOURCE_STATES>(msInfo.fResourceState)));
    } else {
        // Set color attachment image
        colorTexture = info.fTexture;
    }
...
GrD3DRenderTarget::~GrD3DRenderTarget() {
    ...
    SkASSERT(!fMSAATexture);
}

void GrD3DRenderTarget::addResources(GrD3DCommandList& commandList) {
    commandList.addResource(this->msaaTextureResource() ? this->msaaTextureResource()
                                                        : this->resource());
}
...
void GrD3DRenderTarget::releaseInternalObjects() {
    ...
    if (fMSAATexture) {
        fMSAATexture->releaseResource(gpu);
        fMSAATexture.reset();
    }
}
