...
static sk_sp<GrFragmentProcessor> create_fp_for_mask(GrContext* context,
    ...
    return GrDeviceSpaceTextureDecalFragmentProcessor::Make(context, std::move(proxy),
                                                            domainTexels,
...
sk_sp<GrTextureProxy> GrClipStackClip::createAlphaClipMask(
    ...
    if (GrTexture* texture1 = resourceProvider->findAndRefTextureByUniqueKey(key)) {
        return sk_ref_sp(GrSurfaceProxy::MakeWrapped(sk_ref_sp(texture1))->asTextureProxy());
    }
...
sk_sp<GrTextureProxy> GrClipStackClip::createSoftwareClipMask(
    ...
    GrUniqueKey key;
    create_clip_mask_key(reducedClip.elementsGenID(), reducedClip.ibounds(), &key);
    if (GrTexture* texture1 = context->textureProvider()->findAndRefTextureByUniqueKey(key)) {
        return sk_ref_sp(GrSurfaceProxy::MakeWrapped(sk_ref_sp(texture1))->asTextureProxy());
    }
