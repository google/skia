...
static std::unique_ptr<GrFragmentProcessor> create_fp_for_mask(GrSurfaceProxyView mask,
    ...
    auto domain = SkRect::Make(devBound.size()).makeInset(0.5, 0.5);
    auto fp = GrTextureEffect::MakeSubset(std::move(mask), kPremul_SkAlphaType, m, samplerState, subset, domain, caps);
...
bool GrClipStackClip::applyClipMask(GrRecordingContext* context,
    ...
    if ((renderTargetContext->numSamples() <= 1 && reducedClip.maskRequiresAA()) ||
        ...
        if (result) {
            ...
            out->addCoverageFP(create_fp_for_mask(std::move(result), reducedClip.scissor(), *context->priv().caps()));
            return true;
        }
