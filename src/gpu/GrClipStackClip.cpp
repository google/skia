...
bool GrClipStackClip::apply(GrContext* context, GrRenderTargetContext* renderTargetContext,
    ...
    // The opList ID must be looked up AFTER producing the clip mask (if any), because that step can
    // cause a flush or otherwise change which opList our draw is going into.
