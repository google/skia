...
public:
    struct Node {
        template <typename... Args> Node(Args&&... elementConstructorArgs)
                : fElement(std::forward<Args>(elementConstructorArgs)...) {
        }
        T fElement;
        Node* fNext = nullptr;
    };

    template <typename... Args> GrSTArenaList(Args&&... headConstructorArgs)
            : fHead(std::forward<Args>(headConstructorArgs)...) {
    }
