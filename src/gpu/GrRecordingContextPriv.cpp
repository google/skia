...
sk_sp<SkBaseGpuDevice> GrRecordingContextPriv::createDevice(SkBudgeted budgeted,
    ...
    if (this->options().fUseNGA == GrContextOptions::Enable::kYes) {
#ifdef SK_NGA
        return SkGpuDevice_nga::Make(fContext, budgeted, ii, SkBackingFit::kExact, sampleCount,
                                     mipmapped, GrProtected::kNo, origin, props, init);
#endif
    } else {
        return SkGpuDevice::Make(fContext, budgeted, ii, SkBackingFit::kExact, sampleCount,
                                 mipmapped, GrProtected::kNo, origin, props, init);
    }
    return nullptr;
}
