...
private:
    ...
    // The derived class will placement new a GrBackendSemaphore object at the location passed into
    // this function. This is used when flushing with signal semaphores so we can set the clients
    // GrBackendSemaphore object after we've created the internal semaphore.
    virtual void setBackendSemaphore(GrBackendSemaphore* semaphoreLocation) const = 0;
