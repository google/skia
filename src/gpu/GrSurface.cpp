...
size_t GrSurface::WorstCaseSize(const GrSurfaceDesc& desc, bool useNextPow2) {
    ...
    int width = useNextPow2 ? SkTMax(GrResourceProvider::kMinScratchTextureSize, GrNextPow2(desc.fWidth)) : desc.fWidth;
    int height = useNextPow2 ? SkTMax(GrResourceProvider::kMinScratchTextureSize, GrNextPow2(desc.fHeight)) : desc.fHeight;
...
size_t GrSurface::ComputeSize(GrPixelConfig config,
                              ...
                              bool useNextPow2) {
    width = useNextPow2 ? SkTMax(GrResourceProvider::kMinScratchTextureSize, GrNextPow2(width)) : width;
    height = useNextPow2 ?  SkTMax(GrResourceProvider::kMinScratchTextureSize, GrNextPow2(height)) : height;
