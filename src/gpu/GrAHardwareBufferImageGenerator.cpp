...
static bool can_import_protected_content(GrContext* context) {
    if (GrBackend::kOpenGL == context->contextPriv().getBackend()) {
...
static GrBackendTexture make_vk_backend_texture(
    ...
    SkASSERT(context->contextPriv().getBackend() == GrBackend::kVulkan);
...
static GrBackendTexture make_backend_texture(
    ...
    if (GrBackend::kOpenGL == context->contextPriv().getBackend()) {
        ...
        SkASSERT(GrBackend::kVulkan == context->contextPriv().getBackend());
#ifdef SK_VULKAN
...
GrBackendFormat get_backend_format(GrBackend backend, uint32_t bufferFormat) {
    ...
    } else if (backend == GrBackend::kVulkan) {
        switch (bufferFormat) {
            //TODO: find out if we can detect, which graphic buffers support GR_GL_TEXTURE_2D
            case AHARDWAREBUFFER_FORMAT_R8G8B8A8_UNORM:
...
sk_sp<GrTextureProxy> GrAHardwareBufferImageGenerator::makeProxy(GrContext* context) {
    ...
    if (context->contextPriv().getBackend() == GrBackend::kOpenGL) {
...
bool GrAHardwareBufferImageGenerator::onIsValid(GrContext* context) const {
    ...
    return GrBackend::kOpenGL == context->contextPriv().getBackend() ||
           GrBackend::kVulkan == context->contextPriv().getBackend();
}
