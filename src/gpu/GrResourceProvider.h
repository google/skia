...
private:
    ...
    // Used perform any conversions necessary to texel data before creating a texture with existing
    // data or uploading to a scratch texture.
    using TempLevels = SkAutoSTMalloc<14, GrMipLevel>;
    using TempLevelDatas = SkAutoSTArray<14, std::unique_ptr<char[]>>;
    GrColorType prepareLevels(const GrBackendFormat& format, GrColorType, const SkISize& baseSize,
    ...
    // on failure.
    sk_sp<GrTexture> writePixels(sk_sp<GrTexture> texture, GrColorType colorType,
