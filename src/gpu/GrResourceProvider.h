...
public:
     ...
     */
    sk_sp<GrTexture> createApproxTexture(const GrSurfaceDesc3&, uint32_t flags);

    /** Create an exact fit texture with no initial data to upload.
     */
    sk_sp<GrTexture> createTexture(const GrSurfaceDesc3&, SkBudgeted, uint32_t flags = 0);
...
private:
    ...
    sk_sp<GrTexture> refScratchTexture(const GrSurfaceDesc3&, uint32_t scratchTextureFlags);

    /*
     * Try to find an existing scratch texture that exactly matches 'desc'. If successful
     * update the budgeting accordingly.
     */
    sk_sp<GrTexture> getExactScratch(const GrSurfaceDesc3&, SkBudgeted, uint32_t flags);
