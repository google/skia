...
public:
     ...
     * and height as desc specifies. If renderable specifies that the texture should be a render
     * target then result will be a render target. Format and sample count will always match the
     * request. The contents of the texture are undefined.
     */
    sk_sp<GrTexture> createApproxTexture(const GrSurfaceDesc&, GrRenderable, Flags);

    /** Create an exact fit texture with no initial data to upload. */
    sk_sp<GrTexture> createTexture(const GrSurfaceDesc&, GrRenderable, SkBudgeted, Flags = Flags::kNone);
    ...
    sk_sp<GrTexture> createTexture(const GrSurfaceDesc&, GrRenderable renderable,
            SkBudgeted, SkBackingFit, const GrMipLevel&, Flags);
