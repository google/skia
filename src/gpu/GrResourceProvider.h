...
public:
    ...
    /**
     * Finds a resource in the cache, based on the specified key. The caller must be sure that
     * if a resource of exists in the cache with the given unique key then it is of type T prior to
     * calling this.
     */
    template <typename T>
    sk_sp<T> findAndRefTByUniqueKey(const GrUniqueKey& key) {
        return sk_sp<T>(static_cast<T*>(this->findAndRefResourceByUniqueKey(key).release()));
    }
    ...
    sk_sp<const GrBuffer> findOrCreatePatternedIndexBuffer(const uint16_t* pattern,
        ...
        if (auto buffer = this->findAndRefTByUniqueKey<GrBuffer>(key)) {
            return buffer;
        }
    ...
    sk_sp<const GrBuffer> refQuadIndexBuffer() {
        sk_sp<const GrBuffer> buffer = this->findAndRefTByUniqueKey<GrBuffer>(fQuadIndexBufferKey);
        if (buffer) {
            return buffer;
        }
...
private:
    sk_sp<GrGpuResource> findAndRefResourceByUniqueKey(const GrUniqueKey&);
