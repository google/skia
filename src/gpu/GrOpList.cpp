...
void GrOpList::instantiateDeferredProxies(GrResourceProvider* resourceProvider) {
    for (int i = 0; i < fDeferredProxies.count(); ++i) {
        if (!fDeferredProxies[i]->instantiate(resourceProvider)) {
            resourceProvider->clearUniqueKeyOnProxy(fDeferredProxies[i]);
        }
    }
}
...
void GrOpList::prepare(GrOpFlushState* flushState) {
    for (int i = 0; i < fDeferredProxies.count(); ++i) {
        fDeferredProxies[i]->prepareUpload(flushState);
    }
