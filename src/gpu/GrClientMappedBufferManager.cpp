...
GrClientMappedBufferManager::~GrClientMappedBufferManager() {
    this->process();
    // If we're going down before we got the messages we go ahead and unmap all the buffers. It's
    // up to the client to ensure that they aren't being accesed on another thread while this is
    // happening (or afterwards on any thread).
...
void GrClientMappedBufferManager::remove(const sk_sp<GrGpuBuffer>& b) {
    ...
    SkASSERT(std::find(fClientHeldBuffers.begin(), end, b) != end);
    for (auto cur = prev++; cur != end; prev = cur++) {
        if (*cur == b) {
