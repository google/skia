...
void GrGLSLFragmentProcessor::invokeChild(int childIndex, const char* inputColor, EmitArgs& args,
    ...
    this->internalInvokeChild(childIndex, inputColor, args.fOutputColor, args, asFunction);
}

void GrGLSLFragmentProcessor::invokeChild(int childIndex, const char* inputColor,
                                        SkString* outputColor, EmitArgs& args, bool asFunction) {
    ...
    if (asFunction) {
        ...
        this->internalInvokeChild(childIndex, inputColor, outputColor->c_str(), args, asFunction);
    }
}
...
void GrGLSLFragmentProcessor::internalInvokeChild(int childIndex, const char* inputColor,
    ...
    EmitArgs childArgs(fragBuilder,
                       ...
                       childProc,
                       outputColor,
                       asFunction ? "_input" : (inputName.size() > 0 ? inputName.c_str() : nullptr),
                       coordVars,
                       textureSamplers);
    if (asFunction) {
        ...
        fragBuilder->codeAppendf("%s = %s(%s);\n", args.fOutputColor,
                                                   fFunctionNames[childIndex].c_str(),
                                                   inputName.size() > 0 ? inputName.c_str() : "half4(1)");
    } else {
