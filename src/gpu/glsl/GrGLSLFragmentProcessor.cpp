...
void GrGLSLFragmentProcessor::invokeChild(int childIndex, const char* inputColor,
    ...
    if (fFunctionNames[childIndex].size() == 0) {
        ...
        for (int i = 0; i < coordVars.count(); ++i) {
            coordParams.push_back(fragBuilder->ensureCoords2D(coordVars[i]));
        }
        bool needRestore = false;
        GrShaderVar coordVar;
        if (skslCoords.length()) {
            ...
            if (find_transformed_coords(args.fTransformedCoords.childInputs(childIndex),
                                        ...
                                        &coordVar,
                                        &transform)) {
                printf("got transform for %s:\n", coordVar.c_str());
                transform.getMatrix().dump();
                fragBuilder->codeAppendf("float2 %s_saved = %s_override;\n", coordVar.c_str(),
                                         coordVar.c_str());
                fragBuilder->codeAppendf("%s_override = %s;\n", coordVar.c_str(),
                                         skslCoords.c_str());
                needRestore = true;
            }
        }
        ...
        if (needRestore) {
            fragBuilder->codeAppendf("%s_override = %s_saved;\n", coordVar.c_str(),
                                     coordVar.c_str());
        }
    }
}
...
void GrGLSLFragmentProcessor::internalInvokeChild(int childIndex, const char* inputColor,
    ...
    if (skslCoordsArg.length()) {
        ...
        if (find_transformed_coords(args.fTransformedCoords.childInputs(childIndex),
            ...
            fragBuilder->codeAppendf("float2 %s_saved = %s_override;\n", coordVar.c_str(),
                                     coordVar.c_str());
            fragBuilder->codeAppendf("%s_override = (float3(%s, 1) * uCoordTransformMatrix%s).xy;\n",
                                     coordVar.c_str(), skslCoordsArg.c_str(),
                                     coordVar.c_str() + 18);
            needRestore = true;
        }
    }
