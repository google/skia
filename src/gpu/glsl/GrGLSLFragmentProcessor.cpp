...
SkString GrGLSLFragmentProcessor::invokeChild(int childIndex, const char* inputColor,
    ...
    if (skslCoords.empty()) {
        // Empty coords means passing through the coords of the parent, so just inherit it
        skslCoords = args.fLocalCoord;
    }
    ...
    if (childProc.isSampledWithExplicitCoords()) {
        // The child's function takes a half4 color and a float2 coordinate. If the child is
        // sampled explicitly, then skslCoords can be anything.
        SkASSERT(childProc.isSampledWithExplicitCoords() || skslCoords == args.fLocalCoord);
        ...
        SkASSERT(skslCoords == args.fLocalCoord && childProc.sampleMatrix().isNoOp());
    ...
    if (childProc.isSampledWithExplicitCoords()) {
        ...
        if (childProc.sampleMatrix().fHasPerspective) {
            ...
            fragBuilder->codeAppendf("float3 %s = %s * %s.xy1;\n",
                                     coords3.c_str(), skslMatrix.c_str(), args.fLocalCoord);
            ...
            return SkStringPrintf("%s(%s, (%s * %s.xy1).xy)",
                                  fFunctionNames[childIndex].c_str(),
                                  inputColor ? inputColor : "half4(1)",
                                  skslMatrix.c_str(), args.fLocalCoord);
        }
    } else {
