...
void GrGLSLShaderBuilder::appendColorGamutXform(SkString* out,
    ...
    if (colorXformHelper->applySrcTF()) {
        static const GrShaderVar gSrcTFArgs[] = { GrShaderVar("x", kHalf_GrSLType) };
        const char* coeffs = uniformHandler->getUniformCStr(colorXformHelper->srcTFUniform());
        SkString body;
        ...
        this->emitFunction(kHalf_GrSLType, "src_tf", SK_ARRAY_COUNT(gSrcTFArgs), gSrcTFArgs,
                           body.c_str(), &srcTFFuncName);
    }

    SkString dstTFFuncName;
    if (colorXformHelper->applyDstTF()) {
        static const GrShaderVar gDstTFArgs[] = { GrShaderVar("x", kHalf_GrSLType) };
        const char* coeffs = uniformHandler->getUniformCStr(colorXformHelper->dstTFUniform());
        SkString body;
        // Temporaries to make evaluation line readable
        body.appendf("half G = %s[0];", coeffs);
        body.appendf("half A = %s[1];", coeffs);
        ...
        body.append("x = abs(x);");
        body.appendf("return s * ((x < D) ? (C * x) + F : pow(A * x + B, G) + E);");
        this->emitFunction(kHalf_GrSLType, "dst_tf", SK_ARRAY_COUNT(gDstTFArgs), gDstTFArgs,
                           body.c_str(), &dstTFFuncName);
    }
