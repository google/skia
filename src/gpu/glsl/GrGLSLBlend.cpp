...
static void hard_light(GrGLSLFragmentBuilder* fsBuilder, const char* final, const char* src,
                       const char* dst) {
...
static void color_dodge_component(GrGLSLFragmentBuilder* fsBuilder, const char* final,
...
static void color_burn_component(GrGLSLFragmentBuilder* fsBuilder, const char* final,
                                 const char* src, const char* dst, const char component) {
    ...
    fsBuilder->codeAppend("} else {");
    fsBuilder->codeAppendf("float d = max(0.0, %s.a - (%s.a - %s.%c) * %s.a / %s.%c);", dst, dst,
                           dst, component, src, src, component);
...
static void soft_light_component_pos_dst_alpha(GrGLSLFragmentBuilder* fsBuilder, const char* final,
...
static void add_lum_function(GrGLSLFragmentBuilder* fsBuilder, SkString* setLumFunction) {
    ...
    fsBuilder->emitFunction(kFloat_GrSLType, "luminance", SK_ARRAY_COUNT(getLumArgs), getLumArgs,
                            getLumBody.c_str(), &getFunction);

    // Emit the set luminance function.
    GrShaderVar setLumArgs[] = {
            GrShaderVar("hueSat", kVec3f_GrSLType), GrShaderVar("alpha", kFloat_GrSLType),
    ...
    setLumBody.append(
            ...
            "}"
            "return outColor;");
    fsBuilder->emitFunction(kVec3f_GrSLType, "set_luminance", SK_ARRAY_COUNT(setLumArgs),
...
static void add_sat_function(GrGLSLFragmentBuilder* fsBuilder, SkString* setSatFunction) {
    ...
    fsBuilder->emitFunction(kFloat_GrSLType, "saturation", SK_ARRAY_COUNT(getSatArgs), getSatArgs,
    ...
    GrShaderVar helperArgs[] = {
            GrShaderVar("minComp", kFloat_GrSLType), GrShaderVar("midComp", kFloat_GrSLType),
            GrShaderVar("maxComp", kFloat_GrSLType), GrShaderVar("sat", kFloat_GrSLType),
    };
    static const char kHelperBody[] =
            ...
            "return vec3(0, 0, 0);"
            "}";
    fsBuilder->emitFunction(kVec3f_GrSLType, "set_saturation_helper", SK_ARRAY_COUNT(helperArgs),
                            helperArgs, kHelperBody, &helperFunction);

    GrShaderVar setSatArgs[] = {
            GrShaderVar("hueLumColor", kVec3f_GrSLType), GrShaderVar("satColor", kVec3f_GrSLType),
    };
    const char* helpFunc = helperFunction.c_str();
    SkString setSatBody;
    setSatBody.appendf(
            "float sat = %s(satColor);"
    ...
    fsBuilder->emitFunction(kVec3f_GrSLType, "set_saturation", SK_ARRAY_COUNT(setSatArgs),
