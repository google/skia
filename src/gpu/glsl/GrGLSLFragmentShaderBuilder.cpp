...
void GrGLSLFragmentShaderBuilder::applyFnToMultisampleMask(
    ...
    this->codeAppendf(        "for (int i = 0; i < %i; ++i) {",
                              fProgramBuilder->effectiveSampleCnt());
...
void GrGLSLFragmentShaderBuilder::onFinalize() {
    ...
    if (CustomFeatures::kSampleLocations & fProgramBuilder->header().processorFeatures()) {
        ...
        this->definitions().append("const half2 _sampleOffsets[] = half2[](");  // TODO:float?
        for (int i = 0; i < sampleLocations.count(); ++i) {
            ...
            this->definitions().appendf("half2(%f, %f)", offset.x(), offset.y());
