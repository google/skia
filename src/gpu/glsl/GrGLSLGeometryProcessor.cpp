...
void GrGLSLGeometryProcessor::setupPosition(GrGLSLVertexBuilder* vertBuilder,
                                            GrGPArgs* gpArgs,
                                            const char* posName) {
    gpArgs->fPositionVar.set(kHalf2_GrSLType, "pos2");
    vertBuilder->codeAppendf("half2 %s = %s;", gpArgs->fPositionVar.c_str(), posName);
}
    ...
    if (mat.isIdentity()) {
        ...
        *viewMatrixUniform = uniformHandler->addUniform(kVertex_GrShaderFlag,
        ...
        if (!mat.hasPerspective()) {
            gpArgs->fPositionVar.set(kHalf2_GrSLType, "pos2");
            vertBuilder->codeAppendf("half2 %s = (%s * half3(%s, 1)).xy;",
                                     gpArgs->fPositionVar.c_str(), viewMatrixName, posName);
        } else {
            gpArgs->fPositionVar.set(kHalf3_GrSLType, "pos3");
            vertBuilder->codeAppendf("half3 %s = %s * half3(%s, 1);",
                                     gpArgs->fPositionVar.c_str(), viewMatrixName, posName);
        }
    }
}
