...
protected:
    ...
    struct GrGPArgs {
        // Used to specigu The output variable used by a GP to store its device position. It can be
    ...
    // just writes a variable into the output variable with the assumption that the position is 2D.
    // The second version transforms the input position by a view matrix and the output variable is
    // 2D or 3D depending on whether the view matrix is perspective.
