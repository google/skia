...
// whether child processors are written as functions (as opposed to inline code) by default
static constexpr int AS_FUNCTION_DEFAULT = true;
...
public:
    ...
    inline void invokeChild(int childIndex, SkString* outputColor, EmitArgs& parentArgs,
                            bool asFunction = AS_FUNCTION_DEFAULT) {
        this->invokeChild(childIndex, nullptr, outputColor, parentArgs, asFunction);
    }
     ...
     *  therefore in an in/out param. It will be declared in mangled form by invokeChild(). It is
    ...
    void invokeChild(int childIndex, const char* inputColor, SkString* outputColor,
                     EmitArgs& parentArgs, bool asFunction = AS_FUNCTION_DEFAULT);

    // Use the parent's output color to hold child's output, and use the
    // default input color of solid white
    inline void invokeChild(int childIndex, EmitArgs& args, bool asFunction = AS_FUNCTION_DEFAULT) {
        // null pointer cast required to disambiguate the function call
        this->invokeChild(childIndex, (const char*) nullptr, args, asFunction);
    }
...
private:
    void internalInvokeChild(int, const char*, const char*, EmitArgs&, bool);
