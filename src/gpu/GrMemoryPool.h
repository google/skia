...
public:
    ...
    void printStats() const {
        /*int blockCount = 0;
        int liveCount = 0;
        for (const GrBlockAllocator::Block* b : GrBlockAllocator::Blocks(&fAllocator)) {
            blockCount++;
            liveCount += b->metadata(); //b->fLiveCount;
            // b = b->fNext;
        }

        SkDebugf("Stats: block count = %d, live count = %d, size = %d, sizeof(Block) = %d, sizeof(Pool) = %d, sizeof(Alloc) = %d, align = %d\n",
                blockCount, liveCount, this->size(), sizeof(GrBlockAllocator::Block), sizeof(GrMemoryPool), GrAlignTo(sizeof(Header), kAlignment), kAlignment);
    */
    }
