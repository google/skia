 ...
 * than 2^32 bytes, each block is limited to 2^32 bytes.
 */
...
public:
    ...
    static constexpr uint32_t kMinAllocationSize = 1 << 10;
     ...
     */
    std::unique_ptr<GrBlockAllocator> Make(uint32_t preallocSize,
     ...
     * current block.
     */
    void* allocate(uint32_t size, uint8_t align) { return this->allocate(size, align, 0); }

    /**
     * Return the maximum allocation size with the given alignment hat can be made without
     * automatically adding a new block.
     */
    uint32_t avail(uint8_t align) { return this->avail(align, 0); }
     ...
     */
    bool resize(void* p, uint32_t currentSize, uint32_t requestedSize) {
     ...
     */
    bool release(void* p, uint32_t size) { return this->release(this->currentBlock(), p, size); }
...
protected:
    ...
    static constexpr uint32_t kAssignedMarker = 0xCDCDCDCD;
    static constexpr uint32_t kFreedMarker    = 0xEFEFEFEF;
    ...
    GrBlockAllocator(void* preallocStart, uint32_t preallocSize, GrowthPolicy policy,
     ...
     * GrBlockAllocator, and must have (void* preallocStart, uint32_t preallocSize) as its first two
     * arguments, followed by the remaining CtorArgs template values.
     */
    template <typename T, typename... CtorArgs>
    static std::unique_ptr<T> Prealloc(uint32_t preallocSize, CtorArgs... ctorArgs) {
        ...
        return std::unique_ptr<T>(new (mem) T(preallocStart, preallocSize, std::forward<CtorArgs>(ctorArgs)...));
    }
     ...
     * current block.
     */
    void* allocate(uint32_t size, uint8_t align, uint8_t allocMetadataBytes);
     ...
     */
    uint32_t avail(uint8_t align, uint8_t allocMetadataBytes) const {
     ...
     * be resized up to.
     */
    uint32_t avail(Block block, uint8_t align, uint8_t allocMetadataBytes) const;
     ...
     * manually released.
     */
    bool resize(Block block, void* p, uint32_t currentSize, uint32_t requestedSize);

    /**
     * Release the allocation 'p' of 'size' bytes back to the given block pointer. 'block' must have
     * been previously returned by currentBlock() or addBlock(), and not been manually released.
     */
    bool release(Block block, void* ptr, uint32_t size) {
     ...
     */
    Block addBlock(uint32_t minimumSize, uint8_t align, uint8_t allocMetadataBytes);
...
private:
    ...
    struct BlockHeader {
        BlockHeader(uint32_t size, uint8_t metadataBytes);

        ~BlockHeader();
        void operator delete(void* p) { ::operator delete(p); }

        // Return the cursor, aligned to 'align' with prefixed padding for 'allocMetadataBytes'.
        uint32_t cursor(uint8_t align, uint8_t allocMetadataBytes) const;

#ifdef SK_DEBUG
        uint32_t      fSentinel;  // known value to check for bad back pointers to blocks
#endif
        ...
        uint32_t      fSize;      // includes the size of the BlockHeader and requested metadata
        uint32_t      fCursor;    // (this + fCursor) points to next available allocation
    };
    ...
    // N1' is limited to 2^24, so 'preallocSize' must be at least 2^8 in order for block allocations
    // to eventually hit the hard 2^32 limit block size for GrBlockAllocator, which is not
    // unreasonable.
...
public:
    ...
    static std::unique_ptr<GrMemoryPool> Make(size_t preallocSize) {
        return GrBlockAllocator::Prealloc<GrMemoryPool>((uint32_t)(preallocSize));
    }
...
private:
    ...
    struct AllocHeader {
#ifdef SK_DEBUG
        uint32_t fSentinel;      // known value to check for memory stomping (e.g., (CD)*)
        int32_t  fID;            // ID that can be used to track down leaks by clients.
#endif
        uint32_t fOffset;        // offset from block ptr to the allocated ptr
        uint32_t fSize;          // size of the allocation
    };
    ...
    GrMemoryPool(void* preallocStart, uint32_t preallocSize);
...
public:
    static std::unique_ptr<GrOpMemoryPool> Make(size_t preallocSize) {
        return GrBlockAllocator::Prealloc<GrOpMemoryPool>((uint32_t)(preallocSize));
    }
...
private:
    ...
    GrOpMemoryPool(void* preallocStart, uint32_t preallocSize)
