...
GrTexture* GrTextureAdjuster::originalTexture() const {
    return fOriginal->instantiate(fContext->resourceProvider());
}
...
GrTexture* GrTextureAdjuster::refCopy(const CopyParams& copyParams) {
    ...
    GrTexture* copy = CopyOnGpu(fContext, texture, contentArea, copyParams);
    if (copy) {
        if (key.isValid()) {
