...
sk_sp<GrTextureProxy> GrTextureAdjuster::refTextureProxyCopy(const CopyParams& copyParams,
                                                             bool willBeMipped) {
    GrProxyProvider* proxyProvider = fContext3->priv().proxyProvider();
...
sk_sp<GrTextureProxy> GrTextureAdjuster::onRefTextureProxyForParams(
    ...
    SkASSERT(this->width() <= fContext3->priv().caps()->maxTextureSize() &&
             this->height() <= fContext3->priv().caps()->maxTextureSize());

    bool needsCopyForMipsOnly = false;
    if (!params.isRepeated() ||
        !GrGpu::IsACopyNeededForRepeatWrapMode(fContext3->priv().caps(), proxy.get(),
                                               proxy->width(), proxy->height(), params.filter(),
                                               &copyParams, scaleAdjust)) {
        needsCopyForMipsOnly = GrGpu::IsACopyNeededForMips(fContext3->priv().caps(),
