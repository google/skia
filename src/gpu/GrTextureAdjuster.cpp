...
sk_sp<GrFragmentProcessor> GrTextureAdjuster::createFragmentProcessor(
    ...
    SkScalar sizeAdjust[2] = { 1.0f, 1.0f };
    sk_sp<GrTexture> texture(this->refTextureSafeForParams(params, nullptr, sizeAdjust));
    if (!texture) {
        return nullptr;
    }
    ...
    if (texture.get() != this->originalTexture()) {
        contentArea = nullptr;
        textureMatrix.postScale(sizeAdjust[0], sizeAdjust[1]);
    }
