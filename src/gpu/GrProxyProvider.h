...
public:
    ...
    /**
     * A helper that uses findOrCreateProxyByUniqueKey() to find a proxy and then the passed in
     * color type to reconstruct a view for the found proxy. It is assumed that if the proxy is
     * renderable then it was created by GrRenderTargetContext::MakeWithFallback and the fallback
     * color type will be used to create the view. The caller must ensure the origin is correct.
     */
