...
protected:
    GrTextureStripAtlasBase(const Desc& desc) : fDesc(desc) {}
...
public:
    ...
    sk_sp<GrTextureStripAtlasBase> addStrip(GrContext*,
                                            const GrTextureStripAtlasBase::Desc&,
...
private:
    ...
    public:
        ...
        // for SkTDynamicHash
        static const GrTextureStripAtlasBase::Desc& GetKey(const AtlasEntry& entry) {
            return entry.fAtlas->fDesc;
        }
        static uint32_t Hash(const GrTextureStripAtlasBase::Desc& desc) {
            return SkOpts::hash(&desc, sizeof(GrTextureStripAtlasBase::Desc));
        }

        sk_sp<GrTextureStripAtlasBase> fAtlas;
    };

    typedef SkTDynamicHash<AtlasEntry, GrTextureStripAtlasBase::Desc> AtlasHash;
