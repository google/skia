...
public:
    ...
    public:
        ...
        virtual ~SendToGpuHelper() {}
    };

    void sendToGpu(const GrPrimitiveProcessor&, SendToGpuHelper*) const;
...
inline void GrMesh::sendToGpu(const GrPrimitiveProcessor& primProc, SendToGpuHelper* helper) const {
    if (this->isInstanced()) {
        if (!this->isIndexed()) {
            helper->sendInstancedMeshToGpu(primProc, fPrimitiveType, fVertexBuffer.get(),
            ...
            helper->sendIndexedInstancedMeshToGpu(primProc, fPrimitiveType, fIndexBuffer.get(),
                                                  fInstanceIndexData.fIndexCount, 0,
                                                  fVertexBuffer.get(), fBaseVertex,
                                                  fInstanceBuffer.get(),
                                                  fInstanceData.fInstanceCount,
                                                  fInstanceData.fBaseInstance);
        }
        return;
    }
    ...
    if (!this->isIndexed()) {
        ...
        helper->sendMeshToGpu(primProc, fPrimitiveType, fVertexBuffer.get(),
                              fNonIndexNonInstanceData.fVertexCount, fBaseVertex);
        return;
    }

    if (0 == fIndexData.fPatternRepeatCount) {
        helper->sendIndexedMeshToGpu(primProc, fPrimitiveType, fIndexBuffer.get(),
