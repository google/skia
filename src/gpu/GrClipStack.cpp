...
void GrClipStack::RawElement::simplify(const SkIRect& deviceBounds, bool forceAA) {
    ...
    fOuterBounds = GrClip::GetPixelIBounds(outer, fAA, kExterior_Bounds);
    ...
    if (fLocalToDevice.isScaleTranslate()) {
        if (fShape.isRect()) {
            ...
            if (fAA == GrAA::kNo && outer.width() >= 1.f && outer.height() >= 1.f) {
                ...
                fInnerBounds = GrClip::GetPixelIBounds(outer, fAA, kInterior_Bounds);
            ...
            fInnerBounds = GrClip::GetPixelIBounds(inner, fAA, kInterior_Bounds);
