...
static bool shape_contains_rect(const GrShape& a, const SkMatrix& aToDevice, const SkMatrix& deviceToA,
    ...
    if (any(deviceQuad.w4f() < SkPathPriv::kW0PlaneDistance)) {
        // Something in B actually projects behind the W = 0 plane and would beclipped to infinity,
        // so it's extremely unlikely that A can contain B.
        return false;
    }
    if (mixedAAMode) {
        // Transform b to a device-space quad and then outset it so its edges are 1/2px out, giving
        // us a buffer to avoid cases where a non-AA clip or draw would snap outside an aa element.
...
GrClip::Effect GrClipStack::apply(GrRecordingContext* context, GrRenderTargetContext* rtc,
    ...
    SkSTArray<4, const RawElement*> elementsForAtlas;
