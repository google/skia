...
static bool shape_contains_rect(const GrShape& a,
    ...
    // If the shape isn't convex, testing the 4 corners of b for containment could give false pos.
    if (!a.convex()) {
        return false;
    }
...
static constexpr float kNonAAPadding = 0.5f * SK_ScalarSqrt2;
    ...
    if (fAA == e.fAA && fLocalToDevice == e.fLocalToDevice) {
        ...
        if (fShape.isRRect() && e.fShape.isRRect()) {
            ...
            return fShape.path().getGenerationID() == e.fShape.path().getGenerationID() ||
                   (fShape.path().getPoints(nullptr, 0) <= 16 && fShape.path() == e.fShape.path());
        } else {
        ...
        // We don't just use e.fOuterBounds because that includes an intersection with the device
        // and would then mess up the scale matrix we attach to the transform.
        SkRect targetQuery = e.fAA == GrAA::kYes ? tightQuery :
                                    tightQuery.makeOutset(kNonAAPadding, kNonAAPadding);
        if (fAA == GrAA::kNo && this->clipType() == ClipState::kComplex) {
