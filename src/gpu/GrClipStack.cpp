...
static bool shape_contains_rect(const GrShape& a, const SkMatrix& aToDevice,
                                const SkMatrix& deviceToA, const SkRect& b, const SkMatrix& bToDevice) {
    if (!a.convex()) {
        return false;
    }
...
#ifdef SK_DEBUG
    SkMatrix checkInverse;
    SkASSERT(!aToDevice.invert(&checkInverse) || checkInverse == deviceToA);
#endif
...
static std::unique_ptr<GrFragmentProcessor> clip_atlas_fp(GrCoverageCountingPathRenderer* ccpr,
    ...
    if (e.fOp == SkClipOp::kIntersect) {
        ...
        //  - When the input FP is null, this works out to (1-sample(ccpr, 1).a) * input
        ...
        //  - ccpr is this element's atlas mask. fp is the chain of prior analytic FPs, and input
        //    is the starting coverage from the geometry processor.
...
public:
    Draw(const SkRect& drawBounds, GrAAType aa)
            : fBounds(GrClip::GetPixelIBounds(drawBounds, aa, GrClip::kExterior_Bounds))
            , fAAType(aa) {
...
GrClipStack::RawElement::RawElement(const SkMatrix& localToDevice, const GrShape& shape,
    ...
    if (!localToDevice.invert(&fDeviceToLocal)) {
        fDeviceToLocal = SkMatrix::Scale(0.f, 0.f); // FIXME what's something that will disable contains?
    }
}
...
GrClipStack::RawElement& GrClipStack::RawElement::operator=(const RawElement& e) {
    ...
    fDeviceToLocal = e.fDeviceToLocal;
    fOp = e.fOp;
    fAA = e.fAA;
...
static constexpr float kNonAAPadding = 0.5f;

bool GrClipStack::RawElement::contains(const Draw& d) const {
    if (fInnerBounds.contains(d.outerBounds())) {
        return true;
    } else {
        // The draw is already specified in device-space, but map the corners into the local space
        // of the clip. If the draw is non-AA we use the pixel bounds, otherwise we start with its
        // analytic bounds. If the clip is non-AA, we outset by an extra pixel to stay away from
        // the edges.
        SkRect queryBounds = d.aa() == GrAA::kYes ? d.bounds() : SkRect::Make(d.outerBounds());
        if (fAA == GrAA::kNo && this->clipType() == ClipState::kComplex) {
            queryBounds.outset(kNonAAPadding, kNonAAPadding);
        }
        return shape_contains_rect(fShape, fLocalToDevice, fDeviceToLocal, queryBounds, SkMatrix::I());
    }
}
...
bool GrClipStack::RawElement::contains(const SaveRecord& s) const {
    if (fInnerBounds.contains(s.outerBounds())) {
        ...
        // This is very similar to contains(Draw) but an aggregate SaveRecord is always non-AA
...
bool GrClipStack::RawElement::contains(const RawElement& e) const {
    ...
    if (fAA == e.fAA && fLocalToDevice == e.fLocalToDevice) {
        ...
        // containment (a intersect b == a implies b contains a).
        if (fShape.isRRect() && e.fShape.isRRect()) {
            return SkRRectPriv::ConservativeIntersect(fShape.rrect(), e.fShape.rrect())
                    == e.fShape.rrect();
        } else {
            return shape_contains_rect(fShape, SkMatrix::I(), SkMatrix::I(), e.fShape.bounds(), SkMatrix::I());
        }
    } else if (fAA == GrAA::kYes && e.fAA == GrAA::kYes) {
        // Map e's local bounds corners into this' local space for a tighter query
        return shape_contains_rect(fShape, fLocalToDevice, fDeviceToLocal, e.fShape.bounds(), e.fLocalToDevice);
    } else {
        // To deal with non-AA clips, we query with an artificial scale matrix that outsets by an
        // extra pixel in device space to make sure 'contained' shapes are away from a non-aa edge.
        SkRect tightQuery = e.fLocalToDevice.mapRect(e.fShape.bounds());
        SkRect targetQuery = e.fAA == GrAA::kNo ? SkRect::Make(e.fOuterBounds) : tightQuery;
        if (fAA == GrAA::kNo && this->clipType() == ClipState::kComplex) {
            targetQuery.outset(kNonAAPadding, kNonAAPadding);
        }
        SkMatrix padTransform = SkMatrix::MakeRectToRect(tightQuery, targetQuery,
                                                         SkMatrix::ScaleToFit::kFill_ScaleToFit);
        padTransform.preConcat(e.fLocalToDevice);
        return shape_contains_rect(fShape, fLocalToDevice, fDeviceToLocal, e.fShape.bounds(), padTransform);
    }
}
...
void GrClipStack::RawElement::simplify(const SkIRect& deviceBounds) {
    ...
    // mapped bounds of the shape. We assume a smooth clip will be handled by coverage.
    GrAAType aaType = fAA == GrAA::kYes ? GrAAType::kCoverage : GrAAType::kNone;
    fOuterBounds = GrClip::GetPixelIBounds(outer, aaType, kExterior_Bounds);
    ...
    if (fLocalToDevice.isScaleTranslate()) {
        if (fShape.isRect()) {
            ...
            if (fAA == GrAA::kNo && outer.width() >= 1.f && outer.height() >= 1.f) {
                ...
                // clip rects we always just round so that they can be scissor-only (ignoring the
                ...
                fInnerBounds = GrClip::GetPixelIBounds(outer, aaType, kInterior_Bounds);
            ...
            fInnerBounds = GrClip::GetPixelIBounds(inner, aaType, kInterior_Bounds);
    ...
    // Post-conditions on inner and outer bounds for non-empty simplification
    SkASSERT(!fShape.isEmpty());
    SkASSERT(!fOuterBounds.isEmpty() && deviceBounds.contains(fOuterBounds));
    SkASSERT(fInnerBounds.isEmpty() || fOuterBounds.contains(fInnerBounds));
}
...
bool GrClipStack::RawElement::combine(const RawElement& other) {
    ...
    if (shapeUpdated) {
        // This logic works under the assumption that both combined elements were intersect, so that
        // it can avoid the more general work in updateBounds()
...
void GrClipStack::SaveRecord::restoreElements(RawElement::Stack* elements) {
    ...
    // for (RawElement& e : elements->ritems()) {
        ...
        //     break;
        // }
    for (int i = restoreCount - 1; i >= fOldestValidIndex; i--) {
        (*elements)[i].restoreValid(restoreCount);
        // --i;
    }
}
...
bool GrClipStack::SaveRecord::appendElement(RawElement&& toAdd, RawElement::Stack* elements) {
    ...
    int oldestValid = elements->count(); //i + 1;
    ...
    int oldestActiveInvalidIndex = elements->count(); //i + 1;

    // for (RawElement& existing : elements->ritems()) {
    //     if (i < fOldestValidIndex) {
    //         break;
    //     }
    for (int i = elements->count() - 1; i >= fOldestValidIndex; i--) {
        RawElement& existing = (*elements)[i];
...
GrClipStack::GrClipStack(const SkIRect& deviceBounds, const SkMatrixProvider* matrixProvider)
    ...
    // SkDebugf("sizeof(GrClipStack): %d, sizeof(RawElement): %d, sizeof(SaveRecord): %d, sizeof(Mask): %d\n",
    //         sizeof(GrClipStack), sizeof(RawElement), sizeof(SaveRecord), sizeof(Mask));
    // SkDebugf("sizeof(Element::Stack): %d, sizeof(SR::Stack): %d, sizeof(M::Stack): %d\n",
    //         sizeof(RawElement::Stack), sizeof(SaveRecord::Stack), sizeof(Mask::Stack));
}
...
GrClip::PreClipResult GrClipStack::preApply(const SkRect& bounds, GrAAType aa) const {
    // GrClipStackClip testClip(fDeviceBounds.size(), &fMirror, fMatrixProvider);

    // GrClip::PreClipResult testResult = testClip.preApply(bounds, aa);
    Draw draw(bounds, aa);
    if (!draw.applyDeviceBounds(fDeviceBounds)) {
        return GrClip::PreClipResult(GrClip::Effect::kClippedOut, 0); //testResult.fOldConsideredCount);
    }
    ...
    if (cs.state() == ClipState::kEmpty) {
        ...
        SkASSERT(!cs.shader());
        return GrClip::PreClipResult(GrClip::Effect::kUnclipped, 0); //testResult.fOldConsideredCount);
    }
    ...
    switch(get_clip_geometry(cs, draw)) {
        ...
        case ClipGeometry::kBOnly:
            ...
            return GrClip::PreClipResult(cs.shader() ? GrClip::Effect::kClipped : GrClip::Effect::kUnclipped, 0); //testResult.fOldConsideredCount);
        ...
        case ClipGeometry::kBoth: {
            ...
            if (cs.state() == ClipState::kDeviceRect) {
                ...
                return {back.shape().rect(), back.aa(), 0}; //testResult.fOldConsideredCount};
            } else if (cs.state() == ClipState::kDeviceRRect) {
                SkASSERT(back.clipType() == ClipState::kDeviceRRect);
                return {back.shape().rrect(), back.aa(), 0}; //testResult.fOldConsideredCount};
            } else {
                ...
                return GrClip::PreClipResult(GrClip::Effect::kClipped, 0); //testResult.fOldConsideredCount);
            }
            break;
        }
    }
...
GrClip::Effect GrClipStack::apply(GrRecordingContext* context, GrRenderTargetContext* rtc,
    ...
    // SkRect testBounds = *bounds;
    // GrClipStackClip testClip(fDeviceBounds.size(), &fMirror, fMatrixProvider);
    // GrAppliedClip testOut(rtc->dimensions(), rtc->asSurfaceProxy()->backingStoreDimensions());
    // testClip.apply(context, rtc, aa, hasUserStencilSettings, &testOut, &testBounds);

    // out->fOldElementsConsidered = testOut.fElementsConsidered;
    // out->fOldAnalyticElements = testOut.fAnalyticElements;
    // out->fOldCCPRElements = testOut.fCCPRElements;
    // out->fOldMaskElements = testOut.fMaskElements;
    // out->fOldMaskInCache = testOut.fMaskInCache;
    ...
    if (cs.op() == SkClipOp::kIntersect) {
        ...
        // FIXME in some cases, GrREducedClip actually does set the scissor tight.... just not axis-aligned and always rounded out
    } else {
    ...
    SkSTArray<4, const Element*> elementsForAtlas;
    ...
    //         continue;
    //     }
    for (int i = fElements.count() - 1; i >= cs.oldestElementIndex(); --i) {
        const RawElement& e = fElements[i];
        if (e.isInvalid()) {
            continue;
        }
        ...
        switch(get_clip_geometry(e, draw)) {
            ...
            case ClipGeometry::kBoth: {
                ...
                if (!fullyApplied && remainingAnalyticFPs > 0) {
                    std::tie(fullyApplied, clipFP) = analytic_clip_fp(e.asElement(), *caps->shaderCaps(),
                                                                 std::move(clipFP));
                    if (fullyApplied) {
                        ...
                        elementsForAtlas.push_back(&e.asElement());
                        remainingAnalyticFPs--;
                        fullyApplied = true;
                    }
                }
    ...
    /*if (out->fAnalyticElements > testOut.fAnalyticElements ||
        elementsForMask.count() > testOut.fMaskElements ||
        elementsForAtlas.count() > testOut.fCCPRElements) {

        int analyticDelta = out->fAnalyticElements - testOut.fAnalyticElements;
        int maskDelta = elementsForMask.count() - testOut.fMaskElements;
        int atlasDelta = elementsForAtlas.count() - testOut.fCCPRElements;

        bool worse = false;
        if (maskDelta > 0) {
            // More masks is always bad
            worse = true;
        } else if (atlasDelta > 0 && (atlasDelta + maskDelta) > 0) {
            // More CCPR that's not just because a shift from mask to CCPR is bad
            worse = true;
        } else if (analyticDelta > 0 && (analyticDelta + maskDelta + atlasDelta) > 0) {
            // More analytic FPs that aren't just masks/CCPR entries moved to FP is bad
            worse = true;
        }

        if (worse && (analyticDelta + maskDelta + atlasDelta) < 0 && maskDelta <= 0) {
            // if net shapes is lower and we're not adding more masks, it's justifiable not worse
            worse = false;
        }

        if (worse) {
            SkDebugf("Worse clip behavior(%d), analytic %d vs %d, mask %d vs %d, ccpr %d vs %d\n",
                    worse, out->fAnalyticElements, testOut.fAnalyticElements,
                    elementsForMask.count(), testOut.fMaskElements, elementsForAtlas.count(), testOut.fCCPRElements);
            SkDebugf("  mask delta: %d, atlas delta %d, fp delta: %d\n",
                    maskDelta, atlasDelta, analyticDelta);
        }
    }*/

    // Fill out the GrAppliedClip with what we know so far
    out->fMaskElements = elementsForMask.count();
    out->fCCPRElements = elementsForAtlas.count();
    ...
    if (!elementsForMask.empty()) {
        ...
        if ((rtc->numSamples() <= 1 && maskRequiresAA) || stencilUnavailable) {
            ...
            std::tie(hasSWMask, clipFP) = GetSWMaskFP(
                    context, &fMasks, cs.genID(), scissorBounds, elementsForMask.begin(),
                    elementsForMask.count(), std::move(clipFP), &out->fMaskInCache);
        }
    ...
    if (!elementsForAtlas.empty()) {
        ...
        for (int i = 0; i < elementsForAtlas.count(); ++i) {
            SkASSERT(elementsForAtlas[i]->fAA == GrAA::kYes);
            clipFP = clip_atlas_fp(ccpr, opsTaskID, scissorBounds, *(elementsForAtlas[i]),
                                   *caps, std::move(clipFP));
        }
    }
...
void GrClipStack::clip(RawElement&& element) {
    ...
    element.simplify(fDeviceBounds);
...
GrFPResult GrClipStack::GetSWMaskFP(GrRecordingContext* context, Mask::Stack* masks,
                                    ...
                                    std::unique_ptr<GrFragmentProcessor> clipFP, bool* cached) {
    GrProxyProvider* proxyProvider = context->priv().proxyProvider();
    ...
    // make sure that the draw bounds are pre-mapped into the masks space as well.
