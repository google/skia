...
static bool shape_contains_rect(const GrShape& a,
                                const SkMatrix& aToDevice,
                                const SkMatrix& deviceToA,
                                const SkRect& b,
                                const SkMatrix& bToDevice) {
    if (!a.convex()) {
        return false;
    }
...
bool GrClipStack::RawElement::contains(const RawElement& e) const {
    ...
    if (fAA == e.fAA && fLocalToDevice == e.fLocalToDevice) {
        ...
        if (fShape.isRRect() && e.fShape.isRRect()) {
            ...
            return shape_contains_rect(fShape, SkMatrix::I(), SkMatrix::I(),
                                       e.fShape.bounds(), SkMatrix::I());
        }
    } else if (fAA == GrAA::kYes && e.fAA == GrAA::kYes) {
        // Map e's local bounds corners into this' local space for a tighter query
        return shape_contains_rect(fShape, fLocalToDevice, fDeviceToLocal,
                                   e.fShape.bounds(), e.fLocalToDevice);
    } else {
