...
GrClipStack::RawElement::RawElement(const SkMatrix& localToDevice, const GrShape& shape,
    ...
    if (!localToDevice.invert(&fDeviceToLocal)) {
        // Scaling by infinity ensures that when shape_contains_rect maps points from device to
        // local, they become infinity and will then fail any containment check, which is the
        // reasonable behavior a non-invertible localToDevice matrix. However, if both elements have
        // the same non-invertible localToDevice matrix, we can still check containment in their
        // shared local space.
        fDeviceToLocal = SkMatrix::Scale(SK_ScalarInfinity, SK_ScalarInfinity);
    }
}
...
bool GrClipStack::RawElement::combine(const RawElement& other, const SaveRecord& current) {
    ...
    // At the moment, only rect+rect or rrect+rrect are supported (although rect+rrect is a
    // treated as a degenerate case of rrect+rrect).
    bool shapeUpdated = false;
    if (fShape.isRect() && other.fShape.isRect()) {
        if (fLocalToDevice.isIdentity() && other.fLocalToDevice.isIdentity() && fAA != other.fAA) {
        ...
        if (fAA == other.fAA && fLocalToDevice == other.fLocalToDevice) {
            ...
            if (!joined.isEmpty()) {
                ...
                // Like the rect+rect combination is actually empty
                fShape.reset();
                this->markInvalid(current);
                return true;
            }
...
// set ever used a mask (which includes stencil masks).
...
// it's so close to the emperically encountered max.
...
GrClip::Effect GrClipStack::apply(GrRecordingContext* context, GrRenderTargetContext* rtc,
    ...
    if (cs.op() == SkClipOp::kIntersect) {
        // NOTE: We could make the scissor be the intersection of the draw bounds and the clip, but
        // that would mean the scissor rect must change between draw calls and potentially breaks
        // batching across draws that otherwise could have been combined if they had the same
        // GrAppliedClip.
