...
public:
    ...
    static void GenKey(const GrPathProcessor& pathProc,
        ...
        b->add32(SkToInt(pathProc.analysis().readsColor()) |
                 (SkToInt(pathProc.analysis().readsCoverage()) << 1) |
    ...
    void emitCode(EmitArgs& args) override {
        ...
        // Setup uniform color
        if (pathProc.analysis().readsColor()) {
        ...
        if (pathProc.analysis().readsCoverage()) {
    ...
    void setData(const GrGLSLProgramDataManager& pd,
        ...
        if (pathProc.analysis().readsColor() && pathProc.color() != fColor) {
            float c[4];
