...
void* GrMemoryPool::allocate(size_t size, size_t alignment) {
    ...
    VALIDATE;
    size += kPerAllocPad;
    intptr_t ptr = GrSizeAlignUp(fTail->fCurrPtr, alignment);
    size_t delta = ptr - fTail->fCurrPtr;
    if (fTail->fFreeSize < size + delta) {
        ...
        ptr = GrSizeAlignUp(fTail->fCurrPtr, alignment);
        delta = ptr - fTail->fCurrPtr;
    }
