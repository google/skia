...
void* GrMemoryPool::allocate(size_t size) {
    ...
    GrBlockAllocator::ByteRange alloc = fAllocator.allocate<1, sizeof(Header)>(size);
...
static GrBlockAllocator::Block* find_block(GrBlockAllocator* allocator, void* p, int start, int padding) {
    ...
    return reinterpret_cast<GrBlockAllocator::Block*>(reinterpret_cast<uintptr_t>(p) - padding - start);
}
...
void GrMemoryPool::release(void* p) {
    ...
    if (alive == 1) {
        ...
        // block->setMetadata(alive - 1);
        bool res = block->release(header->fStart, header->fEnd);
