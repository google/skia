...
sk_sp<GrTextureProxy> GrProxyProvider::createNonMippedProxyFromBitmap(const SkBitmap& bitmap,
    ...
    sk_sp<GrTextureProxy> proxy = this->createLazyProxy(
            [bitmap](GrResourceProvider* resourceProvider, const LazyCallbackSpec& spec) {
                SkASSERT(spec.fMipMapped == GrMipMapped::kNo);
                GrMipLevel mipLevel = { bitmap.getPixels(), bitmap.rowBytes() };
                auto bmpCT = SkColorTypeToGrColorType(bitmap.colorType());
                return LazyCallbackResult(resourceProvider->createTexture(
                        spec.fDimensions, spec.fFormat, bmpCT, spec.fRenderable, spec.fSampleCnt,
                        spec.fBudgeted, spec.fFit, spec.fProtected, mipLevel));
            },
...
sk_sp<GrTextureProxy> GrProxyProvider::createMippedProxyFromBitmap(const SkBitmap& bitmap,
    ...
    sk_sp<GrTextureProxy> proxy = this->createLazyProxy(
            [bitmap, mipmaps](GrResourceProvider* resourceProvider, const LazyCallbackSpec& spec) {
                ...
                return LazyCallbackResult(resourceProvider->createTexture(
                        spec.fDimensions, spec.fFormat, colorType, GrRenderable::kNo, 1,
                        spec.fBudgeted, GrProtected::kNo, texels.get(), mipLevelCount));
            },
...
sk_sp<GrTextureProxy> GrProxyProvider::createCompressedTextureProxy(
    ...
    sk_sp<GrTextureProxy> proxy = this->createLazyProxy(
            [data](GrResourceProvider* resourceProvider, const LazyCallbackSpec& spec) {
                return LazyCallbackResult(resourceProvider->createCompressedTexture(
                        spec.fDimensions, spec.fFormat, spec.fBudgeted, spec.fMipMapped,
                        spec.fProtected, data.get()));
            },
