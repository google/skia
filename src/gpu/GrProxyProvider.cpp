...
sk_sp<GrTextureProxy> GrProxyProvider::createProxy(sk_sp<SkData> data, const GrSurfaceDesc& desc) {
    ...
    const GrBackendFormat format = this->caps()->getBackendFormatFromGrColorType(ct,
                                                                                 GrSRGBEncoded::kNo);
...
sk_sp<GrTextureProxy> GrProxyProvider::createLazyProxy(LazyInstantiateCallback&& callback,
    ...
    LazyInstantiationType lazyType = !this->recordingDDL() ? LazyInstantiationType::kSingleUse
...
bool GrProxyProvider::recordingDDL() const {
    return fImageContext->priv().asRecordingContext() && !fImageContext->priv().asDirectContext();
}
