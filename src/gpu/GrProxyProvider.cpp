...
sk_sp<GrTextureProxy> GrProxyProvider::createProxy(const GrBackendFormat& format,
    ...
    if (desc.fFlags & kRenderTarget_GrSurfaceFlag) {
        copyDesc.fSampleCnt = caps->getRenderTargetSampleCount(desc.fSampleCnt, desc.fConfig);
    }
...
sk_sp<GrTextureProxy> GrProxyProvider::wrapRenderableBackendTexture(
    ...
    SkASSERT(backendTex.config() == caps->getConfigFromBackendFormat(backendTex.getBackendFormat(),
                                                                      colorType));
    SkASSERT(validate_backend_format_and_config(caps, backendTex.getBackendFormat(),
                                                backendTex.config()));
