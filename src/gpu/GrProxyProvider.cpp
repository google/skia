...
sk_sp<GrTextureProxy> GrProxyProvider::wrapBackendTexture(const GrBackendTexture& backendTex,
    ...
    sk_sp<GrTexture> tex = fResourceProvider1->wrapBackendTexture(backendTex, ownership, cacheable, ioType);
    if (!tex) {
        return nullptr;
    }
...
sk_sp<GrTextureProxy> GrProxyProvider::wrapRenderableBackendTexture(
    ...
    sk_sp<GrTexture> tex =
            fResourceProvider1->wrapRenderableBackendTexture(backendTex, sampleCnt, ownership, cacheable);
    if (!tex) {
        return nullptr;
    }
