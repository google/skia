...
sk_sp<GrTextureProxy> GrProxyProvider::testingOnly_createInstantiatedProxy(
        const GrSurfaceDesc& desc, GrRenderable renderable, GrSurfaceOrigin origin, SkBackingFit fit, SkBudgeted budgeted) {
    GrContext* direct = fImageContext->priv().asDirectContext();
    if (!direct) {
        return nullptr;
    }
    ...
    if (SkBackingFit::kApprox == fit) {
        tex = resourceProvider->createApproxTexture(desc, renderable, GrResourceProvider::Flags::kNoPendingIO);
    } else {
    ...
    sk_sp<GrTextureProxy> proxy = this->createLazyProxy(
            [desc, renderable, budgeted, srcImage, fit](GrResourceProvider* resourceProvider) {
                ...
                return LazyInstantiationResult(resourceProvider->createTexture(
                        desc, renderable, budgeted, fit, mipLevel, GrResourceProvider::Flags::kNoPendingIO));
            },
            format, desc, renderable, kTopLeft_GrSurfaceOrigin, GrMipMapped::kNo, surfaceFlags, fit, budgeted);
...
sk_sp<GrTextureProxy> GrProxyProvider::createProxyFromBitmap(const SkBitmap& bitmap,
    ...
    sk_sp<GrTextureProxy> proxy = this->createLazyProxy(
            ...
            format, desc, GrRenderable::kNo, kTopLeft_GrSurfaceOrigin, GrMipMapped::kYes, SkBackingFit::kExact,
            SkBudgeted::kYes);
...
sk_sp<GrTextureProxy> GrProxyProvider::createLazyProxy(LazyInstantiateCallback&& callback,
    ...
    return this->createLazyProxy(std::move(callback), format, desc, renderable, origin, mipMapped, surfaceFlags,
