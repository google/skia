...
void GrProxyProvider::processInvalidProxyUniqueKey(const GrUniqueKey& key, GrTextureProxy* proxy,
    ...
    SkASSERT(proxy);
    SkDebugf("proccessInvalidProxyUK for texProxy: %p\n", proxy);
    if (!proxy->getUniqueKey().isValid()) {
        SkDebugf("proxy isInstantiated: %d\n", proxy->priv().isInstantiated());
        SkDebugf("proxy isMipped: %d\n", proxy->mipMapped() == GrMipMapped::kYes);
        if (proxy->priv().isInstantiated()) {
            GrTexture* tex = proxy->priv().peekTexture();
            SkDebugf("texture: %p\n", tex);
            SkDebugf("tex isMipped: %d\n", tex->texturePriv().mipMapped() == GrMipMapped::kYes);
        }
        GrTexture* badTex = nullptr;
        badTex->texturePriv().mipMapped();
    }
