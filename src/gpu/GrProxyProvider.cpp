...
sk_sp<GrTextureProxy> GrProxyProvider::CreatePromiseProxy(
                                                      GrContextThreadSafeProxy* ctxTsp,
                                                      LazyInstantiateCallback&& callback,
                                                      const GrBackendFormat& format,
                                                      SkISize dimensions,
                                                      GrMipmapped mipMapped) {
    if (ctxTsp->priv().abandoned()) {
        return nullptr;
    }
    SkASSERT((dimensions.fWidth <= 0 && dimensions.fHeight <= 0) ||
             (dimensions.fWidth >  0 && dimensions.fHeight >  0));

    if (dimensions.fWidth > ctxTsp->priv().caps()->maxTextureSize() ||
        dimensions.fHeight > ctxTsp->priv().caps()->maxTextureSize()) {
        return nullptr;
    }
