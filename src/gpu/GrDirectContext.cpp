...
class GLDooDad : public GrUtilityContext::DooDad {
public:
    GLDooDad(sk_sp<const GrGLInterface> intf) : fIntf(std::move(intf)) {}

private:
    sk_sp<const GrGLInterface> fIntf;

    using INHERITED = GrUtilityContext::DooDad;
};
...
sk_sp<GrUtilityContext> GrDirectContext::makeUtilityGL(sk_sp<const GrGLInterface> glInterface) {
    ...
    sk_sp<GrUtilityContext::DooDad> doodad(new GLDooDad(glInterface));

    sk_sp<GrUtilityContext> tmp(new GrUtilityContext(std::move(doodad), threadSafeProxy));
...
GrUtilityContext::GrUtilityContext(sk_sp<DooDad> dooDad,
                                   sk_sp<GrContextThreadSafeProxy> threadSafeProxy)
    : INHERITED(std::move(threadSafeProxy))
    , fDooDad(std::move(dooDad)) {
}
