...
static bool update_texture_with_pixmaps(GrGpu* gpu,
    ...
    if (textureOrigin == kBottomLeft_GrSurfaceOrigin) {
        ...
        for (int i = 0; i < numLevels; ++i) {
            ...
            tempPixmaps[i].reset(srcData[0].info(), tempStorage.get() + size, tempRB);
...
GrBackendTexture GrDirectContext::createBackendTexture(const SkPixmap srcData[],
    ...
    GrBackendTexture beTex = gpu->createBackendTexture(srcData[0].dimensions(),
                                                       backendFormat,
                                                       renderable,
                                                       mipMapped,
                                                       isProtected);
    ...
    if (!update_texture_with_pixmaps(gpu,
                                     srcData,
                                     numProvidedLevels,
                                     beTex,
                                     textureOrigin,
