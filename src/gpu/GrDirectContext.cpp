...
class GLCompilenator : public GrUtilityContext::Compilenator {
public:
    GLCompilenator(sk_sp<const GrGLInterface> intf) : fIntf(std::move(intf)) {}
...
private:
    ...
    using INHERITED = GrUtilityContext::Compilenator;
};
...
sk_sp<GrUtilityContext> GrDirectContext::makeUtilityGL(sk_sp<const GrGLInterface> glInterface) {
    ...
    sk_sp<GrUtilityContext::Compilenator> compilenator(new GLCompilenator(glInterface));
...
GrUtilityContext::GrUtilityContext(sk_sp<Compilenator> compilenator,
                                   sk_sp<GrContextThreadSafeProxy> threadSafeProxy)
    : INHERITED(std::move(threadSafeProxy))
