...
static void stencil_rect(GrRenderTargetContext* rtc,
    ...
    rtc->priv().stencilRect(clip, ss, std::move(paint), aa, view, rect);
}
...
bool GrReducedClip::drawStencilClipMask(GrRecordingContext* context,
    ...
    for (ElementList::Iter iter(fMaskElements); iter.get(); iter.next()) {
        ...
        if (!drawDirectToClip) {
            ...
            if (Element::DeviceSpaceType::kRect == element->getDeviceSpaceType()) {
                stencil_rect(renderTargetContext, stencilClip.fixedClip(), &kDrawToStencil,
                             GrAA(doStencilMSAA), SkMatrix::I(), element->getDeviceSpaceRect());
            } else {
                if (!clipPath.isEmpty()) {
                    GrShape shape(clipPath, GrStyle::SimpleFill());
        ...
        for (GrUserStencilSettings const* const* pass = stencilPasses; *pass; ++pass) {
            if (drawDirectToClip) {
                if (Element::DeviceSpaceType::kRect == element->getDeviceSpaceType()) {
                    stencil_rect(renderTargetContext, stencilClip, *pass, GrAA(doStencilMSAA),
                ...
                stencil_rect(renderTargetContext, stencilClip, *pass, GrAA(doStencilMSAA),
                             SkMatrix::I(), SkRect::Make(fScissor));
            }
        }
    }
    return true;
