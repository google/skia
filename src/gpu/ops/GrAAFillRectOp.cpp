...
std::unique_ptr<GrDrawOp> Make(GrPaint&& paint, const SkMatrix& viewMatrix, const SkRect& rect,
                               const GrUserStencilSettings* stencil) {
...
std::unique_ptr<GrDrawOp> Make(GrPaint&& paint, const SkMatrix& viewMatrix, const SkRect& rect,
                               const SkRect& devRect, const GrUserStencilSettings* stencil) {
    if (!view_matrix_ok_for_aa_fill_rect(viewMatrix)) {
        return nullptr;
    }
    return AAFillRectOp::Make(std::move(paint), viewMatrix, rect, devRect, nullptr, stencil);
...
std::unique_ptr<GrDrawOp> Make(GrPaint&& paint, const SkMatrix& viewMatrix,
...
std::unique_ptr<GrDrawOp> MakeWithLocalRect(GrPaint&& paint, const SkMatrix& viewMatrix,
...
GR_DRAW_OP_TEST_DEFINE(AAFillRectOp) {
    ...
    } while (!viewMatrix.preservesRightAngles());
