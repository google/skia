...
class StaticVertexAllocator : public GrApproxVertexAllocator {
public:
...
#endif
    void* lock(size_t stride, int vertexCount) override {
        SkASSERT(!fLockStride);
        SkASSERT(stride);
...
private:
    ...
    void drawAA(Target* target, const GrGeometryProcessor* gp) {
        ...
        int firstVertex;
        bool isLinear;
        GrApproxDynamicVertexAllocator allocator(target, &vertexBuffer, &firstVertex);
