...
public:
    ...
    ShapeDataKey(const ShapeDataKey& that) {
        fKey.reset(that.fKey.count());
        memcpy(fKey.get(), that.fKey.get(), fKey.count() * sizeof(uint32_t));
    }

    ShapeDataKey& operator=(const ShapeDataKey&) = delete;

    // for SDF paths
    ShapeDataKey(const GrStyledShape& shape, uint32_t dim) {
    ...
    // for bitmap paths
    ShapeDataKey(const GrStyledShape& shape, const SkMatrix& ctm) {
        // Shapes' keys are for their pre-style geometry, but by now we shouldn't have any
        // relevant styling information.
        SkASSERT(shape.style().isSimpleFill());
        SkASSERT(shape.hasUnstyledKey());
...
public:
    GrSmallPathShapeData(const ShapeDataKey& key) : fKey(std::move(key)) {}
    ...
    static inline const ShapeDataKey& GetKey(const GrSmallPathShapeData& data) {
        return data.fKey;
    }
