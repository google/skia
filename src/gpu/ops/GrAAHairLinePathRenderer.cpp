...
public:
    ...
    static std::unique_ptr<GrDrawOp> Make(GrPaint&& paint,
        ...
        return Helper::FactoryHelper<AAHairlineOp>(std::move(paint), newCoverage, viewMatrix, path, devClipBounds, capLength);
    }
    ...
    AAHairlineOp(const Helper::MakeArgs& helperArgs,
                 ...
                 SkScalar capLength)
            : INHERITED(ClassID()), fHelper(helperArgs, GrAAType::kCoverage), fColor(color), fCoverage(coverage) {
    ...
    RequiresDstTexture finalize(const GrCaps& caps, const GrAppliedClip* clip) override {
        return fHelper.xpRequiresDstTexture(caps, clip, GrProcessorAnalysisCoverage::kSingleChannel, &fColor);
    }
...
bool GrAAHairLinePathRenderer::onDrawPath(const DrawPathArgs& args) {
    ...
    std::unique_ptr<GrDrawOp> op = AAHairlineOp::Make(
            std::move(args.fPaint), *args.fViewMatrix, path, args.fShape->style(), devClipBounds);
...
GR_DRAW_OP_TEST_DEFINE(AAHairlineOp) {
    ...
    return AAHairlineOp::Make(std::move(paint), viewMatrix, path, GrStyle::SimpleHairline(), devClipBounds);
}
