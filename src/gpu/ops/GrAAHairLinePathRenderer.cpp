...
static int gather_lines_and_quads(const SkPath& path,
                                  ...
                                  GrAAHairLinePathRenderer::PtArray* lines1,
                                  GrAAHairLinePathRenderer::PtArray* quads1,
                                  GrAAHairLinePathRenderer::PtArray* conics1,
    ...
    auto addChoppedQuad = [&](const SkPoint srcPts[3], const SkPoint devPts[4],
        ...
        if (SkIRect::Intersects(devClipBounds, ibounds)) {
            ...
            SkASSERT(subdiv >= -1);
            if (-1 == subdiv) {
                SkPoint* pts = lines1->push_back_n(4);
                pts[0] = devPts[0];
                pts[1] = devPts[1];
                pts[2] = devPts[1];
                pts[3] = devPts[2];
                ...
                SkPoint* pts = quads1->push_back_n(3);
                pts[0] = qPts[0];
                pts[1] = qPts[1];
                pts[2] = qPts[2];
        ...
        switch (verb) {
                    ...
                    for (int i = 0; i < conicCnt; ++i) {
                        ...
                        if (SkIRect::Intersects(devClipBounds, ibounds)) {
                            if (is_degen_quad_or_conic(devPts)) {
                                ...
                                SkPoint* pts = conics1->push_back_n(3);
                                pts[0] = cPts[0];
                                pts[1] = cPts[1];
                                pts[2] = cPts[2];
            ...
            case SkPath::kMove_Verb:
                ...
                if (seenZeroLengthVerb && verbsInContour == 1 && capLength > 0) {
                    SkPoint* pts = lines1->push_back_n(2);
...
private:
    ...
    Program characterize(const GrCaps*);
    ...
    Program        fCharacterization = kNo_Program;
    GrMesh*        fMeshes[3] = { nullptr };
...
void AAHairlineOp::characterize(const GrCaps* caps) {
    ...
    fCharacterization = neededPrograms;
}
...
void AAHairlineOp::onCreateProgramInfo(const GrCaps* caps,
    ...
    Program neededPrograms = this->characterize(caps);

    auto pipeline = fHelper.createPipelineWithStencil(*caps, arena, outputView, appliedClip,
                                                      dstProxyView, ??, ??);

//    flushState->executeDrawsAndUploadsForMeshDrawOp(this, chainBounds, pipeline);

    if (neededPrograms & kLine_Program) {
        this->makeLineProgramInfo(*caps, arena, );
    }
    if (neededPrograms & kQuad_Program) {
        this->makeQuadProgramInfo(*caps, arena, );

    }
    if (neededPrograms & kConic_Program) {
        this->makeConicProgramInfo(*caps, arena, );
...
void AAHairlineOp::onPrepareDraws(Target* target) {
    ...
    SkDEBUGCODE(Program predictedPrograms = this->characterize(&target->caps()));
    Program actualPrograms = kNo_Program;
    ...
    if (lineCount) {
        ...
        this->makeLineProgramInfo(target->caps(), target->allocator(),
                                  geometryProcessorViewM,
                                  geometryProcessorLocalM);
    ...
    if (quadCount || conicCount) {
        ...
        if (quadCount > 0) {
            ...
            this->makeQuadProgramInfo(target->caps(), target->allocator(),
                                      geometryProcessorViewM,
                                      geometryProcessorLocalM);

            fMeshes[1] = target->allocMesh();
            fMeshes[1]->setIndexedPatterned(quadsIndexBuffer, kIdxsPerQuad, kQuadNumVertices, quadCount,
        ...
        if (conicCount > 0) {
            ...
            this->makeConicProgramInfo(target->caps(), target->allocator(),
                                       geometryProcessorViewM,
                                       geometryProcessorLocalM);

            fMeshes[2] = target->allocMesh();
            fMeshes[2]->setIndexedPatterned(std::move(quadsIndexBuffer), kIdxsPerQuad, kQuadNumVertices,
