        ...
        private:
            void onEmitCode(EmitArgs& args, GrGPArgs* gpArgs) override {
                ...
                if (GrAA::kYes == textureGP.aa()) {
                    GrGLSLVarying aaDistVarying(kFloat4_GrSLType, GrGLSLVarying::Scope::kVertToFrag);
                    ...
                    args.fFragBuilder->codeAppendf("float mindist = min(min(%s.x, %s.y), min(%s.z, %s.w));",
                                                   aaDistVarying.fsIn(), aaDistVarying.fsIn(),
                                                   aaDistVarying.fsIn(), aaDistVarying.fsIn());
...
private:
    static void AssignTexCoords(Vertex* vertices, const GrQuad& quad, const SkRect& tex) {
        ...
        SkMatrixPriv::MapPointsWithStride(map, &vertices[0].fTextureCoords,
                 sizeof(Vertex), &vertices[0].fPosition, sizeof(Vertex), 4);
...
static void tessellate_quad(const GrQuad& devQuad, const SkRect& srcRect, GrColor color,
                     GrSurfaceOrigin origin, Vertex* vertices, SkScalar iw, SkScalar ih, int textureIdx) {
    SkRect texRect = {
        iw * srcRect.fLeft,
        ih * srcRect.fTop,
        iw * srcRect.fRight,
    ...
    void onPrepareDraws(Target* target) override {
        ...
        if (1 == fProxyCnt) {
            ...
            if (GrAA::kYes == fAA) {
                ...
                for (int i = 0; i < fDraws.count(); ++i) {
                    tessellate_quad<TextureGeometryProcessor::AAVertex, false, GrAA::kYes>(fDraws[i].fQuad, fDraws[i].fSrcRect, fDraws[i].fColor, origin, vertices + 4 * i, iw, ih, 0);
                }
            } else {
                ...
                for (int i = 0; i < fDraws.count(); ++i) {
                    tessellate_quad<TextureGeometryProcessor::Vertex, false, GrAA::kNo>(fDraws[i].fQuad, fDraws[i].fSrcRect, fDraws[i].fColor, origin, vertices + 4 * i, iw, ih, 0);
                }
            }
        } else {
            ...
            if (GrAA::kYes == fAA) {
                SkASSERT(gp->getVertexStride() == sizeof(TextureGeometryProcessor::AAMultiTextureVertex));
                ...
                for (int i = 0; i < fDraws.count(); ++i) {
                    ...
                    tessellate_quad<TextureGeometryProcessor::AAMultiTextureVertex, true, GrAA::kYes>(fDraws[i].fQuad, fDraws[i].fSrcRect, fDraws[i].fColor, origin, vertices + 4 * i, iw[tidx], ih[tidx], tidx);
                }
            } else {
                ...
                for (int i = 0; i < fDraws.count(); ++i) {
                    auto tidx = fDraws[i].fTextureIdx;
                    GrSurfaceOrigin origin = proxies[tidx]->origin();
                    tessellate_quad<TextureGeometryProcessor::MultiTextureVertex, true, GrAA::kNo>(fDraws[i].fQuad, fDraws[i].fSrcRect, fDraws[i].fColor, origin, vertices + 4 * i, iw[tidx], ih[tidx], tidx);
                }
            }
        }
