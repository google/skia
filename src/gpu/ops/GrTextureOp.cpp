...
public:
    ...
    void createBatch(const GrRenderTargetContext::TextureSetEntry set[],
                     int clumpSize,
                     GrAAType aaType) {
...
void GrTextureOp::CreateTextureSetOps(GrRenderTargetContext* rtc,
    ...
    if (aaType == GrAAType::kNone || aaType == GrAAType::kMSAA) {
        ...
        while (state.numLeft() > 0) {
            ...
            state.createBatch(set, clumpSize, aaType);
        }
    } else {
        ...
        while (state.numLeft() > 0) {
            ...
            for (int i = 0; i < state.numLeft(); ++i) {
                ...
                if (set[absIndex].fAAFlags != GrQuadAAFlags::kNone) {
                    ...
                    if (i >= GrResourceProvider::MaxNumAAQuads()) {
                        ...
                        state.createBatch(set, i, runningAA); // maybe downgrading AA here
                        clumped = true;
                        break;
                    }
                    ...
                    if (i >= GrResourceProvider::MaxNumNonAAQuads()) {
                        ...
                        state.createBatch(set, i, runningAA); // definitely downgrading AA here
                        clumped = true;
                        break;
                    }
                }
            ...
            if (!clumped) {
                ...
                state.createBatch(set, state.numLeft(), runningAA); // maybe downgrading AA here
            }
        }
    }
}
