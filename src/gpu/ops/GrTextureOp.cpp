...
__attribute__((no_sanitize("float-cast-overflow")))
    ...
    void onPrepareDraws(Target* target) override {
        ...
        if (1 == fProxyCnt) {
            ...
            if (coverageAA) {
                using Vertex = TextureGeometryProcessor::AAVertex<SkPoint>;
                SkASSERT(gp->getVertexStride() == sizeof(Vertex));
                auto vertices = static_cast<Vertex*>(vdata);
                for (int i = 0; i < fDraws.count(); ++i) {
                    tessellate_quad<Vertex, false, GrAA::kYes>(
                            fDraws[i].fQuad, fDraws[i].fSrcRect, fDraws[i].fColor, origin,
                            vertices + 4 * i, iw, ih, 0);
                }
            } else {
                using Vertex = TextureGeometryProcessor::Vertex<SkPoint>;
                SkASSERT(gp->getVertexStride() == sizeof(Vertex));
                auto vertices = static_cast<Vertex*>(vdata);
                for (int i = 0; i < fDraws.count(); ++i) {
                    tessellate_quad<Vertex, false, GrAA::kNo>(
                            fDraws[i].fQuad, fDraws[i].fSrcRect, fDraws[i].fColor, origin,
                            vertices + 4 * i, iw, ih, 0);
                }
            }
            ...
            if (coverageAA) {
                using Vertex = TextureGeometryProcessor::AAMultiTextureVertex<SkPoint>;
                SkASSERT(gp->getVertexStride() == sizeof(Vertex));
                auto vertices = static_cast<Vertex*>(vdata);
                for (int i = 0; i < fDraws.count(); ++i) {
                    auto tidx = fDraws[i].fTextureIdx;
                    GrSurfaceOrigin origin = proxies[tidx]->origin();
                    tessellate_quad<Vertex, true, GrAA::kYes>(
                            fDraws[i].fQuad, fDraws[i].fSrcRect,
                            fDraws[i].fColor, origin, vertices + 4 * i,
                            iw[tidx], ih[tidx], tidx);
                }
            } else {
                using Vertex = TextureGeometryProcessor::MultiTextureVertex<SkPoint>;
                SkASSERT(gp->getVertexStride() == sizeof(Vertex));
                auto vertices = static_cast<Vertex*>(vdata);
                for (int i = 0; i < fDraws.count(); ++i) {
                    auto tidx = fDraws[i].fTextureIdx;
                    GrSurfaceOrigin origin = proxies[tidx]->origin();
                    tessellate_quad<Vertex, true, GrAA::kNo>(
                            fDraws[i].fQuad, fDraws[i].fSrcRect, fDraws[i].fColor, origin,
