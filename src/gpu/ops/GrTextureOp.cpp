...
public:
    ...
    void getGLSLProcessorKey(const GrShaderCaps&, GrProcessorKeyBuilder* b) const override {
        ...
        x |= kFloat3_GrVertexAttribType == fPositions.gpuType() ? 0 : 2;
    ...
    GrGLSLPrimitiveProcessor* createGLSLInstance(const GrShaderCaps& caps) const override {
        ...
        private:
            void onEmitCode(EmitArgs& args, GrGPArgs* gpArgs) override {
                ...
                if (kFloat2_GrVertexAttribType == textureGP.fPositions.gpuType()) {
                ...
                if (textureGP.usesCoverageEdgeAA()) {
                    ...
                    if (kFloat3_GrVertexAttribType == textureGP.fPositions.gpuType()) {
...
private:
    TextureGeometryProcessor(GrTextureType textureType, GrPixelConfig textureConfig,
        ...
        if (perspective) {
            fPositions = {"position", kFloat3_GrVertexAttribType, kFloat3_GrVertexAttribType};
        } else {
            fPositions = {"position", kFloat2_GrVertexAttribType, kFloat2_GrVertexAttribType};
        }
        fColors = {"color", kUByte4_norm_GrVertexAttribType, kHalf4_GrVertexAttribType};
        fTextureCoords = {"textureCoords", kFloat2_GrVertexAttribType, kFloat2_GrVertexAttribType};
        int vertexAttributeCnt = 3;

        if (domain == Domain::kYes) {
            fDomain = {"domain", kFloat4_GrVertexAttribType, kFloat4_GrVertexAttribType};
            ++vertexAttributeCnt;
        }
        if (coverageAA) {
            fAAEdges[0] = {"aaEdge0", kFloat3_GrVertexAttribType, kFloat3_GrVertexAttribType};
            fAAEdges[1] = {"aaEdge1", kFloat3_GrVertexAttribType, kFloat3_GrVertexAttribType};
            fAAEdges[2] = {"aaEdge2", kFloat3_GrVertexAttribType, kFloat3_GrVertexAttribType};
            fAAEdges[3] = {"aaEdge3", kFloat3_GrVertexAttribType, kFloat3_GrVertexAttribType};
