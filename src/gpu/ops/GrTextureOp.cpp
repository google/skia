...
private:
    // This exists to reduce the number of shaders generated. It does some rounding of samplers
    // counts.
    ...
    TextureGeometryProcessor(sk_sp<GrTextureProxy> proxies[], int proxyCnt, int samplerCnt,
        ...
        if (samplerCnt > 1) {
            // In order to avoid generating a different shader for each n we readd the last sampler
            // up to the limit. We will have a single texture shader and a multi-texture shader.
...
private:
    ...
    bool onCombineIfPossible(GrOp* t, const GrCaps& caps) override {
        ...
        if (TextureGeometryProcessor::SupportsMultitexture(*caps.shaderCaps())) {
            ...
            auto filters = reinterpret_cast<GrSamplerState::Filter*>(fProxyArray + kMaxTextures);
            for (int i = 0; i < that->fProxyCnt; ++i) {
                if (map[i] < 0) {
                    thatProxies[i]->addPendingRead();
                    fProxyArray[-map[i]] = thatProxies[i];
                    filters[-map[i]] = thatFilters[i];
                    map[i] = -map[i];
                }
            }
    ...
    /**
     * Determines a mapping from that's fProxies array to this's fProxies array. A negative map
     * value means that's proxy should be added to this's fProxies array at the absolute value of
