...
public:
    ...
    // using Vertex = GrPerEdgeAA::OptionalAAVertex<Pos, D, AA>;
    ...
    static sk_sp<GrGeometryProcessor> Make(GrTextureType textureType, GrPixelConfig textureConfig,
                                           ...
                                           GrPerEdgeAA::Domain domain, const GrShaderCaps& caps) {
...
private:
    TextureGeometryProcessor(GrTextureType textureType, GrPixelConfig textureConfig,
                             ...
                             bool perspective, GrPerEdgeAA::Domain domain, const GrShaderCaps& caps)
...
private:
    ...
    template <typename Pos, GrPerEdgeAA::Domain D, GrAA AA>
    void tess(void* v, const GrGeometryProcessor* gp, const GrTextureProxy* proxy, int start,
              int cnt) const {
        TRACE_EVENT0("skia", TRACE_FUNC);
        using Vertex = GrPerEdgeAA::Vertex<Pos, SkPoint, D, AA>;
        ...
        for (int i = start; i < start + cnt; ++i) {
            ...
            GrPerEdgeAA::TesselateTexturedQuad(vertices, q.aaFlags(), q.domain(), q.quad(),
                    q.srcRect(), q.color(), origin, this->filter(), iw, ih);
            vertices += 4;
        }
    }
    ...
    void onPrepareDraws(Target* target) override {
        ...
        GrPerEdgeAA::Domain domain = GrPerEdgeAA::Domain::kNo;
        int numProxies = 0;
        int numTotalQuads = 0;
        ...
        for (const auto& op : ChainRange<TextureOp>(this)) {
            ...
            if (op.fDomain) {
                domain = GrPerEdgeAA::Domain::kYes;
            }
        ...
        using TessFn = decltype(&TextureOp::tess<SkPoint, GrPerEdgeAA::Domain::kNo, GrAA::kNo>);
#define TESS_FN_AND_VERTEX_SIZE(Point, Domain, AA)                          \
    {                                                                       \
        &TextureOp::tess<Point, Domain, AA>,                                \
                sizeof(GrPerEdgeAA::Vertex<Point, SkPoint, Domain, AA>) \
    }
        static constexpr struct {
            TessFn fTessFn;
            size_t fVertexSize;
        } kTessFnsAndVertexSizes[] = {
                TESS_FN_AND_VERTEX_SIZE(SkPoint,  GrPerEdgeAA::Domain::kNo,  GrAA::kNo),
                TESS_FN_AND_VERTEX_SIZE(SkPoint,  GrPerEdgeAA::Domain::kNo,  GrAA::kYes),
                TESS_FN_AND_VERTEX_SIZE(SkPoint,  GrPerEdgeAA::Domain::kYes, GrAA::kNo),
                TESS_FN_AND_VERTEX_SIZE(SkPoint,  GrPerEdgeAA::Domain::kYes, GrAA::kYes),
                TESS_FN_AND_VERTEX_SIZE(SkPoint3, GrPerEdgeAA::Domain::kNo,  GrAA::kNo),
                TESS_FN_AND_VERTEX_SIZE(SkPoint3, GrPerEdgeAA::Domain::kNo,  GrAA::kYes),
                TESS_FN_AND_VERTEX_SIZE(SkPoint3, GrPerEdgeAA::Domain::kYes, GrAA::kNo),
                TESS_FN_AND_VERTEX_SIZE(SkPoint3, GrPerEdgeAA::Domain::kYes, GrAA::kYes),
        };
#undef TESS_FN_AND_VERTEX_SIZE
        int tessFnIdx = 0;
        tessFnIdx |= coverageAA               ? 0x1 : 0x0;
        tessFnIdx |= (domain == GrPerEdgeAA::Domain::kYes) ? 0x2 : 0x0;
    ...
    public:
        ...
        GrPerEdgeAA::Domain domain() const { return GrPerEdgeAA::Domain(fHasDomain); }
