...
private:
    ...
    // allocated in the arena passed to 'fillIn'.
    ...
    void onPrepareDraws(Target* target) override {
        ...
        if (!fPrePreparedDesc) {
            ...
            fPrePreparedDesc = &desc;
        }

        size_t vertexSize = fPrePreparedDesc->fVertexSpec.vertexSize();

        GrMesh* meshes = target->allocMeshes(fPrePreparedDesc->fNumProxies);
        sk_sp<const GrBuffer> vbuffer;
        int vertexOffsetInBuffer = 0;
        int numQuadVerticesLeft = fPrePreparedDesc->fNumTotalQuads *
                                  fPrePreparedDesc->fVertexSpec.verticesPerQuad();
        ...
        for (const auto& op : ChainRange<TextureOp>(this)) {
            ...
            for (unsigned p = 0; p < op.fProxyCnt; ++p) {
                ...
                if (fPrePreparedDesc->fDynamicStateArrays) {
                    fPrePreparedDesc->fDynamicStateArrays->fPrimitiveProcessorTextures[meshIndex] =
                        proxy;
                }
                ++meshIndex;
...

        {
            ...
            gp = GrQuadPerEdgeAA::MakeTexturedProcessor(
                fPrePreparedDesc->fVertexSpec, *target->caps().shaderCaps(), backendFormat,
        ...
        target->recordDraw(std::move(gp), meshes,
                           fPrePreparedDesc->fNumProxies,
                           fPrePreparedDesc->fFixedDynamicState,
                           fPrePreparedDesc->fDynamicStateArrays);
    }
