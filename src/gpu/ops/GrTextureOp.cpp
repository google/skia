...
private:
    ...
    struct PrePreparedDesc {
        ...
        void setMeshProxy(int index, GrTextureProxy* proxy) {
            ...
            SkASSERT(fDynamicStateArrays && fDynamicStateArrays->fPrimitiveProcessorTextures);
    ...
    void onPrePrepareDraws(GrRecordingContext* context, const GrAppliedClip* clip) override {
        ...
        // onPrepareDraws doesn't need these fields allocated so do it here instead of in
        // 'allocate'
        fPrePreparedDesc->fVertexOffsets = arena->makeArrayDefault<int>(
                                                                    fPrePreparedDesc->fNumProxies);
        fPrePreparedDesc->fVertices = arena->makeArrayDefault<char>(
                                                            fPrePreparedDesc->totalSizeInBytes());
