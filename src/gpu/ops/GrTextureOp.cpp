...
static void compute_quad_edges_and_outset_vertices(GrQuadAAFlags aaFlags, Sk4f* x, Sk4f* y, Sk4f* a, Sk4f* b, Sk4f* c) {
...
public:
    static void AssignPositionsAndTexCoords(V* vertices, const GrPerspQuad& quad, GrQuadAAFlags aaFlags,
                                            const SkRect& texRect) {
        // Should be kNone for non-AA and kAll for MSAA.
        SkASSERT(aaFlags == GrQuadAAFlags::kNone || aaFlags == GrQuadAAFlags::kAll);
        SkPointPriv::SetRectTriStrip(&vertices[0].fTextureCoords, texRect, sizeof(V));
...
static void tessellate_quad(const GrPerspQuad& devQuad, GrQuadAAFlags aaFlags,
                            const SkRect& srcRect, GrColor color,
...
public:
    static std::unique_ptr<GrDrawOp> Make(GrContext* context,
        ...
        return pool->allocate<TextureOp>(std::move(proxy), filter, color,
                                         srcRect, dstRect, aaType, aaFlags, constraint,
...
private:
    ...
    void tess(void* v, const GrGeometryProcessor* gp) const {
        ...
        for (const auto& draw : fDraws) {
            tessellate_quad<Vertex>(draw.quad(), draw.aaFlags(), draw.srcRect(), draw.color(), origin, fFilter,
...
GR_DRAW_OP_TEST_DEFINE(TextureOp) {
    ...
    return GrTextureOp::Make(context, std::move(proxy), filter, color, srcRect, rect, aaType, aaFlags,
