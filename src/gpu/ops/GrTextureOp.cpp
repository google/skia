...
// Count the number of proxy switches in the entry set. This usually is already computed by
// SkGpuDevice, but when the BatchLengthLimiter chops the set up it must determine a new proxy count
// for each split.
static int proxy_switch_count(const GrRenderTargetContext::TextureSetEntry set[], int count) {
    int actualProxyCount = 0;
...
public:
    ...
    static std::unique_ptr<GrDrawOp> Make(GrRecordingContext* context,
        ...
        // Allocate size based on proxyCnt, since that determines number of ViewCountPairs.
        SkASSERT(proxyCnt <= cnt);

        size_t size = sizeof(TextureOp) + sizeof(ViewCountPair) * (proxyCnt - 1);
        GrOpMemoryPool* pool = context->priv().opMemoryPool();
        void* mem = pool->allocate(size);
        return std::unique_ptr<GrDrawOp>(new (mem) TextureOp(set, cnt, proxyCnt, filter, saturate,
                                                             aaType, constraint, viewMatrix,
...
private:
    ...
    TextureOp(GrRenderTargetContext::TextureSetEntry set[],
        ...
        fMetadata.fProxyCount = SkToUInt(proxyCnt);
...
public:
    ...
    void createOp(GrRenderTargetContext::TextureSetEntry set[],
                  int clumpSize,
                  GrAAType aaType) {
        int clumpProxyCount = proxy_switch_count(&set[fNumClumped], clumpSize);
...
void GrTextureOp::AddTextureSetOps(GrRenderTargetContext* rtc,
    ...
    SkASSERT(proxy_switch_count(set, cnt) == proxyCnt);
    ...
    if (cnt <= SkTMin(GrResourceProvider::MaxNumNonAAQuads(),
        ...
        auto op = TextureOp::Make(context, set, cnt, proxyCnt, filter, saturate, aaType,
