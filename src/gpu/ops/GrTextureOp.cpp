    ...
    TextureOp(const GrRenderTargetContext::TextureSetEntry set[], int cnt,
        ...
        for (unsigned p = 0; p < fProxyCnt; ++p) {
            ...
            if (overallAAType == GrAAType::kNone && aaForQuad != GrAAType::kNone) {
                // Resolve sets aaForQuad to aaType or None, there is never a change from msaa to
                // coverage. So if a specific quad needs AA, we can safely set overallAAType to
