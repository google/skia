...
/**
 * Op that implements GrTextureOp::Make. It draws a textured quads. Each quad can modulate against
 * a constant color. The blend with the destination is always src-over.
 */
...
GR_DRAW_OP_TEST_DEFINE(TextureOp) {
    ...
    auto cs1 = GrTest::TestColorSpace(random);
    auto cs2 = GrTest::TestColorSpace(random);
    return GrTextureOp::Make(std::move(proxy), filter, color, srcRect, rect, viewMatrix,
                             GrColorSpaceXform::Make(cs1.get(), cs2.get()));
}
