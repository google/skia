...
static std::tuple<bool /* filter */, bool /* mipmap */>
filter_and_mm_have_effect(const GrQuad& srcQuad, const GrQuad& dstQuad) {
    ...
    if (srcQuad.asRect(&srcRect) && dstQuad.asRect(&dstRect)) {
        ...
        bool filter = !mm || SkScalarFraction(srcRect.fLeft) != SkScalarFraction(dstRect.fLeft) ||
    ...
    bool filter = !mm && SkScalarIsInt(srcQuad.x(0)) && SkScalarIsInt(srcQuad.y(0)) &&
...
private:
    ...
    struct Metadata {
        ...
        Metadata(const GrSwizzle& swizzle, GrSamplerState::Filter filter, GrSamplerState::MipmapMode mm,
    ...
    TextureOp(GrRenderTargetContext::TextureSetEntry set[],
            ...
            , fTextureColorSpaceXform(std::move(textureColorSpaceXform))
            , fDesc(nullptr)
            , fMetadata(set[0].fProxyView.swizzle(), GrSamplerState::Filter::kNearest, GrSamplerState::MipmapMode::kNone,
                        Subset::kNo, saturate) {
...
public:
    ...
    void createOp(GrRenderTargetContext::TextureSetEntry set[],
        ...
        std::unique_ptr<GrDrawOp> op = TextureOp::Make(fContext, &set[fNumClumped], clumpSize,
                                                       clumpProxyCount, fFilter, fMipmapMode,
                                                       fSaturate, aaType,
    ...
    if (blendMode != SkBlendMode::kSrcOver ||
        ...
        for (int i = 0; i < cnt; ++i) {
            ...
            auto op = Make(context, set[i].fProxyView, set[i].fSrcAlphaType, textureColorSpaceXform,
                           filter, mm, set[i].fColor, saturate, blendMode, aaType,
                           &quad, subset);
