...
public:
    ...
    static std::unique_ptr<GrDrawOp> Make(GrContext* context, const GrRenderTargetContext::TextureSetEntry set[], int cnt, GrSamplerState::Filter filter, GrColor color, GrAAType aaType,
        ...
        return std::unique_ptr<GrDrawOp>(new (mem) TextureOp(set, cnt, filter, color, aaType,
                            viewMatrix, std::move(textureColorSpaceXform),
                            std::move(paintColorSpaceXform)));
    }

    ~TextureOp() override {
        for (const auto& p : fProxies) {
            if (fFinalized) {
                p.fProxy->completedRead();
            } else {
                p.fProxy->unref();
            }
        }
...
private:
    ...
    TextureOp(const GrRenderTargetContext::TextureSetEntry set[], int cnt, GrSamplerState::Filter filter, GrColor color, GrAAType aaType,
              const SkMatrix& viewMatrix, sk_sp<GrColorSpaceXform> textureColorSpaceXform,
              sk_sp<GrColorSpaceXform> paintColorSpaceXform)
            : INHERITED(ClassID())
            , fTextureColorSpaceXform(std::move(textureColorSpaceXform))
        ...
        for (int i = 0; i < cnt; ++i) {
            // TODO: Move this ref
            fProxies[i].fProxy = SkRef(set[i].fProxy.get());
            fProxies[i].fQuadCnt = 1;
            SkASSERT(fProxies[i].fProxy->textureType() == fProxies[0].fProxy->textureType());
            SkASSERT(fProxies[i].fProxy->config() == fProxies[0].fProxy->config());
            auto quad = GrPerspQuad(set[i].fDstRect, viewMatrix);
            bounds.joinPossiblyEmptyRect(quad.bounds());
            GrQuadAAFlags aaFlags = set[i].fAAFlags;
            switch (aaType) {
                case GrAAType::kNone:
                    aaFlags = GrQuadAAFlags::kNone;
                    break;
            ...
            aa = aa || (set[i].fAAFlags != GrQuadAAFlags::kNone);
            fQuads.emplace_back(set->fSrcRect, quad, aaFlags, SkCanvas::kFast_SrcRectConstraint, color);
        }
        if (!aa) {
            fAAType = static_cast<unsigned>(GrAAType::kNone);
        }
    ...
    template <typename Pos, Domain D, GrAA AA>
    void tess(void* v, const GrGeometryProcessor* gp, const GrTextureProxy* proxy, int start, int cnt) const {
        ...
        for (int i = start; i < start + cnt; ++i) {
            ...
            tessellate_quad<Vertex>(q.quad(), q.aaFlags(), q.srcRect(), q.color(),
                                    origin, fFilter, vertices, iw, ih, q.domain());
            vertices += 4;
        }
    }
...
#undef TESS_FN_AND_VERTEX_SIZE
        ...
        for (const auto& op : ChainRange<TextureOp>(this)) {
            ...
            for (int p = 0; p < op.fProxyCnt; ++p) {
                ...
                (op.*(kTessFnsAndVertexSizes[tessFnIdx].fTessFn))(vdata, gp.get(), proxy, q, quadCnt);
    ...
    CombineResult onCombineIfPossible(GrOp* t, const GrCaps& caps) override {
        ...
        if (fProxyCnt > 1 || that->fProxyCnt > 1 || thisProxy->uniqueID() != thatProxy->uniqueID() || that->isChained()) {
...
std::unique_ptr<GrDrawOp> Make(GrContext* context,
                               ...
                               sk_sp<GrColorSpaceXform> textureColorSpaceXform,
                               sk_sp<GrColorSpaceXform> paintColorSpaceXform) {
    return TextureOp::Make(context, set, cnt, filter, color,
            aaType, viewMatrix, std::move(textureColorSpaceXform),
                           std::move(paintColorSpaceXform));
}
