...
public:
    ...
    void createClump(const GrRenderTargetContext::TextureSetEntry set[],
                     int clumpSize,
                     GrAAType aaType) {
        std::unique_ptr<GrDrawOp> tmp = TextureOp::Make(fContext, &set[fNumClumped], clumpSize,
...
std::unique_ptr<GrDrawOp> MakeSet(GrRecordingContext* context,
                                  ...
                                  const SkMatrix& viewMatrix,
                                  sk_sp<GrColorSpaceXform> textureColorSpaceXform) {
    ClumpingState state(context, cnt, filter, saturate, constraint, viewMatrix,
                        std::move(textureColorSpaceXform));
    ...
    if (aaType == GrAAType::kNone || aaType == GrAAType::kMSAA) {
        ...
        while (state.numLeft() > 0) {
            ...
            state.createClump(set, clumpSize, aaType);
        }
    } else {
        ...
        while (state.numLeft() > 0) {
            ...
            for (int i = 0; i < state.numLeft(); ++i) {
                ...
                if (set[absIndex].fAAFlags != GrQuadAAFlags::kNone) {
                    ...
                    if (i >= GrResourceProvider::MaxNumAAQuads()) {
                        ...
                        state.createClump(set, i, runningAA); // maybe downgrading AA here
                        clumped = true;
                        break;
                    }
                    ...
                    if (i >= GrResourceProvider::MaxNumNonAAQuads()) {
                        ...
                        state.createClump(set, i, runningAA); // definitely downgrading AA here
                        clumped = true;
                        break;
                    }
                }
            ...
            if (!clumped) {
                ...
                state.createClump(set, state.numLeft(), runningAA); // maybe downgrading AA here
            }
        }
    }
