...
private:
    ...
    void characterize(PrePreparedDesc* desc) const {
        ...
        SkASSERT(desc->fNumTotalQuads == this->numChainedQuads(true));
    ...
    int totNumQuads() const {
        ...
        int tmp = 0;
        for (unsigned p = 0; p < fProxyCnt; ++p) {
            tmp += fProxyCountPairs[p].fQuadCnt1;
        }
    ...
    int numChainedQuads(bool expectedToBeTheHead) const {

        if (expectedToBeTheHead) {
            SkASSERT(!this->prevInChain());
        }
        ...
        if (fProxyCnt > 1 || that->fProxyCnt > 1 ||
            ...
            if (GrTextureProxy::ProxiesAreCompatibleAsDynamicState(thisProxy, thatProxy) &&
                ...
                if (this->aaType() == GrAAType::kCoverage || upgradeToCoverageAAOnMerge) {
                    if (this->numChainedQuads(false) + that->numChainedQuads(true) > GrResourceProvider::MaxNumAAQuads()) {
                        return CombineResult::kCannotCombine;
                    }
                } else {
                    if (this->numChainedQuads(false) + that->numChainedQuads(true) > GrResourceProvider::MaxNumNonAAQuads()) {
                        return CombineResult::kCannotCombine;
                    }
                }
