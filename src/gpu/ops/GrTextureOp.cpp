...
static bool safe_to_ignore_subset_rect(GrAAType aaType, GrSamplerState::Filter filter,
                                       const DrawQuad& quad,
                                       const SkRect& subsetRect, const SkRect& localBounds) {
...
private:
    ...
    TextureOp(GrSurfaceProxyView proxyView,
        ...
        if (subsetRect) {
            if (safe_to_ignore_subset_rect(aaType, filter, *quad,
                                           *subsetRect, quad->fLocal.bounds())) {
                subsetRect = nullptr;
        ...
        for (int q = 0; q < cnt; ++q) {
            ...
            if (constraint == SkCanvas::kStrict_SrcRectConstraint) {
                ...
                if (!subsetRect->contains(curProxy->backingStoreBoundsRect())) {
                    if (!safe_to_ignore_subset_rect(aaForQuad, filter, quad,
                                                    *subsetRect, quad.fLocal.bounds())) {
