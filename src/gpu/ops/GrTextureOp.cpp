...
// output. If outsetCorners is set then the 'u' and 'v' should hold the texture coordinates on input
    ...
    *c += 0.5f;
    ...
    if (aaFlags != GrQuadAAFlags::kAll) {
        ...
        *c += (1.f - mask);
        if (outsetCorners) {
            ...
            *y += maskCW * -ydiff + mask * nextCW(ydiff);
            // 't' would be 0.5 / length but mask and maskCW already have a 0.5f scale.
            const Sk4f& t = invLengths;
            Sk4f udiff = nextCCW(*u) - *u;
            Sk4f vdiff = nextCCW(*v) - *v;
            *u += t * (maskCW * -udiff + mask * nextCW(udiff));
            *v += t * (maskCW * -vdiff + mask * nextCW(vdiff));
        }
        ...
        *y += 0.5f * (-ydiff + nextCW(ydiff));
        ...
        Sk4f udiff = nextCCW(*u) - *u;
        Sk4f vdiff = nextCCW(*v) - *v;
        *u += t * (-udiff + nextCW(udiff));
        *v += t * (-vdiff + nextCW(vdiff));
    }
}
