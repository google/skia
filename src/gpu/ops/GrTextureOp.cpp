...
static SkRect compute_domain(GrPerEdgeAA::Domain domain, GrSamplerState::Filter filter,
                             GrSurfaceOrigin origin, const SkRect& srcRect, float iw, float ih) {
    static constexpr SkRect kLargeRect = {-2, -2, -2, -2};
    if (domain == GrPerEdgeAA::Domain::kNo) {
...
private:
    ...
    template <int PosDim, GrPerEdgeAA::Domain D, GrAA AA>
    void tess(void* v, const GrGeometryProcessor* gp, const GrTextureProxy* proxy, int start,
              int cnt) const {
        TRACE_EVENT0("skia", TRACE_FUNC);
        using Vertex = GrPerEdgeAA::Vertex<PosDim, GrColor, 2, D, AA>;
        ...
        for (int i = start; i < start + cnt; ++i) {
            ...
            GrPerEdgeAA::Tessellator::Apply<Vertex>(
    ...
    void onPrepareDraws(Target* target) override {
        ...
        using TessFn = decltype(&TextureOp::tess<2, GrPerEdgeAA::Domain::kNo, GrAA::kNo>);
#define TESS_FN_AND_VERTEX_SIZE(Point, Domain, AA)                          \
    {                                                                       \
        &TextureOp::tess<Point, Domain, AA>,                                \
                sizeof(GrPerEdgeAA::Vertex<Point, GrColor, 2, Domain, AA>) \
    }
        static constexpr struct {
            TessFn fTessFn;
            size_t fVertexSize;
        } kTessFnsAndVertexSizes[] = {
                TESS_FN_AND_VERTEX_SIZE(2, GrPerEdgeAA::Domain::kNo,  GrAA::kNo),
                TESS_FN_AND_VERTEX_SIZE(2, GrPerEdgeAA::Domain::kNo,  GrAA::kYes),
                TESS_FN_AND_VERTEX_SIZE(2, GrPerEdgeAA::Domain::kYes, GrAA::kNo),
                TESS_FN_AND_VERTEX_SIZE(2, GrPerEdgeAA::Domain::kYes, GrAA::kYes),
                TESS_FN_AND_VERTEX_SIZE(3, GrPerEdgeAA::Domain::kNo,  GrAA::kNo),
                TESS_FN_AND_VERTEX_SIZE(3, GrPerEdgeAA::Domain::kNo,  GrAA::kYes),
                TESS_FN_AND_VERTEX_SIZE(3, GrPerEdgeAA::Domain::kYes, GrAA::kNo),
                TESS_FN_AND_VERTEX_SIZE(3, GrPerEdgeAA::Domain::kYes, GrAA::kYes),
        };
