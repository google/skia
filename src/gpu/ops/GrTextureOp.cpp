...
private:
    ...
    TextureOp(GrSurfaceProxyView proxyView,
        ...
        int quadCount = this->appendQuad(std::move(quad), color, domain);
    ...
    TextureOp(GrRenderTargetContext::TextureSetEntry set[],
        ...
        for (int q = 0; q < cnt; ++q) {
            ...
            fViewCountPairs[p].fQuadCnt += this->appendQuad(std::move(quad),
                                                            {alpha, alpha, alpha, alpha}, domain);
        }
...
std::unique_ptr<GrDrawOp> GrTextureOp::Make(GrRecordingContext* context,
    ...
    if (blendMode == SkBlendMode::kSrcOver) {
        ...
        return GrFillRectOp::Make(context, std::move(paint), aaType, std::move(quad));
    }
}
...
GR_DRAW_OP_TEST_DEFINE(TextureOp) {
    ...
    return GrTextureOp::Make(context, std::move(proxyView), alphaType, std::move(texXform), filter,
                             ...
                             {GrQuad::MakeFromRect(rect, viewMatrix), GrQuad(srcRect), aaFlags},
