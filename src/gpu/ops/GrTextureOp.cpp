...
private:
    ...
    TextureOp(sk_sp<GrTextureProxy> proxy, GrSamplerState::Filter filter, const SkPMColor4f& color,
        ...
        int draw = fQuads.push_back(quad, quadType);
        fQuads.metadata(draw) = {color, srcRect, constraint, aaFlags};
        fProxyCnt = 1;
        ...
        fDomain = static_cast<unsigned>(fQuads.metadata(draw).domain());
    ...
    TextureOp(const GrRenderTargetContext::TextureSetEntry set[], int cnt,
        ...
        for (unsigned p = 0; p < fProxyCnt; ++p) {
            ...
            int i = fQuads.push_back(quad, quadType);
            fQuads.metadata(i) =
                    {color, set[p].fSrcRect, SkCanvas::kFast_SrcRectConstraint, aaFlags};
        }
    ...
    struct ColorDomainAndAA {
        ColorDomainAndAA(const SkPMColor4f& color, const SkRect& srcRect,
                         SkCanvas::SrcRectConstraint constraint, GrQuadAAFlags aaFlags)
                : fColor(color)
                , fSrcRect(srcRect)
