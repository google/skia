...
std::unique_ptr<GrAAFillRRectOp> GrAAFillRRectOp::Make(
    ...
    return pool->allocate<GrAAFillRRectOp>(*caps.shaderCaps(), m, rrect, std::move(paint));
}
...
constexpr GrPrimitiveProcessor::Attribute GrAAFillRRectOp::RoundRectProc::kVertexAttrib;
...
public:
    void onEmitCode(EmitArgs& args, GrGPArgs* gpArgs) override {
        ...
        // Setup the (x,y) values for our corner/edge's implicit arc function (x^2 + y^2 == 1).
        // If not using HW derivatives, we also set up the derivatives for x and y.
...
void GrAAFillRRectOp::onExecute(GrOpFlushState* flushState) {
    if (!fInstanceBuffer) {
        return;  // Setup failed.
    }
