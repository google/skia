...
struct Vertex {
    int8_t fCornerID;  // Which corner should this vertex offset from?
    int8_t fVertexID;  // Vertex ID within the section. (Edge sections have 3 verts, corners, 4.)
    int8_t fEdgeID;  // Edge sections only: Which edge should we use for AA?
    int8_t fReserved;
};

static constexpr Vertex kVertexData[] = {
        // corner-id    vertex-id    edge-id    reserved
        {          0,           0,         0,          0},  // left edge
        {          2,           2,         0,          0},
        {          0,           2,         0,          0},
        ...
        {          2,           0,        -1,          0},  // bottom-left corner
        {          2,           1,        -1,          0},
        {          2,           2,        -1,          0},
        {          2,           3,        -1,          0},

        {          3,           0,        -1,          0},  // bottom-right corner
        {          3,           2,        -1,          0},
        {          3,           1,        -1,          0},
        {          3,           3,        -1,          0},
};
...
static constexpr uint16_t kIndexData[] = {
    ...
    6,   7,  8,  // right edge
    9,  10, 11,  // bottom edge
    12, 13, 14, 13, 15, 14,  // top-left corner
    16, 17, 18, 17, 19, 18,  // top-right corner
    20, 21, 22, 21, 23, 22,  // bottom-left corner
    24, 25, 26, 25, 27, 26,  // bottom-right corner
};
...
private:
    static constexpr Attribute kVertexAttrib =
            {"corner_vertex_edge_ids", kByte4_GrVertexAttribType, kShort4_GrSLType};
...
public:
    void onEmitCode(EmitArgs& args, GrGPArgs* gpArgs) override {
        ...
        v->codeAppend("short corner_id = corner_vertex_edge_ids[0];");
        ...
        // SkRRect stores radii in "fan" order, whereas corner_id is in "strip" order.
        // The "xywz" swizzle fixes this disparity.
        v->codeAppend("float2 radii = float2(radii_x.xywz[corner_id], radii_y.xywz[corner_id]);");
        ...
        v->codeAppend("short vertex_id = corner_vertex_edge_ids[1];");
        ...
        GrGLSLVarying derivatives(kFloat2x2_GrSLType);
        if (!useHWDerivatives) {
            varyings->addVarying("derivatives", &derivatives,
                                 GrGLSLVaryingHandler::Interpolation::kCanBeFlat);
        }

        v->codeAppend("short edge_axis_id = corner_vertex_edge_ids[2];");
        if (!useHWDerivatives) {
            v->codeAppendf("float2x2 derivatives = inverse(float2x2(skew));");
        }
        v->codeAppend("if (edge_axis_id >= 0) {");
                           // We are an edge segment. Emit a degenerate arc equation that is
                           // actually just a flat line at our corresponding edge. (x^2 + 0 == 1.)
        v->codeAppendf(    "short i = (edge_axis_id & 1);");
        v->codeAppendf(    "%s.xy = (float2(vertexpos[i] * corner[i] * .5 + .5, 0));",
                           arcCoord.vsOut());
        if (!useHWDerivatives) {
            v->codeAppendf("%s = float2x2(derivatives[i], 0, 0);", derivatives.vsOut());
        }
        v->codeAppend("} else {");
                           // We are a corner segment. Emit our corner's arc equation.
        v->codeAppendf(    "%s.xy = (corner + radii - vertexpos) / radii;", arcCoord.vsOut());
        if (!useHWDerivatives) {
            v->codeAppendf("%s = derivatives * float2x2(-2/radii.x, 0, 0, -2/radii.y);",
                           derivatives.vsOut());
        }
        v->codeAppend("}");

        // Transform to device coords.
        v->codeAppend("float2 devcoord = vertexpos * float2x2(skew) + translate;");
        ...
        if (useHWDerivatives) {
            ...
            // derivatives directly. (And we can't just interpolate "grad" because "coord" became
            // nonlinear when we clamped it above zero.)
            f->codeAppendf("float2 grad = %s * coord;", derivatives.fsIn());
