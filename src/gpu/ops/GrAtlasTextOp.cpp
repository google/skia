...
void GrAtlasTextOp::onPrepareDraws(Target* target) {
    ...
    // Where the quad buffer start and end relative to totalGlyphsDrawn.
    ...
    void* vertices = target->makeVertexSpace(
            vertexStride,
            (quadBufferEnd - quadBufferBegin) * quadSize,
    ...
    // totalGlyphsDrawn is all the glyphs for the op [0, this->numGlyphs()). The subRun glyph and
    ...
    for (int i = 0; i < fGeoCount; i++) {
        ...
        // Where the subRun begins and ends relative to totalGlyphsDrawn.
        ...
        while (totalGlyphsDrawn < subRunEnd) {
            // drawStart and drawEnd are indices for the subRun on the
            // interval [0, subRun->fGlyphs.size()). drawEnd is either the end of the subRun or
            // the end of the current quad buffer.
            int drawEnd = std::min(subRunEnd, quadBufferEnd) - subRunBegin;
            int drawStart = totalGlyphsDrawn - subRunBegin;
            auto[ok, glyphsRegenerated] = regenerator.regenerate(drawStart, drawEnd);
            ...
            if (glyphsRegenerated > 0) {
                int quadBufferIndex = totalGlyphsDrawn - quadBufferBegin;
                int subRunIndex = totalGlyphsDrawn - subRunBegin;
                auto vertexBuffer =
                ...
                if (args.fClipRect.isEmpty()) {
                    ...
                    clip_quads(args.fClipRect, vertexBuffer, subRun->quadStart(subRunIndex),
            ...
            // Either the atlas is full (totalGlyphsDrawn != subRunEnd) or the quad buffer is
            // full (totalGlyphsDrawn == quadBufferEnd) do what is needed to make space in the
            // atlas or add more quad buffer space.
            if (totalGlyphsDrawn != subRunEnd || totalGlyphsDrawn == quadBufferEnd) {
                // Flush if not all glyphs drawn because either the quad buffer is full or the
                // atlas is out of space.
                this->flush(target, &flushInfo);
                if (totalGlyphsDrawn == quadBufferEnd) {
                    // Quad buffer is full. Get more buffer.
                    quadBufferBegin = totalGlyphsDrawn;
                    quadBufferEnd = totalGlyphsDrawn +
