...
static void clip_quads(const SkIRect& clipRect, char* currVertex, const char* blobVertices,
    ...
    for (int i = 0; i < glyphCount; ++i) {
        ...
        if (clipRect.contains(positionRect)) {
            ...
            int16_t coordsRectL = SkTAbs(blobCoordsLT[0]);
            int16_t coordsRectT = SkTAbs(blobCoordsLT[1]);
            int16_t coordsRectR = SkTAbs(blobCoordsRB[0]);
            int16_t coordsRectB = SkTAbs(blobCoordsRB[1]);
            bool pageIndexX = (blobCoordsLT[0] < 0);
            bool pageIndexY = (blobCoordsLT[1] < 0);
            ...
            if (pageIndexX) {
                coordsRectL = -coordsRectL;
                coordsRectR = -coordsRectR;
            }
            if (pageIndexY) {
                coordsRectT = -coordsRectT;
                coordsRectB = -coordsRectB;
            }
