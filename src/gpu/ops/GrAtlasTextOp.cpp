...
std::unique_ptr<GrTextBlob::SDFT3DVertex[][4]> GrAtlasTextOp::Geometry::vertexData(
        int offset, int count) const {
    std::unique_ptr<GrTextBlob::SDFT3DVertex[][4]> data{new GrTextBlob::SDFT3DVertex[count][4]};

    fSubRunPtr->textTargetVertexData(data.get(), offset, count, fColor.toBytes_RGBA(), fDrawOrigin);
