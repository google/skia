...
std::unique_ptr<GrDrawOp> GrAtlasTextOp::CreateOpTestingOnly(GrRenderTargetContext* rtc,
                                                             ...
                                                             int x,
                                                             int y) {
    auto direct = rtc->fContext->priv().asDirectContext();
    if (!direct) {
        return nullptr;
    }
    ...
    GrTextContext::Options SDFOptions = {
            rtc->fContext->priv().options().fGlyphsAsPathsFontSize,
            rtc->fContext->priv().options().fMinDistanceFieldFontSize
    };
    ...
    if (!glyphRunList.empty()) {
        ...
        painter->processGlyphRunList(
                ...
                rtc->fContext->priv().caps()->shaderCaps()->supportsDistanceFieldText(),
                SDFOptions, blob.get());
    }
