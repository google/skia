...
void GrAtlasTextOp::init() {
        ...
        if (this->usesDistanceFields()) {
            ...
            fDFGPFlags |= (kAliasedDistanceField_MaskType == fMaskType) ? kAliased_DistanceFieldEffectFlag : 0;
...
static void clip_quads(const SkIRect& clipRect, char* currVertex, const char* blobVertices,
    ...
    for (int i = 0; i < glyphCount; ++i) {
        ...
        if (clipRect.contains(positionRect)) {
            ...
            // If the vertices have 2 coords rather than 3 (DF w/ perspective) then we will write
            ...
            SkPoint3* currPosition = reinterpret_cast<SkPoint3*>(currVertex);
            ...
            *(reinterpret_cast<SkColor*>(currVertex + sizeof(SkPoint))) = color;
            ...
            currPosition = reinterpret_cast<SkPoint3*>(currVertex);
            currPosition->fX = positionRect.fLeft;
        ...
        do {
            ...
            if (this->usesDistanceFields() && !args.fViewMatrix.isIdentity()) {
                ...
                if (args.fViewMatrix.hasPerspective()) {
                    for (int i = 0; i < result.fGlyphsRegenerated * kVerticesPerGlyph; ++i) {
                        auto* pos3 = reinterpret_cast<SkPoint3*>(currVertex + i * vertexStride);
                        SkASSERT(pos3->fZ == 1.f);
                        args.fViewMatrix.mapHomogeneousPoints(pos3, pos3, 1);
                    }
                } else {
...
sk_sp<GrGeometryProcessor> GrAtlasTextOp::setupDfProcessor() const {
    ...
    if (this->usesLocalCoords()) {
        ...
        (void)fGeoData[0].fViewMatrix.invert(&localMatrix);
    }
