...
void GrAtlasTextOp::onPrepareDraws(Target* target) {
    ...
    int vertexBufferFreeGlyphCount = std::min(totalGlyphCount, maxGlyphsInBuffer);
    void* vertices =
            target->makeVertexSpace(vertexStride, vertexBufferFreeGlyphCount * kVerticesPerGlyph,
    ...
    for (int i = 0; i < fGeoCount; i++) {
        ...
        while (!done) {
            ...
            do {
                if (!vertexBufferFreeGlyphCount) {
                    this->flush(target, &flushInfo);
                    vertexBufferFreeGlyphCount = std::min(totalGlyphCount, maxGlyphsInBuffer);
                    vertices = target->makeVertexSpace(
                            vertexStride, vertexBufferFreeGlyphCount * kVerticesPerGlyph,
                            &flushInfo.fVertexBuffer, &flushInfo.fVertexOffset);
                    currVertex = reinterpret_cast<char*>(vertices);
                }
                if (!regenerator.regenerate(&result, vertexBufferFreeGlyphCount)) {
                    result.fFinished = true;
                    break;
                }
                done = result.fFinished;
                int glyphCount = std::min(result.fGlyphsRegenerated, vertexBufferFreeGlyphCount);
                ...
                vertexBufferFreeGlyphCount -= glyphCount;
    ...
    SkASSERT(!vertexBufferFreeGlyphCount);
