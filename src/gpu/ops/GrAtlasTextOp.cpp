...
static void clip_quads(const SkIRect& clipRect, char* currVertex, const char* blobVertices,
    ...
    for (int i = 0; i < glyphCount; ++i) {
        ...
        if (clipRect.contains(positionRect)) {
            ...
            const  // and it avoids a lot of conditionals.
                    SkColor color =
                            *reinterpret_cast<const SkColor*>(blobVertices + sizeof(SkPoint));
            size_t coordOffset = vertexStride - 2*sizeof(uint16_t);
            const uint16_t* blobCoordsLT =
                    reinterpret_cast<const uint16_t*>(blobVertices + coordOffset);
            const uint16_t* blobCoordsRB = reinterpret_cast<const uint16_t*>(
                    blobVertices + 3 * vertexStride + coordOffset);
...
void GrAtlasTextOp::onPrepareDraws(Target* target) {
    ...
    for (int i = 0; i < fGeoCount; i++) {
        ...
        do {
            result = regenerator.rengenerate();
