...
static void clip_quads(const SkIRect& clipRect,
    ...
    for (int i = 0; i < glyphCount; ++i) {
        ...
        SkPoint* blobPositionRB = reinterpret_cast<SkPoint*>(blobVertices + 2*vertexStride);
        ...
        if (clipRect.contains(positionRect)) {
            ...
            // Clamp position and texCoords to the clipRect
...
void GrAtlasTextOp::onPrepareDraws(Target* target) {
    ...
    if (this->usesDistanceFields()) {
        flushInfo.fGeometryProcessor = this->setupDfProcessor(localMatrix);
    } else {
...
//        memcpy(currVertex, blobVertices, byteCount);
        clip_quads(blob->clipRect(), currVertex, reinterpret_cast<unsigned char*>(blobVertices),
...
sk_sp<GrGeometryProcessor> GrAtlasTextOp::setupDfProcessor(const SkMatrix&) const {
