...
void GrAtlasTextOp::onPrepareDraws(Target* target) {
    ...
    // Where the quad buffer start and end relative to totalGlyphsRegened.
    ...
    for (int i = 0; i < fGeoCount; i++) {
        ...
        while (totalGlyphsRegened < subRunEnd) {
            ...
            int drawStart = totalGlyphsRegened - subRunBegin;
            ...
            // Either the atlas is full (totalGlyphsRegened != subRunEnd) or the quad buffer is
            // full (totalGlyphsRegened == quadBufferEnd) do what is needed to make space in the
            // atlas or add more quad buffer space.
            if (totalGlyphsRegened != subRunEnd || totalGlyphsRegened == quadBufferEnd) {
