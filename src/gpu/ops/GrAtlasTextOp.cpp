...
std::unique_ptr<char[]> GrAtlasTextOp::Geometry::vertexData(int offset, int count) const {
    size_t dataSize = fSubRunPtr->quadOffset(count);
    std::unique_ptr<char[]> data{new char[dataSize]};
    this->fillVertexData(data.get(), offset, count);
    return data;
}

void GrAtlasTextOp::Geometry::fillVertexData(void *dst, int offset, int count) const {
    SkASSERT(fDrawMatrix.hasPerspective() == fSubRunPtr->hasW());
    fSubRunPtr->fillVertexData(
            dst, offset, count, fColor.toBytes_RGBA(),
...
void GrAtlasTextOp::onPrepareDraws(Target* target) {
    ...
    for (int i = 0; i < fGeoCount; i++) {
        ...
        GrTextBlob::SubRun* subRun = args.fSubRunPtr;
        SkASSERT((int)subRun->vertexStride() == vertexStride);
        SkASSERT(subRun->hasW() == args.fDrawMatrix.hasPerspective());
        SkASSERT(vmPerspective == args.fDrawMatrix.hasPerspective());
