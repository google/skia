...
public:
     ...
     * pixel3, the driver has a bug when calling vkCmdClearAttachments from a primary command
     * buffer. So we need to track which ops will call a clear cmd (color or stencil) so that we can
     * use a secondary command buffer for those GrVkOpsRenderPass.
     */
