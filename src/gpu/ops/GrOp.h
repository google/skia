...
public:
        ...
        using OpOwner = std::unique_ptr<GrOp>;
    #else
        ...
        using OpOwner =  std::unique_ptr<GrOp, DeleteFromPool>;
    #endif

    template<typename Op, typename... Args>
    static OpOwner Make(GrRecordingContext* context, Args&&... args) {
        return MakeWithExtraMemory<Op>(context, 0, std::forward<Args>(args)...);
    }

    static size_t CalculateSizeWithProcessorSet(size_t opSize);
    static GrProcessorSet* InitializeProcessorSet(void* processorSetMem, GrPaint&& paint);

    template<typename Op, typename... Args>
    static OpOwner MakeWithProcessorSet(
            GrRecordingContext* context, const SkPMColor4f& color,
            GrPaint&& paint, Args&&... args) {
        #if defined(GR_OP_ALLOCATE_USE_NEW)
            char* bytes = (char*)::operator new(CalculateSizeWithProcessorSet(sizeof(Op)));
            char* setMem = bytes + sizeof(Op);
            GrProcessorSet* processorSet = InitializeProcessorSet(setMem, std::move(paint));
            return OpOwner{new (bytes) Op(processorSet, color, std::forward<Args>(args)...)};
        #else
            GrOpMemoryPool* pool = context->priv().opMemoryPool();
            char* bytes = (char*)pool->allocate(CalculateSizeWithProcessorSet(sizeof(Op)));
            char* setMem = bytes + sizeof(Op);
            GrProcessorSet* processorSet = InitializeProcessorSet(setMem, std::move(paint));
            return OpOwner{new (bytes) Op(processorSet, color, std::forward<Args>(args)...), pool};
        #endif
    }

    #if defined(GR_OP_ALLOCATE_USE_NEW)
        template<typename Op, typename... Args>
            static OpOwner MakeWithExtraMemory(
                    GrRecordingContext* context, size_t extraSize, Args&&... args) {
                void* bytes = ::operator new(sizeof(Op) + extraSize);
                return OpOwner{new (bytes) Op(std::forward<Args>(args)...)};
            }
    #else
        template<typename Op, typename... Args>
        static OpOwner MakeWithExtraMemory(
