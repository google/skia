    ...
    static OpOwner MakeWithProcessorSet(
        ...
        template<typename Op, typename... Args>
            static OpOwner MakeWithExtraMemory(
                ...
                return new (bytes) Op(std::forward<Args>(args)...);
            }
    #else
        template<typename Op, typename... Args>
        static OpOwner MakeWithExtraMemory(
