...
public:
    ...
    static std::unique_ptr<GrDrawOp> Make(GrPaint&& paint, const SkMatrix& viewMatrix,
        ...
        return Helper::FactoryHelper<AAStrokeRectOp>(std::move(paint), viewMatrix, rect, stroke, isMiter);
    }
...
std::unique_ptr<GrDrawOp> MakeFillBetweenRects(GrPaint&& paint,
                                                         const SkMatrix& viewMatrix,
                                                         const SkRect& devOutside,
                                                         const SkRect& devInside) {
...
GR_DRAW_OP_TEST_DEFINE(AAStrokeRectOp) {
    ...
    return GrAAStrokeRectOp::Make(std::move(paint), matrix, rect, rec);
}
