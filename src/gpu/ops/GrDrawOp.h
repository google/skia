...
public:
     ...
     * or combining it with a previously recorded op. The applied clip (if any) is available to the
     * op at this time but the op need not record it. It will be kept alongside the op and passed
     * back into the op when it executes.
     */
    virtual void finalize(const GrCaps&, const GrAppliedClip*, bool* xpRequiresDstTexture) = 0;
