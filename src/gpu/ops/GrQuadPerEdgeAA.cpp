...
bool ConfigureMeshIndices(GrMeshDrawOp::Target* target, GrMesh* mesh, const VertexSpec& spec,
    ...
    if (spec.usesCoverageAA()) {
        ...
        sk_sp<const GrGpuBuffer> ibuffer = target->resourceProvider()->refAAQuadIndexBuffer();
        if (!ibuffer) {
            return false;
        }
        ...
        if (quadCount > 1) {
            ...
            sk_sp<const GrGpuBuffer> ibuffer = target->resourceProvider()->refNonAAQuadIndexBuffer();
            if (!ibuffer) {
                return false;
            }
