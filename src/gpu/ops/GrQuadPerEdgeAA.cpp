...
// Assumes no zero-length edges and (dx,dy) normalized.
static AI Edges get_edge_equations(const Sk4f& xs, const Sk4f& ys, const Sk4f& dx, const Sk4f& dy) {
...
static bool get_optimized_outset(const QuadMetadata& metadata, bool rectilinear, Sk4f* outset) {
    if (rectilinear) {
        *outset = 0.5f;
        // Still go through the slow path if any of the edges are less than a pixel in length.
    ...
    *outset = 0.5f * (1.f - cosTheta * cosTheta).rsqrt(); // 1/2sin(theta)
    ...
    // And note that cos(pi - theta) = -cos(theta), which we already have
    Sk4f halfTanTheta = -cosTheta * (*outset);
...
static void outset_projected_vertices(const Sk4f& x2d, const Sk4f& y2d, Vertices* quad) {
    // Calculate 3D quad edges, but don't bother normalizing since the lengths will be baked into
    // the solved equations.
    Sk4f ex = nextCCW(quad->fX) - quad->fX;
    Sk4f ey = nextCCW(quad->fY) - quad->fY;
    Sk4f ew = nextCCW(quad->fW) - quad->fW;
    ...
    Sk4f c3x = quad->fX - x2d * quad->fW;
    Sk4f c3y = quad->fY - y2d * quad->fW;

    // a * c1x + b * c2x + c3x = 0; a * c1y + b * c2y + c3y = 0; solve for a,b
    Sk4f denom = c1x * c2y - c2x * c1y;
    Sk4f a = (c2x * c3y - c3x * c2y) / denom;
    Sk4f b = (c3x * c1y - c1x * c3y) / denom;

    quad->fX += b * nextCW(ex) - a * ex;
    quad->fY += b * nextCW(ey) - a * ey;
    quad->fW += b * nextCW(ew) - a * ew;

    // Now outset the texture coordinates to match new positions
    if (quad->fUVRCount > 0) {
        Sk4f eu = nextCCW(quad->fU) - quad->fU;
        Sk4f ev = nextCCW(quad->fV) - quad->fV;
        quad->fU += b * nextCW(eu) - a * eu;
        quad->fV += b * nextCW(ev) - a * ev;
        if (quad->fUVRCount == 3) {
            Sk4f er = nextCCW(quad->fR) - quad->fR;
            quad->fR += b * nextCW(er) - a * er;
        }
    }
}
...
static Sk4f compute_degenerate_quad(const Sk4f& dx, const Sk4f& dy, const Edges& edges,
                                    const Sk4f& mask, bool outset, Vertices* quad) {
    ...
    Sk4f bad = denom.abs() < kTolerance;
    px = bad.thenElse(nextCCW(px), px);
    py = bad.thenElse(nextCCW(py), py);
    ...
    outset_projected_vertices(px, py, quad);
    return coverage;
}
...
static Sk4f compute_nested_quad_vertices(GrQuadAAFlags aaFlags, bool rectilinear,
    ...
    Sk4f outset = 0.5f;
    bool get_optimized_outset(const QuadMetadata& metadata, bool rectilinear, Sk4f* outset);
    ...
    Sk4f badEdge = metadata.fInvLengths >= 1.f / kTolerance; // len < kTolerance
    // Correct for bad edges by copying adjacent edge information into the bad component
    Sk4f dx = badEdge.thenElse(nextCW(metadata.fDX), metadata.fDX);
    Sk4f dy = badEdge.thenElse(nextCW(metadata.fDY), metadata.fDY);

    // At this point inner and outer are not changed, so they should have the same vertices -> only
    // compute edges once
    Edges edges = get_edge_equations(inner->fX, inner->fY, dx, dy);

    // Calculate both outset and inset, returning the coverage reported for the inset, since the
    // outset will always have 0.0f.
    compute_degenerate_quad(dx, dy, edges, metadata.fMask, true, outer);
    return compute_degenerate_quad(dx, dy, edges, metadata.fMask, false, inner);
}
...
static Sk4f compute_nested_persp_quad_vertices(GrQuadAAFlags aaFlags, Vertices* inner,
                                               Vertices* outer) {
    SkASSERT(inner->fUVRCount == 0 || inner->fUVRCount == 2 || inner->fUVRCount == 3);
    SkASSERT(outer->fUVRCount == inner->fUVRCount);

    // Calculate the projected 2D quad and use it to form projeccted inner/outer quads
    Sk4f iw = inner->fW.invert();
    Sk4f x2d = inner->fX * iw;
    Sk4f y2d = inner->fY * iw;

    Vertices inner2D = { x2d, y2d, 1.f, 0.f, 0.f, 0.f, 0 }; // No uvr outsetting in 2D though
    ...
    outset_projected_vertices(inner2D.fX, inner2D.fY, inner);
    outset_projected_vertices(outer2D.fX, outer2D.fY, outer);
    return coverage;
}
...
static void write_quad(GrVertexWriter* vb, const GrQuadPerEdgeAA::VertexSpec& spec,
    ...
    SkPMColor4f finalColor = color4f;
    for (int i = 0; i < 4; ++i) {
        ...
        if (spec.hasVertexColors()) {
            if (mode == CoverageMode::kWithColor) {
                finalColor = color4f * coverage[i];
            }
            vb->write(GrVertexColor(finalColor, wideColor));
        }
