...
void Tessellator::append(GrQuad* deviceQuad, GrQuad* localQuad,
    ...
    if (fVertexSpec.usesCoverageAA()) {
        ...
        if (aaFlags == GrQuadAAFlags::kNone) {
            ...
            // if (deviceQuad->quadType() == GrQuad::Type::kPerspective) {
            //     // Disable AA on any edge that has a vertex behind the w = 0 plane
            //     skvx::Vec<4, float> w = deviceQuad->w4f();
            //     auto disable = w < 0.05f | skvx::shuffle<1, 3, 0, 2>(w) < 0.05f;
            //     edgeDistances *= if_then_else(disable, 0.f, 1.f);
            // }
