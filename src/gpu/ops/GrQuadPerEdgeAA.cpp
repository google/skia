...
GrQuadPerEdgeAA::GPAttributes::GPAttributes(int posDim, int localDim, bool hasColor, GrAA aa,
                                            Domain domain) {
    SkASSERT(posDim == 2 || posDim == 3);
...
void GrQuadPerEdgeAA::GPAttributes::emitExplicitLocalCoords(GrGLSLPrimitiveProcessor::EmitArgs& args,
                                                            const char* localCoordName,
    ...
    if (this->hasDomain()) {
        ...
        args.fVaryingHandler->addPassThroughAttribute(fDomain, domainName, Interpolation::kCanBeFlat);
        args.fFragBuilder->codeAppendf("%s = clamp(%s, %s.xy, %s.zw);",
                                       localCoordName, localCoordName, domainName, domainName);
    }
}

void GrQuadPerEdgeAA::GPAttributes::emitCoverage(GrGLSLPrimitiveProcessor::EmitArgs& args, const char* edgeDistName) const {
    if (this->usesCoverageAA()) {
