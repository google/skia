...
static void outset_projected_vertices(const V4f& x2d, const V4f& y2d,
    ...
    V4f scale = skvx::if_then_else(newW < kMinW, (kMinW - quad->fW) / (newW - quad->fW), V4f(1.f));
    a *= scale;
    b *= scale;
...
static V4f compute_degenerate_quad(GrQuadAAFlags aaFlags, const V4f& mask, const Edges& edges,
    ...
    if (!skvx::any(d1Or2)) {
        ...
        if (SkScalarAbs(eDenom[0]) > kTolerance) {
            px = skvx::if_then_else(d1v0, V4f(ex[0]), px);
            py = skvx::if_then_else(d1v0, V4f(ey[0]), py);
        }
        if (SkScalarAbs(eDenom[1]) > kTolerance) {
            px = skvx::if_then_else(d2v0, V4f(ex[1]), px);
            py = skvx::if_then_else(d2v0, V4f(ey[1]), py);
        }
