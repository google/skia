...
using M4f = skvx::Vec<4, skvx::M<float>>;
...
static AI void correct_bad_edges(const M4f& bad, V4f* e1, V4f* e2, V4f* e3) {
    if (skvx::any(bad)) {
        ...
        *e1 = skvx::if_then_else(bad, -skvx::shuffle<3, 2, 1, 0>(*e1), *e1);
        *e2 = skvx::if_then_else(bad, -skvx::shuffle<3, 2, 1, 0>(*e2), *e2);
        if (e3) {
            *e3 = skvx::if_then_else(bad, -skvx::shuffle<3, 2, 1, 0>(*e3), *e3);
        }
    }
}
...
static AI void correct_bad_coords(const M4f& bad, V4f* c1, V4f* c2, V4f* c3) {
    if (skvx::any(bad)) {
        *c1 = skvx::if_then_else(bad, nextCCW(*c1), *c1);
        *c2 = skvx::if_then_else(bad, nextCCW(*c2), *c2);
        if (c3) {
            *c3 = skvx::if_then_else(bad, nextCCW(*c3), *c3);
        }
    }
}
...
static AI QuadMetadata get_metadata(const Vertices& vertices, GrQuadAAFlags aaFlags) {
    ...
    V4f invLengths = skvx::rsqrt(fma(dx, dx, dy * dy));
...
static bool get_optimized_outset(const QuadMetadata& metadata, bool rectilinear, V4f* outset) {
    if (rectilinear) {
        *outset = 0.5f;
        ...
        return skvx::all(metadata.fInvLengths <= 1.f);
    }

    if (skvx::any(metadata.fInvLengths >= 1.f / kTolerance)) {
    ...
    if (skvx::any(skvx::abs(cosTheta) >= 0.9f)) {
        return false;
    }
    *outset = 0.5f * skvx::rsqrt(1.f - cosTheta * cosTheta); // 1/2sin(theta)
    ...
    return skvx::all(edgeAdjust > threshold) && skvx::all(edgeAdjust < -threshold);
}
...
static void outset_projected_vertices(const V4f& x2d, const V4f& y2d,
    ...
    if (aaFlags == GrQuadAAFlags::kAll) {
        ...
        M4f useC1x = skvx::abs(c1x) > skvx::abs(c1y);
        M4f useC2x = skvx::abs(c2x) > skvx::abs(c2y);

        denom = skvx::if_then_else(aMask,
    ...
    quad->fY += a * e1y + b * e2y;
    quad->fW += a * e1w + b * e2w;
    correct_bad_coords(skvx::abs(denom) < kTolerance, &quad->fX, &quad->fY, &quad->fW);
    ...
    if (quad->fUVRCount > 0) {
        ...
        if (quad->fUVRCount == 3) {
            quad->fR += a * e1r + b * e2r;
            correct_bad_coords(skvx::abs(denom) < kTolerance, &quad->fU, &quad->fV, &quad->fR);
        } else {
            correct_bad_coords(skvx::abs(denom) < kTolerance, &quad->fU, &quad->fV, nullptr);
        }
    }
}
...
static float get_exact_coverage(const SkPoint& pixelCenter, const Vertices& quad,
    ...
    bool topleftInside = skvx::all((edges.fA * left + edges.fB * top + edges.fC) >= 0.f);
    bool botleftInside = skvx::all((edges.fA * left + edges.fB * bot + edges.fC) >= 0.f);
    bool botrightInside = skvx::all((edges.fA * right + edges.fB * bot + edges.fC) >= 0.f);
    bool toprightInside = skvx::all((edges.fA * right + edges.fB * top + edges.fC) >= 0.f);
...
static V4f compute_degenerate_quad(GrQuadAAFlags aaFlags, const V4f& mask, const Edges& edges,
    ...
    correct_bad_coords(skvx::abs(denom) < kTolerance, &px, &py, nullptr);
    ...
    if (!skvx::any(d1Or2)) {
        ...
        if (SkScalarAbs(eDenom[0]) > kTolerance) {
            px = skvx::if_then_else(d1v0, ex[0], px);
            py = skvx::if_then_else(d1v0, ey[0], py);
        }
        if (SkScalarAbs(eDenom[1]) > kTolerance) {
            px = skvx::if_then_else(d2v0, ex[1], px);
            py = skvx::if_then_else(d2v0, ey[1], py);
        }
...
static V4f compute_nested_quad_vertices(GrQuadAAFlags aaFlags, bool rectilinear,
    ...
    if (!rectilinear) {
        ...
        // non-AA edges.
        domain->fLeft = skvx::min(outer->fX) - 0.5f;
        domain->fRight = skvx::max(outer->fX) + 0.5f;
        domain->fTop = skvx::min(outer->fY) - 0.5f;
        domain->fBottom = skvx::max(outer->fY) + 0.5f;
    }
...
static V4f compute_nested_persp_quad_vertices(const GrQuadAAFlags aaFlags, Vertices* inner,
    ...
    // Don't use V4f.invert() here because it does not preserve 1/1 == 1, which creates rendering
...
static void write_quad(GrVertexWriter* vb, const GrQuadPerEdgeAA::VertexSpec& spec,
                       CoverageMode mode, V4f coverage, SkPMColor4f color4f,
...
void* Tessellate(void* vertices, const VertexSpec& spec, const GrPerspQuad& deviceQuad,
    ...
    if (spec.usesCoverageAA()) {
        ...
        if (spec.deviceQuadType() == GrQuadType::kPerspective) {
            ...
            geomDomain.fLeft = skvx::min(outer.fX);
            geomDomain.fRight = skvx::max(outer.fX);
            geomDomain.fTop = skvx::min(outer.fY);
            geomDomain.fBottom = skvx::max(outer.fY);
        }
