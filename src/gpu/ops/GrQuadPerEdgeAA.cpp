...
public:
    ...
    void getGLSLProcessorKey(const GrShaderCaps&, GrProcessorKeyBuilder* b) const override {
        ...
        b->addBits(1, fTexSubset.isInitialized(),    "subset");
        b->addBits(1, fSampler.isInitialized(),      "textured");
        b->addBits(1, fNeedsPerspective,             "perspective");
        b->addBits(1, (fSaturate == Saturate::kYes), "saturate");

        b->addBits(1, fLocalCoord.isInitialized(),   "hasLocalCoords");
        if (fLocalCoord.isInitialized()) {
            // 2D (0) or 3D (1)
            b->addBits(1, (kFloat3_GrVertexAttribType == fLocalCoord.cpuType()), "localCoordsType");
        }
        b->addBits(1, fColor.isInitialized(),        "hasColor");
