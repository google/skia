...
private:
    ...
    struct CircleVertex {
        ...
        SkScalar fClampValue;
        uint16_t fFalloffFactorN;   // numerator of falloff factor (edge vs. corner)
        uint16_t fFalloffFactorD;   // denominator of falloff factor
    };
    ...
    void fillInCircleVerts(const Geometry& args, bool isStroked, CircleVertex** verts) const {
        ...
        (*verts)->fFalloffFactorN = 65535;
        (*verts)->fFalloffFactorD = 65535;
        (*verts)++;
    ...
    void fillInRRectVerts(const Geometry& args, CircleVertex** verts) const {
        ...
        // corner get skewed to a kite shape rather than a square. To correct for that,
        ...
        for (int i = 0; i < 4; ++i) {
            ...
            (*verts)->fDistanceCorrection = distanceCorrection;
            (*verts)->fClampValue = clampValue;
            (*verts)->fFalloffFactorN = SkScalarRoundToInt(65535 * (outerRadius / umbraInset));
            (*verts)->fFalloffFactorD = 65535;
            (*verts)++;
