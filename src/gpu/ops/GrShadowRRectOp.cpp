...
//
// In the case where the stroke width is greater than twice the corner radius or the
        ...
        if (devStrokeWidth > 0) {
            ...
            if (strokeOnly) {
                ...
                if (devStrokeWidth <= devRect.width() && devStrokeWidth <= devRect.height()) {
                    ...
                    // we will assume they want no special umbraInset. 
                    if (devStrokeWidth <= outerRadius) {
                        umbraInset = outerRadius;
                    }
                    // Beyond that we don't worry about an inner radius, we just need to know if we
...
private:
    ...
    void onPrepareDraws(Target* target) const override {
        ...
        for (int i = 0; i < instanceCount; i++) {
            ...
            // values equal to those in the center of the 9-patch. This will
