...
private:
    ...
    void fillInRRectVerts(const Geometry& args, CircleVertex** verts) const {
        ...
        SkScalar a = 2.0f * umbraInset*(umbraInset - outerRadius);
        SkScalar cosTheta = a / (a + outerRadius * outerRadius);
        uint16_t cornerFalloff = SkScalarTruncToInt(65535.99f * (SK_Scalar1 - cosTheta));
