...
GrSmallPathShapeData* GrSmallPathAtlasMgr::findOrCreate(const GrStyledShape& shape, int desiredDimension) {
    GrSmallPathShapeDataKey key(shape, desiredDimension);
    return this->findOrCreate(key);
}

GrSmallPathShapeData* GrSmallPathAtlasMgr::findOrCreate(const GrStyledShape& shape, const SkMatrix& ctm) {
...
void GrSmallPathAtlasMgr::setUseToken(GrSmallPathShapeData* shapeData, GrDeferredUploadToken token) {
