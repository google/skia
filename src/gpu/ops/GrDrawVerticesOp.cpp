...
std::unique_ptr<GrMeshDrawOp> GrDrawVerticesOp::Make(
    ...
    sk_sp<SkVertices> vertices = SkVertices::MakeCopy(kIgnoredMode, vertexCount,
                                                      positions, colors, localCoords,
                                                      indexCount ? indices : nullptr,
                                                      nullptr);
    if (!vertices) {
        return nullptr;
    }
    SkASSERT(vertices->vertexCount() == vertexCount);
    SkASSERT(vertices->indexCount() == indexCount);
