...
void GrDrawVerticesOp::onPrepareDraws(Target* target) const {
    ...
    sk_sp<GrGeometryProcessor> gp =
            make_gp(clientLocalCoords, fPipelineNeedsLocalCoords, fColorArrayType,
                    fDifferentViewMatrices, fMeshes[0].fViewMatrix, &hasLocalCoordAttribute);
    ...
    for (int i = 0; i < instanceCount; i++) {
        ...
        // Currently we require all meshes to either have explicit local corods or not, though it
