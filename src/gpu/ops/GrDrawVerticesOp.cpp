...
public:
    ...
    public:
                ...
                switch (usage) {
                    ...
                    case SkVertices::Attribute::Usage::kVector: {
                        if (!fLocalToWorldMatrixUniform.isValid()) {
                            fLocalToWorldMatrixUniform = uniformHandler->addUniform(
                                    kVertex_GrShaderFlag, kFloat4x4_GrSLType, "localToWorld");
                        }
                    ...
                    case SkVertices::Attribute::Usage::kBivector: {
                        if (!fBivectorMatrixUniform.isValid()) {
                            fBivectorMatrixUniform = uniformHandler->addUniform(
                                    kVertex_GrShaderFlag, kFloat3x3_GrSLType, "localToWorld_IT");
                        }
                        if (attr.gpuType() == kFloat2_GrSLType) {
                            varyingIn = SkStringPrintf("float3(%s, 0)", attr.name());
                        }
                        varyingIn = SkStringPrintf(
                                "(%s * %s)",
                                uniformHandler->getUniformCStr(fBivectorMatrixUniform),
                                varyingIn.c_str());
                        varyingType = kFloat3_GrSLType;
                        break;
                    }
        ...
        void setData(const GrGLSLProgramDataManager& pdman,
            ...
            if (fBivectorMatrixUniform.isValid()) {
                ...
                pdman.setMatrix3f(fBivectorMatrixUniform, l2wIT);
            }
        }
    ...
    private:
        ...
        UniformHandle fBivectorMatrixUniform;
...
GrOp::CombineResult DrawVerticesOp::onCombineIfPossible(GrOp* t, GrRecordingContext::Arenas*,
    ...
    if ((vThis.hasUsage(SkVertices::Attribute::Usage::kVector) ||
         vThis.hasUsage(SkVertices::Attribute::Usage::kBivector)) &&
        this->fLocalToWorld != that->fLocalToWorld) {
        return CombineResult::kCannotCombine;
    }
