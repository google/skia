...
std::unique_ptr<GrLegacyMeshDrawOp> GrDrawVerticesOp::Make(
    ...
    return std::unique_ptr<GrLegacyMeshDrawOp>(new GrDrawVerticesOp(std::move(vertices), primitiveType,
                                                              color, colorArrayType, viewMatrix));
}

std::unique_ptr<GrLegacyMeshDrawOp> GrDrawVerticesOp::Make(GrColor color, sk_sp<SkVertices> vertices,
