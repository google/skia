...
// Container for snapped-off values of a collection of [ID, Matrix] pairs
...
public:
    void gather(const SkVerticesPriv& info, const SkMarkedMatrixProvider* matrixProvider) {
        for (int i = 0; i < info.attributeCount(); ++i) {
            if (SkCanvas::MarkerID id = info.attributes()[i].fMarkerID) {
                if (std::find_if(fMatrices.begin(), fMatrices.end(),
                                 [id](const auto& m) { return m.fID == id; }) == fMatrices.end()) {
                    SkM44 matrix;
    ...
    bool operator==(const MarkedMatrices& that) const {
        return fMatrices == that.fMatrices;
    }

private:
    // We store one ID + Matrix for each unique marker ID present in a set of custom attributes
    struct MarkedMatrix {
        bool operator==(const MarkedMatrix& that) const {
            return fID == that.fID && fMatrix == that.fMatrix;
        }

        SkCanvas::MarkerID fID;
        SkM44              fMatrix;
    };
    std::vector<MarkedMatrix> fMatrices;
};
        ...
        void onEmitCode(EmitArgs& args, GrGPArgs* gpArgs) override {
            ...
            for (size_t i = kFirstCustomIndex; i < gp.fAttributes.size(); ++i) {
                ...
                if (customAttr.fMarkerID) {
                    ...
                    for (size_t j = 0; j < fCustomMatrixUniforms.size(); ++j) {
                        if (fCustomMatrixUniforms[j].fID == customAttr.fMarkerID &&
                            fCustomMatrixUniforms[j].fNormal == normal) {
                            matrixHandle = fCustomMatrixUniforms[i].fUniform;
                            break;
                        }
                    }
                ...
                // vectors/positions without markerID should use localToDevice... Need to use
                // view matrix uniform? Force to use shader update, by never allowing multiple view
                // matrices?
