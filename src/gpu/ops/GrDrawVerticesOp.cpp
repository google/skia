...
void GrDrawVerticesOp::onPrepareDraws(Target* target) {
    if (fMeshes[0].fVertices->isVolatile()) {
        ...
        // TODO: Change to drawNonVolatile when cache issue is fixed.
        drawVolatile(target);
    }
}
...
void GrDrawVerticesOp::drawVolatile(Target* target) {
    ...
    // Draw the vertices.
    drawVertices(target, gp.get(), vertexBuffer, firstVertex, indexBuffer, firstIndex);
}
...
void GrDrawVerticesOp::drawNonVolatile(Target* target) {
    ...
    if (vertexBuffer) {
        drawVertices(target, gp.get(), vertexBuffer.get(), 0, indexBuffer.get(), 0);
        return;
    }
