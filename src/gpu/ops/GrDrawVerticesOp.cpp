    ...
    public:
        ...
        void onEmitCode(EmitArgs& args, GrGPArgs* gpArgs) override {
            ...
            this->writeOutputPosition(args.fVertBuilder, args.fUniformHandler, gpArgs,
                                      verticesGP.fAttributes.front().name(),
                                      verticesGP.viewMatrix(), &fViewMatrixUniform);

            for (const auto& attr : verticesGP.fAttributes) {
                args.fVaryingHandler->addPassThroughAttribute(attr, attr.name());
            }
...
private:
    ...
    CustomVerticesGP(const SkMatrix& viewMatrix, int attrWidths[], int attrCount)
        ...
        for (int i = 0; i < attrCount; ++i) {
            ...
            fAttributes.push_back({ SkStringPrintf("_vtx_attr%d", i).c_str(),
...
public:
    ...
    DrawCustomVerticesOp(const Helper::MakeArgs&, const SkPMColor4f&, sk_sp<SkVertices>,
                         GrPrimitiveType, GrAAType, const SkMatrix& viewMatrix);
...
GrGeometryProcessor* DrawCustomVerticesOp::makeGP(SkArenaAlloc* arena) {
    int widths[] = { 4 };
    auto gp = CustomVerticesGP::Make(arena, fViewMatrix, widths, 1);
...
std::unique_ptr<GrDrawOp> GrDrawVerticesOp::MakeCustom(GrRecordingContext* context,
    ...
    return GrSimpleMeshDrawOpHelper::FactoryHelper<DrawCustomVerticesOp>(
            context, std::move(paint), std::move(vertices), primType, aaType, viewMatrix);
}
