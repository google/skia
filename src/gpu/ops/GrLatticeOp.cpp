...
public:
    ...
    NonAALatticeOp(Helper::MakeArgs& helperArgs, const SkPMColor4f& color,
                   ...
                   bool hasAlpha, sk_sp<GrColorSpaceXform> colorSpaceXform,
    ...
    GrProcessorSet::Analysis finalize(
        ...
        auto opaque = fPatches[0].fColor.isOpaque() && !fHasAlpha
...
std::unique_ptr<GrDrawOp> MakeNonAA(GrRecordingContext* context,
                                    ...
                                    std::unique_ptr<SkLatticeIter> iter,
                                    const SkRect& dst) {
    return NonAALatticeOp::Make(context, std::move(paint), viewMatrix, std::move(view), hasAlpha,
...
GR_DRAW_OP_TEST_DEFINE(NonAALatticeOp) {
    ...
    return NonAALatticeOp::Make(context, std::move(paint), viewMatrix, std::move(view),
                                true, std::move(csxf), filter, std::move(iter), dst);
}
