 ...
 *
 * @tparam AllowStencil - Does the op support using GrUserStencilSettings? If true it means the op
 * *can* have stencil settings, not that every instance must or is assumed to.
 */
template <bool AllowStencil>
class GrSimpleMeshDrawOpHelper;

template <>
class GrSimpleMeshDrawOpHelper<false> {
public:
    struct MakeArgs;
    ...
    GrSimpleMeshDrawOpHelper(const MakeArgs& args, GrAAType aaType,
            ...
            : fProcessors(args.fProcessorSet), fPipelineFlags(args.fSRGBFlags) {
    ...
    bool isCompatible(const GrSimpleMeshDrawOpHelper<false>& that) const {
        if (SkToBool(fProcessors) != SkToBool(that.fProcessors)) {
            return false;
        }
        if (!SkToBool(fProcessors)) {
            return true;
        }
    ...
    bool xpRequiresDstTexture(const GrCaps& caps, const GrAppliedClip* clip,
        ...
        if (fProcessors) {
            ...
            GrProcessorSet::Analysis analysis =
                    fProcessors->finalize(*color, coverage, clip, false, caps, color);
...
template <>
class GrSimpleMeshDrawOpHelper<true> : public GrSimpleMeshDrawOpHelper<false> {
public:
    GrSimpleMeshDrawOpHelper(const MakeArgs& args, GrAAType aaType,
    ...
    bool isCompatible(const GrSimpleMeshDrawOpHelper<true>& that) const {
...
private:
    ...
    typedef GrSimpleMeshDrawOpHelper<false> INHERITED;
};

template <typename Op, typename... OpArgs>
std::unique_ptr<GrDrawOp> GrSimpleMeshDrawOpHelper<false>::FactoryHelper(GrPaint&& paint,
                                                                         OpArgs... opArgs) {
    typename GrSimpleMeshDrawOpHelper<false>::MakeArgs makeArgs;
