...
public:
    ...
    ~GrAtlasTextOp() override {
        for (const Geometry* g = fHead.get(); g != nullptr; g = g->fNext.get()) {
            g->fBlob->unref();
        }
    }
    ...
    struct Geometry {
        ...
        SkPMColor4f fColor;
        SkDestroyerPtr<Geometry> fNext{nullptr};
    };
...
private:
    ...
    void addGeometry(SkDestroyerPtr<Geometry> geometry) {
        *fTail = std::move(geometry);
    ...
    SkDestroyerPtr<Geometry> fHead;
    SkDestroyerPtr<Geometry>* fTail{&fHead};
