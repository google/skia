...
// Normalized geometry for octagons that circumscribe and inscribe a circle:
...
private:
    void onPrepareDraws(Target* target) override {
        ...
        size_t vertexStride =
            sizeof(SkPoint) +   // Position
            sizeof(GrColor) +   // Color
            sizeof(SkPoint) +   // Offset
            sizeof(SkScalar) +  // Outer radius
            sizeof(SkScalar) +  // Inner radius
            (fClipPlane      ? 3 * sizeof(SkScalar) : 0) +
            (fClipPlaneIsect ? 3 * sizeof(SkScalar) : 0) +
            (fClipPlaneUnion ? 3 * sizeof(SkScalar) : 0) +
...
private:
    void onPrepareDraws(Target* target) override {
        ...
        static constexpr size_t kVertexStride =
            ...
            sizeof(SkScalar) +  // Outer radius
            sizeof(SkScalar) +  // Inner radius
            sizeof(SkScalar) +  // On angle
            sizeof(SkScalar) +  // Total angle
            sizeof(SkScalar) +  // Start angle
            sizeof(SkScalar);   // Phase angle
...
private:
    void onPrepareDraws(Target* target) override {
        ...
        static constexpr size_t kVertexStride =
            ...
            sizeof(GrColor) +  // Color
            sizeof(SkPoint) +  // Offset
            sizeof(SkPoint) +  // Outer radii
            sizeof(SkPoint);   // Inner radii
        SkASSERT(kVertexStride == gp->debugOnly_vertexStride());

        QuadHelper helper(target, kVertexStride, fEllipses.count());
        ...
        for (const auto& ellipse : fEllipses) {
            ...
            verts = WriteVertexData(verts,
                                    ...
                                    SkPoint::Make(xRadRecip, yRadRecip),
                                    SkPoint::Make(xInnerRadRecip, yInnerRadRecip));
...
private:
    void onPrepareDraws(Target* target) override {
        ...
        constexpr size_t kVertexStride =
            sizeof(SkPoint) +  // Position
            sizeof(GrColor) +  // Color
            sizeof(SkPoint) +  // Outer offset
            sizeof(SkPoint);   // Inner offset
        SkASSERT(kVertexStride == gp->debugOnly_vertexStride());
