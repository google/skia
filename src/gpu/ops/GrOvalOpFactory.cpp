...
public:
    CircleGeometryProcessor(bool stroke, bool clipPlane, bool isectPlane, bool unionPlane,
                            GrAA aa, const SkMatrix& localMatrix)
...
public:
    ...
    , fLocalMatrix(localMatrix), fStroke(stroke), fAA(aa) {
        fInPosition = &this->addVertexAttrib("inPosition", kFloat2_GrVertexAttribType);
        fInColor = &this->addVertexAttrib("inColor", kUByte4_norm_GrVertexAttribType);
...
sk_sp<GrGeometryProcessor> EllipseGeometryProcessor::TestCreate(GrProcessorTestData* d) {
    ...
    return sk_sp<GrGeometryProcessor>(
            new EllipseGeometryProcessor(d->fRandom->nextBool(), aa, GrTest::TestMatrix(d->fRandom)));
}
#endif
...
public:
    DIEllipseGeometryProcessor(const SkMatrix& viewMatrix, DIEllipseStyle style, GrAA aa)
            ...
            , fViewMatrix(viewMatrix), fStyle(style), fAA(aa) {
        fInPosition = &this->addVertexAttrib("inPosition", kFloat2_GrVertexAttribType);
        fInColor = &this->addVertexAttrib("inColor", kUByte4_norm_GrVertexAttribType);
...
static std::unique_ptr<GrDrawOp> make_rrect_op(GrPaint&& paint,
    ...
    if (isCircular) {
        return CircularRRectOp::Make(std::move(paint), aa, viewMatrix, bounds, xRadius, scaledStroke.fX,
                                     isStrokeOnly);
...
std::unique_ptr<GrDrawOp> GrOvalOpFactory::MakeCoverageArcOp(GrPaint &&paint, GrAA aa,
                                                             const SkMatrix &viewMatrix,
                                                             const SkRect &oval,
                                                             SkScalar startAngle,
                                                             SkScalar sweepAngle,
...
GR_DRAW_OP_TEST_DEFINE(EllipseOp) {
    ...
    GrAA aa = random->nextBool() ? GrAA::kYes : GrAA::kNo;
    return EllipseOp::Make(std::move(paint), aa, viewMatrix, ellipse, GrTest::TestStrokeRec(random));
}
...
GR_DRAW_OP_TEST_DEFINE(DIEllipseOp) {
    ...
    SkRect ellipse = GrTest::TestSquare(random);
    GrAA aa = random->nextBool() ? GrAA::kYes : GrAA::kNo;
    return DIEllipseOp::Make(std::move(paint), aa, viewMatrix, ellipse, GrTest::TestStrokeRec(random));
}
