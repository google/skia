...
private:
    void onPrepareDraws(Target* target) override {
        ...
        for (const auto& rrect : fRRects) {
            ...
            if (fStroked) {
                xRadRecip = SkScalarInvert(rrect.fXRadius);
                yRadRecip = SkScalarInvert(rrect.fYRadius);
                xInnerRadRecip = SkScalarInvert(rrect.fInnerXRadius);
                yInnerRadRecip = SkScalarInvert(rrect.fInnerYRadius);
                ...
                // We'll also map a unit circle in the vertex attributes rather than computing an
                // ellipse and modifying that distance.
                xRadRecip = SkScalarInvert(rrect.fXRadius + SK_ScalarHalf);
                yRadRecip = SkScalarInvert(rrect.fYRadius + SK_ScalarHalf);
                xInnerRadRecip = SkScalarInvert(rrect.fInnerXRadius - SK_ScalarHalf);
                yInnerRadRecip = SkScalarInvert(rrect.fInnerYRadius - SK_ScalarHalf);
                xMaxOffset = 1;
                yMaxOffset = 1;
            }
