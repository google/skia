            ...
            if (egp.fStroke) {
                ...
                if (egp.fUseScale) {
                    fragBuilder->codeAppendf("grad = 2.0*offset*(%s.z*%s.zw);",
                                             ellipseOffsets.fsIn(), ellipseRadii.fsIn());
                    fragBuilder->codeAppendf("invlen = %s.z*inversesqrt(dot(grad, grad));",
                                             ellipseOffsets.fsIn());
                } else {
    ...
    public:
        ...
        void onEmitCode(EmitArgs& args, GrGPArgs* gpArgs) override {
            ...
            if (DIEllipseStyle::kStroke == diegp.fStyle) {
               ...
               fragBuilder->codeAppend("invlen = inversesqrt(4.0*dot(grad, grad));");
                if (diegp.fUseScale) {
                    fragBuilder->codeAppendf("invlen *= %s.z;", offsets0.fsIn());
                }
                fragBuilder->codeAppend("edgeAlpha *= saturate(0.5+test*invlen);");
...
private:
    void onPrepareDraws(Target* target) override {
        ...
        for (const auto& ellipse : fEllipses) {
            ...
            // The inner radius in the vertex data must be specified in normalized space.
            if (fUseScale) {
                SkScalar scaleFactor = SkTMax(xRadius, yRadius);
...
private:
    void onPrepareDraws(Target* target) override {
        ...
        for (const auto& rrect : fRRects) {
            ...
            if (fUseScale) {
                SkScalar scaleFactor = SkTMax(rrect.fXRadius, rrect.fYRadius);
                for (int i = 0; i < 4; ++i) {
                    verts.write(bounds.fLeft, yCoords[i],
                                color,
                                xMaxOffset, yOuterOffsets[i], scaleFactor,
                                reciprocalRadii);

                    verts.write(bounds.fLeft + xOuterRadius, yCoords[i],
                                color,
                                SK_ScalarNearlyZero, yOuterOffsets[i], scaleFactor,
                                reciprocalRadii);
