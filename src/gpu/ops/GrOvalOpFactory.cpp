...
public:
    CircleGeometryProcessor(bool stroke, bool clipPlane, bool isectPlane, bool unionPlane,
            ...
            , fStroke(stroke) {
        const Attribute* prev = &kInCircleEdge;
        int cnt = 3;
        if (clipPlane) {
            fInClipPlane = {"inClipPlane", kHalf3_GrVertexAttribType, *prev};
            prev = &fInClipPlane;
            ++cnt;
        }
        if (isectPlane) {
            fInIsectPlane = {"inIsectPlane", kHalf3_GrVertexAttribType, *prev};
            prev = &fInIsectPlane;
            ++cnt;
        }
        if (unionPlane) {
            fInUnionPlane = {"inUnionPlane", kHalf3_GrVertexAttribType, *prev};
            prev = &fInUnionPlane;
            ++cnt;
        }
        if (roundCaps) {
            SkASSERT(stroke);
            SkASSERT(clipPlane);
            fInRoundCapCenters = {"inRoundCapCenters", kFloat4_GrVertexAttribType, *prev};
            prev = &fInRoundCapCenters;
            ++cnt;
        }
        this->setVertexAttributeInfo(cnt, prev->nextOffsetInRecord());
    }
...
private:
    ...
    static constexpr Attribute kInCircleEdge = {"inCircleEdge", kFloat4_GrVertexAttribType, kInColor};
...
template <int N> class StaticAttributeHelper {
public:
    struct I {
        const char* name;
    ...
    template <typename... Init2ToN>
    constexpr StaticAttributeHelper(I init1, Init2ToN... init2ToN) {
        this->initAttrs(0, init1, init2ToN...);
    }
private:
    constexpr void initAttrs(int i) { assert(i == 0); }

    template <typename... Init2ToN>
    constexpr void initAttrs(int i, I init1, Init2ToN... init2ToN) {
        assert(i < N);
        kAttributes[i] = {init1.name, init1.type, i == 0 ? 0 : kAttributes[i-1].nextOffsetInRecord()};
        this->initAttrs(i + 1, init2ToN...);
    }

    static constexpr int kNum = N;
    GrPrimitiveProcessor::Attribute kAttributes[kNum];
};
...
public:
    ButtCapDashedCircleGeometryProcessor(const SkMatrix& localMatrix)
        ...
        this->setVertexAttributeInfo(4, kInDashParams.nextOffsetInRecord());
    }
...
private:
    ...
    static constexpr Attribute kInColor = {"inColor", kUByte4_norm_GrVertexAttribType, kInPosition};
    static constexpr Attribute kInCircleEdge = {"inCircleEdge", kFloat4_GrVertexAttribType, kInColor};
    static constexpr Attribute kInDashParams = {"inDashParams", kFloat4_GrVertexAttribType, kInCircleEdge};
...
public:
    ...
    , fLocalMatrix(localMatrix) {
        this->setVertexAttributeInfo(4, kInEllipseRadii.nextOffsetInRecord());
        fStroke = stroke;
    }
...
private:
    ...
    static constexpr Attribute kInPosition = {"inPosition", kFloat2_GrVertexAttribType};
    static constexpr Attribute kInColor = {"inColor", kUByte4_norm_GrVertexAttribType, kInPosition};
    static constexpr Attribute kInEllipseOffset = {"inEllipseOffset", kHalf2_GrVertexAttribType, kInColor};
    static constexpr Attribute kInEllipseRadii = {"inEllipseRadii", kHalf4_GrVertexAttribType, kInEllipseOffset};
...
public:
    DIEllipseGeometryProcessor(const SkMatrix& viewMatrix, DIEllipseStyle style)
        ...
        fStyle = style;
        this->setVertexAttributeInfo(4, kInEllipseOffsets1.nextOffsetInRecord());
    }
...
private:
    ...
    static constexpr Attribute kInPosition = {"inPosition", kFloat2_GrVertexAttribType};
    static constexpr Attribute kInColor = {"inColor", kUByte4_norm_GrVertexAttribType, kInPosition};
    static constexpr Attribute kInEllipseOffsets0 = {"inEllipseOffsets0", kHalf2_GrVertexAttribType, kInColor};
    static constexpr Attribute kInEllipseOffsets1 = {"inEllipseOffsets1", kHalf2_GrVertexAttribType, kInEllipseOffsets0};
