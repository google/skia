...
private:
    ...
    public:
        ...
        void onEmitCode(EmitArgs& args, GrGPArgs* gpArgs) override {
            const ButtCapStrokedCircleGeometryProcessor& bcscgp =
                    args.fGP.cast<ButtCapStrokedCircleGeometryProcessor>();
            GrGLSLVertexBuilder* vertBuilder = args.fVertBuilder;
            GrGLSLVaryingHandler* varyingHandler = args.fVaryingHandler;
            GrGLSLUniformHandler* uniformHandler = args.fUniformHandler;
            GrGLSLFPFragmentBuilder* fragBuilder = args.fFragBuilder;
            ...
            fragBuilder->codeAppend(R"(
                    ...
                    half2 nextDash = half2(dashParams.y - dashParams.w, dashParams.y + dashParams.x - dashParams.w);
                    half2 prevDash = half2(-dashParams.y - dashParams.w, -dashParams.y + dashParams.x - dashParams.w);
                    half dashAlpha = 0;
                )");
        ...
        static void GenKey(const GrGeometryProcessor& gp,
                           ...
                           GrProcessorKeyBuilder* b) {
            const ButtCapStrokedCircleGeometryProcessor& bcscgp =
                    gp.cast<ButtCapStrokedCircleGeometryProcessor>();
        ...
        void setData(const GrGLSLProgramDataManager& pdman, const GrPrimitiveProcessor& primProc,
            ...
            this->setTransformDataHelper(
                    primProc.cast<ButtCapStrokedCircleGeometryProcessor>().fLocalMatrix, pdman,
                    &transformIter);
        }
...
private:
    void onPrepareDraws(Target* target) override {
        ...
        for (const auto& circle : fCircles) {
            ...
            // Compute the vertices of the outer ocaogon.
            SkPoint center = SkPoint::Make(bounds.centerX(), bounds.centerY());
            SkScalar halfWidth = 0.5f * bounds.width();
...
std::unique_ptr<GrDrawOp> GrOvalOpFactory::MakeOvalOp(GrPaint&& paint,
    ...
    if (width > SK_ScalarNearlyZero && SkScalarNearlyEqual(width, oval.height()) &&
        ...
        return CircleOp::Make(std::move(paint), viewMatrix, center, width / 2.f, style);
    }
