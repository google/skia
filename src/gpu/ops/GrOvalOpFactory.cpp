...
public:
    CircleGeometryProcessor(bool stroke, bool clipPlane, bool isectPlane,
                            bool unionPlane, bool roundCaps, const SkMatrix& localMatrix)
            : INHERITED(kCircleGeometryProcessor_ClassID)
            , fLocalMatrix(localMatrix), fStroke(stroke) {
        ...
        if (roundCaps) {
            SkASSERT(stroke);
            SkASSERT(clipPlane);
            fInRoundCapCenters = &this->addVertexAttrib("inRoundCapCenterss", kFloat4_GrVertexAttribType);
        } else {
    ...
    public:
        ...
        void onEmitCode(EmitArgs& args, GrGPArgs* gpArgs) override {
            ...
            if (cgp.fInRoundCapCenters) {
                fragBuilder->codeAppend("float4 roundCaps;");
                varyingHandler->addPassThroughAttribute(cgp.fInRoundCapCenters, "roundCaps");
            ...
            if (cgp.fInClipPlane) {
                ...
                if (cgp.fInRoundCapCenters) {
                    fragBuilder->codeAppendf(
                            "half distanceToCap1 = circleEdge.z * (%s - length(circleEdge.xy - roundCaps.xy));"
                            "half distanceToCap2 = circleEdge.z * (%s - length(circleEdge.xy - roundCaps.zw));"
                            "half capAlpha = min(clamp(distanceToCap1, 0, 1) + clamp(distanceToCap2, 0, 1), 1);"
                            "edgeAlpha = min(edgeAlpha + capAlpha, 1.0);", capRadius.fsIn(), capRadius.fsIn());
                }
            }
...
sk_sp<GrGeometryProcessor> CircleGeometryProcessor::TestCreate(GrProcessorTestData* d) {
    ...
    return sk_sp<GrGeometryProcessor>(new CircleGeometryProcessor(
            stroke, roundCaps, clipPlane, isectPlane, unionPlane, matrix));
}
#endif
...
public:
    ...
    CircleOp(const Helper::MakeArgs& helperArgs, GrColor color, const SkMatrix& viewMatrix,
        ...
        if (arcParams) {
            ...
            // center of the butts.
            if (arcParams->fUseCenter || isStrokeOnly) {
                ...
                fClipPlane = true;
                // We have to be careful about half-circles. In that case the two radial lines are
                // equal and so that edge gets clipped twice. Don't add a second edge.
                auto sweepAbs = SkScalarAbs(arcParams->fSweepAngleRadians);
                if (SkScalarNearlyEqual(sweepAbs, SK_ScalarPI)) {
...
private:
    void onPrepareDraws(Target* target) override {
        ...
        auto vertexCapCenters = [numPlanes](CircleVertex* v) { return (void*)(v->fHalfPlanes + numPlanes); };
