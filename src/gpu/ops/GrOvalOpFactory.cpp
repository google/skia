...
public:
    ...
    static std::unique_ptr<GrDrawOp> Make(GrRecordingContext* context,
        ...
        return Helper::FactoryHelper<EllipseOp>(context, std::move(paint), aa, viewMatrix,
                                                params, stroke);
    }

    EllipseOp(const Helper::MakeArgs& helperArgs, const SkPMColor4f& color,
              GrAA aa, const SkMatrix& viewMatrix, const DeviceSpaceParams& params,
              const SkStrokeRec& stroke)
            : INHERITED(ClassID())
...
public:
    ...
    static std::unique_ptr<GrDrawOp> Make(GrRecordingContext* context,
        ...
        return Helper::FactoryHelper<DIEllipseOp>(context, std::move(paint), aa, params,
                                                  viewMatrix);
    }

    DIEllipseOp(Helper::MakeArgs& helperArgs, const SkPMColor4f& color, GrAA aa,
...
public:
    ...
    static std::unique_ptr<GrDrawOp> Make(GrRecordingContext* context,
                                          ...
                                          bool strokeOnly) {
        return Helper::FactoryHelper<CircularRRectOp>(context, std::move(paint), aa, viewMatrix,
                                                      devRect, devRadius,
                                                      devStrokeWidth, strokeOnly);
    }
    CircularRRectOp(Helper::MakeArgs& helperArgs, const SkPMColor4f& color, GrAA aa,
...
public:
    ...
    static std::unique_ptr<GrDrawOp> Make(GrRecordingContext* context,
        ...
        return Helper::FactoryHelper<EllipticalRRectOp>(context, std::move(paint),
                                                        aa, viewMatrix, devRect,
                                                        devXRadius, devYRadius, devStrokeWidths,
                                                        strokeOnly);
    }

    EllipticalRRectOp(Helper::MakeArgs helperArgs, const SkPMColor4f& color, GrAA aa,
...
static std::unique_ptr<GrDrawOp> make_rrect_op(GrRecordingContext* context,
    ...
    if (isCircular) {
        return CircularRRectOp::Make(context, std::move(paint), aa, viewMatrix, bounds, xRadius,
                                     scaledStroke.fX, isStrokeOnly);
        // otherwise we use the ellipse renderer
    } else {
        return EllipticalRRectOp::Make(context, std::move(paint), aa, viewMatrix, bounds,
...
std::unique_ptr<GrDrawOp> GrOvalOpFactory::MakeRRectOp(GrRecordingContext* context,
    ...
    if (rrect.isOval()) {
        return MakeOvalOp(context, std::move(paint), aa, viewMatrix, rrect.getBounds(),
    ...
    return make_rrect_op(context, std::move(paint), aa, viewMatrix, rrect, stroke);
}
...
std::unique_ptr<GrDrawOp> GrOvalOpFactory::MakeOvalOp(GrRecordingContext* context,
    ...
    if (viewMatrix.rectStaysRect()) {
        return EllipseOp::Make(context, std::move(paint), aa, viewMatrix, oval, style.strokeRec());
    }
    ...
    if (shaderCaps->shaderDerivativeSupport()) {
        ...
        if (a*a + c*c > SK_ScalarNearlyZero && b*b + d*d > SK_ScalarNearlyZero) {
            return DIEllipseOp::Make(context, std::move(paint), aa, viewMatrix, oval,
                                     style.strokeRec());
        }
    }
...
std::unique_ptr<GrDrawOp> GrOvalOpFactory::MakeArcOp(GrRecordingContext* context,
    ...
    CircleOp::ArcParams arcParams = {SkDegreesToRadians(startAngle), SkDegreesToRadians(sweepAngle),
                                     useCenter};
    return CircleOp::Make(context, std::move(paint), aa, viewMatrix,
...
GR_DRAW_OP_TEST_DEFINE(EllipseOp) {
    ...
    return EllipseOp::Make(context, std::move(paint), aa, viewMatrix, ellipse,
...
GR_DRAW_OP_TEST_DEFINE(DIEllipseOp) {
    ...
    SkRect ellipse = GrTest::TestSquare(random);
    return DIEllipseOp::Make(context, std::move(paint), aa, viewMatrix, ellipse,
                             GrTest::TestStrokeRec(random));
}
...
GR_DRAW_OP_TEST_DEFINE(RRectOp) {
    ...
    return make_rrect_op(context, std::move(paint), aa, viewMatrix, rrect,
                         GrTest::TestStrokeRec(random));
}
