...
#if GR_TEST_UTILS
sk_sp<GrGeometryProcessor> ButtCapStrokedCircleGeometryProcessor::TestCreate(GrProcessorTestData* d) {
    const SkMatrix& matrix = GrTest::TestMatrix(d->fRandom);
    return sk_sp<GrGeometryProcessor>(new ButtCapStrokedCircleGeometryProcessor(matrix));
}
#endif
...
GR_DRAW_OP_TEST_DEFINE(ButtCapDashedCircleOp) {
    ...
    SkScalar onAngle = random->nextRangeScalar(0.01, 1000.f);
    SkScalar offAngle = random->nextRangeScalar(0.01, 1000.f);
