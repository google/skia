...
private:
    ...
    void onPrepareDraws(Target* target) override {
        ...
        for (int i = 0; i < instanceCount; i++) {
            ...
            if (fUsesDistanceField) {
                ...
                shapeData = foo->findOrCreate(args.fShape, SkScalarCeilToInt(desiredDimension));
                ...
                shapeData = foo->findOrCreate(args.fShape, args.fViewMatrix);
                if (!shapeData->fAtlasLocator.plotLocator().isValid()) {
    ...
    bool addDFPathToAtlas(GrMeshDrawOp::Target* target, FlushInfo* flushInfo,
        ...
        if (!this->addToAtlas(target, flushInfo, foo->atlas(), width, height, dfStorage.get(),
                              &atlasLocator)) {
            return false;
        }
        ...
        foo->updateCacheInfo(shapeData, atlasLocator, bounds1);
    ...
    bool addBMPathToAtlas(GrMeshDrawOp::Target* target, FlushInfo* flushInfo,
        ...
        foo->updateCacheInfo(shapeData, atlasLocator,
