...
public:
    ...
    void visitProxies(const VisitProxyFunc& func) const override {
        ...
        for (uint32_t i = 0; i < fAtlas->pageCount1(); ++i) {
            SkASSERT(proxies[i]);
            func(proxies[i].get());
        }
    }
...
private:
    ...
    void onPrepareDraws(Target* target) override {
        ...
        uint32_t atlasPageCount = atlas->pageCount1();
        if (!atlasPageCount) {
            return;
        }
    ...
    bool addDFPathToAtlas(GrMeshDrawOp::Target* target, FlushInfo* flushInfo,
        ...
        auto uploadTarget = target->deferredUploadTarget();
        if (!atlas->addToAtlas1(&id, uploadTarget, width, height, dfStorage.get(), &atlasLocation)) {
            this->flush(target, flushInfo);
            if (!atlas->addToAtlas1(&id, uploadTarget, width, height, dfStorage.get(),
                                   &atlasLocation)) {
                return false;
            }
        }
    ...
    bool addBMPathToAtlas(GrMeshDrawOp::Target* target, FlushInfo* flushInfo,
        ...
        GrDrawOpAtlas::AtlasID id;
        auto uploadTarget = target->deferredUploadTarget();
        if (!atlas->addToAtlas1(&id, uploadTarget, dst.width(), dst.height(), dst.addr(),
                               &atlasLocation)) {
            this->flush(target, flushInfo);
            if (!atlas->addToAtlas1(&id, uploadTarget, dst.width(), dst.height(), dst.addr(),
                                   &atlasLocation)) {
    ...
    void flush(GrMeshDrawOp::Target* target, FlushInfo* flushInfo) const {
        ...
        if (gp->numTextureSamplers() != (int)fAtlas->pageCount1()) {
...
bool GrSmallPathRenderer::onDrawPath(const DrawPathArgs& args) {
    ...
    if (!fAtlas) {
        fAtlas = GrDrawOpAtlas::Make(kAlpha_8_GrPixelConfig,
...
GR_DRAW_OP_TEST_DEFINE(SmallPathOp) {
    ...
    if (context->uniqueID() != gTestStruct.fContextID) {
        ...
        gTestStruct.reset();
        gTestStruct.fAtlas = GrDrawOpAtlas::Make(kAlpha_8_GrPixelConfig,
                                                 ATLAS_TEXTURE_WIDTH, ATLAS_TEXTURE_HEIGHT,
                                                 NUM_PLOTS_X, NUM_PLOTS_Y,
                                                 GrDrawOpAtlas::AllowMultitexturing::kYes,
