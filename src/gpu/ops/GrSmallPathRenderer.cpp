...
private:
    ...
    void onPrepareDraws(Target* target) override {
        ...
        for (int i = 0; i < instanceCount; i++) {
            ...
            if (fUsesDistanceField) {
                ...
                shapeData = atlasMgr->findOrCreate(args.fShape, SkScalarCeilToInt(desiredDimension));
    ...
    bool addDFPathToAtlas(GrMeshDrawOp::Target* target, FlushInfo* flushInfo,
                          GrSmallPathAtlasMgr* atlasMgr, ShapeData* shapeData, const GrStyledShape& shape,
        ...
        if (!this->addToAtlas(target, flushInfo, atlasMgr->atlas(), width, height, dfStorage.get(),
                              &atlasLocator)) {
            return false;
        }
    ...
    bool addBMPathToAtlas(GrMeshDrawOp::Target* target, FlushInfo* flushInfo,
        ...
        // add to atlas
        GrDrawOpAtlas::AtlasLocator atlasLocator;
        if (!this->addToAtlas(target, flushInfo, atlasMgr->atlas(), dst.width(), dst.height(), dst.addr(),
                              &atlasLocator)) {
            return false;
        }
    ...
    void flush(GrMeshDrawOp::Target* target, FlushInfo* flushInfo) const {
        ...
        GrDrawOpAtlas* atlas = atlasMgr->atlas();
        SkASSERT(atlas);
