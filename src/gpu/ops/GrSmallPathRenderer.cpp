...
private:
    ...
    bool addDFPathToAtlas(GrMeshDrawOp::Target* target, FlushInfo* flushInfo,
        ...
        if (!this->addToAtlas(target, flushInfo, atlas,
                              width, height, dfStorage.get(), &shapeData->fAtlasLocator)) {
            return false;
        }
    ...
    bool addBMPathToAtlas(GrMeshDrawOp::Target* target, FlushInfo* flushInfo,
        ...
        if (!this->addToAtlas(target, flushInfo, atlas, dst.width(), dst.height(),
                dst.addr(), &shapeData->fAtlasLocator)) {
            return false;
        }
    ...
    void writePathVertices(GrDrawOpAtlas* atlas,
        ...
        auto texCoords = GrVertexWriter::TriStripFromUVs(shapeData->fAtlasLocator.uvs1(
                                fUsesDistanceField ? SK_DistanceFieldInset : kAntiAliasPad));
