...
private:
    ...
    bool addDFPathToAtlas(GrMeshDrawOp::Target* target, FlushInfo* flushInfo,
        ...
        if (GrDrawOpAtlas::ErrorCode::kTryAgain == atlas->addToAtlas(resourceProvider, &id,
                                                                     uploadTarget, width, height,
                                                                     dfStorage.get(),
                                                                     &atlasLocation)) {
            this->flush(target, flushInfo);
            if (GrDrawOpAtlas::ErrorCode::kSucceeded != atlas->addToAtlas(resourceProvider, &id,
                                                                          uploadTarget,
                                                                          width, height,
                                                                          dfStorage.get(),
                                                                          &atlasLocation)) {
    ...
    bool addBMPathToAtlas(GrMeshDrawOp::Target* target, FlushInfo* flushInfo,
        ...
        if (GrDrawOpAtlas::ErrorCode::kTryAgain == atlas->addToAtlas(resourceProvider, &id,
            ...
            this->flush(target, flushInfo);
            if (GrDrawOpAtlas::ErrorCode::kSucceeded != atlas->addToAtlas(resourceProvider,
                                                                          &id, uploadTarget,
                                                                          dst.width(), dst.height(),
                                   dst.addr(), &atlasLocation)) {
                return false;
