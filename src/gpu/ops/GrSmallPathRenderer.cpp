...
public:
    ...
    static std::unique_ptr<GrDrawOp> Make(GrPaint&& paint, const GrShape& shape,
                                          const SkMatrix& viewMatrix, std::unique_ptr<GrDrawOpAtlas> atlas[kNumAtlases],
    ...
    SmallPathOp(Helper::MakeArgs helperArgs, GrColor color, const GrShape& shape,
                const SkMatrix& viewMatrix, std::unique_ptr<GrDrawOpAtlas> atlas[kNumAtlases], ShapeCache* shapeCache,
...
private:
    ...
    void onPrepareDraws(Target* target) override {
        ...
        if (fUsesDistanceField) {
            ...
            sk_sp<GrTextureProxy> proxies[kNumAtlases];
            for (int i = 0; i < kNumAtlases; ++i) {
                proxies[i] = fAtlas[i]->getProxy();
            }
...
//                SkDebugf("%d\n", i);
...
#ifdef MULTITEXTURE
        for (int i = 0; i < kVerticesPerQuad; i++) {
            float* indexPtr = (float*)(offset + sizeof(SkPoint) + sizeof(GrColor) +
                                   2 * sizeof(uint16_t) + i * vertexStride);
            *indexPtr = (float) shapeData->fAtlasIndex;
        }
#endif
    }
    ...
    Helper fHelper;
    GrDrawOpAtlas* fAtlas[kNumAtlases];
