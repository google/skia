...
bool GrSmallPathRenderer::onDrawPath(const DrawPathArgs& args) {
    ...
    if (!fAtlas) {
        const GrBackendFormat format = args.fContext->priv().caps()->getBackendFormatFromColorType(
...
GR_DRAW_OP_TEST_DEFINE(SmallPathOp) {
    ...
    if (context->priv().contextID() != gTestStruct.fContextID) {
        ...
        gTestStruct.reset();
        const GrBackendFormat format = context->priv().caps()->getBackendFormatFromColorType(
                GrColorType::kAlpha_8, GrRenderable::kNo);
