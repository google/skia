...
private:
    ...
    bool addDFPathToAtlas1(GrMeshDrawOp::Target* target, FlushInfo* flushInfo,
        ...
        auto uploadTarget = target->deferredUploadTarget();
        if (GrDrawOpAtlas::ErrorCode::kSucceeded != atlas->addToAtlas1(resourceProvider, &id, uploadTarget, width, height,
    ...
    bool addBMPathToAtlas1(GrMeshDrawOp::Target* target, FlushInfo* flushInfo,
        ...
        GrDrawOpAtlas::AtlasID id;
        auto uploadTarget = target->deferredUploadTarget();
        if (GrDrawOpAtlas::ErrorCode::kSucceeded != atlas->addToAtlas1(resourceProvider, &id, uploadTarget, dst.width(), dst.height(),
