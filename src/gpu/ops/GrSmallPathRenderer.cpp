...
bool GrSmallPathRenderer::onDrawPath(const DrawPathArgs& args) {
    ...
    if (!fAtlas[0]) {
        for (int i = 0; i < kNumAtlases; ++i) {
            fAtlas[i] = GrDrawOpAtlas::Make(args.fContext,
...
GR_DRAW_OP_TEST_DEFINE(SmallPathOp) {
    ...
    if (context->uniqueID() != gTestStruct.fContextID) {
        ...
        gTestStruct.reset();
        for (int i = 0; i < GrSmallPathRenderer::kNumAtlases; ++i) {
            gTestStruct.fAtlas[i] = GrDrawOpAtlas::Make(context, kAlpha_8_GrPixelConfig,
                                                        ATLAS_TEXTURE_WIDTH, ATLAS_TEXTURE_HEIGHT,
                                                        NUM_PLOTS_X, NUM_PLOTS_Y,
