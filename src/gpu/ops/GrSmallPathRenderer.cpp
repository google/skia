...
bool GrSmallPathRenderer::onDrawPath(const DrawPathArgs& args) {
    ...
    if (!fAtlas) {
        const GrBackendFormat format = args.fContext->defaultBackendFormat(kAlpha_8_SkColorType,
                                                                           GrRenderable::kNo);
...
GR_DRAW_OP_TEST_DEFINE(SmallPathOp) {
    ...
    if (context->priv().contextID() != gTestStruct.fContextID) {
        ...
        gTestStruct.reset();
        const GrBackendFormat format = context->defaultBackendFormat(kAlpha_8_SkColorType,
                                                                     GrRenderable::kNo);
