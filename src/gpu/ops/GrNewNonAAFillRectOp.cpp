...
#include "GrDrawOpOptionalPipelineInputs.h"
...
public:
    ...
    NewNonAAFillRectOp(GrPaint&& paint, const SkMatrix& viewMatrix, const SkRect& rect,
            ...
            : GrMeshDrawOp(ClassID()), fOptionals(std::move(paint), useHWAA, stencilSettings) {
    ...
    FixedFunctionFlags fixedFunctionFlags() const override {
        return fOptionals.fixedFunctionFlags();
    }
    ...
    bool xpRequiresDstTexture(const GrCaps& caps, const GrAppliedClip* clip) override {
        return fOptionals.xpRequiresDstTexture(caps, clip, &fRects.front().fColor);
    }
...
private:
    void onPrepareDraws(Target* target) const override {
        ...
        args.fFlags = fOptionals.pipelineFlags();
        args.fProcessors = &fOptionals.processors();
        args.fRenderTarget = target->renderTarget();
        args.fAppliedClip = target->clip();
        args.fDstTexture = target->dstTexture();
        args.fCaps = &target->caps();
        args.fUserStencil = fOptionals.stencilSettings();
        pipeline->init(args);
    ...
    bool onCombineIfPossible(GrOp* t, const GrCaps& caps) override {
        ...
        if (!fOptionals.compatible(that->fOptionals)) {
            return false;
        }
    ...
    GrDrawOpOptionalPipelineInputs<HAS_PROCESORS, USES_STENCIL> fOptionals;
...
std::unique_ptr<GrMeshDrawOp> Make(GrPaint&& paint,
    ...
    return GrMakeDrawOpWithOptionalInputs<NewNonAAFillRectOp>(
            std::move(paint), stencilSettings, viewMatrix, rect, localRect, localMatrix, useHWAA);
}
};  // namespace GrNewNonAAFillRectOp
