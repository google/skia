...
public:
    ...
    static std::unique_ptr<GrDrawOp> Make(const SkMatrix& viewMatrix, GrPaint&& paint, GrAAType aaType,
                                          GrPath* path) {
        return std::unique_ptr<GrDrawOp>(new GrDrawPathOp(viewMatrix, std::move(paint), aaType, path));
    }
...
public:
    ...
    static std::unique_ptr<GrDrawOp> Make(const SkMatrix& viewMatrix, SkScalar scale, SkScalar x,
        ...
        return std::unique_ptr<GrDrawOp>(new GrDrawPathRangeOp(
                viewMatrix, scale, x, y, std::move(paint), fill, aaType, range, instanceData, bounds));
    }
...
private:
    GrDrawPathRangeOp(const SkMatrix& viewMatrix, SkScalar scale, SkScalar x, SkScalar y,
                      GrPaint&& paint, GrPathRendering::FillType fill, GrAAType aaType, GrPathRange* range,
