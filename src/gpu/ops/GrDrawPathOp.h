...
protected:
    ...
    bool usesHWAAWhenAvailable() const override { return true; }
    ...
    bool willXPNeedDstTexture(const GrCaps& caps, const GrAppliedClip* clip) const override {
        ...
        return GrXPFactory::WillNeedDstTexture(fProcessorSet.xpFactory(), caps, fAnalysis);
    }
...
protected:
    ...
    const GrProcessorSet& processors() { return fProcessorSet; }
    ...
    void doFragmentProcessorAnalysis(const GrCaps& caps, const GrAppliedClip* clip) const {
...
public:
    ...
    static std::unique_ptr<GrDrawOp> Make(const SkMatrix& viewMatrix, GrPaint&& paint,
                                          GrPath* path) {
        return std::unique_ptr<GrDrawOp>(new GrDrawPathOp(viewMatrix, std::move(paint), path));
    }
...
private:
    GrDrawPathOp(const SkMatrix& viewMatrix, GrPaint&& paint, const GrPath* path)
            : GrDrawPathOpBase(ClassID(), viewMatrix, std::move(paint), path->getFillType()), fPath(path) {
...
public:
    ...
    static std::unique_ptr<GrDrawOp> Make(const SkMatrix& viewMatrix, SkScalar scale, SkScalar x,
                                          ...
                                          const SkRect& bounds) {
        return std::unique_ptr<GrDrawOp>(new GrDrawPathRangeOp(viewMatrix, scale, x, y, std::move(paint), fill,
...
private:
    GrDrawPathRangeOp(const SkMatrix& viewMatrix, SkScalar scale, SkScalar x, SkScalar y,
                      GrPaint&& paint, GrPathRendering::FillType fill, GrPathRange* range,
