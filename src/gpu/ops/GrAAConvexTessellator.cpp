...
bool GrAAConvexTessellator::extractFromPath(const SkMatrix& m, const SkPath& path) {
    ...
    m.mapRect(&bounds);
    if (!bounds.isFinite()) {
        // We could do something smarter here like clip the path to the bounds of the destination.
        // We'd have to be careful about stroke widths to ensure we don't change the appearance.
        return false;
    }
