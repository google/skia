...
bool GrAAConvexTessellator::extractFromPath(const SkMatrix& m, const SkPath& path) {
    ...
    if (this->numPts() >= 3) {
        ...
        fSide = (cross > 0.0f) ? fSide = SkPointPriv::kRight_Side : SkPointPriv::kLeft_Side;
        fNorms[0] = SkPointPriv::MakeOrthog(fNorms[0], fSide);
        fNorms[0].normalize();
        SkASSERT(SkScalarNearlyEqual(1.0f, fNorms[0].length()));
        for (int cur = 1; cur < fNorms.count() - 1; ++cur) {
            fNorms[cur] = SkPointPriv::MakeOrthog(fPts[cur + 1] - fPts[cur], fSide);
            fNorms[cur].normalize();
            SkASSERT(SkScalarNearlyEqual(1.0f, fNorms[cur].length()));
        }
        fNorms.top() = SkPointPriv::MakeOrthog(fNorms.top(), fSide);
        fNorms.top().normalize();
