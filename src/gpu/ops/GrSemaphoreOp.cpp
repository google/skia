...
public:
    ...
    static std::unique_ptr<GrSignalSemaphoreOp> Make(sk_sp<GrSemaphore> semaphore) {
        return std::unique_ptr<GrSignalSemaphoreOp>(new GrSignalSemaphoreOp(semaphore));
    }

    const char* name() const override { return "SignalSemaphore"; }

private:
    explicit GrSignalSemaphoreOp(sk_sp<GrSemaphore> semaphore) : INHERITED(ClassID(), semaphore) {}
...
public:
    ...
    static std::unique_ptr<GrWaitSemaphoreOp> Make(sk_sp<GrSemaphore> semaphore) {
        return std::unique_ptr<GrWaitSemaphoreOp>(new GrWaitSemaphoreOp(semaphore));
    }

    const char* name() const override { return "WaitSemaphore"; }

private:
    explicit GrWaitSemaphoreOp(sk_sp<GrSemaphore> semaphore) : INHERITED(ClassID(), semaphore) {}
...
std::unique_ptr<GrSemaphoreOp> GrSemaphoreOp::MakeSignal(sk_sp<GrSemaphore> semaphore) {
    return GrSignalSemaphoreOp::Make(semaphore);
}

std::unique_ptr<GrSemaphoreOp> GrSemaphoreOp::MakeWait(sk_sp<GrSemaphore> semaphore) {
    return GrWaitSemaphoreOp::Make(semaphore);
}
