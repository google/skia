...
public:
     ...
     * 'positions' array before application of 'viewMatrix'. If 'indices' is null then 'indexCnt'
     * must be zero and vice versa. In this case the vertices are indexed as 0, 1, ...,
     * 'vertexCount' - 1. 'localCoords' are optional and if null the vertex positions are used as
     * local coords. 'colorArrayType' specifies whether the colors are premul GrColors or unpremul
     * SkColors.
     */
    ...
    /**
     * The GrColor param is used if the vertices lack per-vertex color or 'flags' indicates that the
     * per-vertex color should be ignored.  The 'flags' options are those specified by
     * SkCanvas::VerticesFlags. If the vertices lack tex coords (local coords) or 'flags' indicates
