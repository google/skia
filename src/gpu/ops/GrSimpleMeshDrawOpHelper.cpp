...
const GrPipeline* GrSimpleMeshDrawOpHelper::CreatePipeline1(
                                                const GrCaps* caps,
                                                SkArenaAlloc* arena,
...
const GrPipeline* GrSimpleMeshDrawOpHelper::CreatePipeline(
                                                ...
                                                GrPipeline::InputFlags pipelineFlags,
                                                const GrUserStencilSettings* stencilSettings) {
    return CreatePipeline1(&flushState->caps(),
...
const GrPipeline* GrSimpleMeshDrawOpHelperWithStencil::createPipelineWithStencil(
        GrOpFlushState* flushState) {
    return GrSimpleMeshDrawOpHelper::CreatePipeline1(&flushState->caps(),
                                                     flushState->allocator(),
                                                     flushState->view(),
                                                     flushState->detachAppliedClip(),
                                                     flushState->dstProxyView(),
