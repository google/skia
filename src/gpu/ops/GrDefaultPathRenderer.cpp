...
public:
    ...
    void beginInstance() {
        SkDebugf("  beginInstance : ofs:%d ifs:%d\n", fVertexOffset, fIndexOffset);
    ...
    void endInstance() {
        ...
        SkDebugf("  endInstance   : ofs:%d ifs:%d\n", fVertexOffset, fIndexOffset);
...
private:
    ...
    void allocNewBuffers() {
        ...
        fVertices = static_cast<SkPoint*>(fTarget->makeVertexSpaceAtLeast(fVertexStride,
                                                                          ...
                                                                          &fFirstVertex,
                                                                          &fVerticesInChunk));
        SkDebugf("  allocNewBuffers verts:%p vb:%p fv:%d vic:%d\n", fVertices, fVertexBuffer, fFirstVertex, fVerticesInChunk);
        ...
        if (this->isIndexed()) {
            ...
            fIndices = fTarget->makeIndexSpaceAtLeast(kMinIndicesPerChunk, kFallbackIndicesPerChunk,
                                                      ...
                                                      &fIndicesInChunk);
            SkDebugf("  allocNewBuffers inds:%p ib:%p fi:%d iic:%d\n", fIndices, fIndexBuffer, fFirstIndex, fIndicesInChunk);
        }
        fVertexOffset = 0;
        fIndexOffset = 0;
    }
...
private:
    ...
    void onPrepareDraws(Target* target) const override {
        ...
        SkDebugf("  indexed:%d primType:%d\n", isIndexed ? 1 : 0, (int)primitiveType);
