...
public:
    ...
    void addLine(const SkPoint pts[]) {
        if (!this->needSpace(1, this->indexScale(), &pts[0])) {
            return;
        }
    ...
    void addQuad(const SkPoint pts[], SkScalar srcSpaceTolSqd, SkScalar srcSpaceTol) {
        if (!this->needSpace(GrPathUtils::kMaxPointsPerCurve,
...
private:
    ...
    bool needSpace(int vertsNeeded, int indicesNeeded = 0, const SkPoint* lastPoint = nullptr) {
        if (!fValid) {
            return false;
        }
