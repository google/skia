...
struct PathGeoBuilder {
    ...
    ~PathGeoBuilder() {
        // Put back any allocated but unused vertex/index data
    ...
    SkPoint* vertexBase() { return fVertices + fVertexOffset; }
    ...
    void beginInstance() {
        ...
        fFirstVerbInPath = true;
        fCurIdx = this->indexBase();
        fCurVert = this->vertexBase();
    }
    ...
    void addLine(const SkPoint& p) {
        ...
        if (this->isIndexed()) {
            ...
            append_countour_edge_indices(this->isHairline(), fSubpathIndexStart,
                                         prevIdx, &fCurIdx);
        }
        *(fCurVert++) = p;
    }
    ...
    void addQuad(const SkPoint pts[], SkScalar srcSpaceTolSqd, SkScalar srcSpaceTol) {
        this->needSpace(kMaxVerticesPerCurve, kMaxVerticesPerCurve * this->indexScale());

        add_quad(&fCurVert, this->vertexBase(), pts, srcSpaceTolSqd, srcSpaceTol, this->isIndexed(),
                 this->isHairline(), fSubpathIndexStart, fVertexOffset, &fCurIdx);
    }
...
private:
    ...
    void onPrepareDraws(Target* target) const override {
        ...
        // Our geometry creation code will keep re-allocating buffers and re-using this one mesh
        ...
        for (int i = 0; i < instanceCount; i++) {
            ...
            this->createGeom(pathGeoBuilder, args.fPath, args.fTolerance);
            pathGeoBuilder.endInstance();
        }

        pathGeoBuilder.emitMesh();
    }
    ...
    void createGeom(PathGeoBuilder& builder, const SkPath& path, SkScalar srcSpaceTol) const {
        ...
        while (!done) {
            ...
            switch (verb) {
                ...
                case SkPath::kLine_Verb:
                    builder.addLine(pts[1]);
                    break;
                case SkPath::kConic_Verb:
                    builder.addConic(iter.conicWeight(), pts, srcSpaceTolSqd, srcSpaceTol);
                    break;
                case SkPath::kQuad_Verb:
                    builder.addQuad(pts, srcSpaceTolSqd, srcSpaceTol);
                    break;
                case SkPath::kCubic_Verb:
                    builder.addCubic(pts, srcSpaceTolSqd, srcSpaceTol);
                    break;
            ...
            builder.fFirstVerbInPath = false;
        }
    }
