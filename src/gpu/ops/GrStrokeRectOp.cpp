...
static float compute_inner_coverage(SkScalar maxDevStrokeSize) {
    if (maxDevStrokeSize < SK_ScalarHalf) {
        int scale = SkScalarFloorToInt(512.0f * maxDevStrokeSize / (maxDevStrokeSize + SK_ScalarHalf));
...
void AAStrokeRectOp::generateAAStrokeRectGeometry(GrVertexWriter& vertices,
    ...
    const SkScalar insetX = SkTMin(SK_ScalarHalf, SkScalarHalf(devStrokeSize.fX));
    const SkScalar insetY = SkTMin(SK_ScalarHalf, SkScalarHalf(devStrokeSize.fY));
