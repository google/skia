...
private:
    ...
    bool compatibleWithCoverageAsAlpha(bool usesMSAASurface) const {
        // With MSAA we need to interpolate coverage values outside 0..1, which makes us no longer
        // compatible with coverage as alpha.
        return fHelper.compatibleWithCoverageAsAlpha() && !usesMSAASurface;
    }
...
void AAStrokeRectOp::generateAAStrokeRectGeometry(GrVertexWriter& vertices,
    ...
    if (usesMSAASurface) {
        // http://skbug.com/12206 -- there is a double-blend issue with the bevel version of this
        // op, and if we increase the AA bloat for MSAA it becomes more pronounced. Don't allow
        // bevels with MSAA.
        SkASSERT(miterStroke);
    ...
    if (!degenerate) {
        ...
        if (interiorAABoundary.fLeft > interiorAABoundary.fRight) {
            coverageBackset = (interiorAABoundary.fLeft - interiorAABoundary.fRight) / (interiorOutset * 2);
