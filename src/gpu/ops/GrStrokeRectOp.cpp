...
void AAStrokeRectOp::generateAAStrokeRectGeometry(GrVertexWriter& vertices,
    ...
    if (!degenerate) {
        ...
        if (coverageBackset > 0) {
            // The interior edges crossed. Lerp toward innerCoverage, which is what this op will
            // draw in the degenerate case. This gives a smooth transition into the degenerate case.
