...
static float compute_inner_coverage(SkScalar maxDevStrokeSize) {
    int scale;
    if (maxDevStrokeSize < SK_ScalarHalf) {
        scale = SkScalarFloorToInt(512.0f * maxDevStrokeSize / (maxDevStrokeSize + SK_ScalarHalf));
...
void AAStrokeRectOp::generateAAStrokeRectGeometry(GrVertexWriter& vertices,
    ...
    //    float fooX = devStrokeSize.fX < 1.0f ? devStrokeSize.fX/2.0f : 0.5f;
    //    float fooY = devStrokeSize.fY < 1.0f ? devStrokeSize.fY/2.0f : 0.5f;
    const float fooX = SkTMin(0.5f, devStrokeSize.fX/2.0f);
    const float fooY = SkTMin(0.5f, devStrokeSize.fY / 2.0f);

    const float barX = 1.0f - fooX;
    const float barY = 1.0f - fooY;

    // Outermost rect
    SkDebugf("outermost: ");
    vertices.writeQuad(inset_fan(devOutside, -barX, -barY),
                       outerColor,
                       maybe_coverage(0.0f));

    if (!miterStroke) {
        // Second outermost
        SkDebugf("outermost 2: ");
        vertices.writeQuad(inset_fan(devOutsideAssist, -barX, -barY),
                           outerColor,
                           maybe_coverage(0.0f));
    }

    float innerCoverage = compute_inner_coverage(SkTMax(devStrokeSize.fX, devStrokeSize.fY));
    ...
    if (!degenerate) {
        ...
        SkDebugf("innermost: ");
        vertices.writeQuad(inset_fan(devInside, barX, barY),
                           outerColor,
                           maybe_coverage(0.0f));
    } else {
