...
static void compute_aa_rects(SkRect* devOutside, SkRect* devOutsideAssist, SkRect* devInside,
                             ...
                             SkScalar strokeWidth, bool miterStroke, SkVector* off) {
    SkRect devRect;
    ...
    off->fX = devStrokeSize.fX;
    off->fY = devStrokeSize.fY;
...
public:
    ...
    AAStrokeRectOp(const Helper::MakeArgs& helperArgs, const SkPMColor4f& color,
        ...
        compute_aa_rects(&info.fDevOutside, &info.fDevOutsideAssist, &info.fDevInside,
                         &info.fDegenerate, viewMatrix, rect, stroke.getWidth(), isMiter, &info.fOff);
        info.fColor = color;
        if (isMiter) {
...
void AAStrokeRectOp::generateAAStrokeRectGeometry(GrVertexWriter& vertices,
    ...
    float innerCoverage = setup_scale(SkTMax(vec.fX, vec.fY));
    SkDebugf("innerCoverage %.2f\n", innerCoverage);
    ...
    SkDebugf("innerCoverage %.2f - %s 0x%x - %s 0x%x\n",
             innerCoverage,
             innerColor.fWideColor ? "yes" : "no",
             innerColor.fColor[0],
             outerColor.fWideColor ? "yes" : "no",
             outerColor.fColor[0]);
