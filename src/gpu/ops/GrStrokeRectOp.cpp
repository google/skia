...
static bool compute_aa_rects(const GrCaps& caps, SkRect* devOutside, SkRect* devOutsideAssist,
                             SkRect* devInside, bool* isDegenerate, const SkMatrix& viewMatrix,
                             const SkRect& rect, SkScalar strokeWidth, bool miterStroke,
    ...
    const SkRect visibilityBounds = SkRect::MakeWH(m, m).makeOutset(devStrokeSize.fX + 1,
                                                                    devStrokeSize.fY + 1);
...
public:
    ...
    static GrOp::Owner Make(GrRecordingContext* context,
        ...
        RectInfo info;
        if (!compute_aa_rects(*context->priv().caps(), &info.fDevOutside, &info.fDevOutsideAssist,
                              &info.fDevInside, &info.fDegenerate, viewMatrix, rect,
                              stroke.getWidth(), isMiter, &info.fDevHalfStrokeSize)) {
            return nullptr;
        }
