...
void AAStrokeRectOp::generateAAStrokeRectGeometry(GrVertexWriter& vertices,
    ...
    // For device-space stroke widths less than one we can't inset more than the original device space
    // stroke width if we want to keep the sizing of all the rects correct.
    const float insetX = SkTMin(0.5f, devStrokeSize.fX / 2.0f);
    const float insetY = SkTMin(0.5f, devStrokeSize.fY / 2.0f);

    // But, correspondingly we always want to keep the AA picture frame one pixel wide.
    const float outsetX = 1.0f - insetX;
    const float outsetY = 1.0f - insetY;
