...
GrDrawPathOpBase::GrDrawPathOpBase(uint32_t classID, const SkMatrix& viewMatrix, GrPaint&& paint,
        ...
        : INHERITED(classID), fViewMatrix(viewMatrix), fProcessorSet(std::move(paint)), fAnalysis(paint.getColor()), fFillType(fill), fAA(aa) {}
...
GrPipelineOptimizations GrDrawPathOpBase::initPipeline(const GrOpFlushState* state, const GrAppliedClip* clip,
                                     GrRenderTarget* renderTarget, GrPipeline* pipeline) {
    ...
    args.fAnalysis = &this->fragmentProcessorAnalysis(state->caps(), clip);

    return pipeline->init(args);
}

void GrDrawPathOp::onExecute(GrOpFlushState* state, const GrAppliedClip* clip, GrRenderTarget* renderTarget) {

    GrColor color = this->color();
    GrPipeline pipeline;
    GrPipelineOptimizations optimizations = this->initPipeline(state, clip, renderTarget, &pipeline);
    ...
    stencil.reset(GrPathRendering::GetStencilPassSettings(this->fillType()),
                  clip && clip->hasStencilClip(), renderTarget->renderTargetPriv().numStencilBits());
...
bool GrDrawPathRangeOp::onCombineIfPossible(GrOp* t, const GrCaps& caps, const GrAppliedClip* clip) {
    ...
    if (GrPathRendering::kWinding_FillType != this->fillType() ||
        GrPathRendering::kWinding_FillType != that->fillType() || this->xpReadsDst(caps, clip)) {
        return false;
    }
    SkASSERT(!that->xpReadsDst(caps, clip));
...
void GrDrawPathRangeOp::onExecute(GrOpFlushState* state, const GrAppliedClip* clip,
    ...
    GrPipeline pipeline;
    this->initPipeline(state, clip, renderTarget, &pipeline);
    GrStencilSettings stencil;
    bool stencilClip = clip && clip->hasStencilClip();
    stencil.reset(GrPathRendering::GetStencilPassSettings(this->fillType()), stencilClip,
