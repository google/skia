...
public:
    static std::unique_ptr<GrDrawOp> Make(GrRecordingContext* context,
        ...
        return Helper::FactoryHelper<FillRectOp>(context, std::move(paint), aaType, std::move(quad),
                                                 stencilSettings);
    }
...
std::unique_ptr<GrDrawOp> GrFillRectOp::MakeNonAARect(GrRecordingContext* context,
    ...
    return FillRectOp::Make(context, std::move(paint), GrAAType::kNone, std::move(quad), stencil);
}
...
std::unique_ptr<GrDrawOp> GrFillRectOp::MakeOp(GrRecordingContext* context,
    ...
    *numConsumed = 1;
    ...
    for (int i = 1; i < cnt; ++i) {
        ...
        if (!fillRects->addQuad(std::move(quad), quads[i].fColor, resolvedAA)) {
            break;
        }
...
GR_DRAW_OP_TEST_DEFINE(FillRectOp) {
    ...
    if (random->nextBool()) {
        if (random->nextBool()) {
            ...
            return GrFillRectOp::Make(context, std::move(paint), aaType,
                                      {GrQuad::MakeFromRect(rect, viewMatrix),
                                       GrQuad::MakeFromRect(rect, localMatrix), aaFlags}, stencil);
        } else {
            // Pass local rect directly
            SkRect localRect = GrTest::TestRect(random);
            return GrFillRectOp::Make(context, std::move(paint), aaType,
                                      {GrQuad::MakeFromRect(rect, viewMatrix),
                                       GrQuad(localRect), aaFlags}, stencil);
        }
    } else {
        // The simplest constructor
        return GrFillRectOp::Make(context, std::move(paint), aaType,
                                  {GrQuad::MakeFromRect(rect, viewMatrix),
                                   GrQuad(rect), aaFlags}, stencil);
    }
}
