...
private:
    ...
    friend std::unique_ptr<GrDrawOp> GrFillRectOp::MakeSet(GrContext* context, GrPaint&& paint,
            ...
            const SkMatrix& viewMatrix, const GrRenderTargetContext::QuadSetEntry quads[],
            int quadCount);
...
std::unique_ptr<GrDrawOp> MakeSet(GrContext* context, GrPaint&& paint, GrAAType aaType,
                                  const GrUserStencilSettings* stencilSettings,
                                  const SkMatrix& viewMatrix,
    ...
    if (random->nextBool()) {
        if (random->nextBool()) {
            if (random->nextBool()) {
                ...
                return GrFillRectOp::MakeSet(context, std::move(paint), aaType, stencil, viewMatrix,
                                             quads.begin(), quads.count());
            } else {
                // Single local matrix
                ...
                return GrFillRectOp::MakeWithLocalMatrix(context, std::move(paint), aaType, aaFlags,
                                                         stencil, viewMatrix, localMatrix, rect);
            }
        } else {
            // Pass local rect directly
            SkRect localRect = GrTest::TestRect(random);
            return GrFillRectOp::MakeWithLocalRect(context, std::move(paint), aaType, aaFlags,
                                                   stencil, viewMatrix, rect, localRect);
        }
    } else {
        // The simplest constructor
        return GrFillRectOp::Make(context, std::move(paint), aaType, aaFlags, stencil, viewMatrix,
                                  rect);
    }
}
