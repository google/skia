...
public:
    ...
    FillRectOp(Helper::MakeArgs args, SkPMColor4f paintColor, GrAAType aaType,
        ...
        int i = fDeviceQuads.push_back(deviceQuad, deviceQuadType);
        fDeviceQuads.metadata(i) = { paintColor, edgeFlags };
...
private:
    ...
    void onPrepareDraws(Target* target) override {
        ...
        for (int i = 0; i < this->quadCount(); ++i) {
            ...
            if (!fHelper.isTrivial()) {
                // While local is only initialized when not trivial, the earlier assert
                // makes sure that tessellation's vertex spec will not try to use local coordinates
                // in that situation.
                local = fLocalQuads[i];
            }
    ...
    CombineResult onCombineIfPossible(GrOp* t, const GrCaps& caps) override {
        ...
        // If the paints were compatible, solid color state should be the same
    ...
    void addQuad(const GrPerspQuad& deviceQuad, const GrPerspQuad& localQuad,
        ...
        int i = fDeviceQuads.push_back(deviceQuad, fDeviceQuads.quadType());
        fDeviceQuads.metadata(i) = { color, edgeAA };
        if (!fHelper.isTrivial()) {
            fLocalQuads.push_back(localQuad, localQuadType);
        }
    }
    ...
    // No metadata attached to the local quads, since this list may remain empty when local coords
    // are not needed.
