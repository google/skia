...
public:
    ...
    static sk_sp<GrGeometryProcessor> Make(int posDim, int localPosDim, GrAAType aa) {
        return sk_sp<QuadPerEdgeAAGeometryProcessor>(new QuadPerEdgeAAGeometryProcessor(
                posDim, localPosDim, aa));
    }
    ...
    GrGLSLPrimitiveProcessor* createGLSLInstance(const GrShaderCaps& caps) const override {
        ...
        private:
            void onEmitCode(EmitArgs& args, GrGPArgs* gpArgs) override {
                ...
                // The paint color should already have been converted to device space, so no need
                // append a color transform
                gp.fAttrs.emitColor(args, nullptr, "paintColor");
...
private:
    QuadPerEdgeAAGeometryProcessor(int posDim, int localPosDim, GrAAType aa)
            : INHERITED(kQuadPerEdgeAAGeometryProcessor_ClassID)
            , fAttrs(posDim, localPosDim, /* vertex color */ true, aa,
...
public:
    ...
    RequiresDstTexture finalize(const GrCaps& caps, const GrAppliedClip* clip) override {
        ...
        SkPMColor4f color = fQuads[0].color();
...
private:
    ...
    void tess(void* v, const GrGeometryProcessor* gp) const {
        ...
        for (int i = 0; i < fQuads.count(); ++i) {
            ...
            GrQuadPerEdgeAA::Tessellate<Vertex>(vertices, q.deviceQuad(), q.color().toBytes_RGBA(),
                                                q.srcQuad(), kEmptyDomain, q.aaFlags());
            vertices += 4;
        }
    }
   ...
   void onPrepareDraws(Target* target) override {
        ...
        sk_sp<GrGeometryProcessor> gp = QuadPerEdgeAAGeometryProcessor::Make(posDim, srcDim,
                                                                             fHelper.aaType());
...
#undef TESS_FN_AND_VERTEX_SIZE
    ...
    CombineResult onCombineIfPossible(GrOp* t, const GrCaps& caps) override {
        ...
        // FIXME: fHelper.isCompatible() returns false if the op's aatypes are different, which
        // means we can't lift a none type into a coverage op with unset edge flags. In the event
        // that an SkPaint produced the diverging aa types, this probably isn't too significant
        // since it's likely their processors are different too. But if there are per-edge-aa ops
        // where the center quads in a tileset are set to none, their overall op aa type is
        // normalized to none as well, which then prevents it from being merged with the surrounding
        // tiles, even though those likely do have the same processor set.
        if (!fHelper.isCompatible(that->fHelper, caps, this->bounds(), that->bounds())) {
            return CombineResult::kCannotCombine;
        }

        // If the processor sets are compatible, the two ops are always compatible; it just needs
        // to adjust the state of the op to be the more general types.
        ...
        if (that->fSrcQuadType > fSrcQuadType) {
            fSrcQuadType = that->fSrcQuadType;
        }
