...
private:
    ...
    bool addQuad(const GrQuad& deviceQuad, const GrQuad& localQuad,
        ...
        auto indexBufferOption = GrQuadPerEdgeAA::CalcIndexBufferOption(fHelper.aaType(),
                                                                        fQuads.count());
        SkASSERT(fQuads.count() <= GrQuadPerEdgeAA::QuadLimit(indexBufferOption));
#endif
...
std::unique_ptr<GrDrawOp> GrFillRectOp::MakeSet(GrRecordingContext* context,
                                                GrPaint&& paint,
                                                GrAAType aaType,
...
void GrFillRectOp::CreateFillRectOps(GrRenderTargetContext* rtc,
    ...
    while (numLeft) {
        ...
        std::unique_ptr<GrDrawOp> op = MakeSet(context, std::move(paint), aaType, viewMatrix,
