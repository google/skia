...
GR_DRAW_OP_TEST_DEFINE(NonAAFillRectOp) {
    ...
    const GrUserStencilSettings* stencil = GrGetRandomStencil(random);
    GrAAType aaType = (numColorSamples && random->nextBool()) ? GrAAType::kMSAA : GrAAType::kNone;
    return GrNonAAFillRectOp::Make(std::move(paint), viewMatrix, rect, hasLocalRect ? &localRect : nullptr, hasLocalMatrix ? &localMatrix : nullptr, aaType, stencil);
}
