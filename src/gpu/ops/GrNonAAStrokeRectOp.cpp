...
public:
    ...
    static std::unique_ptr<GrDrawOp> Make(GrPaint&& paint, const SkMatrix& viewMatrix,
                                                    ...
                                                    bool snapToPixelCenters, GrAAType aaType) {
    ...
    NonAAStrokeRectOp(const Helper::MakeArgs& helperArgs, GrColor color, const SkMatrix& viewMatrix,
                      const SkRect& rect, const SkStrokeRec& stroke, bool snapToPixelCenters, GrAAType aaType)
            : INHERITED(ClassID()), fHelper(helperArgs, aaType, snapToPixelCenters ? Helper::Flags::kSnapVerticesToPixelCenters : Helper::Flags::kNone) {
        fColor = color;
...
std::unique_ptr<GrDrawOp> MakeNonAAStroke(GrPaint&& paint,
                                          ...
                                          GrAAType aaType) {
    return NonAAStrokeRectOp::Make(std::move(paint), viewMatrix, rect, stroke, snapToPixelCenters, aaType);
}
}  // namespace GrRectOpFactory
...
GR_DRAW_OP_TEST_DEFINE(NonAAStrokeRectOp) {
    ...
    return NonAAStrokeRectOp::Make(std::move(paint), viewMatrix, rect, strokeRec,
                                     random->nextBool(), aaType);
}
