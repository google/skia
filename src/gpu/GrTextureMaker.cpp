...
sk_sp<GrTextureProxy> GrTextureMaker::onRefTextureProxyForParams(const GrSamplerState& params,
    ...
    if (this->width() > fContext3->priv().caps()->maxTextureSize() ||
    ...
    if (original) {
        if (!params.isRepeated() ||
            !GrGpu::IsACopyNeededForRepeatWrapMode(fContext3->priv().caps(), original.get(),
        ...
        if (!params.isRepeated() ||
            !GrGpu::IsACopyNeededForRepeatWrapMode(fContext3->priv().caps(), nullptr,
