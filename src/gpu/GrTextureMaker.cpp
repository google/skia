...
sk_sp<GrTextureProxy> GrTextureMaker::onRefTextureProxyForParams(const GrSamplerState& params,
    ...
    if (this->width() > fContext3->contextPriv().caps()->maxTextureSize() ||
    ...
    if (original) {
        if (!params.isRepeated() ||
            !GrGpu::IsACopyNeededForRepeatWrapMode(fContext3->contextPriv().caps(), original.get(),
        ...
        if (!params.isRepeated() ||
            !GrGpu::IsACopyNeededForRepeatWrapMode(fContext3->contextPriv().caps(), nullptr,
