...
sk_sp<GrRenderTask> GrCopyRenderTask::Make(GrSurfaceProxyView&& srcView,
                                           ...
                                           const GrCaps* caps) {
    SkASSERT(dstView.asSurfaceProxy());
    SkASSERT(srcView.asSurfaceProxy());
...
void GrCopyRenderTask::gatherProxyIntervals(GrResourceAllocator* alloc) const {
    ...
    alloc->addInterval(fSrcView.asSurfaceProxy(), alloc->curOp(), alloc->curOp(),
...
bool GrCopyRenderTask::onExecute(GrOpFlushState* flushState) {
    GrSurfaceProxy* dstProxy = fTargetView.asSurfaceProxy();
    GrSurfaceProxy* srcProxy = fSrcView.asSurfaceProxy();
