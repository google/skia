...
sk_sp<GrFragmentProcessor> GrTextureAdjuster::createFragmentProcessor(
    ...
    SkAutoTUnref<GrTexture> texture(this->refTextureSafeForParams(params, colorMode, nullptr));
    if (!texture) {
        return nullptr;
    }
...
sk_sp<GrFragmentProcessor> GrTextureMaker::createFragmentProcessor(
    ...
    SkAutoTUnref<GrTexture> texture(this->refTextureForParams(params, colorMode));
    if (!texture) {
        return nullptr;
    }
    SkRect domain;
...
GrTexture* GrTextureMaker::generateTextureForParams(const CopyParams& copyParams, bool willBeMipped,
                                                    SkDestinationSurfaceColorMode colorMode) {
    SkAutoTUnref<GrTexture> original(this->refOriginalTexture(willBeMipped, colorMode));
    if (!original) {
        return nullptr;
    }
