...
bool GrContext_Base::init(sk_sp<const GrCaps> caps) {
    ...
    // Note: We do not initialize the thread safe proxy at this level – only the direct context
    // performs that initialization in its `init`.
    return true;
}
...
const GrCaps* GrContext_Base::caps() const { return fThreadSafeProxy->priv().caps().get(); }
sk_sp<const GrCaps> GrContext_Base::refCaps() const { return fThreadSafeProxy->priv().caps(); }
...
const sk_sp<GrContextThreadSafeProxy>& GrContext_Base::threadSafeProxy() {
