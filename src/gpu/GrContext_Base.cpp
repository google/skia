...
GrBackendFormat GrContext_Base::compressedBackendFormat(SkImage::CompressionType compression) const {
    const GrCaps* caps = this->caps();

    GrBackendFormat format = caps->getBackendFormatFromCompressionType(compression);
    if (!format.isValid()) {
        return {};
    }
