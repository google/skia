...
private:
    ...
    private:
        ...
        public:
            Chain() = default;
            Chain(std::unique_ptr<GrOp>);
            Chain(Chain&&);
            Chain& operator=(Chain&& that);

            bool isEmpty() const { return !SkToBool(fHead); }
        ...
        std::tuple<Chain, Chain> TryConcat(Chain chainA, const DstProxy& dstProxyA,
                                           const GrAppliedClip* appliedClipA, Chain chainB,
                                           const DstProxy& dstProxyB,
                                           const GrAppliedClip* appliedClipB, const GrCaps& caps,
                                           GrOpMemoryPool* pool);
        Chain DoConcat(Chain chainA, Chain chainB, const GrCaps& caps, GrOpMemoryPool* pool);
