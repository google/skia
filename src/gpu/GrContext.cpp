...
bool GrContextPriv::writeSurfacePixels2(GrSurfaceContext* dst, int left, int top, int width,
    ...
    if (dstRTProxy &&
        ...
        auto tempCtx = this->drawingManager()->makeTextureContext(
                temp, dst->colorSpaceInfo().refColorSpace());
        if (!tempCtx) {
            return false;
        }
        ...
        GrPaint paint;
        paint.addColorTextureProcessor(std::move(temp), SkMatrix::I());
        paint.setPorterDuffXPFactory(SkBlendMode::kSrc);
    ...
    convert |= srcColorType != allowedColorType;

    if (!dst->colorSpaceInfo().colorSpace()) {
        // "Legacy" mode - no color space conversions.
        srcColorSpace = nullptr;
    }
    convert |= !SkColorSpace::Equals(srcColorSpace, dst->colorSpaceInfo().colorSpace());

    SkAutoMalloc tempSrc;
    if (convert) {
        ...
        tempSrc.reset(size);
        SkPixmap tempSrcPM(tempSrcII, tempSrc.get(), tempSrcII.minRowBytes());
        if (!src.readPixels(tempSrcPM)) {
            return false;
        }
        srcColorType = allowedColorType;
        buffer = tempSrcPM.addr();
        rowBytes = tempSrcPM.rowBytes();
        if (dstProxy->origin() == kBottomLeft_GrSurfaceOrigin) {
            std::unique_ptr<char[]> row(new char[rowBytes]);
            for (int y = 0; y < height / 2; ++y) {
                memcpy(row.get(), tempSrcPM.addr(0, y), rowBytes);
                memcpy(tempSrcPM.writable_addr(0, y), tempSrcPM.addr(0, height - 1 - y), rowBytes);
                memcpy(tempSrcPM.writable_addr(0, height - 1 - y), row.get(), rowBytes);
            }
        ...
        tempSrc.reset(trimRowBytes * height);
        char* dst = reinterpret_cast<char*>(tempSrc.get()) + trimRowBytes * (height - 1);
