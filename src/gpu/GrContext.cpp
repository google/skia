...
protected:
    bool init(sk_sp<GrGpu> gpu, const GrContextOptions& options) override {
        ...
        GrAtlasGlyphCache* atlasGlyphCache = this->contextPriv().getAtlasGlyphCache();
        GrProxyProvider* proxyProvider = this->contextPriv().proxyProvider();

        fFullAtlasManager = new GrAtlasManager(proxyProvider, atlasGlyphCache,
                                               options.fGlyphCacheTextureMaximumBytes,
                                               allowMultitexturing);
        this->contextPriv().addOnFlushCallbackObject(fFullAtlasManager);

        atlasGlyphCache->setGlyphSizeLimit(fFullAtlasManager->getGlyphSizeLimit());
        return true;
    }
...
bool GrContext::initCommon(const GrContextOptions& options) {
    ...
    if (gpu) {
        ...
        fResourceProvider = new GrResourceProvider(gpu, fResourceCache, &fSingleOwner,
    ...
    fAtlasGlyphCache = new GrAtlasGlyphCache;
