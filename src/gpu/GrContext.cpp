...
bool GrContextPriv::readSurfacePixels2(GrSurfaceContext* src, int left, int top, int width,
    ...
    if (convert) {
        ...
        auto tempII = SkImageInfo::Make(width, height, srcSkColorType,
                                        kPremul_SkAlphaType,
                                        src->colorSpaceInfo().refColorSpace());
        auto finalII = SkImageInfo::Make(width, height, dstSkColorType,
    ...
    if (!fContext->fGpu->readPixels(srcSurface, left, top, width, height, allowedColorType,
                                    buffer, rowBytes)) {
        return false;
    }
