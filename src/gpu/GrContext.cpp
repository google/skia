...
bool GrContextPriv::writeSurfacePixels2(GrSurfaceContext* dst, int left, int top, int width,
    ...
    bool flip = dstProxy->origin() != kTopLeft_GrSurfaceOrigin;
    convert += flip;
    ...
    if (convert) {
        ...
        SkPixmap src(SkImageInfo::Make(width, height, srcSkColorType, srcAlphaType,
                     ...
                     buffer, rowBytes);
        tempSrc.alloc(SkImageInfo::Make(width, height, dstSkColorType, kPremul_SkAlphaType,
