...
bool GrContextPriv::writeSurfacePixels(GrSurfaceContext* dst, int left, int top, int width,
    ...
    useLegacyPath = true;
#endif
    // Newly added color types/configs added only support the new code path.
    if (srcColorType == GrColorType::kRGB_888x || dst->asSurfaceProxy()->config() == kRGB_888_GrPixelConfig) {
        useLegacyPath = false;
    }
...
#endif
    // Newly added color types/configs added only support the new code path.
    if (dstColorType == GrColorType::kRGB_888x || src->asSurfaceProxy()->config() == kRGB_888_GrPixelConfig) {
        useLegacyPath = false;
    }
