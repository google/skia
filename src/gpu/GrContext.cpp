...
//////////////////////////////////////////////////////////////////////////////
GrBackendFormat GrContext::defaultBackendFormat(SkColorType skColorType) const {
    TRACE_EVENT0("skia.gpu", TRACE_FUNC);
    ...
    return caps->getBackendFormatFromColorType(grColorType);
}
...
GrBackendTexture GrContext::createBackendTexture(int width, int height,
    ...
    const GrBackendFormat format = this->defaultBackendFormat(skColorType);
    if (!format.isValid()) {
        return GrBackendTexture();
    }
...
GrBackendTexture GrContext::createBackendTexture(const SkSurfaceCharacterization& c) {
    ...
    const GrBackendFormat format = this->defaultBackendFormat(c.colorType());
    if (!format.isValid()) {
        return GrBackendTexture();
    }
...
GrBackendTexture GrContext::createBackendTexture(int width, int height,
    ...
    GrBackendFormat format = this->defaultBackendFormat(skColorType);
    if (!format.isValid()) {
        return GrBackendTexture();
    }
