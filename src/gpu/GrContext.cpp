...
bool GrContextPriv::writeSurfacePixels(GrSurfaceProxy* dstProxy, SkColorSpace* dstColorSpace,
    ...
    GrSurface* surface = dstProxy->instantiate1(fContext->resourceProvider());
    if (!surface) {
        return false;
    }
    ...
    if (tempProxy) {
        ...
        if (tempProxy) {
            ...
            GrTexture* texture = tempProxy->instantiate3(fContext->resourceProvider());
            if (!texture) {
                return false;
            }
            if (applyPremulToSrc) {
            ...
            sk_sp<GrRenderTargetContext> renderTargetContext(
                this->makeWrappedRenderTargetContext1(sk_ref_sp(renderTarget), nullptr));
...
bool GrContextPriv::readSurfacePixels(GrSurfaceProxy* srcProxy, SkColorSpace* srcColorSpace,
    ...
    GrSurface* src = srcProxy->instantiate1(fContext->resourceProvider());
    if (!src) {
        return false;
    }
    ...
    GrSurface* surfaceToRead = proxyToRead->instantiate1(fContext->resourceProvider());
    if (!surfaceToRead) {
        return false;
    }
...
sk_sp<GrRenderTargetContext> GrContextPriv::makeWrappedRenderTargetContext1(
    ...
    return this->drawingManager()->makeRenderTargetContext1(std::move(proxy),
                                                           std::move(colorSpace),
                                                           surfaceProps);
}
...
sk_sp<GrSurfaceContext> GrContextPriv::makeWrappedSurfaceContext1(sk_sp<GrSurface> surface) {
    ASSERT_SINGLE_OWNER_PRIV
...
sk_sp<GrRenderTargetContext> GrContext::makeRenderTargetContext(SkBackingFit fit,
    ...
    if (SkBackingFit::kExact == fit) {
        tex = this->resourceProvider()->createTexture4(desc, budgeted);
    } else {
...
sk_sp<GrRenderTargetContext> GrContext::makeDeferredRenderTargetContext(
    ...
    sk_sp<GrRenderTargetContext> renderTargetContext(
        fDrawingManager->makeRenderTargetContext1(std::move(rtp),
                                                 std::move(colorSpace),
                                                 surfaceProps));
