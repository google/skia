...
bool GrContextPriv::writeSurfacePixels(GrSurfaceProxy* srcProxy, SkColorSpace* dstColorSpace,
    ...
    sk_sp<GrSurface> surface(sk_ref_sp(srcProxy->instantiate(fContext->resourceProvider())));
    if (!surface) {
        return false;
    }
    ...
    if (!fContext->fGpu->getWritePixelsInfo(surface.get(), width, height, srcConfig,
    ...
    if (!tempProxy) {
        ...
        return fContext->fGpu->writePixels(surface.get(), left, top, width, height, srcConfig,
                                           buffer, rowBytes);
    }
    return true;
}
...
sk_sp<GrRenderTargetContext> GrContext::makeRenderTargetContext(SkBackingFit fit,
    ...
    if (SkBackingFit::kExact == fit) {
        tex = this->resourceProvider()->createTexture1(desc, budgeted);
    } else {
