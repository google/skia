...
bool GrContextPriv::writeSurfacePixels(GrSurfaceProxy* surface, SkColorSpace* dstColorSpace,
    ...
    ASSERT_OWNED_PROXY_PRIV(surface);
    SkASSERT(surface);
...
bool GrContextPriv::readSurfacePixels(GrSurfaceProxy* src, SkColorSpace* srcColorSpace,
                                      int left, int top, int width, int height,
    ...
    if (!fContext->fGpu->getReadPixelsInfo(src, width, height, rowBytes, dstConfig, &drawPreference,
                                           &tempDrawInfo)) {
        return false;
    }
    ...
    if (GrGpu::kNoDraw_DrawPreference != drawPreference) {
        ...
        if (tempRTC) {
            ...
            if (fp) {
                ...
                surfaceToRead = sk_ref_sp(drawnProxy->instantiate(fContext->resourceProvider()));
                left = 0;
                top = 0;
                didTempDraw = true;
            }
    ...
    if (didTempDraw) {
        this->flushSurfaceWrites(drawnProxy.get());
