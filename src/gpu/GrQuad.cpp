...
bool coords_form_rect(const float xs[4], const float ys[4]) {
    ...
    return (SkScalarNearlyEqual(xs[0], xs[1], kTol) && SkScalarNearlyEqual(xs[2], xs[3], kTol) &&
            SkScalarNearlyEqual(ys[0], ys[2], kTol) && SkScalarNearlyEqual(ys[1], ys[3], kTol)) ||
           (SkScalarNearlyEqual(xs[0], xs[2], kTol) && SkScalarNearlyEqual(xs[1], xs[3], kTol) &&
            SkScalarNearlyEqual(ys[0], ys[1], kTol) && SkScalarNearlyEqual(ys[2], ys[3], kTol));
}
