...
static void map_quad_general(const V4f& qx, const V4f& qy, const SkMatrix& m,
    ...
    *xs = skvx::mad(m.getScaleX(), qx, skvx::mad(m.getSkewX(), qy, m.getTranslateX()));
    *ys = skvx::mad(m.getSkewY(), qx, skvx::mad(m.getScaleY(), qy, m.getTranslateY()));
    if (m.hasPerspective()) {
        V4f w = skvx::mad(m.getPerspX(), qx,
                          skvx::mad(m.getPerspY(), qy, m.get(SkMatrix::kMPersp2)));
        if (ws) {
