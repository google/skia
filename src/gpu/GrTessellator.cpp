...
bool check_for_intersection(Edge* edge, Edge* other, EdgeList* activeEdges, Vertex** current,
    ...
    if (edge->intersect(*other, &p, &alpha) && p.isFinite()) {
        ...
        if (p == edge->fTop->fPoint || (c.sweep_lt(p, edge->fTop->fPoint) && !nearly_flat(c, edge))) {
            v = edge->fTop;
        }
        if (p == edge->fBottom->fPoint || (c.sweep_lt(edge->fBottom->fPoint, p) && !nearly_flat(c, edge))) {
            v = edge->fBottom;
        }
        if (p == other->fTop->fPoint || (c.sweep_lt(p, other->fTop->fPoint) && !nearly_flat(c, other))) {
            v = other->fTop;
        }
        if (p == other->fBottom->fPoint || (c.sweep_lt(other->fBottom->fPoint, p) && !nearly_flat(c, other))) {
            v = other->fBottom;
        }
        if (!v) {
