...
void fix_inverted_edges(Vertex* prev, Vertex* next, Edge* prevBisector, Edge* nextBisector, Edge* prevEdge, Comparator& c) {
    if (!prev || !next) {
        return;
    }
    int winding = c.sweep_lt(prev->fPoint, next->fPoint) ? 1 : -1;
...
void boundary_to_aa_mesh(EdgeList* boundary, VertexList* mesh, Comparator& c, SkChunkAlloc& alloc) {
    ...
    for (Edge* e = boundary->fHead; e != nullptr; e = e->fRight) {
        ...
        if (prevInner.intersect(inner, &innerPoint) &&
            ...
            fix_inverted_edges(innerVertices.fTail, innerVertex, prevBisector, bisector, prevEdge, c);
            fix_inverted_edges(outerVertices.fTail, outerVertex, prevBisector, bisector, prevEdge, c);
    ...
    fix_inverted_edges(innerVertices.fTail, innerVertices.fHead, prevBisector, bisector, prevEdge, c);
    fix_inverted_edges(outerVertices.fTail, outerVertices.fHead, prevBisector, bisector, prevEdge, c);
    do {
