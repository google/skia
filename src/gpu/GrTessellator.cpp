...
SkScalar quad_error_at(SkQuadCoeff quad, SkScalar t, SkScalar u) {
...
SkScalar cubic_error_at(SkCubicCoeff cubic, SkScalar t, SkScalar u) {
...
void append_quadratic_to_contour(const SkPoint pts[3], SkScalar toleranceSqd, VertexList* contour,
    ...
    for (;;) {
        u = 1.0f / nPoints;
        if (quad_error_at(quad, t, u) < toleranceSqd) {
            break;
        }
        nPoints++;
    }
...
void append_cubic_to_contour(const SkPoint pts[4], SkScalar toleranceSqd, VertexList* contour,
    ...
    for (;;) {
        u = 1.0f / nPoints;
        if ((nRoots < 1 || cubic_error_at(cubic, t[0], u) < toleranceSqd) &&
            (nRoots < 2 || cubic_error_at(cubic, t[1], u) < toleranceSqd) &&
            (nRoots < 3 || cubic_error_at(cubic, t[2], u) < toleranceSqd)) {
              break;
        }
        nPoints++;
    }
