...
void check_and_fix_inversions(Vertex* prev, Vertex* next, Edge* prevBisector, Edge* nextBisector, Edge* prevEdge, Comparator& c) {
    if (!prev || !next) {
        return;
    }
    int winding = c.sweep_lt(prev->fPoint, next->fPoint) ? 1 : -1;
...
void boundary_to_aa_mesh(EdgeList* boundary, VertexList* mesh, Comparator& c, SkChunkAlloc& alloc) {
    ...
    for (Edge* e = boundary->fHead; e != nullptr; e = e->fRight) {
        ...
        if (prevInner.intersect(inner, &innerPoint) &&
            ...
            check_and_fix_inversions(innerVertices.fTail, innerVertex, prevBisector, bisector, prevEdge, c);
            check_and_fix_inversions(outerVertices.fTail, outerVertex, prevBisector, bisector, prevEdge, c);
    ...
    Edge* bisector = new_edge(outerVertices.fHead, innerVertices.fHead, Edge::Type::kConnector, c, alloc);
    check_and_fix_inversions(innerVertices.fTail, innerVertices.fHead, prevBisector, bisector, prevEdge, c);
    check_and_fix_inversions(outerVertices.fTail, outerVertices.fHead, prevBisector, bisector, prevEdge, c);
    do {
