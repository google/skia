...
struct Comparator {
    enum class Type { kVertical, kHorizontal };
    Comparator(Type type) : fType(type) {}
    bool sweep_lt(const SkPoint& a, const SkPoint& b) {
        return fType == Type::kHorizontal ? sweep_lt_horiz(a, b) : sweep_lt_vert(a, b);
    }
    bool sweep_gt(const SkPoint& a, const SkPoint& b) {
        return fType == Type::kHorizontal ? sweep_gt_horiz(a, b) : sweep_gt_vert(a, b);
    }
    Type fType;
};
...
void merge_sort(VertexList* vertices) {
    ...
    VertexList first(vertices->fHead, slow);
    VertexList second(slow->fNext, vertices->fTail);
    first.fTail->fNext = second.fHead->fPrev = nullptr;

    merge_sort<sweep_lt>(&first);
    merge_sort<sweep_lt>(&second);

    vertices->fHead = vertices->fTail = nullptr;
    Vertex* a = first.fHead;
    Vertex* b = second.fHead;
    while (a && b) {
...
void sort_and_simplify(VertexList* vertices, Comparator& c, SkArenaAlloc& alloc) {
    ...
    if (c.fType == Comparator::Type::kHorizontal) {
...
Poly* contours_to_polys(Vertex** contours, int contourCnt, SkPath::FillType fillType,
                        ...
                        SkArenaAlloc& alloc) {
    Comparator c(pathBounds.width() > pathBounds.height() ? Comparator::Type::kHorizontal
                                                          : Comparator::Type::kVertical);
    VertexList mesh;
