...
void append_cubic_to_contour(const SkPoint pts[4], SkScalar toleranceSqd, VertexList* contour,
    ...
    for (;;) {
        u = 1.0f / nPoints;
        if ((nRoots < 1 || cubic_error_at(pts, t[0], u) < toleranceSqd) &&
            (nRoots < 2 || cubic_error_at(pts, t[1], u) < toleranceSqd) &&
            (nRoots < 3 || cubic_error_at(pts, t[2], u) < toleranceSqd)) {
              break;
        }
        nPoints++;
    }
