...
sk_sp<GrTexture> GrGpu::createTexture(const GrSurfaceDesc& origDesc, SkBudgeted budgeted,
    ...
    bool mustHaveDataForAllLevels = this->caps()->shouldInitializeTextures() &&
                                    !this->caps()->canClearTextureOnCreation();
    if (mipLevelCount) {
        int bpp = GrBytesPerPixel(desc.fConfig);
        if (!validate_levels(desc.fWidth, desc.fHeight, texels, mipLevelCount, bpp, this->caps(), mustHaveDataForAllLevels)) {
            return nullptr;
        }
