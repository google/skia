...
static bool validate_texel_levels(int w, int h, const GrMipLevel* texels, int mipLevelCount, int bpp,
                            const GrCaps* caps) {
...
sk_sp<GrTexture> GrGpu::createTexture(const GrSurfaceDesc& desc,
    ...
    if (tex) {
        ...
        if (texelLevelCount && texels[0].fPixels) {
            ...
            if (!this->writePixels(tex.get(), 0, 0, desc.fWidth, desc.fHeight, textureColorType, dataColorType, texels, texelLevelCount)) {
                return nullptr;
            }
