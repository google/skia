...
void GrGpu::resolveRenderTarget(GrRenderTarget* target, const SkIRect& resolveRect,
    ...
    auto nativeResolveRect = GrNativeRect::MakeRelativeTo(target->height(), resolveRect, origin);
    // HACK: Remove this if() before landing...
    if (resolveRect.right() != target->width() && resolveRect.bottom() != target->height()) {
