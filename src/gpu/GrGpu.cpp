...
sk_sp<GrTexture> GrGpu::createTexture(const GrSurfaceDesc& origDesc, GrSurfaceOrigin origin,
                                      SkBudgeted budgeted,
                                      const GrMipLevel texels[], int mipLevelCount) {
    ...
    sk_sp<GrTexture> tex = this->onCreateTexture(desc, origin, budgeted, texels, mipLevelCount);
    if (tex) {
...
sk_sp<GrTexture> GrGpu::createTexture(const GrSurfaceDesc& desc, GrSurfaceOrigin origin,
                                      SkBudgeted budgeted) {
    return this->createTexture(desc, origin, budgeted, nullptr, 0);
}
...
bool GrGpu::getWritePixelsInfo(GrSurfaceProxy* dstProxy, int width, int height,
    ...
    if (!this->onGetWritePixelsInfo(dstProxy, width, height, srcConfig, drawPreference,
                                    tempDrawInfo)) {
        return false;
    }

    // Check to see if we're going to request that the caller draw when drawing is not possible.
    if (!dstProxy->asRenderTargetProxy() ||
...
const GrGpu::MultisampleSpecs& GrGpu::queryMultisampleSpecs(const GrPipeline& pipeline) {
    ...
    this->onQueryMultisampleSpecs(rt, pipeline.origin(), stencil, &effectiveSampleCnt, &pattern);
