...
bool GrGpu::MipMapsAreCorrect(SkISize dimensions, const BackendTextureData* data, int numLevels) {
    ...
    for (int i = 0; i < numLevels; ++i) {
        if (levelDimensions != data->pixmap(0).dimensions()) {
            return false;
        }
        if (colorType != data->pixmap(0).colorType()) {
            return false;
        }
...
GrBackendTexture GrGpu::createBackendTexture(SkISize dimensions,
    ...
    auto ct = SkColorTypeToGrColorType(data->colorType());
    if (data &&  !caps->areColorTypeAndFormatCompatible(ct, format)) {
        return {};
    }
