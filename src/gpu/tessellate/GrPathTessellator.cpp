...
void GrPathOuterCurveTessellator::prepare(GrMeshDrawOp::Target* target, const SkRect& cullBounds,
    ...
    int minPatchAllocCount = maxTriangles + minCurveAllocCount;
    ...
    private:
        void chopAndWriteQuadratic(GrVertexChunkBuilder* chunker, const SkPoint p[3]) {
            ...
            this->writeTriangle(chunker, q[0], q[2], q[4]);  // Connect the two halves.
        }
        ...
        void chopAndWriteCubic(GrVertexChunkBuilder* chunker, const SkPoint p[4]) {
            ...
            this->writeTriangle(chunker, c[0], c[3], c[6]);  // Connect the two halves.
        }
...
void GrPathWedgeTessellator::prepare(GrMeshDrawOp::Target* target, const SkRect& cullBounds,
    ...
    private:
        void chopAndWriteQuadraticWedges(GrVertexChunkBuilder* chunker, const SkPoint p[3],
            ...
            for (int i = 0; i < 2; ++i) {
                if (fCullTest.areVisible3(q + i*2)) {
                    this->writeQuadraticWedge(chunker, q + i*2, midpoint);
                } else {
                    this->writeFlatWedge(chunker, q[i*2], q[i*2 + 2], midpoint);
                }
            }
        }
        ...
        void chopAndWriteConicWedges(GrVertexChunkBuilder* chunker, const SkConic& conic,
            ...
            for (int i = 0; i < 2; ++i) {
                if (fCullTest.areVisible3(halves[i].fPts)) {
                    this->writeConicWedge(chunker, halves[i].fPts, halves[i].fW, midpoint);
                } else {
                    this->writeFlatWedge(chunker, halves[i].fPts[0], halves[i].fPts[2], midpoint);
                }
            }
        }
        ...
        void chopAndWriteCubicWedges(GrVertexChunkBuilder* chunker, const SkPoint p[4],
            ...
            for (int i = 0; i < 2; ++i) {
                if (fCullTest.areVisible4(c + i*3)) {
                    this->writeCubicWedge(chunker, c + i*3, midpoint);
                } else {
                    this->writeFlatWedge(chunker, c[i*3], c[i*3 + 3], midpoint);
                }
            }
        }
