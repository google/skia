...
GrPathIndirectTessellator::GrPathIndirectTessellator(const SkMatrix& viewMatrix, const SkPath& path,
                                                     bool emitInnerFanTriangles)
        : fEmitInnerFanTriangles(emitInnerFanTriangles) {
...
void GrPathIndirectTessellator::prepare(GrMeshDrawOp::Target* target, const SkMatrix& viewMatrix,
    ...
    fInstanceCount = numTrianglesAtBeginningOfData;
    ...
    SkASSERT(fInstanceCount == numTrianglesAtBeginningOfData + fOuterCurveInstanceCount);
#endif

    vertexAlloc.unlock(fInstanceCount);
}
...
void GrPathHWOuterCurveTessellator::prepare(GrMeshDrawOp::Target* target, const SkMatrix& matrix,
                                            const SkPath& path) {
...
void GrPathHWWedgeTessellator::prepare(GrMeshDrawOp::Target* target, const SkMatrix& matrix,
                                       const SkPath& path) {
    SkASSERT(target->caps().shaderCaps()->tessellationSupport());
    SkASSERT(!fPatchBuffer);
    SkASSERT(fPatchVertexCount == 0);
