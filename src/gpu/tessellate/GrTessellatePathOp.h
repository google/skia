...
private:
    ...
    // These flags specify how the below fTriangleBuffer should be drawn (if any).
    //
    // If kStencil=1 and kFill=0:
    //
    //     We just stencil the triangles (and cubics) during the stencil step. The path gets filled
    //     later using a simple bounding box if needed.
    //
    // If kStencil=1 and kFill=1:
    //
    ...
    //
    // If kStencil=0 and kFill=1:
    //
    //     This means that fTriangleBuffer contains non-overlapping geometry that can be drawn
    ...
    enum class TriangleDrawFlags {
        kNone = 0,
        kStencil = 1,
        kFill = 2
    };

    GR_DECL_BITFIELD_CLASS_OPS_FRIENDS(TriangleDrawFlags);

    sk_sp<const GrBuffer> fTriangleBuffer;
    int fBaseTriangleVertex;
    int fTriangleVertexCount;
    TriangleDrawFlags fTriangleDrawFlags = TriangleDrawFlags::kNone;
