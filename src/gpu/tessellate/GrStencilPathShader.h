...
// Uses indirect (instanced) draws to triangulate and render standalone closed cubics. The caller
// must compute the log2 number of line segments that each cubic must be divided into on the CPU
// (a.k.a resolveLevel; see GrWangsFormula::cubic_log2/quadratic_log2), and then sort the instance
// buffer by resolveLevel for efficient batching of indirect draws.
...
public:
    ...
    // line segments?
    constexpr static uint32_t NumVerticesAtResolveLevel(int resolveLevel) {
    ...
    static GrDrawIndexedIndirectCommand MakeDrawCubicsCmd(int resolveLevel, uint32_t instanceCount,
        ...
        // line segments. Each index value corresponds to a parametric T value on the curve. The
        // triangles are arranged in "middle-out" order, so we can conveniently control the
        // resolveLevel by changing only the indexCount.
        return {NumVerticesAtResolveLevel(resolveLevel), instanceCount, 3, 0, baseInstance};
    }
    ...
    static GrDrawIndexedIndirectCommand MakeDrawTrianglesCmd(uint32_t instanceCount,
                                                             uint32_t baseInstance) {
        // baseIndex=0 has 3 special index values that emit points P0, P1, and P2 respectively.
        return {3, instanceCount, 0, 0, baseInstance};
    }

    // Returns the index buffer that should be bound when drawing with this shader.
    // (The vertex shader uses the raw index value directly, so there is no vertex buffer.)
