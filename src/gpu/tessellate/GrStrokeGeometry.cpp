...
void GrStrokeGeometry::quadraticTo(Verb leftJoinVerb, const SkPoint P[3], float maxCurvatureT) {
    ...
    if (numSegments <= 1) {
        SkDebugf("@@@@@> REENABLE JOINS!\n");
        // this->rotateTo(leftJoinVerb, normals[0], P[1], P[0]);
        this->lineTo(Verb::kInternalRoundJoin, P[2]);
        // this->rotateTo(Verb::kInternalRoundJoin, normals[1], P[1], P[2]);
        return;
    }
    ...
    if (isCurvatureTooStrong) {
        ...
        if (leftT > 0) {
            ...
            this->rotateTo(leftJoinVerb, normals[0], currQuadratic[0]*2 - currQuadratic[1]);
...
return;
        ...
        if (rightT < 1) {
            ...
            //                currQuadratic[3]*2 + currQuadratic[2]);
        }
        return;
    }
...
void GrStrokeGeometry::cubicTo(Verb leftJoinVerb, const SkPoint P[4], float maxCurvatureT,
    ...
    if (numSegments <= 1) {
        ...
        this->lineTo(leftJoinVerb, P[3]);
        // this->rotateTo(Verb::kInternalRoundJoin, normals[1], c2);
        return;
    }
    ...
    if (isCurvatureTooStrong) {
        ...
        if (leftT > 0) {
            ...
            SkPoint c = (ptsBuffer[1] == ptsBuffer[0]) ? ptsBuffer[2] : ptsBuffer[1];
            this->rotateTo(leftJoinVerb, normals[0], c - ptsBuffer[0] * 2);
        }
        ...
        if (rightT < 1) {
            ...
            SkPoint c = (currCubic[2] == currCubic[3]) ? currCubic[1] : currCubic[2];
            this->rotateTo(Verb::kInternalRoundJoin, normals[1], c);
        }
        return;
    }
...
void GrStrokeGeometry::rotateTo(Verb leftJoinVerb, SkVector normal, SkPoint controlPoint) {
    if (this->recordLeftJoinIfNotEmpty(leftJoinVerb, normal)) {
...
bool GrStrokeGeometry::recordLeftJoinIfNotEmpty(Verb joinVerb, SkVector nextNormal) {
    if (fPoints.count() <= fCurrContourFirstPtIdx) {
