...
GrStrokeGeometry::GrStrokeGeometry(GrMeshDrawOp::Target* target, SkTArray<VertexChunk>* chunks,
                                   int totalCombinedVerbCnt)
        : fMaxTessellationSegments(target->caps().shaderCaps()->maxTessellationSegments())
        , fTarget(target)
        , fVertexChunks(chunks) {
    this->allocVertexChunk((totalCombinedVerbCnt * 3) * kNumVerticesPerPatch);
}

GrStrokeGeometry::~GrStrokeGeometry() {
    fTarget->putBackVertices(fCurrChunkVertexCapacity - fVertexChunks->back().fVertexCount,
                             sizeof(SkPoint));
}

void GrStrokeGeometry::allocVertexChunk(int minVertexAllocCount) {
    VertexChunk* chunk = &fVertexChunks->push_back();
...
SkPoint* GrStrokeGeometry::reservePatch() {
    if (fVertexChunks->back().fVertexCount + kNumVerticesPerPatch > fCurrChunkVertexCapacity) {
    ...
    SkASSERT(fVertexChunks->back().fVertexCount + kNumVerticesPerPatch <= fCurrChunkVertexCapacity);
    SkPoint* patch = fCurrChunkVertexData + fVertexChunks->back().fVertexCount;
    fVertexChunks->back().fVertexCount += kNumVerticesPerPatch;
    return patch;
}
