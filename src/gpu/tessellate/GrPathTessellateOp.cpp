...
void GrPathTessellateOp::prePrepareStencilPipeline(const PrePrepareArgs& args) {
    if (fStencilPipeline) {
        return;
    }
    ...
    fStencilPipeline = args.fArena->make<GrPipeline>(
...
// NOTE: It's ok to not check the clip here because the previous stencil pass only wrote to samples
...
void GrPathTessellateOp::prePrepareFillTrianglesProgram(const PrePrepareArgs& args,
    ...
    this->prePrepareFillPipeline(args);
    ...
    if (fStencilTrianglesProgram) {
        ...
        SkASSERT(!fFillPipeline->hasStencilClip());
    ...
    fFillTrianglesProgram = GrPathShader::MakeProgramInfo(
            fillTriangleShader, args.fArena, args.fWriteView, fFillPipeline, *args.fDstProxyView,
...
void GrPathTessellateOp::prePrepareFillCubicHullsProgram(const PrePrepareArgs& args) {
    ...
    fFillPathProgram = GrPathShader::MakeProgramInfo(
            fillCubicHullsShader, args.fArena, args.fWriteView, fFillPipeline, *args.fDstProxyView,
...
void GrPathTessellateOp::prePrepareFillBoundingBoxProgram(const PrePrepareArgs& args) {
    ...
    auto* fillBoundingBoxShader = args.fArena->make<GrFillBoundingBoxShader>(fViewMatrix, fColor,
                                                                             fPath.getBounds());
    fFillPathProgram = GrPathShader::MakeProgramInfo(
            fillBoundingBoxShader, args.fArena, args.fWriteView, fFillPipeline, *args.fDstProxyView,
            args.fXferBarrierFlags, &kTestAndResetStencil, *args.fCaps);
}

void GrPathTessellateOp::prePrepareFillPipeline(const PrePrepareArgs& args) {
    ...
    fFillPipeline = GrSimpleMeshDrawOpHelper::CreatePipeline(
...
void GrPathTessellateOp::onPrepare(GrOpFlushState* flushState) {
    ...
    if (!fStencilPipeline && !fFillPipeline) {
...
void GrPathTessellateOp::drawCoverPass(GrOpFlushState* flushState) {
    if (fFillTrianglesProgram) {
        ...
        flushState->bindTextures(fFillTrianglesProgram->primProc(), nullptr, *fFillPipeline);
        flushState->bindBuffers(nullptr, nullptr, fTriangleBuffer);
        flushState->draw(fTriangleVertexCount, fBaseTriangleVertex);
        ...
        if (fCubicVertexCount > 0) {
            ...
            flushState->bindTextures(fFillPathProgram->primProc(), nullptr, *fFillPipeline);
