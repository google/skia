...
public:
    CpuTriangleAllocator(SkAutoTMalloc<char>* data) : fData(data) {}

    void* lock(size_t stride, int eagerCount) override {
        SkASSERT(!fData->get());
        SkASSERT(stride == sizeof(SkPoint));
        fData->reset(eagerCount * stride);
        return fData->get();
    }

    void unlock(int actualCount) override { SkASSERT(fData->get()); }

    const void* data() const { return fData; }

private:
    SkAutoTMalloc<char>* fData;
};
...
void GrPathTessellateOp::onPrePrepare(GrRecordingContext* context,
    ...
    CpuTriangleAllocator cpuTriangleAllocator(&fOffThreadInnerTriangulation);
    this->prePreparePrograms({context->priv().recordTimeAllocator(), writeView, clip, &dstProxyView,
...
void GrPathTessellateOp::onPrepare(GrOpFlushState* flushState) {
    ...
    if (fTriangleVertexCount != 0) {
        ...
        if (fOffThreadInnerTriangulation) {
            ...
            fOffThreadInnerTriangulation.reset();
        }
        if (fStencilCubicsProgram) {
