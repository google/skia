...
void GrStrokeIndirectTessellator::writeInstance(GrVertexWriter* vertexWriter, const SkPoint pts[4],
                                                SkPoint lastControlPoint, int numTotalEdges) {
    vertexWriter->writeArray(pts, 4);
    vertexWriter->write(lastControlPoint, float(numTotalEdges));
}

void GrStrokeIndirectTessellator::writeCircleInstance(GrVertexWriter* vertexWriter,
                                                      SkPoint center, int numEdgesForCircles) {
    // An empty stroke is a special case that denotes a circle, or 180-degree point stroke.
    vertexWriter->fill(center, 5);
    // Mark numTotalEdges negative so the shader assigns the least possible number of edges to its
    // (empty) preceding join.
    vertexWriter->write(-float(numEdgesForCircles));
}
...
void GrStrokeIndirectTessellator::prepare(GrMeshDrawOp::Target* target, const SkMatrix& viewMatrix,
    ...
    GrVertexWriter resolveLevelWriters[kMaxResolveLevel + 1];
    ...
    for (int i = 0; i <= kMaxResolveLevel; ++i) {
        if (fResolveLevelCounts[i]) {
            ...
            resolveLevelWriters[i] = baseWriter.makeOffset(instanceStride * currentInstanceIdx);
#ifdef SK_DEBUG
        } else {
            resolveLevelWriters[i] = {nullptr};
        }
        if (i > 0) {
    ...
    for (const SkPath& path : pathList) {
        ...
        while (iter.next()) {
            ...
            switch (verb) {
                case Verb::kCircle:
                    this->writeCircleInstance(&resolveLevelWriters[fResolveLevelForCircles], pts[0],
                ...
                case Verb::kContourFinished:
                    ...
                    if (firstCubic) {
                        ...
                        this->writeInstance(&resolveLevelWriters[firstResolveLevel], firstCubic,
                                            lastControlPoint,
                ...
                case Verb::kQuad:
                    ...
                    if (resolveLevel < 0) {
                        ...
                        this->writeCircleInstance(&resolveLevelWriters[fResolveLevelForCircles],
                                                  cusp,
                                                  numEdgesPerResolveLevel[fResolveLevelForCircles]);
                    } else {
                        ...
                        if (-resolveLevel & 1) {  // Are the chop points cusps?
                            for (int i = 1; i <= numChops; ++i) {
                                this->writeCircleInstance(
                                        &resolveLevelWriters[fResolveLevelForCircles], pts_[i*3],
                                        numEdgesPerResolveLevel[fResolveLevelForCircles]);
                            }
                        }
                        resolveLevel = *nextResolveLevel++;
            ...
            for (int i = 0;;) {
                if (!hasLastControlPoint) {
                    ...
                    this->writeInstance(&resolveLevelWriters[resolveLevel], pts_, lastControlPoint,
    ...
    auto finalWriter = resolveLevelWriters[kMaxResolveLevel];
    for (int i = kMaxResolveLevel - 1; i >= 0; --i) {
        if (resolveLevelWriters[i].isValid()) {
            SkASSERT(resolveLevelWriters[i] == endWriters[i]);
            if (!finalWriter.isValid()) {
                finalWriter = resolveLevelWriters[i];
            }
        }
    }
