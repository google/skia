...
void GrPathInnerTriangulateOp::prePreparePrograms(const GrPathShader::ProgramArgs& args,
    ...
    fFanPolys = fFanTriangulator->pathToPolys(&isLinear);
    ...
    if (!isLinear) {
        // Always use indirect draws for now. Our goal in this mode is to maximize GPU performance,
...
void GrPathInnerTriangulateOp::onPrepare(GrOpFlushState* flushState) {
    ...
    const GrInnerFanTriangulator::BreadcrumbTriangleList* breadcrumbList = nullptr;

    if (fFanPolys) {
        GrEagerDynamicVertexAllocator alloc(flushState, &fFanBuffer, &fBaseFanVertex);
        fFanVertexCount = fFanTriangulator->polysToTriangles(fFanPolys, &alloc, &breadcrumbList);
    }

    if (fTessellator) {
        SkASSERT(!fFanPolys || breadcrumbList);
        fTessellator->prepare(flushState, fViewMatrix, fPath, breadcrumbList);
    }
}
