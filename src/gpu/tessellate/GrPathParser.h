...
//
// The incoming patchData array must have allocated at least MaxWedgeVertices() elements.
int EmitCenterWedgePatches(const SkPath&, SkPoint* patchData);

// Returns the maximum number of vertices required to triangulate the given path's inner fan.
...
// collapsing all curves to lines.)
//
// This method works by recursively subdividing the path's verbs rather than emitting a linear
// triangle fan or strip. This can reduce the load on the rasterizer by a great deal on complex
// paths.
//
// Returns the number of vertices written to the array.
//
// The incoming vertexData array must have allocated at least MaxInnerPolygonVertices() elements.
