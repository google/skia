...
constexpr static float kMaxParametricSegments_pow4 = 64*64*64*64;  // 64^4

// Writes out strokes to the given instance chunk array, chopping if necessary so that all instances
// require 64 parametric segments or less. (We don't consider radial segments here. The tessellator
...
public:
    ...
    // that are required by the single instance we've written that needs the most segments.
