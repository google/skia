    ...
    for (auto verb : strokeGeometry.verbs()) {
        ...
        switch (verb) {
            ...
            case Verb::kRotate:
                ...
                scratchPts[1] = scratchPts[2] = pts[ptsIdx+1];
                cubicPts = scratchPts;
                ptsIdx += 2;
                break;
        ...
        if (pendingJoin != Verb::kEndContour) {
            ...
            cubicData[3] = cubicPts[0];
                float tan0 = std::atan2(-cubicData[1].fY + cubicData[0].fY,
                                        cubicData[1].fX - cubicData[0].fX);
                float tan1 = std::atan2(-cubicData[3].fY + cubicData[2].fY,
                                        cubicData[3].fX - cubicData[2].fX);
                SkDebugf("@@@> made join: %f -> %f\n", tan0 * 180/M_PI, tan1 * 180/M_PI);
            switch (pendingJoin) {
                case Verb::kBevelJoin:
