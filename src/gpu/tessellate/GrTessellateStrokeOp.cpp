...
GrTessellateStrokeOp::GrTessellateStrokeOp(const SkMatrix& viewMatrix, const SkPath& path,
    ...
    SkRect devBounds = fPathStrokes.head().fDevPath.getBounds();
    float inflationRadius = fPathStrokes.head().fDevStroke.getInflationRadius();
...
GrOp::CombineResult GrTessellateStrokeOp::onCombineIfPossible(GrOp* grOp,
    ...
    if (fColor != op->fColor || fViewMatrix != op->fViewMatrix || fAAType != op->fAAType ||
        fProcessors != op->fProcessors || this->getMiterLimit() != op->getMiterLimit()) {
    ...
    for (auto verb : strokeGeometry.verbs()) {
        ...
        switch (verb) {
            case Verb::kBeginPath:
                ...
                currStrokeRadius = (*pathStrokesIter).fDevStroke.getWidth() * .5f;
                ++pathStrokesIter;
                continue;
...
void GrTessellateStrokeOp::onExecute(GrOpFlushState* flushState, const SkRect& chainBounds) {
    ...
    GrTessellateStrokeShader strokeShader(fViewMatrix, this->getMiterLimit(), fColor);
