...
void GrTessellateStrokeOp::onPrepare(GrOpFlushState* flushState) {
    ...
    GrStrokeGeometry strokeGeometry(fDevPath.countPoints(), fDevPath.countVerbs());
    ...
    for (auto verb : strokeGeometry.verbs()) {
        ...
        switch (verb) {
            ...
            case Verb::kEndContour:
                ...
                if (pendingJoin == Verb::kEndContour) {
                    // Temporary hack for this adapter: Swap these so lastJoinControlPoint becomes
                    // firstJoinControlPoint next time around, which is what we need for any caps.
                    // This will go away once we ditch the weird verbs created by GrStrokeGeometry.
                    std::swap(patch[1], patch[2]);
                }
                ++i;
        ...
        if (verb != Verb::kRotate && verb != Verb::kEndContour && verb != Verb::kRoundCap) {
