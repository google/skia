...
public:
    ...
    GrDrawAtlasPathOp(int numRenderTargetSamples, sk_sp<GrTextureProxy> atlasProxy,
                      bool isInverseFill, const SkIRect& devIBounds,
                      const SkIPoint16& locationInAtlas, bool transposedInAtlas,
                      const SkMatrix& viewMatrix, GrPaint&& paint, const SkRect& drawBounds)
            : GrDrawOp(ClassID())
