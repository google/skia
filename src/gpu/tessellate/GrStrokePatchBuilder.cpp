...
GrStrokePatchBuilder::GrStrokePatchBuilder(GrMeshDrawOp::Target* target,
    ...
    switch (fStroke.getJoin()) {
        ...
        case SkPaint::kRound_Join:
            ...
            maxNumSegmentsInJoin = std::ceil(SK_ScalarPI * fNumRadialSegmentsPerRadian);
            break;
    }
    ...
    fSoloRoundJoinAlwaysFitsInPatch =
            (SK_ScalarPI * fNumRadialSegmentsPerRadian <= fMaxTessellationSegments);
...
void GrStrokePatchBuilder::lineTo(SkPoint pt, JoinType prevJoinType) {
    ...
    if (fMaxCombinedSegments_withJoin < 1 || prevJoinType == JoinType::kCusp) {
        // Either the stroke has extemely thick round joins and there aren't enough guaranteed
...
void GrStrokePatchBuilder::cubicToRaw(JoinType prevJoinType, const SkPoint pts[4]) {
    ...
    if (!fHasLastControlPoint) {
        ...
        // TODO: Consider deferring the first stroke until we know wether the contour will close.
