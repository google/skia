...
void GrStrokePatchBuilder::writeCaps() {
    if (!fHasPreviousSegment) {
        ...
        fFirstControlPoint = fStartPoint - SkPoint{1,0};
        fLastControlPoint = fStartPoint + SkPoint{1,0};
    ...
    switch (fCurrStrokeCapType) {
        ...
        case SkPaint::kRound_Cap:
            ...
            this->writeJoin(3, fStartPoint, fFirstControlPoint, fFirstControlPoint);
            this->writeJoin(3, fLastPoint, fLastControlPoint, fLastControlPoint);
            break;
        case SkPaint::kSquare_Cap:
            this->writeSquareCap(fStartPoint, fFirstControlPoint);
            this->writeSquareCap(fLastPoint, fLastControlPoint);
            break;
    }
}
...
void GrStrokePatchBuilder::rotateTo(float leftJoinType, const SkPoint& anchorPoint,
    ...
    // Effectively rotate the current normal by drawing a flat, 1-segment cubic. writeCubicSegment
    // automatically adds the necessary join, and the 1-segment cubic serves as a glue that
    // guarantees a water tight edge between the new join and the segment that comes after the
    // rotate.
...
void GrStrokePatchBuilder::close() {
    ...
    // Draw a line back to the beginning. (This will be discarded if fLastPoint == fStartPoint.)
    this->lineTo(fCurrStrokeJoinType, fLastPoint, fStartPoint);
    this->writeJoin(fCurrStrokeJoinType, fStartPoint, fLastControlPoint, fFirstControlPoint);
}
