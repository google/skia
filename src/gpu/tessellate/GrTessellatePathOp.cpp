...
void GrTessellatePathOp::onPrepare(GrOpFlushState* state) {
    ...
    if (cpuTessellationWork * 500 + (256 * 256) < gpuFragmentWork) {  // Don't try below 256x256.
        ...
        if ((fPathVertexCount = GrTriangulator::PathToTriangles(
            ...
            SkASSERT(!fCubicInstanceBuffer || fCubicInstanceCount == numCountedCurves);
            return;
        }
    }
...
void GrTessellatePathOp::prepareOuterCubics(GrOpFlushState* flushState, int numCountedCurves) {
    ...
    GrEagerDynamicVertexAllocator instanceAlloc(flushState, &fCubicInstanceBuffer,
                                                &fBaseCubicInstance);
    auto* instanceData = instanceAlloc.lock<std::array<SkPoint, 4>>(numCountedCurves);
    if (!instanceData) {
        return;
    }
    ...
    instanceAlloc.unlock(fCubicInstanceCount);
}
