...
void GrTessellatePathOp::prepareOuterCubics(GrMeshDrawOp::Target* target, int numCubics,
    ...
    this->prepareOuterCubicsWithTriangles(target, cubicData, /*numTrianglesAtBeginnigOfData=*/0,
...
void GrTessellatePathOp::prepareOuterCubicsWithTriangles(
        GrMeshDrawOp::Target* target, SkPoint* cubicData, int numTrianglesAtBeginnigOfData,
    ...
    if (resolveLevelCounter) {
        ...
        this->prepareIndirectDraws(target, *resolveLevelCounter, cubicData,
                                   numTrianglesAtBeginnigOfData, instanceLocations);
#ifdef SK_DEBUG
        memcpy(endLocations, instanceLocations + 1, kMaxResolveLevel * sizeof(SkPoint*));
        int totalInstanceCount = numTrianglesAtBeginnigOfData +
                                 resolveLevelCounter->totalCubicInstanceCount();
        endLocations[kMaxResolveLevel] = cubicData + totalInstanceCount * 4;
#endif
        fCubicVertexCount = numTrianglesAtBeginnigOfData * 4;
    } else {
        SkASSERT(target->caps().shaderCaps()->tessellationSupport());
        SkASSERT(!numTrianglesAtBeginnigOfData);
    ...
    SkASSERT(fCubicVertexCount == (numTrianglesAtBeginnigOfData + numCubics) * 4);
}

void GrTessellatePathOp::prepareIndirectDraws(GrMeshDrawOp::Target* target,
                                              const GrResolveLevelCounter& resolveLevelCounter,
                                              SkPoint* instanceData,
                                              int numTrianglesAtBeginnigOfData,
    ...
    if (numTrianglesAtBeginnigOfData) {
    ...
    if (numTrianglesAtBeginnigOfData) {
        ...
        indirectData[0] = GrMiddleOutCubicShader::MakeDrawTrianglesCmd(numTrianglesAtBeginnigOfData,
                                                                       baseInstance);
        indirectIdx = 1;
        runningInstanceCount = numTrianglesAtBeginnigOfData;
    }
    ...
    SkASSERT(runningInstanceCount == numTrianglesAtBeginnigOfData +
                                     resolveLevelCounter.totalCubicInstanceCount());
}
