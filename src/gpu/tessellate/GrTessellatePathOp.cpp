...
GR_MAKE_BITFIELD_CLASS_OPS(GrTessellatePathOp::TriangleDrawFlags);
...
void GrTessellatePathOp::onPrepare(GrOpFlushState* state) {
    ...
    if (cpuTessellationWork * 500 + (256 * 256) < gpuFragmentWork) {  // Don't try below 256x256.
        GrEagerDynamicVertexAllocator vertexAlloc(state, &fTriangleBuffer, &fBaseTriangleVertex);
        int numCountedCurves;
        // PathToTriangles(..kSimpleInnerPolygon..) will fail if the inner polygon is not simple.
        if ((fTriangleVertexCount = GrTriangulator::PathToTriangles(
                fPath, 0, SkRect::MakeEmpty(), &vertexAlloc,
            ...
            if (((Flags::kStencilOnly | Flags::kWireframe) & fFlags) ||
                ...
                fTriangleDrawFlags |= TriangleDrawFlags::kStencil;
            }
            if (!(Flags::kStencilOnly & fFlags)) {
                fTriangleDrawFlags |= TriangleDrawFlags::kFill;
            }
...
void GrTessellatePathOp::prepareOuterCubics(GrOpFlushState* flushState, int numCountedCurves,
    ...
    int stride = (instanceAligned) ? sizeof(SkPoint) * 4 : sizeof(SkPoint);
    int vertexOrInstanceCount = (instanceAligned) ? numCountedCurves : numCountedCurves * 4;
    int baseCubic;

    auto* cubicVertexData = static_cast<SkPoint*>(flushState->makeVertexSpace(
            stride, vertexOrInstanceCount, &fCubicBuffer, &baseCubic));
    if (!cubicVertexData) {
        return;
    }
    fBaseCubicVertex = (instanceAligned) ? baseCubic * 4 : baseCubic;
    fCubicVertexCount = 0;
    ...
    while ((verb = iter.next(pts)) != SkPath::kDone_Verb) {
        if (SkPath::kQuad_Verb == verb) {
            ...
            write_quadratic_as_cubic(&cubicVertexData[fCubicVertexCount], pts[0], pts[1], pts[2]);
            fCubicVertexCount += 4;
            continue;
        }
        if (SkPath::kCubic_Verb == verb) {
            SkASSERT(fCubicVertexCount < numCountedCurves * 4);
            memcpy(cubicVertexData + fCubicVertexCount, pts, sizeof(SkPoint) * 4);
            fCubicVertexCount += 4;
            continue;
        }
    }
...
void GrTessellatePathOp::drawStencilPass(GrOpFlushState* state) {
    ...
    if (fTriangleDrawFlags & TriangleDrawFlags::kStencil) {
        SkASSERT(fTriangleBuffer);
...
void GrTessellatePathOp::drawCoverPass(GrOpFlushState* state) {
    ...
    if (fTriangleDrawFlags & TriangleDrawFlags::kFill) {
        SkASSERT(fTriangleBuffer);
        ...
        if (fStencilCubicsShader) {
            ...
            // the base vertex on an instance boundary in order to accomodate this.
