...
void GrTessellatePathOp::onPrepare(GrOpFlushState* state) {
    ...
    float cpuTessellationWork = (float)numVerbs * (32 - SkCLZ(numVerbs));  // N log N.
    if (cpuTessellationWork * 500 < gpuFragmentWork) {
        bool pathIsLinear;
        ...
        if ((fPathVertexCount = GrTessellator::PathToTriangles(
            ...
            if (((Flags::kStencilOnly | Flags::kWireframe) & fFlags) ||
                GrAAType::kCoverage == fAAType || state->appliedClip()->hasStencilClip()) {
            ...
            if (!(Flags::kStencilOnly & fFlags)) {
                fFillPathShader = state->allocator()->make<GrFillTriangleShader>(
                        fViewMatrix, SkPMColor4f{fColor.fB, fColor.fG, fColor.fR, fColor.fA});
            }
            if (!pathIsLinear) {
