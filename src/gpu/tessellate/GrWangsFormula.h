...
inline static float length(const Sk2f& n) {
    Sk2f nn = n*n;
...
inline static float quadratic(float intolerance, const SkPoint pts[]) {
...
// "1/intolerance" pixels from the true curve.
inline static float cubic(float intolerance, const SkPoint pts[]) {
...
//     nextlog2((4..8]) -> 3
//     ...
inline static uint8_t nextlog2(float value) {
    int32_t bits;
    memcpy(&bits, &value, 4);
    bits += (1 << 23) - 1;  // Increment the exponent for non-powers-of-2.
    int exp = (bits >> 23) - 127;
    return exp & ~(exp >> 31);  // Return 0 for denormalized, negative numbers, and exponents < 0.
}
...
inline static uint8_t quadratic_log2(float intolerance, const SkPoint pts[],
...
// "1/intolerance" pixels from the true curve.
inline static uint8_t cubic_log2(float intolerance, const SkPoint pts[],
                                 const GrVectorXform& vectorXform = GrVectorXform()) {
