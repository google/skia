...
public:
    ...
    explicit VectorXform(const SkMatrix&);
...
inline VectorXform::VectorXform(const SkMatrix& m) {
    SkASSERT(!m.hasPerspective());
    if (m.getType() & SkMatrix::kAffine_Mask) {
        fType = Type::kAffine;
        fScaleXY_or_ScaleXSkewY = {m.getScaleX(), m.getSkewY()};
        ...
        fType = Type::kScale;
        fScaleXY_or_ScaleXSkewY = {m.getScaleX(), m.getScaleY()};
        fScaleXYXY = {m.getScaleX(), m.getScaleY(), m.getScaleX(), m.getScaleY()};
    } else {
...
inline Sk2f VectorXform::operator()(const Sk2f& vector) const {
    switch (fType) {
        case Type::kIdentity:
            return vector;
        case Type::kScale:
            return fScaleXY_or_ScaleXSkewY * vector;
        case Type::kAffine:
            return fScaleXY_or_ScaleXSkewY * vector[0] + fSkewXScaleY * vector[1];
    }
    SkUNREACHABLE;
}

inline Sk4f VectorXform::operator()(const Sk4f& vectors) const {
    switch (fType) {
        case Type::kIdentity:
            return vectors;
        case Type::kScale:
