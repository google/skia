...
bool GrGpuTessellationPathRenderer::onDrawPath(const DrawPathArgs& args) {
    ...
    if (this->tryAddPathToAtlas(
            *args.fViewMatrix, path, &devIBounds, args.fAAType, &devToAtlasOffset)) {
...
bool GrGpuTessellationPathRenderer::tryAddPathToAtlas(
        const SkMatrix& viewMatrix, const SkPath& path, SkIRect* devIBounds, GrAAType aaType,
        SkIVector* devToAtlasOffset) {
    ...
    SkPathPriv::Concatenate(uberPath, path, atlasMatrix);
    return true;
}
