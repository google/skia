...
GrFPResult GrTessellationPathRenderer::makeAtlasClipFP(
    ...
    // Allow for padding so GrTextureEffect doesn't enforce the subset unnecessarily.
    atlasSubset.outset(GrDynamicAtlas::kPadding, GrDynamicAtlas::kPadding);
#ifdef SK_DEBUG
    if (!path.isInverseFillType()) {
        ...
        // that ever changes.
        SkASSERT(atlasSubset.contains(atlasDomain));
    }
#endif
...
void GrTessellationPathRenderer::AtlasPathKey::set(const SkMatrix& m, bool antialias,
    ...
    float2 subpixelPositionKey = skvx::trunc(subpixelPosition * kLinearizationPrecision);
...
bool GrTessellationPathRenderer::tryAddPathToAtlas(const GrCaps& caps, const SkMatrix& viewMatrix,
    ...
    *transposedInAtlas = minDimension > maxDimenstion;
    ...
    // Remeber this path's location in the atlas, in case it gets drawn again.
    if (!path.isVolatile()) {
