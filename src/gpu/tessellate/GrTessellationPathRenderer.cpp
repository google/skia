...
void GrTessellationPathRenderer::preFlush(GrOnFlushResourceProvider* onFlushRP,
    ...
    SkDebugf("%i x %i  atlas!\n", fAtlasRenderTasks[0]->atlasProxy()->backingStoreDimensions().fWidth,  fAtlasRenderTasks[0]->atlasProxy()->backingStoreDimensions().fHeight);
    ...
    for (int i = 1; i < fAtlasRenderTasks.count(); ++i) {
        ...
        SkDebugf("%i x %i  atlas!", atlasTask->atlasProxy()->backingStoreDimensions().fWidth,  atlasTask->atlasProxy()->backingStoreDimensions().fHeight);
        if (atlasTask->atlasProxy()->backingStoreDimensions() == firstAtlasTexture->dimensions()) {
            ...
            SkASSERT(atlasTask->atlasProxy()->backingStoreDimensions().area() < firstAtlasTexture->dimensions().area());
            // TODO: Reuse the larger atlas texture anyway?
