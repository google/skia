...
void GrTessellationPathRenderer::initAtlasFlags(const GrShaderCaps& shaderCaps) {
    constexpr static int kIntolerance = GrTessellationPathRenderer::kLinearizationIntolerance;
    fStencilAtlasFlags = OpFlags::kStencilOnly | OpFlags::kDisableHWTessellation;
    fMaxAtlasPathWidth = kMaxAtlasSize / 2;
    ...
    //
    //     GrWangsFormula::worst_case_cubic(kIntolerance, w, kMaxAtlasPathHeight^2 / w)
    //              == maxTessellationSegments
    //
    float k = GrWangsFormula::cubic_k(kIntolerance);
    ...
    SkASSERT(SkScalarNearlyEqual(
            GrWangsFormula::worst_case_cubic(kIntolerance, worstCaseWidth, worstCaseHeight), s, 1));
#endif
...
bool GrTessellationPathRenderer::onDrawPath(const DrawPathArgs& args) {
    ...
    // Find the log2 worst case number of line segments that a curve in this path might need to be
    // divided into.
    ...
    if (worstCaseResolveLevel > kMaxResolveLevel) {
        ...
        if (!Op(viewport, path, kIntersect_SkPathOp, &path)) {
            // The crop can fail if the PathOps encounter NaN or infinities. Still return true
