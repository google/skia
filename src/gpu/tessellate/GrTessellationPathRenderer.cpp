...
GrPathRenderer::CanDrawPath GrTessellationPathRenderer::onCanDrawPath(
    ...
    if (shape.style().strokeRec().getStyle() != SkStrokeRec::kStroke_Style) {
        ...
        // tessellation, we find that the default path renderer can render faster sometimes. Let
        // paths fall through to the default renderer on these platforms for now.
