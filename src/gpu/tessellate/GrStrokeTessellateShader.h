...
public:
    ...
    static float GetJoinType(const SkStrokeRec& stroke) {
        switch (stroke.getJoin()) {
            ...
            case SkPaint::kMiter_Join: return std::max(stroke.getMiter(), 0.f);
        }
        SkUNREACHABLE;
    }
    ...
    struct DynamicStroke {
        ...
        float fRadius;
        float fJoinType;  // fJoin<0 => round. fJoin==0 => bevel. fJoin>0 => fJoin==miterLimit.
    };
    ...
    GrStrokeTessellateShader(Mode mode, const SkMatrix& viewMatrix, const SkStrokeRec& stroke,
                             SkPMColor4f color, ShaderFlags shaderFlags)
