...
public:
    ...
    struct Tolerances {
        ...
        template<int N>
        SK_ALWAYS_INLINE static grvx::vec<N> CalcNumRadialSegmentsPerRadian(
                float parametricIntolerance, grvx::vec<N> strokeWidths) {
            return .5f /
                   grvx::approx_acos(skvx::max(1 - 2 / (parametricIntolerance * strokeWidths), -1));
        }
        SK_ALWAYS_INLINE static float CalcNumRadialSegmentsPerRadian(float parametricIntolerance,
                                                                     float strokeWidth) {
            return CalcNumRadialSegmentsPerRadian<1>(parametricIntolerance, strokeWidth).val;
        }
