...
// converts closes and square caps to lines, and round caps to "cusps" so the user doesn't have to
// worry about it. At each location it also provides a "prevVerb" and "nextVerb" so there is context
// about the join in between. Usage:
//
//     GrStrokeIterator iter(path, stroke);
//     while (iter.loadNext()) {  // Call loadNext() first.
//         iter.prevVerb();
//         iter.prevPts();
//         iter.nextVerb();
//         iter.nextPts();
...
public:
    ...
    enum class Verb {
        ...
        kCusp,  // A "cusp" means a circle drawn as a 180-degree point stroke.
    ...
    // Must be called first. Loads the next pair of "prev" and "next" strokes. Returns false if
...
private:
    ...
    // Finishes the current contour without closing it by enqueuing any necessary caps as well as
    // the contour's first stroke that was deferred.
        ...
        if (fQueueCount) {
            switch (fCapType) {
                ...
                case SkPaint::kRound_Cap: {
                    ...
                    this->enqueue(Verb::kCusp, fFirstPtsInContour);
                    break;
                }
            ...
            switch (fCapType) {
                ...
                case SkPaint::kRound_Cap:
                    ...
                    fFirstVerbInContour = Verb::kCusp;
    ...
    // We implement square caps as two additional "kLine" verbs. This method finds the endpoints for
    // those lines.
