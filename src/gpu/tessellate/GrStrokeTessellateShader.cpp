...
private:
    ...
    void setData(const GrGLSLProgramDataManager& pdman,
        ...
        if (!stroke.isHairlineStyle()) {
            ...
            // In the hairline case we tessellate after transformation. Set up tolerances for an
...
SkString GrStrokeTessellateShader::getTessEvaluationShaderGLSL(
    ...
    if (!this->viewMatrix().isIdentity()) {
        ...
        if (!fStroke.isHairlineStyle()) {
            // In the normal case we need the affine matrix also. (In the hairline case we already
