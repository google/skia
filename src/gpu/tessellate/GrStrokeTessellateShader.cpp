...
class GrStrokeTessellateShader::IndirectImpl : public GrGLSLGeometryProcessor {
    void onEmitCode(EmitArgs& args, GrGPArgs* gpArgs) override {
        ...
        const char* tessControlArgsName;
        ...
        if (tan0 == float2(0)) {
            // The stroke is a point. This special case tells us to draw a 180 degree cusp instead.
            tan0 = float2(1,0);
            tan1 = float2(-1,0);
        })");
        ...
        if (shader.fStroke.getJoin() == SkPaint::kRound_Join) {
            args.fVertBuilder->codeAppend(R"(
            // Determine how many edges to give to the join. We emit the first and final edges of
            // the join twice: once full width and once restricted to half width. This guarantees
            ...
            // The stroke needs at least two edges. Don't assign more to the join than
            // "numTotalEdges - 2".
        ...
        if (combinedEdgeID < 0) {
            // We belong to the preceeding join. The first and final edges get duplicated, so we
            // only have "numEdgesInJoin - 2" segments.
