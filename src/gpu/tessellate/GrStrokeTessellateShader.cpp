...
private:
    void onEmitCode(EmitArgs& args, GrGPArgs* gpArgs) override {
        ...
        if (shader.hasDynamicStroke()) {
            v->codeAppend("vsStrokeArgs = float2(NUM_RADIAL_SEGMENTS_PER_RADIAN, STROKE_RADIUS);");
        }
...
SkString GrStrokeTessellateShader::getTessControlShaderGLSL(
    ...
    if (!this->hasDynamicStroke()) {
        ...
        code.append("in vec2 vsStrokeArgs[];\n");
        code.appendf("#define NUM_RADIAL_SEGMENTS_PER_RADIAN vsStrokeArgs[0].x\n");
        code.appendf("#define STROKE_RADIUS vsStrokeArgs[0].y\n");
    }
    ...
    if (this->hasDynamicStroke()) {
        code.append("patch out float tcsStrokeRadius;\n");
    }
    ...
    if (this->hasDynamicStroke()) {
        code.append("tcsStrokeRadius = STROKE_RADIUS;\n");
    }
...
SkString GrStrokeTessellateShader::getTessEvaluationShaderGLSL(
    ...
    if (!this->hasDynamicStroke()) {
        ...
        code.append("patch in float tcsStrokeRadius;\n");
...
void GrStrokeTessellateShader::getGLSLProcessorKey(const GrShaderCaps&,
    ...
    key <<= 2;
    if (fMode == Mode::kIndirect && !(fShaderFlags & ShaderFlags::kDynamicStroke)) {
        key |= fStroke.getJoin();
    }
