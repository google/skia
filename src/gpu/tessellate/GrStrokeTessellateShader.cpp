...
private:
    void onEmitCode(EmitArgs& args, GrGPArgs* gpArgs) override {
        ...
        // Find the cubic's inflections. There are inflections where F' x F'' == 0.
        ...
        if (float3(a,b,c) == float3(0)) {
            // The curve is a flat line. Search for turnaround cusp points instead.
            a = dot(A, tan0);
            b = 2*dot(B, tan0);
            c = dot(C, tan0);
            ...
            // The curve does not inflect. This means it might rotate more than 180 degrees instead. Craft a
            // quadratic whose roots are the points were rotation == 180 deg and 0.
            // Rotation is 180 at -2*c/b.
            a = b;
            b = 2*c;
            c = 0;
            discr = b*b;
...
SkString GrStrokeTessellateShader::getTessControlShaderGLSL(
    ...
    code.appendf("#define cross(A,B) determinant(mat2(A,B))\n");
