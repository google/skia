...
private:
    void onEmitCode(EmitArgs& args, GrGPArgs* gpArgs) override {
        ...
        // still don't have 3 sections after that point then we just subdivide uniformly in
        // parametric space.
        ...
        // form the original input points in order to guarantee the seams don't crack.
        ...
        if (discr <= 0) {
            ...
            if (coeffs == float3(0)) {
                ...
                // The tangent already equals midtangent at every parametric point at which a
        ...
        if (patchType != kStandardCubicType) {
            ...
            // tangent can be undefined at points that divide rotation in half. Use midtangent
            // instead at these locations.
            float midT = (abs(roots[0] - .5) < abs(roots[1] - .5)) ? roots[0] : roots[1];
            float2 secondDerivateMidtan = float2(3*midT, 1) * float2x2(C);
            if (determinant(float2x2(midtangent, secondDerivateMidtan)) * turnDirAtInf < 0) {
    ...
    void main() {
        ...
        if (joinType >= kBevelJoinType) {
            ...
            if (length(tan0norm - tan1norm) * strokeRadius < uTolerance) {
                ...
                // side of the junction. In case our join was indended to go on the exterior side
