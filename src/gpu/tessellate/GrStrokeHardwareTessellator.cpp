...
private:
    ...
    void internalPatchTo(JoinType prevJoinType, bool prevJoinFitsInPatch, const SkPoint p[4],
        ...
        if (prevJoinType == JoinType::kBowtie) {
            ...
            // continuous tangent angles (i.e., the ending tangent angle of the first chop and
            // the beginning tangent angle of the second both point in the same direction). The
            // tangents will only ever not point in the same direction if we chopped at a cusp
            // point, so that's the only time we actually need a bowtie.
            ...
            // This will only be true if cosTheta * abs(cosTheta) !~= 1, meaning the tangents don't
            // point in the same direction:
            //
            //     (ab)^2 !~= ab * cosTheta * abs(ab * cosTheta)
            //     (ab)^2 !~= ab * cosTheta * ab * abs(cosTheta)  [ab is always positive]
            //     (ab)^2 !~= (ab)^2 * cosTheta * abs(cosTheta)
            //          1 !~= cosTheta * abs(cosTheta)
            //
...
void GrStrokeHardwareTessellator::prepare(GrMeshDrawOp::Target* target,
    ...
    for (auto* pathStroke = fPathStrokeList; pathStroke; pathStroke = pathStroke->fNext) {
        ...
        for (auto [verb, p, w] : SkPathPriv::Iterate(path)) {
            ...
            switch (verb) {
                ...
                case SkPathVerb::kLine:
                    ...
                    if (p[0] == p[1]) {
                        continue;  // Continue afer marking contourIsEmpty as false.
                    }
