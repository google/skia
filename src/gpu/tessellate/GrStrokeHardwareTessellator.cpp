...
public:
    ...
    float fetchNext(PathStrokeList* head) {
        // Hairline support would require local adjustments based on the view matrix.
        ...
        if (fBufferIdx == 4) {
            ...
            // See GrStrokeTessellateShader::Tolerances::CalcNumRadialSegmentsPerRadian().
            fNumRadialSegmentsPerRadian = .5f / grvx::approx_acos(
                    skvx::max(1 - 2 / (fParametricIntolerance * fStrokeWidths), -1));
            fBufferIdx = 0;
        }
...
private:
    ...
    grvx::float4 fNumRadialSegmentsPerRadian;
    const float fParametricIntolerance;
...
void GrStrokeHardwareTessellator::prepare(GrMeshDrawOp::Target* target,
    ...
    // Fast SIMD buffer of "numRadialSegmentsPerRadian" values for dynamic strokes.
    ...
    for (PathStrokeList* pathStroke = fPathStrokeList; pathStroke; pathStroke = pathStroke->fNext) {
        ...
        if (fShaderFlags & ShaderFlags::kDynamicStroke) {
            ...
            SkASSERT(!stroke.isHairlineStyle());  // Hairlines can't be dynamic.
