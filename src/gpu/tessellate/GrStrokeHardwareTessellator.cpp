...
void GrStrokeHardwareTessellator::prepare(GrMeshDrawOp::Target* target, const SkMatrix& viewMatrix,
    ...
    for (const auto& pathStroke : pathStrokeList) {
        ...
        if (previousVerb != SkPathVerb::kMove && previousVerb != SkPathVerb::kClose) {
            this->cap(SkPathPriv::PointData(path)[path.countPoints() - 1], viewMatrix, stroke);
        }
    }
...
void GrStrokeHardwareTessellator::joinTo(JoinType joinType, SkPoint junctionPoint,
    ...
    if (!fSoloRoundJoinAlwaysFitsInPatch && maxDepth != 0 &&
        ...
        if (numRadialSegments > fMaxTessellationSegments) {
            ...
            SkPoint c0, c1;
            // FIXME: This hack ensures "c0 - junctionPoint" gives the exact same ieee fp32 vector
            // as "-(c1 - junctionPoint)". If our current strategy of join chopping sticks, we may
