...
public:
    ...
    void writeConicPatchesTo(const SkPoint p[3], float w) {
        this->internalConicPatchesTo(fStrokeJoinType, p, w, -1);
    }
    ...
    void writeCubicConvex180PatchesTo(const SkPoint p[4]) {
        ...
        if (numChops == 0) {
            ...
            this->internalCubicConvex180PatchesTo(fStrokeJoinType, p, -1);
        } else if (numChops == 1) {
            ...
            this->internalCubicConvex180PatchesTo(fStrokeJoinType, chops, -1);
            this->internalCubicConvex180PatchesTo(JoinType::kBowtie, chops + 3, -1);
        } else {
            SkASSERT(numChops == 2);
            ...
            this->internalCubicConvex180PatchesTo(fStrokeJoinType, chops, -1);
            this->internalCubicConvex180PatchesTo(JoinType::kBowtie, chops + 3, -1);
            this->internalCubicConvex180PatchesTo(JoinType::kBowtie, chops + 6, -1);
        }
    }
...
private:
    ...
    // Recursively chops the given conic and its previous join until the segments fit in
    // tessellation patches.
    void internalConicPatchesTo(JoinType prevJoinType, const SkPoint p[3], float w, int maxDepth) {
        // Zero-length paths need special treatment because they are spec'd to behave differently.
    ...
    void internalCubicConvex180PatchesTo(JoinType prevJoinType, const SkPoint p[4], int maxDepth) {
