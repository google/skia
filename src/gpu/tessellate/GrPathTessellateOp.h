...
private:
    ...
    // Chooses the rendering method we will use and creates the corresponding stencil anf fill
    // programs up front.
    ...
    void prePrepareStencilPipeline(const PrePrepareArgs&);
    ...
    void prePrepareFillPipeline(const PrePrepareArgs&);
    ...
    const GrPipeline* fStencilPipeline = nullptr;

    // This same GrPipeline is used by all programInfos in the fill step.
    const GrPipeline* fFillPipeline = nullptr;
    ...
    // This will be either a bounding box, or if fFillTrianglesProgram exists, it will draw convex
    // hulls over each cubic in the path.
