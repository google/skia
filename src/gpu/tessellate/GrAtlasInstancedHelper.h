...
public:
    ...
    GrAtlasInstancedHelper(sk_sp<GrTextureProxy> atlasProxy, ShaderFlags shaderFlags)
            : fProxy(std::move(atlasProxy))
            , fShaderFlags(shaderFlags) {
    }

    // Returns the atlas texture proxy.
    ...
    bool isCompatible(const GrAtlasInstancedHelper& helper) {
        ...
        return fProxy == helper.fProxy && fShaderFlags == helper.fShaderFlags;
    }
    ...
    // Injects vertex code, fragment code, varyings, and uniforms to implement the atlas clip. The
    // caller is responsible to store "atlasAdjustUniformHandle" and pass it to setUniformData().
...
private:
    const sk_sp<GrTextureProxy> fProxy;
    ShaderFlags fShaderFlags;
};
