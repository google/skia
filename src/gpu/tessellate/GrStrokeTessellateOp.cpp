...
void GrStrokeTessellateOp::prePrepareTessellator(GrPathShader::ProgramArgs&& args,
    ...
    if (fTotalCombinedVerbCnt > 50 && this->canUseHardwareTessellation(caps)) {
        ...
        // Use the fixed-count instanced tessellator if we're drawing relatively few verbs or if the
        // stroke use dynamic color. (The log2-indirect tessellator can't support dynamic color
        // without a z-buffer due to how it reorders strokes.)
