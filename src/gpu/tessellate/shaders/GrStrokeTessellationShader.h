...
public:
    ...
    // Returns the vertex buffer that should be bound when drawing in Mode::kFixedCount (Only for if
    // sk_VertexID is not supported). The buffer holds enough data for a strip as long as
    // 1024(??????) segments.
    static sk_sp<const GrGpuBuffer> FindOrMakeFixedCountVertexBuffer(GrResourceProvider*);
