...
sk_sp<const GrGpuBuffer> GrPathTessellationShader::FindOrMakeMiddleOutVertexBuffer(
    ...
    constexpr static int kCurveVertexCount = (1 << kMaxResolveLevel) + 1;
    int vertexCount = (patchType == PatchType::kWedges) ? kCurveVertexCount + 1
                                                        : kCurveVertexCount;
    ...
    return resourceProvider->findOrMakeStaticBuffer(
            GrGpuBufferType::kVertex, vertexCount * sizeof(float) * 2, uniqueKey,
            [](GrVertexWriter vertexWriter, size_t bufferSize) {
        SkDEBUGCODE(GrVertexWriter endWriter = vertexWriter.makeOffset(bufferSize);)

        if (bufferSize != kCurveVertexCount * sizeof(float) * 2) {
            // We are the wedge buffer. Start out with the fan point. A negative resolve level
            // indicates the fan point.
            SkASSERT(bufferSize == (kCurveVertexCount + 1) * sizeof(float) * 2);
...
sk_sp<const GrGpuBuffer> GrPathTessellationShader::FindOrMakeMiddleOutIndexBuffer(
    ...
    int triangleCount = (patchType == PatchType::kWedges) ? kCurveTriangleCount + 1
                                                          : kCurveTriangleCount;
    ...
    return resourceProvider->findOrMakeStaticBuffer(
            GrGpuBufferType::kIndex, triangleCount * sizeof(uint16_t) * 3, uniqueKey,
            [](GrVertexWriter vertexWriter, size_t bufferSize) {
        ...
        if (triangleCount != kCurveTriangleCount) {
            // We are the wedge buffer. Start out with the fan triangle.
            SkASSERT(triangleCount == kCurveTriangleCount + 1);
        ...
        const auto* neighborInLastResolveLevel =
                &indexData.push_back({nextVertexIdx,
        ...
        SkASSERT(nextVertexIdx == triangleCount + 2);
