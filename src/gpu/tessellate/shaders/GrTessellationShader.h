...
public:
    ...
    static uint32_t PortableInfinityBits32(const GrShaderCaps& shaderCaps) {
        ...
        // On hardware that doesn't support infinity, we pretend that numbers in the "2^127"
        // exponent range are infinity. The rationale for doing this is that these number are so
        // large, they will overflow if we try to do any bezier math with them anyway.
        constexpr static uint32_t kIEEE_32_pseudo_infinity =
                (0xfe << 23) | (1 << 22);  // 1.5 * 2^127.
