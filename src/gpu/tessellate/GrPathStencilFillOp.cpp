...
void GrPathStencilFillOp::prePreparePrograms(const GrPathShader::ProgramArgs& args,
    ...
    if (drawTrianglesAsIndirectCubicDraw || (fOpFlags & OpFlags::kDisableHWTessellation)) {
        ...
        // Next see if we can split up the inner triangles and outer cubics into two draw calls.
        // This allows for a more efficient inner triangle topology that can reduce the rasterizer
        // load by a large margin on complex paths, but also causes greater CPU overhead due to the
        // extra shader switches and draw calls.
