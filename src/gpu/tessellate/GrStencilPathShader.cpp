...
// spec'd to require this, but empirical results results have shown it does not always.
...
SkString GrCubicTessellateShader::getTessControlShaderGLSL(const GrGLSLPrimitiveProcessor*,
    ...
    void main() {
        ...
        // since our coordinates are homogeneous we can omit the multiply by .5. Each result
        // will just be scaled by .5^N in both the numerator and denominator.
        vec3 ab = P[0] + P[1];
        vec3 bc = P[1] + P[2];
        vec3 cd = P[2] + P[3];
        vec3 abc = ab + bc;
