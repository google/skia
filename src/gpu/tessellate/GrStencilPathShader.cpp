...
sk_sp<const GrGpuBuffer> GrMiddleOutCubicShader::FindOrMakeMiddleOutIndexBuffer(
    ...
    constexpr static int indexCount = NumVerticesAtResolveLevel(kMaxResolveLevel) + 3;
    if (auto buffer = resourceProvider->createBuffer(
        ...
        // Indices 0,1,2 have special values that emit points P0, P1, and P2 respectively. (When the
        ...
        // index value corresponds to parametric value T=(index / 2^kMaxResolveLevel). We arrange
        // the triangles in "middle-out" order so the caller can conveniently control the
...
class GrMiddleOutCubicShader::Impl : public GrStencilPathShader::Impl {
    void onEmitCode(EmitArgs& args, GrGPArgs* gpArgs) override {
        SkASSERT(args.fShaderCaps->fpManipulationSupport());  // For ldexp().
