...
// Converts a 4-point input instance into the rational cubic it intended to represent.
...
float4x3 unpack_rational_cubic(float2 p0, float2 p1, float2 p2, float2 p3) {
    ...
    if (isnan(P[3].y)) {
        // This instance is actually a conic. Convert to a rational cubic.
        float w = P[3].x;
...
// spec'd to require this, empiric results have shown it does not return exactly "b" when T=1.
...
SkString GrCubicTessellateShader::getTessControlShaderGLSL(const GrGLSLPrimitiveProcessor*,
    ...
    void main() {
        ...
        // Chop the curve at T=1/2. This would technically mean linearly interpolating at .5, but
        // since our coordinates are homogeneous we can omit the multiply by .5. The final result
        // will just be scaled by .5^3 in both the numerator and denominator.
        vec3 ab = P[0] + P[1];
        vec3 bc = P[1] + P[2];
        vec3 cd = P[2] + P[3];
        vec3 abc = ab + bc;
