    ...
    void onEmitCode(EmitArgs& args, GrGPArgs* gpArgs) override {
        ...
        if (args.fShaderCaps->bitManipulationSupport()) {
            ...
            float find_middle_out_T() {
                ...
                // Ensure T === 1 on the final vertex at each depth.
                return (vertexIdxWithinDepth < (2 << depth))
                        ? ldexp(float(vertexIdxWithinDepth), -depth - 1) : 1;
            })");
        } else {
            // Determines the T value at which to place the given vertex in a "middle-out" topology.
            args.fVertBuilder->insertFunction(R"(
            float find_middle_out_T() {
                ...
                // Ensure T === 1 on the final vertex at each depth.
                return (vertexIdxWithinDepth < firstTriangleAtDepth * 2)
                        ? vertexIdxWithinDepth * exp2(-depth - 1) : 1;
            })");
        }
