...
constexpr int max_wedge_vertex_count(const SkPath& path) {
...
int GrPathParser::EmitCenterWedgePatches(const SkPath& path, GrApproxVertexAllocator* vertexAlloc) {
    auto* vertexData = vertexAlloc->lock<SkPoint>(max_wedge_vertex_count(path));
    if (!vertexData) {
        return 0;
    }
...
constexpr int max_inner_poly_vertex_count(const SkPath& path) {
...
int GrPathParser::EmitInnerPolygonTriangles(const SkPath& path,
                                            GrApproxVertexAllocator* vertexAlloc) {
    int maxVertices = max_inner_poly_vertex_count(path);
...
int GrPathParser::EmitCubicInstances(const SkPath& path, GrApproxVertexAllocator* vertexAlloc) {
