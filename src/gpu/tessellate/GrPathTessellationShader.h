...
public:
    GrTriangleShader(const SkMatrix& viewMatrix, SkPMColor4f color)
            : GrPathTessellationShader(kTessellate_GrFillTriangleShader_ClassID,
                                       GrPrimitiveType::kTriangles, 0, viewMatrix, color) {
        static constexpr Attribute kPtAttrib = {
                "inputPoint", kFloat2_GrVertexAttribType, kFloat2_GrSLType};
        this->setVertexAttributes(&kPtAttrib, 1);
    }

private:
    const char* name() const final { return "GrFillTriangleShader"; }
