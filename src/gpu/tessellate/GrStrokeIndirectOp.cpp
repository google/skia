...
// Computes and writes out the resolveLevels for individual stroke. Maintains a counter of the
...
private:
    ...
    template<int NumPts> struct SIMDQueue {
        ...
        // Loads pts[idx] in SIMD for 4 different strokes, with the x values in the "vec.lo" and the
        // y values in "vec.hi".
        ...
        // Loads 4 lastControlPoints in SIMD, with the x values in "vec.lo" and the y values in
        // "vec.hi".
...
void GrStrokeIndirectOp::prePrepareResolveLevels(SkArenaAlloc* alloc) {
    ...
    int resolveLevelAllocCount = fTotalCombinedVerbCnt * 4;
    ...
    fChopTs = alloc->makeArray<float>(chopTAllocCount);
        ...
        while (iter.loadNext()) {
            ...
            switch (verb) {
                ...
                case Verb::kLine:
                    ...
                    break;
                case Verb::kQuad:
                    find_quadratic_cusp_if_any(pts, &cusps);
                ...
                case Verb::kCubic: {
                    ...
                    if (areCusps) {
                        for (int i = 0; i < numChops; ++i) {
                            SkEvalCubicAt(pts, nextChopTs[i], &cusps.push_back(), nullptr, nullptr);
                        }
                    }
                    if (numChops == 0) {
                        counter.countCubic(pts, lastControlPoint, nextResolveLevel);
                    } else if (numChops == 1) {
                        // A negative resolveLevel indicates how many chops the curve needs.
                        *nextResolveLevel++ = -1;
                ...
                case Verb::kCusp:
                   // The iterater implements round caps as cusps.
    ...
    // Copy the cusps for later, if any.
    fNumCusps = cusps.count();
    if (fNumCusps) {
        fCusps = alloc->makeArray<SkPoint>(cusps.count());
        memcpy(fCusps, cusps.data(), cusps.count() * sizeof(SkPoint));
        fResolveLevelCounts[fResolveLevelForCusps] += fNumCusps;
        fTotalInstanceCount += fNumCusps;
    }
...
void GrStrokeIndirectOp::prepareBuffers(GrMeshDrawOp::Target* target) {
    ...
    for (const SkPath& path : fPathList) {
        ...
        while (iter.loadNext()) {
            int numChops = 0;
            const SkPoint* pts=iter.nextPts(), *pts_=pts;
            switch (iter.nextVerb()) {
                using Verb = GrStrokeIterator::Verb;
                case Verb::kCusp:
                ...
                case Verb::kCubic:
                    resolveLevel = *nextResolveLevel++;
                    if (resolveLevel < 0) {
                        // A negative resolveLevel indicates that the curve needs to be chopped.
                        numChops = -resolveLevel;
    ...
    // Write out cusp instances, if any.
    for (int i = 0; i < fNumCusps; ++i) {
        nextInstanceLocations[fResolveLevelForCusps]++->setCusp(
                fCusps[i], numEdgesPerResolveLevel[fResolveLevelForCusps]);
    }
