            ...
            switch (verb) {
                ...
                case Verb::kQuad: {
                    ...
                    if (a.cross(b) == 0 && a.dot(b) < 0) {
                        // The curve has a cusp. Draw two lines and a cusp instead of a quad.
                        *nextResolveLevel++ = -1;  // -1 signals a cusp.
                        if (counter.countLine(pts, lastControlPoint, nextResolveLevel)) {
                            ++nextResolveLevel;
                        }
                        ++fResolveLevelCounts[fResolveLevelForCusps];  // Cusp instance.
    ...
    for (const SkPath& path : fPathList) {
        ...
        while (iter.next()) {
                    ...
                    if (resolveLevel < 0) {
                        ...
                        if (-resolveLevel & 1) {  // Are the chop points cusps?
                            for (int i = 1; i <= numChops; ++i) {
                                nextInstanceLocations[fResolveLevelForCusps]++->setCusp(
                                        pts_[i*3], numEdgesPerResolveLevel[fResolveLevelForCusps]);
                            }
                        }
                        resolveLevel = *nextResolveLevel++;
                    }
            ...
            for (int i = 0;;) {
                ...
                pts_ += 3;
                // If a non-cubic got chopped, it means it was chopped into lines and a cusp.
