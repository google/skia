...
public:
    DrawAtlasPathShader(const GrTextureProxy* atlasProxy, GrSwizzle swizzle, bool isInverseFill,
        ...
        fAttribs.emplace_back("fillXYWH", kFloat4_GrVertexAttribType, kFloat4_GrSLType);
        if (fUsesLocalCoords) {
        ...
        fAttribs.emplace_back("devToAtlas", kFloat2_GrVertexAttribType, kFloat2_GrSLType);
        if (fIsInverseFill) {
            fAttribs.emplace_back("pathDevBounds", kFloat4_GrVertexAttribType, kFloat4_GrSLType);
        }
...
class DrawAtlasPathShader::Impl : public GrGLSLGeometryProcessor {
    void onEmitCode(EmitArgs& args, GrGPArgs* gpArgs) override {
        ...
        args.fVertBuilder->codeAppendf(R"(
        float2 fillCoord = abs(fillXYWH.zw) * unitCoord;
        float2 devCoord = fillXYWH.xy + fillCoord;
        bool transposed = fillXYWH.w < 0;  // Negative height means the path is transposed.
        float2 atlasCoord = fillXYWH.xy + devToAtlas + (transposed ? fillCoord.yx : fillCoord.xy);
        ...
        if (shader.fIsInverseFill) {
            ...
            args.fVertBuilder->codeAppendf(R"(
            float4 atlasBounds = pathDevBounds + devToAtlas.xyxy;
            if (transposed) {
                // Transpose the rectangle.
                atlasBounds.zw = atlasBounds.xy + (atlasBounds.zw - atlasBounds.xy).yx;
            }
    ...
    if (GrVertexWriter instanceWriter = flushState->makeVertexSpace(
        ...
        for (const Instance* instance = fHeadInstance; instance; instance = instance->fNext) {
            instanceWriter.write(
                    (float)instance->fFillBounds.x(),
                    (float)instance->fFillBounds.y(),
                    (float)instance->fFillBounds.width(),
                    // Negative height means we are transposed in the atlas.
                    (float)(instance->fTransposedInAtlas ? -instance->fFillBounds.height()
                                                         : instance->fFillBounds.height()),
                    GrVertexWriter::If(fUsesLocalCoords,
                                       instance->fLocalToDeviceIfUsingLocalCoords),
                    instance->fColor,
                    (float)instance->fDevToAtlas.x(),
                    (float)instance->fDevToAtlas.y(),
                    GrVertexWriter::If(fIsInverseFill,
                                       SkRect::Make(instance->fPathDevIBounds)));
        }
    }
