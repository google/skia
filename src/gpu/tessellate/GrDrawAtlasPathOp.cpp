...
public:
    DrawAtlasPathShader(const GrTextureProxy* atlasProxy, bool usesLocalCoords)
            : GrGeometryProcessor(kDrawAtlasPathShader_ClassID)
            , fAtlasAccess(GrSamplerState::Filter::kNearest, atlasProxy->backendFormat(),
                           atlasProxy->textureSwizzle())
...
class DrawAtlasPathShader::Impl : public GrGLSLGeometryProcessor {
    void onEmitCode(EmitArgs& args, GrGPArgs* gpArgs) override {
        ...
        args.fVaryingHandler->addPassThroughAttribute(
                shader.instanceAttributes().atMaybeUninitialized(2), args.fOutputColor,
...
void GrDrawAtlasPathOp::onExecute(GrOpFlushState* state, const SkRect& chainBounds) {
    ...
    DrawAtlasPathShader shader(fAtlasProxy.get(), fUsesLocalCoords);
