...
GrOp::Owner GrStrokeOp::Make(GrRecordingContext* context, GrAAType aaType,
                             const SkMatrix& viewMatrix, const SkStrokeRec& stroke,
                             const SkPath& path, GrPaint&& paint, const GrShaderCaps& shaderCaps) {
    if (shaderCaps.tessellationSupport()) {
        return GrOp::Make<GrStrokeTessellateOp>(context, aaType, viewMatrix, stroke, path,
                                                std::move(paint));
    } else {
        SK_ABORT("Indirect stroke not supported yet.");
    }
}
