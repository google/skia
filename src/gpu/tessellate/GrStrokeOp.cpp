...
void GrStrokeOp::prePrepareTessellator(GrPathShader::ProgramArgs&& args, GrAppliedClip&& clip) {
    ...
    const GrPipeline* mixedSamplesStencilPipeline = nullptr;
    if (fAAType == GrAAType::kCoverage) {
        ...
        mixedSamplesStencilPipeline = GrStencilPathShader::MakeStencilPassPipeline(
    ...
    if (fNeedsStencil) {
        auto* stencilPipeline = (mixedSamplesStencilPipeline) ? mixedSamplesStencilPipeline
                                                              : fillPipeline;
