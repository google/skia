...
@cppEnd {
    bool GrConfigConversionEffect::TestForPreservingPMConversions(GrDirectContext* dContext) {
        ...
        auto readRTC = GrLRenderTargetContext::Make(dContext, upmII, SkBackingFit::kExact);
        auto tempRTC = GrLRenderTargetContext::Make(dContext,  pmII, SkBackingFit::kExact);
        if (!readRTC || !tempRTC) {
            return false;
        }
        ...
        readRTC->fillIRectSrcMode(SkIRect::MakeWH(kSize, kSize), std::move(fp1));
        if (!readRTC->readPixels(dContext, upmII, firstRead, 0, {0, 0})) {
            return false;
        }

        auto fp2 = GrConfigConversionEffect::Make(GrTextureEffect::Make(readRTC->readSurfaceView(),
                                                                        readRTC->colorInfo().alphaType()),
                                                  PMConversion::kToPremul);
        tempRTC->fillIRectSrcMode(SkIRect::MakeWH(kSize, kSize), std::move(fp2));

        auto fp3 = GrConfigConversionEffect::Make(GrTextureEffect::Make(tempRTC->readSurfaceView(),
                                                                        tempRTC->colorInfo().alphaType()),
                                                  PMConversion::kToUnpremul);
        readRTC->fillIRectSrcMode(SkIRect::MakeWH(kSize, kSize), std::move(fp3));

        if (!readRTC->readPixels(dContext, upmII, secondRead, 0, {0, 0})) {
            return false;
        }
