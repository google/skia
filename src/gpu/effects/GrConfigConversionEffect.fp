...
@cppEnd {
    bool GrConfigConversionEffect::TestForPreservingPMConversions(GrDirectContext* dContext) {
        ...
        readSFC->fillIRectSrcMode(SkIRect::MakeWH(kSize, kSize), std::move(fp1));
        if (!readSFC->readPixels(dContext, upmII, firstRead, 0, {0, 0})) {
            return false;
        }

        auto fp2 = GrConfigConversionEffect::Make(GrTextureEffect::Make(readSFC->readSurfaceView(),
                                                                        readSFC->colorInfo().alphaType()),
                                                  PMConversion::kToPremul);
        tempSFC->fillIRectSrcMode(SkIRect::MakeWH(kSize, kSize), std::move(fp2));

        auto fp3 = GrConfigConversionEffect::Make(GrTextureEffect::Make(tempSFC->readSurfaceView(),
                                                                        tempSFC->colorInfo().alphaType()),
                                                  PMConversion::kToUnpremul);
        readSFC->fillIRectSrcMode(SkIRect::MakeWH(kSize, kSize), std::move(fp3));
