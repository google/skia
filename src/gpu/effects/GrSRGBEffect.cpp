...
public:
    void emitCode(EmitArgs& args) override {
        ...
        static const GrShaderVar gSrgbArgs[] = {
            GrShaderVar("x", kHighFloat_GrSLType),
        };
        switch (srgbe.mode()) {
            case GrSRGBEffect::Mode::kLinearToSRGB:
                fragBuilder->emitFunction(kHighFloat_GrSLType,
                                          "linear_to_srgb",
        ...
        fragBuilder->codeAppendf("highfloat4 color = %s;", args.fInputColor);
        if (srgbe.alpha() == GrSRGBEffect::Alpha::kPremul) {
            fragBuilder->codeAppendf("highfloat nonZeroAlpha = max(color.a, 0.00001);");
            fragBuilder->codeAppendf("color = highfloat4(color.rgb / nonZeroAlpha, color.a);");
        }
        fragBuilder->codeAppendf("color = highfloat4(%s(color.r), %s(color.g), %s(color.b), color.a);",
                                    srgbFuncName.c_str(),
                                    srgbFuncName.c_str(),
                                    srgbFuncName.c_str());
        if (srgbe.alpha() == GrSRGBEffect::Alpha::kPremul) {
            fragBuilder->codeAppendf("color = highfloat4(color.rgb, 1) * color.a;");
        }
