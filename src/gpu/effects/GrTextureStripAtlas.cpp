...
void GrTextureStripAtlas::lockTexture() {
    ...
    sk_sp<GrTextureProxy> proxy = fDesc.fContext->proxyProvider()->findOrCreateProxyByUniqueKey(
    ...
    if (!proxy) {
        ...
        proxy = GrSurfaceProxy::MakeDeferred(fDesc.fContext->proxyProvider(),
        ...
        fDesc.fContext->proxyProvider()->assignUniqueKeyToProxy(key, proxy.get());
