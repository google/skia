...
void GrGaussianConvolutionFragmentProcessor::Impl::emitCode(EmitArgs& args) {
    ...
    Var kernel(kUniform_Modifier, Array(kFloat4, arrayCount), "Kernel");
    fKernelUni = VarUniformHandle(kernel);

    Var offsets(kUniform_Modifier, Array(kFloat4, arrayCount), "Offsets");
    fOffsetsUni = VarUniformHandle(offsets);

    Var color(kHalf4, "color", Half4(0));
    Declare(color);

    Var coord(kFloat2, "coord", sk_SampleCoord());
    Declare(coord);

    // Manually unroll loop because some drivers don't; yields 20-30% speedup.
    for (int i = 0; i < width; i++) {
        color += Half4(SampleChild(/*index=*/0, coord + offsets[i / 4][i & 3] * increment) *
            kernel[i / 4][i & 0x3]);
    }
    Return(color);
