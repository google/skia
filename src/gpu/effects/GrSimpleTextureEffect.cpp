...
class GrGLSLSimpleTextureEffect : public GrGLSLFragmentProcessor {
 public:
  ...
  void emitCode(EmitArgs& args) override {
    ...
    const GrSimpleTextureEffect& _outer =
        args.fFp.cast<GrSimpleTextureEffect>();
    (void)_outer;
    auto matrix = _outer.matrix();
    (void)matrix;
    SkString sk_TransformedCoords2D_0 =
        fragBuilder->ensureCoords2D(args.fTransformedCoords[0]);
    fragBuilder->codeAppendf("%s = %s * texture(%s, %s).%s;\n",
...
GrSimpleTextureEffect::onTextureSampler(int index) const {
...
std::unique_ptr<GrFragmentProcessor> GrSimpleTextureEffect::TestCreate(
  ...
  int texIdx = testData->fRandom->nextBool()
                   ? GrProcessorUnitTest::kSkiaPMTextureIdx
