...
public:
    ...
    void emitCode(EmitArgs& args) override {
        [[maybe_unused]] const GrCircleBlurFragmentProcessor& _outer =
                args.fFp.cast<GrCircleBlurFragmentProcessor>();

        using namespace SkSL::dsl;
        StartFragmentProcessor(this, &args);
        [[maybe_unused]] const auto& circleRect = _outer.circleRect;
        Var _circleRect(kConst_Modifier, DSLType(kHalf4_Type), "circleRect");
        Declare(_circleRect,
                Half4(circleRect.left(),
                      circleRect.top(),
                      circleRect.right(),
                      circleRect.bottom()));
        [[maybe_unused]] const auto& solidRadius = _outer.solidRadius;
        Var _solidRadius(kConst_Modifier, DSLType(kHalf_Type), "solidRadius");
        Declare(_solidRadius, Half(solidRadius));
        [[maybe_unused]] const auto& textureRadius = _outer.textureRadius;
        Var _textureRadius(kConst_Modifier, DSLType(kHalf_Type), "textureRadius");
        Declare(_textureRadius, Half(textureRadius));
        Var _circleData(kUniform_Modifier, DSLType(kHalf4_Type), "circleData");
        circleDataVar = VarUniformHandle(_circleData);
        DeclareGlobal(_circleData);
        Var _vec(kNo_Modifier, DSLType(kHalf2_Type), "vec");
        Var _dist(kNo_Modifier, DSLType(kHalf_Type), "dist");
        Var _inputColor(kNo_Modifier, DSLType(kHalf4_Type), "inputColor");
        Declare(_vec,
                Half2((Swizzle(sk_FragCoord(), X, Y) - Float2(Swizzle(_circleData, X, Y))) *
                      Float(_circleData.w())));
        Declare(_dist, Length(_vec) + (0.5f - _circleData.z()) * _circleData.w());
        Declare(_inputColor, SampleChild(0));
        Return(_inputColor * SampleChild(1, Float2(Half2(_dist, 0.5f))).w());
...
private:
    void onSetData(const GrGLSLProgramDataManager& data,
        ...
        [[maybe_unused]] const auto& solidRadius = _outer.solidRadius;
        [[maybe_unused]] const auto& textureRadius = _outer.textureRadius;
        [[maybe_unused]] UniformHandle& circleData = circleDataVar;
