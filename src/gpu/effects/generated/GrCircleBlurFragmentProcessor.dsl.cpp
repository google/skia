...
public:
    ...
    void emitCode(EmitArgs& args) override {
        ...
        Var _circleRect(kConst_Modifier,
                        DSLType(kHalf4_Type),
                        "circleRect",
        ...
        Var _solidRadius(kConst_Modifier, DSLType(kHalf_Type), "solidRadius", Half(solidRadius));
        Declare(_solidRadius);
        [[maybe_unused]] const auto& textureRadius = _outer.textureRadius;
        Var _textureRadius(
                kConst_Modifier, DSLType(kHalf_Type), "textureRadius", Half(textureRadius));
        Declare(_textureRadius);
        ...
        Var _vec(kNo_Modifier,
                 DSLType(kHalf2_Type),
                 "vec",
                 Half2((Swizzle(sk_FragCoord(), X, Y) - Float2(Swizzle(_circleData, X, Y))) *
                       Float(_circleData.w())));
        Var _dist(kNo_Modifier,
                  DSLType(kHalf_Type),
                  "dist",
                  Length(_vec) + (0.5f - _circleData.z()) * _circleData.w());
