...
public:
    ...
    void emitCode(EmitArgs& args) override {
        ...
        Var circleRect(kConst_Modifier,
                       DSLType(kHalf4_Type),
                       "circleRect",
                       Half4(_outer.circleRect.left(),
                             _outer.circleRect.top(),
                             _outer.circleRect.right(),
                             _outer.circleRect.bottom()));
        Declare(circleRect);
        Var solidRadius(
                kConst_Modifier, DSLType(kHalf_Type), "solidRadius", Half(_outer.solidRadius));
        Declare(solidRadius);
        Var textureRadius(
                kConst_Modifier, DSLType(kHalf_Type), "textureRadius", Half(_outer.textureRadius));
        Declare(textureRadius);
        Var circleData(kUniform_Modifier, DSLType(kHalf4_Type), "circleData");
        circleDataVar = VarUniformHandle(circleData);
        Var vec(kNo_Modifier,
                DSLType(kHalf2_Type),
                "vec",
                Half2((Swizzle(sk_FragCoord(), X, Y) - Float2(Swizzle(circleData, X, Y))) *
                      Float(circleData.w())));
        Var dist(kNo_Modifier,
                 DSLType(kHalf_Type),
                 "dist",
                 Length(vec) + (0.5f - circleData.z()) * circleData.w());
        Var inputColor(kNo_Modifier, DSLType(kHalf4_Type), "inputColor", SampleChild(0));
        Declare(vec);
        Declare(dist);
        Declare(inputColor);
        Return(inputColor * SampleChild(1, Float2(Half2(dist, 0.5f))).w());
