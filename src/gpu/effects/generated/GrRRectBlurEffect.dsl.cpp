...
public:
    ...
    void emitCode(EmitArgs& args) override {
        [[maybe_unused]] const GrRRectBlurEffect& _outer = args.fFp.cast<GrRRectBlurEffect>();

        using namespace SkSL::dsl;
        StartFragmentProcessor(this, &args);
        [[maybe_unused]] const auto& sigma = _outer.sigma;
        Var _sigma(kConst_Modifier, DSLType(kFloat_Type), "sigma");
        Declare(_sigma, Float(sigma));
        ...
        Var _cornerRadius(kUniform_Modifier, DSLType(kHalf_Type), "cornerRadius");
        cornerRadiusVar = VarUniformHandle(_cornerRadius);
        DeclareGlobal(_cornerRadius);
        Var _proxyRect(kUniform_Modifier, DSLType(kFloat4_Type), "proxyRect");
        proxyRectVar = VarUniformHandle(_proxyRect);
        DeclareGlobal(_proxyRect);
        Var _blurRadius(kUniform_Modifier, DSLType(kHalf_Type), "blurRadius");
        blurRadiusVar = VarUniformHandle(_blurRadius);
        DeclareGlobal(_blurRadius);
        Var _translatedFragPosFloat(kNo_Modifier, DSLType(kFloat2_Type), "translatedFragPosFloat");
        Var _proxyCenter(kNo_Modifier, DSLType(kFloat2_Type), "proxyCenter");
        Var _edgeSize(kNo_Modifier, DSLType(kHalf_Type), "edgeSize");
        Var _fragDirection(kNo_Modifier, DSLType(kHalf2_Type), "fragDirection");
        Var _translatedFragPosHalf(kNo_Modifier, DSLType(kHalf2_Type), "translatedFragPosHalf");
        Var _proxyDims(kNo_Modifier, DSLType(kHalf2_Type), "proxyDims");
        Var _texCoord(kNo_Modifier, DSLType(kHalf2_Type), "texCoord");
        Declare(_translatedFragPosFloat, Swizzle(sk_FragCoord(), X, Y) - Swizzle(_proxyRect, X, Y));
        Declare(_proxyCenter, (Swizzle(_proxyRect, Z, W) - Swizzle(_proxyRect, X, Y)) * 0.5f);
        Declare(_edgeSize, (2.0f * _blurRadius + _cornerRadius) + 0.5f);
        _translatedFragPosFloat -= _proxyCenter;
        Declare(_fragDirection, Half2(Sign(_translatedFragPosFloat)));
        _translatedFragPosFloat = Abs(_translatedFragPosFloat);
        Declare(_translatedFragPosHalf,
                Half2(_translatedFragPosFloat - (_proxyCenter - Float(_edgeSize))));
        _translatedFragPosHalf = Max(_translatedFragPosHalf, 0.0f);
        _translatedFragPosHalf *= _fragDirection;
        _translatedFragPosHalf += Half2(_edgeSize);
        Declare(_proxyDims, Half2(2.0f * _edgeSize));
        Declare(_texCoord, _translatedFragPosHalf / _proxyDims);
        Return(SampleChild(0) * SampleChild(1, Float2(_texCoord)).w());
...
private:
    void onSetData(const GrGLSLProgramDataManager& pdman,
        ...
        [[maybe_unused]] const auto& sigma = _outer.sigma;
        [[maybe_unused]] const auto& rect = _outer.rect;
        [[maybe_unused]] UniformHandle& cornerRadius = cornerRadiusVar;
        [[maybe_unused]] UniformHandle& proxyRect = proxyRectVar;
        [[maybe_unused]] UniformHandle& blurRadius = blurRadiusVar;
