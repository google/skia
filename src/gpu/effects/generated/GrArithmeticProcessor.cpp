...
public:
    ...
    void emitCode(EmitArgs& args) override {
        ...
        kVar = args.fUniformHandler->addUniform(&_outer, kFragment_GrShaderFlag, kFloat4_GrSLType,
                                                "k");
        SkString _input384(args.fInputColor);
        SkString _sample384;
        _sample384 = this->invokeChild(_outer.srcFP_index, _input384.c_str(), args);
        fragBuilder->codeAppendf(
                R"SkSL(half4 src = %s;)SkSL", _sample384.c_str());
        SkString _sample427;
        _sample427 = this->invokeChild(_outer.dstFP_index, args);
        fragBuilder->codeAppendf(
                R"SkSL(
half4 dst = %s;
...
@if (%s) {
    %s.xyz = min(%s.xyz, %s.w);
                ...
                _sample427.c_str(), args.fOutputColor, args.fUniformHandler->getUniformCStr(kVar),
