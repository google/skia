...
public:
    ...
    void emitCode(EmitArgs& args) override {
        ...
        fragBuilder->codeAppendf(
                ...
                args.fUniformHandler->getUniformCStr(circleVar),
                args.fUniformHandler->getUniformCStr(circleVar));
        SkString _input2705 = SkStringPrintf("%s", args.fInputColor);
        SkString _sample2705;
        if (_outer.inputFP_index >= 0) {
            _sample2705 = this->invokeChild(_outer.inputFP_index, _input2705.c_str(), args);
        } else {
            _sample2705 = _input2705;
        }
        fragBuilder->codeAppendf(
                "\nhalf4 inputColor = %s;\n@if ((%d == 1 || %d == 3) || %d == 4) {\n    %s = "
                "inputColor * clamp(d, 0.0, 1.0);\n} else {\n    %s = d > 0.5 ? inputColor : "
                "half4(0.0);\n}\n",
                _sample2705.c_str(), (int)_outer.edgeType, (int)_outer.edgeType,
