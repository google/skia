...
@if (%s) {
    ...
    @if (highp) {
        ...
        xy = max(half2(float2(%s.xy) - pos), half2(pos - float2(%s.zw)));
                ...
                rectHVar.isValid() ? args.fUniformHandler->getUniformCStr(rectHVar) : "half4(0)");
        SkString _coords6831("float2(half2(xy.x, 0.5))");
        SkString _sample6831 = this->invokeChild(1, args, _coords6831.c_str());
        fragBuilder->codeAppendf(
                R"SkSL(
    xCoverage = %s.w;)SkSL",
                _sample6831.c_str());
        SkString _coords6889("float2(half2(xy.y, 0.5))");
        SkString _sample6889 = this->invokeChild(1, args, _coords6889.c_str());
        fragBuilder->codeAppendf(
                R"SkSL(
    yCoverage = %s.w;
} else {
    ...
    @if (highp) {
        ...
        rect.zw = half2(pos - float2(%s.zw));
                ...
                rectHVar.isValid() ? args.fUniformHandler->getUniformCStr(rectHVar) : "half4(0)",
                rectHVar.isValid() ? args.fUniformHandler->getUniformCStr(rectHVar) : "half4(0)");
        SkString _coords8208("float2(half2(rect.x, 0.5))");
        SkString _sample8208 = this->invokeChild(1, args, _coords8208.c_str());
        SkString _coords8271("float2(half2(rect.z, 0.5))");
        SkString _sample8271 = this->invokeChild(1, args, _coords8271.c_str());
        fragBuilder->codeAppendf(
                R"SkSL(
    xCoverage = (1.0 - %s.w) - %s.w;)SkSL",
                _sample8208.c_str(), _sample8271.c_str());
        SkString _coords8335("float2(half2(rect.y, 0.5))");
        SkString _sample8335 = this->invokeChild(1, args, _coords8335.c_str());
        SkString _coords8398("float2(half2(rect.w, 0.5))");
        SkString _sample8398 = this->invokeChild(1, args, _coords8398.c_str());
        fragBuilder->codeAppendf(
                R"SkSL(
    yCoverage = (1.0 - %s.w) - %s.w;
})SkSL",
                _sample8335.c_str(), _sample8398.c_str());
        SkString _sample8467 = this->invokeChild(0, args);
        fragBuilder->codeAppendf(
                R"SkSL(
half4 inputColor = %s;
%s = (inputColor * xCoverage) * yCoverage;
)SkSL",
                _sample8467.c_str(), args.fOutputColor);
    }
