...
@if (%s) {
    ...
    @if (highp) {
        ...
        xy = max(half2(float2(%s.xy) - pos), half2(pos - float2(%s.zw)));
                ...
                rectHVar.isValid() ? args.fUniformHandler->getUniformCStr(rectHVar) : "half4(0)");
        SkString _coords8150("float2(half2(xy.x, 0.5))");
        SkString _sample8150 = this->invokeChild(1, args, _coords8150.c_str());
        fragBuilder->codeAppendf(
                R"SkSL(
    xCoverage = %s.w;)SkSL",
                _sample8150.c_str());
        SkString _coords8208("float2(half2(xy.y, 0.5))");
    ...
    @if (highp) {
        ...
        rect.zw = half2(pos - float2(%s.zw));
                ...
                rectHVar.isValid() ? args.fUniformHandler->getUniformCStr(rectHVar) : "half4(0)",
                rectHVar.isValid() ? args.fUniformHandler->getUniformCStr(rectHVar) : "half4(0)");
        SkString _coords9527("float2(half2(rect.x, 0.5))");
        SkString _sample9527 = this->invokeChild(1, args, _coords9527.c_str());
        SkString _coords9590("float2(half2(rect.z, 0.5))");
        SkString _sample9590 = this->invokeChild(1, args, _coords9590.c_str());
        fragBuilder->codeAppendf(
                R"SkSL(
    xCoverage = (1.0 - %s.w) - %s.w;)SkSL",
                _sample9527.c_str(), _sample9590.c_str());
        SkString _coords9654("float2(half2(rect.y, 0.5))");
        SkString _sample9654 = this->invokeChild(1, args, _coords9654.c_str());
        SkString _coords9717("float2(half2(rect.w, 0.5))");
        SkString _sample9717 = this->invokeChild(1, args, _coords9717.c_str());
        fragBuilder->codeAppendf(
                R"SkSL(
    yCoverage = (1.0 - %s.w) - %s.w;
})SkSL",
                _sample9654.c_str(), _sample9717.c_str());
        SkString _sample9786 = this->invokeChild(0, args);
        fragBuilder->codeAppendf(
                R"SkSL(
half4 inputColor = %s;
%s = (inputColor * xCoverage) * yCoverage;
)SkSL",
                _sample9786.c_str(), args.fOutputColor);
    }
