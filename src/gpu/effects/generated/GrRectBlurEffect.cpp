...
@if (%s) {
    ...
    @if (highp) {
        ...
        xy = max(half2(float2(%s.xy) - pos), half2(pos - float2(%s.zw)));
                ...
                rectHVar.isValid() ? args.fUniformHandler->getUniformCStr(rectHVar) : "half4(0)");
        SkString _coords7649("float2(half2(xy.x, 0.5))");
        SkString _sample7649 = this->invokeChild(1, args, _coords7649.c_str());
        fragBuilder->codeAppendf(
                R"SkSL(
    xCoverage = %s.w;)SkSL",
                _sample7649.c_str());
        SkString _coords7707("float2(half2(xy.y, 0.5))");
        SkString _sample7707 = this->invokeChild(1, args, _coords7707.c_str());
        fragBuilder->codeAppendf(
                R"SkSL(
    yCoverage = %s.w;
} else {
    ...
    @if (highp) {
        ...
        rect.zw = half2(pos - float2(%s.zw));
                ...
                rectHVar.isValid() ? args.fUniformHandler->getUniformCStr(rectHVar) : "half4(0)",
                rectHVar.isValid() ? args.fUniformHandler->getUniformCStr(rectHVar) : "half4(0)");
        SkString _coords9026("float2(half2(rect.x, 0.5))");
        SkString _sample9026 = this->invokeChild(1, args, _coords9026.c_str());
        SkString _coords9089("float2(half2(rect.z, 0.5))");
        SkString _sample9089 = this->invokeChild(1, args, _coords9089.c_str());
        fragBuilder->codeAppendf(
                R"SkSL(
    xCoverage = (1.0 - %s.w) - %s.w;)SkSL",
                _sample9026.c_str(), _sample9089.c_str());
        SkString _coords9153("float2(half2(rect.y, 0.5))");
        SkString _sample9153 = this->invokeChild(1, args, _coords9153.c_str());
        SkString _coords9216("float2(half2(rect.w, 0.5))");
        SkString _sample9216 = this->invokeChild(1, args, _coords9216.c_str());
        fragBuilder->codeAppendf(
                R"SkSL(
    yCoverage = (1.0 - %s.w) - %s.w;
})SkSL",
                _sample9153.c_str(), _sample9216.c_str());
        SkString _sample9285 = this->invokeChild(0, args);
        fragBuilder->codeAppendf(
                R"SkSL(
half4 inputColor = %s;
%s = (inputColor * xCoverage) * yCoverage;
)SkSL",
                _sample9285.c_str(), args.fOutputColor);
    }
