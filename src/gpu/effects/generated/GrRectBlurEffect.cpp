...
public:
    ...
    void emitCode(EmitArgs& args) override {
        ...
        if (!highp) {
            rectHVar = args.fUniformHandler->addUniform(kFragment_GrShaderFlag, kFloat4_GrSLType,
                                                        "rectH");
        }
        sigmaVar =
        ...
        fragBuilder->codeAppendf(
                ...
                "half(%s.x - sk_FragCoord.x);\n    half rDiff = half(sk_FragCoord.x - %s.z);\n    "
                ...
                rectFVar.isValid() ? args.fUniformHandler->getUniformCStr(rectFVar) : "float4(0)",
                rectHVar.isValid() ? args.fUniformHandler->getUniformCStr(rectHVar) : "float4(0)",
                rectHVar.isValid() ? args.fUniformHandler->getUniformCStr(rectHVar) : "float4(0)");
        fragBuilder->codeAppendf(
                "\n    if (x > 0.5) {\n        xCoverage = 0.5625 - ((x3 / 6.0 - (3.0 * x2) * "
                "0.25) + 1.125 * x);\n    } else if (x > -0.5) {\n        xCoverage = 0.5 - (0.75 "
                "* x - x3 / 3.0);\n    } else {\n        xCoverage = 0.4375 + ((-x3 / 6.0 - (3.0 * "
                "x2) * 0.25) - 1.125 * x);\n    }\n}\nhalf y;\n@if (highp) {\n    float tDiff = "
        ...
        fragBuilder->codeAppendf(
                ...
                "yCoverage;\nif (y > 1.5) {\n    yCoverage = 0.0;\n} else if (y < -1.5) {\n    "
                "yCoverage = 1.0;\n} else {\n    half y2 = y * y;\n    half y3 = y2 * y;\n    if "
                "(y > 0.5) {\n        yCoverage = 0.5625 - ((y3 / 6.0 - (3.0 * y2) * 0.25) + 1.125 "
                "* y);\n    } else if (y > -0.5) {\n        yCoverage = 0.5 - (0.75 * y - y3 / "
                "3.0);\n    } else {\n        yCoverage = 0.4375 + ((-y3 / 6.0 - (3.0 * y2) * "
                "0.25) - 1.125 * y);\n    }\n}\n%s = (%s * xCovera",
                rectHVar.isValid() ? args.fUniformHandler->getUniformCStr(rectHVar) : "float4(0)",
                args.fOutputColor, args.fInputColor);
        fragBuilder->codeAppendf("ge) * yCoverage;\n");
    }
