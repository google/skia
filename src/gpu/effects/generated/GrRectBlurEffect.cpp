...
@if (%s) {
    ...
    @if (highp) {
        ...
        xy = max(half2(float2(%s.xy) - pos), half2(pos - float2(%s.zw)));
                ...
                rectHVar.isValid() ? args.fUniformHandler->getUniformCStr(rectHVar) : "half4(0)");
        SkString _coords8315("float2(half2(xy.x, 0.5))");
        SkString _sample8315 = this->invokeChild(1, args, _coords8315.c_str());
        fragBuilder->codeAppendf(
                R"SkSL(
    xCoverage = %s.w;)SkSL",
                _sample8315.c_str());
        SkString _coords8373("float2(half2(xy.y, 0.5))");
        SkString _sample8373 = this->invokeChild(1, args, _coords8373.c_str());
        fragBuilder->codeAppendf(
                R"SkSL(
    yCoverage = %s.w;
} else {
    ...
    @if (highp) {
        ...
        rect.zw = half2(pos - float2(%s.zw));
                ...
                rectHVar.isValid() ? args.fUniformHandler->getUniformCStr(rectHVar) : "half4(0)",
                rectHVar.isValid() ? args.fUniformHandler->getUniformCStr(rectHVar) : "half4(0)");
        SkString _coords9692("float2(half2(rect.x, 0.5))");
        SkString _sample9692 = this->invokeChild(1, args, _coords9692.c_str());
        SkString _coords9755("float2(half2(rect.z, 0.5))");
        SkString _sample9755 = this->invokeChild(1, args, _coords9755.c_str());
        fragBuilder->codeAppendf(
                R"SkSL(
    xCoverage = (1.0 - %s.w) - %s.w;)SkSL",
                _sample9692.c_str(), _sample9755.c_str());
        SkString _coords9819("float2(half2(rect.y, 0.5))");
        SkString _sample9819 = this->invokeChild(1, args, _coords9819.c_str());
        SkString _coords9882("float2(half2(rect.w, 0.5))");
        SkString _sample9882 = this->invokeChild(1, args, _coords9882.c_str());
        fragBuilder->codeAppendf(
                R"SkSL(
    yCoverage = (1.0 - %s.w) - %s.w;
})SkSL",
                _sample9819.c_str(), _sample9882.c_str());
        SkString _sample9951 = this->invokeChild(0, args);
        fragBuilder->codeAppendf(
                R"SkSL(
half4 inputColor = %s;
%s = (inputColor * xCoverage) * yCoverage;
)SkSL",
                _sample9951.c_str(), args.fOutputColor);
    }
