...
@if (%s) {
    ...
    @if (highp) {
        ...
        xy = max(half2(float2(%s.xy) - pos), half2(pos - float2(%s.zw)));
                ...
                rectHVar.isValid() ? args.fUniformHandler->getUniformCStr(rectHVar) : "half4(0)");
        SkString _coords8322("float2(half2(xy.x, 0.5))");
        SkString _sample8322 = this->invokeChild(1, args, _coords8322.c_str());
        fragBuilder->codeAppendf(
                R"SkSL(
    xCoverage = %s.w;)SkSL",
                _sample8322.c_str());
        SkString _coords8380("float2(half2(xy.y, 0.5))");
        SkString _sample8380 = this->invokeChild(1, args, _coords8380.c_str());
        fragBuilder->codeAppendf(
                R"SkSL(
    yCoverage = %s.w;
} else {
    ...
    @if (highp) {
        ...
        rect.zw = half2(pos - float2(%s.zw));
                ...
                rectHVar.isValid() ? args.fUniformHandler->getUniformCStr(rectHVar) : "half4(0)",
                rectHVar.isValid() ? args.fUniformHandler->getUniformCStr(rectHVar) : "half4(0)");
        SkString _coords9699("float2(half2(rect.x, 0.5))");
        SkString _sample9699 = this->invokeChild(1, args, _coords9699.c_str());
        SkString _coords9762("float2(half2(rect.z, 0.5))");
        SkString _sample9762 = this->invokeChild(1, args, _coords9762.c_str());
        fragBuilder->codeAppendf(
                R"SkSL(
    xCoverage = (1.0 - %s.w) - %s.w;)SkSL",
                _sample9699.c_str(), _sample9762.c_str());
        SkString _coords9826("float2(half2(rect.y, 0.5))");
        SkString _sample9826 = this->invokeChild(1, args, _coords9826.c_str());
        SkString _coords9889("float2(half2(rect.w, 0.5))");
        SkString _sample9889 = this->invokeChild(1, args, _coords9889.c_str());
        fragBuilder->codeAppendf(
                R"SkSL(
    yCoverage = (1.0 - %s.w) - %s.w;
})SkSL",
                _sample9826.c_str(), _sample9889.c_str());
        SkString _sample9958 = this->invokeChild(0, args);
        fragBuilder->codeAppendf(
                R"SkSL(
half4 inputColor = %s;
%s = (inputColor * xCoverage) * yCoverage;
)SkSL",
                _sample9958.c_str(), args.fOutputColor);
    }
