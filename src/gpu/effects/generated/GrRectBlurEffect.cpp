...
@if (%s) {
    ...
    @if (highp) {
        ...
        xy = max(half2(float2(%s.xy) - pos), half2(pos - float2(%s.zw)));
                ...
                rectHVar.isValid() ? args.fUniformHandler->getUniformCStr(rectHVar) : "half4(0)");
        SkString _coords7658("float2(half2(xy.x, 0.5))");
        SkString _sample7658 = this->invokeChild(1, args, _coords7658.c_str());
        fragBuilder->codeAppendf(
                R"SkSL(
    xCoverage = %s.w;)SkSL",
                _sample7658.c_str());
        SkString _coords7716("float2(half2(xy.y, 0.5))");
        SkString _sample7716 = this->invokeChild(1, args, _coords7716.c_str());
        fragBuilder->codeAppendf(
                R"SkSL(
    yCoverage = %s.w;
} else {
    ...
    @if (highp) {
        ...
        rect.zw = half2(pos - float2(%s.zw));
                ...
                rectHVar.isValid() ? args.fUniformHandler->getUniformCStr(rectHVar) : "half4(0)",
                rectHVar.isValid() ? args.fUniformHandler->getUniformCStr(rectHVar) : "half4(0)");
        SkString _coords9035("float2(half2(rect.x, 0.5))");
        SkString _sample9035 = this->invokeChild(1, args, _coords9035.c_str());
        SkString _coords9098("float2(half2(rect.z, 0.5))");
        SkString _sample9098 = this->invokeChild(1, args, _coords9098.c_str());
        fragBuilder->codeAppendf(
                R"SkSL(
    xCoverage = (1.0 - %s.w) - %s.w;)SkSL",
                _sample9035.c_str(), _sample9098.c_str());
        SkString _coords9162("float2(half2(rect.y, 0.5))");
        SkString _sample9162 = this->invokeChild(1, args, _coords9162.c_str());
        SkString _coords9225("float2(half2(rect.w, 0.5))");
        SkString _sample9225 = this->invokeChild(1, args, _coords9225.c_str());
        fragBuilder->codeAppendf(
                R"SkSL(
    yCoverage = (1.0 - %s.w) - %s.w;
})SkSL",
                _sample9162.c_str(), _sample9225.c_str());
        SkString _sample9294 = this->invokeChild(0, args);
        fragBuilder->codeAppendf(
                R"SkSL(
half4 inputColor = %s;
%s = (inputColor * xCoverage) * yCoverage;
)SkSL",
                _sample9294.c_str(), args.fOutputColor);
    }
