...
@if (%s) {
    ...
    @if (highp) {
        ...
        xy = max(half2(float2(%s.xy) - pos), half2(pos - float2(%s.zw)));
                ...
                rectHVar.isValid() ? args.fUniformHandler->getUniformCStr(rectHVar) : "half4(0)");
        SkString _coords8393("float2(half2(xy.x, 0.5))");
        SkString _sample8393 = this->invokeChild(1, args, _coords8393.c_str());
        fragBuilder->codeAppendf(
                R"SkSL(
    xCoverage = %s.w;)SkSL",
                _sample8393.c_str());
        SkString _coords8451("float2(half2(xy.y, 0.5))");
        SkString _sample8451 = this->invokeChild(1, args, _coords8451.c_str());
        fragBuilder->codeAppendf(
                R"SkSL(
    yCoverage = %s.w;
} else {
    ...
    @if (highp) {
        ...
        rect.zw = half2(pos - float2(%s.zw));
                ...
                rectHVar.isValid() ? args.fUniformHandler->getUniformCStr(rectHVar) : "half4(0)",
                rectHVar.isValid() ? args.fUniformHandler->getUniformCStr(rectHVar) : "half4(0)");
        SkString _coords9770("float2(half2(rect.x, 0.5))");
        SkString _sample9770 = this->invokeChild(1, args, _coords9770.c_str());
        SkString _coords9833("float2(half2(rect.z, 0.5))");
        SkString _sample9833 = this->invokeChild(1, args, _coords9833.c_str());
        fragBuilder->codeAppendf(
                R"SkSL(
    xCoverage = (1.0 - %s.w) - %s.w;)SkSL",
                _sample9770.c_str(), _sample9833.c_str());
        SkString _coords9897("float2(half2(rect.y, 0.5))");
        SkString _sample9897 = this->invokeChild(1, args, _coords9897.c_str());
        SkString _coords9960("float2(half2(rect.w, 0.5))");
        SkString _sample9960 = this->invokeChild(1, args, _coords9960.c_str());
        fragBuilder->codeAppendf(
                R"SkSL(
    yCoverage = (1.0 - %s.w) - %s.w;
})SkSL",
                _sample9897.c_str(), _sample9960.c_str());
        SkString _sample10029 = this->invokeChild(0, args);
        fragBuilder->codeAppendf(
                R"SkSL(
half4 inputColor = %s;
%s = (inputColor * xCoverage) * yCoverage;
)SkSL",
                _sample10029.c_str(), args.fOutputColor);
    }
