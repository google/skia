...
public:
    ...
    void emitCode(EmitArgs& args) override {
        ...
        Var _prevRect(kNo_Modifier, DSLType(kFloat4_Type), "prevRect");
        DeclareGlobal(_prevRect,
                      Float4(prevRect.left(), prevRect.top(), prevRect.right(), prevRect.bottom()));
        Var _coverage(kNo_Modifier, DSLType(kHalf_Type), "coverage");
        Var _dists4(kNo_Modifier, DSLType(kHalf4_Type), "dists4");
        Var _dists2(kNo_Modifier, DSLType(kHalf2_Type), "dists2");
        Declare(_coverage);
        StaticSwitch(
                _edgeType,
                Case(0),
                Case(2,
                     _coverage =
                             Half(Select(All(GreaterThan(Float4(Swizzle(sk_FragCoord(), X, Y),
                                         ...
                                         /*If False:*/ 0)),
                     Break()),
                Default(Declare(_dists4,
                                Clamp(Half4(1.0f, 1.0f, -1.0f, -1.0f) *
                                              Half4(Swizzle(sk_FragCoord(), X, Y, X, Y) -
                                                    _rectUniform),
                                      0.0f,
                                      1.0f)),
                        Declare(_dists2, (Swizzle(_dists4, X, Y) + Swizzle(_dists4, Z, W)) - 1.0f),
                        _coverage = _dists2.x() * _dists2.y()));
