...
public:
    ...
    void emitCode(EmitArgs& args) override {
        ...
        Var _mask_color(kNo_Modifier, DSLType(kHalf4_Type), "mask_color");
        Var _scale(kNo_Modifier, DSLType(kHalf_Type), "scale");
        Var _1_scale(kNo_Modifier, DSLType(kHalf_Type), "scale");
        Declare(_color, SampleChild(0));
        Declare(_mask_color, SampleChild(1));
        If(_mask_color.w() < 0.5f,
           /*Then:*/
           Block(If(_color.w() > _outerThreshold,
                    /*Then:*/ Block(Declare(_scale, _outerThreshold / _color.w()),
                                    Swizzle(_color, X, Y, Z) *= _scale,
                                    _color.w() = _outerThreshold))),
           /*Else:*/
           If(_color.w() < _innerThreshold,
              /*Then:*/ Block(
                      Declare(_1_scale, _innerThreshold / Max(0.0010000000474974513f, _color.w())),
