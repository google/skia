...
public:
    ...
    void emitCode(EmitArgs& args) override {
        ...
        Var _x(kNo_Modifier, DSLType(kUInt_Type), "x");
        Var _y(kNo_Modifier, DSLType(kUInt_Type), "y");
        Var _m(kNo_Modifier, DSLType(kUInt_Type), "m");
        Var _bits(kNo_Modifier, DSLType(kHalf4_Type), "bits");
        Declare(_color, SampleChild(0));
        Declare(_value);
        StaticIf(sk_Caps.integerSupport,
                 /*Then:*/
                 Block(Declare(_x, UInt(sk_FragCoord().x())),
                       Declare(_y, UInt(sk_FragCoord().y()) ^ _x),
                       Declare(_m,
                               (((((_y & 1u) << 5u | (_x & 1u) << 4u) | (_y & 2u) << 2u) |
                                 (_x & 2u) << 1u) |
                                (_y & 4u) >> 1u) |
                                       (_x & 4u) >> 2u),
                 ...
                 /*Else:*/
                 Block(Declare(_bits,
                               Mod(Half4(Swizzle(sk_FragCoord(), Y, X, Y, X)),
                                   Half4(2.0f, 2.0f, 4.0f, 4.0f))),
