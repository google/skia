...
public:
    ...
    void emitCode(EmitArgs& args) override {
        ...
        Var _c(kNo_Modifier, DSLType(kHalf4_Type), "c", SampleChild(0));
        Var _p(kNo_Modifier,
               DSLType(kHalf4_Type),
               "p",
               Select(_c.y() < _c.z(),
        ...
        Var _eps(kNo_Modifier, DSLType(kHalf_Type), "eps", 9.9999997473787516e-05f);
        Var _pmV(kNo_Modifier, DSLType(kHalf_Type), "pmV", _q.x());
        Var _pmC(kNo_Modifier, DSLType(kHalf_Type), "pmC", _pmV - Min(_q.y(), _q.z()));
        Var _pmL(kNo_Modifier, DSLType(kHalf_Type), "pmL", _pmV - _pmC * 0.5f);
        Var _0_H(kNo_Modifier,
                 DSLType(kHalf_Type),
                 "H",
                 Abs(_q.w() + (_q.y() - _q.z()) / (_pmC * 6.0f + _eps)));
        Var _0_S(kNo_Modifier,
                 DSLType(kHalf_Type),
                 "S",
                 _pmC / ((_c.w() + _eps) - Abs(_pmL * 2.0f - _c.w())));
        Var _0_L(kNo_Modifier, DSLType(kHalf_Type), "L", _pmL / (_c.w() + _eps));
        Declare(_c);
        Declare(_p);
        Declare(_q);
        Declare(_eps);
