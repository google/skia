...
public:
    ...
    void emitCode(EmitArgs& args) override {
        ...
        if (_outer.childProcessor(0)) {
            _sample1099 = this->invokeChild(0, _input1099.c_str(), args);
        } else {
            _sample1099.swap(_input1099);
        }
        fragBuilder->codeAppendf(
                R"SkSL(half4 inColor = %s;)SkSL", _sample1099.c_str());
        SkString _input1150("inColor");
        SkString _sample1150;
        _sample1150 = this->invokeChild(1, _input1150.c_str(), args);
        SkString _input1172("inColor");
        SkString _sample1172;
        if (_outer.childProcessor(2)) {
            _sample1172 = this->invokeChild(2, _input1172.c_str(), args);
        } else {
            _sample1172.swap(_input1172);
        }
        fragBuilder->codeAppendf(
                R"SkSL(
%s = mix(%s, %s, %s);
