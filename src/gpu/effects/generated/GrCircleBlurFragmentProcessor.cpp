...
half dist = length(vec) + (0.5 - %s.z) * %s.w;)SkSL",
                ...
                args.fUniformHandler->getUniformCStr(circleDataVar),
                args.fUniformHandler->getUniformCStr(circleDataVar));
        SkString _sample13776 = this->invokeChild(0, args);
        fragBuilder->codeAppendf(
                R"SkSL(
half4 inputColor = %s;)SkSL",
                _sample13776.c_str());
        SkString _coords13824("float2(half2(dist, 0.5))");
        SkString _sample13824 = this->invokeChild(1, args, _coords13824.c_str());
        fragBuilder->codeAppendf(
                R"SkSL(
%s = inputColor * %s.w;
)SkSL",
                args.fOutputColor, _sample13824.c_str());
    }
