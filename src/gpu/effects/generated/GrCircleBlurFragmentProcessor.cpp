...
public:
    ...
    void emitCode(EmitArgs& args) override {
        ...
        fragBuilder->codeAppendf(
                ...
                args.fUniformHandler->getUniformCStr(circleDataVar),
                args.fUniformHandler->getUniformCStr(circleDataVar));
        SkString _input13190 = SkStringPrintf("%s", args.fInputColor);
        SkString _sample13190;
        if (_outer.inputFP_index >= 0) {
            _sample13190 = this->invokeChild(_outer.inputFP_index, _input13190.c_str(), args);
        } else {
            _sample13190 = "half4(1)";
        }
        fragBuilder->codeAppendf(
                "\nhalf4 inputColor = %s ? %s : %s;\n%s = inputColor * sample(%s, "
                "float2(half2(dist, 0.5))).%s.w;\n",
                _outer.inputFP_index >= 0 ? "true" : "false", _sample13190.c_str(),
