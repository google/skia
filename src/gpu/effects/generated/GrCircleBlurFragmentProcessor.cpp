...
static std::unique_ptr<GrFragmentProcessor> create_profile_effect(GrRecordingContext* rContext,
    ...
    GrSwizzle swizzle =
            rContext->priv().caps()->getReadSwizzle(proxy->backendFormat(), GrColorType::kAlpha_8);
...
half dist = length(vec) + (0.5 - %s.z) * %s.w;)SkSL",
                ...
                args.fUniformHandler->getUniformCStr(circleDataVar),
                args.fUniformHandler->getUniformCStr(circleDataVar));
        SkString _sample14141 = this->invokeChild(0, args);
        fragBuilder->codeAppendf(
                R"SkSL(
half4 inputColor = %s;)SkSL",
                _sample14141.c_str());
        SkString _coords14189("float2(half2(dist, 0.5))");
        SkString _sample14189 = this->invokeChild(1, args, _coords14189.c_str());
        fragBuilder->codeAppendf(
                R"SkSL(
%s = inputColor * %s.w;
)SkSL",
                args.fOutputColor, _sample14189.c_str());
    }
