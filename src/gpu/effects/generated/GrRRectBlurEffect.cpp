...
static uint8_t eval_sub(float top, int y, const uint8_t* integral, int integralSize, float sixSigma) {
    if (top < 0) {
        return 0;
    }
...
static uint8_t eval(int x, int y, const std::vector<float>& topVec,
    ...
    for (int i = 0; i < kernelSize; ++i, ++xSampleLoc) {
        ...
        uint8_t tmp = eval_sub(topVec[xSampleLoc], y, integral, integralSize, sixSigma);
        if (tmp != 255) {
            allWhite = false;
        }
        kernelSum += kernel[i];
...
//    }
    ...
    return accum + 0.5f;
#else
    return eval_sub(topVec[x], y, integral, integralSize, sixSigma);
#endif
}
