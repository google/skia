...
public:
    ...
    void emitCode(EmitArgs& args) override {
        ...
        Var pmConversion(
                kConst_Modifier, DSLType(kInt_Type), "pmConversion", Int((int)_outer.pmConversion));
        Declare(pmConversion);
        ...
        Switch(pmConversion,
               Case(0,
                    Swizzle(color, X, Y, Z) =
                            Floor((Swizzle(color, X, Y, Z) * color.w()) * 255.0f + 0.5f) / 255.0f,
                    Break()),
               Case(1,
                    Swizzle(color, X, Y, Z) = Select(
                            color.w() <= 0.0f,
                            /*If True:*/ Half3(0.0f),
                            /*If False:*/
                                    Floor((Swizzle(color, X, Y, Z) / color.w()) * 255.0f + 0.5f) /
                                    255.0f),
                    Break()));
        Return(color);
