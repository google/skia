...
public:
    ...
    void emitCode(EmitArgs& args) override {
        [[maybe_unused]] const GrConfigConversionEffect& _outer =
                args.fFp.cast<GrConfigConversionEffect>();

        using namespace SkSL::dsl;
        StartFragmentProcessor(this, &args);
        [[maybe_unused]] const auto& pmConversion = _outer.pmConversion;
        Var _pmConversion(kConst_Modifier, DSLType(kInt_Type), "pmConversion");
        Declare(_pmConversion, Int((int)pmConversion));

        args.fFragBuilder->forceHighPrecision();
        Var _color(kNo_Modifier, DSLType(kHalf4_Type), "color");
        Declare(_color, Floor(SampleChild(0) * 255.0f + 0.5f) / 255.0f);
        StaticSwitch(_pmConversion,
                     Case(0,
                          Swizzle(_color, X, Y, Z) =
                                  Floor((Swizzle(_color, X, Y, Z) * _color.w()) * 255.0f + 0.5f) /
                                  255.0f,
                          Break()),
                     Case(1,
                          Swizzle(_color, X, Y, Z) = Select(
                                  _color.w() <= 0.0f,
                                  /*If True:*/ Half3(0.0f),
                                  /*If False:*/
                                          Floor((Swizzle(_color, X, Y, Z) / _color.w()) * 255.0f +
                                                0.5f) /
                                          255.0f),
                          Break()));
        Return(_color);
