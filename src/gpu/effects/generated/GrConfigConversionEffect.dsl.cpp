...
public:
    ...
    void emitCode(EmitArgs& args) override {
        ...
        Var _pmConversion(
                kConst_Modifier, DSLType(kInt_Type), "pmConversion", Int((int)pmConversion));
        Declare(_pmConversion);

        args.fFragBuilder->forceHighPrecision();
        Var _color(kNo_Modifier,
                   DSLType(kHalf4_Type),
                   "color",
                   Floor(SampleChild(0) * 255.0f + 0.5f) / 255.0f);
        Declare(_color);
        Switch(_pmConversion,
               Case(0,
                    Swizzle(_color, X, Y, Z) =
                            Floor((Swizzle(_color, X, Y, Z) * _color.w()) * 255.0f + 0.5f) / 255.0f,
                    Break()),
               Case(1,
