...
public:
    ...
    void emitCode(EmitArgs& args) override {
        ...
        static const char bar[4] = { 'x', 'y', 'z', 'w' };

        fragBuilder->codeAppendf(
            "half4 yuvOne = half4(tmp%d.%c, tmp%d.%c, tmp%d.%c, 1.0) * %s;",
            _outer.foo(0).fIndex, bar[(int)_outer.foo(0).fChannel],
            _outer.foo(1).fIndex, bar[(int)_outer.foo(1).fChannel],
            _outer.foo(2).fIndex, bar[(int)_outer.foo(2).fChannel],
            args.fUniformHandler->getUniformCStr(fColorSpaceMatrixVar));


        if (_outer.foo(3).fIndex >= 0) {
            fragBuilder->codeAppendf(
                "float a = tmp%d.%c;", _outer.foo(3).fIndex, bar[(int)_outer.foo(3).fChannel]);
        } else {
...
void GrYUVtoRGBEffect::onGetGLSLProcessorKey(const GrShaderCaps& caps,
    ...
    uint32_t packed = 0;
    for (int i = 0; i < 4; ++i) {
        if (this->foo(i).fIndex < 0) {
            continue;
        }

        uint8_t index = this->foo(i).fIndex;
        uint8_t chann = (uint8_t) this->foo(i).fChannel;
