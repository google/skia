...
static void border_colors(const GrYUVATextureProxies& yuvaProxies, float planeBorders[4][4]) {
    ...
    for (int i = 0; i < SkYUVAInfo::kYUVAChannelCount; ++i) {
        const auto [plane, channel] = yuvaProxies.yuvaLocations()[i];
        if (plane == -1) {
            return;
        }
...
std::unique_ptr<GrFragmentProcessor> GrYUVtoRGBEffect::Make(const GrYUVATextureProxies& yuvaProxies,
    ...
    for (int i = 0; i < numPlanes; ++i) {
        ...
        if (ssx > 1 || ssy > 1) {
            ...
            scaleX = 1.f/ssx;
            scaleY = 1.f/ssy;
            *planeMatrix.writable() = SkMatrix::Scale(scaleX, scaleY);
            if (subset) {
    ...
    auto fp = std::unique_ptr<GrFragmentProcessor>(
            new GrYUVtoRGBEffect(planeFPs, numPlanes, yuvaProxies.yuvaLocations(), snap,
