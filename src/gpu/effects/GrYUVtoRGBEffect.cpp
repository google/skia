...
GrYUVtoRGBEffect::GrYUVtoRGBEffect(std::unique_ptr<GrFragmentProcessor> planeFPs[4],
    ...
    if (fSnap[0] || fSnap[1]) {
        ...
        for (int i = 0; i < numPlanes; ++i) {
            planeFPs[i]->setSampledWithExplicitCoords();
        }
    }
    for (int i = 0; i < numPlanes; ++i) {
    ...
    public:
        ...
        void emitCode(EmitArgs& args) override {
            ...
            if (yuvEffect.fSnap[0] || yuvEffect.fSnap[1]) {
                SkString inCoord = fragBuilder->ensureCoords2D(
                        args.fTransformedCoords[0].fVaryingPoint, args.fFp.sampleMatrix());
                fragBuilder->codeAppendf("float2 snappedCoords = %s;", inCoord.c_str());
...
GrYUVtoRGBEffect::GrYUVtoRGBEffect(const GrYUVtoRGBEffect& src)
    ...
    for (int i = 0; i < numPlanes; ++i) {
        this->registerChildProcessor(src.childProcessor(i).clone());
    }
    if (fSnap[0] || fSnap[1]) {
        for (int i = 0; i < numPlanes; ++i) {
            this->childProcessor(i).setSampledWithExplicitCoords();
        }
        this->addCoordTransform(&fTransform);
    }
}
