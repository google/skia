...
public:
    ...
    static const SkRect MakeTexelDomainForSampler(const SkIRect& texelRect,
                                                  GrSamplerState::Filter filter,
                                                  Mode modeX,
                                                  Mode modeY) {
        // For Clamp and decal modes, inset by half a texel when using any filtering. Use the
        // original texel domain when it's nearest.
        if (filter == GrSamplerState::Filter::kNearest) {
            return MakeTexelDomain(texelRect);
        }

        SkScalar insetX = ((modeX == kClamp_Mode || modeY == kDecal_Mode) && texelRect.width() > 0)
                ? SK_ScalarHalf : 0;
        SkScalar insetY = ((modeX == kClamp_Mode || modeY == kDecal_Mode) && texelRect.height() > 0)
                ? SK_ScalarHalf : 0;
