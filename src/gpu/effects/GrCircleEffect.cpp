...
public:
    ...
    void emitCode(EmitArgs& args) override {
        ...
        fragBuilder->codeAppendf(
                "mediump half2 prevCenter;\nmediump half prevRadius = %f;\nmediump half d;\n@if "
                "(%d == 2 || %d == 3) {\n    d = (highfloat(length((%s.xy - "
                "half2(sk_FragCoord.xy)) * %s.w)) - 1.0) * %s.z;\n} else {\n    d = half((1.0 - "
                "highfloat(length((%s.xy - half2(sk_FragCoord.xy)) * %s.w))) * "
                "highfloat(%s.z));\n}\n@if ((%d == 1 || %d == 3) || %d == 4) {\n    d = "
                "half(clamp(highfloat(d), 0.0, 1.0));\n} else {\n    d = half(highfloat(d) > 0.5 ? "
                "1.0 : 0.0);\n}\n%s = %s * highfloat(d);\n",
