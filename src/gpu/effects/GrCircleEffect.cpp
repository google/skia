...
class GrGLSLCircleEffect : public GrGLSLFragmentProcessor {
 public:
  ...
  void emitCode(EmitArgs& args) override {
    ...
    (void)radius;
    prevRadius = -1.0;
    fCircleVar = args.fUniformHandler->addUniform(kFragment_GrShaderFlag,
                                                  kHalf4_GrSLType, "circle");
    fragBuilder->codeAppendf(
        "half2 prevCenter;\nhalf prevRadius = %f;\nhalf d;\n@if (%d == 2 || %d "
        "== 3) {\n    d = half((length((float2(%s.xy) - sk_FragCoord.xy) * "
        "float(%s.w)) - 1.0) * float(%s.z));\n} else {\n    d = half((1.0 - "
        "length((float2(%s.xy) - sk_FragCoord.xy) * float(%s.w))) * "
        "float(%s.z));\n}\n@if ((%d == 1 || %d == 3) || %d == 4) {\n    d = "
        "clamp(d, 0.0, 1.0);\n} else {\n    d = d > 0.5 ? 1.0 : 0.0;\n}\n%s = "
        "%s * d;\n",
  ...
  void onSetData(const GrGLSLProgramDataManager& pdman,
    ...
    if (radius != prevRadius || center != prevCenter) {
      ...
      if (GrProcessorEdgeTypeIsInverseFill((GrClipEdgeType)edgeType)) {
        ...
        // When the radius is 0.5 effectiveRadius is 0 which causes an inf * 0
        // in the shader.
...
#if GR_TEST_UTILS
std::unique_ptr<GrFragmentProcessor> GrCircleEffect::TestCreate(
