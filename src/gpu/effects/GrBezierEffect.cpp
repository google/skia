...
public:
    ...
    void setData(const GrGLSLProgramDataManager& pdman, const GrPrimitiveProcessor& primProc,
        ...
        float klmMatrix[3 * 3];
        GrGLSLGetMatrix<3>(klmMatrix, ce.devKLM());
        pdman.setMatrix3f(fDevKLMUniform, klmMatrix);
...
void GrGLCubicEffect::onEmitCode(EmitArgs& args, GrGPArgs* gpArgs) {
    ...
    if (kFillAA_GrProcessorEdgeType == fEdgeType || kHairlineAA_GrProcessorEdgeType == fEdgeType) {
        ...
        vertBuilder->codeAppendf("highp mat3x2 gklm = mat3x2(transpose(%s));", devkLMMatrixName);
        vertBuilder->codeAppendf("%s = vec4(3 * k * gklm[0], -m * gklm[1] - l * gklm[2]);",
                                 gradCoeffs.vsOut());
    }
...
GrCubicEffect::GrCubicEffect(GrColor color, const SkMatrix& viewMatrix, const SkMatrix& devKLM,
                             GrPrimitiveEdgeType edgeType)
    : fColor(color)
    , fViewMatrix(viewMatrix)
    , fDevKLM(devKLM)
