...
const GrColor kUmbraColor = GrColorPackRGBA(0, 0, 255, 64);
const GrColor kPenumbraColor = GrColorPackRGBA(0, 0, 0, 64);

GrSpotShadowTessellator::GrSpotShadowTessellator(const SkMatrix& viewMatrix, const SkPath& path, 
                                                 SkScalar scale, const SkVector& translate, 
                                                 SkScalar radius, bool transparent)
    : fScale(scale)
...
void GrSpotShadowTessellator::mapPoints(SkScalar scale, SkVector xlate, SkPoint* pts, int count) {
    for (int i = 0; i < count; ++i) {
        pts[i] *= scale;
        pts[i] += xlate;
    }
...
void GrSpotShadowTessellator::handleLine(const SkPoint& p) {
    ...
    if (fInitPoints.count() == 2) {
        ...
        this->addInnerPoint(fFirstPoint, kUmbraColor, fRadius);
...
void GrSpotShadowTessellator::handleLine(SkScalar scale, SkVector xlate, SkPoint p) {
...
void GrSpotShadowTessellator::handleQuad(SkScalar scale, SkVector xlate, SkPoint pts[3]) {
    this->mapPoints(scale, xlate, pts, 3);
    this->handleQuad(pts);
}

void GrSpotShadowTessellator::handleCubic(SkScalar scale, SkVector xlate, SkPoint pts[4]) {
...
void GrSpotShadowTessellator::handleConic(SkScalar scale, SkVector xlate, 
...
void GrSpotShadowTessellator::addEdge(const SkPoint& nextPoint, const SkVector& nextNormal) {
    // add next quad
    this->addInnerPoint(nextPoint, kUmbraColor, fRadius);
