...
std::unique_ptr<GrFragmentProcessor> GrRRectBlurEffect::Make(
    GrRecordingContext* context, float sigma, float xformedSigma,
  ...
  // Make sure we can successfully ninepatch this rrect -- the blur sigma has to
  // be sufficiently small relative to both the size of the corner radius and
  // the width (and height) of the rrect.
  SkRRect rrectToDraw;
  SkISize size;
  ...
  bool ninePatchable = SkComputeBlurredRRectParams(
      srcRRect, devRRect, SkRect::MakeEmpty(), sigma, xformedSigma,
      &rrectToDraw, &size, ignored, ignored, ignored, ignored, &ignoredSize,
      &ignoredSize, &ignored32);
  if (!ninePatchable) {
    return nullptr;
  }
  ...
  return std::unique_ptr<GrFragmentProcessor>(new GrRRectBlurEffect(
      xformedSigma, devRRect.getBounds(),
...
class GrGLSLRRectBlurEffect : public GrGLSLFragmentProcessor {
 public:
  ...
  void emitCode(EmitArgs& args) override {
    ...
    fragBuilder->codeAppendf(
        ...
        "threshold = %s + 2.0 * %s;\nhalf2 middle = half2((%s.zw - %s.xy) - "
        "float(2.0 * threshold));\nif (translatedFragPos.x >= threshold && "
        "translatedFragPos.x < middle.x + threshold) {\n    "
        "translatedFragPos.x = threshold;\n} else if (translatedFragPos.x >= "
        "middle.x + threshold) {\n    translatedFragPos.x -= middle.x - "
        "1.0;\n}\nif (translatedFragPos.y > threshold && translatedFragPos.y < "
 ...
 private:
  void onSetData(const GrGLSLProgramDataManager& pdman,
    ...
    pdman.set4f(proxyRect, outset.fLeft, outset.fTop, outset.fRight,
                outset.fBottom);
  }
...
const GrFragmentProcessor::TextureSampler& GrRRectBlurEffect::onTextureSampler(
    int index) const {
  return IthTextureSampler(index, fNinePatchSampler);
}
GR_DEFINE_FRAGMENT_PROCESSOR_TEST(GrRRectBlurEffect);
#if GR_TEST_UTILS
std::unique_ptr<GrFragmentProcessor> GrRRectBlurEffect::TestCreate(
