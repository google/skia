...
static void append_index_uv_varyings(GrGLSLPrimitiveProcessor::EmitArgs& args,
    ...
    // Packing structure: texel coordinates are absolute value of input
    //                    texture index is stored as sign of both x and y
    args.fVertBuilder->codeAppendf("float2 indexTexCoords = float2(%s.x, %s.y);",
                                   inTexCoordsName, inTexCoordsName);
    args.fVertBuilder->codeAppend("float2 unormTexCoords = abs(indexTexCoords);");
    // TODO: describe this better
    // Map (-1, 1) to (0, 1)
    args.fVertBuilder->codeAppend("float2 sign = floor(float2(0.5) - 0.5*sign(indexTexCoords));");
    args.fVertBuilder->codeAppend("float texIdx = 2.0*sign.x + sign.y;");
