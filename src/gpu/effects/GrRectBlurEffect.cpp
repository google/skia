...
class GrGLSLRectBlurEffect : public GrGLSLFragmentProcessor {
 public:
  ...
  void emitCode(EmitArgs& args) override {
    ...
    (void)sigma;
    highPrecision =
        ((((abs(rect.left()) > 16000.0 || abs(rect.top()) > 16000.0) ||
           abs(rect.right()) > 16000.0) ||
          abs(rect.bottom()) > 16000.0) ||
         abs(rect.right() - rect.left()) > 16000.0) ||
        abs(rect.bottom() - rect.top()) > 16000.0;
    fRectVar = args.fUniformHandler->addUniform(kFragment_GrShaderFlag,
                                                kFloat4_GrSLType, "rect");
    if (!highPrecision) {
    ...
    fragBuilder->codeAppendf(
        ...
        "float2(width - float(%s), height - float(%s));\n    float center = "
        "float(2.0 * floor(%s / 2.0 + 0.25) - 1.0);\n    float2 wh = smallDims "
        "- float2(center, center);\n    half hcoord = "
        "half((abs(translatedPos.x - 0.5 * width) - 0.5 * wh.x) / "
        ...
        fragBuilder->getProgramBuilder()
            ...
            .c_str());
    fragBuilder->codeAppendf(
        "%s.w;\n    half vcoord = half((abs(translatedPos.y - 0.5 * height) - "
        "0.5 * wh.y) / float(%s));\n    half vlookup = texture(%s, "
        "float2(float(vcoord), 0.5)).%s.w;\n    %s = (%s * hlookup) * "
        "vlookup;\n} else {\n    half2 translatedPos = half2(sk_FragCoord.xy - "
    ...
    fragBuilder->codeAppendf(
        ...
        "half vcoord = (abs(translatedPos.y - 0.5 * height) - 0.5 * wh.y) / "
 ...
 private:
  void onSetData(const GrGLSLProgramDataManager& pdman,
    ...
    {
      pdman.set4fv(fRectVar, 1,
                   reinterpret_cast<const float*>(&(_outer.rect())));
    }
...
#if GR_TEST_UTILS
std::unique_ptr<GrFragmentProcessor> GrRectBlurEffect::TestCreate(
