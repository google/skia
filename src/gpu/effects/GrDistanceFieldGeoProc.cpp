...
public:
    GrGLDistanceFieldA8TextGeoProc()
            : fViewMatrix1(SkMatrix::InvalidMatrix())
    ...
    void onEmitCode(EmitArgs& args, GrGPArgs* gpArgs) override{
        ...
        vertBuilder->codeAppendf("%s = %s * %s;", uv.vsOut(),
                                 dfTexEffect.inTextureCoords()->fName, atlasSizeInvName);
    ...
    void setData(const GrGLSLProgramDataManager& pdman, const GrPrimitiveProcessor& proc,
        ...
        if (!dfa8gp.viewMatrix().isIdentity() && !fViewMatrix1.cheapEqualTo(dfa8gp.viewMatrix())) {
            fViewMatrix1 = dfa8gp.viewMatrix();
            float viewMatrix[3 * 3];
            GrGLSLGetMatrix<3>(viewMatrix, fViewMatrix1);
            pdman.setMatrix3f(fViewMatrixUniform1, viewMatrix);
        }
...
private:
    SkMatrix      fViewMatrix1;
    UniformHandle fViewMatrixUniform1;
#ifdef SK_GAMMA_APPLY_TO_A8
...
public:
    ...
    void onEmitCode(EmitArgs& args, GrGPArgs* gpArgs) override{
        ...
        vertBuilder->codeAppendf("%s = %s * %s;",
                                 uv.vsOut(), dfTexEffect.inTextureCoords()->fName, atlasSizeInvName);
    ...
    void setData(const GrGLSLProgramDataManager& pdman, const GrPrimitiveProcessor& proc,
        ...
        if (!dfpgp.viewMatrix().isIdentity() && !fViewMatrix1.cheapEqualTo(dfpgp.viewMatrix())) {
            fViewMatrix1 = dfpgp.viewMatrix();
            float viewMatrix[3 * 3];
            GrGLSLGetMatrix<3>(viewMatrix, fViewMatrix1);
            pdman.setMatrix3f(fViewMatrixUniform1, viewMatrix);
        }
...
public:
    ...
    void onEmitCode(EmitArgs& args, GrGPArgs* gpArgs) override{
        ...
        if (dfTexEffect.getFlags() & kBGR_DistanceFieldEffectFlag) {
            fragBuilder->codeAppendf("highp float delta = -%s.x/3.0f;", atlasSizeInvName);
        } else {
            fragBuilder->codeAppendf("highp float delta = %s.x/3.0f;", atlasSizeInvName);
        }
        if (isUniformScale) {
    ...
    void setData(const GrGLSLProgramDataManager& pdman, const GrPrimitiveProcessor& processor,
        ...
        if (!dflcd.viewMatrix().isIdentity() && !fViewMatrix1.cheapEqualTo(dflcd.viewMatrix())) {
            fViewMatrix1 = dflcd.viewMatrix();
            float viewMatrix[3 * 3];
            GrGLSLGetMatrix<3>(viewMatrix, fViewMatrix1);
            pdman.setMatrix3f(fViewMatrixUniform1, viewMatrix);
        }
