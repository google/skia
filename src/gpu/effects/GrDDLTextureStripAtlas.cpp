...
void GrDDLTextureStripAtlas::finish(GrContext* context) {
    ...
    for (int i = 0; i < fCurRow; ++i) {
        SkASSERT(fRows[i].fBitmap1.height() == fDesc.fRowHeight);

        int yPos = i * fDesc.fRowHeight;
        fAtlasBitmap->writePixels(fRows[i].fBitmap1.pixmap(), 0, yPos);
    }
...
void GrDDLTextureStripAtlas::validate() {
    ...
    for (int i = 0; i < fCurRow; ++i) {
        ...
        SkASSERT(this->searchByKey(fRows[i].fBitmap1.getGenerationID()) >= 0);
    }
