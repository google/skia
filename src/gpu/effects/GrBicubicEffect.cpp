...
void GrBicubicEffect::Impl::emitCode(EmitArgs& args) {
    ...
    if (bicubicEffect.fDirection == GrBicubicEffect::Direction::kXY) {
        fragBuilder->codeAppendf("float2 coord = %s - float2(0.5);", args.fLocalCoord);
        ...
        fragBuilder->codeAppendf("float coord = %s.%s - 0.5;", args.fLocalCoord, d);
        ...
        for (int i = 0; i < 4; ++i) {
            ...
            if (bicubicEffect.fDirection == Direction::kX) {
                coord.printf("float2(coord + %d, %s.y)", i - 1, args.fLocalCoord);
            } else {
                coord.printf("float2(%s.x, coord + %d)", args.fLocalCoord, i - 1);
            }
...
GrBicubicEffect::GrBicubicEffect(std::unique_ptr<GrFragmentProcessor> fp,
    ...
    this->registerExplicitlySampledChildProcessor(std::move(fp));
}
