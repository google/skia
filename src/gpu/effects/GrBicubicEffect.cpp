...
class GrBicubicEffect::GLSLEffect : public GrGLSLFragmentProcessor {
public:
...
void GrBicubicEffect::GLSLEffect::emitCode(EmitArgs& args) {
...
void GrBicubicEffect::GLSLEffect::onSetData(const GrGLSLProgramDataManager& pdman,
...
std::unique_ptr<GrFragmentProcessor> GrBicubicEffect::Make(sk_sp<GrSurfaceProxy> proxy,
                                                           ...
                                                           Direction direction) {
    auto fp = GrTextureEffect::Make(std::move(proxy), alphaType, matrix);
...
std::unique_ptr<GrFragmentProcessor> GrBicubicEffect::Make(
        ...
        Direction direction,
        const GrCaps& caps) {
    GrSamplerState sampler(wrapModes, GrSamplerState::Filter::kNearest);
    std::unique_ptr<GrFragmentProcessor> fp;
    fp = GrTextureEffect::Make(std::move(proxy), alphaType, matrix, sampler, caps);
    auto clamp = kPremul_SkAlphaType == alphaType ? Clamp::kPremul : Clamp::kUnpremul;
    return std::unique_ptr<GrFragmentProcessor>(
            new GrBicubicEffect(std::move(fp), direction, clamp));
}
...
std::unique_ptr<GrFragmentProcessor> GrBicubicEffect::MakeSubset(
    ...
    GrSamplerState sampler(wrapModes, GrSamplerState::Filter::kNearest);
    std::unique_ptr<GrFragmentProcessor> fp;
    fp = GrTextureEffect::MakeSubset(std::move(proxy), alphaType, matrix, sampler, subset, caps);
    auto clamp = kPremul_SkAlphaType == alphaType ? Clamp::kPremul : Clamp::kUnpremul;
    return std::unique_ptr<GrFragmentProcessor>(
            new GrBicubicEffect(std::move(fp), direction, clamp));
}

std::unique_ptr<GrFragmentProcessor> GrBicubicEffect::MakeTexelSubset(
        sk_sp<GrSurfaceProxy> proxy,
        SkAlphaType alphaType,
        const SkMatrix& matrix,
        const GrSamplerState::WrapMode wrapModes[2],
        ...
        const GrCaps& caps) {
    GrSamplerState sampler(wrapModes, GrSamplerState::Filter::kNearest);
    auto fp = GrTextureEffect::MakeTexelSubset(
            std::move(proxy), alphaType, matrix, sampler, texelSubset, caps);
    auto clamp = kPremul_SkAlphaType == alphaType ? Clamp::kPremul : Clamp::kUnpremul;
    return std::unique_ptr<GrFragmentProcessor>(
            new GrBicubicEffect(std::move(fp), direction, clamp));
}
...
GrGLSLFragmentProcessor* GrBicubicEffect::onCreateGLSLInstance() const { return new GLSLEffect(); }
...
std::unique_ptr<GrFragmentProcessor> GrBicubicEffect::TestCreate(GrProcessorTestData* d) {
    ...
    if (d->fRandom->nextBool()) {
        ...
        if (d->fRandom->nextBool()) {
            SkRect subset;
            subset.fLeft   = d->fRandom->nextSScalar1() * proxy->width();
            subset.fTop    = d->fRandom->nextSScalar1() * proxy->height();
            subset.fRight  = d->fRandom->nextSScalar1() * proxy->width();
            subset.fBottom = d->fRandom->nextSScalar1() * proxy->height();
            subset.sort();
            return MakeSubset(std::move(proxy), at, m, wm, subset, direction, *d->caps());
        }
        return Make(std::move(proxy), at, m, wm, direction, *d->caps());
    } else {
        return Make(std::move(proxy), at, m, direction);
    }
}
#endif
