...
void GrBicubicEffect::Impl::emitCode(EmitArgs& args) {
    ...
    const char* coeffs;
    fCoefficientUni = args.fUniformHandler->addUniform(&args.fFp, kFragment_GrShaderFlag, kHalf4x4_GrSLType,
                                     "coefficients", &coeffs);
    ...
    if (bicubicEffect.fDirection == GrBicubicEffect::Direction::kXY) {
        ...
        fragBuilder->codeAppendf(
                "half4 wx = %s * half4(1.0, f.x, f.x * f.x, f.x * f.x * f.x);", coeffs);
        fragBuilder->codeAppendf(
                "half4 wy = %s * half4(1.0, f.y, f.y * f.y, f.y * f.y * f.y);", coeffs);
...
void GrBicubicEffect::Impl::onSetData(const GrGLSLProgramDataManager& pdm,
    ...
    switch (bicubicEffect.fKernel) {
        case Kernel::kMitchell: {
            ...
            */
            static constexpr SkM44 kMitchell( 1.0/18.0, -9.0/18.0,  15.0/18.0,  -7.0/18.0,
                                             16.0/18.0,  0.0/18.0, -36.0/18.0,  21.0/18.0,
                                              1.0/18.0,  9.0/18.0,  27.0/18.0, -21.0/18.0,
                                              0.0/18.0,  0.0/18.0,  -6.0/18.0,   7.0/18.0);
            coeffs = &kMitchell;
        }
        break;
        case Kernel::kCatmullRom: {
            ...
            pp. 317–326.
            */
            static constexpr SkM44 kCatmullRom( 0.0, -0.5,  1.0, -0.5,
                                                1.0,  0.0, -2.5,  1.5,
                                                0.0,  0.5,  2.0, -1.5,
                                                0.0,  0.0, -0.5,  0.5);
            coeffs = &kCatmullRom;
...
void GrBicubicEffect::onGetGLSLProcessorKey(const GrShaderCaps& caps,
    ...
    uint32_t key = (static_cast<uint32_t>(fDirection) << 0)
                 | (static_cast<uint32_t>(fClamp)     << 2);
    b->add32(key);
}
