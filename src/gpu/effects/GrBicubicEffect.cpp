...
std::unique_ptr<GrFragmentProcessor> GrBicubicEffect::MakeTexelSubset(
    ...
    auto fp = GrTextureEffect::MakeTexelSubset(
            std::move(view), alphaType, matrix, sampler, texelSubset, caps);
    auto clamp = kPremul_SkAlphaType == alphaType ? Clamp::kPremul : Clamp::kUnpremul;
    return std::unique_ptr<GrFragmentProcessor>(
            new GrBicubicEffect(std::move(fp), direction, clamp));
}
...
std::unique_ptr<GrFragmentProcessor> GrBicubicEffect::TestCreate(GrProcessorTestData* d) {
    ...
    if (d->fRandom->nextBool()) {
        ...
        if (d->fRandom->nextBool()) {
            ...
            subset.sort();
            return MakeSubset(std::move(view), at, m, wm, subset, direction, *d->caps());
        }
        return Make(std::move(view), at, m, wm, direction, *d->caps());
    } else {
