...
void GrBicubicEffect::Impl::emitCode(EmitArgs& args) {
    ...
    // We determine our fractional offset (f) within the texel. We then snap coord to a texel center
    // The snap prevents cases where the starting coords are near a texel boundary and offsets
    // with imperfect precision would cause us to skip/double hit a texel.
    if (bicubicEffect.fDirection == GrBicubicEffect::Direction::kXY) {
        ...
        for (int i = 0; i < 4; ++i) {
            ...
            if (bicubicEffect.fDirection == Direction::kX) {
                coord.printf("coord + half2(%d, 0)", i - 1);
            } else {
                coord.printf("coord + half(0, %d)", i - 1);
            }
