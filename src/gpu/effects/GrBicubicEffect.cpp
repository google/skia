...
void GrBicubicEffect::Impl::onSetData(const GrGLSLProgramDataManager& pdm,
    ...
    SkM44 kernel = SkImageShader::CubicResamplerMatrix(bicubicEffect.fKernel.B,
                                                       bicubicEffect.fKernel.C);
    pdm.setSkM44(fCoefficientUni, kernel);
}
...
std::unique_ptr<GrFragmentProcessor> GrBicubicEffect::TestCreate(GrProcessorTestData* d) {
    ...
    auto kernel = GrBicubicEffect::Kernel{d->fRandom->nextUScalar1(),
