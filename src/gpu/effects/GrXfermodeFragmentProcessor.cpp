    ...
    void onComputeInvariantOutput(GrInvariantOutput* inout) const override {
        ...
        if (SkXfermode::ModeAsCoeff(fMode, &skSrcCoeff, &skDstCoeff)) {
            ...
            if (kDst_Child == fChild) {
                GrGetCoeffBlendKnownComponents(srcCoeff, dstCoeff, inout->knownComponents(),
                                               childOutput.knownComponents(), &blendedComponents);
            } else {
                GrGetCoeffBlendKnownComponents(srcCoeff, dstCoeff, childOutput.knownComponents(),
                                               inout->knownComponents(), &blendedComponents);
            }
