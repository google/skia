    ...
    static OptimizationFlags OptFlags(const GrFragmentProcessor* src,
        ...
        switch (mode) {
            ...
            case SkBlendMode::kDst:
                SkFAIL("Should never create clear, src, or dst");
        ...
        if (does_cpu_blend_impl_match_gpu(mode) && src->hasConstantOutputForConstantInput() &&
            ...
            return flags | kConstantOutputForConstantInput_OptimizationFlag;
        }
        return flags | kNone_OptimizationFlags;
    }
...
// It may seems as though when the child FP is the dst and the mode is kDst (or same for src/kSrc)
// that these factories could simply return the input processor. However, that doesn't have quite
// the same effect as the returned compose FP will replace the input to the child with solid white
// and ignore the original input.
