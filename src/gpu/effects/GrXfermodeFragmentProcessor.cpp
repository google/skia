...
private:
    ...
    SkPMColor4f constantOutputForConstantInput(const SkPMColor4f& input) const override {
        SkPMColor4f src = (fSrcFPIndex >= 0)
                ? ConstantOutputForConstantInput(this->childProcessor(fSrcFPIndex), input)
                : input;
        SkPMColor4f dst = (fDstFPIndex >= 0)
                ? ConstantOutputForConstantInput(this->childProcessor(fDstFPIndex), input)
                : input;
...
void GLComposeFragmentProcessor::emitCode(EmitArgs& args) {
    ...
    switch (behavior) {
        ...
        case ComposeBehavior::kSkModeBehavior:
            ...
            srcColor = this->invokeChild(srcFPIndex, args);
            dstColor = this->invokeChild(dstFPIndex, args.fInputColor, args);
            break;
    }
