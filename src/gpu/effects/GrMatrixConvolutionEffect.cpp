...
void GrGLMatrixConvolutionEffect::emitCode(EmitArgs& args) {
    ...
    if (mce.kernelIsSampled()) {
        fragBuilder->codeAppendf("float kernelSampleCoordX = 0;");
    }
    float kernelStride = 1.0f / (float)GrNextPow2(mce.kernelSize().area());
    fragBuilder->codeAppend("float4 c;");
    // Try not unrolling the loop for arbitrarily large kernels.
    fragBuilder->codeAppendf("for (int i = 0; i < %d; ++i)", kernelWidth * kernelHeight);
    {
        ...
        if (mce.kernelIsSampled()) {
            ...
            fragBuilder->appendTextureLookup(args.fTexSamplers[0],
                                             "half2(half(kernelSampleCoordX), 0.5)");
            fragBuilder->codeAppendf(".w + %s;", kernelBias);
            fragBuilder->codeAppendf("kernelSampleCoordX += %f;", kernelStride);
        } else {
            // Uniform code path not implemented yet.
            fragBuilder->codeAppendf("float k = 0;");
        }

        fragBuilder->codeAppendf("half y;");
        fragBuilder->codeAppendf("half x = modf(half(i) / half(%f), y);", (float)kernelWidth);
        SkSL::String coord;
        coord.appendf("coord + half2(x, y)");
