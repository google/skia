...
void GrGLMatrixConvolutionEffect::emitKernelBlock(EmitArgs& args, SkIPoint loc) {
    ...
    if (mce.kernelIsSampled()) {
        fragBuilder->codeAppendf("half kernelSampleCoordX = 0;");
        fragBuilder->codeAppendf("half iOverWidth = 0;");
        fragBuilder->codeAppendf("for (int i = 0; i < %d; ++i)", (int)mce.kernelSize().area());
    }

    GrGLSLShaderBuilder::ShaderBlock block(fragBuilder);

    fragBuilder->codeAppend("float k;");
    fragBuilder->codeAppend("half2 childSampleOffset;");
    if (mce.kernelIsSampled()) {
        const char* kernelBias = uniformHandler->getUniformCStr(fKernelBiasUni);
        fragBuilder->codeAppend("k = ");
        fragBuilder->appendTextureLookup(args.fTexSamplers[0],
                                         "half2(kernelSampleCoordX, 0.5)");
        fragBuilder->codeAppendf(".w + %s;", kernelBias);
        fragBuilder->codeAppendf("childSampleOffset.x = modf(iOverWidth, childSampleOffset.y);");
        fragBuilder->codeAppendf("iOverWidth += %f;", 1.0f / (float)mce.kernelSize().width());
        float kernelStride = 1.0f / (float)GrNextPow2(mce.kernelSize().area());
        fragBuilder->codeAppendf("kernelSampleCoordX += %f;", kernelStride);
    } else {
        fragBuilder->codeAppendf("childSampleOffset = half2(%d, %d);", loc.x(), loc.y());
        int offset = loc.y() * mce.kernelSize().width() + loc.x();
    ...
    auto sample = this->invokeChild(0, args, "coord + childSampleOffset");
