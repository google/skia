...
GrMatrixConvolutionEffect::KernelWrapper GrMatrixConvolutionEffect::KernelWrapper::Make(
    ...
    bool haveA16 = context->defaultBackendFormat(kA16_float_SkColorType, GrRenderable::kNo).isValid();

    // Asserting just to figure out what gpus don't support this.
    SkASSERT(haveA16);
    SkScalar min = 0; // unset if a16
    if (haveA16) {
        scalableSampler.fGain = 1.0f;
        scalableSampler.fBias = 0.0f;
    } else {
    ...
    SkColorType colorType = haveA16 ? kA16_float_SkColorType : kAlpha_8_SkColorType;
    if (sk_sp<GrTextureProxy> kernelProxy = proxyProvider->findOrCreateProxyByUniqueKey(key)) {
        ...
        for (int i = 0; i < length; i++) {
            if (haveA16) {
                *(_Float16 *)bm.getAddr16(i, 0) = static_cast<_Float16>(values[i]);
            } else {
                *(bm.getAddr8(i, 0)) =
...
void GrGLMatrixConvolutionEffect::emitKernelBlock(EmitArgs& args, SkIPoint loc) {
    ...
    if (mce.kernelIsSampled()) {
        fragBuilder->codeAppendf("float2 kernelCoord = float2(0, 0);");
    ...
    fragBuilder->codeAppend("float2 sourceOffset;");
    if (mce.kernelIsSampled()) {
        ...
        fragBuilder->codeAppendf("sourceOffset = float2(%d, %d);", loc.x(), loc.y());
