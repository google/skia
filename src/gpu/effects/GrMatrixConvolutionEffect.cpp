...
void GrGLMatrixConvolutionEffect::emitCode(EmitArgs& args) {
    ...
    for (int y = 0; y < kernelHeight; y++) {
        for (int x = 0; x < kernelWidth; x++) {
            ...
            if (mce.kernelIsSampled()) {
                ...
                float xCoord = offset / (float)mce.kernelSize().area();

                fragBuilder->codeAppend("half k = ");
                auto texCoord = SkSL::String::printf("half2(%f, 0.5)", xCoord);
                fragBuilder->appendTextureLookup(args.fTexSamplers[0], texCoord.c_str());
    ...
    if (mce.convolveAlpha()) {
        ...
        fragBuilder->codeAppendf("%s.rgb = saturate(sum.rgb * %s + %s);",
...
std::unique_ptr<GrFragmentProcessor> GrMatrixConvolutionEffect::MakeGaussian(
    ...
    fill_in_2D_gaussian_kernel(kernel.data(), kernelSize.width(), kernelSize.height(),
                               sigmaX, sigmaY);
    return Make(context, std::move(srcView), srcBounds, kernelSize, kernel.data(),
