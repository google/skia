...
void GrGLMatrixConvolutionEffect::emitCode(EmitArgs& args) {
    ...
    fragBuilder->codeAppendf("half2 coord = %s - %s;", coords2D.c_str(), kernelOffset);
    ...
    for (int y = 0; y < kernelHeight; y++) {
        for (int x = 0; x < kernelWidth; x++) {
            ...
            if (mce.kernelIsSampled()) {
                // Half-precision is not enough for this computation.
                ...
                fragBuilder->codeAppendf("float k = float(%s[%d]%s);", kernel, offset / 4,
