...
GrMatrixConvolutionEffect::KernelWrapper GrMatrixConvolutionEffect::KernelWrapper::TryMake(
    GrRecordingContext *context, const SkISize &size, const SkScalar *values) {
        ...
        for (int x = 0; x < kernelWidth; x++) {
            ...
            if (mce.kernelIsSampled()) {
                ...
                fragBuilder->appendTextureLookup(args.fTexSamplers[0],
                                                 SkSL::String::printf("half2(%f, 0.5)", texCoord).c_str());
...
std::unique_ptr<GrFragmentProcessor> GrMatrixConvolutionEffect::MakeGaussian(
    ...
    return Make(context, std::move(srcView), srcBounds, kernelSize, kernel.data(), gain, bias,
                kernelOffset, wm, convolveAlpha, caps);
}
