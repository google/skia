...
public:
    void emitCode(EmitArgs& args) override {
        ...
        fragBuilder->emitFunction(kFloat_GrSLType, "transfer_fn",
                                  ...
                                  transferFnBody.c_str(), &tfFuncName);
...
sk_sp<GrFragmentProcessor> GrNonlinearColorSpaceXformEffect::TestCreate(GrProcessorTestData* d) {
    // TODO: Generate a much wider variety of color spaces for this effect (it can handle wacky
    // transfer functions, etc...)
    sk_sp<SkColorSpace> srcSpace = GrTest::TestColorSpace(d->fRandom);
    sk_sp<SkColorSpace> dstSpace = GrTest::TestColorSpace(d->fRandom);
...
sk_sp<GrFragmentProcessor> GrNonlinearColorSpaceXformEffect::Make(SkColorSpace* src,
                                                                  SkColorSpace* dst) {
    ...
    if (!dst->gammaIsLinear()) {
        if (dst->isNumericalTransferFn(&dstTransferFn)) {
            dstTransferFn = invertParametricTransferFn(dstTransferFn);
            ops |= kDstTransfer_Op;
        } else {
            return nullptr;
        }
