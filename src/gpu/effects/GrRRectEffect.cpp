...
public:
    ...
    int getInputFPIndex() const {
        // We only support a single child FP, so it should always be at index 0 if it exists.
        return this->numChildProcessors() ? 0 : -1;
    }
...
std::unique_ptr<GrFragmentProcessor> CircularRRectEffect::Make(
    ...
    return std::unique_ptr<GrFragmentProcessor>(new CircularRRectEffect(
            inputFP ? std::move(*inputFP) : nullptr, edgeType, circularCornerFlags, rrect));
}
...
std::unique_ptr<GrFragmentProcessor> CircularRRectEffect::clone() const {
    std::unique_ptr<GrFragmentProcessor> inputFPClone;
    int inputFPIndex = getInputFPIndex();
    if (inputFPIndex >= 0) {
        const GrFragmentProcessor& inputFP = this->childProcessor(inputFPIndex);
    ...
    return std::unique_ptr<GrFragmentProcessor>(new CircularRRectEffect(
            std::move(inputFPClone), fEdgeType, fCircularCornerFlags, fRRect));
}
...
std::unique_ptr<GrFragmentProcessor> CircularRRectEffect::TestCreate(GrProcessorTestData* d) {
    ...
    do {
        ...
        fp = GrRRectEffect::Make(/*inputFP=*/nullptr, et, rrect, *d->caps()->shaderCaps());
    } while (nullptr == fp);
    return fp;
}
#endif
...
void GLCircularRRectEffect::emitCode(EmitArgs& args) {
    ...
    SkString inputSample = (crre.getInputFPIndex() >= 0)
                ? this->invokeChild(crre.getInputFPIndex(), args.fInputColor, args)
...
std::unique_ptr<GrFragmentProcessor> EllipticalRRectEffect::Make(
    ...
    return std::unique_ptr<GrFragmentProcessor>(new EllipticalRRectEffect(
            inputFP ? std::move(*inputFP) : nullptr, edgeType, rrect));
}
...
std::unique_ptr<GrFragmentProcessor> EllipticalRRectEffect::clone() const {
    ...
    return std::unique_ptr<GrFragmentProcessor>(new EllipticalRRectEffect(
            std::move(inputFPClone), fEdgeType, fRRect));
}
...
void GLEllipticalRRectEffect::emitCode(EmitArgs& args) {
    ...
    SkString inputSample = (erre.getInputFPIndex() >= 0)
                ? this->invokeChild(erre.getInputFPIndex(), args.fInputColor, args)
                : SkString(args.fInputColor);
...
std::unique_ptr<GrFragmentProcessor> GrRRectEffect::Make(
    ...
    if (rrect.isRect()) {
        return GrConvexPolyEffect::Make(inputFP, edgeType, rrect.getBounds());
    }

    if (rrect.isOval()) {
        return GrOvalEffect::Make(inputFP, edgeType, rrect.getBounds(), caps);
    }
    ...
    if (rrect.isSimple()) {
        ...
        if (SkRRectPriv::GetSimpleRadii(rrect).fX == SkRRectPriv::GetSimpleRadii(rrect).fY) {
            return CircularRRectEffect::Make(inputFP, edgeType,
                                             CircularRRectEffect::kAll_CornerFlags, rrect);
        } else {
            return EllipticalRRectEffect::Make(inputFP, edgeType, rrect);
        }
    }
    ...
    if (rrect.isComplex() || rrect.isNinePatch()) {
        ...
        switch (cornerFlags) {
            ...
            case CircularRRectEffect::kBottom_CornerFlags: {
                ...
                return CircularRRectEffect::Make(inputFP, edgeType, cornerFlags, *rr);
            }
