    ...
    switch (crre.getCircularCornerFlags()) {
        ...
        case CircularRRectEffect::kTopLeft_CornerFlag:
            fragBuilder->codeAppendf("float2 dxy = max(%s.xy - sk_FragCoord.xy, 0.0);",
                                     rectName);
            fragBuilder->codeAppendf("half rightAlpha = half(clamp(%s.z - sk_FragCoord.x, 0.0, 1.0));",
                                     rectName);
            fragBuilder->codeAppendf("half bottomAlpha = half(clamp(%s.w - sk_FragCoord.y, 0.0, 1.0));",
                                     rectName);
            fragBuilder->codeAppendf("half alpha = half(bottomAlpha * rightAlpha * %s);",
        ...
        case CircularRRectEffect::kTopRight_CornerFlag:
            fragBuilder->codeAppendf("float2 dxy = max(float2(sk_FragCoord.x - %s.z, "
                                     ...
                                     rectName, rectName);
            fragBuilder->codeAppendf("half leftAlpha = half(clamp(sk_FragCoord.x - %s.x, 0.0, 1.0));",
                                     rectName);
            fragBuilder->codeAppendf("half bottomAlpha = half(clamp(%s.w - sk_FragCoord.y, 0.0, 1.0));",
                                     rectName);
            fragBuilder->codeAppendf("half alpha = half(bottomAlpha * leftAlpha * %s);",
                                     clampedCircleDistance.c_str());
            break;
        case CircularRRectEffect::kBottomRight_CornerFlag:
            ...
            fragBuilder->codeAppendf("half leftAlpha = half(clamp(sk_FragCoord.x - %s.x, 0.0, 1.0));",
                                     rectName);
            fragBuilder->codeAppendf("half topAlpha = half(clamp(sk_FragCoord.y - %s.y, 0.0, 1.0));",
                                     rectName);
...
void GLEllipticalRRectEffect::emitCode(EmitArgs& args) {
    ...
    if (GrClipEdgeType::kFillAA == erre.getEdgeType()) {
        fragBuilder->codeAppend("half alpha = half(clamp(0.5 - approx_dist, 0.0, 1.0));");
    } else {
        fragBuilder->codeAppend("half alpha = half(clamp(0.5 + approx_dist, 0.0, 1.0));");
    }
