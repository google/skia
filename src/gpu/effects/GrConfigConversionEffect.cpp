...
public:
    void emitCode(EmitArgs& args) override {
        ...
        switch (cce.pmConversion()) {
            case GrConfigConversionEffect::kPremul_PMConversion:
                fragBuilder->codeAppend(
                    "color.rgb = floor(color.rgb * color.a * 255.0 + 0.5) / 255.0;");
                break;

            case GrConfigConversionEffect::kUnpremul_PMConversion:
                fragBuilder->codeAppend(
...
bool GrConfigConversionEffect::TestForPreservingPMConversions(GrContext* context) {
    ...
    GrPaint paint2;
    GrPaint paint3;
    sk_sp<GrFragmentProcessor> pmToUPM(new GrConfigConversionEffect(kUnpremul_PMConversion));
    sk_sp<GrFragmentProcessor> upmToPM(new GrConfigConversionEffect(kPremul_PMConversion));
