...
class GrGLSLLumaColorFilterEffect : public GrGLSLFragmentProcessor {
 public:
  ...
  void emitCode(EmitArgs& args) override {
    ...
    const GrLumaColorFilterEffect& _outer =
        args.fFp.cast<GrLumaColorFilterEffect>();
    (void)_outer;
    fragBuilder->codeAppendf(
        "\nhalf luma = clamp(dot(half3(0.21260000000000001, "
        "0.71519999999999995, 0.0722), %s.xyz), 0.0, 1.0);\n%s = half4(0.0, "
        "0.0, 0.0, luma);\n",
...
bool GrLumaColorFilterEffect::onIsEqual(
    const GrFragmentProcessor& other) const {
  const GrLumaColorFilterEffect& that = other.cast<GrLumaColorFilterEffect>();
  (void)that;
  return true;
}
GrLumaColorFilterEffect::GrLumaColorFilterEffect(
    const GrLumaColorFilterEffect& src)
...
std::unique_ptr<GrFragmentProcessor> GrLumaColorFilterEffect::clone() const {
  return std::unique_ptr<GrFragmentProcessor>(
      new GrLumaColorFilterEffect(*this));
}
