...
/**************************************************************************************************
 *** This file was autogenerated from GrConfigConversionEffect.fp; do not
 *modify.
 **************************************************************************************************/
...
class GrConfigConversionEffect : public GrFragmentProcessor {
 public:
  static bool TestForPreservingPMConversions(GrContext* context) {
    ...
    // Fill with every possible premultiplied A, color channel value. There will
    // be 256-y duplicate values in row y. We set r, g, and b to the same value
    // since they are handled identically.
    ...
    const SkImageInfo ii = SkImageInfo::Make(
    ...
    // Adding discard to appease vulkan validation warning about loading
    // uninitialized data on draw
    readRTC->discard();
    ...
    // This function is only ever called if we are in a GrContext that has a
    // GrGpu since we are calling read pixels here. Thus the pixel data will be
    // uploaded immediately and we don't need to keep the pixel data alive in
    // the proxy. Therefore the ReleaseProc is nullptr.
    sk_sp<SkImage> image = SkImage::MakeFromRaster(pixmap, nullptr, nullptr);

    sk_sp<GrTextureProxy> dataProxy = proxyProvider->createTextureProxy(
        std::move(image), kNone_GrSurfaceFlags, 1, SkBudgeted::kYes,
        SkBackingFit::kExact);
    if (!dataProxy) {
      return false;
    }
    ...
    // We do a PM->UPM draw from dataTex to readTex and read the data. Then we
    // do a UPM->PM draw from readTex to tempTex followed by a PM->UPM draw to
    // readTex and finally read the data. We then verify that two reads produced
    // the same values.
    ...
    readRTC->fillRectToRect(GrNoClip(), std::move(paint1), GrAA::kNo,
                            SkMatrix::I(), kRect, kRect);
    ...
    paint2.addColorTextureProcessor(readRTC->asTextureProxyRef(),
                                    SkMatrix::I());
    paint2.addColorFragmentProcessor(std::move(upmToPM));
    paint2.setPorterDuffXPFactory(SkBlendMode::kSrc);

    tempRTC->fillRectToRect(GrNoClip(), std::move(paint2), GrAA::kNo,
                            SkMatrix::I(), kRect, kRect);

    paint3.addColorTextureProcessor(tempRTC->asTextureProxyRef(),
                                    SkMatrix::I());
    paint3.addColorFragmentProcessor(std::move(pmToUPM));
    paint3.setPorterDuffXPFactory(SkBlendMode::kSrc);

    readRTC->fillRectToRect(GrNoClip(), std::move(paint3), GrAA::kNo,
                            SkMatrix::I(), kRect, kRect);
  ...
  static std::unique_ptr<GrFragmentProcessor> Make(
    ...
    std::unique_ptr<GrFragmentProcessor> ccFP(
        new GrConfigConversionEffect(pmConversion));
