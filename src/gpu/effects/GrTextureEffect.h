...
public:
    ...
    static std::unique_ptr<GrFragmentProcessor> MakeTexelSubset(sk_sp<GrSurfaceProxy>,
                                                                ...
                                                                const GrCaps& caps);
    /**
     * The same as above but also takes a  'domain' that specifies any known limit on
     * the post-matrix texture coords that will be used to sample the texture. Specifying
     * this requires knowledge of the how this effect will be nested into a paint, the local
     * coords used with the draw, etc. It is only used to attempt to optimize away the shader
     * subset calculations.
     */
    static std::unique_ptr<GrFragmentProcessor> MakeTexelSubset(sk_sp<GrSurfaceProxy>,
                                                                SkAlphaType,
