...
void main() {
    ...
    half d;
    @if (edgeType == kInverseFillBW_GrProcessorEdgeType ||
         edgeType == kInverseFillAA_GrProcessorEdgeType) {
        d = (length((circle.xy - sk_FragCoord.xy) * circle.w) - 1.0) * circle.z;
    } else {
        d = (1.0 - length((circle.xy - sk_FragCoord.xy) *  circle.w)) * circle.z;
    }
    @if (edgeType == kFillAA_GrProcessorEdgeType ||
         edgeType == kInverseFillAA_GrProcessorEdgeType ||
         edgeType == kHairlineAA_GrProcessorEdgeType) {
        d = clamp(d, 0.0, 1.0);
    } else {
        d = d > 0.5 ? 1.0 : 0.0;
    }
