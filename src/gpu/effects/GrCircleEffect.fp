...
void main() {
    ...
    @if (edgeType == GrClipEdgeType::kInverseFillBW ||
        ...
        d = float((length((circle.xy - sk_FragCoord.xy) * circle.w) - 1.0) * circle.z);
    } else {
        d = float((1.0 - length((circle.xy - sk_FragCoord.xy) *  circle.w)) * circle.z);
    }
    half coverage;
    @if (edgeType == GrClipEdgeType::kFillAA ||
         edgeType == GrClipEdgeType::kInverseFillAA ||
         edgeType == GrClipEdgeType::kHairlineAA) {
        coverage = saturate(half(d));
    } else {
        coverage = d > 0.5 ? half(1.0) : half(0.0);
    }

    sk_OutColor = sk_InColor * coverage;
}
