...
public:
    ...
    static std::unique_ptr<GrSkSLFP> Make(GrContext* context, int index, const char* name,
        ...
        void* resultBytes = GrProcessor::Alloc(sizeof(GrSkSLFP) + sizeof(Inputs),
                                               alignof(GrSkSLFP));
        void* inputBytes = (int8_t*) resultBytes + sizeof(GrSkSLFP);
        memcpy(inputBytes, &inputs, sizeof(Inputs));
        printf("Placement: %p\n", resultBytes);
        printf("SKSLFP align: %d\n", (int) alignof(GrSkSLFP));
        SkASSERT(((int64_t) resultBytes) % alignof(GrSkSLFP) == 0);
    ...
    static std::unique_ptr<GrSkSLFP> Make(GrContext* context, int index, const char* name,
        ...
        void* resultBytes = GrProcessor::Alloc(sizeof(GrSkSLFP) + sizeof(Inputs) +
                                               sizeof(ExtraData), alignof(GrSkSLFP));
        void* inputBytes = (int8_t*) resultBytes + sizeof(GrSkSLFP);
        memcpy(inputBytes, &inputs, sizeof(Inputs));
