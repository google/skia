...
void GrGLConvexPolyEffect::emitCode(EmitArgs& args) {
    ...
    SkString inputSample = (cpe.getInputFPIndex() >= 0)
                ? this->invokeChild(cpe.getInputFPIndex(), args.fInputColor, args)
...
GrConvexPolyEffect::GrConvexPolyEffect(std::unique_ptr<GrFragmentProcessor> inputFP,
    ...
    // Register the input processor and remember its index.
    if (inputFP != nullptr) {
        fInputFPIndex = this->registerChildProcessor(std::move(inputFP));
    }
}
...
GrConvexPolyEffect::GrConvexPolyEffect(const GrConvexPolyEffect& that)
    ...
    if (that.fInputFPIndex >= 0) {
        const GrFragmentProcessor& thatInputFP = that.childProcessor(that.fInputFPIndex);
        std::unique_ptr<GrFragmentProcessor> thatInputFPClone = thatInputFP.clone();
        if (thatInputFP.isSampledWithExplicitCoords()) {
            thatInputFPClone->setSampledWithExplicitCoords();
        }
        fInputFPIndex = this->registerChildProcessor(std::move(thatInputFPClone));
    }
