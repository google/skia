...
static void aa_rect_set_data_hook(const GrGLSLProgramDataManager& pdman,
                                  ...
                                  const GrSkSLFP& fp) {
    AARectInputs& inputs = *(AARectInputs*) fp.inputs();
    SkRect* prevRect = (SkRect*) glslProc.extraData();
    if (!prevRect) {
        prevRect = (SkRect*) glslProc.allocateExtraData(sizeof(SkRect));
        *prevRect = SkRect::MakeXYWH(-1, -1, -1, -1);
    }
