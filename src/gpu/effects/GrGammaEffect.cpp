...
GrGammaEffect::GrGammaEffect(Mode mode, SkScalar gamma)
    : INHERITED(kPreservesOpaqueInput_OptimizationFlag | kConstantOutputForConstantInput), fMode(mode)
    , fGamma(gamma) {
...
GrColor4f GrGammaEffect::constantOutputForConstantInput(GrColor4f input) const {
    switch (fMode) {
