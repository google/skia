...
#if 1
        static constexpr int kNumSubsamples = 4;
        static constexpr float kInvNumSubsamples = 1.f / kNumSubsamples;
        const float invSubsampleWidth = kInvNumSubsamples / width;
        for (int i = 1; i < width - 1; ++i) {
            float accum = 0;
            for (int step = 0; step < kNumSubsamples; ++step) {
                float x = (kNumSubsamples * i + step + 0.5f) * invSubsampleWidth;
                x = (-6 * x + 3) * M_SQRT1_2;
                accum += 0.5f * (std::erf(x) + 1.f);
            }
            *bitmap.getAddr8(i, 0) =
                    SkToU8(sk_float_round2int(255.f * accum * kInvNumSubsamples));
        }
#else
