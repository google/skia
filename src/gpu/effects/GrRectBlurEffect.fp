...
@make {
     static std::unique_ptr<GrFragmentProcessor> Make(std::unique_ptr<GrFragmentProcessor> inputFP,
         ...
         return std::unique_ptr<GrFragmentProcessor>(new GrRectBlurEffect(
                    std::move(inputFP), insetRect, std::move(integral), invSixSigma,
...
void main() {
    ...
    @if (isFast) {
        ...
        half2 xy;
        @if (highp) {
            xy = max(half2(rectF.xy - sk_FragCoord.xy),
                     half2(sk_FragCoord.xy - rectF.zw));
       } else {
            xy = max(half2(rectH.xy - sk_FragCoord.xy),
                     half2(sk_FragCoord.xy - rectH.zw));
        }
        xy *= invSixSigma;
        ...
        half4 lrtb;
        @if (highp) {
            lrtb.xy = half2(sk_FragCoord.xy - rectF.xy);
            lrtb.zw = half2(rectF.zw - sk_FragCoord.xy);
        } else {
            lrtb.xy = half2(sk_FragCoord.xy - rectH.xy);
            lrtb.zw = half2(rectH.zw - sk_FragCoord.xy);
        }
        half4 ilrtb = lrtb * invSixSigma + half4(1);
        xCoverage = 1 - sample(integral, half2(ilrtb.x, 0.5)).a
                      - sample(integral, half2(ilrtb.z, 0.5)).a;
        yCoverage = 1 - sample(integral, half2(ilrtb.y, 0.5)).a
                      - sample(integral, half2(ilrtb.w, 0.5)).a;
    }
