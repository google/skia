...
@make {
     static std::unique_ptr<GrFragmentProcessor> Make(std::unique_ptr<GrFragmentProcessor> inputFP,
         ...
         if (!caps.floatIs32Bits()) {
             ...
             if (SkScalarAbs(srcRect.fLeft)  > 16000.f ||
                 SkScalarAbs(srcRect.fTop)   > 16000.f ||
                 SkScalarAbs(srcRect.fRight) > 16000.f ||
                 SkScalarAbs(srcRect.fBottom) > 16000.f) {
                    return nullptr;
             }
         }
         ...
         SkRect insetRect = {srcRect.fLeft   + threeSigma,
                             srcRect.fTop    + threeSigma,
                             srcRect.fRight  - threeSigma,
                             srcRect.fBottom - threeSigma};
         ...
         return std::unique_ptr<GrFragmentProcessor>(new GrRectBlurEffect(
                    std::move(inputFP), insetRect, !viewMatrix.isIdentity(), invVM, std::move(integral),
                    isFast));
     }
}
