...
static sk_sp<GrTextureProxy> CreateIntegralTexture(GrProxyProvider* proxyProvider,
    ...
    // range centered at zero. All such integrals are just scales of each other. So all we really
    // care about is having enough resolution so that the linear interpolation done in texture
    // lookup doesn't introduce noticeable artifacts. We conservatively choose to have 2 texels for
    // each dst pixel.
    ...
    if (!blurProfile) {
        ...
        // This takes the desired width and the sigma. It expects and asserts that ceil(6 * sigma)
        // == width.
...
@make {
     static std::unique_ptr<GrFragmentProcessor> Make(GrProxyProvider* proxyProvider,
         ...
         float halfWidth = sixSigma / 2;
         auto insetRect = rect.makeInset(halfWidth, halfWidth);
...
void main() {
        ...
        @if (isFast) {
            ...
            // integral. The integral is an S-looking shape that is symmetric about 0, so we just
            // compute x and "backwards" such that texture coord is 1 at the edge and goes to 0 as
            // we move outward.
            half x;
            @ if (highp) {
            ...
            // We can calculate C by integrating K with the points on the line outside the L to R
            // range and subtracting from 1:
            //   C = 1 - <integral of K from from -inf to  L> - <integral of K from R to inf>
            // K is symmetric about x=0 so this is the same as:
