...
static sk_sp<GrTextureProxy> CreateBlurProfileTexture(GrProxyProvider* proxyProvider,
                                                      float sigma) {
    // The "profile" we are caclulating is the integral of a Gaussian with 'sigma' and a half plane.
    // All blur profiles are just scales of each other. So all we really care about is having enough
    // resolution so that the linear interpolation done in texture lookup doesn't introduce
    // noticeable artifacts. SkBlurMask::ComputeBlurProfile() produces profiles with ceil(6 * sigma)
    // entries. We conservatively choose to have 2 texels for each dst pixel.
...
@make {
     static std::unique_ptr<GrFragmentProcessor> Make(GrProxyProvider* proxyProvider,
         ...
         // The profile is calculated such that the midpoint is at the rects edge. To simplify
         // calculating texture coords in the shader we inset the rect such that the profile
         // can be used with one end point aligned to the edges of the rect uniform.
         float halfW = 3.f * sigma;
         auto insetR = rect.makeInset(halfW, halfW);
         // The texture coords should be scaled such that the profile is sampled over a 6 sigma
         // range outward from the inset rect.
