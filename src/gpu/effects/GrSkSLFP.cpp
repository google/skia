...
public:
    ...
    void onSetData(const GrGLSLProgramDataManager& pdman,
        ...
        for (const auto& v : outer.fEffect->fInAndUniformVars) {
            ...
            int count = SkTMax(1, v.fArrayCount);
            const float* data = reinterpret_cast<const float*>(inputs + v.fOffset);
            switch (v.fCPUType) {
                case SkRuntimeEffect::CPUType::kFloat:
                    pdman.set1fv(fUniformHandles[uniIndex++], count, data);
                    break;
                case SkRuntimeEffect::CPUType::kFloat2:
                    pdman.set2fv(fUniformHandles[uniIndex++], count, data);
                    break;
                case SkRuntimeEffect::CPUType::kFloat3:
                    pdman.set3fv(fUniformHandles[uniIndex++], count, data);
                    break;
                case SkRuntimeEffect::CPUType::kFloat4:
                    pdman.set4fv(fUniformHandles[uniIndex++], count, data);
                    break;
                case SkRuntimeEffect::CPUType::kFloat2x2:
                    pdman.setMatrix2fv(fUniformHandles[uniIndex++], count, data);
                    break;
                case SkRuntimeEffect::CPUType::kFloat3x3:
                    pdman.setMatrix3fv(fUniformHandles[uniIndex++], count, data);
                    break;
                case SkRuntimeEffect::CPUType::kFloat4x4:
                    pdman.setMatrix4fv(fUniformHandles[uniIndex++], count, data);
                    break;
                default:
