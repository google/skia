...
GrGLSLFragmentProcessor* GrSkSLFP::onCreateGLSLInstance() const {
    auto specialized = fEffect->getSpecialization(fInputs.get(), fInputSize, fShaderCaps.get());

    // Note: This isn't GLSL, it's actually SkSL (again) but with inline format specifiers.
    SkSL::String glsl;
    ...
    return new GrGLSLSkSLFP(specialized->fContext.get(), glsl, formatArgs, functions);
}
