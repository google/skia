...
GrGLSLFragmentProcessor* GrSkSLFP::onCreateGLSLInstance() const {
    ...
    size_t len = SkBase64::Encode(fInputs.get(), fInputSize, nullptr);
    char* data = (char*) malloc(len + 1);
    SkBase64::Encode(fInputs.get(), fInputSize, data);
    data[len] = 0;
    SkDebugf("shader: %d(%d), %s, ###BEGIN###%s###END###\n", internalId, isClone, data, glsl.c_str());
    free(data);
