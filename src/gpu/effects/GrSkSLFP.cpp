...
std::unique_ptr<GrFragmentProcessor> GrSkSLFP::Make(GrContext* context, int index, const char* name,
                                                    ...
                                                    size_t inputSize) {
    sk_sp<GrSkSLFPFactory> factory = context->contextPriv().getFPFactory(index);
    if (!factory) {
        factory = sk_sp<GrSkSLFPFactory>(new GrSkSLFPFactory(
                                                        name,
                                                        context->contextPriv().caps()->shaderCaps(),
                                                        sksl));
        context->contextPriv().setFPFactory(index, factory);
    }
    return std::unique_ptr<GrFragmentProcessor>(new GrSkSLFP(factory, inputs, inputSize));
}


GrSkSLFP::GrSkSLFP(sk_sp<GrSkSLFPFactory> factory, const void* inputs, size_t inputSize)
: INHERITED(kGrSkSLFP_ClassID, kNone_OptimizationFlags)
, fFactory(std::move(factory))
