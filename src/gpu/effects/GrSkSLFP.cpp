...
public:
    void emitCode(EmitArgs& args) override {
        ...
        // If we have an input child, we treat it as special - invoke it now, and make the result
        // of that be the "input color" for all other purposes later (eg, the default passed via)
        // sample calls, etc.
...
void GrSkSLFP::setInput(std::unique_ptr<GrFragmentProcessor> input) {
    SkASSERTF(fInputChildIndex == -1, "setInput must be called at most once");
