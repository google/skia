...
public:
    ...
    void emitCode(EmitArgs& args) override {
        ...
        fragBuilder->codeAppendf(
                "mediump half2 prevCenter;\nmediump half2 prevRadii = half2(%f, %f);\nbool "
                "useScale = %s;\nmediump half2 d = half2(sk_FragCoord.xy - %s.xy);\n@if (useScale) "
                "{\n    d *= %s.y;\n}\nmediump half2 Z = d * half2(%s.zw);\nmediump half implicit "
                "= dot(Z, d) - 1.0;\nmediump half grad_dot = 4.0 * dot(Z, Z);\ngrad_dot = "
                "half(max(highfloat(grad_dot), 0.0001));\nmediump half approx_dist = "
