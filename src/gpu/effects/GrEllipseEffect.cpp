...
class GrGLSLEllipseEffect : public GrGLSLFragmentProcessor {
 public:
  ...
  void emitCode(EmitArgs& args) override {
    ...
    fEllipseVar = args.fUniformHandler->addUniform(kFragment_GrShaderFlag,
                                                   kFloat4_GrSLType, "ellipse");
    if (useScale) {
      fScaleVar = args.fUniformHandler->addUniform(kFragment_GrShaderFlag,
                                                   kFloat2_GrSLType, "scale");
    }
    fragBuilder->codeAppendf(
        "float2 prevCenter;\nfloat2 prevRadii = float2(%f, %f);\nbool useScale "
        "= %s;\nfloat2 d = sk_FragCoord.xy - %s.xy;\n@if (useScale) {\n    d "
        "*= %s.y;\n}\nfloat2 Z = d * %s.zw;\nfloat implicit = dot(Z, d) - "
 ...
 private:
  void onSetData(const GrGLSLProgramDataManager& pdman,
    ...
    if (radii != prevRadii || center != prevCenter) {
      float invRXSqd;
      float invRYSqd;
      // If we're using a scale factor to work around precision issues, choose
      // the larger radius as the scale factor. The inv radii need to be
      // pre-adjusted by the scale factor.
      if (scale.isValid()) {
...
#if GR_TEST_UTILS
std::unique_ptr<GrFragmentProcessor> GrEllipseEffect::TestCreate(
