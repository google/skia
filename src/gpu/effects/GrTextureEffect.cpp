...
GrTextureEffect::Sampling::Sampling(const GrSurfaceProxy& proxy,
                                    ...
                                    const SkRect* domain,
                                    const GrCaps& caps) {
    if (sampler.filter() == Filter::kMipMap && (sampler.wrapModeX() == Mode::kRepeat ||
                                                sampler.wrapModeY() == Mode::kRepeat)) {
        // Have not yet implemented necessary shader filter code to get LOD selection
        // correct with kRepeat.
        sampler.setFilterMode(Filter::kBilerp);
    }
    struct Span {
        float fA = 0.f, fB = 0.f;
    ...
    public:
        void emitCode(EmitArgs& args) override {
            ...
            if (te.fShaderModes[0] == ShaderMode::kNone &&
                ...
                //
                // 0) Start with interpolated coordinates (unnormalize if doing anything complicated).
