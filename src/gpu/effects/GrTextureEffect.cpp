...
void GrTextureEffect::Impl::emitCode(EmitArgs& args) {
    ...
    if (te.fShaderModes[0] == ShaderMode::kNone &&
        ...
        auto clampToBorderNearest = [&](const char* subsetName,
                                        const char* borderName,
                                        const char* coordDataType,
                                        const char* coordSwizzle,
                                        const char* subsetStartSwizzle,
                                        const char* subsetStopSwizzle) {
            fb->codeAppendf("%s snapped = floor(inCoord.%s + %s(0.001)) + %s(0.5);"
                            "if (snapped != clamp(snapped, %s.%s, %s.%s)) {"
                            "    textureColor = %s;"
                            "}",
                            coordDataType, coordSwizzle, coordDataType, coordDataType,
                            subsetName, subsetStartSwizzle, subsetName, subsetStopSwizzle,
                            borderName);
        };

        if (m[0] == ShaderMode::kClampToBorder_Nearest &&
            m[1] == ShaderMode::kClampToBorder_Nearest) {
            clampToBorderNearest(subsetName, borderName, "float2", "xy", "xy", "zw");
        } else if (m[0] == ShaderMode::kClampToBorder_Nearest) {
            clampToBorderNearest(subsetName, borderName, "float", "x", "x", "z");
        } else if (m[1] == ShaderMode::kClampToBorder_Nearest) {
            clampToBorderNearest(subsetName, borderName, "float", "y", "y", "w");
        }
