...
GrGLSLFragmentProcessor* GrTextureEffect::onCreateGLSLInstance() const {
    ...
    public:
        void emitCode(EmitArgs& args) override {
            ...
            bool hasVariableMatrix = te.sampleMatrix().fFlags & SkSL::SampleMatrix::kVariable_Flag;
            if ((te.sampleMatrix().fFlags & SkSL::SampleMatrix::kFragment_Flag) != 0 ||
                hasVariableMatrix) {
                args.fUniformHandler->writeUniformMappings(te.sampleMatrix().fOwner, fb);
                coords = SkStringPrintf("(%s%s * float3(%s, 1)).xy",
                                        te.sampleMatrix().fExpression.c_str(),
                                        hasVariableMatrix ? " * _matrix" : "",
                                        coords.c_str());
            }
