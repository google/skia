                    ...
                    switch (mode) {
                        ...
                        case ShaderMode::kRepeat:
                            if (filter == Filter::kMipMap) {
                                ...
                                // repeat coords. The upward sloping one is chosen using a weight
                                // that transitions from one set to the other around the inCoord value
                                // of the reflection.. The weight is also a saw-tooth function shifted
                                // from the coord saw-tooth, vertically shifted, and then clamped to 0..1.
                                SkASSERT(extraCoord);
                                ...
                                fb->codeAppendf("%s = w - o + %s.%s;", extraCoord, subsetName,
                                                subsetStartSwizzle);
                                // coordWeight is as the third param of mix() to blend between a
