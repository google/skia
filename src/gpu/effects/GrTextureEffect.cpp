...
bool GrTextureEffect::Sampling::hasBorderAlpha() const {
    ...
    if (fShaderModes[0] == ShaderMode::kClampToBorderFilter  ||
        fShaderModes[0] == ShaderMode::kClampToBorderNearest ||
        fShaderModes[1] == ShaderMode::kClampToBorderFilter  ||
        fShaderModes[1] == ShaderMode::kClampToBorderNearest) {
...
GrTextureEffect::ShaderMode GrTextureEffect::GetShaderMode(GrSamplerState::WrapMode mode,
    ...
    switch (mode) {
        case GrSamplerState::WrapMode::kMirrorRepeat:
            return filter == GrSamplerState::Filter::kNearest ? ShaderMode::kMirrorRepeatNearest
                                                              : ShaderMode::kMirrorRepeatFilter;
    ...
    public:
        void emitCode(EmitArgs& args) override {
            ...
            if (te.fShaderModes[0] == ShaderMode::kNone &&
                ...
                if (te.fShaderModes[0] == ShaderMode::kClampToBorderNearest ||
                    te.fShaderModes[0] == ShaderMode::kClampToBorderFilter ||
                    te.fShaderModes[1] == ShaderMode::kClampToBorderNearest ||
                    te.fShaderModes[1] == ShaderMode::kClampToBorderFilter) {
                ...
                auto modeUsesSubset = [](ShaderMode m) {
                    switch (m) {
                        ...
                        case ShaderMode::kMirrorRepeatNearest:      return true;
                        case ShaderMode::kMirrorRepeatFilter:       return true;
                ...
                const char* norm = nullptr;
                if (normCoords && (m[0] != ShaderMode::kNone ||
                                   m[1] != ShaderMode::kNone)) {
                ...
                auto subsetCoord = [&](ShaderMode mode,
                    ...
                    switch (mode) {
                        ...
                        case ShaderMode::kRepeatMipMap:
                            ...
                            fb->codeAppend("}");
                            break;
                        case ShaderMode::kMirrorRepeatNearest:
                        case ShaderMode::kMirrorRepeatFilter:
                            fb->codeAppend("{");
                            fb->codeAppendf("float w = %s.%s - %s.%s;", subsetName,
                                            subsetStopSwizzle, subsetName, subsetStartSwizzle);
                            fb->codeAppendf("float w2 = 2 * w;");
                ...
                if (m[0] == ShaderMode::kRepeatBilerp ||
                    m[0] == ShaderMode::kClampToBorderFilter) {
                ...
                if (m[1] == ShaderMode::kRepeatBilerp ||
                    m[1] == ShaderMode::kClampToBorderFilter) {
...
bool GrTextureEffect::onIsEqual(const GrFragmentProcessor& other) const {
    ...
    if ((fShaderModes[0] == ShaderMode::kClampToBorderNearest ||
         fShaderModes[0] == ShaderMode::kClampToBorderFilter  ||
         fShaderModes[1] == ShaderMode::kClampToBorderNearest ||
         fShaderModes[1] == ShaderMode::kClampToBorderFilter) &&
