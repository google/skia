...
GrTextureEffect::Sampling::Sampling(const GrSurfaceProxy& proxy,
    ...
    auto resolve = [type, &caps, filter](int size, Mode mode, Span subset, Span domain) {
        ...
        if (filter == Filter::kNearest) {
            if (domain.fA > sk_float_floor(subset.fA) && domain.fB < sk_float_ceil(subset.fB)) {
                domainIsSafe = true;
            }
        } else if (r.fShaderClamp.contains(domain)) {
            domainIsSafe = true;
        }
        if (domainIsSafe) {
            r.fShaderMode = ShaderMode::kNone;
...
std::unique_ptr<GrFragmentProcessor> GrTextureEffect::Make(sk_sp<GrSurfaceProxy> proxy,
                                                           ...
                                                           GrSamplerState sampler,
                                                           const GrCaps& caps) {
    Sampling sampling(*proxy, sampler, SkRect::Make(proxy->dimensions()), nullptr, caps);
    return std::unique_ptr<GrFragmentProcessor>(
...
std::unique_ptr<GrFragmentProcessor> GrTextureEffect::MakeTexelSubset(sk_sp<GrSurfaceProxy> proxy,
                                                                      ...
                                                                      const GrCaps& caps) {
    Sampling sampling(*proxy, sampler, SkRect::Make(subset), nullptr, caps);
    return std::unique_ptr<GrFragmentProcessor>(
            new GrTextureEffect(std::move(proxy), alphaType, matrix, sampling));
}
...
std::unique_ptr<GrFragmentProcessor> GrTextureEffect::MakeTexelSubset(sk_sp<GrSurfaceProxy> proxy,
                                                                      ...
                                                                      const SkRect& domain,
                                                                      const GrCaps& caps) {
    Sampling sampling(*proxy, sampler, SkRect::Make(subset), &domain, caps);
    return std::unique_ptr<GrFragmentProcessor>(
            new GrTextureEffect(std::move(proxy), alphaType, matrix, sampling));
}
...
std::unique_ptr<GrFragmentProcessor> GrTextureEffect::MakeSubset(sk_sp<GrSurfaceProxy> proxy,
                                                                 ...
                                                                 const SkRect& subset,
                                                                 const GrCaps& caps) {
    Sampling sampling(*proxy, sampler, subset, nullptr, caps);
    return std::unique_ptr<GrFragmentProcessor>(
            new GrTextureEffect(std::move(proxy), alphaType, matrix, sampling));
}
...
std::unique_ptr<GrFragmentProcessor> GrTextureEffect::MakeSubset(sk_sp<GrSurfaceProxy> proxy,
                                                                 ...
                                                                 const SkRect& domain,
                                                                 const GrCaps& caps) {
    Sampling sampling(*proxy, sampler, subset, &domain, caps);
    return std::unique_ptr<GrFragmentProcessor>(
            new GrTextureEffect(std::move(proxy), alphaType, matrix, sampling));
}
