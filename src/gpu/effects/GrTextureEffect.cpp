...
std::unique_ptr<GrFragmentProcessor> GrTextureEffect::Make(GrSurfaceProxyView view,
    ...
    bool adjust = MatrixEffectShouldNormalize(view, sampling.fShaderModes);
...
std::unique_ptr<GrFragmentProcessor> GrTextureEffect::MakeCustomLinearFilterInset(
    ...
    return GrMatrixEffect::Make(
            matrix, std::unique_ptr<GrFragmentProcessor>(new GrTextureEffect(
                           std::move(view), alphaType, sampling, adjust)));
}
...
SkMatrix GrTextureEffect::coordAdjustmentMatrix() const {
    ...
    if (MatrixEffectShouldNormalize(fView, fShaderModes)) {
...
bool GrTextureEffect::MatrixEffectShouldNormalize(const GrSurfaceProxyView& view,
                                                  const ShaderMode modes[2]) {
    return view.proxy()->backendFormat().textureType() != GrTextureType::kRectangle &&
           !ShaderModeRequiresUnormCoord(modes[0]) &&
           !ShaderModeRequiresUnormCoord(modes[1]) ;
}
...
void GrTextureEffect::Impl::emitCode(EmitArgs& args) {
    ...
    if (te.fShaderModes[0] == ShaderMode::kNone &&
        ...
        SkASSERT(!(unormCoordsRequiredForShaderMode &&
                   MatrixEffectShouldNormalize(te.fView, te.fShaderModes)));
...
bool GrTextureEffect::onIsEqual(const GrFragmentProcessor& other) const {
    ...
    SkASSERT(fAdjustMatrix == that.fAdjustMatrix);
    return true;
}
...
GrTextureEffect::GrTextureEffect(const GrTextureEffect& src)
        ...
        , fAdjustMatrix(src.fAdjustMatrix) {
