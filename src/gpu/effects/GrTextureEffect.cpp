...
GrGLSLFragmentProcessor* GrTextureEffect::onCreateGLSLInstance() const {
    ...
    public:
        void emitCode(EmitArgs& args) override {
            ...
            if ((te.sampleMatrix().fFlags & SkSL::SampleMatrix::kVariable_Flag) &&
                (te.sampleMatrix().fFlags & SkSL::SampleMatrix::kFragment_Flag)) {
                coords = SkStringPrintf("(_matrix * float3(%s, 1)).xy", coords.c_str());
            }
            if (te.sampleMatrix().fFlags & SkSL::SampleMatrix::kFragment_Flag) {
                args.fUniformHandler->writeUniformMappings(te.sampleMatrix().fOwner, fb);
                coords = SkStringPrintf("(%s * float3(%s, 1)).xy",
