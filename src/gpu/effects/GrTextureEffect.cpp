...
std::unique_ptr<GrFragmentProcessor> GrTextureEffect::Make(GrSurfaceProxyView view,
                                                           ...
                                                           GrSamplerState sampler,
                                                           const GrCaps& caps) {
    Sampling sampling(*view.proxy(), sampler, SkRect::Make(view.proxy()->dimensions()), nullptr, caps);
    return std::unique_ptr<GrFragmentProcessor>(
    ...
    public:
        void emitCode(EmitArgs& args) override {
            ...
            if (te.fShaderModes[0] == ShaderMode::kNone &&
                ...
                // Here is the basic flow of the varioud ShaderModes are implemented in a series of
                // steps. Not all the steps apply to all the modes. We try to emit only the steps that
                // are necessary for the x/y modes and try to do vector ops when the steps are shared by
                // both the x and y mode. The latter makes the shader shorter which seems to avoid some
                // spurious shader compilation failures on Chromecast.
                //
                ...
                auto repeatCoordIfNecessary = [](GrGLSLShaderBuilder* builder, ShaderMode mode,
                                                 const char* inCoord, const char* domainStart,
                                                 const char* domainEnd, bool is2D,
                                                 const char* out) {
                    switch (mode) {
                ...
                if (m[0] == m[1]) {
                    ...
                    repeatCoordIfNecessary(fb, m[0], "inCoords", start.c_str(), end.c_str(), true,
                                           "subsetCoord");
                    if (useClamp[0]) {
                        fb->codeAppendf("float2 clampedCoord = clamp(subsetCoord, %s.xy, %s.zw);",
                                        clampName, clampName);
                    } else {
                        fb->codeAppend("float2 clampedCoord = subsetCoord;");
                    }
                } else {
                    ...
                    repeatCoordIfNecessary(fb, m[0], "inCoords.x", start.c_str(), end.c_str(),
                                           false, "subsetCoord.x");
                    // Repeat the same logic for y.
                    start.printf("%s.y", subsetName);
                    end.printf("%s.w", subsetName);
                    repeatCoordIfNecessary(fb, m[1], "inCoords.y", start.c_str(), end.c_str(),
                                           false, "subsetCoord.y");
                    if (useClamp[0] && useClamp[1]) {
                        fb->codeAppendf("float2 clampedCoord = clamp(subsetCoord, %s.xy, %s.zw);",
                                        clampName, clampName);
                        ...
                        fb->codeAppendf(
                                "float2 clampedCoord = float2(clamp(subsetCoord.x, %s.x, %s.z), "
                                "subsetCoord.y);",
                                clampName, clampName);
                    } else if (useClamp[1]) {
                        fb->codeAppendf(
                                "float2 clampedCoord = float2(subsetCoord.x, clamp(subsetCoord.y, "
                                "%s.y, %s.w));",
                                clampName, clampName);
                    } else {
                        fb->codeAppend("float2 clampedCoord = subsetCoord;");
                ...
                if (smoothEdge[0] || smoothEdge[1]) {
                    ...
                    // There's lots of problems using mix() where the third param is bool or a bool
                    // vector.
                    // 1) MSL/SPIR-V don't support them. TODO: Polyfill in SkSL code gens.
                    // 2) ES2 doesn't support the single bool third param when the other params are
                    //    vectors. TODO: Polyfill in GLSL code gen.
                    // 3) They seem to be broken on Nexus 5. Use polyfills as driver workaround.
#define USE_BOOL_MIX 0
                    if (bothRepeat) {
                        if (USE_BOOL_MIX) {
                            fb->codeAppendf(
                                    "float2 repeatCoord = mix(%s.xy, %s.zw, lessThan(err, half2(0)));",
                                    clampName, clampName);
                        } else {
                            fb->codeAppendf(
                                    "float2 repeatCoord = float2(err.x < 0 ? %s.x : %s.z,"
                                    "                            err.y < 0 ? %s.y : %s.w);",
                        ...
                        if (USE_BOOL_MIX) {
                            fb->codeAppendf("float repeatCoord = mix(%s.x, %s.z, %s < half(0));",
                                            clampName, clampName, errX);
                        } else {
                            fb->codeAppendf("float repeatCoord = %s < 0 ? %s.x : %s.z;",
                                    errX, clampName, clampName);
                        }
                        repeatX.printf("float2(repeatCoord, clampedCoord.y)");
                    } else if (m[1] == ShaderMode::kRepeat) {
                        if (USE_BOOL_MIX) {
                            fb->codeAppendf("float repeatCoord = mix(%s.y, %s.w, %s < half(0));",
                                            clampName, clampName, errY);
                        } else {
                            fb->codeAppendf("float repeatCoord = %s < 0 ? %s.y : %s.w;",
                                    errY, clampName, clampName);
                        }
                    ...
                    if (m[0] == ShaderMode::kDecal && m[1] == ShaderMode::kDecal) {
                        ...
                        fb->codeAppendf("textureColor = mix(textureColor, half4(0), %s);", errX);
                    } else if (m[1] == ShaderMode::kDecal) {
                        fb->codeAppendf("textureColor = mix(textureColor, half4(0), %s);", errY);
                    }
                } else {
                    ...
                    // outside the subset. Otherwise we'd do something like clamp(inCoords,
                    // subset.xy, subset.zw) != inCoords.
                    if (m[0] == ShaderMode::kDecal) {
                        fb->codeAppendf("if ((inCoords.x - min(inCoords.x, %s.x)) *"
                                        "    (max(inCoords.x, %s.z) - inCoords.x) <= 0) {"
                                        "    textureColor = half4(0);"
                                        "}", subsetName, subsetName);
                    }
                    if (m[1] == ShaderMode::kDecal) {
                        fb->codeAppendf("if ((inCoords.y - min(inCoords.y, %s.y)) *"
                                        "    (max(inCoords.y, %s.w) - inCoords.y) <= 0) {"
                                        "    textureColor = half4(0);"
                                        "}", subsetName, subsetName);
                    }
                }
