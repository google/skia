...
//    sk_OutColor = sk_InColor * texture(image, sk_TransformedCoords2D[0], colorXform);
    vec4 tmpColor;
    sk_OutColor = sk_InColor * ((tmpColor = texture(image, sk_TransformedCoords2D[0])) , (colorXform != mat4(1) ? vec4(clamp((colorXform * vec4(tmpColor.rgb, 1.0)).rgb, 0.0, tmpColor.a), tmpColor.a) : tmpColor));
}
