...
@class {
    static GrSurfaceProxyView find_or_create_rrect_blur_mask(GrRecordingContext* context,
        ...
        if (auto view = proxyProvider->findCachedProxyWithColorTypeFallback(key, kMaskOrigin, GrColorType::kAlpha_8)) {
            return view;
        }

        auto rtc = GrRenderTargetContext::MakeWithFallback(
                context, GrColorType::kAlpha_8, nullptr, SkBackingFit::kExact, dimensions, 1, GrMipMapped::kNo, GrProtected::kNo, kMaskOrigin);
        if (!rtc) {
            return {};
        }
