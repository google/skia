...
public:
    ...
    void emitCode(EmitArgs& args) override {
        ...
        fragBuilder->codeAppendf(
                "mediump half value;\nmediump half range;\n@switch (%d) {\n    case 0:\n        "
                "range = half(0.0039215686274509803);\n        break;\n    case 1:\n        range "
                "= half(0.015873015873015872);\n        break;\n    default:\n        range = "
                "half(0.0083333333333333332);\n        break;\n}\n@if (sk_Caps.integerSupport) {\n "
                "   uint x = uint(sk_FragCoord.x);\n    uint y = uint(sk_FragCoord.y);\n    uint m "
                "= (((((y & 1) << 5 | (x & 1) << 4) | (y & 2) << 2) | (x & 2) << 1) | (y & 4) >> "
                "1) | (x & 4) >> 2;\n    value = highfloat(highfloat(highfloat(half(m))) / 64.0) - "
                "0.4921875;\n} else {\n    value = half(fract(sin(dot(sk_FragCoord.xy, "
                "highfloat2(half2(half(12.989800000000001), half(78.233000000000004))))) * "
                "43758.545299999998) - 0.5);\n}\n%s = highfloat4(half4(clamp(%s.xyz + "
                "highfloat(value * range), 0.0, %s.w), half(%s.w)));\n",
