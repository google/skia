...
namespace GrXfermodeFragmentProcessor {
    /** The color input to the returned processor is treated as the src and the passed in processor
        is the dst. */
    sk_sp<GrFragmentProcessor> MakeFromDstProcessor1(sk_sp<GrFragmentProcessor> dst,
                                                    SkBlendMode mode);

    /** The color input to the returned processor is treated as the dst and the passed in processor
        is the src. */
    sk_sp<GrFragmentProcessor> MakeFromSrcProcessor1(sk_sp<GrFragmentProcessor> src,
                                                    SkBlendMode mode);
    ...
    sk_sp<GrFragmentProcessor> MakeFromTwoProcessors1(sk_sp<GrFragmentProcessor> src,
