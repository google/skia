 ...
 public:
  ...
  void emitCode(EmitArgs& args) override {
    ...
    fragBuilder->codeAppendf(
        ...
        "{\n    case 0:\n    case 2:\n        alpha = "
        "half(all(greaterThan(float4(sk_FragCoord.xy, %s.zw), float4(%s.xy, "
        "sk_FragCoord.xy))) ? 1 : 0);\n        break;\n    default:\n        "
        "half xSub, ySub;\n        xSub = min(half(sk_FragCoord.x - %s.x), "
        "0.0);\n        xSub += min(half(%s.z - sk_FragCoord.x), 0.0);\n       "
        " ySub = min(half(sk_FragCoord.y - %s.y), 0.0);\n        ySub += "
        "min(half(%s.w - sk_FragCoord.y), 0.0);\n        alpha = (1.0 + ",
 ...
 private:
  void onSetData(const GrGLSLProgramDataManager& pdman,
    ...
    const SkRect& newRect =
        GrProcessorEdgeTypeIsAA(edgeType) ? rect.makeInset(.5f, .5f) : rect;
    if (newRect != prevRect) {
      pdman.set4f(rectUniform, newRect.fLeft, newRect.fTop, newRect.fRight,
                  newRect.fBottom);
      prevRect = newRect;
    }
  }
...
#if GR_TEST_UTILS
std::unique_ptr<GrFragmentProcessor> GrAARectEffect::TestCreate(
    GrProcessorTestData* d) {
  SkRect rect =
      SkRect::MakeLTRB(d->fRandom->nextSScalar1(), d->fRandom->nextSScalar1(),
