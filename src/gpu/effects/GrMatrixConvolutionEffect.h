...
private:
    ...
    public:
        struct ScalableSampler {
            ...
            ScalableSampler& operator=(const ScalableSampler& that) {
                fBias = that.fBias;
                fGain = that.fGain;
                fSampler = TextureSampler(that.fSampler);
                return *this;
            }
            ScalableSampler& operator=(ScalableSampler&& that) {
                fBias = that.fBias;
                fGain = that.fGain;
                fSampler = std::move(that.fSampler);
                return *this;
            }
        };
        ~KernelWrapper() {
        ...
        KernelWrapper(KernelWrapper&& that) : KernelWrapper(that.fSize) {
            if (that.isSampled()) {
                fScalableSampler = std::move(that.fScalableSampler);
            } else {
                fArray = that.fArray;
            }
        }
        KernelWrapper(const KernelWrapper& that) : KernelWrapper(that.fSize) {
            if (that.isSampled()) {
                fScalableSampler = that.fScalableSampler;
            } else {
                fArray = that.fArray;
            }
        }
