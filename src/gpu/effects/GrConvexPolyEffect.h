...
public:
     ...
     *
     * If the convex-poly effect cannot be created, `success` is set to false and the input FP is
     * returned unchanged. If it can be created, `success` is set to true.
     */
    using MakeResult = std::tuple<bool /*success*/, std::unique_ptr<GrFragmentProcessor> /*fp*/>;
    ...
    static MakeResult Make(std::unique_ptr<GrFragmentProcessor> inputFP,
        ...
        if (n <= 0 || n > kMaxEdges || GrClipEdgeType::kHairlineAA == edgeType) {
            return {/*success=*/false, std::move(inputFP)};
        }

        return {/*success=*/true, std::unique_ptr<GrFragmentProcessor>(
