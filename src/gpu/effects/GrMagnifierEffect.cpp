...
class GrGLSLMagnifierEffect : public GrGLSLFragmentProcessor {
 public:
  ...
  void emitCode(EmitArgs& args) override {
    ...
    fOffsetVar = args.fUniformHandler->addUniform(kFragment_GrShaderFlag,
                                                  kHalf2_GrSLType, "offset");
    SkString sk_TransformedCoords2D_0 =
        fragBuilder->ensureCoords2D(args.fTransformedCoords[0]);
    fragBuilder->codeAppendf(
        "float2 coord = %s;\nfloat2 zoom_coord = float2(%s) + coord * "
        "float2(%s, %s);\nfloat2 delta = (coord - %s.xy) * %s.zw;\ndelta = "
        "min(delta, float2(half2(1.0, 1.0)) - delta);\ndelta *= float2(%s, "
 ...
 private:
  void onSetData(const GrGLSLProgramDataManager& pdman,
...

    {
      ...
      pdman.set4f(boundsUniform, bounds.x() * invW, y,
...
#if GR_TEST_UTILS
std::unique_ptr<GrFragmentProcessor> GrMagnifierEffect::TestCreate(
  ...
  SkIRect bounds =
      SkIRect::MakeWH(SkIntToScalar(kMaxWidth), SkIntToScalar(kMaxHeight));
  SkRect srcRect = SkRect::MakeWH(SkIntToScalar(width), SkIntToScalar(height));

  auto effect = GrMagnifierEffect::Make(
      std::move(proxy), bounds, srcRect, srcRect.width() / bounds.width(),
      srcRect.height() / bounds.height(), bounds.width() / inset,
      bounds.height() / inset);
  SkASSERT(effect);
  return effect;
}
