...
private:
    void onSetData(const GrGLSLProgramDataManager& pdman,
        ...
        GrTexture& src = *_proxy0->priv().peekTexture();
        (void)src;
        auto bounds = _outer.bounds();
        (void)bounds;
        ...
        // FIXME need a better way to get to texturesampler
        GrSurfaceProxy* proxy = _outer.textureSampler(0).proxy();
...

        {
            ...
            pdman.set4f(boundsUniform,
                        bounds.x() * invW,
                        y,
                        SkIntToScalar(tex->width()) / bounds.width(),
                        SkIntToScalar(tex->height()) / bounds.height());
        }
    }
