...
static bool ok_child(GrFragmentProcessor* fp) {
    // We don't support more than 1 coord transform when overriding local coords.
    if (fp->numCoordTransforms() != 1) {
        return false;
    }
    // We're not handling child FP hierarchies correctly when overriding local coords.
...
std::unique_ptr<GrFragmentProcessor> GrDomainEffect::TestCreate(GrProcessorTestData* d) {
    ...
    } while (!ok_child(fp.get()));

    bool filterIfDecal = d->fRandom->nextBool();
    return GrDomainEffect::Make(std::move(fp), matrix, domain, modeX, modeY, filterIfDecal);
}
#endif
