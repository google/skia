...
static bool optimize_modes_for_domain(GrTextureProxy* proxy, const SkRect& domain,
    ...
    if (!proxy) {
        ...
        SkASSERT(*modeX == GrTextureDomain::kIgnore_Mode && *modeY == GrTextureDomain::kIgnore_Mode);
        return true;
    }
...
void GrTextureDomain::GLDomain::sampleTexture(GrGLSLShaderBuilder* builder,
    ...
    if (decalX || decalY) {
        ...
        // in the same shader. When the texture is nearest filtered, fSizeName.z is set to near zero
        // and the domain is not inset by 0.5, then this simulates a hard step function from 0 to 1.
        // When the filtering is bilerp or higher, fSizeName.z is set to 1 and the domain IS inset
        // by 0.5, so it becomes a bounded linear function and makes a smooth transition from the
        // texture color to transparent.
        builder->codeAppendf("if (err > %s.z) { err = 1.0; }", fDecalName.c_str());
...
void GrTextureDomain::GLDomain::setData(const GrGLSLProgramDataManager& pdman,
    ...
    if (kIgnore_Mode != textureDomain.modeX() || kIgnore_Mode != textureDomain.modeY()) {
        ...
        // 0 (texture) to 1 (transparent) very quickly so send a value close to zero. When doing any
        // other form of filtering, the weight should be 1.0 so that it smoothly interpolates
        // between the texture and transparent.
