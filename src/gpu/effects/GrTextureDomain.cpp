...
sk_sp<GrFragmentProcessor> GrTextureDomainEffect::Make(GrResourceProvider* resourceProvider,
    ...
    if (GrTextureDomain::kIgnore_Mode == mode ||
        ...
        return GrSimpleTextureEffect::Make(resourceProvider, std::move(proxy), std::move(colorSpaceXform),
                                           matrix, filterMode);
    } else {
        return sk_sp<GrFragmentProcessor>(
            new GrTextureDomainEffect(resourceProvider, std::move(proxy), std::move(colorSpaceXform),
