...
std::unique_ptr<GrFragmentProcessor> GrDomainEffect::Make(std::unique_ptr<GrFragmentProcessor> fp,
    ...
    // transform and possibly have multiple domain rects to account for different proxy normalization
    // and y-reversals.
    if (count != 1) {
        return fp;
    }
...
std::unique_ptr<GrFragmentProcessor> GrDomainEffect::TestCreate(GrProcessorTestData* d) {
    ...
    do {
        ...
        auto result =
                GrDomainEffect::Make(std::move(child), domain, modeX, modeY, filterIfDecal);
