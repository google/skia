...
GrGLSLFragmentProcessor* GrDeviceSpaceTextureDecalFragmentProcessor::onCreateGLSLInstance() const  {
    ...
    public:
        void emitCode(EmitArgs& args) override {
            ...
            const char* translateAndScaleName;
            fTranslateAndScaleUni = args.fUniformHandler->addUniform(kFragment_GrShaderFlag,
                                                                     kVec4f_GrSLType,
                                                                     kDefault_GrSLPrecision,
                                                                     "translateAndScale",
                                                                     &translateAndScaleName);
            args.fFragBuilder->codeAppendf("vec2 coords = %s.xy * %s.xy + %s.zw;",
                                           args.fFragBuilder->fragmentPosition(),
                                           translateAndScaleName, translateAndScaleName);
    ...
    protected:
        void onSetData(const GrGLSLProgramDataManager& pdman, const GrProcessor& fp) override {
            ...
            float scaleAndTransData[4] = {
                SkIntToScalar(-dstdfp.fDeviceSpaceOffset.fX),
                SkIntToScalar(-dstdfp.fDeviceSpaceOffset.fY),
                1.f / dstdfp.texture(0)->width(),
                1.f / dstdfp.texture(0)->height(),
            };
            if (dstdfp.texture(0)->origin() == kBottomLeft_GrSurfaceOrigin) {
                scaleAndTransData[1] -= dstdfp.texture(0)->height();
                scaleAndTransData[3] = -scaleAndTransData[3];
            }
#else
...
//                -dstdfp.fDeviceSpaceOffset.fX * iw, 0.f
            };
            if (dstdfp.texture(0)->origin() == kBottomLeft_GrSurfaceOrigin) {
...
#endif
            pdman.set4fv(fTranslateAndScaleUni, 1, scaleAndTransData);
        }

    private:
        GrTextureDomain::GLDomain   fGLDomain;
        UniformHandle               fTranslateAndScaleUni;
    };
...
sk_sp<GrFragmentProcessor> GrDeviceSpaceTextureDecalFragmentProcessor::TestCreate(
    ...
    subset.fRight = d->fRandom->nextRangeScalar(subset.fLeft, d->fTextures[texIdx]->width());
    subset.fTop = d->fRandom->nextULessThan(d->fTextures[texIdx]->height() - 1);
    subset.fBottom = d->fRandom->nextRangeScalar(subset.fTop, d->fTextures[texIdx]->height());
    SkIPoint pt;
