...
void GrTextureDomain::GLDomain::sampleProcessor(const GrTextureDomain& textureDomain,
    ...
    this->sample(args.fFragBuilder, args.fUniformHandler, textureDomain, outColor, inCoords, appendProcessorSample);
}
...
std::unique_ptr<GrFragmentProcessor> GrDomainEffect::Make(
    ...
    return Make(std::move(fp), matrix, domain, mode, mode);
}
...
std::unique_ptr<GrFragmentProcessor> GrDomainEffect::Make(
    ...
    return std::unique_ptr<GrFragmentProcessor>(new GrDomainEffect(
            std::move(fp), matrix, domain, modeX, modeY));
}

GrFragmentProcessor::OptimizationFlags GrDomainEffect::Flags(GrFragmentProcessor* fp, GrTextureDomain::Mode modeX, GrTextureDomain::Mode modeY) {
...
GrDomainEffect::GrDomainEffect(const GrDomainEffect& that)
    ...
    this->registerChildProcessor(this->childProcessor(0).clone());
    this->addCoordTransform(&fCoordTransform);
}
...
GrGLSLFragmentProcessor* GrDomainEffect::onCreateGLSLInstance() const  {
    ...
    public:
        void emitCode(EmitArgs& args) override {
            ...
            fGLDomain.sampleProcessor(domain,
                                      args.fInputColor,
                                      args.fOutputColor,
                                      coords2D,
                                      this,
    ...
    protected:
        void onSetData(const GrGLSLProgramDataManager& pdman,
            ...
            const GrTextureDomain& domain = de.fDomain;
            fGLDomain.setData(pdman, domain, true);
        }
...
bool GrDomainEffect::onIsEqual(const GrFragmentProcessor& sBase) const {
    ...
    return this->fDomain == td.fDomain;
}
...
std::unique_ptr<GrFragmentProcessor> GrDomainEffect::TestCreate(GrProcessorTestData* d) {
    SkRect domain;
    domain.fLeft = d->fRandom->nextRangeScalar(-100, 100.f);
    domain.fRight = d->fRandom->nextRangeScalar(domain.fLeft, 200.f);
    domain.fTop = d->fRandom->nextRangeScalar(-100.f, 100.f);
    domain.fBottom = d->fRandom->nextRangeScalar(domain.fTop, 200.f);
    GrTextureDomain::Mode modeX =
            (GrTextureDomain::Mode) d->fRandom->nextULessThan(GrTextureDomain::kModeCount);
    GrTextureDomain::Mode modeY =
            (GrTextureDomain::Mode) d->fRandom->nextULessThan(GrTextureDomain::kModeCount);
    const SkMatrix& matrix = GrTest::TestMatrix(d->fRandom);
    std::unique_ptr<GrFragmentProcessor> fp(GrProcessorUnitTest::MakeChildFP(d));
    return GrDomainEffect::Make(std::move(fp), matrix, domain, modeX, modeY);
}
#endif
