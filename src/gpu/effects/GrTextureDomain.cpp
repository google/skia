...
static bool optimize_modes_for_domain(GrTextureProxy* proxy, const SkRect& domain,
    ...
    if (!proxy) {
        // The only time we don't have a proxy is for the ignored domain
        SkASSERT(*modeX == GrTextureDomain::kIgnore_Mode &&
                 *modeY == GrTextureDomain::kIgnore_Mode);
        return true;
    }
...
void GrTextureDomain::GLDomain::sampleTexture(GrGLSLShaderBuilder* builder,
    ...
    if (fHasMode) {
        SkDebugf("GLDomain::sampleTexture hasMode: %d, eX: %d, eY: %d, aX: %d, aY: %d\n",
             fHasMode, fModeX, fModeY, textureDomain.modeX(), textureDomain.modeY());
        SkDebugf("GLDomain::sampleTexture expected key: %u, actual key: %u\n",
             DomainKey(fModeX, fModeY), DomainKey(textureDomain));
    } else {
        SkDebugf("GLDomain::sampleTexture first time, aX: %d, aY: %d\n",
             textureDomain.modeX(), textureDomain.modeY());
        SkDebugf("GLDomain::sampleTexture first time, actual key: %u\n",
             DomainKey(textureDomain));
    }
...
void GrTextureDomain::GLDomain::setData(const GrGLSLProgramDataManager& pdman,
    ...
    SkDebugf("GLDomain::setData hasMode: %d, eX: %d, eY: %d, aX: %d, aY: %d\n",
             fHasMode, fModeX, fModeY, textureDomain.modeX(), textureDomain.modeY());
    SkDebugf("GLDomain::setData expected key: %u, actual key: %u\n",
             DomainKey(fModeX, fModeY), DomainKey(textureDomain));
    SkASSERT(fHasMode && textureDomain.modeX() == fModeX && textureDomain.modeY() == fModeY);
...
void GrTextureDomainEffect::onGetGLSLProcessorKey(const GrShaderCaps& caps,
    ...
    SkDebugf("TextureDomainEffect, adding domain key: %u\n",
             GrTextureDomain::GLDomain::DomainKey(fTextureDomain));
...
GrGLSLFragmentProcessor* GrTextureDomainEffect::onCreateGLSLInstance() const  {
    ...
    public:
        void emitCode(EmitArgs& args) override {
            ...
            SkDebugf("TextureDomainEffect::emitCode, mode x: %d, mode y: %d, key: %u\n",
    ...
    protected:
        void onSetData(const GrGLSLProgramDataManager& pdman,
            ...
            SkDebugf("TextureDomainEffect::onSetData, mode x: %d, mode y: %d, key: %u\n",
              domain.modeX(), domain.modeY(),
             GrTextureDomain::GLDomain::DomainKey(domain));
...
GrGLSLFragmentProcessor* GrDeviceSpaceTextureDecalFragmentProcessor::onCreateGLSLInstance() const  {
    ...
    public:
        void emitCode(EmitArgs& args) override {
            ...
            SkDebugf("DeviceSpaceDecal::emitCode, mode x: %d, mode y: %d, key: %u\n",
             dstdfp.fTextureDomain.modeX(), dstdfp.fTextureDomain.modeY(),
             GrTextureDomain::GLDomain::DomainKey(dstdfp.fTextureDomain));
    ...
    protected:
        void onSetData(const GrGLSLProgramDataManager& pdman,
            ...
            SkDebugf("DeviceSpaceDecal::onSetData, mode x: %d, mode y: %d, key: %u\n",
             dstdfp.fTextureDomain.modeX(), dstdfp.fTextureDomain.modeY(),
             GrTextureDomain::GLDomain::DomainKey(dstdfp.fTextureDomain));
