...
GrXferProcessor* PDLCDXferProcessor::Create(SkBlendMode xfermode,
    ...
    if (!colorPOI.knownComponents().isConstant()) {
        return nullptr;
    }

    GrColor blendConstant = GrUnpremulColor(colorPOI.knownComponents().color());
...
GrXferProcessor* GrPorterDuffXPFactory::onCreateXferProcessor(const GrCaps& caps,
    ...
    if (analysis.fCoveragePOI.isLCDCoverage()) {
        if (SkBlendMode::kSrcOver == fBlendMode &&
            analysis.fColorPOI.knownComponents().isConstant() &&
        ...
        blendFormula = get_blend_formula(analysis.fColorPOI.knownComponents().isOpaque(),
                                         !analysis.fCoveragePOI.knownComponents().isSolidWhite(),
...
void GrPorterDuffXPFactory::getInvariantBlendedColor(const GrProcOptInfo& colorPOI,
    ...
    bool opaque = colorPOI.knownComponents().isOpaque();
    ...
    switch (colorFormula.fSrcCoeff) {
        ...
        case kOne_GrBlendCoeff:
            blendedColor->fKnownColorComponents = colorPOI.knownComponents();
            return;
...
bool GrPorterDuffXPFactory::SrcOverWillNeedDstTexture(const GrCaps& caps,
    ...
    // don't have support for it.
    static const bool kHasMixedSamples = false;
    bool isOpaque = analysis.fColorPOI.knownComponents().isOpaque();
    bool hasCoverage = !analysis.fCoveragePOI.knownComponents().isSolidWhite();
    SkASSERT(!caps.usesMixedSamples()); // We never use mixed samples without dual source blending.
    auto formula =
