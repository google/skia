...
GrXferProcessor::OptFlags PorterDuffXferProcessor::onGetOptimizations(
    ...
    if (!fBlendFormula.modifiesDst()) {
        ...
        if (analysis.isCompatibleWithCoverageAsAlpha() && fBlendFormula.canTweakAlphaForCoverage()) {
...
public:
    static GrXferProcessor* Create(SkBlendMode xfermode, const FPAnalysis& analysis);
...
GrXferProcessor* PDLCDXferProcessor::Create(SkBlendMode xfermode, const FPAnalysis& analysis) {
...
GrXferProcessor::OptFlags PDLCDXferProcessor::onGetOptimizations(const FPAnalysis&,
                                                                 bool doesStencilWrite,
                                                                 GrColor* overrideColor,
                                                                 const GrCaps& caps) const {
...
bool GrPorterDuffXPFactory::willReadsDst(const FPAnalysis& analysis) const {
...
bool GrPorterDuffXPFactory::onWillReadDstInShader(const GrCaps& caps, const FPAnalysis& analysis) const {
...
GrXferProcessor* GrPorterDuffXPFactory::CreateSrcOverXferProcessor(
    ...
    if (analysis.hasKnownOutputColor() &&
        !caps.shaderCaps()->dualSourceBlendingSupport() &&
        !caps.shaderCaps()->dstReadInShaderSupport()) {
...
bool GrPorterDuffXPFactory::WillSrcOverReadDst(const FPAnalysis& analysis) {
