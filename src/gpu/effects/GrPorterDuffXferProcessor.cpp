...
bool GrPorterDuffXPFactory::isPreCoverageBlendedColorConstant(const GrProcOptInfo& colorInput, GrColor* color) const {
...
bool GrPorterDuffXPFactory::willReadsDst(const GrProcOptInfo& colorInput, const GrProcOptInfo& coverageInput) const {
    BlendFormula colorFormula = gBlendTable[colorInput.isOpaque()][0][(int)fBlendMode];
    SkASSERT(kAdd_GrBlendEquation == colorFormula.fBlendEquation);
...
bool GrPorterDuffXPFactory::WillSrcOverReadDst(const GrXPFactory*, const GrProcOptInfo& colorInput,
                               const GrProcOptInfo& coverageInput) {
    return !coverageInput.isSolidWhite() || !colorInput.isOpaque();
}

bool GrPorterDuffXPFactory::IsSrcOverPreCoverageBlendedColorConstant(const GrXPFactory*, const GrProcOptInfo& colorInput, GrColor* color) {
