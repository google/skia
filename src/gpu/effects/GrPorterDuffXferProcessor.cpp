...
sk_sp<const GrXferProcessor> GrPorterDuffXPFactory::makeXferProcessor(
        ...
        bool hasMixedSamples, const GrCaps& caps, GrClampBehavior dstClamp) const {
    ...
    if ((blendFormula.hasSecondaryOutput() && !caps.shaderCaps()->dualSourceBlendingSupport()) ||
        ...
        (needsClamp && (GrClampBehavior::kUnclamped == dstClamp))) {
...
static inline GrXPFactory::AnalysisProperties analysis_properties(
        ...
        const GrCaps& caps, GrClampBehavior dstClamp, SkBlendMode mode) {
    ...
    if (needsClamp && (GrClampBehavior::kUnclamped == dstClamp)) {
        props |= AnalysisProperties::kReadsDstInShader;
    }
...
GrXPFactory::AnalysisProperties GrPorterDuffXPFactory::analysisProperties(
    ...
    return analysis_properties(color, coverage, caps, dstClamp, fBlendMode);
}
...
GrXPFactory::AnalysisProperties GrPorterDuffXPFactory::SrcOverAnalysisProperties(
    ...
    return analysis_properties(color, coverage, caps, dstClamp, SkBlendMode::kSrcOver);
}
