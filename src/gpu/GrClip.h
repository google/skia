...
/**
 * GrHardClip never uses coverage FPs. It can only enforce the clip that is already in the stencil
 * clip bitplane and/or set fixed-function state like scissor. It's always aliased if MSAA is off.
 */
class GrHardClip : public GrClip {
public:
    /**
     * Sets the appropriate state on GrAppliedClip that applies this hard clip. No coverage FPs will
     * be used. On input 'bounds' is a conservative bounds of the draw that is to be clipped. After
     ...
     */
    bool apply(int rtWidth, int rtHeight, GrAppliedClip* out, SkRect* bounds) const {
        bool ret = this->onApply(rtWidth, rtHeight, out, bounds);
        SkASSERT(!out->numClipCoverageFragmentProcessors());
        return ret;
    }
...
private:
    bool apply(GrContext*, GrRenderTargetContext* rtc, bool useHWAA, bool hasUserStencilSettings,
        ...
        return this->apply(rtc->width(), rtc->height(), out, bounds);
    }

    virtual bool onApply(int rtWidth, int rtHeight, GrAppliedClip*, SkRect* bounds) const = 0;
};
...
private:
    ...
    bool onApply(int rtWidth, int rtHeight, GrAppliedClip*, SkRect*) const final { return true; }
