...
static bool is_valid_lazy(const GrSurfaceDesc& desc, SkBackingFit fit) {
    ...
    // failed instantiation. The former must be "approximate" fit while the latter must be "exact".
    return desc.fConfig != kUnknown_GrPixelConfig &&
           ((desc.fWidth < 0 && desc.fHeight < 0 && fit == SkBackingFit::kApprox) ||
            (desc.fWidth > 0 && desc.fHeight > 0 && fit == SkBackingFit::kExact));
}
