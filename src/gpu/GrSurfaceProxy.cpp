...
sk_sp<GrTextureProxy> GrSurfaceProxy::Copy(GrRecordingContext* context,
    ...
    if (src->asTextureProxy()) {
        ...
        GrSwizzle outputSwizzle = context->priv().caps()->getOutputSwizzle(format, colorType);
        // We won't read from this context so just set the readSwizzle to default
        GrRenderTargetContext dstContext(context, std::move(rtp), colorType, origin, GrSwizzle(),
                                         outputSwizzle, nullptr, nullptr);
