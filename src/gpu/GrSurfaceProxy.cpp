...
GrSurfaceProxy::GrSurfaceProxy(const GrBackendFormat& format,
        ...
        , fSize{desc.fWidth, desc.fHeight}
        , fOrigin(origin)
...
void GrSurfaceProxy::computeScratchKey(GrScratchKey* key) const {
    ...
    GrTexturePriv::ComputeScratchKey(this->config(), this->worstCaseSize(), renderable, sampleCount,
...
SkISize GrSurfaceProxy::worstCaseSize() const {
    ...
    return GrResourceProvider::MakeApprox(fSize);
}
...
sk_sp<GrTextureProxy> GrSurfaceProxy::Copy(GrRecordingContext* context,
    ...
    if (!srcRect.intersect(SkIRect::MakeSize(src->size()))) {
        return nullptr;
    }
...
sk_sp<GrTextureProxy> GrSurfaceProxy::Copy(GrRecordingContext* context, GrSurfaceProxy* src,
    ...
    SkASSERT(!src->isFullyLazy());
    return Copy(context, src, srcColorType, mipMapped, SkIRect::MakeSize(src->size()), fit,
                budgeted);
}
...
void GrSurfaceProxyPriv::exactify(bool allocatedCaseOnly) {
    ...
    if (fProxy->fTarget) {
        ...
        fProxy->fSize = fProxy->fTarget->dimensions();
        return;
    }
...
bool GrSurfaceProxyPriv::doLazyInstantiation(GrResourceProvider* resourceProvider) {
    ...
    if (fProxy->isFullyLazy()) {
        ...
        // the content area.
        fProxy->fSize = surface->dimensions();
    }
