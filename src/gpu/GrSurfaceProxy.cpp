...
void GrSurfaceProxyPriv::doLazyInstantiation(GrResourceProvider* rp) {
    SkASSERT(SkToBool(fProxy->fLazyInstantiateCallback));
    SkASSERT(!fProxy->fTarget);

    sk_sp<GrTexture> texture = fProxy->fLazyInstantiateCallback(rp, &fProxy->fOrigin);
