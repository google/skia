...
sk_sp<GrTextureProxy> GrSurfaceProxy::Copy(GrRecordingContext* context,
    ...
    if (src->asTextureProxy()) {
        auto dstContext = GrSurfaceContext::Make(context, {width, height}, format,
                                                 GrRenderable::kYes, 1, mipMapped, isProtected,
                                                 origin, colorType, kUnknown_SkAlphaType, nullptr,
                                                 fit, budgeted);
        SkASSERT(!dstContext || dstContext->asRenderTargetContext());
        if (dstContext && dstContext->asRenderTargetContext()->blitTexture(src->asTextureProxy(),
                                                                           srcRect, dstPoint)) {
            return dstContext->asTextureProxyRef();
        }
    }
