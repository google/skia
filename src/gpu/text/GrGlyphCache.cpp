...
static bool get_packed_glyph_image(SkGlyphCache* cache, const SkGlyph& glyph, int width,
    ...
    // expected format is 8888 (will happen on macOS since does it not support 565).
...
GrDrawOpAtlas::ErrorCode GrTextStrike::addGlyphToAtlas(
    ...
    if (expectedMaskFormat == kA565_GrMaskFormat &&
        !resourceProvider->caps()->isConfigTexturable(kRGB_565_GrPixelConfig)) {
        expectedMaskFormat = kARGB_GrMaskFormat;
    }
