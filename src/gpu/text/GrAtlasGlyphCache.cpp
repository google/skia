...
bool GrAtlasGlyphCache::initAtlas(GrMaskFormat format) {
    ...
    if (!fAtlases[index]) {
        ...
        fAtlases[index] = GrDrawOpAtlas::Make(
                fContext, config, width, height, numPlotsX, numPlotsY, fAllowMultitexturing,
...
GrAtlasGlyphCache::GrAtlasGlyphCache(GrContext* context, float maxTextureBytes, GrDrawOpAtlas::AllowMultitexturing allowMultitexturing)
