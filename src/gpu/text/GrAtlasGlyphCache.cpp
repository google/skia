...
bool GrAtlasGlyphCache::initAtlas(GrMaskFormat format) {
    ...
    if (!fAtlases[index]) {
        ...
        SkISize plotSize = fAtlasConfigs[index].fPlotSize;
        SkIPoint startPlots = fAtlasConfigs[index].fStartPlots;
        SkIPoint maxPlots = fAtlasConfigs[index].fMaxPlots;

        fAtlases[index] = GrDrawOpAtlas::Make(fContext, config, plotSize, startPlots, maxPlots,
...
GrAtlasGlyphCache::GrAtlasGlyphCache(GrContext* context,
    ...
    for (; log2MaxDim <= log2MaxTextureSize; ++log2MaxDim) {
        unsigned int maxDim = 1 << log2MaxDim;
        unsigned int minDim = 1 << (log2MaxDim - 1);
    ...
    fAtlasConfigs[kA8_GrMaskFormat].init(maxPlot, minPlot, maxDim, maxDim, maxDim, maxDim);

    // A565 uses maxDim x minDim.
    fAtlasConfigs[kA565_GrMaskFormat].init(minPlot, maxPlot, minDim, maxDim, minDim, maxDim);

    // ARGB
    fAtlasConfigs[kARGB_GrMaskFormat].init(minPlot, minPlot, minDim, maxDim, minDim, maxDim);
}
