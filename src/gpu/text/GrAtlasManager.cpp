...
bool GrAtlasManager::hasID(GrMaskFormat format, Bar* bar) const {
    ...
    return fAtlases[index]->hasID1(bar->fFoo.fPlotLocator);
}
...
GrDrawOpAtlas::ErrorCode GrAtlasManager::addToAtlas(
                                ...
                                Bar* bar) {
    return this->getAtlas(format)->addToAtlas(resourceProvider, target, width, height, image, &bar->fFoo);
}
...
void GrAtlasManager::addGlyphToBulkAndSetUseToken1(GrDrawOpAtlas::BulkUseTokenUpdater* updater,
    ...
    if (updater->add(bar->fFoo.fPlotLocator)) {
        this->getAtlas(format)->setLastUseToken(bar->fFoo.fPlotLocator, token);
    }
}
