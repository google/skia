...
ShapeData* GrSmallPathAtlasMgr::findOrCreate(const ShapeDataKey& key) {
...
ShapeData* GrSmallPathAtlasMgr::findOrCreate(const GrStyledShape& shape, int desiredDimension) {
    ShapeDataKey key(shape, desiredDimension);
    return this->findOrCreate(key);
}

ShapeData* GrSmallPathAtlasMgr::findOrCreate(const GrStyledShape& shape, const SkMatrix& ctm) {
    ShapeDataKey key(shape, ctm);
    return this->findOrCreate(key);
}

void GrSmallPathAtlasMgr::updateCacheInfo(ShapeData* shapeData,
                                          GrDrawOpAtlas::AtlasLocator& atlasLocator,
                                          const SkRect& bounds) {
...
void GrSmallPathAtlasMgr::setUseToken(ShapeData* shapeData, GrDeferredUploadToken token) {
