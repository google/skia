...
//////////////////////////////////////////////////////////////////////////////////////////////////
GrAtlasManager::GrAtlasManager(GrProxyProvider* proxyProvider, GrAtlasGlyphCache* atlasGlyphCache,
                               float maxTextureBytes,
                               GrDrawOpAtlas::AllowMultitexturing allowMultitexturing)
            : INHERITED(proxyProvider->refCaps(), maxTextureBytes, allowMultitexturing)
            , fProxyProvider(proxyProvider)
            , fAtlasGlyphCache(atlasGlyphCache) {
}
...
bool GrAtlasManager::addToAtlas(GrResourceProvider* resourceProvider,
    ...
    atlasGlyphCache->setStrikeToPreserve(strike);
...
bool GrAtlasManager::initAtlas(GrMaskFormat format) {
    ...
    if (!fAtlases[index]) {
        ...
        fAtlases[index] = GrDrawOpAtlas::Make(nullptr, config, width, height,
                                              numPlotsX, numPlotsY, fAllowMultitexturing,
                                              &GrAtlasGlyphCache::HandleEviction,
                                              fAtlasGlyphCache);
        if (!fAtlases[index]) {
            return false;
        }
