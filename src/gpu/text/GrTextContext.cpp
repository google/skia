...
static void process_ARGB_fallback(
    ...
    std::vector<SkPoint> transformedPositions;
    SkPoint* finalPositions;
    bool useFastPath =
            viewMatrix.isScaleTranslate() && conservativeMaxGlyphDimension <= maxGlyphDimension;
    if (useFastPath) {
        fallbackCache =
                blob->setupCache(runIndex, props, scalerContextFlags, runPaint, &viewMatrix);
        transformedPositions.insert(
                transformedPositions.begin(), positions.begin(), positions.end());
        ...
        SkScalar maxTextSize = SkGlyphCacheCommon::kSkSideTooBigForAtlas - 2;
