...
void GrTextContext::drawDFText(GrTextBlob* blob, int runIndex,
    ...
    // We apply the fake-gamma by altering the distance in the shader, so we ignore the
    // passed-in scaler context flags. (It's only used when we fall-back to bitmap text).
    if (skPaint.getTextSize() < kSmallDFFontLimit) {
        // When using small text sizes (e.g., for perspective text), the advances from that
        // size may not scale well when we apply the ctm. Instead, we use a larger text size
        // and then rescale the resulting advances to get better fractional placement.
