...
public:
    ...
    bool addGlyphToAtlas(GrProxyProvider*, GrDeferredUploadTarget*, GrGlyph*, SkGlyphCache*,
                         GrMaskFormat expectedMaskFormat);
...
public:
    ...
    bool addToAtlas(GrProxyProvider* proxyProvider, GrAtlasTextStrike* strike,
                    GrDrawOpAtlas::AtlasID* id, GrDeferredUploadTarget* target, GrMaskFormat format,
                    int width, int height, const void* image, SkIPoint16* loc) {
        fPreserveStrike = strike;
        return this->getAtlas(format)->addToAtlas(proxyProvider, id, target,
