 ...
 * used to individual Glyph Masks. The GrAtlasGlyphCache also manages GrDrawOpAtlases, though this
 * is more or less transparent to the client(aside from atlasGeneration, described below).
 * Note - we used to initialize the backing atlas for the GrAtlasGlyphCache at initialization time.
 * However, this caused a regression, even when the GrAtlasGlyphCache was unused. We now initialize
