...
static void get_packed_glyph_image(
    ...
    GrMaskFormat glyphMaskFormat = GrGlyph::FormatFromSkGlyph(glyph.maskFormat());
    if (glyphMaskFormat == expectedMaskFormat) {
        int srcRB = glyph.rowBytes();
        // Notice this comparison is with the glyphs raw mask format, and not its GrFormat.
        ...
        if (glyphMaskFormat == kA565_GrMaskFormat && expectedMaskFormat == kARGB_GrMaskFormat) {
