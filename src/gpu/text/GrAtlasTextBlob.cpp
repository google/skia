...
inline std::unique_ptr<GrDrawOp> GrAtlasTextBlob::makeOp(
    ...
    geometry.fColor = info.maskFormat() == kARGB_GrMaskFormat ? GrColor_WHITE
                                                              : paint.filteredPremulGrColor();
    geometry.fX = x;
    geometry.fY = y;
    op->init();
...
inline void GrAtlasTextBlob::flushRun(GrRenderTargetContext* rtc,
...
void GrAtlasTextBlob::flushBigGlyphs(GrContext* context, GrRenderTargetContext* rtc,
    ...
    for (int i = 0; i < fBigGlyphs.count(); i++) {
        ...
        GrBlurUtils::drawPathWithMaskFilter(context, rtc, clip, bigGlyph.fPath,
                                            paint.skPaint(), ctm, nullptr, clipBounds, false);
    }
}
...
void GrAtlasTextBlob::flushRunAsPaths(GrContext* context, GrRenderTargetContext* rtc,
    ...
    switch (it.positioning()) {
        case SkTextBlob::kDefault_Positioning:
            GrTextUtils::DrawTextAsPath(context, rtc, clip, runPaint.skPaint(), viewMatrix,
                                        (const char *)it.glyphs(),
                                        textLen, x + offset.x(), y + offset.y(), clipBounds);
            break;
        case SkTextBlob::kHorizontal_Positioning:
            GrTextUtils::DrawPosTextAsPath(context, rtc, props, clip, runPaint.skPaint(), viewMatrix,
                                           (const char*)it.glyphs(),
...
void GrAtlasTextBlob::flushCached(GrContext* context, GrRenderTargetContext* rtc,
    ...
    for (int run = 0; !it.done(); it.next(), run++) {
        if (fRuns[run].fDrawAsPaths) {
            this->flushRunAsPaths(context, rtc, props, it, clip, paint,
                                  drawFilter, viewMatrix, clipBounds, x, y);
            continue;
        }
        this->flushRun(rtc, clip, run, viewMatrix, x, y,
                       paint, props, distanceAdjustTable, context->getAtlasGlyphCache());
    }
...
void GrAtlasTextBlob::flushThrowaway(GrContext* context, GrRenderTargetContext* rtc,
    ...
    for (int run = 0; run < fRunCount; run++) {
        this->flushRun(rtc, clip, run,
                       viewMatrix, x, y, paint, props, distanceAdjustTable,
