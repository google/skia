...
void GrTextBlob::SubRun::updateTexCoords(int begin, int end) {
    ...
    for (int i = begin; i < end; i++) {
        ...
        std::array<uint16_t, 4> uvs = glyph->fAtlasLocator.uvs1(pad);
...
std::tuple<bool, int> GrTextBlob::VertexRegenerator::updateTextureCoordinates(
    ...
    for (; i < end; i++) {
        ...
        if (!fFullAtlasManager->hasGlyph(fSubRun->maskFormat(), grGlyph)) {
            ...
            code = grStrike->addGlyphToAtlas1(skGlyph, fSubRun->maskFormat(), fSubRun->needsPadding(),
                                             fResourceProvider, fUploadTarget, fFullAtlasManager,
                                             grGlyph);
