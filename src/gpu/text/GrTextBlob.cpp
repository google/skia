...
DirectMaskSubRun::makeAtlasTextOp(const GrClip* clip, const SkMatrixProvider& viewMatrix,
    ...
    GrTextGeometries* const geoms = context->priv().textGeometries();
    using Geo = GrAtlasTextOp::Geometry;
    Geo& geo = geoms->emplace_back(*this,
                                   drawMatrix,
                                   drawOrigin,
                                   clipRect,
                                   SkRef(fBlob),
...
TransformedMaskSubRun::makeAtlasTextOp(const GrClip* clip,
    ...
    GrRecordingContext* const context = rtc->recordingContext();
    GrTextGeometries* geos = context->priv().textGeometries();
    using Geo = GrAtlasTextOp::Geometry;
    Geo& geo = geos->emplace_back(*this,
                                  drawMatrix,
                                  drawOrigin,
                                  SkIRect::MakeEmpty(),
                                  SkRef(fBlob),
