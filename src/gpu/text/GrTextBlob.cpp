...
std::tuple<bool, int> GrAtlasSubRun::regenerateAtlas(int begin, int end, GrMeshDrawOp::Target *target) {
...
auto GrAtlasSubRun::MakeTransformedMask(
        ...
        GrTextBlob* blob,
        SkArenaAlloc* alloc) -> GrSubRun* {
    return GrAtlasSubRun::InitForAtlas(kTransformedMask, drawables, strikeSpec, format, blob, alloc);
}
...
auto GrAtlasSubRun::InitForAtlas(SubRunType type,
    ...
    GrAtlasSubRun* subRun = alloc->make<GrAtlasSubRun>(type, blob, strikeSpec, format, bounds, vertexData);
