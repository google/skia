...
GrTextBlob::SubRun::SubRun(SubRunType type, GrTextBlob* textBlob, const SkStrikeSpec& strikeSpec,
                           GrMaskFormat format, const SkSpan<VertexData>& vertexData)
        : fType{type}
        , fBlob{textBlob}
        , fMaskFormat{format}
...
std::unique_ptr<char> GrTextBlob::SubRun::vertexData(int offset, int count) const {
    return nullptr;
}
...
void GrTextBlob::addOp(GrTextTarget* target,
    ...
    for (SubRun* subRun = fFirstSubRun; subRun != nullptr; subRun = subRun->fNextSubRun) {
        if (subRun->drawAsPaths()) {
            ...
            int glyphCount = subRun->fVertexData.count();
            if (0 == glyphCount) {
                continue;
            }
...
GrTextBlob::SubRun* GrTextBlob::makeSubRun(SubRunType type,
    ...
    SkSpan<Data> vertexData{
            (Data*)fAlloc.makeBytesAlignedTo(vertexCount * sizeof(Data), alignof(Data)),vertexCount};

    SubRun* subRun = fAlloc.make<SubRun>(type, this, strikeSpec, format, vertexData);
...
std::tuple<bool, int> GrTextBlob::VertexRegenerator::updateTextureCoordinates(
    ...
    int i = begin;
    for (; i < end; i++) {
        GrGlyph* grGlyph = fSubRun->fVertexData[i].glyph.grGlyph;
        SkASSERT(grGlyph);
...
std::tuple<bool, int> GrTextBlob::VertexRegenerator::regenerate(int begin, int end) {
    ...
    if (fSubRun->fAtlasGeneration != currentAtlasGen) {
        ...
        if (success && begin + glyphsPlacedInAtlas == fSubRun->fVertexData.count()) {
        ...
        if (end == fSubRun->fVertexData.count()) {
