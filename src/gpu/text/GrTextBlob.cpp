...
bool GrTextBlob::VertexRegenerator::doRegen(GrTextBlob::VertexRegenerator::Result* result,
    ...
    // brokenRun indicates if the atlas became full at any glyph other than the first glyph of
    // the SubRun.
    //
    // Notes:
    ...
    switch (code) {
        ...
        case GrDrawOpAtlas::ErrorCode::kSucceeded: {
            if (fActions.regenTextureCoordinates) {
                // If !brokenRun, then no glyphs from this SubRun were put into the atlas,
                // otherwise at least one glyph made it into the atlas from a previous call, and
                // this run needs special handling because of the atlas flush in the middle of it.
