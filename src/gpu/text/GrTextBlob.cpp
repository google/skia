...
auto GrTextBlob::SubRun::MakeDirectMask(
        ...
        GrTextBlob* blob,
        SkArenaAlloc* alloc) -> SubRun* {
    return SubRun::Make(kDirectMask, drawables, strikeSpec, format, blob, alloc);
}
...
auto GrTextBlob::SubRun::MakeTransformedMask(
        ...
        GrTextBlob* blob,
        SkArenaAlloc* alloc) -> SubRun* {
    return SubRun::Make(kTransformedMask, drawables, strikeSpec, format, blob, alloc);
}


auto GrTextBlob::SubRun::Make(SubRunType type,
                              const SkZip<SkGlyphVariant, SkPoint>& drawables,
                              const SkStrikeSpec& strikeSpec,
                              GrMaskFormat format,
                              GrTextBlob* blob,
