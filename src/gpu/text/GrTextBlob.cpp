...
sk_sp<GrTextBlob> GrTextBlob::Make(int glyphCount, int runCount) {
    ...
    size_t   blob = 0;
    size_t vertex = blob   + sk_align<alignof(char)>           (sizeof(GrTextBlob) * 1);
    size_t glyphs = vertex + sk_align<alignof(GrGlyph*)>       (sizeof(char) * verticesCount);
    size_t   runs = glyphs + sk_align<alignof(GrTextBlob::Run)>(sizeof(GrGlyph*) * glyphCount);
