...
std::tuple<bool, int> GrTextBlob::VertexRegenerator::updateTextureCoordinatesMaybeStrike(
    ...
    int firstNotInAtlas = begin;
    for (; firstNotInAtlas < end; firstNotInAtlas++) {
        GrGlyph* glyph = fSubRun->fGlyphs[firstNotInAtlas];
...
bool GrTextBlob::VertexRegenerator::regenerate(GrTextBlob::VertexRegenerator::Result* result,
    ...
    if (fActions.regenStrike || fActions.regenTextureCoordinates) {
        ...
        const int end = std::min((int)fSubRun->fGlyphs.size(), fCurrGlyph + maxGlyphs);
        auto [isOk, firstGlyphNotInAtlas] =
                this->updateTextureCoordinatesMaybeStrike(fCurrGlyph, end);
