...
GrTextBlob::SubRun::SubRun(GrTextBlob* textBlob, sk_sp<GrTextStrike>&& grStrike)
...
void GrTextBlob::SubRun::appendGlyphs(const SkZip<SkGlyphVariant, SkPoint>& drawables) {
    ...
    SkScalar strikeToSource = grStrike->strikeToSourceRatio();
    ...
    for (SubRun* subRun = fFirstSubRun; subRun != nullptr; subRun = subRun->fNextSubRun) {
        if (subRun->drawAsPaths()) {
            ...
            for (const auto& pathGlyph : subRun->fPaths) {
                ...
                GrTextStrike* strike = subRun->strike();
                SkMatrix pathMatrix = SkMatrix::MakeScale(strike->strikeToSourceRatio());
