...
GrDrawOpAtlas::ErrorCode VR::addGlyphToAtlas(const SkGlyph& skGlyph,
                                             const SubRun& subRun,
                                             GrGlyph* grGlyph) {
    SkASSERT(grGlyph != nullptr);
    SkASSERT(skGlyph.image() != nullptr);

    GrMaskFormat expectedMaskFormat = fFullAtlasManager->resolveMaskFormat(subRun.maskFormat());
    int bytesPerPixel = GrMaskFormatBytesPerPixel(expectedMaskFormat);

    SkASSERT(!subRun.needsPadding() || skGlyph.maskFormat() != SkMask::kSDF_Format);

    // Add 1 pixel padding around grGlyph if needed.
    const int width = subRun.needsPadding() ? skGlyph.width() + 2 : skGlyph.width();
    const int height = subRun.needsPadding() ? skGlyph.height() + 2 : skGlyph.height();
...
std::tuple<bool, int> GrTextBlob::VertexRegenerator::updateTextureCoordinates(
    ...
    for (; i < end; i++) {
        ...
        if (!fFullAtlasManager->hasGlyph(fSubRun->maskFormat(), grGlyph)) {
            ...
            code = addGlyphToAtlas(skGlyph, *fSubRun, grGlyph);
