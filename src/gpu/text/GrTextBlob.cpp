...
void GrTextBlob::SubRun::appendGlyphs(const SkZip<SkGlyphVariant, SkPoint>& drawables) {
    ...
    GrGlyph** glyphCursor = fGlyphs2.data();
...
void GrTextBlob::SubRun::translateVerticesIfNeeded(
    ...
    if (translation != SkPoint{0, 0}) {
        size_t vertexStride = this->vertexStride();
        for (size_t quad = 0; quad < fGlyphs2.size(); quad++) {
...
void GrTextBlob::flush(GrTextTarget* target, const SkSurfaceProps& props,
    ...
    for (SubRun* subRun = fFirstSubRun; subRun != nullptr; subRun = subRun->fNextSubRun) {
        if (subRun->drawAsPaths()) {
            ...
            int glyphCount = subRun->fGlyphs2.size();
            if (0 == glyphCount) {
                continue;
            }
...
std::tuple<bool, int> GrTextBlob::VertexRegenerator::updateTextureCoordinates1(
    ...
    int i = begin;
    for (; i < end; i++) {
        GrGlyph* grGlyph = fSubRun->fGlyphs2[i];
        SkASSERT(grGlyph);

        if (!fFullAtlasManager->hasGlyph1(fSubRun->maskFormat(), grGlyph)) {
...
std::tuple<bool, int> GrTextBlob::VertexRegenerator::regenerate1(int begin, int end) {
    ...
    if (fSubRun->fAtlasGeneration != currentAtlasGen) {
        ...
        if (success && begin + glyphsPlacedInAtlas == (int)fSubRun->fGlyphs2.size()) {
        ...
        if (end == (int)fSubRun->fGlyphs2.size()) {
