...
void GrTextBlob::SubRun::fillVertexData(
    ...
    switch (fType) {
        case kDirectMask: {
            if (clip.isEmpty()) {
                if (this->maskFormat() != kARGB_GrMaskFormat) {
                    ...
                    SkASSERT(sizeof(Quad) == this->quadOffset(1));
        ...
        case kTransformedSDFT: {
            if (!matrix.hasPerspective()) {
                using Quad = Mask2DVertex[4];
                if (sizeof(Quad) != this->quadOffset(1)) {
                    SkDebugf(
                        "Quad %d offset %d gr format %d is distance field %d hasw %d hasp %d\n",
                            (int)sizeof(Quad), (int)this->quadOffset(1), (int)this->maskFormat(),
                             (int)this->drawAsDistanceFields(), (int)this->hasW(),
                        (int)matrix.hasPerspective());
                }
                //SkASSERT(sizeof(Quad) == this->quadOffset(1)); // here
