...
sk_sp<GrTextBlob> GrTextBlob::Make(const SkGlyphRunList& glyphRunList, const SkMatrix& drawMatrix) {
    ...
    size_t neededForSubRun = GrBagOfBytes::MinimumSizeWithOverhead(
    ...
    size_t allocationSize = sizeof(GrTextBlob) + neededForSubRun;
    ...
    sk_sp<GrTextBlob> blob{new (allocation)
                            GrTextBlob(neededForSubRun, drawMatrix, initialLuminance)};
...
// -- GrBagOfBytes ---------------------------------------------------------------------------------
GrBagOfBytes::GrBagOfBytes(char* bytes, int size, int firstHeapAllocation)
...
GrBagOfBytes::GrBagOfBytes(int firstHeapAllocation)
...
char* GrBagOfBytes::alignedBytes(int unsafeSize, int unsafeAlignment) {
...
GrSubRunAllocator::GrSubRunAllocator(char* bytes, int size, int firstHeapAllocation)
        : fAlloc{bytes, size, firstHeapAllocation} {}
