...
void GrTextBlob::SubRun::appendGlyphs(const SkZip<SkGlyphVariant, SkPoint>& drawables) {
    ...
    SkMatrix matrix{fCurrentMatrix};
    matrix.preTranslate(fCurrentOrigin.x(), fCurrentOrigin.y());
    ...
    for (auto [variant, pos] : drawables) {
        ...
        SkPoint glyphOrigin = matrix.mapXY(pos.x(), pos.y());
        bounds.offset(glyphOrigin);
        fVertexBounds.joinNonEmptyArg(bounds);
