...
std::tuple<bool, int> GrTransformedMaskSubRun::regenerateAtlas(int begin, int end, GrMeshDrawOp::Target *target) const {
    return fGlyphs.regenerateAtlas(begin, end, fMaskFormat, 1, target);
}

void GrTransformedMaskSubRun::fillVertexData(void *vertexDst, int offset, int count, GrColor color, const SkMatrix &drawMatrix, SkPoint drawOrigin, SkIRect clip) const {
...
sk_sp<GrTextBlob> GrTextBlob::Make(const SkGlyphRunList& glyphRunList, const SkMatrix& drawMatrix) {
    ...
    constexpr size_t alignDiff = alignof(GrSDFTSubRun) - alignof(GrSDFTSubRun::VertexData);
    constexpr size_t vertexDataToSubRunPadding = alignDiff > 0 ? alignDiff : 0;
    size_t arenaSize = sizeof(GrSDFTSubRun::VertexData) * glyphRunList.totalGlyphCount()
                     + glyphRunList.runCount() * (sizeof(GrSDFTSubRun) + vertexDataToSubRunPadding);
