...
void GrTextBlob::SubRun::updateTexCoords1(int begin, int end,
    ...
    for (int i = begin; i < end; i++, barArray++) {
        ...
        if (this->drawAsDistanceFields()) {
            u0 = barArray->fAtlasLocator.fLoc.fX + SK_DistanceFieldInset;
            v0 = barArray->fAtlasLocator.fLoc.fY + SK_DistanceFieldInset;
            u1 = u0 + width - 2 * SK_DistanceFieldInset;
            v1 = v0 + height - 2 * SK_DistanceFieldInset;
        } else {
            u0 = barArray->fAtlasLocator.fLoc.fX;
            v0 = barArray->fAtlasLocator.fLoc.fY;
            u1 = u0 + width;
            v1 = v0 + height;
        }

        // We pack the 2bit page index in the low bit of the u and v texture coords
        uint32_t pageIndex = barArray->fAtlasLocator.pageIndex1();
