...
GrTextBlob::SubRun::SubRun(SubRunType type, GrTextBlob* textBlob, const SkStrikeSpec& strikeSpec,
                           GrMaskFormat format, const SkSpan<SkPackedGlyphID>& packedGlyphIDs,
                           const SkSpan<GrGlyph*>& glyphs, const SkSpan<char>& vertexData)
        : fType{type}
        , fBlob{textBlob}
        , fMaskFormat{format}
        , fPackedGlyphIDs(packedGlyphIDs)
        , fGlyphs3{glyphs}
...
GrTextBlob::SubRun::SubRun(GrTextBlob* textBlob, const SkStrikeSpec& strikeSpec)
        ...
        , fPackedGlyphIDs(SkSpan<SkPackedGlyphID>{})
...
void GrTextBlob::SubRun::appendGlyphs(const SkZip<SkGlyphVariant, SkPoint>& drawables) {
    ...
    SkPackedGlyphID* packedIDCursor = fPackedGlyphIDs.data();
    ...
    for (auto [variant, pos] : drawables) {
        ...
        *packedIDCursor++ = skGlyph->getPackedID();
    }
}
...
void GrTextBlob::SubRun::itsAlive(GrStrikeCache* strikeCache) {
    fStrike2 = fStrikeSpec.findOrCreateGrStrike(strikeCache);

    for (auto tmp : fPackedGlyphIDs) {
        GrGlyph* grGlyph = fStrike2->getGlyph1(tmp);
    }
//    fGlyphs3{SkSpan<GrGlyph*>{}}
}
...
GrTextBlob::SubRun* GrTextBlob::makeSubRun(SubRunType type,
    ...
    SkSpan<SkPackedGlyphID> packedGlyphIds{fAlloc.makeArrayDefault<SkPackedGlyphID>(drawables.size()), drawables.size()};
    ...
    SubRun* subRun = fAlloc.make<SubRun>(type, this, strikeSpec, format, packedGlyphIds, glyphs, vertexData);
...
std::tuple<bool, int> GrTextBlob::VertexRegenerator::updateTextureCoordinates(
    ...
    int i = begin;
    for (; i < end; i++) {
        GrGlyph* grGlyph = fSubRun->fGlyphs3[i];
        SkASSERT(grGlyph);
