...
public:
    PathSubRun(bool isAntiAliased,
               ...
               std::unique_ptr<PathGlyph[], SkArena::ArrayDestroyer> pathData);
...
private:
    ...
    const std::unique_ptr<PathGlyph[], SkArena::ArrayDestroyer> fPathData;
};
...
public:
    ...
    static GlyphVector Make(
            const SkStrikeSpec& spec, SkSpan<SkGlyphVariant> glyphs, SkArena* alloc);
...
void GrTextBlob::insertSubRun(std::unique_ptr<GrSubRun, SkArena::Destroyer> subRun) {
