...
void GrTextBlob::flush(GrTextTarget* target, const SkSurfaceProps& props,
    ...
    for (auto& subRun : fSubRuns) {
        if (subRun.drawAsPath()) {
            ...
            SkMatrix sourceMatrix = SkMatrix::MakeScale(subRun.fStrikeSpec.strikeToSourceRatio());
            ...
            // blob.
            sourceMatrix.postTranslate(originShift.x(), originShift.y());
            for (const auto& pathGlyph : subRun.fPaths) {
                SkMatrix pathMatrix{sourceMatrix};
                sourceMatrix.postTranslate(pathGlyph.fOrigin.x(), pathGlyph.fOrigin.y());
