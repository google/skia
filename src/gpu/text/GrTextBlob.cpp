...
void GrTextBlob::SubRun::translateVerticesIfNeeded(
    ...
    if (translation != SkPoint{0, 0}) {
        size_t vertexStride = this->vertexStride();
        for (int quad = 0; quad < this->numGlyphs(); quad++) {
...
void GrTextBlob::SubRun::updateTexCoords1(int begin, int end,
                                          const GrDrawOpAtlas::Foo* fooArray) {
    ...
    for (int i = begin; i < end; i++, fooArray++) {
        ...
        if (this->drawAsDistanceFields()) {
            u0 = fooArray->fLoc.fX + SK_DistanceFieldInset;
            v0 = fooArray->fLoc.fY + SK_DistanceFieldInset;
        ...
        uint32_t pageIndex = fooArray->pageIndex1();
...
void GrTextBlob::flush(GrTextTarget* target, const SkSurfaceProps& props,
    ...
    for (SubRun* subRun = fFirstSubRun; subRun != nullptr; subRun = subRun->fNextSubRun) {
        if (subRun->drawAsPaths()) {
            ...
            int glyphCount = subRun->numGlyphs();
            if (0 == glyphCount) {
                continue;
            }
...
std::tuple<bool, int> GrTextBlob::VertexRegenerator::updateTextureCoordinates1(
    ...
    SkTDArray<GrDrawOpAtlas::Foo*> fooArray;
    fooArray.reserve(end-begin);
    ...
    for (; i < end; i++) {
        ...
        GrDrawOpAtlas::Foo* foo = fFullAtlasManager->getFoo(fSubRun->maskFormat(), grGlyph.packedID());
        if (foo->notResident()) {
            const SkGlyph& skGlyph = *fMetricsAndImages->glyph(grGlyph.packedID());
            if (skGlyph.image() == nullptr) {
                return {false, 0};
            }
            code = grStrike->addGlyphToAtlas1(skGlyph, fSubRun->maskFormat(),
                                              fSubRun->needsTransform(), fResourceProvider,
                                              fUploadTarget, fFullAtlasManager, foo);
            if (code != GrDrawOpAtlas::ErrorCode::kSucceeded) {
                break;
            }
        }
        fFullAtlasManager->addGlyphToBulkAndSetUseToken1(
                fSubRun->bulkUseToken(), fSubRun->maskFormat(), foo,
                tokenTracker->nextDrawToken());

        fooArray.push_back(foo);
    }
    int glyphsPlacedInAtlas = i - begin;

    // Update the quads with the new atlas coordinates.
    fSubRun->updateTexCoords1(begin, begin + glyphsPlacedInAtlas, *fooArray.begin());
...
std::tuple<bool, int> GrTextBlob::VertexRegenerator::regenerate1(int begin, int end) {
    ...
    if (fSubRun->fAtlasGeneration != currentAtlasGen) {
        ...
        if (success && begin + glyphsPlacedInAtlas == fSubRun->numGlyphs()) {
