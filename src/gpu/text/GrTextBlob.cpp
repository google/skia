...
GlyphVector GlyphVector::Make(
    ...
    Variant* variants = alloc->makeArray<Variant>(glyphs.size());
    for (auto [i, gv] : SkMakeEnumerate(glyphs)) {
        variants[i] = gv.glyph()->getPackedID();
    }
...
GrAtlasSubRun* TransformedMaskSubRunNoCache::Make(const SkZip<SkGlyphVariant, SkPoint>& drawables,
                                                  const SkStrikeSpec& strikeSpec,
                                                  GrMaskFormat format,
                                                  SkArenaAlloc* alloc) {
    ...
    GrAtlasSubRun* subRun = alloc->make<TransformedMaskSubRunNoCache>(
...
GrAtlasSubRun* SDFTSubRunNoCache::Make(
        const SkZip<SkGlyphVariant, SkPoint>& drawables,
        const SkFont& runFont,
        const SkStrikeSpec& strikeSpec,
        SkArenaAlloc* alloc) {
    ...
    return alloc->make<SDFTSubRunNoCache>(
            kA8_GrMaskFormat,
            bounds,
            vertexData,
            GlyphVector::Make(strikeSpec, drawables.get<0>(), alloc),
        ...
        public:
            ...
            void processSourceMasks(const SkZip<SkGlyphVariant, SkPoint>& drawables,
                ...
                for (size_t i = 1; i < drawables.size(); i++) {
                    ...
                    if (format != nextFormat) {
                        auto sameFormat = drawables.subspan(startIndex, i - startIndex);
                        GrAtlasSubRun* subRun = TransformedMaskSubRunNoCache::Make(
            ...
            void
            processSourcePaths(const SkZip<SkGlyphVariant, SkPoint>& drawables,
            ...
            void processSourceSDFT(const SkZip<SkGlyphVariant, SkPoint>& drawables,
                ...
                if (drawables.empty()) { return; }
                GrAtlasSubRun* subRun =
                        SDFTSubRunNoCache::Make(drawables, runFont, strikeSpec, fAlloc);
                this->draw(subRun);
            }
        private:
