...
static void assign2D(T& dst, SkPoint pt, GrColor color, GrTextBlob::AtlasPt uv) {
    if constexpr (std::is_same<T, GrTextBlob::ARGB2DVertex>::value) {
        ...
        static_assert(std::is_same<T, GrTextBlob::Mask2DVertex>::value);
        dst = {pt, color, uv};
    }
}
...
static void assign3D(T& dst, SkPoint3 pt, GrColor color, GrTextBlob::AtlasPt uv) {
    if constexpr (std::is_same<T, GrTextBlob::ARGB3DVertex>::value) {
        ...
        static_assert(std::is_same<T, GrTextBlob::Mask3DVertex>::value);
        dst = {pt, color, uv};
    }
}
...
void GrTextBlob::SubRun::fillVertexData(
    ...
    auto transformed2D = [&](auto dst, SkScalar dstPadding, SkScalar srcPadding) {
        ...
        for (auto[dst, vertexData] : SkMakeZip(dst, fVertexData.subspan(offset, count))) {
    ...
    auto transformed3D = [&](auto dst, SkScalar dstPadding, SkScalar srcPadding) {
        ...
        for (auto[dst, vertexData] : SkMakeZip(dst, fVertexData.subspan(offset, count))) {
            ...
            auto[al, at, ar, ab] =
            glyph.grGlyph->fAtlasLocator.getUVs(srcPadding);
            dst[0] = {lt, color, {al, at}};  // L,T
            dst[1] = {lb, color, {al, ab}};  // L,B
            dst[2] = {rt, color, {ar, at}};  // R,T
            dst[3] = {rb, color, {ar, ab}};  // R,B
    ...
    auto direct2D = [&](auto dst, SkIRect* clip) {
        ...
        for (auto[dst, vertexData] : SkMakeZip(dst, fVertexData.subspan(offset, count))) {
            ...
            if (clip == nullptr) {
                ...
                dst[0] = {{dl, dt}, color, {al, at}};  // L,T
                dst[1] = {{dl, db}, color, {al, ab}};  // L,B
                dst[2] = {{dr, dt}, color, {ar, at}};  // R,T
                dst[3] = {{dr, db}, color, {ar, ab}};  // R,B
            } else {
                ...
                dst[0] = {{dl, dt}, color, {tl, tt}};  // L,T
                dst[1] = {{dl, db}, color, {tl, tb}};  // L,B
                dst[2] = {{dr, dt}, color, {tr, tt}};  // R,T
                dst[3] = {{dr, db}, color, {tr, tb}};  // R,B
            }
        }
    };
...
void GrTextBlob::SubRun::textTargetVertexData(
    ...
    for (auto[dst, vertexData] : SkMakeZip(vertexDst, fVertexData.subspan(offset, count))) {
        auto[glyph, pos, rect] = vertexData;
        SkPoint sLT = (SkPoint::Make(rect.l, rect.t) + inset) * strikeToSource + pos + origin,
                sRB = (SkPoint::Make(rect.r, rect.b) - inset) * strikeToSource + pos + origin;
        SkPoint3 lt = SkPoint3{sLT.x(), sLT.y(), 1.f},
                 lb = SkPoint3{sLT.x(), sRB.y(), 1.f},
                 rt = SkPoint3{sRB.x(), sLT.y(), 1.f},
                 rb = SkPoint3{sRB.x(), sRB.y(), 1.f};
        auto[al, at, ar, ab] =
        glyph.grGlyph->fAtlasLocator.getUVs(SK_DistanceFieldInset);
        dst[0] = {lt, color, {al, at}};  // L,T
        dst[1] = {lb, color, {al, ab}};  // L,B
        dst[2] = {rt, color, {ar, at}};  // R,T
        dst[3] = {rb, color, {ar, ab}};  // R,B
...
sk_sp<GrTextBlob> GrTextBlob::Make(const SkGlyphRunList& glyphRunList,
    ...
    size_t vertexDataToSubRunPadding = 8;
