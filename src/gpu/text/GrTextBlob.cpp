...
sk_sp<GrTextBlob> GrTextBlob::Make(const SkGlyphRunList& glyphRunList,
    ...
    size_t subRuns = sizeof(GrTextBlob);
    size_t subRunsSize = glyphRunList.runCount() * sizeof(SubRun);
    static_assert(alignof(SubRun) >= alignof(GrGlyph*));
    size_t glyphs = subRuns + glyphRunList.runCount() * sizeof(SubRun);
    static_assert(alignof(GrGlyph*) >= alignof(SDFT3DVertex));
    size_t vertex = glyphs + sizeof(GrGlyph*) * glyphRunList.totalGlyphCount();
    size_t allocationSize = vertex + quadSize * glyphRunList.totalGlyphCount();
