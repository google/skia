...
bool GrAtlasTextContext::canDraw(const SkPaint& paint,
                                 const SkMatrix& viewMatrix,
                                 const SkSurfaceProps& props,
                                 const GrShaderCaps& shaderCaps) {
    return GrTextUtils::CanDrawAsDistanceFields(paint, viewMatrix, props, shaderCaps) ||
           !SkDraw::ShouldDrawTextAsPaths(paint, viewMatrix);
}
...
void GrAtlasTextContext::RegenerateTextBlob(GrAtlasTextBlob* cacheBlob,
    ...
    for (int run = 0; !it.done(); it.next(), run++) {
        ...
        if (GrTextUtils::CanDrawAsDistanceFields(runPaint.skPaint(), viewMatrix, props,
                                                 shaderCaps)) {
        ...
        } else if (SkDraw::ShouldDrawTextAsPaths(runPaint.skPaint(), viewMatrix)) {
...
inline GrAtlasTextBlob* GrAtlasTextContext::CreateDrawTextBlob(
        GrTextBlobCache* blobCache, GrAtlasGlyphCache* fontCache, const GrShaderCaps& shaderCaps,
        const GrTextUtils::Paint& paint, uint32_t scalerContextFlags, const SkMatrix& viewMatrix,
        const SkSurfaceProps& props, const char text[], size_t byteLength, SkScalar x, SkScalar y) {
    int glyphCount = paint.skPaint().countText(text, byteLength);

    GrAtlasTextBlob* blob = blobCache->createBlob(glyphCount, 1);
    blob->initThrowawayBlob(viewMatrix, x, y);

    if (GrTextUtils::CanDrawAsDistanceFields(paint.skPaint(), viewMatrix, props, shaderCaps)) {
...
inline GrAtlasTextBlob* GrAtlasTextContext::CreateDrawPosTextBlob(
        GrTextBlobCache* blobCache, GrAtlasGlyphCache* fontCache, const GrShaderCaps& shaderCaps,
        const GrTextUtils::Paint& paint, uint32_t scalerContextFlags, const SkMatrix& viewMatrix,
        const SkSurfaceProps& props, const char text[], size_t byteLength, const SkScalar pos[],
...
void GrAtlasTextContext::drawText(GrContext* context, GrRenderTargetContext* rtc,
    ...
    if (this->canDraw(skPaint, viewMatrix, props, *context->caps()->shaderCaps())) {
        sk_sp<GrAtlasTextBlob> blob(CreateDrawTextBlob(
                ...
                *context->caps()->shaderCaps(), paint, ComputeScalerContextFlags(rtc), viewMatrix,
    ...
    GrTextUtils::DrawTextAsPath(context, rtc, clip, paint.skPaint(), viewMatrix, text, byteLength,
                                x, y, regionClipBounds);
}
...
void GrAtlasTextContext::drawPosText(GrContext* context, GrRenderTargetContext* rtc,
    ...
    // fall back to drawing as a path
    GrTextUtils::DrawPosTextAsPath(context, rtc, props, clip, paint.skPaint(), viewMatrix, text,
