...
void GrAtlasTextContext::drawDFPosText(GrAtlasTextBlob* blob, int runIndex,
    ...
    SkTDArray<char> largeFallbackTxt;
    SkTDArray<SkScalar> largeFallbackPos;
    SkScalar largeFallbackTextSize = SK_Scalar1;
    SkScalar largeFallbackTextRatio = SK_Scalar1;
    ...
    while (text < stop) {
        ...
        if (glyph.fWidth) {
            ...
            if (glyph.fMaskFormat != SkMask::kARGB32_Format) {
                ...
                SkScalar maxScale = viewMatrix.getMaxScale();
                SkScalar scaledTextSize = textSize*viewMatrix.getMaxScale();

                // TODO: find if we've set this magic number somewhere
                if (!viewMatrix.hasPerspective() && scaledTextSize > 200) {
                    largeFallbackTxt.append(SkToInt(text - lastText), lastText);
                    *largeFallbackPos.append() = pos[0];
                    if (2 == scalarsPerPosition) {
                        *largeFallbackPos.append() = pos[1];
                    }
                    largeFallbackTextSize = 200 / maxScale;
                    largeFallbackTextRatio = scaledTextSize / 200;
                } else {
    ...
    if (largeFallbackTxt.count()) {
        // Do stuff
        blob->initOverride(runIndex);
        SkPaint largePaint(paint);
        largePaint.setTextSize(largeFallbackTextSize);
        ...
        GrAtlasTextContext::DrawBmpPosText(blob, runIndex, fontCache, props, textPaint,
                                           scalerContextFlags, viewMatrix, largeFallbackTxt.begin(),
                                           largeFallbackTxt.count(), largeFallbackPos.begin(),
                                           scalarsPerPosition, offset, largeFallbackTextRatio);
    }
}
...
void GrAtlasTextContext::DfAppendGlyph(GrAtlasTextBlob* blob, int runIndex,
    ...
    width *= textRatio;
    height *= textRatio;
    SkRect glyphRect = SkRect::MakeXYWH(sx + dx, sy + dx, width, height);
