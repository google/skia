...
GrAtlasTextContext::GrAtlasTextContext(const Options& options)
    ...
    if (fOptions.fMaxDistanceFieldFontSize < 0.f) {
        fOptions.fMaxDistanceFieldFontSize = kDefaultMaxDistanceFieldFontSize;
    }
    if (fOptions.fMinDistanceFieldFontSize < 0.f) {
        fOptions.fMinDistanceFieldFontSize = kDefaultMinDistanceFieldFontSize;
    }
}
...
void GrAtlasTextContext::regenerateTextBlob(GrAtlasTextBlob* cacheBlob,
    ...
    for (int run = 0; !it.done(); it.next(), run++) {
        ...
        if (CanDrawAsDistanceFields(runPaint, viewMatrix, props,
                                    shaderCaps.shaderDerivativeSupport(), fOptions)) {
...
GrAtlasTextContext::makeDrawTextBlob(GrTextBlobCache* blobCache,
    ...
    if (CanDrawAsDistanceFields(paint, viewMatrix, props, shaderCaps.shaderDerivativeSupport(),
                                fOptions)) {
...
GrAtlasTextContext::Options GrAtlasTextContext::MakeDefaultOptions() {
    Options options;
...
bool GrAtlasTextContext::CanDrawAsDistanceFields(const SkPaint& skPaint, const SkMatrix& viewMatrix,
    ...
    if (skPaint.getMaskFilter() || !shaderDerivativeSupport) {
        return false;
    }
