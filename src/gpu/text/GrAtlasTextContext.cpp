...
void GrAtlasTextContext::drawTextBlob(GrContext* context, GrRenderTargetContext* rtc,
    ...
    if (cacheBlob) {
        if (cacheBlob->mustRegenerate(skPaint, grPaint.getColor(), blurRec, viewMatrix, x, y)) {
            ...
            RegenerateTextBlob(cacheBlob.get(), context->getAtlasFontCache(),
            ...
            if (CACHE_SANITY_CHECK) {
                ...
                RegenerateTextBlob(sanityBlob.get(), context->getAtlasFontCache(),
...
inline GrAtlasTextBlob*
GrAtlasTextContext::CreateDrawPosTextBlob(GrTextBlobCache* blobCache, GrAtlasFontCache* fontCache,
...
void GrAtlasTextContext::drawText(GrContext* context,
    ...
    if (context->abandoned()) {
        ...
        sk_sp<GrAtlasTextBlob> blob(
            CreateDrawTextBlob(context->getTextBlobCache(), context->getAtlasFontCache(),
...
DRAW_BATCH_TEST_DEFINE(TextBlobBatch) {
    ...
    return blob
            ->test_makeOp(textLen, 0, 0, viewMatrix, x, y, color, skPaint, gSurfaceProps,
                          gTextContext->dfAdjustTable(), context->getAtlasFontCache())
            .release();
}
