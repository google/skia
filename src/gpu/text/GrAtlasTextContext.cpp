...
void GrAtlasTextContext::DrawBmpText(GrAtlasTextBlob* blob, int runIndex,
    ...
    bool drawAsPaths = SkDraw::ShouldDrawTextAsPaths(paint.skPaint(), viewMatrix);
    const SkMatrix& ctm = drawAsPaths ? SkMatrix::I() : viewMatrix;
    SkPaint tempPaint(paint.skPaint());
    if (drawAsPaths) {
        tempPaint.setStyle(SkPaint::kFill_Style);
    }
    SkGlyphCache* cache = blob->setupCache(runIndex, props, scalerContextFlags, tempPaint, &ctm);
    SkFindAndPlaceGlyph::ProcessText(paint.skPaint().getTextEncoding(), text, byteLength, {x, y},
                                     ctm, paint.skPaint().getTextAlign(), cache,
...
void GrAtlasTextContext::DrawBmpPosText(GrAtlasTextBlob* blob, int runIndex,
    ...
    SkFindAndPlaceGlyph::ProcessPosText(
            paint.skPaint().getTextEncoding(), text, byteLength, offset, ctm, pos,
            ...
            [&](const SkGlyph& glyph, SkPoint position, SkPoint rounding) {
                ...
                BmpAppendGlyph(blob, runIndex, fontCache, &currStrike, glyph,
                               ...
                               SkScalarFloorToScalar(position.fY),
                               paint.filteredPremulColor(), cache, textRatio, drawAsPaths);
            });
...
void GrAtlasTextContext::BmpAppendGlyph(GrAtlasTextBlob* blob, int runIndex,
                                        ...
                                        SkScalar textRatio, bool asPath) {
    if (!*strike) {
    ...
    glyph->fTooLargeForAtlas = glyph->fTooLargeForAtlas || asPath;
