...
class GrAtlasTextIter : SkTextBaseIter {
public:
    GrAtlasTextIter(const char text[], size_t length, const SkPaint& paint,
    ...
    */
    bool GrAtlasTextIter::next(const SkGlyph** skGlyph, const SkPath** path, SkScalar* xpos) {
        SkASSERT(skGlyph);
        SkASSERT(path);
        SkASSERT(xpos);
        if (fText < fStop) {
            ...
            if (glyph.fWidth) {
                if (SkMask::kARGB32_Format == glyph.fMaskFormat) {
                    *skGlyph = &glyph;  // not sure if this is safe
                    *path = nullptr;
                } else {
                    *skGlyph = nullptr;
...
void GrAtlasTextContext::FallbackTextHelper::drawText(GrAtlasTextBlob* blob, int runIndex,
    ...
    if (fFallbackTxt.count()) {
        blob->initOverride(runIndex);   // Hm, we might not even have any base text
        blob->setHasBitmap();
        ...
        if (fUseScaledFallback) {
            ...
            cache = blob->setupCache(runIndex, props, scalerContextFlags, textPaint,
                                     &modMatrix);
        } else {
...
void GrAtlasTextContext::DrawBmpTextAsPaths(GrAtlasTextBlob* blob, int runIndex,
    ...
    GrAtlasTextIter iter(text, byteLength, paint, true);
    FallbackTextHelper fallbackTextHelper(viewMatrix, pathPaint.getTextSize(), iter.getPathScale(),
    ...
    while (iter.next(&iterGlyph, &iterPath, &xpos)) {
        if (iterGlyph) {
            SkPoint pos = SkPoint::Make(xpos, 0);
    ...
    fallbackTextHelper.drawText(blob, runIndex, fontCache, props, paint, scalerContextFlags);
}
...
void GrAtlasTextContext::DrawBmpPosTextAsPaths(GrAtlasTextBlob* blob, int runIndex,
    ...
    FallbackTextHelper fallbackTextHelper(viewMatrix, paint.getTextSize(), matrixScale,
                                          fontCache->getGlyphSizeLimit());
