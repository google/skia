...
public:
     ...
     */
    void regenInBatch(GrDrawOp::Target* target, GrAtlasFontCache* fontCache,
...
private:
    ...
    void regenInOp(GrDrawOp::Target* target, GrAtlasFontCache* fontCache, GrBlobRegenHelper* helper,
    ...
    inline sk_sp<GrDrawOp> makeOp(const Run::SubRunInfo& info, int glyphCount, int run, int subRun,
                                  ...
                                  const SkPaint& skPaint, const SkSurfaceProps& props,
                                  const GrDistanceFieldAdjustTable* distanceAdjustTable,
                                  bool useGammaCorrectDistanceTable, GrAtlasFontCache* cache);
