...
/**
 * Used to produce vertices for a sub run of a blob. The vertices are cached in the blob itself.
 * This is used on each draw to check whether the vertices need to be updated either to reflect
 * the state of the atlas or for different draw parameters. In rare cases the draw may have to
 * interrupted and flushed in the middle of the sub run in order to free up atlas space. Thus
 * this generator is stateful and should be invoked in a loop until the entire sub run has
 * been drawn.
 */
