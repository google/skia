...
bool GrAtlasFontCache::initAtlas(GrMaskFormat format) {
    ...
    if (!fAtlases[index]) {
        ...
        fAtlases[index] = fContext->resourceProvider()->makeAtlas(config, width, height,
                                                                  numPlotsX, numPlotsY,
                                                                  &GrAtlasFontCache::HandleEviction,
                                                                  (void*)this);
        if (!fAtlases[index]) {
            return false;
        }
...
GrAtlasFontCache::GrAtlasFontCache(GrContext* context)
    : fContext(context)
...
GrAtlasFontCache::~GrAtlasFontCache() {
...
void GrAtlasFontCache::HandleEviction(GrBatchAtlas::AtlasID id, void* ptr) {
    GrAtlasFontCache* fontCache = reinterpret_cast<GrAtlasFontCache*>(ptr);
...
void GrAtlasFontCache::dump() const {
...
void GrAtlasFontCache::setAtlasSizes_ForTesting(const GrBatchAtlasConfig configs[3]) {
...
GrAtlasTextStrike::GrAtlasTextStrike(GrAtlasFontCache* owner, const SkDescriptor& key)
    : fFontScalerKey(key)
    , fPool(9/*start allocations at 512 bytes*/)
    , fAtlasFontCache(owner) // no need to ref, it won't go away before we do
    , fAtlasedGlyphs(0)
    , fIsAbandoned(false) {}
...
bool GrAtlasTextStrike::addGlyphToAtlas(GrDrawOp::Target* target,
    ...
    bool success = fAtlasFontCache->addToAtlas(this, &glyph->fID, target, expectedMaskFormat,
