...
public:
    ...
    static constexpr int MinimumSizeWithOverhead(int requestedSize, int assumedAlignment = 1) {
        return CalculateMinimumSizeWithOverhead(
                requestedSize, assumedAlignment, sizeof(Block), kMaxAlignment);
    }

    static constexpr int CalculateMinimumSizeWithOverhead(
        ...
        auto diffOrZero = [](int a, int b) {int diff = a - b; return diff > 0 ? diff : 0; };
        const int biasGivenAlignment = diffOrZero(maxAlignment, assumedAlignment);

        auto alignUp = [](int size, int alignment) {return (size + (alignment - 1)) & -alignment;};

        // minimumSize is the minimum number of bytes needed to assure that at least
        // requestedSize bytes is allocated plus the overhead of the block constrained to placing
        // the block on a kMaxAlignment address.
        // For example given memory from malloc where alignof(max_align_t) = 8:
        //    assumedAlignment = 8, requestedSize = 63, sizeof(Block) = 16, kMaxAlignment = 16,
        //    biasGivenAlignment = 8
        //       temp = 63 + 16 + 8 = 87
        //       minimumSize = alignUp(87, 8) = (87 + 7) & ~7 = 88
        // This results in a situation where 8 bytes may be unused trapped above the block. If
        // byte 64 is aligned to 16 then the block is placed there, else the block is placed at
        // byte 72.
        //
        // Note: on 64-bit machines with alignof(max_align_t) of 16, and 32-bit machines with
        // alignof(max_align_t) of 8 there are no unused bytes.

        int minimumSize =
                alignUp(requestedSize + blockSize + biasGivenAlignment, assumedAlignment);
    ...
    template <int size>
    using Storage = std::array<char, MinimumSizeWithOverhead(size, 1)>;

    // Returns the number of bytes suitable for holding n Ts.
