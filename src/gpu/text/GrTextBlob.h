...
public:
    ...
    static constexpr int MinimumSizeWithOverhead(
        ...
        int minimumSize = alignUp(requestedSize + blockSize, assumedAlignment);
        if (assumedAlignment < maxAlignment) {
            // Because assumedAlignment < maxAlignment minimumSize does not guarantee that block
            // can be placed at a maxAlignment address. Block can be placed at
            // maxAlignment/assumedAlignment different address to achieve alignment, so we need
            ...
            //   0 * assumedAlignment =  0
            //   1 * assumedAlignment =  4
            //   2 * assumedAlignment =  8
            //   3 * assumedAlignment = 12
            // Following this logic, the equation for the additional bytes is
            //   (maxAlignment/assumedAlignment - 1) * assumedAlignment
            //     = maxAlignment - assumedAlignment.
            minimumSize += maxAlignment - assumedAlignment;
        }
...
private:
    ...
    // structure my have pointers into it.
