...
public:
    ...
    /**
     * Uses GrColorInfo's color type to pick the default texture format. Will return a
    ...
    /**
     * A convenience version of fillRectFP that applies a coordinate transformation via
     * GrMatrixEffect.
     */
...
protected:
    /** Accounts for hairline geometry and coverage AA in an op's bounds. */
    static void AdjustOpBounds(SkRect* bounds, const GrOp* op);
    /**
...
private:
    // clang correctly warns if a template specialization includes a storage class. MSVC needs it.
#ifdef    _MSC_VER
