 ...
 * the bitmap's color type. The passed ID is used in the cache key and is from the shared namespace
 * of SkBitmap generation IDs and SkImage unique IDs. It must be valid.
 */
...
/**
 * Like GrMakeCachedBitmapProxyView but uses the bitmap's generation ID as the cache ID.
 */
static inline std::tuple<GrSurfaceProxyView, GrColorType>
GrMakeCachedBitmapProxyView(GrRecordingContext* rContext,
                            const SkBitmap& bmp,
                            GrMipmapped mipmapped = GrMipmapped::kNo) {
    return GrMakeCachedBitmapProxyViewWithID(rContext, bmp, mipmapped, bmp.getGenerationID());
}
