 ...
 * also extracted from the bitmap. If GrMipmapped is kYes it a non-mipmapped result may be returned
 * depending on device capabilities (or for a 1x1 bitmap). If GrMipmapped is kNo it indicates
 * mip maps aren't required but a previously created mipmapped texture may still be returned. A
 * color type is returned in case it is different from the bitmap's color type because of GPU
