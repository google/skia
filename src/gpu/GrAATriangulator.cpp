...
#include "src/gpu/GrTriangulator_structs.h"
#include <queue>
...
GrAATriangulator::GrAATriangulator(const SkPath& path)
        : GrTriangulator(path), fOuterMesh(fAlloc.make<VertexList>()) {
}
    ...
    for (Vertex* outer = mesh->fHead; outer; outer = outer->fNext) {
        if (Vertex* inner = outer->fPartner) {
            if ((inner->fPrev || inner->fNext) && (outer->fPrev || outer->fNext)) {
                ...
                // number.
                this->connectEdge(outer, inner, EdgeType::kConnector, c, 0);
...
void GrAATriangulator::removeNonBoundaryEdges(const VertexList& mesh) {
    ...
    for (Vertex* v = mesh.fHead; v != nullptr; v = v->fNext) {
        ...
        bool prevFilled = leftEnclosingEdge &&
                          ApplyFillType(fPath.getFillType(), leftEnclosingEdge->fWinding);
        for (Edge* e = v->fFirstEdgeAbove; e;) {
            Edge* next = e->fNextEdgeAbove;
            activeEdges.remove(e);
            bool filled = ApplyFillType(fPath.getFillType(), e->fWinding);
...
void GrAATriangulator::connectSSEdge(Vertex* v, Vertex* dest, const Comparator& c) {
    ...
    TESS_LOG("connectSSEdge vertex %g to vertex %g\n", v->fID, dest->fID);
    if (v->fSynthetic) {
        this->connectEdge(v, dest, EdgeType::kConnector, c, 0);
    } else if (v->fPartner) {
...
void Event::apply(VertexList* mesh, const Comparator& c, EventList* events,
    ...
    Vertex* dest = triangulator->createSortedVertex(fPoint, fAlpha, mesh, prev, c);
...
//                SkASSERT(!ssPrev->fNext && !ssNext->fPrev);
    ...
    for (SSEdge* edge : ssEdges) {
        if (Edge* e = edge->fEdge) {
            this->connectEdge(edge->fPrev->fVertex, edge->fNext->fVertex, e->fType, c, 0);
        }
    }
    return complex;
}
...
void GrAATriangulator::strokeBoundary(EdgeList* boundary, VertexList* innerMesh,
    ...
    for (Vertex* v = innerVertices.fHead; v && v->fNext; v = v->fNext) {
        this->connectEdge(v, v->fNext, EdgeType::kInner, c, innerWinding);
    }
    this->connectEdge(innerVertices.fTail, innerVertices.fHead, EdgeType::kInner, c, innerWinding);
    for (Vertex* v = outerVertices.fHead; v && v->fNext; v = v->fNext) {
        this->connectEdge(v, v->fNext, EdgeType::kOuter, c, outerWinding);
    }
    this->connectEdge(outerVertices.fTail, outerVertices.fHead, EdgeType::kOuter, c, outerWinding);
...
void GrAATriangulator::extractBoundary(EdgeList* boundary, Edge* e) {
    TESS_LOG("\nextracting boundary\n");
    bool down = GrTriangulator::ApplyFillType(fPath.getFillType(), e->fWinding);
...
Poly* GrAATriangulator::tessellate(const VertexList& mesh, const Comparator& c) {
    ...
    GrTriangulator::SortMesh(&innerMesh, c);
    GrTriangulator::SortMesh(fOuterMesh, c);
    this->mergeCoincidentVertices(&innerMesh, c);
    bool wasComplex = this->mergeCoincidentVertices(fOuterMesh, c);
    auto result = this->simplify(&innerMesh, c);
    SkASSERT(SimplifyResult::kAbort != result);
    wasComplex = (SimplifyResult::kFoundSelfIntersection == result) || wasComplex;
    ...
    wasComplex = this->collapseOverlapRegions(&innerMesh, c, eventLT) || wasComplex;
    wasComplex = this->collapseOverlapRegions(fOuterMesh, c, eventGT) || wasComplex;
    if (wasComplex) {
...
int64_t GrAATriangulator::countPoints(Poly* polys, SkPathFillType) {
    int64_t count = this->GrTriangulator::countPoints(polys, SkPathFillType::kWinding);

    // Count the outer mesh points.
...
void* GrAATriangulator::polysToTriangles(Poly* polys, void* data, SkPathFillType) {
    data = this->GrTriangulator::polysToTriangles(polys, data, SkPathFillType::kWinding);

    // Convert the outer mesh to triangles.
    for (Vertex* v = fOuterMesh->fHead; v; v = v->fNext) {
        for (Edge* e = v->fFirstEdgeBelow; e; e = e->fNextEdgeBelow) {
            Vertex* v0 = e->fTop;
            Vertex* v1 = e->fBottom;
            ...
            data = this->emitTriangle(v0, v1, v2, 0, data);
            data = this->emitTriangle(v0, v2, v3, 0, data);
        }
    }
