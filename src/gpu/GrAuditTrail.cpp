...
void GrAuditTrail::addOp(const GrOp* op, GrSurfaceProxy::UniqueID proxyID) {
    SkASSERT(fEnabled);
    Op* auditOp = new Op;
...
void GrAuditTrail::copyOutFromOpList(OpInfo* outOpInfo, int opListID) {
    ...
    outOpInfo->fRenderTargetProxyUniqueID = bn->fRenderTargetProxyUniqueID;
...
SkString GrAuditTrail::OpNode::toJson() const {
    SkString json;
    json.append("{");
    json.appendf("\"RenderTarget\": \"%u\",", fRenderTargetProxyUniqueID.asUInt());
    skrect_to_json(&json, "Bounds", fBounds);
