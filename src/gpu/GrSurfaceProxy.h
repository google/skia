...
public:
    ...
    void isUsedAsTaskTarget() { ++fTargetCount; }

    /** How many render tasks has this proxy been the target of? */
    int getCurrentTargetCount() const { return fTargetCount; }
    ...
    //
    // Optionally gets the render task that performs the copy. This is used to speculatively copy
    // to avoid a future RAW hazard. If the write never happens before all reads are done the copy
    // can be canceled if it hasn't already executed.
