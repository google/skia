...
class GrGLSLSingleIntervalGradientColorizer : public GrGLSLFragmentProcessor {
 public:
  ...
  void emitCode(EmitArgs& args) override {
    ...
    auto end = _outer.end();
    (void)end;
    fStartVar = args.fUniformHandler->addUniform(kFragment_GrShaderFlag,
                                                 kHalf4_GrSLType, "start");
    fEndVar = args.fUniformHandler->addUniform(kFragment_GrShaderFlag,
                                               kHalf4_GrSLType, "end");
...
GrSingleIntervalGradientColorizer::onCreateGLSLInstance() const {
  return new GrGLSLSingleIntervalGradientColorizer();
}
void GrSingleIntervalGradientColorizer::onGetGLSLProcessorKey(
    const GrShaderCaps& caps, GrProcessorKeyBuilder* b) const {}
bool GrSingleIntervalGradientColorizer::onIsEqual(
    const GrFragmentProcessor& other) const {
  const GrSingleIntervalGradientColorizer& that =
      other.cast<GrSingleIntervalGradientColorizer>();
  (void)that;
...
GrSingleIntervalGradientColorizer::GrSingleIntervalGradientColorizer(
    ...
    : INHERITED(kGrSingleIntervalGradientColorizer_ClassID,
      ...
      fStart(src.fStart),
      fEnd(src.fEnd) {}
std::unique_ptr<GrFragmentProcessor> GrSingleIntervalGradientColorizer::clone()
    const {
  return std::unique_ptr<GrFragmentProcessor>(
      new GrSingleIntervalGradientColorizer(*this));
}
