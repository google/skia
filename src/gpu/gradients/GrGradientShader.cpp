...
std::unique_ptr<GrFragmentProcessor> MakeGradient(const SkGradientShaderBase& shader,
                                                  const GrFPArgs& args,
    ...
    for (int i = 0; i < shader.fColorCount; i++) {
        ...
        if (allOpaque && !SkScalarNearlyEqual(c.fRGBA[3], 1.0)) {
            allOpaque = false;
        }
        colors.push_back(c);
    }
    ...
    switch(shader.getTileMode()) {
        case SkShader::kRepeat_TileMode:
            master = GrTiledGradientEffect::Make(std::move(colorizer), std::move(layout),
                                                 /* mirror */ false, makePremul);
            break;
        case SkShader::kMirror_TileMode:
            master = GrTiledGradientEffect::Make(std::move(colorizer), std::move(layout),
                                                 /* mirror */ true, makePremul);
            break;
        case SkShader::kClamp_TileMode: {
            ...
            master = GrClampedGradientEffect::Make(std::move(colorizer), std::move(layout),
                    colors[0], colors[shader.fColorCount - 1], makePremul);
            break;
        }
        case SkShader::kDecal_TileMode:
            master = GrClampedGradientEffect::Make(std::move(colorizer), std::move(layout),
                    GrColor4f::TransparentBlack(), GrColor4f::TransparentBlack(), makePremul);
            break;
    }
...
std::unique_ptr<GrFragmentProcessor> MakeLinear(const SkLinearGradient& shader,
                                                const GrFPArgs& args) {
    return MakeGradient(shader, args, GrLinearGradientLayout::Make(shader, args));
}

std::unique_ptr<GrFragmentProcessor> MakeRadial(const SkRadialGradient& shader,
                                                const GrFPArgs& args) {
    return MakeGradient(shader,args, GrRadialGradientLayout::Make(shader, args));
}

std::unique_ptr<GrFragmentProcessor> MakeSweep(const SkSweepGradient& shader,
                                               const GrFPArgs& args) {
    return MakeGradient(shader,args, GrSweepGradientLayout::Make(shader, args));
}
