...
void main() {
    ...
    // If t.x is below 0, use the bottom border color without invoking the child processor. If any
    // t.x is above 1, use the top border color. Otherwise, t is in the [0, 1] range assumed by the
...
@optimizationFlags {
    ...
    (colorizer->preservesOpaqueInput() && gradLayout->preservesOpaqueInput() &&
            bottomBorderColor.isOpaque() && topBorderColor.isOpaque()
