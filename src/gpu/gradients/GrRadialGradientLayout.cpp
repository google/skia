...
class GrGLSLRadialGradientLayout : public GrGLSLFragmentProcessor {
 public:
  ...
  void emitCode(EmitArgs& args) override {
    ...
    const GrRadialGradientLayout& _outer =
        args.fFp.cast<GrRadialGradientLayout>();
    (void)_outer;
...
GrRadialGradientLayout::GrRadialGradientLayout(
    const GrRadialGradientLayout& src)
...
std::unique_ptr<GrFragmentProcessor> GrRadialGradientLayout::clone() const {
  return std::unique_ptr<GrFragmentProcessor>(
      new GrRadialGradientLayout(*this));
}
GR_DEFINE_FRAGMENT_PROCESSOR_TEST(GrRadialGradientLayout);
#if GR_TEST_UTILS
std::unique_ptr<GrFragmentProcessor> GrRadialGradientLayout::TestCreate(
  ...
  std::unique_ptr<GrFragmentProcessor> fp =
      as_SB(shader)->asFragmentProcessor(asFPArgs.args());
  GrAlwaysAssert(fp);
  return fp;
}
#endif
...
std::unique_ptr<GrFragmentProcessor> GrRadialGradientLayout::Make(
  ...
  SkMatrix matrix;
  if (!grad.totalLocalMatrix(args.fPreLocalMatrix, args.fPostLocalMatrix)
           ->invert(&matrix)) {
    return nullptr;
  }
  matrix.postConcat(grad.getGradientMatrix());
  return std::unique_ptr<GrFragmentProcessor>(
      new GrRadialGradientLayout(matrix));
}
