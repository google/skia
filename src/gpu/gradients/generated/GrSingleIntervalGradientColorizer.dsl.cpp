...
public:
    ...
    void emitCode(EmitArgs& args) override {
        [[maybe_unused]] const GrSingleIntervalGradientColorizer& _outer =
                args.fFp.cast<GrSingleIntervalGradientColorizer>();

        using namespace SkSL::dsl;
        StartFragmentProcessor(this, &args);
        Var _start(kUniform_Modifier, DSLType(kHalf4_Type), "start");
        startVar = VarUniformHandle(_start);
        DeclareGlobal(_start);
        Var _end(kUniform_Modifier, DSLType(kHalf4_Type), "end");
        endVar = VarUniformHandle(_end);
        DeclareGlobal(_end);
        Return(Mix(_start, _end, Half(sk_SampleCoord().x())));
