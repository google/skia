...
public:
    ...
    void emitCode(EmitArgs& args) override {
        ...
        Var scale01(kUniform_Modifier, DSLType(kFloat4_Type), "scale01");
        scale01Var = VarUniformHandle(scale01);
        Var bias01(kUniform_Modifier, DSLType(kFloat4_Type), "bias01");
        bias01Var = VarUniformHandle(bias01);
        Var scale23(kUniform_Modifier, DSLType(kFloat4_Type), "scale23");
        scale23Var = VarUniformHandle(scale23);
        Var bias23(kUniform_Modifier, DSLType(kFloat4_Type), "bias23");
        bias23Var = VarUniformHandle(bias23);
        Var threshold(kUniform_Modifier, DSLType(kHalf_Type), "threshold");
        thresholdVar = VarUniformHandle(threshold);
        Var t(kNo_Modifier, DSLType(kHalf_Type), "t", Half(sk_SampleCoord().x()));
        Var scale(kNo_Modifier, DSLType(kFloat4_Type), "scale");
        Var bias(kNo_Modifier, DSLType(kFloat4_Type), "bias");
        Declare(t);
        Declare(scale), Declare(bias);
        If(t < threshold,
           /*Then:*/ Block(scale = scale01, bias = bias01),
           /*Else:*/ Block(scale = scale23, bias = bias23));
