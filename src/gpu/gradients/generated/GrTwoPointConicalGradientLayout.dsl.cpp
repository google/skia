...
public:
    ...
    void emitCode(EmitArgs& args) override {
        [[maybe_unused]] const GrTwoPointConicalGradientLayout& _outer =
                args.fFp.cast<GrTwoPointConicalGradientLayout>();

        using namespace SkSL::dsl;
        StartFragmentProcessor(this, &args);
        [[maybe_unused]] const auto& type = _outer.type;
        Var _type(kConst_Modifier, DSLType(kInt_Type), "type");
        Declare(_type, Int((int)type));
        [[maybe_unused]] const auto& isRadiusIncreasing = _outer.isRadiusIncreasing;
        Var _isRadiusIncreasing(kConst_Modifier, DSLType(kBool_Type), "isRadiusIncreasing");
        Declare(_isRadiusIncreasing, Bool(!!(isRadiusIncreasing)));
        [[maybe_unused]] const auto& isFocalOnCircle = _outer.isFocalOnCircle;
        Var _isFocalOnCircle(kConst_Modifier, DSLType(kBool_Type), "isFocalOnCircle");
        Declare(_isFocalOnCircle, Bool(!!(isFocalOnCircle)));
        [[maybe_unused]] const auto& isWellBehaved = _outer.isWellBehaved;
        Var _isWellBehaved(kConst_Modifier, DSLType(kBool_Type), "isWellBehaved");
        Declare(_isWellBehaved, Bool(!!(isWellBehaved)));
        [[maybe_unused]] const auto& isSwapped = _outer.isSwapped;
        Var _isSwapped(kConst_Modifier, DSLType(kBool_Type), "isSwapped");
        Declare(_isSwapped, Bool(!!(isSwapped)));
        [[maybe_unused]] const auto& isNativelyFocal = _outer.isNativelyFocal;
        Var _isNativelyFocal(kConst_Modifier, DSLType(kBool_Type), "isNativelyFocal");
        Declare(_isNativelyFocal, Bool(!!(isNativelyFocal)));
        Var _focalParams(kUniform_Modifier, DSLType(kHalf2_Type), "focalParams");
        focalParamsVar = VarUniformHandle(_focalParams);
        DeclareGlobal(_focalParams);
        Var _t(kNo_Modifier, DSLType(kFloat_Type), "t");
        Var _v(kNo_Modifier, DSLType(kHalf_Type), "v");
        Var _r0_2(kNo_Modifier, DSLType(kHalf_Type), "r0_2");
        Var _r0(kNo_Modifier, DSLType(kHalf_Type), "r0");
        Var _invR1(kNo_Modifier, DSLType(kHalf_Type), "invR1");
        Var _fx(kNo_Modifier, DSLType(kHalf_Type), "fx");
        Var _x_t(kNo_Modifier, DSLType(kFloat_Type), "x_t");
        Var _temp(kNo_Modifier, DSLType(kFloat_Type), "temp");
        Declare(_t, -1.0f);
        Declare(_v, 1.0f);
        StaticSwitch(
                _type,
                Case(1,
                     Block(Declare(_r0_2, _focalParams.y()),
                           _t = Float(_r0_2) - sk_SampleCoord().y() * sk_SampleCoord().y(),
                           If(_t >= 0.0f,
                              /*Then:*/ Block(_t = sk_SampleCoord().x() + Sqrt(_t)),
                              /*Else:*/ Block(_v = -1.0f))),
                     Break()),
                Case(0,
                     Block(Declare(_r0, _focalParams.x()),
                           StaticIf(_isRadiusIncreasing,
                                    /*Then:*/ Block(_t = Length(sk_SampleCoord()) - Float(_r0)),
                                    /*Else:*/ Block(_t = -Length(sk_SampleCoord()) - Float(_r0)))),
                     Break()),
                Case(2,
                     Block(Declare(_invR1, _focalParams.x()),
                           Declare(_fx, _focalParams.y()),
                           Declare(_x_t, -1.0f),
                           StaticIf(_isFocalOnCircle,
                                    /*Then:*/
                                    Block(_x_t = Dot(sk_SampleCoord(), sk_SampleCoord()) /
                                                 sk_SampleCoord().x()),
                                    /*Else:*/
                                    If(_isWellBehaved,
                                       /*Then:*/
                                       Block(_x_t = Length(sk_SampleCoord()) -
                                                    sk_SampleCoord().x() * Float(_invR1)),
                                       /*Else:*/
                                       Block(Declare(_temp,
                                             ...
                                             If(_temp >= 0.0f,
                                                /*Then:*/ Block(StaticIf(
                                                        _isSwapped || !_isRadiusIncreasing,
                                                        /*Then:*/
                                                        Block(_x_t = -Sqrt(_temp) -
                                                                     sk_SampleCoord().x() *
                                                                             Float(_invR1)),
                                                        /*Else:*/
                                                        Block(_x_t = Sqrt(_temp) -
                                                                     sk_SampleCoord().x() *
                                                                             Float(_invR1)))))))),
                           StaticIf(!_isWellBehaved,
                                    /*Then:*/ Block(If(_x_t <= 0.0f, /*Then:*/ Block(_v = -1.0f)))),
                           StaticIf(_isRadiusIncreasing,
                                    /*Then:*/
                                    Block(StaticIf(_isNativelyFocal,
                                                   /*Then:*/ Block(_t = _x_t),
                                                   /*Else:*/ Block(_t = _x_t + Float(_fx)))),
                                    /*Else:*/
                                    Block(StaticIf(_isNativelyFocal,
                                                   /*Then:*/ Block(_t = -_x_t),
                                                   /*Else:*/ Block(_t = -_x_t + Float(_fx))))),
                           StaticIf(_isSwapped, /*Then:*/ Block(_t = 1.0f - _t))),
                     Break()));
        Return(Half4(Half(_t), _v, 0.0f, 0.0f));
