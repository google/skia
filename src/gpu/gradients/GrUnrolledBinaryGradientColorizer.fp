...
// The 7 threshold positions that define the boundaries of the 8 intervals (excluding t = 0, but
// includes t = 1) are packed into two half4's instead of having up to 7 separate scalar uniforms.
// For low interval counts, the extra components are ignored in the shader, but the uniform
// simplification is worth it. It is assumed thresholds are provided in increasing value, mapped as:
