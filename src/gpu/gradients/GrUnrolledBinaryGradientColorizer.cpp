...
class GrGLSLUnrolledBinaryGradientColorizer : public GrGLSLFragmentProcessor {
 public:
  ...
  void emitCode(EmitArgs& args) override {
    ...
    fBias0_1Var = args.fUniformHandler->addUniform(kFragment_GrShaderFlag,
    ...
    fragBuilder->codeAppendf(
        ...
        fScale2_3Var.isValid()
            ? args.fUniformHandler->getUniformCStr(fScale2_3Var)
            : "float4(0)",
        fBias2_3Var.isValid()
            ? args.fUniformHandler->getUniformCStr(fBias2_3Var)
            : "float4(0)",
        _outer.intervalCount(),
        args.fUniformHandler->getUniformCStr(fThresholds1_7Var),
        fScale4_5Var.isValid()
            ? args.fUniformHandler->getUniformCStr(fScale4_5Var)
            : "float4(0)",
        fBias4_5Var.isValid()
            ? args.fUniformHandler->getUniformCStr(fBias4_5Var)
            : "float4(0)",
        fScale6_7Var.isValid()
            ? args.fUniformHandler->getUniformCStr(fScale6_7Var)
            : "float4(0)",
        fBias6_7Var.isValid()
            ? args.fUniformHandler->getUniformCStr(fBias6_7Var)
            : "float4(0)",
        _outer.intervalCount(),
    ...
    fragBuilder->codeAppendf(
        ...
        "* scale + bias);\n",
        args.fUniformHandler->getUniformCStr(fThresholds9_13Var),
        fScale8_9Var.isValid()
            ? args.fUniformHandler->getUniformCStr(fScale8_9Var)
            : "float4(0)",
        fBias8_9Var.isValid()
            ? args.fUniformHandler->getUniformCStr(fBias8_9Var)
            : "float4(0)",
        fScale10_11Var.isValid()
            ? args.fUniformHandler->getUniformCStr(fScale10_11Var)
            : "float4(0)",
        fBias10_11Var.isValid()
            ? args.fUniformHandler->getUniformCStr(fBias10_11Var)
            : "float4(0)",
        _outer.intervalCount(),
        args.fUniformHandler->getUniformCStr(fThresholds9_13Var),
        fScale12_13Var.isValid()
            ? args.fUniformHandler->getUniformCStr(fScale12_13Var)
            : "float4(0)",
        fBias12_13Var.isValid()
            ? args.fUniformHandler->getUniformCStr(fBias12_13Var)
            : "float4(0)",
        fScale14_15Var.isValid()
            ? args.fUniformHandler->getUniformCStr(fScale14_15Var)
            : "float4(0)",
        fBias14_15Var.isValid()
            ? args.fUniformHandler->getUniformCStr(fBias14_15Var)
            : "float4(0)",
        args.fOutputColor);
  }
...
GrUnrolledBinaryGradientColorizer::onCreateGLSLInstance() const {
...
bool GrUnrolledBinaryGradientColorizer::onIsEqual(
    const GrFragmentProcessor& other) const {
  const GrUnrolledBinaryGradientColorizer& that =
      other.cast<GrUnrolledBinaryGradientColorizer>();
  (void)that;
...
std::unique_ptr<GrFragmentProcessor> GrUnrolledBinaryGradientColorizer::clone()
    const {
  return std::unique_ptr<GrFragmentProcessor>(
      new GrUnrolledBinaryGradientColorizer(*this));
}
...
std::unique_ptr<GrFragmentProcessor> GrUnrolledBinaryGradientColorizer::Make(
  ...
  // Depending on how the positions resolve into hard stops or regular stops,
  // the number of intervals specified by the number of colors/positions can
  // change. For instance, a plain 3 color gradient is two intervals, but a 4
  // color gradient with a hard stop is also two intervals. At the most extreme
  // end, an 8 interval gradient made entirely of hard stops has 16 colors.
  ...
  // The raster implementation also uses scales and biases, but since they must
  // be calculated after the dst color space is applied, it limits our ability
  // to cache their values.
  ...
  for (int i = 0; i < count - 1; i++) {
    if (intervalCount >= kMaxIntervals) {
      // Already reached kMaxIntervals, and haven't run out of color stops so
      // this gradient cannot be represented by this shader.
      return nullptr;
    }
    ...
    SkScalar dt = t1 - t0;
    // If the interval is empty, skip to the next interval. This will
    // automatically create distinct hard stop intervals as needed. It also
    // protects against malformed gradients that have repeated hard stops at the
    // very beginning that are effectively unreachable.
  ...
  return std::unique_ptr<GrFragmentProcessor>(
      new GrUnrolledBinaryGradientColorizer(
          intervalCount, scales[0], scales[1], scales[2], scales[3], scales[4],
          scales[5], scales[6], scales[7], biases[0], biases[1], biases[2],
          biases[3], biases[4], biases[5], biases[6], biases[7],
          SkRect::MakeLTRB(thresholds[0], thresholds[1], thresholds[2],
                           thresholds[3]),
