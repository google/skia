...
public:
     ...
     */
    void calcProcessors(const GrFragmentProcessor* const* processors, int cnt);
    ...
    bool isLCDCoverage() const { return fInOut.isLCDCoverage(); }
...
private:
    void internalReset(GrColor color, GrColorComponentFlags colorFlags, bool isLCDCoverage) {
        fInOut.reset(color, colorFlags, isLCDCoverage);
