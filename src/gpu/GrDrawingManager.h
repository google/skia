...
private:
    ...
    public:
        ...
        // These calls forceably remove an opList from the DAG. They are problematic bc they just remove
        // the opList but don't cleanup any refering pointers (i.e., dependency pointers in the
        // DAG). They work right now bc they are only called at flush time, after the topological sort
        // is complete (so the dangling pointers aren't used).
