...
const GrGLenum* GrBackendFormat::getGLFormat() const {
    if (this->isValid() && GrBackend::kOpenGL == fBackend) {
        return &fGL.fFormat;
    }
    return nullptr;
}
...
const VkFormat* GrBackendFormat::getVkFormat() const {
    if (this->isValid() && GrBackend::kVulkan == fBackend) {
        return &fVkFormat;
    }
    return nullptr;
}
...
GrBackendFormat::GrBackendFormat(GrPixelConfig config)
        : fBackend(GrBackend::kMock)
        , fValid(true)
        , fMockFormat(config) {
}

const GrPixelConfig* GrBackendFormat::getMockFormat() const {
    if (this->isValid() && GrBackend::kMock == fBackend) {
        return &fMockFormat;
    }
    return nullptr;
}
