...
GrBackendFormat::GrBackendFormat(GrColorType colorType, GrSRGBEncoded srgbEncoded, bool foo)
...
GrBackendTexture::GrBackendTexture(int width,
        ...
        , fWidth(width)
        , fHeight(height)
        , fConfig(GrColorTypeToPixelConfig(mockInfo.fColorType, GrSRGBEncoded::kNo))
        , fMipMapped(mipMapped)
        , fBackend(GrBackendApi::kMock)
        , fMockInfo(mockInfo) {}
...
#endif
        case GrBackendApi::kMock:
            return GrBackendFormat::MakeMock(fMockInfo.fColorType, fMockInfo.fSRGBEncoded);
        default:
            return GrBackendFormat();
    }
}
