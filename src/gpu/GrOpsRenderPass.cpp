...
bool GrOpsRenderPass::draw(const GrProgramInfo& programInfo,
    ...
    if (programInfo.pipeline().usesConservativeRaster()) {
        ...
        // Conservative raster, by default, only supports triangles. Implementations can optionally
        // indicate that they also supports points and lines, but we don't currently query or track
        // that info.
