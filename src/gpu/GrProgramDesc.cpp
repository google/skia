...
static void add_fp_sampler_keys(GrProcessorKeyBuilder* b, const GrFragmentProcessor& fp,
    ...
    for (int i = 0; i < numTextureSamplers; ++i) {
        ...
        uint32_t samplerKey = sampler_key(sampler.proxy()->textureType(), sampler.swizzle(), caps);

        if (needs_extra_sampler_key_for_program(backendFormat)) {
            ...
            add_extra_sampler_key_for_program(b, sampler.samplerState(), backendFormat);
        } else {
            b->add32(samplerKey);
        }
    }
