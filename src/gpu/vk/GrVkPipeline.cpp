...
static void setup_multisample_state(const GrRenderTarget* renderTarget,
    ...
    int numSamples = renderTarget->numColorSamples();
...
GrVkPipeline* GrVkPipeline::Create(GrVkGpu* gpu, const GrRenderTarget* renderTarget,
                                   const GrPrimitiveProcessor& primProc,
    ...
    VkPipelineMultisampleStateCreateInfo multisampleInfo;
    setup_multisample_state(renderTarget, primProc, pipeline, gpu->caps(), &multisampleInfo);
