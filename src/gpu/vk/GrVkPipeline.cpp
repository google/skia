...
GrVkPipeline* GrVkPipeline::Create(
        GrVkGpu* gpu,
        const GrFoo& foo,
        //int numColorSamples, const GrPrimitiveProcessor& primProc,
        //const GrPipeline& pipeline,
    ...
    int totalAttributeCnt = foo.primProc().numVertexAttributes() +
                            foo.primProc().numInstanceAttributes();
    ...
    setup_vertex_input_state(foo.primProc(), &vertexInputInfo, &bindingDescs, pAttribs);

    VkPipelineInputAssemblyStateCreateInfo inputAssemblyInfo;
    setup_input_assembly_state(primitiveType, &inputAssemblyInfo);

    VkPipelineDepthStencilStateCreateInfo depthStencilInfo;
    setup_depth_stencil_state(stencil, foo.origin(), &depthStencilInfo);

    VkPipelineViewportStateCreateInfo viewportInfo;
    setup_viewport_scissor_state(&viewportInfo);

    VkPipelineMultisampleStateCreateInfo multisampleInfo;
    setup_multisample_state(foo.numSamples(), foo.primProc(), foo.pipeline(),
                            gpu->caps(), &multisampleInfo);

    // We will only have one color attachment per pipeline.
    VkPipelineColorBlendAttachmentState attachmentStates[1];
    VkPipelineColorBlendStateCreateInfo colorBlendInfo;
    setup_color_blend_state(foo.pipeline(), &colorBlendInfo, attachmentStates);

    VkPipelineRasterizationStateCreateInfo rasterInfo;
    setup_raster_state(foo.pipeline(), gpu->caps(), &rasterInfo);
