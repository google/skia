...
void submit_to_queue(const GrVkInterface* interface,
                     VkQueue queue,
                     VkFence fence,
                     uint32_t waitCount,
...
void GrVkPrimaryCommandBuffer::submitToQueue(
    ...
    if (0 == signalCount && 0 == waitCount) {
        ...
        // the semaphores after we've submitted. Thus in the worse case another submit grabs the
        // mutex and then realizes it doesn't need to submit the semaphore. We will never end up
        // where an semaphore doesn't think it needs to be submitted (cause of querying
