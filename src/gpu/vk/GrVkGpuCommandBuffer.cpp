...
GrVkGpuCommandBuffer::GrVkGpuCommandBuffer(GrVkGpu* gpu, const LoadAndStoreInfo& colorInfo,
...
void GrVkGpuCommandBuffer::init(GrVkRenderTarget* target) {
    ...
    if (rpHandle.isValid()) {
        ...
        cbInfo.fRenderPass =
                fGpu->resourceProvider().findRenderPass(*target, vkColorOps, vkStencilOps);
    }
...
#endif
        if (cbInfo.fBounds.intersect(0, 0, SkIntToScalar(fRenderTarget->width()),
...
void GrVkGpuCommandBuffer::discard(GrRenderTarget* rt) {
    ...
    if (cbInfo.fIsEmpty) {
        ...
        if (rpHandle.isValid()) {
            ...
            cbInfo.fRenderPass = fGpu->resourceProvider().findRenderPass(*fRenderTarget, vkColorOps,
                                                                         vkStencilOps);
        }
...
void GrVkGpuCommandBuffer::onClearStencilClip(GrRenderTarget* rt, const GrFixedClip& clip,
    ...
    if (!clip.scissorEnabled()) {
        ...
        vkRect.setLTRB(scissor.fLeft, fRenderTarget->height() - scissor.fBottom, scissor.fRight,
                       fRenderTarget->height() - scissor.fTop);
    }
...
void GrVkGpuCommandBuffer::addAdditionalCommandBuffer() {
    ...
    if (rpHandle.isValid()) {
        ...
        cbInfo.fRenderPass =
                fGpu->resourceProvider().findRenderPass(*fRenderTarget, vkColorOps, vkStencilOps);
    }
...
sk_sp<GrVkPipelineState> GrVkGpuCommandBuffer::prepareDrawState(
    ...
    sk_sp<GrVkPipelineState> pipelineState =
            fGpu->resourceProvider().findOrCreateCompatiblePipelineState(
                    pipeline, primProc, primitiveType, *cbInfo.fRenderPass);
    if (!pipelineState) {
...
static void prepare_sampled_images(const GrProcessor& processor, GrVkGpu* gpu) {
    for (int i = 0; i < processor.numTextureSamplers(); ++i) {
        ...
        vkTexture->setImageLayout(gpu, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL,
                                  VK_ACCESS_SHADER_READ_BIT, VK_PIPELINE_STAGE_ALL_GRAPHICS_BIT,
                                  false);
    }
}

void GrVkGpuCommandBuffer::onDraw(const GrPipeline& pipeline, const GrPrimitiveProcessor& primProc,
    ...
    sk_sp<GrVkPipelineState> pipelineState =
            this->prepareDrawState(pipeline, primProc, primitiveType);
    if (!pipelineState) {
        return;
    }
    ...
    for (int i = 0; i < meshCount; ++i) {
        ...
        do {
            if (nonIdxMesh->primitiveType() != primitiveType) {
                ...
                pipelineState = this->prepareDrawState(pipeline, primProc, primitiveType);
                if (!pipelineState) {
                    return;
                }
            }
            ...
            if (nonIdxMesh->isIndexed()) {
                cbInfo.fCommandBuffer->drawIndexed(fGpu, nonIdxMesh->indexCount(), 1,
                                                   nonIdxMesh->startIndex(),
                                                   nonIdxMesh->startVertex(), 0);
            } else {
                cbInfo.fCommandBuffer->draw(fGpu, nonIdxMesh->vertexCount(), 1,
                                            nonIdxMesh->startVertex(), 0);
            }
            cbInfo.fIsEmpty = false;
