...
void GrVkGpuRTCommandBuffer::inlineUpload(GrOpFlushState* state,
    ...
    public:
        ...
        void execute(GrVkGpuRTCommandBuffer*) override {
            fFlushState->doUpload(fUpload);
        }
...
void GrVkGpuRTCommandBuffer::copy(GrSurface* src, GrSurfaceOrigin srcOrigin, const SkIRect& srcRect,
    ...
    public:
        ...
        void execute(GrVkGpuRTCommandBuffer* cb) override {
            cb->fGpu->copySurface(cb->fRenderTarget, cb->fOrigin, fSrc.get(), fSrcOrigin,
                                  fSrcRect, fDstPoint, fShouldDiscardDst);
        }
    ...
    fPreCommandBufferTasks.emplace<Copy>(src, srcOrigin, srcRect,
            dstPoint, LoadStoreState::kStartsWithDiscard == cbInfo.fLoadStoreState);
    ++fCommandBufferInfos[fCurrentCmdInfo].fNumPreCmds;
