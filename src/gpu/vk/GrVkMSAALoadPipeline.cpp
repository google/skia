...
GrVkMSAALoadPipeline* GrVkMSAALoadPipeline::Create(GrVkGpu* gpu,
    ...
    if (err) {
        SkDebugf("Failed to create msaa load pipeline. Error: %d\n", err);
        return nullptr;
    }

    return new GrVkMSAALoadPipeline(gpu, vkPipeline, &renderPass);
}

bool GrVkMSAALoadPipeline::isCompatible(const GrVkRenderPass& rp) const {
