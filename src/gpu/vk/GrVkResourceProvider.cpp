...
void GrVkResourceProvider::destroyResources(bool isDeviceLost) {
    // release our active command buffers
    for (int i = 0; i < fActiveCommandBuffers.count(); ++i) {
        SkASSERT(isDeviceLost || fActiveCommandBuffers[i]->finished(fGpu));
    ...
    for (int i = 0; i < fAvailableCommandBuffers.count(); ++i) {
        SkASSERT(isDeviceLost || fAvailableCommandBuffers[i]->finished(fGpu));
