...
public:
    ...
    /** Inserts a list of GPU semaphores that the current GPU-backed API must wait on before
        executing any more commands on the GPU for this secondary CB. The wait semaphore will get
        added to the VkCommandBuffer owned by this GrContext when flush() is called, and not the
        command buffer which the Secondary CB is from. This will guarantee that we wait on the
        semaphore before the secondary command buffer gets executed. Skia will take ownership of the
        underlying semaphores and delete them once they have been signaled and waited on. If this
