...
bool GrVkRenderPass::isCompatible(const GrVkRenderTarget& target, bool hasInputSelfDependecy) const {
    SkASSERT(!(fAttachmentFlags & kExternal_AttachmentFlag));
    ...
    return this->isCompatible(desc, flags, hasInputSelfDependecy);
}
