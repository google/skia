...
bool GrVkRenderPass::isCompatible(const GrVkRenderPass& renderPass) const {
    ...
    if (fAttachmentFlags & kExternal_AttachmentFlag) {
        ...
        SkAssertResult(renderPass.colorAttachmentIndex(&colorAttachmentIndex));
        return fRenderPass == renderPass.vkRenderPass() &&
...
bool GrVkRenderPass::isCompatibleExternalRP(VkRenderPass renderPass,
                                            uint32_t colorAttachmentIndex) const {
    SkASSERT(fAttachmentFlags & kExternal_AttachmentFlag);
    return fRenderPass == renderPass && fColorAttachmentIndex == colorAttachmentIndex;
}
