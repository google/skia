...
bool GrVkMipmapManager::createMipmapProgram(GrVkGpu* gpu) {
...
bool GrVkMipmapManager::generateMipmap(GrVkGpu* gpu, GrVkTexture* texture, bool gammaCorrect) {
    // TODO: Make this an option?
/*
    if (!gpu->vkCaps().doManualMipmapping()) {
        return false;
    }
*/
    ...
    if (VK_NULL_HANDLE == fVertShaderModule) {
        ...
        if (!this->createMipmapProgram(gpu)) {
    ...
    GrVkMipmapPipeline* pipeline = resourceProv.findOrCreateMipmapPipeline(rt,
                                                                           fShaderStageInfo,
                                                                           fPipelineLayout);
    if (!pipeline) {
        return false;
...
void GrVkMipmapManager::destroyResources(GrVkGpu* gpu) {
    if (VK_NULL_HANDLE != fVertShaderModule) {
...
void GrVkMipmapManager::abandonResources() {
    fVertShaderModule = VK_NULL_HANDLE;
    fFragShaderModule = VK_NULL_HANDLE;
    fPipelineLayout = VK_NULL_HANDLE;
