...
const GrVkRenderPass* GrVkRenderTarget::getSimpleRenderPass(GrVkGpu* gpu) {
    if (fCachedSimpleRenderPass) {
        return fCachedSimpleRenderPass;
    }
    return this->createSimpleRenderPass(gpu);
}

const GrVkRenderPass* GrVkRenderTarget::createSimpleRenderPass(GrVkGpu* gpu) {
    SkASSERT(!this->wrapsSecondaryCommandBuffer());
...
const GrVkFramebuffer* GrVkRenderTarget::getFramebuffer(GrVkGpu* gpu) {
    if (fCachedFramebuffer) {
        return fCachedFramebuffer;
    }
    return this->createFramebuffer(gpu);
}

const GrVkFramebuffer* GrVkRenderTarget::createFramebuffer(GrVkGpu* gpu) {
    SkASSERT(!this->wrapsSecondaryCommandBuffer());
...
void GrVkRenderTarget::addResources(GrVkCommandBuffer& commandBuffer) {
    commandBuffer.addResource(this->getFramebuffer(this->getVkGpu()));
