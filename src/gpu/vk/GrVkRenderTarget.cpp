...
VkCommandBuffer GrVkRenderTarget::getExternalSecondaryCommandBuffer() const {
    SkASSERT(this->wrapsSecondaryCommandBuffer());
    return fExternalFramebuffer->externalSecondaryCommandBuffer();
}
...
void GrVkRenderTarget::addWrappedGrSecondaryCommandBuffer(
        std::unique_ptr<GrVkSecondaryCommandBuffer> cmdBuffer) {
    fExternalFramebuffer->addWrappedGrSecondaryCommandBuffer(std::move(cmdBuffer));
}
