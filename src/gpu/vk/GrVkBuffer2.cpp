...
sk_sp<GrVkBuffer2> GrVkBuffer2::MakeUniform(GrVkGpu* gpu, size_t size) {
    // We want to have a cache of many reusable "small" uniform buffers. Thus we bumb the size of
...
sk_sp<GrVkBuffer2> GrVkBuffer2::Make(GrVkGpu* gpu,
    ...
    if (accessPattern != kDynamic_GrAccessPattern) {
        bufInfo.usage |= VK_BUFFER_USAGE_TRANSFER_DST_BIT;
    }
...
void GrVkBuffer2::vkMap(size_t size) {
    ...
    if (this->isVkMappable()) {
        // Not every buffer will use command buffer usages and instead the command buffer just holds
        // normal refs. Systems higher up in Ganesh should be making sure not to reuse a buffer that
        // currently has a ref held by something else. However, we do need to make sure there isn't
        // a buffer with just a command buffer usage that is trying to be mapped.
