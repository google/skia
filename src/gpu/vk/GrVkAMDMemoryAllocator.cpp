...
sk_sp<GrVkMemoryAllocator> GrVkAMDMemoryAllocator::Make(VkInstance instance,
                                                        ...
                                                        bool supportsDedicatedAllocations) {
    return nullptr;
}
#else
...
#define GR_COPY_FUNCTION_KHR(NAME) functions.vk##NAME##KHR = interface->fFunctions.f##NAME
    ...
    // We don't use the memory budget extension so set this to null. When we updated our internal
    // vulkan headers to include the memory budget extension we can at least set the function
    // pointer here but not pass in the extension flag when creating the the allocator.
    functions.vkGetPhysicalDeviceMemoryProperties2KHR = nullptr;
