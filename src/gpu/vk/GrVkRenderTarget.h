...
public:
    ...
    const GrVkFramebuffer* getFramebuffer(GrVkGpu* gpu);
    ...
    const GrVkRenderPass* getSimpleRenderPass(GrVkGpu* gpu);
    GrVkResourceProvider::CompatibleRPHandle compatibleRenderPassHandle(GrVkGpu* gpu) {
        SkASSERT(!this->wrapsSecondaryCommandBuffer());
        if (!fCompatibleRPHandle.isValid()) {
            SkASSERT(!fCachedSimpleRenderPass);
            this->createSimpleRenderPass(gpu);
        }
...
private:
    ...
    const GrVkRenderPass* createSimpleRenderPass(GrVkGpu* gpu);
    const GrVkFramebuffer* createFramebuffer(GrVkGpu* gpu);
