...
private:
    ...
    // the wrapped case we don't know when the command buffer gets submitted and when it is finished
    // on the GPU since the client is in charge of that. However, we do require that the client
    // keeps the GrVkSecondaryCBDrawContext alive and call releaseResources on it once the GPU is
    // finished all the work. Thus we can use this to manage the lifetime of our
