...
public:
    ...
    // set on the the RT and simply reuse that descriptor set for this render target only, as long
    // as the render target lives. This call will not ref the GrVkDescriptorSet so the caller must
    // manually ref it if it wants to keep it alive.
...
private:
    ...
    void resetCachedArrays();

    const GrVkImageView*       fColorAttachmentView;
    std::unique_ptr<GrVkImage> fMSAAImage;
    const GrVkImageView*       fResolveAttachmentView;

    // We can have renderpass with and without stencil, input attachment dependency, and advanced
    // blend dependency. All three being mutually exclusive. Thus we have a total of 8 types of
    // render passes.
