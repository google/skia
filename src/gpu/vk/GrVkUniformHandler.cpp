...
GrGLSLUniformHandler::SamplerHandle GrVkUniformHandler::addSampler(const GrTexture* texture,
    ...
    // Check of we are dealing with an external texture and store the needed information
    const GrVkTexture* vkTexture = static_cast<const GrVkTexture*>(texture);
    if (vkTexture->ycbcrConversionInfo().fYcbcrConversion != VK_NULL_HANDLE) {
        SkASSERT(type == GrTextureType::kExternal);
        info.fSamplerState = state;
        info.fMaxMipLevel = texture->texturePriv().maxMipMapLevel();
        info.fYcbcrInfo = vkTexture->ycbcrConversionInfo();
    }
