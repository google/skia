...
GrVkImage::GrVkImage(const GrVkGpu* gpu,
    ...
    // release the image from the original queue without having that queue. That limits on use
    // the types of queues we can handle.
...
#endif
    if (forSecondaryCB) { fResource = nullptr; }
    else if (fIsBorrowed) {
...
void GrVkImage::setImageLayout(const GrVkGpu* gpu, VkImageLayout newLayout,
    ...
    if (this->currentQueueFamilyIndex() != VK_QUEUE_FAMILY_IGNORED &&
        ...
        // We only support transfering from external or foreign queues here and not arbitrary
        // ones.
