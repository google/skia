...
sk_sp<GrGpu> GrVkGpu::Make(const GrVkBackendContext& backendContext,
    ...
    if (backendContext.fVkExtensions) {
     ...
     if (backendContext.fProtectedContext && !vkGpu->vkCaps().supportsProtectedMemory()) {
        return nullptr;
     }
...
sk_sp<GrGpuBuffer> GrVkGpu::onCreateBuffer(size_t size, GrGpuBufferType type,
    ...
    // In protected context, we cannot copy/update buffers, so set them as dynamic.
    const bool isDynamic = fProtectedContext ? true : kDynamic_GrAccessPattern == accessPattern;
    switch (type) {
        case GrGpuBufferType::kVertex:
            SkASSERT(kDynamic_GrAccessPattern == accessPattern ||
                     kStatic_GrAccessPattern == accessPattern);
            buff = GrVkVertexBuffer::Make(this, size, isDynamic);
            break;
        case GrGpuBufferType::kIndex:
            SkASSERT(kDynamic_GrAccessPattern == accessPattern ||
                     kStatic_GrAccessPattern == accessPattern);
            buff = GrVkIndexBuffer::Make(this, size, isDynamic);
            break;
...
sk_sp<GrTexture> GrVkGpu::onWrapBackendTexture(const GrBackendTexture& backendTex,
    ...
    if (backendTex.isProtected() && !fProtectedContext) {
        return nullptr;
    }
    ...
    surfDesc.fSampleCnt = 1;
    surfDesc.fIsProtected = backendTex.isProtected();
...
sk_sp<GrRenderTarget> GrVkGpu::onWrapBackendRenderTarget(const GrBackendRenderTarget& backendRT){
    ...
    if (backendRT.isProtected() && !fProtectedContext) {
        return nullptr;
    }
    ...
    desc.fIsProtected = backendRT.isProtected();
...
sk_sp<GrRenderTarget> GrVkGpu::onWrapBackendTextureAsRenderTarget(const GrBackendTexture& tex,
    ...
    if (tex.isProtected() && !fProtectedContext) {
        return nullptr;
    }
