...
bool foo(GrVkGpu* gpu, const GrVkAlloc& alloc,
         const SkTArray<size_t>& individualMipOffsets,
         GrPixelConfig config,
         int baseWidth, int baseHeight, const SkColor4f& colorf, VkFormat vkFormat) {
...
bool GrVkGpu::createTestingOnlyVkImage(GrPixelConfig config, int w, int h, bool texturable,
    ...
    bool result;
    if (!srcData) {
        result = foo(this, bufferAlloc, individualMipOffsets,
