...
static bool check_rt_image_info(const GrVkCaps& caps, const GrVkImageInfo& info, bool resolveOnly) {
    if (!caps.isFormatRenderable(info.fFormat, info.fSampleCnt)) {
        return false;
    }
    // We currently require all render targets that we will draw to to be made with color attachment
    // support
