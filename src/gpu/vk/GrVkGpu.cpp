...
bool GrVkGpu::submitCommandBuffer(SyncQueue sync, GrGpuFinishedProc finishedProc,
    ...
    // We must wait to checkCommandBuffers till after we get a new command buffer. The
...
void GrVkGpu::addImageMemoryBarrier(const GrVkResource* resource,
    ...
    // VkImage back to the original queue. In this state we don't submit anymore work and may not a
    // command buffer so we won't do the transfer queues.
