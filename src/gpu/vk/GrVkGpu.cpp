...
bool copy_src_data(GrVkGpu* gpu, const GrVkAlloc& alloc, VkFormat vkFormat,
    ...
    SkASSERT(srcData1 && numMipLevels);
    ...
    for (int level = 0; level < numMipLevels; ++level) {
        const size_t trimRowBytes = srcData1[level].width() * bytesPerPixel;
        SkASSERT(trimRowBytes * srcData1[level].height() <= alloc.fSize);

        SkRectMemcpy(mapPtr + individualMipOffsets[level], trimRowBytes,
                    srcData1[level].addr(), srcData1[level].rowBytes(),
                    trimRowBytes, srcData1[level].height());
    }
...
bool GrVkGpu::createVkImageForBackendSurface(VkFormat vkFormat, int w, int h, bool texturable,
    ...
    SkASSERT(VK_FORMAT_ETC2_R8G8B8_UNORM_BLOCK != vkFormat);
...
GrBackendTexture GrVkGpu::onCreateBackendTexture(int w, int h,
    ...
    SkASSERT(GrGpu::MipMapsAreCorrect(w, h, mipMapped, srcData1, numMipLevels));
    ...
    if (!this->createVkImageForBackendSurface(vkFormat, w, h, true,
                                              ...
                                              srcData1, numMipLevels, color, &info, isProtected)) {
