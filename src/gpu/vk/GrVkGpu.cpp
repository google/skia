...
bool GrVkGpu::onGetReadPixelsInfo(GrSurface* srcSurface, GrSurfaceOrigin srcOrigin, int width,
    ...
    // the src config.
    auto srgbEncodedTemp = GrPixelConfigIsSRGBEncoded(tempDrawInfo->fTempSurfaceDesc.fConfig);
    if (*drawPreference != kNoDraw_DrawPreference) {
        ...
        SkASSERT(tempDrawInfo->fSwizzle == GrSwizzle::RGBA());
        srgbEncodedTemp = GrPixelConfigIsSRGBEncoded(tempDrawInfo->fTempSurfaceDesc.fConfig);
    }
    ...
    ElevateDrawPreference(drawPreference, kRequireDraw_DrawPreference);
    tempDrawInfo->fTempSurfaceDesc.fConfig = GrColorTypeToPixelConfig(dstColorType, srgbEncodedTemp);
