...
sk_sp<GrGpu> GrVkGpu::Make(const GrVkBackendContext& backendContext,
    ...
    if (backendContext.fVkExtensions) {
     ...
     sk_sp<GrVkGpu> vk_gpu(new GrVkGpu(context, options, backendContext, interface,
                                       instanceVersion, physDevVersion));
     if (backendContext.fProtectedContext && !vk_gpu->vkCaps().supportsProtectedMemory()) {
        return nullptr;
     }
     return std::move(vk_gpu);
}
