...
bool GrVkGpu::onGetWritePixelsInfo(GrSurface* dstSurface, GrSurfaceOrigin dstOrigin,
    ...
    if (*drawPreference != kNoDraw_DrawPreference) {
        // We assume the base class has only inserted a draw for sRGB reasons and has not done
        // any swizzling and that the temp surface has the config of the original src data
        // with no swizzling.
...
bool GrVkGpu::onGetReadPixelsInfo(GrSurface* srcSurface, GrSurfaceOrigin srcOrigin, int width,
    ...
    if (*drawPreference != kNoDraw_DrawPreference) {
        // We assume the base class has only inserted a draw for sRGB reasons and has not done
        // any swizzling and that the temp surface has the config of the dst data with no swizzling.
    ...
    // If the base class already required a draw it should not have created a config difference
    // between the temp texture and the dst config type.
    if (*drawPreference > kNoDraw_DrawPreference) {
        SkASSERT(tempDrawInfo->fTempSurfaceDesc.fConfig == tempDrawInfo->fReadConfig);
    }
