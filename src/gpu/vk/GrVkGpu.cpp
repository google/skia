...
GrOpsRenderPass* GrVkGpu::getOpsRenderPass(
    ...
    if (!fCachedOpsRenderPass->set(rt, stencil, origin, bounds, colorInfo, stencilInfo,
                                   sampledProxies, willReadDst, commandPool)) {
        return nullptr;
    }
...
bool GrVkGpu::submitCommandBuffer(SyncQueue sync) {
    ...
    for (auto* commandBuffer : fMainCmdBuffers) commandBuffer->end(this);
    for (auto* pool : fMainCmdPools) pool->close();
...
void GrVkGpu::createCommandBufferIfNecessary() {
    constexpr bool kAlwaysCreate = false;
