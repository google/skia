...
void* GrVkMemory::MapAlloc(const GrVkGpu* gpu, const GrVkAlloc& alloc) {
    ...
    if (alloc.fFlags & GrVkAlloc::kNoncoherent_Flag) {
        ...
        if (0 != (alloc.fSize & (alignment-1))) {
            SkDebugf("alloc.fMemory: %d, alloc.fSize is: %d, alloc.fOffset is: %d, alignmnet is: %d\n",
                     alloc.fMemory, alloc.fSize, alloc.fOffset, alignment);
        }
