...
void GrVkTexture::onRelease() {
    // We're about to be severed from our GrVkResource. If there are idle "finished" procs we have
    // to decide who will handle them. If the resource is still tied to a command buffer we let it
...
void GrVkTexture::onAbandon() {
    // We're about to be severed from our GrVkResource. If there are idle procs we have to decide
    // who will handle them. If the resource is still tied to a command buffer we let it handle
    // them. Otherwise, we handle them.
    if (this->hasResource() && this->resource()->isOwnedByCommandBuffer()) {
        this->removeFinishIdleProcs();
    }
