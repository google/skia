...
struct GrVkTexture::CacheEntry {
    CacheEntry(const GrVkDescriptorSet* fDescSet, GrVkGpu* gpu)
...
void GrVkTexture::onAbandon() {
    ...
    fDescSetCache.foreach([](std::unique_ptr<CacheEntry>* entry) {
...
const GrVkDescriptorSet* GrVkTexture::cachedSingleDescSet(const GrSamplerState& state) {
    if (std::unique_ptr<CacheEntry>* e = fDescSetCache.find(state)) {
...
void GrVkTexture::addDescriptorSetToCache(const GrVkDescriptorSet* descSet,
    ...
    fDescSetCache.insert(state,
                         std::unique_ptr<CacheEntry>(new CacheEntry(descSet, this->getVkGpu())));
}
