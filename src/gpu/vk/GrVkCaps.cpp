...
void GrVkCaps::setColorTypeFormat(GrColorType colorType, VkFormat format, bool primaryOption) {
    int idx = static_cast<int>(colorType);
    if (primaryOption || fColorTypeToFormatTable[idx] == VK_FORMAT_UNDEFINED) {
        fColorTypeToFormatTable[idx] = format;
    }
#ifdef SK_DEBUG
    if (primaryOption) {
        SkASSERT(fPrimaryColorTypeToFormatTable[idx] == VK_FORMAT_UNDEFINED);
        fPrimaryColorTypeToFormatTable[idx] = format;
    }
#endif
}
...
#ifdef SK_DEBUG
    std::fill_n(fPrimaryColorTypeToFormatTable, kGrColorTypeCnt, VK_FORMAT_UNDEFINED);
#endif
    ...
    // Format: VK_FORMAT_R8G8B8A8_UNORM
    {
        ...
        if (SkToBool(info.fOptimalFlags & FormatInfo::kTexturable_Flag)) {
            ...
            // Format: VK_FORMAT_R8G8B8A8_UNORM, Surface: kRGBA_8888
            {
                ...
                this->setColorTypeFormat(ct, format, true);
            }
            // Format: VK_FORMAT_R8G8B8A8_UNORM, Surface: kRGB_888x
            {
                ...
                this->setColorTypeFormat(ct, format, false);
            }
        }
    }
