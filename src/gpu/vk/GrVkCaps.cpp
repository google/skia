...
GrProgramDesc GrVkCaps::makeDesc(const GrRenderTarget* rt, const GrProgramInfo& programInfo) const {
    ...
    if (rt) {
        ...
        if (!vkRT->getSimpleRenderPass()->isExternal()) {
            // This is to ensure ReconstructAttachmentsDescriptor keeps matching getSimpleRenderPass'
            // result
        ...
        // TODO: the damn kExternal_AttachmentFlag in GrVkRenderPass::genKey - need to disallow - precompilation
        // Also need to disable precompilation when wrapping a secondary command buffer
    }
