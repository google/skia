...
void GrVkSecondaryCBDrawContext::flush() {
    ...
    if (dContext) {
        dContext->priv().flushSurface(fDevice->readSurfaceView().proxy());
        dContext->submit();
    }
}
...
bool GrVkSecondaryCBDrawContext::isCompatible(
    ...
    GrBackendFormat rtcFormat = readSurfaceView.asRenderTargetProxy()->backendFormat();
    ...
    return characterization.contextInfo() &&
           ...
           characterization.sampleCount() == readSurfaceView.asRenderTargetProxy()->numSamples() &&
