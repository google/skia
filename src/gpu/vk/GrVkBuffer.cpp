...
void GrVkBuffer::copyCpuDataToGpuBuffer(GrVkGpu* gpu, size_t size,  const void* src) {
    SkASSERT(src);
...
void GrVkBuffer::internalUnmap(GrVkGpu* gpu, size_t size) {
    ...
    if (fDesc.fDynamic) {
        ...
        SkASSERT(fMapPtr);
        this->copyCpuDataToGpuBuffer(gpu, size, fMapPtr);
    }
}
...
bool GrVkBuffer::vkUpdateData(GrVkGpu* gpu, const void* src, size_t srcSizeInBytes,
    ...
    if (fDesc.fDynamic) {
        ...
        this->copyCpuDataToGpuBuffer(gpu, srcSizeInBytes, src);
    }
