...
bool GrVkPipelineStateBuilder::createVkShaderModule(VkShaderStageFlagBits stage,
    ...
    std::unique_ptr<SkSL::Program> program = GrCompileVkShaderModule(fGpu, shaderString.c_str(),
                                                                     stage, shaderModule, stageInfo,
                                                                     settings);
    if (!program) {
        return false;
    }
    ...
    return GrInstallVkShaderModule(fGpu, std::move(program), shaderModule, stageInfo);
}
