...
GrVkPipelineState* GrVkPipelineStateBuilder::CreatePipelineState(
    ...
    GrVkPipelineStateBuilder builder(gpu, renderTarget, origin, pipeline, primProc, primProcProxies, desc);
...
GrVkPipelineState* GrVkPipelineStateBuilder::finalize(const GrStencilSettings& stencil,
    ...
    GrVkPipeline* pipeline = resourceProvider.createPipeline(this->renderTarget(),
                                                             fPrimProc,
                                                             fPipeline,
                                                             stencil,
                                                             shaderStageInfo,
...
bool GrVkPipelineStateBuilder::Desc::Build(Desc* desc,
                                           ...
                                           GrPrimitiveType primitiveType,
                                           GrVkGpu* gpu) {
    if (!INHERITED::Build(desc, renderTarget, primProc, primitiveType == GrPrimitiveType::kPoints,
                          pipeline, gpu)) {
        return false;
    }
