...
public:
    ...
    void ref() const {
        // No barrier required.
        SkDEBUGCODE(int newRefCount = )sk_atomic_fetch_add(&fRefCnt, +1, std::memory_order_relaxed);
    ...
    void unref(GrVkGpu* gpu) const {
        ...
        int newRefCount = sk_atomic_fetch_add(&fRefCnt, -1, std::memory_order_acq_rel);
