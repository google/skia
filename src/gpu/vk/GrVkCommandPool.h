...
public:
    ...
    void releaseIdleBuffers(GrVkResourceProvider* provider, GrVkGpu* gpu);
    ...
    mutable bool fOpen = true;

private:
    mutable VkCommandPool fCommandPool;

    // Array of PrimaryCommandBuffers that are currently in flight
    mutable SkSTArray<4, GrVkPrimaryCommandBuffer*, true> fActivePrimaryBuffers;

    // Array of available primary command buffers that are not in flight
    mutable SkSTArray<4, GrVkPrimaryCommandBuffer*, true> fAvailablePrimaryBuffers;

    // Array of available secondary command buffers that are not in flight
    mutable SkSTArray<4, GrVkSecondaryCommandBuffer*, true> fAvailableSecondaryBuffers;
