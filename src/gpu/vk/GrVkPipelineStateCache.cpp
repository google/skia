...
GrVkResourceProvider::PipelineStateCache::PipelineStateCache(GrVkGpu* gpu, bool foo)
...
GrVkResourceProvider::PipelineStateCache::~PipelineStateCache() {
    ...
    if (c_DisplayVkPipelineCache) {
        using CacheResult = GrFooBar::Stats::ProgramCacheResult;
...
GrVkPipelineState* GrVkResourceProvider::PipelineStateCache::findOrCreatePipelineState(
        ...
        GrFooBar::Stats::ProgramCacheResult* stat) {
    if (stat) {
        *stat = GrFooBar::Stats::ProgramCacheResult::kHit;
    }

    std::unique_ptr<Entry>* entry = fMap.find(desc);
    if (!entry) {
        if (stat) {
            *stat = GrFooBar::Stats::ProgramCacheResult::kMiss;
        }
