...
GrVkDescriptorSetManager* GrVkDescriptorSetManager::CreateSamplerManager(GrVkGpu* gpu,
    ...
    return new GrVkDescriptorSetManager(gpu, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER,
                                        visibilities);
}
...
GrVkDescriptorSetManager::DescriptorPoolManager::DescriptorPoolManager(
    ...
    , fPool(nullptr) {
    if (type == VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER) {
        uint32_t numSamplers = (uint32_t)visibilities.count();
        ...
        for (uint32_t i = 0; i < numSamplers; ++i) {
            ...
            dsSamplerBindings[i].stageFlags = visibility_to_vk_stage_flags(visibilities[i]);
