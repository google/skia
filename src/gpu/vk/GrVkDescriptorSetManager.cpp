...
GrVkDescriptorSetManager::GrVkDescriptorSetManager(GrVkGpu* gpu,
    ...
    if (VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER == type) {
        ...
        SkASSERT(uniformHandler);
        for (int i = 0; i < uniformHandler->numBufferSamplers(); ++i) {
            fBindingVisibilities.push_back(uniformHandler->bufferSamplerVisibility(i));
        }
    } else {
...
bool GrVkDescriptorSetManager::isCompatible(VkDescriptorType type,
    ...
    if (type == VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER) {
        ...
        if (fBindingVisibilities.count() != uniHandler->numBufferSamplers()) {
            return false;
        }
        for (int i = 0; i < uniHandler->numSamplers(); ++i) {
            if (uniHandler->bufferSamplerVisibility(i) != fBindingVisibilities[i]) {
                return false;
            }
        }
    }
...
void GrVkDescriptorSetManager::DescriptorPoolManager::SetupSamplerDescLayout(
        GrVkGpu* gpu,
        VkDescriptorType type,
        const GrVkUniformHandler* uniformHandler,
        const SkTArray<uint32_t>* visibilities,
    ...
    for (uint32_t i = 0; i < numSamplers; ++i) {
        ...
        if (uniformHandler) {
            if (type == VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER) {
                ...
                visibility = uniformHandler->bufferSamplerVisibility(i);
            }
        } else {
    ...
    if (type == VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER ||
        ...
        if (uniformHandler) {
            if (type == VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER) {
                ...
                numSamplers = (uint32_t)uniformHandler->numBufferSamplers();
            }
        } else {
            numSamplers = (uint32_t)visibilities->count();
        }
        SetupSamplerDescLayout(gpu, type, uniformHandler, visibilities, numSamplers, &fDescLayout);
