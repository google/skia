...
GrVkSampler* GrVkSampler::Create(GrVkGpu* gpu, const GrSamplerState& samplerState,
    ...
    if (ycbcrInfo.isValid()) {
        ...
        } else if (!SkToBool(flags & VK_FORMAT_FEATURE_SAMPLED_IMAGE_YCBCR_CONVERSION_SEPARATE_RECONSTRUCTION_FILTER_BIT)) {
    ...
    return new GrVkSampler(sampler, ycbcrConversion,
                           GenerateKey(samplerState, maxMipLevel, ycbcrInfo));
}
