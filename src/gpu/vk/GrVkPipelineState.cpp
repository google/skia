...
void GrVkPipelineState::setData(GrVkGpu* gpu,
    ...
    SkSTArray<8, TextureBinding> textureBindings;

    fGeometryProcessor->setData(fDataManager, primProc,
                                GrFragmentProcessor::CoordTransformIter(pipeline));
    for (int i = 0; i < primProc.numTextureSamplers(); ++i) {
        const auto& sampler = primProc.textureSampler(i);
        textureBindings.push_back({sampler.samplerState(),
    ...
    if (GrTextureProxy* dstTextureProxy = pipeline.dstTextureProxy()) {
        textureBindings.push_back({GrSamplerState::ClampNearest(),
    ...
    if (fNumSamplers) {
        ...
        this->writeSamplers(gpu, textureBindings.begin());
    }
...
void GrVkPipelineState::writeSamplers( GrVkGpu* gpu, const TextureBinding bindings[]) {
