...
GrRenderTargetOpList* GrRenderTargetContext::getRTOpList() {
    ...
    if (!fOpList || fOpList->isClosed()) {
        ...
        if (fHasInitializedStencil) {
            ...
            fOpList->setStencilStoreOp(GrStoreOp::kStore);
            newOpList->setStencilLoadOp(GrLoadOp::kLoad);
            if (!this->caps()->preferStencilDiscard()) {
...
void GrRenderTargetContext::addDrawOp(const GrClip& clip, std::unique_ptr<GrDrawOp> op,
    ...
    if (fixedFunctionFlags & GrDrawOp::FixedFunctionFlags::kUsesStencil) {
        if (!fHasInitializedStencil) {
            // Should have initialized stencil while setting up the stencil clip, if there was one.
            SkASSERT(!appliedClip.hasStencilClip());
