...
sk_sp<GrRenderTargetContext> GrRenderTargetContext::rescale(const SkImageInfo& info,
    ...
    while (stepsX || stepsY) {
        ...
        GrBackendFormat backendFormat = srcContext->asSurfaceProxy()->backendFormat();
...
bool GrRenderTargetContext::asyncRescaleAndReadPixels(
...
bool GrRenderTargetContext::asyncReadPixels(const SkImageInfo& info, int x, int y,
    ...
    if (true || !this->caps()->transferBufferSupport() ||
