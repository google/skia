...
bool GrRenderTargetContext::drawFilledRect(const GrClip& clip,
    ...
    if (GrAAType::kCoverage == aaType) {
        std::unique_ptr<GrDrawOp> op = GrRectOpFactory::MakeAAFill(std::move(paint), viewMatrix,
                                                                  rect, ss);
        if (op) {
            this->addDrawOp(clip, std::move(op));
            return true;
        ...
        std::unique_ptr<GrDrawOp> op = GrRectOpFactory::MakeNonAAFill(std::move(paint), viewMatrix, croppedRect, aaType, ss);
        return true;
    }
...
void GrRenderTargetContextPriv::stencilRect(const GrClip& clip,
    ...
    std::unique_ptr<GrDrawOp> op = GrRectOpFactory::MakeNonAAFill(
            std::move(paint), viewMatrix, rect, aaType, ss);
...
void GrRenderTargetContext::fillRectToRect(const GrClip& clip,
    ...
    std::unique_ptr<GrDrawOp> op = GrRectOpFactory::MakeAAFillWithLocalRect(std::move(paint),
                                                                           viewMatrix,
                                                                           croppedRect,
                                                                           croppedLocalRect);
    if (op) {
...
void GrRenderTargetContext::fillRectWithLocalMatrix(const GrClip& clip,
    ...
    GrAAType aaType = this->chooseAAType(aa, GrAllowMixedSamples::kNo);
    if (GrAAType::kCoverage != aaType) {
        std::unique_ptr<GrDrawOp> op = GrRectOpFactory::MakeNonAAFillWithLocalMatrix(std::move(paint), viewMatrix, localMatrix, croppedRect, aaType);
        this->addDrawOp(clip, std::move(op));
        return;
    }

    std::unique_ptr<GrDrawOp> op =
            GrRectOpFactory::MakeAAFillWithLocalMatrix(std::move(paint), viewMatrix, localMatrix,
                                                      croppedRect);
    if (op) {
        this->addDrawOp(clip, std::move(op));
        return;
