...
bool GrRenderTargetContext::asyncReadPixels(SkColorType ct, SkAlphaType at, sk_sp<SkColorSpace> cs,
    ...
    if (this->origin() == kBottomLeft_GrSurfaceOrigin) {
        ...
        auto rtc = direct->priv().makeDeferredRenderTargetContext(
                ...
                fRenderTargetProxy->config(), cs, 1, GrMipMapped::kNo,
                kTopLeft_GrSurfaceOrigin);
        if (!rtc) {
            return false;
        }
        auto xform = GrColorSpaceXform::Make(this->colorSpaceInfo().colorSpace(), kPremul_SkAlphaType, cs.get(), kPremul_SkAlphaType);
        SkDebugf("xform: %p\n", xform.get());
