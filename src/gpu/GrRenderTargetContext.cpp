...
void GrRenderTargetContext::addDrawOp(const GrClip& clip, std::unique_ptr<GrDrawOp> op) {
    ...
    this->getOpList()->addOp(std::move(op), this, appliedClip.doesClip() ? &appliedClip : nullptr);
}

void GrRenderTargetContext::addMeshDrawOp(const GrPipelineBuilder& pipelineBuilder, const GrClip& clip, std::unique_ptr<GrMeshDrawOp> op) {
    ASSERT_SINGLE_OWNER
    RETURN_IF_ABANDONED
    SkDEBUGCODE(this->validate();)
