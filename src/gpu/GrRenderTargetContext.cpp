...
void GrRenderTargetContext::addDrawOp(const GrClip* clip, std::unique_ptr<GrDrawOp> op,
    ...
    if (clip) {
        ...
        if (!clip->apply(fContext, this, usesHWAA, usesUserStencilBits, &appliedClip, &bounds)) {
            ...
            // FIXME (michaelludwig) - Should replace this with an assert that the clipped bounds
            // are contained in the render target dimensions. GrClip::apply() should guarantee that
            // if it returns true.
            SkAssertResult(bounds.intersect(this->asSurfaceProxy()->getBoundsRect()));
        }
    } else {
