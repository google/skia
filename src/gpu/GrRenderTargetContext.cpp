...
void GrRenderTargetContext::internalClear(const GrFixedClip& clip,
    ...
    if (isFull) {
        if (this->getRTOpList()->resetForFullscreenClear() &&
            ...
            this->getRTOpList()->clearColorOnLoad(color);
            return;
        }
    ...
    if (!paint.numTotalFragmentProcessors()) {
        ...
        if (clip.isRRect(r, &rrect, &aa)) {
            if (rrect.isRect()) {
                ...
                this->drawFilledRect(clip, std::move(paint), aa, SkMatrix::I(), rrect.rect());
            } else {
...
bool GrRenderTargetContext::drawFilledRectAsClear(const GrClip& clip, GrPaint&& paint, GrAA aa,
    ...
    // Should only be trying this optimization when a clear will not be turned back into a draw
    ...
    // Only try converting filled rects into clears if the clear itself won't turn them back into
    // a fill rect op and there's no stencil settings (but we can still fall through to simplify
    // the draw itself since we've consolidated the clip and the draw)
    if (!this->caps()->performColorClearsAsDraws()) {
        ...
        if (combinedRect.contains(rtRect)) {
            ...
            return true;
        } else if (GrClip::IsPixelAligned(combinedRect) && combinedRect.width() > 256 && combinedRect.height() > 256) {
        ...
        // Else fall through to drawing a fill rect with no clip and the combined geometry
    }

    // If we got here, we can't use a scissor + clear, but combinedRect represents the correct geometry
    // combination of quad + clip so we can perform a simplified fill rect op. We do this mostly
    // to avoid mismatches in rounding logic on the CPU vs. the GPU, which frequently appears when
    // drawing and clipping something to the same non-AA rect that never-the-less has non-integer
    // coordinates.
...
void GrRenderTargetContext::internalStencilClear(const GrFixedClip& clip, bool insideStencilMask) {
    if (this->caps()->performStencilClearsAsDraws()) {
        ...
        SkRect rtRect = SkRect::MakeWH(this->width(), this->height());
        GrPaint paint;
        clear_to_grpaint({0.f, 0.f, 0.f, 0.f}, &paint);

        GrAppliedClip appliedClip;
        // Since clip is a fixed clip, we can go right to the applied hard clip and avoid using
        // addDrawOp, which would just send us into a loop
        if (clip.apply(this->width(), this->height(), &appliedClip.hardClip(), &rtRect)) {
            GrXferProcessor::DstProxy dstProxy;
            this->getRTOpList()->addOp(GrFillRectOp::Make(fContext, std::move(paint),
                    GrAAType::kNone, SkMatrix::I(), rtRect, ss), *this->caps(), std::move(appliedClip), dstProxy);
        }
    } else {
...
void GrRenderTargetContext::addDrawOp(const GrClip& clip, std::unique_ptr<GrDrawOp> op,
    ...
    if (fixedFunctionFlags & GrDrawOp::FixedFunctionFlags::kUsesStencil ||
        ...
        if (this->caps()->performStencilClearsAsDraws()) {
            ...
            // have to clear the first time any draw needs it
