...
void GrRenderTargetContext::setNeedsStencil() {
    ...
    if (needsStencilClear) {
        if (this->caps()->performStencilClearsAsDraws()) {
            // Must use an op to perform the clear of the stencil buffer before this op.
            this->internalStencilClear(GrFixedClip::Disabled(), /* inside mask */ false);
        } else {
            // Just make sure the stencil buffer is cleared before the draw op, easy to do it as a
            // load at the start.
