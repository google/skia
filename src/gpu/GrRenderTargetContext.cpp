...
void GrRenderTargetContext::internalClear(const SkIRect* scissor,
    ...
    SkIRect logicalRTBounds = SkIRect::MakeSize(this->dimensions());
    GrScissorState scissorState(this->asSurfaceProxy()->backingStoreDimensions(), logicalRTBounds);
    if (scissor && !scissorState.intersect(*scissor)) {
    ...
    if (scissorState.enabled() && !this->caps()->performColorClearsAsDraws()) {
        ...
        } else if (scissorState.rect() == logicalRTBounds) {
...
bool GrRenderTargetContext::setupDstProxyView(const GrClip* clip, const GrOp& op,
    ...
    SkIRect clippedRect = get_conservative_bounds(this, clip);
