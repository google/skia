...
void GrRenderTargetContext::fillRectWithLocalMatrix(const GrClip& clip,
    ...
    this->drawShapeUsingPathRenderer(clip, std::move(paint), aa, viewAndUnLocalMatrix, GrShape(path));
}
...
void GrRenderTargetContext::drawPath(const GrClip& clip,
    ...
    // TODO: Give shape "inverted empty" type.
    if (shape.isEmpty()) {
...
void GrRenderTargetContext::drawShapeUsingPathRenderer(const GrClip& clip,
    ...
    GrAllowMixedSamples allowMixedSamples =
        originalShape.style().isSimpleHairline() ? GrAllowMixedSamples::kNo : GrAllowMixedSamples::kYes;
    ...
    if (!pr) {
        if (canDrawArgs.fShape->style().applies()) {
            tempShape = canDrawArgs.fShape->applyStyle(GrStyle::Apply::kPathEffectAndStrokeRec, styleScale);
            if (tempShape.isEmpty()) {
                return;
            }
            canDrawArgs.fShape = &tempShape;
