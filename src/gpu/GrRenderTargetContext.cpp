...
void GrRenderTargetContext::drawPaint(const GrClip& clip,
    ...
    if (!paint.numTotalFragmentProcessors()) {
        ...
        // inverse view matrix.
        this->fillRectToRect(clip, std::move(paint), GrAA::kNo, r, r);
    } else {
...
enum class GrRenderTargetContext::QuadOptimization {
    ...
    kDiscard,
    // The rect to draw was converted to a clear op and added to the oplist, so no additional op
    // is necessary. This can only be applied if a constColor is provided.
    kClear,
    // The rect and clip is converted to a direct rounded-rect draw and added to the oplist.
    // This can only be applied if a constColor is provided.
    kRRect,
...
GrRenderTargetContext::QuadOptimization GrRenderTargetContext::attemptQuadOptimization(
        const GrClip& clip, const SkPMColor4f* constColor, GrAA* aa, GrQuadAAFlags* edgeFlags,
        GrPerspQuad* deviceQuad, GrPerspQuad* localQuad) {
    ...
    // Check if we can discard the draw entirely (this is conservative, it may also be dropped later
    // as more details of geometry are discovered)
    if (!clipBounds.intersects(drawBounds)) {
        return QuadOptimization::kDiscard;
    }
    ...
    // draw geometry.
    if (axisAlignedClip) {
        // Tighten clip bounds (if clipRRect.isRect() is true, clipBounds holds the exact geometry
        // of intersection of the render target and the clip rect)
        if (!clipBounds.intersect(clipRRect.rect())) {
            return QuadOptimization::kDiscard;
        }

        if (clipRRect.isRect()) {
            // No rounded corners, so the kClear and kExplicitClip optimizations are possible
            if (deviceQuad->crop(clipBounds, clipAA, &edgeFlags, localQuad)) {
                if (constColor && deviceQuad->quadType() == GrQuadType::kRect) {
                    ...
                    if (drawBounds.contains(rtRect)) {
                        ...
                        return QuadOptimization::kClear;
                        ...
                        drawBounds.round(&clipDevBounds);
                        this->clear(&clipDevBounds, *constColor, CanClearFullscreen::kNo);
                        return QuadOptimization::kClear;
                    }
                }
                ...
                // FIXME determine final AA type based on changes to edge flags??
                return QuadOptimization::kExplicitClip;
            } else {
            ...
            if (deviceQuad->crop(clipBounds, clipAA, &edgeFlags, localQuad) &&
                ...
                return QuadOptimization::kRRect;
            } else {
    ...
    // Crop the quad to the possibly conservative bounds of the clip. Even if this were to return
    // true, the crop rect does not exactly match the clip, so can not report explicit-clip.
    // Since these edges aren't visible, don't update the final edge flags.
    GrQuadAAFlags ignoredFlags = GrQuadAAFlags::kNone;
    deviceQuad->crop(clipBounds, clipAA, &ignoredFlags, localQuad);
...
void GrRenderTargetContext::drawFilledQuad(const GrClip& clip,
    ...
    if (!ss && paint.isConstantBlendedColor(&paintColor)) {
        constColor = &paintColor;
    }

    // FIXME is it worth having attempt() take const& persp quads to represent original, and skip
    // the initial assignment? Then if kNone is returned, it means just use original? Otherwise
    // use the quads that were pointed to?
    GrPerspQuad croppedDeviceQuad = deviceQuad;
    GrPerspQuad croppedLocalQuad = localQuad;
    QuadOptimization opt = this->attemptQuadOptimization(clip, constColor, &aa, &edgeFlags,
                                                         &croppedDeviceQuad, &croppedLocalQuad);

    if (opt >= QuadOptimization::kExplicitClip) {
        // These optimization require caller to add an op themselves
        const GrClip& finalClip = opt == QuadOptimization::kExplicitClip ? GrFixedClip::Disabled()
                                                                         : clip;
