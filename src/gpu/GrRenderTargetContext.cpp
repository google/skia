...
void GrRenderTargetContext::clear(const SkPMColor4f& color) {
    ASSERT_SINGLE_OWNER
    RETURN_IF_ABANDONED
    SkDEBUGCODE(this->validate();)
    ...
    AutoCheckFlush acf(this->drawingManager());
    this->internalClear(GrScissorState(), color, false);
}

void GrRenderTargetContext::clear(const SkIRect& scissor, const SkPMColor4f& color) {
    ASSERT_SINGLE_OWNER
    RETURN_IF_ABANDONED
    SkDEBUGCODE(this->validate();)
    GR_CREATE_TRACE_MARKER_CONTEXT("GrRenderTargetContext", "clear", fContext);
    ...
    AutoCheckFlush acf(this->drawingManager());
    this->internalClear(GrScissorState(scissor), color, false);
}

void GrRenderTargetContextPriv::clearAtleast(const SkIRect& scissor,
                                             const SkPMColor4f& color) {
    ASSERT_SINGLE_OWNER_PRIV
    RETURN_IF_ABANDONED_PRIV
    SkDEBUGCODE(fRenderTargetContext->validate();)
    GR_CREATE_TRACE_MARKER_CONTEXT("GrRenderTargetContextPriv", "clearAtleast",
                                   fRenderTargetContext->fContext);

    AutoCheckFlush acf(fRenderTargetContext->drawingManager());
    fRenderTargetContext->internalClear(GrScissorState(scissor), color, true);
}
...
void GrRenderTargetContext::internalClear(const GrScissorState& scissor,
                                          const SkPMColor4f& color,
    ...
    bool isFull = !scissor.enabled() ||
                  scissor.rect().contains(SkIRect::MakeWH(this->width(), this->height())) ||
    ...
    if (isFull) {
        ...
        if (this->caps()->performPartialClearsAsDraws()) {
            ...
            this->addDrawOp(GrFixedClip::Disabled(),
                            GrFillRectOp::MakeNonAARect(fContext, std::move(paint), SkMatrix::I(),
                                                        SkRect::Make(scissor.rect())));
        } else {
            std::unique_ptr<GrOp> op(GrClearOp::Make(fContext, scissor, color,
                                                     this->asSurfaceProxy()));
...
void GrRenderTargetContext::setNeedsStencil(bool useMixedSamplesIfNotMSAA) {
    ...
    if (!hasInitializedStencil) {
        if (this->caps()->performStencilClearsAsDraws()) {
            ...
            this->internalStencilClear(GrScissorState(), /* inside mask */ false);
        } else {
...
void GrRenderTargetContext::internalStencilClear(const GrScissorState& scissor,
    ...
    if (clearWithDraw) {
        ...
        std::unique_ptr<GrOp> op(GrClearStencilClipOp::Make(
                fContext, scissor, insideStencilMask, this->asRenderTargetProxy()));
        if (!op) {
            return;
        }
        this->addOp(std::move(op));
