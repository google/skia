...
GrAAType GrAdjustAATypeForNonAAAsCoverageFastPath(GrAAType type, const GrPaint& paint,
                                                  const GrCaps& caps) {
    if (GrAAType::kNone != type) {
        return type;
    }
    // Try not to choose a coverage implementation if it would trigger dst copies.
    ...
    if (GrXPFactory::AsBlendMode(paint.getXPFactory(), &mode) &&
        SkBlendMode_SupportsCoverageAsAlpha(mode)) {
        return GrAAType::kCoverage;
    }
    return GrAAType::kNone;
}
...
void GrRenderTargetContext::drawRRect(const GrClip& origClip,
    ...
    GrAAType aaType = this->chooseAAType(aa, GrAllowMixedSamples::kNo);
    if (GrAAType::kCoverage ==
        GrAdjustAATypeForNonAAAsCoverageFastPath(aaType, paint, *fContext->caps())) {
