...
GrRenderTargetContext::QuadOptimization GrRenderTargetContext::attemptQuadOptimization(
    ...
    if (axisAlignedClip) {
        ...
        if (clipRRect.isRect()) {
            ...
            if (deviceQuad->crop(clipBounds, clipAA, &newFlags, localQuad)) {
    ...
    deviceQuad->crop(clipBounds, clipAA, &newFlags, localQuad);
...
sk_sp<GrRenderTargetContext> GrRenderTargetContext::rescale(const SkImageInfo& info,
    ...
    while (stepsX || stepsY) {
        ...
        if (rescaleQuality == kHigh_SkFilterQuality) {
            ...
            paint.addColorFragmentProcessor(std::move(fp));
            paint.setPorterDuffXPFactory(SkBlendMode::kSrc);
            nextRTC->fillRectToRect(GrNoClip(), std::move(paint), GrAA::kNo, SkMatrix::I(),
                                    dstRect, dstRect);
        } else {
