...
bool GrRenderTargetContext::waitOnSemaphores(SkSpan<const GrBackendSemaphore> waitSemaphores,
    ...
    SkTArray<std::unique_ptr<GrSemaphore>> grSemaphores(waitSemaphores.count());
    for (const GrBackendSemaphore& backendSem : waitSemaphores) {
        grSemaphores.push_back(resourceProvider->wrapBackendSemaphore(
            backendSem, GrResourceProvider::SemaphoreWrapType::kWillWait, ownership));
    }
