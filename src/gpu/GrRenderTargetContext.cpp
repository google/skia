...
void GrRenderTargetContext::drawRect(const GrClip& clip,
    ...
    if (stroke.getStyle() == SkStrokeRec::kFill_Style) {
        this->fillQuad(clip, std::move(paint), aa, aa == GrAA::kYes ? GrQuadAAFlags::kAll : GrQuadAAFlags::kNone,
...
void GrRenderTargetContext::internalStencilClear(const GrFixedClip& clip, bool insideStencilMask) {
    if (this->caps()->performStencilClearsAsDraws()) {
        ...
        paint.setPorterDuffXPFactory(SkBlendMode::kSrcOver);
        // FIXME use GrDisableColorXPFactory::Get()
...
void GrRenderTargetContextPriv::fillDeviceRect(const GrClip& clip, GrPaint&& paint, GrAA doMSAA,
                                               const SkRect rect, const GrPerspQuad* localCoords,
                                               const GrUserStencilSettings* stencil) {
    ASSERT_SINGLE_OWNER_PRIV
    RETURN_FALSE_IF_ABANDONED_PRIV
    SkDEBUGCODE(fRenderTargetContext->validate();)
    GR_CREATE_TRACE_MARKER_CONTEXT("GrRenderTargetContextPriv", "fillDeviceRect",
                                   fRenderTargetContext->fContext);
    GrPerspQuad deviceQuad(rect);
    // FIXME need to derive aaType differently from a regular draw, maybe have
    // a fillQuadinternal that takes GrAAType isntead of GrAA?
    fRenderTargetContext->fillQuad(clip, std::move(paint), doMSAA, GrQuadAAFlags::kNone, deviceQuad,
                                   localCoords ? *localCoords : deviceQuad, stencil);
}

void GrRenderTargetContext::fillQuad(const GrClip& clip,
                                     GrPaint&& paint,
                                     GrAA aa,
    ...
    RETURN_IF_ABANDONED
    SkDEBUGCODE(this->validate();)
    GR_CREATE_TRACE_MARKER_CONTEXT("GrRenderTargetContext", "fillQuad", fContext);

    // FIXME apply draw-as-clear optimization
    // FIXME if draw-as-clear falls through, at least also apply the cropping
    ...
    AutoCheckFlush acf(this->drawingManager());
    this->addDrawOp(clip, GrFillRectOp::MakeGeneric(fContext, std::move(paint), aaType, edgeFlags,
                                                    deviceCoords, localCoords, stencil));
}
...
void GrRenderTargetContext::fillRectWithLocalMatrix(const GrClip& clip,
    ...
    this->addDrawOp(clip, GrFillRectOp::MakeGeneric(fContext, std::move(paint), aaType,
            ...
            GrPerspQuad::MakeFromRect(rectToDraw, localMatrix)));
}
