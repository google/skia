...
void GrRenderTargetContextPriv::absClear(const SkIRect& rect, const GrColor color) {
    ...
    if (fRenderTargetContext->fContext->caps()->useDrawInsteadOfClear()) {
        ...
        sk_sp<GrOp> batch(GrClearBatch::Make(rect, color,
                                             fRenderTargetContext->accessRenderTarget()));
        if (!batch) {
            return;
        }
        fRenderTargetContext->getOpList()->addBatch(std::move(batch));
    }
}
