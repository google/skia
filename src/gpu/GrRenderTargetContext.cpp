        ...
        if (clipRRect.isRect()) {
            ...
            if (deviceQuad->crop(clipBounds, clipAA, edgeFlags, localQuad)) {
                ...
                if (*aa == GrAA::kNo && clipAA == GrAA::kYes &&
                    *edgeFlags != GrQuadAAFlags::kNone) {
                    // The clip was anti-aliased and now the draw needs to be upgrade to AA to
                ...
                // FIXME do we need to do this AA upgrade for every crop? Or does it only make
                // sense for ClipApplied?
                // FIXME maybe we always pass in dummy flags and only export the new flags if
                // we hit clipApplied??
