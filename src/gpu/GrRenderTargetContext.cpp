...
bool GrRenderTargetContext::drawFilledRect(const GrClip& clip,
    ...
    if (GrCaps::InstancedSupport::kNone != fContext->caps()->instancedSupport()) {
        ...
        op.reset(ir->recordRect(croppedRect, viewMatrix, paint.getColor(),
...
void GrRenderTargetContext::fillRectWithLocalMatrix(const GrClip& clip,
    ...
    if (view_matrix_ok_for_aa_fill_rect(viewMatrix)) {
        sk_sp<GrDrawOp> op(GrAAFillRectBatch::Create(paint.getColor(), viewMatrix,
...
void GrRenderTargetContext::drawVertices(const GrClip& clip,
    ...
    sk_sp<GrDrawOp> op(new GrDrawVerticesBatch(paint.getColor(),
...
void GrRenderTargetContext::drawRRect(const GrClip& origClip,
    ...
    if (GrCaps::InstancedSupport::kNone != fContext->caps()->instancedSupport() &&
        stroke.isFillStyle()) {
        InstancedRendering* ir = this->getOpList()->instancedRendering();
        sk_sp<GrDrawOp> op(ir->recordRRect(rrect, viewMatrix, paint.getColor(),
                                           paint.isAntiAlias(), fInstancedPipelineInfo,
                                           &useHWAA));
        if (op) {
            GrPipelineBuilder pipelineBuilder(paint, useHWAA);
...
bool GrRenderTargetContext::drawFilledDRRect(const GrClip& clip,
    ...
    if (GrCaps::InstancedSupport::kNone != fContext->caps()->instancedSupport()) {
        bool useHWAA;
        InstancedRendering* ir = this->getOpList()->instancedRendering();
        sk_sp<GrDrawOp> op(ir->recordDRRect(origOuter, origInner, viewMatrix,
...
void GrRenderTargetContext::drawOval(const GrClip& clip,
    ...
    if (GrCaps::InstancedSupport::kNone != fContext->caps()->instancedSupport() &&
        stroke.isFillStyle()) {
        InstancedRendering* ir = this->getOpList()->instancedRendering();
        sk_sp<GrDrawOp> op(ir->recordOval(oval, viewMatrix, paint.getColor(),
                                          paint.isAntiAlias(), fInstancedPipelineInfo,
                                          &useHWAA));
        if (op) {
            GrPipelineBuilder pipelineBuilder(paint, useHWAA);
