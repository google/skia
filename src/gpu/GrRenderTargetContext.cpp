...
static bool preapply_clip_and_device(const GrRenderTargetContext* rtc, const GrClip* clip,
...
GrRenderTargetContext::QuadOptimization GrRenderTargetContext::attemptQuadOptimization(
    ...
    bool conservativeCropOnly = false;

    // Check if clip can be represented as a rounded rect.
    SkRRect clipRRect;
    GrAA clipAA;
    GrClip::ClipEffect effect;
    if (!preapply_clip_and_device(this, clip, drawBounds, &effect, &clipRRect, &clipAA)) {
        switch(effect) {
        ...
        conservativeCropOnly = (stencilSettings && clipAA != *aa) ||
                               (!clipRRect.isRect() && (stencilSettings || !constColor));
    }

    if (conservativeCropOnly) {
...
void GrRenderTargetContextPriv::stencilPath(const GrHardClip* clip,
    ...
    appliedClip.finish(fRenderTargetContext->dimensions(), /* uses stencil */ true);
...
bool GrRenderTargetContextPriv::drawAndStencilPath(const GrHardClip* clip,
    ...
    SkIRect clipConservativeBounds = get_conservative_bounds(fRenderTargetContext, clip);
...
void GrRenderTargetContext::drawShapeUsingPathRenderer(const GrClip* clip,
    ...
    SkIRect clipConservativeBounds = get_conservative_bounds(this, clip);
...
void GrRenderTargetContext::addDrawOp(const GrClip* clip, std::unique_ptr<GrDrawOp> op,
    ...
    GrAppliedClip appliedClip(this->asSurfaceProxy()->backingStoreDimensions());
...
bool GrRenderTargetContext::setupDstProxyView(const GrOp& op,
    ...
    SkIRect copyRect = restrictions.fMustCopyWholeSrc
            ? SkIRect::MakeSize(this->asSurfaceProxy()->backingStoreDimensions())
            : op.bounds().roundOut();
