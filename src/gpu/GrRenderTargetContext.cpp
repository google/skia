...
bool GrRenderTargetContext::asyncReadPixels(SkColorType ct, SkAlphaType at, sk_sp<SkColorSpace> cs,
    ...
    // Insert a draw to a temporary surface if we need to do a y-flip or a color space conversion.
    if (this->origin() == kBottomLeft_GrSurfaceOrigin ||
        !SkColorSpace::Equals(cs.get(), this->colorSpaceInfo().colorSpace())) {
        ...
        auto rtc = direct->priv().makeDeferredRenderTargetContext(
                ...
                fRenderTargetProxy->config(), std::move(cs), 1, GrMipMapped::kNo,
    ...
    fOpList->addOp(GrTransferFromOp::Make(fContext, srcRect, dstCT, buffer, 0), *this->caps());
