...
void GrProcOptInfo::analyzeProcessors(const GrFragmentProcessor* const* processors, int cnt) {
    for (int i = 0; i < cnt; ++i) {
        ...
        if (!knowCurrentOutput && !fAllProcessorsCompatibleWithCoverageAsColor && !fIsOpaque) {
        ...
        if (fAllProcessorsCompatibleWithCoverageAsColor && !fp->compatibleWithCoverageAsAlpha()) {
            fAllProcessorsCompatibleWithCoverageAsColor = false;
        }
