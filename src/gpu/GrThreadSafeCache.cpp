...
GrThreadSafeCache::Entry* GrThreadSafeCache::getEntry(const GrUniqueKey& key,
    ...
    if (fFreeEntryList) {
        ...
        entry = fEntryAllocator.make<Entry>(key, verts);
    }
...
std::tuple<const void*, sk_sp<SkData>> GrThreadSafeCache::findVertsWithData(const GrUniqueKey& key) {
    SkAutoSpinlock lock{fSpinLock};

    return this->internalFindVerts(key);
}

std::tuple<const void*, sk_sp<SkData>> GrThreadSafeCache::internalAddVerts(const GrUniqueKey& key,
                                                                           const void* verts) {
    Entry* tmp = fUniquelyKeyedEntryMap.find(key);
    if (!tmp) {
        tmp = this->getEntry(key, verts);
...
std::tuple<const void*, sk_sp<SkData>> GrThreadSafeCache::addVertsWithData(
                                                                const GrUniqueKey& key,
                                                                const void* verts) {
    SkAutoSpinlock lock{fSpinLock};

    return this->internalAddVerts(key, verts);
}
