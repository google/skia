...
GrXPFactory::AnalysisProperties GrXPFactory::GetAnalysisProperties(
        ...
        const GrCaps& caps,
        GrNeedsClamp needsClamp) {
    AnalysisProperties result;
    if (factory) {
        result = factory->analysisProperties(color, coverage, caps, needsClamp);
    } else {
...
sk_sp<const GrXferProcessor> GrXPFactory::MakeXferProcessor(const GrXPFactory* factory,
    ...
    if (factory) {
        return factory->makeXferProcessor(color, coverage, hasMixedSamples, caps, needsClamp);
    } else {
