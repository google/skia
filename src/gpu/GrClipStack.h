...
public:
    ...
    bool isClipStack() const final { return true; }
...
private:
    ...
    public:
        ...
        using Stack = SkSTArray<1, RawElement>;
    ...
    class SaveRecord {
    public:
        // using Stack = GrTBlockList<SaveRecord, 4>;
        using Stack = SkSTArray<4, SaveRecord>;
...
// FIXME Clean up, go back to GrTBlockList, keep the cached inverse matrix, and finish docs
// then send out for review and starting writing unit tests

// Clip element iteration
class GrClipStack::ElementIter {
public:
    bool operator!=(const ElementIter& o) const {
        return o.fPtr != fPtr && o.fRemaining != fRemaining;
    }

    const Element& operator*() const { return fPtr->asElement(); }

    ElementIter& operator++() {
        do {
            fPtr--;
            fRemaining--;
        } while(fRemaining > 0 && fPtr->isInvalid());
        return *this;
    }

    ElementIter(const RawElement* ptr, int remaining) : fPtr(ptr), fRemaining(remaining) {}

    const RawElement* fPtr;
    int fRemaining;
};

GrClipStack::ElementIter GrClipStack::begin() const {
    int count = fElements.count() - this->currentSaveRecord().oldestElementIndex();
    return ElementIter(fElements.empty() ? nullptr : &fElements.back(), count);
}

GrClipStack::ElementIter GrClipStack::end() const {
    return ElementIter(nullptr, 0);
}
// class GrClipStack::ElementIter {
// public:
//     bool operator!=(const ElementIter& o) const {
