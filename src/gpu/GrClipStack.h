...
private:
    ...
    private:
        // These do more than just compare device-space inner bounds to 'outerBounds', but maps
        // into this element's local system for a more accurate contains check.
        bool contains(const SkIRect& outerBounds) const;
        bool contains(const SkRRect& rrect, const SkMatrix& matrix, GrAA aa);
