...
bool GrAAConvexTessellator::createInsetRings(InsetRing& previousRing, SkScalar initialDepth,
    ...
    for (i = 0; i < kMaxNumRings; ++i) {
        InsetRing* nextRing = this->getNextRing(currentRing);
...
void GrAAConvexTessellator::addOutsetPt(const SkPoint& p, int index, CurveState curve) {

    *fOutsetPts.push() = p;
    *fMap.push() = baz(index);
    *fOutsetCurveState.push() = curve;
}
...
bool GrAAConvexTessellator::extractFromPath(const SkMatrix& m, const SkPath& path) {
    ...
    if (is_duplicate_outset_pt(fOutsetPts.top(), fOutsetPts[0])) {
        fOutsetPts.pop();
        fMap.pop();
    }
    ...
    if (this->numPts() >= 3) {
        if (abs_dist_from_line(fPts.top(), fNorms1.top(), fPts[0]) < kClose) {
            ...
            this->popLastPt();
            fNorms1.pop();
            this->rmLinearPt(this->numPts(), this->numPts()-1, 0);
        }
    ...
    if (this->numPts() >= 3 && abs_dist_from_line(fPts[0], fNorms1.top(), fPts[1]) < kClose) {
        ...
        this->rmLinearPt(0, this->numPts(), 1);
    ...
    this->validate2();
    fInitialOutsetRing.setReserve(this->numOutsetPts());
    for (int i = 0; i < this->numOutsetPts(); ++i) {
        fInitialOutsetRing.add(*this, fOutsetPts[i], -1.0f, fOutsetCurveState[i], fMap[i].f1, fMap[i].f2);
    }
...
GrAAConvexTessellator::InsetRing* GrAAConvexTessellator::getNextRing(InsetRing* lastRing) {
#if 0
    InsetRing* ring = *fRings.push() = new Ring;
...
void GrAAConvexTessellator::OutsetRing::createOuterRing(GrAAConvexTessellator & tess, SkScalar outset,
    ...
    for (int cur = 0; cur < numPts; ++cur) {
        ...
        int perp1Idx = nextRing->add(tess, perp1, coverage, curve, cur, -1);
        ...
        if (duplicate_pt(perp2, perp1)) {
            ...
            perp2Idx = nextRing->add(tess, perp2, coverage, curve, cur, -1);
        }
            ...
            if (curve == kCurve_CurveState) {
                ...
                if (dotProd < kRoundCapThreshold) {
                    ...
                    if (!duplicate_pt(miter, perp1)) {
                        int miterIdx;
                        miterIdx = nextRing->add(tess, miter, coverage, kSharp_CurveState, cur, -1);
                ...
                switch (tess.join()) {
                    case SkPaint::Join::kMiter_Join: {
                        ...
                        // For very shallow angles all the corner points could fuse
                        if (!duplicate_pt(miter, tess.point(perp1Idx))) {
                            int miterIdx;
                            miterIdx = nextRing->add(tess, miter, coverage,
                                                     kSharp_CurveState, cur, -1);
                            //nextRing->addIdx(miterIdx, -1);
                            // The two triangles for the corner
                            this->addTrisForCorner(tess, cur, perp1Idx, miterIdx, perp2Idx);
                        }
...
void GrAAConvexTessellator::OutsetRing::computeBisectors1(SkVector::Side side) {
    ...
    for (int cur = 0; cur < fPts7.count(); prev = cur, ++cur) {
        fPts7[cur].fBisector = fPts7[cur].fNorm + fPts7[prev].fNorm;
...
void GrAAConvexTessellator::lineTo(const SkPoint& p, CurveState curve) {
    if (this->numOutsetPts() > 0 && is_duplicate_outset_pt(p, this->lastOutsetPt())) {
    ...
    this->addOutsetPt(p, origMapGuess, curve); //, 0.0f, initialRingCoverage, false, curve);

    if (this->numPts() > 0 && duplicate_pt(p, this->lastPoint())) {
        // The new outer point maps back to the last inner point
        this->fuse(origMapGuess, this->numPts()-1);
        return;
    }
    ...
    if (this->numPts() >= 2 && abs_dist_from_line(fPts.top(), fNorms1.top(), p) < kClose) {
        ...
        if (duplicate_pt(p, this->lastPoint())) {
            ...
            this->rmLinearPt(this->numPts(), this->numPts(), this->numPts()-1);
        }
    }
...
#if 0
    for (int i = 0; i < fOutsetPts.count(); ++i) {
        SkPoint pt = fOutsetPts[i];
        pt.fX += fStrokeWidth * fOutsetNorms[i].fX;
        pt.fY += fStrokeWidth * fOutsetNorms[i].fY;
        draw_point(canvas, pt, SK_ColorMAGENTA);
    }
#endif
}
