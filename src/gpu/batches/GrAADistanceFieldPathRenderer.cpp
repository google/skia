...
private:
    ...
    bool addPathToAtlas(GrMeshDrawOp::Target* target, FlushInfo* flushInfo, GrBatchAtlas* atlas,
        ...
        shapeData->fScaleToTex = scale;
        dx -= SK_DistanceFieldPad + kAntiAliasPad;
        dy -= SK_DistanceFieldPad + kAntiAliasPad;
        shapeData->fTranslateToTex = SkPoint::Make(atlasLocation.fX - dx,
                                                   atlasLocation.fY - dy);
    ...
    void writePathVertices(GrDrawOp::Target* target,
        ...
        SkRect geoBounds = shapeData->fBounds;
        ...
        SkScalar scale = shapeData->fScaleToTex;
        SkScalar aaBounds = kAntiAliasPad;
        if (kAntiAliasPad*scale/maxScale > kAntiAliasPad + SK_DistanceFieldInset) {
            aaBounds = scale/maxScale;
        }
        geoBounds.outset(aaBounds, aaBounds);

        SkScalar dx = geoBounds.fLeft;
        SkScalar dy = geoBounds.fTop;
        SkScalar width = geoBounds.width();
        SkScalar height = geoBounds.height();
        ...
        width *= scale;
        height *= scale;
        dx += shapeData->fTranslateToTex.fX;
        dy += shapeData->fTranslateToTex.fY;

        SkPoint* positions = reinterpret_cast<SkPoint*>(offset);

        // vertex positions
        // TODO make the vertex attributes a struct
        positions->setRectFan(geoBounds.left(), geoBounds.top(),
                              geoBounds.right(), geoBounds.bottom(), vertexStride);
        ...
        textureCoords->setRectFan((dx) / texture->width(),
                                  (dy) / texture->height(),
                                  (dx + width) / texture->width(),
                                  (dy + height) / texture->height(),
                                  vertexStride);
    }
