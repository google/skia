...
public:
    ...
    GrGpuResource::UniqueID renderTargetUniqueID() const override {
        // Copy surface doesn't currently go through a GrGpuCommandBuffer. By returning an invalid
        // RT ID we force the caller to end the previous command buffer and execute this copy before
        // beginning a new one.
