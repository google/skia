...
static const uint16_t gOverstrokeRRectIndices[] = {
        ...
        // edges
        1, 2, 6, 1, 6, 5, 4, 5, 9, 4, 9, 8, 6, 7, 11, 6, 11, 10, 9, 10, 14, 9, 14, 13,

        // overstroke quads
        // we place this at the end so that we can skip these indices when rendering as stroked
        16, 17, 19, 16, 19, 18, 19, 17, 23, 19, 23, 21, 21, 23, 22, 21, 22, 20, 22, 16, 18, 22, 18,
        20,
};
...
static int rrect_type_to_vert_count(RRectType type) {
    static const int kTypeToVertCount[] = {
            kVertsPerOverstrokeRRect, kVertsPerStrokeRRect, kVertsPerOverstrokeRRect,
    };
...
static int rrect_type_to_index_count(RRectType type) {
    static const int kTypeToIndexCount[] = {
            kIndicesPerOverstrokeRRect, kIndicesPerStrokeRRect, kIndicesPerOverstrokeRRect,
    };
...
static const uint16_t* rrect_type_to_indices(RRectType type) {
    static const uint16_t* kTypeToIndices[] = {
            gOverstrokeRRectIndices, gStrokeRRectIndices, gOverstrokeRRectIndices,
    };
...
public:
    ...
    ShadowCircularRRectBatch(GrColor color, const SkMatrix& viewMatrix, const SkRect& devRect,
                             float devRadius, float blurRadius, float devStrokeWidth,
                             bool strokeOnly)
...
static GrDrawOp* create_shadow_circle_batch(GrColor color, const SkMatrix& viewMatrix,
                                            const SkRect& oval, SkScalar blurRadius,
    ...
    return ShadowCircleBatch::Create(color, viewMatrix, center, width / 2.f, blurRadius,
                                     GrStyle(stroke, nullptr));
}

static GrDrawOp* create_shadow_rrect_batch(GrColor color, const SkMatrix& viewMatrix,
                                           const SkRRect& rrect, SkScalar blurRadius,
                                           const SkStrokeRec& stroke) {
    ...
    return new ShadowCircularRRectBatch(color, viewMatrix, bounds, xRadius, blurRadius,
                                        scaledStroke.fX, isStrokeOnly);
}

GrDrawOp* CreateShadowRRectBatch(GrColor color, const SkMatrix& viewMatrix, const SkRRect& rrect,
                                 const SkScalar blurRadius, const SkStrokeRec& stroke,
                                 const GrShaderCaps* shaderCaps) {
    if (rrect.isOval()) {
        return create_shadow_circle_batch(color, viewMatrix, rrect.getBounds(), blurRadius, stroke,
                                          shaderCaps);
    }
...
DRAW_BATCH_TEST_DEFINE(ShadowCircleBatch) {
    do {
        ...
        GrDrawOp* batch = ShadowCircleBatch::Create(color, viewMatrix, center, radius, blurRadius,
                                                    GrStyle(stroke, nullptr));
        if (batch) {
            return batch;
        }
...
DRAW_BATCH_TEST_DEFINE(ShadowRRectBatch) {
    ...
    SkScalar blurRadius = random->nextSScalar1() * 72.f;
    return create_shadow_rrect_batch(color, viewMatrix, rrect, blurRadius,
