...
private:
    ...
    void onPrepareDraws(Target* target) const override {
        ...
        sk_sp<GrGeometryProcessor> gp(GrRRectShadowGeoProc::Make(!fAllFill, localMatrix));
...
GrDrawBatch* CreateShadowRRectBatch(GrColor color,
                                    ...
                                    const GrShaderCaps* shaderCaps) {
    if (rrect.isOval()) {
        return create_shadow_circle_batch(color, viewMatrix, rrect.getBounds(), stroke, shaderCaps);
    }
    ...
    return create_shadow_rrect_batch(color, needsDistance, viewMatrix, rrect, stroke);
}
