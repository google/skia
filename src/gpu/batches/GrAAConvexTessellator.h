...
public:
    ...
    void addOutsetPt(const SkPoint& p, int index, CurveState curve);
    ...
    SkTDArray<CurveState> fOutsetCurveState;

    void validate2() {
        SkASSERT(fOutsetPts.count() == fMap.count());
        SkASSERT(fOutsetPts.count() == fOutsetCurveState.count());
...
private:
    ...
    class Ring1 {
    public:
        virtual void setReserve(int numPts) = 0;
        virtual void rewind() = 0;

        virtual int numPts() const = 0;
    };

    // The Ring holds a set of indices into the global pool that together define
    // a single polygon inset.
    class InsetRing : public Ring1 {
    public:
        void setReserve(int numPts) override { fPts.setReserve(numPts); }
        void rewind() override { fPts.rewind(); }
    ...
    class OutsetRing : public Ring1 {
    public:
        OutsetRing(bool passOnPts) : fPassOnPts(passOnPts) { }

        void setReserve(int numPts) override { fPts7.setReserve(numPts); }
        void rewind() override { fPts7.rewind(); }

        int numPts() const override { return fPts7.count(); }
    ...
    InsetRing* getNextRing(InsetRing* lastRing);
...
#if 0//GR_AA_CONVEX_TESSELLATOR_VIZ
