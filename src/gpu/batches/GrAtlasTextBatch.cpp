...
SkString GrAtlasTextBatch::dumpInfo() const {
    ...
    for (int i = 0; i < fGeoCount; ++i) {
        str.appendf("%d: Color: 0x%08x Trans: %.2f,%.2f Runs: %d\n", i, fGeoData[i].fColor,
                    fGeoData[i].fX, fGeoData[i].fY, fGeoData[i].fBlob->runCount());
    }
...
void GrAtlasTextBatch::onPrepareDraws(Target* target) const {
    ...
    void* vertices = target->makeVertexSpace(vertexStride, glyphCount * kVerticesPerGlyph,
                                             &vertexBuffer, &flushInfo.fVertexOffset);
...
sk_sp<GrGeometryProcessor> GrAtlasTextBatch::setupDfProcessor(const SkMatrix& viewMatrix,
                                                              SkColor filteredColor, GrColor color,
    ...
    if (isLCD) {
        ...
        GrDistanceFieldLCDTextGeoProc::DistanceAdjust widthAdjust =
                GrDistanceFieldLCDTextGeoProc::DistanceAdjust::Make(redCorrection, greenCorrection,
                                                                    blueCorrection);

        return GrDistanceFieldLCDTextGeoProc::Make(color, viewMatrix, texture, params, widthAdjust,
        ...
        float correction = fDistanceAdjustTable->getAdjustment(lum >> kDistanceAdjustLumShift,
                                                               fUseGammaCorrectDistanceTable);
        return GrDistanceFieldA8TextGeoProc::Make(color, viewMatrix, texture, params, correction,
                                                  flags, this->usesLocalCoords());
#else
        return GrDistanceFieldA8TextGeoProc::Make(color, viewMatrix, texture, params, flags,
                                                  this->usesLocalCoords());
#endif
    }
}

void GrBlobRegenHelper::flush() { fBatch->flush(fTarget, fFlushInfo); }
