...
inline void convertQuadToCubic(const SkPoint p[3], SkPoint out[4]) {
    ...
    float2 c = p1 * (2/3.f);
    out[0] = bit_pun<SkPoint>(p0);
    out[1] = bit_pun<SkPoint>(grvx::fast_fma<2>(p0, 1/3.f, c));
    out[2] = bit_pun<SkPoint>(grvx::fast_fma<2>(p2, 1/3.f, c));
...
//
//   - If the cubic is "serpentine", then the T values are any inflection points where 0 < T < 1.
//
//   - If the cubic is linear, then the T values are any 180-degree cusp points were 0 < T < 1.
//
//   - Otherwise the T value is either the point at which rotation reaches 180 degrees, as long as
//     0 < T < 1, or else none.
//   else .
//
