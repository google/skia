...
int GrPathUtils::findCubicConvex180Chops(const SkPoint pts[], float T[2], bool* areCusps) {
    ...
    SkASSERT(pts);
    SkASSERT(T);
    // The caller must have initialized *areCusps to false (if provided). This function is hot
    // enough that it's costly for us to write out "*areCusps = false" on the 99%+ times that the
    // curve isn't actually a cusp. If the curve turns out to be a cusp, then we will set *areCusps
    // to true.
