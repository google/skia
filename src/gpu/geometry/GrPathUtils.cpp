...
int GrPathUtils::findCubicConvex180Chops(const SkPoint pts[], float T[2]) {
    ...
    if (discr_over_4 < 0) {
        ...
        T[0] = c / b_over_minus_2;
        return T[0] > 0 && T[0] < 1;
    }

    if (a == 0 && b == 0 && c == 0) {
        // The curve is a flat line. The standard inflection function doesn't detect cusps from
        // flat lines. Find cusps by searching instead for points where the tangent is perpendicular
        // to tan0. This will find any cusp point.
        //
