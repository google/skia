...
uint32_t GrPathUtils::quadraticPointCount(const SkPoint points[], SkScalar tol) {
    ...
    // quadratic_log2 bit-shifted to the left gives us the next power of two greater than
    // calling GrWangsFormula::quadratic w/o having to compute a 4th root.
    int wangsLog2 = GrWangsFormula::quadratic_log2(tolerance_to_wangs_precision(tol), points);
    return std::min(1 << wangsLog2, kMaxPointsPerCurve);
}
...
uint32_t GrPathUtils::cubicPointCount(const SkPoint points[],
                                           SkScalar tol) {
    int wangsLog2 = GrWangsFormula::cubic_log2(tolerance_to_wangs_precision(tol), points);
    return std::min(1 << wangsLog2, kMaxPointsPerCurve);
}
