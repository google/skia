...
const TessellationHelper::OutsetRequest& TessellationHelper::getOutsetRequest(
    ...
    if (!fOutsetRequest.fValid || any(edgeDistances != fOutsetRequest.fEdgeDistances)) {
        ...
        if (fDeviceType <= GrQuad::Type::kRectilinear) {
            ...
            float widthChange = 1.f / (edgeDistances[0] + edgeDistances[3]);
            float heightChange = 1.f / (edgeDistances[1] + edgeDistances[2]);
            fOutsetRequest.fInsetDegenerate = (fEdgeVectors.fInvLengths[1] > widthChange) ||
                                              (fEdgeVectors.fInvLengths[0] > heightChange);
