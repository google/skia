...
void TessellationHelper::EdgeVectors::reset(const skvx::Vec<4, float>& xs,
    ...
    if (kDM == DeviceMode::kAssumeRectilinear || quadType < GrQuad::Type::kPerspective) {
        SkASSERT(kDM == DeviceMode::kAny || quadType <= GrQuad::Type::kRectilinear);
        fX2D = xs;
        fY2D = ys;
    } else {
        V4f iw = 1.0 / ws;
    ...
    if (kDM == DeviceMode::kAssumeRectilinear || quadType <= GrQuad::Type::kRectilinear) {
...
void TessellationHelper::Vertices::reset(const GrQuad& deviceQuad, const GrQuad* localQuad) {
    ...
    if constexpr (kDM != DeviceMode::kAssumeRectilinear) {
        fW = deviceQuad.w4f();
    } else {
...
void TessellationHelper::Vertices::moveAlong(const EdgeVectors& edgeVectors,
    ...
    if constexpr (kDM == DeviceMode::kAny) {
        // Must scale by inv sin theta (when kDM == kAssumeRectilinear, inv sin theta is 1, so
        // there's no need for these multiplies).
...
void TessellationHelper::reset(const GrQuad& deviceQuad, const GrQuad* localQuad) {
    ...
    DeviceMode dm = deviceQuad.quadType() <= GrQuad::Type::kRectilinear ? DeviceMode::kAssumeRectilinear : DeviceMode::kAny;
    // dm = DeviceMode::kAny;
    LocalMode lm = localQuad ? (localQuad->hasPerspective() ? LocalMode::kUVR : LocalMode::kUV) : LocalMode::kNone;
...
void TessellationHelper::adjustVertices(const skvx::Vec<4, float>& signedEdgeDistances,
    ...
    if (kDM == DeviceMode::kAssumeRectilinear || fDeviceType < GrQuad::Type::kPerspective) {
        // For non-perspective, non-degenerate quads, moveAlong is correct and most efficient
        SkASSERT(kDM == DeviceMode::kAny || fDeviceType <= GrQuad::Type::kRectilinear);
...
int TessellationHelper::adjustDegenerateVertices(const skvx::Vec<4, float>& signedEdgeDistances,
    ...
    if (kDM == DeviceMode::kAssumeRectilinear || fDeviceType <= GrQuad::Type::kRectilinear) {
...
V4f TessellationHelper::Inset(TessellationHelper* t, const skvx::Vec<4, float>& edgeDistances,
    ...
    if (vertexCount < 3) {
        ...
        if constexpr (kDM == DeviceMode::kAssumeRectilinear) {
            ...
            return t->getEdgeEquations().estimateCoverage(inset.fX / inset.fW,
                                                             inset.fY / inset.fW);
        }
    } else {
        return 1.f;
...
TessellationHelper::FunctionTable TessellationHelper::kFunctionTables[] = {
    FunctionTable(FunctionTable::Config<DeviceMode::kAssumeRectilinear, LocalMode::kNone>()),
    FunctionTable(FunctionTable::Config<DeviceMode::kAny,               LocalMode::kNone>()),
    FunctionTable(FunctionTable::Config<DeviceMode::kAssumeRectilinear, LocalMode::kUV>()),
    FunctionTable(FunctionTable::Config<DeviceMode::kAny,               LocalMode::kUV>()),
    FunctionTable(FunctionTable::Config<DeviceMode::kAssumeRectilinear, LocalMode::kUVR>()),
