...
int ClipToW0(DrawQuad* quad, DrawQuad* extraVertices) {
    ...
    if (clipCount != 1) {
        ...
        v.fW = if_then_else(validW, v.fW, SkPathPriv::kW0PlaneDistance);
        ...
        v2.fW = if_then_else(!validW | !ccwValid, SkPathPriv::kW0PlaneDistance,
        ...
        v.fW = if_then_else(!validW, SkPathPriv::kW0PlaneDistance,
        ...
        v.fV = if_then_else(!validW, next_cw(clip.fR),
...
bool CropToRect(const SkRect& cropRect, GrAA cropAA, GrQuadAAFlags* edgeFlags, GrQuad* quad,
    ...
    V4f devX = quad->x4f();
    V4f devY = quad->y4f();
    // Project the 3D coordinates to 2D (assuming they've been clipped already)
    if (quad->quadType() == GrQuad::Type::kPerspective) {
        V4f devW = quad->w4f();
        SkASSERT(all(devW >= SkPathPriv::kW0PlaneDistance));
        devW = 1.f / devW;
        devX *= devW;
        devY *= devW;
    }
...
void TessellationHelper::Vertices::moveTo(const V4f& x2d, const V4f& y2d, const M4f& mask) {
    ...
    // If fW has gone negative, flip the point to the otherwise of w=0. This only happens if the
    ...
    if (any(fW < 0.f)) {
        V4f scale = if_then_else(fW < 0.f, -1.f, 1.f);
        fX *= scale;
        fY *= scale;
        fW *= scale;
    }
