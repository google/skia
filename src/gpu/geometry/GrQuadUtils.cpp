...
const TessellationHelper::OutsetRequest& TessellationHelper::getOutsetRequest(
    ...
    if (!fOutsetRequest.fValid || any(edgeDistances != fOutsetRequest.fEdgeDistances)) {
        ...
        if (fDeviceType <= GrQuad::Type::kRectilinear) {
            ...
            bool widthCollapse = fEdgeVectors.fInvLengths[1] > 1.f / (edgeDistances[0] + edgeDistances[3]);
            bool heightCollapse = fEdgeVectors.fInvLengths[0] > 1.f / (edgeDistances[1] + edgeDistances[2]);
            fOutsetRequest.fInsetDegenerate = widthCollapse || heightCollapse;
...
void TessellationHelper::Vertices::moveAlong(const EdgeVectors& edgeVectors,
    ...
    SkASSERT(!any(abs(edgeVectors.fCosTheta)) >= 0.9f);
