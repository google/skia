...
void GrStyledShape::writeUnstyledKey(uint32_t* key) const {
    ...
    // Additional bits set for rect and line shapes to disambiguate them from each other. Otherwise
    // they are both 4 floats + flags, which could collide (e.g. draw a diagonal line between two
    // corners of the rect, both with same winding/start etc.).
...
void GrStyledShape::simplify() {
    ...
    fSimplified = fShape.simplify(simplifyFlags);
...
bool GrStyledShape::simplifyStroke(bool originallyClosed) {
    ...
    // TODO: Could a stroke+fill rect with round joins turn into a rrect?
    ...
    if (originallyClosed) {
        ...
        switch(fStyle.strokeRec().getJoin()) {
            case SkPaint::kRound_Join:
                // For point or line, this simply maps to round caps
        ...
        //     fStyle.strokeRec().getJoin() != SkPaint::kDefault_Join) {
            SkStrokeRec rec = fStyle.strokeRec();
    ...
    if (fShape.isPoint()) {
        ...
        if (fShape.line().fP1.fY == fShape.line().fP2.fY) {
            ...
            // Even if we couldn't apply the stroke, if the style was modified above, we should
            // report that so GrRTC can try again.
            return strokeSimplified;
        }
