...
int GrStyledShape::unstyledKeySize() const {
    ...
    if (fShape.isEmpty()) {
        ...
        // + 1 for inverseness
        return sizeof(SkPoint) / sizeof(uint32_t) + 1;
...
void GrStyledShape::writeUnstyledKey(uint32_t* key) const {
    // Bits for winding and fill, kept high to avoid the start index value put in the low bits
    static constexpr uint32_t kCCWDirFlag = 1 << 31;
    static constexpr uint32_t kInvertedFlag = 1 << 30;
    // Additional bits set for rect and line shapes to disambiguate them from
    // each other. Otherwise they are both 4 floats + flags, which can conflict
    ...
    if (fInheritedKey.count()) {
        ...
        if (fShape.isPath()) {
            ...
            *key++ = (uint32_t) fShape.path().getFillType();
            ...
            uint32_t flags = (fShape.dir() == SkPathDirection::kCCW ? kCCWDirFlag : 0) |
                            (fShape.inverted() ? kInvertedFlag : 0) |
                            (fShape.isRect() ? kRectFlag : 0) |
                            (fShape.isLine() ? kLineFlag : 0) |
                            fShape.startIndex();
...
void GrStyledShape::simplify() {
    ...
    bool inverted = !this->style().isDashed() && fShape.inverted();
    ...
    if (fStyle.isSimpleFill()) {
        ...
        if (!fShape.isArc() || this->style().strokeRec().getCap() == SkPaint::kButt_Cap) {
    ...
    if (fShape.isPath()) {
        ...
        if (!this->style().hasNonDashPathEffect() &&
            (this->style().strokeRec().getStyle() == SkStrokeRec::kStroke_Style ||
        ...
        this->applySimpleStroke(wasClosed);
    }
...
void GrStyledShape::applySimpleStroke(bool originallyClosed) {
    // The simplest case is a stroke+fill, mitered rect. This simply becomes a filled style with
    // a larger rect shape.
    if (!this->style().hasPathEffect() && fShape.isRect() &&
    ...
    // Otherwise, if we're a point or a line, we may be to explicitly apply some of the stroking
    // (and even some of the dashing). Any other shape+style is too complicated to reduce.
    if ((!fShape.isPoint() && !fShape.isLine()) || this->style().hasNonDashPathEffect() ||
    ...
    if (this->style().isDashed()) {
        ...
        bool dropDash = false;
        if (fShape.isPoint()) {
            dropDash = this->style().dashIntervalCnt() > 0 &&
                       SkToBool(this->style().dashIntervals()[0]);
        } else {
            dropDash = true;
            for (int i = 1; i < this->style().dashIntervalCnt(); i += 2) {
                if (SkToBool(this->style().dashIntervals()[i])) {
        ...
        // Fall through to modifying the shape to respect the stroke geometry
    ...
    if (fStyle.isSimpleFill()) {
        fShape.reset(); // FIXME preserve inverted
        return;
    ...
    // A point or line that was formed by a degenerate closed shape should use the join type
    // to determine the caps, not original cap style (since the original geometry didn't have any).
    if (originallyClosed) {
        SkPaint::Cap cap;
        switch(fStyle.strokeRec().getJoin()) {
            case SkPaint::kRound_Join:
                // Point or line, this simply maps to round caps
                cap = SkPaint::kRound_Cap;
                break;
            case SkPaint::kBevel_Join:
                // Point or line, this goes to a butt cap since the shape of a bevel as the angle
                // approaches 180 degrees tends to a butt cap. An infinitely thin rectangle produces
                // such 180 degree corners.
                cap = SkPaint::kButt_Cap;
                break;
            case SkPaint::kMiter_Join:
                // At the limit from 180 degree turns, the shape exceeds any miter limit,
                // so it would switch to bevel, which would then map to a butt cap. However, since
                // miter is the default join, we switch to square when the shape is a point. This
                // ensures the default paint will draw something for point geometry.
                cap = fShape.isPoint() ? SkPaint::kSquare_Cap : SkPaint::kButt_Cap;
                break;
            default:
                SkUNREACHABLE;
        }
    ...
    if (fShape.isPoint()) {
        ...
        // allowed rotation angle, this would work for all lines.
        SkRect rect;
        SkVector outset;
