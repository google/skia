...
public:
    ...
    // Returns true if the shape type has been changed. If 'originalyClosed' is not null, it
    // will be set to true if the shape was closed before simplification.
    bool simplify(unsigned flags = kAll_Flags, bool* originallyClosed = nullptr);
