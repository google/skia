 ...
 *
 * In order to lossless simplification of the geometry, it also tracks winding direction, start
...
public:
    ...
    GrShape() : fType(static_cast<uint16_t>(Type::kEmpty)) {}
    explicit GrShape(const SkPoint& point) : GrShape() { this->setPoint(point); }
    explicit GrShape(const SkRect& rect) : GrShape() { this->setRect(rect); }
    explicit GrShape(const SkRRect& rrect) : GrShape() { this->setRRect(rrect); }
    explicit GrShape(const SkPath& path) : GrShape() { this->setPath(path); }
    explicit GrShape(const GrArc& arc) : GrShape() { this->setArc(arc); }
    explicit GrShape(const GrLineSegment& line) : GrShape() { this->setLine(line); }

    explicit GrShape(const GrShape& shape) : GrShape() { *this = shape; }
    ...
    bool isEmpty() const { return static_cast<Type>(fType) == Type::kEmpty; }
    bool isPoint() const { return static_cast<Type>(fType) == Type::kPoint; }
    bool isRect() const { return static_cast<Type>(fType) == Type::kRect; }
    bool isRRect() const { return static_cast<Type>(fType) == Type::kRRect; }
    bool isPath() const { return static_cast<Type>(fType) == Type::kPath; }
    bool isArc() const { return static_cast<Type>(fType) == Type::kArc; }
    bool isLine() const { return static_cast<Type>(fType) == Type::kLine; }

    // Whether or not the shape is meant to be the inverse of its geometry (i.e. its exterior).
    bool inverted() const {
        return this->isPath() ? this->path().isInverseFillType() : SkToBool(fInverted);
    }
    ...
    SkPathDirection dir() const { return static_cast<SkPathDirection>(fDir); }
    ...
    void setPathWindingParams(SkPathDirection dir, unsigned start) {
        SkASSERT(start < 8);
        fDir = static_cast<uint16_t>(dir);
        fStart = static_cast<uint16_t>(start);
    }

    void setInverted(bool inverted) {
        if (this->isPath()) {
            if (inverted != this->path().isInverseFillType()) {
    ...
    //
    // These also reset any extracted direction, start, or inverted state from a prior simplified
    ...
    void setPath(const SkPath& path) {
        if (this->isPath()) {
            ...
            fType = static_cast<uint16_t>(Type::kPath);
        }
    ...
    void simplify(unsigned flags = kAll_Flags);
...
private:
    ...
    void reset(Type type) {
        ...
        fType = static_cast<uint16_t>(type);
        this->setPathWindingParams(kDefaultDir, kDefaultStart);
    ...
    // Bit field encoding shape type, path direction, start, and fill type. These path
    // parameters are preserved externally so that simplification is not lossy. They are
    // ignored if the type is path(), since it implicitly provides their values.
    uint16_t fType     : 3; // GrShape::Type
    uint16_t fStart    : 3; // Restricted to rrects and simpler, where start < 8
    uint16_t fDir      : 2; // SkPathDirection
    uint16_t fInverted : 1;

    static_assert(kTypeCount <= 8);
};
