...
// TODO (michaelludwig) - If this works well and doesn't regress, it will replace GrQuadList
template<typename T>
class GrQuadBuffer {
public:
    GrQuadBuffer()
        ...
        // FIXME (michaelludwig) - Should we switch this to be stack-allocated variant, SkSTArray?
        // FIXME (michaelludwig) - Since this is intended only for ops, can we use the arena to
        //      allocate storage for the quad buffer? Would that require linked-lists when batching
        //      ops together to avoid re-allocating contiguous space for the combined buffer.
...
private:
    struct Header {
        ...
        // Magic number to fill the other bits with to make sure iteration lands on a header
        SkDEBUGCODE(unsigned fMC : 27;)
    };
    static_assert(sizeof(Header) == sizeof(int32_t), "Header should be 4 bytes");

    static constexpr unsigned kMagicNumber = 0xbaffe;
...
void GrQuadBuffer<T>::append(const GrQuad& deviceQuad, T&& metadata, const GrQuad* localQuad) {
    ...
    SkDEBUGCODE(h->fMC = static_cast<unsigned>(kMagicNumber);)
...
void GrQuadBuffer<T>::validate(const char* entry, int expectedCount) const {
    ...
    SkASSERT(this->header(entry)->fMC == kMagicNumber);
}
#endif
