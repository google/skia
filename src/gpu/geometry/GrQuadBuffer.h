...
class GrQuadAllocator {
public:
    static std::unique_ptr<GrQuadAllocator> Make(size_t preallocSize) {
        void* mem = operator new(preallocSize);
        return std::unique_ptr<GrQuadAllocator>(new (mem) GrQuadAllocator(preallocSize));
    }
...
private:
    ...
    GrQuadAllocator(size_t preallocSize)
            : fAlloc(GrBlockAllocator::GrowthPolicy::kFibonacci, preallocSize,
                     preallocSize - sizeof(GrBlockAllocator)) {}

    GrBlockAllocator fAlloc;
};

// The GrQuadBuffer provides a semi-dense iterable of GrQuads + metadata, storing the minimal amount
// of coordinate information per GrQuad. The GrQuadBuffer is effectively a view on top of a
// GrBlockAllocator, but is implemented in such a way that the GrQuadBuffers from multiple ops
// share a single GrBlockAllocator from the recording context.
template<typename T>
class GrQuadBuffer {
private:
    static constexpr size_t kAlign = 4;
    static_assert(alignof(T) == kAlign, "Buffer metadata T must be 4 byte aligned");
...
public:
    ...
    void dumpInfo() const {
        SkDebugf("GrQuadBuffer: ct = %d, dt = %d, lt = %d (%d), f0 = %d, f1 = %d\n",
            fCount, fDeviceType, fLocalType, fHasLocals, fFib0, fFib1);
        Block* h = fHead;
        while(h) {
            SkDebugf(" - Block %d of %d (%.2f%)\n", h->used(), h->fDataSize, (100.f * h->used()) / h->fDataSize);
            h = h->fNext;
        }
    }
    ...
    void append(GrQuadAllocator* arena, const GrQuad& deviceQuad, T&& metadata,
    ...
    void concat(GrQuadAllocator* arena, GrQuadBuffer<T>* that);

    void reserve(GrQuadAllocator* arena, int count, GrQuad::Type expectedDeviceType,
    ...
    size_t remaining() const {
        return fTail ? fTail->remaining() : 0;
    }
#endif
...
private:
    ...
    static_assert(alignof(Header) <= kAlign);
    ...
    // FIXME maybe don't overload "Block" with what's defined in GrBlockAllocator...
    struct Block {
        // fDataSize and fIndex are packed into 16 bits, so make sure not to allocate more
        // than can be indexed.
        static constexpr int kMaxDataSize = (int) UINT16_MAX;
        ...
        Block(int allocatorOffset, size_t bytesAllocated)
                : fNext(nullptr)
                , fAllocatorOffset(allocatorOffset)
                , fDataSize(bytesAllocated - kBlockSize)
                , fIndex(0) {
            // SkDebugf("New block, data size: %d\n", fDataSize);
        ...
        inline GrBlockAllocator::Block* block() const {
            return reinterpret_cast<GrBlockAllocator::Block*>(reinterpret_cast<intptr_t>(this) - fAllocatorOffset);
        }
    ...
    static_assert(sizeof(Block) == 24, "Block should be 16 bytes");
    static_assert(alignof(Block) >= kAlign);

    static constexpr uint32_t kSentinel = 0xbaffe;
    static constexpr int kHeaderSize = sizeof(Header);
    ...
    static constexpr int kBlockSize = sizeof(Block) - 8;
    static constexpr int k2DQuadSize = 8 * sizeof(float);
    static constexpr int k3DQuadSize = 12 * sizeof(float);

    // Each Fibonacci term is packed into 13 bits, so saturate the series at this value
    static constexpr int kMaxFib = (1 << 13) - 1;

    // These point to data allocated in the GrBlockAllocator, preferably the record time allocator
    // of the GrRecordingContext for the ops that store geometry in the buffer.
    Block*   fHead;
    Block*   fTail;
    ...
    Block* addBlock(GrQuadAllocator* arena, int minSize, int maxSize) {
        minSize = std::min(minSize, Block::kMaxDataSize) + kBlockSize;
        maxSize = std::min(maxSize, Block::kMaxDataSize) + kBlockSize;
        int avail = arena->fAlloc.currentBlock()->avail<alignof(Block)>();
        if (avail < maxSize && avail >= minSize) {
            // Allocate a block of the available size
            auto br = arena->fAlloc.allocate<alignof(Block)>(avail);
            return new (br.fBlock->ptr(br.fAlignedOffset)) Block(br.fAlignedOffset, avail);
        } else {
            // Make a max size block
            auto br = arena->fAlloc.allocate<alignof(Block)>(maxSize);
            return new (br.fBlock->ptr(br.fAlignedOffset)) Block(br.fAlignedOffset, maxSize);
        }
    }

    Block* addBlock(GrQuadAllocator* arena, int entrySize) {
        // Determine max based on Fibonacci growth, min will always be the single entry
        int n = fFib0 + fFib1;
        fFib0 = fFib1;
        fFib1 = std::min(n, kMaxFib);
        // FIXME can n * entrySize overflow?
        return this->addBlock(arena, entrySize, n * entrySize);
    }

    bool growBlock(GrQuadAllocator* arena, Block* block, int growSize) {
        GrBlockAllocator::Block* owner = block->block();
        growSize = std::min(owner->avail<kAlign>(), growSize);
        int newSize = kBlockSize + std::min(growSize + block->fDataSize, Block::kMaxDataSize);
        if (owner->resize(block->fAllocatorOffset, block->fAllocatorOffset + kBlockSize + block->fDataSize, newSize)) {
    ...
    void growBlock(GrQuadAllocator* arena, Block* block) {
        int n = fFib0 + fFib1;
        int expectedEntrySize = EntrySize(this->deviceQuadType(),
                                             this->localQuadType(),
                                             fHasLocals);
...
template<typename T>
void GrQuadBuffer<T>::reserve(GrQuadAllocator* arena,
                              int count,
                              GrQuad::Type expectedDeviceType,
                              GrQuad::Type expectedLocalType,
                              bool expectedHasLocals) {
    int entrySize = EntrySize(expectedDeviceType, expectedLocalType, expectedHasLocals);
    int minEntryCount = std::max(count / 3, 1);
    if (fHead) {
...
template<typename T>
void GrQuadBuffer<T>::append(GrQuadAllocator* arena,
...
template<typename T>
void GrQuadBuffer<T>::concat(GrQuadAllocator* arena, GrQuadBuffer<T>* that) {
    if (!fHead) {
        ...
        while(thatHead && thatHead->used() <= fTail->remaining()) {
            ...
            // SkDebugf("Copied %d during concat\n", thatHead->used());
            ...
            // FIXME need to recover the GrBlockAllocator's block thatHead belonged to, and then
            // also need to convert this to the 'start' + 'end' params that release() expects
            bool success = thatHead->block()->release(thatHead->fAllocatorOffset, thatHead->fAllocatorOffset + thatHead->fDataSize + kBlockSize);
        ...
        if (thatHead) {
            ...
            fFib0 = std::max(fFib0, that->fFib0);
            fFib1 = std::max(fFib1, that->fFib1);
    ...
    fCount += that->fCount;
    fDeviceType = std::max(fDeviceType, that->fDeviceType);
    fLocalType = std::max(fLocalType, that->fLocalType);
