...
public:
    ...
    void append(GrBlockAllocator* arena, const GrQuad& deviceQuad, T&& metadata,
    ...
    void concat(GrBlockAllocator* arena, GrQuadBuffer<T>* that);

    void reserve(GrBlockAllocator* arena, int count, GrQuad::Type expectedDeviceType,
...
private:
    ...
    Block* addBlock(GrBlockAllocator* arena, size_t minSize, size_t maxSize) {
        minSize = SkTMin(minSize, Block::kMaxDataSize) + kBlockSize;
        maxSize = SkTMin(maxSize, Block::kMaxDataSize) + kBlockSize;
        size_t avail = arena->avail<kAlign>();
        if (avail < maxSize && avail >= minSize) {
            // Allocate a block of the available size
            void* blockPtr = arena->allocate<kAlign>(avail);
            return new (blockPtr) Block(avail);
        } else {
            // Make a max size block
            void* blockPtr = arena->allocate<kAlign>(maxSize);
            return new (blockPtr) Block(maxSize);
        }
    }

    Block* addBlock(GrBlockAllocator* arena, size_t entrySize) {
    ...
    bool growBlock(GrBlockAllocator* arena, Block* block, size_t growSize) {
        growSize = SkTMin(arena->avail<kAlign>(), growSize);
    ...
    void growBlock(GrBlockAllocator* arena, Block* block) {
        size_t n = fFib0 + fFib1;
...
template<typename T>
void GrQuadBuffer<T>::reserve(GrBlockAllocator* arena,
                              int count,
...
template<typename T>
void GrQuadBuffer<T>::append(GrBlockAllocator* arena,
...
template<typename T>
void GrQuadBuffer<T>::concat(GrBlockAllocator* arena, GrQuadBuffer<T>* that) {
    if (!fHead) {
