...
sk_sp<GrGpu> GrStubGpu::Make(const GrContextOptions& contextOptions, GrContext* context,
                         sk_sp<const GrCaps> caps) {
    return sk_sp<GrGpu>(new GrStubGpu(context, contextOptions, std::move(caps)));
}
...
GrStubGpu::GrStubGpu(GrContext* context, const GrContextOptions& contextOptions,
                     sk_sp<const GrCaps> caps)
        : INHERITED(context) {
    fCaps = caps;
}
...
GrBackendObject GrStubGpu::createTestingOnlyBackendTexture(void* pixels, int w, int h,
...
bool GrStubGpu::isTestingOnlyBackendTexture(GrBackendObject object) const {
    SkASSERT(0);
    return false;
}

void GrStubGpu::deleteTestingOnlyBackendTexture(GrBackendObject object, bool abandonTexture) {
    SkASSERT(0);
}
