...
public:
    ...
    void addDrawOp(GrDrawingManager* drawingMgr, std::unique_ptr<GrDrawOp> op,
                   ...
                   GrDstSampleType dstSampleType, GrTextureResolveManager textureResolveManager,
                   const GrCaps& caps) {
        auto addDependency = [ drawingMgr, textureResolveManager, &caps, this ] (
                GrSurfaceProxy* p, GrMipmapped mipmapped) {
        ...
        if (dstProxyView.proxy()) {
            if (GrDstSampleTypeUsesTexture(dstSampleType)) {
            ...
            if (this->target(0).proxy() == dstProxyView.proxy()) {
                ...
                // texture barriers.
                SkASSERT(GrDstSampleTypeDirectlySamplesDst(dstSampleType));
                fUsesXferBarriers |= true;
            }
            SkASSERT(dstSampleType != GrDstSampleType::kNone);
            SkASSERT(fDstSampleType == GrDstSampleType::kNone || fDstSampleType == dstSampleType);
            SkASSERT(dstSampleType != GrDstSampleType::kAsInputAttachment ||
...
private:
    ...
    // This tracks what type of dst sampling is done by ops in this GrOpsTasks. For a given
    // GrOpsTasks, all ops should either be GrDstSampleType::kNone or one of kAsTexturecopy,
