...
sk_sp<SkBaseGpuDevice> GrDirectContextPriv::createDevice(GrColorType colorType,
    ...
    GrRecordingContext* rContext = fContext;
    return rContext->priv().createDevice(colorType, std::move(proxy), std::move(colorSpace),
                                         origin, props, init);
}
...
sk_sp<SkBaseGpuDevice> GrDirectContextPriv::createDevice(SkBudgeted budgeted,
                                                         ...
                                                         SkBaseGpuDevice::InitContents init) {
    GrRecordingContext* rContext = fContext;
    return rContext->priv().createDevice(budgeted, ii, fit, sampleCount, mipmapped, isProtected,
                                         origin, props, init);
}
