...
GrBackendTextureImageGenerator::RefHelper::RefHelper(GrTexture* texture,
        ...
        , fBorrowingContextID(GrDirectContextPriv::ExplicitContextID::Invalid())
...
GrBackendTextureImageGenerator::Make(sk_sp<GrTexture> texture, GrSurfaceOrigin origin,
    ...
    return std::unique_ptr<SkImageGenerator>(new GrBackendTextureImageGenerator(
          info, texture.get(), origin, dContext->priv().explicitContextID(),
          dContext->priv().contextID(),
...
GrSurfaceProxyView GrBackendTextureImageGenerator::onGenerateTexture(
    ...
    if (fRefHelper->fBorrowingContextID.isValid()) {
        if (fRefHelper->fBorrowingContextID != dContext->priv().explicitContextID()) {
    ...
    fRefHelper->fBorrowingContextID = dContext->priv().explicitContextID();
    ...
    SkASSERT(fRefHelper->fBorrowingContextID == dContext->priv().explicitContextID());
