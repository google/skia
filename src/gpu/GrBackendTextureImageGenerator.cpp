...
GrBackendTextureImageGenerator::RefHelper::RefHelper(
        ...
        , fBorrowingContextID(GrDirectContextPriv::DirectContextID::Invalid())
...
GrBackendTextureImageGenerator::RefHelper::~RefHelper() {
    ...
    GrTextureFreedMessage msg { fOriginalTexture, fFamilyID };
...
GrBackendTextureImageGenerator::Make(sk_sp<GrTexture> texture, GrSurfaceOrigin origin,
    ...
    return std::unique_ptr<SkImageGenerator>(new GrBackendTextureImageGenerator(
          info, texture.get(), origin, dContext->priv().directContextID(),
          dContext->priv().contextID(), std::move(semaphore), backendTexture));
}
...
GrBackendTextureImageGenerator::GrBackendTextureImageGenerator(
        ...
        : INHERITED(info)
        , fRefHelper(new RefHelper(texture, owningContextID, familyID, std::move(semaphore)))
...
GrSurfaceProxyView GrBackendTextureImageGenerator::onGenerateTexture(
    ...
    if (fRefHelper->fBorrowingContextID.isValid()) {
        if (fRefHelper->fBorrowingContextID != dContext->priv().directContextID()) {
    ...
    fRefHelper->fBorrowingContextID = dContext->priv().directContextID();
    ...
    SkASSERT(fRefHelper->fBorrowingContextID == dContext->priv().directContextID());
