...
GrBackendTextureImageGenerator::RefHelper::~RefHelper() {
    SkASSERT(nullptr == fBorrowedTextureProxy);
...
void GrBackendTextureImageGenerator::ReleaseRefHelper_TextureReleaseProc(void* ctx) {
    ...
    refHelper->fBorrowedTextureProxy = nullptr;
...
sk_sp<GrTextureProxy> GrBackendTextureImageGenerator::onGenerateTexture(
    ...
    if (fRefHelper->fBorrowingContextID == context->uniqueID()) {
        ...
        proxy = sk_ref_sp(fRefHelper->fBorrowedTextureProxy);
        SkASSERT(proxy);
    } else {
...
#if 0
        sk_sp<GrTexture> tex = context->resourceProvider()->wrapBackendTexture(fBackendTexture,
        ...
        proxy = GrSurfaceProxy::MakeWrapped1(std::move(tex), fSurfaceOrigin);
#else
        proxy = proxyProvider->createWrappedTextureProxy(fBackendTexture, fSurfaceOrigin,
                                                         kBorrow_GrWrapOwnership,
        ...
        fRefHelper->fBorrowedTextureProxy = proxy.get();
        fRefHelper->ref();
#endif
