...
sk_sp<GrTextureProxy> GrBackendTextureImageGenerator::onGenerateTexture(
    ...
    // be deleted before we actaully execute the lambda.
    uint32_t contextID = context->uniqueID();
    ...
    sk_sp<GrTextureProxy> proxy = GrSurfaceProxy::MakeLazy(
            [refHelper, semaphore, backendTexture, contextID, surfaceOrigin]
            (GrResourceProvider* resourceProvider, GrSurfaceOrigin* outOrigin) {
                ...
                if (refHelper->fBorrowedTexture) {
                    ...
                    tex = resourceProvider->wrapBackendTexture(backendTexture, kBorrow_GrWrapOwnership);
                    if (!tex) {
                        refHelper->unref();
                        return sk_sp<GrTexture>();
                    }
                ...
                SkASSERT(refHelper->fBorrowingContextID == contextID);
