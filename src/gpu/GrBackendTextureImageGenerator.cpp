...
GrBackendTextureImageGenerator::RefHelper::~RefHelper() {
    // Last ref to our texture is gone. Notify the original cache that it can free the last ref,
    // so it happens on the right thread.
    GrGpuResourceFreedMessage msg { fTexture, fOwningContextID };
...
sk_sp<GrTextureProxy> GrBackendTextureImageGenerator::onGenerateTexture(GrContext* context,
    ...
    // If we're on the original context, we could use the original texture, but we'd have no way of
    // detecting that it's no longer in-use. So we always make a wrapped copy, where the release
    // proc informs us that the context is finished with it. This is unfortunate - we'll have two
    // texture objects referencing the same underlying GPU object. However, no client can ever see
    // the original texture, so this should be safe.
