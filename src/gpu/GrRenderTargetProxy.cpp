...
bool GrRenderTargetProxy::canUseStencil(const GrCaps& caps) const {
    ...
    if (this->isLazy()) {
        if (this->backendFormat().backend() == GrBackendApi::kOpenGL) {
            ...
            // the best we can: if a lazy proxy has a texture, we know we can change the stencil.
            // Otherwise, be conservative and block stencil usage.
            // FIXME: Lazy proxies need to tell us if we can use stencil.
            return this->asTextureProxy();
        } else {
            return true;
        }
