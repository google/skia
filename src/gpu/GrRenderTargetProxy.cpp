...
sk_sp<GrSurface> GrRenderTargetProxy::foo(GrResourceProvider* resourceProvider) const {
    static constexpr GrSurfaceFlags kFlags = kRenderTarget_GrSurfaceFlag;

    sk_sp<GrSurface> surface = this->fooImpl(resourceProvider, fSampleCnt, kFlags,
                                             /* isMipped = */ false,
                                             SkDestinationSurfaceColorMode::kLegacy);
    if (!surface) {
        return nullptr;
