...
bool GrConvertPixels(const GrPixelInfo& dstInfo, void* dst, const GrPixelInfo& srcInfo,
    ...
    storeSwizzle = GrSwizzle::Concat(swizzle, storeSwizzle);
    ...
    if (srcInfo.fOrigin != dstInfo.fOrigin) {
        srcCtx.pixels =
                static_cast<char*>(srcCtx.pixels) + srcInfo.fRowBytes * (srcInfo.fHeight - 1);
    ...
    if (SkColorSpace::Equals(srcInfo.fColorInfo.fColorSpace,
        ...
        if (dstIsNormalized && !srcIsNormalized &&
            ...
            GrSwizzle::Concat(loadSwizzle, storeSwizzle).apply(&pipeline);
        }
    } else {
        SkColorSpaceXformSteps steps{srcInfo.fColorInfo.fColorSpace, srcInfo.fColorInfo.fAlphaType,
