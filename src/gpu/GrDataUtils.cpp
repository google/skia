...
bool GrConvertPixels(const GrPixelInfo& dstInfo,       void* dst, size_t dstRB,
    ...
    if ((dstInfo.alphaType() == kOpaque_SkAlphaType) &&
        (srcInfo.alphaType() != kOpaque_SkAlphaType)) {
        return false;
    }
    ...
    auto loadSwizzle =
            get_load_and_get_swizzle(srcInfo.colorType(), &load, &srcIsNormalized);
    loadSwizzle = GrSwizzle::Concat(loadSwizzle, swizzle);

    SkRasterPipeline::StockStage store;
    bool dstIsNormalized;
    auto storeSwizzle =
            get_dst_swizzle_and_store(dstInfo.colorType(), &store, &dstIsNormalized);
