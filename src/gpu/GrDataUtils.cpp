...
bool GrConvertPixels(const GrPixmap& dst, const GrCPixmap& src, bool flipY) {
    ...
    if (dst.colorType() == GrColorType::kRGB_888) {
        ...
        auto [temp, storage] = GrPixmap::Allocate(dst.info().makeColorType(GrColorType::kRGB_888x));
    ...
    if (src.colorType() == dst.colorType() && !alphaOrCSConversion) {
        ...
        if (flipY) {
            ...
            auto d = static_cast<char*>(dst.addr()) + dst.rowBytes()*(dst.height() - 1);
    ...
    bool srcIsSRGB;
    auto loadSwizzle =
            get_load_and_src_swizzle(src.colorType(), &load, &srcIsNormalized, &srcIsSRGB);

    SkRasterPipeline::StockStage store;
    LumMode lumMode;
    bool dstIsNormalized;
    bool dstIsSRGB;
    auto storeSwizzle = get_dst_swizzle_and_store(dst.colorType(), &store, &lumMode,
