...
GrSwizzle get_load_and_get_swizzle(GrColorType ct, SkRasterPipeline::StockStage* load,
                                   bool* isNormalized) {
...
GrSwizzle get_dst_swizzle_and_store(GrColorType ct, SkRasterPipeline::StockStage* store,
        bool* isNormalized) {
    GrSwizzle swizzle("rgba");
    *isNormalized = true;
    switch (ct) {
        ...
        case GrColorType::kAlpha_F16:        *store = SkRasterPipeline::store_af16;    break;
...
static void append_clamp_gamut(SkRasterPipeline* pipeline) {
...
bool GrConvertPixels(const GrPixelInfo& dstInfo, GrSwizzle dstSwizzle, void* dst,
    ...
    srcSwizzle = GrSwizzle::Concat(loadSwizzle, srcSwizzle);
    ...
    dstSwizzle = GrSwizzle::Concat(dstSwizzle, storeSwizzle);
    ...
    if (srcInfo.fOrigin != dstInfo.fOrigin) {
        srcCtx.pixels = static_cast<char*>(srcCtx.pixels) +
                        srcInfo.fRowBytes * (srcInfo.fHeight - 1);
        srcCtx.stride = -srcCtx.stride;
    }
    SkRasterPipeline_<256> pipeline;
    pipeline.append(load, &srcCtx);

    bool alphaTypeConversion = srcInfo.fColorInfo.fAlphaType != dstInfo.fColorInfo.fAlphaType &&
                               srcInfo.fColorInfo.fAlphaType != kOpaque_SkAlphaType;
    if (SkColorSpace::Equals(srcInfo.fColorInfo.fColorSpace,
                             dstInfo.fColorInfo.fColorSpace) && !alphaTypeConversion) {
        if (dstIsNormalized && !srcIsNormalized &&
            dstInfo.fColorInfo.fAlphaType == kPremul_SkAlphaType) {
            srcSwizzle.apply(&pipeline);
            append_clamp_gamut(&pipeline);
            dstSwizzle.apply(&pipeline);
        } else {
            GrSwizzle::Concat(srcSwizzle, dstSwizzle).apply(&pipeline);
        }
    } else {
        SkColorSpaceXformSteps steps{
                srcInfo.fColorInfo.fColorSpace, srcInfo.fColorInfo.fAlphaType,
                dstInfo.fColorInfo.fColorSpace, dstInfo.fColorInfo.fAlphaType};
        srcSwizzle.apply(&pipeline);
        steps.apply(&pipeline, srcIsNormalized);
        if (dstIsNormalized && dstInfo.fColorInfo.fAlphaType == kPremul_SkAlphaType) {
            append_clamp_gamut(&pipeline);
        }
        dstSwizzle.apply(&pipeline);
    }
    pipeline.append(store, &dstCtx);
    pipeline.run(0, 0, srcInfo.fWidth, srcInfo.fHeight);
    return true;
}