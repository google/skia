...
static void BC1_2color_compress(const SkPixmap& pixmap, SkColor otherColor, BC1Block* dest) {
...
void GrTwoColorCompress(const SkPixmap& pixmap, SkColor otherColor,
                        SkImage::CompressionType type, char* dstPixels) {
    if (SkImage::CompressionType::kETC1 == type) {
        ETC1_2color_compress(pixmap, otherColor, (ETC1Block *) dstPixels);
    } else {
        SkASSERT(type == SkImage::CompressionType::kBC1_RGB8_UNORM);
        BC1_2color_compress(pixmap, otherColor, (BC1Block *) dstPixels);
    }
}
