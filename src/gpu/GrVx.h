...
template<int N> inline vec<N> fast_angle_between_vectors(vec<N> ax, vec<N> ay, vec<N> bx,
                                                         vec<N> by) {
    vec<N> ab_cosTheta = fast_fma(ax, bx, ay*by);
    vec<N> ab_pow2 = fast_fma(ay, ay, ax*ax) * fast_fma(by, by, bx*bx);
