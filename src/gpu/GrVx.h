...
static SKVX_ALWAYS_INLINE inline float dot(float2 a, float2 b) {
    float2 ab = a*b;
    return ab[0] + ab[1];
}

static SKVX_ALWAYS_INLINE inline float cross(float2 a, float2 b) {
...
template<int N> SKVX_ALWAYS_INLINE inline vec<N> fast_madd(vec<N> f, vec<N> m, vec<N> a) {
#if FP_FAST_FMAF
...
template<int N> SKVX_ALWAYS_INLINE inline vec<N> approx_acos(vec<N> x) {
...
template<int N>
SKVX_ALWAYS_INLINE inline vec<N> approx_angle_between_vectors(vec<N> ax, vec<N> ay, vec<N> bx,
                                                              vec<N> by) {
...
template<typename T>
SKVX_ALWAYS_INLINE inline void strided_load4(const T* v, skvx::Vec<1,T>& a, skvx::Vec<1,T>& b,
...
SKVX_ALWAYS_INLINE inline typename std::enable_if<N >= 2, void>::type
...
template<> \
SKVX_ALWAYS_INLINE inline void strided_load4(const T* v, skvx::Vec<N,T>& a, skvx::Vec<N,T>& b, \
...
template<>
SKVX_ALWAYS_INLINE inline void strided_load4(const float* v, float4& a, float4& b, float4& c,
                                             float4& d) {
    using skvx::bit_pun;
...
// resorting to these methods.
template<typename T>
SKVX_ALWAYS_INLINE inline void strided_load2(const T* v, skvx::Vec<1,T>& a, skvx::Vec<1,T>& b) {
    a.val = v[0];
    b.val = v[1];
}
template<int N, typename T>
...
template<> \
SKVX_ALWAYS_INLINE inline void strided_load2(const T* v, skvx::Vec<N,T>& a, skvx::Vec<N,T>& b) { \
    auto mat = VLD(v); \
    a = skvx::bit_pun<skvx::Vec<N,T>>(mat.val[0]); \
    b = skvx::bit_pun<skvx::Vec<N,T>>(mat.val[1]); \
}
