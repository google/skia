...
// support. We call this method "fast_madd" to avoid confusion with skvx::fma, which is defined as
// always fused.
...
template<int N> vec<N> inline fast_angle_between_vectors(vec<N> ax, vec<N> ay, vec<N> bx, vec<N> by) {
    auto ab_cosTheta = fast_madd(ax, bx, ay*by);
    auto ab_pow2 = fast_madd(ax, ax, ay*ay) * fast_madd(bx, bx, by*by);
