...
static SKVX_ALWAYS_INLINE float dot(float2 a, float2 b) {
    float2 ab = a*b;
    return ab[0] + ab[1];
}

static SKVX_ALWAYS_INLINE float cross(float2 a, float2 b) {
...
template<int N> SKVX_ALWAYS_INLINE vec<N> fast_madd(vec<N> f, vec<N> m, vec<N> a) {
#if FP_FAST_FMAF
...
template<int N> SKVX_ALWAYS_INLINE vec<N> approx_acos(vec<N> x) {
...
template<int N>
SKVX_ALWAYS_INLINE vec<N> approx_angle_between_vectors(vec<N> ax, vec<N> ay, vec<N> bx, vec<N> by) {
...
template<typename T>
SKVX_ALWAYS_INLINE void strided_load4(const T* v, skvx::Vec<1,T>& a, skvx::Vec<1,T>& b,
...
SKVX_ALWAYS_INLINE typename std::enable_if<N >= 2, void>::type
...
// resorting to these methods.
template<typename T>
SKVX_ALWAYS_INLINE void strided_load2(const T* v, skvx::Vec<1,T>& a, skvx::Vec<1,T>& b) {
    a.val = v[0];
    b.val = v[1];
}
template<int N, typename T>
