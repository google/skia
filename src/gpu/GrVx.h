...
// approximate and/or have LSB differences from platform to plaftorm (e.g., by using hardware FMAs
...
// fused.
template<int N> SKVX_ALWAYS_INLINE vec<N> madd(vec<N> f, vec<N> m, vec<N> a) {
#if FP_FAST_FMAF
...
// Platform-dependent specializations.
    ...
    SKVX_ALWAYS_INLINE float4 fast_rcp(float4 x) {
        __m128 x_ = bit_pun<__m128>(x);
        return bit_pun<float4>(_mm_rcp_ps(x_));
    }
    SKVX_ALWAYS_INLINE float2 fast_rcp(float2 x) {
        return skvx::shuffle<0,1>(fast_rcp(skvx::shuffle<0,1,0,1>(x)));
    }
    #if defined(__AVX__)
        SKVX_ALWAYS_INLINE vec<8> fast_rcp(vec<8> x) {
            __m256 x_ = bit_pun<__m256>(x);
            return bit_pun<vec<8>>(_mm256_rcp_ps(x_));
        }
    #endif

    #define GRVX_FAST_RSQRT_PRECISION_BITS 11
    SKVX_ALWAYS_INLINE float4 fast_rsqrt(float4 x) {
        __m128 x_ = bit_pun<__m128>(x);
        return bit_pun<float4>(_mm_rsqrt_ps(x_));
    }
    SKVX_ALWAYS_INLINE float2 fast_rsqrt(float2 x) {
        return skvx::shuffle<0,1>(fast_rsqrt(skvx::shuffle<0,1,0,1>(x)));
    }
    #if defined(__AVX__)
        SKVX_ALWAYS_INLINE vec<8> fast_rsqrt(vec<8> x) {
            __m256 x_ = bit_pun<__m256>(x);
            return bit_pun<vec<8>>(_mm256_rsqrt_ps(x_));
        }
    #endif
#elif defined(__ARM_NEON)
    #define GRVX_FAST_RCP_PRECISION_BITS 8
    SKVX_ALWAYS_INLINE float4 fast_rcp(float4 x) {
        float32x4_t x_ = bit_pun<float32x4_t>(x);
        return bit_pun<float4>(vrecpeq_f32(x_));
    }
    SKVX_ALWAYS_INLINE float2 fast_rcp(float2 x) {
        float32x2_t x_ = bit_pun<float32x2_t>(x);
        return bit_pun<float2>(vrecpe_f32(x_));
    }

    #define GRVX_FAST_RSQRT_PRECISION_BITS 8
    SKVX_ALWAYS_INLINE float4 fast_rsqrt(float4 x) {
        float32x4_t x_ = bit_pun<float32x4_t>(x);
        return bit_pun<float4>(vrsqrteq_f32(x_));
    }
    SKVX_ALWAYS_INLINE float2 fast_rsqrt(float2 x) {
        float32x2_t x_ = bit_pun<float32x2_t>(x);
        return bit_pun<float2>(vrsqrte_f32(x_));
    }
#endif

#if GRVX_FAST_RCP_PRECISION_BITS
template<int N> SKVX_ALWAYS_INLINE vec<N> fast_rcp(vec<N> x) {
    vec<N> ret;
    ret.lo = fast_rcp(x.lo);
    ret.hi = fast_rcp(x.hi);
    return ret;
}
template<> SKVX_ALWAYS_INLINE vec<1> fast_rcp<1>(vec<1> x) {
    return fast_rcp(float2(x.val)).lo.val;
}
#endif

#if GRVX_FAST_RSQRT_PRECISION_BITS
template<int N> SKVX_ALWAYS_INLINE vec<N> fast_rsqrt(vec<N> x) {
    vec<N> ret;
    ret.lo = fast_rsqrt(x.lo);
    ret.hi = fast_rsqrt(x.hi);
    return ret;
}
template<> SKVX_ALWAYS_INLINE vec<1> fast_rsqrt<1>(vec<1> x) {
...
template<int N> vec<N> fast_angle_between_vectors(vec<N> ax, vec<N> ay, vec<N> bx, vec<N> by) {
...
SKVX_ALWAYS_INLINE float dot(float2 a, float2 b) {
    float2 ab = a*b;
    return ab[0] + ab[1];
}

SKVX_ALWAYS_INLINE float cross(float2 a, float2 b) {
