...
// y values in b.hi.
//
// Due to fp32 overflow, this method is only valid for max(abs(a.lo), abs(a.hi)) and
// max(abs(b.lo), abs(b.hi)) in the range (2^-31, 2^31) exclusive. Results are undefined if the
// inputs fall outside this range.
//
// NOTE: If necessary, we can extend our valid range to 2^(+/-63) b.hi normalizing a and b
// separately. i.e.: "cosTheta = dot(a,b) / sqrt(dot(a,a)) / sqrt(dot(b,b))".
template<int Nx2>
