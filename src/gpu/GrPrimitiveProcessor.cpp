...
void GrPrimitiveProcessor::setVertexAttributeInfo(int attributeCnt, size_t vertexStride) {
    fVertexStride = static_cast<unsigned>(vertexStride);
    fVertexAttributeCnt = static_cast<unsigned>(attributeCnt);
    SkASSERT(fVertexStride == vertexStride);
    SkASSERT(fVertexAttributeCnt == attributeCnt);
#ifdef SK_DEBUG
...
void GrPrimitiveProcessor::setInstanceAttributeInfo(int attributeCnt, size_t instanceStride) {
    fInstanceStride = static_cast<unsigned>(instanceStride);
    fInstanceAttributeCnt = static_cast<unsigned>(attributeCnt);
    SkASSERT(fInstanceStride == instanceStride);
    SkASSERT(fInstanceAttributeCnt == attributeCnt);
#ifdef SK_DEBUG
    for (int i = 0; i < attributeCnt; ++i) {
