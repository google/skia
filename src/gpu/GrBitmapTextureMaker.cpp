...
void GrBitmapTextureMaker::makeMipMappedKey(GrUniqueKey* copyKey) {
    // Destination color space is irrelevant - we always upload the bitmap's contents as-is
    if (fOriginalKey.isValid()) {
        MakeMipMappedKeyFromOriginalKey(fOriginalKey, copyKey);
    }
}

void GrBitmapTextureMaker::didCacheMipMappedCopy(const GrUniqueKey& copyKey,
