...
static void draw_texture_producer(GrContext* context,
    ...
    bool doBicubic2;
    GrSamplerState::Filter fm2 = GrSkFilterQualityToGrFilterMode(
            producer->width(), producer->height(), paint.getFilterQuality(), ctm, srcToDst,
            context->priv().options().fSharpenMipmappedTextures, &doBicubic2);
    SkDebugf("second filter quality pass: sk = %d, gr = %d, %d\n", paint.getFilterQuality(), fm2, doBicubic2);
    ...
    for (int x = 0; x <= nx; x++) {
        for (int y = 0; y <= ny; y++) {
            ...
            if (bitmap.extractSubset(&tmpB, iTileR)) {
                ...
                SkASSERT(tmpB.width() <= this->caps()->maxTextureSize() &&
                         tmpB.height() <= this->caps()->maxTextureSize());
                // We should be respecting the max tile size by the time we get here.
                SkASSERT(tmpB.width() <= this->caps()->maxTileSize() &&
                         tmpB.height() <= this->caps()->maxTileSize());
