...
static void draw_texture_producer(GrRecordingContext* context,
    ...
    if (doBicubic) {
        fp = producer->createBicubicFragmentProcessor(textureMatrix, subset, domain, sampler.wrapModeX(), sampler.wrapModeY());
    } else {
...
void SkGpuDevice::drawImageQuad(const SkImage* image, const SkRect* srcRect, const SkRect* dstRect,
    ...
    auto [fm, mm, bicubic] = GrSkFilterQualityToGrFilterMode(image->dimensions(),
