...
void SkGpuDevice::drawEdgeAAImageSet(const SkCanvas::ImageSetEntry set[], int count,
    ...
    if (!can_use_draw_texture(paint)) {
        ...
        for (int i = 0; i < count; ++i) {
            ...
            SkASSERT(set[i].fDstClipCount == 0 || (dstClips && set[i].fDstClipCount == 4));
    ...
    for (int i = 0; i < count; ++i) {
        ...
        SkASSERT(set[i].fDstClipCount == 0 || (dstClips && set[i].fDstClipCount == 4));
        const SkPoint* clip = set[i].fDstClipCount > 0 ? dstClips + dstClipIndex : nullptr;
        dstClipIndex += set[i].fDstClipCount;
