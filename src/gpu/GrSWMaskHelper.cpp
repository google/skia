...
GrSurfaceProxyView GrSWMaskHelper::MakeTexture(SkIRect bounds,
    ...
    if (auto dContext = context->asDirectContext()) {
        auto taskGroup = dContext->priv().getTaskGroup();
        sk_sp<GrSWMaskHelper> helper(new GrSWMaskHelper(bounds));
        return helper->threadedExecute(std::move(helper), taskGroup, context, fit, std::move(draw));
    } else {
...
GrSurfaceProxyView GrSWMaskHelper::threadedExecute(sk_sp<GrSWMaskHelper> spThis,
