...
sk_sp<GrFragmentProcessor> GrFragmentProcessor::MulOutputByInputUnpremulColor(
    ...
    private:
        ...
        void onComputeInvariantOutput(GrInvariantOutput* inout) const override {
            ...
            if (!inout->knownComponents().hasKnownAlpha()) {
            ...
            if (inout->knownComponents().hasZeroAlpha() ||
                childOutput.knownComponents().hasZeroAlpha()) {
                inout->mulByAlpha(0x0);
                return;
            }
            GrColorComponentFlags commonFlags = childOutput.knownComponents().knownFlags() &
                                                inout->knownComponents().knownFlags();
            GrColor c0 = GrPremulColor(inout->knownComponents().color());
            GrColor c1 = childOutput.knownComponents().color();
            GrColor color = 0x0;
...
sk_sp<GrFragmentProcessor> GrFragmentProcessor::RunInSeries(sk_sp<GrFragmentProcessor>* series,
    ...
    if (info.knownComponents().isConstant()) {
        ...
        return GrConstColorProcessor::Make(GrColor4f::FromGrColor(info.knownComponents().color()),
