...
sk_sp<GrFragmentProcessor> GrFragmentProcessor::MulOutputByInputUnpremulColor(
    ...
    private:
        ...
        void onComputeInvariantOutput(GrInvariantOutput* inout) const override {
            ...
            if (inout->knownColorComponents().hasZeroAlpha() || childOutput.knownColorComponents().hasZeroAlpha()) {
                inout->mulByAlpha(0x0);
                return;
            }
            GrColorComponentFlags commonFlags = childOutput.knownColorComponents().knownComponentFlags() & inout->knownColorComponents().knownComponentFlags();
            ...
            inout->setToOther(GrKnownColorComponents(color, commonFlags), GrInvariantOutput::kWill_ReadInput);
        }
    };
    if (!fp) {
        return nullptr;
...
sk_sp<GrFragmentProcessor> GrFragmentProcessor::RunInSeries(sk_sp<GrFragmentProcessor>* series,
    ...
    if (info.knownColorComponents().isKnownColor()) {
        ...
        return GrConstColorProcessor::Make(GrColor4f::FromGrColor(info.knownColorComponents().color()),
