...
int GrFragmentProcessor::numCoordTransforms() const {
    if (SkToBool(fFlags & kUsesLocalCoordsDirectly_Flag) && !this->isSampledWithExplicitCoords()) {
...
const GrCoordTransform& GrFragmentProcessor::coordTransform(int i) const {
    ...
    SkASSERT(i == 0);
    SkASSERT(SkToBool(fFlags & kUsesLocalCoordsDirectly_Flag) &&
...
void GrFragmentProcessor::addAndPushFlagToChildren(PrivateFlags flag) {
    // This propagates down, so if we've already been marked it, all our children should have it too
    if (!(fFlags & flag)) {
        fFlags |= flag;
...
int GrFragmentProcessor::registerChild(std::unique_ptr<GrFragmentProcessor> child,
    ...
    // If the child is sampled with explicit coordinates and there isn't any other direct reference
    // to the coords in this FP (explicit or from a variable sample matrix), we halt the upwards
    // propagation of this state because it means this FP is fully determining local coordinates for
    // the child FP. Otherwise, we track the indirect use up so that we can easily detect when
    // we need the primitive processor to prepare local coordinates.
