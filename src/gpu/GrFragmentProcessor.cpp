...
void GrFragmentProcessor::setSampleMatrix(SkSL::SampleMatrix matrix) {
    ...
    SkASSERT(fMatrix.fFlags != SkSL::SampleMatrix::kVariable_Flag);
    if ((fMatrix.fFlags == SkSL::SampleMatrix::kVertex_Flag ||
         fMatrix.fFlags == SkSL::SampleMatrix::kFragment_Flag) &&
        (matrix.fFlags != SkSL::SampleMatrix::kFragment_Flag ||
         fMatrix.fExpression != matrix.fExpression)) {
        SkASSERT(fMatrix.fExpression != "");
        fMatrix = SkSL::SampleMatrix(SkSL::SampleMatrix::kVariable_Flag |
                                     SkSL::SampleMatrix::kFragment_Flag,
                                     fMatrix.fOwner, fMatrix.fExpression);
    } else {
        fMatrix = matrix;
    }
