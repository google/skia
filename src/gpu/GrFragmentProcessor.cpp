...
std::unique_ptr<GrFragmentProcessor> GrFragmentProcessor::ColorMatrix(
    ...
    static auto effect = SkMakeRuntimeEffect(SkRuntimeEffect::MakeForColorFilter, R"(
        uniform half4x4 m;
        uniform half4   v;
        uniform int unpremulInput;
        uniform int clampRGBOutput;
        uniform int premulOutput;
    ...
    auto specializeBool = [](bool b) { return GrSkSLFP::Specialize(b ? 1 : 0); };
    ...
    return GrSkSLFP::Make(effect, "ColorMatrix", std::move(child), GrSkSLFP::OptFlags::kNone,
                          "m", m44,
                          "v", v4,
                          "unpremulInput", specializeBool(unpremulInput),
                          "clampRGBOutput", specializeBool(clampRGBOutput),
                          "premulOutput", specializeBool(premulOutput));
}
