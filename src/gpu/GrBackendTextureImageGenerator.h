 ...
 * in another GrContext. It holds onto a semaphore which the procuding Grontext will signal and the
 * consuming GrContext will wait on before using the texture. Only one GrContext can ever be used
 * as a consumer (this is mostly cause of how Vulkan can't allow multiple things to wait on the same
 * semaphore).
 */
