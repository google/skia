...
public:
    using Iter = IterImpl<false, false>;
    using RIter = IterImpl<false, true>;
    using CIter = IterImpl<true, false>;
    using CRIter = IterImpl<true, true>;
    ...
    CIter begin() const { return CIter(fHead); }
    CIter end() const { return CIter(nullptr); }

    /** Reverse mutable iteration */
    RIter rbegin() { return RIter(fTail); }
    RIter rend() { return RIter(nullptr); }

    /** Reverse const iteration */
    CRIter rbegin() const { return CRIter(fTail); }
    CRIter rend() const { return CRIter(nullptr); }
