...
private:
    template <typename T> class IterImpl;

public:
    using iterator = IterImpl<TBase>;
    using const_iterator = IterImpl<const TBase>;
    ...
    TBase& back() {
        SkASSERT(!this->empty());
        return *reinterpret_cast<TBase*>(fTail + 1);
    }
...
GrTRecorder<TBase>::emplaceWithData(size_t extraDataSize, Args... args) {
    ...
    static constexpr size_t kFootAlign = alignof(Header);
...
/**
 * Iterates through a recorder front-to-back. T should be TBase or const TBase.
 */
template <typename TBase> template <typename T> class GrTRecorder<TBase>::IterImpl {
public:
    IterImpl() = default;
    ...
    T& operator*() const { return *reinterpret_cast<T*>(fCurr + 1); }
    T* operator->() const { return reinterpret_cast<T*>(fCurr + 1); }
