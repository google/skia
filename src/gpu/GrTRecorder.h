...
private:
    template <typename T, bool IsReverse> class IterImpl;

public:
    using iterator = IterImpl<TBase, false>;
    using reverse_iterator = IterImpl<TBase, true>;
    using const_iterator = IterImpl<const TBase, false>;
    using const_reverse_iterator = IterImpl<const TBase, true>;
 ...
 */
template <typename TBase> template <typename T, bool IsReverse> class GrTRecorder<TBase>::IterImpl {
private:
    static Footer* Next(Footer* curr) { return IsReverse ? curr->fPrev : curr->fNext; }
    static Footer* Prev(Footer* curr) { return IsReverse ? curr->fNext : curr->fPrev; }
...
public:
    ...
    T& operator*() const { return *fCurr->fTBase; }
    T* operator->() const { return fCurr->fTBase; }
