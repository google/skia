...
std::unique_ptr<GrSurfaceFillContext> GrSurfaceFillContext::Make(GrRecordingContext* context,
    ...
    auto lrtc = std::make_unique<GrSurfaceFillContext>(context,
                                                       std::move(readView),
                                                       std::move(writeView),
                                                       info.colorInfo());
    lrtc->discard();
...
void GrSurfaceFillContext::fillIRectSrcMode(const SkIRect& dstRect,
                                            std::unique_ptr<GrFragmentProcessor> fp) {
    ASSERT_SINGLE_OWNER
    RETURN_IF_ABANDONED
    SkDEBUGCODE(this->validate();)
    GR_CREATE_TRACE_MARKER_CONTEXT("GrSurfaceFillContext", "fillIRectSrcMode", fContext);
...
void GrSurfaceFillContext::addDrawOp(GrOp::Owner owner) {
    ...
    SkRect bounds;
    AdjustOpBounds(&bounds, owner.get());
...
void GrSurfaceFillContext::AdjustOpBounds(SkRect* bounds, const GrOp* op) {
    *bounds = op->bounds();
    if (op->hasZeroArea()) {
        if (op->hasAABloat()) {
