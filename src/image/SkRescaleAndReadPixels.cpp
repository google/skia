...
void SkRescaleAndReadPixels(SkBitmap bmp,
    ...
    if (stepsX < 0 || stepsY < 0) {
        ...
        if (rescaling != SkRescalingMode::kNearest) {
            rescaling = SkRescalingMode::kRepeatedLinear;
        }
    }

    auto rescaling_to_sampling = [](SkRescalingMode rescaling) {
        SkSamplingOptions sampling;
        if (rescaling == SkRescalingMode::kRepeatedLinear) {
            sampling = SkSamplingOptions(SkFilterMode::kLinear, SkMipmapMode::kNone);
        } else if (rescaling == SkRescalingMode::kRepeatedCubic) {
            sampling = SkSamplingOptions({1.0f/3, 1.0f/3});
        }
        return sampling;
    };
    SkSamplingOptions sampling = rescaling_to_sampling(rescaling);
