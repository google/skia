...
SkImage_GpuYUV::SkImage_GpuYUV(sk_sp<GrContext> context, uint32_t uniqueID, SkAlphaType at,
                               SkYUVColorSpace colorSpace, sk_sp<GrTextureProxy> proxies[4],
...
sk_sp<GrRenderTargetContext> SkImage_GpuYUV::flatten() const {
...
sk_sp<GrTextureProxy> SkImage_GpuYUV::asTextureProxyRef() const {
    sk_sp<GrRenderTargetContext> rtc = this->flatten();
...
bool SkImage_GpuYUV::onReadPixels(const SkImageInfo& dstInfo, void* dstPixels, size_t dstRB,
    ...
    sk_sp<GrRenderTargetContext> rtc = this->flatten();
    sk_sp<GrSurfaceProxy> proxy = rtc->asSurfaceProxyRef();
