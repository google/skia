...
sk_sp<SkImage> SkImage_Gpu::onMakeColorTypeAndColorSpace(GrRecordingContext* context,
    ...
    auto colorFP = GrTextureEffect::Make(*this->view(context), this->alphaType());
    colorFP = GrColorSpaceXformEffect::Make(std::move(colorFP), this->colorSpace(),
                                            this->alphaType(), targetCS.get(), this->alphaType());
    SkASSERT(colorFP);
