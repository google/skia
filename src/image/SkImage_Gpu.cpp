...
sk_sp<SkImage> SkImage_Gpu::MakeFromYUVATexturesCopyImpl(GrContext* ctx,
    ...
    bool nv12 = (yuvaIndices[1].index == yuvaIndices[2].index);
    auto ct = nv12 ? kRGBA_8888_SkColorType : kAlpha_8_SkColorType;

    for (int i = 0; i < 4; ++i) {
        // Validate that the yuvaIndices is referred to valid backend texture.
        SkYUVAIndex& yuvaIndex = yuvaIndices[i];
        if (i == 3 && yuvaIndex.index == -1) {
            // Meaning the A plane isn't passed in.
            continue;
        }
        if (yuvaIndex.index == -1) {
            // Y plane, U plane, and V plane must refer to texture being passed in.
            return nullptr;
        }
        auto texture = yuvaTextures[yuvaIndex.index];
    ...
    auto yTexture = yuvaTextures[yuvaIndices[0].index];
    auto uTexture = yuvaTextures[yuvaIndices[1].index];
    auto vTexture = yuvaTextures[yuvaIndices[2].index];
    sk_sp<GrTextureProxy> yProxy = proxyProvider->wrapBackendTexture(yTexture, origin);
    sk_sp<GrTextureProxy> uProxy = proxyProvider->wrapBackendTexture(uTexture, origin);
    sk_sp<GrTextureProxy> vProxy = proxyProvider->wrapBackendTexture(vTexture, origin);
    ...
    SkISize sizes[] = {{yTexture.width(), yTexture.height()},
                       {uTexture.width(), uTexture.height()},
                       {vTexture.width(), vTexture.height()}};
    // TODO: Modify the fragment process to 1. take a single size, 2. sample from different channel
...
sk_sp<SkImage> SkImage::MakeFromYUVTexturesCopy(GrContext* ctx, SkYUVColorSpace colorSpace,
    ...
    SkYUVAIndex yuvaIndices[4] = {SkYUVAIndex{0, 0}, SkYUVAIndex{1, 0}, SkYUVAIndex{2, 0},
                                  SkYUVAIndex{-1, 0}};
...
sk_sp<SkImage> SkImage::MakeFromNV12TexturesCopy(GrContext* ctx, SkYUVColorSpace colorSpace,
                                                 ...
                                                 GrSurfaceOrigin origin,
                                                 sk_sp<SkColorSpace> imageColorSpace) {
    SkYUVAIndex yuvaIndices[4] = {SkYUVAIndex{0, 0}, SkYUVAIndex{1, 0}, SkYUVAIndex{1, 0},
                                  SkYUVAIndex{-1, 0}};
