...
size_t SkImage::getDeferredTextureImageData(const GrContextThreadSafeProxy& proxy,
    ...
    SkImageInfo info;
    size_t pixelSize = 0;
    if (!isScaled && pixmap.info().colorType() == params[0].fColorType && this->peekPixels(&pixmap) && !pixmap.ctable()) {
        info = pixmap.info();
        ...
        if (fillMode) {
            ...
            if (decodeInfo.colorType() != info.colorType()) {
                pixmap.alloc(info);
                // Convert and dither the decoded pixmap to the target pixmap color type.
                decodePixmap.readPixels(pixmap.info(), pixmap.writable_addr(), pixmap.rowBytes(), 0, 0);
            } else {
