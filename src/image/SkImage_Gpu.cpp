...
sk_sp<GrTextureProxy> SkImage_Gpu::asTextureProxyRef(GrContext* context,
    ...
    return this->asTextureRef(context, params, dstColorSpace, texColorSpace, scaleAdjust);
}
...
sk_sp<GrTextureProxy> SkImage_Gpu::asTextureRef(GrContext* ctx, const GrSamplerParams& params,
    ...
    GrTextureAdjuster adjuster(ctx, fProxy, this->alphaType(), this->bounds(),
...
sk_sp<SkImage> SkImage::MakeTextureFromPixmap(GrContext* ctx,
                                              const SkPixmap& pixmap,
                                              SkBudgeted budgeted) {
    if (!ctx) {
        return nullptr;
    }
    sk_sp<GrTextureProxy> proxy(GrUploadPixmapToTextureProxy(ctx->resourceProvider(),
                                                             pixmap, budgeted));
    if (!proxy) {
        return nullptr;
    }
