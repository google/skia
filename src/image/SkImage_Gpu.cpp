...
sk_sp<SkImage> SkImage_Gpu::MakePromiseTexture(sk_sp<GrContextThreadSafeProxy> tsp,
    ...
    GrColorType grColorType = SkColorTypeAndFormatToGrColorType(tsp->priv().caps(),
                                                                colorType,
                                                                backendFormat);
    if (GrColorType::kUnknown == grColorType) {
        return nullptr;
    ...
    if (!tsp->priv().caps()->areColorTypeAndFormatCompatible(grColorType, backendFormat)) {
        return nullptr;
    }

    auto proxy = MakePromiseImageLazyProxy(tsp.get(),
                                           dimensions,
                                           backendFormat,
                                           mipMapped,
                                           textureFulfillProc,
    ...
    GrSwizzle swizzle = tsp->priv().caps()->getReadSwizzle(backendFormat, grColorType);
    GrSurfaceProxyView view(std::move(proxy), origin, swizzle);
    return sk_make_sp<SkImage_Gpu>(GrImageContextPriv::MakeForPromiseImage(std::move(tsp)),
                                   kNeedNewImageUniqueID, std::move(view), colorType,
