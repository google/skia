...
sk_sp<SkImage> SkImage_Gpu::MakePromiseYUVATexture(GrContext* context,
    ...
    SkISize imageSize = SkISize::Make(imageWidth, imageHeight);
    ...
    sk_sp<GrTextureProxy> proxy = proxyProvider->createLazyProxy(
            [=](GrResourceProvider* resourceProvider) mutable {
...
#if 1
                // For the time being, simple return the Y-plane. The reason for this is that
                ...
                return promiseHelpers[0].getTexture(resourceProvider, configs[0]);
#else
