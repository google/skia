...
static sk_sp<SkImage> make_from_yuv_textures_copy(GrContext* ctx, SkYUVColorSpace colorSpace,
    ...
    if (yuvBackendTextures[0].backend() != backend || yuvBackendTextures[1].backend() != backend) {
        return nullptr;
    }
    if (!nv12 && yuvBackendTextures[2].backend() != backend) {
        return nullptr;
    }

    sk_sp<GrTextureProxy> yProxy = GrSurfaceProxy::MakeWrappedBackend(ctx, yuvBackendTextures[0], origin);
    sk_sp<GrTextureProxy> uProxy = GrSurfaceProxy::MakeWrappedBackend(ctx, yuvBackendTextures[1], origin);
...
sk_sp<SkImage> SkImage::MakeFromYUVTexturesCopy1(GrContext* ctx, SkYUVColorSpace colorSpace,
...
sk_sp<SkImage> SkImage::MakeFromNV12TexturesCopy1(GrContext* ctx, SkYUVColorSpace colorSpace,
...
sk_sp<SkImage> SkImage::MakeFromYUVTexturesCopy2(GrContext* ctx, SkYUVColorSpace colorSpace,
...
sk_sp<SkImage> SkImage::MakeFromNV12TexturesCopy2(GrContext* ctx, SkYUVColorSpace colorSpace,
