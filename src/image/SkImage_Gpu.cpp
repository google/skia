...
sk_sp<SkImage> SkImage_Gpu::MakePromiseYUVATexture(GrContext* context,
    ...
    // DDL TODO: we need to create a SkImage_GpuYUVA here! This implementation creates a
    // promise image that draws the planes into a single texture.
    if (!context) {
        return nullptr;
    }
    ...
    sk_sp<GrTextureProxy> proxy = proxyProvider->createLazyProxy(
            [&](GrResourceProvider* resourceProvider) mutable {
                ...
                return sk_sp<GrTexture>(tmp->getTexture());
            },
