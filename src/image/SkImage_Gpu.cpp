...
SkImage_Gpu::SkImage_Gpu(GrContext* context, sk_sp<GrContextThreadSafeProxy> threadSafeProxy,
                         uint32_t uniqueID, SkAlphaType at,
    ...
    if (fContext1 && !fContext1->contextPriv().resourceProvider()) {
        fContext1 = nullptr;
    }
}
...
bool SkImage_Gpu::getROPixels(SkBitmap* dst, SkColorSpace*, CachingHint chint) const {
    ...
    sk_sp<GrSurfaceContext> sContext = fContext1->contextPriv().makeWrappedSurfaceContext(
                                                                                    fProxy,
                                                                                    fColorSpace);
    if (!sContext) {
        return false;
...
sk_sp<GrTextureProxy> SkImage_Gpu::asTextureProxyRef(GrContext* context,
    ...
    if (!fThreadSafeProxy->matches(context)) {
        SkASSERT(0);
        return nullptr;
    }
    ...
    GrTextureAdjuster adjuster(context, fProxy, this->alphaType(), this->uniqueID(),
...
GrBackendObject SkImage_Gpu::onGetTextureHandle(bool flushPendingGrContextIO,
    ...
    if (!fContext1 && !fProxy->priv().isInstantiated()) {
    ...
    if (GrSurfaceProxy::LazyState::kNot != fProxy->lazyInstantiationState()) {
        SkASSERT(fContext1->contextPriv().resourceProvider());
        fProxy->priv().doLazyInstantiation(fContext1->contextPriv().resourceProvider());
    ...
    if (!fProxy->instantiate(fContext1->contextPriv().resourceProvider())) {
        return 0;
    }
    ...
    if (texture) {
        if (flushPendingGrContextIO) {
            fContext1->contextPriv().prepareSurfaceForExternalIO(fProxy.get());
        }
        if (origin) {
...
GrTexture* SkImage_Gpu::onGetTexture() const {
    ...
    if (!proxy->instantiate(fContext1->contextPriv().resourceProvider())) {
        return nullptr;
    }
...
sk_sp<SkImage> SkImage_Gpu::onMakeSubset(const SkIRect& subset) const {
    ...
    sk_sp<GrSurfaceContext> sContext(fContext1->contextPriv().makeDeferredSurfaceContext(
    ...
    return sk_make_sp<SkImage_Gpu>(fContext1, fContext1->threadSafeProxy(), kNeedNewImageUniqueID,
...
static sk_sp<SkImage> new_wrapped_texture_common(GrContext* ctx,
    ...
    return sk_make_sp<SkImage_Gpu>(ctx, ctx->threadSafeProxy(), kNeedNewImageUniqueID,
...
sk_sp<SkImage> SkImage_Gpu::MakeFromYUVTexturesCopyImpl(
    ...
    return sk_make_sp<SkImage_Gpu>(ctx, ctx->threadSafeProxy(),
...
static sk_sp<SkImage> create_image_from_maker(GrContext* context, GrTextureMaker* maker,
    ...
    return sk_make_sp<SkImage_Gpu>(context, context->threadSafeProxy(), id, at,
...
sk_sp<SkImage> SkImage::MakePromiseTexture(sk_sp<GrContextThreadSafeProxy> threadSafeProxy,
    ...
    return sk_make_sp<SkImage_Gpu>(nullptr, std::move(threadSafeProxy),
                                   kNeedNewImageUniqueID, alphaType, std::move(proxy),
...
bool SkImage_Gpu::onIsValid(GrContext* context) const {
    ...
    if (fContext1 && fContext1->contextPriv().abandoned()) {
        return false;
    }

    if (context) {
        if (fContext1 && context != fContext1) {
            return false;
        }
