...
std::unique_ptr<SkCrossContextImageData> SkCrossContextImageData::MakeFromEncoded(
    ...
    std::unique_ptr<GrCrossContextTextureData> cctd = texture->detachBackendTexture();

    return skstd::make_unique<SkCrossContextImageData>(desc, std::move(cctd), info.alphaType(),
                                                       info.refColorSpace());
}
