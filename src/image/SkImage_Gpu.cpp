...
sk_sp<SkImage> SkImage::MakeFromNV12TexturesCopy(GrContext* ctx, SkYUVColorSpace colorSpace,
                                                 const GrBackendTexture yuvTextures[2],
                                                 GrSurfaceOrigin origin,
                                                 sk_sp<SkColorSpace> imageColorSpace) {
    return SkImage_Gpu::MakeFromYUVTexturesCopyImpl(ctx, colorSpace, true, yuvTextures,
                                                    origin, std::move(imageColorSpace));
}
...
sk_sp<SkImage> SkImage::MakeFromNV12TexturesCopy(GrContext* ctx, SkYUVColorSpace colorSpace,
    ...
    return SkImage_Gpu::MakeFromNV12TexturesCopy(ctx, colorSpace, yuvTextures, origin,
                                                 std::move(imageColorSpace));
}
