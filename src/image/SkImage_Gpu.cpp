...
sk_sp<SkImage> SkImage_Gpu::onMakeColorTypeAndColorSpace(SkColorType targetCT,
    ...
    // We reset info's CT because we called MakeWithFallback.
    info.makeColorType(GrColorTypeToSkColorType(surfaceFillContext->colorInfo().colorType()));

    // Draw this image's texture unto the SFC.
