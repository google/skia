...
SkImage_Gpu::SkImage_Gpu(sk_sp<GrContext> context, uint32_t uniqueID, SkColorType ct, SkAlphaType at,
                         sk_sp<SkColorSpace> colorSpace, sk_sp<GrTextureProxy> proxy)
...
static sk_sp<SkImage> new_wrapped_texture_common(GrContext* ctx,
    ...
    return sk_make_sp<SkImage_Gpu>(sk_ref_sp(ctx), kNeedNewImageUniqueID,
            GrColorTypeToSkColorType(colorType), at, std::move(colorSpace), std::move(proxy));
}
