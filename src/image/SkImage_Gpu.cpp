...
static sk_sp<SkImage> make_from_yuv_textures_copy(GrContext* ctx, SkYUVColorSpace colorSpace,
    ...
    sk_sp<GrTexture> yTex1(
        ctx->textureProvider()->wrapBackendTexture(yDesc, kBorrow_GrWrapOwnership));
    sk_sp<GrSurfaceProxy> yProxy = GrSurfaceProxy::MakeWrapped(std::move(yTex1));

    sk_sp<GrTexture> uTex1(
        ctx->textureProvider()->wrapBackendTexture(uDesc, kBorrow_GrWrapOwnership));
    sk_sp<GrSurfaceProxy> uProxy = GrSurfaceProxy::MakeWrapped(std::move(uTex1));
    ...
    paint.addColorFragmentProcessor(
        GrYUVEffect::MakeYUVToRGB(ctx->textureProvider(), yProxy.get(), uProxy.get(), vProxy.get(),
