...
size_t SkImage::getDeferredTextureImageData(const GrContextThreadSafeProxy& proxy,
    ...
    if (!isScaled && this->peekPixels(&pixmap) && pixmap.info().colorType() == dstColorType) {
        info = pixmap.info();
        pixelSize = SkAlign8(pixmap.getSize());
        if (!dstColorSpace) {
    ...
    char* mipLevelPtr = pixelsAsCharPtr + SkAlign8(pixmap.getSize());
    ...
    if (useMipMaps) {
        ...
        for (int generatedMipLevelIndex = 0; generatedMipLevelIndex < mipMapLevelCount - 1;
            ...
            SkASSERT(mipLevelPtr + mipLevel.fPixmap.getSize() <=
                     bufferAsCharPtr + pixelOffset + pixelSize);

            memcpy(mipLevelPtr, mipLevel.fPixmap.addr(), mipLevel.fPixmap.getSize());
            ...
            mipLevelPtr += SkAlign8(mipLevel.fPixmap.getSize());
        }
    }
    return size;
}
