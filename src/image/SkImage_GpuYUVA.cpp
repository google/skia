...
SkImage_GpuYUV::SkImage_GpuYUV(sk_sp<GrContext> context, uint32_t uniqueID,
    ...
    for (int i = 0; i < 4; ++i) {
        ...
        fYUVAIndices[i] = yuvaIndices[i];
        if (kA_SkImageSourceChannel == yuvaIndices[i].fChannel) {
...
sk_sp<GrTextureProxy> SkImage_GpuYUV::asTextureProxyRef() const {
    if (!fRGBProxy) {
        ...
        // TODO: if needed, convert to premul alpha somewhere in here?
        // if (kPremul_AlphaType == fImageAlphaType) {
        //     ?
        // }
...
sk_sp<SkImage> SkImage_GpuYUV::MakeFromYUVATextures(GrContext* ctx,
    ...
    for (int i = 0; i < 4; ++i) {
        ...
        if (kA_SkImageSourceChannel == yuvaIndices[i].fChannel && yuvaIndex.fIndex == -1) {
    ...
    for (int i = 0; i < 4; ++i) {
        ...
        if (textureIndex == -1) {
            SkASSERT(kA_SkImageSourceChannel == yuvaIndices[i].fChannel);
            continue;
        }
    ...
    return sk_make_sp<SkImage_GpuYUV>(sk_ref_sp(ctx), kNeedNewImageUniqueID, colorSpace,
