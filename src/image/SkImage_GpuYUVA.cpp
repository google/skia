...
sk_sp<SkImage> SkImage_GpuYUVA::MakePromiseYUVATexture(
    ...
    // Our contract is that we will always call the release proc even on failure. If we can simplify
    // MakePromiseImageLazyProxy such that it takes a GrRefCntedCallback then we could make them
    // here and move them in.
