...
SkImage_GpuYUVA::SkImage_GpuYUVA(sk_sp<GrImageContext> context,
        ...
        : INHERITED(std::move(context),
                    ...
                    uniqueID,
                    kAssumedColorType,
                    // If an alpha channel is present we always use to kPremul. This is because,
...
SkImage_GpuYUVA::SkImage_GpuYUVA(sk_sp<GrImageContext> context,
    ...
    SkASSERT(fYUVAProxies.isValid() != image->fRGBView);
}
...
bool SkImage_GpuYUVA::setupMipmapsForPlanes(GrRecordingContext* context) const {
    ...
    SkASSERT(fYUVAProxies.isValid() && !fRGBView.proxy());
...
sk_sp<SkImage> SkImage::MakeFromYUVATextures(GrRecordingContext* context,
    ...
    GrYUVATextureProxies yuvaProxies =
            SkImage_GpuBase::MakeYUVAProxies(context, yuvaTextures, std::move(releaseHelper));
...
sk_sp<SkImage> SkImage::MakeFromYUVAPixmaps(GrRecordingContext* context,
    ...
    // Resize the pixamps if necessary.
...
sk_sp<SkImage> SkImage_GpuYUVA::MakePromiseYUVATexture(
    ...
    GrYUVATextureProxies yuvaTextureProxies(
            yuvaBackendTextureInfo.yuvaInfo(), proxies, yuvaBackendTextureInfo.textureOrigin());
