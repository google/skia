...
bool SkImage_GpuYUVA::setupMipmapsForPlanes() const {
    for (int i = 0; i < fNumProxies; ++i) {
        ...
        if (mipCount && GrGpu::IsACopyNeededForMips(fContext1->contextPriv().caps(),
                                                    fProxies[i].get(),
                                                    GrSamplerState::Filter::kMipMap,
                                                    &copyParams)) {
            auto mippedProxy = GrCopyBaseMipMapToTextureProxy(fContext1.get(), fProxies[i].get());
            if (!mippedProxy) {
                return false;
            }
...
sk_sp<GrTextureProxy> SkImage_GpuYUVA::asTextureProxyRef() const {
    if (!fRGBProxy) {
        const GrBackendFormat format =
            fContext1->contextPriv().caps()->getBackendFormatFromColorType(kRGBA_8888_SkColorType);

        // Needs to create a render target in order to draw to it for the yuv->rgb conversion.
        sk_sp<GrRenderTargetContext> renderTargetContext(
            fContext1->contextPriv().makeDeferredRenderTargetContext(
        ...
        if (!RenderYUVAToRGBA(fContext1.get(), renderTargetContext.get(), rect, fYUVColorSpace,
...
sk_sp<GrTextureProxy> SkImage_GpuYUVA::asMippedTextureProxyRef() const {
    ...
    if (auto mippedProxy = GrCopyBaseMipMapToTextureProxy(fContext1.get(), proxy.get())) {
...
/////////////////////////////////////////////////////////////////////////////////////////////////
sk_sp<SkImage> SkImage_GpuYUVA::MakePromiseYUVATexture(const GrCaps* caps,
    ...
    for (int texIdx = 0; texIdx < numTextures; ++texIdx) {
        GrPixelConfig config = caps->getYUVAConfigFromBackendFormat(yuvaFormats[texIdx]);
