...
void SkSurface::asyncRescaleAndReadPixels(const SkImageInfo& info, const SkIRect& srcRect,
    ...
    if (rescaleLinear && !this->getCanvas()->imageInfo().colorSpace()->gammaIsLinear()) {
        auto cs = this->getCanvas()->imageInfo().colorSpace()->makeLinearGamma();
        auto ii = this->getCanvas()->imageInfo().makeColorSpace(std::move(cs));
        ii = ii.makeWH(srcW, srcH);
        ii = ii.makeColorType(kRGBA_F16_SkColorType);
