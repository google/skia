...
sk_sp<GrTextureProxy> SkImage_Raster::asTextureProxyRef(GrContext* context,
    ...
    sk_sp<GrTextureProxy> tex = fPinnedTexture.asTextureProxyRef(context, fBitmap, params,
                                                                 scaleAdjust);
    if (tex) {
        return tex;
    }
...
bool SkImage_Raster::onPinAsTexture(GrContext* ctx) const {
    return fPinnedTexture.pinAsTexture(ctx, fBitmap);
}

void SkImage_Raster::onUnpinAsTexture(GrContext* ctx) const {
    fPinnedTexture.unpinAsTexture(ctx);
}
#endif
