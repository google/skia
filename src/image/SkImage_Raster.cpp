...
sk_sp<SkImage> SkMakeImageFromRasterBitmapPriv(const SkBitmap& bm, SkCopyPixelsMode cpm,
    ...
    if (kAlways_SkCopyPixelsMode == cpm || (!bm.isImmutable() && kNever_SkCopyPixelsMode != cpm)) {
        ...
        if (bm.peekPixels(&pmap)) {
            // If our bitmap is a subset, then MakeRasterCopyPriv won't do the right thing (we lose
            // the origin). Android is the only client of this API, and doesn't make subset bitmaps.
            SkASSERT(is_not_subset(bm));
