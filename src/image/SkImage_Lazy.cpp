...
sk_sp<GrTextureProxy> SkImage_Lazy::lockTextureProxy(GrContext* ctx,
    ...
    enum LockTexturePath {
        ...
        kPreExistingMipped_LockTexturePath,
    ...
    // 4. Ask the generator to return RGB(A) data, which the GPU can convert
    SkBitmap bitmap;
    // If we have a proxy already than one of the previous attempts already generated the base layer
    // which we'll copy into mip proxy later.
