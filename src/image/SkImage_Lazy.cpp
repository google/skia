...
private:
    ...
    /**
     *  If info has a color space, then return kRespect, to indicate that we in color correct
     *  rendering mode. If info does not have a color space, then set its color space to
     *  fColorSpace and return kIgnore.
     */
    SkTransferFunctionBehavior getTrFnBehaviorAndUpdateInfo(SkImageInfo* info) const;

    sk_sp<SharedGenerator> fSharedGenerator;
    const SkImageInfo      fInfo;
    const SkIPoint         fOrigin;
    // This color space is assigned by onMakeColorSpace.
    sk_sp<SkColorSpace> fColorSpace;
...
SkImage_Lazy::SkImage_Lazy(Validator* validator)
        ...
        , fColorSpace(validator->fColorSpace) {
...
SkTransferFunctionBehavior SkImage_Lazy::getTrFnBehaviorAndUpdateInfo(SkImageInfo* info) const {
    if (info->colorSpace()) {
        return SkTransferFunctionBehavior::kRespect;
    }
    if (fColorSpace) {
        *info = info->makeColorSpace(fColorSpace);
    }
...
static bool generate_pixels(SkImageGenerator* gen, const SkPixmap& pmap, int originX, int originY,
    ...
    SkImageGenerator::Options opts;
    opts.fBehavior = behavior;
    if (!gen->getPixels(pmap.info(), dstPM->writable_addr(), dstPM->rowBytes(), &opts)) {
        return false;
    }
...
bool SkImage_Lazy::onReadPixels(const SkImageInfo& dstInfo, void* dstPixels, size_t dstRB,
    ...
    SkImageInfo genInfo = dstInfo;
    SkTransferFunctionBehavior behavior = SkTransferFunctionBehavior::kRespect;
    if (!dstColorSpace) {
        genInfo = dstInfo.makeColorSpace(fColorSpace);
    ...
    if (kDisallow_CachingHint == chint) {
        if (this->lockAsBitmapOnlyIfAlreadyCached(&bm, cacheFormat)) {
            ...
            if (this->directGeneratePixels(genInfo, dstPixels, dstRB, srcX, srcY, behavior)) {
                return true;
            }
            // else fall through
        }
    ...
    if (this->lockAsBitmap(&bm, chint, cacheFormat, genInfo, behavior)) {
        return true;
    }
    return false;
}
...
bool SkImage_Lazy::getROPixels(SkBitmap* bitmap, SkColorSpace* dstColorSpace,
    ...
    CachedFormat cacheFormat = this->chooseCacheFormat(dstColorSpace);
    SkImageInfo info = this->buildCacheInfo(cacheFormat);
    SkTransferFunctionBehavior behavior = this->getTrFnBehaviorAndUpdateInfo(&info);
    return this->lockAsBitmap(bitmap, chint, cacheFormat, info, behavior);
}
...
sk_sp<SkImage> SkImage_Lazy::onMakeSubset(const SkIRect& subset) const {
    ...
    Validator validator(fSharedGenerator, &generatorSubset, fColorSpace);
...
sk_sp<SkImage> SkImage_Lazy::onMakeColorSpace(sk_sp<SkColorSpace> target,
    ...
    if (SkColorSpace::Equals(target.get(), fColorSpace.get())) {
        return sk_ref_sp(const_cast<SkImage*>(reinterpret_cast<const SkImage*>(this)));
    }
...
sk_sp<GrTextureProxy> SkImage_Lazy::lockTextureProxy(GrContext* ctx,
    ...
    SkImageInfo surfaceInfo = this->buildCacheInfo(format);

    // In legacy mode, use the color space from xform canvases when generating pixels.
    SkImageInfo genInfo = surfaceInfo;
    SkTransferFunctionBehavior behavior = this->getTrFnBehaviorAndUpdateInfo(&genInfo);
    ...
    // 2. Ask the generator to natively create one
    {
        ...
        if (sk_sp<GrTextureProxy> proxy =
                    generator->generateTexture(ctx, genInfo, fOrigin, behavior)) {
    ...
    if (!ctx->contextPriv().disableGpuYUVConversion()) {
        // TODO(brianosman): Don't ignore the legacy target color space here.
        const GrSurfaceDesc desc = GrImageInfoToSurfaceDesc(surfaceInfo, *ctx->caps());
        ScopedGenerator generator(fSharedGenerator);
    ...
    // 4. Ask the generator to return RGB(A) data, which the GPU can convert
    SkBitmap bitmap;
    if (this->lockAsBitmap(&bitmap, chint, format, genInfo, behavior)) {
