...
GrSurfaceProxyView SkImage_Lazy::refView(GrRecordingContext* context, GrMipMapped mipMapped) const {
    ...
    GrImageTextureMaker textureMaker(context, this, GrImageCachePolicy::kCached_Budgeted);
...
GrSurfaceProxyView SkImage_Lazy::lockTextureProxyView(GrRecordingContext* ctx,
    ...
    {
        ScopedGenerator generator(fSharedGenerator);
        // We assume that callers that want uncached textures also want to exclusively own them.
        bool forceCopy = cachePolicy != GrImageCachePolicy::kCached_Budgeted;
    ...
    if (mipMapped == GrMipMapped::kNo && !ctx->priv().options().fDisableGpuYUVConversion) {
        ...
        SkBudgeted budgeted = cachePolicy == GrImageCachePolicy::kUncached_Unbudgeted
                                      ? SkBudgeted::kNo
                                      : SkBudgeted::kYes;
    ...
    auto hint = cachePolicy == GrImageCachePolicy::kCached_Budgeted
                        ? CachingHint::kAllow_CachingHint
                        : CachingHint::kDisallow_CachingHint;
    if (SkBitmap bitmap; this->getROPixels(&bitmap, hint)) {
        // We always say uncached here because we will cache it external to the maker based on
        // our cache policy. We're just using the maker to generate the texture.
        auto makerPolicy = cachePolicy == GrImageCachePolicy::kUncached_Unbudgeted
                                   ? GrImageCachePolicy::kUncached_Unbudgeted
                                   : GrImageCachePolicy::kUncached_Budgeted;
