...
SkImageInfo SkImage_Lazy::buildCacheInfo(CachedFormat format) const {
    switch (format) {
        case kLegacy_CachedFormat:
            return fInfo.makeColorSpace(fColorSpace);
...
sk_sp<GrTextureProxy> SkImage_Lazy::lockTextureProxy(GrContext* ctx,
    ...
    // 2. Ask the generator to natively create one
    {
        ...
        // XXX - this will fail in legacy mode b/c cacheInfo.colorSpace is not necessarily renderable.
        // What we want here is for SkPictureImageGenerator::onGenerateTexture to create an 
        // interposing SkColorSpaceXformCanvas.
