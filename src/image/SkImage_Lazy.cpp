...
sk_sp<GrTextureProxy> SkImage_Lazy::asTextureProxyRef(GrRecordingContext* context,
    ...
    GrImageTextureMaker textureMaker = GrImageTextureMaker::Make(context, this, kAllow_CachingHint);
...
sk_sp<GrTextureProxy> SkImage_Lazy::lockTextureProxy(
    ...
    if (!proxy && this->getROPixels(&bitmap, chint)) {
        GrBitmapTextureMaker bitmapMaker = GrBitmapTextureMaker::Make(ctx, bitmap, false, false);
        proxy = bitmapMaker.refTextureProxy(willBeMipped ? GrMipMapped::kYes : GrMipMapped::kNo);
