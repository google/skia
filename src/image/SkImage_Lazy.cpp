...
GrSurfaceProxyView SkImage_Lazy::lockTextureProxyView(GrRecordingContext* ctx,
    ...
    {
        ScopedGenerator generator(fSharedGenerator);
        SkImageGenerator::TexGenPolicy texGenPolicy;
        switch (cachePolicy) {
            case GrImageTexGenPolicy::kDraw:
                texGenPolicy = SkImageGenerator::TexGenPolicy::kCheapest;
                break;
            case GrImageTexGenPolicy::kNew_Uncached_Unbudgeted:
                texGenPolicy = SkImageGenerator::TexGenPolicy::kForceNew_Unbudgeted;
                break;
            case GrImageTexGenPolicy::kNew_Uncached_Budgeted:
                texGenPolicy = SkImageGenerator::TexGenPolicy::kForceNew_Budgeted;
                break;
        }
    ...
    if (mipMapped == GrMipMapped::kNo && !ctx->priv().options().fDisableGpuYUVConversion) {
        ...
        SkBudgeted budgeted = cachePolicy == GrImageTexGenPolicy::kNew_Uncached_Unbudgeted
                                      ? SkBudgeted::kNo
                                      : SkBudgeted::kYes;
    ...
    auto hint = cachePolicy == GrImageTexGenPolicy::kDraw ? CachingHint::kAllow_CachingHint
    ...
    if (SkBitmap bitmap; this->getROPixels(&bitmap, hint)) {
        ...
        auto makerPolicy = cachePolicy == GrImageTexGenPolicy::kNew_Uncached_Unbudgeted
