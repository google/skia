...
SkImage_Lazy::Validator::Validator(sk_sp<SharedGenerator> gen, const SkIRect* subset,
    ...
    // we need a different uniqueID for a different color space
    if (colorSpace) {
        fUniqueID = SkNextID::ImageID();
    }
...
bool SkImage_Lazy::onReadPixels(const SkImageInfo& dstInfo, void* dstPixels, size_t dstRB,
    ...
    if (kDisallow_CachingHint == chint) {
        ...
        if (this->lockAsBitmapOnlyIfAlreadyCached(&bm, cacheFormat)) {
            ...
            if (this->directGeneratePixels(dstInfo, dstPixels, dstRB, srcX, srcY, behavior)) {
                return true;
            }
            // else fall through
        }
...
bool SkImage_Lazy::getROPixels(SkBitmap* bitmap, SkColorSpace* dstColorSpace,
    ...
    if (!dstColorSpace) {
        behavior = SkTransferFunctionBehavior::kIgnore;
        cacheInfo = cacheInfo.makeColorSpace(fColorSpace);
    }
    return this->lockAsBitmap(bitmap, chint, cacheFormat, cacheInfo, behavior);
}
...
sk_sp<GrTextureProxy> SkImage_Lazy::lockTextureProxy(GrContext* ctx,
    ...
    SkImageInfo cacheInfo = this->buildCacheInfo(format);
    SkImageInfo surfaceInfo = cacheInfo;
    ...
    // 2. Ask the generator to natively create one
    {
        ...
        if (sk_sp<GrTextureProxy> proxy =
                    generator->generateTexture(ctx, cacheInfo, fOrigin, behavior)) {
    ...
    if (!ctx->contextPriv().disableGpuYUVConversion()) {
        ...
        SkColorSpace* genColorSpace = fInfo.colorSpace();
        SkColorSpace* imgColorSpace = fColorSpace.get();

        if (sk_sp<GrTextureProxy> proxy =
                    provider.refAsTextureProxy(ctx, desc, true, genColorSpace, imgColorSpace)) {
    ...
    // 4. Ask the generator to return RGB(A) data, which the GPU can convert
    SkBitmap bitmap;
    if (this->lockAsBitmap(&bitmap, chint, format, cacheInfo, behavior)) {
