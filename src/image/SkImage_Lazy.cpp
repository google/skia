...
#if SK_SUPPORT_GPU
    ScopedGenerator generator(fSharedGenerator);
    return TexGenType::kNone != generator->onCanGenerateTexture();
#else
    return false;
#endif
}
...
sk_sp<GrTextureProxy> SkImage_Lazy::lockTextureProxy(GrContext* ctx,
    ...
    {
        ScopedGenerator generator(fSharedGenerator);
        if (CheapTexGenOnly::kYes == onlyIfFast &&
                TexGenType::kCheap != generator->onCanGenerateTexture()) {
            return nullptr;
        }
