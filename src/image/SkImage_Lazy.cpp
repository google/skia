...
sk_sp<GrTextureProxy> SkImage_Lazy::lockTextureProxy(GrContext* ctx,
    ...
    // 1. Check the cache for a pre-existing one
    if (key.isValid()) {
        proxy = ctx->proxyProvider()->findOrCreateProxyByUniqueKey(key, kTopLeft_GrSurfaceOrigin);
        if (proxy) {
    ...
    if (!proxy) {
        ...
        if ((proxy = generator->generateTexture(ctx, genPixelsInfo, fOrigin, behavior,
            ...
            SK_HISTOGRAM_ENUMERATION("LockTexturePath", kNative_LockTexturePath,
                                     kLockTexturePathCount);
            set_key_on_proxy(ctx->proxyProvider(), proxy.get(), nullptr, key);
            if (!willBeMipped || GrMipMapped::kYes == proxy->mipMapped()) {
                return proxy;
            }
        }
    ...
    if (!proxy && this->lockAsBitmap(&bitmap, chint, format, genPixelsInfo, behavior)) {
        ...
        if (!proxy) {
            proxy = GrUploadBitmapToTextureProxy(ctx->proxyProvider(), bitmap, dstColorSpace);
        }
    ...
    if (proxy) {
        ...
        if (auto mippedProxy = GrCopyBaseMipMapToTextureProxy(ctx, proxy.get())) {
            set_key_on_proxy(ctx->proxyProvider(), mippedProxy.get(), proxy.get(), key);
            return mippedProxy;
        }
