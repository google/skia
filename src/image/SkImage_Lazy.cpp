...
sk_sp<SkCachedData> SkImage_Lazy::getPlanes(SkYUVASizeInfo* yuvaSizeInfo,
    ...
    if (!data && generator->queryYUVASpec(&spec, colorTypes, rowbytes) &&
        spec.fDimensions == this->dimensions()) {
        size_t size = 0;
        SkImageInfo infos[SkYUVASpec::kMaxPlanes];
        SkISize planeSizes[SkYUVASpec::kMaxPlanes];
        int n = spec.expectedPlaneDims(planeSizes);
        for (int i = 0; i < n; ++i) {
            infos[i] = SkImageInfo::Make(planeSizes[i], colorTypes[i], kPremul_SkAlphaType);
            size += rowbytes[i] * infos->height();
        }
        data.reset(SkResourceCache::NewCachedData(size));
        const char* addr = static_cast<const char*>(data->data());
        for (int i = 0; i < n; ++i) {
            yuvInfo.fPlanes[i].reset(infos[i], addr, rowbytes[i]);
            addr += rowbytes[i] * infos->height();
        }
        if (generator->getYUVAPlanes(yuvInfo.fPlanes) &&
            init_legacy_yuv_info(spec, yuvInfo.fPlanes, &yuvInfo.fSizeInfo, yuvInfo.fYUVAIndices)) {
            yuvInfo.fColorSpace = spec.fYUVColorSpace;
    ...
    // Try the legacy SkImageGenerator/SkCodec YUV interface
    if (!data) {
