...
sk_sp<SkCachedData> SkImage_Lazy::getPlanes(SkYUVASizeInfo* yuvaSizeInfo,
    ...
    if (!data && generator->queryYUVAInfo(&yuvaPixmapInfo) &&
        yuvaPixmapInfo.yuvaInfo().dimensions() == this->dimensions()) {
        size_t totalSize;
        totalSize = yuvaPixmapInfo.computeTotalBytes();
        data.reset(SkResourceCache::NewCachedData(totalSize));
        SkYUVAPixmaps pixmaps = SkYUVAPixmaps::MakeUnowned(yuvaPixmapInfo,
                                                           data->writable_data(),
                                                           data->size());
