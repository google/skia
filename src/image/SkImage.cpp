...
sk_sp<SkImage> SkImage::makeSubset(const SkIRect& subset, GrRecordingContext* context) const {
    if (subset.isEmpty()) {
        return nullptr;
    }
    ...
    // Fall back to using my own context if it's a matching direct context.
    bool contextMatches = true;
#if SK_SUPPORT_GPU
    auto myContext = as_IB(this)->context();
    if (!context) {
        context = GrAsDirectContext(myContext);
    }
    contextMatches = context ? context->priv().matches(myContext) : !myContext;
#endif

    // optimization : return self if the subset == our bounds and context matches
    if (bounds == subset && contextMatches) {
        return sk_ref_sp(const_cast<SkImage*>(this));
    }

    return as_IB(this)->onMakeSubset(subset, context);
}
