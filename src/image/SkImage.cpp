...
sk_sp<SkImage> SkImage::makeWithFilter(GrContext* grContext,
    ...
    // The output image and offset are relative to the subset rectangle, so the offset needs to be
    // shifted to put it in the correct spot with respect to the original coordinate system
    offset->fX += subset.x();
    offset->fY += subset.y();

    // Final clip against the exact clipBounds (the clip provided in the context gets adjusted to
    // account for pixel-moving filters so doesn't always exactly match when finished)
    SkIRect dstRect = result->subset();
    if (!dstRect.intersect(clipBounds.makeOffset(-offset->x(), -offset->y()))) {
        return nullptr;
    }

    // Adjust the geometric offset if the top-left corner moved as well
    offset->fX += (dstRect.x() - result->subset().x());
    offset->fY += (dstRect.y() - result->subset().y());

    // Note that here we're returning the special image's entire backing store, loose padding
    // and all! But dstRect is relative to the backing store, so it is a valid subset.
    *outSubset = dstRect;
