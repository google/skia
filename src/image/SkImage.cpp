...
SkData* SkImage::encode(SkImageEncoder::Type type, int quality) const {
    ...
    // by asking for a legacy decode (which gives us the raw data in N32), then re-tag the pixmap
    // with the original (encoded) color space.
    if (as_IB(this)->getROPixels(&bm, SkDestinationSurfaceColorMode::kLegacy)) {
...
SkData* SkImage::encode(SkPixelSerializer* serializer) const {
    ...
    if (as_IB(this)->getROPixels(&bm, SkDestinationSurfaceColorMode::kLegacy) &&
        bm.requestLock(&apu)) {
//        SkPixmap& nonConstPixmap = const_cast<SkPixmap&>(apu.pixmap());
//        nonConstPixmap.setColorSpace(sk_ref_sp(as_IB(this)->onEncodedImageInfo().colorSpace()));
