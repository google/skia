...
sk_sp<SkImage> SkImage::makeWithFilter(GrContext* grContext,
    ...
    SkFilterOutputProperties outputProperties(fInfo.colorType(), fInfo.colorSpace());
    ...
    SkFilterContext context(SkMatrix::MakeTrans(-subset.x(), -subset.y()),
                                   clipBounds.makeOffset(-subset.x(), -subset.y()),
                                   cache.get(), outputProperties);

    sk_sp<SkSpecialImage> result = filter->priv().filterImage(
            srcSpecialImage.get(), context, offset);
    if (!result) {
        return nullptr;
    }
