...
std::unique_ptr<GrFragmentProcessor> SkImage_Base::asFragmentProcessor(GrRecordingContext* context,
    ...
    if (sampling.mipmap != SkMipmapMode::kNone &&
        (!context->priv().caps()->mipmapSupport() || this->dimensions().area() <= 1)) {
        sampling = SkSamplingOptions(sampling.filter);
    }
    return this->onAsFragmentProcessor(context, sampling, tileModes, m, subset, domain);
}
#endif
