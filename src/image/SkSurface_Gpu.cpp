...
sk_sp<SkSurface> SkSurface::MakeFromBackendTexture(GrContext* context,
    ...
    auto device = SkGpuDevice::Make(context, std::move(rtc), backendTexture.width(),
                                    backendTexture.height(), SkGpuDevice::kUninit_InitContents);
    if (!device) {
        return nullptr;
    }
...
sk_sp<SkSurface> SkSurface::MakeFromBackendTexture(GrContext* context, const GrBackendTexture& tex,
    ...
    auto device = SkGpuDevice::Make(context, std::move(rtc), tex.width(), tex.height(),
...
sk_sp<SkSurface> SkSurface::MakeFromBackendRenderTarget(GrContext* context,
    ...
    auto device = SkGpuDevice::Make(context, std::move(rtc), rt.width(), rt.height(),
                                    SkGpuDevice::kUninit_InitContents);
    if (!device) {
        return nullptr;
    }
