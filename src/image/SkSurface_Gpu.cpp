...
bool SkSurface_Gpu::Valid1(const SkImageInfo& info) {
    return true;
}

bool SkSurface_Gpu::Valid2(const GrCaps* caps,
...
sk_sp<SkSurface> SkSurface::MakeRenderTarget(GrRecordingContext* context,
    ...
    if (!SkSurface_Gpu::Valid2(caps, format, c.colorSpace())) {
        return nullptr;
    }
...
static bool validate_backend_texture(GrContext* ctx, const GrBackendTexture& tex,
                                     ...
                                     sk_sp<SkColorSpace> cs, bool texturable) {
    if (!tex.isValid()) {
        return false;
    }
...
sk_sp<SkSurface> SkSurface::MakeFromBackendTexture(GrContext* context,
    ...
    if (!validate_backend_texture(context, texCopy, &texCopy.fConfig,
                                  c.sampleCount(), c.colorType(), c.refColorSpace(), true)) {
        return nullptr;
    }

    SkASSERT(texCopy.fConfig == c.config());

    if (!SkSurface_Gpu::Valid2(context->priv().caps(), texCopy.getBackendFormat(),
                               c.colorSpace())) {
        return nullptr;
    }

    sk_sp<GrRenderTargetContext> rtc(context->priv().makeBackendTextureRenderTargetContext(
                texCopy, c.origin(), c.sampleCount(), c.refColorSpace(), &c.surfaceProps(),
...
sk_sp<SkSurface> SkSurface::MakeFromBackendTexture(GrContext* context, const GrBackendTexture& tex,
    ...
    sk_sp<GrRenderTargetContext> rtc(context->priv().makeBackendTextureRenderTargetContext(
                                texCopy, origin, sampleCnt, std::move(colorSpace), props,
                                textureReleaseProc, releaseContext));
    if (!rtc) {
        return nullptr;
    }
...
sk_sp<SkSurface> SkSurface::MakeFromBackendRenderTarget(GrContext* context,
    ...
    if (!SkSurface_Gpu::Valid2(context->priv().caps(), rtCopy.backendFormat(), colorSpace.get())) {
        return nullptr;
    }
