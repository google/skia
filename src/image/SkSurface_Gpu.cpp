...
static GrRenderTarget* prepare_rt_for_external_access(SkSurface_Gpu* surface,
    ...
    GrSurfaceDrawContext* sdc = nullptr; //surface->getDevice()->surfaceDrawContext();
    return nullptr; //sdc->accessRenderTarget();
}
...
sk_sp<SkSurface> SkSurface_Gpu::MakeWrappedRenderTarget1(GrRecordingContext* context,
                                                        std::unique_ptr<GrSurfaceDrawContext> sdc) {
    if (!context) {
        return nullptr;
    }

    auto device = SkGpuDevice::Make1(std::move(sdc), SkBaseGpuDevice::kUninit_InitContents);
    if (!device) {
        return nullptr;
    }
...
sk_sp<SkSurface> SkSurface::MakeFromBackendTexture(GrRecordingContext* rContext,
    ...
    auto sdc = GrSurfaceDrawContext::MakeFromBackendTexture(
            rContext, grColorType, std::move(colorSpace), tex, sampleCnt, origin,
            SkSurfacePropsCopyOrDefault(props), std::move(releaseHelper));
    if (!sdc) {
        return nullptr;
    }

    auto device = SkGpuDevice::Make1(std::move(sdc), SkGpuDevice::kUninit_InitContents);
    if (!device) {
        return nullptr;
    }
    return sk_make_sp<SkSurface_Gpu>(std::move(device));
