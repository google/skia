...
sk_sp<SkImage> SkSurface_Gpu::onNewImageSnapshot(const SkIRect* subset) {
    ...
    if (subset || !rtc->asTextureProxy() || rtc->priv().refsWrappedObjects()) {
        ...
        auto copy = GrSurfaceProxy::Copy(ctx, rtc->asSurfaceProxy(), rtc->origin(),
                                         rtc->mipMapped(), rect, SkBackingFit::kExact, budgeted);
        srcView = {std::move(copy), rtc->origin(), rtc->readSwizzle()};
    } else {
