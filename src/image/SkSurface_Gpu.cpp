...
sk_sp<SkImage> SkSurface_Gpu::onNewImageSnapshot(const SkIRect* subset) {
    ...
    // SkImage's still require a GrContext but the SkGpuDevice only holds a GrRecordingContext.
...
void SkSurface_Gpu::onDraw(SkCanvas* canvas, SkScalar x, SkScalar y, const SkPaint* paint) {
    ...
    auto tryDraw = [&] {
        ...
        auto canvasContext = canvas->recordingContext()->priv().asDirectContext();
        if (!canvasContext) {
            return false;
        }
