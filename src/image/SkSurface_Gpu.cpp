...
sk_sp<SkImage> SkSurface_Gpu::onNewImageSnapshot(const SkIRect* subset) {
    ...
    if (subset || !srcView.asTextureProxy() || rtc->priv().refsWrappedObjects()) {
        ...
        srcView = GrSurfaceProxyView::Copy(direct, std::move(srcView), rtc->mipMapped(), rect,
    ...
    if (srcView.asTextureProxy()) {
        ...
        image = sk_make_sp<SkImage_Gpu>(sk_ref_sp(direct), kNeedNewImageUniqueID,
...
void SkSurface_Gpu::onDraw(SkCanvas* canvas, SkScalar x, SkScalar y, const SkPaint* paint) {
    ...
    auto tryDraw = [&] {
        ...
        auto canvasContext = canvas->recordingContext();
        if (!canvasContext) {
            return false;
        }
