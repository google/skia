...
private:
    ...
    // from) to a "stable" proxy that is a copy of the volatile proxy. It cancels the copy if
    // the stable proxy is never required because the contents of the volatile proxy were never
    // mutated.
    class ProxyChooser {
    public:
