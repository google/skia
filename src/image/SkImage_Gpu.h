...
private:
    ...
    // from) to a "stable" proxy that is a copy of the volatile proxy and cancelling the copy if
    // the stable proxy is never needed.
    class ProxyChooser {
    public:
        ...
        // Checks if there is a volatile proxy that is safe to use. Is so returns it, otherwise
        ...
        // Queries that should be equivalent for the stable or volatile proxy.
