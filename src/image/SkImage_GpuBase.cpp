...
sk_sp<GrTextureProxy> SkImage_GpuBase::MakePromiseImageLazyProxy(
     ...
     * cases where the client provides the same PromiseImageTexture for successive Fulfill calls.
     * The created GrTexture is given a key based on a unique ID associated with the
     * PromiseImageTexture. When the texture enters "idle" state (meaning it is not being used by
     * the GPU and is at rest in the resource cache) the client's Release proc is called
     * using GrTexture's idle proc mechanism. If the same backendTexture is provided for
     ...
     * destroys their GrBackendTexture we invalidate the key.
     *
     * Currently a GrTexture is only reused for a given PromiseImageTexture if the
     * PromiseImageTexture is reused in Fulfill for the same promise SkImage. However, we'd
     * like to relax that so that a PromiseImageTexture can be reused with a different promise
     * SkImage and without recreating a GrTexture.
     */
    ...
    public:
        ...
        sk_sp<GrSurface> operator()(GrResourceProvider* resourceProvider) {
            ...
            SkDeferredDisplayListRecorder::PromiseImageTexture* pit = fFulfillProc(fContext);
            if (!pit) {
                fReleaseProc(fContext, nullptr);
                return sk_sp<GrTexture>();
            }
            bool same = pit->uniqueID() == fLastFulfillID;
            ...
            fLastFulfillID = pit->uniqueID();

            backendTexture = pit->backendTexture();
            backendTexture.fConfig = fConfig;
            if (!backendTexture.isValid()) {
                // Even though the GrBackendTexture is not valid, we must call the release
                // proc to keep our contract of always calling Fulfill and Release in pairs.
                fReleaseProc(fContext, pit);
                return sk_sp<GrTexture>();
            }
            ...
            fIdleContext = sk_make_sp<IdleContext>(fReleaseProc, fContext, pit);
            ...
            fContextID = tex->getContext()->uniqueID();
            pit->addKeyToInvalidate(fContextID, fLastFulfilledKey);
            return std::move(tex);
        }
    ...
    private:
        struct IdleContext : public SkNVRefCnt<IdleContext> {
            ...
            const SkDeferredDisplayListRecorder::PromiseImageTexture* fPromiseImageTexture;
        ...
        // ID of the last PromiseImageTexture given to us by the client.
