...
sk_sp<GrTextureProxy> SkImage_GpuBase::MakePromiseImageLazyProxy(
     ...
     * instead of the GrSurface release proc because the GrTexture is cached and therefor may
    ...
    public:
        PromiseLazyInstantiateCallback(PromiseImageTextureFulfillProc fulfillProc,
            ...
            fIdleHelper = sk_make_sp<GrReleaseProcHelper>(releaseProc, context);
            fIdleHelper->setChild(std::move(doneHelper));
        }
        ...
        ~PromiseLazyInstantiateCallback() {
            if (fIdleHelper) {
                ...
                fIdleHelper->abandon(false);
            }
        }

        sk_sp<GrSurface> operator()(GrResourceProvider* resourceProvider) {
            SkASSERT(fIdleHelper);
            PromiseImageTextureContext textureContext = fIdleHelper->context();
            ...
            if (!promiseTexture) {
                ...
                // fIdleHelper means fulfill was never called.
                fIdleHelper.reset();
            ...
            idleContext->addImageReleaseContext(std::move(fIdleHelper));
            promiseTexture->addKeyToInvalidate(tex->getContext()->priv().contextID(), key);
            return std::move(tex);
        }

    private:
        // The GrTexture's idle callback mechanism is used to call the client's Release and Done
        // procs. This maintains a list of GrReleaseProcHelpers, each added by a
        // PromiseLazyInstantiateCallback in order to allow multiple promise images fulfilled by
        // the same texture.
        class IdleContext {
        public:
            IdleContext() = default;
            ...
            void addImageReleaseContext(sk_sp<GrReleaseProcHelper> context) {
                fReleaseContexts.addToHead(std::move(context));
            }

            static void IdleProc(void* context) { delete static_cast<IdleContext*>(context); }

        private:
            using ReleaseContextList = SkTLList<sk_sp<GrReleaseProcHelper>, 4>;
            ReleaseContextList fReleaseContexts;
        };

        sk_sp<GrReleaseProcHelper> fIdleHelper;
