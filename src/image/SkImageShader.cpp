...
bool SkImageShader::onAppendStages(SkRasterPipeline* p, SkColorSpace* dstCS, SkArenaAlloc* alloc,
    ...
    auto tiling_x = alloc->make<SkJumper_TilingCtx>();
    tiling_x->limit            = (float)pm.width();
    tiling_x->ulp_before_limit = ulp_before(tiling_x->limit);

    auto tiling_y = alloc->make<SkJumper_TilingCtx>();
    tiling_y->limit            = (float)pm.height();
    tiling_y->ulp_before_limit = ulp_before(tiling_y->limit);

    auto append_tiling_and_gather = [&] {
        switch (fTileModeX) {
            case kClamp_TileMode:  p->append(SkRasterPipeline::clamp_x,  tiling_x); break;
            case kMirror_TileMode: p->append(SkRasterPipeline::mirror_x, tiling_x); break;
            case kRepeat_TileMode: p->append(SkRasterPipeline::repeat_x, tiling_x); break;
        }
        switch (fTileModeY) {
            case kClamp_TileMode:  p->append(SkRasterPipeline::clamp_y,  tiling_y); break;
            case kMirror_TileMode: p->append(SkRasterPipeline::mirror_y, tiling_y); break;
            case kRepeat_TileMode: p->append(SkRasterPipeline::repeat_y, tiling_y); break;
        }
