...
//      case  kRGBA_F16_SkColorType:
    ...
    auto append_tiling = [this](SkRasterPipeline* p, Context* ctx) {
        switch (fTileModeX) {
    ...
    auto append_accum = [](SkRasterPipeline* p, Context* ctx, SkColorType ct, bool srgb) {
        switch (ct) {
            case kRGBA_8888_SkColorType:
            case kBGRA_8888_SkColorType:
                if (srgb) {
    ...
    if (quality == kNone_SkFilterQuality) {
        append_tiling(p, ctx);
        append_accum(p, ctx, info.colorType(), dst && info.gammaCloseToSRGB());
    }
