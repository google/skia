...
static SkShader* maybe_color_shader(const sk_sp<SkImage>& image, SkTBlitterAllocator* allocator) {
    ...
    if (!tmp.appendStages(&p, pmap.colorSpace(), &fallback,
                          SkMatrix::I(), kNone_SkFilterQuality)) {
        return nullptr;
    }
    ...
    if (allocator) {
        return allocator->createT<SkColor4Shader>(pm4f.unpremul(), sk_ref_sp(pmap.colorSpace()));
    }
    return new SkColor4Shader(pm4f.unpremul(), sk_ref_sp(pmap.colorSpace()));
}
