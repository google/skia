...
//      case  kRGBA_F16_SkColorType:
    ...
    if (quality == kNone_SkFilterQuality) {
        if (matrix.getScaleX() >= 0) {
            matrix.setTranslateX(std::nextafterf(matrix.getTranslateX(),
                                                 std::floor(matrix.getTranslateX())));
        }
        if (matrix.getScaleY() >= 0) {
            matrix.setTranslateY(std::nextafterf(matrix.getTranslateY(),
                                                 std::floor(matrix.getTranslateY())));
        }
    }
