...
#ifdef SK_DEBUG
        auto atlasGlyphCache = fGrContext->contextPriv().getAtlasManager();
        int numProxies;
...
GrAtlasGlyphCache1* SkInternalAtlasTextContext::atlasGlyphCache1() {
    return fGrContext->contextPriv().getAtlasGlyphCache1();
}
