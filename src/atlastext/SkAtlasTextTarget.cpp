...
void GrAtlasTextOp::executeForTextTarget(SkAtlasTextTarget* target) {
    ...
    for (int i = 0; i < fGeoCount; ++i) {
        GrColor color = 0xFFFF1122;
        GrAtlasTextBlob::VertexRegenerator regenerator(fGeoData[i].fBlob, fGeoData[i].fRun, fGeoData[i].fSubRun, fGeoData[i].fViewMatrix, fGeoData[i].fX, fGeoData[i].fY, color, &context, atlasGlyphCache, &glyphCache);
