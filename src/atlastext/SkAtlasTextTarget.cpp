...
std::unique_ptr<SkAtlasTextTarget> SkAtlasTextTarget::Make(sk_sp<SkAtlasTextContext> context,
    ...
    return std::unique_ptr<SkAtlasTextTarget>(new SkInternalAtlasTextTarget(std::move(context),
                                                                            width, height, handle));
}
...
void SkInternalAtlasTextTarget::drawText(const void* text, size_t byteLength, SkScalar x, SkScalar y, uint32_t color, const SkAtlasTextFont& font) {
    SkPaint paint;
...
void GrAtlasTextOp::executeForTextTarget(SkAtlasTextTarget* target, uint32_t color) {
    ...
    for (int i = 0; i < fGeoCount; ++i) {
        GrAtlasTextBlob::VertexRegenerator regenerator(fGeoData[i].fBlob, fGeoData[i].fRun,
                                                       fGeoData[i].fSubRun, fGeoData[i].fViewMatrix,
                                                       fGeoData[i].fX, fGeoData[i].fY, color,
