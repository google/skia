...
void SkAtlasTextTarget::drawText(const void* text, size_t byteLength, SkScalar x, SkScalar y) {
    ...
    class Target : public GrTextUtils::Target {
    public:
        Target(SkAtlasTextTarget* att) : GrTextUtils::Target(att->width(), att->height(), kColorSpaceInfo), fAtt(att) {}
    ...
    context->drawText(fContext->internal().grContext(), &target, GrNoClip(), paint, SkMatrix::I(), props, (const char*)text, byteLength, x, y, SkIRect::MakeWH(this->width(), this->height()));
}
...
void SkAtlasTextTarget::flush() {
    ...
    for (int i = 0; i < fOps.count(); ++i) {
        fOps[i]->flush(context.renderer());
    }
}

void GrAtlasTextOp::flush(SkAtlasTextRenderer* renderer) {
