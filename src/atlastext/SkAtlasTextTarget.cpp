...
private:
    ...
    SkTArray<std::unique_ptr<GrAtlasTextOp>, true> fOps1;
...
void SkInternalAtlasTextTarget::addDrawOp(const GrClip& clip, std::unique_ptr<GrAtlasTextOp> op) {
    ...
    for (int i = 0; i < n; ++i) {
        GrAtlasTextOp* other = fOps1.fromBack(i).get();
    ...
    fOps1.emplace_back(std::move(op));
}
...
void SkInternalAtlasTextTarget::deleteOps() {
    for (int i = 0; i < fOps1.count(); ++i) {
        ...
        if (fOps1[i]) {
            ...
            fOpMemoryPool->release(foo);
#else
            fOpMemoryPool->release(std::move(fOps1[i]));
#endif
        }
    }
    fOps1.reset();
