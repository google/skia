...
void sk_paint_set_blend_mode(sk_paint_t* paint, sk_blend_mode_t mode) {
    ...
    switch (mode) {
        ...
        MAP( CLEAR_SK_BLEND_MODE,      SkBlendMode::kClear      );
        MAP( SRC_SK_BLEND_MODE,        SkBlendMode::kSrc        );
        MAP( DST_SK_BLEND_MODE,        SkBlendMode::kDst        );
        MAP( SRCOVER_SK_BLEND_MODE,    SkBlendMode::kSrcOver    );
        MAP( DSTOVER_SK_BLEND_MODE,    SkBlendMode::kDstOver    );
        MAP( SRCIN_SK_BLEND_MODE,      SkBlendMode::kSrcIn      );
        MAP( DSTIN_SK_BLEND_MODE,      SkBlendMode::kDstIn      );
        MAP( SRCOUT_SK_BLEND_MODE,     SkBlendMode::kSrcOut     );
        MAP( DSTOUT_SK_BLEND_MODE,     SkBlendMode::kDstOut     );
        MAP( SRCATOP_SK_BLEND_MODE,    SkBlendMode::kSrcATop    );
        MAP( DSTATOP_SK_BLEND_MODE,    SkBlendMode::kDstATop    );
        MAP( XOR_SK_BLEND_MODE,        SkBlendMode::kXor        );
        MAP( PLUS_SK_BLEND_MODE,       SkBlendMode::kPlus       );
        MAP( MODULATE_SK_BLEND_MODE,   SkBlendMode::kModulate   );
        MAP( SCREEN_SK_BLEND_MODE,     SkBlendMode::kScreen     );
        MAP( OVERLAY_SK_BLEND_MODE,    SkBlendMode::kOverlay    );
        MAP( DARKEN_SK_BLEND_MODE,     SkBlendMode::kDarken     );
        MAP( LIGHTEN_SK_BLEND_MODE,    SkBlendMode::kLighten    );
        MAP( COLORDODGE_SK_BLEND_MODE, SkBlendMode::kColorDodge );
        MAP( COLORBURN_SK_BLEND_MODE,  SkBlendMode::kColorBurn  );
        MAP( HARDLIGHT_SK_BLEND_MODE,  SkBlendMode::kHardLight  );
        MAP( SOFTLIGHT_SK_BLEND_MODE,  SkBlendMode::kSoftLight  );
        MAP( DIFFERENCE_SK_BLEND_MODE, SkBlendMode::kDifference );
        MAP( EXCLUSION_SK_BLEND_MODE,  SkBlendMode::kExclusion  );
        MAP( MULTIPLY_SK_BLEND_MODE,   SkBlendMode::kMultiply   );
        MAP( HUE_SK_BLEND_MODE,        SkBlendMode::kHue        );
        MAP( SATURATION_SK_BLEND_MODE, SkBlendMode::kSaturation );
        MAP( COLOR_SK_BLEND_MODE,      SkBlendMode::kColor      );
        MAP( LUMINOSITY_SK_BLEND_MODE, SkBlendMode::kLuminosity );
        #undef MAP
        default:
            return;
    }
