...
struct sk_document_t : public SkRefCnt {
...
void sk_document_ref(sk_document_t* doc) { SkSafeRef(doc); }

void sk_document_unref(sk_document_t* doc) { SkSafeUnref(doc); }

sk_canvas_t* sk_document_begin_page(sk_document_t* doc, float w, float h) {
    return (sk_canvas_t*)doc->fDocument->beginPage(w, h);
}

void sk_document_end_page(sk_document_t* doc) { doc->fDocument->endPage(); }

void sk_document_close(sk_document_t* doc) { doc->fDocument->close(); }
