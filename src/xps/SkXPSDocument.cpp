...
/* Note:  Very little of this file *needs* _WIN32 (That's all in SkXPS.cpp), but no
   clients would want to compile this without that.  */
...
// Super simple clipstack.
class ClipStack {
public:
    ClipStack(SkISize size) { fPathStack.push_back().addRect(SkRect::Make(size)); }
    const SkPath& top() const { return fPathStack.back(); }
    void save() {
        SkPath path = fPathStack.back();
        fPathStack.emplace_back(std::move(path));
    }
    void restore() {
        if (fPathStack.count() > 1) {
            fPathStack.pop_back();
        }
    }
    void clip(const SkPath& path, SkClipOp op, const SkMatrix& ctm) {
        SkPath transformedPath = path;
        transformedPath.transform(ctm);
        SkPath& top = fPathStack.back();
        Op(top, transformedPath, (SkPathOp)op, &top);
    }

private:
    SkTArray<SkPath, true> fPathStack;
};
...
public:
    XPSCanvas(SkXPS* xps, SkISize size)
        : SkNoDrawCanvas(size.width(), size.height()), fXPS(xps), fClipStack(size) {}
    ~XPSCanvas() {}
...
protected:
    ...
    void onClipRect(const SkRect& r, SkClipOp o, ClipEdgeStyle) override;
    void onClipRRect(const SkRRect& rr, SkClipOp o, ClipEdgeStyle) override;
    void onClipPath(const SkPath& p, SkClipOp o, ClipEdgeStyle) override;
    void onClipRegion(const SkRegion& r, SkClipOp o) override;

private:
    const SkPath& clip() const { return fClipStack.top(); }
    const SkMatrix& ctm() const { return this->getTotalMatrix(); }
...
static void draw_points(SkCanvas* canvas, SkCanvas::PointMode mode, size_t count,
                        const SkPoint* pts, const SkPaint& paint) {
    if (SkCanvas::kLines_PointMode == mode) { count &= ~(size_t)1; }
    if ((long)count <= 0 || !pts) { return; }
    if (!canvas->getClipBounds(nullptr)) {
        return;
    }
    SkPath path;
    SkPaint paintCopy(paint);
    paintCopy.setStyle(SkPaint::kStroke_Style);
    switch (mode) {
...
void XPSCanvas::onClipRect(const SkRect& r, SkClipOp o, ClipEdgeStyle) {
    SkPath p;
    p.addRect(r);
    fClipStack.clip(p, o, this->ctm());
}

void XPSCanvas::onClipRRect(const SkRRect& rr, SkClipOp o, ClipEdgeStyle) {
    SkPath p;
    p.addRRect(rr);
    fClipStack.clip(p, o, this->ctm());
}

void XPSCanvas::onClipPath(const SkPath& p, SkClipOp o, ClipEdgeStyle) {
    fClipStack.clip(p, o, this->ctm());
}

void XPSCanvas::onClipRegion(const SkRegion& r, SkClipOp o) {
    SkPath p;
    if (r.getBoundaryPath(&p)) {
        fClipStack.clip(p, o, this->ctm());
    }
}

void XPSCanvas::willSave() { fClipStack.save(); }
...
void XPSCanvas::willRestore() {
    ...
    if (fLayerCounts.count() > 0 &&  this->getSaveCount() == fLayerCounts.back() + 1) {
...
void XPSCanvas::internalDrawText(const void* text, size_t textBytes,
    ...
    fXPS->drawText(this->ctm(), this->clip(), text, textBytes, positioning, origin, pos, paint);
}

void XPSCanvas::onDrawPoints(SkCanvas::PointMode mode, size_t count, const SkPoint pts[],
                             const SkPaint& paint) {
    draw_points(this, mode, count, pts, paint);
}

void XPSCanvas::onDrawPath(const SkPath& path, const SkPaint& paint) {
    fXPS->drawPath(this->ctm(), this->clip(), path, paint);
}

void XPSCanvas::onDrawBitmap(const SkBitmap& bm, SkScalar x, SkScalar y, const SkPaint* p) {
    fXPS->drawBitmap(this->ctm(), this->clip(), bm, SkPoint{x, y}, p);
}
...
public:
    static sk_sp<SkDocument> Make(SkWStream*, void (*)(SkWStream*, bool), SkScalar);
...
sk_sp<SkDocument> XPSDocument::Make(SkWStream* wStream,
    ...
    return sk_sp<SkDocument>(new XPSDocument(wStream, release, xpsFactory.get(), dpi));
}

} // namespace

sk_sp<SkDocument> SkDocument::MakeXPS(SkWStream* wStream, SkScalar dpi) {
    return XPSDocument::Make(wStream, nullptr, dpi);
}
...
sk_sp<SkDocument> SkDocument::MakeXPS(const char path[], SkScalar dpi) {
    ...
    return XPSDocument::Make(wStream.release(), [](SkWStream* p, bool) { delete p; }, dpi);
}
