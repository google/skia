...
public:
    XPSCanvas(SkXPS* xps, int w, int h) : SkNoDrawCanvas(w, h), fXPS(xps) {}
    ~XPSCanvas() {}
...
public:
    XPSDocument(SkWStream* wStream, void (*proc)(SkWStream*, bool aborted), SkScalar dpi)
        : SkDocument(wStream, proc), fXPS(wStream, dpi) {}
    ~XPSDocument() { this->close(); }

protected:
    SkCanvas* onBeginPage(SkScalar w, SkScalar h, const SkRect& trimBox) override {
        SkISize size{SkScalarCeilToInt(w), SkScalarCeilToInt(h)};
        fXPS.newPage(size);
        fCanvas = skstd::make_unique<XPSCanvas>(&fXPS, size.width(), size.height());
...
sk_sp<SkDocument> SkDocument::MakeXPS(SkWStream* stream, SkScalar dpi) {
    return stream ? sk_make_sp<XPSDocument>(stream, nullptr, dpi) : nullptr;
}
...
sk_sp<SkDocument> SkDocument::MakeXPS(const char path[], SkScalar dpi) {
    ...
    return sk_make_sp<XPSDocument>(stream.release(),
                                   [](SkWStream* s, bool) { delete s; }, dpi);
}
