...
#define HS_REQD_SUBGROUP_SIZE()                 \
  __attribute__((intel_reqd_sub_group_size(HS_SLAB_THREADS)))
...
#define HS_BS_KERNEL_PROTO(slab_count,slab_count_ru_log2)               \
  ...
  __attribute__((reqd_work_group_size(HS_SLAB_THREADS*slab_count,1,1))) \
  void                                                                  \
...
#define HS_SLAB_GLOBAL_PREAMBLE()                                       \
  uint const gmem_idx =                                                 \
    (get_global_id(0) & ~(HS_SLAB_THREADS-1)) * HS_SLAB_HEIGHT +        \
    (get_local_id(0) & (HS_SLAB_THREADS-1))
...
#define HS_BS_MERGE_H_PREAMBLE(slab_count)                      \
  uint const smem_l_idx =                                       \
    get_sub_group_id() * (HS_SLAB_THREADS * slab_count) +       \
    get_sub_group_local_id();                                   \
  uint const smem_r_idx =                                       \
    (get_sub_group_id() ^ 1) * (HS_SLAB_THREADS * slab_count) + \
    (get_sub_group_local_id() ^ (HS_SLAB_THREADS - 1))
...
#define HS_BC_MERGE_H_PREAMBLE(slab_count)                              \
  uint const gmem_l_idx =                                               \
    (get_global_id(0) & ~(HS_SLAB_THREADS*slab_count-1)) *              \
    HS_SLAB_HEIGHT + get_local_id(0);                                   \
  uint const smem_l_idx =                                               \
    get_sub_group_id() * (HS_SLAB_THREADS * slab_count) +               \
    get_sub_group_local_id()
...
// this may be faster for 32-bit keys on Intel GEN
#define HS_COND_MIN_MAX_V1(lt,a,b) (lt ? b : a) ^ ((a ^ b) & HS_LTE_TO_MASK(a,b))

//
// FIXME -- EVENTUALLY HANDLE KEY+VAL
//
