...
std::unique_ptr<SkCodec> SkHeifCodec::MakeFromStream(
    ...
    *result = kSuccess;
    return std::unique_ptr<SkCodec>(new SkHeifCodec(std::move(info), heifDecoder.release(), orientation));
}

SkHeifCodec::SkHeifCodec(SkEncodedInfo&& info, HeifDecoder* heifDecoder, SkEncodedOrigin origin)
    : INHERITED(std::move(info), SkColorSpaceXform::kRGBA_8888_ColorFormat, nullptr, origin)
