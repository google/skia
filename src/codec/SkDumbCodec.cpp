...
static SkEncodedInfo info_to_encoded(const SkImageInfo& info) {
    ...
    switch (info.colorType()) {
        case kAlpha_8_SkColorType:   ct = SkEncodedInfo::kAlpha_Color; bits =  8; break;
        case kRGB_565_SkColorType:   ct = SkEncodedInfo::kRGB_Color;   bits =  5; break;
        case kARGB_4444_SkColorType: ct = SkEncodedInfo::kRGBA_Color;  bits =  4; break;
        case kRGBA_8888_SkColorType: ct = SkEncodedInfo::kRGBA_Color;  bits =  8; break;
        case kBGRA_8888_SkColorType: ct = SkEncodedInfo::kBGRA_Color;  bits =  8; break;
        case kGray_8_SkColorType:    ct = SkEncodedInfo::kGray_Color;  bits =  8; break;
        case kRGBA_F16_SkColorType:  ct = SkEncodedInfo::kRGBA_Color;  bits = 16; break;
...
bool SkDumbCodec::IsDumb(const void* buffer, size_t length) {
    return length >= sizeof(kDumbSigSize) && !memcmp(buffer, kDumbSig, sizeof(kDumbSigSize));
}

std::unique_ptr<SkCodec> SkDumbCodec::MakeFromStream(std::unique_ptr<SkStream> stream,
                                                    Result* result) {
    Header head;
    static_assert(sizeof(head.fSig) == sizeof(kDumbSigSize), "wrong size for signature");
...
SkDumbCodec::SkDumbCodec(const SkImageInfo& info, size_t rowBytes, sk_sp<SkData> pixels)
    ...
    , fPixmap(info, pixels.get(), rowBytes)
    , fStorage(std::move(pixels))
{}
