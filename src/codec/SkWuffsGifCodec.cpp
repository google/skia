...
SkCodec::Result SkWuffsGifCodec::onIncrementalDecode(int* rowsDecoded) {
    ...
    switch (fIncrDecStage) {
        ...
        case 3: {
            ...
            if (!fIncrDecSwizzler) {
                wuffs_base__slice_u8 wuffsPalette = fPixelBuffer.palette();
                SkASSERT(wuffsPalette.len == 4 * 256);

                uint8_t* wuffsPalettePtr = wuffsPalette.ptr;
                // TODO: Wuffs' palette is always (B, G, R, A) in memory order,
                // regardless of endianness. Can we always just cast that to
                // SkPMColor (a typedef'ed uint32_t)??
                //
                // There's also pre- vs post-multiplied alpha. For GIF, which
                // has binary alpha, that isn't a problem. But if we use Wuffs
                // for other codecs, and copy/paste this code, it could be
                // problematic.
                SkPMColor* skiaPalette =
                    static_cast<SkPMColor*>(static_cast<void*>(wuffsPalettePtr));

                // CreateSwizzler only reads left and right of the frame.
                SkIRect swizzleRect = SkIRect::MakeLTRB(r.min_incl_x, 0, r.max_excl_x, 0);

                fIncrDecSwizzler.reset(SkSwizzler::CreateSwizzler(
                    getEncodedInfo(), skiaPalette, dstInfo(), Options(), &swizzleRect));
            }
            ...
            if (result == SkCodec::kSuccess) {
                ...
                fIncrDecRowBytes = 0;
                fIncrDecStage = 0;
                fIncrDecSwizzler.reset(nullptr);
            }
            return result;
        }
    }
