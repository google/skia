...
// TODO(nigeltao): use a swizzler instead of load_u32le, store_etc and zero_etc.
...
static inline void zero_u32(uint8_t* p) {
    p[0] = 0;
    p[1] = 0;
    p[2] = 0;
    p[3] = 0;
...
static inline void zero_u16(uint8_t* p) {
    p[0] = 0;
    p[1] = 0;
}
...
std::unique_ptr<SkCodec> SkWuffsGifCodec::MakeFromStream(std::unique_ptr<SkStream> stream,
    ...
    // In Skia's API, the alpha and alphaType we calculate here and return is
    // only for the first frame. For animated images, later frames (with
    ...
    const auto imageInfo = SkImageInfo::Make(imgcfg.pixcfg.width(), imgcfg.pixcfg.height(),
                                             kN32_SkColorType, alphaType, SkColorSpace::MakeSRGB());
...
SkCodec::Result SkWuffsGifCodec::onStartIncrementalDecode(const SkImageInfo& dstInfo,
    ...
    // Set the dst image to zero.
    //
    // TODO(nigeltao): use a swizzler, once I figure out how it works.
