...
std::unique_ptr<SkCodec> SkWuffsGifCodec::MakeFromStream(std::unique_ptr<SkStream> stream,
    ...
    // TODO(nigeltao): C++ style new/delete instead of malloc/free? Also use
    // std::unique_ptr (which gets us RAII) and will implicitly free the memory
    // if we return early (i.e. fail)?
    decoder = static_cast<wuffs_gif__decoder*>(malloc(sizeof *decoder));
    if (!decoder) {
    ...
    decoder->check_wuffs_version(sizeof *decoder, WUFFS_VERSION);
    ...
    *result = kSuccess;
    return std::unique_ptr<SkCodec>(new SkWuffsGifCodec(
        encodedInfo, imageInfo, std::move(stream), pixbuf_ptr, decoder, imgcfg, pixbuf, iobuf));
}
...
SkWuffsGifCodec::SkWuffsGifCodec(const SkEncodedInfo& encodedInfo,
    ...
    // Initialize SkFrameHolder protected fields.
    fScreenWidth = imgcfg.width();
    fScreenHeight = imgcfg.height();
}
...
bool SkWuffsGifCodec::onRewind() {
    ...
    // Reset fDecoder.
    memset(fDecoder, 0, sizeof *fDecoder);
    fDecoder->check_wuffs_version(sizeof *fDecoder, WUFFS_VERSION);
...
SkCodec::Result SkWuffsGifCodec::onIncrementalDecode(int* rowsDecoded) {
    ...
    // Skia is random access: "return the i'th frame". Wuffs is sequential
    // access: "return the next frame". If Skia asks for a future frame, we have
    // to iterate forward. If Skia asks for a past frame, we have to rewind to
    // the beginning and then iterate forward. Overall, this can have quadratic
    // complexity (especially if SkCodec::getPixels calls
    // SkCodec::rewindIfNeeded which almost always triggers a rewind) which
    // obviously isn't good. There is a TODO in SkCodec::getPixels to fix this.
    //
    // First, rewind if necessary.
    if (fDecoder->num_decoded_frame_configs() > static_cast<uint64_t>(fIncrDecFrameIndex)) {
        // TODO(nigeltao): is this legit? Should we call the superclass'
        // SkCodec::rewindIfNeeded method instead?
        if (!stream()->rewind()) {
            return kInternalError;
        }
        onRewind();
    ...
    while (fDecoder->num_decoded_frame_configs() < static_cast<uint64_t>(fIncrDecFrameIndex)) {
...
int SkWuffsGifCodec::onGetFrameCount() {
    if (fFrames.empty()) {
        // Ensure that the Wuffs decoder is positioned at the first frame.
        if (fDecoder->num_decoded_frame_configs() != 0) {
            // TODO(nigeltao): is this legit? Should we call the superclass'
            // SkCodec::rewindIfNeeded method instead?
            if (!stream()->rewind()) {
                return 0;
        ...
        for (int i = 0;; i++) {
            wuffs_base__frame_config fc = ((wuffs_base__frame_config){});
            if (decodeFrameConfig(&fc) != WUFFS_BASE__STATUS_OK) {
                break;
            }
...
const SkFrame* SkWuffsGifCodec::onGetFrame(int i) const {
    return frame(i);
};
...
wuffs_base__status SkWuffsGifCodec::decodeFrameConfig(wuffs_base__frame_config* fc) {
    while (1) {
        ...
        if ((z == WUFFS_BASE__SUSPENSION_SHORT_READ) && fill_buffer(&fIOBuffer, stream())) {
            continue;
        }
        return z;
    }
