...
static inline void store_u32le_switched(uint8_t* p, uint32_t x) {
    // This could probably be optimized, but in any case, we should use a swizzler.
    p[0] = x >> 16;
    p[1] = x >> 8;
    p[2] = x >> 0;
    p[3] = x >> 24;
...
SkWuffsFrame::SkWuffsFrame(wuffs_base__frame_config* fc)
    : INHERITED(fc->index()),
      ...
      fReportedAlpha((fc->blend() == WUFFS_BASE__ANIMATION_BLEND__OPAQUE)
...
std::unique_ptr<SkCodec> SkWuffsGifCodec::MakeFromStream(std::unique_ptr<SkStream> stream,
    ...
    size_t pixbuf_len = imgcfg.pixcfg.pixbuf_len();
    ...
    auto alpha = imgcfg.first_frame_is_opaque() ? SkEncodedInfo::kOpaque_Alpha
...
SkWuffsGifCodec::SkWuffsGifCodec(SkEncodedInfo&& encodedInfo,
    ...
    SkASSERT(iobuf.len == SK_WUFFS_GIF_CODEC_BUFFER_SIZE);
    memmove(fBuffer, iobuf.ptr, iobuf.wi);
    fIOBuffer = ((wuffs_base__io_buffer){
        .ptr = fBuffer,
        .len = SK_WUFFS_GIF_CODEC_BUFFER_SIZE,
        .wi = iobuf.wi,
        .ri = iobuf.ri,
        .pos = iobuf.pos,
        .closed = iobuf.closed,
...
SkCodec::Result SkWuffsGifCodec::onIncrementalDecode(int* rowsDecoded) {
    ...
    switch (fIncrDecStage) {
        case 1: {
            SkCodec::Result result = seekFrame(fIncrDecFrameIndex);
        ...
        case 3: {
            ...
            wuffs_base__rect_ie_u32 r = fIncrDecFrameConfig.bounds();
...
const SkWuffsFrame* SkWuffsGifCodec::frame(int i) const {
...
void SkWuffsGifCodec::readFrames() {
    ...
    for (; (i < INT_MAX) && (static_cast<size_t>(i) < SIZE_MAX); i++) {
...
SkCodec::Result SkWuffsGifCodec::resetDecoder() {
    if (!fStream->rewind()) {
        return SkCodec::kInternalError;
    }
    fIOBuffer.wi = 0;
...
SkCodec::Result SkWuffsGifCodec::seekByte(uint64_t byteIndex) {
    if ((byteIndex >= fIOBuffer.pos) && (byteIndex - fIOBuffer.pos <= fIOBuffer.wi)) {
        fIOBuffer.ri = byteIndex - fIOBuffer.pos;
        return SkCodec::kSuccess;
    }
    if ((byteIndex > SIZE_MAX) || (!fStream->seek(byteIndex))) {
        return SkCodec::kInternalError;
    }
    fIOBuffer.wi = 0;
    fIOBuffer.ri = 0;
    fIOBuffer.pos = byteIndex;
    fIOBuffer.closed = false;
    return SkCodec::kSuccess;
}

SkCodec::Result SkWuffsGifCodec::seekFrame(int frameIndex) {
    ...
    SkCodec::Result res = seekByte(pos);
    if (res != SkCodec::kSuccess) {
        return res;
    }
    if (fDecoder->restart_frame(frameIndex, fIOBuffer.io_position_reader()) != nullptr) {
        return SkCodec::kInternalError;
    }
    return SkCodec::kSuccess;
}
...
wuffs_base__status SkWuffsGifCodec::decodeFrameConfig(wuffs_base__frame_config* fc) {
    while (true) {
        ...
        fNeedResetDecoder = wuffs_base__status__is_suspension(z) || wuffs_base__status__is_error(z);
        return z;
    }
}
