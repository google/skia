...
static bool fill_buffer(wuffs_base__io_buffer* b, SkStream* s) {
  wuffs_base__io_buffer__compact(b);
...
std::unique_ptr<SkCodec> SkWuffsGifCodec::MakeFromStream(
  ...
  wuffs_gif__decoder__check_wuffs_version(decoder, sizeof *decoder,
                                          WUFFS_VERSION);
...
bool SkWuffsGifCodec::onRewind() {
  ...
  wuffs_gif__decoder__check_wuffs_version(fDecoder, sizeof *fDecoder,
                                          WUFFS_VERSION);
...
SkCodec::Result SkWuffsGifCodec::onIncrementalDecode(int* rowsDecoded) {
  ...
  wuffs_base__rect_ie_u32 r = wuffs_base__image_buffer__bounds(&fImageBuffer);
  ...
  // TODO(nigeltao): is "y = 0" and "x = 0" correct? Or should it be "y =
  // r.min_incl_y" and likewise for x? See above.
...
int SkWuffsGifCodec::onGetFrameCount() {
  ...
  //
  // TODO(nigeltao): wuffs_gif__decoder__frame_count returns a uint64_t. Deal
  // with potential uint64_t to int overflow.
  return 1;
}
...
int SkWuffsGifCodec::onGetRepetitionCount() {
  // TODO(nigeltao): deal with potential uint32_t to int overflow. This isn't a
  // problem for GIF per se, but it will be a problem if we copy/paste this
  // code for APNG.
