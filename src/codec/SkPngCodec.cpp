...
void SkPngCodec::applyXformRow(void* dst, const void* src) {
    ...
    switch (fXformMode) {
        ...
        case kSwizzleColor_XformMode:
            ...
            SkAssertResult(this->colorXform()->apply(fXformColorFormat, dst, srcColorFormat, fColorXformSrcRow,
                                              fXformWidth, fXformAlphaType));
            break;
    }
}
...
void SkPngCodec::initializeSwizzler(const SkImageInfo& dstInfo, const Options& options) {
    ...
    if (this->colorXform() && apply_xform_on_decode(dstInfo.colorType(), this->getEncodedInfo().color())) {
