...
// If there is no color profile information, it will use sRGB. Matt's doc at
// https://docs.google.com/document/d/1otH6Dfiw0SAYC-tTFGeqZ__n40QkPbyFMruGoo2mam0/edit has
// some info about why decisions were made.
// FIXME: EncodedInfo implies that this is the data as it was encoded. But we do make
// some arbitrary decisions here. There's not a good way for a client to make the same
// decisions without us just deciding here, though.
std::unique_ptr<SkEncodedInfo::ICCProfile> read_color_profile(png_structp png_ptr,
    ...
    // FIXME: Note that Chromium does this first. Matt's doc (above) explains his
    // thoughts: the ICC is likely more specific, but Matt left Chromium's order
    // for backwards compatibility. His change is at https://codereview.chromium.org/2469473002
    ...
    // Again, note that Chromium would have returned nullptr if this failed. As
    // described in Matt's doc, he felt that we should assume srgb gamut in this
    // case, but left Chromium's behavior unchanged.
    skcms_TransferFunction fn;
    if (PNG_INFO_gAMA == png_get_gAMA_fixed(png_ptr, info_ptr, &gamma)) {
        fn.a = 1.0f;
        fn.b = fn.c = fn.d = fn.e = fn.f = 0.0f;
        fn.g = 1.0 / png_inverted_fixed_point_to_float(gamma);
    } else {
        ...
        if (encodedColorType == PNG_COLOR_TYPE_GRAY_ALPHA) {
            ...
            if (png_get_sBIT(fPng_ptr, fInfo_ptr, &sigBits)) {
                if (8 == sigBits->alpha && kGraySigBit_GrayAlphaIsJustAlpha == sigBits->gray) {
                    color = SkEncodedInfo::kAlpha_Color;
                }
            }
