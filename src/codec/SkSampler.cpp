...
void SkSampler::Fill(const SkImageInfo& info, void* dst, size_t rowBytes,
    ...
    if (SkCodec::kYes_ZeroInitialized == zeroInit && 0 == colorOrIndex) {
        return;
    }
    ...
    switch (info.colorType()) {
        ...
        case kBGRA_8888_SkColorType: {
            ...
            for (int row = 0; row < numRows; row++) {
                sk_memset32((uint32_t*) dstRow, colorOrIndex, width);
        ...
        case kRGB_565_SkColorType: {
            ...
            for (int row = 0; row < numRows; row++) {
                sk_memset16((uint16_t*) dstRow, colorOrIndex, width);
        ...
        case kRGBA_F16_SkColorType: {
            ...
            for (int row = 0; row < numRows; row++) {
                sk_memset64((uint64_t*) dstRow, colorOrIndex, width);
