...
SkCodec::Result SkSampledCodec::onGetAndroidPixels(const SkImageInfo& info, void* pixels,
    ...
    // We are performing a subset decode. Copy so we can use a different fSubset.
...
SkCodec::Result SkSampledCodec::sampledDecode(const SkImageInfo& info, void* pixels,
...

    {
        ...
        if (options.fSubset) {
            ...
            incrementalSubset.fLeft = options.fSubset->fLeft;
            incrementalSubset.fRight = options.fSubset->fRight;
