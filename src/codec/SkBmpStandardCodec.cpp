...
SkBmpStandardCodec::SkBmpStandardCodec(SkEncodedInfo&& info, std::unique_ptr<SkStream> stream,
    ...
    , fIsOpaque(isOpaque)
    , fInIco(inIco)
    , fAndMaskRowBytes(fInIco ? SkAlign4(compute_row_bytes(this->getEncodedInfo().width(), 1)) : 0)
{}
...
int SkBmpStandardCodec::decodeRows(const SkImageInfo& dstInfo, void* dst, size_t dstRowBytes,
    ...
    if (fInIco && fIsOpaque) {
        ...
        const int remainingScanlines = this->getEncodedInfo().height() - startScanline - height;
...
void SkBmpStandardCodec::decodeIcoMask(SkStream* stream, const SkImageInfo& dstInfo,
    ...
    const int sampledWidth = get_scaled_dimension(this->getEncodedInfo().width(), sampleX);
