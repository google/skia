...
public:
    SkWuffsGifFrame(int id, SkEncodedInfo::Alpha reportedAlpha)
        : INHERITED(id), fReportedAlpha(reportedAlpha) {}
...
private:
    ...
    const SkWuffsGifFrame* frame(int i) const;
    void readFrames();
    bool rewind();
    Result seek(int frameIndex);
    ...
    // TODO(nigeltao): drop the std::unique_ptr, to be a
    // std::vector<SkWuffsGifFrame>. This might need SkFrame to not be
    // SkNoncopyable.
    std::vector<std::unique_ptr<SkWuffsGifFrame>> fFrames;
    bool fFramesComplete;
    bool fSuspendedInDecodeFrame;
