...
int SkBmpRLECodec::decodeRows(const SkImageInfo& info, void* dst, size_t dstRowBytes,
    ...
    int decodedHeight = this->doRLE(decodeInfo, decodeDst, decodeRowBytes);
    if (this->colorXform() && decodedHeight) {
...
int SkBmpRLECodec::doRLE(const SkImageInfo& dstInfo, void* dst, size_t dstRowBytes) {
