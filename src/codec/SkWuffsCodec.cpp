...
private:
    ...
    // and before onIncrementalDecode returns kSuccess). Some Skia users expect
    ...
    void   onGetFrameCount1();
    Result onIncrementalDecode1(int* rowsDecoded);
...
SkCodec::Result SkWuffsCodec::onIncrementalDecode(int* rowsDecoded) {
    ...
    SkCodec::Result result = this->onIncrementalDecode1(rowsDecoded);
    if (result == SkCodec::kSuccess) {
        fIncrDecDst = nullptr;
        fIncrDecReaderIOPosition = 0;
        fIncrDecRowBytes = 0;
...
SkCodec::Result SkWuffsCodec::onIncrementalDecode1(int* rowsDecoded) {
...
int SkWuffsCodec::onGetFrameCount() {
    if (!fFramesComplete) {
        this->onGetFrameCount1();
    }
    return fFrames.size();
}

void SkWuffsCodec::onGetFrameCount1() {
