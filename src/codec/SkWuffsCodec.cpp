...
private:
    ...
    // and uses less memory) if applicable can (see the assignment to
    ...
    Result onIncrementalDecodeOnePass(int* rowsDecoded);
    Result onIncrementalDecodeTwoPass(int* rowsDecoded);
...
SkCodec::Result SkWuffsCodec::onStartIncrementalDecode(const SkImageInfo&      dstInfo,
    ...
    fIncrDecOnePass = false;
    if (pixelFormat != WUFFS_BASE__PIXEL_FORMAT__INVALID) {
        bool        independent;
        ...
        bool identityTransform = this->dimensions() == dstInfo.dimensions();
        fIncrDecOnePass = independent && (alphaType == kOpaque_SkAlphaType) && identityTransform;
    }

    result = fIncrDecOnePass
                 ? this->onStartIncrementalDecodeOnePass(dstInfo, static_cast<uint8_t*>(dst),
                                                         rowBytes, pixelFormat, bytesPerPixel)
...
SkCodec::Result SkWuffsCodec::onStartIncrementalDecodeTwoPass() {
    // Either re-use the previously allocated two-pass pixel buffer (and memset
    // to zero), or allocate (and zero initialize) a new one.
    if (fTwoPassPixbufPtr) {
        ...
        const char* status = fPixelBuffer.set_from_slice(
            &fPixelConfig, wuffs_base__make_slice_u8(pixbuf_ptr.get(), SkToSizeT(pixbuf_len)));
        if (status != nullptr) {
            return SkCodec::kInternalError;
        }

        fTwoPassPixbufPtr.swap(pixbuf_ptr);
    }
...
SkCodec::Result SkWuffsCodec::onIncrementalDecode(int* rowsDecoded) {
    ...
    SkCodec::Result result = fIncrDecOnePass ? this->onIncrementalDecodeOnePass(rowsDecoded)
                                             : this->onIncrementalDecodeTwoPass(rowsDecoded);
    if (result == SkCodec::kSuccess) {
        fIncrDecDst = nullptr;
        fIncrDecReaderIOPosition = 0;
        fIncrDecRowBytes = 0;
...
SkCodec::Result SkWuffsCodec::onIncrementalDecodeOnePass(int* rowsDecoded) {
...
SkCodec::Result SkWuffsCodec::onIncrementalDecodeTwoPass(int* rowsDecoded) {
