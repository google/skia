...
static SkCodec::Result reset_and_decode_image_config(wuffs_gif__decoder*       decoder,
    ...
    if (status.repr != nullptr) {
        SkCodecPrintf("initialize: %s", status.repr);
    ...
    while (true) {
        ...
        if (status.repr == nullptr) {
            ...
            SkCodecPrintf("decode_image_config: %s", status.repr);
...
SkCodec::Result SkWuffsCodec::onStartIncrementalDecode(const SkImageInfo&      dstInfo,
...
#ifdef SK_FAVOR_WUFFS_V_0_3_OVER_V_0_2
    wuffs_base__pixel_format pixelFormat = wuffs_base__make_pixel_format(WUFFS_BASE__PIXEL_FORMAT__INVALID);
#else
    ...
    switch (dstInfo.colorType()) {
        case kBGRA_8888_SkColorType:
#ifdef SK_FAVOR_WUFFS_V_0_3_OVER_V_0_2
            pixelFormat = wuffs_base__make_pixel_format(WUFFS_BASE__PIXEL_FORMAT__BGRA_NONPREMUL);
#else
        ...
        case kRGBA_8888_SkColorType:
#ifdef SK_FAVOR_WUFFS_V_0_3_OVER_V_0_2
            pixelFormat = wuffs_base__make_pixel_format(WUFFS_BASE__PIXEL_FORMAT__RGBA_NONPREMUL);
#else
    ...
    fIncrDecOnePass =
#ifdef SK_FAVOR_WUFFS_V_0_3_OVER_V_0_2
        (pixelFormat.repr != WUFFS_BASE__PIXEL_FORMAT__INVALID) &&
#else
...
#ifdef SK_FAVOR_WUFFS_V_0_3_OVER_V_0_2
    pixelConfig.set(pixelFormat.repr, WUFFS_BASE__PIXEL_SUBSAMPLING__NONE, dstInfo.width(),
                    dstInfo.height());
#else
...
#ifdef SK_FAVOR_WUFFS_V_0_3_OVER_V_0_2
    if (status.repr != nullptr) {
        SkCodecPrintf("set_from_table: %s", status.repr);
        return SkCodec::kInternalError;
    }
#else
    if (status != nullptr) {
...
#ifdef SK_FAVOR_WUFFS_V_0_3_OVER_V_0_2
    if (status.repr != nullptr) {
        SkCodecPrintf("set_from_slice: %s", status.repr);
        return SkCodec::kInternalError;
    }
#else
    if (status != nullptr) {
...
const char* SkWuffsCodec::decodeFrame(WhichDecoder which) {
    while (true) {
        ...
        wuffs_base__status status = fDecoders[which]->decode_frame(
            &fPixelBuffer, &fIOBuffer, WUFFS_BASE__PIXEL_BLEND__SRC, wuffs_base__make_slice_u8(fWorkbufPtr.get(), fWorkbufLen),
            NULL);
        if ((status.repr == wuffs_base__suspension__short_read) &&
            fill_buffer(&fIOBuffer, fStream.get())) {
            continue;
