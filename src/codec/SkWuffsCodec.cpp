...
private:
    ...
    // The highest number of fully received frames during all previous runs
    // through the input data. The value is updated whenever we might seek
    // backwards, during seekFrame.
    //
    // A Wuffs decoder runs sequentially through the input. As it proceeds, it
    // sees (either explicitly decodes or implicitly skips) more and more
    // frames. The way Skia uses Wuffs, we sometimes seek backwards, starting a
    // new run. Over time, there are multiple only-moves-forwards runs through
    // the input, and some of them get further (see more frames) than others.
    //
    // The numFullyReceived method returns the highest number of frames seen,
    // "fully received", either from the current run or from a previous run. We
    // don't need to store the number of frames seen for all previous runs,
    // only of the best (furthest along) run. This field is that best value.
    //
    // The value from the current run is "fDecoder->num_decoded_frames()".
...
SkCodec::Result SkWuffsCodec::seekFrame(int frameIndex) {
    ...
    uint64_t current = wuffs_gif__decoder__num_decoded_frames(fDecoder.get());
