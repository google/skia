...
SkCodec::Result SkWuffsCodec::onIncrementalDecode(int* rowsDecoded) {
    ...
    if (!dirty_rect.is_empty()) {
        ...
        SkRect srcRect = SkRect::MakeLTRB(dirty_rect.min_incl_x, dirty_rect.min_incl_y,
                                          dirty_rect.max_excl_x, dirty_rect.max_excl_y);
        SkRect dstRect;
        matrix.mapRect(&dstRect, srcRect);
        srcRect.offsetTo(0, 0);
        canvas->drawBitmapRect(src, srcRect, dstRect, &paint);
    }
