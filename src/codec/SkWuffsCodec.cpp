...
SkCodec::Result SkWuffsCodec::onStartIncrementalDecode(const SkImageInfo&      dstInfo,
    ...
    if (options.fFrameIndex > 0 && dstInfo.colorType() == kRGB_565_SkColorType) {
...
SkCodec::Result SkWuffsCodec::onIncrementalDecode(int* rowsDecoded) {
    ...
    if (!dirty_rect.is_empty()) {
        ...
        const auto& encodedInfo = this->getEncodedInfo();
        const skcms_ICCProfile* srcProfile = encodedInfo.profile();
        if (srcProfile && !SkColorSpace::Make(*srcProfile)) {
            // The skcms_ICCProfile cannot be represented as an SkColorSpace. Convert it to SRGB
            // before drawing. Convert in place.
            const auto srcFormat = encodedInfo.color() == SkEncodedInfo::kBGRA_Color
                    ? skcms_PixelFormat_BGRA_8888 : skcms_PixelFormat_RGBA_8888;
            uint8_t* p = s;
            for (auto y = dirty_rect.min_incl_y; y < dirty_rect.max_excl_y; y++, p += pixels.stride) {
                if (!skcms_Transform(p, srcFormat, skcms_AlphaFormat_Unpremul, srcProfile,
                                     p, srcFormat, skcms_AlphaFormat_Unpremul, skcms_sRGB_profile(),
                                     dirty_rect.width())) {
                    return SkCodec::kInvalidConversion;
                }
            }
        ...
        if (independent) {
            paint.init()->setBlendMode(SkBlendMode::kSrc);
        }

        auto canvas = SkCanvas::MakeRasterDirect(dstInfo(), fIncrDecDst, fIncrDecRowBytes);
        if (!canvas) {
            return SkCodec::kInternalError;
        }
        ...
        canvas->drawBitmap(src, dirty_rect.min_incl_x, dirty_rect.min_incl_y, paint.getMaybeNull());
    }
