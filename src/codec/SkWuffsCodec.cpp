...
SkCodec::Result SkWuffsCodec::resetDecoder() {
    ...
    memset(fDecoder.get(), 0, sizeof__wuffs_gif__decoder());
    if (fDecoder->check_wuffs_version(sizeof__wuffs_gif__decoder(), WUFFS_VERSION) != nullptr) {
        return SkCodec::kInternalError;
    }
    while (true) {
...
std::unique_ptr<SkCodec> SkWuffsCodec_MakeFromStream(std::unique_ptr<SkStream> stream,
    ...
    // the struct at compile time). Instead, we use C's malloc (or calloc),
    // with sizeof__wuffs_gif__decoder returning the appropriate value for the
    ...
    // destructors.
    //
    // In RAII style, we can still use std::unique_ptr with these malloc'ed
    // pointers, but we pair the pointer with C's free instead of C++'s delete.
    void* decoder_raw = calloc(1, sizeof__wuffs_gif__decoder());
    if (!decoder_raw) {
        *result = SkCodec::kInternalError;
        return nullptr;
    }
    std::unique_ptr<wuffs_gif__decoder, decltype(&free)> decoder(
        reinterpret_cast<wuffs_gif__decoder*>(decoder_raw), &free);
    if (decoder->check_wuffs_version(sizeof__wuffs_gif__decoder(), WUFFS_VERSION) != nullptr) {
        *result = SkCodec::kInternalError;
        return nullptr;
    }
    ...
    void* workbuf_ptr_raw = workbuf_len <= SIZE_MAX ? malloc(workbuf_len) : nullptr;
    if (!workbuf_ptr_raw) {
        *result = SkCodec::kInternalError;
        return nullptr;
    }
    std::unique_ptr<uint8_t, decltype(&free)> workbuf_ptr(
        reinterpret_cast<uint8_t*>(workbuf_ptr_raw), &free);

    uint64_t pixbuf_len = imgcfg.pixcfg.pixbuf_len();
    void* pixbuf_ptr_raw = pixbuf_len <= SIZE_MAX ? malloc(pixbuf_len) : nullptr;
    if (!pixbuf_ptr_raw) {
        *result = SkCodec::kInternalError;
        return nullptr;
    }
    std::unique_ptr<uint8_t, decltype(&free)> pixbuf_ptr(reinterpret_cast<uint8_t*>(pixbuf_ptr_raw),
                                                         &free);
