...
int SkWuffsCodec::onGetFrameCount() {
    ...
    // memory to store these I/O positions.
    //
    // Wuffs and SkWuffsCodec tries to minimize relying on the rewindable /
    // seekable assumption. For example, by design, Wuffs per se certainly aims
    // for O(1) memory use (after any pixel buffers are allocated) instead of
    // O(numberOfFrames), and its I/O type, wuffs_base__io_buffer, is not
    // necessarily rewindable or seekable.
    //
    // The Wuffs API provides a limited form of seeking, to the start of an
    // animation frame's data, but does not provide arbitrary save and load of
    // its internal state whilst in the middle of an animation frame.
