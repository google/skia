...
SkCodec::Result SkWebpCodec::onGetPixels(const SkImageInfo& dstInfo, void* dst, size_t rowBytes,
    ...
    SkRasterPipeline convert_to_dst;
    ...
    if (blendWithPrevFrame) {
        ...
        if (xformAlphaType == kUnpremul_SkAlphaType) {
            ...
            convert_to_dst.append(SkRasterPipeline::unpremul);
        }
        if (needs_srgb_to_linear) {
            convert_to_dst.append(SkRasterPipeline::to_srgb);
        }
    }
    ...
    if (this->colorXform()) {
        ...
        if (blendWithPrevFrame) {
            ...
            pick_memory_stages(dstInfo.colorType(), &load_dst, &store_dst);
        } else {
            xformDst = dst;
        }
