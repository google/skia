...
static void blend_line(SkColorType dstCT, void* dst,
    ...
    SkRasterPipeline p;
    SkRasterPipeline::StockStage load, store;
    pick_memory_stages(dstCT, &load, &store);
    ...
    // Load the src.
    // Although |load| is based on the dstCT, srcCT is either the
    // same or the opposite swizzle for 8888.
    p.append(load, src);
    if (dstCT != srcCT) {
        SkASSERT(is_8888(dstCT) && is_8888(srcCT));
