...
// Requires that the src input be unpremultiplied.
...
SkCodec::Result SkWebpCodec::onGetPixels(const SkImageInfo& dstInfo, void* dst, size_t rowBytes,
    ...
    // Order of priority for premultiplying
    // - blend_line, in a linear space, followed by unpremul if necessary
    // - colorXform
    // - libwebp
    const bool needPremul = frame.has_alpha &&
        !blendWithPrevFrame && dstInfo.alphaType() == kPremul_SkAlphaType;
    const bool webpPremultiply = needPremul && !this->colorXform();
    config.output.colorspace = webp_decode_mode(webpInfo.colorType(), webpPremultiply);
    ...
    if (this->colorXform()) {
        ...
        const auto xformAlphaType = needPremul ? kPremul_SkAlphaType :
            frame.has_alpha ? kUnpremul_SkAlphaType : kOpaque_SkAlphaType;
