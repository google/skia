...
SkCodec::Result SkWebpCodec::onGetPixels(const SkImageInfo& dstInfo, void* dst, size_t rowBytes,
    ...
    if (this->colorXform()) {
        ...
        skcms_AlphaFormat srcAlpha, dstAlpha;
        if (!frame.has_alpha) {
            srcAlpha = dstAlpha = skcms_AlphaFormat_Opaque;
        } else if (premulStep == kColorXform) {
            srcAlpha = skcms_AlphaFormat_Unpremul;
            dstAlpha = skcms_AlphaFormat_PremulAsEncoded;
        } else {
            SkASSERT(premulStep == kNone);
            srcAlpha = skcms_AlphaFormat_Unpremul;
            dstAlpha = skcms_AlphaFormat_Unpremul;
        }

        for (int y = 0; y < rowsDecoded; y++) {
            this->applyColorXform(xformDst, xformSrc, scaledWidth, srcAlpha, dstAlpha);
            if (blendWithPrevFrame) {
