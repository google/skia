...
static void pick_stages(SkColorType ct, SkRasterPipeline::StockStage* load,
...
SkCodec::Result SkWebpCodec::onGetPixels(const SkImageInfo& dstInfo, void* dst, size_t rowBytes,
    ...
    const bool blendWithPrevFrame = requiredFrame != kNone && frame.blend_method == WEBP_MUX_BLEND
        && frame.has_alpha;
#if 0
    // FIXME: I need to rebase to have this method.
    if (blendWithPrevFrame && !SkImageInfoIsValidRenderingCS(dstInfo)) {
    ...
    // These variables are only used when blending with the previous frame.
    SkRasterPipeline convert_to_linear_premul;
    SkRasterPipeline unpremul_if_necessary;

    if (blendWithPrevFrame) {
        const bool needs_srgb_to_linear = dstInfo.gammaCloseToSRGB() &&
                options.fPremulBehavior == SkTransferFunctionBehavior::kRespect;
        if (needs_srgb_to_linear) {
            convert_to_linear_premul.append_from_srgb(xformAlphaType);
        }
        if (xformAlphaType == kUnpremul_SkAlphaType) {
            // srcover assumes premultiplied inputs.
            // FIXME: If the background is opaque, no need to premul,
            // though it will not yield incorrect results. (The new
            // frame should not be opaque, or there is no need to
            // blend.)
            convert_to_linear_premul.append(SkRasterPipeline::premul);

            unpremul_if_necessary.append(SkRasterPipeline::unpremul);
        }
        if (needs_srgb_to_linear) {
            unpremul_if_necessary.append(SkRasterPipeline::to_srgb);
        }
    }
    ...
    if (this->colorXform()) {
        ...
        if (blendWithPrevFrame) {
            ...
            pick_stages(dstInfo.colorType(), &load_dst, &store_dst);
        } else {
            xformDst = dst;
        }
        for (int y = 0; y < rowsDecoded; y++) {
            ...
            if (blendWithPrevFrame) {
                ...
                // Load final dst into pipeline's dst
                p.append(load_dst, &dst);
                p.extend(convert_to_linear_premul);
                p.append(SkRasterPipeline::move_src_dst);

                // Load xform's dst into pipeline's src
                p.append(load_dst, &xformDst);
                p.extend(convert_to_linear_premul);

                p.append(SkRasterPipeline::srcover);
                p.extend(unpremul_if_necessary);
        ...
        for (int y = 0; y < rowsDecoded; y++) {
            ...
            // Load the src, which is always BGRA. Pipeline expects RGBA.
            p.append(SkRasterPipeline::load_8888, &src);
            SkASSERT(webpDst.colorType() == kBGRA_8888_SkColorType);
            p.append(SkRasterPipeline::swap_rb);
            p.extend(convert_to_linear_premul);
