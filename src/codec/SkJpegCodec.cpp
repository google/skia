...
static bool is_yuv_supported(const jpeg_decompress_struct* dinfo,
    ...
    SkYUVAInfo::Planes tempPlanes;
    if        (1 == hSampY && 1 == vSampY) {
        tempPlanes = SkYUVAInfo::Planes::kY_U_V_444;
    } else if (2 == hSampY && 1 == vSampY) {
        tempPlanes = SkYUVAInfo::Planes::kY_U_V_422;
    } else if (2 == hSampY && 2 == vSampY) {
        tempPlanes = SkYUVAInfo::Planes::kY_U_V_420;
    } else if (1 == hSampY && 2 == vSampY) {
        tempPlanes = SkYUVAInfo::Planes::kY_U_V_440;
    } else if (4 == hSampY && 1 == vSampY) {
        tempPlanes = SkYUVAInfo::Planes::kY_U_V_411;
    } else if (4 == hSampY && 2 == vSampY) {
        tempPlanes = SkYUVAInfo::Planes::kY_U_V_410;
    } else {
        return false;
    }
    if (planes) {
...
bool SkJpegCodec::onGetYUVAInfo(SkYUVAInfo* yuvaInfo,
                                SkColorType colorTypes[SkYUVAInfo::kMaxPlanes],
    ...
    yuvaInfo->fDimensions = this->dimensions();
    yuvaInfo->fPlanes = planes;
    yuvaInfo->fYUVColorSpace = kJPEG_Full_SkYUVColorSpace;
    yuvaInfo->fOrigin = this->getOrigin();
    yuvaInfo->fSitingX = yuvaInfo->fSitingY = SkYUVAInfo::Siting::kCentered;
    return true;
}
