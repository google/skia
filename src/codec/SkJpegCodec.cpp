...
bool SkJpegCodec::onQueryYUV8(SkYUVASizeInfo* sizeInfo, SkYUVColorSpace* colorSpace) const {
    ...
    for (int i = 0; i < 3; ++i) {
        ...
        sizeInfo->fWidthBytes1[i] = comp_info[i].width_in_blocks * DCTSIZE;
    }

    // JPEG never has an alpha channel
    sizeInfo->fColorTypes[3] = kUnknown_SkColorType;
    sizeInfo->fSizes[3].fHeight = sizeInfo->fSizes[3].fWidth = sizeInfo->fWidthBytes1[3] = 0;
...
SkCodec::Result SkJpegCodec::onGetYUV8Planes(const SkYUVASizeInfo& sizeInfo, void* planes[3]) {
    ...
    if (!supportsYUV ||
            ...
            sizeInfo.fWidthBytes1[0] < defaultInfo.fWidthBytes1[0] ||
            sizeInfo.fWidthBytes1[1] < defaultInfo.fWidthBytes1[1] ||
            sizeInfo.fWidthBytes1[2] < defaultInfo.fWidthBytes1[2]) {
    ...
    for (int i = 0; i < numYRowsPerBlock; i++) {
        rowptrs[i] = SkTAddOffset<JSAMPLE>(planes[0], i * sizeInfo.fWidthBytes1[0]);
    }
    for (int i = 0; i < DCTSIZE; i++) {
        rowptrs[i + 2 * DCTSIZE] = SkTAddOffset<JSAMPLE>(planes[1], i * sizeInfo.fWidthBytes1[1]);
        rowptrs[i + 3 * DCTSIZE] = SkTAddOffset<JSAMPLE>(planes[2], i * sizeInfo.fWidthBytes1[2]);
    }

    // After each loop iteration, we will increment pointers to Y, U, and V.
    size_t blockIncrementY = numYRowsPerBlock * sizeInfo.fWidthBytes1[0];
    size_t blockIncrementU = DCTSIZE * sizeInfo.fWidthBytes1[1];
    size_t blockIncrementV = DCTSIZE * sizeInfo.fWidthBytes1[2];
    ...
    if (remainingRows > 0) {
        ...
        SkAutoTMalloc<JSAMPLE> dummyRow(sizeInfo.fWidthBytes1[0]);
