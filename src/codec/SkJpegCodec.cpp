...
bool SkJpegCodec::ReadHeader(SkStream* stream, SkCodec** codecOut,
    ...
    if (codecOut) {
        ...
        if (iccData) {
            SkColorSpace_Base::InputColorFormat inputColorFormat = SkColorSpace_Base::InputColorFormat::kRGB;
            ...
            colorSpace = SkColorSpace_Base::MakeICC(iccData->data(), iccData->size(), inputColorFormat);
            if (!colorSpace) {
...
SkCodec::Result SkJpegCodec::onGetPixels(const SkImageInfo& dstInfo,
    ...
    if (JCS_CMYK == colorSpace && nullptr == colorXform()) {
