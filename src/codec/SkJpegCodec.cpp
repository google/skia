...
void SkJpegCodec::setRange(int firstRow, int lastRow, void* dst, size_t rowBytes) {
    fFirstRow = firstRow;
    fLastRow = lastRow;
    fDst = dst;
...
SkCodec::Result SkJpegCodec::onStartIncrementalDecode(const SkImageInfo& dstInfo,
    ...
    setRange(firstRow, lastRow, pixels, dstRowBytes);
...
SkCodec::Result SkJpegCodec::onIncrementalDecode(int* rowsDecoded) {
    ...
    // Get fRowsNeeded and fLinesSkipped on first call of incremental decode
    if (fFirstCallToIncrementalDecode) {
        const int samplingOffsetY = get_start_coord(sampleY);
        const int subsetY = options().fSubset ? options().fSubset->fTop : 0;
        fLinesSkipped = samplingOffsetY + subsetY;
    ...
    if (fLinesSkipped) {
        fLinesSkipped -= jpeg_skip_scanlines(dinfo, fLinesSkipped);
        if(fLinesSkipped)
            return SkCodec::kIncompleteInput;
    }

    // Decode row by row of lines here
    while (fRowsWrittenToOutput < fRowsNeeded) {
