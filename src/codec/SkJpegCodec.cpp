...
SkCodec::Result SkJpegCodec::readRows(int count) {
...
SkCodec::Result SkJpegCodec::onGetPixels(const SkImageInfo& dstInfo,
    ...
    result = readRows(dstInfo.height());
...
SkCodec::Result SkJpegCodec::onIncrementalDecode(int* rowsDecoded) {
    ...
    Result result;
    if (sampleY == 1) {
        result = this->readRows(fRowsNeeded - fRowsWrittenToOutput);
    } else {
        while (true) {
            result = this->readRows(1);
