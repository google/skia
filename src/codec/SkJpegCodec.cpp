...
SkCodec::Result SkJpegCodec::readRows(int count, int* rowsDecoded) {
    ...
    uint32_t* xformDst = (uint32_t*) fDst;
    ...
    for (int y = 0; y < count; y++) {
        ...
        if (this->colorXform()) {
            this->applyColorXform(xformDst, swizzleDst, dstWidth, kOpaque_SkAlphaType);
            xformDst = SkTAddOffset<uint32_t>(xformDst, fDstRowBytes);
        }
...
SkCodec::Result SkJpegCodec::onIncrementalDecode(int* rowsDecoded) {
    ...
    if (fLinesToSkip) {
        // FIXME: This does not work for suspending data sources. skbug.com/4036
    ...
    while (true) {
        ...
        if (sampleY == 1) {
            result = readRows(fRowsNeeded - fRowsWrittenToOutput, &currRowsDecoded);
        } else {
            result = readRows(1, &currRowsDecoded);
        }

        fRowsWrittenToOutput += currRowsDecoded;
        if (rowsDecoded) {
        ...
        if (result != kSuccess) {
            return result;
        } else if (fRowsWrittenToOutput == fRowsNeeded) {
            break;
        }
