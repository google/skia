...
#define kMirrorX    SkPixmap::kMirrorX_OrientFlag
#define kMirrorY    SkPixmap::kMirrorY_OrientFlag
#define kSwapXY     SkPixmap::kSwapXY_OrientFlag
...
static SkPixmap::OrientFlags orientation_to_flags(SkCodec::Origin o) {
    unsigned io = static_cast<int>(o) - 1;
    SkASSERT(io < SK_ARRAY_COUNT(gOrientationFlags));
    return static_cast<SkPixmap::OrientFlags>(gOrientationFlags[io]);
}
...
bool SkCodecImageGenerator::onGetPixels(const SkImageInfo& requestInfo, void* requestPixels,
    ...
    switch (result) {
        case SkCodec::kSuccess:
            if (codecMap != &request) {
                return codecMap->orientTo(request, orientation_to_flags(origin));
            }
            // fall through
