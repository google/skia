...
protected:
    ...
    // Handle the next task available on a thread. This method will block until
    //   (1) the next task is finished, or
    //   (2) the finish is called
    // The method will return true for case (1) and false for case (2).
    // When there's no task available and we haven't called finish, we will just wait.
...
protected:
    bool next(int threadId) override;
...
private:
    ...
    struct alignas(MAX_CACHE_LINE) ThreadData {
        ...
        int fRowIndex; // the tile that the current thread is working on
    };
