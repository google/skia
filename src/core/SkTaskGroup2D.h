...
// that the task on the same row will be executed in order (i.e., WorkFn(1, 1, t) is guaranteed to
// finish before calling WorkFn(1, 2, t)). Tasks in different rows can happen in any order.
//
...
// needs to call InitFn(j, t) once to unblock all rows that depend on the initialization of column
// j. (Again, t shouldn't affect the init work to be done; it's just for some non-thread-safe
...
public:
    ...
    // Return false if no initialization is done for this colum.
