...
class SkEffect_Lerp final : public SkEffect {
    SkEffect_Lerp(float weight, sk_sp<SkEffect> e0, sk_sp<SkEffect> e1)
        : fE0(std::move(e0))
        , fE1(std::move(e1))
        , fWeight(weight)
   {
...
public:
    SK_FLATTENABLE_HOOKS(SkEffect_Lerp)
    ...
    bool onAppendStages(const SkStageRec& rec) const override {
        ...
        if (!append_effect_or_paint(rec, fE0.get())) {
            return false;
        }
        rec.fPipeline->append(SkRasterPipeline::store_src, storage->fRGBA);

        if (!append_effect_or_paint(rec, fE1.get())) {
            return false;
        }
        // r,g,b,a are good, as output by fE1
        // need to restore our previously computed color to dr,dg,db,da
...
sk_sp<SkFlattenable> SkEffect_Lerp::CreateProc(SkReadBuffer& buffer) {
    ...
    return SkEffect::Lerp(t, std::move(e0), std::move(e1));
}

sk_sp<SkEffect> SkEffect::Lerp(float t, sk_sp<SkEffect> e0, sk_sp<SkEffect> e1) {
    if (SkScalarIsNaN(t)) {
    ...
    return sk_sp<SkEffect>(new SkEffect_Lerp(t, std::move(e0), std::move(e1)));
}
