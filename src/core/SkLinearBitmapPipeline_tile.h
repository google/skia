...
public:
    ...
    void tileXPoints(Sk4s* xs) {
        *xs = Sk4s::Min(Sk4s::Max(*xs, 0.5f), fXMaxPixel);
    ...
    bool maybeProcessSpan(Span originalSpan, Next* next) {
        ...
        if (dx >= 0) {
            Span leftClamped = span.breakAt(0.5f, dx);
            if (!leftClamped.isEmpty()) {
                leftClamped.clampToSinglePixel({0.5f, y});
            ...
            Span center = span.breakAt(0.5f, dx);
            if (!center.isEmpty()) {
                next->pointSpan(center);
            }
            if (!span.isEmpty()) {
                span.clampToSinglePixel({0.5f, y});
                next->pointSpan(span);
            }
        }
        return true;
...
public:
    YClampStrategy(int32_t max)
        : fYMaxPixel{SkScalar(max) - 0.5f} { }

    void tileYPoints(Sk4s* ys) {
        *ys = Sk4s::Min(Sk4s::Max(*ys, 0.5f), fYMaxPixel);
        assertTiled(*ys, fYMaxPixel + 0.5f);
    }
