...
void SkOverdrawCanvas::onDrawTextHelper(const void* text, size_t byteLength, const SkScalar pos[],
...
void SkOverdrawCanvas::onDrawPosText(const void* text, size_t byteLength, const SkPoint pos[],
                                     const SkPaint& paint) {
    this->onDrawTextHelper(text, byteLength, (SkScalar*) pos, 2, SkPoint::Make(0, 0), paint);
}

void SkOverdrawCanvas::onDrawPosTextH(const void* text, size_t byteLength, const SkScalar xs[],
                                      SkScalar y, const SkPaint& paint) {
    this->onDrawTextHelper(text, byteLength, (SkScalar*) xs, 1, SkPoint::Make(0, y), paint);
}
    ...
    for (;!it.done(); it.next()) {
        ...
        switch (it.positioning()) {
            ...
            case SkTextBlob::kHorizontal_Positioning:
                this->onDrawTextHelper(it.glyphs(), textLen, it.pos(), 1,
                                       SkPoint::Make(x, y + offset.y()), runPaint);
                break;
            case SkTextBlob::kFull_Positioning:
                this->onDrawTextHelper(it.glyphs(), textLen, it.pos(), 2, SkPoint::Make(x, y),
                                       runPaint);
                break;
            default:
                SkASSERT(false);
