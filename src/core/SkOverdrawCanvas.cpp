...
void SkOverdrawCanvas::onDrawEdgeAAQuad(const SkRect& rect, const SkPoint clip[], int clipCount,
                                        SkCanvas::QuadAAFlags aa, SkColor color, SkBlendMode mode) {
    if (clipCount > 0) {
        SkPath path;
        path.addPoly(clip, clipCount, true);
        fList[0]->onDrawPath(path, fPaint);
    } else {
...
void SkOverdrawCanvas::onDrawEdgeAAImageSet(const ImageSetEntry set[], int count,
    ...
    for (int i = 0; i < count; ++i) {
        ...
        if (set[i].fDstClipCount > 0) {
            SkPath path;
            path.addPoly(dstClips + clipIndex, set[i].fDstClipCount, true);
            clipIndex += set[i].fDstClipCount;
            fList[0]->onDrawPath(path, fPaint);
        } else {
