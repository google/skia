...
// starting with an allocation of extraSpace bytes.  If your data (plus a small overhead) fits in
// the user-provided block, SkArenaAlloc never uses the heap, and if it fits in extra_size bytes,
// it'll use the heap only once.  If you pass extraSpace = 0, it allocates blocks for each call to
// make<T>.
//
// Examples:
//
...
public:
    ...
    T* makeArray(size_t count) {
        ...
        // If T is primitive then the memory is initialized. In other words, an array of chars will
        // be zeroed.
        return new(objStart) T[count]();
    }
