...
GrTexture* SkImageCacherator::lockTexture(GrContext* ctx,
                                          const GrUniqueKey& key, const SkImage* client,
                                          SkImage::CachingHint chint, bool willBeMipped,
                                          SkColorSpace* dstColorSpace) {
    sk_sp<GrTextureProxy> proxy = this->lockTextureProxy(ctx, key, client, chint, willBeMipped, 
                                                         dstColorSpace);
    if (!proxy) {
        return nullptr;
    }

    return proxy->instantiate(ctx->resourceProvider());
}
