...
SkImageCacherator::Validator::Validator(sk_sp<SharedGenerator> gen, const SkIRect* subset)
    ...
    fEncodedInfo = info.makeWH(subset->width(), subset->height());
    ...
    if (fEncodedInfo.colorSpace() &&
        SkColorSpace_Base::Type::kXYZ != as_CSB(fEncodedInfo.colorSpace())->type()) {
        fInfo = fEncodedInfo
...
SkImageCacherator::SkImageCacherator(Validator* validator)
    ...
    , fEncodedInfo(validator->fEncodedInfo)
    ...
    switch (fInfo.colorType()) {
        ...
        case kRGBA_8888_SkColorType:
            if (caps.supportsSRGB()) {
                ...
                SkASSERT(caps.supportsHalfFloat());
                return kLinearF16_CachedFormat;
            }
