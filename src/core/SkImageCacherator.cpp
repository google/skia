...
sk_sp<SkColorSpace> SkImageCacherator::getColorSpace(GrContext* ctx,
    ...
    // format, and will (soon) emit them in an arbitrary (destination) space. We will need to
    // stash that information in/on the key so we can return the correct space in case #1.
