...
bool SkImageCacherator::lockAsBitmap(GrContext* context,
                                     SkBitmap* bitmap, const SkImage* client,
    ...
    sk_sp<GrSurfaceContext> sContext(context->contextPriv().makeWrappedSurfaceContext(
                                                                proxy,
                                                                sk_ref_sp(cacheInfo.colorSpace())));
    if (!sContext) {
        bitmap->reset();
        return false;
    }
...
static void set_key_on_proxy(GrTextureProvider* texProvider,
                             GrTextureProxy* proxy, const GrUniqueKey& key) {
    if (key.isValid()) {
        texProvider->assignUniqueKeyToProxy(key, proxy);
    }
}
...
sk_sp<GrTextureProxy> SkImageCacherator::lockTexture(GrContext* ctx, const GrUniqueKey& origKey,
    ...
    // 1. Check the cache for a pre-existing one
    if (key.isValid()) {
        if (sk_sp<GrTextureProxy> proxy = ctx->textureProvider()->findProxyByUniqueKey(key)) {
    ...
    // 2. Ask the generator to natively create one
    {
        ...
        if (sk_sp<GrTextureProxy> proxy = generator->generateTexture(ctx, cacheInfo, fOrigin)) {
            SK_HISTOGRAM_ENUMERATION("LockTexturePath", kNative_LockTexturePath,
                                     kLockTexturePathCount);
            set_key_on_proxy(ctx->textureProvider(), proxy.get(), key);
            return proxy;
        }
    }
    ...
    if (!ctx->contextPriv().disableGpuYUVConversion()) {
        ...
        sk_sp<GrTextureProxy> proxy = provider.refAsTexture(ctx, desc, true);
        if (proxy) {
