    ...
    for (int i = 0; i < vCount; ++i) {
        switch (verbs[i]) {
            ...
            case SkPath::kLine_Verb:
                ptCount = safe.add31(ptCount, 1);
                break;
            case SkPath::kConic_Verb:
                conicCount += 1;
                // fall-through
            case SkPath::kQuad_Verb:
                ptCount = safe.add31(ptCount, 2);
                break;
            case SkPath::kCubic_Verb:
                ptCount = safe.add31(ptCount, 3);
                break;
...
SkPoint* SkPathRef::growForVerb(int /* SkPath::Verb*/ verb, SkScalar weight) {
    ...
    SkSafeMath safe;
    int newPointCnt = safe.add31(fPointCnt, pCnt);
    int newVerbCnt  = safe.add31(fVerbCnt, 1);
    if (!safe) {
