...
// Given the verb array, deduce the required number of pts and conics, or if an invalid verb is encountered,
// return false.
static bool deduce_pts_conics(const uint8_t verbs[], int vCount, int* ptCountPtr, int* conicCountPtr) {
    int ptCount = 0;
    int conicCount = 0;
...
SkPathRef* SkPathRef::CreateFromBuffer(SkRBuffer* buffer) {
    ...
    {
        int pCount, cCount;
        if (!deduce_pts_conics(ref->fVerbs, ref->countVerbs(), &pCount, &cCount) ||
