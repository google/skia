...
static bool deduce_pts_conics(const uint8_t verbs[], int vCount, int* ptCountPtr,
    ...
    if (vCount > 0 && verbs[0] != SkPath::kMove_Verb) {
        return false;
    }
...
SkPathRef* SkPathRef::CreateFromBuffer(SkRBuffer* buffer) {
    ...
    if (!buffer->read(ref->fVerbs.begin(), verbSize) ||
        !buffer->read(ref->fPoints.begin(), pointSize) ||
    ...
    {
        int pCount, cCount;
        if (!validate_verb_sequence(ref->fVerbs.begin(), ref->fVerbs.count())) {
            return nullptr;
        }
        if (!deduce_pts_conics(ref->fVerbs.begin(), ref->fVerbs.count(), &pCount, &cCount) ||
