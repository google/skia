...
void SkPathRef::CreateTransformedCopy(sk_sp<SkPathRef>* dst,
    ...
    // The following is a little optimization to compute fracCount as we map the points so we don't
    // have to iterate through the points array twice. The fracCount is for deciding if DAA (and
    // maybe other AA technique that cumulates coverage without considerinng overlap/intersection)
    // may have artifacts like skbug.com/6886
