...
public:
    ...
    T* make(Args&&... args) {
        if constexpr (std::is_trivially_destructible<T>::value) {
            ...
            return this->adopt<T>(std::forward<Args>(args)...);
        }
    }
...
private:
    ...
    template<typename T> struct ObjectOwner final : public Owner {
        template<typename... Args> ObjectOwner(NextHeader next, Args&&... args)
    ...
    template<typename T, typename... Args>
    T* adopt(Args&&... args) {
        auto adapter = this->makeUnique<ObjectOwner<T>>(
                std::move(fOwnerList), std::forward<Args>(args)...);
        T* result = &adapter->object;
        fOwnerList = std::move(adapter);
        return result;
    }
    ...
    T* adoptArray(int count, Args&&... args) {
        ...
        T* result = array.get();
        auto adaptor = this->makeUnique<ArrayOwner<T>>(std::move(fOwnerList), std::move(array));
        fOwnerList = std::move(adaptor);
        return result;
    }
