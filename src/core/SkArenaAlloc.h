...
public:
    SkFibBlockSizes(uint32_t size, uint32_t firstAllocationSize) : fIndex{0} {
        fFirstAllocateSize = firstAllocationSize > 0 ? firstAllocationSize :
                             size                > 0 ? size                : 1024;

        SkASSERT_RELEASE(0 < fFirstAllocateSize);
        SkASSERT_RELEASE(fFirstAllocateSize < std::min(kMaxSize, (1u << 26) - 1));
    }

    uint32_t nextBlockSize() {
        uint32_t result = SkFibonacci47[fIndex] * fFirstAllocateSize;

        if (fIndex + 1 < SkTo<uint32_t>(SkFibonacci47.size()) &&
            SkFibonacci47[fIndex + 1] < kMaxSize / fFirstAllocateSize)
        {
            fIndex += 1;
        }
...
private:
    uint32_t fIndex : 6;
    uint32_t fFirstAllocateSize : 26;
};
