...
public:
    ...
    static constexpr ptrdiff_t kMaxAlignmentMask = kMaxAlignment - 1;
    ...
    T* makePOD(Args&&... args) {
        ...
        fCapacity = (fCapacity - size) & mask;
        if (fCapacity < 0) {
            this->needMoreBytes(size, alignment, mask);
        }
        return new (fBytes - fCapacity) T {std::forward<Args>(args)...};
    }

    template <typename T, typename... Args>
    SkDestroyerPtr<T> make(Args&&... args) {
        return SkDestroyerPtr<T>{this->template makePOD<T>(std::forward<Args>(args)...)};
    }

private:
    void needMoreBytes(ptrdiff_t size, ptrdiff_t alignment, ptrdiff_t mask);
...
private:
    ...
    char* allocObject(ptrdiff_t size, ptrdiff_t alignment, ptrdiff_t mask) {
        ptrdiff_t alignedOffset = alignment - (reinterpret_cast<ptrdiff_t>(fCursor) & mask);
        if (size > fEnd - fCursor - alignedOffset) {
            ...
            alignedOffset = alignment - (reinterpret_cast<ptrdiff_t>(fCursor) & mask);
        }
