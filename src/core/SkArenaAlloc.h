...
public:
    ...
    SkDestroyerSpan(SkDestroyerSpan&&) = default;
    SkDestroyerSpan& operator=(SkDestroyerSpan&&) = default;
...
private:
    ...
    static constexpr ptrdiff_t kMaxByteSize =
            std::numeric_limits<int32_t>::max() - kMaxAlignment - 32;
...
template<typename T> T* SkArena::makePODArray(size_t size) {
...
template<typename T> SkDestroyerSpan<T> SkArena::makeArray(size_t size) {
    ...
    return SkDestroyerSpan{array};
}

template<typename T, typename I> SkDestroyerSpan<T> SkArena::makeArray(size_t size, I&& initer) {
    static_assert(!std::is_trivially_destructible<T>::value, "This is POD. Use makePODArray.");
    SkSpan<T> array{this->template innerMakeArray<T>(size), size};
    for (auto [i, e] : SkEnumerate(array)) {
        new (&e) T(initer(i));
    }
    return SkDestroyerSpan{array};
}
