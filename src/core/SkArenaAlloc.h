...
private:
    ...
    template<typename T> struct ObjectAdapter final : public Adapter {
        template<typename... Args> ObjectAdapter(NextHeader next, Args&&... args)
                // Pass this to resolve the subclass for Adapter.
...
#elseif 0
    using Action = char* (*)(char*);

    template<typename T, typename DtorPtr>
    T* adapt(DtorPtr object) {
        T* result = object.get();
        struct ActionStorage {
            ActionStorage(DtorPtr p, char* a) : dtorPtr{std::move(p)}, nextAction{a} {}
            ActionStorage(char* storage) { memcpy(this, storage, sizeof(ActionStorage)); }
            DtorPtr dtorPtr;
            char* nextAction;
            void store(SkArena* arena) {
                char* storage = arena->alignedBytes(sizeof(ActionStorage), 1);
                memcpy(storage, this, sizeof(ActionStorage));
                dtorPtr.release();
            }
        };
        auto action = [](char* storage) -> char* {
            ActionStorage act{storage};
            return (char*)act.nextAction;
        };
        auto storeAction = [this](Action action) {
            char* storage = this->alignedBytes(sizeof(Action), 1);
            memcpy(storage, &action, sizeof(Action));
            return (char*)storage;
        };

        ActionStorage act{std::move(object), fNextAction};
        act.store(this);
        fNextAction = storeAction(action);

        return result;
    }

    void wreckDestruction() {
        char* cursor = fNextAction;

        while (cursor != nullptr) {
            Action act;
            memcpy(&act, cursor, sizeof(Action));
            cursor = (act)((char*)cursor + sizeof(Action));
        }
    }

    char* fNextAction{nullptr};
#endif
