...
static size_t custom_data_size(const SkVertices::AttributeType* attrs, int attrCount) {
    size_t size = 0;
    for (int i = 0; i < attrCount; ++i) {
        switch (attrs[i]) {
            case SkVertices::AttributeType::kFloat:       size += 1; break;
            case SkVertices::AttributeType::kFloat2:      size += 2; break;
            case SkVertices::AttributeType::kFloat3:      size += 3; break;
            case SkVertices::AttributeType::kFloat4:      size += 4; break;
            case SkVertices::AttributeType::kByte4_unorm: size += 1; break;
        }
    }
...
SkVertices::Builder::Builder(VertexMode mode,
                             int vertexCount,
                             int indexCount,
                             const SkVertices::AttributeType* attrs,
                             int attrCount) {
...
void SkVertices::Builder::init(const Desc& desc) {
    ...
    sk_careful_memcpy(fVertices->fAttributes, desc.fAttributes,
                      desc.fAttributeCount * sizeof(AttributeType));
...
struct Header_v3 {
    ...
    uint8_t     fAttributes[SkVertices::kMaxCustomAttributes];
    // [pos] + [vertexData] + [texs] + [colors] + [indices]
};
...
sk_sp<SkVertices> SkVertices::Decode(const void* data, size_t length) {
    ...
    AttributeType attrs[kMaxCustomAttributes];
