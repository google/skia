...
struct SkVertices::Sizes {
    Sizes(SkVertices::VertexMode mode, int vertexCount, int indexCount, bool hasTexs, bool hasColors) {
        SkSafeMath safe;
        ...
        if (kTriangleFan_VertexMode == mode) {
            ...
            fISize = safe.mul(numFanTris, 3 * sizeof(uint32_t));
        }
    ...
    // For indexed tri-fans this is the number of temporary indices needed in the builder before
...
void SkVertices::Builder::init(VertexMode mode, int vertexCount, int indexCount,
    ...
    if (sizes.fBuilderTriFanISize) {
        fIntermediateIndices.reset(new uint8_t[sizes.fBuilderTriFanISize]);
    }
...
uint16_t* SkVertices::Builder::indices() {
    ...
    if (fIntermediateIndices) {
        return reinterpret_cast<uint16_t*>(fIntermediateIndices.get());
    }
