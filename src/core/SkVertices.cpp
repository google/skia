...
struct SkVertices::Sizes {
    Sizes(SkVertices::VertexMode mode, int vertexCount, int indexCount, bool hasTexs,
        ...
        fBSize = hasBones ? safe.mul(vertexCount, 4 * sizeof(SkAnimation::Attachment)) : 0;
...
void SkVertices::Builder::init(VertexMode mode, int vertexCount, int indexCount,
    ...
    fVertices->fBones = sizes.fBSize ? (SkAnimation::Attachment*)ptr : nullptr; ptr += sizes.fBSize;
...
SkAnimation::Attachment* SkVertices::Builder::bones() {
    return fVertices ? const_cast<SkAnimation::Attachment*>(fVertices->bones()) : nullptr;
}
...
sk_sp<SkVertices> SkVertices::MakeCopy(VertexMode mode, int vertexCount,
    ...
    sk_careful_memcpy(builder.bones(), bones, sizes.fBSize);
...
// storage = packed | vertex_count | index_count | pos[] | texs[] | colors[] | bones[] | indices[]
//         = header + arrays
...
sk_sp<SkData> SkVertices::encode() const {
    ...
    writer.write(fBones, sizes.fBSize);
...
sk_sp<SkVertices> SkVertices::Decode(const void* data, size_t length) {
    ...
    reader.read(builder.bones(), sizes.fBSize);
    size_t isize = (mode == kTriangleFan_VertexMode) ? sizes.fBuilderTriFanISize : sizes.fISize;
