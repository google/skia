...
sk_sp<SkVertices> SkVertices::Decode(const void* data, size_t length) {
    ...
    if (hasBones) {
        // we just skip the bone data
        struct BoneIndices { uint32_t i[4]; };
        struct BoneWeights { float w[4]; };
        reader.skip(safe_math.mul(vertexCount, sizeof(BoneIndices)));
        reader.skip(safe_math.mul(vertexCount, sizeof(BoneWeights)));
    }
    size_t isize = (mode == kTriangleFan_VertexMode) ? sizes.fBuilderTriFanISize : sizes.fISize;
