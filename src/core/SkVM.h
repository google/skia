...
namespace skvm {
    ...
    public:
        ...
        int liveInstructionCount() const { return fLiveCount; }
        const std::vector<Val> sources() const { return fSources; }
        const std::vector<Val> sinks() const { return fSinks; }

    private:
        std::vector<bool> fLive;
        std::vector<Val> fSources;
        std::vector<Val> fSinks;
        int fLiveCount = 0;
    };
