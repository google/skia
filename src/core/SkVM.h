...
namespace skvm {
    ...
    struct Q14a {
        ...
        Q14a(float f) : imm(f) {
            // f's range will be asserted by the eventual splat_Q14(imm).
        }
    ...
    // FooQ types for use with Q14 color channels,
    // with any 32-bit type double-pumped to match Q14's doubled lane count.
    struct ColorQ {
        Q14 r,g,b,a;
        explicit operator bool() const { return r && g && b && a; }
        Builder* operator->()    const { return a.operator->(); }
