...
namespace skvm {
    ...
    public:
        ...
        // For direct moves between registers.
        // The trivial struct just helps it contrast Mem's pointer indirection.
        struct Direct {
            GP64 reg;
        };
        ...
        struct Operand {
            ...
            Operand(Direct d) : reg  (d.reg), kind(REG  ) {}
        ...
        // TODO: which if any can be Operand?
    ...
    private:
        ...
        // x86-64
        enum W { W0, W1 };   // Are the lanes 64-bit (W1) or less (W0)?
        enum L { L0, L1 };   // Is this a 256-bit operation (L1) or 128-bit (L0)?

        void op(int prefix, int map, int opcode, int dst, int x, Operand y, W,L);

        void op(int p, int m, int o, Ymm dst, Ymm x, Operand y, W w=W0) { op(p,m,o, dst,x,y,w,L1); }
        void op(int p, int m, int o, Ymm dst,        Operand y, W w=W0) { op(p,m,o, dst,0,y,w,L1); }
        void op(int p, int m, int o, Xmm dst, Xmm x, Operand y, W w=W0) { op(p,m,o, dst,x,y,w,L0); }
        void op(int p, int m, int o, Xmm dst,        Operand y, W w=W0) { op(p,m,o, dst,0,y,w,L0); }

        void op(                     int opcode, int opcode_ext, GP64 d       , int imm);
        void op(int prefix, int map, int opcode, int opcode_ext,  Ymm d, Ymm x, int imm);
