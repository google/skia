...
public:
    ...
    // tail() will only be called with tail=1..3 to handle any jagged end of n % 4 pixels.
    ...
    void append(const void* ctx = nullptr) {
        this->append(Body<fn>, Tail<fn>, ctx);
    }

    // If this is the last stage you're going to add, last() is a bit faster than append().
    template <EasyFn fn>
    void last(const void* ctx = nullptr) {
        this->append(BodyLast<fn>, TailLast<fn>, ctx);
    }
...
private:
    ...
    // These functions construct Fns out of EasyFn kernels.
    //
    // The Body versions pass 0 through literally into their kernel so that the
    // optimizer can completely drop any "if (tail) { ... }" code.
    //
    // The Last versions of these functions return directly instead of chaining to the next stage.
    ...
    static void SK_VECTORCALL Body(SkRasterPipeline::Stage* st, size_t x, size_t tail,
        ...
        st->next(x,tail, r,g,b,a, dr,dg,db,da);  // It's faster to pass tail than 0.
    }
    ...
    template <EasyFn kernel>
    static void SK_VECTORCALL BodyLast(SkRasterPipeline::Stage* st, size_t x, size_t tail,
                                       Sk4f  r, Sk4f  g, Sk4f  b, Sk4f  a,
                                       Sk4f dr, Sk4f dg, Sk4f db, Sk4f da) {
        kernel(st->ctx<void*>(), x,0, r,g,b,a, dr,dg,db,da);
    }
    ...
    template <EasyFn kernel>
    static void SK_VECTORCALL TailLast(SkRasterPipeline::Stage* st, size_t x, size_t tail,
                                       Sk4f  r, Sk4f  g, Sk4f  b, Sk4f  a,
                                       Sk4f dr, Sk4f dg, Sk4f db, Sk4f da) {
    #if defined(__clang__)
        __builtin_assume(tail > 0);
