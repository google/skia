...
auto SkStrikeCache::createStrike(
    ...
    if (hasPinner) {
        ...
        if (it == fThreadMap.end()) {
            ...
            SkDebugf("====== create pinned - %x\n", p->getDescriptor().getChecksum());
        } else {
            SkDebugf("====== create pinned - duplicate strike %x\n", p->getDescriptor().getChecksum
            ());
        }
    ...
    while (node != nullptr && (bytesFreed < bytesNeeded || countFreed < countNeeded)) {
        ...
        if (node->fPinner == nullptr || canDelete) {
            ...
            if (node->fPinner != nullptr) {
                ...
                auto it = fThreadMap.find(descChecksum);
                if (it == fThreadMap.end()) {
                    SkDebugf("====== destroy remote - missing thread entry - %x\n",
                             node->getDescriptor().getChecksum());
                } else {
                    if (it->second != &node->fStrike) {
                        SkDebugf("====== destroy remote - second strike found - %x\n",
                                 node->getDescriptor().getChecksum());
                    }
                }
            }
