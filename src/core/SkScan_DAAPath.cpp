...
// For simplicity, we don't have any buckets yet; all deltas are stored in one vector.
using SkAlphaDeltaCollection = std::vector<SkAlphaDelta>;

// Future todo: a mask version of this
class SkDeltaBlitter {
public:
    SkDeltaBlitter(const SkIRect& bounds, SkBlitter* blitter, bool isEvenOdd) {
        fBounds = bounds;
        fBlitter = blitter;
        fIsEvenOdd = isEvenOdd;
    ...
    void addDelta(const SkAlphaDelta& delta) {
#ifdef SK_DEBUG
        ...
        if (fLastY != delta.fY) {
            ...
            fLastY = delta.fY;
            fLastX = delta.fX;
            fRuns[0] = delta.fX - fBounds.fLeft;
            fAlphas[0] = 0;
            fLastAlpha = 0;
        }
...
private:
    ...
    inline SkAlpha convertLastAlpha() {
        // TODO even odd
        return SkTPin(fLastAlpha >> 8, 0, 255);
    }
};
...
static inline void add_alpha_delta_segment(int y, SkFixed rowHeight, const SkAnalyticEdge* edge,
    ...
    // Let's see if multiplying winding is faster than multiplying edge->fWinding.
...
static SkAlphaDeltaCollection gen_sorted_alpha_deltas(
        const SkPath& path, const SkRegion& origClip) {
    SkAlphaDeltaCollection result;
    ...
    if (isInverse) {
        for(int y = clipRect.fTop; y < clipRect.fBottom; ++y) {
            result.push_back({y, clipRect.fLeft, SK_Fixed1}); // fill everything by default
        }
    }
    ...
    for(int edgeIndex = 0; edgeIndex < count; ++edgeIndex) {
        ...
        if (isInverse) { // restores winding; not sure if someone else is going to use it later.
            currE->fWinding *= -1;
        }
    }
...
void blit_sorted_alpha_deltas(const SkAlphaDeltaCollection& deltas, bool isEvenOdd,
        const SkRegion& origClip, SkBlitter* blitter, bool forceRLE) {
    const SkIRect& bounds = origClip.getBounds();
    SkDeltaBlitter deltaBlitter(bounds, blitter, isEvenOdd);

    size_t i = 0;
    for (; i < deltas.size() && deltas[i].fY < bounds.fTop; ++i);
    for (; i < deltas.size() && deltas[i].fY < bounds.fBottom; ++i) {
        const auto& delta = deltas[i];
        if (delta.fX >= bounds.fLeft && delta.fX < bounds.fRight) {
            deltaBlitter.addDelta(delta);
        }
    }
}
...
void SkScan::DAAFillPath(const SkPath& path, const SkRegion& origClip, SkBlitter* blitter,
    ...
    SkAlphaDeltaCollection sortedAlphaDeltas = gen_sorted_alpha_deltas(path, origClip);

    // This step must be done in order in the threaded backend
    blit_sorted_alpha_deltas(sortedAlphaDeltas, path.getFillType() & 1,
            origClip, blitter, forceRLE);
}
