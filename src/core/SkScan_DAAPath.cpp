...
static inline void add_alpha_delta_segment(int y, SkFixed rowHeight, const SkAnalyticEdge* edge,
        SkFixed nextX, SkCoverageDeltaList* deltas) {
...
static void gen_alpha_deltas(
        const SkPath& path, const SkRegion& origClip, SkCoverageDeltaList& result) {
    SkIRect ir;
...
void blit_alpha_deltas(SkCoverageDeltaList& deltas, const SkIRect& bounds,
        SkBlitter* blitter, bool forceRLE, bool isEvenOdd, bool isInverse) {
    blitter->blitCoverageDeltas(&deltas, bounds, isEvenOdd, isInverse);
}
...
void SkScan::DAAFillPath(const SkPath& path, const SkRegion& origClip, SkBlitter* blitter,
    ...
    SkCoverageDeltaList sortedAlphaDeltas(&alloc, clippedIR.fTop, clippedIR.fBottom);
    gen_alpha_deltas(path, origClip, sortedAlphaDeltas);

    // This step must be done in order in the threaded backend
    blit_alpha_deltas(sortedAlphaDeltas, clipBounds, blitter, forceRLE, path.getFillType() & 1,
