            ...
            if ((xorUpperY | xorLowerY | orDX) == 0) {
                ...
                if (rectBot > rectTop) {
                    ...
                    if (L <= R) {
                        ...
                        blitter->blitAntiRect(L - 1, rectTop, R - L, rectBot - rectTop, la, ra);
                    } else {
                        SkAlpha alpha = (r->fUpperX - l->fUpperX) >> 8;
                        blitter->blitV(L - 1, rectTop, rectBot - rectTop, alpha);
                    }
                }
                break;
            }
...
void SkScan::DAAFillPath(const SkPath& path, const SkRegion& origClip, SkBlitter* blitter,
    ...
    bool skipRect = isConvex && !isInverse && (path.isRect(nullptr) || path.isRRect(nullptr));

    // If skipRect, we don't use mask because the majority of the path
    // has already been handled by blitAntiRect.
    if (SkCoverageDeltaMask::Suitable(clippedIR) && !skipRect) {
