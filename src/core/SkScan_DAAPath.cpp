...
// The alpha of right-triangle with width w and height w*dY.
static inline SkFixed partial_triangle_to_alpha(SkFixed w, SkFixed dY) {
    SkASSERT(w <= SK_Fixed1);
    // SkFixedMul(SkFixedMul(w, w), dY) >> 1
    // return ((((w >> 8) * (w >> 8)) >> 8) * (dY >> 8)) >> 1;
    return (w >> 11) * (w >> 11) * (dY >> 11);
}
    ...
    switch (pixelsHit) {
        ...
        case 1: {
            ...
            if (kMayBePartialRow) {
                ...
                // And, sadly, SkFixedMul_lowprec(alpha, SK_Fixed1) is not just alpha.  :(
                alpha = SkFixedMul_lowprec(alpha, rowHeight);
            }
