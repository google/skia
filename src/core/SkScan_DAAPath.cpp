...
static void gen_alpha_deltas(const SkPath& path, const SkRegion& origClip, Deltas& result,
...
void SkScan::DAAFillPath(const SkPath& path, const SkRegion& origClip, SkBlitter* blitter,
    ...
    bool isEmpty                = !ir.intersect(clipBounds);
    ...
    if (rect_overflows_short_shift(ir, 2)) {
    ...
    if (SkCoverageDeltaMask::Suitable(ir)) {
        SkCoverageDeltaMask deltaMask(ir);
        ...
        SkCoverageDeltaList deltaList(&alloc, ir.fTop, ir.fBottom);
