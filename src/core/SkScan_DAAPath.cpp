...
template <bool mayBePartialRow, class Deltas>
static inline void add_coverage_delta_segment(int y,
    ...
    switch (pixelsHit) {
        case 0: {
            // The edge falls on a vertical pixel edge exactly.
            // The pixel to the right of that edge (at x=L) gets the full coverage.
        ...
        case 1: {
            ...
            // The next pixel gets a delta with all the remaining coverage.
            SkFixed fixedR  = SkIntToFixed(R);
            SkFixed alpha   = trapezoid_to_alpha(fixedR - l, fixedR - r);
            if (mayBePartialRow) {
                // Note: we do sometimes see rowHeight == SK_Fixed1 when mayBePartialRow is true.
        ...
        case 2: {
            // The edge intersects two pixels.

            SkFixed middle  = SkIntToFixed(L + 1);
            SkFixed x1      = middle - l;
            SkFixed x2      = r - middle;
            SkFixed alpha1  = partial_triangle_to_alpha(x1, edge->fDY);
            SkFixed alpha2  = rowHeight - partial_triangle_to_alpha(x2, edge->fDY);
