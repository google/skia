...
static constexpr int SHIFT = 1; // the AA shift for SkEdge and SkEdgeBuilder

template<bool isPartial, class Deltas> SK_ALWAYS_INLINE
static void add_coverage_delta_segment(int y, SkFixed rowHeight, const SkEdge* edge,
        SkFixed dY, SkFixed nextX, Deltas* deltas) { // rowHeight=fullAlpha
    ...
    SkFixed l = SkTMin(edge->x(SHIFT), nextX);
    SkFixed r = edge->x(SHIFT) + nextX - l;
    ...
    switch (len) {
        ...
        case 2: {
            ...
            SkFixed x1      = middle - l;
            SkFixed x2      = r - middle;
            SkFixed alpha1  = partialTriangleToAlpha(x1, dY);
            SkFixed alpha2  = rowHeight - partialTriangleToAlpha(x2, dY);
...
class XLessThan {
public:
    bool operator()(const SkEdge* a, const SkEdge* b) {
        return a->fX < b->fX || (a->fX == b->fX && a->fDX < b->fDX);
    }
};
...
public:
    bool operator()(const SkEdge* a, const SkEdge* b) {
        return (a->fFirstY < b->fFirstY) || (a->fFirstY == b->fFirstY && a->fX < b->fX);
    }
};
...
static void gen_alpha_deltas(const SkPath& path, const SkRegion& clipRgn, Deltas& result,
    ...
    int  count               = builder.build_edges(path, &clipRect, SHIFT, pathContainedInClip);
    if (count == 0) {
        return;
    }
    ...
    if (skipRect) {             // only find that rect is skipRect == true
        ...
        for(int i = 0; i < count - 1; ++i) {
            SkEdge* l = list[i];
            SkEdge* r = list[i + 1];
            SkFixed xorUpperY = l->upperY(SHIFT) ^ r->upperY(SHIFT);
            SkFixed xorLowerY = l->lowerY(SHIFT) ^ r->lowerY(SHIFT);
            SkFixed orDX      = l->fDX | r->fDX;
            if ((xorUpperY | xorLowerY | orDX) == 0) { // two edges with identical y and zero dx
                rectTop = SkFixedCeilToInt(l->upperY(SHIFT));
                rectBot = SkFixedFloorToInt(l->lowerY(SHIFT));
                if (rectBot > rectTop) { // if bot == top, the rect is too short for blitAntiRect
                    int L = SkFixedCeilToInt(l->x(SHIFT));
                    int R = SkFixedFloorToInt(r->x(SHIFT));
                    if (L <= R) {
                        SkAlpha la = (SkIntToFixed(L) - l->x(SHIFT)) >> 8;
                        SkAlpha ra = (r->x(SHIFT) - SkIntToFixed(R)) >> 8;
    ...
    for(int edgeIndex = 0; edgeIndex < count; ++edgeIndex) {
        ...
        SkEdge* currE = list[edgeIndex];

        // SkEdge sets fX to the middle (+1/2 pixel), but we need it to be on top (+0 pixel)
        currE->fX -= (currE->fDX >> 1);

        do {
            SkFixed upperFloor  = SkFixedFloorToFixed(currE->upperY(SHIFT));
            SkFixed lowerCeil   = SkFixedCeilToFixed(currE->lowerY(SHIFT));
            int     iy          = SkFixedFloorToInt(upperFloor);
            SkFixed dY          = currE->calcDY();

            if (lowerCeil <= upperFloor + SK_Fixed1) { // only one row is affected by the currE
                SkFixed rowHeight = currE->lowerY(SHIFT) - currE->upperY(SHIFT);
                SkFixed nextX = currE->fX + (SkFixedMul(currE->fDX, rowHeight) << SHIFT);
                add_coverage_delta_segment<true>(iy, rowHeight, currE, dY, nextX >> SHIFT, &result);
                continue;
            }

            // check first row
            SkFixed rowHeight = upperFloor + SK_Fixed1 - currE->upperY(SHIFT);
            SkFixed nextX;
            if (rowHeight != SK_Fixed1) {   // it's a partial row
                nextX = currE->fX + (SkFixedMul(currE->fDX, rowHeight) << SHIFT);
                add_coverage_delta_segment<true>(iy, rowHeight, currE, dY, nextX >> SHIFT, &result);
            ...
            while (true) { // process the full rows in the middle
                ...
                currE->fX = nextX;
                nextX += currE->fDX << SHIFT;

                if (y + SK_Fixed1 > currE->lowerY(SHIFT)) {
                ...
                add_coverage_delta_segment<false>(iy, SK_Fixed1, currE, dY, nextX >> SHIFT, &result);
            }

            // last partial row
            if (SkIntToFixed(iy) < currE->lowerY(SHIFT)) {
                rowHeight = currE->lowerY(SHIFT) - SkIntToFixed(iy);
                nextX = currE->fX + (SkFixedMul(currE->fDX, rowHeight) << SHIFT);
                add_coverage_delta_segment<true>(iy, rowHeight, currE, dY, nextX >> SHIFT, &result);
            }
        // update returns true when we're done with it; hence continue if not done.
        } while (!currE->update(currE->fLastY));
    }
}
