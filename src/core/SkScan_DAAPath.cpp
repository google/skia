...
public:
    ...
    inline void push_back(const SkAlphaDelta& delta) {
        this->checkY(delta.fY);
        if (fCounts[delta.fY] == fMaxCounts[delta.fY]) {
            fMaxCounts[delta.fY] *= 2;
            SkAlphaDelta* newRow = fAlloc->makeArrayDefault<SkAlphaDelta>(fMaxCounts[delta.fY]);
            memcpy(newRow, fRows[delta.fY], sizeof(SkAlphaDelta) * fCounts[delta.fY]);
            fRows[delta.fY] = newRow;
        }
        SkASSERT(fCounts[delta.fY] < fMaxCounts[delta.fY]);

        fRows[delta.fY][fCounts[delta.fY]++] = delta;
    }
