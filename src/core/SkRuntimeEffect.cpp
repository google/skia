...
public:
    ...
    SkColor4f onFilterColor4f(const SkColor4f& color, SkColorSpace* dstCS) const override {
        ...
        sk_sp<SkData> inputs = get_xformed_uniforms(fEffect.get(), fUniforms, nullptr, dstCS);
        if (!inputs) {
            // Can't really fail here, just use the unmodified ones, I guess.
            inputs = fUniforms;
        }

        SkSTArray<1, SkColor4f, true> inputColors;
        for (const auto &child : fChildren) {
