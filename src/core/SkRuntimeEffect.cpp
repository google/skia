...
SkRuntimeEffect::Result SkRuntimeEffect::Make(SkString sksl,
    ...
    // The signature of main, and the declarations in sksl_rt_colorfilter/sksl_rt_blend should guarantee this.
    if (flags & (kAllowColorFilter_Flag | kAllowBlend_Flag)) {
...
sk_sp<SkCustomBlend> SkRuntimeEffect::makeBlend(sk_sp<SkData> uniforms) const {
    ...
    return sk_sp<SkCustomBlend>(new SkRTBlend(sk_ref_sp(this), std::move(uniforms)));
}
...
sk_sp<SkCustomBlend> SkRuntimeBlendBuilder::makeBlend() {
    return this->effect()->makeBlend(this->uniforms());
}
