...
#include "src/core/SkBlendFilterBase.h"
...
SkRuntimeEffect::Result SkRuntimeEffect::Make(SkString sksl,
    ...
    // guarantee this.
    if (flags & (kAllowColorFilter_Flag | kAllowBlendFilter_Flag)) {
    ...
    for (const SkSL::ProgramElement* elem : program->elements()) {
        ...
        if (elem->is<SkSL::GlobalVarDeclaration>()) {
            ...
            if (varType.isEffectChild()) {
                ...
                if (kind == SkSL::ProgramKind::kRuntimeBlendFilter) {
...
SkRuntimeEffect::Result SkRuntimeEffect::MakeForBlendFilter(SkString sksl, const Options& options) {
    auto result = Make(std::move(sksl), options, SkSL::ProgramKind::kRuntimeBlendFilter);
    SkASSERT(!result.effect || result.effect->allowBlendFilter());
    return result;
}
...
SkRuntimeEffect::Result SkRuntimeEffect::MakeForBlendFilter(std::unique_ptr<SkSL::Program> program) {
    auto result = Make(std::move(program), SkSL::ProgramKind::kRuntimeBlendFilter);
    SkASSERT(!result.effect || result.effect->allowBlendFilter());
    return result;
}
...
sk_sp<SkBlendFilter> SkRuntimeEffect::makeBlendFilter(sk_sp<SkData> uniforms) const {
    if (!this->allowBlendFilter()) {
        return nullptr;
    }
    if (!uniforms) {
        uniforms = SkData::MakeEmpty();
