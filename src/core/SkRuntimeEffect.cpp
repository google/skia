...
public:
    ...
    std::unique_ptr<GrFragmentProcessor> asFragmentProcessor(const GrFPArgs& args) const override {
        ...
        sk_sp<SkData> late_bound = nullptr;
        auto writableData = [&]() {
            if (!late_bound) {
                late_bound = SkData::MakeWithCopy(fInputs->data(), fInputs->size());
            }
            return late_bound->writable_data();
        };
        ...
        auto fp = GrSkSLFP::Make(args.fContext, fEffect, "runtime_shader",
                                 late_bound ? late_bound : fInputs);
        for (const auto& child : fChildren) {
    ...
    bool onAppendStages(const SkStageRec& rec) const override {
        ...
        void* late_bound = nullptr;
        auto writableData = [&]() {
            if (!late_bound) {
                late_bound = rec.fAlloc->makeBytesAlignedTo(fEffect->uniformSize(), 4);
                memcpy(late_bound, fInputs->data(), fEffect->uniformSize());
            }
            return late_bound;
        };
        ...
        ctx->inputs = late_bound ? late_bound : fInputs->data();
        ctx->ninputs = fEffect->uniformSize() / 4;
