...
SkRuntimeEffect::EffectResult SkRuntimeEffect::Make(SkString sksl) {
    ...
    size_t offset = 0, uniformOffset = 0;
    ...
    for (auto flag : { SkSL::Modifiers::kIn_Flag, SkSL::Modifiers::kUniform_Flag }) {
        if (flag == SkSL::Modifiers::kUniform_Flag) {
            uniformOffset = SkAlign4(offset);
        }
...
SkRuntimeEffect::SkRuntimeEffect(SkString sksl, std::unique_ptr<SkSL::Compiler> compiler,
        ...
        , fUniformOffset(uniformOffset)
        , fUniformSize(uniformSize) {
    SkASSERT(fCompiler && fBaseProgram);
    SkASSERT(SkIsAlign4(fUniformOffset) && SkIsAlign4(fUniformSize));
}
