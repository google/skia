...
#undef SET_TYPES
                        ...
                        if (var.fModifiers.fLayout.fFlags &
                            SkSL::Layout::Flag::kColorTransform_Flag) {
                            v.fFlags |= Variable::kColorTransform_Flag;
                        }
...
public:
    ...
    std::unique_ptr<GrFragmentProcessor> asFragmentProcessor(const GrFPArgs& args) const override {
        ...
        for (const auto& v : fEffect->inputs()) {
            ...
            } else if (v.fFlags & Flags::kColorTransform_Flag) {
                copyOnWrite();

                SkASSERT(v.fType == Type::kFloat3 || v.fType == Type::kFloat4);
                const auto& steps(args.fDstColorInfo->colorSpaceXformFromSRGB()->steps());
