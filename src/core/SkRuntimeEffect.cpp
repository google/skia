...
static int operator-(SkSL::ByteCodeInstruction x, SkSL::ByteCodeInstruction y) {
    return (int)x - (int)y;
}
...
static std::vector<skvm::F32> program_fn(skvm::Builder* p,
    ...
    for (const uint8_t *ip = fn.code(), *end = ip + fn.size(); ip != end; ) {
      ...
      //auto u16 = [&]{ auto x = sk_unaligned_load<uint16_t>(ip); ip += sizeof(x); return x; };
        ...
        auto unary = [&](const int extra, auto&& fn) {
            const int N = extra+1;
            SkASSERT(0 < N && N <= 4);
            skvm::F32 args[4];
            for (int i = 0; i < N; ++i) {
        ...
        auto binary = [&](const int extra, auto&& fn) {
            const int N = extra+1;
            SkASSERT(0 < N && N <= 4);
            skvm::F32 right[4];
            for (int i = 0; i < N; ++i) {
        ...
        switch (inst) {
            ...
            case Inst::kAddF2:
            case Inst::kAddF3:
            case Inst::kAddF4: binary(Inst::kAddF - inst, std::plus<>{}); break;

            case Inst::kSubtractF:
            case Inst::kSubtractF2:
            case Inst::kSubtractF3:
            case Inst::kSubtractF4: binary(Inst::kSubtractF - inst, std::minus<>{}); break;

            case Inst::kMultiplyF:
            case Inst::kMultiplyF2:
            case Inst::kMultiplyF3:
            case Inst::kMultiplyF4: binary(Inst::kMultiplyF - inst, std::multiplies<>{}); break;

            case Inst::kDivideF:
            case Inst::kDivideF2:
            case Inst::kDivideF3:
            case Inst::kDivideF4: binary(Inst::kDivideF - inst, std::divides<>{}); break;

            case Inst::kATan:
            case Inst::kATan2:
            case Inst::kATan3:
            case Inst::kATan4: unary(inst - Inst::kATan, skvm::approx_atan); break;

            case Inst::kFract:
            case Inst::kFract2:
            case Inst::kFract3:
            case Inst::kFract4: unary(inst - Inst::kFract, skvm::fract); break;

            case Inst::kSqrt:
            case Inst::kSqrt2:
            case Inst::kSqrt3:
            case Inst::kSqrt4: unary(inst - Inst::kSqrt, skvm::sqrt); break;

            case Inst::kSin:
            case Inst::kSin2:
            case Inst::kSin3:
            case Inst::kSin4: unary(inst - Inst::kSin, skvm::approx_sin); break;
