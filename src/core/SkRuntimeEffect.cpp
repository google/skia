...
public:
    SkRTShader(sk_sp<SkRuntimeEffect> effect, sk_sp<SkData> uniforms, const SkMatrix* localMatrix,
               SkRuntimeEffect::ChildPtr* children, size_t childCount, bool isOpaque)
...
sk_sp<SkShader> SkRuntimeEffect::makeShader(sk_sp<SkData> uniforms,
    ...
    for (size_t i = 0; i < childCount; ++i) {
        children.emplace_back(std::move(childShaders[i]));
    }
    return this->makeShader(
            std::move(uniforms), children.begin(), childCount, localMatrix, isOpaque);
}
...
sk_sp<SkColorFilter> SkRuntimeEffect::makeColorFilter(sk_sp<SkData> uniforms) const {
    return this->makeColorFilter(std::move(uniforms), (ChildPtr*)nullptr, 0);
}
