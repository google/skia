...
static skvm::Color program_fn(skvm::Builder* p,
    ...
    auto push = [&](auto val) {
        SkASSERT(val && val.builder == p);
        stack.push_back(val.id);
    };
    auto pop_Val = [&]{ skvm::Val x = stack.back(); stack.pop_back(); return x; };
