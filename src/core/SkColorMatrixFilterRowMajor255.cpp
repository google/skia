...
void SkColorMatrixFilterRowMajor255::onAppendStages(SkRasterPipeline* p,
    ...
    const bool alphaUnchanged = fFlags & kAlphaUnchanged_Flag,
               willStayOpaque = shaderIsOpaque && alphaUnchanged;

    if (!shaderIsOpaque) { p->append(SkRasterPipeline::unpremul); }
    if (           true) { p->append(SkRasterPipeline::matrix_4x5, fTranspose); }
    if (!alphaUnchanged) { p->append(SkRasterPipeline::clamp_a_01); }
...
private:
    ...
    public:
        ...
        void emitCode(EmitArgs& args) override {
            ...
            fragBuilder->codeAppendf("\t%s.a = saturate(%s.a);\n",
