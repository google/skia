    ...
    void onComputeInvariantOutput(GrInvariantOutput* inout) const override {
        ...
        for (int i = 0; i < 4; ++i) {
            ...
            if (0 != fMatrix[kAlphaRowStartIdx + i]) {
                if (!(inout->knownColorComponents().knownComponentFlags() & kRGBAFlags[i])) {
                    inout->setToUnknown(GrInvariantOutput::kWill_ReadInput);
                    return;
                } else {
                    uint32_t component = (inout->knownColorComponents().color() >> kShifts[i]) & 0xFF;
        ...
        inout->setToOther(GrKnownColorComponents(static_cast<uint8_t>(SkScalarPin(outputA, 0, 255)) << GrColor_SHIFT_A, kA_GrColorComponentFlag),
