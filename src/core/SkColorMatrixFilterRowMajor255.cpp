...
bool SkColorMatrixFilterRowMajor255::onAppendStages(SkRasterPipeline* p) const {
    bool shaderIsOpaque = false,  // TODO: plumb this through
         willStayOpaque = shaderIsOpaque && (fFlags & kAlphaUnchanged_Flag);
    bool haveTranslate = fTranspose[16] || fTranspose[17] || fTranspose[18] || fTranspose[19];
    bool needsClamp = needs_clamping(fMatrix);  // Might be nice to rewrite to use fTranspose?
