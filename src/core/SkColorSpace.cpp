...
static SkColorSpace* adobe_rgb() {
    static SkColorSpace* cs = [] {
        SkMatrix44 to_xyz(SkMatrix44::kUninitialized_Constructor);
        to_xyz.set3x3RowMajorf(gAdobeRGB_toXYZD50);
        (void)to_xyz.getType();  // Force typemask to be computed to avoid races.
        return new SkColorSpace_XYZ(k2Dot2Curve_SkGammaNamed, to_xyz);
    }();
    return cs;
}
static SkColorSpace* srgb() {
    static SkColorSpace* cs = [] {
        SkMatrix44 to_xyz(SkMatrix44::kUninitialized_Constructor);
        to_xyz.set3x3RowMajorf(gSRGB_toXYZD50);
        (void)to_xyz.getType();  // Force typemask to be computed to avoid races.
        return new SkColorSpace_XYZ(kSRGB_SkGammaNamed, to_xyz);
    }();
    return cs;
}
static SkColorSpace* srgb_linear() {
    static SkColorSpace* cs = [] {
        ...
        to_xyz.set3x3RowMajorf(gSRGB_toXYZD50);
        (void)to_xyz.getType();  // Force typemask to be computed to avoid races.
        return new SkColorSpace_XYZ(kLinear_SkGammaNamed, to_xyz);
    }();
    return cs;
}
