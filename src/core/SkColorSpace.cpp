...
struct TableEntry {
    // The CICP ID.
    CicpId cicp_id;
    // The SkColorSpacePrimaries.
    SkColorSpacePrimaries sk_primaries;
    // The result of SkColorSpacePrimaries::toXYZD50. This is sometimes stored
    // explicitly, to ensure an exact result.
...
bool GetCicp(CicpId primaries, skcms_Matrix3x3& m) {
    for (const auto& table_entry : cicp_table) {
        if (primaries != table_entry.cicp_id) {
            continue;
        }
        if (table_entry.to_xyzd50) {
            m = *table_entry.to_xyzd50;
            return true;
        }
        if (table_entry.sk_primaries.toXYZD50(&m)) {
            return true;
        }
        return false;
    }
...
bool GetCicpFromMatrix(const skcms_Matrix3x3& m, CicpId& primaries) {
    for (const auto& table_entry : cicp_table) {
        skcms_Matrix3x3 table_entry_to_xyzd50;
        if (table_entry.to_xyzd50) {
            table_entry_to_xyzd50 = *table_entry.to_xyzd50;
        } else if (!table_entry.sk_primaries.toXYZD50(&table_entry_to_xyzd50)) {
            continue;
        }
        if (xyz_almost_equal(m, table_entry_to_xyzd50)) {
...
sk_sp<SkColorSpace> SkColorSpace::MakeCICP(SkNamedPrimaries::CicpId color_primaries,
    ...
    if (!SkNamedPrimaries::GetCicp(color_primaries, primaries_matrix)) {
        return nullptr;
    }
