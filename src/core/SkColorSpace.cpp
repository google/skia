...
sk_sp<SkColorSpace> SkColorSpace::MakeRGB(const skcms_TransferFunction& transferFn,
    ...
    if (is_almost_srgb(transferFn)) {
        ...
        if (xyz_almost_equal(toXYZ, SkNamedGamut::kSRGB)) {
            // NOTE: This isn't a singleton, but we're still snapping to the exact sRGB gamut,
            // to avoid layout test diffs. In a future change, I'd like to do this gamut-snapping
            // regardless of transfer function.
