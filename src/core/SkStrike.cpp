...
SkGlyph* SkStrike::mergeGlyph(SkPackedGlyphID toID, const SkGlyph& from) {
    SkGlyph* glyph = fGlyphMap.findOrNull(toID);
    if (glyph == nullptr) {
        glyph = this->makeGlyph(toID);
    }
    if (glyph->mergeImage(from, &fAlloc)) {
...
const SkPath* SkStrike::preparePath(SkGlyph* glyph) {
    if (!glyph->setPathHasBeenCalled() && glyph->setPath(&fAlloc, fScalerContext.get())) {
...
const SkPath* SkStrike::preparePath(SkGlyph* glyph, const SkPath* path) {
    if (!glyph->setPathHasBeenCalled() && glyph->setPath(&fAlloc, path)) {
        fMemoryUsed += glyph->path()->approximateBytesUsed();
    }
    return glyph->path();
}
