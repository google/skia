...
const SkPath* SkStrike::setPath(SkGlyph* glyph) {
    if (!glyph->pathIsSet() && glyph->setPath(&fAlloc, fScalerContext.get())) {
        fMemoryUsed += glyph->path()->approximateBytesUsed();
    }
    return glyph->path();
}

const SkPath* SkStrike::setPath(SkGlyph* glyph, const SkPath* path) {
    if (!glyph->pathIsSet() && glyph->setPath(&fAlloc, path)) {
        fMemoryUsed += glyph->path()->approximateBytesUsed();
    }
    return glyph->path();
}
...
void SkStrike::forceValidate() const {
    ...
    fGlyphMap.foreach ([&memoryUsed](const SkGlyph* glyphPtr) {
        ...
        if (glyphPtr->pathIsSet() && glyphPtr->path() != nullptr) {
