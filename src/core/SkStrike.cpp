...
SkZip<SkGlyph*, SkPoint> SkStrike::prepareForMaskDrawing2(
        SkPainterPassStorage::PassForGlyphs* pass) {
    for (auto t : *pass) {
        SkPackedGlyphID packedID; SkPoint pos;
        std::tie(packedID, pos) = t;
        if (SkScalarsAreFinite(pos.x(), pos.y())) {
            SkGlyph* glyph = this->glyph(packedID);
            if (!glyph->isEmpty()) {
                if (CanDrawAsMask(*glyph)) {
                    pass->push_back(glyph);
                } else {
                    pass->reject();
                }
            }
...
SkZip<SkGlyph*, SkPoint> SkStrike::prepareForSDFTDrawing2(
        SkPainterPassStorage::PassForGlyphs* pass) {
    for (auto t : *pass) {
        SkPackedGlyphID packedID = std::get<0>(t);
...
SkZip<SkGlyph*, SkPoint> SkStrike::prepareForPathDrawing2(
        SkPainterPassStorage::PassForGlyphs* pass) {
    for (auto t : *pass) {
        SkPackedGlyphID packedID; SkPoint pos;
        std::tie(packedID, pos) = t;
