...
GrSurfaceProxyView SkBlurMaskFilterImpl::filterMaskGPU(GrRecordingContext* context,
    ...
    // kUnknown would make more sense as the alpha-type here (since this is a mask) but we want
    // to ensure that we can issue draws (see below) and kUnknown is not valid for
    ...
    if (!isNormalBlur) {
        // TODO: Could we have a low-level blend option (e.g. GrBlendEquation + GrBlendCoeffs) for
        //  fill context to avoid having to rely on converting to a draw context here?
        auto drawContext = fillContext->asDrawContext();
