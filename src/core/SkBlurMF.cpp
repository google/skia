...
//    }
    ...
    if (canBeRect || canBeCircle) {
        ...
        if (this->ignoreXform()) {
            ...
            // the CTM. If the matrix passed in here is just scale and translate so we can just grab
            // the X and Y scales off the matrix and pre-undo the scale. Otherwise get the min scale
            // and apply it universally.
            if (viewMatrix.isScaleTranslate()) {
                outsetX /= SkScalarAbs(viewMatrix.getScaleX());
                outsetY /= SkScalarAbs(viewMatrix.getScaleY());
            } else {
                SkScalar minScale = viewMatrix.getMinScale();
                if (minScale <= 0) {
                    return false;
                }
                outsetX /= minScale;
