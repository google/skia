...
size_t SkPath::readFromMemory(const void* storage, size_t length) {
    ...
    unsigned version = packed & kVersion_SerializationMask;
...
size_t SkPath::readAsRRect(const void* storage, size_t length) {
    ...
    SkASSERT(type == SerializationType::kRRect);
...
size_t SkPath::readFromMemory_EQ4(const void* storage, size_t length) {
    ...
    SkPath tmp;
    tmp.setFillType(static_cast<FillType>((packed >> kFillType_SerializationShift) & 0x3));
    tmp.incReserve(pts);
