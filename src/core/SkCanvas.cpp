...
void SkCanvas::restore() {
    if (fMCRec->fDeferredSaveCount > 0) {
        ...
        if (fMCStack.count() > 1) {
            ...
            this->didRestore();
#ifdef SK_USE_DEVICE_CLIPPING
            FOR_EACH_DEVICE(device->restore(fMCRec->fMatrix));
#endif
        }
    }
}
...
#ifdef SK_USE_DEVICE_CLIPPING
    FOR_EACH_DEVICE(device->clipRect(rect, op, isAA));
#endif
...
#ifdef SK_USE_DEVICE_CLIPPING
    FOR_EACH_DEVICE(device->clipRRect(rrect, op, isAA));
#endif
...
#ifdef SK_USE_DEVICE_CLIPPING
    FOR_EACH_DEVICE(device->clipPath(path, op, isAA));
#endif
...
#ifdef SK_USE_DEVICE_CLIPPING
    FOR_EACH_DEVICE(device->clipRegion(rgn, op));
#endif
