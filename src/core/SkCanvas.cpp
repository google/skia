...
static void draw_begin_drawbitmap(SkCanvas* canvas, const SkPaint& paint,
                                  bool skipLayerForFilter, const SkRect* bounds,
                                  const std::function<void(SkDrawIter&, const SkPaint&)>& fn) {
    AutoLayerForImageFilter draw(canvas, paint, skipLayerForFilter, bounds);
...
static void draw_begin_drawdevice(SkCanvas* canvas, const SkPaint& paint,
                                  const std::function<void(SkDrawIter&, const SkPaint&)>& fn) {
    AutoLayerForImageFilter draw(canvas, paint, /*skipLayerForFilter=*/true);
    SkDrawIter iter(canvas);
    fn(iter, draw.paint());
}
...
static void draw_begin(SkCanvas* canvas, const SkPaint& paint, const SkRect* bounds,
                       const std::function<void(SkDrawIter&, const SkPaint&)>& fn) {
    AutoLayerForImageFilter draw(canvas, paint, /*skipLayerForFilter=*/false, bounds);
    SkDrawIter iter(canvas);
    fn(iter, draw.paint());
}
...
void SkCanvas::internalDrawDevice(SkBaseDevice* srcDev, const SkPaint* origPaint) {
    SkPaint tmp;
    if (nullptr == origPaint) {
        origPaint = &tmp;
    }

    draw_begin_drawdevice(this, *origPaint, [&](SkDrawIter& iter, const SkPaint& paint) {
        while (iter.next()) {
...
void SkCanvas::translate(SkScalar dx, SkScalar dy) {
    if (dx || dy) {
        ...
        SkDrawIter iter(this);
        while (iter.next()) {
            iter->setGlobalCTM(fMCRec->fMatrix);
        }
...
void SkCanvas::androidFramework_setDeviceClipRestriction(const SkIRect& rect) {
    ...
    if (fClipRestrictionRect.isEmpty()) {
        ...
        SkDrawIter iter(this);
        while (iter.next()) {
            iter->androidFramework_setDeviceClipRestriction(&fClipRestrictionRect);
        }
    } else {
        this->checkForDeferredSave();
        SkDrawIter iter(this);
...
void SkCanvas::temporary_internal_getRgnClip(SkRegion* rgn) {
    ...
    SkDrawIter iter(this);
    while (iter.next()) {
        iter->onAsRgnClip(&tmp);
        accum.accumulate(iter.fDevice, &tmp);
    }
}
...
void SkCanvas::onDrawTextBlob(const SkTextBlob* blob, SkScalar x, SkScalar y,
    ...
    draw_begin(this, paint, bounds, [&](SkDrawIter& iter, const SkPaint& drawPaint) {
        while (iter.next()) {
            fScratchGlyphRunBuilder->drawTextBlob(drawPaint, *blob, {x, y}, &*iter);
        }
    });
}
...
void SkCanvas::onDrawAtlas(const SkImage* atlas, const SkRSXform xform[], const SkRect tex[],
    ...
    draw_begin(this, pnt, nullptr, [&](SkDrawIter& iter, const SkPaint&) {
        while (iter.next()) {
            iter->drawAtlas(atlas, xform, tex, colors, count, bmode, pnt);
        }
    });
}
