...
public:
    ...
    MCRec() : fLayer(nullptr), fDevice(nullptr), fBackImage(nullptr), fDeferredSaveCount(0) {
        fMatrix.setIdentity();
        inc_rec();
    }
...
static bool simplify_layer_paint(SkTCopyOnFirstWrite<SkPaint>* paint) {
    if (paint->get()) {
        if (auto simplifiedCF = image_to_color_filter(*(*paint))) {
            paint->writable()->setColorFilter(std::move(simplifiedCF));
...
public:
    ...
    AutoLayerForImageFilter(SkCanvas* canvas, const SkPaint& paint,
        ...
        if (!simplify_layer_paint(&fPaint)) {
            // The image filter that triggering the auto-layer was not able to be converted into
            // or composed with the paint's original color filter.
...
// Helper macro to run code on the top layer's SkBaseDevice if it's not null
#define ON_DEVICE( code )                                  \
...
// Method 1
//
// Call sites use a single macro :
//    DRAW(paint, bounds = path.getBounds(), device->drawPath(path, drawPaint));
//
// And must assign to special 'bounds' SkRect and use 'drawPaint' when calling
// device code. The bounds code is only evaluated when necessary.

#define BEGIN_DRAW(paint, boundsCode, fastBoundsFunction) \
    bool fastBounds = (paint).canComputeFastBounds(); \
    bool autoLayer = SkToBool((paint).getImageFilter()); \
    SkRect bounds; \
    boundsCode \
    if (fastBounds) { \
        SkRect storage; \
        if (this->quickReject((paint).##fastBoundsFunction(bounds, &storage))) { \
            return; \
        } \
    } \

#define END_DRAW(paint, drawCode) \
    if (autoLayer) { \
        AutoLayerForImageFilter layer(this, paint, &bounds); \
        const SkPaint& drawPaint = layer.paint(); \
        ON_DEVICE(drawCode); \
    } else { \
        const SkPaint& drawPaint = paint; \
        ON_DEVICE(drawCode); \
    }

#define SIMPLE_DRAW(paint, boundsCode, drawCode, fastBoundsFunction) \
    BEGIN_DRAW(paint, if (fastBounds || autoLayer) { boundsCode; }, fastBoundsFunction) \
    this->predrawNotify(); \
    END_DRAW(paint, drawCode) \
    do {} while(0)


#define DRAW(paint, boundsCode, drawCode) \
    SIMPLE_DRAW(paint, boundsCode, drawCode, computeFastBounds)

#define DRAW_STROKED(paint, boundsCode, drawCode) \
    SIMPLE_DRAW(paint, boundsCode, drawCode, computeFastStrokeBounds)

#define DRAW_CONTENT_OVERWRITE(paint, shaderOverrideIsOpaque, boundsCode, drawCode) \
    BEGIN_DRAW(paint, { boundsCode; }, computeFastBounds) \
    this->predrawNotify(&bounds, &(paint), shaderOverrideIsOpaque); \
    END_DRAW(paint, drawCode) \
    do {} while(0)
...
// Method 2
//
// Call sites use 3 macros in sequence:
//    DRAW_PREAMBLE(paint);
//    LAZY_BOUNDS_CHECK(bounds = path.getBounds());
//    DO_DRAW(device->drawPath(path, drawPaint));
//
// But still have to assign to the magic 'bounds' and access the magic 'drawPaint'

#define DRAW_PREAMBLE(paint) \
    const SkPaint& _paint = (paint); \
    bool fastBounds = _paint.canComputeFastBounds(); \
    bool autoLayer = SkToBool(_paint.getImageFilter()); \
    SkRect bounds

#define DRAW_BOUNDS_BASE(code, fastBoundsFunction) \
    code \
    if (fastBounds) { \
        SkRect storage; \
        if (this->quickReject(_paint.##fastBoundsFunction(bounds, &storage))) { \
            return; \
        } \
    } \
    do {} while(0)

#define LAZY_BOUNDS_CHECK(code) \
    DRAW_BOUNDS_BASE(if (fastBounds || autoLayer) { code; }, computeFastBounds)

#define STROKE_BOUNDS_CHECK(code) \
    DRAW_BOUNDS_BASE(if (fastBounds || autoLayer) { code; }, computeFastStrokeBounds)

#define BOUNDS_CHECK(code) \
    DRAW_BOUNDS_BASE({ code; }, computeFastBounds)

#define DRAW_BASE(code) \
    if (autoLayer) { \
        AutoLayerForImageFilter layer(this, _paint, &bounds); \
        const SkPaint& drawPaint = layer.paint(); \
        ON_DEVICE(code); \
    } else { \
        const SkPaint& drawPaint = _paint; \
        ON_DEVICE(code); \
    }
...
#define DO_DRAW_CONTENT_OVERWRITE(code, shaderOverrideIsOpaque) \
    this->predrawNotify(&bounds, &_paint, shaderOverrideIsOpaque); \
    DRAW_BASE(code)

///////////////////////////////////////////////////////////////////////////////
// Method 3
//
// Templates and inner structs per onDrawX function:
//    struct Path {
//        const SkPath& path;
//
//        SkRect bounds() const { return path.getBounds(); }
//        void draw(SkBaseDevice* device, const SkPaint& paint) { device->drawPath(path, paint); }
//    };
//    draw(Path(path), this, paint);

template <typename Draw, bool CheckContentOverwrite = false, bool StrokeBounds = false>
static inline void draw(const Draw& draw, SkCanvas* canvas, const SkPaint& paint,
                        bool shaderOverrideIsOpaque = false) {
    bool fastBounds = paint.canComputeFastBounds();
    bool autoLayer = SkToBool(paint.getImageFilter());

    SkRect lazyBounds;
    if (CheckContentOverwrite || fastBounds || autoLayer) {
        // Compute bounds from draw parameters if needed for the above reasons
        lazyBounds = draw.bounds();
    }
    // Check quick reject
    if (fastBounds) {
        SkRect storage;
        const SkRect& testBounds = StrokeBounds ?
                paint.computeFastStrokeBounds(lazyBounds, &storage) :
                paint.computeFastBounds(lazyBounds, &storage);
        if (canvas->quickReject(testBounds)) {
            return;
        }
    }

    if constexpr (CheckContentOverwrite) {
        canvas->predrawNotify(&lazyBounds, &paint, shaderOverrideIsOpaque);
    } else {
        canvas->predrawNotify();
    }

    if (autoLayer) {
        AutoLayerForImageFilter layer(canvas, paint, &bounds);
        ON_DEVICE(draw.draw(device, layer.paint()));
    } else {
        ON_DEVICE(draw.draw(device, paint));
    }
}

template <typename Draw>
using draw_stroke = draw<Draw, false, true>;

template <typename Draw>
using draw_check_overwrite = draw<Draw, true, false>;
...
void SkCanvas::internalSave() {
    void* newTop = fMCStack.push_back();
    fMCRec = new (newTop) MCRec(*fMCRec);

    ON_DEVICE(device->save());
}
...
void SkCanvas::internalSaveLayer(const SaveLayerRec& rec, SaveLayerStrategy strategy) {
    ...
    fMCRec->fLayer = std::unique_ptr<Layer>(new Layer(std::move(newDevice), paint, stashedMatrix));
    fQuickRejectBounds = qr_clip_bounds(this->computeDeviceClipBounds());
}
...
void SkCanvas::internalRestore() {
    ...
    auto layer = std::move(fMCRec->fLayer);
    ...
    if (fMCRec) {
        ON_DEVICE(device->restore(fMCRec->fMatrix));
    }
    ...
    if (layer) {
        // If we had a layer, its owning MCRec wasn't the base layer, so the current MCRec should
        // be non-null. The base MCRec never has a Layer instance.
        SkASSERT(fMCRec);
...
void SkCanvas::internalDrawDevice(SkBaseDevice* srcDev, const SkPaint* paint) {
    ...
    SkTCopyOnFirstWrite filterPaint(paint);
    (void) simplify_layer_paint(&filterPaint);
    sk_sp<SkImageFilter> filter = filterPaint->refImageFilter();
    if (filter) {
        filterPaint.writable()->setImageFilter(nullptr);
    }

    SkBaseDevice* dstDev = this->getTopDevice();
    // We shouldn't have a layer to restore in the first place if there wasn't a dst device
    SkASSERT(dstDev);
    ...
    if (!filter) {
        ...
        dstDev->drawDevice(srcDev, *filterPaint);
    } else {
        ...
        dstDev->drawFilteredImage(mapping, srcBuffer.get(), filter.get(), *filterPaint);
    }
}
...
void SkCanvas::translate(SkScalar dx, SkScalar dy) {
    if (dx || dy) {
        ...
        ON_DEVICE(device->setGlobalCTM(fMCRec->fMatrix));
...
void SkCanvas::onDrawShadowRec(const SkPath& path, const SkDrawShadowRec& rec) {
    SkPaint paint;
    {
        struct Shadow {
            ...
            SkRect bounds() const { return path.getBounds(); }
            void draw(SkBaseDevice* device, const SkPaint& paint) {
                device->drawShadow(path, rec);
            }
        };
        return draw<Shadow>({path, rec}, this, paint);
    }

    // or
    {
        DRAW(paint, bounds = path.getBounds(), device->drawShadow(path, rec));
    }
    ...
    // or
    {
        DRAW_PREAMBLE(paint);
        LAZY_BOUNDS_CHECK(bounds = path.getBounds());
        DO_DRAW(device->drawShadow(path, rec));
    }
...
void SkCanvas::onDrawPoints(PointMode mode, size_t count, const SkPoint pts[],
...

    {
        struct Points {
            ...
            void draw(SkBaseDevice* device, const SkPaint& paint) {
                device->drawPoints(mode, count, pts, paint);
            }
        };
        draw_stroke<Points>({mode, count, pts}, this, paint);
    }
    // or
    {
        DRAW_STROKED(paint, \
            ...
            device->drawPoints(mode, count, pts, drawPaint));
    }
    ...
    // or
    {
        ...
        DO_DRAW(device->drawPoints(mode, count, pts, drawPaint));
    }
    ...
    this->getTopDevice()->drawPoints(mode, count, pts, draw.paint());
    DRAW_END
}
...
void SkCanvas::onDrawRect(const SkRect& r, const SkPaint& paint) {
...

    {
        struct Rect {
            const SkRect& r;
            SkRect bounds() const { return r; }
            void draw(SkBaseDevice* device, const SkPaint& paint) const {
                device->drawRect(r, paint);
            }
        };
        draw_check_overwrite<Rect>({r}, this, paint);
    }

    // or
    {
        DRAW_CONTENT_OVERWRITE(paint, false, bounds = r, device->drawRect(r, drawPaint));
    }

    // or
    {
        DRAW_PREAMBLE(paint);
        BOUNDS_CHECK(bounds = r);
        DO_DRAW_CONTENT_OVERWRITE(device->drawRect(r, drawPaint), false);
    }
