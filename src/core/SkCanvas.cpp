...
#ifdef SK_USE_DEVICE_CLIPPING
        device->setDeviceClipRestriction(&fClipRestrictionRect);
#endif
    }
...
#ifdef SK_USE_DEVICE_CLIPPING
    newDevice->setDeviceClipRestriction(&fClipRestrictionRect);
    if (layer->fNext) {
        ...
        SkRegion hole(SkIRect::MakeXYWH(ir.fLeft, ir.fTop,
                                        newDevice->width(), newDevice->height()));
        hole.setRect(ir);
        priorDevice->clipRegion(hole, SkClipOp::kDifference);
    }
#endif
}
