...
void SkCanvas::onDrawEdgeAAImageSet(const ImageSetEntry imageSet[], int count,
    ...
    // We could always calculate the set's dstRect union to always check quickReject(), but we can't
    // reject individual entries and Chromium's occlusion culling already makes it likely that at
    // least one entry will be visible. So, we only calculate the draw bounds when it's trivial
    // (count == 1), or we need it for the autolooper (since it greatly improves image filter perf).
