...
void SkCanvas::init(sk_sp<SkBaseDevice> device) {
    ...
    if (device) {
        ...
        fQuickRejectBounds = qr_clip_bounds(SkRect::Make(device->getGlobalBounds()));
...
void SkCanvas::internalSaveLayer(const SaveLayerRec& rec, SaveLayerStrategy strategy) {
    ...
    if (!newDevice) {
        ...
        fQuickRejectBounds = qr_clip_bounds(SkRect::Make(ir));
        return;
    }
...
void SkCanvas::androidFramework_replaceClip(const SkIRect& rect) {
    ...
    fQuickRejectBounds = qr_clip_bounds(SkRect::Make(rect));
}
