...
void SkCanvas::internalSaveLayer(const SaveLayerRec& rec, SaveLayerStrategy strategy) {
    ...
    skif::LayerSpace<SkIRect> layerBounds;
    if (fMCRec->fTopLayer && fMCRec->fTopLayer->fNext) {
    ...
    if (strategy == kNoLayer_SaveLayerStrategy) {
        // At this point, we don't want to make an actual layer, but we have determined what the
        // layer size would be and adjusted the canvas' bounds accordingly. This is necessary so
        // that recording canvas' and virtual canvas wrappers stay in sync with a device canvas.
        return;
    }
