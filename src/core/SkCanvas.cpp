    ...
    if (!newDevice) {
        ...
        if (boundsAffectsClip) {
            if (strategy == kNoLayer_SaveLayerStrategy) {
                // Replacing the clip on the prior top devices simulates where the layer would have
                // been drawn to if in a recording scenario (strategy == kNoLayer and 'ir' is
                // not empty). When 'ir' is empty, this ensures all nested draws are ignored w/o
                // needing to allocate an empty device or layer.
                UPDATE_DEVICE_CLIP(device->replaceClip(ir),
                                   setRect(ir));
            }
            // else the layer device failed to be created, so do nothing, which makes this call
            // to saveLayer() effectively just a save().
        }
        return;
    }
