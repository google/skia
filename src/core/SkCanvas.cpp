...
void SkCanvas::DrawDeviceWithFilter(SkBaseDevice* src, const SkImageFilter* filter,
    ...
    if (!filter) {
        ...
        if (special) {
            ...
            dst->drawSpecial(SkMatrix::I(), special.get(), paint);
        }
        return;
    }
    ...
    if (special) {
        ...
        dst->drawSpecial(dstCTM, special.get(), paint);
    }
}
...
void SkCanvas::internalDrawDevice(SkBaseDevice* srcDev, const SkPaint* paint) {
    ...
    while (iter.next()) {
        ...
        if (!filter) {
            ...
            dstDev->drawDevice(srcDev, layerPaint);
        } else {
...
bool SkCanvas::canDrawBitmapAsSprite(SkScalar x, SkScalar y, int w, int h, const SkPaint& paint) {
    ...
    if (!SkTreatAsSprite(ctm, SkISize::Make(w, h), paint.isAntiAlias())) {
        return false;
    }
