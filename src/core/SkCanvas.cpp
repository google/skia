...
void SkCanvas::onDrawDrawable(SkDrawable* dr, const SkMatrix* matrix) {
    ...
    // Check to see if the devices support drawing the drawable. If not we fall back to the canvas
    // draw.
    if (this->getDevice()->isDrawableSupported(dr)) {
        ...
        while (iter.next()) {
            // All drawables use a matrix that is the current CTM concat'd with the passed in
            // matrix parameter.
            const SkMatrix& ctm = this->getTotalMatrix();
            const SkMatrix& combinedMat = matrix ? SkMatrix::Concat(ctm, *matrix) : ctm;
            iter.fDevice->drawDrawable(dr, combinedMat);
        }
