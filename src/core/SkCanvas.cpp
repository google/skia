...
void SkCanvas::internalSaveLayer(const SaveLayerRec& rec, SaveLayerStrategy strategy) {
    ...
    SkPixelGeometry geo = priorDevice->surfaceProps().pixelGeometry();
...
bool SkCanvas::currentDeviceSupportsLCDText() const {
    SkBaseDevice* top_device = getTopDevice();
    return top_device && top_device->surfaceProps().pixelGeometry() != kUnknown_SkPixelGeometry;
}
