...
bool SkCanvas::writePixels(const SkImageInfo& srcInfo, const void* pixels, size_t rowBytes,
    ...
    // We verify color type at device level, but we won't catch this case because
    // kIndex8 is typically allowed as a src.  It is not allowed here: we don't
    // accept a color table parameter.
    if (kIndex_8_SkColorType == srcInfo.colorType()) {
        return false;
    }

    // Tell our owning surface to bump its generation ID
    const bool completeOverwrite = (x <= 0) && (y <= 0) &&
            (srcInfo.width() + x >= device->width()) &&
            (srcInfo.height() + y >= device->height());
