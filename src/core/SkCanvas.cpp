...
struct DeviceCM {
    ...
    DeviceCM(sk_sp<SkBaseDevice> device, const SkImageFilter* imageFilter, const SkPaint& paint)
            : fNext(nullptr)
            , fDevice(std::move(device))
...
static std::pair<skif::Mapping, skif::LayerSpace<SkIRect>> get_layer_mapping_and_bounds(
    ...
    // be necessary to discard the bounds for subsequent layer calcuations.
    if (mustCoverDst) {
    ...
    int maxDimThreshold = 2 * std::max(SkIRect(targetOutput).width(),
    ...
    if (layerBounds.width() > maxDimThreshold || layerBounds.height() > maxDimThreshold) {
        ...
        int maxDim = localToDst.hasPerspective() ? 1024 : maxDimThreshold;
...
void SkCanvas::internalDrawDeviceWithFilter(SkBaseDevice* src, SkBaseDevice* dst,
    ...
    if (compat == DeviceCompatibleWithFilter::kYes) {
        ...
        if (!localToSrc.invert(&srcToIntermediate)) {
            SkDebugf("drop due to localToSrc\n");
            return;
        }
    ...
    if (!needsIntermediateImage) {
        ...
        if (!SkMatrixPriv::InverseMapRect(srcToIntermediate, &srcRect,
            ...
            SkDebugf("drop due to requiredInput -> srcRect\n");
            return;
        }
        ...
        if (srcSubset.intersect(SkIRect::MakeSize(srcDims)) &&
            ...
            if (!intermediateDevice) {
                SkDebugf("drop due to no device\n");
                return;
            }
...
static const SkImageFilter* optimize_layer_filter(const SkImageFilter* filter, SkPaint* paint) {
    ...
    if (filter && filter->isColorFilterNode(&cf)) {
        paint->setColorFilter(SkColorFilters::Compose(paint->refColorFilter(), sk_ref_sp(cf)));
...
void SkCanvas::internalSaveLayer(const SaveLayerRec& rec, SaveLayerStrategy strategy) {
    ...
    if ((rec.fSaveLayerFlags & kInitWithPrevious_SaveLayerFlag) || rec.fBackdrop) {
        ...
        this->internalDrawDeviceWithFilter(priorDevice, newDevice.get(), backdropPaint,
                                           backdropFilter, DeviceCompatibleWithFilter::kUnknown);
    }
