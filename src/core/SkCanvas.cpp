...
sk_sp<SkImage> SkCanvas::wrapBitmapAsImage(const SkBitmap& bitmap) const {
    if (bitmap.drawsNothing()) {
        return nullptr;
    }

    bool copyIfMutable = this->onMustCopyMutableBitmap();
    if (!copyIfMutable) {
        FOR_EACH_TOP_DEVICE(copyIfMutable |= device->mustCopyMutableBitmap());
    }

    // If the canvas and devices allow it, this image can just be a thin wrapper of the bitmap.
    return SkMakeImageFromRasterBitmap(bitmap, copyIfMutable ? kIfMutable_SkCopyPixelsMode
                                                             : kNever_SkCopyPixelsMode);
}

void SkCanvas::drawBitmap(const SkBitmap& bitmap, SkScalar dx, SkScalar dy, const SkPaint* paint) {
    this->drawImage(this->wrapBitmapAsImage(bitmap), dx, dy, paint);
}

void SkCanvas::drawBitmapRect(const SkBitmap& bitmap, const SkRect& src, const SkRect& dst,
                              const SkPaint* paint, SrcRectConstraint constraint) {
    this->drawImageRect(this->wrapBitmapAsImage(bitmap), src, dst, paint, constraint);
}
