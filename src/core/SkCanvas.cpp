...
void SkCanvas::resetForNextPicture(const SkIRect& bounds) {
    ...
    printf("resetForNextPicture [%d %d %d %d]\n", bounds.fLeft, bounds.fTop, bounds.fRight, bounds.fBottom);
...
void SkCanvas::validateClip() const {
    ...
    if (this->isClipEmpty()) {
        if (!fQuickRejectBounds.isEmpty()) {
            printf("**************************************\n");
            printf("canvas WH = %d x %d\n", this->imageInfo().width(), this->imageInfo().height());
            printf("qr bounds: [%f %f %f %f]\n", fQuickRejectBounds.fLeft, fQuickRejectBounds.fTop, fQuickRejectBounds.fRight, fQuickRejectBounds.fBottom);
            printf("tmp: [%f %f %f %f]\n", tmp.fLeft, tmp.fTop, tmp.fRight, tmp.fBottom);
            SkRect devBounds = this->computeDeviceClipBounds();
            printf("dev: [%f %f %f %f]\n", devBounds.fLeft, devBounds.fTop, devBounds.fRight, devBounds.fBottom);

            SkBaseDevice* dev = this->getTopDevice();
            if (dev) {
                SkIRect devIBounds = dev->devClipBounds();
                printf("direct dev bounds: [%d %d %d %d]\n", devIBounds.fLeft, devIBounds.fTop, devIBounds.fRight, devIBounds.fBottom);
                printf("device WH = %d x %d\n", dev->width(), dev->height());
                SkMatrix m = dev->deviceToGlobal();
                printf("dev-to-global [%f %f %f]\n"
                       "              [%f %f %f]\n"
                       "              [%f %f %f]\n",
                       m.get(0), m.get(1), m.get(2), m.get(3), m.get(4), m.get(5), m.get(6), m.get(7), m.get(8));
            } else {
                printf("no top device\n");
            }
        }
