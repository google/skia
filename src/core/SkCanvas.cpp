...
static SkSamplingOptions clean_up(SkSamplingOptions sampling) {
    ...
    if (sampling.fUseCubic && (!is_unit(sampling.fCubic.B) || !is_unit(sampling.fCubic.C))) {
...
void SkCanvas::drawImageRect(const SkImage* image, const SkRect& src, const SkRect& dst,
    ...
    this->onDrawImageRect2(image, src, dst, clean_up(sampling), paint, constraint);
}
...
void SkCanvas::onDrawImageRect2(const SkImage* image, const SkRect& src, const SkRect& dst,
    ...
    if (r != src) {
        ...
        subset = image->makeSubset(ir, this->recordingContext());
        if (!subset) {
            return;
        }
        image = subset.get();
