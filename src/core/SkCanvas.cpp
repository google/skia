...
public:
    ...
    AutoLayerForImageFilter(SkCanvas* canvas, const SkPaint& origPaint,
        ...
        if (!skipLayerForFilter && fPaint->getImageFilter()) {
            /**
...
// FIXME (michaelludwig) - After image filter refresh, drawImage et al. may not use
// AutoLayerForImageFilter, in which case skipLayerForFilter can go away.
...
void SkCanvas::DrawDeviceWithFilter(SkBaseDevice* src, const SkImageFilter* filter,
    ...
    // I think that all that really needs to happen is that if 'ctm' represents the global CTM
    // for 'dst' (which hasn't been configured yet). But the backdrop is read from src, so maybe
    // we use the relative basis matrix between src and dst and then check that for decomposition
    // needs by the filter. Is this update worth waiting to also handle the nice bounds types?
    // Or should we do it before hand and then see what sort of bounds functions may not have been provided previously?
...
void SkCanvas::internalSaveLayer(const SaveLayerRec& rec, SaveLayerStrategy strategy) {
    ...
    newDevice->setBasisAndOrigin(fMCRec->fMatrix, layerMatrix, ir.fLeft, ir.fTop);
    ...
    if ((rec.fSaveLayerFlags & kInitWithPrevious_SaveLayerFlag) || rec.fBackdrop) {
        DrawBackdropIntoLayer(priorDevice, newDevice.get(), rec.fBackdrop);
    }
    ...
    if (layer->fNext) {
        ...
        SkRegion hole(ir);
        do {
            // FIXME we have to map 'hole' into layer's fDevice, since it's currently relative
            // to 'newDevice's layer space (just layerMatrix, ir already has the origin).
            // I still don't really follow what this next layer means, and it's only set if this
            // layer
            layer = layer->fNext;
