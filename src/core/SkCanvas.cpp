...
void SkCanvas::DrawFilteredImage(SkBaseDevice* dst, sk_sp<SkSpecialImage> src,
                                 const skif::Mapping& mapping, const SkImageFilter* filter,
                                 const SkPaint& paint) {
    ...
    skif::LayerSpace<SkIRect> targetOutput = mapping.deviceToLayer(
            skif::DeviceSpace<SkIRect>(dst->devClipBounds()));
    ...
    SkColorType colorType = dst->imageInfo().colorType();
    if (colorType == kUnknown_SkColorType) {
        colorType = kRGBA_8888_SkColorType;
    }
    ...
    skif::Context ctx(mapping, targetOutput, cache.get(), colorType, dst->imageInfo().colorSpace(),
                      skif::FilterResult<For::kInput>(std::move(src)));

    SkIPoint offset;
    src = as_IFB(filter)->filterImage(ctx).imageAndOffset(&offset);
    if (src) {
        ...
        dst->drawSpecial(src.get(), offset.fX, offset.fY, paint);
    }
}
...
void SkCanvas::internalDrawDevice(SkBaseDevice* srcDev, const SkPaint* paint) {
    ...
    while (iter.next()) {
        ...
        if (!filter) {
            ...
            DrawFilteredImage(dstDev, std::move(srcBuffer), mapping, filter, *noFilterPaint);
        }
    }
...
void SkCanvas::onDrawImage(const SkImage* image, SkScalar x, SkScalar y, const SkPaint* paint) {
    ...
    while (iter.next()) {
        ...
        if (special) {
            ...
            DrawFilteredImage(iter.fDevice, special, mapping, filter.get(), pnt);
        } else {
