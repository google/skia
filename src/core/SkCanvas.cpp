...
void SkCanvas::onDrawDrawable(SkDrawable* dr, const SkMatrix* matrix) {
    ...
    if (this->getDevice()->isDrawableSupported(dr)) {
        ...
        while (iter.next()) {
            ...
            // matrix parameter.
            const SkMatrix& ctm = iter.getMatrix();
            const SkMatrix& combinedMat = SkMatrix::Concat(ctm, matrix ? *matrix : SkMatrix::I());
            iter.fDevice->drawDrawable(dr, combinedMat, iter.fClipStack);
        }
