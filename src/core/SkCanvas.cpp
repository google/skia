...
SkCanvas::SkCanvas(const SkBitmap& bitmap, SkRasterAllocator* alloc, SkRasterAllocator::Handle hndl)
    : fMCStack(sizeof(MCRec), fMCRecStorage, sizeof(fMCRecStorage))
    , fProps(SkSurfaceProps::kLegacyFontHost_InitType)
    , fAllocator(alloc)
    , fConservativeRasterClip(false)
...
void* SkCanvas::accessTopRasterHandle() const {
    if (fAllocator && fMCRec->fTopLayer->fDevice) {
        const SkBaseDevice* dev = fMCRec->fTopLayer->fDevice;
        SkRasterAllocator::Handle handle = dev->getRasterHandle();
...
static bool install(SkBitmap* bm, const SkImageInfo& info,
                    const SkRasterAllocator::Rec& rec) {
    return bm->installPixels(info, rec.fPixels, rec.fRowBytes, nullptr,
                             rec.fReleaseProc, rec.fReleaseCtx);
}

SkRasterAllocator::Handle SkRasterAllocator::allocBitmap(const SkImageInfo& info, SkBitmap* bm) {
    SkRasterAllocator::Rec rec;
...
std::unique_ptr<SkCanvas> SkRasterAllocator::makeCanvas(const SkImageInfo& info, const Rec* rec) {
    SkBitmap bm;
    Handle hndl;
    ...
    return hndl ? std::unique_ptr<SkCanvas>(new SkCanvas(bm, this, hndl)) : nullptr;
}
