...
struct Layer {
    ...
    Layer(sk_sp<SkBaseDevice> device, sk_sp<SkImageFilter> imageFilter,
...
public:
    ...
    void newLayer(sk_sp<SkBaseDevice> layerDevice, sk_sp<SkImageFilter> filter,
                  const SkSamplingOptions& restoreSampling, const SkPaint& restorePaint) {
        SkASSERT(!fBackImage);
        fLayer = std::make_unique<Layer>(std::move(layerDevice), std::move(filter),
                                         restoreSampling, restorePaint);
...
void SkCanvas::internalDrawDeviceWithFilter(SkBaseDevice* src,
    ...
    if (filterInput) {
        SkSamplingOptions sampling{mapping.deviceMatrix().isTranslate() ? SkFilterMode::kNearest
...
void SkCanvas::internalSaveLayer(const SaveLayerRec& rec, SaveLayerStrategy strategy) {
    ...
    // device's coordinate space. We canonicalize on AA since devices already automatically check
    // for image draws that can be turned into non-AA sprites.
    ...
    // const bool restoreAsSprite = newLayerMapping.deviceMatrix().isTranslate();
    // restorePaint.setAntiAlias(!restoreAsSprite);
    // SkSamplingOptions restoreSampling{restoreAsSprite ? SkFilterMode::kNearest
    //                                                   : SkFilterMode::kLinear,
    //                                   SkMipmapMode::kNone};
    ...
    if (initBackdrop) {
        ...
        // SkSamplingOptions backdropSampling{newDevice->localToDevice().isScaleTranslate() ?
