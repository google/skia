...
struct DeviceCM {
    ...
    DeviceCM(sk_sp<SkBaseDevice> device, sk_sp<SkImageFilter> imageFilter, const SkPaint& paint)
            : fNext(nullptr)
            , fDevice(std::move(device))
...
public:
    ...
    */
    ...
    // True if any clip call was made on the canvas while this rec was active
    bool fModifiedClip;
...
private:
    ...
    const SkPaint*  fPaint;     // Will not be null if next returned true
};
...
void SkCanvas::internalSaveLayer(const SaveLayerRec& rec, SaveLayerStrategy strategy) {
    ...
    bool restoreAsSprite;
    skif::Mapping newLayerMapping;
    skif::LayerSpace<SkIRect> layerBounds;
    SkBaseDevice* priorDevice = this->getTopDevice();
    if (!priorDevice || (fMCRec->fTopLayer && fMCRec->fTopLayer->fNext)) {
        // If the prior device is unclipped, the new layer needs to be sized based on the globally
        // tracked clip bounds, which does not include unclipped layers.
        std::tie(newLayerMapping, layerBounds) = get_layer_mapping_and_bounds(
                filter, fMCRec->fMatrix.asM33(),
                skif::DeviceSpace<SkIRect>(this->getDeviceClipBounds()),
                skif::ParameterSpace<SkRect>::Optional(rec.fBounds),
                must_cover_prior_device(rec.fBackdrop, restorePaint));
        restoreAsSprite = SkTreatAsSprite(newLayerMapping.deviceMatrix(),
                                          SkISize(layerBounds.size()), restorePaint);
    } else {
        // Otherwise size the new layer relative to the prior device, which may already be aligned
        // locally for image filters.
        std::tie(newLayerMapping, layerBounds) = get_layer_mapping_and_bounds(
                filter, priorDevice->localToDevice(),
                skif::DeviceSpace<SkIRect>(priorDevice->devClipBounds()),
                skif::ParameterSpace<SkRect>::Optional(rec.fBounds),
                must_cover_prior_device(rec.fBackdrop, restorePaint));
        // Determine draw quality before we update device matrix to map all the way to global
        restoreAsSprite = SkTreatAsSprite(newLayerMapping.deviceMatrix(),
                                          SkISize(layerBounds.size()), restorePaint);
    ...
    if (kFullLayer_SaveLayerStrategy == strategy && priorDevice) {
        ...
        newDevice.reset(priorDevice->onCreateDevice(createInfo, rec.fPaint)); // hint original paint
    }
    ...
    if (!newDevice) {
        ...
        if (boundsAffectsClip) {
            ...
            UPDATE_DEVICE_CLIP(device->replaceClip(globalLayerBounds));
        }
        return;
    }
    ...
    // Adjust quality based on how the new layer will be restored to the prior device
    ...
    DeviceCM* layer = new DeviceCM(newDevice, sk_ref_sp(filter), restorePaint);
...
void SkCanvas::internalRestore() {
    ...
    // and remember this before the restore
    bool updateQRBounds = SkToBool(layer) || fMCRec->fModifiedClip;
    ...
    */
    ...
    if (fMCRec) {
        ...
        // Only recompute QR bounds if the clip or device stack were modified; otherwise it was just
        // matrix state changing which wouldn't change clip bounds
        if (updateQRBounds) {
            fQuickRejectBounds = qr_clip_bounds(this->computeDeviceClipBounds());
        }
        this->validateClip();
...
void SkCanvas::androidFramework_setDeviceClipRestriction(const SkIRect& rect) {
    ...
    if (!fClipRestrictionRect.isEmpty()) {
        // We only resolve deferred saves when the clip restriction isn't being removed
        // (the empty rect). Hopefully this API goes away soon.
        this->checkForDeferredSave();
    }
...
#ifdef SK_DEBUG
    // Verify that fQuickRejectBounds is set properly
    this->validateClip();
...
const SkPaint& SkCanvas::LayerIter::paint() const {
    ...
    // If this is null, caller ignored result of next()
    SkASSERT(paint);
    return *paint;
}
