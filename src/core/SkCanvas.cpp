...
void SkCanvas::onDrawDrawable(SkDrawable* dr, const SkMatrix* matrix) {
    ...
    if (this->getDevice()->drawableSupported(dr)) {
        // We don't actually use a paint for drawDrawable, so we use a default paint here so it does
        // not affect the loop here. We should have a version of the macro the does not require a
        // paint to be passed in.
        SkPaint paint;
