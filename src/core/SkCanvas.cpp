...
void SkCanvas::drawGlyphs(int count, const SkGlyphID glyphs[], const SkPoint positions[],
    ...
    auto buffer = builder.allocRunPos(font, n);
    memcpy(buffer.glyphs, glyphs, n * sizeof(SkGlyphID));
    memcpy(buffer.points(), positions, n * sizeof(SkPoint));
...
void SkCanvas::drawGlyphs(int count, const SkGlyphID* glyphs, const SkPoint* positions,
    ...
    if (n <= 0) { return; }
    SkTextBlobBuilder builder;
    auto buffer = builder.allocRunTextPos(font, n, textSize);
    memcpy(buffer.glyphs, glyphs, n * sizeof(SkGlyphID));
    memcpy(buffer.points(), positions, n * sizeof(SkPoint));
    memcpy(buffer.clusters, clusters, n * sizeof(uint32_t));
    memcpy(buffer.utf8text, utf8text, textSize);
    this->drawTextBlob(builder.make(), origin.x(), origin.y(), paint);
}
