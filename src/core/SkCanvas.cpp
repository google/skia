...
void SkCanvas::DrawDeviceWithFilter(SkBaseDevice* src, const SkImageFilter* filter,
    ...
    if (filter) {
        ...
        SkMatrix localCTM;
        sk_sp<SkImageFilter> modifiedFilter = filter->applyCTM(ctm, &localCTM, true);
...
void SkCanvas::internalSaveLayer(const SaveLayerRec& rec, SaveLayerStrategy strategy) {
    ...
    if (imageFilter) {
        ...
        sk_sp<SkImageFilter> modifiedFilter = imageFilter->applyCTM(stashedMatrix, &modifiedCTM);
        if (*modifiedFilter != *imageFilter) {
