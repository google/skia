...
// "this->getTopDevice()->code;" is "TOP_DEVICE(code);", while handling whether or not the top device
// is actually null.
...
void SkCanvas::onDrawPoints(PointMode mode, size_t count, const SkPoint pts[],
    ...
    if (paint.getImageFilter()) {
        ...
        TOP_DEVICE(drawPoints(mode, count, pts, paint));
    }
}
...
void SkCanvas::onDrawVerticesObject(const SkVertices* vertices, SkBlendMode bmode,
    ...
    if (paint.getImageFilter()) {
        ...
        TOP_DEVICE(drawVertices(vertices, bmode, paint));
    }
}
...
void SkCanvas::onDrawPatch(const SkPoint cubics[12], const SkColor colors[4],
    ...
    if (paint.getImageFilter()) {
        ...
        TOP_DEVICE(drawPatch(cubics, colors, texCoords, bmode, paint));
    }
}
...
void SkCanvas::onDrawAtlas(const SkImage* atlas, const SkRSXform xform[], const SkRect tex[],
    ...
    if (paint->getImageFilter()) {
        ...
        TOP_DEVICE(drawAtlas(atlas, xform, tex, colors, count, bmode, *paint));
    }
}
