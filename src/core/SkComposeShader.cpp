...
bool SkComposeShader::onAppendStages(SkRasterPipeline* pipeline, SkColorSpace* dstCS,
    ...
    float* xy_storage = storage->fXY;
    float* rgba_storage = storage->fRGBA;
    ...
    pipeline->append(SkRasterPipeline::store_f32, &xy_storage);
    ...
    pipeline->append(SkRasterPipeline::store_f32, &rgba_storage);
    // Now we restore the device x,y for the next shader
    pipeline->append(SkRasterPipeline::load_f32, &xy_storage);
    ...
    pipeline->append(SkRasterPipeline::load_f32, &rgba_storage);
    SkBlendMode_AppendStages(fMode, pipeline);
    if (origPaint.getAlpha()) {
        // append apply_alpha stage?
    }
    return true;
}
