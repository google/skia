...
bool SkComposeShader::onAppendStages(SkRasterPipeline* pipeline, SkColorSpace* dstCS,
    ...
    pipeline->append(SkRasterPipeline::store_f32, storage->fXY);
    ...
    pipeline->append(SkRasterPipeline::store_f32, storage->fFirstShader);
    // Now we restore the device x,y for the next shader
    pipeline->append(SkRasterPipeline::load_f32, storage->fXY);
    ...
    pipeline->append(SkRasterPipeline::load_f32, storage->fFirstShader);
