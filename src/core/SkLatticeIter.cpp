...
SkLatticeIter::SkLatticeIter(const SkCanvas::Lattice& lattice, const SkRect& dst) {
    ...
    if (lattice.fRectTypes) {
        ...
        const SkCanvas::Lattice::RectangleType* flags = lattice.fRectTypes;
...
bool SkLatticeIter::next(SkRect* src, SkRect* dst, bool* isSolidColor, SkColor* solidColor) {
    ...
    if (fRectTypes.count() > 0
        ...
        return this->next(src, dst, isSolidColor, solidColor);
    }

    src->set(fSrcX[x], fSrcY[y], fSrcX[x + 1], fSrcY[y + 1]);
    dst->set(fDstX[x], fDstY[y], fDstX[x + 1], fDstY[y + 1]);
    if (isSolidColor && solidColor) {
        *isSolidColor = fRectTypes.count() > 0
                     && SkToBool(SkCanvas::Lattice::kUseSolidColor == fRectTypes[currRect]);
        if (*isSolidColor) {
            *solidColor = fColors[currRect];
        }
    }
    return true;
}
