...
public:
    ...
    bool onFilterPath(SkPath* dst, const SkPath& src, SkStrokeRec* rec,
        ...
        if (as_PEB(fPE1)->filterPath(&tmp, src, rec, cullRect)) {
            ptr = &tmp;
        }
        return as_PEB(fPE0)->filterPath(dst, *ptr, rec, cullRect);
    }
...
public:
    ...
    bool onFilterPath(SkPath* dst, const SkPath& src, SkStrokeRec* rec,
        ...
        return as_PEB(fPE0)->filterPath(dst, src, rec, cullRect) |
               as_PEB(fPE1)->filterPath(dst, src, rec, cullRect);
    }
