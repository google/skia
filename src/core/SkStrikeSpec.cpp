...
SkStrikeSpec::MakeSDFT(const SkFont& font, const SkPaint& paint,
    ...
    SkFont dfFont = options.initDistanceFieldFont(
            font, deviceMatrix, &storage.fStrikeToSourceRatio);
    ...
    std::tie(minScale, maxScale) = options.initDistanceFieldMinMaxScale(
            font.getSize(), deviceMatrix);
