...
template <typename T>
static bool SK_WARN_UNUSED_RESULT read_uint(SkStream* stream, T* t) {
    size_t i;
    if (!stream->readPackedUInt(&i)) { return false; }
    *t = i;
    return true;
}
    ...
    for (size_t id; (id = read_id(stream)) != kSentinel;) {
        switch (id) {
            ...
            case kPostscriptName:
                ...
                break;
            case kFontAxes:
                if (!read_uint(stream, &axisCount)) { return false; }
                axis.reset(axisCount);
                for (size_t i = 0; i < axisCount; ++i) {
                    if (!read_uint(stream, &axis[i])) { return false; }
                }
                break;
            case kFontIndex:
                if (!read_uint(stream, &index)) { return false; }
                break;
