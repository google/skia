...
struct StrideOfA1 {
    StrideOfA1(const uint8_t* ptr, int offset) : fPtr(ptr), fOffset(7 - offset) {}
...
struct StrideOfARGB {
    StrideOfARGB(const uint32_t* ptr) : fPtr(ptr) {}
...
struct StrideOfLCD {
    StrideOfLCD(const uint16_t* ptr) : fPtr(ptr) {}
    ...
    for (int y = 0; y < srcH; y++) {
        ...
        switch (src.fFormat) {
            ...
            case SkMask::kARGB32_Format: {
                ...
                scanW.blur(StrideOfARGB(argbStart), StrideOfARGB(argbStart + srcW),
                           tmpStart, tmpW, tmpStart + tmpW * tmpH);
                } break;
            case SkMask::kLCD16_Format: {
