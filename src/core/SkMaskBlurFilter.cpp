...
namespace {
struct LoadBW {
    static constexpr int StrideOf8 = 1;
    static SK_ALWAYS_INLINE Sk8h load(const uint8_t* from, int width) {
        ...
        for (int i = 0; i < width; ++i) {
            buffer[i] = (value >> (7 - i)) & 1 ? 0xFF : 0;
        }
        auto v = SkNx_cast<uint16_t>(Sk8b::Load(buffer));
        // Convert from 0-255 to 8.8 encoding.
        return v << 8;
    }
};
struct LoadA8 {
    static constexpr int StrideOf8 = 8 * sizeof(uint8_t);
    static SK_ALWAYS_INLINE Sk8h load(const uint8_t* from, int width) {
        SkASSERT(0 < width && width <= 8);
        uint8_t buffer[8];
        if (width < 8) {
...
struct LoadARGB {
    static constexpr int StrideOf8 = 8 * sizeof(uint32_t);
    static SK_ALWAYS_INLINE Sk8h load(const uint8_t* from, int width) {
        SkASSERT(0 < width && width <= 8);
        uint8_t buffer[8];
        sk_bzero(buffer, sizeof(buffer));
        for (int i = 0; i < width; ++i) {
            uint32_t packed = reinterpret_cast<const uint32_t*>(from)[i]
            buffer[i] = SkGetPackedA32(packed);
        }
        auto v = SkNx_cast<uint16_t>(Sk8b::Load(buffer));
        // Convert from 0-255 to 8.8 encoding.
        return v << 8;
...
struct LoadLCD {
    static constexpr int StrideOf8 = 8 * sizeof(uint16_t);
    static SK_ALWAYS_INLINE Sk8h load(const uint8_t* from, int width) {
        SkASSERT(0 < width && width <= 8);
        uint8_t buffer[8];
        sk_bzero(buffer, sizeof(buffer));
        for (int i = 0; i < width; ++i) {
            unsigned packed = reinterpret_cast<const uint16_t*>(from)[i];
...
static SK_ALWAYS_INLINE void blur_row(
    ...
    int x = 0;
    for (; x <= srcW - 8; x += 8) {
        blur(LoadA8::load(src, 8), g0, g1, g2, g3, g4, &d0, &d8);
    ...
    if (srcTail > 0) {
        ...
        blur(LoadA8::load(src, srcTail), g0, g1, g2, g3, g4, &d0, &d8);
...
template <typename Loader> static SK_ALWAYS_INLINE void blur_column(
...
// BlurY will be one of blur_y_radius_(1|2|3|4).
template <typename Loader> static SK_ALWAYS_INLINE void blur_y_rect(
    ...
    int x = 0;
    for (; x <= srcW - 8; x += 8) {
        blur_column<Loader>(blur, radius, 8,
                            g0, g1, g2, g3, g4,
                            src, srcRB, srcH,
                            dst, dstRB);
        src += Loader::StrideOf8;
        dst += 8;
    }

    int xTail = srcW - x;
    if (xTail > 0) {
        blur_column<Loader>(blur, radius, xTail,
                            g0, g1, g2, g3, g4,
                            src, srcRB, srcH,
                            dst, dstRB);
    }
...
template <typename Loader> SK_ATTRIBUTE(noinline) static void direct_blur_y(
        int radius, uint16_t* gauss,
        const uint8_t* src, size_t srcRB, int srcW, int srcH,
              uint8_t* dst, size_t dstRB) {
    ...
    switch (radius) {
        case 1:
            blur_y_rect<Loader>(blur_y_radius_1, 1, gauss,
                                src, srcRB, srcW, srcH,
                                dst, dstRB);
            break;

        case 2:
            blur_y_rect<Loader>(blur_y_radius_2, 2, gauss,
                                src, srcRB, srcW, srcH,
                                dst, dstRB);
            break;

        case 3:
            blur_y_rect<Loader>(blur_y_radius_3, 3, gauss,
                                src, srcRB, srcW, srcH,
                                dst, dstRB);
            break;

        case 4:
            blur_y_rect<Loader>(blur_y_radius_4, 4, gauss,
                                src, srcRB, srcW, srcH,
                                dst, dstRB);
            break;
    ...
    *dst = SkMask::PrepareDestination(radiusX, radiusY, src);
    ...
    switch (src.fFormat) {
        case SkMask::kBW_Format:
            direct_blur_y<LoadBW>(radiusY, gaussFactorsY,
                                  src.fImage, srcRB, srcW, srcH,
                                  dst->fImage + radiusX, dstRB);
            break;
        case SkMask::kA8_Format:
            direct_blur_y<LoadA8>(radiusY, gaussFactorsY,
                                  src.fImage, srcRB, srcW, srcH,
                                  dst->fImage + radiusX, dstRB);
            break;
        case SkMask::kARGB32_Format:
            direct_blur_y<LoadARGB>(radiusY, gaussFactorsY,
                                    src.fImage, srcRB, srcW, srcH,
                                    dst->fImage + radiusX, dstRB);
            break;
        case SkMask::kLCD16_Format:
            direct_blur_y<LoadLCD>(radiusY, gaussFactorsY,
                                   src.fImage, srcRB, srcW, srcH,
                                   dst->fImage + radiusX, dstRB);
            break;
        default:
...
struct StrideOfA1 {
    StrideOfA1(uint8_t* ptr, int offset) : fPtr(ptr), fOffset(7 - offset) {}
    StrideOfA1(const StrideOfA1& that) : fPtr(that.fPtr), fOffset(that.fOffset) {}
    StrideOfA1& operator++() {
        if (0 < fOffset ) {
            --fOffset;
        } else {
    ...
    bool operator<(const StrideOfA1& that) {
...
struct StrideOfARGB {
    StrideOfARGB(uint32_t* ptr) : fPtr(ptr) {}
    StrideOfARGB(const StrideOfARGB& that) : fPtr(that.fPtr) {}
    StrideOfARGB& operator++() { ++fPtr; return *this; }
    StrideOfARGB& operator--() { --fPtr; return *this; }
    uint8_t operator*() { return SkGetPackedA32(*fPtr); }
    bool operator<(const StrideOfARGB& that) { return fPtr < that.fPtr; }
    const uint32_t* fPtr;
};

struct StrideOfLCD {
    StrideOfLCD(uint16_t* ptr) : fPtr(ptr) {}
    StrideOfLCD(const StrideOfLCD& that) : fPtr(that.fPtr) {}
    StrideOfLCD& operator++() { ++fPtr; return *this; }
    StrideOfLCD& operator--() { --fPtr; return *this; }
    uint8_t operator*() {
    ...
    bool operator<(const StrideOfLCD& that) { return fPtr < that.fPtr; }
    const uint16_t* fPtr;
};
...
SkIPoint SkMaskBlurFilter::blur(const SkMask& src, SkMask* dst) const {
    ...
    *dst = SkMask::PrepareDestination(borderW, borderH, src);
    ...
    for (int y = 0; y < srcH; y++) {
        ...
        switch (src.fFormat) {
            case SkMask::kBW_Format:
                scanW.blur(StrideOfA1(srcStart, 0), StrideOfA1(srcStart + (srcW / 8), srcW % 8),
                           tmpStart, tmpW, tmpStart + tmpW * tmpH);
                break;
            case SkMask::kA8_Format:
                scanW.blur(srcStart, srcStart + srcW,
                           tmpStart, tmpW, tmpStart + tmpW * tmpH);
                break;
            case SkMask::kARGB32_Format:
                uint32_t* argbStart = reinterpret_cast<uint32_t*>(srcStart);
                scanW.blur(StrideOfARGB(srcStart), StrideOfARGB(srcStart + srcW),
                           tmpStart, tmpW, tmpStart + tmpW * tmpH);
                break;
            case SkMask::kLCD16_Format: {
                uint16_t* lcdStart = reinterpret_cast<uint16_t*>(srcStart);
                scanW.blur(StrideOfLCD(lcdStart), StrideOfLCD(lcdStart + srcW),
                           tmpStart, tmpW, tmpStart + tmpW * tmpH);
                } break;
            default:
                SK_ABORT("Unhandled format.");
