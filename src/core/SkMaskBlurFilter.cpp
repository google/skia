...
struct LoadA8 {
    static constexpr int StrideOf8 = 8;
    static SK_ALWAYS_INLINE Sk8h load(const uint8_t* from, int width) {
        SkASSERT(0 < width && width <= 8);
        uint8_t buffer[8];
        if (width < 8) {
...
struct LoadARGB {
    static constexpr int StrideOf8 = 8 * 4;
    static SK_ALWAYS_INLINE Sk8h load(const uint8_t* from, int width) {
        SkASSERT(0 < width && width <= 8);
        uint8_t buffer[8];
        sk_bzero(buffer, sizeof(buffer));
...
struct LoadLCD {
    static constexpr int StrideOf8 = 8 * 2;
    static SK_ALWAYS_INLINE Sk8h load(const uint8_t* from, int width) {
        SkASSERT(0 < width && width <= 8);
        uint8_t buffer[8];
        sk_bzero(buffer, sizeof(buffer));
    ...
    switch (src.fFormat) {
        case SkMask::kBW_Format:
            direct_blur_y<LoadBW>(radiusY, gaussFactorsY,
                                  src.fImage + srcAlphaOffset, srcRB, srcW, srcH,
...
struct StrideOfA1 {
    StrideOfA1(uint8_t* ptr, int offset) : fPtr(ptr), fOffset(offset) {}
    ...
    uint8_t operator*() { return ((*fPtr) >> (7 - fOffset)) & 1 ? 0xFF : 0; }
...
struct StrideOf4 {
    StrideOf4(uint8_t* ptr) : fPtr(ptr) {}
    StrideOf4(const StrideOf4& that) : fPtr(that.fPtr) {}
    StrideOf4& operator++() { fPtr += 4; return *this; }
    StrideOf4& operator--() { fPtr -= 4; return *this; }
    uint8_t operator*() { return *fPtr; }
    bool operator<(const StrideOf4& that) { return fPtr < that.fPtr; }
    const uint8_t* fPtr;
};
    ...
    for (int y = 0; y < srcH; y++) {
        ...
        switch (src.fFormat) {
            ...
            case SkMask::kA8_Format:
                ...
                break;
            case SkMask::kARGB32_Format:
                srcStart += SK_A32_SHIFT / 8;
                scanW.blur(StrideOf4(srcStart), StrideOf4(srcStart + srcW * 4),
                           tmpStart, tmpW, tmpStart + tmpW * tmpH);
                break;
            case SkMask::kLCD16_Format: {
