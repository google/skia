...
static uint64_t weight_from_diameter(double sigma, uint32_t d) {
    if (sigma < 2) {
        return d;
    }
    uint64_t d2 = d * d;
...
static uint32_t filter_window(double sigma) {
    if (sigma < 2) {
        uint32_t radius = static_cast<uint32_t>(1.5 * sigma + 0.5);
        return 2 * radius + 1;
    }
...
SkMaskBlurFilter::FilterInfo::FilterInfo(double sigma)
    ...
    , fWeight{weight_from_diameter(sigma, fFilterWindow)}
...
void SkMaskBlurFilter::blurOneScan(
    ...
    if (info.isSmall()) {
        this->blurOneScanAvg(info, src, srcStride, srcEnd, dst, dstStride, dstEnd);
    } else {
...
// Blur one horizontal scan into the dst.
void SkMaskBlurFilter::blurOneScanAvg(
    FilterInfo info,
    const uint8_t* src, size_t srcStride, const uint8_t* srcEnd,
          uint8_t* dst, size_t dstStride,       uint8_t* dstEnd) const {
