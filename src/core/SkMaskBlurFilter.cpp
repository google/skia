...
public:
    ...
    bool needsBlur()    const override { return fPass0Size > 0; }
...
int calculate_space_scale_factors(double sigma, uint16_t* factors) {
    SkASSERT(sigma < 2);
...
using Acc = Sk8h&;
using Val = const Sk8h&;

SK_ALWAYS_INLINE void blur_x_radius_1(
        const Sk8h& vv,
        Sk8h& acc0, Sk8h& acc1,
        const Sk8h& gauss0,
        const Sk8h& gauss1,
        ...
        const Sk8h& gauss3,
        const Sk8h& gauss4) {
    auto v0 = vv.mulHi(gauss0);
    auto v1 = vv.mulHi(gauss1);
...
SK_ALWAYS_INLINE void blur_x_radius_2(
        const Sk8h& vv,
        Sk8h& acc0, Sk8h& acc1,
        const Sk8h& gauss0,
        const Sk8h& gauss1,
...
SK_ALWAYS_INLINE void blur_x_radius_3(
        const Sk8h& vv,
        Sk8h& acc0, Sk8h& acc1,
        const Sk8h& gauss0,
        const Sk8h& gauss1,
...
SK_ALWAYS_INLINE void blur_x_radius_4(
        const Sk8h& vv,
        Sk8h& acc0, Sk8h& acc1,
        const Sk8h& gauss0,
        const Sk8h& gauss1,
...
SK_ALWAYS_INLINE void direct_blur_x_v2(
        Blur blur,
        uint16_t* gauss,
        const uint8_t* src, int srcCount,
        uint8_t* dst, int dstCount) {

    Sk8h acc0{0u}, acc1{0u};
    ...
    for (; x <= srcCount - 8; x += 8){
        auto v = SkNx_cast<uint16_t>(Sk8b::Load(src));
        auto vv = (v << 8) | v;

        blur(vv, acc0, acc1, gauss0, gauss1, gauss2, gauss3, gauss4);

        SkNx_cast<uint8_t>(acc0 >> 8).store(dst);
    ...
    if (srcTail > 0) {
        ...
        for (int i = 0; i < srcTail; i++) {
            buffer[i] = src[i];
        }

        Sk8h v = SkNx_cast<uint16_t>(Sk8b::Load(buffer));
        Sk8h vv = (v << 8) | v;

        blur(vv, acc0, acc1, gauss0, gauss1, gauss2, gauss3, gauss4);

        SkNx_cast<uint8_t>(acc0 >> 8).store(buffer);
        int dstTail = std::min(8, dstCount - x);

        for (int i = 0; i < dstTail; i++) {
            dst[i] = buffer[i];
        }
        acc0 = acc1;
        dst += dstTail;
 ...
 void direct_blur_x(
     ...
     if (radius == 2) {
         direct_blur_x_v2(blur_x_radius_2, gauss, src, srcCount, dst, dstCount);
     } else if (radius == 3) {
         direct_blur_x_v2(blur_x_radius_3, gauss, src, srcCount, dst, dstCount);
     } else if (radius == 4) {
         direct_blur_x_v2(blur_x_radius_4, gauss, src, srcCount, dst, dstCount);
     } else {
         // radius == 1
         direct_blur_x_v2(blur_x_radius_1, gauss, src, srcCount, dst, dstCount);
     }
 }

SK_ALWAYS_INLINE Sk8h blur_y_radius_1(
        Val vv,
        Acc acc0, Acc acc1, Acc acc2, Acc acc3, Acc acc4, Acc acc5, Acc acc6, Acc acc7,
        Val gauss0, Val gauss1, Val gauss2, Val gauss3, Val gauss4) {
    auto v0 = vv.mulHi(gauss0);
    Sk8h v1 = vv.mulHi(gauss1);
...
SK_ALWAYS_INLINE Sk8h blur_y_radius_2(
        Val vv,
        Acc acc0, Acc acc1, Acc acc2, Acc acc3, Acc acc4, Acc acc5, Acc acc6, Acc acc7,
        Val gauss0, Val gauss1, Val gauss2, Val gauss3, Val gauss4) {
    auto v0 = vv.mulHi(gauss0);
...
SK_ALWAYS_INLINE Sk8h blur_y_radius_4(
    Val vv,
    Acc acc0, Acc acc1, Acc acc2, Acc acc3, Acc acc4, Acc acc5, Acc acc6, Acc acc7,
    Val gauss0, Val gauss1, Val gauss2, Val gauss3, Val gauss4) {
    auto v0 = vv.mulHi(gauss0);
...
template<typename Blur>
SK_ALWAYS_INLINE void direct_blur_y_v2(
        Blur blur, int radius,
        uint16_t* gauss, int width,
        const uint8_t* src, size_t srcStride, int srcCount,
        uint8_t* dst, size_t dstStride, int dstCount) {
    SkASSERT(width < 8);

    Sk8h acc0{0u}, acc1{0u}, acc2{0u}, acc3{0u}, acc4{0u}, acc5{0u}, acc6{0u}, acc7{0u};
    Sk8h gauss0{gauss[0]},
         gauss1{gauss[1]},
         gauss2{gauss[2]},
         gauss3{gauss[3]},
    ...
    if (width == 0) {
        ...
        for (; y < srcCount; y += 1) {
            ...
            auto m = blur(vv, acc0, acc1, acc2, acc3, acc4, acc5, acc6, acc7,
                          gauss0, gauss1, gauss2, gauss3, gauss4);
            SkNx_cast<uint8_t>(m >> 8).store(dst);

            src += srcStride;
            dst += dstStride;
        }

        auto flush = [&](Val v0, Val v1) {
            SkNx_cast<uint8_t>(v0 >> 8).store(dst);
            dst += dstStride;
            SkNx_cast<uint8_t>(v1 >> 8).store(dst);
            dst += dstStride;
        };
        ...
        for (; y < srcCount; y += 1) {
            ...
            auto m = blur(vv, acc0, acc1, acc2, acc3, acc4, acc5, acc6, acc7,
                          gauss0, gauss1, gauss2, gauss3, gauss4);
            SkNx_cast<uint8_t>(m >> 8).store(buffer);
        ...
        auto flush = [&](Val v0, Val v1) {
            uint8_t buffer[8];
            SkNx_cast<uint8_t>(v0 >> 8).store(buffer);
            for (int i = 0; i < width; i++) {
                dst[i] = buffer[i];
            }
            dst += dstStride;
            SkNx_cast<uint8_t>(v1 >> 8).store(buffer);
            for (int i = 0; i < width; i++) {
                dst[i] = buffer[i];
            }
            dst += dstStride;
...
void direct_blur_y(
    ...
    if (radius == 2) {
        direct_blur_y_v2(blur_y_radius_2, 2, gauss, width,
                         src, srcStride, srcCount, dst, dstStride, dstCount);
    } else if (radius == 3) {
        direct_blur_y_v2(blur_y_radius_3, 3, gauss, width,
                         src, srcStride, srcCount, dst, dstStride, dstCount);
    } else if (radius == 4) {
        direct_blur_y_v2(blur_y_radius_4, 4, gauss, width,
                         src, srcStride, srcCount, dst, dstStride, dstCount);
    } else {
        // radius == 1
        direct_blur_y_v2(blur_y_radius_1, 1, gauss, width,
                         src, srcStride, srcCount, dst, dstStride, dstCount);
    }
}

bool gUseBessel = true;
SkIPoint small_blur(double sigmaX, double sigmaY, const SkMask& src, SkMask* dst) {
    ...
    int radiusX, radiusY;
    if (gUseBessel) {
        radiusX = calculate_space_scale_factors(sigmaX, gaussFactorsX);
        radiusY = calculate_space_scale_factors(sigmaY, gaussFactorsY);
    } else {
        radiusX = calculate_gauss_factors(sigmaX, gaussFactorsX);
    ...
    for (int y = 0; y < dstH; y++) {
        uint8_t* from = &dst->fImage[y * dst->fRowBytes + radiusX];
...
SkIPoint SkMaskBlurFilter::blur(const SkMask& src, SkMask* dst) const {
    ...
    if (gBlurNew && fSigmaW < 2.0 && fSigmaH < 2.0) {
...
void direct_blur_y2(
        uint16_t gauss[5], int width,
        const uint8_t* src, size_t srcStride, int srcCount,
        uint8_t* dst, size_t dstStride, int dstCount) {
    SkASSERT(width < 8);

    Sk8h acc0{0u}, acc1{0u}, acc2{0u}, acc3{0u};
    Sk8h gauss0{gauss[0]}, gauss1{gauss[1]}, gauss2{gauss[2]};
    ...
    if (width == 0) {
        ...
        SkNx_cast<uint8_t>(acc0 >> 8).store(dst);
        dst += dstStride;
        SkNx_cast<uint8_t>(acc1 >> 8).store(dst);
        dst += dstStride;
        SkNx_cast<uint8_t>(acc2 >> 8).store(dst);
        dst += dstStride;
        SkNx_cast<uint8_t>(acc3 >> 8).store(dst);
        dst += dstStride;
        ...
        for (; y < srcCount; y += 1) {
            ...
            auto v0 = vv.mulHi(gauss[0]);
            auto v1 = vv.mulHi(gauss[1]);
            auto v2 = vv.mulHi(gauss[2]);
        ...
        auto flush = [&](const Sk8h& v) {
            uint8_t buffer[8];
            SkNx_cast<uint8_t>(v >> 8).store(buffer);
            for (int i = 0; i < width; i++) {
                dst[i] = buffer[i];
            }
            dst += dstStride;
