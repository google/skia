...
static SK_ALWAYS_INLINE Sk8h load(const uint8_t* from, int width, uint8_t buffer[8]) {
    if (width < 8) {
...
static SK_ALWAYS_INLINE void store(uint8_t* to, const Sk8h& v, int width, uint8_t buffer[8]) {
...
// system is
// setup to minimize the number of multiplies needed.
//
// Explanation:
//    Blurring a specific mask value is given by the following equation where D_n is the resulting
// mask
// value and S_n is the source value. The example below is for a filter with a radius of 1 and a
// width of 3 (radius == (width-1)/2). The indexes for the source and destination are aligned. The
// filter is given by G_n
// where n is the symmetric filter value.
//
...
// You can see that the D[n+8] and D[n+9] values overlap the two sets, using parts of both S[n
// ..7]
// and S[n+8..n+15].
//
...
static SK_ALWAYS_INLINE void blur_row(
    ...
    int x = 0;
    for (; x <= srcW - 8; x += 8) {
        blur(load(src, 8, buffer), g0, g1, g2, g3, g4, &d0, &d8);

        store(dst, d0, 8, buffer);
    ...
    if (srcTail > 0) {
        ...
        blur(load(src, srcTail, buffer), g0, g1, g2, g3, g4, &d0, &d8);

        int dstTail = std::min(8, dstW - x);
        store(dst, d0, dstTail, buffer);
...
static SK_ALWAYS_INLINE void blur_column(
    ...
    // The buffer does not need to be cleared because the untouched bytes will never be used.
    uint8_t buffer[8];
    Sk8h d01{0u}, d12{0u}, d23{0u}, d34{0u},
         d45{0u}, d56{0u}, d67{0u}, d78{0u};

    auto flush = [&](uint8_t* to, const Sk8h& v0, const Sk8h& v1) {
        store(to, v0, width, buffer);
        to += dstStride;
        store(to, v1, width, buffer);
        return to + dstStride;
    };

    for (int y = 0; y < srcH; y += 1) {
        auto s = load(src, width, buffer);
        auto b = blur(s,
                      g0, g1, g2, g3, g4,
                      &d01, &d12, &d23, &d34, &d45, &d56, &d67, &d78);
        store(dst, b, width, buffer);
        src += srcStride;
        dst += dstStride;
    }
