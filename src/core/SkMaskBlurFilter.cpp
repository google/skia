...
static void direct_blur_x(
    ...
    if (srcTail > 0) {
        ...
        int dstOut = std::min(8, dstCount - x);
        memcpy(dst, buffer, std::min(8, dstOut));
...
static void direct_blur_y(
        int radius, uint16_t gauss[4],const uint8_t* src, size_t srcCount, uint8_t* dst, size_t
dstCount) {
