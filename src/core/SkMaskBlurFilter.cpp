...
static uint64_t weight(uint32_t d) {
    uint64_t d2 = d * d;
    uint64_t d3 = d2 * d;
    if ((d&1) == 0) {
        // d * d * (d + 1);
...
SkIPoint SkMaskBlurFilter::Blur(SkMask* dst, const SkMask& src) const {
    ...
    dst->fBounds.offset(-borderW, -borderH);
...
void SkMaskBlurFilter::BlurOneScan(
    ...
    for (; dst < dstEnd; dst += dstStride, src += srcStride) {
        uint32_t s = src < srcEnd ? *src : 0u;
        sum0 += s;
        sum1 += sum0;
        sum2 += sum1;
        ...
        *buffer0Cursor = s;
        ...
        buffer0Cursor = (buffer0Cursor + 1) < buffer0End ? buffer0Cursor + 1 : buffer0Begin;
        buffer1Cursor = (buffer1Cursor + 1) < buffer1End ? buffer1Cursor + 1 : buffer1Begin;
        buffer2Cursor = (buffer2Cursor + 1) < buffer2End ? buffer2Cursor + 1 : buffer2Begin;
    }
}
