...
void direct_blur_x2(
 ...
 void direct_blur_x(
    ...
    static constexpr uint16_t _____ = 0u;
     ...
     if (radius == 2) {
         direct_blur_x2(gauss, src, srcCount, dst, dstCount);
         return;
     }

    Sk8h acc0{0u}, acc1{0u};

    auto blurX1 = [&](const Sk8h& v) {
        auto v0 = v.mulHi(gauss[0]);
    ...
    auto blurX2 = [](const Sk8h& v, Sk8h& acc0, Sk8h& acc1, uint16_t* gauss) {
    ...
    auto blur = [blurX1, blurX2, blurX3, blurX4, gauss](int radius, const Sk8b& b, Sk8h& acc0, Sk8h&
    acc1) {
        ...
        acc0 = acc1;
        acc1 = 0;
        return SkNx_cast<uint8_t>(answer >> 8);
    };

    int x = 0;
    for (; x <= srcCount - 8; x += 8){
        blur(radius, Sk8b::Load(src), acc0, acc1).store(dst);
        src += 8;
        dst += 8;
    }
    ...
    if (srcTail > 0) {
        ...
        blur(radius, Sk8b::Load(buffer), acc0, acc1).store(buffer);
    ...
    if (dstTail > 0) {
        uint8_t buffer[8];
        blur(radius, Sk8b{0u}, acc0, acc1).store(buffer);
        memcpy(dst, buffer, dstTail);
    }
}
...
void direct_blur_y(
    ...
    if (radius == 2) {
        direct_blur_y2(gauss, width, src, srcStride, srcCount, dst, dstStride, dstCount);
        return;
    }
