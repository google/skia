...
void SkGlyph::installDrawable(SkArenaAlloc* alloc, sk_sp<SkDrawable> drawable) {
    ...
    if (drawable != nullptr) {
        ...
        //fDrawableData->fPath.updateBoundsCache();
        //fDrawableData->fPath.getGenerationID();
...
bool SkGlyph::setDrawable(SkArenaAlloc* alloc, SkScalerContext* scalerContext) {
    if (!this->setDrawableHasBeenCalled()) {
        sk_sp<SkDrawable> drawable = scalerContext->getDrawable(this->getPackedID());
...
const SkDrawable* SkGlyph::drawable() const {
