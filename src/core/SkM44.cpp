...
SkM44& SkM44::setRotateAboutUnitSinCos(SkV3 axis, SkScalar sinAngle, SkScalar cosAngle) {
    SkScalar x = axis.x;
    SkScalar y = axis.y;
    SkScalar z = axis.z;
    SkScalar c = cosAngle;
    ...
    return this->set44(t*x*x + c,   t*x*y - s*c, t*x*z + s*y, 0,
                       t*x*y + s*z, t*y*y + c,   t*y*z - s*x, 0,
                       t*x*z - x*y, t*y*z + s*x, t*z*z + c,   0,
                       0,           0,           0,           1);
}

SkM44& SkM44::setRotateAboutRadians(SkV3 axis, SkScalar radians) {
    SkScalar len = axis.length();
    if (len > 0 && SkScalarIsFinite(len)) {
        this->setRotateAboutUnitRadians(axis * (SK_Scalar1 / len), radians);
    }
    return *this;
}
