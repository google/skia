...
SkRect SkM44::mapRect(const SkRect& src) const {
    const bool hasPerspective = fMat[3] != 0 || fMat[7] != 0 || fMat[11] != 0 || fMat[15] != 1;
    if (hasPerspective) {
        return map_rect_perspective(src, fMat);
    } else {
        return map_rect_affine(src, fMat);
    }
}
...
static SkM44 rect_to_rect(const SkRect& src, const SkRect& dst,
                          bool preserveAspectRatio,
                          bool fitLargestDimension = false) {
    if (src.isEmpty()) {
        return SkM44();
    } else if (dst.isEmpty()) {
    ...
    if (preserveAspectRatio) {
        float s = fitLargestDimension ? std::max(sx, sy) : std::min(sx, sy);
        sx = sy = s;
    }
    ...
    if (preserveAspectRatio) {
        tx += 0.5f * (dst.width()  - sx * src.width());
        ty += 0.5f * (dst.height() - sy * src.height());
    }
...
SkM44 SkM44::RectToRect(const SkRect& src, const SkRect& dst) {
    return rect_to_rect(src, dst, /*preserveAspectRatio=*/false);
}

SkM44 SkM44::ScaleToFit(const SkRect& src, const SkRect& dst) {
    return rect_to_rect(src, dst, /*preserveAspectRatio=*/true, /*fitLargestDimension=*/false);
}

SkM44 SkM44::ScaleToFill(const SkRect& src, const SkRect& dst) {
    return rect_to_rect(src, dst, /*preserveAspectRatio=*/true, /*fitLargestDimension=*/true);
}
