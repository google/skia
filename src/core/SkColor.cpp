...
SkColor SkHSVToColor(U8CPU a, const SkScalar hsv[3]) {
    ...
    unsigned p = (SK_Scalar1 - s) * v * 256;
    unsigned q = (SK_Scalar1 - (s * f)) * v * 256;
    unsigned t = (SK_Scalar1 - (s * (SK_Scalar1 - f))) * v * 256;

    p = SkTMin<unsigned>(p, 255);
    q = SkTMin<unsigned>(q, 255);
    t = SkTMin<unsigned>(t, 255);
