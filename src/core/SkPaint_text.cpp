...
class AutoGlyphPos {
public:
    AutoGlyphPos(const SkFont& font, const uint16_t glyphs[], int count, SkPoint loc) {
        fStorage.reset(count);
        font.getPositions(glyphs, count, fStorage.get(), loc);
    }

    SkPoint* pos() const { return fStorage.get(); }

private:
    SkAutoSTArray<32, SkPoint> fStorage;
};
...
void SkPaint::getTextPath(const void* text, size_t length,
    ...
    font.getPositions(gly.glyphs(), gly.count(), fPos.get(), {x, y});
