...
void SkDraw::drawAtlas(const SkImage* atlas, const SkRSXform xform[], const SkRect textures[],
    ...
    SkStageUpdator* updator = nullptr;
    if (paint.getFilterQuality() != kMedium_SkFilterQuality && !fMatrix->hasPerspective()) {
        updator = as_SB(atlasShader.get())->appendStages2(rec);
    }
    if (!updator) {
        SkDraw draw(*this);
