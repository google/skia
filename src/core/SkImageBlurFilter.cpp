...
public:
    ...
    static GaussBlur Make(SkArenaAlloc* alloc, double sigma,
        ...
        size_t initializeLeft = sLeft-border < dLeft ? dLeft - (sLeft-border) : 0,
               noChangeCount = 0,
               initializeRight = sRight + border > dRight ? (sRight+border) - dRight : 0;
    ...
    Sk4u finalScale(const Sk4u& sum) const {
        auto p = [&](uint64_t q) {
            return static_cast<uint32_t>((q * fWeight + (1ull << 31)) >> 32);
        };
        uint64_t r = sum[0];
        uint64_t g = sum[1];
        uint64_t b = sum[2];
...
sk_sp<SkSpecialImage> SkImageBlurFilter::blur(SkSpecialImage* source,
    ...
    if (windowW > 1 && windowH > 1) {
        ...
        for (size_t x = 0; x < srcW; x++) {
            ...
            auto srcEnd   = inputBM.getAddr32(x, srcH - 1) + inputBM.rowBytesAsPixels();
            auto dstStart = dst.getAddr32(x, 0);
            auto dstEnd   = dst.getAddr32(x, dstH - 1) + dst.rowBytesAsPixels();
