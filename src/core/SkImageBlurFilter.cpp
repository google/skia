...
public:
    ...
    // border; the distance between first src pixel and the first dst pixel that uses that in its calculation.
...
GaussBlur GaussBlur::Make(Sk4u* buffer, uint32_t window,
    ...
    // NB the sums in the blur code use the following technique to avoid adding 1/2 to round the divide.
    //
    //   Sum/d + 1/2 == (Sum + h) / d
    //   Sum + d(1/2) ==  Sum + h
    //     h == (1/2)d
    ...
    return GaussBlur{buffer01Start,  buffer2Start, buffer2End, srcStart, srcEnd, dRight, weight, half};
}
...
void GaussBlur::blur(const uint32_t* src, size_t srcXStride, size_t srcYStride, size_t srcH,
    ...
    for (size_t y = 0; y < srcH; y++) {
        ...
        std::memset(buffer01Start, 0x00, (buffer2End - (Sk4u *) (buffer01Start)) * sizeof(*buffer2Start));
        ...
        // The destination pixels are not effected by the src pixels, change to zero as per the spec.
        ...
        // The leading edge is beyond the end of the source. Assume that the pixels are now 0x0000 until
...
sk_sp<SkSpecialImage> SkImageBlurFilter::blur(SkSpecialImage* source,
    ...
    if (windowW > 1 && windowH > 1) {
        ...
        // The amount 1024 is enough for buffers up to 10 sigma. The tmp bitmap will be allocated on the heap.
