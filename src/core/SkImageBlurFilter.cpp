...
public:
    ...
    static GaussBlur Make(SkArenaAlloc* alloc, double sigma,
        ...
        //static_cast<size_t>(leftInit), static_cast<size_t>(noChangeCount),
        //        static_cast<size_t>(rightInit),
...
void GaussBlur::blur(const uint32_t* src, size_t srcStride, uint32_t* dst, size_t dstStride) const {
    ...
    while (dstIdx < fDstEnd) {
        SkNx_cast<uint8_t>(this->finalScale(processValue(0u))).store(dstCursor);
        dstCursor += dstStride;
        dstIdx++;
    }
}
