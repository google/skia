...
sk_sp<SkFlattenable> SkColorFilter_Matrix::CreateProc(SkReadBuffer& buffer) {
    ...
    const auto is_rgba = buffer.isVersionLT(SkPicturePriv::kMatrixColorFilterDomain_Version)
                       ? true
                       : buffer.readBool();
...
bool SkColorFilter_Matrix::onAppendStages(const SkStageRec& rec,
    ...
    const auto willStayOpaque = shaderIsOpaque && (fFlags & kAlphaUnchanged_Flag),
