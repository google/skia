...
void SkArena::needMoreBytes(ptrdiff_t size, ptrdiff_t alignment, ptrdiff_t mask) {
    ...
    fCapacity = usableBlockSize & mask;
}
...
char* SkArenaAlloc::allocObjectWithFooter(
    ...
    if (!fCursor) {
        ...
        do {
            ...
            alignedOffset =
                    alignment - (reinterpret_cast<ptrdiff_t>(fCursor + skipFooterSize) & mask);
