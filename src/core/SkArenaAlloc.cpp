/*
 * Copyright 2016 Google Inc.
 *
 * Use of this source code is governed by a BSD-style license that can be
 * found in the LICENSE file.
 */

#include "include/private/SkTemplates.h"
#include "src/core/SkArenaAlloc.h"
#include <algorithm>
#include <new>

namespace {
uint32_t first_allocated_block(uint32_t blockSize, uint32_t firstHeapAllocation) {
    return firstHeapAllocation > 0 ? firstHeapAllocation :
           blockSize           > 0 ? blockSize           : 1024;
}

class Block {
public:
    Block() = default;
    Block(char* bytes) { memcpy(this, bytes, sizeof(Block)); }
    Block(char* previous, char* startOfBlock) : fPrevious{previous}, fStartOfBlock{startOfBlock} {}
    void store(char* storage) { memcpy(storage, this, sizeof(Block)); }
    static void DeleteBlocks(char* bytes) {
        while (bytes != nullptr) {
            Block b{bytes};
            // Notice, that a user provided initial block will have a fStartOfBlock of nullptr so
            // the delete will do nothing.
            delete [] b.fStartOfBlock;
            bytes = b.fPrevious;
        }
    }

//private:
    char*  fPrevious{nullptr};

    // If fStartOfBlock is nullptr then this is a block provided from outside, and not owned.
    char* fStartOfBlock{nullptr};
};

bool is_aligned(char* ptr, ptrdiff_t alignment) {
    return ((intptr_t)ptr & (alignment-1)) == 0;
}
bool is_aligned(ptrdiff_t v, ptrdiff_t alignment) {
    return (v & (alignment-1)) == 0;
}
}  // namespace

SkArena::SkArena(char* block, size_t blockSize, size_t firstHeapAllocation)
        : fBytes{CalculateStartingBytes(block, blockSize)}
        , fCapacity{CalculateStartingCapacity(block, fBytes)}
        , fNextHeapAlloc{first_allocated_block(blockSize, firstHeapAllocation)}
        , fYetNextHeapAlloc{fNextHeapAlloc} {
    if (fBytes == nullptr) {
        SkASSERT(fCapacity == 0);
    } else {
        Block b;
        b.store(fBytes);
    }
}

SkArena::~SkArena() {
    Block::DeleteBlocks(fBytes);
}

char* SkArena::alignedBytes(size_t sizeS, size_t alignmentS) {
    SkASSERT_RELEASE(sizeS < kMaxByteSize);
    SkASSERT_RELEASE(alignmentS < kMaxAlignment);
    ptrdiff_t size = SkTo<ptrdiff_t>(sizeS);
    ptrdiff_t alignment = SkTo<ptrdiff_t>(alignmentS);
    fCapacity = RoundDownToAlignment(fCapacity, alignment);
    if (fCapacity < SkTo<ptrdiff_t >(size)) {
        this->needMoreBytes(size, alignment);
    }
    char* bytes = fBytes - fCapacity;
    SkASSERT(fCapacity >= size);
    SkASSERT(is_aligned(fCapacity, alignment));

    fCapacity -= size;
    return bytes;
}

constexpr char* SkArena::CalculateStartingBytes(char* block, ptrdiff_t size) {
    // If the block is null or there is not enough space to place the Block object just set to null.
    // This will signal to CalculateStartingCapacity that size should be zero.
    if (block == nullptr || size < 2 * kMaxAlignment) { return nullptr; }
    return RoundDownToAlignment(block + size - sizeof(Block), kMaxAlignment);
}

constexpr ptrdiff_t SkArena::CalculateStartingCapacity(char* block, char* bytes) {
    if (bytes == nullptr) { return 0; }
    return bytes - block;
}

char* SkArena::RoundDownToAlignment(char* ptr, ptrdiff_t alignment) {
    return (char*)RoundDownToAlignment((intptr_t)ptr, alignment);
}

char* SkArena::RoundUpToAlignment(char* ptr, ptrdiff_t alignment) {
    return (char*)RoundUpToAlignment((intptr_t)ptr, alignment);
}

void SkArena::needMoreBytes(ptrdiff_t requestedSize, ptrdiff_t alignment) {
    ptrdiff_t nextBlockSize = fNextHeapAlloc;
    // Calculate the next heap alloc that won't overflow.
    if (fYetNextHeapAlloc <= kMaxByteSize - fNextHeapAlloc) {
        fNextHeapAlloc += fYetNextHeapAlloc;
        std::swap(fNextHeapAlloc, fYetNextHeapAlloc);
    } else {
        fNextHeapAlloc = kMaxByteSize;
    }

    ptrdiff_t blockSize = CalculateGoodSizeWithOverhead(std::max(requestedSize, nextBlockSize));
    char* bytes = new char[blockSize];

    // Figure out the placement of Block that is aligned to kMaxAlignment so that fBytes is aligned.
    char* blockLocation = RoundDownToAlignment(bytes + blockSize - sizeof(Block), kMaxAlignment);
    SkASSERT(is_aligned(blockLocation, kMaxAlignment));

    // Store the position to the current block (fBytes), and the start of the block (bytes).
    Block b{fBytes, bytes};
    b.store(blockLocation);

    // Reset allocation information.
    fBytes = blockLocation;
    fCapacity = fBytes - bytes;
    fCapacity = RoundDownToAlignment(fCapacity, alignment);
    SkASSERT(is_aligned(fCapacity, alignment));
    SkASSERT(fCapacity >= requestedSize);
}

static char* end_chain(char*) { return nullptr; }

SkArenaAlloc::SkArenaAlloc(char* block, size_t size, size_t firstHeapAllocation)
    : fDtorCursor {block}
    , fCursor     {block}
    , fEnd        {block + ToU32(size)}
    , fNextHeapAlloc{first_allocated_block(ToU32(size), ToU32(firstHeapAllocation))}
    , fYetNextHeapAlloc{fNextHeapAlloc}
{
    if (size < sizeof(Footer)) {
        fEnd = fCursor = fDtorCursor = nullptr;
    }

    if (fCursor != nullptr) {
        this->installFooter(end_chain, 0);
    }
}

SkArenaAlloc::~SkArenaAlloc() {
    RunDtorsOnBlock(fDtorCursor);
}

void SkArenaAlloc::installFooter(FooterAction* action, uint32_t padding) {
    assert(SkTFitsIn<uint8_t>(padding));
    this->installRaw(action);
    this->installRaw((uint8_t)padding);
    fDtorCursor = fCursor;
}

char* SkArenaAlloc::SkipPod(char* footerEnd) {
    char* objEnd = footerEnd - (sizeof(Footer) + sizeof(int32_t));
    int32_t skip;
    memmove(&skip, objEnd, sizeof(int32_t));
    return objEnd - skip;
}

void SkArenaAlloc::RunDtorsOnBlock(char* footerEnd) {
    while (footerEnd != nullptr) {
        FooterAction* action;
        uint8_t       padding;

        memcpy(&action,  footerEnd - sizeof( Footer), sizeof( action));
        memcpy(&padding, footerEnd - sizeof(padding), sizeof(padding));

        footerEnd = action(footerEnd) - (ptrdiff_t)padding;
    }
}

char* SkArenaAlloc::NextBlock(char* footerEnd) {
    char* objEnd = footerEnd - (sizeof(char*) + sizeof(Footer));
    char* next;
    memmove(&next, objEnd, sizeof(char*));
    RunDtorsOnBlock(next);
    delete [] objEnd;
    return nullptr;
}

void SkArenaAlloc::ensureSpace(uint32_t size, uint32_t alignment) {
    constexpr uint32_t headerSize = sizeof(Footer) + sizeof(ptrdiff_t);
    constexpr uint32_t maxSize = std::numeric_limits<uint32_t>::max();
    constexpr uint32_t overhead = headerSize + sizeof(Footer);
    AssertRelease(size <= maxSize - overhead);
    uint32_t objSizeAndOverhead = size + overhead;

    const uint32_t alignmentOverhead = alignment - 1;
    AssertRelease(objSizeAndOverhead <= maxSize - alignmentOverhead);
    objSizeAndOverhead += alignmentOverhead;

    uint32_t minAllocationSize = fNextHeapAlloc;

    // Calculate the next heap alloc that won't overflow.
    if (fYetNextHeapAlloc <= maxSize - fNextHeapAlloc) {
        fNextHeapAlloc += fYetNextHeapAlloc;
        std::swap(fNextHeapAlloc, fYetNextHeapAlloc);
    } else {
        fNextHeapAlloc = maxSize;
    }
    uint32_t allocationSize = std::max(objSizeAndOverhead, minAllocationSize);

    // Round up to a nice size. If > 32K align to 4K boundary else up to max_align_t. The > 32K
    // heuristic is from the JEMalloc behavior.
    {
        uint32_t mask = allocationSize > (1 << 15) ? (1 << 12) - 1 : 16 - 1;
        AssertRelease(allocationSize <= maxSize - mask);
        allocationSize = (allocationSize + mask) & ~mask;
    }

    char* newBlock = new char[allocationSize];

    auto previousDtor = fDtorCursor;
    fCursor = newBlock;
    fDtorCursor = newBlock;
    fEnd = fCursor + allocationSize;
    this->installRaw(previousDtor);
    this->installFooter(NextBlock, 0);
}

char* SkArenaAlloc::allocObjectWithFooter(
        ptrdiff_t sizeIncludingFooter, ptrdiff_t alignment, ptrdiff_t mask) {
    // The size of the footer if needed to skip over pod data.
    ptrdiff_t skipFooterSize = 0;
    // The alignment offset from the cursor to the object start accounting for the skip footer.
    ptrdiff_t alignedOffset;

    if (!fCursor) {
        this->ensureSpace(sizeIncludingFooter + alignment - 1, alignment);
        // For a new block of memory no skip footer is needed.
        SkASSERT(fCursor == fDtorCursor);
        skipFooterSize = 0;
        alignedOffset = -reinterpret_cast<ptrdiff_t>(fCursor) & mask;
    } else {
        do {
            skipFooterSize = fCursor != fDtorCursor ? sizeof(Footer) + sizeof(uint32_t) : 0;
            // The alignment needs to account for the skip footer we are about to add.
            alignedOffset = -reinterpret_cast<ptrdiff_t>(fCursor + skipFooterSize) & mask;
        } while (sizeIncludingFooter > fEnd - fCursor - skipFooterSize - alignedOffset &&
                 ((void)this->ensureSpace(sizeIncludingFooter + alignment - 1, alignment), true));
    }

    // Install a skip footer if needed, thus terminating a run of POD data. The calling code is
    // responsible for installing the footer after the object.
    if (fCursor != fDtorCursor) {
        this->installRaw(ToU32(fCursor - fDtorCursor));
        this->installFooter(SkipPod, 0);
    }

    return fCursor + alignedOffset;
}

static uint32_t to_uint32_t(size_t v) {
    assert(SkTFitsIn<uint32_t>(v));
    return (uint32_t)v;
}

SkArenaAllocWithReset::SkArenaAllocWithReset(char* block,
                                             size_t size,
                                             size_t firstHeapAllocation)
        : SkArenaAlloc(block, size, firstHeapAllocation)
        , fFirstBlock{block}
        , fFirstSize{to_uint32_t(size)}
        , fFirstHeapAllocationSize{to_uint32_t(firstHeapAllocation)} {}

void SkArenaAllocWithReset::reset() {
    this->~SkArenaAllocWithReset();
    new (this) SkArenaAllocWithReset{fFirstBlock, fFirstSize, fFirstHeapAllocationSize};
}
