...
class SkEnumerate {
    ...
    template <typename P = Captured, std::enable_if_t<is_tuple<P>::value, bool> = true>
    static constexpr auto MakeResult(size_t i, P&& v) {
        return std::tuple_cat(std::tuple<size_t>{i}, std::forward<P>(v));
    }
    template <typename P = Captured, std::enable_if_t<!is_tuple<P>::value, bool> = true>
    static constexpr auto MakeResult(size_t i, P& v) {
