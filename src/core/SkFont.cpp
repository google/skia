...
void VisitGlyphs(const SkFont& origFont, const SkPaint* paint, const SkMatrix* ctm,
                 const uint16_t glyphs[], int count, HANDLER handler) {
    if (count <= 0) {
        return;
    }
    ...
    auto cache = SkStrikeCache::FindOrCreateStrikeExclusive(font, paint, ctm);
...
void SkFont::getWidthsBounds(const uint16_t glyphs[], int count, SkScalar widths[], SkRect bounds[],
    ...
    VisitGlyphs(*this, paint, ctm, glyphs, count, [widths, bounds]
...
void SkFont::getPos(const uint16_t glyphs[], int count, SkPoint pos[], SkPoint origin) const {
    VisitGlyphs(*this, nullptr, nullptr, glyphs, count, [pos, origin]
                      (SkGlyphCache* cache, const uint16_t glyphs[], int count, SkScalar scale) {
        SkPoint loc = origin;
        for (int i = 0; i < count; ++i) {
            pos[i] = loc;
...
void SkFont::getXPos(const uint16_t glyphs[], int count, SkScalar xpos[], SkScalar origin) const {
    VisitGlyphs(*this, nullptr, nullptr, glyphs, count, [xpos, origin]
                      (SkGlyphCache* cache, const uint16_t glyphs[], int count, SkScalar scale) {
        SkScalar x = origin;
        for (int i = 0; i < count; ++i) {
            xpos[i] = x;
