...
void SkGlyphRunListPainter::drawForBitmapDevice(
    ...
    const SkPaint& lpaint = glyphRunList.paint();
    // The bitmap blitters can only draw lcd text to a N32 bitmap in srcOver. Otherwise,
    // convert the lcd text into A8 text. The props communicates this to the scaler.
    auto& props = (kN32_SkColorType == fColorType && lpaint.isSrcOver())
                  ? fDeviceProps
    ...
    for (auto& glyphRun : glyphRunList) {
        ...
        if (ShouldDrawAsPath(lpaint, runFont, deviceMatrix)) {
            ...
            bitmapDevice->paintMasks(SkSpan<const SkMask>{masks.begin(), masks.size()}, lpaint);
        }
    }
}
