...
void SkGlyphRunListPainter::drawGlyphRunAsSDFWithARGBFallback(
    ...
    SkAutoDescriptor ad;
    SkScalerContextEffects effects;
    SkScalerContext::CreateDescriptorAndEffectsUsingPaint(dfFont, dfPaint, fDeviceProps, flags,
    ...
    {
        if (glyphCount > 0) {
            processMasks(SkSpan<const GlyphAndPos>{fGlyphPos, SkTo<size_t>(glyphCount)}, strike
            .get(), textScale, minScale, maxScale);
        }
