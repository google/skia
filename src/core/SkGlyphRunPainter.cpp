...
void GrTextContext::regenerateGlyphRunList(GrTextBlob* cacheBlob,
    ...
    class ARGBFallbackHelper {
    public:
        ARGBFallbackHelper(GrTextBlob* cacheBlob, int runIndex, const SkSurfaceProps& props,
                SkScalerContextFlags scalerContextFlags, GrGlyphCache* glyphCache,
                GrColor filteredColor)
            : fBlob{cacheBlob}
            , fRunIndex{runIndex}
            , fProps{props}
            , fScalerContextFlags{scalerContextFlags}
