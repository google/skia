...
void SkGlyphRunListPainter::drawGlyphRunAsPathWithARGBFallback(
    ...
    if (process) {
        if (glyphCount > 0) {
            process->processPathsSource(SkSpan<const GlyphAndPos>{fGlyphPos, SkTo<size_t>
                                                (glyphCount)},
                                        strike.get(),
                                        strikeToSourceRatio);
        }
...
void SkGlyphRunListPainter::drawGlyphRunAsBMPWithPathFallback(
    ...
    if (process) {
        if (glyphCount > 0) {
            ...
            process->processMasksDevice(
                    SkSpan<const GlyphAndPos>{fGlyphPos, SkTo<size_t>(glyphCount)}, strike.get());
        }
        if (!fPaths.empty()) {
            process->processPathsDevice(SkSpan<const GlyphAndPos>{fPaths});
        }
    }
}
