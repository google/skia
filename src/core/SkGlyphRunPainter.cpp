...
void GrTextBlob::Run::appendFallbackGlyph(const sk_sp<GrTextStrike>& strike,
    ...
    if (GrGlyph* glyph = strike->getGlyph(skGlyph)) {
        ...
        if (!glyphRect.isEmpty()) {
            ...
            subRun->setNeedsTransform(transformType);
    ...
    struct ARGBFallbackHelper {
        void operator()(const SkPaint& fallbackPaint, const SkFont& fallbackFont,
            ...
            for (auto glyphID : glyphIDs) {
                ...
                fRun->appendFallbackGlyph(strike, glyph, *glyphPos, textScale, needsTransform);
                glyphPos++;
            }
        }
