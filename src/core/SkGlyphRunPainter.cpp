...
void SkGlyphRunListPainter::processGlyphRunList(const SkGlyphRunList& glyphRunList,
    ...
    for (const auto& glyphRun : glyphRunList) {
        ...
        if (!useSDFT && !usePaths) {
            ...
            SkGlyphinator glyphPtrs = strike->prepareForMaskDrawing(packedGlyphIDs, rejectIndices);
            ...
            SkGlyphinator glyphPtrs = strike->prepareForSDFTDrawing(packedGlyphIDs, rejectIndices);
        ...
        } else if (!rejectIndices.empty()) {
            size_t cursor = 0;
            for (auto i : rejectIndices) {
        ...
        if (!rejectGlyphs.empty()) {
            ...
            SkGlyphinator paths = strike->prepareForPathDrawing(rejectGlyphs, rejectIndices);
