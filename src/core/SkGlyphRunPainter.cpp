...
void SkGlyphRunListPainter::processGlyphRunList(const SkGlyphRunList& glyphRunList,
    ...
    for (const auto& glyphRun : glyphRunList) {
        ...
        if (!useSDFT && !usePaths) {
            ...
            colorAllowed = true;
            std::tie(processFallback, inator) =
                    strike->glyphsBelowSize(inator, maxDimension, colorAllowed);
        ...
        if (processFallback || usePaths) {
            ScopedBuffers _ = this->ensureBuffers(glyphRun);
            if (usePaths) {
                // Process as paths - this should be the .001% case.
                maxDimension = 0;
                colorAllowed = false;
            }
            ...
            SkScalar maxFallbackDimension = 0;
            SkGlyphinator paths;
            std::tie(maxFallbackDimension, paths) = strike->glyphsForFallback(
                    packedGlyphIDs, maxDimension, colorAllowed, fARGBGlyphsIDs, fARGBPositions);
