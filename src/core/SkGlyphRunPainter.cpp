...
void SkGlyphRunListPainter::drawForBitmapDevice(
    ...
    for (auto& glyphRun : glyphRunList) {
        ...
        if (SkStrikeSpec::ShouldDrawAsPath(runPaint, runFont, deviceMatrix)) {
            ...
            drawables.startSource(glyphRun.source(), origin);
            strike->preparePaths(&drawables);
            ...
            bitmapDevice->paintPaths(&drawables, strikeSpec.strikeToSourceRatio(), pathPaint);
        } else {
            ...
            drawables.startDevice(glyphRun.source(), origin, deviceMatrix, *strike);
            strike->prepareImages(&drawables);
            bitmapDevice->paintMasks(&drawables, runPaint);
        }
    }
}
        ...
        if (!rejects.source().empty()) {
            ...
            if (!rejects.source().empty() && rejectedMaxDimension != 0) {
                ...
                if (useDeviceCache) {
                    ...
                    if (process) {
                        process->processDeviceFallback(drawbles.drawable(), strikeSpec);
                    }
                } else {
