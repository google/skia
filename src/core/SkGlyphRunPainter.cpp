    ...
    if (cacheBlob) {
        if (cacheBlob->mustRegenerate(listPaint, glyphRunList.anyRunsSubpixelPositioned(),
            ...
            cacheBlob->generateFromGlyphRunList(
                    grStrikeCache, *context->priv().caps()->shaderCaps(), fOptions,
        ...
        cacheBlob->generateFromGlyphRunList(
                grStrikeCache, *context->priv().caps()->shaderCaps(), fOptions, listPaint,
    ...
    for (const auto& glyphRun : glyphRunList) {
        ...
        if (GrTextContext::CanDrawAsDistanceFields(runPaint, runFont, viewMatrix, props,
            ...
            auto processMasks =
                    [run, blob{this}]
                        ...
                        sk_sp<GrTextStrike> currStrike =
                                blob->findOrMakeStrike(strike->getDescriptor());
            ...
            auto argbFallbackDevice = [blob{this}, run]
                ...
                blob->setHasBitmap();
                sk_sp<GrTextStrike> grStrike = blob->findOrMakeStrike(strike->getDescriptor());
...
sk_sp<GrTextStrike> GrTextBlob::findOrMakeStrike(const SkDescriptor& desc) {
    return fStrikeCache->getStrike(desc);
}
...
std::unique_ptr<GrDrawOp> GrTextContext::createOp_TestingOnly(GrRecordingContext* context,
    ...
    if (!glyphRunList.empty()) {
        ...
        blob->generateFromGlyphRunList(
                strikeCache, *context->priv().caps()->shaderCaps(), textContext->fOptions,
