...
public:
    ...
    bool canDrawDistanceFields(const SkPaint& skPaint, const SkMatrix& viewMatrix,
        ...
        if (!viewMatrix.hasPerspective()) {
            SkScalar scaledTextSize = scaledTextSize();
...
void SkGlyphRunListPainter::drawForGPUDevice(
    ...
    for (const auto& glyphRun : glyphRunList) {
        ...
        if (dftControl.canDrawDistanceFields(runPaint, deviceMatrix, fDeviceProps)) {
            auto cache = SkStrikeCache::FindOrCreateStrikeExclusive(dftPaint);
            ...
            for (auto glyphID : glyphRun.glyphsIDs()) {
                ...
                if (SkScalarsAreFinite(position.x(), position.y())) {
                    ...
                    if (glyph.isEmpty()) {
                        ...
                        if (!SkGlyphCacheCommon::GlyphTooBigForAtlas(glyph)) {
                            this->addBase(glyph, position);
                        } else {
                            this->addPath(glyph, position);
                        }
                    } else {
                        this->addARGB(glyph, position);
                    }
                }
            }
            ...
            if (!fPathIDs.empty()) {
                this->paintPaths(textScale, pathPaint);
            }

            if (!fBaseIDs.empty()) {
                this->paintDFT(textScale, dftPaint);
            }

            if (!fARGBIDs.empty()) {
                this->paintARGB(textScale, argbPaint);
            }
            ...
            for (auto glyphID : glyphRun.glyphsIDs()) {
                ...
                if (SkScalarsAreFinite(devicePos.x(), devicePos.y())) {
                    ...
                    if (glyph.isEmpty()) {
                        ...
                        this->addBase(glyph, devicePos);
                    } else {
                        if (glyph.fMaskFormat != SkMask::kARGB32_Format) {
                            this->addPath(glyph, sourcePos);
                        } else {
                            this->addARGB(glyph, sourcePos);
                        }
                    }
                }
            }
