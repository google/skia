...
void SkGlyphRunListPainter::processGlyphRunList(const SkGlyphRunList& glyphRunList,
    ...
    SkDrawableGlyphBuffer drawbles;

    for (const auto& glyphRun : glyphRunList) {
        rejects.reset(SkMakeZip(glyphRun.glyphsIDs(), glyphRun.positions()));
        const SkFont& runFont = glyphRun.font();
        ...
        if (!useSDFT && !usePaths) {
            ...
            drawbles.startDevice(rejects.source(), origin, viewMatrix, *strike);
            strike->prepareForMaskDrawing(&drawbles, &rejects);

            if (process) {
                // processDeviceMasks must be called even if there are no glyphs to make sure runs
                // are set correctly.
                process->processDeviceMasks(drawbles.drawable(), strikeSpec);
            }
        } else if(useSDFT) {
            ...
            drawbles.startSource(rejects.source(), origin);

            strike->prepareForSDFTDrawing(&drawbles, &rejects);
            ...
            if (process) {
                ...
                process->processSourceSDFT(
                        drawbles.drawable(), strikeSpec, runFont, minScale, maxScale, hasWCoord);
            }
        }
        ...
        if (!rejects.source().empty()) {
            ...
            strike->prepareForPathDrawing(&drawbles, &rejects);

            SkScalar rejectedMaxDimension =
                    pathStrikeSpec.strikeToSourceRatio() * rejects.rejectedMaxDimension();
            if (process) {
                process->processSourcePaths(drawbles.drawable(), pathStrikeSpec);
            }

            rejects.flipRejectsToSource();

            if (!rejects.source().empty() && rejectedMaxDimension != 0) {
...
void GrTextBlob::processDeviceMasks(const SkZip<SkBag, SkPoint>& drawables,
...
void GrTextBlob::processSourcePaths(const SkZip<SkBag, SkPoint>& drawables,
                                    const SkStrikeSpec& strikeSpec) {
    Run* run = this->currentRun();
    this->setHasBitmap();
...
void GrTextBlob::processSourceSDFT(const SkZip<SkBag, SkPoint>& drawables,
                                   const SkStrikeSpec& strikeSpec,
...
void GrTextBlob::processSourceFallback(const SkZip<SkBag, SkPoint>& drawables,
                                       const SkStrikeSpec& strikeSpec,
                                       bool hasW) {
    Run* run = this->currentRun();
...
void GrTextBlob::processDeviceFallback(const SkZip<SkBag, SkPoint>& drawables,
                                       const SkStrikeSpec& strikeSpec) {
    Run* run = this->currentRun();
    this->setHasBitmap();
    sk_sp<GrTextStrike> grStrike = strikeSpec.findOrCreateGrStrike(fStrikeCache);
