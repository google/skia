...
void GrTextBlob::paintMasks(SkSpan<const SkGlyphRunListPainter::GlyphAndPos> glyphAndPos,
                               const SkPaint& paint) {
    this->setHasBitmap();

    Run* run = this->pushBackRun();

    auto grStrikeCache = context->contextPriv().getGlyphCache();
    sk_sp<GrTextStrike> currStrike = grStrikeCache->getStrike(cache.get());
    ...
    for (auto gp : glyphAndPos) {
        ...
        SkPoint position = gp.position;
            const void* glyphImage = cache->findImage(skGlyph);
            if (glyphImage != nullptr) {
                SkPoint pt{SkScalarFloorToScalar(position.fX),
                           SkScalarFloorToScalar(position.fY)};
                run->appendGlyph(this, currStrike,
