...
void GrTextBlob::generateFromGlyphRunList(GrStrikeCache* glyphCache,
    ...
    for (const auto& glyphRun : glyphRunList) {
        ...
        if (GrTextContext::CanDrawAsDistanceFields(runPaint, runFont, viewMatrix, props,
            ...
            GrTextContext::InitDistanceFieldFont(runFont.getSize(),
                                                 viewMatrix,
                                                 options,
                                                 &distanceFieldFont,
                                                 &textScale);
            GrTextContext::InitDistanceFieldPaint(&distanceFieldPaint);
            GrTextContext::InitDistanceFieldFlags(&flags);
            SkScalar minScale, maxScale;
            std::tie(minScale, maxScale) = GrTextContext::InitDistanceFieldBlob(
...

            {
                ...
                glyphPainter->drawGlyphRunAsSDFWithARGBFallback(
                    ...
                    std::move(perSDF), std::move(perPath),
                    std::move(argbFallback));
            }
...
bool SkTextBlobCacheDiffCanvas::TrackLayerDevice::maybeProcessGlyphRunForDFT(
    ...
    GrTextContext::InitDistanceFieldFont(runFont.getSize(),
                                         ...
                                         &dfFont,
                                         &textRatio);
    GrTextContext::InitDistanceFieldPaint(&dfPaint);
    GrTextContext::InitDistanceFieldFlags(&flags);
    SkScalerContextEffects effects;
