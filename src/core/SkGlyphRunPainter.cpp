...
void GrTextBlob::generateFromGlyphRunList(GrGlyphCache* glyphCache,
    ...
    struct ARGBFallbackHelper {
        void operator()(const SkPaint& fallbackPaint, const SkFont& fallbackFont,
            ...
            {
                BothCaches caches = fRun->setupCache(fallbackPaint,
                                                     fallbackFont,
                                                     fProps,
                                                     fScalerContextFlags,
                                                     glyphCacheMatrix,
                                                     fGlyphCache);
                fallbackCache = std::move(caches.skCache);
                strike = std::move(caches.grCache);
            }
    ...
    for (const auto& glyphRun : glyphRunList) {
        ...
        if (GrTextContext::CanDrawAsDistanceFields(runPaint, runFont, viewMatrix, props,
...

            {
                ...
                {
                    auto both = run->setupCache(distanceFieldPaint,
                            distanceFieldFont,
                            props,
                            flags,
                            SkMatrix::I(),
            ...
            sk_sp<GrTextStrike> currStrike;
            {
                auto both = run->setupCache(runPaint,
                                             runFont,
                                             props,
                                             scalerContextFlags,
                                             viewMatrix,
