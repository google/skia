...
void SkGlyphRunListPainter::processARGBFallback(SkScalar maxSourceGlyphDimension,
    ...
    if (useDeviceCache) {
        ...
        auto glyphPosSpan = strike->glyphMetrics2(fARGBGlyphsIDs.data(),
                                                  fARGBPositions.data(),
                                                  fARGBGlyphsIDs.size(),
    ...
    for (const auto& glyphRun : glyphRunList) {
        ...
        if (useSDFT) {
            ...
            for (const SkGlyphPos& glyphPos : glyphPosSpan) {
                ...
                if (glyph.isEmpty()) {
                    // do nothing
                           ...
                           && SkStrikeCommon::kSkSideTooBigForAtlas <= glyph.maxDimension()) {
                    ...
                    addFallback(glyph.getGlyphID(), glyph.fWidth, glyph.fHeight, position);
                }
            }
            ...
            for (const SkGlyphPos& glyphPos : glyphPosSpan) {
                ...
                if (SkStrikeCommon::kSkSideTooBigForAtlas <= glyph.maxDimension()) {
                    fGlyphPos[glyphsWithMaskCount++] = glyphPos;
                } else if (glyph.fMaskFormat != SkMask::kARGB32_Format && glyph.hasPath()) {
                    fPaths.push_back(glyphPos);
                } else {
