...
public:
    ...
    */
    ...
    const SkPath* setPath(SkGlyph*) override;

    // If glyph does not have an existing path, then add a path to glyph.
    const SkPath* setPath(SkGlyph* glyph, const SkPath* path);
