...
sk_sp<SkFlattenable> SkMixer_Split::CreateProc(SkReadBuffer& buffer) {
    ...
    return combine ? combine->makeSplit(std::move(m0), std::move(m1)) : nullptr;
}
...
sk_sp<SkMixer> SkMixer::MakeConst(SkColor c) {
    return sk_sp<SkMixer>(new SkMixer_Const(SkColor4f::FromColor(c).premul()));
}
...
sk_sp<SkMixer> SkMixer::makeSplit(sk_sp<SkMixer> m0, sk_sp<SkMixer> m1) const {
    auto self = sk_ref_sp(this);
    if (!m0 && !m1) {
        return self;
    }
    return sk_sp<SkMixer>(new SkMixer_Split(std::move(m0), std::move(m1), self));
}
