...
public:
    ...
    bool appendStages(SkRasterPipeline* pipeline, SkColorSpace*, SkArenaAlloc*) const override {
        pipeline->append(SkRasterPipeline::lerp_1_float, &fWeight);
        return true;
    }
...
public:
    ...
    bool appendStages(SkRasterPipeline* pipeline, SkColorSpace* dstCS,
        ...
        pipeline->append(SkRasterPipeline::store_rgba, storage->fSrcA);
        pipeline->append(SkRasterPipeline::store_dst_rgba, storage->fSrcB);
        ...
        pipeline->append(SkRasterPipeline::store_rgba, storage->fRes1);

        // Now restore the original colors to call the first mixer
        pipeline->append(SkRasterPipeline::load_rgba, storage->fSrcA);
        pipeline->append(SkRasterPipeline::load_dst_rgba, storage->fSrcB);
        ...
        pipeline->append(SkRasterPipeline::load_dst_rgba, storage->fRes1);
