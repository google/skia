...
//    packed-id < kMaxGlyphID * possible-x-sub-pixel-poistions
// where possible-x-sub-pixel-poistions == 4.
class LowerRangeBitVector {
public:
    constexpr bool test(SkPackedGlyphID packedID) const {
        uint32_t bit = packedID.value();
        return bit < kMaxIndex && fBits.test(bit);
    }
    constexpr void setIfLower(SkPackedGlyphID packedID) {
        uint32_t bit = packedID.value();
        if (bit < kMaxIndex) {
            fBits.set(bit);
        }
...
private:
    ...
    static_assert(GID::kSubPixelX < GID::kGlyphID && GID::kGlyphID < GID::kSubPixelY,
            "SkPackedGlyphID must be organzied: sub-y | glyph id | sub-x");
