...
void SkStrikeServer::SkGlyphCacheState::generatePath(const SkGlyph& glyph) {
    ...
    if (glyph.fPathData == nullptr) {
        const_cast<SkGlyph&>(glyph).addPath(fContext.get(), &fAlloc);
...
SkSpan<const SkGlyphPos> SkStrikeServer::SkGlyphCacheState::prepareForDrawing(
    ...
    // the glyphs returned from this method.
    ...
    for (size_t i = 0; i < n; i++) {
        ...
        if (glyphPtr->maxDimension() <= maxDimension) {
            ...
            // We need to query the path before checking canDrawAsPath since the function returns
            // true only if a path is available.
            if (glyphPtr->fPathData == nullptr) {
                glyphPtr->addPath(fContext.get(), &fAlloc);
                fCachedGlyphPaths.add(glyphPtr->getPackedID());
                fPendingGlyphPaths.push_back(glyphPtr->getPackedID());
            }

            // There is a super rare case where a font doesn't have a path for a glyph, but is
            // bitmap only. In that case use fallback to draw the bitmap glyph.
            needsFallback = !glyphPtr->canDrawAsPath();
        }

        SkASSERT(!needsFallback || glyphPtr->fMaskFormat == SkMask::kARGB32_Format);

        // Make sure to send the glyph to the GPU because we always send the image for a glyph.
        // TODO(herb): We don't need to send the image in the path/fallback cases anymore.
        if (cacheMiss) {
            fCachedGlyphImages.add(packedGlyphID);
            fPendingGlyphImages.push_back(packedGlyphID);
        }
