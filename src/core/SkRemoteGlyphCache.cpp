...
public:
    ...
    T* allocateArray(int count) {
        static_assert(alignof(T) >= 4, "Buffer must be 4 byte aligned");
...
bool read_path(Deserializer* deserializer, SkGlyph* glyph, SkGlyphCache* cache) {
    ...
    ArraySlice<uint32_t> path;
    if (pathSize != 0u) {
        path = deserializer->readArray<uint32_t>(pathSize);
        if (!path.data()) return false;
    }
    return cache->initializePath(glyph, path.data(), path.size());
}
...
void SkStrikeServer::SkGlyphCacheState::addGlyph(SkTypeface* typeface,
                                                 ...
                                                 bool pathOnly) {
    auto* cache = pathOnly ? &fCachedGlyphPaths : &fCachedGlyphImages;
    auto* pending = pathOnly ? &fPendingGlyphPaths : &fPendingGlyphImages;
...
void SkStrikeServer::SkGlyphCacheState::writePendingGlyphs(Serializer* serializer) {
    ...
    for (const auto& glyphID : fPendingGlyphImages) {
        ...
        stationaryGlyph.fImage = serializer->allocateArray<uint32_t>(imageSize);
...
void SkStrikeServer::SkGlyphCacheState::writeGlyphPath(const SkPackedGlyphID& glyphID,
    ...
    path.writeToMemory(serializer->allocateArray<uint32_t>(pathSize));
}
    ...
    for (size_t i = 0; i < strikeCount; ++i) {
        ...
        for (size_t j = 0; j < glyphImagesCount; j++) {
            ...
            *allocatedGlyph = glyph;
            ...
            auto imageSize = glyph.computeImageSize();
            ArraySlice<uint32_t> image;
            if (imageSize != 0u) {
                image = deserializer.readArray<uint32_t>(imageSize);
                if (!image.data()) READ_FAILURE
            }
            if (!strike->initializeImage(image.data(), image.size(), allocatedGlyph)) READ_FAILURE
        }
