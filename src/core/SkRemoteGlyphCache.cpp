...
#include "SkDelayedConstructor.h"
#include "SkDevice.h"
#include "SkDraw.h"
...
static bool readGlyph(SkDelayedConstructor<SkGlyph>& glyph, Deserializer* deserializer) {
    SkPackedGlyphID glyphID;
    if (!deserializer->read<SkPackedGlyphID>(&glyphID)) return false;
    glyph.initialize(glyphID);
...
bool SkStrikeClient::readStrikeData(const volatile void* memory, size_t memorySize) {
    ...
    for (size_t i = 0; i < strikeCount; ++i) {
        ...
        for (size_t j = 0; j < glyphImagesCount; j++) {
            SkDelayedConstructor<SkGlyph> glyph;
