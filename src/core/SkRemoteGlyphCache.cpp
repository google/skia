...
SkStrikeCacheDifferenceSpec::StrikeDifferences::StrikeDifferences(
        SkFontID typefaceID, const SkDescriptor* desc)
...
SkStrikeCacheDifferenceSpec::findStrikeDifferences(const SkDescriptor& desc,
    ...
    if (mapIter == fDescriptorToDifferencesMap.end()) {
        // This evil is to work around a bug in the windows toolchain.
        // This should be a unique_ptr passed to StrikeDifferences.
        const SkDescriptor* newDesc = desc.copy().release();

        mapIter = fDescriptorToDifferencesMap.emplace_hint(
            mapIter, std::make_pair(newDesc, StrikeDifferences{typefaceID, newDesc}));
    }
