...
SkGlyphinator SkStrikeServer::RemoteStrike::prepareForMaskDrawing(
    ...
    for (size_t i = 0; i < glyphPos.n; i++) {
        ...
        if (summary == nullptr) {
            ...
            MaskSummary newSummary = {packedID.value(), FitsInAtlas(*glyph), glyph->isColor()};
...
SkZip<SkGlyph*, SkPoint> SkStrikeServer::RemoteStrike::prepareForMaskDrawing2(
        SkPainterPassStorage::PassForGlyphs* pass) {
    for (auto t : *pass) {
        SkPackedGlyphID packedID;
        std::tie(packedID, std::ignore) = t;
        MaskSummary* summary = fSentGlyphs.find(packedID);
        if (summary == nullptr) {
            // Put the new SkGlyph in the glyphs to send.
            fGlyphMasksToSend.emplace_back(packedID);
...
SkZip<SkGlyph*, SkPoint> SkStrikeServer::RemoteStrike::prepareForSDFTDrawing2(
        SkPainterPassStorage::PassForGlyphs* pass) {
    for (auto t : *pass) {
        SkPackedGlyphID packedID;
        std::tie(packedID, std::ignore) = t;
