...
bool SkStrikeClient::readStrikeData(const volatile void* memory, size_t memorySize) {
    ...
    for (size_t i = 0; i < strikeCount; ++i) {
        ...
        bool metricsHaveBeenSent;
        if (!deserializer.read(&metricsHaveBeenSent)) READ_FAILURE

        SkFontMetrics fontMetrics{};
        if (metricsHaveBeenSent) {
        ...
        // Metrics are only sent the first time. If the metrics are not sent, there must be an
        // existing strike.
        SkASSERT(metricsHaveBeenSent || strike);
        if (strike == nullptr) {
