        ...
        const SkPaint& runPaint)
{
    ...
    SkScalerContext::MakeRecAndEffects(runPaint, &fSurfaceProps, &runMatrix,
                                       ...
                                       &effects, applyTypefaceFiltering);
    ...
    auto* glyphCacheState =
            static_cast<SkStrikeServer*>(fStrikeServer)->getOrCreateCache(std::move(desc));
...
size_t DescHash::operator()(const SkDescriptor* key) const { return key->getChecksum(); }

bool DescEq::operator()(const SkDescriptor* lhs, const SkDescriptor* rhs) const {
...
SkStrikeClient::SkStrikeClient(DiscardableHandleManager* discardableManager)
...
bool SkStrikeClient::readStrikeData(const void* memory, size_t memorySize) {
    ...
    for (size_t i = 0; i < *strikeCount; ++i) {
        ...
        if (!spec || !desc || !fontMetrics) READ_FAILURE
...
bool SkStrikeClient::deleteHandle(SkDiscardableHandleId discardableHandleId) {
...
sk_sp<SkTypeface> SkStrikeClient::deserializeTypeface(const void* buf, size_t len) {
    ...
    return typeface ? *typeface : nullptr;
}
...
SkStrikeClient::DiscardableHandle::DiscardableHandle()
        : fStrikeClient(nullptr), fDiscardableHandleId(-1) {}

SkStrikeClient::DiscardableHandle::DiscardableHandle(SkStrikeClient* strikeClient,
                                                     SkDiscardableHandleId discardableHandleId)
...
bool SkStrikeClient::DiscardableHandle::deleteHandle() {
    // Empty handle.
