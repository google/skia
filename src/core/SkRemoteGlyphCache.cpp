...
void SkStrikeServer::checkpointTrace(SkWStream* wStream) {
...
std::unique_ptr<SkWStream> SkStrikeServer::finishBlobCapture() {
    this->checkpointTrace(fWStream.get());
    fCaptureBlobs = false;
    std::unique_ptr<SkWStream> answer = std::move(fWStream);
    fWStream = nullptr;
    fWriteBuffer.reset();
    fTypefaceSet.reset();
    return answer;
