...
template <typename T>
class ArraySlice : public std::tuple<const T*, size_t> {
public:
    // Additional constructors as needed.
    ArraySlice(const T* data, size_t size) : std::tuple<const T*, size_t>{data, size} { }
    ArraySlice() : ArraySlice<T>(nullptr, 0) { }
    friend const T* begin(const ArraySlice<T>& slice) {
        return slice.data();
    }

    friend const T* end(const ArraySlice<T>& slice) {
