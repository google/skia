...
SkStrikeServer::SkGlyphCacheState* SkStrikeServer::getOrCreateCache(
    ...
    if (it != fRemoteGlyphStateMap.end()) {
        if (it->second->getDeviceDescriptor() != *deviceDesc) {
            {
                auto& d1 = it->second->getDeviceDescriptor();
                auto& d2 = *deviceDesc;
                SkDebugf("Device:\n");
                SkDebugf("Checksum: in: %8x  -  %8x :new\n", d1.getChecksum(), d2.getChecksum());
                SkDebugf("Len     : in: %8d  -  %8d :new\n", d1.getLength(), d2.getLength());
                SkDebugf("Count   : in: %8d  -  %8d :new\n", d1.getCount(), d2.getCount());
            }
            {
                auto& d1 = it->second->getKeyDescriptor();
                auto& d2 = *keyDesc;
                SkDebugf("Key:\n");
                SkDebugf("Checksum: in: %8x  -  %8x :new\n", d1.getChecksum(), d2.getChecksum());
                SkDebugf("Len     : in: %8d  -  %8d :new\n", d1.getLength(), d2.getLength());
