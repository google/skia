...
private:
    ...
    void processGlyphRun(
        ...
        fGlyphManager->newRun(*desc, cache->getFontMetrics(), runPaint);
        ...
        for (uint32_t index = 0; index < it.glyphCount(); index++) {
            ...
            auto glyph = cache->getGlyphIDMetrics(glyphs[index], subPixelPos.x(), subPixelPos.y());
            ...
            fGlyphManager->addGlyph(dst, glyph.getPackedID(), glyphSize, (uint8_t*)glyph.fImage);
        }
        fGlyphManager->endRun();
    }
...
SkTextBlobToDrawAtlasAndPaths::~SkTextBlobToDrawAtlasAndPaths() = default;
