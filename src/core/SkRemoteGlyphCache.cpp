...
private:
    ...
    SkTHashTable<SkGlyphSummary, SkPackedGlyphID, GlyphSummaryTraits> fSentPaths;
...
void SkStrikeServer::RemoteStrike::writePendingGlyphs(Serializer* serializer) {
    ...
    for (SkGlyph& glyph : fGlyphMasksToSend) {
        ...
        if (imageSize > 0 && glyph.maxDimension() <= SkStrikeCommon::kSkSideTooBigForAtlas) {
...
SkStrikeServer::RemoteStrike::prepareForPathDrawing(SkGlyphinator glyphPos,
    ...
    for (size_t i = 0; i < glyphPos.n; i++) {
        SkPackedGlyphID packedID = glyphPos.glyphs[i].packedID;
        SkGlyphSummary* summary = fSentPaths.find(packedID);
        if (summary == nullptr) {
            // Put the new SkGlyph in the glyphs to send.
        ...
        // Reject things that are too big.
        if ((*summary & (kHasColor | kNeedsMask)) != 0) {
            rejects[rejectCursor++] = i;
        }
    }
