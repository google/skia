...
std::tuple<SkData, SkTransport::IOResult> SkTransport::readSkData() {
    size_t size;
    if(this->read(&size, sizeof(size)) == kFail) {
        return nullptr;
    }

    auto data = skstd::make_unique<uint8_t[]>(size);
    if(this->read(data.get(), size) == kFail) {
        return nullptr;
    }
...
static void update_caches_from_strikes_data(SkRemoteGlyphCacheGPU *cache,
...
void SkPrepopulateCache(
...

    {
        ...
        write_strikes_spec(strikeDifference, serializer.get(), transport);
    }
    {
        auto deserializer = skstd::make_unique<Deserializer>();;
        update_caches_from_strikes_data(cache, deserializer.get(), transport);
    }
}
...
void SkStrikeClient::primeStrikeCache(const SkStrikeCacheDifferenceSpec&) {
...
SkTypeface* SkStrikeClient::lookupTypeface(SkFontID id) { return nullptr; }
