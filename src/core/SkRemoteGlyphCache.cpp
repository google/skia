...
public:
    ...
    void
    prepareForMaskDrawing(SkDrawableGlyphBuffer* drawbles, SkSourceGlyphBuffer* rejects) override;

    void
    prepareForSDFTDrawing(SkDrawableGlyphBuffer* drawbles, SkSourceGlyphBuffer* rejects) override;

    void
    prepareForPathDrawing(SkDrawableGlyphBuffer* drawbles, SkSourceGlyphBuffer* rejects) override;
...
void SkStrikeServer::AddGlyphForTesting(RemoteStrike* strike, SkGlyphinator* inator) {
    std::vector<size_t> rejected;
    strike->prepareForMaskDrawing(*inator, rejected);
    SkASSERT(rejected.empty());
}
...
void SkStrikeServer::RemoteStrike::prepareForMaskDrawing(
    ...
    for (auto t : SkMakeEnumerate(drawbles->input())) {
        size_t i; SkBag bag; SkPoint pos;
...
void SkStrikeServer::RemoteStrike::prepareForPathDrawing(
    ...
    for (auto t : SkMakeEnumerate(drawbles->input())) {
        ...
        std::forward_as_tuple(i, std::tie(bag, pos)) = t;
        SkPackedGlyphID packedID = bag;
        SkGlyphID glyphID = packedID.code();
...
bool SkStrikeClient::readStrikeData(const volatile void* memory, size_t memorySize) {
    ...
    for (size_t i = 0; i < strikeCount; ++i) {
        ...
        for (size_t j = 0; j < glyphImagesCount; j++) {
            ...
            if (!glyph->isEmpty() && glyph->isColor() && SkStrikeForGPU::FitsInAtlas(*glyph)) {
                const volatile void* image =
                        deserializer.read(glyph->imageSize(), glyph->formatAlignment());
                if (!image) READ_FAILURE
                glyph->fImage = (void*)image;
