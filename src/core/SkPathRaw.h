...
class SkPathRaw {
    ...
    friend class SkPathRaw_Editor;
    friend class SkPathRaw_Rect;
    friend class SkPathRaw_Oval;
    friend class SkPathRaw_Iter;
    friend class SkPathEdgeIter;
    friend class SkPathPriv;

public:
    SkPathRaw() {}
    SkPathRaw(SkSpan<SkPoint> pts, SkSpan<uint8_t> vbs, SkSpan<SkScalar> cw,
              SkPathFillType ft, SkPathConvexityType ct, SkPathSegmentMask sm, bool isFinite)
        : fPts(pts), fVerbs(vbs), fConicWeights(cw)
        , fFillType(ft), fConvexity(ct), fSegmentMask(sm), fIsFinite(isFinite)
    {}
    explicit SkPathRaw(const SkPath&);
    ...
    SkPoint* points() const { return fPts.begin(); }
    uint8_t* verbs() const { return fVerbs.begin(); }
    SkScalar* conicWeights() const { return fConicWeights.begin(); }
    ...
    void setBounds(const SkRect& r) { fBounds = r; }
    void setIsFinite(bool f) { fIsFinite = f; }
};
...
class SkPathRaw_Editor {
    ...
    void assertRoomFor(int ptCount, bool conic = false) const {
        ...
        SkASSERT(fCurrPt + ptCount <= fRaw->points() + fRaw->countPoints());
        SkASSERT(fCurrVb < fRaw->verbs() + fRaw->countVerbs());
    }
...
public:
    SkPathRaw_Editor(SkPathRaw* raw) : fRaw(raw) {
        fCurrPt = fRaw->points();
        fCurrVb = fRaw->verbs();
        fCurrW  = fRaw->conicWeights();
        fSegmentMask = 0;
    }

    void init(SkPathFillType ft) {
        fRaw->fFillType = ft;
        fRaw->fConvexity = kUnknown_SkPathConvexityType;
        // leave bounds, segmentmask and convexity until seal()
    }
    ...
    // Computes bounds, isFinite, and convexity (if the param set that to kUnknown
    void seal(SkPathConvexityType = kUnknown_SkPathConvexityType);
};
