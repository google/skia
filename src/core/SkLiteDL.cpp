...
#undef M
    ...
    struct DrawVertices final : Op {
        ...
        DrawVertices(const SkVertices* v, const SkSkeleton* b, SkBlendMode m, const SkPaint& p)
            : vertices(sk_ref_sp(const_cast<SkVertices*>(v)))
            , bones(sk_ref_sp(const_cast<SkSkeleton*>(b)))
            , mode(m)
            , paint(p) {}
        sk_sp<SkVertices> vertices;
        sk_sp<SkSkeleton> bones;
        SkBlendMode mode;
        SkPaint paint;
        void draw(SkCanvas* c, const SkMatrix&) const {
            c->drawVertices(vertices, bones, mode, paint);
        }
    };
...
void SkLiteDL::drawVertices(const SkVertices* vertices, const SkSkeleton* bones, SkBlendMode mode,
                            const SkPaint& paint) {
    this->push<DrawVertices>(0, vertices, bones, mode, paint);
}
