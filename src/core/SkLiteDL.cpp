...
#undef M
    ...
    struct DrawEdgeAAQuad final : Op {
        ...
        DrawEdgeAAQuad(const SkRect& rect, const SkPoint clip[], int clipCount,
                       SkCanvas::QuadAAFlags aa, SkColor color, SkBlendMode mode)
            : rect(rect), clipCount(clipCount), aa(aa), color(color), mode(mode) {
        ...
        SkBlendMode mode;
        void draw(SkCanvas* c, const SkMatrix&) const {
            c->experimental_DrawEdgeAAQuad(rect, clip, clipCount, aa, color, mode);
        }
    };
...
void SkLiteDL::drawEdgeAAQuad(const SkRect& rect, const SkPoint clip[], int clipCount,
                              SkCanvas::QuadAAFlags aa, SkColor color, SkBlendMode mode) {
    SkASSERT(clipCount <= 4); // DrawEdgeAAQuad record holds up to 4 points
    this->push<DrawEdgeAAQuad>(0, rect, clip, clipCount, aa, color, mode);
}

void SkLiteDL::drawEdgeAAImageSet(const SkCanvas::ImageSetEntry set[], int count,
                            const SkPoint dstClips[], const SkMatrix preViewMatrices[],
                            const SkPaint* paint, SkCanvas::SrcRectConstraint constraint) {
    int totalDstClipCount = 0;
    int maxMatrixIndex = -1;
    for (int i = 0; i < count; ++i) {
        totalDstClipCount += set[i].fDstClipCount;
        if (set[i].fMatrixIndex > maxMatrixIndex) {
            maxMatrixIndex = set[i].fMatrixIndex;
        }
    }
    size_t bytes = totalDstClipCount * sizeof(SkPoint) + (maxMatrixIndex + 1) * sizeof(SkMatrix);
    ...
    copy_v(pod, dstClips, totalDstClipCount,
           preViewMatrices, maxMatrixIndex + 1);
}
