...
void SkBitmapDevice::drawSpecial(SkSpecialImage* src, int x, int y, const SkPaint& origPaint,
    ...
    if (SkImageFilter* filter = paint->getImageFilter()) {
        ...
        SkFilterOutputProperties outputProperties(fBitmap.colorType(), fBitmap.colorSpace());
        SkFilterContext ctx(matrix, clipBounds, cache.get(), outputProperties);

        filteredImage = filter->priv().filterImage(src, ctx, &offset);
        if (!filteredImage) {
            return;
        }
