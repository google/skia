...
public:
    SkDrawTiler(SkBitmapDevice* dev) : fDevice(dev) {
        ...
        fOrigin.set(0, 0);  // only needed by tiling
    ...
    const SkDraw* next() {
        ...
        if (fNeedsTiling) {
            do {
                ...
                this->stepOrigin();     // might set fDone to true for next iteration
    ...
    void setupTileDraw() {
        SkASSERT(!fDone);
        SkIRect bounds = SkIRect::MakeXYWH(fOrigin.x(), fOrigin.y(),
                                           SkMin32(fRootPixmap.width(),  kMaxDim),
                                           SkMin32(fRootPixmap.height(), kMaxDim));
    ...
    bool more_x() const { return fOrigin.x() + fDraw.fDst.width()  < fRootPixmap.width(); }
    bool more_y() const { return fOrigin.y() + fDraw.fDst.height() < fRootPixmap.height(); }

    void stepOrigin() {
        SkASSERT(!fDone);
        SkASSERT(fNeedsTiling);
        if (this->more_x()) {
            SkASSERT(fDraw.fDst.width() == kMaxDim);
            fOrigin.fX += fDraw.fDst.width();
        } else if (this->more_y()) {
            SkASSERT(fDraw.fDst.height() == kMaxDim);
            fOrigin.fY += fDraw.fDst.height();
            fOrigin.fX = 0;
        }
        fDone = !this->more_x() && !this->more_y();
    }
};
...
void SkBitmapDevice::drawPath(const SkPath& path,
                              ...
                              bool pathIsMutable) {
    LOOP_TILER( drawPath(path, paint, prePathMatrix, pathIsMutable))
}
...
void SkBitmapDevice::drawSprite(const SkBitmap& bitmap, int x, int y, const SkPaint& paint) {
    LOOP_TILER( drawSprite(bitmap, x, y, paint))
}
...
void SkBitmapDevice::drawDevice(SkBaseDevice* device, int x, int y, const SkPaint& origPaint) {
    ...
    LOOP_TILER( drawSprite(static_cast<SkBitmapDevice*>(device)->fBitmap, x, y, *paint))
}
