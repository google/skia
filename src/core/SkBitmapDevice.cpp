...
void SkBitmapDevice::drawSpecial(SkSpecialImage* srcImg, int x, int y, const SkPaint& origPaint,
                                 SkImage* clipImage, const SkMatrix& clipMatrix) {
    SkASSERT(!srcImg->isTextureBacked());

    SkSpecialImage* src = srcImg;
    sk_sp<SkSpecialImage> tmpImage;
    ...
    if (SkImageFilter* filter = paint->getImageFilter()) {
        ...
        tmpImage = filter->filterImage(srcImg, ctx, &offset);
        if (!tmpImage) {
            return;
        }
    ...
    sk_sp<SkImage> snap = surf->makeImageSnapshot();
    SkPixmap pmap;
    SkAssertResult(snap->peekPixels(&pmap));

    const SkMatrix m = SkMatrix::MakeTrans(x - maskBounds.x(), y - maskBounds.y());
    paint.writable()->setShader(srcImage->makeShader(SkShader::kClamp_TileMode,
                                                     SkShader::kClamp_TileMode,
                                                     &m));

    SkAutoDeviceCTMRestore adctmr(this, SkMatrix::I());
    this->drawImage(snap.get(), maskBounds.x(), maskBounds.y(), *paint);
}
