...
public:
    ModifiedDraw(const SkMatrix& devCTM, const SkRasterClip& rc, SkISize rcs, const SkDraw& draw) : SkDraw(draw) {
        SkISize dvs = { draw.fDevice->width(), draw.fDevice->height() };
        SkASSERT(dvs == rcs);
        SkASSERT(devCTM == *draw.fMatrix);
//        SkASSERT(fRC->getBounds() == rc.getBounds());
        fRC = &rc;
    }
};
