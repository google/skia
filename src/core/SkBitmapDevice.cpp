...
void SkBitmapDevice::drawAtlas(const SkImage* atlas, const SkRSXform xform[],
                               ...
                               SkBlendMode mode, SkFilterMode filter, const SkPaint& paint) {
    // set this to true for performance comparisons with the old drawVertices way
    if (false) {
        this->INHERITED::drawAtlas(atlas, xform, tex, colors, count, mode, filter, paint);
        return;
    }
    BDDraw(this).drawAtlas(atlas, xform, tex, colors, count, mode, filter, paint);
}
