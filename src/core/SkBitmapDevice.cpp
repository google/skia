...
struct Bounder {
    ...
    Bounder(const SkRect& r, const SkPaint& paint) {
        if ((fHasBounds = paint.canComputeFastBounds())) {
            (void)paint.computeFastBounds(r, &fBounds);
        }
    }

    operator const SkRect* () const { return fHasBounds ? &fBounds : nullptr; }
};
...
void SkBitmapDevice::drawPaint(const SkPaint& paint) {
    LOOP_TILER( drawPaint(paint), nullptr)
}
...
void SkBitmapDevice::drawPath(const SkPath& path,
                              ...
                              bool pathIsMutable) {
    SkDrawTiler tiler(this, !path.isInverseFillType() ? Bounder(path.getBounds(), paint) : nullptr);
...
void SkBitmapDevice::drawBitmap(const SkBitmap& bitmap, const SkMatrix& matrix,
    ...
    LOOP_TILER( drawBitmap(bitmap, matrix, dstOrNull, paint), nullptr)
}
...
void SkBitmapDevice::drawSprite(const SkBitmap& bitmap, int x, int y, const SkPaint& paint) {
    LOOP_TILER( drawSprite(bitmap, TILER_X(x), TILER_Y(y), paint), nullptr)
}
...
void SkBitmapDevice::drawDevice(SkBaseDevice* device, int x, int y, const SkPaint& origPaint) {
    ...
    LOOP_TILER( drawSprite(static_cast<SkBitmapDevice*>(device)->fBitmap,
                           TILER_X(x), TILER_Y(y), *paint), nullptr)
}
