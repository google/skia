...
static void convert_with_pipeline(const SkImageInfo& dstInfo, void* dstRow, size_t dstRB,
    ...
    if (auto dstAT = dstInfo.colorSpace()) {
        auto srcAT = srcInfo.colorSpace() ? srcInfo.colorSpace()
                                          : sk_srgb_singleton();
        steps.init(srcAT, srcInfo.alphaType(),
                   dstAT, dstInfo.alphaType());
        steps.get()->apply(&pipeline);
    }
