...
static size_t safe_mul(size_t a, uint32_t b) {
    // Safety: if mul is unsafe then size < a * b for all a and b. The argument is that
    // size < (a * b) % 2^64. So, (2^64 - 1) (2^32 - 1) % 2^64 < 2^64-1 which is true.
    uint64_t size = a * b;
    if (size < a) {
        return 0;
    }
...
size_t SkMask::computeTotalImageSize() const {
    return ComputeImageSize(
        fFormat, SkTo<uint16_t>(fBounds.width()), SkTo<uint16_t>(fBounds.height()));
}
...
size_t SkMask::ComputeImageSize(SkMask::Format format, int32_t width, int32_t height) {
    ...
    return safe_mul(size, safeHeight);
}
