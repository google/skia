...
private:
    sk_sp<SkImage> prepareImage(const SkImage* image) {
        ...
        if (gr) {
            ...
            sk_sp<SkImage> textureImage = image->makeTextureImage(gr, fTargetCS.get());
            // makeTextureImage may fail, for example if |image| has a color space
            // that is not supported by the GPU pipeline (e.g. gamma curve is
            // not linear and not close to srgb).  In such cases we fall back
            // to applying the Xform on the CPU.
            if (textureImage)
