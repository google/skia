...
public:
    ...
    void onDrawEdgeAAQuad(const SkRect& rect, const SkPoint clip[], int clipCount,
                          SkCanvas::QuadAAFlags aa, SkColor color, SkBlendMode mode) override {
        fTarget->experimental_DrawEdgeAAQuad(
                rect, clip, clipCount, aa, fXformer->apply(color), mode);
    }
    void onDrawEdgeAAImageSet(const SkCanvas::ImageSetEntry set[], int count,
        ...
        for (int i = 0; i < count; ++i) {
            ...
            xformedSet[i].fDstClipCount = set[i].fDstClipCount;
