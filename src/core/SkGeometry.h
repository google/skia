...
/** Returns the cubic classification.
    ...
    loop they are the double point. For a local cusp, they both represent the cusp point. For a
    cusp at an infinite parameter value, one will be the local inflection point and the other +inf.
    If the curve is degenerate (i.e. quadratic or linear) they are both set to a parameter value of
    infinity.

    d[] is filled with  the cubic inflection function coefficients.

    See "Resolution Independent Curve Rendering using Programmable Graphics Hardware", 4.2 Curve
    Categorization:
