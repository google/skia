...
public:
    ...
    void reset(SkZip<SkGlyphID, SkPoint>& source) {
    ...
    SkZip<SkGlyphID, SkPoint> flipRejectsToSource() {
        fSource = fRejects.first(fRejectSize);
        fRejectSize = 0;
        return fSource;
    }

    SkZip<SkGlyphID, SkPoint> source() const { return fSource; }
...
public:
    void startSource(const SkZip<SkGlyphID, SkPoint>& source, SkPoint origin);

    void startDevice(
            const SkZip<SkGlyphID, SkPoint>& source,
