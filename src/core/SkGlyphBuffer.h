...
public:
    ...
    // Load the buffer with SkPackedGlyphIDs, calculated positions so they can be constant.
    //
    // A final device position is computed int the following manner:
    //  [x,y] = Floor[M][T][x',y']^t
    ...
    // This allows positioning to be P + Q', which give ideal numbers would be an integer.
    void startGPUDevice(
            const SkZip<const SkGlyphID, const SkPoint>& source,
            SkPoint origin, const SkMatrix& viewMatrix,
            const SkGlyphPositionRoundingSpec& roundingSpec);
