...
static void copy_pipeline_pixels(const SkImageInfo& dstInfo, void* dstRow, size_t dstRB,
                                 ...
                                 const uint32_t* colorTable, bool isColorAware) {
    SkRasterPipeline pipeline;
    LoadIndex8 loadIndex8;
    switch (srcInfo.colorType()) {
        ...
        case kIndex_8_SkColorType:
            SkASSERT(colorTable);
            loadIndex8.fSrc = (const uint8_t*) srcRow;
            loadIndex8.fTable = colorTable;
            pipeline.append(SkRasterPipeline::load_index8, &loadIndex8);
            break;
    ...
    for (int y = 0; y < srcInfo.height(); ++y) {
        ...
        // Only necessary for kIndex8 src.
        loadIndex8.fSrc = SkTAddOffset<const uint8_t>(loadIndex8.fSrc, srcRB);
    }
}
...
bool SkPixelInfo::CopyPixels(const SkImageInfo& dstInfo, void* dstPixels, size_t dstRB,
    ...
    // Default: Use the pipeline.
    copy_pipeline_pixels(dstInfo, dstPixels, dstRB, srcInfo, srcPixels, srcRB,
                         ctable ? ctable->readColors() : nullptr, isColorAware);
    return true;
}
