...
static bool copy_pipeline_pixels(const SkImageInfo& dstInfo, void* dstRow, size_t dstRB,
    ...
    bool src_swap_rb = kBGRA_8888_SkColorType == srcInfo.colorType();
    bool dst_swap_rb = kBGRA_8888_SkColorType == dstInfo.colorType();
    if (src_swap_rb && dst_swap_rb) {
        src_swap_rb = dst_swap_rb = false;  // no need to swap twice
    }
