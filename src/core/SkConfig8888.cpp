...
template <typename T>
void do_index8_bpp(const SkImageInfo& dstInfo, void* dstPixels, size_t dstRB,
                   const SkImageInfo& indexInfo, const void* srcPixels, size_t srcRB,
                   SkColorTable* ctable) {
    T dstCTable[256];
    int srcWidth = ctable->count();
    SkImageInfo srcInfo = indexInfo.makeColorType(kN32_SkColorType).makeWH(srcWidth, 1);
    SkPixelInfo::CopyPixels(dstInfo, dstCTable, dstRB, srcInfo, ctable->readColors(),
                            srcWidth * sizeof(T), nullptr);

    T* dstPixelsT = (T*) dstPixels;
    const uint8_t* srcPixels8 = (const uint8_t*) srcPixels;
    for (int y = 0; y < dstInfo.height(); y++) {
        for (int x = 0; x < dstInfo.width(); x++) {
            dstPixelsT[x] = dstCTable[srcPixels8[x]];
        }
        dstPixelsT = SkTAddOffset<T>(dstPixelsT, dstRB);
        srcPixels8 = SkTAddOffset<const uint8_t>(srcPixels8, srcRB);
    }
}

void do_index8(const SkImageInfo& dstInfo, void* dstPixels, size_t dstRB,
               const SkImageInfo& srcInfo, const void* srcPixels, size_t srcRB,
                SkColorTable* ctable) {
    switch (dstInfo.colorType()) {
        case kAlpha_8_SkColorType:
            do_index8_bpp<uint8_t>(dstInfo, dstPixels, dstRB, srcInfo, srcPixels, srcRB, ctable);
            break;
        case kRGB_565_SkColorType:
        case kARGB_4444_SkColorType:
            do_index8_bpp<uint16_t>(dstInfo, dstPixels, dstRB, srcInfo, srcPixels, srcRB, ctable);
            break;
        case kRGBA_8888_SkColorType:
        case kBGRA_8888_SkColorType:
            do_index8_bpp<uint32_t>(dstInfo, dstPixels, dstRB, srcInfo, srcPixels, srcRB, ctable);
            break;
        case kRGBA_F16_SkColorType:
            do_index8_bpp<uint64_t>(dstInfo, dstPixels, dstRB, srcInfo, srcPixels, srcRB, ctable);
            break;
        default:
            SkASSERT(false);
    }
...
bool SkPixelInfo::CopyPixels(const SkImageInfo& dstInfo, void* dstPixels, size_t dstRB,
    ...
    if (kIndex_8_SkColorType == srcInfo.colorType()) {
        SkASSERT(ctable);
        do_index8(dstInfo, dstPixels, dstRB, srcInfo, srcPixels, srcRB, ctable);
        return true;
    }
