...
static bool copy_pipeline_pixels(const SkImageInfo& dstInfo, void* dstRow, size_t dstRB,
    ...
    switch (srcInfo.colorType()) {
        ...
        case kGray_8_SkColorType:
            pipeline.append(SkRasterPipeline::load_g8);
            break;
        default:
            return false;
    }
    ...
    for (int y = 0; y < srcInfo.height(); ++y) {
        p(0,srcInfo.width());
        // The pipeline has pointers to srcRow and dstRow, so we just need to update
        // them in the loop to move between rows of src and dst.
