...
void SkDrawableGlyphBuffer::startSource(const SkZip<SkGlyphID, SkPoint>& source, SkPoint origin) {
...
void SkDrawableGlyphBuffer::startDevice(const SkZip<SkGlyphID, SkPoint>& source, SkPoint origin,
                                        const SkMatrix& viewMatrix, const SkStrikeForGPU& strike) {
    // Properly size the buffers.
    this->reset(source.size());
