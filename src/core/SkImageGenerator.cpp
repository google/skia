...
bool SkImageGenerator::accessScaledPixels(const SkRect& src, const SkMatrix& matrix,
                                          SkFilterQuality fq, ScaledPixelsRec* rec) {
    SkASSERT(fInfo.bounds().contains(src));
    return this->onAccessScaledPixels(src, matrix, fq, rec);
}
