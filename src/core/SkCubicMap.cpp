 ...
 */
static float solve_nice_cubic_halley(float A, float B, float C, float D, float guess) {
    const int MAX_ITERS = 8;
    const float A3 = 3 * A;
    const float B2 = B + B;
...
#endif
    float answer2 = true ?
                    solve_nice_cubic_halley(A, B, C, -x, guess) :
...
float SkCubicMap::guessTFromX(float x) const {
    ...
    float guess = offset + delta * (fTableX[i] - x) / (fTableX[i] - fTableX[i-1]);
