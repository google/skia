...
void SkRasterPipeline::run(size_t x, size_t y, size_t w, size_t h) const {
    ...
    if (fCanUseRunProgramObs) {
        SkOpts::run_pipeline_obs(x,y, x+w,y+h,
                                 fStockStages.begin(), fStockStages.count(),
                                 (void**)fCtxPointers.begin());
        return;
    }
...
std::function<void(size_t, size_t, size_t, size_t)> SkRasterPipeline::compile() const {
    ...
    if (fCanUseRunProgramObs) {
        const auto& stock_stages = fStockStages;
        const auto& ctx_pointers = fCtxPointers;

        return [=](size_t x, size_t y, size_t w, size_t h) {
            SkOpts::run_pipeline_obs(x,y, x+w,y+h,
                                     stock_stages.begin(), stock_stages.count(),
                                     (void**)ctx_pointers.begin());
        };
    }
