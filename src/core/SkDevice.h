...
public:
    ...
    /**
     *  Get the pixel origin of this device, which is defined as the floor of the translation x and
     *  y components of its basis matrix. This is equivalent to the top-left corner of
     *  'getGlobalBounds()'. Returns true if the device is described by a simple basis, in which
     *  case the output 'origin' is sufficient to map the device's contents back to the root space.
     */
     ...
     *  simple basis matrices. In this case, this returns true and 'offset' is set to the integral
...
private:
    ...
    // Convenience to specify the origin separately from the layer matrix, automatically setting
    // the device's CTM to be trans(-x, -y) x layerMatrix, and updating 'fToGlobal' to be
    // globalCTM x layerMatrix^-1 x trans(x, y). Note that (x, y) is in the space described by
    // layerMatrix, and not the root space (i.e. (x, y) is the top-left corner of the layer's
    // content transformed by layerMatrix).
    void setBasisAndOrigin(const SkMatrix& globalCTM, const SkMatrix& layerMatrix, int x, int y) {
        SkMatrix offsetLayerMatrix = layerMatrix;
        // The new layer matrix must map the layer-space (x, y) to (0, 0), hence the negative signs.
        offsetLayerMatrix.postTranslate(-x, -y);
        this->setBasis(globalCTM, offsetLayerMatrix);
    }
    ...
    void setOrigin(const SkMatrix& globalCTM, int x, int y) {
        this->setBasisAndOrigin(globalCTM, globalCTM, x, y);
    }
