...
protected:
    ...
    // Used by SkCanvas to determine how to wrap a legacy SkBitmap as an SkImage. Defaults to true
    // (the safe option), but if the bitmap is rasterized immediately this can return false.
    virtual bool mustCopyMutableBitmap() const { return true; }
...
protected:
    ...
    // The draws do nothing, so don't bother copying
    virtual bool mustCopyMutableBitmap() const override { return false; }
