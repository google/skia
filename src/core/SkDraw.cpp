...
void SkDraw::drawText_asPaths(SkBaseDevice* device, const char text[], size_t byteLength,
    ...
    while (iter.next(&iterPath, &xpos)) {
        ...
        if (iterPath) {
            ...
            device->drawPath(*this, *iterPath, pnt, &matrix, false);
        }
        prevXPos = xpos;
    }
}
...
uint32_t SkDraw::ScalerContextFlags(SkBaseDevice* device) {
    uint32_t flags = SkPaint::kBoostContrast_ScalerContextFlag;
    if (!device->imageInfo().colorSpace()) {
        flags |= SkPaint::kFakeGamma_ScalerContextFlag;
    }
    return flags;
}

void SkDraw::drawText(SkBaseDevice* device, const char text[], size_t byteLength,
    ...
    if (ShouldDrawTextAsPaths(paint, *fMatrix)) {
        this->drawText_asPaths(device, text, byteLength, x, y, paint);
        return;
    }

    SkAutoGlyphCache cache(paint, &device->surfaceProps(), ScalerContextFlags(device), fMatrix);
...
void SkDraw::drawPosText_asPaths(SkBaseDevice* device, const char text[], size_t byteLength,
    ...
    SkPaint::GlyphCacheProc glyphCacheProc = SkPaint::GetGlyphCacheProc(paint.getTextEncoding(),
                                                                        paint.isDevKernText(),
                                                                        true);
    SkAutoGlyphCache cache(paint, &device->surfaceProps(), ScalerContextFlags(device), nullptr);
    ...
    while (text < stop) {
        ...
        if (glyph.fWidth) {
            ...
            if (path) {
                ...
                device->drawPath(*this, *path, paint, &matrix, false);
            }
        }
...
void SkDraw::drawPosText(SkBaseDevice* device, const char text[], size_t byteLength,
                         const SkScalar pos[], int scalarsPerPosition,
    ...
    if (ShouldDrawTextAsPaths(paint, *fMatrix)) {
        this->drawPosText_asPaths(device, text, byteLength, pos, scalarsPerPosition, offset, paint);
        return;
    }
