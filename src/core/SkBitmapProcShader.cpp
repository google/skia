...
public:
    ...
    BitmapProcInfoContext(const SkShader& shader, const SkShaderBase::ContextRec& rec,
                            SkBitmapProcInfo* info)
        : INHERITED(*as_SB(&shader), rec)
        , fInfo(info)
    {
        fFlags = 0;
...
public:
    BitmapProcShaderContext(const SkShader& shader, const SkShaderBase::ContextRec& rec,
...
public:
    LinearPipelineContext(const SkShader& shader, const SkShaderBase::ContextRec& rec,
    ...
    const SkBitmapProvider& provider, const ContextRec& rec, SkArenaAlloc* alloc)
{
    ...
    if (!as_SB(&shader)->computeTotalInverse(*rec.fMatrix, rec.fLocalMatrix, &totalInverse)) {
        return nullptr;
    }
