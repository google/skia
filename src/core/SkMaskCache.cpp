...
public:
        ...
        , fQuality(quality)
    {
        ...
        SkIRect ir;
        rects[0].roundOut(&ir);
        fSizes[0] = {rects[0].width(), rects[0].height()};
        if (2 == count) {
            fSizes[1] = {rects[1].width(), rects[1].height()};
            fSizes[2] = {rects[0].x() - rects[1].x(), rects[0].y() - rects[1].y()};
        } else {
            fSizes[1] = {0, 0};
            fSizes[2] = {0, 0};
        }
        fSizes[3] = {rects[0].x() - ir.x(), rects[0].y() - ir.y()};
