...
SkTextBlob::RunIterator SkTextBlob::begin() const {
    return RunIterator(SkTextBlob::RunRecord::First(this) SkDEBUGCODE(, this));
}

SkTextBlob::RunIterator SkTextBlob::end() const {
    return RunIterator(nullptr SkDEBUGCODE(, this));
}

SkTextBlob::RunIterator& SkTextBlob::RunIterator::operator++() {
    SkASSERT(fRun);
    SkDEBUGCODE(fRun->validate(fStorageTop);)
    fRun = SkTextBlob::RunRecord::Next(fRun);
    return *this;
}

uint32_t SkTextBlob::RunIterator::glyphCount() const {
    return fRun->glyphCount();
}

const uint16_t* SkTextBlob::RunIterator::glyphs() const {
    return fRun->glyphBuffer();
}

const SkScalar* SkTextBlob::RunIterator::pos() const {
    return fRun->posBuffer();
}
...
SkTextBlob::GlyphPositioning SkTextBlob::RunIterator::positioning() const {
    return fRun->positioning();
}

void SkTextBlob::RunIterator::applyFontToPaint(SkPaint* paint) const {
    fRun->font().applyToPaint(paint);
}

uint32_t* SkTextBlob::RunIterator::clusters() const {
    return fRun->clusterBuffer();
}
uint32_t SkTextBlob::RunIterator::textSize() const {
    return fRun->textSize();
}
char* SkTextBlob::RunIterator::text() const {
    return fRun->textBuffer();
}

bool SkTextBlob::RunIterator::isLCD() const {
    return SkToBool(fRun->font().flags() & SkPaint::kLCDRenderText_Flag);
}
