...
sk_sp<SkTextBlob> SkTextBlob::MakeFromText(const void* text, size_t byteLength, const SkFont& font,
    ...
    auto buffer = builder.allocRun(font, count, 0, 0);
    font.textToGlyphs(text, byteLength, encoding, buffer.glyphs, count);
    return builder.make();
}

sk_sp<SkTextBlob> SkTextBlob::MakeFromPosText(const void* text, size_t byteLength, const SkPoint pos[],
...
sk_sp<SkTextBlob> SkTextBlob::MakeFromPosTextH(const void* text, size_t byteLength, const SkScalar xpos[],
