...
template <typename T>
uint32_t unique_id(T* obj) { return obj->uniqueID(); }

template<>
uint32_t unique_id(SkDrawable* obj) {
    return obj->getGenerationID();
}

template <typename T>
int find_or_append(SkTArray<sk_sp<T>>& array, T* obj) {
    const auto obj_id = unique_id(obj);

    for (int i = 0; i < array.count(); i++) {
        if (unique_id(array[i].get()) == obj_id) {
            return i;
        }
    }
