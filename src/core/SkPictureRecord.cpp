...
void SkPictureRecord::onDrawEdgeAAQuad(const SkRect& rect, const SkPoint clip[],
                                       int clipCount, SkCanvas::QuadAAFlags aa,
                                       SkColor color, SkBlendMode mode) {

    // op + rect + aa flags + color + mode + clipCount + clipCount*points
    size_t size = 5 * kUInt32Size + sizeof(rect) + clipCount * sizeof(SkPoint);
    size_t initialOffset = this->addDraw(DRAW_EDGEAA_QUAD, &size);
    this->addRect(rect);
    this->addInt((int) aa);
    this->addInt((int) color);
    this->addInt((int) mode);
    this->addInt(clipCount);
    this->addPoints(clip, clipCount);
    this->validate(initialOffset, size);
}
