...
void SkPictureRecord::onDrawImageSet(const SkCanvas::ImageSetEntry set[], int cnt, float alpha,
                                     SkFilterQuality filterQuality, SkBlendMode mode) {
    // op + count + alpha + fq + mode + (image index, src rect, dst rect, aa flags) * cnt
    size_t size = 4 * kUInt32Size + sizeof(SkScalar) + (2 * kUInt32Size + 2 * sizeof(SkRect)) * cnt;
