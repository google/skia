...
bool load_a2b0_lutn_type(std::vector<SkColorSpace_A2B::Element>* elements, const uint8_t* src,
    ...
    if (!matrix.isIdentity()) {
        // ICC specs (10.8/10.9) say lut8/16Type profiles can only have non-identity matrices
        // if the input color space is not PCSXYZ (which we do not support as an input color space)
...
sk_sp<SkColorSpace> SkColorSpace_Base::MakeICC(const void* input, size_t len,
    ...
    if (a2b0) {
        ...
        switch (header.fInputColorSpace) {
            case kRGB_ColorSpace:
                inputColorFormat = SkColorSpace_Base::InputColorFormat::kRGB;
                break;
            case kCMYK_ColorSpace:
        ...
        return sk_sp<SkColorSpace>(new SkColorSpace_A2B(pcs, inputColorFormat, std::move(data),
                                                        std::move(elements)));
    }
