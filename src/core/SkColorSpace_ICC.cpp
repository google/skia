...
static inline int pcs_channels(SkColorSpace_A2B::PCS pcs) {
    SkASSERT(SkColorSpace_A2B::PCS::kLAB == pcs || SkColorSpace_A2B::PCS::kXYZ == pcs);
    return 3;
}

static inline int ics_channels(SkColorSpace_A2B::ICS ics) {
    switch (ics) {
...
static bool load_a2b0(std::vector<SkColorSpace_A2B::Element>* elements, const uint8_t* src,
                      size_t len, SkColorSpace_A2B::PCS pcs, SkColorSpace_A2B::ICS ics) {
    const uint32_t type = read_big_endian_u32(src);
    switch (type) {
        case kTAG_AtoBType:
            if (len < 32) {
    ...
    if (ics_channels(ics) != elements->front().inputChannels()) {
    ...
    if (pcs_channels(pcs) != elements->back().outputChannels()) {
...
sk_sp<SkColorSpace> SkColorSpace_Base::MakeICC(const void* input, size_t len,
    ...
    if (kRGB_ColorSpace == header.fInputColorSpace) {
        ...
        if (!grayTRC) {
            return_null("grayTRC tag required for Grayscale profiles.");
        }
        if (ICCTag::Find(tags.get(), tagCount, kTAG_A2B0)) {
            return_null("A2B0 Grayscale profiles are not supported (do they even exist?)");
        }
        SkGammas::Data data;
        SkColorSpaceTransferFn params;
        size_t tagBytes;
        ...
        return sk_sp<SkColorSpace>(new SkColorSpace_A2B(pcs, SkColorSpace_A2B::ICS::kGray,
    ...
    if (a2b0) {
        const SkColorSpace_A2B::PCS pcs = kXYZ_PCSSpace == header.fPCS
                                        ? SkColorSpace_A2B::PCS::kXYZ
                                        : SkColorSpace_A2B::PCS::kLAB;
        SkColorSpace_A2B::ICS ics;
        switch (header.fInputColorSpace) {
            case kRGB_ColorSpace:
                ics = SkColorSpace_A2B::ICS::kRGB;
                break;
            case kCMYK_ColorSpace:
                ics = SkColorSpace_A2B::ICS::kCMYK;
                break;
            case kGray_ColorSpace:
                SkASSERT(false);
                return nullptr;
            default:
                SkASSERT(false); // header.fInputColorSpace should be checked earlier
                return_null("ICC profile has invalid color input space");
        }
        std::vector<SkColorSpace_A2B::Element> elements;
        if (!load_a2b0(&elements, a2b0->addr(base), a2b0->fLength, pcs, ics)) {
            return_null("Failed to parse A2B0 tag");
        }
        return sk_sp<SkColorSpace>(new SkColorSpace_A2B(pcs, ics, std::move(profileData),
                                                        std::move(elements)));
    }
