...
template<bool WithIota, typename... Ts>
class SkZipDetail {
    using ReturnTuple = std::tuple<decltype(std::declval<Ts*>()[0])..., size_t>;

    class Iterator {
    public:
        Iterator(const SkZipDetail* zip, size_t index) : fZip{zip}, fIndex{index} { }
    ...
    private:
        const SkZipDetail* const fZip = nullptr;
        size_t fIndex = 0;
    };

public:
    SkZipDetail(size_t) = delete;
    explicit SkZipDetail(size_t size, Ts*... ts)
            : fPointers{ts...}
            , fSize{size} {}

    template<typename... Us>
    explicit SkZipDetail(const SkZipDetail<WithIota, Us...>& that)
...
using SkZip = SkZipDetail<false, Ts...>;
