...
SkYUVAPixmaps SkYUVAPixmaps::MakeUnowned(const SkYUVAPixmapInfo& yuvaPixmapInfo,
                                         void* memory,
                                         size_t sizeInBytes) {
    if (!yuvaPixmapInfo.isValid()) {
        return {};
    }
    if (sizeInBytes < yuvaPixmapInfo.computeTotalBytes()) {
        return {};
    }
    return SkYUVAPixmaps(yuvaPixmapInfo, SkData::MakeWithoutCopy(memory, sizeInBytes));
}
