...
public:
    ...
    static bool CanHandleRect(const SkIRect& bounds) {
        ...
        if (width > MaskAdditiveBlitter::kMAX_WIDTH) { return false; }
...
public:
    ...
    void flush_if_y_changed(SkFixed y, SkFixed nextY) override {
        if (SkFixedFloorToInt(y) != SkFixedFloorToInt(nextY)) { this->flush(); }
    }
...
RunBasedAdditiveBlitter::RunBasedAdditiveBlitter(SkBlitter*     realBlitter,
    ...
    if (isInverse) {
        ...
        if (!sectBounds.intersect(ir, clipBounds)) { sectBounds.setEmpty(); }
    }
...
void RunBasedAdditiveBlitter::blitAntiH(int x, int y, const SkAlpha alpha) {
    ...
    if (this->check(x, 1)) { fOffsetX = fRuns.add(x, 0, 1, 0, alpha, fOffsetX); }
}
...
void RunBasedAdditiveBlitter::blitAntiH(int x, int y, int width, const SkAlpha alpha) {
    ...
    if (this->check(x, width)) { fOffsetX = fRuns.add(x, 0, width, 0, alpha, fOffsetX); }
}
...
static void blit_aaa_trapezoid_row(AdditiveBlitter* blitter,
    ...
    if (len > kQuickLen) { delete[] alphas; }
}
...
static SK_ALWAYS_INLINE void blit_trapezoid_row(AdditiveBlitter* blitter,
    ...
    if (ll > lr) { ll = lr = approximate_intersection(ul, ll, ur, lr); }
...
static bool is_smooth_enough(SkAnalyticEdge* leftE,
    ...
    if (nextCurrE->fUpperX < currE->fUpperX) { std::swap(currE, nextCurrE); }
...
static void aaa_walk_convex_edges(SkAnalyticEdge*  prevHead,
    ...
    for (;;) {
        ...
        while (leftE->fLowerY <= y) {  // Due to smooth jump, we may pass multiple short edges
            if (!leftE->update(y)) {
                if (SkFixedFloorToInt(currE->fUpperY) >= stop_y) { goto END_WALK; }
                leftE = currE;
        ...
        if (SkFixedFloorToInt(y) >= stop_y) { break; }
...
static void insert_new_edges(SkAnalyticEdge* newEdge, SkFixed y, SkFixed* nextNextY) {
    ...
    do {
        ...
        do {
            if (start->fNext == newEdge) { goto nextEdge; }
    ...
    while (true) {
        ...
        while (currE->fUpperY <= y) {
            ...
            if (currE->fLowerY <= nextY) {
                ...
                if (!skipIntersect) { check_intersection(currE, nextY, &nextNextY); }
            }
        ...
        if (forceRLE) { ((RunBasedAdditiveBlitter*)blitter)->flush_if_y_changed(y, nextY); }

        y = nextY;
        if (y >= SkIntToFixed(stop_y)) { break; }
...
static SK_ALWAYS_INLINE void aaa_fill_path(
    ...
    if (0 == count) {
        if (path.isInverseFillType()) {
             ...
             */
            if (rect.fTop < start_y) { rect.fTop = start_y; }
            if (rect.fBottom > stop_y) { rect.fBottom = stop_y; }
            if (!rect.isEmpty()) {
                blitter->getRealBlitter()->blitRect(
    ...
    if (!pathContainedInClip && start_y < clipRect.fTop) { start_y = clipRect.fTop; }
    if (!pathContainedInClip && stop_y > clipRect.fBottom) { stop_y = clipRect.fBottom; }
