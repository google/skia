...
void backward_insert_edge_based_on_x(SkAnalyticEdge* edge,
    ...
    while (prev->fPrev != nullptr && prev->fX > x) {
        if (intersectYs) {
            ...
            if (prev->fX - x <= 2) {
                ...
                // We minus 2 to guarantee that the order will be flipped at newY even if
                // abs(prev->fDX - edge->fDX) is very small.
                double deltaY = (double)(prev->fX - x - 2) / (prev->fDX - edge->fDX);

                // Increase newY from the true intersection by kSafeJump to
                // ensure that the order will be flipped at newY even if there is a small
                // precision error. Note that this kSafeJump shall be less than (1 << 7).
                // A jump of (1 << 7) is 0.5 alpha (1 << 16 is 255 alpha). With rounding,
                // that 0.5 alpha could break our 0 <= alpha <= 256 assert.
                const SkFixed kSafeJump = (1 << 6);
                SkFixed newY = SkDoubleToFixed(SkFixedToDouble(prev->fY) - deltaY) + kSafeJump;
                #ifdef SK_DEBUG
...
SkAnalyticEdge* backward_insert_start(SkAnalyticEdge* prev, SkFixed x) {
    // Note that we make the prevHead's fX to be the leftBound, so edge drift may produce
    // some x that's less than prevHead.fX. Therefore we have to check prev->fPrev to make
    // sure that we don't go left of the prevHead.
    while (prev->fPrev != nullptr && prev->fX > x) {
        prev = prev->fPrev;
    }
    return prev;
...
void aaa_walk_edges(SkAnalyticEdge* prevHead, SkAnalyticEdge* nextTail, SkPath::FillType fillType,
    ...
    for (;;) {
        ...
        bool noRealBlitter = forceRLE && (nextY - y != SK_Fixed1);
...
void aaa_fill_path(const SkPath& path, const SkIRect& clipRect, AdditiveBlitter* blitter,
    ...
    if (!path.isInverseFillType() && path.isConvex()) {
        ...
        aaa_walk_edges(&headEdge, &tailEdge, path.getFillType(), blitter, start_y, stop_y,
                       rect.fLeft << 16, rect.fRight << 16, isUsingMask, forceRLE, intersectYs);
    }
}
