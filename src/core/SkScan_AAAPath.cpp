        ...
        if (intersectYs) {
            ...
            if (prev->fX - x <= kDrift) {
                ...
                SkFixed newY = SkDoubleToFixed(SkFixedToDouble(prev->fY) - deltaY) + 1;
                    ...
                    edge->goY(y);
                #endif
                // TODO maybe only snap where the number of intersections is large, so
                //      we can draw faster and prettier when the number if intersections is small.
                // Note that we may not want to snap y here because that may
                // 1. violate the flipping property we want
                //    (so we have to use ceiling to snap if we have to)
                // 2. introduce too much trapezoid overlap that violate the alpha <= 256 constraint
                //    (so we have to use SkTMin to clamp the alpha)
                ...
                // Again, if we want to snap, we have to use ceiling to ensure the flipping
                intersectYs->push_back(SkAnalyticEdge::ceilSnapY(newY));
                #ifdef SK_DEBUG
...
// Return true if prev->fX, next->fX is too close in the current pixel row
static inline bool edges_too_close(SkAnalyticEdge* prev, SkAnalyticEdge* next) {
    SkASSERT(prev == nullptr || next == nullptr ||
            prev->fPrev == nullptr || next->fNext == nullptr || !next->fIsAdded ||
            prev->fX - SkAbs32(prev->fDX) <= next->fX + SkAbs32(next->fDX));
    // SkASSERT(prev->fPrev == nullptr || prev->fY > prev->fUpperY);
    return (prev && prev->fPrev && next && next->fNext != nullptr && next->fIsAdded &&
            prev->fX + SkAbs32(prev->fDX)/* + (SK_Fixed1 >> 4)*/ >= next->fX - SkAbs32(next->fDX));
}
...
static inline void blit_saved_trapezoid(SkAnalyticEdge* leftE, SkFixed lowerY,
    ...
    blit_trapezoid_row(blitter, y, leftE->fSavedX, riteE->fSavedX, lowerLeft, lowerRite,
            ...
            noRealBlitter || edges_too_close(leftE->fPrev, leftE)
                          || edges_too_close(riteE, riteE->fNext));
...
static inline SkAnalyticEdge* clone_edges(const SkAnalyticEdge& headEdge,
        const SkAutoSMalloc<1024>& storage, int totalSize) {
    char* currStorage = (char*)storage.get();

    SkAnalyticEdge* first = nullptr;
    SkAnalyticEdge* prev = nullptr;
    for (const SkAnalyticEdge* edge = &headEdge; edge != nullptr; edge = edge->fNext) {
        ...
        if (prev != nullptr) {
            prev->fNext = clonedEdge;
        }
        clonedEdge->fPrev = prev;
        prev = clonedEdge;
    }
    return first;
}
...
void aaa_fill_path(const SkPath& path, const SkIRect& clipRect, AdditiveBlitter* blitter,
    ...
    if (!path.isInverseFillType() && path.isConvex()) {
        ...
        int totalSize = compute_all_edges_size(headEdge);
        SkAutoSMalloc<1024> storage(totalSize);
        SkAnalyticEdge* clonedHead = clone_edges(headEdge, storage, totalSize);
