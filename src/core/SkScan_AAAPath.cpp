            ...
            if (prev->fX - x <= kDrift) {
                ...
                SkFixed newY = SkDoubleToFixed(SkFixedToDouble(prev->fY) - deltaY) + 1;
                    ...
                    if (prev->fX - edge->fX >= 256) {
                        SkDebugf("Flip too large: deltaY = %lf, y = %f, newY = %f, x = %d, %d\n",
                                deltaY, SkFixedToFloat(y), SkFixedToFloat(newY),
                                prev->fX, edge->fX);
                    }
                    // When fDX is very large, we have no hope to make the fX close
                    SkASSERT(SkTMax(SkAbs32(prev->fDX), SkAbs32(edge->fDX)) > 128 * SK_Fixed1
                            || prev->fX - edge->fX < 256);
                    prev->goY(y);
                    edge->goY(y);
                #endif
