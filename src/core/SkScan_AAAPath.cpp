...
static SkAlpha partial_triangle_to_alpha(SkFixed a, SkFixed b) {
    ...
    // We allow some error above SK_Fixed1 for a * b due to precision limit.
    // For large b, we won't force any error bound because the precision loss
    // is too large.
#ifdef SK_DEBUG  // TODO TEST
    if ((uint32_t)b <= 0x80000 && SkFixedMul(a, b) > SK_Fixed1 * 1.2) {
        SkDebugf("a = %x, b = %x, a/b = %lf\n", SkFixedMul(a, b) / (double)SK_Fixed1);
    }
    SkASSERT((uint32_t)b > 0x80000 || SkFixedMul(a, b) <= SK_Fixed1 * 1.2);
#endif
