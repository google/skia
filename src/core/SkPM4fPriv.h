...
static inline SkPM4f SkPM4f_from_SkColor(SkColor color, SkColorSpace* dst) {
    ...
    if (dst) {
        ...
        auto xform =
            SkColorSpaceXform::New(SkColorSpace::MakeNamed(SkColorSpace::kSRGB_Named).get(),
                                   as_CSB(dst)->makeLinearGamma().get());
        SkASSERT(xform);
        SkAssertResult(xform->apply(SkColorSpaceXform::kRGBA_F32_ColorFormat,  &color4f,
                                    SkColorSpaceXform::kBGRA_8888_ColorFormat, &color,
