...
// Append stages to convert colors in (src,srcAT) to (dst,kPremul).
static inline void transform_colorspace(SkRasterPipeline* p, SkArenaAlloc* alloc,
                                        SkColorSpace* src, SkColorSpace* dst, SkAlphaType srcAT) {
    // If either colorspace is missing, treat as sRGB.
...
// Convert color from (src,kUnpremul) to (dst,kUnpremul).
static inline SkColor4f transform_colorspace(SkColor4f color,
                                             SkColorSpace* src, SkColorSpace* dst) {
    SkSTArenaAlloc<256> alloc;
    SkRasterPipeline p(&alloc);

    SkJumper_MemoryCtx ctx = { &color, 0 };

    p.append_constant_color(&alloc, color);
    // This next kOpaque_SkAlphaType is a lie, but has the net effect of unpremul -> unpremul.
    transform_colorspace(&p, &alloc, src, dst, kOpaque_SkAlphaType);
    p.append(SkRasterPipeline::store_f32, &ctx);
    p.run(0,0,1,1);

    return color;
}

static inline SkColor4f transform_colorspace(SkColor color, SkColorSpace* dst) {
    return transform_colorspace(SkColor4f::FromColor(color),
                                SkColorSpace::MakeSRGBLinear().get(), dst);
}
