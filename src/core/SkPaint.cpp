...
SkTextBaseIter::SkTextBaseIter(const char text[], size_t length,
    ...
    if (fPaint.getPathEffect() == nullptr) {
        ...
        // be very very small. We call SkScalarDiv below to ensure IEEE divide behavior, since
        // downstream we will check for the resulting coordinates being non-finite anyway. Thus
        // we don't need to check for zero here.
