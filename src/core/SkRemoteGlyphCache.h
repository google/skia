...
private:
    class StrikeDifferences {
    public:
        StrikeDifferences(SkFontID typefaceID, const SkDescriptor* desc) noexcept;
        StrikeDifferences(StrikeDifferences&& sd) noexcept
            : fTypefaceID{sd.fTypefaceID}
            , fDesc{sd.fDesc}
            , fGlyphIDs{std::move(sd.fGlyphIDs)}  {}
        ~StrikeDifferences();
        void operator()(uint16_t glyphID, SkIPoint pos);
        SkFontID fTypefaceID;
        // This should be a unique_ptr, but there is a bug in the clang/windows toolchain.
