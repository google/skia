...
struct SkAnalyticEdge {
    ...
    SkFixed fDY;            // abs(1/fDX); may be SkFDot6Div(SK_FDot6One, 1) when fDX is close to 0.
    ...
    // Get a safe slope so we will never walk out of the boundary
    static inline SkFixed SafeDiv(SkFixed dx, SkFixed dy) {
        SkASSERT(dy >= 0);
        int sign = dx >= 0 ? 1 : -1;
        SkFDot6 dx6 = SkAbs32(dx) >> 10;
        SkFDot6 dy6 = (dy + (1 << 10) - 1) >> 10;
        SkFixed result = dy6 ? sign * QuickSkFDot6Div(dx6, dy6) : SK_MaxS32;
        SkASSERT(result == SK_MaxS32 || SkAbs32(SkFixedMul(result, dy)) <= SkAbs32(dx));
        return result;
    }
...
struct SkAnalyticQuadraticEdge : public SkAnalyticEdge {
    ...
    bool updateQuadratic(bool keepContinuous = false);
};
...
struct SkAnalyticCubicEdge : public SkAnalyticEdge {
    ...
    bool updateCubic(bool keepContinuous = false);
};
...
bool SkAnalyticEdge::setLine(const SkPoint& p0, const SkPoint& p1) {
    ...
    const int accuracy = SkAnalyticEdge::kDefaultAccuracy;
    const int multiplier = (1 << SkAnalyticEdge::kDefaultAccuracy);
    ...
    SkFDot6 dy = (y1 - y0) >> 10;
    SkFDot6 dx = (x1 - x0) >> 10;
    SkFixed slope = SkAnalyticEdge::SafeDiv(x1 - x0, y1 - y0);
    SkFixed absSlope = SkAbs32(SkFixedToFDot6(slope));
