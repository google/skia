...
DRAW(DrawEdgeAAQuad, experimental_DrawEdgeAAQuad(
        r.rect, r.clip, r.clipCount, r.aa, r.color, r.mode));
...
private:
    ...
    Bounds bounds(const DrawEdgeAAQuad& op) const {
        ...
        if (op.clipCount > 0) {
            bounds.setBounds(op.clip, op.clipCount);
        }
        return this->adjustAndMap(bounds, nullptr);
    }
    Bounds bounds(const DrawEdgeAAImageSet& op) const {
        ...
        for (int i = 0; i < op.count; ++i) {
            ...
            if (op.set[i].fDstClipCount > 0) {
                entryBounds.setBounds(op.dstClips + clipIndex, op.set[i].fDstClipCount);
                clipIndex += op.set[i].fDstClipCount;
            }
