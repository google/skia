...
private:
    ...
    void updateClipBounds(const Save&) { fClipBoundsStack.push((SkRect)fClipBoundsStack.top()); }
    void updateClipBounds(const SaveLayer&) { this->updateClipBounds(Save{}); }
    ...
    void pushNewClipBounds(SkRect localBounds) {
        SkRect devBounds;
        if (this->adjustForSaveLayerPaints(&localBounds)
                && fCTM.mapRect(&devBounds, localBounds)
                && devBounds.intersect(fClipBoundsStack.top())) {
            fClipBoundsStack.top() = devBounds;
        }
    }
