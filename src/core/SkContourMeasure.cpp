...
SkScalar SkContourMeasureFactory::compute_quad_segs(const SkPoint pts[3], SkScalar distance,
...
SkScalar SkContourMeasureFactory::compute_conic_segs(const SkConic& conic, SkScalar distance,
...
SkScalar SkContourMeasureFactory::compute_cubic_segs(const SkPoint pts[4], SkScalar distance,
                                                     int mint, int maxt, unsigned ptIndex) {
#if defined(IS_FUZZING_WITH_LIBFUZZER)
    --fSubdivisionsMax;
#endif
...
bool SkContourMeasureFactory::buildSegments() {
    SkPoint         pts[4];
    unsigned        ptIndex = 0;//fFirstPtIndex;
    SkScalar        distance = 0;
...
SkContourMeasureFactory::SkContourMeasureFactory() {
...
SkContourMeasureFactory::SkContourMeasureFactory(const SkPath& path, bool forceClosed,
                                                 SkScalar resScale) {
...
*/
void SkContourMeasureFactory::reset(const SkPath& path, bool forceClosed, SkScalar resScale) {
    if (path.isFinite()) {
        fPath = path;
    } else {
        fPath.reset();
...
sk_sp<SkContourMeasure> SkContourMeasureFactory::nextContour() {
    for (;;) {
        if (this->buildSegments()) {
            if (!SkScalarIsFinite(fLength)) {
                continue;
            }
...
#endif
            auto ptr = new SkContourMeasure(std::move(fSegments), std::move(fPts), fLength, fIsClosed);
            return sk_sp<SkContourMeasure>(ptr);
        }
    }
}
...
bool SkContourMeasure::getPosTan(SkScalar distance, SkPoint* pos, SkVector* tangent) const {
    ...
    const int      count = fSegments.count();
    SkASSERT(length > 0 && count > 0);
