...
static inline void to_xyz_d50(SkMatrix44* toXYZD50, SkColorSpace::Gamut gamut) {
    switch (gamut) {
        case SkColorSpace::kSRGB_Gamut:
            toXYZD50->set3x3RowMajorf(SkNamedGamut::kSRGB);
            break;
        case SkColorSpace::kAdobeRGB_Gamut:
            toXYZD50->set3x3RowMajorf(SkNamedGamut::kAdobeRGB);
            break;
        case SkColorSpace::kDCIP3_D65_Gamut:
            toXYZD50->set3x3RowMajorf(SkNamedGamut::kDCIP3);
            break;
        case SkColorSpace::kRec2020_Gamut:
            toXYZD50->set3x3RowMajorf(SkNamedGamut::kRec2020);
            break;
    }
}
...
static inline bool is_almost_srgb(const SkColorSpaceTransferFn& coeffs) {
    return transfer_fn_almost_equal(SkNamedTransferFn::kSRGB.fA, coeffs.fA) &&
           transfer_fn_almost_equal(SkNamedTransferFn::kSRGB.fB, coeffs.fB) &&
           transfer_fn_almost_equal(SkNamedTransferFn::kSRGB.fC, coeffs.fC) &&
           transfer_fn_almost_equal(SkNamedTransferFn::kSRGB.fD, coeffs.fD) &&
           transfer_fn_almost_equal(SkNamedTransferFn::kSRGB.fE, coeffs.fE) &&
           transfer_fn_almost_equal(SkNamedTransferFn::kSRGB.fF, coeffs.fF) &&
           transfer_fn_almost_equal(SkNamedTransferFn::kSRGB.fG, coeffs.fG);
}
