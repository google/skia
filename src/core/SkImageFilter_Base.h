...
public:
     ...
     *  an outset margin when reading pixels at the edge (to satisfy its kernel), and it expands its
     *  output to include every pixel that had some contribution from the input content bounds.
     *
     ...
     *  have an outset margin when reading pixels at the edge (to satisfy its kernel), will generate
     *  a result larger than its input (so that the blur is visible) and, this, expands its output
     *  to include every pixel that it will touch.
     *
     *  @param mapping       The coordinate space mapping that defines both the transformation
     *                       between local and layer, and layer to root device space, that will be
     *                       used when the filter is later invoked.
