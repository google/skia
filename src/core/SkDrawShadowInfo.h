...
inline SkScalar AmbientShadowBlurRadius(SkScalar height) {
    return height*kAmbientHeightFactor*kAmbientGeomFactor;
}

inline SkScalar AmbientShadowAlpha(SkScalar height) {
    return SkScalarInvert((1.0f + SkTMax(height*kAmbientHeightFactor, 0.0f)));
}

inline SkScalar SpotShadowBlurRadius(SkScalar occluderZ, SkScalar lightZ, SkScalar lightRadius) {
    return lightRadius*SkTPin(occluderZ / (lightZ - occluderZ), 0.0f, 0.95f);
}

inline void GetSpotShadowParams(SkScalar occluderZ, SkScalar lightX, SkScalar lightY,
