    ...
    while (*currVerb < verbCnt && (!allowPartial || !autoClose)) {
        ...
        switch (verb) {
            ...
            case kLine_Verb: {
                ...
                addedLine = true;
                directions[corners] = rect_make_dir(lineDelta.fX, lineDelta.fY); // 0 to 3
                if (0 == corners) {
                    lineStart = lineEnd;
                    corners = 1;
                    closedOrMoved = false;
                ...
                if (autoClose && directions[corners] == directions[0]) {
                    break; // colinear with first
                }
                closedOrMoved = autoClose;
                lineStart = lineEnd;
                if (directions[corners - 1] == directions[corners]) {
