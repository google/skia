...
struct Convexicator {
    ...
    void addPt(const SkPoint& pt) {
        if (SkPathConvexityType::kConcave == fConvexity || !fIsFinite) {
            return;
        }
        ...
        if (0 == fPtCount) {
            ...
            if (!SkScalarIsFinite(lengthSqd)) {
                ...
                if (fDx > 3 || fDy > 3) {
                    fConvexity = SkPathConvexityType::kConcave;
                }
            }
        }
    }
...
SkPathConvexityType SkPath::internalGetConvexity() const {
    ...
    SkPathConvexityType c = this->getConvexityOrUnknown();
    if (c != SkPathConvexityType::kUnknown) {
        return c;
    }
