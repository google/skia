...
SkSPath SkPath::peek() const {
    return fPathRef->peek(this->getFillType(), this->getConvexityTypeOrUnknown());
}
...
bool SkPath::isRect(SkRect* rect, bool* isClosed, SkPathDirection* direction) const {
    ...
    int currVerb = 0;
    const SkPoint* pts = fPathRef->points();
    return SkPathPriv::IsRectContour(this->peek(), false, &currVerb, &pts, isClosed, direction, rect);
}
...
bool SkPathPriv::IsRectContour(const SkSPath& path, bool allowPartial, int* currVerb,
...
bool SkPathPriv::IsNestedFillRects(const SkSPath& path, SkRect rects[2], SkPathDirection dirs[2]) {
    int currVerb = 0;
