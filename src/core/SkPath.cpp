...
size_t SkPath::readFromMemory(const void* storage, size_t length) {
    ...
    // path. If these values are corrupted path may cause the GPU backend to make incorrect
    // rendering choices that lead to a crash. We set them to unknown so that they'll be recomputed
    // if requested.
    fConvexity = kUnknown_Convexity;
    fFirstDirection = SkPathPriv::kUnknown_FirstDirection;
    fFillType = fillType;
