...
bool SkPathPriv::DrawArcIsConvex(SkScalar sweepAngle, bool useCenter, bool isFillNoPathEffect) {
    if (useCenter) {
        return sweepAngle >= -180.f && sweepAngle <= 180.f;
    } else if (isFillNoPathEffect) {
        return true;
    }
    return (sweepAngle >= -360.f && sweepAngle <= 360.f) ||
           SkScalarNearlyZero(SkScalarMod(sweepAngle, 360.f));
}
...
void SkPathPriv::CreateDrawArcPath(SkPath* path, const SkRect& oval, SkScalar startAngle,
    ...
    auto firstDir = sweepAngle > 0 ? SkPathPriv::kCW_FirstDirection
                             : SkPathPriv::kCCW_FirstDirection;
