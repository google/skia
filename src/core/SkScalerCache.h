...
// SkGlyphDigest collects several attributes of a glyph, so looking up the glyph is not needed.
// The value stored in fDigestForPackedGlyphID that is the index into fGlyphForIndex.
class SkGlyphDigest {
public:
...
private:
    ...
    // reference to the SkGlyph as the strike is alive, and fGlyphForIndex
