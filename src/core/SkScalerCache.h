...
private:
    ...
    // an unchanging SkGlyph pointer as long as the strike is alive, and fIndex provides a dense
    // index for glyphs.
    SkTHashMap<SkPackedGlyphID, SkGlyphIndex> fIndexMap SK_GUARDED_BY(fMu);
    std::vector<SkGlyph*> fIndex SK_GUARDED_BY(fMu);
