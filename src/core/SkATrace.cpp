...
decltype(SkATrace::fBeginSection) SkATrace::fBeginSection = nullptr;
decltype(SkATrace::fEndSection) SkATrace::fEndSection     = nullptr;
decltype(SkATrace::fIsEnabled) SkATrace::fIsEnabled       = []{ return false; };
...
const uint8_t* SkATrace::getCategoryGroupEnabled(const char* name) {
    ...
    // we can't use this to do a check for ATrace isEnabled(). Thus we wil always return yes here
