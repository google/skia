    ...
    while (sigma > MAX_BLUR_SIGMA) {
        *scaleFactor *= 2;
        ...
        if (*scaleFactor >= scaleFactorLimit) {
            *scaleFactor *= 0.5f;
            // sigma should be doubled here to undo the halving of this iteration.
            // However, doubling sigma here guarantees that it will be larger than
            // MAX_BLUR_SIGMA, so we just set it to MAX_BLUR_SIGMA instead.
...
static sk_sp<GrRenderTargetContext> reexpand(GrRecordingContext* context,
                                             ...
                                             const SkIPoint& offsetAdjustment,
                                             const SkIRect& rectAdjustment,
...
sk_sp<GrRenderTargetContext> GaussianBlur(GrRecordingContext* context,
    ...
    int scaleFactorX, radiusX, scaleFactorXLimit = srcBounds.width();
    int scaleFactorY, radiusY, scaleFactorYLimit = srcBounds.height();
