...
std::unique_ptr<GrRenderTargetContext> GaussianBlur(GrRecordingContext* context,
    ...
    if (mode != SkTileMode::kDecal) {
        // All non-decal tile modes are equivalent for one pixel width/height src and are amount to
        // a single color value repeated at each column/row. Applying the normalized kernel to that
