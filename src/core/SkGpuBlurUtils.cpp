...
static std::unique_ptr<GrSurfaceFillContext> convolve_gaussian_2d(GrRecordingContext* context,
    ...
    auto surfaceFillContext = GrSurfaceFillContext::Make(context,
                                                         {std::move(srcInfo), dstBounds.size()},
                                                         dstFit,
                                                         1,
                                                         GrMipmapped::kNo,
...
std::unique_ptr<GrSurfaceFillContext> GaussianBlur(GrRecordingContext* context,
    ...
    if (!radiusX && !radiusY) {
        GrImageInfo resultII(srcInfo, dstBounds.size());
    ...
    auto srcCtx = GrSurfaceContext::Make(context, srcView, srcInfo);
    SkASSERT(srcCtx);
    ...
    GrImageInfo rescaledII(srcInfo, {rescaledSize.width() + 2*pad, rescaledSize.height() + 2*pad});
