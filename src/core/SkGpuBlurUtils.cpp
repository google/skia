...
static void fill_in_2D_gaussian_kernel(float* kernel, int width, int height,
    ...
    SkASSERT(sigmaX > SkGpuBlurUtils::kEffectivelyZeroSigma &&
             sigmaY > SkGpuBlurUtils::kEffectivelyZeroSigma);
...
static void convolve_gaussian_1d(GrRenderTargetContext* renderTargetContext,
                                 ...
                                 float sigma,
                                 SkTileMode mode) {
    SkASSERT(radius && sigma > SkGpuBlurUtils::kEffectivelyZeroSigma);
    GrPaint paint;
...
static std::unique_ptr<GrRenderTargetContext> convolve_gaussian(GrRecordingContext* context,
    ...
    SkASSERT(radius > 0 && sigma > SkGpuBlurUtils::kEffectivelyZeroSigma);
