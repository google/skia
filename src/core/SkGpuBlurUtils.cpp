...
static std::unique_ptr<GrRenderTargetContext> convolve_gaussian(GrRecordingContext* context,
    ...
    if (Direction::kX == direction) {
        ...
        *contentRect = {dstBounds.left(), std::max(contentRect->top(), dstBounds.top()), dstBounds.right(), std::min(dstBounds.bottom(), contentRect->bottom())};
    } else {
        ...
        *contentRect = {std::max(contentRect->left(), dstBounds.left()), dstBounds.top(), std::min(contentRect->right(), dstBounds.right()), dstBounds.bottom()};
    }
