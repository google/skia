...
sk_sp<GrRenderTargetContext> GaussianBlur(GrContext* context,
    ...
    for (int i = 1; i < scaleFactorX || i < scaleFactorY; i *= 2) {
        ...
        SkASSERT(dstRect.fLeft <= width && dstRect.fBottom <= height);
        dstRenderTargetContext = context->makeDeferredRenderTargetContext(fit, dstRect.fLeft,
                                                                          dstRect.fBottom,
                                                                          config, colorSpace);
        if (!dstRenderTargetContext) {
            return nullptr;
    ...
    if (scaleFactorX > 1 || scaleFactorY > 1) {
        ...
        dstRenderTargetContext = context->makeDeferredRenderTargetContext(fit,
                                                                          SkTMin(dstRect.width(), width),
