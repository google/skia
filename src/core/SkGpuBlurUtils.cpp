...
static void convolve_gaussian_1d(GrRenderTargetContext* renderTargetContext,
    ...
    std::unique_ptr<GrFragmentProcessor> conv(GrGaussianConvolutionFragmentProcessor::Make(
            std::move(srcView), srcAlphaType, direction, radius, sigma, wm, srcBounds, &srcRect,
...
static std::unique_ptr<GrRenderTargetContext> convolve_gaussian(GrRecordingContext* context,
    ...
    // We've implemented splitting the dst bounds up into ares that do and do not need to
    // use shader based tiling but only for some modes...
    bool canSplit = mode == SkTileMode::kDecal || mode == SkTileMode::kClamp;
    // ...but it's not worth using the splitting if we'll get HW tiling instead of shader tiling.
    bool canHWTile =
    ...
    if (Direction::kX == direction) {
        ...
        int midB = std::max(srcBounds.bottom(), dstBounds.bottom());
        ...
        int midB = std::max(srcBounds.right(), dstBounds.right());
...
static std::unique_ptr<GrRenderTargetContext> two_pass_gaussian(GrRecordingContext* context,
    ...
    if (sigmaX > 0.0f) {
        ...
        // correctly. If we're not going to do a y-pass then we must use the original dstBounds.
        ...
        dstBounds = SkIRect::MakeSize(dstBounds.size()).makeOffset(0, dstBounds.top() - dstTop);
