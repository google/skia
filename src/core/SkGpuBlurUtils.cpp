...
// srcBounds and srcOffset, which are relative to the logical source image, whereas proxyOffset is
// the coordinates of the logical source's origin in the source proxy. Due to how the destination
// bounds are calculated, it is more convenient to have the proxy offset kept separate from the
// logical bounds (which do impact destination decisions). Both functions incorporate the proxy
// offset into the geometry they submit or before calling convolve_gaussian_1d.
...
sk_sp<GrRenderTargetContext> GaussianBlur(GrRecordingContext* context,
    ...
    if (sigmaX > 0.0f && sigmaY > 0.0f &&
        ...
        return convolve_gaussian_2d(context, std::move(srcProxy), localSrcBounds, srcOffset, radiusX,
