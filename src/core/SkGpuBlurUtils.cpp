...
static sk_sp<GrRenderTargetContext> convolve_gaussian(GrContext* context,
    ...
    if (!bottomRect.isEmpty()) {
        dstRenderTargetContext->clear(&bottomRect, 0, GrRenderTargetContext::CanClearFullscreen::kNo);
    }
...
static sk_sp<GrRenderTargetContext> reexpand(GrContext* context,
    ...
    GrFixedClip clip(SkIRect::MakeWH(dstII.width(), dstII.height()));
...
sk_sp<GrRenderTargetContext> GaussianBlur(GrContext* context,
    ...
    if (scaleFactorX > 1 || scaleFactorY > 1) {
        srcProxy = decimate(context, std::move(srcProxy), &srcRect, &srcOffset,
