...
static std::unique_ptr<GrRenderTargetContext> convolve_gaussian(GrRecordingContext* context,
    ...
    if (midRect.isEmpty()) {
        ...
        convolve_gaussian_1d(dstRenderTargetContext.get(), GrNoClip(), leftRect, netOffset, srcProxy,
                             srcAlphaType, direction, radius, sigma, mode, bounds);
        convolve_gaussian_1d(dstRenderTargetContext.get(), GrNoClip(), rightRect, netOffset, srcProxy,
                             srcAlphaType, direction, radius, sigma, mode, bounds);
    ...
    if (sigmaX > 0.0f) {
        dstRenderTargetContext =
                convolve_gaussian(context, std::move(srcProxy), srcColorType, srcAlphaType,
                                  ...
                                  srcBounds, mode, colorSpace, xFit);
        if (!dstRenderTargetContext) {
            return nullptr;
        }
        srcProxy = dstRenderTargetContext->asTextureProxyRef();
    ...
    if (sigmaY > 0.0f) {
        dstRenderTargetContext =
                convolve_gaussian(context, std::move(srcProxy), srcColorType, srcAlphaType,
                                  ...
                                  srcBounds, mode, colorSpace, yFit);
    }
