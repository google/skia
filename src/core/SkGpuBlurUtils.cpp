...
static void convolve_gaussian_1d(GrSurfaceFillContext* sfc,
    ...
    auto srcRect = rtcRect.makeOffset(dstToSrcOffset);
    ...
    std::unique_ptr<GrFragmentProcessor> conv = GrGaussianConvolutionFragmentProcessor::Make(
            ...
            *sfc->caps());
    sfc->fillRectToRectWithFP(srcRect, rtcRect, std::move(conv));
}
...
std::unique_ptr<GrSurfaceDrawContext> GaussianBlur(GrRecordingContext* context,
    ...
    // border).
    // Moreover, if we clamped the rescaled size to 1 column or row about then we still have a sigma
    ...
    auto srcRect = SkIRect::MakeSize(srcView.dimensions());
