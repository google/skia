...
std::unique_ptr<SkColorSpaceXform> SkColorSpaceXform::New(SkColorSpace* srcSpace,
    ...
    return SkColorSpaceXform_Base::New(srcSpace, dstSpace, true);
}
    ...
    switch (csm) {
        case kNone_ColorSpaceMatch:
            return std::unique_ptr<SkColorSpaceXform>(new SkColorSpaceXform_XYZ
                    <kNone_ColorSpaceMatch>(srcSpaceXYZ, srcToDst, dstSpaceXYZ, linearPremul));
        case kGamut_ColorSpaceMatch:
            return std::unique_ptr<SkColorSpaceXform>(new SkColorSpaceXform_XYZ
                    <kGamut_ColorSpaceMatch>(srcSpaceXYZ, srcToDst, dstSpaceXYZ, linearPremul));
        case kFull_ColorSpaceMatch:
            return std::unique_ptr<SkColorSpaceXform>(new SkColorSpaceXform_XYZ
                    <kFull_ColorSpaceMatch>(srcSpaceXYZ, srcToDst, dstSpaceXYZ, linearPremul));
        default:
            SkASSERT(false);
            return nullptr;
    }
...
::SkColorSpaceXform_XYZ(SkColorSpace_XYZ* srcSpace, const SkMatrix44& srcToDst,
                        SkColorSpace_XYZ* dstSpace, bool linearPremul)
    : fLinearPremul(linearPremul)
{
...
::applyPipeline(ColorFormat dstColorFormat, void* dst, ColorFormat srcColorFormat,
    ...
    if (kPremul_SkAlphaType == alphaType && fLinearPremul) {
...
std::unique_ptr<SkColorSpaceXform> SlowIdentityXform(SkColorSpace_XYZ* space) {
    return std::unique_ptr<SkColorSpaceXform>(new SkColorSpaceXform_XYZ
            <kNone_ColorSpaceMatch>(space, SkMatrix::I(), space, true));
}
