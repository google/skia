...
::SkColorSpaceXform_XYZ(SkColorSpace_XYZ* srcSpace, const SkMatrix44& srcToDst,
    ...
    : fLinearPremul(!dstSpace->nonLinearBlending())
{
...
::applyPipeline(ColorFormat dstColorFormat, void* dst, ColorFormat srcColorFormat,
    ...
    bool legacyPremul = kPremul_SkAlphaType == alphaType && !fLinearPremul;
    switch (dstColorFormat) {
        case kRGBA_8888_ColorFormat:
            if (kTable_DstGamma == fDstGamma) {
                ...
                if (legacyPremul) {
                    pipeline.append(SkRasterPipeline::store_tables_premul, &storeTables);
                } else {
        ...
        case kBGRA_8888_ColorFormat:
            if (kTable_DstGamma == fDstGamma) {
                ...
                storeTables.fCount = SkColorSpaceXform_Base::kDstGammaTableSize;
                storeTables.fLegacyPremul = legacyPremul;
                pipeline.append(SkRasterPipeline::swap_rb);
            ...
            break;
        case kRGBA_F16_ColorFormat:
            if (kLinear_DstGamma != fDstGamma || !fLinearPremul) {
                return false;
            }
