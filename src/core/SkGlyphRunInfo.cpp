...
SkGlyphRunInfo SkGlyphRunInfo::Make(const SkPaint& paint, const void* bytes, size_t byteLength) {
    ...
    if (encoding == SkPaint::kGlyphID_TextEncoding) {
        ...
        return SkGlyphRunInfo::Make(
            byteLength, bytes, *typeface, convert_encoding(encoding));
    }
}
...
SkGlyphRunInfo SkGlyphRunInfo::Make(uint16_t count, const SkGlyphID* glyphs) {
    ...
    auto denseIndex = skstd::make_unique_default<DenseIndex[]>(count);
...
SkGlyphRunInfo SkGlyphRunInfo::Make(size_t byteLength, const void* utfN,
    ...
    if (count > 0) {
        ...
        return SkGlyphRunInfo{SkTo<uint16_t>(count), std::move(denseIndex), std::move(glyphs)};
    }

    return SkGlyphRunInfo{};
}
