...
void SkRecorder::onDrawImageSet(const ImageSetEntry set[], int cnt, float alpha,
                                SkFilterQuality filterQuality, SkBlendMode mode) {
    SkAutoTArray<ImageSetEntry> setCopy(cnt);
    for (int i = 0; i < cnt; ++i) {
        setCopy[i] = set[i];
    }
    this->append<SkRecords::DrawImageSet>(std::move(setCopy), cnt, alpha, filterQuality, mode);
}
