...
public:
    ...
    // skif::ParameterSpace<> values are interpeted anew. Layer space and device space are
    // unchanged.
    void concatLocal(const SkMatrix& local) { fParamToLayerMatrix.preConcat(local); }

    // Update the mapping's layer-to-device matrix to be post-concatenated with the specified
    // device space transformation. Since this changes the definition of device space, any
    // skif::DeviceSpace<> values must be updated accordingly. Layer space and parameter space are
    // unchanged.
    void postConcatDevice(const SkMatrix& device) { fLayerToDevMatrix.postConcat(device); }
