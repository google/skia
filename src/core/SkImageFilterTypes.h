...
//   skif::Image<skif::Usage::kInput> -> skif::Image<For::kInput>
// FIXME The Cast function needs skif:: prefix since it's a template, not a full function yet.
// Would be nice to have a CPP-only macro that adds a using namespace skif; so that can go away.
...
public:
    ...
    Context withLayerMatrix(const SkMatrix& ctm) const {
        return Context(ctm, fTargetOutput, fCache, fColorType, fColorSpace, fSource);
    }
    // Create a new context that matches this context, but with an overridden clip bounds rect.
    Context withTargetOutput(const IRect<In::kLayer, For::kOutput>& bounds) const {
        return Context(fLayerMatrix, bounds, fCache, fColorType, fColorSpace, fSource);
    }
    // DEPRECATED: use withTargetOutput() instead
