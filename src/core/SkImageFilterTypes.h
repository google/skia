...
#define SK_USE_FLUENT_IMAGE_FILTER_TYPES \
    ...
    using From = skif::CoordSpace;       \
    using To = skif::CoordSpace;         \
...
// Template-template MACRO that can represent any of the 6 skif::Geometry wrappers
#define SKIF_GEOMETRY_TYPE template<CoordSpace, Usage> class Geometry
...
public:
    ...
    void offset(const Vector<kCS, kU>& v) { fData.offset(SksPoint(v)); }
...
// Internal-use only: the math for transforming coordinates w/o template baggage.
// NOTE: Unlike the later template-typed Transform class, the 6 map functions have distinct names
// since the SkPoint/SkVector typedef prevents regular overloading from being used.
struct RawMatrixTransform {
public:
    using SkType = SkMatrix;

    explicit RawMatrixTransform(const SkMatrix& matrix) : fMatrix(matrix) {}

    SkIRect mapIRect(const SkIRect& geom) {
        return fMatrix.mapRect(SkRect::Make(geom)).roundOut();
    }

    SkRect mapRect(const SkRect& geom) {
        return fMatrix.mapRect(geom);
    }

    SkIPoint mapIPoint(const SkIPoint& geom) {
    ...
    SkPoint mapPoint(const SkPoint& geom) {
        SkPoint p;
        fMatrix.mapPoints(&p, &geom, 1);
        return p;
    }

    SkIVector mapIVector(const SkIVector& geom) {
        SkVector v = SkVector::Make(SkIntToScalar(geom.fX), SkIntToScalar(geom.fY));
        fMatrix.mapVectors(&v, 1);
        return SkIVector::Make(SkScalarRoundToInt(v.fX), SkScalarRoundToInt(v.fY));
    }

    SkVector mapVector(const SkVector& geom) {
        SkVector v;
        fMatrix.mapVectors(&v, &geom, 1);
        return v;
    }
...
struct RawTranslateTransform {
public:
    using SkType = SkIPoint;

    explicit RawTranslateTransform(const SkIPoint& translation) : fTranslation(translation) {}

    SkIRect mapIRect(const SkIRect& geom, const SkIPoint& translation) {
        return geom.makeOffset(fTranslation.fX, fTranslation.fY);
    }

    SkRect mapRect(const SkRect& geom, const SkIPoint& translation) {
        return geom.makeOffset(SkIntToScalar(fTranslation.fX), SkIntToScalar(fTranslation.fY));
    }

    SkIPoint mapIPoint(const SkIPoint& geom, const SkIPoint& translation) {
        return geom + fTranslation;
    }

    SkPoint mapPoint(const SkPoint& geom, const SkIPoint& translation) {
        return geom + SkPoint::Make(SkIntToScalar(fTranslation.fX), SkIntToScalar(fTranslation.fY));
    }

    SkIVector mapIVector(const SkIVector& geom, const SkIPoint& translation) {
        // Vector is direction and magnitude only, so if the coordinate space transformation is
        // described only by a translation, then the vector is unmodified.
        return geom;
    }

    SkVector mapVector(const SkVector& geom, const SkIPoint& translation) {
        // See mapIVector
        return geom;
    }

private:
    SkIPoint fTranslation;
};

// Internal-use only: wraps one of the RawXTransform types and uses function overloading to
// automatically transform each of the template-tagged skif:: geometry types.
template<CoordSpace kIn, CoordSpace kOut, Usage kU,
         typename RawTransform, typename T = typename RawTransform::SkType>
struct TypedTransform {
public:
    IRect<kOut, kU> apply(const IRect<kIn, kU>& geom) {
        return IRect<kOut, kU>(fTransform.mapIRect(SkIRect(geom)));
    }

    Rect<kOut, kU> apply(const Rect<kIn, kU>& geom) {
        return Rect<kOut, kU>(fTransform.mapRect(SkRect(geom)));
    }

    IPoint<kOut, kU> apply(const IPoint<kIn, kU>& geom) {
        return IPoint<kOut, kU>(fTransform.mapIPoint(SkIPoint(geom)));
    }

    Point<kOut, kU> apply(const Point<kIn, kU>& geom) {
        return Point<kOut, kU>(fTransform.mapPoint(SkPoint(geom)));
    }

    IVector<kOut, kU> apply(const IVector<kIn, kU>& geom) {
        return IVector<kOut, kU>(fTransform.mapIVector(SkIVector(geom)));
    }

    Vector<kOut, kU> apply(const Vector<kIn, kU>& geom) {
        return Vector<kOut, kU>(fTransform.mapVector(SkVector(geom)));
    }

protected:
    // Transforms shouldn't be directly instantiated, use the Mapping subclass which connects the
    // appropriate type of transform to each valid pair of coordinate spaces.
    TypedTransform(const T& mapper) : fTransform(mapper) {}

    // The mapping parameters that control the exact coordinate transformation, e.g. the matrix
    RawTransform fTransform;
};

// For general-use: describes a type-safe transformation from one coordinate space to another.
// Only valid in/out transformations define apply() functions for the skif:: geometry types.
// Depending on the in/out combination, the transformation is either specified by a translation or
// a matrix transformation. The Mapping struct can be created once and reused if the same
// transformation will be made multiple times, otherwise static functions are provided to represent
// the same set of coordinate space conversions but can automatically infer template arguments.
template<CoordSpace kIn, CoordSpace kOut, Usage kU>
struct Mapping {};

#define DEF_MATRIX_MAPPING(in, out, matrixName)                                     \
    template<Usage kU>                                                              \
    struct Mapping<CoordSpace::k##in, CoordSpace::k##out, kU>                       \
            : public TypedTransform<CoordSpace::k##in, CoordSpace::k##out,          \
                                    kU, RawMatrixTransform> {                       \
    public:                                                                         \
        Mapping(const SkMatrix& matrixName)                                         \
                : TypedTransform<CoordSpace::k##in, CoordSpace::k##out,             \
                                 kU, RawMatrixTransform>(matrixName) {}             \
    };

#define DEF_OFFSET_MAPPING(in, out, originSpace, op)                                \
    template<Usage kU>                                                              \
    struct Mapping<CoordSpace::k##in, CoordSpace::k##out, kU>                       \
            : public TypedTransform<CoordSpace::k##in, CoordSpace::k##out,          \
                                    kU, RawTranslateTransform> {                    \
    public:                                                                         \
        Mapping(const IPoint<CoordSpace::k##originSpace, kU>& origin)               \
                : TypedTransform<CoordSpace::k##in, CoordSpace::k##out,             \
                                 kU, RawTranslateTransform>(op SkIPoint(origin)) {} \
    };

// Map from parameter space to layer space. This is meant to wrap the Context::layerMatrix() value.
DEF_MATRIX_MAPPING(Parameter, Layer, layerMatrix)
// Map from layer space to the final device space. This is meant to wrap the remainder matrix from
// the decomposition of the CTM = R * S.
DEF_MATRIX_MAPPING(Layer, Device, remainderMatrix)
// The inverse from device space to layer space. This is meant to wrap the inverse of the
// remainder matrix. It does NOT calculate R^-1, as it is presumably already available in SkCanvas.
DEF_MATRIX_MAPPING(Device, Layer, remainderMatrixInv)

// Map from a specific image's (determined by kU) to the shared layer space. The image's origin
// is specified in kLayer space.
DEF_OFFSET_MAPPING(Image, Layer, Layer,)
// Map from the shared layer space to a specific image's space (determined by kU). The origin is
// specified in kLayer space.
DEF_OFFSET_MAPPING(Layer, Image, Layer, -)
// From a specific image's coordinate space to its backing proxy space; most useful for GPU filter
// implementations that must access the texture directly. The origin is specified in kProxy space.
DEF_OFFSET_MAPPING(Image, Proxy, Proxy,);

#undef DEF_OFFSET_MAPPING
#undef DEF_MATRIX_MAPPING
...
template<Usage kUOut, Usage kUIn, SKIF_GEOMETRY_TYPE>
...
template<Usage kU, SKIF_GEOMETRY_TYPE>
Geometry<CoordSpace::kLayer, kU> ParameterToLayer(
        const Geometry<CoordSpace::kParameter, Usage::kInherit>& geom,
        const SkMatrix& layerMatrix) {
    Mapping<CoordSpace::kParameter, CoordSpace::kLayer, Usage::kInherit> mapper(layerMatrix);
    return LayerCast<kU>(mapper.apply(geom));
}
...
template<Usage kU, SKIF_GEOMETRY_TYPE>
Geometry<CoordSpace::kDevice, Usage::kInherit> LayerToDevice(
        const Geometry<CoordSpace::kLayer, kU>& geom,
        const SkMatrix& remainderMatrix) {
    Mapping<CoordSpace::kLayer, CoordSpace::kDevice, Usage::kInherit> mapper(remainderMatrix);
    return mapper.apply(LayerCast<Usage::kInherit>(geom));
}
...
template<Usage kU, SKIF_GEOMETRY_TYPE>
Geometry<CoordSpace::kLayer, kU> DeviceToLayer(
        const Geometry<CoordSpace::kDevice, Usage::kInherit>& geom,
        const SkMatrix& remainderMatrix) {
    Mapping<CoordSpace::kDevice, CoordSpace::kLayer, Usage::kInherit> mapper(remainderMatrix);
    return LayerCast<kU>(mapper.apply(geom));
}
...
Geometry<CoordSpace::kImage, kU> LayerToImage(const Geometry<CoordSpace::kLayer, kU>& geom,
    ...
    Mapping<CoordSpace::kLayer, CoordSpace::kImage, kU> mapper(origin);
    return mapper.apply(geom);
}
...
Geometry<CoordSpace::kLayer, kU> ImageToLayer(const Geometry<CoordSpace::kImage, kU>& geom,
                                              const IPoint<CoordSpace::kLayer, kU>& origin) {
    Mapping<CoordSpace::kImage, CoordSpace::kLayer, kU> mapper(origin);
    return mapper.apply(geom);
}
...
Geometry<CoordSpace::kProxy, kU> ImageToProxy(const Geometry<CoordSpace::kImage, kU>& geom,
    ...
    Mapping<CoordSpace::kImage, CoordSpace::kProxy, kU> mapper(origin);
    return mapper.apply(geom);
}
...
class FilterResult {
public:
    static_assert(kU != Usage::kInherit, "FilterResults cannot use the kInherit usage");
    SK_USE_FLUENT_IMAGE_FILTER_TYPES_IN_CLASS

    FilterResult() : fImage(nullptr), fOrigin(SkIPoint::Make(0, 0)) {}

    FilterResult(sk_sp<SkSpecialImage> image, const IPoint<In::kLayer, kU>& origin)
...
public:
    ...
    Context(const SkMatrix& layerMatrix, const IRect<In::kLayer, For::kOutput>& targetOutput,
