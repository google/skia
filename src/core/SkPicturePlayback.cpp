...
#define BREAK_ON_READ_ERROR(r)  \
    ...
    switch (op) {
        ...
        case DRAW_ATLAS: {
            ...
            int filterInt = flags >> 2;
            if (filterInt == 1 || filterInt == 2) {
                SkFilterMode filter = (SkFilterMode)(filterInt - 1);
                canvas->drawAtlas(atlas, xform, tex, colors, count, mode, filter, cull, paint);
            } else {
                // no explicit filtermode, deduce from paint's filterquality
