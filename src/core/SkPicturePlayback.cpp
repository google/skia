...
#define BREAK_ON_READ_ERROR(r) if (!r->isValid()) { break; }
    ...
    switch (op) {
        ...
        case DRAW_VERTICES: {
            ...
            if (paint) {
                auto vertices = SkVertices::MakeCopy(vmode, vCount, verts, texs, colors,
                                                     iCount, indices);
                if (vertices) {
                    canvas->drawVertices(std::move(vertices), bmode, *paint, flags);
                }
            }
        } break;
        case DRAW_VERTICES_OBJECT: {
            ...
            if (paint && vertices) {
                canvas->drawVertices(vertices, bmode, *paint, flags);
            }
        } break;
        case RESTORE:
            canvas->restore();
