...
sk_sp<SkSpecialSurface> SkSpecialSurface::MakeRenderTarget1(GrRecordingContext* rContext,
 ...
 //                                                          int width, int height,
   //                                                        GrColorType colorType,
    ...
    auto sdc = GrSurfaceDrawContext::Make(
            rContext, colorType, std::move(colorSpace), /**/ SkBackingFit::kApprox, {width, height},
    ...
    auto device = SkGpuDevice::Make1(std::move(sdc),
                                    SkGpuDevice::kUninit_InitContents);
#else
    auto device = SkGpuDevice::Make2(rContext, SkBudgeted::kYes, ii, 1, kBottomLeft_GrSurfaceOrigin,
                                     &props, GrMipmapped::kNo, GrProtected::kNo,
                                     SkGpuDevice::kUninit_InitContents);
#endif
    if (!device) {
        return nullptr;
