...
public:
    SkSpecialSurface_Gpu(GrRecordingContext* context,
                         std::unique_ptr<GrRenderTargetContext> renderTargetContext, int width,
                         int height, const SkIRect& subset)
        ...
        auto device = SkGpuDevice::Make(context->priv().backdoor(), std::move(renderTargetContext),
                                        width, height, SkGpuDevice::kUninit_InitContents);
        if (!device) {
            return;
        }
    ...
    sk_sp<SkSpecialImage> onMakeImageSnapshot() override {
        ...
        sk_sp<SkSpecialImage> tmp(
                SkSpecialImage::MakeDeferredFromGpu(fCanvas->getGrContext(),
                                                    this->subset(),
