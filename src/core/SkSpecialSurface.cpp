...
public:
    SkSpecialSurface_Gpu(std::unique_ptr<GrSurfaceDrawContext> surfaceDrawContext,
                         int width,
                         int height,
                         const SkIRect& subset)
...
sk_sp<SkSpecialSurface> SkSpecialSurface::MakeRenderTarget(GrRecordingContext* context,
    ...
    return sk_make_sp<SkSpecialSurface_Gpu>(std::move(surfaceDrawContext), width, height, subset);
}
