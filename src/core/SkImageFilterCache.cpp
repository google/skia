...
public:
    ...
    struct Value {
        Value(const Key& key, const skif::Image<For::kOutput> image, const SkImageFilter* filter)
            : fKey(key), fImage(image), fFilter(filter) {}

        Key fKey;
        skif::Image<For::kOutput> fImage;
    ...
    bool get(const Key& key, skif::Image<For::kOutput>* result) const override {
        SkASSERT(result);
    ...
    void set(const Key& key, const SkImageFilter* filter,
             const skif::Image<For::kOutput>& result) override {
        SkAutoMutexExclusive mutex(fMutex);
        if (Value* v = fLookup.find(key)) {
