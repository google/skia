...
public:
    ...
    void purgeByKey(const Key& key) override {
        SkAutoMutexAcquire mutex(fMutex);
        if (Value* v = fLookup.find(key)) {
            // This funciton is only called in the destructor of SkImageFilter,
            // within the foreach loop. Since foreach does not allow mutate,
            // we make the filter to be null so that it won't call
            // SkImageFilter::removeInternal() function.
            v->fFilter = nullptr;
            this->removeInternal(v);
        }
    }
