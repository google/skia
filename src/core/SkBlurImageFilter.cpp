...
// GaussBlur implements the common three pass box filter approximation of Gaussian blur, but
// combines all three passes into a single pass. This approach is facilitated by three circular
...
// are not rounded each pass. In cropped situations, the previous three individual pass
// implementation created edges that were too "dark". It created a temporary buffer that was
// the size of the dst. Because of this the values that blur out from the dst after each pass
// are not stored, and therefore counted as zeros in the next pass.
//
//  In general, a window sum has the form:
//     sum_n+1 = sum_n + leading_edge - trailing_edge.
//  If instead we do the subtraction at the end of the previous iteration, allows to just
...
public:
    ...
    static int CalculateWindow(double sigma);

    // Calculate the size in number of Sk4u for the three sum buffers.
    static int CalculateBufferSize(int window);
...
int GaussBlur::CalculateBufferSize(int window) {
...
int GaussBlur::CalculateWindow(double sigma) {
...
static sk_sp<SkSpecialImage> combined_pass_blur(
    ...
    uint32_t windowW = GaussBlur::CalculateWindow(sigma.x()),
             windowH = GaussBlur::CalculateWindow(sigma.y());
...
#else
    uint32_t borderW = GaussBlur::CalculateBorder(GaussBlur::CalculateWindow(fSigma.width())),
             borderH = GaussBlur::CalculateBorder(GaussBlur::CalculateWindow(fSigma.height()));
