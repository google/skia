...
// buffers the width of the window which track values for trailing edges of each of the tree passes.
// This allows the algorithm to use more precision in the calculation because the values are not
// rounded each pass. In addition, it does not suffer from virtually zeroing the edges when the
// filter is cropped in an image filter stack like three separate passes does.
//
//  In general, a window sum has the form:
//     sum_n+1 = sum_n + leading_edge - trailing_edge.
//  If instead we do the subtraction in the previous loop then we can just calculate sum as:
...
sk_sp<SkSpecialImage> SkBlurImageFilterImpl::onFilterImage(SkSpecialImage* source,
    ...
    #endif
    {
            ...
            static constexpr double kCrossTooSmall = 0.26596152026762;
