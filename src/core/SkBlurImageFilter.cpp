...
protected:
    ...
    SkIRect onFilterNodeBounds(const SkIRect& src, const SkMatrix& ctm, const SkIRect* inputRect, MapDirection) const override;
...
sk_sp<SkSpecialImage> SkBlurImageFilterImpl::onFilterImage(SkSpecialImage* source,
    ...
    SkIRect dstBounds;
    if (!this->applyCropRect(this->mapContext(ctx, inputBounds), inputBounds, &dstBounds)) {
        return nullptr;
    }
