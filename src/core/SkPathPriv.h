...
public:
    ...
    */
    ...
    static bool IsRectContour(const SkSPath&, bool allowPartial, int* currVerb,
     ...
     */
    static bool IsNestedFillRects(const SkSPath&, SkRect rect[2],
                                  SkPathDirection dirs[2] = nullptr);

    static bool IsNestedFillRects(const SkPath& path, SkRect rect[2],
                                  SkPathDirection dirs[2] = nullptr) {
        return IsNestedFillRects(path.peek(), rect, dirs);
    }
