...
// Iterates over each contour in a path. Example usage:
//
//   SkTPathContoursIter contour;
//   while (contour.next()) {  // Call next() first to load the path's initial contour.
...
//   }
//
// A caller may specialize this class to define their own resetGeometry() and geometryTo() methods.
// If no special treatment of endpoints is necessary, then use SkPathContoursIter below.
template<typename TSubclass> class SkTPathContoursIter {
public:
    SkTPathContoursIter(const SkPath& path)
            : fPath(path)
    ...
    uint8_t atVerb(int i) const {
        SkASSERT(i >= 0 && i <= fVerbsIdx);
        uint8_t verb = (i < fVerbsIdx) ? fVerbs[i] : SkPath::kDone_Verb;
    ...
    // Advances to the next contour in the path. Returns false if there are no more contours.
    //
    //   SkTPathContoursIter contour;
    //   while (contour.next()) {  // Call next() first to load the path's initial contour.
    //       ...
        ...
        while (fVerbsIdx < fNumRemainingVerbs) {
            switch (uint8_t verb = fVerbs[fVerbsIdx]) {
                ...
                static_assert(SkPath::kCubic_Verb == 4); case 4:
                    ...
                    static_cast<TSubclass*>(this)->geometryTo(fPoints[fPtsIdx - 1]);
                    hasGeometry = true;
                    break;
            }
            ++fVerbsIdx;
...
class SkPathContoursIter : public SkTPathContoursIter<SkPathContoursIter> {
public:
    SkPathContoursIter(const SkPath& path) : SkTPathContoursIter(path) {}
    void resetGeometry(const SkPoint& startPoint) {}  // No-op
    void geometryTo(const SkPoint& endpoint) {}  // No-op
};
