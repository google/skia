...
class SkZeroShaderContext : public SkShaderBase::Context {
public:
    SkZeroShaderContext(const SkShader& shader, const SkShaderBase::ContextRec& rec)
        // Override rec with the identity matrix, so it is guaranteed to be invertible.
        : INHERITED(*as_SB(&shader), SkShaderBase::ContextRec(*rec.fPaint, SkMatrix::I(), nullptr,
