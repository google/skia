...
void SkBlitter::blitCoverageDeltas(SkCoverageDeltaList* deltas, const SkIRect& clip,
    ...
    for(int y = top; y < bottom; ++y) {
        ...
        if (canUseMask && !deltas->sorted(y) && deltas->count(y) << 3 >= clip.width()) {
            // Note that deltas->left()/right() may be different than clip.fLeft/fRight because
            // in the threaded backend, deltas are generated in the initFn with full clip, while
            // blitCoverageDeltas is called in drawFn with a subclip.
#ifdef SK_SUPPORT_LEGACY_THREADED_DAA_BUGS
            SkIRect rowIR = SkIRect::MakeLTRB(clip.fLeft, y, clip.fRight, y + 1);
#else
            SkIRect rowIR = SkIRect::MakeLTRB(deltas->left(), y, deltas->right(), y + 1);
#endif
