...
void SkCollatingCanvas::serializeToStream(SkStream* stream, const SkSerialProcs& procs) const {
...
SkCollatingCanvas::ResultOrError SkCollatingCanvas::CreateFromStream(
    ...
    for (;;) {
        ...
        SkString error = data->parseStreamTag(stream, tag, size, procs);
        if (error) {
...
sk_sp<SkPicture> SkCollatingCanvas::asSkPicture() {
    SkPictureRecorder r;
    SkCanvas* canvas = r.beginRecording(info.fCullRect);
