...
public:
    ...
    const SkPixmap& lowerLevel() const {
    ...
    SkMatrix inverseForUpper() const;
    SkMatrix inverseForLower() const;   // only vaid if mode() == kLinear
...
private:
    ...
    // for inverseFor... helpers
    const SkImage_Base* fImage;
    const SkMatrix      fInverse;
