...
bool SkM4::invert(SkM4* inverse) const {
    double a00 = fMat[0];
    double a01 = fMat[1];
    double a02 = fMat[2];
    double a03 = fMat[3];
...
SkM4& SkM4::operator=(const SkMatrix& src) {
    fMat[0]  = src[SkMatrix::kMScaleX];
    fMat[1]  = src[SkMatrix::kMSkewX];
    fMat[2]  = 0;
    fMat[3]  = src[SkMatrix::kMTransX];
    fMat[4]  = src[SkMatrix::kMSkewY];
    fMat[5]  = src[SkMatrix::kMScaleY];
    fMat[6]  = 0;
    fMat[7]  = src[SkMatrix::kMTransY];
    fMat[8]  = 0;
    fMat[9]  = 0;
    fMat[10] = 1;
    fMat[11] = 0;
    fMat[12] = src[SkMatrix::kMPersp0];
    fMat[13] = src[SkMatrix::kMPersp1];
    fMat[14] = 0;
    fMat[15] = src[SkMatrix::kMPersp2];
    return *this;
}
...
SkM4::operator SkMatrix() const {
    ...
    dst[SkMatrix::kMScaleX] = fMat[0];
    dst[SkMatrix::kMSkewX]  = fMat[4];
    dst[SkMatrix::kMTransX] = fMat[12];

    dst[SkMatrix::kMSkewY]  = fMat[4];
    dst[SkMatrix::kMScaleY] = fMat[5];
    dst[SkMatrix::kMTransY] = fMat[13];

    dst[SkMatrix::kMPersp0] = fMat[3];
    dst[SkMatrix::kMPersp1] = fMat[7];
    dst[SkMatrix::kMPersp2] = fMat[15];
