...
public:
    ...
    using Positioner = std::function<SkPoint(size_t, SkPoint)>;
    ...
    bool ApplySubpixelXAligned(
        ...
        for (size_t i = 0; i < glyphCount; i++) {
            ...
            const SkGlyph& advanceGlyph = cache->getGlyphIDAdvance(glyphId);
            SkPoint advance = {advanceGlyph.fAdvanceX, advanceGlyph.fAdvanceY};
            SkPoint finalPosition = positioner(i, advance);
