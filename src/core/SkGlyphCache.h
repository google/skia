...
public:
    ...
    */
    ...
    using SkStrikePtr = std::unique_ptr<SkGlyphCache, SkGlyphCache::AttachCacheFunctor>;

    static SkStrikePtr FindStrike(const SkDescriptor& desc);

    template <typename ScalerContextCreator>
    static SkStrikePtr FindOrCreateStrike(
        const SkDescriptor& desc, ScalerContextCreator&& creator)
    {
        auto cache = FindStrike(desc);
        if (cache == nullptr) {
            cache = CreateStrike(desc, creator);
        }
        return cache;
    }

    template <typename ScalerContextCreator>
    static SkStrikePtr CreateStrike(
        const SkDescriptor& desc, ScalerContextCreator creator)
    {
        ...
        return SkStrikePtr(new SkGlyphCache(&desc, std::move(context)));
    }
...
using SkStrikePtr = SkGlyphCache::SkStrikePtr;
