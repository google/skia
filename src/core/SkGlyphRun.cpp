...
void GrTextContext::regenerateGlyphRunList(GrTextBlob* cacheBlob,
    ...
    for (const auto& glyphRun : glyphRunList) {
        const SkPaint& skPaint = glyphRun.paint();
        cacheBlob->push_back_run(runIndex);

        cacheBlob->setRunPaintFlags(runIndex, skPaint.getFlags());

        if (CanDrawAsDistanceFields(skPaint, viewMatrix, props,
            ...
            SkScalar textRatio;
            SkPaint dfPaint(skPaint);
            SkScalerContextFlags flags;
            InitDistanceFieldPaint(cacheBlob, &dfPaint, viewMatrix, fOptions, &textRatio, &flags);
            cacheBlob->setHasDistanceField();
            cacheBlob->setSubRunHasDistanceFields(runIndex, skPaint.isLCDRenderText(),
                                                  skPaint.isAntiAlias(), hasWCoord);

            FallbackGlyphRunHelper fallbackTextHelper(
                    viewMatrix, skPaint, glyphCache->getGlyphSizeLimit(), textRatio);
            ...
            fallbackTextHelper.drawGlyphs(
                    ...
                    skPaint, filteredColor, scalerContextFlags);

        } else if (SkDraw::ShouldDrawTextAsPaths(skPaint, viewMatrix)) {
            // Ensure the blob is set for bitmaptext
            cacheBlob->setHasBitmap();

            // setup our std runPaint, in hopes of getting hits in the cache
            SkPaint pathPaint(skPaint);
            SkScalar matrixScale = pathPaint.setupForAsPaths();

            FallbackGlyphRunHelper fallbackTextHelper(
                    viewMatrix, skPaint, glyphCache->getGlyphSizeLimit(), matrixScale);
            ...
            auto cache = cacheBlob->setupCache(
                    runIndex, props, scalerContextFlags, skPaint, &viewMatrix);
