...
// -- SkGlyphRunBuilder ----------------------------------------------------------------------------
void SkGlyphRunBuilder::drawTextUTF8(
        const SkPaint& paint, const SkFont& font, const void* bytes, size_t byteLength, SkPoint origin) {
...
void SkGlyphRunBuilder::drawGlyphsWithPositions(
        const SkPaint& paint, const SkFont& font,  SkSpan<const SkGlyphID> glyphIDs, const SkPoint* pos) {
    if (!glyphIDs.empty()) {
        this->initialize(glyphIDs.size());
