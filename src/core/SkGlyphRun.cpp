...
void SkGlyphRunBuilder::drawTextBlob(const SkPaint& paint, const SkTextBlob& blob, SkPoint origin,
    ...
    for (SkTextBlobRunIterator it(&blob); !it.done(); it.next()) {
        if (it.positioning() != SkTextBlobRunIterator::kRSXform_Positioning) {
            simplifyTextBlobExceptRSXForm(paint, it, positions);
        } else {
...
void SkGlyphRunBuilder::textBlobToGlyphRunListWithoutRSX(
...
void SkGlyphRunBuilder::simplifyTextBlobExceptRSXForm(const SkPaint& paint,
