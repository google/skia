...
// A faster set implementation that does not need any initailization, and reading the set items
// is order the number of items, and not the size of the universe.
// This implementation is based on the paper by Briggs and Torczon, "An Efficient Representation
// for Sparse Sets"
class GlyphSet {
public:
    GlyphSet(SkGlyphID maxGlyphIDs)
    : fMaxGlyphIDs{maxGlyphIDs}
    , fIndexes{skstd::make_unique_default<uint16_t[]>(2 * maxGlyphIDs)}
    , fUniqueGlyphIDs{&fIndexes[maxGlyphIDs]} {
        sk_msan_mark_initialized(fIndexes.get(), &fIndexes[maxGlyphIDs], "works with uninited");
    }

    uint16_t add(SkGlyphID glyphID) {
        if (glyphID >= fMaxGlyphIDs) {
            glyphID = kUndefGlyph;
        }
