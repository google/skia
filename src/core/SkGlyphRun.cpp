...
void SkGlyphRun::temporaryShuntToDrawPosText(SkBaseDevice* device) {
    ...
    device->drawPosText(
            ...
            pos, 2, SkPoint::Make(0, 0), fRunPaint);
}
...
// -- SkGlyphRunList -------------------------------------------------------------------------------
SkGlyphRunList::SkGlyphRunList(SkSpan<SkGlyphRun> glyphRuns, const SkTextBlob* textBlob)
...
void SkGlyphRunBuilder::prepareDrawPosTextH(const SkPaint& paint, const void* bytes,
    ...
    this->drawPosTextH(
            ...
            SkSpan<const char>(), SkSpan<const uint32_t>(), SkPoint::Make(0, 0));
}
    ...
    for (SkTextBlobRunIterator it(&blob); !it.done(); it.next()) {
        ...
        switch (it.positioning()) {
            case SkTextBlob::kDefault_Positioning: {
                this->drawText(runPaint, it.glyphs(), glyphLen, origin + offset, text, clusters);
            }
            break;
            case SkTextBlob::kHorizontal_Positioning: {
                auto constY = offset.y();
                this->drawPosTextH(
                        runPaint, it.glyphs(), glyphLen, it.pos(), constY, text, clusters, origin);
            }
            break;
...
SkGlyphRunList* SkGlyphRunBuilder::useGlyphRunList() {
    new ((void*)&fScratchGlyphRunList)
        SkGlyphRunList{SkSpan<SkGlyphRun>(fGlyphRuns), fTemporaryTextBlobShunt};
...
void SkGlyphRunBuilder::drawPosTextH(
        ...
        SkSpan<const char> text, SkSpan<const uint32_t> clusters, SkPoint origin) {

    SkGlyphID* temporaryShuntGlyphIDs = this->addDenseAndUnique(paint, bytes, byteLength);

    for (size_t i = 0; i < runSize(); i++) {
        fPositions.push_back(SkPoint::Make(xpos[i], constY) + origin);
    }
...
void SkGlyphRunBuilder::drawPosText(
    ...
    for (size_t i = 0; i < runSize(); i++) {
        fPositions.push_back(pos[i] + origin);
    }
