...
#endif
// -- SkGlyphSet2 ----------------------------------------------------------------------------------
uint16_t SkGlyphSet2::add(SkGlyphID glyphID) {
    static constexpr SkGlyphID  kUndefGlyph{0};
...
void SkGlyphSet2::reuse(uint32_t glyphUniverseSize, std::vector<SkGlyphID>* uniqueGlyphIDs) {
    SkASSERT(glyphUniverseSize <= (1 << 16));
    fUniverseSize = glyphUniverseSize;
...
void SkGlyphRunBuilder::prepareDrawPosTextH(const SkPaint& paint, const void* bytes,
    ...
    initializeDenseAndUnique(paint, bytes, byteLength);
