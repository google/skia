...
void SkGlyphRunListDrawer::forEachMappedDrawableGlyph(
    ...
    // This is here because GCC does not understand that a static used in a template is a use
    // even if the template is not expanded.
    // TODO: all this logic should move to the glyph cache.
    auto lookup_glyph_by_subpixel = [] (SkAxisAlignment axisAlignment,
            SkPoint position, SkGlyphID glyphID, SkGlyphCache* cache) {
