...
const SkGlyphRunList& SkGlyphRunBuilder::textToGlyphRunList(
    ...
    if (!glyphIDs.empty()) {
        ...
        SkSpan<const SkPoint> positions = draw_text_positions(font, glyphIDs, origin, fPositions);
...
const SkGlyphRunList& SkGlyphRunBuilder::blobToGlyphRunList(
    ...
    SkVector* scaledRotationsCursor = fScaledRotations.data();
...
void SkGlyphRunBuilder::initialize(const SkTextBlob& blob) {
    ...
    if (totalGlyphs > fMaxTotalRunSize) {
        fMaxTotalRunSize = totalGlyphs;
        fPositions.reset(fMaxTotalRunSize);
    }

    if (rsxFormCount > SkTo<int>(fScaledRotations.size())) {
        fScaledRotations.reserve(rsxFormCount);
    }
