...
void SkGlyphRunBuilder::prepareDrawText(
    ...
    this->initialize(1, glyphIDs.size(), typeface->countGlyphs());
...
void SkGlyphRunBuilder::prepareDrawPosText(const SkPaint& paint, const void* bytes,
    ...
    this->drawPosText(
            ...
            fUniqueGlyphIDIndices.get(), fUniqueGlyphIDs.get(),
            SkSpan<const char>(), SkSpan<const uint32_t>());
}
...
void SkGlyphRunBuilder::prepareTextBlob(
    ...
    // Figure out all the storage needed to presize everything below. The sizing code assumes
    // that the drawFilter does not change the typeface on the paint.
    size_t runListSize = 0;
    size_t totalGlyphs = 0;
    uint16_t maxGlyphUniverse = 0;
    for (SkTextBlobRunIterator it(&blob); !it.done(); it.next()) {
        ...
        auto typeface = SkPaintPriv::GetTypefaceOrDefault(runPaint);
        maxGlyphUniverse = std::max(maxGlyphUniverse, SkTo<uint16_t>(typeface->countGlyphs()));

        runListSize += 1;
    }

    // Presize all the buffers so they don't move during processing.
    this->initialize(runListSize, totalGlyphs, maxGlyphUniverse);

    fTemporaryTextBlobShunt = &blob;
    fOrigin = origin;

    uint16_t* currentDenseIndices = fUniqueGlyphIDIndices.get();
    SkPoint* currentPositions = fPositions.get();
    SkGlyphID* currentUniqueGlyphIDs = fUniqueGlyphIDs.get();
    ...
    for (SkTextBlobRunIterator it(&blob); !it.done(); it.next()) {
        ...
        auto glyphIDs = SkSpan<const SkGlyphID>{it.glyphs(), it.glyphCount()};
