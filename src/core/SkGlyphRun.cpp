...
void SkGlyphRunBuilder::prepareDrawText(
    ...
    this->initialize();
    this->drawText(paint, bytes, byteLength, origin, SkSpan<char>(), SkSpan<uint32_t>());
}
...
void SkGlyphRunBuilder::prepareDrawPosTextH(const SkPaint& paint, const void* bytes,
                                            ...
                                            SkScalar constY) {
    this->initialize();
    this->drawPosTextH(paint, bytes, byteLength, xpos, constY, SkSpan<char>(), SkSpan<uint32_t>());
}

void SkGlyphRunBuilder::prepareDrawPosText(const SkPaint& paint, const void* bytes,
                                           size_t byteLength, const SkPoint* pos) {
    this->initialize();
    this->drawPosText(paint, bytes, byteLength, pos, SkSpan<char>(), SkSpan<uint32_t>());
}
...
void SkGlyphRunBuilder::prepareTextBlob(
    ...
    for (;!it.done(); it.next()) {
        ...
        auto text = SkSpan<char>(it.text(), it.textSize());
...
void SkGlyphRunBuilder::addGlyphRunToList(SkGlyphID* temporaryShuntGlyphIDs) {
...
void SkGlyphRunBuilder::drawText(
        const SkPaint& paint, const void* bytes, size_t byteLength, SkPoint origin,
        SkSpan<char> text, SkSpan<uint32_t> clusters) {
    ...
    this->addGlyphRunToList(temporaryShuntGlyphIDs);
}
