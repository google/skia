...
/**
 * Specifies how a gaussian blur should handle samples that fall outside the source content bounds.
 */
enum class SrcBoundsMode {
    kClamp,  // Clamp texture coords to the domain rectangle.
    kDecal,  // Treat the area outside the domain rectangle as fully transparent.
    kRepeat  // Wrap texture coordinates.
};

constexpr static int kNumSrcBoundsModes = 3;
 ...
 * @param srcBoundsMode   The mode to handle samples outside bounds.
