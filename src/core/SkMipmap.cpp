...
bool SkMipmap::Deserialize(SkMipmapBuilder* builder, const void* data, size_t size) {
    ...
    for (int i = 0; i < count; ++i) {
        ...
        auto codec = SkCodec::MakeFromData(SkData::MakeWithProc(ptr, size, nullptr, nullptr));
        if (!codec) {
            return false;
        }

        SkPixmap pm = builder->level(i);
        if (codec->getInfo().dimensions() != pm.dimensions()) {
            return false;
        }
        if (codec->getPixels(pm) != SkCodec::kSuccess) {
            return false;
        }
    }
