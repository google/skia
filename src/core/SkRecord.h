...
public:
    ...
    T* alloc(Args&&... args) {
        fApproxBytesAllocated += sizeof(T);
        return fAlloc.make<T>(std::forward<Args>(args)...);
    }

    template <typename T>
    T* allocArray(size_t count) {
        fApproxBytesAllocated += count * sizeof(T);
        return fAlloc.makeArrayDefault<T>(count);
    }
    ...
    T* append(Args&&... args) {
        ...
        return fRecords[fCount++].set(this->allocCommand<T>(std::forward<Args>(args)...));
    }
...
private:
    ...
    template <typename T, typename... Args>
    SK_WHEN(!std::is_empty<T>::value, T*) allocCommand(Args&&... args) {
        return this->alloc<T>(std::forward<Args>(args)...);
    }
