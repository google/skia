...
void SkBitmap::getBounds(SkRect* bounds) const {
    SkASSERT(bounds);
    *bounds = SkRect::MakeIWH(this->width(), this->height());
}
...
bool SkBitmap::setAlphaType(SkAlphaType newAlphaType) {
    ...
    if (this->alphaType() != newAlphaType) {
        fPixmap.setAlphaType(newAlphaType);
    }
    SkDEBUGCODE(this->validate();)
    return true;
}
...
SkIPoint SkBitmap::pixelRefOrigin() const {
    ...
    SkASSERT(addr >= pix);
    size_t off = addr - pix;
    return {SkToS32((off % fPixelRef->rowBytes()) / this->bytesPerPixel()),
            SkToS32(off / fPixelRef->rowBytes())};
}
...
void SkBitmap::setPixelRef(sk_sp<SkPixelRef> pr, int dx, int dy) {
    ...
    if (fPixelRef) {
        SkIPoint pixelRefOrigin = {dx, dy};
        fPixmap.setRowBytes(fPixelRef->rowBytes());
        // wish we could assert that a pixelref *always* has pixels
        void* p = fPixelRef->pixels();
        if (p) {
            p = (char*)p
                + pixelRefOrigin.fY * this->rowBytes()
                + pixelRefOrigin.fX * this->bytesPerPixel();
        }
        fPixmap.setPixels(p);
    } else {
        // ignore dx,dy if there is no pixelref
        fPixmap.setPixels(nullptr);
    }
