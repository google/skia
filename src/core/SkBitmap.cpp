...
bool SkBitmap::ReadRawPixels(SkReadBuffer* buffer, SkBitmap* bitmap) {
    ...
    // don't use it to compute minRowBytes64().
    ...
    uint64_t bytes = info.height() * info.minRowBytes64();
    if (!buffer->validate(SkTFitsIn<size_t>(bytes))) {
        return false;
    }

    sk_sp<SkData> data(SkData::MakeUninitialized(SkToSizeT(bytes)));
    unsigned char* dst = (unsigned char*)data->writable_data();
    if (!buffer->readByteArray(dst, SkToSizeT(bytes))) {
        return false;
    }
