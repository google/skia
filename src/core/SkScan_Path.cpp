...
// Just to note during debugging if we encounter unexpected edge conditions.
static void unexpected_edges() {
    SkASSERT(false);
}

static void walk_convex_edges(SkEdge* prevHead, SkBlitter* blitter, int start_y, int stop_y) {
    validate_sort(prevHead->fNext);
...
void sk_fill_path(const SkPath& path, const SkIRect& clipRect, SkBlitter* blitter,
    ...
    if (path.isConvex() && (nullptr == proc) && count >= 2) {
        walk_convex_edges(&headEdge, blitter, start_y, stop_y);
    } else {
