...
void sk_fill_path(const SkSPath& path, const SkIRect& clipRect, SkBlitter* blitter,
...
void SkScan::FillPath(const SkSPath& path, const SkRegion& origClip, SkBlitter* blitter) {
    if (origClip.isEmpty()) {
        return;
    }
...
void SkScan::FillPath(const SkSPath& path, const SkIRect& ir,
                      SkBlitter* blitter) {
    SkRegion rgn(ir);
...
void SkScan::FillTriangle(const SkPoint pts[], const SkRasterClip& clip, SkBlitter* blitter) {
    ...
    if (!SkRect::MakeLTRB(-limit, -limit, limit, limit).contains(r)) {
        FillPath(SkSPath_triangle(pts, r), clip, blitter);
        return;
    }
