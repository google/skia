...
void SkDraw::drawFixedVertices(const SkVertices* vertices, SkBlendMode blendMode,
    ...
    if (!colors) {
        if (!texCoords || texCoords == positions) {
            ...
            while (vertProc(&state)) {
                SkMatrix localM;
                if (!texture_to_matrix(state, positions, texCoords, &localM)) { continue; }
                SkPreConcatMatrixProvider totalMatrix(*fMatrixProvider, localM);
                SkSTArenaAlloc<2048> innerAlloc;
                if (auto blitter = SkCreateSkVMBlitter(
                        fDst, paint, totalMatrix, &innerAlloc, this->fRC->clipShader())) {
                    fill_triangle(state, blitter, *fRC, dev2, dev3);
                }
            }
            return;
