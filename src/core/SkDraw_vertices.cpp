...
public:
    SkTriColorShader(Matrix43** stageHandle, bool isOpaque) : fIsOpaque(isOpaque) {
        *stageHandle = &fM43;
    }
...
void SkDraw::drawVertices(SkVertices::VertexMode vmode, int count,
    ...
    if (colors || textures) {
        ...
        if (colors) {
            ...
            auto triShader = outerAlloc.make<SkTriColorShader>(&matrix43,
                                                               compute_is_opaque(colors, count));
            if (shader) {
                auto composeShader = outerAlloc.make<SkComposeShader>(sk_ref_sp(triShader),
                                                                      sk_ref_sp(shader),
                                                                      bmode);
