...
void SkDraw::drawVertices(SkVertices::VertexMode vmode, int count,
    ...
    if (skeleton && skeleton->getNumBones()) {
        // allocate space for the deformed vertices
        deformed.resize(count);

        // get the bone matrices
        const SkMatrix* influenceMatrices = skeleton->getBones();
        std::vector<SkMatrix> transformedInfluenceMatrices(skeleton->getNumBones());

        // transform the bone matrices by the world transform
        memcpy(transformedInfluenceMatrices.data(),
               influenceMatrices,
               skeleton->getNumBones() * sizeof(SkMatrix));
        for (int i = 1; i < skeleton->getNumBones(); i ++) {
            transformedInfluenceMatrices[i].preConcat(influenceMatrices[0]);
        }
        ...
        for (int i = 0; i < count; i ++) {
            if (bones) {
                ...
                for (uint32_t j = 0; j < 4; j ++) {
                    ...
                    SkSkeleton::Attachment attachment = bones[i * 4 + j];

                    // determine whether to use the identity matrix for 0 index
                    const SkMatrix& influenceMatrix =
                        attachment.index ?
                        transformedInfluenceMatrices[attachment.index] :
                        SkMatrix::I();

                    // get the weight
                    const float influenceWeight = attachment.weight;

                    // transformed = M * v
                    influenceMatrix.mapPoints(&transformed, &vertices[i], 1);

                    // result += transformed * w
                    result += transformed * influenceWeight;
                }
                ...
                const SkMatrix& worldTransform = influenceMatrices[0];
