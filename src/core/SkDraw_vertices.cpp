...
public:
    SkTriColorShader(Matrix43** stageHandle) {
        *stageHandle = &fM43;
    }
...
protected:
    ...
    bool onAppendStages(SkRasterPipeline* pipeine, SkColorSpace* dstCS, SkArenaAlloc* alloc,
                        const SkMatrix&, const SkPaint&, const SkMatrix*,
    ...
    bool onUpdateStage(StageHandle, const SkMatrix& ctm) const override { return true; }
...
void SkDraw::drawVertices(SkVertices::VertexMode vmode, int count,
    ...
    SkArenaAlloc    alloc(arenaStorage, sizeof(arenaStorage));

    SkPoint* devVerts = alloc.makeArray<SkPoint>(count);
    ...
    if (colors || textures) {
        ...
        SkShader::StageHandle   handle = nullptr;

        bool is_opaque = shader ? shader->isOpaque() : true,
             wants_dither = paint.isDither();

        if (colors) {
            dstColors = convert_colors(colors, count, fDst.colorSpace(), &alloc);
            ...
            auto triShader = alloc.make<SkTriColorShader>(&matrix43);
            if (shader) {
                auto composeShader = alloc.make<SkComposeShader>(sk_ref_sp(triShader),
                                                                 sk_ref_sp(shader),
                                                                 bmode);
        ...
        if (!shader->appendStages(&pipeline, fDst.colorSpace(), &alloc, *fMatrix, paint,
                                  nullptr, &handle)) {
            return;
        }

        auto blitter = SkCreateRasterPipelineBlitter(fDst, paint, pipeline, is_opaque, wants_dither,
                                                     &alloc);
        ...
        while (vertProc(&state)) {
            ...
            if (textures) {
                ...
                if (!shader->updateStage(handle, SkMatrix::Concat(*fMatrix, localM))) {
                    continue;
                }
            }
