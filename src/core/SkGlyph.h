...
struct SkPackedGlyphID {
    ...
    constexpr SkPackedGlyphID(const SkPackedGlyphID& id) = default;

    SkPackedGlyphID& operator=(const SkPackedGlyphID& id) = default;
...
public:
    ...
    SkGlyph(SkPackedGlyphID id,
            float advanceX, float advanceY,
            int16_t top, int16_t left,
            uint16_t width, uint16_t height,
            SkMask::Format format, void* image = nullptr);
    ...
    // using the alloc.
    bool mergeImage(const SkGlyph& from, SkArenaAlloc* alloc);
    ...
    // mergePath() or ensurePath(), after being initialized you can call path(), hasPath() and
    // pathSize(). You can always call pathIsInitialized() to detect if the path is initialized.
    //
    // The path information can be in one of three states.
    //    1) pathIsInitialized() == false - fPathData is nullptr and isEmpty() == false. An empty
    //                                      glyph is considered initialized.
    //    2) hasPath() == true - the path exists, and is associated with the glyph.
    //    3) hasPath() == false - the path is initialized, but no path exists for the glyph.
    ...
    // Initialize the path for this glyph using path if no path is associated already. path may
    // be nullptr to indicate that this glyph has no path.
