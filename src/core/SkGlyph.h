...
namespace skglyph {
namespace k {
constexpr inline int16_t max = std::numeric_limits<int16_t>::max();
constexpr inline int16_t min = std::numeric_limits<int16_t>::min();
}  // namespace
...
public:
    SkGlyphRect(int16_t left, int16_t top, int16_t right, int16_t bottom)
        ...
        SkASSERT(left != k::min && top != k::min && right != k::min && bottom != k::min);
    }
    bool empty() const {
...
namespace skglyph {
const inline SkGlyphRect kEmptyRect{ k::max,  k::max, -k::max, -k::max};
const inline SkGlyphRect kFullRect {-k::max, -k::max,  k::max,  k::max};
