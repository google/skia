...
public:
    ...
    // Set the path for this glyph. Return true if the glyph didn't have a path set before and now
    // one exists.
    bool setPath(SkArenaAlloc* alloc, SkScalerContext* scalerContext);
    bool setPath(SkArenaAlloc* alloc, const SkPath* path);

    // Returns true if that path has been set. The empty glyph is always considered set.
    bool pathIsSet() const { return fPathData != nullptr || this->isEmpty(); }
