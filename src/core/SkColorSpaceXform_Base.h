...
public:
    SkColorSpace* fDstSpace;

    SkColorSpaceXformKey(SkColorSpace* dstSpace)
        : fDstSpace(dstSpace)
    {
        this->init(&gColorSpaceXformAddress, 0, sizeof(SkColorSpace*));
    }
};
...
struct SkColorSpaceXformRec : public SkResourceCache::Rec {
    ...
    size_t bytesUsed() const override { return sizeof(SkColorSpaceXformRec); }
