...
namespace {
    ...
    struct Builder : public skvm::Builder {
        ...
        static bool CanBuild(const Key& key) {
            ...
            if (key.shader) {
                ...
                if (0 > as_SB(key.shader)->program(nullptr,
                                                   key.colorSpace,
                                                   skvm::Arg{0}, 0,
        ...
        Builder(const Key& key, size_t* uniform_bytes) {
        #define TODO SkUNREACHABLE
            ...
            if (key.shader) {
                ...
                int more_uniform_bytes = as_SB(key.shader)->program(this,
                                                                    key.colorSpace,
                                                                    uniforms, *uniform_bytes,
                                                                    &src.r, &src.g, &src.b, &src.a);
                SkASSERT(more_uniform_bytes >= 0);
                *uniform_bytes += more_uniform_bytes;
    ...
    private:
        ...
        skvm::Program buildProgram(Coverage coverage) {
            ...
            size_t uniform_bytes = fUniforms.size();
            Builder builder{key, &uniform_bytes};
            fUniforms.resize(uniform_bytes);
        ...
        void updateUniforms() {
            if (fKey.shader) {
                as_SB(fKey.shader)->uniforms(fKey.colorSpace, fUniforms.data() + sizeof(Uniforms));
            }
        }
