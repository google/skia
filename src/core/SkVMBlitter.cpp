...
namespace {
    ...
    struct DitherShader : public SkShaderBase {
        ...
        skvm::Color onProgram(skvm::Builder* p,
            ...
            skvm::Color color = as_SB(fShader)->program(p, ctm,localM, quality,dstCS,
                                                        uniforms,alloc, x,y);
            if (!color) {
                return color;
            }
            ...
            color.r = p->add(color.r, dither);
            color.g = p->add(color.g, dither);
            color.b = p->add(color.b, dither);

            // TODO: this is consistent with the old code but doesn't make sense for unpremul.
            color.r = p->clamp(color.r, p->splat(0.0f), color.a);
            color.g = p->clamp(color.g, p->splat(0.0f), color.a);
            color.b = p->clamp(color.b, p->splat(0.0f), color.a);
            return color;
        }
    };
