...
namespace {
    ...
    struct Params {
        ...
        sk_sp<SkShader>         clip;
        sk_sp<SkBlendFilter>    blendFilter;
        SkColorInfo             dst;
    ...
    struct Key {
        ...
        bool operator==(const Key& that) const {
            return this->shader      == that.shader
                && this->clip        == that.clip
                && this->blendFilter == that.blendFilter
    ...
    static SkString debug_name(const Key& key) {
        return SkStringPrintf("Shader-%" PRIx64 "_Clip-%" PRIx64 "_BlendFilter-%" PRIx64
    ...
    static Key cache_key(const Params& params,
        ...
        if (params.blendFilter) {
            const SkBlendFilterBase* bfb = as_BFB(params.blendFilter);
            skvm::Builder p;

            skvm::Color dst = dst_color(&p, params);
            skvm::Color outColor = bfb->program(&p, src, dst, params.dst, uniforms, alloc);
            if (outColor) {
                blendHash = p.hash();
    ...
    static void build_program(skvm::Builder* p, const Params& params,
        ...
        src = params.blendFilter
                     ? as_BFB(params.blendFilter)->program(p, src, dst, params.dst, uniforms, alloc)
