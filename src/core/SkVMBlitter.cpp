...
namespace {
    ...
    struct Key {
        ...
        uint8_t  colorType,
                 ...
                 coverage;
        uint32_t padding{0};
        // Params::{paint,quality,ctm} are only passed to {shader,clip}->program(),
        // not used here by the blitter itself.  No need to include them in the key;
        // they'll be folded into the shader key as used.
    ...
    struct Builder : public skvm::Builder {
        ...
        static Key CacheKey(const Params& params,
            ...
            auto hash_shader = [&](const sk_sp<SkShader>& shader) {
                ...
                // Any color will do here, and using the dst color space is the quickest.
                skvm::Color paint = p.uniformPremul({0,0,0,0}, params.dst.colorSpace(),
                                                     uniforms, params.dst.colorSpace());
                uint64_t hash = 0;
        ...
        Builder(const Params& params, skvm::Uniforms* uniforms, SkArenaAlloc* alloc) {
            ...
            skvm::Color paint = uniformPremul(params.paint, sk_srgb_singleton(),
                                              uniforms, params.dst.colorSpace());
    ...
    struct DitherShader : public SkShaderBase {
        DitherShader(sk_sp<SkShader> shader)
