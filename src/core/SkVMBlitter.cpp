...
namespace {
    ...
    static bool build_program_q14(skvm::Builder* p, const Params& params,
        ...
        skvm::Color_Q14 src = as_SB(params.shader)->program_Q14(p, device,local, paint,
        ...
        auto from_unorm8 = [](skvm::I32 x) -> skvm::Q14 {
            skvm::Q14 v = to_Q14(x);
            // 16384 / 255 = 64.251, which we do as (128.5*v + 1)/2.
        ...
        if (params.clip) {
            skvm::Color_Q14 clip = as_SB(params.clip)->program_Q14(p, device,local, paint,
                                                                   params.matrices, nullptr,
                                                                   params.quality, params.dst,
                                                                   uniforms, alloc);
            if (!clip) {
        ...
        // Roundtrips with from_unorm8().
        auto to_unorm8 = [](skvm::Q14 x) -> skvm::I32 {
            // Remember 16384/255 ≈ 64.251,
            // so dividing by 64 alone doesn't leave things quite small enough.
            // In ordinary C this would be (x >> 6) - (x > 0x3fbf),
            // but since (x > 0x3fbf) returns 0 or -1, we'll add here instead.
