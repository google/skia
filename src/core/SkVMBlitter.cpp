...
namespace {
    ...
    static Key cache_key(const Params& params,
        ...
        if (params.customBlend) {
            ...
            skvm::Color outColor = params.customBlend->program(&p, src, dst, params.dst,
                                                               uniforms, alloc);
            if (outColor) {
                blendHash = p.hash();
    ...
    static void build_program(skvm::Builder* p, const Params& params,
        ...
        if (params.customBlend) {
            // Apply the custom blend function immediately before the Porter-Duff blend. (When a
            // custom blend is set, the Porter-Duff blend can be set to BlendMode::kSrc to preserve
            // the custom blend result and avoid performing any extra work.)
            src = params.customBlend->program(p, src, dst, params.dst, uniforms, alloc);
        }
    ...
    static Params effective_params(const SkPixmap& device,
        ...
        return {
            std::move(shader),
            std::move(clip),
            nullptr,  // TODO(skia:12080): add support for a custom blend function
