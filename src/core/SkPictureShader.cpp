...
sk_sp<SkShader> SkPictureShader::refBitmapShader(const SkMatrix& viewMatrix, const SkMatrix* localM,
    ...
    // |fColorSpace| will only be set when using an SkColorSpaceXformCanvas.  This canvas
    // is strictly for legacy mode.
...
sk_sp<SkShader> SkPictureShader::onMakeColorSpace(SkColorSpaceXformer* xformer) const {
    return sk_sp<SkPictureShader>(new SkPictureShader(fPicture, fTmx, fTmy,
                                                      &this->getLocalMatrix(), &fTile,
                                                      xformer->dst()));
}
