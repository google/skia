...
namespace skvm {
    ...
    static int jit(Assembler& a, size_t* code,
        ...
        *code = a.size();
        ...
        auto emit = [&](const Program::Instruction& inst, bool body) {
            Op  op = inst.op;
            ...
            switch (op) {
                case Op::store8: a.xtns2h(r(tmp), r(x));
                     ...
                     if (body) { a.strs  (r(tmp), arg[imm]); }
                     else      { a.strb  (r(tmp), arg[imm]); }
                                 break;
                case Op::store32:
                     if (body) { a.strq(r(x), arg[imm]); }
                     else      { a.strs(r(x), arg[imm]); }
                                 break;

                case Op::load8:
                     if (body) { a.ldrs   (r(tmp), arg[imm]); }
                     else      { a.ldrb   (r(tmp), arg[imm]); }
                                 a.uxtlb2h(r(tmp), r(tmp));
                                 a.uxtlh2s(r(d)  , r(tmp));
                                 break;
                case Op::load32:
                     if (body) { a.ldrq(r(d), arg[imm]); }
                     else      { a.ldrs(r(d), arg[imm]); }
                                 break;
