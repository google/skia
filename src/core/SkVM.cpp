...
namespace skvm {
    ...
    bool Program::jit(const std::vector<OptimizedInstruction>& instructions, Assembler* a) const {
        ...
        auto load_from_label = [&](Reg r, A::Label* l) { a->vmovups(r, l);                      };
        ...
        auto load_from_label = [&](Reg r, A::Label* l) { a->ldrq(r, l);         };
        auto load_from_stack = [&](Reg r, Val id) {
            SkASSERT(on_stack[id]);
            a->ldrq(r, A::sp, id);
        };
        ...
        auto emit = [&](Val id, bool scalar) {
            ...
            auto alloc_tmp = [&]() -> Reg {
                ...
                SkASSERT(v == NA || v >= 0);
                if (v >= 0) {
                    if (instructions[v].op != Op::splat && !on_stack[v]) {
                        store_to_stack(r, v);
                    }
                    v = NA;
                }
            ...
            // v is either in a register, at a read-only location in memory, or on the stack.
            // If v is
            //    - in a register, set *reg    and return true
            //    - at a label,    set *l      and return false
            //    - on the stack,  set nothing and return false
            auto locate = [&](Val v, Reg* reg, A::Label** l) -> bool {
                // Quick-check our working registers.
                if (v == id && rd != NA) { *reg = (Reg)rd; return true; }
                if (v ==  x && rx != NA) { *reg = (Reg)rx; return true; }
                if (v ==  y && ry != NA) { *reg = (Reg)ry; return true; }
                if (v ==  z && rz != NA) { *reg = (Reg)rz; return true; }

                // Search inter-instruction register map.
                for (auto [r,val] : SkMakeEnumerate(regs)) {
                    if (val == v) {
                        *reg = update_regs((Reg)r, v);
                        return true;
                    }
                }
                ...
                if (instructions[v].op == Op::splat) {
                    *l = &constants[instructions[v].immy];
                    return false;
                }
            ...
            switch (op) {
                // We've handled constants already.
                case Op::splat: break;
        ...
        auto restore_incoming_state = [&,saved_regs=regs,saved_on_stack=on_stack]{
            for (int i = 0; i < (int)regs.size(); i++) {
                const Reg r = (Reg)i;
                if (regs[r] != saved_regs[r]) {
                    regs[r]  = saved_regs[r];
                    if (Val v = regs[r]; v >= 0) {
                        if (instructions[v].op == Op::splat) {
                            load_from_label(r, &constants[instructions[v].immy]);
                        } else {
                            load_from_stack(r, v);
                        }
                    }
