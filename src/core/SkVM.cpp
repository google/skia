        ...
        auto emit = [&](Val id, bool scalar) {
            ...
            switch (op) {
                ...
                case Op::store128: {
                    ...
                    int ptr = immz>>1,
                        lane = immz&1;
                    a->vpextrd(A::Mem{arg[ptr], 0*16 + 8*lane + 0}, (A::Xmm)r(x), 0);
                    a->vpextrd(A::Mem{arg[ptr], 0*16 + 8*lane + 4}, (A::Xmm)r(y), 0);
                    if (scalar) { break; }
                    ...
                    a->vpextrd(A::Mem{arg[ptr], 4*16 + 8*lane + 0}, (A::Xmm)dst(), 0);
                    ...
                    a->vpextrd(A::Mem{arg[ptr], 4*16 + 8*lane + 4}, (A::Xmm)dst(), 0);
                ...
                case Op::load128: if (scalar) {
                                      ...
                                      A::Ymm tmp = alloc_tmp();
                                      A::Xmm t = (A::Xmm)tmp;
                                      a->vmovd  (t,   A::Mem{arg[immy], 4*(immz+0)}   );
                                      a->vpinsrd(t,t, A::Mem{arg[immy], 4*(immz+1)}, 1);
                                      a->vpinsrd(t,t, A::Mem{arg[immy], 4*(immz+2)}, 2);
                                      a->vpinsrd(t,t, A::Mem{arg[immy], 4*(immz+3)}, 3);

                                      // Load 4 high values into xmm dst(),
                                      A::Xmm d = (A::Xmm)dst();
                                      a->vmovd  (d,   A::Mem{arg[immy], 4*(immz+4)}   );
                                      a->vpinsrd(d,d, A::Mem{arg[immy], 4*(immz+5)}, 1);
                                      a->vpinsrd(d,d, A::Mem{arg[immy], 4*(immz+6)}, 2);
                                      a->vpinsrd(d,d, A::Mem{arg[immy], 4*(immz+7)}, 3);
