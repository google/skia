...
namespace skvm {
    ...
    bool Program::jit(const std::vector<OptimizedInstruction>& instructions, Assembler* a) const {
        ...
        using Reg = A::Ymm;
        constexpr std::array<Val,16> all_regs = {
        ...
        using Reg = A::V;
        constexpr std::array<Val,32> all_regs = {
             NA, NA, NA, NA,  NA, NA, NA, NA,
        ...
        // Our registers start maximally available on the way into the function.
        auto regs = all_regs;

        auto emit = [&](Val id, bool scalar) {
            const auto [op, X,Y,Z, immy,immz, death,can_hoist,used_in_loop] = instructions[id];
            const Val x = X, y = Y, z = Z;  // Can't close a lambda over local bindings directly.

            // As we enter a new instruction, any old TMP registers are now available again.
            for (Val& v : regs) {
                if (v == TMP) {
                    v = NA;
                }
            ...
            auto alloc_reg = [&]() -> Reg {
                auto score = [&](Val v) -> int {
                    if (v == NA) { return 0x7fff'fffe; }  // An empty register is great.

                    // We cannot use REServed registers,
                    // nor registers we need for this instruction.
                    if (v == RES ||
                        v == TMP || v == id || v == x || v == y || v == z) {
                        return -1;
                    }

                    if (!live(v)) { return 0x7fff'ffff; }  // Reusing a register is best.
        ...
        const auto regs_after_hoisting = regs;
