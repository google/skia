    ...
    bool Program::jit(const std::vector<OptimizedInstruction>& instructions,
        ...
        const int K = 8;
            ...
            const A::GP64 N = A::rcx,
                        GP0 = A::rax,
                        GP1 = A::r11,
                        arg[]    = { A::rdx, A::r8, A::r9, A::r10, A::r12 };
            auto enter = [&]{
                ...
                // 2) Load r12 from rsp+48 if there's a fifth arg,
                //    first preserving its original callee-saved value at rsp+0,
                //    which is an ABI reserved shadow area usually for spilling rcx.
                if (fImpl->strides.size() >= 5) {
                    a->mov(A::Mem{A::rsp}, A::r12);
                    a->mov(A::r12, A::Mem{A::rsp, 48});
                }
                // 3) Save ymm6-ymm15 (really just need to save xmm6-xmm15, but this works).
                a->sub(A::rsp, 16*K*4);
                for (int i = 6; i < 16; i++) {
                    a->vmovups(A::Mem{A::rsp, i*K*4}, (A::Ymm)i);
                }
            ...
            auto exit  = [&]{
                ...
                for (int i = 6; i < 16; i++) {
                    a->vmovups((A::Ymm)i, A::Mem{A::rsp, i*K*4});
                }
                a->add(A::rsp, 16*K*4);
                // 2) restore r12 if we used it
                if (fImpl->strides.size() >= 5) {
                    a->mov(A::r12, A::Mem{A::rsp});
                }
