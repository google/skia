    ...
    void Program::eval(int n, void* args[], size_t strides[], int nargs) const {
        ...
        if (!fJIT) {
...

            {
                ...
                fprintf(f, "%zx %zx %s\n", (size_t)fJIT->getCode(), fJIT->meat_ends, name.c_str());
                fprintf(f, "%zx %zx %s-overhead\n", (size_t)fJIT->getCode() + fJIT->meat_ends
                                                  ,         fJIT->getSize() - fJIT->meat_ends
                                                  , name.c_str());
                fclose(f);
            }
        #endif
