        ...
        *registers_used = 0;  // We'll update this as we go.
        ...
        auto emit = [&](Val id, bool scalar) {
            ...
            auto alloc_tmp = [&](int N=1) -> Reg {
                ...
                for (int block = 0; block <= (int)regs.size()-N; block++) {
                    int spills = 0;
                    for (int r = block; r < block+N; r++) {
                        Val v = regs[r];
                        // Registers holding nothing are ideal, nothing to spill.
                        if (v == NA) {
                            continue;
                        }
                ...
                // we'll need to revisit this logic if this assert fires.
