...
namespace skvm {
    ...
    bool SkColorType_to_PixelFormat(SkColorType ct, PixelFormat* f) {
        switch (ct) {
            ...
            case kRGB_888x_SkColorType:   *f = {8,8,8,0,  0,8,16, 0}; return true;
    ...
    Color Builder::unpack_unorm(PixelFormat f, I32 x) {
        return {
            f.r_bits ? from_unorm(f.r_bits, extract(x, f.r_shift, (1<<f.r_bits)-1)) : splat(0.0f),
            f.g_bits ? from_unorm(f.g_bits, extract(x, f.g_shift, (1<<f.g_bits)-1)) : splat(0.0f),
            f.b_bits ? from_unorm(f.b_bits, extract(x, f.b_shift, (1<<f.b_bits)-1)) : splat(0.0f),
            f.a_bits ? from_unorm(f.a_bits, extract(x, f.a_shift, (1<<f.a_bits)-1)) : splat(1.0f),
        };
    }
