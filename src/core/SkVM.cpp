...
namespace skvm {
    ...
    bool Program::jit(const std::vector<OptimizedInstruction>& instructions, Assembler* a) const {
        ...
        for (const OptimizedInstruction& inst : instructions) {
            if (inst.op == Op::splat) {
                SkASSERT(!splats.find(inst.immy));
                splats[inst.immy] = {};
            }
        }
        ...
        auto load_from_memory = [&](Reg r, Val v) {
            if (instructions[v].op == Op::splat) {
                A::Label* l = splats.find(instructions[v].immy);
                SkASSERT(l);
                a->vmovups(r, l);
            } else {
                SkASSERT(on_stack[v]);
                a->vmovups(r, A::Mem{A::rsp, v*K*4});
            }
        };
        auto store_to_stack = [&](Reg r, Val v) {
            on_stack[v] = true;
            a->vmovups(A::Mem{A::rsp, v*K*4}, r);
        };
        ...
        auto emit = [&](Val id, bool scalar) {
            ...
            auto alloc_tmp = [&]() -> Reg {
                ...
                SkASSERT(v == NA || v >= 0);
                if (v >= 0) {
                    if (!on_stack[v] && instructions[v].op != Op::splat) {
                        store_to_stack(r, v);
                    }
                    v = NA;
                }
            ...
            switch (op) {
                case Op::splat: SkASSERT(splats.find(immy)); break;
