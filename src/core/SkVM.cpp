...
namespace skvm {
    ...
    Val Builder::id(I32 x) {
        if (!x.builder) {
            x = this->splat(x.imm);
        }
        assert(x.builder == this);
        return x.val;
    }
    Val Builder::id(F32 x) {
        if (!x.builder) {
            x = this->splat(x.imm);
        }
        assert(x.builder == this);
    ...
    void Builder::assert_true(I32 cond, I32 debug) {
        ...
        (void)this->push(Op::assert_true, id(cond),id(debug),NA);
    #endif
    }

    void Builder::store8 (Arg ptr, I32 val) { this->push(Op::store8 , id(val),NA,NA, ptr.ix); }
    void Builder::store16(Arg ptr, I32 val) { this->push(Op::store16, id(val),NA,NA, ptr.ix); }
    void Builder::store32(Arg ptr, I32 val) { this->push(Op::store32, id(val),NA,NA, ptr.ix); }
    ...
    F32 Builder::add(F32 x, F32 y) {
        float X,Y;
        if (this->allImm(id(x),&X, id(x),&Y)) { return this->splat(X+Y); }
    ...
    I32 Builder::shl_16x2(I32 x, int bits) { return {this->push(Op::shl_i16x2, id(x),NA,NA, bits)}; }
    I32 Builder::shr_16x2(I32 x, int bits) { return {this->push(Op::shr_i16x2, id(x),NA,NA, bits)}; }
    I32 Builder::sra_16x2(I32 x, int bits) { return {this->push(Op::sra_i16x2, id(x),NA,NA, bits)}; }
