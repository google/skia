...
namespace skvm {
    ...
    void Program::eval(int n, void* args[], size_t strides[], int nargs) const {
                 ...
                 size_t strides[], int nargs)
            {
                ...
                for (int i = 0; i < (int)instructions.size(); i++) {
                    ...
                    auto y = inst.y,
                         z = inst.z;
                    switch (op) {
                        case Op::store8:  ok = false; break;
                        case Op::store32: vmovups(ptr[arg[y.imm]], r[x]); break;

                        case Op::load8:  ok = false; break;
                        ...
                        case Op::mul_unorm8: vpmulld(r[d], r[x], r[y.id]);
                                             splats.emplace_back(Xbyak::Label(), 255);
                                             vpaddd(r[d], r[d], ptr[rip + splats.back().first]);
                                             vpsrad(r[d], r[d], 8);
                                             break;
        ...
        if (avx2.ok && n >= 8) {
            bool ran = true;
            switch (nargs) {
                case 0: avx2.getCode<void(*)(int              )>()(n                  ); break;
                case 1: avx2.getCode<void(*)(int, void*       )>()(n, args[0]         ); break;
                case 2: avx2.getCode<void(*)(int, void*, void*)>()(n, args[0], args[1]); break;
