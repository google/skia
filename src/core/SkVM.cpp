    ...
    bool Program::jit(const std::vector<OptimizedInstruction>& instructions, Assembler* a) const {
        ...
        auto emit = [&](Val id, bool scalar) {
            ...
            switch (op) {
                ...
                case Op::splat: if (immy) { a->vbroadcastss(dst(), &splats[immy]); }
                ...
                case Op::splat: if (immy) { a->ldrq(dst(), &splats[immy]); }
        ...
        // We use ldrq to get splats on ARM, so they need to be 16-byte there.
        // On x86 we use vbroadcastss to get splats, so those only need to be 4-byte.
        splats.foreach([&](int imm, A::Label* label) {
            a->align(4);
            a->label(label);
            for (int i = 0; i < K; i++) {
                a->word(imm);
