...
namespace skvm {
    ...
    bool Program::jit(const std::vector<OptimizedInstruction>& instructions, Assembler* a) const {
        ...
        // Entering into the function, our registers simply start maximally available.
        auto regs = all_regs;
        ...
        auto emit = [&](Val id, bool scalar) {
            ...
            auto alloc_reg = [&]() -> Reg {
                auto heuristic = [&](Val v) -> int {
                    // We cannot use TMP or RES registers.
                    if (v == TMP || v == RES) { return 0; }

                    // Best register to choose is one that's empty.
                    if (v == NA) { return 0x7fff'ffff; }

                    // Next best is a value we won't need any more.
