...
namespace skvm {
    ...
    Program::Program(std::vector<Builder::Instruction> instructions,
                     ...
                     std::vector<int>                  strides) : fStrides(std::move(strides)) {
        SkASSERT(instructions.size() == deaths.size());

        this->setupInterpreter(instructions, deaths);
    #if defined(SKVM_JIT)
        this->setupJIT        (instructions, deaths);
    #endif
    }

    // Translate Builder::Instructions to Program::Instructions used by the interpreter.
    void Program::setupInterpreter(std::vector<Builder::Instruction> instructions,
                                   std::vector<Val>                  deaths) {
        struct Analysis {
...
#if defined(SKVM_JIT)
    bool Program::jit(std::vector<Builder::Instruction> instructions,
                      std::vector<Val>                  deaths,
                      Assembler* a) const {
        using A = Assembler;
    #if defined(__x86_64__)
        A::GP64 N = A::rdi,
           args[] = { A::rsi, A::rdx, A::rcx, A::r8, A::r9 };
        if (SK_ARRAY_COUNT(args) < fStrides.size()) {
            return false;
        }
        ...
        std::vector<Reg> val_to_reg(instructions.size());
        ...
        auto emit = [&](Builder::Instruction inst, bool scalar) {
    ...
    a->label(&body); {
        ...
        for (int i = 0; i < (int)instructions.size(); i++) {
            emit(instructions[i], /*scalar=*/K==1);
        }
        for (int i = 0; i < (int)fStrides.size(); i++) {
            a->add(args[i], K*fStrides[i]);
        }
        a->sub(N, K);
        a->jmp(&body);
    }
    ...
    void Program::setupJIT(std::vector<Builder::Instruction> instructions,
                           std::vector<Val> deaths) {
        // Run first wih no buffer to determine a.size(), the number of bytes we'll assemble.
        Assembler a{nullptr};
        if (!this->jit(instructions, deaths, &a)) {
            return;
        }
        ...
        a = Assembler{fJITBuf};
        SkAssertResult(this->jit(instructions, deaths, &a));
