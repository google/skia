...
namespace skvm {
    ...
    F32 Builder::approx_log2(F32 x) {
        ...
        return sub(sub(e, mad(splat(1.498030302f),m, splat(124.225514990f))),
                          div(splat(1.725879990f),add(splat(0.3520887068f), m)));
    }

    F32 Builder::approx_pow2(F32 x) {
        F32 f = fract(x);
        F32 approx = sub(add(x,splat(121.274057500f)),
                         mad(splat(1.490129070f), f, div(splat(27.728023300f),
                                                         sub(splat(4.84252568f),f))));
        return bit_cast(round(mul(splat(1.0f * (1<<23)), approx)));
    }

    F32 Builder::approx_powf(F32 x, F32 y) {
        auto is_x = bit_or(eq(x, splat(0.0f)), eq(x, splat(1.0f)));
