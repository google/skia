...
namespace skvm {
    ...
    std::vector<OptimizedInstruction> Builder::optimize(bool for_jit) const {
        ...
        // This quick calculation produces a maximum... the uses could be from dead code.
        std::vector<int> max_use_count(fProgram.size(), 0);
        for (const Builder::Instruction& inst : fProgram) {
            if (inst.x != NA) { max_use_count[inst.x]++; }
            if (inst.y != NA) { max_use_count[inst.y]++; }
            if (inst.z != NA) { max_use_count[inst.z]++; }
        }
        ...
        for (int i = 0; i < (int)fProgram.size(); i++) {
            ...
            // Convert mul/add, add/mul, mul/sub, and sub/mul to fused ops when
            // it's certain that the fused operation consumes the multiply.
            // TODO: if we did this later, we'd know an exact use_count and get more of these.
            Op lhs_op, rhs_op;
            if (fma_supported()) {
                switch (inst.op) {
                    default: break;
                    ...
                    fuse:
                        if (auto lhs = fProgram[inst.x];
                                lhs.op == Op::mul_f32 && max_use_count[inst.x] == 1) {
                            inst.op = lhs_op;
                            inst.z  = inst.y;
                            inst.x  = lhs.x;
                            inst.y  = lhs.y;
                        ...
                        if (auto rhs = fProgram[inst.y];
                                rhs.op == Op::mul_f32 && max_use_count[inst.y] == 1) {
                            inst.op = rhs_op;
                            inst.z  = inst.x;
                            inst.x  = rhs.x;
                            inst.y  = rhs.y;
