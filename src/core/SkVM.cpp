...
namespace skvm {
    ...
    bool Program::jit(const std::vector<OptimizedInstruction>& instructions, Assembler* a) const {
        ...
        // Map val -> stack slot.
        std::vector<int> on_stack(instructions.size(), NA);
        ...
        auto load_from_stack = [&](Reg r, Val id) {
            SkASSERT(on_stack[id] != NA);
            a->vmovups(r, A::Mem{A::rsp, on_stack[id]*K*4});
        };
        auto store_to_stack  = [&](Reg r, Val id) {
            on_stack[id] = next_stack_slot++;
            a->vmovups(A::Mem{A::rsp, on_stack[id]*K*4}, r);
        };
        ...
        auto load_from_stack = [&](Reg r, Val id) {
            SkASSERT(on_stack[id] != NA);
            a->ldrq(r, A::sp, on_stack[id]);
        };
        auto store_to_stack  = [&](Reg r, Val id) {
            on_stack[id] = next_stack_slot++;
            a->strq(r, A::sp, on_stack[id]);
        };
    #endif
        ...
        auto emit = [&](Val id, bool scalar) {
            ...
            auto any = [&](Val v) -> A::Operand {
                ...
                SkASSERT(v < id);
                return A::Mem{A::rsp, on_stack[v]*K*4};
            };
