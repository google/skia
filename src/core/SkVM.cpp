...
namespace skvm {
    ...
    bool Builder::isImm(Val id, T* imm, Rest... rest) const {
        if (fProgram[id].op == Op::splat) {
            static_assert(sizeof(T) == 4);
            memcpy(imm, &fProgram[id].immy, 4);

            if constexpr (sizeof...(rest) > 0) {
                return this->isImm(rest...);
            }
            return true;
        }
        return false;
    ...
    #ifdef SK_DEBUG
        if (int imm; this->isImm(val.id,&imm)) { SkASSERT(imm); return; }
    ...
    F32 Builder::add(F32 x, F32 y) {
        if (float X,Y; this->isImm(x.id,&X, y.id,&Y)) { return this->splat(X+Y); }
    ...
    F32 Builder::mad(F32 x, F32 y, F32 z) {
        if (float X,Y,Z; this->isImm(x.id,&X, y.id,&Y, z.id,&Z)) { return this->splat(X*Y+Z); }
    ...
    F32 Builder::min(F32 x, F32 y) {
        if (float X,Y; this->isImm(x.id,&X, y.id,&Y)) { return this->splat(std::min(X,Y)); }
        return {this->push(Op::min_f32, x.id, y.id)};
    }
    F32 Builder::max(F32 x, F32 y) {
        if (float X,Y; this->isImm(x.id,&X, y.id,&Y)) { return this->splat(std::max(X,Y)); }
    ...
    I32 Builder:: eq(F32 x, F32 y) {
        if (float X,Y; this->isImm(x.id,&X, y.id,&Y)) { return this->splat(X==Y ? ~0 : 0); }
    ...
    I32 Builder::lt(F32 x, F32 y) {
        if (float X,Y; this->isImm(x.id,&X, y.id,&Y)) { return this->splat(Y> X ? ~0 : 0); }
    ...
    I32 Builder::bit_and(I32 x, I32 y) {
        if (int X,Y; this->isImm(x.id,&X, y.id,&Y)) { return this->splat(X&Y); }
    ...
    I32 Builder::select(I32 x, I32 y, I32 z) {
        if (int X,Y,Z; this->isImm(x.id,&X, y.id,&Y, z.id,&Z)) { return this->splat(X?Y:Z); }
        return {this->push(Op::select, x.id, y.id, z.id)};
    }


    I32 Builder::extract(I32 x, int bits, I32 z) {
        if (int X,Z; this->isImm(x.id,&X, z.id,&Z)) { return this->splat( (unsigned(X)>>bits)&Z ); }
        return {this->push(Op::extract, x.id,NA,z.id, bits,0)};
    }

    I32 Builder::pack(I32 x, I32 y, int bits) {
        if (int X,Y; this->isImm(x.id,&X, y.id,&Y)) { return this->splat(X|(Y<<bits)); }
    ...
    F32 Builder::to_f32(I32 x) {
        if (int X; this->isImm(x.id,&X)) { return this->splat((float)X); }
        return {this->push(Op::to_f32, x.id)};
    }
    I32 Builder::trunc(F32 x) {
        if (float X; this->isImm(x.id,&X)) { return this->splat((int)X); }
        return {this->push(Op::trunc, x.id)};
    }
    I32 Builder::round(F32 x) {
        if (float X; this->isImm(x.id,&X)) { return this->splat((int)lrintf(X)); }
