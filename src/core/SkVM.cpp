...
namespace skvm {
    ...
    void Assembler::op(int opcode, int opcode_ext, Operand dst, int imm) {
        ...
        if (dst.kind == Operand::REG) {
            this->byte(rex(W1,0,0,dst.reg>>3));
            this->byte(opcode);
            this->byte(mod_rm(Mod::Direct, opcode_ext, dst.reg&7));
        } else {
            ...
            this->byte(rex(W1,0,m.index>>3,m.base>>3));
            this->byte(opcode);
            this->byte(mod_rm(mod(m.disp), 0, (need_SIB ? rsp : m.base)&7));
            if (need_SIB) {
                this->byte(sib(m.scale, m.index&7, m.base&7));
            }
            this->bytes(&m.disp, skvm::imm_bytes(mod(m.disp)));
        }
        this->bytes(&imm, imm_bytes);
    }

    void Assembler::add(Operand dst, int imm) { this->op(0,0b000, dst,imm); }
    void Assembler::sub(Operand dst, int imm) { this->op(0,0b101, dst,imm); }
    void Assembler::cmp(Operand reg, int imm) { this->op(0,0b111, reg,imm); }
