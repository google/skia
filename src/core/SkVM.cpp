...
namespace skvm {
    ...
    // Pack mandatory opcode prefix byte to 2-bit encoding for VEX.
    static int opcode_prefix(uint8_t byte) {
        switch (byte) {
            case 0x66: return 0b01;
            case 0xf3: return 0b10;
            case 0xf2: return 0b11;
        }
        // Not sure if this is unreachable or not.
        return 0b00;
    }

    // Pack leading opcode prefix to 5-bit encoding for VEX.
    static int map_select(int byte) {
        switch (byte) {
            case 0x000f: return 0b00001;
            case 0x380f: return 0b00010;
            case 0x3a0f: return 0b00011;
            // Some more cases only used by XOP / TBM.
        }
        SkASSERT(false);
        return 0b00000;
    }
    ...
    static VEX vex(bool WE,   // Like REX W for int operations, or opcode extension for float.
                   bool  R,   // Same as REX R.
                   bool  X,   // Same as REX X.
                   bool  B,   // Same as REX B.
                   int map,   // 5-bit opcode map to use, from map_select().
                   int   y,   // 4-bit second operand y-register.  (Called vvvv in docs.)
                   bool  L,   // Set for 256-bit ymm operations, off for 128-bit xmm.
                   int  pp) { // 2-bit mandatory opcode prefix, from opcode_prefix().
        VEX vex = {0, {0,0,0}};
    ...
    void Assembler::op(int opcode, int opcode_ext, GP64 dst, int imm) {
        ...
        this->byte(opcode);
        this->byte(_233(0b11, opcode_ext, dst));  // Not sure if this is ModRM or what 0b11 means.
    ...
    void Assembler::vpaddd(Ymm dst, Ymm x, Ymm y) {
        VEX prefix = vex(0/*unsure what this means here*/, dst>>3, 0, x>>3,
                         map_select(0x0f), y, 1/*ymm?*/, opcode_prefix(0x66));
        this->byte(prefix.bytes, prefix.len);
        this->byte(0xfe);   // paddd
        this->byte(mod_rm(Mod::Direct, dst, x));
    }
    void Assembler::vpsubd(Ymm dst, Ymm x, Ymm y) {
        X->vpsubd(Xbyak::Ymm(dst), Xbyak::Ymm(x), Xbyak::Ymm(y));
    }
    void Assembler::vpmulld(Ymm dst, Ymm x, Ymm y) {
        X->vpmulld(Xbyak::Ymm(dst), Xbyak::Ymm(x), Xbyak::Ymm(y));
    }
    ...
    static int jit(Assembler& a,
        ...
        auto ar = [](int ix) { return (A::Ymm)ix; };
        auto  r = [](int ix) { return Xbyak::Ymm(ix); };
