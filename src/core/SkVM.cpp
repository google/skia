...
namespace skvm {
    ...
    Program Builder::done() {
        ...
        // Pass through any negative IDs directly; they identify Args, not values.
    ...
    void Builder::store8 (Arg ptr, I32 val) { (void)this->push(Op::store8 , ptr.id, val.id); }
    void Builder::store32(Arg ptr, I32 val) { (void)this->push(Op::store32, ptr.id, val.id); }

    I32 Builder::load8 (Arg ptr) { return {this->push(Op::load8 , ptr.id) }; }
    I32 Builder::load32(Arg ptr) { return {this->push(Op::load32, ptr.id) }; }
