...
namespace skvm {
    ...
    bool Program::jit(const std::vector<OptimizedInstruction>& instructions, Assembler* a) const {
        ...
        // Map Val -> Reg, with sentinel NoReg  meaning no register.
        const int NoReg = -1;
        std::vector<int> val_to_reg(instructions.size(), NoReg);

        // Map Reg -> Val, with sentinel NA indicating a temporary value or an unused register.
        std::vector<Val> reg_to_val(32, NA);
        ...
        auto emit = [&](Val id, bool scalar) {
            ...
            // Allocate a register, distinct for the duration of this instruction.
            auto alloc_reg = [&]() -> Reg {
            ...
            switch (op) {
                ...
                // TODO: restore special cases that don't need vmovdqa
                case Op::fma_f32:  a->vmovdqa     (dst(), r(x));
                                   a->vfmadd132ps (dst(), r(z), r(y));
                                   break;
                case Op::fms_f32:  a->vmovdqa     (dst(), r(x));
                                   a->vfmsub132ps (dst(), r(z), r(y));
                                   break;
                case Op::fnma_f32: a->vmovdqa     (dst(), r(x));
            ...
            while (int found = __builtin_ffs(used)) {
                Reg r = (Reg)(found-1);
                used  &= ~(1<<r);
                avail |=  (1<<r);
