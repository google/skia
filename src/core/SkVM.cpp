        ...
        auto emit = [&](Val id, bool scalar) {
            ...
            switch (op) {
                ...
                case Op::load32: if (scalar) { a->vmovd  ((A::Xmm)r[id], arg[imm]); }
                                 else        { a->vmovups(        r[id], arg[imm]); } break;
                ...
                case Op::extract: if (imm == 0)      { a->vpand (r[id], r[x], r[y]); } else
                                  if (pick(~0,&tmp)) { a->vpsrld(tmp, r[x], imm);
                                                       a->vpand (r[id], tmp, r[y]);
                                                       mark_avail(tmp); }
                                  else { return false; }
                                  break;

                case Op::pack: if (pick(~0,&tmp)) { a->vpslld(tmp, r[y], imm);
                                                    a->vpor  (r[id], tmp, r[x]);
                                                    mark_avail(tmp); }
                               else { return false; }
                               break;
