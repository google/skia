...
namespace skvm {
    ...
    std::vector<OptimizedInstruction> Builder::optimize(bool for_jit) const {
        ...
        for (int i = 0; i < (int)fProgram.size(); i++) {
            ...
            Op lhs_fuse, rhs_fuse;
            if (fma_supported()) {
                switch (inst.op) {
                    default: break;

                    case Op::add_f32: lhs_fuse = Op::fma_f32; rhs_fuse = Op:: fma_f32; goto fuse;
                    case Op::sub_f32: lhs_fuse = Op::fms_f32; rhs_fuse = Op::fnma_f32; goto fuse;
                    fuse: {
                        if (auto mul = fProgram[inst.x];
                                mul.op == Op::mul_f32 && use_count[inst.x] == 1) {
                            inst.op = lhs_fuse;
                            inst.z  = inst.y;
                            inst.x  = mul.x;
                            inst.y  = mul.y;
                        ...
                        if (auto mul = fProgram[inst.y];
                                mul.op == Op::mul_f32 && use_count[inst.y] == 1) {
                            inst.op = rhs_fuse;
                            inst.z  = inst.x;
                            inst.x  = mul.x;
                            inst.y  = mul.y;
