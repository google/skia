...
namespace skvm {
    ...
    bool Program::jit(const std::vector<Builder::Instruction>& instructions,
        ...
        SkTHashMap<int, LabelAndReg> bytes_masks;  // These vary per-lane.
        ...
        auto emit = [&](Val id, bool scalar) {
            ...
            switch (op) {
                ...
                case Op::splat: if (imm) { a->vbroadcastss(dst(), &constants[imm]); }
                ...
                case Op::mul_f32: {
                    int imm;
                    if (is_imm(y, &imm)) { a->vmulps(dst(), r[x], &constants[imm]); break; }
                    if (is_imm(x, &imm)) { a->vmulps(dst(), r[y], &constants[imm]); break; }
                ...
                case Op::splat: if (imm) { a->ldrq(dst(), &constants[imm]); }
