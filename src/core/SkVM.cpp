...
namespace skvm {
    ...
    static void dump_builder_program(const std::vector<Builder::Instruction>& program,
        ...
        for (Val id = 0; id < (Val)program.size(); id++) {
            ...
            switch (op) {
                ...
                case Op::extract: write(o, V{id}, "= extract", V{x}, Shift{immz}, V{y}); break;
    ...
    void Program::dump(SkWStream* o) const {
        ...
        for (int i = 0; i < (int)fInstructions.size(); i++) {
            ...
            switch (op) {
                ...
                case Op::extract: write(o, R{d}, "= extract", R{x}, Shift{immz}, R{y}); break;
    ...
    bool Builder::isSplat(Val id, int* imm) const {
        if (fProgram[id].op == Op::splat) {
            *imm = fProgram[id].immy;
            return true;
        }
        return false;
    }
    ...
    F32 Builder::mul(F32 x, F32 y) {
        ...
        if (this->isSplat(x.id, &imm)) { return {this->push(Op::mul_f32_imm, y.id,NA,NA, imm)}; }
    #endif
    ...
    I32 Builder::extract(I32 x, int bits, I32 y) {
        return {this->push(Op::extract, x.id,y.id,NA, 0,bits)};
    }
        ...
        for ( ; n > 0; start = fLoop, n -= stride, step_args(stride)) {
            ...
            for (int i = start; i < (int)fInstructions.size(); i++) {
                ...
                #define CASE(op) case 2*(int)op: /*fallthrough*/ case 2*(int)op+1
                    ...
                    CASE(Op::extract): r(d).u32 = (r(x).u32 >> immz) & r(y).u32; break;
    ...
    bool Program::jit(const std::vector<Builder::Instruction>& instructions,
        ...
        auto emit = [&](Val id, bool scalar) {
            ...
            switch (op) {
                ...
                case Op::extract: if (immz == 0) { a->vpand (dst(),  r[x], r[y]); }
                                  else           { a->vpsrld(tmp(),  r[x], immz);
                                                   a->vpand (dst(), tmp(), r[y]); }
                                  break;
                ...
                case Op::extract: if (immz) { a->ushr4s(tmp(),  r[x], immz);
