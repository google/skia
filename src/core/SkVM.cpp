...
namespace skvm {
    ...
    static void graph(const Uses& uses, const std::vector<Builder::Instruction>& instructions) {

        static std::unique_ptr<SkWStream> o{
                new SkFILEWStream{SkStringPrintf("/Users/herb/dump-graph.txt").c_str()}};
        ...
        o->writeText("Graph[ {");
        for (Val i = 0; i < (int)instructions.size(); i++) {
            SkSpan<const Val> edges = uses.users(i);
            if (!edges.empty()) {
                o->writeText("\n");
                for (int head : edges) {
                    vertex(i);
