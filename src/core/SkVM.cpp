...
namespace skvm {
    ...
    #ifdef SK_DEBUG
        int imm;
        if (allImm(cond.id,&imm)) { SkASSERT(imm); return; }
    ...
    F32 Builder::add(F32 x, F32 y) {
        if (float X,Y; allImm(x.id,&X, y.id,&Y)) { return splat(X+Y); }
        if (isImm(y.id, 0.0f)) { return x; }   // x+0 == x
        if (isImm(x.id, 0.0f)) { return y; }   // 0+y == y
    ...
    F32 Builder::mul(F32 x, F32 y) {
        if (float X,Y; allImm(x.id,&X, y.id,&Y)) { return splat(X*Y); }
        if (isImm(y.id, 1.0f)) { return x; }  // x*1 == x
        if (isImm(x.id, 1.0f)) { return y; }  // 1*y == y
    ...
    F32 Builder::sqrt(F32 x) {
        if (float X; allImm(x.id,&X)) { return splat(std::sqrt(X)); }
    ...
    F32 Builder::min(F32 x, F32 y) {
        if (float X,Y; allImm(x.id,&X, y.id,&Y)) { return splat(std::min(X,Y)); }
        return {this, push(Op::min_f32, x.id, y.id)};
    }
    F32 Builder::max(F32 x, F32 y) {
        if (float X,Y; allImm(x.id,&X, y.id,&Y)) { return splat(std::max(X,Y)); }
        return {this, push(Op::max_f32, x.id, y.id)};
    }

    I32 Builder::add(I32 x, I32 y) {
        if (int X,Y; allImm(x.id,&X, y.id,&Y)) { return splat(X+Y); }
        if (isImm(x.id, 0)) { return y; }
        if (isImm(y.id, 0)) { return x; }
        return {this, push(Op::add_i32, x.id, y.id)};
    }
    I32 Builder::sub(I32 x, I32 y) {
        if (int X,Y; allImm(x.id,&X, y.id,&Y)) { return splat(X-Y); }
        if (isImm(y.id, 0)) { return x; }
        return {this, push(Op::sub_i32, x.id, y.id)};
    }
    I32 Builder::mul(I32 x, I32 y) {
        if (int X,Y; allImm(x.id,&X, y.id,&Y)) { return splat(X*Y); }
        if (isImm(x.id, 0)) { return splat(0); }
        if (isImm(y.id, 0)) { return splat(0); }
        if (isImm(x.id, 1)) { return y; }
        if (isImm(y.id, 1)) { return x; }
        return {this, push(Op::mul_i32, x.id, y.id)};
    }
    ...
    I32 Builder::shl(I32 x, int bits) {
        if (bits == 0) { return x; }
        if (int X; allImm(x.id,&X)) { return splat(X << bits); }
        return {this, push(Op::shl_i32, x.id,NA,NA, bits)};
    }
    I32 Builder::shr(I32 x, int bits) {
        if (bits == 0) { return x; }
        if (int X; allImm(x.id,&X)) { return splat(unsigned(X) >> bits); }
    ...
    I32 Builder:: eq(F32 x, F32 y) {
        if (float X,Y; allImm(x.id,&X, y.id,&Y)) { return splat(X==Y ? ~0 : 0); }
    ...
    I32 Builder::lt(F32 x, F32 y) {
        if (float X,Y; allImm(x.id,&X, y.id,&Y)) { return splat(Y> X ? ~0 : 0); }
    ...
    I32 Builder:: eq(I32 x, I32 y) {
        ...
        return {this, push(Op:: eq_i32, x.id, y.id)};
    }
    I32 Builder::neq(I32 x, I32 y) {
        return {this, push(Op::neq_i32, x.id, y.id)};
    }
    I32 Builder:: gt(I32 x, I32 y) {
        return {this, push(Op:: gt_i32, x.id, y.id)};
    }
    I32 Builder::gte(I32 x, I32 y) {
        if (x.id == y.id) { return splat(~0); }
        return {this, push(Op::gte_i32, x.id, y.id)};
    }
    ...
    I32 Builder::bit_and(I32 x, I32 y) {
        ...
        if (int X,Y; allImm(x.id,&X, y.id,&Y)) { return splat(X&Y); }
        if (isImm(y.id, 0)) { return splat(0); }   // (x & false) == false
        if (isImm(x.id, 0)) { return splat(0); }   // (false & y) == false
        if (isImm(y.id,~0)) { return x; }          // (x & true) == x
        if (isImm(x.id,~0)) { return y; }          // (true & y) == y
        return {this, push(Op::bit_and, x.id, y.id)};
    }
    I32 Builder::bit_or(I32 x, I32 y) {
        if (x.id == y.id) { return x; }
        if (int X,Y; allImm(x.id,&X, y.id,&Y)) { return splat(X|Y); }
        if (isImm(y.id, 0)) { return x; }           // (x | false) == x
        if (isImm(x.id, 0)) { return y; }           // (false | y) == y
        if (isImm(y.id,~0)) { return splat(~0); }   // (x | true) == true
        if (isImm(x.id,~0)) { return splat(~0); }   // (true | y) == true
    ...
    I32 Builder::select(I32 x, I32 y, I32 z) {
        ...
        if (int X,Y,Z; allImm(x.id,&X, y.id,&Y, z.id,&Z)) { return splat(X?Y:Z); }
        if (isImm(x.id,~0)) { return   y; } // true  ? y : z == y
        if (isImm(x.id, 0)) { return   z; } // false ? y : z == z
        if (isImm(z.id, 0)) { return x&y; } //     x ? y : 0 == x&y
    ...
    I32 Builder::extract(I32 x, int bits, I32 z) {
        if (unsigned Z; allImm(z.id,&Z) && (~0u>>bits) == Z) { return this->shr(x, bits); }
        return bit_and(z, this->shr(x, bits));
    }

    I32 Builder::pack(I32 x, I32 y, int bits) {
        if (int X,Y; allImm(x.id,&X, y.id,&Y)) { return splat(X|(Y<<bits)); }
        return {this, push(Op::pack, x.id,y.id,NA, 0,bits)};
    }

    F32 Builder::floor(F32 x) {
        if (float X; allImm(x.id,&X)) { return splat(floorf(X)); }
        return {this, push(Op::floor, x.id)};
    }
    F32 Builder::to_f32(I32 x) {
        if (int X; allImm(x.id,&X)) { return splat((float)X); }
        return {this, push(Op::to_f32, x.id)};
    }
    I32 Builder::trunc(F32 x) {
        if (float X; allImm(x.id,&X)) { return splat((int)X); }
        return {this, push(Op::trunc, x.id)};
    }
    I32 Builder::round(F32 x) {
        if (float X; allImm(x.id,&X)) { return splat((int)lrintf(X)); }
