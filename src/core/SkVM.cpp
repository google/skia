    ...
    bool Program::jit(const std::vector<OptimizedInstruction>& instructions,
        ...
        using Reg = A::Ymm;
            ...
            auto enter = [&]{
                ...
                SkDebugf("\nsaved %d*K*4 for vals, total %d\n", nstack_slots, nstack_slots*K*4 + 10*16);
            };
            auto exit  = [&]{
                for (int r = 6; r < 16; r++) {
                    if (REServed_regs_to_restore & (1<<r)) {
                        SkDebugf("restoring xmm%d from rsp+%d\n",r, nstack_slots*K*4 + (r-6)*16);
            ...
            auto alloc_tmp = [&]() -> Reg {
                ...
                if (v == RES) {
                    ...
                    if ((REServed_regs_to_restore & (1<<r)) == 0) {
                        SkDebugf("'saving' xmm%d to rsp+%d\n", r, nstack_slots*K*4 + (r-6)*16);
                        REServed_regs_to_restore |= (1<<r);
                    } else {
                        SkDebugf("reusing reserved register ymm%d\n", r);
                    }
