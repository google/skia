...
namespace skvm {
    ...
    // TODO: create a RegSet type to handle all the bit masking.
    ...
    bool Program::jit(const std::vector<Builder::Instruction>& instructions, Assembler* a) const {
        ...
        auto emit = [&](Val id, bool scalar) {
            ...
            auto mark_avail = [&](A::Ymm reg) {
                avail |= 1 << reg;
            };

            // Mark any dying input registers available.
            // TODO: this is wrong... it's okay to assume these registers are free for the purposes
            // of picking r[id], but not for picking tmp.
            if (x != NA && instructions[x].death == id) { mark_avail(r[x]); }
            if (y != NA && instructions[y].death == id) { mark_avail(r[y]); }
            if (z != NA && instructions[z].death == id) { mark_avail(r[z]); }

            // Reserve any available register, following preferences if possible.
            auto pick = [&](uint32_t prefs, A::Ymm* reg) {
                int found = __builtin_ffs(avail & prefs);
                if (found == 0) {
                    found = __builtin_ffs(avail);
                }
                if (found > 0) {
                    *reg = (A::Ymm)(found - 1);
                    avail ^= 1 << *reg;
            ...
            // All ops that are not stores need a register to store their result.
            if (op > Op::store32) {

                // The AVX2 backend is pretty simple in terms of register preference...
                // the only instructions that can really benefit from it are vfmadd???ps.
                uint32_t prefs = ~0;
                if (op == Op::mad_f32) {
                    prefs = (1<<r[x])   // vfmadd132ps
                          | (1<<r[y])   // vfmadd213ps
                          | (1<<r[z]);  // vfmadd231ps
                }
                ...
                if (pick(prefs, &dst)) {
                    ...
                    SkDebugf("\t no registers left\n");
                    return false;
                }
            }

            // TODO: there are lots of situations where r[id] and tmp can
            // safely overlap but that won't happen as written today.
            ...
            switch (op) {
                case Op::store8:
                    if (scalar) {
                        ...
                        a->vpackusdw(tmp, r[x], r[x]);         // pack 32-bit -> 16-bit
                        a->vpermq   (tmp, tmp, 0xd8);          // 64-bit tmp[0,1;2,3] = tmp[0,2;1,3]
                        a->vpackuswb(tmp, tmp, tmp);           // pack 16-bit -> 8-bit
                ...
                case Op::mad_f32:
                    if (r[id] == r[x])  { a->vfmadd132ps(r[x], r[z], r[y]); } else
                    if (r[id] == r[y])  { a->vfmadd213ps(r[y], r[x], r[z]); } else
                    if (r[id] == r[z])  { a->vfmadd231ps(r[z], r[x], r[y]); } else
                    if (pick(~0, &tmp)) { a->vmulps(tmp, r[x], r[y]);
                ...
                case Op::extract:
                    if (imm == 0) {
                        a->vpand (r[id], r[x], r[y]);
                    } else if (pick(~0, &tmp)) {
                        a->vpsrld(tmp, r[x], imm);
        ...
        a->label(&body); {
            ...
            a->jl(&tail);
            for (Val id = 0; id < (Val)instructions.size(); id++) {
                if (!emit(id, /*scalar=*/K==1)) {
                    return false;
                }
            }
            for (int i = 0; i < (int)fStrides.size(); i++) {
                a->add(arg[i], K*fStrides[i]);
            }
            a->sub(N, K);
            a->jmp(&body);
        }
        ...
        if (vpshufb_masks.count() > 0) {
            a->align(32);
            vpshufb_masks.foreach([&](int imm, A::Label* l) {
                a->label(l);
                int mask[4];
                bytes_control(imm, mask);

                // Same patern for bottom 4 and top 4 32-bit lanes.
    ...
    void Program::setupJIT(const std::vector<Builder::Instruction>& instructions,
        ...
        // Run first wih no buffer to determine a.size(), the number of bytes we'll assemble.
        Assembler a{nullptr};
        ...
        this->dumpJIT(debug_name);
    #endif
    }
#endif

#if defined(SKVM_PERF_DUMPS)
    void Program::dumpJIT(const char* debug_name) const {
    #if defined(__aarch64__)
        // cat | llvm-mc -arch aarch64 -disassemble
        auto cur = (const uint8_t*)fJITBuf;
        for (int i = 0; i < (int)fJITSize; i++) {
            if (i % 4 == 0) {
                SkDebugf("\n");
            }
            SkDebugf("0x%02x ", *cur++);
