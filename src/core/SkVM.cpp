    ...
    std::vector<OptimizedInstruction> Builder::optimize(bool for_jit) const {
        ...
        auto pressure_change = [&](Val id) -> int {
          ...
          Builder::Instruction inst = program[id];
            ...
            if (inst.x != NA && use_counts[inst.x].uses == 1) { pressure -= 1; }
            if (inst.y != NA && use_counts[inst.y].uses == 1) { pressure -= 1; }
            if (inst.z != NA && use_counts[inst.z].uses == 1) { pressure -= 1; }
            return pressure;
        };
        ...
        auto compare = [&](const Val& lhs, const Val& rhs) {
            ...
            return pressure_change(lhs) < pressure_change(rhs);
        };
