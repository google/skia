...
namespace skvm {
    ...
    bool Program::jit(const std::vector<OptimizedInstruction>& instructions, Assembler* a) const {
        ...
        // Map Val -> Reg, with NA meaning no register.
        std::vector<int> val_to_reg(instructions.size(), NA);

        // Map Reg -> Val, with NA meaning a temporary value or an unused register.
        Val reg_to_val[32];
        for (Val& v : reg_to_val) {
            v = NA;
        }
        ...
        auto emit = [&](Val id, bool scalar) {
            ...
            // Allocate a register for the duration of this instruction.
            auto alloc_reg = [&]() -> Reg {
                int found = __builtin_ffs(avail);
                SkASSERT(found > 0);
                Reg r = (Reg)(found-1);
                avail &= ~(1<<r);
                used  |=  (1<<r);
            ...
            auto make_available = [&](int r) {
                SkASSERT(r != NA);
                if (Val v = reg_to_val[r]; v != NA) {
                    // If this register holds a value we'll need again, spill it to the stack.
                    if (instructions[v].death > id) {
            ...
            if (true) {
                while (used) {
                    Reg r = (Reg)(__builtin_ffs(used) - 1);
                    used &= ~(1<<r);
                    make_available(r);
                }
                return true;
            }
            // TODO: there's some sort of bug past here that's leading to asserts at runtime.

            // Mark any registers holding temporaries as available.  (We MUST do this.)
            while (used) {
                Reg r = (Reg)(__builtin_ffs(used) - 1);
                used &= ~(1<<r);
            ...
            // With temporaries freed, all live registers are now tracked by reg_to_val.

            // Free any dying value.  (Never a downside to doing this.)
            for (Val v : reg_to_val) {
                if (v != NA && instructions[v].death == id) {
                    make_available(val_to_reg[v]);
                }
            }

            // Proactively spill hoisted values as they're produced.
            // We've already gotten most of our value from the hoisting itself.
            if (val_to_reg[id] != NA && can_hoist) {
                make_available(val_to_reg[id]);
            }

            // Make sure there are enough registers left for the next instruction.
            const int enough = 4;   // TODO: 3 might work too?
            if (int N = enough - __builtin_popcount(avail); N > 0) {
                Val live[SK_ARRAY_COUNT(reg_to_val)];
                auto end = std::copy_if(std::begin(reg_to_val), std::end(reg_to_val),
                                        live, [&](Val v) { return v != NA; });

                // Heursitically, spill what will die latest.
                // This isn't quite ideal (that'd be, spill what's next used latest from now)
                // but it does mean that values which die soon are unlikely to be spilled.
                std::partial_sort(live, live+N, end, [&](Val x, Val y) {
                    return instructions[x].death > instructions[y].death;
                });
                for (int i = 0; i < N; i++) {
                    make_available(val_to_reg[live[i]]);
                }
            }
            return true;
        };
