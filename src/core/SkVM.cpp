...
// JIT code isn't MSAN-instrumented, so we won't see when it uses uninitialized memory,
// and we'll not see the writes it makes as properly initialzing memory.  Instead force
// the interpreter, which should let MSAN see everything our programs do properly.
