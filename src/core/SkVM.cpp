...
namespace skvm {
    ...
    #ifdef SK_DEBUG
        int imm;
        if (this->isImm(val.id,&imm)) { SkASSERT(imm); return; }
    ...
    F32 Builder::add(F32 x, F32 y) {
        float X,Y;
        if (this->isImm(x.id,&X, y.id,&Y)) { return this->splat(X+Y); }
    ...
    F32 Builder::mad(F32 x, F32 y, F32 z) {
        float X,Y,Z;
        if (this->isImm(x.id,&X, y.id,&Y, z.id,&Z)) { return this->splat(X*Y+Z); }
    ...
    F32 Builder::min(F32 x, F32 y) {
        float X,Y;
        if (this->isImm(x.id,&X, y.id,&Y)) { return this->splat(std::min(X,Y)); }
        return {this->push(Op::min_f32, x.id, y.id)};
    }
    F32 Builder::max(F32 x, F32 y) {
        float X,Y;
        if (this->isImm(x.id,&X, y.id,&Y)) { return this->splat(std::max(X,Y)); }
    ...
    I32 Builder::shl(I32 x, int bits) {
        int X;
        if (this->isImm(x.id,&X)) { return this->splat(X << bits); }
        return {this->push(Op::shl_i32, x.id,NA,NA, bits)};
    }
    I32 Builder::shr(I32 x, int bits) {
        int X;
        if (this->isImm(x.id,&X)) { return this->splat(unsigned(X) >> bits); }
    ...
    I32 Builder:: eq(F32 x, F32 y) {
        float X,Y;
        if (this->isImm(x.id,&X, y.id,&Y)) { return this->splat(X==Y ? ~0 : 0); }
    ...
    I32 Builder::lt(F32 x, F32 y) {
        float X,Y;
        if (this->isImm(x.id,&X, y.id,&Y)) { return this->splat(Y> X ? ~0 : 0); }
    ...
    I32 Builder::extract(I32 x, int bits, I32 z) {
        int X,Z;
        if (this->isImm(x.id,&X, z.id,&Z)) { return this->splat( (unsigned(X)>>bits)&Z ); }
        return {this->push(Op::extract, x.id,NA,z.id, bits,0)};
    }

    I32 Builder::pack(I32 x, I32 y, int bits) {
        int X,Y;
        if (this->isImm(x.id,&X, y.id,&Y)) { return this->splat(X|(Y<<bits)); }
    ...
    F32 Builder::to_f32(I32 x) {
        int X;
        if (this->isImm(x.id,&X)) { return this->splat((float)X); }
        return {this->push(Op::to_f32, x.id)};
    }
    I32 Builder::trunc(F32 x) {
        float X;
        if (this->isImm(x.id,&X)) { return this->splat((int)X); }
        return {this->push(Op::trunc, x.id)};
    }
    I32 Builder::round(F32 x) {
        float X;
        if (this->isImm(x.id,&X)) { return this->splat((int)lrintf(X)); }
