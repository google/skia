        ...
        auto emit = [&](Val id, bool scalar) {
            ...
            switch (op) {
                ...
                case Op::gather32: {
                        ...
                        // but we can't treat it as available during this instruction.
                        A::Ymm index = r[x];
                        uint32_t avail_for_gather = avail & ~(1<<index);

                        // Choose dst() to not overlap with index.
                        if (int found = __builtin_ffs(avail_for_gather)) {
                            set_dst((A::Ymm)(found-1));
                            avail_for_gather ^= (1<<dst());
                        } else {
                            ok = false;
                            break;
                        }
