...
SkShaderBase::GradientType SkColorShader::asAGradient(GradientInfo* info) const {
    if (info) {
...
SkShaderBase::GradientType SkColor4Shader::asAGradient(GradientInfo* info) const {
    if (info) {
        if (info->fColors && info->fColorCount >= 1) {
