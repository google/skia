...
void SkRRect::setRectPoints(const SkRect& rect, const SkPoint pts[4]) {
    fRect = rect;
...
static bool radii_are_nine_patch(const SkVector radii[4]) {
    return SkScalarNearlyEqual(radii[SkRRect::kUpperLeft_Corner].fX,
                               radii[SkRRect::kLowerLeft_Corner].fX) &&
           SkScalarNearlyEqual(radii[SkRRect::kUpperLeft_Corner].fY,
                               radii[SkRRect::kUpperRight_Corner].fY) &&
           SkScalarNearlyEqual(radii[SkRRect::kUpperRight_Corner].fX,
                               radii[SkRRect::kLowerRight_Corner].fX) &&
           SkScalarNearlyEqual(radii[SkRRect::kLowerLeft_Corner].fY,
...
static SkRRect::Type compute_alt_type(const SkRect& r, const SkPoint pts[4]) {
    const SkScalar tol = 0.00001f;
    ...
    {
        // our 4 corners form a rectangle

        if (SkScalarNearlyEqual(pts[0].fX, r.fLeft, tol) &&
            SkScalarNearlyEqual(pts[0].fY, r.fTop, tol) &&
            SkScalarNearlyEqual(pts[2].fX, r.fRight, tol) &&
            SkScalarNearlyEqual(pts[2].fY, r.fBottom, tol)) {
            return SkRRect::kRect_Type;
        }

        SkScalar L = pts[0].fX - r.fLeft;
        SkScalar T = pts[0].fY - r.fTop;
        SkScalar R = r.fRight - pts[2].fX;
        SkScalar B = r.fBottom - pts[2].fY;
        ...
        if (SkScalarNearlyEqual(L, R, tol) && SkScalarNearlyEqual(T, B, tol)) {
            if (SkScalarNearlyEqual(pts[0].fX, pts[2].fX, tol) &&
                ...
                return SkRRect::kOval_Type;
            } else {
                return SkRRect::kSimple_Type;
            }
        } else {
            return SkRRect::kNinePatch_Type;
        }
    }
    return SkRRect::kComplex_Type;
}
...
bool SkRRect::ComputeType(const SkRect& rect, const SkPoint pts[4], Type* type) {
    ...
    SkRRect::Type altType = compute_alt_type(rect, pts);
    RETURN_VALID_TYPE(altType);
    ...
    for (int i = 1; i < 4; ++i) {
        ...
        if (!SkScalarNearlyEqual(radii[i].fX, radii[i-1].fX) ||
            !SkScalarNearlyEqual(radii[i].fY, radii[i-1].fY)) {
            allRadiiEqual = false;
        }
    }

    if (allCornersSquare) {
        if (altType != kRect_Type) {
            compute_alt_type(rect, pts);
        }
        SkASSERT(altType == kRect_Type);
    ...
    if (allRadiiEqual) {
        if (radii[0].fX >= SkScalarHalf(rect.width()) &&
            ...
            SkASSERT(altType == kOval_Type);
            RETURN_VALID_TYPE(kOval_Type);
        } else {
            if (altType != kSimple_Type) {
                compute_alt_type(rect, pts);
            }
            SkASSERT(altType == kSimple_Type);
            RETURN_VALID_TYPE(kSimple_Type);
        }
    }

    if (radii_are_nine_patch(radii)) {
        SkASSERT(altType == kNinePatch_Type);
        RETURN_VALID_TYPE(kNinePatch_Type);
    } else {
        SkASSERT(altType == kComplex_Type);
