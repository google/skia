...
void SkRRect::computeType() {
    struct Validator {
        ...
        ~Validator() { SkASSERT(fR->validate()); }
        const SkRRect* fR;
    } autoValidate(this);
 ...
 */
static bool validate_radius_check_predicates(SkScalar rad, SkScalar min, SkScalar max) {
    return (min <= max) && (rad <= max - min) && (min + rad <= max) && (max - rad >= min);
}

bool SkRRect::validate() const {
    ...
    for (int i = 0; i < 4; ++i) {
        if (!validate_radius_check_predicates(fRadii[i].fX, fRect.fLeft, fRect.fRight) ||
