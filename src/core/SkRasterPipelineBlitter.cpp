...
void SkRasterPipelineBlitter::blitMask(const SkMask& mask, const SkIRect& clip) {
    ...
    // Update fMaskPtr to point into this current mask.
    switch (mask.fFormat) {
        case SkMask::kA8_Format:
            fMaskPtr.pixels = mask.fImage;
    ...
    if (fBurstCtx) {
        ...
        for (int y = clip.top(); y < clip.bottom(); y++) {
            ...
            switch (mask.fFormat) {
                case SkMask::kA8_Format:
                    fBlitMaskA8   (x,y, clip.width(),1);
                    break;
                case SkMask::kLCD16_Format:
                    fBlitMaskLCD16(x,y, clip.width(),1);
                    break;
                default:
                    break;
            }
        ...
        switch (mask.fFormat) {
            case SkMask::kA8_Format:
                fBlitMaskA8   (clip.left(),clip.top(), clip.width(),clip.height());
                break;
            case SkMask::kLCD16_Format:
                fBlitMaskLCD16(clip.left(),clip.top(), clip.width(),clip.height());
                break;
            default:
                break;
        }
