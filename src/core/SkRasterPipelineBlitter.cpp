...
SkBlitter* SkRasterPipelineBlitter::Create(const SkPixmap& dst,
    ...
    if (clipShader) {
        ...
        SkColorType clipCT = kAlpha_8_SkColorType;
        SkColorSpace* clipCS = nullptr;
        SkMatrix clipM = SkMatrix::I();
        if (as_SB(clipShader)->appendStages({
            clipP, alloc, clipCT, clipCS, clipPaint, nullptr, clipM
        })) {
            struct Storage {
                // really only need to store alpha, but we don't have that stage yet
                float   fRGBA[4 * SkRasterPipeline_kMaxStride];
            };
            auto storage = alloc->make<Storage>();
            clipP->append(SkRasterPipeline::store_src, storage->fRGBA);
            // save a pointer to the alpha channel, our coverage for clipping
            blitter->fClipShaderBuffer = &storage->fRGBA[3 * SkRasterPipeline_kMaxStride];
        }
    }
    ...
    if (is_constant && blitter->fBlend == SkBlendMode::kSrc && !blitter->fClipShaderBuffer) {
