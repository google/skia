...
private:
    ...
    void append_store (SkRasterPipeline*);

    SkPixmap         fDst;
    SkBlendMode      fBlend;
    SkPM4f           fPaintColor;
    SkRasterPipeline fShader;
    bool             fWillDither = false;
...
SkBlitter* SkRasterPipelineBlitter::Create(const SkPixmap& dst,
    ...
    if (paint.isDither() && dst.info().colorType() == kRGB_565_SkColorType) {
        blitter->fWillDither = true;
    }
    if (shader && shader->asAGradient(nullptr) >= SkShader::kLinear_GradientType &&
            (dst.info().colorType() == kRGB_565_SkColorType ||
             dst.info().colorType() == kRGBA_8888_SkColorType ||
             dst.info().colorType() == kBGRA_8888_SkColorType)) {
        blitter->fWillDither = true;
    }

    bool is_opaque   = paintColor->a() == 1.0f,
         is_constant = !blitter->fWillDither;
    if (shader) {
...
void SkRasterPipelineBlitter::append_store(SkRasterPipeline* p) {
    if (fDst.info().gammaCloseToSRGB()) {
    ...
    if (fWillDither) {
        ...
        switch (fDst.info().colorType()) {
            case   kRGB_565_SkColorType:  fDitherCtx.rate =  1/63.0f; break;
            case kBGRA_8888_SkColorType:
            case kRGBA_8888_SkColorType:  fDitherCtx.rate = 1/255.0f; break;
