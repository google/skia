...
SkBlitter* SkRasterPipelineBlitter::Create(const SkPixmap& dst,
    ...
    if (shader) {
        ...
        if (paintColor->fVec[SkPM4f::A] != 1.0f) {
...
SkBlitter* SkRasterPipelineBlitter::Create(const SkPixmap& dst,
    ...
    bool              is_constant = false;   // we figure a custom shaderPipeline is not constant
    SkPM4f*           paintColor  = &blitter->fPaintColor;
    SkRasterPipeline* pipeline    = &blitter->fShader;
...
void SkRasterPipelineBlitter::finishBuilding(const SkPaint& paint, const SkMatrix& ctm,
    ...
    if ((paint.isDither() && fDst.info().colorType() == kRGB_565_SkColorType) || wants_dither) {
        switch (fDst.info().colorType()) {
            default:                     fDitherCtx.rate =     0.0f; break;
            case   kRGB_565_SkColorType: fDitherCtx.rate =  1/63.0f; break;
            case kRGBA_8888_SkColorType:
            case kBGRA_8888_SkColorType: fDitherCtx.rate = 1/255.0f; break;
        }
    }
