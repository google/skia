...
SkBlitter* SkRasterPipelineBlitter::Create(const SkPixmap& dst,
    ...
    auto blend      = &blitter->fBlend;
    auto paintColor = &blitter->fPaintColor;
    auto pipeline   = &blitter->fShader;

    auto shader      = paint.getShader();
    auto colorFilter = paint.getColorFilter();
    ...
    bool opaque, constant;
    if (shader) {
        opaque   = shader->isOpaque();
        constant = false;
        // TODO: append shader stages, of course!
    } else {
        opaque   = paintColor->a() == 1.0f;
        constant = true;
        pipeline->append(SkRasterPipeline::constant_color, paintColor);
    }

    if (colorFilter) {
        if (!colorFilter->appendStages(pipeline, opaque)) {
            alloc->freeLast();
            return nullptr;
        }
        opaque = opaque && (colorFilter->getFlags() & SkColorFilter::kAlphaUnchanged_Flag);
    }

    if (opaque && *blend == SkBlendMode::kSrcOver) {
