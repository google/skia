...
void SkRasterPipelineBlitter::blitMask(const SkMask& mask, const SkIRect& clip) {
    ...
    // We'll always use fMaskPtr for the first (or usually, only) mask plane.
    ...
    if (mask.fFormat == SkMask::k3D_Format && !fBlitMask3D) {
        ...
        {  // This bit is where we differ from kA8_Format:
            auto ctx = fAlloc->make<SkRasterPipeline_EmbossCtx>();
            extract_mask_plane(1, &ctx->mul);
            extract_mask_plane(2, &ctx->add);
            p.append(SkRasterPipeline::emboss, ctx);
