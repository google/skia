...
SkBlitter* SkRasterPipelineBlitter::Create(const SkPixmap& dst,
    ...
    if (shaderCtx) {
        colorPipeline->append(SkRasterPipeline::load_f32, &blitter->fShaderBufferPtr);
    } else {
...
void SkRasterPipelineBlitter::maybe_shade(int x, int y, int w) {
    if (fShaderCtx) {
        ...
        // We'll be reading from fShaderBuffer + x.
        fShaderBufferPtr = fShaderBuffer.data() - x;
    }
}
