...
SkBlitter* SkCreateRasterPipelineBlitter(const SkPixmap& dst,
                                         const SkPaint& paint,
                                         const SkMatrix& ctm,            // TODO: unused, remove
                                         const SkRasterPipeline& shader,
                                         bool is_opaque,
                                         bool wants_dither,
                                         SkArenaAlloc* alloc) {
    ...
    return SkRasterPipelineBlitter::Create(dst, paint, alloc,
                                           shader, is_opaque, is_constant, wants_dither);
}

static void collapse_into_constant_color(SkRasterPipeline* pipeline, SkPM4f* color) {
    pipeline->append(SkRasterPipeline::store_f32, &color);
    pipeline->run(0,1);
    *pipeline = SkRasterPipeline();
    pipeline->append(SkRasterPipeline::constant_color, color);
}
...
SkBlitter* SkRasterPipelineBlitter::Create(const SkPixmap& dst,
    ...
    if (auto shader = paint.getShader()) {
        ...
        if (is_constant) {
            SkASSERT(!wants_dither);  // That'd be weird.
            collapse_into_constant_color(&shaderPipeline, paintColor);
    ...
    bool is_opaque    = paintColor->() == 1.0f,
         is_constant  = true,
         wants_dither = false;
    return Create(dst, paint, alloc, shaderPipeline, is_opaque, is_constant, wants_dither);
}
...
SkBlitter* SkRasterPipelineBlitter::Create(const SkPixmap& dst,
    ...
    if (is_constant) {
        ...
        collapse_into_constant_color(colorPipeline, constantColor);
    ...
    if (is_constant && blitter->fBlend == SkBlendMode::kSrc) {
        // Run our color pipeline all the way through to write in the dst color format.
        // This is a bit like collapse_into_constant_color(), but it doesn't change colorPipeline.
        SkRasterPipeline p;
