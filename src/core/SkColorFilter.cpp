    ...
    std::unique_ptr<GrFragmentProcessor> asFragmentProcessor(
        ...
        public:
            ...
            std::unique_ptr<GrFragmentProcessor> clone() const override {
                SkASSERT(this->numChildProcessors() > 0);
                const auto hasFP1 = this->numChildProcessors() > 1;
        ...
        private:
            ...
            static OptimizationFlags OptFlags(const std::unique_ptr<GrFragmentProcessor>& fp0,
                ...
                const auto& GetOptFlags = [] (const std::unique_ptr<GrFragmentProcessor>& fp) {
                    auto flags = fp->compatibleWithCoverageAsAlpha()
                                ? kCompatibleWithCoverageAsAlpha_OptimizationFlag : 0
                         | fp->preservesOpaqueInput()
                                ? kPreservesOpaqueInput_OptimizationFlag : 0
                         | fp->hasConstantOutputForConstantInput()
                                ? kConstantOutputForConstantInput_OptimizationFlag : 0;
                    return static_cast<OptimizationFlags>(flags);
                };

                auto flags = kAll_OptimizationFlags;
                if (fp0) {
                    flags &= GetOptFlags(fp0);
                }
                if (fp1) {
                    flags &= GetOptFlags(fp1);
                }
                return flags;
            }
        ...
        auto cf0FP = fCF0 ? fCF0->asFragmentProcessor(context, dstColorSpaceInfo) : nullptr,
