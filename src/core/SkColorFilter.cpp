...
struct SkColorXformColorFilter : public SkColorFilterBase {
    ...
    void flatten(SkWriteBuffer& buffer) const override {
        buffer.writeBool(fA.useDstTF);
        buffer.writeBool(fA.useDstGamut);
        buffer.writeBool(fA.useDstAT);
        if (!fA.useDstTF)    { buffer.writeScalarArray(&fA.tf.g, 7); }
        if (!fA.useDstGamut) { buffer.writeScalarArray(&fA.gamut.vals[0][0], 9); }
        if (!fA.useDstAT)    { buffer.writeInt(fA.at); }

        buffer.writeBool(fB.useDstTF);
        buffer.writeBool(fB.useDstGamut);
        buffer.writeBool(fB.useDstAT);
        if (!fB.useDstTF)    { buffer.writeScalarArray(&fB.tf.g, 7); }
        if (!fB.useDstGamut) { buffer.writeScalarArray(&fB.gamut.vals[0][0], 9); }
        if (!fB.useDstAT)    { buffer.writeInt(fB.at); }
    }
...
sk_sp<SkColorFilter> SkColorFilters::WithWorkingFormat(sk_sp<SkColorFilter>          child,
    ...
    SkColorXformColorFilter::Format dst = { {},true, {},true, {},true };
    SkColorXformColorFilter::Format working = {
        tf    ? *tf    : skcms_TransferFunction{}, tf    == nullptr,
        gamut ? *gamut : skcms_Matrix3x3{}       , gamut == nullptr,
        at    ? *at    : kUnknown_SkAlphaType    , at    == nullptr,
    };

    // input -> (dst->working) -> child -> (working->dst) -> output
