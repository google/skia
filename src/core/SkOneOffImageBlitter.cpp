...
class SkOneOffImageBlitter final : public SkBlitter {
public:
    static SkBlitter* Create(const SkPixmap& dst,
        ...
        return alloc->make<SkOneOffImageBlitter>(mode, src, dst, dstToSrc);
    }

    SkOneOffImageBlitter(SkBlendMode mode, SkPixmap src, SkPixmap dst, SkMatrix dstToSrc)
        : fMode(mode)
        , fSrc(src)
        , fDst(dst)
        , fDstToSrc(dstToSrc) {}
...
private:
    ...
    const SkPMColor* srcAt(int x, int y, SkFixed* off, SkFixed* dx) const {
        SkPoint xy = fDstToSrc.mapXY(x + 0.5f, y + 0.5f);

        *off = SkScalarToFixed(xy.fX);
        *dx  = SkScalarToFixed(fDstToSrc.getScaleX());

        int sy = SkScalarTruncToInt(SkTPin(xy.fY, (SkScalar)0, (SkScalar)fSrc.height() - 1));
        return fSrc.addr32(0,sy);
    }
...
SkBlitter* SkCreateOneOffImageBlitter(const SkPixmap& dst,
                                      const SkPaint& paint,
                                      const SkMatrix& ctm,
                                      SkArenaAlloc* alloc) {
    return SkOneOffImageBlitter::Create(dst, paint, ctm, alloc);
}

void SkOneOffImageBlitter::blitAntiH_Src(int x, int y,
                                         const SkAlpha aa[], const int16_t runs[]) const {
    SkFixed off, dx;
    const SkPMColor* src = this->srcAt(x,y, &off, &dx);
    uint16_t* dst = fDst.writable_addr16(x,y);

    for (int16_t run = *runs; run > 0; runs += run, run = *runs) {
        for (int i = 0; i < run; i++) {
            SkPMColor s = src[this->clampX(off)];
            dst[i] = SkPixel32ToPixel16(SkFourByteInterp(s, SkPixel16ToPixel32(dst[i]), *aa));
            off += dx;
        }
...
void SkOneOffImageBlitter::blitAntiH_SrcOver(int x, int y,
                                             const SkAlpha aa[], const int16_t runs[]) const {
    SkFixed off, dx;
    const SkPMColor* src = this->srcAt(x,y, &off, &dx);
    uint16_t* dst = fDst.writable_addr16(x,y);
    ...
    for (int16_t run = *runs; run > 0; runs += run, run = *runs) {
        for (int i = 0; i < run; i++) {
            SkPMColor s = src[this->clampX(off)];
            dst[i] = SkSrcOver32To16(SkAlphaMulQ(s, SkAlpha255To256(*aa)), dst[i]);
            off += dx;
        }
...
void SkOneOffImageBlitter::blitH_Src(int x, int y, int w) const {
    SkFixed off, dx;
    const SkPMColor* src = this->srcAt(x,y, &off, &dx);
    uint16_t* dst = fDst.writable_addr16(x,y);
...
void SkOneOffImageBlitter::blitH_SrcOver(int x, int y, int w) const {
    SkFixed off, dx;
    const SkPMColor* src = this->srcAt(x,y, &off, &dx);
    uint16_t* dst = fDst.writable_addr16(x,y);
