...
skvm::Color SkColorSpaceXformSteps::apply(skvm::Builder* p, skvm::Color c, skvm::Uniforms* uniforms,
                                          SkArenaAlloc* alloc, bool src_is_normalized) const {
#if defined(SK_LEGACY_SRGB_STAGE_CHOICE)
    src_is_normalized = true;
#endif
    ...
    if (flags.encode) {
        if (src_is_normalized && dstTF_is_sRGB) {
            ...
            c = transfer_function(p, c, dstTFInv);
        }
    }
