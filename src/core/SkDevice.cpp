...
void SkBaseDevice::drawImageSet(const SkCanvas::ImageSetEntry images[], int cnt, float alpha,
                                SkFilterQuality filterQuality, SkBlendMode) {
    SkPaint paint;
    ...
    for (int i = 0; i < cnt; ++i) {
        ...
        // which turns off anti aliasing unless all four edges have AA. This prevents seaming in
        // tiled layers.
        paint.setAntiAlias(images->fAAFlags == SkCanvas::kAll_QuadAAFlags);
