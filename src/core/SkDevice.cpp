...
void SkBaseDevice::drawEdgeAAQuad(const SkRect& r, const SkPoint clip[], int clipCount,
                                  SkCanvas::QuadAAFlags aa, SkColor color, SkBlendMode mode) {
    SkASSERT(clipCount == 0 || (clip && clipCount == 4));
    ...
    if (clipCount > 0) {
        ...
        SkPath clipPath;
        clipPath.addPoly(clip, clipCount, true);
...
void SkBaseDevice::drawEdgeAAImageSet(const SkCanvas::ImageSetEntry images[], int count,
    ...
    for (int i = 0; i < count; ++i) {
        ...
        entryPaint.setAlpha(SkToUInt(SkTClamp(SkScalarRoundToInt(
                paint.getAlphaf() * images[i].fAlpha * 255), 0, 255)));

        bool needsRestore = false;
        SkASSERT(images[i].fMatrixIndex < 0 || preViewMatrices);
        if (images[i].fMatrixIndex >= 0) {
            this->setGlobalCTM(SkMatrix::Concat(
                    this->ctm(), preViewMatrices[images[i].fMatrixIndex]));
            needsRestore = true;
        }

        SkASSERT(images[i].fDstClipCount == 0 || (dstClips && images[i].fDstClipCount == 4));
        if (images[i].fDstClipCount > 0) {
            // Since drawImageRect requires a srcRect, the dst clip is implemented as a true clip
            this->save();
            SkPath clipPath;
            clipPath.addPoly(dstClips + clipIndex, images[i].fDstClipCount, true);
            this->clipPath(clipPath, SkClipOp::kIntersect, entryPaint.isAntiAlias());
            clipIndex += images[i].fDstClipCount;
            needsRestore = true;
        }
