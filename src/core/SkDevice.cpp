...
void SkBaseDevice::drawPatch(const SkPoint cubics[12], const SkColor colors[4],
    ...
    SkISize lod = SkPatchUtils::GetLevelOfDetail(cubics, &this->getCTM());
...
bool SkBaseDevice::drawExternallyScaledImage(const SkImage* image,
                                             const SkRect* src,
                                             const SkRect& dst,
                                             const SkPaint& paint,
    ...
    this->drawBitmapRect(bm, &rec.fSrcRect, dst, *adjustedPaint, constraint);
...
void SkBaseDevice::drawImage(const SkImage* image, SkScalar x, SkScalar y,
    ...
    if (this->drawExternallyScaledImage(image, nullptr,
...
void SkBaseDevice::drawImageRect(const SkImage* image, const SkRect* src,
                                 ...
                                 SkCanvas::SrcRectConstraint constraint) {
    // Default impl : turns everything into raster bitmap
    if (this->drawExternallyScaledImage(image, src, dst, paint, constraint)) {
        return;
    }
...
void SkBaseDevice::drawTextRSXform(const void* text, size_t len,
    ...
    while ((const char*)text < (const char*)stopText) {
        ...
        SkAutoDeviceConcat adc(this, localM);
