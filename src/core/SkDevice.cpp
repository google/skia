...
void SkBaseDevice::drawSpecial(const SkMatrix&, SkSpecialImage*, int x, int y, const SkPaint&) {}
...
void SkBaseDevice::drawDevice(SkBaseDevice* device, const SkPaint& paint) {
    ...
    if (deviceImage) {
        this->drawSpecial(device->getRelativeTransform(*this), deviceImage.get(), 0, 0, paint);
    }
}
...
void SkBaseDevice::drawFilteredImage(const skif::Mapping& mapping, SkSpecialImage* src,
    ...
    if (result) {
        this->drawSpecial(mapping.deviceMatrix(), result.get(), offset.fX, offset.fY, paint);
    }
}
