...
bool SkBaseDevice::getOrigin(SkIPoint* origin) const {
    ...
    // True if the origin fully describes the state of fToGlobal
    return fToGlobal.isTranslate() && origin->fX == fToGlobal.getTranslateX()
            && origin->fY == fToGlobal.getTranslateY();
}
