...
class SkMakeZipDetail {
    ...
    template<typename C> struct Type<C&> {
...
#ifdef SK_DEBUG
    template<typename C> struct DebugSize {
        static constexpr decltype(std::declval<C>().size(), size_t()) Size(const C& c) {
            return c.size();
        }
    };
    template<typename T, size_t N> struct DebugSize<T(&)[N]> {
        static constexpr size_t Size(const T(&)[N]) { return N; }
    };
    template<typename T> struct DebugSize<T*> {
...
public:
    ...
    static constexpr auto MakeZip(Ts&& ... ts) {
        ...
        size_t size = PickOneSize<Ts...>::Size(std::forward<Ts>(ts)...);

#if 0
        // Check that all sizes are the same.
        size_t minSize = SIZE_MAX;
        size_t maxSize = 0;
        for (size_t s : {DebugSize<typename std::remove_const<Ts>::type>::Size(ts)...}) {
            if (s != SIZE_MAX) {
