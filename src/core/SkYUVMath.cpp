...
//           |        tb |
//           | 0 0 0  1  |
static void colormatrix_to_m44(const float src[20], SkMatrix44* dst) {
    for (int r = 0; r < 3; ++r) {
        for (int c = 0; c < 3; ++c) {
...
// inject identity row/color for alpha
static void m44_to_colormatrix(const SkMatrix44& src, float dst[20]) {
    for (int r = 0; r < 3; ++r) {
        for (int c = 0; c < 3; ++c) {
...
void make_rgb_to_yuv_matrix(float mx[20], const YUVCoeff& c) {
    ...
    float m[20] = {
               Kr,     Kg,        Kb,  0, c.addY,
           -Kr*Su, -Kg*Su, (1-Kb)*Su,  0,   0.5f,
        (1-Kr)*Sv, -Kg*Sv,    -Kb*Sv,  0,   0.5f,
                0,      0,         0,  1,      0,
    };
...
void dump(SkYUVColorSpace cs, const float m[20]) {
    dump(m, cs, true);
    SkMatrix44 m44, im44;
    colormatrix_to_m44(m, &m44);
    float im[20];
    if (1) {
        m44_to_colormatrix(m44, im);
        SkASSERT(memcmp(m, im, sizeof(im)) == 0);
    }
    SkAssertResult(m44.invert(&im44));
    m44_to_colormatrix(im44, im);
    dump(im, cs, false);
}

void SkColorMatrix_RGB2YUV(SkYUVColorSpace cs, float m[20]) {
    const YUVCoeff coeff[] = {
        { 0.299f,  0.114f,          1,       0,         1, }, // kJPEG_SkYUVColorSpace
        { 0.299f,  0.114f,  219/255.f, 1.0f/16, 224/255.f, }, // kRec601_SkYUVColorSpace
        { 0.2126f, 0.0722f, 219/255.f, 1.0f/16, 224/255.f, }, // kRec709_SkYUVColorSpace
    };
