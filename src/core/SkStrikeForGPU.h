...
struct SkGlyphinator {
    ...
    struct Storage {
        ...
        SkSTArray<20, SkGlyphID, true> fSourceGlyphsBuffer;
        SkSTArray<20, SkPoint, true> fSourcePositionsBuffer;
...
public:
    SkPainterPassStorage(SkZip<SkGlyphID, SkPoint> pass1) : fSource{pass1} {
        this->ensure(pass1.size());
    }

    template<typename Input, typename Output>
    static Pass <Input, Output> MakePass();
...
private:
    ...
    template<>
    struct Pass<SkPackedGlyphID, SkGlyph*> {
        Pass(size_t inputSize_) : inputSize{inputSize_} { }
        void push_back(SkGlyph* glyph) {
            multiBuffer[outputSize].glyph = glyph;
            mappedPositions[outputSize] = mappedPositions[input];
            outputSize++;
            input++;
        }

        void reject() {
            rejectedGlyphIDs.push_back(inputGlyphIDs[input]);
            rejectedPositions.push_back(inputPositions[input]);
            input++;
        }

        SkPackedGlyphID pop_back() {
            return multiBuffer[input].packedID;
        }

        size_t input{0};
        const size_t inputSize;
        size_t outputSize{0};
        MultiElement* multiBuffer;
        SkPoint* mappedPositions;
        SkGlyphID* inputGlyphIDs;
        SkPoint* inputPositions;
        SkSTArray<20, SkGlyphID, true>& rejectedGlyphIDs;
        SkSTArray<20, SkPoint, true>& rejectedPositions;
    };
...
template<typename Input, typename Output>
auto  SkPainterPassStorage::MakePass() -> Pass<Input, Output> {
