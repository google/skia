...
public:
    std::tuple<int, int> toBlockAndOffset(int index) {
        int i = index + 1;
    ...
    public:
        ...
        constexpr Iterator& operator++() {
            ...
            if (fCursor == fBlockEnd) {
                fBlockIndex++;
                fCursor = fVector->fBlocks[fBlockIndex].get();
...
public:
    ...
    Iterator begin() {
        T* blockEnd = this->blockEnd(0);
        return Iterator{this, 0, fBlocks[0], blockEnd};
    }
    Iterator end() {
        int lastBlock = this->size() - 1;
        T* blockEnd = this->blockEnd(lastBlock);
        return Iterator{this, lastBlock, fBlockCursor, blockEnd};
    }
