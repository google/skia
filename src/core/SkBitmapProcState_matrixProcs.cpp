...
static inline unsigned shift_16(unsigned x) { return x >> 16; }

// Tilers: clamp, repeat, mirror.
static unsigned clamp(SkFixed fx, int max) {
    return SkClampMax(fx >> 16, max);
}
static unsigned repeat(SkFixed fx, int max) {
    SkASSERT(max < 65535);
    return shift_16((unsigned)(fx & 0xFFFF) * (max + 1));
}
static unsigned mirror(SkFixed fx, int max) {
    ...
    return shift_16( ((fx ^ s) & 0xFFFF) * (max + 1) );
}
...
static void nofilter_scale(const SkBitmapProcState& s,
    ...
    static constexpr bool tryDecal = tile == clamp;
...
static uint32_t pack(unsigned (*tile)(SkFixed,int),
                     unsigned (*extract_low_bits)(SkFixed, int),
                     SkFixed f, unsigned max, SkFixed one) {
...
static void filter_scale(const SkBitmapProcState& s,
                         uint32_t xy[], int count, int x, int y) {
    static constexpr bool tryDecal = tile == clamp;
    static constexpr auto extract_low_bits = tile == clamp ? extract_low_bits_clamp
...
static const SkBitmapProcState::MatrixProc ClampX_ClampY_Procs[] = {
    nofilter_scale <clamp>, filter_scale <clamp>,
    nofilter_affine<clamp>, filter_affine<clamp>,
};
static const SkBitmapProcState::MatrixProc RepeatX_RepeatY_Procs[] = {
    nofilter_scale <repeat>, filter_scale <repeat>,
    nofilter_affine<repeat>, filter_affine<repeat>,
};
static const SkBitmapProcState::MatrixProc MirrorX_MirrorY_Procs[] = {
    nofilter_scale <mirror>, filter_scale <mirror>,
    nofilter_affine<mirror>, filter_affine<mirror>,
};
