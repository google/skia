...
static void filter_affine(const SkBitmapProcState& s,
    ...
    do {
        *xy++ = pack(SkFractionalIntToFixed(fy), maxY, oneY);
        fy += dy;
        *xy++ = pack(SkFractionalIntToFixed(fx), maxX, oneX);
        fx += dx;
    } while (--count != 0);
}
...
// Clamp/Clamp and Repeat/Repeat have NEON or portable implementations.
    ...
    static void filter_affine_neon(const SkBitmapProcState& s,
        ...
        if (count >= 4) {
            ...
            wide_fy = vdupq_n_s32(SkFractionalIntToFixed(fy));
            wide_fy = vsetq_lane_s32(SkFractionalIntToFixed(fy + dy), wide_fy, 1);
            wide_fy = vsetq_lane_s32(SkFractionalIntToFixed(fy + dy + dy), wide_fy, 2);
            wide_fy = vsetq_lane_s32(SkFractionalIntToFixed(fy + dy + dy + dy), wide_fy, 3);
            ...
            while (count >= 4) {
                ...
                wide_fx += vdupq_n_s32(SkFractionalIntToFixed(dx + dx + dx + dx));
                fx += dx + dx + dx + dx;
                wide_fy += vdupq_n_s32(SkFractionalIntToFixed(dy + dy + dy + dy));
                fy += dy + dy + dy + dy;
                xy += 8;  // 4 x's, 4 y's
                count -= 4;
            }
...
#ifdef LEGACY_LOCAL_ROTATE_SHADER_ENABLED
