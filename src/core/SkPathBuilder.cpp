...
SkPath SkPathBuilder::snapshot() {
    ...
    return SkPath(sk_sp<SkPathRef>(new SkPathRef(fPts, fVerbs, fConicWeights, fSegmentMask)),
                  lastMoveToIndex, fConvexity, dir, fFillType, isVolatile);
}
...
SkPath SkPathBuilder::detach() {
    ...
    auto path = SkPath(sk_sp<SkPathRef>(new SkPathRef(std::move(fPts), std::move(fVerbs),
                                                      std::move(fConicWeights), fSegmentMask)),
                       lastMoveToIndex, fConvexity, dir, fFillType, isVolatile);
    this->reset();
    return path;
}
...
static PathInfo validate_verbs(const uint8_t vbs[], int verbCount) {
    PathInfo info = {true, 0, 0, 0};

    const int per_verb_pts[] = {
        1, 1, 2, 2, 3, 0
    };
    const int per_verb_masks[] = {
        0,
...
SkPath SkPathBuilder::Make(SkPathFillType ft,
    ...
    bool isVolatile = false;
    SkPathDirection dir = SkPathDirection::kCW;
    SkPathConvexityType conv = SkPathConvexityType::kUnknown;
    ...
    return SkPath(sk_sp<SkPathRef>(new SkPathRef(SkTDArray<SkPoint>(pts, info.points),
                                                 ...
                                                 info.segmentMask)),
                  lastMoveToIndex, conv, dir, ft, isVolatile);
}
