...
static void compute_complexity(const SkSPath& path, SkScalar& avgLength, SkScalar& complexity) {
...
static bool ShouldUseAAA(const SkSPath& path, SkScalar avgLength, SkScalar complexity) {
#if defined(SK_DISABLE_AAA)
    return false;
#else
...
void SkScan::SAAFillPath(const SkSPath& path, SkBlitter* blitter, const SkIRect& ir,
...
void SkScan::AntiFillPath(const SkSPath& path, const SkRegion& origClip,
...
void SkScan::FillPath(const SkSPath& path, const SkRasterClip& clip, SkBlitter* blitter) {
...
void SkScan::AntiFillPath(const SkSPath& path, const SkRasterClip& clip, SkBlitter* blitter) {
    if (clip.isEmpty() || !path.isFinite()) {
        return;
    }
