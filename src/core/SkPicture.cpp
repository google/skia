...
sk_sp<SkPicture> SkPicture::MakeFromStream(SkStream* stream, SkImageDeserializer* factory) {
    return MakeFromStream(stream, factory, nullptr, nullptr, nullptr);
}
...
sk_sp<SkPicture> SkPicture::MakeFromStream(SkStream* stream, SkImageDeserializer* factory,
    ...
    if (index && pics && (info.fFlags & SkPictInfo::kExternal_Flag)) {
      ...
      if (*index < pics->size())
        return sk_make_sp<SkExternalPicture>(info.fCullRect, (*pics)[(*index)++]);
    }
...
void SkPicture::serialize(SkWStream* stream,
    ...
    if (data) {
        ...
        data->serialize(stream, pixelSerializer, typefaceSet, picIds);
    } else {
