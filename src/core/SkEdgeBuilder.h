...
protected:
    ...
    // In polygon mode we preallocated pointers in fAlloc and fEdgeList points there.
...
private:
    ...
    virtual SkRect shiftClip(const SkIRect&) const = 0;
...
public:
    explicit SkBasicEdgeBuilder(int shiftEdgesUp) : fShiftUp(shiftEdgesUp) {}
...
private:
    ...
    SkRect shiftClip(const SkIRect&) const override;
