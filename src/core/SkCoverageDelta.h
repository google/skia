...
public:
    ...
    static bool Suitable(const SkIRect& bounds) {
        // 2 more width and height so we don't have to worry about the boundary
        return (bounds.width() + 2) * (bounds.height() + 2) < MAX_MASK_SIZE;
    }

    SkCoverageDeltaMask(SkCoverageDeltaAllocator* alloc, const SkIRect& bounds)
            : fAlloc(alloc), fBounds(bounds) {
        SkASSERT(Suitable(bounds));
        fExpandedWidth = fBounds.width() + 2;
        memset(fDeltaStorage, 0, sizeof(fDeltaStorage));

        fDeltas = fDeltaStorage;

        // Add one row and one column so we may access fDeltas[index(-1, -1)]
        fDeltas += fExpandedWidth + 1;
    ...
    void convertCoverageToAlpha(bool isEvenOdd, bool isInverse, bool isConvex) {
        ...
        for(int iy = 0; iy < fBounds.height(); ++iy) {
            SkFixed coverage = 0;
            for(int ix = 0; ix < fBounds.width(); ++ix) {
                coverage += deltaRow[ix];
                maskRow[ix] = isConvex ? ConvexCoverageToAlpha(coverage, isInverse)
