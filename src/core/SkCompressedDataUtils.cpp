...
static int to_pos(int x, int y, bool flip) {
    SkASSERT(x >= 0 && x < 4);
    SkASSERT(y >= 0 && y < 4);
...
static SkPMColor foo(const IColor& col, int delta) {
        ...
        for (int x = 0; x < numXBlocks; ++x) {
            ...
            for (int i = 0; i < 4; ++i, ++baseShift) {
                for (int j = 0; j < 4; ++j) {
                    ...
                    int subBlockIndex = to_pos(j, i, flipped);
                    ...
                    *dst->getAddr32(offsetX + j, offsetY + i) = foo(colors[subBlockIndex], delta);
                }
            }
        }
    }
