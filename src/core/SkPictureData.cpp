...
bool new_array_from_buffer(SkReadBuffer& buffer, uint32_t inCount,
    ...
    // Reserve up to this many elements.  If the caller claims to have more, it will have
    // to prove so with successful factory calls.
    static constexpr int kReasonableSize = 2048;
    array.reserve(SkTMin(SkToInt(inCount), kReasonableSize));
