...
void SkPictureData::serialize(SkWStream* stream,
  ...
  for (int i = 0; i < fPictureCount; i++) {
    if (fPictureRefs[i]->isExternal())
      picIds->push_back(fPictureRefs[i]->uniqueID());
  }
}
    ...
    switch (tag) {
        ...
        case SK_PICT_PICTURE_TAG: {
            ...
            for (uint32_t i = 0; i < size; i++) {
                fPictureRefs[i] = SkPicture::MakeFromStream(stream, factory, topLevelTFPlayback, &ext_pic_index, pics).release();
                if (!fPictureRefs[i]) {
                    return false;
                }
                fPictureCount++;
