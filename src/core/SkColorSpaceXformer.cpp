...
// Ensures that we only keep cached objects alive while transforming, i.e. until
// the top-level apply() returns.
// (the cache scope is naturally tied to SkColorSpaceXformer, but clients may choose
// to not discard xformers immediately - in which case, caching indefinitely is undesired.
