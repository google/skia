...
SkBitmapController::State* SkBitmapController::RequestBitmap(const SkBitmapProvider& provider,
    ...
    State* state = SkInPlaceNewCheck<State>(storage, storageSize, provider, inv, quality);
    SkASSERT(state);

    if (!state->pixmap().addr()) {
...
SkBitmapController::State::State(const SkBitmapProvider& provider, const SkMatrix& inv,
                                 SkFilterQuality qual)
    : fInvMatrix(inv), fQuality(qual) {
