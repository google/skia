...
skif::LayerSpace<SkIRect> SkImageFilter_Base::getInputBounds(
    ...
    // Map the device-space clip into the layer space and the known content bounds to layer space
    skif::LayerSpace<SkIRect> layerClip = mapping.deviceToLayer(targetOutput).roundOut();
    skif::LayerSpace<SkIRect> contentBounds =
            knownContentBounds ? mapping.paramToLayer(*knownContentBounds).roundOut() : layerClip;
    // Process the layer-space target output with the filter DAG to determine required input
    skif::LayerSpace<SkIRect> requiredInput = this->onGetInputLayerBounds(
            mapping, layerClip, contentBounds);
...
skif::DeviceSpace<SkIRect> SkImageFilter_Base::getOutputBounds(
    ...
    // Map the input content into the layer space where filtering would occur
    skif::LayerSpace<SkRect> layerContent = mapping.paramToLayer(contentBounds);
    // Determine the filter DAGs impact in layer space
...
skif::LayerSpace<SkIRect> SkImageFilter_Base::visitInputLayerBounds(
    ...
    if (this->countInputs() < 1) {
        ...
        // implicit source image, so arguably could return an empty rect here. 'targetOutput' is
    ...
    for (int i = 0; i < this->countInputs(); ++i) {
        ...
        skif::LayerSpace<SkIRect> requiredInput =
                filter ? as_IFB(filter)->onGetInputLayerBounds(mapping, targetOutput, contentBounds)
                       : targetOutput;
...
skif::LayerSpace<SkIRect> SkImageFilter_Base::onGetInputLayerBounds(
        const skif::Mapping& mapping, const skif::LayerSpace<SkIRect>& targetOutput,
        const skif::LayerSpace<SkIRect>& contentBounds, bool visitChildren) const {
    ...
    SkIRect input = this->onFilterNodeBounds(SkIRect(targetOutput), mapping.layerMatrix(),
...
SkImageFilter_Base::Context SkImageFilter_Base::mapContext(const Context& ctx) const {
    auto childOutput = this->onGetInputLayerBounds(ctx.mapping(), ctx.targetOutput(),
                                                   ctx.targetOutput(), /* visit children */ false);
    return ctx.withNewTargetOutput(childOutput);
}
