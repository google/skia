...
skif::IRect<In::kLayer, For::kOutput> SkImageFilter_Base::getOutputBounds(
        const skif::IRect<In::kLayer, For::kInput>& content, const SkMatrix& layerMatrix) const {
    // TODO: Once SkCanvas does not ignore perspective CTMs for image filters, the hasPerspective()
    // case can go away.
    SkASSERT(layerMatrix.isScaleTranslate() || this->canHandleComplexCTM() ||
             layerMatrix.hasPerspective());
    auto output = this->onGetOutputBounds(content, layerMatrix);
    ...
    if (this->cropRectIsSet()) {
        SkIRect dst;
        this->getCropRect().applyTo(SkIRect(output), layerMatrix, this->affectsTransparentBlack(),
                                    &dst);
...
skif::IRect<In::kLayer, For::kInput> SkImageFilter_Base::getInputLayerBounds(
        const skif::IRect<In::kLayer, For::kOutput>& target, const SkMatrix& layerMatrix,
    ...
    SkASSERT(layerMatrix.isScaleTranslate() || this->canHandleComplexCTM() ||
             layerMatrix.hasPerspective());
    if (originalInput) {
        return this->onGetInputLayerBounds(target, layerMatrix, *originalInput);
    } else {
        // Assume the input fits the target output
        return this->onGetInputLayerBounds(target, layerMatrix,
                                           skif::LayerCast<For::kInput>(target));
    }
}
