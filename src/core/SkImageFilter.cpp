...
static sk_sp<SkImageFilter> apply_ctm_to_filter(sk_sp<SkImageFilter> input, const SkMatrix& ctm,
    ...
    if (ctm.decomposeScale(&scale, &ctmToEmbed)) {
        ...
        // a pre-transformation before applying the image-filter context's CTM. In this case, need
        // ctmToEmbed to be a post-transformation (i.e. after the scale matrix since decomposeScale
        // produces ctm = ctmToEmbed * scale). Giving scale^-1 * ctmToEmbed * scale to the matrix
        // filter achieves this effect.
