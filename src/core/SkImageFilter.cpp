...
skif::Image<For::kOutput> SkImageFilter_Base::filterImage(const skif::Context& context) const {
    ...
    if (context.gpuBacked() && result.image() && !result.image()->isTextureBacked()) {
        ...
        result = skif::Image<For::kOutput>(std::move(asTexture), result.origin());
    }
#endif
...
skif::Image<For::kOutput> SkImageFilter_Base::onFilterImage(const skif::Context& context) const {
    SkIPoint origin;
...
skif::IRect<In::kLayer, For::kOutput> SkImageFilter_Base::onFilterOutputBounds(
    ...
    for (int i = 0; i < this->countInputs(); ++i) {
        ...
        if (filter) {
            ...
            childOutputs.join(filterOutput);
        } else {
            // The input doesn't have a filter set, so this filter would process the dynamic source
            // image, which we assume is the same region as 'content'.
            childOutputs.join(skif::LayerCast<For::kOutput>(content));
        }
    }
...
skif::IRect<In::kLayer, For::kInput> SkImageFilter_Base::onFilterLayerBounds(
        const skif::IRect<In::kLayer, For::kOutput>& target, const SkMatrix& layer,
        const skif::IRect<In::kLayer, For::kInput>& originalInput) const {
...
SkIRect SkImageFilter_Base::onFilterNodeBounds(const SkIRect& src, const SkMatrix&,
    ...
    // NOTE: Both onNodeOutputBounds and onNodeLayoutBounds are meant to have default impls that
    // return 'src' unmodified, but currently they actually default to just calling
    // onFilterNodeBounds for backwards compatibility. Because the default onFilterNodeBounds
    // returns 'src', everything works out okay.
    return src;
}

template<skif::Usage kU>
skif::Image<kU> SkImageFilter_Base::filterInput(int index, const skif::Context& ctx) const {
    ...
    return static_cast<skif::Image<kU>>(std::move(result));
}
...
template skif::Image<For::kInput> SkImageFilter_Base::filterInput(int, const skif::Context&) const;
template skif::Image<For::kInput0> SkImageFilter_Base::filterInput(int, const skif::Context&) const;
template skif::Image<For::kInput1> SkImageFilter_Base::filterInput(int, const skif::Context&) const;
