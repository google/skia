...
SkIRect SkImageFilter::filterBounds(const SkIRect& src, const SkMatrix& ctm,
    ...
    if (kReverse_MapDirection == direction) {
        SkASSERT(inputRect);
        SkIRect bounds = this->onFilterNodeBounds(src, ctm, inputRect, direction);
        return this->onFilterBounds(bounds, ctm, inputRect, direction);
    } else {
        SkASSERT(!inputRect);
        SkIRect bounds = this->onFilterBounds(src, ctm, inputRect, direction);
        bounds = this->onFilterNodeBounds(bounds, ctm, inputRect, direction);
        SkIRect dst;
...
bool SkImageFilter::applyCropRect(const Context& ctx, const SkIRect& srcBounds,
    ...
    SkIRect tmpDst = this->onFilterNodeBounds(srcBounds, ctx.ctm(), nullptr, kForward_MapDirection);
...
SkIRect SkImageFilter::onFilterBounds(const SkIRect& src, const SkMatrix& ctm,
    ...
    for (int i = 0; i < this->countInputs(); ++i) {
        ...
        SkIRect rect = filter ? filter->filterBounds(src, ctm, inputRect, dir) : src;
        if (0 == i) {
            totalBounds = rect;
        } else {
...
SkImageFilter::Context SkImageFilter::mapContext(const Context& ctx, const SkIRect& inputRect) const {
    SkIRect clipBounds = this->onFilterNodeBounds(ctx.clipBounds(), ctx.ctm(), &inputRect,
...
sk_sp<SkSpecialImage> SkImageFilter::filterInput(int index,
    ...
    const SkIRect originalSrcBounds = src->subset();

    sk_sp<SkSpecialImage> result(input->filterImage(src, this->mapContext(ctx, originalSrcBounds), offset));
...
SkIRect SkImageFilter::DetermineRepeatedSrcBound(const SkIRect& srcBounds,
    ...
    SkIRect tmp = SkIRect::MakeLTRB(srcBounds.fLeft + filterRect.fLeft,
                                    srcBounds.fTop + filterRect.fTop,
                                    srcBounds.fRight + filterRect.fRight,
                                    srcBounds.fBottom + filterRect.fBottom);
