...
bool SkImageFilter::IntersectWithRepeat(SkIRect* mungedSrcBounds, const SkIRect& filterRect,
                                        const SkIRect& originalSrcBounds) {
    SkIRect tmp = *mungedSrcBounds;
    if (mungedSrcBounds->fLeft + filterRect.fLeft < originalSrcBounds.fLeft) {
        tmp.fLeft = originalSrcBounds.fLeft;
        tmp.fRight = originalSrcBounds.fRight;
    }
    if (mungedSrcBounds->fRight + filterRect.fRight > originalSrcBounds.fRight) {
        tmp.fRight = originalSrcBounds.fRight;
        tmp.fLeft = originalSrcBounds.fLeft;
    }
    if (mungedSrcBounds->fTop + filterRect.fTop < originalSrcBounds.fTop) {
        tmp.fTop = originalSrcBounds.fTop;
        tmp.fBottom = originalSrcBounds.fBottom;
    }
    if (mungedSrcBounds->fBottom + filterRect.fBottom > originalSrcBounds.fBottom) {
        tmp.fBottom = originalSrcBounds.fBottom;
        tmp.fTop = originalSrcBounds.fTop;
    }
