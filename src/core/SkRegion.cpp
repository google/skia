...
static bool validate_run(const int32_t* runs,
    ...
    // quick sanity check:
    if (runs[runCount - 1] != kSentinel || runs[runCount - 2] != kSentinel) {
        return false;
    }
    const int32_t* const end = runs + runCount;
    SkIRect bounds = {};  // calulated bounds
    SkIRect rect = {};    // current rect
    rect.fTop = *runs++;
    if (rect.fTop == kSentinel) {
        return false;  // no rect can contain kSentinel
    }
    do {
        --ySpanCount;
        if (ySpanCount < 0) {
        ...
        while (xIntervals-- > 0) {
            rect.fLeft = *runs++;
            rect.fRight = *runs++;
            if (rect.fLeft == kSentinel || rect.fRight == kSentinel || rect.isEmpty()) {
                return false;
            }
            bounds.join(rect);
        }
