...
static int expected_run_count(int ySpanCount, int intervalCount) {
...
static bool validate_run(const int32_t* runs,
    ...
    if (ySpanCount < 1 || intervalCount < 2 || runCount < 0 ||
        runCount != expected_run_count(SkToInt(ySpanCount), SkToInt(intervalCount)))
    {
        return false;
    }
