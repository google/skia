...
#if 0
using RunArray = SkSTArray<kRunArrayStackCount, SkRegion::RunType, true>;
static void reserveMore(RunArray* array, int newCount) {
    if (newCount > array->count()) {
         array->push_back_n(newCount - array->count());
     }
}
#else
...
//   - ra.resize(c) followed by assert(ra.count() == c) will fail.
//     Instead: assert(ra.count() >= c))
...
struct RunArray {
    ...
    ~RunArray() { if (fPtr != fStack) { free(fPtr); } }
    int count() const { return fCount; }
    SkRegion::RunType& operator[](int i) { return fPtr[i]; }
    /** Resize the array to a size greater-than-or-equal-to count. */
    void resize(int count) {
        if (count > fCount) {
            // leave at least 50% extra space for future growth.
            count += count >> 1;
            constexpr size_t kSize = sizeof(SkRegion::RunType);
            if (fPtr == fStack) {
                SkRegion::RunType* ptr = (SkRegion::RunType*)sk_malloc_throw(kSize * count);
                memcpy(ptr, fPtr, fCount * kSize);
                fPtr = ptr;
            } else {
                fPtr = (SkRegion::RunType*)sk_realloc_throw(fPtr, kSize * SkToSizeT(count));
            }
            fCount = count;
        }
    }
    ...
    SkRegion::RunType* fPtr = nullptr;
};
static void reserveMore(RunArray* array, int newCount) { array->resize(newCount); }
#endif
...
static int operate_on_span(const SkRegion::RunType a_runs[],
                           const SkRegion::RunType b_runs[],
                           RunArray* array, int dst,
                           int min, int max) {
    // This is a worst-case for this span plus two for TWO terminating sentinels.
    reserveMore(array, dst + distance_to_sentinel(a_runs) + distance_to_sentinel(b_runs) + 2);
    ...
    while (!rec.done()) {
        ...
        if ((unsigned)(rec.fInside - min) <= (unsigned)(max - min) &&
            ...
            if (firstInterval || (*array)[dst - 1] < left) {
                (*array)[dst++] = (SkRegion::RunType)(left);
                (*array)[dst++] = (SkRegion::RunType)(rite);
                firstInterval = false;
            } else {
                // update the right edge
                (*array)[dst - 1] = (SkRegion::RunType)(rite);
            }
        }
    }
    (*array)[dst++] = SkRegion::kRunTypeSentinel;
    return dst;
}
