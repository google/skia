...
using RunArray = SkSTArray<256, SkRegion::RunType, true>;

static SkRegion::RunType* smart_get(RunArray* array, int offset) {
    if (offset >= array->count()) {
        array->push_back_n(offset - array->count() + 1);
    }
    return &(*array)[offset];
}
...
static int operate_on_span(const SkRegion::RunType a_runs[],
    ...
    while (!rec.done()) {
        ...
        if ((unsigned)(rec.fInside - min) <= (unsigned)(max - min) &&
            ...
            if (firstInterval || *smart_get(array, dst - 1) < left) {
                *smart_get(array, dst++) = (SkRegion::RunType)(left);
                *smart_get(array, dst++) = (SkRegion::RunType)(rite);
                firstInterval = false;
            } else {
                // update the right edge
                *smart_get(array, dst - 1) = (SkRegion::RunType)(rite);
            }
        }
    }

    *smart_get(array, dst++) = SkRegion::kRunTypeSentinel;
    return dst;
}
...
public:
    ...
    void addSpan(int bottom, const SkRegion::RunType a_runs[],
        ...
        if (fPrevLen == len &&
            ...
            // update Y value
            *smart_get(fArray, fPrevDst - 2) = (SkRegion::RunType)bottom;
        } else {    // accept the new span
            if (len == 1 && fPrevLen == 0) {
                fTop = (SkRegion::RunType)bottom; // just update our bottom
            } else {
                *smart_get(fArray, start - 2) = (SkRegion::RunType)bottom;
                *smart_get(fArray, start - 1) = SkToS32(len >> 1);
                fPrevDst = start;
                fPrevLen = len;
            }
        }
    ...
    int flush() {
        *smart_get(fArray, fStartDst) = fTop;
        *smart_get(fArray, fPrevDst + fPrevLen) = SkRegion::kRunTypeSentinel;
