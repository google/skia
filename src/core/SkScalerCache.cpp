...
SkGlyphDigest SkScalerCache::addGlyph(SkGlyph* glyph) {
    size_t newIndex = fGlyphForIndex.size();
    SkGlyphDigest digest = SkGlyphDigest{newIndex};
...
std::tuple<SkGlyph*, size_t> SkScalerCache::mergeGlyphAndImage(
    ...
    SkGlyphDigest* denseID = fDigestForPackedGlyphID.find(toID);
    if (denseID != nullptr) {
        ...
        return {fGlyphForIndex[denseID->index()], 0};
    } else {
