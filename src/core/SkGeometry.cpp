...
static float measure_angle_between_vectors(const Sk2f& a, const Sk2f& b) {
...
float SkMeasureNonInflectCubicRotation(const SkPoint pts[4]) {
    ...
    if ((p0 == p1).allTrue()) {
        return measure_angle_between_vectors(p2 - p1, p3 - p2);
    }
    if ((p1 == p2).allTrue()) {
        return measure_angle_between_vectors(p1 - p0, p3 - p2);
    }
    if ((p2 == p3).allTrue()) {
        return measure_angle_between_vectors(p1 - p0, p2 - p1);
    }
