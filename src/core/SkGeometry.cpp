...
SkVector SkFindBisector(SkVector a, SkVector b) {
    ...
    if (a.dot(b) >= 0) {
        ...
        // a,b are >90 degrees apart. Find the bisector of their interior normals instead. (After 90
        // degrees, the normals start cancelling each other out which eventually becomes unstable.)
        v[0].set(-a.fY, +a.fX);
        v[1].set(+b.fY, -b.fX);
    } else {
        // a,b are <90 degrees apart. Find the bisector of their interior normals instead. (After 90
...
void SkChopQuadAtMidTangent(const SkPoint src[3], SkPoint dst[5]) {
    ...
    //
    //   0 = (F'(T) dot bisector) = |2*T 1| * |p0 - 2*p1 + p2| * |nx|
    //                                        |-2*p0 + 2*p1  |   |ny|
    //
    //                     = |2*T 1| * |tan1 - tan0| * |nx|
    //                                 |2*tan0     |   |ny|
...
void SkChopCubicAtMidTangent(const SkPoint src[4], SkPoint dst[7]) {
    ...
    if ((coeffs == 0).allTrue()) {
        ...
        // Chop at a cusp instead, which guarantees neither side rotates more than 180 degrees.
    ...
    if (!(T > 0 && T < 1)) {  // Use "!(positive_logic)" so T=NaN will take this branch.
        // FP precision failed us. We neither detected a flat line nor found a midtangent inside
        // 0..1. Just chop at .5.
        T = .5;
    }
    ...
    SkChopCubicAt(src, dst, T);
    if (isLineCusp) {
        dst[2] = dst[4] = dst[3];  // Ensure the chop points reflect that this is a line cusp.
    }
}
