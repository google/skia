...
float SkMeasureNonInflectCubicRotation(const SkPoint pts[4]) {
    ...
    //
    // The direction of turn is determined by sign(cross(F', F'')). "turn" below is a simplified
    // calculation of "cross(F'(.5), F''(.5))", with scalars removed that don't affect the sign.
    // We already guaranteed the cubic will turn in the same direction for all of T=0..1 because we
    // don't allow inflections.
...
static float dot(const Sk2f& a, const Sk2f& b) {
    Sk2f ab = a*b;
    return ab[0] + ab[1];
}
