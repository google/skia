...
static void sort_and_orient_t_s(double t[2], double s[2]) {
    ...
    if (copysign(s[1], s[0]) * t[0] > -abs(s[0]) * t[1]) {
        std::swap(t[0], t[1]);
        std::swap(s[0], s[1]);
    }
}
...
static SkCubicType classify_cubic(double t[2], double s[2], const double d[4]) {
    double tolerance = SkTMax(abs(d[1]), abs(d[2]));
    tolerance = SkTMax(tolerance, abs(d[3]));
    ...
    if (abs(d[1]) > tolerance) {
        ...
        if (abs(d[2]) > tolerance) {
            ...
            return abs(d[3]) > tolerance ? SkCubicType::kQuadratic : SkCubicType::kLineOrPoint;
        }
    }
}
