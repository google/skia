...
float SkMeasureQuadRotation(const SkPoint pts[3]) {
    ...
    float cosTheta = dot(tan0, tan1) / std::sqrt(lengthsSqd[0] * lengthsSqd[1]);
...
float SkMeasureNonInflectCubicRotation(const SkPoint pts[4]) {
    ...
    if ((p0 == p1).allTrue()) {
        return SkMeasureQuadRotation(pts + 1);
    }
    if ((p1 == p2).allTrue()) {
        SkPoint quad[3] = {pts[0], pts[1], pts[3]};
        return SkMeasureQuadRotation(quad);
    }
    if ((p2 == p3).allTrue()) {
        return SkMeasureQuadRotation(pts);
    }

    // Postulate: When no control points are colocated and there are no inflection points in T=0..1,
    // the rotation is: 360 degrees, minus the angle [p0,p1,p2], minus the angle [p1,p2,p3].
