...
void SkChopCubicAt(const SkPoint src[4], SkPoint dst[],
...
#ifdef SK_DEBUG
    {
        ...
        {
            SkASSERT(tValues[i] > 0 && tValues[i] < SK_Scalar1);
            SkASSERT(tValues[i+1] > 0 && tValues[i+1] < SK_Scalar1);
...
static int solve_cubic_poly(const SkScalar coeff[4], SkScalar tValues[3]) {
    ...
    if (R2MinusQ3 < 0) { // we have 3 real roots
        ...
        *roots++ = SkScalarPin(neg2RootQ * SkScalarCos(theta/3) - adiv3, 0, 1);
        *roots++ = SkScalarPin(neg2RootQ * SkScalarCos((theta + 2*SK_ScalarPI)/3) - adiv3, 0, 1);
        *roots++ = SkScalarPin(neg2RootQ * SkScalarCos((theta - 2*SK_ScalarPI)/3) - adiv3, 0, 1);
...
int SkChopCubicAtMaxCurvature(const SkPoint src[4], SkPoint dst[13],
    ...
    SkScalar roots[3];
    int rootCount = SkFindCubicMaxCurvature(src, tValues);
