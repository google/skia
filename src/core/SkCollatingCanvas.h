...
public:
    ...
    void serializeToStream(SkWStream* stream, const SkSerialProcs& procs) const;

    struct ResultOrError {
        std::unique_ptr<SkCollatingCanvas> cc;
        SkString error;
        operator bool() const {
            return cc.get() != nullptr;
        }
    };
    // Read a picture from a file.
    static ResultOrError CreateFromStream(SkStream* stream, const SkDeserialProcs& procs);
...
private:
    ...
    // Serialization and deserialization related methods
    void flattenToBuffer(SkWriteBuffer& buffer) const;
    SkString parseStreamTag(SkStream* stream, uint32_t tag, uint32_t size,
