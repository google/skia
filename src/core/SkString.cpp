...
sk_sp<SkString::Rec> SkString::Rec::Make(const char text[], size_t len) {
    ...
    if (0 == len) {
        ...
        uint32_t compressedLen = trim_size_t_to_u32(len);

        // add 1 for terminating 0, then align4 so we can have some slop when growing the string
        rec = (Rec*)sk_malloc_throw(SizeOfRec() + SkAlign4(compressedLen + 1));
        rec->fLength = compressedLen;
        rec->fRefCnt.store(+1, std::memory_order_relaxed);
        if (text) {
            memcpy(rec->data(), text, compressedLen);
        }
        rec->data()[compressedLen] = 0;
    }
