...
sk_sp<SkString::Rec> SkString::Rec::Make(const char text[], size_t len) {
    ...
    SkSafeMath safe;
    // We want to add 1 for null termination, add SizeOfRec() for our overhead, and add 3
    // as part of aligning to a multiple of 4.
    size_t allocationSize = safe.add(len, SizeOfRec() + 4);
    allocationSize &= ~(size_t)3;

    SkASSERT_RELEASE(safe.ok() && SkTFitsIn<uint32_t>(allocationSize));
