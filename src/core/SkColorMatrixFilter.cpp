...
sk_sp<SkFlattenable> SkColorMatrixFilter::CreateProc(SkReadBuffer& buffer) {
    ...
    if (buffer.readScalarArray(matrix, 20)) {
        SkScalar* colMajor = matrix;
        // older versions stored rowMajor with translate scaled by 255, so undo that if needed
...
bool SkColorMatrixFilter::onAppendStages(const SkStageRec& rec,
...
std::unique_ptr<GrFragmentProcessor> SkColorMatrixFilter::asFragmentProcessor(
...
sk_sp<SkColorFilter> SkColorFilters::Matrix(const float array[20]) {
    ...
    return sk_sp<SkColorFilter>(new SkColorMatrixFilter(array));
}

///////////////////////////////////////////////////////////////////////////////

sk_sp<SkColorFilter>
SkColorMatrixFilter::MakeSingleChannelOutput(const SkScalar row[5]) {
    ...
    return SkColorFilters::Make(array);
}
