...
public:
        ...
        while (initIndex < fDrawCnt) {
            ...
            if (!initElement.fInitialized && initElement.fNeedInit) {
                ...
                if (initElement.fNeedInit.compare_exchange_strong(
                    ...
                    initElement.fInitData = initElement.fInitFn(threadAlloc);
    ...
    bool tryDraw(int& drawIndex, int tileIndex) {
        ...
        if (drawElement.fInitialized) {
            drawElement.fDrawFn(fDevice->fTileBounds[tileIndex], drawElement.fInitData);
            drawIndex++;
            return true;
        }
        return false;
...
public:
    ...
    bool next(int threadId) final {
        ...
        while (true) {
            ...
            if (drawIndex < fDrawCnt && this->tryDraw(drawIndex, threadId)) {
                return true;
            }
        ...
        while (true) {
            ...
            if (locked) {
                if (tileData.fDrawIndex < fDrawCnt) {
                    processed = this->tryDraw(tileData.fDrawIndex, tileIndex);
                } else {
    ...
    bool next(int threadId) final {
        ...
        while (true) {
            ...
            if (isFinishing && scheduleData.fDrawIndex >= fDrawCnt) {
                ...
                // and drawn.
                while (!this->tryDraw(scheduleData.fDrawIndex, tileIndex)) {}
                return true;
            }
        }
    }
...
#define THREADED_DRAW(drawBounds, actualDrawCall)                                                  \
    do {                                                                                           \
        ...
        element.fDrawFn = [=](const SkIRect& tileBounds, void* initData) {                         \
            SkRasterClip tileRC;                                                                   \
...
#define THREADED_DRAW_WITH_INIT(drawBounds, initCall, drawCall)                                    \
    do {                                                                                           \
        if (fQueueSize == MAX_QUEUE_SIZE) {                                                        \
            this->flush();                                                                         \
        }                                                                                          \
...
void SkThreadedBMPDevice::drawPath(const SkPath& path, const SkPaint& paint,
    ...
    THREADED_DRAW_WITH_INIT(drawBounds,
                            initDrawPath(path, paint, prePathMatrix, false, alloc),
                            postDrawPath(path, paint, prePathMatrix, false, initData));
}
