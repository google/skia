...
static inline SkRect OutsetRectBounds(const SkRect& r, const SkPaint& p) {
    return r.makeOutset(p.getStrokeWidth(), p.getStrokeWidth());
}
...
void SkThreadedBMPDevice::drawRect(const SkRect& r, const SkPaint& paint) {
    SkRect drawBounds = OutsetRectBounds(r, paint);
...
void SkThreadedBMPDevice::drawRRect(const SkRRect& rrect, const SkPaint& paint) {
    ...
    this->drawPath(path, paint, nullptr, fThreadCnt <= 1); // pathIsMutable only if single-threaded
#else
    SkRect drawBounds = OutsetRectBounds(rrect.getBounds(), paint);
...
void SkThreadedBMPDevice::drawPath(const SkPath& path, const SkPaint& paint,
    ...
    SkRect drawBounds = path.isInverseFillType() ? SkRect::MakeLargest()
                                                 : OutsetRectBounds(path.getBounds(), paint);
    THREADED_DRAW(drawBounds, drawPath(path, paint, prePathMatrix,
                                       // pathIsMutable only if single-threaded
                                       pathIsMutable && fThreadCnt <= 1));
}
