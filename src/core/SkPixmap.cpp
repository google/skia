...
SkColor SkPixmap::getColor(int x, int y) const {
    ...
    auto pmColorToColor = [&needsUnpremul](SkPMColor pmColor) {
        return needsUnpremul ? SkUnPreMultiply::PMColorToColor(pmColor)
                             : SkSwizzle_BGRA_to_PMColor(pmColor);
    };
