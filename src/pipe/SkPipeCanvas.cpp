...
void SkPipeCanvas::onDrawVerticesObject(const SkVertices* vertices, const SkSkeleton* bones,
    ...
    if (bones) {
        writer.writeDataAsByteArray(bones->encode().get());
    } else {
        sk_sp<SkData> empty = SkData::MakeEmpty();
        writer.writeDataAsByteArray(empty.get());
    }
