...
static sk_sp<SkImage> make_from_skiaimageformat(const void* encoded, size_t encodedSize) {
    ...
    uint16_t ct_raw;
    uint16_t at_raw;
    if (!stream.readU32(&width_raw)) { return false; }
    if (!stream.readU32(&height_raw)) { return false; }
    if (!stream.readU16(&ct_raw)) { return false; }
    if (!stream.readU16(&at_raw)) { return false; }
    ...
    if (!stream.readU32(&colorSpaceSize)) { return false; }
    ...
    if (!stream.read(pixels->writable_data(), size) == size) { return false; }
