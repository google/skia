...
#ifndef SRC_PARTITION_ALLOC_NOOP_RAW_PTR_H_
#define SRC_PARTITION_ALLOC_NOOP_RAW_PTR_H_
...
//
// Here, skia provides a "no-op" implementation, because partition_alloc
...
#define SK_PA_TRIVIAL_ABI [[clang::trivial_abi]]
#else
#define SK_PA_TRIVIAL_ABI
#endif

#if defined(_MSC_VER)
#define SK_PA_ALWAYS_INLINE __forceinline
#elif defined(__GNUC__) || defined(__clang__)
#define SK_PA_ALWAYS_INLINE __attribute__((always_inline)) inline
#else
#define SK_PA_ALWAYS_INLINE inline
#endif
...
public:
    ...
    // Deliberately implicit, because raw_ptr is supposed to resemble raw ptr.
    SK_PA_ALWAYS_INLINE constexpr raw_ptr(T* p) noexcept : wrapped_ptr_(p) {}

    // Deliberately implicit in order to support implicit upcast.
    template <typename U,
              typename Unused = std::enable_if_t<std::is_convertible_v<U*, T*> &&
                                                 !std::is_void_v<typename std::remove_cv<T>::type>>>
    SK_PA_ALWAYS_INLINE constexpr raw_ptr(const raw_ptr<U, Traits>& ptr) noexcept
    ...
    template <typename U,
              typename Unused = std::enable_if_t<std::is_convertible_v<U*, T*> &&
                                                 !std::is_void_v<typename std::remove_cv<T>::type>>>
    SK_PA_ALWAYS_INLINE constexpr raw_ptr(raw_ptr<U, Traits>&& ptr) noexcept
    ...
    SK_PA_ALWAYS_INLINE constexpr raw_ptr& operator=(T* p) noexcept {
        wrapped_ptr_ = p;
        return *this;
    }
    ...
    template <typename U,
              typename Unused = std::enable_if_t<std::is_convertible_v<U*, T*> &&
                                                 !std::is_void_v<typename std::remove_cv<T>::type>>>
    SK_PA_ALWAYS_INLINE constexpr raw_ptr& operator=(const raw_ptr<U, Traits>& ptr) noexcept {
        wrapped_ptr_ = ptr.wrapped_ptr_;
        return *this;
    }
    ...
    SK_PA_ALWAYS_INLINE constexpr explicit operator bool() const {
        return static_cast<bool>(wrapped_ptr_);
    }

    template <typename U = T,
              typename Unused = std::enable_if_t<!std::is_void_v<typename std::remove_cv<U>::type>>>
    SK_PA_ALWAYS_INLINE constexpr U& operator*() const {
        return *wrapped_ptr_;
    }
    SK_PA_ALWAYS_INLINE constexpr T* operator->() const { return wrapped_ptr_; }

    // Deliberately implicit, because raw_ptr is supposed to resemble raw ptr.
    SK_PA_ALWAYS_INLINE constexpr operator T*() const { return wrapped_ptr_; }

    template <typename U> SK_PA_ALWAYS_INLINE constexpr explicit operator U*() const {
    ...
    SK_PA_ALWAYS_INLINE constexpr raw_ptr& operator++() {
        wrapped_ptr_++;
        return *this;
    }
    SK_PA_ALWAYS_INLINE constexpr raw_ptr& operator--() {
    ...
    SK_PA_ALWAYS_INLINE constexpr U& operator[](Z delta) const {
    ...
    SK_PA_ALWAYS_INLINE constexpr void ClearAndDelete() noexcept { delete ExtractAsDangling(); }
    SK_PA_ALWAYS_INLINE constexpr void ClearAndDeleteArray() noexcept { delete[] ExtractAsDangling(); }
    ...
    SK_PA_ALWAYS_INLINE constexpr raw_ptr<T, Traits> ExtractAsDangling() noexcept {
        T* ptr = wrapped_ptr_;
        wrapped_ptr_ = nullptr;
        return raw_ptr(ptr);
    }
    ...
    template <typename U> SK_PA_ALWAYS_INLINE friend bool operator==(const raw_ptr& lhs, U* rhs) {
        return lhs.wrapped_ptr_ == rhs;
    }
    template <typename U> SK_PA_ALWAYS_INLINE friend bool operator!=(const raw_ptr& lhs, U* rhs) {
        return !(lhs == rhs);
    }
    template <typename U> SK_PA_ALWAYS_INLINE friend bool operator==(U* lhs, const raw_ptr& rhs) {
    ...
    SK_PA_ALWAYS_INLINE friend bool operator==(const raw_ptr& lhs, std::nullptr_t) { return !lhs; }
    SK_PA_ALWAYS_INLINE friend bool operator!=(const raw_ptr& lhs, std::nullptr_t) {
        return !!lhs;  // Use !! otherwise the costly implicit cast will be used.
    }
    SK_PA_ALWAYS_INLINE friend bool operator==(std::nullptr_t, const raw_ptr& rhs) { return !rhs; }
    SK_PA_ALWAYS_INLINE friend bool operator!=(std::nullptr_t, const raw_ptr& rhs) {
        return !!rhs;  // Use !! otherwise the costly implicit cast will be used.
    }

    SK_PA_ALWAYS_INLINE friend constexpr void swap(raw_ptr& lhs, raw_ptr& rhs) noexcept {
    ...
    SK_PA_ALWAYS_INLINE constexpr T* get() const { return wrapped_ptr_; }
...
private:
    ...
    // uninitialized raw_ptr<T>. This is important if we don't want Chrome and Skia standalone to
    // behave differently than other embedders.
    RAW_PTR_EXCLUSION T* wrapped_ptr_ = nullptr;

    template <typename U, RawPtrTraits R> friend class raw_ptr;
};

template <typename U, typename V, RawPtrTraits Traits1, RawPtrTraits Traits2>
SK_PA_ALWAYS_INLINE bool operator==(const raw_ptr<U, Traits1>& lhs, const raw_ptr<V, Traits2>& rhs) {
...
constexpr RawPtrTraits AllowPtrArithmetic = 1 << 1;
...
template <typename T, RawPtrTraits Traits> struct hash<raw_ptr<T, Traits>> {
...
template <typename T, RawPtrTraits Traits> struct iterator_traits<raw_ptr<T, Traits>> {
...
template <typename T, RawPtrTraits Traits> struct pointer_traits<raw_ptr<T, Traits>> {
    using pointer = raw_ptr<T, Traits>;
    using element_type = T;
    using difference_type = ptrdiff_t;

    template <typename U> using rebind = ::raw_ptr<U, Traits>;
...
#endif  // SRC_PARTITION_ALLOC_NOOP_RAW_PTR_H_
