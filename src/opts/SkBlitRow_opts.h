...
// In this variant of SkPMSrcOver each NEON register, dst.val[i], src.val[i], contain the value
...
void blit_row_s32a_opaque(SkPMColor* dst, const SkPMColor* src, int len, U8CPU alpha) {
    ...
    for (; len >= 2; len -= 2, src += 2, dst += 2) {
        uint8x8_t src2 = vld1_u8(reinterpret_cast<uint8_t const*>(src));
        uint8x8_t dst2 = vld1_u8(reinterpret_cast<uint8_t const*>(dst));
        vst1_u8(reinterpret_cast<uint8_t*>(dst), SkPMSrcOver_neon2(dst2, src2));
    }

    if (len != 0) {
        uint8x8_t src2 = vreinterpret_u32_u8(vld1_dup_u32(reinterpret_cast<uint8_t const*>(src)));
        uint8x8_t dst2 = vreinterpret_u32_u8(vld1_dup_u32(reinterpret_cast<uint8_t const*>(dst)));
        uint8x8_t result = SkPMSrcOver_neon2(dst2, src2);
        vst1_lane_u32(reinterpret_cast<uint8_t*>(dst), vreinterpret_u32_u8(result), 0);
    }
    return;
#endif
