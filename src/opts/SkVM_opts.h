        ...
        for ( ; n > 0; start = loop, n -= stride, step_args(stride)) {
            ...
            for (int i = start; i < ninsts; i++) {
                ...
                #define CASE(op) case 2*(int)op: /*fallthrough*/ case 2*(int)op+1
                    ...
                    CASE(Op::i32_to_q14): r[d].i16 = skvx::cast<int16_t>(r[x].i32); break;
                    CASE(Op::q14_to_i32): r[d].i32 = skvx::cast<int32_t>(r[x].i16); break;

                    CASE(Op::f32_to_q14):
                        r[d].i16 = skvx::cast<int16_t>(r[x].f32 * 16384.0f);
                        break;
                    CASE(Op::q14_to_f32):
                        r[d].f32 = skvx::cast<float>(r[x].i16) * (1/16384.0f);
                        break;
