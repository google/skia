...
public:
    ...
    public:
        ...
        int32_t operator[](int k) const {
            SkASSERT(0 <= k && k < 8);
            union { __m256i v; int is[8]; } pun = {fVec};
            return pun.is[k&7];
        }
    ...
    template<> /*static*/ inline Sk8i SkNx_cast<int>(const Sk8h& src) {
        ...
        return { _mm_unpacklo_epi16(src.fVec, zero), _mm_unpackhi_epi16(src.fVec, zero) };
    }
    template<> /*static*/ inline Sk8h SkNx_cast<uint16_t>(const Sk8i& src) {
        return _mm_packs_epi32(src.fLo.fVec, src.fHi.fVec);
    }
...
#if SK_CPU_SSE_LEVEL >= SK_CPU_SSE_LEVEL_AVX2
    static inline Sk8i Sk8f_round(const Sk8f& x) {
        return _mm256_cvtps_epi32(x.fVec);
    }
#else
    static inline Sk8i Sk8f_round(const Sk8f& x) {
        return { Sk4f_round(x.fLo), Sk4f_round(x.fHi) };
    }
#endif
