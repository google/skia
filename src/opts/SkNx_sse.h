...
public:
    ...
    AI void store(void* ptr) const { *(int64_t*)ptr = _mm_cvtsi128_si64(fVec); }
...
template<> AI /*static*/ Sk8b SkNx_cast<uint8_t, int32_t>(const Sk8i& src) {
    const int _ = ~0;
    Sk4i hi, lo;
    SkNx_split(src, &hi, &lo);
    auto tHi = _mm_shuffle_epi8(hi.fVec, _mm_setr_epi8(0,4,8,12, _,_,_,_,  _,_,_,_, _,_,_,_));
    auto tLo = _mm_shuffle_epi8(lo.fVec, _mm_setr_epi8(_,_,_,_,  0,4,8,12, _,_,_,_, _,_,_,_));
    return _mm_or_si128(tHi, tLo);
}
