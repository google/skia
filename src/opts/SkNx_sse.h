...
template<> AI /*static*/ Sk8b SkNx_cast<uint8_t, int32_t>(const Sk8i& src) {
    ...
    auto tLo = _mm_shuffle_epi8(lo.fVec, _mm_setr_epi8(0,4,8,12, _,_,_,_,  _,_,_,_, _,_,_,_));
    auto tHi = _mm_shuffle_epi8(hi.fVec, _mm_setr_epi8(_,_,_,_,  0,4,8,12, _,_,_,_, _,_,_,_));
    return _mm_or_si128(tLo, tHi);
}
