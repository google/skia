...
STAGE(hsl_to_rgb) {
    ...
    auto hue_to_rgb = [](const SkNf& p, const SkNf& q, const SkNf& t) {
        ...
        return (t2 < (1.0f/6.0f)).thenElse(
            p + (q - p)*6.0f*t, (t2 < (3.0f/6.0f)).thenElse(
                q, (t2 < (4.0f/6.0f)).thenElse(
                    p + (q - p)*((4.0f/6.0f) - t2)*6.0f, p)));
    };

    r = (s == 0.f).thenElse(l, hue_to_rgb(p, q, h + (1.0f/3.0f)));
    g = (s == 0.f).thenElse(l, hue_to_rgb(p, q, h));
    b = (s == 0.f).thenElse(l, hue_to_rgb(p, q, h - (1.0f/3.0f)));
}
