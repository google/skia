...
SI void from_u16_be(const void* px, SkNf* r, SkNf* g, SkNf* b, SkNf* a) {
    SkNh rh, gh, bh, ah;
    SkNh::Load4(px, &rh, &gh, &bh, &ah);
...
STAGE_CTX(load_tables_u16_be, const LoadTablesContext*) {
    ...
    r = gather(tail, ctx->fR, SkNx_cast<int>(rh & SkNh(0xFF)));
    g = gather(tail, ctx->fG, SkNx_cast<int>(gh & SkNh(0xFF)));
    b = gather(tail, ctx->fB, SkNx_cast<int>(bh & SkNh(0xFF)));
