...
#define STAGE_MUX(_0,_1,_2,CHOICE, ...) CHOICE
#define STAGE(...) STAGE_MUX(,##__VA_ARGS__, STAGE_CTX(__VA_ARGS__), STAGE_NO_CTX(__VA_ARGS__))
...
STAGE(constant_color, const SkPM4f*) {
    r = ctx->r();
    g = ctx->g();
    b = ctx->b();
    a = ctx->a();
...
// s' = sc for a scalar c.
STAGE(scale_1_float, const float*) {
    SkNf c = *ctx;
...
STAGE(lerp_1_float, const float*) {
    SkNf c = *ctx;
...
STAGE(load_565, const uint16_t**) {
    auto ptr = *ctx + x;
    from_565(load(tail, ptr), &r,&g,&b);
    a = 1.0f;
}
STAGE(load_565_d, const uint16_t**) {
    auto ptr = *ctx + x;
    from_565(load(tail, ptr), &dr,&dg,&db);
    da = 1.0f;
}
STAGE(store_565, uint16_t**) {
    auto ptr = *ctx + x;
...
STAGE(load_f16_d, const uint64_t**) {
    auto ptr = *ctx + x;
...
STAGE(load_8888, const uint32_t**) {
    auto ptr = *ctx + x;
    from_8888(load(tail, ptr), &r, &g, &b, &a);
}
STAGE(load_8888_d, const uint32_t**) {
    auto ptr = *ctx + x;
    from_8888(load(tail, ptr), &dr, &dg, &db, &da);
}
STAGE(store_8888, uint32_t**) {
...
STAGE(load_tables, const LoadTablesContext*) {
    auto ptr = ctx->fSrc + x;
...
STAGE(store_tables, const StoreTablesContext*) {
    auto ptr = ctx->fDst + x;
...
STAGE(matrix_perspective, const float*) {
...
STAGE(parametric_r, const SkColorSpaceTransferFn*) { r = parametric(r, *ctx); }
STAGE(parametric_g, const SkColorSpaceTransferFn*) { g = parametric(g, *ctx); }
STAGE(parametric_b, const SkColorSpaceTransferFn*) { b = parametric(b, *ctx); }
STAGE(parametric_a, const SkColorSpaceTransferFn*) { a = parametric(a, *ctx); }
...
STAGE(table_r, const SkTableTransferFn*) { r = table(r, *ctx); }
STAGE(table_g, const SkTableTransferFn*) { g = table(g, *ctx); }
STAGE(table_b, const SkTableTransferFn*) { b = table(b, *ctx); }
STAGE(table_a, const SkTableTransferFn*) { a = table(a, *ctx); }

STAGE(color_lookup_table, const SkColorLookUpTable*) {
...
STAGE( clamp_x, const float*) { r = clamp (r, *ctx); }
STAGE(repeat_x, const float*) { r = repeat(r, *ctx); }
STAGE(mirror_x, const float*) { r = mirror(r, *ctx); }
STAGE( clamp_y, const float*) { g = clamp (g, *ctx); }
STAGE(repeat_y, const float*) { g = repeat(g, *ctx); }
STAGE(mirror_y, const float*) { g = mirror(g, *ctx); }

STAGE(save_xy, SkImageShaderContext*) {
    r.store(ctx->x);
    g.store(ctx->y);
...
STAGE(accumulate, const SkImageShaderContext*) {
...
STAGE(bilinear_nx, SkImageShaderContext*) { bilinear_x<-1>(ctx, &r); }
STAGE(bilinear_px, SkImageShaderContext*) { bilinear_x<+1>(ctx, &r); }
STAGE(bilinear_ny, SkImageShaderContext*) { bilinear_y<-1>(ctx, &g); }
STAGE(bilinear_py, SkImageShaderContext*) { bilinear_y<+1>(ctx, &g); }
...
STAGE(bicubic_n3x, SkImageShaderContext*) { bicubic_x<-3>(ctx, &r); }
STAGE(bicubic_n1x, SkImageShaderContext*) { bicubic_x<-1>(ctx, &r); }
STAGE(bicubic_p1x, SkImageShaderContext*) { bicubic_x<+1>(ctx, &r); }
STAGE(bicubic_p3x, SkImageShaderContext*) { bicubic_x<+3>(ctx, &r); }

STAGE(bicubic_n3y, SkImageShaderContext*) { bicubic_y<-3>(ctx, &g); }
STAGE(bicubic_n1y, SkImageShaderContext*) { bicubic_y<-1>(ctx, &g); }
STAGE(bicubic_p1y, SkImageShaderContext*) { bicubic_y<+1>(ctx, &g); }
STAGE(bicubic_p3y, SkImageShaderContext*) { bicubic_y<+3>(ctx, &g); }
...
STAGE(gather_a8, const SkImageShaderContext*) {
    const uint8_t* p;
...
STAGE(gather_i8, const SkImageShaderContext*) {
    const uint8_t* p;
    SkNi offset = offset_and_ptr(&p, ctx, r, g);
...
STAGE(gather_g8, const SkImageShaderContext*) {
    const uint8_t* p;
    SkNi offset = offset_and_ptr(&p, ctx, r, g);
...
STAGE(gather_565, const SkImageShaderContext*) {
    const uint16_t* p;
    SkNi offset = offset_and_ptr(&p, ctx, r, g);
...
STAGE(gather_4444, const SkImageShaderContext*) {
    const uint16_t* p;
    SkNi offset = offset_and_ptr(&p, ctx, r, g);

    from_4444(gather(tail, p, offset), &r, &g, &b, &a);
}
STAGE(gather_8888, const SkImageShaderContext*) {
    const uint32_t* p;
    SkNi offset = offset_and_ptr(&p, ctx, r, g);

    from_8888(gather(tail, p, offset), &r, &g, &b, &a);
}
STAGE(gather_f16, const SkImageShaderContext*) {
    const uint64_t* p;
    SkNi offset = offset_and_ptr(&p, ctx, r, g);
