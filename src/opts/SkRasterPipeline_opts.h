...
STAGE(interpreter, SkRasterPipeline_InterpreterCtx* c) {
    float rr[SkRasterPipeline_kMaxStride];
    float gg[SkRasterPipeline_kMaxStride];
    float bb[SkRasterPipeline_kMaxStride];
    float aa[SkRasterPipeline_kMaxStride];

    unaligned_store(rr, r);
    unaligned_store(gg, g);
    unaligned_store(bb, b);
    unaligned_store(aa, a);

    float* args[] = { rr, gg, bb, aa };
    c->byteCode->runStriped(c->fn, args, SK_ARRAY_COUNT(args), tail ? tail : N,
                            (const float*)c->inputs, c->ninputs);

    r = unaligned_load<F>(rr);
    g = unaligned_load<F>(gg);
    b = unaligned_load<F>(bb);
    a = unaligned_load<F>(aa);
}
