...
namespace SK_OPTS_NS {
    ...
    SI void load2(const uint16_t* ptr, size_t tail, U16* r, U16* g) {
        ...
        if (__builtin_expect(tail,0)) {
            ...
            if (tail > 0) {
                *d = high ? _mm_insert_epi32(*d, *(int32_t*)ptr, 2)
                          : _mm_insert_epi32(*d, *(int32_t*)ptr, 0);
            }
        } else {
            _0123 = _mm_loadu_si128(((__m128i*)ptr) + 0);
            _4567 = _mm_loadu_si128(((__m128i*)ptr) + 1);
        }
        auto _0213 = _mm_shuffle_epi32(_0123, 0xD8);
        auto _4657 = _mm_shuffle_epi32(_4567, 0xD8);

        auto _0246 = _mm_movelh_ps(_0213, _4657);
        auto _1357 = _mm_movehl_ps(_4657, _0213);

        auto rg0123 = _mm_unpacklo_epi16(_0246, _1357);  // r0 r1 g0 g1 r2 r3 g0 g3
        auto rg4567 = _mm_unpacklo_epi16(_0246, _1357);  // r4 r6 g4 g5 r6 r7 g6 g7

        *r = _mm_shuffle_ps(rg0123, rg4567, 0x88);
        *g = _mm_shuffle_ps(rg0123, rg4567, 0xDD);
    }
    SI void store2(uint16_t* ptr, size_t tail, U16 r, U16 g) {
    ...
    SI void store2(float* ptr, size_t tail, F r, F g) {
        ...
        if (__builtin_expect(tail, 0)) {
            ...
            if (tail > 0) {
                // _mm256_extract_epi64 is available on 64 bit and not on MSVS.
    #if (defined(_MSC_VER) && !defined(__clang__)) || !defined(__x86_64__)
                auto tmp = high ? _mm256_extractf128_si256(*s, 1) : _mm256_extractf128_si256(*s, 0);
    #  if defined(_M_AMD64) || defined(__x86_64__)
               *(int64_t*)ptr = _mm_extract_epi64(tmp, 0);
    #  else
               *(int32_t*)(ptr + 0) = _mm_extract_epi32(tmp, 0);
               *(int32_t*)(ptr + 1) = _mm_extract_epi32(tmp, 1);
    #  endif
    #else
    ...
    SI void load2(const uint16_t* ptr, size_t tail, U16* r, U16* g) {
        ...
        auto tmp = _mm_shufflelo_epi16(_01, 0xD8);  // r0 r1 g0 g1 r2 g2 r3 g3
             tmp = _mm_shufflehi_epi16(_01, 0xD8);  // r0 r1 g0 g1 r2 r3 g2 g3
        auto R = _mm_shuffle_epi32(tmp, 0x88);      // r0 r1 r2 r3 r0 r1 r2 r3
        auto G = _mm_shuffle_epi32(tmp, 0xDD);      // g0 g1 g2 g3 g0 g1 g2 g3
    ...
    SI void store2(uint16_t* ptr, size_t tail, U16 r, U16 g) {
        ...
        if (__builtin_expect(tail, 0)) {
            if (tail > 1) {
                _mm_storeu_si64(ptr, rg);
                if (tail > 2) {
                    _mm_storeu_si32(ptr, _mm_unpackhi_epi32(rg, rg));
                }
            } else {
                _mm_storeu_si32(ptr, rg);
            }
        } else {
