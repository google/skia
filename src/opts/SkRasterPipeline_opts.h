...
STAGE(save_xy) {
    ...
    // surrounding (x,y).  Fractionally they're all (0.5,0.5) off-center.
    auto fract = [](const SkNf& v) { return v - v.floor(); };
    fract(r + 0.5f).store(sc->fx);
    fract(g + 0.5f).store(sc->fy);
    // TODO: it's worth confirming these are better calculated once and stored
    // than just calculated each time as needed.
}
...
STAGE(accumulate) {
    ...
    // scale contributions in x and y that we multiply together to get the pixel's scale factor.
...
// are combined in direct proportion to their overlapping area with that logical pixel.
// At positive offsets, the x-axis contribution to that rectangular area is fx, or (1-fx)
...
// the center and lower values as you get farther away.  We break this filter function into two
// parts, one for near +/- 0.5 offsets, and one for far +/- 1.5 offsets.
// See GrBicubicEffect for details about this particular Mitchell-Netravali filter.
SI SkNf bicubic_near(const SkNf& t) { return 16/18.0f + 0/18.0f*t - 36/18.0f*t*t + 21/18.0f*t*t*t; }
SI SkNf bicubic_far (const SkNf& t) { return  1/18.0f - 9/18.0f*t + 15/18.0f*t*t -  7/18.0f*t*t*t; }
// TODO: fma, careful parenthesization, etc.
// TODO: rewrite to be oriented for positive offsets instead of negative... less confusing?
