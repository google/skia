...
#elif defined(_MSC_VER) && defined(__clang__) && defined(__x86_64__)
    // SysV ABI makes it very sensible to use wide stages with clang-cl.
    #define ABI __attribute__((sysv_abi))
    #define JUMPER_NARROW_STAGES 0
#elif defined(_MSC_VER)
    // Even though not vectorized, this lets us pass {r,g,b,a} as registers,
    // instead of {b,a} on the stack.  Still need narrow.  (Baby steps...)
    #define ABI __vectorcall
    #define JUMPER_NARROW_STAGES 1
#elif defined(__x86_64__) || defined(__arm64__)
    // These platforms are ideal for wider stages, and the default ABI is fine.
    #define ABI
    #define JUMPER_NARROW_STAGES 0
#else
