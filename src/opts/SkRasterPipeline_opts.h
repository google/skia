...
namespace SK_OPTS_NS {
    ...
    SI F if_then_else(I32 c, F t, F e) {
        return _mm_or_ps(_mm_and_ps(sk_bit_cast<F>(c), t), _mm_andnot_ps(sk_bit_cast<F>(c), e));
    }
...
SI U16 to_half(F f) {
    ...
    return pack(if_then_else(denorm, U32_(0)
