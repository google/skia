...
namespace SK_OPTS_NS {
    ...
    SI SkNx<N,T> load_tail(size_t tail, const T* src) {
        if (tail) {
            ...
            T vals[N] = { src[0] /* zeroes... */ };
            if (N > 1 && tail > 1) { vals[1] = src[1]; }
            if (N > 2 && tail > 2) { vals[2] = src[2]; }
            if (N > 3 && tail > 3) { vals[3] = src[3]; }
            if (N > 4 && tail > 4) { vals[4] = src[4]; }
            if (N > 5 && tail > 5) { vals[5] = src[5]; }
            if (N > 6 && tail > 6) { vals[6] = src[6]; }
    ...
    SI SkNh to_565(const SkNf& r, const SkNf& g, const SkNf& b) {
        return SkNx_cast<uint16_t>( SkNx_round(r * SK_R16_MASK) << SK_R16_SHIFT
                                  | SkNx_round(g * SK_G16_MASK) << SK_G16_SHIFT
                                  | SkNx_round(b * SK_B16_MASK) << SK_B16_SHIFT);
    }
    ...
    STAGE(store_srgb, false) {
        ...
        store_tail(tail, ( sk_linear_to_srgb_noclamp(r) << SK_R32_SHIFT
                         ...
                         |       SkNx_round(255.0f * a) << SK_A32_SHIFT ), (int*)ptr);
    }
