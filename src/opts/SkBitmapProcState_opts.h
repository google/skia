...
#define S32_D32_DX_AVX2_DEFINED 1   // right now i have not implemented S32_alpha_D32_filter_DXDY, this macro exists so that we can use SSE3 version of S32_alpha_D32_filter_DXDY
...
#endif // 1 && SK_CPU_SSE_LEVEL >= SK_CPU_SSE_LEVEL_AVX2
    ...
    static inline __m128i interpolate_in_x_and_y(uint32_t A0, uint32_t A1,
        ...
        if (alphaScale < 256) {
            px = _mm_srli_epi16(_mm_mullo_epi16(px, _mm_set1_epi16(alphaScale)), 8);
        }
        return px;
    };

#ifndef S32_D32_DX_AVX2_DEFINED
    ...
    void S32_alpha_D32_filter_DX(const SkBitmapProcState& s,
        ...
        while (count >= 4) {
            ...
            __m128i AB = interpolate_in_x_and_y(row0[x0[A]], row0[x1[A]],
                                                ...
                                                row0[x0[B]], row0[x1[B]],
                                                row1[x0[B]], row1[x1[B]],
                                                interlaced_x_weights_AB, wy1, s.fAlphaScale);
            ...
            __m128i CD = interpolate_in_x_and_y(row0[x0[C]], row0[x1[C]],
                                                ...
                                                row0[x0[D]], row0[x1[D]],
                                                row1[x0[D]], row1[x1[D]],
                                                interlaced_x_weights_CD, wy1, s.fAlphaScale);
        ...
        while (count --> 0) {
            ...
            __m128i A = interpolate_in_x_and_y(row0[x0], row0[x1],
                                               row1[x0], row1[x1],
                                                      0,        0,
                                                      0,        0,
                                               interlaced_x_weights, _mm_set1_epi16(wy), s.fAlphaScale);

            *colors++ = _mm_cvtsi128_si32(_mm_packus_epi16(A, _mm_setzero_si128()));
        }
    }

#endif // !S32_D32_DX_AVX2_DEFINED
   ...
   /*not static*/ inline void S32_alpha_D32_filter_DXDY(const SkBitmapProcState& s,
        ...
        while (count >= 2) {
            ...
            const __m128i packed = _mm_loadu_si128(reinterpret_cast<const __m128i *>(xy));

            // (x10, y10, x00, y00)
            __m128i xy0_wide = _mm_srli_epi32(packed, 18);
            // (x11, y11, x01, y01)
            __m128i xy1_wide = _mm_and_si128(packed, _mm_set1_epi32(0x3fff));

            __m128i xyl = _mm_unpacklo_epi32( xy0_wide, xy1_wide );
            __m128i xyh = _mm_unpackhi_epi32( xy0_wide, xy1_wide );

            //y10, y00, y11, y01
            //x10, x00, x11, x01
            _mm_storeu_si128(reinterpret_cast<__m128i *>(rl), _mm_unpacklo_epi32( xyl, xyh ));
            _mm_storeu_si128(reinterpret_cast<__m128i *>(bt), _mm_unpackhi_epi32( xyl, xyh ));

            // (x1, y1, x0, y0)
            __m128i wxy = _mm_and_si128(_mm_srli_epi32(packed, 14), _mm_set1_epi32(0xf));
            // (y1, y0, x1, x0)
            wxy = _mm_shuffle_epi8(wxy, _mm_setr_epi8(4,4,4,4,12,12,12,12,0,0,0,0,8,8,8,8));
            __m128i sixteen_minus_xy = _mm_sub_epi8(_mm_set1_epi8(16), wxy);
            __m128i wx = _mm_unpacklo_epi8(sixteen_minus_xy, wxy);
            __m128i wy = _mm_unpackhi_epi8(wxy, _mm_setzero_si128());

            const uint32_t* row00 = reinterpret_cast<const uint32_t*>(src_addr + rl[0] * rb);
            const uint32_t* row01 = reinterpret_cast<const uint32_t*>(src_addr + rl[2] * rb);
            const uint32_t* row10 = reinterpret_cast<const uint32_t*>(src_addr + rl[1] * rb);
            const uint32_t* row11 = reinterpret_cast<const uint32_t*>(src_addr + rl[3] * rb);

            __m128i AB = interpolate_in_x_and_y( row00[bt[0]], row00[bt[2]],
                                                 row01[bt[0]], row01[bt[2]],
                                                 row10[bt[1]], row10[bt[3]],
                                                 row11[bt[1]], row11[bt[3]],
                                                 wx, wy, s.fAlphaScale);

            // Pack lower 4 16 bit values of sum into lower 4 bytes.
            AB = _mm_packus_epi16(AB, _mm_setzero_si128());
            // Extract low int and store.
            _mm_storel_epi64(reinterpret_cast<__m128i*>(colors), AB);
            xy += 4;
            colors += 2;
            count -= 2;
        }
        ...
        while (count-- > 0) {
            ...
            auto row0 = (const uint32_t*)((const uint8_t*)s.fPixmap.addr() +
                                          y0 * s.fPixmap.rowBytes()),
                 row1 = (const uint32_t*)((const uint8_t*)s.fPixmap.addr() +
                                          y1 * s.fPixmap.rowBytes());

            // This is exactly the same flow as the count >= 2 loop above, but writing one pixel.
            int x0, x1, wx;
            decode_packed_coordinates_and_weight(*xy++, &x0, &x1, &wx);
            ...
            __m128i A = interpolate_in_x_and_y(row0[x0], row0[x1], row1[x0], row1[x1], 0, 0, 0, 0,
                                               interlaced_x_weights_A, _mm_set1_epi16(wy), s.fAlphaScale);
    ...
    static void filter_and_scale_by_alpha(unsigned wx, unsigned wy, SkPMColor p00,
                                          SkPMColor p01, SkPMColor p10, SkPMColor p11, SkPMColor* dst,
                                          uint16_t alphaScale) {

        // Load the 4 pixels we're interpolating, in this grid:
        //    | tl  tr |
        //    | bl  br |
        const __m128i tl = _mm_cvtsi32_si128(p00), tr = _mm_cvtsi32_si128(p01),
                    bl = _mm_cvtsi32_si128(p10), br = _mm_cvtsi32_si128(p11);
        ...
        if (alphaScale < 256) {
            ...
            sum = _mm_mullo_epi16(sum, _mm_set1_epi16(alphaScale));
            sum = _mm_srli_epi16(sum, 8);
        }

        *dst = _mm_cvtsi128_si32(_mm_packus_epi16(sum, _mm_setzero_si128()));
      }
