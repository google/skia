...
static void decode_packed_coordinates_and_weight(U32 packed, Out* v0, Out* v1, Out* w) {
    ...
    *w  = (packed >> 14) & 0xf; // Lerp weight for v1; weight for v0 is 16-w.
    ...
    void S32_alpha_D32_filter_DX(const SkBitmapProcState& s,
        ...
        auto row0 = s.fPixmap.addr32(0,y0),
             row1 = s.fPixmap.addr32(0,y1);
        ...
        auto bilerp = [&](skvx::Vec<8,uint32_t> packed_x_coordinates) -> skvx::Vec<8,uint32_t> {
            ...
            // Sum up weighted sample pixels.
            // TODO: plenty of redundant math to optimize here.
            skvx::Vec<32, uint16_t> sum = to_16x4(tl) * (16-wy) * (16-to_16x4(wx))
                                        + to_16x4(bl) * (   wy) * (16-to_16x4(wx))
                                        + to_16x4(tr) * (16-wy) * (   to_16x4(wx))
                                        + to_16x4(br) * (   wy) * (   to_16x4(wx));

            // Get back to [0,255] by dividing by maximum weight 16x16 = 256.
            sum >>= 8;

            // Scale by [0,256] alpha.
            // TODO: try this unconditionally?
        ...
        // TODO: mask load and store,
        while (count --> 0) {
            *colors++ = bilerp(skvx::Vec<8,uint32_t>(*xy++))[0];
        }
