    ...
    *w  = (packed >> 14) & 0xf; // Lerp weight for v1; weight for v0 is 16-w.
    ...
    void S32_alpha_D32_filter_DX(const SkBitmapProcState& s,
        ...
        if (count > 0) {
            ...
            __m256i mask   = _mm256_loadu_si256   ((const __m256i*)masks[count]),
                    coords = _mm256_maskload_epi32((const int*)xy, mask);

            auto px = bilerp(skvx::bit_pun<skvx::Vec<8,uint32_t>>(coords));
            _mm256_maskstore_epi32((int*)colors, mask, skvx::bit_pun<__m256i>(px));
        }
    }
