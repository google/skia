...
for suffix in ['', '_lowp']:
  with open('src/splicer/SkSplicer_generated%s.h' % suffix, 'w') as dst:
    print >>dst,'''/*
 * Copyright 2017 Google Inc.
 *
 * Use of this source code is governed by a BSD-style license that can be
...
''' % (suffix, suffix)
    parse_object_file(dst, 'aarch64%s.o' % suffix, 'unsigned int', '14000000')
    print >>dst,'\n#elif defined(__ARM_NEON__)\n'
    parse_object_file(dst, 'armv7%s.o' % suffix, 'unsigned int', 'eafffffe',
                    target='elf32-littlearm')
    print >>dst,'\n#else\n'
    parse_object_file(dst, 'hsw%s.o' % suffix, 'unsigned char', 'e9 00 00 00 00')
    print >>dst,'\n#endif\n'
    print >>dst,'#endif//SkSplicer_generated%s_DEFINED' % suffix
