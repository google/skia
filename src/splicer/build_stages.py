...
for line in subprocess.check_output(['otool', '-tvj', 'stages.o']).split('\n'):
  ...
  if m:
    name = m.group(1)
    print 'static const char kSplice_' + m.group(1) + '[] = {'
    continue

  # Skip the leading 16 byte address and a tab,
  # leaving the hex and mnemonics of each instruction.
  line = line[17:]
  _hex, mnemonic = line.split('\t', 1)
  _hex = _hex.strip()

  # ret (in done() only) or jmp done (all other stages)
  if _hex == 'c3' or _hex == 'e9 00 00 00 00':
    print '};'
    continue
  ...
  for byte in _bytes:
    print '0x' + byte + ',',
  print ' ' * (48 - 6*len(_bytes)), '//', mnemonic
