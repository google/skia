...
// noinline makes every Stage end in a single jmp (i.e. tail-call) into done(), which is
// the point where we can splice it onto the next Stage.

__attribute__((noinline))
C void done(size_t x, size_t n, void* ctx, K* k,
            F r, F g, F b, F a, F dr, F dg, F db, F da) {
    asm("" :: "r"(x), "r"(n), "r"(ctx), "r"(k),
