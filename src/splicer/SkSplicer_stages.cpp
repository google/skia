...
LOWP(load_8888) {
    ...
    auto to_fixed15 = [](uint8x8_t u8) {
        ...
        L::V u16 = vmovl_u8(u8);  // TODO: tighten up by starting off with vshll_n_u8(u8, 7)?
    ...
    auto to_fixed15 = [k](__m128i u8) {
        L::V u16 = _mm256_cvtepu8_epi16(u8);
        return (u16 << 7) + (u16 >> 1) + ((u16+one<L>(k))>>8);
    };

    // TODO: shorter, more confusing with 256-bit loads and shuffles
