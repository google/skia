...
// (Clang ostensibly has __builtin_convertvector() for cast(), but I don't trust it.)
...
// Clang seems hesistant to use the version of this instruction that loads from memory.
AI static U32 vpmovzxbd(const uint8_t bytes[8]) {
    U32 r;
    asm("vpmovzxbd %1, %0" : "=x"(r) : "m"(*bytes));
    return r;
}
...
STAGE(scale_u8) {
    ...
    auto c = cast(vpmovzxbd(ptr)) * k->_1_255;
