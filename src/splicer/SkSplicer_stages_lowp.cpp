...
STAGE(load_8888) {
    ...
    auto to_fixed15 = [](uint8x8_t u8) {
        ...
        F u16 = vshll_n_u8(u8, 7);         // u16 =  u8*128
...
STAGE(store_8888) {
    ...
    auto from_fixed15 = [](F v) {
        ...
        // But it actually seems to be a little faster to do it as (v saturated+ v) >> 8,
        // which should end up as two instructions: saturated+, then shift and narrow.
        v = (v+v) >> 8;
        return vmovn_u16(v);
    };
    ...
    auto from_fixed15 = [](F v) {
        ...
        // Here we also take the (v saturated+ v) >> 8 approach.
        v = (v+v) >> 8;
