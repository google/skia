...
#elif defined(__ARM_NEON__)
    auto from_fixed15 = [](F v) {
        // Same as aarch64, but first we need to invisibly pad our vectors from 8 to 16 bytes.
        return vqshrn_n_u16(vcombine_u8(v, F{}), 7);
    #if 0
        // This is easiest to do in assembly.
        uint8x8_t r;
        asm("vqshrn.u16 %0, %q1, #7" : "=w"(r) : "w"(v));
        return r;
    #endif
    };
