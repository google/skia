...
STAGE(load_8888) {
    ...
    auto to_fixed15 = [](uint8x8_t u8) {
        ...
        uint16x8_t u16 = vshll_n_u8(u8, 7);         // u16 =  u8*128
    ...
    auto to_fixed15 = [](uint8x8_t u8) {
        ...
        uint16x8_t u16 = vshll_n_u8(u8, 7);
        return vget_low_u16(vrsraq_n_u16(u16, u16, 8));
    };

    uint8x8x4_t rgba;
    rgba = vld4_lane_u8((const uint8_t*)ptr++, rgba, 0);
    rgba = vld4_lane_u8((const uint8_t*)ptr++, rgba, 1);
    rgba = vld4_lane_u8((const uint8_t*)ptr++, rgba, 2);
    rgba = vld4_lane_u8((const uint8_t*)ptr++, rgba, 3);
...
STAGE(store_8888) {
    ...
    auto from_fixed15 = [](F v) {
        ...
        return vqshrn_n_u16(vcombine_u16(v, F{}), 7);
    };
    ...
    vst4_lane_u8((uint8_t*)ptr++, rgba, 0);
    vst4_lane_u8((uint8_t*)ptr++, rgba, 1);
    vst4_lane_u8((uint8_t*)ptr++, rgba, 2);
    vst4_lane_u8((uint8_t*)ptr++, rgba, 3);
