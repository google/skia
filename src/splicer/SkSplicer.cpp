...
namespace {
    ...
    struct Spliced {
        ...
        Spliced(const SkRasterPipeline::Stage* stages, int nstages) {
            ...
            loop(&buf, lowp, loop_start);  // Loop back to handle fStride more pixels if not done.
            after_loop(&buf);
            ret(&buf);  // We're done.
        ...
        void operator()(size_t x, size_t y, size_t n) const {
            size_t body = n/fStride*fStride;   // Largest multiple of fStride (2, 4, 8, or 16) <= n.
            if (fSpliced && body) {            // Can we run fSpliced for at least one fStride?
                ...
                // Fall through to fBackup for any n<fStride last pixels.
                x += body;
                n -= body;
            }
            fBackup(x,y,n);
