...
#undef CASE
    ...
    struct Spliced {
        ...
        #if defined(__aarch64__)
            auto& splice = aarch64_splice;
            auto& inc_x  = aarch64_inc_x;
        #elif defined(__ARM_NEON__)
            // Late generation ARMv7, e.g. Cortex A15 or Krait.
            if (!SkCpu::Supports(SkCpu::NEON|SkCpu::NEON_FMA|SkCpu::VFP_FP16)) {
                return;
            }
            auto& splice = armv7_splice;
            auto& inc_x  = armv7_inc_x;
        #else
            // To keep things simple, only x86-64 for now.
            if (sizeof(void*) != 8) {
                return;
            }
            auto& splice = SkCpu::Supports(SkCpu::HSW)   ?   hsw_splice
                         : SkCpu::Supports(SkCpu::SSE41) ? sse41_splice
                                                         :  sse2_splice;
            auto& inc_x  = SkCpu::Supports(SkCpu::HSW)   ?   hsw_inc_x
                         : SkCpu::Supports(SkCpu::SSE41) ? sse41_inc_x
                                                         :  sse2_inc_x;

            auto& ret = SkCpu::Supports(SkCpu::HSW) ? hsw_ret
                                                    : sse_ret;
        #endif
