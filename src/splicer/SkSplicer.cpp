...
namespace {
    ...
    // Copy len bytes from src to memory that's executable.  Free with munmap(fn, len).
    static void* copy_executable(const void* src, size_t len) {
        // TODO: w^x
    ...
    struct Spliced {
        ...
        Spliced(const SkRasterPipeline::Stage* stages, int nstages) {
            ...
            //splice(&buf, (uint16_t)0x0b0f);  // uncomment to trap on function entry.
            ...
            splice(&buf, cmpq_rsi_rdi);  // if (x < n) { goto loop_start; }
            splice(&buf, jb_near);
            splice(&buf, (int)loop_start - (int)(buf.bytesWritten() + 4));

            // Nope!  We're done.
            splice(&buf, kSplice_done);   // probably just vzeroupper
            splice(&buf, ret);

            auto data = buf.detachAsData();
            fSplicedLen = data->size();
            fSpliced    = copy_executable(data->data(), fSplicedLen);
        }
        ...
        Spliced(const Spliced& o) : fBackup    (o.fBackup)
                                  ...
                                  , fSpliced   (copy_executable(o.fSpliced, fSplicedLen)) {}

        // copy_executable() allocated fSpliced with mmap(), so munmap() it.
        ~Spliced() {
            if (fSpliced) {
        ...
        void operator()(size_t x, size_t y, size_t n) const {
            if (fSpliced) {
                // TODO: At some point we may want to pass in y...
