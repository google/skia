...
SkUnichar SkUTF8_ToUnichar(const char utf8[]) {
    ...
    SkASSERT(SkUTF8_ByteIsValid(c) && !SkUTF8_ByteIsContinuation(c));
...
SkUnichar SkUTF8_NextUnicharWithError(const char** ptr, const char* end) {
    ...
    if (!SkUTF8_ByteIsValid(c) || SkUTF8_ByteIsContinuation(c)) {
        return -1;
    }
    if (hic < 0) {
        uint32_t mask = (uint32_t)~0x3F;
...
size_t SkUTF8_FromUnichar(SkUnichar uni, char utf8[]) {
    ...
    SkUnichar value = uni;
    while (value > 0x7F >> count) {
        *p++ = (char)(0x80 | (value & 0x3F));
        value >>= 6;
        count += 1;
    }
    ...
    if (utf8) {
        ...
        *--utf8 = (char)(~(0xFF >> count) | value);
    }

    SkASSERT(utf8 == nullptr || uni == SkUTF8_ToUnichar(utf8));
    return count;
}
...
static inline uint16_t to_u16(uint32_t value) {
    SkASSERT(value <= UINT16_MAX);
    return (uint16_t)value;
}

size_t SkUTF16_FromUnichar(SkUnichar uni, uint16_t dst[]) {
    if ((unsigned)uni > 0x10FFFF) {
        SkASSERT(false);
        uni = 0;
    }
    ...
    if (dst) {
        if (extra) {
            ...
            dst[0] = to_u16((0xD800 - 64) + (u >> 10));
            dst[1] = to_u16(0xDC00 | (u & 0x3FF));
