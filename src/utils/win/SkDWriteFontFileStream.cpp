...
COM_DECLSPEC_NOTHROW STDMETHODIMP SkDWriteFontFileStreamWrapper::QueryInterface(REFIID iid, void** ppvObject) {
...
COM_DECLSPEC_NOTHROW STDMETHODIMP_(ULONG) SkDWriteFontFileStreamWrapper::AddRef() {
    return InterlockedIncrement(&fRefCount);
}

COM_DECLSPEC_NOTHROW STDMETHODIMP_(ULONG) SkDWriteFontFileStreamWrapper::Release() {
...
COM_DECLSPEC_NOTHROW STDMETHODIMP SkDWriteFontFileStreamWrapper::ReadFileFragment(
...
COM_DECLSPEC_NOTHROW STDMETHODIMP_(void) SkDWriteFontFileStreamWrapper::ReleaseFileFragment(void* fragmentContext) {
    sk_free(fragmentContext);
}

COM_DECLSPEC_NOTHROW STDMETHODIMP SkDWriteFontFileStreamWrapper::GetFileSize(UINT64* fileSize) {
    *fileSize = fStream->getLength();
    return S_OK;
}

COM_DECLSPEC_NOTHROW STDMETHODIMP SkDWriteFontFileStreamWrapper::GetLastWriteTime(UINT64* lastWriteTime) {
