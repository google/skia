...
SkSpotShadowTessellator::SkSpotShadowTessellator(const SkPath& path, const SkMatrix& ctm,
    ...
    //*** can we use offset func here too? ... code is based on points, not indices....
    SkVector normal;
    if (compute_normal(fPrevPoint, fFirstPoint, fDirection, &normal)) {
        SkScalar z = fTransformedHeightFunc(fPrevPoint);
        // TODO: seems like we could use the previous fRadius for this
        this->addArc(this->offset(z)*normal, true);
...
void SkSpotShadowTessellator::projectFromLightAndComputeOffsets(const SkPoint3& lightPos,
    ...
    for (int i = 0; i < fPathPolygon.count(); ++i) {
        ...
        *fPathOffsets.push() = this->offset(pathZ);
        SkScalar factor = baseDZ*SkScalarInvert(lightPos.fZ - pathZ);
        fPathPolygon[i].fX = (lightPos.fZ*fPathPolygon[i].fX - lightPos.fX*pathZ)*factor;
        fPathPolygon[i].fY = (lightPos.fZ*fPathPolygon[i].fY - lightPos.fY*pathZ)*factor;
    }
}
...
void SkSpotShadowTessellator::handlePolyPoint(const SkPoint& p) {
    ...
    if (fInitPoints.count() == 2) {
        ...
        *fColors.push() = fPenumbraColor;
        fRadius = this->offset(fTransformedHeightFunc(fInitPoints[1]));
        this->addEdge(fInitPoints[1], fRadius*unitNormal);
    ...
    if (compute_normal(fPrevPoint, p, fDirection, &normal)) {
        this->addArc(fRadius*normal, true);
        fRadius = this->offset(fTransformedHeightFunc(p));
        this->addEdge(p, fRadius*normal);
    }
}
