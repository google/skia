...
private:
    ...
    void addEdge(const SkVector& nextPoint, const SkVector& nextNormal);
    bool addArcAndIndices(const SkVector& nextNormal, bool finishArc);
...
bool SkSpotShadowTessellator::computeConvexShadow(SkScalar radius) {
    ...
    if (compute_normal(fPrevPoint, fFirstPoint, fDirection, &normal)) {
        normal *= fRadius;
        this->addArcAndIndices(normal, true);
    ...
    if (fPositions.count() >= 3) {
        fPrevUmbraIndex = fFirstVertexIndex;
        fPrevPoint = fFirstPoint;
        if (this->addArcAndIndices(fFirstOutset, false)) {
            if (fFirstUmbraOutside) {
...
bool SkSpotShadowTessellator::handlePolyPoint(const SkPoint& p) {
    ...
    if (fInitPoints.count() == 2) {
        ...
        *fColors.push() = fPenumbraColor;
        *fStartArcIndices.push() = fPositions.count() - 1;
...
void SkSpotShadowTessellator::addEdge(const SkPoint& nextPoint, const SkVector& nextNormal) {
    ...
    // add next penumbra point and quad or triangle
...
bool SkSpotShadowTessellator::addArcAndIndices(const SkVector& nextNormal, bool finishArc) {
    int lastVertIndex = fPositions.count() - 1;
    if (!this->addArc(nextNormal, finishArc)) {
        return false;
    }
    ...
    } else if (fAddToStartArc) {
        for (int i = lastVertIndex + 1; i < fPositions.count(); ++i) {
            *fStartArcIndices.push() = i;
        }
    }
...
// TODO: make this work for concave paths
void SkSpotShadowTessellator::tessellateQuad(uint16_t prevPenumbraIndex, uint16_t currUmbraIndex,
                                             const SkPoint& newPenumbraPoint) {
    SkVector outset0 = fPositions[prevPenumbraIndex] - fPositions[fPrevUmbraIndex];
    SkVector outset1 = newPenumbraPoint - fPositions[currUmbraIndex];
    outset0.normalize();
    outset1.normalize();
    if (outset0.dot(outset1) < 0.97f) {
        SkVector innerTransverse = fPositions[currUmbraIndex] - fPositions[fPrevUmbraIndex];
        SkVector outerTransverse = newPenumbraPoint - fPositions[prevPenumbraIndex];
        SkScalar outerLength = SkVector::Normalize(&outerTransverse);
        outerTransverse *= ((outerLength - innerTransverse.length())*SK_ScalarHalf);

        *fPositions.push() = fPositions[prevPenumbraIndex] + outerTransverse;
        *fColors.push() = fPenumbraColor;
        ...
        // add center quad if necessary
        if (currUmbraIndex != fPrevUmbraIndex) {
            *fPositions.push() = newPenumbraPoint - outerTransverse;
            *fColors.push() = fPenumbraColor;
        ...
        // restart arc and add final triangle
        fAddToStartArc = false;
        *fArcIndices.push() = prevPenumbraIndex;
        ...
        *fColors.push() = fPenumbraColor;
        if (currUmbraIndex != fPrevUmbraIndex) {
            this->adjustFalloff(fPrevUmbraIndex, false);
            this->appendQuad(fPrevUmbraIndex, currUmbraIndex, prevPenumbraIndex,
                             fPositions.count() - 1);
            fAddToStartArc = false;
            *fArcIndices.push() = fPositions.count() - 1;
        } else {
            this->appendTriangle(fPrevUmbraIndex, prevPenumbraIndex, fPositions.count() - 1);
            if (!fArcIndices.isEmpty()) {
                *fArcIndices.push() = fPositions.count() - 1;
            } else if (fAddToStartArc) {
                *fStartArcIndices.push() = fPositions.count() - 1;
...
void SkSpotShadowTessellator::adjustFalloff(int currArcPivot, bool includeStartArc) {
    if (!fArcIndices.isEmpty() && includeStartArc) {
        for (int i = 0; i < fStartArcIndices.count(); ++i) {
            *fArcIndices.push() = fStartArcIndices[i];
        }
    }
    ...
    SkScalar midFalloff0 = outerRadius / umbraInset;
    s = d.cross(v0) / perpDot;
    umbraInset = v1Length*s;
    outerRadius = umbraInset - v1Length;
    SkScalar midFalloff1 = outerRadius / umbraInset;

    // Need corner falloff adjustment to be smaller than either mid adjustment and decaying faster
    SkScalar cornerFalloff = SkTMin(midFalloff0, midFalloff1);
    cornerFalloff *= cornerFalloff;
    SkASSERT(cornerFalloff <= midFalloff0 && cornerFalloff <= midFalloff1);

    // Now adjust color values
    int alpha = SkColorGetA(fPenumbraColor);
    int midFactor = SkScalarTruncToInt(255.9f*midFalloff0);
    fColors[currArcSecondPoint] = SkColorSetARGB(alpha, midFactor, midFactor, midFactor);
    midFactor = SkScalarTruncToInt(255.9f*midFalloff1);
    ...
    SkScalar deltaFalloff0 = midFalloff0 - cornerFalloff;
    SkScalar deltaFalloff1 = midFalloff1 - cornerFalloff;
    for (int i = 2; i < fArcIndices.count()-2; ++i) {
        ...
        int factor = SkScalarTruncToInt(255.9f*(cornerFalloff + s*deltaFalloff0 + t*deltaFalloff1));
