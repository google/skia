...
SkSpotShadowTessellator::SkSpotShadowTessellator(const SkPath& path,
    ...
    // We are going to apply 'scale' and 'xlate' (in that order) to each computed path point. We the
    // effect to be to scale the points relative to the path centroid and then translate them by
    // by the 'translate' param we were passed.
