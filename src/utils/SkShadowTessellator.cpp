...
private:
    ...
    SkColor umbraColor(SkScalar z) {
        SkScalar umbraAlpha = SkScalarInvert(SkDrawShadowMetrics::AmbientAlpha(z));
...
SkAmbientShadowTessellator::SkAmbientShadowTessellator(const SkPath& path,
    ...
    // Set base colors
    SkScalar umbraAlpha = SkScalarInvert(SkDrawShadowMetrics::AmbientAlpha(heightFunc(0, 0)));
...
SkSpotShadowTessellator::SkSpotShadowTessellator(const SkPath& path, const SkMatrix& ctm,
    ...
    if (!ctm.hasPerspective()) {
        ...
        fTransparent = true;
        radius = SkDrawShadowMetrics::SpotBlurRadius(occluderHeight, lightPos.fZ,
                                                           lightRadius);
    }
    fRadius = radius;
