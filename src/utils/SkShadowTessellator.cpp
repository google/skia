...
private:
    ...
    SkScalar offset(SkScalar z) {
        return SkDrawShadowMetrics::AmbientShadowBlurRadius(z);
    }
    SkColor umbraColor(SkScalar z) {
        return SkColorSetARGB(SkDrawShadowMetrics::AmbientShadowAlpha(z) * 255.9999f, 0, 0, 0);
    }
...
SkAmbientShadowTessellator::SkAmbientShadowTessellator(const SkPath& path,
    ...
    // Set base colors
    SkScalar umbraAlpha = SkDrawShadowMetrics::AmbientShadowAlpha(heightFunc(0, 0));
...
SkSpotShadowTessellator::SkSpotShadowTessellator(const SkPath& path, const SkMatrix& ctm,
    ...
    if (!ctm.hasPerspective()) {
        SkScalar scale;
        SkVector translate;
        SkDrawShadowMetrics::GetSpotShadowParams(occluderHeight, lightPos.fX, lightPos.fY,
                                                 fLightZ, lightRadius,
                                                 &radius, &scale, &translate);
        ...
        fTransparent = true;
        radius = SkDrawShadowMetrics::SpotShadowBlurRadius(occluderHeight, lightPos.fZ,
                                                           lightRadius);
    }
    fRadius = radius;
