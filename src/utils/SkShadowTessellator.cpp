...
void SkSpotShadowTessellator::adjustFalloff(int currArcPivot, bool includeStartArc) {
    ...
    int currArcEndPoint = fArcIndices[fArcIndices.count() - 1];
    SkPoint pivot = fPositions[currArcPivot];
    SkVector v0 = fPositions[currArcStartPoint] - pivot;
    SkScalar v0LengthSq = v0.dot(v0);
    SkVector v1 = fPositions[currArcEndPoint] - pivot;
    SkScalar v1LengthSq = v1.dot(v1);
    SkScalar minLengthSq = SkTMin(v0LengthSq, v1LengthSq);

    int alpha = SkColorGetA(fPenumbraColor);

    for (int i = 1; i < fArcIndices.count() - 1; ++i) {
        int index = fArcIndices[i];
        SkVector w = fPositions[index] - pivot;
        SkScalar adjustment = SkTMin(SK_Scalar1, minLengthSq / w.dot(w));
        adjustment *= adjustment;
        int factor = SkScalarTruncToInt(255.9f*adjustment);
