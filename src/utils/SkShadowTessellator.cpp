...
void SkBaseShadowTessellator::handleLine(const SkMatrix& m, SkPoint* p) {
    SkScalar zValue = fHeightFunc(p->fX, p->fY);
    m.mapPoints(p, 1);
    this->handleLine(*p);
}
...
SkSpotShadowTessellator::SkSpotShadowTessellator(const SkPath& path, const SkMatrix& ctm,
    ...
    //**** generate offsets for perspective transforms
    if (ctm.hasPerspective()) {
        this->projectFromLightAndComputeOffsets(lightPos, lightRadius);
    }

    //**** set up offset func

    // check to see if umbra collapses
    //**** use index-based offset func here
    SkScalar minDistSq = fCentroid.distanceToLineSegmentBetweenSqd(fPathPolygon[0],
                                                                   fPathPolygon[1]);
    SkScalar currRadius = this->offset(fTransformedHeightFunc(fPathPolygon[0]));
    SkScalar nextRadius = this->offset(fTransformedHeightFunc(fPathPolygon[1]));
    ...
    for (int i = 1; i < fPathPolygon.count(); ++i) {
        ...
        SkScalar nextRadius = this->offset(fTransformedHeightFunc(nextPoint));
    ...
    // generate inner ring
    std::function<SkScalar(const SkPoint&)> insetRadiusFunc = [this](const SkPoint& p) {
        return this->offset(this->fTransformedHeightFunc(p)) - this->fOffsetAdjust;
    };
...
void SkSpotShadowTessellator::projectFromLightAndComputeOffsets(const SkPoint3& lightPos,
    ...
    for (int i = 0; i < fPathPolygon.count(); ++i) {
        ...
        *fPathOffsets.push() = this->offset(pathZ);
        SkScalar recip = SkScalarInvert(lightPos.z - pathZ);
        fPathPolygon[i].fX = (lightPos.z*fPathPolygon[i].fX - lightPos.x*pathZ)*recip;
        fPathPolygon[i].fY = (lightPos.z*fPathPolygon[i].fY - lightPos.y*pathZ)*recip;
    }
}
