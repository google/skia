...
SkSpotShadowTessellator::SkSpotShadowTessellator(const SkPath& path, const SkMatrix& ctm,
    ...
    if (!ctm.hasPerspective()) {
        ...
        SkPoint3 h0, h1, h2;
        // generate crossing point between top and bottom edges (gives new x-axis)
        h0 = (pts3D[1].cross(pts3D[0])).cross(pts3D[2].cross(pts3D[3]));
        // generate crossing point between left and right edges (gives new y-axis)
        h1 = (pts3D[0].cross(pts3D[3])).cross(pts3D[1].cross(pts3D[2]));
        // generate crossing point between diagonals (gives new origin)
