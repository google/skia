...
bool SkSpotShadowTessellator::computeConcaveShadow(SkScalar radius) {
    ...
    *fPositions.push() = fUmbraPolygon[0];
    *fColors.push() = fUmbraColor;
    fPrevUmbraIndex = 1;
    ...
    int currUmbra = 0;
    int nextUmbra = 1;
    while (penumbraIndices[currPenumbra] < fPathPolygon.count() ||
           umbraIndices[currUmbra] < fPathPolygon.count()) {
        if (penumbraIndices[nextPenumbra] < umbraIndices[nextUmbra]) {
            ...
            } while (penumbraIndices[nextPenumbra] == penumbraIndices[currPenumbra]);
        } else if (penumbraIndices[nextPenumbra] > umbraIndices[nextUmbra]) {
            // fill out umbra arc
            do {
            ...
            } while (umbraIndices[nextUmbra] == umbraIndices[currUmbra]);
        } else {
            // advance both
            *fPositions.push() = penumbraPolygon[nextPenumbra];
            *fColors.push() = fPenumbraColor;
