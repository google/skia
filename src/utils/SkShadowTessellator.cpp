...
void SkBaseShadowTessellator::finishPathPolygon() {
    if (fPathPolygon.count() > 2) {
        const SkPoint& currPoint = fPathPolygon[fPathPolygon.count() - 1];
        const SkPoint& nextPoint = fPathPolygon[0];
        // remove coincident point
        if (duplicate_pt(currPoint, nextPoint)) {
            fPathPolygon.pop();
        // otherwise add to centroid
        } else {
        ...
        // have to recheck this in case we removed a coincident point
        if (fPathPolygon.count() > 2) {
            SkScalar cross = perp_dot(fPathPolygon[fPathPolygon.count() - 2],
...
SkSpotShadowTessellator::SkSpotShadowTessellator(const SkPath& path, const SkMatrix& ctm,
    ...
    } else if (fTransparent || !this->computeConcaveShadow(radius)) {
        return;
    }
