...
SkSpotShadowTessellator::SkSpotShadowTessellator(const SkPath& path, const SkMatrix& ctm,
    ...
    // For the path polygon we are going to apply 'scale' and 'translate' (in that order) to each
    // computed path point. We want the effect to be to scale the points relative to the path
    // bounds center and then translate them.
    SkVector adjustedTranslate = center * (1.f - scale) + translate;
    SkMatrix shadowTransform;
    shadowTransform.setScaleTranslate(scale, scale, adjustedTranslate.fX, adjustedTranslate.fY);
