...
//typedef uint32_t __attribute__((__may_alias__, __aligned__ (1))) uint1;
//namespace {
//union U {
//    const char* c;
//    const uint1* i;
...
constexpr uint32_t hash4(const char* string, size_t n, uint32_t hash) {
    return hash_4_bytes(load(string + 4 * n), hash);
}


// TAIL-RECURSIVE
constexpr uint32_t constexpr_strlen(const char* s, uint32_t acc) {
    return *s ? constexpr_strlen(s + 1, 1 + acc) : 0;
}

// TAIL-RECURSIVE
constexpr uint32_t headhash_tr(const char* string, size_t n, size_t max, uint32_t hash) {
    return n == max ? hash : headhash_tr(string, n + 1, max, hash4(string, n, hash));
}

constexpr uint32_t headhash(const char* string, size_t n, uint32_t hash) {
    return headhash_tr(string, 0, n, hash);
}

// TAIL-RECURSIVE
constexpr uint32_t subload(const char* data, size_t len, uint32_t accumulator) {
...
constexpr uint32_t tailhash(const char* string, size_t len, uint32_t hash) {
    return len == 0 ? hash : hash ^ tailmix(subload(string, len, 0));
}

constexpr uint32_t hasher2(const char* string, size_t len, uint32_t hash) {
    return mix(tailhash(string + 4 * (len / 4), len % 4, headhash(string, len / 4, hash)) ^ len);
}

constexpr uint32_t hasher(const char* string) {
    return hasher2((const char*)string, constexpr_strlen(string, 0), 0);
}
