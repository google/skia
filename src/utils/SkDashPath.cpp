...
static bool cull_path(const SkPath& srcPath, const SkStrokeRec& rec,
    ...
    if (srcPath.isRect(nullptr)) {
        ...
        SkScalar accumPhase = 0;  // Running phase to keep the phase correct around the rect
        while (iter.next(pts) == SkPath::kLine_Verb) {
            // Notice this vector v works with the original unclipped length.
            SkVector v = pts[1] - pts[0];

            if (clip_line(pts, bounds, intervalLength, accumPhase)) {
            ...
            accumPhase = SkScalarMod(accumPhase + SkScalarAbs(v.fX + v.fY), intervalLength);
        }
