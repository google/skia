...
bool SkDashPath::InternalFilter(SkPath* dst, const SkPath& src, SkStrokeRec* rec,
    ...
    do {
        ...
        // Per-verb metrics needed for aligned dashing. Only compute if needed.
        double currentVerbLength;
        double totalVerbLengths;
        double currentVerbAlignErr;
        double firstVerbAlignErr;
        bool firstInVerb;
        if (alignedDashes) {
            currentVerbLength = meas.getVerbLength(0);
            totalVerbLengths  = currentVerbLength;
            currentVerbAlignErr = computeAlignedDashError(currentVerbLength, intervalLength);
            firstVerbAlignErr = currentVerbAlignErr;
            firstInVerb = true;
        }
        ...
        while (distance < length) {
            ...
            if (alignedDashes) {
                ...
                dlen += currentVerbAlignErr * (dlen / intervalLength);
                SkASSERT(dlen >= 0);
                ...
                if (is_even(index)) {
                    ...
                    if (!skipFirstSegment) {
                        addedSegment = true;
                        ++segCount;
                        const bool moveTo = segCount == 1 || !firstInVerb;
