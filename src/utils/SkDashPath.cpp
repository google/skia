...
static bool cull_path(SkPoint pts[2], const SkRect& bounds, SkScalar intervalLength,
    ...
    if (minXY == maxXY) {
        (&pts[1].fX)[xyOffset] += maxXY * FLT_EPSILON * 32;  // 16 does not draw; length stays zero
    }
    return true;
}

static bool contains(const SkRect& rect, const SkPoint& pt) {
...
static bool cull_path(const SkPath& srcPath, const SkStrokeRec& rec,
    ...
    if (isRect) {
        ...
        while (SkPath::kLine_Verb == iter.next(pts)) {
            ...
            if (v.fX ? between(cullRect->fTop, pts[0].fY, cullRect->fBottom) :
                ...
                bool skipMoveTo = contains(*cullRect, pts[0]);
    ...
    SkASSERT(isLine);
    if (!cull_path(pts, bounds, intervalLength, 0)) {
        return false;
    }
    dstPath->moveTo(pts[0]);
    dstPath->lineTo(pts[1]);
