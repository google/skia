...
// For N much smaller, a simple search is faster. Must be at least 2 for the find_with_slope
// algorithm to work (since it looks at the first and last values, and assumes they are not equal.
//
...
void SkCharToGlyphCache::insertCharAndGlyph(int index, SkUnichar unichar, SkGlyphID glyph) {
    ...
    *fV16.insert(index) = glyph;
    ...
    const int count = fK32.count();
    if (count >= 3 && (index == 1 || index == count - 2)) {
        SkASSERT(index >= 1 && index <= count - 2);
