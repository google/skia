...
sk_sp<SkImage> SkAnimCodecPlayer::getFrameAt(int index) {
    ...
    if (requiredFrame != SkCodec::kNoFrame && fImages[requiredFrame]) {
        ...
        auto canvas = SkCanvas::MakeRasterDirect(fImageInfo, data->writable_data(), rb);
    ...
    auto image = SkImage::MakeRasterData(fImageInfo, std::move(data), rb);
    if (origin != kDefault_SkEncodedOrigin) {
        auto imageInfo = fImageInfo.makeDimensions(orientedDims);
