...
const char* SkParse::FindNamedColor(const char* name, size_t len, SkColor* color) {
    const auto* rec = std::lower_bound(std::begin(gNamedColors),
                                       ...
                                       [](const ColorRec& rec, const ColorRec& key) {
                                           // we actually want string::compare, but without copying.
                                           // with c++17 we could use string_view, but for now...
                                           for (size_t i = 0;; ++i) {
                                               // full prefix match
                                               if (!rec.fName[i]) { return false; }

                                               // mismatch
                                               if (rec.fName[i] != key.fName[i]) {
                                                   return rec.fName[i] < key.fName[i];
                                               }
    ...
    if (rec == std::end(gNamedColors) || strcmp(name, rec->fName)) {
        return nullptr;
    }
