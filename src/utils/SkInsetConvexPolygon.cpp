...
// Offset line segment p0-p1 'distance' units in the direction specified by 'dir'
...
bool SkInsetConvexPolygon(const SkPoint* inputPolygonVerts, int inputPolygonSize,
    ...
    for (int i = 0; i < inputPolygonSize; ++i) {
        if (edgeData[i].fValid && (currIndex == -1 ||
            !edgeData[i].fIntersection.equalsWithinTolerance((*insetPolygon)[currIndex], 0.01f))) {
    ...
    if (currIndex >= 1 &&
        (*insetPolygon)[0].equalsWithinTolerance((*insetPolygon)[currIndex], 0.01f)) {
        insetPolygon->pop();
    }
