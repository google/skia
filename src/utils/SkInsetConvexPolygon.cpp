...
static void inset_edge(const SkPoint* points, int index0, int index1,
                       std::function<SkScalar(int index)> insetDistanceFunc, int dir,
                       InsetSegment* inset) {
    SkASSERT(dir == -1 || dir == 1);
    // compute perpendicular
    SkVector perp;
    SkPoint p0 = points[index0];
    SkPoint p1 = points[index1];
    perp.fX = p0.fY - p1.fY;
    perp.fY = p1.fX - p0.fX;
    perp.normalize();
    SkVector normal = perp;
    normal *= insetDistanceFunc(index0);
    inset->fP0 = p0 + normal;
    normal = perp;
    normal *= insetDistanceFunc(index1);
    inset->fP1 = p1 + normal;
}
...
bool SkInsetConvexPolygon(const SkPoint* inputPolygonVerts, int inputPolygonSize,
    ...
    for (int i = 0; i < inputPolygonSize; ++i) {
        ...
        inset_edge(inputPolygonVerts, i, j, insetDistanceFunc, winding,
                   &edgeData[i].fInset);
