...
SkDeferredCanvas::SkDeferredCanvas(SkCanvas* canvas, bool eagerlyEvaluatePictsAndDrawables)
    : INHERITED(canvas->getBaseLayerSize().width(), canvas->getBaseLayerSize().height())
    , fEagerlyEvaluatePictsAndDrawables{eagerlyEvaluatePictsAndDrawables}
{
    this->reset(canvas);
}
...
void SkDeferredCanvas::onDrawPicture(const SkPicture* picture, const SkMatrix* matrix,
                                 const SkPaint* paint) {
    if (fEagerlyEvaluatePictsAndDrawables) {
