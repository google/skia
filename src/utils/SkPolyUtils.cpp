...
bool SkInsetConvexPolygon(const SkPoint* inputPolygonVerts, int inputPolygonSize,
    ...
    SkScalar perpLength = inset*winding;
    for (int curr = 0; curr < inputPolygonSize; prev = curr, ++curr) {
        ...
        perp.setLength(perpLength);
        edgeData[curr].fPrev = &edgeData[prev];
        edgeData[curr].fNext = &edgeData[next];
        edgeData[curr].fInset.fP0 = inputPolygonVerts[curr] + perp;
        edgeData[curr].fInset.fV = v;
        edgeData[curr].init();
    }
...
bool SkOffsetSimplePolygon(const SkPoint* inputPolygonVerts, int inputPolygonSize, SkScalar offset,
    ...
    int numEdges = 0;
    SkVector prevNormal, currNormal;
    SkScalar normalLength = winding*offset;
    for (int currIndex = 0, prevIndex = inputPolygonSize - 1;
        ...
        currNormal.set(inputPolygonVerts[currIndex].fY - inputPolygonVerts[nextIndex].fY,
                       inputPolygonVerts[nextIndex].fX - inputPolygonVerts[currIndex].fX);
        currNormal.setLength(normalLength);
        if (currIndex > 0) {
            // if reflex point, we need to add extra edges
            if (is_reflex_vertex(inputPolygonVerts, winding, offset,
                ...
                int numSteps;
                if (!SkComputeRadialSteps(prevNormal, currNormal, offset,
