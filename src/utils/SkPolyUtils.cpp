...
struct ActiveEdge {
    ...
    bool above(const ActiveEdge& that, SkScalar tolerance = SK_ScalarNearlyZero) {
...
public:
    ...
    bool insert(const ActiveEdge& newEdge) {
    ...
    bool remove(const ActiveEdge& edge) {
        ...
        if (removeIndex < fEdges.count() - 1) {
            ...
            memmove(&fEdges[removeIndex], &fEdges[removeIndex + 1],
                    sizeof(ActiveEdge)*(fEdges.count() - removeIndex - 1));
        }
...
bool SkIsSimplePolygon(const SkPoint* polygon, int polygonSize) {
    ...
    for (int i = 0; i < polygonSize; ++i) {
        ...
        if (v.fFlags & kPrevLeft_VertexFlag) {
            ...
            ActiveEdge edge{ { v.fPosition, polygon[v.fPrevIndex] }, v.fIndex, v.fPrevIndex };
        ...
        if (v.fFlags & kNextLeft_VertexFlag) {
            ...
            ActiveEdge edge{ { v.fPosition, polygon[v.fNextIndex] }, v.fIndex, v.fNextIndex };
            if (!sweepLine.insert(edge)) {
                return false;
            }
        }
...
bool SkOffsetSimplePolygon(const SkPoint* inputPolygonVerts, int inputPolygonSize,
    ...
    for (int currIndex = 0; currIndex < edgeData.count(); ++currIndex) {
