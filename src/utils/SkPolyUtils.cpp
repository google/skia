...
static void reclassify_vertex(TriangulateVertex* p, const SkPoint* polygonVerts,
                              int winding, ReflexHash* reflexHash,
                              SkTInternalLList<TriangulateVertex>* convexList) {
    if (TriangulateVertex::kReflex_VertexType == p->fVertexType) {
        ...
        if (winding*v0.cross(v1) > SK_ScalarNearlyZero) {
            p->fVertexType = TriangulateVertex::kConvex_VertexType;
            reflexHash->remove(p);
...
bool SkTriangulateSimplePolygon(const SkPoint* polygonVerts, uint16_t* indexMap, int polygonSize,
    ...
    for (int i = 0; i < polygonSize; ++i) {
        SkDEBUGCODE(memset(&triangulationVertices[currIndex], 0, sizeof(TriangulateVertex)));
    ...
    // we have trianglized the entire polygon.
    ...
    while (vertexCount > 3) {
        ...
        for (; (earVertex = convexIter.get()); convexIter.next()) {
            SkASSERT(TriangulateVertex::kReflex_VertexType != earVertex->fVertexType);
