...
static int compute_side(const SkPoint& p0, const SkVector& v0, const SkPoint& p) {
    SkVector w = p - p0;
    SkScalar perpDot = v0.cross(w);
...
static inline bool compute_params(const SkVector& w, const SkVector& v,
                                  SkScalar* numer, SkScalar* denom) {
...
static bool compute_intersection(const OffsetSegment& s0, const OffsetSegment& s1,
    ...
    if (SkScalarNearlyZero(denom, SK_ScalarNearlyZero*SK_ScalarNearlyZero)) {
        ...
        if (!SkScalarNearlyZero(w.cross(v0), SK_ScalarNearlyZero*SK_ScalarNearlyZero) ||
            !SkScalarNearlyZero(w.cross(v1), SK_ScalarNearlyZero*SK_ScalarNearlyZero)) {
            return false;
        }
        ...
        if (!SkPointPriv::CanNormalize(v0.fX, v0.fY)) {
            ...
            denomPositive = compute_params(-w, v1, &tNumer, &denom);
            ...
            denomPositive = compute_params(w, v0, &sNumer, &denom);
            tNumer = 0;
            if (outside_interval(sNumer, denom, denomPositive)) {
                ...
                denomPositive = compute_params(w + v1, v0, &sNumer, &denom);
                tNumer = denom;
                if (outside_interval(sNumer, denom, denomPositive)) {
                    ...
                    sNumer = 0;
                    compute_params(-w, v1, &tNumer, &denom);
                }
            }
        }
    } else {
...
static bool in_segment(const SkPoint& p0, const SkPoint& p1, const SkPoint& p) {
    return (p.fX >= SkTMin(p0.fX, p1.fX) && p.fX <= SkTMax(p0.fX, p1.fX) &&
            p.fY >= SkTMin(p0.fY, p1.fY) && p.fY <= SkTMax(p0.fY, p1.fY));
}
...
struct ActiveEdge {
    ...
    bool intersect(const ActiveEdge& that) const {
        ...
        if (side0 == 0 && in_segment(p0, p1, q0)) {
            return true;
        }
        // p1+v1 lies on segment0's line, see if it's inside the segment
        if (side1 == 0 && in_segment(p0, p1, q1)) {
            return true;
        }
        // p0 lies on segment1's line, see if it's inside the segment
        if (side2 == 0 && in_segment(q0, q1, p0)) {
            return true;
        }
        // p0 + v0 lies on segment0's line, see if it's inside the segment
        if (side3 == 0 && in_segment(q0, q1, p1)) {
            return true;
        }
