...
bool SkTriangulateSimplePolygon(const SkPoint* polygonVerts, uint16_t* indexMap, int polygonSize,
    ...
    // get bounds and winding direction
    // setup for winding
    SkScalar quadArea = 0;
    SkVector v0 = polygonVerts[1] - polygonVerts[0];
    // setup for bounds
    Sk4s min, max;
    if (polygonSize & 1) {
        min = max = Sk4s(polygonVerts[polygonSize-1].fX, polygonVerts[polygonSize-1].fY,
                         polygonVerts[polygonSize-1].fX, polygonVerts[polygonSize-1].fY);
        Sk4s xy = Sk4s::Load(polygonVerts);
        min = Sk4s::Min(min, xy);
        max = Sk4s::Max(max, xy);
    } else {
        min = max = Sk4s::Load(polygonVerts);
    }
    for (int curr = 2; curr < polygonSize; ++curr) {
        if (!(curr & 0x1)) {
            Sk4s xy = Sk4s::Load(&polygonVerts[curr]);
            min = Sk4s::Min(min, xy);
            max = Sk4s::Max(max, xy);
        }
        SkVector v1 = polygonVerts[curr] - polygonVerts[0];
    ...
    int prevIndex = polygonSize - 1;
    SkVector vcp = polygonVerts[0] - polygonVerts[prevIndex];
    for (int currIndex = 0; currIndex < polygonSize; ++currIndex) {
        ...
        SkVector vnc = polygonVerts[nextIndex] - polygonVerts[currIndex];
        if (winding*vcp.cross(vnc) > SK_ScalarNearlyZero*SK_ScalarNearlyZero) {
    ...
    for (int currIndex = 0; currIndex < polygonSize; ++currIndex) {
        ...
        if (TriangulationVertex::VertexType::kConvex == currType) {
            ...
            // We prioritize convex vertices with neighboring reflex vertices
            // to help cull out reflex vertices quickly
