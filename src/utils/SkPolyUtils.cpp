...
struct ActiveEdge {
    ActiveEdge() : fChild{ nullptr, nullptr }, fBelow(nullptr), fRed(false) {}
...
public:
    ActiveEdgeList() {}
    ~ActiveEdgeList() { destroy(fTreeHead.fChild[1]); fTreeHead.fChild[1] = nullptr; }

    bool insert(const SkPoint& p0, const SkPoint& p1, uint16_t index0, uint16_t index1) {
        SkVector v = p1 - p0;
        // empty tree case -- easy
        if (!fTreeHead.fChild[1]) {
            ActiveEdge* root = fTreeHead.fChild[1] = new ActiveEdge(p0, v, index0, index1);
            SkASSERT(root);
            if (!root) {
                return false;
            }
        ...
        //verify_tree(fTreeHead.fChild[1]);
    ...
    bool remove(const SkPoint& p0, const SkPoint& p1, uint16_t index0, uint16_t index1) {
        ...
        if (found) {
            ...
            ActiveEdge* pred = grandparent ? curr : nullptr;
