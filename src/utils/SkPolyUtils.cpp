...
bool SkIsConvexPolygon(const SkPoint* polygonVerts, int polygonSize) {
    ...
    SkVector v0 = polygonVerts[0] - polygonVerts[polygonSize - 1];
    SkVector v1 = polygonVerts[1] - polygonVerts[polygonSize - 1];
    SkScalar winding = v0.cross(v1);

    for (int i = 0; i < polygonSize - 1; ++i) {
        int j = i + 1;
        int k = (i + 2) % polygonSize;

        SkVector v0 = polygonVerts[j] - polygonVerts[i];
        SkVector v1 = polygonVerts[k] - polygonVerts[i];
        SkScalar perpDot = v0.cross(v1);
