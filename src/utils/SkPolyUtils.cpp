    ...
    while (head && prevEdge != currEdge) {
        ...
        if (prevEdge->checkIntersection(currEdge, &intersection, &s, &t)) {
            ...
            if (dist0 < 0 && dist1 < 0) {
                ...
                bool prevSameContour = SkScalarNearlyZero(SkPoint::Distance(p1, p0));

                p1 = currEdge->fInset.fP0 + currEdge->fInset.fV;
                p0 = currNextEdge->fInset.fP0;
                bool currSameContour = SkScalarNearlyZero(SkPoint::Distance(p1, p0));

                // we want to step along a contour to find intersections rather than jump to a new one
