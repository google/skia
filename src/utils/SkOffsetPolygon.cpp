...
static bool compute_intersection(const OffsetSegment& s0, const OffsetSegment& s1,
    ...
    SkASSERT(!SkScalarNearlyZero(v0.dot(v0)));
    SkASSERT(!SkScalarNearlyZero(v1.dot(v1)));
...
// relative comparisons to handle large floating point numbers
...
struct Edge {
    ...
    bool above(const Edge& that, SkScalar tolerance = SK_ScalarNearlyZero) {
        SkASSERT(relative_lt(this->fSegment.fP0.fX, that.fSegment.fP0.fX) ||
                 relative_eq(this->fSegment.fP0.fX, that.fSegment.fP0.fX));
...
public:
    ...
    bool remove(const Edge& edge) {
        ...
        if (removeIndex < fEdges.count()-1 &&
            fEdges[removeIndex].intersect(fEdges[removeIndex+1])) {
            return false;
        }
...
static bool is_simple_polygon(const SkPoint* polygon, int polygonSize) {
    ...
    for (int i = 0; i < polygonSize; ++i) {
        ...
        if (!left(polygon[newVertex.fPrevIndex], polygon[i])) {
            newVertex.fFlags |= kPrevRight_VertexFlag;
        }
        if (!left(polygon[newVertex.fNextIndex], polygon[i])) {
            newVertex.fFlags |= kNextRight_VertexFlag;
        }
    ...
    while (vertexQueue.count() > 0) {
        ...
        if (v.fFlags & kPrevRight_VertexFlag) {
            Edge edge{ { v.fPosition, polygon[v.fPrevIndex] },
                         v.fIndex, v.fPrevIndex };
            if (!sweepLine.insert(edge)) {
                break;
            }
        } else {
            Edge edge{ { polygon[v.fPrevIndex], v.fPosition },
                        v.fPrevIndex, v.fIndex };
        ...
        if (v.fFlags & kNextRight_VertexFlag) {
            Edge edge{ { v.fPosition, polygon[v.fNextIndex] },
                         v.fIndex, v.fNextIndex };
            if (!sweepLine.insert(edge)) {
                break;
            }
        } else {
            Edge edge{ { polygon[v.fNextIndex], v.fPosition },
                         v.fNextIndex, v.fIndex };
            if (!sweepLine.remove(edge)) {
                break;
            }
        }
...
// Shouldn't a negative offset mean outset? (To be compatible with InsetConvex, above.)
bool SkOffsetSimplePolygon(const SkPoint* inputPolygonVerts, int inputPolygonSize,
    ...
    bool isSimple = is_simple_polygon(offsetPolygon->begin(), offsetPolygon->count());

    return (offsetPolygon->count() >= 3 && isSimple);
}
