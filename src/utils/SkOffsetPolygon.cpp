...
struct Edge {
    static bool Above(const Edge& e0, const Edge& e1) {  // true if e0 above e1
        ...
        SkVector v0 = e0.fSegment.fP1 - e0.fSegment.fP0;
        SkVector v1 = e1.fSegment.fP0 - e0.fSegment.fP0;
        if (v0.cross(v1) < 0) {
            return true;
        }
        v1 = e1.fSegment.fP1 - e0.fSegment.fP0;
...
bool SkOffsetPolygon(const SkPoint* inputPolygonVerts, int inputPolygonSize,
    ...
    while (prevIndex != currIndex) {
        ...
        if (compute_intersection(edgeData[prevIndex].fInset, edgeData[currIndex].fInset,
            ...
            // minimize points where lines cross to be closer to segments
