...
public:
    ...
    bool insert(const Edge& newEdge) {
        ...
        int insertIndex = 0;
        while (insertIndex < fEdges.count()-1 && fEdges[insertIndex].above(newEdge)) {
            ++insertIndex;
        }
        if (insertIndex > 0) {
            if (newEdge.intersect(fEdges[insertIndex - 1])) {
                return false;
            }
        }
        if (insertIndex < fEdges.count() - 1) {
            if (newEdge.intersect(fEdges[insertIndex + 1])) {
                return false;
            }
        }
...
bool SkOffsetSimplePolygon(const SkPoint* inputPolygonVerts, int inputPolygonSize,
    ...
    // build offset normals and compute area to determine winding
    ...
    return (offsetPolygon->count() >= 3 && is_simple_polygon(&(*offsetPolygon)[0],
                                                             offsetPolygon->count()));
}
