...
public:
    ...
    void insert(const SkPoint& p0, const SkPoint& p1, int index0, int index1) {
        Edge newEdge{ { p0, p1}, index0, index1 };
...
static bool simplify_polygon(SkTDArray<SkPoint>* offsetPolygon) {
    ...
    while (vertexQueue.count() > 0) {
        ...
        // if both edges are to the right, add to edge list
        if (kRightRightMask == v.fFlags) {
            sweepLine.insert(v.fPosition, unsortedVertices[v.fPrevIndex].fPosition,
                             qv.fUnsortedIndex, v.fPrevIndex);
            sweepLine.insert(v.fPosition, unsortedVertices[v.fNextIndex].fPosition,
                             qv.fUnsortedIndex, v.fNextIndex);
            // handle intersections



        // if both to the left, remove both from edge list
        } else if (kLeftLeftMask == v.fFlags) {


        // if one edge to the left and one to the right, replace in edge list
        } else {
            // handle intersections
...
bool SkOffsetPolygon(const SkPoint* inputPolygonVerts, int inputPolygonSize,
    ...
    return simplify_polygon(offsetPolygon);
}
