...
bool SkOffsetPolygon(const SkPoint* inputPolygonVerts, int inputPolygonSize,
    ...
    while (prevIndex != currIndex) {
        ...
        if (compute_intersection(edgeData[prevIndex].fInset, edgeData[currIndex].fInset,
            ...
            // if connected chains, dealing with a collapse
            if (edgeData[prevIndex].fInset.fP0 == edgeData[prevPrevIndex].fInset.fP1 &&
                edgeData[currNextIndex].fInset.fP0 == edgeData[currIndex].fInset.fP1) {
                // minimize points where lines cross to be closer
