    ...
    if (SkScalarNearlyEqual(d0, d1)) {
        ...
        *vector1 = perp;
        ...
        if (SkScalarNearlyZero(dPlenSq)) {
            ...
            vector0->fX = (d0sq*dP.fX - side * d0*dP.fY*discrim) / dPlenSq;
            vector0->fY = (d0sq*dP.fY + side * d0*dP.fX*discrim) / dPlenSq;
        }
        ...
        if (SkScalarNearlyZero(dPlenSq)) {
            ...
            SkScalar discrim = SkScalarSqrt(dPlenSq - d1sq);
            vector1->fX = (d1sq*dP.fX - side * d1*dP.fY*discrim) / dPlenSq;
            vector1->fY = (d1sq*dP.fY + side * d1*dP.fX*discrim) / dPlenSq;
        }
    }
