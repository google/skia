...
SkTypeface* SkOrderedFontMgr::onMatchFaceStyle(const SkTypeface* face,
                                               const SkFontStyle& style) const {
#if 0
    for (const auto& fm : fList) {
...
sk_sp<SkTypeface> SkOrderedFontMgr::onMakeFromData(sk_sp<SkData> data, int ttcIndex) const {
...
sk_sp<SkTypeface> SkOrderedFontMgr::onMakeFromStreamIndex(std::unique_ptr<SkStreamAsset> stream,
                                                          int ttcIndex) const {
    return fList.size() ? fList[0]->makeFromStream(std::move(stream), ttcIndex) : nullptr;
}

sk_sp<SkTypeface> SkOrderedFontMgr::onMakeFromStreamArgs(std::unique_ptr<SkStreamAsset> stream,
                                                         const SkFontArguments& args) const {
    return fList.size() ? fList[0]->makeFromStream(std::move(stream), args) : nullptr;
}

sk_sp<SkTypeface> SkOrderedFontMgr::onMakeFromFontData(std::unique_ptr<SkFontData> data) const {
...
sk_sp<SkTypeface> SkOrderedFontMgr::onLegacyMakeTypeface(const char family[],
