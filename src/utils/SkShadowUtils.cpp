...
void SkShadowUtils::DrawShadow(SkCanvas* canvas, const SkPath& path, SkScalar occluderHeight,
    ...
    if (viewMatrix.isSimilarity()) {
        if (path.isOval(&rect) && rect.width() == rect.height()) {
            SkPaint newPaint;
            newPaint.setColor(color);
            if (ambientAlpha > 0) {
                newPaint.setMaskFilter(SkAmbientShadowMaskFilter::Make(occluderHeight,
                                                                       ambientAlpha, flags));
...
void SkShadowUtils::DrawUncachedShadow(SkCanvas* canvas, const SkPath& path,
    ...
    if (viewMatrix.isSimilarity()) {
        ...
        if (path.isOval(&rect) && rect.width() == rect.height()) {
            ...
            newPaint.setColor(color);
            if (ambientAlpha > 0) {
                newPaint.setMaskFilter(SkAmbientShadowMaskFilter::Make(heightFunc(0, 0),
                                                                       ambientAlpha, flags));
                canvas->drawRect(rect, newPaint);
            }
            if (spotAlpha > 0) {
