...
void SkGpuDevice::drawShadow(const SkPath& path, const SkDrawShadowRec& rec) {
    ...
    const bool force_general = SkToBool(rec.fFlags & SkShadowFlags::kGeometricOnly_ShadowFlag);

    if (!force_general && ctm.rectStaysRect() && ctm.isSimilarity()) {
        ...
        if (rec.fSpotAlpha > 0) {
            spotPaint.setMaskFilter(SkSpotShadowMaskFilter::Make(occluderZ, rec.fLightPos,
