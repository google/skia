...
void draw_shadow(const FACTORY& factory, SkCanvas* canvas, ShadowedPath& path, SkColor color) {
    ...
    SkPaint paint;
    // Run the vertex color throught a GaussianColorFilter and then modulate the grayscale result of
...
void SkShadowUtils::DrawShadow(SkCanvas* canvas, const SkPath& path, SkScalar occluderHeight,
    ...
    if (spotAlpha > 0) {
        ...
        factory.fOffset = SkVector::Make(zRatio * (center.fX - lightXY.fX),
