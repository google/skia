...
void SkGpuDevice::drawShadow(const SkPath& path, const SkDrawShadowRec& rec) {
    if (rec.fFlags & SkShadowFlags::kGeometricOnly_ShadowFlag) {
    ...
    const uint32_t flags = rec.fFlags;
    const SkColor color = rec.fColor;
        ...
        if (path.isRect(&rect)) {
            ...
            if (rec.fAmbientAlpha > 0) {
                newPaint.setMaskFilter(SkAmbientShadowMaskFilter::Make(occluderZ,
                                                                       rec.fAmbientAlpha, flags));
                this->drawRect(rect, newPaint);
            }
            if (rec.fSpotAlpha > 0) {
                newPaint.setMaskFilter(SkSpotShadowMaskFilter::Make(occluderZ, rec.fLightPos,
    ...
    if (ambientAlpha > 0) {
        ...
        if (uncached) {
            sk_sp<SkVertices> vertices = SkShadowTessellator::MakeAmbient(path, viewMatrix,
                                                                          rec.fZPlaneParams,
                                                                          transparent);
            SkColor renderColor = compute_render_color(rec.fColor, ambientAlpha);
            SkPaint paint;
            ...
            factory.fOccluderHeight = rec.fZPlaneParams.fZ;
    ...
    if (spotAlpha > 0) {
        ...
        if (uncached) {
            sk_sp<SkVertices> vertices = SkShadowTessellator::MakeSpot(path, viewMatrix,
                                                                       ...
                                                                       rec.fLightRadius,
                                                                       transparent);
            SkColor renderColor = compute_render_color(rec.fColor, spotAlpha);
            SkPaint paint;
            // Run the vertex color through a GaussianColorFilter and then modulate the grayscale
            // result of that against our 'color' param.
            paint.setColorFilter(SkColorFilter::MakeComposeFilter(
            ...
            float zRatio = SkTPin(occluderHeight / (rec.fLightPos.fZ - occluderHeight), 0.0f, 0.95f);
            SkScalar radius = rec.fLightRadius * zRatio;

            // Compute the translation for the spot shadow.
            SkPoint center = SkPoint::Make(path.getBounds().centerX(), path.getBounds().centerY());
            viewMatrix.mapPoints(&center, 1);
            factory.fOffset = SkVector::Make(zRatio * (center.fX - rec.fLightPos.fX),
                                             zRatio * (center.fY - rec.fLightPos.fY));
            factory.fOccluderHeight = occluderHeight;
            factory.fDevLightPos = rec.fLightPos;
            factory.fLightRadius = rec.fLightRadius;
            SkRect devBounds;
