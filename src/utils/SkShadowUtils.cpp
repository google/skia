...
void SkShadowUtils::ComputeTonalColors(const SkColor4f& ambient4f, const SkColor4f& spot4f,
    ...
    // Ambient
    *ambientColor = SkColorSetARGB(ambient4f.fA*255.999f, 0, 0, 0);

    // Spot
    SkScalar max = SkTMax(SkTMax(spot4f.fR, spot4f.fG), spot4f.fB);
    SkScalar min = SkTMin(SkTMin(spot4f.fR, spot4f.fG), spot4f.fB);
    ...
    SkScalar alphaAdjust = (2.6f + (-2.66667f + 1.06667f*spot4f.fA)*spot4f.fA)*spot4f.fA;
    ...
    //     f(luminance, 0) = 0
    //     f(1, 0.25) = 0.15
    SkScalar greyscaleAlpha = SkTPin(spot4f.fA*(1 - 0.4f*luminance), 0.0f, 1.0f);
    ...
    *spotColor = SkColorSetARGB(tonalAlpha*255.999f,
                                spot4f.fR*unPremulScale*255.999f,
                                spot4f.fG*unPremulScale*255.999f,
                                spot4f.fB*unPremulScale*255.999f);
}
