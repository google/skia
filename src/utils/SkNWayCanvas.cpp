...
void SkNWayCanvas::onDrawImageSet(const SkCanvas::ImageSetEntry set[], int cnt, float alpha,
                                  SkFilterQuality filterQuality, SkBlendMode mode) {
    Iter iter(fList);
    while (iter.next()) {
        iter->experimental_DrawImageSetV0(set, cnt, alpha, filterQuality, mode);
    }
}
