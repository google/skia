...
void SkNWayCanvas::onDrawEdgeAAQuad(const SkRect& rect, const SkPoint clip[], int clipCount,
                                    QuadAAFlags aa, SkColor color, SkBlendMode mode) {
    Iter iter(fList);
    while (iter.next()) {
        iter->experimental_DrawEdgeAAQuad(rect, clip, clipCount, aa, color, mode);
    }
}
