...
bool SkTextLayoutJSON::writeSkShaper(SkJSONWriter* JSONWriter, ParagraphImpl* paragraph) {
    JSONWriter->beginArray("runs", true);
    for (auto& run : paragraph->runs()) {
        ...
        JSONWriter->endObject();
        JSONWriter->beginObject("font");
        JSONWriter->beginObject("metrics");
        JSONWriter->appendFloat("ascent", run.ascent());
        JSONWriter->appendFloat("descent", run.descent());
        JSONWriter->appendFloat("leading", run.leading());
        JSONWriter->endObject();
        JSONWriter->endObject();
        JSONWriter->beginObject("range");
