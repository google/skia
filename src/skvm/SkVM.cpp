...
namespace skvm {
    ...
    void Program::eval(int n, void* args[], size_t strides[], int nargs) const {
        ...
        std::vector<Reg> regs(fInstructions.size() + 1);
...
#if 0
        // Stopping one K-sized chunk early gives us a little more coverage on the scalar code.
        for ( ; n >= K; n -= K) {
#else
        for ( ; n >  K; n -= K) {
#endif
            for (int i = 0; i < (int)fInstructions.size(); i++) {
                ...
                ID   id = i+1;
                switch (op) {
                    case Op::store8: skvx::cast<uint8_t>(regs[y].i32).store(arg(x)); break;
                    case Op::store32:                   (regs[y].i32).store(arg(x)); break;

                    case Op::load8:  regs[id].i32 = skvx::cast<int>(U8 ::Load(arg(x))); break;
                    case Op::load32: regs[id].i32 =                (I32::Load(arg(x))); break;

                    // ~~ ops are handled verbatim same from here ~~ //
                    ...
                    case Op::shl: regs[id].i32 = regs[x].i32 << imm; break;
                    case Op::sra: regs[id].i32 = regs[x].i32 >> imm; break;
                    case Op::shr: regs[id].i32 = skvx::cast<int>(regs[x].u32 >> imm); break;

                    case Op::to_f32: regs[id].f32 = skvx::cast<float>(regs[x].i32); break;
                    case Op::to_i32: regs[id].i32 = skvx::cast<int>(regs[x].f32); break;
                }
            }
        ...
        while (n --> 0) {
            for (int i = 0; i < (int)fInstructions.size(); i++) {
                ...
                ID   id = i+1;
                switch (op) {
                    case Op::store8:  memcpy(arg(x), &regs[y].i32, 1); break;
                    case Op::store32: memcpy(arg(x), &regs[y].i32, 4); break;

                    case Op::load8:  regs[id].i32 = 0; memcpy(&regs[id].i32, arg(x), 1); break;
                    case Op::load32: regs[id].i32 = 0; memcpy(&regs[id].i32, arg(x), 4); break;
