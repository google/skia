...
namespace skvm {
    ...
    // Labels hold their own ID in x so that they're not deduplicated by the index.
    L Builder::label() { return {this->push(Op::label, this->valops.size()+1) }; }
    ...
    void eval(const ValOp* valops, int n, void** args) {
        // Since this is just a simple interpreter,
        // we'll store non-argument Uniforms and Variables
        // together/ in one big array, indexed directly by ID.
        union Slot {
            void*  ptr;
            int    i32;
            float  f32;
        };
        std::vector<Slot> slots(n+1);  // One extra slot because ID 0 is never used.
        ...
        for (int i = 0; i < n; i++) {
            ...
            switch (op) {
                ...
                case Op::jump:
                case Op::jump_if:
                case Op::jump_if_not: break;

                case Op::store8:  *(uint8_t*)uniform(x) = slots[y].i32; break;
                case Op::store32: *(int*    )uniform(x) = slots[y].i32; break;

                case Op::load8:  slots[id].i32 = *(uint8_t*)uniform(x); break;
                case Op::load32: slots[id].i32 = *(int*    )uniform(x); break;

                case Op::splat:  slots[id].i32 = imm; break;

                case Op::add_f32: slots[id].f32 = slots[x].f32 + slots[y].f32               ; break;
                case Op::sub_f32: slots[id].f32 = slots[x].f32 - slots[y].f32               ; break;
                case Op::mul_f32: slots[id].f32 = slots[x].f32 * slots[y].f32               ; break;
                case Op::div_f32: slots[id].f32 = slots[x].f32 / slots[y].f32               ; break;
                case Op::mad_f32: slots[id].f32 = slots[x].f32 * slots[y].f32 + slots[z].f32; break;

                case Op::add_i32: slots[id].i32 = slots[x].i32 + slots[y].i32; break;
                case Op::sub_i32: slots[id].i32 = slots[x].i32 - slots[y].i32; break;
                case Op::mul_i32: slots[id].i32 = slots[x].i32 * slots[y].i32; break;
                case Op::div_i32: slots[id].i32 = slots[x].i32 / slots[y].i32; break;

                case Op::bit_and: slots[id].i32 = slots[x].i32 & slots[y].i32; break;
                case Op::bit_or : slots[id].i32 = slots[x].i32 | slots[y].i32; break;
                case Op::bit_xor: slots[id].i32 = slots[x].i32 ^ slots[y].i32; break;

                case Op::shl: slots[id].i32 =           slots[x].i32 << imm; break;
                case Op::shr: slots[id].i32 = (uint32_t)slots[x].i32 >> imm; break;
                case Op::sra: slots[id].i32 =           slots[x].i32 >> imm; break;

                case Op::to_f32: slots[id].f32 = slots[x].i32; break;
                case Op::to_i32: slots[id].i32 = slots[x].f32; break;

                case Op::uniform: // TODO
                case Op::add1:
                case Op::sub1:
                case Op::mul1:
                case Op::div1:
