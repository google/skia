...
bool SkImageGeneratorCG::onGetPixels(const SkImageInfo& info, void* pixels, size_t rowBytes,
    ...
    SkPixmapPriv::Orienter orienter(info, pixels, rowBytes, fOrigin);
    // FIXME: Using SkCopyPixelsFromCGImage (as opposed to swizzling ourselves) greatly
    ...
    const SkPixmap* pm = orienter.preOrientDst();
    return pm && SkCopyPixelsFromCGImage(*pm, image) && orienter.orientIfNecessary();
}
