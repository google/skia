...
protected:
    ...
    /** Creates a dictionary suitable for setting the axes on a CTFontDescriptor, for the NSCTFontVariationAttribute key. */
    ...
    sk_sp<SkTypeface> onMakeFromStreamArgs(std::unique_ptr<SkStreamAsset> s,
        ...
        SkUniqueCFRef<CTFontDescriptorRef> dataDescriptor(CTFontManagerCreateFontDescriptorFromData(fontData.get()));

        SkASSERT(dataDescriptor);

        SkUniqueCFRef<CTFontRef> ctBase(CTFontCreateWithFontDescriptor(dataDescriptor.get(), 0, nullptr));
        ...
        SkUniqueCFRef<CFMutableDictionaryRef> axisAttributeDict(CFDictionaryCreateMutable(kCFAllocatorDefault, 1,
                                                                                          &kCFTypeDictionaryKeyCallBacks,
                                                                                          &kCFTypeDictionaryValueCallBacks));
        CFDictionaryAddValue(axisAttributeDict.get(), kCTFontVariationAttribute, axisDict.get());
        SkUniqueCFRef<CTFontDescriptorRef> axisDescriptor(CTFontDescriptorCreateWithAttributes(axisAttributeDict.get()));

        SkUniqueCFRef<CTFontRef> ctVariation(CTFontCreateCopyWithAttributes(ctBase.get(), 0, nullptr, axisDescriptor.get()));
