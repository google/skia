...
static constexpr const uint8_t ksRGB2014_icc[] = {
...
static bool supports_LCD() {
    static bool gSupportsLCD = []{
        ...
        constexpr CGFloat range[3][2] = { {0.f, 1.f}, {0.f, 1.f}, {0.f, 1.f}};
        UniqueCFRef<CGDataProviderRef> colorData(
                CGDataProviderCreateWithData(nullptr, ksRGB2014_icc,
                                             SK_ARRAY_COUNT(ksRGB2014_icc), nullptr));
        UniqueCFRef<CGColorSpaceRef> colorspace(
                CGColorSpaceCreateICCBased(3, (CGFloat*)range, colorData.get(), nullptr));
        UniqueCFRef<CGContextRef> cgContext(
                CGBitmapContextCreate(&bitmap, 16, 16, 8, 16*4, colorspace.get(), BITMAP_INFO_RGB));

        UniqueCFRef<CGDataProviderRef> fontData(
                CGDataProviderCreateWithData(nullptr, kSpiderSymbol_ttf,
                                             SK_ARRAY_COUNT(kSpiderSymbol_ttf), nullptr));
        UniqueCFRef<CGFontRef> cgFont(CGFontCreateWithDataProvider(fontData.get()));
        SkASSERT(cgFont);
...
CGRGBPixel* Offscreen::getCG(const SkScalerContext_Mac& context, const SkGlyph& glyph,
    ...
    if (!fRGBSpace) {
        // For outline glyphs it doesn't appear to matter what color space is specified.
        // Regular blends and antialiased text are always (s*a + d*(1-a))
        // and smoothed text is always g=2.0.
        // Color emoji are normally pngs and go through color space conversion.
        constexpr CGFloat range[6][2] = { {0.f, 1.f}, {0.f, 1.f}, {0.f, 1.f} };
        UniqueCFRef<CGDataProviderRef> colorData(
                CGDataProviderCreateWithData(nullptr, ksRGB2014_icc,
                                             SK_ARRAY_COUNT(ksRGB2014_icc), nullptr));
        fRGBSpace.reset(CGColorSpaceCreateICCBased(3, (CGFloat*)range, colorData.get(), nullptr));
    }
