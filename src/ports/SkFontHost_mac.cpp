                                                         ...
                                                         OpszVariation opsz)
{
    ...
    if (opsz.isSet) {
        ...
        if (!opsz ||
            ...
            !CFNumberGetValue(static_cast<CFNumberRef>(opsz.get()),kCFNumberDoubleType,&opsz_value))
        {
            opsz_value = CTFontGetSize(baseFont);
        }
        add_opsz_attr(attr.get(), opsz_value);
    }
    add_notrak_attr(attr.get());

    // On 10.14 and previous a 'SystemUI' CTFont doesn't respect attr
    // unless there is at least one 'matchable' attr.
    SkUniqueCFRef<CFDictionaryRef> empty(CFDictionaryCreate(kCFAllocatorDefault, nullptr, nullptr, 0, nullptr, nullptr));
    CFDictionarySetValue(attr.get(), kCTFontVariationAttribute, empty.get());
