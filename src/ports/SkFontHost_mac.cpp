...
static sk_sp<SkTypeface> create_from_desc(CTFontDescriptorRef desc) {
    ...
    return create_from_CTFontRef(std::move(ctFont), nullptr, {false, 0}, nullptr);
}
                                                         ...
                                                         OpszVariation opsz)
{
    ...
    // Here we need to know if the opsz axis has been explicitly set or not. If it has, set
    // kCTFontOpticalSizeAttribute to the opsz value we want. If not explicitly set, set
    // kCTFontOpticalSizeAttribute to the default opsz value (this could be the requested text size
    // from the SkFont, however).
...

{
    ...
    fCTFont = ctfont_create_exact_copy(ctFont, textSize, typeface->fOpszVariation);
...
static sk_sp<SkTypeface> create_from_dataProvider(SkUniqueCFRef<CGDataProviderRef> provider,
    ...
    return create_from_CTFontRef(std::move(ct), nullptr, {false, 0}, std::move(providedData));
}
...
protected:
    ...
    SkTypeface* onMatchFamilyStyleCharacter(const char familyName[],
        ...
        return create_from_CTFontRef(std::move(fallbackFont), nullptr, {false, 0}, nullptr).release();
    }
    ...
    sk_sp<SkTypeface> onMakeFromFontData(std::unique_ptr<SkFontData> fontData) const override {
        ...
        return create_from_CTFontRef(std::move(ct), std::move(cg), cgVariations.opsz, fontData->detachStream());
    }
