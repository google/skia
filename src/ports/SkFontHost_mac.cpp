...
protected:
    ...
    sk_sp<SkTypeface> onMakeFromData(sk_sp<SkData> data, int ttcIndex) const override {
        ...
        SkUniqueCFRef<CTFontRef> ct = make_ctfont_from_skdata(data, ttcIndex);
        if (!ct) {
            return nullptr;
        }
    ...
    sk_sp<SkTypeface> onMakeFromStreamIndex(std::unique_ptr<SkStreamAsset> stream,
        ...
        sk_sp<SkData> data = make_skdata_from_skstreamasset(stream->duplicate());
        if (!data) {
            return nullptr;
        }
        SkUniqueCFRef<CTFontRef> ct = make_ctfont_from_skdata(std::move(data), ttcIndex);
        if (!ct) {
            return nullptr;
        }
    ...
    static CGFontVariations ctvariation_from_skfontarguments(CTFontRef ct, const SkFontArguments& args) {
        OpszVariation opsz;
    ...
    static sk_sp<SkData> make_skdata_from_skstreamasset(std::unique_ptr<SkStreamAsset> s) {
        size_t size = s->getLength();
        if (const void* base = s->getMemoryBase()) {
            return SkData::MakeWithProc(base, size,
                                        [](const void*, void* ctx) -> void {
                                            delete (SkStreamAsset*)ctx;
                                        }, s.release());
        }
        return SkData::MakeFromStream(s.get(), size);
    }
    static SkUniqueCFRef<CFDataRef> make_cfdata_from_skdata(sk_sp<SkData> data) {
    ...
    static SkUniqueCFRef<CTFontRef> make_ctfont_from_skdata(sk_sp<SkData> data, int ttcIndex) {
        // TODO: Use CTFontManagerCreateFontDescriptorsFromData when available.
        if (ttcIndex != 0) {
            return nullptr;
        }

        SkUniqueCFRef<CFDataRef> cfData(make_cfdata_from_skdata(std::move(data)));
    ...
    sk_sp<SkTypeface> onMakeFromStreamArgs(std::unique_ptr<SkStreamAsset> s,
        ...
        sk_sp<SkData> data = make_skdata_from_skstreamasset(s->duplicate());
        if (!data) {
            return nullptr;
        }
        SkUniqueCFRef<CTFontRef> ct = make_ctfont_from_skdata(std::move(data), args.getCollectionIndex());
        if (!ct) {
            return nullptr;
        }

        CGFontVariations ctVariation = ctvariation_from_skfontarguments(ct.get(), args);
        ...
        return create_from_CTFontRef(std::move(ctVariant), nullptr, ctVariation.opsz, std::move(s));
    }

    /** Creates a dictionary suitable for setting the axes on a CTFont. */
    static CGFontVariations make_ctvariation_from_skfontdata(CTFontRef ct, SkFontData* fontData) {
    ...
    sk_sp<SkTypeface> onMakeFromFontData(std::unique_ptr<SkFontData> fontData) const override {
        ...
        sk_sp<SkData> data = make_skdata_from_skstreamasset(fontData->getStream()->duplicate());
        if (!data) {
            return nullptr;
        }
        SkUniqueCFRef<CTFontRef> ct = make_ctfont_from_skdata(std::move(data), fontData->getIndex());
        if (!ct) {
            return nullptr;
        }

        CGFontVariations ctVariation = make_ctvariation_from_skfontdata(ct.get(), fontData.get());
