...
sk_sp<SkTypeface> SkFontMgr_Fuchsia::FetchTypeface(const char familyName[],
    ...
    for (int i = 0; i < bcp47Count; i++) {
        languages.push_back(fuchsia::intl::LocaleId{.id = bcp47[i]});
    }

    fuchsia::fonts::TypefaceRequest request;
    request.set_query(std::move(fuchsia::fonts::TypefaceQuery()
                                        .set_style(SkToFuchsiaStyle(style))
                                        .set_languages(languages)
                                        .set_code_points({static_cast<uint32_t>(character)})));
    ...
    auto genericFontFamily = GetGenericFontFamilyByName(familyName);
    if (!familyName || *familyName == '\0' || genericFontFamily) {
        if (genericFontFamily) {
            request.mutable_query()->set_fallback_family(genericFontFamily.value());
        }
        allow_fallback = true;
    } else {
        request.mutable_query()->set_family(fuchsia::fonts::FamilyName{.name = familyName});
    }
