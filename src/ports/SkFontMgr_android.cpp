...
private:
    ...
    void addFamily(FontFamily& family, const bool isolated, int i) {
    ...
    void buildNameToFamilyMap(SkTDArray<FontFamily*> families, const bool isolated) {
        ...
        for (FontFamily* family : families) {
            addFamily(*family, isolated, i++);
            family->fallbackFamilies.foreach([this, isolated, &i](SkString, std::unique_ptr<FontFamily>* fallbackFamily) {
                addFamily(*(*fallbackFamily).get(), isolated, i++);
            });
        }
    }
