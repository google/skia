...
SkScalerContext_DW::SkScalerContext_DW(sk_sp<DWriteFontTypeface> typeface,
    ...
    this->getDWriteTypeface()->fFactory->QueryInterface<IDWriteFactory2>(&fFactory2);

    SkTScopedComPtr<IDWriteFontFace2> fontFace2;
    this->getDWriteTypeface()->fDWriteFontFace->QueryInterface<IDWriteFontFace2>(&fontFace2);
...
void SkScalerContext_DW::generatePath(const SkGlyph& glyph, SkPath* path) {
    ...
    uint16_t glyphId = glyph.getGlyphID();
    {
        ...
        float textSize = SkScalarToFloat(fTextSizeRender);
        HRVM(this->getDWriteTypeface()->fDWriteFontFace->GetGlyphRunOutline(textSize),
             &glyphId,
             nullptr, //advances
             nullptr, //offsets
             1, //num glyphs
