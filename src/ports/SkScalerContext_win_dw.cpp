 ...
 * The mutex returned from GetDWriteFactoryMutex() protects the calls that are
 * problematic.
 */
static SkSharedMutex& GetDWriteFactoryMutex() {
...
static bool is_hinted(DWriteFontTypeface* typeface) {
    Exclusive l(GetDWriteFactoryMutex());
...
bool SkScalerContext_DW::generateAdvance(SkGlyph* glyph) {
    ...
    {
        Shared l(GetDWriteFactoryMutex());
