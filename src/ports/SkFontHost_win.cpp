        ...
        while (cur_poly < end_poly) {
            ...
            if (pc->wType == TT_PRIM_LINE) {
                for (uint16_t i = 0; i < cpfx; i++) {
                    ...
                    if (this->currentIsNot(pnt_b)) {
                        this->goingTo(pnt_b);
                        fPath->lineTo( SkFixedToScalar(SkFIXEDToFixed(pnt_b.x)),
            ...
            if (pc->wType == TT_PRIM_QSPLINE) {
                for (uint16_t u = 0; u < cpfx - 1; u++) { // Walk through points in spline
                    ...
                    if (this->currentIsNot(pnt_b) || this->currentIsNot(pnt_c)) {
                        this->goingTo(pnt_c);
                        fPath->quadTo( SkFixedToScalar(SkFIXEDToFixed(pnt_b.x)),
                                      -SkFixedToScalar(SkFIXEDToFixed(pnt_b.y)),
            ...
            if (pc->wType == TT_PRIM_CSPLINE) {
                for (uint16_t w = 0; w < cpfx - 2; w += 2) {
                    if (this->currentIsNot(apfx[w + 0]) ||
                        this->currentIsNot(apfx[w + 1]) ||
                        this->currentIsNot(apfx[w + 2]))
                    {
                        this->goingTo(apfx[w + 2]);
                        fPath->cubicTo( SkFixedToScalar(SkFIXEDToFixed(apfx[w + 0].x)),
...
bool SkScalerContext_GDI::generatePath(SkGlyphID glyph, SkPath* path) {
    ...
    if (fRec.getHinting() != SkFontHinting::kSlight) {
        ...
        if (!sink.process(glyphbuf, total_size,
