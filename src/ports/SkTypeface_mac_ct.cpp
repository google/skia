...
sk_sp<SkTypeface> SkTypeface_Mac::onMakeClone(const SkFontArguments& args) const {
    ...
    if (ctVariation.variation) {
        ...
        if (ctVariation.wrongOpszVariation) {
            // On macOS 16 setting the variation on a system font with an opsz axis and not
            // changing the value of the opsz axis (either by setting it to the same value
            // or not specifying it at all) causes the entire specified variation to be ignored.
            // Work around this by setting the opsz to something which isn't the desired value
            // before setting the entire desired variation.
