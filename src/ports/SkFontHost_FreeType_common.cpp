...
inline SkMatrix ToSkMatrix(FT_Affine23 affine23) {
    ...
    printf("FT_Affine23: xx %f xy %f yx %f yy %f dx %f dy %f\n", SkFixedToScalar(affine23.xx),
...
void colrv1_draw_paint(SkCanvas* canvas,
    ...
    switch (colrv1_paint.format) {
        case FT_COLR_PAINTFORMAT_GLYPH: {
            ...
            SkPath path;
            /* Path maker is configured to load unscaled, but mangles return values through SkFDot6ToScalar, we need to multiply pathes by 64. */
            if (path_maker(face, glyphID, &path)) {
              SkRect path_bounds = path.getBounds();
              printf("PaintGlyph: Path bounds: x %f, y %f, w %f, h %f\n", path_bounds.x(), path_bounds.y(),
                     path_bounds.width(), path_bounds.height());
              SkMatrix scale_fdot6;
              scale_fdot6.setScaleX(64);
              scale_fdot6.setScaleY(64);
              path.transform(scale_fdot6);
        ...
        case FT_COLR_PAINTFORMAT_TRANSFORMED: {
            ...
            printf("PaintTransformed: SkMatrix: scaleX %f skewY %f skewX %f scaleY %f translateX %f translateY %f\n",
                   transform.getScaleX(),
                   transform.getSkewY(),
                   transform.getSkewX(),
                   transform.getScaleY(),
        ...
        case FT_GLYPH_FORMAT_OUTLINE: {
            ...
            if (SkMask::kARGB32_Format == glyph.fMaskFormat) {
                ...
                auto path_maker = [this](FT_Face face, FT_UInt glyphID, SkPath* path) {
                    return this->generateFacePath(face, glyphID, path, true /* load unscaled */);
                };
...
bool SkScalerContext_FreeType_Base::generateFacePath(FT_Face face, SkGlyphID glyphID, SkPath* path, bool load_unscaled) {
