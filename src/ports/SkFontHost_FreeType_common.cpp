    ...
    switch ( face->glyph->format ) {
        case FT_GLYPH_FORMAT_OUTLINE: {
            ...
            if (SkMask::kLCD16_Format == glyph.fMaskFormat) {
                ...
                // The following assumes that the SkMask and FT_Bitmap are not disjoint.
                SkASSERT(mask.fBounds.intersect( ftGlyph.bitmap_left,
                                                -ftGlyph.bitmap_top,
                                                 ftGlyph.bitmap_left + ftGlyph.bitmap.width,
                                                -ftGlyph.bitmap_top + ftGlyph.bitmap.rows));
                ...
                // In b3500af71 (released in VER-2-5-4) FreeType changed rows and width to unsigned.
                static_assert(std::is_same<decltype(ftGlyph.bitmap.rows),
                                           decltype(ftGlyph.bitmap.width)>::value, "");
                using ExtentType = decltype(ftGlyph.bitmap.rows);
                if (SkTo<ExtentType>(mask.fBounds.height() * ftVertScale) < ftGlyph.bitmap.rows) {
                    ftGlyph.bitmap.rows = mask.fBounds.height() * ftVertScale;
                }
                if (SkTo<ExtentType>(mask.fBounds.width() * ftHoriScale) < ftGlyph.bitmap.width) {
                    ftGlyph.bitmap.width = mask.fBounds.width() * ftHoriScale;
                }
                // If the SkMask is bigger, shrink mask to fit bitmap (clearing anything discarded)
                if (ftGlyph.bitmap.rows < SkTo<ExtentType>(mask.fBounds.height() * ftVertScale)) {
                    mask.fBounds.fBottom = mask.fBounds.fTop + ftGlyph.bitmap.rows / ftVertScale;
                }
                if (ftGlyph.bitmap.width < SkTo<ExtentType>(mask.fBounds.width() * ftHoriScale)) {
