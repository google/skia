...
inline SkMatrix ToSkMatrix(FT_Affine23 affine23) {
    ...
    // Adjust order to convert from FreeType's FT_Affine23 column major order to SkMatrix row-major order.
    return return_matrix.setAll(SkFixedToScalar(affine23.xx), SkFixedToScalar(affine23.yx),
                                SkFixedToScalar(affine23.dx), SkFixedToScalar(affine23.xy),
...
void colrv1_draw_paint(SkCanvas* canvas,
    ...
    switch (colrv1_paint.format) {
        ...
        case COLR_PAINTFORMAT_TRANSFORMED: {
            ...
            transform.preConcat(canvas_to_upem_transform).postConcat(upem_to_canvas_transform);
...
bool colrv1_iterate_layers(SkCanvas* canvas,
                           const SkVector& scale,
                           const FT_Color* palette,
                           FT_Face ft_face,
                           PathMaker path_maker,
...
bool colrv1_traverse_paint(SkCanvas* canvas,
    ...
    switch (paint.format) {
        ...
        case COLR_PAINTFORMAT_COLR_GLYPH:
            traverse_result = colrv1_iterate_layers(canvas, scale, palette, face, path_maker,
...
bool colrv1_iterate_layers(SkCanvas* canvas,
    ...
    while (FT_Get_Color_Glyph_Layer_Gradients(ft_face, glyph_id, &opaque_paint, &layer_iterator)) {
    ...
    switch ( face->glyph->format ) {
        case FT_GLYPH_FORMAT_OUTLINE: {
            ...
            if (SkMask::kARGB32_Format == glyph.fMaskFormat) {
                ...
                haveLayers = colrv1_iterate_layers(&canvas, scale, palette, face, path_maker,
                                                   glyph.getGlyphID());
