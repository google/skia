...
void colrv1_draw_paint(SkCanvas* canvas,
    ...
    switch (colrv1_paint.format) {
        ...
        case FT_COLR_PAINTFORMAT_LINEAR_GRADIENT: {
            ...
            line_positions[0].fX = SkFDot6ToScalar(colrv1_paint.u.linear_gradient.p0.x);
            line_positions[0].fY = -SkFDot6ToScalar(colrv1_paint.u.linear_gradient.p0.y);
            line_positions[1].fX = SkFDot6ToScalar(colrv1_paint.u.linear_gradient.p1.x);
            line_positions[1].fY = -SkFDot6ToScalar(colrv1_paint.u.linear_gradient.p1.y);

            SkPoint& p0 = line_positions[0];
            SkPoint& p1 = line_positions[1];
            SkPoint p2;
            p2.set(SkFDot6ToScalar(colrv1_paint.u.linear_gradient.p2.x),
                   -SkFDot6ToScalar(colrv1_paint.u.linear_gradient.p2.y));
            ...
            // Configure gradient transform from FreeType
            SkMatrix gradient_transform = ToSkMatrix(colrv1_paint.u.linear_gradient.gradient_transform);

            sk_sp<SkShader> shader(SkGradientShader::MakeLinear(
                    line_positions, colors, stops, num_color_stops,
                    ToSkTileMode(colrv1_paint.u.linear_gradient.colorline.extend), 0,
                    &gradient_transform));
        ...
        case FT_COLR_PAINTFORMAT_RADIAL_GRADIENT: {
            SkPoint start =
                    SkPoint::Make(SkFDot6ToScalar(colrv1_paint.u.radial_gradient.c0.x),
                                  -SkFDot6ToScalar(colrv1_paint.u.radial_gradient.c0.y));
            /* TODO: Add matrix to FreeType output and incorporate that here. */
            SkScalar radius = SkFDot6ToScalar(colrv1_paint.u.radial_gradient.r0);
            SkPoint end = SkPoint::Make(SkFDot6ToScalar(colrv1_paint.u.radial_gradient.c1.x),
                                        -SkFDot6ToScalar(colrv1_paint.u.radial_gradient.c1.y));
            SkScalar end_radius = SkFDot6ToScalar(colrv1_paint.u.radial_gradient.r1);
            ...
            paint.setShader(SkGradientShader::MakeTwoPointConical(
                    ...
                    ToSkTileMode(colrv1_paint.u.radial_gradient.colorline.extend), 0,
                    &gradient_transform));
    ...
    switch (paint.format) {
        case FT_COLR_PAINTFORMAT_COLR_LAYERS: {
            ...
            while (FT_Get_Paint_Layers(face, &layer_iterator, &opaque_paint_fetch)) {
                colrv1_traverse_paint(canvas, palette, face, path_maker, opaque_paint_fetch);
            }
            break;
        }
        case FT_COLR_PAINTFORMAT_GLYPH:
            ...
            colrv1_draw_paint(canvas, palette, face, path_maker, paint);
            traverse_result = colrv1_traverse_paint(canvas, palette, face, path_maker,
                                                    paint.u.glyph.paint);
            canvas->restore();
            break;
        case FT_COLR_PAINTFORMAT_COLR_GLYPH:
            traverse_result = colrv1_start_glyph(canvas, palette, face, path_maker,
      ...
      case FT_COLR_PAINTFORMAT_ROTATE:
        ...
        case FT_COLR_PAINTFORMAT_COMPOSITE: {
            ...
            traverse_result = traverse_result &&
                              colrv1_traverse_paint(canvas, palette, face, path_maker,
...
bool colrv1_start_glyph(SkCanvas* canvas,
    ...
    if (FT_Get_Color_Glyph_Paint(ft_face, glyph_id, &opaque_paint)) {
        ...
        colrv1_traverse_paint(canvas, palette, ft_face, path_maker, opaque_paint);
    }
    ...
    switch ( face->glyph->format ) {
        case FT_GLYPH_FORMAT_OUTLINE: {
            ...
            if (SkMask::kARGB32_Format == glyph.fMaskFormat) {
                ...
                // TODO: Make the COLRv1 drawing functions members so that they can access generateFacePath?
                auto path_maker = [this](FT_Face face, FT_UInt glyphID, SkPath* path) {
                    return this->generateFacePath(face, glyphID, path);
                };
                haveLayers = colrv1_start_glyph(&canvas, palette, face, path_maker,
                                                glyph.getGlyphID());
#else
                haveLayers = false;
#endif
