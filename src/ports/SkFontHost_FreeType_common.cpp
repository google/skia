    ...
    switch ( face->glyph->format ) {
        case FT_GLYPH_FORMAT_OUTLINE: {
            ...
            if (SkMask::kLCD16_Format == glyph.fMaskFormat) {
                ...
                if (SkToUInt(mask.fBounds.height() * ftVertScale) < ftGlyph.bitmap.rows) {
                    ftGlyph.bitmap.rows = mask.fBounds.height() * ftVertScale;
                }
                if (SkToUInt(mask.fBounds.width() * ftHoriScale) < ftGlyph.bitmap.width) {
                    ftGlyph.bitmap.width = mask.fBounds.width() * ftHoriScale;
                }
                // If the SkMask is bigger, shrink mask to fit bitmap (clearing anything discarded)
                if (ftGlyph.bitmap.rows < SkToUInt(mask.fBounds.height() * ftVertScale)) {
                    mask.fBounds.fBottom = mask.fBounds.fTop + ftGlyph.bitmap.rows / ftVertScale;
                }
                if (ftGlyph.bitmap.width < SkToUInt(mask.fBounds.width() * ftHoriScale)) {
