...
void colrv1_draw_paint(SkCanvas* canvas,
    ...
    switch (colrv1_paint.format) {
        ...
        case FT_COLR_PAINTFORMAT_SWEEP_GRADIENT: {
            ...
            SkScalar startAngle = SkFixedToScalar(colrv1_paint.u.sweep_gradient.startAngle);
            SkScalar endAngle = SkFixedToScalar(colrv1_paint.u.sweep_gradient.endAngle);

            /* TODO: Perform modulo/remainder computation when values are outside the [0, 360) range. */
            ...
            // Skia's angles start from the horizontal x-Axis, rotate left.
            SkMatrix rotate_left_90deg = SkMatrix::RotateDeg(-90.f, center);
            paint.setShader(SkGradientShader::MakeSweep(
                    center.x(), center.y(), colors.data(), stops.data(), num_color_stops,
                    SkTileMode::kDecal, startAngle, endAngle, 0, &rotate_left_90deg));
            canvas->drawPaint(paint);
            break;
        }
