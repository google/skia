    ...
    switch ( face->glyph->format ) {
        case FT_GLYPH_FORMAT_OUTLINE: {
...
#if SK_FREETYPE_MINIMUM_RUNTIME_VERSION > 0x02090100
            ...
            if (SkMask::kARGB32_Format == glyph.fMaskFormat) {
                ...
                if (err) {
                    SK_TRACEFTR(err, "Could not get palette from given fontFace.", palette);
                    return;
                }
                ...
                while (FT_Get_Color_Glyph_Layer(face, glyph.getGlyphID(), &layer_glyph_index,
                                                &layer_color_index,
                                                &layer_iterator)) {
                    have_layers = true;
                    if (layer_color_index == 0xFFFF) {
                        ...
                        SkColor color = SkColorSetARGB(palette[layer_color_index].alpha,
                                                       palette[layer_color_index].red,
                                                       palette[layer_color_index].green,
                                                       palette[layer_color_index].blue);
                        paint.setColor(color);
                    }
                    SkPath path;
                    if (this->generateFacePath(face, layer_glyph_index, &path)) {
                        canvas.drawPath(path, paint);
                    }
                }

                if (!have_layers) {
                    SK_TRACEFTR(err, "Could not get layers from given fontFace.", layer_iterator);
                    return;
                }
            }
