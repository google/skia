                                                               ...
                                                               SkUnichar character) const
{
    ...
    sk_sp<SkTypeface> typeface = this->fallback(dwFamilyName, dwStyle, dwBcp47->get(), character);
    if (typeface) {
...
sk_sp<SkTypeface> SkFontMgr_DirectWrite::onLegacyMakeTypeface(const char familyName[],
    ...
    if (familyName) {
        ...
        if (SUCCEEDED(sk_cstring_to_wchar(familyName, &dwFamilyName))) {
            ...
            if (!fontFamily) {
                sk_sp<SkTypeface> typeface = this->fallback(dwFamilyName, dwStyle,
                                                            fLocaleName.get(), 32);
                if (typeface) {
                    return typeface;
                }
    ...
    if (!fontFamily) {
        // If we have a fallback, find the fallback for the space character.
        sk_sp<SkTypeface> typeface = this->fallback(nullptr, dwStyle, fLocaleName.get(), 32);
        if (typeface) {
            return typeface;
        }
        // Otherwise, try the message font, because we must be on Win 7 or 8.0.
