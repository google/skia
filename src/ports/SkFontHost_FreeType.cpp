    ...
    , fStrikeIndex(-1)
{
    ...
    if (FT_IS_SCALABLE(fFaceRec->fFace)) {
        ...
        SkScalar x_ppem = upem * SkFT_FixedToScalar(fFaceRec->fFace->size->metrics.x_scale) / 64.0f;
        SkScalar y_ppem = upem * SkFT_FixedToScalar(fFaceRec->fFace->size->metrics.y_scale) / 64.0f;
