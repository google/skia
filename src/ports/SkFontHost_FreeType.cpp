...
std::unique_ptr<SkFontData> SkTypeface_FreeType::onMakeCloneFontData(
    ...
    return skstd::make_unique<SkFontData>(
        std::unique_ptr<SkStreamAsset>(this->openStream(&ttcIndex)),
        ttcIndex,
        axisValues.get(),
    ...
    SkString* name, SkFontStyle* style, bool* isFixedPitch, AxisDefinitions* axes) const
{
    ...
    bool success = this->GetAxes(face, axes);
    FT_Done_Face(face);
    return success;
}
