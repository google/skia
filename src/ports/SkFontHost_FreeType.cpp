...
public:
    SkFTVersion(FT_Library library) {
        FT_Library_Version(library, &major, &minor, &patch);
    }
    constexpr SkFTVersion(FT_Int x, FT_Int y, FT_Int z) : major(x), minor(y), patch(z) {}
    constexpr bool operator<(const SkFTVersion& that) const {
        return this->major <  that.major ||
              (this->major == that.major && this->minor <  that.minor) ||
              (this->major == that.major && this->minor == that.minor && this->patch < that.patch);
    }
    constexpr bool operator>=(const SkFTVersion& that) const { return !(*this <  that); }
    constexpr bool operator> (const SkFTVersion& that) const { return  ( that < *this); }
    constexpr bool operator<=(const SkFTVersion& that) const { return !( that < *this); }
};
...
bool SkTypeface_FreeType::Scanner::canMake(SkFontFormat format) const {
    if (fLibrary == nullptr) {
        return false;
    }

    /** This is a very small font which is...
     *  opentype (head, hhea, maxp, hmtx, cmap, loca, glyf, name)
     *  color bitmap (CDBT, CBLC) with a png at size 16
     *  variable (fvar, gvar)
     *  color outlines (COLR, CPAL)
     *
     *  To re-forge the original bytes of the TrueType font file,
     *  remove all ',|( +0x)' from this definition,
     *  copy the data to the clipboard,
     *  run 'xclip -o | xxd -p -r - ftdetect.ttf'.
     */
    static constexpr const uint8_t gxCbdtColrFontData[] = {
        
    };

    bool hasTT = SkToBool(FT_Get_Module(fLibrary, "truetype"));
    bool hasCFF = SkToBool(FT_Get_Module(fLibrary, "cff"));
    bool hasT1 = SkToBool(FT_Get_Module(fLibrary, "type1"));
    SkFTVersion version(fLibrary);

    bool hasMM = false; // TT_CONFIG_OPTION_GX_VAR_SUPPORT
    bool hasFixedSizes = false; // TT_CONFIG_OPTION_EMBEDDED_BITMAPS
    bool hasPngSupport = false; // FT_CONFIG_OPTION_USE_PNG
    bool hasCOLRSupport = false; // TT_CONFIG_OPTION_COLOR_LAYERS
    if (hasTT) {
        FT_Open_Args args;
        memset(&args, 0, sizeof(args));
        args.flags = FT_OPEN_MEMORY;
        args.memory_base = (const FT_Byte*)gxCbdtColrFontData;
        args.memory_size = sizeof(gxCbdtColrFontData);
        FT_Face rawFace;
        if (0 == FT_Open_Face(fLibrary, &args, 0, &rawFace)) {
            std::unique_ptr<FT_FaceRec, SkFunctionWrapper<FT_Error, FT_FaceRec, FT_Done_Face>> face(rawFace);
            hasMM = FT_HAS_MULTIPLE_MASTERS(face);
            hasFixedSizes = FT_HAS_FIXED_SIZES(face);
            if (hasFixedSizes) {
                if (0 == FT_Select_Size(face.get(), 0)) {
                    if (0 == FT_Load_Glyph(face.get(), 0, 0)) {
                        hasPngSupport = true;
                    }
                }
            }
...
#ifdef FT_COLOR_H
            FT_Color *palette;
            if (0 == FT_Palette_Select(face.get(), 0, &palette)) {
                hasCOLRSupport = true;
            }
#endif
        }
    ...
    switch (format) {
        case SkFontFormat::TT:
        case SkFontFormat::TT_Collection:
        case SkFontFormat::TT_Collection_Index:
        case SkFontFormat::TT_glyf:
            return hasTT;
        case SkFontFormat::TT_glyf_COLR:
            return hasTT && hasCOLRSupport && version >= SkFTVersion(2, 10, 0);
        case SkFontFormat::TT_glyf_Variable:
            return hasTT && hasMM;
        case SkFontFormat::TT_glyf_Variable_Index:
            return hasTT && hasMM && version >= SkFTVersion(2, 6, 1);
        case SkFontFormat::TT_CBDT:
        case SkFontFormat::TT_CBDT_Raw:
            return hasTT && hasFixedSizes && version >= SkFTVersion(2, 5, 0);
        case SkFontFormat::TT_CBDT_Png:
            return hasTT && hasFixedSizes && hasPngSupport && version >= SkFTVersion(2, 5, 0);
        case SkFontFormat::TT_sbix:
            return hasTT && hasFixedSizes && version >= SkFTVersion(2, 5, 1);
        case SkFontFormat::TT_sbix_Png:
            return hasTT && hasFixedSizes && hasPngSupport && version >= SkFTVersion(2, 5, 1);
        case SkFontFormat::TT_sbix_Jpeg:
        case SkFontFormat::TT_sbix_Tiff:
        case SkFontFormat::TT_SVG:
            return false;
        case SkFontFormat::TT_CFF:
            return hasCFF;
        case SkFontFormat::TT_CFF2:
            return hasCFF && version >= SkFTVersion(2, 7, 1);
        case SkFontFormat::T1:
        case SkFontFormat::T1_Pfa:
        case SkFontFormat::T1_Pfb:
            return hasT1;
    }
}
