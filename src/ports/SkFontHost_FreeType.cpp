...
int SkTypeface_FreeType::onGetVariationDesignPosition(SkFontArguments::VariationCoordinate position[], size_t count) const {
    AutoFTAccess fta(this);
    FT_Face face = fta.face();
    ...
    if (!position || count < variations->num_axis) {
        ...
        !gFTLibrary->fGetVarDesignCoordinates(face, variations->num_axis, coords.get()))
    {
        for (FT_UInt i = 0; i < variations->num_axis; ++i) {
            position[i].axis = variations->axis[i].tag;
            position[i].value = SkFixedToScalar(coords[i]);
        }
    } else if (static_cast<FT_UInt>(fta.getAxesCount()) == variations->num_axis) {
        for (FT_UInt i = 0; i < variations->num_axis; ++i) {
            position[i].axis = variations->axis[i].tag;
            position[i].value = SkFixedToScalar(fta.getAxes()[i]);
        }
