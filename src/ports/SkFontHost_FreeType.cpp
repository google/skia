...
private:
    ...
    // RHEL >= 7 (6 has 4.3.11, EOL Nov 2020, Phase 3 May 2017)
...
int SkTypeface_FreeType::onGetVariationDesignPosition(
        SkFontArguments::VariationPosition position) const
{
    AutoFTAccess fta(this);
    FT_Face face = fta.face();
    ...
    if (!position.coordinates || position.coordinateCount < SkToInt(variations->num_axis)) {
        ...
        !gFTLibrary->fGetVarDesignCoordinates(face, variations->num_axis, coords.get()))
    {
        for (FT_UInt i = 0; i < variations->num_axis; ++i) {
            position.coordinates[i].axis = variations->axis[i].tag;
            position.coordinates[i].value = SkFixedToScalar(coords[i]);
        }
    } else if (static_cast<FT_UInt>(fta.getAxesCount()) == variations->num_axis) {
        for (FT_UInt i = 0; i < variations->num_axis; ++i) {
            position.coordinates[i].axis = variations->axis[i].tag;
            position.coordinates[i].value = SkFixedToScalar(fta.getAxes()[i]);
        }
    } else if ((face->face_index & 0xFFFF) == 0) {
