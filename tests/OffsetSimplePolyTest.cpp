...
DEF_TEST(OffsetSimplePoly, reporter) {
    ...
    *starPoly.push() = SkPoint::Make(-14.43f, -25.0f);
    ...
    // and past
    result = SkOffsetSimplePolygon(&clippedRRectPoly[0], clippedRRectPoly.count(), 50.5f,
                                   &offsetPoly);
    REPORTER_ASSERT(reporter, !result);

    // and now out
    result = SkOffsetSimplePolygon(&clippedRRectPoly[0], clippedRRectPoly.count(), -0.1f,
                                   &offsetPoly);
    REPORTER_ASSERT(reporter, result);

    result = SkOffsetSimplePolygon(&clippedRRectPoly[0], clippedRRectPoly.count(), -5.6665f,
                                   &offsetPoly);
    REPORTER_ASSERT(reporter, result);

    result = SkOffsetSimplePolygon(&clippedRRectPoly[0], clippedRRectPoly.count(), -50,
                                   &offsetPoly);
    REPORTER_ASSERT(reporter, result);

    // TODO: currently asserting
    //result = SkOffsetSimplePolygon(&clippedRRectPoly[0], clippedRRectPoly.count(), -100,
    //                               &offsetPoly);
    //REPORTER_ASSERT(reporter, result);
