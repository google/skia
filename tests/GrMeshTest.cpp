...
#define VALIDATE(buff) \
    ...
    run_test("setNonIndexedNonInstanced", reporter, rtc, gold, [&](DrawMeshHelper* helper) {
        ...
        for (int y = 0; y < kBoxCountY; ++y) {
            ...
            if (y & 1) {
                mesh.setNonIndexedNonInstanced(); // This should be the initial state.
            }
            mesh.setVertices(vbuff.get(), kBoxCountX * 6, y * kBoxCountX * 6);
    ...
    run_test("setIndexed", reporter, rtc, gold, [&](DrawMeshHelper* helper) {
        ...
        while (i < kBoxCount) {
            ...
            mesh.setVertices(vbuff.get(), repetitionCount * 4, (i - baseRepetition) * 4);
            helper->drawMesh(mesh);
    ...
    run_test("setIndexedPatterned", reporter, rtc, gold, [&](DrawMeshHelper* helper) {
        ...
        // Draw boxes one line at a time to exercise base vertex.
        // setIndexedPatterned intentionally does not support a base index.
        for (int y = 0; y < kBoxCountY; ++y) {
            GrMesh mesh(kTriangles_GrPrimitiveType);
            mesh.setIndexedPatterned(ibuff.get(), 6, kBoxCountX, kIndexPatternRepeatCount);
            mesh.setVertices(vbuff.get(), 4, y * kBoxCountX * 4);
            helper->drawMesh(mesh);
        }
    });
    ...
    for (bool indexed : {false, true}) {
        ...
        run_test(indexed ? "setIndexedInstanced" : "setInstanced",
            ...
            auto ibuff = indexed ? helper->getIndexBuffer() : nullptr;
            ...
            for (int y = 0; y < kBoxCountY; ++y) {
                ...
                if (indexed) {
                    VALIDATE(ibuff);
                    mesh.setIndexedInstanced(ibuff.get(), 6,
                                             instbuff.get(), kBoxCountX, y * kBoxCountX);
                } else {
                    mesh.setInstanced(instbuff.get(), kBoxCountX, y * kBoxCountX);
                }
                switch (y % 3) {
                    case 0: mesh.setVertices(vbuff.get(), 4); break;
                    case 1: mesh.setVertices(nullptr, 4); break;
                    case 2: mesh.setVertices(vbuff2.get(), 4, 2); break;
                }
                helper->drawMesh(mesh);
            }
        });
...
class GLSLMeshTestProcessor : public GrGLSLGeometryProcessor {
    ...
    void onEmitCode(EmitArgs& args, GrGPArgs* gpArgs) final {
        ...
        varyingHandler->addFlatPassThroughAttribute(mp.fColor, args.fOutputColor);
...
void DrawMeshHelper::drawMesh(const GrMesh& mesh) {
    ...
    GrMeshTestProcessor mtp(mesh.isInstanced(), SkToBool(mesh.vertexBuffer()));
...
static void run_test(const char* testName, skiatest::Reporter* reporter,
    ...
    const char* goldPx = reinterpret_cast<const char*>(gold.getPixels());
    if (h != rtc->height() || w != rtc->width()) {
        ERRORF(reporter, "[%s] expectation and rtc not compatible (?).", testName);
        return;
    }

    SkAutoSTMalloc<4*kImageHeight*kImageWidth, char> resultPx(h * rowBytes);
    rtc->clear(nullptr, 0x80808080, true);
    rtc->priv().testingOnly_addDrawOp(skstd::make_unique<GrMeshTestOp>(testFn));
    rtc->readPixels(gold.info(), resultPx, rowBytes, 0, 0, 0);
    for (int y = 0; y < h; ++y) {
        for (int x = 0; x < w; ++x) {
            if (goldPx[y * rowBytes + x] != resultPx[y * rowBytes + x]) {
                ERRORF(reporter, "[%s] rtc does not match expectation.", testName);
                return;
            }
        }
    }
