...
public:
    GrMeshTestProcessor(bool instanced, bool hasVertexBuffer)
        ...
        if (instanced) {
            ...
            this->setInstanceAttributeInfo(1, fInstanceLocation.nextOffsetInRecord());
            if (hasVertexBuffer) {
                fVertex = {"vertex", kHalf2_GrVertexAttribType};
                fColor = {"color", kUByte4_norm_GrVertexAttribType, fVertex};
                this->setVertexAttributeInfo(2, fColor.nextOffsetInRecord());
            } else {
                fColor = {"color", kUByte4_norm_GrVertexAttribType};
                this->setVertexAttributeInfo(1, fColor.nextOffsetInRecord());
            }
        } else {
            fVertex = {"vertex", kHalf2_GrVertexAttribType};
            fColor = {"color", kUByte4_norm_GrVertexAttribType, fVertex};
...
private:
    const Attribute& onVertexAttribute(int i) const override {
        return IthInitializedAttribute(i, fVertex, fColor);
    }

    const Attribute& onInstanceAttribute(int i) const override { return fInstanceLocation; }
