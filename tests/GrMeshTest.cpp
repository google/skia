...
public:
    GrMeshTestProcessor(bool instanced, bool hasVertexBuffer)
        ...
        if (instanced) {
            ...
            fColor = {"color", kUByte4_norm_GrVertexAttribType, fInstanceLocation};
            this->setInstanceAttributeInfo(2, fColor.nextOffsetInRecord());
            if (hasVertexBuffer) {
                fVertex = {"vertex", kHalf2_GrVertexAttribType};
                this->setVertexAttributeInfo(1, fVertex.nextOffsetInRecord());
            }
        } else {
            fVertex = {"vertex", kHalf2_GrVertexAttribType};
