...
DEF_TEST(Codec_partialWuffs, r) {
    const char* path = "images/animated-gif-with-offsets.gif";
    auto file = GetResourceAsData(path);
    if (!file) {
        ERRORF(r, "missing %s", path);
    } else {
        // This is where we failed the test in Chromium.
        file = SkData::MakeSubset(file.get(), 0, 679);
        // 300 is an arbitrary starting point.
        test_partial(r, path, file, 300, 379);
    }
}
