...
public:
    ...
    const SkIRect& expectedBounds() const { return fExpectedBounds; }
...
private:
    ...
    TestCase(SkString name,
        ...
        , fExpectedBounds(expectedBounds) {}
...
public:
    ...
    TestCaseBuilder& state(GrClipStack::ClipState state) {
        ...
        if (fExpectedState == GrClipStack::ClipState::kEmpty) {
            fExpectedBounds = SkIRect::MakeEmpty();
        }
        return *this;
    }
    TestCaseBuilder& bounds(const SkIRect& bounds) {
        fExpectedBounds = bounds;
        return *this;
    }

    TestCase finishTest() {
        TestCase test(fName, fDeviceBounds, fExpectedBounds, fExpectedState,
                      std::move(fActualElements),
                      std::move(fExpectedElements));

        fExpectedBounds = fDeviceBounds;
...
SkString TestCase::getTestName(const std::vector<int>& order, SavePolicy policy) const {
    ...
    name.appendf(": save %s, [", policyName.c_str());
...
void TestCase::run(const std::vector<int>& order, SavePolicy policy,
    ...
    REPORTER_ASSERT(reporter, cs.clipState() == fExpectedState,
                    ...
                    name.c_str(), (int) cs.clipState(), (int) fExpectedState);
    SkIRect bounds = cs.getConservativeBounds();
    REPORTER_ASSERT(reporter, bounds == fExpectedBounds,
                    "%s, bounds expected [%d %d %d %d], actual [%d %d %d %d]",
                    name.c_str(), bounds.fLeft, bounds.fTop, bounds.fRight, bounds.fBottom,
                    fExpectedBounds.fLeft, fExpectedBounds.fTop,
                    fExpectedBounds.fRight, fExpectedBounds.fBottom);
...
DEF_TEST(GrClipStack_RectRectAACombine, r) {
    ...
    SkRect fracRect2 = {fracRect1.fLeft + 0.5f * fracRect1.width(),
                        fracRect1.fTop + 0.5f * fracRect1.height(),
    ...
    run_test_case(r, TestCase::Build("aa+aligned-nonaa", kDeviceBounds)
                              ...
                              .state(GrClipStack::ClipState::kDeviceRect)
                              .bounds(alignedIntersect.roundOut())
                              .finishTest());

    // Other mixed AA modes do not combine
    SkIRect bounds;
    SkAssertResult(bounds.intersect(fracRect1.roundOut(), fracRect2.round()));
...
DEF_TEST(GrClipStack_RectRectNonAxisAligned, r) {
    ...
    run_test_case(r, TestCase::Build("aa", kDeviceBounds)
                              ...
                              .state(GrClipStack::ClipState::kDeviceRect)
                              .bounds(lm.mapRect(fracIntersect).roundOut())
                              .finishTest());
    ...
    run_test_case(r, TestCase::Build("nonaa", kDeviceBounds)
                              ...
                              .expect().nonAA().intersect().localToDevice(lm)
                                       .rect(fracIntersect).finishElements()
                              .state(GrClipStack::ClipState::kDeviceRect)
                              .bounds(lm.mapRect(fracIntersect).round())
                              .finishTest());

    // Local integer coords with mixed AA don't combine, though
    SkIRect bounds;
    SkAssertResult(bounds.intersect(lm.mapRect(fracRect1).roundOut(),
                                    lm.mapRect(fracRect2).round()));
...
DEF_TEST(GrClipStack_RRectRRectAACombine, r) {
    ...
    run_test_case(r, TestCase::Build("aa", kDeviceBounds)
                              ...
                              .bounds(intersect.getBounds().roundOut())
                              .finishTest());
    ...
    SkMatrix lm = SkMatrix::RotateDeg(45.f);
    SkIRect bounds = lm.mapRect(intersect.getBounds()).roundOut();
    run_test_case(r, TestCase::Build("local-aa", kDeviceBounds)
                              .actual().aa().intersect().localToDevice(lm)
                                       .rrect(r1).rrect(r2)
                                       .finishElements()
...
DEF_TEST(GrClipStack_RectRRectCombine, r) {
    ...
    SkVector cutCorners[4] = {{0, 0}, {0, 0}, {2.f, 2.f}, {2.f, 2.f}};
    SkRRect cutRRect;
    cutRRect.setRectRadii({0, 4, 10, 10}, cutCorners);
    run_test_case(r, TestCase::Build("still-rrect", kDeviceBounds)
                              ...
                              .state(GrClipStack::ClipState::kDeviceRRect)
                              .bounds(cutRRect.getBounds().roundOut())
                              .finishTest());
...
DEF_TEST(GrClipStack_ShapeDeviceBoundsClip, r) {
    SkRect crossesDeviceEdge = {20.f, kDeviceBounds.fTop - 13.2f,
                                kDeviceBounds.fRight + 15.5f, 30.f};
    SkIRect insideDeviceBounds = SkRect::MakeLTRB(20.f, kDeviceBounds.fTop,
                                                  kDeviceBounds.fRight, 30.f).roundOut();
...
DEF_TEST(GrClipStack_RepeatElement, r) {
    ...
    run_test_case(r, TestCase::Build("same-rects", kDeviceBounds)
                              ...
                              .bounds(rect.roundOut())
                              .finishTest());
    SkMatrix lm = SkMatrix::RotateDeg(30.f);
    run_test_case(r, TestCase::Build("same-local-rects", kDeviceBounds)
                              ...
                              .state(GrClipStack::ClipState::kComplex)
                              .bounds(lm.mapRect(rect).roundOut())
                              .finishTest());
    ...
    run_test_case(r, TestCase::Build("same-path", kDeviceBounds)
                              ...
                              .state(GrClipStack::ClipState::kComplex)
                              .bounds(path.getBounds().roundOut())
                              .finishTest());
    run_test_case(r, TestCase::Build("same-local-path", kDeviceBounds)
                              ...
                              .expect().localToDevice(lm).path(path)
                                       .finishElements()
                              .state(GrClipStack::ClipState::kComplex)
                              .bounds(lm.mapRect(path.getBounds()).roundOut())
                              .finishTest());
}
...
DEF_TEST(GrClipStack_Offscreen, r) {
    ...
    // Intersect -> empty
    run_test_case(r, TestCase::Build("intersect", kDeviceBounds)
                              .actual().aa().intersect()
                                       .rect(offscreenRect)
    ...
    run_test_case(r, TestCase::Build("difference", kDeviceBounds)
...
DEF_TEST(GrClipStack_DifferenceBounds, r) {
    ...
    SkIRect expectedBounds = {0, 0, 50, kDeviceBounds.fBottom};
...
DEF_TEST(GrClipStack_ScaleTranslate, r) {
    ...
    run_test_case(r, TestCase::Build("st+rrect", kDeviceBounds)
                              ...
                              .expect().rrect(deviceRRect, GrAA::kYes, SkClipOp::kIntersect)
                                       .finishElements()
                              .state(GrClipStack::ClipState::kDeviceRRect)
                              .bounds(deviceRRect.getBounds().roundOut())
                              .finishTest());
...
DEF_TEST(GrClipStack_ConvexPathContains, r) {
    ...
    run_test_case(r, TestCase::Build("convex-int+rect-diff", kDeviceBounds)
                              ...
                              .expectActual()
                              .state(GrClipStack::ClipState::kComplex)
                              .bounds(bigPath.getBounds().roundOut())
                              .finishTest());
...
DEF_TEST(GrClipStack_NonAxisAlignedContains, r) {
    ...
    SkRRect rr1 = SkRRect::MakeRectXY(r1, 1.f, 1.f);

    SkMatrix lm2 = SkMatrix::RotateDeg(-45.f);
    SkRect r2 = {-10.f, -10.f, 10.f, 10.f};
    SkRRect rr2 = SkRRect::MakeRectXY(r2, 1.f, 1.f);
    ...
    run_test_case(r, TestCase::Build("rect-rect-ii", kDeviceBounds)
                              .actual().rect(r1, lm1, GrAA::kYes, SkClipOp::kIntersect)
                                       .rect(r2, lm2, GrAA::kYes, SkClipOp::kIntersect)
                                       .finishElements()
                              .expect().rect(r2, lm2, GrAA::kYes, SkClipOp::kIntersect)
                                       .finishElements()
                              .state(GrClipStack::ClipState::kComplex)
                              .bounds(lm2.mapRect(r2).roundOut())
                              .finishTest());
    run_test_case(r, TestCase::Build("rrect-rrect-ii", kDeviceBounds)
                              .actual().rrect(rr1, lm1, GrAA::kYes, SkClipOp::kIntersect)
                                       .rrect(rr2, lm2, GrAA::kYes, SkClipOp::kIntersect)
                                       .finishElements()
                              .expect().rrect(rr2, lm2, GrAA::kYes, SkClipOp::kIntersect)
                                       .finishElements()
                              .state(GrClipStack::ClipState::kComplex)
                              .bounds(lm2.mapRect(r2).roundOut())
                              .finishTest());
    ...
    run_test_case(r, TestCase::Build("rect-rect-dd", kDeviceBounds)
                              .actual().rect(r1, lm1, GrAA::kYes, SkClipOp::kDifference)
                                       .rect(r2, lm2, GrAA::kYes, SkClipOp::kDifference)
                                       .finishElements()
                              .expect().rect(r2, lm2, GrAA::kYes, SkClipOp::kDifference)
                                       .finishElements()
                              .state(GrClipStack::ClipState::kComplex)
                              .bounds(kDeviceBounds)
                              .finishTest());
    run_test_case(r, TestCase::Build("rrect-rrect-dd", kDeviceBounds)
                              .actual().rrect(rr1, lm1, GrAA::kYes, SkClipOp::kDifference)
                                       .rrect(rr2, lm2, GrAA::kYes, SkClipOp::kDifference)
                                       .finishElements()
                              .expect().rrect(rr2, lm2, GrAA::kYes, SkClipOp::kDifference)
                                       .finishElements()
                              .state(GrClipStack::ClipState::kComplex)
                              .bounds(kDeviceBounds)
                              .finishTest());
    ...
    run_test_case(r, TestCase::Build("rectI+rectD", kDeviceBounds)
                              ...
                              .expectActual()
                              .state(GrClipStack::ClipState::kComplex)
                              .bounds(lm1.mapRect(r1).roundOut())
                              .finishTest());
...
DEF_TEST(GrClipStack_MixedAAContains, r) {
    ...
    run_test_case(r, TestCase::Build("mixed-outeraa-combine", kDeviceBounds)
                              ...
                              .expect().rect(r2Safe, lm2, GrAA::kNo, SkClipOp::kIntersect)
                                       .finishElements()
                              .state(GrClipStack::ClipState::kComplex)
                              .bounds(lm2.mapRect(r2Safe).round())
                              .finishTest());
    // Vice versa
    run_test_case(r, TestCase::Build("mixed-inneraa-combine", kDeviceBounds)
                              ...
                              .expect().rect(r2Safe, lm2, GrAA::kYes, SkClipOp::kIntersect)
                                       .finishElements()
                              .state(GrClipStack::ClipState::kComplex)
                              .bounds(lm2.mapRect(r2Safe).roundOut())
                              .finishTest());
...
// intersecting op (when mixed), or are both kept (when diff'ing).
DEF_TEST(GrClipStack_DisjointShapes, r) {
    SkRect a = {10.f, 10.f, 20.f, 20.f};
    SkRRect b = SkRRect::MakeOval(a.makeOffset({20.f, 0.f}));
    SkPath c = make_octagon(a.makeOffset({0.f, 20.f}));

    // I+I
    run_test_case(r, TestCase::Build("iii", kDeviceBounds)
                              .actual().aa().intersect().rect(a).rrect(b).path(c).finishElements()
                              .state(GrClipStack::ClipState::kEmpty)
                              .finishTest());

    // D+D
    run_test_case(r, TestCase::Build("ddd", kDeviceBounds)
                              .actual().nonAA().difference().rect(a).rrect(b).path(c)
                                       .finishElements()
                              .expectActual()
                              .state(GrClipStack::ClipState::kComplex)
                              .bounds(kDeviceBounds)
    ...
    run_test_case(r, TestCase::Build("idd", kDeviceBounds)
                              .actual().aa().intersect().rect(a)
                                       .nonAA().difference().rrect(b).path(c)
                                       .finishElements()
                              .expect().aa().intersect().rect(a).finishElements()
                              .state(GrClipStack::ClipState::kDeviceRect)
                              .bounds(a.roundOut())
                              .finishTest());
    ...
    run_test_case(r, TestCase::Build("did", kDeviceBounds)
                              .actual().aa().intersect().rrect(b)
                                       .nonAA().difference().rect(a).path(c)
                                       .finishElements()
                              .expect().aa().intersect().rrect(b).finishElements()
                              .state(GrClipStack::ClipState::kDeviceRRect)
                              .bounds(b.getBounds().roundOut())
                              .finishTest());

    // I+D path
    run_test_case(r, TestCase::Build("ddi", kDeviceBounds)
                              .actual().aa().intersect().path(c)
                                       .nonAA().difference().rect(a).rrect(b)
                                       .finishElements()
                              .expect().aa().intersect().path(c).finishElements()
                              .state(GrClipStack::ClipState::kComplex)
                              .bounds(c.getBounds().roundOut())
                              .finishTest());
}
...
DEF_TEST(GrClipStack_ComplexClip, r) {
    ...
    for (ShapeType type : { kRect, kRRect, kConvex }) {
        for (int opBits = 0; opBits < 16; ++opBits) {
            SkString name;
            name.appendf("complex-%d-%d", (int) type, opBits);

            SkIRect expectedBounds = kDeviceBounds;
            ...
            for (int i = 0; i < 4; ++i) {
                ...
                switch(type) {
                    case kRect:
                        b.actual().rect(rects[i], GrAA::kYes, op);
                        if (op == SkClipOp::kIntersect) {
                            SkAssertResult(expectedRectIntersection.intersect(rects[i]));
                        } else {
                            b.expect().rect(rects[i], GrAA::kYes, SkClipOp::kDifference);
                        }
                        break;
                    case kRRect: {
                ...
                if (op == SkClipOp::kIntersect) {
                    SkAssertResult(expectedBounds.intersect(rects[i].roundOut()));
                }
            }
            ...
            run_test_case(r, b.state(state).bounds(expectedBounds).finishTest());
        }
    }
}
...
DEF_TEST(GrClipStack_PreApply, r) {
    ...
    // Contained inside clip is kUnclipped
...
DEF_TEST(GrClipStack_Shader, r) {
    ...
    // it as a device rect
    cs.clipShader(shader);
    GrClip::PreClipResult result = cs.preApply(drawBounds, GrAA::kYes);
    REPORTER_ASSERT(r, result.fEffect == GrClip::Effect::kClipped && !result.fIsRRect,
...
DEF_TEST(GrClipStack_SimpleApply, r) {
    ...
    {
        cs.save();
        cs.clipRect(SkMatrix::I(), {10.2f, 14.342f, 63.f, 23.3f}, GrAA::kYes,
    ...
    // Convex polygons can be an analytic FP
    {
        ...
        testHasCoverageFP(rect);
        cs.restore();
    }
}

// DEF_GPUTEST_FOR_RENDERING_CONTEXTS(GrClipStack_SWMask, r, ctxInfo) {
    // FIXME how to disable CCPR  so we only go through the SW mask for a GPU_TEST??
    // FIXME can this be a mock?
#if GR_TEST_UTILS

DEF_TEST(GrClipStack_SWMask, r) {
    GrContextOptions options;
    options.fGpuPathRenderers = GpuPathRenderers::kNone; // SW only, importantly not CCPR clip atlas
    sk_sp<GrDirectContext> ctx = GrDirectContext::MakeMock(nullptr, options);

    // GrDirectContext* context = ctxInfo.directContext();
    GrDirectContext* context = ctx.get();
    std::unique_ptr<GrRenderTargetContext> rtc = GrRenderTargetContext::Make(
    ...
    auto verifyKeys = [&](const std::vector<GrUniqueKey>& expectedKeys,
        ...
        int numProxies = proxyProvider->numUniqueKeyProxies_TestOnly();
        ...
        for (auto key : expectedKeys) {
            auto proxy = proxyProvider->findProxyByUniqueKey(key);
            REPORTER_ASSERT(r, SkToBool(proxy), "Unable to find proxy for expected mask key");
        }
        for (auto key : releasedKeys) {
            auto proxy = proxyProvider->findProxyByUniqueKey(key);
            REPORTER_ASSERT(r, !SkToBool(proxy), "Proxy not released as expected");
        }
    };
