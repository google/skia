...
private:
    ...
    // The expected bounds are dependent on the order the actual elements were added to the clip
    // stack, so it's more convenient to compute it from test case than have it specified for each
    // test manually.
    SkIRect getExpectedBounds(const std::vector<int>& order) const;
    // This may be tighter than getExpectedBounds() because difference ops always affect it
    SkIRect getOptimalBounds() const;
...
SkIRect TestCase::getExpectedBounds(const std::vector<int>& order) const {
    if (fExpectedState == GrClipStack::ClipState::kEmpty) {
        return SkIRect::MakeEmpty();
    }

    SkIRect expectedBounds = fDeviceBounds;
    // Expected bounds are derived from actual submitted elements because the inner/outer
    // bounds are updated on the fly when shapes are combined, instead of recomputed from
    // the simplified set of shapes. This results in slightly different bounds intersections
    for (int i : order) {
        const GrClipStack::Element& e = fElements[i];
        SkIRect elementBounds = GrClip::GetPixelIBounds(
                e.fLocalToDevice.mapRect(e.fShape.bounds()), e.fAA);
        if ((e.fOp == SkClipOp::kIntersect && !e.fShape.inverted()) ||
            (e.fOp == SkClipOp::kDifference && e.fShape.inverted())) {
            SkAssertResult(expectedBounds.intersect(elementBounds));
        } else {
            SkIRect diff;
            if (SkRectPriv::Subtract(expectedBounds, elementBounds, &diff)) {
                expectedBounds = diff;
            }
        }
    }
...
SkIRect TestCase::getOptimalBounds() const {
    if (fExpectedState == GrClipStack::ClipState::kEmpty) {
        return SkIRect::MakeEmpty();
    }

    SkRegion region(fDeviceBounds);
    for (const GrClipStack::Element& e : fElements) {
        SkIRect elementBounds = GrClip::GetPixelIBounds(
                    e.fLocalToDevice.mapRect(e.fShape.bounds()), e.fAA);
        SkRegion eRegion(elementBounds);
        SkRegion::Op op;
        if ((e.fOp == SkClipOp::kIntersect && !e.fShape.inverted()) ||
            (e.fOp == SkClipOp::kDifference && e.fShape.inverted())) {
...
void TestCase::run(const std::vector<int>& order, SavePolicy policy,
    ...
    SkIRect expectedBounds = this->getExpectedBounds(order);
    SkIRect optimalBounds = this->getOptimalBounds();
    REPORTER_ASSERT(reporter, actualBounds.contains(optimalBounds) ||
                              (actualBounds.isEmpty() && optimalBounds.isEmpty()),
    ...
    REPORTER_ASSERT(reporter, actualBounds == expectedBounds,
                    "%s, bounds expected [%d %d %d %d], actual [%d %d %d %d]",
                    name.c_str(), expectedBounds.fLeft, expectedBounds.fTop,
