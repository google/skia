...
static void codec_yuv(skiatest::Reporter* reporter, const char path[], SkYUVAInfo* expectedSpec) {
    ...
    REPORTER_ASSERT(reporter, expectedSpec->fPlanes == yuvaInfo.fPlanes);
    REPORTER_ASSERT(reporter, expectedSpec->fYUVColorSpace == yuvaInfo.fYUVColorSpace);
    REPORTER_ASSERT(reporter, expectedSpec->fDimensions == yuvaInfo.fDimensions);
    REPORTER_ASSERT(reporter, expectedSpec->fSitingX == yuvaInfo.fSitingX);
    REPORTER_ASSERT(reporter, expectedSpec->fSitingY == yuvaInfo.fSitingY);
    REPORTER_ASSERT(reporter, expectedSpec->fOrigin == yuvaInfo.fOrigin);
...
DEF_TEST(Jpeg_YUV_Codec, r) {
    auto setExpectations = [](SkISize dims, SkYUVAInfo::Planes planes) {
        SkYUVAInfo expectedSpec;
        expectedSpec.fOrigin = kTopLeft_SkEncodedOrigin;
        expectedSpec.fYUVColorSpace = kJPEG_Full_SkYUVColorSpace;
        expectedSpec.fSitingX = SkYUVAInfo::Siting::kCentered;
        expectedSpec.fSitingY = SkYUVAInfo::Siting::kCentered;
    ...
    SkYUVAInfo expectations = setExpectations({128, 128}, SkYUVAInfo::Planes::kY_U_V_420);
    codec_yuv(r, "images/color_wheel.jpg", &expectations);

    // H2V2
    expectations = setExpectations({512, 512}, SkYUVAInfo::Planes::kY_U_V_420);
    codec_yuv(r, "images/mandrill_512_q075.jpg", &expectations);

    // H1V1
    expectations = setExpectations({512, 512}, SkYUVAInfo::Planes::kY_U_V_444);
    codec_yuv(r, "images/mandrill_h1v1.jpg", &expectations);

    // H2V1
    expectations = setExpectations({512, 512}, SkYUVAInfo::Planes::kY_U_V_422);
    codec_yuv(r, "images/mandrill_h2v1.jpg", &expectations);

    // Non-power of two dimensions
    expectations = setExpectations({439, 154}, SkYUVAInfo::Planes::kY_U_V_420);
    codec_yuv(r, "images/cropped_mandrill.jpg", &expectations);

    expectations = setExpectations({8, 8}, SkYUVAInfo::Planes::kY_U_V_420);
