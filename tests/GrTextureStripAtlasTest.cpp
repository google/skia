...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(GrTextureStripAtlasFlush, reporter, ctxInfo) {
...

    {
        ...
        targetProxy = srcProxy->copy(context, targetDesc, SkBudgeted::kYes);
        srcProxy.release();
    }
    ...
    // The atlas' use of its texture shouldn't change which pixels got copied to the target.
    {
        ...
        for (int i = 0; i < sizeof(uint32_t) * desc.fWidth * desc.fHeight; ++i, ++bytes) {
            if (0xFF != *bytes) {
                good = false;
                break;
            }
    ...
    if (!context->caps()->preferVRAMUseOverFlushes()) {
        ...
        GrSurface* srcSurface = srcProxy->instantiate(context->textureProvider());
