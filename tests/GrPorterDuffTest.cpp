...
public:
    struct XPInfo {
        XPInfo(skiatest::Reporter* reporter, SkBlendMode xfermode, const GrCaps& caps,
            ...
            fXPOutputAnalysis =
                    GrXPFactory::GetOutputAnalysis(xpf, analysis.fColorPOI, analysis.fCoveragePOI);
        ...
        GrXPFactory::OutputAnalysis fXPOutputAnalysis;
        int fOptFlags;
    ...
    for (int m = 0; m <= (int)SkBlendMode::kLastCoeffMode; m++) {
        ...
        switch (xfermode) {
            case SkBlendMode::kClear:
                TEST_ASSERT(!xpi.fXPOutputAnalysis.readsDst());
                TEST_ASSERT(xpi.fXPOutputAnalysis.isColorKnown(&color));
                TEST_ASSERT(GrColor_TRANSPARENT_BLACK == color);
            ...
            case SkBlendMode::kSrc:
                TEST_ASSERT(!xpi.fXPOutputAnalysis.readsDst());
                TEST_ASSERT(!xpi.fXPOutputAnalysis.isColorKnown());
                TEST_ASSERT(!xpi.fXPOutputAnalysis.isColorKnown());
    ...
    for (int m = 0; m <= (int)SkBlendMode::kLastCoeffMode; m++) {
        ...
        switch (xfermode) {
            ...
            case SkBlendMode::kSrc:
                TEST_ASSERT(!xpi.fXPOutputAnalysis.readsDst());
                TEST_ASSERT(xpi.fXPOutputAnalysis.isColorKnown(&color));
                TEST_ASSERT(229 == GrColorUnpackR(color));
                TEST_ASSERT(0 == GrColorUnpackG(color));
                TEST_ASSERT(154 == GrColorUnpackB(color));
                TEST_ASSERT(240 == GrColorUnpackA(color));
                TEST_ASSERT(kNone_OptFlags == xpi.fOptFlags);
                TEST_ASSERT(kModulate_OutputType == xpi.fPrimaryOutputType);
                TEST_ASSERT(kNone_OutputType == xpi.fSecondaryOutputType);
                TEST_ASSERT(kAdd_GrBlendEquation == xpi.fBlendInfo.fEquation);
...
static void test_lcd_coverage_fallback_case(skiatest::Reporter* reporter, const GrCaps& caps) {
    ...
    GrXPFactory::OutputAnalysis outputAnalysis =
            GrXPFactory::GetOutputAnalysis(xpf, colorPOI, covPOI);
    TEST_ASSERT(outputAnalysis.readsDst());
    TEST_ASSERT(outputAnalysis.isColorKnown());
