...
DEF_TEST(PathRefSerialization, reporter) {
    ...
    writer.flatten(storage.get());
    {
        ...
        SkReader32 reader(storage.get(), bytesWritten);
        bool success = reader.readPath(&readBack);
    ...
    uint32_t* writtenValues = static_cast<uint32_t*>(storage.get());
    ...
    const uint32_t tooManyObjects = static_cast<uint32_t>(~0);
    size_t offsets[] = {verbCountOffset, pointCountOffset, conicCountOffset};
    for (size_t i = 0; i < 3; ++i) {
        SkAutoMalloc storage_copy(bytesWritten);
        memcpy(storage.get(), storage_copy.get(), bytesWritten);
        static_cast<uint32_t*>(storage_copy.get())[offsets[i]] = tooManyObjects;
        SkPath readBack;
        SkReader32 reader(storage_copy.get(), bytesWritten);
        REPORTER_ASSERT(reporter, !reader.readPath(&readBack));
    }
    ...
    {
        SkPath readBack;
        SkReader32 reader(storage.get(), bytesWritten - 4);
        REPORTER_ASSERT(reporter, !reader.readPath(&readBack));
    }
}
