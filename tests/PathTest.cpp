...
static void check_convexity(skiatest::Reporter* reporter, const SkPath& path,
    ...
    SkPath copy(path); // we make a copy so that we don't cache the result on the passed in path.
    SkPathConvexityType c = (SkPathConvexityType)copy.getConvexity();
...
static void test_convexity2(skiatest::Reporter* reporter) {
    ...
    dent.close();
    check_convexity(reporter, dent, SkPathConvexityType::kConvex);
    ...
    check_convexity(reporter, strokedSin, SkPathConvexityType::kConvex);
    ...
    degenerateConcave.lineTo(41.446522f, 376.25f);
    check_convexity(reporter, degenerateConcave, SkPathConvexityType::kConvex);
    ...
    check_convexity(reporter, badFirstVector, SkPathConvexityType::kConvex);
    ...
    falseBackEdge.close();
    check_convexity(reporter, falseBackEdge, SkPathConvexityType::kConvex);
}
...
static void test_convexity(skiatest::Reporter* reporter) {
    ...
    path.reset();
    path.addRect({0, 0, 10, 10}, SkPathDirection::kCCW);
    check_convexity(reporter, path, SkPathConvexityType::kConvex);
    REPORTER_ASSERT(reporter, SkPathPriv::CheapIsFirstDirection(path, SkPathPriv::kCCW_FirstDirection));

    path.reset();
    path.addRect({0, 0, 10, 10}, SkPathDirection::kCW);
    check_convexity(reporter, path, SkPathConvexityType::kConvex);
    REPORTER_ASSERT(reporter, SkPathPriv::CheapIsFirstDirection(path, SkPathPriv::kCW_FirstDirection));
    ...
    static const struct {
        ...
        { "0 0 10 10 20 20 0 0 10 10", SkPathConvexityType::kConvex, SkPathPriv::kUnknown_FirstDirection },
        ...
        { "0 0 10 10 10 0 0 10", SkPathConvexityType::kConvex, kDontCheckDir },
        { "0 0 10 0 0 10 -10 -10", SkPathConvexityType::kConvex, SkPathPriv::kCW_FirstDirection },
    };
    ...
    for (int index = 0; index < (int) (11 * axisAlignedPtsCount); ++index) {
        ...
        if (curveSelect == 0 || curveSelect == 1 || curveSelect == 2 || curveSelect == 5) {
            ...
            SkPath copy(path); // we make a copy so that we don't cache the result on the passed in path.
            SkPathConvexityType c = copy.getConvexity();
            REPORTER_ASSERT(reporter, SkPathConvexityType::kUnknown == c
                    || SkPathConvexityType::kConvex == c);
        }
    }
...
static void test_circle_with_add_paths(skiatest::Reporter* reporter) {
    ...
    circle.addCircle(0, 0, 10, kCircleDir);
    rect.addRect({5, 5, 20, 20}, SkPathDirection::kCW);
...
static void test_rrect_convexity_is_unknown(skiatest::Reporter* reporter, SkPath* path,
    ...
    REPORTER_ASSERT(reporter, SkPathPriv::CheapIsFirstDirection(*path, SkPathPriv::AsFirstDirection(dir)));
    path->setConvexity(SkPathConvexityType::kUnknown);
    REPORTER_ASSERT(reporter, path->getConvexity() == SkPathConvexityType::kConvex);
    path->reset();
}
...
static void test_rrect(skiatest::Reporter* reporter) {
    ...
    p.addRRect(SkRRect::MakeRectRadii(r, radii));
    test_rrect_is_convex(reporter, &p, SkPathDirection::kCW);
    p.addRRect(SkRRect::MakeRectRadii(r, radii), SkPathDirection::kCCW);
    test_rrect_is_convex(reporter, &p, SkPathDirection::kCCW);
    p.addRRect(SkRRect::MakeRectXY(r, radii[1].fX, radii[1].fY));
    test_rrect_is_convex(reporter, &p, SkPathDirection::kCW);
    p.addRRect(SkRRect::MakeRectXY(r, radii[1].fX, radii[1].fY), SkPathDirection::kCCW);
    test_rrect_is_convex(reporter, &p, SkPathDirection::kCCW);
    ...
    p.addRRect(SkRRect::MakeRectXY(r, 0, 0));
    SkRect returnedRect;
    ...
    p.addRRect(SkRRect::MakeRectXY(tinyR, 5e-11f, 5e-11f));
    test_rrect_convexity_is_unknown(reporter, &p, SkPathDirection::kCW);
}
...
static void test_arc(skiatest::Reporter* reporter) {
    ...
    REPORTER_ASSERT(reporter, p.getConvexity() == SkPathConvexityType::kConvex);
    REPORTER_ASSERT(reporter, SkPathPriv::CheapIsFirstDirection(p, SkPathPriv::kCW_FirstDirection));
    p.setConvexity(SkPathConvexityType::kUnknown);
    REPORTER_ASSERT(reporter, p.getConvexity() == SkPathConvexityType::kConvex);
}
...
DEF_TEST(Paths, reporter) {
    ...
    p.addRRect(SkRRect::MakeRectXY(bounds, 1, 1));
...
DEF_TEST(ClipPath_nonfinite, reporter) {
    ...
    for (bool aa : {false, true}) {
        for (SkPathFillType ft : {SkPathFillType::kWinding, SkPathFillType::kInverseWinding}) {
...
static bool conditional_convex(const SkPath& path, bool is_convex) {
    SkPathConvexityType c = path.getConvexityOrUnknown();
...
void survive(SkPath* path, const Xforms& x, bool isAxisAligned, skiatest::Reporter* reporter,
    ...
    REPORTER_ASSERT(reporter, path2.getConvexityOrUnknown() == SkPathConvexityType::kConvex);
    ...
    REPORTER_ASSERT(reporter, path2.getConvexityOrUnknown() == SkPathConvexityType::kConvex);
    REPORTER_ASSERT(reporter, isa_proc(*path));
    REPORTER_ASSERT(reporter, path->getConvexityOrUnknown() == SkPathConvexityType::kConvex);
...
DEF_TEST(Path_survive_transform, r) {
    ...
    REPORTER_ASSERT(r, path.getConvexity() == SkPathConvexityType::kConvex);
