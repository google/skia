...
DEF_TEST(grvx_approx_angle_between_vectors, r) {
    ...
    for (uint32_t i = 0; i < (1 << 12); ++i) {
        // approx_angle_between_vectors is only valid in the range 2^(+/-30).
        uint4 validRangeExp = skvx::pin(exp, uint4(97), uint4(157));
        uint32_t a=validRangeExp[0], b=validRangeExp[1], c=validRangeExp[2], d=validRangeExp[3];
