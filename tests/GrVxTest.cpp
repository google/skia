...
// Tests several floats and verifies that grvx::fast_rcp precision is always within the specified
// range. Our strategy is to test literally all 2^23 possible mantissas, each with a pseudo-random
// exponent. Ideally we would test every single possible ieee float, but it would be too slow for a
// unit test.
DEF_TEST(grvx_fast_rcp, r) {
    ...
    for (int4 mantissa = {0,1,2,3}; skvx::all(mantissa < (1 << 23)); mantissa += 4) {
        // Denorms (exp=0) return Inf.
        // Don't bother with exp=255. We will ignore NaNs and test infinities later.
        int4 exp = skvx::pin(randomExpValues & 0xff, int4(1), int4(254));
#if defined(__SSE__)
        // fast_rcp unfortunately returns 0 starting at 2^126 (exp=253) on SSE.
        exp = skvx::min(exp, 252);
#endif
    ...
    // 1/+-Inf should return 0.
    REPORTER_ASSERT(r, fast_rcp<1>(std::numeric_limits<float>::infinity()).val == 0);
    REPORTER_ASSERT(r, fast_rcp<1>(-std::numeric_limits<float>::infinity()).val == 0);
}
#endif

#if GRVX_FAST_RSQRT_PRECISION_BITS
// Tests several floats and verifies that grvx::fast_rsqrt precision is always within the specified
// range. Our strategy is to test literally all 2^23 possible mantissas, each with a pseudo-random
// exponent. Ideally we would test every single possible ieee float, but it would be too slow for a
// unit test.
DEF_TEST(grvx_fast_rsqrt, r) {
    ...
    // 1/sqrt(Inf) should return 0.
    REPORTER_ASSERT(r, fast_rsqrt<1>(std::numeric_limits<float>::infinity()).val == 0);
}
#endif
...
DEF_TEST(grvx_fast_angle_between_vectors, r) {
    ...
    REPORTER_ASSERT(r, SkScalarNearlyZero(grvx::fast_angle_between_vectors<1>(0,0,0,0).val,
                                          exp2f(-GRVX_FAST_RCP_PRECISION_BITS)));
    ...
    REPORTER_ASSERT(r, SkScalarNearlyZero(grvx::fast_angle_between_vectors<1>(
            epsilon, epsilon, epsilon, epsilon).val, exp2f(-GRVX_FAST_RCP_PRECISION_BITS)));
