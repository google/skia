...
DEF_TEST(grvx_fast_acos, r) {
    ...
    for (int iter = 0; iter < 10; ++iter) {
        ...
        // Use Newton's method to update the x values to locations closer to their local minima or
        // maxima. (This is where d/dx(error) == 0.)
        x -= err_/err__;
...
static bool check_fast_angle_insted_vectors(skiatest::Reporter* r, SkVector a, SkVector b,
                                            float fastTheta) {
    float expectedTheta = SkMeasureAngleBetweenVectors(a, b);
    float error = expectedTheta - fastTheta;
    if (!(fabsf(error) <= GRVX_FAST_ACOS_MAX_ERROR)) {
        ERRORF(r, "Larger-than-expected error from grvx::fast_angle_insted_vectors\n"
                  "  a=              {%f, %f}\n"
                  "  b=              {%f, %f}\n"
...
static bool check_fast_angle_insted_vectors(skiatest::Reporter* r, SkVector a, SkVector b) {
    ...
    return check_fast_angle_insted_vectors(r, a, b, fastTheta);
}
...
DEF_TEST(grvx_fast_angle_between_vectors, r) {
    ...
    check_fast_angle_insted_vectors(r, {0,0}, {0,0});
    check_fast_angle_insted_vectors(r, {1,1}, {0,0});
    check_fast_angle_insted_vectors(r, {0,0}, {1,1});

    // Test infinities.
    REPORTER_ASSERT(r, SkScalarNearlyZero(grvx::fast_angle_between_vectors<1>(
            std::numeric_limits<float>::infinity(),1,2,3).val));
    check_fast_angle_insted_vectors(r, {std::numeric_limits<float>::infinity(),1}, {2,3});
    check_fast_angle_insted_vectors(r, {0,-std::numeric_limits<float>::infinity()}, {2,3});
    check_fast_angle_insted_vectors(r, {0,1}, {std::numeric_limits<float>::infinity(),3});
    check_fast_angle_insted_vectors(r, {0,1}, {2,-std::numeric_limits<float>::infinity()});

    // Test NaNs.
    REPORTER_ASSERT(r, SkScalarNearlyZero(grvx::fast_angle_between_vectors<1>(
            std::numeric_limits<float>::quiet_NaN(),1,2,3).val));
    check_fast_angle_insted_vectors(r, {std::numeric_limits<float>::quiet_NaN(),1}, {2,3});
    check_fast_angle_insted_vectors(r, {0,std::numeric_limits<float>::quiet_NaN()}, {2,3});
    check_fast_angle_insted_vectors(r, {0,1}, {std::numeric_limits<float>::quiet_NaN(),3});
    check_fast_angle_insted_vectors(r, {0,1}, {2,std::numeric_limits<float>::quiet_NaN()});
    ...
    check_fast_angle_insted_vectors(r, {epsilon, epsilon}, {epsilon, epsilon});
    check_fast_angle_insted_vectors(r, {epsilon, epsilon}, {-epsilon, -epsilon});
    check_fast_angle_insted_vectors(r, {epsilon, -epsilon*2}, {-epsilon*3, epsilon*4});
    ...
    for (int i = 0; i < (1 << 12); ++i) {
        ...
        for (int j = 0; j < 4; ++j) {
            if (!check_fast_angle_insted_vectors(r, {x0[j], y0[j]}, {x1[j], y1[j]}, rads[j])) {
                return;
            }
        }
