...
DEF_TEST(SafeMath, r) {
    ...
    {  // natural type mixing when sizing an allocation
        ...
        // Nothing tricky.  Everything fits fine in any size_t.
        int n = 1024;
        size_t alloc = 0;
        ...
        // Each of product fits safely in a 32-bit size_t, but their sum just barely tips over.
        n = 256*1024*1024;
        alloc = 0;
        alloc = safe.add(alloc, safe.mul(sizeof(int   ), n));
        alloc = safe.add(alloc, safe.mul(sizeof(float ), n));
        ...
        if (sizeof(size_t) == 8) {
            REPORTER_ASSERT(r, safe);
            REPORTER_ASSERT(r, alloc == 4ull * 1024 * 1024 * 1024);
        } else {
            REPORTER_ASSERT(r, SkSafeMath::Unchecked() || !safe);
        }
    }
