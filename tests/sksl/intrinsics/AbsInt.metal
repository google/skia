...
fragment Outputs fragmentMain(Inputs _in [[stage_in]], constant Uniforms& _uniforms [[buffer(0)]], bool _frontFacing [[front_facing]], float4 _fragCoord [[position]]) {
    ...
    _out.sk_FragColor = ((((((abs(int(_uniforms.testInputs.x)) == expected.x && all(abs(int2(_uniforms.testInputs.xy)) == expected.xy)) && all(abs(int3(_uniforms.testInputs.xyz)) == expected.xyz)) && all(abs(int4(_uniforms.testInputs)) == expected)) && 1 == expected.x) && all(int2(1, 0) == expected.xy)) && all(int3(1, 0, 0) == expected.xyz)) && all(abs(int4(constVal)) == expected) ? _uniforms.colorGreen : _uniforms.colorRed;
    return _out;
}
