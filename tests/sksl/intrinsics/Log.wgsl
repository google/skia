...
fn _skslMain(coords: vec2<f32>) -> vec4<f32> {
  {
    let _skTemp0 = log(_globalUniforms.inputVal.x);
    let _skTemp1 = log(_globalUniforms.inputVal.xy);
    let _skTemp2 = log(_globalUniforms.inputVal.xyz);
    let _skTemp3 = log(_globalUniforms.inputVal);
