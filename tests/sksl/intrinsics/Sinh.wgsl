...
fn _skslMain(coords: vec2<f32>) -> vec4<f32> {
  {
    let _skTemp0 = sinh(_globalUniforms.inputVal.x);
    let _skTemp1 = sinh(_globalUniforms.inputVal.xy);
    let _skTemp2 = sinh(_globalUniforms.inputVal.xyz);
    let _skTemp3 = sinh(_globalUniforms.inputVal);
