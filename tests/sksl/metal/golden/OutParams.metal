...
fragment Outputs fragmentMain(Inputs _in [[stage_in]], bool _frontFacing [[front_facing]], float4 _fragCoord [[position]]) {
    ...
    _skOut.sk_FragColor = float4(h, h2.x, h3.x, h4.x);
    float2x2 h2x2;
    ...
    _skOut.sk_FragColor = float4(float(i), float(i2.x), float(i3.x), float(i4.x));
    float f;
    ...
    _skOut.sk_FragColor = float4(f, f2.x, f3.x, f4.x);
    float2x2 f2x2;
    ...
    _skOut.sk_FragColor = float4(f2x2[0].x, f3x3[0].x, f4x4[0].x, 1.0);
    bool b;
    ...
    _skOut.sk_FragColor = float4(float(b), float(b2.x), float(b3.x), float(b4.x));
    return _skOut;
}
