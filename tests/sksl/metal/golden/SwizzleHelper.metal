...
float4 _SwizzleHelper0_for_fn(thread Outputs* _out, thread Globals* _globals, float var0, thread float3& b, float2 var2, thread float3x3& d) {
    float2 var1 = b.yz;
    float3 var3 = d[1].zyx;
    float4 result = fn(_out, _globals, var0, var1, var2, var3);
    b.yz = var1;
    d[1].zyx = var3;
    return result;
}
...
fragment Outputs fragmentMain(Inputs _in [[stage_in]], bool _frontFacing [[front_facing]], float4 _fragCoord [[position]]) {
    ...
    _out->sk_FragColor =     _SwizzleHelper0_for_fn(_out, _globals, 1.0, b, float4x4(3.0)[0].zx, d);
    return *_out;
}
