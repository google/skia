...
zero = 0.0f;
one = 1.0f;
Var _zero(kConst_Modifier, DSLType(kFloat_Type), "zero");
DeclareGlobal(_zero, Float(zero));
Var _one(kConst_Modifier, DSLType(kFloat_Type), "one");
DeclareGlobal(_one, Float(one));
Var _m(kNo_Modifier, DSLType(kHalf4x4_Type), "m");
Var _n(kNo_Modifier, DSLType(kHalf4_Type), "n");
Var _b(kNo_Modifier, DSLType(kBool4_Type), "b");
Declare(_m, Half4x4(Half(_one)));
Declare(_n, Half4(Half(_zero)));
Declare(_b, Bool4(true));
_n.x() = Abs(_n.x());
