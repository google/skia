...
Var _one(kConst_Modifier, DSLType(kHalf_Type), "one");
Declare(_one, Half(one));
[[maybe_unused]] const auto& unused = _outer.unused;
Var _unused(kConst_Modifier, DSLType(kHalf_Type), "unused");
Declare(_unused, Half(unused));
alsoUnused = float(one + one);
Var _alsoUnused(kConst_Modifier, DSLType(kFloat_Type), "alsoUnused");
DeclareGlobal(_alsoUnused, Float(alsoUnused));
Var _color(kNo_Modifier, DSLType(kHalf4_Type), "color");
Declare(_color, Half4(0.0f));
