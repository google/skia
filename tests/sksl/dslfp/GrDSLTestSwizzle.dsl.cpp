...
class GrGLSLDSLTestSwizzle : public GrGLSLFragmentProcessor {
public:
    GrGLSLDSLTestSwizzle() {}
    void emitCode(EmitArgs& args) override {
        const GrDSLTestSwizzle& _outer = args.fFp.cast<GrDSLTestSwizzle>();
        (void) _outer;
...
std::unique_ptr<GrGLSLFragmentProcessor> GrDSLTestSwizzle::onMakeProgramImpl() const {
    return std::make_unique<GrGLSLDSLTestSwizzle>();
}
void GrDSLTestSwizzle::onGetGLSLProcessorKey(const GrShaderCaps& caps, GrProcessorKeyBuilder* b) const {
}
bool GrDSLTestSwizzle::onIsEqual(const GrFragmentProcessor& other) const {
    const GrDSLTestSwizzle& that = other.cast<GrDSLTestSwizzle>();
    (void) that;
    return true;
}
GrDSLTestSwizzle::GrDSLTestSwizzle(const GrDSLTestSwizzle& src)
: INHERITED(kGrDSLTestSwizzle_ClassID, src.optimizationFlags()) {
        this->cloneAndRegisterAllChildProcessors(src);
}
std::unique_ptr<GrFragmentProcessor> GrDSLTestSwizzle::clone() const {
    return std::make_unique<GrDSLTestSwizzle>(*this);
}
#if GR_TEST_UTILS
SkString GrDSLTestSwizzle::onDumpInfo() const {
    return SkString();
}
#endif
