...
class GrGLSLFunctionArgs : public GrGLSLFragmentProcessor {
public:
    GrGLSLFunctionArgs() {}
    void emitCode(EmitArgs& args) override {
        GrGLSLFPFragmentBuilder* fragBuilder = args.fFragBuilder;
        const GrFunctionArgs& _outer = args.fFp.cast<GrFunctionArgs>();
        (void) _outer;
...
GrGLSLFragmentProcessor* GrFunctionArgs::onCreateGLSLInstance() const {
    return new GrGLSLFunctionArgs();
}
void GrFunctionArgs::onGetGLSLProcessorKey(const GrShaderCaps& caps, GrProcessorKeyBuilder* b) const {
}
bool GrFunctionArgs::onIsEqual(const GrFragmentProcessor& other) const {
    const GrFunctionArgs& that = other.cast<GrFunctionArgs>();
    (void) that;
    return true;
}
GrFunctionArgs::GrFunctionArgs(const GrFunctionArgs& src)
: INHERITED(kGrFunctionArgs_ClassID, src.optimizationFlags()) {
        this->cloneAndRegisterAllChildProcessors(src);
}
std::unique_ptr<GrFragmentProcessor> GrFunctionArgs::clone() const {
    return std::make_unique<GrFunctionArgs>(*this);
}
#if GR_TEST_UTILS
SkString GrFunctionArgs::onDumpInfo() const {
    return SkString();
}
#endif
