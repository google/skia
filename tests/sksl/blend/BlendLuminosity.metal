...
half4 blend_luminosity_h4h4h4(half4 src, half4 dst);
half blend_color_luminance_Qhh3(half3 color) {
...
half4 blend_luminosity_h4h4h4(half4 src, half4 dst) {
    return blend_hslc_h4h4h4h2(src, dst, half2(1.0h, 0.0h));
}
fragment Outputs fragmentMain(Inputs _in [[stage_in]], constant Uniforms& _uniforms [[buffer(0)]], bool _frontFacing [[front_facing]], float4 _fragCoord [[position]]) {
    Outputs _out;
    (void)_out;
    _out.sk_FragColor = blend_luminosity_h4h4h4(_uniforms.src, _uniforms.dst);
    return _out;
}
