...
vec4 main() {
    ...
    do {
        if (dst.w == 0.0) {
            {
                _0_blend_soft_light = src;
                break;
            }
        }
        {
            _0_blend_soft_light = vec4(_soft_light_component(src.x, src.w, dst.x, dst.w), _soft_light_component(src.y, src.w, dst.y, dst.w), _soft_light_component(src.z, src.w, dst.z, dst.w), src.w + (1.0 - src.w) * dst.w);
            break;
        }
    } while (false);
