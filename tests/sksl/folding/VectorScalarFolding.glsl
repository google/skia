
out vec4 sk_FragColor;
uniform vec4 colorRed;
uniform vec4 colorGreen;
uniform float unknownInput;
bool test_int() {
    bool ok = true;
    ivec4 x = ivec4(6, 6, 7, 8);
    ok = ok && x == ivec4(6, 6, 7, 8);
    x = ivec4(7, 9, 9, 9);
    ok = ok && x == ivec4(7, 9, 9, 9);
    x = ivec4(9, 9, 10, 10);
    ok = ok && x == ivec4(9, 9, 10, 10);
    x.xyz = ivec3(6, 6, 6);
    ok = ok && x == ivec4(6, 6, 6, 10);
    x.xy = ivec2(3, 3);
    ok = ok && x == ivec4(3, 3, 6, 10);
    x = ivec4(6, 6, 6, 6).yxwz;
    ok = ok && x == ivec4(6);
    x = ivec4(6, 6, 7, 8);
    ok = ok && x == ivec4(6, 6, 7, 8);
    x = ivec4(-7, -9, -9, -9);
    ok = ok && x == ivec4(-7, -9, -9, -9);
    x = ivec4(9, 9, 10, 10);
    ok = ok && x == ivec4(9, 9, 10, 10);
    x.xyz = ivec3(6, 6, 6);
    ok = ok && x == ivec4(6, 6, 6, 10);
    x.xy = ivec2(8, 8);
    ok = ok && x == ivec4(8, 8, 6, 10);
    x = ivec4(200, 100, 50, 25);
    ok = ok && x == ivec4(200, 100, 50, 25);
    x = ivec4(6, 6, 6, 6).yxwz;
    ok = ok && x == ivec4(6);
    int unknown = int(unknownInput);
    x = ivec4(0) + unknown;
    ok = ok && x == ivec4(unknown);
    x = ivec4(0) * unknown;
    ok = ok && x == ivec4(0);
    x = ivec4(0) / unknown;
    ok = ok && x == ivec4(0);
    x = ivec4(1) * unknown;
    ok = ok && x == ivec4(unknown);
    x = unknown * ivec4(1);
    ok = ok && x == ivec4(unknown);
    x = unknown + ivec4(0);
    ok = ok && x == ivec4(unknown);
    x = unknown - ivec4(0);
    ok = ok && x == ivec4(unknown);
    x = unknown / ivec4(1);
    ok = ok && x == ivec4(unknown);
    x = 0 + ivec4(unknown);
    ok = ok && x == ivec4(unknown);
    x = 0 * ivec4(unknown);
    ok = ok && x == ivec4(0);
    x = 0 / ivec4(unknown);
    ok = ok && x == ivec4(0);
    x = 1 * ivec4(unknown);
    ok = ok && x == ivec4(unknown);
    x = ivec4(unknown) + 0;
    ok = ok && x == ivec4(unknown);
    x = ivec4(unknown) * 0;
    ok = ok && x == ivec4(0);
    x = ivec4(unknown) * 1;
    ok = ok && x == ivec4(unknown);
    x = ivec4(unknown) - 0;
    ok = ok && x == ivec4(unknown);
    x = ivec4(unknown);
    x += 1;
    x += 0;
    x -= 1;
    x -= 0;
    x *= 1;
    x /= 1;
    ok = ok && x == ivec4(unknown);
    x = ivec4(unknown);
    x = x + 1;
    x = x + 0;
    x = x - 1;
    x = x - 0;
    x = x * 1;
    x = x / 1;
    ok = ok && x == ivec4(unknown);
    return ok;
}
vec4 main() {
    bool _0_test_half;
    bool _1_ok = true;
    vec4 _2_x = vec4(6.0, 6.0, 7.0, 8.0);
    _1_ok = _1_ok && _2_x == vec4(6.0, 6.0, 7.0, 8.0);
    _2_x = vec4(7.0, 9.0, 9.0, 9.0);
    _1_ok = _1_ok && _2_x == vec4(7.0, 9.0, 9.0, 9.0);
    _2_x = vec4(9.0, 9.0, 10.0, 10.0);
    _1_ok = _1_ok && _2_x == vec4(9.0, 9.0, 10.0, 10.0);
    _2_x.xyz = vec3(6.0, 6.0, 6.0);
    _1_ok = _1_ok && _2_x == vec4(6.0, 6.0, 6.0, 10.0);
    _2_x.xy = vec2(3.0, 3.0);
    _1_ok = _1_ok && _2_x == vec4(3.0, 3.0, 6.0, 10.0);
    _2_x = vec4(6.0, 6.0, 6.0, 6.0).yxwz;
    _1_ok = _1_ok && _2_x == vec4(6.0);
    _2_x = vec4(6.0, 6.0, 7.0, 8.0);
    _1_ok = _1_ok && _2_x == vec4(6.0, 6.0, 7.0, 8.0);
    _2_x = vec4(-7.0, -9.0, -9.0, -9.0);
    _1_ok = _1_ok && _2_x == vec4(-7.0, -9.0, -9.0, -9.0);
    _2_x = vec4(9.0, 9.0, 10.0, 10.0);
    _1_ok = _1_ok && _2_x == vec4(9.0, 9.0, 10.0, 10.0);
    _2_x.xyz = vec3(6.0, 6.0, 6.0);
    _1_ok = _1_ok && _2_x == vec4(6.0, 6.0, 6.0, 10.0);
    _2_x.xy = vec2(8.0, 8.0);
    _1_ok = _1_ok && _2_x == vec4(8.0, 8.0, 6.0, 10.0);
    _2_x = vec4(2.0, 1.0, 0.5, 0.25);
    _1_ok = _1_ok && _2_x == vec4(2.0, 1.0, 0.5, 0.25);
    _2_x = vec4(6.0, 6.0, 6.0, 6.0).yxwz;
    _1_ok = _1_ok && _2_x == vec4(6.0);
    float _3_unknown = unknownInput;
    _2_x = vec4(0.0) + _3_unknown;
    _1_ok = _1_ok && _2_x == vec4(_3_unknown);
    _2_x = vec4(0.0) * _3_unknown;
    _1_ok = _1_ok && _2_x == vec4(0.0);
    _2_x = vec4(0.0) / _3_unknown;
    _1_ok = _1_ok && _2_x == vec4(0.0);
    _2_x = vec4(1.0) * _3_unknown;
    _1_ok = _1_ok && _2_x == vec4(_3_unknown);
    _2_x = _3_unknown * vec4(1.0);
    _1_ok = _1_ok && _2_x == vec4(_3_unknown);
    _2_x = _3_unknown + vec4(0.0);
    _1_ok = _1_ok && _2_x == vec4(_3_unknown);
    _2_x = _3_unknown - vec4(0.0);
    _1_ok = _1_ok && _2_x == vec4(_3_unknown);
    _2_x = _3_unknown / vec4(1.0);
    _1_ok = _1_ok && _2_x == vec4(_3_unknown);
    _2_x = 0.0 + vec4(_3_unknown);
    _1_ok = _1_ok && _2_x == vec4(_3_unknown);
    _2_x = 0.0 * vec4(_3_unknown);
    _1_ok = _1_ok && _2_x == vec4(0.0);
    _2_x = 0.0 / vec4(_3_unknown);
    _1_ok = _1_ok && _2_x == vec4(0.0);
    _2_x = 1.0 * vec4(_3_unknown);
    _1_ok = _1_ok && _2_x == vec4(_3_unknown);
    _2_x = vec4(_3_unknown) + 0.0;
    _1_ok = _1_ok && _2_x == vec4(_3_unknown);
    _2_x = vec4(_3_unknown) * 0.0;
    _1_ok = _1_ok && _2_x == vec4(0.0);
    _2_x = vec4(_3_unknown) * 1.0;
    _1_ok = _1_ok && _2_x == vec4(_3_unknown);
    _2_x = vec4(_3_unknown) - 0.0;
    _1_ok = _1_ok && _2_x == vec4(_3_unknown);
    _2_x = vec4(_3_unknown);
    _2_x += 1.0;
    _2_x += 0.0;
    _2_x -= 1.0;
    _2_x -= 0.0;
    _2_x *= 1.0;
    _2_x /= 1.0;
    _1_ok = _1_ok && _2_x == vec4(_3_unknown);
    _2_x = vec4(_3_unknown);
    _2_x = _2_x + 1.0;
    _2_x = _2_x + 0.0;
    _2_x = _2_x - 1.0;
    _2_x = _2_x - 0.0;
    _2_x = _2_x * 1.0;
    _2_x = _2_x / 1.0;
    _1_ok = _1_ok && _2_x == vec4(_3_unknown);
    return _1_ok && test_int() ? colorGreen : colorRed;

}
