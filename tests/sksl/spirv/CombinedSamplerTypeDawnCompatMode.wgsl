...
fn helper2_h4ZZ(s1_Texture: texture_2d<f32>, s1_Sampler: sampler, s2_Texture: texture_2d<f32>, s2_Sampler: sampler) -> vec4<f32> {
  {
    return textureSample(s1_Texture, s1_Sampler, vec2<f32>(1.0)) + helper_h4Z(s2_Texture, s2_Sampler);
  }
}
fn _skslMain(_stageOut: ptr<function, FSOut>) {
  {
    (*_stageOut).sk_FragColor = (textureSample(aSampler_Texture, aSampler_Sampler, vec2<f32>(0.0)) + helper_h4Z(aSampler_Texture, aSampler_Sampler)) + helper2_h4ZZ(aSampler_Texture, aSampler_Sampler, anotherSampler_Texture, anotherSampler_Sampler);
  }
}
