...
fragment Outputs fragmentMain(Inputs _in [[stage_in]], bool _frontFacing [[front_facing]], float4 _fragCoord [[position]]) {
    ...
    _out->sk_FragColor.x = sqrt(1.0) + 0.0;
    _out->sk_FragColor.x = 0.0 + sqrt(2.0);
    _out->sk_FragColor.x = sqrt(3.0) - 0.0;
    _out->sk_FragColor.x = sqrt(4.0) * 0.0;
    _out->sk_FragColor.x = sqrt(5.0) * 1.0;
    _out->sk_FragColor.x = 1.0 * sqrt(6.0);
    _out->sk_FragColor.x = 0.0 * sqrt(7.0);
    _out->sk_FragColor.x = sqrt(8.0) / 1.0;
    _out->sk_FragColor.x = 0.0 / sqrt(9.0);
    _out->sk_FragColor.x = _out->sk_FragColor.x + 1.0;
    _out->sk_FragColor.x = _out->sk_FragColor.x + 0.0;
    _out->sk_FragColor.x = _out->sk_FragColor.x - 1.0;
    _out->sk_FragColor.x = _out->sk_FragColor.x - 0.0;
    _out->sk_FragColor.x = _out->sk_FragColor.x * 1.0;
