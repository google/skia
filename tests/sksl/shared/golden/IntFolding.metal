...
fragment Outputs fragmentMain(Inputs _in [[stage_in]], bool _frontFacing [[front_facing]], float4 _fragCoord [[position]]) {
    ...
    _out->sk_FragColor.x = float(int(sqrt(1.0)) + 0);
    _out->sk_FragColor.x = float(0 + int(sqrt(2.0)));
    _out->sk_FragColor.x = float(int(sqrt(3.0)) - 0);
    _out->sk_FragColor.x = float(int(sqrt(4.0)) * 0);
    _out->sk_FragColor.x = float(int(sqrt(5.0)) * 1);
    _out->sk_FragColor.x = float(1 * int(sqrt(6.0)));
    _out->sk_FragColor.x = float(0 * int(sqrt(7.0)));
    _out->sk_FragColor.x = float(int(sqrt(8.0)) / 1);
    _out->sk_FragColor.x = float(0 / int(sqrt(9.0)));
    int x = int(sqrt(2.0));
    x += 1;
    x += 0;
    x -= 1;
