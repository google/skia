...
vertex Outputs vertexMain(Inputs _in [[stage_in]], uint sk_VertexID [[vertex_id]], uint sk_InstanceID [[instance_id]]) {
    ...
    _skOut.vcoord_Stage0.x = float(0 == sk_VertexID % 2 ? -1 : 1);
    _skOut.vcoord_Stage0.y = float(0 == sk_VertexID / 2 ? -1 : 1);
    _skOut.sk_Position = float4(vertexpos.x, vertexpos.y, 0.0, 1.0);
