...
fn test_half_b() -> bool {
  {
    ...
    const _skTemp1 = mat2x2<f32>(1.0, 2.0, 3.0, 4.0);
    ok = ok && (all(m3[0] == _skTemp1[0]) && all(m3[1] == _skTemp1[1]));
    const m4: mat2x2<f32> = mat2x2<f32>(6.0, 0.0, 0.0, 6.0);
    const _skTemp2 = mat2x2<f32>(6.0, 0.0, 0.0, 6.0);
    ok = ok && (all(m4[0] == _skTemp2[0]) && all(m4[1] == _skTemp2[1]));
    m3 = m3 * m4;
    const _skTemp3 = mat2x2<f32>(6.0, 12.0, 18.0, 24.0);
    ok = ok && (all(m3[0] == _skTemp3[0]) && all(m3[1] == _skTemp3[1]));
    let _skTemp4 = m1[1].y;
    let m5: mat2x2<f32> = mat2x2<f32>(_skTemp4, 0.0, 0.0, _skTemp4);
    const _skTemp5 = mat2x2<f32>(4.0, 0.0, 0.0, 4.0);
    ok = ok && (all(m5[0] == _skTemp5[0]) && all(m5[1] == _skTemp5[1]));
    m1 = m1 + m5;
    const _skTemp6 = mat2x2<f32>(5.0, 2.0, 3.0, 8.0);
    ok = ok && (all(m1[0] == _skTemp6[0]) && all(m1[1] == _skTemp6[1]));
    const m7: mat2x2<f32> = mat2x2<f32>(5.0, 6.0, 7.0, 8.0);
    const _skTemp7 = mat2x2<f32>(5.0, 6.0, 7.0, 8.0);
    ok = ok && (all(m7[0] == _skTemp7[0]) && all(m7[1] == _skTemp7[1]));
    const m9: mat3x3<f32> = mat3x3<f32>(9.0, 0.0, 0.0, 0.0, 9.0, 0.0, 0.0, 0.0, 9.0);
    const _skTemp8 = mat3x3<f32>(9.0, 0.0, 0.0, 0.0, 9.0, 0.0, 0.0, 0.0, 9.0);
    ok = ok && (all(m9[0] == _skTemp8[0]) && all(m9[1] == _skTemp8[1]) && all(m9[2] == _skTemp8[2]));
    const m10: mat4x4<f32> = mat4x4<f32>(11.0, 0.0, 0.0, 0.0, 0.0, 11.0, 0.0, 0.0, 0.0, 0.0, 11.0, 0.0, 0.0, 0.0, 0.0, 11.0);
    const _skTemp9 = mat4x4<f32>(11.0, 0.0, 0.0, 0.0, 0.0, 11.0, 0.0, 0.0, 0.0, 0.0, 11.0, 0.0, 0.0, 0.0, 0.0, 11.0);
    ...
    m11 = m11 - m10;
    const _skTemp10 = mat4x4<f32>(9.0, 20.0, 20.0, 20.0, 20.0, 9.0, 20.0, 20.0, 20.0, 20.0, 9.0, 20.0, 20.0, 20.0, 20.0, 9.0);
