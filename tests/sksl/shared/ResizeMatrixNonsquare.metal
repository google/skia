...
fragment Outputs fragmentMain(Inputs _in [[stage_in]], constant Uniforms& _uniforms [[buffer(0)]], bool _frontFacing [[front_facing]], float4 _fragCoord [[position]]) {
    ...
    result = result + float3x3_from_float3x2(float3x2(1.0))[0].x;
    result = result + float4x4_from_float4x3(float4x3_from_float4x2(float4x2(1.0)))[0].x;
    result = result + float4x4_from_float3x4(float3x4_from_float2x4(float2x4(1.0)))[0].x;
    result = result + float2x4_from_float4x2(float4x2(1.0))[0].x;
    result = result + float4x2_from_float2x4(float2x4(1.0))[0].x;
