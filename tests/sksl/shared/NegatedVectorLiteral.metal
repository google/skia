...
bool test_float() {
    ...
    float two = 2.0;
    float4 result;
    result.x = float(all(float4(-1.0) == -float4(-float2(-1.0), float2(1.0))) ? 1 : 0);
    result.y = float(any(float4(1.0) != -float4(1.0)) ? 1 : 0);
    result.z = float(all(-float4(two) == float4(-two, float3(-two))) ? 1 : 0);
    result.w = float(all(-float2(-one, one + one) == -float2(one - two, two)) ? 1 : 0);
