...
fn _skslMain(_stageOut: ptr<function, FSOut>) {
  {
    let a: vec4<f32> = vec4<f32>(textureSample(tex_Texture, tex_Sampler, vec2<f32>(0.0)));
    let _skTemp2 = vec3<f32>(0.0);
    let b: vec4<f32> = vec4<f32>(textureSample(tex_Texture, tex_Sampler, _skTemp2.xy / _skTemp2.z));
