...
static void gpu_read_pixels_test_driver(skiatest::Reporter* reporter,
    ...
    auto runTest = [&](const T& src, const SkPixmap& srcPixels, const SkImageInfo& readInfo,
        ...
        static constexpr auto kInitialByte = static_cast<uint8_t>(0xAB);
        std::fill_n(dstPixels.writable_addr8(0, 0), dstPixels.computeByteSize(), kInitialByte);
        ...
        if (success && srcReadRect.intersect(surfBounds, rect)) {
            ...
            int rgbBits = std::min({min_rgb_channel_bits(readCT), min_rgb_channel_bits(srcCT)});
