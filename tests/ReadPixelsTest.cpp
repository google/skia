...
static void gpu_read_pixels_test_driver(skiatest::Reporter* reporter,
    ...
    for (int sat = 0; sat < kLastEnum_SkAlphaType; ++sat) {
        ...
        for (int sct = 0; sct <= kLastEnum_SkColorType; ++sct) {
            ...
            // We could but 1010102 premul is kind of dubious anyway. So for now just keep it
            // opaque. Note that this is the only place where the ref data depends on srcCT and if
            // we remove this we can also but the ref data setup above the srcCT loop.
