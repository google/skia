...
static void gpu_read_pixels_test_driver(GrContext* context, skiatest::Reporter* reporter,
    ...
    auto runTest = [&](SkSurface* surf, SkImage* refImg, const SkImageInfo& readInfo,
        ...
        if (success && srcReadRect.intersect(surfBounds, rect)) {
            ...
            const float numer = csConversion ? 3.f : 2.f;
            const int rgbBits = std::min({min_rgb_channel_bits(readCT),
                                          min_rgb_channel_bits(surfCT),
                                          8});
            const float tol = numer / (1 << rgbBits);
    ...
    for (auto origin : {kTopLeft_GrSurfaceOrigin, kBottomLeft_GrSurfaceOrigin}) {
        for (int sct = 0; sct <= kLastEnum_SkColorType; ++sct) {
            const auto surfCT = static_cast<SkColorType>(sct);
            const auto info = SkImageInfo::Make(kW, kH, surfCT, kPremul_SkAlphaType,
                                                SkColorSpace::MakeSRGB());
            auto surf =
            ...
            const float d = std::sqrt((float)surf->width() * surf->width() +
                ...
                for (const sk_sp<SkColorSpace>& readCS :
                    ...
                    for (int at = 0; at <= kLastEnum_SkAlphaType; ++at) {
                        ...
                        for (const auto& rect : {
                            ...
                            const auto readInfo = SkImageInfo::Make(rect.width(),
                                                                    rect.height(),
                                                                    readCT,
                                                                    readAT,
                                                                    readCS);
                            const SkIPoint readPt = rect.topLeft();
