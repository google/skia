            ...
            for (const auto& rect : {SkIRect::MakeWH(kW, kH),                  // full size
                ...
                for (int rct = 0; rct <= kLastEnum_SkColorType; ++rct) {
                    ...
                    for (const sk_sp<SkColorSpace>& readCS :
                        ...
                        // When there is a conversion allow a diff of two values in the read
                        // color type, except for alpha where we allow no difference.
                        // Allow intermediate truncation to an 8bit format.
                        float tol = 0.f;
                        ...
                        auto error = std::function<ComparePixmapsErrorReporter>(
                                [&](int x, int y, const float diffs[4]) {
                                    ERRORF(reporter,
                                           "Surf Color Type: %d, Read CT: %d, Rect [%d, %d, "
                                           "%d, %d], origin: %d, CS "
