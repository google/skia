...
#if SK_SUPPORT_GPU
static bool fill_src_texture(GrContext* context, GrTextureProxy* proxy) {
    SkBitmap bmp = make_src_bitmap();
    bmp.lockPixels();

    bool result = context->contextPriv().writeSurfacePixels(proxy, nullptr, 
                                                            0, 0, DEV_W, DEV_H,
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(ReadPixels_Texture, reporter, ctxInfo) {
    ...
    for (auto origin : {kBottomLeft_GrSurfaceOrigin, kTopLeft_GrSurfaceOrigin}) {
        for (auto flags : {kNone_GrSurfaceFlags, kRenderTarget_GrSurfaceFlag}) {
            ...
            desc.fOrigin = origin;
            sk_sp<GrTexture> texture(ctxInfo.grContext()->resourceProvider()->createTexture1(
                desc, SkBudgeted::kNo));
