            ...
            for (int bx = 0; bx < bw; ++bx) {
                ...
                if (clippedSrcRect.contains(devx, devy)) {
                    if (checkSurfacePixels) {
                        uint8_t surfaceAlpha = SkGetPackedA32(get_src_color(devx, devy));
...
bool read_should_succeed(const SkImageInfo& dstInfo, const SkImageInfo& srcInfo, bool isGPU) {
    ...
    if (SkColorTypeIsAlwaysOpaque(srcInfo.colorType()) && SkColorTypeIsAlphaOnly(dstInfo.colorType())) {
        return false;
    }
        ...
        for (BitmapInit bmi = kFirstBitmapInit; bmi <= lastBitmapInit; bmi = nextBMI(bmi)) {
            for (size_t c = 0; c < SK_ARRAY_COUNT(gReadPixelsConfigs); ++c) {
                ...
                bool expectSuccess = SkIRect::Intersects(srcRect, DEV_RECT) &&
                                     read_should_succeed(bmp.info(), surfaceInfo, isGPU);
        ...
        for (BitmapInit bmi = kFirstBitmapInit; bmi <= kLast_BitmapInit; bmi = nextBMI(bmi)) {
            for (size_t c = 0; c < SK_ARRAY_COUNT(gReadPixelsConfigs); ++c) {
                ...
                if (startsWithPixels) {
                    ...
                    bool expectSuccess = SkIRect::Intersects(srcRect, DEV_RECT) && read_should_succeed(bmp.info(), surfaceInfo, true);
                    if (success != expectSuccess) {
                        ERRORF(reporter, "Expected success: %d, success: %d", expectSuccess, success);
                        sContext->readPixels(bmp.info(), bmp.getPixels(), bmp.rowBytes(), srcRect.fLeft, srcRect.fTop, flags);
                        read_should_succeed(bmp.info(), surfaceInfo, true);
                    }
