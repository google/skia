                ...
                if (clippedSrcRect.contains(devx, devy)) {
                    if (checkSurfacePixels) {
                        uint8_t surfaceAlpha = (surfaceAlphaType == kOpaque_SkAlphaType)
                                               ? 0xFF : SkGetPackedA32(get_src_color(devx, devy));
                ...
                if (checkSurfacePixels) {
                    ...
                    if (!check_read_pixel(pmPixel, surfacePMColor, didPremul)) {
                        ERRORF(reporter, "Expected readback pixel (%d, %d) value 0x%08x, got 0x%08x. "
                               "Readback was unpremul: %d", bx, by, surfacePMColor, pmPixel, didPremul);
                        return false;
                    }
                }
            } else if (checkBitmapPixels) {
...
ReadSuccessExpectation read_should_succeed(const SkIRect& srcRect,  const SkImageInfo& dstInfo, const SkImageInfo& srcInfo, bool isGPU) {
    ...
    if (srcInfo.alphaType() == kUnpremul_SkAlphaType && dstInfo.alphaType() == kPremul_SkAlphaType) {
        return ReadSuccessExpectation::kNo;
    }
    // We don't currently require reading alpha-only surfaces to succeed because of some pessimistic caps
    // decisions and alpha/red complexity in GL.
    if (SkColorTypeIsAlphaOnly(srcInfo.colorType())) {
        return ReadSuccessExpectation::kMaybe;
    }
    if (!SkColorTypeIsAlwaysOpaque(srcInfo.colorType()) && SkColorTypeIsAlwaysOpaque(dstInfo.colorType())) {
        return ReadSuccessExpectation::kNo;
    }
    return ReadSuccessExpectation::kYes;
}
    ...
    for (size_t rect = 0; rect < SK_ARRAY_COUNT(gReadPixelsTestRects); ++rect) {
        ...
        for (BitmapInit bmi = kFirstBitmapInit; bmi <= lastBitmapInit; bmi = nextBMI(bmi)) {
            for (size_t c = 0; c < SK_ARRAY_COUNT(gReadPixelsConfigs); ++c) {
                ...
                REPORTER_ASSERT(reporter, check_success_expectation(expectSuccess, success), "Read did succeed: %d", success);
                ...
                if (success || startsWithPixels) {
                    check_read(reporter, bmp, srcRect.fLeft, srcRect.fTop,
                               success, startsWithPixels, surfaceInfo.alphaType());
                } else {
        ...
        for (BitmapInit bmi = kFirstBitmapInit; bmi <= kLast_BitmapInit; bmi = nextBMI(bmi)) {
            for (size_t c = 0; c < SK_ARRAY_COUNT(gReadPixelsConfigs); ++c) {
                ...
                if (startsWithPixels) {
                    fill_dst_bmp_with_init_data(&bmp);
                    uint32_t flags = 0;
                    // TODO: These two hacks can go away when the surface context knows the alpha type.
                    // Tell the read to perform an unpremul step.
                    if (gReadPixelsConfigs[c].fAlphaType == kUnpremul_SkAlphaType) {
                        flags = GrContextPriv::kUnpremul_PixelOpsFlag;
                    }
                    // The surface context doesn't know that the src is opaque. We don't support converting non-opaque
                    // data to opaque during a read.
                    if (bmp.alphaType() == kOpaque_SkAlphaType && surfaceInfo.alphaType() != kOpaque_SkAlphaType) {
                        continue;
                    }
                    bool success = sContext->readPixels(bmp.info(), bmp.getPixels(),
                                                        bmp.rowBytes(),
                                                        srcRect.fLeft, srcRect.fTop, flags);
                    auto expectSuccess = read_should_succeed(srcRect, bmp.info(), surfaceInfo, true);
                    REPORTER_ASSERT(reporter, check_success_expectation(expectSuccess, success), "Read did succeed: %d", success);
                    if (success) {
