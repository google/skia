...
static void gpu_read_pixels_test_driver(skiatest::Reporter* reporter,
    ...
    auto runTest = [&](const T& src, const SkPixmap& srcPixels, const SkImageInfo& readInfo,
                       const SkIVector& offset) -> GpuReadResult {
    ...
    const std::vector<SkIRect> bigRectList = {
        // entire thing
    ...
    const std::vector<SkIRect> smallRectList = {
        // entire thing
        SkIRect::MakeWH(kW, kH),
        // fully contained
        SkIRect::MakeLTRB(kW / 4, kH / 4, 3 * kW / 4, 3 * kH / 4),
    ...
    // We ensure we use the big list once per src and read color type and otherwise use the
    // small list to improve test run time.
    ...
    // after one iter.
    // We consider a src or read CT thoroughly tested once it has run through the big rect list
            ...
            for (int rct = 0; rct <= kLastEnum_SkColorType; ++rct) {
                ...
                for (const sk_sp<SkColorSpace>& readCS :
                    ...
                    for (int at = 0; at <= kLastEnum_SkAlphaType; ++at) {
                        ...
                        const std::vector<SkIRect>& rectList =
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(SurfaceContextReadPixels, reporter, ctxInfo) {
    ...
    for (auto renderable : {GrRenderable::kNo, GrRenderable::kYes}) {
        SkDebugf("Rend: %d", renderable);
        for (GrSurfaceOrigin origin : {kTopLeft_GrSurfaceOrigin, kBottomLeft_GrSurfaceOrigin}) {
            SkDebugf("Orig: %d", origin);
