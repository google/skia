...
static constexpr int alpha_channel_bits(SkColorType ct) {
    switch (ct) {
        ...
        case kGray_8_SkColorType:             return 0;   // counting gray as "rgb"
        case kRGBA_F16Norm_SkColorType:       return 10;  // just counting the mantissa
        case kRGBA_F16_SkColorType:           return 10;  // just counting the mantissa
        case kRGBA_F32_SkColorType:           return 23;  // just counting the mantissa
        case kR16G16B16A16_unorm_SkColorType: return 16;
...
struct GpuReadExpectations {
    // Test unpremul sources? We could omit this but it lets us skip generating ref data.
...
static void gpu_read_pixels_test_driver(skiatest::Reporter* reporter,
    ...
    auto runTest = [&](const T& src, const SkPixmap& srcPixels, const SkImageInfo& readInfo,
        ...
        static constexpr auto kInitialByte = static_cast<char>(0xAB);
        ...
        if (!SkIRect::Intersects(rect, surfBounds)) {
            ...
            REPORTER_ASSERT(reporter, !success);
        } else if (!expectations.fUncontainedRectSucceeds && !surfBounds.contains(rect)) {
            REPORTER_ASSERT(reporter, !success);
        } else if (!expectations.fAllowUnpremulRead && readAT == kUnpremul_SkAlphaType) {
            REPORTER_ASSERT(reporter, !success);
        } else if (!success) {
        ...
        if (success && srcReadRect.intersect(surfBounds, rect)) {
            ...
            int rgbBits =
                    std::min({min_rgb_channel_bits(readCT), min_rgb_channel_bits(srcCT)});
            if (srcCT == kRGBA_1010102_SkColorType) {
                rgbBits = std::min(rgbBits, 2);
            }
            ...
            auto error = std::function<ComparePixmapsErrorReporter>([&](int x, int y,
                ...
                ERRORF(reporter,
                       ...
                       ToolUtils::colortype_name(readCT), ToolUtils::alphatype_name(readAT),
                       rect.fLeft, rect.fTop, rect.fRight,
                       rect.fBottom, csConversion, x, y, diffs[0], diffs[1], diffs[2], diffs[3]);
            });
    ...
    for (int sat = 0; sat < kLastEnum_SkAlphaType; ++sat) {
        ...
        if (srcAT == kUnknown_SkAlphaType ||
            (srcAT == kUnpremul_SkAlphaType && !expectations.fAllowUnpremulSrc)) {
            continue;
        }
        // Make src data in F32 with srcAT. We will convert it to each color type we test to
        // initialize the src.
        const auto refInfo = SkImageInfo::Make(kW, kH, kRGBA_F32_SkColorType, srcAT,
                                               SkColorSpace::MakeSRGB());
        ...
        SkPaint paint;
        paint.setShader(SkGradientShader::MakeLinear(kPts1, kColors1, nullptr, 2, SkTileMode::kClamp));
        ...
        paint.setShader(SkGradientShader::MakeLinear(kPts2, kColors2, nullptr, 2, SkTileMode::kClamp));
                ...
                for (const sk_sp<SkColorSpace>& readCS :
                    ...
                    for (int at = 0; at <= kLastEnum_SkAlphaType; ++at) {
                        ...
                        for (const auto& rect : {
                            ...
                            const auto readInfo = SkImageInfo::Make(
                                    rect.width(), rect.height(), readCT, readAT, readCS);
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(AsyncReadPixels, reporter, ctxInfo) {
    ...
    auto reader = std::function<GpuReadSrcFn<Surface>>(
        [](const Surface& surface, const SkIVector& offset, const SkPixmap& pixels) {
            ...
            auto rect = SkIRect::MakeWH(pixels.width(), pixels.height()).makeOffset(offset);
    ...
    expectations.fAllowUnpremulSrc = false;
    expectations.fUncontainedRectSucceeds = false;
    expectations.fAllowUnpremulRead = false;
    ...
    for (GrSurfaceOrigin origin : {kTopLeft_GrSurfaceOrigin, kBottomLeft_GrSurfaceOrigin}) {
        auto factory = std::function<GpuSrcFactory<Surface>>(
            [context = ctxInfo.grContext(), origin](const SkPixmap& src) {
                ...
                auto surf = SkSurface::MakeRenderTarget(context, SkBudgeted::kYes, src.info(), 0, origin, nullptr);
                if (surf) {
                    surf->writePixels(src, 0, 0);
                }
                return surf;
        });
        gpu_read_pixels_test_driver(reporter, expectations, factory, reader);
    }
}
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(ReadPixels_Gpu, reporter, ctxInfo) {
    ...
    GpuReadExpectations expectations;
    expectations.fAllowUnpremulSrc = false;
    expectations.fUncontainedRectSucceeds = true;
    expectations.fAllowUnpremulRead = true;
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(ReadPixels_Texture, reporter, ctxInfo) {
    if (ctxInfo.type() != sk_gpu_test::GrContextFactory::kGL_ContextType) {
        return;
    }
    using SurfaceContext = std::unique_ptr<GrSurfaceContext>;
    auto reader = std::function<GpuReadSrcFn<SurfaceContext>>(
            [](const SurfaceContext& ctx, const SkIVector& offset, const SkPixmap& pixels) {
                return ctx->readPixels(pixels.info(), pixels.writable_addr(), pixels.rowBytes(), offset);
            });
    ...
    for (auto origin : {kBottomLeft_GrSurfaceOrigin, kTopLeft_GrSurfaceOrigin}) {
        for (auto renderable : {GrRenderable::kNo, GrRenderable::kYes}) {
            ...
            auto factory = std::function<GpuSrcFactory<SurfaceContext>>(
                    [context = ctxInfo.grContext(), origin, renderable](const SkPixmap& src) {
              if (src.colorType() == kRGB_888x_SkColorType) {
                  return SurfaceContext();
              }
              sk_sp<GrTextureProxy> proxy = sk_gpu_test::MakeTextureProxyFromData(
                      context, renderable, src.width(), src.height(), src.colorType(),
                      src.alphaType(), origin, src.addr(), src.rowBytes());
              if (!proxy) {
                  return SurfaceContext();
              }
              return context->priv().makeWrappedSurfaceContext(
                      std::move(proxy), SkColorTypeToGrColorType(src.colorType()),
                      src.alphaType());
