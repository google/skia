...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(AsyncReadPixels, reporter, ctxInfo) {
    ...
    for (auto origin : {kTopLeft_GrSurfaceOrigin, kBottomLeft_GrSurfaceOrigin}) {
        ...
        static const auto surfaceCS = SkColorSpace::MakeSRGB();
        static const auto otherCS = SkColorSpace::MakeSRGBLinear();
        for (auto csConversion : {false, true}) {
            for (int c = 0; c <= kLastEnum_SkColorType; ++c) {
                for (const auto& rect :
                    ...
                    auto info = SkImageInfo::Make(kW, kH, ct, kPremul_SkAlphaType, surfaceCS);
                    auto surf = SkSurface::MakeRenderTarget(ctxInfo.grContext(), SkBudgeted::kNo,
                                                            info, 1, origin, nullptr);
                    if (!surf) {
                        continue;
                    }
                    ...
                    auto readCS = csConversion ? otherCS : surfaceCS;
                    info = SkImageInfo::Make(rect.width(), rect.height(), ct, kPremul_SkAlphaType,
                                             readCS);
                    pixmap.alloc(info);
                    surf->asyncReadPixels(ct, kPremul_SkAlphaType, nullptr, rect, callback,
                                          &context);
                    ...
                    if (context.fSuceeded) {
                        ...
                        ref.alloc(info);
                        surf->readPixels(ref, rect.fLeft, rect.fTop);
