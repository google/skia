...
void test(skiatest::Reporter* r, const char* src, float* in, float* expected,
    ...
    if (program) {
        ...
        int returnCount = main->getReturnSlotCount();
        SkSL::Interpreter<1> interpreter(std::move(byteCode));
        SkSL::ByteCode::Vector<1>* result;
        bool success = interpreter.run(main, (SkSL::ByteCode::Vector<1>*) in, nullptr, &result);
...
void vec_test(skiatest::Reporter* r, const char* src) {
    ...
    SkAssertResult(interpreter4.runStriped(main4, 4, (SkSL::ByteCode::Vector<4>**) args, nullptr,
                   nullptr));
...
void test(skiatest::Reporter* r, const char* src, float inR, float inG, float inB, float inA,
    ...
    if (program) {
        ...
        SkSL::Interpreter<1> interpreter(std::move(byteCode));
        bool success = interpreter.run(main, inoutColor, nullptr, nullptr);
        REPORTER_ASSERT(r, success);
...
DEF_TEST(SkSLInterpreterTEMP, r) {
    test(r, "void main(inout half4 color) { color.r = 9; }", 0.25, 0.75, 0, 0, 9, 0.75, 0, 0);
}
...
DEF_TEST(SkSLInterpreterCompound, r) {
...

    {
        ...
        bool success = interpreter.run(rect_height, (SkSL::Interpreter<1>::Vector*) &in, nullptr,
                                       &out);
        REPORTER_ASSERT(r, success);
...

    {
        int in[2] = { 15, 25 };
        SkSL::Interpreter<1>::Vector* out;
        bool success = interpreter.run(make_blue_rect, (SkSL::Interpreter<1>::Vector*) in, nullptr,
                                       &out);
        REPORTER_ASSERT(r, success);
...

    {
        ...
        SkSL::Interpreter<1>::Vector* out;
        bool success = interpreter.run(median, (SkSL::Interpreter<1>::Vector*) in, nullptr, &out);
        REPORTER_ASSERT(r, success);
...

    {
        ...
        SkSL::Interpreter<1>::Vector* out;
        bool success = interpreter.run(sums, (SkSL::Interpreter<1>::Vector*) in, nullptr, &out);
        REPORTER_ASSERT(r, success);
        for (int i = 0; i < 8; ++i) {
    ...
    {
        int in = 2;
        interpreter.setUniforms((SkSL::Interpreter<1>::Vector*) fRects);
        SkSL::Interpreter<1>::Vector* out;
        bool success = interpreter.run(get_rect, (SkSL::Interpreter<1>::Vector*) &in, nullptr,
                                       &out);
        REPORTER_ASSERT(r, success);
...
DEF_TEST(SkSLInterpreterFunctions, r) {
    ...
    SkSL::Interpreter<1>::Vector* out;
    bool success = interpreter.run(main, (SkSL::Interpreter<1>::Vector*) &in, nullptr, &out);
    REPORTER_ASSERT(r, success);
    REPORTER_ASSERT(r, out->fFloat[0] = 6.0f);

    success = interpreter.run(dot3, (SkSL::Interpreter<1>::Vector*) &in, nullptr, &out);
    REPORTER_ASSERT(r, success);
    REPORTER_ASSERT(r, out->fFloat[0] = 9.0f);

    success = interpreter.run(dot2, (SkSL::Interpreter<1>::Vector*) &in, nullptr, &out);
    REPORTER_ASSERT(r, success);
...
DEF_TEST(SkSLInterpreterExternalValues, r) {
    ...
    if (program) {
        ...
        SkSL::Interpreter<1> interpreter(std::move(byteCode));
        SkSL::ByteCode::Vector<1>* result;
        bool success = interpreter.run(main, nullptr, nullptr, &result);
        REPORTER_ASSERT(r, success);
...
DEF_TEST(SkSLInterpreterExternalValuesVector, r) {
    ...
    if (program) {
        ...
        const SkSL::ByteCodeFunction* main = byteCode->getFunction("main");
        SkSL::Interpreter<1> interpreter(std::move(byteCode));
        bool success = interpreter.run(main, nullptr, nullptr, nullptr);
        REPORTER_ASSERT(r, success);
