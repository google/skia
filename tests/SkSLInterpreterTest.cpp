...
DEF_TEST(SkSLInterpreterExternalValues, r) {
    ...
    const char* src = "float main() {"
                      ...
                      "}";
    auto jsonValue = std::make_unique<JSONExternalValue>("root", &dom.root(), &compiler);
    int32_t outValue = -1;
    auto pointerValue = std::make_unique<PointerExternalValue>(
            "outValue", *compiler.context().fInt_Type, &outValue, sizeof(outValue));
    std::vector<SkSL::ExternalValue*> valuesToRegister = { jsonValue.get(), pointerValue.get() };
    std::unique_ptr<SkSL::Program> program = compiler.convertProgram(
            SkSL::Program::kGeneric_Kind, SkSL::String(src), settings, &valuesToRegister);
    REPORTER_ASSERT(r, program);
    if (program) {
        std::unique_ptr<SkSL::ByteCode> byteCode = compiler.toByteCode(*program);
        REPORTER_ASSERT(r, !compiler.errorCount());
...
DEF_TEST(SkSLInterpreterExternalValuesVector, r) {
    ...
    auto externalValue = std::make_unique<PointerExternalValue>(
            "value", *compiler.context().fInt4_Type, value, sizeof(value));
    std::vector<SkSL::ExternalValue*> valuesToRegister = {externalValue.get()};
    std::unique_ptr<SkSL::Program> program = compiler.convertProgram(
            SkSL::Program::kGeneric_Kind, SkSL::String(src), settings, &valuesToRegister);
    REPORTER_ASSERT(r, program);
    if (program) {
...
DEF_TEST(SkSLInterpreterExternalValuesCall, r) {
    ...
    const char* src = "float main() {"
                      "    return external(25);"
                      "}";
    auto externalValue = std::make_unique<FunctionExternalValue>(
...
DEF_TEST(SkSLInterpreterExternalValuesVectorCall, r) {
    ...
    const char* src =
            ...
            "}";
    auto externalValue = std::make_unique<VectorFunctionExternalValue>(
            "external",
