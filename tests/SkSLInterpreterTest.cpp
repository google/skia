...
void test(skiatest::Reporter* r, const char* src, void* in, int expectedCount, void* expected) {
    ...
    if (program) {
        ...
        SkSL::Interpreter::Run(byteCode.get(), main, in, out.get(), 1, nullptr, 0);
        ...
        if (!valid) {
            ...
            for (int i = 0; i < expectedCount; ++i) {
                printf("%s%f", separator, ((float*)expected)[i]);
                separator = ", ";
            }
            printf("), but received (");
            separator = "";
...
void vec_test(skiatest::Reporter* r, const char* src) {
    ...
    for (int i = 0; i < 4; ++i) {
        SkSL::Interpreter::Run(byteCode.get(), main, out_s + i * 4, nullptr, 1, nullptr, 0);
    }

    // Now run in parallel and compare results
    SkSL::Interpreter::Run(byteCode.get(), main, out_v, nullptr, 4, nullptr, 0);
...
void test(skiatest::Reporter* r, const char* src, float inR, float inG, float inB, float inA,
    ...
    if (program) {
        ...
        SkSL::Interpreter::Run(byteCode.get(), main, inoutColor, nullptr, 1, nullptr, 0);
...
DEF_TEST(SkSLInterpreterCompound, r) {
...

    {
        ...
        int out = 0;
        SkSL::Interpreter::Run(byteCode.get(), rect_height, &in, &out, 1, gRects, 16);
...

    {
        int in[2] = { 15, 25 };
        RectAndColor out;
        SkSL::Interpreter::Run(byteCode.get(), make_blue_rect, in, &out, 1, gRects, 16);
...

    {
        ...
        int out = 0;
        SkSL::Interpreter::Run(byteCode.get(), median, in, &out, 1, gRects, 16);
...

    {
        ...
        float out[8] = { 0 };
        SkSL::Interpreter::Run(byteCode.get(), sums, in, out, 1, gRects, 16);
        for (int i = 0; i < 8; ++i) {
...

    {
        ...
        SkSL::Interpreter::Run(byteCode.get(), get_rect, &in, &out, 1, gRects, 16);
...

    {
        ...
        in.fNumRects = 2;
        SkSL::Interpreter::Run(byteCode.get(), fill_rects, &in, nullptr, 1, gRects, 16);
        ManyRects expected;
...
DEF_TEST(SkSLInterpreterFunctions, r) {
    ...
    float out = 0.0f;
    float in = 3.0f;
    SkSL::Interpreter::Run(byteCode.get(), main, &in, &out, 1, nullptr, 0);
    REPORTER_ASSERT(r, out = 6.0f);

    SkSL::Interpreter::Run(byteCode.get(), dot3, &in, &out, 1, nullptr, 0);
    REPORTER_ASSERT(r, out = 9.0f);

    SkSL::Interpreter::Run(byteCode.get(), dot2, &in, &out, 1, nullptr, 0);
    REPORTER_ASSERT(r, out = -1.0f);

    int fibIn = 6;
    int fibOut = 0;
    SkSL::Interpreter::Run(byteCode.get(), fib, &fibIn, &fibOut, 1, nullptr, 0);
...
DEF_TEST(SkSLInterpreterExternalValues, r) {
    ...
    if (program) {
        ...
        SkSL::ByteCodeFunction* main = byteCode->fFunctions[0].get();
        float out;
        SkSL::Interpreter::Run(byteCode.get(), main, nullptr, &out, 1, nullptr, 0);
...
DEF_TEST(SkSLInterpreterExternalValuesVector, r) {
    ...
    if (program) {
        ...
        SkSL::ByteCodeFunction* main = byteCode->fFunctions[0].get();
        SkSL::Interpreter::Run(byteCode.get(), main, nullptr, nullptr, 1, nullptr, 0);
