...
DEF_TEST(SkSLInterpreterCompound, r) {
...

    {
        ...
        int out = 0;
        byteCode->run(rect_height, &in, &out, 1, gRects, 16);
...

    {
        int in[2] = { 15, 25 };
        RectAndColor out;
        byteCode->run(make_blue_rect, in, &out, 1, gRects, 16);
...

    {
        ...
        int out = 0;
        byteCode->run(median, in, &out, 1, gRects, 16);
...

    {
        ...
        float out[8] = { 0 };
        byteCode->run(sums, in, out, 1, gRects, 16);
        for (int i = 0; i < 8; ++i) {
...

    {
        ...
        byteCode->run(get_rect, &in, &out, 1, gRects, 16);
...

    {
        ...
        in.fNumRects = 2;
        byteCode->run(fill_rects, &in, nullptr, 1, gRects, 16);
        ManyRects expected;
...
DEF_TEST(SkSLInterpreterFunctions, r) {
    ...
    int fibIn = 6;
    int fibOut = 0;
    byteCode->run(fib, &fibIn, &fibOut, 1, nullptr, 0);
