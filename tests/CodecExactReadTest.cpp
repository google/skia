...
DEF_TEST(Codec_end, r) {
    for (const char* path : { "plane.png",
        ...
        SkMemoryStream stream(std::move(multiData));
        {
            ...
            if (!codec) {
                ERRORF(r, "Failed to create SkAndroidCodec from %s", path);
                continue;
            }

            SkAndroidCodec::AndroidOptions opts;
            opts.fSampleSize = 2;
            auto sampledSize = codec->getSampledDimensions(opts.fSampleSize);
            auto info = codec->getInfo().makeColorType(kN32_SkColorType)
                    .makeWH(sampledSize.fWidth, sampledSize.fHeight);
            SkBitmap bm;
            bm.allocPixels(info);
            auto result = codec->getAndroidPixels(info, bm.getPixels(), bm.rowBytes(), &opts);
            if (result != SkCodec::kSuccess) {
                ERRORF(r, "Failed to getAndroidPixels from %s", path);
                continue;
            }
        }
        for (int i = 0; i < kNumImages - 1; ++i) {
