...
bool GrDrawingManager::ProgramUnitTest(GrContext* context, int maxStages, int maxLevels) {
    ...
    GrMipMapped mipMapped = GrMipMapped(context->priv().caps()->mipMapSupport());
    {
        ...
        proxies[0] = proxyProvider->createProxy(format, dummyDesc, GrRenderable::kYes,
                kBottomLeft_GrSurfaceOrigin, mipMapped, SkBackingFit::kExact, SkBudgeted::kNo,
    ...
    }
    {
        ...
        proxies[1] = proxyProvider->createProxy(format, dummyDesc, GrRenderable::kNo, kTopLeft_GrSurfaceOrigin,
