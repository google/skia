...
// We create Ops that write a value into a range in a buffer. We create ranges from
// 0..kNumOpPositions-1 starting positions using each entry in kRanges where the range offset is
// applied to the starting position. We repeat each range kNumRepeats times (with a different
// value written by each of the repeats).
namespace {
struct Range {
    int fOffset;
    int fLength;
...
static constexpr Range kRanges[] = {{-1, 4,}, {0, 2}};
...
static constexpr int result_width() {
    int maxLength = -1;
    for (size_t i = 0; i < kNumRanges; ++i) {
        maxLength = std::max(maxLength, kRanges[i].fLength);
    }
    return kNumOpPositions + maxLength - 1;
}

// Determines a position offset so all write inside the result buffer bounds.
static constexpr int position_offset() {
    int minStart = SK_MaxS32;
    for (size_t i = 0; i < kNumRanges; ++i) {
        minStart = std::min(minStart, kRanges[i].fOffset);
    }
    return -minStart;
}
    ...
    void writeResult(int result[]) const {
        for (const auto& op : ChainRange<TestOp>(this)) {
            for (const auto& vr : op.fValueRanges) {
                for (int i = 0; i < vr.fRange.fLength; ++i) {
...
DEF_GPUTEST(OpChainTest, reporter, /*ctxInfo*/) {
    ...
    static constexpr int kNumChainabilities = 100;
    SkRandom random;
    bool repeat = false;
    ...
    for (int p = 0; p < kNumPermutations; ++p) {
        ...
        for (int c = 0; c < kNumChainabilities; ++c) {
            ...
            for (int i = 0; i < kNumOps; ++i) {
                ...
                range.fOffset += pos + position_offset();
