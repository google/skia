...
// Use's Heap's algorithm to generate all permutations of ints 0 to kNumOps - 1.
class Permutations {
public:
    Permutations() {
        ...
        std::fill(fCounter, fCounter + kNumOps, 0);
    }

    const int* get() { return fArray; }
    ...
    bool next() {
        ...
        while (i < kNumOps) {
            if (fCounter[i] < i) {
                if (i & 0b1) {
                    std::swap(fArray[fCounter[i]], fArray[i]);
                } else {
                    std::swap(fArray[0], fArray[i]);
                }
                fCounter[i] += 1;
                i = 0;
                return true;
...
DEF_GPUTEST(OpChainTest, reporter, /*ctxInfo*/) {
    ...
    do {
        for (uint32_t chainabilityBits = 0; chainabilityBits < kNumChainabilityPermutations;
            ...
            for (int i = 0; i < kNumOps; ++i) {
                auto op = TestOp::Make(context.get(), p.get()[i], result, chainabilityBits);
