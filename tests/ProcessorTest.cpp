...
void test_draw_op(GrRenderTargetContext* rtc, sk_sp<GrFragmentProcessor> fp, GrTexture* inputDataTexture) {
    GrPaint paint;
    ...
    auto op = GrNonAAFillRectOp::Make(GrColor_WHITE, SkMatrix::I(), SkRect::MakeWH(rtc->width(), rtc->height()), nullptr, nullptr);
    ...
    for (int i = 0; i < FPFactory::Count(); ++i) {
        ...
        for (int j = 0; j < timesToInvokeFactory; ++j) {
            ...
            rtc->readPixels(
                    ...
                    data.get(), 0, 0, 0);
            bool passing = true;
            if (1) {  // Useful to see what FPs are being tested.
                SkString children;
