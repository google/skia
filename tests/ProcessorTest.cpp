...
DEF_GPUTEST_FOR_GL_RENDERING_CONTEXTS(ProcessorOptimizationValidationTest, reporter, ctxInfo) {
    ...
    for (int y = 0; y < 256; ++y) {
        for (int x = 0; x < 256; ++x) {
            rgbaData.get()[256 * y + x] = texel_color(random.nextULessThan(256),
    ...
    sk_sp<GrTexture> tex1(context->textureProvider()->createTexture(desc, SkBudgeted::kYes, alphaData.get(), 256));
    GrTexture* textures[] = {tex0.get(), tex1.get()};
    GrProcessorTestData testData(&random, context, rtc.get(), textures);

    // Use a different array of premul colors for the out of the fragment processor that preceeds
