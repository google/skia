...
// This assumes that the output buffer will be the same size as the render target context.
...
class TestFPGenerator {
    public:
        ...
        bool init() {
            ...
            constexpr int kTestTextureSize = 256;
        ...
        std::unique_ptr<GrFragmentProcessor> create(std::unique_ptr<GrFragmentProcessor> inputFP,
                                                    int type) {
...
static constexpr SkAlphaType kLogAlphaType = kUnpremul_SkAlphaType;
...
bool fuzzy_color_equals(const SkPMColor4f& c1, const SkPMColor4f& c2) {
    ...
    // too unforgiving).
    constexpr SkScalar kTolerance = 0.01f;
    for (int i = 0; i < 4; i++) {
...
DEF_GPUTEST_FOR_GL_RENDERING_CONTEXTS(ProcessorOptimizationValidationTest, reporter, ctxInfo) {
    ...
    // Create three input textures. Our coverage optimization checks use three frames with a
    // linearly transformed input texture. The first frame is a baseline; the second frame adds 0.2
    // and the third adds 0.4, which should then be present as a fixed difference between the frame
    // outputs if the FP is properly following the modulation requirements of the coverage
    // optimization.
    constexpr SkScalar kInputDelta = 0.2f;
    GrSurfaceProxyView inputTexture1 =
            make_input_texture(context, kRenderSize, kRenderSize, 0 * kInputDelta);
    GrSurfaceProxyView inputTexture2 =
            make_input_texture(context, kRenderSize, kRenderSize, 1 * kInputDelta);
    GrSurfaceProxyView inputTexture3 =
            make_input_texture(context, kRenderSize, kRenderSize, 2 * kInputDelta);

    std::unique_ptr<GrFragmentProcessor> inputTexEffect1FP =
            GrTextureEffect::Make(inputTexture1, kPremul_SkAlphaType);
    std::unique_ptr<GrFragmentProcessor> inputTexEffect2FP =
            GrTextureEffect::Make(inputTexture2, kPremul_SkAlphaType);
    std::unique_ptr<GrFragmentProcessor> inputTexEffect3FP =
            GrTextureEffect::Make(inputTexture3, kPremul_SkAlphaType);
    ...
    for (int i = 0; i < FPFactory::Count(); ++i) {
        ...
        std::unique_ptr<GrFragmentProcessor> fp = fpGenerator.create(/*inputFP=*/nullptr, i);
        ...
        for (int j = 0; j < timesToInvokeFactory; ++j) {
            ...
            if (fp->compatibleWithCoverageAsAlpha()) {
                ...
                std::unique_ptr<GrFragmentProcessor> fp2 =
                        fpGenerator.create(inputTexEffect2FP->clone(), i);
                std::unique_ptr<GrFragmentProcessor> fp3 =
                        fpGenerator.create(inputTexEffect3FP->clone(), i);

                render_fp(context, rtc.get(), std::move(fp2), readData2.data());
                render_fp(context, rtc.get(), std::move(fp3), readData3.data());
            }

            // Draw base frame last so that rtc holds the original FP behavior if we need to dump
            // the image to the log.
            render_fp(context, rtc.get(), fp->clone(), readData1.data());
            ...
            constexpr int kMaxAcceptableFailedPixels = 0;  // Strict when running as SKQP
#else
            constexpr int kMaxAcceptableFailedPixels = 2 * kRenderSize;  // ~0.7% of the image
#endif
            ...
            for (int y = 0; y < kRenderSize; ++y) {
                for (int x = 0; x < kRenderSize; ++x) {
                    ...
                    if (fp->compatibleWithCoverageAsAlpha()) {
                        GrColor ins[3];
                        ins[0] = input;
                        ins[1] = input_texel_color(x, y, 1 * kInputDelta);
...
// progenitor. If a non-deterministic bot fails this test, check the output to see what seed it
// used, then use --processorSeed <seed> (without --randomProcessorTest) to reproduce.
DEF_GPUTEST_FOR_GL_RENDERING_CONTEXTS(ProcessorCloneTest, reporter, ctxInfo) {
    ...
    // Create an input texture that will serve as the input for our effects.
    GrSurfaceProxyView inputTexture = make_input_texture(context, kRenderSize, kRenderSize, 0.0f);
    std::unique_ptr<GrFragmentProcessor> inputTexEffectFP =
            GrTextureEffect::Make(inputTexture, kPremul_SkAlphaType);
    ...
    for (int i = 0; i < GrFragmentProcessorTestFactory::Count(); ++i) {
        constexpr int kTimesToInvokeFactory = 10;
        for (int j = 0; j < kTimesToInvokeFactory; ++j) {
            fpGenerator.reroll();
            std::unique_ptr<GrFragmentProcessor> fp =
                    fpGenerator.create(inputTexEffectFP->clone(), i);
