...
class TestFPGenerator {
    public:
        ...
        std::unique_ptr<GrFragmentProcessor> create(int type) {
...
DEF_GPUTEST_FOR_GL_RENDERING_CONTEXTS(ProcessorOptimizationValidationTest, reporter, ctxInfo) {
    ...
    for (int i = 0; i < FPFactory::Count(); ++i) {
        ...
        std::unique_ptr<GrFragmentProcessor> fp = fpGenerator.create(i);
        ...
        for (int j = 0; j < timesToInvokeFactory; ++j) {
            ...
            if (fp->compatibleWithCoverageAsAlpha()) {
                ...
                render_fp(context, rtc.get(), fpGenerator.create(i), inputTexture2,
                          kPremul_SkAlphaType, readData2.data());
                render_fp(context, rtc.get(), fpGenerator.create(i), inputTexture3,
