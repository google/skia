...
static void describe_fp_children(const GrFragmentProcessor& fp, int indent, SkString* text) {
    for (int index = 0; index < fp.numChildProcessors(); ++index) {
        const GrFragmentProcessor& childFP = fp.childProcessor(index);
        text->appendf("\n%.*c(#%d) -> %s", indent, '\t', index, childFP.name());
        describe_fp_children(childFP, indent + 1, text);
    }
}
...
static SkString describe_fp(const GrFragmentProcessor& fp) {
    SkString text;
    text.printf("\n%s", fp.name());
    describe_fp_children(fp, /*indent=*/1, &text);
    return text;
}
