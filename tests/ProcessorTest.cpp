        ...
        for (int j = 0; j < kTimesToInvokeFactory; ++j) {
            ...
            for (int y = 0; y < kRenderSize && passing; ++y) {
                for (int x = 0; x < kRenderSize && passing; ++x) {
                    ...
                    if (failedPixelCount > kMaxAcceptableFailedPixels) {
                        // Regenerate the FP so we can get its description.
                        fp = fpGenerator.make(i, /*randomTreeDepth=*/3, /*inputFP=*/nullptr);

                        passing = false;
                        idx = firstWrongY * kRenderSize + firstWrongX;
                        ERRORF(reporter,
                               "A clone of processor %s produced different output at (%d, %d).\n"
                               ...
                               readData1[idx], readData2[idx], fpGenerator.initialSeed(),
                               describe_fp(*fp).c_str());
                        if (!loggedFirstFailure) {
