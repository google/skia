        ...
        for (int parentCnt = 0; parentCnt < 2; parentCnt++) {
                                                              ...
                                                              kRGBA_8888_GrPixelConfig, nullptr));
            {
                ...
                sk_sp<GrTextureProxy> proxy1 =
                        proxyProvider->createProxy(desc, SkBackingFit::kExact, SkBudgeted::kYes, 0);
...
#else
        GrMipLevel rgbaMipLevel = { rgbaData.get(), kTestTextureSize * sizeof(GrColor) };
        proxies[0] = proxyProvider->createTextureProxy(desc, SkBudgeted::kYes, rgbaMipLevel);
#endif
    }
...
#else
        GrMipLevel alphaMipLevel = { alphaData.get(), kTestTextureSize };
        proxies[1] = proxyProvider->createTextureProxy(desc, SkBudgeted::kYes, alphaMipLevel);
#endif
    }
...
#else
    GrMipLevel mipLevel = { data.get(), width * sizeof(GrColor) };
