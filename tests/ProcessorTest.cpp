...
static GrColor4f texel_color4f(int i, int j) { return GrColor4f::FromGrColor(input_texel_color(i, j)); }
...
/** Initializes the two test texture proxies that can are available to the FP test factories. */
bool init_test_textures(GrContext* context, SkRandom* random, sk_sp<GrTextureProxy> proxies[2]) {
    ...
    for (int y = 0; y < kTestTextureSize; ++y) {
        for (int x = 0; x < kTestTextureSize; ++x) {
            rgbaData.get()[kTestTextureSize * y + x] =
    ...
    for (int y = 0; y < kTestTextureSize; ++y) {
        for (int x = 0; x < kTestTextureSize; ++x) {
            alphaData.get()[kTestTextureSize * y + x] = random->nextULessThan(256);
        }
    }
...
DEF_GPUTEST_FOR_GL_RENDERING_CONTEXTS(ProcessorOptimizationValidationTest, reporter, ctxInfo) {
    ...
    // Make the destination surface for the test.
    ...
    // Create an texture of premul colors used as the output of the fragment processor that precedes
    // the fragment processor under test.
    std::unique_ptr<GrColor[]> rgbaData(new GrColor[256 * 256]);
    for (int y = 0; y < 256; ++y) {
        for (int x = 0; x < 256; ++x) {
            rgbaData.get()[256 * y + x] = input_texel_color(x, y);
        }
    }
    GrSurfaceDesc desc;
    desc.fOrigin = kBottomLeft_GrSurfaceOrigin;
...
DEF_GPUTEST_FOR_GL_RENDERING_CONTEXTS(ProcessorCloneTest, reporter, ctxInfo) {
    ...
    static const int kRenderSize = 1024;
    sk_sp<GrRenderTargetContext> rtc = context->makeDeferredRenderTargetContext(
    ...
    // Create an texture of premul colors used as the output of the fragment processor that precedes
    // the fragment processor under test.
    std::unique_ptr<GrColor[]> rgbaData(new GrColor[kRenderSize * kRenderSize]);
    for (int y = 0; y < kRenderSize; ++y) {
        for (int x = 0; x < kRenderSize; ++x) {
            rgbaData.get()[kRenderSize * y + x] = input_texel_color(x, y);
        }
    }
    GrSurfaceDesc desc;
    desc.fOrigin = kBottomLeft_GrSurfaceOrigin;
    desc.fWidth = kRenderSize;
    desc.fHeight = kRenderSize;
    desc.fConfig = kRGBA_8888_GrPixelConfig;
    sk_sp<GrTextureProxy> dataProxy = GrSurfaceProxy::MakeDeferred(context->resourceProvider(),
                                                                   desc, SkBudgeted::kYes,
                                                                   rgbaData.get(),
                                                                   kRenderSize * sizeof(GrColor));
    std::unique_ptr<GrColor[]> rgbaData2(new (GrColor[kRenderSize * kRenderSize]));
    ...
    for (int i = 0; i < FPFactory::Count(); ++i) {
        ...
        for (int j = 0; j < timesToInvokeFactory; ++j) {
            fp = FPFactory::MakeIdx(i, &testData);
            auto clone = fp->clone();
            // Currently clone can fail currently.
            if (!clone) {
                continue;
            }
            if (!fp->instantiate(context->resourceProvider()) ||
            ...
            test_draw_op(rtc.get(), fp, dataProxy);
            memset(rgbaData.get(), 0x0, sizeof(GrColor) * kRenderSize * kRenderSize);
            rtc->readPixels(
                    SkImageInfo::Make(kRenderSize, kRenderSize, kRGBA_8888_SkColorType, kPremul_SkAlphaType),
                    rgbaData.get(), 0, 0, 0);
            test_draw_op(rtc.get(), clone, dataProxy);
            memset(rgbaData2.get(), 0x0, sizeof(GrColor) * kRenderSize * kRenderSize);
            rtc->readPixels(
                    SkImageInfo::Make(kRenderSize, kRenderSize, kRGBA_8888_SkColorType, kPremul_SkAlphaType),
                    rgbaData2.get(), 0, 0, 0);
            bool passing = true;
            for (int y = 0; y < kRenderSize && passing; ++y) {
                for (int x = 0; x < kRenderSize && passing; ++x) {
                    ...
                    if (rgbaData.get()[idx] != rgbaData2.get()[idx]) {
                        ERRORF(reporter,
                               "Processor %s made clone produced different output. "
                               "Input color: 0x%08x, Original Output Color: 0x%08x, Clone Output Color: 0x%08x..",
                               fp->name(), input_texel_color(x, y), rgbaData[idx], rgbaData2[idx]);
                        passing = false;
                    }
                }
            }
