...
void render_fp(GrContext* context, GrRenderTargetContext* rtc, GrFragmentProcessor* fp,
    ...
    rtc->readPixels(
            SkImageInfo::Make(inputDataProxy->size(), kRGBA_8888_SkColorType, kPremul_SkAlphaType),
            buffer, 0, {0, 0});
}
...
bool log_texture_proxy(GrContext* context, sk_sp<GrTextureProxy> src, SkString* dst) {
    ...
    SkImageInfo ii = SkImageInfo::Make(src->size(), kRGBA_8888_SkColorType, kLogAlphaType);
    SkBitmap bm;
