            ...
            for (int y = 0; y < 256 && passing; ++y) {
                for (int x = 0; x < 256 && passing; ++x) {
                    ...
                    if (fp->hasConstantOutputForConstantInput(input4f, &expected4f)) {
                        float rDiff = std::abs(output4f.fRGBA[0] - expected4f.fRGBA[0]);
                        float gDiff = std::abs(output4f.fRGBA[1] - expected4f.fRGBA[1]);
                        float bDiff = std::abs(output4f.fRGBA[2] - expected4f.fRGBA[2]);
                        float aDiff = std::abs(output4f.fRGBA[3] - expected4f.fRGBA[3]);
