    ...
    for (int i = 0; i < FPFactory::Count(); ++i) {
        ...
        for (int trial = 0;; ++trial) {
            ...
            if (trial >= kMinimumTrials) {
                ...
                if (!moreTrialsNeeded) {
                    SkDebugf("%25s: succeeded after %d trials\n", fp->name(), trial);
                    SkDebugf("   Preserves opaque input: %s %d trials\n",
                             optimizedForOpaqueInput ? "passed" : "unobserved in",
                             optimizedForOpaqueInput);
                    SkDebugf(" Coverage as alpha compat: %s %d trials\n",
                             optimizedForCoverageAsAlpha ? "passed" : "unobserved in",
                             optimizedForCoverageAsAlpha);
                    SkDebugf("Constant output for input: %s %d trials\n\n",
                             optimizedForConstantOutputForInput ? "passed" : "unobserved in",
