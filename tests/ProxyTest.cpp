...
static void check_texture(skiatest::Reporter* reporter,
    ...
    // 'when-allocated' reports the original WxH. If it is kApprox, make sure that the texture
    // is that size and didn't reuse one of the kExact surfaces in the provider. This is important
    // because upstream usage (e.g. SkImage) reports size based on the when-allocated dimensions
    // and client code may rely on that if they are creating backend resources.
    // NOTE: we store these before instantiating, since after instantiation dimensionsWhenAllocated
    ...
    SkISize expectedSize = texProxy->dimensionsWhenAllocated();
