        ...
        for (auto colorType : { kAlpha_8_SkColorType, kRGBA_8888_SkColorType,
            ...
            for (auto numSamples : {1, 4}) {
                ...
                if (!supportedNumSamples) {
                    SkASSERT(beFormat.isValid());
                    SkDebugf("GrBackend: %d\n", (uint32_t)beFormat.backend());
                    SkDebugf("colorType: %d, numSamples: %d, config: %d\n",
                             grColorType, numSamples, config);
#ifdef SK_METAL
                    if (beFormat.getMtlFormat()) {
                        SkDebugf("mtl format: %d\n", *beFormat.getMtlFormat());
                    }
#endif
                    if (beFormat.getGLFormat()) {
                        SkDebugf("gl format: %d\n", *beFormat.getGLFormat());
                    }
                }
                SkASSERT(supportedNumSamples);
