...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(WrappedProxyTest, reporter, ctxInfo) {
    ...
    for (auto origin : { kBottomLeft_GrSurfaceOrigin, kTopLeft_GrSurfaceOrigin }) {
        for (auto colorType : { kAlpha_8_SkColorType, kRGBA_8888_SkColorType,
            ...
            GrPixelConfig config1 = GrColorTypeToPixelConfig(grColorType);
            SkASSERT(kUnknown_GrPixelConfig != config1);
            ...
            for (auto numSamples : {1, 4}) {
                int supportedNumSamples = caps.getRenderTargetSampleCount(numSamples, config1);
                ...
                if (GrBackendApi::kOpenGL == ctxInfo.backend()) {
                    ...
                    backendRT.setPixelConfig1(config1);
                    sk_sp<GrSurfaceProxy> sProxy(
                            proxyProvider->wrapBackendRenderTarget(backendRT, grColorType,
                                                                   origin, nullptr, nullptr));
