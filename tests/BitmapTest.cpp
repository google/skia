...
static void check_alphas(skiatest::Reporter* reporter, const SkBitmap& bm,
    ...
    for (int y = 0; y < 16; ++y) {
        for (int x = 0; x < 16; ++x) {
            ...
            bool success = pred(expected, actual);
            if (!success) {
                SkDebugf("getalphaf failed: expected %g actual %g colortype %d\n",
                         expected, actual, bm.colorType());
                printf("p:getalphaf failed: expected %g actual %g colortype %d\n",
                         expected, actual, bm.colorType());
                fprintf(stderr, "e:getalphaf failed: expected %g actual %g colortype %d\n",
                         expected, actual, bm.colorType());
            }
            REPORTER_ASSERT(reporter, success);
            alpha += 1;
