...
static void test_basic_draw_as_src(skiatest::Reporter* reporter, GrContext* context,
    ...
    for (auto filter : {GrSamplerParams::kNone_FilterMode,
        ...
        test_read_pixels1(reporter, rtContext.get(), expectedPixelValues,
...
static void test_clear(skiatest::Reporter* reporter, GrSurfaceContext* rectContext) {
    if (GrRenderTargetContext* rtc = rectContext->asRenderTargetContext()) {
        ...
        test_read_pixels1(reporter, rtc, expectedPixels.get(), "RectangleTexture-clear");
    }
}
...
DEF_GPUTEST_FOR_GL_RENDERING_CONTEXTS(RectangleTexture, reporter, ctxInfo) {
    ...
    for (int origin = 0; origin < 2; ++origin) {
        ...
        test_read_pixels1(reporter, rectContext.get(), refPixels, "RectangleTexture-read");

        test_copy_to_surface1(reporter, context->resourceProvider(),
                              rectContext.get(), "RectangleTexture-copy-to");

        test_write_pixels1(reporter, rectContext.get(), true, "RectangleTexture-write");
