...
static void bulk_fill_rect_create_test(skiatest::Reporter* reporter, GrContext* context,
    ...
    for (int i = 0; i < requestedTotNumQuads; ++i) {
        ...
        quads[i].fLocalMatrix;
        quads[i].fAAFlags = gimmeSomeAA(i);
    }

    GrPaint paint;

    auto op = GrFillRectOp::MakeSet(context, std::move(paint), overallAA, SkMatrix::I(),
                                    quads, requestedTotNumQuads);
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(BulkFillRect, reporter, ctxInfo) {
