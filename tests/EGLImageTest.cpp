...
DEF_GPUTEST_FOR_GL_RENDERING_CONTEXTS(EGLImageTest, reporter, ctxInfo) {
    ...
    sk_sp<GrSurfaceContext> surfaceContext = context0->contextPriv().makeBackendSurfaceContext(
            backendTex, kDefault_GrSurfaceOrigin, kNone_GrBackendTextureFlag, 0, nullptr);
    ...
    // Should not be able to wrap as a RT
    {
        sk_sp<GrSurfaceContext> temp = context0->contextPriv().makeBackendSurfaceContext(
            backendTex, kDefault_GrSurfaceOrigin, kRenderTarget_GrBackendTextureFlag, 0, nullptr);
        if (temp) {
    ...
    {
        sk_sp<GrSurfaceContext> temp = context0->contextPriv().makeBackendSurfaceContext(
            backendTex, kDefault_GrSurfaceOrigin, kNone_GrBackendTextureFlag, 4, nullptr);
        if (temp) {
