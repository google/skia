...
DEF_GPUTEST_FOR_GL_RENDERING_CONTEXTS(EGLImageTest, reporter, ctxInfo) {
    ...
    GrTextureProxy* texProxy = surfaceContext->asTextureProxy();
    SkASSERT(texProxy->texPriv().doesNotSupportMipMaps());
    SkASSERT(texProxy->priv().peekTexture()->surfacePriv().doesNotSupportMipMaps());

    SkASSERT(texProxy->texPriv().isClampOnly());
    SkASSERT(texProxy->priv().peekTexture()->surfacePriv().isClampOnly());
