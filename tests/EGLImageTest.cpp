...
static void cleanup(GLTestContext* glctx0, GrGLuint texID0, GLTestContext* glctx1,
                    sk_sp<GrContext> grctx1, GrBackendTexture* grbackendtex,
                    GrEGLImage image1) {
    if (glctx1) {
        glctx1->makeCurrent();
        if (grctx1) {
            if (grbackendtex) {
                ...
                gpu1->deleteTestingOnlyBackendTexture(grbackendtex, false);
            }
        }
...
DEF_GPUTEST_FOR_GL_RENDERING_CONTEXTS(EGLImageTest, reporter, ctxInfo) {
    ...
    if (!gpu1->isTestingOnlyBackendTexture1(backendTexture1)) {
