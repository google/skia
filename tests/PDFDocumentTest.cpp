...
size_t count_bytes(const SkBitmap& bm, bool useDCT) {
    ...
    if (useDCT) {
        doc = SkDocument::MakePDF(&stream, SkDocument::kDefaultiPdfRasterPixelsPerInch,
...
DEF_TEST(SkPDF_pdfa_document, r) {
    ...
    auto doc = SkDocument::MakePDF(&buffer, SkDocument::kDefaultiPdfRasterPixelsPerInch,
    ...
    doc = SkDocument::MakePDF(&buffer, SkDocument::kDefaultiPdfRasterPixelsPerInch,
                              pdfMetadata, nullptr, /* pdfa = */ true);
    doc->beginPage(64, 64)->drawColor(SK_ColorRED);
    doc->close();
