...
DEF_TEST(SkPDF_unicode_metadata, r) {
    ...
    SkDocument::PDFMetadata pdfMetadata;
    pdfMetadata.fTitle   = "𝓐𝓑𝓒𝓓𝓔𝓕𝓖𝓗𝓘𝓙"; // Out of basic multilingual plane
    pdfMetadata.fAuthor  = "ABCDEFGHIJ"; // ASCII
    pdfMetadata.fSubject = "αβγδεζηθικ"; // inside  basic multilingual plane
    pdfMetadata.fPDFA = true;
    ...
    static const char* expectations[] = {
        "<</Title <FEFFD835DCD0D835DCD1D835DCD2D835DCD3D835DCD4D835DCD5D835DCD"
            "6D835DCD7D835DCD8D835DCD9>",
        "/Author (ABCDEFGHIJ)",
        "Subject <FEFF03B103B203B303B403B503B603B703B803B903BA>",
    };
    for (const char* expectation : expectations) {
        if (!contains(data->bytes(), data->size(), expectation)) {
