...
static constexpr SkColor gr_color_to_sk_color(GrColor c) {
    return SkColorSetARGBMacro(GrColorUnpackA(c), GrColorUnpackR(c), GrColorUnpackG(c),
                               GrColorUnpackB(c));
}
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(GrPipelineDynamicStateTest, reporter, ctxInfo) {
    ...
    for (ScissorState scissorState : {ScissorState::kEnabled, ScissorState::kDisabled}) {
        ...
        rtc->readPixels(SkImageInfo::MakeN32Premul(kScreenSize, kScreenSize),
        ...
        for (int y = 0; y < kScreenSize; ++y) {
            for (int x = 0; x < kScreenSize; ++x) {
                ...
                uint32_t expected = gr_color_to_sk_color(kMeshColors[expectedColorIdx]);
