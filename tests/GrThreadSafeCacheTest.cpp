...
private:
    ...
    void onPrePrepare(GrRecordingContext* rContext,
        ...
        if (!fVertexData) {
            // This is now a noop bc this is called in the ctor but is where we would normally
            // create the vertices.
            this->findOrCreateVertices(rContext);
        }
    }
...
void TestHelper::addVertAccess(SkCanvas* canvas,
    ...
    rtc->priv().testingOnly_addDrawOp(GrThreadSafeVertexTestOp::Make(canvas->recordingContext(),
                                                                     &fStats, wh, id));
}
