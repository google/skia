...
public:
    ...
    static std::unique_ptr<GrDrawOp> Make(GrRecordingContext* rContext, int wh, int id) {
        GrOpMemoryPool* pool = rContext->priv().opMemoryPool();

        return pool->allocate<GrMeshTestOp>(wh, id);
    }

private:
    friend class GrOpMemoryPool; // for ctor

    GrMeshTestOp(int wh, int id)
            : INHERITED(ClassID())
            , fWH(wh)
            , fID(id) {
    ...
    void findOrCreateVertices(GrRecordingContext* rContext) {
        ...
        verts[1].set(fWH, 0.0f);
        verts[2].set(0.0f, fWH);
        verts[3].set(fWH, fWH);
    ...
    void onPrePrepare(GrRecordingContext* rContext,
        ...
        this->findOrCreateVertices(rContext);
    }
    ...
    void onPrepare(GrOpFlushState* flushState) override {
        if (!fVertexData) {
            ...
            this->findOrCreateVertices(dContext);
        }

        if (!fVertexData->gpuBuffer()) {
            auto rp = flushState->resourceProvider();
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(GrThreadSafeCache1, reporter, ctxInfo) {
    ...
    helper.addViewAccess(helper.ddlCanvas1(), kImageWH, 1);
    REPORTER_ASSERT(reporter, helper.checkView(helper.ddlCanvas1(), kImageWH,
                                               /*hits*/ 0, /*misses*/ 1, /*refs*/ 1, /*id*/ 1));

    helper.addViewAccess(helper.ddlCanvas2(), kImageWH, 2);
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(GrThreadSafeCache2, reporter, ctxInfo) {
    ...
    helper.addViewAccess(helper.liveCanvas(), kImageWH, 1);
    REPORTER_ASSERT(reporter, helper.checkView(helper.liveCanvas(), kImageWH,
                                               /*hits*/ 0, /*misses*/ 1, /*refs*/ 1, /*id*/ 1));

    helper.addViewAccess(helper.ddlCanvas1(), kImageWH, 2);
    REPORTER_ASSERT(reporter, helper.checkView(helper.ddlCanvas1(), kImageWH,
                                               /*hits*/ 1, /*misses*/ 1, /*refs*/ 2, /*id*/ 1));

    helper.addViewAccess(helper.ddlCanvas2(), kImageWH, 3);
    REPORTER_ASSERT(reporter, helper.checkView(helper.ddlCanvas2(), kImageWH,
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(GrThreadSafeCache3, reporter, ctxInfo) {
    ...
    helper.addViewAccess(helper.liveCanvas(), kImageWH, 2);
    REPORTER_ASSERT(reporter, helper.checkView(helper.liveCanvas(), kImageWH,
                                               /*hits*/ 1, /*misses*/ 1, /*refs*/ 2, /*id*/ 1));
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(GrThreadSafeCache4, reporter, ctxInfo) {
    ...
    helper.addViewAccess(helper.ddlCanvas2(), kImageWH, 2, kFailLookup);
    REPORTER_ASSERT(reporter, helper.checkView(helper.ddlCanvas2(), kImageWH,
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(GrThreadSafeCache4_5, reporter, ctxInfo) {
    ...
    static const bool kFailLookup = true;
    helper.addViewAccess(helper.ddlCanvas1(), kImageWH, 2, kFailLookup);
    REPORTER_ASSERT(reporter, helper.checkView(helper.ddlCanvas1(), kImageWH,
                                               /*hits*/ 0, /*misses*/ 2, /*refs*/ 2, /*id*/ 1));
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(GrThreadSafeCache4_75, reporter, ctxInfo) {
    ...
    helper.addViewAccess(helper.liveCanvas(), kImageWH, kNoID, false, kFailFillingIn);
    REPORTER_ASSERT(reporter, helper.checkView(helper.liveCanvas(), kImageWH,
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(GrThreadSafeCache5, reporter, ctxInfo) {
    ...
    int size = 16;
    helper.addViewAccess(helper.ddlCanvas1(), size, /*id*/ size);
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(GrThreadSafeCache7, reporter, ctxInfo) {
    ...
    helper.addViewAccess(helper.ddlCanvas2(), 2*kImageWH);
    sk_sp<SkDeferredDisplayList> ddl2 = helper.snap2();
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(GrThreadSafeCache8, reporter, ctxInfo) {
    ...
    helper.addViewAccess(helper.liveCanvas(), kImageWH);
    REPORTER_ASSERT(reporter, helper.checkView(helper.liveCanvas(), kImageWH,
                                               /*hits*/ 0, /*misses*/ 1, /*refs*/ 1, kNoID));
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(GrThreadSafeCache10, reporter, ctxInfo) {
    ...
    helper.addViewAccess(helper.liveCanvas(), 2*kImageWH);
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(GrThreadSafeCache13View, reporter, ctxInfo) {
    test_13(ctxInfo.directContext(), reporter,
            &TestHelper::addViewAccess,
            &TestHelper::checkView);
}

DEF_GPUTEST_FOR_RENDERING_CONTEXTS(GrThreadSafeCache13Verts, reporter, ctxInfo) {
    test_13(ctxInfo.directContext(), reporter,
            &TestHelper::addVertAccess,
...
Add 2 verts
  -- findable
  -- don't appear in resource cache
  -- they purge as expected (i.e., MRU, survive w/ refs, go away on full purges)
  -- they work w/ DDLs/external refs
  -- drawing frees them up as expected
  -- they draw correctly


  -- works when interleaved w/ views
#endif
