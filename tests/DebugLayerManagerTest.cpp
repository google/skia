...
static void test_basic(skiatest::Reporter* reporter) {
  ...
  dlm.setFrame(0);
  dlm.storeSkPicture(node, picture1, dirtyRectFull);
  dlm.setFrame(10);
  dlm.storeSkPicture(node, picture2, dirtyRectPartial);
  auto frames = dlm.listFramesForNode(node);

  // Confirm the layer mangager stored these at the right places.
  ...
  for (int i=0; i<10; i++) {
    dlm.setFrame(i);
    auto image = dlm.getLayerAsImage(node);
  ...
  for (int i=10; i<12; i++) {
    ...
    REPORTER_ASSERT(reporter, image->height() == layerHeight);
    image->peekPixels(&pixmap);
    auto paintColor = pixmap.getColor(50, 50);
    REPORTER_ASSERT(reporter, paintColor == SK_ColorGREEN);
    auto c = pixmap.getColor(10, 50);
    SkDebugf("side color was (%d, %d, %d, %d)\n",
      SkColorGetA(c), SkColorGetR(c), SkColorGetG(c), SkColorGetB(c));
    REPORTER_ASSERT(reporter, paintColor == SK_ColorBLUE);
  }
