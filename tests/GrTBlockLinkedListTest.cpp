...
struct C {
    ...
    // Under the hood, GrTBlockLinkedList and GrBlockAllocator round up to max_align_t. If 'C' was
...
template<int N>
static void check_allocator_helper(GrTBlockLinkedList<C, N>* allocator, int cnt, int popCnt,
...
template<int N>
static void check_iterator_helper(GrTBlockLinkedList<C, N>* allocator,
                                  const std::vector<C*>& expected,
                                  skiatest::Reporter* reporter) {
    const GrTBlockLinkedList<C, N>* cAlloc = allocator;
...
template<int N>
static void check_allocator(GrTBlockLinkedList<C, N>* allocator, int cnt, int popCnt,
                            skiatest::Reporter* reporter) {
...
template<int N>
static void run_allocator_test(GrTBlockLinkedList<C, N>* allocator, skiatest::Reporter* reporter) {
...
static void run_concat_test(skiatest::Reporter* reporter, int aCount, int bCount) {
    ...
    GrTBlockLinkedList<C, N1> listA;
    GrTBlockLinkedList<C, N2> listB;
    ...
    // GrTBlockLinkedList guarantees the moved list is empty, but clang-tidy doesn't know about it;
...
static void run_concat_trivial_test(skiatest::Reporter* reporter, int aCount, int bCount) {
    ...
    GrTBlockLinkedList<D, N1> listA;
    GrTBlockLinkedList<D, N2> listB;
...
static void run_reserve_test(skiatest::Reporter* reporter) {
    ...
    GrTBlockLinkedList<C, N> list(kItemsPerBlock);
    ...
    // Because GrTBlockLinkedList normally allocates blocks in fixed sizes, and extraReservedCount >
