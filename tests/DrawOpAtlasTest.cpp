...
void GrDrawOpAtlas::setMaxPages_TestingOnly(int maxPages) {
...
static bool fill_plot(GrDrawOpAtlas* atlas,
    ...
    SkIPoint16 loc;
    bool result = atlas->addToAtlas1(resourceProvider, atlasID, target, kPlotSize, kPlotSize,
...
// add and remove pages. Note that this is similating flush-time behavior.
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(DrawOpAtlas2, reporter, ctxInfo) {
    ...
    auto rtc =  context->makeDeferredRenderTargetContext(SkBackingFit::kApprox, 32, 32,
    ...
    std::unique_ptr<GrDrawOp> op = textContext->foo_test(context, textContext,
                                                         rtc.get(), paint, text, 16, 16);
