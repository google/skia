...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(BasicDrawOpAtlas, reporter, ctxInfo) {
    ...
    GrDrawOpAtlas::Foo plotFoos[kNumPlots * kNumPlots];
    for (int i = 0; i < kNumPlots * kNumPlots; ++i) {
        bool result = fill_plot(
                atlas.get(), resourceProvider, &uploadTarget, &plotFoos[i], i * 32);
    ...
    GrDrawOpAtlas::Foo plotFoo;
    bool result = fill_plot(atlas.get(), resourceProvider, &uploadTarget, &plotFoo, 4 * 32);
    REPORTER_ASSERT(reporter, result);
    check(reporter, atlas.get(), 2, 4, 2);

    // Simulate a lot of draws using only the first plot. The last texture should be compacted.
    for (int i = 0; i < 512; ++i) {
        atlas->setLastUseToken(plotFoos[0].fPlotLocator, uploadTarget.tokenTracker()->nextDrawToken());
