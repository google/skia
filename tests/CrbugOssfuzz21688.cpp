...
DEF_TEST(crbug_ossfuzz_21688_struct, reporter) {
    // Tests that an array of zero-sized structs can be compiled without crashing.
    SkSL::Compiler compiler;
    SkSL::String output;
    SkSL::Program::Settings settings;
    sk_sp<GrShaderCaps> caps = SkSL::ShaderCapsFactory::Default();
    settings.fCaps = caps.get();
    const char* const kProgramText = "struct S {}; void main() { S x[2] = S[2](S(), S()); }";
    std::unique_ptr<SkSL::Program> program = compiler.convertProgram(SkSL::Program::kFragment_Kind,
                                                                     kProgramText, settings);
    REPORTER_ASSERT(reporter, program == nullptr);
    REPORTER_ASSERT(reporter, compiler.errorText().find("cannot construct 'S'") != -1);
}
