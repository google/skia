...
DEF_TEST(filtering, r) {
    for (auto* name : { "images/portrait.jpg",
        ...
        using FilterQuality = std::pair<SkFilterOptions, const char*>;
        for (FilterQuality quality : {
                FilterQuality{{SkSamplingMode::kNearest, SkMipmapMode::kNone},   "nearest"},
                FilterQuality{{SkSamplingMode::kLinear,  SkMipmapMode::kNone},   "bilinear"},
                FilterQuality{{SkSamplingMode::kLinear,  SkMipmapMode::kLinear}, "mipmaps"},
        }) {
            auto shader = cropped->makeShader(SkTileMode::kDecal, SkTileMode::kDecal,
                    quality.first);
            if (!shader) {
                ERRORF(r, "Failed to make %s shader from %s\n", quality.second, name);
                continue;
            }
            paint.setShader(std::move(shader));
            save(croppedSize, scaledSize, paint, baseName, quality.second, r);
        }
