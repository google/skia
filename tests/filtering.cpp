...
DEF_TEST(filtering, r) {
    for (auto* name : { "images/portrait.jpg",
        ...
        auto mipmaps = cropped->makeShader(SkTileMode::kDecal, SkTileMode::kDecal,
                { SkSamplingMode::kLinear, SkMipmapMode::kLinear });
        paint.setShader(std::move(mipmaps));
        save(croppedSize, scaledSize, paint, baseName, "mipmaps", r);
