...
public:
    ...
    sk_sp<SkSurface> createSurface(skiatest::Reporter* reporter);
...
bool VkYcbcrSamplerTestHelper::init(skiatest::Reporter* reporter) {
    ...
    if (!fExtensions.hasExtension(VK_KHR_SAMPLER_YCBCR_CONVERSION_EXTENSION_NAME, 1)) {
        ERRORF(reporter, "Ycbcr extension is not supported");
        return false;
    }
...
sk_sp<SkImage> VkYcbcrSamplerTestHelper::createI420Image(skiatest::Reporter* reporter) {
    ...
    void* mappedBuffer;
    if (fVkMapMemory(fDevice, fImageMemory, 0u, requirements.size, 0u, &mappedBuffer) != VK_SUCCESS) {
...
sk_sp<SkSurface> VkYcbcrSamplerTestHelper::createSurface(skiatest::Reporter* reporter) {
    sk_sp<SkSurface> surface = SkSurface::MakeRenderTarget(
            fGrContext.get(), SkBudgeted::kNo,
...
DEF_GPUTEST(VkYCbcrSampler_DrawImageWithYcbcrSampler, reporter, options) {
    ...
    sk_sp<SkSurface> surface = helper.createSurface(reporter);
    if (!srcImage) {
        return;
    }
