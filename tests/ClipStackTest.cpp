...
static void set_region_to_stack(const SkClipStack& stack, const SkIRect& bounds, SkRegion* region, bool debug=false) {
...
#undef IXYWH
        ...
        for (size_t i = 0; i < SK_ARRAY_COUNT(testCases); ++i) {
            ...
            SkASSERT(expectsScissor == reduced.hasScissor());
            if (expectsScissor) {
