...
static void test_reduced_clip_stack(skiatest::Reporter* reporter) {
    ...
    for (int i = 0; i < kNumTests; ++i) {
        ...
        if (!reduced->maskElements().isEmpty()) {
            REPORTER_ASSERT(reporter, reduced->hasScissor(), "%s", testCase.c_str());
            SkRect stackBounds;
            SkClipStack::BoundsType stackBoundsType;
            stack.getBounds(&stackBounds, &stackBoundsType);
            REPORTER_ASSERT(reporter, reduced->maskRequiresAA() == doAA, "%s", testCase.c_str());
        }
        ...
        REPORTER_ASSERT(reporter, region == reducedRegion, "%s", testCase.c_str());
...
static void test_aa_query(skiatest::Reporter* reporter, const SkString& testName,
    ...
    switch (expectedMethod) {
        case ClipMethod::kSkipDraw:
            ...
            REPORTER_ASSERT(reporter, reduced.maskElements().isEmpty(), "%s", testName.c_str());
            REPORTER_ASSERT(reporter,
        ...
        case ClipMethod::kScissor: {
            ...
            REPORTER_ASSERT(reporter, reduced.maskElements().isEmpty(), "%s", testName.c_str());
            REPORTER_ASSERT(reporter, reduced.hasScissor(), "%s", testName.c_str());
            REPORTER_ASSERT(reporter, expectedScissor == reduced.scissor(), "%s", testName.c_str());
            REPORTER_ASSERT(reporter,
                            GrReducedClip::InitialState::kAllIn == reduced.initialState(),
                            "%s", testName.c_str());
            return;
        ...
        case ClipMethod::kAAElements: {
            ...
            REPORTER_ASSERT(reporter, numExpectedElems == reduced.maskElements().count(),
                            "%s", testName.c_str());
            REPORTER_ASSERT(reporter, reduced.hasScissor(), "%s", testName.c_str());
            REPORTER_ASSERT(reporter, expectedClipIBounds == reduced.scissor(),
                            "%s", testName.c_str());
            REPORTER_ASSERT(reporter,
