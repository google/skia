...
// This test is used to make sure we currectly are tracking the current task index during the assign
// call in the GrResourceAllocator. Specifically we can fall behind if we have intervals that don't
// use the allocator. In this case we need to possible increment the fCurOpsTaskIndex multiple times
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(ResourceAllocatorCurOpsTaskIndexTest,
    ...
    // resource alloctor fCurOpsTaskIndex to fall behind what it really should be.
    GrBackendTexture backEndTex;
    ...
    // We want to trigger the previous proxy to do a intermediateFlush but if it is the
    // last interval we skip the intermediate flush call, so we add another interval here so it is
    // not skipped.
    ...
    // incremented its fCurOpsTaskIndex once instead of the needed two times to skip first two
    // unused intervals.
