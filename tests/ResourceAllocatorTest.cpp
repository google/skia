...
static void overlap_test(skiatest::Reporter* reporter, GrResourceProvider* resourceProvider,
    ...
    alloc.addInterval(p1, 0, 4, true);
    alloc.incOps();
    alloc.addInterval(p2, 1, 2, true);
    alloc.incOps();
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(ResourceAllocatorOverBudgetTest, reporter, ctxInfo) {
...

    {
        ...
        alloc.addInterval(p1, 0, 0, true);
        alloc.incOps();
        alloc.addInterval(p2, 1, 1, true);
        alloc.incOps();
        alloc.markEndOfOpList(0);

        alloc.addInterval(p3, 2, 2, true);
        alloc.incOps();
        alloc.addInterval(p4, 3, 3, true);
        alloc.incOps();
