...
sk_sp<GrSurfaceProxy> make_lazy(GrProxyProvider* proxyProvider, const GrCaps* caps,
    ...
    const GrBackendFormat format = caps->getBackendFormatFromColorType(p.fColorType);
    auto lazyType = deinstantiate ? GrSurfaceProxy::LazyInstantiationType ::kSingleUse
                                  : GrSurfaceProxy::LazyInstantiationType ::kUninstantiate;
    GrInternalSurfaceFlags flags =
            p.fIsRT ? GrInternalSurfaceFlags::kWindowRectsSupport : GrInternalSurfaceFlags::kNone;
