...
// Basic test that two proxies with overlapping intervals are assigned different
// GrSurfaces.
static void inset1(skiatest::Reporter* reporter, GrResourceProvider* resourceProvider) {
    GrSurfaceDesc desc;
...
DEF_GPUTEST_FOR_ALL_CONTEXTS(GrResourceAllocatorTest, reporter, ctxInfo) {
    GrResourceProvider* resourceProvider = ctxInfo.grContext()->resourceProvider();

    inset1(reporter, resourceProvider);
}