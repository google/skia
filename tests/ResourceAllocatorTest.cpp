...
static sk_sp<GrSurfaceProxy> make_deferred(GrProxyProvider* proxyProvider, const GrCaps* caps,
                                           const ProxyParams& p) {
    GrPixelConfig config = GrColorTypeToPixelConfig(p.fColorType1);
    ...
    const GrBackendFormat format = caps->getBackendFormatFromColorType(p.fColorType1);
...
static sk_sp<GrSurfaceProxy> make_backend(GrContext* context, const ProxyParams& p,
    ...
    SkColorType skColorType = GrColorTypeToSkColorType(p.fColorType1);
    ...
    *backendTex = context->createBackendTexture(p.fSize, p.fSize, skColorType,
    ...
    return proxyProvider->wrapBackendTexture(*backendTex, p.fColorType1, p.fOrigin,
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(LazyDeinstantiation, reporter, ctxInfo) {
    ...
    texParams.fColorType1 = GrColorType::kRGBA_8888;
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(ResourceAllocatorOverBudgetTest, reporter, ctxInfo) {
    ...
    const ProxyParams params  = { 64, false, GrColorType::kRGBA_8888,
