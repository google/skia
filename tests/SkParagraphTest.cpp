...
// Checked: DIFF (Skia select part of grapheme)
...
// Checked: DIFF (number of runs, line height rounding error)
DEF_TEST(SkParagraph_GetRectsForRangeStrut, reporter) {
...

    {
        ...
        REPORTER_ASSERT(reporter, SkScalarNearlyEqual(result[1].rect.right(), 118.605f, EPSILON50));
...
// TODO: Implement grapheme clusters
DEF_TEST(SkParagraph_EmojiMultiLineRectsParagraph, reporter) {
    SkDebugf("Failed: need to take in account grapheme clusters (in addition to glypheme clusters).\n");
    return;
    sk_sp<TestFontCollection> fontCollection = sk_make_sp<TestFontCollection>();
    if (!fontCollection->fontsFound()) return;
