...
SkScalar EPSILON100 = 0.01;
SkScalar EPSILON50 = 0.02;
SkScalar EPSILON20 = 0.05;
...
SkBitmap* getBitmap() {
    static SkBitmap* bits = nullptr;
    if (bits == nullptr) {
        bits = new SkBitmap();
    }
    return bits;
}
SkCanvas* getCanvas(bool clean = false) {
...
void drawRects(SkColor color, std::vector<TextBox>& result, bool fill = false) {
    ...
    for (auto& r : result) {
        getCanvas()->drawRect(r.rect, paint);
    }
}

void finalize(const char* testName) {
    getCanvas(true);
    SkString path = SkOSPath::Join("/usr/local/google/home/jlavrova/Temp/skia/", testName);
    SkFILEWStream file(path.c_str());
    if (!SkEncodeImage(&file, *getBitmap(), SkEncodedImageFormat::kPNG, 100)) {
...
DEF_TEST(SkParagraph_RainbowParagraph, reporter) {
    ...
    finalize("SkParagraph_RainbowParagraph.png");
}
...
DEF_TEST(SkParagraph_HeightOverrideParagraph, reporter) {
    ...
    /*
        ...
        std::vector<TextBox> boxes = paragraph->getRectsForRange(0, 0, rect_height_style,
       rect_width_style); for (auto& box : boxes) { canvas.drawRect(box.rect, paint);
        }
        ...
        REPORTER_ASSERT(reporter, boxes[1].rect.top() == 92.805778503417969);
        REPORTER_ASSERT(reporter, boxes[1].rect.right() == 43.84375);
        REPORTER_ASSERT(reporter, boxes[1].rect.bottom() == 165.4957885742187);
    */
}
...
DEF_TEST(SkParagraph_LeftAlignParagraph, reporter) {
    ...
    finalize("SkParagraph_LeftAlignParagraph.png");
}
...
DEF_TEST(SkParagraph_RightAlignParagraph, reporter) {
    ...
    getCanvas()->drawRect(SkRect::MakeXYWH(0, 0, paragraph->getMaxWidth(), paragraph->getHeight()), paint);
    ...
    REPORTER_ASSERT(reporter,
                    paragraph_style.getTextAlign() == impl->paragraphStyle().getTextAlign());
    finalize("SkParagraph_RightAlignParagraph.png");
}
...
DEF_TEST(SkParagraph_WavyDecorationParagraph, reporter) { SkDebugf("Not implemented yet.\n"); }
...
DEF_TEST(SkParagraph_ArabicRectsParagraph, reporter) {
    ...
    drawRects(SK_ColorRED, boxes);

    REPORTER_ASSERT(reporter, boxes.size() == 2ull);

    REPORTER_ASSERT(reporter, SkScalarNearlyEqual(boxes[0].rect.left(), 561.501953, EPSILON100));  // DIFF: 556.54688
    REPORTER_ASSERT(reporter, SkScalarNearlyEqual(boxes[0].rect.top(), -0.26855469, EPSILON100));
    ...
    REPORTER_ASSERT(reporter, SkScalarNearlyEqual(boxes[1].rect.left(), 538.548828, EPSILON100));
    REPORTER_ASSERT(reporter, SkScalarNearlyEqual(boxes[1].rect.top(), -0.26855469, EPSILON100));
    REPORTER_ASSERT(reporter, SkScalarNearlyEqual(boxes[1].rect.right(),  561.501953, EPSILON100));// DIFF: 556.54688
    REPORTER_ASSERT(reporter, SkScalarNearlyEqual(boxes[1].rect.bottom(), 44, EPSILON100));

    finalize("SkParagraph_ArabicRectsParagraph.png");
}
...
DEF_TEST(SkParagraph_ArabicRectsLTRLeftAlignParagraph, reporter) {
    ...
    REPORTER_ASSERT(reporter, SkScalarNearlyEqual(boxes[0].rect.left(), 396.174805, EPSILON100));  // DIFF: 89.40625
    REPORTER_ASSERT(reporter, SkScalarNearlyEqual(boxes[0].rect.top(), -0.26855469, EPSILON100));
    REPORTER_ASSERT(reporter, SkScalarNearlyEqual(boxes[0].rect.right(), 422.414062, EPSILON100)); // DIFF: 121.87891
    REPORTER_ASSERT(reporter, SkScalarNearlyEqual(boxes[0].rect.bottom(), 44, EPSILON100));

    REPORTER_ASSERT(reporter, SkScalarNearlyEqual(boxes[1].rect.left(), 422.414062, EPSILON100));
    REPORTER_ASSERT(reporter, SkScalarNearlyEqual(boxes[1].rect.top(), -0.26855469, EPSILON100));
    REPORTER_ASSERT(reporter, SkScalarNearlyEqual(boxes[1].rect.right(), 428.152344, EPSILON100));
    REPORTER_ASSERT(reporter, SkScalarNearlyEqual(boxes[1].rect.bottom(), 44, EPSILON100));

    finalize("SkParagraph_ArabicRectsLTRLeftAlignParagraph.png");
}
...
DEF_TEST(SkParagraph_ArabicRectsLTRRightAlignParagraph, reporter) {
    ...
    REPORTER_ASSERT(reporter, boxes.size() == 2ull);
    REPORTER_ASSERT(reporter, SkScalarNearlyEqual(boxes[0].rect.left(), 868.022461, EPSILON100));  // DIFF
    REPORTER_ASSERT(reporter, SkScalarNearlyEqual(boxes[0].rect.top(), -0.26855469, EPSILON100));
    REPORTER_ASSERT(reporter, SkScalarNearlyEqual(boxes[0].rect.right(), 894.261719, EPSILON100)); // DIFF
    REPORTER_ASSERT(reporter, SkScalarNearlyEqual(boxes[0].rect.bottom(), 44, EPSILON100));

    REPORTER_ASSERT(reporter, SkScalarNearlyEqual(boxes[1].rect.left(), 894.261719, EPSILON100));  // DIFF
    REPORTER_ASSERT(reporter, SkScalarNearlyEqual(boxes[1].rect.top(), -0.26855469, EPSILON100));
    REPORTER_ASSERT(reporter, SkScalarNearlyEqual(boxes[1].rect.right(), 900, EPSILON100));        // DIFF
    REPORTER_ASSERT(reporter, SkScalarNearlyEqual(boxes[1].rect.bottom(), 44, EPSILON100));

    finalize("SkParagraph_ArabicRectsLTRRightAlignParagraph.png");
}
...
DEF_TEST(SkParagraph_GetGlyphPositionAtCoordinateParagraph, reporter) {
    ...
    paragraph->layout(550);
    paragraph->paint(getCanvas(), 0, 0);
    finalize("SkParagraph_GetGlyphPositionAtCoordinateParagraph.png");
...
DEF_TEST(SkParagraph_GetRectsForRangeParagraph, reporter) {
    ...
    finalize("SkParagraph_GetRectsForRangeParagraph.png");
}
...
DEF_TEST(SkParagraph_GetRectsForRangeTight, reporter) {
    ...
    {
        auto result = paragraph->getRectsForRange(8, 21, heightStyle, widthStyle);
        drawRects(SK_ColorGREEN, result);
    ...
    finalize("SkParagraph_GetRectsForRangeTight.png");
}
...
DEF_TEST(SkParagraph_GetRectsForRangeIncludeLineSpacingMiddle, reporter) {
    ...
    }
    {
        ...
        drawRects(SK_ColorMAGENTA, result);
        ...
        REPORTER_ASSERT(reporter, SkScalarNearlyEqual(result[0].rect.right(), 547.875977f, EPSILON100)); // DIFF
        REPORTER_ASSERT(reporter, SkScalarNearlyEqual(result[0].rect.bottom(), 168.47331f, EPSILON100));

        REPORTER_ASSERT(reporter, SkScalarNearlyEqual(result[1].rect.left(), 547.875977f, EPSILON100));  // DIFF
        ...
        REPORTER_ASSERT(reporter, SkScalarNearlyEqual(result[2].rect.right(), 547.875977f, EPSILON100)); // DIFF
        REPORTER_ASSERT(reporter, SkScalarNearlyEqual(result[2].rect.bottom(), 248.47331f, EPSILON100));

        REPORTER_ASSERT(reporter, SkScalarNearlyEqual(result[3].rect.left(), 547.875977f, EPSILON100));  // DIFF
    ...
    {
        auto result = paragraph->getRectsForRange(19, 22, heightStyle, widthStyle);
        drawRects(SK_ColorBLACK, result);
    ...
    finalize("SkParagraph_GetRectsForRangeIncludeLineSpacingMiddle.png");
}
...
DEF_TEST(SkParagraph_GetRectsForRangeIncludeLineSpacingTop, reporter) {
    ...
    finalize("SkParagraph_GetRectsForRangeIncludeLineSpacingTop.png");
}
...
DEF_TEST(SkParagraph_GetRectsForRangeIncludeLineSpacingBottom, reporter) {
...

    {
        ...
        REPORTER_ASSERT(reporter, SkScalarNearlyEqual(result[0].rect.top(), 16.946615f, EPSILON100));
        REPORTER_ASSERT(reporter, SkScalarNearlyEqual(result[0].rect.right(), 17.4296889f, EPSILON100));
        REPORTER_ASSERT(reporter, SkScalarNearlyEqual(result[0].rect.bottom(), 96.946609, EPSILON100));
    }
    {
        auto result = paragraph->getRectsForRange(2, 8, heightStyle, widthStyle);
        drawRects(SK_ColorBLUE, result);
    ...
    }
    {
        ...
        REPORTER_ASSERT(reporter, SkScalarNearlyEqual(result[0].rect.left(), 190.00781f, EPSILON100));
        REPORTER_ASSERT(reporter, SkScalarNearlyEqual(result[0].rect.top(), 96.946609, EPSILON100));
        REPORTER_ASSERT(reporter, SkScalarNearlyEqual(result[0].rect.right(), 547.875977f, EPSILON100)); // DIFF
        REPORTER_ASSERT(reporter, SkScalarNearlyEqual(result[0].rect.bottom(), 176.94661, EPSILON100));

        REPORTER_ASSERT(reporter, SkScalarNearlyEqual(result[1].rect.left(), 547.875977f, EPSILON100));  // DIFF
        REPORTER_ASSERT(reporter, SkScalarNearlyEqual(result[1].rect.top(), 96.946609, EPSILON100));
        REPORTER_ASSERT(reporter, SkScalarNearlyEqual(result[1].rect.right(), 570.02344f, EPSILON20));
        REPORTER_ASSERT(reporter, SkScalarNearlyEqual(result[1].rect.bottom(), 176.94661, EPSILON100));

        REPORTER_ASSERT(reporter, SkScalarNearlyEqual(result[2].rect.left(), 0, EPSILON100));
        REPORTER_ASSERT(reporter, SkScalarNearlyEqual(result[2].rect.top(), 176.9466, EPSILON100));
        REPORTER_ASSERT(reporter, SkScalarNearlyEqual(result[2].rect.right(), 547.875977f, EPSILON100)); // DIFF
        REPORTER_ASSERT(reporter, SkScalarNearlyEqual(result[2].rect.bottom(), 256.94662, EPSILON100));

        REPORTER_ASSERT(reporter, SkScalarNearlyEqual(result[3].rect.left(), 547.875977f, EPSILON100));  // DIFF
        REPORTER_ASSERT(reporter, SkScalarNearlyEqual(result[3].rect.top(), 176.9466, EPSILON100));
        REPORTER_ASSERT(reporter, SkScalarNearlyEqual(result[3].rect.right(), 570.02344f, EPSILON20));
        REPORTER_ASSERT(reporter, SkScalarNearlyEqual(result[3].rect.bottom(), 256.94662, EPSILON100));

        REPORTER_ASSERT(reporter, SkScalarNearlyEqual(result[4].rect.left(), 0, EPSILON100));
        REPORTER_ASSERT(reporter, SkScalarNearlyEqual(result[4].rect.top(), 256.94662, EPSILON100));
        REPORTER_ASSERT(reporter, SkScalarNearlyEqual(result[4].rect.right(), 570.02344f, EPSILON20));
        REPORTER_ASSERT(reporter, SkScalarNearlyEqual(result[4].rect.bottom(), 336.94662, EPSILON100));

        REPORTER_ASSERT(reporter, SkScalarNearlyEqual(result[5].rect.left(), 0, EPSILON100));
        REPORTER_ASSERT(reporter, SkScalarNearlyEqual(result[5].rect.top(), 336.94662, EPSILON100));
        REPORTER_ASSERT(reporter, SkScalarNearlyEqual(result[5].rect.right(), 570.02344f, EPSILON20));
        REPORTER_ASSERT(reporter, SkScalarNearlyEqual(result[5].rect.bottom(), 416.94662, EPSILON100));
    }
    {
        auto result = paragraph->getRectsForRange(19, 22, heightStyle, widthStyle);
        drawRects(SK_ColorBLACK, result);
        ...
        REPORTER_ASSERT(reporter, SkScalarNearlyEqual(result[1].rect.top(), 16.946615f, EPSILON100));
        REPORTER_ASSERT(reporter, SkScalarNearlyEqual(result[1].rect.right(), 570.02344f, EPSILON20));
        REPORTER_ASSERT(reporter, SkScalarNearlyEqual(result[1].rect.bottom(), 96.946609, EPSILON100));
    }
    {
        auto result = paragraph->getRectsForRange(21, 21, heightStyle, widthStyle);
        REPORTER_ASSERT(reporter, result.empty());
    ...
    finalize("SkParagraph_GetRectsForRangeIncludeLineSpacingBottom.png");
}
...
DEF_TEST(SkParagraph_GetRectsForRangeCenterParagraph, reporter) {
    ...
    finalize("SkParagraph_GetRectsForRangeCenterParagraph.png");
}
...
DEF_TEST(SkParagraph_GetRectsForRangeCenterParagraphNewlineCentered, reporter) {
    ...
    finalize("SkParagraph_GetRectsForRangeCenterParagraphNewlineCentered.png");
}
...
DEF_TEST(SkParagraph_GetRectsForRangeCenterMultiLineParagraph, reporter) {
    ...
    {
        auto result = paragraph->getRectsForRange(5, 6, heightStyle, widthStyle);
        drawRects(SK_ColorYELLOW, result);
        REPORTER_ASSERT(reporter, result.size() == 1);
    ...
    finalize("SkParagraph_GetRectsForRangeCenterMultiLineParagraph.png");
}

DEF_TEST(SkParagraph_GetRectsForRangeStrut, reporter) { SkDebugf("Not implemented yet.\n"); }
...
DEF_TEST(SkParagraph_NewlineParagraph, reporter) {
    ...
    SkFILEWStream file("/usr/local/google/home/jlavrova/Temp/SkParagraph_NewlineParagraph.png");
...
DEF_TEST(SkParagraph_EmojiMultiLineRectsParagraph, reporter) { SkDebugf("Not implemented yet.\n"); }
...
DEF_TEST(SkParagraph_StrutDefaultParagraph, reporter) { SkDebugf("Not implemented yet.\n"); }
