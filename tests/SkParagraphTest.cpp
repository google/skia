...
DEF_TEST(SkParagraph_Ellipsize, reporter) {
    ...
    size_t index = 0;
    line.scanRuns(
       [&index, &line, reporter](TextLine::ClipContext runContext, SkScalar runOffset, TextRange textRange) {
        ++index;
        if (index == 2) {
            REPORTER_ASSERT(reporter, runContext.run->textRange() == line.ellipsis()->textRange());
        }
        return true;
    });
