...
static void check_mipmaps(GrContext* context, const GrBackendTexture& backendTex,
                          ...
                          skiatest::Reporter* reporter, const char* label, bool skip = false);
...
static void check_base_readbacks(GrContext* context, const GrBackendTexture& backendTex,
...

    {
        ...
        if (img) {
            ...
            SkDebugf("result of base check readpixels: %d\n", result);
            if (!result) {
            ...
            SkDebugf("Failed to get img for base readback.\n");
        }
    }
...
static void check_mipmaps(GrContext* context, const GrBackendTexture& backendTex,
    ...
    for (int i = 0, rectSize = 32; i < numMipLevels; ++i, rectSize /= 2) {
        ...
        check_solid_pixmap(reporter, expectedColors[i], actual2, skColorType,
            ...
            surf->flush(SkSurface::BackendSurfaceAccess::kNoAccess, info);
        }
    }
}
...
static void test_pixmap_init(GrContext* context, skiatest::Reporter* reporter,
    ...
    auto checkBackendTexture = [&](SkColor4f colors[6], bool skip) {
        if (!skip && mipMapped == GrMipMapped::kYes) {
    ...
    SkDebugf("checking create\n");
    checkBackendTexture(colors, true);
    //checkBackendTexture(colors, true);
    ...
    SkDebugf("checking update\n");
    checkBackendTexture(colorsNew, false);
