...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(ColorTypeBackendAllocationTest, reporter, ctxInfo) {
    ...
    for (auto colorType : colorTypes) {
        ...
        if (kRGB_565_SkColorType == colorType &&
            !caps->isConfigTexturable(kRGB_565_GrPixelConfig)) {
            continue;
        }
        if (kARGB_4444_SkColorType == colorType &&
            !caps->isConfigTexturable(kRGBA_4444_GrPixelConfig)) {
            continue;
        }
        if (kRGBA_1010102_SkColorType == colorType &&
            !caps->isConfigTexturable(kRGBA_1010102_GrPixelConfig)) {
            continue;
        }
...
DEF_GPUTEST_FOR_ALL_GL_CONTEXTS(GLBackendAllocationTest, reporter, ctxInfo) {
    ...
    for (auto combo : combinations) {
        ...
        if (GR_GL_BGRA8 == combo.fFormat &&
            !caps->isConfigTexturable(kBGRA_8888_GrPixelConfig)) {
            continue;
        }
        if (GR_GL_RGBA32F == combo.fFormat &&
            !caps->isConfigTexturable(kRGBA_float_GrPixelConfig)) {
            continue;
        }
        ...
        for (auto renderable : { GrRenderable::kNo, GrRenderable::kYes }) {
            ...
            if (GrRenderable::kYes == renderable) {
                ...
                if (GR_GL_SRGB8_ALPHA8 == combo.fFormat &&
                    !caps->isConfigRenderable(kSRGBA_8888_GrPixelConfig)) {
                    continue;
                }
            }
...
// This needs to be moved to a .mm file
#if 0
#ifdef SK_METAL

#include "src/gpu/mtl/GrMtlTypes.h"
