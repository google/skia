...
static void check_base_readbacks(GrDirectContext* dContext,
    ...
    for (GrRenderable renderableCtx : {GrRenderable::kNo, GrRenderable::kYes}) {
        ...
        auto swizzle =
                dContext->priv().caps()->getReadSwizzle(backendTex.getBackendFormat(), colorType);
        ...
        if (!surfaceContext->readPixels(dContext, actual.info(), actual.writable_addr(),
            ...
            auto name = SkStringPrintf("%s::readPixels", (renderableCtx == GrRenderable::kYes ? "GrSurfaceFillContext": "GrSurfaceContext"));
...
void test_color_init(GrDirectContext* dContext,
    ...
    auto checkBackendTexture = [&](const SkColor4f& testColor) {
        if (mipMapped == GrMipmapped::kYes) {
            ...
            check_mipmaps(dContext, backendTex, colorType, expectedColors,
                          reporter, "colorinit");
        }

        // The last step in this test will dirty the mipmaps so do it last
        check_base_readbacks(dContext, backendTex, colorType, renderable, testColor,
                             reporter, "colorinit");
    };
