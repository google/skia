...
static std::unique_ptr<SkImageGenerator> make_generator(const char* path, skiatest::Reporter* r) {
    ...
    if (data) {
        ...
        // No error for developers not using --resources
        ERRORF(r, "Could not find %s\n", path); // TODO: Remove
    }
    return nullptr;
}
...
DEF_TEST(NdkDecode, r) {
    ...
    for (auto& rec : recs) {
        ...
        REPORTER_ASSERT(r, gen->getPixels(unpremulInfo, bm.getPixels(), bm.rowBytes()));
    }
}
...
DEF_TEST(NdkDecode_reuseNoColorSpace, r) {
    static const struct {
        ...
        {"images/wide_gamut_yellow_224_224_64.jpeg", SkColorSpace::MakeSRGB()},
        // This image is SRGB, so convert to a different color space.
        {"images/example_1.png", SkColorSpace::MakeRGB(SkNamedTransferFn::k2Dot2, SkNamedGamut::kAdobeRGB)},
    };
    for (auto& rec : recs) {
        auto gen = make_generator(rec.fPath, r);
        if (!gen) continue;
        ...
        REPORTER_ASSERT(r, gen->getPixels(orig.info(), orig.getPixels(), orig.rowBytes()));

        SkBitmap corrected;
        corrected.allocPixels(noColorCorrection.makeColorSpace(rec.fCorrectedColorSpace));
        REPORTER_ASSERT(r, gen->getPixels(corrected.info(), corrected.getPixels(), corrected.rowBytes()));

        bool same = true;
        for (int y = 0; y < gen->getInfo().height(); y++) {
            auto* origLine      =      orig.getAddr32(0, y);
            auto* correctedLine = corrected.getAddr32(0, y);
        ...
        REPORTER_ASSERT(r, gen->getPixels(reuse.info(), reuse.getPixels(), reuse.rowBytes()));
...
DEF_TEST(NdkDecode_jpegDownscale, r) {
    ...
    for (auto& rec : recs) {
        ...
        for (SkISize dims : rec.fSupportedSizes) {
            ...
            SkBitmap bm;
            bm.allocPixels(info);
            if (!gen->getPixels(info, bm.getPixels(), bm.rowBytes())) {
...
DEF_TEST(NdkDecode_reuseJpeg, r) {
    ...
    REPORTER_ASSERT(r, gen->getPixels(downscaled.info(), downscaled.getPixels(), downscaled.rowBytes()));
...
DEF_TEST(NdkDecode_Gray8, r) {
    ...
    for (auto& rec : recs) {
        ...
        SkBitmap bm;
        bm.allocPixels(info);
        bool success = gen->getPixels(bm.info(), bm.getPixels(), bm.rowBytes());
...
DEF_TEST(NdkDecode_Opaque_and_565, r) {
    for (const char* path: {
        ...
        SkBitmap bm;
        bm.allocPixels(info);
        bool success = gen->getPixels(info, bm.getPixels(), bm.rowBytes());
        REPORTER_ASSERT(r, success == gen->getInfo().isOpaque());

        info = info.makeColorType(kRGB_565_SkColorType);
        bm.allocPixels(info);
        success = gen->getPixels(info, bm.getPixels(), bm.rowBytes());
        REPORTER_ASSERT(r, success == gen->getInfo().isOpaque());
    }
}
