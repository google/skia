...
DEF_TEST(SkVM_approx_math, r) {
    ...
    // asin, acos, atan
    {
        ...
        // atan2 -- check special cases first
        struct {
            float x, y, expected;
        } rec[] = {
            { 0, 1, SK_ScalarPI/2 },
        ...
        for (auto& r : rec) {
            SkASSERT(SkScalarNearlyEqual(r.expected, atan2(r.y,r.x)));
            test2(r.y, r.x, atan2(r.y,r.x), tol, [](skvm::F32 y, skvm::F32 x) {
