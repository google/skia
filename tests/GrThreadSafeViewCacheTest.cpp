...
public:
    ...
    void accessCachedView(SkCanvas* canvas,
        ...
        auto view = AccessCachedView(rContext, fBETex, wh, failLookup, &fStats);
        SkASSERT(view);
    ...
    bool checkView(SkCanvas* canvas, int wh, int hits, int misses, int numRefs) {
...

        {
            ...
            sk_sp<GrTextureProxy> result = proxyProvider->findProxyByUniqueKey(key);
            if (result) {
                return false;  // views in this cache should never appear in the recorder's cache
            }
        }
        ...
        if (view.proxy()->isInstantiated()) {
            ...
            if (result) {
                return false;  // views in this cache should never appear in the main cache
            }

            auto resourceProvider = fDContext->priv().resourceProvider();
            sk_sp<GrSurface> surf = resourceProvider->findByUniqueKey<GrSurface>(key);
            if (!surf) {
//                return false;  // if it is instantiated it had better be in the resource cache
            }
        }
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(GrThreadSafeViewCache1, reporter, ctxInfo) {
    ...
    /*auto first =*/ helper.accessCachedView(helper.ddlCanvas1(), kImageWH);
    REPORTER_ASSERT(reporter, helper.stats()->check(0, 1));
    //REPORTER_ASSERT(reporter, is_lazy(first));

    /*auto second =*/ helper.accessCachedView(helper.ddlCanvas2(), kImageWH);
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(GrThreadSafeViewCache2, reporter, ctxInfo) {
    ...
    /*auto first =*/ helper.accessCachedView(helper.liveCanvas(), kImageWH);
    REPORTER_ASSERT(reporter, helper.stats()->check(0, 1));
    //REPORTER_ASSERT(reporter, is_instantiated(first));

    /*auto second =*/ helper.accessCachedView(helper.ddlCanvas1(), kImageWH);
    REPORTER_ASSERT(reporter, helper.stats()->check(1, 1));

    /*auto third =*/ helper.accessCachedView(helper.ddlCanvas2(), kImageWH);

    helper.checkView(helper.ddlCanvas2(), kImageWH, /*hits*/ 2, /*misses*/ 1, /*refs*/ 2);
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(GrThreadSafeViewCache3, reporter, ctxInfo) {
    ...
    /*auto second =*/ helper.accessCachedView(helper.liveCanvas(), kImageWH);
    REPORTER_ASSERT(reporter, helper.stats()->check(1, 1));
    //REPORTER_ASSERT(reporter, first == second);
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(GrThreadSafeViewCache4, reporter, ctxInfo) {
    ...
    /*auto second =*/ helper.accessCachedView(helper.ddlCanvas2(), kImageWH, kFailLookup);
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(GrThreadSafeViewCache5, reporter, ctxInfo) {
    ...
    int size = 16;
    helper.accessCachedView(helper.liveCanvas(), size);

    size_t initialSize = threadSafeViewCache->approxBytesUsed();

    while (initialSize == threadSafeViewCache->approxBytesUsed()) {
        size *= 2;
        helper.accessCachedView(helper.liveCanvas(), size);
    }
}
