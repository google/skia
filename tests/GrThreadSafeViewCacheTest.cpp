...
public:
    struct Stats {
        ...
        bool fSmWrapped = false;
        bool fLgWrapped = false;
    };
    ...
    TestHelper(GrDirectContext* dContext) : fDContext(dContext) {
        ...
        SkAssertResult(CreateBackendTexture(fDContext, &fSmBETex, tmp));

        tmp = create_up_arrow_bitmap(2*kImageWH);
        SkAssertResult(CreateBackendTexture(fDContext, &fLgBETex, tmp));
    }

    ~TestHelper() {
        DeleteBackendTexture(fDContext, fSmBETex);
        DeleteBackendTexture(fDContext, fLgBETex);
    }
...
//        SkASSERT(wh == kImageWH || wh == 2*kImageWH);
    ...
    size_t gpuSize(int wh) const {
        return GrSurface::ComputeSize(*fDContext->priv().caps(), fSmBETex.getBackendFormat(),
...
private:
    ...
    GrBackendTexture fSmBETex;
    GrBackendTexture fLgBETex;
};
...
#if 0
    bool small = wh == kImageWH;

    if (small) {
        SkASSERT(!stats->fSmWrapped);
        stats->fSmWrapped = true;
    } else {
        SkASSERT(!stats->fLgWrapped);
        stats->fLgWrapped = true;;
    }
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(GrThreadSafeViewCache1, reporter, ctxInfo) {
    ...
    REPORTER_ASSERT(reporter, helper.checkView(helper.ddlCanvas1(), kImageWH, /*hits*/ 0, /*misses*/ 1, /*refs*/ 1));

    helper.accessCachedView(helper.ddlCanvas2(), kImageWH);
    REPORTER_ASSERT(reporter, helper.checkView(helper.ddlCanvas2(), kImageWH, /*hits*/ 1, /*misses*/ 1, /*refs*/ 2));
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(GrThreadSafeViewCache2, reporter, ctxInfo) {
    ...
    REPORTER_ASSERT(reporter, helper.checkView(helper.liveCanvas(), kImageWH, /*hits*/ 0, /*misses*/ 1, /*refs*/ 1));

    helper.accessCachedView(helper.ddlCanvas1(), kImageWH);
    REPORTER_ASSERT(reporter, helper.checkView(helper.ddlCanvas1(), kImageWH, /*hits*/ 1, /*misses*/ 1, /*refs*/ 2));

    helper.accessCachedView(helper.ddlCanvas2(), kImageWH);
    REPORTER_ASSERT(reporter, helper.checkView(helper.ddlCanvas2(), kImageWH, /*hits*/ 2, /*misses*/ 1, /*refs*/ 3));
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(GrThreadSafeViewCache3, reporter, ctxInfo) {
    ...
    helper.accessCachedView(helper.liveCanvas(), kImageWH);
    REPORTER_ASSERT(reporter, helper.checkView(helper.liveCanvas(), kImageWH, /*hits*/ 1, /*misses*/ 1, /*refs*/ 2));
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(GrThreadSafeViewCache4, reporter, ctxInfo) {
    ...
    REPORTER_ASSERT(reporter, helper.checkView(helper.ddlCanvas2(), kImageWH, /*hits*/ 0, /*misses*/ 2, /*refs*/ 2));
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(GrThreadSafeViewCache6, reporter, ctxInfo) {
    ...
    REPORTER_ASSERT(reporter, helper.checkView(nullptr, kImageWH, /*hits*/ 0, /*misses*/ 1, /*refs*/ 1));

    helper.accessCachedView(helper.ddlCanvas2(), kImageWH);
    sk_sp<SkDeferredDisplayList> ddl2 = helper.snap2();
    REPORTER_ASSERT(reporter, helper.checkView(nullptr, kImageWH, /*hits*/ 1, /*misses*/ 1, /*refs*/ 2));

    REPORTER_ASSERT(reporter, helper.numCacheEntries() == 1);

    ddl1 = nullptr;
    REPORTER_ASSERT(reporter, helper.checkView(nullptr, kImageWH, /*hits*/ 1, /*misses*/ 1, /*refs*/ 1));

    ddl2 = nullptr;
    REPORTER_ASSERT(reporter, helper.checkView(nullptr, kImageWH, /*hits*/ 1, /*misses*/ 1, /*refs*/ 0));
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(GrThreadSafeViewCache7, reporter, ctxInfo) {
    ...
    sk_sp<SkDeferredDisplayList> ddl2 = helper.snap2();
    REPORTER_ASSERT(reporter, helper.checkView(nullptr, 2*kImageWH, /*hits*/ 0, /*misses*/ 2, /*refs*/ 1));

    REPORTER_ASSERT(reporter, helper.numCacheEntries() == 2);

    helper.threadSafeViewCache()->dropAllUniqueRefs1(nullptr);
    REPORTER_ASSERT(reporter, helper.numCacheEntries() == 2);
    ...
    helper.threadSafeViewCache()->dropAllRefs1();
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(GrThreadSafeViewCache10, reporter, ctxInfo) {
    ...
    REPORTER_ASSERT(reporter, helper.checkView(helper.liveCanvas(), 2*kImageWH, /*hits*/ 1, /*misses*/ 2, /*refs*/ 1));

    helper.accessCachedView(helper.ddlCanvas2(), 2*kImageWH);
    sk_sp<SkDeferredDisplayList> ddl2 = helper.snap2();
    REPORTER_ASSERT(reporter, helper.checkView(helper.ddlCanvas2(), 2*kImageWH, /*hits*/ 2, /*misses*/ 2, /*refs*/ 2));
    ...
    REPORTER_ASSERT(reporter, helper.numCacheEntries() == 2);
    REPORTER_ASSERT(reporter, helper.checkView(helper.liveCanvas(), kImageWH, /*hits*/ 2, /*misses*/ 2, /*refs*/ 0));
    REPORTER_ASSERT(reporter, helper.checkView(helper.liveCanvas(), 2*kImageWH, /*hits*/ 2, /*misses*/ 2, /*refs*/ 0));
