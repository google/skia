...
static SkPath up_arrow_path(int wh) {
    float halfW = wh / 2.0f;
    float halfH = wh / 2.0f;
...
static SkBitmap create_up_arrow_bitmap(int wh) {
    SkBitmap bitmap;
...
bool TestHelper::FillInViewOnGpu(GrDirectContext* dContext, int wh, Stats* stats,
    ...
    rtc->drawPath(nullptr, std::move(paint), GrAA::kYes, SkMatrix::I(), path, GrStyle::SimpleFill());
