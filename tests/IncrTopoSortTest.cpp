...
// A queue adaptor for an SkTDArray
class NodeQueue {
public:
    NodeQueue() : fFront(0) {}

    int count() const {
        return fFoo.count() - fFront;
    }
...
public:
    ...
    bool addEdges(SkTDArray<Node*> dependedOn, Node* dependent) {
        ...
        // sort. This means that we will be proceeding from left to right in the sort when
    ...
    void shift(int index) {
        ...
        while (fQueue.count()) {
            ...
            if (node->visited()) {
                ...
                SkASSERT(!fNodes[index - numRemoved]);
                fNodes[index - numRemoved] = node;
                node->setIndexInSort(index - numRemoved);
            }

            Node* tmp = fQueue.top();
            while (fQueue.count() && node.get() == tmp) {
                numRemoved--;

                SkASSERT(!fNodes[index - numRemoved]);
                fNodes[index - numRemoved] = sk_ref_sp(tmp);
                tmp->setIndexInSort(index - numRemoved);
