...
static void upload_image_thread_proc(void* data) {
    ...
    ContextInfo info = ctx->fFactory.getSharedContextInfo(ctx->fContext);
    ctx->fCCID = SkCrossContextImageData::MakeFromEncoded(info.grContext(), ctx->fEncoded, nullptr);
...
DEF_GPUTEST(CrossContextImage_SharedContextOtherThread, reporter, /*factory*/) {
    ...
    for (auto format : { SkEncodedImageFormat::kPNG, SkEncodedImageFormat::kJPEG }) {
        ...
        for (int i = 0; i < GrContextFactory::kContextTypeCnt; ++i) {
            ...
            ContextInfo info = ctx.fFactory.getContextInfo(ctxType);
            if (!info.grContext()) {
                continue;
            }

            ctx.fContext = info.grContext();
            ctx.fEncoded = encoded;

            SkThread uploadThread(upload_image_thread_proc, &ctx);
            SkASSERT(uploadThread.start());
