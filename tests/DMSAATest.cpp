    ...
    for (int y = 0; y < h; ++y) {
        for (int x = 0; x < w; ++x) {
            if (!fuzzy_equals(pix, color)) {
                ERRORF(reporter, "SDC color mismatch at (%d, %d).\n"
                                 "Got      [%0.3f, %0.3f, %0.3f, %0.3f]\n"
                                 "Expected [%0.3f, %0.3f, %0.3f, %0.3f]",
                       x, y, pix[0], pix[1], pix[2], pix[3], color.fR, color.fG, color.fB, color.fA);
                //return;
            }
            pix += 4;
        }
...
DEF_GPUTEST_FOR_CONTEXTS(DMSAA_dst_read_with_existing_barrier,
    ...
    SkDebugf("can use dynamic msaa: %d\n", sdc->canUseDynamicMSAA());
    SkDebugf("can use tex barriers: %d\n", sdc->caps()->textureBarrierSupport());
    SkDebugf("can use msaa render to tex: %d\n", sdc->caps()->msaaResolvesAutomatically());

    SkDebugf("before initialize\n");
    // Initialize the texture and dmsaa attachment with transparent.
    draw_paint_with_dmsaa(sdc.get(), SK_PMColor4fTRANSPARENT, SkBlendMode::kSrc);
    check_sdc_color(reporter, sdc.get(), ctx, SK_PMColor4fTRANSPARENT);

    SkDebugf("before clear\n");
    sdc->clear(SK_PMColor4fWHITE);
    SkPMColor4f dstColor = SK_PMColor4fWHITE;

    SkDebugf("before draw 1\n");
   ...
   // draw_paint_with_aa(sdc.get(), kTransYellow, SkBlendMode::kSrcOver);
    dstColor = SkBlendMode_Apply(SkBlendMode::kDarken, kTransYellow, dstColor);

    SkDebugf("before added extra check\n");
    check_sdc_color(reporter, sdc.get(), ctx, dstColor);

    SkDebugf("before draw 2\n");
