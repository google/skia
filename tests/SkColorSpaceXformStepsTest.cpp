...
DEF_TEST(SkColorSpaceXformSteps, r) {
    ...
    for (uint32_t t = 0; t < 32; t++) {
        ...
        // There a a couple impossible outputs, so consider those bits tested.
        //
    ...
    // Make sure our extended constructor matches the old UnpremulToUnpremul().
    for (auto src : { srgb, adobe, srgb22, srgb1, adobe1 })
    for (auto dst : { srgb, adobe, srgb22, srgb1, adobe1 }) {
        SkColorSpaceXformSteps got{src.get(), unpremul,
                                   dst.get(), unpremul},
                              want = SkColorSpaceXformSteps::UnpremulToUnpremul(src.get(),
                                                                                dst.get());
        REPORTER_ASSERT(r, got.flags.mask() == want.flags.mask());
    }
}
