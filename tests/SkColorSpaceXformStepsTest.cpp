...
DEF_TEST(SkColorSpaceXformSteps, r) {
    ...
    struct {
        ...
        { srgb_N, srgb_N, kPremul_SkAlphaType,
        ...
        { srgb_L, srgb_L, kPremul_SkAlphaType,
        ...
        { srgb_L, srgb_N, kPremul_SkAlphaType,
            false,  // src is encoded as f(s*a),a, so we linearize before unpremul.
            true,   // Linearize,
            true,   // then unpremul.
        ...
        { srgb_N, srgb_L, kPremul_SkAlphaType,
            true,   // src is encoded as f(s)*a,a, so we unpremul to f(s),a before linearizing.
            true,   // Linearize to s,a.
            false,
        ...
        //    varying alpha type and non-linear blending bit (above is 1, 7 cases left).
        //
    ...
    for (auto t : tests) {
        ...
        if (t.srcAT == kOpaque_SkAlphaType) {
            SkColorSpaceXformSteps unpremul(t.src.get(), kUnpremul_SkAlphaType, t.dst.get());
            check_eq(r, unpremul, steps);
        }
    }
}
