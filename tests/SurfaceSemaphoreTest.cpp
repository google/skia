...
void surface_semaphore_test(skiatest::Reporter* reporter,
    ...
    GrBackendTexture backendTex = mainImage->getBackendTexture(false);

    draw_child(reporter, childInfo1, backendTex, semaphores[0]);

#ifdef SK_VULKAN
    if (const GrVkImageInfo* vkInfo = backendTex.getVkImageInfo()) {
        ...
        GrVkImageInfo* nonConstVKInfo = const_cast<GrVkImageInfo*>(vkInfo);
        nonConstVKInfo->updateImageLayout(VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
    }
#endif

    draw_child(reporter, childInfo2, backendTex, semaphores[1]);
}
