...
static GrOp::OpOwner create_op(GrDirectContext* dContext, SkRect rect,
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(TextureOpTest, reporter, ctxInfo) {
    ...
    GrOp::OpOwner opA = create_op(dContext, kOpARect, proxyViewA, false);
    GrOp::OpOwner opB = create_op(dContext, kOpBRect, proxyViewB, false);
    ...
    GrOp::OpOwner opC = create_op(dContext, kOpCRect, proxyViewC, true);
    GrOp::OpOwner opD = create_op(dContext, kOpDRect, proxyViewA, true);
