...
static void check_chain(GrOpsTask::OpChain* chain, SkRect firstRect, SkRect lastRect,
                        int expectedNumOps) {
    int actualNumOps = 0;
    for (const auto& op : GrOp::ChainRange<>(chain->head())) {
        ...
        if (actualNumOps == 1) {
            SkASSERT(op.bounds() == firstRect.makeOutset(0.5f, 0.5f));
        } else if (actualNumOps == expectedNumOps) {
            SkASSERT(op.bounds() == lastRect.makeOutset(0.5f, 0.5f));
        }
    }

    SkASSERT(actualNumOps == expectedNumOps);
}
...
// This unit tests exercises the crbug.com/1112259 case
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(TextureOpTest, reporter, ctxInfo) {
    ...
    // opA & opB can chain together but can't merge
    ...
    GrOpsTask::OpChain chain1(std::move(opA), GrProcessorSet::EmptySetAnalysis(), &noClip, nullptr);
    ...
    // opC & opD can also chain together but can't merge
    ...
    GrOpsTask::OpChain chain2(std::move(opC), GrProcessorSet::EmptySetAnalysis(), &noClip, nullptr);
