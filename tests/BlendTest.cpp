...
DEF_GPUTEST_FOR_GL_RENDERING_CONTEXTS(ES2BlendWithNoTexture, reporter, ctxInfo) {
    ...
    for (auto testCase : testCases) {
        ...
        SkColor outColor = bitmap.getColor(outPoint.x(), outPoint.y());
        SkString out_string = SkStringPrintf("%x", outColor);
        SkDebugf("out: %d :  %s\n", sampleCnt, out_string.c_str());
        REPORTER_ASSERT_MESSAGE(reporter, outColor == SkColorSetRGB(0xFF, 0xFF, 0x80), out_string);
        SkColor inColor = bitmap.getColor(inPoint.x(), inPoint.y());
        SkString in_string = SkStringPrintf("%x", inColor);
        SkDebugf("in: %d : %s\n", sampleCnt, in_string.c_str());
        REPORTER_ASSERT_MESSAGE(reporter, inColor == SkColorSetRGB(0x80, 0xFF, 0x80), in_string);
