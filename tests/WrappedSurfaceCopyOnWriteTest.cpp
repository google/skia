...
DEF_GPUTEST_FOR_ALL_CONTEXTS(WrappedSurfaceCopyOnWrite, reporter, ctxInfo) {
    ...
    auto surfaceProxyID = [&](const sk_sp<SkSurface>& surf) {
      return SkCanvasPriv::TopDeviceSurfaceDrawContext(surf->getCanvas())->asSurfaceProxy()->uniqueID();
    };
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(SkipCopyTaskTest, reporter, ctxInfo) {
    ...
    auto task = dContext->priv().drawingManager()->newCopyRenderTask(src->asSurfaceProxyRef(),
    ...
    float kTol[4] = {};
    auto errorReporter = std::function<ComparePixmapsErrorReporter>(
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(SkipOpsTaskTest, reporter, ctxInfo) {
    ...
    auto task = sk_ref_sp(dst->getOpsTask());
