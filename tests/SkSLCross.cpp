...
// This is a GPU test that ensures the SkSL 2d cross() intrinsic outputs the correct sign (negative,
// positive, or zero).
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(SkSLCross, reporter, ctxInfo) {
    ...
    run_test(reporter, directContext, rtc.get(), {0, 2.287}, {0, -7.741}, 0);  // Zero.
    run_test(reporter, directContext, rtc.get(), {62.17, 0}, {-43.49, 0}, 0);  // Zero.
    // cross(a,a) == 0
    run_test(reporter, directContext, rtc.get(), {58.39, 26.71}, {58.39, 26.71}, 0);
    // cross(a,a) == 0
    run_test(reporter, directContext, rtc.get(), {58.39, 26.71e9f}, {58.39, 26.71e9f}, 0);
    // cross(a,a*k) == 0
    run_test(reporter, directContext, rtc.get(), {389, 271}, {389*3, 271*3}, 0);
}
...
static void run_test(skiatest::Reporter* reporter, GrDirectContext* directContext,
    ...
    GrColor result;
    rtc->readPixels(directContext, SkImageInfo::MakeN32Premul(1,1), &result, 4, {0, 0});
