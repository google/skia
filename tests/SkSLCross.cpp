    ...
    class Impl : public GrGLSLFragmentProcessor {
        void emitCode(EmitArgs& args) override {
            ...
            fAUniform = args.fUniformHandler->addUniform(&fp, kFragment_GrShaderFlag,
                                                         GrSLType::kHalf2_GrSLType, "a", &a);
            fBUniform = args.fUniformHandler->addUniform(&fp, kFragment_GrShaderFlag,
                                                         GrSLType::kHalf2_GrSLType, "b", &b);
            args.fFragBuilder->codeAppendf(R"(
                    half crossProduct = cross(%s, %s);
                    half2 visualization = clamp(half2(-sign(crossProduct), sign(crossProduct)),
                                                half2(0), half2(1));
                    %s = half4(visualization, 0, 1);)", a, b, args.fOutputColor);
        }
