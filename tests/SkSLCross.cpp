...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(SkSLCross, reporter, ctxInfo) {
    ...
    auto sdc = skgpu::v1::SurfaceDrawContext::Make(directContext, GrColorType::kRGBA_8888, nullptr,
                                                   SkBackingFit::kExact, {1, 1}, SkSurfaceProps());
    if (!sdc) {
        ERRORF(reporter, "could not create render target context.");
        return;
    }
    run_test(reporter, directContext, sdc.get(), {3,4}, {5,6}, -2);  // Negative.
    run_test(reporter, directContext, sdc.get(), {3,4}, {-5,-6}, 2);  // Positive.
    run_test(reporter, directContext, sdc.get(), {0, 2.287f}, {0, -7.741f}, 0);  // Zero.
    run_test(reporter, directContext, sdc.get(), {62.17f, 0}, {-43.49f, 0}, 0);  // Zero.
}
