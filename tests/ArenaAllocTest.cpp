...
DEF_TEST(ArenaAlloc, r) {
...

    {
        ...
        struct Node {
            Node(std::unique_ptr<Node, SkArena::Destroyer<Node>> next)
                : fNext{std::move(next)} { created++; }
            ~Node() { destroyed++; }
            std::unique_ptr<Node, SkArena::Destroyer<Node>> fNext;
        };

        std::unique_ptr<Node, SkArena::Destroyer<Node>> current = nullptr;
        for (int i = 0; i < 128; i++) {
