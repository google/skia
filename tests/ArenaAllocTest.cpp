...
template<typename T>
using UP = std::unique_ptr<T, SkArena::Destroyer<T>>;
...
DEF_TEST(ArenaAlloc, r) {
...

    {
        ...
        struct N {
            N(std::unique_ptr<N, SkArena::Destroyer<N>> next) : fNext{std::move(next)} { created++; }
            ~N() { destroyed++; }
            std::unique_ptr<N, SkArena::Destroyer<N>> fNext;
        };

        std::unique_ptr<N, SkArena::Destroyer<N>> current = nullptr;
        for (int i = 0; i < 128; i++) {
            current = arena.makeUnique<N>(std::move(current));
        }
        REPORTER_ASSERT(r, created == 128);
        REPORTER_ASSERT(r, destroyed == 0);
    }
...

    {
        ...
        SkSTArena<64> arena;
        auto a = arena.makeArray<I>(8, [](size_t i ) { return i; });
        for (size_t i = 0; i < 8; i++) {
