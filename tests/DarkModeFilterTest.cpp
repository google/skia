...
DEF_TEST(DarkModeFilter_FilterImage, reporter) {
    SkDarkModeConfig config;
    config.fInvertStyle = SkDarkModeConfig::InvertStyle::kInvertLightness;
    ...
    SkRect r = SkRect::MakeLTRB(SkIntToScalar(2), SkIntToScalar(2), SkIntToScalar(8),
                                SkIntToScalar(8));
    canvasPaint.drawRect(r, paint);

    paint.setColorFilter(SkDarkModeFilter::Make(config));
    ...
    for (int y = r.top(); y < r.bottom(); ++y) {
        for (int x = r.left(); x < r.right(); ++x) {
            ...
            REPORTER_ASSERT(reporter,
                            filterColor == paint.getColorFilter()->filterColor(paintColor));
        }
    }
}

DEF_TEST(DarkModeFilter_SanityCheck, reporter) {
    SkDarkModeConfig config;
    config.fInvertStyle = SkDarkModeConfig::InvertStyle::kInvertLightness;
    sk_sp<SkColorFilter> filter = SkDarkModeFilter::Make(config);
...
DEF_TEST(DarkModeFilter_InvalidInputs, reporter) {
    SkDarkModeConfig config;
    REPORTER_ASSERT(reporter, config.isValid());

    // Valid invert style
    config.fInvertStyle = SkDarkModeConfig::InvertStyle::kInvertBrightness;
    REPORTER_ASSERT(reporter, config.isValid());
    config.fInvertStyle = SkDarkModeConfig::InvertStyle::kInvertLightness;
    REPORTER_ASSERT(reporter, config.isValid());
    sk_sp<SkColorFilter> filter = SkDarkModeFilter::Make(config);
    ...
    config.fInvertStyle = static_cast<SkDarkModeConfig::InvertStyle>(999);
    REPORTER_ASSERT(reporter, !config.isValid());
    filter = SkDarkModeFilter::Make(config);
    REPORTER_ASSERT(reporter, !filter);
