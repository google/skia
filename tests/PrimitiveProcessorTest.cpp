...
public:
    ...
    const char* name() const override { return "Dummy Ip"; }
...
DEF_GPUTEST_FOR_ALL_CONTEXTS(VertexAttributeCount, reporter, ctxInfo) {
    ...
    renderTargetContext->priv().testingOnly_addDrawOp(grPaint, GrAAType::kNone, Op::Make(attribCnt));
    context->flush();
#if GR_GPU_STATS
...
#endif
    context->resetGpuStats();
    renderTargetContext->priv().testingOnly_addDrawOp(grPaint, GrAAType::kNone, Op::Make(attribCnt + 1);
    context->flush();
#if GR_GPU_STATS
    REPORTER_ASSERT(reporter, context->getGpu()->stats()->numDraws() == 0);
