...
static void test_image_backed(skiatest::Reporter* reporter,
    ...
    sk_sp<SkSpecialImage> fullImg(SkSpecialImage::MakeFromImage(direct, full, srcImage));

    const SkIRect& subset = SkIRect::MakeXYWH(kPad, kPad, kSmallerSize, kSmallerSize);

    sk_sp<SkSpecialImage> subsetImg(SkSpecialImage::MakeFromImage(direct, subset, srcImage));
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(ImageFilterCache_ImageBackedGPU, reporter, ctxInfo) {
    ...
    GrSurfaceProxyView srcView = create_proxy_view(direct);
    if (!srcView.proxy()) {
        return;
    }

    if (!srcView.proxy()->instantiate(direct->priv().resourceProvider())) {
        return;
    }
    ...
    sk_sp<SkImage> srcImage(SkImage::MakeFromTexture(direct,
                                                     backendTex,
                                                     texOrigin,
    ...
    test_image_backed(reporter, direct, srcImage);
}
