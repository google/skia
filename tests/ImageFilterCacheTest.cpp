...
static void test_find_existing(skiatest::Reporter* reporter,
    ...
    cache->set(key1, filter.get(), skif::Image<For::kOutput>(image, offset));

    skif::Image<For::kOutput> foundImage;
...
static void test_dont_find_if_diff_key(skiatest::Reporter* reporter,
    ...
    SkIPoint offset = SkIPoint::Make(3, 4);
    auto filter = make_filter();
    cache->set(key0, filter.get(), skif::Image<For::kOutput>(image, offset));
...
static void test_internal_purge(skiatest::Reporter* reporter, const sk_sp<SkSpecialImage>& image) {
    ...
    cache->set(key1, filter1.get(), skif::Image<For::kOutput>(image, offset));
    ...
    cache->set(key2, filter2.get(), skif::Image<For::kOutput>(image, offset));
