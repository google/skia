...
DEF_GPUTEST_FOR_METAL_CONTEXT(MtlCopySurfaceTest, reporter, ctxInfo) {
    ...
    // Despite Apple's documentation, this appears to be always the case.
    // Checking just to be sure.
    REPORTER_ASSERT(reporter, drawable.texture.usage & MTLTextureUsageShaderRead);
    ...
    GrBackendTexture backendTexture(1024, 768, GrMipMapped::kNo, fbInfo);

    GrProxyProvider* proxyProvider = context->priv().proxyProvider();
    sk_sp<GrSurfaceProxy> srcProxy = proxyProvider->wrapRenderableBackendTexture(
                                             backendTexture, kTopLeft_GrSurfaceOrigin, 1,
                                             GrColorType::kBGRA_8888, kBorrow_GrWrapOwnership,
                                             GrWrapCacheable::kNo);
