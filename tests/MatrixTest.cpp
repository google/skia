...
DEF_TEST(Matrix_maprects, r) {
    ...
    {
        // We have special-cases in mapRect for different matrix types, so we want to test them
        SkMatrix m0 = SkMatrix::MakeTrans(1e20, 1e20);
        SkMatrix m1 = SkMatrix::MakeScale(1e20, 1e20);
        SkMatrix m2; m2.setRotate(30); m2.postScale(1e20, 1e20);

        for (const auto& m : { m0, m1, m2 }) {
            SkRect rect = { 0, 0, 1e20, 1e20 };
