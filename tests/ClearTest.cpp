...
static bool check_rect(GrRenderTargetContext* dc, const SkIRect& rect, uint32_t expectedValue,
...
static bool reset_dc(sk_sp<GrRenderTargetContext>* dc, GrContext* context, int w, int h) {
    ...
    *dc = context->makeRenderTargetContext(SkBackingFit::kExact, w, h, kRGBA_8888_GrPixelConfig,
                                           nullptr);
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(ClearBatch, reporter, ctxInfo) {
    ...
    if (!reset_dc(&rtContext, context, kW, kH)) {
