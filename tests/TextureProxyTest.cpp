...
int GrResourceProvider::numUniqueKeyProxies_TestOnly() const {
    return fUniqueKeyProxies.count();
}
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(TextureProxyTest, reporter, ctxInfo) {
    ...
    GrResourceCache* cache = ctxInfo.grContext()->getResourceCache();

    SkASSERT(!provider->numUniqueKeyProxies_TestOnly());
    ...
    for (auto fit : { SkBackingFit::kExact, SkBackingFit::kApprox }) {
        for (auto create : { new_deferred, new_wrapped }) {
            ...
            SkASSERT(1 == provider->numUniqueKeyProxies_TestOnly());
            ...
            proxy = nullptr;
            SkASSERT(0 == provider->numUniqueKeyProxies_TestOnly());
            SkASSERT(1 == cache->getResourceCount());
