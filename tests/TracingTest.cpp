...
static void test_trace_objects() {
    ...
    // Specific objects can be tracked through time with the TRACE_EVENT_OBJECT_ macros.
    ...
    {
        // Other events (duration or instant) can refer directly to objects. For Skia's JSON tracer,
        // having an argument whose name starts with '#' will trigger the creation of JSON that links
        // the event to the object (with a direct link to the most recent snapshot).
