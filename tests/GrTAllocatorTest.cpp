...
// FIXME This is useful, but only tests things that aren't trivially copyable
// Should do another version with a TrivialC that can be verified that the memcpys are working
// can we share code? Or does it not work since we have to check gInstCount in this function?
...
static void run_concat_test(skiatest::Reporter* reporter, int aCount, int bCount) {
    ...
    // NOLINTNEXTLINE(bugprone-use-after-move): GrTAllocator guarantees the moved list is empty,
    // but clang-tidy doesn't know about it; in practice we won't really be using moved lists so
    // this won't pollute our main code base with lots of warning disables.
