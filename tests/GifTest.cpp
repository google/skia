...
DEF_TEST(Codec_AnimatedTransparentGif, r) {
    ...
    // use565==0 means to use the natural SkColorType for the GIF image
    // (kN32_SkColorType). use565==1 means to use kRGB_565_SkColorType.
    for (int use565 = 0; use565 < 2; use565++) {
        SkBitmap bm;
        bm.allocPixels(use565 > 0 ? info.makeColorType(kRGB_565_SkColorType) : info);
        ...
        for (int i = 0; i < 2; i++) {
            ...
            if (i > 0) {
                ...
                expectedPixels[1][1] = use565 > 0 ? 0xFF0000FF : 0xFF0000FF;
                expectedPixels[1][3] = use565 > 0 ? 0xFF000052 : 0xFF000055;
            }
