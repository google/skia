...
static void test_charsToGlyphs(skiatest::Reporter* reporter, const sk_sp<SkTypeface>& face) {
    ...
    uint16_t cacheGlyphIds[256];
    auto codepointCache = face->codepointToGlyphCache();
    ...
    for (size_t testIndex = 0; testIndex < SK_ARRAY_COUNT(charsToGlyphs_TestData); ++testIndex) {
        ...
        codepointCache->utfNToGlyphs(
            test.chars, test.charsByteLength, test.typefaceEncoding, cacheGlyphIds);
        ...
        for (int i = 0; i < test.charCount; ++i) {
...

            {
                SkString a;
                a.appendf("%s, paintGlyphIds[%d] = %d, faceGlyphIds[%d] = %d, face = %s",
                          test.name, i, (int) paintGlyphIds[i], i, (int) cacheGlyphIds[i],
                          name.c_str());
                REPORTER_ASSERT(reporter, paintGlyphIds[i] == cacheGlyphIds[i], a.c_str());
            }
        }
