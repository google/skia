...
DEF_GPUTEST_FOR_ALL_CONTEXTS(GpuColorFilter, reporter, ctxInfo) {
    ...
    for (size_t i = 0; i < SK_ARRAY_COUNT(filterTests); ++i) {
        ...
        REPORTER_ASSERT(reporter, filterColor(inout.knownColorComponents().color(), inout.knownColorComponents().knownComponentFlags()) ==
                                  test.outputColor);
        REPORTER_ASSERT(reporter, test.outputComponents == inout.knownColorComponents().knownComponentFlags());
    }
}
