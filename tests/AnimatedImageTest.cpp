...
class Listener : public SkAnimatedImage::OnAnimationEndListener {
public:
    Listener() : fReachedEnd(false) {}

    bool fReachedEnd;

protected:
    void onAnimationEnd(SkAnimatedImage*) {
        fReachedEnd = true;
    }
};
...
DEF_TEST(AnimatedImage, r) {
    ...
    for (const char* file : { "images/alphabetAnim.gif",
        ...
        sk_sp<Listener> listener = sk_sp<Listener>(new Listener);
        animatedImage->setOnAnimationEndListener(listener);
        REPORTER_ASSERT(r, !listener->fReachedEnd);
        ...
        for (int loopCount : { 0, 1, 2, 5 }) {
            ...
            for (int loops = 0; loops <= loopCount; loops++) {
                ...
                for (size_t i = 0; i < frameInfos.size(); ++i) {
                    double next = animatedImage->update(currentTime);
                    if (!listener->fReachedEnd) {
                        currentTime = next;
                    }
                }
            }
            if (!listener->fReachedEnd) {
                ERRORF(r, "Did not reach end with %i loops", loopCount);
            }
        }
    }
}
