...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(ProxyRefTest, reporter, ctxInfo) {
    ...
    for (auto make : { make_deferred, make_wrapped }) {
        // A single write
        {
            ...
            if (proxy.get()) {
                GrPendingIOResource2 fWrite(proxy.get());

                int backingRefs = proxy->isWrapped_ForTesting() ? 1 : -1;

                check_refs(reporter, proxy.get(), 1, backingRefs);
        ...
        // A single read
        {
            sk_sp<GrTextureProxy> proxy((*make)(proxyProvider, caps));
            if (proxy.get()) {
                GrPendingIOResource2 fRead(proxy.get());
        ...
        // A single read/write pair
        {
            sk_sp<GrTextureProxy> proxy((*make)(proxyProvider, caps));
            if (proxy.get()) {
                GrPendingIOResource2 fRW(proxy.get());
