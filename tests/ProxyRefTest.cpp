...
static sk_sp<GrSurfaceProxy> make_deferred(GrContext* ctx, GrTextureProvider* provider) {
    GrSurfaceDesc desc;
    ...
    return GrSurfaceProxy::MakeDeferred(ctx, desc, SkBackingFit::kApprox, SkBudgeted::kYes);
}

static sk_sp<GrSurfaceProxy> make_wrapped(GrContext* ctx, GrTextureProvider* provider) {
    GrSurfaceDesc desc;
    desc.fFlags = kRenderTarget_GrSurfaceFlag;
    desc.fWidth = kWidthHeight;
    desc.fHeight = kWidthHeight;
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(ProxyRefTest, reporter, ctxInfo) {
    ...
    for (auto make : { make_deferred, make_wrapped }) {
        // A single write
        {
            sk_sp<GrSurfaceProxy> sProxy((*make)(ctxInfo.grContext(), provider));
