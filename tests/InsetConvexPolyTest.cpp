...
DEF_TEST(InsetConvexPoly, reporter) {
    ...
    *rrectPoly.push() = SkPoint::Make(-100 - 2.5f, 50 + 4.330127f);
    ...
    result = SkInsetConvexPolygon(&rrectPoly[0], 55, rrectPoly.count(), &insetPoly);
    REPORTER_ASSERT(reporter, result);
    REPORTER_ASSERT(reporter, is_convex(insetPoly));
    REPORTER_ASSERT(reporter, insetPoly.count() == 4);
    ...
    // past full inset
    result = SkInsetConvexPolygon(&rrectPoly[0], 75, rrectPoly.count(), &insetPoly);
    REPORTER_ASSERT(reporter, !result);
    ...
    *clippedRRectPoly.push() = SkPoint::Make(342.289948f, 432.947998f);
    ...
    result = SkInsetConvexPolygon(&clippedRRectPoly[0], clippedRRectPoly.count(), 32.3699417f, &insetPoly);
    REPORTER_ASSERT(reporter, result);
    REPORTER_ASSERT(reporter, is_convex(insetPoly));
}
