...
DEF_TEST(InsetConvexPoly, reporter) {
    ...
    *rrectPoly.push() = SkPoint::Make(-100 - 2.5f, 50 + 4.330127f);
    ...
    bool result = SkInsetConvexPolygon(rrectPoly, 3, &insetPoly);
    REPORTER_ASSERT(reporter, result);
    REPORTER_ASSERT(reporter, is_convex(insetPoly));

    // inset to rect
    result = SkInsetConvexPolygon(rrectPoly, 10, &insetPoly);
    REPORTER_ASSERT(reporter, result);
    REPORTER_ASSERT(reporter, is_convex(insetPoly));
    ...
    result = SkInsetConvexPolygon(rrectPoly, 55, &insetPoly);
    REPORTER_ASSERT(reporter, result);
    REPORTER_ASSERT(reporter, is_convex(insetPoly));
    REPORTER_ASSERT(reporter, insetPoly.count() == 4);
    ...
    // past full inset
    result = SkInsetConvexPolygon(rrectPoly, 75, &insetPoly);
    REPORTER_ASSERT(reporter, !result);
    ...
    *clippedRRectPoly.push() = SkPoint::Make(342.289948f, 432.947998f);
    ...
    result = SkInsetConvexPolygon(clippedRRectPoly, 32.3699417f, &insetPoly);
    REPORTER_ASSERT(reporter, result);
    REPORTER_ASSERT(reporter, is_convex(insetPoly));
}
