...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(SkRemoteGlyphCache_DrawTextAsDFT, reporter, ctxInfo) {
    ...
    GrSDFTControl options = direct->priv().asRecordingContext()->priv().getSDFTOptions(true);
    REPORTER_ASSERT(reporter, options.drawingType(font, paint, matrix) == GrSDFTControl::kSDFT);
