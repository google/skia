...
DEF_TEST(SkRemoteGlyphCache_TypefaceSerialization, reporter) {
    ...
    auto server_tf = ToolUtils::DefaultTypeface();
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(SkRemoteGlyphCache_StrikeSerialization, reporter, ctxInfo) {
    ...
    // Server.
    auto serverTf = ToolUtils::TypefaceFromName("monospace", SkFontStyle());
...
DEF_TEST(SkRemoteGlyphCache_PurgesServerEntries, reporter) {
    ...
    // one.
    {
        auto serverTf = ToolUtils::TypefaceFromName("Georgia", SkFontStyle());
        int glyphCount = 10;
        auto serverBlob = buildTextBlob(serverTf, glyphCount);
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(SkRemoteGlyphCache_DrawTextAsPath, reporter, ctxInfo) {
    ...
    REPORTER_ASSERT(reporter, SkStrikeSpec::ShouldDrawAsPath(paint,
                                                             SkFont(ToolUtils::DefaultTypeface()),
                                                             SkMatrix::I()));
