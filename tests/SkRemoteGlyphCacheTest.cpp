...
public:
    ...
    size_t handleCount() { return fNextHandleId; }
...
DEF_TEST(SkRemoteGlyphCache_StrikePinningClient, reporter) {
    ...
    REPORTER_ASSERT(reporter, SkStrikeCache::CountCachesForTesting(clientTf.get()) == 1u);
    discardableManager->unlockAndDeleteAll();
    SkGraphics::PurgeFontCache();
    REPORTER_ASSERT(reporter, SkStrikeCache::CountCachesForTesting(clientTf.get()) == 0u);
}
