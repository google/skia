...
DEF_TEST(SkRemoteGlyphCache_SearchOfDesperation, reporter) {
    ...
    // Build a fallback cache.
    {
        ...
        SkGlyph prototype{lostGlyphID};
        prototype.fMaskFormat = SkMask::kA8_Format;
        prototype.fHeight = 1;
        prototype.fWidth = 2;
        prototype.fImage = (void*)glyphImage;
        fallbackCache->mergeGlyph(lostGlyphID, prototype);
    }
    ...
    // Look for the lost glyph.
    {
        ...
        testCache->ensureImage(lostGlyph);
...
DEF_TEST(SkRemoteGlyphCache_ReWriteGlyph, reporter) {
    ...
    // Build a fallback cache.
    {
        ...
        SkGlyph prototypeGlyph{lostGlyphID};
        prototypeGlyph.fMaskFormat = fakeMask;
        prototypeGlyph.fHeight = 1;
        prototypeGlyph.fWidth = 2;
        prototypeGlyph.fImage = (void*)glyphImage;
        fallbackCache->mergeGlyph(lostGlyphID, prototypeGlyph);
    }
...

    {
        ...
        REPORTER_ASSERT(reporter, glyph && glyph->fMaskFormat == fakeMask);
        if (glyph) {
            REPORTER_ASSERT(reporter,
