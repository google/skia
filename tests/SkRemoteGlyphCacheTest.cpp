...
SkBitmap RasterBlob(sk_sp<SkTextBlob> blob, GrContext* context,
    ...
    SkASSERT_RELEASE(surface->getCanvas());
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(SkRemoteGlyphCache_DrawTextAsDFT, reporter, ctxInfo) {
    if (!ctxInfo.grContext()->supportsDistanceFieldText()) {
        return;
    }

// TODO(khushalsagar): Figure out why this test fails for vulkan on Win.
#if defined(SK_BUILD_FOR_WIN)
    if (ctxInfo.grContext()->contextPriv().getBackend() == kVulkan_GrBackend) {
        return;
    }
#endif
    ...
    REPORTER_ASSERT(reporter, GrTextContext::CanDrawAsDistanceFields(paint, matrix, surfaceProps,
                                                                     true, options));
    ...
    settings.surfaceProps = surfaceProps;
    SkBitmap expected = RasterBlob(serverBlob, ctxInfo.grContext(), settings);
    SkBitmap actual = RasterBlob(clientBlob, ctxInfo.grContext(), settings);
    COMPARE_BLOBS(expected, actual, reporter);
    REPORTER_ASSERT(reporter, !discardableManager->hasCacheMiss());
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(SkRemoteGlyphCache_EmptyTypeface, reporter, ctxInfo) {
