...
// FIXME need to redo these tests with the new limited restricted API.
// FIXME this CL shouldn't have the nicer public API wrapper yet, but that might need the simple
//     non low-level tests moved to that CL as well, instead of just deleting them.
// FIXME we also need to sort out the new allocation initial and growth sizing rules, since we now
// have the ability to represent both and need to decide exactly how the prealloc size, fixed size,
// and increment size get combined.
