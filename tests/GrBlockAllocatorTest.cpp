...
template<size_t N>
static Block* block(GrSBlockAllocator<N>& pool, int blockIndex) {
    Block* found = nullptr;
    int i = 0;
...
DEF_TEST(GrBlockAllocatorReset, r) {
    ...
    REPORTER_ASSERT(r, block(pool, 0)->metadata() == 0);
...
DEF_TEST(GrBlockAllocatorReleaseBlock, r) {
    ...
    for (int gp = 0; gp < GrBlockAllocator::kGrowthPolicyCount; ++gp) {
        ...
        block(pool, 0)->setMetadata(1);
        block(pool, 1)->setMetadata(2);
        block(pool, 2)->setMetadata(3);
        block(pool, 3)->setMetadata(4);
        ...
        pool->releaseBlock(block(pool, 0));
        REPORTER_ASSERT(r, pool->totalSize() == pool->preallocSize());
        REPORTER_ASSERT(r, block_count(pool) == 1);
        REPORTER_ASSERT(r, firstAlloc == alloc_byte(pool));
        REPORTER_ASSERT(r, block(pool, 0)->metadata() == 0); // metadata reset too
