...
struct PromiseTextureChecker {
    explicit PromiseTextureChecker(const GrBackendTexture& tex)
            ...
            , fReleaseCount(0)
            , fDoneCount(0) {}
    SkDeferredDisplayListRecorder::PromiseImageTexture fTexture;
    int fFulfillCount;
    int fReleaseCount;
    int fDoneCount;
    ...
    /**
     * Replaces the backend texture that this checker will return from fullfil. Also, transfers
     ...
     * valid replacement.
     */
    SkDeferredDisplayListRecorder::PromiseImageTexture replaceTexture(const GrBackendTexture& tex = GrBackendTexture()) {
        // Can't change this while in active fulfillment.
        SkASSERT(fFulfillCount == fReleaseCount);
        auto temp = std::move(fTexture);
        fTexture = SkDeferredDisplayListRecorder::PromiseImageTexture(tex);
        return temp;
    }

    SkTArray<GrUniqueKey> uniqueKeys() const { return fTexture.testingOnly_uniqueKeysToInvalidate(); }

    static SkDeferredDisplayListRecorder::PromiseImageTexture* Fulfill(void* self) {
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(PromiseImageTextureReuse, reporter, ctxInfo) {
    ...
    REPORTER_ASSERT(reporter, check_fulfill_and_release_cnts(promiseChecker,
                                                             ...
                                                             expectedDoneCnt,
                                                             reporter));
    REPORTER_ASSERT(reporter, GrBackendTexture::TestingOnly_Equals(promiseChecker.fLastFulfilledTexture, backendTex1));
    ...
    REPORTER_ASSERT(reporter, check_fulfill_and_release_cnts(promiseChecker,
                                                             ...
                                                             expectedDoneCnt,
                                                             reporter));
    REPORTER_ASSERT(reporter, GrBackendTexture::TestingOnly_Equals(promiseChecker.replaceTexture().backendTexture(), backendTex1));
    gpu->deleteTestingOnlyBackendTexture(backendTex1);

    // We should invalidate the key on the previously cached texture (after ensuring invalidation
    // messages have been processed by calling purgeAsNeeded.)
    ...
    REPORTER_ASSERT(reporter, check_fulfill_and_release_cnts(promiseChecker,
                                                             ...
                                                             expectedDoneCnt,
                                                             reporter));
    REPORTER_ASSERT(reporter, GrBackendTexture::TestingOnly_Equals(promiseChecker.fLastFulfilledTexture, backendTex2));
    ...
    // Because we have kept the PromiseImageTexture alive, we should be able to use it again and
    // hit the cache.
    ctx->contextPriv().getResourceCache()->purgeAsNeeded();
    REPORTER_ASSERT(reporter, ctx->contextPriv().resourceProvider()->findByUniqueKey<>(texKey2));
    ...
    SkDeferredDisplayListRecorder::PromiseImageTexture promiseImageTexture2 = promiseChecker.replaceTexture(backendTex3);
    REPORTER_ASSERT(reporter, GrBackendTexture::TestingOnly_Equals(promiseImageTexture2.backendTexture(), backendTex2));
    ...
    // If we delete the PromiseImageTexture we should trigger both key removals.
    REPORTER_ASSERT(reporter, GrBackendTexture::TestingOnly_Equals(promiseChecker.replaceTexture().backendTexture(), backendTex3));
