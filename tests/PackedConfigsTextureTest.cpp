...
static void run_test(skiatest::Reporter* reporter, GrContext* context,
    ...
    for (auto origin : { kTopLeft_GrSurfaceOrigin, kBottomLeft_GrSurfaceOrigin }) {
        ...
        desc.fHeight = DEV_H;
        desc.fConfig = config;
        desc.fOrigin = origin; //0 == origin ? kTopLeft_GrSurfaceOrigin : kBottomLeft_GrSurfaceOrigin;
        ...
        SkDEBUGCODE(bool result =) sContext->readPixels(dstInfo, readBuffer.begin(), 0, 0, 0);
        SkASSERT(result);
