...
void wrap_rt_test(skiatest::Reporter* reporter, GrContext* context) {
    ...
    gpu->deleteTestingOnlyBackendTexture(&origBackendTex, false);
}

void wrap_trt_test(skiatest::Reporter* reporter, GrContext* context) {
    GrVkGpu* gpu = static_cast<GrVkGpu*>(context->getGpu());

    GrBackendTexture backendTex = gpu->createTestingOnlyBackendTexture(nullptr, kW, kH,
                                                                       kPixelConfig,
                                                                       true, GrMipMapped::kNo);
    const GrVkImageInfo* imageInfo = backendTex.getVkImageInfo();
    ...
    gpu->deleteTestingOnlyBackendTexture(&backendTex, true);
}
