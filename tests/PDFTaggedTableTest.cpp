...
using SkPDF::StructureElementAttribute;
using SkPDF::StructureElementAttributeType;
using SkPDF::TableHeaderScope;
...
DEF_TEST(SkPDF_tagged_table, r) {
    ...
    constexpr int kMaxAttributes = 100;
    ...
    StructureElementAttribute attributes[kMaxAttributes];
    int next_attribute = 0;
    StructureElementAttribute* attr = &attributes[next_attribute++];
    attr->fType = StructureElementAttributeType::kBoundingBox;
    attr->fBoundingBox.fLeft = 72;
    attr->fBoundingBox.fTop = 72;
    attr->fBoundingBox.fRight = 360;
    attr->fBoundingBox.fBottom = 360;
    table.fAttributes = attr;
    table.fAttributeCount = 1;
    ...
    for (int rowIndex = 0; rowIndex < kRowCount; rowIndex++) {
        ...
        for (int colIndex = 0; colIndex < kColCount; colIndex++) {
            ...
            if (cellIndex == 13) {
                StructureElementAttribute* attr =
                        &attributes[next_attribute++];
                attr->fType = StructureElementAttributeType::kColSpan;
                attr->fColSpan = 2;
                cell.fAttributes = attr;
                cell.fAttributeCount = 1;
            } else if (cell.fType == SkPDF::DocumentStructureType::kTH) {
                StructureElementAttribute* attr =
                        &attributes[next_attribute++];
                attr->fType = StructureElementAttributeType::kTableHeaderScope;
                attr->fTableHeaderScope = rowIndex == 0 ?
                        TableHeaderScope::kColumn : TableHeaderScope::kRow;
                cell.fAttributes = attr;
                cell.fAttributeCount = 1;
            }
        }
