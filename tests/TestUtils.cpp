...
bool check_solid_pixels(const SkColor4f& col, const SkPixmap& pixmap,
    ...
    const size_t kFloatBpp = GrColorTypeBytesPerPixel(GrColorType::kRGBA_F32);

    char floatA[kFloatBpp];
    // First convert 'col' to be compatible with 'pixmap'
    {
        GrPixelInfo srcInfo(GrColorType::kRGBA_F32, kUnpremul_SkAlphaType, pixmap.refColorSpace(), 1, 1);
        GrPixelInfo dstInfo(GrColorType::kRGBA_F32, pixmap.alphaType(), pixmap.refColorSpace(), 1, 1);

        SkAssertResult(GrConvertPixels(dstInfo, floatA, kFloatBpp, srcInfo, col.vec(), kFloatBpp));
    }

    size_t floatRowBytes = kFloatBpp * pixmap.width();
    std::unique_ptr<char[]> floatB(new char[floatRowBytes * pixmap.height()]);
    // The convert 'pixmap' to RGBA_F32
    {
    ...
    auto atB = std::function<AccessPixelFn>(
        [kFloatBpp, floatRowBytes](const char* floatBuffer, int x, int y) {
            return reinterpret_cast<const float*>(floatBuffer + y * floatRowBytes + x * kFloatBpp);
        });

    return compare_pixels(pixmap.width(), pixmap.height(), floatA, atA, floatB.get(), atB,
                          tolRGBA, error);
}
