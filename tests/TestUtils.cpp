...
void test_read_pixels(skiatest::Reporter* reporter, GrContext* context,
    ...
    if (!read) {
        ERRORF(reporter, "Error reading from texture.");
    }
...
void test_write_pixels(skiatest::Reporter* reporter, GrContext* context,
    ...
    if (!write) {
        if (expectedToWork) {
            ERRORF(reporter, "Error writing to texture.");
        }
        return;
    }

    if (write && !expectedToWork) {
        ERRORF(reporter, "writePixels succeeded when it wasn't supposed to.");
        return;
    }

    test_read_pixels(reporter, context, dstContext, pixels.get());
}
