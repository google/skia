...
void TestWritePixels(skiatest::Reporter* reporter,
    ...
    int pixelCnt = srcContext->width() * srcContext->height();
    SkAutoTMalloc<uint32_t> pixels(pixelCnt);
    for (int y = 0; y < srcContext->width(); ++y) {
        for (int x = 0; x < srcContext->height(); ++x) {
            pixels.get()[y * srcContext->width() + x] =
    ...
    SkImageInfo ii = SkImageInfo::Make(srcContext->width(), srcContext->height(),
                                       kRGBA_8888_SkColorType, kPremul_SkAlphaType);
    bool write = srcContext->writePixels(ii, pixels.get(), 0, {0, 0});
    if (!write) {
        if (expectedToWork) {
    ...
    TestReadPixels(reporter, srcContext, pixels.get(), testName);
}
