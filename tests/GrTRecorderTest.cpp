...
static void test_subclasses_iter(skiatest::Reporter*, Order&, Base::Recorder::iterator&,
                                 int = 0);
...
static void test_alignment(skiatest::Reporter* reporter) {
    ...
    for (const auto& x : recorder) {
        ...
        auto pointer = reinterpret_cast<uint64_t>(&x);
        auto mask = static_cast<uint64_t>(expectedAlignments[i]) - 1;
