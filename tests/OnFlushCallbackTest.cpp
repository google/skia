...
public:
    ...
    GrSurfaceProxyView getAtlasView(GrProxyProvider* proxyProvider, const GrCaps* caps) {
        ...
        fAtlasView = GrSurfaceProxyView(std::move(proxy), kBottomLeft_GrSurfaceOrigin, readSwizzle);
        return fAtlasView;
    }
    ...
    void preFlush(GrOnFlushResourceProvider* resourceProvider,
        ...
        auto rtc = resourceProvider->makeRenderTargetContext(fAtlasView, GrColorType::kRGBA_8888,
                                                             nullptr, nullptr);
...
DEF_GPUTEST_FOR_GL_RENDERING_CONTEXTS(OnFlushCallbackTest, reporter, ctxInfo) {
    ...
    for (int i = 0; i < kNumViews; ++i) {
        views[i] = make_upstream_image(
                context, &object, i*3,
                object.getAtlasView(proxyProvider, context->priv().caps()), kPremul_SkAlphaType);
    }
