...
static sk_sp<GrTextureProxy> make_upstream_image(GrContext* context, AtlasObject* object, int start,
    ...
    for (int i = 0; i < 3; ++i) {
        ...
        auto fp = GrSimpleTextureEffect::Make(atlasProxy, atlasAlphaType, SkBlendMode::kSrc,
                                              SkMatrix::I());
        GrPaint paint;
...
DEF_GPUTEST_FOR_GL_RENDERING_CONTEXTS(OnFlushCallbackTest, reporter, ctxInfo) {
    ...
    for (int i = 0; i < kNumProxies-1; ++i) {
        ...
        GrPaint paint;
        auto fp = GrSimpleTextureEffect::Make(std::move(proxies[i]), kPremul_SkAlphaType,
                                              SkBlendMode::kSrc, t);
        paint.setPorterDuffXPFactory(SkBlendMode::kSrc);
        paint.addColorFragmentProcessor(std::move(fp));
