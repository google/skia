    ...
    for (auto join : {SkPaint::kMiter_Join, SkPaint::kRound_Join}) {
        ...
        for (int i = 0; i < 18; ++i) {
            ...
            auto op = GrOp::Make<GrStrokeIndirectOp>(ctx, GrAAType::kMSAA,
                                                     SkMatrix::Scale(scale, scale), stroke, path,
                                                     GrPaint());
            auto strokeIndirectOp = op->cast<GrStrokeIndirectOp>();
            strokeIndirectOp->testingOnly_verifyPrePrepareResolveLevels(r, target);
            strokeIndirectOp->testingOnly_verifyPrepareBuffers(r, target);
        }
    }
}
...
static float test_tolerance(SkPaint::Join joinType) {
    // Ensure our fast approximation falls within 1.35 tessellation segments of the "correct"
    // answer. This is more than good enough when are scale is up to 65k.
...
void GrStrokeIndirectOp::testingOnly_verifyPrePrepareResolveLevels(skiatest::Reporter* r,
    ...
    for (const SkPath& path : fPathList) {
        ...
        for (auto iter = iterate.begin(); iter != iterate.end(); ++iter) {
            ...
            switch (verb) {
                ...
                case SkPathVerb::kCubic: {
                    ...
                    n = GrPathUtils::chopCubicAtConvex180(pts, chops);
                    for (int i = 0; i <= n; ++i) {
                ...
                case SkPathVerb::kClose:
                    if (pts[0] != startPoint) {
                        ...
                        if (fStroke.getJoin() == SkPaint::kRound_Join) {
                            // Line from pts[0] to startPoint, with a preceeding join.
                            float rotation = SkMeasureAngleBetweenVectors(pts[0] - lastControlPoint,
                                                                          startPoint - pts[0]);
                    ...
                    for (float numSegments : firstNumSegments) {
                        // The first stroke's resolve levels aren't written out until the end of the
                        // contour.
                        if (!check_resolve_level(r, numSegments, *nextResolveLevel++, tolerance)) {
                            return;
                        }
        ...
        for (float numSegments : firstNumSegments) {
            // The first stroke's resolve levels aren't written out until the end of the contour.
            if (!check_resolve_level(r, numSegments, *nextResolveLevel++, tolerance)) {
                return;
            }
        }
...
void GrStrokeIndirectOp::testingOnly_verifyPrepareBuffers(skiatest::Reporter* r,
                                                          GrMeshDrawOp::Target* target) {
    ...
    auto instance = (const IndirectInstance*)target->makeVertexSpace(0, 0, nullptr, nullptr);
    auto indirect = target->makeDrawIndirectSpace(0, nullptr, nullptr);
