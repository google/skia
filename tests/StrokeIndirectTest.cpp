...
static bool check_first_resolve_levels(skiatest::Reporter* r,
    ...
    if (firstNumSegments.count() > 1) {
        REPORTER_ASSERT(r, *(*nextResolveLevel)++ == -firstNumSegments.count() + 1);
    }
        ...
        for (auto iter = iterate.begin(); iter != iterate.end(); ++iter) {
            ...
            switch (verb) {
                ...
                case SkPathVerb::kQuad: {
                    ...
                    float numParametricSegments = GrWangsFormula::quadratic(
                ...
                case SkPathVerb::kCubic: {
                    ...
                    float T[2];
                    n = GrPathUtils::findCubicConvex180Chops(pts, T);
                    SkChopCubicAt(pts, chops, T, n);
                    if (!isFirstStroke && n > 0) {
                        REPORTER_ASSERT(r, *nextResolveLevel++ == -n);
                    }
                    for (int i = 0; i <= n; ++i) {
