...
bool does_full_buffer_contain_correct_values(GrColor* srcBuffer,
    ...
    GrColor* dstPtr = bottomUp ? dstBuffer + width*(height-1) : dstBuffer;
    int dstIncrement = bottomUp ? -width : +width;
...
void basic_transfer_test(skiatest::Reporter* reporter, GrContext* context, GrPixelConfig config,
    ...
    SkAutoTMalloc<GrColor> srcBuffer(kTextureWidth*kTextureHeight);
    SkAutoTMalloc<GrColor> dstBuffer(kTextureWidth*kTextureHeight);

    fill_transfer_data(0, 0, kTextureWidth, kTextureHeight, kTextureWidth, srcBuffer.get());

    // create and fill transfer buffer
    size_t size = GrBytesPerPixel(config)*kTextureWidth*kTextureHeight;
    ...
    fill_transfer_data(kLeft, kTop, kWidth, kHeight, kTextureWidth, srcBuffer.get());
    data = buffer->map();
    memcpy(data, srcBuffer.get(), size);
    buffer->unmap();

    size_t offset = sizeof(GrColor)*(kTop*kTextureWidth + kLeft);
    result = context->getGpu()->transferPixels(tex.get(), kLeft, kTop, kWidth, kHeight, config,
                                               buffer, offset, 0, &fence);
    REPORTER_ASSERT(reporter, result);
    if (result) {
        context->getGpu()->waitFence(fence);
    }
