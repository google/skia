...
void basic_transfer_from_test(skiatest::Reporter* reporter, const sk_gpu_test::ContextInfo& ctxInfo,
    ...
    if (!context->priv().caps()->isConfigTexturable(desc.fConfig) ||
        (renderable == GrRenderable::kYes && !context->priv().caps()->isConfigRenderable(desc.fConfig))) {
        return;
    }
    ...
    sk_sp<GrTexture> tex = resourceProvider->createTexture(desc, renderable, SkBudgeted::kNo, &data, 1);
    if (!tex) {
        return;
    }
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(TransferPixelsToTest, reporter, ctxInfo) {
    ...
    for (auto renderable : {GrRenderable::kNo, GrRenderable::kYes}) {
        for (auto colorType : {GrColorType::kRGBA_8888, GrColorType::kRGBA_8888_SRGB, GrColorType::kBGRA_8888}) {
