...
static sk_sp<SkPromiseImageTexture> promise_img_fulfill(void* ctx) {
    auto direct = static_cast<GrDirectContext*>(ctx);
    GrBackendTexture tex = direct->createBackendTexture(16, 16, kRGBA_8888_SkColorType, GrMipMapped::kNo, GrRenderable::kYes);
    SkASSERT(tex.isValid());
    return SkPromiseImageTexture::Make(tex);
}

static void promise_img_release(void*) {}

static void promise_img_done(void*) {}
...
DEF_GPUTEST(GrDDLImage_MakeSubset, reporter, options) {
    ...
    for (int ct = 0; ct < sk_gpu_test::GrContextFactory::kContextTypeCnt; ++ct) {
        ...
        auto promiseImage = recorder.makePromiseTexture(
            ...
            promise_img_release, promise_img_done, direct);
