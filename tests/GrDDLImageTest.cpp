...
DEF_GPUTEST(GrDDLImage_MakeSubset_CrossCtx, reporter, options) {
    ...
    for (int ct = 0; ct < sk_gpu_test::GrContextFactory::kContextTypeCnt; ++ct) {
        ...
        // 1. raster + context:
        auto subImg1 = rasterImg->makeSubset(direct, subsetBounds);
        ...
        auto subImg3 = promiseImage->makeSubset(direct, subsetBounds);
        REPORTER_ASSERT(reporter, subImg3->isValid(direct));

        // 4. recording ctx image + nullptr:
        auto subImg4 = promiseImage->makeSubset(nullptr, subsetBounds);
        ...
        // 5. gpu image + context:
        auto subImg5 = gpuImage->makeSubset(direct, subsetBounds);
        REPORTER_ASSERT(reporter, subImg5->isValid(direct));

        // 6. gpu image + nullptr:
        auto subImg6 = gpuImage->makeSubset(nullptr, subsetBounds);
