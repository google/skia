...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(DDLOperatorEqTest, reporter, ctxInfo) {
    ...
    for (int i = 0; i < SurfaceParameters::kNumParams; ++i) {
        ...
        SkSurfaceCharacterization char1 = params1.createCharacterization1(context);
        if (!char1.isValid()) {
        ...
        for (int j = 0; j < SurfaceParameters::kNumParams; ++j) {
            ...
            SkSurfaceCharacterization char2 = params2.createCharacterization1(context);
            if (!char2.isValid()) {
                continue;  // can happen on some platforms (ChromeOS)
            }
...

    {
        ...
        SkSurfaceCharacterization valid = params.createCharacterization1(context);
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(DDLMakeRenderTargetTest, reporter, ctxInfo) {
    ...
    for (int i = 0; i < SurfaceParameters::kNumParams; ++i) {
        ...
        {
            SkSurfaceCharacterization c = params.createCharacterization1(context);
            GrBackendTexture backend;
        ...
        {
            sk_sp<SkSurface> s = SkSurface::MakeRenderTarget1(context, c, SkBudgeted::kYes);
        ...
        {
            sk_sp<SkSurface> s = SkSurface::MakeFromBackendTexture1(context, c, backend);
            REPORTER_ASSERT(reporter, s);
            REPORTER_ASSERT(reporter, s->isCompatible(c));
        }
...
DEF_GPUTEST_FOR_GL_RENDERING_CONTEXTS(DDLCompatibilityTest, reporter, ctxInfo) {
    ...
    for (int ct = 0; ct <= kLastEnum_SkColorType; ++ct) {
        ...
        s = SkSurface::MakeRenderTarget1(context, c, SkBudgeted::kYes);
        REPORTER_ASSERT(reporter, s);
        if (!s) {
            continue;
        }
