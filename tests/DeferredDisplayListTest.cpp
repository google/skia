...
void DDLSurfaceCharacterizationTestImpl(GrDirectContext* dContext, skiatest::Reporter* reporter) {
    ...
    // First, create a DDL using the stock SkSurface parameters
    {
        ...
        dContext->flushAndSubmit(/*sync*/ true);
    }
    ...
    for (int i = 0; i < SurfaceParameters::kNumParams; ++i) {
        ...
        if (SurfaceParameters::kSampleCount == i) {
            ...
            GrBackendTexture backendTex =
                    s->getBackendTexture(SkSurface::kFlushRead_TextureHandleAccess);
            if (backendTex.isValid()) {
                format = backendTex.getBackendFormat();
            } else {
                GrBackendRenderTarget backendRT =
                        s->getBackendRenderTarget(SkSurface::kFlushRead_TextureHandleAccess);
                SkASSERT(backendRT.isValid());
                format = backendTex.getBackendFormat();
            }
            int supportedSampleCount =
                    caps->getRenderTargetSampleCount(params.sampleCount(), format);
            if (supportedSampleCount == 1) {
                continue;
            }
        }
