...
DEF_GPUTEST_FOR_ALL_CONTEXTS(SkSurfaceCharacterization, reporter, ctxInfo) {
    ...
    for (int i = 0; i < SurfaceParameters::kNumParams; ++i) {
        ...
        if (SurfaceParameters::kSampleCount == i) {
            ...
            SkDebugf("requested %d actual %d %d %d\n", params.sampleCount(), supportedSampleCount,
                gpuSurf->getDevice()->accessRenderTargetContext()->asRenderTargetProxy()->numColorSamples(),
                gpuSurf->getDevice()->accessRenderTargetContext()->asRenderTargetProxy()->numStencilSamples());
