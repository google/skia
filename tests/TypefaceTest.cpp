...
DEF_TEST(TypefaceVariationIndex, reporter) {
    ...
    if (!typeface) {
        ...
        // gets one fo these things make sense.
        return;
    }
    ...
    SkFontParameters::Axis axisRead[1];
    count = typeface->getAxes(axisRead);
    if (count == -1) {
        return;
    }
    ...
    REPORTER_ASSERT(reporter, axisRead[0].fTag == SkSetFourByteTag('w','g','h','t'));
    REPORTER_ASSERT(reporter, axisRead[0].fStyleValue == 1);
}
