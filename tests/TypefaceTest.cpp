...
DEF_TEST(TypefaceAxes, reporter) {
    ...
    SkFontArguments::Axis axis[] = { { SkSetFourByteTag('w','g','h','t'), SK_ScalarSqrt2 } };
    ...
    SkFontArguments::VariationCoordinate positionRead[SK_ARRAY_COUNT(axis)];
    count = typeface->getVariationDesignPosition(positionRead, SK_ARRAY_COUNT(positionRead));
    REPORTER_ASSERT(reporter, count == SK_ARRAY_COUNT(axis));

    REPORTER_ASSERT(reporter, positionRead[0].axis == axis[0].fTag);
...
DEF_TEST(TypefaceVariationIndex, reporter) {
    ...
    SkFontArguments::VariationCoordinate positionRead[1];
    count = typeface->getVariationDesignPosition(positionRead, SK_ARRAY_COUNT(positionRead));
    if (count == -1) {
        return;
    }
