...
DEF_TEST(TypefaceAxes, reporter) {
    ...
    // GetFontAxisCount (and releated) seem to incorrectly collapse multiple axes with the same tag.
    ...
    auto test = [&](SkTypeface* typeface, const Variation& expected, int alsoAcceptedAxisTagCount) {
        ...
        for (size_t actualIdx = 0; actualIdx < actualCount; ++actualIdx) {
            bool actualFound = false;
            for (size_t expectedIdx = 0; expectedIdx < expected.coordinateCount; ++expectedIdx) {
