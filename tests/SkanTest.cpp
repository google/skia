...
static void check_inval(skiatest::Reporter* reporter, const sk_sp<skan::Node>& root,
...
static void inval_test1(skiatest::Reporter* reporter) {
    auto color  = skan::Color::Make(0xff000000);
    auto r1     = skan::Rect::Make(SkRect::MakeWH(100, 100)),
         r2     = skan::Rect::Make(SkRect::MakeWH(100, 100));
    auto grp    = skan::Group::Make();
    auto matrix = skan::Matrix::Make(SkMatrix::I());
    auto root   = skan::Transform::Make(grp, matrix);

    grp->addChild(skan::Draw::Make(r1, color));
    grp->addChild(skan::Draw::Make(r2, color));
...
static void inval_test2(skiatest::Reporter* reporter) {
    auto color = skan::Color::Make(0xff000000);
    auto rect  = skan::Rect::Make(SkRect::MakeWH(100, 100));
    auto m1    = skan::Matrix::Make(SkMatrix::I()),
         m2    = skan::Matrix::Make(SkMatrix::I(), m1);
    auto t1    = skan::Transform::Make(skan::Draw::Make(rect, color), m2),
         t2    = skan::Transform::Make(skan::Draw::Make(rect, color), m1);
    auto root  = skan::Group::Make();
    root->addChild(t1);
    root->addChild(t2);
...
DEF_TEST(SkanInvalidation, reporter) {
