...
static void TestInlineVector_basic(skiatest::Reporter* reporter) {
    ...
    T b[5] = { 0, 1, 2, 3, 4 };
    a = SkInlinedVector<N, T>(b, b + SK_ARRAY_COUNT(b));
...
static void test_swap(skiatest::Reporter* reporter) {
    test_swap<SkInlinedVector< 5, int>>(reporter);
    test_swap<SkInlinedVector<10, int>>(reporter);
    test_swap<SkInlinedVector<20, int>>(reporter);
    ...
    test_swap<SkInlinedVector< 5, MoveOnlyInt>>(reporter);
    test_swap<SkInlinedVector<10, MoveOnlyInt>>(reporter);
    test_swap<SkInlinedVector<20, MoveOnlyInt>>(reporter);
}

static void test_self_assignment(skiatest::Reporter* reporter) {
    SkInlinedVector<1, int> a;
    a.push_back(1);
    REPORTER_ASSERT(reporter, !a.empty());
    REPORTER_ASSERT(reporter, a.size() == 1);
    REPORTER_ASSERT(reporter, a[0] == 1);
...
DEF_TEST(SkInlinedVector, reporter) {
