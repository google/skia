...
path.close();
// DEBUG_UNDER_DEVELOPMENT  fails with image mismatch
testSimplifyCheck(reporter, path, filename, !SkOpGlobalState::DebugRunFail());
}
...
path.close();
// DEBUG_UNDER_DEVELOPMENT  fails with wrong result
testSimplifyCheck(reporter, path, filename, !SkOpGlobalState::DebugRunFail());
}
...
path.close();
// DEBUG_UNDER_DEVELOPMENT  fails internally (unexpectedly) with angle sort (flip ambiguity)
testSimplifyFuzz(reporter, path, filename);
}
...
path.close();
// DEBUG_UNDER_DEVELOPMENT  fails to generate correct result
testSimplifyCheck(reporter, path, filename, !SkOpGlobalState::DebugRunFail());
}
