...
static void check_equivalence(skiatest::Reporter* r, const GrShape& a, const GrShape& b,
    ...
    if (SkPathPriv::ConvertToNonInverseFillType(pathA.getFillType()) !=
        SkPathPriv::ConvertToNonInverseFillType(pathB.getFillType())) {
    ...
    if (allowSameRRectButDiffStartAndDir) {
        ...
        if (ignoreInversenessDifference) {
            pA.setFillType(SkPathPriv::ConvertToNonInverseFillType(pathA.getFillType()));
            pB.setFillType(SkPathPriv::ConvertToNonInverseFillType(pathB.getFillType()));
        }
...
private:
    static void CheckBounds(skiatest::Reporter* r, const GrShape& shape, const SkRect& bounds) {
        ...
        SkPath p = path;
        p.setFillType(SkPathPriv::ConvertToNonInverseFillType(path.getFillType()));
