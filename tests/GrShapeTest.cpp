...
private:
    ...
    void init(skiatest::Reporter* r, SkScalar scale) {
        ...
        if (fBase.style().isSimpleFill()) {
            GrShape filledShape = GrShape::MakeFilled(fBase);
            Key filledKey;
            make_key(&filledKey, filledShape);
            check_equivalence(r, filledShape, fBase, filledKey, fBaseKey);
            if (fBase.inverseFilled()) {
                filledShape = GrShape::MakeFilledNonInverted(fBase);
                make_key(&filledKey, filledShape);
                REPORTER_ASSERT(r, !filledShape.inverseFilled());
                REPORTER_ASSERT(r, filledKey != fBaseKey || !filledKey.count());
            }
        }
        if (fAppliedPE.style().isSimpleFill()) {
            GrShape filledShape = GrShape::MakeFilled(fAppliedPE);
            Key filledKey;
            make_key(&filledKey, filledShape);
            check_equivalence(r, filledShape, fAppliedPE, filledKey, fAppliedPEKey);
            if (fAppliedPE.inverseFilled()) {
                filledShape = GrShape::MakeFilledNonInverted(fAppliedPE);
                make_key(&filledKey, filledShape);
                REPORTER_ASSERT(r, !filledShape.inverseFilled());
                REPORTER_ASSERT(r, filledKey != fAppliedPEKey || !filledKey.count());
            }
        }
        if (fAppliedFull.style().isSimpleFill()) {
            GrShape filledShape = GrShape::MakeFilled(fAppliedFull);
            Key filledKey;
            make_key(&filledKey, filledShape);
            check_equivalence(r, filledShape, fAppliedFull, filledKey, fAppliedFullKey);
            if (fAppliedFull.inverseFilled()) {
                filledShape = GrShape::MakeFilledNonInverted(fAppliedFull);
                make_key(&filledKey, filledShape);
                REPORTER_ASSERT(r, !filledShape.inverseFilled());
                REPORTER_ASSERT(r, filledKey != fAppliedFullKey || !filledKey.count());
            }
        }
    }
