...
DEF_GPUTEST(GrContext_oomed, reporter, options) {
    ...
    for (int ct = 0; ct < sk_gpu_test::GrContextFactory::kContextTypeCnt; ++ct) {
        ...
        GrContext* context = factory.get(
                contextType, sk_gpu_test::GrContextFactory::ContextOverrides::kRandomGLOOM);
        if (!context) {
            continue;
        }
        ...
        for (int run = 0; run < 100; ++run) {
            ...
            for (int i = 0; i < 1000; ++i) {
                ...
                // issuing a couple draw calls using scratch resources created in a previous
                // iteration.
