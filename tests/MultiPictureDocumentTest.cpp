...
static sk_sp<SkImage> makeAHardwareBufferTestImage(
    ...
    // Make SkImage from buffer in a way that mimics libes/hwui/AutoBackendTextureRelease
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(SkMultiPictureDocument_AHardwarebuffer,
    ...
    for (int i=0; i<NUM_FRAMES; i++) {
        SkCanvas* pictureCanvas = multipic->beginPage(WIDTH, HEIGHT);
        draw_basic(pictureCanvas, i, image);
        multipic->endPage();
        // Also draw the picture to an image for later comparison
        auto surf = SkSurface::MakeRaster(info);
        draw_basic(surf->getCanvas(), i, image);
        expectedImages.push_back(surf->makeImageSnapshot());

        // Change ahbuffer contents. if code under test has not copied it, this will be detected
        // when the deserialized frames are compared with expectedImages
