...
static void test_nested_blends(skiatest::Reporter* reporter, SkSurface* surface) {
    ...
    const SkColor4f kExpected = {0.25f, 0.75f, 0.0f, 0.5f};
    const float kTolerance[4] = {0.01f, 0.01f, 0.0f, 0.01f};
    std::function<ComparePixmapsErrorReporter> error = [reporter, &kExpected](
                                                               int x, int y, const float diffs[4]) {
        ERRORF(reporter,
               "Wrong color, expected (%.2f %.2f %.2f %.2f), diffs (%.3f, %.3f, %.3f, %.3f)",
               kExpected.fR, kExpected.fG, kExpected.fB, kExpected.fA,
               diffs[0], diffs[1], diffs[2], diffs[3]);
    };
    CheckSolidPixels(kExpected, pixmap, kTolerance, error);
}
