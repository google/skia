...
sk_sp<SkSurface> EGLTestHelper::importHardwareBufferForWrite(skiatest::Reporter* reporter,
    ...
    if (!surface) {
        ERRORF(reporter, "Failed to make gl SkSurface");
        return nullptr;
    }
...
sk_sp<SkImage> VulkanTestHelper::importHardwareBufferForRead(skiatest::Reporter* reporter,
    ...
    grImageInfo.fImage = fImage;
    grImageInfo.fAlloc = GrVkAlloc(fMemory, 0, hwbProps.allocationSize, 0);
    ...
    grImageInfo.fInitialQueueFamily = VK_QUEUE_FAMILY_EXTERNAL;
    grImageInfo.fCurrentQueueFamily = VK_QUEUE_FAMILY_EXTERNAL;
...
static void cleanup_resources(VulkanTestHelper* vulkanHelper, EGLTestHelper* eglHelper,
                              AHardwareBuffer* buffer) {
    if (vulkanHelper) {
...
void run_test(skiatest::Reporter* reporter, const GrContextOptions& options, SrcType srcType) {
    ...
    if (SrcType::kCPU == srcType) {
        ...
        for (int y = 0; y < DEV_H; ++y) {
            for (int x = 0; x < DEV_W; ++x) {
                ...
                if (srcPixel != dstPixel) {
                    ERRORF(reporter, "CPU HWB Expected readpixel (%d, %d) value 0x%08x, got 0x%08x.",
                           x, y,  srcPixel, dstPixel);
                }
            }
        }
        ...
        ///////////////////////////////////////////////////////////////////////////
        // Move AHardwareBuffer into GL, wrap the texture in an SkSurface and draw
        // the bitmap to it
        ///////////////////////////////////////////////////////////////////////////

        sk_sp<SkSurface> glSurface = eglHelper.importHardwareBufferForWrite(reporter, buffer);
        if (!glSurface) {
            cleanup_resources(&vulkanHelper, &eglHelper, buffer);
            return;
        }
        ...
        glSurface->getCanvas()->drawImage(srcBmpImage, 0, 0);

        bool readResult = glSurface->readPixels(dstBitmapEGL, 0, 0);
        if (!readResult) {
        ...
        glSurface.reset();
        eglHelper.doClientSync();
        eglHelper.cleanup();
    }

    ///////////////////////////////////////////////////////////////////////////
    // Import the HWB into vulkan and draw it to a surface
    ///////////////////////////////////////////////////////////////////////////

    sk_sp<SkImage> wrappedImage = vulkanHelper.importHardwareBufferForRead(reporter, buffer);
    if (!wrappedImage) {
        cleanup_resources(&vulkanHelper, &eglHelper, buffer);
        return;
    }

    GrContext* grVkContext = vulkanHelper.grContext();

    // Make SkSurface to render into
    SkImageInfo imageInfo = SkImageInfo::Make(DEV_W, DEV_H, kRGBA_8888_SkColorType,
                                              kPremul_SkAlphaType, nullptr);

    sk_sp<SkSurface> dstSurf = SkSurface::MakeRenderTarget(grVkContext,
    ...
    if (!readResult) {
        ...
        wrappedImage.reset();
        grVkContext->contextPriv().getGpu()->testingOnly_flushGpuAndSync();
        cleanup_resources(&vulkanHelper, &eglHelper, buffer);
        return;
    }
...
DEF_GPUTEST(VulkanHardwareBuffer_CPU, reporter, options) {
    run_test(reporter, options, SrcType::kCPU);
}

DEF_GPUTEST(VulkanHardwareBuffer_EGL, reporter, options) {
    run_test(reporter, options, SrcType::kEGL);
}
