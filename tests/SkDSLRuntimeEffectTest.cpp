...
static void test_RuntimeEffect_Shaders(skiatest::Reporter* r, GrRecordingContext* rContext) {
    ...
    {
        effect.start();
        Var gColor(kUniform_Modifier, kInt4);
        Function(kHalf4, "main").define(
    ...
    // ... and support *returning* float4, not just half4
    {
        effect.start();
        Var p(kFloat2, "p");
        Function(kFloat4, "main", p).define(
            Return(Float4(p - 0.5, 0, 1))
        );
        effect.end();
        effect.test(0xFF000000, 0xFF0000FF, 0xFF00FF00, 0xFF00FFFF);
    ...
    {
        effect.start();
        Var p1(kInOut_Modifier, kFloat2, "p");
        Function moveCoords(kVoid, "moveCoords", p1);
        moveCoords.define(
            p1 -= 0.5
        );
        Var p2(kFloat2, "p");
        Function(kFloat4, "main", p2).define(
            moveCoords(p2),
