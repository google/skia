...
DEF_GPUTEST_FOR_MOCK_CONTEXT(DSLFloat, r, ctxInfo) {
    ...
    REPORTER_ASSERT(r, e1.release()->description() == "3.4028234663852886e+38");

    Expression e2 = Float2(std::numeric_limits<float>::min());
    REPORTER_ASSERT(r, e2.release()->description() == "float2(1.1754943508222875e-38)");

    Expression e3 = Float2(-0.5, 1);
    REPORTER_ASSERT(r, e3.release()->description() == "float2(-0.5, 1.0)");

    Expression e4 = Float3(0.75);
    REPORTER_ASSERT(r, e4.release()->description() == "float3(0.75)");
...
DEF_GPUTEST_FOR_MOCK_CONTEXT(DSLHalf, r, ctxInfo) {
    ...
    Expression e2 = Half2(std::numeric_limits<float>::min());
    REPORTER_ASSERT(r, e2.release()->description() == "half2(1.1754943508222875e-38)");

    Expression e3 = Half2(-0.5, 1);
    REPORTER_ASSERT(r, e3.release()->description() == "half2(-0.5, 1.0)");

    Expression e4 = Half3(0.75);
    REPORTER_ASSERT(r, e4.release()->description() == "half3(0.75)");
