...
DEF_GPUTEST_FOR_MOCK_CONTEXT(DSLBlock, r, ctxInfo) {
    AutoDSLContext context(ctxInfo.directContext()->priv().getGpu());
    Statement x = Block();
    REPORTER_ASSERT(r, whitespace_insensitive_compare(x.release()->description().c_str(), "{ }"));
    Var a(kInt, "a"), b(kInt, "b");
    Statement y = Block(Declare(a, 1), Declare(b, 2), a = b);
    REPORTER_ASSERT(r, whitespace_insensitive_compare(y.release()->description().c_str(),
                                                      "{ int a = 1; int b = 2; (a = b); }"));
}
