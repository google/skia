...
DEF_GPUTEST_FOR_MOCK_CONTEXT(DSLFor, r, ctxInfo) {
...

    {
        ...
        Statement s = Block(
            Declare(y),
            Declare(z),
            For((Declare(x), y = 0, z = 0), x < 10, (++x, ++y, z -= x), x += y)
        );
        EXPECT_EQUAL(s, "{ int y; int z; { int x = 0; (y = 0); (z = 0); for (; (x < 10); "
                        "((++x, ++y), (z -= x))) (x += y); } }");
    }

    {
        ...
        Statement s = Block(
            ...
            Declare(y),
            Declare(z),
            For((x = 0, y = 0, z = 0), x < 10, (++x, ++y, z -= x), x += y)
        );
        EXPECT_EQUAL(s, "{ int x; int y; int z; for ((((x = 0), (y = 0)), (z = 0)); (x < 10); "
                        "((++x, ++y), (z -= x))) (x += y); }");
    }
