...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(ReadOnlyTexture, reporter, context_info) {
    ...
    for (auto ioType : {kRead_GrIOType, kRW_GrIOType}) {
        ...
        auto readResult = surfContext->readPixels(srcPixmap.info(), read.writable_addr(), 0, {0, 0});
        ...
        auto writeResult = surfContext->writePixels(srcPixmap.info(), srcPixmap.addr(), 0, {0, 0});
