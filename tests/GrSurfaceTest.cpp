...
DEF_GPUTEST(InitialTextureClear, reporter, baseOptions) {
    ...
    for (int ct = 0; ct < sk_gpu_test::GrContextFactory::kContextTypeCnt; ++ct) {
        ...
        const std::vector<GrCaps::TestingCombination>& combos = caps->getTestingCombinations();
        ...
        for (auto combo : combos) {
            ...
            uint32_t expectedClearColor = GrPixelConfigIsOpaque(desc.fConfig) ? 0xFF000000 : 0;
                                ...
                                if (texCtx->readPixels(info, data.get(), 0, {0, 0})) {
                                    for (int i = 0; i < kSize * kSize; ++i) {
                                        if (expectedClearColor != data.get()[i]) {
                                            ERRORF(reporter,
                                                   "Failed on conf %d (pix %d) 0x%x != 0x%x",
                                                   desc.fConfig, i, expectedClearColor,
                                                   data.get()[i]);
                                            break;
                                        }
                                    }
                        ...
                        for (int i = 0; i < 2; ++i) {
                            ...
                            if (renderable == GrRenderable::kYes) {
                                surfCtx = context->priv().makeDeferredRenderTargetContext(
                                        ...
                                        1, GrMipMapped::kNo, origin, nullptr);
                            } else {
                                ...
                                for (int i = 0; i < kSize * kSize; ++i) {
                                    if (expectedClearColor != data.get()[i]) {
                                        ERRORF(reporter, "Failed on conf %d (pix %d) 0x%x != 0x%x",
                                               desc.fConfig, i, expectedClearColor, data.get()[i]);
                                        break;
                                    }
                                }
                            }
