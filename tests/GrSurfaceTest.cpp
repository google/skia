...
DEF_GPUTEST_FOR_ALL_CONTEXTS(GrSurfaceRenderability, reporter, ctxInfo) {
    ...
    for (GrPixelConfig config : configs) {
        for (GrSurfaceOrigin origin : { kTopLeft_GrSurfaceOrigin, kBottomLeft_GrSurfaceOrigin }) {
            ...
            REPORTER_ASSERT_MESSAGE(reporter, SkToBool(tex) == ict,
                                    "config:%d, tex:%d, isConfigTexturable:%d", config,
                                    SkToBool(tex), ict);
            ...
            REPORTER_ASSERT_MESSAGE(reporter, SkToBool(tex) == icr,
            ...
            REPORTER_ASSERT_MESSAGE(reporter, SkToBool(tex) == icr,
                                    "config:%d, tex:%d, isConfigRenderable(true):%d", config,
                                    SkToBool(tex), icr);
        }
    }
}
