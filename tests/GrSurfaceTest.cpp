...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(ReadOnlyTexture, reporter, context_info) {
    auto fillPixels = [](SkBitmap* bm, const std::function<uint32_t(int x, int y)>& f) {
        for (int y = 0; y < bm->height(); ++y) {
            for (int x = 0; x < bm->width(); ++x) {
                *bm->getAddr32(x, y) = f(x, y);
            }
        }
    };
    ...
    for (auto ioType : {kRead_GrIOType, kRW_GrIOType}) {
        ...
        create_backend_texture(context, &backendTex, srcBM, GrMipMapped::kNo, Renderable::kYes);
        ...
        auto writeResult = surfContext->writePixels(ii, pixels.addr(), 0, 0, 0);
        ...
        if (context->priv().caps()->mipMapSupport()) {
            backendTex = context->priv().getGpu()->createTestingOnlyBackendTexture(
                    nullptr, kSize, kSize, GrColorType::kRGBA_8888, true, GrMipMapped::kYes);
