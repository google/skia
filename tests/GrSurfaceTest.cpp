                    ...
                    for (int i = 0; i < 2; ++i) {
                        ...
                        if (texCtx->readPixels(info, data.get(), 0, 0, 0)) {
                            ...
                            for (int j = 0; j < kSize * kSize; ++j) {
                                if (cmp != data.get()[j]) {
                                    ERRORF(reporter,
                                           "Failed on instantiated-pass config %d loop %d pix %d rt %d (0x%x vs 0x%x)",
                                           desc.fConfig, i, j, rt, cmp, data.get()[j]);
                                    break;
                                }
                            }
                        }
                    ...
                    for (int i = 0; i < 2; ++i) {
                        ...
                        if (surfCtx->readPixels(info, data.get(), 0, 0, 0)) {
                            ...
                            for (int j = 0; j < kSize * kSize; ++j) {
                                if (cmp != data.get()[j]) {
                                    ERRORF(reporter,
                                           "Failed on deferred-pass config %d loop %d pix %d rt %d (0x%x vs 0x%x)",
                                           desc.fConfig, i, j, rt, cmp, data.get()[j]);
                                    break;
                                }
                            }
