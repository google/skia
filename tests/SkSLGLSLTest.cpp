...
DEF_TEST(SkSLComplexDelete, r) {
    test(r,
        ...
        "void main() {"
        "vec4 tmpColor;"
        "sk_FragColor = vec4(1.0) * (tmpColor = texture(sampler, vec2(1)) , colorXform != mat4(1.0) ? vec4(clamp((mat4(colorXform) * vec4(tmpColor.xyz, 1.0)).xyz, 0.0, tmpColor.w), tmpColor.w) : tmpColor);"
        "}",
        *SkSL::ShaderCapsFactory::Default(),
        "#version 400\n"
        "out vec4 sk_FragColor;\n"
