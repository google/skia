...
void main() {
    ...
    sk_FragColor = vec4(vec2(v.y, 0).yx, 1.0, 1.0);
    ...
    sk_FragColor = vec4(vec3(v.yz, 1).zxy, 1.0);
    sk_FragColor = vec4(vec3(v.y, 0, 1).yxz, 1.0);
    sk_FragColor = vec4(vec2(v.z, 1).yyx, 1.0);
    sk_FragColor = v;
    ...
    sk_FragColor = vec3(v.xw, 1).xzzy;
    sk_FragColor = vec3(v.x, 1, 0).xyzy;
    sk_FragColor = vec4(v.yzw, 1).wxyz;
    sk_FragColor = vec4(v.yz, 0, 1).zxyw;
    sk_FragColor = vec4(v.yw, 0, 1).zxwy;
    sk_FragColor = vec2(v.y, 1).yxyy;
    sk_FragColor = vec3(v.zw, 0).zzxy;
    sk_FragColor = vec3(v.z, 0, 1).yyxz;
    sk_FragColor = vec3(v.w, 0, 1).yzzx;
}
)__GLSL__",
         SkSL::Program::kFragment_Kind
         );
...
void main() {
    float v = sqrt(1.0);
    sk_FragColor = vec4(v);
    sk_FragColor = vec4(vec4(v).xyz, 0).wzyx;
    sk_FragColor = vec3(vec4(v).xw, 0).zzxy;
    sk_FragColor = vec3(vec3(vec4(v).xw, 0).yx, 1).zzxy;
    sk_FragColor = vec3(vec4(v).zy, 1).xyzz;
    sk_FragColor = vec4(v);
...
DEF_TEST(SkSLSwizzleScalar, r) {
    ...
    test(r,
         ...
         *SkSL::ShaderCapsFactory::Default(),
         ...
         "out vec4 sk_FragColor;\n"
         "void main() {\n"
         "    sk_FragColor = vec3(sqrt(4.0), 0, 1).yxyz;\n"
         "}\n");
    test(r,
         "void main() {"
         ...
         *SkSL::ShaderCapsFactory::Default(),
         ...
         "out vec4 sk_FragColor;\n"
         "void main() {\n"
         "    sk_FragColor = vec3(vec2(sqrt(4.0)), 0).zxzy;\n"
         "}\n");
}
