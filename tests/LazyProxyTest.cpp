...
class LazyFailedInstansiationTestOp : public GrDrawOp {
public:
    DEFINE_OP_CLASS_ID

    LazyFailedInstansiationTestOp(GrProxyProvider* proxyProvider, int* testExecuteValue,
...
private:
    const char* name() const override { return "LazyFailedInstansiationTestOp"; }
    FixedFunctionFlags fixedFunctionFlags() const override { return FixedFunctionFlags::kNone; }
    RequiresDstTexture finalize(const GrCaps&, const GrAppliedClip*,
                                GrPixelConfigIsClamped) override {
        return RequiresDstTexture::kNo;
...
// Test that when a lazy proxy fails to instantiate during flush that we dropt he Op that it was
// associated with.
DEF_GPUTEST(LazyProxyFailedInstantiationTest, reporter, /* options */) {
    ...
    for (bool failInstantiation : {false, true}) {
        ...
        rtc->priv().testingOnly_addDrawOp(
                skstd::make_unique<LazyFailedInstansiationTestOp>(proxyProvider, &executeTestValue,
                                                                  failInstantiation));
        ctx->flush();
