...
It nominally contains 3 frames, but all of them are invalid. Presumbly it came
from a fuzzer doing random mutations and copies. The breakdown:
...
@02B 14 bytes Image Descriptor body (pixel data): 0x06 0x06 ... 0x00
   - lit_width =     6
   - 0x06 precedes a 6 byte block of LZW data
   - 0x04 precedes a 4 byte block of LZW data
   - 0x00 byte means "end of data" for this frame
...
@043  5 bytes Image Descriptor body (pixel data): 0x66 0xDE ... 0x3D
   - lit_width =   102, INVALID, OUTSIDE THE RANGE [2, 8]
   - 0xDE precedes a 222 byte block of LZW data, which is INVALIDly truncated
...
DEF_TEST(Codec_InvalidAnimated, r) {
    ...
    for (int i = 0; static_cast<size_t>(i) < frameInfos.size(); i++) {
        ...
        // GIF implementation.
        //
        // This test (without SK_HAS_WUFFS_GIF_LIBRARY) is overly specific to
        // the old implementation. As a fuzzer-discovered test, it's likely
        // that what's fundamentally being tested isn't that decoding an
        // invalid GIF leads to kSuccess, but that decoding an invalid GIF
        // doesn't lead to an ASAN violation.
        //
...
DEF_TEST(Codec_ossfuzz6274, r) {
    ...
    // factory method returns a nullptr SkImage*, instead of returning a
    // non-null but otherwise all-transparent SkImage*.
    //
