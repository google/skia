...
static bool peek_pony(const void* ptr, size_t len) {
    return len >= 4 && 0 == memcmp(ptr, "pony", 4);
}
...
static std::unique_ptr<SkCodec> make_pony2(std::unique_ptr<SkStream>, SkCodec::Result* res) {
    *res = SkCodec::kErrorInInput;
    return nullptr;
}
...
DEF_TEST(Codec_Register, r) {
    ...
    SkCodec::Register(peek_pony, make_pony);
    ...
    // Register a second method to decode ponies - it should be called instead
    // of the first.
    SkCodec::Register(peek_pony, make_pony2);

    REPORTER_ASSERT(r, nullptr ==
            SkCodec::MakeFromStream(SkMemoryStream::MakeDirect(encoded, SK_ARRAY_COUNT(encoded)),
                                    &res));
    REPORTER_ASSERT(r, res == SkCodec::kErrorInInput);
}
