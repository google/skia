...
static bool check_read(skiatest::Reporter* reporter, const SkBitmap& expectedBitmp,
    ...
    for (int y = 0; y < DEV_H && result; ++y) {
        for (int x = 0; x < DEV_W && result; ++x) {
            ...
            const uint32_t dstPixel = *sourceBitmap.getAddr32(x, y);
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(GrAHardwareBuffer_BasicDrawTest,
    ...
    REPORTER_ASSERT(reporter, surface->readPixels(dstBitmap, 0, 0));
