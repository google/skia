...
DEF_TEST(AndroidCodec_computeSampleSize, r) {
    ...
    for (const char* file : { "images/color_wheel.webp",
        ...
        for (SkISize size : { plus(dims, -1), times(dims, .15f), times(dims, .6f),
                              { (int) (dims.width() * .25f), (int) (dims.height() * .75f ) } }) {
            ...
            if (codec->getEncodedFormat() == SkEncodedImageFormat::kWEBP) {
                ...
                REPORTER_ASSERT(r, size == requested);
            } else {
                if (computedSampleSize == 1) {
        ...
        // Various up-scales.
        for (SkISize size : { dims, plus(dims, 5), times(dims, 2) }) {
            const int computedSampleSize = codec->computeSampleSize(&size);
            REPORTER_ASSERT(r, computedSampleSize == 1);
        ...
        for (int sampleSize : { 1, 2, 3, 4, 8, 16, 32 }) {
            ...
            const int computedSampleSize = codec->computeSampleSize(&size);
            if (sampledDims != size || computedSampleSize > sampleSize) {
