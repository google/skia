...
DEF_TEST(AndroidCodec_orientation, r) {
    ...
    for (char i = '1'; i <= '8'; ++i) {
        ...
        // For backwards compatibility with BitmapFactory, SkAndroidCodec does not, so its
        // dimensions may be reversed.
        ...
        options.fRespectEncodedOrigin = true;
        SkBitmap fromAndroidCodec;
        fromAndroidCodec.allocPixels(info);
        auto result = androidCodec->getAndroidPixels(info, fromAndroidCodec.getPixels(),
                                                     fromAndroidCodec.rowBytes(), &options);
