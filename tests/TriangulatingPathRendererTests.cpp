...
static void verify_simple_inner_polygons(skiatest::Reporter* r, const char* shapeName,
    ...
    for (auto fillType : {SkPathFillType::kWinding}) {
        ...
        bool isLinear;
        vertexCount = triangulator.pathToTriangles(&vertexAlloc, &breadcrumbList, &isLinear);
        ...
        for (const auto* node = breadcrumbList->head(); node; node = node->fNext) {
            add_tri_edges(r, trianglePlusBreadcrumbEdges, node->fPts);
            ++breadcrumbCount;
        }
        REPORTER_ASSERT(r, breadcrumbCount == breadcrumbList->count());
