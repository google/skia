...
DEF_TEST(WriteICCProfile, r) {
    ...
    for (auto test : tests) {
    ...
    #if defined(SK_USE_SKCMS)
        skcms_ICCProfile skcms;
        REPORTER_ASSERT(r, skcms_Parse(profile->data(), profile->size(), &skcms));

        skcms_ICCTag desc;
        REPORTER_ASSERT(r, skcms_GetTagBySignature(&skcms,
        ...
        REPORTER_ASSERT(r, nullptr != memmem(desc.buf, desc.size,
                                             big_endian_utf16, 2*strlen(test.desc)));
    #endif
    }
