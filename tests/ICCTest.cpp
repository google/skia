...
static inline void test_raw_transfer_fn(skiatest::Reporter* r, SkICC* icc) {
    ...
    REPORTER_ASSERT(r, 0.0f == tables.fRed.fTable[0]);
    REPORTER_ASSERT(r, 0.0f == tables.fGreen.fTable[0]);
    REPORTER_ASSERT(r, 0.0f == tables.fBlue.fTable[0]);
    REPORTER_ASSERT(r, 1.0f == tables.fRed.fTable[tables.fRed.fCount - 1]);
    REPORTER_ASSERT(r, 1.0f == tables.fGreen.fTable[tables.fGreen.fCount - 1]);
    REPORTER_ASSERT(r, 1.0f == tables.fBlue.fTable[tables.fBlue.fCount - 1]);
}
