...
static SkBitmap read_back(GrRenderTargetContext* rtc, int width, int height) {
    ...
    SkAssertResult(rtc->readPixels(dstII, bm.getAddr(0, 0), bm.rowBytes(), 0, 0, 0));
...
static void run_test(GrContext* ctx, skiatest::Reporter* reporter) {
...

    {
        ...
        if (!correct) {
            for (int y = kBigSize/2+1; y < kBigSize-kPad-1; ++y) {
                SkDebugf("%d: ", y);
                for (int x = kPad+1; x < kBigSize-kPad-1; ++x) {
                    SkDebugf("0x:%x, ", bm.getColor(x, y));
                }
                SkDebugf("\n");
            }        
        }
