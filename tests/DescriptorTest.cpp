...
DEF_TEST(Descriptor_empty, r) {
    ...
    desc->init();
    REPORTER_ASSERT(r, desc->isValid(size));
...
DEF_TEST(Descriptor_valid_simple, r) {
    ...
    REPORTER_ASSERT(r, desc->isValid(size));
    REPORTER_ASSERT(r, desc->getLength() == size);
    REPORTER_ASSERT(r, !desc->isValid(size - 4));
}
...
DEF_TEST(Descriptor_invalid_size, r) {
    ...
    REPORTER_ASSERT(r, desc->getLength() == size + effect_size);
    REPORTER_ASSERT(r, !desc->isValid(size));
    REPORTER_ASSERT(r, desc->isValid(size + effect_size));
}
