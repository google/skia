...
DEF_TEST(SkRuntimeShaderBuilderSetUniforms, r) {
    ...
    TmpOffset tmpOffset;
    REPORTER_ASSERT(r, b.uniform("offset").set(&tmpOffset, 1));

#ifndef SK_DEBUG
    REPORTER_ASSERT(r, !b.uniform("x").set(&tmpOffset, 1));
    REPORTER_ASSERT(r, !b.uniform("offset").set(&tmpOffset, 2));
#endif

    // add extra value to ensure that set doesn't try to use sizeof(array)
    float origin[] = { 2.0f, 3.0f, 4.0f };
    REPORTER_ASSERT(r, b.uniform("offset").set(origin, 2));
