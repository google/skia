...
DEF_TEST(SkRuntimeEffectForBlendFilter, r) {
    // Tests that the blend filter factory rejects or accepts certain SkSL constructs
    auto test_valid = [r](const char* sksl) {
        auto [effect, errorText] = SkRuntimeEffect::MakeForBlendFilter(SkString(sksl));
        REPORTER_ASSERT(r, effect, "%s", errorText.c_str());
    };
    ...
    // Child shaders are currently unsupported in blend filters
