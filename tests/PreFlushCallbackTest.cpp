 ...
 */
class AtlasedRectOp : public NonAARectOp {
public:
    DEFINE_OP_CLASS_ID
 ...
 */
class AtlasObject : public GrPreFlushCallbackObject {
public:
    ...
    void addOp(uint32_t opListID, AtlasedRectOp* op) {
        ...
        for (int i = 0; i < fOps2.count(); ++i) {
            if (opListID == fOps2[i].fID) {
                header = &(fOps2[i]);
            }
        }

        if (!header) {
            fOps2.push({opListID, nullptr});
            header = &(fOps2[fOps2.count()-1]);
        }
     ...
     */
    sk_sp<GrRenderTargetContext> preFlush(GrPreFlushResourceProvider* resourceProvider,
        ...
        for (int i = 0; i < opLists.count(); ++i) {
            if (LinkedListHeader* list = this->getList(opLists[i]->uniqueID())) {
                lists.push(list);
            }
        }
...
private:
    ...
    SkTDArray<LinkedListHeader>  fOps2;
...
static sk_sp<GrTextureProxy> make_upstream_image(GrContext* context, AtlasObject* object, int start,
    ...
    for (int i = 0; i < 3; ++i) {
        ...
        uint32_t opListID = rtc->priv().testingOnly_addDrawOp(std::move(paint),
                                                              GrAAType::kNone,
                                                              std::move(op));
