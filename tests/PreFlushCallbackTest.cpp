...
public:
    ...
    sk_sp<GrRenderTargetContext> preFlush(GrPreFlushResourceProvider* resourceProvider,
        ...
        for (int i = 0; i < this->numOps(); ++i) {
            AtlasedRectOp* atlasedOp = this->op(i);
...
private:
    ...
    AtlasedRectOp* op(int index) { return fOps[index]; }
    ...
    // TODO: rather than store an external list of ops we should be chaining w/in AtlasedRectOp
    SkTDArray<AtlasedRectOp*>    fOps;
