...
public:
    ...
    void releaseImage() override {
        ...
        if (fSrcImage != VK_NULL_HANDLE) {
            SkASSERT(fVkDestroyImage);
            fVkDestroyImage(fDevice, fSrcImage, nullptr);
            fSrcImage = VK_NULL_HANDLE;
        }

        if (fSrcMemory != VK_NULL_HANDLE) {
            SkASSERT(fVkFreeMemory);
            fVkFreeMemory(fDevice, fSrcMemory, nullptr);
            fSrcMemory = VK_NULL_HANDLE;
        }

        if (fDstImage != VK_NULL_HANDLE) {
            SkASSERT(fVkDestroyImage);
            fVkDestroyImage(fDevice, fDstImage, nullptr);
            fDstImage = VK_NULL_HANDLE;
        }

        if (fDstMemory != VK_NULL_HANDLE) {
            SkASSERT(fVkFreeMemory);
            fVkFreeMemory(fDevice, fDstMemory, nullptr);
            fDstMemory = VK_NULL_HANDLE;
        }
    }
    void cleanup() override {
        this->releaseImage();
...
private:
    ...
    VkImage fSrcImage = VK_NULL_HANDLE;
    VkDeviceMemory fSrcMemory = VK_NULL_HANDLE;

    VkImage fDstImage = VK_NULL_HANDLE;
    VkDeviceMemory fDstMemory = VK_NULL_HANDLE;
...
bool VulkanTestHelper::importHardwareBuffer(skiatest::Reporter* reporter,
    ...
    VkImage image;
    err = fVkCreateImage(fDevice, &imageCreateInfo, nullptr, &image);
    if (VK_SUCCESS != err) {
    ...
    err = fVkAllocateMemory(fDevice, &allocInfo, nullptr, &memory);
    if (VK_SUCCESS != err) {
    ...
    outImageInfo->fImage = image;
    outImageInfo->fAlloc = GrVkAlloc(memory, 0, hwbProps.allocationSize, 0);
