...
static SkSpan<const char> get_cluster_text(const uint32_t* clusters, int index,
                                           int glyphCount, const char* textPtr,
                                           const char* endPtr) {
    uint32_t max = SkToU32(endPtr - textPtr);
    ...
    return {textPtr + cluster, SkToSizeT(next - cluster)};
}
    ...
    void commitRun() override {
        if (fOutput) {
            ...
            for (int i = 0; i < fCurrentGlyphCount; ++i) {
                ...
                SkSpan<const char> text = get_cluster_text(fCurrentBuffer.clusters, i,
                                                           fCurrentGlyphCount, fText, fStop);
