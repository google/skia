...
// Ensures ccpr always reuses the same atlas texture in the animation usecase.
class CCPR_cache_animationAtlasReuse : public CCPRCacheTest {
    void onRun(skiatest::Reporter* reporter, CCPRPathDrawer& ccpr,
        ...
        // Ensures we always reuse the same atlas texture in the animation usecase.
        for (int i = 0; i < 12; ++i) {
    ...
    void onRun(skiatest::Reporter* reporter, CCPRPathDrawer& ccpr,
        ...
        for (int i = 0; i < 5; ++i) {
            ...
            for (GrCCPathCacheEntry* entry : lru) {
                ...
                REPORTER_ASSERT(reporter, 0 == entry->testingOnly_getOnFlushRefCnt());
                if (0 == i) {
                    ...
                    REPORTER_ASSERT(reporter, !cachedAtlas->onFlushProxy());
                    REPORTER_ASSERT(reporter, 0 == cachedAtlas->testingOnly_getOnFlushRefCnt());
                }
                ++count;
            }
...
class CCPR_cache_multiTileCache : public CCPRCacheTest {
    void onRun(skiatest::Reporter* reporter, CCPRPathDrawer& ccpr,
               const RecordLastMockAtlasIDs& atlasIDRecorder) override {
        const SkMatrix m = SkMatrix::MakeScale(kCanvasSize*3, kCanvasSize*3);
