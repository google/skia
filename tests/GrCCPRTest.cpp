...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(GrCCPRTest, reporter, ctxInfo) {
    ...
    const GrGLContextInfo& glctxInfo = ((GrGLGpu*)ctx->getGpu())->ctxInfo();
    SkDebugf("@@@> glctxInfo.standard(): %i\n", glctxInfo.standard());
    SkDebugf("@@@> glctxInfo.version(): %x\n", glctxInfo.version());
    SkDebugf("@@@> glctxInfo.glslGeneration(): %i\n", glctxInfo.glslGeneration());
    SkDebugf("@@@> glctxInfo.vendor(): %i\n", glctxInfo.vendor());
    SkDebugf("@@@> glctxInfo.renderer(): %i\n", glctxInfo.renderer());
    SkDebugf("@@@> glctxInfo.angleBackend(): %i\n", glctxInfo.angleBackend());
    SkDebugf("@@@> glctxInfo.angleVendor(): %i\n", glctxInfo.angleVendor());
    SkDebugf("@@@> glctxInfo.angleRenderer(): %i\n", glctxInfo.angleRenderer());
    SkDebugf("@@@> glctxInfo.driver(): %i\n", glctxInfo.driver());
    SkDebugf("@@@> glctxInfo.driverVersion(): %x\n", glctxInfo.driverVersion());

    ccpr.flush(); // If this crashes, fiddle with fMaxInstancesPerDrawArraysWithoutCrashing in your
                  // platform's GrGLCaps.
    SkDebugf("@@@@@@@@> 5\n");
}
