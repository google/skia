...
class CCPR_evictCacheEntryForPendingDrawOp : public CCPRRenderingTest {
  void onRun(skiatest::Reporter* reporter, const CCPRPathDrawer& ccpr) const override {
    ...
    // now draw the cached path in an incompatible matrix. Previous draw's cached atlas should not
    // be invalidated. otherwise, this flush would render two paths. For kA8_Multisample coverage
    // type, this would cause crash since allocated resources only allows rendering of one path.
