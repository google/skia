...
// This bug only affects CCPR with kA8_Multisample coverage type.
// It might be easier to run dm in an Android device with GLES3 MSAA compatible GPU.
class CCPR_multisampleCacheBug : public CCPRRenderingTest {
  void onRun(skiatest::Reporter* reporter, const CCPRPathDrawer& ccpr) const override {
    ...
    // in this flush first draw the cached path
    SkPath path;
    path.addRect(kRect);
    ccpr.drawPath(path);

    auto m = SkMatrix::Translate(0.1, 0.1);

    // now draw the cached path in an incompatible matrix. Previous draw's cache should not be
    // invalidated. otherwise, this flush would render two paths, but resource won't be allocated
    // for first draw because in resource spec it was marked as cached.
    ccpr.drawPath(path, m);
    ccpr.flush();
  }
};
DEF_CCPR_RENDERING_TEST(CCPR_multisampleCacheBug)
