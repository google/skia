...
check_pixels(skiatest::Reporter* reporter, int w, int h,
    ...
    for (int j = 0; j < h; ++j) {
        for (int i = 0; i < w; ++i) {
            int32_t expected = exepctedData[j * w + i];
            int32_t actual = actualData[j * w + i];
            if (expected != actual) {
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(IntTexture, reporter, ctxInfo) {
    ...
    // We  overwriting the top left quarter of the texture with the bottom right quarter of the
    // original data.
