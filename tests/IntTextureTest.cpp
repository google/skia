...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(IntTexture, reporter, ctxInfo) {
    ...
    {
        sk_sp<GrSurfaceProxy> copy(GrSurfaceContext::TestCopy(context, desc, texture.get(), SkBudgeted::kYes));
    ...
    // Test that copying to a non-integer (8888) texture fails.
    {
        ...
        sk_sp<GrSurfaceProxy> copy(GrSurfaceContext::TestCopy(context, nonIntDesc, texture.get(), SkBudgeted::kYes));
        REPORTER_ASSERT(reporter, !copy);
    }
