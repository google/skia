...
DEF_GPUTEST(GrContextFactory_abandon, reporter, options) {
    for (int i = 0; i < GrContextFactory::kContextTypeCnt; ++i) {
        ...
        REPORTER_ASSERT(reporter, info2.context());
        REPORTER_ASSERT(reporter, info2.testContext());

        REPORTER_ASSERT(reporter, info1.context() != info2.context());
...
DEF_GPUTEST(GrContextFactory_sharedContexts, reporter, options) {
    for (int i = 0; i < GrContextFactory::kContextTypeCnt; ++i) {
        ...
        ContextInfo info2 = testFactory.getSharedContextInfo(info1.context());
        REPORTER_ASSERT(reporter, !info2.context());
        info1.context()->unref();
        ...
        ContextInfo info4 = testFactory.getSharedContextInfo(info3.context());
        if (!info4.context()) {
            continue;
        }
        REPORTER_ASSERT(reporter, info3.context() != info4.context());
        REPORTER_ASSERT(reporter, info3.testContext() != info4.testContext());

        // Passing a different index should create a new (unique) context.
        ContextInfo info5 = testFactory.getSharedContextInfo(info3.context(), 1);
        REPORTER_ASSERT(reporter, info5.context());
        REPORTER_ASSERT(reporter, info5.testContext());
        REPORTER_ASSERT(reporter, info5.context() != info4.context());
...
DEF_GPUTEST(GrContextFactory_executorAndTaskGroup, reporter, options) {
    for (int i = 0; i < GrContextFactory::kContextTypeCnt; ++i) {
        ...
        if (GrContext* serialContext = serialInfo.context()) {
        ...
        if (GrContext* threadedContext = threadedInfo.context()) {
...
DEF_GPUTEST_FOR_ALL_CONTEXTS(GrContextDump, reporter, ctxInfo) {
    ...
    SkString result = ctxInfo.context()->dump();
