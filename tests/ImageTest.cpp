...
DEF_GPUTEST_FOR_GL_RENDERING_CONTEXTS(SkImage_MakeCrossContextRelease, reporter, ctxInfo) {
    ...
    // When we create a cross-context image, the original texture is ref'd on the context that
    // creates it. We asynchronously free that texture (via SkMessageBus), so we need to force that
    // here, or our bots will think this is a leak.
    ctx->getResourceCache()->purgeAsNeeded();
}
