...
bool GrDrawingManager::ProgramUnitTest(GrContext* context, int maxStages, int maxLevels) {
    ...
    if (drawingManager->flush2(nullptr, 0, SkSurface::BackendSurfaceAccess::kNoAccess, GrFlushInfo(),
    ...
    for (int i = 0; i < fpFactoryCnt; ++i) {
        ...
        for (int j = 0; j < 10; ++j) {
            ...
            GrDrawRandomOp(&random, renderTargetContext.get(), std::move(paint));
            if (drawingManager->flush2(nullptr, 0, SkSurface::BackendSurfaceAccess::kNoAccess,
