...
static sk_sp<GrSurfaceProxy> make_wrapped_FBO0(GrTextureProvider* provider,
    ...
    return GrSurfaceProxy::Make(std::move(defaultFBO));
}
...
static sk_sp<GrSurfaceProxy> make_wrapped_offscreen_rt(GrTextureProvider* provider, 
    ...
    return GrSurfaceProxy::Make(std::move(tex));
}
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(DefferredProxyConversionTest, reporter, ctxInfo) {
    ...
    {
        sk_sp<GrSurfaceProxy> sProxy(GrRenderTargetProxy::MakeDeferred(caps, desc,
    ...
    {
        sk_sp<GrSurfaceProxy> sProxy(GrTextureProxy::MakeDeferred(caps, provider, desc,
                                                                  SkBackingFit::kApprox,
                                                                  SkBudgeted::kYes));
