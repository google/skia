...
static sk_sp<GrSurfaceProxy> make_offscreen_rt(GrProxyProvider* provider,
                                               const GrSurfaceDesc& desc,
                                               GrSurfaceOrigin origin) {
    return provider->testingOnly_createInstantiatedProxy(desc, GrRenderable::kYes, origin, SkBackingFit::kExact,
                                                         SkBudgeted::kYes);
}
...
static sk_sp<GrSurfaceProxy> make_texture(GrProxyProvider* provider,
                                          ...
                                          GrSurfaceOrigin origin) {
    return provider->testingOnly_createInstantiatedProxy(desc, renderable, origin, SkBackingFit::kExact,
                                                         SkBudgeted::kYes);
}
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(DefferredProxyConversionTest, reporter, ctxInfo) {
                    ...
                    GrColorType::kRGBA_8888);
    {
        sk_sp<GrTextureProxy> proxy = proxyProvider->createProxy(
                format, desc, GrRenderable::kYes, kBottomLeft_GrSurfaceOrigin, SkBackingFit::kApprox, SkBudgeted::kYes);
