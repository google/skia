...
// Test to make sure we don't call our release proc on an image until we've tranfered it back to its
// original queue family.
DEF_GPUTEST_FOR_VULKAN_CONTEXT(VkReleaseExternalQueueTest, reporter, ctxInfo) {
    ...
    for (bool useExternal : {false, true}) {
        ...
        // Reseting the image should only trigger the release proc if we are not using an external
