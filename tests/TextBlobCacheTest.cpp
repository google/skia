...
static sk_sp<SkTextBlob> make_large_blob() {
    ...
    const int malloc_size = 0x3c3c3bd; // x86 size
    char *text = (char*)malloc(malloc_size + 1);
    if (text == nullptr){
        return nullptr;
    }
    ...
    static const int maxGlyphLen = malloc_size;
    SkGlyphID *glyphs = (SkGlyphID*)malloc(maxGlyphLen * sizeof(SkGlyphID));
    int glyphCount =
            font.textToGlyphs(text, malloc_size, SkTextEncoding::kUTF8, glyphs, maxGlyphLen);
    SkTextBlobBuilder builder;
    const auto& runBuffer = builder.allocRun(font, glyphCount, 0, 0);
    for (int i = 0; i < glyphCount; i++) {
        runBuffer.glyphs[i] = glyphs[i];
