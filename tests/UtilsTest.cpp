...
public:
    ...
    template<typename... Us>
    SkZip(SkZip<Us...>&& that)
            : fPointers{std::move(that.fPointers)}
            , fSize{that.fSize} {}
...
template <typename Iter, typename C = SkUnit>
class SkIota {
...
SkIota<Iter> SkMakeIota(C& c) {
    SkDebugf("using ref\n");
    return SkIota<Iter>{std::begin(c), std::end(c)};
}

template <typename C, typename Iter = decltype(std::begin(std::declval<C>()))>
SkIota<Iter, C> SkMakeIota(C&& c) {
    SkDebugf("using in place\n");
    return SkIota<Iter, C>{std::move(c)};
}
