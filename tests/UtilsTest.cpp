...
public:
    ...
    explicit SkZip2(const SkZip2<Us...>& that) : fPointers{that.fPointers}, fSize{that.fSize} {}
...
public:
    ...
    static auto MakeZip(Ts&& ... ts)
        -> SkZip2<Decay_t<decltype(std::forward<Ts>(ts))>...> {
        ...
        return SkZip2<Decay_t<decltype(std::forward<Ts>(ts))>...>{maxSize, std::forward<Ts>(ts)...};
    }
};
...
DEF_TEST(SkZip, reporter) {
    ...
    // Check SkZip calls
    {
        SkZip2<uint16_t*, float*, std::vector<int>&, std::array<int, 4>&, SkSpan<int>&>
                z{4, A, B, C, D, S};
    ...
    {
        SkIota iota{4};
        auto zz = SkMakeZip(SkMakeSpan(C), S, A, &B[0], iota, SkIota{4});
    ...
    {
        for (auto t : SkMakeZip(C, S, A, &B[0], SkIota{4})) {
            size_t i = std::get<4>(t);
            REPORTER_ASSERT(reporter, std::get<0>(t) == C[i]);
            REPORTER_ASSERT(reporter, std::get<1>(t) == S[i]);
            REPORTER_ASSERT(reporter, std::get<2>(t) == A[i]);
