...
DEF_TEST(Utils, reporter) {
    ...
    for (size_t i = 0; i < SK_ARRAY_COUNT(gTest); i++) {
        ...
        SkUnichar   u1 = SkUTF8_NextUnicharReplace(&p, stop);
...
DEF_TEST(SkUTF_NextUTF8_ToUTF8, r) {
    ...
    for (auto testCase : testCases) {
        ...
        REPORTER_ASSERT(r, uni == -1 || str == testCase.utf8String + strlen(testCase.utf8String));
        REPORTER_ASSERT(r, uni != -1 || str == testCase.utf8String);
