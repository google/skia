...
static sk_sp<GrRenderTargetContext> draw_mipmap_into_new_render_target(
    ...
    sk_sp<GrRenderTargetContext> rtc = drawingManager->makeRenderTargetContext(
            renderTarget, colorType, nullptr, nullptr, true);
    rtc->drawTexture(GrNoClip(), mipmapProxy, GrSamplerState::Filter::kMipMap,
                     SkBlendMode::kSrcOver, {1,1,1,1}, SkRect::MakeWH(4, 4), SkRect::MakeWH(1,1),
                     GrAA::kYes, GrQuadAAFlags::kAll, SkCanvas::kFast_SrcRectConstraint,
                     SkMatrix::I(), nullptr);
    return rtc;
}
...
DEF_GPUTEST_FOR_ALL_CONTEXTS(GrManyDependentsMipMappedTest, reporter, ctxInfo) {
    ...
    mipmapRTC->clear(nullptr, {.1, .2, .3, .4}, GrRenderTargetContext::CanClearFullscreen::kYes);
    ...
    sk_sp<GrRenderTargetContext> rtc1 = draw_mipmap_into_new_render_target(
            drawingManager, proxyProvider, colorType, mipmapProxy);
    ...
    // Draw the dirty mipmap texture into a second target.
