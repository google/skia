...
static std::unique_ptr<GrRenderTargetContext> draw_mipmap_into_new_render_target(
        GrRecordingContext* dContext, GrProxyProvider* proxyProvider, GrColorType colorType,
    ...
    auto rtc = GrRenderTargetContext::Make(
            dContext, colorType, nullptr, std::move(renderTarget), kTopLeft_GrSurfaceOrigin,
            nullptr);
...
DEF_GPUTEST(GrManyDependentsMipMappedTest, reporter, /* options */) {
    ...
    for (auto enableSortingAndReduction : {Enable::kYes, Enable::kNo}) {
        ...
        auto rtc1 = draw_mipmap_into_new_render_target(dContext.get(), proxyProvider, colorType,
        ...
        auto rtc2 = draw_mipmap_into_new_render_target(dContext.get(), proxyProvider, colorType,
                                                       alphaType, mipmapView, Filter::kMipMap);
        ...
        rtc1 = draw_mipmap_into_new_render_target(dContext.get(), proxyProvider, colorType,
        ...
        rtc2 = draw_mipmap_into_new_render_target(dContext.get(), proxyProvider, colorType,
