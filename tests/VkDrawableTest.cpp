...
public:
    ...
    static void ImportSubmitted(TestDrawable* td) {
        // Typical use case here would be to create a fence that we we submit to the gpu and then
        // wait on that before releasing the GrVkSecondaryCBDrawContext resources. To simulate that
        // for this test (and since we are running single threaded), we will just force a synce of
        ...
        // releasing the GrVkSecondaryCBDrawContext. If we had done it too early the call to release
        // the GrContext will show validation errors that we free'd vulkan objects while they were
        // still in use.
