...
public:
    ...
    class GpuDraw : public GpuDrawHandler {
    public:
        GpuDraw(const GrVkInterface* interface, int32_t width, int32_t height)
            : INHERITED()
            , fInterface(interface)
            , fWidth(width)
            , fHeight(height) {}
    ...
    std::unique_ptr<GpuDrawHandler> onSnapGpuDrawHandler(GrBackendApi backendApi,
        ...
        std::unique_ptr<GpuDraw> draw(new GpuDraw(fInterface, fWidth, fHeight));
