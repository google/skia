...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(GrContext_colorTypeSupportedAsSurface, reporter, ctxInfo) {
    for (int ct = 0; ct < kLastEnum_SkColorType; ++ct) {
        ...
        bool can = ctxInfo.grContext()->colorTypeSupportedAsSurface(colorType, false);
        auto surf = SkSurface::MakeRenderTarget(ctxInfo.grContext(), SkBudgeted::kYes, info, 0,
                                                nullptr);
        ...
        can = ctxInfo.grContext()->colorTypeSupportedAsSurface(colorType, true);
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(GrContext_maxSurfaceSamplesForColorType, reporter, ctxInfo) {
    for (int ct = 0; ct < kLastEnum_SkColorType; ++ct) {
        ...
        SkColorType colorType = static_cast<SkColorType>(ct);
        int max = ctxInfo.grContext()->maxSurfaceSamplesForColorType(colorType);
        bool canWithMSAA = ctxInfo.grContext()->colorTypeSupportedAsSurface(colorType, true);
        REPORTER_ASSERT(reporter, SkToBool(max) == canWithMSAA, "ct: %d", colorType);
        ...
        static constexpr int kLargeSampleCnt = SK_MaxS32;
        auto info = SkImageInfo::Make(kSize, kSize, colorType, kOpaque_SkAlphaType, nullptr);
        auto surf = SkSurface::MakeFromBackendTexture(ctxInfo.grContext(), backendTex,
                                                      kTopLeft_GrSurfaceOrigin, kLargeSampleCnt,
                                                      colorType, nullptr, nullptr);
