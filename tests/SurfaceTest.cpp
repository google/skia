...
static SkBudgeted is_budgeted(const sk_sp<SkImage> image, GrRecordingContext* rc) {
    return is_budgeted(image.get(), rc);
}
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(SurfaceBudget, reporter, ctxInfo) {
    ...
    for (auto budgeted : { SkBudgeted::kNo, SkBudgeted::kYes }) {
        ...
        REPORTER_ASSERT(reporter, budgeted == is_budgeted(surface));
        REPORTER_ASSERT(reporter, budgeted == is_budgeted(image, dContext));
