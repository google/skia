...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(GrContext_colorTypeSupportedAsSurface, reporter, ctxInfo) {
    ...
    for (int ct = 0; ct < kLastEnum_SkColorType; ++ct) {
        ...
        for (int sampleCnt : {1, 2}) {
            auto surf = MakeBackendRenderTargetSurface(context, {16, 16}, sampleCnt, kTopLeft_GrSurfaceOrigin, colorType);
            bool can = context->colorTypeSupportedAsSurface(colorType) &&
                       context->maxSurfaceSampleCountForColorType(colorType) >= sampleCnt;
            REPORTER_ASSERT(reporter, can == SkToBool(surf), "ct: %d, sc: %d, can: %d, surf: %d", colorType, sampleCnt,
                            can, SkToBool(surf));
            if (surf) {
                auto rtc = ((SkSurface_Gpu*)(surf.get()))->getDevice()->accessRenderTargetContext();
...
static sk_sp<SkSurface> create_gpu_surface_backend_render_target(
    ...
    SkPMColor4f pmColor{color.fR, color.fG, color.fB, color.fA};
    auto surf = MakeBackendRenderTargetSurface(ctx, {kWidth, kHeight}, sampleCnt, kTopLeft_GrSurfaceOrigin, kRGBA_8888_SkColorType, nullptr, nullptr, &pmColor);
    if (!surf) {
        return nullptr;
    }
