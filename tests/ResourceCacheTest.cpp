...
static void test_purge_invalidated(skiatest::Reporter* reporter) {
    ...
    Bus::Post(Msg(key1, context->contextID()));
    Bus::Post(Msg(key2, context->contextID()));
    cache->purgeAsNeeded();
    ...
    Bus::Post(Msg(key3, context->contextID()));
    cache->purgeAsNeeded();
...
static void test_free_resource_messages(skiatest::Reporter* reporter) {
    ...
    GrGpuResourceFreedMessage msg1{wrapped1, context->contextID()};
    SkMessageBus<GrGpuResourceFreedMessage>::Post(msg1);
    cache->purgeAsNeeded();

    REPORTER_ASSERT(reporter, 2 == TestResource::NumAlive());

    GrGpuResourceFreedMessage msg2{wrapped3, context->contextID()};
