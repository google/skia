...
static sk_sp<GrTexture> make_mipmap_texture(GrTextureProvider* provider,
    ...
    SkAutoTDelete<SkMipMap> mipmaps = SkMipMap::Build(bm, SkSourceGammaTreatment::kIgnore, nullptr);
    SkASSERT(mipmaps);
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(GPUMemorySize, reporter, ctxInfo) {
    ...
    REPORTER_ASSERT(reporter, kSize*kSize*4*4 == size || kSize*kSize*4*5 == size);
    ...
    size = tex->gpuMemorySize();
    REPORTER_ASSERT(reporter, kSize*kSize*4*4+(kSize*kSize*4)/3 == size ||
                              kSize*kSize*4*5+(kSize*kSize*4)/3 == size);
