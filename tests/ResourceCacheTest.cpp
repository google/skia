...
DEF_GPUTEST_FOR_CONTEXTS(ResourceCacheStencilBuffers, &is_rendering_and_not_angle_es3, reporter,
    ...
    if (context->caps()->maxSampleCount() >= 4) {
        ...
        if (context->caps()->maxSampleCount() >= 8 &&
            ...
            smallMSAART1.reset(resourceProvider->createTexture1(smallMSAADesc, SkBudgeted::kNo));
...
static sk_sp<GrTexture> make_normal_texture(GrResourceProvider* provider,
    ...
    return sk_sp<GrTexture>(provider->createTexture1(desc, SkBudgeted::kYes));
}

static sk_sp<GrTextureProxy> make_mipmap_texture(GrResourceProvider* provider,
                                                 GrSurfaceFlags flags,
                                                 int width, int height,
                                                 int sampleCnt) {
    SkBitmap bm;
    ...
    return provider->createMipMappedTexture(desc, SkBudgeted::kYes,
