...
static void test_time_purge(skiatest::Reporter* reporter) {
    ...
    for (int cnt : kCnts) {
        std::unique_ptr<GrStdClock::time_point[]> timeStamps(new GrStdClock::time_point[cnt]);
        {
            ...
            for (int i = 0; i < cnt; ++i) {
                ...
                r->resourcePriv().setUniqueKey(k);
                r->unref();
                timeStamps.get()[i] = GrStdClock::now();
            }
        ...
        // purged.
        {
            ...
            for (int i = 0; i < (cnt / 2); ++i) {
                ...
                cache->purgeResourcesNotUsedSince(GrStdClock::now());
