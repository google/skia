...
DEF_GPUTEST_FOR_CONTEXTS(ResourceCacheStencilBuffers, &is_rendering_and_not_angle_es3, reporter,
    ...
    sk_sp<GrTexture> smallRT0(resourceProvider->createTexture4(smallDesc, SkBudgeted::kYes));
    if (smallRT0 && smallRT0->asRenderTarget()) {
        resourceProvider->attachStencilAttachment(smallRT0->asRenderTarget());
    }

    sk_sp<GrTexture> smallRT1(resourceProvider->createTexture4(smallDesc, SkBudgeted::kYes));
    ...
    sk_sp<GrTexture> smallRT2(resourceProvider->createTexture4(smallDesc, SkBudgeted::kNo));
    ...
    sk_sp<GrTexture> bigRT(resourceProvider->createTexture4(bigDesc, SkBudgeted::kNo));
    ...
    if (context->caps()->maxSampleCount() >= 4) {
        ...
        sk_sp<GrTexture> smallMSAART0(resourceProvider->createTexture4(smallMSAADesc,
                                                                      SkBudgeted::kNo));
        ...
        sk_sp<GrTexture> smallMSAART1(resourceProvider->createTexture4(smallMSAADesc,
                                                                      SkBudgeted::kNo));
        ...
        if (context->caps()->maxSampleCount() >= 8 &&
            ...
            smallMSAART1 = resourceProvider->createTexture4(smallMSAADesc, SkBudgeted::kNo);
            sk_sp<GrTexture> smallMSAART1(
                resourceProvider->createTexture4(smallMSAADesc, SkBudgeted::kNo));
            if (smallMSAART1 && smallMSAART1->asRenderTarget()) {
                resourceProvider->attachStencilAttachment(smallMSAART1->asRenderTarget());
            }
            REPORTER_ASSERT(reporter,
...
static sk_sp<GrTexture> make_normal_texture(GrResourceProvider* provider,
    ...
    return provider->createTexture4(desc, SkBudgeted::kYes);
}
