...
static void test_time_purge(skiatest::Reporter* reporter) {
    ...
    for (int frameCnt : kFrameCnts) {
        std::unique_ptr<std::chrono::steady_clock::time_point[]> timeStamps(new std::chrono::steady_clock::time_point[frameCnt]);
        {
        ...
        // purged.
        {
            ...
            for (int i = 0; i < frameCnt >> 1; ++i) {
                ...
                context->purgeResourceNotUsedSince(std::chrono::steady_clock::now());
                REPORTER_ASSERT(reporter, frameCnt / 2 - i - 1 == cache->getResourceCount());
            }
