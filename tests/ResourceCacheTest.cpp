...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(PurgeToMakeHeadroom, reporter, ctxInfo) {
    ...
    for (bool success : { true, false }) {
        ...
        // Make a texture then discard it to make it purgeable
        auto fixedTex = make_normal_texture(resourceProvider, GrRenderable::kNo, {16, 16}, 1);
        REPORTER_ASSERT(reporter, fixedTex->gpuMemorySize() == kTexSize);

        auto purgeableTex = make_normal_texture(resourceProvider, GrRenderable::kNo, {16, 16}, 1);
        if (success) {
