...
/*static*/ void test_unbudgeted_to_scratch(skiatest::Reporter* reporter) {
    ...
    for (int i = 0; i < 2; ++i) {
        ...
        resource = static_cast<TestResource*>(cache->findAndRefScratchResource(key, TestResource::kDefaultSize));
...
static void test_remove_scratch_key(skiatest::Reporter* reporter) {
    ...
    GrGpuResource* find;
    find = cache->findAndRefScratchResource(scratchKey, TestResource::kDefaultSize);
...
static void test_scratch_key_consistency(skiatest::Reporter* reporter) {
    ...
    TestResource::ComputeScratchKey(TestResource::kB_SimulatedProperty, &scratchKey);
    GrGpuResource* find = cache->findAndRefScratchResource(scratchKey, TestResource::kDefaultSize);
    REPORTER_ASSERT(reporter, find != nullptr);
    find->unref();

    scratchKey2 = scratchKey;
    find = cache->findAndRefScratchResource(scratchKey2, TestResource::kDefaultSize);
    REPORTER_ASSERT(reporter, find != nullptr);
...
static void test_purge_invalidated(skiatest::Reporter* reporter) {
    ...
    // Get rid of c.
    cache->purgeAllUnlocked();
    scratch = cache->findAndRefScratchResource(scratchKey, TestResource::kDefaultSize);
    REPORTER_ASSERT(reporter, 0 == TestResource::NumAlive());
    REPORTER_ASSERT(reporter, 0 == cache->getResourceCount());
    REPORTER_ASSERT(reporter, 0 == cache->getResourceBytes());
