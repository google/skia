...
static SkIRect frame_rect_on_screen(const SkIRect& frameRect, const SkIRect& screenRect) {
...
void SkGifImageReader::setRequiredFrame(SkGIFFrameContext* frame) {
    ...
    const bool clearPrevFrameToBackground =
    ...
    // FIXME: This could be
    // frameCoversPrevFrame = !transValid && frameRect.contains(prevFrameRect);
    // arguably, it doesn't cover it if it has alpha, and we don't need transValid
    // otherwise.
    // We could do the same for fillsScreen, I think...
    const bool frameCoversPrevFrame = frameRect.contains(prevFrameRect);
    if (!transValid && frameCoversPrevFrame) {
        // This is still conservative. It is possible that prevRequiredFrame is
        // covered by frame (but not prevFrame), so frame does not truly depend
        // on it.
