    ...
    for (const unsigned char* ch = block; bytesInBlock-- > 0; ch++) {
	    ...
	    if (code == avail && oldcode != -1) {
	        ...
	        // value to get the code related to that.
             ...
             // rowIter assigned with firtschar thereby completing the code.
	     ...
	     while (remaining > 0) {
	         ...
	         // related to the oldcode in a batch of SK_DICTIONARY_WORD_SIZE bytes.

	         memcpy(p, suffix[code], SK_DICTIONARY_WORD_SIZE);
	         code = prefix[code];
	     ...
	     if (avail < SK_MAX_DICTIONARY_ENTRIES && oldcode != -1) {
	         ...
	         memcpy(suffix[avail], suffix[oldcode], SK_DICTIONARY_WORD_SIZE);
...
bool SkGIFLZWContext::prepareToDecode()
{
    ...
    const size_t kMaxBytes = (kMaxSequence + SK_DICTIONARY_WORD_SIZE - 1)
    ...
    for (int i = 0; i < clearCode; ++i) {
        memset(suffix[i], 0, SK_DICTIONARY_WORD_SIZE);
        suffix[i][0] = i;
        suffixLength[i] = 1;
