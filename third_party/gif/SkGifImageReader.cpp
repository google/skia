    ...
    for (const unsigned char* ch = block; bytesInBlock-- > 0; ch++) {
	    ...
	    if (code == avail && oldcode != -1) {
	     ...
	     // value to get the code related to that.
             ...
             // Round remaining up to multiple of kDictionaryWordSize, because that's
             ...
             remaining += -remaining & (SK_MAX_DICTIONARY_WORD_SIZE - 1) ;
             unsigned char *p = rowIter + remaining;

             // place rowIter so that after writing pixels related to oldchar (in memcpy below)
             // rowIter can be assigned with firtschar thereby completing the code.
	     rowIter += suffixLength[code];

	     while (remaining > 0) {
	       p -= SK_MAX_DICTIONARY_WORD_SIZE;
	       // memcpy() with a constant size should be replaced with native
	       // word-sized load/store operations making it much faster than a call
	       // with variable length. In memcpy suffix[code] will copy all the code
	       // related to the oldcode in a batch of 8 bytes.

	       memcpy(p, suffix[code], SK_MAX_DICTIONARY_WORD_SIZE);
	       code = prefix[code];
	       remaining -= SK_MAX_DICTIONARY_WORD_SIZE;
	     }
	     int firstchar = static_cast<unsigned char>(code);  // (strictly `suffix[code][0]`)
	     ...
	     int l = (codeLength - 1) & (SK_MAX_DICTIONARY_WORD_SIZE - 1);
	     ...
	     memcpy(suffix[avail], suffix[oldcode], SK_MAX_DICTIONARY_WORD_SIZE);
...
bool SkGIFLZWContext::prepareToDecode()
{
    ...
    // encodable sequence.
    const size_t kMaxSequence = SK_MAX_DICTIONARY_ENTRIES - 1;
    const size_t maxBytes = (kMaxSequence + SK_MAX_DICTIONARY_WORD_SIZE - 1)
                         & ~(SK_MAX_DICTIONARY_WORD_SIZE - 1);
    ...
    for (int i = 0; i < clearCode; ++i) {
        memset(suffix[i], 0, SK_MAX_DICTIONARY_WORD_SIZE);
        suffix[i][0] = i;
        suffixLength[i] = 1;
