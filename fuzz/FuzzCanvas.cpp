...
static void fuzz_canvas(Fuzz* fuzz, SkCanvas* canvas, int depth = 9) {
    ...
    for (unsigned i = 0; i < N; ++i) {
        ...
        switch (drawCommand) {
            ...
            case 62: {
                // TODO (Zepeng) make this code faster so that we can renable this
                /*
            ...
            case 64: {
                size_t size;
                bool useSkData;
                fuzz->next(&size, &useSkData);
                SkData* value;
                if (useSkData) {
                    value = SkData::MakeUninitialized(size).get();
                    fuzz->next(&value);
                }
                SkRect rect;
                fuzz->next(&rect, &value);
                const char* key = make_fuzz_string(fuzz);
                canvas->drawAnnotation(rect, key, useSkData ? value : nullptr);
                break;
            }
            default:
                SkASSERT(false);
