...
inline void set_any_enum_range(Fuzz* fuzz, SkPaint* paint, T rmin, T rmax) {
    int16_t value{};
    fuzz->nextRange(&value, SkToS16(rmin), SkToS16(rmax));
    (paint->*S)((T)value);
}
...
void FuzzPaint(Fuzz* fuzz, SkPaint* paint) {
    ...
    set_any_enum_range<SkBlendMode, &SkPaint::setBlendMode>(
            fuzz, paint, (SkBlendMode)0, SkBlendMode::kLastMode);
...
void FuzzCanvas(Fuzz* fuzz, SkCanvas* canvas) {
    if (!fuzz || !canvas) { return; }
    unsigned N{};
    fuzz->nextRange(&N, 0, 2000);
    for (unsigned i = 0; i < N; ++i) {
        if (fuzz->exhausted()) { return; }
        SkPaint paint;
