...
static sk_sp<SkShader> make_fuzz_shader(Fuzz* fuzz, int depth) {
    ...
    switch (shaderType) {
        ...
        case 3:
            ...
            fuzz->nextEnum(&tmX, 0, SkTileMode::kLastTileMode);
            fuzz->nextEnum(&tmY, 0, SkTileMode::kLastTileMode);
            fuzz->next(&useMatrix);
            if (useMatrix) {
...
inline T make_fuzz_enum_range(Fuzz* fuzz, Min minv, Max maxv) {
    T value;
...
static void fuzz_paint(Fuzz* fuzz, SkPaint* paint, int depth) {
    ...
    paint->setBlendMode(    make_fuzz_enum_range<SkBlendMode>(fuzz, 0, SkBlendMode::kLastMode));
    paint->setFilterQuality(make_fuzz_enum_range<SkFilterQuality>(fuzz, 0, kLast_SkFilterQuality));
    paint->setStyle(        make_fuzz_enum_range<SkPaint::Style>(fuzz, 0, 2));
    ...
    if (paint->getStyle() != SkPaint::kFill_Style) {
        ...
        paint->setStrokeCap(  make_fuzz_enum_range<SkPaint::Cap>(fuzz, 0, SkPaint::kLast_Cap));
        paint->setStrokeJoin( make_fuzz_enum_range<SkPaint::Join>(fuzz, 0, SkPaint::kLast_Join));
    }
}
...
static SkFont fuzz_font(Fuzz* fuzz) {
    ...
    font.setHinting(            make_fuzz_enum_range<SkFontHinting>(fuzz, 0, SkFontHinting::kFull));
    font.setEdging(             make_fuzz_enum_range<SkFont::Edging>(fuzz, 0,
                                                      (int)SkFont::Edging::kSubpixelAntiAlias));
    return font;
}

static SkTextEncoding fuzz_paint_text_encoding(Fuzz* fuzz) {
    return make_fuzz_enum_range<SkTextEncoding>(fuzz, 0, 3);
}
