...
static void add_white(Fuzz* fuzz, SkString* atom) {
    ...
    uint8_t reps;
    fuzz->nextRange<uint8_t>(&reps, 0, 2);
    for (uint8_t rep = 0; rep < reps; ++rep) {
        uint8_t index;
        fuzz->nextRange<uint8_t>(&index, 0, SK_ARRAY_COUNT(gWhiteSpace) - 1);
...
SkString MakeRandomParsePathPiece(Fuzz* fuzz) {
    SkString atom;
    uint8_t index;
    fuzz->nextRange<uint8_t>(&index, 0, SK_ARRAY_COUNT(gLegal) - 1);
    ...
    uint8_t reps;
    fuzz->nextRange<uint8_t>(&reps, 1, 3);
