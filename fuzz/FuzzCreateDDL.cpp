...
static SkColorType surfaceType;
static GrSurfaceOrigin origin;
...
static SkImageInfo gen_fuzzed_imageinfo(Fuzz* fuzz) {
...
static SurfaceAndChar create_surface_and_characterization(Fuzz* fuzz, GrDirectContext* dContext) {
    SkImageInfo ii = gen_fuzzed_imageinfo(fuzz);
...
DEF_FUZZ(CreateDDL, fuzz) {
    ...
    auto[surface, c] = create_surface_and_characterization(fuzz, dContext);
    if (!surface) {
        return;
    }
