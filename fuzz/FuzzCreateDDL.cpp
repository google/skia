...
static sk_sp<SkDeferredDisplayList> make_ddl(Fuzz* fuzz, GrDirectContext* dContext,
    ...
    if (!canvas) {
        SkDebugf("Could not create canvas %s", GrBackendApiToStr(dContext->backend()));
        return nullptr;
    }
...
static sk_sp<SkSurface> make_surface(Fuzz* fuzz, GrDirectContext* dContext, const SkImageInfo& ii) {
    ...
    auto surface = SkSurface::MakeRenderTarget(dContext, budget, ii, kSampleCount, origin, nullptr);
    return surface;
}
...
static SurfaceAndChar create_surface_characterization(Fuzz* fuzz, GrDirectContext* dContext) {
