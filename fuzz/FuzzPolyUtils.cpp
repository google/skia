...
DEF_FUZZ(PolyUtils, fuzz) {
    unsigned int count; //*** need to make polyutils routines take unsigned value
    //*** count can be really really big -- limit values that we accept?
    fuzz->next(&count);
...
//    ignoreResult(SkIsConvexPolygon(polygon, count));
    ...
    SkScalar offset;
    fuzz->next(&offset);
    ignoreResult(SkOffsetSimplePolygon(polygon, count, inset, &output));
