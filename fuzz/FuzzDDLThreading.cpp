...
struct PromiseImageInfo : SkRefCnt, SkNoncopyable {
    ...
    ~PromiseImageInfo() {
        // If we hit this, then the image or the texture is outliving this object which
        // we don't want.
        ...
        fTexture.reset();
        State s = fState;
        SkASSERT_RELEASE(s == State::kInitial || s == State::kDone);
    }
...
public:
    ...
    void doneWithPromiseImage(PromiseImageInfo&);
private:
    ...
    std::queue<sk_sp<SkPromiseImageTexture>> fReusableTextures SK_GUARDED_BY(fMutex);
...
DDLFuzzer::DDLFuzzer(Fuzz* fuzz, ContextType contextType) : fFuzz(fuzz) {
    ...
    fSurface->characterize(&fSurfaceCharacterization);
...
sk_sp<SkPromiseImageTexture> DDLFuzzer::fulfillPromiseImage(PromiseImageInfo& promiseImage) {
    ...
    bool tryReuse = make_fuzz_t<bool>(fFuzz);
    if (tryReuse) {
...
void DDLFuzzer::releasePromiseImage(PromiseImageInfo& promiseImage) {
    ...
    if (reuse) {
        ...
        if (this->isOnGPUThread()) {
            ...
            fGpuTaskGroup.add([this, tex = promiseImage.fTexture->backendTexture()]{
                fContext->deleteBackendTexture(tex);
            });
        }
    }
...
void DDLFuzzer::doneWithPromiseImage(PromiseImageInfo& promiseImage) {
    using State = PromiseImageInfo::State;
    State old = promiseImage.fState.exchange(State::kDone, std::memory_order_relaxed);
    if (old != State::kInitial && old != State::kDone) {
        fFuzz->signalBug();
    }
}
...
static void fuzz_promise_image_done(void* ctxIn) {
    PromiseImageInfo& fuzzPromiseImage = *(PromiseImageInfo*)ctxIn;
    fuzzPromiseImage.fFuzzer->doneWithPromiseImage(fuzzPromiseImage);
}

void DDLFuzzer::initPromiseImage(int index) {
    PromiseImageInfo& promiseImage = fPromiseImages[index];
    promiseImage.fFuzzer = this;
    promiseImage.fIndex = index;
    GrBackendFormat backendFmt = fContext->defaultBackendFormat(kRGBA_8888_SkColorType,
                                                               GrRenderable::kYes);
    promiseImage.fImage = fContext->makePromiseTexture(backendFmt,
                                                       kPromiseImageSize.width(),
                                                       kPromiseImageSize.height(),
                                                       GrMipMapped::kNo,
...
void DDLFuzzer::recordAndPlayDDL() {
    ...
    SkDeferredDisplayList* ddl = recorder.detach().release();
    fGpuTaskGroup.add([=]{
        bool success = fSurface->draw(ddl);
        if (!success) {
            fFuzz->signalBug();
        }
        // TODO: We shouldn't have to wait for the flush before deleting the DDL.
        // We need the drawing manager to ref the DDL so that we can unref it and
        // it can manage releasing the DDL's resources.
