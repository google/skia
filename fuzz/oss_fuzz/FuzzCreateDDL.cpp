...
static sk_sp<SkSurface> make_surface(GrDirectContext* dContext, const SkImageInfo& ii) {
    auto surface = SkSurface::MakeRenderTarget(dContext, SkBudgeted::kYes, ii,
        1, GrSurfaceOrigin::kBottomLeft_GrSurfaceOrigin, nullptr);
    return surface;
}
...
static std::tuple<sk_sp<SkSurface>, SkSurfaceCharacterization> create_surface_characterization(GrDirectContext* dContext) {
    ...
    auto surface = make_surface(dContext, ii);
    if (!surface) {
        return {};
    }
    return {surface, c};
...
bool FuzzCreateDDL(Fuzz* fuzz) {
    ...
    sk_gpu_test::ContextInfo ctxInfo = factory.getContextInfo(
        factory.ContextType::kGL_ContextType // TODO(zepeng) try to use other type
    );
    GrDirectContext* dContext = ctxInfo.directContext();

    std::cout << dContext << std::endl;

    auto[surface, c] = create_surface_characterization(dContext);
