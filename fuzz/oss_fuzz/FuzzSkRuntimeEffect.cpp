...
bool FuzzSkRuntimeEffect(sk_sp<SkData> bytes) {
    sk_sp<SkData> codeBytes = SkData::MakeSubset(bytes.get(), 0, bytes->size() - 256);
    ...
    if (!effect || effect->inputSize() > 256) { // if there is not enough uniform bytes to be passed in
        return false;
    }
