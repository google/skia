...
def skia_select(conditions, results):
    """Replaces select() for conditions [UNIX, ANDROID, IOS, WASM]
        ...
        Fourth argument temporarily optional until all uses can be updated.
    ...
    """
    # TODO(nifong): re-enable check of argument length after submitting both skia-side and google3-side changes.
