...
Error ViaDDL::draw(const Src& src, SkBitmap* bitmap, SkWStream* stream, SkString* log) const {
    ...
    auto draw = [&](SkCanvas* canvas) -> Error {
        ...
        for (int replay = 0; replay < fNumReplays; ++replay) {
            ...
            // This recreates the promise SkImages on each replay iteration. We are relying on this
            // currently to test reusing the same SkPromiseImageTexture fulfilling different
            // SkImages on each replay.
