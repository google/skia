...
Result GMSrc::hailMary(GrContext*) const {
    return Result(Result::Status::Ok, "");

#if 0
    std::unique_ptr<skiagm::GM> gm(fFactory());
    SkString msg;
    skiagm::DrawResult drawResult = gm->hailMary(nullptr, &msg);
    switch (drawResult) {
...
Result GMSrc::draw7(GrContext* context, SkCanvas* canvas) const {
    std::unique_ptr<skiagm::GM> gm(fFactory());
    SkString msg;

    skiagm::DrawResult hailMaryResult = gm->hailMary(context, &msg);
...
Result BRDSrc::draw7(GrContext* context, SkCanvas* canvas) const {
...
Result CodecSrc::draw7(GrContext* context, SkCanvas* canvas) const {
...
Result AndroidCodecSrc::draw7(GrContext* context, SkCanvas* canvas) const {
    sk_sp<SkData> encoded(SkData::MakeFromFileName(fPath.c_str()));
    if (!encoded) {
        return Result::Fatal("Couldn't read %s.", fPath.c_str());
    }
...
Result ImageGenSrc::draw7(GrContext* context, SkCanvas* canvas) const {
...
Result ColorCodecSrc::draw7(GrContext* context, SkCanvas* canvas) const {
    sk_sp<SkData> encoded(SkData::MakeFromFileName(fPath.c_str()));
    if (!encoded) {
        return Result::Fatal("Couldn't read %s.", fPath.c_str());
    }
...
Result SKPSrc::draw7(GrContext* context, SkCanvas* canvas) const {
...
Result BisectSrc::draw7(GrContext* context, SkCanvas* canvas) const {
    struct FoundPath {
        SkPath fPath;
        SkPaint fPaint;
    ...
    Result result = this->INHERITED::draw7(context, &pathFinder);
    if (!result.isOk()) {
        return result;
    }
...
Result SkottieSrc::draw7(GrContext* context, SkCanvas* canvas) const {
...
Result SVGSrc::draw7(GrContext* context, SkCanvas* canvas) const {
    if (!fDom) {
...
Result MSKPSrc::draw7(GrContext* context, SkCanvas* c) const {
    return this->draw11(0, context, c);
}
Result MSKPSrc::draw11(int i, GrContext* context, SkCanvas* canvas) const {
    if (this->pageCount() == 0) {
...
Result NullSink::draw(const Src& src, SkBitmap*, SkWStream*, SkString*) const {
    return src.draw7(nullptr, SkMakeNullCanvas().get());
}
...
Result GPUSink::onDraw(const Src& src, SkBitmap* dst, SkWStream*, SkString* log,
    ...
    SkCanvas* canvas = surface->getCanvas();
    Result result = src.draw7(context, canvas);
    if (!result.isOk()) {
        return result;
    }
...
Result GPUOOPRSink::ooprDraw(const Src& src,
    ...
    Result result = src.hailMary(context);
    ...
    result = src.draw7(context, recorder.getCanvas());
    if (!result.isOk()) {
        return result;
    }
...
Result GPUDDLSink::ddlDraw(const Src& src,
    ...
    Result result = src.draw7(gpuThreadCtx, recorder.beginRecording(SkIntToScalar(size.width()),
...
static Result draw_skdocument(const Src& src, SkDocument* doc, SkWStream* dst) {
    ...
    for (int i = 0; i < pageCount; ++i) {
        ...
        Result result = src.draw11(i, nullptr, canvas);
        if (!result.isOk()) {
            return result;
        }
        doc->endPage();
...
Result SKPSink::draw(const Src& src, SkBitmap*, SkWStream* dst, SkString*) const {
    ...
    SkPictureRecorder recorder;
    Result result = src.draw7(nullptr, recorder.beginRecording(size.width(), size.height()));
    if (!result.isOk()) {
        return result;
    }
...
Result DebugSink::draw(const Src& src, SkBitmap*, SkWStream* dst, SkString*) const {
    ...
    Result result = src.draw7(nullptr, &debugCanvas);
    if (!result.isOk()) {
        return result;
    }
...
Result SVGSink::draw(const Src& src, SkBitmap*, SkWStream* dst, SkString*) const {
    ...
    return src.draw11(fPageIndex, nullptr,
...
Result RasterSink::draw(const Src& src, SkBitmap* dst, SkWStream*, SkString*) const {
    ...
    SkCanvas canvas(*dst);
    return src.draw7(nullptr, &canvas);
}
...
static Result draw_to_canvas(Sink* sink, SkBitmap* bitmap, SkWStream* stream, SkString* log,
    ...
    public:
        ...
        Result  draw7(GrContext* context, SkCanvas* canvas) const override { return fDraw(canvas); }
...
Result ViaSerialization::draw(
    ...
    auto size = src.size();
    SkPictureRecorder recorder;
    Result result = src.draw7(nullptr, recorder.beginRecording(SkIntToScalar(size.width()),
                                                               SkIntToScalar(size.height())));
    if (!result.isOk()) {
        return result;
    }
