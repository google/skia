...
Result BRDSrc::draw(GrContext* context, SkCanvas* canvas) const {
...
Result CodecSrc::draw(GrContext* context, SkCanvas* canvas) const {
...
Result AndroidCodecSrc::draw(GrContext* context, SkCanvas* canvas) const {
    sk_sp<SkData> encoded(SkData::MakeFromFileName(fPath.c_str()));
    if (!encoded) {
        return Result::Fatal("Couldn't read %s.", fPath.c_str());
    }
...
Result ImageGenSrc::draw(GrContext* context, SkCanvas* canvas) const {
...
Result ColorCodecSrc::draw(GrContext* context, SkCanvas* canvas) const {
    sk_sp<SkData> encoded(SkData::MakeFromFileName(fPath.c_str()));
    if (!encoded) {
        return Result::Fatal("Couldn't read %s.", fPath.c_str());
    }
...
Result SKPSrc::draw(GrContext* context, SkCanvas* canvas) const {
...
Result SkottieSrc::draw(GrContext* context, SkCanvas* canvas) const {
...
Result SVGSrc::draw(GrContext* context, SkCanvas* canvas) const {
    if (!fDom) {
...
Result MSKPSrc::draw(int i, GrContext* context, SkCanvas* canvas) const {
    if (this->pageCount() == 0) {
...
static Result draw_to_canvas(Sink* sink, SkBitmap* bitmap, SkWStream* stream, SkString* log,
    ...
    public:
        ...
        Result  draw(GrContext* context, SkCanvas* canvas) const override { return fDraw(canvas); }
