...
sk_sp<SkPicture> read_skp(const char* path) {
    std::unique_ptr<SkStream> stream = SkStream::MakeFromFile(path);
    if (!stream) {
        return nullptr;
    }
...
static const SkRect kSKPViewport = {0, 0, 4000, 4000};
...
static const int kNumDDLYTiles = 8;
...
Error DDLSKPSrc::draw(SkCanvas* canvas) const {
    ...
    public:
        ...
        void preprocess(SkPicture* pic) {
            SkDeferredDisplayListMaker maker(fCharacterization);

            SkCanvas* subCanvas = maker.canvas();
    ...
    const SkIRect cullRect = pic->cullRect().roundOut();
    ...
    for (int y = 0; y < kNumDDLYTiles; ++y) {
        for (int x = 0; x < kNumDDLXTiles; ++x) {
            SkIRect clip = SkIRect::MakeXYWH(x * kDDLTileSize, y * kDDLTileSize,
