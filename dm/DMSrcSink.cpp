...
//        gpuTaskGroup->wait();
    ...
    DDLTileHelper tiles(context, dstCharacterization, viewport, kNumDivisions);

    tiles.createBackendTextures(nullptr, context);

    // Reinflate the compressed picture individually for each thread.
    sk_sp<SkPicture> pic = promiseImageHelper.reinflateSKP2(context, compressedPictureData.get());

    tiles.kickOffThreadedWork(nullptr, nullptr, context, pic.get());
...
//    SkTaskGroup gpuTaskGroup(*fGPUThread);
    ...
    Result result = this->ddlDraw(src, surface, mainCtx); // &recordingTaskGroup, &gpuTaskGroup,
//                                  mainTestCtx, mainCtx);
    if (!result.isOk()) {
        return result;
    }
