...
Error ViaDDL::draw(const Src& src, SkBitmap* bitmap, SkWStream* stream, SkString* log) const {
    ...
    auto draw = [&](SkCanvas* canvas) -> Error {
        ...
        // We draw N times, with a clear between. Between each run we invalidate/delete half of the textures
        // backing promise images. So half the images exercise reusing a cached GrTexture and the other half
        // exercise the case whem the client provides a different backing texture in fulfill.
