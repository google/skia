...
Error GPUPrecompileTestingSink::draw(const Src& src, SkBitmap* dst, SkWStream* wStream,
    ...
    // A very small number of GMs generate so many shaders that pre-compiling them won't help,
    // because we just end up overflowing the (fixed size) runtime program cache, then
