...
Result GPUDDLSink::ddlDraw(const Src& src,
    ...
    sk_sp<SkPicture> newSKP = promiseImageHelper.recreateSKP(contexts[0].directContext(),
                                                             inputPicture.get());
    if (!newSKP) {
    ...
    DDLTileHelper tiles(contexts[0].directContext(),
                        dstCharacterization, viewport,
                        kNumDivisions, kNumDivisions,
                        /* addRandomPaddingToDst */ false);

    tiles.createBackendTextures(gpuTaskGroup, contexts[0].directContext());

    tiles.kickOffThreadedWork(recordingTaskGroup, gpuTaskGroup, contexts[0].directContext(), newSKP.get());
...
Result GPUDDLSink::draw(const Src& src, SkBitmap* dst, SkWStream*, SkString* log) const {
    ...
    std::array<ContextInfo, kNumRecordingThreads+1> contexts;
