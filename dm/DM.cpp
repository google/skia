...
public:
     ...
     */
    sk_sp(const sk_sp<T>& that) : fPtr(SkSafeRef(that.get())) { SkDebugf("copy\n"); }
    template <typename U,
              typename = typename std::enable_if<std::is_convertible<U*, T*>::value>::type>
    sk_sp(const sk_sp<U>& that) : fPtr(SkSafeRef(that.get())) { SkDebugf("copy\n"); }
     ...
     */
    sk_sp(sk_sp<T>&& that) : fPtr(that.release()) { SkDebugf("move\n"); }
    template <typename U,
              typename = typename std::enable_if<std::is_convertible<U*, T*>::value>::type>
    sk_sp(sk_sp<U>&& that) : fPtr(that.release()) { SkDebugf("move\n"); }
...
struct B : public SkRefCnt {
};
struct D : public B {
};

sk_sp<B> test() {
    fake_sksp::sk_sp<D> d(new D());
    return d;
}
}
