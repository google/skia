...
// sizeof is a multiple of GrMemoryPool::kAlignment for 8 or 16 byte alignment
using Aligned = std::aligned_storage<48, GrMemoryPool::kAlignment>::type;
static_assert(sizeof(Aligned) == 48);
...
// if it's 8 or 16, as desired).
using Unaligned = std::aligned_storage<36, 4>::type;
static_assert(sizeof(Unaligned) == 36);
static_assert(GrMemoryPool::kAlignment != 4);
static_assert(sizeof(Unaligned) % GrMemoryPool::kAlignment != 0);

// When max_align_t == 16, the padded Unaligned will also be 48, but if max_align_t == 8, then
// it will be padded to only 40.
