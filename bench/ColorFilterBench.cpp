...
static sk_sp<SkImageFilter> make_brightness(float amount, sk_sp<SkImageFilter> input) {
    ...
    SkScalar matrix[20] = { 1, 0, 0, 0, amount255,
                            ...
                            0, 0, 1, 0, amount255,
                            0, 0, 0, 1, 0 };
    sk_sp<SkColorFilter> filter(SkColorFilterMaker::MatrixRowMajor255(matrix));
...
static sk_sp<SkImageFilter> make_mode_blue(sk_sp<SkImageFilter> input) {
    sk_sp<SkColorFilter> filter(SkColorFilterMaker::Blend(SK_ColorBLUE, SkBlendMode::kSrcIn));
    return SkColorFilterImageFilter::Make(std::move(filter), std::move(input));
}
...
protected:
    ...
    void onDelayedSetup() override {
        ...
        auto cf0 = SkColorFilterMaker::Blend(0x88FF3366, SkBlendMode::kDstIn);
        auto cf1 = SkColorFilterMaker::Blend(0x88FF3366, SkBlendMode::kDstATop);
        ...
        fFilter = SkColorFilterMaker::Mixer(mx, cf0, cf1);
    }
    void onDraw(int loops, SkCanvas* canvas) override {
