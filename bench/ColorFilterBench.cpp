...
DEF_BENCH( return new ColorFilterBench("src_runtime", []() {
        static sk_sp<SkRuntimeColorFilterEffect> gEffect =
                SkRuntimeColorFilterEffect::Make(SkString(RuntimeNone_GPU_SRC)).effect;
        sk_sp<SkColorFilter> input = nullptr;
        return gEffect->makeColorFilter(SkData::MakeEmpty(), &input, 1);
    });)
DEF_BENCH( return new ColorFilterBench("matrix_runtime", []() {
        static sk_sp<SkRuntimeColorFilterEffect> gEffect =
                SkRuntimeColorFilterEffect::Make(SkString(RuntimeColorMatrix_GPU_SRC)).effect;
        sk_sp<SkColorFilter> input = nullptr;
