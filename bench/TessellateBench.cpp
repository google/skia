...
DEF_PATH_TESS_BENCH(GrPathIndirectTessellator, make_cubic_path(), SkMatrix::I()) {
    GrPathIndirectTessellator tess(fMatrix, fPath, false);
    tess.prepare(fTarget.get(), fMatrix, fPath);
}

DEF_PATH_TESS_BENCH(GrPathHWOuterCurveTessellator, make_cubic_path(), SkMatrix::I()) {
    GrPathHWOuterCurveTessellator tess;
    tess.prepare(fTarget.get(), fMatrix, fPath);
}

DEF_PATH_TESS_BENCH(GrPathHWWedgeTessellator, make_cubic_path(), SkMatrix::I()) {
    GrPathHWWedgeTessellator tess;
    tess.prepare(fTarget.get(), fMatrix, fPath);
}
