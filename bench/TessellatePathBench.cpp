...
public:
    ...
    void* makeVertexSpace(size_t vertexSize, int vertexCount, sk_sp<const GrBuffer>*,
        ...
        if (vertexSize * vertexCount > sizeof(fStaticVertexData)) {
            SkString failMsg;
            failMsg.appendf("FATAL: wanted %llu bytes of static vertex data; only have %llu.\n",
                            vertexSize * vertexCount, SK_ARRAY_COUNT(fStaticVertexData));
            SK_ABORT(failMsg.c_str());
        }
    ...
    GrDrawIndexedIndirectCommand* makeDrawIndexedIndirectSpace(
        ...
        if (drawCount > staticBufferCount) {
            SkString failMsg;
            failMsg.appendf("FATAL: wanted %i static drawIndexedIndirect elements; only have %i.\n",
