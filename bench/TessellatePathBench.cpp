...
public:
    ...
    class prepareMiddleOutStencilGeometry_log2;
    class prepareOuterCubics;
    class prepareOuterCubics_log2;
    class prepareStencilWedges;
...
DEF_TESS_BENCH(prepareMiddleOutStencilGeometry, make_cubic_path(), SkMatrix::I(), target, op) {
    op->prepareMiddleOutStencilGeometry(target, nullptr, true);
}

DEF_TESS_BENCH(prepareMiddleOutStencilGeometry_log2, make_cubic_path(), SkMatrix::I(), target, op) {
    GrResolveLevelCounter resolveLevelCounter(op->fPath.countVerbs());
    op->prepareMiddleOutStencilGeometry(target, &resolveLevelCounter, false);
}

DEF_TESS_BENCH(prepareOuterCubics, make_cubic_path(), SkMatrix::I(), target, op) {
    SkPoint* vertexData = op->makeCubicBuffer(
            target, kNumCubicsInChalkboard,
            GrTessellatePathOp::CubicDataAlignment::kVertexBoundary);
    op->prepareOuterCubics(target, vertexData, nullptr);
}

DEF_TESS_BENCH(prepareOuterCubics_log2, make_cubic_path(), SkMatrix::I(), target, op) {
    GrResolveLevelCounter resolveLevelCounter(kNumCubicsInChalkboard);
    resolveLevelCounter.countAllBeziersInPath(4, op->fPath, SkMatrix::I());
    SkPoint* cubicData = op->makeCubicBuffer(target, resolveLevelCounter.totalCurveInstanceCount(),
                                             CubicDataAlignment::kInstanceBoundary);
    op->prepareOuterCubics(target, cubicData, &resolveLevelCounter);
}

DEF_TESS_BENCH(prepareStencilWedges, make_cubic_path(), SkMatrix::I(), target, op) {
    op->prepareStencilWedges(target);
}
...
DEF_TESS_BENCH(wangs_formula_cubic_log2_scale, make_cubic_path(), SkMatrix::MakeScale(1.1f, 0.9f),
               target, op) {
    benchmark_wangs_formula_cubic_log2(op->fViewMatrix, op->fPath);
}
