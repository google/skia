...
DEF_TESS_BENCH(prepareMiddleOutStencilGeometry, make_cubic_path(), SkMatrix::I(), target, op) {
    op->prepareMiddleOutTrianglesAndCubics(target, nullptr, true);
}
...
DEF_TESS_BENCH(prepareMiddleOutStencilGeometry_log2, make_cubic_path(), SkMatrix::I(), target, op) {
    ...
    op->prepareMiddleOutTrianglesAndCubics(target, &resolveLevelCounter, false);
}
...
DEF_TESS_BENCH(prepareOuterCubics_log2, make_cubic_path(), SkMatrix::I(), target, op) {
    ...
    op->prepareOuterCubics(target, resolveLevelCounter.totalCubicInstanceCount(),
                           CubicDataAlignment::kInstanceBoundary, &resolveLevelCounter);
}
