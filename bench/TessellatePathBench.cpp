...
public:
    WangsFormulaBench()
            : TestingOnly_Benchmark("wangs_formula_cubic_log2", make_cubic_path(), SkMatrix::I()) {
    }
    void runBench(GrOpFlushState* flushState, GrTessellatePathOp* op) override {
        for (auto [verb, pts, w] : SkPathPriv::Iterate(op->fPath)) {
            if (verb == SkPathVerb::kCubic) {
                GrWangsFormula::cubic_log2(4, pts);
            }
        }
    }
};

DEF_BENCH( return new GrTessellatePathOp::TestingOnly_Benchmark::WangsFormulaBench(); );
