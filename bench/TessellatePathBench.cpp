...
class GrTessellatePathOp::TestingOnly_Benchmark::WangsFormulaBench
        : public GrTessellatePathOp::TestingOnly_Benchmark {
public:
    WangsFormulaBench(GrVectorXform xform, const char* suffix)
    ...
    void runBench(GrOpFlushState* flushState, GrTessellatePathOp* op) override {
        ...
        for (auto [verb, pts, w] : SkPathPriv::Iterate(op->fPath)) {
            if (verb == SkPathVerb::kCubic) {
                sum += GrWangsFormula::cubic_log2(4, pts, fXform);
            }
        }
...
private:
    GrVectorXform fXform;
};

DEF_BENCH(
    return new GrTessellatePathOp::TestingOnly_Benchmark::WangsFormulaBench(GrVectorXform(), "");
);
DEF_BENCH(
    return new GrTessellatePathOp::TestingOnly_Benchmark::WangsFormulaBench(
            GrVectorXform(SkMatrix::MakeScale(1.1f, 0.9f)), "_scale");
);
DEF_BENCH(
    return new GrTessellatePathOp::TestingOnly_Benchmark::WangsFormulaBench(
            GrVectorXform(SkMatrix::MakeAll(.9f,0.9f,0,  1.1f,1.1f,0, 0,0,1)), "_affine");
);
