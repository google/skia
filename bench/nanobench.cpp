...
struct GPUTarget : public Target {
    ...
    void endTiming() override {
        if (this->contextInfo.testContext()) {
            this->contextInfo.testContext()->flushAndWaitOnSync(contextInfo.context());
        }
    }
    ...
    void fillOptions(NanoJSONResultsWriter& log) override {
        ...
        if (this->contextInfo.backend() == GrBackendApi::kOpenGL) {
            const GrGLInterface* gl =
                    static_cast<GrGLGpu*>(this->contextInfo.context()->priv().getGpu())
                            ->glInterface();
    ...
    void dumpStats() override {
        auto context = this->contextInfo.context();
