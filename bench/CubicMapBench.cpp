...
struct CubicMapNext3 {
    CubicMapNext3(SkPoint P1, SkPoint P2) {
        ...
        // (-QB + sqrt(QB2 - 4QA(QC - x)) / 2QA

        // {{qa->8*(h-2*q),qb->-2*(h-4*q),qc->0}}
        fQA[0]  = 8 * (half - 2 * quarter);
        fQB[0]  = -2 * (half - 4 * quarter);
        fQB2[0] = fQB[0] * fQB[0];
        fQC[0]  = 0;

        // {{qa -> 8*(1 + h - 2*qqq), qb -> -2*(5 + 7*h - 12*qqq), qc -> 3 + 6*h - 8*qqq}}
        fQA[1]  = 8 * (1 + half - 2 * threeQuarters);
        fQB[1]  = -2 * (5 + 7 * half - 12 * threeQuarters);
        fQB2[1] = fQB[1] * fQB[1];
        fQC[1]  = 3 + 6 * half - 8 * threeQuarters;
    }
    ...
    SkScalar computeYFromX(SkScalar x) {
        ...
        //(-QB + sqrt(QB2 - 4QA(QC - x)) / 2QA
        SkScalar t = (-fQB[i] + std::sqrtf(fQB2[i] - 4 * fQA[i]*(fQC[i] - x))) / (2 * fQA[i]);
        t = t - eval_poly(t, fA.fX, fB.fX, fC.fX, -x) / eval_poly(t, fDA, fDB, fDC);

        //SkDebugf("%g~%g ", x, eval_poly(t, fA.fX, fB.fX, fC.fX, 0));
    ...
    SkScalar fQA[2], fQB[2], fQB2[2], fQC[2];
};
