...
static SkNx<4,float> eval_poly(SkNx<4,float> t, SkNx<4,float> b) {
    return b;
}

template <typename... Rest>
static SkNx<4,float> eval_poly(SkNx<4,float> t, SkNx<4,float> m, SkNx<4,float> b, Rest... rest) {
    return eval_poly(t, SkNx_fma(m,t,b), rest...);
}
...
struct CubicMapNext3 {
    CubicMapNext3(SkPoint P1, SkPoint P2) {
        ...
        for (int i = 0; i < quadCount; i++) {
            ...
            float v1 = eval_poly(s1, A, B, C, D),
                  v2 = eval_poly(s2, A, B, C, D),
                  v3 = eval_poly(s3, A, B, C, D);

            fCuts[i] = v3;

            // qa = v1/((s1-s2)(s1-s3))-v2/((s1-s2)(s2-s3))+v3/((s1-s3)(s2-s3))
            // qb = -((s3+s2)v1)/((s1-s2)(s1-s3))+((s1+s3)v2)/((s1-s2)(s2-s3))-((s2+s1)v3)/((s1-s3)(s2-s3))
...
#else
        float A = fA.fX, B = fB.fX, C = fC.fX, D = -x;
    ...
    SkScalar fT[8], fU[8], fV[8], fW[8], fX[8];
    SkScalar fCuts[8];
