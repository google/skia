    ...
    void onDraw(int loops, SkCanvas* canvas) override {
        ...
        for (int i = 0; i < loops; i++) {
            ...
            switch (fOutput) {
                ...
                case Output::kGLSL:  SkAssertResult(fCompiler.toGLSL(*program,  &result));
                case Output::kMetal: SkAssertResult(fCompiler.toMetal(*program, &result));
                case Output::kSPIRV: SkAssertResult(fCompiler.toSPIRV(*program, &result));
            }
        }
    }
...
static const char* name ## _SRC = text;                                                          \
