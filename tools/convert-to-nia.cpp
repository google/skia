...
void writeNixHeader(uint32_t magicU32le, uint32_t width, uint32_t height) {
    uint8_t data[16];
...
bool writeNiaDuration(uint64_t totalDurationMillis) {
...
void writeNiePixels(uint32_t width, uint32_t height, const SkBitmap& bm) {
...
void writeNiaPadding(uint32_t width, uint32_t height) {
...
void writeNiaFooter(int repetitionCount, bool stillImage) {
    uint8_t data[8];
...
int main(int argc, char** argv) {
    ...
    for (int i = 0; i < frameCount; i++) {
        ...
        if (!firstFrameOnly) {
            if (i == 0) {
                writeNixHeader(0x41AFC36E,  // "nïA" magic string as a u32le.
                               bm.width(), bm.height());
            }

            if (!writeNiaDuration(totalDurationMillis)) {
                SkDebugf("Unsupported animation duration.\n");
                return 1;
            }
        }
        ...
        writeNixHeader(0x45AFC36E,  // "nïE" magic string as a u32le.
                       width, height);
        writeNiePixels(width, height, bm);
        ...
        writeNiaPadding(width, height);
    }
    writeNiaFooter(codec->getRepetitionCount(), stillImage);
    return 0;
}
