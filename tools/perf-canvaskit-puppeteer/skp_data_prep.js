...
for (const key of SKP_Set) {
    if (SIMD_DATA[key] && RELEASE_DATA[key]) {
        ...
        difference[key] = {
            total_frame_ms_average: SIMD_DATA[key].total_frame_ms_average - RELEASE_DATA[key].total_frame_ms_average,
            total_frame_ms_median: SIMD_DATA[key].total_frame_ms_median - RELEASE_DATA[key].total_frame_ms_median
        };
...
console.table({
    'Average frame time average': {
        ...
        'difference': (simd_total_frame_ms_average - release_total_frame_ms_average) / commonSKPDataCount
    },
    'Median frame time average': {
        ...
        'difference': (simd_total_frame_ms_median - release_total_frame_ms_median) / commonSKPDataCount
    }
});

const frameTimeMedianDifferenceSorted =
    Object.entries(difference).sort(
        ([key1, value1], [key2, value2]) => value1.total_frame_ms_median - value2.total_frame_ms_median
    );
