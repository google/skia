...
* Checks out ...
*...
* Uploads the Skia change to Android's internal Gerrit instance.
...
def get_change_details(change_num):
  response = urllib2.urlopen('https://skia-review.googlesource.com/changes/%s/detail?o=ALL_REVISIONS' % change_num, timeout=5)
...
"""
  ...
  # TODO(rmistry): Do we need the gn_to_bp.py stuff from
  # https://skia.googlesource.com/buildbot/+/master/autoroll/go/repo_manager/android_repo_manager.go#321
