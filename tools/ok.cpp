    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (0 == strcmp("-h", argv[i])) {
            printf("%s [-j n] [-m regex] [-s regex] [-w dir] [-h]    \n"
                   ...
                   "   -w: If set, write .pngs into this directory.  \n"
    ...
    for (auto r = skiagm::GMRegistry::Head(); r; r = r->next()) {
        spawn([=] {
            ...
            if (!write_dir.empty()) {
                ...
                for (auto segment : std::vector<std::string>{"/gm-8888", "/"+name+".png"}) {
                    // TODO: Make any directories we'll need ahead of time, not inside spawn().
