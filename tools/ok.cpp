...
template <typename T>
std::unique_ptr<T> move_unique(T& v) {
...
struct SWSink : Sink {
    ...
    static std::unique_ptr<Sink> Create(SkISize size, Options options) {
        ...
        SWSink sink;
        sink.surface = SkSurface::MakeRaster(info);
        return move_unique(sink);
    }
...
struct {
    const char* name;
    std::unique_ptr<Sink> (*factory)(SkISize, Options);
} sinks[] = {
    {"sw",  SWSink::Create },
};
...
int main(int argc, char** argv) {
    ...
    std::unique_ptr<Sink> (*sink_factory)(SkISize, Options) = nullptr;
    Options sink_options;

    auto help = [&] {
        std::string stream_types, sink_types;
        for (auto s : streams) {
        ...
        printf("%s [-j N] [-m regex] [-s regex] [-w dir] [-h] src[:k=v,...] sink[:k=v,...] \n"
                ...
                " sink: how to draw that content: %s                                       \n"
                " Some srcs and sinks have options, e.g. skp:dir=skps sw:ct=565            \n",
                argv[0], stream_types.c_str(), sink_types.c_str());
        return 1;
    };
    ...
    for (int i = 1; i < argc; i++) {
        ...
        for (auto s : sinks) {
            ...
            if (0 == strncmp(s.name, argv[i], len)) {
                switch (argv[i][len]) {
                    case  ':': len++;
                    case '\0': sink_factory = s.factory;
                               sink_options = Options{argv[i]+len};
                }
            }
        }
    }
    if (!stream || !sink_factory) { return help(); }
    ...
    for (std::unique_ptr<Src> owned = stream->next(); owned; owned = stream->next()) {
        ...
        spawn([=] {
            ...
            auto sink = sink_factory(src->size(), sink_options);

            auto canvas = sink->canvas();
            src->draw(canvas);
            canvas->restoreToCount(0);

            if (!write_dir.empty()) {
                sink->write(write_dir + "/" + name);
            }
            return Status::OK;
        });
    }
