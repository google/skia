...
void DDLTileHelper::TileData::createDDL(const sk_sp<SkPicture>& picture,
...
void DDLTileHelper::createDDLsInParallel() {
    ...
    SkTaskGroup().batch(this->numTiles(), [&](int i) {
        fTiles[i].createDDL(fReconstitutedPicture, fPromiseImages);
    });
...
void DDLTileHelper::kickOffThreadedWork(SkTaskGroup* recordingTaskGroup,
    ...
    for (int i = 0; i < this->numTiles(); ++i) {
        ...
        recordingTaskGroup->add([this, tile, gpuTaskGroup, gpuThreadContext]() {
                                    tile->createDDL(fReconstitutedPicture, fPromiseImages);
...
void DDLTileHelper::drawAllTilesDirectly(GrContext* context) {
    for (int i = 0; i < this->numTiles(); ++i) {
        fTiles[i].drawSKPDirectly(context, fReconstitutedPicture);
    }
}
