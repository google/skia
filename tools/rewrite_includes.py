...
# Rewrite any #includes of short names to either,
#
#   - "include/path/to/header.h" if the header is under include/
#   - "../relative/path/to/header.h" if not.
#
# This means Skia should generally be able to build with only one
# -I directive, to point it to where to find include/.
for root in roots:
  for path, dirs, files in os.walk(root):
    for file_name in files:
        ...
        with open(file_path, 'w') as output:
          for line in lines:
            ...
            if (len(parts) == 3
              ...
              # But some of our users want to be able to relocate include/.
              # So if we're crossing from outside include/ into it,
              # rewrite those paths relative to the directory above include/.
              # We want #include "include/core/SkCanvas.h",
              # not #include "../../include/core/SkCanvas.h".
