...
# with the exception of SkUserConfig.h which is a little tricky.
headers = {}
for root in roots:
  for path, _, files in os.walk(root):
    for file_name in files:
      if file_name.endswith('.h') and file_name != "SkUserConfig.h":
        ...
        with open(file_path, 'w') as output:
          for line in lines:
            ...
            if (len(parts) == 3
              ...
              print >>output, parts[0] + '"%s"' % os.path.relpath(header, path)
            else:
