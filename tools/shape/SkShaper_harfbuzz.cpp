...
public:
    static SkTLazy<BiDiRunIterator> Make(const char* utf8text, size_t textBytes, UBiDiLevel initialBidi) {
        ...
        ret.init(utf8text, std::move(bidi));
        return ret;
    }
    BiDiRunIterator(const char* utf8text, ICUBiDi bidi)
        : fBidi(std::move(bidi))
        , fEndOfCurrentRun(utf8text)
...
public:
    static SkTLazy<ScriptRunIterator> Make(const char* utf8text, size_t textBytes,
                                           hb_unicode_funcs_t* hbUnicode)
    {
        SkTLazy<ScriptRunIterator> ret;
        ret.init(utf8text, textBytes, hbUnicode);
        return ret;
    }
    ScriptRunIterator(const char* utf8text, size_t textBytes, hb_unicode_funcs_t* hbUnicode)
        : fCurrent(utf8text), fEnd(fCurrent + textBytes)
...
public:
    static SkTLazy<FontRunIterator> Make(const char* utf8text, size_t textBytes,
                                         sk_sp<SkTypeface> typeface,
                                         hb_font_t* hbFace,
                                         sk_sp<SkFontMgr> fallbackMgr)
    {
        SkTLazy<FontRunIterator> ret;
        ret.init(utf8text, textBytes, std::move(typeface), hbFace, std::move(fallbackMgr));
        return ret;
    }
    FontRunIterator(const char* utf8text, size_t textBytes, sk_sp<SkTypeface> typeface,
...
static constexpr bool is_RTL(UBiDiLevel level) {
    return (level & 1);
}
...
struct ShapedRunGlyphIterator {
        ...
        : fRuns(&origRuns), fRunIndex(0), fGlyphIndex(0)
    {
        ...
        if (runs.count() > fRunIndex && is_RTL(runs[fRunIndex].fLevel)) {
    ...
    ShapedGlyph* next() {
        ...
        if (is_LTR(runs[fRunIndex].fLevel)) {
            if (fGlyphIndex < runs[fRunIndex].fNumGlyphs - 1) {
                ...
                ++fRunIndex;
                if (is_RTL(runs[fRunIndex].fLevel)) {
                    fGlyphIndex = runs[fRunIndex].fNumGlyphs - 1;
                }
            }
        } else {
    ...
    SkTArray<ShapedRun> runs;
{
    ...
    SkTLazy<BiDiRunIterator> maybeBidi(BiDiRunIterator::Make(utf8text, textBytes, bidiLevel));
    ...
    SkTLazy<ScriptRunIterator> maybeScript(ScriptRunIterator::Make(utf8text, textBytes, hbUnicode));
    ...
    SkTLazy<FontRunIterator> maybeFont(FontRunIterator::Make(utf8text, textBytes,
                                                             fImpl->fTypeface,
    ...
    while (runSegmenter.advanceRuns()) {
        ...
        int32_t clusterOffset = utf8Start - utf8text;
        uint32_t previousCluster = 0xFFFFFFFF;
        for (unsigned i = 0; i < len; ++i) {
            ShapedGlyph& glyph = run.fGlyphs[is_LTR(run.fLevel) ? i : len - i - 1];
...
// The runs are in logical order, but the shaped glyphs within them are in visual order.
// Use a ShapedRunGlyphIterator to iterate over the glyphs in logical order to mark line endings.
{
...

{
    ...
    while (ShapedGlyph* glyph = glyphIterator.current()) {
        ...
        SkAutoTMalloc<UBiDiLevel> runLevels(numRuns);
        for (int i = 0; i < numRuns; ++i) {
            runLevels[i] = runs[previousBreak.fRunIndex + i].fLevel;
        }
        SkAutoTMalloc<int32_t> logicalFromVisual(numRuns);
        ...
        for (int i = 0; i < numRuns; ++i) {
            ...
            int startGlyphIndex;
            int endGlyphIndex;
            if (is_LTR(runs[logicalIndex].fLevel)) {
                startGlyphIndex = (logicalIndex == previousBreak.fRunIndex)
                                ? previousBreak.fGlyphIndex
                                : 0;
                endGlyphIndex = (logicalIndex == runIndex)
                              ? glyphIndex + 1
                              : runs[logicalIndex].fNumGlyphs;
            } else {
                startGlyphIndex = (logicalIndex == runIndex)
                                ? glyphIndex
                                : 0;
                endGlyphIndex = (logicalIndex == previousBreak.fRunIndex)
                              ? previousBreak.fGlyphIndex + 1
                              : runs[logicalIndex].fNumGlyphs;
            }
