...
public:
    static SkTLazy<BiDiRunIterator> Make(const icu::UnicodeString& utf16, UBiDiLevel initialBidi) {
        ...
        ret.init(std::move(bidi));
        return ret;
    }
    BiDiRunIterator(ICUBiDi bidi)
        : fBidi(std::move(bidi))
        , fEndOfCurrentRun(ubidi_getText(fBidi.get()))
        , fLevel(UBIDI_DEFAULT_LTR)
    {}
    void consume() override {
        SkASSERT(fEndOfCurrentRun - ubidi_getText(fBidi.get()) < ubidi_getLength(fBidi.get()));
        int32_t logicalPosition = fEndOfCurrentRun - ubidi_getText(fBidi.get());
        int32_t endPosition = ubidi_getLength(fBidi.get());
        fLevel = ubidi_getLevelAt(fBidi.get(), logicalPosition);
        ++logicalPosition;
        UBiDiLevel level;
        while (logicalPosition < endPosition) {
            level = ubidi_getLevelAt(fBidi.get(), logicalPosition);
            if (level != fLevel) {
                break;
            }
            ++logicalPosition;
        }
        fEndOfCurrentRun = ubidi_getText(fBidi.get()) + logicalPosition;
    }
    const UChar* endOfCurrentRun() const override {
        return fEndOfCurrentRun;
    }
    bool atEnd() const override {
        return fEndOfCurrentRun - ubidi_getText(fBidi.get()) == ubidi_getLength(fBidi.get());
    }
