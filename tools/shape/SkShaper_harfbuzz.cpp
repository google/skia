...
SkScalar SkShaper::shape(SkTextBlobBuilder* builder,
                         ...
                         SkPoint point) const {
    SkASSERT(builder);
    UBiDiDirection textDirection = UBIDI_LTR;
    ...
    UnicodeString utf16 = icu::UnicodeString::fromUTF8(icu::StringPiece(utf8text, textBytes));

    ICUBiDi bidi(ubidi_openSized(utf16.length(), 0, &errorCode));
    if (U_FAILURE(errorCode)) {
        SkDebugf("Bidi error: %s", u_errorName(errorCode));
        return (SkScalar)x;
    }
    SkASSERT(bidi);

    ubidi_setPara(bidi.get(), utf16.getBuffer(), utf16.length(),
    ...
    int32_t runCount = ubidi_countRuns(bidi.get(), &errorCode);
    if (U_FAILURE(errorCode)) {
        SkDebugf("Bidi error: %s", u_errorName(errorCode));
        return (SkScalar)x;
    }
