    ...
    for (int32_t i = 0; i < runCount; ++i) {
        ...
        while (utf16End < utf16.getBuffer() + start + length) {
            ...
            while (utf16Current < utf16End) {
                ...
                if (fallbackHBFont && fImpl->fTypeface->charsToGlyphs(&u, SkTypeface::kUTF32_Encoding, nullptr, 1)) {
                    fallback.reset();
                    doPartialRun = true;
                }
                if (!paint.getTypeface()->charsToGlyphs(&u, SkTypeface::kUTF32_Encoding, nullptr, 1)) {
                    fallback.reset(SkFontMgr::RefDefault()->matchFamilyStyleCharacter(nullptr, fImpl->fTypeface->fontStyle(), nullptr, 0, u));
                    if (fallback) {
                        doPartialRun = true;
                    }
                }
