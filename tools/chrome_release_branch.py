...
def get_current_milestone():
  ...
  sk_milestone = os.path.join(_TOOLS_DIR, os.pardir, SK_MILESTONE_H)
  ...
  print >> sys.stderr, 'Failed to parse SkMilestone.h; has the format changed?'
  sys.exit(1)
...
def create_new_branch(new_branch, branch_at):
  ...
  with git_utils.NewGitCheckout(SKIA_REPO):
    git.git('checkout', '-b', b)
    git.git('reset', '--hard', branch_at)
    git.git('push', '-u', 'origin', b)
