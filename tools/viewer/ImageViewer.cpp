...
void ImageViewer::drawImages(SkCanvas* canvas) {
    ...
    const int kPadH = 5;
    const int kPadV = kPadH + labelPaint.getFontSpacing();
    const int kMenuHeight = 20;

    // Get total width of images we're going to display
    int maxImgWidth = 0;
    int maxImgHeight = 0;
    int numImagesToDisplay = 0;
    for (int i = 0; i < fImages.count(); ++i) {
        if (fVisibleDirs[i]) {
            maxImgWidth = SkTMax(maxImgWidth, fImages[i].fImage->width());
            maxImgHeight = SkTMax(maxImgHeight, fImages[i].fImage->height());
            ++numImagesToDisplay;
        }
    }

    int windowWidth = fWindow->width();
    int windowHeight = fWindow->height() - kMenuHeight;
    fScale = 0.0f;
    int bestNumRows = 0;

    for (int numRows = 1; numRows <= numImagesToDisplay; ++numRows) {
        // What uniform scale factor is needed to get images to tile over this many rows?
        int imagesPerRow = (numImagesToDisplay + (numRows - 1)) / numRows;
        int availWidth = windowWidth - ((imagesPerRow + 1) * kPadH);
        int availHeight = windowHeight - ((numRows + 1) * kPadV);
        int totalImgWidth = maxImgWidth * imagesPerRow;
        int totalImgHeight = maxImgHeight * numRows;
        SkScalar hScale = SkIntToScalar(availWidth) / totalImgWidth;
        SkScalar vScale = SkIntToScalar(availHeight) / totalImgHeight;
        SkScalar scale = SkTMin(hScale, vScale);
        if (scale > fScale) {
            fScale = scale;
            bestNumRows = numRows;
        }
    ...
    canvas->translate(kPadH, kPadV + kMenuHeight);
    canvas->save();
    int imagesPerRow = bestNumRows ? (numImagesToDisplay + (bestNumRows - 1)) / bestNumRows : 0;
    int imagesOnThisRow = 0;
    for (int i = 0; i < fImages.count(); ++i) {
        if (fVisibleDirs[i]) {
            canvas->drawString(fImages[i].fLabel, 0, 0, labelPaint);
            canvas->save();
            canvas->translate(0, kPadH);
            canvas->scale(fScale, fScale);
            canvas->drawImage(fImages[i].fImage, 0, 0, &imgPaint);
            canvas->restore();
            if (++imagesOnThisRow >= imagesPerRow) {
                canvas->restore();
                canvas->translate(0, fImages[i].fImage->height() * fScale + kPadV);
                canvas->save();
                imagesOnThisRow = 0;
            } else {
                canvas->translate(fImages[i].fImage->width() * fScale + kPadH, 0);
            }
        }
    }
}
