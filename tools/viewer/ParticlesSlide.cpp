...
public:
    ...
    void apply(SkRandom&, SkPoint& position, SkVector&, SkVector& velocity, float elapsed) {
        ...
        toPoint.normalize();
        velocity += toPoint * (fConstant + (fInvSquare / lenSquare));
    }
...
public:
    ...
    // Update rules
    sk_sp<SkParticleAffector> fAffector;
...
public:
    ...
    void update(SkRandom& random, const SkAnimTimer& timer) {
        ...
        for (int i = 0; i < fCount; ++i) {
            ...
            if (fParams->fAffector) {
                fParams->fAffector->apply(random, fParticles[i].fPosition, fParticles[i].fRight,
