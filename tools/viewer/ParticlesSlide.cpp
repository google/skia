...
public:
    ...
    static void Register(const char* typeName, Factory factory) {
        SkASSERT(gTypeMap.find(SkString(typeName)) == nullptr);
        gTypeMap.set(SkString(typeName), factory);
    }

private:
    static SkTHashMap<SkString, Factory> gTypeMap;
};

#define REFLECT(TYPE)                                               \
    const char* getTypeName() const override { return #TYPE; }      \
    static sk_sp<SkReflected> CreateInstance() { return new TYPE; }

#define REGISTER_REFLECTED(TYPE) SkReflected::Register(#TYPE, TYPE::CreateInstance)
