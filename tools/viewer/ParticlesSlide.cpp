...
public:
    ...
    SkPoint emit(SkRandom& random) const {
...
public:
    ...
    void update(SkRandom& random, const SkAnimTimer& timer) {
        ...
        for (int i = 0; i < fCount; ++i) {
            ...
            // TODO: Speeds in units/second
            fXforms[i].fTx += random.nextRangeScalar(-0.5f, 0.5f);
            fXforms[i].fTy -= random.nextRangeScalar(0.5f, 1.5f);
        }
        ...
        for (int i = 0; i < numToSpawn; ++i) {
            ...
            SkPoint pt = fEmitter->emit(random);
            fXforms[fCount].set(1.0f, 0.0f, pt.fX, pt.fY);
...
ParticlesSlide::ParticlesSlide() {
    ...
    sk_sp<SkImage> image;
    {
        ...
        paint.setAlpha(96);
        surface->getCanvas()->drawCircle(1, 1, 1, paint);
