...
public:
    SkGuiVisitor(std::function<void(sk_sp<SkParticleEffectParams>)> playEffect)
            : fPlayEffect(std::move(playEffect)) {
...
#undef IF_OPEN
    ...
    void visit(sk_sp<SkReflected>& e, const SkReflected::Type* baseType) override {
        if (fTreeStack.back()) {
            ...
            if (e && baseType == SkParticleEffectParams::GetType()) {
                fPlayEffect(sk_ref_sp(static_cast<SkParticleEffectParams*>(e.get())));
            }
        }
    }
...
private:
    ...
    std::function<void(sk_sp<SkParticleEffectParams>)> fPlayEffect;
};
...
void ParticlesSlide::loadLibrary(const char* filename) {
    ...
    fEffects = SkParticleEffectLibrary::MakeFromFileName(path.c_str());
    if (!fEffects) {
        fEffects.reset(new SkParticleEffectLibrary());
    }
    fRunning.reset();
}
...
//        }
        ...
        auto effectCallback = [](sk_sp<SkParticleEffectParams>)
        SkGuiVisitor gui;
        fEffects->visitFields(&gui);
    }
    ImGui::End();
