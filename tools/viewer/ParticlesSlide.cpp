...
public:
    ...
    SkRSXform emit(SkRandom& random, float cx, float cy) const {
        ...
        const float c = tan.fY;
        const float s = -tan.fX;
        return SkRSXform::Make(c, s, pos.fX + -c * cx + s * cy, pos.fY + -s * cx - c * cy);
    }
...
void ParticlesSlide::buildEmitter() {
    ...
    SkTextUtils::GetPath(fEmitterText, strlen(fEmitterText), kUTF8_SkTextEncoding, 0, 0, font,
                         &fPath);
    auto emitter = skstd::make_unique<SkParticleEmitter>(fPath);
