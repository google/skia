...
struct SkCurve {
    SkCurve() {
        fSegments.push_back(sk_sp<SkCurveSegment>(new SkCurveSegment));
    }
...
public:
    ...
    virtual void apply(SkRandom& random, SkParticlePoseAndVelocity& pv, float elapsed) = 0;
};
...
public:
    ...
    void apply(SkRandom&, SkParticlePoseAndVelocity& pv, float elapsed) override {
        pv.fVelocity.fLinear += fForce * elapsed;
    }
...
public:
    ...
    void apply(SkRandom&, SkParticlePoseAndVelocity& pv, float elapsed) override {
        ...
        toPoint.normalize();
        pv.fVelocity.fLinear += toPoint * (fConstant + (fInvSquare / lenSquare)) * elapsed;
    }
...
public:
    ...
    void apply(SkRandom& random, SkParticlePoseAndVelocity& pv, float elapsed) override {
        pv.fVelocity.fLinear.fX += fX.eval(random);
    }
    ...
    void update(SkRandom& random, const SkAnimTimer& timer) {
        ...
        for (int i = 0; i < fCount; ++i) {
            ...
            for (auto affector : fParams->fAffectors) {
                if (affector) {
                    affector->apply(random, fParticles[i].fPV, elapsed);
                }
            }
