...
public:
    ...
    skottie::Animation* animation() { return fAnimation.get(); }
...
bool SkottieSlide2::animate(const SkAnimTimer& timer) {
    for (auto& rec : fAnims) {
        ...
        rec.fWrapper->animation()->animationTick(timer.msec() - rec.fTimeBase);
    }
    return true;
}
...
bool SkottieSlide2::onMouse(SkScalar x, SkScalar y, sk_app::Window::InputState state,
    ...
    if (fTrackingCell >= 0 && state == sk_app::Window::kUp_InputState) {
        ...
        if (fTrackingCell == index) {
            ...
            fAnims[index].fWrapper->animation()->setShowInval(fAnims[index].fShowAnimationInval);
        }
        fTrackingCell = -1;
    }
