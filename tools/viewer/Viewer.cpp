...
static ImVec2 ImGui_DragPoint(const char* label, float* x, float* y,
...
static void ImGui_Primaries(SkColorSpacePrimaries* primaries, SkPaint* gamutPaint) {
    ...
    // Primary markers
    ImVec2 r = ImGui_DragPoint("R", &primaries->fRX, &primaries->fRY, pos, size);
    ImVec2 g = ImGui_DragPoint("G", &primaries->fGX, &primaries->fGY, pos, size);
    ImVec2 b = ImGui_DragPoint("B", &primaries->fBX, &primaries->fBY, pos, size);
    ImVec2 w = ImGui_DragPoint("W", &primaries->fWX, &primaries->fWY, pos, size);
