    ...
    fCommands.addCommand('c', "Modes", "Cycle color mode", [this]() {
        switch (fColorMode) {
            case ColorMode::kLegacy:
                this->setColorMode(ColorMode::kLegacyPlusPlus);
                break;
            case ColorMode::kLegacyPlusPlus:
...
void Viewer::updateTitle() {
    ...
    switch (fColorMode) {
        case ColorMode::kLegacy:
            title.append(" N32 Legacy");
            break;
        case ColorMode::kLegacyPlusPlus:
            title.append(" N32 Legacy++");
            break;
        case ColorMode::kColorManaged8888:
            title.append(" N32 ColorManaged");
            break;
        case ColorMode::kColorManagedF16:
            title.append(" F16 ColorManaged");
            break;
    }
...
void Viewer::drawSlide(SkCanvas* canvas) {
    ...
    // If we're in F16 or legacy++ (crazy flag), or the gamut isn't sRGB, or we're zooming,
    // we need to render offscreen
    bool colorManaged = (ColorMode::kLegacy != fColorMode);
    sk_sp<SkSurface> offscreenSurface = nullptr;
    if (ColorMode::kColorManagedF16 == fColorMode || ColorMode::kLegacyPlusPlus == fColorMode ||
        fShowZoomWindow ||
        (colorManaged && !primaries_equal(fColorSpacePrimaries, gSrgbPrimaries))) {
        sk_sp<SkColorSpace> cs = nullptr;
        if (colorManaged) {
            SkColorSpace::RenderTargetGamma transferFn = (ColorMode::kColorManagedF16 == fColorMode)
            ...
            uint32_t flags = (ColorMode::kLegacyPlusPlus == fColorMode)
    ...
    if (offscreenSurface) {
        ...
        sk_sp<SkColorSpace> cs = (ColorMode::kColorManagedF16 == fColorMode)
    ...
    if (fShowImGuiDebugWindow) {
        ...
        if (ImGui::Begin("Tools", &fShowImGuiDebugWindow,
            ...
            if (ImGui::CollapsingHeader("Color Mode")) {
                ...
                cmButton(ColorMode::kLegacy, "Legacy");
                cmButton(ColorMode::kLegacyPlusPlus, "Legacy++");
