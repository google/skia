...
static bool on_mouse_handler(int x, int y, Window::InputState state, uint32_t modifiers,
                             void* userData) {
//    Viewer* viewer = reinterpret_cast<Viewer*>(userData);
    // TODO: Pass mouse data along to viewer... etc
    gMouseX = x;
    gMouseY = y;
    switch (state) {
...
//            fCommands.attach(fWindow);
    ...
    gImguiFontBitmap.allocPixels(SkImageInfo::Make(w, h, kRGBA_8888_SkColorType,
                                                   kPremul_SkAlphaType));
    unsigned int* rgba = (unsigned int*)pixels;
    for (int i = 0; i < w * h; ++i) {
        // Premul...
        unsigned int a = rgba[i] >> 24;
        rgba[i] = a << 24 | a << 16 | a << 8 | a;

    }
//    io.Fonts->GetTexDataAsAlpha8(&pixels, &w, &h);
//    gImguiFontBitmap.allocPixels(SkImageInfo::Make(w, h, kAlpha_8_SkColorType,
//                                                   kPremul_SkAlphaType));
    memcpy(gImguiFontBitmap.getPixels(), pixels, gImguiFontBitmap.getSize());
...
void Viewer::onPaint(SkCanvas* canvas) {
    ...
    io.DisplaySize.x = static_cast<float>(fWindow->width());
    io.DisplaySize.y = static_cast<float>(fWindow->height());
    io.MousePos.x = static_cast<float>(gMouseX);
    io.MousePos.y = static_cast<float>(gMouseY);
    io.MouseDown[0] = gMouseButton0;
        ...
        for (int j = 0; j < drawList->CmdBuffer.size(); ++j) {
            ...
            if (drawCmd->UserCallback) {
                ...
                canvas->drawVertices(SkCanvas::kTriangles_VertexMode, drawList->VtxBuffer.size(),
                                     pos.begin(), uv.begin(), color.begin(), nullptr,
