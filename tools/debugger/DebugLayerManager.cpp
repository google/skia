...
void DebugLayerManager::storeSkPicture(int nodeId, sk_sp<SkPicture> picture, SkIRect dirty) {
  ...
  SkDebugf("create debugcanvas for drawEvent {%d, %d}\n", fFrame, nodeId);
  auto debugCanvas = std::make_unique<DebugCanvas>(layerBounds, this);
  // Only draw picture to the debug canvas once.
  SkDebugf("drawPicture\n");
  debugCanvas->drawPicture(picture);
  SkDebugf("getsize\n");
  int numCommands = debugCanvas->getSize();

  SkDebugf("make struct\n");
  DrawEvent event = {
...
sk_sp<SkImage> DebugLayerManager::getLayerAsImageFromFrame(const int nodeId, const int frame) {
  ...
  if (drawEvent.image) { return drawEvent.image; }
  ...
  auto surface = SkSurface::MakeRaster(SkImageInfo::Make(drawEvent.layerBounds,
    kAlpha_8_SkColorType, kUnpremul_SkAlphaType, nullptr));
