...
std::vector<DebugLayerManager::DrawEventSummary> DebugLayerManager::summarizeEvents() const {
    std::vector<DrawEventSummary> result;
  for (const auto& node : listNodesForFrame(fFrame)) {
    auto* evt = fDraws.find({fFrame, node});
    if (!evt) { continue; }
    result.push_back({
