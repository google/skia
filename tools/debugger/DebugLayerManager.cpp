...
void DebugLayerManager::setCommand(int nodeId, int frame, int command) {
  ...
  if (!drawEvent) {
    SkDebugf("Could not set command playhead for event {%d, %d}, it is not tracked by DebugLayerManager.\n",
      frame, nodeId);
    return;
  }
...
void DebugLayerManager::storeSkPicture(int nodeId, sk_sp<SkPicture> picture, SkIRect dirty) {
  const LayerKey k = {fFrame, nodeId};
  ...
  // Setting this allows a layer to contain another layer
  debugCanvas->setLayerManager(this);
  // Only draw picture to the debug canvas once.
  debugCanvas->drawPicture(picture);
  int numCommands = debugCanvas->getSize();

  DrawEvent event = {
    fFrame == 0 || dirty==layerBounds,            // fullReDraw
    nullptr,                                      // image
...
sk_sp<SkImage> DebugLayerManager::getLayerAsImageFromFrame(const int nodeId, const int frame) {
  ...
  SkDebugf("\ngetLayerAsImageFromFrame {frame:%d, node:%d}\n", frame, nodeId);
  ...
  if (drawEvent.image) {
    SkDebugf("Return cached iamge from {frame:%d, node:%d}\n", frameN, nodeId);
  ...
  for (; i<relevantFrames.size() && relevantFrames[i]<=frameN; i++) {
    SkDebugf("Drawing event {frame:%d, node:%d} frameN=%d, i=%d\n", relevantFrames[i], nodeId, frameN, i);
    fDraws[{relevantFrames[i], nodeId}].debugCanvas->drawTo(canvas, drawEvent.command);
    canvas->flush();
  }
...
std::vector<int> DebugLayerManager::listNodesForFrame(const int frame) const {
...
std::vector<int> DebugLayerManager::listFramesForNode(const int nodeId) const {
  std::vector<int> result;
  for (const auto& key : keys) {
