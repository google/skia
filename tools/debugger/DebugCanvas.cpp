...
void DebugCanvas::onDrawImageRect(const SkImage*    image,
    ...
    if (fnextDrawImageRectLayerId != -1 && fLayerManager) {
        ...
        // To render the layer as it would have looked on the frame this DebugCanvas draws, can be
        // produced by calling fLayerManager->getLayerAsImage(id). This must be done just before
        // drawTo(command) is called, since it depends on the index into the layer's commands
