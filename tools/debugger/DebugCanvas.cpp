...
namespace {
    ...
    static SkPaint originXPaint = SkPaint({1.0, 0, 0, 1.0});
    static SkPaint originYPaint = SkPaint({0, 1.0, 0, 1.0});
...
void DebugCanvas::drawTo(SkCanvas* originalCanvas, int index, int m) {
    ...
    if (fShowOrigin) {
        // Draw an origin cross at the origin before restoring to assist in visualizing the current matrix.
