...
static const char* color_type_name(SkColorType colorType) {
    const char* desc = ToolUtils::ColorTypeToString(colorType);
    SkASSERT(desc);
    return desc;
}

static const char* alpha_type_name(SkAlphaType alphaType) {
    const char* desc = ToolUtils::AlphaTypeToString(alphaType);
    SkASSERT(desc);
    return desc;
}
...
static void apply_paint_style(const SkPaint& paint, SkJSONWriter& writer) {
    ...
    if (style != SkPaint::kFill_Style) {
        const char* desc = ToolUtils::PaintStyleToString(style);
        SkASSERT(desc);
...
static void apply_paint_cap(const SkPaint& paint, SkJSONWriter& writer) {
    ...
    if (cap != SkPaint::kDefault_Cap) {
        const char* desc = ToolUtils::PaintCapToString(cap);
        SkASSERT(desc);
...
static void apply_paint_join(const SkPaint& paint, SkJSONWriter& writer) {
    ...
    if (join != SkPaint::kDefault_Join) {
        const char* desc = ToolUtils::PaintJoinToString(join);
        SkASSERT(desc);
...
static void apply_paint_filterquality(const SkPaint& paint, SkJSONWriter& writer) {
    ...
    if (quality != kNone_SkFilterQuality) {
        const char* desc = ToolUtils::FilterQualityToString(quality);
        SkASSERT(desc);
