...
// A class to asist in playing back and debugging an mskp file containing offscreen layer commands.

// Holds SkPictures that have been used in hardware layers in one or more DebugCanvases. During
// recording of the mskp file on android, each layer had a RenderNode id, which is recorded with
// the layer's draw commands.
// Creates one surface (cpu or gpu at the user's request) for each layer, and renders
...
// layer as it was on any frame.

// For example an SkPicture may be valid for drawing RenderNode 32 from frame 0 until the next
// layer draw event, or the end of the animation, whichever comes first.

// Clients may ask for a rendering of a given layer by it's RenderNode id, and
// this class will return a rendering of how it looked on the frame curretly set at fFrame.
// returning an SkImage snapshot of the internally managed SkSurface.

class DebugCanvas;

class DebugLayerManager {
public:
    DebugLayerManager(int frameCount) : fFrameCount(frameCount) {};
    ...
    void storeSkPicture(int nodeId, sk_sp<SkPicture> picture, SkIRect dirty);

    // Set the frame number.
    // determines which set of commands to use during draw, and where to store new SkPictures.
    void setFrame(const int f) { fFrame = f; };

    // Set's the command playback head for a given picture/draw event
    void setCommand(int nodeId, int frame, int command);

    // getLayerAsImage draw the given layer as it would have looked on fFrame and returns an image.
    ...
    sk_sp<SkImage> getLayerAsImage(const int nodeId) {
        return getLayerAsImageFromFrame(nodeId, fFrame);
    };

    // Return the list of node ids which have DrawEvents on the given frame
    std::vector<int> listNodesForFrame(const int frame) const;
    // Return the list of frames on which the given node had DrawEvents.
    std::vector<int> listFramesForNode(const int nodeId) const;
...
private:
    ...
    struct LayerHash {
        uint32_t operator()(const LayerKey& k) { return (k.frame << 16) + k.nodeId; }
    };
    ...
    SkTHashMap<LayerKey, DrawEvent, LayerHash> fDraws;
    // The list of all keys in the map above (it has no keys() method)
    std::vector<LayerKey> keys;
    // private version of getLayerAsImage where you get to specify the frame number.
    sk_sp<SkImage> getLayerAsImageFromFrame(const int nodeId, const int frame);
};
