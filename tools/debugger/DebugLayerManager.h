...
// this class will return a rendering of how it looked on the frame currently set at fFrame.
...
public:
    ...
    // This statefulneess is here to simplify DebugCanvas, which is frame-unaware. Should this ever
    // need to be removed, it'll have to be tracked in DebugCanvas instead.
    ...
    // getLayerAsImage draws the given layer as it would have looked on fFrame and returns an image.
    ...
    // Mean to be bindable by emscripted and retuned to the javascript side
    ...
    std::vector<DrawEventSummary> summarizeEvents() const;
...
private:
    ...
    struct LayerHash {
        uint32_t operator()(const LayerKey& k) { return (k.frame << 1) + k.nodeId; }
    };
