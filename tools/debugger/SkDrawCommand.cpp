...
Json::Value SkDrawCommand::MakeJsonLattice(const SkCanvas::Lattice& lattice) {
    Json::Value result(Json::objectValue);
    result["XCount"] = Json::Value(lattice.fXCount);
    result["YCount"] = Json::Value(lattice.fYCount);
    if (nullptr != lattice.fBounds) {
        result["bounds"] = MakeJsonIRect(*lattice.fBounds);
    }
...
SkDrawImageLatticeCommand::SkDrawImageLatticeCommand(const SkImage* image, const SkCanvas::Lattice& lattice,
                                                     const SkRect& dst, const SkPaint* paint)
