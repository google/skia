...
static bool asset_exists(AssetManager* mgr, const char* path) {
    return mgr && nullptr != mgr->open(path);
}

static bool copy(AssetManager* mgr, const char* path, const char* dst) {
    if (mgr) {
...
static SkBitmap ReadPngRgba8888FromFile(AssetManager* assetManager, const char* path) {
    SkBitmap bitmap;
