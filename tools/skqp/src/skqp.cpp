...
static void readlist(SkQPAssetManager* mgr, const char* path,
    ...
    std::unique_ptr<SkStreamAsset> asset = mgr->open(path);
    ...
    while (true) {
        ...
        SkASSERT(find > ptr);
        dst->insert(std::string(ptr, find - ptr));
        ptr = find;
    }
}

static bool is_empty(const std::unordered_set<std::string>& set) {
    return set.empty();
}
static bool in_set(const char* s, const  std::unordered_set<std::string>& set) {
    return set.count(std::string(s)) > 0;
}
...
static std::vector<SkQP::UnitTest> get_unit_tests(
    ...
    for (const skiatest::TestRegistry* r = skiatest::TestRegistry::Head(); r; r = r->next()) {
        ...
        if ((knownGpuUnitTests.empty() || in_set(test.name, knownGpuUnitTests))
            && test.needsGpu) {
            tests.push_back(&test);
        }
    }
    struct {
        bool operator()(SkQP::UnitTest u, SkQP::UnitTest v) const {
...
static std::vector<SkQP::GMFactory> get_gms(const std::unordered_set<std::string>& knownGMs) {
    ...
    for (const skiagm::GMRegistry* r = skiagm::GMRegistry::Head(); r; r = r->next()) {
        ...
        if ((is_empty(knownGMs) || in_set(name.c_str(), knownGMs))) {
            gmsWithNames.push_back(GmAndName{f, std::move(name)});
        }
    }
    struct {
        bool operator()(GmAndName u, GmAndName v) const { return u.gName < v.gName; }
    } less;
    std::sort(gmsWithNames.begin(), gmsWithNames.end(), less);
...
static void copy_stream(SkStreamAsset* src, const std::string& dst) {
    SkFILEWStream wStream(dst.c_str());
    src->rewind();
    bool success = wStream.isValid() && SkStreamCopy(&wStream, src);
    SkASSERT_RELEASE(success);
}
...
void SkQP::init(std::unique_ptr<SkQPAssetManager> am, const char* reportDirectory,
    ...
    fExperimentalMode = experimentalMode;
    ...
    readlist(mgr, "skqp/KnownGpuUnitTests.txt", &fKnownGpuUnitTests);
    readlist(mgr, "skqp/KnownGMs.txt", &fKnownGMs);

    fUnitTests = get_unit_tests(fKnownGpuUnitTests);
    fGMs = get_gms(fKnownGMs);
...
std::tuple<int, int, std::string> SkQP::evaluateGM(SkQP::SkiaBackend backend,
    ...
    if (in_set(name, fDoNotScoreInCompatibilityTestMode)) {
    ...
    std::ostringstream tmp(imagesDirectory);
    tmp << '/' << SkQP::GetBackendName(backend) << '_' << name << '_';
...
const std::vector<std::string>& SkQP::executeTest(SkQP::UnitTest test) {
    SkASSERT_RELEASE(fAssetManager);
