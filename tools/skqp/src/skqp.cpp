...
static void get_render_tests(SkQPAssetManager* mgr,
    ...
    std::sort(gmsWithNames.begin(), gmsWithNames.end(),
              [](GmAndName u, GmAndName v) { return u.fName < v.fName; });
    gmlist->reserve(gmsWithNames.size());
    for (const GmAndName& gmn : gmsWithNames) {
        gmlist->push_back(gmn.fFact);
    }
}
...
std::tuple<SkQP::RenderOutcome, std::string> SkQP::evaluateGM(SkQP::SkiaBackend backend,
    ...
    SkASSERT_RELEASE(fAssetManager);
    static constexpr SkColorType ct = kRGBA_8888_SkColorType;
    ...
    int64_t passingThreshold = fGMThresholds.empty() ? 0 : fGMThresholds[std::string(name)];
    ...
    copy_stream(modelResult.fMaxPng.get(), imagesPathPrefix2 + PATH_MAX_PNG);
    copy_stream(modelResult.fMinPng.get(), imagesPathPrefix2 + PATH_MIN_PNG);
