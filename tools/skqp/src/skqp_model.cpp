...
static SkBitmap decode(std::unique_ptr<SkStreamAsset> asset) {
    SkBitmap bitmap;
    if (auto codec = SkCodec::MakeFromStream(std::move(asset))) {
...
skqp::ModelResult skqp::CheckAgainstModel(const char* name,
    ...
    SkBitmap max_image = decode(mgr->open(max_path.c_str()));
    SkBitmap min_image = decode(mgr->open(min_path.c_str()));
