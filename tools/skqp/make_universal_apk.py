...
def check_ninja():
    with open(os.devnull, 'w') as devnull:
        return 0 == subprocess.call(['ninja', '--version'], stdout=devnull, stderr=devnull)
...
def make_apk(architectures,
             ...
             debug,
             skia_dir):
    assert '/' in [os.sep, os.altsep]  # 'a/b' rather than os.path.join('a', 'b')
    assert check_ninja()
    assert os.path.exists(android_ndk)
    assert os.path.exists(android_home)
    assert os.path.exists(skia_dir)
    assert all(arch in skia_to_android_arch_name_map for arch in architectures)
    assert architectures

    os.chdir(skia_dir)

    # These are the locations in the tree where the gradle needs or will create
    # not-checked-in files.  Treat themi specially to keep the tree clean.
    build_paths = ['platform_tools/android/apps/.gradle',
                   'platform_tools/android/apps/skqp/build',
                   'platform_tools/android/apps/skqp/src/main/libs',
    ...
    shutil.rmtree('platform_tools/android/apps/%s/src/main/libs' % app, True)

    if os.path.exists('platform_tools/android/apps/skqp/src/main/assets/files.checksum'):
...
def main():
    ...
    build_dir = os.environ.get('SKQP_BUILD_DIR', 'out/skqp')  #TODO: document
    final_output_dir = os.environ.get('SKQP_OUTPUT_DIR', 'out/skqp')  #TODO: document
    debug = bool(os.environ.get('SKQP_DEBUG', ''))  #TODO: document
    android_ndk = os.environ['ANDROID_NDK']
    android_home = os.environ['ANDROID_HOME']
    skia_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), os.pardir, os.pardir))
