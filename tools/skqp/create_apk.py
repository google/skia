...
def accept_android_license(android_home):
    license = subprocess.Popen(
            [android_home + '/tools/bin/sdkmanager', '--licenses'],
            stdin=subprocess.PIPE)
    while license.poll() is None:
        license.stdin.write('y\n')
        time.sleep(1)
...
def create_apk_impl(options):
    build_dir, final_output_dir = options.build_dir, options.final_output_dir
    ...
    with RemoveFiles(*build_paths):
        for arch in options.architectures:
            ...
            gn_args = skqp_gn_args.GetGNArgs(
                    arch, options.android_ndk, options.debug, 26)
        ...
        accept_android_license(options.android_home)
        env_copy = os.environ.copy()
        env_copy['ANDROID_HOME'] = options.android_home
        env_copy['ANDROID_NDK_HOME'] = options.android_ndk
    ...
    arches = '_'.join(sorted(options.architectures))
...
class SkQP_Build_Options:
    ...
    def gn_args(self, arch):
        return skqp_gn_args.GetGNArgs(arch, self.android_ndk, debug, 26)
