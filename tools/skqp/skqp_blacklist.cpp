...
static const char* kDoNotExecuteInExperimentalMode[] = {
};

static const char* kNoScoreInCompatibilityTestMode[] = {
    "circular-clips",
...
static const char* kKnownGpuUnitTests[] = {
    "ApplyGamma",
    "ClearOp",
    "ClipMaskCache",
    ...
    "GrContextFactory_RequiredSRGBSupport",
...
static const char* kKnownGMs[] = {
    ...
    "dashing5_bw",
    "dashtextcaps",
    "deferred_texture_image_high",
    "deferred_texture_image_low",
    "deferred_texture_image_low_dithered",
    "deferred_texture_image_medium_decoded",
    "deferred_texture_image_medium_decoded_indexed",
    "deferred_texture_image_medium_encoded",
    "deferred_texture_image_medium_encoded_indexed",
...
static bool in_sorted_array(const char* key, const char** begin, size_t count) {
    const char** last = begin + count - 1;
    while (begin <= last) {
        const char** middle = begin + ((last - begin) >> 1);
        int cmp = strcmp(key, *middle);
        if (cmp == 0) {
            return true;
        } else if (cmp < 0) {
            last = middle - 1;
...
#define IN_ARRAY(key, array) in_sorted_array(key, array, (sizeof(array) / sizeof(array[0])))

bool skqp::DoNotScoreInCompatibilityTestMode(const char* gmName) {
    return IN_ARRAY(gmName, kNoScoreInCompatibilityTestMode);
}

bool skqp::DoNotExecuteInExperimentalMode(const char* gmName) {
    return IN_ARRAY(gmName, kDoNotExecuteInExperimentalMode);
}

bool skqp::IsKnownGpuUnitTest(const char* testName) {
    return IN_ARRAY(testName, kKnownGpuUnitTests);
}

bool skqp::IsKnownGM(const char* gmName) { return IN_ARRAY(gmName, kKnownGMs); }
