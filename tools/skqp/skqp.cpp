...
static std::string gGmkbDirectoryPath;
        ...
        if (fTest.fGMFactory) {
            ...
            std::tie(result, error) =
                    gm_runner::EvaluateGM(fTest.fBackend, fTest.fGMFactory,
                                          gGmkbDirectoryPath.c_str(), gReportDirectoryPath.c_str());
...
void register_skia_tests() {
    ...
    std::vector<gm_runner::GMFactory> gms = gm_runner::GetGMFactories(gGmkbDirectoryPath.c_str());
