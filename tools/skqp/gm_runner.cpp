...
static GMKB_ImageData evaluate_gm(SkiaBackend backend,
                                  skiagm::GM* gm,
    ...
    std::unique_ptr<sk_gpu_test::TestContext> testCtx = make_test_context(backend);
    if (!testCtx) {
        return GMKB_ImageData{nullptr, w, h};
    }
    testCtx->makeCurrent();
    ...
    return GMKB_ImageData{pix, w, h};
}
std::tuple<float, Error> EvaluateGM(SkiaBackend backend,
                                    GMFactory gmFact,
                                    const char* gmkbDirectoryPath,
                                    const char* reportDirectoryPath) {
    std::vector<uint32_t> pixels;
    std::unique_ptr<skiagm::GM> gm(gmFact(nullptr));
    GMKB_ImageData imgData = evaluate_gm(backend, gm.get(), &pixels);
    if (!imgData.pix) {
        return std::make_tuple(FLT_MAX, Error::SkiaFailure);
    }
    GMKB_Error e;
    float value = GMKB_Check(imgData, gm->getName(), GetBackendName(backend), gmkbDirectoryPath,
                             reportDirectoryPath, &e);
    Error error = GMKB_Error_bad_input == e ? Error::BadSkiaOutput
                : GMKB_Error_bad_data  == e ? Error::BadGMKBData
                                            : Error::None;
...
std::vector<GMFactory> GetGMFactories(const char* gmkbDirectoryPath) {
    std::vector<GMFactory> result;
    for (const skiagm::GMRegistry* r = skiagm::GMRegistry::Head(); r; r = r->next()) {
        GMFactory f = r->factory();

        if (GMKB_IsGoodGM(GetGMName(f).c_str(), gmkbDirectoryPath)) {
