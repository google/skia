...
func (a TestRecordArray) Len() int { return len(a) }
func (a TestRecordArray) Swap(i, j int) { a[i], a[j] = a[j], a[i] }
func (a TestRecordArray) Less(i, j int) bool { return a[i].TestName < a[j].TestName }
...
func main() {
	...
	check(err)
	sort.Sort(TestRecordArray(records))
