...
DDLPromiseImageHelper::PromiseImageCallbackContext::~PromiseImageCallbackContext() {
    ...
    if (fPromiseImageTexture) {
        GrGpu* gpu = fContext1->priv().getGpu();
...
void DDLPromiseImageHelper::PromiseImageInfo::createPromiseImage(
    ...
    if (this->isYUV()) {
        ...
        for (int i = 0; i < textureCount; ++i) {
            const GrBackendTexture& backendTex = this->promiseTexture(i)->backendTexture();
            backendFormats[i] = backendTex.getBackendFormat();
            SkASSERT(backendFormats[i].isValid());
            contexts[i] = this->refCallbackContext(i).release();
        ...
        for (int i = 0; i < textureCount; ++i) {
            this->callbackContext(i)->wasAddedToImage();
        }
    } else {
        const GrBackendTexture& backendTex = this->promiseTexture(0)->backendTexture();
        ...
        fPromiseImage1 = foo->makePromiseTexture(
                                        ...
                                        DDLPromiseImageHelper::PromiseImageReleaseProc,
                                        DDLPromiseImageHelper::PromiseImageDoneProc,
                                        (void*)this->refCallbackContext(0).release(),
                                        delayReleaseCallback);
        this->callbackContext(0)->wasAddedToImage();
    }
