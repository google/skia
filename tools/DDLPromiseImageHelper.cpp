 ...
 */
void DDLPromiseImageHelper::UploadTexturesForPromiseImage(GrContext* context,
    ...
    if (taskGroup) {
        for (int i = 0; i < fImageInfo.count(); ++i) {
            ...
            taskGroup->add([context, info]() {
                              ...
                              UploadTexturesForPromiseImage(context, info);
                           });
        }
    } else {
        // TODO: rm this after the DDL-via is removed
        for (int i = 0; i < fImageInfo.count(); ++i) {
            UploadTexturesForPromiseImage(context, &fImageInfo[i]);
        }
    }
}
