...
int main(int argc, char** argv) {
    ...
    cl::Program program(ctx,
                        "__kernel void mul(int n, __global const float* a,    "
    ...
    cl_ok(ok);
    cl_ok(mul.setArg(0, 1000));
    cl_ok(mul.setArg(1, A));
    cl_ok(mul.setArg(2, B));
    cl_ok(mul.setArg(3, P));
    ...
    cl_ok(queue.enqueueReadBuffer(P, CL_TRUE/*?*/, 0/*?*/, sizeof(float)*p.size(), p.data()));
