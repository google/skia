...
void SkTestSVGTypeface::getMetrics(SkGlyph* glyph) const {
...
protected:
    ...
    void generateMetrics(SkGlyph* glyph) override {
        ...
        this->geTestSVGTypeface()->getMetrics(glyph);
        const SkVector advance = fMatrix.mapXY(SkFloatToScalar(glyph->fAdvanceX),
                                               SkFloatToScalar(glyph->fAdvanceY));
        glyph->fAdvanceX = SkScalarToFloat(advance.fX);
        glyph->fAdvanceY = SkScalarToFloat(advance.fY);
    ...
    void generateImage(const SkGlyph& glyph) override {
        ...
        SkBitmap bm;
        bm.installPixels(SkImageInfo::MakeS32(glyph.fWidth, glyph.fHeight, kPremul_SkAlphaType),
        ...
        SkSVGDOM& svg = *this->geTestSVGTypeface()->fGlyphs[glyph.getGlyphID()].fSvg.get();
        svg.render(&canvas);
    }
...
public:
    ...
    void onDrawPath(const SkPath& platonicPath, const SkPaint& originalPaint) override {
        ...
        // Dark enough to also be a union on the base glyph.
        ...
        fGlyf->fLayers.emplace_back(SkTestSVGTypeface::GlyfLayerInfo{ colorIndex, bounds });
