...
void SkSVGTestTypeface::exportTtxCommon(SkWStream* out, const char* type,
                                        const SkTArray<GlyfInfo>* glyfInfo) const
{
    ...
    if (glyfInfo) {
        for (int i = 0; i < fGlyphs.count(); ++i) {
            for (int j = 0; j < (*glyfInfo)[i].layers.count(); ++j) {
                out->writeText("    <GlyphID name=\"glyf");
                    out->writeHexAsText(i, 4);
                    out->writeText("l");
        ...
        for (int i = 0; i < fGlyphs.count(); ++i) {
            for (int j = 0; j < (*glyfInfo)[i].layers.count(); ++j) {
                out->writeText("    <mtx name=\"glyf");
                    ...
                    out->writeDecAsText(fGlyphs[i].fAdvance);
                    out->writeText("\" lsb=\"");
                    out->writeDecAsText(fGlyphs[i].fOrigin.fX + (*glyfInfo)[i].layers[j].bounds.fLeft);
                    out->writeText("\"/>\n");
            }
        }
    }
...
public:
    SkCOLRCanvas(SkIRect bounds, SkGlyphID glyphId,
                 SkSVGTestTypeface::GlyfInfo* glyf, SkTHashMap<SkColor, int>* colors, SkWStream* out)
        : SkNoDrawCanvas(bounds.width(), bounds.height())
        , fOut(out)
        , fGlyphId(glyphId)
        , fLayerId(0)
        , fGlyf(glyf)
    ...
    void onDrawPath(const SkPath& platonicPath, const SkPaint& originalPaint) override {
        ...
        fGlyf->layers.emplace_back(SkSVGTestTypeface::GlyfLayerInfo{ colorIndex, bounds });
    ...
    void finishGlyph() {
        ...
        fGlyf->bounds = this->writePath(baseGlyph, false);
    }
...
private:
    ...
    SkSVGTestTypeface::GlyfInfo* fGlyf;
...
void SkSVGTestTypeface::exportTtxColr(SkWStream* out) const {
    ...
    for (int i = 0; i < fGlyphs.count(); ++i) {
        ...
        SkIRect ibounds;
        newBounds.roundOut(&ibounds);
        SkCOLRCanvas canvas(ibounds, i, &glyfInfos.emplace_back(), &colors, &glyfOut);
        svg.render(&canvas);
        canvas.finishGlyph();
    }
    glyfOut.writeText("  </glyf>\n");
    ...
    for (int i = 0; i < fGlyphs.count(); ++i) {
        out->writeText("    <ColorGlyph name=\"glyf");
            out->writeHexAsText(i, 4);
            out->writeText("\">\n");
        for (int j = 0; j < glyfInfos[i].layers.count(); ++j) {
            const int colorIndex = glyfInfos[i].layers[j].layerColorIndex;
