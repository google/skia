...
void SkSVGTestTypeface::exportTtxCommon(SkWStream* out, const SkTArray<SkTArray<int>>* extraLayers) const {
    ...
    if (extraLayers) {
        for (int i = 0; i < fGlyphs.count(); ++i) {
            for (int j = 0; j < (*extraLayers)[i].count(); ++j) {
                out->writeText("    <GlyphID name=\"glyf");
                    out->writeHexAsText(i, 4);
                    out->writeText("l");
...
public:
    SkCOLRCanvas(SkIRect realBounds, SkGlyphID glyphId, SkTArray<int>* layerColor, SkTHashMap<SkColor, int>* colors, SkWStream* out)
        : SkNoDrawCanvas(realBounds.width(), realBounds.height())
        , fBounds(realBounds)
        , fOut(out)
        , fGlyphId(glyphId)
        , fLayerId(0)
        , fLayerColor(layerColor)
        , fColors(colors)
    { }

    void writePoint(int x, int y, bool on) {
        fOut->writeText("        <pt x=\"");
        fOut->writeDecAsText(x);
        fOut->writeText("\" y=\"");
        fOut->writeDecAsText(y);
        fOut->writeText("\" on=\"");
        fOut->write8(on ? '1' : '0');
        fOut->writeText("\"/>\n");
    }
    void writePath(const SkPath& path, bool layer) {
        // Convert to quads.
        SkPath quads;
    ...
    void onDrawPath(const SkPath& platonicPath, const SkPaint& originalPaint) override {
        ...
        fLayerColor->emplace_back(colorIndex);
    ...
    void finishGlyph() {
        ...
        this->writePath(baseGlyph, false);
    }
...
void SkSVGTestTypeface::exportTtxColr(SkWStream* out) const {
    ...
    SkTArray<SkTArray<int>> glyphLayerColorIndexes(fGlyphs.count());

    // Need to know all the glyphs up front for the common tables.
    SkDynamicMemoryWStream glyfOut;
    glyfOut.writeText("  <glyf>\n");
    for (int i = 0; i < fGlyphs.count(); ++i) {
        const SkSVGTestTypeface::Glyph& glyphData = this->fGlyphs[i];
        SkSVGDOM& svg = *glyphData.fSvg.get();
        SkSize containerSize = svg.containerSize();
        ...
        SkCOLRCanvas canvas(ibounds, i, &glyphLayerColorIndexes.emplace_back(), &colors, &glyfOut);
        svg.render(&canvas);
        canvas.finishGlyph();
    }
    glyfOut.writeText("  </glyf>\n");

    this->exportTtxCommon(out, &glyphLayerColorIndexes);
    ...
    for (int i = 0; i < fGlyphs.count(); ++i) {
        out->writeText("    <ColorGlyph name=\"glyf");
            out->writeHexAsText(i, 4);
            out->writeText("\">\n");
        for (int j = 0; j < glyphLayerColorIndexes[i].count(); ++j) {
            const int colorIndex = glyphLayerColorIndexes[i][j];
    ...
    colors.foreach([&out](const SkColor& c, const int* i) {
