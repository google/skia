...
template<typename T> struct SizedArray {
    template<size_t N> constexpr SizedArray(T(&t)[N]) : fArray(t), fSize(N) {}
    constexpr SizedArray(const SizedArray& that) : fArray(that.fArray), fSize(that.fSize) {}
    const T* begin() const { return &fArray[0]; }
    const T* end() const { return &fArray[fSize]; }
private:
    T* fArray;
    const size_t fSize;
};
...
static constexpr FontDesc kMonoFontsData[] = {
    {normal,     "LiberationMono-Regular.ttf"},
    {bold,       "LiberationMono-Bold.ttf"},
    {italic,     "LiberationMono-Italic.ttf"},
    {bolditalic, "LiberationMono-BoldItalic.ttf"},
};
static constexpr SizedArray<const FontDesc> kMonoFonts(kMonoFontsData);

static constexpr FontDesc kSansFontsData[] = {
    {normal,     "LiberationSans-Regular.ttf"},
    {bold,       "LiberationSans-Bold.ttf"},
    {italic,     "LiberationSans-Italic.ttf"},
    {bolditalic, "LiberationSans-BoldItalic.ttf"},
};
static constexpr SizedArray<const FontDesc> kSansFonts(kSansFontsData);

static constexpr FontDesc kSerifFontsData[] = {
    {normal,     "LiberationSerif-Regular.ttf"},
    {bold,       "LiberationSerif-Bold.ttf"},
    {italic,     "LiberationSerif-Italic.ttf"},
    {bolditalic, "LiberationSerif-BoldItalic.ttf"},
};
static constexpr SizedArray<const FontDesc> kSerifFonts(kSerifFontsData);

struct FontFamilyDesc {
    char const * const fGenericName;
    char const * const fFamilyName;
    char const * const fIdentifierName;
    SizedArray<const FontDesc> const fFonts;
};
static constexpr std::array<FontFamilyDesc, 3>  gFamilies = {{
...
static void generate_fonts(const char* basepath) {
    FILE* out = nullptr;
    for (const FontFamilyDesc& family : gFamilies) {
...
static void generate_index(const char* defaultName) {
    ...
    for (const FontFamilyDesc& family : gFamilies) {
        for (const FontDesc& font : family.fFonts) {
            if (defaultIndex < 0 && !strcmp(defaultName, family.fGenericName)) {
