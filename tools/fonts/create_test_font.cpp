...
template<typename T> struct SizedArray {
    ...
    constexpr SizedArray(const SizedArray& that) = delete;
    constexpr const T* begin() const { return &fArray[0]; }
    constexpr const T* end() const { return &fArray[fSize]; }
...
static void generate_fonts(const char* basepath, const SizedArray<const FontFamilyDesc>& families) {
    FILE* out = nullptr;
...
static void generate_index(const SizedArray<const FontFamilyDesc>& families,
...
int main(int , char * const []) {
    ...
    static constexpr SizedArray<const FontFamilyDesc> kFamilies(kFamiliesData);
