/**
 * Command line application to run benchmarks in webpages using puppeteer. Different webpages can
 * be specified to measure different aspects. The HTML page run contains the JS code to run the
 * scenario and either 1) produce the perf output as a JSON object or 2) defer to puppeteer reading
 * the tracing data.
 *
 */
...
// by convention, bench_html start with the name of the primary thing they are testing.
if (options.bench_html.startsWith('canvaskit')) {
...
const lottieJSON = fs.readFileSync(options.input, 'utf8');
...
const viewPort = {width: 1100, height: 1100};
