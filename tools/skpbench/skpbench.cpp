...
public:
    ...
    SubmitFinishTracker* newSubmitTracker(GrContext* context);
...
static void run_ddl_benchmark(GrContext* context, SkCanvas* finalCanvas, SkPicture* inputPicture,
...
int main(int argc, char** argv) {
    ...
    if (!FLAGS_gpuClock) {
        if (FLAGS_ddl) {
            run_ddl_benchmark(ctx, canvas, skp.get(), &samples);
        } else if (!mskp) {
...
static void flush_with_sync(GrContext* context, GpuSync& gpuSync) {
    ...
    flushInfo.fFinishedContext = gpuSync.newSubmitTracker(context);
...
SubmitFinishTracker* GpuSync::newSubmitTracker(GrContext* context) {
    fFinishTrackers[fCurrentSubmitIdx].reset(new SubmitFinishTracker(context));

    SubmitFinishTracker* tracker = fFinishTrackers[fCurrentSubmitIdx].get();
    tracker->ref();
