...
static void ddl_sample(GrContext* context, DDLTileHelper* tiles, GpuSync& gpuSync,
    ...
    SkExecuteWithTaskGroup(gpuTaskGroup, [&]{
...
static void run_ddl_benchmark(sk_gpu_test::TestContext* testContext, GrContext *context,
    ...
    // Otherwise, we transfer testContext onto the GPU thread for the rest of this function.
    ...
    if (!FLAGS_comparableDDL && !FLAGS_comparableSKP) {
        ...
        recordingTaskGroup = std::unique_ptr<SkTaskGroup>(new SkTaskGroup);
        gpuTaskGroup = std::unique_ptr<SkTaskGroup>(new SkTaskGroup(*gpuThread));
