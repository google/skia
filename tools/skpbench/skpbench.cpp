...
struct Sample {
    ...
    Sample() : fFrames(0), fDDLRecordDuration(0), fDuration(0) {}
    ...
    double ddlSeconds() const { return std::chrono::duration<double>(fDDLRecordDuration).count(); }
    double ddlMS() const {
        return std::chrono::duration<double, std::milli>(fDDLRecordDuration).count();
    }
    double ddlValue() const { return FLAGS_fps ? fFrames / this->ddlSeconds()
                                               : this->ddlMS() / fFrames; }

    static const char* metric() { return FLAGS_fps ? "fps" : "ms"; }

    int        fFrames;
    duration   fDDLRecordDuration; // DDL recording time - only in DDL mode
    duration   fDuration;
};
...
static void ddl_sample(GrContext* context, DDLTileHelper* tiles, GpuSync* gpuSync, Sample* sample) {
    ...
    clock::time_point ddlEnd = clock::now();
    ...
    if (sample) {
        SkASSERT(gpuSync);
        sample->fDDLRecordDuration += ddlEnd - start;
        sample->fDuration += gpuEnd - start;
        sample->fFrames++;
    }
}
...
static void run_ddl_benchmark(const sk_gpu_test::FenceSync* fenceSync,
    ...
    ddl_sample(context, &tiles, nullptr, nullptr);
    GpuSync gpuSync(fenceSync);
    ddl_sample(context, &tiles, &gpuSync, nullptr);
    ...
    do {
        ...
        do {
            ddl_sample(context, &tiles, &gpuSync, &sample);
...
static void run_benchmark(const sk_gpu_test::FenceSync* fenceSync, SkCanvas* canvas,
    ...
    draw_skp_and_flush(canvas, skp.get()); // draw1
    GpuSync gpuSync(fenceSync);

    draw_skp_and_flush(canvas, skp.get()); // draw2
