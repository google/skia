...
struct Sample {
    ...
    Sample() : fFrames(0), fCPUDuration(0), fDuration(0) {}
    ...
    int        fFrames;
    duration   fCPUDuration; // DDL recording time - only in DDL mode
    duration   fDuration;
};
...
static void ddl_sample(GrContext* context, DDLTileHelper* tiles, GpuSync* gpuSync, Sample* sample) {
    ...
    clock::time_point gpuCPU = clock::now();
    ...
    if (sample) {
        ...
        std::chrono::nanoseconds cpuTime = cpuEnd - start;
        sample->fCPUDuration += cpuTime;

        sample->fDuration += gpuCPU - start;
        sample->fFrames++;
    }
}
