...
class HardwarePixel2(HardwareAndroid):
  ...
  def __enter__(self):
    ...
    assert('msm_therm' == self._adb.check('cat /sys/class/thermal/thermal_zone10/type').strip())
    assert('pm8998_tz' == self._adb.check('cat /sys/class/thermal/thermal_zone7/type').strip())
  ...
  def sanity_check(self):
    ...
    result = self._adb.check(' '.join(
      ...
      #
      # It doesn't seem to respect the min_freq/max_freq values when not under load.
