...
function strip_static_color(c) {
    if (!Array.isArray(c)) return;

    for (var i = 0; i < c.length; ++i) {
        //c[i] = Math.round(c[i] * 10000) / 10000;

        var c255 = Math.round(c[i] * 255);
        c[i] = Math.trunc(c255 / 255 * 1000) / 1000;
    }
}

function strip_animatable_color(c) {
    if (!c) return;

    var val = c['k'];

    if (is_animated_property(c)) {
        // Enabling this may yiled observable (but insignificant) diffs due to interpolation.
        if (true) {
            val.forEach(function (kf) {
                strip_static_color(kf['s']);
                strip_static_color(kf['e']);
            });
        }
    } else {
        strip_static_color(val);
    }
}
...
function strip_shape(shape) {
    ...
    switch (shape['ty']) {
    case 'el':
        strip_animatable_defaults(shape, { 'p': [ 0, 0 ] });
        strip_property(shape['s']);
        break;
    case 'fl':
        strip_animatable_defaults(shape, { 'o': 100, 'c': [0, 0, 0, 1] });
        strip_animatable_color(shape['c']);
        break;
    case 'mm':
        strip_static_defaults(shape, { 'mm': 1 });
        break;
    case 'sh':
        strip_static_defaults(shape, { 'c': false });
        strip_static_defaults(shape, { 'closed': false });

        strip_animatable_defaults(shape, {'ks': 'nodefault'});
...
function strip_root(root) {
    ...
    if (Array.isArray(assets)) {
        assets.forEach(strip_comp);
        if (assets.length === 0) delete root["assets"];
    }
