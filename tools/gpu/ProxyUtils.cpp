...
sk_sp<GrTextureProxy> MakeTextureProxyFromData(GrContext* context, bool isRT, int width, int height,
                                               SkColorType colorType, GrSRGBEncoded srgbEncoded,
    ...
    if (kBottomLeft_GrSurfaceOrigin == origin) {
        ...
        auto backendTex = context->contextPriv().getGpu()->createTestingOnlyBackendTexture(
                nullptr, width, height, colorType, isRT, GrMipMapped::kNo, 0);
        ...
        GrPixelConfig config = GrColorTypeToPixelConfig(grCT, srgbEncoded);
    ...
    if (!context->contextPriv().writeSurfacePixels(sContext.get(), 0, 0, width, height, grCT,
