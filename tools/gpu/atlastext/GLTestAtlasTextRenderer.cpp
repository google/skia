...
public:
    ...
    void setTextureData(void* textureHandle, const void* data, int x, int y, int width, int height, size_t rowBytes) override;

    void drawSDFGlyphs(void* targetHandle, void* textureHandle, const SDFVertex vertices[], int quadCnt) override;

    void* makeTarget(int width, int height) override;

    void targetDeleted(void* target) override;

    SkBitmap readTarget(void* target) override;
...
#define checkgl() do {auto error = fContext->gl()->fFunctions.fGetError(); if (error != GR_GL_NO_ERROR) {SkDebugf("GL ERROR: %d\n", error);}} while (false)
...
GLTestAtlasTextRenderer::GLTestAtlasTextRenderer(std::unique_ptr<GLTestContext> context) : fContext(std::move(context)) {
    ...
    static constexpr const char kFS[] = R"(
        layout (location = 2) uniform sampler2D uSampler;
...
void* GLTestAtlasTextRenderer::createTexture(AtlasFormat format, int width, int height) {
    ...
    AtlasTexture atlas  = {.fID = id, .fFormat = format, .fWidth = width, .fHeight = height};
...
void GLTestAtlasTextRenderer::drawSDFGlyphs(void* targetHandle, void* textureHandle,
    ...
    float uniformScaleAndTranslate[4] = {2.f / target->fWidth, -1.f,
                                         2.f / target->fHeight, -1.f};
    callgl(Uniform4fv, 0, 1, uniformScaleAndTranslate);
    callgl(Uniform2f, 1, 1.f / atlas->fWidth, 1.f / atlas->fHeight);
    callgl(Uniform1i, 2, 0);
...
void* GLTestAtlasTextRenderer::makeTarget(int width, int height) {
    GrGLuint fbo;
    ...
    callgl(RenderbufferStorage, GR_GL_RENDERBUFFER, GR_GL_RGBA, width, height);
    ...
    checkgl();
    return new Target{.fFBOID = fbo, .fRBID = rb, .fWidth = width, .fHeight = height};
}
...
SkBitmap GLTestAtlasTextRenderer::readTarget(void* target) {
    Target* t = reinterpret_cast<Target*>(target);

    auto info = SkImageInfo::Make(t->fWidth, t->fHeight, kRGBA_8888_SkColorType,
                                  kPremul_SkAlphaType);
    SkBitmap bmp;
...
sk_sp<TestAtlasTextRenderer> MakeGLTestAtlasTextRenderer() {
    ...
    return sk_make_sp<GLTestAtlasTextRenderer>(std::move(context));
}
