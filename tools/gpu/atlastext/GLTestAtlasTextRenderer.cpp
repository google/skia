...
public:
    ...
    std::unique_ptr<uint32_t[]> readTarget(void* target) override;
...
void GLTestAtlasTextRenderer::drawSDFGlyphs(void* targetHandle, void* textureHandle,
    ...
    callgl(VertexAttribIPointer, 1, 2, GR_GL_UNSIGNED_SHORT, sizeof(SDFVertex),
           reinterpret_cast<const char*>(vertices) + 2 * sizeof(float));
...
void* GLTestAtlasTextRenderer::makeTarget(int width, int height) {
    ...
    return new Target{fbo, rb, width, height};
}
...
std::unique_ptr<uint32_t[]> GLTestAtlasTextRenderer::readTarget(void* target) {
    Target* t = reinterpret_cast<Target*>(target);
    callgl(BindFramebuffer, GR_GL_FRAMEBUFFER, t->fFBOID);
    std::unique_ptr<uint32_t[]> pixels(new uint32_t[t->fWidth * t->fHeight]);
    callgl(ReadPixels, 0, 0, t->fWidth, t->fHeight, GR_GL_RGBA, GR_GL_UNSIGNED_BYTE, pixels.get());
    return pixels;
}
...
std::unique_ptr<TestAtlasTextRenderer> MakeGLTestAtlasTextRenderer() {
    ...
    return std::unique_ptr<TestAtlasTextRenderer>(new GLTestAtlasTextRenderer(std::move(context)));
}
