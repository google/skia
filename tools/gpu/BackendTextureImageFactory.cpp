...
public:
    ...
    static sk_sp<ManagedBackendTexture> Make(GrContext* context, Args&&... args) {
        ...
        mbet->fTexture =
                context->createBackendTexture(std::forward<Args>(args)..., Proc, mbet->pass());
        return mbet;
    }
...
sk_sp<SkImage> MakeBackendTextureImage(GrContext* context,
    ...
    return SkImage::MakeFromTexture(context, mbet->texture(), origin, src->colorType(),
                                    src->alphaType(), src->refColorSpace(),
                                    ManagedBackendTexture::Proc, mbet->pass());
}
}  // namespace sk_gpu_test
