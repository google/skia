...
bool CreateVkBackendContext(GrVkGetProc getProc,
    ...
    *debugCallback = VK_NULL_HANDLE;
    ...
    for (int i = 0; i < deviceExtensions.count(); ++i) {
        ...
        if (strncmp(deviceExtensions[i].extensionName, "VK_KHX", 6) != 0 &&
            strncmp(deviceExtensions[i].extensionName, "VK_NVX", 6) != 0) {

            if (!hasKHRBufferDeviceAddress ||
                strcmp(deviceExtensions[i].extensionName, "VK_EXT_buffer_device_address") != 0) {
