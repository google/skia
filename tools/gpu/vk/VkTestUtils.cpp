...
bool CreateVkBackendContext(GrVkGetProc getProc,
    ...
    if (instanceVersion >= VK_MAKE_VERSION(1, 1, 0)) {
        // If the instance version is 1.0 we must leave have the apiVersion also be 1.0. However, if
        // the instance version is 1.1 or higher we can set the apiVersion to be whatever the
        // highest api we may use in skia (technically it can be arbitrary). So for now we set it to
        // 1.1 since that is the highest vulkan version.
