...
std::tuple<int, int, int> VkYcbcrSamplerHelper::GetExpected(int x, int y, int width, int height) {
    return {
               16 + (x + y) * 219 / (width + height - 2),
               16 + x * 224 / (width - 1),
               16 + y * 224 / (height - 1)
           };
}
...
sk_sp<SkImage> VkYcbcrSamplerHelper::createI420Image(int width, int height) {
    ...
    texture = GrBackendTexture(width, height, imageInfo);
