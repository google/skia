...
#define ACQUIRE_DEVICE_VK_PROC(name)                                                          \
    f1Vk##name = reinterpret_cast<PFN_vk##name>(getProc("vk" #name, VK_NULL_HANDLE, fDevice1)); \
    if (f1Vk##name == nullptr) {                                                               \
        SkDebugf("Function ptr for vk%s could not be acquired\n", #name);                     \
        return false;                                                                         \
    }
...
VkYcbcrSamplerHelper::~VkYcbcrSamplerHelper() {
    ...
    if (fImage1 != VK_NULL_HANDLE) {
        GR_VK_CALL(interface, DestroyImage(fDevice1, fImage1, nullptr));
        fImage1 = VK_NULL_HANDLE;
    }

    if (fImageMemory1 != VK_NULL_HANDLE) {
        GR_VK_CALL(interface, FreeMemory(fDevice1, fImageMemory1, nullptr));
        fImageMemory1 = VK_NULL_HANDLE;
    }
    ...
    if (fDebugCallback1 != VK_NULL_HANDLE) {
        fDestroyDebugCallback(fBackendContext.fInstance, fDebugCallback1, nullptr);
    }
...
sk_sp<SkImage> VkYcbcrSamplerHelper::createI420Image(int width, int height) {
    ...
    SkASSERT(fImage1 == VK_NULL_HANDLE);
    err = GR_VK_CALL(interface, CreateImage(fDevice1, &vkImageInfo, nullptr, &fImage1));
    if (err != VK_SUCCESS) {
    ...
    GR_VK_CALL(interface, GetImageMemoryRequirements(fDevice1, fImage1, &requirements));
    ...
    SkASSERT(fImageMemory1 == VK_NULL_HANDLE);
    err = GR_VK_CALL(interface, AllocateMemory(fDevice1, &allocInfo, nullptr, &fImageMemory1));
    if (err != VK_SUCCESS) {
    ...
    void* mappedBuffer;
    err = GR_VK_CALL(interface, MapMemory(fDevice1, fImageMemory1, 0u, requirements.size,
                                          0u, &mappedBuffer));
    if (err != VK_SUCCESS) {
    ...
    GR_VK_CALL(interface, GetImageSubresourceLayout(fDevice1, fImage1, &subresource, &yLayout));
    ...
    GR_VK_CALL(interface, GetImageSubresourceLayout(fDevice1, fImage1, &subresource, &uvLayout));
    ...
    flushRange.memory = fImageMemory1;
    flushRange.offset = 0;
    flushRange.size = VK_WHOLE_SIZE;
    err = GR_VK_CALL(interface, FlushMappedMemoryRanges(fDevice1, 1, &flushRange));
    if (err != VK_SUCCESS) {
        SkDebugf("Failed to flush buffer memory.");
        return nullptr;
    }
    GR_VK_CALL(interface, UnmapMemory(fDevice1, fImageMemory1));

    // Bind image memory.
    err = GR_VK_CALL(interface, BindImageMemory(fDevice1, fImage1, fImageMemory1, 0u));
    if (err != VK_SUCCESS) {
    ...
    GrVkAlloc alloc(fImageMemory1, 0 /* offset */, requirements.size, 0 /* flags */);
    GrVkImageInfo imageInfo(fImage1, alloc, VK_IMAGE_TILING_LINEAR, VK_IMAGE_LAYOUT_UNDEFINED,
