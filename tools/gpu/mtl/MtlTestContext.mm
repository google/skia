    ...
    static MtlTestContext* Create(MtlTestContext* sharedContext) {
        ...
        if (sharedContext) {
            ...
            for (id<MTLDevice> dev in availableDevices.get()) {
                if (!dev.isLowPower) {
                    device = sk_ret_cfp<id<MTLDevice>(dev);
                    break;
                }
                if (dev.isRemovable) {
                    device = sk_ret_cfp<id<MTLDevice>(dev);
            ...
            device.reset(MTLCreateSystemDefaultDevice());
#endif
            backendContext.fDevice = sk_ret_cfp<GrMTLHandle>((GrMTLHandle)device.get());
            sk_cfp<id<MTLCommandQueue>> queue([*device newCommandQueue]);
            backendContext.fQueue = sk_ret_cfp<GrMTLHandle>((GrMTLHandle)queue.get());
        }
