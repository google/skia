...
std::unique_ptr<LazyYUVImage> LazyYUVImage::Make(sk_sp<SkData> data, bool buildMips) {
    std::unique_ptr<LazyYUVImage> image(new LazyYUVImage());
    if (image->reset(std::move(data), buildMips)) {
        return image;
    } else {
        return nullptr;
    }
...
bool LazyYUVImage::reset(sk_sp<SkData> data, bool buildMips) {
    fBuildMips = buildMips;
