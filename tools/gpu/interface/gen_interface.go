...
type StandardGetter func(FeatureSet) []Requirement
...
func fillAssembleTemplate(template string, features []FeatureSet, getReqs StandardGetter) string {
	content := ""
	for _, feature := range features {
		reqs := getReqs(feature)
		for idx, req := range reqs {
			ifStart := assembleIfBlock(req, idx+1, len(reqs))
...
func assembleIfBlock(req Requirement, curBlock, totalBlocks int) string {
	mv := req.MinVersion
...
func generateValidateInterface(features []FeatureSet) {
	...
	for _, feature := range features {
		...
		if allReqsAreCore(feature) {
			content += functionCheck(feature.Functions, 1)
		} else {
			content += TAB
			branchChecks := []string{}
			for _, branch := range branches {
				reqs := branch.GetReqs(feature)
				if reqs == nil || len(reqs) == 0 {
					continue
				}
				expr := []string{}
				for _, r := range reqs {
					e := validateIfExpression(r)
					if e != "" {
						expr = append(expr, e)
					}
				}
				check := ""
				if len(expr) == 0 {
					check = fmt.Sprintf("%s", branch.StandardCheck)
				} else {
					lineBreak := "\n" + TAB + "      "
					check = fmt.Sprintf("(%s && (%s%s))", branch.StandardCheck, lineBreak, strings.Join(expr, " || "+lineBreak))
				}
				branchChecks = append(branchChecks, check)
			}
			content += fmt.Sprintf("if (%s) {\n", strings.Join(branchChecks, " || \n"+TAB+"   "))
			content += functionCheck(feature.Functions, 2)
...
func functionCheck(functions []string, indentLevel int) string {
	indent := strings.Repeat(TAB, indentLevel)

	checks := []string{}
	for _, function := range functions {
		checks = append(checks, "!fFunctions."+function)
	}
...
func validateIfExpression(req Requirement) string {
	mv := req.MinVersion
	...
	// We know it has an extension
	if mv == nil {
		return fmt.Sprintf("fExtensions.has(%q)", req.Extension)
	} else {
		return fmt.Sprintf("(glVer >= GR_GL_VER(%d,%d) && fExtensions.has(%q))", mv[0], mv[1], req.Extension)
	}
}
