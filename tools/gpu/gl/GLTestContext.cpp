...
std::unique_ptr<sk_gpu_test::FenceSync> GLFenceSync::MakeIfSupported(
    ...
    if (kGL_GrGLStandard == ctx->gl()->fStandard) {
        if (GrGLGetVersion(ctx->gl()) >= GR_GL_VER(4,5)) {
            SkDebugf("version of gl is over 4.5\n");
        } else {
            SkDebugf("version of gl is NOT over 4.5. it is %d\n", GrGLGetVersion(ctx->gl()));
            SkDebugf("4.5 version is: %d\n", GR_GL_VER(4,5));
        }
        if (GrGLGetVersion(ctx->gl()) < GR_GL_VER(3,2) && !ctx->gl()->hasExtension("GL_ARB_sync")) {
            SkDebugf("standard but no sync\n");
            return nullptr;
        }
        SkDebugf("standard with 3.2 or arb\n");
        ret.reset(new GLFenceSync(ctx));
    } else {
        if (ctx->gl()->hasExtension("GL_APPLE_sync")) {
            SkDebugf("apple fence sync\n");
            ret.reset(new GLFenceSync(ctx, "APPLE"));
        } else if (ctx->gl()->hasExtension("GL_NV_fence")) {
            SkDebugf("nv fence sync\n");
            ret.reset(new GLNVFenceSync(ctx));
        } else if (GrGLGetVersion(ctx->gl()) >= GR_GL_VER(3, 0)) {
            SkDebugf("gles fence 3.0\n");
            ret.reset(new GLFenceSync(ctx));
        } else {
            SkDebugf("gles but with no fence\n");
            return nullptr;
        }
    }
    if (!ret->validate()) {
...
bool GLFenceSync::waitFence(sk_gpu_test::PlatformFence fence) const {
    ...
    return GL_WAIT_FAILED != fGLClientWaitSync(glsync, 0, -1);
}
