...
type FeatureSet struct {
	// One set of requirements for each of the GL "standards" that
...
type Requirement struct {
	...
	MultipleExtensions *[]string  `json:"multiple_ext"`
	MinVersion         *GLVersion `json:"min_version"`
	SuffixOverride     *string    `json:"suffix"`
}

// a hard code function does not use the C++ macro and just directly
...
var GLRequirements = func(fs FeatureSet) []Requirement {
	return fs.GLReqs
}

var GLESRequirements = func(fs FeatureSet) []Requirement {
	return fs.GLESReqs
}

var WebGLRequirements = func(fs FeatureSet) []Requirement {
	return fs.WebGLReqs
}
		...
		for idx, req := range reqs {
			...
			for idx, function := range feature.Functions {
				...
				if feature.EGLProc {
					...
					suffix := deriveSuffix((*req.MultipleExtensions)[idx])
					block = addLine(block, fmt.Sprintf("GET_PROC_SUFFIX(%s, %s);", function, suffix))
				}
			}
...
func requirementIfExpression(req Requirement, isLocal bool) string {
	...
	// We know it has an extension or multiple extensions
	if req.Extension != "" {
		if mv == nil {
	...
	if req.MultipleExtensions == nil {
		abort(fmt.Sprintf("ERROR: requirement must have ext or multiple_ext\n"))
	}
	checks := []string{}
	for _, ext := range *req.MultipleExtensions {
		checks = append(checks, fmt.Sprintf("%s.has(%q)", extVar, ext))
	}
	if mv == nil {
		return "(" + strings.Join(checks, " && ") + ")"
	} else {
		return fmt.Sprintf("(glVer >= GR_GL_VER(%d,%d) && (%s))", mv[0], mv[1], strings.Join(checks, " && "))
	}
...
func functionCheck(feature FeatureSet, indentLevel int) string {
	...
	if len(checks) == 0 {
		return strings.Repeat(TAB, indentLevel) + "// all functions were marked optional\n"
	}
...
func validateFeatures(features []FeatureSet) {
	...
	for _, feature := range features {
		for _, fn := range feature.Functions {
			if seen[fn] {
				abort(fmt.Sprintf("ERROR: Duplicate function %s\n", fn))
			}
			seen[fn] = true
		}
	}
...
func main() {
	...
	if err != nil {
		abort(fmt.Sprintf("Invalid JSON: %s", err))
	}
