...
void EGLGLTestContext::setupFenceSync(sk_sp<const GrGLInterface> interface) {
    ...
    auto grEGLClientWaitSyncKHR = (PFNEGLCLIENTWAITSYNCKHRPROC) eglGetProcAddress("eglClientWaitSyncKHR");
    ...
    glInt->fFunctions.fFenceSync = [grEGLCreateSyncKHR](GrGLenum condition, GrGLbitfield flags) {
    ...
    glInt->fFunctions.fDeleteSync = [grEGLDestroySyncKHR](GrGLsync sync) {
        EGLFenceData* data = reinterpret_cast<EGLFenceData*>(sync);
        grEGLDestroySyncKHR(data->fDisplay, data->fSync);
        delete data;
    };

    glInt->fFunctions.fClientWaitSync =
            [grEGLClientWaitSyncKHR] (GrGLsync sync, GrGLbitfield flags,
    ...
    glInt->fFunctions.fWaitSync =
            [grEGLClientWaitSyncKHR, grEGLWaitSyncKHR](GrGLsync sync, GrGLbitfield flags,
                                                       GrGLuint64 timeout) {
        EGLFenceData* data = reinterpret_cast<EGLFenceData*>(sync);
    ...
    glInt->fFunctions.fIsSync = [](GrGLsync sync) -> GrGLboolean {
        SkASSERT(false);
        return true;
    };
}
