...
static EGLContext create_egl_context(EGLDisplay display,
...
EGLGLTestContext::EGLGLTestContext(GrGLStandard forcedGpuAPI, EGLGLTestContext* shareContext)
    ...
    for (; nullptr == gl.get() && api < apiLimit; ++api) {
        ...
        if (kGL_GrGLStandard == kStandards[api]) {
            ...
            fContext = create_egl_context(fDisplay, surfaceConfig, eglShareContext, 3);
            if (EGL_NO_CONTEXT == fContext) {
                fContext = create_egl_context(fDisplay, surfaceConfig, eglShareContext, 2);
            }
        }
        if (EGL_NO_CONTEXT == fContext) {
