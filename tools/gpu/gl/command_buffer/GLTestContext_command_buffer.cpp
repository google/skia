...
// The command buffer does not correctly imlement eglGetCurrent. It always returns EGL_NO_<foo>.
// So we implement themselves and hook eglMakeCurrent to store the current values in TLS.
...
EGLSurface fakeGetCurrentSurface(EGLint readdraw) {
    if (EGL_DRAW == readdraw) {
        return gCurrDrawSurface;
    } else if (EGL_READ == readdraw) {
