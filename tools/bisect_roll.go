...
func main() {
	...
	gUrl := "https://chromium-review.googlesource.com"
	...
	if latestSuccess != -1 {
		...
		log("Recommend landing successful roll %s/%d", gUrl, rollCls[suggestedCommit].Issue)
		...
		if issue, ok := rollCls[suggestedCommit]; ok && issue.Result == autoroll.ROLL_RESULT_IN_PROGRESS {
			log("Recommend waiting for the current in-progress roll to finish: %s/%d", gUrl, issue.Issue)
			suggestedCommit = ""
		} else {
	...
	log("Uploaded %s/%d", gUrl, issue)

	// TODO(borenet): Optionally:
	// - Wait for the CQ to finish.
	// - GOTO beginning.
}
