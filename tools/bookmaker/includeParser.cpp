    ...
    for (auto& token : tokens) {
        ...
        switch (token.fMarkType) {
            case MarkType::kEnum:
                ...
                this->writeString("enum " + token.fName + " {");
                this->lfAlways(1);
                for (auto& child : token.fChildren) {
...
bool IncludeParser::parseEnum(Definition* child, Definition* markupDef) {
    ...
    do {
        ...
        if (',' == dataEnd[0]) {
            parser.skipTo(dataEnd);
            dataEnd = parser.anyOf("/\n");
        }
        if ('/' == dataEnd[0] && ('/' == dataEnd[1] || '*'  == dataEnd[1])) {
            if ('/' == dataEnd[1]) {
                parser.skipToEndBracket('\n');
            } else {
                parser.skipToEndBracket("*/");
            }
            const char* commentStart = &dataEnd[2];
            ...
            if (!this->parseComment(parser.fFileName, commentStart, commentEnd, parser.fLineCount,
                    comment)) {
                return false;
            }
        }
