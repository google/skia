...
void IncludeParser::stringAppend(string& result, char ch) const {
...
string IncludeParser::writeCodeBlock(const Definition& iDef, MarkType markType) const {
    TextParser i(&iDef);
    return writeCodeBlock(i, markType);
}

string IncludeParser::writeCodeBlock(TextParser& i, MarkType markType) const {
    string result;
    CheckCode check;
    char last;
    int lastIndent = 0;
    ...
    if (MarkType::kFunction != markType && lastIndent > 0) {
        this->stringAppend(result, '}');
    }
    this->stringAppend(result, ';');
    if (MarkType::kFunction != markType) {
        this->stringAppend(result, '\n');
    }
    return result;
}
...
void IncludeParser::writeCodeBlock(const BmhParser& bmhParser) {
    ...
    for (auto& enumMapper : fIEnumMap) {
        string enumName = enumMapper.first;
        auto finder = bmhParser.fClassMap.find(enumName);
        if (bmhParser.fClassMap.end() != finder) {
            enumMapper.second->fCode = this->writeCodeBlock(*enumMapper.second,
                    MarkType::kEnumClass);
            continue;
        }
        finder = bmhParser.fEnumMap.find(enumName);
        if (bmhParser.fClassMap.end() != finder) {
            enumMapper.second->fCode = this->writeCodeBlock(*enumMapper.second, MarkType::kEnum);
            continue;
        }
        SkASSERT(string::npos != enumName.find("::"));
    }
}
