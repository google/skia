...
void IncludeParser::dumpGlobals() {
    ...
    lastSlash += 1;
    string filename = fFileName.substr(lastSlash, lastDotH - lastSlash);
    for (const auto& entry : fIDefineMap) {
        this->dumpDefine(entry.second);
        this->dumpCommonTail(entry.second);
    }
    for (const auto& entry : fIFunctionMap) {
        this->dumpMethod(entry.second, filename);
        this->dumpCommonTail(entry.second);
    }
    for (const auto& entry : fIEnumMap) {
        this->dumpEnum(entry.second, filename);
        this->dumpCommonTail(entry.second);
    }
    ...
    for (const auto& entry : fITypedefMap) {
        ...
        this->writeSpace();
        this->writeString(entry.second.fName);
        this->lf(2);
        this->dumpComment(entry.second);
...
bool IncludeParser::parseDefine(Definition* child, Definition* markupDef) {
    ...
    if (!markupDef) {
        Definition& defineDef = fIDefineMap[nameStr];
        SkASSERT(!defineDef.fStart);
        defineDef.fStart = child->fContentStart;
        defineDef.fContentStart = nameStart;
        defineDef.fName = nameStr;
        defineDef.fFiddle = nameStr;
        defineDef.fContentEnd = child->fContentEnd;
        defineDef.fTerminator = child->fContentEnd;
        defineDef.fMarkType = MarkType::kDefine;
        defineDef.fLineCount = child->fLineCount;
        defineDef.fFileName = child->fFileName;
        for (Param param : params) {
            defineDef.fTokens.emplace_back(MarkType::kParam, param.fStart, param.fEnd,
                    child->fLineCount, &defineDef, '\0');
            Definition* paramChild = &defineDef.fTokens.back();
...
bool IncludeParser::parseMethod(Definition* child, Definition* markupDef) {
    ...
    if (!markupDef) {
        ...
        name = this->uniqueName(fIFunctionMap, name);
...
bool IncludeParser::parseTypedef(Definition* child, Definition* markupDef) {
    ...
    if (!markupDef) {
        ...
        typedefDef.fFileName = child->fFileName;
        return true;
    }
