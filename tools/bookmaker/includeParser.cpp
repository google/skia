...
bool IncludeParser::dumpGlobals() {
    ...
    for (const auto& entry : fITypedefMap) {
        ...
        this->writeSpace();
        TextParser parser(entry.second);
...
bool IncludeParser::parseDefine(Definition* child, Definition* markupDef) {
    ...
    if (!markupDef) {
        fParent->fTokens.emplace_back(MarkType::kDefine, nameStart, child->fContentEnd,
                child->fLineCount, fParent, '\0');
        Definition* globalMarkupChild = &fParent->fTokens.back();
        string globalUniqueName = this->uniqueName(fIDefineMap, nameStr);
        globalMarkupChild->fName = globalUniqueName;
        globalMarkupChild->fTerminator = globalMarkupChild->fContentEnd;
...
bool IncludeParser::parseMethod(Definition* child, Definition* markupDef) {
    ...
    if (!markupDef) {
        ...
        fParent->fTokens.emplace_back(MarkType::kMethod, start, end, tokenIter->fLineCount,
                fParent, '\0');
        Definition* globalMarkupChild = &fParent->fTokens.back();
...
bool IncludeParser::parseTypedef(Definition* child, Definition* markupDef) {
    ...
    if (!markupDef) {
        fParent->fTokens.emplace_back(MarkType::kTypedef, child->fContentStart, child->fContentEnd,
                child->fLineCount, fParent, '\0');
        Definition* globalMarkupChild = &fParent->fTokens.back();
