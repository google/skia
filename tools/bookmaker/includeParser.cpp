...
bool IncludeParser::advanceInclude(TextParser& i) {
    ...
    if (i.startsWith("//")) {
        const char* start = i.fChar;
        i.skipToLineStart(&fCheck.fIndent, &fCheck.fWriteReturn);
        fPriorUndocumentable = this->isUndocumentable(i.fFileName, start, i.fChar, i.fLineCount);
        return false;
    }
    if (i.startsWith("/*")) {
        const char* start = i.fChar;
        i.skipToEndBracket("*/");
        fUndocumentable = this->isUndocumentable(i.fFileName, start, i.fChar, i.fLineCount);
        i.skipToLineStart(&fCheck.fIndent, &fCheck.fWriteReturn);
        return false;
    }
    ...
    /* TODO: if previous comment (or comment on same line) has experimental/deprecated/private
       (but not "to be deprecated" or "may be deprecated") treat the associated symbol as private
     */
...
string IncludeParser::writeCodeBlock(TextParser& i, MarkType markType, int startIndent) {
    ...
    // start here
    // need to rewrite this to use info already parsed because it knows whats deprecated or private
    // probably can just iterate through parent fToken array since it is already marked undocumented
    fUndocumentable = false;
    fPriorUndocumentable = false;
    do {
        if (!this->advanceInclude(i)) {
            if (fUndocumentable || fPriorUndocumentable) {
                SkDebugf("");
            }
            fUndocumentable = fPriorUndocumentable = false;
            continue;
        }
        do {
            last = i.peek();
