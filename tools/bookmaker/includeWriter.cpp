    ...
    while (iterState->fDefIter != iterState->fDefEnd) {
        ...
        if (ItemState::kName == state) {
            ...
            if (ItemState::kValue == state) {  // write enum value
                ...
                string itemValue =
                        string(token.fContentStart, (int) (lastEnd - token.fContentStart));
        ...
        if (ItemState::kValue == state) {
            ...
            if (!end) {  // write expression continuation
                string itemValueContinuation = string(lastEnd,
                        (int) (token.fContentEnd - lastEnd));
                itemValues.push_back(itemValueContinuation);
                continue;
            }
        }
        if (ItemState::kNone != state) {
            ...
            if (trimNeeded > 0 && itemValues.size() < 2) {
                if (itemValues.size()) {
                    int valueSpare = enumItemCommentTab - enumItemValueTab - itemValues[0].length();
                ...
                if (trimNeeded > 0) {
                    ...
                    if (itemValues.size()) {
                        enumItemCommentTab += itemValues[0].length() + 3;
                    }
                    crAfterName = true;
                }
            }
            ...
            if (itemValues.size()) {
                ...
                for (auto oneValue : itemValues) {
                    if (subsequent) {
                        this->lfcr();
                    } else {
                        this->writeSpace();
                    }
                    this->writeString(oneValue);
                    subsequent = true;
                }
            }
            this->writeString(",");
