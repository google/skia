...
void IncludeWriter::checkEnumLengths(const Definition& child, string enumName, ItemLength* length) const {
    ...
    if (length->fCurName + length->fCurValue + comment.length() < lineLimit) {
...
string IncludeWriter::enumMemberComment(const Definition* currentEnumItem,
    ...
    for (auto constItem : currentEnumItem->fChildren) {
        ...
        if (MarkType::kDeprecated == constItem->fMarkType) {
...
void IncludeWriter::enumMemberOut(const Definition* currentEnumItem, const Definition& child,
    ...
    if (valueLength) {
        valueLength += 3; // 3: space = space
    }
    int enumItemCommentTab = SkTMax(enumItemValueTab + valueLength, fEnumItemCommentTab);
    ...
    if (trimNeeded > 0) {
        ...
        if (trimNeeded > 0) {
            crAfterName = true;
            enumItemCommentTab = fEnumItemCommentTab;
            enumItemValueTab = fEnumItemValueTab;
        }
    }
    this->lfcr();
