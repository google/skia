...
bool StatusIter::parseFromFile(const char* path) {
    ...
    fBlockIndex = 0;
    // find first array that matches fFilter, fSuffix
...
bool StatusIter::startBlock() {
    if (fBlockIndex >= SK_ARRAY_COUNT(block_names)) {
        return false;
    }
    fBlock = fRoot[block_names[fBlockIndex]];
    if (fBlock.empty()) {
        ++fBlockIndex;
        return this->startBlock();
    }
...
bool StatusIter::startDirectory() {
    Json::ValueType valueType = fBlock.type();
