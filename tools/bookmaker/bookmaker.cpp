...
void HackParser::topicIter(const Definition* topic) {
    ...
    for (auto child : topic->fChildren) {
        ...
        switch (child->fMarkType) {
            ...
            case MarkType::kTopic:
            case MarkType::kSubtopic:
                oneLiner = this->searchTable(fRelatedFunctions, child);
                if (!hasLine && "" != oneLiner) {
                    const char* start = fChar;
                    const char* end = child->fContentStart;
                    fprintf(fOut, "%.*s", (int) (end - start), start);
            ...
            case MarkType::kStruct:
            case MarkType::kClass:
                oneLiner = this->searchTable(fClassesAndStructs, child);
                if (!hasLine && "" != oneLiner) {
                    const char* start = fChar;
                    const char* end = child->fContentStart;
                    fprintf(fOut, "%.*s", (int) (end - start), start);
            ...
            case MarkType::kEnum:
            case MarkType::kEnumClass:
                oneLiner = this->searchTable(fConstants, child);
                if (!hasLine && "" != oneLiner) {
                    const char* start = fChar;
                    const char* end = child->fContentStart;
                    fprintf(fOut, "%.*s", (int) (end - start), start);
                ...
                break;
            case MarkType::kMember:
                oneLiner = this->searchTable(fMembers, child);
                if (!hasLine && "" != oneLiner) {
                    const char* start = fChar;
                    const char* end = child->fContentStart;
                    fprintf(fOut, "%.*s", (int) (end - start), start);
