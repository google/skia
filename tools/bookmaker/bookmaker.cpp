            ...
            if (isAlias) {
                ...
                for (auto aliasChild : child->fChildren) {
                    if (MarkType::kSubstitute == aliasChild->fMarkType) {
                        ...
                        this->setUpSubstitute(sub, topic.second, &fGlobals);
                    }
                }
            }
            this->setUpSubstitute(name, topic.second, &fGlobals);
        }
        if (hasSubstitute) {
            continue;
        }
        string lowered = this->loweredTopic(topic.first, topic.second, &fGlobals);
