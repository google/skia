...
string BmhParser::loweredTopic(string name, Definition* def, NameMap* names) {
    string lowered;
    SkASSERT('_' != name[0]);
    char last = '_';
    for (char c : name) {
        ...
        lowered += c;
        if (' ' == c) {
            this->setUpPartialSubstitute(lowered, names);
        }
    }
    if (isupper(last)) {
...
void BmhParser::setUpGlobalSubstitutes() {
    ...
    for (auto bMap : { &fClassMap, &fConstMap, &fDefineMap, &fEnumMap, &fMethodMap,
        ...
        for (auto& entry : *bMap) {
            ...
            this->setUpSubstitutes(parent, Substitutes::kGlobal, &fGlobalNames);
        }
    }
            ...
            if (isAlias) {
                ...
                for (auto aliasChild : child->fChildren) {
                    if (MarkType::kSubstitute == aliasChild->fMarkType) {
                        ...
                        this->setUpSubstitute(sub, topic.second, &fGlobalNames);
                    }
                }
            }
            this->setUpSubstitute(name, topic.second, &fGlobalNames);
        }
        if (hasSubstitute) {
            continue;
        }
        string lowered = this->loweredTopic(topic.first, topic.second, &fGlobalNames);
...
void BmhParser::setUpSubstitutes(const Definition* parent, Substitutes sub, NameMap* names) {
    for (const auto& child : parent->fChildren) {
        ...
        if (MarkType::kClass == markType || MarkType::kStruct == markType
            ...
            for (auto entry : nameMap->fRefMap) {
                string scoped = entry.first;
                if (string::npos != scoped.find("fXMin")) {
                    SkDebugf("start here");
                }
                if (string::npos == scoped.find("::")) {
                    continue;
                }
                SkASSERT(names->fRefMap.end() == names->fRefMap.find(scoped));
                names->fRefMap[scoped] = entry.second;
                auto scopedLinkIter = nameMap->fLinkMap.find(entry.first);
                if (nameMap->fLinkMap.end() != scopedLinkIter) {
                    names->fLinkMap[scoped] = scopedLinkIter->second;
                }
            }
        }
        ...
        if (MarkType::kSubtopic == markType) {
            if (Substitutes::kLocal == sub && string::npos != child->fName.find('_')) {
                string lowered = this->loweredTopic(child->fName, child, names);
...
void BmhParser::setUpPartialSubstitute(string name, NameMap* names) {
    auto iter = names->fRefMap.find(name);
    if (names->fRefMap.end() != iter) {
        SkASSERT(nullptr == iter->second);
        return;
    }
    names->fRefMap[name] = nullptr;
}

void BmhParser::setUpSubstitute(string name, Definition* def, NameMap* names) {
    SkASSERT(names->fRefMap.end() == names->fRefMap.find(name));
    names->fRefMap[name] = def;
    SkASSERT(names->fLinkMap.end() == names->fLinkMap.find(name));
    string str = ParserCommon::HtmlFileName(def->fFileName) + '#' + def->fName;
    names->fLinkMap[name] = str;
    ...
    do {
        ...
        stop = space - 1;
        this->setUpPartialSubstitute(partial, names);
    } while (true);
}
