...
Enum (anonymous) should not emit "SkXXX::_anonymous"
      e.g., remove anonymous header, e.g. Enum SkPaint::::anonymous_2
Probably the body of the consts could be examined to emit "Enum for SaveLayerFlag values"
should add an alternate title so the code doesn't have to guess
alternative fix: replace anonymous enums with named ones and constexpr: review.skia.org/123584
...
followed by more elaborate descriptions, examples, seealso.
