...
DEFINE_string2(status, j, "", "File containing status of documentation. (Use in place of -b -i)");
...
int main(int argc, char** const argv) {
    ...
    SkCommandLineFlags::SetUsage(
        ...
        "              bookmaker -j path/to/status.json -x\n"
        "              bookmaker -j path/to/status.json -p\n");
    bool help = false;
    ...
    if (!FLAGS_include.isEmpty() && FLAGS_tokens) {
        ...
        if (FLAGS_crosscheck) {
            ...
            if (!FLAGS_status.isEmpty() && !includeParser.parseStatus(FLAGS_include[0], ".h",
            ...
            if (!FLAGS_status.isEmpty() && !includeWriter.parseStatus(FLAGS_include[0], ".h",
                    StatusFilter::kCompleted)) {
                return -1;
            }
