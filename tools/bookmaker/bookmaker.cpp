...
, { "Code",         MarkType::kCode,         R_O, E_N, M_CSST | M_E | M(Method) | M(Define) | M(Typedef) }
...
bool BmhParser::exampleToScript(Definition* def, ExampleOptions exampleOptions,
    ...
    size_t nameStart = def->fFileName.rfind(SkOSPath::SEPARATOR);
    if (SkOSPath::SEPARATOR != '/') {
        size_t altNameStart = def->fFileName.rfind('/');
