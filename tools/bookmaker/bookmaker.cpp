...
void BmhParser::setUpGlobalSubstitutes() {
    for (auto& entry : fExternals) {
        ...
        SkASSERT(fRefMap.end() == fRefMap.find(externalName));
        fRefMap[externalName] = nullptr;
    }
    for (auto bMap : { &fClassMap, &fConstMap, &fDefineMap, &fEnumMap, &fMethodMap,
        ...
        for (auto& entry : *bMap) {
            ...
            SkASSERT(fGlobalLinkMap.end() == fGlobalLinkMap.find(name));
            string ref = ParserCommon::HtmlFileName(parent->fFileName) + '#' + parent->fFiddle;
            fGlobalLinkMap[name] = ref;
            SkASSERT(fRefMap.end() == fRefMap.find(name));
            fRefMap[name] = const_cast<Definition*>(parent);
            BmhParser::SetUpSubstitutes(parent, Substitutes::kGlobal,
                    fGlobalLinkMap, fRefMap);
        }
    }
    for (auto& topic : fTopicMap) {
        ...
        SkDEBUGCODE(auto globalIter = fGlobalLinkMap.find(lowered));
        SkASSERT(fGlobalLinkMap.end() == globalIter);
        fGlobalLinkMap[lowered] =
                ParserCommon::HtmlFileName(topic.second->fFileName) + '#' + topic.first;
        SkASSERT(fRefMap.end() == fRefMap.find(lowered));
        fRefMap[lowered] = topic.second;
    }
    ...
    do {
        ...
        if (fRefMap.end() == fRefMap.find(word)) {
            fRefMap[word] = nullptr;
        } else {
...
void BmhParser::setUpLocalSubstitutes(Definition* parent) {
    fLocalLinkMap.clear();
    fLocalRefMap.clear();
    BmhParser::SetUpSubstitutes(parent, Substitutes::kLocal, fLocalLinkMap, fLocalRefMap);
}

void BmhParser::SetUpSubstitutes(const Definition* parent, Substitutes sub,
    ...
    for (const auto& child : parent->fChildren) {
        ...
        if (MarkType::kClass == markType || MarkType::kStruct == markType
            ...
            SkASSERT(linkMap.end() == linkMap.find(name));
            linkMap[name] = ref;
            SkASSERT(refMap.end() == refMap.find(name));
            refMap[name] = child;
        }
        if (MarkType::kClass == markType || MarkType::kStruct == markType
                || MarkType::kSubtopic == markType) {
            BmhParser::SetUpSubstitutes(child, sub, linkMap, refMap);
        }
    }
}

void BmhParser::setUpPartialSubstitute(string name) {
    auto iter = fRefMap.find(name);
    if (fRefMap.end() != iter) {
        SkASSERT(nullptr == iter->second);
        return;
    }
    fRefMap[name] = nullptr;
}
