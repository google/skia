...
bool BmhParser::exampleToScript(Definition* def, ExampleOptions exampleOptions,
        string* result) const {
    if ("SkBitmap_bytesPerPixel" == def->fFiddle) {
        SkDebugf("");
    }
    bool hasFiddle = true;
...
bool BmhParser::popParentStack(Definition* definition) {
    ...
    if (fMC != end[0]) {
        ...
        SkASSERT(&end[-2] >= definition->fContentStart && fMC == end[-1] && fMC == end[-2]);
        end -= 2;
    }
    definition->fContentEnd = end;
    definition->fTerminator = fChar;
    if (MarkType::kReturn == definition->fMarkType) {
        if (string::npos != definition->fFiddle.find("tryAllocPixels")) {
        SkDebugf("");
        }
    }
