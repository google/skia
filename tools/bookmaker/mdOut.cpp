...
string const_out_table_data_code_ref(const Definition* def) {
    return kTD_Left + "<a href='" + def->fFiddle + "'> <code>" + def->fName + "</code> </a></td>";
}

string const_out_table_data_code_def(const Definition* def) {
...
void MdOut::constOut(const Definition* def) {
    ...
    for (auto child : def->fChildren) {
        ...
        this->lfAlways(1);
        FPRINTF(const_out_table_data_code_ref(def).c_str());
        this->lfAlways(1);
...
void MdOut::markTypeOut(Definition* def) {
    ...
    switch (def->fMarkType) {
        ...
        // optional link to subtopic with longer descriptions, examples
            ...
            this->writePending();
            FPRINTF(const_out_table_data_code_def(def).c_str());
            this->lfAlways(1);
