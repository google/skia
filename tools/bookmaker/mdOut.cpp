    ...
    if (resolved.length()) {
        ...
        while (!paragraph.eof()) {
            ...
            if (lineLength) {
                ...
                if (BmhParser::Resolvable::kEscape == resolvable) {
                    str = preformat(str);
                }
...
void MdOut::rowOut(const char* name, const string& description) {
    this->lfAlways(1);
    FPRINTF("| ");
    this->resolveOut(name, name + strlen(name), BmhParser::Resolvable::kEscape);
    FPRINTF(" | ");
    this->resolveOut(&description.front(), &description.back() + 1, BmhParser::Resolvable::kEscape);
    FPRINTF(" |");
    this->lf(1);
}
...
void MdOut::subtopicOut(const TableContents& tableContents) {
    ...
    for (auto entry : items) {
        ...
        if (tableContents.fShowClones && entry.second->fCloned) {
            ...
            builder += '_';
            this->rowOut("", entry.second->formatFunction(Definition::Format::kOmitReturn));
            do {
                ...
                this->rowOut("", child->formatFunction(Definition::Format::kOmitReturn));
            } while (++cloneNo);
        }
    }
}
