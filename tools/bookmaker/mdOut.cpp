...
string table_data_code_ref(string ref, string name) {
    return kTD_Left + "<a href='" + ref + "'> <code>" + name + "</code> </a></td>";
}

string table_data_code_ref(const Definition* ref) {
    return table_data_code_ref(ref->fFiddle, ref->fName);
}

string table_data_code_def(string def, string name) {
    return kTD_Left + "<a name='" + def + "'> <code>" + name + "</code> </a></td>";
}

string table_data_code_def(const Definition* def) {
    return table_data_code_def(def->fFiddle, def->fName);
}

string table_data_const(const Definition* def, const char** textStartPtr) {
...
string out_table_data_description_start() {
    return kTD_Left;
}

string out_table_data_description(string str) {
    return kTD_Left + str + "</td>";
}

string out_table_data_description(const Definition* def) {
...
string out_table_data_details(string details) {
...
bool MdOut::buildRefFromFile(const char* name, const char* outDir) {
    ...
    for (auto key : keys) {
        ...
        fPopulators[kConstants].fDetails =
                "Constants defined by <code>enum</code>, <code>enum class</code>,"
                " <code>#define</code>, <code>const</code>, and <code>constexpr</code>.";
        ...
        fPopulators[kMemberFunctions].fOneLiner = "static functions and member methods";
        ...
        fPopulators[kRelatedFunctions].fOneLiner = "similar methods grouped together";
        fPopulators[kSubtopics].fName = "";
        fPopulators[kSubtopics].fOneLiner = "";
...
void MdOut::summaryOut(const Definition* def, MarkType markType, string name) {
    ...
    for (auto child : def->fChildren) {
        ...
        if (MarkType::kConst == markType) {
            ...
            this->lfAlways(1);
            FPRINTF(table_data_code_ref(memberName, memberName).c_str());
        }
        this->lfAlways(1);
    ...
    for (auto item : { kClassesAndStructs, kConstants, kConstructors, kDefines,
        ...
        for (auto entry : this->populator(item).fMembers) {
            if (entry->csParent() == csParent || entry->topicParent() == topicParent) {
                ...
                string link = "<a href='#" + string(item)  + "'>" + popItem->second.fName + "</a>";
...
void MdOut::subtopicOut(const TableContents& tableContents) {
    ...
    if (tableContents.fDetails.length()) {
        this->writeString(tableContents.fDetails);
        this->lfAlways(1);
    }
    FPRINTF("%s", kTableDeclaration);
    this->lfAlways(1);
