...
string MdOut::addReferences(const char* refStart, const char* refEnd,
    ...
    do {
        ...
        if (BmhParser::Resolvable::kCode == resolvable && !t.eof()) {
            if ('{' == t.peek()) {
                if (KeyWord::kClass == keyWord || KeyWord::kStruct == keyWord) {
                    if ("" != t.localName()) {
                        const Definition* classDef = this->isDefined(t, t.localName(), resolvable);
                        if (classDef->isRoot()) {
                ...
                lastKeyWord = KeyWord::kNone;
                keyWord = KeyWord::kNone;
            }
        }
        bool sawCloseBrace = t.skipToMethodStart();
        // start here
        // fixes blendmode global function but
        // breaks tons of other stuff
        if (false && BmhParser::Resolvable::kCode == resolvable && sawCloseBrace) {
            if (fRoot->fParent) {
                fRoot = fRoot->fParent->asRoot();
                t.setLocalName(fRoot->fName);
            }
