...
struct BraceState {
    BraceState(const RootDefinition* root, const char* ch, KeyWord last, KeyWord keyWord, int count)
        : fRoot(root)
        , fChar(ch)
        , fLastKey(last)
        , fKeyWord(keyWord)
...
//    bool structClassSet = false;
    ...
    if (BmhParser::Resolvable::kCode == resolvable) {
        braceStack.push_back({fRoot, t.fChar, lastKeyWord, keyWord, 0});
    }
    do {
        ++distFromParam;
        ...
        if (BmhParser::Resolvable::kCode == resolvable) {
            ...
            if (braceCount > priorBrace) {
                if (KeyWord::kClass == keyWord || KeyWord::kStruct == keyWord) {
                    ...
                    const Definition* classDef = this->isDefined(t, ref, resolvable);
                    if (!classDef) {
                        string className = fRoot->csParent()->fName;
                        string withRoot = className + "::" + ref;
                        classDef = this->isDefined(t, withRoot, resolvable);
                        SkASSERT(classDef);
                    }
                    if (classDef->isRoot()) {
                        fRoot = const_cast<Definition*>(classDef)->asRoot();
                        t.setLocalName(fRoot->fName);
                    }
                }
                braceStack.push_back({fRoot, t.fChar, lastKeyWord, keyWord, braceCount});
            } else if (braceCount < priorBrace) {
                lastKeyWord = braceStack.back().fLastKey;
                keyWord = braceStack.back().fKeyWord;
                if (KeyWord::kClass == keyWord || KeyWord::kStruct == keyWord) {
                    fRoot = fRoot->fParent->asRoot();
                    t.setLocalName(MarkType::kTopic == fRoot->fMarkType ? "" : fRoot->fName);
                }
                braceStack.pop_back();
            }
        }
...
#if 0
            if ((KeyWord::kStruct == keyWord || KeyWord::kClass == keyWord)
                    && BmhParser::Resolvable::kCode == resolvable && !structClassSet) {
                structClassSet = true;
                t.setLocalName(ref);
            }
#endif
...
//                        || '.' != t.backup(ref.c_str())
            ...
            if (KeyWord::kPrivate != newKeyWord && KeyWord::kProtected != newKeyWord
                ...
                keyWord = newKeyWord;
            #if 0
                if (KeyWord::kEnum != lastKeyWord) {    // don't treat enum class like class
                    if (BmhParser::Resolvable::kCode == resolvable
                            && (KeyWord::kClass == keyWord || KeyWord::kStruct == keyWord)) {
                        structClassSet = false;
                        t.setLocalName("");
                    }
                }
            #endif
...
void MdOut::addCodeBlock(const Definition* def, string& result) const {
    ...
    if (string::npos != def->fName.find("Canvas")) {
        SkDebugf("");
    }
