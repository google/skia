...
//                        || '.' != t.backup(ref.c_str())
                    ...
                    if ('.' == wordStart[0] && distFromParam == 1) {
                        ...
                        SkDebugf("");
                    //    start here;
                        // probably need to have cached last word, mark that it was a param,
                        // cache param description? and be able to lookup param class or struct?
                    }
                    if (BmhParser::Resolvable::kOut != resolvable) {
