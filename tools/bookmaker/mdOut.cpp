...
void MdOut::addCodeBlock(const Definition* def, string& result) const {
    fDebugWriteCodeBlock = string::npos != def->fFileName.find("Color");
    ...
    for (auto member : def->fChildren) {
        ...
        if (KeyWord::kClass == member->fKeyWord || KeyWord::kStruct == member->fKeyWord) {
            // todo: Make sure this was written somewhere else
            SkASSERT(0);
            continue;
        }
        ...
        if (Bracket::kParen == member->fBracket && prior) {
            TextParser function(member->fFileName, priorTerminator, member->fTerminator + 1,
                    member->fLineCount);
            this->stringAppend(result,
                    fIncludeParser.writeCodeBlock(function, MarkType::kFunction));
            this->stringAppend(result, '\n');
            wroteFunction = true;
            continue;
        }
        ...
        if (KeyWord::kConstExpr == member->fKeyWord) {
            ...
            start here;
            // get next member
            // parse text between members
            // if this member content end next char is ';', write only this member content
            // if this member content end next char is '=', write to next member content end
            //      plus write ')'
            this->stringAppend(result, ';');
            this->stringAppend(result, '\n');
            continue;
