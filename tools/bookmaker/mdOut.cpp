        ...
        if (BmhParser::Resolvable::kCode == resolvable && !t.eof()) {
            if ('{' == t.peek()) {
                if (KeyWord::kClass == keyWord || KeyWord::kStruct == keyWord) {
                    if (className.length()) {
                        const Definition* classDef = this->isDefined(t, className, resolvable);
                        if (classDef->isRoot()) {
...
//                t.next();
            ...
            if ("" == className && (KeyWord::kStruct == keyWord || KeyWord::kClass == keyWord)) {
                className = ref;
            }
            continue;
        }
...
void MdOut::addCodeBlock(const Definition* def, string& result) const {
    ...
    for (auto member : def->fChildren) {
        ...
        if (KeyWord::kIfndef == member->fKeyWord) {
            addCodeBlock(member, result);
            continue;
        }
        if (KeyWord::kClass == member->fKeyWord
                || KeyWord::kStruct == member->fKeyWord
                || KeyWord::kEnum == member->fKeyWord) {
            ...
            result += fIncludeParser.codeBlock(markType, name);
            result += '\n';
            continue;
        }
        ...
        if (Bracket::kParen == member->fBracket && prior) {
            TextParser function(member->fFileName, prior->fTerminator, member->fTerminator + 1,
                    member->fLineCount);
            result += fIncludeParser.writeCodeBlock(function, MarkType::kFunction);
            wroteFunction = true;
        }
        SkDebugf("");
    }
...
void MdOut::markTypeOut(Definition* def, const Definition** prior) {
 ...
 //               SkASSERT(parent->csParent());
                ...
                if (MarkType::kClass == grand->fMarkType
                    ...
                    string codeBlock = fIncludeParser.codeBlock(*grand);
                    ...
                    const Definition* def = fIncludeParser.include(incName + ".h");
                    SkASSERT(def);
                    string codeBlock;
                    this->addCodeBlock(def, codeBlock);
                    this->resolveOut(codeBlock.c_str(), codeBlock.c_str() + codeBlock.length(),
                            this->resolvable(parent));
                    SkDebugf("");
                }
