    ...
    for (auto entry : items) {
        ...
        if (tableContents.fShowClones && entry.second->fCloned) {
            ...
            do {
                ...
                start here;
                // need to hierarchically walk csParent since it has subtopics with methods
                for (auto child : csParent->fChildren) {
                    if (!child->fClone) {
                        continue;
                    }
                    if (match == child->fName) {
                        // there's probably a function already to get the full function
