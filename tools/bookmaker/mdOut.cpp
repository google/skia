...
void MdOut::populateTables(const Definition* def) {
    for (auto child : def->fChildren) {
        ...
        if (fClassStack.size() > 0 && (fClassStack.back()->fName + "::Make" == child->fName
                || fClassStack.back()->fName + "::make" == child->fName)) {
            this->populator(kConstructors).push_back(child);
        }
    }
}
...
void MdOut::rowOut(const char* name, const string& description) {
    ...
    FPRINTF(" | ");
    this->resolveOut(&description.front(), &description.back(), BmhParser::Resolvable::kYes);
    FPRINTF(" |\n");
}
