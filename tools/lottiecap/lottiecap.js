...
let targetURL = `http://localhost:${options.port}/#${options.renderer}`;
...
async function driveBrowser() {
  ...
  try {
     // 15 seconds is plenty of time to wait for the json to be loaded once
     // This usually times out for super large json.
    await page.goto(targetURL, {
      timeout: 15000,
      waitUntil: 'networkidle0'
    });
    // 15 seconds is generally plenty of time to wait for the frames to
    // be drawn. This usually times out for json that causes errors in
    // the player.
