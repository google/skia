...
void SkChromeTracingTracer::flush() {
    ...
    uint64_t begin = std::chrono::high_resolution_clock::now().time_since_epoch().count();
    ...
    memStream.writeToAndReset(&fileStream);

    uint64_t end = std::chrono::high_resolution_clock::now().time_since_epoch().count();
    SkDebugf("Elapsed: %f ms\n", static_cast<double>(end - begin) * 1E-6);
    fileStream.flush();
}
