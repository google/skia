...
Json::Value SkChromeTracingTracer::traceEventToJson(const TraceEvent& traceEvent,
    ...
    json["ts"] = static_cast<double>(
            static_cast<int64_t>(traceEvent.fClockBegin - clockOffset)) * 1E-3;
...
void SkChromeTracingTracer::flush() {
    ...
    // If high_resolution_clock::is_steady, this is guaranteed to give us the minimum clock across
    // all events. Even if it isn't, we handle underflow correctly traceEventToJson (we just might
    // end up with some events having a negative timestamp, which is fine).
    uint64_t clockOffset = 0;
    if (!fBlocks.empty()) {
        clockOffset = fBlocks[0][0].fClockBegin;
    } else if (fEventsInCurBlock > 0) {
        clockOffset = fCurBlock[0].fClockBegin;
    }

    for (int i = 0; i < fBlocks.count(); ++i) {
        for (int j = 0; j < kEventsPerBlock; ++j) {
            root.append(this->traceEventToJson(fBlocks[i][j], &baseTypeResolver, clockOffset));
        }
    }

    for (int i = 0; i < fEventsInCurBlock; ++i) {
        root.append(this->traceEventToJson(fCurBlock[i], &baseTypeResolver, clockOffset));
    }
