...
public:
    ...
    template <typename T, typename... Args>
    T* emplace(Args&&... args) {
        T* result = new (fCursor) T{std::forward<Args>(args)...};
        fCursor += sizeof(T);
        return result;
    };
...
public:
    ...
    void generateFontMetrics(const SkTypefaceProxy& tf,
        ...
        Op* op = transport->startEmplace(OpCode::kFontMetrics, tf, rec);
        transport->endWrite();
        op = transport->startRead<Op>();
        transport->endRead();
    ...
    void generateMetrics(const SkTypefaceProxy& tf,
        ...
        Op* op = transport->startEmplace(OpCode::kGlyphMetrics, tf, rec);
