...
template <typename T>
class ArraySlice : public std::tuple<const T*, uint32_t> {
public:
    ArraySlice(const T* data, uint32_t size) : std::tuple<const T*, uint32_t>{data, size} { }
    ...
    static ArraySlice Empty() {return ArraySlice{nullptr, 0};}
};
...
public:
    ...
    void readSpecFromTransport(Transport* transport, PerStrike perStrike, PerGlyph perGlyph) {

    }

    template <typename PerStrike, typename PerGlyph>
    void readDataFromTransport(Transport* transport, PerStrike perStrike, PerGlyph perGlyph) {
        auto header = transport->startRead<Header>();
        for (int i = 0; i < header->strikeCount; i++) {
            auto strike = transport->read<StrikeSpec>();
            auto desc = transport->readDescriptor();
            auto strikeData = perStrike(strike, desc);
            for (int j = 0; j < strike->glyphCount; j++) {
                auto glyph = transport->read<SkGlyph>();
                ArraySlice<uint8_t> image = ArraySlice<uint8_t>::Empty();
                auto imageSize = glyph->computeImageSize();
                if (glyph->computeImageSize() != 0) {
                    image = transport->readArray<uint8_t>(imageSize);
                }
                perGlyph(strikeData, *glyph, image);
            }
        }
    }
...
public:
    ...
    void generateMetrics(const SkTypefaceProxy& tf,
        ...
        SK_ABORT("generateMetrics should not be called.");

        // Send generateMetrics
        Op* op = this->startOpWrite(OpCode::kGlyphMetrics, tf, rec);
        op->glyph = *glyph;
        fTransport->endWrite();

        // Receive generateMetrics
        op = fTransport->startRead<Op>();
        *glyph = op->glyph;
        fTransport->endRead();
    }
    ...
    void generateImage(const SkTypefaceProxy& tf,
        ...
        // Send generateImage
        Op* op = this->startOpWrite(OpCode::kGlyphImage, tf, rec);
        op->glyph = glyph;
        fTransport->endWrite();

        // Receive generateImage
        fTransport->startRead<Op>();
        auto image = fTransport->readArray<uint8_t>(glyph.rowBytes() * glyph.fHeight);
        memcpy(glyph.fImage, image.data(), glyph.rowBytes() * glyph.fHeight);
    }
...
static void final_draw(std::string outFilename,
    ...
    for (int i = 0; i < 1; i++)
    {
        ...
        transport->startEmplace<Op>(OpCode::kPrepopulateCache, SkFontID{0}, SkScalerContextRec{});
        filter.writeSpecToTransport(transport);
        transport->endWrite();
        // TODO: read from the transport;

        filter.readDataFromTransport(transport, blah, blah);
...
static void gpu(int readFd, int writeFd) {
    ...
    final_draw("test.png", &transport, &procs, picData.get());
}
    ...
    const std::string& skpName, int readFd, int writeFd)
{
    ...
    if (!gUseGpu) {
        final_draw("test-direct.png", &transport, nullptr, stream.get());
        return 0;
    }
        ...
        if (op == nullptr) { std::cout << "Exit op loop" << std::endl; break;}
            ...
            switch (op->opCode) {
                ...
                case OpCode::kGlyphImage : {
                    ...
                    op->glyph.fImage =
                        transport.allocateArray<uint8_t>(op->glyph.rowBytes() * op->glyph.fHeight);
                ...
                case OpCode::kPrepopulateCache : {
                    ...
                    auto header = in.read<TextBlobFilterCanvas::Header>();
                    out.startWrite<TextBlobFilterCanvas::Header>(*header);
                    for (int i = 0; i < header->strikeCount; i++) {
                        auto strike = in.read<TextBlobFilterCanvas::StrikeSpec>();
                        auto desc = in.readDescriptor();
                        desc->assertChecksum();
                        out.write<TextBlobFilterCanvas::StrikeSpec>(*strike);
                        out.writeDescriptor(*desc);
                        SkScalerContextRecDescriptor recDesc{*desc};
                        auto sc = rc.generateScalerContext(recDesc, strike->typefaceID);
                        (void) sc;
                        auto glyphIDs = in.readArray<SkPackedGlyphID>(strike->glyphCount);
                        for (auto glyphID : glyphIDs) {
                            ...
                            glyph.fImage =
                                out.allocateArray<uint8_t>(
                                    glyph.rowBytes() * glyph.fHeight);
                            sc->getImage(glyph);
                        }
                        out.endWrite();
                    }
                    std::cout << "Prepopulate size: " << in.size() << std::endl;
                    std::cout << "Prepopulate response size: " << out.size() << std::endl;
                    break;
                }
                default:
                    SK_ABORT("Bad op");
