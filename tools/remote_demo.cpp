...
public:
    ...
    static Transport DoubleBuffer(const Transport& transport) {
        return Transport{transport.fReadFd, transport.fWriteFd};
    }
...
public:
    struct Strike {
        Strike(SkFontID typefaceID_, uint32_t descLength_, int glyphCount_)
    ...
    void fillInTransport(Transport* transport) {
        transport->emplace<Header>((int)fDescMap.size());
        for (auto& i : fDescMap) {
            auto accum = &i.second;
            transport->emplace<Strike>(
...
public:
    ...
    void generateImage(const SkTypefaceProxy& tf,
        ...
        memcpy(glyph.fImage, image, glyph.rowBytes() * glyph.fHeight);
    }
    ...
    void generatePath(const SkTypefaceProxy& tf,
        ...
        path->readFromMemory(rawPath, op->pathSize);
        fTransport->endRead();
    }
...
static void final_draw(std::string outFilename,
    ...
    for (int i = 0; i < 1; i++)
    {
        ...
        filter.fillInTransport(transport);
        transport->endWrite();
            ...
            switch (op->opCode) {
                ...
                case OpCode::kPrepopulateCache : {
                    ...
                    for (int i = 0; i < header->strikeCount; i++) {
                        auto strike = in.read<TextBlobFilterCanvas::Strike>();
                        auto desc = in.readDescriptor();
                        desc->assertChecksum();
                        (void) desc;
                        auto glyphs = in.readArray<SkPackedGlyphID>(strike->glyphCount);
                        (void) glyphs;
                    }
                    std::cout << "Prepopulate size: " << transport.size() << std::endl;
                    break;
                }
                default:
                    SK_ABORT("Bad op");
