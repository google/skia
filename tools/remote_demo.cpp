    ...
    void readDataFromTransport(Transport* transport, PerStrike perStrike, PerGlyph perGlyph) {
        ...
        for (int i = 0; i < header->strikeCount; i++) {
            ...
            for (int j = 0; j < strike->glyphCount; j++) {
                ...
                ArraySlice<uint8_t> image = ArraySlice<uint8_t>{};
                auto imageSize = glyph->computeImageSize();
                if (imageSize != 0) {
...
static void final_draw(std::string outFilename,
    ...
    for (int i = 0; i < 1; i++)
    {
        ...
        auto perStrike = [](TextBlobFilterCanvas::StrikeSpec*, SkDescriptor*) {
            return true;
        };

        auto perGlyph = [](bool, SkGlyph*, ArraySlice<uint8_t>) { };

        transport->startRead();
        filter.readDataFromTransport(transport, perStrike, perGlyph);
        transport->endRead();
