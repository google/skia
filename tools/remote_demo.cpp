...
static void gpu(int readFd, int writeFd) {
    uint8_t picBuffer[10 * 1024 * kPageSize];
    ssize_t picSize = read(readFd, picBuffer, sizeof(picBuffer));
    ...
    auto pic = SkPicture::MakeFromData(picBuffer, picSize, &procs);
...
static int renderer(const std::string& skpName, int readFd, int writeFd) {
    ...
    write(writeFd, stream->data(), stream->size());
    std::cerr << "Waiting for scaler context ops." << std::endl;

    uint8_t glyphBuffer[1024 * kPageSize];

    Op* op = (Op*)glyphBuffer;
    while (true) {
        ssize_t size = read(readFd, glyphBuffer, sizeof(*op));
        if (size < 0) {break;}
        ...
        write(writeFd, glyphBuffer, writeSize);
    }
