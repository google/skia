...
protected:
        ...
        const SkTextBlob* blob, SkScalar x, SkScalar y, const SkPaint& paint) override
    {
        ...
        for (;!it.done(); it.next()) {
            ...
            this->processGlyphRun(blobMatrix, it, &runPaint);
        }
    }
...
private:
    ...
    void processGlyphRun(
        ...
        SkDrawLooper* looper = runPaint->getDrawLooper();
        SkDrawLooper::BlurShadowRec blurChanges;
        int numPasses;
        if (looper && looper->asABlurShadow(&blurChanges)) {
            numPasses = 2;
        } else {
            numPasses = 1;
            SkASSERT(looper == nullptr);
        }
        ...
        for (int pass = numPasses; pass > 0; pass--) {
            ...
            SkColor prevColor = 0;
            if (pass == 2) {
                prevColor = runPaint->getColor();
                runPaint->setColor(blurChanges.fColor);
                runMatrix.preTranslate(blurChanges.fOffset.x(),
                                       blurChanges.fOffset.y());
            }
            ...
            for (uint32_t index = 0; index < it.glyphCount(); index++) {
                ...
                if (cache &&
                    cache->isGlyphIdCached(glyphs[index], subPixelPos.x(), subPixelPos.y())) {
                    continue;
                }
...
static void prepopulate_cache(
    ...
    auto perStrike = [&strikeData, cache](TextBlobFilterCanvas::StrikeSpec* spec,
        ...
        SkScalerContextEffects effects;
        strikeData.cache = SkGlyphCache::DetachCache(tf, effects, desc, fontMetrics);
#if 0
        std::cout << std::hex << "prepop cache " << (intptr_t)strikeData.cache
    ...
    auto perGlyph = [&strikeData](SkGlyph* glyph, SkArraySlice<uint8_t> image) {
        //std::cout << glyph->getPackedID().raw() << " ";
