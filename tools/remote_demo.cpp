...
class ServerDiscardableManager : public SkRendererStrikeCacheGenerator::DiscardableHandleManager {
public:
...
class ClientDiscardableManager : public SkGPUStikeCachePopulator::DiscardableHandleManager {
public:
...
static bool push_font_data(const SkPicture& pic, SkRendererStrikeCacheGenerator* strikeServer, int writeFd) {
...
static void final_draw(std::string outFilename, SkData* picData, SkGPUStikeCachePopulator* client,
    ...
    auto decode = [](const void* data, size_t length, void* ctx) -> sk_sp<SkTypeface> {
        return reinterpret_cast<SkGPUStikeCachePopulator*>(ctx)->deserializeTypeface(data, length);
    };
...
static void gpu(int readFd, int writeFd) {
    ...
    if (gUseGpu) {
        ...
        SkGPUStikeCachePopulator strikeClient(discardableManager);
    ...
    const std::string& skpName, int readFd, int writeFd)
{
    ...
    SkRendererStrikeCacheGenerator server(&discardableManager);
    ...
    if (gUseGpu) {
        ...
        auto encode = [](SkTypeface* tf, void* ctx) -> sk_sp<SkData> {
            return reinterpret_cast<SkRendererStrikeCacheGenerator*>(ctx)->serializeTypeface(tf);
        };
