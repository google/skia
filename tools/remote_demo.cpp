...
template <typename T>
class SkArraySlice : public std::tuple<const T*, uint32_t> {
public:
    // Additional constructors as needed.
    SkArraySlice(const T* data, uint32_t size) : std::tuple<const T*, uint32_t>{data, size} { }
...
public:
    ...
    IOResult endWrite() {
        ...
        std::cout << "endWrite written: " << written << std::endl;
        return kSuccess;
    }
...
private:
    ...
    void processGlyphRun(
        ...
        if (iter == fDescMap.end()) {
            ...
            std::cout << "outgoing typefaceid: " << newAccum.typefaceID << std::endl;
            std::cout << "outgoing tf uniqueid: " << runPaint.getTypeface()->uniqueID()
            << std::endl;
...
static void final_draw(std::string outFilename,
    ...
    if (cache != nullptr) {
        for (int i = 0; i < 1; i++) {
            ...
            transport->endWrite();
            std::cout << "write prepopulate spec: " << transport->size() << std::endl;

            struct {
                SkGlyphCache* cache;
            } strikeData;

            auto perStrike = [&strikeData, cache](TextBlobFilterCanvas::StrikeSpec* spec,
                                           SkDescriptor* desc) {
                auto tf = cache->lookupTypeface(spec->typefaceID);
                std::cout << "incoming typefaceid: " << spec->typefaceID << std::endl;
                std::cout << "incoming unique tf id: " << tf->uniqueID() << std::endl;
                // TODO: implement effects handling.
                SkScalerContextEffects effects;
                strikeData.cache = SkGlyphCache::DetachCache(tf, effects, desc);
                std::cout << std::hex << "cache " << (intptr_t)strikeData.cache << " desc: "
                          << desc->getChecksum() << " typeface id :" << spec->typefaceID <<
                                                                                         std::endl;
            };

            auto perGlyph = [&strikeData](SkGlyph* glyph, SkArraySlice<uint8_t> image) {
                SkGlyph* allocatedGlyph = strikeData.cache->getRawGlyphByID(glyph->getPackedID());
                *allocatedGlyph = *glyph;
                allocatedGlyph->allocImage(strikeData.cache->getAlloc());
                memcpy(allocatedGlyph->fImage, image.data(), image.size());
            };

            auto finishStrike = [&strikeData]() {
                SkGlyphCache::AttachCache(strikeData.cache);
            };
            ...
            std::cout << "read prepopulate data: " << transport->size() << std::endl;
