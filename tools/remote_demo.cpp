...
public:
    ...
    void generateFontMetrics(const SkTypefaceProxy& tf,
        ...
        std::cout << "op - getFontMetrics" << std::endl;
...
static void prepopulate_cache(
    ...
    auto perStrike = [&strikeData, cache](TextBlobFilterCanvas::StrikeSpec* spec,
        ...
        uint32_t size;
        SkScalerContextRec* rec2 =
            (SkScalerContextRec*)desc->findEntry(kRec_SkDescriptorTag, &size);
        std::cout << std::hex << "prepop cache " << (intptr_t)strikeData.cache
                  << " desc: " << desc->getChecksum()
                  << " desc tf: " << rec2->fFontID
                  << " typeface id :" << tf->uniqueID() << std::endl;
        rec2->dumpFields();
    };
