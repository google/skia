...
public:
    ...
    void setAllowPurging(bool allow) { allowPurging = allow; }
...
static void final_draw(std::string outFilename, SkData* picData, SkStrikeClient* client,
    ...
    for (int i = 0; i < 100; i++) {
        if (gPurgeFontCaches) {
            discardableManager->setAllowPurging(true);
            SkGraphics::PurgeFontCache();
            discardableManager->setAllowPurging(false);
        }
        ...
        if (client != nullptr) {
            ...
            if (fontData) client->readStrikeData(fontData->data(), fontData->size());
        }
...
static void gpu(int readFd, int writeFd) {
    ...
    if (gUseGpu) {
        ...
        ClientDiscardableManager discardableManager;
        sk_sp<SkStrikeClient> strikeClient = SkStrikeClient::Create(&discardableManager);

        final_draw("test.png", picData.get(), strikeClient.get(), &discardableManager, readFd,
                   writeFd);
    }
    ...
    const std::string& skpName, int readFd, int writeFd)
{
    ...
    auto server = SkStrikeServer::Create(&discardableManager);
    ...
    if (gUseGpu) {
        ...
        while (true) {
            ...
            push_font_data(*pic.get(), server.get(), writeFd);
        }
    } else {
        stream = skpData;
