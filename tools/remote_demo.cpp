...
public:
    ...
    void generateFontMetrics(const SkTypefaceProxy& tf,
        ...
        SkRemoteScalerOp* op = this->createOp(0, tf, rec);
    ...
    void generateMetrics(const SkTypefaceProxy& tf,
        ...
        SkRemoteScalerOp* op = this->createOp(1, tf, rec);
    ...
    void generateImage(const SkTypefaceProxy& tf,
        ...
        SkRemoteScalerOp* op = this->createOp(2, tf, rec);
        memcpy(&op->glyph, &glyph, sizeof(glyph));
        write(fWriteFd, fBuffer, sizeof(*op));
        read(fReadFd, fBuffer, sizeof(fBuffer));
        ...
        memcpy(glyph.fImage, fBuffer + sizeof(SkRemoteScalerOp), glyph.rowBytes() * glyph.fHeight);
        op->~SkRemoteScalerOp();
    }
    ...
    void generatePath(const SkTypefaceProxy& tf,
        ...
        SkRemoteScalerOp* op = this->createOp(3, tf, rec);
        op->glyphId = glyph;
        write(fWriteFd, fBuffer, sizeof(*op));
        read(fReadFd, fBuffer, sizeof(fBuffer));
        path->readFromMemory(fBuffer + sizeof(SkRemoteScalerOp), op->pathSize);
        op->~SkRemoteScalerOp();
    }

private:
    SkRemoteScalerOp* createOp(uint32_t opID, const SkTypefaceProxy& tf,
                 const SkScalerContextRec& rec) {
        SkRemoteScalerOp* op = new (fBuffer) SkRemoteScalerOp();
        op->op = opID;
...
static std::unique_ptr<SkScalerContext> scaler_context_from_op(SkRemoteScalerOp* op) {
...
static int renderer(const std::string& skpName, int readFd, int writeFd) {
    ...
    SkRemoteScalerOp* op = (SkRemoteScalerOp*)glyphBuffer.get();
    while (true) {
        ...
        std::cerr << "op: " << op << " op->op: " << op->op << std::endl;
            ...
            switch (op->op) {
                ...
                case 2: {
                    ...
                    op->glyph.fImage = &glyphBuffer[sizeof(SkRemoteScalerOp)];
                ...
                case 3: {
                    ...
                    op->pathSize = path.writeToMemory(&glyphBuffer[sizeof(SkRemoteScalerOp)]);
