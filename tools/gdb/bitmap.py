    ...
    if str(val.type.strip_typedefs()) == 'SkBitmap':
      ...
      if color_type == ColorType.bgra_8888.value:
        if alpha_type == AlphaType.unpremul.value:
          image = Image.frombytes("RGBA", size, memory_data.tobytes(), "raw", "BGRA", row_bytes, 1)
        elif alpha_type == AlphaType.premul.value:
          image = Image.frombytes("RGBa", size, memory_data.tobytes(), "raw", "BGRa", row_bytes, 1)

      if image:
        # Apparently this fails if passed a premultiplied alpha image, since it cannot convert to RBG.
        image.show()
      else:
        print ("Need to add support for %s %s." % (str(ColorType(int(color_type))), str(AlphaType(int(alpha_type)))))
