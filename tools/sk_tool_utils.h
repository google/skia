...
namespace sk_tool_utils {
    ...
    // Constructs a star by walking a regular 'numPts'-sided polygon with even/odd fill:
    //
    //   moveTo(pts[0]);
    //   lineTo(pts[step % numPts]);
    //   lineTo(pts[step*2 % numPts]);
    //   ...
    //
    // x,y will both be in the range 0..1.
    // numPts=5, step=2 will produce a classic five-point star.
    //
    // numPts and step must be co-prime.
    SkPath make_star(int numPts = 5, int step = 2);
