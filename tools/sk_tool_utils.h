...
namespace sk_tool_utils {
    ...
    inline bool EncodeImageToFile(const char* path, const SkPixmap& src, SkEncodedImageFormat f, int q) {
        SkFILEWStream file(path);
        return file.isValid() && SkEncodeImage(&file, src, f, q);
    }
    inline bool EncodeImageToFile(const char* path, const SkBitmap& src, SkEncodedImageFormat f, int q) {
        SkFILEWStream file(path);
        return file.isValid() && SkEncodeImage(&file, src, f, q);
    }
    inline sk_sp<SkData> EncodeImageToData(const SkPixmap& src, SkEncodedImageFormat f, int q) {
        SkDynamicMemoryWStream buf;
        return SkEncodeImage(&buf, src , f, q) ? buf.detachAsData() : nullptr;
    }
    inline sk_sp<SkData> EncodeImageToData(const SkBitmap& src, SkEncodedImageFormat f, int q) {
        SkDynamicMemoryWStream buf;
        return SkEncodeImage(&buf, src, f, q) ? buf.detachAsData() : nullptr;
    }
    /**
