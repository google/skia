...
static DEFINE_string(tf   ,   "srgb", "The transfer function for any raster backend.");
    ...
    static DEFINE_string(api, "gles", "API to use when --backend gpu.");
#else
    static DEFINE_string(api, "gl", "API to use when --backend gpu.");
#endif
static DEFINE_int   (samples ,         0, "Samples per pixel when --backend gpu.");
static DEFINE_bool  (nvpr    ,     false, "Use NV_path_rendering when --backend gpu?");
static DEFINE_bool  (stencils,      true, "If false, avoid stencil buffers when --backend gpu.");
static DEFINE_bool  (dit     ,     false, "Use device-independent text when --backend gpu.");
static DEFINE_string(surf    , "default", "Backing store for surface when --backend gpu.");
...
using Backend = sk_sp<SkImage>(*)(std::function<void(SkCanvas*)>,
                                  SkImageInfo,
                                  GrContextFactory*);
...
static sk_sp<SkImage> draw_with_gpu(std::function<void(SkCanvas*)> draw,
    ...
    const FlagOption<GrContextFactory::ContextType> kAPIs[] = {
        ...
        { "nullgl"         , GrContextFactory::kNullGL_ContextType },
    ...
    if (!parse_flag(FLAGS_api , "api" , kAPIs        , &api) ||
        !parse_flag(FLAGS_surf, "surf", kSurfaceTypes, &surfaceType)) {
        return nullptr;
    }
...
int main(int argc, char** argv) {
    ...
    const FlagOption<Backend> kBackends[] = {
        { "cpu", draw_with_cpu },
        { "gpu", draw_with_gpu },
    };
    SkColorType              ct;
    ...
    for (auto source : sources) {
        ...
        sk_sp<SkImage> image = backend(source.draw,
                                       info.makeWH(source.size.width(),
                                                   source.size.height()),
                                       &factory);
        if (!image) {
