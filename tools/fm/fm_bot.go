...
func callFM(fm string, flags []string, sources []string) {
	...
	if err != nil {
		log.Printf("%v failed (%v):\n%s\n", strings.Join(sources, " "), err, output)
	} else if *verbose {
...
func main() {
	...
	for _, job := range jobs {
		...
		log.Println("Running", strings.Join(job, " "))
		...
		// TODO: interlace work when a process is available?
		// Apparently it's common to use a bounded channel as a semaphore,
		// where wait() is `ch <- 42` (put any arbitrary value on the channel,
		// blocking if no slot's available) and signal() is `<-ch`, plucking
		// off a value and making a slot available.   Cute.  Or maybe I just
		// want a channel of (sources []string, flags []string) and then start
		// *processLimit goroutines that pluck them off as available?
		wg := &sync.WaitGroup{}
		...
		wg.Wait()
		log.Println("Done", strings.Join(job, " "))
	}
}
