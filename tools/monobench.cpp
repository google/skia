    ...
    if (benches.size() > 1) {
        ...
        if (common_prefix) {
            for (auto& bench : benches) {
                bench.name.replace(0, common_prefix, kElipsis);
            }
        }
        ...
        if (common_suffix) {
            for (auto& bench : benches) {
                bench.name.replace(bench.name.size() - common_suffix, common_suffix, kElipsis);
            }
        }

        SkDebugf("%s%s%s\n", prefix.c_str(), kElipsis, suffix.c_str());
    }
