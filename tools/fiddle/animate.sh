...
# Create a 2 second long animation from the Raster output of fiddle at 15 fps.
mkdir -p /tmp/animation
for a in $(seq -f "%05g" 0 30)
do
    ./out/Release/fiddle --duration 2 --frame `bc -l <<< "$a/30"` | ./tools/fiddle/parse-fiddle-output
