...
static bool setup_backend_objects(GrContext* context, int width, int height, int sampleCnt) {
    if (!context) {
        return false;
    }
    ...
    {
        backingTexture = context->resourceProvider()->createTexture(backingDesc, SkBudgeted::kNo);
        if (!backingTexture) {
            return false;
        }
    ...
    {
        sk_sp<GrTexture> tmp = context->resourceProvider()->createTexture(backingDesc,
                                                                          SkBudgeted::kNo);
    ...
    {
        backingTextureRenderTarget = context->resourceProvider()->createTexture(backingDesc,
                                                                                SkBudgeted::kNo);
...
int main(int argc, char** argv) {
    ...
    if (options.gpu) {
        ...
        if (!grContext) {
            ...
            if (!setup_backend_objects(grContext.get(), options.size.width(), options.size.height(),
                                       0)) {
