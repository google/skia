...
int get_error(uint32_t value, uint32_t value_max, uint32_t value_min) {
    int error = 0;
    for (int j : {0, 8, 16, 24}) {
...
float GMK_Check(GMK_ImageData image, const char* name, const char* backend) {
    ...
    std::vector<uint32_t> max_image_pixels, min_image_pixels;
    GMK_ImageData max_image = ReadPngRgba8888FromFile(max_path.c_str(), &max_image_pixels);
    if (!max_image.pix) {
        return 9001.0f;
    }
    std::string min_path = path_join(path.c_str(), PATH_MIN_PNG);
    GMK_ImageData min_image = ReadPngRgba8888FromFile(min_path.c_str(), &min_image_pixels);
    if (!min_image.pix) {
        return 9002.0f;
    }
    if (max_image.width != min_image.width || max_image.height != max_image.height) {
        return 9003.0f;
    }
    ...
    assert(max_image_pixels.size() == N);
    assert(min_image_pixels.size() == N);
    int badness = 0;
    for (size_t i = 0; i < N; ++i) {
        int error = get_error(image.pix[i], max_image.pix[i], min_image.pix[i]);
        if (error > 0) {
    ...
    if (badness > 0) {
        ...
        SkAssertResult(WritePngRgba8888ToFile(image, error_path.c_str()));
        ...
        SkAssertResult(WritePngRgba8888ToFile(
                    GMK_ImageData{errors.data(), image.width, image.height},
                    error_path.c_str()));
    }
