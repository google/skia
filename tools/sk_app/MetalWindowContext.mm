...
void MetalWindowContext::initializeContext() {
    ...
    if (fDisplayParams.fEnableBinaryArchive) {
        if (@available(macOS 11.0, iOS 14.0, *)) {
            ...
            NSError* error;
            fPipelineArchive.reset([fDevice newBinaryArchiveWithDescriptor:desc error:&error]);
            if (!fPipelineArchive) {
    ...
    backendContext.fDevice = sk_ret_cfp<GrMTLHandle>((GrMTLHandle)fDevice.get());
    backendContext.fQueue = sk_ret_cfp<GrMTLHandle>(((GrMTLHandle)fQueue.get());
#if GR_METAL_SDK_VERSION >= 230
    if (@available(macOS 11.0, iOS 14.0, *)) {
