...
bool MetalWindowContext_mac::onInitializeContext() {
    ...
    fMTKView = [[MetalView alloc] initWithFrame:rect];
    if (nil == fMTKView) {
        return false;
    }

    fMTKView.device = fDevice;
    fMTKView.colorPixelFormat = MTLPixelFormatBGRA8Unorm;
    // Not sure how to get just 8-bit stencil -- MTLPixelFormatStencil8?
    fMTKView.depthStencilPixelFormat = MTLPixelFormatDepth24Unorm_Stencil8;
