if which x86_64-w64-mingw32-gcc; then
    ...
    if [ "$(uname)" == "Darwin" ]; then
        platformSpecificFlags="-framework CoreServices -framework CoreGraphics -framework CoreText -framework CoreFoundation -std=c++1y -lstdc++"
    else
        # TODO find out what is needed here
        platformSpecificFlags=
    fi
...
Libs: -L\${libdir} $platformSpecificFlags -lskia
EOF