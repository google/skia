 ...
 * loop initializers. Making this a separate class is to prevent comma from appearing outside of a
 * for loop initializer; the opposite problem (things that are not legal in for loop initializers
 * appearing in loop initializers) is already error checked in ForStatement::Convert, so
 * DSLForLoopInitializer does not check that its contents are in fact a legal for loop initializer.
 */
class DSLForLoopInitializer {
public:
