 ...
 * loop initializers. Making this a separate class from DSLStatement allows us to limit the
 * locations where this construct is permitted to appear. The opposite problem (things that are not
 * legal in for loop initializers appearing in loop initializers) is already error checked in
 * ForStatement::Convert, so DSLForLoopInitializer does not check that its contents are in fact a
 * legal for loop initializer.
 */
class DSLForLoopInitializer {
public:
