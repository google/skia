...
// At the end we can drop in platform-specific implementations that override these defaults.
    ...
    template <int N, typename T, typename Op, typename... Ys>
    static inline auto apply(Op op, Vec<N,T> x, Ys... ys) -> Vec<N, decltype(op(x[0], ys[0]...))> {
        Vec<N, decltype(op(x[0], ys[0]...))> r;
        std::transform(x.vals, x.vals+N, ys.vals..., r.vals, op);
        return r;
    }

    ___ Vec<N,T> operator+(Vec<N,T> x, Vec<N,T> y) { return apply(std::plus      <T>{}, x,y); }
    ___ Vec<N,T> operator-(Vec<N,T> x, Vec<N,T> y) { return apply(std::minus     <T>{}, x,y); }
    ___ Vec<N,T> operator*(Vec<N,T> x, Vec<N,T> y) { return apply(std::multiplies<T>{}, x,y); }
    ___ Vec<N,T> operator/(Vec<N,T> x, Vec<N,T> y) { return apply(std::divides   <T>{}, x,y); }

    ___ Vec<N,T> operator^(Vec<N,T> x, Vec<N,T> y) { return apply(std::bit_xor<T>{}, x,y); }
    ___ Vec<N,T> operator&(Vec<N,T> x, Vec<N,T> y) { return apply(std::bit_and<T>{}, x,y); }
    ___ Vec<N,T> operator|(Vec<N,T> x, Vec<N,T> y) { return apply(std::bit_or <T>{}, x,y); }

    ___ Vec<N,T> operator!(Vec<N,T> x) { return apply(std::logical_not<T>{}, x); }
    ___ Vec<N,T> operator-(Vec<N,T> x) { return apply(std::negate     <T>{}, x); }
    ___ Vec<N,T> operator~(Vec<N,T> x) { return apply(std::bit_not    <T>{}, x); }

    ___ Vec<N,T> operator<<(Vec<N,T> x, int bits) { return apply([bits](T a) { return a << bits; }, x); }
    ___ Vec<N,T> operator>>(Vec<N,T> x, int bits) { return apply([bits](T a) { return a >> bits; }, x); }

    ___ Vec<N, mask<T>> operator==(Vec<N,T> x, Vec<N,T> y) { return apply([](T a, T b) -> mask<T> { return a == b ? ~0 : 0; }, x,y); }
    ___ Vec<N, mask<T>> operator!=(Vec<N,T> x, Vec<N,T> y) { return apply([](T a, T b) -> mask<T> { return a != b ? ~0 : 0; }, x,y); }
    ___ Vec<N, mask<T>> operator<=(Vec<N,T> x, Vec<N,T> y) { return apply([](T a, T b) -> mask<T> { return a <= b ? ~0 : 0; }, x,y); }
    ___ Vec<N, mask<T>> operator>=(Vec<N,T> x, Vec<N,T> y) { return apply([](T a, T b) -> mask<T> { return a >= b ? ~0 : 0; }, x,y); }
    ___ Vec<N, mask<T>> operator< (Vec<N,T> x, Vec<N,T> y) { return apply([](T a, T b) -> mask<T> { return a <  b ? ~0 : 0; }, x,y); }
