...
struct Vec {
    ...
    // Methods belong here in Vec only if:
    ...
    const T* begin() const { return vals  ; }
          T* begin()       { return vals  ; }
    const T*   end() const { return vals+N; }
          T*   end()       { return vals+N; }
...
// Vector/scalar operations just splat the scalar to a vector...
...
// ... and same deal for scalar/vector operations.
...
static inline ALWAYS_INLINE skvx::Vec<N,D> cast(skvx::Vec<N,S> src) {
    ...
    return skvx::bit_pun(__builtin_convertvector(skvx::vext(src), skvx::VExt<N,D>));
#else
