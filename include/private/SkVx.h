...
SIN Vec<N,float> fma(const Vec<N,float>& x, const Vec<N,float>& y, const Vec<N,float>& z) {
    // Clang's codegen is terrible if we map(fmaf, ...) directly.  Spooky.
    auto fn = [](float x, float y, float z) { return fmaf(x,y,z); };
    return map(fn, x,y,z);
}

// skvx::fast_mul_add() returns a fused or unfused mul-add, whichever is faster.
SIN Vec<N,float> fast_mul_add(const Vec<N,float>& x, const Vec<N,float>& y, const Vec<N,float>& z) {
