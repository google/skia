...
static inline Vec<N,uint8_t> approx_scale(const Vec<N,uint8_t>& x, const Vec<N,uint8_t>& y) {
    ...
    // by emscripten:
        ...
        static inline Vec<4,int32_t> mad(const Vec<4,int32_t>& f,
                                       const Vec<4,int32_t>& m,
                                       const Vec<4,int32_t>& a) {
            return to_vec<4,int32_t>(wasm_i32x4_add(
        ...
        static inline Vec<4,uint32_t> mad(const Vec<4,uint32_t>& f,
                                       const Vec<4,uint32_t>& m,
                                       const Vec<4,uint32_t>& a) {
            return to_vec<4,uint32_t>(wasm_i32x4_add(
                wasm_i32x4_mul(to_vext(f), to_vext(m)),
                to_vext(a)
            ));
        }
