...
// This gives SkVx a consistent ABI, letting them be passed between files compiled
...
struct SkVx {
    ...
    SkVx(std::initializer_list<T> xs) {
        for (int i = 0; i < N; i++) {
...
// At the end we can drop in platform-specific implementations that override these defaults.
    ...
    static inline auto binop(SkVx<N,T> x, SkVx<N,T> y, Op op) -> SkVx<N, decltype(op(x[0], y[0]))> {
    ...
    ___ SkVx<N, mask<T>> operator==(SkVx<N,T> x, SkVx<N,T> y) { return binop(x,y, [](T a, T b) { return a == b ? mask<T>(~0) : mask<T>(0); }); }
    ___ SkVx<N, mask<T>> operator!=(SkVx<N,T> x, SkVx<N,T> y) { return binop(x,y, [](T a, T b) { return a != b ? mask<T>(~0) : mask<T>(0); }); }
    ___ SkVx<N, mask<T>> operator<=(SkVx<N,T> x, SkVx<N,T> y) { return binop(x,y, [](T a, T b) { return a <= b ? mask<T>(~0) : mask<T>(0); }); }
    ___ SkVx<N, mask<T>> operator>=(SkVx<N,T> x, SkVx<N,T> y) { return binop(x,y, [](T a, T b) { return a >= b ? mask<T>(~0) : mask<T>(0); }); }
    ___ SkVx<N, mask<T>> operator< (SkVx<N,T> x, SkVx<N,T> y) { return binop(x,y, [](T a, T b) { return a <  b ? mask<T>(~0) : mask<T>(0); }); }
