...
// Apply op() to each lane of one or more input vectors, returning a new vector of the results.
template <typename Op, int N, typename T, typename... Ys>
static inline auto map(Op op, Vec<N,T> x, Ys... ys) -> Vec<N, decltype(op(x[0], ys[0]...))> {
    Vec<N, decltype(op(x[0], ys[0]...))> results;
    std::transform(x.vals, x.vals+N, ys.vals..., results.vals, op);
    return results;
}
...
// At the end we can drop in platform-specific implementations that override these defaults.
    ...
    ___ Vec<N,T> operator+(Vec<N,T> x, Vec<N,T> y) { return map(std::plus      <T>{}, x,y); }
    ___ Vec<N,T> operator-(Vec<N,T> x, Vec<N,T> y) { return map(std::minus     <T>{}, x,y); }
    ___ Vec<N,T> operator*(Vec<N,T> x, Vec<N,T> y) { return map(std::multiplies<T>{}, x,y); }
    ___ Vec<N,T> operator/(Vec<N,T> x, Vec<N,T> y) { return map(std::divides   <T>{}, x,y); }

    ___ Vec<N,T> operator^(Vec<N,T> x, Vec<N,T> y) { return map(std::bit_xor<T>{}, x,y); }
    ___ Vec<N,T> operator&(Vec<N,T> x, Vec<N,T> y) { return map(std::bit_and<T>{}, x,y); }
    ___ Vec<N,T> operator|(Vec<N,T> x, Vec<N,T> y) { return map(std::bit_or <T>{}, x,y); }

    ___ Vec<N,T> operator!(Vec<N,T> x) { return map(std::logical_not<T>{}, x); }
    ___ Vec<N,T> operator-(Vec<N,T> x) { return map(std::negate     <T>{}, x); }
    ___ Vec<N,T> operator~(Vec<N,T> x) { return map(std::bit_not    <T>{}, x); }

    ___ Vec<N,T> operator<<(Vec<N,T> x, int bits) { return map([bits](T a) { return a << bits; }, x); }
    ___ Vec<N,T> operator>>(Vec<N,T> x, int bits) { return map([bits](T a) { return a >> bits; }, x); }

    ___ Vec<N, mask<T>> operator==(Vec<N,T> x, Vec<N,T> y) { return map([](T a, T b) -> mask<T> { return a == b ? ~0 : 0; }, x,y); }
    ___ Vec<N, mask<T>> operator!=(Vec<N,T> x, Vec<N,T> y) { return map([](T a, T b) -> mask<T> { return a != b ? ~0 : 0; }, x,y); }
    ___ Vec<N, mask<T>> operator<=(Vec<N,T> x, Vec<N,T> y) { return map([](T a, T b) -> mask<T> { return a <= b ? ~0 : 0; }, x,y); }
    ___ Vec<N, mask<T>> operator>=(Vec<N,T> x, Vec<N,T> y) { return map([](T a, T b) -> mask<T> { return a >= b ? ~0 : 0; }, x,y); }
    ___ Vec<N, mask<T>> operator< (Vec<N,T> x, Vec<N,T> y) { return map([](T a, T b) -> mask<T> { return a <  b ? ~0 : 0; }, x,y); }
...
___ Vec<N,T> min(Vec<N,T> x, Vec<N,T> y) { return map([](T a, T b) { return std::min(a,b); }, x,y); }
___ Vec<N,T> max(Vec<N,T> x, Vec<N,T> y) { return map([](T a, T b) { return std::max(a,b); }, x,y); }
