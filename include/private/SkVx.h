...
SIN Vec<N,float> fma(const Vec<N,float>& x, const Vec<N,float>& y, const Vec<N,float>& z) {
    // For reasons I don't understand, the codegen is much worse if we pass fmaf directly.  Spooky.
    auto fn = [](float x, float y, float z) { return fmaf(x,y,z); };
    return map(fn, x,y,z);
}

// skvx::mul_add() returns a fused or unfused mul-add, whichever is faster.
#if defined(__clang__)
    #pragma STDC FP_CONTRACT ON
    // GCC does not obey any pragma to control FP contraction, but it's on by default
    // when compiling C++.  (C is oddly more nuanced, on with -std=gnu?? but off with -std=c??.)
    // GCC contracts more aggressivly than Clang does, even across multiple expressions.
    //
    // MSVC's docs claim FP contraction is enabled by default with `#pragma
    // fp_contract off` to disable, but I've never seen it actually do anything.
#endif
SIN Vec<N,float> mul_add(const Vec<N,float>& x, const Vec<N,float>& y, const Vec<N,float>& z) {
    return x*y+z;
}
#if defined(__clang__)
