 ...
 *  - SkSTArray does not actually have inline storage, because std::vector<> doesn't support this.
 *  - MEM_MOVE is not honored. std::vector always copies and moves objects using their constructors.
 *    This can expose edge cases where SkTArray wraps a type that doesn't support std::move (!)
 */
