 ...
 *  *less* safe than a SkTArray, but may err on the side of being more safe. For instance, the
 *  MEM_MOVE optimization is ignored.)
 */
...
public:
    ...
    T* move_back_n(int n, T t[]) {
        SkASSERT(n >= 0);
        fVector.insert(fVector.end(), std::move_iterator(t), std::move_iterator(t + n));
        return &*(fVector.end() - n);
    }
