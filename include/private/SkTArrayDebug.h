...
public:
    ...
    void removeShuffle(int n) {
        if (n != this->count() - 1) {
            fVector[size_t(n)] = std::move(fVector.back());
        }
        fVector.pop_back();
    }
    ...
    T& operator[] (int i)             { return fVector[size_t(i)]; }
    const T& operator[] (int i) const { return fVector[size_t(i)]; }
    T& at(int i)                      { return fVector.at(size_t(i)); }
    const T& at(int i) const          { return fVector.at(size_t(i)); }
    ...
    T& fromBack(int i)             { return fVector.rbegin()[size_t(i)]; }
    const T& fromBack(int i) const { return fVector.rbegin()[size_t(i)]; }
...
public:
    ...
    SkSTArray(const SkTArray<T, MEM_MOVE>& that) : SkTArray<T, MEM_MOVE>(that) {}
    SkSTArray(SkTArray<T, MEM_MOVE>&& that) : SkTArray<T, MEM_MOVE>(std::move(that)) {}
    SkSTArray& operator=(const SkTArray<T, MEM_MOVE>& that) {
        this->fVector = static_cast<const SkSTArray<N, T, MEM_MOVE>&>(that).fVector;
        return *this;
    }
    SkSTArray& operator=(SkTArray<T, MEM_MOVE>&& that) {
        this->fVector = std::move(static_cast<const SkSTArray<N, T, MEM_MOVE>&>(that).fVector);
        return *this;
    }
};
