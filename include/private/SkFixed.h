...
static inline SkFixed SkFloatPinToFixed(float x) {
    ...
    if (x > kMaxFloat) return SK_FixedMax;
    static const SkFixed kMinFixed = 0x80000000;
    static const float kMinFloat = SkFixedToFloat(kMinFixed);
    if (x < kMinFloat) return kMinFixed;
    x *= SK_Fixed1;
...
#define SkFixed3232ToFloat(x)  (x * (1.0f / SK_Fixed3232_1))
