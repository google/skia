...
enum GrPixelConfig {
    ...
    kRGB_ETC_GrPixelConfig,  //*** make internal?
...
static inline size_t GrBytesPerPixel(GrPixelConfig config) {
    switch (config) {
        ...
        case kRG_float_GrPixelConfig:
            return 8;
        case kUnknown_GrPixelConfig:
        case kRGB_ETC_GrPixelConfig: //*** this seems strange
            return 0;
    }
    SK_ABORT("Invalid pixel config");
    return 0;
...
static inline SkColorType GrColorTypeToSkColorType(GrColorType ct) {
    switch (ct) {
        ...
        case GrColorType::kRGB_ETC:      return kUnknown_SkColorType;
    }
...
static inline uint32_t GrColorTypeComponentFlags(GrColorType ct) {
    switch (ct) {
        ...
        case GrColorType::kRGB_ETC:      return kRGB_SkColorTypeComponentFlags;
    }
    SK_ABORT("Invalid GrColorType");
    return kUnknown_SkColorType;
}
...
static inline int GrColorTypeBytesPerPixel(GrColorType ct) {
    switch (ct) {
        case GrColorType::kUnknown:      return 0;
        case GrColorType::kRGB_ETC:      return 0;
...
static inline GrPixelConfig GrColorTypeToPixelConfig(GrColorType config,
    ...
    switch (config) {
        ...
        case GrColorType::kRGB_ETC:
            return (GrSRGBEncoded::kYes == srgbEncoded) ? kUnknown_GrPixelConfig
                                                        : kRGB_ETC_GrPixelConfig;
    }
    SK_ABORT("Invalid GrColorType");
    return kUnknown_GrPixelConfig;
}
