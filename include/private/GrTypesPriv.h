...
static constexpr GrSRGBEncoded GrPixelConfigIsSRGBEncoded(GrPixelConfig config) {
    switch (config) {
...
/**
 * What is the encoding of channel data in a GrColorType. Currently this is the same for all
 * color channels present in a GrColorType.
 */
enum GrColorTypeEncoding {
    kUnorm,
    // kSRGBUnorm,
    // kSnorm,
    kFloat,
...
/**
 * How many bits are there for each color component in a GrColorType. Curently all the non-zero
 * channels share a single GrColorTypeEncoding. This could be expanded to store masks for each
 * channel.
 */
...
public:
    ...
    constexpr GrColorTypeEncoding encoding() const { return fEnconding; }
...
static constexpr GrColorTypeDesc GrGetColorTypeDesc(GrColorType ct) {
    switch (ct) {
        ...
        case GrColorType::kABGR_4444:
            return GrColorTypeDesc::MakeRGBA(4, 4, GrColorTypeEncoding::kUnorm);
...
static constexpr GrColorType GrPixelConfigToColorTypeAndEncoding(GrPixelConfig config,
