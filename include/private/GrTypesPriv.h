...
/**
 * Describes a GrColorType by how many bits are are used for each color component and how they are
...
private:
    ...
    constexpr GrColorTypeDesc(int r, int g, int b, int a, int gray, GrColorTypeEncoding encoding)
            : fRBits(r), fGBits(g), fBBits(b), fABits(a), fGrayBits(gray), fEnconding(encoding) {
