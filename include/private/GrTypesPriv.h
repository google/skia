...
/**
 * We have code paths for some shape that can draw non-AA using boolean fragment shader coverage.
 * However, this may not be desirable to use this if it will trigger a dst copy. If the caller
 * of GrChooseAAType passes kYes and GrAA::kNo then GrChooseAAType will consider whether it is
 * effecient to use coverage to implement a non-AA draw and if so return GrAAType::kCoverage.
 */
enum class GrAllowNonAABinaryCoverage : bool { kNo = false, kYes = true };

/** GrPaint is only required if GrAllowNonAABinaryCoverage is kYes. */
