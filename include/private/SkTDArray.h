...
public:
    SkMemoryBlock();
    SkMemoryBlock(size_t bytes, const void* data);
    void grow(size_t n, size_t insertion, size_t tailEnd);
    void shrinkToFit(size_t n);

    void* ptrOf(size_t i) const { return (uint8_t*)fData + i; }
...
public:
    ...
    T* prepend() {
        auto existing = fBlock.size();
        auto answer = fBlock.insert(0, 1);
        fBlock.copyTo(1, 0, existing);
        return &fBlock[0];
    }
