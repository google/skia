...
/*bool*/ SkTFitsIn(S src) {
    ...
    (std::is_signed<D>::value && std::is_unsigned<S>::value && sizeof(D) <= sizeof(S)) ?
        src <= (S)std::numeric_limits<sk_strip_enum<D>::type>::max() :

#if 0
    // Correct (simple) version that trips up MSVC's /RTCc run-time checking:
    (S)(D)src == src;
#else
    (std::is_signed<S>::value) ?
        (intmax_t)src >= (intmax_t)std::numeric_limits<sk_strip_enum<D>::type>::min() &&
        (intmax_t)src <= (intmax_t)std::numeric_limits<sk_strip_enum<D>::type>::max() :

    // std::is_unsigned<S> ?
        (uintmax_t)src <= (uintmax_t)std::numeric_limits<sk_strip_enum<D>::type>::max();
#endif
}
