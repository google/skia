 ...
 * This makes "(S)(D)s == s" a possibly useful test.
 *
 * In the case of testing a smaller signed value fits in a larger unsigned things go wonky. Consider
 * (int8_t)(uint16_t)-1 == -1 => (int8_t)0xFFFF == -1 => [implementation defined] == -1
 *
 * In the case of testing a larger unsigned value fits in a smaller signed things go wonky. Consider
 * (uint32_t)(int8_t)0xFFFF == 0xFFFF => (uint32_t)-1 == 0xFFFF => 0xFFFF == 0xFFFF => true.
 *
 * Consider the cases:
 ...
 * sX -> sx: (sX)(sx)sX == sX, first cast results in implementation defined value, second cast is defined, test works
 ...
 *                                (uint32_t)(int8_t)0xFFFF == 0xFFFF
 *                             => (uint32_t)-1             == 0xFFFF
 *                             => 0xFFFF                   == 0xFFFF
 *                             => true
 *
 * So for the eight cases 3 are trivially true, 3 more are valid casts, and 2 are special.
...
template <typename S> struct SkTInRange_True {
    using can_be_false= std::false_type;
    using source_type = S;
    static bool fits(S) {
        return true;
    }
...
template <typename D, typename S> struct SkTInRange_Cast {
    using can_be_false = std::true_type;
    using source_type = S;
