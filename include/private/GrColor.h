...
/**
 * Returns true if all channels are in [0,1]. Premul colors with RGB > A are out-of-gamut, but this
 * is checking if a color fits in a byte vertex attribute (vs. half-float), so we don't care.
 */
static inline bool SkPMColor4fIsNormalized(const SkPMColor4f& color) {
