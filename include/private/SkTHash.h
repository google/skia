...
public:
    ...
    // If there is key/value entry in the table with this key, return an Iter to the value.
    // If not, return an iterator to the end of the table.
    ...
    // Key-value pairs are returned by the iterator.
    struct Pair {
        K key;
        V val;
