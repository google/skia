...
/**
 *  Return the size of the block of memory that is at least `size`, rounded up to a value that
 *  underlying allocator can use without adding additional padding. This should be used before
 *  malloc'ing in lieu of requesting the usable size after an allocation.
 */
SK_API extern size_t sk_malloc_good_size(size_t size);
