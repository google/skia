...
public:
    ...
    // Blend filter SkSL requires an entry point that looks like:
    //     vec4 main(vec4 srcColor, vec4 dstColor) { ... }
    static Result MakeForBlendFilter(SkString sksl, const Options&);
    ...
    static Result MakeForBlendFilter(SkString sksl) {
        return MakeForBlendFilter(std::move(sksl), Options{});
    }
    ...
    static Result MakeForBlendFilter(std::unique_ptr<SkSL::Program> program);
    ...
    sk_sp<SkBlendFilter> makeBlendFilter(sk_sp<SkData> uniforms) const;
