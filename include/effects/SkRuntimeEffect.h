...
public:
    ...
    using EffectResult = std::pair<sk_sp<SkRuntimeEffect>, SkString>;
...
private:
    ...
    using SpecializeResult = std::pair<std::unique_ptr<SkSL::Program>, SkString>;
    ...
    using ByteCodeResult = std::pair<std::unique_ptr<SkSL::ByteCode>, SkString>;
...
struct SkRuntimeShaderBuilder {
    SkRuntimeShaderBuilder(sk_sp<SkRuntimeEffect> effect)
    ...
    struct BuilderInput {
        ...
        template <typename T> BuilderInput& operator=(const T& val) {
            if (!fVar) {
