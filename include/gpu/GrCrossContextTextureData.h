...
struct GrCrossContextTextureData {
    // TODO: Set fBackend to something illegal? There isn't a value in the enum.
    // memset whole struct to 0xFF?
    GrCrossContextTextureData() {}

    GrCrossContextTextureData(const GrGLTextureInfo& info)
        : fBackend(kOpenGL_GrBackend)
        , fGLBackendInfo(info) {}

#if SK_VULKAN
    GrCrossContextTextureData(const GrVkImageInfo& info)
