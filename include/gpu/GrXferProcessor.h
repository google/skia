...
public:
    using FPAnalysis = GrProcessorSet::FPAnalysis;
     ...
     */
    OptFlags getOptimizations(const FPAnalysis&,
...
private:
    ...
    virtual OptFlags onGetOptimizations(const FPAnalysis&,
                                        bool doesStencilWrite,
                                        GrColor* overrideColor,
...
public:
    ...
    GrXferProcessor* createXferProcessor(const FPAnalysis&,
     ...
     */
    static bool WillReadDst(const GrXPFactory*, const FPAnalysis&);
     ...
     */
    static bool WillNeedDstTexture(const GrXPFactory*, const GrCaps&, const FPAnalysis&);
...
private:
    ...
    virtual bool willReadsDst(const FPAnalysis& pipelineAnalysis) const = 0;
    ...
    bool willReadDstInShader(const GrCaps& caps, const FPAnalysis& analysis) const;
     ...
     */
    virtual bool onWillReadDstInShader(const GrCaps&, const FPAnalysis&) const = 0;
};
#if defined(__GNUC__) || defined(__clang)
