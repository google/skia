...
public:
     ...
     */
    static bool IsPreCoverageBlendedColorConstant(const GrXPFactory*, const GrProcOptInfo& colorInput, GrColor* color);

    /**
     * This will return true if the xfer processor needs the dst color in the shader and the way
     * that it will be made available to it is by sampling a texture.
     */
...
private:
    ...
    virtual bool isPreCoverageBlendedColorConstant(const GrProcOptInfo& colorInput, GrColor* color) const = 0;

    /** Subclass-specific implementation of IsPreCoverageBlendedColorConstant(). */
    virtual bool willReadsDst(const GrProcOptInfo& colorInput, const GrProcOptInfo& coverageInput) const = 0;
