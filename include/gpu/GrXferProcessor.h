...
public:
    ...
    GrXferProcessor* createXferProcessor(const GrPipelineAnalysis&,
                                         ...
                                         const GrCaps& caps) const;
    /**
     * Used to know whether the post-blended (but pre-coverage) color computed by the
     * GrXferProcessor is a constant and also whether the destination will be read either for color
     * blending or for coverage application.
     */
    struct OutputAnalysis {
        enum class ReadsDst { kNo, kYes };
        OutputAnalysis() : fFlags(kReadsDst_Flag) {}

        static OutputAnalysis MakeColor(GrColor color, ReadsDst readsDst) {
            OutputAnalysis result;
            result.fColor = color;
            result.fFlags = kKnownColor_Flag | ((ReadsDst::kYes == readsDst) ? kReadsDst_Flag : 0);
            return result;
        }

        static OutputAnalysis MakeUnknown(ReadsDst readsDst) {
            OutputAnalysis result;
            result.fFlags = (ReadsDst::kYes == readsDst) ? kReadsDst_Flag : 0;
            return result;
        }

        /**
         * Will the XP require the destination color either in the shader or the fixed function
         * blend.
         */
        bool readsDst() const { return SkToBool(fFlags & kReadsDst_Flag); }

        bool isColorKnown(GrColor* blendedColor = nullptr) const {
            if (!(fFlags & kKnownColor_Flag)) {
                return false;
            }
            if (blendedColor) {
                *blendedColor = fColor;
    ...
    private:
        enum Flags { kKnownColor_Flag = 0x1, kReadsDst_Flag = 0x2 };
        uint32_t fFlags;
        GrColor fColor;
    };

    static OutputAnalysis GetOutputAnalysis(const GrXPFactory*, const GrProcOptInfo& colorPOI,
...
private:
    /**
     * Returns information about the blened output color, produced by XPs from this factory,
     * and whether the final post-coverage color depends on the original destination color.
     */
    virtual OutputAnalysis outputAnalysis(const GrProcOptInfo& colorPOI,
