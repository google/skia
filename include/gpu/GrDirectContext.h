...
// GrGpu that can only compile programs/build pipelines.
class GrCompilenator : public SkRefCnt {
public:
    virtual bool compile(const GrProgramDesc&, const GrProgramInfo&) = 0;
};

class GrUtilityContextPriv;
...
public:
    ...
    // Provides access to functions that aren't part of the public API.
    GrUtilityContextPriv priv();
    const GrUtilityContextPriv priv() const;  // NOLINT(readability-const-return-type)

protected:
    friend class GrDirectContext; // for ctor

    GrUtilityContext(sk_sp<GrCompilenator>, sk_sp<GrContextThreadSafeProxy>);

    bool compile(const GrProgramDesc&, const GrProgramInfo&);

private:
    sk_sp<GrCompilenator>  fCompilenator;
    sk_sp<GrDirectContext> fDirectContext; // TODO: a temporary hack

    friend class GrUtilityContextPriv;
...
public:
    ...
    bool compile(const GrProgramDesc& desc, const GrProgramInfo& programInfo) {
        return fContext->compile(desc, programInfo);
    }

private:
    explicit GrUtilityContextPriv(GrUtilityContext* context) : fContext(context) {}
    GrUtilityContextPriv(const GrUtilityContextPriv&) = delete;
    GrUtilityContextPriv& operator=(const GrUtilityContextPriv&) = delete;

    // No taking addresses of this type.
    const GrUtilityContextPriv* operator&() const;
    GrUtilityContextPriv* operator&();

    GrUtilityContext* fContext;

    friend class GrUtilityContext; // to construct/copy this type.
};

inline GrUtilityContextPriv GrUtilityContext::priv() { return GrUtilityContextPriv(this); }

inline const GrUtilityContextPriv GrUtilityContext::priv () const {  // NOLINT(readability-const-return-type)
    return GrUtilityContextPriv(const_cast<GrUtilityContext*>(this));
}
