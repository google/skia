...
private:
    class ExplicitContextID {
    public:
        ExplicitContextID();

        bool operator==(const ExplicitContextID& that) const { return fID == that.fID; }
        bool operator!=(const ExplicitContextID& that) const { return !(*this == that); }

        static ExplicitContextID Invalid();
        void makeInvalid() { fID = SK_InvalidUniqueID; }
        bool isValid() const { return fID != SK_InvalidUniqueID; }

    private:
        constexpr ExplicitContextID(uint32_t id) : fID(id) {}
