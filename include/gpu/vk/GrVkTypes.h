 ...
 * will then execute that command buffer within a render pass it is using for its own draws. The drawable is also
 * given the attachment of the color index, a compatible VkRenderPass, and the VkFormat of the color
 * attachment so that it can make VkPipeline objects for the draws. The SkDrawable must not alter
 * the state of the VkRenderpass or sub pass.
 *
 ...
 * to render offscreen textures which will be sampled in draws added to the passed in VkCommandBuffer.
 * If this is done the SkDrawable is in charge of adding the required memory barriers to the queue
 * for the sampled images since the Skia backend will not do this.
 */
