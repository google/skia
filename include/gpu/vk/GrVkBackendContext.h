...
// It is not guarenteed VkPhysicalDeviceProperties2 will be in the clients header so we forward
// decleare it here to be same.
...
struct SK_API GrVkBackendContext {
    ...
    // VkPhysicalDeviceFeatures2 struct is needed so we know if the client enabled any extensions
    // specific features. If fDeviceFeatures2 is not null then we ignore fDeviceFeatures.
