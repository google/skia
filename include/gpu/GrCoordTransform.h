...
public:
    ...
    GrCoordTransform(bool bFoo, const GrTexture* texture, GrSamplerParams::FilterMode filter) {
        ...
        this->reset(bFoo, texture, filter);
    }
    ...
    GrCoordTransform(const SkMatrix& m, bool bFoo, const GrTexture* texture,
        ...
        SkDEBUGCODE(fInProcessor = false);
        SkASSERT(texture);
        this->reset10(m, bFoo, texture, filter);
    }
    ...
    void reset10(const SkMatrix&, bool bFoo, const GrTexture*, GrSamplerParams::FilterMode filter);
...
private:
    void reset(bool bFoo, const GrTexture* texture, GrSamplerParams::FilterMode filter) {
        SkASSERT(bFoo);
        SkASSERT(!fInProcessor);
        SkASSERT(texture);
        this->reset10(SkMatrix::I(), bFoo, texture, filter);
    }
