...
public:
    ...
    public:
        static UniqueResourceID InvalidID() {
            return UniqueResourceID(uint32_t(SK_InvalidUniqueID));
        }

        UniqueResourceID() {}

        explicit UniqueResourceID(uint32_t id) : fID(id) {}

        uint32_t asUInt() const { return fID; }

        bool operator==(const UniqueResourceID& other) const {
    ...
    class Blarg : public UniqueResourceID {
    public:
        Blarg() : UniqueResourceID(uint32_t(SK_InvalidUniqueID)) {}

        void operator=(const UniqueResourceID& id) { fID = id.asUInt(); }
    };
     ...
     * 0.
     */
    UniqueResourceID uniqueID() const { return fUniqueID; }
...
private:
    ...
    const UniqueResourceID      fUniqueID;
