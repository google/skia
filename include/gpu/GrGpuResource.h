 ...
 * GrGpuResource.
 *
 * Gpu resources have a single ref. It is used by code that is issuing draw calls
 * that read and write the resource via GrOpsTasks and by any object that must own a
...
public:
    ...
    void unref() const {
        ...
        if (--fRefCnt == 0) {
            static_cast<const DERIVED*>(this)->notifyRefCountIsZero();
        }
    }
...
public:
     ...
     *         false otherwise.
     */
    bool wasDestroyed1() const { return nullptr == fGpu; }
...
private:
    ...
    void notifyRefCountIsZero() const;
    ...
    friend class GrIORef<GrGpuResource>; // to access notifyRefCntIsZero.
};
