...
public:
    ...
    bool isConfigRenderableXX(GrPixelConfig config, bool withMSAA) const {
        return this->maxSampleCount(config) > (withMSAA ? 1 : 0);
    }
    ...
    virtual int maxSampleCount(GrPixelConfig config) const = 0;
    ...
    bool validateTextureDesc(const GrSurfaceDesc&, GrMipMapped) const;
