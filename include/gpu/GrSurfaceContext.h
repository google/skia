...
public:
    ...
    bool copy(GrSurfaceProxy* src) {
        return this->onCopy(src, SkIRect::MakeWH(src->width(), src->height()), SkIPoint::Make(0, 0));
    }
    ...
    static sk_sp<GrSurfaceProxy> Copy(GrContext* context, GrSurfaceProxy* src,
                                      SkBudgeted budgeted) {
        return Copy(context, src,
                    SkIRect::MakeWH(src->width(), src->height()), SkIPoint::Make(0, 0),
                    budgeted);
    }
