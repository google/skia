...
/**
 * Array of SkImages and SkSurfaces which we will prepare for external use when passed into a flush
 * call on GrContext. All the SkImages and SkSurfaces must be GPU backed.
 *
 * Currently this only has an effect if the backend API is Vulkan. In this case, all the underlying
 * VkImages associated with the SkImages and SkSurfaces will be transitioned into the queue in which
 * they were originally wrapped or created with. This allows a client to wrap a VkImage from a queue
 * which is different from the graphics queue and then have Skia transition it back to that queue
 * without needing to delete the SkImage or SkSurface.
 */
