...
public:
    ...
    */
    ...
    /**
        ...
        textureFulfillProc and textureReleaseProc may be called multiple times, but are always
        balanced with each other. If textureFulfillProc is called then textureReleaseProc will
        always be called even if textureFulfillProc failed. textureDoneProc is always called even
        if image creation fails or if the image is never fulfilled (e.g. it is never drawn or all
     ...
     */
    sk_sp<SkImage> SK_WARN_UNUSED_RESULT makePromiseTexture(
     ...
     */
    sk_sp<SkImage> SK_WARN_UNUSED_RESULT makeYUVAPromiseTexture(
