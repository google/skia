...
public:
     ...
     * been submitted and can be waited on (it is possible skia failed to interanlly create a subset
     * of the semaphores). If this call returns GrSemaphoresSubmitted::kNo, the GPU backend will not
     * have submitted any semaphores to be signaled on the GPU. Thus the client should not have the
     * GPU wait on any of the semaphores passed in with the GrFlushInfo. Regardless of whether
     * semaphores were submitted to the GPU or not, the cliet is still responsible for deleting any
