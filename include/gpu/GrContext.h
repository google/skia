...
protected:
    virtual bool init17(const GrContextOptions&) = 0; // must be called after the ctor!

private:
    friend class GrImageContext; // for ctor

    GrContextWeakest(GrBackendApi backend, uint32_t uniqueID, sk_sp<const GrCaps> caps);
...
private:
    ...
    GrImageContext(GrBackendApi backend, uint32_t uniqueID, sk_sp<const GrCaps> caps)
            : INHERITED(backend, uniqueID, std::move(caps)) {}
...
private:
    ...
    GrRecordingContext(GrBackendApi backend, uint32_t uniqueID, sk_sp<const GrCaps> caps)
            : INHERITED(backend, uniqueID, std::move(caps)) {}
...
protected:
    GrContext(GrBackendApi, int32_t id = SK_InvalidGenID, sk_sp<const GrCaps> caps = nullptr);
