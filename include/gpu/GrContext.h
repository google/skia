...
public:
    ...
    /**
     * Did the underlying 3D API report an out-of-memory error. If this returns true it is reset
     * and will return false until another out-of-memory error is reported by the 3D API. If the
     * context is abandoned then this will report false.
     *
     * Currently this is only implemented for OpenGL. Note that client calls to glGetError() may
     * swallow GL_OUT_OF_MEMORY errors and therefore hide the error from Skia. Also, it is advised
     * not to use GrContextOptions::fSkipGLErrorChecks enabled if relying on this query. That option
     * may prevent GrContext from ever checking the GL context for OOM.
     */
    bool oomed();
