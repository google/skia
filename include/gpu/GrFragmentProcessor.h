...
public:
    ...
    */
    ...
    /**
     * This is called on fragment processors in the color data flow. A GrDrawOp may premultiply its
     * antialiasing coverage into its GrGeometryProcessor's color output under the following
     * scenario:
     *   * all the color fragment processors report true to this query,
     *   * there are no coverage fragment processors, and
     *   * the blend mode arithmetic allows it.
     * To be compatible a fragment processor's output must be a modulation of its input color or
     * alpha with a computed premul color or alpha that is in 0..1 range. The computed color or
     * alpha that is modulated against the input cannot depend on the input's alpha. It cannot be
     * depend on its input's color channels unless it unpremultiplies the input color channels by
     * the input alpha.
     */
