 ...
 * encouraged to provide a bounds on the vertex positions they can compute one more cheaply than
...
public:
    ...
    static sk_sp<SkVertices> MakeIndexed(SkCanvas::VertexMode mode,
                                         ...
                                         int indexCnt) {
        if (!positions || !indices || vertexCnt <= 0 || indexCnt <= 3) {
            return nullptr;
        }
        return sk_sp<SkVertices>(new SkVertices(mode, std::move(positions), std::move(colors),
