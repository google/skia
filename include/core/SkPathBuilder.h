...
public:
    ...
    SkPathBuilder& addOval(const SkRect&, SkPathDirection dir = SkPathDirection::kCW);
    SkPathBuilder& addRRect(const SkRRect&, SkPathDirection dir = SkPathDirection::kCW);

    void incReserve(int extraPtCount, int extraVerbCount = -1);

    static SkPath Make(SkPathFillType,
                       const SkPoint[],  int pointCount,
                       const uint8_t[],  int verbCount,
                       const SkScalar[], int conicWeightCount);
    static SkPath Make(SkPathFillType ft,
                       const SkPoint pts[],  int pointCount,
                       const uint8_t vbs[],  int verbCount) {
        return Make(ft, pts, pointCount, vbs, verbCount, nullptr, 0);
    }
