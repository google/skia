...
constexpr SkYUVAPixmapInfo::SupportedDataTypes SkYUVAPixmapInfo::SupportedDataTypes::All() {
    size_t bits = 0;
    for (int c = 1; c <= 4; ++c) {
        for (size_t dt = 0; dt <= kDataTypeCnt; ++dt) {
            if (DefaultColorTypeForDataType(static_cast<DataType>(dt), c) != kUnknown_SkColorType) {
                bits |= 1 << (dt + kDataTypeCnt*(c-1));
            }
        }
    }
...
constexpr bool SkYUVAPixmapInfo::SupportedDataTypes::supported(PlanarConfig config,
    ...
    for (int i = 0; i < n; ++i) {
        int c = SkYUVAInfo::NumChannelsInPlane(config, i);
        SkASSERT(c >= 1 && c <= 4);
        if (!fDataTypeSupport[static_cast<size_t>(type) + (c - 1)*kDataTypeCnt]) {
            return false;
        }
    }
    return true;
