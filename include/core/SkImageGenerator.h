...
public:
     ...
     *
     *  TexGenPolicy determines whether or not a new texture must be created (and its budget status)
     *  or whether this may (but is not required to) return a pre-existing texture that is retained
     *  by the generator (kCheapest)
     */
    enum class TexGenPolicy {
        kCheapest,
        kForceNew_Unbudgeted,
        kForceNew_Budgeted,
    };
    GrSurfaceProxyView generateTexture(GrRecordingContext*, const SkImageInfo& info,
                                       const SkIPoint& origin, GrMipMapped,
                                       TexGenPolicy = TexGenPolicy::kCheapest);
...
protected:
    ...
    virtual GrSurfaceProxyView onGenerateTexture(GrRecordingContext*, const SkImageInfo&,
                                                 const SkIPoint&, GrMipMapped, TexGenPolicy);
#endif
