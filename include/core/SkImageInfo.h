...
enum class SkYUVAFormat {
    // 4:2:0 formats, 24 bpp
    kP016, // 16-bit Y plane + 2x2 down sampled interleaved U/V plane (2 textures)
    // 4:2:0 formats, "15 bpp" (but really 24 bpp)
    kP010, // same as kP016 except "10 bpp". Note that it is the same memory layout
    ...
    kY416,  // 16-bit AVYU values all interleaved (1 texture)

    // 4:4:4 formats, 32 bpp
    kAYUV,  // 8-bit YUVA values all interleaved (1 texture)
    kY410,  // AVYU w/ 10bpp for YUV and 2 for A all interleaved (1 texture)

    // 4:2:0 formats, 12 bpp
    kNV12, // 8-bit Y plane + 2x2 down sampled interleaved U/V planes (2 textures)
    kNV21, // same as kNV12 but w/ U/V reversed in the interleaved texture (2 textures)

    kI420, // 8-bit Y plane + separate 2x2 down sampled U and V planes (3 textures)
    kYV12, // 8-bit Y plane + separate 2x2 down sampled V and U planes (3 textures)
