...
public:
    ...
    textIndex = click(x, y, target_lines==ignore_y = ...);

    Range[] selectionToRanges(index0, index1)

    line, run, glyph_index, (x,y) (width/height) = indexToPosition(index);

    // utilities
    new_index = moveIndex(index, {
        left_char, right_char, left_word, right_word, ... line_start, line_end
    });

    Range = find_word_boundary(index?)
};
