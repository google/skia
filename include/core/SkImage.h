...
public:
     ...
     *  Will return NULL if the specified backend texture is unsupported.
     *
     *  This is not supported if the GrContext is a deferred display list canvas since if the
     *  SkImage never gets used and flushed to a real GrContext, we have no way to be able to delete
     *  the underlying backend texture.
     *
     *  DEPRECATED: This factory is deprecated and clients should use the factory below which takes
     *  an SkColorType.
     */
