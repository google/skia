...
public:
    ...
    /** Creates SkImage from SkPixmap and copy of pixels. Since pixels are copied, SkPixmap
        ...
        @param pixmap  SkYUVColorSpace, pixel address, and row bytes
        @return        copy of SkPixmap pixels, or nullptr
    */
    static sk_sp<SkImage> MakeRasterCopy(const SkPixmap& pixmap);

    /** Creates SkImage from SkYUVColorSpace, sharing pixels.

        SkImage is returned if SkYUVColorSpace is valid. Valid SkYUVColorSpace parameters include:
        dimensions are greater than zero;
        each dimension fits in 29 bits;
        color type and alpha type are valid, and color type is not kUnknown_SkColorType;
        ...
        @param info      contains width, height, alpha type, color type, SkColorSpace
    ...
    */
    ...
    /** Creates SkImage from bitmap, sharing or copying bitmap pixels. If the bitmap
        ...
        @param bitmap  SkYUVColorSpace, row bytes, and pixels
    ...
    */
    ...
    /** Returns alpha type, one of: kUnknown_SkAlphaType, kOpaque_SkAlphaType,
        ...
        alpha type returned was a parameter to an SkImage constructor,
        or was parsed from encoded data.

        @return  alpha type in SkImage
    */
    ...
    */
    ...
    /** Returns true if pixels ignore their alpha value and are treated as fully opaque.
        ...
        @return  true if alpha type is kOpaque_SkAlphaType
    */
    ...
    */
    ...
    /** Copies SkImage pixel address, row bytes, and SkYUVColorSpace to pixmap, if address
    ...
    */
    ...
    /** Copies SkRect of pixels from SkImage to dstPixels. Copy starts at offset (srcX, srcY),
        ...
        dstInfo specifies width, height, color type, alpha type, and SkColorSpace of
        ...
        Pixels are copied only if pixel conversion is possible. If SkImage color type is
        kGray_8_SkColorType, or kAlpha_8_SkColorType; dstInfo.colorType() must match.
        If SkImage color type is kGray_8_SkColorType, dstInfo.colorSpace() must match.
        If SkImage alpha type is kOpaque_SkAlphaType, dstInfo.alphaType() must
    ...
    */
    ...
    /** Copies a SkRect of pixels from SkImage to dst. Copy starts at (srcX, srcY), and
        ...
        dst specifies width, height, color type, alpha type, SkColorSpace, pixel storage,
        ...
        If SkImage color type is kGray_8_SkColorType, dst.colorSpace() must match.
        If SkImage alpha type is kOpaque_SkAlphaType, dst.alphaType() must
    ...
    */
    struct DeferredTextureImageUsageParams {DeferredTextureImageUsageParams(const SkMatrix matrix, const SkFilterQuality quality,
    ...
    */
    ...
    /** \enum SkImage::LegacyBitmapMode
        Used by asLegacyBitmap() to specify if SkBitmap can be written to.
    */
    enum LegacyBitmapMode {
        kRO_LegacyBitmapMode, //!< Returned bitmap is read-only and immutable.

        /** Returned bitmap is read-write and mutable. May be deprecated in the future. */
