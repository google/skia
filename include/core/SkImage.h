...
public:
    ...
    */
    ...
    /** Flushes an pending uses of texture-backed images in the GPU backend. If the image is not
        texture-backed (including promise texture images), if the image was not used in any
        non-culled draws recorded on the passed GrDirectContext, or if the the GrContext does not
