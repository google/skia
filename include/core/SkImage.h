...
public:
    ...
    */
     ...
     *
     *  When the image's ref count reaches zero, the original GrContext will be used to destroy the
     *  texture. It is assumed that it will be safe to access. That is, the GL context is current
     *  on the unreffing thread if using the GL backend and no other thread is accessing the
     *  GrContext.
     *
