...
public:
    ...
    */
    ...
    /** Transitions the backend-end object of a gpu backed SkImage to a state that is suitable for
        ...
        VkQueueFamilyIndex that the VkImage was in when the SkImage was created.

        Similar to drawing calls, the actual transitioning of the SkImage will not happen until the
        next time the image is flushed and is ordered around other operations on the SkImage.
        Thus if any operations are called using the image between calling prepareForExternalIO and
        calling GrContext::flush, the image will no longer be guaranteed to be transitioned to the
        correct external state. Therefore, prepareForExternalIO should only be called immediately
