...
/** \class SkBitmap
    ...
    SkBitmap is not thread safe. Each thread must have its own copy if SkBitmap fields,
...
public:
    ...
    */
    ...
    /** Once set, pixels can not change. Any other bitmap sharing the same SkPixelRef
        are also marked as immutable. Once SkPixelRef is marked immutable, the setting
        cannot be cleared.
    ...
    */
    ...
    /** Replaces pixel values with color built from r, g, and b with color alpha set
    ...
    */
    ...
    /** Copies a SkRect of pixels from src. Copy starts at (dstX, dstY), and does not exceed
        ...
        false if width() or height() is zero or negative. Returns false if abs(dstX) >= this->width(), or if abs(dstY) >= this->height() .
    ...
    */
    ...
    /** Copies SkBitmap pixel address, row bytes, and ImageInfo to pixmap, if address
