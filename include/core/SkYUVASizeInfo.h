    ...
    bool operator==(const SkYUVASizeInfo& that) const {
        for (int i = 0; i < 4; ++i) {
            ...
            if (kUnknown_SkColorType == fColorTypes[i]) {
                SkASSERT(!fSizes[i].fWidth && !fSizes[i].fHeight && !fWidthBytes1[i]);
                SkASSERT(!that.fSizes[i].fWidth && !that.fSizes[i].fHeight && !that.fWidthBytes1[i]);
                continue;
            }

            if (fSizes[i] != that.fSizes[i] || fWidthBytes1[i] != that.fWidthBytes1[i]) {
                return false;
            }
        }
    ...
    size_t computeTotalBytes() const {
        ...
        for (int i = 0; i < 4; ++i) {
            SkASSERT(kUnknown_SkColorType != fColorTypes[i] ||
                        (!fSizes[i].fWidth && !fSizes[i].fHeight && !fWidthBytes1[i]));

            totalBytes += fWidthBytes1[i] * fSizes[i].height();
        }

        return totalBytes;
    }

    void computePlanes(void* base, void* planes[4]) const;
