...
int32_t                                                            fBottom;
    ...
    */
    ...
    /** Returns constructed SkIRect set to
        (x, y, x + w, y + h). Does not validate input;
    ...
    */
    ...
    /** Sets SkIRect to
        (x, y, x + width, y + height). Does not validate input;
...
public:
    ...
    */
    ...
    /** For each of count SkPoint in pts, grows SkRect to include (pt.fX, pt.fY), modifying
        it so that:
        fLeft <= pt.fX <= fRight && fTop <= pt.fY <= fBottom. SkPoint may be followed with other data in each array element. stride is number
