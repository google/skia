...
public:
    ...
    */
    ...
    /** Grows SkRect to include (pt.fX, pt.fY), modifying it so that: fLeft <= pt.fX <= fRight && fTop <= pt.fY <= fBottom .
    ...
    */
    ...
    /** For each of count SkPoint in pts, grows SkRect to include (pt.fX, pt.fY), modifying
        it so that: fLeft <= pt.fX <= fRight && fTop <= pt.fY <= fBottom . SkPoint may be followed with other data in each array element. stride is number
 ...
 SkScalarCeilToInt(fRight), SkScalarCeilToInt(fBottom)) .
    ...
    */
    ...
    /** Sets SkRect by rounding up fLeft and fTop; and
        discarding the fractional portion of fRight and fbottom, using (SkScalarCeilToInt(fLeft), SkScalarCeilToInt(fTop),
