...
public:
    ...
    /**
     * Given image dimensions, a plaer configuration, subsampling, and origin, determine the
     * expected size of  each plane. Returns the number of expected planes. planeDimensions[0]
     * through planeDimensons[<ret>] are written. The input image dimensions are as displayed
     * (after the planes have been transformed to the intended display orientation). The plane
     * dimensions are output as the planes stored in memory (may be rotated from image dimensions).
     */
