...
public:
    ...
    */
    ...
    /** Copy into 'position' (allocated by the caller) the design variation coordinates.
     *
     *  @param position the buffer into which to write the design variation coordinates.
     *  @param count the number of entries available in position.
     *
     *  @return The number of axes, or -1 if there is an error.
     *  If 'position != nullptr' and 'count >= numAxes' then 'position' will be filled with the
     *  variation coordinates describing the position of this typeface in design variation space.
     *  It is possible the number of axes can be retrieved but actual position cannot.
     */
    int getVariationDesignPosition(SkFontArguments::VariationCoordinate position[], size_t count) const;
...
protected:
    ...
    */
    ...
    virtual int onGetVariationDesignPosition(SkFontArguments::VariationCoordinate position[], size_t count) const = 0;
