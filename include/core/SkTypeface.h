...
public:
    ...
    */
    ...
    /** Copy into 'position.coordinates' (allocated by the caller) the design variation coordinates.
     *
     *  @param position.coordinates the buffer into which to write the design variation coordinates.
     *  @param position.coordinateCount the number of entries available in position.coordinates.
     *
     *  @return The number of axes, or -1 if there is an error.
     *  If 'position.coordinates != nullptr' and 'position.coordinateCount >= numAxes' then
     *  'position' will be filled with the variation coordinates describing the position of this
     *  typeface in design variation space.
     *  It is possible the number of axes can be retrieved but actual position cannot.
     */
    int getVariationDesignPosition(SkFontArguments::VariationPosition position) const;
...
protected:
    ...
    */
    ...
    virtual int onGetVariationDesignPosition(SkFontArguments::VariationPosition) const = 0;
