...
public:
    ...
    constexpr SkSurfaceProps() : fFlags(0), fPixelGeometry(kUnknown_SkPixelGeometry) {}
    ...
    // Returns a reference to a default-constructed SkSurfaceProps; useful to prevent having to
    // create a temporary variable when calling APIs that take a pointer.
    static constexpr const SkSurfaceProps& Default() { return kDefaultProps; }
