...
/** \class SkCanvas
    ...
    Each SkCanvas draw call transforms the geometry of the object by the concatenation of all SkMatrix
    values in the stack.
    The transformed geometry is clipped by the intersection of all of clip values in the stack.
    The SkCanvas draw calls take SkPaint parameter for drawing state.
    Create SkPaint to supply the drawing state, such as color,
    SkTypeface, SkPaint text size, SkPaint stroke width, SkShader and so on.
    ...
    Document-based SkCanvas and other SkCanvas subclasses reference SkBaseDevice describing the destination.
*/
...
public:
    ...
    /** Allocates raster canvas that will draw directly into pixels.
        ...
        Valid parameters include: info dimensions are zero or positive;
        ...
        @param pixels    pointer to destination pixels buffer. Buffer size should be info height
                         times rowBytes times bytes required for SkColorType
        @param rowBytes  The interval from one SkSurface row to the next; equal to or greater than
    ...
    */
    ...
    /** Creates SkCanvas with raster surface with inline image specification that draws into pixels.
        ...
        info contains SkColorType and SkAlphaType supported by raster surface;
        pixels is not nullptr;
        rowBytes is zero or large enough to contain width pixels of SkColorType.
        ...
        @param rowBytes  interval from one SkSurface row to the next; equal to or greater than
                         width times four
        @return          SkCanvas if all parameters are valid; otherwise, nullptr
    */
    ...
    */
    ...
    /** Draws SkCanvas::_anonymous, if any.
    ...
    */
    ...
    /** Returns SkImageInfo for SkCanvas. If SkCanvas is not associated with raster surface or
        gpu surface, returns SkImageInfo::SkImageInfo() is returned SkColorType is set to kUnknown_SkColorType.
    ...
    */
    ...
    /** Triggers the immediate execution of all pending draw operations.
        If SkCanvas is associated with gpu surface, resolve all pending GPU operations.
    */
    void flush();
    ...
    */
    ...
    /** Creates SkSurface matching info and props, and associates it with SkCanvas.
        If SkCanvas is already associated with SkSurface, it cannot create a new SkSurface.

        @param info   Initialize SkSurface with width, height, SkColorType, SkAlphaType, and color space
        @param props  Use to match if provided, or use the SkSurfaceProps in SkCanvas otherwise
        @return       SkSurface matching info and props, or nullptr if no match is available
    */
    sk_sp<SkSurface> makeSurface(const SkImageInfo& info, const SkSurfaceProps* props = nullptr);

    /** Returns GPU_Context of the gpu surface associated with SkCanvas.
    ...
    */
    ...
    /** Returns the pixel base address, SkImageInfo, rowBytes, and origin if the pixels
        ...
        @param info      If not nullptr, copies writable pixels' SkImageInfo
        @param rowBytes  If not nullptr, copies writable pixels' row bytes
        @param origin    If not nullptr, copies SkCanvas top layer origin, its top left corner
    ...
    */
    ...
    /** Returns true if SkCanvas has direct access to its pixels.
        ...
        Pixels are readable when SkBaseDevice is raster. Pixels are not readable when SkCanvas is returned from
        gpu surface, returned by SkDocument::beginPage, returned by SkPictureRecorder::beginRecording,
        or SkCanvas is the base of a utility class like SkDumpCanvas.

        pixmap pixel address is only valid while SkCanvas is in scope and unchanged. Any SkCanvas or SkSurface call may
        invalidate the pixmap values.

        @param pixmap  storage for SkCanvas pixel state if SkCanvas pixels are readable; otherwise, ignored
        @return        true if SkCanvas has direct access to pixels
    */
    bool peekPixels(SkPixmap* pixmap);

    /** Copies rectangle of pixels from SkCanvas into dstPixels, converting their SkColorType and SkAlphaType.
        Pixels are readable when SkBaseDevice is raster. Pixels are not readable when SkCanvas is returned from
        gpu surface, returned by SkDocument::beginPage, returned by SkPictureRecorder::beginRecording,
        or SkCanvas is the base of a utility class like SkDumpCanvas.

        Pixel values are converted only if SkCanvas SkColorType and SkAlphaType does not match dstInfo.
        Only pixels within the rectangle that intersect SkCanvas pixels are copied.
        dstPixels outside the rectangle intersection are unchanged.

        @param dstInfo      Dimensions, SkColorType, and SkAlphaType of dstPixels
    ...
    */
    ...
    /** Copies rectangle of pixels from SkCanvas into SkPixmap, converting their SkColorType and SkAlphaType.
        Pixels are readable when SkBaseDevice is raster. Pixels are not readable when SkCanvas is returned from
        gpu surface, returned by SkDocument::beginPage, returned by SkPictureRecorder::beginRecording,
        or SkCanvas is the base of a utility class like SkDumpCanvas.

        Pixel values are converted only if SkCanvas SkColorType and SkAlphaType does not match bitmap SkImageInfo.
        Only SkPixmap pixels within the rectangle that intersect SkCanvas pixels are copied.
        SkPixmap pixels outside the rectangle intersection are unchanged.

        @param pixmap  Receives pixels copied from SkCanvas
        @param srcX    Offset into readable pixels in x
        @param srcY    Offset into readable pixels in y
        @return        true if pixels were copied
    */
    ...
    /** Copies pixels enclosed by bitmap offset to (x, y) from SkCanvas into bitmap, converting their SkColorType and SkAlphaType.
        Pixels are readable when SkBaseDevice is raster. Pixels are not readable when SkCanvas is returned from
        gpu surface, returned by SkDocument::beginPage, returned by SkPictureRecorder::beginRecording,
        or SkCanvas is the base of a utility class like SkDumpCanvas.
        ...
        @param bitmap  Receives pixels copied from SkCanvas
        @param srcX    Offset into readable pixels in x
        @param srcY    Offset into readable pixels in y
        @return        true if pixels were copied
    */
    ...
    /** Copies to SkCanvas pixels, ignoring the SkMatrix and clip, converting to match
        info SkColorType and info SkAlphaType.

        Pixel values are converted only if SkCanvas SkColorType and SkAlphaType does not match info.
        Only pixels within the source rectangle that intersect SkCanvas pixel bounds are copied.
        SkCanvas pixels outside the rectangle intersection are unchanged.

        @param info      Dimensions, SkColorType, and SkAlphaType of pixels
    ...
    */
    ...
    /** Writes to SkCanvas pixels, ignoring the SkMatrix and clip, converting to match
        bitmap SkColorType and bitmap SkAlphaType.

        Pixel values are converted only if SkCanvas SkColorType and SkAlphaType does not match bitmap.
        Only pixels within the source rectangle that intersect SkCanvas pixel bounds are copied.
        SkCanvas pixels outside the rectangle intersection are unchanged.

        @param bitmap  Provides pixels copied to SkCanvas
        @param x       Offset into SkCanvas writable pixels in x
        @param y       Offset into SkCanvas writable pixels in y
        @return        true if pixels were written to SkCanvas
    */
    ...
    */
    ...
    /** Saves SkMatrix, clip, and SkDrawFilter (Draw_Filter deprecated on most platforms),
        and allocates an offscreen SkBitmap for subsequent drawing.
        Calling restore() discards changes to SkMatrix, clip, and SkDrawFilter,
        ...
        SkRect bounds suggests but does not define the offscreen size. To clip drawing to
    ...
    */
    ...
    /** Saves SkMatrix, clip, and SkDrawFilter (Draw_Filter deprecated on most platforms),
        and allocates an offscreen SkBitmap for subsequent drawing.
        Calling restore() discards changes to SkMatrix, clip, and SkDrawFilter,
        and draws the offscreen SkBitmap.
    ...
    */
    ...
    /** Saves SkMatrix, clip, and SkDrawFilter (Draw_Filter deprecated on most platforms),
        ...
        lcd text is preserved when the offscreen is drawn to the prior layer.
    ...
    */
    enum {
        /** Creates offscreen without transparency. Flag is ignored if layer SkPaint contains
            SkImageFilter or SkColorFilter.
        */
        kIsOpaque_SaveLayerFlag               = 1 << 0,

        /** Creates offscreen for LCD text. Flag is ignored if layer SkPaint contains
            SkImageFilter or SkColorFilter.
        */
    ...
    struct SaveLayerRec {
        ...
        */
        ...
        /** Sets fBounds, fPaint, fBackdrop, and fSaveLayerFlags.
            ...
            @param bounds          Offscreen dimensions; may be nullptr
            @param paint           Applied to offscreen when overlaying prior layer; may be nullptr
            @param backdrop        Copies prior layer to offscreen with SkImageFilter; may be nullptr
            @param saveLayerFlags  SaveLayerRec options to modify offscreen
        ...
        */
        ...
        /** fPaint modifies how the offscreen overlays the prior layer; may be nullptr.
            alpha, SkBlendMode, SkColorFilter, SkDrawLooper, SkImageFilter, and
            SkMaskFilter affect the offscreen draw.
        */
        ...
        */
        ...
        /** restore() clips the layer offscreen by the alpha channel of fClipMask when
            the offscreen is copied to SkBaseDevice. fClipMask may be nullptr.    .
        */
    ...
    */
    ...
    /** Restores state to SkMatrix, clip, and SkDrawFilter
    ...
    */
    ...
    /** Skew SkMatrix by sx on the x-axis and sy on the y-axis. A positive value of sx skews the
        drawing right as y increases; a positive value of sy skews the drawing down as x increases.

        Mathematically, replace SkMatrix with a skew matrix
        pre-multiplied with SkMatrix.
    ...
    */
    ...
    /** Replace SkMatrix with matrix pre-multiplied with SkMatrix.

        This has the effect of transforming the drawn geometry by matrix, before transforming
        the result with SkMatrix.

        @param matrix  Pre-multiply with SkMatrix
    */
    ...
    /** Replace SkMatrix with matrix.
        ...
        @param matrix  Copied into SkMatrix
    */
    ...
    */
    ...
    /** Fill clip with SkPaint paint. SkPaint components
        SkRasterizer, SkMaskFilter, SkShader, SkColorFilter, SkImageFilter, and SkBlendMode
        affect drawing; SkPathEffect is ignored.
    ...
    */
    ...
    /** Draw pts using clip, SkMatrix and SkPaint paint.
        ...
        If mode is kPoints_PointMode, the shape of point drawn depends on paint SkPaint::Cap.
        If paint is set to SkPaint::kRound_Cap, each point draws a circle of diameter SkPaint stroke width.
        If paint is set to SkPaint::kSquare_Cap or SkPaint::kButt_Cap,
        each point draws a square of width and height SkPaint stroke width.
        ...
        SkPaint::Join, and unlike drawPath(), does not create a mask from all points and lines
        before drawing.
    ...
    */
    ...
    /** Draw point at (x, y) using clip, SkMatrix and SkPaint paint.
        ...
        If paint is set to SkPaint::kRound_Cap, draw a circle of diameter SkPaint stroke width.
        If paint is set to SkPaint::kSquare_Cap or SkPaint::kButt_Cap,
    ...
    */
    ...
    /** Draw point p using clip, SkMatrix and SkPaint paint.
        ...
        @param p      Left top edge of circle or square
        @param paint  Stroke, blend, color, and so on, used to draw
    */
    ...
    */
    ...
    /** Draw ovals oval using clip, SkMatrix, and SkPaint.
        In paint: SkPaint::Style determines if ovals is stroked or filled;
        if stroked, SkPaint stroke width describes the line thickness.

        @param oval   SkRect bounds of ovals
        @param paint  SkPaint stroke or fill, blend, color, and so on, used to draw
    */
    ...
    */
    ...
    /** Draw SkRRect outer and inner
        ...
        In paint: SkPaint::Style determines if rrect is stroked or filled;
        if stroked, SkPaint stroke width describes the line thickness.
        If stroked and SkRRect corner has zero length radii, SkPaint::Join can draw
    ...
    */
    ...
    /** Draw circles at (cx, cy) with radius using clip, SkMatrix, and SkPaint paint.
        If radius is zero or less, nothing is drawn.
        In paint: SkPaint::Style determines if circles is stroked or filled;
        if stroked, SkPaint stroke width describes the line thickness.

        @param cx      Circles center on the x-axis
        @param cy      Circles center on the y-axis
        @param radius  Half the diameter of circles
        @param paint   SkPaint stroke or fill, blend, color, and so on, used to draw
    */
    ...
    */
    ...
    /** Draw arcs using clip, SkMatrix, and SkPaint paint.
        Arcs is part of ovals bounded by oval, sweeping from startAngle to startAngle plus
        sweepAngle. startAngle and sweepAngle are in degrees.
        startAngle of zero places start point at the right middle edge of oval.
        A positive sweepAngle places arcs end point clockwise from start point;
        a negative sweepAngle places arcs end point counterclockwise from start point.
        sweepAngle may exceed 360 degrees, a full circle.
        If useCenter is true, draw a wedge that includes lines from oval
        center to arcs end points. If useCenter is false, draw arcs between end points.

        If SkRect oval is empty or sweepAngle is zero, nothing is drawn.

        @param oval        SkRect bounds of ovals containing arcs to draw
        @param startAngle  Angle in degrees where arcs begins
    ...
    */
    ...
    /** Draw SkRRect bounded by SkRect rect, with corner radii (rx, ry) using clip, SkMatrix,
        and SkPaint paint.
        In paint: SkPaint::Style determines if SkRRect is stroked or filled;
        if stroked, SkPaint stroke width describes the line thickness.
        If rx or ry are less than zero, they are treated as if they are zero.
        If rx plus ry exceeds rect width or rect height, radii are scaled down to fit.
        If rx and ry are zero, SkRRect is drawn as SkRect and if stroked is affected by SkPaint::Join.
    ...
    */
    ...
    /** Draw SkPath path using clip, SkMatrix, and SkPaint paint.
        ...
        In paint: SkPaint::Style determines if SkRRect is stroked or filled:
        if filled, SkPath::FillType determines whether path contour describes inside or outside of fill;
        if stroked, SkPaint stroke width describes the line thickness, SkPaint::Cap describes line ends,
        and SkPaint::Join describes how corners are drawn.
    ...
    */
    ...
    /** Draw SkImage image, with its top-left corner at (left, top),
        ...
        If paint is supplied, apply SkColorFilter, alpha, SkImageFilter, SkBlendMode, and SkDrawLooper.
        If image is kAlpha_8_SkColorType, apply SkShader.
        if paint contains SkMaskFilter, generate mask from image bounds. If generated mask extends
        beyond image bounds, replicate image edge colors, just as SkShader made from
        ...
        @param paint  SkPaint containing SkBlendMode, SkColorFilter, SkImageFilter, and so on; or nullptr
    */
    ...
    /** Draw SkImage image, with its top-left corner at (left, top),
        ...
        If SkPaint paint is supplied, apply SkColorFilter, alpha, SkImageFilter, SkBlendMode, and SkDrawLooper.
        If image is kAlpha_8_SkColorType, apply SkShader.
        if paint contains SkMaskFilter, generate mask from image bounds. If generated mask extends
        beyond image bounds, replicate image edge colors, just as SkShader made from
        SkImage::makeShader with SkShader::kClamp_TileMode set replicates the image's edge
    ...
    */
    ...
    /** \enum SkCanvas::SrcRectConstraint
        SrcRectConstraint controls the behavior at the edge of the SkRect src, provided to drawImageRect(),
        trading off speed for precision.

        SkImageFilter in SkPaint may sample multiple pixels in the image.
        SkRect src restricts the bounds of pixels that may be read. SkImageFilter may slow
        down if it cannot read outside the bounds, when sampling near the edge of SkRect src.
    ...
    */
    ...
    /** Draw SkRect src of SkImage image, scaled and translated to fill SkRect dst.
        ...
        SkImage::makeShader with SkShader::kClamp_TileMode set replicates the image's edge
        color when it samples outside of its bounds.
        constraint set to kStrict_SrcRectConstraint limits SkPaint SkFilterQuality to sample within src;
    ...
    */
    ...
    /** Draw SkIRect isrc of SkImage image, scaled and translated to fill SkRect dst.
        ...
        color when it samples outside of its bounds.
        cons set to kStrict_SrcRectConstraint limits SkPaint SkFilterQuality to sample within src;
        set to kFast_SrcRectConstraint allows sampling outside to improve performance.
    ...
    */
    ...
    /** Draw SkImage image stretched differentially to fit into SkRect dst.
        SkIRect center divides the image into nine sections: four sides, four corners, and the center.
    ...
    */
    ...
    /** Draw SkBitmap bitmap, with its top-left corner at (left, top),
        using clip, SkMatrix, and optional SkPaint paint.
        If paint is supplied, apply SkColorFilter, alpha, SkImageFilter, SkBlendMode, and SkDrawLooper.
        If bitmap is kAlpha_8_SkColorType, apply SkShader.
        if paint contains SkMaskFilter, generate mask from bitmap bounds. If generated mask extends
        beyond bitmap bounds, replicate bitmap edge colors, just as SkShader made from
    ...
    */
    ...
    /** Draw SkBitmap bitmap stretched differentially to fit into SkRect dst.
        SkIRect center divides the bitmap into nine sections: four sides, four corners, and the center.
        corners are unscaled or scaled down proportionately if their sides are larger than dst;
        center and four sides are scaled to fit remaining space, if any.
        Additionally transform draw using clip, SkMatrix, and optional SkPaint paint.
        If SkPaint paint is supplied, apply SkColorFilter, alpha, SkImageFilter, SkBlendMode, and SkDrawLooper.
    ...
    */
    ...
    /** Draw text, with origin at (x, y), using clip, SkMatrix, and SkPaint paint.
        text's meaning depends on SkPaint::TextEncoding; by default, text encoding is utf-8.
        x and y meaning depends on SkPaint::Align and SkPaint vertical text; by default text
        draws left to right, positioning the first glyph's left side bearing at x and its
        baseline at y. Text size is affected by SkMatrix and SkPaint text size.

        All elements of paint: SkPathEffect, SkRasterizer, SkMaskFilter, SkShader, SkColorFilter,
        SkImageFilter, and SkDrawLooper; apply to text. By default, draws filled 12 point black
        glyphs.
    ...
    */
    ...
    /** Draw null terminated string, with origin at (x, y), using clip, SkMatrix, and SkPaint paint.
        string's meaning depends on SkPaint::TextEncoding; by default, string encoding is utf-8.
        Other values of SkPaint::TextEncoding are unlikely to produce the desired results, since
        zero bytes may be embedded in the string.
        x and y meaning depends on SkPaint::Align and SkPaint vertical text; by default string
        draws left to right, positioning the first glyph's left side bearing at x and its
        baseline at y. Text size is affected by SkMatrix and SkPaint text size.

        All elements of paint: SkPathEffect, SkRasterizer, SkMaskFilter, SkShader, SkColorFilter,
        SkImageFilter, and SkDrawLooper; apply to string. By default, drawString() draws filled 12 point black
        glyphs.
    ...
    */
    ...
    /** Draw each glyph in text with the origin in pos array, using clip, SkMatrix, and SkPaint paint.
        The number of entries in pos array must match the number of glyphs described by byteLength of text.
        text's meaning depends on SkPaint::TextEncoding; by default, text encoding is utf-8.
        pos elements' meaning depends on SkPaint::Align and SkPaint vertical text; by default each
        glyph's left side bearing is positioned at x and its
        baseline is positioned at y. Text size is affected by SkMatrix and SkPaint text size.
    ...
    */
    ...
    /** Draw each glyph in text with its (x, y) origin composed from xpos array and constY, using clip, SkMatrix, and SkPaint paint.
        ...
        baseline is positioned at constY. Text size is affected by SkMatrix and SkPaint text size.
    ...
    */
    ...
    /** Draw text on SkPath path, using clip, SkMatrix, and SkPaint paint.
        ...
        text's meaning depends on SkPaint::TextEncoding; by default, text encoding is utf-8.
        Origin meaning depends on SkPaint::Align and SkPaint vertical text; by default text
    ...
    */
    ...
    /** Draw text, transforming each glyph by the corresponding SkRSXform,
        ...
        each glyph.
        Optional SkRect cullRect is a conservative bounds of text,
        taking into account SkRSXform and paint. If cullrect is outside of clip, canvas can
        skip drawing.
    ...
    */
    ...
    /** Draw SkTextBlob blob at (x, y), using clip, SkMatrix, and SkPaint paint.
        ...
        SkTypeface, SkPaint text size, SkPaint text scale x, SkPaint text skew x, SkPaint::Align,
        SkPaint::Hinting, anti-alias, SkPaint fake bold, font embedded bitmaps, full hinting spacing,
        lcd text, linear text, subpixel text, and SkPaint vertical text.
    ...
    */
    ...
    /** Draw picture picture, using clip and SkMatrix;
        transforming picture with SkMatrix matrix, if provided;
        and use SkPaint paint alpha, SkColorFilter, SkImageFilter, and SkBlendMode, if provided.
        ...
        @param picture  Recorded drawing commands to play
        @param matrix   Optional SkMatrix to rotate, scale, translate, and so on; or nullptr
        @param paint    Optional SkPaint to apply transparency, filtering, and so on; or nullptr
    */
    ...
    */
    ...
    /** Draw vertices vertices, a triangle mesh, using clip and SkMatrix.
        If vertices texs and vertices colors are defined in vertices, and SkPaint paint contains SkShader,
        ...
        @param paint     Specifies the SkShader, used as vertices texture, if present
    */
    ...
    */
    ...
    /** Draw a cubic coons patch: the interpolation of four cubics with shared corners,
        ...
        The coons patch uses clip and SkMatrix, SkPaint paint's SkShader, SkColorFilter, alpha,
        SkImageFilter, and SkBlendMode. If SkShader is provided it is treated as the coons
        patch texture; SkBlendMode mode combines color colors and SkShader if both are provided.

        @param cubics     SkPoint array cubics specifying the four cubics starting at the top left corner,
        ...
        @param colors     Color array color associating colors with corners in top left, top right, bottom right,
                          bottom left order
        @param texCoords  SkPoint array texCoords mapping SkShader as texture to corners in same order, if paint
                          contains SkShader; or nullptr
        @param mode       SkBlendMode for colors and SkShader if present
    ...
    */
    ...
    /** Draw a set of sprites from atlas, using clip, SkMatrix, and optional SkPaint paint.
        paint uses anti-alias, alpha, SkColorFilter, SkImageFilter, and SkBlendMode to draw, if present.
        For each entry in the array, SkRect tex locates sprite in atlas, and SkRSXform xform transforms it
        ...
        @param colors    Color, one per sprite, blended with sprite using SkBlendMode; or nullptr
        @param count     Number of sprites to draw
        @param mode      SkBlendMode combining colors and sprites
        @param cullRect  SkRect bounds of transformed sprites for efficient clipping; or nullptr
        @param paint     SkPaint SkColorFilter, SkImageFilter, SkBlendMode, and so on; or nullptr
    */
    ...
    */
    ...
    /** Associate SkRect on SkCanvas when an annotation; a key-value pair, where the key is
        ...
        @param key    SkString used for lookup
