...
public:
    ...
    */
    ...
    /** Returns true if SkCanvas has direct access to its pixels.
        ...
        pixmap pixel address is only valid while SkCanvas is in scope and unchanged. Any
    ...
    */
    ...
    /** Copies rectangle of pixels from SkCanvas into dstPixels, converting their
        SkColorType and SkAlphaType. Pixels are readable when SkBaseDevice is
        raster. Pixels are not readable when SkCanvas is returned from GPU surface,
        ...
        Pixel values are converted only if SkCanvas SkColorType and SkAlphaType
        does not match dstInfo. Only pixels within the rectangle that intersect SkCanvas
    ...
    */
    ...
    /** Copies rectangle of pixels from SkCanvas into SkPixmap, converting their
        SkColorType and SkAlphaType. Pixels are readable when SkBaseDevice is raster.
        Pixels are not readable when SkCanvas is returned from GPU surface, returned by
        SkDocument::beginPage, returned by SkPictureRecorder::beginRecording,
        or SkCanvas is the base of a utility class like SkDumpCanvas.

        Pixel values are converted only if SkCanvas SkColorType and SkAlphaType
        does not match bitmap SkImageInfo. Only SkPixmap pixels within the rectangle that
        intersect SkCanvas pixels are copied. SkPixmap pixels outside the rectangle
    ...
    */
    ...
    /** Copies pixels enclosed by bitmap offset to (x, y) from SkCanvas into bitmap,
        converting their SkColorType and SkAlphaType.
        Pixels are readable when SkBaseDevice is raster. Pixels are not readable when SkCanvas
        is returned from GPU surface, returned by SkDocument::beginPage, returned by
        SkPictureRecorder::beginRecording, or SkCanvas is the base of a utility class
        ...
        Pixel values are converted only if SkCanvas SkColorType and SkAlphaType
        does not match bitmap SkImageInfo. Only pixels within the rectangle that intersect
        SkCanvas pixels are copied. Bitamp pixels outside the rectangle intersection are
        unchanged.
    ...
    */
    ...
    /** Copies to SkCanvas pixels, ignoring the SkMatrix and clip, converting to match
        ...
        SkCanvas pixel bounds are copied. SkCanvas pixels outside the rectangle intersection
        are unchanged.
        ...
        */
        ...
        /** Sets fBounds, fPaint, fBackdrop, and fSaveLayerFlags.
            ...
            @param paint           applied to offscreen when overlaying prior layer;
                                   may be nullptr
            @param backdrop        prior layer copied to offscreen with SkImageFilter;
                                   may be nullptr
            @param saveLayerFlags  SaveLayerRec options to modify offscreen
    ...
    */
    ...
    /** Skew SkMatrix by sx on the x-axis and sy on the y-axis. A positive value of sx
        ...
        Mathematically, replace SkMatrix with a skew matrix pre-multiplied with SkMatrix.
    ...
    */
    ...
    /** Replace SkMatrix with matrix pre-multiplied with existing SkMatrix.

        This has the effect of transforming the drawn geometry by matrix, before
        transforming the result with existing SkMatrix.

        @param matrix  matrix to pre-multiply with existing SkMatrix
    */
    ...
    */
    ...
    /** \enum SkCanvas::SrcRectConstraint
        SrcRectConstraint controls the behavior at the edge of the SkRect src, provided to
        drawImageRect(), trading off speed for precision.

        SkImageFilter in SkPaint may sample multiple pixels in the image. SkRect src
        restricts the bounds of pixels that may be read. SkImageFilter may slow down if
        it cannot read outside the bounds, when sampling near the edge of SkRect src.
    ...
    */
    ...
    /** Draw a cubic coons patch: the interpolation of four cubics with shared corners,
        ...
        @param texCoords  SkPoint array of texure coordinates, mapping SkShader to corners;
                          may be nullptr
