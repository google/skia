...
public:
    ...
    /**
        ...
        difference is that the caller need not have created the texture or populated it with the
        ...
        backend API context/device. The texture supplied by the client must remain valid and its
        contents cannot not be mutated until textureDoneProc is called.
    ...
    /**
        ...
        SkImage from YUV[A] data. The source data may be planar (spread across multiple textures).
        In the extreme Y, U, V, and A are all in different planes and thus the image is specified by
        ...
        image. Separate textureFullfillProc, textureReleaseProc, and textureDoneProc calls are made
        for each texture and each texture has its own PromiseImageTextureContext.
     */
