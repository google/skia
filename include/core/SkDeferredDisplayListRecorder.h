...
public:
    ...
    /**
        ...
        backend API context/device, and be populated with the image pixel data. The supplied texture
        must remain valid and its contents cannot not be mutated until textureDoneProc is called.
    ...
    /**
        ...
        SkImage from YUV[A] data. The source data may be planar (i.e. spread across multiple
        textures). In the extreme Y, U, V, and A are all in different planes and thus the image is
        specified by four textures. 'yuvAIndices' specifies the mapping from texture color channels
        to Y, U, V, and possibly A components. It therefore indicates how many unique textures
        compose the full image. Separate textureFullfillProc, textureReleaseProc, and
        textureDoneProc calls are made for each texture and each texture has its own
        PromiseImageTextureContext. 'yuvFormats', 'yuvaSizes', and 'textureContexts' have one entry
        for each of the up to four textures, as indicated by 'yuvaIndices'.
     */
