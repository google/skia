...
public:
    ...
    /** Legacy compatability version that takes an additional proc that is never called. */
    sk_sp<SkImage> makePromiseTexture(const GrBackendFormat& backendFormat,
                                      int width,
                                      int height,
                                      GrMipmapped mipMapped,
    ...
    /**
        ...
        image. Separate textureFulfillProc and textureReleaseProc calls are made (to the same proc)
