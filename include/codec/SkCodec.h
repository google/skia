...
public:
    ...
    /**
     * If decoding to YUVA is supported, this returns true and updates spec to be a specification
     * of the planar layout and YUVA->RGBA transformation. If YUVA decoding is not supported then
     * returns false.
     */
...
protected:
    ...
    virtual bool onGetYUVASpec(SkYUVASpec*,
                               SkColorType[SkYUVASpec::kMaxPlanes],
                               size_t /*rowbytes*/[SkYUVASpec::kMaxPlanes]) const {
        return false;
    }

    virtual Result onGetYUVAPlanes(const SkPixmap[SkYUVASpec::kMaxPlanes]) {
