...
public:
    ...
    void onDrawContent(SkCanvas* canvas) override {
        ...
        static constexpr float kAxisSpace = 10.f;
        SkPath localSpace;
        for (float y = localContentRect.fTop + kAxisSpace; y <= localContentRect.fBottom;
                y += kAxisSpace) {
            localSpace.moveTo(localContentRect.fLeft, y);
            localSpace.lineTo(localContentRect.fRight, y);
        }
        for (float x = localContentRect.fLeft + kAxisSpace; x <= localContentRect.fRight;
                x += kAxisSpace) {
            localSpace.moveTo(x, localContentRect.fTop);
            localSpace.lineTo(x, localContentRect.fBottom);
        }
