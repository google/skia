...
protected:
    ...
    void onDrawContent(SkCanvas* canvas) override {
        ...
        for (auto& pos : positions) {
            auto result = paragraph->getGlyphPositionAtCoordinate(pos.fX, pos.fY);
            SkDebugf("(%f,%f) -> %d %c\n", pos.fX, pos.fY, result.position,
                    result.affinity == Affinity::kDownstream ? 'd' : 'u');
            SkRect rect = SkRect::MakeXYWH(pos.fX, pos.fY, 2, 2);
