...
protected:
    ...
    void onDrawContent(SkCanvas* canvas) override {
        ...
        auto draw = [&](size_t start, size_t end, SkColor color) {
            auto results = paragraph->getRectsForRange(start, end, RectHeightStyle::kMax, RectWidthStyle::kTight);
            SkPaint red;
            red.setColor(color);
            ...
            for (auto& box : results) {
                canvas->drawRect(box.rect, red);
            }
        };
        //draw(0, 8, SK_ColorBLUE);
        draw(7, 8, SK_ColorRED);
    }
