...
CurveData* StrokeData::curveData() {
    ...
    bool trim_end(CurveData* curve1, CurveData* curve2
            SkDEBUGPARAMS(bool skipAssert) SkDEBUGPARAMS(const char* testName)) {
        // construct segments from each path
        SkSTArenaAlloc<4096> allocator;  // FIXME: constant-ize, tune
        SkOpContourHead* contourList = static_cast<SkOpContourHead*>(&curve1->fContour);
        SkOpGlobalState globalState(contourList, &allocator
                SkDEBUGPARAMS(skipAssert) SkDEBUGPARAMS(testName));
        SkOpEdgeBuilder builder(curve1->fInner.fPath, curve2->fInner.fPath, contourList,
                &globalState);
        if (!builder.finish()) {
            return false;
        }
        SkOpCoincidence coincidence(&globalState);
        AddIntersectTs(&curve1->fContour, curve1->fContour.next(), &coincidence);
    ...
    void StrokePath::calcBisects(SkScalar width) {
        ...
        for (int ptIndex = 0; ptIndex < fCurveData.count(); ++ptIndex) {
            ...
            if (SkPath::kLine_Verb == curPtData->fPrev->fVerb &&
                ...
                trim_end(curPtData, curPtData->fPrev
                        SkDEBUGPARAMS(true) SkDEBUGPARAMS(nullptr));
            }
