...
static int num_points(GrCCPRCoverageProcessor::Mode mode)  {
    return mode >= GrCCPRCoverageProcessor::Mode::kSerpInset ? 4 : 3;
}

static int is_curve(GrCCPRCoverageProcessor::Mode mode)  {
...
private:
    ...
    void inval() {
        this->updateGpuData();
        this->INHERITED::inval(nullptr);
    }

    void updateGpuData();

    Mode fMode = Mode::kAnchorHulls;
    ...
    SkSTArray<3, std::array<int, 4>>   fGpuInstances;
...
void CCPRGeometryView::updateGpuData() {
    ...
    if (4 == vertexCount) {
        ...
        if (fMode < Mode::kLoopInset && SkCubicType::kLoop == type) {
    ...
    if (4 == vertexCount) {
        ...
        for (int i = 0; i < instanceCount; ++i) {
            fGpuInstances.push_back() = {{0, 0, controlPointsIdx + i * 4, i}};
        }
    } else if (is_curve(fMode)) {
        fGpuInstances.push_back() = {{0, 0, 2, 0}};
    } else {
        fGpuInstances.push_back() = {{0, 2, 1, 0}};
    }
}
