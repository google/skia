...
public:
    ...
    void drawContent(SkCanvas* canvas, SkColor color, int index, bool drawFront) override {
        ...
        SkRuntimeEffect::InputBuilder uniforms(fEffect);
        uniforms["lightPos"] = fLight.computeWorldPos(fSphere);
        ...
        paint.setShader(fEffect->makeShader(std::move(uniforms.fData), children, 2, nullptr, true));
