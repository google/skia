...
class SampleBump3D : public Sample3DView {
    ...
    void onOnceBeforeDraw() override {
        ...
        fShader = make_bump(img)->makeShader(SkMatrix::MakeScale(3, 3));
        ...
        const char code[] = R"(
            ...
            void main(float x, float y, inout half4 color) {
                ...
                float3 plane_norm = (localToWorld * float4(0, 0, 1, 0)).xyz;
                half4 bmp = sample(bump_map);
                plane_norm.x += convert_bump_to_delta(bmp.r);
                plane_norm.y += convert_bump_to_delta(bmp.g);
