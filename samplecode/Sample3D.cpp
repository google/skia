...
public:
    SkMatrix44 get44(const SkRect& r, const SkMatrix& model) const {
        SkMatrix44  camera,
                    perspective,
                    translate,
                    viewport;
        ...
        return viewport * perspective * camera * translate * fRot * inv(viewport) * model;
    }
...
struct Face {
    ...
    SkMatrix44 asM44(SkScalar scale) const {
        return RY(fRy) * RX(fRx);
    }
};

const Face faces[] = {
    { SK_ScalarPI/2,             0 }, // top
    {             0, SK_ScalarPI/3 }, // left
    {             0,             0 }, // front
};
...
class SampleRR3D : public Sample3DView {
    ...
    void drawContent(SkCanvas* canvas, const SkMatrix44& m) {
        ...
        SkMatrix44 mx = this->get44({0, 0, 400, 400}, m);
        SkDebugf("ctm\n"); mx.dump();
