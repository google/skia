...
class SampleBump3D : public Sample3DView {
    ...
    SkM44 fRotation,        // part of model
          fSphereRotation;  // temp during a click/drag
...
public:
    ...
    void onDrawContent(SkCanvas* canvas) override {
        ...
        for (auto f : faces) {
            SkAutoCanvasRestore acr(canvas, true);
            this->drawContent(canvas, fSphereRotation * fRotation * f.asM44(200), f.fColor);
        }
    ...
    bool onClick(Click* click) override {
        ...
        if (click->fState == skui::InputState::kUp) {
            fRotation = fSphereRotation * fRotation;
            fSphereRotation.setIdentity();
        } else {
            fSphereRotation = fSphere.computeRotation({click->fOrig.fX, click->fOrig.fY},
