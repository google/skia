...
public:
    ...
    void draw(SkCanvas* canvas, SkPaint* paint, SkScalar tx, SkScalar ty, SkScalar angle) override {
        ...
        // Use medium quality filter to get mipmaps, but don't use bicubic with highest filtering.
        SkPaint blit;
        blit.setFilterQuality(kMedium_SkFilterQuality);
        this->redrawFiltered(canvas, std::move(image), &blit);
    }

    // Redraw the results of draw() at scale with no filtering
    void redrawAtScale(SkCanvas* canvas, SkScalar scale) {
    ...
    void redrawTouchedPixels(SkCanvas* canvas) {
        ...
        SkPaint blit;
        blit.setFilterQuality(kMedium_SkFilterQuality); // still downsampling in this case
...
private:
    ...
    void drawShape(SkCanvas* canvas, const char* name, int gridX,
        ...
        canvas->drawRect(SkRect::MakeXYWH(x + 4.f, 4.f, ShapeRenderer::kTileWidth,
                ShapeRenderer::kTileHeight), outline);
        canvas->drawRect(SkRect::MakeXYWH(x + ShapeRenderer::kTileWidth + 8.f, 4.f,
                ShapeRenderer::kTileWidth, ShapeRenderer::kTileHeight), outline);
        canvas->drawRect(SkRect::MakeXYWH(x, 0.f,  8 * ShapeRenderer::kTileWidth,
                8 * ShapeRenderer::kTileHeight), outline);
        ...
        // And lastly, draw it at 8X zoom
        canvas->save();
        canvas->translate(x, 0.f);
        shape->redrawAtScale(canvas, 8.0f);
        canvas->restore();
