...
protected:
    ...
    void onDrawContent(SkCanvas* canvas) override {
        ...
        if (fShowSub) {
            ...
            SkPoint tmp[3 + 2 + 1];
            for (int i = 0; i < 3; ++i) {
                canvas->drawLine(fPts[i], fPts[i+1], paint);
                tmp[i] = lerp(fPts[i], fPts[i+1], fT);
            }
            for (int i = 0; i < 2; ++i) {
                canvas->drawLine(tmp[i], tmp[i+1], paint);
                tmp[3 + i] = lerp(tmp[i], tmp[i+1], fT);
            }
            canvas->drawLine(tmp[3], tmp[4], paint);
            tmp[5] = lerp(tmp[3], tmp[4], fT);

            paint.setColor(SK_ColorBLUE);
            paint.setStyle(SkPaint::kFill_Style);
            for (int i = 0; i < 6; ++i) {
                canvas->drawCircle(tmp[i].fX, tmp[i].fY, 4, paint);
            }
