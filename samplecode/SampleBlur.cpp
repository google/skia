...
void make_paint(SkPaint* paint, sk_sp<SkTypeface> typeface) {
...
std::unique_ptr<SkFILEStream> GetResourceAsStream2(const char* resource) {
...
sk_sp<SkTypeface> bar(const char* resource) {
    std::unique_ptr<SkFILEStream> stream(GetResourceAsStream2(resource));
    if (!stream) {
        return nullptr;
    }
...
protected:
    ...
    void init() {
      ...
      //        fTypeface = bar("C:\\src\\skia.1\\android-fonts\\NotoSansCJK-Regular.ttc");
        fTypeface = bar("/Users/robertphillips/src/skia.0/android-fonts/NotoSansCJK-Regular.ttc");
