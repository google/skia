...
static const char* gNames[] = {
    #if SK_SAMPLES_USE_SKIMAGES_DIR
...
static sk_sp<SkData> data(const char* name) {
    #if SK_SAMPLES_USE_SKIMAGES_DIR
    return SkData::MakeFromFileName(name);
    #else
    return GetResourceAsData(name);
    #endif
}
    ...
    void onOnceBeforeDraw() override {
        if (fBitmaps.empty()) {
            ...
            for (const char* name : gNames) {
                SkBitmap bitmap;
                (void)decode_file(data(name), &bitmap);
                fBitmaps.push_back(std::move(bitmap));
            }
            for (const char* name : gNames) {
                SkBitmap bitmap;
                (void)decode_file(data(name), &bitmap, kRGB_565_SkColorType);
                fBitmaps.push_back(std::move(bitmap));
            }
