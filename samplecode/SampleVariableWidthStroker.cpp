...
private:
    ...
    OffsetSegments strokeLine(const PathSegment& line, float tmin, float tmax, bool needsMove);

    /** Strokes a quadratic segment */
    OffsetSegments strokeQuad(const PathSegment& quad, float tmin, float tmax, bool needsMove);
...
SkPath SkVarWidthStroker::getFillPath(const SkPath& path,
    ...
    const float dt = 1.0f / numSegs;
    float t = 0;
    while ((segment.fVerb = it.next(&segment.fPoints[0])) != SkPath::kDone_Verb) {
        // Stroke the current segment
        switch (segment.fVerb) {
            case SkPath::kLine_Verb:
                offsetSegs = strokeLine(segment, t, t + dt, firstSegment);
                break;
            case SkPath::kQuad_Verb:
                offsetSegs = strokeQuad(segment, t, t + dt, firstSegment);
                break;
        ...
        if (!firstSegment) {
            ...
            // Append the join
            const float innerRadius = varWidthInner.eval(t);
            const float outerRadius = varWidth.eval(t);
...
SkVarWidthStroker::OffsetSegments SkVarWidthStroker::strokeLine(const PathSegment& line,
    ...
    ScalarBezCurve varWidth, varWidthInner;
    fVarWidth.split(tmin, tmax, &varWidth);
    fVarWidthInner.split(tmin, tmax, &varWidthInner);
    varWidthInner = ScalarBezCurve::Mul(varWidthInner, -1);
