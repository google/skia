...
constexpr inline SkPoint rotate180(const SkPoint& p) { return p * -1; }
constexpr inline bool isClockwise(const SkPoint& a, const SkPoint& b) { return a.cross(b) > 0; }
...
std::vector<SkVarWidthStroker::PathSegment> SkVarWidthStroker::strokeSegment(
    ...
    while (!stack.empty()) {
        ...
        if (viz::outerErr == nullptr) {
            using namespace viz;
            outerErr.reset(new ScalarBezCurve(E));
