...
struct FilterNode {
    ...
    // maybe used as an input in an earlier node. Null when this represents the "source" input
    ...
    // calculated to match what the filter compute as part of the whole DAG. fForwardIsolatedBounds
...
static void compute_forward_bounds(const SkIRect& srcRect, FilterNode* node) {
    if (node->fFilter) {
        // For forward filtering, the leaves of the DAG are evaluated first and are then propagated
        // to the root. This means that every filter's filterBounds() function sees the original src
        // rect. It is never dependent on the parent node (unlike reverse filtering), so calling
        // filterBounds() on an intermediate node gives us the correct intermediate values.
        node->fForwardBounds = node->fFilter->filterBounds(
                srcRect, node->fLocalCTM, SkImageFilter::kForward_MapDirection, nullptr);

        // For isolated forward filtering, it uses the same input but should not be propagated to
        // the inputs, so get the filter node bounds directly.
        node->fForwardIsolatedBounds = SkFilterNodeBounds(
                node->fFilter.get(), srcRect, node->fLocalCTM,
                SkImageFilter::kForward_MapDirection, nullptr);
    } else {
        node->fForwardBounds = srcRect;
        node->fForwardIsolatedBounds = srcRect;
    }

    // Fill in children
    for (int i = 0; i < node->fInputNodes.count(); ++i) {
        compute_forward_bounds(srcRect, &node->fInputNodes[i]);
    }
}

static void compute_reverse_bounds(const SkIRect& inputRect, const SkIRect* parentReverseBounds,
                                   const SkIRect* parentLayerBounds, FilterNode* node) {
    // The layer bounds (matching what SkCanvas computes), use the content rect mapped by the entire
    // CTM as its input rect, while 'inputRect' is assumed to be just the content mapped by the
    // local matrix for the filter.
    // FIXME (michaelludwig) - Once SkCanvas' filter handling is fixed, this should be
    // (remainder * local)
    SkIRect incorrectCanvasInputRect;
    SkMatrix ctm = SkMatrix::Concat(node->fLocalCTM, node->fRemainingCTM);
    ctm.mapRect(node->fContent).roundOut(&incorrectCanvasInputRect);

    if (!node->fFilter) {
        node->fReverseLocalBounds = inputRect;
        node->fReverseLocalIsolatedBounds = inputRect;
        node->fLayerBounds = incorrectCanvasInputRect;
        return;
    }

    // The fully isolated reverse bounds uses the original input rect as the source, without having
    // been fed through the parent chain, and does not propagate to its inputs
    node->fReverseLocalIsolatedBounds = SkFilterNodeBounds(
            node->fFilter.get(), inputRect, node->fLocalCTM,
            SkImageFilter::kReverse_MapDirection, &inputRect);

    // The regular reverse bounds are computed by calling filterBounds() using the parent rectangle
    // as inputs (or inputRect at the root), but does propagate to its inputs
    SkIRect reverseInput = parentReverseBounds ? *parentReverseBounds : inputRect;
    node->fReverseLocalBounds = node->fFilter->filterBounds(
            reverseInput, node->fLocalCTM, SkImageFilter::kReverse_MapDirection, &inputRect);

    // NOTE: SkCanvas provides the incorrect canvas input rect as the last pointer as well, which
    // is why we use 'incorrectCanvasInputRect' there instead of just '&inputRect'. We don't use
    // '&layerInput' because that changes as we move through the DAG and inputRect is does not.
    SkIRect layerInput = parentLayerBounds ? *parentLayerBounds : incorrectCanvasInputRect;
    node->fLayerBounds = node->fFilter->filterBounds(
            layerInput, node->fLocalCTM, SkImageFilter::kReverse_MapDirection,
            &incorrectCanvasInputRect);

    // Recurse to children
    if (node->fInputNodes.count() > 0) {
        // Calculate this node's reverse/layer bounds that accounts for parent filtering (like the
        // already computed fReverseLocalBounds), but does not propagate to its inputs (like the
        // already computed fReverseLocalIsolatedBounds).
        SkIRect nextParentReverseBounds = SkFilterNodeBounds(
            node->fFilter.get(), reverseInput, node->fLocalCTM,
            SkImageFilter::kReverse_MapDirection, &inputRect);
        SkIRect nextParentLayerBounds = SkFilterNodeBounds(
            node->fFilter.get(), layerInput, node->fLocalCTM,
            SkImageFilter::kReverse_MapDirection, &incorrectCanvasInputRect);

        for (int i = 0; i < node->fInputNodes.count(); ++i) {
            compute_reverse_bounds(inputRect, &nextParentReverseBounds, &nextParentLayerBounds,
                                   &node->fInputNodes[i]);
        }
    }
}
...
static FilterNode build_dag(const SkMatrix& ctm, const SkRect& rect,
    ...
    SkIRect inputRect;
    local.mapRect(rect).roundOut(&inputRect);
    compute_forward_bounds(inputRect, &rootNode.fInputNodes[0]);
    compute_reverse_bounds(inputRect, nullptr, nullptr, &rootNode.fInputNodes[0]);
    return rootNode;
}
...
static float draw_dag(SkCanvas* canvas, SkSurface* nodeSurface, const FilterNode& node) {
    ...
    for (uint i = 0; i < node.fInputNodes.size(); ++i) {
