...
/*
 * The slide is where the drawing of the quad is handled. Every frame, a new quad is generated from
 * the bone data in generateQuad(). During generation, the vertices in the quad are assigned bones
 * and their corresponding weights according to the bones' weight functions. For each vertex, the
 * highest 4 weights are chosen and normalized so that they add to 1.
 * The content of the quad is drawn in an SkSurface in EditorContent at the top.
 */
class EditorContent {
public:
