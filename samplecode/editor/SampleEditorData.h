...
/*
 * Data structs contain the raw data needed to represent the animation so that it can be saved and
 * loaded. They contain serialize() functions, which given an SkJSONWriter, will write out their
 * own data in JSON format. They also contain static Deserialize() functions, which given
 * skjson::Value objects, will output the deserialized data.
 */
...
/*
 * Weight functions take a bone's position, a vertex's position, and returns the value of the
 * weight function for that bone at the given vertex position. They can have configurable
 * attributes.
 */
...
/*
 * This is a weight function that applies a uniform weight across all vertices.
 */
...
/*
 * This is a weight function that applies weights according to a Gaussian centered at the point.
 * Rather than being a true 2D Gaussian, the weight is calculated using a 1D Gaussian with the
 * error being the distance between the point and the bone. As the weights are normalized
 * afterwards anyways, using this pseudo 2D Gaussian is no problem.
 */
...
/*
 * The cubic data consists of a start point, two control points, and an end point. The start point
 * is a pointer to the previous cubic's end point.
 */
...
/*
 * The path data consists of a start point and a set of cubic data. Whenever update() is called,
 * all the cubic data objects are updated so that their start points point to the last point of the
 * previous cubic data object, or the start point for the first cubic data object. This is done
 * because the path should be continuous.
 */
...
/*
 * The bone data contains the bone's current orientation, its bind orientation, its transformation
 * matrix, its weight function, and its path data. During serialization, the bone's current
 * orientation and thus transformation matrix is left out of the data, as the current orientation
 * isn't needed to recreate the animation, and the transformation matrix can be calculated from the
 * current and binding orientations. Whenever any of the data changes, update() should be called to
...
/*
 * The animation struct holds the size and speed of the animation. It also contains a vector of
 * bone data structs, which make up the majority of the animation data.
 */
