...
public:
    CircDrawable2(SkRect r, SkColor c, const SkBlendMode* modePtr) : fModePtr(modePtr) {
...
public:
    XferDemo2() {
        ...
        for (int i = 0; i < N; ++i) {
            fDrs[i].reset(new CircDrawable2(SkRect::MakeXYWH(100 + i * 100.f, 100 + i * 100.f, 200, 200), colors[i], &fMode));
        }
        this->addButtons();
    }
