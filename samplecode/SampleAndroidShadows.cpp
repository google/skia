...
protected:
    ...
    void drawSpotShadowAlt(SkCanvas* canvas, const SkPath& path, SkScalar zValue,
        ...
        if (strokedArea > filledArea) {
            ...
            SkRect tmp = shadowRRect.rect().makeInset(insetAmount/2.0f, insetAmount/2.0f);
            SkScalar tmpR = SkTMax(shadowRRect.getSimpleRadii().fX - insetAmount/2.0f, minRadius);

            shadowRRect = SkRRect::MakeRectXY(tmp, tmpR, tmpR);
            paint.setStyle(SkPaint::kStroke_Style);
