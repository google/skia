...
protected:
    ...
    void onDrawContent(SkCanvas* canvas) override {
        ...
        for (unsigned int i = 0; i < SK_ARRAY_COUNT(starVerts) - 1; ++i) {
        ...
        SkOffsetSimplePolygon(starVerts, SK_ARRAY_COUNT(starVerts), -22.0f, &offsetPoly);
        ...
        paint.setColor(SK_ColorBLACK);
        for (unsigned i = 0; i < SK_ARRAY_COUNT(notchVerts) - 1; ++i) {
        ...
        if (SkOffsetSimplePolygon(notchVerts, SK_ARRAY_COUNT(notchVerts), -22.0f, &offsetPoly)) {
            for (int i = 0; i < offsetPoly.count() - 1; ++i) {
                canvas->drawLine(offsetPoly[i], offsetPoly[i + 1], paint);
            }
            canvas->drawLine(offsetPoly[offsetPoly.count() - 1], offsetPoly[0], paint);
        ...
        paint.setColor(SK_ColorBLACK);
        for (unsigned i = 0; i < SK_ARRAY_COUNT(tabVerts) - 1; ++i) {
            canvas->drawLine(tabVerts[i], tabVerts[i + 1], paint);
        }
        canvas->drawLine(tabVerts[SK_ARRAY_COUNT(tabVerts) - 1], tabVerts[0], paint);

        SkOffsetSimplePolygon(tabVerts, SK_ARRAY_COUNT(tabVerts), -22.0f, &offsetPoly);
        for (int i = 0; i < offsetPoly.count() - 1; ++i) {
            canvas->drawLine(offsetPoly[i], offsetPoly[i + 1], paint);
        }
        canvas->drawLine(offsetPoly[offsetPoly.count() - 1], offsetPoly[0], paint);
