...
protected:
    ...
    bool onAnimate(const SkAnimTimer& timer) override {
        ...
        fRect1->setR(fRect1->getL() + kSize * 0.5f * (1 + std::sin(timer.msec() / kRate)));
        fRect1->setB(fRect1->getT() + kSize * 0.5f * (1 + std::cos(timer.msec() / kRate)));
        fRect2->setR(fRect2->getL() + kSize * 0.5f * (1 + std::cos(SK_ScalarPI / 2 +
                                                                   timer.msec() / kRate)));
        fRect2->setB(fRect2->getT() + kSize * 0.5f * (1 + std::sin(SK_ScalarPI / 2 +
                                                                   timer.msec() / kRate)));

        fColor1->setColor(SkColorSetARGB(128 * (1 + std::sin(timer.msec() / kRate)), 0, 0x80, 0));
        fColor2->setColor(SkColorSetARGB(128 * (1 + std::cos(timer.msec() / kRate)), 0, 0, 0x80));
        return true;
    }
