...
protected:
    ...
    void onDrawContent(SkCanvas* canvas) override {
        ...
        str.printf("%s-%s: tot Area: %ld %%full: %.2f (%.2f) numTextures: %d/%d",
                   ...
                   fCurRandRect,
                   kNumRandRects);
        canvas->drawUTF8String(str, 50, kHeight + 50, blackBigFont, SkPaint());

        str.printf("Press \'j\' to toggle rectanizer");
        canvas->drawUTF8String(str, 50, kHeight + 100, blackBigFont, SkPaint());

        str.printf("Press \'h\' to toggle rects");
        canvas->drawUTF8String(str, 50, kHeight + 150, blackBigFont, SkPaint());
    }
