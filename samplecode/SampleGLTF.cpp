...
std::unique_ptr<GrFragmentProcessor> SkGLTFLightingShaderImpl::asFragmentProcessor(const GrFPArgs& args) const {
    ...
    auto dst = args.fDstColorSpaceInfo->refColorSpace();
    if (dst == nullptr) {
        // Assume destination is srgb and use linear transform for brdf stage
        dst = SkColorSpace::MakeSRGB();
    }

    auto src = dst->makeLinearGamma();
    auto fp = GrColorSpaceXformEffect::Make(std::move(base), src.get(), dst.get());
