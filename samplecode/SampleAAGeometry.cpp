...
class AAGeometryView : public SampleView {
    ...
    PathUndo* fUndo;
    SkPoint fPriorBisect;  // two deep stack to look for overlap before drawing
...
public:
    ...
    void draw_bisect(SkCanvas* canvas, const SkVector& lastVector, const SkVector& vector,
                const SkPoint& pt, SkPath::Verb verb, int counter) {
        ...
        if (fBisectButton.enabled()) {
            ...
            if (fLastBisectSet) {
                if (fPriorBisectSet) {
                    // start here
                    // need to skip fLastBisect[0] if prior verb is quad 
                }
                fPriorBisectSet = true;
                fPriorBisect = fLastBisect[0];
                 canvas->drawLine(fLastBisect[0], bisectEnd, fDebugSkeletonPaint);
    //           SkDebugf("{{{%g,%g %g,%g}}} id=%d last=%s this=%s\n", bisectEnd.fX, bisectEnd.fY,
    //                   fLastBisect[0].fX, fLastBisect[0].fY, counter, 
    //                   SkPath::kLine_Verb == fLastVerb ? "line" : "quad",
    //                   SkPath::kLine_Verb == verb ? "line" : "quad");
            } else {
    ...
    struct PointData {
        ...
        SkVector fTangent;  // from this point to next, if any
        SkPath::Verb fVerb;
        int fCtrlIndex;  // always 0 for line, 1 for quad/conic ctrl, 2 for 2nd cubic ctrl
        int fPathIndex;  // index of this point within path
        int fPrevIndex; // index of previous point in ptData, -1 if start
        int fNextIndex; // index of next point in ptData, -1 if end
        bool fEndPoint;
        bool fHasLength;
    };

    void draw_bisects(SkCanvas* canvas, bool activeOnly) {
        SkTDArray<PointData> ptData; 
        int pointCount = fPath.countPoints();
        ptData.setCount(pointCount);
        SkPoint pts[4];
        SkPath::Verb verb, firstVerb = SkPath::kDone_Verb;
        ...
        SkPoint firstPoint;
        SkPoint lastPoint;
        PointData* lastPtData = nullptr;
        PointData* curPtData = &ptData[0];
        while ((verb = iter.next(pts)) != SkPath::kDone_Verb) {
            switch (verb) {
            case SkPath::kMove_Verb:
                contourStart = ptIndex;
                firstPoint = pts[0];
                // intentionally fall through
            case SkPath::kClose_Verb:
                if (lastPtData) {
                    SkASSERT(0 <= lastPtIndex);
                    lastPtData->fNextIndex = ptIndex;
                    PointData* curPtData = &ptData[ptIndex];
                    *curPtData = *lastPtData;
                    curPtData->fPt = lastPoint;
                    curPtData->fTangent = firstPoint - lastPoint;
                    curPtData->fHasLength = curPtData->fTangent.setLength(fWidthControl.fValLo);
                    curPtData->fPathIndex = ptIndex;
                    curPtData->fPrevIndex = lastPtIndex;
                    if (SkPath::kClose_Verb == verb) {
                        curPtData->fVerb = firstVerb;
                        curPtData->fCtrlIndex = 0;
                        curPtData->fNextIndex = contourStart;
                    } else {
                        curPtData->fVerb = lastPtData->fVerb;
                        curPtData->fCtrlIndex = lastPtData->fCtrlIndex + 1;
                    }
                    lastPtData = nullptr;
                    lastPtIndex = -1;
                }                
            ...
            case SkPath::kConic_Verb:
            case SkPath::kCubic_Verb: {
                if (SkPath::kDone_Verb == firstVerb) {
                    firstVerb = verb;
                }
                curPtData->fPt = pts[0];
                curPtData->fL.set(SK_ScalarNaN, SK_ScalarNaN);  // don't know yet
                curPtData->fR.set(SK_ScalarNaN, SK_ScalarNaN);  // don't know yet
                curPtData->fVerb = verb;
                curPtData->fNextIndex = -1;
                lastPoint = pts[1];
                curPtData->fTangent = lastPoint - curPtData->fPt;
                curPtData->fHasLength = curPtData->fTangent.setLength(fWidthControl.fValLo);
                curPtData->fCtrlIndex = 0;
                curPtData->fPathIndex = ptIndex;
                curPtData->fPrevIndex = lastPtIndex;
                curPtData->fEndPoint = true;
                if (curPtData->fHasLength) {
                    if (lastPtData) {
                        lastPtData->fNextIndex = ptIndex;
                    }
                    lastPtData = curPtData;
                    lastPtIndex = ptIndex;
                    curPtData = &ptData[++ptIndex];
                    if (SkPath::kLine_Verb == verb) {
                        break;
                    }
                    *curPtData = *lastPtData; 
                ...
                do {
                    ++ctrlIndex;
                    loopLimit = SkPath::kCubic_Verb != verb || ctrlIndex >= 2;
                    curPtData->fPt = lastPoint;
                    lastPoint = pts[ctrlIndex + 1];
                    curPtData->fTangent = lastPoint - curPtData->fPt;
                    curPtData->fHasLength = curPtData->fTangent.setLength(fWidthControl.fValLo);
                    curPtData->fCtrlIndex = ctrlIndex;
                    curPtData->fPathIndex = ptIndex;
                    curPtData->fPrevIndex = lastPtIndex;
                    curPtData->fEndPoint = false;
                    if (curPtData->fHasLength) {
                        if (lastPtData) {
                            lastPtData->fNextIndex = ptIndex;
                        }
        ...
        for (int ptIndex = 0; ptIndex < ptData.count(); ++ptIndex) {
            PointData* curPtData = &ptData[ptIndex];
            if (!curPtData->fEndPoint) {
                continue;
            }
            if (SkASSERT(curPtData->fHasLength) {
                continue;
            }
            SkVector bisect;
            PointData* priorPtData;
            const SkVector* lastV;
            const SkVector& nextV = curPtData->fTangent;
            if (-1 == curPtData->fPrevIndex) {
                priorPtData = nullptr;
                lastV = nullptr;
            } else {
                priorPtData = &ptData[curPtData->fPrevIndex];
                lastV = &priorPtData->fTangent;
            }

            if (-1 == curPtData->fPrevIndex) {
                bisect.set(curPtData->fTangent.fY, -curPtData->fTangent.fX);
                curPtData->fL = curPtData->fPt + bisect;
                curPtData->fR = curPtData->fPt - bisect;
                continue;
            }
            ...
            curPtData->fL = curPtData->fPt + bisect;
            curPtData->fR = curPtData->fPt - bisect;
            if (SkPath::kLine_Verb != priorPtData->fVerb && SkPath::kLine_Verb != curPtData->fVerb) {
                // need to compute intersection of inner pair
                SkASSERT(0);  // incomplete
                continue;
            }
            if (SkPath::kLine_Verb != priorPtData->fVerb) {
        ...
        while ((verb = iter.next(pts)) != SkPath::kDone_Verb) {
            ...
            switch (verb) {
                ...
                case SkPath::kClose_Verb:
                    if (foundFirst) {
                        draw_bisect(canvas, lastVector, firstVector, firstPt, fFirstVerb, counter);
                        if (fLastBisectSet) {
      ...
      //                      canvas->drawLine(fFirstBisect[1], fLastBisect[1], fSkeletonPaint);
            ...
            if (SkPath::kLine_Verb <= verb && verb <= SkPath::kCubic_Verb) {
                if (!foundFirst) {
                    ...
                    draw_bisect(canvas, lastVector, vector, pts[0], verb, counter);
                }
                lastVector = nextLast;
            }
            fLastPt = pts[0];
