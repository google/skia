...
public:
    ...
    void set_point_array() {
        ...
        fStrokePath.fCurveData = SkTArray<CurveData>(verbCount);
        SkPath::Iter iter(fPath, false);
        int ptIndex = 0;
        CurveData* curCurve = &fStrokePath.fCurveData.push_back();
        bool degenerate;
        ...
        while ((curCurve->fVerb = iter.next(curCurve->fPts)) != SkPath::kDone_Verb) {
            ...
            case SkPath::kCubic_Verb:
                ...
                if (!degenerate) {
                    ...
                    prevCurve = curCurve;
                    curCurve = &fStrokePath.fCurveData.push_back();
                }
                ptIndex += ptCount;
                break;
            }
        }
        if (!fStrokePath.fCurveData.count()) {
            return;
        }
        SkASSERT(SkPath::kDone_Verb == curCurve->fVerb);
        SkASSERT(curCurve == &fStrokePath.fCurveData.back());
        fStrokePath.fCurveData.pop_back();
    }
