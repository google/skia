...
struct CurveData {
    ...
    SkPath fOuter;
    CurveData* fPrev;
    SkPoint fL;   // mid angle formed by prev, this segs; length == stroke width; on the left
    SkPoint fR;   // mid angle formed by prev, this segs; length == stroke width; on the right
    SkPoint fBisects[2];
...
public:
    ...
    void draw_bisect(SkCanvas* canvas, const CurveData* curveData) {
        const SkPoint& pt = curveData->lastPt();
        const CurveData* prev = curveData->fPrev;
        if (!prev) {
            return;
        }
        if (fBisectButton.enabled()) {
            canvas->drawLine(pt, curveData->fBisects[1], fSkeletonPaint);
            canvas->drawLine(prev->fBisects[1], curveData->fBisects[1], fDebugSkeletonPaint);
            canvas->drawLine(pt, curveData->fBisects[0], fSkeletonPaint);
            canvas->drawLine(prev->fBisects[0], curveData->fBisects[0], fSkeletonPaint);
        }
        const SkVector& lastV = prev->fTangents[1];
        const SkVector& nextV = curveData->fTangents[0];
        if (fBisectButton.enabled()) {
        ...
        if (fJoinButton.enabled()) {
            ...
            canvas->drawLine(curveData->fBisects[1].fX, curveData->fBisects[1].fY,
    ...
    void set_point_array() {
        ...
        while ((curCurve->fVerb = iter.next(curCurve->fPts)) != SkPath::kDone_Verb) {
            ...
            case SkPath::kMove_Verb:
                ...
                break;
            case SkPath::kClose_Verb:
                if (CurvePos::kUnknown == curCurve->fPos) {
                    fCurveData.pop_back();
                }
                curCurve = &fCurveData.back();
                curCurve->fPos = CurvePos::kLast;
                break;
            case SkPath::kLine_Verb:
                ptCount -= 1;
            case SkPath::kQuad_Verb:
            ...
            case SkPath::kCubic_Verb: {
                ...
                if (!degenerate) {
                    ...
                    curCurve->fPos = CurvePos::kUnknown;
                }
                } break;
            }
        }
        ...
        for (curCurve = fCurveData.begin(); curCurve != fCurveData.end(); ++curCurve) {
            curCurve->fL.set(SK_ScalarNaN, SK_ScalarNaN);  // don't know yet
            curCurve->fR.set(SK_ScalarNaN, SK_ScalarNaN);  // don't know yet
            curCurve->fPrev = prevCurve;
            if (CurvePos::kFirst == curCurve->fPos) {
                firstCurve = curCurve;
            } else if (CurvePos::kLast == curCurve->fPos) {
                firstCurve->fPrev = curCurve;
            }
            prevCurve = curCurve;
        }
    }
    ...
    void calc_bisects() {
        for (int ptIndex = 0; ptIndex < fCurveData.count(); ++ptIndex) {
            ...
            SkVector bisect;
            const SkVector& last_v = curPtData->fPrev->fTangents[1];
            const SkVector& next_v = curPtData->fTangents[0];
            bisect.set(last_v.fX + next_v.fX, last_v.fY + next_v.fY);
            SkVector angleOpp = last_v - next_v;
            SkScalar oppLen = angleOpp.length() / 2;
            bisect.setLength(fWidthControl.fValLo * fWidthControl.fValLo / oppLen);
            curPtData->fL = curPtData->fPts[0] + bisect;
            curPtData->fR = curPtData->fPts[0] - bisect;
            ...
            SkVector lastV = curPtData->fPrev->fTangents[1];
            SkScalar lastLen = lastV.length();
            SkVector nextV = curPtData->fTangents[0];
            ...
            bisect = { lastV.fX + nextV.fX, lastV.fY + nextV.fY };
            angleOpp = lastV - nextV;
            angleOpp.scale(0.5);
            oppLen = angleOpp.length();
            ...
            const SkPoint& pt = curPtData->fPts[0];
            curPtData->fBisects[0] = pt + bisect;
            curPtData->fBisects[1] = pt - bisect;
            if (SkPath::kQuad_Verb == curPtData->fPrev->fVerb &&
                    SkPath::kLine_Verb == curPtData->fVerb) {
                curPtData->fPrev->fEndT = adjust_quad(curPtData->fPrev->fPts, 2,
                        curPtData->lastPt());
            } else if (SkPath::kLine_Verb == curPtData->fPrev->fVerb &&
                    SkPath::kQuad_Verb == curPtData->fVerb) {
                curPtData->fStartT = adjust_quad(curPtData->fPts, 0, curPtData->fPrev->fPts[0]);
            }
        }
        for (int ptIndex = 0; ptIndex < fCurveData.count(); ++ptIndex) {
            CurveData* curPtData = &fCurveData[ptIndex];
            ...
            for (PathStroker::StrokeType strokeType : { PathStroker::kInner_StrokeType,
                ...
                // get next tangent
                SkScalar startT = PathStroker::kInner_StrokeType == strokeType ?
                        curPtData->fStartT : 0;
                SkScalar endT = PathStroker::kInner_StrokeType == strokeType ?
                        curPtData->fEndT : 1;
                quadConstruct.init(startT, endT);
                pathStroker.quadStroke(pts, &quadConstruct);
                const SkPath& active = pathStroker.outer().isEmpty() ? pathStroker.inner() :
                        pathStroker.outer();
                (PathStroker::kInner_StrokeType == strokeType ?
                        curPtData->fInner : curPtData->fOuter) = active;
            }
        }
        for (int ptIndex = 0; ptIndex < fCurveData.count(); ++ptIndex) {
            CurveData* curPtData = &fCurveData[ptIndex];
            ...
            if (SkPath::kQuad_Verb == curPtData->fPrev->fVerb &&
                    SkPath::kLine_Verb == curPtData->fVerb) {
                curPtData->fPrev->fInner.getLastPt(&curPtData->fBisects[0]);
            } else if (SkPath::kLine_Verb == curPtData->fPrev->fVerb &&
                    SkPath::kQuad_Verb == curPtData->fVerb) {
                curPtData->fBisects[0] = curPtData->fInner.getPoint(0);
            }
        }
    }
    ...
    void draw_bisects(SkCanvas* canvas, bool activeOnly) {
        ...
        for (int ptIndex = 0; ptIndex < fCurveData.count(); ++ptIndex) {
            ...
            if (SkPath::kQuad_Verb == curPtData->fVerb) {
                ...
                canvas->drawPath(curPtData->fInner, fSkeletonPaint);
                canvas->drawPath(curPtData->fOuter, fSkeletonPaint);
