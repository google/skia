...
public:
    ...
    void draw_bisect(SkCanvas* canvas, const SkVector& lastVector, const SkVector& vector,
        ...
        if (fBisectButton.enabled()) {
            ...
            if (fLastBisectSet && SkPath::kLine_Verb == fLastVerb && SkPath::kLine_Verb == verb ) {
                canvas->drawLine(bisectEnd, fLastBisect[0], fSkeletonPaint);
            } else {
        ...
        while ((verb = iter.next(pts)) != SkPath::kDone_Verb) {
            ...
            switch (verb) {
                ...
                case SkPath::kQuad_Verb: {
                    ...
                    canvas->drawPath(pathStroker.inner(), fSkeletonPaint);
                    SkDebugf("inner\n");
                    pathStroker.inner().dump();
                    canvas->drawPath(pathStroker.outer(), fSkeletonPaint);
                    SkDebugf("outer\n");
                    pathStroker.outer().dump();
                    }
                    } break;
                case SkPath::kConic_Verb:
                    nextLast = pts[1] - pts[2];
                ...
                case SkPath::kClose_Verb:
                    if (foundFirst) {
                        draw_bisect(canvas, lastVector, firstVector, firstPt, SkPath::kLine_Verb);
                        if (fLastBisectSet) {
