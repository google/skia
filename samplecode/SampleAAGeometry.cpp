...
struct PathPair {
    SkPath* fPath;  // contains cached bounds
...
struct CurveData {
    ...
    CurveData* fPrev;
    SkTArray<SkPath> fStorage;  // paths for fInner and fOuter are allocated here
    CurvePos fPos;
    StrokeData fInner;
    StrokeData fOuter;
    ...
    void calc_bisects() {
        for (int ptIndex = 0; ptIndex < fCurveData.count(); ++ptIndex) {
            ...
            if (SkPath::kLine_Verb == curPtData->fPrev->fVerb &&
                ...
                curPtData->calcLineBisects(fWidthControl.fValLo);
                continue;
            }
        ...
        for (int ptIndex = 0; ptIndex < fCurveData.count(); ++ptIndex) {
            ...
            for (PathStroker::StrokeType strokeType : { PathStroker::kInner_StrokeType,
                ...
                // get next tangent
                quadConstruct.init(0, 1);
                pathStroker.quadStroke(pts, &quadConstruct);
                const SkPath& active = pathStroker.outer().isEmpty() ? pathStroker.inner() :
                        pathStroker.outer();
                curPtData->fStorage.push_back(active);
                StrokeData& strokeData = PathStroker::kInner_StrokeType == strokeType ?
                        curPtData->fInner : curPtData->fOuter;
                strokeData.fStroke->fPath = &curPtData->fStorage.back();
            }
        }
        for (int ptIndex = 0; ptIndex < fCurveData.count(); ++ptIndex) {
            CurveData* curPtData = &fCurveData[ptIndex];
            ...
            if (SkPath::kQuad_Verb == curPtData->fPrev->fVerb &&
                    SkPath::kLine_Verb == curPtData->fVerb) {
                curPtData->fPrev->fInner.fStroke->fPath->getLastPt(&curPtData->fInner.fBisect);
            } else if (SkPath::kLine_Verb == curPtData->fPrev->fVerb &&
                    SkPath::kQuad_Verb == curPtData->fVerb) {
                curPtData->fInner.fBisect = curPtData->fInner.fStroke->fPath->getPoint(0);
            }
        }
    }
    ...
    void draw_bisects(SkCanvas* canvas, bool activeOnly) {
        ...
        for (int ptIndex = 0; ptIndex < fCurveData.count(); ++ptIndex) {
            ...
            if (SkPath::kQuad_Verb == curPtData->fVerb) {
                ...
                canvas->drawPath(*curPtData->fInner.fStroke->fPath, fSkeletonPaint);
                canvas->drawPath(*curPtData->fOuter.fStroke->fPath, fSkeletonPaint);
