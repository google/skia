...
class SampleFitCubicToCircle : public Sample {
    ...
    double fControlLength;  // Length of P0->P1 and/or P2->P3 in unit circle space.
...
// Fits a cubic to an arc on the unit circle with endpoints (x0, y0) and (x1, y1). Using the below
// 3 constraints, we arrive at the formula used in the method:
//
...
static float fit_cubic_to_unit_circle(double x0, double y0, double x1, double y1,
    ...
    // "Control length" is the length of P0->P1 and/or P2->P3 in unit circle space.
    double controlLength = (std::sqrt(1 + dot) * kM + kA) / std::sqrt(1 - dot);
    *X = {x0, x0 - y0 * controlLength, x1 + y1 * controlLength, x1};
    *Y = {y0, y0 + x0 * controlLength, y1 - x1 * controlLength, y1};
