...
void update_pos(const sk_sp<skan::RRect>& rr, const SkPoint& pos) {
...
protected:
    void onOnceBeforeDraw() override {
        ...
        auto bg_path  = skan::Path::Make(bgPath);
        auto bg_paint = skan::Color::Make(SK_ColorBLACK);
        bg_paint->setStyle(SkPaint::kStroke_Style);
        bg_paint->setStrokeWidth(kBackgroundStroke);

        auto ball_paint    = skan::Color::Make(SK_ColorGREEN),
             paddle0_paint = skan::Color::Make(SK_ColorBLUE),
             paddle1_paint = skan::Color::Make(SK_ColorRED),
             shadow_paint  = skan::Color::Make(SK_ColorBLACK);
        ...
        auto group = skan::Group::Make();
        group->addChild(skan::Draw::Make(std::move(bg_path), std::move(bg_paint)));
        group->addChild(skan::Draw::Make(fPaddle0.shadowNode, shadow_paint));
        group->addChild(skan::Draw::Make(fPaddle1.shadowNode, shadow_paint));
        group->addChild(skan::Draw::Make(fBall.shadowNode, shadow_paint));
        group->addChild(skan::Draw::Make(fPaddle0.objectNode, paddle0_paint));
        group->addChild(skan::Draw::Make(fPaddle1.objectNode, paddle1_paint));
        group->addChild(skan::Draw::Make(fBall.objectNode, ball_paint));

        // Handle everything in a normalized 1x1 space.
        fContentMatrix = skan::Matrix::Make(
            SkMatrix::MakeRectToRect(SkRect::MakeWH(1, 1),
                                     SkRect::MakeIWH(this->width(), this->height()),
                                     SkMatrix::kFill_ScaleToFit));
        auto root = skan::Transform::Make(std::move(group), fContentMatrix);
        fScene = skan::Scene::Make(std::move(root), skan::AnimatorList());
    ...
    bool onQuery(SkEvent* evt) override {
        if (SampleCode::TitleQ(*evt)) {
            SampleCode::TitleR(evt, "SkanPong");
            return true;
        }
...
private:
    struct Object {
        void initialize(const SkRRect& rrect, const SkPoint& p, const SkVector& s) {
            objectNode = skan::RRect::Make(rrect);
            shadowNode = skan::RRect::Make(rrect);
    ...
    sk_sp<skan::Matrix>          fContentMatrix;
