...
static SkScalar get_area_coverage(const bool edgeAA[4], const SkPoint corners[4], const SkPoint& point) {
    ...
    float area = get_area_coverage_sk4f(xs, ys, a, b, c, flipped, point);*/
...
static float get_max_coverage(const Sk4f& xs, const Sk4f& ys, const SkPoint& p) {
    static bool allOn[4] = {true, true, true, true};
    SkPoint corners[4] = {{xs[0],ys[0]}, {xs[1],ys[1]}, {xs[3],ys[3]}, {xs[2],ys[2]}};
    return get_area_coverage(allOn, corners, p);
}
...
static int get_shape_sk4f(SkCanvas* canvas, const bool edgeAA[4], const SkPoint corners[4],
    ...
    if (useFastPath) {
        ...
        // Make sure normals point into the shape
        bool flipped = false;
        // FIXME shouldn't probably test against CW. Should test against opposite corner. When testing
        // against CW, we may get invalid results when the edge is empty. (dx,dy) already have moved
        // the
        Sk4f test = fma(dy, nextCW(xs), fma(-dx, nextCW(ys), c));
        if ((test < -1e-2f).anyTrue()) {
            // Flip
            a = -dy;
            b = dx;
            c = -c;
        ...
        if ((dists1 >= 1e-2f).allTrue() && (dists2 >= 1e-2f).allTrue()) {
            ...
            px = 0.25f * (xs[0] + xs[1] + xs[2] + xs[3]);
            py = 0.25f * (ys[0] + ys[1] + ys[2] + ys[3]);
            float maxCoverage = get_max_coverage(xs, ys, {px[0], py[0]});
            ...
            if (dists1[2] < 1e-2f && dists1[3] < 1e-2f) {
                ...
                float mc02 = get_max_coverage(xs, ys, {px[0], py[0]});
                float mc13 = get_max_coverage(xs, ys, {px[1], py[1]});
                ...
                float mc01 = get_max_coverage(xs, ys, {px[0], py[0]});
                float mc23 = get_max_coverage(xs, ys, {px[1], py[1]});
            ...
            if (eDenom[0] > 1e-3f || eDenom[0] < -1e-3f) {
                px = (dists2 < -1e-4f).thenElse(ex[0], px);
                py = (dists2 < -1e-4f).thenElse(ey[0], py);
            }
            if (eDenom[1] > 1e-3f || eDenom[1] < -1e-3f) {
                px = (dists1 < -1e-4f).thenElse(ex[1], px);
                py = (dists1 < -1e-4f).thenElse(ey[1], py);
            }
            pts = 3;
...
/*
static float get_area_coverage_sk4f(const Sk4f& xs, const Sk4f& ys, const Sk4f& a, const Sk4f& b, const Sk4f& c, bool flipped, const SkPoint& pixelCenter) {
    static const int kCCW[] = {0, 1, 3, 2}; // Ordering of vertices given default tri-strip that produces CCW
    static const int kFlippedCCW[] = {0, 2, 3, 1}; // Ordering of vertices given inverted tri-strip that produces CCW

    bool log = pixelCenter.fX == 1.5f && pixelCenter.fY == 1.5f;
    ...
    // Whether or not the 4 corners of the pixel are inside the quad geometry
    bool tlInside = ((a * left + b * top + c) >= 0.f).allTrue();
    bool blInside = ((a * left + b * bot + c) >= 0.f).allTrue();
    bool brInside = ((a * right + b * bot + c) >= 0.f).allTrue();
    bool trInside = ((a * right + b * top + c) >= 0.f).allTrue();

    // FIXME after debugging, do optimization that says if all 4 are true, just return 1.f

    if (log) {
    SkDebugf("Pixel center: [%.2f %.2f]\n", pixelCenter.fX, pixelCenter.fY);
    SkDebugf("Quad: %d [%.3f %.3f %.3f %.3f], [%.3f %.3f %.3f %.3f]\n", flipped,
            xs[0], xs[1], xs[2], xs[3], ys[0], ys[1], ys[2], ys[3]);
    SkDebugf("Corners: tl %d, bl %d, br %d, tr %d\n", tlInside, blInside, brInside, trInside);
}
    // Add a point to the intersection hull, remembering first point (for closing) and the
    // current point, as well as the running area total
    ...
    for (int j = 0; j < 4; ++j) {
        ...
        if (log) {
            SkDebugf("Moving from p%d to p%d\n", i, ni);
            SkDebugf("  left data: %d -> %d at %.3f\n", leftValid[i], leftValid[ni], leftCross[ei]);
            SkDebugf("  right data: %d -> %d at %.3f\n", rightValid[i], rightValid[ni], rightCross[ei]);
            SkDebugf("  top data: %d -> %d at %.3f\n", topValid[i], topValid[ni], topCross[ei]);
            SkDebugf("  bot data: %d -> %d at %.3f\n", botValid[i], botValid[ni], botCross[ei]);
        }
        ...
        // the corner, which is why the two axes are separated, since left/right outside->inside
        // crossings are exclusive; same with top/bottom.

        if (!leftValid[i] && leftValid[ni]) {
            // Crossed into valid side of left edge. Calculate intersection point and if it is
            // within pixel side limits, include it in the boundary's list of points
            if (leftCross[ei] >= top && leftCross[ei] <= bot) {
                accumulate({left, leftCross[ei]});
                addedIntersection = true;
            }
            crossedEdges = true;
        } else if (!rightValid[i] && rightValid[ni]) {
            // Ditto but for right
            if (rightCross[ei] >= top && rightCross[ei] <= bot) {
                accumulate({right, rightCross[ei]});
                addedIntersection = true;
            }
            crossedEdges = true;
        }
        if (!topValid[i] && topValid[ni]) {
            // Ditto but for top
            if (topCross[ei] >= left && topCross[ei] <= right) {
                accumulate({topCross[ei], top});
                addedIntersection = true;
            }
            crossedEdges = true;
        } else if (!botValid[i] && botValid[ni]) {
            // Ditto but for bottom
            if (botCross[ei] >= left && botCross[ei] <= right) {
                accumulate({botCross[ei], bot});
                addedIntersection = true;
            }
            crossedEdges = true;
        }

        // Next check if there are any inside -> outside edge crossings. These are checked 2nd
        // to preserve the CCW winding of the intersection boundary.
        if (leftValid[i] && !leftValid[ni]) {
            if (leftCross[ei] >= top && leftCross[ei] <= bot) {
                accumulate({left, leftCross[ei]});
                addedIntersection = true;
        ...
        if (crossedEdges && !addedIntersection) {
            // Don't really care about which way the edge was crossed here, just that it was crossed
            // For a crossed edge, test the attached corner whose other side is currently invalid or crossed.
            // This can lead to repeated points, but those just accumulate zero area

            // FIXME we do actually care, because if we add two corners, we need to still keep the CCW
            // ordering preserved. In order to do that, we need to handle outside->inside edge-related
            // corners first and then do the inside->outside corner crossing.
            if (leftValid[i] ^ leftValid[ni]) {
                if (!topValid[i] || !topValid[ni]) {
                    // Test TL
                    if (tlInside) {
            ...
            if (rightValid[i] ^ rightValid[ni]) {
                if (!topValid[i] || !topValid[ni]) {
                    // Test TR
                    if (trInside) {
                        accumulate({right, top});
                    }
                } else if (!botValid[i] || !botValid[ni]) {
                    // Test BR
                    if (brInside) {
                        accumulate({right, bot});
                    }
                }
            }
            if (topValid[i] ^ topValid[ni]) {
    ...
    if (intersected) {
        ...
        if (log)
        SkDebugf("Final area: %.4f\n", -0.5f * area);
        return -0.5f * area;
    } else {
        if (log)
        SkDebugf("No intersection\n");
        return 0.f;
    }
}
*/
