...
#
# Generate Android.gen.bp and various SkUserConfig.h for Android Framework
# using Skia's GN configuration.
#
# To test this locally, run python3 gn/gn_to_bp.py --out /tmp/somewhere
#
...
parser = argparse.ArgumentParser(description='Generate Android.gen.bp for Skia.',
                                 formatter_class=argparse.RawDescriptionHelpFormatter,
                                 epilog='''
Example usage:
  python3 gn/gn_to_bp.py --out /tmp/skia_android_gen_bp
''')
parser.add_argument('--gn', default='gn', help='Path to gn executable.')
parser.add_argument('--out', default='.', help='Output directory.')
args = parser.parse_args()

def out_path(p):
  return os.path.join(args.out, p)

# First we start off with a template for Android.gen.bp,
...
// Android Internal. The autoroller will then create the updated Android.gen.bp
...
def generate_args(target_os, enable_gpu, libskia_variant = LibSkiaVariant.HWUI):
  ...
  if target_os == '"win"':
    ...
    # we're just creating Android.gen.bp, so we don't need them. Populate with
...
def main():
  ...
  def strip_slashes(lst):
    return {p.lstrip('/') for p in lst if not p.startswith(build_dir)}
  ...
  skqp_sdk_version = 26
  js_skqp = gn_to_bp_utils.GenerateJSONFromGN(args.gn, skqp_gn_args.GetGNArgs(api_level=skqp_sdk_version,
                                                                     debug=False,
                                                                     is_android_gen_bp=True))
  ...
  shutil.copy('third_party/externals/vulkanmemoryallocator/LICENSE.txt', out_path('vma_android/'))
  ...
  write_config(out_path('mac/include/config/SkUserConfig.h'),   mac_defines, 'MAC')
  write_config(out_path('win/include/config/SkUserConfig.h'),   win_defines, 'WIN')

  # OK!  We have everything to fill in Android.gen.bp...
  with open(out_path('Android.gen.bp'), 'w') as Android_gen_bp:
    print(bp.substitute({
