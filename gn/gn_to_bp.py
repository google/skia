...
bench_srcs      = strip_slashes(js['targets']['//:nanobench']['sources'])
bench_includes  = strip_slashes(js['targets']['//:nanobench']['include_dirs'])

def GrabDependentSrcs(name, srcs_to_extend, exception):
  # Grab the sources from targets name depends on (optional Skia components).
  for dep in js['targets'][name]['deps']:
    ...
    if exception and exception in dep:
      continue
...
GrabDependentSrcs('//:nanobench', bench_srcs, 'skia')

# No need to list headers.
srcs        = [s for s in srcs       if not s.endswith('.h')]
dm_srcs     = [s for s in dm_srcs    if not s.endswith('.h')]
bench_srcs  = [s for s in bench_srcs if not s.endswith('.h')]
...
with open('Android.bp', 'w') as f:
  print >>f, bp.substitute({
    ...
    'tool_cppflags'     : bpfmt(8, tool_cppflags),
    ...
    'bench_includes'    : bpfmt(8, bench_includes),
    'bench_srcs'        : bpfmt(8, bench_srcs),
  })
