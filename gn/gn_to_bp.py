...
def generate_args(target_os, enable_gpu, mini = False):
  d = {
  ...
  if target_os == '"android"' and not mini:
  ...
  if mini:
    ...
    d['skia_use_libpng_encode'] = 'false'
...
gn_args_mini  = generate_args('"android"', True, True)
...
js_mini         = gn_to_bp_utils.GenerateJSONFromGN(gn_args_mini)
mini_srcs       = strip_slashes(js_mini['targets']['//:skia']['sources'])
gn_to_bp_utils.GrabDependentValues(js_mini, '//:skia', 'sources', mini_srcs,
                                   None)
mini_srcs       = strip_headers(mini_srcs)
...
mini_defines    = get_defines(js_mini)
...
mkdir_if_not_exists('mini/include/config/')
...
write_android_config('mini/include/config/SkUserConfig.h', mini_defines)
