...
def WriteTarget(out, target, project):
  ...
  # As of cmake 3.11 add_library must have sources. If there are no sources, add empty.cpp as the
  #  file to compile.
  if target.gn_type == 'source_set' and len(sources) == 0:
    out.write('target_sources("${target}" PRIVATE empty.cpp)\n')
...
def WriteProject(project):
  ...
  out.write('file(WRITE "empty.cpp" "")\n')
