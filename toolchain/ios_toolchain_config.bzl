"""
This file specifies a clang toolchain that can run on a Mac host to build for iOS.

Hermetic toolchains still need access to Xcode for sys headers included in Skia's codebase.

See download_ios_toolchain.bzl for more details on the creation of the toolchain.
...
def _ios_toolchain_info(ctx):
    ...
    return cc_common.create_cc_toolchain_config_info(
        ...
        cxx_builtin_include_directories = [
            ...
            "%sysroot%/symlinks/xcode/iOSSDK/System/Library/Frameworks/",
        ],
...
provide_ios_toolchain_config = rule(
    attrs = _import_platform_constraints(),
    provides = [CcToolchainConfigInfo],
    implementation = _ios_toolchain_info,
)
...
def _make_action_configs():
    ...
    clang_tool = tool(path = "ios_trampolines/clang_trampoline_ios.sh")
    ar_tool = tool(path = "ios_trampolines/ar_trampoline_ios.sh")
    ...
    objcpp_exec_action = action_config(
        ...
        flag_groups = [
            flag_group(
                flags = [
                    ...
                    "-isysroot",
                    XCODE_IOSSDK_SYMLINK,
                    "-isystem",
                    EXTERNAL_TOOLCHAIN + "/include/c++/v1",
                    "-isystem",
                    XCODE_IOSSDK_SYMLINK + "/usr/include",
                    "-isystem",
                    EXTERNAL_TOOLCHAIN + "/lib/clang/15.0.1/include",
                    # Set the framework path to the iOS SDK framework directory. This has
                    ...
                    "-iframework",
                    XCODE_IOSSDK_SYMLINK + "/System/Library/Frameworks",
...
def _make_target_specific_flags(ctx):
    ios_sim_target = flag_set(
        ...
        flag_groups = [
            flag_group(
                flags = [
                    "--target=arm64-apple-ios14.5-simulator",
                ],
            ),
        ],
    )
    ...
    if _has_platform_constraint(ctx, "@platforms//cpu:arm64"):
        target_specific_features.append(
            feature(
                name = "_ios_sim_target",
                enabled = True,
                flag_sets = [ios_sim_target],
            ),
        )
